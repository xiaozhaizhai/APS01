...
<#assign viewCode = 'WOM_7.5.0.0_putInMaterial_PutInMaterial' />
<#assign propertyCode = '' />
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'taskID.taskState.value' || item == '*'>
														 <#if item?index_of('.')!=-1>  
											...
											<#if item == 'putinStaff.name' || item == '*'>
			 						        ...
			 						        <#assign customGeneratedFlag=false />
												<#if item == 'putinStaff.name' && !customGeneratedFlag >
												</#if>
			 						        <#assign customGeneratedFlag=false />
												<#if item == 'putinStaff.name' && !customGeneratedFlag >
												</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'taskID.tableNo' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.menuInfoname.randon1489670907556')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'taskID.taskState.value' || item == '*'>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'putinStaff.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.staff.dimissionStaff_xls.staffName43566')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
												<#if item == 'putinStaff.name' && !customGeneratedFlag >
												</#if>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'remark' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1490604919519')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												...
												<#if item == 'taskID.taskState.value' || item == '*'>
													<Cell>
														...
														<Data ss:Type="String">${getText('${(data.taskID.taskState.value)!}')}</Data>
