...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="proBatch.batchText">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="proBatch.id">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'proBatch.batchText' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'proBatch.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									...
									<#assign customGeneratedFlag=false />
										<#if item == 'putinEndDate' && !customGeneratedFlag >
					...
					<#if item?length gt 0>
								...
								<#if item == 'proBatch.batchText' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.proBatch.batchText)??>${(data.proBatch.batchText)!}</#if></Data>
								...
								<#if item == 'proBatch.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="Number"><#if (data.proBatch.id)??>${(data.proBatch.id)!}</#if></Data>
								...
								<#if item == 'taskActiveId.name' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.taskActiveId.name)??>${(data.taskActiveId.name)!}</#if></Data>
								...
								<#if item == 'taskActiveId.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="Number"><#if (data.taskActiveId.id)??>${(data.taskActiveId.id)!}</#if></Data>
