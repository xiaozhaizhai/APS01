...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="taskProcessID.factoryId.name">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		...
  		<Style ss:ID="taskProcessID.processState">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'taskProcessID.factoryId.name' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'taskProcessID.longTime' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'taskProcessID.processState.value' || item == 'taskProcessID.processState.id' || item == '*'>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'taskProcessID.processType.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1489720664942')}</Data>
										</Cell>
								</#if>
								<#if item == 'taskProcessID.factoryId.name' || item == '*'>
										<Cell>
								...
								<#if item == 'state.value' || item == 'state.id' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.201903271915')}</Data>
										</Cell>
								</#if>
								<#if item == 'startTime' || item == '*'>
										<Cell>
					...
					<#if item?length gt 0>
								...
								<#if item == 'taskProcessID.factoryId.name' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.taskProcessID.factoryId.name)??>${(data.taskProcessID.factoryId.name)!}</#if></Data>
								...
								<#if item == 'taskProcessID.processState.value' || item == 'taskProcessID.processState.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.taskProcessID.processState.value)!}')}</Data>
