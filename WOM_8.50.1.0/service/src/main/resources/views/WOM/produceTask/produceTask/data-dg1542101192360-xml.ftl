...
<GridData DefaultNodeExpanded="false"   dgName="dg1542101192360" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<processId.name
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(AdjustOrTempHead_dg1542101192360Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(AdjustOrTempHead_dg1542101192360Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/WOM/produceTask/produceTaskProcess/taskProcessExelogRef.action','dg1542101192360processId.name','${getText("WOM.viewtitle.randon1495700893633")}',sPTGridID,nRow,sFieldName,AdjustOrTempHead_dg1542101192360Widget,null,false,false,'','WOM_7.5.0.0_produceTask_taskProcessExelogRef')"
									readOnly="true" hideEditOnReadOnly="true"
					sort="false"  onPropertyChange="AdjustOrTempHead_dg1542101192360Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
	...
	<tbody>
		...
		<row  onClick="AdjustOrTempHead_dg1542101192360Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
