...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="taskActiveID.activeType">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="taskActiveID.exeSystem">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'taskActiveID.activeType.value' || item == 'taskActiveID.activeType.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item?split('.')[0]}"/>
									</#if>
									<#if item == 'taskActiveID.exeSystem.value' || item == 'taskActiveID.exeSystem.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item?split('.')[0]}"/>
									</#if>
									...
									<#assign customGeneratedFlag=false />
										<#if item == 'endTime' && !customGeneratedFlag >
					...
					<#if item?length gt 0>
								...
								<#if item == 'taskActiveID.activeType.value' || item == 'taskActiveID.activeType.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.taskActiveID.activeType.value)!}')}</Data>
								...
								<#if item == 'taskActiveID.exeSystem.value' || item == 'taskActiveID.exeSystem.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.taskActiveID.exeSystem.value)!}')}</Data>
