...
<#assign viewCode = 'WOM_7.5.0.0_produceTask_TaskBeforeProcess' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="processId.longTime">
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'processId.longTime' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'processId.remark' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'processId.longTime' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1489720689233')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'processId.remark' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1489720730887')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
							...
							<Row ss:AutoFitHeight="0">
								<#list exportFieldList as item>
									<#if item?length gt 0>
												<#if item == 'processId.name' || item == '*'>
													<Cell>
													<#if checkFieldPermission('processId.name','WOM_7.5.0.0_produceTask_TaskBeforeProcess','WOM_7.5.0.0_produceTask_TaskBeforeProcess_processId||WOM_7.5.0.0_produceTask_ProduceTaskProcess_name') != 0>
														<#if data.processId.name??><Data ss:Type="String"><![CDATA[${(data.processId.name)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
												<#if item == 'processId.longTime' || item == '*'>
													<Cell>
													<#if checkFieldPermission('processId.longTime','WOM_7.5.0.0_produceTask_TaskBeforeProcess','WOM_7.5.0.0_produceTask_TaskBeforeProcess_processId||WOM_7.5.0.0_produceTask_ProduceTaskProcess_longTime') != 0>
														<#if (data.processId.longTime)?has_content><Data ss:Type="Number">#{(data.processId.longTime)!; m6M6}</Data></#if>
													</#if>
													</Cell>
												</#if>
												<#if item == 'processId.remark' || item == '*'>
													<Cell>
													<#if checkFieldPermission('processId.remark','WOM_7.5.0.0_produceTask_TaskBeforeProcess','WOM_7.5.0.0_produceTask_TaskBeforeProcess_processId||WOM_7.5.0.0_produceTask_ProduceTaskProcess_remark') != 0>
														<#if data.processId.remark??><Data ss:Type="String"><![CDATA[${(data.processId.remark)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
								...
								<#if item??>
									<#if item.name?index_of(".")!=-1>
											...
											<Cell  ss:StyleID="${item.styleid}" >
												<#if checkFieldPermission('${(item.name)!}','WOM_7.5.0.0_produceTask_TaskBeforeProcess','${(item.propertyCode)!}') != 0>
