...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="activeDealID.activeName">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="activeDealID.activeType">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="activeDealID.product.productCode">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="activeDealID.product.productName">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									<#if item == 'activeDealID.activeName' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'activeDealID.activeType.value' || item == 'activeDealID.activeType.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item?split('.')[0]}"/>
									</#if>
									<#if item == 'activeState.value' || item == 'activeState.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item?split('.')[0]}"/>
									</#if>
									<#if item == 'activeDealID.product.productCode' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'activeDealID.product.productName' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'activeState.value' || item == 'activeState.id' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1542098169000')}</Data>
										</Cell>
								</#if>
								<#if item == 'activeDealID.product.productCode' || item == '*'>
										<Cell>
					...
					<#if item?length gt 0>
								<#if item == 'activeDealID.activeName' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.activeDealID.activeName)??>${(data.activeDealID.activeName)!}</#if></Data>
								...
								<#if item == 'activeDealID.activeType.value' || item == 'activeDealID.activeType.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.activeDealID.activeType.value)!}')}</Data>
								...
								<#if item == 'activeDealID.product.productCode' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.activeDealID.product.productCode)??>${(data.activeDealID.product.productCode)!}</#if></Data>
								...
								<#if item == 'activeDealID.product.productName' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.activeDealID.product.productName)??>${(data.activeDealID.product.productName)!}</#if></Data>
