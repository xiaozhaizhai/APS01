...
<#assign viewCode = 'WOM_7.5.0.0_materialReject_RejectReason' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="isChangeNeed">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="rejectReson">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'isChangeNeed' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1543218766334')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'demo' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1543218754563')}</Data></Cell>
											</#if>
									...
									<#if item?length gt 0>
												<#if item == 'rejectReson' || item == '*'>
													<Cell>
														...
														<#if data.rejectReson??><Data ss:Type="String"><![CDATA[${(data.rejectReson)!}]]></Data></#if>
												...
												<#if item == 'isChangeNeed' || item == '*'>
													<Cell>
														...
														<Data ss:Type="String"><#if data.isChangeNeed?? && data.isChangeNeed>${getText('common.radio.true')}<#else>${getText('common.radio.false')}</#if></Data>
