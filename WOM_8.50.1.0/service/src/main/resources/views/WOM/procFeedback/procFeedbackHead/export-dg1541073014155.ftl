...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="standingCrop.manulOrder.manulOrderNum">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'standingCrop.manulOrder.manulOrderNum' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
					...
					<#if item?length gt 0>
								...
								<#if item == 'standingCrop.manulOrder.manulOrderNum' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.standingCrop.manulOrder.manulOrderNum)??>${(data.standingCrop.manulOrder.manulOrderNum)!}</#if></Data>
