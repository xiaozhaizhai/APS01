...
<GridData DefaultNodeExpanded="false"  MultiSelect="checkbox"  dgName="dg1542186009188" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<craftName
					sort="false"  onPropertyChange="CraftStandards_dg1542186009188Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
	...
	<tbody>
		...
		<row  onClick="CraftStandards_dg1542186009188Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
			<#if cpObjHideKey?? && cpObjHideKey?has_content>
