...
<GridData DefaultNodeExpanded="false"   dgName="dg1490776543260" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<quality
					...
					sort="false"  onPropertyChange="TaskReportingPart_dg1490776543260Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<storeID.placeSetCode
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(TaskReportingPart_dg1490776543260Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(TaskReportingPart_dg1490776543260Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/storeSet/storeSet/storeRef.action','dg1490776543260storeID.placeSetCode','${getText("MESBasic.viewtitle.randon1489735291062")}',sPTGridID,nRow,sFieldName,TaskReportingPart_dg1490776543260Widget,null,false,false,'','MESBasic_1_storeSet_storeRef')"
					...
					showFormat="SELECTCOMP" 
						type="SELECTCOMP" 
					width="120">货位编码</storeID.placeSetCode>
				<#assign isRef = false >
				...
				<#list customPropViewMappings as cpvm>
					...
					<#if cpvm.property.type == 'OBJECT'>
						...
						<#if cpvm.refView??>
							...
							onFocus="CUI.toggleClearBtn_DG_IE(TaskReportingPart_dg1490776543260Widget,nRow,sFieldName)"
							onBtnClick="CUI.clearCellValue_DG_IE(TaskReportingPart_dg1490776543260Widget,nRow,sFieldName,true)"
							subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('${viewUrl!}','dg1490776543260${cpvm.propertyLayRec}','${getText("${viewTitle}")}',sPTGridID,nRow,'${cpvm.propertyLayRec}',TaskReportingPart_dg1490776543260Widget,null,false,true,'${mainDisplayName},${pkName}','${viewCode}')"
							hideEditOnReadOnly="true"
						</#if>
					</#if>
						sort="false"
						onPropertyChange="TaskReportingPart_dg1490776543260Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
					...
					<#elseif cpvm.property.type == 'DATETIME'>
						...
						onClick="initCalender(TaskReportingPart_dg1490776543260Widget,'TaskReportingPart_dg1490776543260Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						onBtnClick="showCalender(TaskReportingPart_dg1490776543260Widget,'TaskReportingPart_dg1490776543260Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						control="Text"
	...
	<tbody>
		...
		<row  onClick="TaskReportingPart_dg1490776543260Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
					<c>${(row.storeID.id)!}</c>
			<#if cpObjHideKey?? && cpObjHideKey?has_content>
