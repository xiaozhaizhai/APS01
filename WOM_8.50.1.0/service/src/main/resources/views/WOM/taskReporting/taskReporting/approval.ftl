...
<@s.form id="WOM_taskReporting_taskReporting_approval_form" cssStyle="height:100%;"  ecform="true">
							...
							<#list transitions as t>
								...
								<#if t.selectPeople??&&t.selectPeople gt 0>
										...
										<#elseif t.selectPeople==3>
											<@mneclient reftitle="${getText('ec.edit.refStaff')}" mneTip="${getText('ec.edit.assignStaff')}" isWrap=true multiDivStyle="height:25px;overflow:hidden;" conditionfunc="WOM.taskReporting.taskReporting._querycustomFunc('assignStaffSelect_${t.name}')"  name="assignStaffSelect_${t.name}" id="assignStaffSelect_${t.name}" type="userRange" url="/foundation/user/common/userListGroupFrameset.action" displayFieldName="staffname"  ids="" names=""  onkeyupfuncname="getassignStaffSelect_${t.name}MultiInfo()" funcparam="unassignUserSupport=true&multiSelect=true" clicked=true multiple=true mnewidth=260 isEdit=true selectPeople="${t.selectPeople}" outcome="${t.name}"/>
										<#elseif t.selectPeople==4>
											<@mneclient reftitle="${getText('ec.edit.refStaff')}" mneTip="${getText('ec.edit.assignStaff')}" isWrap=true multiDivStyle="height:25px;overflow:hidden;" conditionfunc="WOM.taskReporting.taskReporting._querycustomFunc('assignStaffSelect_${t.name}')"  name="assignStaffSelect_${t.name}" id="assignStaffSelect_${t.name}" type="userRange" url="/foundation/user/common/userListGroupFrameset.action" displayFieldName="staffname"  ids="" names=""  onkeyupfuncname="getassignStaffSelect_${t.name}MultiInfo()" funcparam="unassignUserSupport=true&multiSelect=true" clicked=true multiple=true mnewidth=260 isEdit=true selectPeople="${t.selectPeople}" outcome="${t.name}"/>
										<#elseif t.selectPeople==5>
											<@mneclient reftitle="${getText('ec.edit.refStaff')}" mneTip="${getText('ec.edit.assignStaff')}" isWrap=true multiDivStyle="height:25px;overflow:hidden;" conditionfunc="WOM.taskReporting.taskReporting._querycustomFunc('assignStaffSelect_${t.name}')"  name="assignStaffSelect_${t.name}" id="assignStaffSelect_${t.name}" type="userRange" url="/foundation/user/common/groupUserList.action" displayFieldName="staffname"  ids="" names=""  onkeyupfuncname="getassignStaffSelect_${t.name}MultiInfo()" funcparam="unassignUserSupport=true&multiSelect=true" clicked=true multiple=true mnewidth=260 isEdit=true deploymentId="${deploymentId!''}" selectPeople="${t.selectPeople}" outcome="${t.name}"/>
										</#if>
										<#-- 
										<#if t.selectPeople==1>
...
<script type="text/javascript">
	//批量处理提交
	WOM.taskReporting.taskReporting.approval=function(){
		CUI.Dialog.toggleAllButton();
		if(!WOM.taskReporting.taskReporting.validate()){
			CUI.Dialog.toggleAllButton();
			return false;
		}
		var outcomeStr="";
		...
		$('input[id*="__workflow_outcome_nodification_"]:checked').each(function(){
   		...
   		var url="/WOM/taskReporting/taskReporting/bulkSubmit.action";
   		CUI.ajax({
		type : "POST",
		url : url,
		...
		success : function(msg){
			try{
				if(msg.dealSuccessFlag==true) {
					...
					setTimeout(function(){
					try{WOM.taskReporting.taskReporting.taskReportingList.AppDlg.close();}catch(e1){}
	...
	//页面加载完成后设置textarea高度
	$(function(){
			var dialogH = WOM.taskReporting.taskReporting.taskReportingList.AppDlg._config.height-60-${rowlen}*20;
			if(dialogH<100){
	...
	//验证
	WOM.taskReporting.taskReporting.validate = function(){
