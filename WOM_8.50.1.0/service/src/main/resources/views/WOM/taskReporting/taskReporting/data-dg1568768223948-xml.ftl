...
<GridData DefaultNodeExpanded="false"   dgName="dg1568768223948" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<batchNum
					...
					sort="false"  onPropertyChange="TaskReportingPart_dg1568768223948Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<wareId.wareName
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(TaskReportingPart_dg1568768223948Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(TaskReportingPart_dg1568768223948Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/wareMan/ware/wareRef.action','dg1568768223948wareId.wareName','${getText("MESBasic.viewtitle.randon1484031981880")}',sPTGridID,nRow,sFieldName,TaskReportingPart_dg1568768223948Widget,null,false,false,'','MESBasic_1_wareMan_wareRef')"
				...
				<storeID.placeSetCode
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(TaskReportingPart_dg1568768223948Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(TaskReportingPart_dg1568768223948Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/storeSet/storeSet/storeRef.action','dg1568768223948storeID.placeSetCode','${getText("MESBasic.viewtitle.randon1489735291062")}',sPTGridID,nRow,sFieldName,TaskReportingPart_dg1568768223948Widget,null,false,false,'','MESBasic_1_storeSet_storeRef')"
									readOnly="true" hideEditOnReadOnly="true"
					sort="false"  onPropertyChange="TaskReportingPart_dg1568768223948Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
	...
	<tbody>
		...
		<row  onClick="TaskReportingPart_dg1568768223948Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
					<c>${(row.wareId.id)!}</c>
					<c>${(row.storeID.id)!}</c>
