...
<#assign viewCode = 'WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="formulaId.applyDate">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="formulaId.applyStaff.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="formulaId.tableNo">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="formulaId.wareID.wareCode">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="formulaId.wareID.wareName">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		...
	  		<Style ss:ID="storeID.wareId.cargoStatue">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'formulaId.tableNo' || item == '*'>
											...
											<#if item == 'formulaId.applyStaff.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'formulaId.applyDate' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'prepareNum' || item == '*'>
											...
											<#if item == 'formulaId.wareID.wareCode' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'formulaId.wareID.wareName' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'storeID.wareId.cargoStatue.value' || item == '*'>
														 <#if item?index_of('.')!=-1>  
											...
											<#if item == 'storeID.placeSetName' || item == '*'>
			 						        ...
			 						        <#assign customGeneratedFlag=false />
												<#if item == 'storeID.placeSetName' && !customGeneratedFlag >
												</#if>
											<#if item == 'remark' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'formulaId.applyStaff.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.dimissionStaff_xls.staffNamesqr')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'formulaId.applyDate' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1492392059136')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'prepareNum' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon14899771198901112')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'hasDoneNum' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertyshowName.randon1569031657847.flag')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'formulaId.wareID.wareCode' || item == '*'>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'remark' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1489977145770')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												<#if item == 'formulaId.tableNo' || item == '*'>
													<Cell>
														...
														<#if data.formulaId.tableNo??><Data ss:Type="String"><![CDATA[${(data.formulaId.tableNo)!}]]></Data></#if>
												...
												<#if item == 'formulaId.applyStaff.name' || item == '*'>
													<Cell>
														...
														<#if data.formulaId.applyStaff.name??><Data ss:Type="String"><![CDATA[${(data.formulaId.applyStaff.name)!}]]></Data></#if>
												...
												<#if item == 'formulaId.applyDate' || item == '*'>
													<Cell ss:StyleID="s8">
														...
														<#if data.formulaId.applyDate??><Data ss:Type="DateTime">${(data.formulaId.applyDate)!?string("yyyy-MM-dd'T'HH:mm:ss.SSS")}</Data></#if>
												...
												<#if item == 'prepareNum' || item == '*'>
													<Cell>
														...
														<#if (data.prepareNum)?has_content><Data ss:Type="Number">#{(data.prepareNum)!; m6M6}</Data></#if>
												...
												<#if item == 'formulaId.wareID.wareCode' || item == '*'>
													<Cell>
														...
														<#if data.formulaId.wareID.wareCode??><Data ss:Type="String"><![CDATA[${(data.formulaId.wareID.wareCode)!}]]></Data></#if>
												...
												<#if item == 'formulaId.wareID.wareName' || item == '*'>
													<Cell>
														...
														<#if data.formulaId.wareID.wareName??><Data ss:Type="String"><![CDATA[${(data.formulaId.wareID.wareName)!}]]></Data></#if>
												...
												<#if item == 'storeID.wareId.cargoStatue.value' || item == '*'>
													<Cell>
														...
														<Data ss:Type="String">${getText('${(data.storeID.wareId.cargoStatue.value)!}')}</Data>
