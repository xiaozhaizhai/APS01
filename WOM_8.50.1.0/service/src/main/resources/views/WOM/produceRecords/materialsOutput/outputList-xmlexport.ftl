...
<#assign viewCode = 'WOM_7.5.0.0_produceRecords_MaterialsOutput' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="activeID.activeId.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="batch">
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'activeID.activeId.name' || item == '*'>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'batch' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1541146730158')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'produceNum' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1541146740613')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												<#if item == 'activeID.activeId.name' || item == '*'>
													<Cell>
														...
														<#if data.activeID.activeId.name??><Data ss:Type="String"><![CDATA[${(data.activeID.activeId.name)!}]]></Data></#if>
												...
												<#if item == 'batch' || item == '*'>
													<Cell>
														...
														<#if data.batch??><Data ss:Type="String"><![CDATA[${(data.batch)!}]]></Data></#if>
												...
												<#if item == 'produceNum' || item == '*'>
													<Cell>
														...
														<#if (data.produceNum)?has_content><Data ss:Type="Number">#{(data.produceNum)!; m6M6}</Data></#if>
