...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="armStoreID.id">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="armWareID.id">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'armWareID.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'productId.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'armStoreID.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
 						<#assign count = count + 1>
						</#if>
					...
					<#if item?length gt 0>
								...
								<#if item == 'armWareID.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="Number"><#if (data.armWareID.id)??>${(data.armWareID.id)!}</#if></Data>
								...
								<#if item == 'armStoreID.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="Number"><#if (data.armStoreID.id)??>${(data.armStoreID.id)!}</#if></Data>
