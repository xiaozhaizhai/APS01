...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="taskActiveId.planNum">
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'taskActiveId.planNum' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'taskActiveId.planNum' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1489730174515')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'doneNum' || item == '*'>
									...
									<#if item?length gt 0>
												...
												<#if item == 'taskActiveId.planNum' || item == '*'>
													<Cell>
														...
														<#if (data.taskActiveId.planNum)?has_content><Data ss:Type="Number">#{(data.taskActiveId.planNum)!; m6M6}</Data></#if>
