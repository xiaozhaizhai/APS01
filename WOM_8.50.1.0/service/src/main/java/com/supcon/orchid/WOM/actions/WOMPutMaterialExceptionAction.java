...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_putInMaterialException_PutMaterialException,WOM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.services.ConsulService;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMPutMaterialExceptionAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMPutMaterialException> {
	...
	private WOMPutMaterialException putMaterialException;
	...
	//导出excel时,主模型内容
	List<WOMPutMaterialException> queryResults = new ArrayList<WOMPutMaterialException>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/putInMaterialException/putMaterialException/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/putInMaterialException/putMaterialException/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_putInMaterialException_PutMaterialException" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMPutMaterialException> putMaterialExceptions;
	
	public List<WOMPutMaterialException> getPutMaterialExceptions() {
		return putMaterialExceptions;
	}
	...
	private Page<WOMPutMaterialException> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			putMaterialException = new WOMPutMaterialException();
		} else {
			if(id > 0){
				putMaterialException = putMaterialExceptionService.getPutMaterialException(id, viewCode);
				if(tableInfoId == null && putMaterialException != null){
					tableInfoId = putMaterialException.getTableInfoId();
				}
			}
		}
	...
	public Page<WOMPutMaterialException> getPage() {
		if (null == page) {
			page = new Page<WOMPutMaterialException>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMPutMaterialException getPutMaterialException() {
		return putMaterialException;
	}		

	public void setPutMaterialException(WOMPutMaterialException putMaterialException) {
		this.putMaterialException = putMaterialException;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/putInMaterialException/putMaterialException/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = putMaterialExceptionService.getPutMaterialExceptionAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/putInMaterialException/putMaterialException/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String putMaterialExceptionReferenceCopy() throws Exception{
			WOMPutMaterialException oldPutMaterialException = null;
			if(isEditNew()){
				...
				oldPutMaterialException = new WOMPutMaterialException();
			}else{
			//WOMPutMaterialException putMaterialException = putMaterialExceptionService.getPutMaterialException(this.putMaterialException.getId());
			this.creatorInfo = creatorService.get(putMaterialException.getCreateStaffId(), putMaterialException.getCreatePositionId(), putMaterialException.getCreateDepartmentId());
			//setPutMaterialException(putMaterialException);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(putMaterialException.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,putMaterialException.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, putMaterialException.getId());
				...
				oldPutMaterialException = putMaterialExceptionService.getPutMaterialException(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldPutMaterialException.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_putInMaterialException_" + oldPutMaterialException.getId();
			...
			putMaterialException = putMaterialExceptionService.getPutMaterialException(refId);
			putMaterialException.setPending(pending);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterialException_PutMaterialException");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMPutMaterialException.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPutMaterialException, null);
						Method setValue = WOMPutMaterialException.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(putMaterialException, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			putMaterialException.setCreateDepartmentId(oldPutMaterialException.getCreateDepartmentId());
			putMaterialException.setCreatePositionId(oldPutMaterialException.getCreatePositionId());
			putMaterialException.setCreateStaffId(oldPutMaterialException.getCreateStaffId());
			putMaterialException.setOwnerDepartmentId(oldPutMaterialException.getOwnerDepartmentId());
			putMaterialException.setOwnerPositionId(oldPutMaterialException.getOwnerPositionId());
			putMaterialException.setOwnerStaffId(oldPutMaterialException.getOwnerStaffId());
			putMaterialException.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "referenceCopy");
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "putinExceptionEdit.ftl") }),
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "putinExceptionEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=putMaterialExceptionService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return putMaterialExceptionReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			putMaterialException = putMaterialExceptionService.getPutMaterialException(this.putMaterialException.getId(), viewCode);
			this.creatorInfo = creatorService.get(putMaterialException.getCreateStaffId(), putMaterialException.getCreatePositionId(), putMaterialException.getCreateDepartmentId());
			//setPutMaterialException(putMaterialException);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = putMaterialExceptionService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = putMaterialExceptionService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + putMaterialException.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_putInMaterialException_" + putMaterialException.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<WOMPutMaterialException> tmpList = new ArrayList<>();
			tmpList.add(putMaterialException);
			putMaterialExceptionService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = putMaterialExceptionService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,putMaterialException.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			putMaterialExceptionService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "putinExceptionView.ftl") }),
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "putinExceptionViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionList", results = { @Result(type = FREEMARKER, location = "putinExceptionList.ftl") })
	})
	public String viewList() throws Exception{
		...
		//mainViewPath = "/WOM/putInMaterialException/putMaterialException/putinExceptionView.action";
		mainViewPath=putMaterialExceptionService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = putMaterialExceptionService.getWorkFlowInfo(workFlowMenuCode);
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = putMaterialExceptionService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(putMaterialExceptionService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(putMaterialExceptionService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_putInMaterialException_PutMaterialException", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.putinID.tableNo,result.putinPartID.productId.productCode,result.putinPartID.productId.productName,result.oldBatchNumStr,result.batchNumStr,result.oldUseNum,result.useNum,result.oldLossNum,result.lossNum,result.createTime,result.remark,result.putinID.id,result.putinPartID.productId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="putinExceptionList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"putMaterialException\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"putMaterialException\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			putMaterialExceptionService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<WOMPutMaterialException> putMaterialExceptionList=page.getResult();
				for(WOMPutMaterialException lz:putMaterialExceptionList){
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.putinID.tableNo,result.putinPartID.productId.productCode,result.putinPartID.productId.productName,result.oldBatchNumStr,result.batchNumStr,result.oldUseNum,result.useNum,result.oldLossNum,result.lossNum,result.createTime,result.remark,result.putinID.id,result.putinPartID.productId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"putMaterialException\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"putMaterialException\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"putMaterialException\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	 ...
	 * @return
	 */
	@Action(value = "/WOM/putInMaterialException/putMaterialException/allTempleteDownload", results = { @Result(name="excel",type="excel",location="putinExceptionList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterialException_PutMaterialException");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		putMaterialExceptionService.findDataGridPage(dg,dgclass,getDgPage(),putMaterialException,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(putMaterialException.getId() != null && putMaterialException.getId() != -1){
				putMaterialException = putMaterialExceptionService.getPutMaterialException(putMaterialException.getId());
			}
			putMaterialExceptionService.findDataGridPage(dg,dgclass,getDgPage(),putMaterialException,searchCondition,params);
	...
	private void save() throws Exception {
		...
		putMaterialExceptionService.save(putMaterialException,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(putMaterialException.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + putMaterialException.getProcessKey() + "_" + p.getActivityName() + "_WOM_7.5.0.0_putInMaterialException_" + putMaterialException.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(putMaterialException.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/WOM/putInMaterialException/putMaterialException/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		putMaterialExceptionService.retrialThisTable(putMaterialException,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "retrial");
		return SUCCESS;
	}
	@Actions({
		@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/putInMaterialException/putMaterialException/putinExceptionView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "submit");
		bap_validate_datagrids=putMaterialExceptionService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (putMaterialException != null && putMaterialException.getStatus() != null && putMaterialException.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				putMaterialExceptionService.saveSuperEditPutMaterialException(putMaterialException, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				putMaterialExceptionService.submit(putMaterialException,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", putMaterialException.getTableInfoId());
		responseMap.put("id", putMaterialException.getId());
		...
		fileuploads.put("linkId", putMaterialException.getTableInfoId());
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/putInMaterialException/putMaterialException/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "bulkSubmit");
		putMaterialExceptionService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/WOM/putInMaterialException/putMaterialException/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		putMaterialExceptionService.deletePutMaterialException(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterialException/putMaterialException/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "restore");
		putMaterialExceptionService.restorePutMaterialException(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/putInMaterialException/putMaterialException/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/putInMaterialException/putMaterialException/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "showDealInfoData");
		if (null != tableInfoId) {
			int count=putMaterialExceptionService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = putMaterialExceptionService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = putMaterialExceptionService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMPutMaterialException> findExportDataInfos(Page<WOMPutMaterialException> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = putMaterialExceptionService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = putMaterialExceptionService.getAssProperty(p.getCode());	
									//String assModelCode = putMaterialExceptionService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			putMaterialExceptionService.findPutMaterialExceptions(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			putMaterialExceptionService.findPutMaterialExceptions(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMPutMaterialException> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMPutMaterialException result : results){
	...
	public boolean isEditNew(){
		//return null == this.putMaterialException || null == this.putMaterialException.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && putMaterialException != null && putMaterialException.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + putMaterialException.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return putMaterialExceptionService.isRecallAble(tableInfoId);
	}

	@Action(value = "/WOM/putInMaterialException/putMaterialException/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = putMaterialExceptionService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + putMaterialException.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + putMaterialException.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/putInMaterialException/putMaterialException/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "batchApproval");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/putInMaterialException/putMaterialException/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/putInMaterialException/putMaterialException/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/putInMaterialException/putMaterialException/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterialException/putMaterialException/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMPutMaterialExceptionService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_putInMaterialException", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_putInMaterialException");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterialException/putMaterialException/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_putInMaterialException_PutMaterialException", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_putInMaterialException_PutMaterialException", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/putInMaterialException/putMaterialException/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/putInMaterialException/putMaterialException/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_putInMaterialException_PutMaterialException";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterialException_PutMaterialException", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/putInMaterialException/putMaterialException/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		if(null != id) {
			putMaterialException = putMaterialExceptionService.getPutMaterialException(id);
		}
		...
		_auditLog.setColumnStringB(null == putMaterialException.getTableNo() ? "" : putMaterialException.getTableNo().toString());
		_auditLog.setColumnStringA(null == putMaterialException.getId() ? "" : putMaterialException.getId().toString());
		_auditLog.setColumnStringC(putMaterialException.getId().toString());
		_auditLog.setColumnStringD(WOMPutMaterialException.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_putInMaterialException");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMPutMaterialExceptionAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, putMaterialException,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, putMaterialException,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,putMaterialException);
		...
		printlog.setEntity("WOM_7.5.0.0_putInMaterialException");
		...
		printlog.setBusinessId(putMaterialException.getId());
		printlog.setBussinessKey(null == putMaterialException.getTableNo() ? "" : putMaterialException.getTableNo().toString());
		printlog.setMainDisplay(null == putMaterialException.getId() ? "" : putMaterialException.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/putInMaterialException/putMaterialException/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(putMaterialException,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterialException/putMaterialException/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(putMaterialExceptionService.getPutMaterialException(Long.parseLong(arrId[i])));
	...
	@Action(value = "/WOM/putInMaterialException/putMaterialException/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/putInMaterialException/putMaterialException/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, putMaterialException,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				putMaterialException = putMaterialExceptionService.getPutMaterialException(Long.parseLong(arrId[i]));
				if(null != putMaterialException){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == putMaterialException.getId() ? "" : "," + putMaterialException.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == putMaterialException.getId() ? "" : putMaterialException.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == putMaterialException.getTableNo() ? "" : "," + putMaterialException.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == putMaterialException.getTableNo() ? "" : putMaterialException.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == putMaterialException.getId() ? "" : "," + putMaterialException.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == putMaterialException.getId() ? "" : putMaterialException.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, putMaterialException,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,putMaterialException);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//putMaterialExceptionService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/putInMaterialException/putMaterialException/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMPutMaterialException> findExportPageInfos(Page<WOMPutMaterialException> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_putInMaterialException" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = putMaterialExceptionService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMPutMaterialException queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = putMaterialExceptionService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = putMaterialExceptionService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterialException_PutMaterialException");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_putInMaterialException_PutMaterialException")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_putInMaterialException_PutMaterialException");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterialException_PutMaterialException");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = putMaterialExceptionService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_putInMaterialException_PutMaterialException,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_putInMaterialException_PutMaterialException,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_putInMaterialException_PutMaterialException,WOM_7.5.0.0) */
// 自定义代码
	 @Resource
	 private ConsulService consulService;
	 ///WOM/putInMaterialException/putMaterialException
	 //是否启用MES物料管理
	 @Action(value = "/WOM/putInMaterialException/putMaterialException/getIsmesProBatch", results = { @Result(type = JSON, params = {
	    		"root", "responseMap" }) })
	 public String getIsmesProBatch(){
		 Boolean ismesProBatch=consulService.getValueAsBoolean("platform/bap/WOM/WOM.mesProBatch");
		 responseMap.put("result", ismesProBatch);
		 return SUCCESS;
	 }
/* CUSTOM CODE END */
}