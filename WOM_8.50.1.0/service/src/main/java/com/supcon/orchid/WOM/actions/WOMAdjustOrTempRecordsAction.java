...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMAdjustOrTempRecordsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMAdjustOrTempRecords> {
	...
	private WOMAdjustOrTempRecords adjustOrTempRecords;
	...
	//导出excel时,主模型内容
	List<WOMAdjustOrTempRecords> queryResults = new ArrayList<WOMAdjustOrTempRecords>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/adjustOrTempRecords/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/adjustOrTempRecords/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_AdjustOrTempRecords" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMAdjustOrTempRecords> adjustOrTempRecordss;
	
	public List<WOMAdjustOrTempRecords> getAdjustOrTempRecordss() {
		return adjustOrTempRecordss;
	}
	...
	private Page<WOMAdjustOrTempRecords> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			adjustOrTempRecords = new WOMAdjustOrTempRecords();
			if(adjustOrTempRecords.getActiveRecordState() != null) {
				adjustOrTempRecords.setActiveRecordState((SystemCode) systemCodeService.getSystemCode(adjustOrTempRecords.getActiveRecordState().getId()));
			}
			if(adjustOrTempRecords.getActiveType() != null) {
				adjustOrTempRecords.setActiveType((SystemCode) systemCodeService.getSystemCode(adjustOrTempRecords.getActiveType().getId()));
			}
			if(adjustOrTempRecords.getMissonType() != null) {
				adjustOrTempRecords.setMissonType((SystemCode) systemCodeService.getSystemCode(adjustOrTempRecords.getMissonType().getId()));
			}
			if(adjustOrTempRecords.getRecordDataSource() != null) {
				adjustOrTempRecords.setRecordDataSource((SystemCode) systemCodeService.getSystemCode(adjustOrTempRecords.getRecordDataSource().getId()));
			}
		} else {
			if(id > 0){
				adjustOrTempRecords = adjustOrTempRecordsService.getAdjustOrTempRecords(id, viewCode);
			}
		}
	...
	public Page<WOMAdjustOrTempRecords> getPage() {
		if (null == page) {
			page = new Page<WOMAdjustOrTempRecords>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMAdjustOrTempRecords getAdjustOrTempRecords() {
		return adjustOrTempRecords;
	}		

	public void setAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords) {
		this.adjustOrTempRecords = adjustOrTempRecords;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = adjustOrTempRecordsService.getAdjustOrTempRecordsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1542097599573
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String adjustOrTempRecordsReferenceCopy() throws Exception{
			WOMAdjustOrTempRecords oldAdjustOrTempRecords = null;
			if(isEditNew()){
				...
				oldAdjustOrTempRecords = new WOMAdjustOrTempRecords();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,adjustOrTempRecords.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, adjustOrTempRecords.getId());
				...
				oldAdjustOrTempRecords = adjustOrTempRecordsService.getAdjustOrTempRecords(id);
			}
			
			
			adjustOrTempRecords = adjustOrTempRecordsService.getAdjustOrTempRecords(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_AdjustOrTempRecords");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMAdjustOrTempRecords.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldAdjustOrTempRecords, null);
						Method setValue = WOMAdjustOrTempRecords.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(adjustOrTempRecords, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			adjustOrTempRecords.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=adjustOrTempRecordsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return adjustOrTempRecordsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			adjustOrTempRecords = adjustOrTempRecordsService.getAdjustOrTempRecords(this.adjustOrTempRecords.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,adjustOrTempRecords.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			adjustOrTempRecordsService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = adjustOrTempRecordsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(adjustOrTempRecordsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(adjustOrTempRecordsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/adjustOrTempRecords/adjustActiveRef", results = { @Result(type = FREEMARKER, location = "adjustActiveRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"adjustOrTempRecords\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"adjustOrTempRecords\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"adjustOrTempRecords\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"adjustOrTempRecords\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			adjustOrTempRecordsService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/adjustOrTempRecords/adjustActiveRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.processName,result.acticeName,result.activeType.id,result.activeType.value,result.activeRecordState.id,result.activeRecordState.value,result.actualUse,result.startTime,result.endTime,result.remark,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="adjustActiveRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		adjustOrTempRecordsService.findDataGridPage(dg,dgclass,getDgPage(),adjustOrTempRecords,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(adjustOrTempRecords.getId() != null && adjustOrTempRecords.getId() != -1){
				adjustOrTempRecords = adjustOrTempRecordsService.getAdjustOrTempRecords(adjustOrTempRecords.getId());
			}
			adjustOrTempRecordsService.findDataGridPage(dg,dgclass,getDgPage(),adjustOrTempRecords,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == adjustOrTempRecords.getId()) {
		...
		adjustOrTempRecordsService.saveAdjustOrTempRecords(adjustOrTempRecords,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "submit");
		bap_validate_datagrids=adjustOrTempRecordsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", adjustOrTempRecords.getId());
		...
		fileuploads.put("linkId", adjustOrTempRecords.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/adjustOrTempRecords/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		adjustOrTempRecordsService.deleteAdjustOrTempRecords(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "restore");
		adjustOrTempRecordsService.restoreAdjustOrTempRecords(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMAdjustOrTempRecords> findExportDataInfos(Page<WOMAdjustOrTempRecords> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = adjustOrTempRecordsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = adjustOrTempRecordsService.getAssProperty(p.getCode());	
									//String assModelCode = adjustOrTempRecordsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			adjustOrTempRecordsService.findAdjustOrTempRecordss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			adjustOrTempRecordsService.findAdjustOrTempRecordss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMAdjustOrTempRecords> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMAdjustOrTempRecords result : results){
	...
	public boolean isEditNew(){
		//return null == this.adjustOrTempRecords || null == this.adjustOrTempRecords.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/adjustOrTempRecords/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/adjustOrTempRecords/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMAdjustOrTempRecordsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_AdjustOrTempRecords", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_AdjustOrTempRecords", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_AdjustOrTempRecords";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempRecords", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMAdjustOrTempRecords adjustOrTempRecords = null;
		if(null != id) {
			adjustOrTempRecords = adjustOrTempRecordsService.getAdjustOrTempRecords(id);
		}
		...
		_auditLog.setColumnStringB(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
		_auditLog.setColumnStringA(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
		_auditLog.setColumnStringC(adjustOrTempRecords.getId().toString());
		_auditLog.setColumnStringD(WOMAdjustOrTempRecords.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMAdjustOrTempRecordsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, adjustOrTempRecords,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, adjustOrTempRecords,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,adjustOrTempRecords);
		...
		printlog.setBusinessId(adjustOrTempRecords.getId());
		printlog.setBussinessKey(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
		printlog.setMainDisplay(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(adjustOrTempRecords,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(adjustOrTempRecordsService.getAdjustOrTempRecords(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, adjustOrTempRecords,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				adjustOrTempRecords = adjustOrTempRecordsService.getAdjustOrTempRecords(Long.parseLong(arrId[i]));
				if(null != adjustOrTempRecords){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == adjustOrTempRecords.getId() ? "" : "," + adjustOrTempRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == adjustOrTempRecords.getId() ? "" : "," + adjustOrTempRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == adjustOrTempRecords.getId() ? "" : "," + adjustOrTempRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, adjustOrTempRecords,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,adjustOrTempRecords);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//adjustOrTempRecordsService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/adjustOrTempRecords/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_AdjustOrTempRecords");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_AdjustOrTempRecords")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_AdjustOrTempRecords");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_AdjustOrTempRecords");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = adjustOrTempRecordsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
