...
@Repository
public class WOMBatchNumRuleDaoImpl extends ExtendedGenericDaoImpl<WOMBatchNumRule, Long> implements WOMBatchNumRuleDao {
	@Override
	...
	@Override
	public WOMBatchNumRuleDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMBatchNumRuleDealInfo dealInfo = new WOMBatchNumRuleDealInfo();
	...
	@Override
	public WOMBatchNumRuleDealInfo copyAndSaveDealInfo(DealInfo di, WOMBatchNumRule mainObj, Staff dealStaff) {
		WOMBatchNumRuleDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMBatchNumRuleDealInfo.class);
		...
		List<WOMBatchNumRuleDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
