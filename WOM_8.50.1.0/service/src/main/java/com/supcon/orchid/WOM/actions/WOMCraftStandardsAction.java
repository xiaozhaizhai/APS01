...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_procFeedback_CraftStandards,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMCraftStandardsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMCraftStandards> {
	...
	private WOMCraftStandards craftStandards;
	...
	//导出excel时,主模型内容
	List<WOMCraftStandards> queryResults = new ArrayList<WOMCraftStandards>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/procFeedback/craftStandards/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/procFeedback/craftStandards/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_procFeedback_CraftStandards" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMCraftStandards> craftStandardss;
	
	public List<WOMCraftStandards> getCraftStandardss() {
		return craftStandardss;
	}
	...
	private Page<WOMCraftStandards> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			craftStandards = new WOMCraftStandards();
		} else {
			if(id > 0){
				craftStandards = craftStandardsService.getCraftStandards(id, viewCode);
			}
		}
	...
	public Page<WOMCraftStandards> getPage() {
		if (null == page) {
			page = new Page<WOMCraftStandards>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMCraftStandards getCraftStandards() {
		return craftStandards;
	}		

	public void setCraftStandards(WOMCraftStandards craftStandards) {
		this.craftStandards = craftStandards;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/procFeedback/craftStandards/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = craftStandardsService.getCraftStandardsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/procFeedback/craftStandards/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1540964644168
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String craftStandardsReferenceCopy() throws Exception{
			WOMCraftStandards oldCraftStandards = null;
			if(isEditNew()){
				...
				oldCraftStandards = new WOMCraftStandards();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,craftStandards.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, craftStandards.getId());
				...
				oldCraftStandards = craftStandardsService.getCraftStandards(id);
			}
			
			
			craftStandards = craftStandardsService.getCraftStandards(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_CraftStandards");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMCraftStandards.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCraftStandards, null);
						Method setValue = WOMCraftStandards.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(craftStandards, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			craftStandards.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/craftStandards/craftStandardEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "craftStandardEdit.ftl") }),
		@Action(value = "/WOM/procFeedback/craftStandards/craftStandardEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "craftStandardEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=craftStandardsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return craftStandardsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			craftStandards = craftStandardsService.getCraftStandards(this.craftStandards.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,craftStandards.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = craftStandardsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(craftStandardsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(craftStandardsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_procFeedback_CraftStandards", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/craftStandards/craftStandardRef", results = { @Result(type = FREEMARKER, location = "craftStandardRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"craftStandards\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"craftStandards\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"craftStandards\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"craftStandards\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			craftStandardsService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/craftStandards/craftStandardRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.craftName,result.standardValue,result.remark,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="craftStandardRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		craftStandardsService.findDataGridPage(dg,dgclass,getDgPage(),craftStandards,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(craftStandards.getId() != null && craftStandards.getId() != -1){
				craftStandards = craftStandardsService.getCraftStandards(craftStandards.getId());
			}
			craftStandardsService.findDataGridPage(dg,dgclass,getDgPage(),craftStandards,searchCondition,params);
	...
	private void save() throws Exception {
		...
		craftStandardsService.saveCraftStandards(craftStandards,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/procFeedback/craftStandards/craftStandardEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "submit");
		bap_validate_datagrids=craftStandardsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", craftStandards.getId());
		...
		fileuploads.put("linkId", craftStandards.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/procFeedback/craftStandards/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		craftStandardsService.deleteCraftStandards(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/craftStandards/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "restore");
		craftStandardsService.restoreCraftStandards(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMCraftStandards> findExportDataInfos(Page<WOMCraftStandards> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = craftStandardsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = craftStandardsService.getAssProperty(p.getCode());	
									//String assModelCode = craftStandardsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			craftStandardsService.findCraftStandardss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			craftStandardsService.findCraftStandardss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMCraftStandards> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMCraftStandards result : results){
	...
	public boolean isEditNew(){
		//return null == this.craftStandards || null == this.craftStandards.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/procFeedback/craftStandards/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/procFeedback/craftStandards/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/procFeedback/craftStandards/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/craftStandards/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMCraftStandardsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/craftStandards/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMCraftStandardsService"
						, "com.supcon.orchid.RM.services.RMPrecessStandardsService"       
						, "com.supcon.orchid.WOM.services.WOMProcFeedbackHeadService"       
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_procFeedback_CraftStandards", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_procFeedback_CraftStandards", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/procFeedback/craftStandards/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/procFeedback/craftStandards/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_procFeedback_CraftStandards";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftStandards", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/procFeedback/craftStandards/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMCraftStandards craftStandards = null;
		if(null != id) {
			craftStandards = craftStandardsService.getCraftStandards(id);
		}
		...
		_auditLog.setColumnStringB(null == craftStandards.getId() ? "" : craftStandards.getId().toString());
		_auditLog.setColumnStringA(null == craftStandards.getId() ? "" : craftStandards.getId().toString());
		_auditLog.setColumnStringC(craftStandards.getId().toString());
		_auditLog.setColumnStringD(WOMCraftStandards.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMCraftStandardsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, craftStandards,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, craftStandards,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,craftStandards);
		...
		printlog.setBusinessId(craftStandards.getId());
		printlog.setBussinessKey(null == craftStandards.getId() ? "" : craftStandards.getId().toString());
		printlog.setMainDisplay(null == craftStandards.getId() ? "" : craftStandards.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/procFeedback/craftStandards/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(craftStandards,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/craftStandards/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(craftStandardsService.getCraftStandards(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/procFeedback/craftStandards/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/procFeedback/craftStandards/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, craftStandards,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				craftStandards = craftStandardsService.getCraftStandards(Long.parseLong(arrId[i]));
				if(null != craftStandards){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == craftStandards.getId() ? "" : "," + craftStandards.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == craftStandards.getId() ? "" : craftStandards.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == craftStandards.getId() ? "" : "," + craftStandards.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == craftStandards.getId() ? "" : craftStandards.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == craftStandards.getId() ? "" : "," + craftStandards.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == craftStandards.getId() ? "" : craftStandards.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, craftStandards,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,craftStandards);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/procFeedback/craftStandards/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_CraftStandards");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_procFeedback_CraftStandards")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_procFeedback_CraftStandards");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_CraftStandards");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = craftStandardsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_procFeedback_CraftStandards,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_procFeedback_CraftStandards,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_procFeedback_CraftStandards,WOM_7.5.0.0) */
// 自定义代码
