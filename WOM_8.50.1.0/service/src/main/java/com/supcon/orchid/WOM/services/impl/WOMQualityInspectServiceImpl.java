...
import com.supcon.orchid.WOM.services.WOMQualityInspectImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMQualityInspectDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_qualityInspectService")
@Transactional
public class WOMQualityInspectServiceImpl  extends BaseServiceImpl<WOMQualityInspect> implements WOMQualityInspectService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMQualityInspect> bapBaseService;
	...
	@Autowired
	private WOMQualityInspectImportService qualityInspectImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMQualityInspect.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  qualityInspectDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMQualityInspect.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = qualityInspectDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMQualityInspect.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMQualityInspect.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  qualityInspectDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMQualityInspect.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMQualityInspect.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMQualityInspect> list = (List<WOMQualityInspect>) qualityInspectDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMQualityInspect getQualityInspect(long id){
		return getQualityInspect(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMQualityInspect getQualityInspect(long id, String viewCode){
		WOMQualityInspect qualityInspect = qualityInspectDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getQualityInspectAsJSON(long id, String include){
		WOMQualityInspect qualityInspect = qualityInspectDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(qualityInspect, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.common.delete",operType="3")
	public void deleteQualityInspect(WOMQualityInspect qualityInspect){
		if(null != qualityInspect && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMQualityInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
			AuditUtil.setColumnStringA(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
			AuditUtil.setColumnStringC(qualityInspect.getId().toString());
		}
		beforeDeleteQualityInspect(qualityInspect);
		qualityInspectDao.delete(qualityInspect);
		afterDeleteQualityInspect(qualityInspect);
		qualityInspectDao.flush();
		List<WOMQualityInspect> params = new ArrayList<WOMQualityInspect>();
		params.add(qualityInspect);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_qualityRecords");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.common.delete",operType="3")
	public void deleteQualityInspect(List<Long> qualityInspectIds){
		deleteQualityInspect(qualityInspectIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.common.delete",operType="3")
	public void deleteQualityInspect(List<Long> qualityInspectIds, String eventTopic) {
		List<WOMQualityInspect> qualityInspects = new ArrayList<WOMQualityInspect>();
		for(Long qualityInspectId : qualityInspectIds){
			WOMQualityInspect qualityInspect = getQualityInspect(qualityInspectId);
			qualityInspects.add(qualityInspect);
			if(null != qualityInspect && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMQualityInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == qualityInspect.getId() ? "" : "," + qualityInspect.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == qualityInspect.getId() ? "" : "," + qualityInspect.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == qualityInspect.getId() ? "" : "," + qualityInspect.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(qualityInspects != null && qualityInspects.size() > 0 ){
			for(WOMQualityInspect qualityInspect : qualityInspects){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),qualityInspect, qualityInspect.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(qualityInspectIds != null && qualityInspectIds.size() > 0) {
			String hql = "update " + WOMQualityInspect.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = qualityInspectDao.createQuery(hql);
			query.setParameterList("ids", qualityInspectIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMQualityInspect qualityInspectz : qualityInspects){
				qualityInspectz.setValid(false);
				List<WOMQualityInspect> params = new ArrayList<WOMQualityInspect>();
				params.add(qualityInspectz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_qualityRecords");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.common.delete",operType="3")
	public void deleteQualityInspect(long qualityInspectId, int qualityInspectVersion){
		this.deleteQualityInspect(qualityInspectId, qualityInspectVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.common.delete",operType="3")
	public void deleteQualityInspect(long qualityInspectId, int qualityInspectVersion,SignatureLog signatureLog){
		WOMQualityInspect qualityInspect = getQualityInspect(qualityInspectId);
		if(qualityInspect != null && !qualityInspect.isValid()){
		...
		beforeDeleteQualityInspect(qualityInspect);
		qualityInspectDao.delete(qualityInspectId, qualityInspectVersion);
		afterDeleteQualityInspect(qualityInspect);
		qualityInspectDao.flush();
		List<WOMQualityInspect> params = new ArrayList<WOMQualityInspect>();
		params.add(qualityInspect);
		...
		if(signatureLog != null ) {
			...
			businessKey=qualityInspect.getId();
			signatureLog.setTableId(qualityInspect.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.common.delete",operType="3")
	public void deleteQualityInspect(String qualityInspectIds){
		this.deleteQualityInspect(qualityInspectIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.common.delete",operType="3")
	public void deleteQualityInspect(String qualityInspectIds,SignatureLog signatureLog){
		deleteCollection(qualityInspectIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteQualityInspect(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_qualityRecords#modelCode:WOM_7.5.0.0_qualityRecords_QualityInspect#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="ec.engine.restore",operType="10")
	public void restoreQualityInspect(String qualityInspectIds){
		restoreCollection(qualityInspectIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreQualityInspect(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="ec.engine.restore",operType="10")
	public void restoreQualityInspect(long qualityInspectId){
		
		findBusinessKeyUsed(qualityInspectId);	//判断业务主键是否重复
		...
		beforeRestoreQualityInspect(qualityInspect);
		qualityInspect.setValid(true);
		qualityInspectDao.update(qualityInspect);
		afterRestoreQualityInspect(qualityInspect);
		List<WOMQualityInspect> params = new ArrayList<WOMQualityInspect>();
		params.add(qualityInspect);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long qualityInspectId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_qualityRecords_QualityInspect");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMQualityInspect.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMQualityInspect.TABLE_NAME +" where id = ? )";
			List<Object> list =  qualityInspectDao.createNativeQuery(sql,qualityInspectId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseQualityInspect(List<WOMQualityInspect>  qualityInspects){
		for(WOMQualityInspect qualityInspect:qualityInspects)  {
			saveQualityInspect(qualityInspect, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseQualityInspect(List<WOMQualityInspect>  qualityInspects){
		for(WOMQualityInspect qualityInspect:qualityInspects)  {
			saveQualityInspect(qualityInspect, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="ec.flow.save")
	public void saveQualityInspect(WOMQualityInspect qualityInspect, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveQualityInspect(qualityInspect, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="ec.flow.save")
	public void saveQualityInspect(WOMQualityInspect qualityInspect, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveQualityInspect(qualityInspect, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateQualityInspectCodes(WOMQualityInspect qualityInspect) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateQualityInspectCodes(qualityInspect, false);
	}
	
	public void generateQualityInspectCodes(WOMQualityInspect qualityInspect, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateQualityInspectSummarys(WOMQualityInspect qualityInspect) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateQualityInspectSummarys(qualityInspect, false);
	}

	public void generateQualityInspectSummarys(WOMQualityInspect qualityInspect, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect")
	public void saveQualityInspect(WOMQualityInspect qualityInspect,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveQualityInspect(qualityInspect, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect")
	public void saveQualityInspect(WOMQualityInspect qualityInspect,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_qualityRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_qualityRecords");
		String url = null;
		if(qualityInspect.getId() != null && qualityInspect.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMQualityInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_qualityRecords/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMQualityInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_qualityRecords/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, qualityInspect);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(qualityInspect);
		beforeSaveQualityInspect(qualityInspect, viewIsView);

		if (viewIsView) {
			qualityInspectDao.saveWithRevertVersion(qualityInspect);
		} else {
			if(isNew)
				qualityInspectDao.save(qualityInspect);
			else
				qualityInspectDao.merge(qualityInspect);
		}
		if(null != qualityInspect && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMQualityInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
			AuditUtil.setColumnStringA(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
		...
		dealDatagridsSave(qualityInspect,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateQualityInspectCodes(qualityInspect, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateQualityInspectSummarys(qualityInspect, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveQualityInspect(qualityInspect, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, qualityInspect);
			}
		}
		List<WOMQualityInspect> params = new ArrayList<WOMQualityInspect>();
		qualityInspectDao.flush();
		qualityInspectDao.clear();
		qualityInspect = qualityInspectDao.load(qualityInspect.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMQualityInspect qualityInspect){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("qualityInspect", qualityInspect);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect")
	public void mergeQualityInspect(WOMQualityInspect qualityInspect, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(qualityInspect);
		beforeSaveQualityInspect(qualityInspect);
		qualityInspectDao.merge(qualityInspect);
		if(null != qualityInspect && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMQualityInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
			AuditUtil.setColumnStringA(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
		...
		dealDatagridsSave(qualityInspect,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveQualityInspect(qualityInspect);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMQualityInspect> findQualityInspects(Page<WOMQualityInspect> page, Criterion... criterions) {
		return qualityInspectDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMQualityInspectDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMQualityInspectDealInfo> list=qualityInspectDao.createQuery("from " + WOMQualityInspectDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMQualityInspectDealInfo dealInfo) {
		qualityInspectDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMQualityInspectDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) qualityInspectDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMQualityInspectDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = qualityInspectDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = qualityInspectDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMQualityInspectDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = qualityInspectDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findQualityInspects(Page<WOMQualityInspect> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findQualityInspects(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findQualityInspects(Page<WOMQualityInspect> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findQualityInspects(Page<WOMQualityInspect> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMQualityInspect> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"qualityInspect\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"qualityInspect\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"qualityInspect\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_qualityRecords_QualityInspect" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_qualityRecords_QualityInspect" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  qualityInspectDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  qualityInspectDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_qualityRecords").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"qualityInspect\"","WOM_7.5.0.0_qualityRecords",customCondition,"WOM_QUALITY_INSPECTS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"qualityInspect\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMQualityInspectDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"qualityInspect\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"qualityInspect\".LAY_REC ASC, \"qualityInspect\".SORT ASC");
				} else {
					orderPart.append(" \"qualityInspect\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) qualityInspectDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=qualityInspectDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = qualityInspectDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMQualityInspect> result = new ArrayList<WOMQualityInspect>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMQualityInspect.class,qualityInspectDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMQualityInspect.class,qualityInspectDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMQualityInspect qualityInspect : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(qualityInspect.getId(), "WOM_qualityRecords_qualityInspect");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(qualityInspect.getId(), "WOM_qualityRecords_qualityInspect");
							if(null != documents && !documents.isEmpty()) {
								qualityInspect.setDocument(documents.get(0));
								qualityInspect.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_qualityRecords")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_qualityRecords")+"WOM_7.5.0.0_qualityRecords".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_qualityRecords",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMQualityInspect> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMQualityInspect> result) {
		View view = null;
		...
		Map<Long, WOMQualityInspect> rsMap = new HashMap<Long, WOMQualityInspect>();
		for (WOMQualityInspect qualityInspect : result) {
			ids.add(qualityInspect.getId());
			rsMap.put(qualityInspect.getId(), qualityInspect);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(qualityInspectDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = qualityInspectDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMQualityInspect qualityInspect,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(qualityInspect.getOwnerStaffId());
		ti.setOwnerPositionId(qualityInspect.getOwnerPositionId());
		ti.setOwnerDepartmentId(qualityInspect.getOwnerDepartmentId());
		
		ti.setCreateStaff(qualityInspect.getCreateStaff());
		ti.setCreateStaffId(qualityInspect.getCreateStaffId());
		ti.setCreateTime(qualityInspect.getCreateTime());
		ti.setDeleteStaff(qualityInspect.getDeleteStaff());
		ti.setDeleteStaffId(qualityInspect.getDeleteStaffId());
		ti.setDeleteTime(qualityInspect.getDeleteTime());
		ti.setCreateDepartmentId(qualityInspect.getCreateDepartmentId());
		ti.setModifyStaff(qualityInspect.getModifyStaff());
		ti.setModifyStaffId(qualityInspect.getModifyStaffId());
		ti.setCreatePositionId(qualityInspect.getCreatePositionId());
		ti.setPositionLayRec(qualityInspect.getPositionLayRec());
		ti.setTableNo(qualityInspect.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_qualityRecords");
		ti.setTargetTableName("WOM_QUALITY_INSPECTS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_qualityRecords_QualityInspect" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  qualityInspectDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"qualityInspect\"","WOM_7.5.0.0_qualityRecords","","WOM_QUALITY_INSPECTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"qualityInspect\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"qualityInspect\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"qualityInspect\"","WOM_7.5.0.0_qualityRecords","","WOM_QUALITY_INSPECTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("qualityInspect.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"qualityInspect\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"qualityInspect\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"qualityInspect\".CID = 1 OR \"qualityInspect\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("qualityInspect.mc\"").append(".QUALITY_INSPECT) not in ("+notinValue+")");
			}
			
			SQLQuery query = qualityInspectDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("qualityRecords", "WOM_7.5.0.0_qualityRecords", "qualityRecords_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=qualityInspectDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMQualityInspect.ENTITY_CODE;}
	protected String getModuleCode(){return WOMQualityInspect.MODULE_CODE;}
	protected String getModelCode(){return WOMQualityInspect.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param qualityInspect 模型实例
	 */
	public void checkUniqueConstraint(WOMQualityInspect qualityInspect){
	}
	
	
	@Override
	public List<WOMQualityInspect> getQualityInspects(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMQualityInspect.class);
		if(params!=null){
			...
			return qualityInspectDao.findByHql("from " + WOMQualityInspect.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return qualityInspectDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMQualityInspect> findQualityInspectsByHql(String hql, Object... objects){
		...
		return   qualityInspectDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMQualityInspect loadQualityInspectByBussinessKey(WOMQualityInspect qualityInspect){
		return loadQualityInspectByBussinessKey(qualityInspect.getId());
	}
	
	@Override
	public Page<WOMQualityInspect> getQualityInspects(Page<WOMQualityInspect> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMQualityInspect.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = qualityInspectDao.findByPage(page, sql);
			}else{
				page = qualityInspectDao.findAllByPage(page);
			}
			return page;
		}
		page = qualityInspectDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public WOMQualityInspect loadQualityInspectByBussinessKey(Serializable bussinessKey){
		return qualityInspectDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMQualityInspect> findByProperty(String propertyName, Object object){
		return qualityInspectDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMQualityInspect findEntityByProperty(String propertyName, Object object){
		return qualityInspectDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMQualityInspect.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = qualityInspectDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_qualityRecords_QualityInspect","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_qualityRecords", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMQualityInspect qualityInspect,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,qualityInspectDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMQualityInspect.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, qualityInspect);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",qualityInspect.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMQualityInspect.TABLE_NAME +" where valid = 1";
			list = qualityInspectDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMQualityInspect.TABLE_NAME;
			list = qualityInspectDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMQualityInspect.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = qualityInspectDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMQualityInspectDaoImpl qualityInspectDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchQualityInspect(final List<WOMQualityInspect> insertObjs, final List<WOMQualityInspect> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchQualityInspect(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchQualityInspect(final List<WOMQualityInspect> insertObjs, final List<WOMQualityInspect> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1541146800376"),importCount));
		...
		Map<Long, WOMQualityInspect> oldUpdateObjsMap = new HashMap<Long, WOMQualityInspect>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMQualityInspect bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMQualityInspect> bizObjList = qualityInspectDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMQualityInspect bizObj = bizObjList.get(mIndex);
		...
		final List<WOMQualityInspect> nmeObjs = new ArrayList<WOMQualityInspect>();
		...
		long id=autoGeneratorID.getNextId(WOMQualityInspect.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMQualityInspect qualityInspect:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMQualityInspect.TABLE_NAME,1,dbId);
			if(qualityInspect.getId() != null ){
				m.put(qualityInspect.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			qualityInspect.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) qualityInspectDao.createNativeQuery(sql, "WOM_7.5.0.0_qualityRecords_QualityInspect"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		qualityInspectDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMQualityInspect.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMQualityInspect qualityInspect = updateObjs.get(updateIndex);
						Long id = qualityInspect.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = qualityInspect.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(qualityInspect);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMQualityInspect.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMQualityInspect.TABLE_NAME + " order by id desc";
//					Query query = qualityInspectDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMQualityInspect qualityInspect = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = qualityInspect.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		qualityInspectDaoImpl.getSession().flush();
		qualityInspectDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMQualityInspect bizObj = insertObjs.get(insertIndex);
			qualityInspectImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMQualityInspect bizObj = updateObjs.get(updateIndex);
			WOMQualityInspect oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			qualityInspectImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = qualityInspectDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				qualityInspectDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = qualityInspectDao.createNativeQuery(sql, "WOM_7.5.0.0_qualityRecords_QualityInspect").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = qualityInspectDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = qualityInspectDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = qualityInspectDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?qualityInspectDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  qualityInspectDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,qualityInspectDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="ec.flow.save")
	public void saveQualityInspect(WOMQualityInspect qualityInspect, DataGridService dataGridService){
		saveQualityInspect(qualityInspect, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="ec.flow.save")
	public void saveQualityInspect(WOMQualityInspect qualityInspect, DataGridService dataGridService, String viewCode){
		saveQualityInspect(qualityInspect, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect")
	public void saveQualityInspect(WOMQualityInspect qualityInspect, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveQualityInspect(qualityInspect, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect")
	public void saveQualityInspect(WOMQualityInspect qualityInspect, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_qualityRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_qualityRecords");
		String url = null;
		if(null != qualityInspect.getId() && qualityInspect.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMQualityInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect")
	public Map<Object, Object> qualityInspectDataGridImport(WOMQualityInspect qualityInspect, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = qualityInspect.getVirtualId();
		
		qualityInspect = qualityInspectDao.load(qualityInspect.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, qualityInspect.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = qualityInspect.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(qualityInspect);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, qualityInspect.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect")
	public void mergeQualityInspect(WOMQualityInspect qualityInspect, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(qualityInspect);
		beforeSaveQualityInspect(qualityInspect);
		qualityInspectDao.merge(qualityInspect);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_qualityRecords",model="WOM_7.5.0.0_qualityRecords_QualityInspect",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMQualityInspect> getByPage(Page<WOMQualityInspect> page,DetachedCriteria detachedCriteria){
		return qualityInspectDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_qualityRecords").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveQualityInspect(WOMQualityInspect qualityInspect, Object... objects){
		checkUniqueConstraint(qualityInspect);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveQualityInspect(WOMQualityInspect qualityInspect, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteQualityInspect(WOMQualityInspect qualityInspect){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteQualityInspect(WOMQualityInspect qualityInspect){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreQualityInspect(WOMQualityInspect qualityInspect){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreQualityInspect(WOMQualityInspect qualityInspect){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMQualityInspect qualityInspect, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMQualityInspect qualityInspect, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportQualityInspect(List<WOMQualityInspect> insertObjs, List<WOMQualityInspect> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportQualityInspect,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportQualityInspect(List<WOMQualityInspect> insertObjs, List<WOMQualityInspect> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportQualityInspect,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
