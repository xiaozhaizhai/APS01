...
import com.supcon.orchid.WOM.services.WOMMakeBatOrderPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMMakeBatOrderPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_makeBatOrderPartService")
@Transactional
public class WOMMakeBatOrderPartServiceImpl  extends BaseServiceImpl<WOMMakeBatOrderPart> implements WOMMakeBatOrderPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMMakeBatOrderPart> bapBaseService;
	...
	@Autowired
	private WOMMakeBatOrderPartImportService makeBatOrderPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMMakeBatOrderPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  makeBatOrderPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMMakeBatOrderPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = makeBatOrderPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMMakeBatOrderPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMMakeBatOrderPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  makeBatOrderPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMMakeBatOrderPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMMakeBatOrderPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMMakeBatOrderPart> list = (List<WOMMakeBatOrderPart>) makeBatOrderPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMMakeBatOrderPart getMakeBatOrderPart(long id){
		return getMakeBatOrderPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMakeBatOrderPart getMakeBatOrderPart(long id, String viewCode){
		WOMMakeBatOrderPart makeBatOrderPart = makeBatOrderPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMakeBatOrderPartAsJSON(long id, String include){
		WOMMakeBatOrderPart makeBatOrderPart = makeBatOrderPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(makeBatOrderPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart){
		if(null != makeBatOrderPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
			AuditUtil.setColumnStringA(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
			AuditUtil.setColumnStringC(makeBatOrderPart.getId().toString());
		}
		beforeDeleteMakeBatOrderPart(makeBatOrderPart);
		makeBatOrderPartDao.deletePhysical(makeBatOrderPart);
		afterDeleteMakeBatOrderPart(makeBatOrderPart);
		makeBatOrderPartDao.flush();
		List<WOMMakeBatOrderPart> params = new ArrayList<WOMMakeBatOrderPart>();
		params.add(makeBatOrderPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderPart(List<Long> makeBatOrderPartIds){
		deleteMakeBatOrderPart(makeBatOrderPartIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderPart(List<Long> makeBatOrderPartIds, String eventTopic) {
		List<WOMMakeBatOrderPart> makeBatOrderParts = new ArrayList<WOMMakeBatOrderPart>();
		for(Long makeBatOrderPartId : makeBatOrderPartIds){
			WOMMakeBatOrderPart makeBatOrderPart = getMakeBatOrderPart(makeBatOrderPartId);
			makeBatOrderParts.add(makeBatOrderPart);
			if(null != makeBatOrderPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == makeBatOrderPart.getId() ? "" : "," + makeBatOrderPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == makeBatOrderPart.getId() ? "" : "," + makeBatOrderPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == makeBatOrderPart.getId() ? "" : "," + makeBatOrderPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(makeBatOrderParts != null && makeBatOrderParts.size() > 0 ){
			for(WOMMakeBatOrderPart makeBatOrderPart : makeBatOrderParts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMMakeBatOrderPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(makeBatOrderPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),makeBatOrderPart, makeBatOrderPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(makeBatOrderPartIds != null && makeBatOrderPartIds.size() > 0) {
			String hql = "delete " + WOMMakeBatOrderPart.JPA_NAME + " where id in(:ids)";
			Query query = makeBatOrderPartDao.createQuery(hql);
			query.setParameterList("ids", makeBatOrderPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMMakeBatOrderPart makeBatOrderPartz : makeBatOrderParts){
				makeBatOrderPartz.setValid(false);
				List<WOMMakeBatOrderPart> params = new ArrayList<WOMMakeBatOrderPart>();
				params.add(makeBatOrderPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderPart(long makeBatOrderPartId, int makeBatOrderPartVersion){
		this.deleteMakeBatOrderPart(makeBatOrderPartId, makeBatOrderPartVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderPart(long makeBatOrderPartId, int makeBatOrderPartVersion,SignatureLog signatureLog){
		WOMMakeBatOrderPart makeBatOrderPart = getMakeBatOrderPart(makeBatOrderPartId);
		...
		beforeDeleteMakeBatOrderPart(makeBatOrderPart);
		makeBatOrderPartDao.delete(makeBatOrderPartId, makeBatOrderPartVersion);
		afterDeleteMakeBatOrderPart(makeBatOrderPart);
		makeBatOrderPartDao.flush();
		List<WOMMakeBatOrderPart> params = new ArrayList<WOMMakeBatOrderPart>();
		params.add(makeBatOrderPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=makeBatOrderPart.getId();
			signatureLog.setTableId(makeBatOrderPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderPart(String makeBatOrderPartIds){
		this.deleteMakeBatOrderPart(makeBatOrderPartIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderPart(String makeBatOrderPartIds,SignatureLog signatureLog){
		deleteCollection(makeBatOrderPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMakeBatOrderPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_batchingMaterialNeed#modelCode:WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="ec.engine.restore",operType="10")
	public void restoreMakeBatOrderPart(String makeBatOrderPartIds){
		restoreCollection(makeBatOrderPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreMakeBatOrderPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="ec.engine.restore",operType="10")
	public void restoreMakeBatOrderPart(long makeBatOrderPartId){
		
		findBusinessKeyUsed(makeBatOrderPartId);	//判断业务主键是否重复
		...
		beforeRestoreMakeBatOrderPart(makeBatOrderPart);
		makeBatOrderPart.setValid(true);
		makeBatOrderPartDao.update(makeBatOrderPart);
		afterRestoreMakeBatOrderPart(makeBatOrderPart);
		List<WOMMakeBatOrderPart> params = new ArrayList<WOMMakeBatOrderPart>();
		params.add(makeBatOrderPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long makeBatOrderPartId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMMakeBatOrderPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMMakeBatOrderPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  makeBatOrderPartDao.createNativeQuery(sql,makeBatOrderPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseMakeBatOrderPart(List<WOMMakeBatOrderPart>  makeBatOrderParts){
		for(WOMMakeBatOrderPart makeBatOrderPart:makeBatOrderParts)  {
			saveMakeBatOrderPart(makeBatOrderPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMakeBatOrderPart(List<WOMMakeBatOrderPart>  makeBatOrderParts){
		for(WOMMakeBatOrderPart makeBatOrderPart:makeBatOrderParts)  {
			saveMakeBatOrderPart(makeBatOrderPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="ec.flow.save")
	public void saveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMakeBatOrderPart(makeBatOrderPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="ec.flow.save")
	public void saveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMakeBatOrderPart(makeBatOrderPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateMakeBatOrderPartCodes(WOMMakeBatOrderPart makeBatOrderPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMakeBatOrderPartCodes(makeBatOrderPart, false);
	}
	
	public void generateMakeBatOrderPartCodes(WOMMakeBatOrderPart makeBatOrderPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateMakeBatOrderPartSummarys(WOMMakeBatOrderPart makeBatOrderPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMakeBatOrderPartSummarys(makeBatOrderPart, false);
	}

	public void generateMakeBatOrderPartSummarys(WOMMakeBatOrderPart makeBatOrderPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart")
	public void saveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveMakeBatOrderPart(makeBatOrderPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart")
	public void saveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
		String url = null;
		if(makeBatOrderPart.getId() != null && makeBatOrderPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, makeBatOrderPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(makeBatOrderPart);
		beforeSaveMakeBatOrderPart(makeBatOrderPart, viewIsView);

		if (viewIsView) {
			makeBatOrderPartDao.saveWithRevertVersion(makeBatOrderPart);
		} else {
			if(isNew)
				makeBatOrderPartDao.save(makeBatOrderPart);
			else
				makeBatOrderPartDao.merge(makeBatOrderPart);
		}
		if(null != makeBatOrderPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
			AuditUtil.setColumnStringA(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
		...
		dealDatagridsSave(makeBatOrderPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateMakeBatOrderPartCodes(makeBatOrderPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMakeBatOrderPartSummarys(makeBatOrderPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMakeBatOrderPart(makeBatOrderPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, makeBatOrderPart);
			}
		}
		List<WOMMakeBatOrderPart> params = new ArrayList<WOMMakeBatOrderPart>();
		makeBatOrderPartDao.flush();
		makeBatOrderPartDao.clear();
		makeBatOrderPart = makeBatOrderPartDao.load(makeBatOrderPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMMakeBatOrderPart makeBatOrderPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("makeBatOrderPart", makeBatOrderPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart")
	public void mergeMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(makeBatOrderPart);
		beforeSaveMakeBatOrderPart(makeBatOrderPart);
		makeBatOrderPartDao.merge(makeBatOrderPart);
		if(null != makeBatOrderPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
			AuditUtil.setColumnStringA(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
		...
		dealDatagridsSave(makeBatOrderPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveMakeBatOrderPart(makeBatOrderPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMMakeBatOrderPart> findMakeBatOrderParts(Page<WOMMakeBatOrderPart> page, Criterion... criterions) {
		return makeBatOrderPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMakeBatOrderParts(Page<WOMMakeBatOrderPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMakeBatOrderParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMakeBatOrderParts(Page<WOMMakeBatOrderPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMakeBatOrderParts(Page<WOMMakeBatOrderPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMMakeBatOrderPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"makeBatOrderPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"makeBatOrderPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"makeBatOrderPart\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  makeBatOrderPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  makeBatOrderPartDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_batchingMaterialNeed").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"makeBatOrderPart\".LAY_REC ASC, \"makeBatOrderPart\".SORT ASC");
				} else {
					orderPart.append(" \"makeBatOrderPart\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) makeBatOrderPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=makeBatOrderPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = makeBatOrderPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMMakeBatOrderPart> result = new ArrayList<WOMMakeBatOrderPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMMakeBatOrderPart.class,makeBatOrderPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMMakeBatOrderPart.class,makeBatOrderPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMMakeBatOrderPart makeBatOrderPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(makeBatOrderPart.getId(), "WOM_batchingMaterialNeed_makeBatOrderPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(makeBatOrderPart.getId(), "WOM_batchingMaterialNeed_makeBatOrderPart");
							if(null != documents && !documents.isEmpty()) {
								makeBatOrderPart.setDocument(documents.get(0));
								makeBatOrderPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMMakeBatOrderPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMMakeBatOrderPart> result) {
		View view = null;
		...
		Map<Long, WOMMakeBatOrderPart> rsMap = new HashMap<Long, WOMMakeBatOrderPart>();
		for (WOMMakeBatOrderPart makeBatOrderPart : result) {
			ids.add(makeBatOrderPart.getId());
			rsMap.put(makeBatOrderPart.getId(), makeBatOrderPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(makeBatOrderPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = makeBatOrderPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  makeBatOrderPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"makeBatOrderPart\"","WOM_7.5.0.0_batchingMaterialNeed","","WOM_MAKE_BAT_ORDER_PARTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"makeBatOrderPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"makeBatOrderPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"makeBatOrderPart\"","WOM_7.5.0.0_batchingMaterialNeed","","WOM_MAKE_BAT_ORDER_PARTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("makeBatOrderPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"makeBatOrderPart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"makeBatOrderPart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"makeBatOrderPart\".CID = 1 OR \"makeBatOrderPart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("makeBatOrderPart.mc\"").append(".MAKE_BAT_ORDER_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = makeBatOrderPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=makeBatOrderPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMMakeBatOrderPart.ENTITY_CODE;}
	protected String getModuleCode(){return WOMMakeBatOrderPart.MODULE_CODE;}
	protected String getModelCode(){return WOMMakeBatOrderPart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param makeBatOrderPart 模型实例
	 */
	public void checkUniqueConstraint(WOMMakeBatOrderPart makeBatOrderPart){
	}
	
	
	@Override
	public List<WOMMakeBatOrderPart> getMakeBatOrderParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMakeBatOrderPart.class);
		if(params!=null){
			...
			return makeBatOrderPartDao.findByHql("from " + WOMMakeBatOrderPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return makeBatOrderPartDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMMakeBatOrderPart> findMakeBatOrderPartsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMakeBatOrderPart.class);
	...
	@Override
	public List<WOMMakeBatOrderPart> findMakeBatOrderPartsByHql(String hql, Object... objects){
		
		return   makeBatOrderPartDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMMakeBatOrderPart loadMakeBatOrderPartByBussinessKey(WOMMakeBatOrderPart makeBatOrderPart){
		return loadMakeBatOrderPartByBussinessKey(makeBatOrderPart.getId());
	}
	
	@Override
	public Page<WOMMakeBatOrderPart> getMakeBatOrderParts(Page<WOMMakeBatOrderPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMakeBatOrderPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = makeBatOrderPartDao.findByPage(page, sql);
			}else{
				page = makeBatOrderPartDao.findAllByPage(page);
			}
			return page;
		}
		page = makeBatOrderPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMMakeBatOrderPart> findByTreeDataGridMakeBatOrderHead(List<WOMMakeBatOrderPart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMMakeBatOrderPart makeBatOrderPart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridMakeBatOrderHead(Page<WOMMakeBatOrderPart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMMakeBatOrderPart> makeBatOrderParts = dgPage.getResult();
			for(WOMMakeBatOrderPart makeBatOrderPart : makeBatOrderParts){
				
			}
			dgPage.setResult(makeBatOrderParts);
		}
	...
	@Override
	@Transactional
	public void deleteMakeBatOrderPartByMakeBatOrderHead(List<Long> makeBatOrderHeadIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("headIDList", makeBatOrderHeadIds);
		String hql = "select id from WOMMakeBatOrderPart where "
						+ "headID.id in (:headIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> makeBatOrderPartIds1 = makeBatOrderPartDao.findByHql(hql, params);
		if(makeBatOrderPartIds1 != null && !makeBatOrderPartIds1.isEmpty()) {
		}
		hql = "delete WOMMakeBatOrderPart where "
						+ "headID.id in (:headIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		makeBatOrderPartDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMakeBatOrderPart loadMakeBatOrderPartByBussinessKey(Serializable bussinessKey){
		return makeBatOrderPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMMakeBatOrderPart> findByProperty(String propertyName, Object object){
		return makeBatOrderPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMakeBatOrderPart findEntityByProperty(String propertyName, Object object){
		return makeBatOrderPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMMakeBatOrderPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = makeBatOrderPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart","runtime");
		try {
	...
	public void dealDatagridsSave(WOMMakeBatOrderPart makeBatOrderPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,makeBatOrderPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMMakeBatOrderPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, makeBatOrderPart);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",makeBatOrderPart.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMMakeBatOrderPart.TABLE_NAME +" where valid = 1";
			list = makeBatOrderPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMMakeBatOrderPart.TABLE_NAME;
			list = makeBatOrderPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMMakeBatOrderPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = makeBatOrderPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMMakeBatOrderPartDaoImpl makeBatOrderPartDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchMakeBatOrderPart(final List<WOMMakeBatOrderPart> insertObjs, final List<WOMMakeBatOrderPart> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchMakeBatOrderPart(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchMakeBatOrderPart(final List<WOMMakeBatOrderPart> insertObjs, final List<WOMMakeBatOrderPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1491801852784"),importCount));
		...
		Map<Long, WOMMakeBatOrderPart> oldUpdateObjsMap = new HashMap<Long, WOMMakeBatOrderPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMMakeBatOrderPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMMakeBatOrderPart> bizObjList = makeBatOrderPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMMakeBatOrderPart bizObj = bizObjList.get(mIndex);
		...
		final List<WOMMakeBatOrderPart> nmeObjs = new ArrayList<WOMMakeBatOrderPart>();
		...
		long id=autoGeneratorID.getNextId(WOMMakeBatOrderPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMMakeBatOrderPart makeBatOrderPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMMakeBatOrderPart.TABLE_NAME,1,dbId);
			if(makeBatOrderPart.getId() != null ){
				m.put(makeBatOrderPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) makeBatOrderPartDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		makeBatOrderPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMMakeBatOrderPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMMakeBatOrderPart makeBatOrderPart = updateObjs.get(updateIndex);
						Long id = makeBatOrderPart.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = makeBatOrderPart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(makeBatOrderPart);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMMakeBatOrderPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMMakeBatOrderPart.TABLE_NAME + " order by id desc";
//					Query query = makeBatOrderPartDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMMakeBatOrderPart makeBatOrderPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = makeBatOrderPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		makeBatOrderPartDaoImpl.getSession().flush();
		makeBatOrderPartDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMMakeBatOrderPart bizObj = insertObjs.get(insertIndex);
			makeBatOrderPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMMakeBatOrderPart bizObj = updateObjs.get(updateIndex);
			WOMMakeBatOrderPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			makeBatOrderPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMMakeBatOrderPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMMakeBatOrderPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) makeBatOrderPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMMakeBatOrderPart> result = getResult(page, query, new PendingResultTransformer(WOMMakeBatOrderPart.class,makeBatOrderPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMMakeBatOrderPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = makeBatOrderPartDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = makeBatOrderPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = makeBatOrderPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = makeBatOrderPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?makeBatOrderPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  makeBatOrderPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,makeBatOrderPartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="ec.flow.save")
	public void saveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, DataGridService dataGridService){
		saveMakeBatOrderPart(makeBatOrderPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="ec.flow.save")
	public void saveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, DataGridService dataGridService, String viewCode){
		saveMakeBatOrderPart(makeBatOrderPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart")
	public void saveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveMakeBatOrderPart(makeBatOrderPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart")
	public void saveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchingMaterialNeed";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
		String url = null;
		if(null != makeBatOrderPart.getId() && makeBatOrderPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart")
	public Map<Object, Object> makeBatOrderPartDataGridImport(WOMMakeBatOrderPart makeBatOrderPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = makeBatOrderPart.getVirtualId();
		
		makeBatOrderPart = makeBatOrderPartDao.load(makeBatOrderPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, makeBatOrderPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = makeBatOrderPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(makeBatOrderPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, makeBatOrderPart.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart")
	public void mergeMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(makeBatOrderPart);
		beforeSaveMakeBatOrderPart(makeBatOrderPart);
		makeBatOrderPartDao.merge(makeBatOrderPart);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart",desc="foundation.batch.save",operType="4")
	public void batchSaveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_batchingMaterialNeed";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
	...
	@Override
	public Page<WOMMakeBatOrderPart> getByPage(Page<WOMMakeBatOrderPart> page,DetachedCriteria detachedCriteria){
		return makeBatOrderPartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, Object... objects){
		checkUniqueConstraint(makeBatOrderPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportMakeBatOrderPart(List<WOMMakeBatOrderPart> makeBatOrderParts, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportMakeBatOrderPart(List<WOMMakeBatOrderPart> makeBatOrderParts){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportMakeBatOrderPart(List<WOMMakeBatOrderPart> makeBatOrderParts, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportMakeBatOrderPart(List<WOMMakeBatOrderPart> makeBatOrderParts){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMMakeBatOrderPart makeBatOrderPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMMakeBatOrderPart makeBatOrderPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportMakeBatOrderPart(List<WOMMakeBatOrderPart> insertObjs, List<WOMMakeBatOrderPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMakeBatOrderPart,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportMakeBatOrderPart(List<WOMMakeBatOrderPart> insertObjs, List<WOMMakeBatOrderPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMakeBatOrderPart,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
