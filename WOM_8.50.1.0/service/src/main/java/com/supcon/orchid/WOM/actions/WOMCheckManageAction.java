...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMCheckManageAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMCheckManage> {
	...
	private WOMCheckManage checkManage;
	...
	//导出excel时,主模型内容
	List<WOMCheckManage> queryResults = new ArrayList<WOMCheckManage>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/checkManage/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/checkManage/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_generalManage_CheckManage" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMCheckManage> checkManages;
	
	public List<WOMCheckManage> getCheckManages() {
		return checkManages;
	}
	...
	private Page<WOMCheckManage> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			checkManage = new WOMCheckManage();
		} else {
			if(id > 0){
				checkManage = checkManageService.getCheckManage(id, viewCode);
			}
		}
	...
	public Page<WOMCheckManage> getPage() {
		if (null == page) {
			page = new Page<WOMCheckManage>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMCheckManage getCheckManage() {
		return checkManage;
	}		

	public void setCheckManage(WOMCheckManage checkManage) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/checkManage/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = checkManageService.getCheckManageAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/checkManage/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1502696273660
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String checkManageReferenceCopy() throws Exception{
			WOMCheckManage oldCheckManage = null;
			if(isEditNew()){
				...
				oldCheckManage = new WOMCheckManage();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,checkManage.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, checkManage.getId());
				...
				oldCheckManage = checkManageService.getCheckManage(id);
			}
			
			
			checkManage = checkManageService.getCheckManage(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_CheckManage");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMCheckManage.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCheckManage, null);
						Method setValue = WOMCheckManage.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(checkManage, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			checkManage.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=checkManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return checkManageReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			checkManage = checkManageService.getCheckManage(this.checkManage.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,checkManage.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = checkManageService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(checkManageService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(checkManageService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_generalManage_CheckManage", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"checkManage\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"checkManage\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"inspectDept\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"checkManage\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"checkManage\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			checkManageService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		checkManageService.findDataGridPage(dg,dgclass,getDgPage(),checkManage,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(checkManage.getId() != null && checkManage.getId() != -1){
				checkManage = checkManageService.getCheckManage(checkManage.getId());
			}
			checkManageService.findDataGridPage(dg,dgclass,getDgPage(),checkManage,searchCondition,params);
	...
	private void save() throws Exception {
		...
		checkManageService.saveCheckManage(checkManage,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "submit");
		bap_validate_datagrids=checkManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", checkManage.getId());
		...
		fileuploads.put("linkId", checkManage.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/generalManage/checkManage/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		checkManageService.deleteCheckManage(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/checkManage/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "restore");
		checkManageService.restoreCheckManage(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMCheckManage> findExportDataInfos(Page<WOMCheckManage> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = checkManageService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = checkManageService.getAssProperty(p.getCode());	
									//String assModelCode = checkManageService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			checkManageService.findCheckManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			checkManageService.findCheckManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMCheckManage> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMCheckManage result : results){
	...
	public boolean isEditNew(){
		//return null == this.checkManage || null == this.checkManage.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/generalManage/checkManage/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/generalManage/checkManage/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/generalManage/checkManage/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/checkManage/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMCheckManageService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/checkManage/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_generalManage_CheckManage", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_generalManage_CheckManage", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/generalManage/checkManage/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/generalManage/checkManage/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_generalManage_CheckManage";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_CheckManage", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/generalManage/checkManage/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == checkManage.getId() ? "" : checkManage.getId().toString());
		_auditLog.setColumnStringA(null == checkManage.getId() ? "" : checkManage.getId().toString());
		_auditLog.setColumnStringC(checkManage.getId().toString());
		_auditLog.setColumnStringD(WOMCheckManage.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMCheckManageAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, checkManage,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, checkManage,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,checkManage);
		...
		printlog.setBusinessId(checkManage.getId());
		printlog.setBussinessKey(null == checkManage.getId() ? "" : checkManage.getId().toString());
		printlog.setMainDisplay(null == checkManage.getId() ? "" : checkManage.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/generalManage/checkManage/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(checkManage,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/checkManage/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(checkManageService.getCheckManage(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/generalManage/checkManage/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/generalManage/checkManage/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, checkManage,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				checkManage = checkManageService.getCheckManage(Long.parseLong(arrId[i]));
				if(null != checkManage){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == checkManage.getId() ? "" : "," + checkManage.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == checkManage.getId() ? "" : checkManage.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == checkManage.getId() ? "" : "," + checkManage.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == checkManage.getId() ? "" : checkManage.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == checkManage.getId() ? "" : "," + checkManage.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == checkManage.getId() ? "" : checkManage.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, checkManage,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,checkManage);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/generalManage/checkManage/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_CheckManage");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_generalManage_CheckManage")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_generalManage_CheckManage");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_CheckManage");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = checkManageService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
