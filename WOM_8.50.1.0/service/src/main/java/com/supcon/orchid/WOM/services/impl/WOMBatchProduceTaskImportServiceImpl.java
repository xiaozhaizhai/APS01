...
@Service("wOMBatchProduceTaskImportService")
public class WOMBatchProduceTaskImportServiceImpl implements WOMBatchProduceTaskImportService {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="保存业务信息",operType="1")
	public void saveImportDataLog(WOMBatchProduceTask bizObj, WOMBatchProduceTask oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("WOM.entityname.randon1495089852928") + "信息");
			...
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getTableNo() ? "" : "," + oldBizObj.getTableNo().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getTableNo() ? "" : oldBizObj.getTableNo().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("WOM.entityname.randon1495089852928") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
			...
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getTableNo() ? "" : "," + bizObj.getTableNo().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getTableNo() ? "" : bizObj.getTableNo().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(WOMBatchProduceTask bizObj, String bussinesskeyFieldName,
