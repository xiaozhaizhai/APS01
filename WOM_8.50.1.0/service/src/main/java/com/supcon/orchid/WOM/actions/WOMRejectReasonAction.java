...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMRejectReasonAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMRejectReason> {
	...
	//导出excel时,主模型内容
	List<WOMRejectReason> queryResults = new ArrayList<WOMRejectReason>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/materialReject/rejectReason/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/materialReject/rejectReason/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_materialReject_RejectReason" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMRejectReason> rejectReasons;
	
	public List<WOMRejectReason> getRejectReasons() {
	...
	private Page<WOMRejectReason> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			rejectReason = new WOMRejectReason();
		} else {
			if(id > 0){
				rejectReason = rejectReasonService.getRejectReason(id, viewCode);
			}
		}
	...
	public Page<WOMRejectReason> getPage() {
		if (null == page) {
			page = new Page<WOMRejectReason>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/materialReject/rejectReason/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = rejectReasonService.getRejectReasonAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/materialReject/rejectReason/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String rejectReasonReferenceCopy() throws Exception{
			WOMRejectReason oldRejectReason = null;
			if(isEditNew()){
				...
				oldRejectReason = new WOMRejectReason();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,rejectReason.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, rejectReason.getId());
				...
				oldRejectReason = rejectReasonService.getRejectReason(id);
			}
			
			
			rejectReason = rejectReasonService.getRejectReason(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_materialReject_RejectReason");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMRejectReason.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldRejectReason, null);
						Method setValue = WOMRejectReason.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(rejectReason, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			rejectReason.setCreateDepartmentId(oldRejectReason.getCreateDepartmentId());
			rejectReason.setCreatePositionId(oldRejectReason.getCreatePositionId());
			rejectReason.setCreateStaffId(oldRejectReason.getCreateStaffId());
			rejectReason.setOwnerDepartmentId(oldRejectReason.getOwnerDepartmentId());
			rejectReason.setOwnerPositionId(oldRejectReason.getOwnerPositionId());
			rejectReason.setOwnerStaffId(oldRejectReason.getOwnerStaffId());
			rejectReason.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/materialReject/rejectReason/rejectReasonEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "rejectReasonEdit.ftl") }),
		@Action(value = "/WOM/materialReject/rejectReason/rejectReasonEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "rejectReasonEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=rejectReasonService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return rejectReasonReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			rejectReason = rejectReasonService.getRejectReason(this.rejectReason.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,rejectReason.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/WOM/materialReject/rejectReason/rejectReasonList", results = { @Result(type = FREEMARKER, location = "rejectReasonList.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/WOM/materialReject/rejectReason/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = rejectReasonService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(rejectReasonService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(rejectReasonService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_materialReject_RejectReason", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/materialReject/rejectReason/rejectReasonRef", results = { @Result(type = FREEMARKER, location = "rejectReasonRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/materialReject/rejectReason/rejectReasonList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.rejectReson,result.isChangeNeed,result.demo,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="rejectReasonList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"rejectReason\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"rejectReason\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"rejectReason\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"rejectReason\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			rejectReasonService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/materialReject/rejectReason/rejectReasonList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/materialReject/rejectReason/rejectReasonRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.rejectReson,result.isChangeNeed,result.demo,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="rejectReasonRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/materialReject/rejectReason/allTempleteDownload", results = { @Result(name="excel",type="excel",location="rejectReasonList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_materialReject_RejectReason");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		rejectReasonService.findDataGridPage(dg,dgclass,getDgPage(),rejectReason,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(rejectReason.getId() != null && rejectReason.getId() != -1){
				rejectReason = rejectReasonService.getRejectReason(rejectReason.getId());
			}
			rejectReasonService.findDataGridPage(dg,dgclass,getDgPage(),rejectReason,searchCondition,params);
	...
	private void save() throws Exception {
		...
		rejectReasonService.saveRejectReason(rejectReason,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/materialReject/rejectReason/rejectReasonEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "submit");
		bap_validate_datagrids=rejectReasonService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", rejectReason.getId());
		...
		fileuploads.put("linkId", rejectReason.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/materialReject/rejectReason/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		rejectReasonService.deleteRejectReason(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/materialReject/rejectReason/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "restore");
		rejectReasonService.restoreRejectReason(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/materialReject/rejectReason/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/materialReject/rejectReason/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "showDealInfoData");
		if (null != tableInfoId) {
			int count=rejectReasonService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = rejectReasonService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = rejectReasonService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMRejectReason> findExportDataInfos(Page<WOMRejectReason> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = rejectReasonService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = rejectReasonService.getAssProperty(p.getCode());	
									//String assModelCode = rejectReasonService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			rejectReasonService.findRejectReasons(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			rejectReasonService.findRejectReasons(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMRejectReason> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMRejectReason result : results){
	...
	public boolean isEditNew(){
		//return null == this.rejectReason || null == this.rejectReason.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/materialReject/rejectReason/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/materialReject/rejectReason/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/materialReject/rejectReason/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/materialReject/rejectReason/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMRejectReasonService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_materialReject", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_materialReject");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/materialReject/rejectReason/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_materialReject_RejectReason", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_materialReject_RejectReason", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/materialReject/rejectReason/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/materialReject/rejectReason/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_materialReject_RejectReason";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_materialReject_RejectReason", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/materialReject/rejectReason/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMRejectReason rejectReason = null;
		if(null != id) {
			rejectReason = rejectReasonService.getRejectReason(id);
		}
		...
		_auditLog.setColumnStringB(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
		_auditLog.setColumnStringA(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
		_auditLog.setColumnStringC(rejectReason.getId().toString());
		_auditLog.setColumnStringD(WOMRejectReason.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_materialReject");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMRejectReasonAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, rejectReason,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, rejectReason,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,rejectReason);
		...
		printlog.setEntity("WOM_7.5.0.0_materialReject");
		...
		printlog.setBusinessId(rejectReason.getId());
		printlog.setBussinessKey(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
		printlog.setMainDisplay(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/materialReject/rejectReason/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(rejectReason,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/materialReject/rejectReason/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(rejectReasonService.getRejectReason(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/materialReject/rejectReason/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/materialReject/rejectReason/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, rejectReason,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				rejectReason = rejectReasonService.getRejectReason(Long.parseLong(arrId[i]));
				if(null != rejectReason){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == rejectReason.getId() ? "" : "," + rejectReason.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == rejectReason.getId() ? "" : "," + rejectReason.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == rejectReason.getId() ? "" : "," + rejectReason.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, rejectReason,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,rejectReason);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/materialReject/rejectReason/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMRejectReason> findExportPageInfos(Page<WOMRejectReason> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_materialReject" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = rejectReasonService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMRejectReason queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = rejectReasonService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = rejectReasonService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_materialReject_RejectReason");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_materialReject_RejectReason")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_materialReject_RejectReason");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_materialReject_RejectReason");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = rejectReasonService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
