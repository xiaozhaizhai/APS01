...
@Repository
public class WOMBatchProduceTaskDaoImpl extends ExtendedGenericDaoImpl<WOMBatchProduceTask, Long> implements WOMBatchProduceTaskDao {
	@Override
	...
	@Override
	public WOMBatchProduceTaskDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMBatchProduceTaskDealInfo dealInfo = new WOMBatchProduceTaskDealInfo();
	...
	@Override
	public WOMBatchProduceTaskDealInfo copyAndSaveDealInfo(DealInfo di, WOMBatchProduceTask mainObj, Staff dealStaff) {
		WOMBatchProduceTaskDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMBatchProduceTaskDealInfo.class);
		...
		List<WOMBatchProduceTaskDealInfo> dealinfo = criteria.list();
	...
	public void addSupervision(Supervision supervision) {
		...
		WOMBatchProduceTaskSupervision WOMBatchProduceTaskSupervision = new WOMBatchProduceTaskSupervision();
		BeanUtils.copyProperties(supervision, WOMBatchProduceTaskSupervision);
		WOMBatchProduceTaskSupervision.setValid(true);
		getSession().save(WOMBatchProduceTaskSupervision);
	}

	@Override
	public void addSupervision(Supervision supervision, WOMBatchProduceTask mainObj) {
		getSession().save(supervision);
		WOMBatchProduceTaskSupervision localSupervision = new WOMBatchProduceTaskSupervision();
	...
	public void deleteSupervision(Long tableInfoId, List<Long> idList) {
		...
		List<WOMBatchProduceTaskSupervision> _supervisions = getSession().createCriteria(WOMBatchProduceTaskSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.in("staff.id", idList))
				.add(Restrictions.eq("valid", true)).list();
		for (WOMBatchProduceTaskSupervision item : _supervisions) {
			item.setValid(false);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMBatchProduceTaskSupervision> findSupervision(Long tableInfoId) {
		List<WOMBatchProduceTaskSupervision> supervisions = getSession().createCriteria(WOMBatchProduceTaskSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("valid", true)).addOrder(Order.asc("id")).list();
		return supervisions;
	}
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMBatchProduceTaskSupervision> findSupervision(Long tableInfoId, Long StaffId) {
		List<WOMBatchProduceTaskSupervision> supervisions = getSession().createCriteria(WOMBatchProduceTaskSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("staff.id", StaffId)).add(Restrictions.eq("valid", true)).list();
		return supervisions;
	}
	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
