...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMProduceMaterialSumAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProduceMaterialSum> {
	...
	private WOMProduceMaterialSum produceMaterialSum;
	...
	//导出excel时,主模型内容
	List<WOMProduceMaterialSum> queryResults = new ArrayList<WOMProduceMaterialSum>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/produceMaterialSum/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/produceMaterialSum/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_ProduceMaterialSum" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProduceMaterialSum> produceMaterialSums;
	
	public List<WOMProduceMaterialSum> getProduceMaterialSums() {
		return produceMaterialSums;
	}
	...
	private Page<WOMProduceMaterialSum> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			produceMaterialSum = new WOMProduceMaterialSum();
			if(produceMaterialSum.getProperty() != null) {
				produceMaterialSum.setProperty((SystemCode) systemCodeService.getSystemCode(produceMaterialSum.getProperty().getId()));
			}
		} else {
			if(id > 0){
				produceMaterialSum = produceMaterialSumService.getProduceMaterialSum(id, viewCode);
			}
		}
	...
	public Page<WOMProduceMaterialSum> getPage() {
		if (null == page) {
			page = new Page<WOMProduceMaterialSum>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProduceMaterialSum getProduceMaterialSum() {
		return produceMaterialSum;
	}		

	public void setProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum) {
		this.produceMaterialSum = produceMaterialSum;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/produceMaterialSum/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = produceMaterialSumService.getProduceMaterialSumAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/produceMaterialSum/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1489671141506
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String produceMaterialSumReferenceCopy() throws Exception{
			WOMProduceMaterialSum oldProduceMaterialSum = null;
			if(isEditNew()){
				...
				oldProduceMaterialSum = new WOMProduceMaterialSum();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,produceMaterialSum.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, produceMaterialSum.getId());
				...
				oldProduceMaterialSum = produceMaterialSumService.getProduceMaterialSum(id);
			}
			
			
			produceMaterialSum = produceMaterialSumService.getProduceMaterialSum(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceMaterialSum");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProduceMaterialSum.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProduceMaterialSum, null);
						Method setValue = WOMProduceMaterialSum.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(produceMaterialSum, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			produceMaterialSum.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=produceMaterialSumService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return produceMaterialSumReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			produceMaterialSum = produceMaterialSumService.getProduceMaterialSum(this.produceMaterialSum.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,produceMaterialSum.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			produceMaterialSumService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = produceMaterialSumService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(produceMaterialSumService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(produceMaterialSumService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_ProduceMaterialSum", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"produceMaterialSum\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"produceMaterialSum\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"produceMaterialSum\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"produceMaterialSum\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			produceMaterialSumService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		produceMaterialSumService.findDataGridPage(dg,dgclass,getDgPage(),produceMaterialSum,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(produceMaterialSum.getId() != null && produceMaterialSum.getId() != -1){
				produceMaterialSum = produceMaterialSumService.getProduceMaterialSum(produceMaterialSum.getId());
			}
			produceMaterialSumService.findDataGridPage(dg,dgclass,getDgPage(),produceMaterialSum,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == produceMaterialSum.getId()) {
		...
		produceMaterialSumService.saveProduceMaterialSum(produceMaterialSum,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "submit");
		bap_validate_datagrids=produceMaterialSumService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", produceMaterialSum.getId());
		...
		fileuploads.put("linkId", produceMaterialSum.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/produceMaterialSum/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		produceMaterialSumService.deleteProduceMaterialSum(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceMaterialSum/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "restore");
		produceMaterialSumService.restoreProduceMaterialSum(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProduceMaterialSum> findExportDataInfos(Page<WOMProduceMaterialSum> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = produceMaterialSumService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = produceMaterialSumService.getAssProperty(p.getCode());	
									//String assModelCode = produceMaterialSumService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			produceMaterialSumService.findProduceMaterialSums(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			produceMaterialSumService.findProduceMaterialSums(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProduceMaterialSum> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProduceMaterialSum result : results){
	...
	public boolean isEditNew(){
		//return null == this.produceMaterialSum || null == this.produceMaterialSum.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/produceMaterialSum/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/produceMaterialSum/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/produceMaterialSum/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceMaterialSum/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProduceMaterialSumService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceMaterialSum/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_ProduceMaterialSum", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_ProduceMaterialSum", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/produceMaterialSum/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/produceMaterialSum/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_ProduceMaterialSum";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceMaterialSum", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/produceMaterialSum/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
		_auditLog.setColumnStringA(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
		_auditLog.setColumnStringC(produceMaterialSum.getId().toString());
		_auditLog.setColumnStringD(WOMProduceMaterialSum.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProduceMaterialSumAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, produceMaterialSum,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, produceMaterialSum,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,produceMaterialSum);
		...
		printlog.setBusinessId(produceMaterialSum.getId());
		printlog.setBussinessKey(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
		printlog.setMainDisplay(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/produceMaterialSum/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(produceMaterialSum,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceMaterialSum/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(produceMaterialSumService.getProduceMaterialSum(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/produceMaterialSum/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/produceMaterialSum/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, produceMaterialSum,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				produceMaterialSum = produceMaterialSumService.getProduceMaterialSum(Long.parseLong(arrId[i]));
				if(null != produceMaterialSum){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == produceMaterialSum.getId() ? "" : "," + produceMaterialSum.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == produceMaterialSum.getId() ? "" : "," + produceMaterialSum.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == produceMaterialSum.getId() ? "" : "," + produceMaterialSum.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, produceMaterialSum,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,produceMaterialSum);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//produceMaterialSumService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/produceMaterialSum/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceMaterialSum");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_ProduceMaterialSum")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_ProduceMaterialSum");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceMaterialSum");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = produceMaterialSumService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
	private Long headId;
	...
	/**
	  ...
	  * @throws Exception
	  */
	@Action(value = "/WOM/produceTask/produceMaterialSum/deleteByheadId", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag" }) })
	public String deleteByheadId() throws Exception{
		produceMaterialSumService.deleteByheadId(headId);
		responseMap.put("dealSuccessFlag", true);
		return SUCCESS;
	}
/* CUSTOM CODE END */
