...
@Repository
public class WOMMaterialsOutputDaoImpl extends ExtendedGenericDaoImpl<WOMMaterialsOutput, Long> implements WOMMaterialsOutputDao {
	@Override
	...
	@Override
	public WOMMaterialsOutputDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMMaterialsOutputDealInfo dealInfo = new WOMMaterialsOutputDealInfo();
	...
	@Override
	public WOMMaterialsOutputDealInfo copyAndSaveDealInfo(DealInfo di, WOMMaterialsOutput mainObj, Staff dealStaff) {
		WOMMaterialsOutputDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMMaterialsOutputDealInfo.class);
		...
		List<WOMMaterialsOutputDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
