...
import com.supcon.orchid.WOM.entities.WOMStandingcropRefMneCode;
import com.supcon.orchid.WOM.services.WOMStandingcropRefService;
import com.supcon.orchid.WOM.services.WOMStandingcropRefImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMStandingcropRefDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_standingcropRefService")
@Transactional
public class WOMStandingcropRefServiceImpl  extends BaseServiceImpl<WOMStandingcropRef> implements WOMStandingcropRefService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMStandingcropRef> bapBaseService;
	...
	@Autowired
	private WOMStandingcropRefImportService standingcropRefImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMStandingcropRef.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  standingcropRefDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMStandingcropRef.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = standingcropRefDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMStandingcropRef.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMStandingcropRef.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  standingcropRefDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMStandingcropRef.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMStandingcropRef.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMStandingcropRef> list = (List<WOMStandingcropRef>) standingcropRefDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMStandingcropRef getStandingcropRef(long id){
		return getStandingcropRef(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMStandingcropRef getStandingcropRef(long id, String viewCode){
		WOMStandingcropRef standingcropRef = standingcropRefDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getStandingcropRefAsJSON(long id, String include){
		WOMStandingcropRef standingcropRef = standingcropRefDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(standingcropRef, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.common.delete",operType="3")
	public void deleteStandingcropRef(WOMStandingcropRef standingcropRef){
		if(null != standingcropRef && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMStandingcropRef.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == standingcropRef.getId() ? "" : standingcropRef.getId().toString());
			AuditUtil.setColumnStringA(null == standingcropRef.getBatchText() ? "" : standingcropRef.getBatchText().toString());
			AuditUtil.setColumnStringC(standingcropRef.getId().toString());
		}
		beforeDeleteStandingcropRef(standingcropRef);
		standingcropRefDao.delete(standingcropRef);
		afterDeleteStandingcropRef(standingcropRef);
		standingcropRefDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/standingcropRef/del", standingcropRef,WOMStandingcropRefMneCode.class);
		List<WOMStandingcropRef> params = new ArrayList<WOMStandingcropRef>();
		params.add(standingcropRef);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_standingcropRef");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.common.delete",operType="3")
	public void deleteStandingcropRef(List<Long> standingcropRefIds){
		deleteStandingcropRef(standingcropRefIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.common.delete",operType="3")
	public void deleteStandingcropRef(List<Long> standingcropRefIds, String eventTopic) {
		List<WOMStandingcropRef> standingcropRefs = new ArrayList<WOMStandingcropRef>();
		for(Long standingcropRefId : standingcropRefIds){
			WOMStandingcropRef standingcropRef = getStandingcropRef(standingcropRefId);
			standingcropRefs.add(standingcropRef);
			if(null != standingcropRef && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMStandingcropRef.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == standingcropRef.getBatchText() ? "" : "," + standingcropRef.getBatchText().toString()));
				} else {
					AuditUtil.setColumnStringA(null == standingcropRef.getBatchText() ? "" : standingcropRef.getBatchText().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == standingcropRef.getId() ? "" : "," + standingcropRef.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == standingcropRef.getId() ? "" : standingcropRef.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == standingcropRef.getId() ? "" : "," + standingcropRef.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == standingcropRef.getId() ? "" : standingcropRef.getId().toString());
				}
			}
		}
		...
		if(standingcropRefs != null && standingcropRefs.size() > 0 ){
			for(WOMStandingcropRef standingcropRef : standingcropRefs){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),standingcropRef, standingcropRef.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(standingcropRefIds != null && standingcropRefIds.size() > 0) {
			String hql = "update " + WOMStandingcropRef.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = standingcropRefDao.createQuery(hql);
			query.setParameterList("ids", standingcropRefIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMStandingcropRef standingcropRefz : standingcropRefs){
				standingcropRefz.setValid(false);
				List<WOMStandingcropRef> params = new ArrayList<WOMStandingcropRef>();
				params.add(standingcropRefz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_standingcropRef");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.common.delete",operType="3")
	public void deleteStandingcropRef(long standingcropRefId, int standingcropRefVersion){
		this.deleteStandingcropRef(standingcropRefId, standingcropRefVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.common.delete",operType="3")
	public void deleteStandingcropRef(long standingcropRefId, int standingcropRefVersion,SignatureLog signatureLog){
		WOMStandingcropRef standingcropRef = getStandingcropRef(standingcropRefId);
		...
		beforeDeleteStandingcropRef(standingcropRef);
		standingcropRefDao.delete(standingcropRefId, standingcropRefVersion);
		afterDeleteStandingcropRef(standingcropRef);
		standingcropRefDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/standingcropRef/del", standingcropRef,WOMStandingcropRefMneCode.class);
		List<WOMStandingcropRef> params = new ArrayList<WOMStandingcropRef>();
		...
		if(signatureLog != null ) {
			...
			businessKey=standingcropRef.getId();
			signatureLog.setTableId(standingcropRef.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.common.delete",operType="3")
	public void deleteStandingcropRef(String standingcropRefIds){
		this.deleteStandingcropRef(standingcropRefIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.common.delete",operType="3")
	public void deleteStandingcropRef(String standingcropRefIds,SignatureLog signatureLog){
		deleteCollection(standingcropRefIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteStandingcropRef(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_standingcropRef#modelCode:WOM_7.5.0.0_standingcropRef_StandingcropRef#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="ec.engine.restore",operType="10")
	public void restoreStandingcropRef(String standingcropRefIds){
		restoreCollection(standingcropRefIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreStandingcropRef(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="ec.engine.restore",operType="10")
	public void restoreStandingcropRef(long standingcropRefId){
		
		findBusinessKeyUsed(standingcropRefId);	//判断业务主键是否重复
		...
		beforeRestoreStandingcropRef(standingcropRef);
		standingcropRef.setValid(true);
		standingcropRefDao.update(standingcropRef);
		afterRestoreStandingcropRef(standingcropRef);
		standingcropRefDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/standingcropRef/add", standingcropRef,WOMStandingcropRefMneCode.class);
		List<WOMStandingcropRef> params = new ArrayList<WOMStandingcropRef>();
		params.add(standingcropRef);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long standingcropRefId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_standingcropRef_StandingcropRef");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMStandingcropRef.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMStandingcropRef.TABLE_NAME +" where id = ? )";
			List<Object> list =  standingcropRefDao.createNativeQuery(sql,standingcropRefId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseStandingcropRef(List<WOMStandingcropRef>  standingcropRefs){
		for(WOMStandingcropRef standingcropRef:standingcropRefs)  {
			saveStandingcropRef(standingcropRef, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseStandingcropRef(List<WOMStandingcropRef>  standingcropRefs){
		for(WOMStandingcropRef standingcropRef:standingcropRefs)  {
			saveStandingcropRef(standingcropRef, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="ec.flow.save")
	public void saveStandingcropRef(WOMStandingcropRef standingcropRef, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveStandingcropRef(standingcropRef, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="ec.flow.save")
	public void saveStandingcropRef(WOMStandingcropRef standingcropRef, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveStandingcropRef(standingcropRef, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateStandingcropRefCodes(WOMStandingcropRef standingcropRef) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateStandingcropRefCodes(standingcropRef, false);
	}
	
	public void generateStandingcropRefCodes(WOMStandingcropRef standingcropRef, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateStandingcropRefSummarys(WOMStandingcropRef standingcropRef) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateStandingcropRefSummarys(standingcropRef, false);
	}

	public void generateStandingcropRefSummarys(WOMStandingcropRef standingcropRef, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef")
	public void saveStandingcropRef(WOMStandingcropRef standingcropRef,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveStandingcropRef(standingcropRef, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef")
	public void saveStandingcropRef(WOMStandingcropRef standingcropRef,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_standingcropRef";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_standingcropRef");
		String url = null;
		if(standingcropRef.getId() != null && standingcropRef.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMStandingcropRef.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_standingcropRef/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMStandingcropRef.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_standingcropRef/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, standingcropRef);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(standingcropRef);
		beforeSaveStandingcropRef(standingcropRef, viewIsView);

		if (viewIsView) {
			standingcropRefDao.saveWithRevertVersion(standingcropRef);
		} else {
			if(isNew)
		...
		dealDatagridsSave(standingcropRef,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateStandingcropRefCodes(standingcropRef, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateStandingcropRefSummarys(standingcropRef, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveStandingcropRef(standingcropRef, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, standingcropRef);
			}
		}
		List<WOMStandingcropRef> params = new ArrayList<WOMStandingcropRef>();
		standingcropRefDao.flush();
		standingcropRefDao.clear();
		standingcropRef = standingcropRefDao.load(standingcropRef.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMStandingcropRef standingcropRef){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("standingcropRef", standingcropRef);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef")
	public void mergeStandingcropRef(WOMStandingcropRef standingcropRef, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(standingcropRef);
		beforeSaveStandingcropRef(standingcropRef);
		standingcropRefDao.merge(standingcropRef);
		if(null != standingcropRef && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMStandingcropRef.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == standingcropRef.getId() ? "" : standingcropRef.getId().toString());
			AuditUtil.setColumnStringA(null == standingcropRef.getBatchText() ? "" : standingcropRef.getBatchText().toString());
		...
		dealDatagridsSave(standingcropRef,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveStandingcropRef(standingcropRef);
		standingcropRefDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/standingcropRef/add", standingcropRef,WOMStandingcropRefMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMStandingcropRef> findStandingcropRefs(Page<WOMStandingcropRef> page, Criterion... criterions) {
		return standingcropRefDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMStandingcropRefDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMStandingcropRefDealInfo> list=standingcropRefDao.createQuery("from " + WOMStandingcropRefDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMStandingcropRefDealInfo dealInfo) {
		standingcropRefDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMStandingcropRefDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) standingcropRefDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMStandingcropRefDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = standingcropRefDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = standingcropRefDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMStandingcropRefDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = standingcropRefDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStandingcropRefs(Page<WOMStandingcropRef> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findStandingcropRefs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStandingcropRefs(Page<WOMStandingcropRef> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStandingcropRefs(Page<WOMStandingcropRef> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMStandingcropRef> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"standingcropRef\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"standingcropRef\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"standingcropRef\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_standingcropRef_StandingcropRef" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_standingcropRef_StandingcropRef" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  standingcropRefDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  standingcropRefDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_standingcropRef").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"standingcropRef\"","WOM_7.5.0.0_standingcropRef",customCondition,"MATERIAL_STANDINGCROPS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"standingcropRef\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMStandingcropRefDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"standingcropRef\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"standingcropRef\".LAY_REC ASC, \"standingcropRef\".SORT ASC");
				} else {
					orderPart.append(" \"standingcropRef\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) standingcropRefDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=standingcropRefDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = standingcropRefDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMStandingcropRef> result = new ArrayList<WOMStandingcropRef>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMStandingcropRef.class,standingcropRefDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMStandingcropRef.class,standingcropRefDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMStandingcropRef standingcropRef : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(standingcropRef.getId(), "WOM_standingcropRef_standingcropRef");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(standingcropRef.getId(), "WOM_standingcropRef_standingcropRef");
							if(null != documents && !documents.isEmpty()) {
								standingcropRef.setDocument(documents.get(0));
								standingcropRef.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_standingcropRef")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_standingcropRef")+"WOM_7.5.0.0_standingcropRef".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_standingcropRef",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMStandingcropRef> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMStandingcropRef> result) {
		View view = null;
		...
		Map<Long, WOMStandingcropRef> rsMap = new HashMap<Long, WOMStandingcropRef>();
		for (WOMStandingcropRef standingcropRef : result) {
			ids.add(standingcropRef.getId());
			rsMap.put(standingcropRef.getId(), standingcropRef);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(standingcropRefDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = standingcropRefDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMStandingcropRef standingcropRef,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(standingcropRef.getOwnerStaffId());
		ti.setOwnerPositionId(standingcropRef.getOwnerPositionId());
		ti.setOwnerDepartmentId(standingcropRef.getOwnerDepartmentId());
		
		ti.setCreateStaff(standingcropRef.getCreateStaff());
		ti.setCreateStaffId(standingcropRef.getCreateStaffId());
		ti.setCreateTime(standingcropRef.getCreateTime());
		ti.setDeleteStaff(standingcropRef.getDeleteStaff());
		ti.setDeleteStaffId(standingcropRef.getDeleteStaffId());
		ti.setDeleteTime(standingcropRef.getDeleteTime());
		ti.setCreateDepartmentId(standingcropRef.getCreateDepartmentId());
		ti.setModifyStaff(standingcropRef.getModifyStaff());
		ti.setModifyStaffId(standingcropRef.getModifyStaffId());
		ti.setCreatePositionId(standingcropRef.getCreatePositionId());
		ti.setPositionLayRec(standingcropRef.getPositionLayRec());
		ti.setTableNo(standingcropRef.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_standingcropRef");
		ti.setTargetTableName("MATERIAL_STANDINGCROPS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_standingcropRef_StandingcropRef" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  standingcropRefDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"standingcropRef\"","WOM_7.5.0.0_standingcropRef","","MATERIAL_STANDINGCROPS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"standingcropRef\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"standingcropRef\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"standingcropRef\"","WOM_7.5.0.0_standingcropRef","","MATERIAL_STANDINGCROPS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("standingcropRef.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"standingcropRef\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"standingcropRef\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"standingcropRef\".CID = 1 OR \"standingcropRef\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("standingcropRef.mc\"").append(".STANDINGCROP_REF) not in ("+notinValue+")");
			}
			
			SQLQuery query = standingcropRefDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("standingcropRef", "WOM_7.5.0.0_standingcropRef", "standingcropRef_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=standingcropRefDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMStandingcropRef.ENTITY_CODE;}
	protected String getModuleCode(){return WOMStandingcropRef.MODULE_CODE;}
	protected String getModelCode(){return WOMStandingcropRef.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param standingcropRef 模型实例
	 */
	public void checkUniqueConstraint(WOMStandingcropRef standingcropRef){
	}
	
	
	@Override
	public List<WOMStandingcropRef> getStandingcropRefs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMStandingcropRef.class);
		if(params!=null){
			...
			return standingcropRefDao.findByHql("from " + WOMStandingcropRef.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return standingcropRefDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMStandingcropRef> findStandingcropRefsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMStandingcropRef.class);
	...
	@Override
	public List<WOMStandingcropRef> findStandingcropRefsByHql(String hql, Object... objects){
		
		return   standingcropRefDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMStandingcropRef loadStandingcropRefByBussinessKey(WOMStandingcropRef standingcropRef){
		return loadStandingcropRefByBussinessKey(standingcropRef.getId());
	}
	
	@Override
	public Page<WOMStandingcropRef> getStandingcropRefs(Page<WOMStandingcropRef> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMStandingcropRef.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = standingcropRefDao.findByPage(page, sql);
			}else{
				page = standingcropRefDao.findAllByPage(page);
			}
			return page;
		}
		page = standingcropRefDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMStandingcropRef loadStandingcropRefByBussinessKey(Serializable bussinessKey){
		return standingcropRefDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMStandingcropRef> findByProperty(String propertyName, Object object){
		return standingcropRefDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMStandingcropRef findEntityByProperty(String propertyName, Object object){
		return standingcropRefDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMStandingcropRef.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = standingcropRefDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_standingcropRef_StandingcropRef","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_standingcropRef", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMStandingcropRef standingcropRef,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,standingcropRefDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMStandingcropRef.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, standingcropRef);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",standingcropRef.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMStandingcropRef.TABLE_NAME +" where valid = 1";
			list = standingcropRefDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMStandingcropRef.TABLE_NAME;
			list = standingcropRefDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMStandingcropRef.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = standingcropRefDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMStandingcropRefDaoImpl standingcropRefDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchStandingcropRef(final List<WOMStandingcropRef> insertObjs, final List<WOMStandingcropRef> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchStandingcropRef(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchStandingcropRef(final List<WOMStandingcropRef> insertObjs, final List<WOMStandingcropRef> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1490064355590"),importCount));
		...
		Map<Long, WOMStandingcropRef> oldUpdateObjsMap = new HashMap<Long, WOMStandingcropRef>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMStandingcropRef bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMStandingcropRef> bizObjList = standingcropRefDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMStandingcropRef bizObj = bizObjList.get(mIndex);
		...
		final List<WOMStandingcropRef> nmeObjs = new ArrayList<WOMStandingcropRef>();
		...
		long id=autoGeneratorID.getNextId(WOMStandingcropRef.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMStandingcropRef standingcropRef:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMStandingcropRef.TABLE_NAME,1,dbId);
			if(standingcropRef.getId() != null ){
				m.put(standingcropRef.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) standingcropRefDao.createNativeQuery(sql, "WOM_7.5.0.0_standingcropRef_StandingcropRef"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		standingcropRefDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMStandingcropRef.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMStandingcropRef standingcropRef = updateObjs.get(updateIndex);
						Long id = standingcropRef.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = standingcropRef.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(standingcropRef);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMStandingcropRef.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMStandingcropRef.TABLE_NAME + " order by id desc";
//					Query query = standingcropRefDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMStandingcropRef standingcropRef = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = standingcropRef.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = WOMStandingcropRefMneCode.class;
					...
					for(WOMStandingcropRef standingcropRef:updateObjs)  {
						Long id = standingcropRef.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(WOMStandingcropRef standingcropRef:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(standingcropRef);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, standingcropRef.getId());
		...
		standingcropRefDaoImpl.getSession().flush();
		standingcropRefDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMStandingcropRef bizObj = insertObjs.get(insertIndex);
			standingcropRefImportService.saveImportDataLog(bizObj, null, "id", "batchText");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMStandingcropRef bizObj = updateObjs.get(updateIndex);
			WOMStandingcropRef oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			standingcropRefImportService.saveImportDataLog(bizObj, oldBizObj, "id", "batchText");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = standingcropRefDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				standingcropRefDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = standingcropRefDao.createNativeQuery(sql, "WOM_7.5.0.0_standingcropRef_StandingcropRef").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = standingcropRefDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = standingcropRefDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = standingcropRefDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?standingcropRefDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  standingcropRefDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,standingcropRefDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="ec.flow.save")
	public void saveStandingcropRef(WOMStandingcropRef standingcropRef, DataGridService dataGridService){
		saveStandingcropRef(standingcropRef, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="ec.flow.save")
	public void saveStandingcropRef(WOMStandingcropRef standingcropRef, DataGridService dataGridService, String viewCode){
		saveStandingcropRef(standingcropRef, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef")
	public void saveStandingcropRef(WOMStandingcropRef standingcropRef, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveStandingcropRef(standingcropRef, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef")
	public void saveStandingcropRef(WOMStandingcropRef standingcropRef, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_standingcropRef";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_standingcropRef");
		String url = null;
		if(null != standingcropRef.getId() && standingcropRef.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMStandingcropRef.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef")
	public Map<Object, Object> standingcropRefDataGridImport(WOMStandingcropRef standingcropRef, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = standingcropRef.getVirtualId();
		
		standingcropRef = standingcropRefDao.load(standingcropRef.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, standingcropRef.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = standingcropRef.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(standingcropRef);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, standingcropRef.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef")
	public void mergeStandingcropRef(WOMStandingcropRef standingcropRef, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(standingcropRef);
		beforeSaveStandingcropRef(standingcropRef);
		standingcropRefDao.merge(standingcropRef);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="foundation.batch.save",operType="4")
	public void batchSaveStandingcropRef(WOMStandingcropRef standingcropRef, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_standingcropRef";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_standingcropRef");
	...
	@Override
	public Page<WOMStandingcropRef> getByPage(Page<WOMStandingcropRef> page,DetachedCriteria detachedCriteria){
		return standingcropRefDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_standingcropRef").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveStandingcropRef(WOMStandingcropRef standingcropRef, Object... objects){
		checkUniqueConstraint(standingcropRef);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveStandingcropRef(WOMStandingcropRef standingcropRef, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportStandingcropRef(List<WOMStandingcropRef> standingcropRefs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportStandingcropRef(List<WOMStandingcropRef> standingcropRefs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportStandingcropRef(List<WOMStandingcropRef> standingcropRefs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportStandingcropRef(List<WOMStandingcropRef> standingcropRefs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteStandingcropRef(WOMStandingcropRef standingcropRef){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteStandingcropRef(WOMStandingcropRef standingcropRef){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreStandingcropRef(WOMStandingcropRef standingcropRef){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreStandingcropRef(WOMStandingcropRef standingcropRef){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMStandingcropRef standingcropRef, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMStandingcropRef standingcropRef, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportStandingcropRef(List<WOMStandingcropRef> insertObjs, List<WOMStandingcropRef> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportStandingcropRef,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportStandingcropRef(List<WOMStandingcropRef> insertObjs, List<WOMStandingcropRef> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportStandingcropRef,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
