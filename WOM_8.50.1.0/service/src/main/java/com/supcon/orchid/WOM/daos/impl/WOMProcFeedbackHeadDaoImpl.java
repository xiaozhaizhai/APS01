...
@Repository
public class WOMProcFeedbackHeadDaoImpl extends ExtendedGenericDaoImpl<WOMProcFeedbackHead, Long> implements WOMProcFeedbackHeadDao {
	@Override
	...
	@Override
	public WOMProcFeedbackHeadDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMProcFeedbackHeadDealInfo dealInfo = new WOMProcFeedbackHeadDealInfo();
	...
	@Override
	public WOMProcFeedbackHeadDealInfo copyAndSaveDealInfo(DealInfo di, WOMProcFeedbackHead mainObj, Staff dealStaff) {
		WOMProcFeedbackHeadDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMProcFeedbackHeadDealInfo.class);
		...
		List<WOMProcFeedbackHeadDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
