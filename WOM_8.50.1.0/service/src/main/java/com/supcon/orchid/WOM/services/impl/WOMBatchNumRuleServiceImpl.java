...
import com.supcon.orchid.WOM.services.WOMBatchNumRuleImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchNumRuleDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_batchNumRuleService")
@Transactional
public class WOMBatchNumRuleServiceImpl  extends BaseServiceImpl<WOMBatchNumRule> implements WOMBatchNumRuleService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMBatchNumRule> bapBaseService;
	...
	@Autowired
	private WOMBatchNumRuleImportService batchNumRuleImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchNumRule.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchNumRuleDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchNumRule.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchNumRuleDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchNumRule.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchNumRule.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchNumRuleDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchNumRule.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchNumRule.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchNumRule> list = (List<WOMBatchNumRule>) batchNumRuleDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMBatchNumRule getBatchNumRule(long id){
		return getBatchNumRule(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchNumRule getBatchNumRule(long id, String viewCode){
		WOMBatchNumRule batchNumRule = batchNumRuleDao.load(id);

		// 一对多情况处理
		if(batchNumRule != null){
		}
		return batchNumRule;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchNumRuleAsJSON(long id, String include){
		WOMBatchNumRule batchNumRule = batchNumRuleDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchNumRule, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.common.delete",operType="3")
	public void deleteBatchNumRule(WOMBatchNumRule batchNumRule){
		if(null != batchNumRule && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchNumRule.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
			AuditUtil.setColumnStringA(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
			AuditUtil.setColumnStringC(batchNumRule.getId().toString());
		}
		beforeDeleteBatchNumRule(batchNumRule);
		batchNumRuleDao.delete(batchNumRule);
		afterDeleteBatchNumRule(batchNumRule);
		batchNumRuleDao.flush();
		List<WOMBatchNumRule> params = new ArrayList<WOMBatchNumRule>();
		params.add(batchNumRule);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchNumRule");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.common.delete",operType="3")
	public void deleteBatchNumRule(List<Long> batchNumRuleIds){
		deleteBatchNumRule(batchNumRuleIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.common.delete",operType="3")
	public void deleteBatchNumRule(List<Long> batchNumRuleIds, String eventTopic) {
		List<WOMBatchNumRule> batchNumRules = new ArrayList<WOMBatchNumRule>();
		for(Long batchNumRuleId : batchNumRuleIds){
			WOMBatchNumRule batchNumRule = getBatchNumRule(batchNumRuleId);
			batchNumRules.add(batchNumRule);
			if(null != batchNumRule && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchNumRule.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchNumRule.getId() ? "" : "," + batchNumRule.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchNumRule.getId() ? "" : "," + batchNumRule.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchNumRule.getId() ? "" : "," + batchNumRule.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchNumRules != null && batchNumRules.size() > 0 ){
			for(WOMBatchNumRule batchNumRule : batchNumRules){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchNumRule, batchNumRule.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(batchNumRuleIds != null && batchNumRuleIds.size() > 0) {
			String hql = "update " + WOMBatchNumRule.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = batchNumRuleDao.createQuery(hql);
			query.setParameterList("ids", batchNumRuleIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchNumRule batchNumRulez : batchNumRules){
				batchNumRulez.setValid(false);
				List<WOMBatchNumRule> params = new ArrayList<WOMBatchNumRule>();
				params.add(batchNumRulez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_batchNumRule");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.common.delete",operType="3")
	public void deleteBatchNumRule(long batchNumRuleId, int batchNumRuleVersion){
		this.deleteBatchNumRule(batchNumRuleId, batchNumRuleVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.common.delete",operType="3")
	public void deleteBatchNumRule(long batchNumRuleId, int batchNumRuleVersion,SignatureLog signatureLog){
		WOMBatchNumRule batchNumRule = getBatchNumRule(batchNumRuleId);
		if(batchNumRule != null && !batchNumRule.isValid()){
		...
		beforeDeleteBatchNumRule(batchNumRule);
		batchNumRuleDao.delete(batchNumRuleId, batchNumRuleVersion);
		afterDeleteBatchNumRule(batchNumRule);
		batchNumRuleDao.flush();
		List<WOMBatchNumRule> params = new ArrayList<WOMBatchNumRule>();
		params.add(batchNumRule);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchNumRule.getId();
			signatureLog.setTableId(batchNumRule.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.common.delete",operType="3")
	public void deleteBatchNumRule(String batchNumRuleIds){
		this.deleteBatchNumRule(batchNumRuleIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.common.delete",operType="3")
	public void deleteBatchNumRule(String batchNumRuleIds,SignatureLog signatureLog){
		deleteCollection(batchNumRuleIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchNumRule(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_batchNumRule#modelCode:WOM_7.5.0.0_batchNumRule_BatchNumRule#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="ec.engine.restore",operType="10")
	public void restoreBatchNumRule(String batchNumRuleIds){
		restoreCollection(batchNumRuleIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchNumRule(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="ec.engine.restore",operType="10")
	public void restoreBatchNumRule(long batchNumRuleId){
		
		findBusinessKeyUsed(batchNumRuleId);	//判断业务主键是否重复
		...
		beforeRestoreBatchNumRule(batchNumRule);
		batchNumRule.setValid(true);
		batchNumRuleDao.update(batchNumRule);
		afterRestoreBatchNumRule(batchNumRule);
		List<WOMBatchNumRule> params = new ArrayList<WOMBatchNumRule>();
		params.add(batchNumRule);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchNumRuleId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_batchNumRule_BatchNumRule");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchNumRule.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchNumRule.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchNumRuleDao.createNativeQuery(sql,batchNumRuleId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchNumRule(List<WOMBatchNumRule>  batchNumRules){
		for(WOMBatchNumRule batchNumRule:batchNumRules)  {
			saveBatchNumRule(batchNumRule, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchNumRule(List<WOMBatchNumRule>  batchNumRules){
		for(WOMBatchNumRule batchNumRule:batchNumRules)  {
			saveBatchNumRule(batchNumRule, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="ec.flow.save")
	public void saveBatchNumRule(WOMBatchNumRule batchNumRule, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchNumRule(batchNumRule, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="ec.flow.save")
	public void saveBatchNumRule(WOMBatchNumRule batchNumRule, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchNumRule(batchNumRule, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchNumRuleCodes(WOMBatchNumRule batchNumRule) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchNumRuleCodes(batchNumRule, false);
	}
	
	public void generateBatchNumRuleCodes(WOMBatchNumRule batchNumRule, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchNumRuleSummarys(WOMBatchNumRule batchNumRule) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchNumRuleSummarys(batchNumRule, false);
	}

	public void generateBatchNumRuleSummarys(WOMBatchNumRule batchNumRule, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule")
	public void saveBatchNumRule(WOMBatchNumRule batchNumRule,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchNumRule(batchNumRule, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule")
	public void saveBatchNumRule(WOMBatchNumRule batchNumRule,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchNumRule";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchNumRule");
		String url = null;
		if(batchNumRule.getId() != null && batchNumRule.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchNumRule.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_batchNumRule/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchNumRule.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_batchNumRule/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchNumRule);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchNumRule);
		beforeSaveBatchNumRule(batchNumRule, viewIsView);

		if (viewIsView) {
			batchNumRuleDao.saveWithRevertVersion(batchNumRule);
		} else {
			if(isNew)
				batchNumRuleDao.save(batchNumRule);
			else
				batchNumRuleDao.merge(batchNumRule);
		}
		if(null != batchNumRule && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchNumRule.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
			AuditUtil.setColumnStringA(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
		...
		dealDatagridsSave(batchNumRule,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchNumRuleCodes(batchNumRule, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchNumRuleSummarys(batchNumRule, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchNumRule(batchNumRule, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchNumRule);
			}
		}
		List<WOMBatchNumRule> params = new ArrayList<WOMBatchNumRule>();
		batchNumRuleDao.flush();
		batchNumRuleDao.clear();
		batchNumRule = batchNumRuleDao.load(batchNumRule.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchNumRule batchNumRule){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchNumRule", batchNumRule);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule")
	public void mergeBatchNumRule(WOMBatchNumRule batchNumRule, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchNumRule);
		beforeSaveBatchNumRule(batchNumRule);
		batchNumRuleDao.merge(batchNumRule);
		if(null != batchNumRule && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchNumRule.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
			AuditUtil.setColumnStringA(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
		...
		dealDatagridsSave(batchNumRule,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchNumRule(batchNumRule);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchNumRule> findBatchNumRules(Page<WOMBatchNumRule> page, Criterion... criterions) {
		return batchNumRuleDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMBatchNumRuleDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMBatchNumRuleDealInfo> list=batchNumRuleDao.createQuery("from " + WOMBatchNumRuleDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMBatchNumRuleDealInfo dealInfo) {
		batchNumRuleDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMBatchNumRuleDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) batchNumRuleDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMBatchNumRuleDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = batchNumRuleDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = batchNumRuleDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMBatchNumRuleDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = batchNumRuleDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchNumRules(Page<WOMBatchNumRule> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchNumRules(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchNumRules(Page<WOMBatchNumRule> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchNumRules(Page<WOMBatchNumRule> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchNumRule> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchNumRule\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchNumRule\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchNumRule\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_batchNumRule_BatchNumRule" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_batchNumRule_BatchNumRule" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  batchNumRuleDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchNumRuleDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_batchNumRule").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchNumRule\"","WOM_7.5.0.0_batchNumRule",customCondition,"WOM_BATCH_NUM_RULES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"batchNumRule\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMBatchNumRuleDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"batchNumRule\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchNumRule\".LAY_REC ASC, \"batchNumRule\".SORT ASC");
				} else {
					orderPart.append(" \"batchNumRule\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchNumRuleDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchNumRuleDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchNumRuleDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchNumRule> result = new ArrayList<WOMBatchNumRule>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchNumRule.class,batchNumRuleDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchNumRule.class,batchNumRuleDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchNumRule batchNumRule : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchNumRule.getId(), "WOM_batchNumRule_batchNumRule");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchNumRule.getId(), "WOM_batchNumRule_batchNumRule");
							if(null != documents && !documents.isEmpty()) {
								batchNumRule.setDocument(documents.get(0));
								batchNumRule.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_batchNumRule")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_batchNumRule")+"WOM_7.5.0.0_batchNumRule".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_batchNumRule",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchNumRule> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchNumRule> result) {
		View view = null;
		...
		Map<Long, WOMBatchNumRule> rsMap = new HashMap<Long, WOMBatchNumRule>();
		for (WOMBatchNumRule batchNumRule : result) {
			ids.add(batchNumRule.getId());
			rsMap.put(batchNumRule.getId(), batchNumRule);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchNumRuleDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchNumRuleDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMBatchNumRule batchNumRule,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(batchNumRule.getOwnerStaffId());
		ti.setOwnerPositionId(batchNumRule.getOwnerPositionId());
		ti.setOwnerDepartmentId(batchNumRule.getOwnerDepartmentId());
		
		ti.setCreateStaff(batchNumRule.getCreateStaff());
		ti.setCreateStaffId(batchNumRule.getCreateStaffId());
		ti.setCreateTime(batchNumRule.getCreateTime());
		ti.setDeleteStaff(batchNumRule.getDeleteStaff());
		ti.setDeleteStaffId(batchNumRule.getDeleteStaffId());
		ti.setDeleteTime(batchNumRule.getDeleteTime());
		ti.setCreateDepartmentId(batchNumRule.getCreateDepartmentId());
		ti.setModifyStaff(batchNumRule.getModifyStaff());
		ti.setModifyStaffId(batchNumRule.getModifyStaffId());
		ti.setCreatePositionId(batchNumRule.getCreatePositionId());
		ti.setPositionLayRec(batchNumRule.getPositionLayRec());
		ti.setTableNo(batchNumRule.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_batchNumRule");
		ti.setTargetTableName("WOM_BATCH_NUM_RULES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_batchNumRule_BatchNumRule" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchNumRuleDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchNumRule\"","WOM_7.5.0.0_batchNumRule","","WOM_BATCH_NUM_RULES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchNumRule\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchNumRule\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchNumRule\"","WOM_7.5.0.0_batchNumRule","","WOM_BATCH_NUM_RULES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("batchNumRule.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchNumRule\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchNumRule\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchNumRule\".CID = 1 OR \"batchNumRule\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchNumRule.mc\"").append(".BATCH_NUM_RULE) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchNumRuleDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("batchNumRule", "WOM_7.5.0.0_batchNumRule", "batchNumRule_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchNumRuleDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMBatchNumRule.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchNumRule.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchNumRule.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchNumRule 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchNumRule batchNumRule){
	}
	
	
	@Override
	public List<WOMBatchNumRule> getBatchNumRules(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchNumRule.class);
		if(params!=null){
			...
			return batchNumRuleDao.findByHql("from " + WOMBatchNumRule.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchNumRuleDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMBatchNumRule> findBatchNumRulesByHql(String hql, Object... objects){
		...
		return   batchNumRuleDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchNumRule loadBatchNumRuleByBussinessKey(WOMBatchNumRule batchNumRule){
		return loadBatchNumRuleByBussinessKey(batchNumRule.getId());
	}
	
	@Override
	public Page<WOMBatchNumRule> getBatchNumRules(Page<WOMBatchNumRule> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchNumRule.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchNumRuleDao.findByPage(page, sql);
			}else{
				page = batchNumRuleDao.findAllByPage(page);
			}
			return page;
		}
		page = batchNumRuleDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public WOMBatchNumRule loadBatchNumRuleByBussinessKey(Serializable bussinessKey){
		return batchNumRuleDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchNumRule> findByProperty(String propertyName, Object object){
		return batchNumRuleDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchNumRule findEntityByProperty(String propertyName, Object object){
		return batchNumRuleDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchNumRule.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchNumRuleDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_batchNumRule_BatchNumRule","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_batchNumRule", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMBatchNumRule batchNumRule,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchNumRuleDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchNumRule.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchNumRule);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchNumRule.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchNumRule.TABLE_NAME +" where valid = 1";
			list = batchNumRuleDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchNumRule.TABLE_NAME;
			list = batchNumRuleDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchNumRule.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchNumRuleDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchNumRuleDaoImpl batchNumRuleDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchNumRule(final List<WOMBatchNumRule> insertObjs, final List<WOMBatchNumRule> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchNumRule(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchNumRule(final List<WOMBatchNumRule> insertObjs, final List<WOMBatchNumRule> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1490408991387"),importCount));
		...
		Map<Long, WOMBatchNumRule> oldUpdateObjsMap = new HashMap<Long, WOMBatchNumRule>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchNumRule bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchNumRule> bizObjList = batchNumRuleDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchNumRule bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchNumRule> nmeObjs = new ArrayList<WOMBatchNumRule>();
		...
		long id=autoGeneratorID.getNextId(WOMBatchNumRule.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchNumRule batchNumRule:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchNumRule.TABLE_NAME,1,dbId);
			if(batchNumRule.getId() != null ){
				m.put(batchNumRule.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchNumRuleDao.createNativeQuery(sql, "WOM_7.5.0.0_batchNumRule_BatchNumRule"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchNumRuleDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchNumRule.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMBatchNumRule batchNumRule = updateObjs.get(updateIndex);
						Long id = batchNumRule.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchNumRule.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchNumRule);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchNumRule.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMBatchNumRule.TABLE_NAME + " order by id desc";
//					Query query = batchNumRuleDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMBatchNumRule batchNumRule = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchNumRule.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchNumRuleDaoImpl.getSession().flush();
		batchNumRuleDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchNumRule bizObj = insertObjs.get(insertIndex);
			batchNumRuleImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchNumRule bizObj = updateObjs.get(updateIndex);
			WOMBatchNumRule oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchNumRuleImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = batchNumRuleDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				batchNumRuleDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = batchNumRuleDao.createNativeQuery(sql, "WOM_7.5.0.0_batchNumRule_BatchNumRule").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchNumRuleDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchNumRuleDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchNumRuleDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchNumRuleDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchNumRuleDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchNumRuleDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="ec.flow.save")
	public void saveBatchNumRule(WOMBatchNumRule batchNumRule, DataGridService dataGridService){
		saveBatchNumRule(batchNumRule, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="ec.flow.save")
	public void saveBatchNumRule(WOMBatchNumRule batchNumRule, DataGridService dataGridService, String viewCode){
		saveBatchNumRule(batchNumRule, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule")
	public void saveBatchNumRule(WOMBatchNumRule batchNumRule, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchNumRule(batchNumRule, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule")
	public void saveBatchNumRule(WOMBatchNumRule batchNumRule, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchNumRule";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchNumRule");
		String url = null;
		if(null != batchNumRule.getId() && batchNumRule.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchNumRule.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule")
	public Map<Object, Object> batchNumRuleDataGridImport(WOMBatchNumRule batchNumRule, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchNumRule.getVirtualId();
		
		batchNumRule = batchNumRuleDao.load(batchNumRule.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchNumRule.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchNumRule.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchNumRule);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchNumRule.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule")
	public void mergeBatchNumRule(WOMBatchNumRule batchNumRule, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchNumRule);
		beforeSaveBatchNumRule(batchNumRule);
		batchNumRuleDao.merge(batchNumRule);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchNumRule",model="WOM_7.5.0.0_batchNumRule_BatchNumRule",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMBatchNumRule> getByPage(Page<WOMBatchNumRule> page,DetachedCriteria detachedCriteria){
		return batchNumRuleDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_batchNumRule").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBatchNumRule(WOMBatchNumRule batchNumRule, Object... objects){
		checkUniqueConstraint(batchNumRule);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
      	String hqlString = "from WOMBatchNumRule where valid = 1 and ruleType = ? ";
		List<WOMBatchNumRule> batchNumRuleList = produceTaskDao.findByHql(hqlString, batchNumRule.getRuleType());
		if(batchNumRule.getId()==null && batchNumRuleList!=null && batchNumRuleList.size()>0){
			throw new BAPException("WOM.showMessage00000050");
		}
		if(batchNumRule.getId()!=null && batchNumRuleList!=null && batchNumRuleList.size()>1){
			throw new BAPException("WOM.showMessage00000050");
		}
/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchNumRule(WOMBatchNumRule batchNumRule, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchNumRule(WOMBatchNumRule batchNumRule){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchNumRule(WOMBatchNumRule batchNumRule){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchNumRule(WOMBatchNumRule batchNumRule){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchNumRule(WOMBatchNumRule batchNumRule){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchNumRule batchNumRule, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchNumRule batchNumRule, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportBatchNumRule(List<WOMBatchNumRule> insertObjs, List<WOMBatchNumRule> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchNumRule,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportBatchNumRule(List<WOMBatchNumRule> insertObjs, List<WOMBatchNumRule> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchNumRule,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
