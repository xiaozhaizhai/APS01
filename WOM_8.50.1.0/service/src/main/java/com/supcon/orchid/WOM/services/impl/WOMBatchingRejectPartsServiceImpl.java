...
import com.supcon.orchid.WOM.services.WOMBatchingRejectPartsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchingRejectPartsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_batchingRejectPartsService")
@Transactional
public class WOMBatchingRejectPartsServiceImpl  extends BaseServiceImpl<WOMBatchingRejectParts> implements WOMBatchingRejectPartsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMBatchingRejectParts> bapBaseService;
	...
	@Autowired
	private WOMBatchingRejectPartsImportService batchingRejectPartsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchingRejectParts.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchingRejectPartsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchingRejectParts.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchingRejectPartsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchingRejectParts.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchingRejectParts.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchingRejectPartsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchingRejectParts.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchingRejectParts.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchingRejectParts> list = (List<WOMBatchingRejectParts>) batchingRejectPartsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingRejectParts getBatchingRejectParts(long id){
		return getBatchingRejectParts(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingRejectParts getBatchingRejectParts(long id, String viewCode){
		WOMBatchingRejectParts batchingRejectParts = batchingRejectPartsDao.load(id);

		// 一对多情况处理
		if(batchingRejectParts != null){
		}
		return batchingRejectParts;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchingRejectPartsAsJSON(long id, String include){
		WOMBatchingRejectParts batchingRejectParts = batchingRejectPartsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchingRejectParts, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.common.delete",operType="3")
	public void deleteBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts){
		if(null != batchingRejectParts && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingRejectParts.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
			AuditUtil.setColumnStringA(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
			AuditUtil.setColumnStringC(batchingRejectParts.getId().toString());
		}
		beforeDeleteBatchingRejectParts(batchingRejectParts);
		batchingRejectPartsDao.deletePhysical(batchingRejectParts);
		afterDeleteBatchingRejectParts(batchingRejectParts);
		batchingRejectPartsDao.flush();
		List<WOMBatchingRejectParts> params = new ArrayList<WOMBatchingRejectParts>();
		params.add(batchingRejectParts);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.common.delete",operType="3")
	public void deleteBatchingRejectParts(List<Long> batchingRejectPartsIds){
		deleteBatchingRejectParts(batchingRejectPartsIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.common.delete",operType="3")
	public void deleteBatchingRejectParts(List<Long> batchingRejectPartsIds, String eventTopic) {
		List<WOMBatchingRejectParts> batchingRejectPartss = new ArrayList<WOMBatchingRejectParts>();
		for(Long batchingRejectPartsId : batchingRejectPartsIds){
			WOMBatchingRejectParts batchingRejectParts = getBatchingRejectParts(batchingRejectPartsId);
			batchingRejectPartss.add(batchingRejectParts);
			if(null != batchingRejectParts && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingRejectParts.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchingRejectParts.getId() ? "" : "," + batchingRejectParts.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchingRejectParts.getId() ? "" : "," + batchingRejectParts.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchingRejectParts.getId() ? "" : "," + batchingRejectParts.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
				}
			}
		}
		
		for(WOMBatchingRejectParts batchingRejectParts : batchingRejectPartss){
			beforeDeleteBatchingRejectParts(batchingRejectParts);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchingRejectPartss != null && batchingRejectPartss.size() > 0 ){
			for(WOMBatchingRejectParts batchingRejectParts : batchingRejectPartss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMBatchingRejectParts.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchingRejectParts));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchingRejectParts, batchingRejectParts.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchingRejectPartsIds != null && batchingRejectPartsIds.size() > 0) {
			String hql = "delete " + WOMBatchingRejectParts.JPA_NAME + " where id in(:ids)";
			Query query = batchingRejectPartsDao.createQuery(hql);
			query.setParameterList("ids", batchingRejectPartsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchingRejectParts batchingRejectPartsz : batchingRejectPartss){
				batchingRejectPartsz.setValid(false);
				List<WOMBatchingRejectParts> params = new ArrayList<WOMBatchingRejectParts>();
				params.add(batchingRejectPartsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_batchingMaterial");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.common.delete",operType="3")
	public void deleteBatchingRejectParts(long batchingRejectPartsId, int batchingRejectPartsVersion){
		this.deleteBatchingRejectParts(batchingRejectPartsId, batchingRejectPartsVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.common.delete",operType="3")
	public void deleteBatchingRejectParts(long batchingRejectPartsId, int batchingRejectPartsVersion,SignatureLog signatureLog){
		WOMBatchingRejectParts batchingRejectParts = getBatchingRejectParts(batchingRejectPartsId);
		if(batchingRejectParts != null && !batchingRejectParts.isValid()){
		...
		beforeDeleteBatchingRejectParts(batchingRejectParts);
		batchingRejectPartsDao.delete(batchingRejectPartsId, batchingRejectPartsVersion);
		afterDeleteBatchingRejectParts(batchingRejectParts);
		batchingRejectPartsDao.flush();
		List<WOMBatchingRejectParts> params = new ArrayList<WOMBatchingRejectParts>();
		params.add(batchingRejectParts);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchingRejectParts.getId();
			signatureLog.setTableId(batchingRejectParts.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.common.delete",operType="3")
	public void deleteBatchingRejectParts(String batchingRejectPartsIds){
		this.deleteBatchingRejectParts(batchingRejectPartsIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.common.delete",operType="3")
	public void deleteBatchingRejectParts(String batchingRejectPartsIds,SignatureLog signatureLog){
		deleteCollection(batchingRejectPartsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchingRejectParts(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_batchingMaterial#modelCode:WOM_7.5.0.0_batchingMaterial_BatchingRejectParts#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="ec.engine.restore",operType="10")
	public void restoreBatchingRejectParts(String batchingRejectPartsIds){
		restoreCollection(batchingRejectPartsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchingRejectParts(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="ec.engine.restore",operType="10")
	public void restoreBatchingRejectParts(long batchingRejectPartsId){
		
		findBusinessKeyUsed(batchingRejectPartsId);	//判断业务主键是否重复
		...
		beforeRestoreBatchingRejectParts(batchingRejectParts);
		batchingRejectParts.setValid(true);
		batchingRejectPartsDao.update(batchingRejectParts);
		afterRestoreBatchingRejectParts(batchingRejectParts);
		List<WOMBatchingRejectParts> params = new ArrayList<WOMBatchingRejectParts>();
		params.add(batchingRejectParts);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchingRejectPartsId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchingRejectParts.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchingRejectParts.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchingRejectPartsDao.createNativeQuery(sql,batchingRejectPartsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchingRejectParts(List<WOMBatchingRejectParts>  batchingRejectPartss){
		for(WOMBatchingRejectParts batchingRejectParts:batchingRejectPartss)  {
			saveBatchingRejectParts(batchingRejectParts, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchingRejectParts(List<WOMBatchingRejectParts>  batchingRejectPartss){
		for(WOMBatchingRejectParts batchingRejectParts:batchingRejectPartss)  {
			saveBatchingRejectParts(batchingRejectParts, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="ec.flow.save")
	public void saveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchingRejectParts(batchingRejectParts, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="ec.flow.save")
	public void saveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchingRejectParts(batchingRejectParts, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchingRejectPartsCodes(WOMBatchingRejectParts batchingRejectParts) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchingRejectPartsCodes(batchingRejectParts, false);
	}
	
	public void generateBatchingRejectPartsCodes(WOMBatchingRejectParts batchingRejectParts, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchingRejectPartsSummarys(WOMBatchingRejectParts batchingRejectParts) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchingRejectPartsSummarys(batchingRejectParts, false);
	}

	public void generateBatchingRejectPartsSummarys(WOMBatchingRejectParts batchingRejectParts, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts")
	public void saveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchingRejectParts(batchingRejectParts, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts")
	public void saveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterial");
		String url = null;
		if(batchingRejectParts.getId() != null && batchingRejectParts.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingRejectParts.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchingRejectParts);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchingRejectParts);
		beforeSaveBatchingRejectParts(batchingRejectParts, viewIsView);

		if (viewIsView) {
			batchingRejectPartsDao.saveWithRevertVersion(batchingRejectParts);
		} else {
			if(isNew)
				batchingRejectPartsDao.save(batchingRejectParts);
			else
				batchingRejectPartsDao.merge(batchingRejectParts);
		}
		if(null != batchingRejectParts && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingRejectParts.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
			AuditUtil.setColumnStringA(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
		...
		dealDatagridsSave(batchingRejectParts,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchingRejectPartsCodes(batchingRejectParts, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchingRejectPartsSummarys(batchingRejectParts, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchingRejectParts(batchingRejectParts, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchingRejectParts);
			}
		}
		List<WOMBatchingRejectParts> params = new ArrayList<WOMBatchingRejectParts>();
		batchingRejectPartsDao.flush();
		batchingRejectPartsDao.clear();
		batchingRejectParts = batchingRejectPartsDao.load(batchingRejectParts.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchingRejectParts batchingRejectParts){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchingRejectParts", batchingRejectParts);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts")
	public void mergeBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchingRejectParts);
		beforeSaveBatchingRejectParts(batchingRejectParts);
		batchingRejectPartsDao.merge(batchingRejectParts);
		if(null != batchingRejectParts && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingRejectParts.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
			AuditUtil.setColumnStringA(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
		...
		dealDatagridsSave(batchingRejectParts,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchingRejectParts(batchingRejectParts);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchingRejectParts> findBatchingRejectPartss(Page<WOMBatchingRejectParts> page, Criterion... criterions) {
		return batchingRejectPartsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingRejectPartss(Page<WOMBatchingRejectParts> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchingRejectPartss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingRejectPartss(Page<WOMBatchingRejectParts> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingRejectPartss(Page<WOMBatchingRejectParts> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchingRejectParts> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchingRejectParts\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchingRejectParts\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchingRejectParts\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"batchingRejectParts\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_BATCHING_MATERIALS \"batchingMaterial\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  batchingRejectPartsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchingRejectPartsDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_batchingMaterial").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchingRejectParts\".LAY_REC ASC, \"batchingRejectParts\".SORT ASC");
				} else {
					orderPart.append(" \"batchingRejectParts\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchingRejectPartsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchingRejectPartsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchingRejectPartsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchingRejectParts> result = new ArrayList<WOMBatchingRejectParts>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchingRejectParts.class,batchingRejectPartsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchingRejectParts.class,batchingRejectPartsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchingRejectParts batchingRejectParts : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchingRejectParts.getId(), "WOM_batchingMaterial_batchingRejectParts");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchingRejectParts.getId(), "WOM_batchingMaterial_batchingRejectParts");
							if(null != documents && !documents.isEmpty()) {
								batchingRejectParts.setDocument(documents.get(0));
								batchingRejectParts.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchingRejectParts> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchingRejectParts> result) {
		View view = null;
		...
		Map<Long, WOMBatchingRejectParts> rsMap = new HashMap<Long, WOMBatchingRejectParts>();
		for (WOMBatchingRejectParts batchingRejectParts : result) {
			ids.add(batchingRejectParts.getId());
			rsMap.put(batchingRejectParts.getId(), batchingRejectParts);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchingRejectPartsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchingRejectPartsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_batchingMaterial_BatchingRejectParts" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchingRejectPartsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchingRejectParts\"","WOM_7.5.0.0_batchingMaterial","","WOM_BATCHING_REJECT_PARTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchingRejectParts\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchingRejectParts\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchingRejectParts\"","WOM_7.5.0.0_batchingMaterial","","WOM_BATCHING_REJECT_PARTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_batchingMaterial_BatchingRejectParts" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchingRejectParts.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchingRejectParts\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchingRejectParts\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchingRejectParts\".CID = 1 OR \"batchingRejectParts\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchingRejectParts.mc\"").append(".BATCHING_REJECT_PARTS) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchingRejectPartsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchingRejectPartsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMBatchingRejectParts.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchingRejectParts.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchingRejectParts.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchingRejectParts 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchingRejectParts batchingRejectParts){
	}
	
	
	@Override
	public List<WOMBatchingRejectParts> getBatchingRejectPartss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchingRejectParts.class);
		if(params!=null){
			...
			return batchingRejectPartsDao.findByHql("from " + WOMBatchingRejectParts.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchingRejectPartsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMBatchingRejectParts> findBatchingRejectPartssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchingRejectParts.class);
	...
	@Override
	public List<WOMBatchingRejectParts> findBatchingRejectPartssByHql(String hql, Object... objects){
		
		return   batchingRejectPartsDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchingRejectParts loadBatchingRejectPartsByBussinessKey(WOMBatchingRejectParts batchingRejectParts){
		return loadBatchingRejectPartsByBussinessKey(batchingRejectParts.getId());
	}
	
	@Override
	public Page<WOMBatchingRejectParts> getBatchingRejectPartss(Page<WOMBatchingRejectParts> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchingRejectParts.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchingRejectPartsDao.findByPage(page, sql);
			}else{
				page = batchingRejectPartsDao.findAllByPage(page);
			}
			return page;
		}
		page = batchingRejectPartsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchingRejectParts> findByTreeDataGridBatchingMaterialPart(List<WOMBatchingRejectParts> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMBatchingRejectParts batchingRejectParts : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridBatchingMaterialPart(Page<WOMBatchingRejectParts> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMBatchingRejectParts> batchingRejectPartss = dgPage.getResult();
			for(WOMBatchingRejectParts batchingRejectParts : batchingRejectPartss){
				
			}
			dgPage.setResult(batchingRejectPartss);
		}
	...
	@Override
	@Transactional
	public void deleteBatchingRejectPartsByBatchingMaterialPart(List<Long> batchingMaterialPartIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("batchingPartIdList", batchingMaterialPartIds);
		String hql = "select id from WOMBatchingRejectParts where "
						+ "batchingPartId.id in (:batchingPartIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> batchingRejectPartsIds1 = batchingRejectPartsDao.findByHql(hql, params);
		if(batchingRejectPartsIds1 != null && !batchingRejectPartsIds1.isEmpty()) {
		}
		hql = "delete WOMBatchingRejectParts where "
						+ "batchingPartId.id in (:batchingPartIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		batchingRejectPartsDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingRejectParts loadBatchingRejectPartsByBussinessKey(Serializable bussinessKey){
		return batchingRejectPartsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchingRejectParts> findByProperty(String propertyName, Object object){
		return batchingRejectPartsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingRejectParts findEntityByProperty(String propertyName, Object object){
		return batchingRejectPartsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchingRejectParts.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchingRejectPartsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts","runtime");
		try {
	...
	public void dealDatagridsSave(WOMBatchingRejectParts batchingRejectParts,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchingRejectPartsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchingRejectParts.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchingRejectParts);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(batchingRejectParts.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(batchingRejectParts.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(batchingRejectParts.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchingRejectParts.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchingRejectParts.TABLE_NAME +" where valid = 1";
			list = batchingRejectPartsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchingRejectParts.TABLE_NAME;
			list = batchingRejectPartsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchingRejectParts.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchingRejectPartsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchingRejectPartsDaoImpl batchingRejectPartsDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMBatchingRejectParts.TABLE_NAME + " where id = ?";
		String tableNo = batchingRejectPartsDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchBatchingRejectParts(final List<WOMBatchingRejectParts> insertObjs, final List<WOMBatchingRejectParts> updateObjs, 
		...
		return this.importBatchBatchingRejectParts(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchingRejectParts(final List<WOMBatchingRejectParts> insertObjs, final List<WOMBatchingRejectParts> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1543236237180"),importCount));
		...
		Map<Long, WOMBatchingRejectParts> oldUpdateObjsMap = new HashMap<Long, WOMBatchingRejectParts>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchingRejectParts bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchingRejectParts> bizObjList = batchingRejectPartsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchingRejectParts bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchingRejectParts> nmeObjs = new ArrayList<WOMBatchingRejectParts>();
		...
		//String idSql = "select id from " + WOMBatchingRejectParts.TABLE_NAME + " order by id desc";
		//Query query = batchingRejectPartsDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMBatchingRejectParts.JPA_NAME + " order by id desc";
		Query query =  batchingRejectPartsDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMBatchingRejectParts.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchingRejectParts batchingRejectParts:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchingRejectParts.TABLE_NAME,1,dbId);
			if(batchingRejectParts.getId() != null ){
				m.put(batchingRejectParts.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			batchingRejectParts.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchingRejectPartsDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterial_BatchingRejectParts"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchingRejectPartsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchingRejectParts.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMBatchingRejectParts batchingRejectParts = updateObjs.get(updateIndex);
						Long id = batchingRejectParts.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchingRejectParts.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchingRejectParts);
							...
							nmeObjs.add(batchingRejectParts);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchingRejectParts.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMBatchingRejectParts batchingRejectParts = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchingRejectParts.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchingRejectPartsDaoImpl.getSession().flush();
		batchingRejectPartsDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = batchingRejectPartsDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_batchingMaterial_BatchingRejectParts").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = batchingRejectPartsDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_batchingMaterial_BatchingRejectParts").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchingRejectParts bizObj = insertObjs.get(insertIndex);
			batchingRejectPartsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchingRejectParts bizObj = updateObjs.get(updateIndex);
			WOMBatchingRejectParts oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchingRejectPartsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMBatchingRejectParts> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMBatchingRejectParts> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchingRejectPartsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMBatchingRejectParts> result = getResult(page, query, new PendingResultTransformer(WOMBatchingRejectParts.class,batchingRejectPartsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMBatchingRejectParts model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchingRejectPartsDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterial_BatchingRejectParts").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchingRejectPartsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchingRejectPartsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchingRejectPartsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchingRejectPartsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchingRejectPartsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchingRejectPartsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="ec.flow.save")
	public void saveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, DataGridService dataGridService){
		saveBatchingRejectParts(batchingRejectParts, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="ec.flow.save")
	public void saveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, DataGridService dataGridService, String viewCode){
		saveBatchingRejectParts(batchingRejectParts, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts")
	public void saveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchingRejectParts(batchingRejectParts, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts")
	public void saveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchingMaterial";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterial");
		String url = null;
		if(null != batchingRejectParts.getId() && batchingRejectParts.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingRejectParts.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts")
	public Map<Object, Object> batchingRejectPartsDataGridImport(WOMBatchingRejectParts batchingRejectParts, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchingRejectParts.getVirtualId();
		
		batchingRejectParts = batchingRejectPartsDao.load(batchingRejectParts.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchingRejectParts.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchingRejectParts.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchingRejectParts);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchingRejectParts.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts")
	public void mergeBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchingRejectParts);
		beforeSaveBatchingRejectParts(batchingRejectParts);
		batchingRejectPartsDao.merge(batchingRejectParts);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_batchingMaterial";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterial");
	...
	@Override
	public Page<WOMBatchingRejectParts> getByPage(Page<WOMBatchingRejectParts> page,DetachedCriteria detachedCriteria){
		return batchingRejectPartsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, Object... objects){
		checkUniqueConstraint(batchingRejectParts);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchingRejectParts(List<WOMBatchingRejectParts> batchingRejectPartss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchingRejectParts(List<WOMBatchingRejectParts> batchingRejectPartss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchingRejectParts(List<WOMBatchingRejectParts> batchingRejectPartss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchingRejectParts(List<WOMBatchingRejectParts> batchingRejectPartss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchingRejectParts batchingRejectParts, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchingRejectParts batchingRejectParts, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchingRejectParts(List<WOMBatchingRejectParts> insertObjs, List<WOMBatchingRejectParts> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchingRejectParts,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchingRejectParts(List<WOMBatchingRejectParts> insertObjs, List<WOMBatchingRejectParts> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchingRejectParts,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
