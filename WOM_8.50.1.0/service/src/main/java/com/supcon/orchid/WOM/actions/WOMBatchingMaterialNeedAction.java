...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchingMaterialNeedAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchingMaterialNeed> {
	...
	private WOMBatchingMaterialNeed batchingMaterialNeed;
	...
	//导出excel时,主模型内容
	List<WOMBatchingMaterialNeed> queryResults = new ArrayList<WOMBatchingMaterialNeed>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterialNeed/batchingMaterialNeed/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterialNeed/batchingMaterialNeed/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchingMaterialNeed> batchingMaterialNeeds;
	
	public List<WOMBatchingMaterialNeed> getBatchingMaterialNeeds() {
		return batchingMaterialNeeds;
	}
	...
	private Page<WOMBatchingMaterialNeed> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchingMaterialNeed = new WOMBatchingMaterialNeed();
			if(batchingMaterialNeed.getActiveType() != null) {
				batchingMaterialNeed.setActiveType((SystemCode) systemCodeService.getSystemCode(batchingMaterialNeed.getActiveType().getId()));
			}
			if(batchingMaterialNeed.getBatchSite() != null) {
				batchingMaterialNeed.setBatchSite((SystemCode) systemCodeService.getSystemCode(batchingMaterialNeed.getBatchSite().getId()));
			}
			if(batchingMaterialNeed.getOfferSystem() != null) {
				batchingMaterialNeed.setOfferSystem((SystemCode) systemCodeService.getSystemCode(batchingMaterialNeed.getOfferSystem().getId()));
			}
			if(batchingMaterialNeed.getState() != null) {
				batchingMaterialNeed.setState((SystemCode) systemCodeService.getSystemCode(batchingMaterialNeed.getState().getId()));
			}
		} else {
			if(id > 0){
				batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(id, viewCode);
			}
		}
	...
	public Page<WOMBatchingMaterialNeed> getPage() {
		if (null == page) {
			page = new Page<WOMBatchingMaterialNeed>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchingMaterialNeed getBatchingMaterialNeed() {
		return batchingMaterialNeed;
	}		

	public void setBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed) {
		this.batchingMaterialNeed = batchingMaterialNeed;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchingMaterialNeedService.getBatchingMaterialNeedAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String batchingMaterialNeedReferenceCopy() throws Exception{
			WOMBatchingMaterialNeed oldBatchingMaterialNeed = null;
			if(isEditNew()){
				...
				oldBatchingMaterialNeed = new WOMBatchingMaterialNeed();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchingMaterialNeed.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchingMaterialNeed.getId());
				...
				oldBatchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(id);
			}
			
			
			batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchingMaterialNeed.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchingMaterialNeed, null);
						Method setValue = WOMBatchingMaterialNeed.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchingMaterialNeed, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchingMaterialNeed.setCreateDepartmentId(oldBatchingMaterialNeed.getCreateDepartmentId());
			batchingMaterialNeed.setCreatePositionId(oldBatchingMaterialNeed.getCreatePositionId());
			batchingMaterialNeed.setCreateStaffId(oldBatchingMaterialNeed.getCreateStaffId());
			batchingMaterialNeed.setOwnerDepartmentId(oldBatchingMaterialNeed.getOwnerDepartmentId());
			batchingMaterialNeed.setOwnerPositionId(oldBatchingMaterialNeed.getOwnerPositionId());
			batchingMaterialNeed.setOwnerStaffId(oldBatchingMaterialNeed.getOwnerStaffId());
			batchingMaterialNeed.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchingMaterialNeedService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchingMaterialNeedReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(this.batchingMaterialNeed.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchingMaterialNeed.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchingMaterialNeedService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/batchNeedList", results = { @Result(type = FREEMARKER, location = "batchNeedList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchingMaterialNeedService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchingMaterialNeedService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchingMaterialNeedService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/batchNeedRef", results = { @Result(type = FREEMARKER, location = "batchNeedRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/batchNeedList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.taskID.tableNo,result.taskID.productBatchNum,result.productId.productCode,result.productId.productName,result.taskProcess.name,result.factoryId.name,result.taskActive.name,result.actOrder,result.offerOrder,result.batchSite.id,result.batchSite.value,result.offerSystem.id,result.offerSystem.value,result.container,result.planNum,result.hasOrderNum,result.useNum,result.actualNum,result.state.id,result.state.value,result.taskID.id,result.productId.id,result.taskProcess.id,result.factoryId.id,result.taskActive.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batchNeedList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchingMaterialNeed\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchingMaterialNeed\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchingMaterialNeed\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchingMaterialNeed\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchingMaterialNeedService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/batchNeedList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/batchNeedRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.taskID.tableNo,result.productId.productCode,result.productId.productName,result.taskProcess.name,result.factoryId.name,result.taskActive.name,result.offerOrder,result.offerSystem.id,result.offerSystem.value,result.planNum,result.hasOrderNum,result.useNum,result.state.id,result.state.value,result.taskID.id,result.productId.id,result.taskProcess.id,result.factoryId.id,result.taskActive.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchNeedRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/allTempleteDownload", results = { @Result(name="excel",type="excel",location="batchNeedList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchingMaterialNeedService.findDataGridPage(dg,dgclass,getDgPage(),batchingMaterialNeed,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchingMaterialNeed.getId() != null && batchingMaterialNeed.getId() != -1){
				batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(batchingMaterialNeed.getId());
			}
			batchingMaterialNeedService.findDataGridPage(dg,dgclass,getDgPage(),batchingMaterialNeed,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchingMaterialNeed.getId()) {
		...
		batchingMaterialNeedService.saveBatchingMaterialNeed(batchingMaterialNeed,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "submit");
		bap_validate_datagrids=batchingMaterialNeedService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchingMaterialNeed.getId());
		...
		fileuploads.put("linkId", batchingMaterialNeed.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchingMaterialNeedService.deleteBatchingMaterialNeed(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "restore");
		batchingMaterialNeedService.restoreBatchingMaterialNeed(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "showDealInfoData");
		if (null != tableInfoId) {
			int count=batchingMaterialNeedService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = batchingMaterialNeedService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = batchingMaterialNeedService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchingMaterialNeed> findExportDataInfos(Page<WOMBatchingMaterialNeed> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchingMaterialNeedService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchingMaterialNeedService.getAssProperty(p.getCode());	
									//String assModelCode = batchingMaterialNeedService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchingMaterialNeedService.findBatchingMaterialNeeds(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchingMaterialNeedService.findBatchingMaterialNeeds(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchingMaterialNeed> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchingMaterialNeed result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchingMaterialNeed || null == this.batchingMaterialNeed.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchingMaterialNeed/batchingMaterialNeed/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchingMaterialNeed/batchingMaterialNeed/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchingMaterialNeedService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_batchingMaterialNeed", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_batchingMaterialNeed");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchingMaterialNeedService"
						...
						, "com.supcon.orchid.WOM.services.WOMMakeBatOrderHeadService"
						, "com.supcon.orchid.WOM.services.WOMMakeBatOrderPartService"
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
		_auditLog.setColumnStringA(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
		_auditLog.setColumnStringC(batchingMaterialNeed.getId().toString());
		_auditLog.setColumnStringD(WOMBatchingMaterialNeed.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_batchingMaterialNeed");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchingMaterialNeedAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchingMaterialNeed,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchingMaterialNeed,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchingMaterialNeed);
		...
		printlog.setEntity("WOM_7.5.0.0_batchingMaterialNeed");
		...
		printlog.setBusinessId(batchingMaterialNeed.getId());
		printlog.setBussinessKey(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
		printlog.setMainDisplay(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchingMaterialNeed,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchingMaterialNeedService.getBatchingMaterialNeed(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchingMaterialNeed,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(Long.parseLong(arrId[i]));
				if(null != batchingMaterialNeed){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchingMaterialNeed.getId() ? "" : "," + batchingMaterialNeed.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchingMaterialNeed.getId() ? "" : "," + batchingMaterialNeed.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchingMaterialNeed.getId() ? "" : "," + batchingMaterialNeed.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchingMaterialNeed,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchingMaterialNeed);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchingMaterialNeedService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/batchingMaterialNeed/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMBatchingMaterialNeed> findExportPageInfos(Page<WOMBatchingMaterialNeed> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_batchingMaterialNeed" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = batchingMaterialNeedService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMBatchingMaterialNeed queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = batchingMaterialNeedService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = batchingMaterialNeedService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchingMaterialNeedService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
