...
import com.supcon.orchid.WOM.services.WOMProcFeedbackHeadImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMProcFeedbackHeadDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.MESBasic.services.MESBasicProductService;
import com.supcon.orchid.MESBasic.entities.MESBasicProduct;
/* CUSTOM CODE END */

@Service("wOM_procFeedbackHeadService")
@Transactional
public class WOMProcFeedbackHeadServiceImpl  extends BaseServiceImpl<WOMProcFeedbackHead> implements WOMProcFeedbackHeadService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMProcFeedbackHead> bapBaseService;
	...
	@Autowired
	private WOMProcFeedbackHeadImportService procFeedbackHeadImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMProcFeedbackHead.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  procFeedbackHeadDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMProcFeedbackHead.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = procFeedbackHeadDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProcFeedbackHead.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProcFeedbackHead.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  procFeedbackHeadDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMProcFeedbackHead.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMProcFeedbackHead.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMProcFeedbackHead> list = (List<WOMProcFeedbackHead>) procFeedbackHeadDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProcFeedbackHead getProcFeedbackHead(long id){
		return getProcFeedbackHead(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProcFeedbackHead getProcFeedbackHead(long id, String viewCode){
		WOMProcFeedbackHead procFeedbackHead = procFeedbackHeadDao.load(id);

		// 一对多情况处理
		if(procFeedbackHead != null){
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProcFeedbackHeadAsJSON(long id, String include){
		WOMProcFeedbackHead procFeedbackHead = procFeedbackHeadDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(procFeedbackHead, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.common.delete",operType="3")
	public void deleteProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead){
		if(null != procFeedbackHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProcFeedbackHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
			AuditUtil.setColumnStringA(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
			AuditUtil.setColumnStringC(procFeedbackHead.getId().toString());
		}
		beforeDeleteProcFeedbackHead(procFeedbackHead);
		procFeedbackHeadDao.delete(procFeedbackHead);
		afterDeleteProcFeedbackHead(procFeedbackHead);
		procFeedbackHeadDao.flush();
		List<WOMProcFeedbackHead> params = new ArrayList<WOMProcFeedbackHead>();
		params.add(procFeedbackHead);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.common.delete",operType="3")
	public void deleteProcFeedbackHead(List<Long> procFeedbackHeadIds){
		deleteProcFeedbackHead(procFeedbackHeadIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.common.delete",operType="3")
	public void deleteProcFeedbackHead(List<Long> procFeedbackHeadIds, String eventTopic) {
		List<WOMProcFeedbackHead> procFeedbackHeads = new ArrayList<WOMProcFeedbackHead>();
		for(Long procFeedbackHeadId : procFeedbackHeadIds){
			WOMProcFeedbackHead procFeedbackHead = getProcFeedbackHead(procFeedbackHeadId);
			procFeedbackHeads.add(procFeedbackHead);
			if(null != procFeedbackHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProcFeedbackHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == procFeedbackHead.getId() ? "" : "," + procFeedbackHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == procFeedbackHead.getId() ? "" : "," + procFeedbackHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == procFeedbackHead.getId() ? "" : "," + procFeedbackHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(procFeedbackHeads != null && procFeedbackHeads.size() > 0 ){
			for(WOMProcFeedbackHead procFeedbackHead : procFeedbackHeads){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),procFeedbackHead, procFeedbackHead.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(procFeedbackHeadIds != null && procFeedbackHeadIds.size() > 0) {
			String hql = "update " + WOMProcFeedbackHead.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = procFeedbackHeadDao.createQuery(hql);
			query.setParameterList("ids", procFeedbackHeadIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMProcFeedbackHead procFeedbackHeadz : procFeedbackHeads){
				procFeedbackHeadz.setValid(false);
				List<WOMProcFeedbackHead> params = new ArrayList<WOMProcFeedbackHead>();
				params.add(procFeedbackHeadz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_procFeedback");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.common.delete",operType="3")
	public void deleteProcFeedbackHead(long procFeedbackHeadId, int procFeedbackHeadVersion){
		this.deleteProcFeedbackHead(procFeedbackHeadId, procFeedbackHeadVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.common.delete",operType="3")
	public void deleteProcFeedbackHead(long procFeedbackHeadId, int procFeedbackHeadVersion,SignatureLog signatureLog){
		WOMProcFeedbackHead procFeedbackHead = getProcFeedbackHead(procFeedbackHeadId);
		if(procFeedbackHead != null && !procFeedbackHead.isValid()){
		...
		beforeDeleteProcFeedbackHead(procFeedbackHead);
		procFeedbackHeadDao.delete(procFeedbackHeadId, procFeedbackHeadVersion);
		afterDeleteProcFeedbackHead(procFeedbackHead);
		procFeedbackHeadDao.flush();
		List<WOMProcFeedbackHead> params = new ArrayList<WOMProcFeedbackHead>();
		params.add(procFeedbackHead);
		...
		if(signatureLog != null ) {
			...
			businessKey=procFeedbackHead.getId();
			signatureLog.setTableId(procFeedbackHead.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.common.delete",operType="3")
	public void deleteProcFeedbackHead(String procFeedbackHeadIds){
		this.deleteProcFeedbackHead(procFeedbackHeadIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.common.delete",operType="3")
	public void deleteProcFeedbackHead(String procFeedbackHeadIds,SignatureLog signatureLog){
		deleteCollection(procFeedbackHeadIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProcFeedbackHead(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_procFeedback#modelCode:WOM_7.5.0.0_procFeedback_ProcFeedbackHead#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="ec.engine.restore",operType="10")
	public void restoreProcFeedbackHead(String procFeedbackHeadIds){
		restoreCollection(procFeedbackHeadIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProcFeedbackHead(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="ec.engine.restore",operType="10")
	public void restoreProcFeedbackHead(long procFeedbackHeadId){
		
		findBusinessKeyUsed(procFeedbackHeadId);	//判断业务主键是否重复
		...
		beforeRestoreProcFeedbackHead(procFeedbackHead);
		procFeedbackHead.setValid(true);
		procFeedbackHeadDao.update(procFeedbackHead);
		afterRestoreProcFeedbackHead(procFeedbackHead);
		List<WOMProcFeedbackHead> params = new ArrayList<WOMProcFeedbackHead>();
		params.add(procFeedbackHead);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long procFeedbackHeadId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_procFeedback_ProcFeedbackHead");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMProcFeedbackHead.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMProcFeedbackHead.TABLE_NAME +" where id = ? )";
			List<Object> list =  procFeedbackHeadDao.createNativeQuery(sql,procFeedbackHeadId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProcFeedbackHead(List<WOMProcFeedbackHead>  procFeedbackHeads){
		for(WOMProcFeedbackHead procFeedbackHead:procFeedbackHeads)  {
			saveProcFeedbackHead(procFeedbackHead, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProcFeedbackHead(List<WOMProcFeedbackHead>  procFeedbackHeads){
		for(WOMProcFeedbackHead procFeedbackHead:procFeedbackHeads)  {
			saveProcFeedbackHead(procFeedbackHead, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="ec.flow.save")
	public void saveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProcFeedbackHead(procFeedbackHead, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="ec.flow.save")
	public void saveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProcFeedbackHead(procFeedbackHead, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProcFeedbackHeadCodes(WOMProcFeedbackHead procFeedbackHead) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcFeedbackHeadCodes(procFeedbackHead, false);
	}
	
	public void generateProcFeedbackHeadCodes(WOMProcFeedbackHead procFeedbackHead, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProcFeedbackHeadSummarys(WOMProcFeedbackHead procFeedbackHead) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcFeedbackHeadSummarys(procFeedbackHead, false);
	}

	public void generateProcFeedbackHeadSummarys(WOMProcFeedbackHead procFeedbackHead, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead")
	public void saveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProcFeedbackHead(procFeedbackHead, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead")
	public void saveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
		String url = null;
		if(procFeedbackHead.getId() != null && procFeedbackHead.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProcFeedbackHead.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, procFeedbackHead);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(procFeedbackHead);
		beforeSaveProcFeedbackHead(procFeedbackHead, viewIsView);

		if (viewIsView) {
			procFeedbackHeadDao.saveWithRevertVersion(procFeedbackHead);
		} else {
			if(isNew)
				procFeedbackHeadDao.save(procFeedbackHead);
			else
				procFeedbackHeadDao.merge(procFeedbackHead);
		}
		if(null != procFeedbackHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProcFeedbackHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
			AuditUtil.setColumnStringA(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
		...
		dealDatagridsSave(procFeedbackHead,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProcFeedbackHeadCodes(procFeedbackHead, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProcFeedbackHeadSummarys(procFeedbackHead, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProcFeedbackHead(procFeedbackHead, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, procFeedbackHead);
			}
		}
		List<WOMProcFeedbackHead> params = new ArrayList<WOMProcFeedbackHead>();
		procFeedbackHeadDao.flush();
		procFeedbackHeadDao.clear();
		procFeedbackHead = procFeedbackHeadDao.load(procFeedbackHead.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMProcFeedbackHead procFeedbackHead){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("procFeedbackHead", procFeedbackHead);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead")
	public void mergeProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(procFeedbackHead);
		beforeSaveProcFeedbackHead(procFeedbackHead);
		procFeedbackHeadDao.merge(procFeedbackHead);
		if(null != procFeedbackHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProcFeedbackHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
			AuditUtil.setColumnStringA(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
		...
		dealDatagridsSave(procFeedbackHead,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProcFeedbackHead(procFeedbackHead);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMProcFeedbackHead> findProcFeedbackHeads(Page<WOMProcFeedbackHead> page, Criterion... criterions) {
		return procFeedbackHeadDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMProcFeedbackHeadDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMProcFeedbackHeadDealInfo> list=procFeedbackHeadDao.createQuery("from " + WOMProcFeedbackHeadDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMProcFeedbackHeadDealInfo dealInfo) {
		procFeedbackHeadDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMProcFeedbackHeadDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) procFeedbackHeadDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMProcFeedbackHeadDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = procFeedbackHeadDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = procFeedbackHeadDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMProcFeedbackHeadDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = procFeedbackHeadDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcFeedbackHeads(Page<WOMProcFeedbackHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProcFeedbackHeads(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcFeedbackHeads(Page<WOMProcFeedbackHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcFeedbackHeads(Page<WOMProcFeedbackHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMProcFeedbackHead> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"procFeedbackHead\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"procFeedbackHead\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"procFeedbackHead\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_procFeedback_ProcFeedbackHead" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_procFeedback_ProcFeedbackHead" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  procFeedbackHeadDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  procFeedbackHeadDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_procFeedback").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"procFeedbackHead\"","WOM_7.5.0.0_procFeedback",customCondition,"WOM_PROC_FEEDBACK_HEADS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"procFeedbackHead\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMProcFeedbackHeadDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"procFeedbackHead\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"procFeedbackHead\".LAY_REC ASC, \"procFeedbackHead\".SORT ASC");
				} else {
					orderPart.append(" \"procFeedbackHead\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) procFeedbackHeadDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=procFeedbackHeadDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = procFeedbackHeadDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMProcFeedbackHead> result = new ArrayList<WOMProcFeedbackHead>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMProcFeedbackHead.class,procFeedbackHeadDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMProcFeedbackHead.class,procFeedbackHeadDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMProcFeedbackHead procFeedbackHead : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(procFeedbackHead.getId(), "WOM_procFeedback_procFeedbackHead");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(procFeedbackHead.getId(), "WOM_procFeedback_procFeedbackHead");
							if(null != documents && !documents.isEmpty()) {
								procFeedbackHead.setDocument(documents.get(0));
								procFeedbackHead.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMProcFeedbackHead> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMProcFeedbackHead> result) {
		View view = null;
		...
		Map<Long, WOMProcFeedbackHead> rsMap = new HashMap<Long, WOMProcFeedbackHead>();
		for (WOMProcFeedbackHead procFeedbackHead : result) {
			ids.add(procFeedbackHead.getId());
			rsMap.put(procFeedbackHead.getId(), procFeedbackHead);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(procFeedbackHeadDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = procFeedbackHeadDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMProcFeedbackHead procFeedbackHead,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(procFeedbackHead.getOwnerStaffId());
		ti.setOwnerPositionId(procFeedbackHead.getOwnerPositionId());
		ti.setOwnerDepartmentId(procFeedbackHead.getOwnerDepartmentId());
		
		ti.setCreateStaff(procFeedbackHead.getCreateStaff());
		ti.setCreateStaffId(procFeedbackHead.getCreateStaffId());
		ti.setCreateTime(procFeedbackHead.getCreateTime());
		ti.setDeleteStaff(procFeedbackHead.getDeleteStaff());
		ti.setDeleteStaffId(procFeedbackHead.getDeleteStaffId());
		ti.setDeleteTime(procFeedbackHead.getDeleteTime());
		ti.setCreateDepartmentId(procFeedbackHead.getCreateDepartmentId());
		ti.setModifyStaff(procFeedbackHead.getModifyStaff());
		ti.setModifyStaffId(procFeedbackHead.getModifyStaffId());
		ti.setCreatePositionId(procFeedbackHead.getCreatePositionId());
		ti.setPositionLayRec(procFeedbackHead.getPositionLayRec());
		ti.setTableNo(procFeedbackHead.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_procFeedback");
		ti.setTargetTableName("WOM_PROC_FEEDBACK_HEADS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_procFeedback_ProcFeedbackHead" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  procFeedbackHeadDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"procFeedbackHead\"","WOM_7.5.0.0_procFeedback","","WOM_PROC_FEEDBACK_HEADS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"procFeedbackHead\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"procFeedbackHead\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"procFeedbackHead\"","WOM_7.5.0.0_procFeedback","","WOM_PROC_FEEDBACK_HEADS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("procFeedbackHead.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"procFeedbackHead\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"procFeedbackHead\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"procFeedbackHead\".CID = 1 OR \"procFeedbackHead\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("procFeedbackHead.mc\"").append(".PROC_FEEDBACK_HEAD) not in ("+notinValue+")");
			}
			
			SQLQuery query = procFeedbackHeadDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=procFeedbackHeadDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMProcFeedbackHead.ENTITY_CODE;}
	protected String getModuleCode(){return WOMProcFeedbackHead.MODULE_CODE;}
	protected String getModelCode(){return WOMProcFeedbackHead.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param procFeedbackHead 模型实例
	 */
	public void checkUniqueConstraint(WOMProcFeedbackHead procFeedbackHead){
	}
	
	
	@Override
	public List<WOMProcFeedbackHead> getProcFeedbackHeads(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProcFeedbackHead.class);
		if(params!=null){
			...
			return procFeedbackHeadDao.findByHql("from " + WOMProcFeedbackHead.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return procFeedbackHeadDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMProcFeedbackHead> findProcFeedbackHeadsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProcFeedbackHead.class);
	...
	@Override
	public List<WOMProcFeedbackHead> findProcFeedbackHeadsByHql(String hql, Object... objects){
		
		return   procFeedbackHeadDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMProcFeedbackHead loadProcFeedbackHeadByBussinessKey(WOMProcFeedbackHead procFeedbackHead){
		return loadProcFeedbackHeadByBussinessKey(procFeedbackHead.getId());
	}
	
	@Override
	public Page<WOMProcFeedbackHead> getProcFeedbackHeads(Page<WOMProcFeedbackHead> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProcFeedbackHead.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = procFeedbackHeadDao.findByPage(page, sql);
			}else{
				page = procFeedbackHeadDao.findAllByPage(page);
			}
			return page;
		}
		page = procFeedbackHeadDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProcFeedbackHead loadProcFeedbackHeadByBussinessKey(Serializable bussinessKey){
		return procFeedbackHeadDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProcFeedbackHead> findByProperty(String propertyName, Object object){
		return procFeedbackHeadDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProcFeedbackHead findEntityByProperty(String propertyName, Object object){
		return procFeedbackHeadDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMProcFeedbackHead.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = procFeedbackHeadDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_procFeedback_ProcFeedbackHead","runtime");
		try {
	...
	public void dealDatagridsSave(WOMProcFeedbackHead procFeedbackHead,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,procFeedbackHeadDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMProcFeedbackHead.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, procFeedbackHead);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",procFeedbackHead.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProcFeedbackHead.TABLE_NAME +" where valid = 1";
			list = procFeedbackHeadDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProcFeedbackHead.TABLE_NAME;
			list = procFeedbackHeadDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMProcFeedbackHead.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = procFeedbackHeadDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMProcFeedbackHeadDaoImpl procFeedbackHeadDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProcFeedbackHead(final List<WOMProcFeedbackHead> insertObjs, final List<WOMProcFeedbackHead> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProcFeedbackHead(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProcFeedbackHead(final List<WOMProcFeedbackHead> insertObjs, final List<WOMProcFeedbackHead> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1540952404135"),importCount));
		...
		Map<Long, WOMProcFeedbackHead> oldUpdateObjsMap = new HashMap<Long, WOMProcFeedbackHead>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMProcFeedbackHead bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMProcFeedbackHead> bizObjList = procFeedbackHeadDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMProcFeedbackHead bizObj = bizObjList.get(mIndex);
		...
		final List<WOMProcFeedbackHead> nmeObjs = new ArrayList<WOMProcFeedbackHead>();
		...
		long id=autoGeneratorID.getNextId(WOMProcFeedbackHead.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMProcFeedbackHead procFeedbackHead:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMProcFeedbackHead.TABLE_NAME,1,dbId);
			if(procFeedbackHead.getId() != null ){
				m.put(procFeedbackHead.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			procFeedbackHead.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) procFeedbackHeadDao.createNativeQuery(sql, "WOM_7.5.0.0_procFeedback_ProcFeedbackHead"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		procFeedbackHeadDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMProcFeedbackHead.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMProcFeedbackHead procFeedbackHead = updateObjs.get(updateIndex);
						Long id = procFeedbackHead.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = procFeedbackHead.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(procFeedbackHead);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMProcFeedbackHead.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMProcFeedbackHead.TABLE_NAME + " order by id desc";
//					Query query = procFeedbackHeadDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMProcFeedbackHead procFeedbackHead = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = procFeedbackHead.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		procFeedbackHeadDaoImpl.getSession().flush();
		procFeedbackHeadDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMProcFeedbackHead bizObj = insertObjs.get(insertIndex);
			procFeedbackHeadImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMProcFeedbackHead bizObj = updateObjs.get(updateIndex);
			WOMProcFeedbackHead oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			procFeedbackHeadImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = procFeedbackHeadDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				procFeedbackHeadDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = procFeedbackHeadDao.createNativeQuery(sql, "WOM_7.5.0.0_procFeedback_ProcFeedbackHead").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = procFeedbackHeadDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = procFeedbackHeadDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = procFeedbackHeadDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?procFeedbackHeadDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  procFeedbackHeadDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,procFeedbackHeadDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1542186009188Page(Page<WOMCraftStandards> dg1542186009188Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftStandards.class);
		criteria.add(Restrictions.eq("procFeedbackID", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"craftStandards\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		craftStandardsDao.findByPage(dg1542186009188Page, criteria);
		craftStandardsService.findByNormalDataGridProcFeedbackHead(dg1542186009188Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1542186009283Page(Page<WOMCraftRecords> dg1542186009283Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftRecords.class);
		criteria.add(Restrictions.eq("header", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"craftRecords\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		craftRecordsDao.findByPage(dg1542186009283Page, criteria);
		craftRecordsService.findByNormalDataGridProcFeedbackHead(dg1542186009283Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1542186009357Page(Page<WOMBatchCharge> dg1542186009357Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchCharge.class);
		criteria.add(Restrictions.eq("headerId", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"batchCharge\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		batchChargeDao.findByPage(dg1542186009357Page, criteria);
		batchChargeService.findByNormalDataGridProcFeedbackHead(dg1542186009357Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1542248606162Page(Page<WOMCraftStandards> dg1542248606162Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftStandards.class);
		criteria.add(Restrictions.eq("procFeedbackID", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftStandardsDao.findByPage(dg1542248606162Page, criteria);
		craftStandardsService.findByNormalDataGridProcFeedbackHead(dg1542248606162Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1542248606200Page(Page<WOMCraftRecords> dg1542248606200Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftRecords.class);
		criteria.add(Restrictions.eq("header", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftRecordsDao.findByPage(dg1542248606200Page, criteria);
		craftRecordsService.findByNormalDataGridProcFeedbackHead(dg1542248606200Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1542248606236Page(Page<WOMBatchCharge> dg1542248606236Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchCharge.class);
		criteria.add(Restrictions.eq("headerId", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		batchChargeDao.findByPage(dg1542248606236Page, criteria);
		batchChargeService.findByNormalDataGridProcFeedbackHead(dg1542248606236Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1540988573983Page(Page<WOMCraftStandards> dg1540988573983Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftStandards.class);
		criteria.add(Restrictions.eq("procFeedbackID", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftStandardsDao.findByPage(dg1540988573983Page, criteria);
		craftStandardsService.findByNormalDataGridProcFeedbackHead(dg1540988573983Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541043071770Page(Page<WOMCraftRecords> dg1541043071770Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftRecords.class);
		criteria.add(Restrictions.eq("header", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftRecordsDao.findByPage(dg1541043071770Page, criteria);
		craftRecordsService.findByNormalDataGridProcFeedbackHead(dg1541043071770Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541066103156Page(Page<WOMBatchCharge> dg1541066103156Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchCharge.class);
		criteria.add(Restrictions.eq("headerId", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		batchChargeDao.findByPage(dg1541066103156Page, criteria);
		batchChargeService.findByNormalDataGridProcFeedbackHead(dg1541066103156Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541302488990Page(Page<WOMCraftStandards> dg1541302488990Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftStandards.class);
		criteria.add(Restrictions.eq("procFeedbackID", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftStandardsDao.findByPage(dg1541302488990Page, criteria);
		craftStandardsService.findByNormalDataGridProcFeedbackHead(dg1541302488990Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541302489005Page(Page<WOMCraftRecords> dg1541302489005Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftRecords.class);
		criteria.add(Restrictions.eq("header", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftRecordsDao.findByPage(dg1541302489005Page, criteria);
		craftRecordsService.findByNormalDataGridProcFeedbackHead(dg1541302489005Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541302489036Page(Page<WOMBatchCharge> dg1541302489036Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchCharge.class);
		criteria.add(Restrictions.eq("headerId", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		batchChargeDao.findByPage(dg1541302489036Page, criteria);
		batchChargeService.findByNormalDataGridProcFeedbackHead(dg1541302489036Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541073013999Page(Page<WOMCraftStandards> dg1541073013999Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftStandards.class);
		criteria.add(Restrictions.eq("procFeedbackID", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftStandardsDao.findByPage(dg1541073013999Page, criteria);
		craftStandardsService.findByNormalDataGridProcFeedbackHead(dg1541073013999Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541073014077Page(Page<WOMCraftRecords> dg1541073014077Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftRecords.class);
		criteria.add(Restrictions.eq("header", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftRecordsDao.findByPage(dg1541073014077Page, criteria);
		craftRecordsService.findByNormalDataGridProcFeedbackHead(dg1541073014077Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541073014155Page(Page<WOMBatchCharge> dg1541073014155Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchCharge.class);
		criteria.add(Restrictions.eq("headerId", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		batchChargeDao.findByPage(dg1541073014155Page, criteria);
		batchChargeService.findByNormalDataGridProcFeedbackHead(dg1541073014155Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541154477969Page(Page<WOMCraftStandards> dg1541154477969Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftStandards.class);
		criteria.add(Restrictions.eq("procFeedbackID", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftStandardsDao.findByPage(dg1541154477969Page, criteria);
		craftStandardsService.findByNormalDataGridProcFeedbackHead(dg1541154477969Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541154478000Page(Page<WOMCraftRecords> dg1541154478000Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCraftRecords.class);
		criteria.add(Restrictions.eq("header", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		craftRecordsDao.findByPage(dg1541154478000Page, criteria);
		craftRecordsService.findByNormalDataGridProcFeedbackHead(dg1541154478000Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1541154478031Page(Page<WOMBatchCharge> dg1541154478031Page,WOMProcFeedbackHead procFeedbackHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchCharge.class);
		criteria.add(Restrictions.eq("headerId", procFeedbackHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		batchChargeDao.findByPage(dg1541154478031Page, criteria);
		batchChargeService.findByNormalDataGridProcFeedbackHead(dg1541154478031Page);
	}
	
	 
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="ec.flow.save")
	public void saveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, DataGridService dataGridService){
		saveProcFeedbackHead(procFeedbackHead, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="ec.flow.save")
	public void saveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, DataGridService dataGridService, String viewCode){
		saveProcFeedbackHead(procFeedbackHead, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead")
	public void saveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProcFeedbackHead(procFeedbackHead, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead")
	public void saveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_procFeedback";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
		String url = null;
		if(null != procFeedbackHead.getId() && procFeedbackHead.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProcFeedbackHead.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead")
	public Map<Object, Object> procFeedbackHeadDataGridImport(WOMProcFeedbackHead procFeedbackHead, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = procFeedbackHead.getVirtualId();
		
		procFeedbackHead = procFeedbackHeadDao.load(procFeedbackHead.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, procFeedbackHead.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = procFeedbackHead.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(procFeedbackHead);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, procFeedbackHead.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead")
	public void mergeProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(procFeedbackHead);
		beforeSaveProcFeedbackHead(procFeedbackHead);
		procFeedbackHeadDao.merge(procFeedbackHead);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_ProcFeedbackHead",desc="foundation.batch.save",operType="4")
	public void batchSaveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_procFeedback";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
	...
	@Override
	public Page<WOMProcFeedbackHead> getByPage(Page<WOMProcFeedbackHead> page,DetachedCriteria detachedCriteria){
		return procFeedbackHeadDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, Object... objects){
		checkUniqueConstraint(procFeedbackHead);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
		procFeedbackHead.setModifyTime(new Date());
/* CUSTOM CODE END */
	}
	
	private void afterSaveProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
   //	checkAndUpdateOnhand(procFeedbackHead.getId());
/* CUSTOM CODE END */
	}
	
	@Override
	public void beforeExcelImportProcFeedbackHead(List<WOMProcFeedbackHead> procFeedbackHeads, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProcFeedbackHead(List<WOMProcFeedbackHead> procFeedbackHeads){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProcFeedbackHead(List<WOMProcFeedbackHead> procFeedbackHeads, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProcFeedbackHead(List<WOMProcFeedbackHead> procFeedbackHeads){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMProcFeedbackHead procFeedbackHead, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMProcFeedbackHead procFeedbackHead, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProcFeedbackHead(List<WOMProcFeedbackHead> insertObjs, List<WOMProcFeedbackHead> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProcFeedbackHead,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProcFeedbackHead(List<WOMProcFeedbackHead> insertObjs, List<WOMProcFeedbackHead> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProcFeedbackHead,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
	/**
	 * 报工时，如果删除工序，需要将工序执行记录连带删除
	 * @param processIds
	 * @return
	 */
	@Override
	public String deleteProcessRowByProcessId(List<Long> processIds) {
		String sql = "UPDATE WOMCraftStandards SET VALID = 0 , DELETE_TIME = :date WHERE ID in (:processIds)";
		procFeedbackHeadDao.createQuery(sql).setParameter("date",new Date()).setParameterList("processIds",processIds).executeUpdate();
		sql = "UPDATE WOMCraftRecords SET VALID = 0 , DELETE_TIME = :date WHERE standardCraft.id in (:processIds)";
		procFeedbackHeadDao.createQuery(sql).setParameter("date",new Date()).setParameterList("processIds",processIds).executeUpdate();
		return "SUCCESS";
	}
	 ...
	 */
	@Override
	public String deleteCraftRecordByRecordId(List<Long> recordIds) {
		String sql = "UPDATE WOMCraftRecords SET VALID = 0 , DELETE_TIME = :date WHERE ID in (:recordIds)";
		procFeedbackHeadDao.createQuery(sql).setParameter("date",new Date()).setParameterList("recordIds",recordIds).executeUpdate();
		return "SUCCESS";
	}
	@Autowired
	private MESBasicProductService productService;
	@Override
	public List<WOMBatchCharge> getpreMaterialPart(String preMaterialId,String productCode, List<WOMBatchCharge> partList) {
		WOMPrepareMaterial prepareMaterial = prepareMaterialService.getPrepareMaterial(Long.parseLong(preMaterialId));
		if (prepareMaterial != null) {
          	String productHql = "from " + MESBasicProduct.JPA_NAME + " where valid = 1 and productCode = ? ";
          log.info("工序报工物品编码"+productCode);
			List<MESBasicProduct> productList = productService.findProductsByHql(productHql,productCode);
			if (productList.size()>0){
              String hqlChart = "from WOMPrepareMaterialPart where valid = 1 and formulaId = ? and productId= ?";		
				List<WOMPrepareMaterialPart> materialPartList = prepareMaterialPartDao.findByHql(hqlChart,prepareMaterial,productList.get(0));
			if (materialPartList != null && materialPartList.size() > 0) {
				for (WOMPrepareMaterialPart materialPart : materialPartList) {
					//新创建一条表体数据，并赋值
					WOMBatchCharge batchCharge = new WOMBatchCharge();
					//赋值物品
					batchCharge.setGood(materialPart.getProductId());
					//赋值使用数量
					if (materialPart.getUseNum() != null && materialPart.getUseNum().doubleValue() > 0) {
						batchCharge.setActualUseNum(materialPart.getUseNum());
					}
					//赋值批号
					if (materialPart.getMaterialBatchNum() != null && materialPart.getMaterialBatchNum() != "") {
						batchCharge.setBatch(materialPart.getMaterialBatchNum());
					}
					//赋值仓库
					if (materialPart.getWare() != null) {
						batchCharge.setWareID(materialPart.getWare());
					}
					boolean exist = true;
					if (partList != null) {
						for (WOMBatchCharge part : partList) {
							if (part.getGood().equals(batchCharge.getGood())) {
								//如果物品一样，则数量累计相加
								part.setActualUseNum(part.getActualUseNum().add(batchCharge.getActualUseNum()));
								exist = false;
							}
						}
						if (exist) {
							//如果物品不存在，新加物品
							partList.add(batchCharge);
						}
					}
				}
			}else{
				throw new BAPException("备料单中没有对应物料！");
			}
			}else{
				throw new BAPException("物料基础中查询不到对应物料！");
			}
			return partList;
		}
		return null;
	...
	/*@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void checkAndUpdateOnhand(Long id) {
        ArrayList<Object> list = new ArrayList<>();
        list.add(id);
        List<WOMBatchCharge> batchCharges = batchChargeService.getBatchCharges("HEADER_ID=?", list);
        if (batchCharges.size()>0) {
            for (int i = 0; i < batchCharges.size(); i++) {
                if (null != batchCharges.get(i).getStandingCrop()) {
                    if (batchCharges.get(i).getStandingCrop().getOnhand().compareTo(batchCharges.get(i).getActualUseNum()) < 0) {
                        batchCharges.get(i).getStandingCrop().setOnhand(batchCharges.get(i).getStandingCrop().getOnhand().subtract(batchCharges.get(i).getActualUseNum()));
                        batchChargeDao.merge(batchCharges.get(i));
                    } else {
                        throw new BAPException("投料明细：物品" + batchCharges.get(i).getGood().getProductCode() + "使用量不能多于现存量--" + batchCharges.get(i).getStandingCrop().getOnhand());
                    }
                }
            }
        }
