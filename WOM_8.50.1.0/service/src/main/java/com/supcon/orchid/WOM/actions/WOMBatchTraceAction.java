...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchTimeLiner_BatchTrace,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchTraceAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchTrace> {
	...
	private WOMBatchTrace batchTrace;
	...
	//导出excel时,主模型内容
	List<WOMBatchTrace> queryResults = new ArrayList<WOMBatchTrace>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchTimeLiner/batchTrace/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchTimeLiner/batchTrace/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchTimeLiner_BatchTrace" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchTrace> batchTraces;
	
	public List<WOMBatchTrace> getBatchTraces() {
		return batchTraces;
	}
	...
	private Page<WOMBatchTrace> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchTrace = new WOMBatchTrace();
		} else {
			if(id > 0){
				batchTrace = batchTraceService.getBatchTrace(id, viewCode);
			}
		}
	...
	public Page<WOMBatchTrace> getPage() {
		if (null == page) {
			page = new Page<WOMBatchTrace>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchTrace getBatchTrace() {
		return batchTrace;
	}		

	public void setBatchTrace(WOMBatchTrace batchTrace) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchTimeLiner/batchTrace/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchTraceService.getBatchTraceAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchTimeLiner/batchTrace/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1544495113649
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchTraceReferenceCopy() throws Exception{
			WOMBatchTrace oldBatchTrace = null;
			if(isEditNew()){
				...
				oldBatchTrace = new WOMBatchTrace();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchTrace.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchTrace.getId());
				...
				oldBatchTrace = batchTraceService.getBatchTrace(id);
			}
			
			
			batchTrace = batchTraceService.getBatchTrace(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchTimeLiner_BatchTrace");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchTrace.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchTrace, null);
						Method setValue = WOMBatchTrace.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchTrace, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchTrace.setCreateDepartmentId(oldBatchTrace.getCreateDepartmentId());
			batchTrace.setCreatePositionId(oldBatchTrace.getCreatePositionId());
			batchTrace.setCreateStaffId(oldBatchTrace.getCreateStaffId());
			batchTrace.setOwnerDepartmentId(oldBatchTrace.getOwnerDepartmentId());
			batchTrace.setOwnerPositionId(oldBatchTrace.getOwnerPositionId());
			batchTrace.setOwnerStaffId(oldBatchTrace.getOwnerStaffId());
			batchTrace.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchTimeLiner/batchTrace/batchTraceEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchTraceEdit.ftl") }),
		@Action(value = "/WOM/batchTimeLiner/batchTrace/batchTraceEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchTraceEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchTraceService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchTraceReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchTrace = batchTraceService.getBatchTrace(this.batchTrace.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchTrace.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/WOM/batchTimeLiner/batchTrace/batchTraceList", results = { @Result(type = FREEMARKER, location = "batchTraceList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchTraceService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchTraceService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchTraceService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchTimeLiner_BatchTrace", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchTimeLiner/batchTrace/batchTraceList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.prodTaskId.productBatchNum,result.prodTaskId.factoryId.name,result.prodTaskId.productId.productCode,result.prodTaskId.productId.productName,result.prodTaskId.id,result.prodTaskId.factoryId.id,result.prodTaskId.productId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batchTraceList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchTrace\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchTrace\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchTrace\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchTrace\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/WOM/batchTimeLiner/batchTrace/batchTraceList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/batchTimeLiner/batchTrace/allTempleteDownload", results = { @Result(name="excel",type="excel",location="batchTraceList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_batchTimeLiner_BatchTrace");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchTraceService.findDataGridPage(dg,dgclass,getDgPage(),batchTrace,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchTrace.getId() != null && batchTrace.getId() != -1){
				batchTrace = batchTraceService.getBatchTrace(batchTrace.getId());
			}
			batchTraceService.findDataGridPage(dg,dgclass,getDgPage(),batchTrace,searchCondition,params);
	...
	private void save() throws Exception {
		...
		batchTraceService.saveBatchTrace(batchTrace,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/batchTimeLiner/batchTrace/batchTraceEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/batchTimeLiner/batchTrace/communication/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/batchTimeLiner/batchTrace/inspection/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/batchTimeLiner/batchTrace/messageBoard/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/batchTimeLiner/batchTrace/supplierManage/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "submit");
		bap_validate_datagrids=batchTraceService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchTrace.getId());
		...
		fileuploads.put("linkId", batchTrace.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchTimeLiner/batchTrace/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchTraceService.deleteBatchTrace(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchTimeLiner/batchTrace/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "restore");
		batchTraceService.restoreBatchTrace(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/batchTimeLiner/batchTrace/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/batchTimeLiner/batchTrace/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "showDealInfoData");
		if (null != tableInfoId) {
			int count=batchTraceService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = batchTraceService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = batchTraceService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchTrace> findExportDataInfos(Page<WOMBatchTrace> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchTraceService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchTraceService.getAssProperty(p.getCode());	
									//String assModelCode = batchTraceService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchTraceService.findBatchTraces(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchTraceService.findBatchTraces(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchTrace> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchTrace result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchTrace || null == this.batchTrace.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/batchTimeLiner/batchTrace/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchTimeLiner/batchTrace/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchTimeLiner/batchTrace/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchTimeLiner/batchTrace/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchTraceService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_batchTimeLiner", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_batchTimeLiner");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchTimeLiner/batchTrace/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchTimeLiner_BatchTrace", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchTimeLiner_BatchTrace", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchTimeLiner/batchTrace/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchTimeLiner/batchTrace/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchTimeLiner_BatchTrace";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchTimeLiner_BatchTrace", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchTimeLiner/batchTrace/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMBatchTrace batchTrace = null;
		if(null != id) {
			batchTrace = batchTraceService.getBatchTrace(id);
		}
		...
		_auditLog.setColumnStringB(null == batchTrace.getId() ? "" : batchTrace.getId().toString());
		_auditLog.setColumnStringA(null == batchTrace.getId() ? "" : batchTrace.getId().toString());
		_auditLog.setColumnStringC(batchTrace.getId().toString());
		_auditLog.setColumnStringD(WOMBatchTrace.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_batchTimeLiner");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchTraceAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchTrace,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchTrace,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchTrace);
		...
		printlog.setEntity("WOM_7.5.0.0_batchTimeLiner");
		...
		printlog.setBusinessId(batchTrace.getId());
		printlog.setBussinessKey(null == batchTrace.getId() ? "" : batchTrace.getId().toString());
		printlog.setMainDisplay(null == batchTrace.getId() ? "" : batchTrace.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchTimeLiner/batchTrace/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchTrace,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchTimeLiner/batchTrace/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchTraceService.getBatchTrace(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/batchTimeLiner/batchTrace/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchTimeLiner/batchTrace/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchTrace,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchTrace = batchTraceService.getBatchTrace(Long.parseLong(arrId[i]));
				if(null != batchTrace){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchTrace.getId() ? "" : "," + batchTrace.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchTrace.getId() ? "" : batchTrace.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchTrace.getId() ? "" : "," + batchTrace.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchTrace.getId() ? "" : batchTrace.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchTrace.getId() ? "" : "," + batchTrace.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchTrace.getId() ? "" : batchTrace.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchTrace,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchTrace);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/batchTimeLiner/batchTrace/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMBatchTrace> findExportPageInfos(Page<WOMBatchTrace> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_batchTimeLiner" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = batchTraceService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMBatchTrace queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = batchTraceService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = batchTraceService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchTimeLiner_BatchTrace");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchTimeLiner_BatchTrace")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchTimeLiner_BatchTrace");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchTimeLiner_BatchTrace");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchTraceService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchTimeLiner_BatchTrace,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchTimeLiner_BatchTrace,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchTimeLiner_BatchTrace,WOM_7.5.0.0) */
// 自定义代码
	/**
	 *  查看视图公用主方法
	 */
	@Actions({
			...
			@Action(value = "/WOM/batchTimeLiner/batchTrace/batchTraceView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchTraceView.ftl") }),
			@Action(value = "/WOM/batchTimeLiner/batchTrace/batchTraceMain",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchTraceMain.ftl") }),
			@Action(value = "/WOM/batchTimeLiner/batchTrace/supplierManage",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "supplierManage.ftl") }),
			@Action(value = "/WOM/batchTimeLiner/batchTrace/inspection",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "inspection.ftl") }),
			@Action(value = "/WOM/batchTimeLiner/batchTrace/communication",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "communication.ftl") }),
			@Action(value = "/WOM/batchTimeLiner/batchTrace/messageBoard",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "messageBoard.ftl") }),
	})
	public String viewView1() throws Exception{
		String viewCode=request.getAttribute("ViewCode").toString();
		String viewName=request.getAttribute("ViewName").toString();
		View printView = viewServiceFoundation.getView(viewCode);
		String printName=request.getRequestURI().replaceAll("^.*/"+printView.getName(),"").replaceAll(".action$", "");
