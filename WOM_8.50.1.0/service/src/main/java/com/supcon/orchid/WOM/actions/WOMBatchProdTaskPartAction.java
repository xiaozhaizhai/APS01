...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchProdTaskPartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchProdTaskPart> {
	...
	private WOMBatchProdTaskPart batchProdTaskPart;
	...
	//导出excel时,主模型内容
	List<WOMBatchProdTaskPart> queryResults = new ArrayList<WOMBatchProdTaskPart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchProduceTask/batchProdTaskPart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchProduceTask/batchProdTaskPart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchProdTaskPart> batchProdTaskParts;
	
	public List<WOMBatchProdTaskPart> getBatchProdTaskParts() {
		return batchProdTaskParts;
	}
	...
	private Page<WOMBatchProdTaskPart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchProdTaskPart = new WOMBatchProdTaskPart();
			if(batchProdTaskPart.getBulkType() != null) {
				batchProdTaskPart.setBulkType((SystemCode) systemCodeService.getSystemCode(batchProdTaskPart.getBulkType().getId()));
			}
			if(batchProdTaskPart.getOrderType() != null) {
				batchProdTaskPart.setOrderType((SystemCode) systemCodeService.getSystemCode(batchProdTaskPart.getOrderType().getId()));
			}
			if(batchProdTaskPart.getTaskType() != null) {
				batchProdTaskPart.setTaskType((SystemCode) systemCodeService.getSystemCode(batchProdTaskPart.getTaskType().getId()));
			}
		} else {
			if(id > 0){
				batchProdTaskPart = batchProdTaskPartService.getBatchProdTaskPart(id, viewCode);
			}
		}
	...
	public Page<WOMBatchProdTaskPart> getPage() {
		if (null == page) {
			page = new Page<WOMBatchProdTaskPart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchProdTaskPart getBatchProdTaskPart() {
		return batchProdTaskPart;
	}		

	public void setBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart) {
		this.batchProdTaskPart = batchProdTaskPart;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchProdTaskPartService.getBatchProdTaskPartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1495089930032
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchProdTaskPartReferenceCopy() throws Exception{
			WOMBatchProdTaskPart oldBatchProdTaskPart = null;
			if(isEditNew()){
				...
				oldBatchProdTaskPart = new WOMBatchProdTaskPart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchProdTaskPart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchProdTaskPart.getId());
				...
				oldBatchProdTaskPart = batchProdTaskPartService.getBatchProdTaskPart(id);
			}
			
			
			batchProdTaskPart = batchProdTaskPartService.getBatchProdTaskPart(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchProdTaskPart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchProdTaskPart, null);
						Method setValue = WOMBatchProdTaskPart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchProdTaskPart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchProdTaskPart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/setPartsEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "setPartsEdit.ftl") }),
		@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/setPartsEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "setPartsEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchProdTaskPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchProdTaskPartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchProdTaskPart = batchProdTaskPartService.getBatchProdTaskPart(this.batchProdTaskPart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchProdTaskPart.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchProdTaskPartService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchProdTaskPartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchProdTaskPartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchProdTaskPartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchProdTaskPart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchProdTaskPart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchProdTaskPart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchProdTaskPart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchProdTaskPartService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchProdTaskPartService.findDataGridPage(dg,dgclass,getDgPage(),batchProdTaskPart,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchProdTaskPart.getId() != null && batchProdTaskPart.getId() != -1){
				batchProdTaskPart = batchProdTaskPartService.getBatchProdTaskPart(batchProdTaskPart.getId());
			}
			batchProdTaskPartService.findDataGridPage(dg,dgclass,getDgPage(),batchProdTaskPart,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchProdTaskPart.getId()) {
		...
		batchProdTaskPartService.saveBatchProdTaskPart(batchProdTaskPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/setPartsEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "submit");
		bap_validate_datagrids=batchProdTaskPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchProdTaskPart.getId());
		...
		fileuploads.put("linkId", batchProdTaskPart.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchProdTaskPartService.deleteBatchProdTaskPart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "restore");
		batchProdTaskPartService.restoreBatchProdTaskPart(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchProdTaskPart> findExportDataInfos(Page<WOMBatchProdTaskPart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchProdTaskPartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchProdTaskPartService.getAssProperty(p.getCode());	
									//String assModelCode = batchProdTaskPartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchProdTaskPartService.findBatchProdTaskParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchProdTaskPartService.findBatchProdTaskParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchProdTaskPart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchProdTaskPart result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchProdTaskPart || null == this.batchProdTaskPart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchProduceTask/batchProdTaskPart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchProduceTask/batchProdTaskPart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchProdTaskPartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchProdTaskPartService"
						...
						, "com.supcon.orchid.WOM.services.WOMBatchProduceTaskService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
		_auditLog.setColumnStringA(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
		_auditLog.setColumnStringC(batchProdTaskPart.getId().toString());
		_auditLog.setColumnStringD(WOMBatchProdTaskPart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchProdTaskPartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchProdTaskPart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchProdTaskPart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchProdTaskPart);
		...
		printlog.setBusinessId(batchProdTaskPart.getId());
		printlog.setBussinessKey(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
		printlog.setMainDisplay(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchProdTaskPart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchProdTaskPartService.getBatchProdTaskPart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchProdTaskPart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchProdTaskPart = batchProdTaskPartService.getBatchProdTaskPart(Long.parseLong(arrId[i]));
				if(null != batchProdTaskPart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchProdTaskPart.getId() ? "" : "," + batchProdTaskPart.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchProdTaskPart.getId() ? "" : "," + batchProdTaskPart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchProdTaskPart.getId() ? "" : "," + batchProdTaskPart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchProdTaskPart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchProdTaskPart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchProdTaskPartService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchProduceTask/batchProdTaskPart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchProdTaskPartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
		if(batchProdTaskPart.getProductBatchNum()!=null){
			batchProdTaskPart.setProductBatchNum(batchProdTaskPart.getProductBatchNum().trim());
		}
/* CUSTOM CODE END */
	}
	
	private void bapAfterCustomMethod(String code, String methodName) throws Exception {
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
