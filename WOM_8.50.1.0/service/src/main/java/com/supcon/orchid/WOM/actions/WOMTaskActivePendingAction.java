...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMTaskActivePendingAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMTaskActivePending> {
	...
	private WOMTaskActivePending taskActivePending;
	...
	//导出excel时,主模型内容
	List<WOMTaskActivePending> queryResults = new ArrayList<WOMTaskActivePending>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/taskActivePending/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/taskActivePending/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_TaskActivePending" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMTaskActivePending> taskActivePendings;
	
	public List<WOMTaskActivePending> getTaskActivePendings() {
		return taskActivePendings;
	}
	...
	private Page<WOMTaskActivePending> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			taskActivePending = new WOMTaskActivePending();
			if(taskActivePending.getActiveType() != null) {
				taskActivePending.setActiveType((SystemCode) systemCodeService.getSystemCode(taskActivePending.getActiveType().getId()));
			}
			if(taskActivePending.getState() != null) {
				taskActivePending.setState((SystemCode) systemCodeService.getSystemCode(taskActivePending.getState().getId()));
			}
		} else {
			if(id > 0){
				taskActivePending = taskActivePendingService.getTaskActivePending(id, viewCode);
			}
		}
	...
	public Page<WOMTaskActivePending> getPage() {
		if (null == page) {
			page = new Page<WOMTaskActivePending>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMTaskActivePending getTaskActivePending() {
		return taskActivePending;
	}		

	public void setTaskActivePending(WOMTaskActivePending taskActivePending) {
		this.taskActivePending = taskActivePending;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/taskActivePending/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = taskActivePendingService.getTaskActivePendingAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/taskActivePending/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1495779524387
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String taskActivePendingReferenceCopy() throws Exception{
			WOMTaskActivePending oldTaskActivePending = null;
			if(isEditNew()){
				...
				oldTaskActivePending = new WOMTaskActivePending();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskActivePending.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, taskActivePending.getId());
				...
				oldTaskActivePending = taskActivePendingService.getTaskActivePending(id);
			}
			
			
			taskActivePending = taskActivePendingService.getTaskActivePending(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_TaskActivePending");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMTaskActivePending.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTaskActivePending, null);
						Method setValue = WOMTaskActivePending.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(taskActivePending, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=taskActivePendingService.findValidateDatagrids(getDgClassMap());
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return taskActivePendingReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			taskActivePending = taskActivePendingService.getTaskActivePending(this.taskActivePending.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskActivePending.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			taskActivePendingService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			List<Object[]> list = taskActivePendingService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_TaskActivePending", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"taskActivePending\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"taskActivePending\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			taskActivePendingService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		taskActivePendingService.findDataGridPage(dg,dgclass,getDgPage(),taskActivePending,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if(refId!=null&&refId>0){
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			taskActivePendingService.findDataGridPage(dg,dgclass,getDgPage(),taskActivePending,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == taskActivePending.getId()) {
			responseMap.put("operate", "add");
		} else {
			responseMap.put("operate", "edit");
		}
		taskActivePendingService.saveTaskActivePending(taskActivePending,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "submit");
		bap_validate_datagrids=taskActivePendingService.findValidateDatagrids(getDgClassMap());
			save();
			responseMap.put("id", taskActivePending.getId());
		...
		fileuploads.put("linkId", taskActivePending.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/produceTask/taskActivePending/delete", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "delete");
		taskActivePendingService.deleteTaskActivePending(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "delete");
		return SUCCESS;
	}
	...
	@Override
	public Page<WOMTaskActivePending> findExportDataInfos(Page<WOMTaskActivePending> page) {
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				String supplyConfig = "";
				//if(useForImportFlag){//必填字段一定导出
					List<Property> properties = modelServiceFoundation.findProperties(view.getAssModel());
				    List<String> runningCustomPropertyCode = taskActivePendingService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
			       	for(Property p : properties){
			       		if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
			       			...
			       			supplyConfig += generteAuxXml(p);
			       		}else if(!p.getNullable() && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) &&
			       			...
			       			if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
			       				Model assModel = null;
			       				if(!p.getIsCustom()){
									String assProperty = taskActivePendingService.getAssProperty(p.getCode());	
									String assModelCode = taskActivePendingService.getPropertyModelCode(assProperty);
			       			...
			       			supplyConfig += generteAuxXml(p);
			       		}else if( p.getName().equals("ownerStaff") && useForImportFlag ){//用于导出时，基础表单必导所有者
			       			String assProperty = taskActivePendingService.getAssProperty(p.getCode());	
			       			String assModelCode = taskActivePendingService.getPropertyModelCode(assProperty);
			       		...
			       		}else if( p.getName().equals("ownerPosition") && !p.getModel().getEntity().getIsBase() && useForImportFlag){//用于导出时，流程表单导出拥有者岗位
			       			String assProperty = taskActivePendingService.getAssProperty(p.getCode());	
			       			String assModelCode = taskActivePendingService.getPropertyModelCode(assProperty);
			       			Model assModel = modelServiceFoundation.getModel(assModelCode);
				...
				for(Map m : exportMaps){
					if(m.get("name").toString().equals("ownerStaff.name") || m.get("name").toString().equals("ownerStaff.code") || 
							m.get("name").toString().equals("ownerPosition.name")	|| m.get("name").toString().equals("ownerPosition.code")){
		...
		if(null != searchObjects && searchObjects.length > 0) {
			taskActivePendingService.findTaskActivePendings(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, searchObjects);
		} else {
			taskActivePendingService.findTaskActivePendings(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMTaskActivePending> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMTaskActivePending result : results){
	...
	public boolean isEditNew(){
		//return null == this.taskActivePending || null == this.taskActivePending.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/taskActivePending/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/taskActivePending/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/taskActivePending/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/taskActivePending/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMTaskActivePendingService"
		};
		//辅模型用到的service也一起放进来
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "imports");
		...
		try {
			if(dataFile != null && datagridCode != null && datagridCode.length() > 0){
				...
				orgName = name[0];
				xlsResult = importService.imports(tid, dataFile, datagridCode, time, name[0], fileType, newService);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/taskActivePending/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_TaskActivePending", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_TaskActivePending", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/taskActivePending/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/taskActivePending/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_TaskActivePending";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskActivePending", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/taskActivePending/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		WOMTaskActivePending taskActivePending = null;
		if(null != id) {
			taskActivePending = taskActivePendingService.getTaskActivePending(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, taskActivePending, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, taskActivePending, null, null, true, this);
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		taskActivePendingService.print(1);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/taskActivePending/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/taskActivePending/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				taskActivePending = taskActivePendingService.getTaskActivePending(Integer.parseInt(id));
				if(taskActivePending != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, taskActivePending, null, null, true, this);
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(isCustom != null && isCustom.equals("true")){
			...
			fillContent = p.getFillcontent();
		}
		if(fillContent != null){
			String[] str = fillContent.split(",");
			String[] str2 = str[2].split(":");
			String scEntityCode = str2[1].substring(1, str2[1].length()-2);
			List<String> values = taskActivePendingService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
