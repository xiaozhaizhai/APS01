...
import com.supcon.orchid.WOM.services.WOMMakeBatOrderHeadImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMMakeBatOrderHeadDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_makeBatOrderHeadService")
@Transactional
public class WOMMakeBatOrderHeadServiceImpl  extends BaseServiceImpl<WOMMakeBatOrderHead> implements WOMMakeBatOrderHeadService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMMakeBatOrderHead> bapBaseService;
	...
	@Autowired
	private WOMMakeBatOrderHeadImportService makeBatOrderHeadImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMMakeBatOrderHead.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  makeBatOrderHeadDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMMakeBatOrderHead.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = makeBatOrderHeadDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMMakeBatOrderHead.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMMakeBatOrderHead.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  makeBatOrderHeadDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMMakeBatOrderHead.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMMakeBatOrderHead.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMMakeBatOrderHead> list = (List<WOMMakeBatOrderHead>) makeBatOrderHeadDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMMakeBatOrderHead getMakeBatOrderHead(long id){
		return getMakeBatOrderHead(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMakeBatOrderHead getMakeBatOrderHead(long id, String viewCode){
		WOMMakeBatOrderHead makeBatOrderHead = makeBatOrderHeadDao.load(id);

		// 一对多情况处理
		if(makeBatOrderHead != null){
			this.setHeadIDNeedID(makeBatOrderHead, viewCode);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMakeBatOrderHeadAsJSON(long id, String include){
		WOMMakeBatOrderHead makeBatOrderHead = makeBatOrderHeadDao.load(id);
		// 一对多情况处理
			this.setHeadIDNeedID(makeBatOrderHead);

		if(makeBatOrderHead == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(makeBatOrderHead, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead){
		if(null != makeBatOrderHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
			AuditUtil.setColumnStringA(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
			AuditUtil.setColumnStringC(makeBatOrderHead.getId().toString());
		}
		beforeDeleteMakeBatOrderHead(makeBatOrderHead);
		makeBatOrderHeadDao.deletePhysical(makeBatOrderHead);
		afterDeleteMakeBatOrderHead(makeBatOrderHead);
		makeBatOrderHeadDao.flush();
		List<WOMMakeBatOrderHead> params = new ArrayList<WOMMakeBatOrderHead>();
		params.add(makeBatOrderHead);
		...
		String delIds = "";
		// 一对多情况处理
		delIds = makeBatOrderHead.getHeadIDNeedIDmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			...
			if (!idList.isEmpty()) {
				List<WOMMakeBatOrderPart> tmpList = makeBatOrderPartDao.findByCriteria(Restrictions.eq("headID", makeBatOrderHead), Restrictions.in("needID.id", idList), Restrictions.eq("valid", true));
				for (WOMMakeBatOrderPart item : tmpList) {
					makeBatOrderPartDao.deletePhysical(item);
				}
			}
		}
			// TODO delete
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderHead(List<Long> makeBatOrderHeadIds){
		deleteMakeBatOrderHead(makeBatOrderHeadIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderHead(List<Long> makeBatOrderHeadIds, String eventTopic) {
		makeBatOrderPartService.deleteMakeBatOrderPartByMakeBatOrderHead(makeBatOrderHeadIds);
		List<WOMMakeBatOrderHead> makeBatOrderHeads = new ArrayList<WOMMakeBatOrderHead>();
		for(Long makeBatOrderHeadId : makeBatOrderHeadIds){
			WOMMakeBatOrderHead makeBatOrderHead = getMakeBatOrderHead(makeBatOrderHeadId);
			makeBatOrderHeads.add(makeBatOrderHead);
			if(null != makeBatOrderHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == makeBatOrderHead.getId() ? "" : "," + makeBatOrderHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == makeBatOrderHead.getId() ? "" : "," + makeBatOrderHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == makeBatOrderHead.getId() ? "" : "," + makeBatOrderHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(makeBatOrderHeads != null && makeBatOrderHeads.size() > 0 ){
			for(WOMMakeBatOrderHead makeBatOrderHead : makeBatOrderHeads){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMMakeBatOrderHead.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(makeBatOrderHead));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),makeBatOrderHead, makeBatOrderHead.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(makeBatOrderHeadIds != null && makeBatOrderHeadIds.size() > 0) {
			String hql = "delete " + WOMMakeBatOrderHead.JPA_NAME + " where id in(:ids)";
			Query query = makeBatOrderHeadDao.createQuery(hql);
			query.setParameterList("ids", makeBatOrderHeadIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMMakeBatOrderHead makeBatOrderHeadz : makeBatOrderHeads){
				makeBatOrderHeadz.setValid(false);
				List<WOMMakeBatOrderHead> params = new ArrayList<WOMMakeBatOrderHead>();
				params.add(makeBatOrderHeadz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderHead(long makeBatOrderHeadId, int makeBatOrderHeadVersion){
		this.deleteMakeBatOrderHead(makeBatOrderHeadId, makeBatOrderHeadVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderHead(long makeBatOrderHeadId, int makeBatOrderHeadVersion,SignatureLog signatureLog){
		WOMMakeBatOrderHead makeBatOrderHead = getMakeBatOrderHead(makeBatOrderHeadId);
		...
		beforeDeleteMakeBatOrderHead(makeBatOrderHead);
		makeBatOrderHeadDao.delete(makeBatOrderHeadId, makeBatOrderHeadVersion);
		afterDeleteMakeBatOrderHead(makeBatOrderHead);
		makeBatOrderHeadDao.flush();
		List<WOMMakeBatOrderHead> params = new ArrayList<WOMMakeBatOrderHead>();
		params.add(makeBatOrderHead);
		...
		if(signatureLog != null ) {
			...
			businessKey=makeBatOrderHead.getId();
			signatureLog.setTableId(makeBatOrderHead.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderHead(String makeBatOrderHeadIds){
		this.deleteMakeBatOrderHead(makeBatOrderHeadIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.common.delete",operType="3")
	public void deleteMakeBatOrderHead(String makeBatOrderHeadIds,SignatureLog signatureLog){
		deleteCollection(makeBatOrderHeadIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMakeBatOrderHead(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_batchingMaterialNeed#modelCode:WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="ec.engine.restore",operType="10")
	public void restoreMakeBatOrderHead(String makeBatOrderHeadIds){
		restoreCollection(makeBatOrderHeadIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreMakeBatOrderHead(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="ec.engine.restore",operType="10")
	public void restoreMakeBatOrderHead(long makeBatOrderHeadId){
		
		findBusinessKeyUsed(makeBatOrderHeadId);	//判断业务主键是否重复
		...
		beforeRestoreMakeBatOrderHead(makeBatOrderHead);
		makeBatOrderHead.setValid(true);
		makeBatOrderHeadDao.update(makeBatOrderHead);
		afterRestoreMakeBatOrderHead(makeBatOrderHead);
		List<WOMMakeBatOrderHead> params = new ArrayList<WOMMakeBatOrderHead>();
		params.add(makeBatOrderHead);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long makeBatOrderHeadId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMMakeBatOrderHead.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMMakeBatOrderHead.TABLE_NAME +" where id = ? )";
			List<Object> list =  makeBatOrderHeadDao.createNativeQuery(sql,makeBatOrderHeadId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseMakeBatOrderHead(List<WOMMakeBatOrderHead>  makeBatOrderHeads){
		for(WOMMakeBatOrderHead makeBatOrderHead:makeBatOrderHeads)  {
			saveMakeBatOrderHead(makeBatOrderHead, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMakeBatOrderHead(List<WOMMakeBatOrderHead>  makeBatOrderHeads){
		for(WOMMakeBatOrderHead makeBatOrderHead:makeBatOrderHeads)  {
			saveMakeBatOrderHead(makeBatOrderHead, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="ec.flow.save")
	public void saveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMakeBatOrderHead(makeBatOrderHead, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="ec.flow.save")
	public void saveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMakeBatOrderHead(makeBatOrderHead, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateMakeBatOrderHeadCodes(WOMMakeBatOrderHead makeBatOrderHead) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMakeBatOrderHeadCodes(makeBatOrderHead, false);
	}
	
	public void generateMakeBatOrderHeadCodes(WOMMakeBatOrderHead makeBatOrderHead, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateMakeBatOrderHeadSummarys(WOMMakeBatOrderHead makeBatOrderHead) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMakeBatOrderHeadSummarys(makeBatOrderHead, false);
	}

	public void generateMakeBatOrderHeadSummarys(WOMMakeBatOrderHead makeBatOrderHead, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead")
	public void saveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveMakeBatOrderHead(makeBatOrderHead, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead")
	public void saveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
		String url = null;
		if(makeBatOrderHead.getId() != null && makeBatOrderHead.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderHead.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, makeBatOrderHead);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(makeBatOrderHead);
		beforeSaveMakeBatOrderHead(makeBatOrderHead, viewIsView);

		if (viewIsView) {
			makeBatOrderHeadDao.saveWithRevertVersion(makeBatOrderHead);
		} else {
			if(isNew)
				makeBatOrderHeadDao.save(makeBatOrderHead);
			else
				makeBatOrderHeadDao.merge(makeBatOrderHead);
		}
		if(null != makeBatOrderHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
			AuditUtil.setColumnStringA(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
		...
		dealDatagridsSave(makeBatOrderHead,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
			this.dealHeadIDNeedID(makeBatOrderHead);
		
		// 根据配置规则生成编码
		try {
			generateMakeBatOrderHeadCodes(makeBatOrderHead, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMakeBatOrderHeadSummarys(makeBatOrderHead, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMakeBatOrderHead(makeBatOrderHead, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, makeBatOrderHead);
			}
		}
		List<WOMMakeBatOrderHead> params = new ArrayList<WOMMakeBatOrderHead>();
		makeBatOrderHeadDao.flush();
		makeBatOrderHeadDao.clear();
		makeBatOrderHead = makeBatOrderHeadDao.load(makeBatOrderHead.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMMakeBatOrderHead makeBatOrderHead){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("makeBatOrderHead", makeBatOrderHead);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead")
	public void mergeMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(makeBatOrderHead);
		beforeSaveMakeBatOrderHead(makeBatOrderHead);
		makeBatOrderHeadDao.merge(makeBatOrderHead);
		if(null != makeBatOrderHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
			AuditUtil.setColumnStringA(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
		...
		dealDatagridsSave(makeBatOrderHead,null,dgLists,dgDeleteIDs,assFileUploads);

			// 一对多情况处理
			this.dealHeadIDNeedID(makeBatOrderHead);
		afterSaveMakeBatOrderHead(makeBatOrderHead);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMMakeBatOrderHead> findMakeBatOrderHeads(Page<WOMMakeBatOrderHead> page, Criterion... criterions) {
		return makeBatOrderHeadDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMakeBatOrderHeads(Page<WOMMakeBatOrderHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMakeBatOrderHeads(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMakeBatOrderHeads(Page<WOMMakeBatOrderHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMakeBatOrderHeads(Page<WOMMakeBatOrderHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMMakeBatOrderHead> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"makeBatOrderHead\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"makeBatOrderHead\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"makeBatOrderHead\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_batchingMaterialNeed" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  makeBatOrderHeadDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  makeBatOrderHeadDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_batchingMaterialNeed").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"makeBatOrderHead\".LAY_REC ASC, \"makeBatOrderHead\".SORT ASC");
				} else {
					orderPart.append(" \"makeBatOrderHead\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) makeBatOrderHeadDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=makeBatOrderHeadDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = makeBatOrderHeadDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMMakeBatOrderHead> result = new ArrayList<WOMMakeBatOrderHead>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMMakeBatOrderHead.class,makeBatOrderHeadDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMMakeBatOrderHead.class,makeBatOrderHeadDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMMakeBatOrderHead makeBatOrderHead : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(makeBatOrderHead.getId(), "WOM_batchingMaterialNeed_makeBatOrderHead");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(makeBatOrderHead.getId(), "WOM_batchingMaterialNeed_makeBatOrderHead");
							if(null != documents && !documents.isEmpty()) {
								makeBatOrderHead.setDocument(documents.get(0));
								makeBatOrderHead.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMMakeBatOrderHead> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMMakeBatOrderHead> result) {
		View view = null;
		...
		Map<Long, WOMMakeBatOrderHead> rsMap = new HashMap<Long, WOMMakeBatOrderHead>();
		for (WOMMakeBatOrderHead makeBatOrderHead : result) {
			ids.add(makeBatOrderHead.getId());
			rsMap.put(makeBatOrderHead.getId(), makeBatOrderHead);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(makeBatOrderHeadDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = makeBatOrderHeadDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  makeBatOrderHeadDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"makeBatOrderHead\"","WOM_7.5.0.0_batchingMaterialNeed","","WOM_MAKE_BAT_ORDER_HEADS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"makeBatOrderHead\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"makeBatOrderHead\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"makeBatOrderHead\"","WOM_7.5.0.0_batchingMaterialNeed","","WOM_MAKE_BAT_ORDER_HEADS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("makeBatOrderHead.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"makeBatOrderHead\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"makeBatOrderHead\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"makeBatOrderHead\".CID = 1 OR \"makeBatOrderHead\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("makeBatOrderHead.mc\"").append(".MAKE_BAT_ORDER_HEAD) not in ("+notinValue+")");
			}
			
			SQLQuery query = makeBatOrderHeadDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=makeBatOrderHeadDao.createNativeQuery(sql, menuCode);
	...
	// ================ 生成多选控件数据保存方法 start====================
	private void dealHeadIDNeedID(WOMMakeBatOrderHead makeBatOrderHead) {
		String addIds = makeBatOrderHead.getHeadIDNeedIDAddIds();
		String delIds = makeBatOrderHead.getHeadIDNeedIDDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					WOMMakeBatOrderPart tmp = new WOMMakeBatOrderPart();
					tmp.setHeadID(makeBatOrderHead);
					WOMBatchingMaterialNeed indirectObj = new WOMBatchingMaterialNeed();
					indirectObj.setId(Long.valueOf(item));
					tmp.setNeedID(indirectObj);
					tmp.setCid(makeBatOrderHead.getCid());
					makeBatOrderPartDao.save(tmp);
				}
			}
		}
	}

	private void setHeadIDNeedID(WOMMakeBatOrderHead makeBatOrderHead) {
		setHeadIDNeedID(makeBatOrderHead, null);
	}

	private void setHeadIDNeedID(WOMMakeBatOrderHead makeBatOrderHead, String viewCode) {
		List<WOMMakeBatOrderPart> tmpList = makeBatOrderPartDao.findByCriteria(Order.asc("id"), Restrictions.eq("headID", makeBatOrderHead), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (WOMMakeBatOrderPart item : tmpList) {
			if(item.getNeedID() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getNeedID().getId();
				String displayField = "id";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getNeedID(), displayField);
			}
		}
		makeBatOrderHead.setMakeBatOrderPartList(tmpList);
		makeBatOrderHead.setHeadIDNeedIDmultiselectIDs(ids);
		makeBatOrderHead.setHeadIDNeedIDmultiselectNames(names);
	}
	// ================ 生成多选控件数据保存方法 end ====================

	protected String getEntityCode(){return WOMMakeBatOrderHead.ENTITY_CODE;}
	protected String getModuleCode(){return WOMMakeBatOrderHead.MODULE_CODE;}
	protected String getModelCode(){return WOMMakeBatOrderHead.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param makeBatOrderHead 模型实例
	 */
	public void checkUniqueConstraint(WOMMakeBatOrderHead makeBatOrderHead){
	}
	
	
	@Override
	public List<WOMMakeBatOrderHead> getMakeBatOrderHeads(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMakeBatOrderHead.class);
		if(params!=null){
			...
			return makeBatOrderHeadDao.findByHql("from " + WOMMakeBatOrderHead.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return makeBatOrderHeadDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMMakeBatOrderHead> findMakeBatOrderHeadsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMakeBatOrderHead.class);
	...
	@Override
	public List<WOMMakeBatOrderHead> findMakeBatOrderHeadsByHql(String hql, Object... objects){
		
		return   makeBatOrderHeadDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMMakeBatOrderHead loadMakeBatOrderHeadByBussinessKey(WOMMakeBatOrderHead makeBatOrderHead){
		return loadMakeBatOrderHeadByBussinessKey(makeBatOrderHead.getId());
	}
	
	@Override
	public Page<WOMMakeBatOrderHead> getMakeBatOrderHeads(Page<WOMMakeBatOrderHead> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMakeBatOrderHead.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = makeBatOrderHeadDao.findByPage(page, sql);
			}else{
				page = makeBatOrderHeadDao.findAllByPage(page);
			}
			return page;
		}
		page = makeBatOrderHeadDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMakeBatOrderHead loadMakeBatOrderHeadByBussinessKey(Serializable bussinessKey){
		return makeBatOrderHeadDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMMakeBatOrderHead> findByProperty(String propertyName, Object object){
		return makeBatOrderHeadDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMakeBatOrderHead findEntityByProperty(String propertyName, Object object){
		return makeBatOrderHeadDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMMakeBatOrderHead.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = makeBatOrderHeadDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead","runtime");
		try {
	...
	public void dealDatagridsSave(WOMMakeBatOrderHead makeBatOrderHead,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,makeBatOrderHeadDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMMakeBatOrderHead.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, makeBatOrderHead);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",makeBatOrderHead.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMMakeBatOrderHead.TABLE_NAME +" where valid = 1";
			list = makeBatOrderHeadDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMMakeBatOrderHead.TABLE_NAME;
			list = makeBatOrderHeadDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMMakeBatOrderHead.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = makeBatOrderHeadDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMMakeBatOrderHeadDaoImpl makeBatOrderHeadDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchMakeBatOrderHead(final List<WOMMakeBatOrderHead> insertObjs, final List<WOMMakeBatOrderHead> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchMakeBatOrderHead(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchMakeBatOrderHead(final List<WOMMakeBatOrderHead> insertObjs, final List<WOMMakeBatOrderHead> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1491799480124"),importCount));
		...
		Map<Long, WOMMakeBatOrderHead> oldUpdateObjsMap = new HashMap<Long, WOMMakeBatOrderHead>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMMakeBatOrderHead bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMMakeBatOrderHead> bizObjList = makeBatOrderHeadDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMMakeBatOrderHead bizObj = bizObjList.get(mIndex);
		...
		final List<WOMMakeBatOrderHead> nmeObjs = new ArrayList<WOMMakeBatOrderHead>();
		...
		long id=autoGeneratorID.getNextId(WOMMakeBatOrderHead.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMMakeBatOrderHead makeBatOrderHead:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMMakeBatOrderHead.TABLE_NAME,1,dbId);
			if(makeBatOrderHead.getId() != null ){
				m.put(makeBatOrderHead.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) makeBatOrderHeadDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		makeBatOrderHeadDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMMakeBatOrderHead.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMMakeBatOrderHead makeBatOrderHead = updateObjs.get(updateIndex);
						Long id = makeBatOrderHead.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = makeBatOrderHead.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(makeBatOrderHead);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMMakeBatOrderHead.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMMakeBatOrderHead.TABLE_NAME + " order by id desc";
//					Query query = makeBatOrderHeadDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMMakeBatOrderHead makeBatOrderHead = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = makeBatOrderHead.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		makeBatOrderHeadDaoImpl.getSession().flush();
		makeBatOrderHeadDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMMakeBatOrderHead bizObj = insertObjs.get(insertIndex);
			makeBatOrderHeadImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMMakeBatOrderHead bizObj = updateObjs.get(updateIndex);
			WOMMakeBatOrderHead oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			makeBatOrderHeadImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMMakeBatOrderHead> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMMakeBatOrderHead> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) makeBatOrderHeadDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMMakeBatOrderHead> result = getResult(page, query, new PendingResultTransformer(WOMMakeBatOrderHead.class,makeBatOrderHeadDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMMakeBatOrderHead model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = makeBatOrderHeadDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = makeBatOrderHeadDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = makeBatOrderHeadDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = makeBatOrderHeadDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?makeBatOrderHeadDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  makeBatOrderHeadDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,makeBatOrderHeadDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1491803986856Page(Page<WOMMakeBatOrderPart> dg1491803986856Page,WOMMakeBatOrderHead makeBatOrderHead, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMMakeBatOrderPart.class);
		criteria.add(Restrictions.eq("headID", makeBatOrderHead));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"makeBatOrderPart\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		makeBatOrderPartDao.findByPage(dg1491803986856Page, criteria);
		makeBatOrderPartService.findByNormalDataGridMakeBatOrderHead(dg1491803986856Page);
	}
	
	 
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="ec.flow.save")
	public void saveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, DataGridService dataGridService){
		saveMakeBatOrderHead(makeBatOrderHead, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="ec.flow.save")
	public void saveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, DataGridService dataGridService, String viewCode){
		saveMakeBatOrderHead(makeBatOrderHead, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead")
	public void saveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveMakeBatOrderHead(makeBatOrderHead, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead")
	public void saveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchingMaterialNeed";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
		String url = null;
		if(null != makeBatOrderHead.getId() && makeBatOrderHead.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMakeBatOrderHead.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead")
	public Map<Object, Object> makeBatOrderHeadDataGridImport(WOMMakeBatOrderHead makeBatOrderHead, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = makeBatOrderHead.getVirtualId();
		
		makeBatOrderHead = makeBatOrderHeadDao.load(makeBatOrderHead.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, makeBatOrderHead.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = makeBatOrderHead.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(makeBatOrderHead);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, makeBatOrderHead.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead")
	public void mergeMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(makeBatOrderHead);
		beforeSaveMakeBatOrderHead(makeBatOrderHead);
		makeBatOrderHeadDao.merge(makeBatOrderHead);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead",desc="foundation.batch.save",operType="4")
	public void batchSaveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_batchingMaterialNeed";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
	...
	@Override
	public Page<WOMMakeBatOrderHead> getByPage(Page<WOMMakeBatOrderHead> page,DetachedCriteria detachedCriteria){
		return makeBatOrderHeadDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, Object... objects){
		checkUniqueConstraint(makeBatOrderHead);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportMakeBatOrderHead(List<WOMMakeBatOrderHead> makeBatOrderHeads, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportMakeBatOrderHead(List<WOMMakeBatOrderHead> makeBatOrderHeads){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportMakeBatOrderHead(List<WOMMakeBatOrderHead> makeBatOrderHeads, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportMakeBatOrderHead(List<WOMMakeBatOrderHead> makeBatOrderHeads){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMMakeBatOrderHead makeBatOrderHead, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMMakeBatOrderHead makeBatOrderHead, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportMakeBatOrderHead(List<WOMMakeBatOrderHead> insertObjs, List<WOMMakeBatOrderHead> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMakeBatOrderHead,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportMakeBatOrderHead(List<WOMMakeBatOrderHead> insertObjs, List<WOMMakeBatOrderHead> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMakeBatOrderHead,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
