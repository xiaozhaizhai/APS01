...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMPrepareMaterialPartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMPrepareMaterialPart> {
	...
	private WOMPrepareMaterialPart prepareMaterialPart;
	...
	//导出excel时,主模型内容
	List<WOMPrepareMaterialPart> queryResults = new ArrayList<WOMPrepareMaterialPart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/prepareMaterial/prepareMaterialPart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/prepareMaterial/prepareMaterialPart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMPrepareMaterialPart> prepareMaterialParts;
	
	public List<WOMPrepareMaterialPart> getPrepareMaterialParts() {
		return prepareMaterialParts;
	}
	...
	private Page<WOMPrepareMaterialPart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			prepareMaterialPart = new WOMPrepareMaterialPart();
		} else {
			if(id > 0){
				prepareMaterialPart = prepareMaterialPartService.getPrepareMaterialPart(id, viewCode);
			}
		}
	...
	public Page<WOMPrepareMaterialPart> getPage() {
		if (null == page) {
			page = new Page<WOMPrepareMaterialPart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMPrepareMaterialPart getPrepareMaterialPart() {
		return prepareMaterialPart;
	}		

	public void setPrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart) {
		this.prepareMaterialPart = prepareMaterialPart;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prepareMaterialPartService.getPrepareMaterialPartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1489976792140
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String prepareMaterialPartReferenceCopy() throws Exception{
			WOMPrepareMaterialPart oldPrepareMaterialPart = null;
			if(isEditNew()){
				...
				oldPrepareMaterialPart = new WOMPrepareMaterialPart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prepareMaterialPart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prepareMaterialPart.getId());
				...
				oldPrepareMaterialPart = prepareMaterialPartService.getPrepareMaterialPart(id);
			}
			
			
			prepareMaterialPart = prepareMaterialPartService.getPrepareMaterialPart(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMPrepareMaterialPart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPrepareMaterialPart, null);
						Method setValue = WOMPrepareMaterialPart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prepareMaterialPart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prepareMaterialPart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=prepareMaterialPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return prepareMaterialPartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			prepareMaterialPart = prepareMaterialPartService.getPrepareMaterialPart(this.prepareMaterialPart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prepareMaterialPart.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			prepareMaterialPartService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/prepareDetailsList", results = { @Result(type = FREEMARKER, location = "prepareDetailsList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = prepareMaterialPartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(prepareMaterialPartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(prepareMaterialPartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/preparePartRef", results = { @Result(type = FREEMARKER, location = "preparePartRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/prepareDetailsList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.formulaId.tableNo,result.productId.productCode,result.productId.productName,result.formulaId.applyStaff.name,result.formulaId.applyDate,result.prepareNum,result.hasDoneNum,result.formulaId.wareID.wareCode,result.formulaId.wareID.wareName,result.storeID.wareId.cargoStatue.id,result.storeID.wareId.cargoStatue.value,result.storeID.placeSetCode,result.storeID.placeSetName,result.remark,result.formulaId.id,result.productId.id,result.formulaId.applyStaff.id,result.formulaId.wareID.id,result.storeID.wareId.id,result.storeID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="prepareDetailsList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"prepareMaterialPart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"prepareMaterialPart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"prepareMaterialPart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"prepareMaterialPart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			prepareMaterialPartService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/prepareDetailsList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/preparePartRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.formulaId.tableNo,result.formulaId.manulOrderMain.manulOrderNum,result.productId.productName,result.productId.productCode,result.useNum,result.productId.productBaseUnit.name,result.hasDoneNum,result.batchNum,result.formulaId.incode,result.formulaId.outCode,result.rspos,result.rsnum,result.formulaId.backFlag,result.formulaId.id,result.formulaId.manulOrderMain.id,result.productId.id,result.productId.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="preparePartRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		prepareMaterialPartService.findDataGridPage(dg,dgclass,getDgPage(),prepareMaterialPart,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(prepareMaterialPart.getId() != null && prepareMaterialPart.getId() != -1){
				prepareMaterialPart = prepareMaterialPartService.getPrepareMaterialPart(prepareMaterialPart.getId());
			}
			prepareMaterialPartService.findDataGridPage(dg,dgclass,getDgPage(),prepareMaterialPart,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == prepareMaterialPart.getId()) {
		...
		prepareMaterialPartService.savePrepareMaterialPart(prepareMaterialPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "submit");
		bap_validate_datagrids=prepareMaterialPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", prepareMaterialPart.getId());
		...
		fileuploads.put("linkId", prepareMaterialPart.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prepareMaterialPartService.deletePrepareMaterialPart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "restore");
		prepareMaterialPartService.restorePrepareMaterialPart(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMPrepareMaterialPart> findExportDataInfos(Page<WOMPrepareMaterialPart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = prepareMaterialPartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = prepareMaterialPartService.getAssProperty(p.getCode());	
									//String assModelCode = prepareMaterialPartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			prepareMaterialPartService.findPrepareMaterialParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			prepareMaterialPartService.findPrepareMaterialParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMPrepareMaterialPart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMPrepareMaterialPart result : results){
	...
	public boolean isEditNew(){
		//return null == this.prepareMaterialPart || null == this.prepareMaterialPart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/prepareMaterial/prepareMaterialPart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/prepareMaterial/prepareMaterialPart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMPrepareMaterialPartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
		_auditLog.setColumnStringA(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
		_auditLog.setColumnStringC(prepareMaterialPart.getId().toString());
		_auditLog.setColumnStringD(WOMPrepareMaterialPart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMPrepareMaterialPartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, prepareMaterialPart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, prepareMaterialPart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,prepareMaterialPart);
		...
		printlog.setBusinessId(prepareMaterialPart.getId());
		printlog.setBussinessKey(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
		printlog.setMainDisplay(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(prepareMaterialPart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(prepareMaterialPartService.getPrepareMaterialPart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, prepareMaterialPart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				prepareMaterialPart = prepareMaterialPartService.getPrepareMaterialPart(Long.parseLong(arrId[i]));
				if(null != prepareMaterialPart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == prepareMaterialPart.getId() ? "" : "," + prepareMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == prepareMaterialPart.getId() ? "" : "," + prepareMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == prepareMaterialPart.getId() ? "" : "," + prepareMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, prepareMaterialPart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,prepareMaterialPart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//prepareMaterialPartService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterialPart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = prepareMaterialPartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
		if("WOM_7.5.0.0_prepareMaterial_preparePartRef".equals(code) && "preparePartRef".equals(methodName)){
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
