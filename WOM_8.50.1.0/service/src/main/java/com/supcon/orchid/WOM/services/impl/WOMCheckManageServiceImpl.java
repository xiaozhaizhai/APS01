...
import com.supcon.orchid.WOM.services.WOMCheckManageImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMCheckManageDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_checkManageService")
@Transactional
public class WOMCheckManageServiceImpl  extends BaseServiceImpl<WOMCheckManage> implements WOMCheckManageService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMCheckManage> bapBaseService;
	...
	@Autowired
	private WOMCheckManageImportService checkManageImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMCheckManage.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  checkManageDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMCheckManage.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = checkManageDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMCheckManage.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMCheckManage.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  checkManageDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMCheckManage.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMCheckManage.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMCheckManage> list = (List<WOMCheckManage>) checkManageDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMCheckManage getCheckManage(long id){
		return getCheckManage(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMCheckManage getCheckManage(long id, String viewCode){
		WOMCheckManage checkManage = checkManageDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getCheckManageAsJSON(long id, String include){
		WOMCheckManage checkManage = checkManageDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(checkManage, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.common.delete",operType="3")
	public void deleteCheckManage(WOMCheckManage checkManage){
		if(null != checkManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCheckManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkManage.getId() ? "" : checkManage.getId().toString());
			AuditUtil.setColumnStringA(null == checkManage.getId() ? "" : checkManage.getId().toString());
			AuditUtil.setColumnStringC(checkManage.getId().toString());
		}
		beforeDeleteCheckManage(checkManage);
		checkManageDao.deletePhysical(checkManage);
		afterDeleteCheckManage(checkManage);
		checkManageDao.flush();
		List<WOMCheckManage> params = new ArrayList<WOMCheckManage>();
		params.add(checkManage);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.common.delete",operType="3")
	public void deleteCheckManage(List<Long> checkManageIds){
		deleteCheckManage(checkManageIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.common.delete",operType="3")
	public void deleteCheckManage(List<Long> checkManageIds, String eventTopic) {
		List<WOMCheckManage> checkManages = new ArrayList<WOMCheckManage>();
		for(Long checkManageId : checkManageIds){
			WOMCheckManage checkManage = getCheckManage(checkManageId);
			checkManages.add(checkManage);
			if(null != checkManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCheckManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == checkManage.getId() ? "" : "," + checkManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == checkManage.getId() ? "" : checkManage.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == checkManage.getId() ? "" : "," + checkManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == checkManage.getId() ? "" : checkManage.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == checkManage.getId() ? "" : "," + checkManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == checkManage.getId() ? "" : checkManage.getId().toString());
				}
			}
		}
		...
		if(checkManages != null && checkManages.size() > 0 ){
			for(WOMCheckManage checkManage : checkManages){
				...
				java.lang.reflect.Field[] fields = WOMCheckManage.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(checkManage));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),checkManage, checkManage.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(checkManageIds != null && checkManageIds.size() > 0) {
			String hql = "delete " + WOMCheckManage.JPA_NAME + " where id in(:ids)";
			Query query = checkManageDao.createQuery(hql);
			query.setParameterList("ids", checkManageIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMCheckManage checkManagez : checkManages){
				checkManagez.setValid(false);
				List<WOMCheckManage> params = new ArrayList<WOMCheckManage>();
				params.add(checkManagez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_generalManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.common.delete",operType="3")
	public void deleteCheckManage(long checkManageId, int checkManageVersion){
		this.deleteCheckManage(checkManageId, checkManageVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.common.delete",operType="3")
	public void deleteCheckManage(long checkManageId, int checkManageVersion,SignatureLog signatureLog){
		WOMCheckManage checkManage = getCheckManage(checkManageId);
		...
		beforeDeleteCheckManage(checkManage);
		checkManageDao.delete(checkManageId, checkManageVersion);
		afterDeleteCheckManage(checkManage);
		checkManageDao.flush();
		List<WOMCheckManage> params = new ArrayList<WOMCheckManage>();
		params.add(checkManage);
		...
		if(signatureLog != null ) {
			...
			businessKey=checkManage.getId();
			signatureLog.setTableId(checkManage.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.common.delete",operType="3")
	public void deleteCheckManage(String checkManageIds){
		this.deleteCheckManage(checkManageIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.common.delete",operType="3")
	public void deleteCheckManage(String checkManageIds,SignatureLog signatureLog){
		deleteCollection(checkManageIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteCheckManage(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_generalManage#modelCode:WOM_7.5.0.0_generalManage_CheckManage#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="ec.engine.restore",operType="10")
	public void restoreCheckManage(String checkManageIds){
		restoreCollection(checkManageIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreCheckManage(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="ec.engine.restore",operType="10")
	public void restoreCheckManage(long checkManageId){
		
		findBusinessKeyUsed(checkManageId);	//判断业务主键是否重复
		...
		beforeRestoreCheckManage(checkManage);
		checkManage.setValid(true);
		checkManageDao.update(checkManage);
		afterRestoreCheckManage(checkManage);
		List<WOMCheckManage> params = new ArrayList<WOMCheckManage>();
		params.add(checkManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long checkManageId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_generalManage_CheckManage");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMCheckManage.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMCheckManage.TABLE_NAME +" where id = ? )";
			List<Object> list =  checkManageDao.createNativeQuery(sql,checkManageId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseCheckManage(List<WOMCheckManage>  checkManages){
		for(WOMCheckManage checkManage:checkManages)  {
			saveCheckManage(checkManage, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseCheckManage(List<WOMCheckManage>  checkManages){
		for(WOMCheckManage checkManage:checkManages)  {
			saveCheckManage(checkManage, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="ec.flow.save")
	public void saveCheckManage(WOMCheckManage checkManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveCheckManage(checkManage, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="ec.flow.save")
	public void saveCheckManage(WOMCheckManage checkManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveCheckManage(checkManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateCheckManageCodes(WOMCheckManage checkManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCheckManageCodes(checkManage, false);
	}
	
	public void generateCheckManageCodes(WOMCheckManage checkManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateCheckManageSummarys(WOMCheckManage checkManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCheckManageSummarys(checkManage, false);
	}

	public void generateCheckManageSummarys(WOMCheckManage checkManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage")
	public void saveCheckManage(WOMCheckManage checkManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveCheckManage(checkManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage")
	public void saveCheckManage(WOMCheckManage checkManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(checkManage.getId() != null && checkManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCheckManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, checkManage);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(checkManage);
		beforeSaveCheckManage(checkManage, viewIsView);

		if (viewIsView) {
			checkManageDao.saveWithRevertVersion(checkManage);
		} else {
			if(isNew)
				checkManageDao.save(checkManage);
			else
				checkManageDao.merge(checkManage);
		}
		if(null != checkManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCheckManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkManage.getId() ? "" : checkManage.getId().toString());
			AuditUtil.setColumnStringA(null == checkManage.getId() ? "" : checkManage.getId().toString());
		...
		dealDatagridsSave(checkManage,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateCheckManageCodes(checkManage, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateCheckManageSummarys(checkManage, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveCheckManage(checkManage, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, checkManage);
			}
		}
		List<WOMCheckManage> params = new ArrayList<WOMCheckManage>();
		checkManageDao.flush();
		checkManageDao.clear();
		checkManage = checkManageDao.load(checkManage.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMCheckManage checkManage){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("checkManage", checkManage);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage")
	public void mergeCheckManage(WOMCheckManage checkManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(checkManage);
		beforeSaveCheckManage(checkManage);
		checkManageDao.merge(checkManage);
		if(null != checkManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCheckManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkManage.getId() ? "" : checkManage.getId().toString());
			AuditUtil.setColumnStringA(null == checkManage.getId() ? "" : checkManage.getId().toString());
		...
		dealDatagridsSave(checkManage,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveCheckManage(checkManage);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMCheckManage> findCheckManages(Page<WOMCheckManage> page, Criterion... criterions) {
		return checkManageDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckManages(Page<WOMCheckManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findCheckManages(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckManages(Page<WOMCheckManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckManages(Page<WOMCheckManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMCheckManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"checkManage\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"checkManage\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"checkManage\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"checkManage\".PRODUCE_TASK IN ( SELECT ID FROM 指令单 \"produceTaskManage\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  checkManageDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  checkManageDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_generalManage").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"checkManage\".LAY_REC ASC, \"checkManage\".SORT ASC");
				} else {
					orderPart.append(" \"checkManage\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) checkManageDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=checkManageDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = checkManageDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMCheckManage> result = new ArrayList<WOMCheckManage>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMCheckManage.class,checkManageDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMCheckManage.class,checkManageDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMCheckManage checkManage : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(checkManage.getId(), "WOM_generalManage_checkManage");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(checkManage.getId(), "WOM_generalManage_checkManage");
							if(null != documents && !documents.isEmpty()) {
								checkManage.setDocument(documents.get(0));
								checkManage.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMCheckManage> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMCheckManage> result) {
		View view = null;
		...
		Map<Long, WOMCheckManage> rsMap = new HashMap<Long, WOMCheckManage>();
		for (WOMCheckManage checkManage : result) {
			ids.add(checkManage.getId());
			rsMap.put(checkManage.getId(), checkManage);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(checkManageDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = checkManageDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_generalManage_CheckManage" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_generalManage_CheckManage" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  checkManageDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"checkManage\"","WOM_7.5.0.0_generalManage","","WOM_CHECK_MENAGE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"checkManage\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"checkManage\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"checkManage\"","WOM_7.5.0.0_generalManage","","WOM_CHECK_MENAGE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_generalManage_CheckManage" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("checkManage.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"checkManage\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"checkManage\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"checkManage\".CID = 1 OR \"checkManage\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("checkManage.mc\"").append(".CHECK_MANAGE) not in ("+notinValue+")");
			}
			
			SQLQuery query = checkManageDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=checkManageDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMCheckManage.ENTITY_CODE;}
	protected String getModuleCode(){return WOMCheckManage.MODULE_CODE;}
	protected String getModelCode(){return WOMCheckManage.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param checkManage 模型实例
	 */
	public void checkUniqueConstraint(WOMCheckManage checkManage){
	}
	
	
	@Override
	public List<WOMCheckManage> getCheckManages(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMCheckManage.class);
		if(params!=null){
			...
			return checkManageDao.findByHql("from " + WOMCheckManage.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return checkManageDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMCheckManage> findCheckManagesByHql(String hql, Object... objects){
		...
		return   checkManageDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMCheckManage loadCheckManageByBussinessKey(WOMCheckManage checkManage){
		return loadCheckManageByBussinessKey(checkManage.getId());
	}
	
	@Override
	public Page<WOMCheckManage> getCheckManages(Page<WOMCheckManage> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMCheckManage.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = checkManageDao.findByPage(page, sql);
			}else{
				page = checkManageDao.findAllByPage(page);
			}
			return page;
		}
		page = checkManageDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMCheckManage> findByTreeDataGridProduceTaskManage(List<WOMCheckManage> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMCheckManage checkManage : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTaskManage(Page<WOMCheckManage> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMCheckManage> checkManages = dgPage.getResult();
			for(WOMCheckManage checkManage : checkManages){
				
			}
			dgPage.setResult(checkManages);
		}
	...
	public WOMCheckManage loadCheckManageByBussinessKey(Serializable bussinessKey){
		return checkManageDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMCheckManage> findByProperty(String propertyName, Object object){
		return checkManageDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMCheckManage findEntityByProperty(String propertyName, Object object){
		return checkManageDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMCheckManage.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = checkManageDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_generalManage_CheckManage","runtime");
		try {
	...
	public void dealDatagridsSave(WOMCheckManage checkManage,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,checkManageDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMCheckManage.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, checkManage);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",checkManage.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMCheckManage.TABLE_NAME +" where valid = 1";
			list = checkManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMCheckManage.TABLE_NAME;
			list = checkManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMCheckManage.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = checkManageDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMCheckManageDaoImpl checkManageDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchCheckManage(final List<WOMCheckManage> insertObjs, final List<WOMCheckManage> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchCheckManage(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchCheckManage(final List<WOMCheckManage> insertObjs, final List<WOMCheckManage> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1502696273660"),importCount));
		...
		Map<Long, WOMCheckManage> oldUpdateObjsMap = new HashMap<Long, WOMCheckManage>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMCheckManage bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMCheckManage> bizObjList = checkManageDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMCheckManage bizObj = bizObjList.get(mIndex);
		...
		final List<WOMCheckManage> nmeObjs = new ArrayList<WOMCheckManage>();
		...
		long id=autoGeneratorID.getNextId(WOMCheckManage.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMCheckManage checkManage:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMCheckManage.TABLE_NAME,1,dbId);
			if(checkManage.getId() != null ){
				m.put(checkManage.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) checkManageDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_CheckManage"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		checkManageDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMCheckManage.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMCheckManage checkManage = updateObjs.get(updateIndex);
						Long id = checkManage.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = checkManage.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMCheckManage.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMCheckManage.TABLE_NAME + " order by id desc";
//					Query query = checkManageDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMCheckManage checkManage = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = checkManage.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		checkManageDaoImpl.getSession().flush();
		checkManageDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMCheckManage bizObj = insertObjs.get(insertIndex);
			checkManageImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMCheckManage bizObj = updateObjs.get(updateIndex);
			WOMCheckManage oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			checkManageImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMCheckManage> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMCheckManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) checkManageDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMCheckManage> result = getResult(page, query, new PendingResultTransformer(WOMCheckManage.class,checkManageDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMCheckManage model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = checkManageDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_CheckManage").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = checkManageDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = checkManageDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = checkManageDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?checkManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  checkManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,checkManageDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="ec.flow.save")
	public void saveCheckManage(WOMCheckManage checkManage, DataGridService dataGridService){
		saveCheckManage(checkManage, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="ec.flow.save")
	public void saveCheckManage(WOMCheckManage checkManage, DataGridService dataGridService, String viewCode){
		saveCheckManage(checkManage, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage")
	public void saveCheckManage(WOMCheckManage checkManage, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveCheckManage(checkManage, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage")
	public void saveCheckManage(WOMCheckManage checkManage, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(null != checkManage.getId() && checkManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCheckManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage")
	public Map<Object, Object> checkManageDataGridImport(WOMCheckManage checkManage, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = checkManage.getVirtualId();
		
		checkManage = checkManageDao.load(checkManage.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, checkManage.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = checkManage.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(checkManage);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, checkManage.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage")
	public void mergeCheckManage(WOMCheckManage checkManage, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(checkManage);
		beforeSaveCheckManage(checkManage);
		checkManageDao.merge(checkManage);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_CheckManage",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMCheckManage> getByPage(Page<WOMCheckManage> page,DetachedCriteria detachedCriteria){
		return checkManageDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveCheckManage(WOMCheckManage checkManage, Object... objects){
		checkUniqueConstraint(checkManage);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveCheckManage(WOMCheckManage checkManage, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteCheckManage(WOMCheckManage checkManage){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteCheckManage(WOMCheckManage checkManage){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreCheckManage(WOMCheckManage checkManage){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreCheckManage(WOMCheckManage checkManage){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMCheckManage checkManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMCheckManage checkManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportCheckManage(List<WOMCheckManage> insertObjs, List<WOMCheckManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportCheckManage,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportCheckManage(List<WOMCheckManage> insertObjs, List<WOMCheckManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportCheckManage,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_generalManage_CheckManage,WOM_7.5.0.0) */
// 自定义代码
