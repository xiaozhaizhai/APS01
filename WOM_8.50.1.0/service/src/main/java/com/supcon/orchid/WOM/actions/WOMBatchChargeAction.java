...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchChargeAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchCharge> {
	...
	private WOMBatchCharge batchCharge;
	...
	//导出excel时,主模型内容
	List<WOMBatchCharge> queryResults = new ArrayList<WOMBatchCharge>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/procFeedback/batchCharge/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/procFeedback/batchCharge/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_procFeedback_BatchCharge" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchCharge> batchCharges;
	
	public List<WOMBatchCharge> getBatchCharges() {
		return batchCharges;
	}
	...
	private Page<WOMBatchCharge> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchCharge = new WOMBatchCharge();
		} else {
			if(id > 0){
				batchCharge = batchChargeService.getBatchCharge(id, viewCode);
			}
		}
	...
	public Page<WOMBatchCharge> getPage() {
		if (null == page) {
			page = new Page<WOMBatchCharge>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchCharge getBatchCharge() {
		return batchCharge;
	}		

	public void setBatchCharge(WOMBatchCharge batchCharge) {
		this.batchCharge = batchCharge;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/procFeedback/batchCharge/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchChargeService.getBatchChargeAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/procFeedback/batchCharge/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1540953150452
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchChargeReferenceCopy() throws Exception{
			WOMBatchCharge oldBatchCharge = null;
			if(isEditNew()){
				...
				oldBatchCharge = new WOMBatchCharge();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchCharge.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchCharge.getId());
				...
				oldBatchCharge = batchChargeService.getBatchCharge(id);
			}
			
			
			batchCharge = batchChargeService.getBatchCharge(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_BatchCharge");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchCharge.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchCharge, null);
						Method setValue = WOMBatchCharge.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchCharge, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchCharge.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchChargeService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchChargeReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchCharge = batchChargeService.getBatchCharge(this.batchCharge.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchCharge.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchChargeService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchChargeService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchChargeService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_procFeedback_BatchCharge", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchCharge\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchCharge\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchCharge\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchCharge\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchChargeService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchChargeService.findDataGridPage(dg,dgclass,getDgPage(),batchCharge,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchCharge.getId() != null && batchCharge.getId() != -1){
				batchCharge = batchChargeService.getBatchCharge(batchCharge.getId());
			}
			batchChargeService.findDataGridPage(dg,dgclass,getDgPage(),batchCharge,searchCondition,params);
	...
	private void save() throws Exception {
		...
		batchChargeService.saveBatchCharge(batchCharge,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "submit");
		bap_validate_datagrids=batchChargeService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchCharge.getId());
		...
		fileuploads.put("linkId", batchCharge.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/procFeedback/batchCharge/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchChargeService.deleteBatchCharge(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/batchCharge/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "restore");
		batchChargeService.restoreBatchCharge(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchCharge> findExportDataInfos(Page<WOMBatchCharge> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchChargeService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchChargeService.getAssProperty(p.getCode());	
									//String assModelCode = batchChargeService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchChargeService.findBatchCharges(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchChargeService.findBatchCharges(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchCharge> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchCharge result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchCharge || null == this.batchCharge.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/procFeedback/batchCharge/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/procFeedback/batchCharge/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/procFeedback/batchCharge/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/batchCharge/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchChargeService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/batchCharge/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_procFeedback_BatchCharge", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_procFeedback_BatchCharge", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/procFeedback/batchCharge/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/procFeedback/batchCharge/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_procFeedback_BatchCharge";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_BatchCharge", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/procFeedback/batchCharge/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMBatchCharge batchCharge = null;
		if(null != id) {
			batchCharge = batchChargeService.getBatchCharge(id);
		}
		...
		_auditLog.setColumnStringB(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
		_auditLog.setColumnStringA(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
		_auditLog.setColumnStringC(batchCharge.getId().toString());
		_auditLog.setColumnStringD(WOMBatchCharge.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchChargeAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchCharge,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchCharge,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchCharge);
		...
		printlog.setBusinessId(batchCharge.getId());
		printlog.setBussinessKey(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
		printlog.setMainDisplay(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/procFeedback/batchCharge/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchCharge,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/batchCharge/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchChargeService.getBatchCharge(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/procFeedback/batchCharge/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/procFeedback/batchCharge/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchCharge,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchCharge = batchChargeService.getBatchCharge(Long.parseLong(arrId[i]));
				if(null != batchCharge){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchCharge.getId() ? "" : "," + batchCharge.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchCharge.getId() ? "" : "," + batchCharge.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchCharge.getId() ? "" : "," + batchCharge.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchCharge,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchCharge);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/procFeedback/batchCharge/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_BatchCharge");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_procFeedback_BatchCharge")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_procFeedback_BatchCharge");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_BatchCharge");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchChargeService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
