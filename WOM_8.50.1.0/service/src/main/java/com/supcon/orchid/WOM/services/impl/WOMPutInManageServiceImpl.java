...
import com.supcon.orchid.WOM.services.WOMPutInManageImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMPutInManageDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_putInManageService")
@Transactional
public class WOMPutInManageServiceImpl  extends BaseServiceImpl<WOMPutInManage> implements WOMPutInManageService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMPutInManage> bapBaseService;
	...
	@Autowired
	private WOMPutInManageImportService putInManageImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMPutInManage.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  putInManageDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMPutInManage.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = putInManageDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMPutInManage.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMPutInManage.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  putInManageDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMPutInManage.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMPutInManage.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMPutInManage> list = (List<WOMPutInManage>) putInManageDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMPutInManage getPutInManage(long id){
		return getPutInManage(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPutInManage getPutInManage(long id, String viewCode){
		WOMPutInManage putInManage = putInManageDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPutInManageAsJSON(long id, String include){
		WOMPutInManage putInManage = putInManageDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(putInManage, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.common.delete",operType="3")
	public void deletePutInManage(WOMPutInManage putInManage){
		if(null != putInManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == putInManage.getId() ? "" : putInManage.getId().toString());
			AuditUtil.setColumnStringA(null == putInManage.getId() ? "" : putInManage.getId().toString());
			AuditUtil.setColumnStringC(putInManage.getId().toString());
		}
		beforeDeletePutInManage(putInManage);
		putInManageDao.deletePhysical(putInManage);
		afterDeletePutInManage(putInManage);
		putInManageDao.flush();
		List<WOMPutInManage> params = new ArrayList<WOMPutInManage>();
		params.add(putInManage);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.common.delete",operType="3")
	public void deletePutInManage(List<Long> putInManageIds){
		deletePutInManage(putInManageIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.common.delete",operType="3")
	public void deletePutInManage(List<Long> putInManageIds, String eventTopic) {
		List<WOMPutInManage> putInManages = new ArrayList<WOMPutInManage>();
		for(Long putInManageId : putInManageIds){
			WOMPutInManage putInManage = getPutInManage(putInManageId);
			putInManages.add(putInManage);
			if(null != putInManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == putInManage.getId() ? "" : "," + putInManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == putInManage.getId() ? "" : putInManage.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == putInManage.getId() ? "" : "," + putInManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == putInManage.getId() ? "" : putInManage.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == putInManage.getId() ? "" : "," + putInManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == putInManage.getId() ? "" : putInManage.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(putInManages != null && putInManages.size() > 0 ){
			for(WOMPutInManage putInManage : putInManages){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMPutInManage.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(putInManage));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),putInManage, putInManage.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(putInManageIds != null && putInManageIds.size() > 0) {
			String hql = "delete " + WOMPutInManage.JPA_NAME + " where id in(:ids)";
			Query query = putInManageDao.createQuery(hql);
			query.setParameterList("ids", putInManageIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMPutInManage putInManagez : putInManages){
				putInManagez.setValid(false);
				List<WOMPutInManage> params = new ArrayList<WOMPutInManage>();
				params.add(putInManagez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_generalManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.common.delete",operType="3")
	public void deletePutInManage(long putInManageId, int putInManageVersion){
		this.deletePutInManage(putInManageId, putInManageVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.common.delete",operType="3")
	public void deletePutInManage(long putInManageId, int putInManageVersion,SignatureLog signatureLog){
		WOMPutInManage putInManage = getPutInManage(putInManageId);
		...
		beforeDeletePutInManage(putInManage);
		putInManageDao.delete(putInManageId, putInManageVersion);
		afterDeletePutInManage(putInManage);
		putInManageDao.flush();
		List<WOMPutInManage> params = new ArrayList<WOMPutInManage>();
		params.add(putInManage);
		...
		if(signatureLog != null ) {
			...
			businessKey=putInManage.getId();
			signatureLog.setTableId(putInManage.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.common.delete",operType="3")
	public void deletePutInManage(String putInManageIds){
		this.deletePutInManage(putInManageIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.common.delete",operType="3")
	public void deletePutInManage(String putInManageIds,SignatureLog signatureLog){
		deleteCollection(putInManageIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePutInManage(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_generalManage#modelCode:WOM_7.5.0.0_generalManage_PutInManage#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="ec.engine.restore",operType="10")
	public void restorePutInManage(String putInManageIds){
		restoreCollection(putInManageIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePutInManage(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="ec.engine.restore",operType="10")
	public void restorePutInManage(long putInManageId){
		
		findBusinessKeyUsed(putInManageId);	//判断业务主键是否重复
		...
		beforeRestorePutInManage(putInManage);
		putInManage.setValid(true);
		putInManageDao.update(putInManage);
		afterRestorePutInManage(putInManage);
		List<WOMPutInManage> params = new ArrayList<WOMPutInManage>();
		params.add(putInManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long putInManageId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_generalManage_PutInManage");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMPutInManage.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMPutInManage.TABLE_NAME +" where id = ? )";
			List<Object> list =  putInManageDao.createNativeQuery(sql,putInManageId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePutInManage(List<WOMPutInManage>  putInManages){
		for(WOMPutInManage putInManage:putInManages)  {
			savePutInManage(putInManage, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePutInManage(List<WOMPutInManage>  putInManages){
		for(WOMPutInManage putInManage:putInManages)  {
			savePutInManage(putInManage, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="ec.flow.save")
	public void savePutInManage(WOMPutInManage putInManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePutInManage(putInManage, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="ec.flow.save")
	public void savePutInManage(WOMPutInManage putInManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePutInManage(putInManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePutInManageCodes(WOMPutInManage putInManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePutInManageCodes(putInManage, false);
	}
	
	public void generatePutInManageCodes(WOMPutInManage putInManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePutInManageSummarys(WOMPutInManage putInManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePutInManageSummarys(putInManage, false);
	}

	public void generatePutInManageSummarys(WOMPutInManage putInManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage")
	public void savePutInManage(WOMPutInManage putInManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePutInManage(putInManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage")
	public void savePutInManage(WOMPutInManage putInManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(putInManage.getId() != null && putInManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, putInManage);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(putInManage);
		beforeSavePutInManage(putInManage, viewIsView);

		if (viewIsView) {
			putInManageDao.saveWithRevertVersion(putInManage);
		} else {
			if(isNew)
				putInManageDao.save(putInManage);
			else
				putInManageDao.merge(putInManage);
		}
		if(null != putInManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == putInManage.getId() ? "" : putInManage.getId().toString());
			AuditUtil.setColumnStringA(null == putInManage.getId() ? "" : putInManage.getId().toString());
		...
		dealDatagridsSave(putInManage,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePutInManageCodes(putInManage, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePutInManageSummarys(putInManage, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePutInManage(putInManage, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, putInManage);
			}
		}
		List<WOMPutInManage> params = new ArrayList<WOMPutInManage>();
		putInManageDao.flush();
		putInManageDao.clear();
		putInManage = putInManageDao.load(putInManage.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMPutInManage putInManage){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("putInManage", putInManage);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage")
	public void mergePutInManage(WOMPutInManage putInManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(putInManage);
		beforeSavePutInManage(putInManage);
		putInManageDao.merge(putInManage);
		if(null != putInManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == putInManage.getId() ? "" : putInManage.getId().toString());
			AuditUtil.setColumnStringA(null == putInManage.getId() ? "" : putInManage.getId().toString());
		...
		dealDatagridsSave(putInManage,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePutInManage(putInManage);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMPutInManage> findPutInManages(Page<WOMPutInManage> page, Criterion... criterions) {
		return putInManageDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPutInManages(Page<WOMPutInManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPutInManages(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPutInManages(Page<WOMPutInManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPutInManages(Page<WOMPutInManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMPutInManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"putInManage\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"putInManage\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"putInManage\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"putInManage\".PRODUCE_TASK IN ( SELECT ID FROM 指令单 \"produceTaskManage\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  putInManageDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  putInManageDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_generalManage").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"putInManage\".LAY_REC ASC, \"putInManage\".SORT ASC");
				} else {
					orderPart.append(" \"putInManage\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) putInManageDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=putInManageDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = putInManageDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMPutInManage> result = new ArrayList<WOMPutInManage>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMPutInManage.class,putInManageDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMPutInManage.class,putInManageDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMPutInManage putInManage : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(putInManage.getId(), "WOM_generalManage_putInManage");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(putInManage.getId(), "WOM_generalManage_putInManage");
							if(null != documents && !documents.isEmpty()) {
								putInManage.setDocument(documents.get(0));
								putInManage.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMPutInManage> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMPutInManage> result) {
		View view = null;
		...
		Map<Long, WOMPutInManage> rsMap = new HashMap<Long, WOMPutInManage>();
		for (WOMPutInManage putInManage : result) {
			ids.add(putInManage.getId());
			rsMap.put(putInManage.getId(), putInManage);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(putInManageDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = putInManageDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_generalManage_PutInManage" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_generalManage_PutInManage" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  putInManageDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"putInManage\"","WOM_7.5.0.0_generalManage","","WOM_PUT_IN_MENAGE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"putInManage\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"putInManage\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"putInManage\"","WOM_7.5.0.0_generalManage","","WOM_PUT_IN_MENAGE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_generalManage_PutInManage" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("putInManage.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"putInManage\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"putInManage\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"putInManage\".CID = 1 OR \"putInManage\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("putInManage.mc\"").append(".PUT_IN_MANAGE) not in ("+notinValue+")");
			}
			
			SQLQuery query = putInManageDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=putInManageDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMPutInManage.ENTITY_CODE;}
	protected String getModuleCode(){return WOMPutInManage.MODULE_CODE;}
	protected String getModelCode(){return WOMPutInManage.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param putInManage 模型实例
	 */
	public void checkUniqueConstraint(WOMPutInManage putInManage){
	}
	
	
	@Override
	public List<WOMPutInManage> getPutInManages(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPutInManage.class);
		if(params!=null){
			...
			return putInManageDao.findByHql("from " + WOMPutInManage.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return putInManageDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMPutInManage> findPutInManagesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPutInManage.class);
	...
	@Override
	public List<WOMPutInManage> findPutInManagesByHql(String hql, Object... objects){
		
		return   putInManageDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMPutInManage loadPutInManageByBussinessKey(WOMPutInManage putInManage){
		return loadPutInManageByBussinessKey(putInManage.getId());
	}
	
	@Override
	public Page<WOMPutInManage> getPutInManages(Page<WOMPutInManage> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPutInManage.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = putInManageDao.findByPage(page, sql);
			}else{
				page = putInManageDao.findAllByPage(page);
			}
			return page;
		}
		page = putInManageDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMPutInManage> findByTreeDataGridProduceTaskManage(List<WOMPutInManage> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMPutInManage putInManage : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTaskManage(Page<WOMPutInManage> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMPutInManage> putInManages = dgPage.getResult();
			for(WOMPutInManage putInManage : putInManages){
				
			}
			dgPage.setResult(putInManages);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPutInManage loadPutInManageByBussinessKey(Serializable bussinessKey){
		return putInManageDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMPutInManage> findByProperty(String propertyName, Object object){
		return putInManageDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPutInManage findEntityByProperty(String propertyName, Object object){
		return putInManageDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMPutInManage.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = putInManageDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_generalManage_PutInManage","runtime");
		try {
	...
	public void dealDatagridsSave(WOMPutInManage putInManage,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,putInManageDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMPutInManage.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, putInManage);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",putInManage.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMPutInManage.TABLE_NAME +" where valid = 1";
			list = putInManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMPutInManage.TABLE_NAME;
			list = putInManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMPutInManage.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = putInManageDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMPutInManageDaoImpl putInManageDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPutInManage(final List<WOMPutInManage> insertObjs, final List<WOMPutInManage> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPutInManage(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPutInManage(final List<WOMPutInManage> insertObjs, final List<WOMPutInManage> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1502695666438"),importCount));
		...
		Map<Long, WOMPutInManage> oldUpdateObjsMap = new HashMap<Long, WOMPutInManage>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMPutInManage bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMPutInManage> bizObjList = putInManageDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMPutInManage bizObj = bizObjList.get(mIndex);
		...
		final List<WOMPutInManage> nmeObjs = new ArrayList<WOMPutInManage>();
		...
		long id=autoGeneratorID.getNextId(WOMPutInManage.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMPutInManage putInManage:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMPutInManage.TABLE_NAME,1,dbId);
			if(putInManage.getId() != null ){
				m.put(putInManage.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) putInManageDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_PutInManage"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		putInManageDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMPutInManage.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMPutInManage putInManage = updateObjs.get(updateIndex);
						Long id = putInManage.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = putInManage.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMPutInManage.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMPutInManage.TABLE_NAME + " order by id desc";
//					Query query = putInManageDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMPutInManage putInManage = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = putInManage.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		putInManageDaoImpl.getSession().flush();
		putInManageDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMPutInManage bizObj = insertObjs.get(insertIndex);
			putInManageImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMPutInManage bizObj = updateObjs.get(updateIndex);
			WOMPutInManage oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			putInManageImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMPutInManage> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMPutInManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) putInManageDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMPutInManage> result = getResult(page, query, new PendingResultTransformer(WOMPutInManage.class,putInManageDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMPutInManage model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = putInManageDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_PutInManage").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = putInManageDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = putInManageDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = putInManageDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?putInManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  putInManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,putInManageDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="ec.flow.save")
	public void savePutInManage(WOMPutInManage putInManage, DataGridService dataGridService){
		savePutInManage(putInManage, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="ec.flow.save")
	public void savePutInManage(WOMPutInManage putInManage, DataGridService dataGridService, String viewCode){
		savePutInManage(putInManage, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage")
	public void savePutInManage(WOMPutInManage putInManage, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePutInManage(putInManage, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage")
	public void savePutInManage(WOMPutInManage putInManage, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(null != putInManage.getId() && putInManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage")
	public Map<Object, Object> putInManageDataGridImport(WOMPutInManage putInManage, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = putInManage.getVirtualId();
		
		putInManage = putInManageDao.load(putInManage.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, putInManage.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = putInManage.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(putInManage);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, putInManage.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage")
	public void mergePutInManage(WOMPutInManage putInManage, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(putInManage);
		beforeSavePutInManage(putInManage);
		putInManageDao.merge(putInManage);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_PutInManage",desc="foundation.batch.save",operType="4")
	public void batchSavePutInManage(WOMPutInManage putInManage, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
	...
	@Override
	public Page<WOMPutInManage> getByPage(Page<WOMPutInManage> page,DetachedCriteria detachedCriteria){
		return putInManageDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSavePutInManage(WOMPutInManage putInManage, Object... objects){
		checkUniqueConstraint(putInManage);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePutInManage(WOMPutInManage putInManage, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPutInManage(List<WOMPutInManage> putInManages, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPutInManage(List<WOMPutInManage> putInManages){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPutInManage(List<WOMPutInManage> putInManages, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPutInManage(List<WOMPutInManage> putInManages){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePutInManage(WOMPutInManage putInManage){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePutInManage(WOMPutInManage putInManage){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePutInManage(WOMPutInManage putInManage){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePutInManage(WOMPutInManage putInManage){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMPutInManage putInManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMPutInManage putInManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPutInManage(List<WOMPutInManage> insertObjs, List<WOMPutInManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPutInManage,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPutInManage(List<WOMPutInManage> insertObjs, List<WOMPutInManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPutInManage,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
