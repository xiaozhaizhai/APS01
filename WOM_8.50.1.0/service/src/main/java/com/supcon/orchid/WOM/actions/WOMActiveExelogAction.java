...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_activeExelog_ActiveExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMActiveExelogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMActiveExelog> {
	...
	private WOMActiveExelog activeExelog;
	...
	//导出excel时,主模型内容
	List<WOMActiveExelog> queryResults = new ArrayList<WOMActiveExelog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/activeExelog/activeExelog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/activeExelog/activeExelog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_activeExelog_ActiveExelog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMActiveExelog> activeExelogs;
	
	public List<WOMActiveExelog> getActiveExelogs() {
		return activeExelogs;
	}
	...
	private Page<WOMActiveExelog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			activeExelog = new WOMActiveExelog();
			if(activeExelog.getExeState() != null) {
				activeExelog.setExeState((SystemCode) systemCodeService.getSystemCode(activeExelog.getExeState().getId()));
			}
		} else {
			if(id > 0){
				activeExelog = activeExelogService.getActiveExelog(id, viewCode);
				if(tableInfoId == null && activeExelog != null){
					tableInfoId = activeExelog.getTableInfoId();
				}
			}
		}
	...
	public Page<WOMActiveExelog> getPage() {
		if (null == page) {
			page = new Page<WOMActiveExelog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMActiveExelog getActiveExelog() {
		return activeExelog;
	}		

	public void setActiveExelog(WOMActiveExelog activeExelog) {
		this.activeExelog = activeExelog;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/activeExelog/activeExelog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = activeExelogService.getActiveExelogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/activeExelog/activeExelog/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String activeExelogReferenceCopy() throws Exception{
			WOMActiveExelog oldActiveExelog = null;
			if(isEditNew()){
				...
				oldActiveExelog = new WOMActiveExelog();
			}else{
			//WOMActiveExelog activeExelog = activeExelogService.getActiveExelog(this.activeExelog.getId());
			this.creatorInfo = creatorService.get(activeExelog.getCreateStaffId(), activeExelog.getCreatePositionId(), activeExelog.getCreateDepartmentId());
			//setActiveExelog(activeExelog);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(activeExelog.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,activeExelog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, activeExelog.getId());
				...
				oldActiveExelog = activeExelogService.getActiveExelog(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldActiveExelog.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_activeExelog_" + oldActiveExelog.getId();
			...
			activeExelog = activeExelogService.getActiveExelog(refId);
			activeExelog.setPending(pending);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_activeExelog_ActiveExelog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMActiveExelog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldActiveExelog, null);
						Method setValue = WOMActiveExelog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(activeExelog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			activeExelog.setCreateDepartmentId(oldActiveExelog.getCreateDepartmentId());
			activeExelog.setCreatePositionId(oldActiveExelog.getCreatePositionId());
			activeExelog.setCreateStaffId(oldActiveExelog.getCreateStaffId());
			activeExelog.setOwnerDepartmentId(oldActiveExelog.getOwnerDepartmentId());
			activeExelog.setOwnerPositionId(oldActiveExelog.getOwnerPositionId());
			activeExelog.setOwnerStaffId(oldActiveExelog.getOwnerStaffId());
			activeExelog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "referenceCopy");
	...
	@Actions({
		...
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "activeExelogEdit.ftl") }),
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "activeExelogEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=activeExelogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return activeExelogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			activeExelog = activeExelogService.getActiveExelog(this.activeExelog.getId(), viewCode);
			this.creatorInfo = creatorService.get(activeExelog.getCreateStaffId(), activeExelog.getCreatePositionId(), activeExelog.getCreateDepartmentId());
			//setActiveExelog(activeExelog);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = activeExelogService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = activeExelogService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + activeExelog.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_activeExelog_" + activeExelog.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<WOMActiveExelog> tmpList = new ArrayList<>();
			tmpList.add(activeExelog);
			activeExelogService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = activeExelogService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,activeExelog.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "activeExelogView.ftl") }),
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "activeExelogViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogList", results = { @Result(type = FREEMARKER, location = "activeExelogList.ftl") })
	})
	public String viewList() throws Exception{
		...
		//mainViewPath = "/WOM/activeExelog/activeExelog/activeExelogView.action";
		mainViewPath=activeExelogService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = activeExelogService.getWorkFlowInfo(workFlowMenuCode);
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = activeExelogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(activeExelogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(activeExelogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_activeExelog_ActiveExelog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.taskID.productId.productName,result.taskID.productId.productCode,result.taskID.productBatchNum,result.taskProcessID.name,result.taskProcessID.formulaId.code,result.taskProcessID.formulaId.name,result.factoryId.name,result.taskID.factoryId.name,result.taskActiveID.name,result.startTime,result.endTime,result.staffID.name,result.exeState.id,result.exeState.value,result.remark,result.taskID.id,result.taskID.productId.id,result.taskProcessID.id,result.taskProcessID.formulaId.id,result.factoryId.id,result.taskID.factoryId.id,result.taskActiveID.id,result.staffID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="activeExelogList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"activeExelog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"activeExelog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			activeExelogService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<WOMActiveExelog> activeExelogList=page.getResult();
				for(WOMActiveExelog lz:activeExelogList){
	...
	@Actions({
		...
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.taskID.productId.productName,result.taskID.productId.productCode,result.taskID.productBatchNum,result.taskProcessID.name,result.taskProcessID.formulaId.code,result.taskProcessID.formulaId.name,result.factoryId.name,result.taskID.factoryId.name,result.taskActiveID.name,result.startTime,result.endTime,result.staffID.name,result.exeState.id,result.exeState.value,result.remark,result.taskID.id,result.taskID.productId.id,result.taskProcessID.id,result.taskProcessID.formulaId.id,result.factoryId.id,result.taskID.factoryId.id,result.taskActiveID.id,result.staffID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"activeExelog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"activeExelog\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"activeExelog\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	 ...
	 * @return
	 */
	@Action(value = "/WOM/activeExelog/activeExelog/allTempleteDownload", results = { @Result(name="excel",type="excel",location="activeExelogList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_activeExelog_ActiveExelog");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		activeExelogService.findDataGridPage(dg,dgclass,getDgPage(),activeExelog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(activeExelog.getId() != null && activeExelog.getId() != -1){
				activeExelog = activeExelogService.getActiveExelog(activeExelog.getId());
			}
			activeExelogService.findDataGridPage(dg,dgclass,getDgPage(),activeExelog,searchCondition,params);
	...
	private void save() throws Exception {
		...
		activeExelogService.save(activeExelog,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(activeExelog.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + activeExelog.getProcessKey() + "_" + p.getActivityName() + "_WOM_7.5.0.0_activeExelog_" + activeExelog.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(activeExelog.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/WOM/activeExelog/activeExelog/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		activeExelogService.retrialThisTable(activeExelog,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "retrial");
		return SUCCESS;
	}
	@Actions({
		@Action(value = "/WOM/activeExelog/activeExelog/activeExelogEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/activeExelog/activeExelog/activeExelogView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "submit");
		bap_validate_datagrids=activeExelogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (activeExelog != null && activeExelog.getStatus() != null && activeExelog.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				activeExelogService.saveSuperEditActiveExelog(activeExelog, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				activeExelogService.submit(activeExelog,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", activeExelog.getTableInfoId());
		responseMap.put("id", activeExelog.getId());
		...
		fileuploads.put("linkId", activeExelog.getTableInfoId());
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/activeExelog/activeExelog/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "bulkSubmit");
		activeExelogService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/WOM/activeExelog/activeExelog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		activeExelogService.deleteActiveExelog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/activeExelog/activeExelog/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "restore");
		activeExelogService.restoreActiveExelog(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/activeExelog/activeExelog/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/activeExelog/activeExelog/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "showDealInfoData");
		if (null != tableInfoId) {
			int count=activeExelogService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = activeExelogService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = activeExelogService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMActiveExelog> findExportDataInfos(Page<WOMActiveExelog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = activeExelogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = activeExelogService.getAssProperty(p.getCode());	
									//String assModelCode = activeExelogService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			activeExelogService.findActiveExelogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			activeExelogService.findActiveExelogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMActiveExelog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMActiveExelog result : results){
	...
	public boolean isEditNew(){
		//return null == this.activeExelog || null == this.activeExelog.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && activeExelog != null && activeExelog.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + activeExelog.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return activeExelogService.isRecallAble(tableInfoId);
	}

	@Action(value = "/WOM/activeExelog/activeExelog/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = activeExelogService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + activeExelog.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + activeExelog.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/activeExelog/activeExelog/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "batchApproval");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/activeExelog/activeExelog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/activeExelog/activeExelog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/activeExelog/activeExelog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/activeExelog/activeExelog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMActiveExelogService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_activeExelog", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_activeExelog");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/activeExelog/activeExelog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_activeExelog_ActiveExelog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_activeExelog_ActiveExelog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/activeExelog/activeExelog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/activeExelog/activeExelog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_activeExelog_ActiveExelog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_activeExelog_ActiveExelog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/activeExelog/activeExelog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		if(null != id) {
			activeExelog = activeExelogService.getActiveExelog(id);
		}
		...
		_auditLog.setColumnStringB(null == activeExelog.getTableNo() ? "" : activeExelog.getTableNo().toString());
		_auditLog.setColumnStringA(null == activeExelog.getId() ? "" : activeExelog.getId().toString());
		_auditLog.setColumnStringC(activeExelog.getId().toString());
		_auditLog.setColumnStringD(WOMActiveExelog.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_activeExelog");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMActiveExelogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, activeExelog,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, activeExelog,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,activeExelog);
		...
		printlog.setEntity("WOM_7.5.0.0_activeExelog");
		...
		printlog.setBusinessId(activeExelog.getId());
		printlog.setBussinessKey(null == activeExelog.getTableNo() ? "" : activeExelog.getTableNo().toString());
		printlog.setMainDisplay(null == activeExelog.getId() ? "" : activeExelog.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/activeExelog/activeExelog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(activeExelog,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/activeExelog/activeExelog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(activeExelogService.getActiveExelog(Long.parseLong(arrId[i])));
	...
	@Action(value = "/WOM/activeExelog/activeExelog/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/activeExelog/activeExelog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, activeExelog,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				activeExelog = activeExelogService.getActiveExelog(Long.parseLong(arrId[i]));
				if(null != activeExelog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == activeExelog.getId() ? "" : "," + activeExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == activeExelog.getId() ? "" : activeExelog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == activeExelog.getTableNo() ? "" : "," + activeExelog.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == activeExelog.getTableNo() ? "" : activeExelog.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == activeExelog.getId() ? "" : "," + activeExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == activeExelog.getId() ? "" : activeExelog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, activeExelog,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,activeExelog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/activeExelog/activeExelog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMActiveExelog> findExportPageInfos(Page<WOMActiveExelog> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_activeExelog" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = activeExelogService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMActiveExelog queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = activeExelogService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = activeExelogService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_activeExelog_ActiveExelog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_activeExelog_ActiveExelog")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_activeExelog_ActiveExelog");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_activeExelog_ActiveExelog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = activeExelogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_activeExelog_ActiveExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_activeExelog_ActiveExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_activeExelog_ActiveExelog,WOM_7.5.0.0) */
// 自定义代码
