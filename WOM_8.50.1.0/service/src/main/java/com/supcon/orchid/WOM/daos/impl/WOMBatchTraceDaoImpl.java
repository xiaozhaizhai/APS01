...
@Repository
public class WOMBatchTraceDaoImpl extends ExtendedGenericDaoImpl<WOMBatchTrace, Long> implements WOMBatchTraceDao {
	@Override
	...
	@Override
	public WOMBatchTraceDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMBatchTraceDealInfo dealInfo = new WOMBatchTraceDealInfo();
	...
	@Override
	public WOMBatchTraceDealInfo copyAndSaveDealInfo(DealInfo di, WOMBatchTrace mainObj, Staff dealStaff) {
		WOMBatchTraceDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMBatchTraceDealInfo.class);
		...
		List<WOMBatchTraceDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_batchTimeLiner_BatchTrace,WOM_7.5.0.0) */
// 自定义代码
