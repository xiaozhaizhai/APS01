...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMDayBatchPlanPartsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMDayBatchPlanParts> {
	...
	private WOMDayBatchPlanParts dayBatchPlanParts;
	...
	//导出excel时,主模型内容
	List<WOMDayBatchPlanParts> queryResults = new ArrayList<WOMDayBatchPlanParts>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/producePlanRef/dayBatchPlanParts/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/producePlanRef/dayBatchPlanParts/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_producePlanRef_DayBatchPlanParts" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMDayBatchPlanParts> dayBatchPlanPartss;
	
	public List<WOMDayBatchPlanParts> getDayBatchPlanPartss() {
		return dayBatchPlanPartss;
	}
	...
	private Page<WOMDayBatchPlanParts> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			dayBatchPlanParts = new WOMDayBatchPlanParts();
		} else {
			if(id > 0){
				dayBatchPlanParts = dayBatchPlanPartsService.getDayBatchPlanParts(id, viewCode);
			}
		}
	...
	public Page<WOMDayBatchPlanParts> getPage() {
		if (null == page) {
			page = new Page<WOMDayBatchPlanParts>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMDayBatchPlanParts getDayBatchPlanParts() {
		return dayBatchPlanParts;
	}		

	public void setDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts) {
		this.dayBatchPlanParts = dayBatchPlanParts;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = dayBatchPlanPartsService.getDayBatchPlanPartsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1489822962143
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String dayBatchPlanPartsReferenceCopy() throws Exception{
			WOMDayBatchPlanParts oldDayBatchPlanParts = null;
			if(isEditNew()){
				...
				oldDayBatchPlanParts = new WOMDayBatchPlanParts();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,dayBatchPlanParts.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, dayBatchPlanParts.getId());
				...
				oldDayBatchPlanParts = dayBatchPlanPartsService.getDayBatchPlanParts(id);
			}
			
			
			dayBatchPlanParts = dayBatchPlanPartsService.getDayBatchPlanParts(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMDayBatchPlanParts.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldDayBatchPlanParts, null);
						Method setValue = WOMDayBatchPlanParts.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(dayBatchPlanParts, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			dayBatchPlanParts.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=dayBatchPlanPartsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return dayBatchPlanPartsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			dayBatchPlanParts = dayBatchPlanPartsService.getDayBatchPlanParts(this.dayBatchPlanParts.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,dayBatchPlanParts.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			dayBatchPlanPartsService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = dayBatchPlanPartsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(dayBatchPlanPartsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(dayBatchPlanPartsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/batchPlanOneRef", results = { @Result(type = FREEMARKER, location = "batchPlanOneRef.ftl") })
		,
		@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/batchPlanRef", results = { @Result(type = FREEMARKER, location = "batchPlanRef.ftl") })
		,
		@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/planRef", results = { @Result(type = FREEMARKER, location = "planRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"dayBatchPlanParts\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"dayBatchPlanParts\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"factoryCode\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"manudeptId\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"dayBatchPlanParts\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"dayBatchPlanParts\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			dayBatchPlanPartsService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/batchPlanOneRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.dayPlanInfo.tableNo,result.productId.productCode,result.productId.productName,result.batchNum,result.quantity,result.formularID.code,result.formularID.name,result.factoryCode.name,result.startTime,result.endTime,result.dayPlanInfo.id,result.productId.id,result.formularID.id,result.factoryCode.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchPlanOneRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/batchPlanRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.dayPlanInfo.tableNo,result.productId.productCode,result.productId.productName,result.batchNum,result.quantity,result.formularID.code,result.formularID.name,result.factoryCode.name,result.startTime,result.endTime,result.dayPlanInfo.id,result.productId.id,result.formularID.id,result.factoryCode.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchPlanRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/planRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.dayPlanInfo.tableNo,result.productId.productCode,result.productId.productName,result.productId.productModel,result.productId.productSpecif,result.productId.productBaseUnit.name,result.batchNum,result.quantity,result.formularID.code,result.formularID.name,result.formularID.setProcess.id,result.formularID.setProcess.value,result.formularID.produceType.id,result.formularID.produceType.value,result.factoryCode.name,result.formularID.supportEbr,result.startTime,result.endTime,result.dayPlanInfo.id,result.productId.id,result.productId.productBaseUnit.id,result.formularID.id,result.factoryCode.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="planRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		dayBatchPlanPartsService.findDataGridPage(dg,dgclass,getDgPage(),dayBatchPlanParts,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(dayBatchPlanParts.getId() != null && dayBatchPlanParts.getId() != -1){
				dayBatchPlanParts = dayBatchPlanPartsService.getDayBatchPlanParts(dayBatchPlanParts.getId());
			}
			dayBatchPlanPartsService.findDataGridPage(dg,dgclass,getDgPage(),dayBatchPlanParts,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == dayBatchPlanParts.getId()) {
		...
		dayBatchPlanPartsService.saveDayBatchPlanParts(dayBatchPlanParts,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "submit");
		bap_validate_datagrids=dayBatchPlanPartsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", dayBatchPlanParts.getId());
		...
		fileuploads.put("linkId", dayBatchPlanParts.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		dayBatchPlanPartsService.deleteDayBatchPlanParts(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "restore");
		dayBatchPlanPartsService.restoreDayBatchPlanParts(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMDayBatchPlanParts> findExportDataInfos(Page<WOMDayBatchPlanParts> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = dayBatchPlanPartsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = dayBatchPlanPartsService.getAssProperty(p.getCode());	
									//String assModelCode = dayBatchPlanPartsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			dayBatchPlanPartsService.findDayBatchPlanPartss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			dayBatchPlanPartsService.findDayBatchPlanPartss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMDayBatchPlanParts> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMDayBatchPlanParts result : results){
	...
	public boolean isEditNew(){
		//return null == this.dayBatchPlanParts || null == this.dayBatchPlanParts.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/producePlanRef/dayBatchPlanParts/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/producePlanRef/dayBatchPlanParts/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMDayBatchPlanPartsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMDayBatchPlanPartsService"
						...
						, "com.supcon.orchid.WOM.services.WOMDayBatchPlanInfoService"       
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
		_auditLog.setColumnStringA(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
		_auditLog.setColumnStringC(dayBatchPlanParts.getId().toString());
		_auditLog.setColumnStringD(WOMDayBatchPlanParts.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMDayBatchPlanPartsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, dayBatchPlanParts,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, dayBatchPlanParts,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,dayBatchPlanParts);
		...
		printlog.setBusinessId(dayBatchPlanParts.getId());
		printlog.setBussinessKey(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
		printlog.setMainDisplay(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(dayBatchPlanParts,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(dayBatchPlanPartsService.getDayBatchPlanParts(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, dayBatchPlanParts,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				dayBatchPlanParts = dayBatchPlanPartsService.getDayBatchPlanParts(Long.parseLong(arrId[i]));
				if(null != dayBatchPlanParts){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == dayBatchPlanParts.getId() ? "" : "," + dayBatchPlanParts.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == dayBatchPlanParts.getId() ? "" : "," + dayBatchPlanParts.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == dayBatchPlanParts.getId() ? "" : "," + dayBatchPlanParts.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, dayBatchPlanParts,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,dayBatchPlanParts);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//dayBatchPlanPartsService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanParts/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = dayBatchPlanPartsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
		//指令单打开参照界面参数
      	if("WOM_7.5.0.0_producePlanRef_planRef".equals(code)&&"planRefQuery".equals(methodName)){	//测试方案参照查询时
          	if(planWorkType.equals("makeTask")){//制造
          		String sql= "\"dayBatchPlanParts\".FORMULARID in(select ID from RM_FORMULAS where (( PRODUCE_TYPE = 'produceType/produceType1' or  PRODUCE_TYPE = 'produceType/produceType4' or  PRODUCE_TYPE = 'produceType/produceType5') and valid = 1)) and \"dayBatchPlanParts\".ID not in(select PLAN_PAR_ID from WOM_PRODUCE_TASKS where ( (STATUS = '88' or  STATUS = '77') and PLAN_PAR_ID is not null and valid = 1))";
          		params.add(new Param("customerSql", sql));
          	}
          	if(planWorkType.equals("packageTask")){//包装
          		String sql= "\"dayBatchPlanParts\".FORMULARID in(select ID from RM_FORMULAS where (( PRODUCE_TYPE = 'produceType/produceType2' or  PRODUCE_TYPE = 'produceType/produceType3') and valid = 1)) and \"dayBatchPlanParts\".ID not in(select PLAN_PAR_ID from WOM_PRODUCE_TASKS where ( (STATUS = '88' or  STATUS = '77') and PLAN_PAR_ID is not null and valid = 1))";
          		params.add(new Param("customerSql", sql));
          	}
          	if(planWorkType.equals("recycleTask")){//回收
          		String sql= "\"dayBatchPlanParts\".FORMULARID in(select ID from RM_FORMULAS where ( PRODUCE_TYPE = 'produceType/produceType6' and valid = 1)) and \"dayBatchPlanParts\".ID not in(select PLAN_PAR_ID from WOM_PRODUCE_TASKS where ( (STATUS = '88' or  STATUS = '77') and PLAN_PAR_ID is not null and valid = 1))";
          		params.add(new Param("customerSql", sql));
          	}
          	if(planWorkType.equals("otherTask")){//其他
          		String sql= "\"dayBatchPlanParts\".FORMULARID in(select ID from RM_FORMULAS where ( PRODUCE_TYPE = 'produceType/produceType7' and valid = 1)) and \"dayBatchPlanParts\".ID not in(select PLAN_PAR_ID from WOM_PRODUCE_TASKS where ( (STATUS = '88' or  STATUS = '77') and PLAN_PAR_ID is not null and valid = 1))";
          		params.add(new Param("customerSql", sql));
          	}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
	private String planWorkType;//计划单据编号类型：制造/包装



	public String getPlanWorkType() {
		return planWorkType;
	}

	public void setPlanWorkType(String planWorkType) {
		this.planWorkType = planWorkType;
	}
/* CUSTOM CODE END */
}