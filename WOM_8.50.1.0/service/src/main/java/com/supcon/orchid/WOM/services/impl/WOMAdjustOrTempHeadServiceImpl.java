...
import com.supcon.orchid.WOM.services.WOMAdjustOrTempHeadImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMAdjustOrTempHeadDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.WOM.entities.WOMProduceTaskProcess;
...
@Service("wOM_adjustOrTempHeadService")
@Transactional
public class WOMAdjustOrTempHeadServiceImpl  extends BaseServiceImpl<WOMAdjustOrTempHead> implements WOMAdjustOrTempHeadService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMAdjustOrTempHead> bapBaseService;
	...
	@Autowired
	private WOMAdjustOrTempHeadImportService adjustOrTempHeadImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMAdjustOrTempHead.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  adjustOrTempHeadDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMAdjustOrTempHead.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = adjustOrTempHeadDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMAdjustOrTempHead.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMAdjustOrTempHead.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  adjustOrTempHeadDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMAdjustOrTempHead.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMAdjustOrTempHead.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMAdjustOrTempHead> list = (List<WOMAdjustOrTempHead>) adjustOrTempHeadDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMAdjustOrTempHead getAdjustOrTempHead(long id){
		return getAdjustOrTempHead(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMAdjustOrTempHead getAdjustOrTempHead(long id, String viewCode){
		WOMAdjustOrTempHead adjustOrTempHead = adjustOrTempHeadDao.load(id);

		// 一对多情况处理
		if(adjustOrTempHead != null){
		}
		return adjustOrTempHead;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getAdjustOrTempHeadAsJSON(long id, String include){
		WOMAdjustOrTempHead adjustOrTempHead = adjustOrTempHeadDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(adjustOrTempHead, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead){
		if(null != adjustOrTempHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
			AuditUtil.setColumnStringA(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
			AuditUtil.setColumnStringC(adjustOrTempHead.getId().toString());
		}
		beforeDeleteAdjustOrTempHead(adjustOrTempHead);
		adjustOrTempHeadDao.deletePhysical(adjustOrTempHead);
		afterDeleteAdjustOrTempHead(adjustOrTempHead);
		adjustOrTempHeadDao.flush();
		List<WOMAdjustOrTempHead> params = new ArrayList<WOMAdjustOrTempHead>();
		params.add(adjustOrTempHead);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempHead(List<Long> adjustOrTempHeadIds){
		deleteAdjustOrTempHead(adjustOrTempHeadIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempHead(List<Long> adjustOrTempHeadIds, String eventTopic) {
		List<WOMAdjustOrTempHead> adjustOrTempHeads = new ArrayList<WOMAdjustOrTempHead>();
		for(Long adjustOrTempHeadId : adjustOrTempHeadIds){
			WOMAdjustOrTempHead adjustOrTempHead = getAdjustOrTempHead(adjustOrTempHeadId);
			adjustOrTempHeads.add(adjustOrTempHead);
			if(null != adjustOrTempHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == adjustOrTempHead.getId() ? "" : "," + adjustOrTempHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == adjustOrTempHead.getId() ? "" : "," + adjustOrTempHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == adjustOrTempHead.getId() ? "" : "," + adjustOrTempHead.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
				}
			}
		}
		
		for(WOMAdjustOrTempHead adjustOrTempHead : adjustOrTempHeads){
			beforeDeleteAdjustOrTempHead(adjustOrTempHead);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(adjustOrTempHeads != null && adjustOrTempHeads.size() > 0 ){
			for(WOMAdjustOrTempHead adjustOrTempHead : adjustOrTempHeads){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMAdjustOrTempHead.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(adjustOrTempHead));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),adjustOrTempHead, adjustOrTempHead.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(adjustOrTempHeadIds != null && adjustOrTempHeadIds.size() > 0) {
			String hql = "delete " + WOMAdjustOrTempHead.JPA_NAME + " where id in(:ids)";
			Query query = adjustOrTempHeadDao.createQuery(hql);
			query.setParameterList("ids", adjustOrTempHeadIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMAdjustOrTempHead adjustOrTempHeadz : adjustOrTempHeads){
				adjustOrTempHeadz.setValid(false);
				List<WOMAdjustOrTempHead> params = new ArrayList<WOMAdjustOrTempHead>();
				params.add(adjustOrTempHeadz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempHead(long adjustOrTempHeadId, int adjustOrTempHeadVersion){
		this.deleteAdjustOrTempHead(adjustOrTempHeadId, adjustOrTempHeadVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempHead(long adjustOrTempHeadId, int adjustOrTempHeadVersion,SignatureLog signatureLog){
		WOMAdjustOrTempHead adjustOrTempHead = getAdjustOrTempHead(adjustOrTempHeadId);
		if(adjustOrTempHead != null && !adjustOrTempHead.isValid()){
		...
		beforeDeleteAdjustOrTempHead(adjustOrTempHead);
		adjustOrTempHeadDao.delete(adjustOrTempHeadId, adjustOrTempHeadVersion);
		afterDeleteAdjustOrTempHead(adjustOrTempHead);
		adjustOrTempHeadDao.flush();
		List<WOMAdjustOrTempHead> params = new ArrayList<WOMAdjustOrTempHead>();
		params.add(adjustOrTempHead);
		...
		if(signatureLog != null ) {
			...
			businessKey=adjustOrTempHead.getId();
			signatureLog.setTableId(adjustOrTempHead.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempHead(String adjustOrTempHeadIds){
		this.deleteAdjustOrTempHead(adjustOrTempHeadIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempHead(String adjustOrTempHeadIds,SignatureLog signatureLog){
		deleteCollection(adjustOrTempHeadIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteAdjustOrTempHead(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_AdjustOrTempHead#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="ec.engine.restore",operType="10")
	public void restoreAdjustOrTempHead(String adjustOrTempHeadIds){
		restoreCollection(adjustOrTempHeadIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreAdjustOrTempHead(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="ec.engine.restore",operType="10")
	public void restoreAdjustOrTempHead(long adjustOrTempHeadId){
		
		findBusinessKeyUsed(adjustOrTempHeadId);	//判断业务主键是否重复
		...
		beforeRestoreAdjustOrTempHead(adjustOrTempHead);
		adjustOrTempHead.setValid(true);
		adjustOrTempHeadDao.update(adjustOrTempHead);
		afterRestoreAdjustOrTempHead(adjustOrTempHead);
		List<WOMAdjustOrTempHead> params = new ArrayList<WOMAdjustOrTempHead>();
		params.add(adjustOrTempHead);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long adjustOrTempHeadId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_AdjustOrTempHead");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMAdjustOrTempHead.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMAdjustOrTempHead.TABLE_NAME +" where id = ? )";
			List<Object> list =  adjustOrTempHeadDao.createNativeQuery(sql,adjustOrTempHeadId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseAdjustOrTempHead(List<WOMAdjustOrTempHead>  adjustOrTempHeads){
		for(WOMAdjustOrTempHead adjustOrTempHead:adjustOrTempHeads)  {
			saveAdjustOrTempHead(adjustOrTempHead, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseAdjustOrTempHead(List<WOMAdjustOrTempHead>  adjustOrTempHeads){
		for(WOMAdjustOrTempHead adjustOrTempHead:adjustOrTempHeads)  {
			saveAdjustOrTempHead(adjustOrTempHead, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="ec.flow.save")
	public void saveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveAdjustOrTempHead(adjustOrTempHead, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="ec.flow.save")
	public void saveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveAdjustOrTempHead(adjustOrTempHead, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateAdjustOrTempHeadCodes(WOMAdjustOrTempHead adjustOrTempHead) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAdjustOrTempHeadCodes(adjustOrTempHead, false);
	}
	
	public void generateAdjustOrTempHeadCodes(WOMAdjustOrTempHead adjustOrTempHead, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateAdjustOrTempHeadSummarys(WOMAdjustOrTempHead adjustOrTempHead) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAdjustOrTempHeadSummarys(adjustOrTempHead, false);
	}

	public void generateAdjustOrTempHeadSummarys(WOMAdjustOrTempHead adjustOrTempHead, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead")
	public void saveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveAdjustOrTempHead(adjustOrTempHead, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead")
	public void saveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(adjustOrTempHead.getId() != null && adjustOrTempHead.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempHead.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, adjustOrTempHead);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(adjustOrTempHead);
		beforeSaveAdjustOrTempHead(adjustOrTempHead, viewIsView);

		if (viewIsView) {
			adjustOrTempHeadDao.saveWithRevertVersion(adjustOrTempHead);
		} else {
			if(isNew)
				adjustOrTempHeadDao.save(adjustOrTempHead);
			else
				adjustOrTempHeadDao.merge(adjustOrTempHead);
		}
		if(null != adjustOrTempHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
			AuditUtil.setColumnStringA(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
		...
		dealDatagridsSave(adjustOrTempHead,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateAdjustOrTempHeadCodes(adjustOrTempHead, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateAdjustOrTempHeadSummarys(adjustOrTempHead, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveAdjustOrTempHead(adjustOrTempHead, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, adjustOrTempHead);
			}
		}
		List<WOMAdjustOrTempHead> params = new ArrayList<WOMAdjustOrTempHead>();
		adjustOrTempHeadDao.flush();
		adjustOrTempHeadDao.clear();
		adjustOrTempHead = adjustOrTempHeadDao.load(adjustOrTempHead.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMAdjustOrTempHead adjustOrTempHead){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("adjustOrTempHead", adjustOrTempHead);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead")
	public void mergeAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(adjustOrTempHead);
		beforeSaveAdjustOrTempHead(adjustOrTempHead);
		adjustOrTempHeadDao.merge(adjustOrTempHead);
		if(null != adjustOrTempHead && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempHead.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
			AuditUtil.setColumnStringA(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
		...
		dealDatagridsSave(adjustOrTempHead,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveAdjustOrTempHead(adjustOrTempHead);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMAdjustOrTempHead> findAdjustOrTempHeads(Page<WOMAdjustOrTempHead> page, Criterion... criterions) {
		return adjustOrTempHeadDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAdjustOrTempHeads(Page<WOMAdjustOrTempHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findAdjustOrTempHeads(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAdjustOrTempHeads(Page<WOMAdjustOrTempHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAdjustOrTempHeads(Page<WOMAdjustOrTempHead> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMAdjustOrTempHead> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"adjustOrTempHead\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"adjustOrTempHead\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"adjustOrTempHead\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"adjustOrTempHead\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  adjustOrTempHeadDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  adjustOrTempHeadDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"adjustOrTempHead\".LAY_REC ASC, \"adjustOrTempHead\".SORT ASC");
				} else {
					orderPart.append(" \"adjustOrTempHead\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) adjustOrTempHeadDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=adjustOrTempHeadDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = adjustOrTempHeadDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMAdjustOrTempHead> result = new ArrayList<WOMAdjustOrTempHead>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMAdjustOrTempHead.class,adjustOrTempHeadDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMAdjustOrTempHead.class,adjustOrTempHeadDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMAdjustOrTempHead adjustOrTempHead : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(adjustOrTempHead.getId(), "WOM_produceTask_adjustOrTempHead");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(adjustOrTempHead.getId(), "WOM_produceTask_adjustOrTempHead");
							if(null != documents && !documents.isEmpty()) {
								adjustOrTempHead.setDocument(documents.get(0));
								adjustOrTempHead.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMAdjustOrTempHead> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMAdjustOrTempHead> result) {
		View view = null;
		...
		Map<Long, WOMAdjustOrTempHead> rsMap = new HashMap<Long, WOMAdjustOrTempHead>();
		for (WOMAdjustOrTempHead adjustOrTempHead : result) {
			ids.add(adjustOrTempHead.getId());
			rsMap.put(adjustOrTempHead.getId(), adjustOrTempHead);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(adjustOrTempHeadDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = adjustOrTempHeadDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_AdjustOrTempHead" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_AdjustOrTempHead" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  adjustOrTempHeadDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"adjustOrTempHead\"","WOM_7.5.0.0_produceTask","","WOM_ADJUST_OR_TEMP_HEADS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"adjustOrTempHead\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"adjustOrTempHead\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"adjustOrTempHead\"","WOM_7.5.0.0_produceTask","","WOM_ADJUST_OR_TEMP_HEADS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_AdjustOrTempHead" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("adjustOrTempHead.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"adjustOrTempHead\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"adjustOrTempHead\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"adjustOrTempHead\".CID = 1 OR \"adjustOrTempHead\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("adjustOrTempHead.mc\"").append(".ADJUST_OR_TEMP_HEAD) not in ("+notinValue+")");
			}
			
			SQLQuery query = adjustOrTempHeadDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=adjustOrTempHeadDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMAdjustOrTempHead.ENTITY_CODE;}
	protected String getModuleCode(){return WOMAdjustOrTempHead.MODULE_CODE;}
	protected String getModelCode(){return WOMAdjustOrTempHead.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param adjustOrTempHead 模型实例
	 */
	public void checkUniqueConstraint(WOMAdjustOrTempHead adjustOrTempHead){
	}
	
	
	@Override
	public List<WOMAdjustOrTempHead> getAdjustOrTempHeads(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMAdjustOrTempHead.class);
		if(params!=null){
			...
			return adjustOrTempHeadDao.findByHql("from " + WOMAdjustOrTempHead.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return adjustOrTempHeadDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMAdjustOrTempHead> findAdjustOrTempHeadsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMAdjustOrTempHead.class);
	...
	@Override
	public List<WOMAdjustOrTempHead> findAdjustOrTempHeadsByHql(String hql, Object... objects){
		
		return   adjustOrTempHeadDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMAdjustOrTempHead loadAdjustOrTempHeadByBussinessKey(WOMAdjustOrTempHead adjustOrTempHead){
		return loadAdjustOrTempHeadByBussinessKey(adjustOrTempHead.getId());
	}
	
	@Override
	public Page<WOMAdjustOrTempHead> getAdjustOrTempHeads(Page<WOMAdjustOrTempHead> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMAdjustOrTempHead.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = adjustOrTempHeadDao.findByPage(page, sql);
			}else{
				page = adjustOrTempHeadDao.findAllByPage(page);
			}
			return page;
		}
		page = adjustOrTempHeadDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMAdjustOrTempHead> findByTreeDataGridProduceTask(List<WOMAdjustOrTempHead> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMAdjustOrTempHead adjustOrTempHead : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTask(Page<WOMAdjustOrTempHead> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMAdjustOrTempHead> adjustOrTempHeads = dgPage.getResult();
			for(WOMAdjustOrTempHead adjustOrTempHead : adjustOrTempHeads){
				
			}
			dgPage.setResult(adjustOrTempHeads);
		}
	...
	@Override
	@Transactional
	public void deleteAdjustOrTempHeadByProduceTaskActive(List<Long> produceTaskActiveIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("processActiveIDList", produceTaskActiveIds);
		String hql = "select id from WOMAdjustOrTempHead where "
						+ "processActiveID.id in (:processActiveIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> adjustOrTempHeadIds1 = adjustOrTempHeadDao.findByHql(hql, params);
		if(adjustOrTempHeadIds1 != null && !adjustOrTempHeadIds1.isEmpty()) {
		}
		hql = "delete WOMAdjustOrTempHead where "
						+ "processActiveID.id in (:processActiveIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		adjustOrTempHeadDao.bulkExecute(hql, params);
	}
	@Override
	@Transactional
	public void deleteAdjustOrTempHeadByProduceTaskProcess(List<Long> produceTaskProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("processIdList", produceTaskProcessIds);
		String hql = "select id from WOMAdjustOrTempHead where "
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMAdjustOrTempHead loadAdjustOrTempHeadByBussinessKey(Serializable bussinessKey){
		return adjustOrTempHeadDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMAdjustOrTempHead> findByProperty(String propertyName, Object object){
		return adjustOrTempHeadDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMAdjustOrTempHead findEntityByProperty(String propertyName, Object object){
		return adjustOrTempHeadDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMAdjustOrTempHead.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = adjustOrTempHeadDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_AdjustOrTempHead","runtime");
		try {
	...
	public void dealDatagridsSave(WOMAdjustOrTempHead adjustOrTempHead,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,adjustOrTempHeadDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMAdjustOrTempHead.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, adjustOrTempHead);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(adjustOrTempHead.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(adjustOrTempHead.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(adjustOrTempHead.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",adjustOrTempHead.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMAdjustOrTempHead.TABLE_NAME +" where valid = 1";
			list = adjustOrTempHeadDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMAdjustOrTempHead.TABLE_NAME;
			list = adjustOrTempHeadDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMAdjustOrTempHead.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = adjustOrTempHeadDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMAdjustOrTempHeadDaoImpl adjustOrTempHeadDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMAdjustOrTempHead.TABLE_NAME + " where id = ?";
		String tableNo = adjustOrTempHeadDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchAdjustOrTempHead(final List<WOMAdjustOrTempHead> insertObjs, final List<WOMAdjustOrTempHead> updateObjs, 
		...
		return this.importBatchAdjustOrTempHead(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchAdjustOrTempHead(final List<WOMAdjustOrTempHead> insertObjs, final List<WOMAdjustOrTempHead> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1542097427304"),importCount));
		...
		Map<Long, WOMAdjustOrTempHead> oldUpdateObjsMap = new HashMap<Long, WOMAdjustOrTempHead>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMAdjustOrTempHead bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMAdjustOrTempHead> bizObjList = adjustOrTempHeadDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMAdjustOrTempHead bizObj = bizObjList.get(mIndex);
		...
		final List<WOMAdjustOrTempHead> nmeObjs = new ArrayList<WOMAdjustOrTempHead>();
		...
		//String idSql = "select id from " + WOMAdjustOrTempHead.TABLE_NAME + " order by id desc";
		//Query query = adjustOrTempHeadDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMAdjustOrTempHead.JPA_NAME + " order by id desc";
		Query query =  adjustOrTempHeadDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMAdjustOrTempHead.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMAdjustOrTempHead adjustOrTempHead:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMAdjustOrTempHead.TABLE_NAME,1,dbId);
			if(adjustOrTempHead.getId() != null ){
				m.put(adjustOrTempHead.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			adjustOrTempHead.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) adjustOrTempHeadDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_AdjustOrTempHead"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		adjustOrTempHeadDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMAdjustOrTempHead.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMAdjustOrTempHead adjustOrTempHead = updateObjs.get(updateIndex);
						Long id = adjustOrTempHead.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = adjustOrTempHead.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(adjustOrTempHead);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMAdjustOrTempHead.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMAdjustOrTempHead adjustOrTempHead = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = adjustOrTempHead.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		adjustOrTempHeadDaoImpl.getSession().flush();
		adjustOrTempHeadDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = adjustOrTempHeadDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_AdjustOrTempHead").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = adjustOrTempHeadDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_AdjustOrTempHead").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMAdjustOrTempHead bizObj = insertObjs.get(insertIndex);
			adjustOrTempHeadImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMAdjustOrTempHead bizObj = updateObjs.get(updateIndex);
			WOMAdjustOrTempHead oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			adjustOrTempHeadImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMAdjustOrTempHead> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMAdjustOrTempHead> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) adjustOrTempHeadDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMAdjustOrTempHead> result = getResult(page, query, new PendingResultTransformer(WOMAdjustOrTempHead.class,adjustOrTempHeadDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMAdjustOrTempHead model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = adjustOrTempHeadDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_AdjustOrTempHead").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = adjustOrTempHeadDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = adjustOrTempHeadDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = adjustOrTempHeadDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?adjustOrTempHeadDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  adjustOrTempHeadDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,adjustOrTempHeadDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="ec.flow.save")
	public void saveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, DataGridService dataGridService){
		saveAdjustOrTempHead(adjustOrTempHead, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="ec.flow.save")
	public void saveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, DataGridService dataGridService, String viewCode){
		saveAdjustOrTempHead(adjustOrTempHead, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead")
	public void saveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveAdjustOrTempHead(adjustOrTempHead, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead")
	public void saveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != adjustOrTempHead.getId() && adjustOrTempHead.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempHead.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead")
	public Map<Object, Object> adjustOrTempHeadDataGridImport(WOMAdjustOrTempHead adjustOrTempHead, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = adjustOrTempHead.getVirtualId();
		
		adjustOrTempHead = adjustOrTempHeadDao.load(adjustOrTempHead.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, adjustOrTempHead.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = adjustOrTempHead.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(adjustOrTempHead);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, adjustOrTempHead.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead")
	public void mergeAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(adjustOrTempHead);
		beforeSaveAdjustOrTempHead(adjustOrTempHead);
		adjustOrTempHeadDao.merge(adjustOrTempHead);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempHead",desc="foundation.batch.save",operType="4")
	public void batchSaveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
	...
	@Override
	public Page<WOMAdjustOrTempHead> getByPage(Page<WOMAdjustOrTempHead> page,DetachedCriteria detachedCriteria){
		return adjustOrTempHeadDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, Object... objects){
		checkUniqueConstraint(adjustOrTempHead);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportAdjustOrTempHead(List<WOMAdjustOrTempHead> adjustOrTempHeads, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportAdjustOrTempHead(List<WOMAdjustOrTempHead> adjustOrTempHeads){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportAdjustOrTempHead(List<WOMAdjustOrTempHead> adjustOrTempHeads, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportAdjustOrTempHead(List<WOMAdjustOrTempHead> adjustOrTempHeads){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMAdjustOrTempHead adjustOrTempHead, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMAdjustOrTempHead adjustOrTempHead, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportAdjustOrTempHead(List<WOMAdjustOrTempHead> insertObjs, List<WOMAdjustOrTempHead> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportAdjustOrTempHead,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportAdjustOrTempHead(List<WOMAdjustOrTempHead> insertObjs, List<WOMAdjustOrTempHead> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportAdjustOrTempHead,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
	@Resource
    protected SystemCodeService systemCodeService;

	@Override
    public Long getAdjustActivePage(Long activeRecordId) {
        Object result = procFeedbackHeadDao.createNativeQuery(" select id from wom_proc_feedback_heads  where ADJUST_DEAL_RECORD=? and valid = 1", activeRecordId).uniqueResult();
        if (result != null) {
	...
	@Override
    public String generatDealActive(WOMProduceTaskProcess taskProcess,String purpose,Long activeId) {
        String type = null;
        if (StringUtils.equals(purpose,"isForAdjust")) {
            type = "womActiveSource/03";
        }else {
            type = "womActiveSource/04";
        }
        if (((Number)adjustOrTempHeadDao.createNativeQuery("SELECT COUNT(ID) FROM WOM_ADJUST_OR_TEMP_HEADS WHERE PROCESS_ID=? AND VALID = 1 AND DATA_SOURCE=?",taskProcess.getId(),type).uniqueResult()).intValue() == 0) {
            List<RMFormulaProcessActive> formulaProcessActives = adjustOrTempHeadDao.findByHql(" from RMFormulaProcessActive where valid = 1 and "+ purpose +" = 1 and processId=?", taskProcess.getFormularProcess());
            for (RMFormulaProcessActive formulaProcessActive : formulaProcessActives) {
                WOMAdjustOrTempHead adjustOrTempHead = new WOMAdjustOrTempHead();
                if (activeId != null) {
                    adjustOrTempHead.setProcessActiveID(produceTaskActiveDao.get(activeId));
                }
                adjustOrTempHead.setTableInfoId(taskProcess.getHeadId().getTableInfoId());
                adjustOrTempHead.setFormulaProcessActive(formulaProcessActive);
                adjustOrTempHead.setActiveType(formulaProcessActive.getActiveType());
                adjustOrTempHead.setProcessId(taskProcess);
                adjustOrTempHead.setActiveName(formulaProcessActive.getName());
                adjustOrTempHead.setAdjustHeadID(taskProcess.getHeadId());
                adjustOrTempHead.setProduct(formulaProcessActive.getProduct());
                adjustOrTempHead.setQualityStandard(formulaProcessActive.getQualityStandard());
                if (StringUtils.equals("isForTemp",purpose)) {
                    adjustOrTempHead.setDataSource((SystemCode) systemCodeService.getSystemCode("womActiveSource/04"));
                }else {
                    adjustOrTempHead.setDataSource((SystemCode) systemCodeService.getSystemCode("womActiveSource/03"));
                }
                adjustOrTempHead.setIsAgile(formulaProcessActive.getIsAgile());
                adjustOrTempHeadDao.save(adjustOrTempHead);
            }
            adjustOrTempHeadDao.flush();
            adjustOrTempHeadDao.clear();
	...
	@Override
    public String toTempActive(Long processId) {
        WOMProduceTaskProcess taskProcess = produceTaskProcessDao.get(processId);
        generatDealActive(taskProcess,"isForTemp",null);
        return "SUCCESS";
    }
// add 2019年3月13日15:40:15 用于找工序中是否存在机动活动
	@Override
    public String vaildTempActive(Long processId) {
      	//根据工单工序找到配方工序活动类型为机动活动的数据
      	String sql = " select pa.id from RM_FORMULA_PROCESS_ACTIVES  pa  " +
				" join RM_FORMULA_PROCESSES fr on fr.id = pa.PROCESS_ID " +
				" join WOM_PRODUCE_TASK_PROCESSES tp on  tp.FORMULAR_PROCESS = fr.id " +
				" where tp.id = ? and pa.IS_FOR_TEMP = 1 and pa.valid = 1 and fr.valid = 1 ";
        List<Object> tempActiveId = produceTaskProcessDao.createNativeQuery(sql, processId).list();
		 String data = "";
		 for(Object activeId : tempActiveId){
		 	data += String.valueOf(activeId)+",";
		 }
        return data;
    }
/* CUSTOM CODE END */
