...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMProduceTaskActiveAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProduceTaskActive> {
	...
	private WOMProduceTaskActive produceTaskActive;
	...
	//导出excel时,主模型内容
	List<WOMProduceTaskActive> queryResults = new ArrayList<WOMProduceTaskActive>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/produceTaskActive/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/produceTaskActive/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_ProduceTaskActive" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProduceTaskActive> produceTaskActives;
	
	public List<WOMProduceTaskActive> getProduceTaskActives() {
		return produceTaskActives;
	}
	...
	private Page<WOMProduceTaskActive> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			produceTaskActive = new WOMProduceTaskActive();
			if(produceTaskActive.getActiveSource() != null) {
				produceTaskActive.setActiveSource((SystemCode) systemCodeService.getSystemCode(produceTaskActive.getActiveSource().getId()));
			}
			if(produceTaskActive.getActiveState() != null) {
				produceTaskActive.setActiveState((SystemCode) systemCodeService.getSystemCode(produceTaskActive.getActiveState().getId()));
			}
			if(produceTaskActive.getActiveType() != null) {
				produceTaskActive.setActiveType((SystemCode) systemCodeService.getSystemCode(produceTaskActive.getActiveType().getId()));
			}
			if(produceTaskActive.getExeSystem() != null) {
				produceTaskActive.setExeSystem((SystemCode) systemCodeService.getSystemCode(produceTaskActive.getExeSystem().getId()));
			}
			if(produceTaskActive.getProperty() != null) {
				produceTaskActive.setProperty((SystemCode) systemCodeService.getSystemCode(produceTaskActive.getProperty().getId()));
			}
		} else {
			if(id > 0){
				produceTaskActive = produceTaskActiveService.getProduceTaskActive(id, viewCode);
			}
		}
	...
	public Page<WOMProduceTaskActive> getPage() {
		if (null == page) {
			page = new Page<WOMProduceTaskActive>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProduceTaskActive getProduceTaskActive() {
		return produceTaskActive;
	}		

	public void setProduceTaskActive(WOMProduceTaskActive produceTaskActive) {
		this.produceTaskActive = produceTaskActive;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/produceTaskActive/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = produceTaskActiveService.getProduceTaskActiveAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/produceTaskActive/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1489671093316
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String produceTaskActiveReferenceCopy() throws Exception{
			WOMProduceTaskActive oldProduceTaskActive = null;
			if(isEditNew()){
				...
				oldProduceTaskActive = new WOMProduceTaskActive();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,produceTaskActive.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, produceTaskActive.getId());
				...
				oldProduceTaskActive = produceTaskActiveService.getProduceTaskActive(id);
			}
			
			
			produceTaskActive = produceTaskActiveService.getProduceTaskActive(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceTaskActive");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProduceTaskActive.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProduceTaskActive, null);
						Method setValue = WOMProduceTaskActive.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(produceTaskActive, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			produceTaskActive.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/produceTaskActive/avtiveEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "avtiveEdit.ftl") }),
		@Action(value = "/WOM/produceTask/produceTaskActive/avtiveEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "avtiveEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=produceTaskActiveService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return produceTaskActiveReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			produceTaskActive = produceTaskActiveService.getProduceTaskActive(this.produceTaskActive.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,produceTaskActive.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			produceTaskActiveService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = produceTaskActiveService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(produceTaskActiveService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(produceTaskActiveService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_ProduceTaskActive", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/produceTaskActive/taskActiveExelogRef", results = { @Result(type = FREEMARKER, location = "taskActiveExelogRef.ftl") })
		,
		@Action(value = "/WOM/produceTask/produceTaskActive/taskActiveRef", results = { @Result(type = FREEMARKER, location = "taskActiveRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"produceTaskActive\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"produceTaskActive\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"produceTaskActive\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"produceTaskActive\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			produceTaskActiveService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/produceTaskActive/taskActiveExelogRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headID.tableNo,result.orderProcessId.name,result.name,result.activeType.id,result.activeType.value,result.headID.factoryId.name,result.headID.productId.productName,result.headID.productId.productCode,result.isReplace,result.isMixQuality,result.minQuality,result.maxQuality,result.standardNum,result.planNum,result.headID.id,result.orderProcessId.id,result.headID.factoryId.id,result.headID.productId.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="taskActiveExelogRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/produceTask/produceTaskActive/taskActiveRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.orderProcessId.name,result.name,result.activeType.id,result.activeType.value,result.factoryId.name,result.productId.productCode,result.productId.productName,result.isReplace,result.isMixQuality,result.minQuality,result.maxQuality,result.standardNum,result.planNum,result.orderProcessId.id,result.factoryId.id,result.productId.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="taskActiveRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		produceTaskActiveService.findDataGridPage(dg,dgclass,getDgPage(),produceTaskActive,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(produceTaskActive.getId() != null && produceTaskActive.getId() != -1){
				produceTaskActive = produceTaskActiveService.getProduceTaskActive(produceTaskActive.getId());
			}
			produceTaskActiveService.findDataGridPage(dg,dgclass,getDgPage(),produceTaskActive,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == produceTaskActive.getId()) {
		...
		produceTaskActiveService.saveProduceTaskActive(produceTaskActive,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/produceTask/produceTaskActive/avtiveEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "submit");
		bap_validate_datagrids=produceTaskActiveService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", produceTaskActive.getId());
		...
		fileuploads.put("linkId", produceTaskActive.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/produceTaskActive/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		produceTaskActiveService.deleteProduceTaskActive(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceTaskActive/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "restore");
		produceTaskActiveService.restoreProduceTaskActive(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProduceTaskActive> findExportDataInfos(Page<WOMProduceTaskActive> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = produceTaskActiveService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = produceTaskActiveService.getAssProperty(p.getCode());	
									//String assModelCode = produceTaskActiveService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			produceTaskActiveService.findProduceTaskActives(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			produceTaskActiveService.findProduceTaskActives(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProduceTaskActive> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProduceTaskActive result : results){
	...
	public boolean isEditNew(){
		//return null == this.produceTaskActive || null == this.produceTaskActive.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/produceTaskActive/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/produceTaskActive/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/produceTaskActive/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceTaskActive/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProduceTaskActiveService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceTaskActive/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_ProduceTaskActive", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_ProduceTaskActive", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/produceTaskActive/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/produceTaskActive/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_ProduceTaskActive";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskActive", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/produceTaskActive/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
		_auditLog.setColumnStringA(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
		_auditLog.setColumnStringC(produceTaskActive.getId().toString());
		_auditLog.setColumnStringD(WOMProduceTaskActive.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProduceTaskActiveAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, produceTaskActive,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, produceTaskActive,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,produceTaskActive);
		...
		printlog.setBusinessId(produceTaskActive.getId());
		printlog.setBussinessKey(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
		printlog.setMainDisplay(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/produceTaskActive/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(produceTaskActive,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceTaskActive/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(produceTaskActiveService.getProduceTaskActive(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/produceTaskActive/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/produceTaskActive/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, produceTaskActive,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				produceTaskActive = produceTaskActiveService.getProduceTaskActive(Long.parseLong(arrId[i]));
				if(null != produceTaskActive){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == produceTaskActive.getId() ? "" : "," + produceTaskActive.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == produceTaskActive.getId() ? "" : "," + produceTaskActive.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == produceTaskActive.getId() ? "" : "," + produceTaskActive.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, produceTaskActive,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,produceTaskActive);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//produceTaskActiveService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/produceTaskActive/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceTaskActive");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_ProduceTaskActive")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_ProduceTaskActive");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceTaskActive");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = produceTaskActiveService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
    if ("WOM_7.5.0.0_produceTask_taskActiveExelogRef".equals(code)&& "taskActiveExelogRefQuery".equals(methodName)) { // 根据物品过滤数据
			if (orderProcessID != null && !"".equals(orderProcessID)&&headID != null && !"".equals(headID)){
				String customerSql ="(\"produceTaskActive\".HEADID in(select ID from WOM_PRODUCE_TASKS where ( ID = "+ headID + " and valid = 1)) and  \"produceTaskActive\".ORDER_PROCESS_ID in(select ID from WOM_PRODUCE_TASK_PROCESSES where ( ID = " + orderProcessID + " and valid = 1)))";
				params.add(new Param("customerSql", customerSql));
			}else if (headID != null && !"".equals(headID)) {//ACTIVE_ID    ProduceTaskActive
				String customerSql = "(\"produceTaskActive\".HEADID in(select ID from WOM_PRODUCE_TASKS where ( ID = "+ headID + " and valid = 1)))";
				params.add(new Param("customerSql", customerSql));
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
	private Long headID;//指令单ID
	private Long orderProcessID;//指令单工序
