...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMInspectAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMInspect> {
	...
	//导出excel时,主模型内容
	List<WOMInspect> queryResults = new ArrayList<WOMInspect>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/inspect/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/inspect/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_generalManage_Inspect" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMInspect> inspects;
	
	public List<WOMInspect> getInspects() {
		return inspects;
	}
	...
	public void prepare() throws Exception {
		if (null == id) {
			inspect = new WOMInspect();
		} else {
			if(id > 0){
				inspect = inspectService.getInspect(id, viewCode);
			}
		}
	...
	public Page<WOMInspect> getPage() {
		if (null == page) {
			page = new Page<WOMInspect>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMInspect getInspect() {
		return inspect;
	}		

	public void setInspect(WOMInspect inspect) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/inspect/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = inspectService.getInspectAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/inspect/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1502698759360
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String inspectReferenceCopy() throws Exception{
			WOMInspect oldInspect = null;
			if(isEditNew()){
				...
				oldInspect = new WOMInspect();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,inspect.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, inspect.getId());
				...
				oldInspect = inspectService.getInspect(id);
			}
			
			
			inspect = inspectService.getInspect(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_Inspect");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMInspect.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldInspect, null);
						Method setValue = WOMInspect.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(inspect, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			inspect.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=inspectService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return inspectReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			inspect = inspectService.getInspect(this.inspect.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,inspect.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = inspectService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(inspectService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(inspectService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_generalManage_Inspect", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"inspect\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"inspect\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"inspect\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"inspect\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		inspectService.findDataGridPage(dg,dgclass,getDgPage(),inspect,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(inspect.getId() != null && inspect.getId() != -1){
				inspect = inspectService.getInspect(inspect.getId());
			}
			inspectService.findDataGridPage(dg,dgclass,getDgPage(),inspect,searchCondition,params);
	...
	private void save() throws Exception {
		...
		inspectService.saveInspect(inspect,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "submit");
		bap_validate_datagrids=inspectService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", inspect.getId());
		...
		fileuploads.put("linkId", inspect.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/generalManage/inspect/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		inspectService.deleteInspect(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/inspect/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "restore");
		inspectService.restoreInspect(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMInspect> findExportDataInfos(Page<WOMInspect> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = inspectService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = inspectService.getAssProperty(p.getCode());	
									//String assModelCode = inspectService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			inspectService.findInspects(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			inspectService.findInspects(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMInspect> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMInspect result : results){
	...
	public boolean isEditNew(){
		//return null == this.inspect || null == this.inspect.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/generalManage/inspect/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/generalManage/inspect/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/generalManage/inspect/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/inspect/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMInspectService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/inspect/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_generalManage_Inspect", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_generalManage_Inspect", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/generalManage/inspect/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/generalManage/inspect/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_generalManage_Inspect";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_Inspect", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/generalManage/inspect/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == inspect.getId() ? "" : inspect.getId().toString());
		_auditLog.setColumnStringA(null == inspect.getId() ? "" : inspect.getId().toString());
		_auditLog.setColumnStringC(inspect.getId().toString());
		_auditLog.setColumnStringD(WOMInspect.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMInspectAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, inspect,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, inspect,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,inspect);
		...
		printlog.setBusinessId(inspect.getId());
		printlog.setBussinessKey(null == inspect.getId() ? "" : inspect.getId().toString());
		printlog.setMainDisplay(null == inspect.getId() ? "" : inspect.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/generalManage/inspect/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(inspect,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/inspect/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(inspectService.getInspect(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/generalManage/inspect/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/generalManage/inspect/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, inspect,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				inspect = inspectService.getInspect(Long.parseLong(arrId[i]));
				if(null != inspect){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == inspect.getId() ? "" : "," + inspect.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == inspect.getId() ? "" : inspect.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == inspect.getId() ? "" : "," + inspect.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == inspect.getId() ? "" : inspect.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == inspect.getId() ? "" : "," + inspect.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == inspect.getId() ? "" : inspect.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, inspect,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,inspect);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/generalManage/inspect/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_Inspect");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_generalManage_Inspect")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_generalManage_Inspect");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_Inspect");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = inspectService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
