...
import com.supcon.orchid.WOM.services.WOMBatchChargeDetailsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchChargeDetailsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_batchChargeDetailsService")
@Transactional
public class WOMBatchChargeDetailsServiceImpl  extends BaseServiceImpl<WOMBatchChargeDetails> implements WOMBatchChargeDetailsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMBatchChargeDetails> bapBaseService;
	...
	@Autowired
	private WOMBatchChargeDetailsImportService batchChargeDetailsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchChargeDetails.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchChargeDetailsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchChargeDetails.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchChargeDetailsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchChargeDetails.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchChargeDetails.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchChargeDetailsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchChargeDetails.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchChargeDetails.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchChargeDetails> list = (List<WOMBatchChargeDetails>) batchChargeDetailsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchChargeDetails getBatchChargeDetails(long id){
		return getBatchChargeDetails(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchChargeDetails getBatchChargeDetails(long id, String viewCode){
		WOMBatchChargeDetails batchChargeDetails = batchChargeDetailsDao.load(id);

		// 一对多情况处理
		if(batchChargeDetails != null){
		}
		return batchChargeDetails;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchChargeDetailsAsJSON(long id, String include){
		WOMBatchChargeDetails batchChargeDetails = batchChargeDetailsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchChargeDetails, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.common.delete",operType="3")
	public void deleteBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails){
		if(null != batchChargeDetails && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchChargeDetails.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
			AuditUtil.setColumnStringA(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
			AuditUtil.setColumnStringC(batchChargeDetails.getId().toString());
		}
		beforeDeleteBatchChargeDetails(batchChargeDetails);
		batchChargeDetailsDao.deletePhysical(batchChargeDetails);
		afterDeleteBatchChargeDetails(batchChargeDetails);
		batchChargeDetailsDao.flush();
		List<WOMBatchChargeDetails> params = new ArrayList<WOMBatchChargeDetails>();
		params.add(batchChargeDetails);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.common.delete",operType="3")
	public void deleteBatchChargeDetails(List<Long> batchChargeDetailsIds){
		deleteBatchChargeDetails(batchChargeDetailsIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.common.delete",operType="3")
	public void deleteBatchChargeDetails(List<Long> batchChargeDetailsIds, String eventTopic) {
		List<WOMBatchChargeDetails> batchChargeDetailss = new ArrayList<WOMBatchChargeDetails>();
		for(Long batchChargeDetailsId : batchChargeDetailsIds){
			WOMBatchChargeDetails batchChargeDetails = getBatchChargeDetails(batchChargeDetailsId);
			batchChargeDetailss.add(batchChargeDetails);
			if(null != batchChargeDetails && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchChargeDetails.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchChargeDetails.getId() ? "" : "," + batchChargeDetails.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchChargeDetails.getId() ? "" : "," + batchChargeDetails.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchChargeDetails.getId() ? "" : "," + batchChargeDetails.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
				}
			}
		}
		
		for(WOMBatchChargeDetails batchChargeDetails : batchChargeDetailss){
			beforeDeleteBatchChargeDetails(batchChargeDetails);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchChargeDetailss != null && batchChargeDetailss.size() > 0 ){
			for(WOMBatchChargeDetails batchChargeDetails : batchChargeDetailss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMBatchChargeDetails.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchChargeDetails));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchChargeDetails, batchChargeDetails.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchChargeDetailsIds != null && batchChargeDetailsIds.size() > 0) {
			String hql = "delete " + WOMBatchChargeDetails.JPA_NAME + " where id in(:ids)";
			Query query = batchChargeDetailsDao.createQuery(hql);
			query.setParameterList("ids", batchChargeDetailsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchChargeDetails batchChargeDetailsz : batchChargeDetailss){
				batchChargeDetailsz.setValid(false);
				List<WOMBatchChargeDetails> params = new ArrayList<WOMBatchChargeDetails>();
				params.add(batchChargeDetailsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.common.delete",operType="3")
	public void deleteBatchChargeDetails(long batchChargeDetailsId, int batchChargeDetailsVersion){
		this.deleteBatchChargeDetails(batchChargeDetailsId, batchChargeDetailsVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.common.delete",operType="3")
	public void deleteBatchChargeDetails(long batchChargeDetailsId, int batchChargeDetailsVersion,SignatureLog signatureLog){
		WOMBatchChargeDetails batchChargeDetails = getBatchChargeDetails(batchChargeDetailsId);
		if(batchChargeDetails != null && !batchChargeDetails.isValid()){
		...
		beforeDeleteBatchChargeDetails(batchChargeDetails);
		batchChargeDetailsDao.delete(batchChargeDetailsId, batchChargeDetailsVersion);
		afterDeleteBatchChargeDetails(batchChargeDetails);
		batchChargeDetailsDao.flush();
		List<WOMBatchChargeDetails> params = new ArrayList<WOMBatchChargeDetails>();
		params.add(batchChargeDetails);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchChargeDetails.getId();
			signatureLog.setTableId(batchChargeDetails.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.common.delete",operType="3")
	public void deleteBatchChargeDetails(String batchChargeDetailsIds){
		this.deleteBatchChargeDetails(batchChargeDetailsIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.common.delete",operType="3")
	public void deleteBatchChargeDetails(String batchChargeDetailsIds,SignatureLog signatureLog){
		deleteCollection(batchChargeDetailsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchChargeDetails(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_BatchChargeDetails#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="ec.engine.restore",operType="10")
	public void restoreBatchChargeDetails(String batchChargeDetailsIds){
		restoreCollection(batchChargeDetailsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchChargeDetails(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="ec.engine.restore",operType="10")
	public void restoreBatchChargeDetails(long batchChargeDetailsId){
		
		findBusinessKeyUsed(batchChargeDetailsId);	//判断业务主键是否重复
		...
		beforeRestoreBatchChargeDetails(batchChargeDetails);
		batchChargeDetails.setValid(true);
		batchChargeDetailsDao.update(batchChargeDetails);
		afterRestoreBatchChargeDetails(batchChargeDetails);
		List<WOMBatchChargeDetails> params = new ArrayList<WOMBatchChargeDetails>();
		params.add(batchChargeDetails);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchChargeDetailsId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_BatchChargeDetails");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchChargeDetails.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchChargeDetails.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchChargeDetailsDao.createNativeQuery(sql,batchChargeDetailsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchChargeDetails(List<WOMBatchChargeDetails>  batchChargeDetailss){
		for(WOMBatchChargeDetails batchChargeDetails:batchChargeDetailss)  {
			saveBatchChargeDetails(batchChargeDetails, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchChargeDetails(List<WOMBatchChargeDetails>  batchChargeDetailss){
		for(WOMBatchChargeDetails batchChargeDetails:batchChargeDetailss)  {
			saveBatchChargeDetails(batchChargeDetails, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="ec.flow.save")
	public void saveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchChargeDetails(batchChargeDetails, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="ec.flow.save")
	public void saveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchChargeDetails(batchChargeDetails, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchChargeDetailsCodes(WOMBatchChargeDetails batchChargeDetails) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchChargeDetailsCodes(batchChargeDetails, false);
	}
	
	public void generateBatchChargeDetailsCodes(WOMBatchChargeDetails batchChargeDetails, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchChargeDetailsSummarys(WOMBatchChargeDetails batchChargeDetails) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchChargeDetailsSummarys(batchChargeDetails, false);
	}

	public void generateBatchChargeDetailsSummarys(WOMBatchChargeDetails batchChargeDetails, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails")
	public void saveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchChargeDetails(batchChargeDetails, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails")
	public void saveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(batchChargeDetails.getId() != null && batchChargeDetails.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchChargeDetails.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchChargeDetails);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchChargeDetails);
		beforeSaveBatchChargeDetails(batchChargeDetails, viewIsView);

		if (viewIsView) {
			batchChargeDetailsDao.saveWithRevertVersion(batchChargeDetails);
		} else {
			if(isNew)
				batchChargeDetailsDao.save(batchChargeDetails);
			else
				batchChargeDetailsDao.merge(batchChargeDetails);
		}
		if(null != batchChargeDetails && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchChargeDetails.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
			AuditUtil.setColumnStringA(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
		...
		dealDatagridsSave(batchChargeDetails,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchChargeDetailsCodes(batchChargeDetails, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchChargeDetailsSummarys(batchChargeDetails, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchChargeDetails(batchChargeDetails, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchChargeDetails);
			}
		}
		List<WOMBatchChargeDetails> params = new ArrayList<WOMBatchChargeDetails>();
		batchChargeDetailsDao.flush();
		batchChargeDetailsDao.clear();
		batchChargeDetails = batchChargeDetailsDao.load(batchChargeDetails.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchChargeDetails batchChargeDetails){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchChargeDetails", batchChargeDetails);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails")
	public void mergeBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchChargeDetails);
		beforeSaveBatchChargeDetails(batchChargeDetails);
		batchChargeDetailsDao.merge(batchChargeDetails);
		if(null != batchChargeDetails && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchChargeDetails.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
			AuditUtil.setColumnStringA(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
		...
		dealDatagridsSave(batchChargeDetails,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchChargeDetails(batchChargeDetails);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchChargeDetails> findBatchChargeDetailss(Page<WOMBatchChargeDetails> page, Criterion... criterions) {
		return batchChargeDetailsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchChargeDetailss(Page<WOMBatchChargeDetails> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchChargeDetailss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchChargeDetailss(Page<WOMBatchChargeDetails> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchChargeDetailss(Page<WOMBatchChargeDetails> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchChargeDetails> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchChargeDetails\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchChargeDetails\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchChargeDetails\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"batchChargeDetails\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  batchChargeDetailsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchChargeDetailsDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchChargeDetails\".LAY_REC ASC, \"batchChargeDetails\".SORT ASC");
				} else {
					orderPart.append(" \"batchChargeDetails\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchChargeDetailsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchChargeDetailsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchChargeDetailsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchChargeDetails> result = new ArrayList<WOMBatchChargeDetails>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchChargeDetails.class,batchChargeDetailsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchChargeDetails.class,batchChargeDetailsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchChargeDetails batchChargeDetails : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchChargeDetails.getId(), "WOM_produceTask_batchChargeDetails");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchChargeDetails.getId(), "WOM_produceTask_batchChargeDetails");
							if(null != documents && !documents.isEmpty()) {
								batchChargeDetails.setDocument(documents.get(0));
								batchChargeDetails.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchChargeDetails> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchChargeDetails> result) {
		View view = null;
		...
		Map<Long, WOMBatchChargeDetails> rsMap = new HashMap<Long, WOMBatchChargeDetails>();
		for (WOMBatchChargeDetails batchChargeDetails : result) {
			ids.add(batchChargeDetails.getId());
			rsMap.put(batchChargeDetails.getId(), batchChargeDetails);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchChargeDetailsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchChargeDetailsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_BatchChargeDetails" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_BatchChargeDetails" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchChargeDetailsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchChargeDetails\"","WOM_7.5.0.0_produceTask","","WOM_BATCH_CHARGE_DETAILS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchChargeDetails\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchChargeDetails\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchChargeDetails\"","WOM_7.5.0.0_produceTask","","WOM_BATCH_CHARGE_DETAILS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_BatchChargeDetails" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchChargeDetails.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchChargeDetails\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchChargeDetails\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchChargeDetails\".CID = 1 OR \"batchChargeDetails\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchChargeDetails.mc\"").append(".BATCH_CHARGE_DETAILS) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchChargeDetailsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchChargeDetailsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMBatchChargeDetails.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchChargeDetails.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchChargeDetails.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchChargeDetails 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchChargeDetails batchChargeDetails){
	}
	
	
	@Override
	public List<WOMBatchChargeDetails> getBatchChargeDetailss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchChargeDetails.class);
		if(params!=null){
			...
			return batchChargeDetailsDao.findByHql("from " + WOMBatchChargeDetails.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchChargeDetailsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMBatchChargeDetails> findBatchChargeDetailssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchChargeDetails.class);
	...
	@Override
	public List<WOMBatchChargeDetails> findBatchChargeDetailssByHql(String hql, Object... objects){
		
		return   batchChargeDetailsDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchChargeDetails loadBatchChargeDetailsByBussinessKey(WOMBatchChargeDetails batchChargeDetails){
		return loadBatchChargeDetailsByBussinessKey(batchChargeDetails.getId());
	}
	
	@Override
	public Page<WOMBatchChargeDetails> getBatchChargeDetailss(Page<WOMBatchChargeDetails> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchChargeDetails.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchChargeDetailsDao.findByPage(page, sql);
			}else{
				page = batchChargeDetailsDao.findAllByPage(page);
			}
			return page;
		}
		page = batchChargeDetailsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteBatchChargeDetailsByProdTaskActiExelog(List<Long> prodTaskActiExelogIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("taskActiExelogList", prodTaskActiExelogIds);
		String hql = "select id from WOMBatchChargeDetails where "
						+ "taskActiExelog.id in (:taskActiExelogList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> batchChargeDetailsIds1 = batchChargeDetailsDao.findByHql(hql, params);
		if(batchChargeDetailsIds1 != null && !batchChargeDetailsIds1.isEmpty()) {
		}
		hql = "delete WOMBatchChargeDetails where "
						+ "taskActiExelog.id in (:taskActiExelogList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		batchChargeDetailsDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchChargeDetails loadBatchChargeDetailsByBussinessKey(Serializable bussinessKey){
		return batchChargeDetailsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchChargeDetails> findByProperty(String propertyName, Object object){
		return batchChargeDetailsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchChargeDetails findEntityByProperty(String propertyName, Object object){
		return batchChargeDetailsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchChargeDetails.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchChargeDetailsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_BatchChargeDetails","runtime");
		try {
	...
	public void dealDatagridsSave(WOMBatchChargeDetails batchChargeDetails,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchChargeDetailsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchChargeDetails.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchChargeDetails);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(batchChargeDetails.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(batchChargeDetails.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(batchChargeDetails.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchChargeDetails.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchChargeDetails.TABLE_NAME +" where valid = 1";
			list = batchChargeDetailsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchChargeDetails.TABLE_NAME;
			list = batchChargeDetailsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchChargeDetails.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchChargeDetailsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchChargeDetailsDaoImpl batchChargeDetailsDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMBatchChargeDetails.TABLE_NAME + " where id = ?";
		String tableNo = batchChargeDetailsDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchBatchChargeDetails(final List<WOMBatchChargeDetails> insertObjs, final List<WOMBatchChargeDetails> updateObjs, 
		...
		return this.importBatchBatchChargeDetails(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchChargeDetails(final List<WOMBatchChargeDetails> insertObjs, final List<WOMBatchChargeDetails> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1543547557562"),importCount));
		...
		Map<Long, WOMBatchChargeDetails> oldUpdateObjsMap = new HashMap<Long, WOMBatchChargeDetails>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchChargeDetails bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchChargeDetails> bizObjList = batchChargeDetailsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchChargeDetails bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchChargeDetails> nmeObjs = new ArrayList<WOMBatchChargeDetails>();
		...
		//String idSql = "select id from " + WOMBatchChargeDetails.TABLE_NAME + " order by id desc";
		//Query query = batchChargeDetailsDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMBatchChargeDetails.JPA_NAME + " order by id desc";
		Query query =  batchChargeDetailsDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMBatchChargeDetails.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchChargeDetails batchChargeDetails:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchChargeDetails.TABLE_NAME,1,dbId);
			if(batchChargeDetails.getId() != null ){
				m.put(batchChargeDetails.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			batchChargeDetails.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchChargeDetailsDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_BatchChargeDetails"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchChargeDetailsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchChargeDetails.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMBatchChargeDetails batchChargeDetails = updateObjs.get(updateIndex);
						Long id = batchChargeDetails.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchChargeDetails.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchChargeDetails);
							...
							nmeObjs.add(batchChargeDetails);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchChargeDetails.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMBatchChargeDetails batchChargeDetails = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchChargeDetails.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchChargeDetailsDaoImpl.getSession().flush();
		batchChargeDetailsDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = batchChargeDetailsDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_BatchChargeDetails").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = batchChargeDetailsDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_BatchChargeDetails").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchChargeDetails bizObj = insertObjs.get(insertIndex);
			batchChargeDetailsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchChargeDetails bizObj = updateObjs.get(updateIndex);
			WOMBatchChargeDetails oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchChargeDetailsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMBatchChargeDetails> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMBatchChargeDetails> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchChargeDetailsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMBatchChargeDetails> result = getResult(page, query, new PendingResultTransformer(WOMBatchChargeDetails.class,batchChargeDetailsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMBatchChargeDetails model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchChargeDetailsDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_BatchChargeDetails").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchChargeDetailsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchChargeDetailsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchChargeDetailsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchChargeDetailsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchChargeDetailsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchChargeDetailsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="ec.flow.save")
	public void saveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, DataGridService dataGridService){
		saveBatchChargeDetails(batchChargeDetails, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="ec.flow.save")
	public void saveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, DataGridService dataGridService, String viewCode){
		saveBatchChargeDetails(batchChargeDetails, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails")
	public void saveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchChargeDetails(batchChargeDetails, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails")
	public void saveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != batchChargeDetails.getId() && batchChargeDetails.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchChargeDetails.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails")
	public Map<Object, Object> batchChargeDetailsDataGridImport(WOMBatchChargeDetails batchChargeDetails, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchChargeDetails.getVirtualId();
		
		batchChargeDetails = batchChargeDetailsDao.load(batchChargeDetails.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchChargeDetails.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchChargeDetails.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchChargeDetails);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchChargeDetails.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails")
	public void mergeBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchChargeDetails);
		beforeSaveBatchChargeDetails(batchChargeDetails);
		batchChargeDetailsDao.merge(batchChargeDetails);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchChargeDetails",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
	...
	@Override
	public Page<WOMBatchChargeDetails> getByPage(Page<WOMBatchChargeDetails> page,DetachedCriteria detachedCriteria){
		return batchChargeDetailsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, Object... objects){
		checkUniqueConstraint(batchChargeDetails);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchChargeDetails(List<WOMBatchChargeDetails> batchChargeDetailss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchChargeDetails(List<WOMBatchChargeDetails> batchChargeDetailss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchChargeDetails(List<WOMBatchChargeDetails> batchChargeDetailss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchChargeDetails(List<WOMBatchChargeDetails> batchChargeDetailss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchChargeDetails batchChargeDetails, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchChargeDetails batchChargeDetails, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchChargeDetails(List<WOMBatchChargeDetails> insertObjs, List<WOMBatchChargeDetails> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchChargeDetails,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchChargeDetails(List<WOMBatchChargeDetails> insertObjs, List<WOMBatchChargeDetails> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchChargeDetails,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
