...
import com.supcon.orchid.WOM.services.WOMMaterialsOutputImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMMaterialsOutputDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_materialsOutputService")
@Transactional
public class WOMMaterialsOutputServiceImpl  extends BaseServiceImpl<WOMMaterialsOutput> implements WOMMaterialsOutputService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMMaterialsOutput> bapBaseService;
	...
	@Autowired
	private WOMMaterialsOutputImportService materialsOutputImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMMaterialsOutput.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  materialsOutputDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMMaterialsOutput.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = materialsOutputDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMMaterialsOutput.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMMaterialsOutput.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  materialsOutputDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMMaterialsOutput.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMMaterialsOutput.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMMaterialsOutput> list = (List<WOMMaterialsOutput>) materialsOutputDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMaterialsOutput getMaterialsOutput(long id){
		return getMaterialsOutput(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMaterialsOutput getMaterialsOutput(long id, String viewCode){
		WOMMaterialsOutput materialsOutput = materialsOutputDao.load(id);

		// 一对多情况处理
		if(materialsOutput != null){
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMaterialsOutputAsJSON(long id, String include){
		WOMMaterialsOutput materialsOutput = materialsOutputDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(materialsOutput, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.common.delete",operType="3")
	public void deleteMaterialsOutput(WOMMaterialsOutput materialsOutput){
		if(null != materialsOutput && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMaterialsOutput.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
			AuditUtil.setColumnStringA(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
			AuditUtil.setColumnStringC(materialsOutput.getId().toString());
		}
		beforeDeleteMaterialsOutput(materialsOutput);
		materialsOutputDao.delete(materialsOutput);
		afterDeleteMaterialsOutput(materialsOutput);
		materialsOutputDao.flush();
		List<WOMMaterialsOutput> params = new ArrayList<WOMMaterialsOutput>();
		params.add(materialsOutput);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceRecords");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.common.delete",operType="3")
	public void deleteMaterialsOutput(List<Long> materialsOutputIds){
		deleteMaterialsOutput(materialsOutputIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.common.delete",operType="3")
	public void deleteMaterialsOutput(List<Long> materialsOutputIds, String eventTopic) {
		List<WOMMaterialsOutput> materialsOutputs = new ArrayList<WOMMaterialsOutput>();
		for(Long materialsOutputId : materialsOutputIds){
			WOMMaterialsOutput materialsOutput = getMaterialsOutput(materialsOutputId);
			materialsOutputs.add(materialsOutput);
			if(null != materialsOutput && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMaterialsOutput.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == materialsOutput.getId() ? "" : "," + materialsOutput.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == materialsOutput.getId() ? "" : "," + materialsOutput.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == materialsOutput.getId() ? "" : "," + materialsOutput.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(materialsOutputs != null && materialsOutputs.size() > 0 ){
			for(WOMMaterialsOutput materialsOutput : materialsOutputs){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),materialsOutput, materialsOutput.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(materialsOutputIds != null && materialsOutputIds.size() > 0) {
			String hql = "update " + WOMMaterialsOutput.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = materialsOutputDao.createQuery(hql);
			query.setParameterList("ids", materialsOutputIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMMaterialsOutput materialsOutputz : materialsOutputs){
				materialsOutputz.setValid(false);
				List<WOMMaterialsOutput> params = new ArrayList<WOMMaterialsOutput>();
				params.add(materialsOutputz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceRecords");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.common.delete",operType="3")
	public void deleteMaterialsOutput(long materialsOutputId, int materialsOutputVersion){
		this.deleteMaterialsOutput(materialsOutputId, materialsOutputVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.common.delete",operType="3")
	public void deleteMaterialsOutput(long materialsOutputId, int materialsOutputVersion,SignatureLog signatureLog){
		WOMMaterialsOutput materialsOutput = getMaterialsOutput(materialsOutputId);
		if(materialsOutput != null && !materialsOutput.isValid()){
		...
		beforeDeleteMaterialsOutput(materialsOutput);
		materialsOutputDao.delete(materialsOutputId, materialsOutputVersion);
		afterDeleteMaterialsOutput(materialsOutput);
		materialsOutputDao.flush();
		List<WOMMaterialsOutput> params = new ArrayList<WOMMaterialsOutput>();
		params.add(materialsOutput);
		...
		if(signatureLog != null ) {
			...
			businessKey=materialsOutput.getId();
			signatureLog.setTableId(materialsOutput.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.common.delete",operType="3")
	public void deleteMaterialsOutput(String materialsOutputIds){
		this.deleteMaterialsOutput(materialsOutputIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.common.delete",operType="3")
	public void deleteMaterialsOutput(String materialsOutputIds,SignatureLog signatureLog){
		deleteCollection(materialsOutputIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMaterialsOutput(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceRecords#modelCode:WOM_7.5.0.0_produceRecords_MaterialsOutput#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="ec.engine.restore",operType="10")
	public void restoreMaterialsOutput(String materialsOutputIds){
		restoreCollection(materialsOutputIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreMaterialsOutput(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="ec.engine.restore",operType="10")
	public void restoreMaterialsOutput(long materialsOutputId){
		
		findBusinessKeyUsed(materialsOutputId);	//判断业务主键是否重复
		...
		beforeRestoreMaterialsOutput(materialsOutput);
		materialsOutput.setValid(true);
		materialsOutputDao.update(materialsOutput);
		afterRestoreMaterialsOutput(materialsOutput);
		List<WOMMaterialsOutput> params = new ArrayList<WOMMaterialsOutput>();
		params.add(materialsOutput);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long materialsOutputId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceRecords_MaterialsOutput");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMMaterialsOutput.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMMaterialsOutput.TABLE_NAME +" where id = ? )";
			List<Object> list =  materialsOutputDao.createNativeQuery(sql,materialsOutputId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseMaterialsOutput(List<WOMMaterialsOutput>  materialsOutputs){
		for(WOMMaterialsOutput materialsOutput:materialsOutputs)  {
			saveMaterialsOutput(materialsOutput, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMaterialsOutput(List<WOMMaterialsOutput>  materialsOutputs){
		for(WOMMaterialsOutput materialsOutput:materialsOutputs)  {
			saveMaterialsOutput(materialsOutput, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="ec.flow.save")
	public void saveMaterialsOutput(WOMMaterialsOutput materialsOutput, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMaterialsOutput(materialsOutput, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="ec.flow.save")
	public void saveMaterialsOutput(WOMMaterialsOutput materialsOutput, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMaterialsOutput(materialsOutput, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateMaterialsOutputCodes(WOMMaterialsOutput materialsOutput) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMaterialsOutputCodes(materialsOutput, false);
	}
	
	public void generateMaterialsOutputCodes(WOMMaterialsOutput materialsOutput, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateMaterialsOutputSummarys(WOMMaterialsOutput materialsOutput) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMaterialsOutputSummarys(materialsOutput, false);
	}

	public void generateMaterialsOutputSummarys(WOMMaterialsOutput materialsOutput, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput")
	public void saveMaterialsOutput(WOMMaterialsOutput materialsOutput,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveMaterialsOutput(materialsOutput, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput")
	public void saveMaterialsOutput(WOMMaterialsOutput materialsOutput,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceRecords");
		String url = null;
		if(materialsOutput.getId() != null && materialsOutput.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMaterialsOutput.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_produceRecords/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMaterialsOutput.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_produceRecords/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, materialsOutput);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(materialsOutput);
		beforeSaveMaterialsOutput(materialsOutput, viewIsView);

		if (viewIsView) {
			materialsOutputDao.saveWithRevertVersion(materialsOutput);
		} else {
			if(isNew)
				materialsOutputDao.save(materialsOutput);
			else
				materialsOutputDao.merge(materialsOutput);
		}
		if(null != materialsOutput && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMaterialsOutput.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
			AuditUtil.setColumnStringA(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
		...
		dealDatagridsSave(materialsOutput,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateMaterialsOutputCodes(materialsOutput, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMaterialsOutputSummarys(materialsOutput, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMaterialsOutput(materialsOutput, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, materialsOutput);
			}
		}
		List<WOMMaterialsOutput> params = new ArrayList<WOMMaterialsOutput>();
		materialsOutputDao.flush();
		materialsOutputDao.clear();
		materialsOutput = materialsOutputDao.load(materialsOutput.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMMaterialsOutput materialsOutput){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("materialsOutput", materialsOutput);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput")
	public void mergeMaterialsOutput(WOMMaterialsOutput materialsOutput, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(materialsOutput);
		beforeSaveMaterialsOutput(materialsOutput);
		materialsOutputDao.merge(materialsOutput);
		if(null != materialsOutput && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMaterialsOutput.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
			AuditUtil.setColumnStringA(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
		...
		dealDatagridsSave(materialsOutput,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveMaterialsOutput(materialsOutput);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMMaterialsOutput> findMaterialsOutputs(Page<WOMMaterialsOutput> page, Criterion... criterions) {
		return materialsOutputDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMMaterialsOutputDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMMaterialsOutputDealInfo> list=materialsOutputDao.createQuery("from " + WOMMaterialsOutputDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMMaterialsOutputDealInfo dealInfo) {
		materialsOutputDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMMaterialsOutputDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) materialsOutputDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMMaterialsOutputDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = materialsOutputDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = materialsOutputDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMMaterialsOutputDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = materialsOutputDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMaterialsOutputs(Page<WOMMaterialsOutput> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMaterialsOutputs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMaterialsOutputs(Page<WOMMaterialsOutput> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMaterialsOutputs(Page<WOMMaterialsOutput> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMMaterialsOutput> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"materialsOutput\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"materialsOutput\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"materialsOutput\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceRecords_MaterialsOutput" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_produceRecords_MaterialsOutput" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  materialsOutputDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  materialsOutputDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceRecords").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"materialsOutput\"","WOM_7.5.0.0_produceRecords",customCondition,"WOM_MATERIALS_OUTPUTS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"materialsOutput\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMMaterialsOutputDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"materialsOutput\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"materialsOutput\".LAY_REC ASC, \"materialsOutput\".SORT ASC");
				} else {
					orderPart.append(" \"materialsOutput\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) materialsOutputDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=materialsOutputDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = materialsOutputDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMMaterialsOutput> result = new ArrayList<WOMMaterialsOutput>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMMaterialsOutput.class,materialsOutputDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMMaterialsOutput.class,materialsOutputDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMMaterialsOutput materialsOutput : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(materialsOutput.getId(), "WOM_produceRecords_materialsOutput");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(materialsOutput.getId(), "WOM_produceRecords_materialsOutput");
							if(null != documents && !documents.isEmpty()) {
								materialsOutput.setDocument(documents.get(0));
								materialsOutput.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_produceRecords")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_produceRecords")+"WOM_7.5.0.0_produceRecords".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_produceRecords",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMMaterialsOutput> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMMaterialsOutput> result) {
		View view = null;
		...
		Map<Long, WOMMaterialsOutput> rsMap = new HashMap<Long, WOMMaterialsOutput>();
		for (WOMMaterialsOutput materialsOutput : result) {
			ids.add(materialsOutput.getId());
			rsMap.put(materialsOutput.getId(), materialsOutput);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(materialsOutputDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = materialsOutputDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMMaterialsOutput materialsOutput,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(materialsOutput.getOwnerStaffId());
		ti.setOwnerPositionId(materialsOutput.getOwnerPositionId());
		ti.setOwnerDepartmentId(materialsOutput.getOwnerDepartmentId());
		
		ti.setCreateStaff(materialsOutput.getCreateStaff());
		ti.setCreateStaffId(materialsOutput.getCreateStaffId());
		ti.setCreateTime(materialsOutput.getCreateTime());
		ti.setDeleteStaff(materialsOutput.getDeleteStaff());
		ti.setDeleteStaffId(materialsOutput.getDeleteStaffId());
		ti.setDeleteTime(materialsOutput.getDeleteTime());
		ti.setCreateDepartmentId(materialsOutput.getCreateDepartmentId());
		ti.setModifyStaff(materialsOutput.getModifyStaff());
		ti.setModifyStaffId(materialsOutput.getModifyStaffId());
		ti.setCreatePositionId(materialsOutput.getCreatePositionId());
		ti.setPositionLayRec(materialsOutput.getPositionLayRec());
		ti.setTableNo(materialsOutput.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_produceRecords");
		ti.setTargetTableName("WOM_MATERIALS_OUTPUTS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_produceRecords_MaterialsOutput" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  materialsOutputDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"materialsOutput\"","WOM_7.5.0.0_produceRecords","","WOM_MATERIALS_OUTPUTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"materialsOutput\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"materialsOutput\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"materialsOutput\"","WOM_7.5.0.0_produceRecords","","WOM_MATERIALS_OUTPUTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("materialsOutput.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"materialsOutput\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"materialsOutput\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"materialsOutput\".CID = 1 OR \"materialsOutput\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("materialsOutput.mc\"").append(".MATERIALS_OUTPUT) not in ("+notinValue+")");
			}
			
			SQLQuery query = materialsOutputDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("produceRecords", "WOM_7.5.0.0_produceRecords", "produceRecords_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=materialsOutputDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMMaterialsOutput.ENTITY_CODE;}
	protected String getModuleCode(){return WOMMaterialsOutput.MODULE_CODE;}
	protected String getModelCode(){return WOMMaterialsOutput.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param materialsOutput 模型实例
	 */
	public void checkUniqueConstraint(WOMMaterialsOutput materialsOutput){
	}
	
	
	@Override
	public List<WOMMaterialsOutput> getMaterialsOutputs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMaterialsOutput.class);
		if(params!=null){
			...
			return materialsOutputDao.findByHql("from " + WOMMaterialsOutput.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return materialsOutputDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMMaterialsOutput> findMaterialsOutputsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMaterialsOutput.class);
	...
	@Override
	public List<WOMMaterialsOutput> findMaterialsOutputsByHql(String hql, Object... objects){
		
		return   materialsOutputDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMMaterialsOutput loadMaterialsOutputByBussinessKey(WOMMaterialsOutput materialsOutput){
		return loadMaterialsOutputByBussinessKey(materialsOutput.getId());
	}
	
	@Override
	public Page<WOMMaterialsOutput> getMaterialsOutputs(Page<WOMMaterialsOutput> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMaterialsOutput.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = materialsOutputDao.findByPage(page, sql);
			}else{
				page = materialsOutputDao.findAllByPage(page);
			}
			return page;
		}
		page = materialsOutputDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMaterialsOutput loadMaterialsOutputByBussinessKey(Serializable bussinessKey){
		return materialsOutputDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMMaterialsOutput> findByProperty(String propertyName, Object object){
		return materialsOutputDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMaterialsOutput findEntityByProperty(String propertyName, Object object){
		return materialsOutputDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMMaterialsOutput.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = materialsOutputDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceRecords_MaterialsOutput","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_produceRecords", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMMaterialsOutput materialsOutput,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,materialsOutputDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMMaterialsOutput.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, materialsOutput);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",materialsOutput.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMMaterialsOutput.TABLE_NAME +" where valid = 1";
			list = materialsOutputDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMMaterialsOutput.TABLE_NAME;
			list = materialsOutputDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMMaterialsOutput.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = materialsOutputDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMMaterialsOutputDaoImpl materialsOutputDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchMaterialsOutput(final List<WOMMaterialsOutput> insertObjs, final List<WOMMaterialsOutput> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchMaterialsOutput(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchMaterialsOutput(final List<WOMMaterialsOutput> insertObjs, final List<WOMMaterialsOutput> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1541146599059"),importCount));
		...
		Map<Long, WOMMaterialsOutput> oldUpdateObjsMap = new HashMap<Long, WOMMaterialsOutput>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMMaterialsOutput bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMMaterialsOutput> bizObjList = materialsOutputDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMMaterialsOutput bizObj = bizObjList.get(mIndex);
		...
		final List<WOMMaterialsOutput> nmeObjs = new ArrayList<WOMMaterialsOutput>();
		...
		long id=autoGeneratorID.getNextId(WOMMaterialsOutput.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMMaterialsOutput materialsOutput:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMMaterialsOutput.TABLE_NAME,1,dbId);
			if(materialsOutput.getId() != null ){
				m.put(materialsOutput.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			materialsOutput.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) materialsOutputDao.createNativeQuery(sql, "WOM_7.5.0.0_produceRecords_MaterialsOutput"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		materialsOutputDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMMaterialsOutput.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMMaterialsOutput materialsOutput = updateObjs.get(updateIndex);
						Long id = materialsOutput.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = materialsOutput.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(materialsOutput);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMMaterialsOutput.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMMaterialsOutput.TABLE_NAME + " order by id desc";
//					Query query = materialsOutputDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMMaterialsOutput materialsOutput = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = materialsOutput.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		materialsOutputDaoImpl.getSession().flush();
		materialsOutputDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMMaterialsOutput bizObj = insertObjs.get(insertIndex);
			materialsOutputImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMMaterialsOutput bizObj = updateObjs.get(updateIndex);
			WOMMaterialsOutput oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			materialsOutputImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = materialsOutputDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				materialsOutputDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = materialsOutputDao.createNativeQuery(sql, "WOM_7.5.0.0_produceRecords_MaterialsOutput").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = materialsOutputDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = materialsOutputDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = materialsOutputDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?materialsOutputDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  materialsOutputDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,materialsOutputDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="ec.flow.save")
	public void saveMaterialsOutput(WOMMaterialsOutput materialsOutput, DataGridService dataGridService){
		saveMaterialsOutput(materialsOutput, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="ec.flow.save")
	public void saveMaterialsOutput(WOMMaterialsOutput materialsOutput, DataGridService dataGridService, String viewCode){
		saveMaterialsOutput(materialsOutput, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput")
	public void saveMaterialsOutput(WOMMaterialsOutput materialsOutput, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveMaterialsOutput(materialsOutput, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput")
	public void saveMaterialsOutput(WOMMaterialsOutput materialsOutput, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceRecords");
		String url = null;
		if(null != materialsOutput.getId() && materialsOutput.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMMaterialsOutput.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput")
	public Map<Object, Object> materialsOutputDataGridImport(WOMMaterialsOutput materialsOutput, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = materialsOutput.getVirtualId();
		
		materialsOutput = materialsOutputDao.load(materialsOutput.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, materialsOutput.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = materialsOutput.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(materialsOutput);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, materialsOutput.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput")
	public void mergeMaterialsOutput(WOMMaterialsOutput materialsOutput, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(materialsOutput);
		beforeSaveMaterialsOutput(materialsOutput);
		materialsOutputDao.merge(materialsOutput);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceRecords",model="WOM_7.5.0.0_produceRecords_MaterialsOutput",desc="foundation.batch.save",operType="4")
	public void batchSaveMaterialsOutput(WOMMaterialsOutput materialsOutput, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceRecords");
	...
	@Override
	public Page<WOMMaterialsOutput> getByPage(Page<WOMMaterialsOutput> page,DetachedCriteria detachedCriteria){
		return materialsOutputDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_produceRecords").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveMaterialsOutput(WOMMaterialsOutput materialsOutput, Object... objects){
		checkUniqueConstraint(materialsOutput);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveMaterialsOutput(WOMMaterialsOutput materialsOutput, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportMaterialsOutput(List<WOMMaterialsOutput> materialsOutputs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportMaterialsOutput(List<WOMMaterialsOutput> materialsOutputs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportMaterialsOutput(List<WOMMaterialsOutput> materialsOutputs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportMaterialsOutput(List<WOMMaterialsOutput> materialsOutputs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMaterialsOutput(WOMMaterialsOutput materialsOutput){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteMaterialsOutput(WOMMaterialsOutput materialsOutput){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreMaterialsOutput(WOMMaterialsOutput materialsOutput){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreMaterialsOutput(WOMMaterialsOutput materialsOutput){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMMaterialsOutput materialsOutput, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMMaterialsOutput materialsOutput, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportMaterialsOutput(List<WOMMaterialsOutput> insertObjs, List<WOMMaterialsOutput> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMaterialsOutput,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportMaterialsOutput(List<WOMMaterialsOutput> insertObjs, List<WOMMaterialsOutput> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMaterialsOutput,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
