...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
...
public class WOMStandingcropRefAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMStandingcropRef> {
	...
	//导出excel时,主模型内容
	List<WOMStandingcropRef> queryResults = new ArrayList<WOMStandingcropRef>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/standingcropRef/standingcropRef/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/standingcropRef/standingcropRef/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_standingcropRef_StandingcropRef" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMStandingcropRef> standingcropRefs;
	
	public List<WOMStandingcropRef> getStandingcropRefs() {
		return standingcropRefs;
	}
	...
	private Page<WOMStandingcropRef> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			standingcropRef = new WOMStandingcropRef();
		} else {
			if(id > 0){
				standingcropRef = standingcropRefService.getStandingcropRef(id, viewCode);
			}
		}
	...
	public Page<WOMStandingcropRef> getPage() {
		if (null == page) {
			page = new Page<WOMStandingcropRef>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/standingcropRef/standingcropRef/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = standingcropRefService.getStandingcropRefAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/standingcropRef/standingcropRef/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String standingcropRefReferenceCopy() throws Exception{
			WOMStandingcropRef oldStandingcropRef = null;
			if(isEditNew()){
				...
				oldStandingcropRef = new WOMStandingcropRef();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,standingcropRef.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, standingcropRef.getId());
				...
				oldStandingcropRef = standingcropRefService.getStandingcropRef(id);
			}
			
			
			standingcropRef = standingcropRefService.getStandingcropRef(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_standingcropRef_StandingcropRef");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMStandingcropRef.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldStandingcropRef, null);
						Method setValue = WOMStandingcropRef.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(standingcropRef, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			standingcropRef.setCreateDepartmentId(oldStandingcropRef.getCreateDepartmentId());
			standingcropRef.setCreatePositionId(oldStandingcropRef.getCreatePositionId());
			standingcropRef.setCreateStaffId(oldStandingcropRef.getCreateStaffId());
			standingcropRef.setOwnerDepartmentId(oldStandingcropRef.getOwnerDepartmentId());
			standingcropRef.setOwnerPositionId(oldStandingcropRef.getOwnerPositionId());
			standingcropRef.setOwnerStaffId(oldStandingcropRef.getOwnerStaffId());
			standingcropRef.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=standingcropRefService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return standingcropRefReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			standingcropRef = standingcropRefService.getStandingcropRef(this.standingcropRef.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,standingcropRef.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			standingcropRefService.print(0);
		}
		return SUCCESS;
	}
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/WOM/standingcropRef/standingcropRef/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = standingcropRefService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(standingcropRefService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(standingcropRefService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_standingcropRef_StandingcropRef", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumAllRef", results = { @Result(type = FREEMARKER, location = "batchNumAllRef.ftl") })
		,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumOneBatRef", results = { @Result(type = FREEMARKER, location = "batchNumOneBatRef.ftl") })
		,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumOneMakeTask", results = { @Result(type = FREEMARKER, location = "batchNumOneMakeTask.ftl") })
		,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumOneRef", results = { @Result(type = FREEMARKER, location = "batchNumOneRef.ftl") })
		,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumOneRefExp", results = { @Result(type = FREEMARKER, location = "batchNumOneRefExp.ftl") })
		,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumPutInMaRef", results = { @Result(type = FREEMARKER, location = "batchNumPutInMaRef.ftl") })
		,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumRef", results = { @Result(type = FREEMARKER, location = "batchNumRef.ftl") })
		,
		@Action(value = "/WOM/standingcropRef/standingcropRef/manulOrderRef", results = { @Result(type = FREEMARKER, location = "manulOrderRef.ftl") })
		,
		@Action(value = "/WOM/standingcropRef/standingcropRef/prepareNumRef", results = { @Result(type = FREEMARKER, location = "prepareNumRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"standingcropRef\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"standingcropRef\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("storeSet.layRec") && request.getParameter("storeSet.layRec").trim().length() > 0) {
			params.add(new Param("\"placeSet\".LAY_REC", request.getParameter("storeSet.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"standingcropRef\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"standingcropRef\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			standingcropRefService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumAllRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchText,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.wareID.cargoStatue.id,result.wareID.cargoStatue.value,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.onhand,result.frozenQuantity,result.availiQuantity,result.good.id,result.wareID.id,result.placeSet.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchNumAllRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumOneBatRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchText,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.wareID.cargoStatue.id,result.wareID.cargoStatue.value,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.onhand,result.frozenQuantity,result.availiQuantity,result.good.id,result.wareID.id,result.placeSet.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchNumOneBatRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumOneMakeTask-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchText,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.onhand,result.frozenQuantity,result.availiQuantity,result.good.id,result.wareID.id,result.placeSet.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchNumOneMakeTask-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumOneRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchText,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.wareID.cargoStatue.id,result.wareID.cargoStatue.value,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.onhand,result.frozenQuantity,result.availiQuantity,result.good.id,result.wareID.id,result.placeSet.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchNumOneRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumOneRefExp-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchText,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.onhand,result.frozenQuantity,result.availiQuantity,result.good.id,result.wareID.id,result.placeSet.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchNumOneRefExp-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumPutInMaRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchText,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.wareID.cargoStatue.id,result.wareID.cargoStatue.value,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.onhand,result.frozenQuantity,result.availiQuantity,result.good.id,result.wareID.id,result.placeSet.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchNumPutInMaRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/standingcropRef/standingcropRef/batchNumRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchText,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.onhand,result.frozenQuantity,result.availiQuantity,result.good.id,result.wareID.id,result.placeSet.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchNumRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/standingcropRef/standingcropRef/manulOrderRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.good.productCode,result.batchText,result.wareID.wareCode,result.availiQuantity,result.good.id,result.wareID.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="manulOrderRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/standingcropRef/standingcropRef/prepareNumRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.good.productCode,result.good.productName,result.good.productBaseUnit.name,result.manulOrder.manulOrderNum,result.createTime,result.onhand,result.batchText,result.wareID.wareCode,result.wareID.wareName,result.createStaff.name,result.good.id,result.good.productBaseUnit.id,result.manulOrder.id,result.wareID.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="prepareNumRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		standingcropRefService.findDataGridPage(dg,dgclass,getDgPage(),standingcropRef,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(standingcropRef.getId() != null && standingcropRef.getId() != -1){
				standingcropRef = standingcropRefService.getStandingcropRef(standingcropRef.getId());
			}
			standingcropRefService.findDataGridPage(dg,dgclass,getDgPage(),standingcropRef,searchCondition,params);
	...
	private void save() throws Exception {
		...
		standingcropRefService.saveStandingcropRef(standingcropRef,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "submit");
		bap_validate_datagrids=standingcropRefService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", standingcropRef.getId());
		...
		fileuploads.put("linkId", standingcropRef.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/standingcropRef/standingcropRef/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		standingcropRefService.deleteStandingcropRef(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/standingcropRef/standingcropRef/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "restore");
		standingcropRefService.restoreStandingcropRef(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/standingcropRef/standingcropRef/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/standingcropRef/standingcropRef/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "showDealInfoData");
		if (null != tableInfoId) {
			int count=standingcropRefService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = standingcropRefService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = standingcropRefService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMStandingcropRef> findExportDataInfos(Page<WOMStandingcropRef> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = standingcropRefService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = standingcropRefService.getAssProperty(p.getCode());	
									//String assModelCode = standingcropRefService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			standingcropRefService.findStandingcropRefs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			standingcropRefService.findStandingcropRefs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMStandingcropRef> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMStandingcropRef result : results){
	...
	public boolean isEditNew(){
		//return null == this.standingcropRef || null == this.standingcropRef.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/standingcropRef/standingcropRef/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/standingcropRef/standingcropRef/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/standingcropRef/standingcropRef/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/standingcropRef/standingcropRef/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMStandingcropRefService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_standingcropRef", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_standingcropRef");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/standingcropRef/standingcropRef/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_standingcropRef_StandingcropRef", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_standingcropRef_StandingcropRef", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/standingcropRef/standingcropRef/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/standingcropRef/standingcropRef/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_standingcropRef_StandingcropRef";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_standingcropRef_StandingcropRef", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/standingcropRef/standingcropRef/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == standingcropRef.getId() ? "" : standingcropRef.getId().toString());
		_auditLog.setColumnStringA(null == standingcropRef.getBatchText() ? "" : standingcropRef.getBatchText().toString());
		_auditLog.setColumnStringC(standingcropRef.getId().toString());
		_auditLog.setColumnStringD(WOMStandingcropRef.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_standingcropRef");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMStandingcropRefAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, standingcropRef,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, standingcropRef,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,standingcropRef);
		...
		printlog.setEntity("WOM_7.5.0.0_standingcropRef");
		...
		printlog.setBusinessId(standingcropRef.getId());
		printlog.setBussinessKey(null == standingcropRef.getId() ? "" : standingcropRef.getId().toString());
		printlog.setMainDisplay(null == standingcropRef.getBatchText() ? "" : standingcropRef.getBatchText().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/standingcropRef/standingcropRef/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(standingcropRef,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/standingcropRef/standingcropRef/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(standingcropRefService.getStandingcropRef(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/standingcropRef/standingcropRef/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/standingcropRef/standingcropRef/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, standingcropRef,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				standingcropRef = standingcropRefService.getStandingcropRef(Long.parseLong(arrId[i]));
				if(null != standingcropRef){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == standingcropRef.getBatchText() ? "" : "," + standingcropRef.getBatchText().toString()));
					} else {
						_auditLog.setColumnStringA(null == standingcropRef.getBatchText() ? "" : standingcropRef.getBatchText().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == standingcropRef.getId() ? "" : "," + standingcropRef.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == standingcropRef.getId() ? "" : standingcropRef.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == standingcropRef.getId() ? "" : "," + standingcropRef.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == standingcropRef.getId() ? "" : standingcropRef.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, standingcropRef,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,standingcropRef);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//standingcropRefService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/standingcropRef/standingcropRef/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMStandingcropRef> findExportPageInfos(Page<WOMStandingcropRef> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_standingcropRef" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = standingcropRefService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMStandingcropRef queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = standingcropRefService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = standingcropRefService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_standingcropRef_StandingcropRef");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_standingcropRef_StandingcropRef")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_standingcropRef_StandingcropRef");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_standingcropRef_StandingcropRef");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = standingcropRefService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
		if("WOM_7.5.0.0_standingcropRef_batchNumAllRef".equals(code)&&"batchNumAllRefQuery".equals(methodName)){ //质量标准参照查询时
          	//如果存在物品，只能选择该物品的批号
			if(goodID!=null&&!"".equals(goodID)){
				//如果启用批号，只能选用该计划单下的批号
	          	if(consulService.getValueAsBoolean("platform/bap/WOM/WOM.proBatch") && produceTaskId!=null && !"".equals(produceTaskId)){
					String customerSql = "( \"standingcropRef\".ONHAND > (case when \"standingcropRef\".FROZEN_QUANTITY is null then 0 else \"standingcropRef\".FROZEN_QUANTITY end ) and \"standingcropRef\".ID in(select BATCH_NUM from WOM_MATERIAL_BATCH_NUMS where ( ACTIVE_ID in(select ID from WOM_PRODUCE_TASK_ACTIVES where ( HEADID in(select ID from WOM_PRODUCE_TASKS where ( ID = "+produceTaskId+" and valid = 1)) and valid = 1)) and valid = 1)) and \"standingcropRef\".GOOD in(select PRODUCT_ID from S2BASE_PRODUCT where ( PRODUCT_ID = "+goodID+" and valid = 1)))";
					params.add(new Param("customerSql", customerSql));
				}else{
					String customerSql = "( \"standingcropRef\".ONHAND > (case when \"standingcropRef\".FROZEN_QUANTITY is null then 0 else \"standingcropRef\".FROZEN_QUANTITY end ) and \"standingcropRef\".GOOD in(select PRODUCT_ID from S2BASE_PRODUCT where ( PRODUCT_ID = "+goodID+" and valid = 1)))";
					params.add(new Param("customerSql", customerSql));
				}
			}else{
				//如果启用批号，只能选用该计划单下的批号
	          	if(consulService.getValueAsBoolean("platform/bap/WOM/WOM.proBatch") && produceTaskId!=null && !"".equals(produceTaskId)){
					String customerSql = "( \"standingcropRef\".ONHAND > (case when \"standingcropRef\".FROZEN_QUANTITY is null then 0 else \"standingcropRef\".FROZEN_QUANTITY end ) and \"standingcropRef\".ID in(select BATCH_NUM from WOM_MATERIAL_BATCH_NUMS where ( ACTIVE_ID in(select ID from WOM_PRODUCE_TASK_ACTIVES where ( HEADID in(select ID from WOM_PRODUCE_TASKS where ( ID = "+produceTaskId+" and valid = 1)) and valid = 1)) and valid = 1)))";
					params.add(new Param("customerSql", customerSql));
				}else{
					//TODO
				}
      	...
      	if("WOM_7.5.0.0_standingcropRef_batchNumOneRef".equals(code)&&"batchNumOneRefQuery".equals(methodName)){ //质量标准参照查询时
          	//如果存在物品，只能选择该物品的批号
			if(goodID!=null&&!"".equals(goodID)){
				//如果启用批号，只能选用该计划单下的批号
	          	//if(consulService.getValueAsBoolean("platform/bap/WOM/WOM.proBatch") && produceTaskId!=null && !"".equals(produceTaskId)){
				...
				//	params.add(new Param("customerSql", customerSql));
				//}else{
					String customerSql = "( \"standingcropRef\".ONHAND > (case when \"standingcropRef\".FROZEN_QUANTITY is null then 0 else \"standingcropRef\".FROZEN_QUANTITY end ) and \"standingcropRef\".GOOD in(select PRODUCT_ID from S2BASE_PRODUCT where ( PRODUCT_ID = "+goodID+" and valid = 1))  and  \"standingcropRef\".BATCH_TEXT is not null )";
					params.add(new Param("customerSql", customerSql));
				//}
			}else{
				//如果启用批号，只能选用该计划单下的批号
	          	if(consulService.getValueAsBoolean("platform/bap/WOM/WOM.proBatch") && produceTaskId!=null && !"".equals(produceTaskId)){
					//String customerSql = "( \"standingcropRef\".ONHAND > (case when \"standingcropRef\".FROZEN_QUANTITY is null then 0 else \"standingcropRef\".FROZEN_QUANTITY end ) and \"standingcropRef\".ID in(select BATCH_NUM from WOM_MATERIAL_BATCH_NUMS where ( ACTIVE_ID in(select ID from WOM_PRODUCE_TASK_ACTIVES where ( HEADID in(select ID from WOM_PRODUCE_TASKS where ( ID = "+produceTaskId+" and valid = 1)) and valid = 1)) and valid = 1)))";
					//params.add(new Param("customerSql", customerSql));
					String customerSql = "( \"standingcropRef\".ONHAND > (case when \"standingcropRef\".FROZEN_QUANTITY is null then 0 else \"standingcropRef\".FROZEN_QUANTITY end ))";
					params.add(new Param("customerSql", customerSql));
				}else{
					//TODO
				}
        ...
        //投入异常实际物料批号过滤
        if("WOM_7.5.0.0_standingcropRef_batchNumOneRefExp".equals(code)&&"batchNumOneRefExpQuery".equals(methodName)){
            if(goodID!=null&&!"".equals(goodID)){
                 //如果启用MES物料管理， 根据物品ID对批号进行过滤  投料异常编辑使用
                if(consulService.getValueAsBoolean("platform/bap/WOM/WOM.mesProBatch")){
                    String customerSql="( \"standingcropRef\".GOOD in(select PRODUCT_ID from WOM_PUT_IN_MATERIAL_PARTS where ( PRODUCT_ID = "+goodID+" and valid = 1)))";
                    params.add(new Param("customerSql", customerSql));
                }
            }
        }
        ...
        //产品过滤批号
        if("WOM_7.5.0.0_standingcropRef_batchNumOneBatRef".equals(code)&&"batchNumOneBatRefQuery".equals(methodName)){
          if(goodID!=null&&!"".equals(goodID)){
            //如果启用MES物料管理， 根据物品ID对批号进行过滤 配料指令单使用
            if(consulService.getValueAsBoolean("platform/bap/WOM/WOM.mesProBatch")){
              String customerSql="( \"standingcropRef\".GOOD in(select PRODUCT_ID from S2BASE_PRODUCT where ( PRODUCT_ID = "+goodID+" and valid = 1)))";
              params.add(new Param("customerSql", customerSql));
            }
          }
        }
        ...
        //制造指令单-产品过滤批号
        if("WOM_7.5.0.0_standingcropRef_batchNumOneMakeTask".equals(code)&&"batchNumOneMakeTaskQuery".equals(methodName)){
          if(goodID!=null&&!"".equals(goodID)){
            //指令单设置物料批号
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
	@Resource
	 ...
	 * @return
	 */
	@Action(value = "/WOM/standingcropRef/standingcropRef/getStockInfo", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.good.productCode,result.batchText,result.wareID.wareCode,result.availiQuantity,result.good.id,result.wareID.id,result.attrMap.*"})
	})
	public String getStockInfo() throws Exception{
		List<WOMStandingcropRef> result = womMesToSapService.getStockInfo(materialCode, wareCode, batchNum);
		Page<WOMStandingcropRef> refPage = getPage();
		refPage.setResult(result);
		refPage.setPageNo(1);
		refPage.setPageSize(65535);
		refPage.setTotalCount(result.size());
		return SUCCESS;
	}
/* CUSTOM CODE END */
}