...
import com.supcon.orchid.WOM.services.WOMBatchingMaterialPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchingMaterialPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
@Service("wOM_batchingMaterialPartService")
@Transactional
public class WOMBatchingMaterialPartServiceImpl  extends BaseServiceImpl<WOMBatchingMaterialPart> implements WOMBatchingMaterialPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMBatchingMaterialPart> bapBaseService;
	...
	@Autowired
	private WOMBatchingMaterialPartImportService batchingMaterialPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchingMaterialPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchingMaterialPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchingMaterialPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchingMaterialPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchingMaterialPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchingMaterialPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchingMaterialPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchingMaterialPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchingMaterialPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchingMaterialPart> list = (List<WOMBatchingMaterialPart>) batchingMaterialPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMBatchingMaterialPart getBatchingMaterialPart(long id){
		return getBatchingMaterialPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingMaterialPart getBatchingMaterialPart(long id, String viewCode){
		WOMBatchingMaterialPart batchingMaterialPart = batchingMaterialPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchingMaterialPartAsJSON(long id, String include){
		WOMBatchingMaterialPart batchingMaterialPart = batchingMaterialPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchingMaterialPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart){
		if(null != batchingMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
			AuditUtil.setColumnStringA(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
			AuditUtil.setColumnStringC(batchingMaterialPart.getId().toString());
		}
		beforeDeleteBatchingMaterialPart(batchingMaterialPart);
		batchingMaterialPartDao.deletePhysical(batchingMaterialPart);
		afterDeleteBatchingMaterialPart(batchingMaterialPart);
		batchingMaterialPartDao.flush();
		List<WOMBatchingMaterialPart> params = new ArrayList<WOMBatchingMaterialPart>();
		params.add(batchingMaterialPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialPart(List<Long> batchingMaterialPartIds){
		deleteBatchingMaterialPart(batchingMaterialPartIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialPart(List<Long> batchingMaterialPartIds, String eventTopic) {
		batchingRejectPartsService.deleteBatchingRejectPartsByBatchingMaterialPart(batchingMaterialPartIds);
		List<WOMBatchingMaterialPart> batchingMaterialParts = new ArrayList<WOMBatchingMaterialPart>();
		for(Long batchingMaterialPartId : batchingMaterialPartIds){
			WOMBatchingMaterialPart batchingMaterialPart = getBatchingMaterialPart(batchingMaterialPartId);
			batchingMaterialParts.add(batchingMaterialPart);
			if(null != batchingMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchingMaterialPart.getId() ? "" : "," + batchingMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchingMaterialPart.getId() ? "" : "," + batchingMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchingMaterialPart.getId() ? "" : "," + batchingMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchingMaterialParts != null && batchingMaterialParts.size() > 0 ){
			for(WOMBatchingMaterialPart batchingMaterialPart : batchingMaterialParts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMBatchingMaterialPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchingMaterialPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchingMaterialPart, batchingMaterialPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchingMaterialPartIds != null && batchingMaterialPartIds.size() > 0) {
			String hql = "delete " + WOMBatchingMaterialPart.JPA_NAME + " where id in(:ids)";
			Query query = batchingMaterialPartDao.createQuery(hql);
			query.setParameterList("ids", batchingMaterialPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchingMaterialPart batchingMaterialPartz : batchingMaterialParts){
				batchingMaterialPartz.setValid(false);
				List<WOMBatchingMaterialPart> params = new ArrayList<WOMBatchingMaterialPart>();
				params.add(batchingMaterialPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_batchingMaterial");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialPart(long batchingMaterialPartId, int batchingMaterialPartVersion){
		this.deleteBatchingMaterialPart(batchingMaterialPartId, batchingMaterialPartVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialPart(long batchingMaterialPartId, int batchingMaterialPartVersion,SignatureLog signatureLog){
		WOMBatchingMaterialPart batchingMaterialPart = getBatchingMaterialPart(batchingMaterialPartId);
		...
		beforeDeleteBatchingMaterialPart(batchingMaterialPart);
		batchingMaterialPartDao.delete(batchingMaterialPartId, batchingMaterialPartVersion);
		afterDeleteBatchingMaterialPart(batchingMaterialPart);
		batchingMaterialPartDao.flush();
		List<WOMBatchingMaterialPart> params = new ArrayList<WOMBatchingMaterialPart>();
		params.add(batchingMaterialPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchingMaterialPart.getId();
			signatureLog.setTableId(batchingMaterialPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialPart(String batchingMaterialPartIds){
		this.deleteBatchingMaterialPart(batchingMaterialPartIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialPart(String batchingMaterialPartIds,SignatureLog signatureLog){
		deleteCollection(batchingMaterialPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchingMaterialPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_batchingMaterial#modelCode:WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="ec.engine.restore",operType="10")
	public void restoreBatchingMaterialPart(String batchingMaterialPartIds){
		restoreCollection(batchingMaterialPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchingMaterialPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="ec.engine.restore",operType="10")
	public void restoreBatchingMaterialPart(long batchingMaterialPartId){
		
		findBusinessKeyUsed(batchingMaterialPartId);	//判断业务主键是否重复
		...
		beforeRestoreBatchingMaterialPart(batchingMaterialPart);
		batchingMaterialPart.setValid(true);
		batchingMaterialPartDao.update(batchingMaterialPart);
		afterRestoreBatchingMaterialPart(batchingMaterialPart);
		List<WOMBatchingMaterialPart> params = new ArrayList<WOMBatchingMaterialPart>();
		params.add(batchingMaterialPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchingMaterialPartId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchingMaterialPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchingMaterialPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchingMaterialPartDao.createNativeQuery(sql,batchingMaterialPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchingMaterialPart(List<WOMBatchingMaterialPart>  batchingMaterialParts){
		for(WOMBatchingMaterialPart batchingMaterialPart:batchingMaterialParts)  {
			saveBatchingMaterialPart(batchingMaterialPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchingMaterialPart(List<WOMBatchingMaterialPart>  batchingMaterialParts){
		for(WOMBatchingMaterialPart batchingMaterialPart:batchingMaterialParts)  {
			saveBatchingMaterialPart(batchingMaterialPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="ec.flow.save")
	public void saveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchingMaterialPart(batchingMaterialPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="ec.flow.save")
	public void saveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchingMaterialPart(batchingMaterialPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchingMaterialPartCodes(WOMBatchingMaterialPart batchingMaterialPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchingMaterialPartCodes(batchingMaterialPart, false);
	}
	
	public void generateBatchingMaterialPartCodes(WOMBatchingMaterialPart batchingMaterialPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchingMaterialPartSummarys(WOMBatchingMaterialPart batchingMaterialPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchingMaterialPartSummarys(batchingMaterialPart, false);
	}

	public void generateBatchingMaterialPartSummarys(WOMBatchingMaterialPart batchingMaterialPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart")
	public void saveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchingMaterialPart(batchingMaterialPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart")
	public void saveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterial");
		String url = null;
		if(batchingMaterialPart.getId() != null && batchingMaterialPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchingMaterialPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchingMaterialPart);
		beforeSaveBatchingMaterialPart(batchingMaterialPart, viewIsView);

		if (viewIsView) {
			batchingMaterialPartDao.saveWithRevertVersion(batchingMaterialPart);
		} else {
			if(isNew)
				batchingMaterialPartDao.save(batchingMaterialPart);
			else
				batchingMaterialPartDao.merge(batchingMaterialPart);
		}
		if(null != batchingMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
			AuditUtil.setColumnStringA(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
		...
		dealDatagridsSave(batchingMaterialPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchingMaterialPartCodes(batchingMaterialPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchingMaterialPartSummarys(batchingMaterialPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchingMaterialPart(batchingMaterialPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchingMaterialPart);
			}
		}
		List<WOMBatchingMaterialPart> params = new ArrayList<WOMBatchingMaterialPart>();
		batchingMaterialPartDao.flush();
		batchingMaterialPartDao.clear();
		batchingMaterialPart = batchingMaterialPartDao.load(batchingMaterialPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchingMaterialPart batchingMaterialPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchingMaterialPart", batchingMaterialPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart")
	public void mergeBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchingMaterialPart);
		beforeSaveBatchingMaterialPart(batchingMaterialPart);
		batchingMaterialPartDao.merge(batchingMaterialPart);
		if(null != batchingMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
			AuditUtil.setColumnStringA(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
		...
		dealDatagridsSave(batchingMaterialPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchingMaterialPart(batchingMaterialPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchingMaterialPart> findBatchingMaterialParts(Page<WOMBatchingMaterialPart> page, Criterion... criterions) {
		return batchingMaterialPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingMaterialParts(Page<WOMBatchingMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchingMaterialParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingMaterialParts(Page<WOMBatchingMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingMaterialParts(Page<WOMBatchingMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchingMaterialPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchingMaterialPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchingMaterialPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchingMaterialPart\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"batchingMaterialPart\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_BATCHING_MATERIALS \"batchingMaterial\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_batchingMaterial" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  batchingMaterialPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchingMaterialPartDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_batchingMaterial").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = dataPermissionService.getPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchingMaterial\"","WOM_7.5.0.0_batchingMaterial","","WOM_BATCHING_MATERIALS",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchingMaterialPart\".LAY_REC ASC, \"batchingMaterialPart\".SORT ASC");
				} else {
					orderPart.append(" \"batchingMaterialPart\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchingMaterialPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchingMaterialPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchingMaterialPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchingMaterialPart> result = new ArrayList<WOMBatchingMaterialPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchingMaterialPart.class,batchingMaterialPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchingMaterialPart.class,batchingMaterialPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchingMaterialPart batchingMaterialPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchingMaterialPart.getId(), "WOM_batchingMaterial_batchingMaterialPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchingMaterialPart.getId(), "WOM_batchingMaterial_batchingMaterialPart");
							if(null != documents && !documents.isEmpty()) {
								batchingMaterialPart.setDocument(documents.get(0));
								batchingMaterialPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchingMaterialPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchingMaterialPart> result) {
		View view = null;
		...
		Map<Long, WOMBatchingMaterialPart> rsMap = new HashMap<Long, WOMBatchingMaterialPart>();
		for (WOMBatchingMaterialPart batchingMaterialPart : result) {
			ids.add(batchingMaterialPart.getId());
			rsMap.put(batchingMaterialPart.getId(), batchingMaterialPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchingMaterialPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchingMaterialPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchingMaterialPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchingMaterialPart\"","WOM_7.5.0.0_batchingMaterial","","WOM_BATCHING_MATERIAL_PARTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchingMaterialPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchingMaterialPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchingMaterialPart\"","WOM_7.5.0.0_batchingMaterial","","WOM_BATCHING_MATERIAL_PARTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchingMaterialPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchingMaterialPart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchingMaterialPart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchingMaterialPart\".CID = 1 OR \"batchingMaterialPart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchingMaterialPart.mc\"").append(".BATCHING_MATERIAL_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchingMaterialPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchingMaterialPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMBatchingMaterialPart.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchingMaterialPart.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchingMaterialPart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchingMaterialPart 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchingMaterialPart batchingMaterialPart){
	}
	
	
	@Override
	public List<WOMBatchingMaterialPart> getBatchingMaterialParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchingMaterialPart.class);
		if(params!=null){
			...
			return batchingMaterialPartDao.findByHql("from " + WOMBatchingMaterialPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchingMaterialPartDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMBatchingMaterialPart> findBatchingMaterialPartsByHql(String hql, Object... objects){
		...
		return   batchingMaterialPartDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchingMaterialPart loadBatchingMaterialPartByBussinessKey(WOMBatchingMaterialPart batchingMaterialPart){
		return loadBatchingMaterialPartByBussinessKey(batchingMaterialPart.getId());
	}
	
	@Override
	public Page<WOMBatchingMaterialPart> getBatchingMaterialParts(Page<WOMBatchingMaterialPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchingMaterialPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchingMaterialPartDao.findByPage(page, sql);
			}else{
				page = batchingMaterialPartDao.findAllByPage(page);
			}
			return page;
		}
		page = batchingMaterialPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchingMaterialPart> findByTreeDataGridBatchingMaterial(List<WOMBatchingMaterialPart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMBatchingMaterialPart batchingMaterialPart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridBatchingMaterial(Page<WOMBatchingMaterialPart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMBatchingMaterialPart> batchingMaterialParts = dgPage.getResult();
			for(WOMBatchingMaterialPart batchingMaterialPart : batchingMaterialParts){
				
			}
			dgPage.setResult(batchingMaterialParts);
		}
	...
	public WOMBatchingMaterialPart loadBatchingMaterialPartByBussinessKey(Serializable bussinessKey){
		return batchingMaterialPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchingMaterialPart> findByProperty(String propertyName, Object object){
		return batchingMaterialPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingMaterialPart findEntityByProperty(String propertyName, Object object){
		return batchingMaterialPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchingMaterialPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchingMaterialPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart","runtime");
		try {
	...
	public void dealDatagridsSave(WOMBatchingMaterialPart batchingMaterialPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchingMaterialPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchingMaterialPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchingMaterialPart);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(batchingMaterialPart.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(batchingMaterialPart.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(batchingMaterialPart.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchingMaterialPart.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchingMaterialPart.TABLE_NAME +" where valid = 1";
			list = batchingMaterialPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchingMaterialPart.TABLE_NAME;
			list = batchingMaterialPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchingMaterialPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchingMaterialPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchingMaterialPartDaoImpl batchingMaterialPartDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMBatchingMaterialPart.TABLE_NAME + " where id = ?";
		String tableNo = batchingMaterialPartDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchBatchingMaterialPart(final List<WOMBatchingMaterialPart> insertObjs, final List<WOMBatchingMaterialPart> updateObjs, 
		...
		return this.importBatchBatchingMaterialPart(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchingMaterialPart(final List<WOMBatchingMaterialPart> insertObjs, final List<WOMBatchingMaterialPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1491789803097"),importCount));
		...
		Map<Long, WOMBatchingMaterialPart> oldUpdateObjsMap = new HashMap<Long, WOMBatchingMaterialPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchingMaterialPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchingMaterialPart> bizObjList = batchingMaterialPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchingMaterialPart bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchingMaterialPart> nmeObjs = new ArrayList<WOMBatchingMaterialPart>();
		...
		//String idSql = "select id from " + WOMBatchingMaterialPart.TABLE_NAME + " order by id desc";
		//Query query = batchingMaterialPartDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMBatchingMaterialPart.JPA_NAME + " order by id desc";
		Query query =  batchingMaterialPartDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMBatchingMaterialPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchingMaterialPart batchingMaterialPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchingMaterialPart.TABLE_NAME,1,dbId);
			if(batchingMaterialPart.getId() != null ){
				m.put(batchingMaterialPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchingMaterialPartDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchingMaterialPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchingMaterialPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMBatchingMaterialPart batchingMaterialPart = updateObjs.get(updateIndex);
						Long id = batchingMaterialPart.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchingMaterialPart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchingMaterialPart);
							...
							nmeObjs.add(batchingMaterialPart);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchingMaterialPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMBatchingMaterialPart batchingMaterialPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchingMaterialPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchingMaterialPartDaoImpl.getSession().flush();
		batchingMaterialPartDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = batchingMaterialPartDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = batchingMaterialPartDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchingMaterialPart bizObj = insertObjs.get(insertIndex);
			batchingMaterialPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchingMaterialPart bizObj = updateObjs.get(updateIndex);
			WOMBatchingMaterialPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchingMaterialPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMBatchingMaterialPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMBatchingMaterialPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(linkMap != null && linkMap.get("needChange")!=null && linkMap.get("needChange").equals("no!!")){
				...
				s.append(" FROM WOM_BATCHING_MATERIALS \"batchingMaterial\" ");
			}else{
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchingMaterialPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMBatchingMaterialPart> result = getResult(page, query, new PendingResultTransformer(WOMBatchingMaterialPart.class,batchingMaterialPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMBatchingMaterialPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchingMaterialPartDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchingMaterialPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchingMaterialPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchingMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchingMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchingMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchingMaterialPartDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1543461038761Page(Page<WOMBatchingRejectParts> dg1543461038761Page,WOMBatchingMaterialPart batchingMaterialPart, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchingRejectParts.class);
		criteria.add(Restrictions.eq("batchingPartId", batchingMaterialPart));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"batchingRejectParts\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		batchingRejectPartsDao.findByPage(dg1543461038761Page, criteria);
		batchingRejectPartsService.findByNormalDataGridBatchingMaterialPart(dg1543461038761Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1543470616564Page(Page<WOMBatchingRejectParts> dg1543470616564Page,WOMBatchingMaterialPart batchingMaterialPart, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchingRejectParts.class);
		criteria.add(Restrictions.eq("batchingPartId", batchingMaterialPart));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		batchingRejectPartsDao.findByPage(dg1543470616564Page, criteria);
		batchingRejectPartsService.findByNormalDataGridBatchingMaterialPart(dg1543470616564Page);
	}
	
	 
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="ec.flow.save")
	public void saveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, DataGridService dataGridService){
		saveBatchingMaterialPart(batchingMaterialPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="ec.flow.save")
	public void saveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, DataGridService dataGridService, String viewCode){
		saveBatchingMaterialPart(batchingMaterialPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart")
	public void saveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchingMaterialPart(batchingMaterialPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart")
	public void saveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchingMaterial";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterial");
		String url = null;
		if(null != batchingMaterialPart.getId() && batchingMaterialPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart")
	public Map<Object, Object> batchingMaterialPartDataGridImport(WOMBatchingMaterialPart batchingMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchingMaterialPart.getVirtualId();
		
		batchingMaterialPart = batchingMaterialPartDao.load(batchingMaterialPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchingMaterialPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchingMaterialPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchingMaterialPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchingMaterialPart.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart")
	public void mergeBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchingMaterialPart);
		beforeSaveBatchingMaterialPart(batchingMaterialPart);
		batchingMaterialPartDao.merge(batchingMaterialPart);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterial",model="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMBatchingMaterialPart> getByPage(Page<WOMBatchingMaterialPart> page,DetachedCriteria detachedCriteria){
		return batchingMaterialPartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, Object... objects){
		checkUniqueConstraint(batchingMaterialPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
      
		if(batchingMaterialPart.getHeadId()!=null){
			long headid = batchingMaterialPart.getHeadId().getId();
			WOMBatchingMaterial batchingMaterial = batchingMaterialService.getBatchingMaterial(headid);
			batchingMaterialPart.setTaskID(batchingMaterial.getTaskID());
			batchingMaterialPart.setNeedPartId(batchingMaterial.getNeedPartId());
			batchingMaterialPart.setTableInfoId(batchingMaterial.getTableInfoId());
			if(batchingMaterial.getNeedPartId()!=null){
				long needpartId = batchingMaterial.getNeedPartId().getId();
				WOMBatchingMaterialNeed batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(needpartId);
				batchingMaterialPart.setTaskProcessId(batchingMaterialNeed.getTaskProcess());
				batchingMaterialPart.setFactoryId(batchingMaterialNeed.getFactoryId());
				batchingMaterialPart.setTaskActiveId(batchingMaterialNeed.getTaskActive());
				batchingMaterialPart.setActiveType(batchingMaterialNeed.getActiveType());
				batchingMaterialPart.setPutBatMateId(batchingMaterialNeed.getPutBatMateId());
				batchingMaterialPart.setOfferOrder(batchingMaterialNeed.getOfferOrder());
				batchingMaterialPart.setOfferSystem(batchingMaterialNeed.getOfferSystem());	
				//配料记录新添加了几个字段需要赋值  by pl 20170621
              	//pahseID
				batchingMaterialPart.setPhaseID(batchingMaterialNeed.getPhaseID());
				if(batchingMaterialNeed.getTaskActive()!=null){
					//配料顺序与投料顺序
					batchingMaterialPart.setOfferOrder(batchingMaterialNeed.getTaskActive().getActiveId()==null?null:batchingMaterialNeed.getTaskActive().getActiveId().getIngredientsOrder());
					batchingMaterialPart.setActOrder(  batchingMaterialNeed.getTaskActive().getActiveId()==null?null:batchingMaterialNeed.getTaskActive().getActiveId().getActOrder());
				}
				if(batchingMaterialNeed.getTaskID()!=null){
					batchingMaterialPart.setTaskBatch(batchingMaterialNeed.getTaskID().getProductBatchNum());
				}
				batchingMaterialPart.setBatchSite(batchingMaterialNeed.getBatchSite());
				batchingMaterialPart.setContainer(batchingMaterialNeed.getContainer());
			}
			if(batchingMaterialPart.getState() == null){
				batchingMaterialPart.setState((SystemCode) systemCodeService.getSystemCode("womBatPartState/01"));
			}
		}
/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
		batchingMaterialPartDao.flush();
		batchingMaterialPartDao.clear();
		batchingMaterialPart = batchingMaterialPartDao.load(batchingMaterialPart.getId());
        ...
        //将配料记录的ID赋值给标签Code
		batchingMaterialPart.setLableCode(batchingMaterialPart.getId().toString());
		
		//根据配料仓确定配料的目的仓库货位
		if(batchingMaterialPart.getArmWareID()==null){
			MESBasicFactoryModel fac= batchingMaterialPart.getFactoryId();
			//不是多货位
			if(fac!=null && !fac.getIsOnlyStore()){
				MESBasicObjWareStore ws=objWareStoreService.getObjWareStores("objFactory="+fac.getId(), null).size()>0?
						objWareStoreService.getObjWareStores("objFactory="+fac.getId(), null).get(0):null;
						batchingMaterialPart.setArmWareID(ws!=null?ws.getWare():null);
						batchingMaterialPart.setArmStoreID(ws!=null?ws.getStore():null);
			}
			//多货位
			if(fac!=null && fac.getIsOnlyStore()){
				MESBasicObjWareStore ws=objWareStoreService.getObjWareStores("objFactory="+fac.getId()+" and product="+batchingMaterialPart.getProductId().getId(), null).size()>0?
						objWareStoreService.getObjWareStores("objFactory="+fac.getId(), null).get(0):null;
						batchingMaterialPart.setArmWareID(ws!=null?ws.getWare():null);
						batchingMaterialPart.setArmStoreID(ws!=null?ws.getStore():null);
			}
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchingMaterialPart batchingMaterialPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchingMaterialPart batchingMaterialPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportBatchingMaterialPart(List<WOMBatchingMaterialPart> insertObjs, List<WOMBatchingMaterialPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchingMaterialPart,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportBatchingMaterialPart(List<WOMBatchingMaterialPart> insertObjs, List<WOMBatchingMaterialPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchingMaterialPart,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
	...
	/**
	 * 将表体id以  逗号 分隔开，找到对应每一个对象 更改状态为已配送
	 * @return
	 * @throws Exception
	 */
	@Override
	public boolean batch(String partIds){
		 String[] partList = partIds.split(",");
		 for (String partId : partList) {
			WOMBatchingMaterialPart batchingMaterialPart = getBatchingMaterialPart(Long.parseLong(partId));
			if(batchingMaterialPart!=null){
				batchingMaterialPart.setState((SystemCode) systemCodeService.getSystemCode("womBatPartState/02"));
              	batchingMaterialPart.setBatchDate(new Date());
				batchingMaterialPartDao.merge(batchingMaterialPart);
              	//修改已配数量
				if(batchingMaterialPart.getNeedPartId()!=null){
					WOMBatchingMaterialNeed batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(batchingMaterialPart.getNeedPartId().getId());
					//已配数量
					BigDecimal useNum = batchingMaterialNeed.getUseNum();
					if(useNum==null || useNum.equals("")){
						useNum = new BigDecimal(0);
					}
					if(batchingMaterialPart.getDoneNum()!=null){
						useNum = useNum.add(batchingMaterialPart.getDoneNum());
					}
	...
	public void addDownNum(WOMBatchingMaterialPart batchingMaterialPart) {
		if(batchingMaterialPart.getDoneNum()!=null){
			if(batchingMaterialPart.getHeadId()!=null){
				...
				BigDecimal doneNum = batchingMaterial.getDoneNum();
				if(doneNum==null || doneNum.equals("")){
					doneNum = new BigDecimal(0);
				}
				batchingMaterial.setDoneNum(doneNum.add(batchingMaterialPart.getDoneNum()));
    ...
    /**
	 * 配料签收  获取pda对应工作单元和生产批次
	 */
	@Override
	public List<Map<String,Object>> getBatchMaterialWorkunitBatch(String factoryCode) {		
		String[] factoryCodeArr = factoryCode.split(",");
		String sql = "SELECT factory.NODENAME,womPart.TASK_BATCH,womPart.FACTORY_ID "+
					"FROM WOM_BATCHING_MATERIAL_PARTS womPart,FACTORY_TBCOMPANYSTRUCTURE factory "+
					"WHERE womPart.FACTORY_ID = factory.ID AND womPart.STATE = 'womBatPartState/02' "+
					"AND womPart.valid = 1 AND factory.CODE IN ( ";
					
		for(int i= 0;i<factoryCodeArr.length ;i++){
			sql += "'" + factoryCodeArr[i] + "',";
		}
		sql = sql.substring(0, sql.length()-1);
		sql += " )  GROUP BY factory.NODENAME,womPart.TASK_BATCH,womPart.FACTORY_ID ";
		List<Object[]> mixMaterialObjList = batchingMaterialPartDao.createNativeQuery(sql).list();
		List<Map<String,Object>> mixMaterialList = new ArrayList<Map<String,Object>>();
		for(int i = 0;i< mixMaterialObjList.size();i++){
			Map<String,Object> map = new HashMap<String,Object>();
			Object[] obj = mixMaterialObjList.get(i);
			map.put("NODENAME", obj[0]);
			map.put("TASK_BATCH", obj[1]);
			map.put("FACTORY_ID", obj[2]);
			mixMaterialList.add(map);
		}
		return mixMaterialList;
	}	
	/**
	 * 配料签收  根据工作单元和状态为已配送获取pda对应工作单元和生产批次的明细
	 */
	@Override
	public List<Map<String, Object>> getBatchMaterialDetail(String factoryID,String taskBatch) {
		String sql = "SELECT womPart.ID ID,OFFER_ORDER,ACT_ORDER,DONE_NUM,BATCH_NUM,CONTAINER,BATCH_SITE,product.PRODUCT_MODEL PRODUCT_MODEL "+
					"FROM WOM_BATCHING_MATERIAL_PARTS womPart,S2BASE_PRODUCT product "+
					"WHERE  womPart.PRODUCT_ID = product.PRODUCT_ID and FACTORY_ID = ? AND TASK_BATCH = ? AND STATE = 'womBatPartState/02' " +
					"AND womPart.valid = 1 "+
					"ORDER BY BATCH_SITE DESC,CONTAINER ASC, OFFER_ORDER ASC,ACT_ORDER ASC, BATCH_NUM ASC, DONE_NUM DESC ";
		List<Map<String, Object>> womBatchingMaterialPartsList = batchingMaterialPartDao.createNativeQuery(sql, factoryID,taskBatch).setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
		for (int i = 0; i < womBatchingMaterialPartsList.size(); i++) {
          //根据配料方式，若“配于现场”为“投”；若“配放于容器”为“配”。
			if ("batchSite/batchSet1".equals((String)womBatchingMaterialPartsList.get(i).get("BATCH_SITE"))) {
				womBatchingMaterialPartsList.get(i).put("BATCH_SITE","投");
			}else{
				womBatchingMaterialPartsList.get(i).put("BATCH_SITE","配");
			}
		}
		return womBatchingMaterialPartsList;
	}
	

	/**
	 * 配料签收    PDA签收更改状态，回填接收人和接收时间
	 */
	@Override
	public void updateBatchMaterialDetail(String batchMaterialDetailID,String userName) {
		Staff staff = userService.getUser(userName)!=null?userService.getUser(userName).getStaff():null;  //根据登陆用户名获取Staff
		for(int i=0;i<batchMaterialDetailID.split(",").length;i++){
 			String id=batchMaterialDetailID.split(",")[i];
 			WOMBatchingMaterialPart part=getBatchingMaterialPart(Long.parseLong(id));
 			//配料状态更新为已送达
 			part.setState(new SystemCode("womBatPartState/03"));
 			//接收时间
 			part.setReceiveDate(new Date());
 			//接收人 
 			part.setReceiveStaff(staff);
 			batchingMaterialPartDao.update(part);
 		}
	}
	...
	public void updateBatchMaterialState(WOMBatchingMaterialPart part){
		part.setState(new SystemCode("womBatPartState/04"));
		batchingMaterialPartDao.update(part);
	}
	...
	public void saveJsonPart(String dglist) {
		List<WOMBatchingMaterialPart> parts=(List<WOMBatchingMaterialPart>)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dglist, WOMBatchingMaterialPart.class, batchingMaterialPartDao.getSessionFactory());
		if(parts != null && parts.size() > 0){
			for(WOMBatchingMaterialPart part: parts){
				batchingMaterialPartDao.save(part);
				
				batchingMaterialPartDao.flush();
				batchingMaterialPartDao.clear();
				part = batchingMaterialPartDao.load(part.getId());
				part.setPutBatMateId(part.getHeadId().getNeedPartId().getPutBatMateId());
				part.setNeedPartId(part.getHeadId().getNeedPartId());
				part.setTaskProcessId(part.getHeadId().getNeedPartId().getTaskProcess());
				part.setTaskActiveId(part.getHeadId().getNeedPartId().getTaskActive());
				part.setPhaseID(part.getHeadId().getNeedPartId().getPhaseID());
				part.setContainer(part.getHeadId().getNeedPartId().getContainer());
				part.setOfferOrder(part.getHeadId().getNeedPartId().getOfferOrder());
				part.setActOrder(part.getHeadId().getNeedPartId().getActOrder());
				part.setBatchSite(part.getHeadId().getNeedPartId().getBatchSite());
				part.setActiveType(part.getHeadId().getNeedPartId().getActiveType());
				part.setTaskBatch(part.getHeadId().getTaskID().getProductBatchNum());
				part.setLableCode(part.getId()!= null?part.getId().toString():"1000");
				batchingMaterialPartDao.merge(part);
			}
		}
	}
	...
	public void addPrintCount(String printDataIds) {
		String[] printDataIdArr = printDataIds.split(",");
		for(int i = 0 ; i < printDataIdArr.length; i++){
			WOMBatchingMaterialPart bmp = getBatchingMaterialPart(Long.valueOf(printDataIdArr[i]));
			if(bmp != null){
				int printCount = bmp.getPrintCount() != null?bmp.getPrintCount():0;
				bmp.setPrintCount(printCount + 1);
				batchingMaterialPartDao.save(bmp);
				batchingMaterialPartDao.flush();
				batchingMaterialPartDao.clear();
			}
		}
	...
	public boolean checkEdit(Map<String, String> params){
		WOMBatchingMaterialPart bmp = getBatchingMaterialPart(Long.valueOf(params.get("id")));
		if(bmp.getDoneNum() != null && bmp.getDoneNum().compareTo(new BigDecimal(params.get("doneNum"))) != 0 ){
			return false;
		}
		if(bmp.getBatchNum() != null && !bmp.getBatchNum().equals(params.get("batchNum"))){
			return false;
		}
		if(bmp.getBatchNumObj() != null && !bmp.getBatchNumObj().getBatchText().equals(params.get("batchNumObj"))){
			return false;
		}
		if(params.get("wardId") != null && params.get("wardId").length() > 0 && !params.get("wardId").equals("null") && bmp.getWareID().getId().intValue() != Integer.valueOf(params.get("wardId")) ){
			return false;
		}
		if(params.get("storeId") != null && params.get("storeId").length() > 0 && !params.get("storeId").equals("null") && bmp.getStoreID().getId().intValue() != Integer.valueOf(params.get("storeId")) ){
			return false;
		}
		return true;
	}
	...
	/**
	 * 检查是否编辑
	 * batchingMaterialPartIds 待删除数据Id
	 */
		...
		/*if(batchingMaterialParts != null && batchingMaterialParts.size() > 0 ){
			for(WOMBatchingMaterialPart batchingMaterialPart : batchingMaterialParts){
				...
				try {
					...
					dataAuditLogService.saveCustomerAudit("删除",batchingMaterialPart, batchingMaterialPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}*/
...
//		}
	...
	public void saveBatchingMaterialPartCustom(WOMBatchingMaterialPart batchingMaterialPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		...
		dealDatagridsSaveCustom(batchingMaterialPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
    ...
    private void dealDatagridsSaveCustom(WOMBatchingMaterialPart batchingMaterialPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		if(dgLists==null&&dgDeleteIDs==null){
			return;
		}
		List<DataGrid> dgs=viewServiceFoundation.getDataGrids(viewCode, "runtime");
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()), _reflect_dgclass,batchingMaterialPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchingMaterialPart.class);
						java.lang.reflect.Method _reflect_saveMethod=_reflect_serviceType.getMethod("save"+_targetModel.getModelName(),_reflect_dgclass,Map.class,Map.class,Map.class,String.class,String.class,boolean[].class);
						for(Object odg:dglist){
							WOMBatchingRejectParts rejectParts = (WOMBatchingRejectParts) odg;
							
							WOMBatchingMaterialPart batchingMaterialPartDeal = getBatchingMaterialPart(rejectParts.getBatchingPartId().getId());
							//不是待配料状态的单据
							if(batchingMaterialPartDeal.getState() != null && !"womBatPartState/01".equals(batchingMaterialPartDeal.getState().getId()) ){
								...
								//回填退料数量和退料原因至配料记录
								batchingMaterialPartDeal = getBatchingMaterialPart(rejectParts.getBatchingPartId().getId());
								batchingMaterialPartDeal.setRejectNum(rejectParts.getRejectNum());
								batchingMaterialPartDeal.setRejectReason(rejectParts.getRejectReasonId());
								batchingMaterialPartDeal.setState(new SystemCode("womBatPartState/05"));
								batchingMaterialPartDao.save(batchingMaterialPartDeal);
								batchingMaterialPartDao.flush();
								
								//配料指令减少去退料数量
								WOMBatchingMaterial batchingMaterial = batchingMaterialService.getBatchingMaterial(rejectParts.getBatchingId().getId());
								batchingMaterial.setDoneNum(batchingMaterial.getDoneNum().subtract(rejectParts.getRejectNum()));
								batchingMaterialDao.save(batchingMaterial);
								batchingMaterialDao.flush();
				
								//配料需求，减少相应数量。根据原因，修改状态。
								WOMBatchingMaterialNeed batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(batchingMaterial.getNeedPartId().getId());
								batchingMaterialNeed.setUseNum(batchingMaterialNeed.getUseNum().subtract(rejectParts.getRejectNum()));
								WOMRejectReason rejectReason = rejectReasonService.getRejectReason(rejectParts.getRejectReasonId().getId());
								if(rejectReason.getIsChangeNeed()){
									batchingMaterialNeed.setState(new SystemCode("womNeedState/01"));
								}
								...
								//删除配料记录
								batchingMaterialPartDeal = getBatchingMaterialPart(rejectParts.getBatchingPartId().getId());
								batchingMaterialPartDeal.setValid(false);
								batchingMaterialPartDao.save(batchingMaterialPartDeal);
								batchingMaterialPartDao.flush();
								...
								//配料需求，减少相应数量。根据原因，修改状态。   配料下达时，配料需求的数量才会增加
								/*WOMBatchingMaterialNeed batchingMaterialNeed = batchingMaterialNeedService.getBatchingMaterialNeed(rejectParts.getBatchingId().getNeedPartId().getId());
								batchingMaterialNeed.setUseNum(batchingMaterialNeed.getUseNum().subtract(rejectParts.getRejectNum()));
								if(rejectParts.getRejectReasonId().getIsChangeNeed()){
									batchingMaterialNeed.setState(new SystemCode("womNeedState/01"));
								}
								batchingMaterialNeedDao.save(batchingMaterialNeed);
								batchingMaterialNeedDao.flush();*/
    ...
    //配料记录状态查询
  	public String checkHasWaitPutinRecord(String batchingMaterialPartIds){
  		String[] batchingMaterialPartArr = batchingMaterialPartIds.split(",");
  		
  		for(String batchingMaterialPartId : batchingMaterialPartArr){
  			WOMBatchingMaterialPart batchingMPart = getBatchingMaterialPart(Long.valueOf(batchingMaterialPartId));
  			List<WOMWaitPutinRecords> putinRecords = waitPutinRecordsDao.findByCriteria(Restrictions.eq("batchMaterialID", batchingMPart),Restrictions.eq("valid", true),Restrictions.eq("isFinish", false));
  			if(putinRecords != null && putinRecords.size() > 0){
  				return "选中配料记录存在未执行的待投料记录，不能退料！";
  			}
  		}
    ...
    public Map getPrintParam() {
        ...
        boolean need = true;
        String sql = "select deployment_id,table_info_id from wom_batching_materials where valid = ?";
        List<Object[]> list = batchingMaterialPartDao.createNativeQuery(sql, 1).list();
        if (list != null && list.size() > 0) {
            for (Object[] o : list) {
                Deployment deployment = processService.getDeployment(Long.valueOf(o[0].toString()));
                if (null != deployment) {
                    printTemplate = printService.loadPrintTemplate("WOM_7.5.0.0_batchingMaterial_batMaterialEdit", deployment.getProcessKey(), deployment.getProcessVersion());
                    if (printTemplate != null) {
                        result.put("result", true);
                        result.put("tableInfoId", o[0].toString());
                        result.put("deploymentId", o[1].toString());
                        need = false;
                        break;
                    }
                }
