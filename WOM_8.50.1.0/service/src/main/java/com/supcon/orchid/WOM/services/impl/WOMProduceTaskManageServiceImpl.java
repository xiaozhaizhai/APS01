...
import com.supcon.orchid.WOM.services.WOMProduceTaskManageImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMProduceTaskManageDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
@Service("wOM_produceTaskManageService")
@Transactional
public class WOMProduceTaskManageServiceImpl  extends BaseServiceImpl<WOMProduceTaskManage> implements WOMProduceTaskManageService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMProduceTaskManage> bapBaseService;
	...
	@Autowired
	private WOMProduceTaskManageImportService produceTaskManageImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMProduceTaskManage.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  produceTaskManageDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMProduceTaskManage.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = produceTaskManageDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProduceTaskManage.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProduceTaskManage.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  produceTaskManageDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMProduceTaskManage.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMProduceTaskManage.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMProduceTaskManage> list = (List<WOMProduceTaskManage>) produceTaskManageDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMProduceTaskManage getProduceTaskManage(long id){
		return getProduceTaskManage(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskManage getProduceTaskManage(long id, String viewCode){
		WOMProduceTaskManage produceTaskManage = produceTaskManageDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProduceTaskManageAsJSON(long id, String include){
		WOMProduceTaskManage produceTaskManage = produceTaskManageDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(produceTaskManage, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskManage(WOMProduceTaskManage produceTaskManage){
		if(null != produceTaskManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
			AuditUtil.setColumnStringC(produceTaskManage.getId().toString());
		}
		beforeDeleteProduceTaskManage(produceTaskManage);
		produceTaskManageDao.delete(produceTaskManage);
		afterDeleteProduceTaskManage(produceTaskManage);
		produceTaskManageDao.flush();
		List<WOMProduceTaskManage> params = new ArrayList<WOMProduceTaskManage>();
		params.add(produceTaskManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskManage(List<Long> produceTaskManageIds){
		deleteProduceTaskManage(produceTaskManageIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskManage(List<Long> produceTaskManageIds, String eventTopic) {
		List<WOMProduceTaskManage> produceTaskManages = new ArrayList<WOMProduceTaskManage>();
		for(Long produceTaskManageId : produceTaskManageIds){
			WOMProduceTaskManage produceTaskManage = getProduceTaskManage(produceTaskManageId);
			produceTaskManages.add(produceTaskManage);
			if(null != produceTaskManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == produceTaskManage.getId() ? "" : "," + produceTaskManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == produceTaskManage.getId() ? "" : "," + produceTaskManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == produceTaskManage.getId() ? "" : "," + produceTaskManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(produceTaskManages != null && produceTaskManages.size() > 0 ){
			for(WOMProduceTaskManage produceTaskManage : produceTaskManages){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),produceTaskManage, produceTaskManage.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(produceTaskManageIds != null && produceTaskManageIds.size() > 0) {
			String hql = "update " + WOMProduceTaskManage.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = produceTaskManageDao.createQuery(hql);
			query.setParameterList("ids", produceTaskManageIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMProduceTaskManage produceTaskManagez : produceTaskManages){
				produceTaskManagez.setValid(false);
				List<WOMProduceTaskManage> params = new ArrayList<WOMProduceTaskManage>();
				params.add(produceTaskManagez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_generalManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskManage(long produceTaskManageId, int produceTaskManageVersion){
		this.deleteProduceTaskManage(produceTaskManageId, produceTaskManageVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskManage(long produceTaskManageId, int produceTaskManageVersion,SignatureLog signatureLog){
		WOMProduceTaskManage produceTaskManage = getProduceTaskManage(produceTaskManageId);
		...
		beforeDeleteProduceTaskManage(produceTaskManage);
		produceTaskManageDao.delete(produceTaskManageId, produceTaskManageVersion);
		afterDeleteProduceTaskManage(produceTaskManage);
		produceTaskManageDao.flush();
		List<WOMProduceTaskManage> params = new ArrayList<WOMProduceTaskManage>();
		params.add(produceTaskManage);
		...
		if(signatureLog != null ) {
			...
			businessKey=produceTaskManage.getId();
			signatureLog.setTableId(produceTaskManage.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskManage(String produceTaskManageIds){
		this.deleteProduceTaskManage(produceTaskManageIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskManage(String produceTaskManageIds,SignatureLog signatureLog){
		deleteCollection(produceTaskManageIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProduceTaskManage(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_generalManage#modelCode:WOM_7.5.0.0_generalManage_ProduceTaskManage#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="ec.engine.restore",operType="10")
	public void restoreProduceTaskManage(String produceTaskManageIds){
		restoreCollection(produceTaskManageIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProduceTaskManage(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="ec.engine.restore",operType="10")
	public void restoreProduceTaskManage(long produceTaskManageId){
		
		findBusinessKeyUsed(produceTaskManageId);	//判断业务主键是否重复
		...
		beforeRestoreProduceTaskManage(produceTaskManage);
		produceTaskManage.setValid(true);
		produceTaskManageDao.update(produceTaskManage);
		afterRestoreProduceTaskManage(produceTaskManage);
		List<WOMProduceTaskManage> params = new ArrayList<WOMProduceTaskManage>();
		params.add(produceTaskManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long produceTaskManageId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_generalManage_ProduceTaskManage");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMProduceTaskManage.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMProduceTaskManage.TABLE_NAME +" where id = ? )";
			List<Object> list =  produceTaskManageDao.createNativeQuery(sql,produceTaskManageId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProduceTaskManage(List<WOMProduceTaskManage>  produceTaskManages){
		for(WOMProduceTaskManage produceTaskManage:produceTaskManages)  {
			saveProduceTaskManage(produceTaskManage, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProduceTaskManage(List<WOMProduceTaskManage>  produceTaskManages){
		for(WOMProduceTaskManage produceTaskManage:produceTaskManages)  {
			saveProduceTaskManage(produceTaskManage, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="ec.flow.save")
	public void saveProduceTaskManage(WOMProduceTaskManage produceTaskManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProduceTaskManage(produceTaskManage, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="ec.flow.save")
	public void saveProduceTaskManage(WOMProduceTaskManage produceTaskManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProduceTaskManage(produceTaskManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProduceTaskManageCodes(WOMProduceTaskManage produceTaskManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProduceTaskManageCodes(produceTaskManage, false);
	}
	
	public void generateProduceTaskManageCodes(WOMProduceTaskManage produceTaskManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProduceTaskManageSummarys(WOMProduceTaskManage produceTaskManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProduceTaskManageSummarys(produceTaskManage, false);
	}

	public void generateProduceTaskManageSummarys(WOMProduceTaskManage produceTaskManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage")
	public void saveProduceTaskManage(WOMProduceTaskManage produceTaskManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProduceTaskManage(produceTaskManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage")
	public void saveProduceTaskManage(WOMProduceTaskManage produceTaskManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(produceTaskManage.getId() != null && produceTaskManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_generalManage/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_generalManage/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, produceTaskManage);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(produceTaskManage);
		beforeSaveProduceTaskManage(produceTaskManage, viewIsView);

		if (viewIsView) {
			produceTaskManageDao.saveWithRevertVersion(produceTaskManage);
		} else {
			if(isNew)
				produceTaskManageDao.save(produceTaskManage);
			else
				produceTaskManageDao.merge(produceTaskManage);
		}
		if(null != produceTaskManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
		...
		dealDatagridsSave(produceTaskManage,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProduceTaskManageCodes(produceTaskManage, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProduceTaskManageSummarys(produceTaskManage, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProduceTaskManage(produceTaskManage, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, produceTaskManage);
			}
		}
		List<WOMProduceTaskManage> params = new ArrayList<WOMProduceTaskManage>();
		produceTaskManageDao.flush();
		produceTaskManageDao.clear();
		produceTaskManage = produceTaskManageDao.load(produceTaskManage.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMProduceTaskManage produceTaskManage){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("produceTaskManage", produceTaskManage);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage")
	public void mergeProduceTaskManage(WOMProduceTaskManage produceTaskManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(produceTaskManage);
		beforeSaveProduceTaskManage(produceTaskManage);
		produceTaskManageDao.merge(produceTaskManage);
		if(null != produceTaskManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
		...
		dealDatagridsSave(produceTaskManage,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProduceTaskManage(produceTaskManage);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMProduceTaskManage> findProduceTaskManages(Page<WOMProduceTaskManage> page, Criterion... criterions) {
		return produceTaskManageDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMProduceTaskManageDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMProduceTaskManageDealInfo> list=produceTaskManageDao.createQuery("from " + WOMProduceTaskManageDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMProduceTaskManageDealInfo dealInfo) {
		produceTaskManageDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMProduceTaskManageDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) produceTaskManageDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMProduceTaskManageDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = produceTaskManageDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = produceTaskManageDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMProduceTaskManageDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = produceTaskManageDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskManages(Page<WOMProduceTaskManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProduceTaskManages(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskManages(Page<WOMProduceTaskManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskManages(Page<WOMProduceTaskManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMProduceTaskManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"produceTaskManage\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"produceTaskManage\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"produceTaskManage\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_generalManage_ProduceTaskManage" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_generalManage_ProduceTaskManage" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  produceTaskManageDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  produceTaskManageDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_generalManage").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceTaskManage\"","WOM_7.5.0.0_generalManage",customCondition,"指令单",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"produceTaskManage\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMProduceTaskManageDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"produceTaskManage\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"produceTaskManage\".LAY_REC ASC, \"produceTaskManage\".SORT ASC");
				} else {
					orderPart.append(" \"produceTaskManage\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) produceTaskManageDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=produceTaskManageDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = produceTaskManageDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMProduceTaskManage> result = new ArrayList<WOMProduceTaskManage>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMProduceTaskManage.class,produceTaskManageDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMProduceTaskManage.class,produceTaskManageDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMProduceTaskManage produceTaskManage : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(produceTaskManage.getId(), "WOM_generalManage_produceTaskManage");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(produceTaskManage.getId(), "WOM_generalManage_produceTaskManage");
							if(null != documents && !documents.isEmpty()) {
								produceTaskManage.setDocument(documents.get(0));
								produceTaskManage.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_generalManage")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_generalManage")+"WOM_7.5.0.0_generalManage".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_generalManage",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMProduceTaskManage> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMProduceTaskManage> result) {
		View view = null;
		...
		Map<Long, WOMProduceTaskManage> rsMap = new HashMap<Long, WOMProduceTaskManage>();
		for (WOMProduceTaskManage produceTaskManage : result) {
			ids.add(produceTaskManage.getId());
			rsMap.put(produceTaskManage.getId(), produceTaskManage);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(produceTaskManageDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = produceTaskManageDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMProduceTaskManage produceTaskManage,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(produceTaskManage.getOwnerStaffId());
		ti.setOwnerPositionId(produceTaskManage.getOwnerPositionId());
		ti.setOwnerDepartmentId(produceTaskManage.getOwnerDepartmentId());
		
		ti.setCreateStaff(produceTaskManage.getCreateStaff());
		ti.setCreateStaffId(produceTaskManage.getCreateStaffId());
		ti.setCreateTime(produceTaskManage.getCreateTime());
		ti.setDeleteStaff(produceTaskManage.getDeleteStaff());
		ti.setDeleteStaffId(produceTaskManage.getDeleteStaffId());
		ti.setDeleteTime(produceTaskManage.getDeleteTime());
		ti.setCreateDepartmentId(produceTaskManage.getCreateDepartmentId());
		ti.setModifyStaff(produceTaskManage.getModifyStaff());
		ti.setModifyStaffId(produceTaskManage.getModifyStaffId());
		ti.setCreatePositionId(produceTaskManage.getCreatePositionId());
		ti.setPositionLayRec(produceTaskManage.getPositionLayRec());
		ti.setTableNo(produceTaskManage.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_generalManage");
		ti.setTargetTableName("指令单");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_generalManage_ProduceTaskManage" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  produceTaskManageDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceTaskManage\"","WOM_7.5.0.0_generalManage","","指令单",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"produceTaskManage\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"produceTaskManage\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceTaskManage\"","WOM_7.5.0.0_generalManage","","指令单",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("produceTaskManage.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"produceTaskManage\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"produceTaskManage\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"produceTaskManage\".CID = 1 OR \"produceTaskManage\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("produceTaskManage.mc\"").append(".PRODUCE_TASK_MANAGE) not in ("+notinValue+")");
			}
			
			SQLQuery query = produceTaskManageDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("generalManage", "WOM_7.5.0.0_generalManage", "generalManage_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=produceTaskManageDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMProduceTaskManage.ENTITY_CODE;}
	protected String getModuleCode(){return WOMProduceTaskManage.MODULE_CODE;}
	protected String getModelCode(){return WOMProduceTaskManage.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param produceTaskManage 模型实例
	 */
	public void checkUniqueConstraint(WOMProduceTaskManage produceTaskManage){
	}
	
	
	@Override
	public List<WOMProduceTaskManage> getProduceTaskManages(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskManage.class);
		if(params!=null){
			...
			return produceTaskManageDao.findByHql("from " + WOMProduceTaskManage.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return produceTaskManageDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMProduceTaskManage> findProduceTaskManagesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskManage.class);
	...
	@Override
	public List<WOMProduceTaskManage> findProduceTaskManagesByHql(String hql, Object... objects){
		
		return   produceTaskManageDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMProduceTaskManage loadProduceTaskManageByBussinessKey(WOMProduceTaskManage produceTaskManage){
		return loadProduceTaskManageByBussinessKey(produceTaskManage.getId());
	}
	
	@Override
	public Page<WOMProduceTaskManage> getProduceTaskManages(Page<WOMProduceTaskManage> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskManage.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = produceTaskManageDao.findByPage(page, sql);
			}else{
				page = produceTaskManageDao.findAllByPage(page);
			}
			return page;
		}
		page = produceTaskManageDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskManage loadProduceTaskManageByBussinessKey(Serializable bussinessKey){
		return produceTaskManageDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProduceTaskManage> findByProperty(String propertyName, Object object){
		return produceTaskManageDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskManage findEntityByProperty(String propertyName, Object object){
		return produceTaskManageDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMProduceTaskManage.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = produceTaskManageDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_generalManage_ProduceTaskManage","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_generalManage", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMProduceTaskManage produceTaskManage,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,produceTaskManageDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMProduceTaskManage.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, produceTaskManage);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",produceTaskManage.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProduceTaskManage.TABLE_NAME +" where valid = 1";
			list = produceTaskManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProduceTaskManage.TABLE_NAME;
			list = produceTaskManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMProduceTaskManage.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = produceTaskManageDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMProduceTaskManageDaoImpl produceTaskManageDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProduceTaskManage(final List<WOMProduceTaskManage> insertObjs, final List<WOMProduceTaskManage> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProduceTaskManage(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProduceTaskManage(final List<WOMProduceTaskManage> insertObjs, final List<WOMProduceTaskManage> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1502695530568"),importCount));
		...
		Map<Long, WOMProduceTaskManage> oldUpdateObjsMap = new HashMap<Long, WOMProduceTaskManage>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMProduceTaskManage bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMProduceTaskManage> bizObjList = produceTaskManageDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMProduceTaskManage bizObj = bizObjList.get(mIndex);
		...
		final List<WOMProduceTaskManage> nmeObjs = new ArrayList<WOMProduceTaskManage>();
		...
		long id=autoGeneratorID.getNextId(WOMProduceTaskManage.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMProduceTaskManage produceTaskManage:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMProduceTaskManage.TABLE_NAME,1,dbId);
			if(produceTaskManage.getId() != null ){
				m.put(produceTaskManage.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) produceTaskManageDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_ProduceTaskManage"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		produceTaskManageDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMProduceTaskManage.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMProduceTaskManage produceTaskManage = updateObjs.get(updateIndex);
						Long id = produceTaskManage.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = produceTaskManage.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(produceTaskManage);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMProduceTaskManage.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMProduceTaskManage.TABLE_NAME + " order by id desc";
//					Query query = produceTaskManageDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMProduceTaskManage produceTaskManage = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = produceTaskManage.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		produceTaskManageDaoImpl.getSession().flush();
		produceTaskManageDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMProduceTaskManage bizObj = insertObjs.get(insertIndex);
			produceTaskManageImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMProduceTaskManage bizObj = updateObjs.get(updateIndex);
			WOMProduceTaskManage oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			produceTaskManageImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = produceTaskManageDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				produceTaskManageDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = produceTaskManageDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_ProduceTaskManage").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = produceTaskManageDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = produceTaskManageDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = produceTaskManageDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?produceTaskManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  produceTaskManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,produceTaskManageDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1502700158273Page(Page<WOMPutInManage> dg1502700158273Page,WOMProduceTaskManage produceTaskManage, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMPutInManage.class);
		criteria.add(Restrictions.eq("produceTask", produceTaskManage));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"putInManage\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		putInManageDao.findByPage(dg1502700158273Page, criteria);
		putInManageService.findByNormalDataGridProduceTaskManage(dg1502700158273Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1502700453811Page(Page<WOMCheckManage> dg1502700453811Page,WOMProduceTaskManage produceTaskManage, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMCheckManage.class);
		criteria.add(Restrictions.eq("produceTask", produceTaskManage));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"checkManage\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		checkManageDao.findByPage(dg1502700453811Page, criteria);
		checkManageService.findByNormalDataGridProduceTaskManage(dg1502700453811Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1502700759865Page(Page<WOMTaskReportManage> dg1502700759865Page,WOMProduceTaskManage produceTaskManage, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMTaskReportManage.class);
		criteria.add(Restrictions.eq("produceTask", produceTaskManage));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"taskReportManage\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		taskReportManageDao.findByPage(dg1502700759865Page, criteria);
		taskReportManageService.findByNormalDataGridProduceTaskManage(dg1502700759865Page);
	}
	
	 
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="ec.flow.save")
	public void saveProduceTaskManage(WOMProduceTaskManage produceTaskManage, DataGridService dataGridService){
		saveProduceTaskManage(produceTaskManage, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="ec.flow.save")
	public void saveProduceTaskManage(WOMProduceTaskManage produceTaskManage, DataGridService dataGridService, String viewCode){
		saveProduceTaskManage(produceTaskManage, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage")
	public void saveProduceTaskManage(WOMProduceTaskManage produceTaskManage, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProduceTaskManage(produceTaskManage, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage")
	public void saveProduceTaskManage(WOMProduceTaskManage produceTaskManage, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(null != produceTaskManage.getId() && produceTaskManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage")
	public Map<Object, Object> produceTaskManageDataGridImport(WOMProduceTaskManage produceTaskManage, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = produceTaskManage.getVirtualId();
		
		produceTaskManage = produceTaskManageDao.load(produceTaskManage.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, produceTaskManage.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = produceTaskManage.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(produceTaskManage);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, produceTaskManage.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage")
	public void mergeProduceTaskManage(WOMProduceTaskManage produceTaskManage, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(produceTaskManage);
		beforeSaveProduceTaskManage(produceTaskManage);
		produceTaskManageDao.merge(produceTaskManage);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_ProduceTaskManage",desc="foundation.batch.save",operType="4")
	public void batchSaveProduceTaskManage(WOMProduceTaskManage produceTaskManage, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
	...
	@Override
	public Page<WOMProduceTaskManage> getByPage(Page<WOMProduceTaskManage> page,DetachedCriteria detachedCriteria){
		return produceTaskManageDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_generalManage").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveProduceTaskManage(WOMProduceTaskManage produceTaskManage, Object... objects){
		checkUniqueConstraint(produceTaskManage);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProduceTaskManage(WOMProduceTaskManage produceTaskManage, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProduceTaskManage(List<WOMProduceTaskManage> produceTaskManages, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProduceTaskManage(List<WOMProduceTaskManage> produceTaskManages){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProduceTaskManage(List<WOMProduceTaskManage> produceTaskManages, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProduceTaskManage(List<WOMProduceTaskManage> produceTaskManages){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProduceTaskManage(WOMProduceTaskManage produceTaskManage){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProduceTaskManage(WOMProduceTaskManage produceTaskManage){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProduceTaskManage(WOMProduceTaskManage produceTaskManage){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProduceTaskManage(WOMProduceTaskManage produceTaskManage){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMProduceTaskManage produceTaskManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMProduceTaskManage produceTaskManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProduceTaskManage(List<WOMProduceTaskManage> insertObjs, List<WOMProduceTaskManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProduceTaskManage,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProduceTaskManage(List<WOMProduceTaskManage> insertObjs, List<WOMProduceTaskManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProduceTaskManage,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
	@Autowired
	...
	//根据指令单查找所有生效投料单表体数据
	@Override
	public List<WOMPutInManage> getPartData(String produceTaskId) {
		List<WOMPutInManage> putInManageList= new ArrayList<WOMPutInManage>();
		String hql = "from WOMPutInMaterial where valid = 1 and status = 99 and taskID.id = ?";
		List<WOMPutInMaterial> putInMaterialList = taskReportingPartDao.findByHql(hql,Long.parseLong(produceTaskId));
		if(putInMaterialList!=null && putInMaterialList.size()>0)
			for (WOMPutInMaterial putInMaterial : putInMaterialList) {
				String hqlChart = "from WOMPutInMaterialPart where valid = 1 and headId = ?";
				List<WOMPutInMaterialPart> putInMaterialParts = taskReportingPartDao.findByHql(hqlChart,putInMaterial);
				if(putInMaterialParts!=null && putInMaterialParts.size()>0)
					for (WOMPutInMaterialPart putInMaterialPart : putInMaterialParts) {
						WOMPutInManage putInManage = new WOMPutInManage();
						putInManage.setPutInMetarialPart(putInMaterialPart);
						putInManage.setPutInUrl(batchDealService.getOpenViewUrl("WOM_7.5.0.0_putInMaterial_putinView","WOM_7.5.0.0_putInMaterial_putinList",putInMaterial.getTableInfoId() , putInMaterial.getId()));
	...
	//根据指令单查找所有生效报工单表体数据
	@Override
	public List<WOMTaskReportManage> getTaskReport(String produceTaskId) {
		List<WOMTaskReportManage> putInManageList= new ArrayList<WOMTaskReportManage>();
		String hql = "from WOMTaskReporting where valid = 1 and status = 99 and taskID.id = ?";
		List<WOMTaskReporting> taskReportingList = taskReportingPartDao.findByHql(hql,Long.parseLong(produceTaskId));
		if(taskReportingList!=null && taskReportingList.size()>0)
			for (WOMTaskReporting taskReporting : taskReportingList) {
				String hqlChart = "from WOMTaskReportingPart where valid = 1 and headId = ?";
				List<WOMTaskReportingPart> taskReportingParts = taskReportingPartDao.findByHql(hqlChart,taskReporting);
				if(taskReportingParts!=null && taskReportingParts.size()>0)
					for (WOMTaskReportingPart taskReportingPart : taskReportingParts) {
						WOMTaskReportManage taskReportManage = new WOMTaskReportManage();
						taskReportManage.setTaskReportPart(taskReportingPart);
						taskReportManage.setTaskReportUrl(batchDealService.getOpenViewUrl("WOM_7.5.0.0_taskReporting_taskReportingView","WOM_7.5.0.0_taskReporting_taskReportingList",taskReporting.getTableInfoId() , taskReporting.getId()));
						putInManageList.add(taskReportManage);
					}
			}
		return putInManageList;
	}
	//根据指令单查找所有生效请检单表体数据
	@Override
	public List<WOMCheckManage> getCheckManage(String batchNum) {
		List<WOMCheckManage> putInManageList= new ArrayList<WOMCheckManage>();
		String sql = "SELECT   cc.TABLE_NO,cc.TABLE_INFO_ID,cc.ID,bb.TABLE_NO AS Check_TABLE_NO,bb.TABLE_INFO_ID AS Check_TABLE_INFO_ID, "
				+ " bb.ID AS Check_ID,cc.PRODUCTID,cc.PART_CODE,dd.NAME,bb.RESULT,cc.DEPTID,bb.CHECK_DEPTID,cc.INSPECT_DATE,bb.CHECK_DATE "
				+ " FROM   QUALITYMAN_MANU_INSPECTS cc LEFT JOIN QUALITYMAN_MANU_CHECKS bb ON cc.PART_CODE = bb.PART_CODE AND bb.VALID = 1 AND bb.STATUS = 99 "
				+ " LEFT JOIN LIMSBASIC_STANDS dd ON dd.ID = bb.STANDID WHERE  cc.VALID = 1 AND cc.PART_CODE =? AND cc.status=99";
        SQLQuery query = produceTaskManageDao.createNativeQuery(sql,batchNum);
		List<Object[]> list_produceTaskManage=query.list();
		if(list_produceTaskManage!=null && !list_produceTaskManage.isEmpty()){
			for (Object objs[] : list_produceTaskManage) {
				//
				WOMCheckManage changeManage = new WOMCheckManage();
				//请检单编码
				if(objs[0]!=null && !"".equals(String.valueOf(objs[0]))){
					changeManage.setInspectName(String.valueOf(objs[0]));
				}
				//请检单url
				if(objs[1]!=null && !"".equals(String.valueOf(objs[1])) && objs[2]!=null && !"".equals(String.valueOf(objs[2]))){
					changeManage.setInspectUrl(batchDealService.getOpenViewUrl("qualityMan_7.5.0.0_manuInspect_inspectView","qualityMan_7.5.0.0_manuInspect_inspectList", Long.parseLong(String.valueOf(objs[1])), Long.parseLong(String.valueOf(objs[2]))));
				}
				//报告单编码
				if(objs[3]!=null && !"".equals(String.valueOf(objs[3]))){
					changeManage.setChenkReportName(String.valueOf(objs[3]));
				}
				//报告单url
				if(objs[4]!=null && !"".equals(String.valueOf(objs[4])) && objs[5]!=null && !"".equals(String.valueOf(objs[5]))){
					changeManage.setChenkReportUrl(batchDealService.getOpenViewUrl("qualityMan_7.5.0.0_manuCheck_checkView","qualityMan_7.5.0.0_manuCheck_checkList", Long.parseLong(String.valueOf(objs[4])), Long.parseLong(String.valueOf(objs[5]))));
				}
				//物品id
				if(objs[6]!=null && !"".equals(String.valueOf(objs[6]))){
					changeManage.setProduct(productService.getProduct(Long.parseLong(String.valueOf(objs[6]))));
				}
				//批号
				if(objs[7]!=null && !"".equals(String.valueOf(objs[7]))){
					changeManage.setBatchNum(String.valueOf(objs[7]));
				}
				//质量标准
				if(objs[8]!=null && !"".equals(String.valueOf(objs[8]))){
					changeManage.setStandard(String.valueOf(objs[8]));
				}
				//检验结论
				if(objs[9]!=null && !"".equals(String.valueOf(objs[9]))){
					changeManage.setResult(String.valueOf(objs[9]));
				}
				//请检部门
				if(objs[10]!=null && !"".equals(String.valueOf(objs[10]))){
					changeManage.setInspectDept(departmentService.get(Long.parseLong(String.valueOf(objs[10]))));
				}
				//检验部门
				if(objs[11]!=null && !"".equals(String.valueOf(objs[11]))){
					changeManage.setCheckDept(departmentService.get(Long.parseLong(String.valueOf(objs[11]))));
				}
				//请检时间
				if(objs[12]!=null && !"".equals(String.valueOf(objs[12]))){
					changeManage.setInspectTime((Date)objs[12]);
				}
				//检验时间
				if(objs[13]!=null && !"".equals(String.valueOf(objs[13]))){
					changeManage.setCheckTime((Date)objs[13]);
				}
				putInManageList.add(changeManage);
			}
		}
		return putInManageList;
	...
	@Override
	public void taskEnd(WOMProduceTask produceTask) {
		produceTask.setTaskState((SystemCode) systemCodeService.getSystemCode("womTaskState/04"));
