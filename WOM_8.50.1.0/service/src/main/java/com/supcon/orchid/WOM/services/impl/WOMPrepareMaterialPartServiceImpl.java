...
import com.supcon.orchid.WOM.services.WOMPrepareMaterialPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMPrepareMaterialPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_prepareMaterialPartService")
@Transactional
public class WOMPrepareMaterialPartServiceImpl  extends BaseServiceImpl<WOMPrepareMaterialPart> implements WOMPrepareMaterialPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMPrepareMaterialPart> bapBaseService;
	...
	@Autowired
	private WOMPrepareMaterialPartImportService prepareMaterialPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMPrepareMaterialPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  prepareMaterialPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMPrepareMaterialPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = prepareMaterialPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMPrepareMaterialPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMPrepareMaterialPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  prepareMaterialPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMPrepareMaterialPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMPrepareMaterialPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMPrepareMaterialPart> list = (List<WOMPrepareMaterialPart>) prepareMaterialPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMPrepareMaterialPart getPrepareMaterialPart(long id){
		return getPrepareMaterialPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPrepareMaterialPart getPrepareMaterialPart(long id, String viewCode){
		WOMPrepareMaterialPart prepareMaterialPart = prepareMaterialPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPrepareMaterialPartAsJSON(long id, String include){
		WOMPrepareMaterialPart prepareMaterialPart = prepareMaterialPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(prepareMaterialPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart){
		if(null != prepareMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPrepareMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
			AuditUtil.setColumnStringA(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
			AuditUtil.setColumnStringC(prepareMaterialPart.getId().toString());
		}
		beforeDeletePrepareMaterialPart(prepareMaterialPart);
		prepareMaterialPartDao.deletePhysical(prepareMaterialPart);
		afterDeletePrepareMaterialPart(prepareMaterialPart);
		prepareMaterialPartDao.flush();
		List<WOMPrepareMaterialPart> params = new ArrayList<WOMPrepareMaterialPart>();
		params.add(prepareMaterialPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePrepareMaterialPart(List<Long> prepareMaterialPartIds){
		deletePrepareMaterialPart(prepareMaterialPartIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePrepareMaterialPart(List<Long> prepareMaterialPartIds, String eventTopic) {
		List<WOMPrepareMaterialPart> prepareMaterialParts = new ArrayList<WOMPrepareMaterialPart>();
		for(Long prepareMaterialPartId : prepareMaterialPartIds){
			WOMPrepareMaterialPart prepareMaterialPart = getPrepareMaterialPart(prepareMaterialPartId);
			prepareMaterialParts.add(prepareMaterialPart);
			if(null != prepareMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPrepareMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == prepareMaterialPart.getId() ? "" : "," + prepareMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == prepareMaterialPart.getId() ? "" : "," + prepareMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == prepareMaterialPart.getId() ? "" : "," + prepareMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(prepareMaterialParts != null && prepareMaterialParts.size() > 0 ){
			for(WOMPrepareMaterialPart prepareMaterialPart : prepareMaterialParts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMPrepareMaterialPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(prepareMaterialPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),prepareMaterialPart, prepareMaterialPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(prepareMaterialPartIds != null && prepareMaterialPartIds.size() > 0) {
			String hql = "delete " + WOMPrepareMaterialPart.JPA_NAME + " where id in(:ids)";
			Query query = prepareMaterialPartDao.createQuery(hql);
			query.setParameterList("ids", prepareMaterialPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMPrepareMaterialPart prepareMaterialPartz : prepareMaterialParts){
				prepareMaterialPartz.setValid(false);
				List<WOMPrepareMaterialPart> params = new ArrayList<WOMPrepareMaterialPart>();
				params.add(prepareMaterialPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_prepareMaterial");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePrepareMaterialPart(long prepareMaterialPartId, int prepareMaterialPartVersion){
		this.deletePrepareMaterialPart(prepareMaterialPartId, prepareMaterialPartVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePrepareMaterialPart(long prepareMaterialPartId, int prepareMaterialPartVersion,SignatureLog signatureLog){
		WOMPrepareMaterialPart prepareMaterialPart = getPrepareMaterialPart(prepareMaterialPartId);
		...
		beforeDeletePrepareMaterialPart(prepareMaterialPart);
		prepareMaterialPartDao.delete(prepareMaterialPartId, prepareMaterialPartVersion);
		afterDeletePrepareMaterialPart(prepareMaterialPart);
		prepareMaterialPartDao.flush();
		List<WOMPrepareMaterialPart> params = new ArrayList<WOMPrepareMaterialPart>();
		params.add(prepareMaterialPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=prepareMaterialPart.getId();
			signatureLog.setTableId(prepareMaterialPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePrepareMaterialPart(String prepareMaterialPartIds){
		this.deletePrepareMaterialPart(prepareMaterialPartIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePrepareMaterialPart(String prepareMaterialPartIds,SignatureLog signatureLog){
		deleteCollection(prepareMaterialPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePrepareMaterialPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_prepareMaterial#modelCode:WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="ec.engine.restore",operType="10")
	public void restorePrepareMaterialPart(String prepareMaterialPartIds){
		restoreCollection(prepareMaterialPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePrepareMaterialPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="ec.engine.restore",operType="10")
	public void restorePrepareMaterialPart(long prepareMaterialPartId){
		
		findBusinessKeyUsed(prepareMaterialPartId);	//判断业务主键是否重复
		...
		beforeRestorePrepareMaterialPart(prepareMaterialPart);
		prepareMaterialPart.setValid(true);
		prepareMaterialPartDao.update(prepareMaterialPart);
		afterRestorePrepareMaterialPart(prepareMaterialPart);
		List<WOMPrepareMaterialPart> params = new ArrayList<WOMPrepareMaterialPart>();
		params.add(prepareMaterialPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long prepareMaterialPartId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMPrepareMaterialPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMPrepareMaterialPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  prepareMaterialPartDao.createNativeQuery(sql,prepareMaterialPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePrepareMaterialPart(List<WOMPrepareMaterialPart>  prepareMaterialParts){
		for(WOMPrepareMaterialPart prepareMaterialPart:prepareMaterialParts)  {
			savePrepareMaterialPart(prepareMaterialPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePrepareMaterialPart(List<WOMPrepareMaterialPart>  prepareMaterialParts){
		for(WOMPrepareMaterialPart prepareMaterialPart:prepareMaterialParts)  {
			savePrepareMaterialPart(prepareMaterialPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="ec.flow.save")
	public void savePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePrepareMaterialPart(prepareMaterialPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="ec.flow.save")
	public void savePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePrepareMaterialPart(prepareMaterialPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePrepareMaterialPartCodes(WOMPrepareMaterialPart prepareMaterialPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePrepareMaterialPartCodes(prepareMaterialPart, false);
	}
	
	public void generatePrepareMaterialPartCodes(WOMPrepareMaterialPart prepareMaterialPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePrepareMaterialPartSummarys(WOMPrepareMaterialPart prepareMaterialPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePrepareMaterialPartSummarys(prepareMaterialPart, false);
	}

	public void generatePrepareMaterialPartSummarys(WOMPrepareMaterialPart prepareMaterialPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart")
	public void savePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePrepareMaterialPart(prepareMaterialPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart")
	public void savePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_prepareMaterial");
		String url = null;
		if(prepareMaterialPart.getId() != null && prepareMaterialPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPrepareMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, prepareMaterialPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(prepareMaterialPart);
		beforeSavePrepareMaterialPart(prepareMaterialPart, viewIsView);

		if (viewIsView) {
			prepareMaterialPartDao.saveWithRevertVersion(prepareMaterialPart);
		} else {
			if(isNew)
				prepareMaterialPartDao.save(prepareMaterialPart);
			else
				prepareMaterialPartDao.merge(prepareMaterialPart);
		}
		if(null != prepareMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPrepareMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
			AuditUtil.setColumnStringA(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
		...
		dealDatagridsSave(prepareMaterialPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePrepareMaterialPartCodes(prepareMaterialPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePrepareMaterialPartSummarys(prepareMaterialPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePrepareMaterialPart(prepareMaterialPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, prepareMaterialPart);
			}
		}
		List<WOMPrepareMaterialPart> params = new ArrayList<WOMPrepareMaterialPart>();
		prepareMaterialPartDao.flush();
		prepareMaterialPartDao.clear();
		prepareMaterialPart = prepareMaterialPartDao.load(prepareMaterialPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMPrepareMaterialPart prepareMaterialPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("prepareMaterialPart", prepareMaterialPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart")
	public void mergePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(prepareMaterialPart);
		beforeSavePrepareMaterialPart(prepareMaterialPart);
		prepareMaterialPartDao.merge(prepareMaterialPart);
		if(null != prepareMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPrepareMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
			AuditUtil.setColumnStringA(null == prepareMaterialPart.getId() ? "" : prepareMaterialPart.getId().toString());
		...
		dealDatagridsSave(prepareMaterialPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePrepareMaterialPart(prepareMaterialPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMPrepareMaterialPart> findPrepareMaterialParts(Page<WOMPrepareMaterialPart> page, Criterion... criterions) {
		return prepareMaterialPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPrepareMaterialParts(Page<WOMPrepareMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPrepareMaterialParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPrepareMaterialParts(Page<WOMPrepareMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPrepareMaterialParts(Page<WOMPrepareMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMPrepareMaterialPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"prepareMaterialPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"prepareMaterialPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"prepareMaterialPart\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"prepareMaterialPart\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PREPARE_MATERIALS \"prepareMaterial\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_prepareMaterial" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  prepareMaterialPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  prepareMaterialPartDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_prepareMaterial").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = dataPermissionService.getPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prepareMaterial\"","WOM_7.5.0.0_prepareMaterial","","WOM_PREPARE_MATERIALS",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"prepareMaterialPart\".LAY_REC ASC, \"prepareMaterialPart\".SORT ASC");
				} else {
					orderPart.append(" \"prepareMaterialPart\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) prepareMaterialPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=prepareMaterialPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = prepareMaterialPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMPrepareMaterialPart> result = new ArrayList<WOMPrepareMaterialPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMPrepareMaterialPart.class,prepareMaterialPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMPrepareMaterialPart.class,prepareMaterialPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMPrepareMaterialPart prepareMaterialPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(prepareMaterialPart.getId(), "WOM_prepareMaterial_prepareMaterialPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(prepareMaterialPart.getId(), "WOM_prepareMaterial_prepareMaterialPart");
							if(null != documents && !documents.isEmpty()) {
								prepareMaterialPart.setDocument(documents.get(0));
								prepareMaterialPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMPrepareMaterialPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMPrepareMaterialPart> result) {
		View view = null;
		...
		Map<Long, WOMPrepareMaterialPart> rsMap = new HashMap<Long, WOMPrepareMaterialPart>();
		for (WOMPrepareMaterialPart prepareMaterialPart : result) {
			ids.add(prepareMaterialPart.getId());
			rsMap.put(prepareMaterialPart.getId(), prepareMaterialPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(prepareMaterialPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = prepareMaterialPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  prepareMaterialPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prepareMaterialPart\"","WOM_7.5.0.0_prepareMaterial","","WOM_PREPARE_MATERIAL_PARTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"prepareMaterialPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"prepareMaterialPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prepareMaterialPart\"","WOM_7.5.0.0_prepareMaterial","","WOM_PREPARE_MATERIAL_PARTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("prepareMaterialPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"prepareMaterialPart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"prepareMaterialPart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"prepareMaterialPart\".CID = 1 OR \"prepareMaterialPart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("prepareMaterialPart.mc\"").append(".PREPARE_MATERIAL_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = prepareMaterialPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=prepareMaterialPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMPrepareMaterialPart.ENTITY_CODE;}
	protected String getModuleCode(){return WOMPrepareMaterialPart.MODULE_CODE;}
	protected String getModelCode(){return WOMPrepareMaterialPart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param prepareMaterialPart 模型实例
	 */
	public void checkUniqueConstraint(WOMPrepareMaterialPart prepareMaterialPart){
	}
	
	
	@Override
	public List<WOMPrepareMaterialPart> getPrepareMaterialParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPrepareMaterialPart.class);
		if(params!=null){
			...
			return prepareMaterialPartDao.findByHql("from " + WOMPrepareMaterialPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return prepareMaterialPartDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMPrepareMaterialPart> findPrepareMaterialPartsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPrepareMaterialPart.class);
	...
	@Override
	public List<WOMPrepareMaterialPart> findPrepareMaterialPartsByHql(String hql, Object... objects){
		
		return   prepareMaterialPartDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMPrepareMaterialPart loadPrepareMaterialPartByBussinessKey(WOMPrepareMaterialPart prepareMaterialPart){
		return loadPrepareMaterialPartByBussinessKey(prepareMaterialPart.getId());
	}
	
	@Override
	public Page<WOMPrepareMaterialPart> getPrepareMaterialParts(Page<WOMPrepareMaterialPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPrepareMaterialPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = prepareMaterialPartDao.findByPage(page, sql);
			}else{
				page = prepareMaterialPartDao.findAllByPage(page);
			}
			return page;
		}
		page = prepareMaterialPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMPrepareMaterialPart> findByTreeDataGridPrepareMaterial(List<WOMPrepareMaterialPart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMPrepareMaterialPart prepareMaterialPart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridPrepareMaterial(Page<WOMPrepareMaterialPart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMPrepareMaterialPart> prepareMaterialParts = dgPage.getResult();
			for(WOMPrepareMaterialPart prepareMaterialPart : prepareMaterialParts){
				
			}
			dgPage.setResult(prepareMaterialParts);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPrepareMaterialPart loadPrepareMaterialPartByBussinessKey(Serializable bussinessKey){
		return prepareMaterialPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMPrepareMaterialPart> findByProperty(String propertyName, Object object){
		return prepareMaterialPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPrepareMaterialPart findEntityByProperty(String propertyName, Object object){
		return prepareMaterialPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMPrepareMaterialPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = prepareMaterialPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart","runtime");
		try {
	...
	public void dealDatagridsSave(WOMPrepareMaterialPart prepareMaterialPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,prepareMaterialPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMPrepareMaterialPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, prepareMaterialPart);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(prepareMaterialPart.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(prepareMaterialPart.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(prepareMaterialPart.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",prepareMaterialPart.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMPrepareMaterialPart.TABLE_NAME +" where valid = 1";
			list = prepareMaterialPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMPrepareMaterialPart.TABLE_NAME;
			list = prepareMaterialPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMPrepareMaterialPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = prepareMaterialPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMPrepareMaterialPartDaoImpl prepareMaterialPartDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMPrepareMaterialPart.TABLE_NAME + " where id = ?";
		String tableNo = prepareMaterialPartDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchPrepareMaterialPart(final List<WOMPrepareMaterialPart> insertObjs, final List<WOMPrepareMaterialPart> updateObjs, 
		...
		return this.importBatchPrepareMaterialPart(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPrepareMaterialPart(final List<WOMPrepareMaterialPart> insertObjs, final List<WOMPrepareMaterialPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1489976792140"),importCount));
		...
		Map<Long, WOMPrepareMaterialPart> oldUpdateObjsMap = new HashMap<Long, WOMPrepareMaterialPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMPrepareMaterialPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMPrepareMaterialPart> bizObjList = prepareMaterialPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMPrepareMaterialPart bizObj = bizObjList.get(mIndex);
		...
		final List<WOMPrepareMaterialPart> nmeObjs = new ArrayList<WOMPrepareMaterialPart>();
		...
		//String idSql = "select id from " + WOMPrepareMaterialPart.TABLE_NAME + " order by id desc";
		//Query query = prepareMaterialPartDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMPrepareMaterialPart.JPA_NAME + " order by id desc";
		Query query =  prepareMaterialPartDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMPrepareMaterialPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMPrepareMaterialPart prepareMaterialPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMPrepareMaterialPart.TABLE_NAME,1,dbId);
			if(prepareMaterialPart.getId() != null ){
				m.put(prepareMaterialPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) prepareMaterialPartDao.createNativeQuery(sql, "WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		prepareMaterialPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMPrepareMaterialPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMPrepareMaterialPart prepareMaterialPart = updateObjs.get(updateIndex);
						Long id = prepareMaterialPart.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = prepareMaterialPart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(prepareMaterialPart);
							...
							nmeObjs.add(prepareMaterialPart);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMPrepareMaterialPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMPrepareMaterialPart prepareMaterialPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = prepareMaterialPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		prepareMaterialPartDaoImpl.getSession().flush();
		prepareMaterialPartDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = prepareMaterialPartDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = prepareMaterialPartDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMPrepareMaterialPart bizObj = insertObjs.get(insertIndex);
			prepareMaterialPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMPrepareMaterialPart bizObj = updateObjs.get(updateIndex);
			WOMPrepareMaterialPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			prepareMaterialPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMPrepareMaterialPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMPrepareMaterialPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(linkMap != null && linkMap.get("needChange")!=null && linkMap.get("needChange").equals("no!!")){
				...
				s.append(" FROM WOM_PREPARE_MATERIALS \"prepareMaterial\" ");
			}else{
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) prepareMaterialPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMPrepareMaterialPart> result = getResult(page, query, new PendingResultTransformer(WOMPrepareMaterialPart.class,prepareMaterialPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMPrepareMaterialPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = prepareMaterialPartDao.createNativeQuery(sql, "WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = prepareMaterialPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = prepareMaterialPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = prepareMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?prepareMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  prepareMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,prepareMaterialPartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="ec.flow.save")
	public void savePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, DataGridService dataGridService){
		savePrepareMaterialPart(prepareMaterialPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="ec.flow.save")
	public void savePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, DataGridService dataGridService, String viewCode){
		savePrepareMaterialPart(prepareMaterialPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart")
	public void savePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePrepareMaterialPart(prepareMaterialPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart")
	public void savePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_prepareMaterial";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_prepareMaterial");
		String url = null;
		if(null != prepareMaterialPart.getId() && prepareMaterialPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPrepareMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart")
	public Map<Object, Object> prepareMaterialPartDataGridImport(WOMPrepareMaterialPart prepareMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = prepareMaterialPart.getVirtualId();
		
		prepareMaterialPart = prepareMaterialPartDao.load(prepareMaterialPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, prepareMaterialPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = prepareMaterialPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(prepareMaterialPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, prepareMaterialPart.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart")
	public void mergePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(prepareMaterialPart);
		beforeSavePrepareMaterialPart(prepareMaterialPart);
		prepareMaterialPartDao.merge(prepareMaterialPart);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_prepareMaterial",model="WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart",desc="foundation.batch.save",operType="4")
	public void batchSavePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_prepareMaterial";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_prepareMaterial");
	...
	@Override
	public Page<WOMPrepareMaterialPart> getByPage(Page<WOMPrepareMaterialPart> page,DetachedCriteria detachedCriteria){
		return prepareMaterialPartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSavePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, Object... objects){
		checkUniqueConstraint(prepareMaterialPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPrepareMaterialPart(List<WOMPrepareMaterialPart> prepareMaterialParts, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPrepareMaterialPart(List<WOMPrepareMaterialPart> prepareMaterialParts){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPrepareMaterialPart(List<WOMPrepareMaterialPart> prepareMaterialParts, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPrepareMaterialPart(List<WOMPrepareMaterialPart> prepareMaterialParts){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePrepareMaterialPart(WOMPrepareMaterialPart prepareMaterialPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMPrepareMaterialPart prepareMaterialPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMPrepareMaterialPart prepareMaterialPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPrepareMaterialPart(List<WOMPrepareMaterialPart> insertObjs, List<WOMPrepareMaterialPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPrepareMaterialPart,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPrepareMaterialPart(List<WOMPrepareMaterialPart> insertObjs, List<WOMPrepareMaterialPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPrepareMaterialPart,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_prepareMaterial_PrepareMaterialPart,WOM_7.5.0.0) */
// 自定义代码
