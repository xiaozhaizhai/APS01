...
import com.supcon.orchid.WOM.services.WOMInspectImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMInspectDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_inspectService")
@Transactional
public class WOMInspectServiceImpl  extends BaseServiceImpl<WOMInspect> implements WOMInspectService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMInspect> bapBaseService;
	...
	@Autowired
	private WOMInspectImportService inspectImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMInspect.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  inspectDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMInspect.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = inspectDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMInspect.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMInspect.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  inspectDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMInspect.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMInspect.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMInspect> list = (List<WOMInspect>) inspectDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInspect getInspect(long id){
		return getInspect(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInspect getInspect(long id, String viewCode){
		WOMInspect inspect = inspectDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getInspectAsJSON(long id, String include){
		WOMInspect inspect = inspectDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(inspect, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.common.delete",operType="3")
	public void deleteInspect(WOMInspect inspect){
		if(null != inspect && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inspect.getId() ? "" : inspect.getId().toString());
			AuditUtil.setColumnStringA(null == inspect.getId() ? "" : inspect.getId().toString());
			AuditUtil.setColumnStringC(inspect.getId().toString());
		}
		beforeDeleteInspect(inspect);
		inspectDao.deletePhysical(inspect);
		afterDeleteInspect(inspect);
		inspectDao.flush();
		List<WOMInspect> params = new ArrayList<WOMInspect>();
		params.add(inspect);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.common.delete",operType="3")
	public void deleteInspect(List<Long> inspectIds){
		deleteInspect(inspectIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.common.delete",operType="3")
	public void deleteInspect(List<Long> inspectIds, String eventTopic) {
		List<WOMInspect> inspects = new ArrayList<WOMInspect>();
		for(Long inspectId : inspectIds){
			WOMInspect inspect = getInspect(inspectId);
			inspects.add(inspect);
			if(null != inspect && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == inspect.getId() ? "" : "," + inspect.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == inspect.getId() ? "" : inspect.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == inspect.getId() ? "" : "," + inspect.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == inspect.getId() ? "" : inspect.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == inspect.getId() ? "" : "," + inspect.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == inspect.getId() ? "" : inspect.getId().toString());
				}
			}
		}
		
		for(WOMInspect inspect : inspects){
			beforeDeleteInspect(inspect);
		}
		...
		if(inspects != null && inspects.size() > 0 ){
			for(WOMInspect inspect : inspects){
				...
				java.lang.reflect.Field[] fields = WOMInspect.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(inspect));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),inspect, inspect.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(inspectIds != null && inspectIds.size() > 0) {
			String hql = "delete " + WOMInspect.JPA_NAME + " where id in(:ids)";
			Query query = inspectDao.createQuery(hql);
			query.setParameterList("ids", inspectIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMInspect inspectz : inspects){
				inspectz.setValid(false);
				List<WOMInspect> params = new ArrayList<WOMInspect>();
				params.add(inspectz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.common.delete",operType="3")
	public void deleteInspect(long inspectId, int inspectVersion){
		this.deleteInspect(inspectId, inspectVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.common.delete",operType="3")
	public void deleteInspect(long inspectId, int inspectVersion,SignatureLog signatureLog){
		WOMInspect inspect = getInspect(inspectId);
		...
		beforeDeleteInspect(inspect);
		inspectDao.delete(inspectId, inspectVersion);
		afterDeleteInspect(inspect);
		inspectDao.flush();
		List<WOMInspect> params = new ArrayList<WOMInspect>();
		params.add(inspect);
		...
		if(signatureLog != null ) {
			...
			businessKey=inspect.getId();
			signatureLog.setTableId(inspect.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.common.delete",operType="3")
	public void deleteInspect(String inspectIds){
		this.deleteInspect(inspectIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.common.delete",operType="3")
	public void deleteInspect(String inspectIds,SignatureLog signatureLog){
		deleteCollection(inspectIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteInspect(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_generalManage#modelCode:WOM_7.5.0.0_generalManage_Inspect#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="ec.engine.restore",operType="10")
	public void restoreInspect(String inspectIds){
		restoreCollection(inspectIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreInspect(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="ec.engine.restore",operType="10")
	public void restoreInspect(long inspectId){
		
		findBusinessKeyUsed(inspectId);	//判断业务主键是否重复
		...
		beforeRestoreInspect(inspect);
		inspect.setValid(true);
		inspectDao.update(inspect);
		afterRestoreInspect(inspect);
		List<WOMInspect> params = new ArrayList<WOMInspect>();
		params.add(inspect);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long inspectId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_generalManage_Inspect");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMInspect.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMInspect.TABLE_NAME +" where id = ? )";
			List<Object> list =  inspectDao.createNativeQuery(sql,inspectId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseInspect(List<WOMInspect>  inspects){
		for(WOMInspect inspect:inspects)  {
			saveInspect(inspect, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseInspect(List<WOMInspect>  inspects){
		for(WOMInspect inspect:inspects)  {
			saveInspect(inspect, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="ec.flow.save")
	public void saveInspect(WOMInspect inspect, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveInspect(inspect, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="ec.flow.save")
	public void saveInspect(WOMInspect inspect, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveInspect(inspect, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateInspectCodes(WOMInspect inspect) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateInspectCodes(inspect, false);
	}
	
	public void generateInspectCodes(WOMInspect inspect, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateInspectSummarys(WOMInspect inspect) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateInspectSummarys(inspect, false);
	}

	public void generateInspectSummarys(WOMInspect inspect, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect")
	public void saveInspect(WOMInspect inspect,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveInspect(inspect, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect")
	public void saveInspect(WOMInspect inspect,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		if(inspect.getId() != null && inspect.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, inspect);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(inspect);
		beforeSaveInspect(inspect, viewIsView);

		if (viewIsView) {
			inspectDao.saveWithRevertVersion(inspect);
		} else {
			if(isNew)
				inspectDao.save(inspect);
			else
				inspectDao.merge(inspect);
		}
		if(null != inspect && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inspect.getId() ? "" : inspect.getId().toString());
			AuditUtil.setColumnStringA(null == inspect.getId() ? "" : inspect.getId().toString());
		...
		dealDatagridsSave(inspect,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateInspectCodes(inspect, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateInspectSummarys(inspect, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveInspect(inspect, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, inspect);
			}
		}
		List<WOMInspect> params = new ArrayList<WOMInspect>();
		inspectDao.flush();
		inspectDao.clear();
		inspect = inspectDao.load(inspect.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMInspect inspect){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect")
	public void mergeInspect(WOMInspect inspect, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(inspect);
		beforeSaveInspect(inspect);
		inspectDao.merge(inspect);
		if(null != inspect && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inspect.getId() ? "" : inspect.getId().toString());
			AuditUtil.setColumnStringA(null == inspect.getId() ? "" : inspect.getId().toString());
		...
		dealDatagridsSave(inspect,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveInspect(inspect);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMInspect> findInspects(Page<WOMInspect> page, Criterion... criterions) {
		return inspectDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInspects(Page<WOMInspect> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findInspects(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInspects(Page<WOMInspect> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInspects(Page<WOMInspect> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMInspect> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"inspect\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"inspect\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"inspect\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  inspectDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  inspectDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_generalManage").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"inspect\".LAY_REC ASC, \"inspect\".SORT ASC");
				} else {
					orderPart.append(" \"inspect\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) inspectDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=inspectDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = inspectDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMInspect> result = new ArrayList<WOMInspect>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMInspect.class,inspectDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMInspect.class,inspectDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMInspect inspect : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(inspect.getId(), "WOM_generalManage_inspect");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(inspect.getId(), "WOM_generalManage_inspect");
							if(null != documents && !documents.isEmpty()) {
								inspect.setDocument(documents.get(0));
								inspect.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMInspect> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMInspect> result) {
		View view = null;
		...
		Map<Long, WOMInspect> rsMap = new HashMap<Long, WOMInspect>();
		for (WOMInspect inspect : result) {
			ids.add(inspect.getId());
			rsMap.put(inspect.getId(), inspect);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(inspectDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = inspectDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_generalManage_Inspect" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_generalManage_Inspect" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  inspectDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"inspect\"","WOM_7.5.0.0_generalManage","","WOM_INSPECTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"inspect\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"inspect\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"inspect\"","WOM_7.5.0.0_generalManage","","WOM_INSPECTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_generalManage_Inspect" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("inspect.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"inspect\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("inspect.mc\"").append(".INSPECT) not in ("+notinValue+")");
			}
			
			SQLQuery query = inspectDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=inspectDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMInspect.ENTITY_CODE;}
	protected String getModuleCode(){return WOMInspect.MODULE_CODE;}
	protected String getModelCode(){return WOMInspect.MODEL_CODE;}
	 ...
	 * @param inspect 模型实例
	 */
	public void checkUniqueConstraint(WOMInspect inspect){
	}
	
	
	@Override
	public List<WOMInspect> getInspects(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMInspect.class);
		if(params!=null){
			...
			return inspectDao.findByHql("from " + WOMInspect.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return inspectDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMInspect> findInspectsByHql(String hql, Object... objects){
		...
		return   inspectDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMInspect loadInspectByBussinessKey(WOMInspect inspect){
		return loadInspectByBussinessKey(inspect.getId());
	}
	
	@Override
	public Page<WOMInspect> getInspects(Page<WOMInspect> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMInspect.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = inspectDao.findByPage(page, sql);
			}else{
				page = inspectDao.findAllByPage(page);
			}
			return page;
		}
		page = inspectDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public WOMInspect loadInspectByBussinessKey(Serializable bussinessKey){
		return inspectDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMInspect> findByProperty(String propertyName, Object object){
		return inspectDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInspect findEntityByProperty(String propertyName, Object object){
		return inspectDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMInspect.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = inspectDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_generalManage_Inspect","runtime");
		try {
	...
	public void dealDatagridsSave(WOMInspect inspect,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,inspectDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMInspect.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, inspect);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",inspect.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMInspect.TABLE_NAME +" where valid = 1";
			list = inspectDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMInspect.TABLE_NAME;
			list = inspectDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMInspect.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = inspectDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMInspectDaoImpl inspectDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchInspect(final List<WOMInspect> insertObjs, final List<WOMInspect> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchInspect(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchInspect(final List<WOMInspect> insertObjs, final List<WOMInspect> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1502698759360"),importCount));
		...
		Map<Long, WOMInspect> oldUpdateObjsMap = new HashMap<Long, WOMInspect>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMInspect bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMInspect> bizObjList = inspectDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMInspect bizObj = bizObjList.get(mIndex);
		...
		final List<WOMInspect> nmeObjs = new ArrayList<WOMInspect>();
		...
		long id=autoGeneratorID.getNextId(WOMInspect.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMInspect inspect:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMInspect.TABLE_NAME,1,dbId);
			if(inspect.getId() != null ){
				m.put(inspect.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) inspectDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_Inspect"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		inspectDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMInspect.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMInspect inspect = updateObjs.get(updateIndex);
						Long id = inspect.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = inspect.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMInspect.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMInspect.TABLE_NAME + " order by id desc";
//					Query query = inspectDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMInspect inspect = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = inspect.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		inspectDaoImpl.getSession().flush();
		inspectDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMInspect bizObj = insertObjs.get(insertIndex);
			inspectImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMInspect bizObj = updateObjs.get(updateIndex);
			WOMInspect oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			inspectImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMInspect> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMInspect> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) inspectDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMInspect> result = getResult(page, query, new PendingResultTransformer(WOMInspect.class,inspectDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMInspect model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = inspectDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_Inspect").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = inspectDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = inspectDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = inspectDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?inspectDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  inspectDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,inspectDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="ec.flow.save")
	public void saveInspect(WOMInspect inspect, DataGridService dataGridService){
		saveInspect(inspect, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="ec.flow.save")
	public void saveInspect(WOMInspect inspect, DataGridService dataGridService, String viewCode){
		saveInspect(inspect, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect")
	public void saveInspect(WOMInspect inspect, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveInspect(inspect, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect")
	public void saveInspect(WOMInspect inspect, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(null != inspect.getId() && inspect.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInspect.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect")
	public Map<Object, Object> inspectDataGridImport(WOMInspect inspect, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = inspect.getVirtualId();
		
		inspect = inspectDao.load(inspect.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, inspect.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = inspect.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(inspect);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, inspect.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect")
	public void mergeInspect(WOMInspect inspect, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(inspect);
		beforeSaveInspect(inspect);
		inspectDao.merge(inspect);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_Inspect",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMInspect> getByPage(Page<WOMInspect> page,DetachedCriteria detachedCriteria){
		return inspectDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveInspect(WOMInspect inspect, Object... objects){
		checkUniqueConstraint(inspect);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveInspect(WOMInspect inspect, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteInspect(WOMInspect inspect){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteInspect(WOMInspect inspect){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreInspect(WOMInspect inspect){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreInspect(WOMInspect inspect){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMInspect inspect, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMInspect inspect, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportInspect(List<WOMInspect> insertObjs, List<WOMInspect> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportInspect,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportInspect(List<WOMInspect> insertObjs, List<WOMInspect> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportInspect,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_generalManage_Inspect,WOM_7.5.0.0) */
// 自定义代码
