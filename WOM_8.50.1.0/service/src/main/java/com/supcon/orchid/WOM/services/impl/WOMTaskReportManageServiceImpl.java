...
import com.supcon.orchid.WOM.services.WOMTaskReportManageImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMTaskReportManageDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_taskReportManageService")
@Transactional
public class WOMTaskReportManageServiceImpl  extends BaseServiceImpl<WOMTaskReportManage> implements WOMTaskReportManageService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMTaskReportManage> bapBaseService;
	...
	@Autowired
	private WOMTaskReportManageImportService taskReportManageImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMTaskReportManage.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  taskReportManageDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMTaskReportManage.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = taskReportManageDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMTaskReportManage.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMTaskReportManage.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  taskReportManageDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMTaskReportManage.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMTaskReportManage.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMTaskReportManage> list = (List<WOMTaskReportManage>) taskReportManageDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMTaskReportManage getTaskReportManage(long id){
		return getTaskReportManage(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskReportManage getTaskReportManage(long id, String viewCode){
		WOMTaskReportManage taskReportManage = taskReportManageDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getTaskReportManageAsJSON(long id, String include){
		WOMTaskReportManage taskReportManage = taskReportManageDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(taskReportManage, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportManage(WOMTaskReportManage taskReportManage){
		if(null != taskReportManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
			AuditUtil.setColumnStringA(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
			AuditUtil.setColumnStringC(taskReportManage.getId().toString());
		}
		beforeDeleteTaskReportManage(taskReportManage);
		taskReportManageDao.deletePhysical(taskReportManage);
		afterDeleteTaskReportManage(taskReportManage);
		taskReportManageDao.flush();
		List<WOMTaskReportManage> params = new ArrayList<WOMTaskReportManage>();
		params.add(taskReportManage);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportManage(List<Long> taskReportManageIds){
		deleteTaskReportManage(taskReportManageIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportManage(List<Long> taskReportManageIds, String eventTopic) {
		List<WOMTaskReportManage> taskReportManages = new ArrayList<WOMTaskReportManage>();
		for(Long taskReportManageId : taskReportManageIds){
			WOMTaskReportManage taskReportManage = getTaskReportManage(taskReportManageId);
			taskReportManages.add(taskReportManage);
			if(null != taskReportManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == taskReportManage.getId() ? "" : "," + taskReportManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == taskReportManage.getId() ? "" : "," + taskReportManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == taskReportManage.getId() ? "" : "," + taskReportManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(taskReportManages != null && taskReportManages.size() > 0 ){
			for(WOMTaskReportManage taskReportManage : taskReportManages){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMTaskReportManage.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(taskReportManage));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),taskReportManage, taskReportManage.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(taskReportManageIds != null && taskReportManageIds.size() > 0) {
			String hql = "delete " + WOMTaskReportManage.JPA_NAME + " where id in(:ids)";
			Query query = taskReportManageDao.createQuery(hql);
			query.setParameterList("ids", taskReportManageIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMTaskReportManage taskReportManagez : taskReportManages){
				taskReportManagez.setValid(false);
				List<WOMTaskReportManage> params = new ArrayList<WOMTaskReportManage>();
				params.add(taskReportManagez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_generalManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportManage(long taskReportManageId, int taskReportManageVersion){
		this.deleteTaskReportManage(taskReportManageId, taskReportManageVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportManage(long taskReportManageId, int taskReportManageVersion,SignatureLog signatureLog){
		WOMTaskReportManage taskReportManage = getTaskReportManage(taskReportManageId);
		...
		beforeDeleteTaskReportManage(taskReportManage);
		taskReportManageDao.delete(taskReportManageId, taskReportManageVersion);
		afterDeleteTaskReportManage(taskReportManage);
		taskReportManageDao.flush();
		List<WOMTaskReportManage> params = new ArrayList<WOMTaskReportManage>();
		params.add(taskReportManage);
		...
		if(signatureLog != null ) {
			...
			businessKey=taskReportManage.getId();
			signatureLog.setTableId(taskReportManage.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportManage(String taskReportManageIds){
		this.deleteTaskReportManage(taskReportManageIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportManage(String taskReportManageIds,SignatureLog signatureLog){
		deleteCollection(taskReportManageIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteTaskReportManage(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_generalManage#modelCode:WOM_7.5.0.0_generalManage_TaskReportManage#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="ec.engine.restore",operType="10")
	public void restoreTaskReportManage(String taskReportManageIds){
		restoreCollection(taskReportManageIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreTaskReportManage(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="ec.engine.restore",operType="10")
	public void restoreTaskReportManage(long taskReportManageId){
		
		findBusinessKeyUsed(taskReportManageId);	//判断业务主键是否重复
		...
		beforeRestoreTaskReportManage(taskReportManage);
		taskReportManage.setValid(true);
		taskReportManageDao.update(taskReportManage);
		afterRestoreTaskReportManage(taskReportManage);
		List<WOMTaskReportManage> params = new ArrayList<WOMTaskReportManage>();
		params.add(taskReportManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long taskReportManageId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_generalManage_TaskReportManage");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMTaskReportManage.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMTaskReportManage.TABLE_NAME +" where id = ? )";
			List<Object> list =  taskReportManageDao.createNativeQuery(sql,taskReportManageId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseTaskReportManage(List<WOMTaskReportManage>  taskReportManages){
		for(WOMTaskReportManage taskReportManage:taskReportManages)  {
			saveTaskReportManage(taskReportManage, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseTaskReportManage(List<WOMTaskReportManage>  taskReportManages){
		for(WOMTaskReportManage taskReportManage:taskReportManages)  {
			saveTaskReportManage(taskReportManage, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="ec.flow.save")
	public void saveTaskReportManage(WOMTaskReportManage taskReportManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveTaskReportManage(taskReportManage, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="ec.flow.save")
	public void saveTaskReportManage(WOMTaskReportManage taskReportManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveTaskReportManage(taskReportManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateTaskReportManageCodes(WOMTaskReportManage taskReportManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTaskReportManageCodes(taskReportManage, false);
	}
	
	public void generateTaskReportManageCodes(WOMTaskReportManage taskReportManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateTaskReportManageSummarys(WOMTaskReportManage taskReportManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTaskReportManageSummarys(taskReportManage, false);
	}

	public void generateTaskReportManageSummarys(WOMTaskReportManage taskReportManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage")
	public void saveTaskReportManage(WOMTaskReportManage taskReportManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveTaskReportManage(taskReportManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage")
	public void saveTaskReportManage(WOMTaskReportManage taskReportManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(taskReportManage.getId() != null && taskReportManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, taskReportManage);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(taskReportManage);
		beforeSaveTaskReportManage(taskReportManage, viewIsView);

		if (viewIsView) {
			taskReportManageDao.saveWithRevertVersion(taskReportManage);
		} else {
			if(isNew)
				taskReportManageDao.save(taskReportManage);
			else
				taskReportManageDao.merge(taskReportManage);
		}
		if(null != taskReportManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
			AuditUtil.setColumnStringA(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
		...
		dealDatagridsSave(taskReportManage,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateTaskReportManageCodes(taskReportManage, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateTaskReportManageSummarys(taskReportManage, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveTaskReportManage(taskReportManage, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, taskReportManage);
			}
		}
		List<WOMTaskReportManage> params = new ArrayList<WOMTaskReportManage>();
		taskReportManageDao.flush();
		taskReportManageDao.clear();
		taskReportManage = taskReportManageDao.load(taskReportManage.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMTaskReportManage taskReportManage){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("taskReportManage", taskReportManage);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage")
	public void mergeTaskReportManage(WOMTaskReportManage taskReportManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(taskReportManage);
		beforeSaveTaskReportManage(taskReportManage);
		taskReportManageDao.merge(taskReportManage);
		if(null != taskReportManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
			AuditUtil.setColumnStringA(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
		...
		dealDatagridsSave(taskReportManage,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveTaskReportManage(taskReportManage);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMTaskReportManage> findTaskReportManages(Page<WOMTaskReportManage> page, Criterion... criterions) {
		return taskReportManageDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskReportManages(Page<WOMTaskReportManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findTaskReportManages(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskReportManages(Page<WOMTaskReportManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskReportManages(Page<WOMTaskReportManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMTaskReportManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"taskReportManage\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"taskReportManage\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"taskReportManage\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"taskReportManage\".PRODUCE_TASK IN ( SELECT ID FROM 指令单 \"produceTaskManage\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_generalManage" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  taskReportManageDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  taskReportManageDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_generalManage").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceTaskManage\"","WOM_7.5.0.0_generalManage","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"taskReportManage\".LAY_REC ASC, \"taskReportManage\".SORT ASC");
				} else {
					orderPart.append(" \"taskReportManage\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) taskReportManageDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=taskReportManageDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = taskReportManageDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMTaskReportManage> result = new ArrayList<WOMTaskReportManage>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMTaskReportManage.class,taskReportManageDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMTaskReportManage.class,taskReportManageDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMTaskReportManage taskReportManage : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(taskReportManage.getId(), "WOM_generalManage_taskReportManage");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(taskReportManage.getId(), "WOM_generalManage_taskReportManage");
							if(null != documents && !documents.isEmpty()) {
								taskReportManage.setDocument(documents.get(0));
								taskReportManage.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMTaskReportManage> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMTaskReportManage> result) {
		View view = null;
		...
		Map<Long, WOMTaskReportManage> rsMap = new HashMap<Long, WOMTaskReportManage>();
		for (WOMTaskReportManage taskReportManage : result) {
			ids.add(taskReportManage.getId());
			rsMap.put(taskReportManage.getId(), taskReportManage);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(taskReportManageDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = taskReportManageDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_generalManage_TaskReportManage" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_generalManage_TaskReportManage" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  taskReportManageDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskReportManage\"","WOM_7.5.0.0_generalManage","","WOM_TASK_REPORT_MENAGE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"taskReportManage\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"taskReportManage\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskReportManage\"","WOM_7.5.0.0_generalManage","","WOM_TASK_REPORT_MENAGE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_generalManage_TaskReportManage" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("taskReportManage.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"taskReportManage\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"taskReportManage\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"taskReportManage\".CID = 1 OR \"taskReportManage\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("taskReportManage.mc\"").append(".TASK_REPORT_MANAGE) not in ("+notinValue+")");
			}
			
			SQLQuery query = taskReportManageDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=taskReportManageDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMTaskReportManage.ENTITY_CODE;}
	protected String getModuleCode(){return WOMTaskReportManage.MODULE_CODE;}
	protected String getModelCode(){return WOMTaskReportManage.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param taskReportManage 模型实例
	 */
	public void checkUniqueConstraint(WOMTaskReportManage taskReportManage){
	}
	
	
	@Override
	public List<WOMTaskReportManage> getTaskReportManages(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskReportManage.class);
		if(params!=null){
			...
			return taskReportManageDao.findByHql("from " + WOMTaskReportManage.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return taskReportManageDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMTaskReportManage> findTaskReportManagesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskReportManage.class);
	...
	@Override
	public List<WOMTaskReportManage> findTaskReportManagesByHql(String hql, Object... objects){
		
		return   taskReportManageDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMTaskReportManage loadTaskReportManageByBussinessKey(WOMTaskReportManage taskReportManage){
		return loadTaskReportManageByBussinessKey(taskReportManage.getId());
	}
	
	@Override
	public Page<WOMTaskReportManage> getTaskReportManages(Page<WOMTaskReportManage> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskReportManage.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = taskReportManageDao.findByPage(page, sql);
			}else{
				page = taskReportManageDao.findAllByPage(page);
			}
			return page;
		}
		page = taskReportManageDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMTaskReportManage> findByTreeDataGridProduceTaskManage(List<WOMTaskReportManage> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMTaskReportManage taskReportManage : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTaskManage(Page<WOMTaskReportManage> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMTaskReportManage> taskReportManages = dgPage.getResult();
			for(WOMTaskReportManage taskReportManage : taskReportManages){
				
			}
			dgPage.setResult(taskReportManages);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskReportManage loadTaskReportManageByBussinessKey(Serializable bussinessKey){
		return taskReportManageDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMTaskReportManage> findByProperty(String propertyName, Object object){
		return taskReportManageDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskReportManage findEntityByProperty(String propertyName, Object object){
		return taskReportManageDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMTaskReportManage.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = taskReportManageDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_generalManage_TaskReportManage","runtime");
		try {
	...
	public void dealDatagridsSave(WOMTaskReportManage taskReportManage,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,taskReportManageDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMTaskReportManage.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, taskReportManage);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",taskReportManage.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMTaskReportManage.TABLE_NAME +" where valid = 1";
			list = taskReportManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMTaskReportManage.TABLE_NAME;
			list = taskReportManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMTaskReportManage.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = taskReportManageDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMTaskReportManageDaoImpl taskReportManageDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchTaskReportManage(final List<WOMTaskReportManage> insertObjs, final List<WOMTaskReportManage> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchTaskReportManage(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchTaskReportManage(final List<WOMTaskReportManage> insertObjs, final List<WOMTaskReportManage> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1502698712286"),importCount));
		...
		Map<Long, WOMTaskReportManage> oldUpdateObjsMap = new HashMap<Long, WOMTaskReportManage>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMTaskReportManage bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMTaskReportManage> bizObjList = taskReportManageDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMTaskReportManage bizObj = bizObjList.get(mIndex);
		...
		final List<WOMTaskReportManage> nmeObjs = new ArrayList<WOMTaskReportManage>();
		...
		long id=autoGeneratorID.getNextId(WOMTaskReportManage.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMTaskReportManage taskReportManage:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMTaskReportManage.TABLE_NAME,1,dbId);
			if(taskReportManage.getId() != null ){
				m.put(taskReportManage.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) taskReportManageDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_TaskReportManage"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		taskReportManageDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMTaskReportManage.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMTaskReportManage taskReportManage = updateObjs.get(updateIndex);
						Long id = taskReportManage.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = taskReportManage.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(taskReportManage);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMTaskReportManage.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMTaskReportManage.TABLE_NAME + " order by id desc";
//					Query query = taskReportManageDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMTaskReportManage taskReportManage = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = taskReportManage.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		taskReportManageDaoImpl.getSession().flush();
		taskReportManageDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMTaskReportManage bizObj = insertObjs.get(insertIndex);
			taskReportManageImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMTaskReportManage bizObj = updateObjs.get(updateIndex);
			WOMTaskReportManage oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			taskReportManageImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMTaskReportManage> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMTaskReportManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) taskReportManageDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMTaskReportManage> result = getResult(page, query, new PendingResultTransformer(WOMTaskReportManage.class,taskReportManageDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMTaskReportManage model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = taskReportManageDao.createNativeQuery(sql, "WOM_7.5.0.0_generalManage_TaskReportManage").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = taskReportManageDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = taskReportManageDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = taskReportManageDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?taskReportManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  taskReportManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,taskReportManageDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="ec.flow.save")
	public void saveTaskReportManage(WOMTaskReportManage taskReportManage, DataGridService dataGridService){
		saveTaskReportManage(taskReportManage, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="ec.flow.save")
	public void saveTaskReportManage(WOMTaskReportManage taskReportManage, DataGridService dataGridService, String viewCode){
		saveTaskReportManage(taskReportManage, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage")
	public void saveTaskReportManage(WOMTaskReportManage taskReportManage, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveTaskReportManage(taskReportManage, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage")
	public void saveTaskReportManage(WOMTaskReportManage taskReportManage, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
		String url = null;
		if(null != taskReportManage.getId() && taskReportManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage")
	public Map<Object, Object> taskReportManageDataGridImport(WOMTaskReportManage taskReportManage, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = taskReportManage.getVirtualId();
		
		taskReportManage = taskReportManageDao.load(taskReportManage.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, taskReportManage.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = taskReportManage.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(taskReportManage);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, taskReportManage.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage")
	public void mergeTaskReportManage(WOMTaskReportManage taskReportManage, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(taskReportManage);
		beforeSaveTaskReportManage(taskReportManage);
		taskReportManageDao.merge(taskReportManage);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_generalManage",model="WOM_7.5.0.0_generalManage_TaskReportManage",desc="foundation.batch.save",operType="4")
	public void batchSaveTaskReportManage(WOMTaskReportManage taskReportManage, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_generalManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_generalManage");
	...
	@Override
	public Page<WOMTaskReportManage> getByPage(Page<WOMTaskReportManage> page,DetachedCriteria detachedCriteria){
		return taskReportManageDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveTaskReportManage(WOMTaskReportManage taskReportManage, Object... objects){
		checkUniqueConstraint(taskReportManage);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveTaskReportManage(WOMTaskReportManage taskReportManage, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportTaskReportManage(List<WOMTaskReportManage> taskReportManages, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportTaskReportManage(List<WOMTaskReportManage> taskReportManages){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportTaskReportManage(List<WOMTaskReportManage> taskReportManages, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportTaskReportManage(List<WOMTaskReportManage> taskReportManages){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteTaskReportManage(WOMTaskReportManage taskReportManage){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteTaskReportManage(WOMTaskReportManage taskReportManage){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreTaskReportManage(WOMTaskReportManage taskReportManage){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreTaskReportManage(WOMTaskReportManage taskReportManage){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMTaskReportManage taskReportManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMTaskReportManage taskReportManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportTaskReportManage(List<WOMTaskReportManage> insertObjs, List<WOMTaskReportManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportTaskReportManage,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportTaskReportManage(List<WOMTaskReportManage> insertObjs, List<WOMTaskReportManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportTaskReportManage,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
