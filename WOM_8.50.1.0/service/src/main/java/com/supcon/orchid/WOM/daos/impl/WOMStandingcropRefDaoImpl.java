...
@Repository
public class WOMStandingcropRefDaoImpl extends ExtendedGenericDaoImpl<WOMStandingcropRef, Long> implements WOMStandingcropRefDao {
	@Override
	...
	@Override
	public WOMStandingcropRefDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMStandingcropRefDealInfo dealInfo = new WOMStandingcropRefDealInfo();
	...
	@Override
	public WOMStandingcropRefDealInfo copyAndSaveDealInfo(DealInfo di, WOMStandingcropRef mainObj, Staff dealStaff) {
		WOMStandingcropRefDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMStandingcropRefDealInfo.class);
		...
		List<WOMStandingcropRefDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_standingcropRef_StandingcropRef,WOM_7.5.0.0) */
// 自定义代码
