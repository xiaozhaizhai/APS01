...
import com.supcon.orchid.WOM.services.WOMWaitPutinRecordsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMWaitPutinRecordsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

import com.supcon.orchid.foundation.services.UserService;
/* CUSTOM CODE END */

@Service("wOM_waitPutinRecordsService")
@Transactional
public class WOMWaitPutinRecordsServiceImpl  extends BaseServiceImpl<WOMWaitPutinRecords> implements WOMWaitPutinRecordsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMWaitPutinRecords> bapBaseService;
	...
	@Autowired
	private WOMWaitPutinRecordsImportService waitPutinRecordsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMWaitPutinRecords.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  waitPutinRecordsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMWaitPutinRecords.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = waitPutinRecordsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMWaitPutinRecords.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMWaitPutinRecords.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  waitPutinRecordsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMWaitPutinRecords.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMWaitPutinRecords.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMWaitPutinRecords> list = (List<WOMWaitPutinRecords>) waitPutinRecordsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMWaitPutinRecords getWaitPutinRecords(long id){
		return getWaitPutinRecords(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMWaitPutinRecords getWaitPutinRecords(long id, String viewCode){
		WOMWaitPutinRecords waitPutinRecords = waitPutinRecordsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getWaitPutinRecordsAsJSON(long id, String include){
		WOMWaitPutinRecords waitPutinRecords = waitPutinRecordsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(waitPutinRecords, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.common.delete",operType="3")
	public void deleteWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords){
		if(null != waitPutinRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMWaitPutinRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
			AuditUtil.setColumnStringA(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
			AuditUtil.setColumnStringC(waitPutinRecords.getId().toString());
		}
		beforeDeleteWaitPutinRecords(waitPutinRecords);
		waitPutinRecordsDao.deletePhysical(waitPutinRecords);
		afterDeleteWaitPutinRecords(waitPutinRecords);
		waitPutinRecordsDao.flush();
		List<WOMWaitPutinRecords> params = new ArrayList<WOMWaitPutinRecords>();
		params.add(waitPutinRecords);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.common.delete",operType="3")
	public void deleteWaitPutinRecords(List<Long> waitPutinRecordsIds){
		deleteWaitPutinRecords(waitPutinRecordsIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.common.delete",operType="3")
	public void deleteWaitPutinRecords(List<Long> waitPutinRecordsIds, String eventTopic) {
		prodTaskActiExelogService.deleteProdTaskActiExelogByWaitPutinRecords(waitPutinRecordsIds);
		List<WOMWaitPutinRecords> waitPutinRecordss = new ArrayList<WOMWaitPutinRecords>();
		for(Long waitPutinRecordsId : waitPutinRecordsIds){
			WOMWaitPutinRecords waitPutinRecords = getWaitPutinRecords(waitPutinRecordsId);
			waitPutinRecordss.add(waitPutinRecords);
			if(null != waitPutinRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMWaitPutinRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == waitPutinRecords.getId() ? "" : "," + waitPutinRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == waitPutinRecords.getId() ? "" : "," + waitPutinRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == waitPutinRecords.getId() ? "" : "," + waitPutinRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(waitPutinRecordss != null && waitPutinRecordss.size() > 0 ){
			for(WOMWaitPutinRecords waitPutinRecords : waitPutinRecordss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMWaitPutinRecords.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(waitPutinRecords));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),waitPutinRecords, waitPutinRecords.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(waitPutinRecordsIds != null && waitPutinRecordsIds.size() > 0) {
			String hql = "delete " + WOMWaitPutinRecords.JPA_NAME + " where id in(:ids)";
			Query query = waitPutinRecordsDao.createQuery(hql);
			query.setParameterList("ids", waitPutinRecordsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMWaitPutinRecords waitPutinRecordsz : waitPutinRecordss){
				waitPutinRecordsz.setValid(false);
				List<WOMWaitPutinRecords> params = new ArrayList<WOMWaitPutinRecords>();
				params.add(waitPutinRecordsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.common.delete",operType="3")
	public void deleteWaitPutinRecords(long waitPutinRecordsId, int waitPutinRecordsVersion){
		this.deleteWaitPutinRecords(waitPutinRecordsId, waitPutinRecordsVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.common.delete",operType="3")
	public void deleteWaitPutinRecords(long waitPutinRecordsId, int waitPutinRecordsVersion,SignatureLog signatureLog){
		WOMWaitPutinRecords waitPutinRecords = getWaitPutinRecords(waitPutinRecordsId);
		...
		beforeDeleteWaitPutinRecords(waitPutinRecords);
		waitPutinRecordsDao.delete(waitPutinRecordsId, waitPutinRecordsVersion);
		afterDeleteWaitPutinRecords(waitPutinRecords);
		waitPutinRecordsDao.flush();
		List<WOMWaitPutinRecords> params = new ArrayList<WOMWaitPutinRecords>();
		params.add(waitPutinRecords);
		...
		if(signatureLog != null ) {
			...
			businessKey=waitPutinRecords.getId();
			signatureLog.setTableId(waitPutinRecords.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.common.delete",operType="3")
	public void deleteWaitPutinRecords(String waitPutinRecordsIds){
		this.deleteWaitPutinRecords(waitPutinRecordsIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.common.delete",operType="3")
	public void deleteWaitPutinRecords(String waitPutinRecordsIds,SignatureLog signatureLog){
		deleteCollection(waitPutinRecordsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteWaitPutinRecords(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_WaitPutinRecords#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="ec.engine.restore",operType="10")
	public void restoreWaitPutinRecords(String waitPutinRecordsIds){
		restoreCollection(waitPutinRecordsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreWaitPutinRecords(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="ec.engine.restore",operType="10")
	public void restoreWaitPutinRecords(long waitPutinRecordsId){
		
		findBusinessKeyUsed(waitPutinRecordsId);	//判断业务主键是否重复
		...
		beforeRestoreWaitPutinRecords(waitPutinRecords);
		waitPutinRecords.setValid(true);
		waitPutinRecordsDao.update(waitPutinRecords);
		afterRestoreWaitPutinRecords(waitPutinRecords);
		List<WOMWaitPutinRecords> params = new ArrayList<WOMWaitPutinRecords>();
		params.add(waitPutinRecords);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long waitPutinRecordsId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_WaitPutinRecords");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMWaitPutinRecords.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMWaitPutinRecords.TABLE_NAME +" where id = ? )";
			List<Object> list =  waitPutinRecordsDao.createNativeQuery(sql,waitPutinRecordsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseWaitPutinRecords(List<WOMWaitPutinRecords>  waitPutinRecordss){
		for(WOMWaitPutinRecords waitPutinRecords:waitPutinRecordss)  {
			saveWaitPutinRecords(waitPutinRecords, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseWaitPutinRecords(List<WOMWaitPutinRecords>  waitPutinRecordss){
		for(WOMWaitPutinRecords waitPutinRecords:waitPutinRecordss)  {
			saveWaitPutinRecords(waitPutinRecords, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="ec.flow.save")
	public void saveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveWaitPutinRecords(waitPutinRecords, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="ec.flow.save")
	public void saveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveWaitPutinRecords(waitPutinRecords, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateWaitPutinRecordsCodes(WOMWaitPutinRecords waitPutinRecords) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateWaitPutinRecordsCodes(waitPutinRecords, false);
	}
	
	public void generateWaitPutinRecordsCodes(WOMWaitPutinRecords waitPutinRecords, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateWaitPutinRecordsSummarys(WOMWaitPutinRecords waitPutinRecords) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateWaitPutinRecordsSummarys(waitPutinRecords, false);
	}

	public void generateWaitPutinRecordsSummarys(WOMWaitPutinRecords waitPutinRecords, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords")
	public void saveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveWaitPutinRecords(waitPutinRecords, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords")
	public void saveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(waitPutinRecords.getId() != null && waitPutinRecords.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMWaitPutinRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, waitPutinRecords);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(waitPutinRecords);
		beforeSaveWaitPutinRecords(waitPutinRecords, viewIsView);

		if (viewIsView) {
			waitPutinRecordsDao.saveWithRevertVersion(waitPutinRecords);
		} else {
			if(isNew)
				waitPutinRecordsDao.save(waitPutinRecords);
			else
				waitPutinRecordsDao.merge(waitPutinRecords);
		}
		if(null != waitPutinRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMWaitPutinRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
			AuditUtil.setColumnStringA(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
		...
		dealDatagridsSave(waitPutinRecords,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateWaitPutinRecordsCodes(waitPutinRecords, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateWaitPutinRecordsSummarys(waitPutinRecords, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveWaitPutinRecords(waitPutinRecords, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, waitPutinRecords);
			}
		}
		List<WOMWaitPutinRecords> params = new ArrayList<WOMWaitPutinRecords>();
		waitPutinRecordsDao.flush();
		waitPutinRecordsDao.clear();
		waitPutinRecords = waitPutinRecordsDao.load(waitPutinRecords.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMWaitPutinRecords waitPutinRecords){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("waitPutinRecords", waitPutinRecords);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords")
	public void mergeWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(waitPutinRecords);
		beforeSaveWaitPutinRecords(waitPutinRecords);
		waitPutinRecordsDao.merge(waitPutinRecords);
		if(null != waitPutinRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMWaitPutinRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
			AuditUtil.setColumnStringA(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
		...
		dealDatagridsSave(waitPutinRecords,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveWaitPutinRecords(waitPutinRecords);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMWaitPutinRecords> findWaitPutinRecordss(Page<WOMWaitPutinRecords> page, Criterion... criterions) {
		return waitPutinRecordsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findWaitPutinRecordss(Page<WOMWaitPutinRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findWaitPutinRecordss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findWaitPutinRecordss(Page<WOMWaitPutinRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findWaitPutinRecordss(Page<WOMWaitPutinRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMWaitPutinRecords> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"waitPutinRecords\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"waitPutinRecords\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"waitPutinRecords\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"waitPutinRecords\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  waitPutinRecordsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  waitPutinRecordsDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"waitPutinRecords\".LAY_REC ASC, \"waitPutinRecords\".SORT ASC");
				} else {
					orderPart.append(" \"waitPutinRecords\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) waitPutinRecordsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=waitPutinRecordsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = waitPutinRecordsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMWaitPutinRecords> result = new ArrayList<WOMWaitPutinRecords>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMWaitPutinRecords.class,waitPutinRecordsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMWaitPutinRecords.class,waitPutinRecordsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMWaitPutinRecords waitPutinRecords : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(waitPutinRecords.getId(), "WOM_produceTask_waitPutinRecords");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(waitPutinRecords.getId(), "WOM_produceTask_waitPutinRecords");
							if(null != documents && !documents.isEmpty()) {
								waitPutinRecords.setDocument(documents.get(0));
								waitPutinRecords.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMWaitPutinRecords> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMWaitPutinRecords> result) {
		View view = null;
		...
		Map<Long, WOMWaitPutinRecords> rsMap = new HashMap<Long, WOMWaitPutinRecords>();
		for (WOMWaitPutinRecords waitPutinRecords : result) {
			ids.add(waitPutinRecords.getId());
			rsMap.put(waitPutinRecords.getId(), waitPutinRecords);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(waitPutinRecordsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = waitPutinRecordsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_WaitPutinRecords" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_WaitPutinRecords" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  waitPutinRecordsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"waitPutinRecords\"","WOM_7.5.0.0_produceTask","","WOM_WAIT_PUTIN_RECORDS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"waitPutinRecords\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"waitPutinRecords\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"waitPutinRecords\"","WOM_7.5.0.0_produceTask","","WOM_WAIT_PUTIN_RECORDS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_WaitPutinRecords" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("waitPutinRecords.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"waitPutinRecords\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"waitPutinRecords\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"waitPutinRecords\".CID = 1 OR \"waitPutinRecords\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("waitPutinRecords.mc\"").append(".WAIT_PUTIN_RECORDS) not in ("+notinValue+")");
			}
			
			SQLQuery query = waitPutinRecordsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=waitPutinRecordsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMWaitPutinRecords.ENTITY_CODE;}
	protected String getModuleCode(){return WOMWaitPutinRecords.MODULE_CODE;}
	protected String getModelCode(){return WOMWaitPutinRecords.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param waitPutinRecords 模型实例
	 */
	public void checkUniqueConstraint(WOMWaitPutinRecords waitPutinRecords){
	}
	
	
	@Override
	public List<WOMWaitPutinRecords> getWaitPutinRecordss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMWaitPutinRecords.class);
		if(params!=null){
			...
			return waitPutinRecordsDao.findByHql("from " + WOMWaitPutinRecords.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return waitPutinRecordsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMWaitPutinRecords> findWaitPutinRecordssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMWaitPutinRecords.class);
	...
	@Override
	public List<WOMWaitPutinRecords> findWaitPutinRecordssByHql(String hql, Object... objects){
		
		return   waitPutinRecordsDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMWaitPutinRecords loadWaitPutinRecordsByBussinessKey(WOMWaitPutinRecords waitPutinRecords){
		return loadWaitPutinRecordsByBussinessKey(waitPutinRecords.getId());
	}
	
	@Override
	public Page<WOMWaitPutinRecords> getWaitPutinRecordss(Page<WOMWaitPutinRecords> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMWaitPutinRecords.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = waitPutinRecordsDao.findByPage(page, sql);
			}else{
				page = waitPutinRecordsDao.findAllByPage(page);
			}
			return page;
		}
		page = waitPutinRecordsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteWaitPutinRecordsByProduceTaskActive(List<Long> produceTaskActiveIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("taskActiveIDList", produceTaskActiveIds);
		String hql = "select id from WOMWaitPutinRecords where "
						+ "taskActiveID.id in (:taskActiveIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> waitPutinRecordsIds1 = waitPutinRecordsDao.findByHql(hql, params);
		if(waitPutinRecordsIds1 != null && !waitPutinRecordsIds1.isEmpty()) {
			// 针对自关联模型
			prodTaskActiExelogService.deleteProdTaskActiExelogByWaitPutinRecords(waitPutinRecordsIds1);
		}
		hql = "delete WOMWaitPutinRecords where "
						+ "taskActiveID.id in (:taskActiveIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		waitPutinRecordsDao.bulkExecute(hql, params);
	}
	@Override
	@Transactional
	public void deleteWaitPutinRecordsByProduceTaskProcess(List<Long> produceTaskProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMWaitPutinRecords loadWaitPutinRecordsByBussinessKey(Serializable bussinessKey){
		return waitPutinRecordsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMWaitPutinRecords> findByProperty(String propertyName, Object object){
		return waitPutinRecordsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMWaitPutinRecords findEntityByProperty(String propertyName, Object object){
		return waitPutinRecordsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMWaitPutinRecords.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = waitPutinRecordsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_WaitPutinRecords","runtime");
		try {
	...
	public void dealDatagridsSave(WOMWaitPutinRecords waitPutinRecords,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,waitPutinRecordsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMWaitPutinRecords.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, waitPutinRecords);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(waitPutinRecords.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(waitPutinRecords.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(waitPutinRecords.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",waitPutinRecords.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMWaitPutinRecords.TABLE_NAME +" where valid = 1";
			list = waitPutinRecordsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMWaitPutinRecords.TABLE_NAME;
			list = waitPutinRecordsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMWaitPutinRecords.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = waitPutinRecordsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMWaitPutinRecordsDaoImpl waitPutinRecordsDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMWaitPutinRecords.TABLE_NAME + " where id = ?";
		String tableNo = waitPutinRecordsDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchWaitPutinRecords(final List<WOMWaitPutinRecords> insertObjs, final List<WOMWaitPutinRecords> updateObjs, 
		...
		return this.importBatchWaitPutinRecords(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchWaitPutinRecords(final List<WOMWaitPutinRecords> insertObjs, final List<WOMWaitPutinRecords> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1496884906781"),importCount));
		...
		Map<Long, WOMWaitPutinRecords> oldUpdateObjsMap = new HashMap<Long, WOMWaitPutinRecords>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMWaitPutinRecords bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMWaitPutinRecords> bizObjList = waitPutinRecordsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMWaitPutinRecords bizObj = bizObjList.get(mIndex);
		...
		final List<WOMWaitPutinRecords> nmeObjs = new ArrayList<WOMWaitPutinRecords>();
		...
		//String idSql = "select id from " + WOMWaitPutinRecords.TABLE_NAME + " order by id desc";
		//Query query = waitPutinRecordsDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMWaitPutinRecords.JPA_NAME + " order by id desc";
		Query query =  waitPutinRecordsDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMWaitPutinRecords.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMWaitPutinRecords waitPutinRecords:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMWaitPutinRecords.TABLE_NAME,1,dbId);
			if(waitPutinRecords.getId() != null ){
				m.put(waitPutinRecords.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) waitPutinRecordsDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_WaitPutinRecords"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		waitPutinRecordsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMWaitPutinRecords.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMWaitPutinRecords waitPutinRecords = updateObjs.get(updateIndex);
						Long id = waitPutinRecords.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = waitPutinRecords.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(waitPutinRecords);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMWaitPutinRecords.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMWaitPutinRecords waitPutinRecords = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = waitPutinRecords.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		waitPutinRecordsDaoImpl.getSession().flush();
		waitPutinRecordsDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = waitPutinRecordsDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_WaitPutinRecords").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = waitPutinRecordsDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_WaitPutinRecords").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMWaitPutinRecords bizObj = insertObjs.get(insertIndex);
			waitPutinRecordsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMWaitPutinRecords bizObj = updateObjs.get(updateIndex);
			WOMWaitPutinRecords oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			waitPutinRecordsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMWaitPutinRecords> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMWaitPutinRecords> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) waitPutinRecordsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMWaitPutinRecords> result = getResult(page, query, new PendingResultTransformer(WOMWaitPutinRecords.class,waitPutinRecordsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMWaitPutinRecords model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = waitPutinRecordsDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_WaitPutinRecords").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = waitPutinRecordsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = waitPutinRecordsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = waitPutinRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?waitPutinRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  waitPutinRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,waitPutinRecordsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="ec.flow.save")
	public void saveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, DataGridService dataGridService){
		saveWaitPutinRecords(waitPutinRecords, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="ec.flow.save")
	public void saveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, DataGridService dataGridService, String viewCode){
		saveWaitPutinRecords(waitPutinRecords, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords")
	public void saveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveWaitPutinRecords(waitPutinRecords, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords")
	public void saveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != waitPutinRecords.getId() && waitPutinRecords.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMWaitPutinRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords")
	public Map<Object, Object> waitPutinRecordsDataGridImport(WOMWaitPutinRecords waitPutinRecords, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = waitPutinRecords.getVirtualId();
		
		waitPutinRecords = waitPutinRecordsDao.load(waitPutinRecords.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, waitPutinRecords.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = waitPutinRecords.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(waitPutinRecords);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, waitPutinRecords.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords")
	public void mergeWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(waitPutinRecords);
		beforeSaveWaitPutinRecords(waitPutinRecords);
		waitPutinRecordsDao.merge(waitPutinRecords);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_WaitPutinRecords",desc="foundation.batch.save",operType="4")
	public void batchSaveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
	...
	@Override
	public Page<WOMWaitPutinRecords> getByPage(Page<WOMWaitPutinRecords> page,DetachedCriteria detachedCriteria){
		return waitPutinRecordsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, Object... objects){
		checkUniqueConstraint(waitPutinRecords);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportWaitPutinRecords(List<WOMWaitPutinRecords> waitPutinRecordss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportWaitPutinRecords(List<WOMWaitPutinRecords> waitPutinRecordss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportWaitPutinRecords(List<WOMWaitPutinRecords> waitPutinRecordss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportWaitPutinRecords(List<WOMWaitPutinRecords> waitPutinRecordss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMWaitPutinRecords waitPutinRecords, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMWaitPutinRecords waitPutinRecords, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportWaitPutinRecords(List<WOMWaitPutinRecords> insertObjs, List<WOMWaitPutinRecords> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportWaitPutinRecords,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportWaitPutinRecords(List<WOMWaitPutinRecords> insertObjs, List<WOMWaitPutinRecords> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportWaitPutinRecords,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
	  ...
	  private UserService userService;
	    ...
	    /**
		 * 根据phaseID及是否已执行查询待投料记录
		 */
		@Override
		@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
		public List<Map<String, Object>> getWaitPutinRecords(String phaseID,String isFinish) {
			List<Map<String, Object>> waitPutinRecordsList = new ArrayList<Map<String,Object>>();  //待投料记录
			List<Map<String, Object>> putinRecordsList = new ArrayList<Map<String,Object>>();    //已投料记录
			Map<String, Object> map = null;
			boolean isFinishFlag  = new Boolean(isFinish);
			String hql = "";
			if(!isFinishFlag){  //待投记录
				hql = "from WOMWaitPutinRecords where phaseID = ? and valid =1 and isFinish = ? order by actOrder,"
						+ "materialNum desc";	
		      	Object params[] = {phaseID,isFinishFlag};
				List<WOMWaitPutinRecords> womWaitPutinRecords = waitPutinRecordsDao.createQuery(hql,params).list();
				for (WOMWaitPutinRecords womWaitPutinRecord : womWaitPutinRecords) {
					map = new HashMap<String, Object>();
					map.put("id", womWaitPutinRecord.getId());
					map.put("actOrder", womWaitPutinRecord.getActOrder());
					map.put("batchNum", womWaitPutinRecord.getBatchNum());
					map.put("materialNum", womWaitPutinRecord.getMaterialNum());
	                map.put("actualNum", womWaitPutinRecord.getActualNum() == null ? 0 : womWaitPutinRecord.getActualNum());
					map.put("productNum", womWaitPutinRecord.getProductNum());
					map.put("productModel", womWaitPutinRecord.getProductID() == null ? "" : womWaitPutinRecord.getProductID().getProductModel());
					map.put("lableCode", womWaitPutinRecord.getLableCode());
	                map.put("activeType", womWaitPutinRecord.getActiveType() == null? "" : womWaitPutinRecord.getActiveType().getId());
		            map.put("activeName", womWaitPutinRecord.getTaskActiveID() == null ? "" : womWaitPutinRecord.getTaskActiveID().getName());	
	                waitPutinRecordsList.add(map);
				}
				return waitPutinRecordsList;
			}else{  //已投料记录
				hql = "from WOMProdTaskActiExelog where phaseID = ? and valid = 1 and isPDA = true and isFinish = true ";
				List<WOMProdTaskActiExelog> womPutinRecordsList = prodTaskActiExelogDao.createQuery(hql, phaseID).list();
				for(WOMProdTaskActiExelog womPutinRecords : womPutinRecordsList){
					map = new HashMap<String,Object>();
					map.put("id", womPutinRecords.getId());
					map.put("actOrder", womPutinRecords.getWaitPutinRecordID() != null ? womPutinRecords.getWaitPutinRecordID().getActOrder() : null);
					map.put("actualNum", womPutinRecords.getActualNum());
					map.put("productModel", womPutinRecords.getProductID() != null ? womPutinRecords.getProductID().getProductModel() : "");
					putinRecordsList.add(map);
				}
				return putinRecordsList;
			}
		}
		 ...
		 * 将待投料记录修改为已执行
		 */
		public void updateWaitPutinRecordState(WOMWaitPutinRecords record,String userName){
			record.setIsFinish(true);  //状态改为已执行
			record.setStaffID(userService.getUser(userName)!=null ? userService.getUser(userName).getStaff():null); //回填操作人
			record.setActTime(new Date());  //回填操作时间
			waitPutinRecordsDao.update(record);
		}

        /**
		 * PDA手工投料 更新待投料记录：实际投料数量，投料人，时间
		 */
		@Override
		public void updateActualNum(WOMWaitPutinRecords waitPutinRecord,
				Double currentNum, String userName) {
			Staff staff = userService.getUser(userName)!=null?userService.getUser(userName).getStaff():null;  //根据登陆用户名获取Staff
			waitPutinRecord.setStaffID(staff); //回填操作人
			
			BigDecimal oldActualNum = waitPutinRecord.getActualNum() != null ? waitPutinRecord.getActualNum():new BigDecimal(0);
			waitPutinRecord.setActualNum(oldActualNum.add(new BigDecimal(currentNum)));  //回填单次实际投料数量
			waitPutinRecord.setActTime(new Date()); //回填操作时间
			waitPutinRecordsDao.merge(waitPutinRecord);
		}

        /**
		 * PDA其他处理，根据指令单工序ID获取掺料活动
		 * @param taskProcessID
		 */
		@Override
		public List<WOMWaitPutinRecords> getWaitPutinAdmixtureList(Long taskProcessID) {
			String hql = "from WOMWaitPutinRecords where taskProcessID.id = ? and activeSource.id = 'womActiveSource/02' and isFinish = false and mobileTerminal = true ";
			List<WOMWaitPutinRecords> list = waitPutinRecordsDao.createQuery(hql, taskProcessID).list();
			return list;
		}
/* CUSTOM CODE END */
}
