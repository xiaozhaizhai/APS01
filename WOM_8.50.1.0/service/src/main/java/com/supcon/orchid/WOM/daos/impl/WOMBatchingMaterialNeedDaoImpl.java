...
@Repository
public class WOMBatchingMaterialNeedDaoImpl extends ExtendedGenericDaoImpl<WOMBatchingMaterialNeed, Long> implements WOMBatchingMaterialNeedDao {
	@Override
	...
	@Override
	public WOMBatchingMaterialNeedDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMBatchingMaterialNeedDealInfo dealInfo = new WOMBatchingMaterialNeedDealInfo();
	...
	@Override
	public WOMBatchingMaterialNeedDealInfo copyAndSaveDealInfo(DealInfo di, WOMBatchingMaterialNeed mainObj, Staff dealStaff) {
		WOMBatchingMaterialNeedDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMBatchingMaterialNeedDealInfo.class);
		...
		List<WOMBatchingMaterialNeedDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
