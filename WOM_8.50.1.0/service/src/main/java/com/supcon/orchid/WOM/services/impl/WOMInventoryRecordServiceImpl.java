...
import com.supcon.orchid.WOM.services.WOMInventoryRecordImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMInventoryRecordDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
import java.util.Calendar;
import com.supcon.orchid.WOM.POJO.SAPMaterial;
import java.text.SimpleDateFormat;
import com.supcon.orchid.WOM.services.WomMesToSapService;
import com.supcon.orchid.WOM.services.impl.utils.XmlMapUtil;
/* CUSTOM CODE END */

@Service("wOM_inventoryRecordService")
@Transactional
public class WOMInventoryRecordServiceImpl  extends BaseServiceImpl<WOMInventoryRecord> implements WOMInventoryRecordService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMInventoryRecord> bapBaseService;
	...
	@Autowired
	private WOMInventoryRecordImportService inventoryRecordImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMInventoryRecord.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  inventoryRecordDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMInventoryRecord.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = inventoryRecordDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMInventoryRecord.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMInventoryRecord.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  inventoryRecordDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMInventoryRecord.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMInventoryRecord.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMInventoryRecord> list = (List<WOMInventoryRecord>) inventoryRecordDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInventoryRecord getInventoryRecord(long id){
		return getInventoryRecord(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInventoryRecord getInventoryRecord(long id, String viewCode){
		WOMInventoryRecord inventoryRecord = inventoryRecordDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getInventoryRecordAsJSON(long id, String include){
		WOMInventoryRecord inventoryRecord = inventoryRecordDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(inventoryRecord, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.common.delete",operType="3")
	public void deleteInventoryRecord(WOMInventoryRecord inventoryRecord){
		if(null != inventoryRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
			AuditUtil.setColumnStringA(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
			AuditUtil.setColumnStringC(inventoryRecord.getId().toString());
		}
		beforeDeleteInventoryRecord(inventoryRecord);
		inventoryRecordDao.delete(inventoryRecord);
		afterDeleteInventoryRecord(inventoryRecord);
		inventoryRecordDao.flush();
		List<WOMInventoryRecord> params = new ArrayList<WOMInventoryRecord>();
		params.add(inventoryRecord);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_inventoryRecords");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.common.delete",operType="3")
	public void deleteInventoryRecord(List<Long> inventoryRecordIds){
		deleteInventoryRecord(inventoryRecordIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.common.delete",operType="3")
	public void deleteInventoryRecord(List<Long> inventoryRecordIds, String eventTopic) {
		List<WOMInventoryRecord> inventoryRecords = new ArrayList<WOMInventoryRecord>();
		for(Long inventoryRecordId : inventoryRecordIds){
			WOMInventoryRecord inventoryRecord = getInventoryRecord(inventoryRecordId);
			inventoryRecords.add(inventoryRecord);
			if(null != inventoryRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == inventoryRecord.getId() ? "" : "," + inventoryRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == inventoryRecord.getId() ? "" : "," + inventoryRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == inventoryRecord.getId() ? "" : "," + inventoryRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
				}
			}
		}
		
		for(WOMInventoryRecord inventoryRecord : inventoryRecords){
			beforeDeleteInventoryRecord(inventoryRecord);
		}
		...
		if(inventoryRecords != null && inventoryRecords.size() > 0 ){
			for(WOMInventoryRecord inventoryRecord : inventoryRecords){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),inventoryRecord, inventoryRecord.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(inventoryRecordIds != null && inventoryRecordIds.size() > 0) {
			String hql = "update " + WOMInventoryRecord.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = inventoryRecordDao.createQuery(hql);
			query.setParameterList("ids", inventoryRecordIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMInventoryRecord inventoryRecordz : inventoryRecords){
				inventoryRecordz.setValid(false);
				List<WOMInventoryRecord> params = new ArrayList<WOMInventoryRecord>();
				params.add(inventoryRecordz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_inventoryRecords");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.common.delete",operType="3")
	public void deleteInventoryRecord(long inventoryRecordId, int inventoryRecordVersion){
		this.deleteInventoryRecord(inventoryRecordId, inventoryRecordVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.common.delete",operType="3")
	public void deleteInventoryRecord(long inventoryRecordId, int inventoryRecordVersion,SignatureLog signatureLog){
		WOMInventoryRecord inventoryRecord = getInventoryRecord(inventoryRecordId);
		if(inventoryRecord != null && !inventoryRecord.isValid()){
		...
		beforeDeleteInventoryRecord(inventoryRecord);
		inventoryRecordDao.delete(inventoryRecordId, inventoryRecordVersion);
		afterDeleteInventoryRecord(inventoryRecord);
		inventoryRecordDao.flush();
		List<WOMInventoryRecord> params = new ArrayList<WOMInventoryRecord>();
		params.add(inventoryRecord);
		...
		if(signatureLog != null ) {
			...
			businessKey=inventoryRecord.getId();
			signatureLog.setTableId(inventoryRecord.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.common.delete",operType="3")
	public void deleteInventoryRecord(String inventoryRecordIds){
		this.deleteInventoryRecord(inventoryRecordIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.common.delete",operType="3")
	public void deleteInventoryRecord(String inventoryRecordIds,SignatureLog signatureLog){
		deleteCollection(inventoryRecordIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteInventoryRecord(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_inventoryRecords#modelCode:WOM_7.5.0.0_inventoryRecords_InventoryRecord#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="ec.engine.restore",operType="10")
	public void restoreInventoryRecord(String inventoryRecordIds){
		restoreCollection(inventoryRecordIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreInventoryRecord(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="ec.engine.restore",operType="10")
	public void restoreInventoryRecord(long inventoryRecordId){
		
		findBusinessKeyUsed(inventoryRecordId);	//判断业务主键是否重复
		...
		beforeRestoreInventoryRecord(inventoryRecord);
		inventoryRecord.setValid(true);
		inventoryRecordDao.update(inventoryRecord);
		afterRestoreInventoryRecord(inventoryRecord);
		List<WOMInventoryRecord> params = new ArrayList<WOMInventoryRecord>();
		params.add(inventoryRecord);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long inventoryRecordId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_inventoryRecords_InventoryRecord");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMInventoryRecord.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMInventoryRecord.TABLE_NAME +" where id = ? )";
			List<Object> list =  inventoryRecordDao.createNativeQuery(sql,inventoryRecordId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseInventoryRecord(List<WOMInventoryRecord>  inventoryRecords){
		for(WOMInventoryRecord inventoryRecord:inventoryRecords)  {
			saveInventoryRecord(inventoryRecord, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseInventoryRecord(List<WOMInventoryRecord>  inventoryRecords){
		for(WOMInventoryRecord inventoryRecord:inventoryRecords)  {
			saveInventoryRecord(inventoryRecord, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="ec.flow.save")
	public void saveInventoryRecord(WOMInventoryRecord inventoryRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveInventoryRecord(inventoryRecord, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="ec.flow.save")
	public void saveInventoryRecord(WOMInventoryRecord inventoryRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveInventoryRecord(inventoryRecord, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateInventoryRecordCodes(WOMInventoryRecord inventoryRecord) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateInventoryRecordCodes(inventoryRecord, false);
	}
	
	public void generateInventoryRecordCodes(WOMInventoryRecord inventoryRecord, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateInventoryRecordSummarys(WOMInventoryRecord inventoryRecord) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateInventoryRecordSummarys(inventoryRecord, false);
	}

	public void generateInventoryRecordSummarys(WOMInventoryRecord inventoryRecord, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord")
	public void saveInventoryRecord(WOMInventoryRecord inventoryRecord,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveInventoryRecord(inventoryRecord, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord")
	public void saveInventoryRecord(WOMInventoryRecord inventoryRecord,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_inventoryRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_inventoryRecords");
		String url = null;
		if(inventoryRecord.getId() != null && inventoryRecord.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_inventoryRecords/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_inventoryRecords/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, inventoryRecord);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(inventoryRecord);
		beforeSaveInventoryRecord(inventoryRecord, viewIsView);

		if (viewIsView) {
			inventoryRecordDao.saveWithRevertVersion(inventoryRecord);
		} else {
			if(isNew)
				inventoryRecordDao.save(inventoryRecord);
			else
				inventoryRecordDao.merge(inventoryRecord);
		}
		if(null != inventoryRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
			AuditUtil.setColumnStringA(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
		...
		dealDatagridsSave(inventoryRecord,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateInventoryRecordCodes(inventoryRecord, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateInventoryRecordSummarys(inventoryRecord, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveInventoryRecord(inventoryRecord, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, inventoryRecord);
			}
		}
		List<WOMInventoryRecord> params = new ArrayList<WOMInventoryRecord>();
		inventoryRecordDao.flush();
		inventoryRecordDao.clear();
		inventoryRecord = inventoryRecordDao.load(inventoryRecord.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMInventoryRecord inventoryRecord){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("inventoryRecord", inventoryRecord);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord")
	public void mergeInventoryRecord(WOMInventoryRecord inventoryRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(inventoryRecord);
		beforeSaveInventoryRecord(inventoryRecord);
		inventoryRecordDao.merge(inventoryRecord);
		if(null != inventoryRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
			AuditUtil.setColumnStringA(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
		...
		dealDatagridsSave(inventoryRecord,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveInventoryRecord(inventoryRecord);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMInventoryRecord> findInventoryRecords(Page<WOMInventoryRecord> page, Criterion... criterions) {
		return inventoryRecordDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMInventoryRecordDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMInventoryRecordDealInfo> list=inventoryRecordDao.createQuery("from " + WOMInventoryRecordDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMInventoryRecordDealInfo dealInfo) {
		inventoryRecordDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMInventoryRecordDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) inventoryRecordDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMInventoryRecordDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = inventoryRecordDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = inventoryRecordDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMInventoryRecordDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = inventoryRecordDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInventoryRecords(Page<WOMInventoryRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findInventoryRecords(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInventoryRecords(Page<WOMInventoryRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInventoryRecords(Page<WOMInventoryRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMInventoryRecord> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"inventoryRecord\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"inventoryRecord\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"inventoryRecord\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_inventoryRecords_InventoryRecord" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_inventoryRecords_InventoryRecord" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  inventoryRecordDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  inventoryRecordDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_inventoryRecords").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"inventoryRecord\"","WOM_7.5.0.0_inventoryRecords",customCondition,"WOM_INVENTORY_RECORDS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"inventoryRecord\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMInventoryRecordDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"inventoryRecord\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"inventoryRecord\".LAY_REC ASC, \"inventoryRecord\".SORT ASC");
				} else {
					orderPart.append(" \"inventoryRecord\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) inventoryRecordDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=inventoryRecordDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = inventoryRecordDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMInventoryRecord> result = new ArrayList<WOMInventoryRecord>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMInventoryRecord.class,inventoryRecordDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMInventoryRecord.class,inventoryRecordDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMInventoryRecord inventoryRecord : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(inventoryRecord.getId(), "WOM_inventoryRecords_inventoryRecord");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(inventoryRecord.getId(), "WOM_inventoryRecords_inventoryRecord");
							if(null != documents && !documents.isEmpty()) {
								inventoryRecord.setDocument(documents.get(0));
								inventoryRecord.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_inventoryRecords")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_inventoryRecords")+"WOM_7.5.0.0_inventoryRecords".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_inventoryRecords",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMInventoryRecord> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMInventoryRecord> result) {
		View view = null;
		...
		Map<Long, WOMInventoryRecord> rsMap = new HashMap<Long, WOMInventoryRecord>();
		for (WOMInventoryRecord inventoryRecord : result) {
			ids.add(inventoryRecord.getId());
			rsMap.put(inventoryRecord.getId(), inventoryRecord);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(inventoryRecordDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = inventoryRecordDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMInventoryRecord inventoryRecord,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(inventoryRecord.getOwnerStaffId());
		ti.setOwnerPositionId(inventoryRecord.getOwnerPositionId());
		ti.setOwnerDepartmentId(inventoryRecord.getOwnerDepartmentId());
		
		ti.setCreateStaff(inventoryRecord.getCreateStaff());
		ti.setCreateStaffId(inventoryRecord.getCreateStaffId());
		ti.setCreateTime(inventoryRecord.getCreateTime());
		ti.setDeleteStaff(inventoryRecord.getDeleteStaff());
		ti.setDeleteStaffId(inventoryRecord.getDeleteStaffId());
		ti.setDeleteTime(inventoryRecord.getDeleteTime());
		ti.setCreateDepartmentId(inventoryRecord.getCreateDepartmentId());
		ti.setModifyStaff(inventoryRecord.getModifyStaff());
		ti.setModifyStaffId(inventoryRecord.getModifyStaffId());
		ti.setCreatePositionId(inventoryRecord.getCreatePositionId());
		ti.setPositionLayRec(inventoryRecord.getPositionLayRec());
		ti.setTableNo(inventoryRecord.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_inventoryRecords");
		ti.setTargetTableName("WOM_INVENTORY_RECORDS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_inventoryRecords_InventoryRecord" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  inventoryRecordDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"inventoryRecord\"","WOM_7.5.0.0_inventoryRecords","","WOM_INVENTORY_RECORDS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"inventoryRecord\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"inventoryRecord\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"inventoryRecord\"","WOM_7.5.0.0_inventoryRecords","","WOM_INVENTORY_RECORDS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("inventoryRecord.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"inventoryRecord\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"inventoryRecord\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"inventoryRecord\".CID = 1 OR \"inventoryRecord\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("inventoryRecord.mc\"").append(".INVENTORY_RECORD) not in ("+notinValue+")");
			}
			
			SQLQuery query = inventoryRecordDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("inventoryRecords", "WOM_7.5.0.0_inventoryRecords", "inventoryRecords_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=inventoryRecordDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMInventoryRecord.ENTITY_CODE;}
	protected String getModuleCode(){return WOMInventoryRecord.MODULE_CODE;}
	protected String getModelCode(){return WOMInventoryRecord.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param inventoryRecord 模型实例
	 */
	public void checkUniqueConstraint(WOMInventoryRecord inventoryRecord){
	}
	
	
	@Override
	public List<WOMInventoryRecord> getInventoryRecords(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMInventoryRecord.class);
		if(params!=null){
			...
			return inventoryRecordDao.findByHql("from " + WOMInventoryRecord.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return inventoryRecordDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMInventoryRecord> findInventoryRecordsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMInventoryRecord.class);
	...
	@Override
	public List<WOMInventoryRecord> findInventoryRecordsByHql(String hql, Object... objects){
		
		return   inventoryRecordDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMInventoryRecord loadInventoryRecordByBussinessKey(WOMInventoryRecord inventoryRecord){
		return loadInventoryRecordByBussinessKey(inventoryRecord.getId());
	}
	
	@Override
	public Page<WOMInventoryRecord> getInventoryRecords(Page<WOMInventoryRecord> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMInventoryRecord.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = inventoryRecordDao.findByPage(page, sql);
			}else{
				page = inventoryRecordDao.findAllByPage(page);
			}
			return page;
		}
		page = inventoryRecordDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInventoryRecord loadInventoryRecordByBussinessKey(Serializable bussinessKey){
		return inventoryRecordDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMInventoryRecord> findByProperty(String propertyName, Object object){
		return inventoryRecordDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInventoryRecord findEntityByProperty(String propertyName, Object object){
		return inventoryRecordDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMInventoryRecord.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = inventoryRecordDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_inventoryRecords_InventoryRecord","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_inventoryRecords", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMInventoryRecord inventoryRecord,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,inventoryRecordDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMInventoryRecord.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, inventoryRecord);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",inventoryRecord.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMInventoryRecord.TABLE_NAME +" where valid = 1";
			list = inventoryRecordDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMInventoryRecord.TABLE_NAME;
			list = inventoryRecordDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMInventoryRecord.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = inventoryRecordDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMInventoryRecordDaoImpl inventoryRecordDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchInventoryRecord(final List<WOMInventoryRecord> insertObjs, final List<WOMInventoryRecord> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchInventoryRecord(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchInventoryRecord(final List<WOMInventoryRecord> insertObjs, final List<WOMInventoryRecord> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1566888253801"),importCount));
		...
		Map<Long, WOMInventoryRecord> oldUpdateObjsMap = new HashMap<Long, WOMInventoryRecord>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMInventoryRecord bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMInventoryRecord> bizObjList = inventoryRecordDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMInventoryRecord bizObj = bizObjList.get(mIndex);
		...
		final List<WOMInventoryRecord> nmeObjs = new ArrayList<WOMInventoryRecord>();
		...
		long id=autoGeneratorID.getNextId(WOMInventoryRecord.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMInventoryRecord inventoryRecord:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMInventoryRecord.TABLE_NAME,1,dbId);
			if(inventoryRecord.getId() != null ){
				m.put(inventoryRecord.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) inventoryRecordDao.createNativeQuery(sql, "WOM_7.5.0.0_inventoryRecords_InventoryRecord"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		inventoryRecordDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMInventoryRecord.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMInventoryRecord inventoryRecord = updateObjs.get(updateIndex);
						Long id = inventoryRecord.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = inventoryRecord.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(inventoryRecord);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMInventoryRecord.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMInventoryRecord.TABLE_NAME + " order by id desc";
//					Query query = inventoryRecordDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMInventoryRecord inventoryRecord = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = inventoryRecord.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		inventoryRecordDaoImpl.getSession().flush();
		inventoryRecordDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMInventoryRecord bizObj = insertObjs.get(insertIndex);
			inventoryRecordImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMInventoryRecord bizObj = updateObjs.get(updateIndex);
			WOMInventoryRecord oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			inventoryRecordImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = inventoryRecordDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				inventoryRecordDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = inventoryRecordDao.createNativeQuery(sql, "WOM_7.5.0.0_inventoryRecords_InventoryRecord").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = inventoryRecordDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = inventoryRecordDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = inventoryRecordDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?inventoryRecordDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  inventoryRecordDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,inventoryRecordDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1566900683310Page(Page<WOMInventoryDetail> dg1566900683310Page,WOMInventoryRecord inventoryRecord, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMInventoryDetail.class);
		criteria.add(Restrictions.eq("head", inventoryRecord));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"inventoryDetail\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		inventoryDetailDao.findByPage(dg1566900683310Page, criteria);
		inventoryDetailService.findByNormalDataGridInventoryRecord(dg1566900683310Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1566899530350Page(Page<WOMInventoryDetail> dg1566899530350Page,WOMInventoryRecord inventoryRecord, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMInventoryDetail.class);
		criteria.add(Restrictions.eq("head", inventoryRecord));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		inventoryDetailDao.findByPage(dg1566899530350Page, criteria);
		inventoryDetailService.findByNormalDataGridInventoryRecord(dg1566899530350Page);
	}
	
	 
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="ec.flow.save")
	public void saveInventoryRecord(WOMInventoryRecord inventoryRecord, DataGridService dataGridService){
		saveInventoryRecord(inventoryRecord, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="ec.flow.save")
	public void saveInventoryRecord(WOMInventoryRecord inventoryRecord, DataGridService dataGridService, String viewCode){
		saveInventoryRecord(inventoryRecord, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord")
	public void saveInventoryRecord(WOMInventoryRecord inventoryRecord, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveInventoryRecord(inventoryRecord, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord")
	public void saveInventoryRecord(WOMInventoryRecord inventoryRecord, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_inventoryRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_inventoryRecords");
		String url = null;
		if(null != inventoryRecord.getId() && inventoryRecord.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord")
	public Map<Object, Object> inventoryRecordDataGridImport(WOMInventoryRecord inventoryRecord, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = inventoryRecord.getVirtualId();
		
		inventoryRecord = inventoryRecordDao.load(inventoryRecord.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, inventoryRecord.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = inventoryRecord.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(inventoryRecord);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, inventoryRecord.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord")
	public void mergeInventoryRecord(WOMInventoryRecord inventoryRecord, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(inventoryRecord);
		beforeSaveInventoryRecord(inventoryRecord);
		inventoryRecordDao.merge(inventoryRecord);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryRecord",desc="foundation.batch.save",operType="4")
	public void batchSaveInventoryRecord(WOMInventoryRecord inventoryRecord, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_inventoryRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_inventoryRecords");
	...
	@Override
	public Page<WOMInventoryRecord> getByPage(Page<WOMInventoryRecord> page,DetachedCriteria detachedCriteria){
		return inventoryRecordDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_inventoryRecords").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveInventoryRecord(WOMInventoryRecord inventoryRecord, Object... objects){
		checkUniqueConstraint(inventoryRecord);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveInventoryRecord(WOMInventoryRecord inventoryRecord, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportInventoryRecord(List<WOMInventoryRecord> inventoryRecords, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportInventoryRecord(List<WOMInventoryRecord> inventoryRecords){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportInventoryRecord(List<WOMInventoryRecord> inventoryRecords, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportInventoryRecord(List<WOMInventoryRecord> inventoryRecords){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteInventoryRecord(WOMInventoryRecord inventoryRecord){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteInventoryRecord(WOMInventoryRecord inventoryRecord){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreInventoryRecord(WOMInventoryRecord inventoryRecord){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreInventoryRecord(WOMInventoryRecord inventoryRecord){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMInventoryRecord inventoryRecord, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMInventoryRecord inventoryRecord, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportInventoryRecord(List<WOMInventoryRecord> insertObjs, List<WOMInventoryRecord> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportInventoryRecord,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportInventoryRecord(List<WOMInventoryRecord> insertObjs, List<WOMInventoryRecord> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportInventoryRecord,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
	@Autowired
	private WomMesToSapService womMesToSapService;

	@Override
	public void generateInRecords() {
		// 获取本月生产订单获取领用记录
		...
		List<WOMStandingcropRef> standingcropRefs = standingcropRefService.findStandingcropRefsByHql("from " + WOMStandingcropRef.JPA_NAME + " where onhand != 0 and manulOrder.planStartTime between ? and ? order by manulOrder.manulOrderNum", start.getTime(), new Date());
		if (standingcropRefs.size() == 0) {
			return;
		}
		// 将领用记录转换为盘库记录
		// 表头
		WOMInventoryRecord womInventoryRecord = new WOMInventoryRecord();
		womInventoryRecord.setInventoryDate(new Date());
		womInventoryRecord.setInFlag(false);
		womInventoryRecord.setOutFlag(false);
		inventoryRecordDao.save(womInventoryRecord);
		// 表体
		for (WOMStandingcropRef standingcropRef : standingcropRefs) {
			WOMInventoryDetail womInventoryDetail = new WOMInventoryDetail();
			// 领用人员
			womInventoryDetail.setApplyStaff(standingcropRef.getPrepareOrder().getApplyStaff());
			// 物料批号
			womInventoryDetail.setBatchText(standingcropRef.getBatchText());
			// 盘库记录表头
			womInventoryDetail.setHead(womInventoryRecord);
			// 内部编码
			womInventoryDetail.setInCode(standingcropRef.getPrepareOrder().getIncode());
			// 生产订单
			womInventoryDetail.setManulOrder(standingcropRef.getManulOrder());
			// 物料
			womInventoryDetail.setMaterial(standingcropRef.getGood());
			// 现存量
			womInventoryDetail.setOnhand(standingcropRef.getOnhand());
			// 出库单号
			womInventoryDetail.setOutCode(standingcropRef.getPrepareOrder().getOutCode());
			// 仓库
			womInventoryDetail.setWareID(standingcropRef.getWareID());

			inventoryDetailDao.save(womInventoryDetail);
		}

		// 调用sap接口入库
		flat(womInventoryRecord.getId(), "1");
	}

	@Override
	public void generateOutRecords() {
		// 获取上个月的盘库记录
		Calendar start = Calendar.getInstance();
		...
		List<WOMInventoryRecord> inventoryRecords = findInventoryRecordsByHql("from " + WOMInventoryRecord.JPA_NAME + " where inventoryDate between ? and ?", start.getTime(), end.getTime());
		if (inventoryRecords.size() > 0) {
			// 调用sap接口出库
			flat(inventoryRecords.get(0).getId(), "0");
		}
	}

	@Transactional
	@Override
	public Map<String, String> flat(Long recordId, String operateFlag) {
		Map<String, String> result = new HashMap<>();
		if (StringUtils.isBlank(operateFlag) || !("0".equals(operateFlag) || "1".equals(operateFlag))) {
			throw new BAPException("请选择操作类型（入库/出库）");
		}
		List<WOMInventoryDetail> inventoryDetails = inventoryDetailService.findInventoryDetailsByHql("from " + WOMInventoryDetail.JPA_NAME + " where head.id = ?", recordId);
		if (inventoryDetails.size() == 0) {
			result.put("flag", "true");
			return result;
		}
		WOMInventoryRecord womInventoryRecord = inventoryDetails.get(0).getHead();
		if ("0".equals(operateFlag) && womInventoryRecord.getOutFlag()) {
			result.put("flag", "false");
			result.put("message", "物料已出库，请勿重复操作");
			return result;
		}
		if ("1".equals(operateFlag) && womInventoryRecord.getInFlag()) {
			result.put("flag", "false");
			result.put("message", "物料已入库，请勿重复操作");
			return result;
		}

		// 将物料转换成SAP需要的格式
		List<SAPMaterial> sapMaterials = new ArrayList<>(inventoryDetails.size());
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		String now = sdf.format(new Date());
		for (WOMInventoryDetail inventoryDetail : inventoryDetails) {
			SAPMaterial temp = new SAPMaterial();
			temp.setBUDAT(now);
			temp.setBLDAT(now);
			temp.setMATNR(inventoryDetail.getMaterial().getProductCode());
			if (inventoryDetail.getWareID() != null) {
				temp.setLGORT(inventoryDetail.getWareID().getWareCode());
			}
			temp.setCHARG(inventoryDetail.getBatchText());
			// 出库0正数, 入库1负数
			String onhand = "1".equals(operateFlag) ? inventoryDetail.getOnhand().negate().toString() : inventoryDetail.getOnhand().toString();
			temp.setMENGE(onhand);
			temp.setMEINS(inventoryDetail.getMaterial().getProductBaseUnit().getCode());
			temp.setAUFNR(inventoryDetail.getManulOrder().getManulOrderNum());
          	temp.setSOBKZ(inventoryDetail.getEdition());
			//temp.setSOBKZ(inventoryDetail.getManulOrder().getEdition());
			temp.setZNBBH(inventoryDetail.getInCode());
			temp.setZCKDH(inventoryDetail.getOutCode());
			temp.setZLYRY(inventoryDetail.getApplyStaff().getCode());
			sapMaterials.add(temp);
		}
		// 调用sap接口
		String xml = XmlMapUtil.convertListToXml(sapMaterials, "DOCUMENT", "ROOT");
		Map<String, String> flagResult = womMesToSapService.flat(xml);
		if ("false".equals(flagResult.get("flag"))) {
			inventoryRecordDao.createNativeQuery("INSERT INTO outbound_wslogs(ID, CID, CREATE_TIME, ERROR_MES, FLAG, NAME, RETURN_DATA, SEND_DATA, WS_TYPE) VALUE(?, ?, ?, ?, ?, ?, ?, ?, ?)",
					new Date().getTime(),
					1000,
					new Date(),
					flagResult.get("mes"),
		...
		if ("0".equals(operateFlag)) {
			// 出库,修改领用记录,设置可用量为0
			for (WOMInventoryDetail inventoryDetail : inventoryDetails) {
				SQLQuery query = standingcropRefDao.createNativeQuery("UPDATE " + WOMStandingcropRef.TABLE_NAME + " SET onhand = onhand + " + inventoryDetail.getOnhand() + " WHERE MANUL_ORDER = " + inventoryDetail.getManulOrder().getId() + " AND GOOD = " + inventoryDetail.getMaterial().getId());
				query.executeUpdate();
			}
			// 修改盘库标志
			womInventoryRecord.setOutFlag(true);
		} else {
			// 入库,修改领用记录，恢复可用量
			for (WOMInventoryDetail inventoryDetail : inventoryDetails) {
				SQLQuery query = standingcropRefDao.createNativeQuery("UPDATE " + WOMStandingcropRef.TABLE_NAME + " SET onhand = 0 WHERE MANUL_ORDER = " + inventoryDetail.getManulOrder().getId() + " AND GOOD = " + inventoryDetail.getMaterial().getId());
				query.executeUpdate();
			}
			// 修改盘库标志
			womInventoryRecord.setInFlag(true);
		}
		inventoryRecordDao.save(womInventoryRecord);
		result.put("flag", "true");
		return result;
