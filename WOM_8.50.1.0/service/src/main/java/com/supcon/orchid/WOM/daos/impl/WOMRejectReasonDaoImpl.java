...
@Repository
public class WOMRejectReasonDaoImpl extends ExtendedGenericDaoImpl<WOMRejectReason, Long> implements WOMRejectReasonDao {
	@Override
	...
	@Override
	public WOMRejectReasonDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMRejectReasonDealInfo dealInfo = new WOMRejectReasonDealInfo();
	...
	@Override
	public WOMRejectReasonDealInfo copyAndSaveDealInfo(DealInfo di, WOMRejectReason mainObj, Staff dealStaff) {
		WOMRejectReasonDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMRejectReasonDealInfo.class);
		...
		List<WOMRejectReasonDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
