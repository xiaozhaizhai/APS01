...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchPhaseExelogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchPhaseExelog> {
	...
	private WOMBatchPhaseExelog batchPhaseExelog;
	...
	//导出excel时,主模型内容
	List<WOMBatchPhaseExelog> queryResults = new ArrayList<WOMBatchPhaseExelog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/batchPhaseExelog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/batchPhaseExelog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_BatchPhaseExelog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchPhaseExelog> batchPhaseExelogs;
	
	public List<WOMBatchPhaseExelog> getBatchPhaseExelogs() {
		return batchPhaseExelogs;
	}
	...
	private Page<WOMBatchPhaseExelog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchPhaseExelog = new WOMBatchPhaseExelog();
			if(batchPhaseExelog.getExeSystem() != null) {
				batchPhaseExelog.setExeSystem((SystemCode) systemCodeService.getSystemCode(batchPhaseExelog.getExeSystem().getId()));
			}
		} else {
			if(id > 0){
				batchPhaseExelog = batchPhaseExelogService.getBatchPhaseExelog(id, viewCode);
			}
		}
	...
	public Page<WOMBatchPhaseExelog> getPage() {
		if (null == page) {
			page = new Page<WOMBatchPhaseExelog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchPhaseExelog getBatchPhaseExelog() {
		return batchPhaseExelog;
	}		

	public void setBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog) {
		this.batchPhaseExelog = batchPhaseExelog;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/batchPhaseExelog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchPhaseExelogService.getBatchPhaseExelogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/batchPhaseExelog/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1496882746569
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchPhaseExelogReferenceCopy() throws Exception{
			WOMBatchPhaseExelog oldBatchPhaseExelog = null;
			if(isEditNew()){
				...
				oldBatchPhaseExelog = new WOMBatchPhaseExelog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchPhaseExelog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchPhaseExelog.getId());
				...
				oldBatchPhaseExelog = batchPhaseExelogService.getBatchPhaseExelog(id);
			}
			
			
			batchPhaseExelog = batchPhaseExelogService.getBatchPhaseExelog(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_BatchPhaseExelog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchPhaseExelog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchPhaseExelog, null);
						Method setValue = WOMBatchPhaseExelog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchPhaseExelog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchPhaseExelog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchPhaseExelogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchPhaseExelogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchPhaseExelog = batchPhaseExelogService.getBatchPhaseExelog(this.batchPhaseExelog.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchPhaseExelog.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchPhaseExelogService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchPhaseExelogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchPhaseExelogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchPhaseExelogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_BatchPhaseExelog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchPhaseExelog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchPhaseExelog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchPhaseExelog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchPhaseExelog\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchPhaseExelogService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchPhaseExelogService.findDataGridPage(dg,dgclass,getDgPage(),batchPhaseExelog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchPhaseExelog.getId() != null && batchPhaseExelog.getId() != -1){
				batchPhaseExelog = batchPhaseExelogService.getBatchPhaseExelog(batchPhaseExelog.getId());
			}
			batchPhaseExelogService.findDataGridPage(dg,dgclass,getDgPage(),batchPhaseExelog,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchPhaseExelog.getId()) {
		...
		batchPhaseExelogService.saveBatchPhaseExelog(batchPhaseExelog,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "submit");
		bap_validate_datagrids=batchPhaseExelogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchPhaseExelog.getId());
		...
		fileuploads.put("linkId", batchPhaseExelog.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/batchPhaseExelog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchPhaseExelogService.deleteBatchPhaseExelog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/batchPhaseExelog/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "restore");
		batchPhaseExelogService.restoreBatchPhaseExelog(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchPhaseExelog> findExportDataInfos(Page<WOMBatchPhaseExelog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchPhaseExelogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchPhaseExelogService.getAssProperty(p.getCode());	
									//String assModelCode = batchPhaseExelogService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchPhaseExelogService.findBatchPhaseExelogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchPhaseExelogService.findBatchPhaseExelogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchPhaseExelog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchPhaseExelog result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchPhaseExelog || null == this.batchPhaseExelog.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/batchPhaseExelog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/batchPhaseExelog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/batchPhaseExelog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/batchPhaseExelog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchPhaseExelogService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/batchPhaseExelog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_BatchPhaseExelog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_BatchPhaseExelog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/batchPhaseExelog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/batchPhaseExelog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_BatchPhaseExelog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchPhaseExelog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/batchPhaseExelog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
		_auditLog.setColumnStringA(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
		_auditLog.setColumnStringC(batchPhaseExelog.getId().toString());
		_auditLog.setColumnStringD(WOMBatchPhaseExelog.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchPhaseExelogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchPhaseExelog,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchPhaseExelog,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchPhaseExelog);
		...
		printlog.setBusinessId(batchPhaseExelog.getId());
		printlog.setBussinessKey(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
		printlog.setMainDisplay(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/batchPhaseExelog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchPhaseExelog,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/batchPhaseExelog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchPhaseExelogService.getBatchPhaseExelog(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/batchPhaseExelog/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/batchPhaseExelog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchPhaseExelog,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchPhaseExelog = batchPhaseExelogService.getBatchPhaseExelog(Long.parseLong(arrId[i]));
				if(null != batchPhaseExelog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchPhaseExelog.getId() ? "" : "," + batchPhaseExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchPhaseExelog.getId() ? "" : "," + batchPhaseExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchPhaseExelog.getId() ? "" : "," + batchPhaseExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchPhaseExelog,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchPhaseExelog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchPhaseExelogService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/batchPhaseExelog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_BatchPhaseExelog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_BatchPhaseExelog")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_BatchPhaseExelog");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_BatchPhaseExelog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchPhaseExelogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
