...
import com.supcon.orchid.WOM.services.WOMBatchProduceTaskImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchProduceTaskDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
...
@Service("wOM_batchProduceTaskService")
@Transactional
public class WOMBatchProduceTaskServiceImpl  extends BaseServiceImpl<WOMBatchProduceTask> implements WOMBatchProduceTaskService,IScriptService, InitializingBean, DisposableBean {
	...
	@Autowired
	private WOMBatchProduceTaskVariablesProvider variablesProvider;
	@Autowired
	...
	@Resource
	private IBAPBaseService<WOMBatchProduceTask> bapBaseService;
	...
	@Autowired
	private WOMBatchProduceTaskImportService batchProduceTaskImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchProduceTask.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchProduceTaskDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchProduceTask.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchProduceTaskDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchProduceTask.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchProduceTask.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchProduceTaskDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchProduceTask.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchProduceTask.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchProduceTask> list = (List<WOMBatchProduceTask>) batchProduceTaskDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMBatchProduceTask getBatchProduceTask(long id){
		return getBatchProduceTask(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchProduceTask getBatchProduceTask(long id, String viewCode){
		WOMBatchProduceTask batchProduceTask = batchProduceTaskDao.load(id);

		// 一对多情况处理
		if(batchProduceTask != null){
			this.setHeadIdFactoryId(batchProduceTask, viewCode);
			this.setHeadIdProductId(batchProduceTask, viewCode);
			this.setHeadIdFormularId(batchProduceTask, viewCode);
			this.setHeadIdBulkBatchNum(batchProduceTask, viewCode);
			this.setHeadIdBulkProduct(batchProduceTask, viewCode);
			this.setSupervision(batchProduceTask);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchProduceTaskAsJSON(long id, String include){
		WOMBatchProduceTask batchProduceTask = batchProduceTaskDao.load(id);
		// 一对多情况处理
			this.setHeadIdFactoryId(batchProduceTask);
			this.setHeadIdProductId(batchProduceTask);
			this.setHeadIdFormularId(batchProduceTask);
			this.setHeadIdBulkBatchNum(batchProduceTask);
			this.setHeadIdBulkProduct(batchProduceTask);
			this.dealSupervision(batchProduceTask);

		if(batchProduceTask == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(batchProduceTask, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.delete",operType="3")
	public void deleteBatchProduceTask(WOMBatchProduceTask batchProduceTask){
		if(null != batchProduceTask && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
			AuditUtil.setColumnStringA(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
			AuditUtil.setColumnStringC(batchProduceTask.getId().toString());
		}
		beforeDeleteBatchProduceTask(batchProduceTask);
		batchProduceTaskDao.delete(batchProduceTask);
		afterDeleteBatchProduceTask(batchProduceTask);
		batchProduceTaskDao.flush();
		List<WOMBatchProduceTask> params = new ArrayList<WOMBatchProduceTask>();
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask));
		batchProduceTask.setBatchProdTaskPartList(batchProdTaskPartList);
		params.add(batchProduceTask);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchProduceTask");
		props.put("eventType", "delete");
		String delIds = "";
		// 一对多情况处理
		delIds = batchProduceTask.getHeadIdFactoryIdmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			...
			if (!idList.isEmpty()) {
				List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask), Restrictions.in("factoryId.id", idList), Restrictions.eq("valid", true));
				for (WOMBatchProdTaskPart item : tmpList) {
					batchProdTaskPartDao.deletePhysical(item);
				}
			}
		}
		delIds = batchProduceTask.getHeadIdProductIdmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask), Restrictions.in("productId.id", idList), Restrictions.eq("valid", true));
				for (WOMBatchProdTaskPart item : tmpList) {
					batchProdTaskPartDao.deletePhysical(item);
				}
			}
		}
		delIds = batchProduceTask.getHeadIdFormularIdmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask), Restrictions.in("formularId.id", idList), Restrictions.eq("valid", true));
				for (WOMBatchProdTaskPart item : tmpList) {
					batchProdTaskPartDao.deletePhysical(item);
				}
			}
		}
		delIds = batchProduceTask.getHeadIdBulkBatchNummultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask), Restrictions.in("bulkBatchNum.id", idList), Restrictions.eq("valid", true));
				for (WOMBatchProdTaskPart item : tmpList) {
					batchProdTaskPartDao.deletePhysical(item);
				}
			}
		}
		delIds = batchProduceTask.getHeadIdBulkProductmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask), Restrictions.in("bulkProduct.id", idList), Restrictions.eq("valid", true));
				for (WOMBatchProdTaskPart item : tmpList) {
					batchProdTaskPartDao.deletePhysical(item);
				}
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.delete",operType="3")
	public void deleteBatchProduceTask(List<Long> batchProduceTaskIds){
		deleteBatchProduceTask(batchProduceTaskIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.delete",operType="3")
	public void deleteBatchProduceTask(List<Long> batchProduceTaskIds, String eventTopic) {
		List<WOMBatchProduceTask> batchProduceTasks = new ArrayList<WOMBatchProduceTask>();
		for(Long batchProduceTaskId : batchProduceTaskIds){
			WOMBatchProduceTask batchProduceTask = getBatchProduceTask(batchProduceTaskId);
			batchProduceTasks.add(batchProduceTask);
			if(null != batchProduceTask && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchProduceTask.getId() ? "" : "," + batchProduceTask.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchProduceTask.getTableNo() ? "" : "," + batchProduceTask.getTableNo().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchProduceTask.getId() ? "" : "," + batchProduceTask.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchProduceTasks != null && batchProduceTasks.size() > 0 ){
			for(WOMBatchProduceTask batchProduceTask : batchProduceTasks){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchProduceTask, batchProduceTask.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(batchProduceTaskIds != null && batchProduceTaskIds.size() > 0) {
			String hql = "update " + WOMBatchProduceTask.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = batchProduceTaskDao.createQuery(hql);
			query.setParameterList("ids", batchProduceTaskIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchProduceTask batchProduceTaskz : batchProduceTasks){
				batchProduceTaskz.setValid(false);
				List<WOMBatchProduceTask> params = new ArrayList<WOMBatchProduceTask>();
				List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTaskz));
				batchProduceTaskz.setBatchProdTaskPartList(batchProdTaskPartList);
				params.add(batchProduceTaskz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_batchProduceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.delete",operType="3")
	public void deleteBatchProduceTask(long batchProduceTaskId, int batchProduceTaskVersion){
		this.deleteBatchProduceTask(batchProduceTaskId, batchProduceTaskVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.delete",operType="3")
	public void deleteBatchProduceTask(long batchProduceTaskId, int batchProduceTaskVersion,SignatureLog signatureLog){
		WOMBatchProduceTask batchProduceTask = getBatchProduceTask(batchProduceTaskId);
		...
		beforeDeleteBatchProduceTask(batchProduceTask);
		batchProduceTaskDao.delete(batchProduceTaskId, batchProduceTaskVersion);
		afterDeleteBatchProduceTask(batchProduceTask);
		batchProduceTaskDao.flush();
		List<WOMBatchProduceTask> params = new ArrayList<WOMBatchProduceTask>();
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask));
		...
		if(signatureLog != null ) {
			...
			businessKey=batchProduceTask.getTableNo();
			signatureLog.setTableId(batchProduceTask.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.delete",operType="3")
	public void deleteBatchProduceTask(String batchProduceTaskIds){
		this.deleteBatchProduceTask(batchProduceTaskIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.delete",operType="3")
	public void deleteBatchProduceTask(String batchProduceTaskIds,SignatureLog signatureLog){
		deleteCollection(batchProduceTaskIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchProduceTask(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_batchProduceTask#modelCode:WOM_7.5.0.0_batchProduceTask_BatchProduceTask#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.engine.restore",operType="10")
	public void restoreBatchProduceTask(String batchProduceTaskIds){
		restoreCollection(batchProduceTaskIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchProduceTask(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.engine.restore",operType="10")
	public void restoreBatchProduceTask(long batchProduceTaskId){
		
		findBusinessKeyUsed(batchProduceTaskId);	//判断业务主键是否重复
		...
		beforeRestoreBatchProduceTask(batchProduceTask);
		batchProduceTask.setValid(true);
		batchProduceTaskDao.update(batchProduceTask);
		afterRestoreBatchProduceTask(batchProduceTask);
		List<WOMBatchProduceTask> params = new ArrayList<WOMBatchProduceTask>();
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask));
		batchProduceTask.setBatchProdTaskPartList(batchProdTaskPartList);
		params.add(batchProduceTask);
	...
	public void findBusinessKeyUsed(long batchProduceTaskId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_batchProduceTask_BatchProduceTask");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchProduceTask.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchProduceTask.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchProduceTaskDao.createNativeQuery(sql,batchProduceTaskId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchProduceTask(List<WOMBatchProduceTask>  batchProduceTasks){
		for(WOMBatchProduceTask batchProduceTask:batchProduceTasks)  {
			saveBatchProduceTask(batchProduceTask, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchProduceTask(List<WOMBatchProduceTask>  batchProduceTasks){
		for(WOMBatchProduceTask batchProduceTask:batchProduceTasks)  {
			saveBatchProduceTask(batchProduceTask, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.save")
	public void saveBatchProduceTask(WOMBatchProduceTask batchProduceTask, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchProduceTask(batchProduceTask, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.save")
	public void saveBatchProduceTask(WOMBatchProduceTask batchProduceTask, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchProduceTask(batchProduceTask, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.dealInfo.superEdit",operType="4")
	public void saveSuperEditBatchProduceTask(WOMBatchProduceTask batchProduceTask, WorkFlowVar workFlowVar,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode, boolean... isImport){
		this.saveSuperEditBatchProduceTask(batchProduceTask, workFlowVar, dgLists, dgDeleteIDs, assFileUploads, viewCode, null, isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.dealInfo.superEdit",operType="4")
	public void saveSuperEditBatchProduceTask(WOMBatchProduceTask batchProduceTask, WorkFlowVar workFlowVar,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode, SignatureLog signatureLog, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_batchProduceTask";
		...
		saveSingleSuperEditBatchProduceTask(workFlowVar, dgLists, dgDeleteIDs, assFileUploads, view, entityCode, events, batchProduceTask, isImport);
		if(null != batchProduceTask && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
			AuditUtil.setColumnStringA(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
			AuditUtil.setColumnStringC(batchProduceTask.getId().toString());
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.upload.importFlow",operType="4")
	public void importBatchProduceTaskWorkFlow(WOMBatchProduceTask batchProduceTask, String viewCode){
		 WorkFlowVar workFlowVar =new WorkFlowVar();
		 workFlowVar.setOperateType("submit");
		 saveSuperEditBatchProduceTask(batchProduceTask, workFlowVar, null,null,null, viewCode, null,true);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.batch.upload.importFlow",operType="4")
	public void batchImportBatchProduceTaskWorkFlow(List<WOMBatchProduceTask>  batchProduceTasks, View view){
		 WorkFlowVar workFlowVar =new WorkFlowVar();
		 workFlowVar.setOperateType("submit");
		 batchSaveSuperEditBatchProduceTask(batchProduceTasks, workFlowVar, null,null,null, view, true);
	}
	...
	public void saveSingleSuperEditBatchProduceTask(WorkFlowVar workFlowVar,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, View view, String entityCode, List<Event> events,WOMBatchProduceTask batchProduceTask, boolean... isImport) {
		if(events != null && events.size() > 0){
			executeGScript(entityCode, "beforeSubmit", events, batchProduceTask);
		}
		ReflectUtils.filterObjectIdIsNVL(batchProduceTask);
		if (null == workFlowVar) {
		...
		workFlowVar.setEntityCode("WOM_7.5.0.0_batchProduceTask");
		workFlowVar.setTableName(WOMBatchProduceTask.TABLE_NAME);
		workFlowVar.setScriptExcuteBeanName("wOM_batchProduceTaskService");
		workFlowVar.setStatus(batchProduceTask);
		workFlowVar.setCrossCompanyFlag(false);
		workFlowVar.setGroupEnabled(false);
		batchProduceTask.setWorkFlowVar(workFlowVar);
		
		boolean isSuperAdd = false;
		if (batchProduceTask.getId() == null) {
			batchProduceTask.setStatus(EntityTableInfo.STATUS_EFFECTED);
			batchProduceTask.setEffectiveState(1);
			isSuperAdd = true;
		}
		beforeSubmitBatchProduceTask(batchProduceTask, workFlowVar, true);
		batchSaveBatchProduceTask(batchProduceTask,dgLists, dgDeleteIDs,assFileUploads, view,events, null,isImport);
		DealInfo di = taskService.generateSuperEditDealInfo(batchProduceTask.getTableInfoId(), isSuperAdd,isImport);
		batchProduceTaskDao.flush();
		workFlowVar.setModelId(batchProduceTask.getId());
		workFlowVar.setTableInfoId(batchProduceTask.getTableInfoId());
		workFlowVar.setInitiatorPositionId(batchProduceTask.getCreatePositionId());
		workFlowVar.setOwnerId(batchProduceTask.getOwnerStaffId());
		workFlowVar.setOwnerPositionId(batchProduceTask.getOwnerPositionId());
		workFlowVar.setTableNo(batchProduceTask.getTableNo());
		
		afterSubmitProcessBatchProduceTask(batchProduceTask, di, workFlowVar, view.getCode(), true);
		afterSubmitBatchProduceTask(batchProduceTask, workFlowVar, true);
		if (view != null) {
			if(events != null && events.size() > 0){
				executeGScript(entityCode, "afterSubmit", events, batchProduceTask);
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchProduceTask(WOMBatchProduceTask batchProduceTask,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads ,View view,List<Event>  events, String eventTopic, boolean... isImport){
		...
		props.put("entityCode", "WOM_7.5.0.0_batchProduceTask");
		String url = null;
		if(batchProduceTask.getId()!=null && batchProduceTask.getId()>0){
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_batchProduceTask/modify";
		}else{
			props.put("eventType", "add");
			url = "com/supcon/orchid/entities/sync/wOM_7500_batchProduceTask/add";
		}
		Boolean viewIsView = false;
		if(view != null ){
			viewIsView = (view.getType() == ViewType.VIEW);
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchProduceTask);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchProduceTask);
		beforeSaveBatchProduceTask(batchProduceTask, viewIsView);

		if (null == batchProduceTask.getId()) {
			if(null == batchProduceTask.getStatus()){
				batchProduceTask.setStatus(EntityTableInfo.STATUS_RUNNING);
			}
			batchProduceTask.setTableNo(generateTableNo());
			if (viewIsView) {
				batchProduceTaskDao.saveWithRevertVersion(batchProduceTask);
			} else {
				batchProduceTaskDao.save(batchProduceTask);
			}
			// 保存TableInfo,如果是基础不保存
			EntityTableInfo ti = getTableInfo(batchProduceTask,isImport);
			batchProduceTaskDao.saveTableInfo(ti);
			batchProduceTask.setTableInfoId(ti.getId());
			if(batchProduceTask.getOwnerStaffId()==null){
				batchProduceTask.setOwnerStaffId(batchProduceTask.getCreateStaffId());
				ti.setOwnerStaffId(batchProduceTask.getCreateStaffId());
			}
			if(batchProduceTask.getOwnerDepartmentId()==null){
				batchProduceTask.setOwnerDepartmentId(batchProduceTask.getCreateDepartmentId());
				ti.setOwnerDepartmentId(batchProduceTask.getCreateDepartmentId());
			}
			if(batchProduceTask.getOwnerPositionId()==null){
				batchProduceTask.setOwnerPositionId(batchProduceTask.getCreatePositionId());
				ti.setOwnerPositionId(batchProduceTask.getCreatePositionId());
			}
			batchProduceTask.setCreateStaffId(batchProduceTask.getCreateStaffId());
			batchProduceTask.setCreatePositionId(batchProduceTask.getCreatePositionId());
			batchProduceTask.setCreateDepartmentId(batchProduceTask.getCreateDepartmentId());
			// 当公司为空时，默认插入当前公司
			if (batchProduceTask.getCompany() == null) {
				batchProduceTask.setCompany(getCurrentCompany());
			}
			if (batchProduceTask.getCid() == null || batchProduceTask.getCid() == -1l) {
				batchProduceTask.setCid(getCurrentCompanyId());
			}
			...
			if (viewIsView) {
				...
				batchProduceTaskDao.merge(batchProduceTask);
			}
		}
		dealDatagridsSave(batchProduceTask,view.getCode(),dgLists,dgDeleteIDs,assFileUploads);
		
		// 一对多情况处理
		this.dealHeadIdFactoryId(batchProduceTask);
		this.dealHeadIdProductId(batchProduceTask);
		this.dealHeadIdFormularId(batchProduceTask);
		this.dealHeadIdBulkBatchNum(batchProduceTask);
		this.dealHeadIdBulkProduct(batchProduceTask);
		this.dealSupervision(batchProduceTask);
		
		// 根据配置规则生成编码
		try {
			generateBatchProduceTaskCodes(batchProduceTask, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchProduceTaskSummarys(batchProduceTask, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchProduceTask(batchProduceTask, viewIsView);
		

		if(view != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchProduceTask);
			}
		}
		List<WOMBatchProduceTask> params = new ArrayList<WOMBatchProduceTask>();
		batchProduceTaskDao.flush();
		batchProduceTaskDao.clear();
		batchProduceTask = batchProduceTaskDao.load(batchProduceTask.getId());
	...
	public void generateBatchProduceTaskCodes(WOMBatchProduceTask batchProduceTask) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchProduceTaskCodes(batchProduceTask, false);
	}
	
	public void generateBatchProduceTaskCodes(WOMBatchProduceTask batchProduceTask, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchProduceTaskSummarys(WOMBatchProduceTask batchProduceTask) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchProduceTaskSummarys(batchProduceTask, false);
	}

	public void generateBatchProduceTaskSummarys(WOMBatchProduceTask batchProduceTask, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void saveBatchProduceTask(WOMBatchProduceTask batchProduceTask,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchProduceTask(batchProduceTask, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void saveBatchProduceTask(WOMBatchProduceTask batchProduceTask,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchProduceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		...
		String url = null;
		if(batchProduceTask.getId() != null && batchProduceTask.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		dealDatagridsSave(batchProduceTask,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
			this.dealHeadIdFactoryId(batchProduceTask);
			this.dealHeadIdProductId(batchProduceTask);
			this.dealHeadIdFormularId(batchProduceTask);
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchProduceTask batchProduceTask){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchProduceTask", batchProduceTask);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void mergeBatchProduceTask(WOMBatchProduceTask batchProduceTask, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchProduceTask);
		beforeSaveBatchProduceTask(batchProduceTask);
		if (null == batchProduceTask.getId()) {
			batchProduceTask.setStatus(EntityTableInfo.STATUS_RUNNING);
			batchProduceTask.setTableNo(generateTableNo());
			batchProduceTaskDao.save(batchProduceTask);
			// 保存TableInfo,如果是基础不保存
				EntityTableInfo ti = getTableInfo(batchProduceTask);
				batchProduceTaskDao.saveTableInfo(ti);
				batchProduceTask.setTableInfoId(ti.getId());
				if(batchProduceTask.getOwnerStaffId()==null){
					batchProduceTask.setOwnerStaffId(batchProduceTask.getCreateStaffId());
		...
		dealDatagridsSave(batchProduceTask,null,dgLists,dgDeleteIDs,assFileUploads);
			...
			this.dealHeadIdBulkProduct(batchProduceTask);
			this.dealSupervision(batchProduceTask);
		afterSaveBatchProduceTask(batchProduceTask);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchProduceTask> findBatchProduceTasks(Page<WOMBatchProduceTask> page, Criterion... criterions) {
		return batchProduceTaskDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMBatchProduceTaskDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMBatchProduceTaskDealInfo> list=batchProduceTaskDao.createQuery("from " + WOMBatchProduceTaskDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMBatchProduceTaskDealInfo dealInfo) {
		batchProduceTaskDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMBatchProduceTaskDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) batchProduceTaskDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMBatchProduceTaskDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = batchProduceTaskDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = batchProduceTaskDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMBatchProduceTaskDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = batchProduceTaskDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="撤回",operType="4")
	public Pending recall(long tableInfoId) {
		Long modelId = null;
		WOMBatchProduceTask  batchProduceTask = null;
		List<Object> list = batchProduceTaskDao.createNativeQuery("select ID from " + WOMBatchProduceTask.TABLE_NAME + " where TABLE_INFO_ID = ?", tableInfoId).list();
		if (list != null && list.size() > 0) {
			if (list.get(0) != null) {
				modelId = ((Number) list.get(0)).longValue();
				batchProduceTask = this.getBatchProduceTask(modelId);
			}
		}
	...
	public boolean existsSupervision(Long tableInfoId, Long staffId) {
		List<WOMBatchProduceTaskSupervision> list = batchProduceTaskDao.findSupervision(tableInfoId, staffId);
		if(list!=null && !list.isEmpty()) {
			return true;
		}
		return false;
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void save(WOMBatchProduceTask batchProduceTask,Long deploymentId,Long pendingId,WorkFlowVar workFlowVar,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		save(batchProduceTask, deploymentId, pendingId, workFlowVar, dgLists,dgDeleteIDs,assFileUploads, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void save(WOMBatchProduceTask batchProduceTask, Long deploymentId,Long pendingId, WorkFlowVar workFlowVar, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode) {
		ReflectUtils.filterObjectIdIsNVL(batchProduceTask);
		boolean first = null == batchProduceTask.getId();
		if (first){
			if(workFlowVar==null){
				workFlowVar = new WorkFlowVar();
			}
			Long createStaffUserId = null;
		   	if(null != batchProduceTask.getCreateStaff() && null != batchProduceTask.getCreateStaff().getUser()){
		    	createStaffUserId = batchProduceTask.getCreateStaff().getUser().getId();
		   	}
		...
		batchProduceTask.setWorkFlowVar(workFlowVar);
		if(batchProduceTask.getDeploymentId() == null && deploymentId != null) {
			batchProduceTask.setDeploymentId(deploymentId);
			Deployment d = taskService.getDeployment(deploymentId);
			if(d != null) {
				batchProduceTask.setProcessKey(d.getProcessKey());
				batchProduceTask.setProcessVersion(d.getProcessVersion());
			}
		}

		saveBatchProduceTask(batchProduceTask, dgLists,dgDeleteIDs,assFileUploads, viewCode);
		if(null != batchProduceTask && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
			}
		...
		// SAVE也有DealInfo
		DealInfo di = taskService.doSaveDealInfo(batchProduceTask, instance, deploymentId, pendingId, workFlowVar);
		batchProduceTaskDao.copyAndSaveDealInfo(di, batchProduceTask, creatorService.getStaffFromSession());
		batchProduceTaskDao.flush(); //将Session的实例同步到数据库
		
		afterSaveProcessBatchProduceTask(batchProduceTask, di, workFlowVar, viewCode);
	}
	@Override
	...
	public void syncEntity(Long id, String type) {
		...
		batchProduceTaskDao.flush();
		batchProduceTaskDao.clear();
		WOMBatchProduceTask batchProduceTask = getBatchProduceTask(id);
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask));
		batchProduceTask.setBatchProdTaskPartList(batchProdTaskPartList);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");                                                                                                                   
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		return batchProduceTaskDao.findLastDealInfo(tableInfoId, activityName);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.submit",operType="4")
	public void submit(WOMBatchProduceTask batchProduceTask,Long deploymentId,Long pendingId,WorkFlowVar workFlowVar,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		submit(batchProduceTask, deploymentId, pendingId, workFlowVar, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.submit",operType="4")
	public void submit(WOMBatchProduceTask batchProduceTask, Long deploymentId, Long pendingId, WorkFlowVar workFlowVar, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode) {
		this.submit(batchProduceTask, deploymentId, pendingId, workFlowVar, dgLists,dgDeleteIDs,assFileUploads, null,null);
	}
	
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.submit",operType="4")
	public void submit(WOMBatchProduceTask batchProduceTask, Long deploymentId, Long pendingId, WorkFlowVar workFlowVar, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode,SignatureLog signatureLog) {
		String entityCode = "WOM_7.5.0.0_batchProduceTask";
		List<Event> events = null;
		if(viewCode != null){
			View view = viewServiceFoundation.getView(viewCode);
		...
		beforeSubmitBatchProduceTask(batchProduceTask, workFlowVar);
		saveBatchProduceTask(batchProduceTask, dgLists,dgDeleteIDs,assFileUploads, viewCode);
		if(null != batchProduceTask && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
		...
		Integer preStatus = batchProduceTask.getStatus();
		batchProduceTaskDao.flush(); //如果不flush无法更新effect信息
		workFlowVar.setModelId(batchProduceTask.getId());
		workFlowVar.setTableInfoId(batchProduceTask.getTableInfoId());
		workFlowVar.setInitiatorPositionId(batchProduceTask.getCreatePositionId());
		workFlowVar.setOwnerId(batchProduceTask.getOwnerStaffId());
		workFlowVar.setOwnerPositionId(batchProduceTask.getOwnerPositionId());
		workFlowVar.setTableNo(batchProduceTask.getTableNo());
		DealInfo di = taskService.take(pendingId, deploymentId, batchProduceTask.getId(), creatorService.getStaffFromSession(), workFlowVar);
		if(pendingId == null) {
			DealInfo saveDi = taskService.doSaveDealInfo(batchProduceTask, di, deploymentId, pendingId, workFlowVar, true);
			batchProduceTaskDao.copyAndSaveDealInfo(saveDi, batchProduceTask, creatorService.getStaffFromSession());
		}
		batchProduceTaskDao.copyAndSaveDealInfo(di, batchProduceTask, creatorService.getStaffFromSession());
		if(workFlowVar.getOutcomeMap()!=null&&workFlowVar.getOutcomeMap().size()>1){
			for(Map<String, ?> map:workFlowVar.getOutcomeMap()){
				...
				if(type.equals("notification")){
					...
					WOMBatchProduceTaskDealInfo batchProduceTaskDealInfo=batchProduceTaskDao.copyAndSaveDealInfo(di, batchProduceTask, creatorService.getStaffFromSession());
					batchProduceTaskDealInfo.setOutcome(outcome);
					batchProduceTaskDealInfo.setOutcomeDes(dec);
					if(assignUser!=null&&assignUser.length()>0){
						batchProduceTaskDealInfo.setAssignStaff(taskService.getAssignUserName(assignUser));
					}
				}
			}
		}
		...
		if (ecDealInfos != null && ecDealInfos.size() > 0) {
			for (DealInfo ecDealInfo : ecDealInfos) {
				batchProduceTaskDao.copyAndSaveDealInfo(ecDealInfo);
			}
			taskService.clearExpectedConsignDealInfos();
		}
		
		if(batchProduceTask.getStatus() == 99) {
			if(!preStatus.equals(batchProduceTask.getStatus())){
				dataAuditLogService.saveCustomerAudit("2", batchProduceTask, batchProduceTask.getId(), new Object[]{batchProduceTask.getStatus(),batchProduceTask.getEffectTime()}, new Object[]{preStatus,null}, new String[]{"status","effectTime"});
			}
			batchProduceTaskDao.deleteSupervision(batchProduceTask.getTableInfoId());
		}
		//将临时处理意见从cookie中删除
		String ctype = "TEMP_DEALINFO_" + batchProduceTask.getProcessKey() + "_" + di.getActivityName() + "_WOM_7.5.0.0_batchProduceTask_" + batchProduceTask.getId();
		...
		batchProduceTaskDao.flush(); //将Session中的处理意见同步到数据库
		afterSubmitProcessBatchProduceTask(batchProduceTask, di, workFlowVar, viewCode);
		afterSubmitBatchProduceTask(batchProduceTask, workFlowVar);
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSubmit", events, batchProduceTask);
			}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchProduceTasks(Page<WOMBatchProduceTask> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchProduceTasks(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchProduceTasks(Page<WOMBatchProduceTask> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchProduceTasks(Page<WOMBatchProduceTask> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchProduceTask> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchProduceTask\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if ("dataTable-sortColOrder".equals(param.getName())) {
						...
						if(type != Sql.TYPE_LIST_PENDING && sortOrderByStr.toString().length() > 0 && sortOrderByStr.toString().startsWith("\"p\".TASK_DESCRIPTION")) {
							sortOrderByStr.append(",").append("\"batchProduceTask\".STATUS ").append((String) param.getValue());
						}
						continue;
					}
					...
					if((advQuery == null || advQuery.length() == 0 || advQuery.indexOf("\"batchProduceTask\".STATUS") == -1) 
						&& (classifySql == null || classifySql.length() == 0 || classifySql.indexOf("\"batchProduceTask\".STATUS") == -1)) {
						s.append(" AND \"batchProduceTask\".STATUS <> 0");
					}
					if(type == Sql.TYPE_LIST_PENDING) {
						s.append(" AND \"batchProduceTask\".STATUS <> 77");
					}
					if (advQuery != null && advQuery.length() > 0) {
						s.append(" AND ").append(advQuery);
						if (advValues != null) {
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchProduceTask\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchProduceTask\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_batchProduceTask_BatchProduceTask" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_batchProduceTask_BatchProduceTask" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  batchProduceTaskDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchProduceTaskDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_batchProduceTask").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchProduceTask\"","WOM_7.5.0.0_batchProduceTask",customCondition,"WOM_BATCH_PRODUCE_TASKS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"batchProduceTask\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMBatchProduceTaskDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(type != Sql.TYPE_LIST_PENDING && sortOrderByStr.toString().length() > 0 && sortOrderByStr.toString().startsWith("\"p\".TASK_DESCRIPTION")) {
				totalSql.append(" LEFT OUTER JOIN ").append("(SELECT TASK_DESCRIPTION, TABLE_INFO_ID FROM ").append(Pending.TABLE_NAME).append(" WHERE ID IN (SELECT MAX(ID) FROM ").append(Pending.TABLE_NAME).append(" GROUP BY TABLE_INFO_ID))").append(" \"p\" ON \"p\".TABLE_INFO_ID= \"batchProduceTask\".TABLE_INFO_ID ");
			}
			if(queryType == 1) {
				s.append(" AND \"batchProduceTask\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchProduceTask\".LAY_REC ASC, \"batchProduceTask\".SORT ASC");
				} else {
					orderPart.append(" \"batchProduceTask\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchProduceTaskDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchProduceTaskDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchProduceTaskDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchProduceTask> result = new ArrayList<WOMBatchProduceTask>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchProduceTask.class,batchProduceTaskDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchProduceTask.class,batchProduceTaskDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchProduceTask batchProduceTask : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchProduceTask.getTableInfoId(), "Table");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchProduceTask.getTableInfoId(), "Table");
							if(null != documents && !documents.isEmpty()) {
								batchProduceTask.setDocument(documents.get(0));
								batchProduceTask.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(queryType == 0) {
					if (type == Sql.TYPE_LIST_QUERY) {
						if (!result.isEmpty()) {
							...
							for (WOMBatchProduceTask batchProduceTask : result)
								tIds.add(batchProduceTask.getTableInfoId());
							...
							for(int i = 0 ; i < count ; i++){
								plist.addAll(batchProduceTaskDao.createNativeQuery(pSql).setParameterList("ids", tIds.subList(PERTIME * i, PERTIME * i + (i < count - 1 ? PERTIME : (tIds.size() % PERTIME)))).list());
							}
							...
							for(int j = 0 ; j < count ; j++){
								mylist.addAll(batchProduceTaskDao.createNativeQuery(myPql).setParameterList("ids", tIds.subList(PERTIME * j, PERTIME * j + (j < count - 1 ? PERTIME : (tIds.size() % PERTIME)))).list());
							}
							// List<Object[]> plist = batchProduceTaskDao.createNativeQuery(pSql).setParameterList("ids", tIds).list();
							if (!plist.isEmpty()) {
							/*
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_batchProduceTask")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_batchProduceTask")+"WOM_7.5.0.0_batchProduceTask".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_batchProduceTask",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchProduceTask> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchProduceTask> result) {
		View view = null;
		...
		Map<Long, WOMBatchProduceTask> rsMap = new HashMap<Long, WOMBatchProduceTask>();
		for (WOMBatchProduceTask batchProduceTask : result) {
			ids.add(batchProduceTask.getId());
			rsMap.put(batchProduceTask.getId(), batchProduceTask);
		}
		for (Field f : fields) {
				...
				if (fieldMap.get("assoFlag") != null && "true".equalsIgnoreCase(fieldMap.get("assoFlag").toString())) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchProduceTaskDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchProduceTaskDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	
	public List<WOMBatchProduceTask> getTableInfoStatus(List<WOMBatchProduceTask> result, boolean isPendingQuery, Long ... deploymentId) {
		List<Long> tIds = new ArrayList<Long>(result.size());
		User currentUser = (User) getCurrentUser();
		...
		if (isPendingQuery) {
			for (WOMBatchProduceTask batchProduceTask : result) {
				if (batchProduceTask.getPending() != null) {
					if (batchProduceTask.getStatus() == 77) {
						batchProduceTask.getPending().setTaskDescription(hangUp);
					} else if (batchProduceTask.getPending().getProxySource() != null && !batchProduceTask.getPending().getProxySource().isEmpty()
							|| batchProduceTask.getPending().getSourceStaff() != null && !batchProduceTask.getPending().getSourceStaff().isEmpty()) {
						String statsName = batchProduceTask.getPending().getTaskDescription();
						statsName = InternationalResource.get(statsName, currentUser.getLanguage());
						statsName += "(" + dai + ")";
						batchProduceTask.getPending().setTaskDescription(statsName);
					}
				}
			}
		} else {
			Map<Long, WOMBatchProduceTask> map = new HashMap<>();
			for (WOMBatchProduceTask batchProduceTask : result) {
				tIds.add(batchProduceTask.getTableInfoId());
				map.put(batchProduceTask.getTableInfoId(), batchProduceTask);
				Pending pending = batchProduceTask.getPending();
				if (pending != null) {
			...
			int count = (tIds.size() / PERTIME) + (tIds.size() % PERTIME == 0 ? 0 : 1);
			for (int i = 0; i < count; i++) {
				plist.addAll(batchProduceTaskDao
			...
			for (Object[] o : plist) {
				long pTableInfoId = ((Number) o[0]).longValue();
				WOMBatchProduceTask batchProduceTask = map.get(pTableInfoId);
				Pending pending = batchProduceTask.getPending();
				if (pending == null) {
					pending = new Pending();
					batchProduceTask.setPending(pending);
				}
				String taskDesc = (o[1] != null) ? InternationalResource.get(o[1].toString(), currentUser.getLanguage()) : "";
				if (pending.getTaskDescription() != null) {
					pending.setTaskDescription(pending.getTaskDescription() + "," + taskDesc);
			...
			for (WOMBatchProduceTask batchProduceTask : result) {
				Pending p = batchProduceTask.getPending();
				if(p == null) {
					p = new Pending();
				}
				if (batchProduceTask.getStatus() == 99) {
				...
				} else if (batchProduceTask.getStatus() == 0) {
	...
	protected EntityTableInfo getTableInfo(WOMBatchProduceTask batchProduceTask,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();

		if(batchProduceTask.getDeploymentId() != null) {
			ti.setDeploymentId(batchProduceTask.getDeploymentId());
			ti.setProcessKey(batchProduceTask.getProcessKey());
			ti.setProcessVersion(batchProduceTask.getProcessVersion());
		}

		ti.setOwnerStaffId(batchProduceTask.getOwnerStaffId());
		ti.setOwnerPositionId(batchProduceTask.getOwnerPositionId());
		ti.setOwnerDepartmentId(batchProduceTask.getOwnerDepartmentId());
		
		ti.setCreateStaff(batchProduceTask.getCreateStaff());
		ti.setCreateStaffId(batchProduceTask.getCreateStaffId());
		ti.setCreateTime(batchProduceTask.getCreateTime());
		ti.setDeleteStaff(batchProduceTask.getDeleteStaff());
		ti.setDeleteStaffId(batchProduceTask.getDeleteStaffId());
		ti.setDeleteTime(batchProduceTask.getDeleteTime());
		ti.setCreateDepartmentId(batchProduceTask.getCreateDepartmentId());
		ti.setModifyStaff(batchProduceTask.getModifyStaff());
		ti.setModifyStaffId(batchProduceTask.getModifyStaffId());
		ti.setCreatePositionId(batchProduceTask.getCreatePositionId());
		ti.setPositionLayRec(batchProduceTask.getPositionLayRec());
		ti.setTableNo(batchProduceTask.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_batchProduceTask");
		if(isImport!=null&&isImport.length>0&&isImport[0]) {
			ti.setStatus(EntityTableInfo.STATUS_EFFECTED);
			ti.setEffectiveState(batchProduceTask.getEffectiveState());
		}else {
			ti.setStatus(batchProduceTask.getStatus());
			ti.setEffectiveState(batchProduceTask.getEffectiveState());
		}
		ti.setTargetTableName("WOM_BATCH_PRODUCE_TASKS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_batchProduceTask_BatchProduceTask" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchProduceTaskDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						if(sqlType!=null&&sqlType.trim().length()>0)  {
							if(Integer.valueOf(sqlType)==Sql.TYPE_LIST_PENDING) {	
								sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchProduceTask\".TABLE_INFO_ID ");
								sb.append(" WHERE  \"batchProduceTask\".STATUS <> 0  ");
								innerJoinAppendFlag = true;
							}else if(Integer.valueOf(sqlType)==Sql.TYPE_LIST_QUERY)  {
								
							}
						}
						powerSql = dataPermissionService.getPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchProduceTask\"","WOM_7.5.0.0_batchProduceTask","","WOM_BATCH_PRODUCE_TASKS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchProduceTask\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchProduceTask\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchProduceTask\"","WOM_7.5.0.0_batchProduceTask","","WOM_BATCH_PRODUCE_TASKS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
					}
					sb.append(powerSql);
			...
			mnecodeSql.append("  (\"").append("batchProduceTask.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchProduceTask\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchProduceTask\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchProduceTask\".CID = 1 OR \"batchProduceTask\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchProduceTask.mc\"").append(".BATCH_PRODUCE_TASK) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchProduceTaskDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("batchProduceTask", "WOM_7.5.0.0_batchProduceTask", "batchProduceTask_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchProduceTaskDao.createNativeQuery(sql, menuCode);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.retrial",operType="4")
	public void retrialThisTable(WOMBatchProduceTask batchProduceTask, Long tableInfoId,String scriptCode) {
		this.retrialThisTable(batchProduceTask, tableInfoId,scriptCode,null);
	}
	...
	public void retrialThisTable(WOMBatchProduceTask batchProduceTask, Long tableInfoId,String scriptCode,SignatureLog signatureLog) {
		beforeRetrial(batchProduceTask);
		batchProduceTask.setStatus(88);
		batchProduceTaskDao.save(batchProduceTask);
		batchProduceTaskDao.flush(); //如果不flush无法更新effect信息
		ProcessInstance instance = null;
		WOMBatchProduceTaskDealInfo del=getDealInfoEntity(tableInfoId);
		...
		batchProduceTaskDao.bulkExecute(hql, new Object[]{"WOM_7.5.0.0_batchProduceTask", del.getProcessKey(), del.getProcessVersion(), "notification"});
		taskService.completeTaskHistory(tableInfoId);
		List<Transition> transitions=processService.findFirstTransitions(deploymentId);
		WorkFlowVar workFlowVar = new WorkFlowVar();
		workFlowVar.setVariablesProvider(variablesProvider);
		workFlowVar.setModelId(batchProduceTask.getId());
		workFlowVar.setProcessInitiator(batchProduceTask.getCreateStaff().getUser().getId());
		workFlowVar.setTableInfoId(batchProduceTask.getTableInfoId());
		workFlowVar.setDeploymentId(deploymentId);
		workFlowVar.setEntityCode("WOM_7.5.0.0_batchProduceTask");
		workFlowVar.setInitiatorPositionId(batchProduceTask.getCreatePositionId());
		...
		instance = instanceService.start(deploymentId, null, workFlowVar);
		DealInfo di = taskService.doSaveDealInfo(batchProduceTask, instance, deploymentId, null, workFlowVar);
		if(null != batchProduceTask && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
			}
		...
		WOMBatchProduceTaskDealInfo dl=new WOMBatchProduceTaskDealInfo();
		...
		staff.setId(batchProduceTask.getOwnerStaffId());
		dl.setStaff(staff);
		dl.setUserId(di.getUserId());
		dl.setDealInfoType(DealInfoType.NORMAL);
		batchProduceTaskDao.saveDealInfo(dl);
		batchProduceTaskDao.createNativeQuery("update EC_TABLE_INFO set STATUS=88 where id=?", batchProduceTask.getTableInfoId()).executeUpdate();
		if(null!=scriptCode&&!"".equals(scriptCode)){
			doScript(scriptCode,batchProduceTask);
		}
		afterRetrial(batchProduceTask);
		//crossCompanyDao.copyAndSaveDealInfo(di, batchProduceTask, creatorService.getStaffFromSession());
		if(signatureLog != null ) {
			Object businessKey=null;
			businessKey=batchProduceTask.getTableNo();
	...
	public void doScript(String code ,WOMBatchProduceTask batchProduceTask){

		Map<String, Object> variables = new HashMap<String, Object>();
		variables.put("db", DbUtils.getInstance());
		variables.put("batchProduceTask", batchProduceTask);
		try {
			Script script = scriptService.get("WOM_7.5.0.0_batchProduceTask", code);
			if (null == script || null == script.getCode()){
	...
	public void bulkSubmit(String ids,Long deploymentId,String pendingIds,String outcomeStr){
		...
		for(int i=0;i<idArr.length;i++){
			WOMBatchProduceTask batchProduceTask=batchProduceTaskDao.load(Long.parseLong(idArr[i]));
			...
			workFlowVar.setOwnerPositionId(batchProduceTask.getOwnerPositionId());
			workFlowVar.setTableNo(batchProduceTask.getTableNo());
			workFlowVar.setTableName(batchProduceTask.TABLE_NAME);
			workFlowVar.setScriptExcuteBeanName("wOM_batchProduceTaskService");
			workFlowVar.setStatus(batchProduceTask);
			workFlowVar.setCrossCompanyFlag(false);
			workFlowVar.setGroupEnabled(false);
	...
	// ================ 生成多选控件数据保存方法 start====================
	private void dealHeadIdFactoryId(WOMBatchProduceTask batchProduceTask) {
		String addIds = batchProduceTask.getHeadIdFactoryIdAddIds();
		String delIds = batchProduceTask.getHeadIdFactoryIdDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					WOMBatchProdTaskPart tmp = new WOMBatchProdTaskPart();
					tmp.setHeadId(batchProduceTask);
					MESBasicFactoryModel indirectObj = new MESBasicFactoryModel();
					indirectObj.setId(Long.valueOf(item));
					tmp.setFactoryId(indirectObj);
					tmp.setCid(batchProduceTask.getCid());
					batchProdTaskPartDao.save(tmp);
				}
			}
		}
	}

	private void setHeadIdFactoryId(WOMBatchProduceTask batchProduceTask) {
		setHeadIdFactoryId(batchProduceTask, null);
	}

	private void setHeadIdFactoryId(WOMBatchProduceTask batchProduceTask, String viewCode) {
		List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Order.asc("id"), Restrictions.eq("headId", batchProduceTask), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (WOMBatchProdTaskPart item : tmpList) {
		...
		batchProduceTask.setBatchProdTaskPartList(tmpList);
		batchProduceTask.setHeadIdFactoryIdmultiselectIDs(ids);
		batchProduceTask.setHeadIdFactoryIdmultiselectNames(names);
	}
	private void dealHeadIdProductId(WOMBatchProduceTask batchProduceTask) {
		String addIds = batchProduceTask.getHeadIdProductIdAddIds();
		String delIds = batchProduceTask.getHeadIdProductIdDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
	...
	private void setHeadIdProductId(WOMBatchProduceTask batchProduceTask) {
		setHeadIdProductId(batchProduceTask, null);
	}

	private void setHeadIdProductId(WOMBatchProduceTask batchProduceTask, String viewCode) {
		List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Order.asc("id"), Restrictions.eq("headId", batchProduceTask), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (WOMBatchProdTaskPart item : tmpList) {
		...
		batchProduceTask.setBatchProdTaskPartList(tmpList);
		batchProduceTask.setHeadIdProductIdmultiselectIDs(ids);
		batchProduceTask.setHeadIdProductIdmultiselectNames(names);
	}
	private void dealHeadIdFormularId(WOMBatchProduceTask batchProduceTask) {
		String addIds = batchProduceTask.getHeadIdFormularIdAddIds();
		String delIds = batchProduceTask.getHeadIdFormularIdDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
	...
	private void setHeadIdFormularId(WOMBatchProduceTask batchProduceTask) {
		setHeadIdFormularId(batchProduceTask, null);
	}

	private void setHeadIdFormularId(WOMBatchProduceTask batchProduceTask, String viewCode) {
		List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Order.asc("id"), Restrictions.eq("headId", batchProduceTask), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (WOMBatchProdTaskPart item : tmpList) {
		...
		batchProduceTask.setBatchProdTaskPartList(tmpList);
		batchProduceTask.setHeadIdFormularIdmultiselectIDs(ids);
		batchProduceTask.setHeadIdFormularIdmultiselectNames(names);
	}
	private void dealHeadIdBulkBatchNum(WOMBatchProduceTask batchProduceTask) {
		String addIds = batchProduceTask.getHeadIdBulkBatchNumAddIds();
		String delIds = batchProduceTask.getHeadIdBulkBatchNumDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					...
					indirectObj.setId(Long.valueOf(item));
					tmp.setBulkBatchNum(indirectObj);
					tmp.setCid(batchProduceTask.getCid());
					batchProdTaskPartDao.save(tmp);
				}
			}
	...
	private void setHeadIdBulkBatchNum(WOMBatchProduceTask batchProduceTask) {
		setHeadIdBulkBatchNum(batchProduceTask, null);
	}

	private void setHeadIdBulkBatchNum(WOMBatchProduceTask batchProduceTask, String viewCode) {
		List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Order.asc("id"), Restrictions.eq("headId", batchProduceTask), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (WOMBatchProdTaskPart item : tmpList) {
			if(item.getBulkBatchNum() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getBulkBatchNum().getId();
				String displayField = "batchText";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getBulkBatchNum(), displayField);
			}
		}
		batchProduceTask.setBatchProdTaskPartList(tmpList);
		batchProduceTask.setHeadIdBulkBatchNummultiselectIDs(ids);
		batchProduceTask.setHeadIdBulkBatchNummultiselectNames(names);
	}
	private void dealHeadIdBulkProduct(WOMBatchProduceTask batchProduceTask) {
		String addIds = batchProduceTask.getHeadIdBulkProductAddIds();
		String delIds = batchProduceTask.getHeadIdBulkProductDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					...
					MESBasicProduct indirectObj = new MESBasicProduct();
					indirectObj.setId(Long.valueOf(item));
					tmp.setBulkProduct(indirectObj);
					tmp.setCid(batchProduceTask.getCid());
					batchProdTaskPartDao.save(tmp);
				}
			}
	...
	private void setHeadIdBulkProduct(WOMBatchProduceTask batchProduceTask) {
		setHeadIdBulkProduct(batchProduceTask, null);
	}

	private void setHeadIdBulkProduct(WOMBatchProduceTask batchProduceTask, String viewCode) {
		List<WOMBatchProdTaskPart> tmpList = batchProdTaskPartDao.findByCriteria(Order.asc("id"), Restrictions.eq("headId", batchProduceTask), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (WOMBatchProdTaskPart item : tmpList) {
			if(item.getBulkProduct() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getBulkProduct().getId();
				String displayField = "productName";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getBulkProduct(), displayField);
			}
		}
		batchProduceTask.setBatchProdTaskPartList(tmpList);
		batchProduceTask.setHeadIdBulkProductmultiselectIDs(ids);
		batchProduceTask.setHeadIdBulkProductmultiselectNames(names);
	}
	private void dealSupervision(WOMBatchProduceTask batchProduceTask) {
		String addIds = batchProduceTask.getSupervisionAddIds();
		String delIds = batchProduceTask.getSupervisionDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				batchProduceTaskDao.deleteSupervision(batchProduceTask.getTableInfoId(), idList);
			}
		}
		if (addIds != null && addIds.length() > 0) {
			String[] ids = addIds.split(",");
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					...
					tmp.setTableInfoId(batchProduceTask.getTableInfoId());
					Staff indirectObj = new Staff();
					indirectObj.setId(Long.valueOf(item));
					tmp.setStaff(indirectObj);
					batchProduceTaskDao.addSupervision(tmp, batchProduceTask);
	...
	private void setSupervision(WOMBatchProduceTask batchProduceTask) {
		List<WOMBatchProduceTaskSupervision> tmpList = batchProduceTaskDao.findSupervision(batchProduceTask.getTableInfoId());
		String ids = "";
		String names = "";
		for (WOMBatchProduceTaskSupervision item : tmpList) {
		...
		batchProduceTask.setSupervisionList(tmpList);
		batchProduceTask.setSupervisionmultiselectIDs(ids);
		batchProduceTask.setSupervisionmultiselectNames(names);
	}
	// ================ 生成多选控件数据保存方法 end ====================

	protected String getEntityCode(){return WOMBatchProduceTask.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchProduceTask.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchProduceTask.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchProduceTask 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchProduceTask batchProduceTask){
		if(null == batchProduceTask.getTableNo() || StringUtils.isEmpty(batchProduceTask.getTableNo().toString())){
			return;
		}
		if(batchProduceTask.getId()==null ){
			List<WOMBatchProduceTask> batchProduceTasks = batchProduceTaskDao.findByCriteria(Restrictions.eq("tableNo", batchProduceTask.getTableNo()));
			if(batchProduceTasks!=null && !batchProduceTasks.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"ec.common.tableNo", getCurrentUser().getLanguage()), batchProduceTask.getTableNo());
			}
		} else {
			List<WOMBatchProduceTask> batchProduceTasks = batchProduceTaskDao.findByCriteria(Restrictions.ne("id", batchProduceTask.getId()), Restrictions.eq("tableNo", batchProduceTask.getTableNo()));
			if(batchProduceTasks!=null && !batchProduceTasks.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"ec.common.tableNo", getCurrentUser().getLanguage()), batchProduceTask.getTableNo());
			}
	...
	@Override
	public List<WOMBatchProduceTask> getBatchProduceTasks(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchProduceTask.class);
		if(params!=null){
			...
			return batchProduceTaskDao.findByHql("from " + WOMBatchProduceTask.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchProduceTaskDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMBatchProduceTask> findBatchProduceTasksByHql(String hql, Object... objects){
		...
		return   batchProduceTaskDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchProduceTask loadBatchProduceTaskByBussinessKey(WOMBatchProduceTask batchProduceTask){
		return loadBatchProduceTaskByBussinessKey(batchProduceTask.getTableNo());
	}
	
	@Override
	public Page<WOMBatchProduceTask> getBatchProduceTasks(Page<WOMBatchProduceTask> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchProduceTask.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchProduceTaskDao.findByPage(page, sql);
			}else{
				page = batchProduceTaskDao.findAllByPage(page);
			}
			return page;
		}
		page = batchProduceTaskDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public WOMBatchProduceTask loadBatchProduceTaskByBussinessKey(Serializable bussinessKey){
		return batchProduceTaskDao.findEntityByCriteria(Restrictions.eq("tableNo", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchProduceTask> findByProperty(String propertyName, Object object){
		return batchProduceTaskDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchProduceTask findEntityByProperty(String propertyName, Object object){
		return batchProduceTaskDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchProduceTask.JPA_NAME + " set valid=0 where tableNo in (:businessKeys)";
			Query query = batchProduceTaskDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_batchProduceTask_BatchProduceTask","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_batchProduceTask", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMBatchProduceTask batchProduceTask,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchProduceTaskDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchProduceTask.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchProduceTask);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(batchProduceTask.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(batchProduceTask.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(batchProduceTask.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchProduceTask.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchProduceTask.TABLE_NAME +" where valid = 1";
			list = batchProduceTaskDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchProduceTask.TABLE_NAME;
			list = batchProduceTaskDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchProduceTask.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchProduceTaskDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchProduceTaskDaoImpl batchProduceTaskDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMBatchProduceTask.TABLE_NAME + " where id = ?";
		String tableNo = batchProduceTaskDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	...
	public List<String> getTableNoData(){
		...
		String sql = "select table_no from " + WOMBatchProduceTask.TABLE_NAME;
		list = batchProduceTaskDao.createNativeQuery(sql).list();
		return list;
	}
	...
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchProduceTaskWorkFlow(final List<WOMBatchProduceTask> insertObjs, final List<WOMBatchProduceTask> updateObjs, 
		...
		return this.importBatchBatchProduceTaskWorkFlow(insertObjs, updateObjs, columnInfo, entityCode, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchProduceTaskWorkFlow(final List<WOMBatchProduceTask> insertObjs, final List<WOMBatchProduceTask> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1495089898131"),importCount));
		...
		Map<Long, WOMBatchProduceTask> oldUpdateObjsMap = new HashMap<Long, WOMBatchProduceTask>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchProduceTask bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchProduceTask> bizObjList = batchProduceTaskDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchProduceTask bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchProduceTask> nmeObjs = new ArrayList<WOMBatchProduceTask>();
											
		for(WOMBatchProduceTask batchProduceTask:insertObjs){//没有table_no的新增项，系统生成
			if(batchProduceTask.getTableNo() == null || (batchProduceTask.getTableNo() != null && batchProduceTask.getTableNo().equals(""))){
				batchProduceTask.setTableNo(generateTableNo());	
			}
		}
		
		Map<Object, Long> m = new HashMap<Object, Long>();
		//String idSql = "select id from " + WOMBatchProduceTask.TABLE_NAME + " order by id desc";
		//Query query = batchProduceTaskDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMBatchProduceTask.JPA_NAME + " order by id desc";
		Query query =  batchProduceTaskDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMBatchProduceTask.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchProduceTask batchProduceTask:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchProduceTask.TABLE_NAME,1,dbId);
			if(batchProduceTask.getTableNo() != null && !batchProduceTask.getTableNo().equals("") ){
				m.put(batchProduceTask.getTableNo(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchProduceTaskDao.createNativeQuery(sql, "WOM_7.5.0.0_batchProduceTask_BatchProduceTask"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		batchProduceTaskDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchProduceTask.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMBatchProduceTask batchProduceTask = updateObjs.get(updateIndex);
						Long id = batchProduceTask.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchProduceTask.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchProduceTask);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchProduceTask.TABLE_NAME + " (";
					...
					insertSql = insertSql.substring(0, insertSql.length()-1);
					insertSql += ")";
					String insertDiSql = "INSERT INTO " + WOMBatchProduceTask.TABLE_NAME + "_DI " +
					...
					Long diId = autoGeneratorID.getNextId(WOMBatchProduceTask.TABLE_NAME + "_DI",insSize, "ID");
					Long dealInfoId = autoGeneratorID.getNextId("WF_DEAL_INFO",insSize);
					
					for(int insertIndex = 0; insertIndex<insSize; insertIndex++)  {
						WOMBatchProduceTask batchProduceTask = insertObjs.get(insertIndex);
						...
						if(!colExclude.contains(4))
							insertps.setLong(columnMaps.size() + 5, batchProduceTask.getOwnerPositionId());
						...
						if(!colExclude.contains(9))
							insertps.setString(columnMaps.size() + 10,batchProduceTask.getPositionLayRec());
						if(!colExclude.contains(10))
							insertps.setLong(columnMaps.size() + 11,batchProduceTask.getOwnerPosition().getDepartment().getId());
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchProduceTask.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
						...
						ecTableInfops.setString(7,WOMBatchProduceTask.TABLE_NAME);
						ecTableInfops.setInt(8,99);
						ecTableInfops.setLong(9,batchProduceTask.getOwnerPosition().getId());
						ecTableInfops.setLong(10,batchProduceTask.getOwnerStaff().getId());
						ecTableInfops.setString(11,entityCode);
						ecTableInfops.setString(12,batchProduceTask.getPositionLayRec());
						ecTableInfops.setString(13,batchProduceTask.getTableNo());
						ecTableInfops.setLong(14,batchProduceTask.getOwnerPosition().getDepartment().getId());
		...
		batchProduceTaskDaoImpl.getSession().flush();
		batchProduceTaskDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = batchProduceTaskDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_batchProduceTask_BatchProduceTask").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = batchProduceTaskDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_batchProduceTask_BatchProduceTask").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchProduceTask bizObj = insertObjs.get(insertIndex);
			batchProduceTaskImportService.saveImportDataLog(bizObj, null, "tableNo", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchProduceTask bizObj = updateObjs.get(updateIndex);
			WOMBatchProduceTask oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchProduceTaskImportService.saveImportDataLog(bizObj, oldBizObj, "tableNo", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = batchProduceTaskDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				batchProduceTaskDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = batchProduceTaskDao.createNativeQuery(sql, "WOM_7.5.0.0_batchProduceTask_BatchProduceTask").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchProduceTaskDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchProduceTaskDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchProduceTaskDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchProduceTaskDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchProduceTaskDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchProduceTaskDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1495090968956Page(Page<WOMBatchProdTaskPart> dg1495090968956Page,WOMBatchProduceTask batchProduceTask, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchProdTaskPart.class);
		criteria.add(Restrictions.eq("headId", batchProduceTask));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"batchProdTaskPart\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		batchProdTaskPartDao.findByPage(dg1495090968956Page, criteria);
		batchProdTaskPartService.findByNormalDataGridBatchProduceTask(dg1495090968956Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1496366180645Page(Page<WOMBatchProdTaskPart> dg1496366180645Page,WOMBatchProduceTask batchProduceTask, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMBatchProdTaskPart.class);
		criteria.add(Restrictions.eq("headId", batchProduceTask));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		batchProdTaskPartDao.findByPage(dg1496366180645Page, criteria);
		batchProdTaskPartService.findByNormalDataGridBatchProduceTask(dg1496366180645Page);
	}
	
	 
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.save")
	public void saveBatchProduceTask(WOMBatchProduceTask batchProduceTask, DataGridService dataGridService){
		saveBatchProduceTask(batchProduceTask, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.save")
	public void saveBatchProduceTask(WOMBatchProduceTask batchProduceTask, DataGridService dataGridService, String viewCode){
		saveBatchProduceTask(batchProduceTask, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void saveBatchProduceTask(WOMBatchProduceTask batchProduceTask, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchProduceTask(batchProduceTask, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void saveBatchProduceTask(WOMBatchProduceTask batchProduceTask, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchProduceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchProduceTask");
		String url = null;
		if(null != batchProduceTask.getId() && batchProduceTask.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public Map<Object, Object> batchProduceTaskDataGridImport(WOMBatchProduceTask batchProduceTask, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchProduceTask.getVirtualId();
		
		batchProduceTask = batchProduceTaskDao.load(batchProduceTask.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchProduceTask.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchProduceTask.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchProduceTask);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchProduceTask.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void mergeBatchProduceTask(WOMBatchProduceTask batchProduceTask, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchProduceTask);
		beforeSaveBatchProduceTask(batchProduceTask);
		if (null == batchProduceTask.getId()) {
			batchProduceTask.setStatus(EntityTableInfo.STATUS_RUNNING);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.print.page.print",operType="4")
	...
	@Override
	public Page<WOMBatchProduceTask> getByPage(Page<WOMBatchProduceTask> page,DetachedCriteria detachedCriteria){
		return batchProduceTaskDao.findByPage(page, detachedCriteria);
	}
	@Override
	public List<WOMBatchProdTaskPart> getBatchProdTaskPartList(WOMBatchProduceTask batchProduceTask){
		return batchProdTaskPartDao.findByCriteria(Restrictions.eq("headId", batchProduceTask),Restrictions.eq("valid", true));
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.common.super.edit",operType="4")
	public void saveSuperEditBatchProduceTask(WOMBatchProduceTask batchProduceTask, WorkFlowVar workFlowVar, DataGridService dataGridService, String viewCode, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_batchProduceTask";
		List<Event> events = null;
		View view = null;
		if(viewCode != null){
		...
		saveSingleSuperEditBatchProduceTask(workFlowVar, dataGridService, view, entityCode, events, batchProduceTask, isImport);
		if(null != batchProduceTask && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
			AuditUtil.setColumnStringA(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
			AuditUtil.setColumnStringC(batchProduceTask.getId().toString());
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="foundation.batch.import")
	...
	private void saveSingleSuperEditBatchProduceTask(WorkFlowVar workFlowVar, DataGridService dataGridService, View view, String entityCode, List<Event> events,WOMBatchProduceTask batchProduceTask, boolean... isImport) {
		if(events != null && events.size() > 0){
			executeGScript(entityCode, "beforeSubmit", events, batchProduceTask);
		}
		ReflectUtils.filterObjectIdIsNVL(batchProduceTask);
		...
		beforeSubmitBatchProduceTask(batchProduceTask, workFlowVar, true);
		batchSaveBatchProduceTask(batchProduceTask, dataGridService, view,events, null,isImport);
		DealInfo di = taskService.generateSuperEditDealInfo(batchProduceTask.getTableInfoId(), isSuperAdd,isImport);
		batchProduceTaskDao.flush();
		workFlowVar.setModelId(batchProduceTask.getId());
		workFlowVar.setTableInfoId(batchProduceTask.getTableInfoId());
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void save(WOMBatchProduceTask batchProduceTask,Long deploymentId,Long pendingId,WorkFlowVar workFlowVar,DataGridService dataGridService){
		save(batchProduceTask, deploymentId, pendingId, workFlowVar, dataGridService, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask")
	public void save(WOMBatchProduceTask batchProduceTask, Long deploymentId,Long pendingId, WorkFlowVar workFlowVar, DataGridService dataGridService, String viewCode) {
		ReflectUtils.filterObjectIdIsNVL(batchProduceTask);
		boolean first = null == batchProduceTask.getId();
		if (first){
			if(workFlowVar==null){
		...
		saveBatchProduceTask(batchProduceTask, dataGridService, viewCode);
		if(null != batchProduceTask && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProduceTask.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
			AuditUtil.setColumnStringA(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
			AuditUtil.setColumnStringC(batchProduceTask.getId().toString());
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.submit",operType="4")
	public void submit(WOMBatchProduceTask batchProduceTask,Long deploymentId,Long pendingId,WorkFlowVar workFlowVar,DataGridService dataGridService){
		submit(batchProduceTask, deploymentId, pendingId, workFlowVar, dataGridService, null);
	}
	
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProduceTask",desc="ec.flow.submit",operType="4")
	public void submit(WOMBatchProduceTask batchProduceTask, Long deploymentId, Long pendingId, WorkFlowVar workFlowVar, DataGridService dataGridService, String viewCode) {
		String entityCode = "WOM_7.5.0.0_batchProduceTask";
		List<Event> events = null;
		if(viewCode != null){
			View view = viewServiceFoundation.getView(viewCode);
	...
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_batchProduceTask").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBatchProduceTask(WOMBatchProduceTask batchProduceTask, Object... objects){
		checkUniqueConstraint(batchProduceTask);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
      	batchProduceTaskDao.flush();
		batchProduceTaskDao.clear();
/* CUSTOM CODE END */
	...
	private void afterSaveBatchProduceTask(WOMBatchProduceTask batchProduceTask, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
      
      
            batchProduceTaskDao.flush();
			batchProduceTaskDao.clear();
			if(batchProduceTask.getTaskType()!=null){
				List<WOMBatchProdTaskPart> batchTasks= batchProdTaskPartService.getBatchProdTaskParts("headId="+batchProduceTask.getId(), null);
				for(WOMBatchProdTaskPart batchTask:batchTasks){
					//将表头的生产性质赋值给表体
					//batchTask.setOrderType(batchProduceTask.getOrderType());
					//将表头指令单类型赋值给表体
					batchTask.setTaskType(batchProduceTask.getTaskType());
					//如果已经生成的指令单，则进行更新
					if(batchTask.getIsAddTask()){
						List<WOMProduceTask> tasks=produceTaskService.getProduceTasks("tasksID="+batchTask.getId()+"and valid=1", null);
						if(tasks.size()>0){
							WOMProduceTask task=tasks.get(0);
							...
							task.setPackageType(batchTask.getBulkType());
							task.setBulkProduct(batchTask.getBulkProduct());
							//指令单类型
							task.setTaskType(batchTask.getTaskType());
							//待包装信息
							produceTaskDao.update(task);
							batchProdTaskPartDao.update(batchTask);
						}
					}else{
						//如果产品  配方 数量均不为空 则生成指令单
						if(batchTask.getFormularId()!=null  &&  batchTask.getProductId()!=null  && batchTask.getProductNum()!=null){
							produceTaskService.makeProduceOrderByBatchTask(batchTask); 				
							//设置为已经生成指令单
							batchTask.setIsAddTask(true);
							batchProdTaskPartDao.update(batchTask);
						}
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchProduceTask(WOMBatchProduceTask batchProduceTask){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchProduceTask(WOMBatchProduceTask batchProduceTask){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchProduceTask(WOMBatchProduceTask batchProduceTask){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchProduceTask(WOMBatchProduceTask batchProduceTask){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchProduceTask batchProduceTask, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchProduceTask batchProduceTask, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}

	private void beforeSubmitBatchProduceTask(WOMBatchProduceTask batchProduceTask, WorkFlowVar workFlowVar){
	/* CUSTOM CODE START(serviceimpl,beforeSubmit,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeSubmitBatchProduceTask(WOMBatchProduceTask batchProduceTask, WorkFlowVar workFlowVar, Boolean superEdit){
	/* CUSTOM CODE START(serviceimpl,beforeSubmitSuperEdit,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSubmitBatchProduceTask(WOMBatchProduceTask batchProduceTask, WorkFlowVar workFlowVar){
	/* CUSTOM CODE START(serviceimpl,afterSubmit,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void afterSubmitBatchProduceTask(WOMBatchProduceTask batchProduceTask, WorkFlowVar workFlowVar, Boolean superEdit){
	/* CUSTOM CODE START(serviceimpl,afterSubmitSuperEdit,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRetrial(WOMBatchProduceTask batchProduceTask){
	/* CUSTOM CODE START(serviceimpl,beforeRetrial,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterRetrial(WOMBatchProduceTask batchProduceTask){
	/* CUSTOM CODE START(serviceimpl,afterRetrial,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterSubmitProcessBatchProduceTask(WOMBatchProduceTask batchProduceTask, DealInfo di, WorkFlowVar workFlowVar, String viewCode){
		/* CUSTOM CODE START(serviceimpl,afterSubmitProcess,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSubmitProcessBatchProduceTask(WOMBatchProduceTask batchProduceTask, DealInfo di, WorkFlowVar workFlowVar, String viewCode, Boolean superEdit){
		/* CUSTOM CODE START(serviceimpl,afterSubmitProcessSuperEdit,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProcessBatchProduceTask(WOMBatchProduceTask batchProduceTask, DealInfo di, WorkFlowVar workFlowVar, String viewCode){
		/* CUSTOM CODE START(serviceimpl,afterSaveProcess,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportBatchProduceTask(List<WOMBatchProduceTask> insertObjs, List<WOMBatchProduceTask> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchProduceTask,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportBatchProduceTask(List<WOMBatchProduceTask> insertObjs, List<WOMBatchProduceTask> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchProduceTask,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
	@Autowired
	...
	public void delBatchNum(WOMBatchProduceTask batchProduceTask) {
		String hqlChart = "from WOMBatchProdTaskPart where valid = 1 and headId = ?";
		List<WOMBatchProdTaskPart> batchProduceTaskPartList = batchProdTaskPartDao.findByHql(hqlChart,batchProduceTask);
		for (WOMBatchProdTaskPart womBatchProdTaskPart : batchProduceTaskPartList) {
			if(womBatchProdTaskPart.getProductBatchNum()!=null && !"".equals(womBatchProdTaskPart.getProductBatchNum())){
				delBatchNum(womBatchProdTaskPart.getProductBatchNum());
				createBatchDealInfo(womBatchProdTaskPart.getProductBatchNum(),batchProduceTask, womBatchProdTaskPart.getProductId(), womBatchProdTaskPart.getProductNum(), "批量指令单作废处理","publicSetTableState/03");
			}
	 ...
	 * @param remark 备注
	 */
	public void createBatchDealInfo(String batchNum,WOMBatchProduceTask batchProduceTask,MESBasicProduct product,BigDecimal amount,String remark,String tableState){
		//生成批次处理信息
		PublicSetBatchDeal batchDealinfo = new PublicSetBatchDeal();
		batchDealinfo.setBatchNum(batchNum); 
		batchDealinfo.setProduct(product);
		...
		batchDealinfo.setDealTableID(batchProduceTask.getId());
		batchDealinfo.setDealTableNo(batchProduceTask.getTableNo());
      	batchDealinfo.setDealType((SystemCode) systemCodeService.getSystemCode("publicSetDealWay/produceTask"));
      	batchDealinfo.setSourceBatchNum(batchNum);
		batchDealinfo.setDealTableURL(batchDealService.getOpenViewUrl("WOM_7.5.0.0_batchProduceTask_batchTastView","WOM_7.5.0.0_batchProduceTask_batchTaskList", batchProduceTask.getTableInfoId(), batchProduceTask.getId()));
	...
	public List<String> getOldBatchNum(WOMBatchProduceTask batchProduceTask) {
		...
		String hqlChart = "from WOMBatchProdTaskPart where valid = 1 and headId = ?";
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByHql(hqlChart,batchProduceTask);
		for (WOMBatchProdTaskPart batchProdTaskPart : batchProdTaskPartList) {
			if(batchProdTaskPart.getProductBatchNum()!=null && !batchProdTaskPart.getProductBatchNum().equals("")){
				oldBatchNum.add(batchProdTaskPart.getProductBatchNum());
			}
		}
		return oldBatchNum;
	}
	...
	public Map<String, BigDecimal> getBatchAmout(WOMBatchProduceTask batchProduceTask) {
		String hqlChart = "from WOMBatchProdTaskPart where valid = 1 and headId = ?";
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByHql(hqlChart,batchProduceTask);
		return getBatchNum(batchProdTaskPartList);
	}

	/**
	 * 将表体根据批号重组 String 批号  baiDecimal 数量
	 * @param batchProdTaskPartList
	 * @return
	 */
	public Map<String, BigDecimal> getBatchNum(List<WOMBatchProdTaskPart> batchProdTaskPartList){
		//需要返回的map
		Map<String, BigDecimal> amount=new HashMap<String,BigDecimal>();
		//区分重复的批号
		List<String> batchNum = new ArrayList<String>();
		for (WOMBatchProdTaskPart batchProdTaskPart : batchProdTaskPartList) {
			if(batchProdTaskPart.getProductBatchNum()!=null){
				if(batchProdTaskPart.getProductNum()!=null){
					amount.put(batchProdTaskPart.getProductBatchNum(), batchProdTaskPart.getProductNum());
				}else{
					amount.put(batchProdTaskPart.getProductBatchNum(), new BigDecimal(0));
				}
			}
		}
		return amount;
	...
	public Map<String, MESBasicProduct> getBatchPro(WOMBatchProduceTask batchProduceTask) {
		String hqlChart = "from WOMBatchProdTaskPart where valid = 1 and headId = ?";
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByHql(hqlChart,batchProduceTask);
		return getBatchPro(batchProdTaskPartList);
	}

	/**
	 * 将表体根据批号重组 String 批号  MESBasicProduct 物品
	 * @param batchProdTaskPartList
	 * @return
	 */
	private Map<String, MESBasicProduct> getBatchPro(List<WOMBatchProdTaskPart> batchProdTaskPartList) {
		//需要返回的map
		Map<String, MESBasicProduct> product=new HashMap<String,MESBasicProduct>();
		//区分重复的批号
		List<String> batchNum = new ArrayList<String>();
		for (WOMBatchProdTaskPart batchProdTaskPart : batchProdTaskPartList) {
			if(batchProdTaskPart.getProductBatchNum()!=null){
				if(batchProdTaskPart.getProductId()!=null && batchProdTaskPart.getProductId().getId()!=null){
					product.put(batchProdTaskPart.getProductBatchNum(), batchProdTaskPart.getProductId());
				}else{
					product.put(batchProdTaskPart.getProductBatchNum(), null);
				}
			}
		}
		return product;
	...
	public void operate(WOMBatchProduceTask batchProduceTask,
		...
		for (String batchNum : oldBatchNum) {
			if(!newBatchNum.contains(batchNum)){
				...
				createBatchDealInfo(batchNum, batchProduceTask, oldProduct.get(batchNum), oldAmount.get(batchNum), "表体删除该批号信息", "publicSetTableState/01");
			}
		}
		//新批号与老批号对比，如果不存在，执行添加
		for (String batchNum : newBatchNum) {
			if(!oldBatchNum.contains(batchNum)){
				addBatchNum(batchProduceTask,batchNum,newAmount,newProduct);
			}else{
				//与之前相比  批号不变  查看数量是否变化
				if(newAmount.get(batchNum).compareTo(oldAmount.get(batchNum))!=0 || !((newProduct.get(batchNum)==null && oldProduct.get(batchNum)==null) || (newProduct.get(batchNum)!=null && oldProduct.get(batchNum)!=null && (newProduct.get(batchNum).getId().equals(oldProduct.get(batchNum).getId()))))){
					//不相等情况
					//生成批次处理信息
					createBatchDealInfo(batchNum, batchProduceTask, newProduct.get(batchNum),newAmount.get(batchNum), "批量指令单修改","publicSetTableState/01");
					PublicSetBatchInfo batchInfo = batchInfoService.getbatchInformationBybatchNum(batchNum);
					batchInfo.setProduct(newProduct.get(batchNum));
	 ...
	 * @param newProduct
	 */
	private void addBatchNum(WOMBatchProduceTask batchProduceTask, String batchNum,Map<String, BigDecimal> newAmount,Map<String, MESBasicProduct> newProduct) {
		//生成批号记录
		createBatchInfo(batchNum, newProduct.get(batchNum));
		//生成批次处理信息
		createBatchDealInfo(batchNum, batchProduceTask, newProduct.get(batchNum),newAmount.get(batchNum), "批量指令单新增批号","publicSetTableState/01");
	}
	 ...
	 * @param taskReporting 报工单
	 */
	public void createBatchInfo(String batchNum,MESBasicProduct product){
		PublicSetBatchInfo batchInfo = new PublicSetBatchInfo();
		batchInfo.setBatchNum(batchNum);
		batchInfo.setProduct(product);
	 ...
	 */
	@Override
	public void addDealBatch(WOMBatchProduceTask batchProduceTask,
			String tableState, String remark) {
		String hqlChart = "from WOMBatchProdTaskPart where valid = 1 and headId = ?";
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByHql(hqlChart,batchProduceTask);
		
		for (WOMBatchProdTaskPart womBatchProdTaskPart : batchProdTaskPartList) {
			createBatchDealInfo(womBatchProdTaskPart.getProductBatchNum(), batchProduceTask, womBatchProdTaskPart.getProductId(), womBatchProdTaskPart.getProductNum(), remark, tableState);
		}
	}
	 ...
	 */
	@Override
	public void setBatch(WOMBatchProduceTask batchProduceTask,
			List<WOMBatchProdTaskPart> dg1495090968956Lists) {
		batchProduceTaskDao.flush();
		batchProduceTaskDao.clear();
		...
		List<WOMBatchProdTaskPart> batchProdTaskPartList = batchProdTaskPartDao.findByHql(hqlChart,batchProduceTask);
      	...
      	List<String> newBatchNum = new ArrayList<String>();
		for (WOMBatchProdTaskPart batchProdTaskPart : batchProdTaskPartList) {
			if(batchProdTaskPart.getProductBatchNum()!=null){
				amount.put(batchProdTaskPart.getProductBatchNum(), batchProdTaskPart.getProductNum());
			}else{
      	...
      	for (String batchNumber : amount.keySet()) {
      		//生成批号记录
      		createBatchInfo(batchNumber, product.get(batchNumber));
			//生成批次处理信息
			createBatchDealInfo(batchNumber, batchProduceTask, product.get(batchNumber),amount.get(batchNumber), "批量指令单新增批号","publicSetTableState/01");
      	}
