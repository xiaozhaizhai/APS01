...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchingRejectPartsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchingRejectParts> {
	...
	private WOMBatchingRejectParts batchingRejectParts;
	...
	//导出excel时,主模型内容
	List<WOMBatchingRejectParts> queryResults = new ArrayList<WOMBatchingRejectParts>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterial/batchingRejectParts/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterial/batchingRejectParts/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchingMaterial_BatchingRejectParts" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchingRejectParts> batchingRejectPartss;
	
	public List<WOMBatchingRejectParts> getBatchingRejectPartss() {
		return batchingRejectPartss;
	}
	...
	private Page<WOMBatchingRejectParts> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchingRejectParts = new WOMBatchingRejectParts();
		} else {
			if(id > 0){
				batchingRejectParts = batchingRejectPartsService.getBatchingRejectParts(id, viewCode);
			}
		}
	...
	public Page<WOMBatchingRejectParts> getPage() {
		if (null == page) {
			page = new Page<WOMBatchingRejectParts>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchingRejectParts getBatchingRejectParts() {
		return batchingRejectParts;
	}		

	public void setBatchingRejectParts(WOMBatchingRejectParts batchingRejectParts) {
		this.batchingRejectParts = batchingRejectParts;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchingRejectPartsService.getBatchingRejectPartsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1543236237180
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchingRejectPartsReferenceCopy() throws Exception{
			WOMBatchingRejectParts oldBatchingRejectParts = null;
			if(isEditNew()){
				...
				oldBatchingRejectParts = new WOMBatchingRejectParts();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchingRejectParts.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchingRejectParts.getId());
				...
				oldBatchingRejectParts = batchingRejectPartsService.getBatchingRejectParts(id);
			}
			
			
			batchingRejectParts = batchingRejectPartsService.getBatchingRejectParts(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchingRejectParts.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchingRejectParts, null);
						Method setValue = WOMBatchingRejectParts.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchingRejectParts, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchingRejectParts.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchingRejectPartsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchingRejectPartsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchingRejectParts = batchingRejectPartsService.getBatchingRejectParts(this.batchingRejectParts.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchingRejectParts.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchingRejectPartsService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingRejectParts/rejectList", results = { @Result(type = FREEMARKER, location = "rejectList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchingRejectPartsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchingRejectPartsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchingRejectPartsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingRejectParts/rejectList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchingId.tableNo,result.produceTaskId.tableNo,result.batchNum,result.produceTaskId.factoryId.name,result.produceTaskId.productId.productName,result.rejectProduct.productCode,result.rejectProduct.productName,result.productBatchNum,result.rejectNum,result.rejectReasonId.rejectReson,result.ware.wareCode,result.ware.wareName,result.ware.cargoStatue.id,result.ware.cargoStatue.value,result.store.placeSetCode,result.store.placeSetName,result.rejectExeStaff.name,result.rejectTime,result.isReceived,result.rejectReceiveStaff.name,result.receiveDate,result.batchingPartId.state.id,result.batchingPartId.state.value,result.batchingId.id,result.produceTaskId.id,result.produceTaskId.factoryId.id,result.produceTaskId.productId.id,result.rejectProduct.id,result.rejectReasonId.id,result.ware.id,result.store.id,result.rejectExeStaff.id,result.rejectReceiveStaff.id,result.batchingPartId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="rejectList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchingRejectParts\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchingRejectParts\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("storeSet.layRec") && request.getParameter("storeSet.layRec").trim().length() > 0) {
			params.add(new Param("\"store\".LAY_REC", request.getParameter("storeSet.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchingRejectParts\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchingRejectParts\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchingRejectPartsService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingRejectParts/rejectList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchingRejectPartsService.findDataGridPage(dg,dgclass,getDgPage(),batchingRejectParts,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchingRejectParts.getId() != null && batchingRejectParts.getId() != -1){
				batchingRejectParts = batchingRejectPartsService.getBatchingRejectParts(batchingRejectParts.getId());
			}
			batchingRejectPartsService.findDataGridPage(dg,dgclass,getDgPage(),batchingRejectParts,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchingRejectParts.getId()) {
		...
		batchingRejectPartsService.saveBatchingRejectParts(batchingRejectParts,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "submit");
		bap_validate_datagrids=batchingRejectPartsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchingRejectParts.getId());
		...
		fileuploads.put("linkId", batchingRejectParts.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchingRejectPartsService.deleteBatchingRejectParts(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "restore");
		batchingRejectPartsService.restoreBatchingRejectParts(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchingRejectParts> findExportDataInfos(Page<WOMBatchingRejectParts> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchingRejectPartsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchingRejectPartsService.getAssProperty(p.getCode());	
									//String assModelCode = batchingRejectPartsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchingRejectPartsService.findBatchingRejectPartss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchingRejectPartsService.findBatchingRejectPartss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchingRejectParts> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchingRejectParts result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchingRejectParts || null == this.batchingRejectParts.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchingMaterial/batchingRejectParts/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchingMaterial/batchingRejectParts/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchingRejectPartsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchingMaterial_BatchingRejectParts";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMBatchingRejectParts batchingRejectParts = null;
		if(null != id) {
			batchingRejectParts = batchingRejectPartsService.getBatchingRejectParts(id);
		}
		...
		_auditLog.setColumnStringB(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
		_auditLog.setColumnStringA(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
		_auditLog.setColumnStringC(batchingRejectParts.getId().toString());
		_auditLog.setColumnStringD(WOMBatchingRejectParts.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchingRejectPartsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchingRejectParts,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchingRejectParts,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchingRejectParts);
		...
		printlog.setBusinessId(batchingRejectParts.getId());
		printlog.setBussinessKey(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
		printlog.setMainDisplay(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchingRejectParts,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchingRejectPartsService.getBatchingRejectParts(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchingRejectParts,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchingRejectParts = batchingRejectPartsService.getBatchingRejectParts(Long.parseLong(arrId[i]));
				if(null != batchingRejectParts){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchingRejectParts.getId() ? "" : "," + batchingRejectParts.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchingRejectParts.getId() ? "" : "," + batchingRejectParts.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchingRejectParts.getId() ? "" : "," + batchingRejectParts.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchingRejectParts.getId() ? "" : batchingRejectParts.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchingRejectParts,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchingRejectParts);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchingRejectPartsService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchingMaterial/batchingRejectParts/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterial_BatchingRejectParts");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchingRejectPartsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchingMaterial_BatchingRejectParts,WOM_7.5.0.0) */
// 自定义代码
