...
@Repository
public class WOMProduceTaskDaoImpl extends ExtendedGenericDaoImpl<WOMProduceTask, Long> implements WOMProduceTaskDao {
	@Override
	...
	@Override
	public WOMProduceTaskDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMProduceTaskDealInfo dealInfo = new WOMProduceTaskDealInfo();
	...
	@Override
	public WOMProduceTaskDealInfo copyAndSaveDealInfo(DealInfo di, WOMProduceTask mainObj, Staff dealStaff) {
		WOMProduceTaskDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMProduceTaskDealInfo.class);
		...
		List<WOMProduceTaskDealInfo> dealinfo = criteria.list();
	...
	public void addSupervision(Supervision supervision) {
		...
		WOMProduceTaskSupervision WOMProduceTaskSupervision = new WOMProduceTaskSupervision();
		BeanUtils.copyProperties(supervision, WOMProduceTaskSupervision);
		WOMProduceTaskSupervision.setValid(true);
		getSession().save(WOMProduceTaskSupervision);
	}

	@Override
	public void addSupervision(Supervision supervision, WOMProduceTask mainObj) {
		getSession().save(supervision);
		WOMProduceTaskSupervision localSupervision = new WOMProduceTaskSupervision();
	...
	public void deleteSupervision(Long tableInfoId, List<Long> idList) {
		...
		List<WOMProduceTaskSupervision> _supervisions = getSession().createCriteria(WOMProduceTaskSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.in("staff.id", idList))
				.add(Restrictions.eq("valid", true)).list();
		for (WOMProduceTaskSupervision item : _supervisions) {
			item.setValid(false);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMProduceTaskSupervision> findSupervision(Long tableInfoId) {
		List<WOMProduceTaskSupervision> supervisions = getSession().createCriteria(WOMProduceTaskSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("valid", true)).addOrder(Order.asc("id")).list();
		return supervisions;
	}
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMProduceTaskSupervision> findSupervision(Long tableInfoId, Long StaffId) {
		List<WOMProduceTaskSupervision> supervisions = getSession().createCriteria(WOMProduceTaskSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("staff.id", StaffId)).add(Restrictions.eq("valid", true)).list();
		return supervisions;
	}
	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_produceTask_ProduceTask,WOM_7.5.0.0) */
// 自定义代码
