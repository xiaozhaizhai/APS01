...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMMakeBatOrderPartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMMakeBatOrderPart> {
	...
	private WOMMakeBatOrderPart makeBatOrderPart;
	...
	//导出excel时,主模型内容
	List<WOMMakeBatOrderPart> queryResults = new ArrayList<WOMMakeBatOrderPart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterialNeed/makeBatOrderPart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterialNeed/makeBatOrderPart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMMakeBatOrderPart> makeBatOrderParts;
	
	public List<WOMMakeBatOrderPart> getMakeBatOrderParts() {
		return makeBatOrderParts;
	}
	...
	private Page<WOMMakeBatOrderPart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			makeBatOrderPart = new WOMMakeBatOrderPart();
		} else {
			if(id > 0){
				makeBatOrderPart = makeBatOrderPartService.getMakeBatOrderPart(id, viewCode);
			}
		}
	...
	public Page<WOMMakeBatOrderPart> getPage() {
		if (null == page) {
			page = new Page<WOMMakeBatOrderPart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMMakeBatOrderPart getMakeBatOrderPart() {
		return makeBatOrderPart;
	}		

	public void setMakeBatOrderPart(WOMMakeBatOrderPart makeBatOrderPart) {
		this.makeBatOrderPart = makeBatOrderPart;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = makeBatOrderPartService.getMakeBatOrderPartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1491801852784
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String makeBatOrderPartReferenceCopy() throws Exception{
			WOMMakeBatOrderPart oldMakeBatOrderPart = null;
			if(isEditNew()){
				...
				oldMakeBatOrderPart = new WOMMakeBatOrderPart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,makeBatOrderPart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, makeBatOrderPart.getId());
				...
				oldMakeBatOrderPart = makeBatOrderPartService.getMakeBatOrderPart(id);
			}
			
			
			makeBatOrderPart = makeBatOrderPartService.getMakeBatOrderPart(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMMakeBatOrderPart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMakeBatOrderPart, null);
						Method setValue = WOMMakeBatOrderPart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(makeBatOrderPart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			makeBatOrderPart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=makeBatOrderPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return makeBatOrderPartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			makeBatOrderPart = makeBatOrderPartService.getMakeBatOrderPart(this.makeBatOrderPart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,makeBatOrderPart.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			makeBatOrderPartService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = makeBatOrderPartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(makeBatOrderPartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(makeBatOrderPartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"makeBatOrderPart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"makeBatOrderPart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"makeBatOrderPart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"makeBatOrderPart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			makeBatOrderPartService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		makeBatOrderPartService.findDataGridPage(dg,dgclass,getDgPage(),makeBatOrderPart,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(makeBatOrderPart.getId() != null && makeBatOrderPart.getId() != -1){
				makeBatOrderPart = makeBatOrderPartService.getMakeBatOrderPart(makeBatOrderPart.getId());
			}
			makeBatOrderPartService.findDataGridPage(dg,dgclass,getDgPage(),makeBatOrderPart,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == makeBatOrderPart.getId()) {
		...
		makeBatOrderPartService.saveMakeBatOrderPart(makeBatOrderPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "submit");
		bap_validate_datagrids=makeBatOrderPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", makeBatOrderPart.getId());
		...
		fileuploads.put("linkId", makeBatOrderPart.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		makeBatOrderPartService.deleteMakeBatOrderPart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "restore");
		makeBatOrderPartService.restoreMakeBatOrderPart(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMMakeBatOrderPart> findExportDataInfos(Page<WOMMakeBatOrderPart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = makeBatOrderPartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = makeBatOrderPartService.getAssProperty(p.getCode());	
									//String assModelCode = makeBatOrderPartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			makeBatOrderPartService.findMakeBatOrderParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			makeBatOrderPartService.findMakeBatOrderParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMMakeBatOrderPart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMMakeBatOrderPart result : results){
	...
	public boolean isEditNew(){
		//return null == this.makeBatOrderPart || null == this.makeBatOrderPart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchingMaterialNeed/makeBatOrderPart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchingMaterialNeed/makeBatOrderPart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMMakeBatOrderPartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
		_auditLog.setColumnStringA(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
		_auditLog.setColumnStringC(makeBatOrderPart.getId().toString());
		_auditLog.setColumnStringD(WOMMakeBatOrderPart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMMakeBatOrderPartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, makeBatOrderPart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, makeBatOrderPart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,makeBatOrderPart);
		...
		printlog.setBusinessId(makeBatOrderPart.getId());
		printlog.setBussinessKey(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
		printlog.setMainDisplay(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(makeBatOrderPart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(makeBatOrderPartService.getMakeBatOrderPart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, makeBatOrderPart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				makeBatOrderPart = makeBatOrderPartService.getMakeBatOrderPart(Long.parseLong(arrId[i]));
				if(null != makeBatOrderPart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == makeBatOrderPart.getId() ? "" : "," + makeBatOrderPart.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == makeBatOrderPart.getId() ? "" : "," + makeBatOrderPart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == makeBatOrderPart.getId() ? "" : "," + makeBatOrderPart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == makeBatOrderPart.getId() ? "" : makeBatOrderPart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, makeBatOrderPart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,makeBatOrderPart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//makeBatOrderPartService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderPart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = makeBatOrderPartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderPart,WOM_7.5.0.0) */
// 自定义代码
