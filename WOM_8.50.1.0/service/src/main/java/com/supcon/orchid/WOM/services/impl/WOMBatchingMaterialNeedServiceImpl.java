...
import com.supcon.orchid.WOM.services.WOMBatchingMaterialNeedImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchingMaterialNeedDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_batchingMaterialNeedService")
@Transactional
public class WOMBatchingMaterialNeedServiceImpl  extends BaseServiceImpl<WOMBatchingMaterialNeed> implements WOMBatchingMaterialNeedService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMBatchingMaterialNeed> bapBaseService;
	...
	@Autowired
	private WOMBatchingMaterialNeedImportService batchingMaterialNeedImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchingMaterialNeed.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchingMaterialNeedDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchingMaterialNeed.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchingMaterialNeedDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchingMaterialNeed.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchingMaterialNeed.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchingMaterialNeedDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchingMaterialNeed.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchingMaterialNeed.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchingMaterialNeed> list = (List<WOMBatchingMaterialNeed>) batchingMaterialNeedDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMBatchingMaterialNeed getBatchingMaterialNeed(long id){
		return getBatchingMaterialNeed(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingMaterialNeed getBatchingMaterialNeed(long id, String viewCode){
		WOMBatchingMaterialNeed batchingMaterialNeed = batchingMaterialNeedDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchingMaterialNeedAsJSON(long id, String include){
		WOMBatchingMaterialNeed batchingMaterialNeed = batchingMaterialNeedDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchingMaterialNeed, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed){
		if(null != batchingMaterialNeed && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialNeed.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
			AuditUtil.setColumnStringA(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
			AuditUtil.setColumnStringC(batchingMaterialNeed.getId().toString());
		}
		beforeDeleteBatchingMaterialNeed(batchingMaterialNeed);
		batchingMaterialNeedDao.delete(batchingMaterialNeed);
		afterDeleteBatchingMaterialNeed(batchingMaterialNeed);
		batchingMaterialNeedDao.flush();
		List<WOMBatchingMaterialNeed> params = new ArrayList<WOMBatchingMaterialNeed>();
		params.add(batchingMaterialNeed);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialNeed(List<Long> batchingMaterialNeedIds){
		deleteBatchingMaterialNeed(batchingMaterialNeedIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialNeed(List<Long> batchingMaterialNeedIds, String eventTopic) {
		List<WOMBatchingMaterialNeed> batchingMaterialNeeds = new ArrayList<WOMBatchingMaterialNeed>();
		for(Long batchingMaterialNeedId : batchingMaterialNeedIds){
			WOMBatchingMaterialNeed batchingMaterialNeed = getBatchingMaterialNeed(batchingMaterialNeedId);
			batchingMaterialNeeds.add(batchingMaterialNeed);
			if(null != batchingMaterialNeed && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialNeed.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchingMaterialNeed.getId() ? "" : "," + batchingMaterialNeed.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchingMaterialNeed.getId() ? "" : "," + batchingMaterialNeed.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchingMaterialNeed.getId() ? "" : "," + batchingMaterialNeed.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchingMaterialNeeds != null && batchingMaterialNeeds.size() > 0 ){
			for(WOMBatchingMaterialNeed batchingMaterialNeed : batchingMaterialNeeds){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchingMaterialNeed, batchingMaterialNeed.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(batchingMaterialNeedIds != null && batchingMaterialNeedIds.size() > 0) {
			String hql = "update " + WOMBatchingMaterialNeed.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = batchingMaterialNeedDao.createQuery(hql);
			query.setParameterList("ids", batchingMaterialNeedIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchingMaterialNeed batchingMaterialNeedz : batchingMaterialNeeds){
				batchingMaterialNeedz.setValid(false);
				List<WOMBatchingMaterialNeed> params = new ArrayList<WOMBatchingMaterialNeed>();
				params.add(batchingMaterialNeedz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialNeed(long batchingMaterialNeedId, int batchingMaterialNeedVersion){
		this.deleteBatchingMaterialNeed(batchingMaterialNeedId, batchingMaterialNeedVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialNeed(long batchingMaterialNeedId, int batchingMaterialNeedVersion,SignatureLog signatureLog){
		WOMBatchingMaterialNeed batchingMaterialNeed = getBatchingMaterialNeed(batchingMaterialNeedId);
		...
		beforeDeleteBatchingMaterialNeed(batchingMaterialNeed);
		batchingMaterialNeedDao.delete(batchingMaterialNeedId, batchingMaterialNeedVersion);
		afterDeleteBatchingMaterialNeed(batchingMaterialNeed);
		batchingMaterialNeedDao.flush();
		List<WOMBatchingMaterialNeed> params = new ArrayList<WOMBatchingMaterialNeed>();
		params.add(batchingMaterialNeed);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchingMaterialNeed.getId();
			signatureLog.setTableId(batchingMaterialNeed.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialNeed(String batchingMaterialNeedIds){
		this.deleteBatchingMaterialNeed(batchingMaterialNeedIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.common.delete",operType="3")
	public void deleteBatchingMaterialNeed(String batchingMaterialNeedIds,SignatureLog signatureLog){
		deleteCollection(batchingMaterialNeedIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchingMaterialNeed(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_batchingMaterialNeed#modelCode:WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="ec.engine.restore",operType="10")
	public void restoreBatchingMaterialNeed(String batchingMaterialNeedIds){
		restoreCollection(batchingMaterialNeedIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchingMaterialNeed(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="ec.engine.restore",operType="10")
	public void restoreBatchingMaterialNeed(long batchingMaterialNeedId){
		
		findBusinessKeyUsed(batchingMaterialNeedId);	//判断业务主键是否重复
		...
		beforeRestoreBatchingMaterialNeed(batchingMaterialNeed);
		batchingMaterialNeed.setValid(true);
		batchingMaterialNeedDao.update(batchingMaterialNeed);
		afterRestoreBatchingMaterialNeed(batchingMaterialNeed);
		List<WOMBatchingMaterialNeed> params = new ArrayList<WOMBatchingMaterialNeed>();
		params.add(batchingMaterialNeed);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchingMaterialNeedId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchingMaterialNeed.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchingMaterialNeed.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchingMaterialNeedDao.createNativeQuery(sql,batchingMaterialNeedId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchingMaterialNeed(List<WOMBatchingMaterialNeed>  batchingMaterialNeeds){
		for(WOMBatchingMaterialNeed batchingMaterialNeed:batchingMaterialNeeds)  {
			saveBatchingMaterialNeed(batchingMaterialNeed, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchingMaterialNeed(List<WOMBatchingMaterialNeed>  batchingMaterialNeeds){
		for(WOMBatchingMaterialNeed batchingMaterialNeed:batchingMaterialNeeds)  {
			saveBatchingMaterialNeed(batchingMaterialNeed, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="ec.flow.save")
	public void saveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchingMaterialNeed(batchingMaterialNeed, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="ec.flow.save")
	public void saveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchingMaterialNeed(batchingMaterialNeed, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchingMaterialNeedCodes(WOMBatchingMaterialNeed batchingMaterialNeed) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchingMaterialNeedCodes(batchingMaterialNeed, false);
	}
	
	public void generateBatchingMaterialNeedCodes(WOMBatchingMaterialNeed batchingMaterialNeed, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchingMaterialNeedSummarys(WOMBatchingMaterialNeed batchingMaterialNeed) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchingMaterialNeedSummarys(batchingMaterialNeed, false);
	}

	public void generateBatchingMaterialNeedSummarys(WOMBatchingMaterialNeed batchingMaterialNeed, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed")
	public void saveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchingMaterialNeed(batchingMaterialNeed, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed")
	public void saveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchingMaterialNeed";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
		String url = null;
		if(batchingMaterialNeed.getId() != null && batchingMaterialNeed.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialNeed.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_batchingMaterialNeed/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialNeed.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_batchingMaterialNeed/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchingMaterialNeed);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchingMaterialNeed);
		beforeSaveBatchingMaterialNeed(batchingMaterialNeed, viewIsView);

		if (viewIsView) {
			batchingMaterialNeedDao.saveWithRevertVersion(batchingMaterialNeed);
		} else {
			if(isNew)
		...
		dealDatagridsSave(batchingMaterialNeed,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchingMaterialNeedCodes(batchingMaterialNeed, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchingMaterialNeedSummarys(batchingMaterialNeed, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchingMaterialNeed(batchingMaterialNeed, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchingMaterialNeed);
			}
		}
		List<WOMBatchingMaterialNeed> params = new ArrayList<WOMBatchingMaterialNeed>();
		batchingMaterialNeedDao.flush();
		batchingMaterialNeedDao.clear();
		batchingMaterialNeed = batchingMaterialNeedDao.load(batchingMaterialNeed.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchingMaterialNeed batchingMaterialNeed){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchingMaterialNeed", batchingMaterialNeed);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed")
	public void mergeBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchingMaterialNeed);
		beforeSaveBatchingMaterialNeed(batchingMaterialNeed);
		batchingMaterialNeedDao.merge(batchingMaterialNeed);
		if(null != batchingMaterialNeed && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialNeed.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
			AuditUtil.setColumnStringA(null == batchingMaterialNeed.getId() ? "" : batchingMaterialNeed.getId().toString());
		...
		dealDatagridsSave(batchingMaterialNeed,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchingMaterialNeed(batchingMaterialNeed);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchingMaterialNeed> findBatchingMaterialNeeds(Page<WOMBatchingMaterialNeed> page, Criterion... criterions) {
		return batchingMaterialNeedDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMBatchingMaterialNeedDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMBatchingMaterialNeedDealInfo> list=batchingMaterialNeedDao.createQuery("from " + WOMBatchingMaterialNeedDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMBatchingMaterialNeedDealInfo dealInfo) {
		batchingMaterialNeedDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMBatchingMaterialNeedDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) batchingMaterialNeedDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMBatchingMaterialNeedDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = batchingMaterialNeedDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = batchingMaterialNeedDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMBatchingMaterialNeedDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = batchingMaterialNeedDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingMaterialNeeds(Page<WOMBatchingMaterialNeed> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchingMaterialNeeds(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingMaterialNeeds(Page<WOMBatchingMaterialNeed> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchingMaterialNeeds(Page<WOMBatchingMaterialNeed> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchingMaterialNeed> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchingMaterialNeed\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchingMaterialNeed\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchingMaterialNeed\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  batchingMaterialNeedDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchingMaterialNeedDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_batchingMaterialNeed").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchingMaterialNeed\"","WOM_7.5.0.0_batchingMaterialNeed",customCondition,"WOM_BATCHING_MATERIAL_NEEDS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"batchingMaterialNeed\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMBatchingMaterialNeedDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"batchingMaterialNeed\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchingMaterialNeed\".LAY_REC ASC, \"batchingMaterialNeed\".SORT ASC");
				} else {
					orderPart.append(" \"batchingMaterialNeed\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchingMaterialNeedDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchingMaterialNeedDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchingMaterialNeedDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchingMaterialNeed> result = new ArrayList<WOMBatchingMaterialNeed>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchingMaterialNeed.class,batchingMaterialNeedDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchingMaterialNeed.class,batchingMaterialNeedDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchingMaterialNeed batchingMaterialNeed : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchingMaterialNeed.getId(), "WOM_batchingMaterialNeed_batchingMaterialNeed");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchingMaterialNeed.getId(), "WOM_batchingMaterialNeed_batchingMaterialNeed");
							if(null != documents && !documents.isEmpty()) {
								batchingMaterialNeed.setDocument(documents.get(0));
								batchingMaterialNeed.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_batchingMaterialNeed")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_batchingMaterialNeed")+"WOM_7.5.0.0_batchingMaterialNeed".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_batchingMaterialNeed",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchingMaterialNeed> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchingMaterialNeed> result) {
		View view = null;
		...
		Map<Long, WOMBatchingMaterialNeed> rsMap = new HashMap<Long, WOMBatchingMaterialNeed>();
		for (WOMBatchingMaterialNeed batchingMaterialNeed : result) {
			ids.add(batchingMaterialNeed.getId());
			rsMap.put(batchingMaterialNeed.getId(), batchingMaterialNeed);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchingMaterialNeedDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchingMaterialNeedDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMBatchingMaterialNeed batchingMaterialNeed,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(batchingMaterialNeed.getOwnerStaffId());
		ti.setOwnerPositionId(batchingMaterialNeed.getOwnerPositionId());
		ti.setOwnerDepartmentId(batchingMaterialNeed.getOwnerDepartmentId());
		
		ti.setCreateStaff(batchingMaterialNeed.getCreateStaff());
		ti.setCreateStaffId(batchingMaterialNeed.getCreateStaffId());
		ti.setCreateTime(batchingMaterialNeed.getCreateTime());
		ti.setDeleteStaff(batchingMaterialNeed.getDeleteStaff());
		ti.setDeleteStaffId(batchingMaterialNeed.getDeleteStaffId());
		ti.setDeleteTime(batchingMaterialNeed.getDeleteTime());
		ti.setCreateDepartmentId(batchingMaterialNeed.getCreateDepartmentId());
		ti.setModifyStaff(batchingMaterialNeed.getModifyStaff());
		ti.setModifyStaffId(batchingMaterialNeed.getModifyStaffId());
		ti.setCreatePositionId(batchingMaterialNeed.getCreatePositionId());
		ti.setPositionLayRec(batchingMaterialNeed.getPositionLayRec());
		ti.setTableNo(batchingMaterialNeed.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_batchingMaterialNeed");
		ti.setTargetTableName("WOM_BATCHING_MATERIAL_NEEDS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchingMaterialNeedDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchingMaterialNeed\"","WOM_7.5.0.0_batchingMaterialNeed","","WOM_BATCHING_MATERIAL_NEEDS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchingMaterialNeed\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchingMaterialNeed\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchingMaterialNeed\"","WOM_7.5.0.0_batchingMaterialNeed","","WOM_BATCHING_MATERIAL_NEEDS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("batchingMaterialNeed.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchingMaterialNeed\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchingMaterialNeed\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchingMaterialNeed\".CID = 1 OR \"batchingMaterialNeed\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchingMaterialNeed.mc\"").append(".BATCHING_MATERIAL_NEED) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchingMaterialNeedDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("batchingMaterialNeed", "WOM_7.5.0.0_batchingMaterialNeed", "batchingMaterialNeed_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchingMaterialNeedDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMBatchingMaterialNeed.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchingMaterialNeed.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchingMaterialNeed.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchingMaterialNeed 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchingMaterialNeed batchingMaterialNeed){
	}
	
	
	@Override
	public List<WOMBatchingMaterialNeed> getBatchingMaterialNeeds(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchingMaterialNeed.class);
		if(params!=null){
			...
			return batchingMaterialNeedDao.findByHql("from " + WOMBatchingMaterialNeed.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchingMaterialNeedDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMBatchingMaterialNeed> findBatchingMaterialNeedsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchingMaterialNeed.class);
	...
	@Override
	public List<WOMBatchingMaterialNeed> findBatchingMaterialNeedsByHql(String hql, Object... objects){
		
		return   batchingMaterialNeedDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchingMaterialNeed loadBatchingMaterialNeedByBussinessKey(WOMBatchingMaterialNeed batchingMaterialNeed){
		return loadBatchingMaterialNeedByBussinessKey(batchingMaterialNeed.getId());
	}
	
	@Override
	public Page<WOMBatchingMaterialNeed> getBatchingMaterialNeeds(Page<WOMBatchingMaterialNeed> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchingMaterialNeed.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchingMaterialNeedDao.findByPage(page, sql);
			}else{
				page = batchingMaterialNeedDao.findAllByPage(page);
			}
			return page;
		}
		page = batchingMaterialNeedDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingMaterialNeed loadBatchingMaterialNeedByBussinessKey(Serializable bussinessKey){
		return batchingMaterialNeedDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchingMaterialNeed> findByProperty(String propertyName, Object object){
		return batchingMaterialNeedDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchingMaterialNeed findEntityByProperty(String propertyName, Object object){
		return batchingMaterialNeedDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchingMaterialNeed.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchingMaterialNeedDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_batchingMaterialNeed", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMBatchingMaterialNeed batchingMaterialNeed,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchingMaterialNeedDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchingMaterialNeed.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchingMaterialNeed);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchingMaterialNeed.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchingMaterialNeed.TABLE_NAME +" where valid = 1";
			list = batchingMaterialNeedDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchingMaterialNeed.TABLE_NAME;
			list = batchingMaterialNeedDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchingMaterialNeed.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchingMaterialNeedDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchingMaterialNeedDaoImpl batchingMaterialNeedDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchingMaterialNeed(final List<WOMBatchingMaterialNeed> insertObjs, final List<WOMBatchingMaterialNeed> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchingMaterialNeed(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchingMaterialNeed(final List<WOMBatchingMaterialNeed> insertObjs, final List<WOMBatchingMaterialNeed> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1490613780787"),importCount));
		...
		Map<Long, WOMBatchingMaterialNeed> oldUpdateObjsMap = new HashMap<Long, WOMBatchingMaterialNeed>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchingMaterialNeed bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchingMaterialNeed> bizObjList = batchingMaterialNeedDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchingMaterialNeed bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchingMaterialNeed> nmeObjs = new ArrayList<WOMBatchingMaterialNeed>();
		...
		long id=autoGeneratorID.getNextId(WOMBatchingMaterialNeed.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchingMaterialNeed batchingMaterialNeed:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchingMaterialNeed.TABLE_NAME,1,dbId);
			if(batchingMaterialNeed.getId() != null ){
				m.put(batchingMaterialNeed.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchingMaterialNeedDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchingMaterialNeedDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchingMaterialNeed.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMBatchingMaterialNeed batchingMaterialNeed = updateObjs.get(updateIndex);
						Long id = batchingMaterialNeed.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchingMaterialNeed.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchingMaterialNeed);
							...
							nmeObjs.add(batchingMaterialNeed);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchingMaterialNeed.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMBatchingMaterialNeed.TABLE_NAME + " order by id desc";
//					Query query = batchingMaterialNeedDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMBatchingMaterialNeed batchingMaterialNeed = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchingMaterialNeed.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchingMaterialNeedDaoImpl.getSession().flush();
		batchingMaterialNeedDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchingMaterialNeed bizObj = insertObjs.get(insertIndex);
			batchingMaterialNeedImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchingMaterialNeed bizObj = updateObjs.get(updateIndex);
			WOMBatchingMaterialNeed oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchingMaterialNeedImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = batchingMaterialNeedDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				batchingMaterialNeedDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = batchingMaterialNeedDao.createNativeQuery(sql, "WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchingMaterialNeedDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchingMaterialNeedDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchingMaterialNeedDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchingMaterialNeedDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchingMaterialNeedDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchingMaterialNeedDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="ec.flow.save")
	public void saveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, DataGridService dataGridService){
		saveBatchingMaterialNeed(batchingMaterialNeed, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="ec.flow.save")
	public void saveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, DataGridService dataGridService, String viewCode){
		saveBatchingMaterialNeed(batchingMaterialNeed, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed")
	public void saveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchingMaterialNeed(batchingMaterialNeed, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed")
	public void saveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchingMaterialNeed";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
		String url = null;
		if(null != batchingMaterialNeed.getId() && batchingMaterialNeed.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchingMaterialNeed.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed")
	public Map<Object, Object> batchingMaterialNeedDataGridImport(WOMBatchingMaterialNeed batchingMaterialNeed, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchingMaterialNeed.getVirtualId();
		
		batchingMaterialNeed = batchingMaterialNeedDao.load(batchingMaterialNeed.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchingMaterialNeed.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchingMaterialNeed.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchingMaterialNeed);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchingMaterialNeed.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed")
	public void mergeBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchingMaterialNeed);
		beforeSaveBatchingMaterialNeed(batchingMaterialNeed);
		batchingMaterialNeedDao.merge(batchingMaterialNeed);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchingMaterialNeed",model="WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_batchingMaterialNeed";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchingMaterialNeed");
	...
	@Override
	public Page<WOMBatchingMaterialNeed> getByPage(Page<WOMBatchingMaterialNeed> page,DetachedCriteria detachedCriteria){
		return batchingMaterialNeedDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_batchingMaterialNeed").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, Object... objects){
		checkUniqueConstraint(batchingMaterialNeed);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchingMaterialNeed(List<WOMBatchingMaterialNeed> batchingMaterialNeeds, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchingMaterialNeed(List<WOMBatchingMaterialNeed> batchingMaterialNeeds){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchingMaterialNeed(List<WOMBatchingMaterialNeed> batchingMaterialNeeds, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchingMaterialNeed(List<WOMBatchingMaterialNeed> batchingMaterialNeeds){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchingMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchingMaterialNeed batchingMaterialNeed, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchingMaterialNeed batchingMaterialNeed, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchingMaterialNeed(List<WOMBatchingMaterialNeed> insertObjs, List<WOMBatchingMaterialNeed> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchingMaterialNeed,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchingMaterialNeed(List<WOMBatchingMaterialNeed> insertObjs, List<WOMBatchingMaterialNeed> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchingMaterialNeed,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_batchingMaterialNeed_BatchingMaterialNeed,WOM_7.5.0.0) */
// 自定义代码
	/**
	 * 根据指令单号删除配料需求记录
	 */
	...
	public void deleteBytaskId(long taskId) {
		List<WOMBatchingMaterialNeed> needList = new ArrayList<WOMBatchingMaterialNeed>();
		String hql = "from WOMBatchingMaterialNeed where valid = 1 and taskID.id=?";
		needList = batchingMaterialNeedDao.findByHql(hql, taskId);
		if(needList.size()>0){
			for(WOMBatchingMaterialNeed need:needList){
				deleteMaterialByMaterialNeed(need);
				//batchingMaterialNeedDao.delete(need);
				
				String deleteSql = "update wom_batching_material_needs set valid = 0 where id = ?";
				batchingMaterialNeedDao.createNativeQuery(deleteSql, need.getId()).executeUpdate();
				batchingMaterialNeedDao.flush();
			}
		}
	}
	 ...
	 * 根据配料需求记录删除配料指令
	 */
	public void deleteMaterialByMaterialNeed(WOMBatchingMaterialNeed batchingMaterialNeed){
		List<WOMBatchingMaterial> materialList = new ArrayList<WOMBatchingMaterial>();
		String hql = "from WOMBatchingMaterial where valid = 1 and needPartId = ?";
		materialList = batchingMaterialDao.findByHql(hql, batchingMaterialNeed);
		if(materialList != null && materialList.size() > 0){
			for(WOMBatchingMaterial batchingMaterial :materialList){
				deleteMaterialPartByMaterial(batchingMaterial);
				//batchingMaterialDao.delete(batchingMaterial);
				
				String deleteSql = "update wom_batching_materials set status = 0 where id = ?";
				batchingMaterialDao.createNativeQuery(deleteSql, batchingMaterial.getId()).executeUpdate();
				batchingMaterialDao.flush();
				
				String pendingSql = "select id from WF_PENDING where table_info_id = ?";
				List<BigDecimal> pendingIds = batchingMaterialDao.createNativeQuery(pendingSql,batchingMaterial.getTableInfoId()).list();
				String deleteSql2 = "delete wf_pending where ";
				String idString = "";
				if(pendingIds != null && pendingIds.size() > 0){
					for(BigDecimal wfId : pendingIds){
						idString += " id = " + wfId + " or";
					}
				}
				if(idString.length() > 0){
					String deletePending = deleteSql2 + idString.substring(0, idString.length()-2);
					batchingMaterialDao.createNativeQuery(deletePending).executeUpdate();
				}
			}
		}
	}
	 ...
	 * 根据配料指令删除配料记录
	 */
	public void deleteMaterialPartByMaterial(WOMBatchingMaterial batchingMaterial){
		List<WOMBatchingMaterialPart> partList = new ArrayList<WOMBatchingMaterialPart>();
		String hql = "from WOMBatchingMaterialPart where valid = 1 and headId = ?";
		partList = batchingMaterialPartDao.findByHql(hql, batchingMaterial);
		if(partList != null && partList.size() > 0){
			for(WOMBatchingMaterialPart part : partList){
				if(part.getState() != null && "womBatPartState/01".equals(part.getState().getId())  ){
					String deleteSql = "update wom_batching_material_parts set valid = 0 where id = ?";
					batchingMaterialPartDao.createNativeQuery(deleteSql, part.getId()).executeUpdate();
					//batchingMaterialPartDao.delete(part);
					batchingMaterialPartDao.flush();
				}
				
				if(part.getState() != null && ("womBatPartState/02".equals(part.getState().getId()) || "womBatPartState/03".equals(part.getState().getId()) ) ){//已配料或者已签收记录退料
					part.setRejectNum(part.getDoneNum());//配料记录退料退料数量
					WOMRejectReason rejectReson = rejectReasonService.getRejectReason(Long.valueOf(1000));
					part.setRejectReason(rejectReson);//配料记录退料退料原因
					part.setState(new SystemCode("womBatPartState/05"));
					batchingMaterialPartDao.save(part);
					batchingMaterialPartDao.flush();
					
					User user = (User) getCurrentUser();  //获取当前用户
					Staff staff =  user != null?user.getStaff():(Staff)part.getCreateStaff();
					
					WOMBatchingRejectParts rejectRecord = new WOMBatchingRejectParts();
	                rejectRecord.setBatchNum(part.getTaskID().getProductBatchNum());//生产批号
	                rejectRecord.setBatchingId(part.getHeadId());//配料指令
	                rejectRecord.setBatchingPartId(part);//配料记录
	                rejectRecord.setProduceTaskId(part.getTaskID());//制造指令单
	                rejectRecord.setProductBatchNum(part.getBatchNum());//物料批号
	               
	                rejectRecord.setRejectExeStaff(staff);//退料操作人
	                rejectRecord.setRejectNum(part.getDoneNum());//退料数量
	                rejectRecord.setRejectProduct(part.getProductId());//退料物品
	                rejectRecord.setRejectReasonId(rejectReson);//退料原因
	                rejectRecord.setRejectTime(new Date());//退料时间
	                rejectRecord.setStore(part.getStoreID());//仓库
	                rejectRecord.setWare(part.getWareID());//货位
	                rejectRecord.setTableInfoId(part.getTableInfoId());
	                batchingRejectPartsService.saveBatchingRejectParts(rejectRecord, null);
				}
			}
		}
	}
