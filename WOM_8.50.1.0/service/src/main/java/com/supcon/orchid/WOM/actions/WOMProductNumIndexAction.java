...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMProductNumIndexAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProductNumIndex> {
	...
	private WOMProductNumIndex productNumIndex;
	...
	//导出excel时,主模型内容
	List<WOMProductNumIndex> queryResults = new ArrayList<WOMProductNumIndex>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/productNumIndex/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/productNumIndex/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_ProductNumIndex" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProductNumIndex> productNumIndexs;
	
	public List<WOMProductNumIndex> getProductNumIndexs() {
		return productNumIndexs;
	}
	...
	private Page<WOMProductNumIndex> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			productNumIndex = new WOMProductNumIndex();
		} else {
			if(id > 0){
				productNumIndex = productNumIndexService.getProductNumIndex(id, viewCode);
			}
		}
	...
	public Page<WOMProductNumIndex> getPage() {
		if (null == page) {
			page = new Page<WOMProductNumIndex>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProductNumIndex getProductNumIndex() {
		return productNumIndex;
	}		

	public void setProductNumIndex(WOMProductNumIndex productNumIndex) {
		this.productNumIndex = productNumIndex;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/productNumIndex/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = productNumIndexService.getProductNumIndexAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/productNumIndex/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1563257962732
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String productNumIndexReferenceCopy() throws Exception{
			WOMProductNumIndex oldProductNumIndex = null;
			if(isEditNew()){
				...
				oldProductNumIndex = new WOMProductNumIndex();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,productNumIndex.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, productNumIndex.getId());
				...
				oldProductNumIndex = productNumIndexService.getProductNumIndex(id);
			}
			
			
			productNumIndex = productNumIndexService.getProductNumIndex(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProductNumIndex");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProductNumIndex.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProductNumIndex, null);
						Method setValue = WOMProductNumIndex.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(productNumIndex, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			productNumIndex.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=productNumIndexService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return productNumIndexReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			productNumIndex = productNumIndexService.getProductNumIndex(this.productNumIndex.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,productNumIndex.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			productNumIndexService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = productNumIndexService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(productNumIndexService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(productNumIndexService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_ProductNumIndex", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"productNumIndex\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"productNumIndex\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"productNumIndex\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"productNumIndex\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			productNumIndexService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		productNumIndexService.findDataGridPage(dg,dgclass,getDgPage(),productNumIndex,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(productNumIndex.getId() != null && productNumIndex.getId() != -1){
				productNumIndex = productNumIndexService.getProductNumIndex(productNumIndex.getId());
			}
			productNumIndexService.findDataGridPage(dg,dgclass,getDgPage(),productNumIndex,searchCondition,params);
	...
	private void save() throws Exception {
		...
		productNumIndexService.saveProductNumIndex(productNumIndex,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "submit");
		bap_validate_datagrids=productNumIndexService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", productNumIndex.getId());
		...
		fileuploads.put("linkId", productNumIndex.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/productNumIndex/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		productNumIndexService.deleteProductNumIndex(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/productNumIndex/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "restore");
		productNumIndexService.restoreProductNumIndex(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProductNumIndex> findExportDataInfos(Page<WOMProductNumIndex> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = productNumIndexService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = productNumIndexService.getAssProperty(p.getCode());	
									//String assModelCode = productNumIndexService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			productNumIndexService.findProductNumIndexs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			productNumIndexService.findProductNumIndexs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProductNumIndex> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProductNumIndex result : results){
	...
	public boolean isEditNew(){
		//return null == this.productNumIndex || null == this.productNumIndex.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/productNumIndex/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/productNumIndex/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/productNumIndex/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/productNumIndex/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProductNumIndexService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/productNumIndex/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_ProductNumIndex", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_ProductNumIndex", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/productNumIndex/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/productNumIndex/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_ProductNumIndex";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProductNumIndex", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/productNumIndex/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMProductNumIndex productNumIndex = null;
		if(null != id) {
			productNumIndex = productNumIndexService.getProductNumIndex(id);
		}
		...
		_auditLog.setColumnStringB(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
		_auditLog.setColumnStringC(productNumIndex.getId().toString());
		_auditLog.setColumnStringD(WOMProductNumIndex.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProductNumIndexAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, productNumIndex,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, productNumIndex,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,productNumIndex);
		...
		printlog.setBusinessId(productNumIndex.getId());
		printlog.setBussinessKey(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/productNumIndex/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(productNumIndex,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/productNumIndex/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(productNumIndexService.getProductNumIndex(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/productNumIndex/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/productNumIndex/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, productNumIndex,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				productNumIndex = productNumIndexService.getProductNumIndex(Long.parseLong(arrId[i]));
				if(null != productNumIndex){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == productNumIndex.getId() ? "" : "," + productNumIndex.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == productNumIndex.getId() ? "" : "," + productNumIndex.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, productNumIndex,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,productNumIndex);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//productNumIndexService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/productNumIndex/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProductNumIndex");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_ProductNumIndex")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_ProductNumIndex");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProductNumIndex");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = productNumIndexService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
