...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMProdTaskProcExelogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProdTaskProcExelog> {
	...
	private WOMProdTaskProcExelog prodTaskProcExelog;
	...
	//导出excel时,主模型内容
	List<WOMProdTaskProcExelog> queryResults = new ArrayList<WOMProdTaskProcExelog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/prodTaskProcExelog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/prodTaskProcExelog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_ProdTaskProcExelog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProdTaskProcExelog> prodTaskProcExelogs;
	
	public List<WOMProdTaskProcExelog> getProdTaskProcExelogs() {
		return prodTaskProcExelogs;
	}
	...
	private Page<WOMProdTaskProcExelog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			prodTaskProcExelog = new WOMProdTaskProcExelog();
			if(prodTaskProcExelog.getState() != null) {
				prodTaskProcExelog.setState((SystemCode) systemCodeService.getSystemCode(prodTaskProcExelog.getState().getId()));
			}
		} else {
			if(id > 0){
				prodTaskProcExelog = prodTaskProcExelogService.getProdTaskProcExelog(id, viewCode);
			}
		}
	...
	public Page<WOMProdTaskProcExelog> getPage() {
		if (null == page) {
			page = new Page<WOMProdTaskProcExelog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProdTaskProcExelog getProdTaskProcExelog() {
		return prodTaskProcExelog;
	}		

	public void setProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog) {
		this.prodTaskProcExelog = prodTaskProcExelog;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prodTaskProcExelogService.getProdTaskProcExelogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1495777422447
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String prodTaskProcExelogReferenceCopy() throws Exception{
			WOMProdTaskProcExelog oldProdTaskProcExelog = null;
			if(isEditNew()){
				...
				oldProdTaskProcExelog = new WOMProdTaskProcExelog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodTaskProcExelog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prodTaskProcExelog.getId());
				...
				oldProdTaskProcExelog = prodTaskProcExelogService.getProdTaskProcExelog(id);
			}
			
			
			prodTaskProcExelog = prodTaskProcExelogService.getProdTaskProcExelog(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProdTaskProcExelog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProdTaskProcExelog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProdTaskProcExelog, null);
						Method setValue = WOMProdTaskProcExelog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prodTaskProcExelog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prodTaskProcExelog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=prodTaskProcExelogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return prodTaskProcExelogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			prodTaskProcExelog = prodTaskProcExelogService.getProdTaskProcExelog(this.prodTaskProcExelog.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodTaskProcExelog.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			prodTaskProcExelogService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = prodTaskProcExelogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(prodTaskProcExelogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(prodTaskProcExelogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/prodTaskProcExelog/processRecords", results = { @Result(type = FREEMARKER, location = "processRecords.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"prodTaskProcExelog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"prodTaskProcExelog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"prodTaskProcExelog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"prodTaskProcExelog\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			prodTaskProcExelogService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/prodTaskProcExelog/processRecords-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchNum,result.batchUnitID,result.startTime,result.endTime,result.remark,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="processRecords-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		prodTaskProcExelogService.findDataGridPage(dg,dgclass,getDgPage(),prodTaskProcExelog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(prodTaskProcExelog.getId() != null && prodTaskProcExelog.getId() != -1){
				prodTaskProcExelog = prodTaskProcExelogService.getProdTaskProcExelog(prodTaskProcExelog.getId());
			}
			prodTaskProcExelogService.findDataGridPage(dg,dgclass,getDgPage(),prodTaskProcExelog,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == prodTaskProcExelog.getId()) {
		...
		prodTaskProcExelogService.saveProdTaskProcExelog(prodTaskProcExelog,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "submit");
		bap_validate_datagrids=prodTaskProcExelogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", prodTaskProcExelog.getId());
		...
		fileuploads.put("linkId", prodTaskProcExelog.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/prodTaskProcExelog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prodTaskProcExelogService.deleteProdTaskProcExelog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "restore");
		prodTaskProcExelogService.restoreProdTaskProcExelog(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProdTaskProcExelog> findExportDataInfos(Page<WOMProdTaskProcExelog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = prodTaskProcExelogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = prodTaskProcExelogService.getAssProperty(p.getCode());	
									//String assModelCode = prodTaskProcExelogService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			prodTaskProcExelogService.findProdTaskProcExelogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			prodTaskProcExelogService.findProdTaskProcExelogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProdTaskProcExelog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProdTaskProcExelog result : results){
	...
	public boolean isEditNew(){
		//return null == this.prodTaskProcExelog || null == this.prodTaskProcExelog.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/prodTaskProcExelog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/prodTaskProcExelog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProdTaskProcExelogService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_ProdTaskProcExelog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_ProdTaskProcExelog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_ProdTaskProcExelog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskProcExelog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
		_auditLog.setColumnStringA(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
		_auditLog.setColumnStringC(prodTaskProcExelog.getId().toString());
		_auditLog.setColumnStringD(WOMProdTaskProcExelog.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProdTaskProcExelogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, prodTaskProcExelog,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, prodTaskProcExelog,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,prodTaskProcExelog);
		...
		printlog.setBusinessId(prodTaskProcExelog.getId());
		printlog.setBussinessKey(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
		printlog.setMainDisplay(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(prodTaskProcExelog,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(prodTaskProcExelogService.getProdTaskProcExelog(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, prodTaskProcExelog,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				prodTaskProcExelog = prodTaskProcExelogService.getProdTaskProcExelog(Long.parseLong(arrId[i]));
				if(null != prodTaskProcExelog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == prodTaskProcExelog.getId() ? "" : "," + prodTaskProcExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == prodTaskProcExelog.getId() ? "" : "," + prodTaskProcExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == prodTaskProcExelog.getId() ? "" : "," + prodTaskProcExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, prodTaskProcExelog,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,prodTaskProcExelog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//prodTaskProcExelogService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/prodTaskProcExelog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProdTaskProcExelog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_ProdTaskProcExelog")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_ProdTaskProcExelog");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProdTaskProcExelog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = prodTaskProcExelogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
