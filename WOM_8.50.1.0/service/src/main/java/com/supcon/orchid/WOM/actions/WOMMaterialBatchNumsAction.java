...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMMaterialBatchNumsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMMaterialBatchNums> {
	...
	private WOMMaterialBatchNums materialBatchNums;
	...
	//导出excel时,主模型内容
	List<WOMMaterialBatchNums> queryResults = new ArrayList<WOMMaterialBatchNums>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/materialBatchNums/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/materialBatchNums/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_MaterialBatchNums" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMMaterialBatchNums> materialBatchNumss;
	
	public List<WOMMaterialBatchNums> getMaterialBatchNumss() {
		return materialBatchNumss;
	}
	...
	private Page<WOMMaterialBatchNums> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			materialBatchNums = new WOMMaterialBatchNums();
		} else {
			if(id > 0){
				materialBatchNums = materialBatchNumsService.getMaterialBatchNums(id, viewCode);
			}
		}
	...
	public Page<WOMMaterialBatchNums> getPage() {
		if (null == page) {
			page = new Page<WOMMaterialBatchNums>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMMaterialBatchNums getMaterialBatchNums() {
		return materialBatchNums;
	}		

	public void setMaterialBatchNums(WOMMaterialBatchNums materialBatchNums) {
		this.materialBatchNums = materialBatchNums;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/materialBatchNums/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = materialBatchNumsService.getMaterialBatchNumsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/materialBatchNums/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1490066410355
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String materialBatchNumsReferenceCopy() throws Exception{
			WOMMaterialBatchNums oldMaterialBatchNums = null;
			if(isEditNew()){
				...
				oldMaterialBatchNums = new WOMMaterialBatchNums();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,materialBatchNums.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, materialBatchNums.getId());
				...
				oldMaterialBatchNums = materialBatchNumsService.getMaterialBatchNums(id);
			}
			
			
			materialBatchNums = materialBatchNumsService.getMaterialBatchNums(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_MaterialBatchNums");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMMaterialBatchNums.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMaterialBatchNums, null);
						Method setValue = WOMMaterialBatchNums.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(materialBatchNums, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=materialBatchNumsService.findValidateDatagrids(getDgClassMap());
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return materialBatchNumsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			materialBatchNums = materialBatchNumsService.getMaterialBatchNums(this.materialBatchNums.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,materialBatchNums.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			materialBatchNumsService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			List<Object[]> list = materialBatchNumsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_MaterialBatchNums", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"materialBatchNums\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"materialBatchNums\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			materialBatchNumsService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		materialBatchNumsService.findDataGridPage(dg,dgclass,getDgPage(),materialBatchNums,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if(refId!=null&&refId>0){
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			materialBatchNumsService.findDataGridPage(dg,dgclass,getDgPage(),materialBatchNums,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == materialBatchNums.getId()) {
			responseMap.put("operate", "add");
		} else {
			responseMap.put("operate", "edit");
		}
		materialBatchNumsService.saveMaterialBatchNums(materialBatchNums,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "submit");
		bap_validate_datagrids=materialBatchNumsService.findValidateDatagrids(getDgClassMap());
			save();
			responseMap.put("id", materialBatchNums.getId());
		...
		fileuploads.put("linkId", materialBatchNums.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/produceTask/materialBatchNums/delete", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "delete");
		materialBatchNumsService.deleteMaterialBatchNums(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "delete");
		return SUCCESS;
	}
	...
	@Override
	public Page<WOMMaterialBatchNums> findExportDataInfos(Page<WOMMaterialBatchNums> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = materialBatchNumsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = materialBatchNumsService.getAssProperty(p.getCode());	
									//String assModelCode = materialBatchNumsService.getPropertyModelCode(assProperty);
				...
				if(bkIsInherent){//较早版本的业务包可能version字段作为业务主键，此种情况通过id管理
		...
		if(null != searchObjects && searchObjects.length > 0) {
			materialBatchNumsService.findMaterialBatchNumss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			materialBatchNumsService.findMaterialBatchNumss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMMaterialBatchNums> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMMaterialBatchNums result : results){
	...
	public boolean isEditNew(){
		//return null == this.materialBatchNums || null == this.materialBatchNums.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/materialBatchNums/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/materialBatchNums/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/materialBatchNums/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/materialBatchNums/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMMaterialBatchNumsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/materialBatchNums/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_MaterialBatchNums", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_MaterialBatchNums", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/materialBatchNums/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/materialBatchNums/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_MaterialBatchNums";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_MaterialBatchNums", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/materialBatchNums/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		WOMMaterialBatchNums materialBatchNums = null;
		if(null != id) {
			materialBatchNums = materialBatchNumsService.getMaterialBatchNums(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, materialBatchNums, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, materialBatchNums, null, null, true, this);
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		materialBatchNumsService.print(1);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/materialBatchNums/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/materialBatchNums/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				materialBatchNums = materialBatchNumsService.getMaterialBatchNums(Integer.parseInt(id));
				if(materialBatchNums != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, materialBatchNums, null, null, true, this);
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_MaterialBatchNums");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_MaterialBatchNums")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_MaterialBatchNums");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_MaterialBatchNums");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(isCustom != null && isCustom.equals("true")){
			...
			List<CustomPropertyModelMapping> modelMappings = null;
			if(excelModelMappings!=null && excelModelMappings.get(property.getModel().getCode())!=null ){
				modelMappings = (List<CustomPropertyModelMapping>) excelModelMappings.get(property.getModel().getCode());
			}else{
				modelMappings = viewServiceFoundation.findCustomPropertyForAsso(property.getModel().getCode(), property.getCode());
				this.excelModelMappings.put(property.getModel().getCode(), modelMappings);
			}
			if(modelMappings != null && modelMappings.size()>0){
		...
		if(fillContent != null){
			...
			List<String> values = materialBatchNumsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void bapAfterCustomMethod(String code, String methodName) throws Exception {
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
}