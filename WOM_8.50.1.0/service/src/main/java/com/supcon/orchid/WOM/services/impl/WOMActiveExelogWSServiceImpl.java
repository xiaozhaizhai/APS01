...
import com.supcon.orchid.WOM.services.WOMActiveExelogWSService;
import com.supcon.orchid.WOM.services.WOMActiveExelogService;
	
/* CUSTOM CODE START(wsserviceimpl,import,WOM_7.5.0.0_activeExelog_ActiveExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="WOMActiveExelog",targetNamespace = "http://ws.supcon.com")
public class WOMActiveExelogWSServiceImpl implements WOMActiveExelogWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getActiveExelogByBusinessKey(
		...
		try{
			...
			WOMActiveExelog activeExelog = activeExelogService.loadActiveExelogByBussinessKey(businessKey);
			if(activeExelog == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, activeExelog);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateActiveExelog(
		...
		try{
			if(activeExelog == null){
				return set500Error(result, "指令单活动执行记录不能为空。");
			}
			...
			if(!beforeUpdateActiveExelog(activeExelog)){
				return set404Error(result);
			}
			activeExelogService.saveSuperEditActiveExelog(activeExelog, null, null, null, null, null, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addActiveExelog(
		...
		try{
			...
			if(activeExelog.getTableNo() == null){
				return set500Error(result, "业务主键不能为空。");
			}
			...
			try{
				checkActiveExelogData(activeExelog);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			activeExelogService.saveSuperEditActiveExelog(activeExelog, null, null, null);
			setSuccessResult(result, activeExelog.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteActiveExelogByBusinessKey(
		...
		try{
			...
			try{
				activeExelog = activeExelogService.loadActiveExelogByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddActiveExelog(
		...
		try{
			...
			int count = 1;
			for(WOMActiveExelog activeExelog : activeExelogList){
				if(activeExelog.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(activeExelog.getTableNo() == null || activeExelog.getTableNo().toString().trim().isEmpty()){
			...
			activeExelogService.batchSaveSuperEditActiveExelog(activeExelogList, null, null, null);
			for(WOMActiveExelog activeExelog : activeExelogList){
				idList.add(activeExelog.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdateActiveExelog(
		...
		try{
			...
			activeExelogService.batchSaveSuperEditActiveExelog(activeExelogList, null, null, null,null, null);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteActiveExelogByPk(
		...
		try{
			...
			for(String k : pk){
				...
				WOMActiveExelog temp = null;
				try{
					temp = activeExelogService.getActiveExelog(id);
				}catch(NullPointerException e){}
			...
			activeExelogService.deleteActiveExelog(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteActiveExelogByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				WOMActiveExelog temp = null;
				try{
					temp = activeExelogService.loadActiveExelogByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getActiveExelogPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(WOMActiveExelog.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),WOMActiveExelog.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,WOMActiveExelog.class);
				} catch (Exception e) {
			...
			Page<WOMActiveExelog> page = new Page<WOMActiveExelog>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<WOMActiveExelog> datas = activeExelogService.getByPage(page, detachedCriteria);
			List<WOMActiveExelog> activeExelogList = datas.getResult();
			if (activeExelogList == null || activeExelogList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(activeExelogList.toArray());
			PaginationDTO<WOMActiveExelog> pagination = new PaginationDTO<WOMActiveExelog>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			WOMActiveExelog activeExelog = activeExelogService.getActiveExelog(id);
			if (activeExelog == null || !activeExelog.isValid()) {
				result.setErrorMessage("找不到指令单活动执行记录");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取指令单活动执行记录对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param activeExelog
	 * @return
	 */
	private WOMActiveExelog getActiveExelog(WOMActiveExelog activeExelog) {
		if (activeExelog.getId() == null && (activeExelog.getTableNo() == null || activeExelog.getTableNo().toString().trim() == ""))
			throw new RuntimeException("指令单活动执行记录对象业务主键和id不能同时为空");
		if (activeExelog.getId() != null) {
			WOMActiveExelog result = activeExelogService.getActiveExelog(activeExelog .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return activeExelogService.loadActiveExelogByBussinessKey(activeExelog.getTableNo());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个指令单活动执行记录，则返回false，否则返回true
	 * @param activeExelog
	 */
	private boolean beforeUpdateActiveExelog(WOMActiveExelog activeExelog) {
		activeExelog.setStatus(99);
		if(activeExelog.getTableInfoId() == null){
			throw new RuntimeException("tableInfoId不能为空。");
		}
		WOMActiveExelog temp = getActiveExelog(activeExelog);
		if (temp == null) {
			return false;
		}
		if(activeExelog.getId() == null){
			activeExelog.setId(temp.getId());		
		}
		if(activeExelog.getTableNo() == null){
			activeExelog.setTableNo(temp.getTableNo());
		}
		
		activeExelog.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除指令单活动执行记录对象下所有辅模型对象中的指令单活动执行记录对象
	 * @param activeExelog
	 */
	private void clearCircle(WOMActiveExelog activeExelog) {
	 ...
	 */
	private void checkActiveExelogData(WOMActiveExelog activeExelog){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,WOM_7.5.0.0_activeExelog_ActiveExelog,WOM_7.5.0.0) */
// 自定义代码
