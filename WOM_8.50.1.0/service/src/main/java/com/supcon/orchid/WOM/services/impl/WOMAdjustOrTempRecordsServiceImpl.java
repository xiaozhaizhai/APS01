...
import com.supcon.orchid.WOM.services.WOMAdjustOrTempRecordsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMAdjustOrTempRecordsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_adjustOrTempRecordsService")
@Transactional
public class WOMAdjustOrTempRecordsServiceImpl  extends BaseServiceImpl<WOMAdjustOrTempRecords> implements WOMAdjustOrTempRecordsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMAdjustOrTempRecords> bapBaseService;
	...
	@Autowired
	private WOMAdjustOrTempRecordsImportService adjustOrTempRecordsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMAdjustOrTempRecords.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  adjustOrTempRecordsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMAdjustOrTempRecords.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = adjustOrTempRecordsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMAdjustOrTempRecords.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMAdjustOrTempRecords.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  adjustOrTempRecordsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMAdjustOrTempRecords.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMAdjustOrTempRecords.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMAdjustOrTempRecords> list = (List<WOMAdjustOrTempRecords>) adjustOrTempRecordsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMAdjustOrTempRecords getAdjustOrTempRecords(long id){
		return getAdjustOrTempRecords(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMAdjustOrTempRecords getAdjustOrTempRecords(long id, String viewCode){
		WOMAdjustOrTempRecords adjustOrTempRecords = adjustOrTempRecordsDao.load(id);

		// 一对多情况处理
		if(adjustOrTempRecords != null){
		}
		return adjustOrTempRecords;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getAdjustOrTempRecordsAsJSON(long id, String include){
		WOMAdjustOrTempRecords adjustOrTempRecords = adjustOrTempRecordsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(adjustOrTempRecords, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords){
		if(null != adjustOrTempRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
			AuditUtil.setColumnStringA(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
			AuditUtil.setColumnStringC(adjustOrTempRecords.getId().toString());
		}
		beforeDeleteAdjustOrTempRecords(adjustOrTempRecords);
		adjustOrTempRecordsDao.deletePhysical(adjustOrTempRecords);
		afterDeleteAdjustOrTempRecords(adjustOrTempRecords);
		adjustOrTempRecordsDao.flush();
		List<WOMAdjustOrTempRecords> params = new ArrayList<WOMAdjustOrTempRecords>();
		params.add(adjustOrTempRecords);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempRecords(List<Long> adjustOrTempRecordsIds){
		deleteAdjustOrTempRecords(adjustOrTempRecordsIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempRecords(List<Long> adjustOrTempRecordsIds, String eventTopic) {
		List<WOMAdjustOrTempRecords> adjustOrTempRecordss = new ArrayList<WOMAdjustOrTempRecords>();
		for(Long adjustOrTempRecordsId : adjustOrTempRecordsIds){
			WOMAdjustOrTempRecords adjustOrTempRecords = getAdjustOrTempRecords(adjustOrTempRecordsId);
			adjustOrTempRecordss.add(adjustOrTempRecords);
			if(null != adjustOrTempRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == adjustOrTempRecords.getId() ? "" : "," + adjustOrTempRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == adjustOrTempRecords.getId() ? "" : "," + adjustOrTempRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == adjustOrTempRecords.getId() ? "" : "," + adjustOrTempRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
				}
			}
		}
		
		for(WOMAdjustOrTempRecords adjustOrTempRecords : adjustOrTempRecordss){
			beforeDeleteAdjustOrTempRecords(adjustOrTempRecords);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(adjustOrTempRecordss != null && adjustOrTempRecordss.size() > 0 ){
			for(WOMAdjustOrTempRecords adjustOrTempRecords : adjustOrTempRecordss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMAdjustOrTempRecords.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(adjustOrTempRecords));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),adjustOrTempRecords, adjustOrTempRecords.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(adjustOrTempRecordsIds != null && adjustOrTempRecordsIds.size() > 0) {
			String hql = "delete " + WOMAdjustOrTempRecords.JPA_NAME + " where id in(:ids)";
			Query query = adjustOrTempRecordsDao.createQuery(hql);
			query.setParameterList("ids", adjustOrTempRecordsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMAdjustOrTempRecords adjustOrTempRecordsz : adjustOrTempRecordss){
				adjustOrTempRecordsz.setValid(false);
				List<WOMAdjustOrTempRecords> params = new ArrayList<WOMAdjustOrTempRecords>();
				params.add(adjustOrTempRecordsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempRecords(long adjustOrTempRecordsId, int adjustOrTempRecordsVersion){
		this.deleteAdjustOrTempRecords(adjustOrTempRecordsId, adjustOrTempRecordsVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempRecords(long adjustOrTempRecordsId, int adjustOrTempRecordsVersion,SignatureLog signatureLog){
		WOMAdjustOrTempRecords adjustOrTempRecords = getAdjustOrTempRecords(adjustOrTempRecordsId);
		if(adjustOrTempRecords != null && !adjustOrTempRecords.isValid()){
		...
		beforeDeleteAdjustOrTempRecords(adjustOrTempRecords);
		adjustOrTempRecordsDao.delete(adjustOrTempRecordsId, adjustOrTempRecordsVersion);
		afterDeleteAdjustOrTempRecords(adjustOrTempRecords);
		adjustOrTempRecordsDao.flush();
		List<WOMAdjustOrTempRecords> params = new ArrayList<WOMAdjustOrTempRecords>();
		params.add(adjustOrTempRecords);
		...
		if(signatureLog != null ) {
			...
			businessKey=adjustOrTempRecords.getId();
			signatureLog.setTableId(adjustOrTempRecords.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempRecords(String adjustOrTempRecordsIds){
		this.deleteAdjustOrTempRecords(adjustOrTempRecordsIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.common.delete",operType="3")
	public void deleteAdjustOrTempRecords(String adjustOrTempRecordsIds,SignatureLog signatureLog){
		deleteCollection(adjustOrTempRecordsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteAdjustOrTempRecords(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_AdjustOrTempRecords#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="ec.engine.restore",operType="10")
	public void restoreAdjustOrTempRecords(String adjustOrTempRecordsIds){
		restoreCollection(adjustOrTempRecordsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreAdjustOrTempRecords(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="ec.engine.restore",operType="10")
	public void restoreAdjustOrTempRecords(long adjustOrTempRecordsId){
		
		findBusinessKeyUsed(adjustOrTempRecordsId);	//判断业务主键是否重复
		...
		beforeRestoreAdjustOrTempRecords(adjustOrTempRecords);
		adjustOrTempRecords.setValid(true);
		adjustOrTempRecordsDao.update(adjustOrTempRecords);
		afterRestoreAdjustOrTempRecords(adjustOrTempRecords);
		List<WOMAdjustOrTempRecords> params = new ArrayList<WOMAdjustOrTempRecords>();
		params.add(adjustOrTempRecords);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long adjustOrTempRecordsId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_AdjustOrTempRecords");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMAdjustOrTempRecords.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMAdjustOrTempRecords.TABLE_NAME +" where id = ? )";
			List<Object> list =  adjustOrTempRecordsDao.createNativeQuery(sql,adjustOrTempRecordsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseAdjustOrTempRecords(List<WOMAdjustOrTempRecords>  adjustOrTempRecordss){
		for(WOMAdjustOrTempRecords adjustOrTempRecords:adjustOrTempRecordss)  {
			saveAdjustOrTempRecords(adjustOrTempRecords, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseAdjustOrTempRecords(List<WOMAdjustOrTempRecords>  adjustOrTempRecordss){
		for(WOMAdjustOrTempRecords adjustOrTempRecords:adjustOrTempRecordss)  {
			saveAdjustOrTempRecords(adjustOrTempRecords, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="ec.flow.save")
	public void saveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveAdjustOrTempRecords(adjustOrTempRecords, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="ec.flow.save")
	public void saveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveAdjustOrTempRecords(adjustOrTempRecords, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateAdjustOrTempRecordsCodes(WOMAdjustOrTempRecords adjustOrTempRecords) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAdjustOrTempRecordsCodes(adjustOrTempRecords, false);
	}
	
	public void generateAdjustOrTempRecordsCodes(WOMAdjustOrTempRecords adjustOrTempRecords, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateAdjustOrTempRecordsSummarys(WOMAdjustOrTempRecords adjustOrTempRecords) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAdjustOrTempRecordsSummarys(adjustOrTempRecords, false);
	}

	public void generateAdjustOrTempRecordsSummarys(WOMAdjustOrTempRecords adjustOrTempRecords, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords")
	public void saveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveAdjustOrTempRecords(adjustOrTempRecords, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords")
	public void saveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(adjustOrTempRecords.getId() != null && adjustOrTempRecords.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, adjustOrTempRecords);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(adjustOrTempRecords);
		beforeSaveAdjustOrTempRecords(adjustOrTempRecords, viewIsView);

		if (viewIsView) {
			adjustOrTempRecordsDao.saveWithRevertVersion(adjustOrTempRecords);
		} else {
			if(isNew)
				adjustOrTempRecordsDao.save(adjustOrTempRecords);
			else
				adjustOrTempRecordsDao.merge(adjustOrTempRecords);
		}
		if(null != adjustOrTempRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
			AuditUtil.setColumnStringA(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
		...
		dealDatagridsSave(adjustOrTempRecords,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateAdjustOrTempRecordsCodes(adjustOrTempRecords, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateAdjustOrTempRecordsSummarys(adjustOrTempRecords, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveAdjustOrTempRecords(adjustOrTempRecords, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, adjustOrTempRecords);
			}
		}
		List<WOMAdjustOrTempRecords> params = new ArrayList<WOMAdjustOrTempRecords>();
		adjustOrTempRecordsDao.flush();
		adjustOrTempRecordsDao.clear();
		adjustOrTempRecords = adjustOrTempRecordsDao.load(adjustOrTempRecords.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMAdjustOrTempRecords adjustOrTempRecords){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("adjustOrTempRecords", adjustOrTempRecords);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords")
	public void mergeAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(adjustOrTempRecords);
		beforeSaveAdjustOrTempRecords(adjustOrTempRecords);
		adjustOrTempRecordsDao.merge(adjustOrTempRecords);
		if(null != adjustOrTempRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
			AuditUtil.setColumnStringA(null == adjustOrTempRecords.getId() ? "" : adjustOrTempRecords.getId().toString());
		...
		dealDatagridsSave(adjustOrTempRecords,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveAdjustOrTempRecords(adjustOrTempRecords);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMAdjustOrTempRecords> findAdjustOrTempRecordss(Page<WOMAdjustOrTempRecords> page, Criterion... criterions) {
		return adjustOrTempRecordsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAdjustOrTempRecordss(Page<WOMAdjustOrTempRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findAdjustOrTempRecordss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAdjustOrTempRecordss(Page<WOMAdjustOrTempRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAdjustOrTempRecordss(Page<WOMAdjustOrTempRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMAdjustOrTempRecords> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"adjustOrTempRecords\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"adjustOrTempRecords\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"adjustOrTempRecords\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"adjustOrTempRecords\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  adjustOrTempRecordsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  adjustOrTempRecordsDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"adjustOrTempRecords\".LAY_REC ASC, \"adjustOrTempRecords\".SORT ASC");
				} else {
					orderPart.append(" \"adjustOrTempRecords\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) adjustOrTempRecordsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=adjustOrTempRecordsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = adjustOrTempRecordsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMAdjustOrTempRecords> result = new ArrayList<WOMAdjustOrTempRecords>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMAdjustOrTempRecords.class,adjustOrTempRecordsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMAdjustOrTempRecords.class,adjustOrTempRecordsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMAdjustOrTempRecords adjustOrTempRecords : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(adjustOrTempRecords.getId(), "WOM_produceTask_adjustOrTempRecords");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(adjustOrTempRecords.getId(), "WOM_produceTask_adjustOrTempRecords");
							if(null != documents && !documents.isEmpty()) {
								adjustOrTempRecords.setDocument(documents.get(0));
								adjustOrTempRecords.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMAdjustOrTempRecords> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMAdjustOrTempRecords> result) {
		View view = null;
		...
		Map<Long, WOMAdjustOrTempRecords> rsMap = new HashMap<Long, WOMAdjustOrTempRecords>();
		for (WOMAdjustOrTempRecords adjustOrTempRecords : result) {
			ids.add(adjustOrTempRecords.getId());
			rsMap.put(adjustOrTempRecords.getId(), adjustOrTempRecords);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(adjustOrTempRecordsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = adjustOrTempRecordsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_AdjustOrTempRecords" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_AdjustOrTempRecords" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  adjustOrTempRecordsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"adjustOrTempRecords\"","WOM_7.5.0.0_produceTask","","WOM_ADJUST_OR_TEMP_RECORDS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"adjustOrTempRecords\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"adjustOrTempRecords\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"adjustOrTempRecords\"","WOM_7.5.0.0_produceTask","","WOM_ADJUST_OR_TEMP_RECORDS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_AdjustOrTempRecords" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("adjustOrTempRecords.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"adjustOrTempRecords\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"adjustOrTempRecords\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"adjustOrTempRecords\".CID = 1 OR \"adjustOrTempRecords\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("adjustOrTempRecords.mc\"").append(".ADJUST_OR_TEMP_RECORDS) not in ("+notinValue+")");
			}
			
			SQLQuery query = adjustOrTempRecordsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=adjustOrTempRecordsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMAdjustOrTempRecords.ENTITY_CODE;}
	protected String getModuleCode(){return WOMAdjustOrTempRecords.MODULE_CODE;}
	protected String getModelCode(){return WOMAdjustOrTempRecords.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param adjustOrTempRecords 模型实例
	 */
	public void checkUniqueConstraint(WOMAdjustOrTempRecords adjustOrTempRecords){
	}
	
	
	@Override
	public List<WOMAdjustOrTempRecords> getAdjustOrTempRecordss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMAdjustOrTempRecords.class);
		if(params!=null){
			...
			return adjustOrTempRecordsDao.findByHql("from " + WOMAdjustOrTempRecords.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return adjustOrTempRecordsDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMAdjustOrTempRecords> findAdjustOrTempRecordssByHql(String hql, Object... objects){
		...
		return   adjustOrTempRecordsDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMAdjustOrTempRecords loadAdjustOrTempRecordsByBussinessKey(WOMAdjustOrTempRecords adjustOrTempRecords){
		return loadAdjustOrTempRecordsByBussinessKey(adjustOrTempRecords.getId());
	}
	
	@Override
	public Page<WOMAdjustOrTempRecords> getAdjustOrTempRecordss(Page<WOMAdjustOrTempRecords> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMAdjustOrTempRecords.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = adjustOrTempRecordsDao.findByPage(page, sql);
			}else{
				page = adjustOrTempRecordsDao.findAllByPage(page);
			}
			return page;
		}
		page = adjustOrTempRecordsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMAdjustOrTempRecords> findByTreeDataGridProduceTask(List<WOMAdjustOrTempRecords> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMAdjustOrTempRecords adjustOrTempRecords : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTask(Page<WOMAdjustOrTempRecords> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMAdjustOrTempRecords> adjustOrTempRecordss = dgPage.getResult();
			for(WOMAdjustOrTempRecords adjustOrTempRecords : adjustOrTempRecordss){
				
			}
			dgPage.setResult(adjustOrTempRecordss);
		}
	...
	public WOMAdjustOrTempRecords loadAdjustOrTempRecordsByBussinessKey(Serializable bussinessKey){
		return adjustOrTempRecordsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMAdjustOrTempRecords> findByProperty(String propertyName, Object object){
		return adjustOrTempRecordsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMAdjustOrTempRecords findEntityByProperty(String propertyName, Object object){
		return adjustOrTempRecordsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMAdjustOrTempRecords.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = adjustOrTempRecordsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_AdjustOrTempRecords","runtime");
		try {
	...
	public void dealDatagridsSave(WOMAdjustOrTempRecords adjustOrTempRecords,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,adjustOrTempRecordsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMAdjustOrTempRecords.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, adjustOrTempRecords);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(adjustOrTempRecords.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(adjustOrTempRecords.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(adjustOrTempRecords.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",adjustOrTempRecords.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMAdjustOrTempRecords.TABLE_NAME +" where valid = 1";
			list = adjustOrTempRecordsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMAdjustOrTempRecords.TABLE_NAME;
			list = adjustOrTempRecordsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMAdjustOrTempRecords.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = adjustOrTempRecordsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMAdjustOrTempRecordsDaoImpl adjustOrTempRecordsDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMAdjustOrTempRecords.TABLE_NAME + " where id = ?";
		String tableNo = adjustOrTempRecordsDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchAdjustOrTempRecords(final List<WOMAdjustOrTempRecords> insertObjs, final List<WOMAdjustOrTempRecords> updateObjs, 
		...
		return this.importBatchAdjustOrTempRecords(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchAdjustOrTempRecords(final List<WOMAdjustOrTempRecords> insertObjs, final List<WOMAdjustOrTempRecords> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1542097599573"),importCount));
		...
		Map<Long, WOMAdjustOrTempRecords> oldUpdateObjsMap = new HashMap<Long, WOMAdjustOrTempRecords>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMAdjustOrTempRecords bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMAdjustOrTempRecords> bizObjList = adjustOrTempRecordsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMAdjustOrTempRecords bizObj = bizObjList.get(mIndex);
		...
		final List<WOMAdjustOrTempRecords> nmeObjs = new ArrayList<WOMAdjustOrTempRecords>();
		...
		//String idSql = "select id from " + WOMAdjustOrTempRecords.TABLE_NAME + " order by id desc";
		//Query query = adjustOrTempRecordsDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMAdjustOrTempRecords.JPA_NAME + " order by id desc";
		Query query =  adjustOrTempRecordsDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMAdjustOrTempRecords.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMAdjustOrTempRecords adjustOrTempRecords:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMAdjustOrTempRecords.TABLE_NAME,1,dbId);
			if(adjustOrTempRecords.getId() != null ){
				m.put(adjustOrTempRecords.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			adjustOrTempRecords.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) adjustOrTempRecordsDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_AdjustOrTempRecords"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		adjustOrTempRecordsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMAdjustOrTempRecords.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMAdjustOrTempRecords adjustOrTempRecords = updateObjs.get(updateIndex);
						Long id = adjustOrTempRecords.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = adjustOrTempRecords.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(adjustOrTempRecords);
							...
							nmeObjs.add(adjustOrTempRecords);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMAdjustOrTempRecords.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMAdjustOrTempRecords adjustOrTempRecords = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = adjustOrTempRecords.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		adjustOrTempRecordsDaoImpl.getSession().flush();
		adjustOrTempRecordsDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = adjustOrTempRecordsDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_AdjustOrTempRecords").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = adjustOrTempRecordsDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_AdjustOrTempRecords").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMAdjustOrTempRecords bizObj = insertObjs.get(insertIndex);
			adjustOrTempRecordsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMAdjustOrTempRecords bizObj = updateObjs.get(updateIndex);
			WOMAdjustOrTempRecords oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			adjustOrTempRecordsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMAdjustOrTempRecords> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMAdjustOrTempRecords> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) adjustOrTempRecordsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMAdjustOrTempRecords> result = getResult(page, query, new PendingResultTransformer(WOMAdjustOrTempRecords.class,adjustOrTempRecordsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMAdjustOrTempRecords model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = adjustOrTempRecordsDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_AdjustOrTempRecords").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = adjustOrTempRecordsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = adjustOrTempRecordsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = adjustOrTempRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?adjustOrTempRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  adjustOrTempRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,adjustOrTempRecordsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="ec.flow.save")
	public void saveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, DataGridService dataGridService){
		saveAdjustOrTempRecords(adjustOrTempRecords, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="ec.flow.save")
	public void saveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, DataGridService dataGridService, String viewCode){
		saveAdjustOrTempRecords(adjustOrTempRecords, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords")
	public void saveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveAdjustOrTempRecords(adjustOrTempRecords, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords")
	public void saveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != adjustOrTempRecords.getId() && adjustOrTempRecords.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMAdjustOrTempRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords")
	public Map<Object, Object> adjustOrTempRecordsDataGridImport(WOMAdjustOrTempRecords adjustOrTempRecords, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = adjustOrTempRecords.getVirtualId();
		
		adjustOrTempRecords = adjustOrTempRecordsDao.load(adjustOrTempRecords.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, adjustOrTempRecords.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = adjustOrTempRecords.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(adjustOrTempRecords);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, adjustOrTempRecords.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords")
	public void mergeAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(adjustOrTempRecords);
		beforeSaveAdjustOrTempRecords(adjustOrTempRecords);
		adjustOrTempRecordsDao.merge(adjustOrTempRecords);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_AdjustOrTempRecords",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMAdjustOrTempRecords> getByPage(Page<WOMAdjustOrTempRecords> page,DetachedCriteria detachedCriteria){
		return adjustOrTempRecordsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, Object... objects){
		checkUniqueConstraint(adjustOrTempRecords);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreAdjustOrTempRecords(WOMAdjustOrTempRecords adjustOrTempRecords){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMAdjustOrTempRecords adjustOrTempRecords, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMAdjustOrTempRecords adjustOrTempRecords, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportAdjustOrTempRecords(List<WOMAdjustOrTempRecords> insertObjs, List<WOMAdjustOrTempRecords> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportAdjustOrTempRecords,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportAdjustOrTempRecords(List<WOMAdjustOrTempRecords> insertObjs, List<WOMAdjustOrTempRecords> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportAdjustOrTempRecords,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_AdjustOrTempRecords,WOM_7.5.0.0) */
// 自定义代码
