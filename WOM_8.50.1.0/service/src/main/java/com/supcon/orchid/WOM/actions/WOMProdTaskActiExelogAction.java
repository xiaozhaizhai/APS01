...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_ProdTaskActiExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMProdTaskActiExelogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProdTaskActiExelog> {
	...
	private WOMProdTaskActiExelog prodTaskActiExelog;
	...
	//导出excel时,主模型内容
	List<WOMProdTaskActiExelog> queryResults = new ArrayList<WOMProdTaskActiExelog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/prodTaskActiExelog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/prodTaskActiExelog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_ProdTaskActiExelog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProdTaskActiExelog> prodTaskActiExelogs;
	
	public List<WOMProdTaskActiExelog> getProdTaskActiExelogs() {
		return prodTaskActiExelogs;
	}
	...
	private Page<WOMProdTaskActiExelog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(prodTaskActiExelog.getActiveSource() != null) {
				prodTaskActiExelog.setActiveSource((SystemCode) systemCodeService.getSystemCode(prodTaskActiExelog.getActiveSource().getId()));
			}
			if(prodTaskActiExelog.getActiveType() != null) {
				prodTaskActiExelog.setActiveType((SystemCode) systemCodeService.getSystemCode(prodTaskActiExelog.getActiveType().getId()));
			}
			if(prodTaskActiExelog.getDataSource() != null) {
				prodTaskActiExelog.setDataSource((SystemCode) systemCodeService.getSystemCode(prodTaskActiExelog.getDataSource().getId()));
			}
			if(prodTaskActiExelog.getDataType() != null) {
				prodTaskActiExelog.setDataType((SystemCode) systemCodeService.getSystemCode(prodTaskActiExelog.getDataType().getId()));
			}
			if(prodTaskActiExelog.getState() != null) {
				prodTaskActiExelog.setState((SystemCode) systemCodeService.getSystemCode(prodTaskActiExelog.getState().getId()));
			}
		} else {
			if(id > 0){
				prodTaskActiExelog = prodTaskActiExelogService.getProdTaskActiExelog(id, viewCode);
			}
		}
	...
	public Page<WOMProdTaskActiExelog> getPage() {
		if (null == page) {
			page = new Page<WOMProdTaskActiExelog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProdTaskActiExelog getProdTaskActiExelog() {
		return prodTaskActiExelog;
	}		

	public void setProdTaskActiExelog(WOMProdTaskActiExelog prodTaskActiExelog) {
		this.prodTaskActiExelog = prodTaskActiExelog;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prodTaskActiExelogService.getProdTaskActiExelogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1495778243721
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String prodTaskActiExelogReferenceCopy() throws Exception{
			WOMProdTaskActiExelog oldProdTaskActiExelog = null;
			if(isEditNew()){
				...
				oldProdTaskActiExelog = new WOMProdTaskActiExelog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodTaskActiExelog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prodTaskActiExelog.getId());
				...
				oldProdTaskActiExelog = prodTaskActiExelogService.getProdTaskActiExelog(id);
			}
			
			
			prodTaskActiExelog = prodTaskActiExelogService.getProdTaskActiExelog(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProdTaskActiExelog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProdTaskActiExelog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProdTaskActiExelog, null);
						Method setValue = WOMProdTaskActiExelog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prodTaskActiExelog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prodTaskActiExelog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=prodTaskActiExelogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return prodTaskActiExelogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			prodTaskActiExelog = prodTaskActiExelogService.getProdTaskActiExelog(this.prodTaskActiExelog.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodTaskActiExelog.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			prodTaskActiExelogService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/prodTaskActiExelog/activeExeLogList", results = { @Result(type = FREEMARKER, location = "activeExeLogList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = prodTaskActiExelogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(prodTaskActiExelogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(prodTaskActiExelogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/prodTaskActiExelog/activeExeLogList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.taskID.tableNo,result.batchNum,result.taskProcessID.name,result.activeName,result.activeType.id,result.activeType.value,result.isPassCheck,result.finalInspection,result.dataSource.id,result.dataSource.value,result.activeSource.id,result.activeSource.value,result.productID.productCode,result.productID.productName,result.productNum,result.planNum,result.actualNum,result.checkResult,result.checkResultID,result.rejectsDealID,result.startTime,result.endTime,result.isFinish,result.formularActice.isAnaly,result.isAnaly,result.remark,result.activeBatchStateID.name,result.taskID.id,result.taskProcessID.id,result.productID.id,result.formularActice.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"prodTaskActiExelog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"prodTaskActiExelog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"prodTaskActiExelog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"prodTaskActiExelog\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			prodTaskActiExelogService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/prodTaskActiExelog/activeExeLogList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		prodTaskActiExelogService.findDataGridPage(dg,dgclass,getDgPage(),prodTaskActiExelog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(prodTaskActiExelog.getId() != null && prodTaskActiExelog.getId() != -1){
				prodTaskActiExelog = prodTaskActiExelogService.getProdTaskActiExelog(prodTaskActiExelog.getId());
			}
			prodTaskActiExelogService.findDataGridPage(dg,dgclass,getDgPage(),prodTaskActiExelog,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == prodTaskActiExelog.getId()) {
		...
		prodTaskActiExelogService.saveProdTaskActiExelog(prodTaskActiExelog,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "submit");
		bap_validate_datagrids=prodTaskActiExelogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", prodTaskActiExelog.getId());
		...
		fileuploads.put("linkId", prodTaskActiExelog.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/prodTaskActiExelog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prodTaskActiExelogService.deleteProdTaskActiExelog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "restore");
		prodTaskActiExelogService.restoreProdTaskActiExelog(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProdTaskActiExelog> findExportDataInfos(Page<WOMProdTaskActiExelog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = prodTaskActiExelogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = prodTaskActiExelogService.getAssProperty(p.getCode());	
									//String assModelCode = prodTaskActiExelogService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			prodTaskActiExelogService.findProdTaskActiExelogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			prodTaskActiExelogService.findProdTaskActiExelogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProdTaskActiExelog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProdTaskActiExelog result : results){
	...
	public boolean isEditNew(){
		//return null == this.prodTaskActiExelog || null == this.prodTaskActiExelog.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/prodTaskActiExelog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/prodTaskActiExelog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProdTaskActiExelogService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_ProdTaskActiExelog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_ProdTaskActiExelog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_ProdTaskActiExelog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProdTaskActiExelog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == prodTaskActiExelog.getId() ? "" : prodTaskActiExelog.getId().toString());
		_auditLog.setColumnStringA(null == prodTaskActiExelog.getId() ? "" : prodTaskActiExelog.getId().toString());
		_auditLog.setColumnStringC(prodTaskActiExelog.getId().toString());
		_auditLog.setColumnStringD(WOMProdTaskActiExelog.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProdTaskActiExelogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, prodTaskActiExelog,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, prodTaskActiExelog,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,prodTaskActiExelog);
		...
		printlog.setBusinessId(prodTaskActiExelog.getId());
		printlog.setBussinessKey(null == prodTaskActiExelog.getId() ? "" : prodTaskActiExelog.getId().toString());
		printlog.setMainDisplay(null == prodTaskActiExelog.getId() ? "" : prodTaskActiExelog.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(prodTaskActiExelog,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(prodTaskActiExelogService.getProdTaskActiExelog(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, prodTaskActiExelog,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				prodTaskActiExelog = prodTaskActiExelogService.getProdTaskActiExelog(Long.parseLong(arrId[i]));
				if(null != prodTaskActiExelog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == prodTaskActiExelog.getId() ? "" : "," + prodTaskActiExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == prodTaskActiExelog.getId() ? "" : prodTaskActiExelog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == prodTaskActiExelog.getId() ? "" : "," + prodTaskActiExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == prodTaskActiExelog.getId() ? "" : prodTaskActiExelog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == prodTaskActiExelog.getId() ? "" : "," + prodTaskActiExelog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == prodTaskActiExelog.getId() ? "" : prodTaskActiExelog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, prodTaskActiExelog,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,prodTaskActiExelog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//prodTaskActiExelogService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/prodTaskActiExelog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProdTaskActiExelog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_ProdTaskActiExelog")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_ProdTaskActiExelog");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProdTaskActiExelog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = prodTaskActiExelogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_ProdTaskActiExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_ProdTaskActiExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_ProdTaskActiExelog,WOM_7.5.0.0) */
// 自定义代码
