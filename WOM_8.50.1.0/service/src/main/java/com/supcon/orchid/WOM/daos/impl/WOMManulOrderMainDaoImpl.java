...
@Repository
public class WOMManulOrderMainDaoImpl extends ExtendedGenericDaoImpl<WOMManulOrderMain, Long> implements WOMManulOrderMainDao {
	@Override
	...
	@Override
	public WOMManulOrderMainDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMManulOrderMainDealInfo dealInfo = new WOMManulOrderMainDealInfo();
	...
	@Override
	public WOMManulOrderMainDealInfo copyAndSaveDealInfo(DealInfo di, WOMManulOrderMain mainObj, Staff dealStaff) {
		WOMManulOrderMainDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMManulOrderMainDealInfo.class);
		...
		List<WOMManulOrderMainDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
