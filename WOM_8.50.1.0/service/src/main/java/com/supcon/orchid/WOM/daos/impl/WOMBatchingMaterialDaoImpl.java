...
@Repository
public class WOMBatchingMaterialDaoImpl extends ExtendedGenericDaoImpl<WOMBatchingMaterial, Long> implements WOMBatchingMaterialDao {
	@Override
	...
	@Override
	public WOMBatchingMaterialDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMBatchingMaterialDealInfo dealInfo = new WOMBatchingMaterialDealInfo();
	...
	@Override
	public WOMBatchingMaterialDealInfo copyAndSaveDealInfo(DealInfo di, WOMBatchingMaterial mainObj, Staff dealStaff) {
		WOMBatchingMaterialDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMBatchingMaterialDealInfo.class);
		...
		List<WOMBatchingMaterialDealInfo> dealinfo = criteria.list();
	...
	public void addSupervision(Supervision supervision) {
		...
		WOMBatchingMaterialSupervision WOMBatchingMaterialSupervision = new WOMBatchingMaterialSupervision();
		BeanUtils.copyProperties(supervision, WOMBatchingMaterialSupervision);
		WOMBatchingMaterialSupervision.setValid(true);
		getSession().save(WOMBatchingMaterialSupervision);
	}

	@Override
	public void addSupervision(Supervision supervision, WOMBatchingMaterial mainObj) {
		getSession().save(supervision);
		WOMBatchingMaterialSupervision localSupervision = new WOMBatchingMaterialSupervision();
	...
	public void deleteSupervision(Long tableInfoId, List<Long> idList) {
		...
		List<WOMBatchingMaterialSupervision> _supervisions = getSession().createCriteria(WOMBatchingMaterialSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.in("staff.id", idList))
				.add(Restrictions.eq("valid", true)).list();
		for (WOMBatchingMaterialSupervision item : _supervisions) {
			item.setValid(false);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMBatchingMaterialSupervision> findSupervision(Long tableInfoId) {
		List<WOMBatchingMaterialSupervision> supervisions = getSession().createCriteria(WOMBatchingMaterialSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("valid", true)).addOrder(Order.asc("id")).list();
		return supervisions;
	}
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMBatchingMaterialSupervision> findSupervision(Long tableInfoId, Long StaffId) {
		List<WOMBatchingMaterialSupervision> supervisions = getSession().createCriteria(WOMBatchingMaterialSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("staff.id", StaffId)).add(Restrictions.eq("valid", true)).list();
		return supervisions;
	}
	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_batchingMaterial_BatchingMaterial,WOM_7.5.0.0) */
// 自定义代码
