...
import com.supcon.orchid.WOM.services.WOMDayBatchPlanPartsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMDayBatchPlanPartsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_dayBatchPlanPartsService")
@Transactional
public class WOMDayBatchPlanPartsServiceImpl  extends BaseServiceImpl<WOMDayBatchPlanParts> implements WOMDayBatchPlanPartsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMDayBatchPlanParts> bapBaseService;
	...
	@Autowired
	private WOMDayBatchPlanPartsImportService dayBatchPlanPartsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMDayBatchPlanParts.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  dayBatchPlanPartsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMDayBatchPlanParts.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = dayBatchPlanPartsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMDayBatchPlanParts.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMDayBatchPlanParts.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  dayBatchPlanPartsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMDayBatchPlanParts.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMDayBatchPlanParts.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMDayBatchPlanParts> list = (List<WOMDayBatchPlanParts>) dayBatchPlanPartsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMDayBatchPlanParts getDayBatchPlanParts(long id){
		return getDayBatchPlanParts(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMDayBatchPlanParts getDayBatchPlanParts(long id, String viewCode){
		WOMDayBatchPlanParts dayBatchPlanParts = dayBatchPlanPartsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getDayBatchPlanPartsAsJSON(long id, String include){
		WOMDayBatchPlanParts dayBatchPlanParts = dayBatchPlanPartsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(dayBatchPlanParts, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.common.delete",operType="3")
	public void deleteDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts){
		if(null != dayBatchPlanParts && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMDayBatchPlanParts.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
			AuditUtil.setColumnStringA(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
			AuditUtil.setColumnStringC(dayBatchPlanParts.getId().toString());
		}
		beforeDeleteDayBatchPlanParts(dayBatchPlanParts);
		dayBatchPlanPartsDao.deletePhysical(dayBatchPlanParts);
		afterDeleteDayBatchPlanParts(dayBatchPlanParts);
		dayBatchPlanPartsDao.flush();
		List<WOMDayBatchPlanParts> params = new ArrayList<WOMDayBatchPlanParts>();
		params.add(dayBatchPlanParts);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.common.delete",operType="3")
	public void deleteDayBatchPlanParts(List<Long> dayBatchPlanPartsIds){
		deleteDayBatchPlanParts(dayBatchPlanPartsIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.common.delete",operType="3")
	public void deleteDayBatchPlanParts(List<Long> dayBatchPlanPartsIds, String eventTopic) {
		List<WOMDayBatchPlanParts> dayBatchPlanPartss = new ArrayList<WOMDayBatchPlanParts>();
		for(Long dayBatchPlanPartsId : dayBatchPlanPartsIds){
			WOMDayBatchPlanParts dayBatchPlanParts = getDayBatchPlanParts(dayBatchPlanPartsId);
			dayBatchPlanPartss.add(dayBatchPlanParts);
			if(null != dayBatchPlanParts && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMDayBatchPlanParts.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == dayBatchPlanParts.getId() ? "" : "," + dayBatchPlanParts.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == dayBatchPlanParts.getId() ? "" : "," + dayBatchPlanParts.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == dayBatchPlanParts.getId() ? "" : "," + dayBatchPlanParts.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(dayBatchPlanPartss != null && dayBatchPlanPartss.size() > 0 ){
			for(WOMDayBatchPlanParts dayBatchPlanParts : dayBatchPlanPartss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMDayBatchPlanParts.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(dayBatchPlanParts));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),dayBatchPlanParts, dayBatchPlanParts.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(dayBatchPlanPartsIds != null && dayBatchPlanPartsIds.size() > 0) {
			String hql = "delete " + WOMDayBatchPlanParts.JPA_NAME + " where id in(:ids)";
			Query query = dayBatchPlanPartsDao.createQuery(hql);
			query.setParameterList("ids", dayBatchPlanPartsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMDayBatchPlanParts dayBatchPlanPartsz : dayBatchPlanPartss){
				dayBatchPlanPartsz.setValid(false);
				List<WOMDayBatchPlanParts> params = new ArrayList<WOMDayBatchPlanParts>();
				params.add(dayBatchPlanPartsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_producePlanRef");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.common.delete",operType="3")
	public void deleteDayBatchPlanParts(long dayBatchPlanPartsId, int dayBatchPlanPartsVersion){
		this.deleteDayBatchPlanParts(dayBatchPlanPartsId, dayBatchPlanPartsVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.common.delete",operType="3")
	public void deleteDayBatchPlanParts(long dayBatchPlanPartsId, int dayBatchPlanPartsVersion,SignatureLog signatureLog){
		WOMDayBatchPlanParts dayBatchPlanParts = getDayBatchPlanParts(dayBatchPlanPartsId);
		...
		beforeDeleteDayBatchPlanParts(dayBatchPlanParts);
		dayBatchPlanPartsDao.delete(dayBatchPlanPartsId, dayBatchPlanPartsVersion);
		afterDeleteDayBatchPlanParts(dayBatchPlanParts);
		dayBatchPlanPartsDao.flush();
		List<WOMDayBatchPlanParts> params = new ArrayList<WOMDayBatchPlanParts>();
		params.add(dayBatchPlanParts);
		...
		if(signatureLog != null ) {
			...
			businessKey=dayBatchPlanParts.getId();
			signatureLog.setTableId(dayBatchPlanParts.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.common.delete",operType="3")
	public void deleteDayBatchPlanParts(String dayBatchPlanPartsIds){
		this.deleteDayBatchPlanParts(dayBatchPlanPartsIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.common.delete",operType="3")
	public void deleteDayBatchPlanParts(String dayBatchPlanPartsIds,SignatureLog signatureLog){
		deleteCollection(dayBatchPlanPartsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteDayBatchPlanParts(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_producePlanRef#modelCode:WOM_7.5.0.0_producePlanRef_DayBatchPlanParts#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="ec.engine.restore",operType="10")
	public void restoreDayBatchPlanParts(String dayBatchPlanPartsIds){
		restoreCollection(dayBatchPlanPartsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreDayBatchPlanParts(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="ec.engine.restore",operType="10")
	public void restoreDayBatchPlanParts(long dayBatchPlanPartsId){
		
		findBusinessKeyUsed(dayBatchPlanPartsId);	//判断业务主键是否重复
		...
		beforeRestoreDayBatchPlanParts(dayBatchPlanParts);
		dayBatchPlanParts.setValid(true);
		dayBatchPlanPartsDao.update(dayBatchPlanParts);
		afterRestoreDayBatchPlanParts(dayBatchPlanParts);
		List<WOMDayBatchPlanParts> params = new ArrayList<WOMDayBatchPlanParts>();
		params.add(dayBatchPlanParts);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long dayBatchPlanPartsId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMDayBatchPlanParts.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMDayBatchPlanParts.TABLE_NAME +" where id = ? )";
			List<Object> list =  dayBatchPlanPartsDao.createNativeQuery(sql,dayBatchPlanPartsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseDayBatchPlanParts(List<WOMDayBatchPlanParts>  dayBatchPlanPartss){
		for(WOMDayBatchPlanParts dayBatchPlanParts:dayBatchPlanPartss)  {
			saveDayBatchPlanParts(dayBatchPlanParts, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseDayBatchPlanParts(List<WOMDayBatchPlanParts>  dayBatchPlanPartss){
		for(WOMDayBatchPlanParts dayBatchPlanParts:dayBatchPlanPartss)  {
			saveDayBatchPlanParts(dayBatchPlanParts, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="ec.flow.save")
	public void saveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveDayBatchPlanParts(dayBatchPlanParts, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="ec.flow.save")
	public void saveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveDayBatchPlanParts(dayBatchPlanParts, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateDayBatchPlanPartsCodes(WOMDayBatchPlanParts dayBatchPlanParts) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateDayBatchPlanPartsCodes(dayBatchPlanParts, false);
	}
	
	public void generateDayBatchPlanPartsCodes(WOMDayBatchPlanParts dayBatchPlanParts, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateDayBatchPlanPartsSummarys(WOMDayBatchPlanParts dayBatchPlanParts) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateDayBatchPlanPartsSummarys(dayBatchPlanParts, false);
	}

	public void generateDayBatchPlanPartsSummarys(WOMDayBatchPlanParts dayBatchPlanParts, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts")
	public void saveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveDayBatchPlanParts(dayBatchPlanParts, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts")
	public void saveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_producePlanRef");
		String url = null;
		if(dayBatchPlanParts.getId() != null && dayBatchPlanParts.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMDayBatchPlanParts.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, dayBatchPlanParts);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(dayBatchPlanParts);
		beforeSaveDayBatchPlanParts(dayBatchPlanParts, viewIsView);

		if (viewIsView) {
			dayBatchPlanPartsDao.saveWithRevertVersion(dayBatchPlanParts);
		} else {
			if(isNew)
				dayBatchPlanPartsDao.save(dayBatchPlanParts);
			else
				dayBatchPlanPartsDao.merge(dayBatchPlanParts);
		}
		if(null != dayBatchPlanParts && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMDayBatchPlanParts.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
			AuditUtil.setColumnStringA(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
		...
		dealDatagridsSave(dayBatchPlanParts,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateDayBatchPlanPartsCodes(dayBatchPlanParts, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateDayBatchPlanPartsSummarys(dayBatchPlanParts, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveDayBatchPlanParts(dayBatchPlanParts, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, dayBatchPlanParts);
			}
		}
		List<WOMDayBatchPlanParts> params = new ArrayList<WOMDayBatchPlanParts>();
		dayBatchPlanPartsDao.flush();
		dayBatchPlanPartsDao.clear();
		dayBatchPlanParts = dayBatchPlanPartsDao.load(dayBatchPlanParts.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMDayBatchPlanParts dayBatchPlanParts){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("dayBatchPlanParts", dayBatchPlanParts);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts")
	public void mergeDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(dayBatchPlanParts);
		beforeSaveDayBatchPlanParts(dayBatchPlanParts);
		dayBatchPlanPartsDao.merge(dayBatchPlanParts);
		if(null != dayBatchPlanParts && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMDayBatchPlanParts.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
			AuditUtil.setColumnStringA(null == dayBatchPlanParts.getId() ? "" : dayBatchPlanParts.getId().toString());
		...
		dealDatagridsSave(dayBatchPlanParts,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveDayBatchPlanParts(dayBatchPlanParts);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMDayBatchPlanParts> findDayBatchPlanPartss(Page<WOMDayBatchPlanParts> page, Criterion... criterions) {
		return dayBatchPlanPartsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDayBatchPlanPartss(Page<WOMDayBatchPlanParts> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findDayBatchPlanPartss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDayBatchPlanPartss(Page<WOMDayBatchPlanParts> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDayBatchPlanPartss(Page<WOMDayBatchPlanParts> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMDayBatchPlanParts> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"dayBatchPlanParts\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"dayBatchPlanParts\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"dayBatchPlanParts\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"dayBatchPlanParts\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM PPM_DAY_PLAN_INFOS \"dayBatchPlanInfo\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_producePlanRef" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  dayBatchPlanPartsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  dayBatchPlanPartsDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_producePlanRef").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = dataPermissionService.getPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"dayBatchPlanInfo\"","WOM_7.5.0.0_producePlanRef","","PPM_DAY_PLAN_INFOS",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"dayBatchPlanParts\".LAY_REC ASC, \"dayBatchPlanParts\".SORT ASC");
				} else {
					orderPart.append(" \"dayBatchPlanParts\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) dayBatchPlanPartsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=dayBatchPlanPartsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = dayBatchPlanPartsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMDayBatchPlanParts> result = new ArrayList<WOMDayBatchPlanParts>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMDayBatchPlanParts.class,dayBatchPlanPartsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMDayBatchPlanParts.class,dayBatchPlanPartsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMDayBatchPlanParts dayBatchPlanParts : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(dayBatchPlanParts.getId(), "WOM_producePlanRef_dayBatchPlanParts");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(dayBatchPlanParts.getId(), "WOM_producePlanRef_dayBatchPlanParts");
							if(null != documents && !documents.isEmpty()) {
								dayBatchPlanParts.setDocument(documents.get(0));
								dayBatchPlanParts.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMDayBatchPlanParts> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMDayBatchPlanParts> result) {
		View view = null;
		...
		Map<Long, WOMDayBatchPlanParts> rsMap = new HashMap<Long, WOMDayBatchPlanParts>();
		for (WOMDayBatchPlanParts dayBatchPlanParts : result) {
			ids.add(dayBatchPlanParts.getId());
			rsMap.put(dayBatchPlanParts.getId(), dayBatchPlanParts);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(dayBatchPlanPartsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = dayBatchPlanPartsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_producePlanRef_DayBatchPlanParts" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  dayBatchPlanPartsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"dayBatchPlanParts\"","WOM_7.5.0.0_producePlanRef","","PPM_DAY_PLAN_PARS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"dayBatchPlanParts\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"dayBatchPlanParts\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"dayBatchPlanParts\"","WOM_7.5.0.0_producePlanRef","","PPM_DAY_PLAN_PARS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_producePlanRef_DayBatchPlanParts" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("dayBatchPlanParts.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"dayBatchPlanParts\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"dayBatchPlanParts\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"dayBatchPlanParts\".CID = 1 OR \"dayBatchPlanParts\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("dayBatchPlanParts.mc\"").append(".DAY_BATCH_PLAN_PARTS) not in ("+notinValue+")");
			}
			
			SQLQuery query = dayBatchPlanPartsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=dayBatchPlanPartsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMDayBatchPlanParts.ENTITY_CODE;}
	protected String getModuleCode(){return WOMDayBatchPlanParts.MODULE_CODE;}
	protected String getModelCode(){return WOMDayBatchPlanParts.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param dayBatchPlanParts 模型实例
	 */
	public void checkUniqueConstraint(WOMDayBatchPlanParts dayBatchPlanParts){
	}
	
	
	@Override
	public List<WOMDayBatchPlanParts> getDayBatchPlanPartss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMDayBatchPlanParts.class);
		if(params!=null){
			...
			return dayBatchPlanPartsDao.findByHql("from " + WOMDayBatchPlanParts.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return dayBatchPlanPartsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMDayBatchPlanParts> findDayBatchPlanPartssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMDayBatchPlanParts.class);
	...
	@Override
	public List<WOMDayBatchPlanParts> findDayBatchPlanPartssByHql(String hql, Object... objects){
		
		return   dayBatchPlanPartsDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMDayBatchPlanParts loadDayBatchPlanPartsByBussinessKey(WOMDayBatchPlanParts dayBatchPlanParts){
		return loadDayBatchPlanPartsByBussinessKey(dayBatchPlanParts.getId());
	}
	
	@Override
	public Page<WOMDayBatchPlanParts> getDayBatchPlanPartss(Page<WOMDayBatchPlanParts> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMDayBatchPlanParts.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = dayBatchPlanPartsDao.findByPage(page, sql);
			}else{
				page = dayBatchPlanPartsDao.findAllByPage(page);
			}
			return page;
		}
		page = dayBatchPlanPartsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMDayBatchPlanParts loadDayBatchPlanPartsByBussinessKey(Serializable bussinessKey){
		return dayBatchPlanPartsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMDayBatchPlanParts> findByProperty(String propertyName, Object object){
		return dayBatchPlanPartsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMDayBatchPlanParts findEntityByProperty(String propertyName, Object object){
		return dayBatchPlanPartsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMDayBatchPlanParts.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = dayBatchPlanPartsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_producePlanRef_DayBatchPlanParts","runtime");
		try {
	...
	public void dealDatagridsSave(WOMDayBatchPlanParts dayBatchPlanParts,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,dayBatchPlanPartsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMDayBatchPlanParts.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, dayBatchPlanParts);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(dayBatchPlanParts.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(dayBatchPlanParts.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(dayBatchPlanParts.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",dayBatchPlanParts.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMDayBatchPlanParts.TABLE_NAME +" where valid = 1";
			list = dayBatchPlanPartsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMDayBatchPlanParts.TABLE_NAME;
			list = dayBatchPlanPartsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMDayBatchPlanParts.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = dayBatchPlanPartsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMDayBatchPlanPartsDaoImpl dayBatchPlanPartsDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMDayBatchPlanParts.TABLE_NAME + " where id = ?";
		String tableNo = dayBatchPlanPartsDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchDayBatchPlanParts(final List<WOMDayBatchPlanParts> insertObjs, final List<WOMDayBatchPlanParts> updateObjs, 
		...
		return this.importBatchDayBatchPlanParts(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchDayBatchPlanParts(final List<WOMDayBatchPlanParts> insertObjs, final List<WOMDayBatchPlanParts> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1489822962143"),importCount));
		...
		Map<Long, WOMDayBatchPlanParts> oldUpdateObjsMap = new HashMap<Long, WOMDayBatchPlanParts>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMDayBatchPlanParts bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMDayBatchPlanParts> bizObjList = dayBatchPlanPartsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMDayBatchPlanParts bizObj = bizObjList.get(mIndex);
		...
		final List<WOMDayBatchPlanParts> nmeObjs = new ArrayList<WOMDayBatchPlanParts>();
		...
		//String idSql = "select id from " + WOMDayBatchPlanParts.TABLE_NAME + " order by id desc";
		//Query query = dayBatchPlanPartsDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMDayBatchPlanParts.JPA_NAME + " order by id desc";
		Query query =  dayBatchPlanPartsDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMDayBatchPlanParts.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMDayBatchPlanParts dayBatchPlanParts:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMDayBatchPlanParts.TABLE_NAME,1,dbId);
			if(dayBatchPlanParts.getId() != null ){
				m.put(dayBatchPlanParts.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) dayBatchPlanPartsDao.createNativeQuery(sql, "WOM_7.5.0.0_producePlanRef_DayBatchPlanParts"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		dayBatchPlanPartsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMDayBatchPlanParts.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMDayBatchPlanParts dayBatchPlanParts = updateObjs.get(updateIndex);
						Long id = dayBatchPlanParts.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = dayBatchPlanParts.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(dayBatchPlanParts);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMDayBatchPlanParts.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMDayBatchPlanParts dayBatchPlanParts = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = dayBatchPlanParts.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		dayBatchPlanPartsDaoImpl.getSession().flush();
		dayBatchPlanPartsDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = dayBatchPlanPartsDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_producePlanRef_DayBatchPlanParts").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = dayBatchPlanPartsDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_producePlanRef_DayBatchPlanParts").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMDayBatchPlanParts bizObj = insertObjs.get(insertIndex);
			dayBatchPlanPartsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMDayBatchPlanParts bizObj = updateObjs.get(updateIndex);
			WOMDayBatchPlanParts oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			dayBatchPlanPartsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMDayBatchPlanParts> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMDayBatchPlanParts> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(linkMap != null && linkMap.get("needChange")!=null && linkMap.get("needChange").equals("no!!")){
				...
				s.append(" FROM PPM_DAY_PLAN_INFOS \"dayBatchPlanInfo\" ");
			}else{
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) dayBatchPlanPartsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMDayBatchPlanParts> result = getResult(page, query, new PendingResultTransformer(WOMDayBatchPlanParts.class,dayBatchPlanPartsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMDayBatchPlanParts model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = dayBatchPlanPartsDao.createNativeQuery(sql, "WOM_7.5.0.0_producePlanRef_DayBatchPlanParts").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = dayBatchPlanPartsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = dayBatchPlanPartsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = dayBatchPlanPartsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?dayBatchPlanPartsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  dayBatchPlanPartsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,dayBatchPlanPartsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="ec.flow.save")
	public void saveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, DataGridService dataGridService){
		saveDayBatchPlanParts(dayBatchPlanParts, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="ec.flow.save")
	public void saveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, DataGridService dataGridService, String viewCode){
		saveDayBatchPlanParts(dayBatchPlanParts, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts")
	public void saveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveDayBatchPlanParts(dayBatchPlanParts, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts")
	public void saveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_producePlanRef";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_producePlanRef");
		String url = null;
		if(null != dayBatchPlanParts.getId() && dayBatchPlanParts.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMDayBatchPlanParts.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts")
	public Map<Object, Object> dayBatchPlanPartsDataGridImport(WOMDayBatchPlanParts dayBatchPlanParts, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = dayBatchPlanParts.getVirtualId();
		
		dayBatchPlanParts = dayBatchPlanPartsDao.load(dayBatchPlanParts.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, dayBatchPlanParts.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = dayBatchPlanParts.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(dayBatchPlanParts);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, dayBatchPlanParts.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts")
	public void mergeDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(dayBatchPlanParts);
		beforeSaveDayBatchPlanParts(dayBatchPlanParts);
		dayBatchPlanPartsDao.merge(dayBatchPlanParts);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_producePlanRef",model="WOM_7.5.0.0_producePlanRef_DayBatchPlanParts",desc="foundation.batch.save",operType="4")
	public void batchSaveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_producePlanRef";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_producePlanRef");
	...
	@Override
	public Page<WOMDayBatchPlanParts> getByPage(Page<WOMDayBatchPlanParts> page,DetachedCriteria detachedCriteria){
		return dayBatchPlanPartsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, Object... objects){
		checkUniqueConstraint(dayBatchPlanParts);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportDayBatchPlanParts(List<WOMDayBatchPlanParts> dayBatchPlanPartss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportDayBatchPlanParts(List<WOMDayBatchPlanParts> dayBatchPlanPartss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportDayBatchPlanParts(List<WOMDayBatchPlanParts> dayBatchPlanPartss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportDayBatchPlanParts(List<WOMDayBatchPlanParts> dayBatchPlanPartss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreDayBatchPlanParts(WOMDayBatchPlanParts dayBatchPlanParts){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMDayBatchPlanParts dayBatchPlanParts, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMDayBatchPlanParts dayBatchPlanParts, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportDayBatchPlanParts(List<WOMDayBatchPlanParts> insertObjs, List<WOMDayBatchPlanParts> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportDayBatchPlanParts,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportDayBatchPlanParts(List<WOMDayBatchPlanParts> insertObjs, List<WOMDayBatchPlanParts> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportDayBatchPlanParts,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_producePlanRef_DayBatchPlanParts,WOM_7.5.0.0) */
// 自定义代码
