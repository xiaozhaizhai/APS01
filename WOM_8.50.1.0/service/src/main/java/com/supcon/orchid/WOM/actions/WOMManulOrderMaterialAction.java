...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMManulOrderMaterialAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMManulOrderMaterial> {
	...
	private WOMManulOrderMaterial manulOrderMaterial;
	...
	//导出excel时,主模型内容
	List<WOMManulOrderMaterial> queryResults = new ArrayList<WOMManulOrderMaterial>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/manulOrder/manulOrderMaterial/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/manulOrder/manulOrderMaterial/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_manulOrder_ManulOrderMaterial" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMManulOrderMaterial> manulOrderMaterials;
	
	public List<WOMManulOrderMaterial> getManulOrderMaterials() {
		return manulOrderMaterials;
	}
	...
	private Page<WOMManulOrderMaterial> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			manulOrderMaterial = new WOMManulOrderMaterial();
		} else {
			if(id > 0){
				manulOrderMaterial = manulOrderMaterialService.getManulOrderMaterial(id, viewCode);
			}
		}
	...
	public Page<WOMManulOrderMaterial> getPage() {
		if (null == page) {
			page = new Page<WOMManulOrderMaterial>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMManulOrderMaterial getManulOrderMaterial() {
		return manulOrderMaterial;
	}		

	public void setManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = manulOrderMaterialService.getManulOrderMaterialAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1563193064716
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String manulOrderMaterialReferenceCopy() throws Exception{
			WOMManulOrderMaterial oldManulOrderMaterial = null;
			if(isEditNew()){
				...
				oldManulOrderMaterial = new WOMManulOrderMaterial();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,manulOrderMaterial.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, manulOrderMaterial.getId());
				...
				oldManulOrderMaterial = manulOrderMaterialService.getManulOrderMaterial(id);
			}
			
			
			manulOrderMaterial = manulOrderMaterialService.getManulOrderMaterial(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_manulOrder_ManulOrderMaterial");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMManulOrderMaterial.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldManulOrderMaterial, null);
						Method setValue = WOMManulOrderMaterial.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(manulOrderMaterial, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			manulOrderMaterial.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=manulOrderMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return manulOrderMaterialReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			manulOrderMaterial = manulOrderMaterialService.getManulOrderMaterial(this.manulOrderMaterial.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,manulOrderMaterial.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			manulOrderMaterialService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = manulOrderMaterialService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(manulOrderMaterialService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(manulOrderMaterialService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMaterial/orderMaterialRef", results = { @Result(type = FREEMARKER, location = "orderMaterialRef.ftl") })
		,
		@Action(value = "/WOM/manulOrder/manulOrderMaterial/papreOrderRef", results = { @Result(type = FREEMARKER, location = "papreOrderRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"manulOrderMaterial\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"manulOrderMaterial\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"manulOrderMaterial\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"manulOrderMaterial\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			manulOrderMaterialService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMaterial/orderMaterialRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.manulOrderId.manulOrderNum,result.materialBatchNum,result.material.productCode,result.unit.unitGroup.name,result.needNumber,result.ware.wareCode,result.planStartTime,result.planEndTime,result.rsNUM,result.rsPOS,result.saleOrderNum,result.saleOrderProject,result.manulOrderId.id,result.material.id,result.unit.unitGroup.id,result.ware.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="orderMaterialRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/manulOrder/manulOrderMaterial/papreOrderRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.material.productCode,result.material.productName,result.material.unit,result.ware.wareCode,result.materialBatchNum,result.material.id,result.ware.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="papreOrderRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		manulOrderMaterialService.findDataGridPage(dg,dgclass,getDgPage(),manulOrderMaterial,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(manulOrderMaterial.getId() != null && manulOrderMaterial.getId() != -1){
				manulOrderMaterial = manulOrderMaterialService.getManulOrderMaterial(manulOrderMaterial.getId());
			}
			manulOrderMaterialService.findDataGridPage(dg,dgclass,getDgPage(),manulOrderMaterial,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == manulOrderMaterial.getId()) {
		...
		manulOrderMaterialService.saveManulOrderMaterial(manulOrderMaterial,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "submit");
		bap_validate_datagrids=manulOrderMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", manulOrderMaterial.getId());
		...
		fileuploads.put("linkId", manulOrderMaterial.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/manulOrder/manulOrderMaterial/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		manulOrderMaterialService.deleteManulOrderMaterial(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "restore");
		manulOrderMaterialService.restoreManulOrderMaterial(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMManulOrderMaterial> findExportDataInfos(Page<WOMManulOrderMaterial> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = manulOrderMaterialService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = manulOrderMaterialService.getAssProperty(p.getCode());	
									//String assModelCode = manulOrderMaterialService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			manulOrderMaterialService.findManulOrderMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			manulOrderMaterialService.findManulOrderMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMManulOrderMaterial> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMManulOrderMaterial result : results){
	...
	public boolean isEditNew(){
		//return null == this.manulOrderMaterial || null == this.manulOrderMaterial.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/manulOrder/manulOrderMaterial/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/manulOrder/manulOrderMaterial/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMManulOrderMaterialService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_manulOrder_ManulOrderMaterial", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_manulOrder_ManulOrderMaterial", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_manulOrder_ManulOrderMaterial";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMaterial", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMManulOrderMaterial manulOrderMaterial = null;
		if(null != id) {
			manulOrderMaterial = manulOrderMaterialService.getManulOrderMaterial(id);
		}
		...
		_auditLog.setColumnStringB(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
		_auditLog.setColumnStringA(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
		_auditLog.setColumnStringC(manulOrderMaterial.getId().toString());
		_auditLog.setColumnStringD(WOMManulOrderMaterial.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMManulOrderMaterialAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, manulOrderMaterial,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, manulOrderMaterial,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,manulOrderMaterial);
		...
		printlog.setBusinessId(manulOrderMaterial.getId());
		printlog.setBussinessKey(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
		printlog.setMainDisplay(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(manulOrderMaterial,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(manulOrderMaterialService.getManulOrderMaterial(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, manulOrderMaterial,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				manulOrderMaterial = manulOrderMaterialService.getManulOrderMaterial(Long.parseLong(arrId[i]));
				if(null != manulOrderMaterial){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == manulOrderMaterial.getId() ? "" : "," + manulOrderMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == manulOrderMaterial.getId() ? "" : "," + manulOrderMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == manulOrderMaterial.getId() ? "" : "," + manulOrderMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, manulOrderMaterial,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,manulOrderMaterial);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//manulOrderMaterialService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/manulOrder/manulOrderMaterial/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_manulOrder_ManulOrderMaterial");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_manulOrder_ManulOrderMaterial")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_manulOrder_ManulOrderMaterial");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_manulOrder_ManulOrderMaterial");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = manulOrderMaterialService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
