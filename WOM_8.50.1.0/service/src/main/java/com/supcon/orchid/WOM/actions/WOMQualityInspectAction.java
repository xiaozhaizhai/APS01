...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMQualityInspectAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMQualityInspect> {
	...
	private WOMQualityInspect qualityInspect;
	...
	//导出excel时,主模型内容
	List<WOMQualityInspect> queryResults = new ArrayList<WOMQualityInspect>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/qualityRecords/qualityInspect/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/qualityRecords/qualityInspect/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_qualityRecords_QualityInspect" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMQualityInspect> qualityInspects;
	
	public List<WOMQualityInspect> getQualityInspects() {
	...
	private Page<WOMQualityInspect> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			qualityInspect = new WOMQualityInspect();
			if(qualityInspect.getInspectType() != null) {
				qualityInspect.setInspectType((SystemCode) systemCodeService.getSystemCode(qualityInspect.getInspectType().getId()));
			}
		} else {
			if(id > 0){
				qualityInspect = qualityInspectService.getQualityInspect(id, viewCode);
			}
		}
	...
	public Page<WOMQualityInspect> getPage() {
		if (null == page) {
			page = new Page<WOMQualityInspect>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMQualityInspect getQualityInspect() {
		return qualityInspect;
	}		

	public void setQualityInspect(WOMQualityInspect qualityInspect) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/qualityRecords/qualityInspect/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = qualityInspectService.getQualityInspectAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/qualityRecords/qualityInspect/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1541146800376
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String qualityInspectReferenceCopy() throws Exception{
			WOMQualityInspect oldQualityInspect = null;
			if(isEditNew()){
				...
				oldQualityInspect = new WOMQualityInspect();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,qualityInspect.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, qualityInspect.getId());
				...
				oldQualityInspect = qualityInspectService.getQualityInspect(id);
			}
			
			
			qualityInspect = qualityInspectService.getQualityInspect(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_qualityRecords_QualityInspect");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMQualityInspect.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldQualityInspect, null);
						Method setValue = WOMQualityInspect.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(qualityInspect, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			qualityInspect.setCreateDepartmentId(oldQualityInspect.getCreateDepartmentId());
			qualityInspect.setCreatePositionId(oldQualityInspect.getCreatePositionId());
			qualityInspect.setCreateStaffId(oldQualityInspect.getCreateStaffId());
			qualityInspect.setOwnerDepartmentId(oldQualityInspect.getOwnerDepartmentId());
			qualityInspect.setOwnerPositionId(oldQualityInspect.getOwnerPositionId());
			qualityInspect.setOwnerStaffId(oldQualityInspect.getOwnerStaffId());
			qualityInspect.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "qualityInspectEdit.ftl") }),
		@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "qualityInspectEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=qualityInspectService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return qualityInspectReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			qualityInspect = qualityInspectService.getQualityInspect(this.qualityInspect.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,qualityInspect.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "qualityInspectView.ftl") }),
		@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "qualityInspectViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectList", results = { @Result(type = FREEMARKER, location = "qualityInspectList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = qualityInspectService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(qualityInspectService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(qualityInspectService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_qualityRecords_QualityInspect", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.activeID.orderProcessId.headId.productBatchNum,result.activeID.name,result.remark,result.activeID.headID.productBatchNum,result.activeID.orderProcessId.headId.id,result.activeID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="qualityInspectList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"qualityInspect\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"qualityInspect\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"qualityInspect\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"qualityInspect\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			qualityInspectService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/qualityRecords/qualityInspect/allTempleteDownload", results = { @Result(name="excel",type="excel",location="qualityInspectList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_qualityRecords_QualityInspect");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		qualityInspectService.findDataGridPage(dg,dgclass,getDgPage(),qualityInspect,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(qualityInspect.getId() != null && qualityInspect.getId() != -1){
				qualityInspect = qualityInspectService.getQualityInspect(qualityInspect.getId());
			}
			qualityInspectService.findDataGridPage(dg,dgclass,getDgPage(),qualityInspect,searchCondition,params);
	...
	private void save() throws Exception {
		...
		qualityInspectService.saveQualityInspect(qualityInspect,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/qualityRecords/qualityInspect/qualityInspectView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "submit");
		bap_validate_datagrids=qualityInspectService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", qualityInspect.getId());
		...
		fileuploads.put("linkId", qualityInspect.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/qualityRecords/qualityInspect/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		qualityInspectService.deleteQualityInspect(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/qualityRecords/qualityInspect/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "restore");
		qualityInspectService.restoreQualityInspect(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/qualityRecords/qualityInspect/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/qualityRecords/qualityInspect/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "showDealInfoData");
		if (null != tableInfoId) {
			int count=qualityInspectService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = qualityInspectService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = qualityInspectService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMQualityInspect> findExportDataInfos(Page<WOMQualityInspect> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = qualityInspectService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = qualityInspectService.getAssProperty(p.getCode());	
									//String assModelCode = qualityInspectService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			qualityInspectService.findQualityInspects(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			qualityInspectService.findQualityInspects(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMQualityInspect> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMQualityInspect result : results){
	...
	public boolean isEditNew(){
		//return null == this.qualityInspect || null == this.qualityInspect.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/qualityRecords/qualityInspect/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/qualityRecords/qualityInspect/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/qualityRecords/qualityInspect/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/qualityRecords/qualityInspect/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMQualityInspectService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_qualityRecords", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_qualityRecords");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/qualityRecords/qualityInspect/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_qualityRecords_QualityInspect", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_qualityRecords_QualityInspect", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/qualityRecords/qualityInspect/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/qualityRecords/qualityInspect/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_qualityRecords_QualityInspect";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_qualityRecords_QualityInspect", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/qualityRecords/qualityInspect/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMQualityInspect qualityInspect = null;
		if(null != id) {
			qualityInspect = qualityInspectService.getQualityInspect(id);
		}
		...
		_auditLog.setColumnStringB(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
		_auditLog.setColumnStringA(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
		_auditLog.setColumnStringC(qualityInspect.getId().toString());
		_auditLog.setColumnStringD(WOMQualityInspect.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_qualityRecords");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMQualityInspectAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, qualityInspect,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, qualityInspect,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,qualityInspect);
		...
		printlog.setEntity("WOM_7.5.0.0_qualityRecords");
		...
		printlog.setBusinessId(qualityInspect.getId());
		printlog.setBussinessKey(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
		printlog.setMainDisplay(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/qualityRecords/qualityInspect/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(qualityInspect,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/qualityRecords/qualityInspect/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(qualityInspectService.getQualityInspect(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/qualityRecords/qualityInspect/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/qualityRecords/qualityInspect/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, qualityInspect,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				qualityInspect = qualityInspectService.getQualityInspect(Long.parseLong(arrId[i]));
				if(null != qualityInspect){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == qualityInspect.getId() ? "" : "," + qualityInspect.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == qualityInspect.getId() ? "" : "," + qualityInspect.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == qualityInspect.getId() ? "" : "," + qualityInspect.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == qualityInspect.getId() ? "" : qualityInspect.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, qualityInspect,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,qualityInspect);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/qualityRecords/qualityInspect/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMQualityInspect> findExportPageInfos(Page<WOMQualityInspect> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_qualityRecords" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = qualityInspectService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMQualityInspect queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = qualityInspectService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = qualityInspectService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_qualityRecords_QualityInspect");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_qualityRecords_QualityInspect")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_qualityRecords_QualityInspect");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_qualityRecords_QualityInspect");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = qualityInspectService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
