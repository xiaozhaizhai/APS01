...
@Repository
public class WOMProduceTaskManageDaoImpl extends ExtendedGenericDaoImpl<WOMProduceTaskManage, Long> implements WOMProduceTaskManageDao {
	@Override
	...
	@Override
	public WOMProduceTaskManageDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMProduceTaskManageDealInfo dealInfo = new WOMProduceTaskManageDealInfo();
	...
	@Override
	public WOMProduceTaskManageDealInfo copyAndSaveDealInfo(DealInfo di, WOMProduceTaskManage mainObj, Staff dealStaff) {
		WOMProduceTaskManageDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMProduceTaskManageDealInfo.class);
		...
		List<WOMProduceTaskManageDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
