...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMProduceTaskProcessAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProduceTaskProcess> {
	...
	private WOMProduceTaskProcess produceTaskProcess;
	...
	//导出excel时,主模型内容
	List<WOMProduceTaskProcess> queryResults = new ArrayList<WOMProduceTaskProcess>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/produceTaskProcess/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/produceTaskProcess/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_ProduceTaskProcess" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProduceTaskProcess> produceTaskProcesss;
	
	public List<WOMProduceTaskProcess> getProduceTaskProcesss() {
		return produceTaskProcesss;
	}
	...
	private Page<WOMProduceTaskProcess> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			produceTaskProcess = new WOMProduceTaskProcess();
			if(produceTaskProcess.getProcessState() != null) {
				produceTaskProcess.setProcessState((SystemCode) systemCodeService.getSystemCode(produceTaskProcess.getProcessState().getId()));
			}
		} else {
			if(id > 0){
				produceTaskProcess = produceTaskProcessService.getProduceTaskProcess(id, viewCode);
			}
		}
	...
	public Page<WOMProduceTaskProcess> getPage() {
		if (null == page) {
			page = new Page<WOMProduceTaskProcess>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProduceTaskProcess getProduceTaskProcess() {
		return produceTaskProcess;
	}		

	public void setProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess) {
		this.produceTaskProcess = produceTaskProcess;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/produceTaskProcess/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = produceTaskProcessService.getProduceTaskProcessAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/produceTaskProcess/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1489671052624
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String produceTaskProcessReferenceCopy() throws Exception{
			WOMProduceTaskProcess oldProduceTaskProcess = null;
			if(isEditNew()){
				...
				oldProduceTaskProcess = new WOMProduceTaskProcess();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,produceTaskProcess.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, produceTaskProcess.getId());
				...
				oldProduceTaskProcess = produceTaskProcessService.getProduceTaskProcess(id);
			}
			
			
			produceTaskProcess = produceTaskProcessService.getProduceTaskProcess(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceTaskProcess");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProduceTaskProcess.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProduceTaskProcess, null);
						Method setValue = WOMProduceTaskProcess.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(produceTaskProcess, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			produceTaskProcess.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=produceTaskProcessService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return produceTaskProcessReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			produceTaskProcess = produceTaskProcessService.getProduceTaskProcess(this.produceTaskProcess.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,produceTaskProcess.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			produceTaskProcessService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = produceTaskProcessService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(produceTaskProcessService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(produceTaskProcessService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_ProduceTaskProcess", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/produceTaskProcess/taskProcessExelogRef", results = { @Result(type = FREEMARKER, location = "taskProcessExelogRef.ftl") })
		,
		@Action(value = "/WOM/produceTask/produceTaskProcess/taskProcessRef", results = { @Result(type = FREEMARKER, location = "taskProcessRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"produceTaskProcess\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"produceTaskProcess\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"produceTaskProcess\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"produceTaskProcess\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			produceTaskProcessService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/produceTask/produceTaskProcess/taskProcessExelogRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headId.tableNo,result.name,result.longTime,result.remark,result.headId.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="taskProcessExelogRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/produceTask/produceTaskProcess/taskProcessRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.longTime,result.remark,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="taskProcessRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		produceTaskProcessService.findDataGridPage(dg,dgclass,getDgPage(),produceTaskProcess,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(produceTaskProcess.getId() != null && produceTaskProcess.getId() != -1){
				produceTaskProcess = produceTaskProcessService.getProduceTaskProcess(produceTaskProcess.getId());
			}
			produceTaskProcessService.findDataGridPage(dg,dgclass,getDgPage(),produceTaskProcess,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == produceTaskProcess.getId()) {
		...
		produceTaskProcessService.saveProduceTaskProcess(produceTaskProcess,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "submit");
		bap_validate_datagrids=produceTaskProcessService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", produceTaskProcess.getId());
		...
		fileuploads.put("linkId", produceTaskProcess.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/produceTaskProcess/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		produceTaskProcessService.deleteProduceTaskProcess(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceTaskProcess/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "restore");
		produceTaskProcessService.restoreProduceTaskProcess(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProduceTaskProcess> findExportDataInfos(Page<WOMProduceTaskProcess> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = produceTaskProcessService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = produceTaskProcessService.getAssProperty(p.getCode());	
									//String assModelCode = produceTaskProcessService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			produceTaskProcessService.findProduceTaskProcesss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			produceTaskProcessService.findProduceTaskProcesss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProduceTaskProcess> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProduceTaskProcess result : results){
	...
	public boolean isEditNew(){
		//return null == this.produceTaskProcess || null == this.produceTaskProcess.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/produceTaskProcess/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/produceTaskProcess/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/produceTaskProcess/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceTaskProcess/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProduceTaskProcessService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceTaskProcess/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_ProduceTaskProcess", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_ProduceTaskProcess", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/produceTaskProcess/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/produceTaskProcess/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_ProduceTaskProcess";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_ProduceTaskProcess", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/produceTaskProcess/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
		_auditLog.setColumnStringA(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
		_auditLog.setColumnStringC(produceTaskProcess.getId().toString());
		_auditLog.setColumnStringD(WOMProduceTaskProcess.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProduceTaskProcessAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, produceTaskProcess,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, produceTaskProcess,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,produceTaskProcess);
		...
		printlog.setBusinessId(produceTaskProcess.getId());
		printlog.setBussinessKey(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
		printlog.setMainDisplay(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/produceTaskProcess/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(produceTaskProcess,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/produceTaskProcess/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(produceTaskProcessService.getProduceTaskProcess(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/produceTaskProcess/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/produceTaskProcess/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, produceTaskProcess,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				produceTaskProcess = produceTaskProcessService.getProduceTaskProcess(Long.parseLong(arrId[i]));
				if(null != produceTaskProcess){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == produceTaskProcess.getId() ? "" : "," + produceTaskProcess.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == produceTaskProcess.getId() ? "" : "," + produceTaskProcess.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == produceTaskProcess.getId() ? "" : "," + produceTaskProcess.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, produceTaskProcess,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,produceTaskProcess);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//produceTaskProcessService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/produceTaskProcess/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceTaskProcess");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_ProduceTaskProcess")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_ProduceTaskProcess");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceTaskProcess");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = produceTaskProcessService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
     if ("WOM_7.5.0.0_produceTask_taskProcessExelogRef".equals(code)
				&& "taskProcessExelogRefQuery".equals(methodName)) { // 根据物品过滤数据
			if (taskID != null && !"".equals(taskID)) {
				String customerSql = "(\"produceTaskProcess\".HEAD_ID in(select ID from WOM_PRODUCE_TASKS where ( ID = "
						+ taskID + " and valid = 1)))";
				params.add(new Param("customerSql", customerSql));
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
	private Long taskID;
	private String oProcessID;//指令单工序id
	
	public String getoProcessID() {
		return oProcessID;
	}

	public void setoProcessID(String oProcessID) {
