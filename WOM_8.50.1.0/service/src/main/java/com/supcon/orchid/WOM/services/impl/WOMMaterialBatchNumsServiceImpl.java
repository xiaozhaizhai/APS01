...
import com.supcon.orchid.WOM.daos.WOMMaterialBatchNumsDao;
import com.supcon.orchid.WOM.services.WOMMaterialBatchNumsService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMMaterialBatchNumsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */


@Service("wOM_materialBatchNumsService")
@Transactional
public class WOMMaterialBatchNumsServiceImpl  extends BaseServiceImpl<WOMMaterialBatchNums> implements WOMMaterialBatchNumsService, InitializingBean, DisposableBean {
	...
	@Autowired
	private WOMMaterialBatchNumsDao materialBatchNumsDao;
	@Autowired
	...
	@Resource
	private IBAPBaseService<WOMMaterialBatchNums> bapBaseService;
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMMaterialBatchNums.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)";
		Query query =  materialBatchNumsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMMaterialBatchNums.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = materialBatchNumsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMMaterialBatchNums.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMMaterialBatchNums.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  materialBatchNumsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMMaterialBatchNums.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMMaterialBatchNums.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMMaterialBatchNums> list = (List<WOMMaterialBatchNums>) materialBatchNumsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMMaterialBatchNums getMaterialBatchNums(long id){
		return getMaterialBatchNums(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMaterialBatchNums getMaterialBatchNums(long id, String viewCode){
		WOMMaterialBatchNums materialBatchNums = materialBatchNumsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMaterialBatchNumsAsJSON(long id, String include){
		WOMMaterialBatchNums materialBatchNums = materialBatchNumsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(materialBatchNums, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="删除",operType="3")
	public void deleteMaterialBatchNums(WOMMaterialBatchNums materialBatchNums){
		beforeDeleteMaterialBatchNums(materialBatchNums);
		materialBatchNumsDao.deletePhysical(materialBatchNums);
		afterDeleteMaterialBatchNums(materialBatchNums);
		List<WOMMaterialBatchNums> params = new ArrayList<WOMMaterialBatchNums>();
		params.add(materialBatchNums);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="删除",operType="3")
	public void deleteMaterialBatchNums(List<Long> materialBatchNumsIds){
		deleteMaterialBatchNums(materialBatchNumsIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="删除",operType="3")
	public void deleteMaterialBatchNums(List<Long> materialBatchNumsIds, String eventTopic) {
		List<WOMMaterialBatchNums> materialBatchNumss = new ArrayList<WOMMaterialBatchNums>();
		for(Long materialBatchNumsId : materialBatchNumsIds){
			WOMMaterialBatchNums materialBatchNums = getMaterialBatchNums(materialBatchNumsId);
			materialBatchNumss.add(materialBatchNums);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(materialBatchNumss != null && materialBatchNumss.size() > 0 ){
			for(WOMMaterialBatchNums materialBatchNums : materialBatchNumss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMMaterialBatchNums.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(materialBatchNums));
						}
					}
					dataAuditLogService.saveCustomerAudit("删除",materialBatchNums, materialBatchNums.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(materialBatchNumsIds != null && materialBatchNumsIds.size() > 0) {
			String hql = "delete " + WOMMaterialBatchNums.JPA_NAME + " where id in(:ids)";
			Query query = materialBatchNumsDao.createQuery(hql);
			query.setParameterList("ids", materialBatchNumsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMMaterialBatchNums materialBatchNumsz : materialBatchNumss){
				materialBatchNumsz.setValid(false);
				List<WOMMaterialBatchNums> params = new ArrayList<WOMMaterialBatchNums>();
				params.add(materialBatchNumsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="删除",operType="3")
	public void deleteMaterialBatchNums(long materialBatchNumsId, int materialBatchNumsVersion){
		
		WOMMaterialBatchNums materialBatchNums = getMaterialBatchNums(materialBatchNumsId);
		
		beforeDeleteMaterialBatchNums(materialBatchNums);
		materialBatchNumsDao.delete(materialBatchNumsId, materialBatchNumsVersion);
		afterDeleteMaterialBatchNums(materialBatchNums);
		List<WOMMaterialBatchNums> params = new ArrayList<WOMMaterialBatchNums>();
		params.add(materialBatchNums);
		Map<String, Object> props = new HashMap<String, Object>();
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="删除",operType="3")
	public void deleteMaterialBatchNums(String materialBatchNumsIds){
		deleteCollection(materialBatchNumsIds);
	}
	...
	private void deleteCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMaterialBatchNums(Long.valueOf(id), Integer.valueOf(version));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="批量导入",operType="4")
	public  void batchImportBaseMaterialBatchNums(List<WOMMaterialBatchNums>  materialBatchNumss){
		for(WOMMaterialBatchNums materialBatchNums:materialBatchNumss)  {
			saveMaterialBatchNums(materialBatchNums, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMaterialBatchNums(List<WOMMaterialBatchNums>  materialBatchNumss){
		for(WOMMaterialBatchNums materialBatchNums:materialBatchNumss)  {
			saveMaterialBatchNums(materialBatchNums, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="保存")
	public void saveMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMaterialBatchNums(materialBatchNums, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="保存")
	public void saveMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMaterialBatchNums(materialBatchNums, dgLists,dgDeleteIDs,assFileUploads, viewCode, null);
	}


	private void generateMaterialBatchNumsCodes(WOMMaterialBatchNums materialBatchNums) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMaterialBatchNumsCodes(materialBatchNums, false);
	}
	
	private void generateMaterialBatchNumsCodes(WOMMaterialBatchNums materialBatchNums, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	private void generateMaterialBatchNumsSummarys(WOMMaterialBatchNums materialBatchNums) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMaterialBatchNumsSummarys(materialBatchNums, false);
	}

	private void generateMaterialBatchNumsSummarys(WOMMaterialBatchNums materialBatchNums, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums")
	public void saveMaterialBatchNums(WOMMaterialBatchNums materialBatchNums,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(materialBatchNums.getId()!=null && materialBatchNums.getId()>0){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, materialBatchNums);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(materialBatchNums);
		beforeSaveMaterialBatchNums(materialBatchNums, viewIsView);

		if (viewIsView) {
			materialBatchNumsDao.saveWithRevertVersion(materialBatchNums);
		} else {
			if(isNew)
					materialBatchNumsDao.save(materialBatchNums);
				else
					materialBatchNumsDao.merge(materialBatchNums);
		}
		dealDatagridsSave(materialBatchNums,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateMaterialBatchNumsCodes(materialBatchNums, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMaterialBatchNumsSummarys(materialBatchNums, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMaterialBatchNums(materialBatchNums, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, materialBatchNums);
			}
		}
		List<WOMMaterialBatchNums> params = new ArrayList<WOMMaterialBatchNums>();
		materialBatchNumsDao.flush();
		materialBatchNumsDao.clear();
		materialBatchNums = materialBatchNumsDao.load(materialBatchNums.getId());
		
		
		params.add(materialBatchNums);
	}
	
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMMaterialBatchNums materialBatchNums){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("materialBatchNums", materialBatchNums);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums")
	public void mergeMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(materialBatchNums);
		beforeSaveMaterialBatchNums(materialBatchNums);
		materialBatchNumsDao.merge(materialBatchNums);

		dealDatagridsSave(materialBatchNums,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveMaterialBatchNums(materialBatchNums);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMMaterialBatchNums> findMaterialBatchNumss(Page<WOMMaterialBatchNums> page, Criterion... criterions) {
		return materialBatchNumsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMaterialBatchNumss(Page<WOMMaterialBatchNums> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMaterialBatchNumss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMaterialBatchNumss(Page<WOMMaterialBatchNums> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMaterialBatchNumss(Page<WOMMaterialBatchNums> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void commonQuery(Page<WOMMaterialBatchNums> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"materialBatchNums\"" : "\"" + key.substring(0, lastDotPos) + "\"";
				...
				if(type == Sql.TYPE_LIST_REFERENCE){
					if(null != crossCompanyFlag && !crossCompanyFlag){
						s.append(" AND \"materialBatchNums\".CID = ").append(getCurrentCompanyId());
					}
				}
...
//				}
			...
			if (hasWhere)
				...
				s.append(" WHERE ");
			s.append(" ( \"materialBatchNums\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			List<Object> powerCodeList =  materialBatchNumsDao.createNativeQuery(powerCodeSql, powerCode).list();
			if(powerCodeList.size() > 0) {
				...
				List<Object> checkList =  materialBatchNumsDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
				if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"materialBatchNums\".LAY_REC ASC, \"materialBatchNums\".SORT ASC");
				} else {
					orderPart.append(" \"materialBatchNums\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) materialBatchNumsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) materialBatchNumsDao.createNativeQuery(countSql, arr)
			...
			if(!noQueryFlag)  {
				SQLQuery query = materialBatchNumsDao.createNativeQuery(realSql, arr);
				List<WOMMaterialBatchNums> result = new ArrayList<WOMMaterialBatchNums>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMMaterialBatchNums.class,materialBatchNumsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMMaterialBatchNums.class,materialBatchNumsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMMaterialBatchNums materialBatchNums : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(materialBatchNums.getId(), "WOM_produceTask_materialBatchNums");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(materialBatchNums.getId(), "WOM_produceTask_materialBatchNums");
							if(null != documents && !documents.isEmpty()) {
								materialBatchNums.setDocument(documents.get(0));
								materialBatchNums.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMMaterialBatchNums> result) {
		View view = viewServiceFoundation.getView(viewCode);
		...
		Map<Long, WOMMaterialBatchNums> rsMap = new HashMap<Long, WOMMaterialBatchNums>();
		for (WOMMaterialBatchNums materialBatchNums : result) {
			ids.add(materialBatchNums.getId());
			rsMap.put(materialBatchNums.getId(), materialBatchNums);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(materialBatchNumsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = materialBatchNumsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_MaterialBatchNums" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_MaterialBatchNums" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  materialBatchNumsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"materialBatchNums\"","WOM_7.5.0.0_produceTask","","WOM_MATERIAL_BATCH_NUMS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"materialBatchNums\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"materialBatchNums\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"materialBatchNums\"","WOM_7.5.0.0_produceTask","","WOM_MATERIAL_BATCH_NUMS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_MaterialBatchNums" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  UPPER(\"").append("materialBatchNums.mc\"").append(".MNE_CODE) like UPPER(?) escape '&' AND \"materialBatchNums\".VALID = 1");
			...
			if(!cross){
				mnecodeSql.append(" AND \"materialBatchNums\".CID = ?");
				totalValues.add(getCurrentCompanyId());
			}
			
			SQLQuery query = materialBatchNumsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=materialBatchNumsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMMaterialBatchNums.ENTITY_CODE;}
	protected String getModuleCode(){return WOMMaterialBatchNums.MODULE_CODE;}
	protected String getModelCode(){return WOMMaterialBatchNums.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param materialBatchNums 模型实例
	 */
	private void checkUniqueConstraint(WOMMaterialBatchNums materialBatchNums){
	}
	
	
	private void beforeSaveMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, Object... objects){
		checkUniqueConstraint(materialBatchNums);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void afterSaveMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
		//保存时提交数据，更改活动数据
		materialBatchNumsDao.flush();
		materialBatchNumsDao.clear();
		materialBatchNums = materialBatchNumsDao.load(materialBatchNums.getId());
		activeId = produceTaskActiveService.getProduceTaskActive(materialBatchNums.getActiveId().getId());
      	if(activeId!=null){
            List<WOMMaterialBatchNums> BatchNums = getMaterialBatchNumss("activeId="+activeId.getId(), null);
            BigDecimal sum = new BigDecimal(0);
            if(BatchNums!=null){
                for (WOMMaterialBatchNums womMaterialBatchNums : BatchNums) {
                    sum = sum.add(womMaterialBatchNums.getPlanNum());
                }
            }
            String powerOperateSql = "update wom_produce_task_actives set PLAN_NUM = ? where ID=?";
            SQLQuery  sqlQuery = materialBatchNumsDao.createNativeQuery(powerOperateSql, sum,materialBatchNums.getActiveId().getId());
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */	
	}
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMaterialBatchNums(WOMMaterialBatchNums materialBatchNums){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void afterDeleteMaterialBatchNums(WOMMaterialBatchNums materialBatchNums){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	private void customGenerateCodes(WOMMaterialBatchNums materialBatchNums, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMMaterialBatchNums materialBatchNums, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}

	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportMaterialBatchNums(List<WOMMaterialBatchNums> insertObjs, List<WOMMaterialBatchNums> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMaterialBatchNums,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	@Override
	public void afterImportMaterialBatchNums(List<WOMMaterialBatchNums> insertObjs, List<WOMMaterialBatchNums> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMaterialBatchNums,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	@Override
	public List<WOMMaterialBatchNums> getMaterialBatchNumss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMaterialBatchNums.class);
		if(params!=null){
			...
			return materialBatchNumsDao.findByHql("from " + WOMMaterialBatchNums.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return materialBatchNumsDao.findByCriteria(detachedCriteria);
	}
	
	@Override
	public WOMMaterialBatchNums loadMaterialBatchNumsByBussinessKey(WOMMaterialBatchNums materialBatchNums){
		return loadMaterialBatchNumsByBussinessKey(materialBatchNums.getId());
	}
	
	@Override
	public Page<WOMMaterialBatchNums> getMaterialBatchNumss(Page<WOMMaterialBatchNums> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMMaterialBatchNums.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = materialBatchNumsDao.findByPage(page, sql);
			}else{
				page = materialBatchNumsDao.findAllByPage(page);
			}
			return page;
		}
		page = materialBatchNumsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMMaterialBatchNums> findByTreeDataGridProduceTaskActive(List<WOMMaterialBatchNums> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMMaterialBatchNums materialBatchNums : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTaskActive(Page<WOMMaterialBatchNums> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMMaterialBatchNums> materialBatchNumss = dgPage.getResult();
			for(WOMMaterialBatchNums materialBatchNums : materialBatchNumss){
				
			}
			dgPage.setResult(materialBatchNumss);
		}
	...
	@Override
	@Transactional
	public void deleteMaterialBatchNumsByProduceTaskActive(List<Long> produceTaskActiveIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("activeIdList", produceTaskActiveIds);
		String hql = "select id from WOMMaterialBatchNums where "
						+ "activeId.id in (:activeIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> materialBatchNumsIds1 = materialBatchNumsDao.findByHql(hql, params);
		if(materialBatchNumsIds1 != null && !materialBatchNumsIds1.isEmpty()) {
		}
		hql = "delete WOMMaterialBatchNums where "
						+ "activeId.id in (:activeIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		materialBatchNumsDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMaterialBatchNums loadMaterialBatchNumsByBussinessKey(Serializable bussinessKey){
		return materialBatchNumsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMMaterialBatchNums> findByProperty(String propertyName, Object object){
		return materialBatchNumsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMMaterialBatchNums findEntityByProperty(String propertyName, Object object){
		return materialBatchNumsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMMaterialBatchNums.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = materialBatchNumsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_MaterialBatchNums","runtime");
		try {
	...
	private void dealDatagridsSave(WOMMaterialBatchNums materialBatchNums,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()), _reflect_dgclass,materialBatchNumsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMMaterialBatchNums.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, materialBatchNums);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(materialBatchNums.getTableInfoId());
							}else{
								((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(materialBatchNums.getTableInfoId());
							}
							if(isTree){
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",materialBatchNums.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMMaterialBatchNums.TABLE_NAME;
			list = materialBatchNumsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	
	@Autowired
	private WOMMaterialBatchNumsDaoImpl materialBatchNumsDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMMaterialBatchNums.TABLE_NAME + " where id = ?";
		String tableNo = materialBatchNumsDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public Map<Object, Long> importBatchMaterialBatchNums(final List<WOMMaterialBatchNums> insertObjs, final List<WOMMaterialBatchNums> updateObjs, 
		...
		final List<WOMMaterialBatchNums> nmeObjs = new ArrayList<WOMMaterialBatchNums>();
		...
		//String idSql = "select id from " + WOMMaterialBatchNums.TABLE_NAME + " order by id desc";
		//Query query = materialBatchNumsDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMMaterialBatchNums.JPA_NAME + " order by id desc";
		Query query =  materialBatchNumsDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMMaterialBatchNums.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMMaterialBatchNums materialBatchNums:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMMaterialBatchNums.TABLE_NAME,1,dbId);
			if(materialBatchNums.getId() != null ){
				m.put(materialBatchNums.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) materialBatchNumsDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_MaterialBatchNums"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		materialBatchNumsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMMaterialBatchNums.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMMaterialBatchNums materialBatchNums = updateObjs.get(updateIndex);
						Long id = materialBatchNums.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = materialBatchNums.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(materialBatchNums);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMMaterialBatchNums.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
					...
					insertSql += dbId + ",CID,TABLE_INFO_ID,CREATE_STAFF_ID) VALUES (";
					for(int i = 0;i<columnMaps.size();i++){
						insertSql += "?,";
					}
					insertSql += "?,?,?,?)";
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMMaterialBatchNums materialBatchNums = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = materialBatchNums.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		materialBatchNumsDaoImpl.getSession().flush();
		materialBatchNumsDaoImpl.getSession().clear();
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMMaterialBatchNums> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMMaterialBatchNums> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMMaterialBatchNums> result = getResult(page, query, new PendingResultTransformer(WOMMaterialBatchNums.class,materialBatchNumsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMMaterialBatchNums model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = materialBatchNumsDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_MaterialBatchNums").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = materialBatchNumsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = materialBatchNumsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = materialBatchNumsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?materialBatchNumsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  materialBatchNumsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,materialBatchNumsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="保存")
	public void saveMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, DataGridService dataGridService){
		saveMaterialBatchNums(materialBatchNums, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="保存")
	public void saveMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, DataGridService dataGridService, String viewCode){
		saveMaterialBatchNums(materialBatchNums, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums")
	public void saveMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums")
	public Map<Object, Object> materialBatchNumsDataGridImport(WOMMaterialBatchNums materialBatchNums, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = materialBatchNums.getVirtualId();
		
		materialBatchNums = materialBatchNumsDao.load(materialBatchNums.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, materialBatchNums.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = materialBatchNums.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(materialBatchNums);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, materialBatchNums.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums")
	public void mergeMaterialBatchNums(WOMMaterialBatchNums materialBatchNums, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(materialBatchNums);
		beforeSaveMaterialBatchNums(materialBatchNums);
		materialBatchNumsDao.merge(materialBatchNums);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="excel导出",operType="4")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="页面打印",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_MaterialBatchNums",desc="批量保存",operType="4")
	...
	@Override
	public Page<WOMMaterialBatchNums> getByPage(Page<WOMMaterialBatchNums> page,DetachedCriteria detachedCriteria){
		return materialBatchNumsDao.findByPage(page, detachedCriteria);
	}
	
	
	
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_MaterialBatchNums,WOM_7.5.0.0) */
// 自定义代码
	WOMProduceTaskActive activeId ;
/* CUSTOM CODE END */
}
