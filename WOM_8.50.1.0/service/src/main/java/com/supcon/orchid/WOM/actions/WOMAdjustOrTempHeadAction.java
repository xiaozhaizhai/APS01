...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
...
public class WOMAdjustOrTempHeadAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMAdjustOrTempHead> {
	...
	private WOMAdjustOrTempHead adjustOrTempHead;
	...
	//导出excel时,主模型内容
	List<WOMAdjustOrTempHead> queryResults = new ArrayList<WOMAdjustOrTempHead>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/adjustOrTempHead/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/adjustOrTempHead/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_AdjustOrTempHead" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMAdjustOrTempHead> adjustOrTempHeads;
	
	public List<WOMAdjustOrTempHead> getAdjustOrTempHeads() {
		return adjustOrTempHeads;
	}
	...
	private Page<WOMAdjustOrTempHead> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			adjustOrTempHead = new WOMAdjustOrTempHead();
			if(adjustOrTempHead.getActiveMissonType() != null) {
				adjustOrTempHead.setActiveMissonType((SystemCode) systemCodeService.getSystemCode(adjustOrTempHead.getActiveMissonType().getId()));
			}
			if(adjustOrTempHead.getActiveType() != null) {
				adjustOrTempHead.setActiveType((SystemCode) systemCodeService.getSystemCode(adjustOrTempHead.getActiveType().getId()));
			}
			if(adjustOrTempHead.getDataSource() != null) {
				adjustOrTempHead.setDataSource((SystemCode) systemCodeService.getSystemCode(adjustOrTempHead.getDataSource().getId()));
			}
		} else {
			if(id > 0){
				adjustOrTempHead = adjustOrTempHeadService.getAdjustOrTempHead(id, viewCode);
			}
		}
	...
	public Page<WOMAdjustOrTempHead> getPage() {
		if (null == page) {
			page = new Page<WOMAdjustOrTempHead>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMAdjustOrTempHead getAdjustOrTempHead() {
		return adjustOrTempHead;
	}		

	public void setAdjustOrTempHead(WOMAdjustOrTempHead adjustOrTempHead) {
		this.adjustOrTempHead = adjustOrTempHead;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/adjustOrTempHead/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = adjustOrTempHeadService.getAdjustOrTempHeadAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/adjustOrTempHead/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1542097427304
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String adjustOrTempHeadReferenceCopy() throws Exception{
			WOMAdjustOrTempHead oldAdjustOrTempHead = null;
			if(isEditNew()){
				...
				oldAdjustOrTempHead = new WOMAdjustOrTempHead();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,adjustOrTempHead.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, adjustOrTempHead.getId());
				...
				oldAdjustOrTempHead = adjustOrTempHeadService.getAdjustOrTempHead(id);
			}
			
			
			adjustOrTempHead = adjustOrTempHeadService.getAdjustOrTempHead(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_AdjustOrTempHead");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMAdjustOrTempHead.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldAdjustOrTempHead, null);
						Method setValue = WOMAdjustOrTempHead.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(adjustOrTempHead, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			adjustOrTempHead.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=adjustOrTempHeadService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return adjustOrTempHeadReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			adjustOrTempHead = adjustOrTempHeadService.getAdjustOrTempHead(this.adjustOrTempHead.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,adjustOrTempHead.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			adjustOrTempHeadService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = adjustOrTempHeadService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(adjustOrTempHeadService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(adjustOrTempHeadService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_AdjustOrTempHead", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"adjustOrTempHead\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"adjustOrTempHead\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"adjustOrTempHead\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"adjustOrTempHead\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			adjustOrTempHeadService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		adjustOrTempHeadService.findDataGridPage(dg,dgclass,getDgPage(),adjustOrTempHead,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(adjustOrTempHead.getId() != null && adjustOrTempHead.getId() != -1){
				adjustOrTempHead = adjustOrTempHeadService.getAdjustOrTempHead(adjustOrTempHead.getId());
			}
			adjustOrTempHeadService.findDataGridPage(dg,dgclass,getDgPage(),adjustOrTempHead,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == adjustOrTempHead.getId()) {
		...
		adjustOrTempHeadService.saveAdjustOrTempHead(adjustOrTempHead,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "submit");
		bap_validate_datagrids=adjustOrTempHeadService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", adjustOrTempHead.getId());
		...
		fileuploads.put("linkId", adjustOrTempHead.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/adjustOrTempHead/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		adjustOrTempHeadService.deleteAdjustOrTempHead(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempHead/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "restore");
		adjustOrTempHeadService.restoreAdjustOrTempHead(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMAdjustOrTempHead> findExportDataInfos(Page<WOMAdjustOrTempHead> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = adjustOrTempHeadService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = adjustOrTempHeadService.getAssProperty(p.getCode());	
									//String assModelCode = adjustOrTempHeadService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			adjustOrTempHeadService.findAdjustOrTempHeads(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			adjustOrTempHeadService.findAdjustOrTempHeads(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMAdjustOrTempHead> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMAdjustOrTempHead result : results){
	...
	public boolean isEditNew(){
		//return null == this.adjustOrTempHead || null == this.adjustOrTempHead.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/adjustOrTempHead/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/adjustOrTempHead/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/adjustOrTempHead/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempHead/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMAdjustOrTempHeadService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempHead/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_AdjustOrTempHead", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_AdjustOrTempHead", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/adjustOrTempHead/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/adjustOrTempHead/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_AdjustOrTempHead";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_AdjustOrTempHead", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/adjustOrTempHead/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		if(null != id) {
			adjustOrTempHead = adjustOrTempHeadService.getAdjustOrTempHead(id);
		}
		...
		_auditLog.setColumnStringB(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
		_auditLog.setColumnStringA(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
		_auditLog.setColumnStringC(adjustOrTempHead.getId().toString());
		_auditLog.setColumnStringD(WOMAdjustOrTempHead.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMAdjustOrTempHeadAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, adjustOrTempHead,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, adjustOrTempHead,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,adjustOrTempHead);
		...
		printlog.setBusinessId(adjustOrTempHead.getId());
		printlog.setBussinessKey(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
		printlog.setMainDisplay(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempHead/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(adjustOrTempHead,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/adjustOrTempHead/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(adjustOrTempHeadService.getAdjustOrTempHead(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/adjustOrTempHead/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/adjustOrTempHead/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, adjustOrTempHead,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				adjustOrTempHead = adjustOrTempHeadService.getAdjustOrTempHead(Long.parseLong(arrId[i]));
				if(null != adjustOrTempHead){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == adjustOrTempHead.getId() ? "" : "," + adjustOrTempHead.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == adjustOrTempHead.getId() ? "" : "," + adjustOrTempHead.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == adjustOrTempHead.getId() ? "" : "," + adjustOrTempHead.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == adjustOrTempHead.getId() ? "" : adjustOrTempHead.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, adjustOrTempHead,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,adjustOrTempHead);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//adjustOrTempHeadService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/adjustOrTempHead/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_AdjustOrTempHead");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_AdjustOrTempHead")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_AdjustOrTempHead");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_AdjustOrTempHead");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = adjustOrTempHeadService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_AdjustOrTempHead,WOM_7.5.0.0) */
// 自定义代码
	@Autowired
    ...
    public void setActiveRecordId(Long activeRecordId) {
	 ...
	 * @return
	 */
	@Action(value = "/WOM/produceTask/adjustOrTempHead/adjustStartActive", results = { @Result(type = JSON, params = {
			"root", "responseMap" }) })
	public String adjustStartActive(){
        WOMResult womResult = null;
        Map<String, Object> map = new HashMap<>();
		if (processId != null && dealId != null) {
            ProcedureReport procedureReport = handler.choseProcfeedWay(DealsEnum.MES);
            map.put("processId",processId);
            map.put("dealId",dealId);
            womResult = procedureReport.adjustStartActive(map);
		}
		responseMap.put("result", womResult.getData() != null ? (Long)womResult.getData() : null);
		return SUCCESS;
	}
	/**
	 * 不良品处理时，调整时，获取调整页面id
	 * @return
	 */
        @Action(value = "/WOM/produceTask/adjustOrTempHead/getAdjustActivePage", results = { @Result(type = JSON, params = {
			"root", "responseMap" }) })
	public String getAdjustActivePage(){
		Long result = null;
		if (activeRecordId != null) {
			result=adjustOrTempHeadService.getAdjustActivePage(activeRecordId);
		}
	 ...
	 * @return
	 */
	@Action(value = "/WOM/produceTask/adjustOrTempHead/adjustEndActive", results = { @Result(type = JSON, params = {
			"root", "responseMap" }) })
	public String adjustEndActive(){
        WOMResult womResult = null;
        Map<String, Object> map = new HashMap<>();
        if (activeRecordId != null) {
            ProcedureReport procedureReport = handler.choseProcfeedWay(DealsEnum.MES);
            map.put("activeRecordId",activeRecordId);
            map.put("inComeType","people");
            womResult = procedureReport.adjustEndActive(map);
        }
	 ...
	 * @return
	 */
	@Action(value = "/WOM/produceTask/adjustOrTempHead/toTempActive", results = { @Result(type = JSON, params = {
			"root", "responseMap" }) })
	public String toTempActive(){
		String result = null;
		if (processId != null) {
			result=adjustOrTempHeadService.toTempActive(processId);
		}
		responseMap.put("result", result);
		return SUCCESS;
	}
  /***
	...
	*
	*/
  @Action(value = "/WOM/produceTask/adjustOrTempHead/vaildTempActive", results = { @Result(type = JSON, params = {
              "root", "responseMap" }) })
  public String vaildTempActive(){
          String result = null;
          if (processId != null) {
              result=adjustOrTempHeadService.vaildTempActive(processId);
          }
          responseMap.put("result", result);
          return SUCCESS;
      }
