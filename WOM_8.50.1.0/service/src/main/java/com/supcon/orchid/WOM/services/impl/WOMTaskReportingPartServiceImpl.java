...
import com.supcon.orchid.WOM.services.WOMTaskReportingPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMTaskReportingPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_taskReportingPartService")
@Transactional
public class WOMTaskReportingPartServiceImpl  extends BaseServiceImpl<WOMTaskReportingPart> implements WOMTaskReportingPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMTaskReportingPart> bapBaseService;
	...
	@Autowired
	private WOMTaskReportingPartImportService taskReportingPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMTaskReportingPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  taskReportingPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMTaskReportingPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = taskReportingPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMTaskReportingPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMTaskReportingPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  taskReportingPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMTaskReportingPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMTaskReportingPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMTaskReportingPart> list = (List<WOMTaskReportingPart>) taskReportingPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMTaskReportingPart getTaskReportingPart(long id){
		return getTaskReportingPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskReportingPart getTaskReportingPart(long id, String viewCode){
		WOMTaskReportingPart taskReportingPart = taskReportingPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getTaskReportingPartAsJSON(long id, String include){
		WOMTaskReportingPart taskReportingPart = taskReportingPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(taskReportingPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportingPart(WOMTaskReportingPart taskReportingPart){
		if(null != taskReportingPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportingPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
			AuditUtil.setColumnStringA(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
			AuditUtil.setColumnStringC(taskReportingPart.getId().toString());
		}
		beforeDeleteTaskReportingPart(taskReportingPart);
		taskReportingPartDao.deletePhysical(taskReportingPart);
		afterDeleteTaskReportingPart(taskReportingPart);
		taskReportingPartDao.flush();
		List<WOMTaskReportingPart> params = new ArrayList<WOMTaskReportingPart>();
		params.add(taskReportingPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportingPart(List<Long> taskReportingPartIds){
		deleteTaskReportingPart(taskReportingPartIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportingPart(List<Long> taskReportingPartIds, String eventTopic) {
		List<WOMTaskReportingPart> taskReportingParts = new ArrayList<WOMTaskReportingPart>();
		for(Long taskReportingPartId : taskReportingPartIds){
			WOMTaskReportingPart taskReportingPart = getTaskReportingPart(taskReportingPartId);
			taskReportingParts.add(taskReportingPart);
			if(null != taskReportingPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportingPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == taskReportingPart.getId() ? "" : "," + taskReportingPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == taskReportingPart.getId() ? "" : "," + taskReportingPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == taskReportingPart.getId() ? "" : "," + taskReportingPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
				}
			}
		}
		...
		if(taskReportingParts != null && taskReportingParts.size() > 0 ){
			for(WOMTaskReportingPart taskReportingPart : taskReportingParts){
				...
				java.lang.reflect.Field[] fields = WOMTaskReportingPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(taskReportingPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),taskReportingPart, taskReportingPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(taskReportingPartIds != null && taskReportingPartIds.size() > 0) {
			String hql = "delete " + WOMTaskReportingPart.JPA_NAME + " where id in(:ids)";
			Query query = taskReportingPartDao.createQuery(hql);
			query.setParameterList("ids", taskReportingPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMTaskReportingPart taskReportingPartz : taskReportingParts){
				taskReportingPartz.setValid(false);
				List<WOMTaskReportingPart> params = new ArrayList<WOMTaskReportingPart>();
				params.add(taskReportingPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_taskReporting");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportingPart(long taskReportingPartId, int taskReportingPartVersion){
		this.deleteTaskReportingPart(taskReportingPartId, taskReportingPartVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportingPart(long taskReportingPartId, int taskReportingPartVersion,SignatureLog signatureLog){
		WOMTaskReportingPart taskReportingPart = getTaskReportingPart(taskReportingPartId);
		...
		beforeDeleteTaskReportingPart(taskReportingPart);
		taskReportingPartDao.delete(taskReportingPartId, taskReportingPartVersion);
		afterDeleteTaskReportingPart(taskReportingPart);
		taskReportingPartDao.flush();
		List<WOMTaskReportingPart> params = new ArrayList<WOMTaskReportingPart>();
		params.add(taskReportingPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=taskReportingPart.getId();
			signatureLog.setTableId(taskReportingPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportingPart(String taskReportingPartIds){
		this.deleteTaskReportingPart(taskReportingPartIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.common.delete",operType="3")
	public void deleteTaskReportingPart(String taskReportingPartIds,SignatureLog signatureLog){
		deleteCollection(taskReportingPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteTaskReportingPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_taskReporting#modelCode:WOM_7.5.0.0_taskReporting_TaskReportingPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="ec.engine.restore",operType="10")
	public void restoreTaskReportingPart(String taskReportingPartIds){
		restoreCollection(taskReportingPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreTaskReportingPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="ec.engine.restore",operType="10")
	public void restoreTaskReportingPart(long taskReportingPartId){
		
		findBusinessKeyUsed(taskReportingPartId);	//判断业务主键是否重复
		...
		beforeRestoreTaskReportingPart(taskReportingPart);
		taskReportingPart.setValid(true);
		taskReportingPartDao.update(taskReportingPart);
		afterRestoreTaskReportingPart(taskReportingPart);
		List<WOMTaskReportingPart> params = new ArrayList<WOMTaskReportingPart>();
		params.add(taskReportingPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long taskReportingPartId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_taskReporting_TaskReportingPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMTaskReportingPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMTaskReportingPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  taskReportingPartDao.createNativeQuery(sql,taskReportingPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseTaskReportingPart(List<WOMTaskReportingPart>  taskReportingParts){
		for(WOMTaskReportingPart taskReportingPart:taskReportingParts)  {
			saveTaskReportingPart(taskReportingPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseTaskReportingPart(List<WOMTaskReportingPart>  taskReportingParts){
		for(WOMTaskReportingPart taskReportingPart:taskReportingParts)  {
			saveTaskReportingPart(taskReportingPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="ec.flow.save")
	public void saveTaskReportingPart(WOMTaskReportingPart taskReportingPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveTaskReportingPart(taskReportingPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="ec.flow.save")
	public void saveTaskReportingPart(WOMTaskReportingPart taskReportingPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveTaskReportingPart(taskReportingPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateTaskReportingPartCodes(WOMTaskReportingPart taskReportingPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTaskReportingPartCodes(taskReportingPart, false);
	}
	
	public void generateTaskReportingPartCodes(WOMTaskReportingPart taskReportingPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateTaskReportingPartSummarys(WOMTaskReportingPart taskReportingPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTaskReportingPartSummarys(taskReportingPart, false);
	}

	public void generateTaskReportingPartSummarys(WOMTaskReportingPart taskReportingPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart")
	public void saveTaskReportingPart(WOMTaskReportingPart taskReportingPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveTaskReportingPart(taskReportingPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart")
	public void saveTaskReportingPart(WOMTaskReportingPart taskReportingPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_taskReporting");
		String url = null;
		if(taskReportingPart.getId() != null && taskReportingPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportingPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, taskReportingPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(taskReportingPart);
		beforeSaveTaskReportingPart(taskReportingPart, viewIsView);

		if (viewIsView) {
			taskReportingPartDao.saveWithRevertVersion(taskReportingPart);
		} else {
			if(isNew)
				taskReportingPartDao.save(taskReportingPart);
			else
				taskReportingPartDao.merge(taskReportingPart);
		}
		if(null != taskReportingPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportingPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
			AuditUtil.setColumnStringA(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
		...
		dealDatagridsSave(taskReportingPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateTaskReportingPartCodes(taskReportingPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateTaskReportingPartSummarys(taskReportingPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveTaskReportingPart(taskReportingPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, taskReportingPart);
			}
		}
		List<WOMTaskReportingPart> params = new ArrayList<WOMTaskReportingPart>();
		taskReportingPartDao.flush();
		taskReportingPartDao.clear();
		taskReportingPart = taskReportingPartDao.load(taskReportingPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMTaskReportingPart taskReportingPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("taskReportingPart", taskReportingPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart")
	public void mergeTaskReportingPart(WOMTaskReportingPart taskReportingPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(taskReportingPart);
		beforeSaveTaskReportingPart(taskReportingPart);
		taskReportingPartDao.merge(taskReportingPart);
		if(null != taskReportingPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportingPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
			AuditUtil.setColumnStringA(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
		...
		dealDatagridsSave(taskReportingPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveTaskReportingPart(taskReportingPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMTaskReportingPart> findTaskReportingParts(Page<WOMTaskReportingPart> page, Criterion... criterions) {
		return taskReportingPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskReportingParts(Page<WOMTaskReportingPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findTaskReportingParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskReportingParts(Page<WOMTaskReportingPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskReportingParts(Page<WOMTaskReportingPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMTaskReportingPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"taskReportingPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"taskReportingPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"taskReportingPart\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"taskReportingPart\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_TASK_REPORTINGS \"taskReporting\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_taskReporting" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  taskReportingPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  taskReportingPartDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_taskReporting").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = dataPermissionService.getPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskReporting\"","WOM_7.5.0.0_taskReporting","","WOM_TASK_REPORTINGS",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"taskReportingPart\".LAY_REC ASC, \"taskReportingPart\".SORT ASC");
				} else {
					orderPart.append(" \"taskReportingPart\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) taskReportingPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=taskReportingPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = taskReportingPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMTaskReportingPart> result = new ArrayList<WOMTaskReportingPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMTaskReportingPart.class,taskReportingPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMTaskReportingPart.class,taskReportingPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMTaskReportingPart taskReportingPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(taskReportingPart.getId(), "WOM_taskReporting_taskReportingPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(taskReportingPart.getId(), "WOM_taskReporting_taskReportingPart");
							if(null != documents && !documents.isEmpty()) {
								taskReportingPart.setDocument(documents.get(0));
								taskReportingPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMTaskReportingPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMTaskReportingPart> result) {
		View view = null;
		...
		Map<Long, WOMTaskReportingPart> rsMap = new HashMap<Long, WOMTaskReportingPart>();
		for (WOMTaskReportingPart taskReportingPart : result) {
			ids.add(taskReportingPart.getId());
			rsMap.put(taskReportingPart.getId(), taskReportingPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(taskReportingPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = taskReportingPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_taskReporting_TaskReportingPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_taskReporting_TaskReportingPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  taskReportingPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskReportingPart\"","WOM_7.5.0.0_taskReporting","","WOM_TASK_REPORTING_PARTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"taskReportingPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"taskReportingPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskReportingPart\"","WOM_7.5.0.0_taskReporting","","WOM_TASK_REPORTING_PARTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_taskReporting_TaskReportingPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("taskReportingPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"taskReportingPart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"taskReportingPart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"taskReportingPart\".CID = 1 OR \"taskReportingPart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("taskReportingPart.mc\"").append(".TASK_REPORTING_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = taskReportingPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=taskReportingPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMTaskReportingPart.ENTITY_CODE;}
	protected String getModuleCode(){return WOMTaskReportingPart.MODULE_CODE;}
	protected String getModelCode(){return WOMTaskReportingPart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param taskReportingPart 模型实例
	 */
	public void checkUniqueConstraint(WOMTaskReportingPart taskReportingPart){
	}
	
	
	@Override
	public List<WOMTaskReportingPart> getTaskReportingParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskReportingPart.class);
		if(params!=null){
			...
			return taskReportingPartDao.findByHql("from " + WOMTaskReportingPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return taskReportingPartDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMTaskReportingPart> findTaskReportingPartsByHql(String hql, Object... objects){
		...
		return   taskReportingPartDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMTaskReportingPart loadTaskReportingPartByBussinessKey(WOMTaskReportingPart taskReportingPart){
		return loadTaskReportingPartByBussinessKey(taskReportingPart.getId());
	}
	
	@Override
	public Page<WOMTaskReportingPart> getTaskReportingParts(Page<WOMTaskReportingPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskReportingPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = taskReportingPartDao.findByPage(page, sql);
			}else{
				page = taskReportingPartDao.findAllByPage(page);
			}
			return page;
		}
		page = taskReportingPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMTaskReportingPart> findByTreeDataGridTaskReporting(List<WOMTaskReportingPart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMTaskReportingPart taskReportingPart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridTaskReporting(Page<WOMTaskReportingPart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMTaskReportingPart> taskReportingParts = dgPage.getResult();
			for(WOMTaskReportingPart taskReportingPart : taskReportingParts){
				
			}
			dgPage.setResult(taskReportingParts);
		}
	...
	public WOMTaskReportingPart loadTaskReportingPartByBussinessKey(Serializable bussinessKey){
		return taskReportingPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMTaskReportingPart> findByProperty(String propertyName, Object object){
		return taskReportingPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskReportingPart findEntityByProperty(String propertyName, Object object){
		return taskReportingPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMTaskReportingPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = taskReportingPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_taskReporting_TaskReportingPart","runtime");
		try {
	...
	public void dealDatagridsSave(WOMTaskReportingPart taskReportingPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,taskReportingPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMTaskReportingPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, taskReportingPart);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(taskReportingPart.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(taskReportingPart.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(taskReportingPart.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",taskReportingPart.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMTaskReportingPart.TABLE_NAME +" where valid = 1";
			list = taskReportingPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMTaskReportingPart.TABLE_NAME;
			list = taskReportingPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMTaskReportingPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = taskReportingPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMTaskReportingPartDaoImpl taskReportingPartDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMTaskReportingPart.TABLE_NAME + " where id = ?";
		String tableNo = taskReportingPartDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchTaskReportingPart(final List<WOMTaskReportingPart> insertObjs, final List<WOMTaskReportingPart> updateObjs, 
		...
		return this.importBatchTaskReportingPart(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchTaskReportingPart(final List<WOMTaskReportingPart> insertObjs, final List<WOMTaskReportingPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1490773102678"),importCount));
		...
		Map<Long, WOMTaskReportingPart> oldUpdateObjsMap = new HashMap<Long, WOMTaskReportingPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMTaskReportingPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMTaskReportingPart> bizObjList = taskReportingPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMTaskReportingPart bizObj = bizObjList.get(mIndex);
		...
		final List<WOMTaskReportingPart> nmeObjs = new ArrayList<WOMTaskReportingPart>();
		...
		//String idSql = "select id from " + WOMTaskReportingPart.TABLE_NAME + " order by id desc";
		//Query query = taskReportingPartDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMTaskReportingPart.JPA_NAME + " order by id desc";
		Query query =  taskReportingPartDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMTaskReportingPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMTaskReportingPart taskReportingPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMTaskReportingPart.TABLE_NAME,1,dbId);
			if(taskReportingPart.getId() != null ){
				m.put(taskReportingPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) taskReportingPartDao.createNativeQuery(sql, "WOM_7.5.0.0_taskReporting_TaskReportingPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		taskReportingPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMTaskReportingPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMTaskReportingPart taskReportingPart = updateObjs.get(updateIndex);
						Long id = taskReportingPart.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = taskReportingPart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(taskReportingPart);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMTaskReportingPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMTaskReportingPart taskReportingPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = taskReportingPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		taskReportingPartDaoImpl.getSession().flush();
		taskReportingPartDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = taskReportingPartDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_taskReporting_TaskReportingPart").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = taskReportingPartDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_taskReporting_TaskReportingPart").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMTaskReportingPart bizObj = insertObjs.get(insertIndex);
			taskReportingPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMTaskReportingPart bizObj = updateObjs.get(updateIndex);
			WOMTaskReportingPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			taskReportingPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMTaskReportingPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMTaskReportingPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(linkMap != null && linkMap.get("needChange")!=null && linkMap.get("needChange").equals("no!!")){
				...
				s.append(" FROM WOM_TASK_REPORTINGS \"taskReporting\" ");
			}else{
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) taskReportingPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMTaskReportingPart> result = getResult(page, query, new PendingResultTransformer(WOMTaskReportingPart.class,taskReportingPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMTaskReportingPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = taskReportingPartDao.createNativeQuery(sql, "WOM_7.5.0.0_taskReporting_TaskReportingPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = taskReportingPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = taskReportingPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = taskReportingPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?taskReportingPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  taskReportingPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,taskReportingPartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="ec.flow.save")
	public void saveTaskReportingPart(WOMTaskReportingPart taskReportingPart, DataGridService dataGridService){
		saveTaskReportingPart(taskReportingPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="ec.flow.save")
	public void saveTaskReportingPart(WOMTaskReportingPart taskReportingPart, DataGridService dataGridService, String viewCode){
		saveTaskReportingPart(taskReportingPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart")
	public void saveTaskReportingPart(WOMTaskReportingPart taskReportingPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveTaskReportingPart(taskReportingPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart")
	public void saveTaskReportingPart(WOMTaskReportingPart taskReportingPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_taskReporting";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_taskReporting");
		String url = null;
		if(null != taskReportingPart.getId() && taskReportingPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMTaskReportingPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart")
	public Map<Object, Object> taskReportingPartDataGridImport(WOMTaskReportingPart taskReportingPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = taskReportingPart.getVirtualId();
		
		taskReportingPart = taskReportingPartDao.load(taskReportingPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, taskReportingPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = taskReportingPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(taskReportingPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, taskReportingPart.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart")
	public void mergeTaskReportingPart(WOMTaskReportingPart taskReportingPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(taskReportingPart);
		beforeSaveTaskReportingPart(taskReportingPart);
		taskReportingPartDao.merge(taskReportingPart);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_taskReporting",model="WOM_7.5.0.0_taskReporting_TaskReportingPart",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMTaskReportingPart> getByPage(Page<WOMTaskReportingPart> page,DetachedCriteria detachedCriteria){
		return taskReportingPartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveTaskReportingPart(WOMTaskReportingPart taskReportingPart, Object... objects){
		checkUniqueConstraint(taskReportingPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveTaskReportingPart(WOMTaskReportingPart taskReportingPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteTaskReportingPart(WOMTaskReportingPart taskReportingPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteTaskReportingPart(WOMTaskReportingPart taskReportingPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreTaskReportingPart(WOMTaskReportingPart taskReportingPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreTaskReportingPart(WOMTaskReportingPart taskReportingPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMTaskReportingPart taskReportingPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMTaskReportingPart taskReportingPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportTaskReportingPart(List<WOMTaskReportingPart> insertObjs, List<WOMTaskReportingPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportTaskReportingPart,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportTaskReportingPart(List<WOMTaskReportingPart> insertObjs, List<WOMTaskReportingPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportTaskReportingPart,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
