...
import com.supcon.orchid.WOM.daos.WOMTaskBeforeProcessDao;
import com.supcon.orchid.WOM.services.WOMTaskBeforeProcessService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMTaskBeforeProcessDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */


@Service("wOM_taskBeforeProcessService")
@Transactional
public class WOMTaskBeforeProcessServiceImpl  extends BaseServiceImpl<WOMTaskBeforeProcess> implements WOMTaskBeforeProcessService, InitializingBean, DisposableBean {
	...
	@Autowired
	private WOMTaskActivePendingDao taskActivePendingDao;
	@Autowired
	private WOMTaskActivePendingService taskActivePendingService;
	@Autowired
	private WOMTaskBeforeProcessDao taskBeforeProcessDao;
	@Autowired
	...
	@Resource
	private IBAPBaseService<WOMTaskBeforeProcess> bapBaseService;
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMTaskBeforeProcess.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)";
		Query query =  taskBeforeProcessDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMTaskBeforeProcess.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = taskBeforeProcessDao.createQuery(hql);
	...
	public WOMTaskBeforeProcess getTaskBeforeProcess(long id){
		return getTaskBeforeProcess(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskBeforeProcess getTaskBeforeProcess(long id, String viewCode){
		WOMTaskBeforeProcess taskBeforeProcess = taskBeforeProcessDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getTaskBeforeProcessAsJSON(long id, String include){
		WOMTaskBeforeProcess taskBeforeProcess = taskBeforeProcessDao.load(id);
		// 一对多情况处理

		if(taskBeforeProcess == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(taskBeforeProcess, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="删除",operType="3")
	public void deleteTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess){
		beforeDeleteTaskBeforeProcess(taskBeforeProcess);
		taskBeforeProcessDao.deletePhysical(taskBeforeProcess);
		afterDeleteTaskBeforeProcess(taskBeforeProcess);
		List<WOMTaskBeforeProcess> params = new ArrayList<WOMTaskBeforeProcess>();
		params.add(taskBeforeProcess);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="删除",operType="3")
	public void deleteTaskBeforeProcess(List<Long> taskBeforeProcessIds){
		deleteTaskBeforeProcess(taskBeforeProcessIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="删除",operType="3")
	public void deleteTaskBeforeProcess(List<Long> taskBeforeProcessIds, String eventTopic) {
		List<WOMTaskBeforeProcess> taskBeforeProcesss = new ArrayList<WOMTaskBeforeProcess>();
		for(Long taskBeforeProcessId : taskBeforeProcessIds){
			WOMTaskBeforeProcess taskBeforeProcess = getTaskBeforeProcess(taskBeforeProcessId);
			taskBeforeProcesss.add(taskBeforeProcess);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(taskBeforeProcesss != null && taskBeforeProcesss.size() > 0 ){
			for(WOMTaskBeforeProcess taskBeforeProcess : taskBeforeProcesss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMTaskBeforeProcess.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(taskBeforeProcess));
						}
					}
					dataAuditLogService.saveCustomerAudit("删除",taskBeforeProcess, taskBeforeProcess.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(taskBeforeProcessIds != null && taskBeforeProcessIds.size() > 0) {
			String hql = "delete " + WOMTaskBeforeProcess.JPA_NAME + " where id in(:ids)";
			Query query = taskBeforeProcessDao.createQuery(hql);
			query.setParameterList("ids", taskBeforeProcessIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMTaskBeforeProcess taskBeforeProcessz : taskBeforeProcesss){
				taskBeforeProcessz.setValid(false);
				List<WOMTaskBeforeProcess> params = new ArrayList<WOMTaskBeforeProcess>();
				params.add(taskBeforeProcessz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="删除",operType="3")
	public void deleteTaskBeforeProcess(long taskBeforeProcessId, int taskBeforeProcessVersion){
		
		WOMTaskBeforeProcess taskBeforeProcess = getTaskBeforeProcess(taskBeforeProcessId);
		
		beforeDeleteTaskBeforeProcess(taskBeforeProcess);
		taskBeforeProcessDao.delete(taskBeforeProcessId, taskBeforeProcessVersion);
		afterDeleteTaskBeforeProcess(taskBeforeProcess);
		List<WOMTaskBeforeProcess> params = new ArrayList<WOMTaskBeforeProcess>();
		params.add(taskBeforeProcess);
		Map<String, Object> props = new HashMap<String, Object>();
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="删除",operType="3")
	public void deleteTaskBeforeProcess(String taskBeforeProcessIds){
		deleteCollection(taskBeforeProcessIds);
	}
	...
	private void deleteCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteTaskBeforeProcess(Long.valueOf(id), Integer.valueOf(version));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="批量导入",operType="4")
	public  void batchImportBaseTaskBeforeProcess(List<WOMTaskBeforeProcess>  taskBeforeProcesss){
		for(WOMTaskBeforeProcess taskBeforeProcess:taskBeforeProcesss)  {
			saveTaskBeforeProcess(taskBeforeProcess, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseTaskBeforeProcess(List<WOMTaskBeforeProcess>  taskBeforeProcesss){
		for(WOMTaskBeforeProcess taskBeforeProcess:taskBeforeProcesss)  {
			saveTaskBeforeProcess(taskBeforeProcess, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="保存")
	public void saveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveTaskBeforeProcess(taskBeforeProcess, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="保存")
	public void saveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveTaskBeforeProcess(taskBeforeProcess, dgLists,dgDeleteIDs,assFileUploads, viewCode, null);
	}


	private void generateTaskBeforeProcessCodes(WOMTaskBeforeProcess taskBeforeProcess) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTaskBeforeProcessCodes(taskBeforeProcess, false);
	}
	
	private void generateTaskBeforeProcessCodes(WOMTaskBeforeProcess taskBeforeProcess, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	private void generateTaskBeforeProcessSummarys(WOMTaskBeforeProcess taskBeforeProcess) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTaskBeforeProcessSummarys(taskBeforeProcess, false);
	}

	private void generateTaskBeforeProcessSummarys(WOMTaskBeforeProcess taskBeforeProcess, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess")
	public void saveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(taskBeforeProcess.getId()!=null && taskBeforeProcess.getId()>0){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, taskBeforeProcess);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(taskBeforeProcess);
		beforeSaveTaskBeforeProcess(taskBeforeProcess, viewIsView);

		if (viewIsView) {
			taskBeforeProcessDao.saveWithRevertVersion(taskBeforeProcess);
		} else {
			if(isNew)
					taskBeforeProcessDao.save(taskBeforeProcess);
				else
					taskBeforeProcessDao.merge(taskBeforeProcess);
		}
		dealDatagridsSave(taskBeforeProcess,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateTaskBeforeProcessCodes(taskBeforeProcess, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateTaskBeforeProcessSummarys(taskBeforeProcess, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveTaskBeforeProcess(taskBeforeProcess, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, taskBeforeProcess);
			}
		}
		List<WOMTaskBeforeProcess> params = new ArrayList<WOMTaskBeforeProcess>();
		taskBeforeProcessDao.flush();
		taskBeforeProcessDao.clear();
		taskBeforeProcess = taskBeforeProcessDao.load(taskBeforeProcess.getId());
		
		
		params.add(taskBeforeProcess);
	}
	
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMTaskBeforeProcess taskBeforeProcess){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("taskBeforeProcess", taskBeforeProcess);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess")
	public void mergeTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(taskBeforeProcess);
		beforeSaveTaskBeforeProcess(taskBeforeProcess);
		taskBeforeProcessDao.merge(taskBeforeProcess);

		dealDatagridsSave(taskBeforeProcess,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveTaskBeforeProcess(taskBeforeProcess);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMTaskBeforeProcess> findTaskBeforeProcesss(Page<WOMTaskBeforeProcess> page, Criterion... criterions) {
		return taskBeforeProcessDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskBeforeProcesss(Page<WOMTaskBeforeProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, List<Param> params) {
		findTaskBeforeProcesss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskBeforeProcesss(Page<WOMTaskBeforeProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql) {
		commonQuery(page, viewCode, type, processKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskBeforeProcesss(Page<WOMTaskBeforeProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void commonQuery(Page<WOMTaskBeforeProcess> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"taskBeforeProcess\"" : "\"" + key.substring(0, lastDotPos) + "\"";
				...
				if(type == Sql.TYPE_LIST_REFERENCE){
					if(null != crossCompanyFlag && !crossCompanyFlag){
						s.append(" AND \"taskBeforeProcess\".CID = ").append(getCurrentCompanyId());
					}
				}
...
//				}
			...
			if (hasWhere)
				...
				s.append(" WHERE ");
			s.append(" ( \"taskBeforeProcess\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			List<Object> powerCodeList =  taskBeforeProcessDao.createNativeQuery(powerCodeSql, powerCode).list();
			if(powerCodeList.size() > 0) {
				...
				List<Object> checkList =  taskBeforeProcessDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
				if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"taskBeforeProcess\".LAY_REC ASC, \"taskBeforeProcess\".SORT ASC");
				} else {
					orderPart.append(" \"taskBeforeProcess\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) taskBeforeProcessDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) taskBeforeProcessDao.createNativeQuery(countSql, arr)
			...
			if(!noQueryFlag)  {
				SQLQuery query = taskBeforeProcessDao.createNativeQuery(realSql, arr);
				List<WOMTaskBeforeProcess> result = new ArrayList<WOMTaskBeforeProcess>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMTaskBeforeProcess.class,taskBeforeProcessDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMTaskBeforeProcess.class,taskBeforeProcessDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMTaskBeforeProcess taskBeforeProcess : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(taskBeforeProcess.getId(), "WOM_produceTask_taskBeforeProcess");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(taskBeforeProcess.getId(), "WOM_produceTask_taskBeforeProcess");
							if(null != documents && !documents.isEmpty()) {
								taskBeforeProcess.setDocument(documents.get(0));
								taskBeforeProcess.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMTaskBeforeProcess> result) {
		View view = viewServiceFoundation.getView(viewCode);
		...
		Map<Long, WOMTaskBeforeProcess> rsMap = new HashMap<Long, WOMTaskBeforeProcess>();
		for (WOMTaskBeforeProcess taskBeforeProcess : result) {
			ids.add(taskBeforeProcess.getId());
			rsMap.put(taskBeforeProcess.getId(), taskBeforeProcess);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(taskBeforeProcessDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = taskBeforeProcessDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_TaskBeforeProcess" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_TaskBeforeProcess" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  taskBeforeProcessDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskBeforeProcess\"","WOM_7.5.0.0_produceTask","","WOM_TASK_BEFORE_PROCESSES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"taskBeforeProcess\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"taskBeforeProcess\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskBeforeProcess\"","WOM_7.5.0.0_produceTask","","WOM_TASK_BEFORE_PROCESSES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_TaskBeforeProcess" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  UPPER(\"").append("taskBeforeProcess.mc\"").append(".MNE_CODE) like UPPER(?) escape '&' AND \"taskBeforeProcess\".VALID = 1");
			...
			if(!cross){
				mnecodeSql.append(" AND \"taskBeforeProcess\".CID = ?");
				totalValues.add(getCurrentCompanyId());
			}
			
			SQLQuery query = taskBeforeProcessDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void destroy() throws Exception {
		...
		taskActivePendingDao = null;
		taskActivePendingService = null;
		taskBeforeProcessDao = null;
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=taskBeforeProcessDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMTaskBeforeProcess.ENTITY_CODE;}
	protected String getModuleCode(){return WOMTaskBeforeProcess.MODULE_CODE;}
	protected String getModelCode(){return WOMTaskBeforeProcess.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param taskBeforeProcess 模型实例
	 */
	private void checkUniqueConstraint(WOMTaskBeforeProcess taskBeforeProcess){
	}
	
	
	private void beforeSaveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, Object... objects){
		checkUniqueConstraint(taskBeforeProcess);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void afterSaveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	@Override
	public void beforeExcelImportTaskBeforeProcess(List<WOMTaskBeforeProcess> taskBeforeProcesss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	@Override
	public void afterExcelImportTaskBeforeProcess(List<WOMTaskBeforeProcess> taskBeforeProcesss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	@Override
	public void beforeExcelBatchImportTaskBeforeProcess(List<WOMTaskBeforeProcess> taskBeforeProcesss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */	
	}
	
	@Override
	public void afterExcelBatchImportTaskBeforeProcess(List<WOMTaskBeforeProcess> taskBeforeProcesss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void afterDeleteTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	private void customGenerateCodes(WOMTaskBeforeProcess taskBeforeProcess, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMTaskBeforeProcess taskBeforeProcess, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}

	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportTaskBeforeProcess(List<WOMTaskBeforeProcess> insertObjs, List<WOMTaskBeforeProcess> updateObjs, List<Map<String,String>> columnInfo,List<Map<String, Map<Integer, Map<Integer, String>>>> errMsgSheet){//主辅模型导入before方法
		/* CUSTOM CODE START(serviceimpl,beforeImportTaskBeforeProcess,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	@Override
	public void afterImportTaskBeforeProcess(List<WOMTaskBeforeProcess> insertObjs, List<WOMTaskBeforeProcess> updateObjs, List<Map<String,String>> columnInfo){//主辅模型导入after方法
		/* CUSTOM CODE START(serviceimpl,afterImportTaskBeforeProcess,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	@Override
	public List<WOMTaskBeforeProcess> getTaskBeforeProcesss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskBeforeProcess.class);
		if(params!=null){
			...
			return taskBeforeProcessDao.findByHql("from " + WOMTaskBeforeProcess.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return taskBeforeProcessDao.findByCriteria(detachedCriteria);
	}
	
	@Override
	public WOMTaskBeforeProcess loadTaskBeforeProcessByBussinessKey(WOMTaskBeforeProcess taskBeforeProcess){
		return loadTaskBeforeProcessByBussinessKey(taskBeforeProcess.getId());
	}
	
	@Override
	public Page<WOMTaskBeforeProcess> getTaskBeforeProcesss(Page<WOMTaskBeforeProcess> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskBeforeProcess.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = taskBeforeProcessDao.findByPage(page, sql);
			}else{
				page = taskBeforeProcessDao.findAllByPage(page);
			}
			return page;
		}
		page = taskBeforeProcessDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteTaskBeforeProcessByProduceTaskProcess(List<Long> produceTaskProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("beforeProcessIDList", produceTaskProcessIds);
		params.put("processIdList", produceTaskProcessIds);
		String hql = "select id from WOMTaskBeforeProcess where "
						+ "beforeProcessID.id in (:beforeProcessIDList) or "
						+ "processId.id in (:processIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> taskBeforeProcessIds1 = taskBeforeProcessDao.findByHql(hql, params);
		if(taskBeforeProcessIds1 != null && !taskBeforeProcessIds1.isEmpty()) {
		}
		hql = "delete WOMTaskBeforeProcess where "
						+ "beforeProcessID.id in (:beforeProcessIDList) or "
						+ "processId.id in (:processIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		taskBeforeProcessDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskBeforeProcess loadTaskBeforeProcessByBussinessKey(Serializable bussinessKey){
		return taskBeforeProcessDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMTaskBeforeProcess> findByProperty(String propertyName, Object object){
		return taskBeforeProcessDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskBeforeProcess findEntityByProperty(String propertyName, Object object){
		return taskBeforeProcessDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMTaskBeforeProcess.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = taskBeforeProcessDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_TaskBeforeProcess","runtime");
		try {
	...
	private void dealDatagridsSave(WOMTaskBeforeProcess taskBeforeProcess,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()), _reflect_dgclass,taskBeforeProcessDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMTaskBeforeProcess.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, taskBeforeProcess);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(taskBeforeProcess.getTableInfoId());
							}else{
								((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(taskBeforeProcess.getTableInfoId());
							}
							if(isTree){
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",taskBeforeProcess.getTableInfoId());	
	...
	public List<String> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMTaskBeforeProcess.TABLE_NAME;
			list = taskBeforeProcessDao.createNativeQuery(sql).list();
		}
		return list;
	}
	
	@Autowired
	private WOMTaskBeforeProcessDaoImpl taskBeforeProcessDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMTaskBeforeProcess.TABLE_NAME + " where id = ?";
		String tableNo = taskBeforeProcessDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public Map<Object, Long> importBatchTaskBeforeProcess(final List<WOMTaskBeforeProcess> insertObjs, 
			final List<WOMTaskBeforeProcess> updateObjs, List<Map<String,String>> columnInfo, Map<String, Map<Integer, Long>> tableInfoIds){
		...
		final List<WOMTaskBeforeProcess> nmeObjs = new ArrayList<WOMTaskBeforeProcess>();
		...
		//String idSql = "select id from " + WOMTaskBeforeProcess.TABLE_NAME + " order by id desc";
		//Query query = taskBeforeProcessDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMTaskBeforeProcess.JPA_NAME + " order by id desc";
		Query query =  taskBeforeProcessDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMTaskBeforeProcess.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMTaskBeforeProcess taskBeforeProcess:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMTaskBeforeProcess.TABLE_NAME,1,dbId);
			if(taskBeforeProcess.getId() != null ){
				m.put(taskBeforeProcess.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) taskBeforeProcessDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_TaskBeforeProcess"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		taskBeforeProcessDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMTaskBeforeProcess.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMTaskBeforeProcess taskBeforeProcess = updateObjs.get(updateIndex);
						Long id = taskBeforeProcess.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = taskBeforeProcess.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(taskBeforeProcess);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMTaskBeforeProcess.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMTaskBeforeProcess taskBeforeProcess = insertObjs.get(insertIndex);
						Long id = insertIds.get(insertIndex);
						insertps.setLong(columnMaps.size() + 1, id);
//						insertps.setLong(columnMaps.size() + 1, taskBeforeProcess.getId());
//						insertps.setLong(columnMaps.size() + 1, autoGeneratorID.getNextId(WOMTaskBeforeProcess.TABLE_NAME,1,dbId));
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = taskBeforeProcess.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		taskBeforeProcessDaoImpl.getSession().flush();
		taskBeforeProcessDaoImpl.getSession().clear();
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMTaskBeforeProcess> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMTaskBeforeProcess> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMTaskBeforeProcess> result = getResult(page, query, new PendingResultTransformer(WOMTaskBeforeProcess.class,taskBeforeProcessDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMTaskBeforeProcess model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = taskBeforeProcessDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_TaskBeforeProcess").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = taskBeforeProcessDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = taskBeforeProcessDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = taskBeforeProcessDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?taskBeforeProcessDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  taskBeforeProcessDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,taskBeforeProcessDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="保存")
	public void saveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, DataGridService dataGridService){
		saveTaskBeforeProcess(taskBeforeProcess, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="保存")
	public void saveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, DataGridService dataGridService, String viewCode){
		saveTaskBeforeProcess(taskBeforeProcess, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess")
	public void saveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess")
	public void mergeTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(taskBeforeProcess);
		beforeSaveTaskBeforeProcess(taskBeforeProcess);
		taskBeforeProcessDao.merge(taskBeforeProcess);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="excel导出",operType="4")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="页面打印",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskBeforeProcess",desc="批量保存",operType="4")
	public void batchSaveTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
	...
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
}
