...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_prepareMaterial_PrepareMaterial,WOM_7.5.0.0) */
// 自定义代码
...
public class WOMPrepareMaterialAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMPrepareMaterial> {
	...
	//导出excel时,主模型内容
	List<WOMPrepareMaterial> queryResults = new ArrayList<WOMPrepareMaterial>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/prepareMaterial/prepareMaterial/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/prepareMaterial/prepareMaterial/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_prepareMaterial_PrepareMaterial" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMPrepareMaterial> prepareMaterials;
	
	public List<WOMPrepareMaterial> getPrepareMaterials() {
	...
	private Page<WOMPrepareMaterial> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			prepareMaterial = new WOMPrepareMaterial();
		} else {
			if(id > 0){
				prepareMaterial = prepareMaterialService.getPrepareMaterial(id, viewCode);
				if(tableInfoId == null && prepareMaterial != null){
					tableInfoId = prepareMaterial.getTableInfoId();
				}
			}
		}
	...
	public Page<WOMPrepareMaterial> getPage() {
		if (null == page) {
			page = new Page<WOMPrepareMaterial>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prepareMaterialService.getPrepareMaterialAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String prepareMaterialReferenceCopy() throws Exception{
			WOMPrepareMaterial oldPrepareMaterial = null;
			if(isEditNew()){
				...
				oldPrepareMaterial = new WOMPrepareMaterial();
			}else{
			//WOMPrepareMaterial prepareMaterial = prepareMaterialService.getPrepareMaterial(this.prepareMaterial.getId());
			this.creatorInfo = creatorService.get(prepareMaterial.getCreateStaffId(), prepareMaterial.getCreatePositionId(), prepareMaterial.getCreateDepartmentId());
			//setPrepareMaterial(prepareMaterial);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(prepareMaterial.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prepareMaterial.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prepareMaterial.getId());
				...
				oldPrepareMaterial = prepareMaterialService.getPrepareMaterial(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldPrepareMaterial.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_prepareMaterial_" + oldPrepareMaterial.getId();
			...
			prepareMaterial = prepareMaterialService.getPrepareMaterial(refId);
			prepareMaterial.setPending(pending);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_prepareMaterial_PrepareMaterial");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMPrepareMaterial.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPrepareMaterial, null);
						Method setValue = WOMPrepareMaterial.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prepareMaterial, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prepareMaterial.setCreateDepartmentId(oldPrepareMaterial.getCreateDepartmentId());
			prepareMaterial.setCreatePositionId(oldPrepareMaterial.getCreatePositionId());
			prepareMaterial.setCreateStaffId(oldPrepareMaterial.getCreateStaffId());
			prepareMaterial.setOwnerDepartmentId(oldPrepareMaterial.getOwnerDepartmentId());
			prepareMaterial.setOwnerPositionId(oldPrepareMaterial.getOwnerPositionId());
			prepareMaterial.setOwnerStaffId(oldPrepareMaterial.getOwnerStaffId());
			prepareMaterial.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "referenceCopy");
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prepareBackEdit.ftl") }),
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prepareBackEditPrint.ftl")})
		,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prepareMaterialEdit.ftl") }),
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prepareMaterialEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=prepareMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return prepareMaterialReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			prepareMaterial = prepareMaterialService.getPrepareMaterial(this.prepareMaterial.getId(), viewCode);
			this.creatorInfo = creatorService.get(prepareMaterial.getCreateStaffId(), prepareMaterial.getCreatePositionId(), prepareMaterial.getCreateDepartmentId());
			//setPrepareMaterial(prepareMaterial);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = prepareMaterialService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = prepareMaterialService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + prepareMaterial.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_prepareMaterial_" + prepareMaterial.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<WOMPrepareMaterial> tmpList = new ArrayList<>();
			tmpList.add(prepareMaterial);
			prepareMaterialService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = prepareMaterialService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prepareMaterial.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			prepareMaterialService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prepareBackView.ftl") }),
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prepareBackViewPrint.ftl")})
		,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prepareMaterialView.ftl") }),
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prepareMaterialViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackList", results = { @Result(type = FREEMARKER, location = "prepareBackList.ftl") })
		,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialList", results = { @Result(type = FREEMARKER, location = "prepareMaterialList.ftl") })
	})
	public String viewList() throws Exception{
		...
		//mainViewPath = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialView.action";
		mainViewPath=prepareMaterialService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = prepareMaterialService.getWorkFlowInfo(workFlowMenuCode);
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = prepareMaterialService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(prepareMaterialService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(prepareMaterialService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackRef", results = { @Result(type = FREEMARKER, location = "prepareBackRef.ftl") })
		,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialRef", results = { @Result(type = FREEMARKER, location = "prepareMaterialRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.applyStaff.name,result.applyDate,result.wareID.wareCode,result.wareID.wareName,result.remark,result.backFlag,result.applyStaff.id,result.wareID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="prepareBackList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.applyStaff.name,result.applyDate,result.wareID.wareCode,result.wareID.wareName,result.remark,result.applyStaff.id,result.wareID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="prepareMaterialList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"prepareMaterial\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"prepareMaterial\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			prepareMaterialService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<WOMPrepareMaterial> prepareMaterialList=page.getResult();
				for(WOMPrepareMaterial lz:prepareMaterialList){
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.applyStaff.name,result.applyDate,result.wareID.wareCode,result.wareID.wareName,result.remark,result.backFlag,result.applyStaff.id,result.wareID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="prepareBackList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.applyStaff.name,result.applyDate,result.wareID.wareCode,result.wareID.wareName,result.remark,result.applyStaff.id,result.wareID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"prepareMaterial\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"prepareMaterial\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.manulOrderMain.manulOrderNum,result.manulOrderMain.saleOrderNum,result.effectiveState,result.createTime,result.backFlag,result.manulOrderMain.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="prepareBackRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.manulOrderMain.manulOrderNum,result.manulOrderMain.saleOrderNum,result.effectiveState,result.createTime,result.outCode,result.incode,result.manulOrderMain.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="prepareMaterialRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"prepareMaterial\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	 ...
	 * @return
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/allTempleteDownload", results = { @Result(name="excel",type="excel",location="prepareBackList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_prepareMaterial_PrepareMaterial");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		prepareMaterialService.findDataGridPage(dg,dgclass,getDgPage(),prepareMaterial,"", null);
	...
	private Page<WOMPrepareMaterialPart> dg1566463128930Page;
	public Page<WOMPrepareMaterialPart> getDg1566463128930Page(){
		if(null == dg1566463128930Page){
			dg1566463128930Page = new Page<WOMPrepareMaterialPart>(1,20);
		}
		return dg1566463128930Page;
	}
	
	private Class dg1566463128930Clazz = WOMPrepareMaterialPart.class;
	
	public Class getDg1566463128930Clazz() {
		return dg1566463128930Clazz;
	}
	
	private List<WOMPrepareMaterialPart> dg1566463128930List;
	public List<WOMPrepareMaterialPart> getDg1566463128930List(){
		if(null == dg1566463128930List){
			dg1566463128930List = new ArrayList<WOMPrepareMaterialPart>();
		}
		return dg1566463128930List;
	}
	public void setDg1566463128930ListJson(String json) {
		//dg1566463128930List = (List<WOMPrepareMaterialPart>) prepareMaterialService.generateObjectFromJson(json, WOMPrepareMaterialPart.class);
	}
	
	private List<Long> dg1566463128930DeletedIds;
	public List<Long> getDg1566463128930DeletedIds(){
		if(null == dg1566463128930DeletedIds){
			dg1566463128930DeletedIds = new ArrayList<Long>();
		}
		return dg1566463128930DeletedIds;
	}

	private Page<WOMPrepareMaterialPart> dg1567400338620Page;
	public Page<WOMPrepareMaterialPart> getDg1567400338620Page(){
		if(null == dg1567400338620Page){
			dg1567400338620Page = new Page<WOMPrepareMaterialPart>(1,20);
		}
		return dg1567400338620Page;
	}
	
	private Class dg1567400338620Clazz = WOMPrepareMaterialPart.class;
	
	public Class getDg1567400338620Clazz() {
		return dg1567400338620Clazz;
	}
	
	private List<WOMPrepareMaterialPart> dg1567400338620List;
	public List<WOMPrepareMaterialPart> getDg1567400338620List(){
		if(null == dg1567400338620List){
			dg1567400338620List = new ArrayList<WOMPrepareMaterialPart>();
		}
		return dg1567400338620List;
	}
	public void setDg1567400338620ListJson(String json) {
		//dg1567400338620List = (List<WOMPrepareMaterialPart>) prepareMaterialService.generateObjectFromJson(json, WOMPrepareMaterialPart.class);
	}
	
	private List<Long> dg1567400338620DeletedIds;
	public List<Long> getDg1567400338620DeletedIds(){
		if(null == dg1567400338620DeletedIds){
			dg1567400338620DeletedIds = new ArrayList<Long>();
		}
		return dg1567400338620DeletedIds;
	}

	private Page<WOMPrepareMaterialPart> dg1567404925228Page;
	public Page<WOMPrepareMaterialPart> getDg1567404925228Page(){
		if(null == dg1567404925228Page){
			dg1567404925228Page = new Page<WOMPrepareMaterialPart>(1,20);
		}
		return dg1567404925228Page;
	}
	
	private Class dg1567404925228Clazz = WOMPrepareMaterialPart.class;
	
	public Class getDg1567404925228Clazz() {
		return dg1567404925228Clazz;
	}
	
	private List<WOMPrepareMaterialPart> dg1567404925228List;
	public List<WOMPrepareMaterialPart> getDg1567404925228List(){
		if(null == dg1567404925228List){
			dg1567404925228List = new ArrayList<WOMPrepareMaterialPart>();
		}
		return dg1567404925228List;
	}
	public void setDg1567404925228ListJson(String json) {
		//dg1567404925228List = (List<WOMPrepareMaterialPart>) prepareMaterialService.generateObjectFromJson(json, WOMPrepareMaterialPart.class);
	}
	
	private List<Long> dg1567404925228DeletedIds;
	public List<Long> getDg1567404925228DeletedIds(){
		if(null == dg1567404925228DeletedIds){
			dg1567404925228DeletedIds = new ArrayList<Long>();
		}
		return dg1567404925228DeletedIds;
	}

	private Page<WOMPrepareMaterialPart> dg1489977909385Page;
	public Page<WOMPrepareMaterialPart> getDg1489977909385Page(){
		if(null == dg1489977909385Page){
			dg1489977909385Page = new Page<WOMPrepareMaterialPart>(1,20);
		}
		return dg1489977909385Page;
	}
	
	private Class dg1489977909385Clazz = WOMPrepareMaterialPart.class;
	
	public Class getDg1489977909385Clazz() {
		return dg1489977909385Clazz;
	}
	
	private List<WOMPrepareMaterialPart> dg1489977909385List;
	public List<WOMPrepareMaterialPart> getDg1489977909385List(){
		if(null == dg1489977909385List){
			dg1489977909385List = new ArrayList<WOMPrepareMaterialPart>();
		}
		return dg1489977909385List;
	}
	public void setDg1489977909385ListJson(String json) {
		//dg1489977909385List = (List<WOMPrepareMaterialPart>) prepareMaterialService.generateObjectFromJson(json, WOMPrepareMaterialPart.class);
	}
	
	private List<Long> dg1489977909385DeletedIds;
	public List<Long> getDg1489977909385DeletedIds(){
		if(null == dg1489977909385DeletedIds){
			dg1489977909385DeletedIds = new ArrayList<Long>();
		}
		return dg1489977909385DeletedIds;
	}

	private Page<WOMPrepareMaterialPart> dg1489979054472Page;
	public Page<WOMPrepareMaterialPart> getDg1489979054472Page(){
		if(null == dg1489979054472Page){
			dg1489979054472Page = new Page<WOMPrepareMaterialPart>(1,20);
		}
		return dg1489979054472Page;
	}
	
	private Class dg1489979054472Clazz = WOMPrepareMaterialPart.class;
	
	public Class getDg1489979054472Clazz() {
		return dg1489979054472Clazz;
	}
	
	private List<WOMPrepareMaterialPart> dg1489979054472List;
	public List<WOMPrepareMaterialPart> getDg1489979054472List(){
		if(null == dg1489979054472List){
			dg1489979054472List = new ArrayList<WOMPrepareMaterialPart>();
		}
		return dg1489979054472List;
	}
	public void setDg1489979054472ListJson(String json) {
		//dg1489979054472List = (List<WOMPrepareMaterialPart>) prepareMaterialService.generateObjectFromJson(json, WOMPrepareMaterialPart.class);
	}
	
	private List<Long> dg1489979054472DeletedIds;
	public List<Long> getDg1489979054472DeletedIds(){
		if(null == dg1489979054472DeletedIds){
			dg1489979054472DeletedIds = new ArrayList<Long>();
		}
		return dg1489979054472DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/data-dg1566463128930", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.useNum,result.hasDoneNum,result.ware.wareCode,result.batchNum,result.remark,result.id,result.version,result.useNeedRef.id,result.manuOrder.id,result.unit.id,result.formulaId.id,result.productId.id,result.storeID.id,result.ware.id,result.wareId.id,result.batchNumObj.id" , "dataGridCode", "WOM_7.5.0.0_prepareMaterial_prepareBackEditdg1566463128930" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1566463128930-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1566463128930.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/data-dg1567400338620", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.useNum,result.batchNum,result.hasDoneNum,result.ware.wareCode,result.remark,result.id,result.version,result.useNeedRef.id,result.manuOrder.id,result.unit.id,result.formulaId.id,result.productId.id,result.storeID.id,result.ware.id,result.wareId.id,result.batchNumObj.id" , "dataGridCode", "WOM_7.5.0.0_prepareMaterial_prepareBackViewdg1567400338620" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1567400338620-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1567400338620.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/data-dg1567404925228", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.useNum,result.hasDoneNum,result.ware.wareCode,result.remark,result.batchNum,result.id,result.version,result.useNeedRef.id,result.manuOrder.id,result.unit.id,result.formulaId.id,result.productId.id,result.storeID.id,result.ware.id,result.wareId.id,result.batchNumObj.id" , "dataGridCode", "WOM_7.5.0.0_prepareMaterial_prepareBackViewdg1567404925228" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1567404925228-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1567404925228.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/data-dg1489977909385", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.hasDoneNum,result.useNeedRef.batchText,result.useNum,result.productId.productBaseUnit.name,result.batchNum,result.materialBatchNum,result.wareCode,result.ware.wareCode,result.remark,result.edition,result.manuOrder.materialBatchNum,result.storeID.placeSetName,result.storeID.placeSetCode,result.id,result.version,result.useNeedRef.id,result.manuOrder.id,result.unit.id,result.formulaId.id,result.productId.id,result.storeID.id,result.ware.id,result.wareId.id,result.batchNumObj.id" , "dataGridCode", "WOM_7.5.0.0_prepareMaterial_prepareMaterialEditdg1489977909385" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1489977909385-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1489977909385.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/data-dg1489979054472", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.batchNum,result.ware.wareCode,result.useNum,result.hasDoneNum,result.productId.productBaseUnit.name,result.remark,result.prepareNum,result.WOM_7_5_0_0_prepareMaterial_prepareMaterialViewdg1489979054472_DATAGRID_CUSTOM_7bba6976_adda_4cb0_ad2c_1218e7830270,result.storeID.placeSetName,result.storeID.placeSetCode,result.id,result.version,result.useNeedRef.id,result.manuOrder.id,result.unit.id,result.formulaId.id,result.productId.id,result.storeID.id,result.ware.id,result.wareId.id,result.batchNumObj.id" , "dataGridCode", "WOM_7.5.0.0_prepareMaterial_prepareMaterialViewdg1489979054472" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1489979054472-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1489979054472.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(prepareMaterial.getId() != null && prepareMaterial.getId() != -1){
				prepareMaterial = prepareMaterialService.getPrepareMaterial(prepareMaterial.getId());
			}
			prepareMaterialService.findDataGridPage(dg,dgclass,getDgPage(),prepareMaterial,searchCondition,params);
	...
	private void save() throws Exception {
		...
		prepareMaterialService.save(prepareMaterial,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(prepareMaterial.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + prepareMaterial.getProcessKey() + "_" + p.getActivityName() + "_WOM_7.5.0.0_prepareMaterial_" + prepareMaterial.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(prepareMaterial.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prepareMaterialService.retrialThisTable(prepareMaterial,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "retrial");
		return SUCCESS;
	}
	@Actions({
		@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareBackView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "submit");
		bap_validate_datagrids=prepareMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (prepareMaterial != null && prepareMaterial.getStatus() != null && prepareMaterial.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				prepareMaterialService.saveSuperEditPrepareMaterial(prepareMaterial, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				prepareMaterialService.submit(prepareMaterial,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", prepareMaterial.getTableInfoId());
		responseMap.put("id", prepareMaterial.getId());
		...
		fileuploads.put("linkId", prepareMaterial.getTableInfoId());
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "bulkSubmit");
		prepareMaterialService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/WOM/prepareMaterial/prepareMaterial/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prepareMaterialService.deletePrepareMaterial(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "restore");
		prepareMaterialService.restorePrepareMaterial(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/prepareMaterial/prepareMaterial/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/prepareMaterial/prepareMaterial/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "showDealInfoData");
		if (null != tableInfoId) {
			int count=prepareMaterialService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = prepareMaterialService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = prepareMaterialService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMPrepareMaterial> findExportDataInfos(Page<WOMPrepareMaterial> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = prepareMaterialService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = prepareMaterialService.getAssProperty(p.getCode());	
									//String assModelCode = prepareMaterialService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			prepareMaterialService.findPrepareMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			prepareMaterialService.findPrepareMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMPrepareMaterial> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMPrepareMaterial result : results){
	...
	public boolean isEditNew(){
		//return null == this.prepareMaterial || null == this.prepareMaterial.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && prepareMaterial != null && prepareMaterial.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + prepareMaterial.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return prepareMaterialService.isRecallAble(tableInfoId);
	}

	@Action(value = "/WOM/prepareMaterial/prepareMaterial/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = prepareMaterialService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + prepareMaterial.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + prepareMaterial.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/prepareMaterial/prepareMaterial/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "batchApproval");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/prepareMaterial/prepareMaterial/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/prepareMaterial/prepareMaterial/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMPrepareMaterialService"
						, "com.supcon.orchid.WOM.services.WOMPrepareMaterialPartService"
						...
						, "com.supcon.orchid.WOM.services.WOMManulOrderMaterialService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_prepareMaterial", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_prepareMaterial");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_prepareMaterial_PrepareMaterial", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_prepareMaterial_PrepareMaterial", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_prepareMaterial_PrepareMaterial";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_prepareMaterial_PrepareMaterial", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == prepareMaterial.getTableNo() ? "" : prepareMaterial.getTableNo().toString());
		_auditLog.setColumnStringA(null == prepareMaterial.getId() ? "" : prepareMaterial.getId().toString());
		_auditLog.setColumnStringC(prepareMaterial.getId().toString());
		_auditLog.setColumnStringD(WOMPrepareMaterial.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_prepareMaterial");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMPrepareMaterialAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, prepareMaterial,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, prepareMaterial,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,prepareMaterial);
		...
		printlog.setEntity("WOM_7.5.0.0_prepareMaterial");
		...
		printlog.setBusinessId(prepareMaterial.getId());
		printlog.setBussinessKey(null == prepareMaterial.getTableNo() ? "" : prepareMaterial.getTableNo().toString());
		printlog.setMainDisplay(null == prepareMaterial.getId() ? "" : prepareMaterial.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(prepareMaterial,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(prepareMaterialService.getPrepareMaterial(Long.parseLong(arrId[i])));
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, prepareMaterial,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				prepareMaterial = prepareMaterialService.getPrepareMaterial(Long.parseLong(arrId[i]));
				if(null != prepareMaterial){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == prepareMaterial.getId() ? "" : "," + prepareMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == prepareMaterial.getId() ? "" : prepareMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == prepareMaterial.getTableNo() ? "" : "," + prepareMaterial.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == prepareMaterial.getTableNo() ? "" : prepareMaterial.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == prepareMaterial.getId() ? "" : "," + prepareMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == prepareMaterial.getId() ? "" : prepareMaterial.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, prepareMaterial,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,prepareMaterial);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//prepareMaterialService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMPrepareMaterial> findExportPageInfos(Page<WOMPrepareMaterial> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_prepareMaterial" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = prepareMaterialService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMPrepareMaterial queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = prepareMaterialService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = prepareMaterialService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_prepareMaterial_PrepareMaterial");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_prepareMaterial_PrepareMaterial")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_prepareMaterial_PrepareMaterial");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_prepareMaterial_PrepareMaterial");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = prepareMaterialService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_prepareMaterial_PrepareMaterial,WOM_7.5.0.0) */
// 自定义代码
		if("WOM_7.5.0.0_prepareMaterial_PrepareMaterial".equals(code) && "submit".equals(methodName)){
			//只在编辑界面起作用
			if(dgLists!=null && dgLists.get("dg1489977909385")!=null && dgLists.get("dg1489977909385").length()>0){
				dg1489977909385List = (List<WOMPrepareMaterialPart>) prepareMaterialService.generateObjectFromJson(dgLists.get("dg1489977909385"), WOMPrepareMaterialPart.class);
			}

		}
		if("WOM_7.5.0.0_prepareMaterial_PrepareMaterial".equals(code) && "prepareMaterialRef".equals(methodName)){
			condition = "manulOrderNum=" + manulOrderNum;
		}
/* CUSTOM CODE END */
	}
	
	private void bapAfterCustomMethod(String code, String methodName) throws Exception {
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_prepareMaterial_PrepareMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_prepareMaterial_PrepareMaterial,WOM_7.5.0.0) */
// 自定义代码
	private String manulOrderNum;
	...
	private String conditionRef;//数量*配方#数量*配方#。。。
	private String idList;//备料明细idList
	...
	//根据参照刷新表体数据
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/callback-data-dg1489977909385", results = {
			@Result(type = JSON, params = { "root", "dg1489977909385Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.ware.wareCode,result.batchNum,result.useNum,result.WOM_7_5_0_0_prepareMaterial_prepareMaterialEditdg1489977909385_DATAGRID_CUSTOM_2b480368_134e_4bc8_862c_1acfdc51e4c2,result.version,result.ware.id,result.productId.id" , "dataGridCode", "WOM_7.5.0.0_prepareMaterial_prepareMaterialEditdg1489977909385" })})
	public String callbackdataGridDg1489977909385() throws Exception{
		List<WOMPrepareMaterialPart> partList= new ArrayList<WOMPrepareMaterialPart>();
		//将字符串根据#拆分，得到n组数据
		String[] conditionList = conditionRef.split("#");
		//遍历，将数据*拆分，前半段为数量，后半段为配方id
		for (String conditions : conditionList) {
			String[] conList = conditions.split("[*]");
			if(conList!=null && conList.length==2){
				String num = conList[0];//数量
				String formulaID = conList[1];//配方id
				//根据数量，配方id，已有的list返回新登list
				partList = prepareMaterialService.getPart(num,formulaID,partList);
			}
		}
		if(dg1489977909385Page==null) dg1489977909385Page = getDg1489977909385Page();
		dg1489977909385Page.setResult(partList);
		return SUCCESS;
	}
	private String head;
	...
	@Actions({
			@Action(value = "/WOM/procFeedback/procFeedbackHead/getMaterial", results = {
					@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.useNeedRef.batchText,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.useNum,result.hasDoneNum,result.batchNum,result.ware.wareCode,result.remark,result.id,result.version,result.useNeedRef.id,result.manuOrder.id,result.unit.id,result.formulaId.id,result.productId.id,result.storeID.id,result.ware.id,result.wareId.id,result.batchNumObj.id" , "dataGridCode", "WOM_7.5.0.0_prepareMaterial_prepareBackEditdg1566463128930" }),
					@Result(name="xml",type = "freemarkerXml",location="data-dg1566463128930-xml.ftl"),
					@Result(name="excel",type="excel",location="export-dg1566463128930.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String callbackTASKdataGridDg1566463128930() throws Exception{
		List<WOMPrepareMaterialPart> partList= new ArrayList<WOMPrepareMaterialPart>();
		partList = prepareMaterialService.getPrepareBackPart(head, partList);
		if(dg1566463128930Page==null) dg1566463128930Page = getDg1566463128930Page();
		dg1566463128930Page.setResult(partList);
		return SUCCESS;
	...
	//根据指令单参照刷新表体数据
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/callbackTASK-data-dg1489977909385", results = {
			@Result(type = JSON, params = { "root", "dg1489977909385Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.productId.productCode,result.productId.productName,result.prepareNum,result.hasDoneNum,result.productId.productBaseUnit.name,result.ware.wareCode,result.batchNum,result.useNum,result.WOM_7_5_0_0_prepareMaterial_prepareMaterialEditdg1489977909385_DATAGRID_CUSTOM_2b480368_134e_4bc8_862c_1acfdc51e4c2,result.version,result.ware.id,result.productId.id,result.edition", "dataGridCode", "WOM_7.5.0.0_prepareMaterial_prepareMaterialEditdg1489977909385" })})
	public String callbackTASKdataGridDg1489977909385() throws Exception{
		List<WOMPrepareMaterialPart> partList= new ArrayList<WOMPrepareMaterialPart>();
		//将字符串根据#拆分，得到n组数据
		String[] conditionList = conditionRef.split("#");
		//遍历，得到每个指令单id
		for (String taskID : conditionList) {
			//根据指令单id，已有的list返回新登list
			partList = prepareMaterialService.getTaskPart(taskID,partList);
			//partList = prepareMaterialService.getorderPart(taskID,partList);
		}
		if(dg1489977909385Page==null) dg1489977909385Page = getDg1489977909385Page();
		dg1489977909385Page.setResult(partList);
		return SUCCESS;
	 ...
	 * @return
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/toAppropriation", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag,sampleId,pendingID,tableID" }) })
	public String toAppropriation() throws Exception{
		String[] partidList=idList.split("#");
		for(String partID:partidList){
			WOMPrepareMaterialPart womPrepareMaterialPart=prepareMaterialService.findPrepareMaterialPart(Long.parseLong(partID)).get(0);//备料明细
			WOMPrepareMaterial 	PrepareMaterial=womPrepareMaterialPart.getFormulaId();
			String srcId=PrepareMaterial.getId().toString();//调拨单上游单据id
			List<Object> status=prepareMaterialService.findstatusfromAppropriation(srcId);
			for(int i=0;i<status.size();i++){
				if(status.get(i).toString().equals("88")){
					//若有未生效且未作废的数据被调拨单引用，不允许下推并进行提示；
					throw new BAPException("WOM.prepareMaterialtoAppropriationExceptiondbdexe123");
				}
			}
			if(PrepareMaterial.getStatus()!=99){
				//选择的备料明细对应的备料单未生效，不允许下推!
				throw new BAPException("WOM.prepareMaterialtoAppropriationException123456");
			}else if(womPrepareMaterialPart.getHasDoneNum() != null){
				BigDecimal num=womPrepareMaterialPart.getHasDoneNum().subtract(womPrepareMaterialPart.getPrepareNum());//已备数量-申请数量
				if(num.compareTo(BigDecimal.valueOf(0))==1||num.compareTo(BigDecimal.valueOf(0))==0){//已备数量-申请数量 ≥0不允许下推
					//有已备数量大于等于申请数量的数据，不允许下推!
					throw new BAPException("WOM.prepareMaterialtoAppropriationException123456111447");
				}
			}
		}
		prepareMaterialService.downToAppropriation(partidList);
		responseMap.put("dealSuccessFlag", true);
		return SUCCESS;
	}
	 ...
	 * @return
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/prepareMaterialOutgo", results = { @Result(type = JSON, params = { "root", "responseMap"}) })
	public String prepareMaterialOutgo() throws Exception{
		log.info("prepareMaterialOutgo - id: {}", testId);
		Map<String, String> result = womMesToSapService.prepareMaterialOutgo(testId);
	 ...
	 * @return
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/saveProduct", results = { @Result(type = JSON, params = { "root", "responseMap"}) })
	public String saveProduct() throws Exception{
		log.info("prepareMaterialOutgo - {}", testId);
		Map<String, String> result = womMesToSapService.saveProduct(testId);
		responseMap.putAll(result);
		return SUCCESS;
	}
	 ...
	 * @return
	 */
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/scheduleInventory", results = { @Result(type = JSON, params = { "root", "responseMap"}) })
	public String scheduleInventory() throws Exception{
		womMesToSapService.scheduleInventory();
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/prepareMaterial/prepareMaterial/sapSwitch", results = { @Result(type = JSON, params = { "root", "responseMap"}) })
	public String checkSapSwitch() throws Exception{
        Boolean flag = consulService.getValueAsBoolean("platform/bap/WOM/WOM.isSynSAP");
        responseMap.put("sapSwitch", flag);
		return SUCCESS;
	}
/* CUSTOM CODE END */
}