...
import com.supcon.orchid.WOM.services.WOMInventoryDetailImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMInventoryDetailDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_inventoryDetailService")
@Transactional
public class WOMInventoryDetailServiceImpl  extends BaseServiceImpl<WOMInventoryDetail> implements WOMInventoryDetailService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMInventoryDetail> bapBaseService;
	...
	@Autowired
	private WOMInventoryDetailImportService inventoryDetailImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMInventoryDetail.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  inventoryDetailDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMInventoryDetail.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = inventoryDetailDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMInventoryDetail.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMInventoryDetail.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  inventoryDetailDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMInventoryDetail.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMInventoryDetail.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMInventoryDetail> list = (List<WOMInventoryDetail>) inventoryDetailDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInventoryDetail getInventoryDetail(long id){
		return getInventoryDetail(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInventoryDetail getInventoryDetail(long id, String viewCode){
		WOMInventoryDetail inventoryDetail = inventoryDetailDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getInventoryDetailAsJSON(long id, String include){
		WOMInventoryDetail inventoryDetail = inventoryDetailDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(inventoryDetail, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.common.delete",operType="3")
	public void deleteInventoryDetail(WOMInventoryDetail inventoryDetail){
		if(null != inventoryDetail && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryDetail.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
			AuditUtil.setColumnStringA(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
			AuditUtil.setColumnStringC(inventoryDetail.getId().toString());
		}
		beforeDeleteInventoryDetail(inventoryDetail);
		inventoryDetailDao.deletePhysical(inventoryDetail);
		afterDeleteInventoryDetail(inventoryDetail);
		inventoryDetailDao.flush();
		List<WOMInventoryDetail> params = new ArrayList<WOMInventoryDetail>();
		params.add(inventoryDetail);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.common.delete",operType="3")
	public void deleteInventoryDetail(List<Long> inventoryDetailIds){
		deleteInventoryDetail(inventoryDetailIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.common.delete",operType="3")
	public void deleteInventoryDetail(List<Long> inventoryDetailIds, String eventTopic) {
		List<WOMInventoryDetail> inventoryDetails = new ArrayList<WOMInventoryDetail>();
		for(Long inventoryDetailId : inventoryDetailIds){
			WOMInventoryDetail inventoryDetail = getInventoryDetail(inventoryDetailId);
			inventoryDetails.add(inventoryDetail);
			if(null != inventoryDetail && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryDetail.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == inventoryDetail.getId() ? "" : "," + inventoryDetail.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == inventoryDetail.getId() ? "" : "," + inventoryDetail.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == inventoryDetail.getId() ? "" : "," + inventoryDetail.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
				}
			}
		}
		...
		if(inventoryDetails != null && inventoryDetails.size() > 0 ){
			for(WOMInventoryDetail inventoryDetail : inventoryDetails){
				...
				java.lang.reflect.Field[] fields = WOMInventoryDetail.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(inventoryDetail));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),inventoryDetail, inventoryDetail.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(inventoryDetailIds != null && inventoryDetailIds.size() > 0) {
			String hql = "delete " + WOMInventoryDetail.JPA_NAME + " where id in(:ids)";
			Query query = inventoryDetailDao.createQuery(hql);
			query.setParameterList("ids", inventoryDetailIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMInventoryDetail inventoryDetailz : inventoryDetails){
				inventoryDetailz.setValid(false);
				List<WOMInventoryDetail> params = new ArrayList<WOMInventoryDetail>();
				params.add(inventoryDetailz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_inventoryRecords");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.common.delete",operType="3")
	public void deleteInventoryDetail(long inventoryDetailId, int inventoryDetailVersion){
		this.deleteInventoryDetail(inventoryDetailId, inventoryDetailVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.common.delete",operType="3")
	public void deleteInventoryDetail(long inventoryDetailId, int inventoryDetailVersion,SignatureLog signatureLog){
		WOMInventoryDetail inventoryDetail = getInventoryDetail(inventoryDetailId);
		if(inventoryDetail != null && !inventoryDetail.isValid()){
		...
		beforeDeleteInventoryDetail(inventoryDetail);
		inventoryDetailDao.delete(inventoryDetailId, inventoryDetailVersion);
		afterDeleteInventoryDetail(inventoryDetail);
		inventoryDetailDao.flush();
		List<WOMInventoryDetail> params = new ArrayList<WOMInventoryDetail>();
		params.add(inventoryDetail);
		...
		if(signatureLog != null ) {
			...
			businessKey=inventoryDetail.getId();
			signatureLog.setTableId(inventoryDetail.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.common.delete",operType="3")
	public void deleteInventoryDetail(String inventoryDetailIds){
		this.deleteInventoryDetail(inventoryDetailIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.common.delete",operType="3")
	public void deleteInventoryDetail(String inventoryDetailIds,SignatureLog signatureLog){
		deleteCollection(inventoryDetailIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteInventoryDetail(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_inventoryRecords#modelCode:WOM_7.5.0.0_inventoryRecords_InventoryDetail#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="ec.engine.restore",operType="10")
	public void restoreInventoryDetail(String inventoryDetailIds){
		restoreCollection(inventoryDetailIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreInventoryDetail(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="ec.engine.restore",operType="10")
	public void restoreInventoryDetail(long inventoryDetailId){
		
		findBusinessKeyUsed(inventoryDetailId);	//判断业务主键是否重复
		...
		beforeRestoreInventoryDetail(inventoryDetail);
		inventoryDetail.setValid(true);
		inventoryDetailDao.update(inventoryDetail);
		afterRestoreInventoryDetail(inventoryDetail);
		List<WOMInventoryDetail> params = new ArrayList<WOMInventoryDetail>();
		params.add(inventoryDetail);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long inventoryDetailId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_inventoryRecords_InventoryDetail");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMInventoryDetail.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMInventoryDetail.TABLE_NAME +" where id = ? )";
			List<Object> list =  inventoryDetailDao.createNativeQuery(sql,inventoryDetailId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseInventoryDetail(List<WOMInventoryDetail>  inventoryDetails){
		for(WOMInventoryDetail inventoryDetail:inventoryDetails)  {
			saveInventoryDetail(inventoryDetail, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseInventoryDetail(List<WOMInventoryDetail>  inventoryDetails){
		for(WOMInventoryDetail inventoryDetail:inventoryDetails)  {
			saveInventoryDetail(inventoryDetail, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="ec.flow.save")
	public void saveInventoryDetail(WOMInventoryDetail inventoryDetail, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveInventoryDetail(inventoryDetail, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="ec.flow.save")
	public void saveInventoryDetail(WOMInventoryDetail inventoryDetail, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveInventoryDetail(inventoryDetail, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateInventoryDetailCodes(WOMInventoryDetail inventoryDetail) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateInventoryDetailCodes(inventoryDetail, false);
	}
	
	public void generateInventoryDetailCodes(WOMInventoryDetail inventoryDetail, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateInventoryDetailSummarys(WOMInventoryDetail inventoryDetail) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateInventoryDetailSummarys(inventoryDetail, false);
	}

	public void generateInventoryDetailSummarys(WOMInventoryDetail inventoryDetail, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail")
	public void saveInventoryDetail(WOMInventoryDetail inventoryDetail,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveInventoryDetail(inventoryDetail, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail")
	public void saveInventoryDetail(WOMInventoryDetail inventoryDetail,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_inventoryRecords");
		String url = null;
		if(inventoryDetail.getId() != null && inventoryDetail.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryDetail.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, inventoryDetail);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(inventoryDetail);
		beforeSaveInventoryDetail(inventoryDetail, viewIsView);

		if (viewIsView) {
			inventoryDetailDao.saveWithRevertVersion(inventoryDetail);
		} else {
			if(isNew)
				inventoryDetailDao.save(inventoryDetail);
			else
				inventoryDetailDao.merge(inventoryDetail);
		}
		if(null != inventoryDetail && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryDetail.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
			AuditUtil.setColumnStringA(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
		...
		dealDatagridsSave(inventoryDetail,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateInventoryDetailCodes(inventoryDetail, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateInventoryDetailSummarys(inventoryDetail, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveInventoryDetail(inventoryDetail, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, inventoryDetail);
			}
		}
		List<WOMInventoryDetail> params = new ArrayList<WOMInventoryDetail>();
		inventoryDetailDao.flush();
		inventoryDetailDao.clear();
		inventoryDetail = inventoryDetailDao.load(inventoryDetail.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMInventoryDetail inventoryDetail){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("inventoryDetail", inventoryDetail);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail")
	public void mergeInventoryDetail(WOMInventoryDetail inventoryDetail, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(inventoryDetail);
		beforeSaveInventoryDetail(inventoryDetail);
		inventoryDetailDao.merge(inventoryDetail);
		if(null != inventoryDetail && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryDetail.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
			AuditUtil.setColumnStringA(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
		...
		dealDatagridsSave(inventoryDetail,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveInventoryDetail(inventoryDetail);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMInventoryDetail> findInventoryDetails(Page<WOMInventoryDetail> page, Criterion... criterions) {
		return inventoryDetailDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInventoryDetails(Page<WOMInventoryDetail> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findInventoryDetails(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInventoryDetails(Page<WOMInventoryDetail> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInventoryDetails(Page<WOMInventoryDetail> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMInventoryDetail> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"inventoryDetail\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"inventoryDetail\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"inventoryDetail\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"inventoryDetail\".HEAD IN ( SELECT ID FROM WOM_INVENTORY_RECORDS \"inventoryRecord\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_inventoryRecords" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  inventoryDetailDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  inventoryDetailDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_inventoryRecords").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"inventoryRecord\"","WOM_7.5.0.0_inventoryRecords","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"inventoryDetail\".LAY_REC ASC, \"inventoryDetail\".SORT ASC");
				} else {
					orderPart.append(" \"inventoryDetail\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) inventoryDetailDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=inventoryDetailDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = inventoryDetailDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMInventoryDetail> result = new ArrayList<WOMInventoryDetail>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMInventoryDetail.class,inventoryDetailDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMInventoryDetail.class,inventoryDetailDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMInventoryDetail inventoryDetail : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(inventoryDetail.getId(), "WOM_inventoryRecords_inventoryDetail");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(inventoryDetail.getId(), "WOM_inventoryRecords_inventoryDetail");
							if(null != documents && !documents.isEmpty()) {
								inventoryDetail.setDocument(documents.get(0));
								inventoryDetail.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMInventoryDetail> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMInventoryDetail> result) {
		View view = null;
		...
		Map<Long, WOMInventoryDetail> rsMap = new HashMap<Long, WOMInventoryDetail>();
		for (WOMInventoryDetail inventoryDetail : result) {
			ids.add(inventoryDetail.getId());
			rsMap.put(inventoryDetail.getId(), inventoryDetail);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(inventoryDetailDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = inventoryDetailDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_inventoryRecords_InventoryDetail" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_inventoryRecords_InventoryDetail" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  inventoryDetailDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"inventoryDetail\"","WOM_7.5.0.0_inventoryRecords","","WOM_INVENTORY_DETAILS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"inventoryDetail\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"inventoryDetail\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"inventoryDetail\"","WOM_7.5.0.0_inventoryRecords","","WOM_INVENTORY_DETAILS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_inventoryRecords_InventoryDetail" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("inventoryDetail.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"inventoryDetail\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"inventoryDetail\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"inventoryDetail\".CID = 1 OR \"inventoryDetail\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("inventoryDetail.mc\"").append(".INVENTORY_DETAIL) not in ("+notinValue+")");
			}
			
			SQLQuery query = inventoryDetailDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=inventoryDetailDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMInventoryDetail.ENTITY_CODE;}
	protected String getModuleCode(){return WOMInventoryDetail.MODULE_CODE;}
	protected String getModelCode(){return WOMInventoryDetail.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param inventoryDetail 模型实例
	 */
	public void checkUniqueConstraint(WOMInventoryDetail inventoryDetail){
	}
	
	
	@Override
	public List<WOMInventoryDetail> getInventoryDetails(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMInventoryDetail.class);
		if(params!=null){
			...
			return inventoryDetailDao.findByHql("from " + WOMInventoryDetail.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return inventoryDetailDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMInventoryDetail> findInventoryDetailsByHql(String hql, Object... objects){
		...
		return   inventoryDetailDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMInventoryDetail loadInventoryDetailByBussinessKey(WOMInventoryDetail inventoryDetail){
		return loadInventoryDetailByBussinessKey(inventoryDetail.getId());
	}
	
	@Override
	public Page<WOMInventoryDetail> getInventoryDetails(Page<WOMInventoryDetail> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMInventoryDetail.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = inventoryDetailDao.findByPage(page, sql);
			}else{
				page = inventoryDetailDao.findAllByPage(page);
			}
			return page;
		}
		page = inventoryDetailDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMInventoryDetail> findByTreeDataGridInventoryRecord(List<WOMInventoryDetail> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMInventoryDetail inventoryDetail : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridInventoryRecord(Page<WOMInventoryDetail> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMInventoryDetail> inventoryDetails = dgPage.getResult();
			for(WOMInventoryDetail inventoryDetail : inventoryDetails){
				
			}
			dgPage.setResult(inventoryDetails);
		}
	...
	public WOMInventoryDetail loadInventoryDetailByBussinessKey(Serializable bussinessKey){
		return inventoryDetailDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMInventoryDetail> findByProperty(String propertyName, Object object){
		return inventoryDetailDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMInventoryDetail findEntityByProperty(String propertyName, Object object){
		return inventoryDetailDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMInventoryDetail.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = inventoryDetailDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_inventoryRecords_InventoryDetail","runtime");
		try {
	...
	public void dealDatagridsSave(WOMInventoryDetail inventoryDetail,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,inventoryDetailDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMInventoryDetail.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, inventoryDetail);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",inventoryDetail.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMInventoryDetail.TABLE_NAME +" where valid = 1";
			list = inventoryDetailDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMInventoryDetail.TABLE_NAME;
			list = inventoryDetailDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMInventoryDetail.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = inventoryDetailDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMInventoryDetailDaoImpl inventoryDetailDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchInventoryDetail(final List<WOMInventoryDetail> insertObjs, final List<WOMInventoryDetail> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchInventoryDetail(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchInventoryDetail(final List<WOMInventoryDetail> insertObjs, final List<WOMInventoryDetail> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1566888284025"),importCount));
		...
		Map<Long, WOMInventoryDetail> oldUpdateObjsMap = new HashMap<Long, WOMInventoryDetail>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMInventoryDetail bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMInventoryDetail> bizObjList = inventoryDetailDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMInventoryDetail bizObj = bizObjList.get(mIndex);
		...
		final List<WOMInventoryDetail> nmeObjs = new ArrayList<WOMInventoryDetail>();
		...
		long id=autoGeneratorID.getNextId(WOMInventoryDetail.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMInventoryDetail inventoryDetail:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMInventoryDetail.TABLE_NAME,1,dbId);
			if(inventoryDetail.getId() != null ){
				m.put(inventoryDetail.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) inventoryDetailDao.createNativeQuery(sql, "WOM_7.5.0.0_inventoryRecords_InventoryDetail"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		inventoryDetailDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMInventoryDetail.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMInventoryDetail inventoryDetail = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = inventoryDetail.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(inventoryDetail);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMInventoryDetail.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMInventoryDetail.TABLE_NAME + " order by id desc";
//					Query query = inventoryDetailDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMInventoryDetail inventoryDetail = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = inventoryDetail.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		inventoryDetailDaoImpl.getSession().flush();
		inventoryDetailDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMInventoryDetail bizObj = insertObjs.get(insertIndex);
			inventoryDetailImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMInventoryDetail bizObj = updateObjs.get(updateIndex);
			WOMInventoryDetail oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			inventoryDetailImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMInventoryDetail> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMInventoryDetail> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) inventoryDetailDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMInventoryDetail> result = getResult(page, query, new PendingResultTransformer(WOMInventoryDetail.class,inventoryDetailDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMInventoryDetail model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = inventoryDetailDao.createNativeQuery(sql, "WOM_7.5.0.0_inventoryRecords_InventoryDetail").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = inventoryDetailDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = inventoryDetailDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = inventoryDetailDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?inventoryDetailDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  inventoryDetailDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,inventoryDetailDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="ec.flow.save")
	public void saveInventoryDetail(WOMInventoryDetail inventoryDetail, DataGridService dataGridService){
		saveInventoryDetail(inventoryDetail, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="ec.flow.save")
	public void saveInventoryDetail(WOMInventoryDetail inventoryDetail, DataGridService dataGridService, String viewCode){
		saveInventoryDetail(inventoryDetail, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail")
	public void saveInventoryDetail(WOMInventoryDetail inventoryDetail, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveInventoryDetail(inventoryDetail, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail")
	public void saveInventoryDetail(WOMInventoryDetail inventoryDetail, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_inventoryRecords";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_inventoryRecords");
		String url = null;
		if(null != inventoryDetail.getId() && inventoryDetail.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMInventoryDetail.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail")
	public Map<Object, Object> inventoryDetailDataGridImport(WOMInventoryDetail inventoryDetail, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = inventoryDetail.getVirtualId();
		
		inventoryDetail = inventoryDetailDao.load(inventoryDetail.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, inventoryDetail.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = inventoryDetail.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(inventoryDetail);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, inventoryDetail.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail")
	public void mergeInventoryDetail(WOMInventoryDetail inventoryDetail, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(inventoryDetail);
		beforeSaveInventoryDetail(inventoryDetail);
		inventoryDetailDao.merge(inventoryDetail);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_inventoryRecords",model="WOM_7.5.0.0_inventoryRecords_InventoryDetail",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMInventoryDetail> getByPage(Page<WOMInventoryDetail> page,DetachedCriteria detachedCriteria){
		return inventoryDetailDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveInventoryDetail(WOMInventoryDetail inventoryDetail, Object... objects){
		checkUniqueConstraint(inventoryDetail);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveInventoryDetail(WOMInventoryDetail inventoryDetail, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteInventoryDetail(WOMInventoryDetail inventoryDetail){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteInventoryDetail(WOMInventoryDetail inventoryDetail){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreInventoryDetail(WOMInventoryDetail inventoryDetail){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreInventoryDetail(WOMInventoryDetail inventoryDetail){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMInventoryDetail inventoryDetail, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMInventoryDetail inventoryDetail, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportInventoryDetail(List<WOMInventoryDetail> insertObjs, List<WOMInventoryDetail> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportInventoryDetail,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportInventoryDetail(List<WOMInventoryDetail> insertObjs, List<WOMInventoryDetail> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportInventoryDetail,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
