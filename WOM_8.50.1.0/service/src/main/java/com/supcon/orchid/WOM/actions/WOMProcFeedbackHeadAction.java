...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMProcFeedbackHeadAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProcFeedbackHead> {
	...
	private WOMProcFeedbackHead procFeedbackHead;
	...
	//导出excel时,主模型内容
	List<WOMProcFeedbackHead> queryResults = new ArrayList<WOMProcFeedbackHead>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/procFeedback/procFeedbackHead/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/procFeedback/procFeedbackHead/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_procFeedback_ProcFeedbackHead" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProcFeedbackHead> procFeedbackHeads;
	
	public List<WOMProcFeedbackHead> getProcFeedbackHeads() {
		return procFeedbackHeads;
	}
	...
	private Page<WOMProcFeedbackHead> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			procFeedbackHead = new WOMProcFeedbackHead();
		} else {
			if(id > 0){
				procFeedbackHead = procFeedbackHeadService.getProcFeedbackHead(id, viewCode);
			}
		}
	...
	public Page<WOMProcFeedbackHead> getPage() {
		if (null == page) {
			page = new Page<WOMProcFeedbackHead>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProcFeedbackHead getProcFeedbackHead() {
		return procFeedbackHead;
	}		

	public void setProcFeedbackHead(WOMProcFeedbackHead procFeedbackHead) {
		this.procFeedbackHead = procFeedbackHead;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/procFeedback/procFeedbackHead/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = procFeedbackHeadService.getProcFeedbackHeadAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/procFeedback/procFeedbackHead/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String procFeedbackHeadReferenceCopy() throws Exception{
			WOMProcFeedbackHead oldProcFeedbackHead = null;
			if(isEditNew()){
				...
				oldProcFeedbackHead = new WOMProcFeedbackHead();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,procFeedbackHead.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, procFeedbackHead.getId());
				...
				oldProcFeedbackHead = procFeedbackHeadService.getProcFeedbackHead(id);
			}
			
			
			procFeedbackHead = procFeedbackHeadService.getProcFeedbackHead(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_ProcFeedbackHead");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProcFeedbackHead.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProcFeedbackHead, null);
						Method setValue = WOMProcFeedbackHead.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(procFeedbackHead, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			procFeedbackHead.setCreateDepartmentId(oldProcFeedbackHead.getCreateDepartmentId());
			procFeedbackHead.setCreatePositionId(oldProcFeedbackHead.getCreatePositionId());
			procFeedbackHead.setCreateStaffId(oldProcFeedbackHead.getCreateStaffId());
			procFeedbackHead.setOwnerDepartmentId(oldProcFeedbackHead.getOwnerDepartmentId());
			procFeedbackHead.setOwnerPositionId(oldProcFeedbackHead.getOwnerPositionId());
			procFeedbackHead.setOwnerStaffId(oldProcFeedbackHead.getOwnerStaffId());
			procFeedbackHead.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/procFeedbackHead/adjustBatchEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "adjustBatchEdit.ftl") }),
		@Action(value = "/WOM/procFeedback/procFeedbackHead/adjustBatchEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "adjustBatchEditPrint.ftl")})
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "procFeedbackEdit.ftl") }),
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "procFeedbackEditPrint.ftl")})
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedBatchEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "procFeedBatchEdit.ftl") }),
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedBatchEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "procFeedBatchEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=procFeedbackHeadService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return procFeedbackHeadReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			procFeedbackHead = procFeedbackHeadService.getProcFeedbackHead(this.procFeedbackHead.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,procFeedbackHead.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			procFeedbackHeadService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/procFeedbackHead/adjustBatchView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "adjustBatchView.ftl") }),
		@Action(value = "/WOM/procFeedback/procFeedbackHead/adjustBatchViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "adjustBatchViewPrint.ftl")})
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "procFeedbackView.ftl") }),
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "procFeedbackViewPrint.ftl")})
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedBatchView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "procFeedBatchView.ftl") }),
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedBatchViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "procFeedBatchViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackList", results = { @Result(type = FREEMARKER, location = "procFeedbackList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = procFeedbackHeadService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(procFeedbackHeadService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(procFeedbackHeadService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackRef", results = { @Result(type = FREEMARKER, location = "procFeedbackRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.taskId.productBatchNum,result.processActive.processId.name,result.processActive.name,result.processActive.startTime,result.processActive.endTime,result.taskId.id,result.processActive.processId.id,result.processActive.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="procFeedbackList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"procFeedbackHead\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"procFeedbackHead\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"procFeedbackHead\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"procFeedbackHead\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			procFeedbackHeadService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.taskId.productBatchNum,result.processActive.processId.name,result.processActive.name,result.processActive.startTime,result.processActive.endTime,result.taskId.id,result.processActive.processId.id,result.processActive.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="procFeedbackRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/procFeedback/procFeedbackHead/allTempleteDownload", results = { @Result(name="excel",type="excel",location="procFeedbackList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_ProcFeedbackHead");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		procFeedbackHeadService.findDataGridPage(dg,dgclass,getDgPage(),procFeedbackHead,"", null);
	...
	private Page<WOMCraftStandards> dg1542186009188Page;
	public Page<WOMCraftStandards> getDg1542186009188Page(){
		if(null == dg1542186009188Page){
			dg1542186009188Page = new Page<WOMCraftStandards>(1,20);
		}
		return dg1542186009188Page;
	}
	
	private Class dg1542186009188Clazz = WOMCraftStandards.class;
	
	public Class getDg1542186009188Clazz() {
		return dg1542186009188Clazz;
	}
	
	private List<WOMCraftStandards> dg1542186009188List;
	public List<WOMCraftStandards> getDg1542186009188List(){
		if(null == dg1542186009188List){
			dg1542186009188List = new ArrayList<WOMCraftStandards>();
		}
		return dg1542186009188List;
	}
	public void setDg1542186009188ListJson(String json) {
		//dg1542186009188List = (List<WOMCraftStandards>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftStandards.class);
	}
	
	private List<Long> dg1542186009188DeletedIds;
	public List<Long> getDg1542186009188DeletedIds(){
		if(null == dg1542186009188DeletedIds){
			dg1542186009188DeletedIds = new ArrayList<Long>();
		}
		return dg1542186009188DeletedIds;
	}

	private Page<WOMCraftRecords> dg1542186009283Page;
	public Page<WOMCraftRecords> getDg1542186009283Page(){
		if(null == dg1542186009283Page){
			dg1542186009283Page = new Page<WOMCraftRecords>(1,20);
		}
		return dg1542186009283Page;
	}
	
	private Class dg1542186009283Clazz = WOMCraftRecords.class;
	
	public Class getDg1542186009283Clazz() {
		return dg1542186009283Clazz;
	}
	
	private List<WOMCraftRecords> dg1542186009283List;
	public List<WOMCraftRecords> getDg1542186009283List(){
		if(null == dg1542186009283List){
			dg1542186009283List = new ArrayList<WOMCraftRecords>();
		}
		return dg1542186009283List;
	}
	public void setDg1542186009283ListJson(String json) {
		//dg1542186009283List = (List<WOMCraftRecords>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftRecords.class);
	}
	
	private List<Long> dg1542186009283DeletedIds;
	public List<Long> getDg1542186009283DeletedIds(){
		if(null == dg1542186009283DeletedIds){
			dg1542186009283DeletedIds = new ArrayList<Long>();
		}
		return dg1542186009283DeletedIds;
	}

	private Page<WOMBatchCharge> dg1542186009357Page;
	public Page<WOMBatchCharge> getDg1542186009357Page(){
		if(null == dg1542186009357Page){
			dg1542186009357Page = new Page<WOMBatchCharge>(1,20);
		}
		return dg1542186009357Page;
	}
	
	private Class dg1542186009357Clazz = WOMBatchCharge.class;
	
	public Class getDg1542186009357Clazz() {
		return dg1542186009357Clazz;
	}
	
	private List<WOMBatchCharge> dg1542186009357List;
	public List<WOMBatchCharge> getDg1542186009357List(){
		if(null == dg1542186009357List){
			dg1542186009357List = new ArrayList<WOMBatchCharge>();
		}
		return dg1542186009357List;
	}
	public void setDg1542186009357ListJson(String json) {
		//dg1542186009357List = (List<WOMBatchCharge>) procFeedbackHeadService.generateObjectFromJson(json, WOMBatchCharge.class);
	}
	
	private List<Long> dg1542186009357DeletedIds;
	public List<Long> getDg1542186009357DeletedIds(){
		if(null == dg1542186009357DeletedIds){
			dg1542186009357DeletedIds = new ArrayList<Long>();
		}
		return dg1542186009357DeletedIds;
	}

	private Page<WOMCraftStandards> dg1542248606162Page;
	public Page<WOMCraftStandards> getDg1542248606162Page(){
		if(null == dg1542248606162Page){
			dg1542248606162Page = new Page<WOMCraftStandards>(1,20);
		}
		return dg1542248606162Page;
	}
	
	private Class dg1542248606162Clazz = WOMCraftStandards.class;
	
	public Class getDg1542248606162Clazz() {
		return dg1542248606162Clazz;
	}
	
	private List<WOMCraftStandards> dg1542248606162List;
	public List<WOMCraftStandards> getDg1542248606162List(){
		if(null == dg1542248606162List){
			dg1542248606162List = new ArrayList<WOMCraftStandards>();
		}
		return dg1542248606162List;
	}
	public void setDg1542248606162ListJson(String json) {
		//dg1542248606162List = (List<WOMCraftStandards>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftStandards.class);
	}
	
	private List<Long> dg1542248606162DeletedIds;
	public List<Long> getDg1542248606162DeletedIds(){
		if(null == dg1542248606162DeletedIds){
			dg1542248606162DeletedIds = new ArrayList<Long>();
		}
		return dg1542248606162DeletedIds;
	}

	private Page<WOMCraftRecords> dg1542248606200Page;
	public Page<WOMCraftRecords> getDg1542248606200Page(){
		if(null == dg1542248606200Page){
			dg1542248606200Page = new Page<WOMCraftRecords>(1,20);
		}
		return dg1542248606200Page;
	}
	
	private Class dg1542248606200Clazz = WOMCraftRecords.class;
	
	public Class getDg1542248606200Clazz() {
		return dg1542248606200Clazz;
	}
	
	private List<WOMCraftRecords> dg1542248606200List;
	public List<WOMCraftRecords> getDg1542248606200List(){
		if(null == dg1542248606200List){
			dg1542248606200List = new ArrayList<WOMCraftRecords>();
		}
		return dg1542248606200List;
	}
	public void setDg1542248606200ListJson(String json) {
		//dg1542248606200List = (List<WOMCraftRecords>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftRecords.class);
	}
	
	private List<Long> dg1542248606200DeletedIds;
	public List<Long> getDg1542248606200DeletedIds(){
		if(null == dg1542248606200DeletedIds){
			dg1542248606200DeletedIds = new ArrayList<Long>();
		}
		return dg1542248606200DeletedIds;
	}

	private Page<WOMBatchCharge> dg1542248606236Page;
	public Page<WOMBatchCharge> getDg1542248606236Page(){
		if(null == dg1542248606236Page){
			dg1542248606236Page = new Page<WOMBatchCharge>(1,20);
		}
		return dg1542248606236Page;
	}
	
	private Class dg1542248606236Clazz = WOMBatchCharge.class;
	
	public Class getDg1542248606236Clazz() {
		return dg1542248606236Clazz;
	}
	
	private List<WOMBatchCharge> dg1542248606236List;
	public List<WOMBatchCharge> getDg1542248606236List(){
		if(null == dg1542248606236List){
			dg1542248606236List = new ArrayList<WOMBatchCharge>();
		}
		return dg1542248606236List;
	}
	public void setDg1542248606236ListJson(String json) {
		//dg1542248606236List = (List<WOMBatchCharge>) procFeedbackHeadService.generateObjectFromJson(json, WOMBatchCharge.class);
	}
	
	private List<Long> dg1542248606236DeletedIds;
	public List<Long> getDg1542248606236DeletedIds(){
		if(null == dg1542248606236DeletedIds){
			dg1542248606236DeletedIds = new ArrayList<Long>();
		}
		return dg1542248606236DeletedIds;
	}

	private Page<WOMCraftStandards> dg1540988573983Page;
	public Page<WOMCraftStandards> getDg1540988573983Page(){
		if(null == dg1540988573983Page){
			dg1540988573983Page = new Page<WOMCraftStandards>(1,20);
		}
		return dg1540988573983Page;
	}
	
	private Class dg1540988573983Clazz = WOMCraftStandards.class;
	
	public Class getDg1540988573983Clazz() {
		return dg1540988573983Clazz;
	}
	
	private List<WOMCraftStandards> dg1540988573983List;
	public List<WOMCraftStandards> getDg1540988573983List(){
		if(null == dg1540988573983List){
			dg1540988573983List = new ArrayList<WOMCraftStandards>();
		}
		return dg1540988573983List;
	}
	public void setDg1540988573983ListJson(String json) {
		//dg1540988573983List = (List<WOMCraftStandards>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftStandards.class);
	}
	
	private List<Long> dg1540988573983DeletedIds;
	public List<Long> getDg1540988573983DeletedIds(){
		if(null == dg1540988573983DeletedIds){
			dg1540988573983DeletedIds = new ArrayList<Long>();
		}
		return dg1540988573983DeletedIds;
	}

	private Page<WOMCraftRecords> dg1541043071770Page;
	public Page<WOMCraftRecords> getDg1541043071770Page(){
		if(null == dg1541043071770Page){
			dg1541043071770Page = new Page<WOMCraftRecords>(1,20);
		}
		return dg1541043071770Page;
	}
	
	private Class dg1541043071770Clazz = WOMCraftRecords.class;
	
	public Class getDg1541043071770Clazz() {
		return dg1541043071770Clazz;
	}
	
	private List<WOMCraftRecords> dg1541043071770List;
	public List<WOMCraftRecords> getDg1541043071770List(){
		if(null == dg1541043071770List){
			dg1541043071770List = new ArrayList<WOMCraftRecords>();
		}
		return dg1541043071770List;
	}
	public void setDg1541043071770ListJson(String json) {
		//dg1541043071770List = (List<WOMCraftRecords>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftRecords.class);
	}
	
	private List<Long> dg1541043071770DeletedIds;
	public List<Long> getDg1541043071770DeletedIds(){
		if(null == dg1541043071770DeletedIds){
			dg1541043071770DeletedIds = new ArrayList<Long>();
		}
		return dg1541043071770DeletedIds;
	}

	private Page<WOMBatchCharge> dg1541066103156Page;
	public Page<WOMBatchCharge> getDg1541066103156Page(){
		if(null == dg1541066103156Page){
			dg1541066103156Page = new Page<WOMBatchCharge>(1,20);
		}
		return dg1541066103156Page;
	}
	
	private Class dg1541066103156Clazz = WOMBatchCharge.class;
	
	public Class getDg1541066103156Clazz() {
		return dg1541066103156Clazz;
	}
	
	private List<WOMBatchCharge> dg1541066103156List;
	public List<WOMBatchCharge> getDg1541066103156List(){
		if(null == dg1541066103156List){
			dg1541066103156List = new ArrayList<WOMBatchCharge>();
		}
		return dg1541066103156List;
	}
	public void setDg1541066103156ListJson(String json) {
		//dg1541066103156List = (List<WOMBatchCharge>) procFeedbackHeadService.generateObjectFromJson(json, WOMBatchCharge.class);
	}
	
	private List<Long> dg1541066103156DeletedIds;
	public List<Long> getDg1541066103156DeletedIds(){
		if(null == dg1541066103156DeletedIds){
			dg1541066103156DeletedIds = new ArrayList<Long>();
		}
		return dg1541066103156DeletedIds;
	}

	private Page<WOMCraftStandards> dg1541302488990Page;
	public Page<WOMCraftStandards> getDg1541302488990Page(){
		if(null == dg1541302488990Page){
			dg1541302488990Page = new Page<WOMCraftStandards>(1,20);
		}
		return dg1541302488990Page;
	}
	
	private Class dg1541302488990Clazz = WOMCraftStandards.class;
	
	public Class getDg1541302488990Clazz() {
		return dg1541302488990Clazz;
	}
	
	private List<WOMCraftStandards> dg1541302488990List;
	public List<WOMCraftStandards> getDg1541302488990List(){
		if(null == dg1541302488990List){
			dg1541302488990List = new ArrayList<WOMCraftStandards>();
		}
		return dg1541302488990List;
	}
	public void setDg1541302488990ListJson(String json) {
		//dg1541302488990List = (List<WOMCraftStandards>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftStandards.class);
	}
	
	private List<Long> dg1541302488990DeletedIds;
	public List<Long> getDg1541302488990DeletedIds(){
		if(null == dg1541302488990DeletedIds){
			dg1541302488990DeletedIds = new ArrayList<Long>();
		}
		return dg1541302488990DeletedIds;
	}

	private Page<WOMCraftRecords> dg1541302489005Page;
	public Page<WOMCraftRecords> getDg1541302489005Page(){
		if(null == dg1541302489005Page){
			dg1541302489005Page = new Page<WOMCraftRecords>(1,20);
		}
		return dg1541302489005Page;
	}
	
	private Class dg1541302489005Clazz = WOMCraftRecords.class;
	
	public Class getDg1541302489005Clazz() {
		return dg1541302489005Clazz;
	}
	
	private List<WOMCraftRecords> dg1541302489005List;
	public List<WOMCraftRecords> getDg1541302489005List(){
		if(null == dg1541302489005List){
			dg1541302489005List = new ArrayList<WOMCraftRecords>();
		}
		return dg1541302489005List;
	}
	public void setDg1541302489005ListJson(String json) {
		//dg1541302489005List = (List<WOMCraftRecords>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftRecords.class);
	}
	
	private List<Long> dg1541302489005DeletedIds;
	public List<Long> getDg1541302489005DeletedIds(){
		if(null == dg1541302489005DeletedIds){
			dg1541302489005DeletedIds = new ArrayList<Long>();
		}
		return dg1541302489005DeletedIds;
	}

	private Page<WOMBatchCharge> dg1541302489036Page;
	public Page<WOMBatchCharge> getDg1541302489036Page(){
		if(null == dg1541302489036Page){
			dg1541302489036Page = new Page<WOMBatchCharge>(1,20);
		}
		return dg1541302489036Page;
	}
	
	private Class dg1541302489036Clazz = WOMBatchCharge.class;
	
	public Class getDg1541302489036Clazz() {
		return dg1541302489036Clazz;
	}
	
	private List<WOMBatchCharge> dg1541302489036List;
	public List<WOMBatchCharge> getDg1541302489036List(){
		if(null == dg1541302489036List){
			dg1541302489036List = new ArrayList<WOMBatchCharge>();
		}
		return dg1541302489036List;
	}
	public void setDg1541302489036ListJson(String json) {
		//dg1541302489036List = (List<WOMBatchCharge>) procFeedbackHeadService.generateObjectFromJson(json, WOMBatchCharge.class);
	}
	
	private List<Long> dg1541302489036DeletedIds;
	public List<Long> getDg1541302489036DeletedIds(){
		if(null == dg1541302489036DeletedIds){
			dg1541302489036DeletedIds = new ArrayList<Long>();
		}
		return dg1541302489036DeletedIds;
	}

	private Page<WOMCraftStandards> dg1541073013999Page;
	public Page<WOMCraftStandards> getDg1541073013999Page(){
		if(null == dg1541073013999Page){
			dg1541073013999Page = new Page<WOMCraftStandards>(1,20);
		}
		return dg1541073013999Page;
	}
	
	private Class dg1541073013999Clazz = WOMCraftStandards.class;
	
	public Class getDg1541073013999Clazz() {
		return dg1541073013999Clazz;
	}
	
	private List<WOMCraftStandards> dg1541073013999List;
	public List<WOMCraftStandards> getDg1541073013999List(){
		if(null == dg1541073013999List){
			dg1541073013999List = new ArrayList<WOMCraftStandards>();
		}
		return dg1541073013999List;
	}
	public void setDg1541073013999ListJson(String json) {
		//dg1541073013999List = (List<WOMCraftStandards>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftStandards.class);
	}
	
	private List<Long> dg1541073013999DeletedIds;
	public List<Long> getDg1541073013999DeletedIds(){
		if(null == dg1541073013999DeletedIds){
			dg1541073013999DeletedIds = new ArrayList<Long>();
		}
		return dg1541073013999DeletedIds;
	}

	private Page<WOMCraftRecords> dg1541073014077Page;
	public Page<WOMCraftRecords> getDg1541073014077Page(){
		if(null == dg1541073014077Page){
			dg1541073014077Page = new Page<WOMCraftRecords>(1,20);
		}
		return dg1541073014077Page;
	}
	
	private Class dg1541073014077Clazz = WOMCraftRecords.class;
	
	public Class getDg1541073014077Clazz() {
		return dg1541073014077Clazz;
	}
	
	private List<WOMCraftRecords> dg1541073014077List;
	public List<WOMCraftRecords> getDg1541073014077List(){
		if(null == dg1541073014077List){
			dg1541073014077List = new ArrayList<WOMCraftRecords>();
		}
		return dg1541073014077List;
	}
	public void setDg1541073014077ListJson(String json) {
		//dg1541073014077List = (List<WOMCraftRecords>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftRecords.class);
	}
	
	private List<Long> dg1541073014077DeletedIds;
	public List<Long> getDg1541073014077DeletedIds(){
		if(null == dg1541073014077DeletedIds){
			dg1541073014077DeletedIds = new ArrayList<Long>();
		}
		return dg1541073014077DeletedIds;
	}

	private Page<WOMBatchCharge> dg1541073014155Page;
	public Page<WOMBatchCharge> getDg1541073014155Page(){
		if(null == dg1541073014155Page){
			dg1541073014155Page = new Page<WOMBatchCharge>(1,20);
		}
		return dg1541073014155Page;
	}
	
	private Class dg1541073014155Clazz = WOMBatchCharge.class;
	
	public Class getDg1541073014155Clazz() {
		return dg1541073014155Clazz;
	}
	
	private List<WOMBatchCharge> dg1541073014155List;
	public List<WOMBatchCharge> getDg1541073014155List(){
		if(null == dg1541073014155List){
			dg1541073014155List = new ArrayList<WOMBatchCharge>();
		}
		return dg1541073014155List;
	}
	public void setDg1541073014155ListJson(String json) {
		//dg1541073014155List = (List<WOMBatchCharge>) procFeedbackHeadService.generateObjectFromJson(json, WOMBatchCharge.class);
	}
	
	private List<Long> dg1541073014155DeletedIds;
	public List<Long> getDg1541073014155DeletedIds(){
		if(null == dg1541073014155DeletedIds){
			dg1541073014155DeletedIds = new ArrayList<Long>();
		}
		return dg1541073014155DeletedIds;
	}

	private Page<WOMCraftStandards> dg1541154477969Page;
	public Page<WOMCraftStandards> getDg1541154477969Page(){
		if(null == dg1541154477969Page){
			dg1541154477969Page = new Page<WOMCraftStandards>(1,20);
		}
		return dg1541154477969Page;
	}
	
	private Class dg1541154477969Clazz = WOMCraftStandards.class;
	
	public Class getDg1541154477969Clazz() {
		return dg1541154477969Clazz;
	}
	
	private List<WOMCraftStandards> dg1541154477969List;
	public List<WOMCraftStandards> getDg1541154477969List(){
		if(null == dg1541154477969List){
			dg1541154477969List = new ArrayList<WOMCraftStandards>();
		}
		return dg1541154477969List;
	}
	public void setDg1541154477969ListJson(String json) {
		//dg1541154477969List = (List<WOMCraftStandards>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftStandards.class);
	}
	
	private List<Long> dg1541154477969DeletedIds;
	public List<Long> getDg1541154477969DeletedIds(){
		if(null == dg1541154477969DeletedIds){
			dg1541154477969DeletedIds = new ArrayList<Long>();
		}
		return dg1541154477969DeletedIds;
	}

	private Page<WOMCraftRecords> dg1541154478000Page;
	public Page<WOMCraftRecords> getDg1541154478000Page(){
		if(null == dg1541154478000Page){
			dg1541154478000Page = new Page<WOMCraftRecords>(1,20);
		}
		return dg1541154478000Page;
	}
	
	private Class dg1541154478000Clazz = WOMCraftRecords.class;
	
	public Class getDg1541154478000Clazz() {
		return dg1541154478000Clazz;
	}
	
	private List<WOMCraftRecords> dg1541154478000List;
	public List<WOMCraftRecords> getDg1541154478000List(){
		if(null == dg1541154478000List){
			dg1541154478000List = new ArrayList<WOMCraftRecords>();
		}
		return dg1541154478000List;
	}
	public void setDg1541154478000ListJson(String json) {
		//dg1541154478000List = (List<WOMCraftRecords>) procFeedbackHeadService.generateObjectFromJson(json, WOMCraftRecords.class);
	}
	
	private List<Long> dg1541154478000DeletedIds;
	public List<Long> getDg1541154478000DeletedIds(){
		if(null == dg1541154478000DeletedIds){
			dg1541154478000DeletedIds = new ArrayList<Long>();
		}
		return dg1541154478000DeletedIds;
	}

	private Page<WOMBatchCharge> dg1541154478031Page;
	public Page<WOMBatchCharge> getDg1541154478031Page(){
		if(null == dg1541154478031Page){
			dg1541154478031Page = new Page<WOMBatchCharge>(1,20);
		}
		return dg1541154478031Page;
	}
	
	private Class dg1541154478031Clazz = WOMBatchCharge.class;
	
	public Class getDg1541154478031Clazz() {
		return dg1541154478031Clazz;
	}
	
	private List<WOMBatchCharge> dg1541154478031List;
	public List<WOMBatchCharge> getDg1541154478031List(){
		if(null == dg1541154478031List){
			dg1541154478031List = new ArrayList<WOMBatchCharge>();
		}
		return dg1541154478031List;
	}
	public void setDg1541154478031ListJson(String json) {
		//dg1541154478031List = (List<WOMBatchCharge>) procFeedbackHeadService.generateObjectFromJson(json, WOMBatchCharge.class);
	}
	
	private List<Long> dg1541154478031DeletedIds;
	public List<Long> getDg1541154478031DeletedIds(){
		if(null == dg1541154478031DeletedIds){
			dg1541154478031DeletedIds = new ArrayList<Long>();
		}
		return dg1541154478031DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1542186009188", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.craftName,result.standardValue,result.remark,result.id,result.version,result.procFeedbackID.id,result.craft.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1542186009188-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1542186009188.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1542186009283", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1542186009283-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1542186009283.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1542186009357", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.standingCrop.onhand,result.standingCrop.prepareOrder.outCode,result.standingCrop.batchText,result.actualUseNum,result.good.productBaseUnit.name,result.putinDate,result.putinEndDate,result.standingCrop.availiQuantity,result.remark,result.batch,result.id,result.version,result.good.id,result.standingCrop.id,result.headerId.id,result.wareID.id,result.placeSet.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1542186009357-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1542186009357.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1542248606162", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.craftName,result.standardValue,result.remark,result.id,result.version,result.procFeedbackID.id,result.craft.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1542248606162-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1542248606162.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1542248606200", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1542248606200-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1542248606200.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1542248606236", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.standingCrop.onhand,result.batch,result.actualUseNum,result.good.productBaseUnit.name,result.putinDate,result.putinEndDate,result.remark,result.id,result.version,result.good.id,result.standingCrop.id,result.headerId.id,result.wareID.id,result.placeSet.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1542248606236-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1542248606236.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1540988573983", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.craftName,result.standardValue,result.remark,result.id,result.version,result.procFeedbackID.id,result.craft.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1540988573983-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1540988573983.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541043071770", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541043071770-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541043071770.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541066103156", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.batch,result.materialInWareNum,result.actualUseNum,result.putinDate,result.putinEndDate,result.good.productBaseUnit.name,result.remark,result.id,result.version,result.good.id,result.standingCrop.id,result.headerId.id,result.wareID.id,result.placeSet.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541066103156-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541066103156.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541302488990", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.craftName,result.standardValue,result.remark,result.id,result.version,result.procFeedbackID.id,result.craft.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541302488990-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541302488990.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541302489005", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541302489005-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541302489005.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541302489036", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.placeSet.placeSetCode,result.placeSet.placeSetName,result.batch,result.materialInWareNum,result.actualUseNum,result.good.productBaseUnit.name,result.putinDate,result.putinEndDate,result.remark,result.id,result.version,result.good.id,result.standingCrop.id,result.headerId.id,result.wareID.id,result.placeSet.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541302489036-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541302489036.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541073013999", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.craftName,result.standardValue,result.remark,result.id,result.version,result.procFeedbackID.id,result.craft.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541073013999-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541073013999.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541073014077", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541073014077-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541073014077.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541073014155", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.standingCrop.onhand,result.standingCrop.manulOrder.manulOrderNum,result.standingCrop.batchText,result.actualUseNum,result.good.productBaseUnit.name,result.putinEndDate,result.placeSet.placeSetCode,result.standingCrop.availiQuantity,result.putinDate,result.remark,result.batch,result.id,result.version,result.good.id,result.standingCrop.id,result.headerId.id,result.wareID.id,result.placeSet.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541073014155-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541073014155.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541154477969", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.craftName,result.standardValue,result.remark,result.id,result.version,result.procFeedbackID.id,result.craft.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541154477969-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541154477969.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541154478000", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.standardCraft.craftName,result.standardCraft.standardValue,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541154478000-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541154478000.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/procFeedback/procFeedbackHead/data-dg1541154478031", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.good.productCode,result.good.productName,result.wareID.wareCode,result.wareID.wareName,result.standingCrop.onhand,result.batch,result.actualUseNum,result.good.productBaseUnit.name,result.putinDate,result.placeSet.placeSetName,result.placeSet.placeSetCode,result.putinEndDate,result.remark,result.id,result.version,result.good.id,result.standingCrop.id,result.headerId.id,result.wareID.id,result.placeSet.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541154478031-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541154478031.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(procFeedbackHead.getId() != null && procFeedbackHead.getId() != -1){
				procFeedbackHead = procFeedbackHeadService.getProcFeedbackHead(procFeedbackHead.getId());
			}
			procFeedbackHeadService.findDataGridPage(dg,dgclass,getDgPage(),procFeedbackHead,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == procFeedbackHead.getId()) {
		...
		procFeedbackHeadService.saveProcFeedbackHead(procFeedbackHead,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/procFeedback/procFeedbackHead/adjustBatchEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/procFeedback/procFeedbackHead/adjustBatchView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedbackView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedBatchEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/procFeedback/procFeedbackHead/procFeedBatchView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "submit");
		bap_validate_datagrids=procFeedbackHeadService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", procFeedbackHead.getId());
		...
		fileuploads.put("linkId", procFeedbackHead.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/procFeedback/procFeedbackHead/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		procFeedbackHeadService.deleteProcFeedbackHead(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/procFeedbackHead/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "restore");
		procFeedbackHeadService.restoreProcFeedbackHead(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/procFeedback/procFeedbackHead/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/procFeedback/procFeedbackHead/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "showDealInfoData");
		if (null != tableInfoId) {
			int count=procFeedbackHeadService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = procFeedbackHeadService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = procFeedbackHeadService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProcFeedbackHead> findExportDataInfos(Page<WOMProcFeedbackHead> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = procFeedbackHeadService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = procFeedbackHeadService.getAssProperty(p.getCode());	
									//String assModelCode = procFeedbackHeadService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			procFeedbackHeadService.findProcFeedbackHeads(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			procFeedbackHeadService.findProcFeedbackHeads(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProcFeedbackHead> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProcFeedbackHead result : results){
	...
	public boolean isEditNew(){
		//return null == this.procFeedbackHead || null == this.procFeedbackHead.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/procFeedback/procFeedbackHead/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/procFeedback/procFeedbackHead/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/procFeedback/procFeedbackHead/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/procFeedbackHead/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProcFeedbackHeadService"
						, "com.supcon.orchid.WOM.services.WOMCraftStandardsService"
						, "com.supcon.orchid.WOM.services.WOMCraftRecordsService"
						, "com.supcon.orchid.WOM.services.WOMBatchChargeService"
						...
						, "com.supcon.orchid.WOM.services.WOMPrepareMaterialService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_procFeedback", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_procFeedback");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/procFeedbackHead/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_procFeedback_ProcFeedbackHead", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_procFeedback_ProcFeedbackHead", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/procFeedback/procFeedbackHead/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/procFeedback/procFeedbackHead/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_procFeedback_ProcFeedbackHead";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_ProcFeedbackHead", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/procFeedback/procFeedbackHead/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMProcFeedbackHead procFeedbackHead = null;
		if(null != id) {
			procFeedbackHead = procFeedbackHeadService.getProcFeedbackHead(id);
		}
		...
		_auditLog.setColumnStringB(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
		_auditLog.setColumnStringA(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
		_auditLog.setColumnStringC(procFeedbackHead.getId().toString());
		_auditLog.setColumnStringD(WOMProcFeedbackHead.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_procFeedback");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProcFeedbackHeadAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, procFeedbackHead,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, procFeedbackHead,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,procFeedbackHead);
		...
		printlog.setEntity("WOM_7.5.0.0_procFeedback");
		...
		printlog.setBusinessId(procFeedbackHead.getId());
		printlog.setBussinessKey(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
		printlog.setMainDisplay(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/procFeedback/procFeedbackHead/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(procFeedbackHead,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/procFeedbackHead/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(procFeedbackHeadService.getProcFeedbackHead(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/procFeedback/procFeedbackHead/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/procFeedback/procFeedbackHead/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, procFeedbackHead,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				procFeedbackHead = procFeedbackHeadService.getProcFeedbackHead(Long.parseLong(arrId[i]));
				if(null != procFeedbackHead){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == procFeedbackHead.getId() ? "" : "," + procFeedbackHead.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == procFeedbackHead.getId() ? "" : "," + procFeedbackHead.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == procFeedbackHead.getId() ? "" : "," + procFeedbackHead.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == procFeedbackHead.getId() ? "" : procFeedbackHead.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, procFeedbackHead,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,procFeedbackHead);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//procFeedbackHeadService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/procFeedback/procFeedbackHead/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMProcFeedbackHead> findExportPageInfos(Page<WOMProcFeedbackHead> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_procFeedback" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = procFeedbackHeadService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMProcFeedbackHead queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = procFeedbackHeadService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = procFeedbackHeadService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_ProcFeedbackHead");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_procFeedback_ProcFeedbackHead")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_procFeedback_ProcFeedbackHead");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_ProcFeedbackHead");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = procFeedbackHeadService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_procFeedback_ProcFeedbackHead,WOM_7.5.0.0) */
// 自定义代码
    
    private String dg1541043071770List_Json;

    public String getDg1541043071770List_Json() {
        return dg1541043071770List_Json;
    }

    public void setDg1541043071770List_Json(String dg1541043071770List_Json) {
        this.dg1541043071770List_Json = dg1541043071770List_Json;
    }

    @Actions({
            @Action(value = "/WOM/procFeedback/procFeedbackHead/ref-data-dg1541043071770", results = {
                    @Result(type = JSON, params = { "root", "dg1541043071770Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.standardCraft.craftName,result.standardCraft.standardValue,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
                    @Result(name="xml",type = "freemarkerXml",location="data-dg1541043071770-xml.ftl"),
                    @Result(name="excel",type="excel",location="export-dg1541043071770.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
    })
    public String refdataGridDg1541043071770() throws Exception{
        dg1541043071770List = (List<WOMCraftRecords>) craftRecordsService.generateObjectFromJson(dg1541043071770List_Json, WOMCraftRecords.class);
        dg1541043071770Page = getDg1541043071770Page();
        if(dg1541043071770List!=null&&dg1541043071770List.size()>0){
            dg1541043071770Page.setResult(dg1541043071770List);
        }
        if (null != rt && rt.length() > 0){if("json".equals(rt)) return SUCCESS; return rt;}
        if(getDg1541043071770Page().isExportFlag()) {
            return "excel";
        }else{
            return SUCCESS;
        }
    }
	private String dg1541302489005List_Json;

    public String getDg1541302489005List_Json() {
        return dg1541302489005List_Json;
    }

    public void setDg1541302489005List_Json(String dg1541302489005List_Json) {
        this.dg1541302489005List_Json = dg1541302489005List_Json;
    }

    @Actions({
            @Action(value = "/WOM/procFeedback/procFeedbackHead/ref-data-dg1541302489005", results = {
                    @Result(type = JSON, params = { "root", "dg1541302489005Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.standardCraft.craftName,result.standardCraft.standardValue,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
                    @Result(name="xml",type = "freemarkerXml",location="data-dg1541302489005-xml.ftl"),
                    @Result(name="excel",type="excel",location="export-dg1541302489005.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
    })
    public String refdataGridDg1541302489005() throws Exception{
        dg1541302489005List = (List<WOMCraftRecords>) craftRecordsService.generateObjectFromJson(dg1541302489005List_Json, WOMCraftRecords.class);
        dg1541302489005Page = getDg1541302489005Page();
        if(dg1541302489005List!=null&&dg1541302489005List.size()>0){
            dg1541302489005Page.setResult(dg1541302489005List);
        }
        if (null != rt && rt.length() > 0){if("json".equals(rt)) return SUCCESS; return rt;}
        if(getDg1541302489005Page().isExportFlag()) {
            return "excel";
        }else{
            return SUCCESS;
        }
    }
    private String dg1541073014077List_Json;

    public String getDg1541073014077List_Json() {
        return dg1541073014077List_Json;
    }

    public void setDg1541073014077List_Json(String dg1541073014077List_Json) {
        this.dg1541073014077List_Json = dg1541073014077List_Json;
    }

    @Actions({
            @Action(value = "/WOM/procFeedback/procFeedbackHead/ref-data-dg1541073014077", results = {
                    @Result(type = JSON, params = { "root", "dg1541073014077Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.standardCraft.craftName,result.standardCraft.standardValue,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
                    @Result(name="xml",type = "freemarkerXml",location="data-dg1541073014077-xml.ftl"),
                    @Result(name="excel",type="excel",location="export-dg1541073014077.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
    })
    public String refdataGridDg1541073014077() throws Exception{
        dg1541073014077List = (List<WOMCraftRecords>) craftRecordsService.generateObjectFromJson(dg1541073014077List_Json, WOMCraftRecords.class);
        dg1541073014077Page = getDg1541073014077Page();
        if(dg1541073014077List!=null&&dg1541073014077List.size()>0){
            dg1541073014077Page.setResult(dg1541073014077List);
        }
        if (null != rt && rt.length() > 0){if("json".equals(rt)) return SUCCESS; return rt;}
        if(getDg1541073014077Page().isExportFlag()) {
            return "excel";
        }else{
            return SUCCESS;
        }
    }
    private String dg1541154478000List_Json;

    public String getDg1541154478000List_Json() {
        return dg1541154478000List_Json;
    }

    public void setDg1541154478000List_Json(String dg1541154478000List_Json) {
        this.dg1541154478000List_Json = dg1541154478000List_Json;
    }

    @Actions({
            @Action(value = "/WOM/procFeedback/procFeedbackHead/ref-data-dg1541154478000", results = {
                    @Result(type = JSON, params = { "root", "dg1541154478000Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.standardCraft.craftName,result.standardCraft.standardValue,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
                    @Result(name="xml",type = "freemarkerXml",location="data-dg1541154478000-xml.ftl"),
                    @Result(name="excel",type="excel",location="export-dg1541154478000.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
    })
    public String refdataGridDg1541154478000() throws Exception{
        dg1541154478000List = (List<WOMCraftRecords>) craftRecordsService.generateObjectFromJson(dg1541154478000List_Json, WOMCraftRecords.class);
        dg1541154478000Page = getDg1541154478000Page();
        if(dg1541154478000List!=null&&dg1541154478000List.size()>0){
            dg1541154478000Page.setResult(dg1541154478000List);
        }
        if (null != rt && rt.length() > 0){if("json".equals(rt)) return SUCCESS; return rt;}
        if(getDg1541154478000Page().isExportFlag()) {
            return "excel";
        }else{
            return SUCCESS;
        }
     ...
     * @return
     */
    @Action(value = "/WOM/procFeedback/procFeedbackHead/deleteProcessRow", results = { @Result(type = JSON, params = {
            "root", "responseMap" }) })
    public String deleteProcessRow(){
        String result = null;
        if (processIds != null) {
            result=procFeedbackHeadService.deleteProcessRowByProcessId(processIds);
        }
     ...
     * @return
     */
    @Action(value = "/WOM/procFeedback/procFeedbackHead/deleteCraftRecord", results = { @Result(type = JSON, params = {
            "root", "responseMap" }) })
    public String deleteCraftRecord(){
        String result = null;
        if (recordIds != null) {
            result=procFeedbackHeadService.deleteCraftRecordByRecordId(recordIds);
        }
        responseMap.put("result", result);
        return SUCCESS;
    }
	private String dg1542186009283List_Json;

    public String getDg1542186009283List_Json() {
        return dg1542186009283List_Json;
    }

    public void setDg1542186009283List_Json(String dg1542186009283List_Json) {
        this.dg1542186009283List_Json = dg1542186009283List_Json;
    }

    @Actions({
            @Action(value = "/WOM/procFeedback/procFeedbackHead/ref-data-dg1542186009283", results = {
                    @Result(type = JSON, params = { "root", "dg1542186009283Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.standardCraft.craftName,result.standardCraft.standardValue,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
                    @Result(name="xml",type = "freemarkerXml",location="data-dg1542186009283-xml.ftl"),
                    @Result(name="excel",type="excel",location="export-dg1542186009283.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
    })
    public String refdataGridDg1542186009283() throws Exception{
        dg1542186009283List = (List<WOMCraftRecords>) craftRecordsService.generateObjectFromJson(dg1542186009283List_Json, WOMCraftRecords.class);
        dg1542186009283Page = getDg1542186009283Page();
        if(dg1542186009283List!=null&&dg1542186009283List.size()>0){
            dg1542186009283Page.setResult(dg1542186009283List);
        }
        if (null != rt && rt.length() > 0){if("json".equals(rt)) return SUCCESS; return rt;}
        if(getDg1542186009283Page().isExportFlag()) {
            return "excel";
        }else{
            return SUCCESS;
        }
    }
		private String dg1542248606200List_Json;

    public String getDg1542248606200List_Json() {
        return dg1542248606200List_Json;
    }

    public void setDg1542248606200List_Json(String dg1542248606200List_Json) {
        this.dg1542248606200List_Json = dg1542248606200List_Json;
    }

    @Actions({
            @Action(value = "/WOM/procFeedback/procFeedbackHead/ref-data-dg1542248606200", results = {
                    @Result(type = JSON, params = { "root", "dg1542248606200Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.standardCraft.craftName,result.standardCraft.standardValue,result.recordStartTime,result.recordEndTime,result.actualValue,result.remark,result.id,result.version,result.standardCraft.id,result.header.id" }),
                    @Result(name="xml",type = "freemarkerXml",location="data-dg1542248606200-xml.ftl"),
                    @Result(name="excel",type="excel",location="export-dg1542248606200.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
    })
    public String refdataGridDg1542248606200() throws Exception{
        dg1542248606200List = (List<WOMCraftRecords>) craftRecordsService.generateObjectFromJson(dg1542248606200List_Json, WOMCraftRecords.class);
        dg1542248606200Page = getDg1542248606200Page();
        if(dg1542248606200List!=null&&dg1542248606200List.size()>0){
            dg1542248606200Page.setResult(dg1542248606200List);
        }
        if (null != rt && rt.length() > 0){if("json".equals(rt)) return SUCCESS; return rt;}
        if(getDg1542248606200Page().isExportFlag()) {
            return "excel";
        }else{
            return SUCCESS;
        }
    }
	private String preMaterialIds;
	private String productCode;
	public String getPreMaterialIds() {
		return preMaterialIds;
	}

	public void setPreMaterialIds(String preMaterialId) {
		this.preMaterialIds = preMaterialId;
	}
	 ...
	 */
	@Actions({
			@Action(value = "/WOM/procFeedback/procFeedbackHead/getMaterial", results = {
					@Result(type = JSON, params = { "root", "dg1541073014155Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.good.productName,result.wareID.wareCode,result.id,result.version,result.valid,result.cid,result.status,result.batch,result.good.productCode,result.batchText,result.wareID.wareName,result.actualUseNum,result.good.id,result.wareID.id,result.good.productBaseUnit.name,result.good.productBaseUnit.id,result.attrMap.*"}),
					@Result(name="xml",type = "freemarkerXml",location="data-dg1541073014155Page-xml.ftl"),
					@Result(name="excel",type="excel",location="export-dg1541073014155.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String getStockInfo() throws Exception{
		List<WOMBatchCharge> partList= new ArrayList<WOMBatchCharge>();
		//将字符串根据#拆分，得到n组数据
		String[] conditionList = preMaterialIds.split("#");
		for (String preMaterialId : conditionList) {
			//根据指令单id，已有的list返回新登list
			partList = procFeedbackHeadService.getpreMaterialPart(preMaterialId,productCode,partList);
		}
		if(dg1541073014155Page==null) dg1541073014155Page = getDg1541073014155Page();
		dg1541073014155Page.setResult(partList);
		return SUCCESS;
	}
/* CUSTOM CODE END */
}