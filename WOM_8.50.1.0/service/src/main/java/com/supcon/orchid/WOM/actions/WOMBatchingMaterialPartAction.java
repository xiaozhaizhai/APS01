...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.ec.entities.PrintTemplate;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchingMaterialPartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchingMaterialPart> {
	...
	private WOMBatchingMaterialPart batchingMaterialPart;
	...
	//导出excel时,主模型内容
	List<WOMBatchingMaterialPart> queryResults = new ArrayList<WOMBatchingMaterialPart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterial/batchingMaterialPart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterial/batchingMaterialPart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchingMaterialPart> batchingMaterialParts;
	
	public List<WOMBatchingMaterialPart> getBatchingMaterialParts() {
		return batchingMaterialParts;
	}
	...
	private Page<WOMBatchingMaterialPart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchingMaterialPart = new WOMBatchingMaterialPart();
			if(batchingMaterialPart.getActiveType() != null) {
				batchingMaterialPart.setActiveType((SystemCode) systemCodeService.getSystemCode(batchingMaterialPart.getActiveType().getId()));
			}
			if(batchingMaterialPart.getBatchSite() != null) {
				batchingMaterialPart.setBatchSite((SystemCode) systemCodeService.getSystemCode(batchingMaterialPart.getBatchSite().getId()));
			}
			if(batchingMaterialPart.getOfferSystem() != null) {
				batchingMaterialPart.setOfferSystem((SystemCode) systemCodeService.getSystemCode(batchingMaterialPart.getOfferSystem().getId()));
			}
			if(batchingMaterialPart.getState() != null) {
				batchingMaterialPart.setState((SystemCode) systemCodeService.getSystemCode(batchingMaterialPart.getState().getId()));
			}
		} else {
			if(id > 0){
				batchingMaterialPart = batchingMaterialPartService.getBatchingMaterialPart(id, viewCode);
			}
		}
	...
	public Page<WOMBatchingMaterialPart> getPage() {
		if (null == page) {
			page = new Page<WOMBatchingMaterialPart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchingMaterialPart getBatchingMaterialPart() {
		return batchingMaterialPart;
	}		

	public void setBatchingMaterialPart(WOMBatchingMaterialPart batchingMaterialPart) {
		this.batchingMaterialPart = batchingMaterialPart;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchingMaterialPartService.getBatchingMaterialPartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1491789803097
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchingMaterialPartReferenceCopy() throws Exception{
			WOMBatchingMaterialPart oldBatchingMaterialPart = null;
			if(isEditNew()){
				...
				oldBatchingMaterialPart = new WOMBatchingMaterialPart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchingMaterialPart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchingMaterialPart.getId());
				...
				oldBatchingMaterialPart = batchingMaterialPartService.getBatchingMaterialPart(id);
			}
			
			
			batchingMaterialPart = batchingMaterialPartService.getBatchingMaterialPart(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchingMaterialPart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchingMaterialPart, null);
						Method setValue = WOMBatchingMaterialPart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchingMaterialPart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchingMaterialPart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batchMaterialEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchMaterialEdit.ftl") }),
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batchMaterialEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchMaterialEditPrint.ftl")})
		,
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/rejectReciveEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "rejectReciveEdit.ftl") }),
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/rejectReciveEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "rejectReciveEditPrint.ftl")})
		,
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/rejectRecordEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "rejectRecordEdit.ftl") }),
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/rejectRecordEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "rejectRecordEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchingMaterialPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchingMaterialPartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchingMaterialPart = batchingMaterialPartService.getBatchingMaterialPart(this.batchingMaterialPart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchingMaterialPart.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchingMaterialPartService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batchMaterialView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchMaterialView.ftl") }),
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batchMaterialViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchMaterialViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batMaterialPartList", results = { @Result(type = FREEMARKER, location = "batMaterialPartList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchingMaterialPartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchingMaterialPartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchingMaterialPartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batPartForProcRef", results = { @Result(type = FREEMARKER, location = "batPartForProcRef.ftl") })
		,
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batPartShowRef", results = { @Result(type = FREEMARKER, location = "batPartShowRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batMaterialPartList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headId.tableNo,result.taskID.tableNo,result.taskID.productBatchNum,result.taskID.factoryId.name,result.factoryId.name,result.taskID.productId.productName,result.productId.productCode,result.productId.productName,result.batchNum,result.doneNum,result.wareID.wareCode,result.wareID.wareName,result.wareID.cargoStatue.id,result.wareID.cargoStatue.value,result.storeID.placeSetCode,result.storeID.placeSetName,result.offerSystem.id,result.offerSystem.value,result.exeStaff.name,result.exeDate,result.batchDate,result.receiveStaff.name,result.receiveDate,result.printCount,result.state.id,result.state.value,result.remark,result.taskBatch,result.headId.id,result.taskID.id,result.taskID.factoryId.id,result.factoryId.id,result.taskID.productId.id,result.productId.id,result.wareID.id,result.storeID.id,result.exeStaff.id,result.receiveStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batMaterialPartList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchingMaterialPart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchingMaterialPart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("storeSet.layRec") && request.getParameter("storeSet.layRec").trim().length() > 0) {
			params.add(new Param("\"storeID\".LAY_REC", request.getParameter("storeSet.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchingMaterialPart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchingMaterialPart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchingMaterialPartService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batMaterialPartList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batPartForProcRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headId.tableNo,result.productId.productCode,result.productId.productName,result.batchNum,result.taskActiveId.planNum,result.doneNum,result.productId.productBaseUnit.name,result.wareID.wareCode,result.wareID.wareName,result.storeID.placeSetCode,result.storeID.placeSetName,result.offerSystem.id,result.offerSystem.value,result.exeStaff.name,result.exeDate,result.receiveStaff.name,result.receiveDate,result.state.id,result.state.value,result.taskID.tableNo,result.headId.id,result.productId.id,result.taskActiveId.id,result.productId.productBaseUnit.id,result.wareID.id,result.storeID.id,result.exeStaff.id,result.receiveStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batPartForProcRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batPartShowRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headId.tableNo,result.taskID.tableNo,result.productId.productCode,result.productId.productName,result.batchNum,result.doneNum,result.wareID.wareCode,result.wareID.wareName,result.storeID.placeSetCode,result.storeID.placeSetName,result.offerSystem.id,result.offerSystem.value,result.exeStaff.name,result.exeDate,result.receiveStaff.name,result.receiveDate,result.state.id,result.state.value,result.headId.id,result.taskID.id,result.productId.id,result.wareID.id,result.storeID.id,result.exeStaff.id,result.receiveStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batPartShowRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchingMaterialPartService.findDataGridPage(dg,dgclass,getDgPage(),batchingMaterialPart,"", null);
	...
	private Page<WOMBatchingRejectParts> dg1543461038761Page;
	public Page<WOMBatchingRejectParts> getDg1543461038761Page(){
		if(null == dg1543461038761Page){
			dg1543461038761Page = new Page<WOMBatchingRejectParts>(1,20);
		}
		return dg1543461038761Page;
	}
	
	private Class dg1543461038761Clazz = WOMBatchingRejectParts.class;
	
	public Class getDg1543461038761Clazz() {
		return dg1543461038761Clazz;
	}
	
	private List<WOMBatchingRejectParts> dg1543461038761List;
	public List<WOMBatchingRejectParts> getDg1543461038761List(){
		if(null == dg1543461038761List){
			dg1543461038761List = new ArrayList<WOMBatchingRejectParts>();
		}
		return dg1543461038761List;
	}
	public void setDg1543461038761ListJson(String json) {
		//dg1543461038761List = (List<WOMBatchingRejectParts>) batchingMaterialPartService.generateObjectFromJson(json, WOMBatchingRejectParts.class);
	}
	
	private List<Long> dg1543461038761DeletedIds;
	public List<Long> getDg1543461038761DeletedIds(){
		if(null == dg1543461038761DeletedIds){
			dg1543461038761DeletedIds = new ArrayList<Long>();
		}
		return dg1543461038761DeletedIds;
	}

	private Page<WOMBatchingRejectParts> dg1543470616564Page;
	public Page<WOMBatchingRejectParts> getDg1543470616564Page(){
		if(null == dg1543470616564Page){
			dg1543470616564Page = new Page<WOMBatchingRejectParts>(1,20);
		}
		return dg1543470616564Page;
	}
	
	private Class dg1543470616564Clazz = WOMBatchingRejectParts.class;
	
	public Class getDg1543470616564Clazz() {
		return dg1543470616564Clazz;
	}
	
	private List<WOMBatchingRejectParts> dg1543470616564List;
	public List<WOMBatchingRejectParts> getDg1543470616564List(){
		if(null == dg1543470616564List){
			dg1543470616564List = new ArrayList<WOMBatchingRejectParts>();
		}
		return dg1543470616564List;
	}
	public void setDg1543470616564ListJson(String json) {
		//dg1543470616564List = (List<WOMBatchingRejectParts>) batchingMaterialPartService.generateObjectFromJson(json, WOMBatchingRejectParts.class);
	}
	
	private List<Long> dg1543470616564DeletedIds;
	public List<Long> getDg1543470616564DeletedIds(){
		if(null == dg1543470616564DeletedIds){
			dg1543470616564DeletedIds = new ArrayList<Long>();
		}
		return dg1543470616564DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/data-dg1543461038761", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.batchNum,result.rejectProduct.productCode,result.rejectProduct.productName,result.productBatchNum,result.ware.wareName,result.ware.wareCode,result.ware.wareState.id,result.ware.wareState.value,result.ware.id,result.store.placeSetName,result.store.placeSetCode,result.store.id,result.rejectNum,result.rejectReasonId.rejectReson,result.rejectExeStaff.name,result.rejectTime,result.produceTaskId.tableNo,result.produceTaskId.factoryId.name,result.produceTaskId.productId.productName,result.batchingId.id,result.batchingPartId.id,result.produceTaskId.id,result.rejectReceiveStaff.name,result.receiveDate,result.tableInfoId,result.WOM_7_5_0_0_batchingMaterial_rejectReciveEditdg1543461038761_DATAGRID_CUSTOM_ef469ae6_fa1d_4051_8203_6e2549c55ca7,result.id,result.version,result.store.id,result.rejectExeStaff.id,result.rejectReceiveStaff.id,result.batchingId.id,result.rejectProduct.id,result.batchingPartId.id,result.rejectReasonId.id,result.ware.id,result.produceTaskId.id" , "dataGridCode", "WOM_7.5.0.0_batchingMaterial_rejectReciveEditdg1543461038761" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1543461038761-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1543461038761.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/data-dg1543470616564", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.batchNum,result.rejectProduct.productCode,result.rejectProduct.productName,result.productBatchNum,result.rejectNum,result.rejectReasonId.rejectReson,result.rejectReasonId.isChangeNeed,result.ware.wareCode,result.ware.wareName,result.ware.cargoStatue.id,result.ware.cargoStatue.value,result.store.placeSetCode,result.store.placeSetName,result.produceTaskId.tableNo,result.produceTaskId.factoryId.name,result.produceTaskId.productId.productName,result.batchingPartId.state.id,result.batchingPartId.state.value,result.batchingId.id,result.batchingPartId.id,result.produceTaskId.id,result.rejectExeStaff.name,result.rejectTime,result.id,result.version,result.store.id,result.rejectExeStaff.id,result.rejectReceiveStaff.id,result.batchingId.id,result.rejectProduct.id,result.batchingPartId.id,result.rejectReasonId.id,result.ware.id,result.produceTaskId.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1543470616564-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1543470616564.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchingMaterialPart.getId() != null && batchingMaterialPart.getId() != -1){
				batchingMaterialPart = batchingMaterialPartService.getBatchingMaterialPart(batchingMaterialPart.getId());
			}
			batchingMaterialPartService.findDataGridPage(dg,dgclass,getDgPage(),batchingMaterialPart,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchingMaterialPart.getId()) {
		...
		batchingMaterialPartService.saveBatchingMaterialPart(batchingMaterialPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batchMaterialEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batchMaterialView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/rejectReciveEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/rejectRecordEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "submit");
		bap_validate_datagrids=batchingMaterialPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchingMaterialPart.getId());
		...
		fileuploads.put("linkId", batchingMaterialPart.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchingMaterialPartService.deleteBatchingMaterialPart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "restore");
		batchingMaterialPartService.restoreBatchingMaterialPart(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchingMaterialPart> findExportDataInfos(Page<WOMBatchingMaterialPart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchingMaterialPartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchingMaterialPartService.getAssProperty(p.getCode());	
									//String assModelCode = batchingMaterialPartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchingMaterialPartService.findBatchingMaterialParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchingMaterialPartService.findBatchingMaterialParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchingMaterialPart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchingMaterialPart result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchingMaterialPart || null == this.batchingMaterialPart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchingMaterial/batchingMaterialPart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchingMaterial/batchingMaterialPart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchingMaterialPartService"
						...
						, "com.supcon.orchid.WOM.services.WOMRejectReasonService"
						...
						, "com.supcon.orchid.WOM.services.WOMBatchingMaterialService"
					, "com.supcon.orchid.WOM.services.WOMBatchingMaterialService"
					, "com.supcon.orchid.WOM.services.WOMBatchingMaterialService"
					, "com.supcon.orchid.WOM.services.WOMBatchingMaterialService"
					, "com.supcon.orchid.WOM.services.WOMBatchingMaterialService"
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
		_auditLog.setColumnStringA(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
		_auditLog.setColumnStringC(batchingMaterialPart.getId().toString());
		_auditLog.setColumnStringD(WOMBatchingMaterialPart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchingMaterialPartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchingMaterialPart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchingMaterialPart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchingMaterialPart);
		...
		printlog.setBusinessId(batchingMaterialPart.getId());
		printlog.setBussinessKey(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
		printlog.setMainDisplay(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchingMaterialPart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchingMaterialPartService.getBatchingMaterialPart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchingMaterialPart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchingMaterialPart = batchingMaterialPartService.getBatchingMaterialPart(Long.parseLong(arrId[i]));
				if(null != batchingMaterialPart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchingMaterialPart.getId() ? "" : "," + batchingMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchingMaterialPart.getId() ? "" : "," + batchingMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchingMaterialPart.getId() ? "" : "," + batchingMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchingMaterialPart.getId() ? "" : batchingMaterialPart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchingMaterialPart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchingMaterialPart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchingMaterialPartService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchingMaterialPartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
		if (org.apache.commons.lang.StringUtils.equals(code,"WOM_7.5.0.0_batchingMaterial_batPartForProcRef")) {
			condition = "activeId=" + activeId;
		}
/* CUSTOM CODE END */
	}
	
	private void bapAfterCustomMethod(String code, String methodName) throws Exception {
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
		//表体保存时调用
		if("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart".equals(code)&&"submit".equals(methodName)){
			if("out".equals(out)){
				//将配料数量累加到表体批送数量
				batchingMaterialPartService.addDownNum(batchingMaterialPart);
			}
		}
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart,WOM_7.5.0.0) */
// 自定义代码
	private String headID;//表头id
	private String proID;//物品id
	private String proCode;//物品Code
	private String proName;//物品Name
	private String partIds;//组织数据，partid
	private String out;//进行配料添加操作所传递的参数
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/batch", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag,asCode" }) })
	public String batch() throws Exception{
		if(batchingMaterialPartService.batch(partIds)){
			responseMap.put("dealSuccessFlag", true);
		}
		return SUCCESS;
	}
	...
	//打印控件参数获取
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/printControlParam", results = { @Result(type = JSON, params = {
			"root", "responseMap" }) })
	public String getPrintControlParam() {
		/*List<WOMBatchingMaterialPart> wOMBatchingMaterialPartList= batchingMaterialPartService.getBatchingMaterialParts(null,null);
		if(wOMBatchingMaterialPartList != null && wOMBatchingMaterialPartList.size() > 0 ){
			//responseMap.put("result", true);
			//responseMap.put("tableInfoId", wOMBatchingMaterialPartList.get(0).getHeadId().getTableInfoId());
			//responseMap.put("deploymentId", wOMBatchingMaterialPartList.get(0).getHeadId().getDeploymentId());
			boolean need = true;
			for(WOMBatchingMaterialPart part : wOMBatchingMaterialPartList){
				if(part.getHeadId() != null){
					Deployment deployment = processService.getDeployment(part.getHeadId().getDeploymentId());
					if(null != deployment) {
						...
						if(printTemplate != null){
							responseMap.put("result", true);
							responseMap.put("tableInfoId", part.getHeadId().getTableInfoId());
							responseMap.put("deploymentId", part.getHeadId().getDeploymentId());
							need = false;
							break;
						}
					}
		...
		responseMap = batchingMaterialPartService.getPrintParam();
		return SUCCESS;
	}

	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/arrayLoadXMLRefOrData", results = {@Result(name = SUCCESS, type = "json", params = {"root", "printXmlInfo"})})
	public String arrayLoadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		WOMBatchingMaterialPart batchingMaterialPart = null;
		//String printHeadId = request.getParameter("headId");
		if (null != id) {
			batchingMaterialPart = batchingMaterialPartService.getBatchingMaterialPart(id);
		}

		// 获取打印模板信息
		String templateCode = request.getParameter("templateCode");//"WOM_7.5.0.0_batchingMaterial_batchMaterial";
		PrintTemplate printTemplate = printService.getPrintTemplateByCode(templateCode);

		//xmlRefernece = printService.buildPrintXmlRef(viewCode, batchingMaterialPart.getHeadId(), null, tableInfoId, false, this);
		//xmlData = printService.getPrintXmlRef(printTemplate, batchingMaterialPart, batchingMaterialPart.getHeadId().getTableInfoId(), viewCode, true);

		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchingMaterialPart.getHeadId(), batchingMaterialPart.getHeadId().getTableInfoId(), viewCode, false);
		// 获取打印插件XML对应的数据信息
		xmlData = generatePrintXmlData(batchingMaterialPart);
	...
	// 拼接自定义xml模板
	private String generatePrintXmlData(WOMBatchingMaterialPart batchingMaterialPart) {
		String xml = "";
		String printXmlData = printService.buildPrintXmlRef(viewCode, batchingMaterialPart.getHeadId(), null, null, true, this);

		if (printXmlData != null && printXmlData.length() > 0 && printXmlData.contains("masterfields=\"id\">") && printXmlData.contains("</BatchingMaterialParts>")) {
			String forntXml = printXmlData.substring(0, printXmlData.indexOf("masterfields=\"id\">") + 18);
			String backXml = printXmlData.substring(printXmlData.lastIndexOf("</BatchingMaterialParts>"), printXmlData.length());
			StringBuilder sb = new StringBuilder();
			sb.append("<BatchingMaterialPart>");
			sb.append("<id>");
			sb.append(batchingMaterialPart.getId() != null ? batchingMaterialPart.getId() : "");//配料记录ID
			sb.append("</id>");
			sb.append("<productId.productBaseUnit.name>");//主单位名称
			sb.append(batchingMaterialPart.getProductId() != null && batchingMaterialPart.getProductId().getProductBaseUnit() != null ? batchingMaterialPart.getProductId().getProductBaseUnit().getName() : "");//主单位名称
			sb.append("</productId.productBaseUnit.name>");
			sb.append("<doneNum>");
			sb.append(batchingMaterialPart.getDoneNum() != null ? batchingMaterialPart.getDoneNum() : "");//配送数量
			sb.append("</doneNum>");
			sb.append("<factoryId.area>");
			sb.append(batchingMaterialPart.getFactoryId() != null ? batchingMaterialPart.getFactoryId().getArea() : "");//工作单元->位置
			sb.append("</factoryId.area>");
			sb.append("<factoryId.area.id>");
			sb.append(batchingMaterialPart.getFactoryId() != null && batchingMaterialPart.getFactoryId().getArea() != null? batchingMaterialPart.getFactoryId().getArea().getId() : "");
			sb.append("</factoryId.area.id>");
			sb.append("<factoryId.area.value>");
			sb.append(batchingMaterialPart.getFactoryId() != null && batchingMaterialPart.getFactoryId().getArea() != null ? batchingMaterialPart.getFactoryId().getArea().getValue() : "");
			sb.append("</factoryId.area.value>");
			sb.append("<storeID.placeSetName>");
			sb.append(batchingMaterialPart.getStoreID() != null ? batchingMaterialPart.getStoreID().getPlaceSetName() : "");
			sb.append("</storeID.placeSetName>");
			sb.append("<wareID.cargoStatue>");
			sb.append(batchingMaterialPart.getWareID() != null ? batchingMaterialPart.getWareID().getCargoStatue() : "");
			sb.append("</wareID.cargoStatue>");
			sb.append("<productId.productName>");
			sb.append(batchingMaterialPart.getProductId() != null ? batchingMaterialPart.getProductId().getProductName() : "");
			sb.append("</productId.productName>");
			sb.append("<wareID.wareCode>");
			sb.append(batchingMaterialPart.getWareID() != null ? batchingMaterialPart.getWareID().getWareCode() : "");
			sb.append("</wareID.wareCode>");
			sb.append("<wareID.id>");
			sb.append(batchingMaterialPart.getWareID() != null ? batchingMaterialPart.getWareID().getId() : "");
			sb.append("</wareID.id>");
			sb.append("<storeID.placeSetCode>");
			sb.append(batchingMaterialPart.getStoreID() != null ? batchingMaterialPart.getStoreID().getPlaceSetCode() : "");
			sb.append("</storeID.placeSetCode>");
			sb.append("<productId.id>");
			sb.append(batchingMaterialPart.getProductId() != null ? batchingMaterialPart.getProductId().getId() : "");
			sb.append("</productId.id>");
			sb.append("<taskBatch>");
			sb.append(batchingMaterialPart.getTaskBatch() != null ? batchingMaterialPart.getTaskBatch() : "");//生产批号text
			sb.append("</taskBatch>");
			sb.append("<headId.tableNo>");
			sb.append(batchingMaterialPart.getHeadId() != null ? batchingMaterialPart.getHeadId().getTableNo() : "");
			sb.append("</headId.tableNo>");
			sb.append("<productId.productCode>");
			sb.append(batchingMaterialPart.getProductId() != null ? batchingMaterialPart.getProductId().getProductCode() : "");
			sb.append("</productId.productCode>");
			sb.append("<productId.mnecode>");
			sb.append(batchingMaterialPart.getProductId() != null ? (batchingMaterialPart.getProductId().getMnecode() != null ? batchingMaterialPart.getProductId().getMnecode() : "") : "");//物料助记码
			sb.append("</productId.mnecode>");
			sb.append("<productId.productName>");
			sb.append(batchingMaterialPart.getProductId() != null ? batchingMaterialPart.getProductId().getProductName() : "");//物料名称
			sb.append("</productId.productName>");
			sb.append("<headId.id>");
			sb.append(batchingMaterialPart.getHeadId() != null ? batchingMaterialPart.getHeadId().getId() : "");
			sb.append("</headId.id>");
			sb.append("<taskID.productId.productName>");
			sb.append(batchingMaterialPart.getTaskID() != null && batchingMaterialPart.getTaskID().getProductId() != null ? batchingMaterialPart.getTaskID().getProductId().getProductName() : "");//生产指令单ID->产品->物品名称
			sb.append("</taskID.productId.productName>");
			sb.append("<wareID.wareName>");
			sb.append(batchingMaterialPart.getWareID() != null ? batchingMaterialPart.getWareID().getWareName() : "");
			sb.append("</wareID.wareName>");
			sb.append("<needPartId.id>");
			sb.append(batchingMaterialPart.getNeedPartId() != null ? batchingMaterialPart.getNeedPartId().getId() : "");//指定物料批号id
			sb.append("</needPartId.id>");
			sb.append("<headId.needNum>");
			sb.append(batchingMaterialPart.getHeadId() != null ? batchingMaterialPart.getHeadId().getNeedNum() : "");//需配数量
			sb.append("</headId.needNum>");
			sb.append("<factoryId.name>");
			sb.append(batchingMaterialPart.getFactoryId() != null ? batchingMaterialPart.getFactoryId().getName() : "");//工作单元->节点名称
			sb.append("</factoryId.name>");
			sb.append("<productId.productBaseUnit.rate>");
			sb.append(batchingMaterialPart.getProductId() != null && batchingMaterialPart.getProductId().getProductBaseUnit() != null ? batchingMaterialPart.getProductId().getProductBaseUnit().getRate() : "");//主单位比率
			sb.append("</productId.productBaseUnit.rate>");
			sb.append("<offerSystem>");
			sb.append(batchingMaterialPart.getOfferSystem() != null ? batchingMaterialPart.getOfferSystem() : "");
			sb.append("</offerSystem>");
			sb.append("<tableInfoId>");
			sb.append(batchingMaterialPart.getTableInfoId() != null ? batchingMaterialPart.getTableInfoId() : "");
			sb.append("</tableInfoId>");
			sb.append("<actOrder>");
			sb.append(batchingMaterialPart.getActOrder() != null ? batchingMaterialPart.getActOrder() : "");
			sb.append("</actOrder>");
			sb.append("<lableCode>");
			sb.append(batchingMaterialPart.getLableCode() != null ? batchingMaterialPart.getLableCode() : "");
			sb.append("</lableCode>");
			sb.append("<state>");
			sb.append(batchingMaterialPart.getState() != null ? batchingMaterialPart.getState() : "");
			sb.append("</state>");
			sb.append("<state.id>");
			sb.append(batchingMaterialPart.getState() != null ? batchingMaterialPart.getState().getId() : "");
			sb.append("</state.id>");
			sb.append("<state.value>");
			sb.append(batchingMaterialPart.getState() != null ? batchingMaterialPart.getState().getValue() : "");
			sb.append("</state.value>");
			sb.append("<remark>");
			sb.append(batchingMaterialPart.getRemark() != null ? batchingMaterialPart.getRemark() : "");//备注
			sb.append("</remark>");
			sb.append("<printCount>");
			sb.append(batchingMaterialPart.getPrintCount() != null ? batchingMaterialPart.getPrintCount() : "");
			sb.append("</printCount>");
			sb.append("<taskID.id>");
			sb.append(batchingMaterialPart.getTaskID() != null ? batchingMaterialPart.getTaskID().getId() : "");
			sb.append("</taskID.id>");
			sb.append("<exeDate>");
			sb.append(batchingMaterialPart.getExeDate() != null ? batchingMaterialPart.getExeDate() : "");
			sb.append("</exeDate>");
			sb.append("<storeID.id>");
			sb.append(batchingMaterialPart.getStoreID() != null ? batchingMaterialPart.getStoreID().getId() : "");
			sb.append("</storeID.id>");
			sb.append("<batchNum>");
			sb.append(batchingMaterialPart.getBatchNum() != null ? batchingMaterialPart.getBatchNum() : "");//物料批号
			sb.append("</batchNum>");
			sb.append("<factoryId.id>");
			sb.append(batchingMaterialPart.getFactoryId() != null ? batchingMaterialPart.getFactoryId().getId() : "");//工作单元名称
			sb.append("</factoryId.id>");
			sb.append("<batchNumObj.batchText>");
			sb.append(batchingMaterialPart.getBatchNumObj() != null ? batchingMaterialPart.getBatchNumObj().getBatchText() : "");//物料批号Obj
			sb.append("</batchNumObj.batchText>");

			sb.append("<container>");
			sb.append(batchingMaterialPart.getContainer() != null ? batchingMaterialPart.getContainer() : "");
			sb.append("</container>");
			sb.append("</BatchingMaterialPart>");
			xml = forntXml + sb.toString() + backXml;
			log.info(xml);
		}
		return xml;
	}

	private String printDataIds;
	public String getPrintDataIds() {
		return printDataIds;
	}
	public void setPrintDataIds(String printDataIds) {
		this.printDataIds = printDataIds;
	}
	/*
	 * * 打印后，相应数据打印次数+1
	 */
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/addPrintCount", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"map" }) })
	public String addPrintCount() throws Exception {
		System.out.println(printDataIds);
		batchingMaterialPartService.addPrintCount(printDataIds);
		map.put("dealSuccessFlag", true);
		return SUCCESS;
	}
	...
	public String getBatchNumObj() {
		return batchNumObj;
	}

	public void setBatchNumObj(String batchNumObj) {
	 ...
	 * * 检查是否编辑
	 */
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/checkEdited", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"map" }) })
	public String checkEdited() throws Exception {
		Map<String, String> params = new HashMap<String, String>();
		params.put("id", partId);
		params.put("wareId", wareId);
		params.put("storeId", storeId);
		params.put("batchNum", batchNum);
		params.put("batchNumObj", batchNumObj);
		params.put("doneNum", doneNum);
		boolean result = batchingMaterialPartService.checkEdit(params);

		map.put("dealSuccessFlag", result);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchingMaterial/batchingMaterialPart/rejectRecordEdit/submitCustom", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	public String rejectRecordSubmitCustom() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterial_BatchingMaterialPart", "submit");
		bap_validate_datagrids=batchingMaterialPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		...
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchingMaterialPartService.saveBatchingMaterialPartCustom(batchingMaterialPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
