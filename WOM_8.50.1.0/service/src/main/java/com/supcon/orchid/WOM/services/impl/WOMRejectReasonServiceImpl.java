...
import com.supcon.orchid.WOM.entities.WOMRejectReasonMneCode;
import com.supcon.orchid.WOM.services.WOMRejectReasonService;
import com.supcon.orchid.WOM.services.WOMRejectReasonImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMRejectReasonDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_rejectReasonService")
@Transactional
public class WOMRejectReasonServiceImpl  extends BaseServiceImpl<WOMRejectReason> implements WOMRejectReasonService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMRejectReason> bapBaseService;
	...
	@Autowired
	private WOMRejectReasonImportService rejectReasonImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMRejectReason.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  rejectReasonDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMRejectReason.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = rejectReasonDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMRejectReason.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMRejectReason.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  rejectReasonDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMRejectReason.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMRejectReason.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMRejectReason> list = (List<WOMRejectReason>) rejectReasonDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMRejectReason getRejectReason(long id){
		return getRejectReason(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMRejectReason getRejectReason(long id, String viewCode){
		WOMRejectReason rejectReason = rejectReasonDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getRejectReasonAsJSON(long id, String include){
		WOMRejectReason rejectReason = rejectReasonDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(rejectReason, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.common.delete",operType="3")
	public void deleteRejectReason(WOMRejectReason rejectReason){
		if(null != rejectReason && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMRejectReason.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
			AuditUtil.setColumnStringA(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
			AuditUtil.setColumnStringC(rejectReason.getId().toString());
		}
		beforeDeleteRejectReason(rejectReason);
		rejectReasonDao.delete(rejectReason);
		afterDeleteRejectReason(rejectReason);
		rejectReasonDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/rejectReason/del", rejectReason,WOMRejectReasonMneCode.class);
		List<WOMRejectReason> params = new ArrayList<WOMRejectReason>();
		params.add(rejectReason);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_materialReject");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.common.delete",operType="3")
	public void deleteRejectReason(List<Long> rejectReasonIds){
		deleteRejectReason(rejectReasonIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.common.delete",operType="3")
	public void deleteRejectReason(List<Long> rejectReasonIds, String eventTopic) {
		List<WOMRejectReason> rejectReasons = new ArrayList<WOMRejectReason>();
		for(Long rejectReasonId : rejectReasonIds){
			WOMRejectReason rejectReason = getRejectReason(rejectReasonId);
			rejectReasons.add(rejectReason);
			if(null != rejectReason && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMRejectReason.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == rejectReason.getId() ? "" : "," + rejectReason.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == rejectReason.getId() ? "" : "," + rejectReason.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == rejectReason.getId() ? "" : "," + rejectReason.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
				}
			}
		}
		
		for(WOMRejectReason rejectReason : rejectReasons){
			beforeDeleteRejectReason(rejectReason);
		}
		...
		if(rejectReasons != null && rejectReasons.size() > 0 ){
			for(WOMRejectReason rejectReason : rejectReasons){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),rejectReason, rejectReason.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(rejectReasonIds != null && rejectReasonIds.size() > 0) {
			String hql = "update " + WOMRejectReason.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = rejectReasonDao.createQuery(hql);
			query.setParameterList("ids", rejectReasonIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMRejectReason rejectReasonz : rejectReasons){
				rejectReasonz.setValid(false);
				List<WOMRejectReason> params = new ArrayList<WOMRejectReason>();
				params.add(rejectReasonz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_materialReject");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.common.delete",operType="3")
	public void deleteRejectReason(long rejectReasonId, int rejectReasonVersion){
		this.deleteRejectReason(rejectReasonId, rejectReasonVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.common.delete",operType="3")
	public void deleteRejectReason(long rejectReasonId, int rejectReasonVersion,SignatureLog signatureLog){
		WOMRejectReason rejectReason = getRejectReason(rejectReasonId);
		if(rejectReason != null && !rejectReason.isValid()){
		...
		beforeDeleteRejectReason(rejectReason);
		rejectReasonDao.delete(rejectReasonId, rejectReasonVersion);
		afterDeleteRejectReason(rejectReason);
		rejectReasonDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/rejectReason/del", rejectReason,WOMRejectReasonMneCode.class);
		List<WOMRejectReason> params = new ArrayList<WOMRejectReason>();
		...
		if(signatureLog != null ) {
			...
			businessKey=rejectReason.getId();
			signatureLog.setTableId(rejectReason.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.common.delete",operType="3")
	public void deleteRejectReason(String rejectReasonIds){
		this.deleteRejectReason(rejectReasonIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.common.delete",operType="3")
	public void deleteRejectReason(String rejectReasonIds,SignatureLog signatureLog){
		deleteCollection(rejectReasonIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteRejectReason(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_materialReject#modelCode:WOM_7.5.0.0_materialReject_RejectReason#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="ec.engine.restore",operType="10")
	public void restoreRejectReason(String rejectReasonIds){
		restoreCollection(rejectReasonIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreRejectReason(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="ec.engine.restore",operType="10")
	public void restoreRejectReason(long rejectReasonId){
		
		findBusinessKeyUsed(rejectReasonId);	//判断业务主键是否重复
		...
		beforeRestoreRejectReason(rejectReason);
		rejectReason.setValid(true);
		rejectReasonDao.update(rejectReason);
		afterRestoreRejectReason(rejectReason);
		rejectReasonDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/rejectReason/add", rejectReason,WOMRejectReasonMneCode.class);
		List<WOMRejectReason> params = new ArrayList<WOMRejectReason>();
		params.add(rejectReason);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long rejectReasonId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_materialReject_RejectReason");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMRejectReason.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMRejectReason.TABLE_NAME +" where id = ? )";
			List<Object> list =  rejectReasonDao.createNativeQuery(sql,rejectReasonId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseRejectReason(List<WOMRejectReason>  rejectReasons){
		for(WOMRejectReason rejectReason:rejectReasons)  {
			saveRejectReason(rejectReason, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseRejectReason(List<WOMRejectReason>  rejectReasons){
		for(WOMRejectReason rejectReason:rejectReasons)  {
			saveRejectReason(rejectReason, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="ec.flow.save")
	public void saveRejectReason(WOMRejectReason rejectReason, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveRejectReason(rejectReason, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="ec.flow.save")
	public void saveRejectReason(WOMRejectReason rejectReason, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveRejectReason(rejectReason, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateRejectReasonCodes(WOMRejectReason rejectReason) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateRejectReasonCodes(rejectReason, false);
	}
	
	public void generateRejectReasonCodes(WOMRejectReason rejectReason, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateRejectReasonSummarys(WOMRejectReason rejectReason) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateRejectReasonSummarys(rejectReason, false);
	}

	public void generateRejectReasonSummarys(WOMRejectReason rejectReason, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason")
	public void saveRejectReason(WOMRejectReason rejectReason,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveRejectReason(rejectReason, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason")
	public void saveRejectReason(WOMRejectReason rejectReason,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_materialReject";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_materialReject");
		String url = null;
		if(rejectReason.getId() != null && rejectReason.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMRejectReason.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_materialReject/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMRejectReason.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_materialReject/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, rejectReason);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(rejectReason);
		beforeSaveRejectReason(rejectReason, viewIsView);

		if (viewIsView) {
			rejectReasonDao.saveWithRevertVersion(rejectReason);
		} else {
			if(isNew)
				rejectReasonDao.save(rejectReason);
			else
				rejectReasonDao.merge(rejectReason);
		}
		if(null != rejectReason && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMRejectReason.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
			AuditUtil.setColumnStringA(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
		...
		dealDatagridsSave(rejectReason,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateRejectReasonCodes(rejectReason, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateRejectReasonSummarys(rejectReason, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveRejectReason(rejectReason, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, rejectReason);
			}
		}
		List<WOMRejectReason> params = new ArrayList<WOMRejectReason>();
		rejectReasonDao.flush();
		rejectReasonDao.clear();
		rejectReason = rejectReasonDao.load(rejectReason.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMRejectReason rejectReason){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason")
	public void mergeRejectReason(WOMRejectReason rejectReason, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(rejectReason);
		beforeSaveRejectReason(rejectReason);
		rejectReasonDao.merge(rejectReason);
		if(null != rejectReason && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMRejectReason.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
			AuditUtil.setColumnStringA(null == rejectReason.getId() ? "" : rejectReason.getId().toString());
		...
		dealDatagridsSave(rejectReason,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveRejectReason(rejectReason);
		rejectReasonDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/rejectReason/add", rejectReason,WOMRejectReasonMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMRejectReason> findRejectReasons(Page<WOMRejectReason> page, Criterion... criterions) {
		return rejectReasonDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMRejectReasonDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMRejectReasonDealInfo> list=rejectReasonDao.createQuery("from " + WOMRejectReasonDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMRejectReasonDealInfo dealInfo) {
		rejectReasonDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMRejectReasonDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) rejectReasonDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMRejectReasonDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = rejectReasonDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = rejectReasonDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMRejectReasonDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = rejectReasonDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRejectReasons(Page<WOMRejectReason> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findRejectReasons(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRejectReasons(Page<WOMRejectReason> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRejectReasons(Page<WOMRejectReason> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMRejectReason> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"rejectReason\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"rejectReason\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"rejectReason\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_materialReject_RejectReason" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_materialReject_RejectReason" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  rejectReasonDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  rejectReasonDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_materialReject").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rejectReason\"","WOM_7.5.0.0_materialReject",customCondition,"WOM_REJECT_REASONS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"rejectReason\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMRejectReasonDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"rejectReason\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"rejectReason\".LAY_REC ASC, \"rejectReason\".SORT ASC");
				} else {
					orderPart.append(" \"rejectReason\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) rejectReasonDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=rejectReasonDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = rejectReasonDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMRejectReason> result = new ArrayList<WOMRejectReason>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMRejectReason.class,rejectReasonDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMRejectReason.class,rejectReasonDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMRejectReason rejectReason : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(rejectReason.getId(), "WOM_materialReject_rejectReason");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(rejectReason.getId(), "WOM_materialReject_rejectReason");
							if(null != documents && !documents.isEmpty()) {
								rejectReason.setDocument(documents.get(0));
								rejectReason.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_materialReject")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_materialReject")+"WOM_7.5.0.0_materialReject".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_materialReject",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMRejectReason> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMRejectReason> result) {
		View view = null;
		...
		Map<Long, WOMRejectReason> rsMap = new HashMap<Long, WOMRejectReason>();
		for (WOMRejectReason rejectReason : result) {
			ids.add(rejectReason.getId());
			rsMap.put(rejectReason.getId(), rejectReason);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(rejectReasonDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = rejectReasonDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMRejectReason rejectReason,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(rejectReason.getOwnerStaffId());
		ti.setOwnerPositionId(rejectReason.getOwnerPositionId());
		ti.setOwnerDepartmentId(rejectReason.getOwnerDepartmentId());
		
		ti.setCreateStaff(rejectReason.getCreateStaff());
		ti.setCreateStaffId(rejectReason.getCreateStaffId());
		ti.setCreateTime(rejectReason.getCreateTime());
		ti.setDeleteStaff(rejectReason.getDeleteStaff());
		ti.setDeleteStaffId(rejectReason.getDeleteStaffId());
		ti.setDeleteTime(rejectReason.getDeleteTime());
		ti.setCreateDepartmentId(rejectReason.getCreateDepartmentId());
		ti.setModifyStaff(rejectReason.getModifyStaff());
		ti.setModifyStaffId(rejectReason.getModifyStaffId());
		ti.setCreatePositionId(rejectReason.getCreatePositionId());
		ti.setPositionLayRec(rejectReason.getPositionLayRec());
		ti.setTableNo(rejectReason.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_materialReject");
		ti.setTargetTableName("WOM_REJECT_REASONS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_materialReject_RejectReason" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  rejectReasonDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rejectReason\"","WOM_7.5.0.0_materialReject","","WOM_REJECT_REASONS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"rejectReason\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"rejectReason\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rejectReason\"","WOM_7.5.0.0_materialReject","","WOM_REJECT_REASONS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("rejectReason.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"rejectReason\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"rejectReason\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"rejectReason\".CID = 1 OR \"rejectReason\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("rejectReason.mc\"").append(".REJECT_REASON) not in ("+notinValue+")");
			}
			
			SQLQuery query = rejectReasonDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("materialReject", "WOM_7.5.0.0_materialReject", "materialReject_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=rejectReasonDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMRejectReason.ENTITY_CODE;}
	protected String getModuleCode(){return WOMRejectReason.MODULE_CODE;}
	protected String getModelCode(){return WOMRejectReason.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param rejectReason 模型实例
	 */
	public void checkUniqueConstraint(WOMRejectReason rejectReason){
	}
	
	
	@Override
	public List<WOMRejectReason> getRejectReasons(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMRejectReason.class);
		if(params!=null){
			...
			return rejectReasonDao.findByHql("from " + WOMRejectReason.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return rejectReasonDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMRejectReason> findRejectReasonsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMRejectReason.class);
	...
	@Override
	public List<WOMRejectReason> findRejectReasonsByHql(String hql, Object... objects){
		
		return   rejectReasonDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMRejectReason loadRejectReasonByBussinessKey(WOMRejectReason rejectReason){
		return loadRejectReasonByBussinessKey(rejectReason.getId());
	}
	
	@Override
	public Page<WOMRejectReason> getRejectReasons(Page<WOMRejectReason> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMRejectReason.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = rejectReasonDao.findByPage(page, sql);
			}else{
				page = rejectReasonDao.findAllByPage(page);
			}
			return page;
		}
		page = rejectReasonDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMRejectReason loadRejectReasonByBussinessKey(Serializable bussinessKey){
		return rejectReasonDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMRejectReason> findByProperty(String propertyName, Object object){
		return rejectReasonDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMRejectReason findEntityByProperty(String propertyName, Object object){
		return rejectReasonDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMRejectReason.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = rejectReasonDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_materialReject_RejectReason","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_materialReject", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMRejectReason rejectReason,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,rejectReasonDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMRejectReason.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, rejectReason);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",rejectReason.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMRejectReason.TABLE_NAME +" where valid = 1";
			list = rejectReasonDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMRejectReason.TABLE_NAME;
			list = rejectReasonDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMRejectReason.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = rejectReasonDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMRejectReasonDaoImpl rejectReasonDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchRejectReason(final List<WOMRejectReason> insertObjs, final List<WOMRejectReason> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchRejectReason(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchRejectReason(final List<WOMRejectReason> insertObjs, final List<WOMRejectReason> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1543218738977"),importCount));
		...
		Map<Long, WOMRejectReason> oldUpdateObjsMap = new HashMap<Long, WOMRejectReason>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMRejectReason bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMRejectReason> bizObjList = rejectReasonDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMRejectReason bizObj = bizObjList.get(mIndex);
		...
		final List<WOMRejectReason> nmeObjs = new ArrayList<WOMRejectReason>();
		...
		long id=autoGeneratorID.getNextId(WOMRejectReason.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMRejectReason rejectReason:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMRejectReason.TABLE_NAME,1,dbId);
			if(rejectReason.getId() != null ){
				m.put(rejectReason.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) rejectReasonDao.createNativeQuery(sql, "WOM_7.5.0.0_materialReject_RejectReason"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		rejectReasonDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMRejectReason.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMRejectReason rejectReason = updateObjs.get(updateIndex);
						Long id = rejectReason.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = rejectReason.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(rejectReason);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMRejectReason.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMRejectReason.TABLE_NAME + " order by id desc";
//					Query query = rejectReasonDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMRejectReason rejectReason = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = rejectReason.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = WOMRejectReasonMneCode.class;
					...
					for(WOMRejectReason rejectReason:updateObjs)  {
						Long id = rejectReason.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(WOMRejectReason rejectReason:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(rejectReason);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, rejectReason.getId());
		...
		rejectReasonDaoImpl.getSession().flush();
		rejectReasonDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMRejectReason bizObj = insertObjs.get(insertIndex);
			rejectReasonImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMRejectReason bizObj = updateObjs.get(updateIndex);
			WOMRejectReason oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			rejectReasonImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = rejectReasonDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				rejectReasonDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = rejectReasonDao.createNativeQuery(sql, "WOM_7.5.0.0_materialReject_RejectReason").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = rejectReasonDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = rejectReasonDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = rejectReasonDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?rejectReasonDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  rejectReasonDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,rejectReasonDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="ec.flow.save")
	public void saveRejectReason(WOMRejectReason rejectReason, DataGridService dataGridService){
		saveRejectReason(rejectReason, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="ec.flow.save")
	public void saveRejectReason(WOMRejectReason rejectReason, DataGridService dataGridService, String viewCode){
		saveRejectReason(rejectReason, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason")
	public void saveRejectReason(WOMRejectReason rejectReason, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveRejectReason(rejectReason, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason")
	public void saveRejectReason(WOMRejectReason rejectReason, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_materialReject";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_materialReject");
		String url = null;
		if(null != rejectReason.getId() && rejectReason.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMRejectReason.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason")
	public Map<Object, Object> rejectReasonDataGridImport(WOMRejectReason rejectReason, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = rejectReason.getVirtualId();
		
		rejectReason = rejectReasonDao.load(rejectReason.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, rejectReason.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = rejectReason.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(rejectReason);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, rejectReason.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason")
	public void mergeRejectReason(WOMRejectReason rejectReason, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(rejectReason);
		beforeSaveRejectReason(rejectReason);
		rejectReasonDao.merge(rejectReason);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_materialReject",model="WOM_7.5.0.0_materialReject_RejectReason",desc="foundation.batch.save",operType="4")
	public void batchSaveRejectReason(WOMRejectReason rejectReason, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_materialReject";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_materialReject");
	...
	@Override
	public Page<WOMRejectReason> getByPage(Page<WOMRejectReason> page,DetachedCriteria detachedCriteria){
		return rejectReasonDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_materialReject").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveRejectReason(WOMRejectReason rejectReason, Object... objects){
		checkUniqueConstraint(rejectReason);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveRejectReason(WOMRejectReason rejectReason, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportRejectReason(List<WOMRejectReason> rejectReasons, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportRejectReason(List<WOMRejectReason> rejectReasons){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportRejectReason(List<WOMRejectReason> rejectReasons, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportRejectReason(List<WOMRejectReason> rejectReasons){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteRejectReason(WOMRejectReason rejectReason){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteRejectReason(WOMRejectReason rejectReason){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreRejectReason(WOMRejectReason rejectReason){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreRejectReason(WOMRejectReason rejectReason){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMRejectReason rejectReason, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMRejectReason rejectReason, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportRejectReason(List<WOMRejectReason> insertObjs, List<WOMRejectReason> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportRejectReason,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportRejectReason(List<WOMRejectReason> insertObjs, List<WOMRejectReason> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportRejectReason,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_materialReject_RejectReason,WOM_7.5.0.0) */
// 自定义代码
