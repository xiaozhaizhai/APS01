...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchChargeDetailsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchChargeDetails> {
	...
	private WOMBatchChargeDetails batchChargeDetails;
	...
	//导出excel时,主模型内容
	List<WOMBatchChargeDetails> queryResults = new ArrayList<WOMBatchChargeDetails>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/batchChargeDetails/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/batchChargeDetails/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_BatchChargeDetails" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchChargeDetails> batchChargeDetailss;
	
	public List<WOMBatchChargeDetails> getBatchChargeDetailss() {
		return batchChargeDetailss;
	}
	...
	private Page<WOMBatchChargeDetails> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchChargeDetails = new WOMBatchChargeDetails();
		} else {
			if(id > 0){
				batchChargeDetails = batchChargeDetailsService.getBatchChargeDetails(id, viewCode);
			}
		}
	...
	public Page<WOMBatchChargeDetails> getPage() {
		if (null == page) {
			page = new Page<WOMBatchChargeDetails>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchChargeDetails getBatchChargeDetails() {
		return batchChargeDetails;
	}		

	public void setBatchChargeDetails(WOMBatchChargeDetails batchChargeDetails) {
		this.batchChargeDetails = batchChargeDetails;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/batchChargeDetails/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchChargeDetailsService.getBatchChargeDetailsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/batchChargeDetails/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1543547557562
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchChargeDetailsReferenceCopy() throws Exception{
			WOMBatchChargeDetails oldBatchChargeDetails = null;
			if(isEditNew()){
				...
				oldBatchChargeDetails = new WOMBatchChargeDetails();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchChargeDetails.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchChargeDetails.getId());
				...
				oldBatchChargeDetails = batchChargeDetailsService.getBatchChargeDetails(id);
			}
			
			
			batchChargeDetails = batchChargeDetailsService.getBatchChargeDetails(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_BatchChargeDetails");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchChargeDetails.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchChargeDetails, null);
						Method setValue = WOMBatchChargeDetails.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchChargeDetails, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchChargeDetails.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchChargeDetailsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchChargeDetailsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchChargeDetails = batchChargeDetailsService.getBatchChargeDetails(this.batchChargeDetails.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchChargeDetails.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchChargeDetailsService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchChargeDetailsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchChargeDetailsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchChargeDetailsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_BatchChargeDetails", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchChargeDetails\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchChargeDetails\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchChargeDetails\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchChargeDetails\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchChargeDetailsService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchChargeDetailsService.findDataGridPage(dg,dgclass,getDgPage(),batchChargeDetails,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchChargeDetails.getId() != null && batchChargeDetails.getId() != -1){
				batchChargeDetails = batchChargeDetailsService.getBatchChargeDetails(batchChargeDetails.getId());
			}
			batchChargeDetailsService.findDataGridPage(dg,dgclass,getDgPage(),batchChargeDetails,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchChargeDetails.getId()) {
		...
		batchChargeDetailsService.saveBatchChargeDetails(batchChargeDetails,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "submit");
		bap_validate_datagrids=batchChargeDetailsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchChargeDetails.getId());
		...
		fileuploads.put("linkId", batchChargeDetails.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/batchChargeDetails/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchChargeDetailsService.deleteBatchChargeDetails(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/batchChargeDetails/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "restore");
		batchChargeDetailsService.restoreBatchChargeDetails(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchChargeDetails> findExportDataInfos(Page<WOMBatchChargeDetails> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchChargeDetailsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchChargeDetailsService.getAssProperty(p.getCode());	
									//String assModelCode = batchChargeDetailsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchChargeDetailsService.findBatchChargeDetailss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchChargeDetailsService.findBatchChargeDetailss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchChargeDetails> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchChargeDetails result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchChargeDetails || null == this.batchChargeDetails.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/batchChargeDetails/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/batchChargeDetails/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/batchChargeDetails/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/batchChargeDetails/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchChargeDetailsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/batchChargeDetails/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_BatchChargeDetails", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_BatchChargeDetails", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/batchChargeDetails/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/batchChargeDetails/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_BatchChargeDetails";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_BatchChargeDetails", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/batchChargeDetails/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMBatchChargeDetails batchChargeDetails = null;
		if(null != id) {
			batchChargeDetails = batchChargeDetailsService.getBatchChargeDetails(id);
		}
		...
		_auditLog.setColumnStringB(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
		_auditLog.setColumnStringA(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
		_auditLog.setColumnStringC(batchChargeDetails.getId().toString());
		_auditLog.setColumnStringD(WOMBatchChargeDetails.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchChargeDetailsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchChargeDetails,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchChargeDetails,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchChargeDetails);
		...
		printlog.setBusinessId(batchChargeDetails.getId());
		printlog.setBussinessKey(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
		printlog.setMainDisplay(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/batchChargeDetails/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchChargeDetails,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/batchChargeDetails/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchChargeDetailsService.getBatchChargeDetails(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/batchChargeDetails/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/batchChargeDetails/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchChargeDetails,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchChargeDetails = batchChargeDetailsService.getBatchChargeDetails(Long.parseLong(arrId[i]));
				if(null != batchChargeDetails){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchChargeDetails.getId() ? "" : "," + batchChargeDetails.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchChargeDetails.getId() ? "" : "," + batchChargeDetails.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchChargeDetails.getId() ? "" : "," + batchChargeDetails.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchChargeDetails.getId() ? "" : batchChargeDetails.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchChargeDetails,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchChargeDetails);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchChargeDetailsService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/batchChargeDetails/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_BatchChargeDetails");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_BatchChargeDetails")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_BatchChargeDetails");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_BatchChargeDetails");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchChargeDetailsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_BatchChargeDetails,WOM_7.5.0.0) */
// 自定义代码
