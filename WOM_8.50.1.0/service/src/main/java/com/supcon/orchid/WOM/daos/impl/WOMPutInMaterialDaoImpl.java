...
@Repository
public class WOMPutInMaterialDaoImpl extends ExtendedGenericDaoImpl<WOMPutInMaterial, Long> implements WOMPutInMaterialDao {
	@Override
	...
	@Override
	public WOMPutInMaterialDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMPutInMaterialDealInfo dealInfo = new WOMPutInMaterialDealInfo();
	...
	@Override
	public WOMPutInMaterialDealInfo copyAndSaveDealInfo(DealInfo di, WOMPutInMaterial mainObj, Staff dealStaff) {
		WOMPutInMaterialDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMPutInMaterialDealInfo.class);
		...
		List<WOMPutInMaterialDealInfo> dealinfo = criteria.list();
	...
	public void addSupervision(Supervision supervision) {
		...
		WOMPutInMaterialSupervision WOMPutInMaterialSupervision = new WOMPutInMaterialSupervision();
		BeanUtils.copyProperties(supervision, WOMPutInMaterialSupervision);
		WOMPutInMaterialSupervision.setValid(true);
		getSession().save(WOMPutInMaterialSupervision);
	}

	@Override
	public void addSupervision(Supervision supervision, WOMPutInMaterial mainObj) {
		getSession().save(supervision);
		WOMPutInMaterialSupervision localSupervision = new WOMPutInMaterialSupervision();
	...
	public void deleteSupervision(Long tableInfoId, List<Long> idList) {
		...
		List<WOMPutInMaterialSupervision> _supervisions = getSession().createCriteria(WOMPutInMaterialSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.in("staff.id", idList))
				.add(Restrictions.eq("valid", true)).list();
		for (WOMPutInMaterialSupervision item : _supervisions) {
			item.setValid(false);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMPutInMaterialSupervision> findSupervision(Long tableInfoId) {
		List<WOMPutInMaterialSupervision> supervisions = getSession().createCriteria(WOMPutInMaterialSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("valid", true)).addOrder(Order.asc("id")).list();
		return supervisions;
	}
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMPutInMaterialSupervision> findSupervision(Long tableInfoId, Long StaffId) {
		List<WOMPutInMaterialSupervision> supervisions = getSession().createCriteria(WOMPutInMaterialSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("staff.id", StaffId)).add(Restrictions.eq("valid", true)).list();
		return supervisions;
	}
	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_putInMaterial_PutInMaterial,WOM_7.5.0.0) */
// 自定义代码
