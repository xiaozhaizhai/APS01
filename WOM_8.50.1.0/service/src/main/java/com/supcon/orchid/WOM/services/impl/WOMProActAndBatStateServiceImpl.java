...
import com.supcon.orchid.WOM.services.WOMProActAndBatStateImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMProActAndBatStateDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_proActAndBatStateService")
@Transactional
public class WOMProActAndBatStateServiceImpl  extends BaseServiceImpl<WOMProActAndBatState> implements WOMProActAndBatStateService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMProActAndBatState> bapBaseService;
	...
	@Autowired
	private WOMProActAndBatStateImportService proActAndBatStateImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMProActAndBatState.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  proActAndBatStateDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMProActAndBatState.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = proActAndBatStateDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProActAndBatState.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProActAndBatState.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  proActAndBatStateDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMProActAndBatState.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMProActAndBatState.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMProActAndBatState> list = (List<WOMProActAndBatState>) proActAndBatStateDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMProActAndBatState getProActAndBatState(long id){
		return getProActAndBatState(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProActAndBatState getProActAndBatState(long id, String viewCode){
		WOMProActAndBatState proActAndBatState = proActAndBatStateDao.load(id);

		// 一对多情况处理
		if(proActAndBatState != null){
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProActAndBatStateAsJSON(long id, String include){
		WOMProActAndBatState proActAndBatState = proActAndBatStateDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(proActAndBatState, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.common.delete",operType="3")
	public void deleteProActAndBatState(WOMProActAndBatState proActAndBatState){
		if(null != proActAndBatState && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProActAndBatState.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
			AuditUtil.setColumnStringA(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
			AuditUtil.setColumnStringC(proActAndBatState.getId().toString());
		}
		beforeDeleteProActAndBatState(proActAndBatState);
		proActAndBatStateDao.delete(proActAndBatState);
		afterDeleteProActAndBatState(proActAndBatState);
		proActAndBatStateDao.flush();
		List<WOMProActAndBatState> params = new ArrayList<WOMProActAndBatState>();
		params.add(proActAndBatState);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_proActiveAndBatchState");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.common.delete",operType="3")
	public void deleteProActAndBatState(List<Long> proActAndBatStateIds){
		deleteProActAndBatState(proActAndBatStateIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.common.delete",operType="3")
	public void deleteProActAndBatState(List<Long> proActAndBatStateIds, String eventTopic) {
		List<WOMProActAndBatState> proActAndBatStates = new ArrayList<WOMProActAndBatState>();
		for(Long proActAndBatStateId : proActAndBatStateIds){
			WOMProActAndBatState proActAndBatState = getProActAndBatState(proActAndBatStateId);
			proActAndBatStates.add(proActAndBatState);
			if(null != proActAndBatState && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProActAndBatState.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == proActAndBatState.getId() ? "" : "," + proActAndBatState.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == proActAndBatState.getId() ? "" : "," + proActAndBatState.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == proActAndBatState.getId() ? "" : "," + proActAndBatState.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(proActAndBatStates != null && proActAndBatStates.size() > 0 ){
			for(WOMProActAndBatState proActAndBatState : proActAndBatStates){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),proActAndBatState, proActAndBatState.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(proActAndBatStateIds != null && proActAndBatStateIds.size() > 0) {
			String hql = "update " + WOMProActAndBatState.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = proActAndBatStateDao.createQuery(hql);
			query.setParameterList("ids", proActAndBatStateIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMProActAndBatState proActAndBatStatez : proActAndBatStates){
				proActAndBatStatez.setValid(false);
				List<WOMProActAndBatState> params = new ArrayList<WOMProActAndBatState>();
				params.add(proActAndBatStatez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_proActiveAndBatchState");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.common.delete",operType="3")
	public void deleteProActAndBatState(long proActAndBatStateId, int proActAndBatStateVersion){
		this.deleteProActAndBatState(proActAndBatStateId, proActAndBatStateVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.common.delete",operType="3")
	public void deleteProActAndBatState(long proActAndBatStateId, int proActAndBatStateVersion,SignatureLog signatureLog){
		WOMProActAndBatState proActAndBatState = getProActAndBatState(proActAndBatStateId);
		if(proActAndBatState != null && !proActAndBatState.isValid()){
		...
		beforeDeleteProActAndBatState(proActAndBatState);
		proActAndBatStateDao.delete(proActAndBatStateId, proActAndBatStateVersion);
		afterDeleteProActAndBatState(proActAndBatState);
		proActAndBatStateDao.flush();
		List<WOMProActAndBatState> params = new ArrayList<WOMProActAndBatState>();
		params.add(proActAndBatState);
		...
		if(signatureLog != null ) {
			...
			businessKey=proActAndBatState.getId();
			signatureLog.setTableId(proActAndBatState.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.common.delete",operType="3")
	public void deleteProActAndBatState(String proActAndBatStateIds){
		this.deleteProActAndBatState(proActAndBatStateIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.common.delete",operType="3")
	public void deleteProActAndBatState(String proActAndBatStateIds,SignatureLog signatureLog){
		deleteCollection(proActAndBatStateIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProActAndBatState(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_proActiveAndBatchState#modelCode:WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="ec.engine.restore",operType="10")
	public void restoreProActAndBatState(String proActAndBatStateIds){
		restoreCollection(proActAndBatStateIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProActAndBatState(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="ec.engine.restore",operType="10")
	public void restoreProActAndBatState(long proActAndBatStateId){
		
		findBusinessKeyUsed(proActAndBatStateId);	//判断业务主键是否重复
		...
		beforeRestoreProActAndBatState(proActAndBatState);
		proActAndBatState.setValid(true);
		proActAndBatStateDao.update(proActAndBatState);
		afterRestoreProActAndBatState(proActAndBatState);
		List<WOMProActAndBatState> params = new ArrayList<WOMProActAndBatState>();
		params.add(proActAndBatState);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long proActAndBatStateId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMProActAndBatState.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMProActAndBatState.TABLE_NAME +" where id = ? )";
			List<Object> list =  proActAndBatStateDao.createNativeQuery(sql,proActAndBatStateId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProActAndBatState(List<WOMProActAndBatState>  proActAndBatStates){
		for(WOMProActAndBatState proActAndBatState:proActAndBatStates)  {
			saveProActAndBatState(proActAndBatState, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProActAndBatState(List<WOMProActAndBatState>  proActAndBatStates){
		for(WOMProActAndBatState proActAndBatState:proActAndBatStates)  {
			saveProActAndBatState(proActAndBatState, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="ec.flow.save")
	public void saveProActAndBatState(WOMProActAndBatState proActAndBatState, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProActAndBatState(proActAndBatState, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="ec.flow.save")
	public void saveProActAndBatState(WOMProActAndBatState proActAndBatState, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProActAndBatState(proActAndBatState, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProActAndBatStateCodes(WOMProActAndBatState proActAndBatState) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProActAndBatStateCodes(proActAndBatState, false);
	}
	
	public void generateProActAndBatStateCodes(WOMProActAndBatState proActAndBatState, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProActAndBatStateSummarys(WOMProActAndBatState proActAndBatState) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProActAndBatStateSummarys(proActAndBatState, false);
	}

	public void generateProActAndBatStateSummarys(WOMProActAndBatState proActAndBatState, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState")
	public void saveProActAndBatState(WOMProActAndBatState proActAndBatState,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProActAndBatState(proActAndBatState, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState")
	public void saveProActAndBatState(WOMProActAndBatState proActAndBatState,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_proActiveAndBatchState";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_proActiveAndBatchState");
		String url = null;
		if(proActAndBatState.getId() != null && proActAndBatState.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProActAndBatState.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_proActiveAndBatchState/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProActAndBatState.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_proActiveAndBatchState/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, proActAndBatState);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(proActAndBatState);
		beforeSaveProActAndBatState(proActAndBatState, viewIsView);

		if (viewIsView) {
			proActAndBatStateDao.saveWithRevertVersion(proActAndBatState);
		} else {
			if(isNew)
				proActAndBatStateDao.save(proActAndBatState);
			else
				proActAndBatStateDao.merge(proActAndBatState);
		}
		if(null != proActAndBatState && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProActAndBatState.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
			AuditUtil.setColumnStringA(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
		...
		dealDatagridsSave(proActAndBatState,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProActAndBatStateCodes(proActAndBatState, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProActAndBatStateSummarys(proActAndBatState, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProActAndBatState(proActAndBatState, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, proActAndBatState);
			}
		}
		List<WOMProActAndBatState> params = new ArrayList<WOMProActAndBatState>();
		proActAndBatStateDao.flush();
		proActAndBatStateDao.clear();
		proActAndBatState = proActAndBatStateDao.load(proActAndBatState.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMProActAndBatState proActAndBatState){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("proActAndBatState", proActAndBatState);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState")
	public void mergeProActAndBatState(WOMProActAndBatState proActAndBatState, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(proActAndBatState);
		beforeSaveProActAndBatState(proActAndBatState);
		proActAndBatStateDao.merge(proActAndBatState);
		if(null != proActAndBatState && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProActAndBatState.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
			AuditUtil.setColumnStringA(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
		...
		dealDatagridsSave(proActAndBatState,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProActAndBatState(proActAndBatState);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMProActAndBatState> findProActAndBatStates(Page<WOMProActAndBatState> page, Criterion... criterions) {
		return proActAndBatStateDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMProActAndBatStateDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMProActAndBatStateDealInfo> list=proActAndBatStateDao.createQuery("from " + WOMProActAndBatStateDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMProActAndBatStateDealInfo dealInfo) {
		proActAndBatStateDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMProActAndBatStateDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) proActAndBatStateDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMProActAndBatStateDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = proActAndBatStateDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = proActAndBatStateDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMProActAndBatStateDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = proActAndBatStateDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProActAndBatStates(Page<WOMProActAndBatState> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProActAndBatStates(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProActAndBatStates(Page<WOMProActAndBatState> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProActAndBatStates(Page<WOMProActAndBatState> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMProActAndBatState> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"proActAndBatState\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"proActAndBatState\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"proActAndBatState\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  proActAndBatStateDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  proActAndBatStateDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_proActiveAndBatchState").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"proActAndBatState\"","WOM_7.5.0.0_proActiveAndBatchState",customCondition,"WOM_PRO_ACT_AND_BAT_STATES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"proActAndBatState\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMProActAndBatStateDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"proActAndBatState\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"proActAndBatState\".LAY_REC ASC, \"proActAndBatState\".SORT ASC");
				} else {
					orderPart.append(" \"proActAndBatState\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) proActAndBatStateDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=proActAndBatStateDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = proActAndBatStateDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMProActAndBatState> result = new ArrayList<WOMProActAndBatState>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMProActAndBatState.class,proActAndBatStateDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMProActAndBatState.class,proActAndBatStateDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMProActAndBatState proActAndBatState : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(proActAndBatState.getId(), "WOM_proActiveAndBatchState_proActAndBatState");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(proActAndBatState.getId(), "WOM_proActiveAndBatchState_proActAndBatState");
							if(null != documents && !documents.isEmpty()) {
								proActAndBatState.setDocument(documents.get(0));
								proActAndBatState.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_proActiveAndBatchState")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_proActiveAndBatchState")+"WOM_7.5.0.0_proActiveAndBatchState".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_proActiveAndBatchState",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMProActAndBatState> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMProActAndBatState> result) {
		View view = null;
		...
		Map<Long, WOMProActAndBatState> rsMap = new HashMap<Long, WOMProActAndBatState>();
		for (WOMProActAndBatState proActAndBatState : result) {
			ids.add(proActAndBatState.getId());
			rsMap.put(proActAndBatState.getId(), proActAndBatState);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(proActAndBatStateDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = proActAndBatStateDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMProActAndBatState proActAndBatState,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(proActAndBatState.getOwnerStaffId());
		ti.setOwnerPositionId(proActAndBatState.getOwnerPositionId());
		ti.setOwnerDepartmentId(proActAndBatState.getOwnerDepartmentId());
		
		ti.setCreateStaff(proActAndBatState.getCreateStaff());
		ti.setCreateStaffId(proActAndBatState.getCreateStaffId());
		ti.setCreateTime(proActAndBatState.getCreateTime());
		ti.setDeleteStaff(proActAndBatState.getDeleteStaff());
		ti.setDeleteStaffId(proActAndBatState.getDeleteStaffId());
		ti.setDeleteTime(proActAndBatState.getDeleteTime());
		ti.setCreateDepartmentId(proActAndBatState.getCreateDepartmentId());
		ti.setModifyStaff(proActAndBatState.getModifyStaff());
		ti.setModifyStaffId(proActAndBatState.getModifyStaffId());
		ti.setCreatePositionId(proActAndBatState.getCreatePositionId());
		ti.setPositionLayRec(proActAndBatState.getPositionLayRec());
		ti.setTableNo(proActAndBatState.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_proActiveAndBatchState");
		ti.setTargetTableName("WOM_PRO_ACT_AND_BAT_STATES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  proActAndBatStateDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"proActAndBatState\"","WOM_7.5.0.0_proActiveAndBatchState","","WOM_PRO_ACT_AND_BAT_STATES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"proActAndBatState\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"proActAndBatState\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"proActAndBatState\"","WOM_7.5.0.0_proActiveAndBatchState","","WOM_PRO_ACT_AND_BAT_STATES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("proActAndBatState.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"proActAndBatState\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"proActAndBatState\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"proActAndBatState\".CID = 1 OR \"proActAndBatState\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("proActAndBatState.mc\"").append(".PRO_ACT_AND_BAT_STATE) not in ("+notinValue+")");
			}
			
			SQLQuery query = proActAndBatStateDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("proActiveAndBatchState", "WOM_7.5.0.0_proActiveAndBatchState", "proActiveAndBatchState_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=proActAndBatStateDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMProActAndBatState.ENTITY_CODE;}
	protected String getModuleCode(){return WOMProActAndBatState.MODULE_CODE;}
	protected String getModelCode(){return WOMProActAndBatState.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param proActAndBatState 模型实例
	 */
	public void checkUniqueConstraint(WOMProActAndBatState proActAndBatState){
	}
	
	
	@Override
	public List<WOMProActAndBatState> getProActAndBatStates(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProActAndBatState.class);
		if(params!=null){
			...
			return proActAndBatStateDao.findByHql("from " + WOMProActAndBatState.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return proActAndBatStateDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMProActAndBatState> findProActAndBatStatesByHql(String hql, Object... objects){
		...
		return   proActAndBatStateDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMProActAndBatState loadProActAndBatStateByBussinessKey(WOMProActAndBatState proActAndBatState){
		return loadProActAndBatStateByBussinessKey(proActAndBatState.getId());
	}
	
	@Override
	public Page<WOMProActAndBatState> getProActAndBatStates(Page<WOMProActAndBatState> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProActAndBatState.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = proActAndBatStateDao.findByPage(page, sql);
			}else{
				page = proActAndBatStateDao.findAllByPage(page);
			}
			return page;
		}
		page = proActAndBatStateDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public WOMProActAndBatState loadProActAndBatStateByBussinessKey(Serializable bussinessKey){
		return proActAndBatStateDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProActAndBatState> findByProperty(String propertyName, Object object){
		return proActAndBatStateDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProActAndBatState findEntityByProperty(String propertyName, Object object){
		return proActAndBatStateDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMProActAndBatState.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = proActAndBatStateDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_proActiveAndBatchState", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMProActAndBatState proActAndBatState,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,proActAndBatStateDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMProActAndBatState.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, proActAndBatState);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",proActAndBatState.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProActAndBatState.TABLE_NAME +" where valid = 1";
			list = proActAndBatStateDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProActAndBatState.TABLE_NAME;
			list = proActAndBatStateDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMProActAndBatState.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = proActAndBatStateDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMProActAndBatStateDaoImpl proActAndBatStateDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProActAndBatState(final List<WOMProActAndBatState> insertObjs, final List<WOMProActAndBatState> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProActAndBatState(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProActAndBatState(final List<WOMProActAndBatState> insertObjs, final List<WOMProActAndBatState> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1509445507769"),importCount));
		...
		Map<Long, WOMProActAndBatState> oldUpdateObjsMap = new HashMap<Long, WOMProActAndBatState>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMProActAndBatState bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMProActAndBatState> bizObjList = proActAndBatStateDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMProActAndBatState bizObj = bizObjList.get(mIndex);
		...
		final List<WOMProActAndBatState> nmeObjs = new ArrayList<WOMProActAndBatState>();
		...
		long id=autoGeneratorID.getNextId(WOMProActAndBatState.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMProActAndBatState proActAndBatState:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMProActAndBatState.TABLE_NAME,1,dbId);
			if(proActAndBatState.getId() != null ){
				m.put(proActAndBatState.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) proActAndBatStateDao.createNativeQuery(sql, "WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		proActAndBatStateDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMProActAndBatState.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMProActAndBatState proActAndBatState = updateObjs.get(updateIndex);
						Long id = proActAndBatState.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = proActAndBatState.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(proActAndBatState);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMProActAndBatState.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMProActAndBatState.TABLE_NAME + " order by id desc";
//					Query query = proActAndBatStateDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMProActAndBatState proActAndBatState = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = proActAndBatState.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		proActAndBatStateDaoImpl.getSession().flush();
		proActAndBatStateDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMProActAndBatState bizObj = insertObjs.get(insertIndex);
			proActAndBatStateImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMProActAndBatState bizObj = updateObjs.get(updateIndex);
			WOMProActAndBatState oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			proActAndBatStateImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = proActAndBatStateDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				proActAndBatStateDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = proActAndBatStateDao.createNativeQuery(sql, "WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = proActAndBatStateDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = proActAndBatStateDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = proActAndBatStateDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?proActAndBatStateDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  proActAndBatStateDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,proActAndBatStateDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="ec.flow.save")
	public void saveProActAndBatState(WOMProActAndBatState proActAndBatState, DataGridService dataGridService){
		saveProActAndBatState(proActAndBatState, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="ec.flow.save")
	public void saveProActAndBatState(WOMProActAndBatState proActAndBatState, DataGridService dataGridService, String viewCode){
		saveProActAndBatState(proActAndBatState, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState")
	public void saveProActAndBatState(WOMProActAndBatState proActAndBatState, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProActAndBatState(proActAndBatState, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState")
	public void saveProActAndBatState(WOMProActAndBatState proActAndBatState, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_proActiveAndBatchState";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_proActiveAndBatchState");
		String url = null;
		if(null != proActAndBatState.getId() && proActAndBatState.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProActAndBatState.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState")
	public Map<Object, Object> proActAndBatStateDataGridImport(WOMProActAndBatState proActAndBatState, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = proActAndBatState.getVirtualId();
		
		proActAndBatState = proActAndBatStateDao.load(proActAndBatState.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, proActAndBatState.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = proActAndBatState.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(proActAndBatState);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, proActAndBatState.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState")
	public void mergeProActAndBatState(WOMProActAndBatState proActAndBatState, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(proActAndBatState);
		beforeSaveProActAndBatState(proActAndBatState);
		proActAndBatStateDao.merge(proActAndBatState);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_proActiveAndBatchState",model="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMProActAndBatState> getByPage(Page<WOMProActAndBatState> page,DetachedCriteria detachedCriteria){
		return proActAndBatStateDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_proActiveAndBatchState").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveProActAndBatState(WOMProActAndBatState proActAndBatState, Object... objects){
		checkUniqueConstraint(proActAndBatState);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProActAndBatState(WOMProActAndBatState proActAndBatState, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
     //名称非空且唯一
     long batchStateID=proActAndBatState.getActiveBatchStateID().getId();
     int size1=this.getProActAndBatStates("valid=1 and activeBatchStateID.id="+batchStateID, null).size();
     if(size1>1){
    	 throw new BAPException("该生产检验名称已存在，请重新选择！");
     }       
     if(proActAndBatState.getQuaWithoutPass()){
		  int size = this.getProActAndBatStates("valid=1 and quaWithoutPass=1 and id!="+proActAndBatState.getId(), null).size();
		  if(size>0){
			  throw new BAPException("合格且无需放行为是的数据仅允许设置一条!");
		  }
	 }
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProActAndBatState(WOMProActAndBatState proActAndBatState){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProActAndBatState(WOMProActAndBatState proActAndBatState){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProActAndBatState(WOMProActAndBatState proActAndBatState){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProActAndBatState(WOMProActAndBatState proActAndBatState){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMProActAndBatState proActAndBatState, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMProActAndBatState proActAndBatState, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProActAndBatState(List<WOMProActAndBatState> insertObjs, List<WOMProActAndBatState> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProActAndBatState,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProActAndBatState(List<WOMProActAndBatState> insertObjs, List<WOMProActAndBatState> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProActAndBatState,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
