...
import com.supcon.orchid.WOM.services.WOMBatchChargeImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchChargeDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_batchChargeService")
@Transactional
public class WOMBatchChargeServiceImpl  extends BaseServiceImpl<WOMBatchCharge> implements WOMBatchChargeService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMBatchCharge> bapBaseService;
	...
	@Autowired
	private WOMBatchChargeImportService batchChargeImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchCharge.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchChargeDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchCharge.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchChargeDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchCharge.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchCharge.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchChargeDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchCharge.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchCharge.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchCharge> list = (List<WOMBatchCharge>) batchChargeDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchCharge getBatchCharge(long id){
		return getBatchCharge(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchCharge getBatchCharge(long id, String viewCode){
		WOMBatchCharge batchCharge = batchChargeDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchChargeAsJSON(long id, String include){
		WOMBatchCharge batchCharge = batchChargeDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchCharge, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.common.delete",operType="3")
	public void deleteBatchCharge(WOMBatchCharge batchCharge){
		if(null != batchCharge && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchCharge.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
			AuditUtil.setColumnStringA(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
			AuditUtil.setColumnStringC(batchCharge.getId().toString());
		}
		beforeDeleteBatchCharge(batchCharge);
		batchChargeDao.deletePhysical(batchCharge);
		afterDeleteBatchCharge(batchCharge);
		batchChargeDao.flush();
		List<WOMBatchCharge> params = new ArrayList<WOMBatchCharge>();
		params.add(batchCharge);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.common.delete",operType="3")
	public void deleteBatchCharge(List<Long> batchChargeIds){
		deleteBatchCharge(batchChargeIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.common.delete",operType="3")
	public void deleteBatchCharge(List<Long> batchChargeIds, String eventTopic) {
		List<WOMBatchCharge> batchCharges = new ArrayList<WOMBatchCharge>();
		for(Long batchChargeId : batchChargeIds){
			WOMBatchCharge batchCharge = getBatchCharge(batchChargeId);
			batchCharges.add(batchCharge);
			if(null != batchCharge && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchCharge.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchCharge.getId() ? "" : "," + batchCharge.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchCharge.getId() ? "" : "," + batchCharge.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchCharge.getId() ? "" : "," + batchCharge.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
				}
			}
		}
		
		for(WOMBatchCharge batchCharge : batchCharges){
			beforeDeleteBatchCharge(batchCharge);
		}
		...
		if(batchCharges != null && batchCharges.size() > 0 ){
			for(WOMBatchCharge batchCharge : batchCharges){
				...
				java.lang.reflect.Field[] fields = WOMBatchCharge.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchCharge));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchCharge, batchCharge.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchChargeIds != null && batchChargeIds.size() > 0) {
			String hql = "delete " + WOMBatchCharge.JPA_NAME + " where id in(:ids)";
			Query query = batchChargeDao.createQuery(hql);
			query.setParameterList("ids", batchChargeIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchCharge batchChargez : batchCharges){
				batchChargez.setValid(false);
				List<WOMBatchCharge> params = new ArrayList<WOMBatchCharge>();
				params.add(batchChargez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_procFeedback");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.common.delete",operType="3")
	public void deleteBatchCharge(long batchChargeId, int batchChargeVersion){
		this.deleteBatchCharge(batchChargeId, batchChargeVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.common.delete",operType="3")
	public void deleteBatchCharge(long batchChargeId, int batchChargeVersion,SignatureLog signatureLog){
		WOMBatchCharge batchCharge = getBatchCharge(batchChargeId);
		if(batchCharge != null && !batchCharge.isValid()){
		...
		beforeDeleteBatchCharge(batchCharge);
		batchChargeDao.delete(batchChargeId, batchChargeVersion);
		afterDeleteBatchCharge(batchCharge);
		batchChargeDao.flush();
		List<WOMBatchCharge> params = new ArrayList<WOMBatchCharge>();
		params.add(batchCharge);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchCharge.getId();
			signatureLog.setTableId(batchCharge.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.common.delete",operType="3")
	public void deleteBatchCharge(String batchChargeIds){
		this.deleteBatchCharge(batchChargeIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.common.delete",operType="3")
	public void deleteBatchCharge(String batchChargeIds,SignatureLog signatureLog){
		deleteCollection(batchChargeIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchCharge(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_procFeedback#modelCode:WOM_7.5.0.0_procFeedback_BatchCharge#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="ec.engine.restore",operType="10")
	public void restoreBatchCharge(String batchChargeIds){
		restoreCollection(batchChargeIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchCharge(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="ec.engine.restore",operType="10")
	public void restoreBatchCharge(long batchChargeId){
		
		findBusinessKeyUsed(batchChargeId);	//判断业务主键是否重复
		...
		beforeRestoreBatchCharge(batchCharge);
		batchCharge.setValid(true);
		batchChargeDao.update(batchCharge);
		afterRestoreBatchCharge(batchCharge);
		List<WOMBatchCharge> params = new ArrayList<WOMBatchCharge>();
		params.add(batchCharge);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchChargeId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_procFeedback_BatchCharge");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchCharge.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchCharge.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchChargeDao.createNativeQuery(sql,batchChargeId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchCharge(List<WOMBatchCharge>  batchCharges){
		for(WOMBatchCharge batchCharge:batchCharges)  {
			saveBatchCharge(batchCharge, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchCharge(List<WOMBatchCharge>  batchCharges){
		for(WOMBatchCharge batchCharge:batchCharges)  {
			saveBatchCharge(batchCharge, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="ec.flow.save")
	public void saveBatchCharge(WOMBatchCharge batchCharge, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchCharge(batchCharge, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="ec.flow.save")
	public void saveBatchCharge(WOMBatchCharge batchCharge, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchCharge(batchCharge, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchChargeCodes(WOMBatchCharge batchCharge) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchChargeCodes(batchCharge, false);
	}
	
	public void generateBatchChargeCodes(WOMBatchCharge batchCharge, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchChargeSummarys(WOMBatchCharge batchCharge) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchChargeSummarys(batchCharge, false);
	}

	public void generateBatchChargeSummarys(WOMBatchCharge batchCharge, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge")
	public void saveBatchCharge(WOMBatchCharge batchCharge,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchCharge(batchCharge, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge")
	public void saveBatchCharge(WOMBatchCharge batchCharge,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
		String url = null;
		if(batchCharge.getId() != null && batchCharge.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchCharge.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchCharge);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchCharge);
		beforeSaveBatchCharge(batchCharge, viewIsView);

		if (viewIsView) {
			batchChargeDao.saveWithRevertVersion(batchCharge);
		} else {
			if(isNew)
				batchChargeDao.save(batchCharge);
			else
				batchChargeDao.merge(batchCharge);
		}
		if(null != batchCharge && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchCharge.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
			AuditUtil.setColumnStringA(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
		...
		dealDatagridsSave(batchCharge,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchChargeCodes(batchCharge, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchChargeSummarys(batchCharge, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchCharge(batchCharge, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchCharge);
			}
		}
		List<WOMBatchCharge> params = new ArrayList<WOMBatchCharge>();
		batchChargeDao.flush();
		batchChargeDao.clear();
		batchCharge = batchChargeDao.load(batchCharge.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchCharge batchCharge){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchCharge", batchCharge);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge")
	public void mergeBatchCharge(WOMBatchCharge batchCharge, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchCharge);
		beforeSaveBatchCharge(batchCharge);
		batchChargeDao.merge(batchCharge);
		if(null != batchCharge && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchCharge.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
			AuditUtil.setColumnStringA(null == batchCharge.getId() ? "" : batchCharge.getId().toString());
		...
		dealDatagridsSave(batchCharge,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchCharge(batchCharge);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchCharge> findBatchCharges(Page<WOMBatchCharge> page, Criterion... criterions) {
		return batchChargeDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchCharges(Page<WOMBatchCharge> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchCharges(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchCharges(Page<WOMBatchCharge> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchCharges(Page<WOMBatchCharge> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchCharge> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchCharge\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchCharge\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchCharge\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"batchCharge\".HEADER_ID IN ( SELECT ID FROM WOM_PROC_FEEDBACK_HEADS \"procFeedbackHead\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  batchChargeDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchChargeDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_procFeedback").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchCharge\".LAY_REC ASC, \"batchCharge\".SORT ASC");
				} else {
					orderPart.append(" \"batchCharge\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchChargeDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchChargeDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchChargeDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchCharge> result = new ArrayList<WOMBatchCharge>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchCharge.class,batchChargeDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchCharge.class,batchChargeDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchCharge batchCharge : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchCharge.getId(), "WOM_procFeedback_batchCharge");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchCharge.getId(), "WOM_procFeedback_batchCharge");
							if(null != documents && !documents.isEmpty()) {
								batchCharge.setDocument(documents.get(0));
								batchCharge.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchCharge> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchCharge> result) {
		View view = null;
		...
		Map<Long, WOMBatchCharge> rsMap = new HashMap<Long, WOMBatchCharge>();
		for (WOMBatchCharge batchCharge : result) {
			ids.add(batchCharge.getId());
			rsMap.put(batchCharge.getId(), batchCharge);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchChargeDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchChargeDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_procFeedback_BatchCharge" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_procFeedback_BatchCharge" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchChargeDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchCharge\"","WOM_7.5.0.0_procFeedback","","WOM_BATCH_CHARGES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchCharge\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchCharge\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchCharge\"","WOM_7.5.0.0_procFeedback","","WOM_BATCH_CHARGES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_procFeedback_BatchCharge" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchCharge.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchCharge\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchCharge\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchCharge\".CID = 1 OR \"batchCharge\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchCharge.mc\"").append(".BATCH_CHARGE) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchChargeDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchChargeDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMBatchCharge.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchCharge.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchCharge.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchCharge 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchCharge batchCharge){
	}
	
	
	@Override
	public List<WOMBatchCharge> getBatchCharges(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchCharge.class);
		if(params!=null){
			...
			return batchChargeDao.findByHql("from " + WOMBatchCharge.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchChargeDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMBatchCharge> findBatchChargesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchCharge.class);
	...
	@Override
	public List<WOMBatchCharge> findBatchChargesByHql(String hql, Object... objects){
		
		return   batchChargeDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchCharge loadBatchChargeByBussinessKey(WOMBatchCharge batchCharge){
		return loadBatchChargeByBussinessKey(batchCharge.getId());
	}
	
	@Override
	public Page<WOMBatchCharge> getBatchCharges(Page<WOMBatchCharge> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchCharge.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchChargeDao.findByPage(page, sql);
			}else{
				page = batchChargeDao.findAllByPage(page);
			}
			return page;
		}
		page = batchChargeDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchCharge> findByTreeDataGridProcFeedbackHead(List<WOMBatchCharge> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMBatchCharge batchCharge : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProcFeedbackHead(Page<WOMBatchCharge> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMBatchCharge> batchCharges = dgPage.getResult();
			for(WOMBatchCharge batchCharge : batchCharges){
				
			}
			dgPage.setResult(batchCharges);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchCharge loadBatchChargeByBussinessKey(Serializable bussinessKey){
		return batchChargeDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchCharge> findByProperty(String propertyName, Object object){
		return batchChargeDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchCharge findEntityByProperty(String propertyName, Object object){
		return batchChargeDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchCharge.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchChargeDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_procFeedback_BatchCharge","runtime");
		try {
	...
	public void dealDatagridsSave(WOMBatchCharge batchCharge,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchChargeDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchCharge.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchCharge);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchCharge.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchCharge.TABLE_NAME +" where valid = 1";
			list = batchChargeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchCharge.TABLE_NAME;
			list = batchChargeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchCharge.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchChargeDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchChargeDaoImpl batchChargeDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchCharge(final List<WOMBatchCharge> insertObjs, final List<WOMBatchCharge> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchCharge(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchCharge(final List<WOMBatchCharge> insertObjs, final List<WOMBatchCharge> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1540953150452"),importCount));
		...
		Map<Long, WOMBatchCharge> oldUpdateObjsMap = new HashMap<Long, WOMBatchCharge>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchCharge bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchCharge> bizObjList = batchChargeDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchCharge bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchCharge> nmeObjs = new ArrayList<WOMBatchCharge>();
		...
		long id=autoGeneratorID.getNextId(WOMBatchCharge.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchCharge batchCharge:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchCharge.TABLE_NAME,1,dbId);
			if(batchCharge.getId() != null ){
				m.put(batchCharge.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			batchCharge.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchChargeDao.createNativeQuery(sql, "WOM_7.5.0.0_procFeedback_BatchCharge"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchChargeDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchCharge.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMBatchCharge batchCharge = updateObjs.get(updateIndex);
						Long id = batchCharge.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchCharge.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchCharge.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMBatchCharge.TABLE_NAME + " order by id desc";
//					Query query = batchChargeDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMBatchCharge batchCharge = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchCharge.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchChargeDaoImpl.getSession().flush();
		batchChargeDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchCharge bizObj = insertObjs.get(insertIndex);
			batchChargeImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchCharge bizObj = updateObjs.get(updateIndex);
			WOMBatchCharge oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchChargeImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMBatchCharge> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMBatchCharge> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchChargeDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMBatchCharge> result = getResult(page, query, new PendingResultTransformer(WOMBatchCharge.class,batchChargeDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMBatchCharge model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchChargeDao.createNativeQuery(sql, "WOM_7.5.0.0_procFeedback_BatchCharge").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchChargeDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchChargeDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchChargeDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchChargeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchChargeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchChargeDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="ec.flow.save")
	public void saveBatchCharge(WOMBatchCharge batchCharge, DataGridService dataGridService){
		saveBatchCharge(batchCharge, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="ec.flow.save")
	public void saveBatchCharge(WOMBatchCharge batchCharge, DataGridService dataGridService, String viewCode){
		saveBatchCharge(batchCharge, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge")
	public void saveBatchCharge(WOMBatchCharge batchCharge, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchCharge(batchCharge, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge")
	public void saveBatchCharge(WOMBatchCharge batchCharge, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_procFeedback";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
		String url = null;
		if(null != batchCharge.getId() && batchCharge.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchCharge.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge")
	public Map<Object, Object> batchChargeDataGridImport(WOMBatchCharge batchCharge, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchCharge.getVirtualId();
		
		batchCharge = batchChargeDao.load(batchCharge.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchCharge.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchCharge.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchCharge);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchCharge.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge")
	public void mergeBatchCharge(WOMBatchCharge batchCharge, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchCharge);
		beforeSaveBatchCharge(batchCharge);
		batchChargeDao.merge(batchCharge);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_BatchCharge",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchCharge(WOMBatchCharge batchCharge, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_procFeedback";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
	...
	@Override
	public Page<WOMBatchCharge> getByPage(Page<WOMBatchCharge> page,DetachedCriteria detachedCriteria){
		return batchChargeDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchCharge(WOMBatchCharge batchCharge, Object... objects){
		checkUniqueConstraint(batchCharge);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchCharge(WOMBatchCharge batchCharge, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchCharge(List<WOMBatchCharge> batchCharges, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchCharge(List<WOMBatchCharge> batchCharges){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchCharge(List<WOMBatchCharge> batchCharges, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchCharge(List<WOMBatchCharge> batchCharges){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchCharge(WOMBatchCharge batchCharge){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchCharge(WOMBatchCharge batchCharge){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchCharge(WOMBatchCharge batchCharge){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchCharge(WOMBatchCharge batchCharge){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchCharge batchCharge, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchCharge batchCharge, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchCharge(List<WOMBatchCharge> insertObjs, List<WOMBatchCharge> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchCharge,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchCharge(List<WOMBatchCharge> insertObjs, List<WOMBatchCharge> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchCharge,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_procFeedback_BatchCharge,WOM_7.5.0.0) */
// 自定义代码
