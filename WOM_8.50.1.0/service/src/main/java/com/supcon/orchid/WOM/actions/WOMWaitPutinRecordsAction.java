...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMWaitPutinRecordsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMWaitPutinRecords> {
	...
	private WOMWaitPutinRecords waitPutinRecords;
	...
	//导出excel时,主模型内容
	List<WOMWaitPutinRecords> queryResults = new ArrayList<WOMWaitPutinRecords>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/waitPutinRecords/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/waitPutinRecords/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_WaitPutinRecords" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMWaitPutinRecords> waitPutinRecordss;
	
	public List<WOMWaitPutinRecords> getWaitPutinRecordss() {
		return waitPutinRecordss;
	}
	...
	private Page<WOMWaitPutinRecords> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			waitPutinRecords = new WOMWaitPutinRecords();
			if(waitPutinRecords.getActiveSource() != null) {
				waitPutinRecords.setActiveSource((SystemCode) systemCodeService.getSystemCode(waitPutinRecords.getActiveSource().getId()));
			}
			if(waitPutinRecords.getActiveType() != null) {
				waitPutinRecords.setActiveType((SystemCode) systemCodeService.getSystemCode(waitPutinRecords.getActiveType().getId()));
			}
		} else {
			if(id > 0){
				waitPutinRecords = waitPutinRecordsService.getWaitPutinRecords(id, viewCode);
			}
		}
	...
	public Page<WOMWaitPutinRecords> getPage() {
		if (null == page) {
			page = new Page<WOMWaitPutinRecords>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMWaitPutinRecords getWaitPutinRecords() {
		return waitPutinRecords;
	}		

	public void setWaitPutinRecords(WOMWaitPutinRecords waitPutinRecords) {
		this.waitPutinRecords = waitPutinRecords;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/waitPutinRecords/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = waitPutinRecordsService.getWaitPutinRecordsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/waitPutinRecords/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1496884906781
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String waitPutinRecordsReferenceCopy() throws Exception{
			WOMWaitPutinRecords oldWaitPutinRecords = null;
			if(isEditNew()){
				...
				oldWaitPutinRecords = new WOMWaitPutinRecords();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,waitPutinRecords.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, waitPutinRecords.getId());
				...
				oldWaitPutinRecords = waitPutinRecordsService.getWaitPutinRecords(id);
			}
			
			
			waitPutinRecords = waitPutinRecordsService.getWaitPutinRecords(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_WaitPutinRecords");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMWaitPutinRecords.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldWaitPutinRecords, null);
						Method setValue = WOMWaitPutinRecords.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(waitPutinRecords, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			waitPutinRecords.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=waitPutinRecordsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return waitPutinRecordsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			waitPutinRecords = waitPutinRecordsService.getWaitPutinRecords(this.waitPutinRecords.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,waitPutinRecords.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			waitPutinRecordsService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = waitPutinRecordsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(waitPutinRecordsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(waitPutinRecordsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_WaitPutinRecords", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"waitPutinRecords\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"waitPutinRecords\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"waitPutinRecords\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"waitPutinRecords\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			waitPutinRecordsService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		waitPutinRecordsService.findDataGridPage(dg,dgclass,getDgPage(),waitPutinRecords,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(waitPutinRecords.getId() != null && waitPutinRecords.getId() != -1){
				waitPutinRecords = waitPutinRecordsService.getWaitPutinRecords(waitPutinRecords.getId());
			}
			waitPutinRecordsService.findDataGridPage(dg,dgclass,getDgPage(),waitPutinRecords,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == waitPutinRecords.getId()) {
		...
		waitPutinRecordsService.saveWaitPutinRecords(waitPutinRecords,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "submit");
		bap_validate_datagrids=waitPutinRecordsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", waitPutinRecords.getId());
		...
		fileuploads.put("linkId", waitPutinRecords.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceTask/waitPutinRecords/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		waitPutinRecordsService.deleteWaitPutinRecords(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/waitPutinRecords/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "restore");
		waitPutinRecordsService.restoreWaitPutinRecords(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMWaitPutinRecords> findExportDataInfos(Page<WOMWaitPutinRecords> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = waitPutinRecordsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = waitPutinRecordsService.getAssProperty(p.getCode());	
									//String assModelCode = waitPutinRecordsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			waitPutinRecordsService.findWaitPutinRecordss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			waitPutinRecordsService.findWaitPutinRecordss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMWaitPutinRecords> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMWaitPutinRecords result : results){
	...
	public boolean isEditNew(){
		//return null == this.waitPutinRecords || null == this.waitPutinRecords.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/waitPutinRecords/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/waitPutinRecords/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/waitPutinRecords/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/waitPutinRecords/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMWaitPutinRecordsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/waitPutinRecords/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_WaitPutinRecords", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_WaitPutinRecords", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/waitPutinRecords/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/waitPutinRecords/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_WaitPutinRecords";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_WaitPutinRecords", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/waitPutinRecords/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
		_auditLog.setColumnStringA(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
		_auditLog.setColumnStringC(waitPutinRecords.getId().toString());
		_auditLog.setColumnStringD(WOMWaitPutinRecords.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMWaitPutinRecordsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, waitPutinRecords,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, waitPutinRecords,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,waitPutinRecords);
		...
		printlog.setBusinessId(waitPutinRecords.getId());
		printlog.setBussinessKey(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
		printlog.setMainDisplay(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceTask/waitPutinRecords/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(waitPutinRecords,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/waitPutinRecords/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(waitPutinRecordsService.getWaitPutinRecords(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceTask/waitPutinRecords/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/waitPutinRecords/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, waitPutinRecords,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				waitPutinRecords = waitPutinRecordsService.getWaitPutinRecords(Long.parseLong(arrId[i]));
				if(null != waitPutinRecords){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == waitPutinRecords.getId() ? "" : "," + waitPutinRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == waitPutinRecords.getId() ? "" : "," + waitPutinRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == waitPutinRecords.getId() ? "" : "," + waitPutinRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == waitPutinRecords.getId() ? "" : waitPutinRecords.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, waitPutinRecords,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,waitPutinRecords);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//waitPutinRecordsService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/waitPutinRecords/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_WaitPutinRecords");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceTask_WaitPutinRecords")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceTask_WaitPutinRecords");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_WaitPutinRecords");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = waitPutinRecordsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_WaitPutinRecords,WOM_7.5.0.0) */
// 自定义代码
