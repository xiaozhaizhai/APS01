...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMPutInMaterialPartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMPutInMaterialPart> {
	...
	private WOMPutInMaterialPart putInMaterialPart;
	...
	//导出excel时,主模型内容
	List<WOMPutInMaterialPart> queryResults = new ArrayList<WOMPutInMaterialPart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/putInMaterial/putInMaterialPart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/putInMaterial/putInMaterialPart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_putInMaterial_PutInMaterialPart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMPutInMaterialPart> putInMaterialParts;
	
	public List<WOMPutInMaterialPart> getPutInMaterialParts() {
		return putInMaterialParts;
	}
	...
	private Page<WOMPutInMaterialPart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			putInMaterialPart = new WOMPutInMaterialPart();
			if(putInMaterialPart.getPutinType() != null) {
				putInMaterialPart.setPutinType((SystemCode) systemCodeService.getSystemCode(putInMaterialPart.getPutinType().getId()));
			}
		} else {
			if(id > 0){
				putInMaterialPart = putInMaterialPartService.getPutInMaterialPart(id, viewCode);
			}
		}
	...
	public Page<WOMPutInMaterialPart> getPage() {
		if (null == page) {
			page = new Page<WOMPutInMaterialPart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMPutInMaterialPart getPutInMaterialPart() {
		return putInMaterialPart;
	}		

	public void setPutInMaterialPart(WOMPutInMaterialPart putInMaterialPart) {
		this.putInMaterialPart = putInMaterialPart;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = putInMaterialPartService.getPutInMaterialPartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1490604876056
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String putInMaterialPartReferenceCopy() throws Exception{
			WOMPutInMaterialPart oldPutInMaterialPart = null;
			if(isEditNew()){
				...
				oldPutInMaterialPart = new WOMPutInMaterialPart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,putInMaterialPart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, putInMaterialPart.getId());
				...
				oldPutInMaterialPart = putInMaterialPartService.getPutInMaterialPart(id);
			}
			
			
			putInMaterialPart = putInMaterialPartService.getPutInMaterialPart(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterial_PutInMaterialPart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMPutInMaterialPart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPutInMaterialPart, null);
						Method setValue = WOMPutInMaterialPart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(putInMaterialPart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			putInMaterialPart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=putInMaterialPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return putInMaterialPartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			putInMaterialPart = putInMaterialPartService.getPutInMaterialPart(this.putInMaterialPart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,putInMaterialPart.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			putInMaterialPartService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterialPart/putInPartList", results = { @Result(type = FREEMARKER, location = "putInPartList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = putInMaterialPartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(putInMaterialPartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(putInMaterialPartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterialPart/putinRef", results = { @Result(type = FREEMARKER, location = "putinRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterialPart/putInPartList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headId.taskID.tableNo,result.headId.taskID.productId.productCode,result.headId.taskID.productId.productName,result.headId.taskID.productBatchNum,result.productId.productCode,result.productId.productName,result.batchNum,result.putinStaff.name,result.putinDate,result.putinEndDate,result.putinType.id,result.putinType.value,result.taskProcessId.name,result.factoryId.name,result.taskActiveId.name,result.useNum,result.lossNum,result.remark,result.headId.taskID.id,result.headId.taskID.productId.id,result.productId.id,result.putinStaff.id,result.taskProcessId.id,result.factoryId.id,result.taskActiveId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="putInPartList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"putInMaterialPart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"putInMaterialPart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"putInMaterialPart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"putInMaterialPart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			putInMaterialPartService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterialPart/putInPartList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterialPart/putinRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headId.tableNo,result.productId.productCode,result.productId.productName,result.batchNum,result.wareID.wareCode,result.wareID.wareName,result.storeID.placeSetCode,result.storeID.placeSetName,result.useNum,result.lossNum,result.headId.id,result.productId.id,result.wareID.id,result.storeID.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="putinRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		putInMaterialPartService.findDataGridPage(dg,dgclass,getDgPage(),putInMaterialPart,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(putInMaterialPart.getId() != null && putInMaterialPart.getId() != -1){
				putInMaterialPart = putInMaterialPartService.getPutInMaterialPart(putInMaterialPart.getId());
			}
			putInMaterialPartService.findDataGridPage(dg,dgclass,getDgPage(),putInMaterialPart,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == putInMaterialPart.getId()) {
		...
		putInMaterialPartService.savePutInMaterialPart(putInMaterialPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "submit");
		bap_validate_datagrids=putInMaterialPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", putInMaterialPart.getId());
		...
		fileuploads.put("linkId", putInMaterialPart.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/putInMaterial/putInMaterialPart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		putInMaterialPartService.deletePutInMaterialPart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "restore");
		putInMaterialPartService.restorePutInMaterialPart(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMPutInMaterialPart> findExportDataInfos(Page<WOMPutInMaterialPart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = putInMaterialPartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = putInMaterialPartService.getAssProperty(p.getCode());	
									//String assModelCode = putInMaterialPartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			putInMaterialPartService.findPutInMaterialParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			putInMaterialPartService.findPutInMaterialParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMPutInMaterialPart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMPutInMaterialPart result : results){
	...
	public boolean isEditNew(){
		//return null == this.putInMaterialPart || null == this.putInMaterialPart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/putInMaterial/putInMaterialPart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/putInMaterial/putInMaterialPart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMPutInMaterialPartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_putInMaterial_PutInMaterialPart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_putInMaterial_PutInMaterialPart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_putInMaterial_PutInMaterialPart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterialPart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
		_auditLog.setColumnStringA(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
		_auditLog.setColumnStringC(putInMaterialPart.getId().toString());
		_auditLog.setColumnStringD(WOMPutInMaterialPart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMPutInMaterialPartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, putInMaterialPart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, putInMaterialPart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,putInMaterialPart);
		...
		printlog.setBusinessId(putInMaterialPart.getId());
		printlog.setBussinessKey(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
		printlog.setMainDisplay(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(putInMaterialPart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(putInMaterialPartService.getPutInMaterialPart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, putInMaterialPart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				putInMaterialPart = putInMaterialPartService.getPutInMaterialPart(Long.parseLong(arrId[i]));
				if(null != putInMaterialPart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == putInMaterialPart.getId() ? "" : "," + putInMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == putInMaterialPart.getId() ? "" : "," + putInMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == putInMaterialPart.getId() ? "" : "," + putInMaterialPart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, putInMaterialPart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,putInMaterialPart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//putInMaterialPartService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/putInMaterial/putInMaterialPart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterial_PutInMaterialPart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_putInMaterial_PutInMaterialPart")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_putInMaterial_PutInMaterialPart");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterial_PutInMaterialPart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = putInMaterialPartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
    if("WOM_7.5.0.0_putInMaterial_putinRef".equals(code)&&"putinRefQuery".equals(methodName)){ //根据物品过滤数据
			if(putinID!=null&&!"".equals(putinID)){
				String customerSql = "(\"putInMaterialPart\".HEAD_ID in(select ID from WOM_PUT_IN_MATERIALS where ( ID = "+putinID+" and valid = 1)))";
				params.add(new Param("customerSql", customerSql));
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
	//投料单ID
