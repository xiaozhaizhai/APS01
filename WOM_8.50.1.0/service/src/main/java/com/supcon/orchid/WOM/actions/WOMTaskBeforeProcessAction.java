...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMTaskBeforeProcessAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMTaskBeforeProcess> {
	...
	private WOMTaskBeforeProcess taskBeforeProcess;
	...
	//导出excel时,主模型内容
	List<WOMTaskBeforeProcess> queryResults = new ArrayList<WOMTaskBeforeProcess>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/taskBeforeProcess/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceTask/taskBeforeProcess/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceTask_TaskBeforeProcess" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMTaskBeforeProcess> taskBeforeProcesss;
	
	public List<WOMTaskBeforeProcess> getTaskBeforeProcesss() {
		return taskBeforeProcesss;
	}
	...
	private Page<WOMTaskBeforeProcess> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			taskBeforeProcess = new WOMTaskBeforeProcess();
		} else {
			if(id > 0){
				taskBeforeProcess = taskBeforeProcessService.getTaskBeforeProcess(id, viewCode);
			}
		}
	...
	public Page<WOMTaskBeforeProcess> getPage() {
		if (null == page) {
			page = new Page<WOMTaskBeforeProcess>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMTaskBeforeProcess getTaskBeforeProcess() {
		return taskBeforeProcess;
	}		

	public void setTaskBeforeProcess(WOMTaskBeforeProcess taskBeforeProcess) {
		this.taskBeforeProcess = taskBeforeProcess;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/taskBeforeProcess/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = taskBeforeProcessService.getTaskBeforeProcessAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceTask/taskBeforeProcess/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1489733511072
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String taskBeforeProcessReferenceCopy() throws Exception{
			WOMTaskBeforeProcess oldTaskBeforeProcess = null;
			if(isEditNew()){
				...
				oldTaskBeforeProcess = new WOMTaskBeforeProcess();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskBeforeProcess.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, taskBeforeProcess.getId());
				...
				oldTaskBeforeProcess = taskBeforeProcessService.getTaskBeforeProcess(id);
			}
			
			
			taskBeforeProcess = taskBeforeProcessService.getTaskBeforeProcess(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_TaskBeforeProcess");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMTaskBeforeProcess.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTaskBeforeProcess, null);
						Method setValue = WOMTaskBeforeProcess.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(taskBeforeProcess, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=taskBeforeProcessService.findValidateDatagrids(getDgClassMap());
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return taskBeforeProcessReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			taskBeforeProcess = taskBeforeProcessService.getTaskBeforeProcess(this.taskBeforeProcess.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskBeforeProcess.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			taskBeforeProcessService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			List<Object[]> list = taskBeforeProcessService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceTask_TaskBeforeProcess", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"taskBeforeProcess\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"taskBeforeProcess\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			taskBeforeProcessService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		taskBeforeProcessService.findDataGridPage(dg,dgclass,getDgPage(),taskBeforeProcess,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if(refId!=null&&refId>0){
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			taskBeforeProcessService.findDataGridPage(dg,dgclass,getDgPage(),taskBeforeProcess,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == taskBeforeProcess.getId()) {
			responseMap.put("operate", "add");
		} else {
			responseMap.put("operate", "edit");
		}
		taskBeforeProcessService.saveTaskBeforeProcess(taskBeforeProcess,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "submit");
		bap_validate_datagrids=taskBeforeProcessService.findValidateDatagrids(getDgClassMap());
			save();
			responseMap.put("id", taskBeforeProcess.getId());
		...
		fileuploads.put("linkId", taskBeforeProcess.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/produceTask/taskBeforeProcess/delete", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "delete");
		taskBeforeProcessService.deleteTaskBeforeProcess(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "delete");
		return SUCCESS;
	}
	...
	@Override
	public Page<WOMTaskBeforeProcess> findExportDataInfos(Page<WOMTaskBeforeProcess> page) {
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				//if(useForImportFlag){//必填字段一定导出
				    ...
				    List<String> runningCustomPropertyCode = taskBeforeProcessService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
			       	for(Property p : properties){
			       		if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
			       			...
			       			if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
			       				Model assModel = null;
			       				if(!p.getIsCustom()){
									String assProperty = taskBeforeProcessService.getAssProperty(p.getCode());	
									String assModelCode = taskBeforeProcessService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			taskBeforeProcessService.findTaskBeforeProcesss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, searchObjects);
		} else {
			taskBeforeProcessService.findTaskBeforeProcesss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMTaskBeforeProcess> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMTaskBeforeProcess result : results){
	...
	public boolean isEditNew(){
		//return null == this.taskBeforeProcess || null == this.taskBeforeProcess.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceTask/taskBeforeProcess/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceTask/taskBeforeProcess/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceTask/taskBeforeProcess/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/taskBeforeProcess/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMTaskBeforeProcessService"
		};
		//辅模型用到的service也一起放进来
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceTask/taskBeforeProcess/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceTask_TaskBeforeProcess", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceTask_TaskBeforeProcess", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceTask/taskBeforeProcess/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceTask/taskBeforeProcess/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceTask_TaskBeforeProcess";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceTask_TaskBeforeProcess", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceTask/taskBeforeProcess/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		WOMTaskBeforeProcess taskBeforeProcess = null;
		if(null != id) {
			taskBeforeProcess = taskBeforeProcessService.getTaskBeforeProcess(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, taskBeforeProcess, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, taskBeforeProcess, null, null, true, this);
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		taskBeforeProcessService.print(1);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceTask/taskBeforeProcess/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceTask/taskBeforeProcess/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				taskBeforeProcess = taskBeforeProcessService.getTaskBeforeProcess(Integer.parseInt(id));
				if(taskBeforeProcess != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, taskBeforeProcess, null, null, true, this);
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = taskBeforeProcessService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void bapAfterCustomMethod(String code, String methodName) throws Exception {
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceTask_TaskBeforeProcess,WOM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
}