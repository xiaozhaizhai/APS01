...
import com.supcon.orchid.WOM.services.WOMProdTaskProcExelogImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMProdTaskProcExelogDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_prodTaskProcExelogService")
@Transactional
public class WOMProdTaskProcExelogServiceImpl  extends BaseServiceImpl<WOMProdTaskProcExelog> implements WOMProdTaskProcExelogService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMProdTaskProcExelog> bapBaseService;
	...
	@Autowired
	private WOMProdTaskProcExelogImportService prodTaskProcExelogImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMProdTaskProcExelog.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMProdTaskProcExelog.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = prodTaskProcExelogDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProdTaskProcExelog.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProdTaskProcExelog.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  prodTaskProcExelogDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMProdTaskProcExelog.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMProdTaskProcExelog.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMProdTaskProcExelog> list = (List<WOMProdTaskProcExelog>) prodTaskProcExelogDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMProdTaskProcExelog getProdTaskProcExelog(long id){
		return getProdTaskProcExelog(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProdTaskProcExelog getProdTaskProcExelog(long id, String viewCode){
		WOMProdTaskProcExelog prodTaskProcExelog = prodTaskProcExelogDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProdTaskProcExelogAsJSON(long id, String include){
		WOMProdTaskProcExelog prodTaskProcExelog = prodTaskProcExelogDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(prodTaskProcExelog, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.common.delete",operType="3")
	public void deleteProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog){
		if(null != prodTaskProcExelog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProdTaskProcExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
			AuditUtil.setColumnStringA(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
			AuditUtil.setColumnStringC(prodTaskProcExelog.getId().toString());
		}
		beforeDeleteProdTaskProcExelog(prodTaskProcExelog);
		prodTaskProcExelogDao.deletePhysical(prodTaskProcExelog);
		afterDeleteProdTaskProcExelog(prodTaskProcExelog);
		prodTaskProcExelogDao.flush();
		List<WOMProdTaskProcExelog> params = new ArrayList<WOMProdTaskProcExelog>();
		params.add(prodTaskProcExelog);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.common.delete",operType="3")
	public void deleteProdTaskProcExelog(List<Long> prodTaskProcExelogIds){
		deleteProdTaskProcExelog(prodTaskProcExelogIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.common.delete",operType="3")
	public void deleteProdTaskProcExelog(List<Long> prodTaskProcExelogIds, String eventTopic) {
		prodTaskActiExelogService.deleteProdTaskActiExelogByProdTaskProcExelog(prodTaskProcExelogIds);
		List<WOMProdTaskProcExelog> prodTaskProcExelogs = new ArrayList<WOMProdTaskProcExelog>();
		for(Long prodTaskProcExelogId : prodTaskProcExelogIds){
			WOMProdTaskProcExelog prodTaskProcExelog = getProdTaskProcExelog(prodTaskProcExelogId);
			prodTaskProcExelogs.add(prodTaskProcExelog);
			if(null != prodTaskProcExelog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProdTaskProcExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == prodTaskProcExelog.getId() ? "" : "," + prodTaskProcExelog.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == prodTaskProcExelog.getId() ? "" : "," + prodTaskProcExelog.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == prodTaskProcExelog.getId() ? "" : "," + prodTaskProcExelog.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(prodTaskProcExelogs != null && prodTaskProcExelogs.size() > 0 ){
			for(WOMProdTaskProcExelog prodTaskProcExelog : prodTaskProcExelogs){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMProdTaskProcExelog.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(prodTaskProcExelog));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),prodTaskProcExelog, prodTaskProcExelog.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(prodTaskProcExelogIds != null && prodTaskProcExelogIds.size() > 0) {
			String hql = "delete " + WOMProdTaskProcExelog.JPA_NAME + " where id in(:ids)";
			Query query = prodTaskProcExelogDao.createQuery(hql);
			query.setParameterList("ids", prodTaskProcExelogIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMProdTaskProcExelog prodTaskProcExelogz : prodTaskProcExelogs){
				prodTaskProcExelogz.setValid(false);
				List<WOMProdTaskProcExelog> params = new ArrayList<WOMProdTaskProcExelog>();
				params.add(prodTaskProcExelogz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.common.delete",operType="3")
	public void deleteProdTaskProcExelog(long prodTaskProcExelogId, int prodTaskProcExelogVersion){
		this.deleteProdTaskProcExelog(prodTaskProcExelogId, prodTaskProcExelogVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.common.delete",operType="3")
	public void deleteProdTaskProcExelog(long prodTaskProcExelogId, int prodTaskProcExelogVersion,SignatureLog signatureLog){
		WOMProdTaskProcExelog prodTaskProcExelog = getProdTaskProcExelog(prodTaskProcExelogId);
		...
		beforeDeleteProdTaskProcExelog(prodTaskProcExelog);
		prodTaskProcExelogDao.delete(prodTaskProcExelogId, prodTaskProcExelogVersion);
		afterDeleteProdTaskProcExelog(prodTaskProcExelog);
		prodTaskProcExelogDao.flush();
		List<WOMProdTaskProcExelog> params = new ArrayList<WOMProdTaskProcExelog>();
		params.add(prodTaskProcExelog);
		...
		if(signatureLog != null ) {
			...
			businessKey=prodTaskProcExelog.getId();
			signatureLog.setTableId(prodTaskProcExelog.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.common.delete",operType="3")
	public void deleteProdTaskProcExelog(String prodTaskProcExelogIds){
		this.deleteProdTaskProcExelog(prodTaskProcExelogIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.common.delete",operType="3")
	public void deleteProdTaskProcExelog(String prodTaskProcExelogIds,SignatureLog signatureLog){
		deleteCollection(prodTaskProcExelogIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProdTaskProcExelog(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_ProdTaskProcExelog#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="ec.engine.restore",operType="10")
	public void restoreProdTaskProcExelog(String prodTaskProcExelogIds){
		restoreCollection(prodTaskProcExelogIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProdTaskProcExelog(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="ec.engine.restore",operType="10")
	public void restoreProdTaskProcExelog(long prodTaskProcExelogId){
		
		findBusinessKeyUsed(prodTaskProcExelogId);	//判断业务主键是否重复
		...
		beforeRestoreProdTaskProcExelog(prodTaskProcExelog);
		prodTaskProcExelog.setValid(true);
		prodTaskProcExelogDao.update(prodTaskProcExelog);
		afterRestoreProdTaskProcExelog(prodTaskProcExelog);
		List<WOMProdTaskProcExelog> params = new ArrayList<WOMProdTaskProcExelog>();
		params.add(prodTaskProcExelog);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long prodTaskProcExelogId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_ProdTaskProcExelog");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMProdTaskProcExelog.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMProdTaskProcExelog.TABLE_NAME +" where id = ? )";
			List<Object> list =  prodTaskProcExelogDao.createNativeQuery(sql,prodTaskProcExelogId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProdTaskProcExelog(List<WOMProdTaskProcExelog>  prodTaskProcExelogs){
		for(WOMProdTaskProcExelog prodTaskProcExelog:prodTaskProcExelogs)  {
			saveProdTaskProcExelog(prodTaskProcExelog, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProdTaskProcExelog(List<WOMProdTaskProcExelog>  prodTaskProcExelogs){
		for(WOMProdTaskProcExelog prodTaskProcExelog:prodTaskProcExelogs)  {
			saveProdTaskProcExelog(prodTaskProcExelog, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="ec.flow.save")
	public void saveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProdTaskProcExelog(prodTaskProcExelog, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="ec.flow.save")
	public void saveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProdTaskProcExelog(prodTaskProcExelog, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProdTaskProcExelogCodes(WOMProdTaskProcExelog prodTaskProcExelog) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdTaskProcExelogCodes(prodTaskProcExelog, false);
	}
	
	public void generateProdTaskProcExelogCodes(WOMProdTaskProcExelog prodTaskProcExelog, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProdTaskProcExelogSummarys(WOMProdTaskProcExelog prodTaskProcExelog) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdTaskProcExelogSummarys(prodTaskProcExelog, false);
	}

	public void generateProdTaskProcExelogSummarys(WOMProdTaskProcExelog prodTaskProcExelog, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog")
	public void saveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProdTaskProcExelog(prodTaskProcExelog, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog")
	public void saveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(prodTaskProcExelog.getId() != null && prodTaskProcExelog.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProdTaskProcExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, prodTaskProcExelog);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(prodTaskProcExelog);
		beforeSaveProdTaskProcExelog(prodTaskProcExelog, viewIsView);

		if (viewIsView) {
			prodTaskProcExelogDao.saveWithRevertVersion(prodTaskProcExelog);
		} else {
			if(isNew)
				prodTaskProcExelogDao.save(prodTaskProcExelog);
			else
				prodTaskProcExelogDao.merge(prodTaskProcExelog);
		}
		if(null != prodTaskProcExelog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProdTaskProcExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
			AuditUtil.setColumnStringA(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
		...
		dealDatagridsSave(prodTaskProcExelog,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProdTaskProcExelogCodes(prodTaskProcExelog, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProdTaskProcExelogSummarys(prodTaskProcExelog, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProdTaskProcExelog(prodTaskProcExelog, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, prodTaskProcExelog);
			}
		}
		List<WOMProdTaskProcExelog> params = new ArrayList<WOMProdTaskProcExelog>();
		prodTaskProcExelogDao.flush();
		prodTaskProcExelogDao.clear();
		prodTaskProcExelog = prodTaskProcExelogDao.load(prodTaskProcExelog.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMProdTaskProcExelog prodTaskProcExelog){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("prodTaskProcExelog", prodTaskProcExelog);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog")
	public void mergeProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(prodTaskProcExelog);
		beforeSaveProdTaskProcExelog(prodTaskProcExelog);
		prodTaskProcExelogDao.merge(prodTaskProcExelog);
		if(null != prodTaskProcExelog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProdTaskProcExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
			AuditUtil.setColumnStringA(null == prodTaskProcExelog.getId() ? "" : prodTaskProcExelog.getId().toString());
		...
		dealDatagridsSave(prodTaskProcExelog,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProdTaskProcExelog(prodTaskProcExelog);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMProdTaskProcExelog> findProdTaskProcExelogs(Page<WOMProdTaskProcExelog> page, Criterion... criterions) {
		return prodTaskProcExelogDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdTaskProcExelogs(Page<WOMProdTaskProcExelog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProdTaskProcExelogs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdTaskProcExelogs(Page<WOMProdTaskProcExelog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdTaskProcExelogs(Page<WOMProdTaskProcExelog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMProdTaskProcExelog> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"prodTaskProcExelog\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"prodTaskProcExelog\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"prodTaskProcExelog\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"prodTaskProcExelog\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  prodTaskProcExelogDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  prodTaskProcExelogDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"prodTaskProcExelog\".LAY_REC ASC, \"prodTaskProcExelog\".SORT ASC");
				} else {
					orderPart.append(" \"prodTaskProcExelog\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) prodTaskProcExelogDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=prodTaskProcExelogDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = prodTaskProcExelogDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMProdTaskProcExelog> result = new ArrayList<WOMProdTaskProcExelog>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMProdTaskProcExelog.class,prodTaskProcExelogDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMProdTaskProcExelog.class,prodTaskProcExelogDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMProdTaskProcExelog prodTaskProcExelog : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(prodTaskProcExelog.getId(), "WOM_produceTask_prodTaskProcExelog");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(prodTaskProcExelog.getId(), "WOM_produceTask_prodTaskProcExelog");
							if(null != documents && !documents.isEmpty()) {
								prodTaskProcExelog.setDocument(documents.get(0));
								prodTaskProcExelog.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMProdTaskProcExelog> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMProdTaskProcExelog> result) {
		View view = null;
		...
		Map<Long, WOMProdTaskProcExelog> rsMap = new HashMap<Long, WOMProdTaskProcExelog>();
		for (WOMProdTaskProcExelog prodTaskProcExelog : result) {
			ids.add(prodTaskProcExelog.getId());
			rsMap.put(prodTaskProcExelog.getId(), prodTaskProcExelog);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(prodTaskProcExelogDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = prodTaskProcExelogDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_ProdTaskProcExelog" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_ProdTaskProcExelog" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  prodTaskProcExelogDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodTaskProcExelog\"","WOM_7.5.0.0_produceTask","","WOM_PROD_TASK_PROC_EXELOGS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"prodTaskProcExelog\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"prodTaskProcExelog\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodTaskProcExelog\"","WOM_7.5.0.0_produceTask","","WOM_PROD_TASK_PROC_EXELOGS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_ProdTaskProcExelog" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("prodTaskProcExelog.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"prodTaskProcExelog\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"prodTaskProcExelog\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"prodTaskProcExelog\".CID = 1 OR \"prodTaskProcExelog\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("prodTaskProcExelog.mc\"").append(".PROD_TASK_PROC_EXELOG) not in ("+notinValue+")");
			}
			
			SQLQuery query = prodTaskProcExelogDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=prodTaskProcExelogDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMProdTaskProcExelog.ENTITY_CODE;}
	protected String getModuleCode(){return WOMProdTaskProcExelog.MODULE_CODE;}
	protected String getModelCode(){return WOMProdTaskProcExelog.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param prodTaskProcExelog 模型实例
	 */
	public void checkUniqueConstraint(WOMProdTaskProcExelog prodTaskProcExelog){
	}
	
	
	@Override
	public List<WOMProdTaskProcExelog> getProdTaskProcExelogs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProdTaskProcExelog.class);
		if(params!=null){
			...
			return prodTaskProcExelogDao.findByHql("from " + WOMProdTaskProcExelog.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return prodTaskProcExelogDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMProdTaskProcExelog> findProdTaskProcExelogsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProdTaskProcExelog.class);
	...
	@Override
	public List<WOMProdTaskProcExelog> findProdTaskProcExelogsByHql(String hql, Object... objects){
		
		return   prodTaskProcExelogDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMProdTaskProcExelog loadProdTaskProcExelogByBussinessKey(WOMProdTaskProcExelog prodTaskProcExelog){
		return loadProdTaskProcExelogByBussinessKey(prodTaskProcExelog.getId());
	}
	
	@Override
	public Page<WOMProdTaskProcExelog> getProdTaskProcExelogs(Page<WOMProdTaskProcExelog> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProdTaskProcExelog.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = prodTaskProcExelogDao.findByPage(page, sql);
			}else{
				page = prodTaskProcExelogDao.findAllByPage(page);
			}
			return page;
		}
		page = prodTaskProcExelogDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProdTaskProcExelog> findByTreeDataGridProduceTask(List<WOMProdTaskProcExelog> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMProdTaskProcExelog prodTaskProcExelog : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTask(Page<WOMProdTaskProcExelog> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMProdTaskProcExelog> prodTaskProcExelogs = dgPage.getResult();
			for(WOMProdTaskProcExelog prodTaskProcExelog : prodTaskProcExelogs){
				
			}
			dgPage.setResult(prodTaskProcExelogs);
		}
	...
	@Override
	@Transactional
	public void deleteProdTaskProcExelogByProduceTaskProcess(List<Long> produceTaskProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("taskProcessIDList", produceTaskProcessIds);
		String hql = "select id from WOMProdTaskProcExelog where "
						+ "taskProcessID.id in (:taskProcessIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> prodTaskProcExelogIds1 = prodTaskProcExelogDao.findByHql(hql, params);
		if(prodTaskProcExelogIds1 != null && !prodTaskProcExelogIds1.isEmpty()) {
			// 针对自关联模型
			prodTaskActiExelogService.deleteProdTaskActiExelogByProdTaskProcExelog(prodTaskProcExelogIds1);
		}
		hql = "delete WOMProdTaskProcExelog where "
						+ "taskProcessID.id in (:taskProcessIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		prodTaskProcExelogDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProdTaskProcExelog loadProdTaskProcExelogByBussinessKey(Serializable bussinessKey){
		return prodTaskProcExelogDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProdTaskProcExelog> findByProperty(String propertyName, Object object){
		return prodTaskProcExelogDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProdTaskProcExelog findEntityByProperty(String propertyName, Object object){
		return prodTaskProcExelogDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMProdTaskProcExelog.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = prodTaskProcExelogDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_ProdTaskProcExelog","runtime");
		try {
	...
	public void dealDatagridsSave(WOMProdTaskProcExelog prodTaskProcExelog,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,prodTaskProcExelogDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMProdTaskProcExelog.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, prodTaskProcExelog);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(prodTaskProcExelog.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(prodTaskProcExelog.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(prodTaskProcExelog.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",prodTaskProcExelog.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProdTaskProcExelog.TABLE_NAME +" where valid = 1";
			list = prodTaskProcExelogDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProdTaskProcExelog.TABLE_NAME;
			list = prodTaskProcExelogDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMProdTaskProcExelog.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = prodTaskProcExelogDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMProdTaskProcExelogDaoImpl prodTaskProcExelogDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMProdTaskProcExelog.TABLE_NAME + " where id = ?";
		String tableNo = prodTaskProcExelogDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchProdTaskProcExelog(final List<WOMProdTaskProcExelog> insertObjs, final List<WOMProdTaskProcExelog> updateObjs, 
		...
		return this.importBatchProdTaskProcExelog(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProdTaskProcExelog(final List<WOMProdTaskProcExelog> insertObjs, final List<WOMProdTaskProcExelog> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1495777422447"),importCount));
		...
		Map<Long, WOMProdTaskProcExelog> oldUpdateObjsMap = new HashMap<Long, WOMProdTaskProcExelog>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMProdTaskProcExelog bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMProdTaskProcExelog> bizObjList = prodTaskProcExelogDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMProdTaskProcExelog bizObj = bizObjList.get(mIndex);
		...
		final List<WOMProdTaskProcExelog> nmeObjs = new ArrayList<WOMProdTaskProcExelog>();
		...
		//String idSql = "select id from " + WOMProdTaskProcExelog.TABLE_NAME + " order by id desc";
		//Query query = prodTaskProcExelogDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMProdTaskProcExelog.JPA_NAME + " order by id desc";
		Query query =  prodTaskProcExelogDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMProdTaskProcExelog.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMProdTaskProcExelog prodTaskProcExelog:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMProdTaskProcExelog.TABLE_NAME,1,dbId);
			if(prodTaskProcExelog.getId() != null ){
				m.put(prodTaskProcExelog.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) prodTaskProcExelogDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProdTaskProcExelog"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		prodTaskProcExelogDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMProdTaskProcExelog.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMProdTaskProcExelog prodTaskProcExelog = updateObjs.get(updateIndex);
						Long id = prodTaskProcExelog.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = prodTaskProcExelog.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(prodTaskProcExelog);
							...
							nmeObjs.add(prodTaskProcExelog);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMProdTaskProcExelog.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMProdTaskProcExelog prodTaskProcExelog = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = prodTaskProcExelog.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		prodTaskProcExelogDaoImpl.getSession().flush();
		prodTaskProcExelogDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = prodTaskProcExelogDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_ProdTaskProcExelog").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = prodTaskProcExelogDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_ProdTaskProcExelog").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMProdTaskProcExelog bizObj = insertObjs.get(insertIndex);
			prodTaskProcExelogImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMProdTaskProcExelog bizObj = updateObjs.get(updateIndex);
			WOMProdTaskProcExelog oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			prodTaskProcExelogImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMProdTaskProcExelog> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMProdTaskProcExelog> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) prodTaskProcExelogDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMProdTaskProcExelog> result = getResult(page, query, new PendingResultTransformer(WOMProdTaskProcExelog.class,prodTaskProcExelogDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMProdTaskProcExelog model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = prodTaskProcExelogDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProdTaskProcExelog").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = prodTaskProcExelogDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = prodTaskProcExelogDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = prodTaskProcExelogDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?prodTaskProcExelogDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  prodTaskProcExelogDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,prodTaskProcExelogDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="ec.flow.save")
	public void saveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, DataGridService dataGridService){
		saveProdTaskProcExelog(prodTaskProcExelog, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="ec.flow.save")
	public void saveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, DataGridService dataGridService, String viewCode){
		saveProdTaskProcExelog(prodTaskProcExelog, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog")
	public void saveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProdTaskProcExelog(prodTaskProcExelog, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog")
	public void saveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != prodTaskProcExelog.getId() && prodTaskProcExelog.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProdTaskProcExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog")
	public Map<Object, Object> prodTaskProcExelogDataGridImport(WOMProdTaskProcExelog prodTaskProcExelog, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = prodTaskProcExelog.getVirtualId();
		
		prodTaskProcExelog = prodTaskProcExelogDao.load(prodTaskProcExelog.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, prodTaskProcExelog.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = prodTaskProcExelog.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(prodTaskProcExelog);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, prodTaskProcExelog.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog")
	public void mergeProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(prodTaskProcExelog);
		beforeSaveProdTaskProcExelog(prodTaskProcExelog);
		prodTaskProcExelogDao.merge(prodTaskProcExelog);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProdTaskProcExelog",desc="foundation.batch.save",operType="4")
	public void batchSaveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
	...
	@Override
	public Page<WOMProdTaskProcExelog> getByPage(Page<WOMProdTaskProcExelog> page,DetachedCriteria detachedCriteria){
		return prodTaskProcExelogDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, Object... objects){
		checkUniqueConstraint(prodTaskProcExelog);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProdTaskProcExelog(List<WOMProdTaskProcExelog> prodTaskProcExelogs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProdTaskProcExelog(List<WOMProdTaskProcExelog> prodTaskProcExelogs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProdTaskProcExelog(List<WOMProdTaskProcExelog> prodTaskProcExelogs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProdTaskProcExelog(List<WOMProdTaskProcExelog> prodTaskProcExelogs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProdTaskProcExelog(WOMProdTaskProcExelog prodTaskProcExelog){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMProdTaskProcExelog prodTaskProcExelog, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMProdTaskProcExelog prodTaskProcExelog, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProdTaskProcExelog(List<WOMProdTaskProcExelog> insertObjs, List<WOMProdTaskProcExelog> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProdTaskProcExelog,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProdTaskProcExelog(List<WOMProdTaskProcExelog> insertObjs, List<WOMProdTaskProcExelog> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProdTaskProcExelog,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_ProdTaskProcExelog,WOM_7.5.0.0) */
// 自定义代码
