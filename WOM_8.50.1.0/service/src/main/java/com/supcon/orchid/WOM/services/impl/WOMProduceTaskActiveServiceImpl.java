...
import com.supcon.orchid.WOM.services.WOMProduceTaskActiveImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMProduceTaskActiveDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
...
@Service("wOM_produceTaskActiveService")
@Transactional
public class WOMProduceTaskActiveServiceImpl  extends BaseServiceImpl<WOMProduceTaskActive> implements WOMProduceTaskActiveService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMProduceTaskActive> bapBaseService;
	...
	@Autowired
	private WOMProduceTaskActiveImportService produceTaskActiveImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMProduceTaskActive.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  produceTaskActiveDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMProduceTaskActive.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = produceTaskActiveDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProduceTaskActive.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProduceTaskActive.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  produceTaskActiveDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMProduceTaskActive.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMProduceTaskActive.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMProduceTaskActive> list = (List<WOMProduceTaskActive>) produceTaskActiveDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMProduceTaskActive getProduceTaskActive(long id){
		return getProduceTaskActive(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskActive getProduceTaskActive(long id, String viewCode){
		WOMProduceTaskActive produceTaskActive = produceTaskActiveDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProduceTaskActiveAsJSON(long id, String include){
		WOMProduceTaskActive produceTaskActive = produceTaskActiveDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(produceTaskActive, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskActive(WOMProduceTaskActive produceTaskActive){
		if(null != produceTaskActive && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskActive.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
			AuditUtil.setColumnStringC(produceTaskActive.getId().toString());
		}
		beforeDeleteProduceTaskActive(produceTaskActive);
		produceTaskActiveDao.deletePhysical(produceTaskActive);
		afterDeleteProduceTaskActive(produceTaskActive);
		produceTaskActiveDao.flush();
		List<WOMProduceTaskActive> params = new ArrayList<WOMProduceTaskActive>();
		params.add(produceTaskActive);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskActive(List<Long> produceTaskActiveIds){
		deleteProduceTaskActive(produceTaskActiveIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskActive(List<Long> produceTaskActiveIds, String eventTopic) {
		adjustOrTempHeadService.deleteAdjustOrTempHeadByProduceTaskActive(produceTaskActiveIds);
		prodTaskActiExelogService.deleteProdTaskActiExelogByProduceTaskActive(produceTaskActiveIds);
		waitPutinRecordsService.deleteWaitPutinRecordsByProduceTaskActive(produceTaskActiveIds);
		List<WOMProduceTaskActive> produceTaskActives = new ArrayList<WOMProduceTaskActive>();
		for(Long produceTaskActiveId : produceTaskActiveIds){
			WOMProduceTaskActive produceTaskActive = getProduceTaskActive(produceTaskActiveId);
			produceTaskActives.add(produceTaskActive);
			if(null != produceTaskActive && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskActive.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == produceTaskActive.getId() ? "" : "," + produceTaskActive.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == produceTaskActive.getId() ? "" : "," + produceTaskActive.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == produceTaskActive.getId() ? "" : "," + produceTaskActive.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
				}
			}
		}
		...
		if(produceTaskActives != null && produceTaskActives.size() > 0 ){
			for(WOMProduceTaskActive produceTaskActive : produceTaskActives){
				...
				java.lang.reflect.Field[] fields = WOMProduceTaskActive.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(produceTaskActive));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),produceTaskActive, produceTaskActive.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(produceTaskActiveIds != null && produceTaskActiveIds.size() > 0) {
			String hql = "delete " + WOMProduceTaskActive.JPA_NAME + " where id in(:ids)";
			Query query = produceTaskActiveDao.createQuery(hql);
			query.setParameterList("ids", produceTaskActiveIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMProduceTaskActive produceTaskActivez : produceTaskActives){
				produceTaskActivez.setValid(false);
				List<WOMProduceTaskActive> params = new ArrayList<WOMProduceTaskActive>();
				params.add(produceTaskActivez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskActive(long produceTaskActiveId, int produceTaskActiveVersion){
		this.deleteProduceTaskActive(produceTaskActiveId, produceTaskActiveVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskActive(long produceTaskActiveId, int produceTaskActiveVersion,SignatureLog signatureLog){
		WOMProduceTaskActive produceTaskActive = getProduceTaskActive(produceTaskActiveId);
		...
		beforeDeleteProduceTaskActive(produceTaskActive);
		produceTaskActiveDao.delete(produceTaskActiveId, produceTaskActiveVersion);
		afterDeleteProduceTaskActive(produceTaskActive);
		produceTaskActiveDao.flush();
		List<WOMProduceTaskActive> params = new ArrayList<WOMProduceTaskActive>();
		params.add(produceTaskActive);
		...
		if(signatureLog != null ) {
			...
			businessKey=produceTaskActive.getId();
			signatureLog.setTableId(produceTaskActive.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskActive(String produceTaskActiveIds){
		this.deleteProduceTaskActive(produceTaskActiveIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskActive(String produceTaskActiveIds,SignatureLog signatureLog){
		deleteCollection(produceTaskActiveIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProduceTaskActive(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_ProduceTaskActive#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="ec.engine.restore",operType="10")
	public void restoreProduceTaskActive(String produceTaskActiveIds){
		restoreCollection(produceTaskActiveIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProduceTaskActive(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="ec.engine.restore",operType="10")
	public void restoreProduceTaskActive(long produceTaskActiveId){
		
		findBusinessKeyUsed(produceTaskActiveId);	//判断业务主键是否重复
		...
		beforeRestoreProduceTaskActive(produceTaskActive);
		produceTaskActive.setValid(true);
		produceTaskActiveDao.update(produceTaskActive);
		afterRestoreProduceTaskActive(produceTaskActive);
		List<WOMProduceTaskActive> params = new ArrayList<WOMProduceTaskActive>();
		params.add(produceTaskActive);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long produceTaskActiveId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_ProduceTaskActive");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMProduceTaskActive.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMProduceTaskActive.TABLE_NAME +" where id = ? )";
			List<Object> list =  produceTaskActiveDao.createNativeQuery(sql,produceTaskActiveId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProduceTaskActive(List<WOMProduceTaskActive>  produceTaskActives){
		for(WOMProduceTaskActive produceTaskActive:produceTaskActives)  {
			saveProduceTaskActive(produceTaskActive, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProduceTaskActive(List<WOMProduceTaskActive>  produceTaskActives){
		for(WOMProduceTaskActive produceTaskActive:produceTaskActives)  {
			saveProduceTaskActive(produceTaskActive, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="ec.flow.save")
	public void saveProduceTaskActive(WOMProduceTaskActive produceTaskActive, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProduceTaskActive(produceTaskActive, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="ec.flow.save")
	public void saveProduceTaskActive(WOMProduceTaskActive produceTaskActive, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProduceTaskActive(produceTaskActive, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProduceTaskActiveCodes(WOMProduceTaskActive produceTaskActive) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProduceTaskActiveCodes(produceTaskActive, false);
	}
	
	public void generateProduceTaskActiveCodes(WOMProduceTaskActive produceTaskActive, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProduceTaskActiveSummarys(WOMProduceTaskActive produceTaskActive) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProduceTaskActiveSummarys(produceTaskActive, false);
	}

	public void generateProduceTaskActiveSummarys(WOMProduceTaskActive produceTaskActive, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive")
	public void saveProduceTaskActive(WOMProduceTaskActive produceTaskActive,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProduceTaskActive(produceTaskActive, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive")
	public void saveProduceTaskActive(WOMProduceTaskActive produceTaskActive,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(produceTaskActive.getId() != null && produceTaskActive.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskActive.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, produceTaskActive);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(produceTaskActive);
		beforeSaveProduceTaskActive(produceTaskActive, viewIsView);

		if (viewIsView) {
			produceTaskActiveDao.saveWithRevertVersion(produceTaskActive);
		} else {
			if(isNew)
				produceTaskActiveDao.save(produceTaskActive);
			else
				produceTaskActiveDao.merge(produceTaskActive);
		}
		if(null != produceTaskActive && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskActive.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
		...
		dealDatagridsSave(produceTaskActive,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProduceTaskActiveCodes(produceTaskActive, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProduceTaskActiveSummarys(produceTaskActive, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProduceTaskActive(produceTaskActive, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, produceTaskActive);
			}
		}
		List<WOMProduceTaskActive> params = new ArrayList<WOMProduceTaskActive>();
		produceTaskActiveDao.flush();
		produceTaskActiveDao.clear();
		produceTaskActive = produceTaskActiveDao.load(produceTaskActive.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMProduceTaskActive produceTaskActive){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("produceTaskActive", produceTaskActive);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive")
	public void mergeProduceTaskActive(WOMProduceTaskActive produceTaskActive, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(produceTaskActive);
		beforeSaveProduceTaskActive(produceTaskActive);
		produceTaskActiveDao.merge(produceTaskActive);
		if(null != produceTaskActive && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskActive.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskActive.getId() ? "" : produceTaskActive.getId().toString());
		...
		dealDatagridsSave(produceTaskActive,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProduceTaskActive(produceTaskActive);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMProduceTaskActive> findProduceTaskActives(Page<WOMProduceTaskActive> page, Criterion... criterions) {
		return produceTaskActiveDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskActives(Page<WOMProduceTaskActive> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProduceTaskActives(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskActives(Page<WOMProduceTaskActive> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskActives(Page<WOMProduceTaskActive> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMProduceTaskActive> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"produceTaskActive\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"produceTaskActive\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"produceTaskActive\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"produceTaskActive\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  produceTaskActiveDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  produceTaskActiveDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"produceTaskActive\".LAY_REC ASC, \"produceTaskActive\".SORT ASC");
				} else {
					orderPart.append(" \"produceTaskActive\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) produceTaskActiveDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=produceTaskActiveDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = produceTaskActiveDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMProduceTaskActive> result = new ArrayList<WOMProduceTaskActive>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMProduceTaskActive.class,produceTaskActiveDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMProduceTaskActive.class,produceTaskActiveDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMProduceTaskActive produceTaskActive : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(produceTaskActive.getId(), "WOM_produceTask_produceTaskActive");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(produceTaskActive.getId(), "WOM_produceTask_produceTaskActive");
							if(null != documents && !documents.isEmpty()) {
								produceTaskActive.setDocument(documents.get(0));
								produceTaskActive.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMProduceTaskActive> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMProduceTaskActive> result) {
		View view = null;
		...
		Map<Long, WOMProduceTaskActive> rsMap = new HashMap<Long, WOMProduceTaskActive>();
		for (WOMProduceTaskActive produceTaskActive : result) {
			ids.add(produceTaskActive.getId());
			rsMap.put(produceTaskActive.getId(), produceTaskActive);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(produceTaskActiveDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = produceTaskActiveDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_ProduceTaskActive" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_ProduceTaskActive" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  produceTaskActiveDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceTaskActive\"","WOM_7.5.0.0_produceTask","","WOM_PRODUCE_TASK_ACTIVES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"produceTaskActive\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"produceTaskActive\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceTaskActive\"","WOM_7.5.0.0_produceTask","","WOM_PRODUCE_TASK_ACTIVES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_ProduceTaskActive" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("produceTaskActive.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"produceTaskActive\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"produceTaskActive\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"produceTaskActive\".CID = 1 OR \"produceTaskActive\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("produceTaskActive.mc\"").append(".PRODUCE_TASK_ACTIVE) not in ("+notinValue+")");
			}
			
			SQLQuery query = produceTaskActiveDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=produceTaskActiveDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMProduceTaskActive.ENTITY_CODE;}
	protected String getModuleCode(){return WOMProduceTaskActive.MODULE_CODE;}
	protected String getModelCode(){return WOMProduceTaskActive.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param produceTaskActive 模型实例
	 */
	public void checkUniqueConstraint(WOMProduceTaskActive produceTaskActive){
	}
	
	
	@Override
	public List<WOMProduceTaskActive> getProduceTaskActives(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskActive.class);
		if(params!=null){
			...
			return produceTaskActiveDao.findByHql("from " + WOMProduceTaskActive.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return produceTaskActiveDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMProduceTaskActive> findProduceTaskActivesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskActive.class);
	...
	@Override
	public List<WOMProduceTaskActive> findProduceTaskActivesByHql(String hql, Object... objects){
		
		return   produceTaskActiveDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMProduceTaskActive loadProduceTaskActiveByBussinessKey(WOMProduceTaskActive produceTaskActive){
		return loadProduceTaskActiveByBussinessKey(produceTaskActive.getId());
	}
	
	@Override
	public Page<WOMProduceTaskActive> getProduceTaskActives(Page<WOMProduceTaskActive> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskActive.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = produceTaskActiveDao.findByPage(page, sql);
			}else{
				page = produceTaskActiveDao.findAllByPage(page);
			}
			return page;
		}
		page = produceTaskActiveDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProduceTaskActive> findByTreeDataGridProduceTask(List<WOMProduceTaskActive> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMProduceTaskActive produceTaskActive : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTask(Page<WOMProduceTaskActive> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMProduceTaskActive> produceTaskActives = dgPage.getResult();
			for(WOMProduceTaskActive produceTaskActive : produceTaskActives){
				
			}
			dgPage.setResult(produceTaskActives);
		}
	...
	@Override
	@Transactional
	public void deleteProduceTaskActiveByProduceTaskProcess(List<Long> produceTaskProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("orderProcessIdList", produceTaskProcessIds);
		String hql = "select id from WOMProduceTaskActive where "
						+ "orderProcessId.id in (:orderProcessIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> produceTaskActiveIds1 = produceTaskActiveDao.findByHql(hql, params);
		if(produceTaskActiveIds1 != null && !produceTaskActiveIds1.isEmpty()) {
			// 针对自关联模型
			adjustOrTempHeadService.deleteAdjustOrTempHeadByProduceTaskActive(produceTaskActiveIds1);
			// 针对自关联模型
			prodTaskActiExelogService.deleteProdTaskActiExelogByProduceTaskActive(produceTaskActiveIds1);
			// 针对自关联模型
			waitPutinRecordsService.deleteWaitPutinRecordsByProduceTaskActive(produceTaskActiveIds1);
		}
		hql = "delete WOMProduceTaskActive where "
						+ "orderProcessId.id in (:orderProcessIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		produceTaskActiveDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskActive loadProduceTaskActiveByBussinessKey(Serializable bussinessKey){
		return produceTaskActiveDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProduceTaskActive> findByProperty(String propertyName, Object object){
		return produceTaskActiveDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskActive findEntityByProperty(String propertyName, Object object){
		return produceTaskActiveDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMProduceTaskActive.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = produceTaskActiveDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_ProduceTaskActive","runtime");
		try {
	...
	public void dealDatagridsSave(WOMProduceTaskActive produceTaskActive,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,produceTaskActiveDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMProduceTaskActive.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, produceTaskActive);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(produceTaskActive.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(produceTaskActive.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(produceTaskActive.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",produceTaskActive.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProduceTaskActive.TABLE_NAME +" where valid = 1";
			list = produceTaskActiveDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProduceTaskActive.TABLE_NAME;
			list = produceTaskActiveDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMProduceTaskActive.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = produceTaskActiveDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMProduceTaskActiveDaoImpl produceTaskActiveDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMProduceTaskActive.TABLE_NAME + " where id = ?";
		String tableNo = produceTaskActiveDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchProduceTaskActive(final List<WOMProduceTaskActive> insertObjs, final List<WOMProduceTaskActive> updateObjs, 
		...
		return this.importBatchProduceTaskActive(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProduceTaskActive(final List<WOMProduceTaskActive> insertObjs, final List<WOMProduceTaskActive> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1489671093316"),importCount));
		...
		Map<Long, WOMProduceTaskActive> oldUpdateObjsMap = new HashMap<Long, WOMProduceTaskActive>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMProduceTaskActive bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMProduceTaskActive> bizObjList = produceTaskActiveDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMProduceTaskActive bizObj = bizObjList.get(mIndex);
		...
		final List<WOMProduceTaskActive> nmeObjs = new ArrayList<WOMProduceTaskActive>();
		...
		//String idSql = "select id from " + WOMProduceTaskActive.TABLE_NAME + " order by id desc";
		//Query query = produceTaskActiveDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMProduceTaskActive.JPA_NAME + " order by id desc";
		Query query =  produceTaskActiveDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMProduceTaskActive.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMProduceTaskActive produceTaskActive:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMProduceTaskActive.TABLE_NAME,1,dbId);
			if(produceTaskActive.getId() != null ){
				m.put(produceTaskActive.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) produceTaskActiveDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProduceTaskActive"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		produceTaskActiveDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMProduceTaskActive.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMProduceTaskActive produceTaskActive = updateObjs.get(updateIndex);
						Long id = produceTaskActive.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = produceTaskActive.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(produceTaskActive);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMProduceTaskActive.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMProduceTaskActive produceTaskActive = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = produceTaskActive.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		produceTaskActiveDaoImpl.getSession().flush();
		produceTaskActiveDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = produceTaskActiveDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_ProduceTaskActive").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = produceTaskActiveDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_ProduceTaskActive").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMProduceTaskActive bizObj = insertObjs.get(insertIndex);
			produceTaskActiveImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMProduceTaskActive bizObj = updateObjs.get(updateIndex);
			WOMProduceTaskActive oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			produceTaskActiveImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMProduceTaskActive> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMProduceTaskActive> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) produceTaskActiveDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMProduceTaskActive> result = getResult(page, query, new PendingResultTransformer(WOMProduceTaskActive.class,produceTaskActiveDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMProduceTaskActive model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = produceTaskActiveDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProduceTaskActive").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = produceTaskActiveDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = produceTaskActiveDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = produceTaskActiveDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?produceTaskActiveDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  produceTaskActiveDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,produceTaskActiveDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="ec.flow.save")
	public void saveProduceTaskActive(WOMProduceTaskActive produceTaskActive, DataGridService dataGridService){
		saveProduceTaskActive(produceTaskActive, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="ec.flow.save")
	public void saveProduceTaskActive(WOMProduceTaskActive produceTaskActive, DataGridService dataGridService, String viewCode){
		saveProduceTaskActive(produceTaskActive, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive")
	public void saveProduceTaskActive(WOMProduceTaskActive produceTaskActive, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProduceTaskActive(produceTaskActive, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive")
	public void saveProduceTaskActive(WOMProduceTaskActive produceTaskActive, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != produceTaskActive.getId() && produceTaskActive.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskActive.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive")
	public Map<Object, Object> produceTaskActiveDataGridImport(WOMProduceTaskActive produceTaskActive, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = produceTaskActive.getVirtualId();
		
		produceTaskActive = produceTaskActiveDao.load(produceTaskActive.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, produceTaskActive.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = produceTaskActive.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(produceTaskActive);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, produceTaskActive.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive")
	public void mergeProduceTaskActive(WOMProduceTaskActive produceTaskActive, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(produceTaskActive);
		beforeSaveProduceTaskActive(produceTaskActive);
		produceTaskActiveDao.merge(produceTaskActive);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskActive",desc="foundation.batch.save",operType="4")
	public void batchSaveProduceTaskActive(WOMProduceTaskActive produceTaskActive, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
	...
	@Override
	public Page<WOMProduceTaskActive> getByPage(Page<WOMProduceTaskActive> page,DetachedCriteria detachedCriteria){
		return produceTaskActiveDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProduceTaskActive(WOMProduceTaskActive produceTaskActive, Object... objects){
		checkUniqueConstraint(produceTaskActive);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
      //包装类型指令单配方为空 将表体活动类型设为投料
		if(produceTaskActive.getHeadID().getFormularId()==null&&((SystemCode) systemCodeService.getSystemCode("produceType/produceType2")).equals(produceTaskActive.getHeadID().getWorkType())){
			produceTaskActive.setActiveType((SystemCode) systemCodeService.getSystemCode("activeType/activeType3"));//活动类型设为投料
		}
/* CUSTOM CODE END */
	}
	
	private void afterSaveProduceTaskActive(WOMProduceTaskActive produceTaskActive, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProduceTaskActive(List<WOMProduceTaskActive> produceTaskActives, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProduceTaskActive(List<WOMProduceTaskActive> produceTaskActives){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProduceTaskActive(List<WOMProduceTaskActive> produceTaskActives, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProduceTaskActive(List<WOMProduceTaskActive> produceTaskActives){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProduceTaskActive(WOMProduceTaskActive produceTaskActive){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProduceTaskActive(WOMProduceTaskActive produceTaskActive){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProduceTaskActive(WOMProduceTaskActive produceTaskActive){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProduceTaskActive(WOMProduceTaskActive produceTaskActive){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMProduceTaskActive produceTaskActive, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMProduceTaskActive produceTaskActive, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProduceTaskActive(List<WOMProduceTaskActive> insertObjs, List<WOMProduceTaskActive> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProduceTaskActive,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProduceTaskActive(List<WOMProduceTaskActive> insertObjs, List<WOMProduceTaskActive> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProduceTaskActive,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_ProduceTaskActive,WOM_7.5.0.0) */
// 自定义代码
	@Autowired
	private MESBasicFactoryModelService factoryModelService;
	@Resource
	protected SystemCodeService systemCodeService;
	//根据物品id，指令单id找到对应的工序，工作单元，活动
	@Override
	public WOMProduceTaskActive getTask(String proID, String taskID) {
		String hqlString = "from WOMProduceTaskActive where valid = 1 and headID.id = ? and productId.id = ?";
		List<WOMProduceTaskActive> produceTaskActiveList = produceTaskActiveDao.findByHql(hqlString, Long.parseLong(taskID),Long.parseLong(proID));
		if(produceTaskActiveList!=null){
			if(produceTaskActiveList.size()>0){			
				//如果是配料活动，则找出所对应的投配料活动
				if(produceTaskActiveList.get(0).getActiveType()!=null && produceTaskActiveList.get(0).getActiveType().getId().equalsIgnoreCase("activeType/activeType2") ){
					if(produceTaskActiveList.get(0).getPutBatMateId()!=null){
						List<WOMProduceTaskActive> produceTaskActiveList2 = this.getProduceTaskActives(
								"headID="+Long.parseLong(taskID)+" and  putBatMateId="+produceTaskActiveList.get(0).getPutBatMateId().getId()+"and activeType='activeType/activeType4' ", null);
						if(produceTaskActiveList2.size()>0){
							return produceTaskActiveList2.get(0);
						}
					}
				}else{
					return produceTaskActiveList.get(0);
				}
			}
		}
		return null;
	...
	@Override
	public MESBasicDepartmentWare getFactory(String proID, String factoryId) {
		//获得对应生产线
		MESBasicFactoryModel factoryModel = factoryModelService.getFactoryModel(Long.parseLong(factoryId));
		//找到生产线下级的 生产线
		String hql = "from MESBasicFactoryModel where valid = 1 and layRec like ?";
		List<MESBasicFactoryModel> factoryModelList = produceTaskActiveDao.findByHql(hql,"%"+factoryId+"%");
		//获得生产线的上级 生产线
		while(factoryModel.getParentId()!=-1){
			long factoryModelID = factoryModel.getParentId();
			factoryModel = factoryModelService.getFactoryModel(factoryModelID);
			factoryModelList.add(factoryModel);
		}
		//组织 生产线ID
		List<Long> factoryModelIDList = new ArrayList<Long>();
		for(MESBasicFactoryModel factory : factoryModelList){
			factoryModelIDList.add(factory.getId());
		}
		//组织产品id
		List<Long> ProIDList = new ArrayList<Long>();
		ProIDList.add(Long.parseLong(proID));
		//根据生产线 和 物品 找到车间仓
		String hqlString = "from MESBasicDepartmentWare where valid = 1 and factoryId.id in (:factoryid) and id in (select headID.id from MESBasicFacWareMaterial where valid = 1 and productId.id in (:proID))";
		Query query = produceTaskActiveDao.createQuery(hqlString);
		query.setParameterList("factoryid", factoryModelIDList);
		query.setParameterList("proID", ProIDList);
		List<MESBasicDepartmentWare> departmentWare = query.list();
		//存在对应车间仓时，返回该对象
		if(departmentWare!=null && departmentWare.size()!=0){
			return departmentWare.get(0);
		}else{
			return null;
		}
	}
