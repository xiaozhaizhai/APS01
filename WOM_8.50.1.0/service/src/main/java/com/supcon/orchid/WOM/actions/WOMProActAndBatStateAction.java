...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMProActAndBatStateAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProActAndBatState> {
	...
	private WOMProActAndBatState proActAndBatState;
	...
	//导出excel时,主模型内容
	List<WOMProActAndBatState> queryResults = new ArrayList<WOMProActAndBatState>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/proActiveAndBatchState/proActAndBatState/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/proActiveAndBatchState/proActAndBatState/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProActAndBatState> proActAndBatStates;
	
	public List<WOMProActAndBatState> getProActAndBatStates() {
		return proActAndBatStates;
	}
	...
	private Page<WOMProActAndBatState> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			proActAndBatState = new WOMProActAndBatState();
			if(proActAndBatState.getActiveEnd() != null) {
				proActAndBatState.setActiveEnd((SystemCode) systemCodeService.getSystemCode(proActAndBatState.getActiveEnd().getId()));
			}
		} else {
			if(id > 0){
				proActAndBatState = proActAndBatStateService.getProActAndBatState(id, viewCode);
			}
		}
	...
	public Page<WOMProActAndBatState> getPage() {
		if (null == page) {
			page = new Page<WOMProActAndBatState>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProActAndBatState getProActAndBatState() {
		return proActAndBatState;
	}		

	public void setProActAndBatState(WOMProActAndBatState proActAndBatState) {
		this.proActAndBatState = proActAndBatState;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = proActAndBatStateService.getProActAndBatStateAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1509445507769
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String proActAndBatStateReferenceCopy() throws Exception{
			WOMProActAndBatState oldProActAndBatState = null;
			if(isEditNew()){
				...
				oldProActAndBatState = new WOMProActAndBatState();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,proActAndBatState.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, proActAndBatState.getId());
				...
				oldProActAndBatState = proActAndBatStateService.getProActAndBatState(id);
			}
			
			
			proActAndBatState = proActAndBatStateService.getProActAndBatState(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProActAndBatState.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProActAndBatState, null);
						Method setValue = WOMProActAndBatState.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(proActAndBatState, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			proActAndBatState.setCreateDepartmentId(oldProActAndBatState.getCreateDepartmentId());
			proActAndBatState.setCreatePositionId(oldProActAndBatState.getCreatePositionId());
			proActAndBatState.setCreateStaffId(oldProActAndBatState.getCreateStaffId());
			proActAndBatState.setOwnerDepartmentId(oldProActAndBatState.getOwnerDepartmentId());
			proActAndBatState.setOwnerPositionId(oldProActAndBatState.getOwnerPositionId());
			proActAndBatState.setOwnerStaffId(oldProActAndBatState.getOwnerStaffId());
			proActAndBatState.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/stateEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "stateEdit.ftl") }),
		@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/stateEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "stateEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=proActAndBatStateService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return proActAndBatStateReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			proActAndBatState = proActAndBatStateService.getProActAndBatState(this.proActAndBatState.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,proActAndBatState.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			proActAndBatStateService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/stateList", results = { @Result(type = FREEMARKER, location = "stateList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = proActAndBatStateService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(proActAndBatStateService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(proActAndBatStateService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/stateRef", results = { @Result(type = FREEMARKER, location = "stateRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/stateList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.activeBatchStateID.name,result.activeBatchStateID.isInitial,result.activeBatchStateID.isConclusion,result.quaWithoutPass,result.regectsDeal,result.qualifiedPass,result.activeEnd.id,result.activeEnd.value,result.activeBatchStateID.batchUse,result.activeBatchStateID.batchUseForDisplay,result.remark,result.activeBatchStateID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="stateList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"proActAndBatState\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"proActAndBatState\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"proActAndBatState\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"proActAndBatState\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			proActAndBatStateService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/stateList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/stateRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.activeBatchStateID.name,result.activeBatchStateID.isInitial,result.activeBatchStateID.isConclusion,result.quaWithoutPass,result.regectsDeal,result.qualifiedPass,result.activeEnd.id,result.activeEnd.value,result.activeBatchStateID.batchUse,result.activeBatchStateID.batchUseForDisplay,result.remark,result.activeBatchStateID.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="stateRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/allTempleteDownload", results = { @Result(name="excel",type="excel",location="stateList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		proActAndBatStateService.findDataGridPage(dg,dgclass,getDgPage(),proActAndBatState,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(proActAndBatState.getId() != null && proActAndBatState.getId() != -1){
				proActAndBatState = proActAndBatStateService.getProActAndBatState(proActAndBatState.getId());
			}
			proActAndBatStateService.findDataGridPage(dg,dgclass,getDgPage(),proActAndBatState,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == proActAndBatState.getId()) {
		...
		proActAndBatStateService.saveProActAndBatState(proActAndBatState,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/stateEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "submit");
		bap_validate_datagrids=proActAndBatStateService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", proActAndBatState.getId());
		...
		fileuploads.put("linkId", proActAndBatState.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		proActAndBatStateService.deleteProActAndBatState(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "restore");
		proActAndBatStateService.restoreProActAndBatState(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "showDealInfoData");
		if (null != tableInfoId) {
			int count=proActAndBatStateService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = proActAndBatStateService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = proActAndBatStateService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProActAndBatState> findExportDataInfos(Page<WOMProActAndBatState> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = proActAndBatStateService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = proActAndBatStateService.getAssProperty(p.getCode());	
									//String assModelCode = proActAndBatStateService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			proActAndBatStateService.findProActAndBatStates(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			proActAndBatStateService.findProActAndBatStates(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProActAndBatState> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProActAndBatState result : results){
	...
	public boolean isEditNew(){
		//return null == this.proActAndBatState || null == this.proActAndBatState.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/proActiveAndBatchState/proActAndBatState/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/proActiveAndBatchState/proActAndBatState/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProActAndBatStateService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_proActiveAndBatchState", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_proActiveAndBatchState");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		if(null != id) {
			proActAndBatState = proActAndBatStateService.getProActAndBatState(id);
		}
		...
		_auditLog.setColumnStringB(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
		_auditLog.setColumnStringA(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
		_auditLog.setColumnStringC(proActAndBatState.getId().toString());
		_auditLog.setColumnStringD(WOMProActAndBatState.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_proActiveAndBatchState");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProActAndBatStateAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, proActAndBatState,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, proActAndBatState,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,proActAndBatState);
		...
		printlog.setEntity("WOM_7.5.0.0_proActiveAndBatchState");
		...
		printlog.setBusinessId(proActAndBatState.getId());
		printlog.setBussinessKey(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
		printlog.setMainDisplay(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(proActAndBatState,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(proActAndBatStateService.getProActAndBatState(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, proActAndBatState,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				proActAndBatState = proActAndBatStateService.getProActAndBatState(Long.parseLong(arrId[i]));
				if(null != proActAndBatState){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == proActAndBatState.getId() ? "" : "," + proActAndBatState.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == proActAndBatState.getId() ? "" : "," + proActAndBatState.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == proActAndBatState.getId() ? "" : "," + proActAndBatState.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == proActAndBatState.getId() ? "" : proActAndBatState.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, proActAndBatState,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,proActAndBatState);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//proActAndBatStateService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/proActiveAndBatchState/proActAndBatState/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMProActAndBatState> findExportPageInfos(Page<WOMProActAndBatState> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_proActiveAndBatchState" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = proActAndBatStateService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMProActAndBatState queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = proActAndBatStateService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = proActAndBatStateService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = proActAndBatStateService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_proActiveAndBatchState_ProActAndBatState,WOM_7.5.0.0) */
// 自定义代码
