...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_taskReporting_TaskReporting,WOM_7.5.0.0) */
// 自定义代码
import java.math.BigDecimal;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMTaskReportingAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMTaskReporting> {
	...
	//导出excel时,主模型内容
	List<WOMTaskReporting> queryResults = new ArrayList<WOMTaskReporting>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/taskReporting/taskReporting/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/taskReporting/taskReporting/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_taskReporting_TaskReporting" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMTaskReporting> taskReportings;
	
	public List<WOMTaskReporting> getTaskReportings() {
		return taskReportings;
	}
	...
	private Page<WOMTaskReporting> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(taskReporting.getReportType() != null) {
				taskReporting.setReportType((SystemCode) systemCodeService.getSystemCode(taskReporting.getReportType().getId()));
			}
		} else {
			if(id > 0){
				taskReporting = taskReportingService.getTaskReporting(id, viewCode);
				if(tableInfoId == null && taskReporting != null){
					tableInfoId = taskReporting.getTableInfoId();
				}
			}
		}
	...
	public Page<WOMTaskReporting> getPage() {
		if (null == page) {
			page = new Page<WOMTaskReporting>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/taskReporting/taskReporting/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = taskReportingService.getTaskReportingAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/taskReporting/taskReporting/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String taskReportingReferenceCopy() throws Exception{
			WOMTaskReporting oldTaskReporting = null;
			if(isEditNew()){
				...
				oldTaskReporting = new WOMTaskReporting();
			}else{
			//WOMTaskReporting taskReporting = taskReportingService.getTaskReporting(this.taskReporting.getId());
			this.creatorInfo = creatorService.get(taskReporting.getCreateStaffId(), taskReporting.getCreatePositionId(), taskReporting.getCreateDepartmentId());
			//setTaskReporting(taskReporting);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(taskReporting.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskReporting.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, taskReporting.getId());
				...
				oldTaskReporting = taskReportingService.getTaskReporting(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldTaskReporting.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_taskReporting_" + oldTaskReporting.getId();
			...
			taskReporting = taskReportingService.getTaskReporting(refId);
			taskReporting.setPending(pending);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_taskReporting_TaskReporting");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMTaskReporting.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTaskReporting, null);
						Method setValue = WOMTaskReporting.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(taskReporting, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			taskReporting.setCreateDepartmentId(oldTaskReporting.getCreateDepartmentId());
			taskReporting.setCreatePositionId(oldTaskReporting.getCreatePositionId());
			taskReporting.setCreateStaffId(oldTaskReporting.getCreateStaffId());
			taskReporting.setOwnerDepartmentId(oldTaskReporting.getOwnerDepartmentId());
			taskReporting.setOwnerPositionId(oldTaskReporting.getOwnerPositionId());
			taskReporting.setOwnerStaffId(oldTaskReporting.getOwnerStaffId());
			taskReporting.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "referenceCopy");
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "modReportingEdit.ftl") }),
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "modReportingEditPrint.ftl")})
		,
		@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "taskModReportingEdit.ftl") }),
		@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "taskModReportingEditPrint.ftl")})
		,
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "taskReportingEdit.ftl") }),
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "taskReportingEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=taskReportingService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return taskReportingReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			taskReporting = taskReportingService.getTaskReporting(this.taskReporting.getId(), viewCode);
			this.creatorInfo = creatorService.get(taskReporting.getCreateStaffId(), taskReporting.getCreatePositionId(), taskReporting.getCreateDepartmentId());
			//setTaskReporting(taskReporting);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = taskReportingService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = taskReportingService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + taskReporting.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_taskReporting_" + taskReporting.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<WOMTaskReporting> tmpList = new ArrayList<>();
			tmpList.add(taskReporting);
			taskReportingService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = taskReportingService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskReporting.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "modReportingView.ftl") }),
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "modReportingViewPrint.ftl")})
		,
		@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "taskModReportingView.ftl") }),
		@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "taskModReportingViewPrint.ftl")})
		,
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "taskReportingView.ftl") }),
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "taskReportingViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingList", results = { @Result(type = FREEMARKER, location = "modReportingList.ftl") })
		,
		@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingList", results = { @Result(type = FREEMARKER, location = "taskModReportingList.ftl") })
		,
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingList", results = { @Result(type = FREEMARKER, location = "taskReportingList.ftl") })
	})
	public String viewList() throws Exception{
		...
		//mainViewPath = "/WOM/taskReporting/taskReporting/taskModReportingView.action";
		mainViewPath=taskReportingService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = taskReportingService.getWorkFlowInfo(workFlowMenuCode);
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = taskReportingService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(taskReportingService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(taskReportingService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_taskReporting_TaskReporting", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingRef", results = { @Result(type = FREEMARKER, location = "taskReportingRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.batchNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.remark,result.taskID.id,result.productID.id,result.reportStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="modReportingList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.batchNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.remark,result.effectiveState,result.taskID.id,result.productID.id,result.reportStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="taskModReportingList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.batchNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.remark,result.effectiveState,result.taskID.id,result.productID.id,result.reportStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="taskReportingList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"taskReporting\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"taskReporting\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			taskReportingService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<WOMTaskReporting> taskReportingList=page.getResult();
				for(WOMTaskReporting lz:taskReportingList){
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.batchNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.remark,result.taskID.id,result.productID.id,result.reportStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="modReportingList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.batchNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.remark,result.effectiveState,result.taskID.id,result.productID.id,result.reportStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="taskModReportingList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.batchNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.remark,result.effectiveState,result.taskID.id,result.productID.id,result.reportStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"taskReporting\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"taskReporting\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReporting/taskReportingRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.batchNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.remark,result.taskID.id,result.productID.id,result.reportStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="taskReportingRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"taskReporting\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	 ...
	 * @return
	 */
	@Action(value = "/WOM/taskReporting/taskReporting/allTempleteDownload", results = { @Result(name="excel",type="excel",location="modReportingList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_taskReporting_TaskReporting");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		taskReportingService.findDataGridPage(dg,dgclass,getDgPage(),taskReporting,"", null);
	...
	private Page<WOMTaskReportingPart> dg1566214652249Page;
	public Page<WOMTaskReportingPart> getDg1566214652249Page(){
		if(null == dg1566214652249Page){
			dg1566214652249Page = new Page<WOMTaskReportingPart>(1,20);
		}
		return dg1566214652249Page;
	}
	
	private Class dg1566214652249Clazz = WOMTaskReportingPart.class;
	
	public Class getDg1566214652249Clazz() {
		return dg1566214652249Clazz;
	}
	
	private List<WOMTaskReportingPart> dg1566214652249List;
	public List<WOMTaskReportingPart> getDg1566214652249List(){
		if(null == dg1566214652249List){
			dg1566214652249List = new ArrayList<WOMTaskReportingPart>();
		}
		return dg1566214652249List;
	}
	public void setDg1566214652249ListJson(String json) {
		//dg1566214652249List = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(json, WOMTaskReportingPart.class);
	}
	
	private List<Long> dg1566214652249DeletedIds;
	public List<Long> getDg1566214652249DeletedIds(){
		if(null == dg1566214652249DeletedIds){
			dg1566214652249DeletedIds = new ArrayList<Long>();
		}
		return dg1566214652249DeletedIds;
	}

	private Page<WOMTaskReportingPart> dg1567142928640Page;
	public Page<WOMTaskReportingPart> getDg1567142928640Page(){
		if(null == dg1567142928640Page){
			dg1567142928640Page = new Page<WOMTaskReportingPart>(1,20);
		}
		return dg1567142928640Page;
	}
	
	private Class dg1567142928640Clazz = WOMTaskReportingPart.class;
	
	public Class getDg1567142928640Clazz() {
		return dg1567142928640Clazz;
	}
	
	private List<WOMTaskReportingPart> dg1567142928640List;
	public List<WOMTaskReportingPart> getDg1567142928640List(){
		if(null == dg1567142928640List){
			dg1567142928640List = new ArrayList<WOMTaskReportingPart>();
		}
		return dg1567142928640List;
	}
	public void setDg1567142928640ListJson(String json) {
		//dg1567142928640List = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(json, WOMTaskReportingPart.class);
	}
	
	private List<Long> dg1567142928640DeletedIds;
	public List<Long> getDg1567142928640DeletedIds(){
		if(null == dg1567142928640DeletedIds){
			dg1567142928640DeletedIds = new ArrayList<Long>();
		}
		return dg1567142928640DeletedIds;
	}

	private Page<WOMTaskReportingPart> dg1568768223948Page;
	public Page<WOMTaskReportingPart> getDg1568768223948Page(){
		if(null == dg1568768223948Page){
			dg1568768223948Page = new Page<WOMTaskReportingPart>(1,20);
		}
		return dg1568768223948Page;
	}
	
	private Class dg1568768223948Clazz = WOMTaskReportingPart.class;
	
	public Class getDg1568768223948Clazz() {
		return dg1568768223948Clazz;
	}
	
	private List<WOMTaskReportingPart> dg1568768223948List;
	public List<WOMTaskReportingPart> getDg1568768223948List(){
		if(null == dg1568768223948List){
			dg1568768223948List = new ArrayList<WOMTaskReportingPart>();
		}
		return dg1568768223948List;
	}
	public void setDg1568768223948ListJson(String json) {
		//dg1568768223948List = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(json, WOMTaskReportingPart.class);
	}
	
	private List<Long> dg1568768223948DeletedIds;
	public List<Long> getDg1568768223948DeletedIds(){
		if(null == dg1568768223948DeletedIds){
			dg1568768223948DeletedIds = new ArrayList<Long>();
		}
		return dg1568768223948DeletedIds;
	}

	private Page<WOMTaskReportingPart> dg1568768369308Page;
	public Page<WOMTaskReportingPart> getDg1568768369308Page(){
		if(null == dg1568768369308Page){
			dg1568768369308Page = new Page<WOMTaskReportingPart>(1,20);
		}
		return dg1568768369308Page;
	}
	
	private Class dg1568768369308Clazz = WOMTaskReportingPart.class;
	
	public Class getDg1568768369308Clazz() {
		return dg1568768369308Clazz;
	}
	
	private List<WOMTaskReportingPart> dg1568768369308List;
	public List<WOMTaskReportingPart> getDg1568768369308List(){
		if(null == dg1568768369308List){
			dg1568768369308List = new ArrayList<WOMTaskReportingPart>();
		}
		return dg1568768369308List;
	}
	public void setDg1568768369308ListJson(String json) {
		//dg1568768369308List = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(json, WOMTaskReportingPart.class);
	}
	
	private List<Long> dg1568768369308DeletedIds;
	public List<Long> getDg1568768369308DeletedIds(){
		if(null == dg1568768369308DeletedIds){
			dg1568768369308DeletedIds = new ArrayList<Long>();
		}
		return dg1568768369308DeletedIds;
	}

	private Page<WOMTaskReportingPart> dg1492079431325Page;
	public Page<WOMTaskReportingPart> getDg1492079431325Page(){
		if(null == dg1492079431325Page){
			dg1492079431325Page = new Page<WOMTaskReportingPart>(1,20);
		}
		return dg1492079431325Page;
	}
	
	private Class dg1492079431325Clazz = WOMTaskReportingPart.class;
	
	public Class getDg1492079431325Clazz() {
		return dg1492079431325Clazz;
	}
	
	private List<WOMTaskReportingPart> dg1492079431325List;
	public List<WOMTaskReportingPart> getDg1492079431325List(){
		if(null == dg1492079431325List){
			dg1492079431325List = new ArrayList<WOMTaskReportingPart>();
		}
		return dg1492079431325List;
	}
	public void setDg1492079431325ListJson(String json) {
		//dg1492079431325List = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(json, WOMTaskReportingPart.class);
	}
	
	private List<Long> dg1492079431325DeletedIds;
	public List<Long> getDg1492079431325DeletedIds(){
		if(null == dg1492079431325DeletedIds){
			dg1492079431325DeletedIds = new ArrayList<Long>();
		}
		return dg1492079431325DeletedIds;
	}

	private Page<WOMTaskReportingPart> dg1492079604060Page;
	public Page<WOMTaskReportingPart> getDg1492079604060Page(){
		if(null == dg1492079604060Page){
			dg1492079604060Page = new Page<WOMTaskReportingPart>(1,20);
		}
		return dg1492079604060Page;
	}
	
	private Class dg1492079604060Clazz = WOMTaskReportingPart.class;
	
	public Class getDg1492079604060Clazz() {
		return dg1492079604060Clazz;
	}
	
	private List<WOMTaskReportingPart> dg1492079604060List;
	public List<WOMTaskReportingPart> getDg1492079604060List(){
		if(null == dg1492079604060List){
			dg1492079604060List = new ArrayList<WOMTaskReportingPart>();
		}
		return dg1492079604060List;
	}
	public void setDg1492079604060ListJson(String json) {
		//dg1492079604060List = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(json, WOMTaskReportingPart.class);
	}
	
	private List<Long> dg1492079604060DeletedIds;
	public List<Long> getDg1492079604060DeletedIds(){
		if(null == dg1492079604060DeletedIds){
			dg1492079604060DeletedIds = new ArrayList<Long>();
		}
		return dg1492079604060DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/taskReporting/taskReporting/data-dg1566214652249", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.batchNum,result.reportNum,result.modifyNum,result.content,result.modifyCont,result.remark,result.id,result.version,result.taskID.id,result.productID.id,result.reportStaff.id,result.storeID.id,result.headId.id,result.wareId.id" , "dataGridCode", "WOM_7.5.0.0_taskReporting_modReportingEditdg1566214652249" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1566214652249-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1566214652249.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/taskReporting/taskReporting/data-dg1567142928640", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.batchNum,result.reportNum,result.modifyNum,result.content,result.modifyCont,result.WOM_7_5_0_0_taskReporting_taskReportingEditdg1492079431325_DATAGRID_CUSTOM_2da3aea4_d82c_4c1e_ad7f_a20f5b64f05f,result.remark,result.id,result.version,result.taskID.id,result.productID.id,result.reportStaff.id,result.storeID.id,result.headId.id,result.wareId.id" , "dataGridCode", "WOM_7.5.0.0_taskReporting_modReportingViewdg1567142928640" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1567142928640-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1567142928640.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/taskReporting/taskReporting/data-dg1568768223948", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.batchNum,result.wareId.wareName,result.reportNum,result.content,result.remark,result.storeID.placeSetName,result.storeID.placeSetCode,result.id,result.version,result.taskID.id,result.productID.id,result.reportStaff.id,result.storeID.id,result.headId.id,result.wareId.id" , "dataGridCode", "WOM_7.5.0.0_taskReporting_taskModReportingEditdg1568768223948" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1568768223948-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1568768223948.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/taskReporting/taskReporting/data-dg1568768369308", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.batchNum,result.reportNum,result.storeID.placeSetCode,result.content,result.storeID.placeSetName,result.WOM_7_5_0_0_taskReporting_taskReportingViewdg1492079604060_DATAGRID_CUSTOM_3e04867d_a05b_4fc7_84c3_c0d771c4ce79,result.remark,result.id,result.version,result.taskID.id,result.productID.id,result.reportStaff.id,result.storeID.id,result.headId.id,result.wareId.id" , "dataGridCode", "WOM_7.5.0.0_taskReporting_taskModReportingViewdg1568768369308" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1568768369308-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1568768369308.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/taskReporting/taskReporting/data-dg1492079431325", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.batchNum,result.wareId.wareName,result.reportNum,result.content,result.remark,result.storeID.placeSetName,result.storeID.placeSetCode,result.id,result.version,result.taskID.id,result.productID.id,result.reportStaff.id,result.storeID.id,result.headId.id,result.wareId.id" , "dataGridCode", "WOM_7.5.0.0_taskReporting_taskReportingEditdg1492079431325" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1492079431325-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1492079431325.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/taskReporting/taskReporting/data-dg1492079604060", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.batchNum,result.reportNum,result.storeID.placeSetCode,result.content,result.storeID.placeSetName,result.WOM_7_5_0_0_taskReporting_taskReportingViewdg1492079604060_DATAGRID_CUSTOM_3e04867d_a05b_4fc7_84c3_c0d771c4ce79,result.remark,result.id,result.version,result.taskID.id,result.productID.id,result.reportStaff.id,result.storeID.id,result.headId.id,result.wareId.id" , "dataGridCode", "WOM_7.5.0.0_taskReporting_taskReportingViewdg1492079604060" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1492079604060-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1492079604060.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(taskReporting.getId() != null && taskReporting.getId() != -1){
				taskReporting = taskReportingService.getTaskReporting(taskReporting.getId());
			}
			taskReportingService.findDataGridPage(dg,dgclass,getDgPage(),taskReporting,searchCondition,params);
	...
	private void save() throws Exception {
		...
		taskReportingService.save(taskReporting,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(taskReporting.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + taskReporting.getProcessKey() + "_" + p.getActivityName() + "_WOM_7.5.0.0_taskReporting_" + taskReporting.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(taskReporting.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/WOM/taskReporting/taskReporting/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		taskReportingService.retrialThisTable(taskReporting,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "retrial");
		return SUCCESS;
	}
	@Actions({
		@Action(value = "/WOM/taskReporting/taskReporting/modReportingEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/taskReporting/taskReporting/modReportingView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/taskReporting/taskReporting/taskModReportingView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/taskReporting/taskReporting/taskReportingEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/taskReporting/taskReporting/taskReportingView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "submit");
		bap_validate_datagrids=taskReportingService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (taskReporting != null && taskReporting.getStatus() != null && taskReporting.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				taskReportingService.saveSuperEditTaskReporting(taskReporting, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				taskReportingService.submit(taskReporting,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", taskReporting.getTableInfoId());
		responseMap.put("id", taskReporting.getId());
		...
		fileuploads.put("linkId", taskReporting.getTableInfoId());
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/taskReporting/taskReporting/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "bulkSubmit");
		taskReportingService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/WOM/taskReporting/taskReporting/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		taskReportingService.deleteTaskReporting(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/taskReporting/taskReporting/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "restore");
		taskReportingService.restoreTaskReporting(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/taskReporting/taskReporting/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/taskReporting/taskReporting/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "showDealInfoData");
		if (null != tableInfoId) {
			int count=taskReportingService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = taskReportingService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = taskReportingService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMTaskReporting> findExportDataInfos(Page<WOMTaskReporting> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = taskReportingService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = taskReportingService.getAssProperty(p.getCode());	
									//String assModelCode = taskReportingService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			taskReportingService.findTaskReportings(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			taskReportingService.findTaskReportings(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMTaskReporting> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMTaskReporting result : results){
	...
	public boolean isEditNew(){
		//return null == this.taskReporting || null == this.taskReporting.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && taskReporting != null && taskReporting.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + taskReporting.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return taskReportingService.isRecallAble(tableInfoId);
	}

	@Action(value = "/WOM/taskReporting/taskReporting/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = taskReportingService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + taskReporting.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + taskReporting.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/taskReporting/taskReporting/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "batchApproval");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/taskReporting/taskReporting/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/taskReporting/taskReporting/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/taskReporting/taskReporting/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/taskReporting/taskReporting/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMTaskReportingService"
						, "com.supcon.orchid.WOM.services.WOMTaskReportingPartService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_taskReporting", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_taskReporting");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/taskReporting/taskReporting/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_taskReporting_TaskReporting", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_taskReporting_TaskReporting", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/taskReporting/taskReporting/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/taskReporting/taskReporting/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_taskReporting_TaskReporting";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReporting", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/taskReporting/taskReporting/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == taskReporting.getTableNo() ? "" : taskReporting.getTableNo().toString());
		_auditLog.setColumnStringA(null == taskReporting.getId() ? "" : taskReporting.getId().toString());
		_auditLog.setColumnStringC(taskReporting.getId().toString());
		_auditLog.setColumnStringD(WOMTaskReporting.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_taskReporting");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMTaskReportingAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, taskReporting,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, taskReporting,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,taskReporting);
		...
		printlog.setEntity("WOM_7.5.0.0_taskReporting");
		...
		printlog.setBusinessId(taskReporting.getId());
		printlog.setBussinessKey(null == taskReporting.getTableNo() ? "" : taskReporting.getTableNo().toString());
		printlog.setMainDisplay(null == taskReporting.getId() ? "" : taskReporting.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/taskReporting/taskReporting/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(taskReporting,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/taskReporting/taskReporting/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(taskReportingService.getTaskReporting(Long.parseLong(arrId[i])));
	...
	@Action(value = "/WOM/taskReporting/taskReporting/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/taskReporting/taskReporting/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, taskReporting,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				taskReporting = taskReportingService.getTaskReporting(Long.parseLong(arrId[i]));
				if(null != taskReporting){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == taskReporting.getId() ? "" : "," + taskReporting.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == taskReporting.getId() ? "" : taskReporting.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == taskReporting.getTableNo() ? "" : "," + taskReporting.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == taskReporting.getTableNo() ? "" : taskReporting.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == taskReporting.getId() ? "" : "," + taskReporting.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == taskReporting.getId() ? "" : taskReporting.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, taskReporting,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,taskReporting);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/taskReporting/taskReporting/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMTaskReporting> findExportPageInfos(Page<WOMTaskReporting> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_taskReporting" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = taskReportingService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMTaskReporting queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = taskReportingService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = taskReportingService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_taskReporting_TaskReporting");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_taskReporting_TaskReporting")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_taskReporting_TaskReporting");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_taskReporting_TaskReporting");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = taskReportingService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_taskReporting_TaskReporting,WOM_7.5.0.0) */
// 自定义代码
		if(code.equals("WOM_7.5.0.0_taskReporting_TaskReporting") && methodName.equals("submit")){
			if("cancel".equals(workFlowVar.getOutcomeType()) ){
				//作废处理
				//作废时执行  如果表体存在批次，则删除批次信息
				//验证为整批报工还是分批报工    整批返回true
				//if(taskReporting.getBatchNum()!=null && !taskReporting.getBatchNum().equals("") && taskReportingService.checkReportType(taskReporting)){
					//整批报工   after中处理
					//TODO
					//taskReportingService.createBatchDealInfo(taskReporting.getBatchNum(), taskReporting, null, "报工单作废处理","publicSetTableState/03");
				//}else if(taskReporting.getBatchNum()!=null && !taskReporting.getBatchNum().equals("") && !taskReportingService.checkReportType(taskReporting)){
					taskReportingService.delBatchNum(taskReporting);
				//}	
			}else {
				//每次保存时调用
				if(taskReporting.getId() == null){
					addNew = true;
				} else if(taskReportingService.checkOldExit(taskReporting.getId())){
					addNew = true;
				}else{
					//非首次保存
					//验证为整批报工还是分批报工    整批返回true
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_taskReporting_TaskReporting,WOM_7.5.0.0) */
// 自定义代码
		if(code.equals("WOM_7.5.0.0_taskReporting_TaskReporting") && methodName.equals("submit")){
			if(taskReporting.getStatus() == 99){
				//更新批次信息表
				taskReportingService.updateBatchInfo(taskReporting);
				//添加批次处理信息
				taskReportingService.addDealBatch(taskReporting,(SystemCode) systemCodeService.getSystemCode("publicSetTableState/02"),"报工单生效");
				//插入关系表
				taskReportingService.addBatchRelation(taskReporting);
			}
			if(addNew){		
				if(taskReporting.getBatchNum()!=null && !taskReporting.getBatchNum().equals("") && taskReportingService.checkReportType(taskReporting)){
					//添加批次处理信息
					taskReportingService.addDealBatch(taskReporting,(SystemCode) systemCodeService.getSystemCode("publicSetTableState/01"),"报工单整批报工");
				}else{
					List<WOMTaskReportingPart> dgJsonLists = new ArrayList<>();
					if (null!=dgLists.get("dg1492079431325")) {
						 dgJsonLists = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(dgLists.get("dg1492079431325"), WOMTaskReportingPart.class);
					}else if(null!=dgLists.get("dg1566214652249")){
						dgJsonLists = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(dgLists.get("dg1566214652249"), WOMTaskReportingPart.class);
					}
					//新增时添加批号信息  只在分批时执行
					taskReportingService.setBatch(taskReporting,dgJsonLists);
				}
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_taskReporting_TaskReporting,WOM_7.5.0.0) */
// 自定义代码
	private boolean addNew = false;//是否第一次保存
	private String produceTaskId;//指令单id
	private String produceTaskCode;//指定单编码
	private String produceId;//产品id
	private String produceCode;//产品编码
	private String produceName;//产品名称
	private String batchNum;//生产批号
	private String generalManage;//综合管理打开界面标识
	...
	public String getProduceTaskCode() {
		return produceTaskCode;
	}

	public void setProduceTaskCode(String produceTaskCode) {
	...
	public void updateDetail(){
		if(dgLists!=null){
			Map<String, BigDecimal> oldAmount=new HashMap<String,BigDecimal>();
			Map<String, BigDecimal> newAmount=new HashMap<String,BigDecimal>();
			
			//将当前的表体组成一个没有重复的批号list
			List<String> newBatchNum = new ArrayList<String>();
			List<WOMTaskReportingPart> dg1492079431325Lists = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(dgLists.get("dg1492079431325"), WOMTaskReportingPart.class);
			for (WOMTaskReportingPart reportingPart : dg1492079431325Lists) {
				if(reportingPart.getBatchNum()!=null && !reportingPart.getBatchNum().equals("")){
					if(!newBatchNum.contains(reportingPart.getBatchNum())){
						newBatchNum.add(reportingPart.getBatchNum());
						//如果此批号不存在，保存数量
						if(reportingPart.getReportNum()!=null){
							newAmount.put(reportingPart.getBatchNum(), reportingPart.getReportNum());
						}else{
							newAmount.put(reportingPart.getBatchNum(), new BigDecimal(0));
						}
					}else{
						//如果数量存在，数量进行累加
						if(reportingPart.getReportNum()!=null){
							newAmount.put(reportingPart.getBatchNum(), newAmount.get(reportingPart.getBatchNum()).add(reportingPart.getReportNum()));
						}else{
							//TODO
						}
					}
			...
			oldBatchNum = taskReportingService.getOldBatchNum(taskReporting);
			//获取之前批号数量
			oldAmount = taskReportingService.getBatchAmout(taskReporting);
			//执行新增或修改操作
			//taskReportingService.operate(taskReporting,newBatchNum,oldBatchNum,newAmount,oldAmount);
          	//分批-分批
			if(taskReporting.getBatchNum()!=null && !taskReporting.getBatchNum().equals("") && !taskReportingService.checkReportType(taskReporting) && !checkNew()){
				taskReportingService.operate(taskReporting,newBatchNum,oldBatchNum,newAmount,oldAmount);
			}
			//整批-整批
			else if(taskReporting.getBatchNum()!=null && !taskReporting.getBatchNum().equals("") && taskReportingService.checkReportType(taskReporting) && checkNew()){
				taskReportingService.operateAllToAll(taskReporting,newBatchNum,oldBatchNum,newAmount,oldAmount);
			}
			//整批-分批
			else if(taskReporting.getBatchNum()!=null && !taskReporting.getBatchNum().equals("") && taskReportingService.checkReportType(taskReporting) && !checkNew()){
				taskReportingService.operateAllToPart(taskReporting,newBatchNum,oldBatchNum,newAmount,oldAmount);
			}
			//分批-整批
			else if(taskReporting.getBatchNum()!=null && !taskReporting.getBatchNum().equals("") && !taskReportingService.checkReportType(taskReporting) && checkNew()){
				taskReportingService.operatePartToAll(taskReporting,newBatchNum,oldBatchNum,newAmount,oldAmount);
			}
		}
	}
	/**
	 * 验证当前表体为整批或分批    整批返回true
	 * @return
	 */
	public boolean checkNew(){
		List<WOMTaskReportingPart> dg1492079431325Lists = (List<WOMTaskReportingPart>) taskReportingService.generateObjectFromJson(dgLists.get("dg1492079431325"), WOMTaskReportingPart.class);
		if(dg1492079431325Lists!=null && dg1492079431325Lists.size()!=0){
			if(taskReporting.getBatchNum().equals(dg1492079431325Lists.get(0).getBatchNum())){
				return true;
			}else{
				return false;
			}
    ...
    private String 	taskReportingType;
    ...
    public String getTaskReportingType() {
        return taskReportingType;
    }

    public void setTaskReportingType(String taskReportingType) {
        this.taskReportingType = taskReportingType;
    }
    ...
    /**
     * 根据指令单ID或者活动ID来查找产出明细
     * @return
     */
    @Action(value = "/WOM/taskReporting/taskReporting/getProduceOutByTaskOrActive", results = { @Result(type = JSON, params = {
            "root", "responseMap","excludes", "*", "includes", "produceRecords,produceRecords.productID,produceRecords.productID.productSpecif,produceRecords.productID.productCode,produceRecords.productID.productName,produceRecords.reportStaff,produceRecords.reportStaff.name,produceRecords.reportDate,produceRecords.wareID,produceRecords.wareID.cargoStatue.id,produceRecords.wareID.wareCode,produceRecords.wareID.wareName,produceRecords.taskReportingPartList,produceRecords.taskReportingPartList.batchNum,produceRecords.taskReportingPartList.reportNum,produceRecords.taskReportingPartList.reportDate,produceRecords.taskReportingPartList.remark,produceRecords.taskReportingPartList.reportStaff.name,produceRecords.taskReportingPartList.storeID,produceRecords.taskReportingPartList.storeID.placeSetCode,produceRecords.taskReportingPartList.storeID.placeSetName,produceRecords.taskReportingPartList.reportDate" })
    })
    public String getBatchingRecordByProduceTaskId(){
        List<WOMTaskReporting> WOMTaskReportings = taskReportingService.getProduceOutByTaskOrActive(activeId,taskReportingType);

        responseMap.put("produceRecords", WOMTaskReportings);
        return SUCCESS;
    }
/* CUSTOM CODE END */
}