...
@Repository
public class WOMInventoryRecordDaoImpl extends ExtendedGenericDaoImpl<WOMInventoryRecord, Long> implements WOMInventoryRecordDao {
	@Override
	...
	@Override
	public WOMInventoryRecordDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMInventoryRecordDealInfo dealInfo = new WOMInventoryRecordDealInfo();
	...
	@Override
	public WOMInventoryRecordDealInfo copyAndSaveDealInfo(DealInfo di, WOMInventoryRecord mainObj, Staff dealStaff) {
		WOMInventoryRecordDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMInventoryRecordDealInfo.class);
		...
		List<WOMInventoryRecordDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
