...
@Service("wOMStandingcropRefImportService")
public class WOMStandingcropRefImportServiceImpl implements WOMStandingcropRefImportService {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_standingcropRef",model="WOM_7.5.0.0_standingcropRef_StandingcropRef",desc="保存业务信息",operType="1")
	public void saveImportDataLog(WOMStandingcropRef bizObj, WOMStandingcropRef oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("WOM.entityname.randon1490063991944") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getBatchText() ? "" : "," + oldBizObj.getBatchText().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getBatchText() ? "" : oldBizObj.getBatchText().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("WOM.entityname.randon1490063991944") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getBatchText() ? "" : "," + bizObj.getBatchText().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getBatchText() ? "" : bizObj.getBatchText().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(WOMStandingcropRef bizObj, String bussinesskeyFieldName,
