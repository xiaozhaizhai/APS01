...
import com.supcon.orchid.WOM.services.WOMManulOrderMainImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMManulOrderMainDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_manulOrderMainService")
@Transactional
public class WOMManulOrderMainServiceImpl  extends BaseServiceImpl<WOMManulOrderMain> implements WOMManulOrderMainService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMManulOrderMain> bapBaseService;
	...
	@Autowired
	private WOMManulOrderMainImportService manulOrderMainImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMManulOrderMain.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  manulOrderMainDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMManulOrderMain.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = manulOrderMainDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMManulOrderMain.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMManulOrderMain.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  manulOrderMainDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMManulOrderMain.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMManulOrderMain.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMManulOrderMain> list = (List<WOMManulOrderMain>) manulOrderMainDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMManulOrderMain getManulOrderMain(long id){
		return getManulOrderMain(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMManulOrderMain getManulOrderMain(long id, String viewCode){
		WOMManulOrderMain manulOrderMain = manulOrderMainDao.load(id);

		// 一对多情况处理
		if(manulOrderMain != null){
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getManulOrderMainAsJSON(long id, String include){
		WOMManulOrderMain manulOrderMain = manulOrderMainDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(manulOrderMain, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMain(WOMManulOrderMain manulOrderMain){
		if(null != manulOrderMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
			AuditUtil.setColumnStringA(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
			AuditUtil.setColumnStringC(manulOrderMain.getId().toString());
		}
		beforeDeleteManulOrderMain(manulOrderMain);
		manulOrderMainDao.delete(manulOrderMain);
		afterDeleteManulOrderMain(manulOrderMain);
		manulOrderMainDao.flush();
		List<WOMManulOrderMain> params = new ArrayList<WOMManulOrderMain>();
		params.add(manulOrderMain);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_manulOrder");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMain(List<Long> manulOrderMainIds){
		deleteManulOrderMain(manulOrderMainIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMain(List<Long> manulOrderMainIds, String eventTopic) {
		List<WOMManulOrderMain> manulOrderMains = new ArrayList<WOMManulOrderMain>();
		for(Long manulOrderMainId : manulOrderMainIds){
			WOMManulOrderMain manulOrderMain = getManulOrderMain(manulOrderMainId);
			manulOrderMains.add(manulOrderMain);
			if(null != manulOrderMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == manulOrderMain.getId() ? "" : "," + manulOrderMain.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == manulOrderMain.getId() ? "" : "," + manulOrderMain.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == manulOrderMain.getId() ? "" : "," + manulOrderMain.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
				}
			}
		}
		
		for(WOMManulOrderMain manulOrderMain : manulOrderMains){
			beforeDeleteManulOrderMain(manulOrderMain);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(manulOrderMains != null && manulOrderMains.size() > 0 ){
			for(WOMManulOrderMain manulOrderMain : manulOrderMains){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),manulOrderMain, manulOrderMain.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(manulOrderMainIds != null && manulOrderMainIds.size() > 0) {
			String hql = "update " + WOMManulOrderMain.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = manulOrderMainDao.createQuery(hql);
			query.setParameterList("ids", manulOrderMainIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMManulOrderMain manulOrderMainz : manulOrderMains){
				manulOrderMainz.setValid(false);
				List<WOMManulOrderMain> params = new ArrayList<WOMManulOrderMain>();
				params.add(manulOrderMainz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_manulOrder");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMain(long manulOrderMainId, int manulOrderMainVersion){
		this.deleteManulOrderMain(manulOrderMainId, manulOrderMainVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMain(long manulOrderMainId, int manulOrderMainVersion,SignatureLog signatureLog){
		WOMManulOrderMain manulOrderMain = getManulOrderMain(manulOrderMainId);
		if(manulOrderMain != null && !manulOrderMain.isValid()){
		...
		beforeDeleteManulOrderMain(manulOrderMain);
		manulOrderMainDao.delete(manulOrderMainId, manulOrderMainVersion);
		afterDeleteManulOrderMain(manulOrderMain);
		manulOrderMainDao.flush();
		List<WOMManulOrderMain> params = new ArrayList<WOMManulOrderMain>();
		params.add(manulOrderMain);
		...
		if(signatureLog != null ) {
			...
			businessKey=manulOrderMain.getId();
			signatureLog.setTableId(manulOrderMain.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMain(String manulOrderMainIds){
		this.deleteManulOrderMain(manulOrderMainIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMain(String manulOrderMainIds,SignatureLog signatureLog){
		deleteCollection(manulOrderMainIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteManulOrderMain(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_manulOrder#modelCode:WOM_7.5.0.0_manulOrder_ManulOrderMain#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="ec.engine.restore",operType="10")
	public void restoreManulOrderMain(String manulOrderMainIds){
		restoreCollection(manulOrderMainIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreManulOrderMain(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="ec.engine.restore",operType="10")
	public void restoreManulOrderMain(long manulOrderMainId){
		
		findBusinessKeyUsed(manulOrderMainId);	//判断业务主键是否重复
		...
		beforeRestoreManulOrderMain(manulOrderMain);
		manulOrderMain.setValid(true);
		manulOrderMainDao.update(manulOrderMain);
		afterRestoreManulOrderMain(manulOrderMain);
		List<WOMManulOrderMain> params = new ArrayList<WOMManulOrderMain>();
		params.add(manulOrderMain);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long manulOrderMainId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_manulOrder_ManulOrderMain");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMManulOrderMain.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMManulOrderMain.TABLE_NAME +" where id = ? )";
			List<Object> list =  manulOrderMainDao.createNativeQuery(sql,manulOrderMainId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseManulOrderMain(List<WOMManulOrderMain>  manulOrderMains){
		for(WOMManulOrderMain manulOrderMain:manulOrderMains)  {
			saveManulOrderMain(manulOrderMain, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseManulOrderMain(List<WOMManulOrderMain>  manulOrderMains){
		for(WOMManulOrderMain manulOrderMain:manulOrderMains)  {
			saveManulOrderMain(manulOrderMain, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="ec.flow.save")
	public void saveManulOrderMain(WOMManulOrderMain manulOrderMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveManulOrderMain(manulOrderMain, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="ec.flow.save")
	public void saveManulOrderMain(WOMManulOrderMain manulOrderMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveManulOrderMain(manulOrderMain, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateManulOrderMainCodes(WOMManulOrderMain manulOrderMain) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateManulOrderMainCodes(manulOrderMain, false);
	}
	
	public void generateManulOrderMainCodes(WOMManulOrderMain manulOrderMain, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateManulOrderMainSummarys(WOMManulOrderMain manulOrderMain) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateManulOrderMainSummarys(manulOrderMain, false);
	}

	public void generateManulOrderMainSummarys(WOMManulOrderMain manulOrderMain, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain")
	public void saveManulOrderMain(WOMManulOrderMain manulOrderMain,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveManulOrderMain(manulOrderMain, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain")
	public void saveManulOrderMain(WOMManulOrderMain manulOrderMain,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_manulOrder";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_manulOrder");
		String url = null;
		if(manulOrderMain.getId() != null && manulOrderMain.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMain.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/wOM_7500_manulOrder/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMain.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/wOM_7500_manulOrder/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, manulOrderMain);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(manulOrderMain);
		beforeSaveManulOrderMain(manulOrderMain, viewIsView);

		if (viewIsView) {
			manulOrderMainDao.saveWithRevertVersion(manulOrderMain);
		} else {
			if(isNew)
				manulOrderMainDao.save(manulOrderMain);
			else
				manulOrderMainDao.merge(manulOrderMain);
		}
		if(null != manulOrderMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
			AuditUtil.setColumnStringA(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
		...
		dealDatagridsSave(manulOrderMain,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateManulOrderMainCodes(manulOrderMain, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateManulOrderMainSummarys(manulOrderMain, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveManulOrderMain(manulOrderMain, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, manulOrderMain);
			}
		}
		List<WOMManulOrderMain> params = new ArrayList<WOMManulOrderMain>();
		manulOrderMainDao.flush();
		manulOrderMainDao.clear();
		manulOrderMain = manulOrderMainDao.load(manulOrderMain.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMManulOrderMain manulOrderMain){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("manulOrderMain", manulOrderMain);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain")
	public void mergeManulOrderMain(WOMManulOrderMain manulOrderMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(manulOrderMain);
		beforeSaveManulOrderMain(manulOrderMain);
		manulOrderMainDao.merge(manulOrderMain);
		if(null != manulOrderMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
			AuditUtil.setColumnStringA(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
		...
		dealDatagridsSave(manulOrderMain,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveManulOrderMain(manulOrderMain);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMManulOrderMain> findManulOrderMains(Page<WOMManulOrderMain> page, Criterion... criterions) {
		return manulOrderMainDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private WOMManulOrderMainDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<WOMManulOrderMainDealInfo> list=manulOrderMainDao.createQuery("from " + WOMManulOrderMainDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(WOMManulOrderMainDealInfo dealInfo) {
		manulOrderMainDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+WOMManulOrderMainDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) manulOrderMainDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+WOMManulOrderMainDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = manulOrderMainDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = manulOrderMainDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ WOMManulOrderMainDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = manulOrderMainDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findManulOrderMains(Page<WOMManulOrderMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findManulOrderMains(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findManulOrderMains(Page<WOMManulOrderMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findManulOrderMains(Page<WOMManulOrderMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMManulOrderMain> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"manulOrderMain\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"manulOrderMain\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"manulOrderMain\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_manulOrder_ManulOrderMain" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "WOM_7.5.0.0_manulOrder_ManulOrderMain" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  manulOrderMainDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  manulOrderMainDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_manulOrder").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"manulOrderMain\"","WOM_7.5.0.0_manulOrder",customCondition,"WOM_MENUL_ORDER_MAIN",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"manulOrderMain\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+WOMManulOrderMainDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"manulOrderMain\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"manulOrderMain\".LAY_REC ASC, \"manulOrderMain\".SORT ASC");
				} else {
					orderPart.append(" \"manulOrderMain\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) manulOrderMainDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=manulOrderMainDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = manulOrderMainDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMManulOrderMain> result = new ArrayList<WOMManulOrderMain>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMManulOrderMain.class,manulOrderMainDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMManulOrderMain.class,manulOrderMainDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMManulOrderMain manulOrderMain : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(manulOrderMain.getId(), "WOM_manulOrder_manulOrderMain");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(manulOrderMain.getId(), "WOM_manulOrder_manulOrderMain");
							if(null != documents && !documents.isEmpty()) {
								manulOrderMain.setDocument(documents.get(0));
								manulOrderMain.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("WOM_7.5.0.0_manulOrder")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("WOM_7.5.0.0_manulOrder")+"WOM_7.5.0.0_manulOrder".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("WOM_7.5.0.0_manulOrder",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<WOMManulOrderMain> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMManulOrderMain> result) {
		View view = null;
		...
		Map<Long, WOMManulOrderMain> rsMap = new HashMap<Long, WOMManulOrderMain>();
		for (WOMManulOrderMain manulOrderMain : result) {
			ids.add(manulOrderMain.getId());
			rsMap.put(manulOrderMain.getId(), manulOrderMain);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(manulOrderMainDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = manulOrderMainDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(WOMManulOrderMain manulOrderMain,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(manulOrderMain.getOwnerStaffId());
		ti.setOwnerPositionId(manulOrderMain.getOwnerPositionId());
		ti.setOwnerDepartmentId(manulOrderMain.getOwnerDepartmentId());
		
		ti.setCreateStaff(manulOrderMain.getCreateStaff());
		ti.setCreateStaffId(manulOrderMain.getCreateStaffId());
		ti.setCreateTime(manulOrderMain.getCreateTime());
		ti.setDeleteStaff(manulOrderMain.getDeleteStaff());
		ti.setDeleteStaffId(manulOrderMain.getDeleteStaffId());
		ti.setDeleteTime(manulOrderMain.getDeleteTime());
		ti.setCreateDepartmentId(manulOrderMain.getCreateDepartmentId());
		ti.setModifyStaff(manulOrderMain.getModifyStaff());
		ti.setModifyStaffId(manulOrderMain.getModifyStaffId());
		ti.setCreatePositionId(manulOrderMain.getCreatePositionId());
		ti.setPositionLayRec(manulOrderMain.getPositionLayRec());
		ti.setTableNo(manulOrderMain.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("WOM_7.5.0.0_manulOrder");
		ti.setTargetTableName("WOM_MENUL_ORDER_MAIN");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="WOM_7.5.0.0_manulOrder_ManulOrderMain" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  manulOrderMainDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"manulOrderMain\"","WOM_7.5.0.0_manulOrder","","WOM_MENUL_ORDER_MAIN",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"manulOrderMain\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"manulOrderMain\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"manulOrderMain\"","WOM_7.5.0.0_manulOrder","","WOM_MENUL_ORDER_MAIN",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("manulOrderMain.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"manulOrderMain\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"manulOrderMain\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"manulOrderMain\".CID = 1 OR \"manulOrderMain\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("manulOrderMain.mc\"").append(".MANUL_ORDER_MAIN) not in ("+notinValue+")");
			}
			
			SQLQuery query = manulOrderMainDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("manulOrder", "WOM_7.5.0.0_manulOrder", "manulOrder_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=manulOrderMainDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMManulOrderMain.ENTITY_CODE;}
	protected String getModuleCode(){return WOMManulOrderMain.MODULE_CODE;}
	protected String getModelCode(){return WOMManulOrderMain.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param manulOrderMain 模型实例
	 */
	public void checkUniqueConstraint(WOMManulOrderMain manulOrderMain){
	}
	
	
	@Override
	public List<WOMManulOrderMain> getManulOrderMains(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMManulOrderMain.class);
		if(params!=null){
			...
			return manulOrderMainDao.findByHql("from " + WOMManulOrderMain.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return manulOrderMainDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMManulOrderMain> findManulOrderMainsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMManulOrderMain.class);
	...
	@Override
	public List<WOMManulOrderMain> findManulOrderMainsByHql(String hql, Object... objects){
		
		return   manulOrderMainDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMManulOrderMain loadManulOrderMainByBussinessKey(WOMManulOrderMain manulOrderMain){
		return loadManulOrderMainByBussinessKey(manulOrderMain.getId());
	}
	
	@Override
	public Page<WOMManulOrderMain> getManulOrderMains(Page<WOMManulOrderMain> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMManulOrderMain.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = manulOrderMainDao.findByPage(page, sql);
			}else{
				page = manulOrderMainDao.findAllByPage(page);
			}
			return page;
		}
		page = manulOrderMainDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMManulOrderMain loadManulOrderMainByBussinessKey(Serializable bussinessKey){
		return manulOrderMainDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMManulOrderMain> findByProperty(String propertyName, Object object){
		return manulOrderMainDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMManulOrderMain findEntityByProperty(String propertyName, Object object){
		return manulOrderMainDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMManulOrderMain.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = manulOrderMainDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_manulOrder_ManulOrderMain","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("WOM_7.5.0.0_manulOrder", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(WOMManulOrderMain manulOrderMain,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,manulOrderMainDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMManulOrderMain.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, manulOrderMain);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",manulOrderMain.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMManulOrderMain.TABLE_NAME +" where valid = 1";
			list = manulOrderMainDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMManulOrderMain.TABLE_NAME;
			list = manulOrderMainDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMManulOrderMain.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = manulOrderMainDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMManulOrderMainDaoImpl manulOrderMainDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchManulOrderMain(final List<WOMManulOrderMain> insertObjs, final List<WOMManulOrderMain> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchManulOrderMain(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchManulOrderMain(final List<WOMManulOrderMain> insertObjs, final List<WOMManulOrderMain> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1563192841368"),importCount));
		...
		Map<Long, WOMManulOrderMain> oldUpdateObjsMap = new HashMap<Long, WOMManulOrderMain>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMManulOrderMain bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMManulOrderMain> bizObjList = manulOrderMainDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMManulOrderMain bizObj = bizObjList.get(mIndex);
		...
		final List<WOMManulOrderMain> nmeObjs = new ArrayList<WOMManulOrderMain>();
		...
		long id=autoGeneratorID.getNextId(WOMManulOrderMain.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMManulOrderMain manulOrderMain:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMManulOrderMain.TABLE_NAME,1,dbId);
			if(manulOrderMain.getId() != null ){
				m.put(manulOrderMain.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			manulOrderMain.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) manulOrderMainDao.createNativeQuery(sql, "WOM_7.5.0.0_manulOrder_ManulOrderMain"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		manulOrderMainDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMManulOrderMain.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMManulOrderMain manulOrderMain = updateObjs.get(updateIndex);
						Long id = manulOrderMain.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = manulOrderMain.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(manulOrderMain);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMManulOrderMain.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMManulOrderMain.TABLE_NAME + " order by id desc";
//					Query query = manulOrderMainDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMManulOrderMain manulOrderMain = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = manulOrderMain.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		manulOrderMainDaoImpl.getSession().flush();
		manulOrderMainDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMManulOrderMain bizObj = insertObjs.get(insertIndex);
			manulOrderMainImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMManulOrderMain bizObj = updateObjs.get(updateIndex);
			WOMManulOrderMain oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			manulOrderMainImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = manulOrderMainDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				manulOrderMainDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = manulOrderMainDao.createNativeQuery(sql, "WOM_7.5.0.0_manulOrder_ManulOrderMain").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = manulOrderMainDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = manulOrderMainDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = manulOrderMainDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?manulOrderMainDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  manulOrderMainDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,manulOrderMainDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1563346128169Page(Page<WOMManulOrderMaterial> dg1563346128169Page,WOMManulOrderMain manulOrderMain, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMManulOrderMaterial.class);
		criteria.add(Restrictions.eq("manulOrderId", manulOrderMain));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"manulOrderMaterial\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		manulOrderMaterialDao.findByPage(dg1563346128169Page, criteria);
		manulOrderMaterialService.findByNormalDataGridManulOrderMain(dg1563346128169Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1568964886993Page(Page<WOMManulOrderMaterial> dg1568964886993Page,WOMManulOrderMain manulOrderMain, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(WOMManulOrderMaterial.class);
		criteria.add(Restrictions.eq("manulOrderId", manulOrderMain));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		manulOrderMaterialDao.findByPage(dg1568964886993Page, criteria);
		manulOrderMaterialService.findByNormalDataGridManulOrderMain(dg1568964886993Page);
	}
	
	 
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="ec.flow.save")
	public void saveManulOrderMain(WOMManulOrderMain manulOrderMain, DataGridService dataGridService){
		saveManulOrderMain(manulOrderMain, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="ec.flow.save")
	public void saveManulOrderMain(WOMManulOrderMain manulOrderMain, DataGridService dataGridService, String viewCode){
		saveManulOrderMain(manulOrderMain, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain")
	public void saveManulOrderMain(WOMManulOrderMain manulOrderMain, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveManulOrderMain(manulOrderMain, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain")
	public void saveManulOrderMain(WOMManulOrderMain manulOrderMain, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_manulOrder";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_manulOrder");
		String url = null;
		if(null != manulOrderMain.getId() && manulOrderMain.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMain.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain")
	public Map<Object, Object> manulOrderMainDataGridImport(WOMManulOrderMain manulOrderMain, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = manulOrderMain.getVirtualId();
		
		manulOrderMain = manulOrderMainDao.load(manulOrderMain.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, manulOrderMain.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = manulOrderMain.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(manulOrderMain);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, manulOrderMain.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain")
	public void mergeManulOrderMain(WOMManulOrderMain manulOrderMain, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(manulOrderMain);
		beforeSaveManulOrderMain(manulOrderMain);
		manulOrderMainDao.merge(manulOrderMain);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMain",desc="foundation.batch.save",operType="4")
	public void batchSaveManulOrderMain(WOMManulOrderMain manulOrderMain, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_manulOrder";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_manulOrder");
	...
	@Override
	public Page<WOMManulOrderMain> getByPage(Page<WOMManulOrderMain> page,DetachedCriteria detachedCriteria){
		return manulOrderMainDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("WOM_7.5.0.0_manulOrder").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveManulOrderMain(WOMManulOrderMain manulOrderMain, Object... objects){
		checkUniqueConstraint(manulOrderMain);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveManulOrderMain(WOMManulOrderMain manulOrderMain, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportManulOrderMain(List<WOMManulOrderMain> manulOrderMains, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportManulOrderMain(List<WOMManulOrderMain> manulOrderMains){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportManulOrderMain(List<WOMManulOrderMain> manulOrderMains, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportManulOrderMain(List<WOMManulOrderMain> manulOrderMains){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteManulOrderMain(WOMManulOrderMain manulOrderMain){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteManulOrderMain(WOMManulOrderMain manulOrderMain){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreManulOrderMain(WOMManulOrderMain manulOrderMain){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreManulOrderMain(WOMManulOrderMain manulOrderMain){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMManulOrderMain manulOrderMain, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMManulOrderMain manulOrderMain, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportManulOrderMain(List<WOMManulOrderMain> insertObjs, List<WOMManulOrderMain> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportManulOrderMain,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportManulOrderMain(List<WOMManulOrderMain> insertObjs, List<WOMManulOrderMain> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportManulOrderMain,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
	@Override
	public void finshOrder(String orderIDs,String flag) {
		...
		if(idList != null && idList.size() > 0) {
			if (null!=flag && !("".equals(flag))) {
				String hql = "update " + WOMManulOrderMain.JPA_NAME + " set isFinshed = false where id in(:ids)";
				Query query = manulOrderMainDao.createQuery(hql);
				query.setParameterList("ids", idList);
				query.executeUpdate();

			}else {
				String hql = "update " + WOMManulOrderMain.JPA_NAME + " set isFinshed = true where id in(:ids)";
				Query query = manulOrderMainDao.createQuery(hql);
				query.setParameterList("ids", idList);
				query.executeUpdate();
			}
