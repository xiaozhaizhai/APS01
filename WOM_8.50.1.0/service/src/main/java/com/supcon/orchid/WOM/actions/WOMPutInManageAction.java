...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMPutInManageAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMPutInManage> {
	...
	private WOMPutInManage putInManage;
	...
	//导出excel时,主模型内容
	List<WOMPutInManage> queryResults = new ArrayList<WOMPutInManage>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/putInManage/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/putInManage/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_generalManage_PutInManage" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMPutInManage> putInManages;
	
	public List<WOMPutInManage> getPutInManages() {
		return putInManages;
	}
	...
	private Page<WOMPutInManage> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			putInManage = new WOMPutInManage();
		} else {
			if(id > 0){
				putInManage = putInManageService.getPutInManage(id, viewCode);
			}
		}
	...
	public Page<WOMPutInManage> getPage() {
		if (null == page) {
			page = new Page<WOMPutInManage>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMPutInManage getPutInManage() {
		return putInManage;
	}		

	public void setPutInManage(WOMPutInManage putInManage) {
		this.putInManage = putInManage;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/putInManage/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = putInManageService.getPutInManageAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/putInManage/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1502695666438
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String putInManageReferenceCopy() throws Exception{
			WOMPutInManage oldPutInManage = null;
			if(isEditNew()){
				...
				oldPutInManage = new WOMPutInManage();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,putInManage.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, putInManage.getId());
				...
				oldPutInManage = putInManageService.getPutInManage(id);
			}
			
			
			putInManage = putInManageService.getPutInManage(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_PutInManage");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMPutInManage.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPutInManage, null);
						Method setValue = WOMPutInManage.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(putInManage, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			putInManage.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=putInManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return putInManageReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			putInManage = putInManageService.getPutInManage(this.putInManage.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,putInManage.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = putInManageService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(putInManageService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(putInManageService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_generalManage_PutInManage", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"putInManage\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"putInManage\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"putInManage\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"putInManage\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			putInManageService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		putInManageService.findDataGridPage(dg,dgclass,getDgPage(),putInManage,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(putInManage.getId() != null && putInManage.getId() != -1){
				putInManage = putInManageService.getPutInManage(putInManage.getId());
			}
			putInManageService.findDataGridPage(dg,dgclass,getDgPage(),putInManage,searchCondition,params);
	...
	private void save() throws Exception {
		...
		putInManageService.savePutInManage(putInManage,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "submit");
		bap_validate_datagrids=putInManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", putInManage.getId());
		...
		fileuploads.put("linkId", putInManage.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/generalManage/putInManage/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		putInManageService.deletePutInManage(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/putInManage/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "restore");
		putInManageService.restorePutInManage(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMPutInManage> findExportDataInfos(Page<WOMPutInManage> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = putInManageService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = putInManageService.getAssProperty(p.getCode());	
									//String assModelCode = putInManageService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			putInManageService.findPutInManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			putInManageService.findPutInManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMPutInManage> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMPutInManage result : results){
	...
	public boolean isEditNew(){
		//return null == this.putInManage || null == this.putInManage.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/generalManage/putInManage/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/generalManage/putInManage/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/generalManage/putInManage/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/putInManage/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMPutInManageService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/putInManage/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_generalManage_PutInManage", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_generalManage_PutInManage", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/generalManage/putInManage/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/generalManage/putInManage/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_generalManage_PutInManage";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_PutInManage", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/generalManage/putInManage/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == putInManage.getId() ? "" : putInManage.getId().toString());
		_auditLog.setColumnStringA(null == putInManage.getId() ? "" : putInManage.getId().toString());
		_auditLog.setColumnStringC(putInManage.getId().toString());
		_auditLog.setColumnStringD(WOMPutInManage.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMPutInManageAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, putInManage,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, putInManage,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,putInManage);
		...
		printlog.setBusinessId(putInManage.getId());
		printlog.setBussinessKey(null == putInManage.getId() ? "" : putInManage.getId().toString());
		printlog.setMainDisplay(null == putInManage.getId() ? "" : putInManage.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/generalManage/putInManage/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(putInManage,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/putInManage/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(putInManageService.getPutInManage(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/generalManage/putInManage/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/generalManage/putInManage/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, putInManage,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				putInManage = putInManageService.getPutInManage(Long.parseLong(arrId[i]));
				if(null != putInManage){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == putInManage.getId() ? "" : "," + putInManage.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == putInManage.getId() ? "" : putInManage.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == putInManage.getId() ? "" : "," + putInManage.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == putInManage.getId() ? "" : putInManage.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == putInManage.getId() ? "" : "," + putInManage.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == putInManage.getId() ? "" : putInManage.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, putInManage,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,putInManage);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/generalManage/putInManage/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_PutInManage");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_generalManage_PutInManage")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_generalManage_PutInManage");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_PutInManage");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = putInManageService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_generalManage_PutInManage,WOM_7.5.0.0) */
// 自定义代码
