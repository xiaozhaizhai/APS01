...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchNumRuleAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchNumRule> {
	...
	//导出excel时,主模型内容
	List<WOMBatchNumRule> queryResults = new ArrayList<WOMBatchNumRule>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchNumRule/batchNumRule/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchNumRule/batchNumRule/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchNumRule_BatchNumRule" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchNumRule> batchNumRules;
	
	public List<WOMBatchNumRule> getBatchNumRules() {
		return batchNumRules;
	}
	...
	private Page<WOMBatchNumRule> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchNumRule = new WOMBatchNumRule();
			if(batchNumRule.getRuleType() != null) {
				batchNumRule.setRuleType((SystemCode) systemCodeService.getSystemCode(batchNumRule.getRuleType().getId()));
			}
		} else {
			if(id > 0){
				batchNumRule = batchNumRuleService.getBatchNumRule(id, viewCode);
			}
		}
	...
	public Page<WOMBatchNumRule> getPage() {
		if (null == page) {
			page = new Page<WOMBatchNumRule>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchNumRule/batchNumRule/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchNumRuleService.getBatchNumRuleAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchNumRule/batchNumRule/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String batchNumRuleReferenceCopy() throws Exception{
			WOMBatchNumRule oldBatchNumRule = null;
			if(isEditNew()){
				...
				oldBatchNumRule = new WOMBatchNumRule();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchNumRule.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchNumRule.getId());
				...
				oldBatchNumRule = batchNumRuleService.getBatchNumRule(id);
			}
			
			
			batchNumRule = batchNumRuleService.getBatchNumRule(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchNumRule_BatchNumRule");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchNumRule.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchNumRule, null);
						Method setValue = WOMBatchNumRule.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchNumRule, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchNumRule.setCreateDepartmentId(oldBatchNumRule.getCreateDepartmentId());
			batchNumRule.setCreatePositionId(oldBatchNumRule.getCreatePositionId());
			batchNumRule.setCreateStaffId(oldBatchNumRule.getCreateStaffId());
			batchNumRule.setOwnerDepartmentId(oldBatchNumRule.getOwnerDepartmentId());
			batchNumRule.setOwnerPositionId(oldBatchNumRule.getOwnerPositionId());
			batchNumRule.setOwnerStaffId(oldBatchNumRule.getOwnerStaffId());
			batchNumRule.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchNumRule/batchNumRule/batchRuleEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchRuleEdit.ftl") }),
		@Action(value = "/WOM/batchNumRule/batchNumRule/batchRuleEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchRuleEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchNumRuleService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchNumRuleReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchNumRule = batchNumRuleService.getBatchNumRule(this.batchNumRule.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchNumRule.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/WOM/batchNumRule/batchNumRule/batchRuleList", results = { @Result(type = FREEMARKER, location = "batchRuleList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchNumRuleService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchNumRuleService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchNumRuleService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchNumRule_BatchNumRule", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchNumRule/batchNumRule/batchRulRef", results = { @Result(type = FREEMARKER, location = "batchRulRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/batchNumRule/batchNumRule/batchRuleList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.ruleType.id,result.ruleType.value,result.isAccountPeriod,result.menu,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batchRuleList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchNumRule\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchNumRule\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchNumRule\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchNumRule\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchNumRuleService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/batchNumRule/batchNumRule/batchRuleList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/batchNumRule/batchNumRule/batchRulRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.ruleType.id,result.ruleType.value,result.isAccountPeriod,result.menu,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchRulRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/batchNumRule/batchNumRule/allTempleteDownload", results = { @Result(name="excel",type="excel",location="batchRuleList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_batchNumRule_BatchNumRule");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchNumRuleService.findDataGridPage(dg,dgclass,getDgPage(),batchNumRule,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchNumRule.getId() != null && batchNumRule.getId() != -1){
				batchNumRule = batchNumRuleService.getBatchNumRule(batchNumRule.getId());
			}
			batchNumRuleService.findDataGridPage(dg,dgclass,getDgPage(),batchNumRule,searchCondition,params);
	...
	private void save() throws Exception {
		...
		batchNumRuleService.saveBatchNumRule(batchNumRule,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/batchNumRule/batchNumRule/batchRuleEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "submit");
		bap_validate_datagrids=batchNumRuleService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchNumRule.getId());
		...
		fileuploads.put("linkId", batchNumRule.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchNumRule/batchNumRule/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchNumRuleService.deleteBatchNumRule(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchNumRule/batchNumRule/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "restore");
		batchNumRuleService.restoreBatchNumRule(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/batchNumRule/batchNumRule/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/batchNumRule/batchNumRule/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "showDealInfoData");
		if (null != tableInfoId) {
			int count=batchNumRuleService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = batchNumRuleService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = batchNumRuleService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchNumRule> findExportDataInfos(Page<WOMBatchNumRule> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchNumRuleService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchNumRuleService.getAssProperty(p.getCode());	
									//String assModelCode = batchNumRuleService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchNumRuleService.findBatchNumRules(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchNumRuleService.findBatchNumRules(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchNumRule> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchNumRule result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchNumRule || null == this.batchNumRule.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/batchNumRule/batchNumRule/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchNumRule/batchNumRule/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchNumRule/batchNumRule/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchNumRule/batchNumRule/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchNumRuleService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_batchNumRule", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_batchNumRule");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchNumRule/batchNumRule/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchNumRule_BatchNumRule", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchNumRule_BatchNumRule", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchNumRule/batchNumRule/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchNumRule/batchNumRule/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchNumRule_BatchNumRule";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchNumRule_BatchNumRule", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchNumRule/batchNumRule/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMBatchNumRule batchNumRule = null;
		if(null != id) {
			batchNumRule = batchNumRuleService.getBatchNumRule(id);
		}
		...
		_auditLog.setColumnStringB(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
		_auditLog.setColumnStringA(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
		_auditLog.setColumnStringC(batchNumRule.getId().toString());
		_auditLog.setColumnStringD(WOMBatchNumRule.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_batchNumRule");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchNumRuleAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchNumRule,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchNumRule,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchNumRule);
		...
		printlog.setEntity("WOM_7.5.0.0_batchNumRule");
		...
		printlog.setBusinessId(batchNumRule.getId());
		printlog.setBussinessKey(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
		printlog.setMainDisplay(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchNumRule/batchNumRule/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchNumRule,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchNumRule/batchNumRule/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchNumRuleService.getBatchNumRule(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/batchNumRule/batchNumRule/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchNumRule/batchNumRule/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchNumRule,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchNumRule = batchNumRuleService.getBatchNumRule(Long.parseLong(arrId[i]));
				if(null != batchNumRule){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchNumRule.getId() ? "" : "," + batchNumRule.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchNumRule.getId() ? "" : "," + batchNumRule.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchNumRule.getId() ? "" : "," + batchNumRule.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchNumRule.getId() ? "" : batchNumRule.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchNumRule,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchNumRule);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/batchNumRule/batchNumRule/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMBatchNumRule> findExportPageInfos(Page<WOMBatchNumRule> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_batchNumRule" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = batchNumRuleService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMBatchNumRule queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = batchNumRuleService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = batchNumRuleService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchNumRule_BatchNumRule");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchNumRule_BatchNumRule")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchNumRule_BatchNumRule");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchNumRule_BatchNumRule");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchNumRuleService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
		if(code.equals("WOM_7.5.0.0_batchNumRule_batchRuleEdit")){
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceTask");
			if(model==null){
				throw new BAPException("WOM.showMessage00000005","WOM_7.5.0.0_produceTask_ProduceTask");
			}
		}
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
    private Model model;
	 ...
	 */
	@Actions({ 
		@Action(value = "/WOM/property/select_dateproperty", results = { @Result(name = SUCCESS, type = FREEMARKER, location = "/views/ec/property/select_dateproperty.ftl") })
		,@Action(value = "/WOM/property/select_property", results = { @Result(name = SUCCESS, type = FREEMARKER, location = "/views/ec/property/select_property.ftl") })
	})
	public String select_property() throws Exception {
		model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceTask_ProduceTask");
		if(model!=null && model.getCode()!=null) {
			model = modelServiceFoundation.getModelWithProperties(model.getCode());
			model.setAssociatedInfos(modelServiceFoundation.findAssociatedInfoNotIncludeBackAsso(model));
		}
		return SUCCESS;
	}
/* CUSTOM CODE END */
