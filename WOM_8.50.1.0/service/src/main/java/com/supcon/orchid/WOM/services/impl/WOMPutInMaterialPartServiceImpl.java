...
import com.supcon.orchid.WOM.services.WOMPutInMaterialPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMPutInMaterialPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_putInMaterialPartService")
@Transactional
public class WOMPutInMaterialPartServiceImpl  extends BaseServiceImpl<WOMPutInMaterialPart> implements WOMPutInMaterialPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMPutInMaterialPart> bapBaseService;
	...
	@Autowired
	private WOMPutInMaterialPartImportService putInMaterialPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMPutInMaterialPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  putInMaterialPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMPutInMaterialPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = putInMaterialPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMPutInMaterialPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMPutInMaterialPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  putInMaterialPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMPutInMaterialPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMPutInMaterialPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMPutInMaterialPart> list = (List<WOMPutInMaterialPart>) putInMaterialPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMPutInMaterialPart getPutInMaterialPart(long id){
		return getPutInMaterialPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPutInMaterialPart getPutInMaterialPart(long id, String viewCode){
		WOMPutInMaterialPart putInMaterialPart = putInMaterialPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPutInMaterialPartAsJSON(long id, String include){
		WOMPutInMaterialPart putInMaterialPart = putInMaterialPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(putInMaterialPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart){
		if(null != putInMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
			AuditUtil.setColumnStringA(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
			AuditUtil.setColumnStringC(putInMaterialPart.getId().toString());
		}
		beforeDeletePutInMaterialPart(putInMaterialPart);
		putInMaterialPartDao.deletePhysical(putInMaterialPart);
		afterDeletePutInMaterialPart(putInMaterialPart);
		putInMaterialPartDao.flush();
		List<WOMPutInMaterialPart> params = new ArrayList<WOMPutInMaterialPart>();
		params.add(putInMaterialPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePutInMaterialPart(List<Long> putInMaterialPartIds){
		deletePutInMaterialPart(putInMaterialPartIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePutInMaterialPart(List<Long> putInMaterialPartIds, String eventTopic) {
		List<WOMPutInMaterialPart> putInMaterialParts = new ArrayList<WOMPutInMaterialPart>();
		for(Long putInMaterialPartId : putInMaterialPartIds){
			WOMPutInMaterialPart putInMaterialPart = getPutInMaterialPart(putInMaterialPartId);
			putInMaterialParts.add(putInMaterialPart);
			if(null != putInMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == putInMaterialPart.getId() ? "" : "," + putInMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == putInMaterialPart.getId() ? "" : "," + putInMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == putInMaterialPart.getId() ? "" : "," + putInMaterialPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
				}
			}
		}
		...
		if(putInMaterialParts != null && putInMaterialParts.size() > 0 ){
			for(WOMPutInMaterialPart putInMaterialPart : putInMaterialParts){
				...
				java.lang.reflect.Field[] fields = WOMPutInMaterialPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(putInMaterialPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),putInMaterialPart, putInMaterialPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(putInMaterialPartIds != null && putInMaterialPartIds.size() > 0) {
			String hql = "delete " + WOMPutInMaterialPart.JPA_NAME + " where id in(:ids)";
			Query query = putInMaterialPartDao.createQuery(hql);
			query.setParameterList("ids", putInMaterialPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMPutInMaterialPart putInMaterialPartz : putInMaterialParts){
				putInMaterialPartz.setValid(false);
				List<WOMPutInMaterialPart> params = new ArrayList<WOMPutInMaterialPart>();
				params.add(putInMaterialPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_putInMaterial");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePutInMaterialPart(long putInMaterialPartId, int putInMaterialPartVersion){
		this.deletePutInMaterialPart(putInMaterialPartId, putInMaterialPartVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePutInMaterialPart(long putInMaterialPartId, int putInMaterialPartVersion,SignatureLog signatureLog){
		WOMPutInMaterialPart putInMaterialPart = getPutInMaterialPart(putInMaterialPartId);
		...
		beforeDeletePutInMaterialPart(putInMaterialPart);
		putInMaterialPartDao.delete(putInMaterialPartId, putInMaterialPartVersion);
		afterDeletePutInMaterialPart(putInMaterialPart);
		putInMaterialPartDao.flush();
		List<WOMPutInMaterialPart> params = new ArrayList<WOMPutInMaterialPart>();
		params.add(putInMaterialPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=putInMaterialPart.getId();
			signatureLog.setTableId(putInMaterialPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePutInMaterialPart(String putInMaterialPartIds){
		this.deletePutInMaterialPart(putInMaterialPartIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.common.delete",operType="3")
	public void deletePutInMaterialPart(String putInMaterialPartIds,SignatureLog signatureLog){
		deleteCollection(putInMaterialPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePutInMaterialPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_putInMaterial#modelCode:WOM_7.5.0.0_putInMaterial_PutInMaterialPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="ec.engine.restore",operType="10")
	public void restorePutInMaterialPart(String putInMaterialPartIds){
		restoreCollection(putInMaterialPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePutInMaterialPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="ec.engine.restore",operType="10")
	public void restorePutInMaterialPart(long putInMaterialPartId){
		
		findBusinessKeyUsed(putInMaterialPartId);	//判断业务主键是否重复
		...
		beforeRestorePutInMaterialPart(putInMaterialPart);
		putInMaterialPart.setValid(true);
		putInMaterialPartDao.update(putInMaterialPart);
		afterRestorePutInMaterialPart(putInMaterialPart);
		List<WOMPutInMaterialPart> params = new ArrayList<WOMPutInMaterialPart>();
		params.add(putInMaterialPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long putInMaterialPartId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_putInMaterial_PutInMaterialPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMPutInMaterialPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMPutInMaterialPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  putInMaterialPartDao.createNativeQuery(sql,putInMaterialPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePutInMaterialPart(List<WOMPutInMaterialPart>  putInMaterialParts){
		for(WOMPutInMaterialPart putInMaterialPart:putInMaterialParts)  {
			savePutInMaterialPart(putInMaterialPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePutInMaterialPart(List<WOMPutInMaterialPart>  putInMaterialParts){
		for(WOMPutInMaterialPart putInMaterialPart:putInMaterialParts)  {
			savePutInMaterialPart(putInMaterialPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="ec.flow.save")
	public void savePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePutInMaterialPart(putInMaterialPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="ec.flow.save")
	public void savePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePutInMaterialPart(putInMaterialPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePutInMaterialPartCodes(WOMPutInMaterialPart putInMaterialPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePutInMaterialPartCodes(putInMaterialPart, false);
	}
	
	public void generatePutInMaterialPartCodes(WOMPutInMaterialPart putInMaterialPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePutInMaterialPartSummarys(WOMPutInMaterialPart putInMaterialPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePutInMaterialPartSummarys(putInMaterialPart, false);
	}

	public void generatePutInMaterialPartSummarys(WOMPutInMaterialPart putInMaterialPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart")
	public void savePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePutInMaterialPart(putInMaterialPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart")
	public void savePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_putInMaterial");
		String url = null;
		if(putInMaterialPart.getId() != null && putInMaterialPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, putInMaterialPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(putInMaterialPart);
		beforeSavePutInMaterialPart(putInMaterialPart, viewIsView);

		if (viewIsView) {
			putInMaterialPartDao.saveWithRevertVersion(putInMaterialPart);
		} else {
			if(isNew)
				putInMaterialPartDao.save(putInMaterialPart);
			else
		...
		dealDatagridsSave(putInMaterialPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePutInMaterialPartCodes(putInMaterialPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePutInMaterialPartSummarys(putInMaterialPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePutInMaterialPart(putInMaterialPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, putInMaterialPart);
			}
		}
		List<WOMPutInMaterialPart> params = new ArrayList<WOMPutInMaterialPart>();
		putInMaterialPartDao.flush();
		putInMaterialPartDao.clear();
		putInMaterialPart = putInMaterialPartDao.load(putInMaterialPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMPutInMaterialPart putInMaterialPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("putInMaterialPart", putInMaterialPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart")
	public void mergePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(putInMaterialPart);
		beforeSavePutInMaterialPart(putInMaterialPart);
		putInMaterialPartDao.merge(putInMaterialPart);
		if(null != putInMaterialPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
			AuditUtil.setColumnStringA(null == putInMaterialPart.getId() ? "" : putInMaterialPart.getId().toString());
		...
		dealDatagridsSave(putInMaterialPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePutInMaterialPart(putInMaterialPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMPutInMaterialPart> findPutInMaterialParts(Page<WOMPutInMaterialPart> page, Criterion... criterions) {
		return putInMaterialPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPutInMaterialParts(Page<WOMPutInMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPutInMaterialParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPutInMaterialParts(Page<WOMPutInMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPutInMaterialParts(Page<WOMPutInMaterialPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMPutInMaterialPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"putInMaterialPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"putInMaterialPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"putInMaterialPart\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"putInMaterialPart\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PUT_IN_MATERIALS \"putInMaterial\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_putInMaterial" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  putInMaterialPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  putInMaterialPartDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_putInMaterial").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = dataPermissionService.getPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"putInMaterial\"","WOM_7.5.0.0_putInMaterial","","WOM_PUT_IN_MATERIALS",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"putInMaterialPart\".LAY_REC ASC, \"putInMaterialPart\".SORT ASC");
				} else {
					orderPart.append(" \"putInMaterialPart\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) putInMaterialPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=putInMaterialPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = putInMaterialPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMPutInMaterialPart> result = new ArrayList<WOMPutInMaterialPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMPutInMaterialPart.class,putInMaterialPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMPutInMaterialPart.class,putInMaterialPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMPutInMaterialPart putInMaterialPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(putInMaterialPart.getId(), "WOM_putInMaterial_putInMaterialPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(putInMaterialPart.getId(), "WOM_putInMaterial_putInMaterialPart");
							if(null != documents && !documents.isEmpty()) {
								putInMaterialPart.setDocument(documents.get(0));
								putInMaterialPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMPutInMaterialPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMPutInMaterialPart> result) {
		View view = null;
		...
		Map<Long, WOMPutInMaterialPart> rsMap = new HashMap<Long, WOMPutInMaterialPart>();
		for (WOMPutInMaterialPart putInMaterialPart : result) {
			ids.add(putInMaterialPart.getId());
			rsMap.put(putInMaterialPart.getId(), putInMaterialPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(putInMaterialPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = putInMaterialPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_putInMaterial_PutInMaterialPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_putInMaterial_PutInMaterialPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  putInMaterialPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"putInMaterialPart\"","WOM_7.5.0.0_putInMaterial","","WOM_PUT_IN_MATERIAL_PARTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"putInMaterialPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"putInMaterialPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"putInMaterialPart\"","WOM_7.5.0.0_putInMaterial","","WOM_PUT_IN_MATERIAL_PARTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_putInMaterial_PutInMaterialPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("putInMaterialPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"putInMaterialPart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"putInMaterialPart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"putInMaterialPart\".CID = 1 OR \"putInMaterialPart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("putInMaterialPart.mc\"").append(".PUT_IN_MATERIAL_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = putInMaterialPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=putInMaterialPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMPutInMaterialPart.ENTITY_CODE;}
	protected String getModuleCode(){return WOMPutInMaterialPart.MODULE_CODE;}
	protected String getModelCode(){return WOMPutInMaterialPart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param putInMaterialPart 模型实例
	 */
	public void checkUniqueConstraint(WOMPutInMaterialPart putInMaterialPart){
	}
	
	
	@Override
	public List<WOMPutInMaterialPart> getPutInMaterialParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPutInMaterialPart.class);
		if(params!=null){
			...
			return putInMaterialPartDao.findByHql("from " + WOMPutInMaterialPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return putInMaterialPartDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMPutInMaterialPart> findPutInMaterialPartsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPutInMaterialPart.class);
	...
	@Override
	public List<WOMPutInMaterialPart> findPutInMaterialPartsByHql(String hql, Object... objects){
		
		return   putInMaterialPartDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMPutInMaterialPart loadPutInMaterialPartByBussinessKey(WOMPutInMaterialPart putInMaterialPart){
		return loadPutInMaterialPartByBussinessKey(putInMaterialPart.getId());
	}
	
	@Override
	public Page<WOMPutInMaterialPart> getPutInMaterialParts(Page<WOMPutInMaterialPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMPutInMaterialPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = putInMaterialPartDao.findByPage(page, sql);
			}else{
				page = putInMaterialPartDao.findAllByPage(page);
			}
			return page;
		}
		page = putInMaterialPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMPutInMaterialPart> findByTreeDataGridPutInMaterial(List<WOMPutInMaterialPart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMPutInMaterialPart putInMaterialPart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridPutInMaterial(Page<WOMPutInMaterialPart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMPutInMaterialPart> putInMaterialParts = dgPage.getResult();
			for(WOMPutInMaterialPart putInMaterialPart : putInMaterialParts){
				
			}
			dgPage.setResult(putInMaterialParts);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPutInMaterialPart loadPutInMaterialPartByBussinessKey(Serializable bussinessKey){
		return putInMaterialPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMPutInMaterialPart> findByProperty(String propertyName, Object object){
		return putInMaterialPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMPutInMaterialPart findEntityByProperty(String propertyName, Object object){
		return putInMaterialPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMPutInMaterialPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = putInMaterialPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_putInMaterial_PutInMaterialPart","runtime");
		try {
	...
	public void dealDatagridsSave(WOMPutInMaterialPart putInMaterialPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,putInMaterialPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMPutInMaterialPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, putInMaterialPart);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(putInMaterialPart.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(putInMaterialPart.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(putInMaterialPart.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",putInMaterialPart.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMPutInMaterialPart.TABLE_NAME +" where valid = 1";
			list = putInMaterialPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMPutInMaterialPart.TABLE_NAME;
			list = putInMaterialPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMPutInMaterialPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = putInMaterialPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMPutInMaterialPartDaoImpl putInMaterialPartDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMPutInMaterialPart.TABLE_NAME + " where id = ?";
		String tableNo = putInMaterialPartDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchPutInMaterialPart(final List<WOMPutInMaterialPart> insertObjs, final List<WOMPutInMaterialPart> updateObjs, 
		...
		return this.importBatchPutInMaterialPart(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPutInMaterialPart(final List<WOMPutInMaterialPart> insertObjs, final List<WOMPutInMaterialPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1490604876056"),importCount));
		...
		Map<Long, WOMPutInMaterialPart> oldUpdateObjsMap = new HashMap<Long, WOMPutInMaterialPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMPutInMaterialPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMPutInMaterialPart> bizObjList = putInMaterialPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMPutInMaterialPart bizObj = bizObjList.get(mIndex);
		...
		final List<WOMPutInMaterialPart> nmeObjs = new ArrayList<WOMPutInMaterialPart>();
		...
		//String idSql = "select id from " + WOMPutInMaterialPart.TABLE_NAME + " order by id desc";
		//Query query = putInMaterialPartDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMPutInMaterialPart.JPA_NAME + " order by id desc";
		Query query =  putInMaterialPartDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMPutInMaterialPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMPutInMaterialPart putInMaterialPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMPutInMaterialPart.TABLE_NAME,1,dbId);
			if(putInMaterialPart.getId() != null ){
				m.put(putInMaterialPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) putInMaterialPartDao.createNativeQuery(sql, "WOM_7.5.0.0_putInMaterial_PutInMaterialPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		putInMaterialPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMPutInMaterialPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMPutInMaterialPart putInMaterialPart = updateObjs.get(updateIndex);
						Long id = putInMaterialPart.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = putInMaterialPart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(putInMaterialPart);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMPutInMaterialPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMPutInMaterialPart putInMaterialPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = putInMaterialPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		putInMaterialPartDaoImpl.getSession().flush();
		putInMaterialPartDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = putInMaterialPartDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_putInMaterial_PutInMaterialPart").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = putInMaterialPartDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_putInMaterial_PutInMaterialPart").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMPutInMaterialPart bizObj = insertObjs.get(insertIndex);
			putInMaterialPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMPutInMaterialPart bizObj = updateObjs.get(updateIndex);
			WOMPutInMaterialPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			putInMaterialPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMPutInMaterialPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMPutInMaterialPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(linkMap != null && linkMap.get("needChange")!=null && linkMap.get("needChange").equals("no!!")){
				...
				s.append(" FROM WOM_PUT_IN_MATERIALS \"putInMaterial\" ");
			}else{
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) putInMaterialPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMPutInMaterialPart> result = getResult(page, query, new PendingResultTransformer(WOMPutInMaterialPart.class,putInMaterialPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMPutInMaterialPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = putInMaterialPartDao.createNativeQuery(sql, "WOM_7.5.0.0_putInMaterial_PutInMaterialPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = putInMaterialPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = putInMaterialPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = putInMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?putInMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  putInMaterialPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,putInMaterialPartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="ec.flow.save")
	public void savePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, DataGridService dataGridService){
		savePutInMaterialPart(putInMaterialPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="ec.flow.save")
	public void savePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, DataGridService dataGridService, String viewCode){
		savePutInMaterialPart(putInMaterialPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart")
	public void savePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePutInMaterialPart(putInMaterialPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart")
	public void savePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_putInMaterial";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_putInMaterial");
		String url = null;
		if(null != putInMaterialPart.getId() && putInMaterialPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMPutInMaterialPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart")
	public Map<Object, Object> putInMaterialPartDataGridImport(WOMPutInMaterialPart putInMaterialPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = putInMaterialPart.getVirtualId();
		
		putInMaterialPart = putInMaterialPartDao.load(putInMaterialPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, putInMaterialPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = putInMaterialPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(putInMaterialPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, putInMaterialPart.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart")
	public void mergePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(putInMaterialPart);
		beforeSavePutInMaterialPart(putInMaterialPart);
		putInMaterialPartDao.merge(putInMaterialPart);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_putInMaterial",model="WOM_7.5.0.0_putInMaterial_PutInMaterialPart",desc="foundation.batch.save",operType="4")
	public void batchSavePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_putInMaterial";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_putInMaterial");
	...
	@Override
	public Page<WOMPutInMaterialPart> getByPage(Page<WOMPutInMaterialPart> page,DetachedCriteria detachedCriteria){
		return putInMaterialPartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSavePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, Object... objects){
		checkUniqueConstraint(putInMaterialPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
		putInMaterialPart.setPutinType(putInMaterialService.getPutInMaterial(putInMaterialPart.getHeadId().getId()).getPutinType());
		putInMaterialPart.setTaskID(putInMaterialService.getPutInMaterial(putInMaterialPart.getHeadId().getId()).getTaskID());
      	//当开始时间 结束时间都不为空时，将耗时进行填写
		if(putInMaterialPart.getPutinDate()!=null && putInMaterialPart.getPutinEndDate()!=null){
...
/* CUSTOM CODE END */
	...
	private void afterSavePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPutInMaterialPart(List<WOMPutInMaterialPart> putInMaterialParts, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPutInMaterialPart(List<WOMPutInMaterialPart> putInMaterialParts){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPutInMaterialPart(List<WOMPutInMaterialPart> putInMaterialParts, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPutInMaterialPart(List<WOMPutInMaterialPart> putInMaterialParts){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePutInMaterialPart(WOMPutInMaterialPart putInMaterialPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMPutInMaterialPart putInMaterialPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMPutInMaterialPart putInMaterialPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPutInMaterialPart(List<WOMPutInMaterialPart> insertObjs, List<WOMPutInMaterialPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPutInMaterialPart,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPutInMaterialPart(List<WOMPutInMaterialPart> insertObjs, List<WOMPutInMaterialPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPutInMaterialPart,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_putInMaterial_PutInMaterialPart,WOM_7.5.0.0) */
// 自定义代码
