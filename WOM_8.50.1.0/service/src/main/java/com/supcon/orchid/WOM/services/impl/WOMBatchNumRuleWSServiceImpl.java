...
import com.supcon.orchid.WOM.services.WOMBatchNumRuleWSService;
import com.supcon.orchid.WOM.services.WOMBatchNumRuleService;
	
/* CUSTOM CODE START(wsserviceimpl,import,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="WOMBatchNumRule",targetNamespace = "http://ws.supcon.com")
public class WOMBatchNumRuleWSServiceImpl implements WOMBatchNumRuleWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getBatchNumRuleByBusinessKey(
		...
		try{
			...
			WOMBatchNumRule batchNumRule = batchNumRuleService.loadBatchNumRuleByBussinessKey(businessKey);
			if(batchNumRule == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, batchNumRule);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateBatchNumRule(
		...
		try{
			if(batchNumRule == null){
				return set500Error(result, "批号规则不能为空。");
			}
			...
			if(!beforeUpdateBatchNumRule(batchNumRule)){
				return set404Error(result);
			}
			batchNumRuleService.saveBatchNumRule(batchNumRule, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addBatchNumRule(
		...
		try{
			...
			try{
				checkBatchNumRuleData(batchNumRule);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			batchNumRuleService.saveBatchNumRule(batchNumRule, null);
			setSuccessResult(result, batchNumRule.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteBatchNumRuleByBusinessKey(
		...
		try{
			...
			try{
				batchNumRule = batchNumRuleService.loadBatchNumRuleByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddBatchNumRule(
		...
		try{
			...
			int count = 1;
			for(WOMBatchNumRule batchNumRule : batchNumRuleList){
				if(batchNumRule.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(batchNumRule.getId() == null || batchNumRule.getId().toString().trim().isEmpty()){
			...
			batchNumRuleService.batchImportBaseBatchNumRule(batchNumRuleList);
			for(WOMBatchNumRule batchNumRule : batchNumRuleList){
				idList.add(batchNumRule.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteBatchNumRuleByPk(
		...
		try{
			...
			for(String k : pk){
				...
				WOMBatchNumRule temp = null;
				try{
					temp = batchNumRuleService.getBatchNumRule(id);
				}catch(NullPointerException e){}
			...
			batchNumRuleService.deleteBatchNumRule(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteBatchNumRuleByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				WOMBatchNumRule temp = null;
				try{
					temp = batchNumRuleService.loadBatchNumRuleByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getBatchNumRulePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(WOMBatchNumRule.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),WOMBatchNumRule.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,WOMBatchNumRule.class);
				} catch (Exception e) {
			...
			Page<WOMBatchNumRule> page = new Page<WOMBatchNumRule>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<WOMBatchNumRule> datas = batchNumRuleService.getByPage(page, detachedCriteria);
			List<WOMBatchNumRule> batchNumRuleList = datas.getResult();
			if (batchNumRuleList == null || batchNumRuleList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(batchNumRuleList.toArray());
			PaginationDTO<WOMBatchNumRule> pagination = new PaginationDTO<WOMBatchNumRule>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			WOMBatchNumRule batchNumRule = batchNumRuleService.getBatchNumRule(id);
			if (batchNumRule == null || !batchNumRule.isValid()) {
				result.setErrorMessage("找不到批号规则");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取批号规则对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param batchNumRule
	 * @return
	 */
	private WOMBatchNumRule getBatchNumRule(WOMBatchNumRule batchNumRule) {
		if (batchNumRule.getId() == null && (batchNumRule.getId() == null || batchNumRule.getId().toString().trim() == ""))
			throw new RuntimeException("批号规则对象业务主键和id不能同时为空");
		if (batchNumRule.getId() != null) {
			WOMBatchNumRule result = batchNumRuleService.getBatchNumRule(batchNumRule .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return batchNumRuleService.loadBatchNumRuleByBussinessKey(batchNumRule.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个批号规则，则返回false，否则返回true
	 * @param batchNumRule
	 */
	private boolean beforeUpdateBatchNumRule(WOMBatchNumRule batchNumRule) {
		WOMBatchNumRule temp = getBatchNumRule(batchNumRule);
		if (temp == null) {
			return false;
		}
		if(batchNumRule.getId() == null){
			batchNumRule.setId(temp.getId());		
		}
		if(batchNumRule.getId() == null){
			batchNumRule.setId(temp.getId());
		}
		
		batchNumRule.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除批号规则对象下所有辅模型对象中的批号规则对象
	 * @param batchNumRule
	 */
	private void clearCircle(WOMBatchNumRule batchNumRule) {
	 ...
	 */
	private void checkBatchNumRuleData(WOMBatchNumRule batchNumRule){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,WOM_7.5.0.0_batchNumRule_BatchNumRule,WOM_7.5.0.0) */
// 自定义代码
