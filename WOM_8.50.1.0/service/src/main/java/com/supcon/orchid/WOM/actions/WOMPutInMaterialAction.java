...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_putInMaterial_PutInMaterial,WOM_7.5.0.0) */
// 自定义代码
...
public class WOMPutInMaterialAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMPutInMaterial> {
	...
	//导出excel时,主模型内容
	List<WOMPutInMaterial> queryResults = new ArrayList<WOMPutInMaterial>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/putInMaterial/putInMaterial/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/putInMaterial/putInMaterial/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_putInMaterial_PutInMaterial" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMPutInMaterial> putInMaterials;
	
	public List<WOMPutInMaterial> getPutInMaterials() {
		return putInMaterials;
	}
	...
	private Page<WOMPutInMaterial> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(putInMaterial.getPutinType() != null) {
				putInMaterial.setPutinType((SystemCode) systemCodeService.getSystemCode(putInMaterial.getPutinType().getId()));
			}
		} else {
			if(id > 0){
				putInMaterial = putInMaterialService.getPutInMaterial(id, viewCode);
				if(tableInfoId == null && putInMaterial != null){
					tableInfoId = putInMaterial.getTableInfoId();
				}
			}
		}
	...
	public Page<WOMPutInMaterial> getPage() {
		if (null == page) {
			page = new Page<WOMPutInMaterial>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMPutInMaterial getPutInMaterial() {
		return putInMaterial;
	}		

	public void setPutInMaterial(WOMPutInMaterial putInMaterial) {
		this.putInMaterial = putInMaterial;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/putInMaterial/putInMaterial/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = putInMaterialService.getPutInMaterialAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/putInMaterial/putInMaterial/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String putInMaterialReferenceCopy() throws Exception{
			WOMPutInMaterial oldPutInMaterial = null;
			if(isEditNew()){
				...
				oldPutInMaterial = new WOMPutInMaterial();
			}else{
			//WOMPutInMaterial putInMaterial = putInMaterialService.getPutInMaterial(this.putInMaterial.getId());
			this.creatorInfo = creatorService.get(putInMaterial.getCreateStaffId(), putInMaterial.getCreatePositionId(), putInMaterial.getCreateDepartmentId());
			//setPutInMaterial(putInMaterial);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(putInMaterial.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,putInMaterial.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, putInMaterial.getId());
				...
				oldPutInMaterial = putInMaterialService.getPutInMaterial(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldPutInMaterial.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_putInMaterial_" + oldPutInMaterial.getId();
			...
			putInMaterial = putInMaterialService.getPutInMaterial(refId);
			putInMaterial.setPending(pending);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterial_PutInMaterial");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMPutInMaterial.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPutInMaterial, null);
						Method setValue = WOMPutInMaterial.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(putInMaterial, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			putInMaterial.setCreateDepartmentId(oldPutInMaterial.getCreateDepartmentId());
			putInMaterial.setCreatePositionId(oldPutInMaterial.getCreatePositionId());
			putInMaterial.setCreateStaffId(oldPutInMaterial.getCreateStaffId());
			putInMaterial.setOwnerDepartmentId(oldPutInMaterial.getOwnerDepartmentId());
			putInMaterial.setOwnerPositionId(oldPutInMaterial.getOwnerPositionId());
			putInMaterial.setOwnerStaffId(oldPutInMaterial.getOwnerStaffId());
			putInMaterial.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "referenceCopy");
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "putinEdit.ftl") }),
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "putinEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=putInMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return putInMaterialReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			putInMaterial = putInMaterialService.getPutInMaterial(this.putInMaterial.getId(), viewCode);
			this.creatorInfo = creatorService.get(putInMaterial.getCreateStaffId(), putInMaterial.getCreatePositionId(), putInMaterial.getCreateDepartmentId());
			//setPutInMaterial(putInMaterial);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = putInMaterialService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = putInMaterialService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + putInMaterial.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_putInMaterial_" + putInMaterial.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<WOMPutInMaterial> tmpList = new ArrayList<>();
			tmpList.add(putInMaterial);
			putInMaterialService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = putInMaterialService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,putInMaterial.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "putinView.ftl") }),
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "putinViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinList", results = { @Result(type = FREEMARKER, location = "putinList.ftl") })
	})
	public String viewList() throws Exception{
		...
		//mainViewPath = "/WOM/putInMaterial/putInMaterial/putinView.action";
		mainViewPath=putInMaterialService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = putInMaterialService.getWorkFlowInfo(workFlowMenuCode);
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = putInMaterialService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(putInMaterialService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(putInMaterialService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_putInMaterial_PutInMaterial", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinEXRefList", results = { @Result(type = FREEMARKER, location = "putinEXRefList.ftl") })
		,
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinRefList", results = { @Result(type = FREEMARKER, location = "putinRefList.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.taskID.taskState.id,result.taskID.taskState.value,result.taskID.productId.productCode,result.taskID.productId.productName,result.taskID.productBatchNum,result.putinStaff.name,result.remark,result.taskID.id,result.taskID.productId.id,result.putinStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="putinList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"putInMaterial\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"putInMaterial\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			putInMaterialService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<WOMPutInMaterial> putInMaterialList=page.getResult();
				for(WOMPutInMaterial lz:putInMaterialList){
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.taskID.taskState.id,result.taskID.taskState.value,result.taskID.productId.productCode,result.taskID.productId.productName,result.taskID.productBatchNum,result.putinStaff.name,result.remark,result.taskID.id,result.taskID.productId.id,result.putinStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"putInMaterial\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"putInMaterial\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinEXRefList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.taskID.productId.productCode,result.taskID.productId.productName,result.taskID.productBatchNum,result.putinStaff.name,result.remark,result.taskID.id,result.taskID.productId.id,result.putinStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="putinEXRefList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinRefList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.taskID.tableNo,result.taskID.productId.productCode,result.taskID.productId.productName,result.taskID.productBatchNum,result.putinStaff.name,result.remark,result.taskID.id,result.taskID.productId.id,result.putinStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="putinRefList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"putInMaterial\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	 ...
	 * @return
	 */
	@Action(value = "/WOM/putInMaterial/putInMaterial/allTempleteDownload", results = { @Result(name="excel",type="excel",location="putinList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterial_PutInMaterial");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		putInMaterialService.findDataGridPage(dg,dgclass,getDgPage(),putInMaterial,"", null);
	...
	private Page<WOMPutInMaterialPart> dg1490605831075Page;
	public Page<WOMPutInMaterialPart> getDg1490605831075Page(){
		if(null == dg1490605831075Page){
			dg1490605831075Page = new Page<WOMPutInMaterialPart>(1,20);
		}
		return dg1490605831075Page;
	}
	
	private Class dg1490605831075Clazz = WOMPutInMaterialPart.class;
	
	public Class getDg1490605831075Clazz() {
		return dg1490605831075Clazz;
	}
	
	private List<WOMPutInMaterialPart> dg1490605831075List;
	public List<WOMPutInMaterialPart> getDg1490605831075List(){
		if(null == dg1490605831075List){
			dg1490605831075List = new ArrayList<WOMPutInMaterialPart>();
		}
		return dg1490605831075List;
	}
	public void setDg1490605831075ListJson(String json) {
		//dg1490605831075List = (List<WOMPutInMaterialPart>) putInMaterialService.generateObjectFromJson(json, WOMPutInMaterialPart.class);
	}
	
	private List<Long> dg1490605831075DeletedIds;
	public List<Long> getDg1490605831075DeletedIds(){
		if(null == dg1490605831075DeletedIds){
			dg1490605831075DeletedIds = new ArrayList<Long>();
		}
		return dg1490605831075DeletedIds;
	}

	private Page<WOMPutInMaterialPart> dg1491534562130Page;
	public Page<WOMPutInMaterialPart> getDg1491534562130Page(){
		if(null == dg1491534562130Page){
			dg1491534562130Page = new Page<WOMPutInMaterialPart>(1,20);
		}
		return dg1491534562130Page;
	}
	
	private Class dg1491534562130Clazz = WOMPutInMaterialPart.class;
	
	public Class getDg1491534562130Clazz() {
		return dg1491534562130Clazz;
	}
	
	private List<WOMPutInMaterialPart> dg1491534562130List;
	public List<WOMPutInMaterialPart> getDg1491534562130List(){
		if(null == dg1491534562130List){
			dg1491534562130List = new ArrayList<WOMPutInMaterialPart>();
		}
		return dg1491534562130List;
	}
	public void setDg1491534562130ListJson(String json) {
		//dg1491534562130List = (List<WOMPutInMaterialPart>) putInMaterialService.generateObjectFromJson(json, WOMPutInMaterialPart.class);
	}
	
	private List<Long> dg1491534562130DeletedIds;
	public List<Long> getDg1491534562130DeletedIds(){
		if(null == dg1491534562130DeletedIds){
			dg1491534562130DeletedIds = new ArrayList<Long>();
		}
		return dg1491534562130DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/putInMaterial/putInMaterial/data-dg1490605831075", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.id,result.proBatch.batchText,result.proBatch.id,result.batchNum,result.wareID.wareCode,result.wareID.wareName,result.wareID.id,result.wareID.cargoStatue.id,result.wareID.cargoStatue.value,result.storeID.placeSetCode,result.storeID.placeSetName,result.storeID.id,result.formluActive.name,result.formluActive.id,result.taskActiveId.name,result.taskActiveId.id,result.factoryId.name,result.factoryId.id,result.taskProcessId.name,result.taskProcessId.id,result.useNum,result.lossNum,result.putinDate,result.putinEndDate,result.WOM_7_5_0_0_putInMaterial_putinEditdg1490605831075_DATAGRID_CUSTOM_61534c3b_af7d_4d34_9583_b7a1948d55f9,result.remark,result.id,result.version,result.taskID.id,result.taskActiveId.id,result.formluActive.id,result.productId.id,result.factoryId.id,result.storeID.id,result.putinStaff.id,result.proBatch.id,result.adjustActive.id,result.taskProcessId.id,result.headId.id,result.wareID.id" , "dataGridCode", "WOM_7.5.0.0_putInMaterial_putinEditdg1490605831075" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1490605831075-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1490605831075.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/putInMaterial/putInMaterial/data-dg1491534562130", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.id,result.batchNum,result.wareID.wareCode,result.wareID.wareName,result.wareID.id,result.wareID.cargoStatue.id,result.wareID.cargoStatue.value,result.storeID.placeSetCode,result.storeID.placeSetName,result.storeID.id,result.formluActive.name,result.formluActive.id,result.factoryId.name,result.factoryId.id,result.taskProcessId.name,result.taskProcessId.id,result.useNum,result.lossNum,result.putinDate,result.putinEndDate,result.takeTime,result.WOM_7_5_0_0_putInMaterial_putinViewdg1491534562130_DATAGRID_CUSTOM_38c7b8f9_8826_4cb0_ba59_74793b2409dd,result.remark,result.id,result.version,result.taskID.id,result.taskActiveId.id,result.formluActive.id,result.productId.id,result.factoryId.id,result.storeID.id,result.putinStaff.id,result.proBatch.id,result.adjustActive.id,result.taskProcessId.id,result.headId.id,result.wareID.id" , "dataGridCode", "WOM_7.5.0.0_putInMaterial_putinViewdg1491534562130" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1491534562130-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1491534562130.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(putInMaterial.getId() != null && putInMaterial.getId() != -1){
				putInMaterial = putInMaterialService.getPutInMaterial(putInMaterial.getId());
			}
			putInMaterialService.findDataGridPage(dg,dgclass,getDgPage(),putInMaterial,searchCondition,params);
	...
	private void save() throws Exception {
		...
		putInMaterialService.save(putInMaterial,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(putInMaterial.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + putInMaterial.getProcessKey() + "_" + p.getActivityName() + "_WOM_7.5.0.0_putInMaterial_" + putInMaterial.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(putInMaterial.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/WOM/putInMaterial/putInMaterial/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		putInMaterialService.retrialThisTable(putInMaterial,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "retrial");
		return SUCCESS;
	}
	@Actions({
		@Action(value = "/WOM/putInMaterial/putInMaterial/putinEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/putInMaterial/putInMaterial/putinView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "submit");
		bap_validate_datagrids=putInMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (putInMaterial != null && putInMaterial.getStatus() != null && putInMaterial.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				putInMaterialService.saveSuperEditPutInMaterial(putInMaterial, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				putInMaterialService.submit(putInMaterial,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", putInMaterial.getTableInfoId());
		responseMap.put("id", putInMaterial.getId());
		...
		fileuploads.put("linkId", putInMaterial.getTableInfoId());
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/putInMaterial/putInMaterial/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "bulkSubmit");
		putInMaterialService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/WOM/putInMaterial/putInMaterial/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		putInMaterialService.deletePutInMaterial(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterial/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "restore");
		putInMaterialService.restorePutInMaterial(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/putInMaterial/putInMaterial/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/putInMaterial/putInMaterial/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "showDealInfoData");
		if (null != tableInfoId) {
			int count=putInMaterialService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = putInMaterialService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = putInMaterialService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMPutInMaterial> findExportDataInfos(Page<WOMPutInMaterial> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = putInMaterialService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = putInMaterialService.getAssProperty(p.getCode());	
									//String assModelCode = putInMaterialService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			putInMaterialService.findPutInMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			putInMaterialService.findPutInMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMPutInMaterial> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMPutInMaterial result : results){
	...
	public boolean isEditNew(){
		//return null == this.putInMaterial || null == this.putInMaterial.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && putInMaterial != null && putInMaterial.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + putInMaterial.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return putInMaterialService.isRecallAble(tableInfoId);
	}

	@Action(value = "/WOM/putInMaterial/putInMaterial/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = putInMaterialService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + putInMaterial.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + putInMaterial.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/putInMaterial/putInMaterial/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "batchApproval");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/putInMaterial/putInMaterial/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/putInMaterial/putInMaterial/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/putInMaterial/putInMaterial/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterial/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMPutInMaterialService"
						, "com.supcon.orchid.WOM.services.WOMPutInMaterialPartService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_putInMaterial", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_putInMaterial");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterial/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_putInMaterial_PutInMaterial", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_putInMaterial_PutInMaterial", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/putInMaterial/putInMaterial/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/putInMaterial/putInMaterial/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_putInMaterial_PutInMaterial";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_putInMaterial_PutInMaterial", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/putInMaterial/putInMaterial/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == putInMaterial.getTableNo() ? "" : putInMaterial.getTableNo().toString());
		_auditLog.setColumnStringA(null == putInMaterial.getId() ? "" : putInMaterial.getId().toString());
		_auditLog.setColumnStringC(putInMaterial.getId().toString());
		_auditLog.setColumnStringD(WOMPutInMaterial.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_putInMaterial");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMPutInMaterialAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, putInMaterial,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, putInMaterial,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,putInMaterial);
		...
		printlog.setEntity("WOM_7.5.0.0_putInMaterial");
		...
		printlog.setBusinessId(putInMaterial.getId());
		printlog.setBussinessKey(null == putInMaterial.getTableNo() ? "" : putInMaterial.getTableNo().toString());
		printlog.setMainDisplay(null == putInMaterial.getId() ? "" : putInMaterial.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterial/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(putInMaterial,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/putInMaterial/putInMaterial/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(putInMaterialService.getPutInMaterial(Long.parseLong(arrId[i])));
	...
	@Action(value = "/WOM/putInMaterial/putInMaterial/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/putInMaterial/putInMaterial/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, putInMaterial,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				putInMaterial = putInMaterialService.getPutInMaterial(Long.parseLong(arrId[i]));
				if(null != putInMaterial){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == putInMaterial.getId() ? "" : "," + putInMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == putInMaterial.getId() ? "" : putInMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == putInMaterial.getTableNo() ? "" : "," + putInMaterial.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == putInMaterial.getTableNo() ? "" : putInMaterial.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == putInMaterial.getId() ? "" : "," + putInMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == putInMaterial.getId() ? "" : putInMaterial.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, putInMaterial,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,putInMaterial);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/putInMaterial/putInMaterial/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMPutInMaterial> findExportPageInfos(Page<WOMPutInMaterial> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_putInMaterial" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = putInMaterialService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMPutInMaterial queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = putInMaterialService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = putInMaterialService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterial_PutInMaterial");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_putInMaterial_PutInMaterial")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_putInMaterial_PutInMaterial");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_putInMaterial_PutInMaterial");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = putInMaterialService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_putInMaterial_PutInMaterial,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_putInMaterial_PutInMaterial,WOM_7.5.0.0) */
// 自定义代码
      	//复制时调用，处理下方路由消失的情况
      	if("WOM_7.5.0.0_putInMaterial_putinEdit".equals(code) && "putinEdit".equals(methodName)){
			if("copy".equals(isCopy)){
				this.creatorInfo = creatorService.getCurrent();
				if (null == pendingId && null != deploymentId) {
					transitions = processService.findFirstTransitions(deploymentId);
					Deployment d = processService.getDeployment(deploymentId);
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_putInMaterial_PutInMaterial,WOM_7.5.0.0) */
// 自定义代码
	@Resource
	private ConsulService consulService;
	private String proID;//物品id
	private String taskID;//指令单id
	private String factoryId;//生产线id
	...
	private String formularCode;//配方编码
	private String formularName;//配方标识
	private String factoryIdName;//生产线
	private String generalManage;//综合管理打开界面标识
	private String formularProcess;//配方属性
	
	public String getFormularProcess() {
		return formularProcess;
	}

	public void setFormularProcess(String formularProcess) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/putInMaterial/putInMaterial/productCallBack", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag,orderProcessId,orderProcessName,factoryId,factoryName,taskActiveId,taskActiveName" }) })
	public String getASCode() throws Exception{
		//根据参数获得指令单活动表
		WOMProduceTaskActive produceTaskActive = produceTaskActiveService.getTask(proID,taskID);
		if(produceTaskActive != null){
			responseMap.put("dealSuccessFlag", true);
			//工序
			if(produceTaskActive.getOrderProcessId()!=null){
				responseMap.put("orderProcessId", produceTaskActive.getOrderProcessId().getId());
				responseMap.put("orderProcessName", produceTaskProcessService.getProduceTaskProcess(produceTaskActive.getOrderProcessId().getId()).getName());
			}
			//工作单元
			if(produceTaskActive.getFactoryId()!=null){
				responseMap.put("factoryId", produceTaskActive.getFactoryId().getId());
				responseMap.put("factoryName", factoryModelService.getFactoryModel(produceTaskActive.getFactoryId().getId()).getName());
			}
			//活动名称
			responseMap.put("taskActiveId", produceTaskActive.getId());
			responseMap.put("taskActiveName", produceTaskActive.getName());
		}else{
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/putInMaterial/putInMaterial/productCallBackfactory", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag,storeId,storeName,wareId,wareName,cargoStatueID,storeCode,wareCode" }) })
	public String productCallBackfactory() throws Exception{
		//根据参数获得车间仓
		MESBasicDepartmentWare facWareMaterial = produceTaskActiveService.getFactory(proID,factoryId);
		if(facWareMaterial != null){
			responseMap.put("dealSuccessFlag", true);
			//货位
			if(facWareMaterial.getStoreId()!=null){
				responseMap.put("storeId", facWareMaterial.getStoreId().getId());
				responseMap.put("storeName", storeSetService.getStoreSet(facWareMaterial.getStoreId().getId()).getPlaceSetName());
				responseMap.put("storeCode", storeSetService.getStoreSet(facWareMaterial.getStoreId().getId()).getPlaceSetCode());
			}
			//仓库
			if(facWareMaterial.getWareId()!=null){
				responseMap.put("wareId", facWareMaterial.getWareId().getId());
				responseMap.put("wareName", wareService.getWare(facWareMaterial.getWareId().getId()).getWareName());
				responseMap.put("wareCode", wareService.getWare(facWareMaterial.getWareId().getId()).getWareCode());
				//是否启用货位
				responseMap.put("cargoStatueID",wareService.getWare(facWareMaterial.getWareId().getId()).getCargoStatue().getId());
			}
		}else{
			responseMap.put("dealSuccessFlag", false);
		}
    ...
    //是否启用MES物料管理
    @Action(value = "/WOM/putInMaterial/putInMaterial/getIsMesProBatch", results = { @Result(type = JSON, params = {
    		"root", "responseMap" }) })
	...
	//根据指令单物料清单生成表体物品数据
    @Action(value = "/WOM/putInMaterial/putInMaterial/getWidgetData", results = { 
    		@Result(type = JSON, params = { "root", "dg1490605831075Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.productId.productCode,result.productId.productName,result.productId.id,result.proBatch.batchText,result.batchNum,result.wareID.wareCode,result.wareID.wareName,result.wareID.id,result.wareID.cargoStatue.id,result.wareID.cargoStatue.value,result.storeID.placeSetCode,result.storeID.placeSetName,result.storeID.id,result.taskActiveId.name,result.taskActiveId.id,result.factoryId.name,result.factoryId.id,result.taskProcessId.name,result.taskProcessId.id,result.useNum,result.lossNum,result.putinDate,result.putinEndDate,result.WOM_7_5_0_0_putInMaterial_putinEditdg1490605831075_DATAGRID_CUSTOM_61534c3b_af7d_4d34_9583_b7a1948d55f9,result.remark,result.id,result.version,result.taskID.id,result.taskActiveId.id,result.storeID.id,result.productId.id,result.factoryId.id,result.proBatch.id,result.taskProcessId.id,result.putinStaff.id,result.headId.id,result.wareID.id,result.id.id" , "dataGridCode", "WOM_7.5.0.0_putInMaterial_putinEditdg1490605831075" }) })
    public String getWidgetData() throws Exception{
    	List<WOMPutInMaterialPart> partList= putInMaterialService.getPart(produceTaskId);
    	if(dg1490605831075Page==null) dg1490605831075Page = getDg1490605831075Page();  
    	if(partList!=null && partList.size()>0){
    		dg1490605831075Page.setResult(partList);
    	}
    	return SUCCESS;
    }
	@Action(value = "/WOM/putInMaterial/putInMaterial/checkTask", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag,checkTask" }) })
	public String checkTask() throws Exception{
		boolean checkTask = putInMaterialService.checkTask(Long.valueOf(taskID));
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("checkTask", checkTask);
		return SUCCESS;
	}
/* CUSTOM CODE END */
}