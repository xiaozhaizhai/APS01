...
import com.supcon.orchid.WOM.services.WOMProduceMaterialSumImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMProduceMaterialSumDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_produceMaterialSumService")
@Transactional
public class WOMProduceMaterialSumServiceImpl  extends BaseServiceImpl<WOMProduceMaterialSum> implements WOMProduceMaterialSumService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMProduceMaterialSum> bapBaseService;
	...
	@Autowired
	private WOMProduceMaterialSumImportService produceMaterialSumImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMProduceMaterialSum.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  produceMaterialSumDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMProduceMaterialSum.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = produceMaterialSumDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProduceMaterialSum.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProduceMaterialSum.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  produceMaterialSumDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMProduceMaterialSum.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMProduceMaterialSum.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMProduceMaterialSum> list = (List<WOMProduceMaterialSum>) produceMaterialSumDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMProduceMaterialSum getProduceMaterialSum(long id){
		return getProduceMaterialSum(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceMaterialSum getProduceMaterialSum(long id, String viewCode){
		WOMProduceMaterialSum produceMaterialSum = produceMaterialSumDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProduceMaterialSumAsJSON(long id, String include){
		WOMProduceMaterialSum produceMaterialSum = produceMaterialSumDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(produceMaterialSum, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.common.delete",operType="3")
	public void deleteProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum){
		if(null != produceMaterialSum && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceMaterialSum.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
			AuditUtil.setColumnStringA(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
			AuditUtil.setColumnStringC(produceMaterialSum.getId().toString());
		}
		beforeDeleteProduceMaterialSum(produceMaterialSum);
		produceMaterialSumDao.deletePhysical(produceMaterialSum);
		afterDeleteProduceMaterialSum(produceMaterialSum);
		produceMaterialSumDao.flush();
		List<WOMProduceMaterialSum> params = new ArrayList<WOMProduceMaterialSum>();
		params.add(produceMaterialSum);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.common.delete",operType="3")
	public void deleteProduceMaterialSum(List<Long> produceMaterialSumIds){
		deleteProduceMaterialSum(produceMaterialSumIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.common.delete",operType="3")
	public void deleteProduceMaterialSum(List<Long> produceMaterialSumIds, String eventTopic) {
		List<WOMProduceMaterialSum> produceMaterialSums = new ArrayList<WOMProduceMaterialSum>();
		for(Long produceMaterialSumId : produceMaterialSumIds){
			WOMProduceMaterialSum produceMaterialSum = getProduceMaterialSum(produceMaterialSumId);
			produceMaterialSums.add(produceMaterialSum);
			if(null != produceMaterialSum && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceMaterialSum.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == produceMaterialSum.getId() ? "" : "," + produceMaterialSum.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == produceMaterialSum.getId() ? "" : "," + produceMaterialSum.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == produceMaterialSum.getId() ? "" : "," + produceMaterialSum.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(produceMaterialSums != null && produceMaterialSums.size() > 0 ){
			for(WOMProduceMaterialSum produceMaterialSum : produceMaterialSums){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMProduceMaterialSum.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(produceMaterialSum));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),produceMaterialSum, produceMaterialSum.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(produceMaterialSumIds != null && produceMaterialSumIds.size() > 0) {
			String hql = "delete " + WOMProduceMaterialSum.JPA_NAME + " where id in(:ids)";
			Query query = produceMaterialSumDao.createQuery(hql);
			query.setParameterList("ids", produceMaterialSumIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMProduceMaterialSum produceMaterialSumz : produceMaterialSums){
				produceMaterialSumz.setValid(false);
				List<WOMProduceMaterialSum> params = new ArrayList<WOMProduceMaterialSum>();
				params.add(produceMaterialSumz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.common.delete",operType="3")
	public void deleteProduceMaterialSum(long produceMaterialSumId, int produceMaterialSumVersion){
		this.deleteProduceMaterialSum(produceMaterialSumId, produceMaterialSumVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.common.delete",operType="3")
	public void deleteProduceMaterialSum(long produceMaterialSumId, int produceMaterialSumVersion,SignatureLog signatureLog){
		WOMProduceMaterialSum produceMaterialSum = getProduceMaterialSum(produceMaterialSumId);
		...
		beforeDeleteProduceMaterialSum(produceMaterialSum);
		produceMaterialSumDao.delete(produceMaterialSumId, produceMaterialSumVersion);
		afterDeleteProduceMaterialSum(produceMaterialSum);
		produceMaterialSumDao.flush();
		List<WOMProduceMaterialSum> params = new ArrayList<WOMProduceMaterialSum>();
		params.add(produceMaterialSum);
		...
		if(signatureLog != null ) {
			...
			businessKey=produceMaterialSum.getId();
			signatureLog.setTableId(produceMaterialSum.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.common.delete",operType="3")
	public void deleteProduceMaterialSum(String produceMaterialSumIds){
		this.deleteProduceMaterialSum(produceMaterialSumIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.common.delete",operType="3")
	public void deleteProduceMaterialSum(String produceMaterialSumIds,SignatureLog signatureLog){
		deleteCollection(produceMaterialSumIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProduceMaterialSum(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_ProduceMaterialSum#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="ec.engine.restore",operType="10")
	public void restoreProduceMaterialSum(String produceMaterialSumIds){
		restoreCollection(produceMaterialSumIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProduceMaterialSum(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="ec.engine.restore",operType="10")
	public void restoreProduceMaterialSum(long produceMaterialSumId){
		
		findBusinessKeyUsed(produceMaterialSumId);	//判断业务主键是否重复
		...
		beforeRestoreProduceMaterialSum(produceMaterialSum);
		produceMaterialSum.setValid(true);
		produceMaterialSumDao.update(produceMaterialSum);
		afterRestoreProduceMaterialSum(produceMaterialSum);
		List<WOMProduceMaterialSum> params = new ArrayList<WOMProduceMaterialSum>();
		params.add(produceMaterialSum);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long produceMaterialSumId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_ProduceMaterialSum");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMProduceMaterialSum.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMProduceMaterialSum.TABLE_NAME +" where id = ? )";
			List<Object> list =  produceMaterialSumDao.createNativeQuery(sql,produceMaterialSumId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProduceMaterialSum(List<WOMProduceMaterialSum>  produceMaterialSums){
		for(WOMProduceMaterialSum produceMaterialSum:produceMaterialSums)  {
			saveProduceMaterialSum(produceMaterialSum, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProduceMaterialSum(List<WOMProduceMaterialSum>  produceMaterialSums){
		for(WOMProduceMaterialSum produceMaterialSum:produceMaterialSums)  {
			saveProduceMaterialSum(produceMaterialSum, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="ec.flow.save")
	public void saveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProduceMaterialSum(produceMaterialSum, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="ec.flow.save")
	public void saveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProduceMaterialSum(produceMaterialSum, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProduceMaterialSumCodes(WOMProduceMaterialSum produceMaterialSum) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProduceMaterialSumCodes(produceMaterialSum, false);
	}
	
	public void generateProduceMaterialSumCodes(WOMProduceMaterialSum produceMaterialSum, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProduceMaterialSumSummarys(WOMProduceMaterialSum produceMaterialSum) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProduceMaterialSumSummarys(produceMaterialSum, false);
	}

	public void generateProduceMaterialSumSummarys(WOMProduceMaterialSum produceMaterialSum, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum")
	public void saveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProduceMaterialSum(produceMaterialSum, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum")
	public void saveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(produceMaterialSum.getId() != null && produceMaterialSum.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceMaterialSum.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, produceMaterialSum);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(produceMaterialSum);
		beforeSaveProduceMaterialSum(produceMaterialSum, viewIsView);

		if (viewIsView) {
			produceMaterialSumDao.saveWithRevertVersion(produceMaterialSum);
		} else {
			if(isNew)
				produceMaterialSumDao.save(produceMaterialSum);
			else
				produceMaterialSumDao.merge(produceMaterialSum);
		}
		if(null != produceMaterialSum && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceMaterialSum.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
			AuditUtil.setColumnStringA(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
		...
		dealDatagridsSave(produceMaterialSum,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProduceMaterialSumCodes(produceMaterialSum, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProduceMaterialSumSummarys(produceMaterialSum, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProduceMaterialSum(produceMaterialSum, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, produceMaterialSum);
			}
		}
		List<WOMProduceMaterialSum> params = new ArrayList<WOMProduceMaterialSum>();
		produceMaterialSumDao.flush();
		produceMaterialSumDao.clear();
		produceMaterialSum = produceMaterialSumDao.load(produceMaterialSum.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMProduceMaterialSum produceMaterialSum){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("produceMaterialSum", produceMaterialSum);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum")
	public void mergeProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(produceMaterialSum);
		beforeSaveProduceMaterialSum(produceMaterialSum);
		produceMaterialSumDao.merge(produceMaterialSum);
		if(null != produceMaterialSum && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceMaterialSum.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
			AuditUtil.setColumnStringA(null == produceMaterialSum.getId() ? "" : produceMaterialSum.getId().toString());
		...
		dealDatagridsSave(produceMaterialSum,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProduceMaterialSum(produceMaterialSum);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMProduceMaterialSum> findProduceMaterialSums(Page<WOMProduceMaterialSum> page, Criterion... criterions) {
		return produceMaterialSumDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceMaterialSums(Page<WOMProduceMaterialSum> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProduceMaterialSums(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceMaterialSums(Page<WOMProduceMaterialSum> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceMaterialSums(Page<WOMProduceMaterialSum> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMProduceMaterialSum> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"produceMaterialSum\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"produceMaterialSum\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"produceMaterialSum\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"produceMaterialSum\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  produceMaterialSumDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  produceMaterialSumDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"produceMaterialSum\".LAY_REC ASC, \"produceMaterialSum\".SORT ASC");
				} else {
					orderPart.append(" \"produceMaterialSum\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) produceMaterialSumDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=produceMaterialSumDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = produceMaterialSumDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMProduceMaterialSum> result = new ArrayList<WOMProduceMaterialSum>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMProduceMaterialSum.class,produceMaterialSumDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMProduceMaterialSum.class,produceMaterialSumDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMProduceMaterialSum produceMaterialSum : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(produceMaterialSum.getId(), "WOM_produceTask_produceMaterialSum");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(produceMaterialSum.getId(), "WOM_produceTask_produceMaterialSum");
							if(null != documents && !documents.isEmpty()) {
								produceMaterialSum.setDocument(documents.get(0));
								produceMaterialSum.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMProduceMaterialSum> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMProduceMaterialSum> result) {
		View view = null;
		...
		Map<Long, WOMProduceMaterialSum> rsMap = new HashMap<Long, WOMProduceMaterialSum>();
		for (WOMProduceMaterialSum produceMaterialSum : result) {
			ids.add(produceMaterialSum.getId());
			rsMap.put(produceMaterialSum.getId(), produceMaterialSum);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(produceMaterialSumDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = produceMaterialSumDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_ProduceMaterialSum" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_ProduceMaterialSum" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  produceMaterialSumDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceMaterialSum\"","WOM_7.5.0.0_produceTask","","WOM_PRODUCE_MATERIAL_SUMS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"produceMaterialSum\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"produceMaterialSum\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceMaterialSum\"","WOM_7.5.0.0_produceTask","","WOM_PRODUCE_MATERIAL_SUMS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_ProduceMaterialSum" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("produceMaterialSum.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"produceMaterialSum\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"produceMaterialSum\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"produceMaterialSum\".CID = 1 OR \"produceMaterialSum\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("produceMaterialSum.mc\"").append(".PRODUCE_MATERIAL_SUM) not in ("+notinValue+")");
			}
			
			SQLQuery query = produceMaterialSumDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=produceMaterialSumDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMProduceMaterialSum.ENTITY_CODE;}
	protected String getModuleCode(){return WOMProduceMaterialSum.MODULE_CODE;}
	protected String getModelCode(){return WOMProduceMaterialSum.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param produceMaterialSum 模型实例
	 */
	public void checkUniqueConstraint(WOMProduceMaterialSum produceMaterialSum){
	}
	
	
	@Override
	public List<WOMProduceMaterialSum> getProduceMaterialSums(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceMaterialSum.class);
		if(params!=null){
			...
			return produceMaterialSumDao.findByHql("from " + WOMProduceMaterialSum.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return produceMaterialSumDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMProduceMaterialSum> findProduceMaterialSumsByHql(String hql, Object... objects){
		...
		return   produceMaterialSumDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMProduceMaterialSum loadProduceMaterialSumByBussinessKey(WOMProduceMaterialSum produceMaterialSum){
		return loadProduceMaterialSumByBussinessKey(produceMaterialSum.getId());
	}
	
	@Override
	public Page<WOMProduceMaterialSum> getProduceMaterialSums(Page<WOMProduceMaterialSum> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceMaterialSum.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = produceMaterialSumDao.findByPage(page, sql);
			}else{
				page = produceMaterialSumDao.findAllByPage(page);
			}
			return page;
		}
		page = produceMaterialSumDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProduceMaterialSum> findByTreeDataGridProduceTask(List<WOMProduceMaterialSum> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMProduceMaterialSum produceMaterialSum : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTask(Page<WOMProduceMaterialSum> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMProduceMaterialSum> produceMaterialSums = dgPage.getResult();
			for(WOMProduceMaterialSum produceMaterialSum : produceMaterialSums){
				
			}
			dgPage.setResult(produceMaterialSums);
		}
	...
	public WOMProduceMaterialSum loadProduceMaterialSumByBussinessKey(Serializable bussinessKey){
		return produceMaterialSumDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProduceMaterialSum> findByProperty(String propertyName, Object object){
		return produceMaterialSumDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceMaterialSum findEntityByProperty(String propertyName, Object object){
		return produceMaterialSumDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMProduceMaterialSum.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = produceMaterialSumDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_ProduceMaterialSum","runtime");
		try {
	...
	public void dealDatagridsSave(WOMProduceMaterialSum produceMaterialSum,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,produceMaterialSumDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMProduceMaterialSum.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, produceMaterialSum);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(produceMaterialSum.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(produceMaterialSum.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(produceMaterialSum.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",produceMaterialSum.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProduceMaterialSum.TABLE_NAME +" where valid = 1";
			list = produceMaterialSumDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProduceMaterialSum.TABLE_NAME;
			list = produceMaterialSumDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMProduceMaterialSum.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = produceMaterialSumDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMProduceMaterialSumDaoImpl produceMaterialSumDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMProduceMaterialSum.TABLE_NAME + " where id = ?";
		String tableNo = produceMaterialSumDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchProduceMaterialSum(final List<WOMProduceMaterialSum> insertObjs, final List<WOMProduceMaterialSum> updateObjs, 
		...
		return this.importBatchProduceMaterialSum(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProduceMaterialSum(final List<WOMProduceMaterialSum> insertObjs, final List<WOMProduceMaterialSum> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1489671141506"),importCount));
		...
		Map<Long, WOMProduceMaterialSum> oldUpdateObjsMap = new HashMap<Long, WOMProduceMaterialSum>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMProduceMaterialSum bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMProduceMaterialSum> bizObjList = produceMaterialSumDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMProduceMaterialSum bizObj = bizObjList.get(mIndex);
		...
		final List<WOMProduceMaterialSum> nmeObjs = new ArrayList<WOMProduceMaterialSum>();
		...
		//String idSql = "select id from " + WOMProduceMaterialSum.TABLE_NAME + " order by id desc";
		//Query query = produceMaterialSumDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMProduceMaterialSum.JPA_NAME + " order by id desc";
		Query query =  produceMaterialSumDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMProduceMaterialSum.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMProduceMaterialSum produceMaterialSum:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMProduceMaterialSum.TABLE_NAME,1,dbId);
			if(produceMaterialSum.getId() != null ){
				m.put(produceMaterialSum.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) produceMaterialSumDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProduceMaterialSum"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		produceMaterialSumDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMProduceMaterialSum.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMProduceMaterialSum produceMaterialSum = updateObjs.get(updateIndex);
						Long id = produceMaterialSum.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = produceMaterialSum.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(produceMaterialSum);
							...
							nmeObjs.add(produceMaterialSum);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMProduceMaterialSum.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMProduceMaterialSum produceMaterialSum = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = produceMaterialSum.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		produceMaterialSumDaoImpl.getSession().flush();
		produceMaterialSumDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = produceMaterialSumDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_ProduceMaterialSum").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = produceMaterialSumDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_ProduceMaterialSum").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMProduceMaterialSum bizObj = insertObjs.get(insertIndex);
			produceMaterialSumImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMProduceMaterialSum bizObj = updateObjs.get(updateIndex);
			WOMProduceMaterialSum oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			produceMaterialSumImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMProduceMaterialSum> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMProduceMaterialSum> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) produceMaterialSumDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMProduceMaterialSum> result = getResult(page, query, new PendingResultTransformer(WOMProduceMaterialSum.class,produceMaterialSumDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMProduceMaterialSum model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = produceMaterialSumDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProduceMaterialSum").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = produceMaterialSumDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = produceMaterialSumDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = produceMaterialSumDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?produceMaterialSumDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  produceMaterialSumDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,produceMaterialSumDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="ec.flow.save")
	public void saveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, DataGridService dataGridService){
		saveProduceMaterialSum(produceMaterialSum, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="ec.flow.save")
	public void saveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, DataGridService dataGridService, String viewCode){
		saveProduceMaterialSum(produceMaterialSum, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum")
	public void saveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProduceMaterialSum(produceMaterialSum, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum")
	public void saveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != produceMaterialSum.getId() && produceMaterialSum.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceMaterialSum.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum")
	public Map<Object, Object> produceMaterialSumDataGridImport(WOMProduceMaterialSum produceMaterialSum, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = produceMaterialSum.getVirtualId();
		
		produceMaterialSum = produceMaterialSumDao.load(produceMaterialSum.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, produceMaterialSum.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = produceMaterialSum.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(produceMaterialSum);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, produceMaterialSum.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum")
	public void mergeProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(produceMaterialSum);
		beforeSaveProduceMaterialSum(produceMaterialSum);
		produceMaterialSumDao.merge(produceMaterialSum);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceMaterialSum",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMProduceMaterialSum> getByPage(Page<WOMProduceMaterialSum> page,DetachedCriteria detachedCriteria){
		return produceMaterialSumDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, Object... objects){
		checkUniqueConstraint(produceMaterialSum);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProduceMaterialSum(WOMProduceMaterialSum produceMaterialSum){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMProduceMaterialSum produceMaterialSum, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMProduceMaterialSum produceMaterialSum, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProduceMaterialSum(List<WOMProduceMaterialSum> insertObjs, List<WOMProduceMaterialSum> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProduceMaterialSum,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProduceMaterialSum(List<WOMProduceMaterialSum> insertObjs, List<WOMProduceMaterialSum> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProduceMaterialSum,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_ProduceMaterialSum,WOM_7.5.0.0) */
// 自定义代码
	/**
	 * 根据指令单id删除用料汇总
	 */
	...
	public void deleteByheadId(long headId) {
		WOMProduceTask produceTask=	produceTaskDao.load(headId);
		List<WOMProduceMaterialSum> produceMaterialSums = getProduceMaterialSums("headId="+produceTask.getId(), null);
