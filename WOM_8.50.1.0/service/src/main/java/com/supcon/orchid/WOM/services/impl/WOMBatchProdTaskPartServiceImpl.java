...
import com.supcon.orchid.WOM.services.WOMBatchProdTaskPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchProdTaskPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
...
@Service("wOM_batchProdTaskPartService")
@Transactional
public class WOMBatchProdTaskPartServiceImpl  extends BaseServiceImpl<WOMBatchProdTaskPart> implements WOMBatchProdTaskPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMBatchProdTaskPart> bapBaseService;
	...
	@Autowired
	private WOMBatchProdTaskPartImportService batchProdTaskPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchProdTaskPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchProdTaskPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchProdTaskPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchProdTaskPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchProdTaskPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchProdTaskPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchProdTaskPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchProdTaskPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchProdTaskPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchProdTaskPart> list = (List<WOMBatchProdTaskPart>) batchProdTaskPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMBatchProdTaskPart getBatchProdTaskPart(long id){
		return getBatchProdTaskPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchProdTaskPart getBatchProdTaskPart(long id, String viewCode){
		WOMBatchProdTaskPart batchProdTaskPart = batchProdTaskPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchProdTaskPartAsJSON(long id, String include){
		WOMBatchProdTaskPart batchProdTaskPart = batchProdTaskPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchProdTaskPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart){
		if(null != batchProdTaskPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProdTaskPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
			AuditUtil.setColumnStringA(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
			AuditUtil.setColumnStringC(batchProdTaskPart.getId().toString());
		}
		beforeDeleteBatchProdTaskPart(batchProdTaskPart);
		batchProdTaskPartDao.deletePhysical(batchProdTaskPart);
		afterDeleteBatchProdTaskPart(batchProdTaskPart);
		batchProdTaskPartDao.flush();
		List<WOMBatchProdTaskPart> params = new ArrayList<WOMBatchProdTaskPart>();
		params.add(batchProdTaskPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchProdTaskPart(List<Long> batchProdTaskPartIds){
		deleteBatchProdTaskPart(batchProdTaskPartIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchProdTaskPart(List<Long> batchProdTaskPartIds, String eventTopic) {
		List<WOMBatchProdTaskPart> batchProdTaskParts = new ArrayList<WOMBatchProdTaskPart>();
		for(Long batchProdTaskPartId : batchProdTaskPartIds){
			WOMBatchProdTaskPart batchProdTaskPart = getBatchProdTaskPart(batchProdTaskPartId);
			batchProdTaskParts.add(batchProdTaskPart);
			if(null != batchProdTaskPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProdTaskPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchProdTaskPart.getId() ? "" : "," + batchProdTaskPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchProdTaskPart.getId() ? "" : "," + batchProdTaskPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchProdTaskPart.getId() ? "" : "," + batchProdTaskPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchProdTaskParts != null && batchProdTaskParts.size() > 0 ){
			for(WOMBatchProdTaskPart batchProdTaskPart : batchProdTaskParts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMBatchProdTaskPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchProdTaskPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchProdTaskPart, batchProdTaskPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchProdTaskPartIds != null && batchProdTaskPartIds.size() > 0) {
			String hql = "delete " + WOMBatchProdTaskPart.JPA_NAME + " where id in(:ids)";
			Query query = batchProdTaskPartDao.createQuery(hql);
			query.setParameterList("ids", batchProdTaskPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchProdTaskPart batchProdTaskPartz : batchProdTaskParts){
				batchProdTaskPartz.setValid(false);
				List<WOMBatchProdTaskPart> params = new ArrayList<WOMBatchProdTaskPart>();
				params.add(batchProdTaskPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_batchProduceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchProdTaskPart(long batchProdTaskPartId, int batchProdTaskPartVersion){
		this.deleteBatchProdTaskPart(batchProdTaskPartId, batchProdTaskPartVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchProdTaskPart(long batchProdTaskPartId, int batchProdTaskPartVersion,SignatureLog signatureLog){
		WOMBatchProdTaskPart batchProdTaskPart = getBatchProdTaskPart(batchProdTaskPartId);
		...
		beforeDeleteBatchProdTaskPart(batchProdTaskPart);
		batchProdTaskPartDao.delete(batchProdTaskPartId, batchProdTaskPartVersion);
		afterDeleteBatchProdTaskPart(batchProdTaskPart);
		batchProdTaskPartDao.flush();
		List<WOMBatchProdTaskPart> params = new ArrayList<WOMBatchProdTaskPart>();
		params.add(batchProdTaskPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchProdTaskPart.getId();
			signatureLog.setTableId(batchProdTaskPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchProdTaskPart(String batchProdTaskPartIds){
		this.deleteBatchProdTaskPart(batchProdTaskPartIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.common.delete",operType="3")
	public void deleteBatchProdTaskPart(String batchProdTaskPartIds,SignatureLog signatureLog){
		deleteCollection(batchProdTaskPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchProdTaskPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_batchProduceTask#modelCode:WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="ec.engine.restore",operType="10")
	public void restoreBatchProdTaskPart(String batchProdTaskPartIds){
		restoreCollection(batchProdTaskPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchProdTaskPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="ec.engine.restore",operType="10")
	public void restoreBatchProdTaskPart(long batchProdTaskPartId){
		
		findBusinessKeyUsed(batchProdTaskPartId);	//判断业务主键是否重复
		...
		beforeRestoreBatchProdTaskPart(batchProdTaskPart);
		batchProdTaskPart.setValid(true);
		batchProdTaskPartDao.update(batchProdTaskPart);
		afterRestoreBatchProdTaskPart(batchProdTaskPart);
		List<WOMBatchProdTaskPart> params = new ArrayList<WOMBatchProdTaskPart>();
		params.add(batchProdTaskPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchProdTaskPartId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchProdTaskPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchProdTaskPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchProdTaskPartDao.createNativeQuery(sql,batchProdTaskPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchProdTaskPart(List<WOMBatchProdTaskPart>  batchProdTaskParts){
		for(WOMBatchProdTaskPart batchProdTaskPart:batchProdTaskParts)  {
			saveBatchProdTaskPart(batchProdTaskPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchProdTaskPart(List<WOMBatchProdTaskPart>  batchProdTaskParts){
		for(WOMBatchProdTaskPart batchProdTaskPart:batchProdTaskParts)  {
			saveBatchProdTaskPart(batchProdTaskPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="ec.flow.save")
	public void saveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchProdTaskPart(batchProdTaskPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="ec.flow.save")
	public void saveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchProdTaskPart(batchProdTaskPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchProdTaskPartCodes(WOMBatchProdTaskPart batchProdTaskPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchProdTaskPartCodes(batchProdTaskPart, false);
	}
	
	public void generateBatchProdTaskPartCodes(WOMBatchProdTaskPart batchProdTaskPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
			batchProdTaskPartDao.flush();
			batchProdTaskPartDao.clear();
			WOMBatchProdTaskPart modelinstance = getBatchProdTaskPart(batchProdTaskPart.getId());
			...
			boolean isLegal;
		// productBatchNum
		if (batchProdTaskPart.getProductBatchNum() == null || batchProdTaskPart.getProductBatchNum().isEmpty()) {
			String productBatchNum_property_code = "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart_productBatchNum";
			...
			if(null != property){
				...
				if(isLegal){
					...
					batchProdTaskPart.setProductBatchNum(productBatchNumnewCodeValue);
				}
				
				if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProdTaskPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if (viewIsView) {
			batchProdTaskPartDao.saveWithRevertVersion(modelinstance);
		} else {
			batchProdTaskPartDao.save(modelinstance);
		}
	}
	
	public void generateBatchProdTaskPartSummarys(WOMBatchProdTaskPart batchProdTaskPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchProdTaskPartSummarys(batchProdTaskPart, false);
	}

	public void generateBatchProdTaskPartSummarys(WOMBatchProdTaskPart batchProdTaskPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart")
	public void saveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchProdTaskPart(batchProdTaskPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart")
	public void saveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_batchProduceTask");
		String url = null;
		if(batchProdTaskPart.getId() != null && batchProdTaskPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProdTaskPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchProdTaskPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchProdTaskPart);
		beforeSaveBatchProdTaskPart(batchProdTaskPart, viewIsView);

		if (viewIsView) {
			batchProdTaskPartDao.saveWithRevertVersion(batchProdTaskPart);
		} else {
			if(isNew)
				batchProdTaskPartDao.save(batchProdTaskPart);
			else
				batchProdTaskPartDao.merge(batchProdTaskPart);
		}
		if(null != batchProdTaskPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProdTaskPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
			AuditUtil.setColumnStringA(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
		...
		dealDatagridsSave(batchProdTaskPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchProdTaskPartCodes(batchProdTaskPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchProdTaskPartSummarys(batchProdTaskPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchProdTaskPart(batchProdTaskPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchProdTaskPart);
			}
		}
		List<WOMBatchProdTaskPart> params = new ArrayList<WOMBatchProdTaskPart>();
		batchProdTaskPartDao.flush();
		batchProdTaskPartDao.clear();
		batchProdTaskPart = batchProdTaskPartDao.load(batchProdTaskPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchProdTaskPart batchProdTaskPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchProdTaskPart", batchProdTaskPart);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart")
	public void mergeBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchProdTaskPart);
		beforeSaveBatchProdTaskPart(batchProdTaskPart);
		batchProdTaskPartDao.merge(batchProdTaskPart);
		if(null != batchProdTaskPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProdTaskPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
			AuditUtil.setColumnStringA(null == batchProdTaskPart.getId() ? "" : batchProdTaskPart.getId().toString());
		...
		dealDatagridsSave(batchProdTaskPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchProdTaskPart(batchProdTaskPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchProdTaskPart> findBatchProdTaskParts(Page<WOMBatchProdTaskPart> page, Criterion... criterions) {
		return batchProdTaskPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchProdTaskParts(Page<WOMBatchProdTaskPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchProdTaskParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchProdTaskParts(Page<WOMBatchProdTaskPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchProdTaskParts(Page<WOMBatchProdTaskPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchProdTaskPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchProdTaskPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchProdTaskPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchProdTaskPart\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"batchProdTaskPart\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_BATCH_PRODUCE_TASKS \"batchProduceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_batchProduceTask" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  batchProdTaskPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchProdTaskPartDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_batchProduceTask").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = dataPermissionService.getPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchProduceTask\"","WOM_7.5.0.0_batchProduceTask","","WOM_BATCH_PRODUCE_TASKS",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchProdTaskPart\".LAY_REC ASC, \"batchProdTaskPart\".SORT ASC");
				} else {
					orderPart.append(" \"batchProdTaskPart\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchProdTaskPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchProdTaskPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchProdTaskPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchProdTaskPart> result = new ArrayList<WOMBatchProdTaskPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchProdTaskPart.class,batchProdTaskPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchProdTaskPart.class,batchProdTaskPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchProdTaskPart batchProdTaskPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchProdTaskPart.getId(), "WOM_batchProduceTask_batchProdTaskPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchProdTaskPart.getId(), "WOM_batchProduceTask_batchProdTaskPart");
							if(null != documents && !documents.isEmpty()) {
								batchProdTaskPart.setDocument(documents.get(0));
								batchProdTaskPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchProdTaskPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchProdTaskPart> result) {
		View view = null;
		...
		Map<Long, WOMBatchProdTaskPart> rsMap = new HashMap<Long, WOMBatchProdTaskPart>();
		for (WOMBatchProdTaskPart batchProdTaskPart : result) {
			ids.add(batchProdTaskPart.getId());
			rsMap.put(batchProdTaskPart.getId(), batchProdTaskPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchProdTaskPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchProdTaskPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchProdTaskPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchProdTaskPart\"","WOM_7.5.0.0_batchProduceTask","","WOM_BATCH_PROD_TASK_PARTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchProdTaskPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchProdTaskPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchProdTaskPart\"","WOM_7.5.0.0_batchProduceTask","","WOM_BATCH_PROD_TASK_PARTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchProdTaskPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchProdTaskPart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchProdTaskPart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchProdTaskPart\".CID = 1 OR \"batchProdTaskPart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchProdTaskPart.mc\"").append(".BATCH_PROD_TASK_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchProdTaskPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		...
		String formatStr = "";
			//新的编码字段不需要初始化pattern
		this.productBatchNumCounter = this.counterManager.addCounter("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart_productBatchNum", "WOM_7.5.0.0_batchProduceTask", formatStr, CounterType.All);
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchProdTaskPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMBatchProdTaskPart.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchProdTaskPart.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchProdTaskPart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchProdTaskPart 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchProdTaskPart batchProdTaskPart){
	}
	
	
	@Override
	public List<WOMBatchProdTaskPart> getBatchProdTaskParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchProdTaskPart.class);
		if(params!=null){
			...
			return batchProdTaskPartDao.findByHql("from " + WOMBatchProdTaskPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchProdTaskPartDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMBatchProdTaskPart> findBatchProdTaskPartsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchProdTaskPart.class);
	...
	@Override
	public List<WOMBatchProdTaskPart> findBatchProdTaskPartsByHql(String hql, Object... objects){
		
		return   batchProdTaskPartDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchProdTaskPart loadBatchProdTaskPartByBussinessKey(WOMBatchProdTaskPart batchProdTaskPart){
		return loadBatchProdTaskPartByBussinessKey(batchProdTaskPart.getId());
	}
	
	@Override
	public Page<WOMBatchProdTaskPart> getBatchProdTaskParts(Page<WOMBatchProdTaskPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchProdTaskPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchProdTaskPartDao.findByPage(page, sql);
			}else{
				page = batchProdTaskPartDao.findAllByPage(page);
			}
			return page;
		}
		page = batchProdTaskPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchProdTaskPart> findByTreeDataGridBatchProduceTask(List<WOMBatchProdTaskPart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMBatchProdTaskPart batchProdTaskPart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridBatchProduceTask(Page<WOMBatchProdTaskPart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMBatchProdTaskPart> batchProdTaskParts = dgPage.getResult();
			for(WOMBatchProdTaskPart batchProdTaskPart : batchProdTaskParts){
				
			}
			dgPage.setResult(batchProdTaskParts);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchProdTaskPart loadBatchProdTaskPartByBussinessKey(Serializable bussinessKey){
		return batchProdTaskPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchProdTaskPart> findByProperty(String propertyName, Object object){
		return batchProdTaskPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchProdTaskPart findEntityByProperty(String propertyName, Object object){
		return batchProdTaskPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchProdTaskPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchProdTaskPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart","runtime");
		try {
	...
	public void dealDatagridsSave(WOMBatchProdTaskPart batchProdTaskPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchProdTaskPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchProdTaskPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchProdTaskPart);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(batchProdTaskPart.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(batchProdTaskPart.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(batchProdTaskPart.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchProdTaskPart.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchProdTaskPart.TABLE_NAME +" where valid = 1";
			list = batchProdTaskPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchProdTaskPart.TABLE_NAME;
			list = batchProdTaskPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchProdTaskPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchProdTaskPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchProdTaskPartDaoImpl batchProdTaskPartDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMBatchProdTaskPart.TABLE_NAME + " where id = ?";
		String tableNo = batchProdTaskPartDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchBatchProdTaskPart(final List<WOMBatchProdTaskPart> insertObjs, final List<WOMBatchProdTaskPart> updateObjs, 
		...
		return this.importBatchBatchProdTaskPart(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchProdTaskPart(final List<WOMBatchProdTaskPart> insertObjs, final List<WOMBatchProdTaskPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1495089930032"),importCount));
		...
		Map<Long, WOMBatchProdTaskPart> oldUpdateObjsMap = new HashMap<Long, WOMBatchProdTaskPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchProdTaskPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchProdTaskPart> bizObjList = batchProdTaskPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchProdTaskPart bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchProdTaskPart> nmeObjs = new ArrayList<WOMBatchProdTaskPart>();
		...
		//String idSql = "select id from " + WOMBatchProdTaskPart.TABLE_NAME + " order by id desc";
		//Query query = batchProdTaskPartDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMBatchProdTaskPart.JPA_NAME + " order by id desc";
		Query query =  batchProdTaskPartDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMBatchProdTaskPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchProdTaskPart batchProdTaskPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchProdTaskPart.TABLE_NAME,1,dbId);
			if(batchProdTaskPart.getId() != null ){
				m.put(batchProdTaskPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchProdTaskPartDao.createNativeQuery(sql, "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchProdTaskPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchProdTaskPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMBatchProdTaskPart batchProdTaskPart = updateObjs.get(updateIndex);
						Long id = batchProdTaskPart.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchProdTaskPart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchProdTaskPart);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchProdTaskPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMBatchProdTaskPart batchProdTaskPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchProdTaskPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchProdTaskPartDaoImpl.getSession().flush();
		batchProdTaskPartDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = batchProdTaskPartDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = batchProdTaskPartDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchProdTaskPart bizObj = insertObjs.get(insertIndex);
			batchProdTaskPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchProdTaskPart bizObj = updateObjs.get(updateIndex);
			WOMBatchProdTaskPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchProdTaskPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMBatchProdTaskPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMBatchProdTaskPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(linkMap != null && linkMap.get("needChange")!=null && linkMap.get("needChange").equals("no!!")){
				...
				s.append(" FROM WOM_BATCH_PRODUCE_TASKS \"batchProduceTask\" ");
			}else{
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchProdTaskPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMBatchProdTaskPart> result = getResult(page, query, new PendingResultTransformer(WOMBatchProdTaskPart.class,batchProdTaskPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMBatchProdTaskPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchProdTaskPartDao.createNativeQuery(sql, "WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchProdTaskPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchProdTaskPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchProdTaskPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchProdTaskPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchProdTaskPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchProdTaskPartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="ec.flow.save")
	public void saveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, DataGridService dataGridService){
		saveBatchProdTaskPart(batchProdTaskPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="ec.flow.save")
	public void saveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, DataGridService dataGridService, String viewCode){
		saveBatchProdTaskPart(batchProdTaskPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart")
	public void saveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchProdTaskPart(batchProdTaskPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart")
	public void saveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_batchProduceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchProduceTask");
		String url = null;
		if(null != batchProdTaskPart.getId() && batchProdTaskPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchProdTaskPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart")
	public Map<Object, Object> batchProdTaskPartDataGridImport(WOMBatchProdTaskPart batchProdTaskPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchProdTaskPart.getVirtualId();
		
		batchProdTaskPart = batchProdTaskPartDao.load(batchProdTaskPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchProdTaskPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchProdTaskPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchProdTaskPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchProdTaskPart.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart")
	public void mergeBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchProdTaskPart);
		beforeSaveBatchProdTaskPart(batchProdTaskPart);
		batchProdTaskPartDao.merge(batchProdTaskPart);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_batchProduceTask",model="WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_batchProduceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_batchProduceTask");
	...
	@Override
	public Page<WOMBatchProdTaskPart> getByPage(Page<WOMBatchProdTaskPart> page,DetachedCriteria detachedCriteria){
		return batchProdTaskPartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, Object... objects){
		checkUniqueConstraint(batchProdTaskPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
		//生成批號
		if("".equals(batchProdTaskPart.getProductBatchNum()) || batchProdTaskPart.getProductBatchNum()==null){
			batchNumCodes(batchProdTaskPart);
		}
/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchProdTaskPart(List<WOMBatchProdTaskPart> batchProdTaskParts, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchProdTaskPart(List<WOMBatchProdTaskPart> batchProdTaskParts){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchProdTaskPart(List<WOMBatchProdTaskPart> batchProdTaskParts, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchProdTaskPart(List<WOMBatchProdTaskPart> batchProdTaskParts){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchProdTaskPart(WOMBatchProdTaskPart batchProdTaskPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchProdTaskPart batchProdTaskPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchProdTaskPart batchProdTaskPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchProdTaskPart(List<WOMBatchProdTaskPart> insertObjs, List<WOMBatchProdTaskPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchProdTaskPart,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchProdTaskPart(List<WOMBatchProdTaskPart> insertObjs, List<WOMBatchProdTaskPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchProdTaskPart,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_batchProduceTask_BatchProdTaskPart,WOM_7.5.0.0) */
// 自定义代码
	@Resource
	...
	//生产批号自动生成
	public void batchNumCodes(WOMBatchProdTaskPart batchProdTaskPart){
      
		WOMProduceTask produceTask = new WOMProduceTask();
		if(batchProdTaskPart.getProductId()!=null)
			produceTask.setProductId(productService.getProduct(batchProdTaskPart.getProductId().getId()));
		if(batchProdTaskPart.getFactoryId()!=null)
			produceTask.setFactoryId(factoryModelService.getFactoryModel(batchProdTaskPart.getFactoryId().getId()));
		if(batchProdTaskPart.getFormularId()!=null)
			produceTask.setFormularId(formulaService.getFormula(batchProdTaskPart.getFormularId().getId()));
		if(batchProdTaskPart.getBulkType()!=null)
			produceTask.setPackageType(batchProdTaskPart.getBulkType());
		produceTask.setActStartTime(batchProdTaskPart.getPlanStartTime());
		produceTask.setActEndTime(batchProdTaskPart.getPlanEndTime());
		produceTask.setTaskType(batchProduceTaskService.getBatchProduceTask(batchProdTaskPart.getHeadId().getId()).getTaskType());
		produceTaskService.batchNumCodes(produceTask);
		batchProdTaskPart.setProductBatchNum(produceTask.getProductBatchNum());
