...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
public class WOMProduceTaskManageAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMProduceTaskManage> {
	...
	private WOMProduceTaskManage produceTaskManage;
	...
	//导出excel时,主模型内容
	List<WOMProduceTaskManage> queryResults = new ArrayList<WOMProduceTaskManage>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/produceTaskManage/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/produceTaskManage/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_generalManage_ProduceTaskManage" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMProduceTaskManage> produceTaskManages;
	
	public List<WOMProduceTaskManage> getProduceTaskManages() {
		return produceTaskManages;
	}
	...
	private Page<WOMProduceTaskManage> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			produceTaskManage = new WOMProduceTaskManage();
		} else {
			if(id > 0){
				produceTaskManage = produceTaskManageService.getProduceTaskManage(id, viewCode);
			}
		}
	...
	public Page<WOMProduceTaskManage> getPage() {
		if (null == page) {
			page = new Page<WOMProduceTaskManage>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMProduceTaskManage getProduceTaskManage() {
		return produceTaskManage;
	}		

	public void setProduceTaskManage(WOMProduceTaskManage produceTaskManage) {
		this.produceTaskManage = produceTaskManage;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/produceTaskManage/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = produceTaskManageService.getProduceTaskManageAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/produceTaskManage/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String produceTaskManageReferenceCopy() throws Exception{
			WOMProduceTaskManage oldProduceTaskManage = null;
			if(isEditNew()){
				...
				oldProduceTaskManage = new WOMProduceTaskManage();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,produceTaskManage.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, produceTaskManage.getId());
				...
				oldProduceTaskManage = produceTaskManageService.getProduceTaskManage(id);
			}
			
			
			produceTaskManage = produceTaskManageService.getProduceTaskManage(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_ProduceTaskManage");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMProduceTaskManage.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProduceTaskManage, null);
						Method setValue = WOMProduceTaskManage.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(produceTaskManage, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			produceTaskManage.setCreateDepartmentId(oldProduceTaskManage.getCreateDepartmentId());
			produceTaskManage.setCreatePositionId(oldProduceTaskManage.getCreatePositionId());
			produceTaskManage.setCreateStaffId(oldProduceTaskManage.getCreateStaffId());
			produceTaskManage.setOwnerDepartmentId(oldProduceTaskManage.getOwnerDepartmentId());
			produceTaskManage.setOwnerPositionId(oldProduceTaskManage.getOwnerPositionId());
			produceTaskManage.setOwnerStaffId(oldProduceTaskManage.getOwnerStaffId());
			produceTaskManage.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/generalManage/produceTaskManage/generalDetail",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "generalDetail.ftl") }),
		@Action(value = "/WOM/generalManage/produceTaskManage/generalDetailPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "generalDetailPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=produceTaskManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return produceTaskManageReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			produceTaskManage = produceTaskManageService.getProduceTaskManage(this.produceTaskManage.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,produceTaskManage.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			produceTaskManageService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = produceTaskManageService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(produceTaskManageService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(produceTaskManageService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_generalManage_ProduceTaskManage", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"produceTaskManage\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"produceTaskManage\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"produceTaskManage\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"produceTaskManage\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			produceTaskManageService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		produceTaskManageService.findDataGridPage(dg,dgclass,getDgPage(),produceTaskManage,"", null);
	...
	private Page<WOMPutInManage> dg1502700158273Page;
	public Page<WOMPutInManage> getDg1502700158273Page(){
		if(null == dg1502700158273Page){
			dg1502700158273Page = new Page<WOMPutInManage>(1,20);
		}
		return dg1502700158273Page;
	}
	
	private Class dg1502700158273Clazz = WOMPutInManage.class;
	
	public Class getDg1502700158273Clazz() {
		return dg1502700158273Clazz;
	}
	
	private List<WOMPutInManage> dg1502700158273List;
	public List<WOMPutInManage> getDg1502700158273List(){
		if(null == dg1502700158273List){
			dg1502700158273List = new ArrayList<WOMPutInManage>();
		}
		return dg1502700158273List;
	}
	public void setDg1502700158273ListJson(String json) {
		//dg1502700158273List = (List<WOMPutInManage>) produceTaskManageService.generateObjectFromJson(json, WOMPutInManage.class);
	}
	
	private List<Long> dg1502700158273DeletedIds;
	public List<Long> getDg1502700158273DeletedIds(){
		if(null == dg1502700158273DeletedIds){
			dg1502700158273DeletedIds = new ArrayList<Long>();
		}
		return dg1502700158273DeletedIds;
	}

	private Page<WOMCheckManage> dg1502700453811Page;
	public Page<WOMCheckManage> getDg1502700453811Page(){
		if(null == dg1502700453811Page){
			dg1502700453811Page = new Page<WOMCheckManage>(1,20);
		}
		return dg1502700453811Page;
	}
	
	private Class dg1502700453811Clazz = WOMCheckManage.class;
	
	public Class getDg1502700453811Clazz() {
		return dg1502700453811Clazz;
	}
	
	private List<WOMCheckManage> dg1502700453811List;
	public List<WOMCheckManage> getDg1502700453811List(){
		if(null == dg1502700453811List){
			dg1502700453811List = new ArrayList<WOMCheckManage>();
		}
		return dg1502700453811List;
	}
	public void setDg1502700453811ListJson(String json) {
		//dg1502700453811List = (List<WOMCheckManage>) produceTaskManageService.generateObjectFromJson(json, WOMCheckManage.class);
	}
	
	private List<Long> dg1502700453811DeletedIds;
	public List<Long> getDg1502700453811DeletedIds(){
		if(null == dg1502700453811DeletedIds){
			dg1502700453811DeletedIds = new ArrayList<Long>();
		}
		return dg1502700453811DeletedIds;
	}

	private Page<WOMTaskReportManage> dg1502700759865Page;
	public Page<WOMTaskReportManage> getDg1502700759865Page(){
		if(null == dg1502700759865Page){
			dg1502700759865Page = new Page<WOMTaskReportManage>(1,20);
		}
		return dg1502700759865Page;
	}
	
	private Class dg1502700759865Clazz = WOMTaskReportManage.class;
	
	public Class getDg1502700759865Clazz() {
		return dg1502700759865Clazz;
	}
	
	private List<WOMTaskReportManage> dg1502700759865List;
	public List<WOMTaskReportManage> getDg1502700759865List(){
		if(null == dg1502700759865List){
			dg1502700759865List = new ArrayList<WOMTaskReportManage>();
		}
		return dg1502700759865List;
	}
	public void setDg1502700759865ListJson(String json) {
		//dg1502700759865List = (List<WOMTaskReportManage>) produceTaskManageService.generateObjectFromJson(json, WOMTaskReportManage.class);
	}
	
	private List<Long> dg1502700759865DeletedIds;
	public List<Long> getDg1502700759865DeletedIds(){
		if(null == dg1502700759865DeletedIds){
			dg1502700759865DeletedIds = new ArrayList<Long>();
		}
		return dg1502700759865DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/generalManage/produceTaskManage/data-dg1502700158273", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.putInMetarialPart.headId.tableNo,result.putInUrl,result.putInMetarialPart.productId.productCode,result.putInMetarialPart.productId.productName,result.putInMetarialPart.batchNum,result.putInMetarialPart.putinStaff.name,result.putInMetarialPart.useNum,result.putInMetarialPart.lossNum,result.putInMetarialPart.putinDate,result.putInMetarialPart.remark,result.id,result.version,result.produceTask.id,result.putInMetarialPart.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1502700158273-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1502700158273.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/generalManage/produceTaskManage/data-dg1502700453811", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.inspectName,result.inspectUrl,result.chenkReportName,result.chenkReportUrl,result.product.productCode,result.product.productName,result.batchNum,result.standard,result.result,result.inspectDept.name,result.checkDept.name,result.inspectTime,result.checkTime,result.id,result.version,result.produceTask.id,result.product.id,result.inspectDept.id,result.checkDept.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1502700453811-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1502700453811.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/generalManage/produceTaskManage/data-dg1502700759865", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.taskReportPart.headId.tableNo,result.taskReportUrl,result.taskReportPart.reportStaff.name,result.taskReportPart.reportNum,result.taskReportPart.reportType.id,result.taskReportPart.reportType.value,result.taskReportPart.reportDate,result.taskReportPart.batchNum,result.taskReportPart.wareId.wareName,result.taskReportPart.storeID.placeSetName,result.taskReportPart.remark,result.id,result.version,result.produceTask.id,result.taskReportPart.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1502700759865-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1502700759865.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(produceTaskManage.getId() != null && produceTaskManage.getId() != -1){
				produceTaskManage = produceTaskManageService.getProduceTaskManage(produceTaskManage.getId());
			}
			produceTaskManageService.findDataGridPage(dg,dgclass,getDgPage(),produceTaskManage,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == produceTaskManage.getId()) {
		...
		produceTaskManageService.saveProduceTaskManage(produceTaskManage,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/generalManage/produceTaskManage/generalDetail/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "submit");
		bap_validate_datagrids=produceTaskManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", produceTaskManage.getId());
		...
		fileuploads.put("linkId", produceTaskManage.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/generalManage/produceTaskManage/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		produceTaskManageService.deleteProduceTaskManage(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/produceTaskManage/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "restore");
		produceTaskManageService.restoreProduceTaskManage(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/generalManage/produceTaskManage/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/generalManage/produceTaskManage/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "showDealInfoData");
		if (null != tableInfoId) {
			int count=produceTaskManageService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = produceTaskManageService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = produceTaskManageService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMProduceTaskManage> findExportDataInfos(Page<WOMProduceTaskManage> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = produceTaskManageService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = produceTaskManageService.getAssProperty(p.getCode());	
									//String assModelCode = produceTaskManageService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			produceTaskManageService.findProduceTaskManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			produceTaskManageService.findProduceTaskManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMProduceTaskManage> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMProduceTaskManage result : results){
	...
	public boolean isEditNew(){
		//return null == this.produceTaskManage || null == this.produceTaskManage.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/generalManage/produceTaskManage/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/generalManage/produceTaskManage/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/generalManage/produceTaskManage/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/produceTaskManage/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMProduceTaskManageService"
						, "com.supcon.orchid.WOM.services.WOMPutInManageService"
						, "com.supcon.orchid.WOM.services.WOMPutInMaterialService"
						...
						, "com.supcon.orchid.WOM.services.WOMCheckManageService"
						, "com.supcon.orchid.foundation.services.DepartmentService"
						, "com.supcon.orchid.WOM.services.WOMTaskReportManageService"
						, "com.supcon.orchid.WOM.services.WOMTaskReportingService"
					...
					, "com.supcon.orchid.WOM.services.WOMCheckManageService"
					, "com.supcon.orchid.WOM.services.WOMCheckManageService"
					, "com.supcon.orchid.WOM.services.WOMInspectService"
					, "com.supcon.orchid.WOM.services.WOMInspectService"
					, "com.supcon.orchid.WOM.services.WOMInspectService"
					, "com.supcon.orchid.WOM.services.WOMInspectService"
					, "com.supcon.orchid.WOM.services.WOMInspectService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_generalManage", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_generalManage");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/produceTaskManage/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_generalManage_ProduceTaskManage", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_generalManage_ProduceTaskManage", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/generalManage/produceTaskManage/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/generalManage/produceTaskManage/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_generalManage_ProduceTaskManage";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_ProduceTaskManage", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/generalManage/produceTaskManage/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
		_auditLog.setColumnStringA(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
		_auditLog.setColumnStringC(produceTaskManage.getId().toString());
		_auditLog.setColumnStringD(WOMProduceTaskManage.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_generalManage");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMProduceTaskManageAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, produceTaskManage,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, produceTaskManage,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,produceTaskManage);
		...
		printlog.setEntity("WOM_7.5.0.0_generalManage");
		...
		printlog.setBusinessId(produceTaskManage.getId());
		printlog.setBussinessKey(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
		printlog.setMainDisplay(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/generalManage/produceTaskManage/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(produceTaskManage,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/produceTaskManage/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(produceTaskManageService.getProduceTaskManage(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/generalManage/produceTaskManage/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/generalManage/produceTaskManage/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, produceTaskManage,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				produceTaskManage = produceTaskManageService.getProduceTaskManage(Long.parseLong(arrId[i]));
				if(null != produceTaskManage){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == produceTaskManage.getId() ? "" : "," + produceTaskManage.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == produceTaskManage.getId() ? "" : "," + produceTaskManage.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == produceTaskManage.getId() ? "" : "," + produceTaskManage.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == produceTaskManage.getId() ? "" : produceTaskManage.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, produceTaskManage,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,produceTaskManage);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//produceTaskManageService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/generalManage/produceTaskManage/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMProduceTaskManage> findExportPageInfos(Page<WOMProduceTaskManage> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_generalManage" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = produceTaskManageService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMProduceTaskManage queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = produceTaskManageService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = produceTaskManageService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_ProduceTaskManage");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_generalManage_ProduceTaskManage")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_generalManage_ProduceTaskManage");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_ProduceTaskManage");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = produceTaskManageService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_generalManage_ProduceTaskManage,WOM_7.5.0.0) */
// 自定义代码
	@Resource
	protected WOMProduceTaskService produceTaskService;
	@Resource
	protected X6BasicBaseUnitService baseUnitService;
	@Resource
	protected RMFormulaService formulaService;
	@Resource
	protected RMPickSiteService pickSiteService;
	@Resource
	protected RMSampleProjService sampleProjService;
	@Resource
	protected RMStandService standService;
	private String produceTaskId;//指令单id
	private String batchNum;//指令单编码
	...
	//根据指令单查找所有生效投料单表体数据
    @Action(value = "/WOM/generalManage/produceTaskManage/getPartData", results = { 
    		@Result(type = JSON, params = { "root", "dg1502700158273Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.putInMetarialPart.productId.productCode,result.putInMetarialPart.productId.productName,result.putInMetarialPart.batchNum,result.putInMetarialPart.putinStaff.name,result.putInMetarialPart.useNum,result.putInMetarialPart.lossNum,result.putInMetarialPart.putinDate,result.putInMetarialPart.remark,result.id,result.version,result.produceTask.id,result.putInMetarialPart.id,result.putInUrl,result.putInMetarialPart.headId.tableNo" }) })
    public String getPartData() throws Exception{
    	List<WOMPutInManage> partList= produceTaskManageService.getPartData(produceTaskId);
    	if(dg1502700158273Page==null) dg1502700158273Page = getDg1502700158273Page();  
    	if(partList!=null && partList.size()>0){
    		dg1502700158273Page.setResult(partList);
    	}
    	return SUCCESS;
    }
    
    //根据指令单查找所有生效报工单表体数据
    @Action(value = "/WOM/generalManage/produceTaskManage/getTaskReport", results = { 
    		@Result(type = JSON, params = { "root", "dg1502700759865Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.taskReportPart.headId.tableNo,result.taskReportPart.reportStaff.name,result.taskReportPart.reportNum,result.taskReportPart.reportType.id,result.taskReportPart.reportType.value,result.taskReportPart.reportDate,result.taskReportPart.batchNum,result.taskReportPart.wareId.wareName,result.taskReportPart.storeID.placeSetName,result.taskReportPart.remark,result.id,result.version,result.produceTask.id,result.taskReportPart.id,result.taskReportUrl" }) })
    public String getTaskReport() throws Exception{
    	List<WOMTaskReportManage> partList= produceTaskManageService.getTaskReport(produceTaskId);
    	if(dg1502700759865Page==null) dg1502700759865Page = getDg1502700759865Page();  
    	if(partList!=null && partList.size()>0){
    		dg1502700759865Page.setResult(partList);
    	}
    	return SUCCESS;
    }

	//根据指令单查找所有生效请检单表体数据
    @Action(value = "/WOM/generalManage/produceTaskManage/getCheckManage", results = { 
    		@Result(type = JSON, params = { "root", "dg1502700453811Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.inspectName,result.inspectUrl,result.chenkReportName,result.chenkReportUrl,result.product.productCode,result.product.productName,result.batchNum,result.standard,result.result,result.inspectDept.name,result.checkDept.name,result.inspectTime,result.checkTime,result.id,result.version,result.produceTask.id,result.product.id,result.inspectDept.id,result.checkDept.id" }) })
    public String getCheckManage() throws Exception{
    	List<WOMCheckManage> partList= produceTaskManageService.getCheckManage(batchNum);
    	if(dg1502700453811Page==null) dg1502700453811Page = getDg1502700453811Page();  
    	if(partList!=null && partList.size()>0){
    		dg1502700453811Page.setResult(partList);
    	}
    	return SUCCESS;
    }
            
    //初始化数据
    @Action(value = "/WOM/generalManage/produceTaskManage/getCheckDetail", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag,productId,productName,productCode,produceUnit,batchNum,pickSiteId,pickSiteName,sampleProjId,sampleProjName,standId,standName,produceUnitId" }) })
    public String getCheckDetail() throws Exception{
    	...
    	if(produceTask.getProductId()!=null && produceTask.getProductId().getId()!=null){
    		MESBasicProduct product = productService.getProduct(produceTask.getProductId().getId());
    		responseMap.put("productId", product.getId());
    		responseMap.put("productName", product.getProductName());
    		responseMap.put("productCode", product.getProductCode());
    		if(product.getProductBaseUnit()!=null && product.getProductBaseUnit().getId()!=null){
    			responseMap.put("produceUnit", baseUnitService.getBaseUnit(product.getProductBaseUnit().getId()).getName());
    			responseMap.put("produceUnitId", baseUnitService.getBaseUnit(product.getProductBaseUnit().getId()).getId());
    		}
    	}
    	if(!"".equals(produceTask.getProductBatchNum())){
    		responseMap.put("batchNum", produceTask.getProductBatchNum());
    	}
    	if(produceTask.getFormularId()!=null && produceTask.getFormularId().getId()!=null){
    		RMFormula formula = formulaService.getFormula(produceTask.getFormularId().getId());
    		if(formula.getPickSite()!=null && formula.getPickSite().getId()!=null){
    			responseMap.put("pickSiteId", pickSiteService.getPickSite(formula.getPickSite().getId()).getId());
    			responseMap.put("pickSiteName", pickSiteService.getPickSite(formula.getPickSite().getId()).getName());
    		}
    		if(formula.getSampleProj()!=null && formula.getSampleProj().getId()!=null){
    			responseMap.put("sampleProjId", sampleProjService.getSampleProj(formula.getSampleProj().getId()).getId());
    			responseMap.put("sampleProjName", sampleProjService.getSampleProj(formula.getSampleProj().getId()).getName());
    		}
    		if(formula.getQualityStandard()!=null && formula.getQualityStandard().getId()!=null){
    			responseMap.put("standId", standService.getStand(formula.getQualityStandard().getId()).getId());
    			responseMap.put("standName", standService.getStand(formula.getQualityStandard().getId()).getName());
    		}
    	}
    	responseMap.put("dealSuccessFlag", true);
    	return SUCCESS;
    ...
    //初始化数据
    @Action(value = "/WOM/generalManage/produceTaskManage/taskEnd", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag" }) })
    public String taskEnd() throws Exception{
    	WOMProduceTask produceTask = produceTaskService.getProduceTask(Long.parseLong(produceTaskId));
    	produceTaskManageService.taskEnd(produceTask);
    	responseMap.put("dealSuccessFlag", true);
    	return SUCCESS;
    }
	//综合管理页面
	@Action(value="/WOM/produceTask/produceTask/getdatagrid",results={@Result(name="success",type=FREEMARKER,location="../../generalManage/produceTaskManage/generalDetail.ftl")})
