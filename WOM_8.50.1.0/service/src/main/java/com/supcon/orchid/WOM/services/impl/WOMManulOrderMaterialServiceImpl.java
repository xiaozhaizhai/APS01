...
import com.supcon.orchid.WOM.services.WOMManulOrderMaterialImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMManulOrderMaterialDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_manulOrderMaterialService")
@Transactional
public class WOMManulOrderMaterialServiceImpl  extends BaseServiceImpl<WOMManulOrderMaterial> implements WOMManulOrderMaterialService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMManulOrderMaterial> bapBaseService;
	...
	@Autowired
	private WOMManulOrderMaterialImportService manulOrderMaterialImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMManulOrderMaterial.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  manulOrderMaterialDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMManulOrderMaterial.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = manulOrderMaterialDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMManulOrderMaterial.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMManulOrderMaterial.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  manulOrderMaterialDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMManulOrderMaterial.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMManulOrderMaterial.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMManulOrderMaterial> list = (List<WOMManulOrderMaterial>) manulOrderMaterialDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMManulOrderMaterial getManulOrderMaterial(long id){
		return getManulOrderMaterial(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMManulOrderMaterial getManulOrderMaterial(long id, String viewCode){
		WOMManulOrderMaterial manulOrderMaterial = manulOrderMaterialDao.load(id);

		// 一对多情况处理
		if(manulOrderMaterial != null){
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getManulOrderMaterialAsJSON(long id, String include){
		WOMManulOrderMaterial manulOrderMaterial = manulOrderMaterialDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(manulOrderMaterial, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial){
		if(null != manulOrderMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
			AuditUtil.setColumnStringC(manulOrderMaterial.getId().toString());
		}
		beforeDeleteManulOrderMaterial(manulOrderMaterial);
		manulOrderMaterialDao.deletePhysical(manulOrderMaterial);
		afterDeleteManulOrderMaterial(manulOrderMaterial);
		manulOrderMaterialDao.flush();
		List<WOMManulOrderMaterial> params = new ArrayList<WOMManulOrderMaterial>();
		params.add(manulOrderMaterial);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMaterial(List<Long> manulOrderMaterialIds){
		deleteManulOrderMaterial(manulOrderMaterialIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMaterial(List<Long> manulOrderMaterialIds, String eventTopic) {
		List<WOMManulOrderMaterial> manulOrderMaterials = new ArrayList<WOMManulOrderMaterial>();
		for(Long manulOrderMaterialId : manulOrderMaterialIds){
			WOMManulOrderMaterial manulOrderMaterial = getManulOrderMaterial(manulOrderMaterialId);
			manulOrderMaterials.add(manulOrderMaterial);
			if(null != manulOrderMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == manulOrderMaterial.getId() ? "" : "," + manulOrderMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == manulOrderMaterial.getId() ? "" : "," + manulOrderMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == manulOrderMaterial.getId() ? "" : "," + manulOrderMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
				}
			}
		}
		
		for(WOMManulOrderMaterial manulOrderMaterial : manulOrderMaterials){
			beforeDeleteManulOrderMaterial(manulOrderMaterial);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(manulOrderMaterials != null && manulOrderMaterials.size() > 0 ){
			for(WOMManulOrderMaterial manulOrderMaterial : manulOrderMaterials){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMManulOrderMaterial.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(manulOrderMaterial));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),manulOrderMaterial, manulOrderMaterial.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(manulOrderMaterialIds != null && manulOrderMaterialIds.size() > 0) {
			String hql = "delete " + WOMManulOrderMaterial.JPA_NAME + " where id in(:ids)";
			Query query = manulOrderMaterialDao.createQuery(hql);
			query.setParameterList("ids", manulOrderMaterialIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMManulOrderMaterial manulOrderMaterialz : manulOrderMaterials){
				manulOrderMaterialz.setValid(false);
				List<WOMManulOrderMaterial> params = new ArrayList<WOMManulOrderMaterial>();
				params.add(manulOrderMaterialz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_manulOrder");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMaterial(long manulOrderMaterialId, int manulOrderMaterialVersion){
		this.deleteManulOrderMaterial(manulOrderMaterialId, manulOrderMaterialVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMaterial(long manulOrderMaterialId, int manulOrderMaterialVersion,SignatureLog signatureLog){
		WOMManulOrderMaterial manulOrderMaterial = getManulOrderMaterial(manulOrderMaterialId);
		if(manulOrderMaterial != null && !manulOrderMaterial.isValid()){
		...
		beforeDeleteManulOrderMaterial(manulOrderMaterial);
		manulOrderMaterialDao.delete(manulOrderMaterialId, manulOrderMaterialVersion);
		afterDeleteManulOrderMaterial(manulOrderMaterial);
		manulOrderMaterialDao.flush();
		List<WOMManulOrderMaterial> params = new ArrayList<WOMManulOrderMaterial>();
		params.add(manulOrderMaterial);
		...
		if(signatureLog != null ) {
			...
			businessKey=manulOrderMaterial.getId();
			signatureLog.setTableId(manulOrderMaterial.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMaterial(String manulOrderMaterialIds){
		this.deleteManulOrderMaterial(manulOrderMaterialIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.common.delete",operType="3")
	public void deleteManulOrderMaterial(String manulOrderMaterialIds,SignatureLog signatureLog){
		deleteCollection(manulOrderMaterialIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteManulOrderMaterial(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_manulOrder#modelCode:WOM_7.5.0.0_manulOrder_ManulOrderMaterial#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="ec.engine.restore",operType="10")
	public void restoreManulOrderMaterial(String manulOrderMaterialIds){
		restoreCollection(manulOrderMaterialIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreManulOrderMaterial(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="ec.engine.restore",operType="10")
	public void restoreManulOrderMaterial(long manulOrderMaterialId){
		
		findBusinessKeyUsed(manulOrderMaterialId);	//判断业务主键是否重复
		...
		beforeRestoreManulOrderMaterial(manulOrderMaterial);
		manulOrderMaterial.setValid(true);
		manulOrderMaterialDao.update(manulOrderMaterial);
		afterRestoreManulOrderMaterial(manulOrderMaterial);
		List<WOMManulOrderMaterial> params = new ArrayList<WOMManulOrderMaterial>();
		params.add(manulOrderMaterial);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long manulOrderMaterialId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_manulOrder_ManulOrderMaterial");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMManulOrderMaterial.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMManulOrderMaterial.TABLE_NAME +" where id = ? )";
			List<Object> list =  manulOrderMaterialDao.createNativeQuery(sql,manulOrderMaterialId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseManulOrderMaterial(List<WOMManulOrderMaterial>  manulOrderMaterials){
		for(WOMManulOrderMaterial manulOrderMaterial:manulOrderMaterials)  {
			saveManulOrderMaterial(manulOrderMaterial, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseManulOrderMaterial(List<WOMManulOrderMaterial>  manulOrderMaterials){
		for(WOMManulOrderMaterial manulOrderMaterial:manulOrderMaterials)  {
			saveManulOrderMaterial(manulOrderMaterial, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="ec.flow.save")
	public void saveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveManulOrderMaterial(manulOrderMaterial, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="ec.flow.save")
	public void saveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveManulOrderMaterial(manulOrderMaterial, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateManulOrderMaterialCodes(WOMManulOrderMaterial manulOrderMaterial) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateManulOrderMaterialCodes(manulOrderMaterial, false);
	}
	
	public void generateManulOrderMaterialCodes(WOMManulOrderMaterial manulOrderMaterial, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateManulOrderMaterialSummarys(WOMManulOrderMaterial manulOrderMaterial) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateManulOrderMaterialSummarys(manulOrderMaterial, false);
	}

	public void generateManulOrderMaterialSummarys(WOMManulOrderMaterial manulOrderMaterial, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial")
	public void saveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveManulOrderMaterial(manulOrderMaterial, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial")
	public void saveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_manulOrder");
		String url = null;
		if(manulOrderMaterial.getId() != null && manulOrderMaterial.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, manulOrderMaterial);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(manulOrderMaterial);
		beforeSaveManulOrderMaterial(manulOrderMaterial, viewIsView);

		if (viewIsView) {
			manulOrderMaterialDao.saveWithRevertVersion(manulOrderMaterial);
		} else {
			if(isNew)
				manulOrderMaterialDao.save(manulOrderMaterial);
			else
				manulOrderMaterialDao.merge(manulOrderMaterial);
		}
		if(null != manulOrderMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
		...
		dealDatagridsSave(manulOrderMaterial,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateManulOrderMaterialCodes(manulOrderMaterial, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateManulOrderMaterialSummarys(manulOrderMaterial, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveManulOrderMaterial(manulOrderMaterial, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, manulOrderMaterial);
			}
		}
		List<WOMManulOrderMaterial> params = new ArrayList<WOMManulOrderMaterial>();
		manulOrderMaterialDao.flush();
		manulOrderMaterialDao.clear();
		manulOrderMaterial = manulOrderMaterialDao.load(manulOrderMaterial.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMManulOrderMaterial manulOrderMaterial){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("manulOrderMaterial", manulOrderMaterial);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial")
	public void mergeManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(manulOrderMaterial);
		beforeSaveManulOrderMaterial(manulOrderMaterial);
		manulOrderMaterialDao.merge(manulOrderMaterial);
		if(null != manulOrderMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == manulOrderMaterial.getId() ? "" : manulOrderMaterial.getId().toString());
		...
		dealDatagridsSave(manulOrderMaterial,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveManulOrderMaterial(manulOrderMaterial);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMManulOrderMaterial> findManulOrderMaterials(Page<WOMManulOrderMaterial> page, Criterion... criterions) {
		return manulOrderMaterialDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findManulOrderMaterials(Page<WOMManulOrderMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findManulOrderMaterials(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findManulOrderMaterials(Page<WOMManulOrderMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findManulOrderMaterials(Page<WOMManulOrderMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMManulOrderMaterial> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"manulOrderMaterial\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"manulOrderMaterial\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"manulOrderMaterial\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"manulOrderMaterial\".MANUL_ORDER_ID IN ( SELECT ID FROM WOM_MENUL_ORDER_MAIN \"manulOrderMain\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "WOM_7.5.0.0_manulOrder" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  manulOrderMaterialDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  manulOrderMaterialDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_manulOrder").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"manulOrderMain\"","WOM_7.5.0.0_manulOrder","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"manulOrderMaterial\".LAY_REC ASC, \"manulOrderMaterial\".SORT ASC");
				} else {
					orderPart.append(" \"manulOrderMaterial\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) manulOrderMaterialDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=manulOrderMaterialDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = manulOrderMaterialDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMManulOrderMaterial> result = new ArrayList<WOMManulOrderMaterial>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMManulOrderMaterial.class,manulOrderMaterialDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMManulOrderMaterial.class,manulOrderMaterialDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMManulOrderMaterial manulOrderMaterial : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(manulOrderMaterial.getId(), "WOM_manulOrder_manulOrderMaterial");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(manulOrderMaterial.getId(), "WOM_manulOrder_manulOrderMaterial");
							if(null != documents && !documents.isEmpty()) {
								manulOrderMaterial.setDocument(documents.get(0));
								manulOrderMaterial.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMManulOrderMaterial> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMManulOrderMaterial> result) {
		View view = null;
		...
		Map<Long, WOMManulOrderMaterial> rsMap = new HashMap<Long, WOMManulOrderMaterial>();
		for (WOMManulOrderMaterial manulOrderMaterial : result) {
			ids.add(manulOrderMaterial.getId());
			rsMap.put(manulOrderMaterial.getId(), manulOrderMaterial);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(manulOrderMaterialDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = manulOrderMaterialDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_manulOrder_ManulOrderMaterial" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_manulOrder_ManulOrderMaterial" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  manulOrderMaterialDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"manulOrderMaterial\"","WOM_7.5.0.0_manulOrder","","WOM_MENUL_ORDER_MATERIAL",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"manulOrderMaterial\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"manulOrderMaterial\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"manulOrderMaterial\"","WOM_7.5.0.0_manulOrder","","WOM_MENUL_ORDER_MATERIAL",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_manulOrder_ManulOrderMaterial" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("manulOrderMaterial.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"manulOrderMaterial\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"manulOrderMaterial\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"manulOrderMaterial\".CID = 1 OR \"manulOrderMaterial\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("manulOrderMaterial.mc\"").append(".MANUL_ORDER_MATERIAL) not in ("+notinValue+")");
			}
			
			SQLQuery query = manulOrderMaterialDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=manulOrderMaterialDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMManulOrderMaterial.ENTITY_CODE;}
	protected String getModuleCode(){return WOMManulOrderMaterial.MODULE_CODE;}
	protected String getModelCode(){return WOMManulOrderMaterial.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param manulOrderMaterial 模型实例
	 */
	public void checkUniqueConstraint(WOMManulOrderMaterial manulOrderMaterial){
	}
	
	
	@Override
	public List<WOMManulOrderMaterial> getManulOrderMaterials(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMManulOrderMaterial.class);
		if(params!=null){
			...
			return manulOrderMaterialDao.findByHql("from " + WOMManulOrderMaterial.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return manulOrderMaterialDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMManulOrderMaterial> findManulOrderMaterialsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMManulOrderMaterial.class);
	...
	@Override
	public List<WOMManulOrderMaterial> findManulOrderMaterialsByHql(String hql, Object... objects){
		
		return   manulOrderMaterialDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMManulOrderMaterial loadManulOrderMaterialByBussinessKey(WOMManulOrderMaterial manulOrderMaterial){
		return loadManulOrderMaterialByBussinessKey(manulOrderMaterial.getId());
	}
	
	@Override
	public Page<WOMManulOrderMaterial> getManulOrderMaterials(Page<WOMManulOrderMaterial> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMManulOrderMaterial.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = manulOrderMaterialDao.findByPage(page, sql);
			}else{
				page = manulOrderMaterialDao.findAllByPage(page);
			}
			return page;
		}
		page = manulOrderMaterialDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMManulOrderMaterial> findByTreeDataGridManulOrderMain(List<WOMManulOrderMaterial> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMManulOrderMaterial manulOrderMaterial : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridManulOrderMain(Page<WOMManulOrderMaterial> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMManulOrderMaterial> manulOrderMaterials = dgPage.getResult();
			for(WOMManulOrderMaterial manulOrderMaterial : manulOrderMaterials){
				
			}
			dgPage.setResult(manulOrderMaterials);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMManulOrderMaterial loadManulOrderMaterialByBussinessKey(Serializable bussinessKey){
		return manulOrderMaterialDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMManulOrderMaterial> findByProperty(String propertyName, Object object){
		return manulOrderMaterialDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMManulOrderMaterial findEntityByProperty(String propertyName, Object object){
		return manulOrderMaterialDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMManulOrderMaterial.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = manulOrderMaterialDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_manulOrder_ManulOrderMaterial","runtime");
		try {
	...
	public void dealDatagridsSave(WOMManulOrderMaterial manulOrderMaterial,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,manulOrderMaterialDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMManulOrderMaterial.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, manulOrderMaterial);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",manulOrderMaterial.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMManulOrderMaterial.TABLE_NAME +" where valid = 1";
			list = manulOrderMaterialDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMManulOrderMaterial.TABLE_NAME;
			list = manulOrderMaterialDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMManulOrderMaterial.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = manulOrderMaterialDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMManulOrderMaterialDaoImpl manulOrderMaterialDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchManulOrderMaterial(final List<WOMManulOrderMaterial> insertObjs, final List<WOMManulOrderMaterial> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchManulOrderMaterial(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchManulOrderMaterial(final List<WOMManulOrderMaterial> insertObjs, final List<WOMManulOrderMaterial> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1563193064716"),importCount));
		...
		Map<Long, WOMManulOrderMaterial> oldUpdateObjsMap = new HashMap<Long, WOMManulOrderMaterial>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMManulOrderMaterial bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMManulOrderMaterial> bizObjList = manulOrderMaterialDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMManulOrderMaterial bizObj = bizObjList.get(mIndex);
		...
		final List<WOMManulOrderMaterial> nmeObjs = new ArrayList<WOMManulOrderMaterial>();
		...
		long id=autoGeneratorID.getNextId(WOMManulOrderMaterial.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMManulOrderMaterial manulOrderMaterial:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMManulOrderMaterial.TABLE_NAME,1,dbId);
			if(manulOrderMaterial.getId() != null ){
				m.put(manulOrderMaterial.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			manulOrderMaterial.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) manulOrderMaterialDao.createNativeQuery(sql, "WOM_7.5.0.0_manulOrder_ManulOrderMaterial"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		manulOrderMaterialDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMManulOrderMaterial.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMManulOrderMaterial manulOrderMaterial = updateObjs.get(updateIndex);
						Long id = manulOrderMaterial.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = manulOrderMaterial.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(manulOrderMaterial);
							...
							nmeObjs.add(manulOrderMaterial);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMManulOrderMaterial.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMManulOrderMaterial.TABLE_NAME + " order by id desc";
//					Query query = manulOrderMaterialDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMManulOrderMaterial manulOrderMaterial = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = manulOrderMaterial.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		manulOrderMaterialDaoImpl.getSession().flush();
		manulOrderMaterialDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMManulOrderMaterial bizObj = insertObjs.get(insertIndex);
			manulOrderMaterialImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMManulOrderMaterial bizObj = updateObjs.get(updateIndex);
			WOMManulOrderMaterial oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			manulOrderMaterialImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMManulOrderMaterial> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMManulOrderMaterial> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) manulOrderMaterialDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMManulOrderMaterial> result = getResult(page, query, new PendingResultTransformer(WOMManulOrderMaterial.class,manulOrderMaterialDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMManulOrderMaterial model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = manulOrderMaterialDao.createNativeQuery(sql, "WOM_7.5.0.0_manulOrder_ManulOrderMaterial").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = manulOrderMaterialDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = manulOrderMaterialDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = manulOrderMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?manulOrderMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  manulOrderMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,manulOrderMaterialDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="ec.flow.save")
	public void saveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, DataGridService dataGridService){
		saveManulOrderMaterial(manulOrderMaterial, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="ec.flow.save")
	public void saveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, DataGridService dataGridService, String viewCode){
		saveManulOrderMaterial(manulOrderMaterial, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial")
	public void saveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveManulOrderMaterial(manulOrderMaterial, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial")
	public void saveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_manulOrder";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_manulOrder");
		String url = null;
		if(null != manulOrderMaterial.getId() && manulOrderMaterial.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMManulOrderMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial")
	public Map<Object, Object> manulOrderMaterialDataGridImport(WOMManulOrderMaterial manulOrderMaterial, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = manulOrderMaterial.getVirtualId();
		
		manulOrderMaterial = manulOrderMaterialDao.load(manulOrderMaterial.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, manulOrderMaterial.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = manulOrderMaterial.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(manulOrderMaterial);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, manulOrderMaterial.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial")
	public void mergeManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(manulOrderMaterial);
		beforeSaveManulOrderMaterial(manulOrderMaterial);
		manulOrderMaterialDao.merge(manulOrderMaterial);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_manulOrder",model="WOM_7.5.0.0_manulOrder_ManulOrderMaterial",desc="foundation.batch.save",operType="4")
	public void batchSaveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_manulOrder";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_manulOrder");
	...
	@Override
	public Page<WOMManulOrderMaterial> getByPage(Page<WOMManulOrderMaterial> page,DetachedCriteria detachedCriteria){
		return manulOrderMaterialDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, Object... objects){
		checkUniqueConstraint(manulOrderMaterial);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportManulOrderMaterial(List<WOMManulOrderMaterial> manulOrderMaterials, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportManulOrderMaterial(List<WOMManulOrderMaterial> manulOrderMaterials){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportManulOrderMaterial(List<WOMManulOrderMaterial> manulOrderMaterials, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportManulOrderMaterial(List<WOMManulOrderMaterial> manulOrderMaterials){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreManulOrderMaterial(WOMManulOrderMaterial manulOrderMaterial){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMManulOrderMaterial manulOrderMaterial, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMManulOrderMaterial manulOrderMaterial, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportManulOrderMaterial(List<WOMManulOrderMaterial> insertObjs, List<WOMManulOrderMaterial> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportManulOrderMaterial,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportManulOrderMaterial(List<WOMManulOrderMaterial> insertObjs, List<WOMManulOrderMaterial> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportManulOrderMaterial,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_manulOrder_ManulOrderMaterial,WOM_7.5.0.0) */
// 自定义代码
