...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMManulOrderMainAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMManulOrderMain> {
	...
	//导出excel时,主模型内容
	List<WOMManulOrderMain> queryResults = new ArrayList<WOMManulOrderMain>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/manulOrder/manulOrderMain/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/manulOrder/manulOrderMain/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_manulOrder_ManulOrderMain" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMManulOrderMain> manulOrderMains;
	
	public List<WOMManulOrderMain> getManulOrderMains() {
		return manulOrderMains;
	}
	...
	private Page<WOMManulOrderMain> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			manulOrderMain = new WOMManulOrderMain();
		} else {
			if(id > 0){
				manulOrderMain = manulOrderMainService.getManulOrderMain(id, viewCode);
			}
		}
	...
	public Page<WOMManulOrderMain> getPage() {
		if (null == page) {
			page = new Page<WOMManulOrderMain>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/manulOrder/manulOrderMain/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = manulOrderMainService.getManulOrderMainAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/manulOrder/manulOrderMain/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1563192841368
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String manulOrderMainReferenceCopy() throws Exception{
			WOMManulOrderMain oldManulOrderMain = null;
			if(isEditNew()){
				...
				oldManulOrderMain = new WOMManulOrderMain();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,manulOrderMain.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, manulOrderMain.getId());
				...
				oldManulOrderMain = manulOrderMainService.getManulOrderMain(id);
			}
			
			
			manulOrderMain = manulOrderMainService.getManulOrderMain(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_manulOrder_ManulOrderMain");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMManulOrderMain.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldManulOrderMain, null);
						Method setValue = WOMManulOrderMain.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(manulOrderMain, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			manulOrderMain.setCreateDepartmentId(oldManulOrderMain.getCreateDepartmentId());
			manulOrderMain.setCreatePositionId(oldManulOrderMain.getCreatePositionId());
			manulOrderMain.setCreateStaffId(oldManulOrderMain.getCreateStaffId());
			manulOrderMain.setOwnerDepartmentId(oldManulOrderMain.getOwnerDepartmentId());
			manulOrderMain.setOwnerPositionId(oldManulOrderMain.getOwnerPositionId());
			manulOrderMain.setOwnerStaffId(oldManulOrderMain.getOwnerStaffId());
			manulOrderMain.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrdersEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "manulOrdersEdit.ftl") }),
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrdersEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "manulOrdersEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=manulOrderMainService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return manulOrderMainReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			manulOrderMain = manulOrderMainService.getManulOrderMain(this.manulOrderMain.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,manulOrderMain.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrdersView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "manulOrdersView.ftl") }),
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrdersViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "manulOrdersViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulAllOrdersRef", results = { @Result(type = FREEMARKER, location = "manulAllOrdersRef.ftl") })
		,
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrderList", results = { @Result(type = FREEMARKER, location = "manulOrderList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = manulOrderMainService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(manulOrderMainService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(manulOrderMainService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_manulOrder_ManulOrderMain", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrdersRef", results = { @Result(type = FREEMARKER, location = "manulOrdersRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulAllOrdersRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.manulOrderNum,result.product.productCode,result.product.productName,result.ware.wareAddress,result.orderNumber,result.orderTyep,result.edition,result.unit.name,result.planStartTime,result.planEndTime,result.saleOrderNum,result.saleOrderProject,result.product.unitGroup.name,result.product.productSpecif,result.product.productModel,result.product.id,result.ware.id,result.unit.id,result.product.unitGroup.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="manulAllOrdersRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrderList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.manulOrderNum,result.isFinshed,result.product.productCode,result.product.productName,result.unit.name,result.orderNumber,result.orderTyep,result.edition,result.planStartTime,result.planEndTime,result.hasManualNumber,result.manulStaff.name,result.ware.wareName,result.saleOrderNum,result.saleOrderProject,result.remarks,result.product.id,result.unit.id,result.manulStaff.id,result.ware.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="manulOrderList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"manulOrderMain\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"manulOrderMain\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"manulOrderMain\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"manulOrderMain\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			manulOrderMainService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulAllOrdersRef-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrderList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrdersRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.manulOrderNum,result.product.productCode,result.product.productName,result.ware.wareAddress,result.orderNumber,result.orderTyep,result.edition,result.unit.name,result.planStartTime,result.planEndTime,result.saleOrderNum,result.saleOrderProject,result.product.unitGroup.name,result.product.productSpecif,result.product.productModel,result.product.id,result.ware.id,result.unit.id,result.product.unitGroup.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="manulOrdersRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/manulOrder/manulOrderMain/allTempleteDownload", results = { @Result(name="excel",type="excel",location="manulAllOrdersRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_manulOrder_ManulOrderMain");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		manulOrderMainService.findDataGridPage(dg,dgclass,getDgPage(),manulOrderMain,"", null);
	...
	private Page<WOMManulOrderMaterial> dg1563346128169Page;
	public Page<WOMManulOrderMaterial> getDg1563346128169Page(){
		if(null == dg1563346128169Page){
			dg1563346128169Page = new Page<WOMManulOrderMaterial>(1,20);
		}
		return dg1563346128169Page;
	}
	
	private Class dg1563346128169Clazz = WOMManulOrderMaterial.class;
	
	public Class getDg1563346128169Clazz() {
		return dg1563346128169Clazz;
	}
	
	private List<WOMManulOrderMaterial> dg1563346128169List;
	public List<WOMManulOrderMaterial> getDg1563346128169List(){
		if(null == dg1563346128169List){
			dg1563346128169List = new ArrayList<WOMManulOrderMaterial>();
		}
		return dg1563346128169List;
	}
	public void setDg1563346128169ListJson(String json) {
		//dg1563346128169List = (List<WOMManulOrderMaterial>) manulOrderMainService.generateObjectFromJson(json, WOMManulOrderMaterial.class);
	}
	
	private List<Long> dg1563346128169DeletedIds;
	public List<Long> getDg1563346128169DeletedIds(){
		if(null == dg1563346128169DeletedIds){
			dg1563346128169DeletedIds = new ArrayList<Long>();
		}
		return dg1563346128169DeletedIds;
	}

	private Page<WOMManulOrderMaterial> dg1568964886993Page;
	public Page<WOMManulOrderMaterial> getDg1568964886993Page(){
		if(null == dg1568964886993Page){
			dg1568964886993Page = new Page<WOMManulOrderMaterial>(1,20);
		}
		return dg1568964886993Page;
	}
	
	private Class dg1568964886993Clazz = WOMManulOrderMaterial.class;
	
	public Class getDg1568964886993Clazz() {
		return dg1568964886993Clazz;
	}
	
	private List<WOMManulOrderMaterial> dg1568964886993List;
	public List<WOMManulOrderMaterial> getDg1568964886993List(){
		if(null == dg1568964886993List){
			dg1568964886993List = new ArrayList<WOMManulOrderMaterial>();
		}
		return dg1568964886993List;
	}
	public void setDg1568964886993ListJson(String json) {
		//dg1568964886993List = (List<WOMManulOrderMaterial>) manulOrderMainService.generateObjectFromJson(json, WOMManulOrderMaterial.class);
	}
	
	private List<Long> dg1568964886993DeletedIds;
	public List<Long> getDg1568964886993DeletedIds(){
		if(null == dg1568964886993DeletedIds){
			dg1568964886993DeletedIds = new ArrayList<Long>();
		}
		return dg1568964886993DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/manulOrder/manulOrderMain/data-dg1563346128169", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.material.productCode,result.material.productName,result.ablePass,result.ware.wareName,result.ware.wareCode,result.unit.name,result.needNumber,result.materialBatchNum,result.edition,result.coOperation,result.rsNUM,result.rsPOS,result.id,result.version,result.unit.id,result.manulOrderId.id,result.material.id,result.ware.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1563346128169-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1563346128169.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/manulOrder/manulOrderMain/data-dg1568964886993", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.material.productCode,result.material.productName,result.ablePass,result.ware.wareName,result.ware.wareCode,result.unit.name,result.needNumber,result.materialBatchNum,result.edition,result.coOperation,result.rsNUM,result.rsPOS,result.id,result.version,result.unit.id,result.manulOrderId.id,result.material.id,result.ware.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1568964886993-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1568964886993.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(manulOrderMain.getId() != null && manulOrderMain.getId() != -1){
				manulOrderMain = manulOrderMainService.getManulOrderMain(manulOrderMain.getId());
			}
			manulOrderMainService.findDataGridPage(dg,dgclass,getDgPage(),manulOrderMain,searchCondition,params);
	...
	private void save() throws Exception {
		...
		manulOrderMainService.saveManulOrderMain(manulOrderMain,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrdersEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/manulOrder/manulOrderMain/manulOrdersView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "submit");
		bap_validate_datagrids=manulOrderMainService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", manulOrderMain.getId());
		...
		fileuploads.put("linkId", manulOrderMain.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/manulOrder/manulOrderMain/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		manulOrderMainService.deleteManulOrderMain(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMain/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "restore");
		manulOrderMainService.restoreManulOrderMain(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/manulOrder/manulOrderMain/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/manulOrder/manulOrderMain/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "showDealInfoData");
		if (null != tableInfoId) {
			int count=manulOrderMainService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = manulOrderMainService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = manulOrderMainService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMManulOrderMain> findExportDataInfos(Page<WOMManulOrderMain> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = manulOrderMainService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = manulOrderMainService.getAssProperty(p.getCode());	
									//String assModelCode = manulOrderMainService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			manulOrderMainService.findManulOrderMains(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			manulOrderMainService.findManulOrderMains(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMManulOrderMain> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMManulOrderMain result : results){
	...
	public boolean isEditNew(){
		//return null == this.manulOrderMain || null == this.manulOrderMain.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/manulOrder/manulOrderMain/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/manulOrder/manulOrderMain/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/manulOrder/manulOrderMain/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMain/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMManulOrderMainService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_manulOrder", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_manulOrder");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMain/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_manulOrder_ManulOrderMain", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_manulOrder_ManulOrderMain", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/manulOrder/manulOrderMain/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/manulOrder/manulOrderMain/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_manulOrder_ManulOrderMain";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_manulOrder_ManulOrderMain", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/manulOrder/manulOrderMain/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMManulOrderMain manulOrderMain = null;
		if(null != id) {
			manulOrderMain = manulOrderMainService.getManulOrderMain(id);
		}
		...
		_auditLog.setColumnStringB(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
		_auditLog.setColumnStringA(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
		_auditLog.setColumnStringC(manulOrderMain.getId().toString());
		_auditLog.setColumnStringD(WOMManulOrderMain.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_manulOrder");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMManulOrderMainAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, manulOrderMain,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, manulOrderMain,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,manulOrderMain);
		...
		printlog.setEntity("WOM_7.5.0.0_manulOrder");
		...
		printlog.setBusinessId(manulOrderMain.getId());
		printlog.setBussinessKey(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
		printlog.setMainDisplay(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMain/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(manulOrderMain,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/manulOrder/manulOrderMain/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(manulOrderMainService.getManulOrderMain(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/manulOrder/manulOrderMain/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/manulOrder/manulOrderMain/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, manulOrderMain,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				manulOrderMain = manulOrderMainService.getManulOrderMain(Long.parseLong(arrId[i]));
				if(null != manulOrderMain){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == manulOrderMain.getId() ? "" : "," + manulOrderMain.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == manulOrderMain.getId() ? "" : "," + manulOrderMain.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == manulOrderMain.getId() ? "" : "," + manulOrderMain.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == manulOrderMain.getId() ? "" : manulOrderMain.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, manulOrderMain,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,manulOrderMain);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/manulOrder/manulOrderMain/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMManulOrderMain> findExportPageInfos(Page<WOMManulOrderMain> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_manulOrder" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = manulOrderMainService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMManulOrderMain queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = manulOrderMainService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = manulOrderMainService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_manulOrder_ManulOrderMain");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_manulOrder_ManulOrderMain")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_manulOrder_ManulOrderMain");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_manulOrder_ManulOrderMain");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = manulOrderMainService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_manulOrder_ManulOrderMain,WOM_7.5.0.0) */
// 自定义代码
	private String orderIDs;

	private String finshBack;

	public String getFinshBack() {
		return finshBack;
	}

	public void setFinshBack(String finshBack) {
		this.finshBack = finshBack;
	}
	...
	@Action(value = "/WOM/manulOrder/manulOrderMain/finshOrder", results = { @Result(type = JSON, params = { "root", "responseMap"}) })
	public  String finshOrder() throws Exception{
		manulOrderMainService.finshOrder(orderIDs,finshBack);
		return SUCCESS;
	}
/* CUSTOM CODE END */
}