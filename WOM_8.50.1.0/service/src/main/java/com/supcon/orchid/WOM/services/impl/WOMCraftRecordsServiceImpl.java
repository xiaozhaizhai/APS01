...
import com.supcon.orchid.WOM.services.WOMCraftRecordsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMCraftRecordsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_craftRecordsService")
@Transactional
public class WOMCraftRecordsServiceImpl  extends BaseServiceImpl<WOMCraftRecords> implements WOMCraftRecordsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMCraftRecords> bapBaseService;
	...
	@Autowired
	private WOMCraftRecordsImportService craftRecordsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMCraftRecords.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  craftRecordsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMCraftRecords.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = craftRecordsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMCraftRecords.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMCraftRecords.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  craftRecordsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMCraftRecords.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMCraftRecords.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMCraftRecords> list = (List<WOMCraftRecords>) craftRecordsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMCraftRecords getCraftRecords(long id){
		return getCraftRecords(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMCraftRecords getCraftRecords(long id, String viewCode){
		WOMCraftRecords craftRecords = craftRecordsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getCraftRecordsAsJSON(long id, String include){
		WOMCraftRecords craftRecords = craftRecordsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(craftRecords, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.common.delete",operType="3")
	public void deleteCraftRecords(WOMCraftRecords craftRecords){
		if(null != craftRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCraftRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
			AuditUtil.setColumnStringA(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
			AuditUtil.setColumnStringC(craftRecords.getId().toString());
		}
		beforeDeleteCraftRecords(craftRecords);
		craftRecordsDao.deletePhysical(craftRecords);
		afterDeleteCraftRecords(craftRecords);
		craftRecordsDao.flush();
		List<WOMCraftRecords> params = new ArrayList<WOMCraftRecords>();
		params.add(craftRecords);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.common.delete",operType="3")
	public void deleteCraftRecords(List<Long> craftRecordsIds){
		deleteCraftRecords(craftRecordsIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.common.delete",operType="3")
	public void deleteCraftRecords(List<Long> craftRecordsIds, String eventTopic) {
		List<WOMCraftRecords> craftRecordss = new ArrayList<WOMCraftRecords>();
		for(Long craftRecordsId : craftRecordsIds){
			WOMCraftRecords craftRecords = getCraftRecords(craftRecordsId);
			craftRecordss.add(craftRecords);
			if(null != craftRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCraftRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == craftRecords.getId() ? "" : "," + craftRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == craftRecords.getId() ? "" : "," + craftRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == craftRecords.getId() ? "" : "," + craftRecords.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
				}
			}
		}
		
		for(WOMCraftRecords craftRecords : craftRecordss){
			beforeDeleteCraftRecords(craftRecords);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(craftRecordss != null && craftRecordss.size() > 0 ){
			for(WOMCraftRecords craftRecords : craftRecordss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMCraftRecords.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(craftRecords));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),craftRecords, craftRecords.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(craftRecordsIds != null && craftRecordsIds.size() > 0) {
			String hql = "delete " + WOMCraftRecords.JPA_NAME + " where id in(:ids)";
			Query query = craftRecordsDao.createQuery(hql);
			query.setParameterList("ids", craftRecordsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMCraftRecords craftRecordsz : craftRecordss){
				craftRecordsz.setValid(false);
				List<WOMCraftRecords> params = new ArrayList<WOMCraftRecords>();
				params.add(craftRecordsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_procFeedback");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.common.delete",operType="3")
	public void deleteCraftRecords(long craftRecordsId, int craftRecordsVersion){
		this.deleteCraftRecords(craftRecordsId, craftRecordsVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.common.delete",operType="3")
	public void deleteCraftRecords(long craftRecordsId, int craftRecordsVersion,SignatureLog signatureLog){
		WOMCraftRecords craftRecords = getCraftRecords(craftRecordsId);
		if(craftRecords != null && !craftRecords.isValid()){
		...
		beforeDeleteCraftRecords(craftRecords);
		craftRecordsDao.delete(craftRecordsId, craftRecordsVersion);
		afterDeleteCraftRecords(craftRecords);
		craftRecordsDao.flush();
		List<WOMCraftRecords> params = new ArrayList<WOMCraftRecords>();
		params.add(craftRecords);
		...
		if(signatureLog != null ) {
			...
			businessKey=craftRecords.getId();
			signatureLog.setTableId(craftRecords.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.common.delete",operType="3")
	public void deleteCraftRecords(String craftRecordsIds){
		this.deleteCraftRecords(craftRecordsIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.common.delete",operType="3")
	public void deleteCraftRecords(String craftRecordsIds,SignatureLog signatureLog){
		deleteCollection(craftRecordsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteCraftRecords(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_procFeedback#modelCode:WOM_7.5.0.0_procFeedback_CraftRecords#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="ec.engine.restore",operType="10")
	public void restoreCraftRecords(String craftRecordsIds){
		restoreCollection(craftRecordsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreCraftRecords(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="ec.engine.restore",operType="10")
	public void restoreCraftRecords(long craftRecordsId){
		
		findBusinessKeyUsed(craftRecordsId);	//判断业务主键是否重复
		...
		beforeRestoreCraftRecords(craftRecords);
		craftRecords.setValid(true);
		craftRecordsDao.update(craftRecords);
		afterRestoreCraftRecords(craftRecords);
		List<WOMCraftRecords> params = new ArrayList<WOMCraftRecords>();
		params.add(craftRecords);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long craftRecordsId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_procFeedback_CraftRecords");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMCraftRecords.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMCraftRecords.TABLE_NAME +" where id = ? )";
			List<Object> list =  craftRecordsDao.createNativeQuery(sql,craftRecordsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseCraftRecords(List<WOMCraftRecords>  craftRecordss){
		for(WOMCraftRecords craftRecords:craftRecordss)  {
			saveCraftRecords(craftRecords, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseCraftRecords(List<WOMCraftRecords>  craftRecordss){
		for(WOMCraftRecords craftRecords:craftRecordss)  {
			saveCraftRecords(craftRecords, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="ec.flow.save")
	public void saveCraftRecords(WOMCraftRecords craftRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveCraftRecords(craftRecords, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="ec.flow.save")
	public void saveCraftRecords(WOMCraftRecords craftRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveCraftRecords(craftRecords, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateCraftRecordsCodes(WOMCraftRecords craftRecords) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCraftRecordsCodes(craftRecords, false);
	}
	
	public void generateCraftRecordsCodes(WOMCraftRecords craftRecords, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateCraftRecordsSummarys(WOMCraftRecords craftRecords) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCraftRecordsSummarys(craftRecords, false);
	}

	public void generateCraftRecordsSummarys(WOMCraftRecords craftRecords, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords")
	public void saveCraftRecords(WOMCraftRecords craftRecords,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveCraftRecords(craftRecords, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords")
	public void saveCraftRecords(WOMCraftRecords craftRecords,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
		String url = null;
		if(craftRecords.getId() != null && craftRecords.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCraftRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, craftRecords);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(craftRecords);
		beforeSaveCraftRecords(craftRecords, viewIsView);

		if (viewIsView) {
			craftRecordsDao.saveWithRevertVersion(craftRecords);
		} else {
			if(isNew)
				craftRecordsDao.save(craftRecords);
			else
				craftRecordsDao.merge(craftRecords);
		}
		if(null != craftRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCraftRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
			AuditUtil.setColumnStringA(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
		...
		dealDatagridsSave(craftRecords,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateCraftRecordsCodes(craftRecords, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateCraftRecordsSummarys(craftRecords, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveCraftRecords(craftRecords, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, craftRecords);
			}
		}
		List<WOMCraftRecords> params = new ArrayList<WOMCraftRecords>();
		craftRecordsDao.flush();
		craftRecordsDao.clear();
		craftRecords = craftRecordsDao.load(craftRecords.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMCraftRecords craftRecords){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("craftRecords", craftRecords);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords")
	public void mergeCraftRecords(WOMCraftRecords craftRecords, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(craftRecords);
		beforeSaveCraftRecords(craftRecords);
		craftRecordsDao.merge(craftRecords);
		if(null != craftRecords && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCraftRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
			AuditUtil.setColumnStringA(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
		...
		dealDatagridsSave(craftRecords,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveCraftRecords(craftRecords);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMCraftRecords> findCraftRecordss(Page<WOMCraftRecords> page, Criterion... criterions) {
		return craftRecordsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCraftRecordss(Page<WOMCraftRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findCraftRecordss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCraftRecordss(Page<WOMCraftRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCraftRecordss(Page<WOMCraftRecords> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMCraftRecords> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"craftRecords\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"craftRecords\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"craftRecords\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"craftRecords\".HEADER IN ( SELECT ID FROM WOM_PROC_FEEDBACK_HEADS \"procFeedbackHead\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  craftRecordsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  craftRecordsDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_procFeedback").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"craftRecords\".LAY_REC ASC, \"craftRecords\".SORT ASC");
				} else {
					orderPart.append(" \"craftRecords\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) craftRecordsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=craftRecordsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = craftRecordsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMCraftRecords> result = new ArrayList<WOMCraftRecords>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMCraftRecords.class,craftRecordsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMCraftRecords.class,craftRecordsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMCraftRecords craftRecords : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(craftRecords.getId(), "WOM_procFeedback_craftRecords");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(craftRecords.getId(), "WOM_procFeedback_craftRecords");
							if(null != documents && !documents.isEmpty()) {
								craftRecords.setDocument(documents.get(0));
								craftRecords.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMCraftRecords> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMCraftRecords> result) {
		View view = null;
		...
		Map<Long, WOMCraftRecords> rsMap = new HashMap<Long, WOMCraftRecords>();
		for (WOMCraftRecords craftRecords : result) {
			ids.add(craftRecords.getId());
			rsMap.put(craftRecords.getId(), craftRecords);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(craftRecordsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = craftRecordsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_procFeedback_CraftRecords" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_procFeedback_CraftRecords" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  craftRecordsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"craftRecords\"","WOM_7.5.0.0_procFeedback","","WOM_CRAFT_RECORDS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"craftRecords\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"craftRecords\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"craftRecords\"","WOM_7.5.0.0_procFeedback","","WOM_CRAFT_RECORDS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_procFeedback_CraftRecords" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("craftRecords.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"craftRecords\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"craftRecords\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"craftRecords\".CID = 1 OR \"craftRecords\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("craftRecords.mc\"").append(".CRAFT_RECORDS) not in ("+notinValue+")");
			}
			
			SQLQuery query = craftRecordsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=craftRecordsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMCraftRecords.ENTITY_CODE;}
	protected String getModuleCode(){return WOMCraftRecords.MODULE_CODE;}
	protected String getModelCode(){return WOMCraftRecords.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param craftRecords 模型实例
	 */
	public void checkUniqueConstraint(WOMCraftRecords craftRecords){
	}
	
	
	@Override
	public List<WOMCraftRecords> getCraftRecordss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMCraftRecords.class);
		if(params!=null){
			...
			return craftRecordsDao.findByHql("from " + WOMCraftRecords.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return craftRecordsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMCraftRecords> findCraftRecordssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMCraftRecords.class);
	...
	@Override
	public List<WOMCraftRecords> findCraftRecordssByHql(String hql, Object... objects){
		
		return   craftRecordsDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMCraftRecords loadCraftRecordsByBussinessKey(WOMCraftRecords craftRecords){
		return loadCraftRecordsByBussinessKey(craftRecords.getId());
	}
	
	@Override
	public Page<WOMCraftRecords> getCraftRecordss(Page<WOMCraftRecords> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMCraftRecords.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = craftRecordsDao.findByPage(page, sql);
			}else{
				page = craftRecordsDao.findAllByPage(page);
			}
			return page;
		}
		page = craftRecordsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMCraftRecords> findByTreeDataGridProcFeedbackHead(List<WOMCraftRecords> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMCraftRecords craftRecords : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProcFeedbackHead(Page<WOMCraftRecords> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMCraftRecords> craftRecordss = dgPage.getResult();
			for(WOMCraftRecords craftRecords : craftRecordss){
				
			}
			dgPage.setResult(craftRecordss);
		}
	...
	@Override
	@Transactional
	public void deleteCraftRecordsByCraftStandards(List<Long> craftStandardsIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("standardCraftList", craftStandardsIds);
		String hql = "select id from WOMCraftRecords where "
						+ "standardCraft.id in (:standardCraftList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> craftRecordsIds1 = craftRecordsDao.findByHql(hql, params);
		if(craftRecordsIds1 != null && !craftRecordsIds1.isEmpty()) {
		}
		hql = "delete WOMCraftRecords where "
						+ "standardCraft.id in (:standardCraftList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		craftRecordsDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMCraftRecords loadCraftRecordsByBussinessKey(Serializable bussinessKey){
		return craftRecordsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMCraftRecords> findByProperty(String propertyName, Object object){
		return craftRecordsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMCraftRecords findEntityByProperty(String propertyName, Object object){
		return craftRecordsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMCraftRecords.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = craftRecordsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_procFeedback_CraftRecords","runtime");
		try {
	...
	public void dealDatagridsSave(WOMCraftRecords craftRecords,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,craftRecordsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMCraftRecords.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, craftRecords);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",craftRecords.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMCraftRecords.TABLE_NAME +" where valid = 1";
			list = craftRecordsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMCraftRecords.TABLE_NAME;
			list = craftRecordsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMCraftRecords.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = craftRecordsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMCraftRecordsDaoImpl craftRecordsDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchCraftRecords(final List<WOMCraftRecords> insertObjs, final List<WOMCraftRecords> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchCraftRecords(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchCraftRecords(final List<WOMCraftRecords> insertObjs, final List<WOMCraftRecords> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1540952446403"),importCount));
		...
		Map<Long, WOMCraftRecords> oldUpdateObjsMap = new HashMap<Long, WOMCraftRecords>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMCraftRecords bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMCraftRecords> bizObjList = craftRecordsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMCraftRecords bizObj = bizObjList.get(mIndex);
		...
		final List<WOMCraftRecords> nmeObjs = new ArrayList<WOMCraftRecords>();
		...
		long id=autoGeneratorID.getNextId(WOMCraftRecords.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMCraftRecords craftRecords:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMCraftRecords.TABLE_NAME,1,dbId);
			if(craftRecords.getId() != null ){
				m.put(craftRecords.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			craftRecords.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) craftRecordsDao.createNativeQuery(sql, "WOM_7.5.0.0_procFeedback_CraftRecords"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		craftRecordsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMCraftRecords.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						WOMCraftRecords craftRecords = updateObjs.get(updateIndex);
						Long id = craftRecords.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = craftRecords.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(craftRecords);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMCraftRecords.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + WOMCraftRecords.TABLE_NAME + " order by id desc";
//					Query query = craftRecordsDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMCraftRecords craftRecords = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = craftRecords.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		craftRecordsDaoImpl.getSession().flush();
		craftRecordsDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMCraftRecords bizObj = insertObjs.get(insertIndex);
			craftRecordsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMCraftRecords bizObj = updateObjs.get(updateIndex);
			WOMCraftRecords oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			craftRecordsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMCraftRecords> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMCraftRecords> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) craftRecordsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMCraftRecords> result = getResult(page, query, new PendingResultTransformer(WOMCraftRecords.class,craftRecordsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMCraftRecords model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = craftRecordsDao.createNativeQuery(sql, "WOM_7.5.0.0_procFeedback_CraftRecords").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = craftRecordsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = craftRecordsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = craftRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?craftRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  craftRecordsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,craftRecordsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="ec.flow.save")
	public void saveCraftRecords(WOMCraftRecords craftRecords, DataGridService dataGridService){
		saveCraftRecords(craftRecords, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="ec.flow.save")
	public void saveCraftRecords(WOMCraftRecords craftRecords, DataGridService dataGridService, String viewCode){
		saveCraftRecords(craftRecords, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords")
	public void saveCraftRecords(WOMCraftRecords craftRecords, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveCraftRecords(craftRecords, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords")
	public void saveCraftRecords(WOMCraftRecords craftRecords, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_procFeedback";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
		String url = null;
		if(null != craftRecords.getId() && craftRecords.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMCraftRecords.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords")
	public Map<Object, Object> craftRecordsDataGridImport(WOMCraftRecords craftRecords, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = craftRecords.getVirtualId();
		
		craftRecords = craftRecordsDao.load(craftRecords.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, craftRecords.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = craftRecords.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(craftRecords);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, craftRecords.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords")
	public void mergeCraftRecords(WOMCraftRecords craftRecords, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(craftRecords);
		beforeSaveCraftRecords(craftRecords);
		craftRecordsDao.merge(craftRecords);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_procFeedback",model="WOM_7.5.0.0_procFeedback_CraftRecords",desc="foundation.batch.save",operType="4")
	public void batchSaveCraftRecords(WOMCraftRecords craftRecords, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_procFeedback";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_procFeedback");
	...
	@Override
	public Page<WOMCraftRecords> getByPage(Page<WOMCraftRecords> page,DetachedCriteria detachedCriteria){
		return craftRecordsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveCraftRecords(WOMCraftRecords craftRecords, Object... objects){
		checkUniqueConstraint(craftRecords);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
		if (craftRecords.getStandardID() != null && craftRecords.getHeader() != null) {
			craftRecords.setStandardCraft(craftStandardsService.getCraftStandards(craftRecords.getStandardID()));
			craftRecords.setHeader(procFeedbackHeadService.getProcFeedbackHead(craftRecords.getHeader().getId()));
		}
/* CUSTOM CODE END */
	}
	
	private void afterSaveCraftRecords(WOMCraftRecords craftRecords, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportCraftRecords(List<WOMCraftRecords> craftRecordss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportCraftRecords(List<WOMCraftRecords> craftRecordss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportCraftRecords(List<WOMCraftRecords> craftRecordss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportCraftRecords(List<WOMCraftRecords> craftRecordss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteCraftRecords(WOMCraftRecords craftRecords){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteCraftRecords(WOMCraftRecords craftRecords){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreCraftRecords(WOMCraftRecords craftRecords){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreCraftRecords(WOMCraftRecords craftRecords){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMCraftRecords craftRecords, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMCraftRecords craftRecords, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportCraftRecords(List<WOMCraftRecords> insertObjs, List<WOMCraftRecords> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportCraftRecords,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportCraftRecords(List<WOMCraftRecords> insertObjs, List<WOMCraftRecords> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportCraftRecords,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
