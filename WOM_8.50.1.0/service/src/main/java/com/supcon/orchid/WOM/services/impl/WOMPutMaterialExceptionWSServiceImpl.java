...
import com.supcon.orchid.WOM.services.WOMPutMaterialExceptionWSService;
import com.supcon.orchid.WOM.services.WOMPutMaterialExceptionService;
	
/* CUSTOM CODE START(wsserviceimpl,import,WOM_7.5.0.0_putInMaterialException_PutMaterialException,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="WOMPutMaterialException",targetNamespace = "http://ws.supcon.com")
public class WOMPutMaterialExceptionWSServiceImpl implements WOMPutMaterialExceptionWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getPutMaterialExceptionByBusinessKey(
		...
		try{
			...
			WOMPutMaterialException putMaterialException = putMaterialExceptionService.loadPutMaterialExceptionByBussinessKey(businessKey);
			if(putMaterialException == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, putMaterialException);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updatePutMaterialException(
		...
		try{
			if(putMaterialException == null){
				return set500Error(result, "投料异常处理单不能为空。");
			}
			...
			if(!beforeUpdatePutMaterialException(putMaterialException)){
				return set404Error(result);
			}
			putMaterialExceptionService.saveSuperEditPutMaterialException(putMaterialException, null, null, null, null, null, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addPutMaterialException(
		...
		try{
			...
			if(putMaterialException.getTableNo() == null){
				return set500Error(result, "业务主键不能为空。");
			}
			...
			try{
				checkPutMaterialExceptionData(putMaterialException);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			putMaterialExceptionService.saveSuperEditPutMaterialException(putMaterialException, null, null, null);
			setSuccessResult(result, putMaterialException.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deletePutMaterialExceptionByBusinessKey(
		...
		try{
			...
			try{
				putMaterialException = putMaterialExceptionService.loadPutMaterialExceptionByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddPutMaterialException(
		...
		try{
			...
			int count = 1;
			for(WOMPutMaterialException putMaterialException : putMaterialExceptionList){
				if(putMaterialException.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(putMaterialException.getTableNo() == null || putMaterialException.getTableNo().toString().trim().isEmpty()){
			...
			putMaterialExceptionService.batchSaveSuperEditPutMaterialException(putMaterialExceptionList, null, null, null);
			for(WOMPutMaterialException putMaterialException : putMaterialExceptionList){
				idList.add(putMaterialException.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdatePutMaterialException(
		...
		try{
			...
			putMaterialExceptionService.batchSaveSuperEditPutMaterialException(putMaterialExceptionList, null, null, null,null, null);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeletePutMaterialExceptionByPk(
		...
		try{
			...
			for(String k : pk){
				...
				WOMPutMaterialException temp = null;
				try{
					temp = putMaterialExceptionService.getPutMaterialException(id);
				}catch(NullPointerException e){}
			...
			putMaterialExceptionService.deletePutMaterialException(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeletePutMaterialExceptionByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				WOMPutMaterialException temp = null;
				try{
					temp = putMaterialExceptionService.loadPutMaterialExceptionByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getPutMaterialExceptionPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(WOMPutMaterialException.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),WOMPutMaterialException.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,WOMPutMaterialException.class);
				} catch (Exception e) {
			...
			Page<WOMPutMaterialException> page = new Page<WOMPutMaterialException>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<WOMPutMaterialException> datas = putMaterialExceptionService.getByPage(page, detachedCriteria);
			List<WOMPutMaterialException> putMaterialExceptionList = datas.getResult();
			if (putMaterialExceptionList == null || putMaterialExceptionList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(putMaterialExceptionList.toArray());
			PaginationDTO<WOMPutMaterialException> pagination = new PaginationDTO<WOMPutMaterialException>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByBusinessKey(
		...
		try {
			...
			clearCircle(putMaterialException);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			WOMPutMaterialException putMaterialException = putMaterialExceptionService.getPutMaterialException(id);
			if (putMaterialException == null || !putMaterialException.isValid()) {
				result.setErrorMessage("找不到投料异常处理单");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取投料异常处理单对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param putMaterialException
	 * @return
	 */
	private WOMPutMaterialException getPutMaterialException(WOMPutMaterialException putMaterialException) {
		if (putMaterialException.getId() == null && (putMaterialException.getTableNo() == null || putMaterialException.getTableNo().toString().trim() == ""))
			throw new RuntimeException("投料异常处理单对象业务主键和id不能同时为空");
		if (putMaterialException.getId() != null) {
			WOMPutMaterialException result = putMaterialExceptionService.getPutMaterialException(putMaterialException .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return putMaterialExceptionService.loadPutMaterialExceptionByBussinessKey(putMaterialException.getTableNo());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个投料异常处理单，则返回false，否则返回true
	 * @param putMaterialException
	 */
	private boolean beforeUpdatePutMaterialException(WOMPutMaterialException putMaterialException) {
		putMaterialException.setStatus(99);
		if(putMaterialException.getTableInfoId() == null){
			throw new RuntimeException("tableInfoId不能为空。");
		}
		WOMPutMaterialException temp = getPutMaterialException(putMaterialException);
		if (temp == null) {
			return false;
		}
		if(putMaterialException.getId() == null){
			putMaterialException.setId(temp.getId());		
		}
		if(putMaterialException.getTableNo() == null){
			putMaterialException.setTableNo(temp.getTableNo());
		}
		
		putMaterialException.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除投料异常处理单对象下所有辅模型对象中的投料异常处理单对象
	 * @param putMaterialException
	 */
	private void clearCircle(WOMPutMaterialException putMaterialException) {
	 ...
	 */
	private void checkPutMaterialExceptionData(WOMPutMaterialException putMaterialException){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,WOM_7.5.0.0_putInMaterialException_PutMaterialException,WOM_7.5.0.0) */
// 自定义代码
