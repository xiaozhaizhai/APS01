...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMTaskReportingPartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMTaskReportingPart> {
	...
	private WOMTaskReportingPart taskReportingPart;
	...
	//导出excel时,主模型内容
	List<WOMTaskReportingPart> queryResults = new ArrayList<WOMTaskReportingPart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/taskReporting/taskReportingPart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/taskReporting/taskReportingPart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_taskReporting_TaskReportingPart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMTaskReportingPart> taskReportingParts;
	
	public List<WOMTaskReportingPart> getTaskReportingParts() {
		return taskReportingParts;
	}
	...
	private Page<WOMTaskReportingPart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			taskReportingPart = new WOMTaskReportingPart();
			if(taskReportingPart.getReportType() != null) {
				taskReportingPart.setReportType((SystemCode) systemCodeService.getSystemCode(taskReportingPart.getReportType().getId()));
			}
		} else {
			if(id > 0){
				taskReportingPart = taskReportingPartService.getTaskReportingPart(id, viewCode);
			}
		}
	...
	public Page<WOMTaskReportingPart> getPage() {
		if (null == page) {
			page = new Page<WOMTaskReportingPart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMTaskReportingPart getTaskReportingPart() {
		return taskReportingPart;
	}		

	public void setTaskReportingPart(WOMTaskReportingPart taskReportingPart) {
		this.taskReportingPart = taskReportingPart;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/taskReporting/taskReportingPart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = taskReportingPartService.getTaskReportingPartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/taskReporting/taskReportingPart/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1490773102678
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String taskReportingPartReferenceCopy() throws Exception{
			WOMTaskReportingPart oldTaskReportingPart = null;
			if(isEditNew()){
				...
				oldTaskReportingPart = new WOMTaskReportingPart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskReportingPart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, taskReportingPart.getId());
				...
				oldTaskReportingPart = taskReportingPartService.getTaskReportingPart(id);
			}
			
			
			taskReportingPart = taskReportingPartService.getTaskReportingPart(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_taskReporting_TaskReportingPart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMTaskReportingPart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTaskReportingPart, null);
						Method setValue = WOMTaskReportingPart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(taskReportingPart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			taskReportingPart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=taskReportingPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return taskReportingPartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			taskReportingPart = taskReportingPartService.getTaskReportingPart(this.taskReportingPart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskReportingPart.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			taskReportingPartService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReportingPart/reportDetaliList", results = { @Result(type = FREEMARKER, location = "reportDetaliList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = taskReportingPartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(taskReportingPartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(taskReportingPartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_taskReporting_TaskReportingPart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReportingPart/reportDetaliRef", results = { @Result(type = FREEMARKER, location = "reportDetaliRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReportingPart/reportDetaliList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headId.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.proBatchNum,result.reportNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.wareId.wareCode,result.wareId.wareName,result.storeID.placeSetCode,result.storeID.placeSetName,result.batchNum,result.remark,result.headId.id,result.taskID.id,result.productID.id,result.reportStaff.id,result.wareId.id,result.storeID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="reportDetaliList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"taskReportingPart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"taskReportingPart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"taskReportingPart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"taskReportingPart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			taskReportingPartService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReportingPart/reportDetaliList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/taskReporting/taskReportingPart/reportDetaliRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.headId.tableNo,result.taskID.tableNo,result.productID.productCode,result.productID.productName,result.proBatchNum,result.headId.inputCode,result.content,result.reportNum,result.reportStaff.name,result.reportDate,result.reportType.id,result.reportType.value,result.wareId.wareCode,result.wareId.wareName,result.batchNum,result.remark,result.headId.id,result.taskID.id,result.productID.id,result.reportStaff.id,result.wareId.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="reportDetaliRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		taskReportingPartService.findDataGridPage(dg,dgclass,getDgPage(),taskReportingPart,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(taskReportingPart.getId() != null && taskReportingPart.getId() != -1){
				taskReportingPart = taskReportingPartService.getTaskReportingPart(taskReportingPart.getId());
			}
			taskReportingPartService.findDataGridPage(dg,dgclass,getDgPage(),taskReportingPart,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == taskReportingPart.getId()) {
		...
		taskReportingPartService.saveTaskReportingPart(taskReportingPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "submit");
		bap_validate_datagrids=taskReportingPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", taskReportingPart.getId());
		...
		fileuploads.put("linkId", taskReportingPart.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/taskReporting/taskReportingPart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		taskReportingPartService.deleteTaskReportingPart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/taskReporting/taskReportingPart/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "restore");
		taskReportingPartService.restoreTaskReportingPart(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMTaskReportingPart> findExportDataInfos(Page<WOMTaskReportingPart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = taskReportingPartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = taskReportingPartService.getAssProperty(p.getCode());	
									//String assModelCode = taskReportingPartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			taskReportingPartService.findTaskReportingParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			taskReportingPartService.findTaskReportingParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMTaskReportingPart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMTaskReportingPart result : results){
	...
	public boolean isEditNew(){
		//return null == this.taskReportingPart || null == this.taskReportingPart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/taskReporting/taskReportingPart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/taskReporting/taskReportingPart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/taskReporting/taskReportingPart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/taskReporting/taskReportingPart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMTaskReportingPartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/taskReporting/taskReportingPart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_taskReporting_TaskReportingPart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_taskReporting_TaskReportingPart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/taskReporting/taskReportingPart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/taskReporting/taskReportingPart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_taskReporting_TaskReportingPart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_taskReporting_TaskReportingPart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/taskReporting/taskReportingPart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
		_auditLog.setColumnStringA(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
		_auditLog.setColumnStringC(taskReportingPart.getId().toString());
		_auditLog.setColumnStringD(WOMTaskReportingPart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMTaskReportingPartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, taskReportingPart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, taskReportingPart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,taskReportingPart);
		...
		printlog.setBusinessId(taskReportingPart.getId());
		printlog.setBussinessKey(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
		printlog.setMainDisplay(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/taskReporting/taskReportingPart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(taskReportingPart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/taskReporting/taskReportingPart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(taskReportingPartService.getTaskReportingPart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/taskReporting/taskReportingPart/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/taskReporting/taskReportingPart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, taskReportingPart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				taskReportingPart = taskReportingPartService.getTaskReportingPart(Long.parseLong(arrId[i]));
				if(null != taskReportingPart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == taskReportingPart.getId() ? "" : "," + taskReportingPart.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == taskReportingPart.getId() ? "" : "," + taskReportingPart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == taskReportingPart.getId() ? "" : "," + taskReportingPart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == taskReportingPart.getId() ? "" : taskReportingPart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, taskReportingPart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,taskReportingPart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//taskReportingPartService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/taskReporting/taskReportingPart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_taskReporting_TaskReportingPart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_taskReporting_TaskReportingPart")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_taskReporting_TaskReportingPart");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_taskReporting_TaskReportingPart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = taskReportingPartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_taskReporting_TaskReportingPart,WOM_7.5.0.0) */
// 自定义代码
