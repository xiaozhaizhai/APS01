...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMInventoryDetailAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMInventoryDetail> {
	...
	private WOMInventoryDetail inventoryDetail;
	...
	//导出excel时,主模型内容
	List<WOMInventoryDetail> queryResults = new ArrayList<WOMInventoryDetail>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/inventoryRecords/inventoryDetail/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/inventoryRecords/inventoryDetail/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_inventoryRecords_InventoryDetail" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMInventoryDetail> inventoryDetails;
	
	public List<WOMInventoryDetail> getInventoryDetails() {
	...
	private Page<WOMInventoryDetail> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			inventoryDetail = new WOMInventoryDetail();
		} else {
			if(id > 0){
				inventoryDetail = inventoryDetailService.getInventoryDetail(id, viewCode);
			}
		}
	...
	public Page<WOMInventoryDetail> getPage() {
		if (null == page) {
			page = new Page<WOMInventoryDetail>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMInventoryDetail getInventoryDetail() {
		return inventoryDetail;
	}		

	public void setInventoryDetail(WOMInventoryDetail inventoryDetail) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = inventoryDetailService.getInventoryDetailAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1566888284025
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String inventoryDetailReferenceCopy() throws Exception{
			WOMInventoryDetail oldInventoryDetail = null;
			if(isEditNew()){
				...
				oldInventoryDetail = new WOMInventoryDetail();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,inventoryDetail.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, inventoryDetail.getId());
				...
				oldInventoryDetail = inventoryDetailService.getInventoryDetail(id);
			}
			
			
			inventoryDetail = inventoryDetailService.getInventoryDetail(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_inventoryRecords_InventoryDetail");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMInventoryDetail.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldInventoryDetail, null);
						Method setValue = WOMInventoryDetail.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(inventoryDetail, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			inventoryDetail.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=inventoryDetailService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return inventoryDetailReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			inventoryDetail = inventoryDetailService.getInventoryDetail(this.inventoryDetail.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,inventoryDetail.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			inventoryDetailService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/inventoryRecords/inventoryDetail/itemList", results = { @Result(type = FREEMARKER, location = "itemList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = inventoryDetailService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(inventoryDetailService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(inventoryDetailService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_inventoryRecords_InventoryDetail", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/inventoryRecords/inventoryDetail/itemList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.manulOrder.manulOrderNum,result.material.productName,result.material.productCode,result.batchText,result.wareID.wareName,result.onhand,result.material.storeUnit.name,result.manulOrder.id,result.material.id,result.wareID.id,result.material.storeUnit.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"inventoryDetail\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"inventoryDetail\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"inventoryDetail\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"inventoryDetail\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			inventoryDetailService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/inventoryRecords/inventoryDetail/itemList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		inventoryDetailService.findDataGridPage(dg,dgclass,getDgPage(),inventoryDetail,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(inventoryDetail.getId() != null && inventoryDetail.getId() != -1){
				inventoryDetail = inventoryDetailService.getInventoryDetail(inventoryDetail.getId());
			}
			inventoryDetailService.findDataGridPage(dg,dgclass,getDgPage(),inventoryDetail,searchCondition,params);
	...
	private void save() throws Exception {
		...
		inventoryDetailService.saveInventoryDetail(inventoryDetail,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "submit");
		bap_validate_datagrids=inventoryDetailService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", inventoryDetail.getId());
		...
		fileuploads.put("linkId", inventoryDetail.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/inventoryRecords/inventoryDetail/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		inventoryDetailService.deleteInventoryDetail(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "restore");
		inventoryDetailService.restoreInventoryDetail(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMInventoryDetail> findExportDataInfos(Page<WOMInventoryDetail> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = inventoryDetailService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = inventoryDetailService.getAssProperty(p.getCode());	
									//String assModelCode = inventoryDetailService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			inventoryDetailService.findInventoryDetails(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			inventoryDetailService.findInventoryDetails(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMInventoryDetail> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMInventoryDetail result : results){
	...
	public boolean isEditNew(){
		//return null == this.inventoryDetail || null == this.inventoryDetail.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/inventoryRecords/inventoryDetail/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/inventoryRecords/inventoryDetail/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMInventoryDetailService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMInventoryDetailService"
						...
						, "com.supcon.orchid.WOM.services.WOMInventoryRecordService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_inventoryRecords_InventoryDetail", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_inventoryRecords_InventoryDetail", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_inventoryRecords_InventoryDetail";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryDetail", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMInventoryDetail inventoryDetail = null;
		if(null != id) {
			inventoryDetail = inventoryDetailService.getInventoryDetail(id);
		}
		...
		_auditLog.setColumnStringB(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
		_auditLog.setColumnStringA(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
		_auditLog.setColumnStringC(inventoryDetail.getId().toString());
		_auditLog.setColumnStringD(WOMInventoryDetail.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMInventoryDetailAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, inventoryDetail,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, inventoryDetail,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,inventoryDetail);
		...
		printlog.setBusinessId(inventoryDetail.getId());
		printlog.setBussinessKey(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
		printlog.setMainDisplay(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(inventoryDetail,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(inventoryDetailService.getInventoryDetail(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, inventoryDetail,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				inventoryDetail = inventoryDetailService.getInventoryDetail(Long.parseLong(arrId[i]));
				if(null != inventoryDetail){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == inventoryDetail.getId() ? "" : "," + inventoryDetail.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == inventoryDetail.getId() ? "" : "," + inventoryDetail.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == inventoryDetail.getId() ? "" : "," + inventoryDetail.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == inventoryDetail.getId() ? "" : inventoryDetail.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, inventoryDetail,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,inventoryDetail);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//inventoryDetailService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/inventoryRecords/inventoryDetail/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_inventoryRecords_InventoryDetail");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_inventoryRecords_InventoryDetail")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_inventoryRecords_InventoryDetail");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_inventoryRecords_InventoryDetail");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = inventoryDetailService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_inventoryRecords_InventoryDetail,WOM_7.5.0.0) */
// 自定义代码
