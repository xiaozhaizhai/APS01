...
import com.supcon.orchid.WOM.services.WOMProductNumIndexImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMProductNumIndexDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_productNumIndexService")
@Transactional
public class WOMProductNumIndexServiceImpl  extends BaseServiceImpl<WOMProductNumIndex> implements WOMProductNumIndexService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMProductNumIndex> bapBaseService;
	...
	@Autowired
	private WOMProductNumIndexImportService productNumIndexImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMProductNumIndex.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  productNumIndexDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMProductNumIndex.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = productNumIndexDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProductNumIndex.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProductNumIndex.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  productNumIndexDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMProductNumIndex.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMProductNumIndex.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMProductNumIndex> list = (List<WOMProductNumIndex>) productNumIndexDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProductNumIndex getProductNumIndex(long id){
		return getProductNumIndex(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProductNumIndex getProductNumIndex(long id, String viewCode){
		WOMProductNumIndex productNumIndex = productNumIndexDao.load(id);

		// 一对多情况处理
		if(productNumIndex != null){
		}
		return productNumIndex;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProductNumIndexAsJSON(long id, String include){
		WOMProductNumIndex productNumIndex = productNumIndexDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(productNumIndex, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.common.delete",operType="3")
	public void deleteProductNumIndex(WOMProductNumIndex productNumIndex){
		if(null != productNumIndex && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProductNumIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
			AuditUtil.setColumnStringA(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
			AuditUtil.setColumnStringC(productNumIndex.getId().toString());
		}
		beforeDeleteProductNumIndex(productNumIndex);
		productNumIndexDao.deletePhysical(productNumIndex);
		afterDeleteProductNumIndex(productNumIndex);
		productNumIndexDao.flush();
		List<WOMProductNumIndex> params = new ArrayList<WOMProductNumIndex>();
		params.add(productNumIndex);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.common.delete",operType="3")
	public void deleteProductNumIndex(List<Long> productNumIndexIds){
		deleteProductNumIndex(productNumIndexIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.common.delete",operType="3")
	public void deleteProductNumIndex(List<Long> productNumIndexIds, String eventTopic) {
		List<WOMProductNumIndex> productNumIndexs = new ArrayList<WOMProductNumIndex>();
		for(Long productNumIndexId : productNumIndexIds){
			WOMProductNumIndex productNumIndex = getProductNumIndex(productNumIndexId);
			productNumIndexs.add(productNumIndex);
			if(null != productNumIndex && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProductNumIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == productNumIndex.getId() ? "" : "," + productNumIndex.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == productNumIndex.getId() ? "" : "," + productNumIndex.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == productNumIndex.getId() ? "" : "," + productNumIndex.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
				}
			}
		}
		
		for(WOMProductNumIndex productNumIndex : productNumIndexs){
			beforeDeleteProductNumIndex(productNumIndex);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(productNumIndexs != null && productNumIndexs.size() > 0 ){
			for(WOMProductNumIndex productNumIndex : productNumIndexs){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMProductNumIndex.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(productNumIndex));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),productNumIndex, productNumIndex.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(productNumIndexIds != null && productNumIndexIds.size() > 0) {
			String hql = "delete " + WOMProductNumIndex.JPA_NAME + " where id in(:ids)";
			Query query = productNumIndexDao.createQuery(hql);
			query.setParameterList("ids", productNumIndexIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMProductNumIndex productNumIndexz : productNumIndexs){
				productNumIndexz.setValid(false);
				List<WOMProductNumIndex> params = new ArrayList<WOMProductNumIndex>();
				params.add(productNumIndexz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.common.delete",operType="3")
	public void deleteProductNumIndex(long productNumIndexId, int productNumIndexVersion){
		this.deleteProductNumIndex(productNumIndexId, productNumIndexVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.common.delete",operType="3")
	public void deleteProductNumIndex(long productNumIndexId, int productNumIndexVersion,SignatureLog signatureLog){
		WOMProductNumIndex productNumIndex = getProductNumIndex(productNumIndexId);
		if(productNumIndex != null && !productNumIndex.isValid()){
		...
		beforeDeleteProductNumIndex(productNumIndex);
		productNumIndexDao.delete(productNumIndexId, productNumIndexVersion);
		afterDeleteProductNumIndex(productNumIndex);
		productNumIndexDao.flush();
		List<WOMProductNumIndex> params = new ArrayList<WOMProductNumIndex>();
		params.add(productNumIndex);
		...
		if(signatureLog != null ) {
			...
			businessKey=productNumIndex.getId();
			signatureLog.setTableId(productNumIndex.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.common.delete",operType="3")
	public void deleteProductNumIndex(String productNumIndexIds){
		this.deleteProductNumIndex(productNumIndexIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.common.delete",operType="3")
	public void deleteProductNumIndex(String productNumIndexIds,SignatureLog signatureLog){
		deleteCollection(productNumIndexIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProductNumIndex(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_ProductNumIndex#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="ec.engine.restore",operType="10")
	public void restoreProductNumIndex(String productNumIndexIds){
		restoreCollection(productNumIndexIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProductNumIndex(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="ec.engine.restore",operType="10")
	public void restoreProductNumIndex(long productNumIndexId){
		
		findBusinessKeyUsed(productNumIndexId);	//判断业务主键是否重复
		...
		beforeRestoreProductNumIndex(productNumIndex);
		productNumIndex.setValid(true);
		productNumIndexDao.update(productNumIndex);
		afterRestoreProductNumIndex(productNumIndex);
		List<WOMProductNumIndex> params = new ArrayList<WOMProductNumIndex>();
		params.add(productNumIndex);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long productNumIndexId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_ProductNumIndex");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMProductNumIndex.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMProductNumIndex.TABLE_NAME +" where id = ? )";
			List<Object> list =  productNumIndexDao.createNativeQuery(sql,productNumIndexId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProductNumIndex(List<WOMProductNumIndex>  productNumIndexs){
		for(WOMProductNumIndex productNumIndex:productNumIndexs)  {
			saveProductNumIndex(productNumIndex, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProductNumIndex(List<WOMProductNumIndex>  productNumIndexs){
		for(WOMProductNumIndex productNumIndex:productNumIndexs)  {
			saveProductNumIndex(productNumIndex, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="ec.flow.save")
	public void saveProductNumIndex(WOMProductNumIndex productNumIndex, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProductNumIndex(productNumIndex, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="ec.flow.save")
	public void saveProductNumIndex(WOMProductNumIndex productNumIndex, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProductNumIndex(productNumIndex, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProductNumIndexCodes(WOMProductNumIndex productNumIndex) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductNumIndexCodes(productNumIndex, false);
	}
	
	public void generateProductNumIndexCodes(WOMProductNumIndex productNumIndex, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProductNumIndexSummarys(WOMProductNumIndex productNumIndex) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductNumIndexSummarys(productNumIndex, false);
	}

	public void generateProductNumIndexSummarys(WOMProductNumIndex productNumIndex, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex")
	public void saveProductNumIndex(WOMProductNumIndex productNumIndex,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProductNumIndex(productNumIndex, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex")
	public void saveProductNumIndex(WOMProductNumIndex productNumIndex,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(productNumIndex.getId() != null && productNumIndex.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProductNumIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, productNumIndex);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(productNumIndex);
		beforeSaveProductNumIndex(productNumIndex, viewIsView);

		if (viewIsView) {
			productNumIndexDao.saveWithRevertVersion(productNumIndex);
		} else {
			if(isNew)
				productNumIndexDao.save(productNumIndex);
			else
				productNumIndexDao.merge(productNumIndex);
		}
		if(null != productNumIndex && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProductNumIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
			AuditUtil.setColumnStringA(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
		...
		dealDatagridsSave(productNumIndex,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProductNumIndexCodes(productNumIndex, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProductNumIndexSummarys(productNumIndex, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProductNumIndex(productNumIndex, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, productNumIndex);
			}
		}
		List<WOMProductNumIndex> params = new ArrayList<WOMProductNumIndex>();
		productNumIndexDao.flush();
		productNumIndexDao.clear();
		productNumIndex = productNumIndexDao.load(productNumIndex.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMProductNumIndex productNumIndex){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("productNumIndex", productNumIndex);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex")
	public void mergeProductNumIndex(WOMProductNumIndex productNumIndex, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(productNumIndex);
		beforeSaveProductNumIndex(productNumIndex);
		productNumIndexDao.merge(productNumIndex);
		if(null != productNumIndex && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProductNumIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
			AuditUtil.setColumnStringA(null == productNumIndex.getId() ? "" : productNumIndex.getId().toString());
		...
		dealDatagridsSave(productNumIndex,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProductNumIndex(productNumIndex);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMProductNumIndex> findProductNumIndexs(Page<WOMProductNumIndex> page, Criterion... criterions) {
		return productNumIndexDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductNumIndexs(Page<WOMProductNumIndex> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProductNumIndexs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductNumIndexs(Page<WOMProductNumIndex> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductNumIndexs(Page<WOMProductNumIndex> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMProductNumIndex> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"productNumIndex\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"productNumIndex\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"productNumIndex\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"productNumIndex\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  productNumIndexDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  productNumIndexDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"productNumIndex\".LAY_REC ASC, \"productNumIndex\".SORT ASC");
				} else {
					orderPart.append(" \"productNumIndex\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) productNumIndexDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=productNumIndexDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = productNumIndexDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMProductNumIndex> result = new ArrayList<WOMProductNumIndex>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMProductNumIndex.class,productNumIndexDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMProductNumIndex.class,productNumIndexDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMProductNumIndex productNumIndex : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(productNumIndex.getId(), "WOM_produceTask_productNumIndex");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(productNumIndex.getId(), "WOM_produceTask_productNumIndex");
							if(null != documents && !documents.isEmpty()) {
								productNumIndex.setDocument(documents.get(0));
								productNumIndex.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMProductNumIndex> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMProductNumIndex> result) {
		View view = null;
		...
		Map<Long, WOMProductNumIndex> rsMap = new HashMap<Long, WOMProductNumIndex>();
		for (WOMProductNumIndex productNumIndex : result) {
			ids.add(productNumIndex.getId());
			rsMap.put(productNumIndex.getId(), productNumIndex);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(productNumIndexDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = productNumIndexDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_ProductNumIndex" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_ProductNumIndex" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  productNumIndexDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productNumIndex\"","WOM_7.5.0.0_produceTask","","WOM_PRODUCT_NUM_INDICES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"productNumIndex\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"productNumIndex\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productNumIndex\"","WOM_7.5.0.0_produceTask","","WOM_PRODUCT_NUM_INDICES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_ProductNumIndex" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("productNumIndex.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"productNumIndex\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"productNumIndex\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"productNumIndex\".CID = 1 OR \"productNumIndex\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("productNumIndex.mc\"").append(".PRODUCT_NUM_INDEX) not in ("+notinValue+")");
			}
			
			SQLQuery query = productNumIndexDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=productNumIndexDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMProductNumIndex.ENTITY_CODE;}
	protected String getModuleCode(){return WOMProductNumIndex.MODULE_CODE;}
	protected String getModelCode(){return WOMProductNumIndex.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param productNumIndex 模型实例
	 */
	public void checkUniqueConstraint(WOMProductNumIndex productNumIndex){
	}
	
	
	@Override
	public List<WOMProductNumIndex> getProductNumIndexs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProductNumIndex.class);
		if(params!=null){
			...
			return productNumIndexDao.findByHql("from " + WOMProductNumIndex.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return productNumIndexDao.findByCriteria(detachedCriteria);
	}
	...
	public List<WOMProductNumIndex> findProductNumIndexsByHql(String hql, Object... objects){
		...
		return   productNumIndexDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMProductNumIndex loadProductNumIndexByBussinessKey(WOMProductNumIndex productNumIndex){
		return loadProductNumIndexByBussinessKey(productNumIndex.getId());
	}
	
	@Override
	public Page<WOMProductNumIndex> getProductNumIndexs(Page<WOMProductNumIndex> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProductNumIndex.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = productNumIndexDao.findByPage(page, sql);
			}else{
				page = productNumIndexDao.findAllByPage(page);
			}
			return page;
		}
		page = productNumIndexDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public WOMProductNumIndex loadProductNumIndexByBussinessKey(Serializable bussinessKey){
		return productNumIndexDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProductNumIndex> findByProperty(String propertyName, Object object){
		return productNumIndexDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProductNumIndex findEntityByProperty(String propertyName, Object object){
		return productNumIndexDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMProductNumIndex.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = productNumIndexDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_ProductNumIndex","runtime");
		try {
	...
	public void dealDatagridsSave(WOMProductNumIndex productNumIndex,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,productNumIndexDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMProductNumIndex.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, productNumIndex);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(productNumIndex.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(productNumIndex.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(productNumIndex.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",productNumIndex.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProductNumIndex.TABLE_NAME +" where valid = 1";
			list = productNumIndexDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProductNumIndex.TABLE_NAME;
			list = productNumIndexDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMProductNumIndex.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = productNumIndexDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMProductNumIndexDaoImpl productNumIndexDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMProductNumIndex.TABLE_NAME + " where id = ?";
		String tableNo = productNumIndexDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchProductNumIndex(final List<WOMProductNumIndex> insertObjs, final List<WOMProductNumIndex> updateObjs, 
		...
		return this.importBatchProductNumIndex(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProductNumIndex(final List<WOMProductNumIndex> insertObjs, final List<WOMProductNumIndex> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1563257962732"),importCount));
		...
		Map<Long, WOMProductNumIndex> oldUpdateObjsMap = new HashMap<Long, WOMProductNumIndex>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMProductNumIndex bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMProductNumIndex> bizObjList = productNumIndexDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMProductNumIndex bizObj = bizObjList.get(mIndex);
		...
		final List<WOMProductNumIndex> nmeObjs = new ArrayList<WOMProductNumIndex>();
		...
		//String idSql = "select id from " + WOMProductNumIndex.TABLE_NAME + " order by id desc";
		//Query query = productNumIndexDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMProductNumIndex.JPA_NAME + " order by id desc";
		Query query =  productNumIndexDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMProductNumIndex.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMProductNumIndex productNumIndex:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMProductNumIndex.TABLE_NAME,1,dbId);
			if(productNumIndex.getId() != null ){
				m.put(productNumIndex.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			productNumIndex.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) productNumIndexDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProductNumIndex"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		productNumIndexDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMProductNumIndex.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMProductNumIndex productNumIndex = updateObjs.get(updateIndex);
						Long id = productNumIndex.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = productNumIndex.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(productNumIndex);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMProductNumIndex.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMProductNumIndex productNumIndex = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = productNumIndex.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		productNumIndexDaoImpl.getSession().flush();
		productNumIndexDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = productNumIndexDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_ProductNumIndex").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = productNumIndexDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_ProductNumIndex").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMProductNumIndex bizObj = insertObjs.get(insertIndex);
			productNumIndexImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMProductNumIndex bizObj = updateObjs.get(updateIndex);
			WOMProductNumIndex oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			productNumIndexImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMProductNumIndex> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMProductNumIndex> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) productNumIndexDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMProductNumIndex> result = getResult(page, query, new PendingResultTransformer(WOMProductNumIndex.class,productNumIndexDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMProductNumIndex model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = productNumIndexDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProductNumIndex").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = productNumIndexDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = productNumIndexDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = productNumIndexDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?productNumIndexDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  productNumIndexDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,productNumIndexDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="ec.flow.save")
	public void saveProductNumIndex(WOMProductNumIndex productNumIndex, DataGridService dataGridService){
		saveProductNumIndex(productNumIndex, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="ec.flow.save")
	public void saveProductNumIndex(WOMProductNumIndex productNumIndex, DataGridService dataGridService, String viewCode){
		saveProductNumIndex(productNumIndex, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex")
	public void saveProductNumIndex(WOMProductNumIndex productNumIndex, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProductNumIndex(productNumIndex, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex")
	public void saveProductNumIndex(WOMProductNumIndex productNumIndex, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != productNumIndex.getId() && productNumIndex.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProductNumIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex")
	public Map<Object, Object> productNumIndexDataGridImport(WOMProductNumIndex productNumIndex, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = productNumIndex.getVirtualId();
		
		productNumIndex = productNumIndexDao.load(productNumIndex.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, productNumIndex.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = productNumIndex.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(productNumIndex);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, productNumIndex.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex")
	public void mergeProductNumIndex(WOMProductNumIndex productNumIndex, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(productNumIndex);
		beforeSaveProductNumIndex(productNumIndex);
		productNumIndexDao.merge(productNumIndex);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProductNumIndex",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<WOMProductNumIndex> getByPage(Page<WOMProductNumIndex> page,DetachedCriteria detachedCriteria){
		return productNumIndexDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProductNumIndex(WOMProductNumIndex productNumIndex, Object... objects){
		checkUniqueConstraint(productNumIndex);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProductNumIndex(WOMProductNumIndex productNumIndex, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProductNumIndex(WOMProductNumIndex productNumIndex){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProductNumIndex(WOMProductNumIndex productNumIndex){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProductNumIndex(WOMProductNumIndex productNumIndex){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProductNumIndex(WOMProductNumIndex productNumIndex){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMProductNumIndex productNumIndex, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMProductNumIndex productNumIndex, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProductNumIndex(List<WOMProductNumIndex> insertObjs, List<WOMProductNumIndex> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProductNumIndex,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProductNumIndex(List<WOMProductNumIndex> insertObjs, List<WOMProductNumIndex> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProductNumIndex,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_ProductNumIndex,WOM_7.5.0.0) */
// 自定义代码
