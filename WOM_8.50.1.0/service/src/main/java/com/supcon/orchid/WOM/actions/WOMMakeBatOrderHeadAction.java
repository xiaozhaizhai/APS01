...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMMakeBatOrderHeadAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMMakeBatOrderHead> {
	...
	private WOMMakeBatOrderHead makeBatOrderHead;
	...
	//导出excel时,主模型内容
	List<WOMMakeBatOrderHead> queryResults = new ArrayList<WOMMakeBatOrderHead>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterialNeed/makeBatOrderHead/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchingMaterialNeed/makeBatOrderHead/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMMakeBatOrderHead> makeBatOrderHeads;
	
	public List<WOMMakeBatOrderHead> getMakeBatOrderHeads() {
		return makeBatOrderHeads;
	}
	...
	private Page<WOMMakeBatOrderHead> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			makeBatOrderHead = new WOMMakeBatOrderHead();
		} else {
			if(id > 0){
				makeBatOrderHead = makeBatOrderHeadService.getMakeBatOrderHead(id, viewCode);
			}
		}
	...
	public Page<WOMMakeBatOrderHead> getPage() {
		if (null == page) {
			page = new Page<WOMMakeBatOrderHead>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMMakeBatOrderHead getMakeBatOrderHead() {
		return makeBatOrderHead;
	}		

	public void setMakeBatOrderHead(WOMMakeBatOrderHead makeBatOrderHead) {
		this.makeBatOrderHead = makeBatOrderHead;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = makeBatOrderHeadService.getMakeBatOrderHeadAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1491799480124
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String makeBatOrderHeadReferenceCopy() throws Exception{
			WOMMakeBatOrderHead oldMakeBatOrderHead = null;
			if(isEditNew()){
				...
				oldMakeBatOrderHead = new WOMMakeBatOrderHead();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,makeBatOrderHead.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, makeBatOrderHead.getId());
				...
				oldMakeBatOrderHead = makeBatOrderHeadService.getMakeBatOrderHead(id);
			}
			
			
			makeBatOrderHead = makeBatOrderHeadService.getMakeBatOrderHead(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMMakeBatOrderHead.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMakeBatOrderHead, null);
						Method setValue = WOMMakeBatOrderHead.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(makeBatOrderHead, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			makeBatOrderHead.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/makeBatchingOrder",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "makeBatchingOrder.ftl") }),
		@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/makeBatchingOrderPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "makeBatchingOrderPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=makeBatOrderHeadService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return makeBatOrderHeadReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			makeBatOrderHead = makeBatOrderHeadService.getMakeBatOrderHead(this.makeBatOrderHead.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,makeBatOrderHead.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			makeBatOrderHeadService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = makeBatOrderHeadService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(makeBatOrderHeadService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(makeBatOrderHeadService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"makeBatOrderHead\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"makeBatOrderHead\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"makeBatOrderHead\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"makeBatOrderHead\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			makeBatOrderHeadService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		makeBatOrderHeadService.findDataGridPage(dg,dgclass,getDgPage(),makeBatOrderHead,"", null);
	...
	private Page<WOMMakeBatOrderPart> dg1491803986856Page;
	public Page<WOMMakeBatOrderPart> getDg1491803986856Page(){
		if(null == dg1491803986856Page){
			dg1491803986856Page = new Page<WOMMakeBatOrderPart>(1,20);
		}
		return dg1491803986856Page;
	}
	
	private Class dg1491803986856Clazz = WOMMakeBatOrderPart.class;
	
	public Class getDg1491803986856Clazz() {
		return dg1491803986856Clazz;
	}
	
	private List<WOMMakeBatOrderPart> dg1491803986856List;
	public List<WOMMakeBatOrderPart> getDg1491803986856List(){
		if(null == dg1491803986856List){
			dg1491803986856List = new ArrayList<WOMMakeBatOrderPart>();
		}
		return dg1491803986856List;
	}
	public void setDg1491803986856ListJson(String json) {
		//dg1491803986856List = (List<WOMMakeBatOrderPart>) makeBatOrderHeadService.generateObjectFromJson(json, WOMMakeBatOrderPart.class);
	}
	
	private List<Long> dg1491803986856DeletedIds;
	public List<Long> getDg1491803986856DeletedIds(){
		if(null == dg1491803986856DeletedIds){
			dg1491803986856DeletedIds = new ArrayList<Long>();
		}
		return dg1491803986856DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/data-dg1491803986856", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.needID.id,result.needID.productId.productCode,result.needID.productId.productName,result.num,result.WOM_7_5_0_0_batchingMaterialNeed_makeBatchingOrderdg1491803986856_DATAGRID_CUSTOM_8a031a7c_14ab_4fe1_9d97_0a8b34f52c29,result.remark,result.id,result.version,result.headID.id,result.needID.id" , "dataGridCode", "WOM_7.5.0.0_batchingMaterialNeed_makeBatchingOrderdg1491803986856" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1491803986856-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1491803986856.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(makeBatOrderHead.getId() != null && makeBatOrderHead.getId() != -1){
				makeBatOrderHead = makeBatOrderHeadService.getMakeBatOrderHead(makeBatOrderHead.getId());
			}
			makeBatOrderHeadService.findDataGridPage(dg,dgclass,getDgPage(),makeBatOrderHead,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == makeBatOrderHead.getId()) {
		...
		makeBatOrderHeadService.saveMakeBatOrderHead(makeBatOrderHead,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/makeBatchingOrder/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "submit");
		bap_validate_datagrids=makeBatOrderHeadService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", makeBatOrderHead.getId());
		...
		fileuploads.put("linkId", makeBatOrderHead.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		makeBatOrderHeadService.deleteMakeBatOrderHead(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "restore");
		makeBatOrderHeadService.restoreMakeBatOrderHead(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMMakeBatOrderHead> findExportDataInfos(Page<WOMMakeBatOrderHead> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = makeBatOrderHeadService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = makeBatOrderHeadService.getAssProperty(p.getCode());	
									//String assModelCode = makeBatOrderHeadService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			makeBatOrderHeadService.findMakeBatOrderHeads(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			makeBatOrderHeadService.findMakeBatOrderHeads(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMMakeBatOrderHead> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMMakeBatOrderHead result : results){
	...
	public boolean isEditNew(){
		//return null == this.makeBatOrderHead || null == this.makeBatOrderHead.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchingMaterialNeed/makeBatOrderHead/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchingMaterialNeed/makeBatOrderHead/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMMakeBatOrderHeadService"
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
		_auditLog.setColumnStringA(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
		_auditLog.setColumnStringC(makeBatOrderHead.getId().toString());
		_auditLog.setColumnStringD(WOMMakeBatOrderHead.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMMakeBatOrderHeadAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, makeBatOrderHead,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, makeBatOrderHead,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,makeBatOrderHead);
		...
		printlog.setBusinessId(makeBatOrderHead.getId());
		printlog.setBussinessKey(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
		printlog.setMainDisplay(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(makeBatOrderHead,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(makeBatOrderHeadService.getMakeBatOrderHead(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, makeBatOrderHead,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				makeBatOrderHead = makeBatOrderHeadService.getMakeBatOrderHead(Long.parseLong(arrId[i]));
				if(null != makeBatOrderHead){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == makeBatOrderHead.getId() ? "" : "," + makeBatOrderHead.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == makeBatOrderHead.getId() ? "" : "," + makeBatOrderHead.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == makeBatOrderHead.getId() ? "" : "," + makeBatOrderHead.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == makeBatOrderHead.getId() ? "" : makeBatOrderHead.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, makeBatOrderHead,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,makeBatOrderHead);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//makeBatOrderHeadService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchingMaterialNeed/makeBatOrderHead/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = makeBatOrderHeadService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchingMaterialNeed_MakeBatOrderHead,WOM_7.5.0.0) */
// 自定义代码
