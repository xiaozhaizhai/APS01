...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMDayBatchPlanInfoAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMDayBatchPlanInfo> {
	...
	private WOMDayBatchPlanInfo dayBatchPlanInfo;
	...
	//导出excel时,主模型内容
	List<WOMDayBatchPlanInfo> queryResults = new ArrayList<WOMDayBatchPlanInfo>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/producePlanRef/dayBatchPlanInfo/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/producePlanRef/dayBatchPlanInfo/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMDayBatchPlanInfo> dayBatchPlanInfos;
	
	public List<WOMDayBatchPlanInfo> getDayBatchPlanInfos() {
		return dayBatchPlanInfos;
	}
	...
	private Page<WOMDayBatchPlanInfo> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			dayBatchPlanInfo = new WOMDayBatchPlanInfo();
		} else {
			if(id > 0){
				dayBatchPlanInfo = dayBatchPlanInfoService.getDayBatchPlanInfo(id, viewCode);
				if(tableInfoId == null && dayBatchPlanInfo != null){
					tableInfoId = dayBatchPlanInfo.getTableInfoId();
				}
			}
		}
	...
	public Page<WOMDayBatchPlanInfo> getPage() {
		if (null == page) {
			page = new Page<WOMDayBatchPlanInfo>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMDayBatchPlanInfo getDayBatchPlanInfo() {
		return dayBatchPlanInfo;
	}		

	public void setDayBatchPlanInfo(WOMDayBatchPlanInfo dayBatchPlanInfo) {
		this.dayBatchPlanInfo = dayBatchPlanInfo;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = dayBatchPlanInfoService.getDayBatchPlanInfoAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String dayBatchPlanInfoReferenceCopy() throws Exception{
			WOMDayBatchPlanInfo oldDayBatchPlanInfo = null;
			if(isEditNew()){
				...
				oldDayBatchPlanInfo = new WOMDayBatchPlanInfo();
			}else{
			//WOMDayBatchPlanInfo dayBatchPlanInfo = dayBatchPlanInfoService.getDayBatchPlanInfo(this.dayBatchPlanInfo.getId());
			this.creatorInfo = creatorService.get(dayBatchPlanInfo.getCreateStaffId(), dayBatchPlanInfo.getCreatePositionId(), dayBatchPlanInfo.getCreateDepartmentId());
			//setDayBatchPlanInfo(dayBatchPlanInfo);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(dayBatchPlanInfo.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,dayBatchPlanInfo.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, dayBatchPlanInfo.getId());
				...
				oldDayBatchPlanInfo = dayBatchPlanInfoService.getDayBatchPlanInfo(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldDayBatchPlanInfo.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_producePlanRef_" + oldDayBatchPlanInfo.getId();
			...
			dayBatchPlanInfo = dayBatchPlanInfoService.getDayBatchPlanInfo(refId);
			dayBatchPlanInfo.setPending(pending);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMDayBatchPlanInfo.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldDayBatchPlanInfo, null);
						Method setValue = WOMDayBatchPlanInfo.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(dayBatchPlanInfo, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			dayBatchPlanInfo.setCreateDepartmentId(oldDayBatchPlanInfo.getCreateDepartmentId());
			dayBatchPlanInfo.setCreatePositionId(oldDayBatchPlanInfo.getCreatePositionId());
			dayBatchPlanInfo.setCreateStaffId(oldDayBatchPlanInfo.getCreateStaffId());
			dayBatchPlanInfo.setOwnerDepartmentId(oldDayBatchPlanInfo.getOwnerDepartmentId());
			dayBatchPlanInfo.setOwnerPositionId(oldDayBatchPlanInfo.getOwnerPositionId());
			dayBatchPlanInfo.setOwnerStaffId(oldDayBatchPlanInfo.getOwnerStaffId());
			dayBatchPlanInfo.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "referenceCopy");
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=dayBatchPlanInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return dayBatchPlanInfoReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			dayBatchPlanInfo = dayBatchPlanInfoService.getDayBatchPlanInfo(this.dayBatchPlanInfo.getId(), viewCode);
			this.creatorInfo = creatorService.get(dayBatchPlanInfo.getCreateStaffId(), dayBatchPlanInfo.getCreatePositionId(), dayBatchPlanInfo.getCreateDepartmentId());
			//setDayBatchPlanInfo(dayBatchPlanInfo);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = dayBatchPlanInfoService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = dayBatchPlanInfoService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + dayBatchPlanInfo.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_producePlanRef_" + dayBatchPlanInfo.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<WOMDayBatchPlanInfo> tmpList = new ArrayList<>();
			tmpList.add(dayBatchPlanInfo);
			dayBatchPlanInfoService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = dayBatchPlanInfoService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,dayBatchPlanInfo.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			dayBatchPlanInfoService.print(0);
		}
		return SUCCESS;
	}
	...
	public String viewList() throws Exception{
		...
		//mainViewPath = "";
		mainViewPath=dayBatchPlanInfoService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = dayBatchPlanInfoService.getWorkFlowInfo(workFlowMenuCode);
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = dayBatchPlanInfoService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(dayBatchPlanInfoService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(dayBatchPlanInfoService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/headPlanRef", results = { @Result(type = FREEMARKER, location = "headPlanRef.ftl") })
	})
	...
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"dayBatchPlanInfo\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"dayBatchPlanInfo\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"reportDeptID\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		if(getPage().isExportFlag()) {
			...
			dayBatchPlanInfoService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<WOMDayBatchPlanInfo> dayBatchPlanInfoList=page.getResult();
				for(WOMDayBatchPlanInfo lz:dayBatchPlanInfoList){
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"dayBatchPlanInfo\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"dayBatchPlanInfo\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/headPlanRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.reportDeptID.name,result.createTime,result.reportDeptID.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="headPlanRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"dayBatchPlanInfo\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		dayBatchPlanInfoService.findDataGridPage(dg,dgclass,getDgPage(),dayBatchPlanInfo,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(dayBatchPlanInfo.getId() != null && dayBatchPlanInfo.getId() != -1){
				dayBatchPlanInfo = dayBatchPlanInfoService.getDayBatchPlanInfo(dayBatchPlanInfo.getId());
			}
			dayBatchPlanInfoService.findDataGridPage(dg,dgclass,getDgPage(),dayBatchPlanInfo,searchCondition,params);
	...
	private void save() throws Exception {
		...
		dayBatchPlanInfoService.save(dayBatchPlanInfo,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(dayBatchPlanInfo.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + dayBatchPlanInfo.getProcessKey() + "_" + p.getActivityName() + "_WOM_7.5.0.0_producePlanRef_" + dayBatchPlanInfo.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(dayBatchPlanInfo.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		dayBatchPlanInfoService.retrialThisTable(dayBatchPlanInfo,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "retrial");
		return SUCCESS;
	}
	@Actions({
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "submit");
		bap_validate_datagrids=dayBatchPlanInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (dayBatchPlanInfo != null && dayBatchPlanInfo.getStatus() != null && dayBatchPlanInfo.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				dayBatchPlanInfoService.saveSuperEditDayBatchPlanInfo(dayBatchPlanInfo, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				dayBatchPlanInfoService.submit(dayBatchPlanInfo,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", dayBatchPlanInfo.getTableInfoId());
		responseMap.put("id", dayBatchPlanInfo.getId());
		...
		fileuploads.put("linkId", dayBatchPlanInfo.getTableInfoId());
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "bulkSubmit");
		dayBatchPlanInfoService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		dayBatchPlanInfoService.deleteDayBatchPlanInfo(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "restore");
		dayBatchPlanInfoService.restoreDayBatchPlanInfo(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "showDealInfoData");
		if (null != tableInfoId) {
			int count=dayBatchPlanInfoService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = dayBatchPlanInfoService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = dayBatchPlanInfoService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMDayBatchPlanInfo> findExportDataInfos(Page<WOMDayBatchPlanInfo> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = dayBatchPlanInfoService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = dayBatchPlanInfoService.getAssProperty(p.getCode());	
									//String assModelCode = dayBatchPlanInfoService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			dayBatchPlanInfoService.findDayBatchPlanInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			dayBatchPlanInfoService.findDayBatchPlanInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMDayBatchPlanInfo> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMDayBatchPlanInfo result : results){
	...
	public boolean isEditNew(){
		//return null == this.dayBatchPlanInfo || null == this.dayBatchPlanInfo.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && dayBatchPlanInfo != null && dayBatchPlanInfo.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + dayBatchPlanInfo.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return dayBatchPlanInfoService.isRecallAble(tableInfoId);
	}

	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = dayBatchPlanInfoService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + dayBatchPlanInfo.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + dayBatchPlanInfo.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/producePlanRef/dayBatchPlanInfo/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "batchApproval");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/producePlanRef/dayBatchPlanInfo/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/producePlanRef/dayBatchPlanInfo/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMDayBatchPlanInfoService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_producePlanRef", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_producePlanRef");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == dayBatchPlanInfo.getTableNo() ? "" : dayBatchPlanInfo.getTableNo().toString());
		_auditLog.setColumnStringA(null == dayBatchPlanInfo.getId() ? "" : dayBatchPlanInfo.getId().toString());
		_auditLog.setColumnStringC(dayBatchPlanInfo.getId().toString());
		_auditLog.setColumnStringD(WOMDayBatchPlanInfo.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_producePlanRef");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMDayBatchPlanInfoAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, dayBatchPlanInfo,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, dayBatchPlanInfo,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,dayBatchPlanInfo);
		...
		printlog.setEntity("WOM_7.5.0.0_producePlanRef");
		...
		printlog.setBusinessId(dayBatchPlanInfo.getId());
		printlog.setBussinessKey(null == dayBatchPlanInfo.getTableNo() ? "" : dayBatchPlanInfo.getTableNo().toString());
		printlog.setMainDisplay(null == dayBatchPlanInfo.getId() ? "" : dayBatchPlanInfo.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(dayBatchPlanInfo,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(dayBatchPlanInfoService.getDayBatchPlanInfo(Long.parseLong(arrId[i])));
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, dayBatchPlanInfo,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				dayBatchPlanInfo = dayBatchPlanInfoService.getDayBatchPlanInfo(Long.parseLong(arrId[i]));
				if(null != dayBatchPlanInfo){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == dayBatchPlanInfo.getId() ? "" : "," + dayBatchPlanInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == dayBatchPlanInfo.getId() ? "" : dayBatchPlanInfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == dayBatchPlanInfo.getTableNo() ? "" : "," + dayBatchPlanInfo.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == dayBatchPlanInfo.getTableNo() ? "" : dayBatchPlanInfo.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == dayBatchPlanInfo.getId() ? "" : "," + dayBatchPlanInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == dayBatchPlanInfo.getId() ? "" : dayBatchPlanInfo.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, dayBatchPlanInfo,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,dayBatchPlanInfo);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//dayBatchPlanInfoService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/producePlanRef/dayBatchPlanInfo/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMDayBatchPlanInfo> findExportPageInfos(Page<WOMDayBatchPlanInfo> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_producePlanRef" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = dayBatchPlanInfoService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMDayBatchPlanInfo queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = dayBatchPlanInfoService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = dayBatchPlanInfoService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = dayBatchPlanInfoService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo,WOM_7.5.0.0) */
// 自定义代码
