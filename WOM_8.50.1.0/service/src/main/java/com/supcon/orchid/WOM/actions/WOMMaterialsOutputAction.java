...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMMaterialsOutputAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMMaterialsOutput> {
	...
	private WOMMaterialsOutput materialsOutput;
	...
	//导出excel时,主模型内容
	List<WOMMaterialsOutput> queryResults = new ArrayList<WOMMaterialsOutput>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceRecords/materialsOutput/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/produceRecords/materialsOutput/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_produceRecords_MaterialsOutput" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMMaterialsOutput> materialsOutputs;
	
	public List<WOMMaterialsOutput> getMaterialsOutputs() {
	...
	private Page<WOMMaterialsOutput> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			materialsOutput = new WOMMaterialsOutput();
		} else {
			if(id > 0){
				materialsOutput = materialsOutputService.getMaterialsOutput(id, viewCode);
			}
		}
	...
	public Page<WOMMaterialsOutput> getPage() {
		if (null == page) {
			page = new Page<WOMMaterialsOutput>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMMaterialsOutput getMaterialsOutput() {
		return materialsOutput;
	}		

	public void setMaterialsOutput(WOMMaterialsOutput materialsOutput) {
		this.materialsOutput = materialsOutput;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceRecords/materialsOutput/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = materialsOutputService.getMaterialsOutputAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/produceRecords/materialsOutput/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String materialsOutputReferenceCopy() throws Exception{
			WOMMaterialsOutput oldMaterialsOutput = null;
			if(isEditNew()){
				...
				oldMaterialsOutput = new WOMMaterialsOutput();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,materialsOutput.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, materialsOutput.getId());
				...
				oldMaterialsOutput = materialsOutputService.getMaterialsOutput(id);
			}
			
			
			materialsOutput = materialsOutputService.getMaterialsOutput(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceRecords_MaterialsOutput");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMMaterialsOutput.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMaterialsOutput, null);
						Method setValue = WOMMaterialsOutput.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(materialsOutput, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			materialsOutput.setCreateDepartmentId(oldMaterialsOutput.getCreateDepartmentId());
			materialsOutput.setCreatePositionId(oldMaterialsOutput.getCreatePositionId());
			materialsOutput.setCreateStaffId(oldMaterialsOutput.getCreateStaffId());
			materialsOutput.setOwnerDepartmentId(oldMaterialsOutput.getOwnerDepartmentId());
			materialsOutput.setOwnerPositionId(oldMaterialsOutput.getOwnerPositionId());
			materialsOutput.setOwnerStaffId(oldMaterialsOutput.getOwnerStaffId());
			materialsOutput.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceRecords/materialsOutput/outputEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "outputEdit.ftl") }),
		@Action(value = "/WOM/produceRecords/materialsOutput/outputEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "outputEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=materialsOutputService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return materialsOutputReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			materialsOutput = materialsOutputService.getMaterialsOutput(this.materialsOutput.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,materialsOutput.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			materialsOutputService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceRecords/materialsOutput/outputView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "outputView.ftl") }),
		@Action(value = "/WOM/produceRecords/materialsOutput/outputViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "outputViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/produceRecords/materialsOutput/outputList", results = { @Result(type = FREEMARKER, location = "outputList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = materialsOutputService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(materialsOutputService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(materialsOutputService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_produceRecords_MaterialsOutput", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/produceRecords/materialsOutput/outputList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.activeID.activeId.name,result.batch,result.produceNum,result.activeID.name,result.activeID.activeId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="outputList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"materialsOutput\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"materialsOutput\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"materialsOutput\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"materialsOutput\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			materialsOutputService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/produceRecords/materialsOutput/outputList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/produceRecords/materialsOutput/allTempleteDownload", results = { @Result(name="excel",type="excel",location="outputList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_produceRecords_MaterialsOutput");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		materialsOutputService.findDataGridPage(dg,dgclass,getDgPage(),materialsOutput,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(materialsOutput.getId() != null && materialsOutput.getId() != -1){
				materialsOutput = materialsOutputService.getMaterialsOutput(materialsOutput.getId());
			}
			materialsOutputService.findDataGridPage(dg,dgclass,getDgPage(),materialsOutput,searchCondition,params);
	...
	private void save() throws Exception {
		...
		materialsOutputService.saveMaterialsOutput(materialsOutput,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/produceRecords/materialsOutput/outputEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/produceRecords/materialsOutput/outputView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "submit");
		bap_validate_datagrids=materialsOutputService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", materialsOutput.getId());
		...
		fileuploads.put("linkId", materialsOutput.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/produceRecords/materialsOutput/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		materialsOutputService.deleteMaterialsOutput(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceRecords/materialsOutput/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "restore");
		materialsOutputService.restoreMaterialsOutput(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/produceRecords/materialsOutput/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/produceRecords/materialsOutput/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "showDealInfoData");
		if (null != tableInfoId) {
			int count=materialsOutputService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = materialsOutputService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = materialsOutputService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMMaterialsOutput> findExportDataInfos(Page<WOMMaterialsOutput> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = materialsOutputService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = materialsOutputService.getAssProperty(p.getCode());	
									//String assModelCode = materialsOutputService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			materialsOutputService.findMaterialsOutputs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			materialsOutputService.findMaterialsOutputs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMMaterialsOutput> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMMaterialsOutput result : results){
	...
	public boolean isEditNew(){
		//return null == this.materialsOutput || null == this.materialsOutput.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/produceRecords/materialsOutput/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/produceRecords/materialsOutput/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/produceRecords/materialsOutput/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceRecords/materialsOutput/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMMaterialsOutputService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_produceRecords", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_produceRecords");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceRecords/materialsOutput/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_produceRecords_MaterialsOutput", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_produceRecords_MaterialsOutput", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/produceRecords/materialsOutput/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/produceRecords/materialsOutput/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_produceRecords_MaterialsOutput";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_produceRecords_MaterialsOutput", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/produceRecords/materialsOutput/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMMaterialsOutput materialsOutput = null;
		if(null != id) {
			materialsOutput = materialsOutputService.getMaterialsOutput(id);
		}
		...
		_auditLog.setColumnStringB(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
		_auditLog.setColumnStringA(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
		_auditLog.setColumnStringC(materialsOutput.getId().toString());
		_auditLog.setColumnStringD(WOMMaterialsOutput.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_produceRecords");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMMaterialsOutputAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, materialsOutput,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, materialsOutput,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,materialsOutput);
		...
		printlog.setEntity("WOM_7.5.0.0_produceRecords");
		...
		printlog.setBusinessId(materialsOutput.getId());
		printlog.setBussinessKey(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
		printlog.setMainDisplay(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/produceRecords/materialsOutput/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(materialsOutput,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/produceRecords/materialsOutput/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(materialsOutputService.getMaterialsOutput(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/produceRecords/materialsOutput/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/produceRecords/materialsOutput/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, materialsOutput,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				materialsOutput = materialsOutputService.getMaterialsOutput(Long.parseLong(arrId[i]));
				if(null != materialsOutput){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == materialsOutput.getId() ? "" : "," + materialsOutput.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == materialsOutput.getId() ? "" : "," + materialsOutput.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == materialsOutput.getId() ? "" : "," + materialsOutput.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == materialsOutput.getId() ? "" : materialsOutput.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, materialsOutput,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,materialsOutput);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//materialsOutputService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/produceRecords/materialsOutput/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMMaterialsOutput> findExportPageInfos(Page<WOMMaterialsOutput> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_produceRecords" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = materialsOutputService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMMaterialsOutput queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = materialsOutputService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = materialsOutputService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceRecords_MaterialsOutput");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_produceRecords_MaterialsOutput")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_produceRecords_MaterialsOutput");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_produceRecords_MaterialsOutput");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = materialsOutputService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_produceRecords_MaterialsOutput,WOM_7.5.0.0) */
// 自定义代码
