...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
import java.math.BigDecimal;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMBatchProduceTaskAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMBatchProduceTask> {
	...
	private WOMBatchProduceTask batchProduceTask;
	...
	//导出excel时,主模型内容
	List<WOMBatchProduceTask> queryResults = new ArrayList<WOMBatchProduceTask>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchProduceTask/batchProduceTask/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/batchProduceTask/batchProduceTask/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_batchProduceTask_BatchProduceTask" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMBatchProduceTask> batchProduceTasks;
	
	public List<WOMBatchProduceTask> getBatchProduceTasks() {
		return batchProduceTasks;
	}
	...
	private Page<WOMBatchProduceTask> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchProduceTask = new WOMBatchProduceTask();
			if(batchProduceTask.getOrderType() != null) {
				batchProduceTask.setOrderType((SystemCode) systemCodeService.getSystemCode(batchProduceTask.getOrderType().getId()));
			}
			if(batchProduceTask.getTaskType() != null) {
				batchProduceTask.setTaskType((SystemCode) systemCodeService.getSystemCode(batchProduceTask.getTaskType().getId()));
			}
		} else {
			if(id > 0){
				batchProduceTask = batchProduceTaskService.getBatchProduceTask(id, viewCode);
				if(tableInfoId == null && batchProduceTask != null){
					tableInfoId = batchProduceTask.getTableInfoId();
				}
			}
		}
	...
	public Page<WOMBatchProduceTask> getPage() {
		if (null == page) {
			page = new Page<WOMBatchProduceTask>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMBatchProduceTask getBatchProduceTask() {
		return batchProduceTask;
	}		

	public void setBatchProduceTask(WOMBatchProduceTask batchProduceTask) {
		this.batchProduceTask = batchProduceTask;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchProduceTaskService.getBatchProduceTaskAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String batchProduceTaskReferenceCopy() throws Exception{
			WOMBatchProduceTask oldBatchProduceTask = null;
			if(isEditNew()){
				...
				oldBatchProduceTask = new WOMBatchProduceTask();
			}else{
			//WOMBatchProduceTask batchProduceTask = batchProduceTaskService.getBatchProduceTask(this.batchProduceTask.getId());
			this.creatorInfo = creatorService.get(batchProduceTask.getCreateStaffId(), batchProduceTask.getCreatePositionId(), batchProduceTask.getCreateDepartmentId());
			//setBatchProduceTask(batchProduceTask);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(batchProduceTask.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchProduceTask.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchProduceTask.getId());
				...
				oldBatchProduceTask = batchProduceTaskService.getBatchProduceTask(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldBatchProduceTask.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_batchProduceTask_" + oldBatchProduceTask.getId();
			...
			batchProduceTask = batchProduceTaskService.getBatchProduceTask(refId);
			batchProduceTask.setPending(pending);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchProduceTask_BatchProduceTask");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMBatchProduceTask.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchProduceTask, null);
						Method setValue = WOMBatchProduceTask.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchProduceTask, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchProduceTask.setCreateDepartmentId(oldBatchProduceTask.getCreateDepartmentId());
			batchProduceTask.setCreatePositionId(oldBatchProduceTask.getCreatePositionId());
			batchProduceTask.setCreateStaffId(oldBatchProduceTask.getCreateStaffId());
			batchProduceTask.setOwnerDepartmentId(oldBatchProduceTask.getOwnerDepartmentId());
			batchProduceTask.setOwnerPositionId(oldBatchProduceTask.getOwnerPositionId());
			batchProduceTask.setOwnerStaffId(oldBatchProduceTask.getOwnerStaffId());
			batchProduceTask.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "referenceCopy");
	...
	@Actions({
		...
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTastEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchTastEdit.ftl") }),
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTastEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchTastEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchProduceTaskService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchProduceTaskReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			batchProduceTask = batchProduceTaskService.getBatchProduceTask(this.batchProduceTask.getId(), viewCode);
			this.creatorInfo = creatorService.get(batchProduceTask.getCreateStaffId(), batchProduceTask.getCreatePositionId(), batchProduceTask.getCreateDepartmentId());
			//setBatchProduceTask(batchProduceTask);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = batchProduceTaskService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = batchProduceTaskService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + batchProduceTask.getProcessKey() + "_" + pending.getActivityName() + "_WOM_7.5.0.0_batchProduceTask_" + batchProduceTask.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<WOMBatchProduceTask> tmpList = new ArrayList<>();
			tmpList.add(batchProduceTask);
			batchProduceTaskService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = batchProduceTaskService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchProduceTask.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchProduceTaskService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTastView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchTastView.ftl") }),
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTastViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "batchTastViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTaskList", results = { @Result(type = FREEMARKER, location = "batchTaskList.ftl") })
	})
	public String viewList() throws Exception{
		...
		//mainViewPath = "/WOM/batchProduceTask/batchProduceTask/batchTastView.action";
		mainViewPath=batchProduceTaskService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = batchProduceTaskService.getWorkFlowInfo(workFlowMenuCode);
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchProduceTaskService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchProduceTaskService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchProduceTaskService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTaskList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.staffID.name,result.makeTime,result.taskType.id,result.taskType.value,result.remark,result.staffID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batchTaskList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"batchProduceTask\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchProduceTask\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			batchProduceTaskService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<WOMBatchProduceTask> batchProduceTaskList=page.getResult();
				for(WOMBatchProduceTask lz:batchProduceTaskList){
	...
	@Actions({
		...
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTaskList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.staffID.name,result.makeTime,result.taskType.id,result.taskType.value,result.remark,result.staffID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchProduceTask\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchProduceTask\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTaskList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"batchProduceTask\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	 ...
	 * @return
	 */
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/allTempleteDownload", results = { @Result(name="excel",type="excel",location="batchTaskList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_batchProduceTask_BatchProduceTask");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchProduceTaskService.findDataGridPage(dg,dgclass,getDgPage(),batchProduceTask,"", null);
	...
	private Page<WOMBatchProdTaskPart> dg1495090968956Page;
	public Page<WOMBatchProdTaskPart> getDg1495090968956Page(){
		if(null == dg1495090968956Page){
			dg1495090968956Page = new Page<WOMBatchProdTaskPart>(1,20);
		}
		return dg1495090968956Page;
	}
	
	private Class dg1495090968956Clazz = WOMBatchProdTaskPart.class;
	
	public Class getDg1495090968956Clazz() {
		return dg1495090968956Clazz;
	}
	
	private List<WOMBatchProdTaskPart> dg1495090968956List;
	public List<WOMBatchProdTaskPart> getDg1495090968956List(){
		if(null == dg1495090968956List){
			dg1495090968956List = new ArrayList<WOMBatchProdTaskPart>();
		}
		return dg1495090968956List;
	}
	public void setDg1495090968956ListJson(String json) {
		//dg1495090968956List = (List<WOMBatchProdTaskPart>) batchProduceTaskService.generateObjectFromJson(json, WOMBatchProdTaskPart.class);
	}
	
	private List<Long> dg1495090968956DeletedIds;
	public List<Long> getDg1495090968956DeletedIds(){
		if(null == dg1495090968956DeletedIds){
			dg1495090968956DeletedIds = new ArrayList<Long>();
		}
		return dg1495090968956DeletedIds;
	}

	private Page<WOMBatchProdTaskPart> dg1496366180645Page;
	public Page<WOMBatchProdTaskPart> getDg1496366180645Page(){
		if(null == dg1496366180645Page){
			dg1496366180645Page = new Page<WOMBatchProdTaskPart>(1,20);
		}
		return dg1496366180645Page;
	}
	
	private Class dg1496366180645Clazz = WOMBatchProdTaskPart.class;
	
	public Class getDg1496366180645Clazz() {
		return dg1496366180645Clazz;
	}
	
	private List<WOMBatchProdTaskPart> dg1496366180645List;
	public List<WOMBatchProdTaskPart> getDg1496366180645List(){
		if(null == dg1496366180645List){
			dg1496366180645List = new ArrayList<WOMBatchProdTaskPart>();
		}
		return dg1496366180645List;
	}
	public void setDg1496366180645ListJson(String json) {
		//dg1496366180645List = (List<WOMBatchProdTaskPart>) batchProduceTaskService.generateObjectFromJson(json, WOMBatchProdTaskPart.class);
	}
	
	private List<Long> dg1496366180645DeletedIds;
	public List<Long> getDg1496366180645DeletedIds(){
		if(null == dg1496366180645DeletedIds){
			dg1496366180645DeletedIds = new ArrayList<Long>();
		}
		return dg1496366180645DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/data-dg1495090968956", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.id,result.productNum,result.productBatchNum,result.factoryId.name,result.factoryId.id,result.formularId.code,result.formularId.name,result.formularId.id,result.bulkProduct.productCode,result.bulkProduct.productName,result.bulkProduct.id,result.bulkBatchNum.batchText,result.bulkBatchNum.id,result.bulkNum,result.bulkType.id,result.bulkType.value,result.planStartTime,result.planEndTime,result.WOM_7_5_0_0_batchProduceTask_batchTastEditdg1495090968956_DATAGRID_CUSTOM_b234ab19_bd55_4dd8_a87c_87932ad121b2,result.remark,result.id,result.version,result.bulkBatchNum.id,result.productId.id,result.factoryId.id,result.bulkProduct.id,result.headId.id,result.formularId.id" , "dataGridCode", "WOM_7.5.0.0_batchProduceTask_batchTastEditdg1495090968956" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1495090968956-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1495090968956.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/data-dg1496366180645", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.id,result.productNum,result.productBatchNum,result.factoryId.name,result.factoryId.id,result.formularId.code,result.formularId.name,result.formularId.id,result.bulkProduct.productCode,result.bulkProduct.productName,result.bulkProduct.id,result.bulkBatchNum.batchText,result.bulkBatchNum.id,result.bulkNum,result.bulkType.id,result.bulkType.value,result.planStartTime,result.planEndTime,result.WOM_7_5_0_0_batchProduceTask_batchTastViewdg1496366180645_DATAGRID_CUSTOM_a3a49f2e_57b8_42f2_acc5_29b3654d6d90,result.remark,result.id,result.version,result.bulkBatchNum.id,result.productId.id,result.factoryId.id,result.bulkProduct.id,result.headId.id,result.formularId.id" , "dataGridCode", "WOM_7.5.0.0_batchProduceTask_batchTastViewdg1496366180645" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1496366180645-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1496366180645.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchProduceTask.getId() != null && batchProduceTask.getId() != -1){
				batchProduceTask = batchProduceTaskService.getBatchProduceTask(batchProduceTask.getId());
			}
			batchProduceTaskService.findDataGridPage(dg,dgclass,getDgPage(),batchProduceTask,searchCondition,params);
	...
	private void save() throws Exception {
		...
		batchProduceTaskService.save(batchProduceTask,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(batchProduceTask.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + batchProduceTask.getProcessKey() + "_" + p.getActivityName() + "_WOM_7.5.0.0_batchProduceTask_" + batchProduceTask.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(batchProduceTask.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchProduceTaskService.retrialThisTable(batchProduceTask,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "retrial");
		return SUCCESS;
	}
	@Actions({
		@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTastEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchTastView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "submit");
		bap_validate_datagrids=batchProduceTaskService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (batchProduceTask != null && batchProduceTask.getStatus() != null && batchProduceTask.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				batchProduceTaskService.saveSuperEditBatchProduceTask(batchProduceTask, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				batchProduceTaskService.submit(batchProduceTask,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", batchProduceTask.getTableInfoId());
		responseMap.put("id", batchProduceTask.getId());
		...
		fileuploads.put("linkId", batchProduceTask.getTableInfoId());
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "submit");
		return SUCCESS;
	}
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "bulkSubmit");
		batchProduceTaskService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/WOM/batchProduceTask/batchProduceTask/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchProduceTaskService.deleteBatchProduceTask(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "restore");
		batchProduceTaskService.restoreBatchProduceTask(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/batchProduceTask/batchProduceTask/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/batchProduceTask/batchProduceTask/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "showDealInfoData");
		if (null != tableInfoId) {
			int count=batchProduceTaskService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = batchProduceTaskService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = batchProduceTaskService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMBatchProduceTask> findExportDataInfos(Page<WOMBatchProduceTask> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchProduceTaskService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchProduceTaskService.getAssProperty(p.getCode());	
									//String assModelCode = batchProduceTaskService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchProduceTaskService.findBatchProduceTasks(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchProduceTaskService.findBatchProduceTasks(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMBatchProduceTask> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMBatchProduceTask result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchProduceTask || null == this.batchProduceTask.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && batchProduceTask != null && batchProduceTask.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + batchProduceTask.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return batchProduceTaskService.isRecallAble(tableInfoId);
	}

	@Action(value = "/WOM/batchProduceTask/batchProduceTask/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = batchProduceTaskService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + batchProduceTask.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + batchProduceTask.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchProduceTask/batchProduceTask/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "batchApproval");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/batchProduceTask/batchProduceTask/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/batchProduceTask/batchProduceTask/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMBatchProduceTaskService"
						, "com.supcon.orchid.WOM.services.WOMBatchProdTaskPartService"      
						, "com.supcon.orchid.WOM.services.WOMBatchProdTaskPartService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_batchProduceTask", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_batchProduceTask");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_batchProduceTask_BatchProduceTask", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_batchProduceTask_BatchProduceTask", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_batchProduceTask_BatchProduceTask";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_batchProduceTask_BatchProduceTask", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
		_auditLog.setColumnStringA(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
		_auditLog.setColumnStringC(batchProduceTask.getId().toString());
		_auditLog.setColumnStringD(WOMBatchProduceTask.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_batchProduceTask");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMBatchProduceTaskAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchProduceTask,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchProduceTask,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchProduceTask);
		...
		printlog.setEntity("WOM_7.5.0.0_batchProduceTask");
		...
		printlog.setBusinessId(batchProduceTask.getId());
		printlog.setBussinessKey(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
		printlog.setMainDisplay(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchProduceTask,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchProduceTaskService.getBatchProduceTask(Long.parseLong(arrId[i])));
	...
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchProduceTask,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchProduceTask = batchProduceTaskService.getBatchProduceTask(Long.parseLong(arrId[i]));
				if(null != batchProduceTask){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchProduceTask.getId() ? "" : "," + batchProduceTask.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchProduceTask.getTableNo() ? "" : "," + batchProduceTask.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchProduceTask.getTableNo() ? "" : batchProduceTask.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchProduceTask.getId() ? "" : "," + batchProduceTask.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchProduceTask.getId() ? "" : batchProduceTask.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchProduceTask,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchProduceTask);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchProduceTaskService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/batchProduceTask/batchProduceTask/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMBatchProduceTask> findExportPageInfos(Page<WOMBatchProduceTask> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_batchProduceTask" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = batchProduceTaskService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMBatchProduceTask queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = batchProduceTaskService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = batchProduceTaskService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchProduceTask_BatchProduceTask");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_batchProduceTask_BatchProduceTask")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_batchProduceTask_BatchProduceTask");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_batchProduceTask_BatchProduceTask");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchProduceTaskService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
		if(code.equals("WOM_7.5.0.0_batchProduceTask_BatchProduceTask") && methodName.equals("submit")){
			if("cancel".equals(workFlowVar.getOutcomeType()) ){
				//作废处理
				//作废时执行  如果表体存在批次，则删除批次信息
				batchProduceTaskService.delBatchNum(batchProduceTask);
			}else{
				//每次保存时调用
				if(batchProduceTask.getId() == null){
					...
					//if(dgLists!=null){
						...
						oldBatchNum = new ArrayList<String>();
						oldBatchNum = batchProduceTaskService.getOldBatchNum(batchProduceTask);
						
						//获取之前批号数量
						oldAmount = batchProduceTaskService.getBatchAmout(batchProduceTask);
						//获取之前批号物品
						oldProduct = batchProduceTaskService.getBatchPro(batchProduceTask);
					//}
				}
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
		if(code.equals("WOM_7.5.0.0_batchProduceTask_BatchProduceTask") && methodName.equals("submit")){
			if(batchProduceTask.getStatus() == 99){
				//添加批次处理信息
				batchProduceTaskService.addDealBatch(batchProduceTask,"publicSetTableState/02","批量指令单生效");
			}
			if(!"cancel".equals(workFlowVar.getOutcomeType()) ){
				if(addNew){
					List<WOMBatchProdTaskPart> dg1495090968956Lists = (List<WOMBatchProdTaskPart>) batchProduceTaskService.generateObjectFromJson(dgLists.get("dg1495090968956"), WOMBatchProdTaskPart.class);
					//新增时添加批号信息  只在分批时执行
					batchProduceTaskService.setBatch(batchProduceTask,dg1495090968956Lists);
				}else{
					//将保存前的表体组成一个没有重复的批号list
					newBatchNum = new ArrayList<String>();
					newBatchNum = batchProduceTaskService.getOldBatchNum(batchProduceTask);
					
					//获取之前批号数量
					newAmount = batchProduceTaskService.getBatchAmout(batchProduceTask);
					//获取之前批号物品
					newProduct = batchProduceTaskService.getBatchPro(batchProduceTask);
					//执行新增或修改操作
					batchProduceTaskService.operate(batchProduceTask,newBatchNum,oldBatchNum,newAmount,oldAmount,newProduct,oldProduct);
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_batchProduceTask_BatchProduceTask,WOM_7.5.0.0) */
// 自定义代码
	private boolean addNew = false;//是否第一次保存
	//将保存前的表体组成一个没有重复的批号list
	List<String> oldBatchNum;
	//将保存后的表体组成一个没有重复的批号list
	List<String> newBatchNum;
	//用来进行前后数量对比  
	//键 批号 值 数量
	...
	//用来进行前后物品对比  
	//键 批号 值 物品
	Map<String, MESBasicProduct> oldProduct=new HashMap<String,MESBasicProduct>();
	Map<String, MESBasicProduct> newProduct=new HashMap<String,MESBasicProduct>();
/* CUSTOM CODE END */
}