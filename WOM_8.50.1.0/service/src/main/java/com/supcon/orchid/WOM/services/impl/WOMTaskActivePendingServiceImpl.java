...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMTaskActivePendingDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */


@Service("wOM_taskActivePendingService")
@Transactional
public class WOMTaskActivePendingServiceImpl  extends BaseServiceImpl<WOMTaskActivePending> implements WOMTaskActivePendingService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMTaskActivePending> bapBaseService;
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMTaskActivePending.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)";
		Query query =  taskActivePendingDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMTaskActivePending.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = taskActivePendingDao.createQuery(hql);
	...
	public WOMTaskActivePending getTaskActivePending(long id){
		return getTaskActivePending(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskActivePending getTaskActivePending(long id, String viewCode){
		WOMTaskActivePending taskActivePending = taskActivePendingDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getTaskActivePendingAsJSON(long id, String include){
		WOMTaskActivePending taskActivePending = taskActivePendingDao.load(id);
		// 一对多情况处理

		if(taskActivePending == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(taskActivePending, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="删除",operType="3")
	public void deleteTaskActivePending(WOMTaskActivePending taskActivePending){
		beforeDeleteTaskActivePending(taskActivePending);
		taskActivePendingDao.deletePhysical(taskActivePending);
		afterDeleteTaskActivePending(taskActivePending);
		List<WOMTaskActivePending> params = new ArrayList<WOMTaskActivePending>();
		params.add(taskActivePending);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="删除",operType="3")
	public void deleteTaskActivePending(List<Long> taskActivePendingIds){
		deleteTaskActivePending(taskActivePendingIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="删除",operType="3")
	public void deleteTaskActivePending(List<Long> taskActivePendingIds, String eventTopic) {
		List<WOMTaskActivePending> taskActivePendings = new ArrayList<WOMTaskActivePending>();
		for(Long taskActivePendingId : taskActivePendingIds){
			WOMTaskActivePending taskActivePending = getTaskActivePending(taskActivePendingId);
			taskActivePendings.add(taskActivePending);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(taskActivePendings != null && taskActivePendings.size() > 0 ){
			for(WOMTaskActivePending taskActivePending : taskActivePendings){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMTaskActivePending.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(taskActivePending));
						}
					}
					dataAuditLogService.saveCustomerAudit("删除",taskActivePending, taskActivePending.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(taskActivePendingIds != null && taskActivePendingIds.size() > 0) {
			String hql = "delete " + WOMTaskActivePending.JPA_NAME + " where id in(:ids)";
			Query query = taskActivePendingDao.createQuery(hql);
			query.setParameterList("ids", taskActivePendingIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMTaskActivePending taskActivePendingz : taskActivePendings){
				taskActivePendingz.setValid(false);
				List<WOMTaskActivePending> params = new ArrayList<WOMTaskActivePending>();
				params.add(taskActivePendingz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="删除",operType="3")
	public void deleteTaskActivePending(long taskActivePendingId, int taskActivePendingVersion){
		
		WOMTaskActivePending taskActivePending = getTaskActivePending(taskActivePendingId);
		
		beforeDeleteTaskActivePending(taskActivePending);
		taskActivePendingDao.delete(taskActivePendingId, taskActivePendingVersion);
		afterDeleteTaskActivePending(taskActivePending);
		List<WOMTaskActivePending> params = new ArrayList<WOMTaskActivePending>();
		params.add(taskActivePending);
		Map<String, Object> props = new HashMap<String, Object>();
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="删除",operType="3")
	public void deleteTaskActivePending(String taskActivePendingIds){
		deleteCollection(taskActivePendingIds);
	}
	...
	private void deleteCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteTaskActivePending(Long.valueOf(id), Integer.valueOf(version));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="批量导入",operType="4")
	public  void batchImportBaseTaskActivePending(List<WOMTaskActivePending>  taskActivePendings){
		for(WOMTaskActivePending taskActivePending:taskActivePendings)  {
			saveTaskActivePending(taskActivePending, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseTaskActivePending(List<WOMTaskActivePending>  taskActivePendings){
		for(WOMTaskActivePending taskActivePending:taskActivePendings)  {
			saveTaskActivePending(taskActivePending, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="保存")
	public void saveTaskActivePending(WOMTaskActivePending taskActivePending, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveTaskActivePending(taskActivePending, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="保存")
	public void saveTaskActivePending(WOMTaskActivePending taskActivePending, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveTaskActivePending(taskActivePending, dgLists,dgDeleteIDs,assFileUploads, viewCode, null);
	}


	private void generateTaskActivePendingCodes(WOMTaskActivePending taskActivePending) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTaskActivePendingCodes(taskActivePending, false);
	}
	
	private void generateTaskActivePendingCodes(WOMTaskActivePending taskActivePending, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	private void generateTaskActivePendingSummarys(WOMTaskActivePending taskActivePending) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTaskActivePendingSummarys(taskActivePending, false);
	}

	private void generateTaskActivePendingSummarys(WOMTaskActivePending taskActivePending, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending")
	public void saveTaskActivePending(WOMTaskActivePending taskActivePending,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(taskActivePending.getId()!=null && taskActivePending.getId()>0){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, taskActivePending);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(taskActivePending);
		beforeSaveTaskActivePending(taskActivePending, viewIsView);

		if (viewIsView) {
			taskActivePendingDao.saveWithRevertVersion(taskActivePending);
		} else {
			if(isNew)
					taskActivePendingDao.save(taskActivePending);
				else
					taskActivePendingDao.merge(taskActivePending);
		}
		dealDatagridsSave(taskActivePending,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateTaskActivePendingCodes(taskActivePending, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateTaskActivePendingSummarys(taskActivePending, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveTaskActivePending(taskActivePending, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, taskActivePending);
			}
		}
		List<WOMTaskActivePending> params = new ArrayList<WOMTaskActivePending>();
		taskActivePendingDao.flush();
		taskActivePendingDao.clear();
		taskActivePending = taskActivePendingDao.load(taskActivePending.getId());
		
		
		params.add(taskActivePending);
	}
	
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMTaskActivePending taskActivePending){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("taskActivePending", taskActivePending);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending")
	public void mergeTaskActivePending(WOMTaskActivePending taskActivePending, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(taskActivePending);
		beforeSaveTaskActivePending(taskActivePending);
		taskActivePendingDao.merge(taskActivePending);

		dealDatagridsSave(taskActivePending,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveTaskActivePending(taskActivePending);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMTaskActivePending> findTaskActivePendings(Page<WOMTaskActivePending> page, Criterion... criterions) {
		return taskActivePendingDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskActivePendings(Page<WOMTaskActivePending> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, List<Param> params) {
		findTaskActivePendings(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskActivePendings(Page<WOMTaskActivePending> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql) {
		commonQuery(page, viewCode, type, processKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTaskActivePendings(Page<WOMTaskActivePending> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void commonQuery(Page<WOMTaskActivePending> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"taskActivePending\"" : "\"" + key.substring(0, lastDotPos) + "\"";
				...
				if(type == Sql.TYPE_LIST_REFERENCE){
					if(null != crossCompanyFlag && !crossCompanyFlag){
						s.append(" AND \"taskActivePending\".CID = ").append(getCurrentCompanyId());
					}
				}
...
//				}
			...
			if (hasWhere)
				...
				s.append(" WHERE ");
			s.append(" ( \"taskActivePending\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			List<Object> powerCodeList =  taskActivePendingDao.createNativeQuery(powerCodeSql, powerCode).list();
			if(powerCodeList.size() > 0) {
				...
				List<Object> checkList =  taskActivePendingDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
				if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"taskActivePending\".LAY_REC ASC, \"taskActivePending\".SORT ASC");
				} else {
					orderPart.append(" \"taskActivePending\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) taskActivePendingDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) taskActivePendingDao.createNativeQuery(countSql, arr)
			...
			if(!noQueryFlag)  {
				SQLQuery query = taskActivePendingDao.createNativeQuery(realSql, arr);
				List<WOMTaskActivePending> result = new ArrayList<WOMTaskActivePending>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMTaskActivePending.class,taskActivePendingDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMTaskActivePending.class,taskActivePendingDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMTaskActivePending taskActivePending : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(taskActivePending.getId(), "WOM_produceTask_taskActivePending");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(taskActivePending.getId(), "WOM_produceTask_taskActivePending");
							if(null != documents && !documents.isEmpty()) {
								taskActivePending.setDocument(documents.get(0));
								taskActivePending.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMTaskActivePending> result) {
		View view = viewServiceFoundation.getView(viewCode);
		...
		Map<Long, WOMTaskActivePending> rsMap = new HashMap<Long, WOMTaskActivePending>();
		for (WOMTaskActivePending taskActivePending : result) {
			ids.add(taskActivePending.getId());
			rsMap.put(taskActivePending.getId(), taskActivePending);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(taskActivePendingDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = taskActivePendingDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_TaskActivePending" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_TaskActivePending" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  taskActivePendingDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskActivePending\"","WOM_7.5.0.0_produceTask","","WOM_TASK_ACTIVE_PENDINGS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"taskActivePending\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"taskActivePending\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"taskActivePending\"","WOM_7.5.0.0_produceTask","","WOM_TASK_ACTIVE_PENDINGS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_TaskActivePending" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  UPPER(\"").append("taskActivePending.mc\"").append(".MNE_CODE) like UPPER(?) escape '&' AND \"taskActivePending\".VALID = 1");
			...
			if(!cross){
				mnecodeSql.append(" AND \"taskActivePending\".CID = ?");
				totalValues.add(getCurrentCompanyId());
			}
			
			SQLQuery query = taskActivePendingDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=taskActivePendingDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMTaskActivePending.ENTITY_CODE;}
	protected String getModuleCode(){return WOMTaskActivePending.MODULE_CODE;}
	protected String getModelCode(){return WOMTaskActivePending.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param taskActivePending 模型实例
	 */
	private void checkUniqueConstraint(WOMTaskActivePending taskActivePending){
	}
	
	
	private void beforeSaveTaskActivePending(WOMTaskActivePending taskActivePending, Object... objects){
		checkUniqueConstraint(taskActivePending);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveTaskActivePending(WOMTaskActivePending taskActivePending, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteTaskActivePending(WOMTaskActivePending taskActivePending){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteTaskActivePending(WOMTaskActivePending taskActivePending){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateCodes(WOMTaskActivePending taskActivePending, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMTaskActivePending taskActivePending, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportTaskActivePending(List<WOMTaskActivePending> insertObjs, List<WOMTaskActivePending> updateObjs, List<Map<String,String>> columnInfo,List<Map<String, Map<Integer, Map<Integer, String>>>> errMsgSheet){//主辅模型导入before方法
		/* CUSTOM CODE START(serviceimpl,beforeImportTaskActivePending,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportTaskActivePending(List<WOMTaskActivePending> insertObjs, List<WOMTaskActivePending> updateObjs, List<Map<String,String>> columnInfo){//主辅模型导入after方法
		/* CUSTOM CODE START(serviceimpl,afterImportTaskActivePending,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public List<WOMTaskActivePending> getTaskActivePendings(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskActivePending.class);
		if(params!=null){
			...
			return taskActivePendingDao.findByHql("from " + WOMTaskActivePending.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return taskActivePendingDao.findByCriteria(detachedCriteria);
	}
	
	@Override
	public WOMTaskActivePending loadTaskActivePendingByBussinessKey(WOMTaskActivePending taskActivePending){
		return loadTaskActivePendingByBussinessKey(taskActivePending.getId());
	}
	
	@Override
	public Page<WOMTaskActivePending> getTaskActivePendings(Page<WOMTaskActivePending> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMTaskActivePending.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = taskActivePendingDao.findByPage(page, sql);
			}else{
				page = taskActivePendingDao.findAllByPage(page);
			}
			return page;
		}
		page = taskActivePendingDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteTaskActivePendingByProdTaskProcExelog(List<Long> prodTaskProcExelogIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("processExeLogList", prodTaskProcExelogIds);
		String hql = "select id from WOMTaskActivePending where "
						+ "processExeLog.id in (:processExeLogList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> taskActivePendingIds1 = taskActivePendingDao.findByHql(hql, params);
		if(taskActivePendingIds1 != null && !taskActivePendingIds1.isEmpty()) {
		}
		hql = "delete WOMTaskActivePending where "
						+ "processExeLog.id in (:processExeLogList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		taskActivePendingDao.bulkExecute(hql, params);
	}
	@Override
	@Transactional
	public void deleteTaskActivePendingByProduceTaskActive(List<Long> produceTaskActiveIds){
		Map<String, Object> params = new HashMap<String, Object>();
	...
	@Override
	@Transactional
	public void deleteTaskActivePendingByProduceTaskProcess(List<Long> produceTaskProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
	...
	public WOMTaskActivePending loadTaskActivePendingByBussinessKey(Serializable bussinessKey){
		return taskActivePendingDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMTaskActivePending> findByProperty(String propertyName, Object object){
		return taskActivePendingDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMTaskActivePending findEntityByProperty(String propertyName, Object object){
		return taskActivePendingDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMTaskActivePending.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = taskActivePendingDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_TaskActivePending","runtime");
		try {
	...
	private void dealDatagridsSave(WOMTaskActivePending taskActivePending,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()), _reflect_dgclass,taskActivePendingDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMTaskActivePending.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, taskActivePending);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(taskActivePending.getTableInfoId());
							}else{
								((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(taskActivePending.getTableInfoId());
							}
							if(isTree){
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",taskActivePending.getTableInfoId());	
	...
	public List<String> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMTaskActivePending.TABLE_NAME;
			list = taskActivePendingDao.createNativeQuery(sql).list();
		}
		return list;
	}
	
	@Autowired
	private WOMTaskActivePendingDaoImpl taskActivePendingDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMTaskActivePending.TABLE_NAME + " where id = ?";
		String tableNo = taskActivePendingDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public Map<Object, Long> importBatchTaskActivePending(final List<WOMTaskActivePending> insertObjs, 
			final List<WOMTaskActivePending> updateObjs, List<Map<String,String>> columnInfo, Map<String, Map<Integer, Long>> tableInfoIds){
		...
		final List<WOMTaskActivePending> nmeObjs = new ArrayList<WOMTaskActivePending>();
		...
		//String idSql = "select id from " + WOMTaskActivePending.TABLE_NAME + " order by id desc";
		//Query query = taskActivePendingDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMTaskActivePending.JPA_NAME + " order by id desc";
		Query query =  taskActivePendingDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMTaskActivePending.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMTaskActivePending taskActivePending:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMTaskActivePending.TABLE_NAME,1,dbId);
			if(taskActivePending.getId() != null ){
				m.put(taskActivePending.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) taskActivePendingDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_TaskActivePending"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		taskActivePendingDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMTaskActivePending.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMTaskActivePending taskActivePending = updateObjs.get(updateIndex);
						Long id = taskActivePending.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = taskActivePending.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(taskActivePending);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMTaskActivePending.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMTaskActivePending taskActivePending = insertObjs.get(insertIndex);
						Long id = insertIds.get(insertIndex);
						insertps.setLong(columnMaps.size() + 1, id);
//						insertps.setLong(columnMaps.size() + 1, taskActivePending.getId());
//						insertps.setLong(columnMaps.size() + 1, autoGeneratorID.getNextId(WOMTaskActivePending.TABLE_NAME,1,dbId));
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = taskActivePending.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		taskActivePendingDaoImpl.getSession().flush();
		taskActivePendingDaoImpl.getSession().clear();
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMTaskActivePending> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMTaskActivePending> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMTaskActivePending> result = getResult(page, query, new PendingResultTransformer(WOMTaskActivePending.class,taskActivePendingDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMTaskActivePending model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = taskActivePendingDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_TaskActivePending").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = taskActivePendingDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = taskActivePendingDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = taskActivePendingDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?taskActivePendingDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  taskActivePendingDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,taskActivePendingDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="保存")
	public void saveTaskActivePending(WOMTaskActivePending taskActivePending, DataGridService dataGridService){
		saveTaskActivePending(taskActivePending, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="保存")
	public void saveTaskActivePending(WOMTaskActivePending taskActivePending, DataGridService dataGridService, String viewCode){
		saveTaskActivePending(taskActivePending, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending")
	public void saveTaskActivePending(WOMTaskActivePending taskActivePending, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending")
	public void mergeTaskActivePending(WOMTaskActivePending taskActivePending, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(taskActivePending);
		beforeSaveTaskActivePending(taskActivePending);
		taskActivePendingDao.merge(taskActivePending);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="excel导出",operType="4")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="页面打印",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_TaskActivePending",desc="批量保存",operType="4")
	...
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_TaskActivePending,WOM_7.5.0.0) */
// 自定义代码
