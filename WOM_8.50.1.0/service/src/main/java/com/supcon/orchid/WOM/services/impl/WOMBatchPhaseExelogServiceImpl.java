...
import com.supcon.orchid.WOM.services.WOMBatchPhaseExelogImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMBatchPhaseExelogDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
import java.text.SimpleDateFormat;
/* CUSTOM CODE END */

@Service("wOM_batchPhaseExelogService")
@Transactional
public class WOMBatchPhaseExelogServiceImpl  extends BaseServiceImpl<WOMBatchPhaseExelog> implements WOMBatchPhaseExelogService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMBatchPhaseExelog> bapBaseService;
	...
	@Autowired
	private WOMBatchPhaseExelogImportService batchPhaseExelogImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMBatchPhaseExelog.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchPhaseExelogDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMBatchPhaseExelog.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchPhaseExelogDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchPhaseExelog.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMBatchPhaseExelog.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchPhaseExelogDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMBatchPhaseExelog.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMBatchPhaseExelog.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMBatchPhaseExelog> list = (List<WOMBatchPhaseExelog>) batchPhaseExelogDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMBatchPhaseExelog getBatchPhaseExelog(long id){
		return getBatchPhaseExelog(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchPhaseExelog getBatchPhaseExelog(long id, String viewCode){
		WOMBatchPhaseExelog batchPhaseExelog = batchPhaseExelogDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchPhaseExelogAsJSON(long id, String include){
		WOMBatchPhaseExelog batchPhaseExelog = batchPhaseExelogDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchPhaseExelog, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.common.delete",operType="3")
	public void deleteBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog){
		if(null != batchPhaseExelog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchPhaseExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
			AuditUtil.setColumnStringA(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
			AuditUtil.setColumnStringC(batchPhaseExelog.getId().toString());
		}
		beforeDeleteBatchPhaseExelog(batchPhaseExelog);
		batchPhaseExelogDao.deletePhysical(batchPhaseExelog);
		afterDeleteBatchPhaseExelog(batchPhaseExelog);
		batchPhaseExelogDao.flush();
		List<WOMBatchPhaseExelog> params = new ArrayList<WOMBatchPhaseExelog>();
		params.add(batchPhaseExelog);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.common.delete",operType="3")
	public void deleteBatchPhaseExelog(List<Long> batchPhaseExelogIds){
		deleteBatchPhaseExelog(batchPhaseExelogIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.common.delete",operType="3")
	public void deleteBatchPhaseExelog(List<Long> batchPhaseExelogIds, String eventTopic) {
		List<WOMBatchPhaseExelog> batchPhaseExelogs = new ArrayList<WOMBatchPhaseExelog>();
		for(Long batchPhaseExelogId : batchPhaseExelogIds){
			WOMBatchPhaseExelog batchPhaseExelog = getBatchPhaseExelog(batchPhaseExelogId);
			batchPhaseExelogs.add(batchPhaseExelog);
			if(null != batchPhaseExelog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchPhaseExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchPhaseExelog.getId() ? "" : "," + batchPhaseExelog.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchPhaseExelog.getId() ? "" : "," + batchPhaseExelog.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchPhaseExelog.getId() ? "" : "," + batchPhaseExelog.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchPhaseExelogs != null && batchPhaseExelogs.size() > 0 ){
			for(WOMBatchPhaseExelog batchPhaseExelog : batchPhaseExelogs){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMBatchPhaseExelog.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchPhaseExelog));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchPhaseExelog, batchPhaseExelog.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchPhaseExelogIds != null && batchPhaseExelogIds.size() > 0) {
			String hql = "delete " + WOMBatchPhaseExelog.JPA_NAME + " where id in(:ids)";
			Query query = batchPhaseExelogDao.createQuery(hql);
			query.setParameterList("ids", batchPhaseExelogIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMBatchPhaseExelog batchPhaseExelogz : batchPhaseExelogs){
				batchPhaseExelogz.setValid(false);
				List<WOMBatchPhaseExelog> params = new ArrayList<WOMBatchPhaseExelog>();
				params.add(batchPhaseExelogz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.common.delete",operType="3")
	public void deleteBatchPhaseExelog(long batchPhaseExelogId, int batchPhaseExelogVersion){
		this.deleteBatchPhaseExelog(batchPhaseExelogId, batchPhaseExelogVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.common.delete",operType="3")
	public void deleteBatchPhaseExelog(long batchPhaseExelogId, int batchPhaseExelogVersion,SignatureLog signatureLog){
		WOMBatchPhaseExelog batchPhaseExelog = getBatchPhaseExelog(batchPhaseExelogId);
		...
		beforeDeleteBatchPhaseExelog(batchPhaseExelog);
		batchPhaseExelogDao.delete(batchPhaseExelogId, batchPhaseExelogVersion);
		afterDeleteBatchPhaseExelog(batchPhaseExelog);
		batchPhaseExelogDao.flush();
		List<WOMBatchPhaseExelog> params = new ArrayList<WOMBatchPhaseExelog>();
		params.add(batchPhaseExelog);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchPhaseExelog.getId();
			signatureLog.setTableId(batchPhaseExelog.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.common.delete",operType="3")
	public void deleteBatchPhaseExelog(String batchPhaseExelogIds){
		this.deleteBatchPhaseExelog(batchPhaseExelogIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.common.delete",operType="3")
	public void deleteBatchPhaseExelog(String batchPhaseExelogIds,SignatureLog signatureLog){
		deleteCollection(batchPhaseExelogIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchPhaseExelog(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_BatchPhaseExelog#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="ec.engine.restore",operType="10")
	public void restoreBatchPhaseExelog(String batchPhaseExelogIds){
		restoreCollection(batchPhaseExelogIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchPhaseExelog(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="ec.engine.restore",operType="10")
	public void restoreBatchPhaseExelog(long batchPhaseExelogId){
		
		findBusinessKeyUsed(batchPhaseExelogId);	//判断业务主键是否重复
		...
		beforeRestoreBatchPhaseExelog(batchPhaseExelog);
		batchPhaseExelog.setValid(true);
		batchPhaseExelogDao.update(batchPhaseExelog);
		afterRestoreBatchPhaseExelog(batchPhaseExelog);
		List<WOMBatchPhaseExelog> params = new ArrayList<WOMBatchPhaseExelog>();
		params.add(batchPhaseExelog);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchPhaseExelogId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_BatchPhaseExelog");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMBatchPhaseExelog.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMBatchPhaseExelog.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchPhaseExelogDao.createNativeQuery(sql,batchPhaseExelogId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchPhaseExelog(List<WOMBatchPhaseExelog>  batchPhaseExelogs){
		for(WOMBatchPhaseExelog batchPhaseExelog:batchPhaseExelogs)  {
			saveBatchPhaseExelog(batchPhaseExelog, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchPhaseExelog(List<WOMBatchPhaseExelog>  batchPhaseExelogs){
		for(WOMBatchPhaseExelog batchPhaseExelog:batchPhaseExelogs)  {
			saveBatchPhaseExelog(batchPhaseExelog, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="ec.flow.save")
	public void saveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchPhaseExelog(batchPhaseExelog, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="ec.flow.save")
	public void saveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchPhaseExelog(batchPhaseExelog, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchPhaseExelogCodes(WOMBatchPhaseExelog batchPhaseExelog) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchPhaseExelogCodes(batchPhaseExelog, false);
	}
	
	public void generateBatchPhaseExelogCodes(WOMBatchPhaseExelog batchPhaseExelog, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchPhaseExelogSummarys(WOMBatchPhaseExelog batchPhaseExelog) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchPhaseExelogSummarys(batchPhaseExelog, false);
	}

	public void generateBatchPhaseExelogSummarys(WOMBatchPhaseExelog batchPhaseExelog, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog")
	public void saveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchPhaseExelog(batchPhaseExelog, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog")
	public void saveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(batchPhaseExelog.getId() != null && batchPhaseExelog.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchPhaseExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchPhaseExelog);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchPhaseExelog);
		beforeSaveBatchPhaseExelog(batchPhaseExelog, viewIsView);

		if (viewIsView) {
			batchPhaseExelogDao.saveWithRevertVersion(batchPhaseExelog);
		} else {
			if(isNew)
				batchPhaseExelogDao.save(batchPhaseExelog);
			else
				batchPhaseExelogDao.merge(batchPhaseExelog);
		}
		if(null != batchPhaseExelog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchPhaseExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
			AuditUtil.setColumnStringA(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
		...
		dealDatagridsSave(batchPhaseExelog,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchPhaseExelogCodes(batchPhaseExelog, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchPhaseExelogSummarys(batchPhaseExelog, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchPhaseExelog(batchPhaseExelog, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchPhaseExelog);
			}
		}
		List<WOMBatchPhaseExelog> params = new ArrayList<WOMBatchPhaseExelog>();
		batchPhaseExelogDao.flush();
		batchPhaseExelogDao.clear();
		batchPhaseExelog = batchPhaseExelogDao.load(batchPhaseExelog.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMBatchPhaseExelog batchPhaseExelog){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchPhaseExelog", batchPhaseExelog);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog")
	public void mergeBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchPhaseExelog);
		beforeSaveBatchPhaseExelog(batchPhaseExelog);
		batchPhaseExelogDao.merge(batchPhaseExelog);
		if(null != batchPhaseExelog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchPhaseExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
			AuditUtil.setColumnStringA(null == batchPhaseExelog.getId() ? "" : batchPhaseExelog.getId().toString());
		...
		dealDatagridsSave(batchPhaseExelog,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchPhaseExelog(batchPhaseExelog);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMBatchPhaseExelog> findBatchPhaseExelogs(Page<WOMBatchPhaseExelog> page, Criterion... criterions) {
		return batchPhaseExelogDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchPhaseExelogs(Page<WOMBatchPhaseExelog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchPhaseExelogs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchPhaseExelogs(Page<WOMBatchPhaseExelog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchPhaseExelogs(Page<WOMBatchPhaseExelog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMBatchPhaseExelog> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchPhaseExelog\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchPhaseExelog\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchPhaseExelog\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"batchPhaseExelog\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  batchPhaseExelogDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchPhaseExelogDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchPhaseExelog\".LAY_REC ASC, \"batchPhaseExelog\".SORT ASC");
				} else {
					orderPart.append(" \"batchPhaseExelog\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchPhaseExelogDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchPhaseExelogDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchPhaseExelogDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMBatchPhaseExelog> result = new ArrayList<WOMBatchPhaseExelog>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMBatchPhaseExelog.class,batchPhaseExelogDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMBatchPhaseExelog.class,batchPhaseExelogDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMBatchPhaseExelog batchPhaseExelog : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchPhaseExelog.getId(), "WOM_produceTask_batchPhaseExelog");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchPhaseExelog.getId(), "WOM_produceTask_batchPhaseExelog");
							if(null != documents && !documents.isEmpty()) {
								batchPhaseExelog.setDocument(documents.get(0));
								batchPhaseExelog.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMBatchPhaseExelog> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMBatchPhaseExelog> result) {
		View view = null;
		...
		Map<Long, WOMBatchPhaseExelog> rsMap = new HashMap<Long, WOMBatchPhaseExelog>();
		for (WOMBatchPhaseExelog batchPhaseExelog : result) {
			ids.add(batchPhaseExelog.getId());
			rsMap.put(batchPhaseExelog.getId(), batchPhaseExelog);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchPhaseExelogDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchPhaseExelogDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_BatchPhaseExelog" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_BatchPhaseExelog" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchPhaseExelogDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchPhaseExelog\"","WOM_7.5.0.0_produceTask","","WOM_BATCH_PHASE_EXELOGS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchPhaseExelog\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchPhaseExelog\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchPhaseExelog\"","WOM_7.5.0.0_produceTask","","WOM_BATCH_PHASE_EXELOGS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_BatchPhaseExelog" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchPhaseExelog.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchPhaseExelog\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchPhaseExelog\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchPhaseExelog\".CID = 1 OR \"batchPhaseExelog\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchPhaseExelog.mc\"").append(".BATCH_PHASE_EXELOG) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchPhaseExelogDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchPhaseExelogDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMBatchPhaseExelog.ENTITY_CODE;}
	protected String getModuleCode(){return WOMBatchPhaseExelog.MODULE_CODE;}
	protected String getModelCode(){return WOMBatchPhaseExelog.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchPhaseExelog 模型实例
	 */
	public void checkUniqueConstraint(WOMBatchPhaseExelog batchPhaseExelog){
	}
	
	
	@Override
	public List<WOMBatchPhaseExelog> getBatchPhaseExelogs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchPhaseExelog.class);
		if(params!=null){
			...
			return batchPhaseExelogDao.findByHql("from " + WOMBatchPhaseExelog.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchPhaseExelogDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMBatchPhaseExelog> findBatchPhaseExelogsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchPhaseExelog.class);
	...
	@Override
	public List<WOMBatchPhaseExelog> findBatchPhaseExelogsByHql(String hql, Object... objects){
		
		return   batchPhaseExelogDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMBatchPhaseExelog loadBatchPhaseExelogByBussinessKey(WOMBatchPhaseExelog batchPhaseExelog){
		return loadBatchPhaseExelogByBussinessKey(batchPhaseExelog.getId());
	}
	
	@Override
	public Page<WOMBatchPhaseExelog> getBatchPhaseExelogs(Page<WOMBatchPhaseExelog> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMBatchPhaseExelog.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchPhaseExelogDao.findByPage(page, sql);
			}else{
				page = batchPhaseExelogDao.findAllByPage(page);
			}
			return page;
		}
		page = batchPhaseExelogDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteBatchPhaseExelogByProduceTaskProcess(List<Long> produceTaskProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("taskProcessIDList", produceTaskProcessIds);
		String hql = "select id from WOMBatchPhaseExelog where "
						+ "taskProcessID.id in (:taskProcessIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> batchPhaseExelogIds1 = batchPhaseExelogDao.findByHql(hql, params);
		if(batchPhaseExelogIds1 != null && !batchPhaseExelogIds1.isEmpty()) {
		}
		hql = "delete WOMBatchPhaseExelog where "
						+ "taskProcessID.id in (:taskProcessIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		batchPhaseExelogDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchPhaseExelog loadBatchPhaseExelogByBussinessKey(Serializable bussinessKey){
		return batchPhaseExelogDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMBatchPhaseExelog> findByProperty(String propertyName, Object object){
		return batchPhaseExelogDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMBatchPhaseExelog findEntityByProperty(String propertyName, Object object){
		return batchPhaseExelogDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMBatchPhaseExelog.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchPhaseExelogDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_BatchPhaseExelog","runtime");
		try {
	...
	public void dealDatagridsSave(WOMBatchPhaseExelog batchPhaseExelog,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchPhaseExelogDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMBatchPhaseExelog.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchPhaseExelog);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(batchPhaseExelog.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(batchPhaseExelog.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(batchPhaseExelog.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchPhaseExelog.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchPhaseExelog.TABLE_NAME +" where valid = 1";
			list = batchPhaseExelogDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMBatchPhaseExelog.TABLE_NAME;
			list = batchPhaseExelogDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMBatchPhaseExelog.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchPhaseExelogDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMBatchPhaseExelogDaoImpl batchPhaseExelogDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMBatchPhaseExelog.TABLE_NAME + " where id = ?";
		String tableNo = batchPhaseExelogDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchBatchPhaseExelog(final List<WOMBatchPhaseExelog> insertObjs, final List<WOMBatchPhaseExelog> updateObjs, 
		...
		return this.importBatchBatchPhaseExelog(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchPhaseExelog(final List<WOMBatchPhaseExelog> insertObjs, final List<WOMBatchPhaseExelog> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1496882746569"),importCount));
		...
		Map<Long, WOMBatchPhaseExelog> oldUpdateObjsMap = new HashMap<Long, WOMBatchPhaseExelog>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMBatchPhaseExelog bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMBatchPhaseExelog> bizObjList = batchPhaseExelogDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMBatchPhaseExelog bizObj = bizObjList.get(mIndex);
		...
		final List<WOMBatchPhaseExelog> nmeObjs = new ArrayList<WOMBatchPhaseExelog>();
		...
		//String idSql = "select id from " + WOMBatchPhaseExelog.TABLE_NAME + " order by id desc";
		//Query query = batchPhaseExelogDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMBatchPhaseExelog.JPA_NAME + " order by id desc";
		Query query =  batchPhaseExelogDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMBatchPhaseExelog.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMBatchPhaseExelog batchPhaseExelog:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMBatchPhaseExelog.TABLE_NAME,1,dbId);
			if(batchPhaseExelog.getId() != null ){
				m.put(batchPhaseExelog.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchPhaseExelogDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_BatchPhaseExelog"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchPhaseExelogDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMBatchPhaseExelog.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMBatchPhaseExelog batchPhaseExelog = updateObjs.get(updateIndex);
						Long id = batchPhaseExelog.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchPhaseExelog.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchPhaseExelog);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMBatchPhaseExelog.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMBatchPhaseExelog batchPhaseExelog = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchPhaseExelog.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchPhaseExelogDaoImpl.getSession().flush();
		batchPhaseExelogDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = batchPhaseExelogDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_BatchPhaseExelog").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = batchPhaseExelogDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_BatchPhaseExelog").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMBatchPhaseExelog bizObj = insertObjs.get(insertIndex);
			batchPhaseExelogImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMBatchPhaseExelog bizObj = updateObjs.get(updateIndex);
			WOMBatchPhaseExelog oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchPhaseExelogImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMBatchPhaseExelog> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMBatchPhaseExelog> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchPhaseExelogDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMBatchPhaseExelog> result = getResult(page, query, new PendingResultTransformer(WOMBatchPhaseExelog.class,batchPhaseExelogDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMBatchPhaseExelog model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchPhaseExelogDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_BatchPhaseExelog").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchPhaseExelogDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchPhaseExelogDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchPhaseExelogDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchPhaseExelogDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchPhaseExelogDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchPhaseExelogDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="ec.flow.save")
	public void saveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, DataGridService dataGridService){
		saveBatchPhaseExelog(batchPhaseExelog, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="ec.flow.save")
	public void saveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, DataGridService dataGridService, String viewCode){
		saveBatchPhaseExelog(batchPhaseExelog, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog")
	public void saveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchPhaseExelog(batchPhaseExelog, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog")
	public void saveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != batchPhaseExelog.getId() && batchPhaseExelog.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMBatchPhaseExelog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog")
	public Map<Object, Object> batchPhaseExelogDataGridImport(WOMBatchPhaseExelog batchPhaseExelog, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchPhaseExelog.getVirtualId();
		
		batchPhaseExelog = batchPhaseExelogDao.load(batchPhaseExelog.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchPhaseExelog.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchPhaseExelog.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchPhaseExelog);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchPhaseExelog.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog")
	public void mergeBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchPhaseExelog);
		beforeSaveBatchPhaseExelog(batchPhaseExelog);
		batchPhaseExelogDao.merge(batchPhaseExelog);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_BatchPhaseExelog",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
	...
	@Override
	public Page<WOMBatchPhaseExelog> getByPage(Page<WOMBatchPhaseExelog> page,DetachedCriteria detachedCriteria){
		return batchPhaseExelogDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, Object... objects){
		checkUniqueConstraint(batchPhaseExelog);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchPhaseExelog(List<WOMBatchPhaseExelog> batchPhaseExelogs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchPhaseExelog(List<WOMBatchPhaseExelog> batchPhaseExelogs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchPhaseExelog(List<WOMBatchPhaseExelog> batchPhaseExelogs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchPhaseExelog(List<WOMBatchPhaseExelog> batchPhaseExelogs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchPhaseExelog(WOMBatchPhaseExelog batchPhaseExelog){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMBatchPhaseExelog batchPhaseExelog, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMBatchPhaseExelog batchPhaseExelog, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchPhaseExelog(List<WOMBatchPhaseExelog> insertObjs, List<WOMBatchPhaseExelog> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchPhaseExelog,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchPhaseExelog(List<WOMBatchPhaseExelog> insertObjs, List<WOMBatchPhaseExelog> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchPhaseExelog,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_BatchPhaseExelog,WOM_7.5.0.0) */
// 自定义代码
	
	/**
	 * 根据工作单元，PDA查询待投料的phase
	 * @param factoryCode
	 * @return
	 */
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<Map<String, Object>> getSpecificBatchExelog(String factoryCode) {
      SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
		String hql = "from WOMBatchPhaseExelog where valid = 1 and allowPutin = true and isFinish = false "
				+ "and mobileTerminal = true and factoryId.code in ( ";
		StringBuilder sb = new StringBuilder(hql);
		String[] params = factoryCode.split(",");
		for (String param : params) {
			sb.append("'").append(param).append("'").append(",");
		}
		sb.deleteCharAt(sb.length() - 1);
		sb.append(")");
		List<WOMBatchPhaseExelog> womBatchPhaseExelogList = batchPhaseExelogDao.createQuery(sb.toString()).list();
		List<Map<String, Object>> mapList = new ArrayList<Map<String,Object>>();
		Map<String, Object> map = null;
		if (womBatchPhaseExelogList != null) {
			for (WOMBatchPhaseExelog phaseExelog : womBatchPhaseExelogList) {
				map = new HashMap<String, Object>();
				map.put("id", phaseExelog.getId());
				if (null != phaseExelog.getFactoryId()) {
					map.put("factoryName", phaseExelog.getFactoryId().getName());
				}
				map.put("phaseID", phaseExelog.getPhaseID());
				map.put("batchNum", phaseExelog.getBatchNum());
				map.put("startTime", phaseExelog.getStartTime() == null ? "" : sdf.format(phaseExelog.getStartTime()));
				mapList.add(map);
			}
		}	
		return mapList;
	 ...
	 * 将phase进度表中的记录进行更新
	 */
	public void updateBatchPhaseState(String phaseID){
		List<WOMBatchPhaseExelog> phs=getBatchPhaseExelogs("phaseID='"+phaseID+"'", null);
		for(WOMBatchPhaseExelog ph:phs){
			ph.setIsFinish(true);
			ph.setEndTime(new Date());
