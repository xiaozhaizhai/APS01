...
@Repository
public class WOMTaskReportingDaoImpl extends ExtendedGenericDaoImpl<WOMTaskReporting, Long> implements WOMTaskReportingDao {
	@Override
	...
	@Override
	public WOMTaskReportingDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMTaskReportingDealInfo dealInfo = new WOMTaskReportingDealInfo();
	...
	@Override
	public WOMTaskReportingDealInfo copyAndSaveDealInfo(DealInfo di, WOMTaskReporting mainObj, Staff dealStaff) {
		WOMTaskReportingDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMTaskReportingDealInfo.class);
		...
		List<WOMTaskReportingDealInfo> dealinfo = criteria.list();
	...
	public void addSupervision(Supervision supervision) {
		...
		WOMTaskReportingSupervision WOMTaskReportingSupervision = new WOMTaskReportingSupervision();
		BeanUtils.copyProperties(supervision, WOMTaskReportingSupervision);
		WOMTaskReportingSupervision.setValid(true);
		getSession().save(WOMTaskReportingSupervision);
	}

	@Override
	public void addSupervision(Supervision supervision, WOMTaskReporting mainObj) {
		getSession().save(supervision);
		WOMTaskReportingSupervision localSupervision = new WOMTaskReportingSupervision();
	...
	public void deleteSupervision(Long tableInfoId, List<Long> idList) {
		...
		List<WOMTaskReportingSupervision> _supervisions = getSession().createCriteria(WOMTaskReportingSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.in("staff.id", idList))
				.add(Restrictions.eq("valid", true)).list();
		for (WOMTaskReportingSupervision item : _supervisions) {
			item.setValid(false);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMTaskReportingSupervision> findSupervision(Long tableInfoId) {
		List<WOMTaskReportingSupervision> supervisions = getSession().createCriteria(WOMTaskReportingSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("valid", true)).addOrder(Order.asc("id")).list();
		return supervisions;
	}
	@SuppressWarnings("unchecked")
	@Override
	public List<WOMTaskReportingSupervision> findSupervision(Long tableInfoId, Long StaffId) {
		List<WOMTaskReportingSupervision> supervisions = getSession().createCriteria(WOMTaskReportingSupervision.class)
				.add(Restrictions.eq("tableInfoId", tableInfoId)).add(Restrictions.eq("staff.id", StaffId)).add(Restrictions.eq("valid", true)).list();
		return supervisions;
	}
	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_taskReporting_TaskReporting,WOM_7.5.0.0) */
// 自定义代码
