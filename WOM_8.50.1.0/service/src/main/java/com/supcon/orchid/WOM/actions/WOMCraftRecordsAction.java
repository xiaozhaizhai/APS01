...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMCraftRecordsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMCraftRecords> {
	...
	private WOMCraftRecords craftRecords;
	...
	//导出excel时,主模型内容
	List<WOMCraftRecords> queryResults = new ArrayList<WOMCraftRecords>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/procFeedback/craftRecords/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/procFeedback/craftRecords/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_procFeedback_CraftRecords" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMCraftRecords> craftRecordss;
	
	public List<WOMCraftRecords> getCraftRecordss() {
		return craftRecordss;
	}
	...
	private Page<WOMCraftRecords> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			craftRecords = new WOMCraftRecords();
		} else {
			if(id > 0){
				craftRecords = craftRecordsService.getCraftRecords(id, viewCode);
			}
		}
	...
	public Page<WOMCraftRecords> getPage() {
		if (null == page) {
			page = new Page<WOMCraftRecords>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMCraftRecords getCraftRecords() {
		return craftRecords;
	}		

	public void setCraftRecords(WOMCraftRecords craftRecords) {
		this.craftRecords = craftRecords;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/procFeedback/craftRecords/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = craftRecordsService.getCraftRecordsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/procFeedback/craftRecords/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1540952446403
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String craftRecordsReferenceCopy() throws Exception{
			WOMCraftRecords oldCraftRecords = null;
			if(isEditNew()){
				...
				oldCraftRecords = new WOMCraftRecords();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,craftRecords.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, craftRecords.getId());
				...
				oldCraftRecords = craftRecordsService.getCraftRecords(id);
			}
			
			
			craftRecords = craftRecordsService.getCraftRecords(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_CraftRecords");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMCraftRecords.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCraftRecords, null);
						Method setValue = WOMCraftRecords.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(craftRecords, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			craftRecords.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/procFeedback/craftRecords/craftRecordsEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "craftRecordsEdit.ftl") }),
		@Action(value = "/WOM/procFeedback/craftRecords/craftRecordsEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "craftRecordsEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=craftRecordsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return craftRecordsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			craftRecords = craftRecordsService.getCraftRecords(this.craftRecords.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,craftRecords.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = craftRecordsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(craftRecordsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(craftRecordsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_procFeedback_CraftRecords", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"craftRecords\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"craftRecords\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"craftRecords\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"craftRecords\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			craftRecordsService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		craftRecordsService.findDataGridPage(dg,dgclass,getDgPage(),craftRecords,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(craftRecords.getId() != null && craftRecords.getId() != -1){
				craftRecords = craftRecordsService.getCraftRecords(craftRecords.getId());
			}
			craftRecordsService.findDataGridPage(dg,dgclass,getDgPage(),craftRecords,searchCondition,params);
	...
	private void save() throws Exception {
		...
		craftRecordsService.saveCraftRecords(craftRecords,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/procFeedback/craftRecords/craftRecordsEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "submit");
		bap_validate_datagrids=craftRecordsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", craftRecords.getId());
		...
		fileuploads.put("linkId", craftRecords.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/procFeedback/craftRecords/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		craftRecordsService.deleteCraftRecords(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/craftRecords/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "restore");
		craftRecordsService.restoreCraftRecords(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMCraftRecords> findExportDataInfos(Page<WOMCraftRecords> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = craftRecordsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = craftRecordsService.getAssProperty(p.getCode());	
									//String assModelCode = craftRecordsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			craftRecordsService.findCraftRecordss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			craftRecordsService.findCraftRecordss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMCraftRecords> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMCraftRecords result : results){
	...
	public boolean isEditNew(){
		//return null == this.craftRecords || null == this.craftRecords.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/procFeedback/craftRecords/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/procFeedback/craftRecords/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/procFeedback/craftRecords/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/craftRecords/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMCraftRecordsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/craftRecords/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_procFeedback_CraftRecords", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_procFeedback_CraftRecords", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/procFeedback/craftRecords/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/procFeedback/craftRecords/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_procFeedback_CraftRecords";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_procFeedback_CraftRecords", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/procFeedback/craftRecords/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMCraftRecords craftRecords = null;
		if(null != id) {
			craftRecords = craftRecordsService.getCraftRecords(id);
		}
		...
		_auditLog.setColumnStringB(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
		_auditLog.setColumnStringA(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
		_auditLog.setColumnStringC(craftRecords.getId().toString());
		_auditLog.setColumnStringD(WOMCraftRecords.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMCraftRecordsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, craftRecords,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, craftRecords,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,craftRecords);
		...
		printlog.setBusinessId(craftRecords.getId());
		printlog.setBussinessKey(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
		printlog.setMainDisplay(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/procFeedback/craftRecords/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(craftRecords,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/procFeedback/craftRecords/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(craftRecordsService.getCraftRecords(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/procFeedback/craftRecords/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/procFeedback/craftRecords/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, craftRecords,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				craftRecords = craftRecordsService.getCraftRecords(Long.parseLong(arrId[i]));
				if(null != craftRecords){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == craftRecords.getId() ? "" : "," + craftRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == craftRecords.getId() ? "" : "," + craftRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == craftRecords.getId() ? "" : "," + craftRecords.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == craftRecords.getId() ? "" : craftRecords.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, craftRecords,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,craftRecords);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/WOM/procFeedback/craftRecords/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_CraftRecords");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_procFeedback_CraftRecords")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_procFeedback_CraftRecords");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_procFeedback_CraftRecords");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = craftRecordsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_procFeedback_CraftRecords,WOM_7.5.0.0) */
// 自定义代码
