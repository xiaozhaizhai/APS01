...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMInventoryRecordAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMInventoryRecord> {
	...
	private WOMInventoryRecord inventoryRecord;
	...
	//导出excel时,主模型内容
	List<WOMInventoryRecord> queryResults = new ArrayList<WOMInventoryRecord>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/inventoryRecords/inventoryRecord/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/inventoryRecords/inventoryRecord/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_inventoryRecords_InventoryRecord" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMInventoryRecord> inventoryRecords;
	
	public List<WOMInventoryRecord> getInventoryRecords() {
	...
	private Page<WOMInventoryRecord> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			inventoryRecord = new WOMInventoryRecord();
		} else {
			if(id > 0){
				inventoryRecord = inventoryRecordService.getInventoryRecord(id, viewCode);
			}
		}
	...
	public Page<WOMInventoryRecord> getPage() {
		if (null == page) {
			page = new Page<WOMInventoryRecord>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMInventoryRecord getInventoryRecord() {
		return inventoryRecord;
	}		

	public void setInventoryRecord(WOMInventoryRecord inventoryRecord) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = inventoryRecordService.getInventoryRecordAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String inventoryRecordReferenceCopy() throws Exception{
			WOMInventoryRecord oldInventoryRecord = null;
			if(isEditNew()){
				...
				oldInventoryRecord = new WOMInventoryRecord();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,inventoryRecord.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, inventoryRecord.getId());
				...
				oldInventoryRecord = inventoryRecordService.getInventoryRecord(id);
			}
			
			
			inventoryRecord = inventoryRecordService.getInventoryRecord(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_inventoryRecords_InventoryRecord");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMInventoryRecord.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldInventoryRecord, null);
						Method setValue = WOMInventoryRecord.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(inventoryRecord, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			inventoryRecord.setCreateDepartmentId(oldInventoryRecord.getCreateDepartmentId());
			inventoryRecord.setCreatePositionId(oldInventoryRecord.getCreatePositionId());
			inventoryRecord.setCreateStaffId(oldInventoryRecord.getCreateStaffId());
			inventoryRecord.setOwnerDepartmentId(oldInventoryRecord.getOwnerDepartmentId());
			inventoryRecord.setOwnerPositionId(oldInventoryRecord.getOwnerPositionId());
			inventoryRecord.setOwnerStaffId(oldInventoryRecord.getOwnerStaffId());
			inventoryRecord.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/itemEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemEdit.ftl") }),
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/itemEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=inventoryRecordService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return inventoryRecordReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			inventoryRecord = inventoryRecordService.getInventoryRecord(this.inventoryRecord.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,inventoryRecord.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			inventoryRecordService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/itemView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemView.ftl") }),
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/itemViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/inventoryList", results = { @Result(type = FREEMARKER, location = "inventoryList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = inventoryRecordService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(inventoryRecordService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(inventoryRecordService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_inventoryRecords_InventoryRecord", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/inventoryList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.inventoryDate,result.inFlag,result.outFlag,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="inventoryList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"inventoryRecord\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"inventoryRecord\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"inventoryRecord\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"inventoryRecord\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			inventoryRecordService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/inventoryList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/allTempleteDownload", results = { @Result(name="excel",type="excel",location="inventoryList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("WOM_7.5.0.0_inventoryRecords_InventoryRecord");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		inventoryRecordService.findDataGridPage(dg,dgclass,getDgPage(),inventoryRecord,"", null);
	...
	private Page<WOMInventoryDetail> dg1566900683310Page;
	public Page<WOMInventoryDetail> getDg1566900683310Page(){
		if(null == dg1566900683310Page){
			dg1566900683310Page = new Page<WOMInventoryDetail>(1,20);
		}
		return dg1566900683310Page;
	}
	
	private Class dg1566900683310Clazz = WOMInventoryDetail.class;
	
	public Class getDg1566900683310Clazz() {
		return dg1566900683310Clazz;
	}
	
	private List<WOMInventoryDetail> dg1566900683310List;
	public List<WOMInventoryDetail> getDg1566900683310List(){
		if(null == dg1566900683310List){
			dg1566900683310List = new ArrayList<WOMInventoryDetail>();
		}
		return dg1566900683310List;
	}
	public void setDg1566900683310ListJson(String json) {
		//dg1566900683310List = (List<WOMInventoryDetail>) inventoryRecordService.generateObjectFromJson(json, WOMInventoryDetail.class);
	}
	
	private List<Long> dg1566900683310DeletedIds;
	public List<Long> getDg1566900683310DeletedIds(){
		if(null == dg1566900683310DeletedIds){
			dg1566900683310DeletedIds = new ArrayList<Long>();
		}
		return dg1566900683310DeletedIds;
	}

	private Page<WOMInventoryDetail> dg1566899530350Page;
	public Page<WOMInventoryDetail> getDg1566899530350Page(){
		if(null == dg1566899530350Page){
			dg1566899530350Page = new Page<WOMInventoryDetail>(1,20);
		}
		return dg1566899530350Page;
	}
	
	private Class dg1566899530350Clazz = WOMInventoryDetail.class;
	
	public Class getDg1566899530350Clazz() {
		return dg1566899530350Clazz;
	}
	
	private List<WOMInventoryDetail> dg1566899530350List;
	public List<WOMInventoryDetail> getDg1566899530350List(){
		if(null == dg1566899530350List){
			dg1566899530350List = new ArrayList<WOMInventoryDetail>();
		}
		return dg1566899530350List;
	}
	public void setDg1566899530350ListJson(String json) {
		//dg1566899530350List = (List<WOMInventoryDetail>) inventoryRecordService.generateObjectFromJson(json, WOMInventoryDetail.class);
	}
	
	private List<Long> dg1566899530350DeletedIds;
	public List<Long> getDg1566899530350DeletedIds(){
		if(null == dg1566899530350DeletedIds){
			dg1566899530350DeletedIds = new ArrayList<Long>();
		}
		return dg1566899530350DeletedIds;
	}

	@Actions({
		
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/data-dg1566900683310", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.manulOrder.manulOrderNum,result.material.productName,result.material.productCode,result.batchText,result.wareID.wareName,result.onhand,result.material.storeUnit.name,result.id,result.version,result.head.id,result.material.id,result.wareID.id,result.applyStaff.id,result.manulOrder.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1566900683310-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1566900683310.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/data-dg1566899530350", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.manulOrder.manulOrderNum,result.material.productName,result.material.productCode,result.batchText,result.wareID.wareName,result.onhand,result.material.storeUnit.name,result.id,result.version,result.head.id,result.material.id,result.wareID.id,result.applyStaff.id,result.manulOrder.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1566899530350-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1566899530350.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(inventoryRecord.getId() != null && inventoryRecord.getId() != -1){
				inventoryRecord = inventoryRecordService.getInventoryRecord(inventoryRecord.getId());
			}
			inventoryRecordService.findDataGridPage(dg,dgclass,getDgPage(),inventoryRecord,searchCondition,params);
	...
	private void save() throws Exception {
		...
		inventoryRecordService.saveInventoryRecord(inventoryRecord,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/WOM/inventoryRecords/inventoryRecord/itemEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/WOM/inventoryRecords/inventoryRecord/itemView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "submit");
		bap_validate_datagrids=inventoryRecordService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", inventoryRecord.getId());
		...
		fileuploads.put("linkId", inventoryRecord.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/inventoryRecords/inventoryRecord/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		inventoryRecordService.deleteInventoryRecord(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "restore");
		inventoryRecordService.restoreInventoryRecord(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/WOM/inventoryRecords/inventoryRecord/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/WOM/inventoryRecords/inventoryRecord/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "showDealInfoData");
		if (null != tableInfoId) {
			int count=inventoryRecordService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = inventoryRecordService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = inventoryRecordService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMInventoryRecord> findExportDataInfos(Page<WOMInventoryRecord> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = inventoryRecordService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = inventoryRecordService.getAssProperty(p.getCode());	
									//String assModelCode = inventoryRecordService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			inventoryRecordService.findInventoryRecords(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			inventoryRecordService.findInventoryRecords(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMInventoryRecord> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMInventoryRecord result : results){
	...
	public boolean isEditNew(){
		//return null == this.inventoryRecord || null == this.inventoryRecord.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/inventoryRecords/inventoryRecord/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/inventoryRecords/inventoryRecord/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMInventoryRecordService"
						, "com.supcon.orchid.WOM.services.WOMInventoryDetailService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("WOM_7.5.0.0_inventoryRecords", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("WOM_7.5.0.0_inventoryRecords");
		for(Model m : models){
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_inventoryRecords_InventoryRecord", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_inventoryRecords_InventoryRecord", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_inventoryRecords_InventoryRecord";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_inventoryRecords_InventoryRecord", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		WOMInventoryRecord inventoryRecord = null;
		if(null != id) {
			inventoryRecord = inventoryRecordService.getInventoryRecord(id);
		}
		...
		_auditLog.setColumnStringB(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
		_auditLog.setColumnStringA(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
		_auditLog.setColumnStringC(inventoryRecord.getId().toString());
		_auditLog.setColumnStringD(WOMInventoryRecord.MODEL_CODE);
		...
		_auditLog.setEntity("WOM_7.5.0.0_inventoryRecords");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMInventoryRecordAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, inventoryRecord,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, inventoryRecord,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,inventoryRecord);
		...
		printlog.setEntity("WOM_7.5.0.0_inventoryRecords");
		...
		printlog.setBusinessId(inventoryRecord.getId());
		printlog.setBussinessKey(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
		printlog.setMainDisplay(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(inventoryRecord,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(inventoryRecordService.getInventoryRecord(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, inventoryRecord,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				inventoryRecord = inventoryRecordService.getInventoryRecord(Long.parseLong(arrId[i]));
				if(null != inventoryRecord){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == inventoryRecord.getId() ? "" : "," + inventoryRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == inventoryRecord.getId() ? "" : "," + inventoryRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == inventoryRecord.getId() ? "" : "," + inventoryRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == inventoryRecord.getId() ? "" : inventoryRecord.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, inventoryRecord,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,inventoryRecord);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//inventoryRecordService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<WOMInventoryRecord> findExportPageInfos(Page<WOMInventoryRecord> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "WOM_7.5.0.0_inventoryRecords" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = inventoryRecordService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(WOMInventoryRecord queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = inventoryRecordService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = inventoryRecordService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_inventoryRecords_InventoryRecord");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_inventoryRecords_InventoryRecord")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_inventoryRecords_InventoryRecord");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_inventoryRecords_InventoryRecord");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = inventoryRecordService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_inventoryRecords_InventoryRecord,WOM_7.5.0.0) */
// 自定义代码
private Long recordId;
	...
	/**
	 * 根据当月领用记录生成平库记录，并调用SAP接口入库
	 * @return
	 */
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/generateInRecords", results = { @Result(type = JSON, params = { "root", "responseMap"}) })
	public String generateInRecords() throws Exception{
		inventoryRecordService.generateInRecords();
		return SUCCESS;
	}

	/**
	 * 获取上个月平库记录，调用SAP接口出库（领料）
	 * @return
	 */
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/generateOutRecords", results = { @Result(type = JSON, params = { "root", "responseMap"}) })
	public String generateOutRecords() throws Exception{
		inventoryRecordService.generateOutRecords();
		return SUCCESS;
	}
	 ...
	 * @return
	 */
	@Action(value = "/WOM/inventoryRecords/inventoryRecord/flat", results = { @Result(type = JSON, params = { "root", "responseMap"}) })
	public String flat() throws Exception{
		Map<String, String> result = inventoryRecordService.flat(recordId, operateFlag);
