...
import com.supcon.orchid.WOM.services.WOMProduceTaskProcessImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.WOM.daos.impl.WOMProduceTaskProcessDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("wOM_produceTaskProcessService")
@Transactional
public class WOMProduceTaskProcessServiceImpl  extends BaseServiceImpl<WOMProduceTaskProcess> implements WOMProduceTaskProcessService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<WOMProduceTaskProcess> bapBaseService;
	...
	@Autowired
	private WOMProduceTaskProcessImportService produceTaskProcessImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + WOMProduceTaskProcess.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  produceTaskProcessDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + WOMProduceTaskProcess.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = produceTaskProcessDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProduceTaskProcess.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + WOMProduceTaskProcess.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  produceTaskProcessDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + WOMProduceTaskProcess.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + WOMProduceTaskProcess.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<WOMProduceTaskProcess> list = (List<WOMProduceTaskProcess>) produceTaskProcessDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public WOMProduceTaskProcess getProduceTaskProcess(long id){
		return getProduceTaskProcess(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskProcess getProduceTaskProcess(long id, String viewCode){
		WOMProduceTaskProcess produceTaskProcess = produceTaskProcessDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProduceTaskProcessAsJSON(long id, String include){
		WOMProduceTaskProcess produceTaskProcess = produceTaskProcessDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(produceTaskProcess, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess){
		if(null != produceTaskProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
			AuditUtil.setColumnStringC(produceTaskProcess.getId().toString());
		}
		beforeDeleteProduceTaskProcess(produceTaskProcess);
		produceTaskProcessDao.deletePhysical(produceTaskProcess);
		afterDeleteProduceTaskProcess(produceTaskProcess);
		produceTaskProcessDao.flush();
		List<WOMProduceTaskProcess> params = new ArrayList<WOMProduceTaskProcess>();
		params.add(produceTaskProcess);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskProcess(List<Long> produceTaskProcessIds){
		deleteProduceTaskProcess(produceTaskProcessIds, null);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskProcess(List<Long> produceTaskProcessIds, String eventTopic) {
		adjustOrTempHeadService.deleteAdjustOrTempHeadByProduceTaskProcess(produceTaskProcessIds);
		batchPhaseExelogService.deleteBatchPhaseExelogByProduceTaskProcess(produceTaskProcessIds);
		prodTaskActiExelogService.deleteProdTaskActiExelogByProduceTaskProcess(produceTaskProcessIds);
		prodTaskProcExelogService.deleteProdTaskProcExelogByProduceTaskProcess(produceTaskProcessIds);
		produceTaskActiveService.deleteProduceTaskActiveByProduceTaskProcess(produceTaskProcessIds);
		waitPutinRecordsService.deleteWaitPutinRecordsByProduceTaskProcess(produceTaskProcessIds);
		List<WOMProduceTaskProcess> produceTaskProcesss = new ArrayList<WOMProduceTaskProcess>();
		for(Long produceTaskProcessId : produceTaskProcessIds){
			WOMProduceTaskProcess produceTaskProcess = getProduceTaskProcess(produceTaskProcessId);
			produceTaskProcesss.add(produceTaskProcess);
			if(null != produceTaskProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == produceTaskProcess.getId() ? "" : "," + produceTaskProcess.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == produceTaskProcess.getId() ? "" : "," + produceTaskProcess.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == produceTaskProcess.getId() ? "" : "," + produceTaskProcess.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(produceTaskProcesss != null && produceTaskProcesss.size() > 0 ){
			for(WOMProduceTaskProcess produceTaskProcess : produceTaskProcesss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = WOMProduceTaskProcess.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(produceTaskProcess));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),produceTaskProcess, produceTaskProcess.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(produceTaskProcessIds != null && produceTaskProcessIds.size() > 0) {
			String hql = "delete " + WOMProduceTaskProcess.JPA_NAME + " where id in(:ids)";
			Query query = produceTaskProcessDao.createQuery(hql);
			query.setParameterList("ids", produceTaskProcessIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(WOMProduceTaskProcess produceTaskProcessz : produceTaskProcesss){
				produceTaskProcessz.setValid(false);
				List<WOMProduceTaskProcess> params = new ArrayList<WOMProduceTaskProcess>();
				params.add(produceTaskProcessz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "WOM_7.5.0.0_produceTask");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskProcess(long produceTaskProcessId, int produceTaskProcessVersion){
		this.deleteProduceTaskProcess(produceTaskProcessId, produceTaskProcessVersion,null);
	}
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskProcess(long produceTaskProcessId, int produceTaskProcessVersion,SignatureLog signatureLog){
		WOMProduceTaskProcess produceTaskProcess = getProduceTaskProcess(produceTaskProcessId);
		...
		beforeDeleteProduceTaskProcess(produceTaskProcess);
		produceTaskProcessDao.delete(produceTaskProcessId, produceTaskProcessVersion);
		afterDeleteProduceTaskProcess(produceTaskProcess);
		produceTaskProcessDao.flush();
		List<WOMProduceTaskProcess> params = new ArrayList<WOMProduceTaskProcess>();
		params.add(produceTaskProcess);
		...
		if(signatureLog != null ) {
			...
			businessKey=produceTaskProcess.getId();
			signatureLog.setTableId(produceTaskProcess.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskProcess(String produceTaskProcessIds){
		this.deleteProduceTaskProcess(produceTaskProcessIds,null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.common.delete",operType="3")
	public void deleteProduceTaskProcess(String produceTaskProcessIds,SignatureLog signatureLog){
		deleteCollection(produceTaskProcessIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProduceTaskProcess(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:WOM_7.5.0.0#entityCode:WOM_7.5.0.0_produceTask#modelCode:WOM_7.5.0.0_produceTask_ProduceTaskProcess#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="ec.engine.restore",operType="10")
	public void restoreProduceTaskProcess(String produceTaskProcessIds){
		restoreCollection(produceTaskProcessIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProduceTaskProcess(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="ec.engine.restore",operType="10")
	public void restoreProduceTaskProcess(long produceTaskProcessId){
		
		findBusinessKeyUsed(produceTaskProcessId);	//判断业务主键是否重复
		...
		beforeRestoreProduceTaskProcess(produceTaskProcess);
		produceTaskProcess.setValid(true);
		produceTaskProcessDao.update(produceTaskProcess);
		afterRestoreProduceTaskProcess(produceTaskProcess);
		List<WOMProduceTaskProcess> params = new ArrayList<WOMProduceTaskProcess>();
		params.add(produceTaskProcess);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long produceTaskProcessId){
		Property property = modelServiceFoundation.getBussinessProperty("WOM_7.5.0.0_produceTask_ProduceTaskProcess");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + WOMProduceTaskProcess.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ WOMProduceTaskProcess.TABLE_NAME +" where id = ? )";
			List<Object> list =  produceTaskProcessDao.createNativeQuery(sql,produceTaskProcessId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProduceTaskProcess(List<WOMProduceTaskProcess>  produceTaskProcesss){
		for(WOMProduceTaskProcess produceTaskProcess:produceTaskProcesss)  {
			saveProduceTaskProcess(produceTaskProcess, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProduceTaskProcess(List<WOMProduceTaskProcess>  produceTaskProcesss){
		for(WOMProduceTaskProcess produceTaskProcess:produceTaskProcesss)  {
			saveProduceTaskProcess(produceTaskProcess, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="ec.flow.save")
	public void saveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProduceTaskProcess(produceTaskProcess, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="ec.flow.save")
	public void saveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProduceTaskProcess(produceTaskProcess, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProduceTaskProcessCodes(WOMProduceTaskProcess produceTaskProcess) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProduceTaskProcessCodes(produceTaskProcess, false);
	}
	
	public void generateProduceTaskProcessCodes(WOMProduceTaskProcess produceTaskProcess, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProduceTaskProcessSummarys(WOMProduceTaskProcess produceTaskProcess) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProduceTaskProcessSummarys(produceTaskProcess, false);
	}

	public void generateProduceTaskProcessSummarys(WOMProduceTaskProcess produceTaskProcess, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess")
	public void saveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProduceTaskProcess(produceTaskProcess, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess")
	public void saveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(produceTaskProcess.getId() != null && produceTaskProcess.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, produceTaskProcess);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(produceTaskProcess);
		beforeSaveProduceTaskProcess(produceTaskProcess, viewIsView);

		if (viewIsView) {
			produceTaskProcessDao.saveWithRevertVersion(produceTaskProcess);
		} else {
			if(isNew)
				produceTaskProcessDao.save(produceTaskProcess);
			else
				produceTaskProcessDao.merge(produceTaskProcess);
		}
		if(null != produceTaskProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
		...
		dealDatagridsSave(produceTaskProcess,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProduceTaskProcessCodes(produceTaskProcess, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProduceTaskProcessSummarys(produceTaskProcess, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProduceTaskProcess(produceTaskProcess, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, produceTaskProcess);
			}
		}
		List<WOMProduceTaskProcess> params = new ArrayList<WOMProduceTaskProcess>();
		produceTaskProcessDao.flush();
		produceTaskProcessDao.clear();
		produceTaskProcess = produceTaskProcessDao.load(produceTaskProcess.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, WOMProduceTaskProcess produceTaskProcess){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("produceTaskProcess", produceTaskProcess);
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess")
	public void mergeProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(produceTaskProcess);
		beforeSaveProduceTaskProcess(produceTaskProcess);
		produceTaskProcessDao.merge(produceTaskProcess);
		if(null != produceTaskProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
			AuditUtil.setColumnStringA(null == produceTaskProcess.getId() ? "" : produceTaskProcess.getId().toString());
		...
		dealDatagridsSave(produceTaskProcess,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProduceTaskProcess(produceTaskProcess);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<WOMProduceTaskProcess> findProduceTaskProcesss(Page<WOMProduceTaskProcess> page, Criterion... criterions) {
		return produceTaskProcessDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskProcesss(Page<WOMProduceTaskProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProduceTaskProcesss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskProcesss(Page<WOMProduceTaskProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProduceTaskProcesss(Page<WOMProduceTaskProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<WOMProduceTaskProcess> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"produceTaskProcess\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"produceTaskProcess\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"produceTaskProcess\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"produceTaskProcess\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM WOM_PRODUCE_TASKS \"produceTask\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  produceTaskProcessDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  produceTaskProcessDao.createNativeQuery(powerOperateSql, "WOM_7.5.0.0_produceTask").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"produceTaskProcess\".LAY_REC ASC, \"produceTaskProcess\".SORT ASC");
				} else {
					orderPart.append(" \"produceTaskProcess\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) produceTaskProcessDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=produceTaskProcessDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = produceTaskProcessDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<WOMProduceTaskProcess> result = new ArrayList<WOMProduceTaskProcess>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(WOMProduceTaskProcess.class,produceTaskProcessDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(WOMProduceTaskProcess.class,produceTaskProcessDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (WOMProduceTaskProcess produceTaskProcess : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(produceTaskProcess.getId(), "WOM_produceTask_produceTaskProcess");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(produceTaskProcess.getId(), "WOM_produceTask_produceTaskProcess");
							if(null != documents && !documents.isEmpty()) {
								produceTaskProcess.setDocument(documents.get(0));
								produceTaskProcess.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<WOMProduceTaskProcess> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<WOMProduceTaskProcess> result) {
		View view = null;
		...
		Map<Long, WOMProduceTaskProcess> rsMap = new HashMap<Long, WOMProduceTaskProcess>();
		for (WOMProduceTaskProcess produceTaskProcess : result) {
			ids.add(produceTaskProcess.getId());
			rsMap.put(produceTaskProcess.getId(), produceTaskProcess);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(produceTaskProcessDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = produceTaskProcessDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "WOM_7.5.0.0_produceTask_ProduceTaskProcess" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="WOM_7.5.0.0_produceTask_ProduceTaskProcess" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  produceTaskProcessDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceTaskProcess\"","WOM_7.5.0.0_produceTask","","WOM_PRODUCE_TASK_PROCESSES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"produceTaskProcess\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"produceTaskProcess\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"produceTaskProcess\"","WOM_7.5.0.0_produceTask","","WOM_PRODUCE_TASK_PROCESSES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "WOM_7.5.0.0_produceTask_ProduceTaskProcess" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("produceTaskProcess.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"produceTaskProcess\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"produceTaskProcess\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"produceTaskProcess\".CID = 1 OR \"produceTaskProcess\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("produceTaskProcess.mc\"").append(".PRODUCE_TASK_PROCESS) not in ("+notinValue+")");
			}
			
			SQLQuery query = produceTaskProcessDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=produceTaskProcessDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return WOMProduceTaskProcess.ENTITY_CODE;}
	protected String getModuleCode(){return WOMProduceTaskProcess.MODULE_CODE;}
	protected String getModelCode(){return WOMProduceTaskProcess.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param produceTaskProcess 模型实例
	 */
	public void checkUniqueConstraint(WOMProduceTaskProcess produceTaskProcess){
	}
	
	
	@Override
	public List<WOMProduceTaskProcess> getProduceTaskProcesss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskProcess.class);
		if(params!=null){
			...
			return produceTaskProcessDao.findByHql("from " + WOMProduceTaskProcess.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return produceTaskProcessDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<WOMProduceTaskProcess> findProduceTaskProcesssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskProcess.class);
	...
	@Override
	public List<WOMProduceTaskProcess> findProduceTaskProcesssByHql(String hql, Object... objects){
		
		return   produceTaskProcessDao.findByHql(hql, objects);
	}
	
	@Override
	public WOMProduceTaskProcess loadProduceTaskProcessByBussinessKey(WOMProduceTaskProcess produceTaskProcess){
		return loadProduceTaskProcessByBussinessKey(produceTaskProcess.getId());
	}
	
	@Override
	public Page<WOMProduceTaskProcess> getProduceTaskProcesss(Page<WOMProduceTaskProcess> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(WOMProduceTaskProcess.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = produceTaskProcessDao.findByPage(page, sql);
			}else{
				page = produceTaskProcessDao.findAllByPage(page);
			}
			return page;
		}
		page = produceTaskProcessDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProduceTaskProcess> findByTreeDataGridProduceTask(List<WOMProduceTaskProcess> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(WOMProduceTaskProcess produceTaskProcess : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduceTask(Page<WOMProduceTaskProcess> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<WOMProduceTaskProcess> produceTaskProcesss = dgPage.getResult();
			for(WOMProduceTaskProcess produceTaskProcess : produceTaskProcesss){
				
			}
			dgPage.setResult(produceTaskProcesss);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskProcess loadProduceTaskProcessByBussinessKey(Serializable bussinessKey){
		return produceTaskProcessDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<WOMProduceTaskProcess> findByProperty(String propertyName, Object object){
		return produceTaskProcessDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public WOMProduceTaskProcess findEntityByProperty(String propertyName, Object object){
		return produceTaskProcessDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + WOMProduceTaskProcess.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = produceTaskProcessDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("WOM_7.5.0.0_produceTask_ProduceTaskProcess","runtime");
		try {
	...
	public void dealDatagridsSave(WOMProduceTaskProcess produceTaskProcess,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,produceTaskProcessDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), WOMProduceTaskProcess.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, produceTaskProcess);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(produceTaskProcess.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(produceTaskProcess.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(produceTaskProcess.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",produceTaskProcess.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProduceTaskProcess.TABLE_NAME +" where valid = 1";
			list = produceTaskProcessDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ WOMProduceTaskProcess.TABLE_NAME;
			list = produceTaskProcessDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ WOMProduceTaskProcess.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = produceTaskProcessDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private WOMProduceTaskProcessDaoImpl produceTaskProcessDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + WOMProduceTaskProcess.TABLE_NAME + " where id = ?";
		String tableNo = produceTaskProcessDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchProduceTaskProcess(final List<WOMProduceTaskProcess> insertObjs, final List<WOMProduceTaskProcess> updateObjs, 
		...
		return this.importBatchProduceTaskProcess(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProduceTaskProcess(final List<WOMProduceTaskProcess> insertObjs, final List<WOMProduceTaskProcess> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("WOM.modelname.randon1489671052624"),importCount));
		...
		Map<Long, WOMProduceTaskProcess> oldUpdateObjsMap = new HashMap<Long, WOMProduceTaskProcess>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			WOMProduceTaskProcess bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<WOMProduceTaskProcess> bizObjList = produceTaskProcessDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					WOMProduceTaskProcess bizObj = bizObjList.get(mIndex);
		...
		final List<WOMProduceTaskProcess> nmeObjs = new ArrayList<WOMProduceTaskProcess>();
		...
		//String idSql = "select id from " + WOMProduceTaskProcess.TABLE_NAME + " order by id desc";
		//Query query = produceTaskProcessDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + WOMProduceTaskProcess.JPA_NAME + " order by id desc";
		Query query =  produceTaskProcessDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(WOMProduceTaskProcess.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(WOMProduceTaskProcess produceTaskProcess:insertObjs)  {
			//id = autoGeneratorID.getNextId(WOMProduceTaskProcess.TABLE_NAME,1,dbId);
			if(produceTaskProcess.getId() != null ){
				m.put(produceTaskProcess.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) produceTaskProcessDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProduceTaskProcess"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		produceTaskProcessDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + WOMProduceTaskProcess.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						WOMProduceTaskProcess produceTaskProcess = updateObjs.get(updateIndex);
						Long id = produceTaskProcess.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = produceTaskProcess.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(produceTaskProcess);
							...
							nmeObjs.add(produceTaskProcess);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + WOMProduceTaskProcess.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						WOMProduceTaskProcess produceTaskProcess = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = produceTaskProcess.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		produceTaskProcessDaoImpl.getSession().flush();
		produceTaskProcessDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = produceTaskProcessDao.createNativeQuery(sqlQueryBussinesskey, "WOM_7.5.0.0_produceTask_ProduceTaskProcess").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = produceTaskProcessDao.createNativeQuery(sqlQueryMainDispaly, "WOM_7.5.0.0_produceTask_ProduceTaskProcess").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			WOMProduceTaskProcess bizObj = insertObjs.get(insertIndex);
			produceTaskProcessImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			WOMProduceTaskProcess bizObj = updateObjs.get(updateIndex);
			WOMProduceTaskProcess oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			produceTaskProcessImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<WOMProduceTaskProcess> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<WOMProduceTaskProcess> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) produceTaskProcessDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<WOMProduceTaskProcess> result = getResult(page, query, new PendingResultTransformer(WOMProduceTaskProcess.class,produceTaskProcessDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(WOMProduceTaskProcess model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = produceTaskProcessDao.createNativeQuery(sql, "WOM_7.5.0.0_produceTask_ProduceTaskProcess").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = produceTaskProcessDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = produceTaskProcessDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = produceTaskProcessDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?produceTaskProcessDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  produceTaskProcessDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,produceTaskProcessDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="ec.flow.save")
	public void saveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, DataGridService dataGridService){
		saveProduceTaskProcess(produceTaskProcess, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="ec.flow.save")
	public void saveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, DataGridService dataGridService, String viewCode){
		saveProduceTaskProcess(produceTaskProcess, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess")
	public void saveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProduceTaskProcess(produceTaskProcess, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess")
	public void saveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
		String url = null;
		if(null != produceTaskProcess.getId() && produceTaskProcess.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || WOMProduceTaskProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess")
	public Map<Object, Object> produceTaskProcessDataGridImport(WOMProduceTaskProcess produceTaskProcess, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = produceTaskProcess.getVirtualId();
		
		produceTaskProcess = produceTaskProcessDao.load(produceTaskProcess.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, produceTaskProcess.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = produceTaskProcess.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(produceTaskProcess);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, produceTaskProcess.getId());
		}
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess")
	public void mergeProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(produceTaskProcess);
		beforeSaveProduceTaskProcess(produceTaskProcess);
		produceTaskProcessDao.merge(produceTaskProcess);
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="WOM_7.5.0.0_produceTask",model="WOM_7.5.0.0_produceTask_ProduceTaskProcess",desc="foundation.batch.save",operType="4")
	public void batchSaveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "WOM_7.5.0.0_produceTask";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "WOM_7.5.0.0_produceTask");
	...
	@Override
	public Page<WOMProduceTaskProcess> getByPage(Page<WOMProduceTaskProcess> page,DetachedCriteria detachedCriteria){
		return produceTaskProcessDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, Object... objects){
		checkUniqueConstraint(produceTaskProcess);
	/* CUSTOM CODE START(serviceimpl,beforeSave,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProduceTaskProcess(List<WOMProduceTaskProcess> produceTaskProcesss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProduceTaskProcess(List<WOMProduceTaskProcess> produceTaskProcesss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProduceTaskProcess(List<WOMProduceTaskProcess> produceTaskProcesss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProduceTaskProcess(List<WOMProduceTaskProcess> produceTaskProcesss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess){
	/* CUSTOM CODE START(serviceimpl,afterDelete,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProduceTaskProcess(WOMProduceTaskProcess produceTaskProcess){
	/* CUSTOM CODE START(serviceimpl,afterRestore,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(WOMProduceTaskProcess produceTaskProcess, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(WOMProduceTaskProcess produceTaskProcess, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProduceTaskProcess(List<WOMProduceTaskProcess> insertObjs, List<WOMProduceTaskProcess> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProduceTaskProcess,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProduceTaskProcess(List<WOMProduceTaskProcess> insertObjs, List<WOMProduceTaskProcess> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProduceTaskProcess,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,WOM_7.5.0.0_produceTask_ProduceTaskProcess,WOM_7.5.0.0) */
// 自定义代码
