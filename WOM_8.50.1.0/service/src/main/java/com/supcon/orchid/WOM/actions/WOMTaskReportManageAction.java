...
/* CUSTOM CODE START(action,import,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class WOMTaskReportManageAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<WOMTaskReportManage> {
	...
	private WOMTaskReportManage taskReportManage;
	...
	//导出excel时,主模型内容
	List<WOMTaskReportManage> queryResults = new ArrayList<WOMTaskReportManage>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/taskReportManage/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/WOM/generalManage/taskReportManage/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/WOM_7.5.0.0_generalManage_TaskReportManage" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<WOMTaskReportManage> taskReportManages;
	
	public List<WOMTaskReportManage> getTaskReportManages() {
		return taskReportManages;
	}
	...
	private Page<WOMTaskReportManage> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			taskReportManage = new WOMTaskReportManage();
		} else {
			if(id > 0){
				taskReportManage = taskReportManageService.getTaskReportManage(id, viewCode);
			}
		}
	...
	public Page<WOMTaskReportManage> getPage() {
		if (null == page) {
			page = new Page<WOMTaskReportManage>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public WOMTaskReportManage getTaskReportManage() {
		return taskReportManage;
	}		

	public void setTaskReportManage(WOMTaskReportManage taskReportManage) {
		this.taskReportManage = taskReportManage;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/taskReportManage/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = taskReportManageService.getTaskReportManageAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/WOM/generalManage/taskReportManage/edit-extra-col")
	...
	/**
	 * WOM.modelname.randon1502698712286
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String taskReportManageReferenceCopy() throws Exception{
			WOMTaskReportManage oldTaskReportManage = null;
			if(isEditNew()){
				...
				oldTaskReportManage = new WOMTaskReportManage();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskReportManage.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, taskReportManage.getId());
				...
				oldTaskReportManage = taskReportManageService.getTaskReportManage(id);
			}
			
			
			taskReportManage = taskReportManageService.getTaskReportManage(refId);
			Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_TaskReportManage");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = WOMTaskReportManage.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTaskReportManage, null);
						Method setValue = WOMTaskReportManage.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(taskReportManage, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			taskReportManage.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=taskReportManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return taskReportManageReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			taskReportManage = taskReportManageService.getTaskReportManage(this.taskReportManage.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,taskReportManage.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			taskReportManageService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = taskReportManageService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(taskReportManageService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(taskReportManageService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("WOM_7.5.0.0_generalManage_TaskReportManage", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"taskReportManage\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"taskReportManage\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"taskReportManage\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"taskReportManage\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			taskReportManageService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		taskReportManageService.findDataGridPage(dg,dgclass,getDgPage(),taskReportManage,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(taskReportManage.getId() != null && taskReportManage.getId() != -1){
				taskReportManage = taskReportManageService.getTaskReportManage(taskReportManage.getId());
			}
			taskReportManageService.findDataGridPage(dg,dgclass,getDgPage(),taskReportManage,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == taskReportManage.getId()) {
		...
		taskReportManageService.saveTaskReportManage(taskReportManage,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "submit");
		bap_validate_datagrids=taskReportManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", taskReportManage.getId());
		...
		fileuploads.put("linkId", taskReportManage.getId());
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "submit");
		return SUCCESS;
	}

	@Action(value = "/WOM/generalManage/taskReportManage/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		taskReportManageService.deleteTaskReportManage(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/taskReportManage/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "restore");
		taskReportManageService.restoreTaskReportManage(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<WOMTaskReportManage> findExportDataInfos(Page<WOMTaskReportManage> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = taskReportManageService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = taskReportManageService.getAssProperty(p.getCode());	
									//String assModelCode = taskReportManageService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			taskReportManageService.findTaskReportManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			taskReportManageService.findTaskReportManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<WOMTaskReportManage> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(WOMTaskReportManage result : results){
	...
	public boolean isEditNew(){
		//return null == this.taskReportManage || null == this.taskReportManage.getId();
		return null == this.id;
	}
	...
	@Action(value = "/WOM/generalManage/taskReportManage/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "attachmentInit");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/WOM/generalManage/taskReportManage/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/WOM/generalManage/taskReportManage/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/taskReportManage/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMTaskReportManageService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "imports");
		...
		bapAfterCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/taskReportManage/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.WOM.services.WOMTaskReportManageService"
						, "com.supcon.orchid.WOM.services.WOMProduceTaskManageService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "WOM_7.5.0.0_generalManage_TaskReportManage", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "WOM_7.5.0.0_generalManage_TaskReportManage", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/WOM/generalManage/taskReportManage/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/WOM/generalManage/taskReportManage/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="WOM_7.5.0.0_generalManage_TaskReportManage";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("WOM_7.5.0.0_generalManage_TaskReportManage", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/WOM/generalManage/taskReportManage/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
		_auditLog.setColumnStringA(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
		_auditLog.setColumnStringC(taskReportManage.getId().toString());
		_auditLog.setColumnStringD(WOMTaskReportManage.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.WOM.actions.WOMTaskReportManageAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, taskReportManage,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, taskReportManage,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,taskReportManage);
		...
		printlog.setBusinessId(taskReportManage.getId());
		printlog.setBussinessKey(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
		printlog.setMainDisplay(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/WOM/generalManage/taskReportManage/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(taskReportManage,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/WOM/generalManage/taskReportManage/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(taskReportManageService.getTaskReportManage(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/WOM/generalManage/taskReportManage/loadBatchXmlRefer",
	...
	@Action(value = "/WOM/generalManage/taskReportManage/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, taskReportManage,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				taskReportManage = taskReportManageService.getTaskReportManage(Long.parseLong(arrId[i]));
				if(null != taskReportManage){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == taskReportManage.getId() ? "" : "," + taskReportManage.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == taskReportManage.getId() ? "" : "," + taskReportManage.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == taskReportManage.getId() ? "" : "," + taskReportManage.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == taskReportManage.getId() ? "" : taskReportManage.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, taskReportManage,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,taskReportManage);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//taskReportManageService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/WOM/generalManage/taskReportManage/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_TaskReportManage");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("WOM_7.5.0.0_generalManage_TaskReportManage")!=null  ){
			model = (Model) excelExportModels.get("WOM_7.5.0.0_generalManage_TaskReportManage");
		}else{
			model = modelServiceFoundation.getModel("WOM_7.5.0.0_generalManage_TaskReportManage");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = taskReportManageService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,WOM_7.5.0.0_generalManage_TaskReportManage,WOM_7.5.0.0) */
// 自定义代码
