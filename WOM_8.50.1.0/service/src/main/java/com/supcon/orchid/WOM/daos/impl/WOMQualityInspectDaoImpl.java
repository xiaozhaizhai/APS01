...
@Repository
public class WOMQualityInspectDaoImpl extends ExtendedGenericDaoImpl<WOMQualityInspect, Long> implements WOMQualityInspectDao {
	@Override
	...
	@Override
	public WOMQualityInspectDealInfo copyAndSaveDealInfo(DealInfo di) {
		WOMQualityInspectDealInfo dealInfo = new WOMQualityInspectDealInfo();
	...
	@Override
	public WOMQualityInspectDealInfo copyAndSaveDealInfo(DealInfo di, WOMQualityInspect mainObj, Staff dealStaff) {
		WOMQualityInspectDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(WOMQualityInspectDealInfo.class);
		...
		List<WOMQualityInspectDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,WOM_7.5.0.0_qualityRecords_QualityInspect,WOM_7.5.0.0) */
// 自定义代码
