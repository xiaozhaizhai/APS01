...
<#assign viewCode = 'WOM_7.5.0.0_inventoryRecords_InventoryDetail' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="manulOrder.manulOrderNum">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		...
	  		<Style ss:ID="material.storeUnit.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="onhand">
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'manulOrder.manulOrderNum' || item == '*'>
											...
											<#if item == 'material.storeUnit.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'batchText' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertyshowName.randon1566867607924.flag')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'wareID.wareName' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertyshowName.randon1566867588172.flag')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'onhand' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1566803719573')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'material.storeUnit.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertyshowName.randon1566809784054.flag')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												<#if item == 'manulOrder.manulOrderNum' || item == '*'>
													<Cell>
														...
														<#if data.manulOrder.manulOrderNum??><Data ss:Type="String"><![CDATA[${(data.manulOrder.manulOrderNum)!}]]></Data></#if>
												...
												<#if item == 'material.productName' || item == '*'>
													<Cell>
														...
														<#if data.material.productName??><Data ss:Type="String"><![CDATA[${(data.material.productName)!}]]></Data></#if>
												...
												<#if item == 'material.storeUnit.name' || item == '*'>
													<Cell>
														...
														<#if data.material.storeUnit.name??><Data ss:Type="String"><![CDATA[${(data.material.storeUnit.name)!}]]></Data></#if>
