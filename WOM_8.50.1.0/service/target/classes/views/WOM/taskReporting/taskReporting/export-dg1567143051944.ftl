 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'modifyNum' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1566214136861')}</Data>
										</Cell>
								</#if>
								<#if item == 'content' || item == '*'>
										<Cell>
								...
								<#if item == 'modifyCont' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1566214257744')}</Data>
										</Cell>
								</#if>
								<#assign customGeneratedFlag=false />
									<#if item == 'modifyCont' && !customGeneratedFlag >
