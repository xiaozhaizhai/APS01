 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'reportNum' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1492079279822')}</Data>
										</Cell>
								</#if>
								<#if item == 'storeID.placeSetCode' || item == '*'>
										<Cell>
								...
								<#if item == 'content' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1565060960816')}</Data>
										</Cell>
								</#if>
								<#if item == 'storeID.placeSetName' || item == '*'>
										<Cell>
