 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="storeID.placeSetName">
  			...
  			<Protection/>
			 						    	...
			 						    	<#assign customPropList = showCusProps.get("taskReportingPart")  />
											<#-- 排布自定义字段 -->
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								<#if item == 'quality' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1490773208429')}</Data>
										</Cell>
								</#if>
								<#if item == 'storeID.placeSetCode' || item == '*'>
										<Cell>
			...
			<Row ss:AutoFitHeight="0">
				<#list exportFieldList as item>
					<#if item?length gt 0>
								<#if item == 'quality' || item == '*'>
									<Cell>
									<#if checkFieldPermission('quality','WOM_7.5.0.0_taskReporting_TaskReportingPart','WOM_7.5.0.0_taskReporting_TaskReportingPart_quality') != 0>
										<#if (data.quality)?has_content><Data ss:Type="Number">#{(data.quality)!; m6M6}</Data></#if>
									</#if>
									</Cell>
								</#if>
								<#if item == 'storeID.placeSetCode' || item == '*'>
									<Cell>
									<#if checkFieldPermission('storeID.placeSetCode','WOM_7.5.0.0_taskReporting_TaskReportingPart','WOM_7.5.0.0_taskReporting_TaskReportingPart_storeID||MESBasic_1_storeSet_StoreSet_placeSetCode') != 0>
								...
								<#if item == 'storeID.placeSetName' || item == '*'>
									<Cell>
									<#if checkFieldPermission('storeID.placeSetName','WOM_7.5.0.0_taskReporting_TaskReportingPart','WOM_7.5.0.0_taskReporting_TaskReportingPart_storeID||MESBasic_1_storeSet_StoreSet_placeSetName') != 0>
								...
								<#if item == 'batchNum' || item == '*'>
									<Cell>
									<#if checkFieldPermission('batchNum','WOM_7.5.0.0_taskReporting_TaskReportingPart','WOM_7.5.0.0_taskReporting_TaskReportingPart_batchNum') != 0>
								...
								<#if item == 'remark' || item == '*'>
									<Cell>
									<#if checkFieldPermission('remark','WOM_7.5.0.0_taskReporting_TaskReportingPart','WOM_7.5.0.0_taskReporting_TaskReportingPart_remark') != 0>
