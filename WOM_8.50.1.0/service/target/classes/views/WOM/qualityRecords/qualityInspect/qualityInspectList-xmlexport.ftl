...
<#assign viewCode = 'WOM_7.5.0.0_qualityRecords_QualityInspect' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="activeID.orderProcessId.headId.productBatchNum">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="remark">
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'activeID.orderProcessId.headId.productBatchNum' || item == '*'>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'remark' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1541146864746')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												<#if item == 'activeID.orderProcessId.headId.productBatchNum' || item == '*'>
													<Cell>
														...
														<#if data.activeID.orderProcessId.headId.productBatchNum??><Data ss:Type="String"><![CDATA[${(data.activeID.orderProcessId.headId.productBatchNum)!}]]></Data></#if>
												...
												<#if item == 'activeID.name' || item == '*'>
													<Cell>
														...
														<#if data.activeID.name??><Data ss:Type="String"><![CDATA[${(data.activeID.name)!}]]></Data></#if>
