 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="ware.wareCode">
  			...
  			<Protection/>
			 						    	...
			 						    	<#assign customPropList = showCusProps.get("prepareMaterialPart")  />
											<#-- 排布自定义字段 -->
					...
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#assign customGeneratedFlag=false />
										<#if item == 'prepareNum' && !customGeneratedFlag >
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'useNum' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertyshowName.randon1566367515055.flag')}</Data>
										</Cell>
								</#if>
								<#if item == 'hasDoneNum' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertyshowName.randon1567751630613.flag')}</Data>
										</Cell>
								</#if>
								<#if item == 'productId.productBaseUnit.name' || item == '*'>
										<Cell>
								...
								<#if item == 'prepareNum' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1489977119890')}</Data>
										</Cell>
								</#if>
								<#assign customGeneratedFlag=false />
									<#if item == 'prepareNum' && !customGeneratedFlag >
