...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="applyDate">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="applyStaff.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'applyStaff.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'applyDate' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											...
											<#if item == 'wareID.wareName' || item == '*'>
			 						        ...
			 						        <#assign customGeneratedFlag=false />
												<#if item == 'wareID.wareName' && !customGeneratedFlag >
												</#if>
											<#if item == 'remark' || item == '*'>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'applyStaff.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1492392014124')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'applyDate' || item == '*'>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'remark' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertyshowName.randon1566464114370.flag')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'backFlag' || item == '*'>
									...
									<#if item?length gt 0>
												...
												<#if item == 'applyStaff.name' || item == '*'>
													<Cell>
														...
														<#if data.applyStaff.name??><Data ss:Type="String"><![CDATA[${(data.applyStaff.name)!}]]></Data></#if>
												...
												<#if item == 'applyDate' || item == '*'>
													<Cell ss:StyleID="s8">
														...
														<#if data.applyDate??><Data ss:Type="DateTime">${(data.applyDate)!?string("yyyy-MM-dd'T'HH:mm:ss.SSS")}</Data></#if>
