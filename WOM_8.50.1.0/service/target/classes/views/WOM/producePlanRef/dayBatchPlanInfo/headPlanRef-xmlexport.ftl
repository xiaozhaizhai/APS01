...
<#assign viewCode = 'WOM_7.5.0.0_producePlanRef_DayBatchPlanInfo' />
<#assign propertyCode = '' />
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'reportDeptID.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'reportDeptID.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.fzwdy.xm.mainPositionId.department.name56')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'createTime' || item == '*'>
									...
									<#if item?length gt 0>
												...
												<#if item == 'reportDeptID.name' || item == '*'>
													<Cell>
														...
														<#if data.reportDeptID.name??><Data ss:Type="String"><![CDATA[${(data.reportDeptID.name)!}]]></Data></#if>
