			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'onhand' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertyshowName.randon1566782448157.flag')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'batchText' || item == '*'>
									...
									<#if item?length gt 0>
												...
												<#if item == 'good.productBaseUnit.name' || item == '*'>
													<Cell>
														...
														<#if data.good.productBaseUnit.name??><Data ss:Type="String"><![CDATA[${(data.good.productBaseUnit.name)!}]]></Data></#if>
