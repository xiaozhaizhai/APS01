...
<head>
	...
	<body>
		...
		<@s.form errorBarId="workbenchErrorBar" hiddenField="" onsubmitMethod="WOM.generalManage.produceTaskManage.generalDetail.beforeSubmitMethod()" id="WOM_generalManage_produceTaskManage_generalDetail_form" namespace="/WOM/generalManage/produceTaskManage/generalDetail" action="submit?__pc__=${(Parameters.__pc__)!}&_bapFieldPermissonModelCode_=WOM_7.5.0.0_generalManage_ProduceTaskManage&_bapFieldPermissonModelName_=ProduceTaskManage" callback="WOM.generalManage.produceTaskManage.generalDetail._callBackInfo" ecform="true">
		...
		<div style="float:none;width:auto;" class="edit-main grid-s5m0 col-main main-wrap" id="WOM_generalManage_produceTaskManage_generalDetail_main_div">
    					...
    					<div class="edit-main" id="WOM_generalManage_produceTaskManage_generalDetail">
    						<div class="edit-content">
								...
								<input type="hidden" name="viewselect" value="generalDetail">
								<input type="hidden" name="datagridKey" value="WOM_generalManage_produceTaskManage_generalDetail_datagrids">
								<input type="hidden" name="viewCode" value="WOM_7.5.0.0_generalManage_generalDetail">
								<input type="hidden" name="modelName" value="ProduceTaskManage">
								<input type="hidden" id="id" name="id" value="${id!}" />
								<input type="hidden" name="produceTaskManage.version" value="${(produceTaskManage.version)!0}" />
								<input type="hidden" name="produceTaskManage.extraCol" value='' />
...
<div class="edit-panes-s" >
			...
			<#if newObj?? && !newObj && !(refId?? && (refId > 0))>
				<#assign buttons="[{text:\"${getText('ec.view.addline')}\",handler:function(event){this.addNewRow(event);},iconClass:\"cui-btn-add\",useInMore:\"false\"},{text:\"${getText('ec.view.deleteline')}\",iconClass:\"cui-btn-del\",useInMore:\"false\",handler:function(event){DT_PutInManage_dg1502700158273_deldatagrid();}}]">
...
<div class="edit-panes-s" >
			...
			<#if newObj?? && !newObj && !(refId?? && (refId > 0))>
				<#assign buttons="[{text:\"${getText('ec.view.addline')}\",handler:function(event){this.addNewRow(event);},iconClass:\"cui-btn-add\",useInMore:\"false\"},{text:\"${getText('ec.view.deleteline')}\",iconClass:\"cui-btn-del\",useInMore:\"false\",handler:function(event){DT_CheckManage_dg1502700453811_deldatagrid();}}]">
				<#assign canImportExcel=false>
			<#else>
				<#assign buttons="[{text:\"${getText('ec.view.addline')}\",handler:function(event){this.addNewRow(event);},iconClass:\"cui-btn-add\",useInMore:\"false\"},{text:\"${getText('ec.view.deleteline')}\",iconClass:\"cui-btn-del\",useInMore:\"false\",handler:function(event){DT_CheckManage_dg1502700453811_deldatagrid();}}]">
				<#assign canImportExcel=false>
			...
			<@datagrid withoutConfigTable=true id="CheckManage_dg1502700453811" viewType="${viewType}" renderOverEvent="dg1502700453811RenderOverEvent" route="${routeFlag}" formId="WOM_generalManage_produceTaskManage_generalDetail_form" noPermissionKeys="inspect.name,inspect.url,chenkReport.name,chenkReport.url,product.productCode,product.productName,batchNum,standard,result,inspectDept.name,checkDept.name,inspectTime,checkTime" modelCode="WOM_7.5.0.0_generalManage_CheckManage" dataUrl="${dUrl}" postData="&dgPage.pageSize=65536" dataGridName="dg1502700453811" dtPage="dgPage"  hidekeyPrefix="dg1502700453811" hidekey="['id','version','inspect.id','inspect.name','inspect.id','inspect.url','chenkReport.id','chenkReport.name','chenkReport.id','chenkReport.url','product.id','product.productCode','product.id','product.productName','inspectDept.id','inspectDept.name','checkDept.id','checkDept.name'${cpObjHideKey!''}]" transMethod="post" paginator=false editable=false  noPadding=true isEdit=true tabViewIndex=1 pageInitMethod="dg1502700453811PageInitMethod"  defaultNodeExpanded=nodeExpanded  cannotAddNewRow=false cannotDeleteRow=true assModelDisplayName=assModelDisplayName  activexButton="" exportExcel=exportExcel addNewRow=true   canInsertRow=false  insertRowAble=false firstLoad=firstLoad >		
							<#assign inspect_name_displayDefaultType=''>
								<#assign inspect_name_defaultValue=''>
										<#assign inspect_name_defaultValue=''>
																	<#if (inspect_name_defaultValue!)?string=="currentUser">
								<#assign inspect_name_defaultValue='${staffJson!}'>
							<#elseif (inspect_name_defaultValue!)?string=="currentPost">
								<#assign inspect_name_defaultValue='${postJson!}'>
							<#elseif (inspect_name_defaultValue!)?string=="currentDepart">
								<#assign inspect_name_defaultValue='${deptJson!}'>
							<#elseif (inspect_name_defaultValue!)?string=="currentComp">
								<#assign inspect_name_defaultValue='${compJson!}'>
							</#if>

									<@datacolumn key="inspect.name"        showFormat="SELECTCOMP" defaultValue="${(inspect_name_defaultValue!)?string}" defaultDisplay="${(inspect_name_displayDefaultType!)?string}" decimal="" editable=false type="selectcomp" showType="selectcomp" notnull=false     textalign="left"  viewUrl="" viewCode="" mneenable=false crossCompany=false viewTitle="${getText('')}"  label="${getText('WOM.propertydisplayName.randon1502698838092')}" width=100 showFormatFunc=""/>
							<#assign inspect_url_displayDefaultType=''>
								<#assign inspect_url_defaultValue=''>
										<#assign inspect_url_defaultValue=''>
																	<#if (inspect_url_defaultValue!)?string=="currentUser">
								<#assign inspect_url_defaultValue='${staffJson!}'>
							<#elseif (inspect_url_defaultValue!)?string=="currentPost">
								<#assign inspect_url_defaultValue='${postJson!}'>
							<#elseif (inspect_url_defaultValue!)?string=="currentDepart">
								<#assign inspect_url_defaultValue='${deptJson!}'>
							<#elseif (inspect_url_defaultValue!)?string=="currentComp">
								<#assign inspect_url_defaultValue='${compJson!}'>
							</#if>

									<@datacolumn key="inspect.url"        showFormat="TEXT" defaultValue="${(inspect_url_defaultValue!)?string}" defaultDisplay="${(inspect_url_displayDefaultType!)?string}" decimal="" editable=false type="textfield" showType="textfield" notnull=false     textalign="center" hiddenCol=true viewUrl="" viewCode="" mneenable=false crossCompany=false viewTitle="${getText('')}"  label="${getText('WOM.propertydisplayName.randon1502698875134')}" width=100 showFormatFunc=""/>
							<#assign chenkReport_name_displayDefaultType=''>
								<#assign chenkReport_name_defaultValue=''>
										<#assign chenkReport_name_defaultValue=''>
																	<#if (chenkReport_name_defaultValue!)?string=="currentUser">
								<#assign chenkReport_name_defaultValue='${staffJson!}'>
							<#elseif (chenkReport_name_defaultValue!)?string=="currentPost">
								<#assign chenkReport_name_defaultValue='${postJson!}'>
							<#elseif (chenkReport_name_defaultValue!)?string=="currentDepart">
								<#assign chenkReport_name_defaultValue='${deptJson!}'>
							<#elseif (chenkReport_name_defaultValue!)?string=="currentComp">
								<#assign chenkReport_name_defaultValue='${compJson!}'>
							</#if>

									<@datacolumn key="chenkReport.name"        showFormat="SELECTCOMP" defaultValue="${(chenkReport_name_defaultValue!)?string}" defaultDisplay="${(chenkReport_name_displayDefaultType!)?string}" decimal="" editable=false type="selectcomp" showType="selectcomp" notnull=false     textalign="left"  viewUrl="" viewCode="" mneenable=false crossCompany=false viewTitle="${getText('')}"  label="${getText('WOM.propertydisplayName.randon1502698946470')}" width=100 showFormatFunc=""/>
							<#assign chenkReport_url_displayDefaultType=''>
								<#assign chenkReport_url_defaultValue=''>
										<#assign chenkReport_url_defaultValue=''>
																	<#if (chenkReport_url_defaultValue!)?string=="currentUser">
								<#assign chenkReport_url_defaultValue='${staffJson!}'>
							<#elseif (chenkReport_url_defaultValue!)?string=="currentPost">
								<#assign chenkReport_url_defaultValue='${postJson!}'>
							<#elseif (chenkReport_url_defaultValue!)?string=="currentDepart">
								<#assign chenkReport_url_defaultValue='${deptJson!}'>
							<#elseif (chenkReport_url_defaultValue!)?string=="currentComp">
								<#assign chenkReport_url_defaultValue='${compJson!}'>
							</#if>

									<@datacolumn key="chenkReport.url"        showFormat="TEXT" defaultValue="${(chenkReport_url_defaultValue!)?string}" defaultDisplay="${(chenkReport_url_displayDefaultType!)?string}" decimal="" editable=false type="textfield" showType="textfield" notnull=false     textalign="center" hiddenCol=true viewUrl="" viewCode="" mneenable=false crossCompany=false viewTitle="${getText('')}"  label="${getText('WOM.propertydisplayName.randon1502698958009')}" width=100 showFormatFunc=""/>
...
<div class="edit-panes-s" >
			...
			<#if newObj?? && !newObj && !(refId?? && (refId > 0))>
				<#assign buttons="[{text:\"${getText('ec.view.addline')}\",handler:function(event){this.addNewRow(event);},iconClass:\"cui-btn-add\",useInMore:\"false\"},{text:\"${getText('ec.view.deleteline')}\",iconClass:\"cui-btn-del\",useInMore:\"false\",handler:function(event){DT_TaskReportManage_dg1502700759865_deldatagrid();}}]">
				<#assign canImportExcel=false>
			<#else>
				<#assign buttons="[{text:\"${getText('ec.view.addline')}\",handler:function(event){this.addNewRow(event);},iconClass:\"cui-btn-add\",useInMore:\"false\"},{text:\"${getText('ec.view.deleteline')}\",iconClass:\"cui-btn-del\",useInMore:\"false\",handler:function(event){DT_TaskReportManage_dg1502700759865_deldatagrid();}}]">
				<#assign canImportExcel=false>
...
<!-- CUSTOM CODE END -->
		...
		WOM.generalManage.produceTaskManage.generalDetail._selectEvent = function(objName,_selectType,url,title,refparam,obj,customCallBack,crossCompanyFlag,gridEventObj,oGrid,isMultiselect,oColumn){
			...
			if(gridEventObj) {
				...
				WOM.generalManage.produceTaskManage.generalDetail._currRow = obj.parentNode.parentNode.parentNode;
			}else{
				if(arr.length > 2){
					_prefix = '.'+ arr[0] + '.'+ arr[1];
				}
