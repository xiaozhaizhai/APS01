...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="reWorkBatchNum.batchNum">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="tableNo">
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'reWorkBatchNum.batchNum' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'reWorkBatchNum.batchNum' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1493951276332rework')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'productBatchNum' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon14896683253641233123')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'formularId.code' || item == '*'>
									...
									<#if item?length gt 0>
												...
												<#if item == 'reWorkBatchNum.batchNum' || item == '*'>
													<Cell>
														...
														<#if data.reWorkBatchNum.batchNum??><Data ss:Type="String"><![CDATA[${(data.reWorkBatchNum.batchNum)!}]]></Data></#if>
