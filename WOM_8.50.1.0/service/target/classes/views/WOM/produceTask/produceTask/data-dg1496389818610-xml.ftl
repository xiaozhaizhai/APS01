...
<GridData DefaultNodeExpanded="false"   dgName="dg1496389818610" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<productId.productCode
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(ProduceMaterialSum_dg1496389818610Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(ProduceMaterialSum_dg1496389818610Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/product/product/productRefNew.action','dg1496389818610productId.productCode','${getText("MESBasic.viewtitle.randon1475137145215")}',sPTGridID,nRow,sFieldName,ProduceMaterialSum_dg1496389818610Widget,null,false,false,'','MESBasic_1_product_productRefNew')"
									readOnly="true" hideEditOnReadOnly="true"
					sort="true"  onPropertyChange="ProduceMaterialSum_dg1496389818610Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<#list customPropViewMappings as cpvm>
					...
					<#if cpvm.property.type == 'OBJECT'>
						...
						<#if cpvm.refView??>
							...
							onFocus="CUI.toggleClearBtn_DG_IE(ProduceMaterialSum_dg1496389818610Widget,nRow,sFieldName)"
							onBtnClick="CUI.clearCellValue_DG_IE(ProduceMaterialSum_dg1496389818610Widget,nRow,sFieldName,true)"
							subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('${viewUrl!}','dg1496389818610${cpvm.propertyLayRec}','${getText("${viewTitle}")}',sPTGridID,nRow,'${cpvm.propertyLayRec}',ProduceMaterialSum_dg1496389818610Widget,null,false,true,'${mainDisplayName},${pkName}','${viewCode}')"
							hideEditOnReadOnly="true"
						</#if>
					</#if>
						sort="true"
						onPropertyChange="ProduceMaterialSum_dg1496389818610Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
					...
					<#elseif cpvm.property.type == 'DATETIME'>
						...
						onClick="initCalender(ProduceMaterialSum_dg1496389818610Widget,'ProduceMaterialSum_dg1496389818610Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						onBtnClick="showCalender(ProduceMaterialSum_dg1496389818610Widget,'ProduceMaterialSum_dg1496389818610Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						control="Text"
