...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="formulaBom.productId.productCode">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									<#if item == 'formulaBom.productId.productCode' || item == '*'>
					...
					<#if item?length gt 0>
								<#if item == 'formulaBom.productId.productCode' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.formulaBom.productId.productCode)??>${(data.formulaBom.productId.productCode)!}</#if></Data>
