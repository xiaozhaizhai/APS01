...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="batchNum.availiQuantity">
  			<NumberFormat ss:Format="0.00"/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="batchNum.batchText">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="batchNum.good.productCode">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="batchNum.good.productName">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="batchNum.onhand">
  			<NumberFormat ss:Format="0.00"/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="planNum">
  			...
  			<Protection/>
			 						    	...
			 						    	<#assign customPropList = showCusProps.get("materialBatchNums")  />
											<#-- 排布自定义字段 -->
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									<#if item == 'batchNum.batchText' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'batchNum.good.productCode' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'batchNum.good.productName' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'batchNum.onhand' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'batchNum.availiQuantity' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									...
									<#assign customGeneratedFlag=false />
										<#if item == 'planNum' && !customGeneratedFlag >
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'batchNum.onhand' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1490064713692')}</Data>
										</Cell>
								</#if>
								<#if item == 'batchNum.availiQuantity' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1490064478475')}</Data>
										</Cell>
								</#if>
								<#if item == 'planNum' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1490346958585')}</Data>
										</Cell>
								</#if>
								<#assign customGeneratedFlag=false />
									<#if item == 'planNum' && !customGeneratedFlag >
			...
			<Row ss:AutoFitHeight="0">
				<#list exportFieldList as item>
					<#if item?length gt 0>
								<#if item == 'batchNum.batchText' || item == '*'>
									<Cell>
									<#if checkFieldPermission('batchNum.batchText','WOM_7.5.0.0_produceTask_MaterialBatchNums','WOM_7.5.0.0_produceTask_MaterialBatchNums_batchNum||WOM_7.5.0.0_standingcropRef_StandingcropRef_batchText') != 0>
										<Data ss:Type="String"><#if (data.batchNum.batchText)??>${(data.batchNum.batchText)!}</#if></Data>
									</#if>
									</Cell>
								</#if>
								<#if item == 'batchNum.good.productCode' || item == '*'>
									<Cell>
									<#if checkFieldPermission('batchNum.good.productCode','WOM_7.5.0.0_produceTask_MaterialBatchNums','WOM_7.5.0.0_produceTask_MaterialBatchNums_batchNum||WOM_7.5.0.0_standingcropRef_StandingcropRef_good||MESBasic_1_product_Product_productCode') != 0>
										<Data ss:Type="String"><#if (data.batchNum.good.productCode)??>${(data.batchNum.good.productCode)!}</#if></Data>
									</#if>
									</Cell>
								</#if>
								<#if item == 'batchNum.good.productName' || item == '*'>
									<Cell>
									<#if checkFieldPermission('batchNum.good.productName','WOM_7.5.0.0_produceTask_MaterialBatchNums','WOM_7.5.0.0_produceTask_MaterialBatchNums_batchNum||WOM_7.5.0.0_standingcropRef_StandingcropRef_good||MESBasic_1_product_Product_productName') != 0>
										<Data ss:Type="String"><#if (data.batchNum.good.productName)??>${(data.batchNum.good.productName)!}</#if></Data>
									</#if>
									</Cell>
								</#if>
								<#if item == 'batchNum.onhand' || item == '*'>
									<Cell>
									<#if checkFieldPermission('batchNum.onhand','WOM_7.5.0.0_produceTask_MaterialBatchNums','WOM_7.5.0.0_produceTask_MaterialBatchNums_batchNum||WOM_7.5.0.0_standingcropRef_StandingcropRef_onhand') != 0>
										<#if (data.batchNum.onhand)?has_content><Data ss:Type="Number">#{(data.batchNum.onhand)!; m6M6}</Data></#if>
									</#if>
									</Cell>
								</#if>
								<#if item == 'batchNum.availiQuantity' || item == '*'>
									<Cell>
									<#if checkFieldPermission('batchNum.availiQuantity','WOM_7.5.0.0_produceTask_MaterialBatchNums','WOM_7.5.0.0_produceTask_MaterialBatchNums_batchNum||WOM_7.5.0.0_standingcropRef_StandingcropRef_availiQuantity') != 0>
										<#if (data.batchNum.availiQuantity)?has_content><Data ss:Type="Number">#{(data.batchNum.availiQuantity)!; m6M6}</Data></#if>
									</#if>
									</Cell>
								</#if>
								<#if item == 'planNum' || item == '*'>
									<Cell>
									<#if checkFieldPermission('planNum','WOM_7.5.0.0_produceTask_MaterialBatchNums','WOM_7.5.0.0_produceTask_MaterialBatchNums_planNum') != 0>
