...
<#assign viewCode = 'WOM_7.5.0.0_procFeedback_ProcFeedbackHead' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="processActive.endTime">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="processActive.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="processActive.processId.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="processActive.startTime">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'processActive.processId.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'processActive.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'processActive.startTime' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'processActive.endTime' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'processActive.startTime' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1540953385761')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'processActive.endTime' || item == '*'>
													<Cell><Data ss:Type="String">${getText('WOM.propertydisplayName.randon1540953443908')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												...
												<#if item == 'processActive.processId.name' || item == '*'>
													<Cell>
														...
														<#if data.processActive.processId.name??><Data ss:Type="String"><![CDATA[${(data.processActive.processId.name)!}]]></Data></#if>
												...
												<#if item == 'processActive.name' || item == '*'>
													<Cell>
														...
														<#if data.processActive.name??><Data ss:Type="String"><![CDATA[${(data.processActive.name)!}]]></Data></#if>
												...
												<#if item == 'processActive.startTime' || item == '*'>
													<Cell ss:StyleID="s8">
														...
														<#if data.processActive.startTime??><Data ss:Type="DateTime">${(data.processActive.startTime)!?string("yyyy-MM-dd'T'HH:mm:ss.SSS")}</Data></#if>
												...
												<#if item == 'processActive.endTime' || item == '*'>
													<Cell ss:StyleID="s8">
														...
														<#if data.processActive.endTime??><Data ss:Type="DateTime">${(data.processActive.endTime)!?string("yyyy-MM-dd'T'HH:mm:ss.SSS")}</Data></#if>
