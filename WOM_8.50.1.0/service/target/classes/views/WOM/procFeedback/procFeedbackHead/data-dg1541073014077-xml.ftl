...
<GridData DefaultNodeExpanded="false"  MultiSelect="checkbox"  dgName="dg1541073014077" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<recordStartTime
					sort="false"  onPropertyChange="CraftRecords_dg1541073014077Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    
				    
						readOnly="true" hideEditOnReadOnly="true" 
						hideButtonsOnReadOnly="true"
									  onBlur="document.body.click()" 
									   buttonImg="Calendar" onClick="initCalender(CraftRecords_dg1541073014077Widget,'CraftRecords_dg1541073014077Widget',nRow,sFieldName,'datetime')" 
									   onBtnClick="showCalender(CraftRecords_dg1541073014077Widget,'CraftRecords_dg1541073014077Widget',nRow,sFieldName,'datetime')" 
	...
	<tbody>
		...
		<row  onClick="CraftRecords_dg1541073014077Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
			<#if cpObjHideKey?? && cpObjHideKey?has_content>
