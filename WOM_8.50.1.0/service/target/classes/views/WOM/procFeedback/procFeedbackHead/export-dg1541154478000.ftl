...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="standardCraft.craftName">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="standardCraft.standardValue">
  			<NumberFormat ss:Format="0.000000"/>
  			<Protection/>
  		</Style>
 	</Styles>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									<#if item == 'standardCraft.craftName' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'standardCraft.standardValue' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								<#if item == 'standardCraft.craftName' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1541034087253')}</Data>
										</Cell>
								</#if>
								<#if item == 'standardCraft.standardValue' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('WOM.propertydisplayName.randon1541034112568')}</Data>
										</Cell>
								</#if>
								<#if item == 'recordStartTime' || item == '*'>
										<Cell>
					...
					<#if item?length gt 0>
								<#if item == 'standardCraft.craftName' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.standardCraft.craftName)??>${(data.standardCraft.craftName)!}</#if></Data>
								...
								<#if item == 'standardCraft.standardValue' || item == '*'>
									<Cell>
										...
										<#if (data.standardCraft.standardValue)?has_content><Data ss:Type="Number">#{(data.standardCraft.standardValue)!; m6M6}</Data></#if>
