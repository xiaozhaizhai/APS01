	...
	return ec.common.showDialog(url, '委托', 260, 450, [{
		...
		handler:function(){
			if($("#proxyUsers_MultiIDs").val()==""){
				proxyPendIngDialogErrorBarWidget.showMessage('請選擇被委托人');
				return ;
			}else{
			...
			$.ajax({
				...
				success: function(res) {
					if(res.dealSuccessFlag == true){
						proxyPendIngDialogErrorBarWidget.showMessage('處理成功！', 's');
						setTimeout(function(){
							try{
						...
						proxyPendIngDialogErrorBarWidget.showMessage('處理失敗！');
					}
				},
...
ec.common.remind=function(tableInfoId,id){
	...
	// url, title, height, width, buttons
	ec.common.showDialog(url, "催辦頁面", 480, 750, [{
					name:'確定',
					icon: "yes",
					handler:function(){
						var userIds='';
						...
						if(userIds!=""){
							...
							remidnSubmitFormDialogErrorBarWidget.showMessage("請至少選擇一人！");
							return false;
						}
						...
						if(!pandion&&!email&&!sms){
							remidnSubmitFormDialogErrorBarWidget.showMessage("請至少需要選擇一種提醒方式！");
							return false;
						}
						var i=0;
						i += pandion ? 1 : 0;
						...
						if(remindContent!=""){
							if(sms&&remindContent.length>60){
								remidnSubmitFormDialogErrorBarWidget.showMessage("短信的催辦內容不能超過60個字符。");
								return false;
							}
						}
						$("#remindTypeId").val(i);
						...
						$.ajax({
							...
							success: function(res) {
								if(res.dealSuccessFlag == true){
									remidnSubmitFormDialogErrorBarWidget.showMessage('處理成功！', 's');
									setTimeout(function(){
										objMe.close();
									},1500);
								}else{
									remidnSubmitFormDialogErrorBarWidget.showMessage('處理失敗！');
								}
							},
							error : function(XMLHttpRequest, textStatus, errorThrown) {
								var msg = CUI.parseJSON(XMLHttpRequest.responseText);
