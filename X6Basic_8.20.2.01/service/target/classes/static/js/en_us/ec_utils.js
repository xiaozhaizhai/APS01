...
ec.common.proxyPending=function(pendingId){
	...
	return ec.common.showDialog(url, 'Trust', 260, 450, [{
		name: 'OK',
		icon: "yes",
		handler:function(){
			if($("#proxyUsers_MultiIDs").val()==""){
				proxyPendIngDialogErrorBarWidget.showMessage('Please choose a trustee!');
				return ;
			}else{
			...
			$.ajax({
				...
				success: function(res) {
					if(res.dealSuccessFlag == true){
						proxyPendIngDialogErrorBarWidget.showMessage('Successfully handled!', 's');
						setTimeout(function(){
							try{
						...
						proxyPendIngDialogErrorBarWidget.showMessage('Handle failed!');
					}
				},
...
ec.common.remind=function(tableInfoId,id){
	...
	// url, title, height, width, buttons
	ec.common.showDialog(url, "Reminder Page", 480, 750, [{
					name:'OK',
					icon: "yes",
					handler:function(){
						var userIds='';
						...
						if(userIds!=""){
							...
							remidnSubmitFormDialogErrorBarWidget.showMessage("Please select at least a person!");
							return false;
						}
						...
						if(!pandion&&!email&&!sms){
							remidnSubmitFormDialogErrorBarWidget.showMessage("Please choose at least one way of reminding!");
							return false;
						}
						var i=0;
						i += pandion ? 1 : 0;
						...
						if(remindContent!=""){
							if(sms&&remindContent.length>60){
								remidnSubmitFormDialogErrorBarWidget.showMessage("Remind content length can'''t exceed 60 bytes!");
								return false;
							}
						}
						$("#remindTypeId").val(i);
						...
						$.ajax({
							...
							success: function(res) {
								if(res.dealSuccessFlag == true){
									remidnSubmitFormDialogErrorBarWidget.showMessage('Successfully handled!', 's');
									setTimeout(function(){
										objMe.close();
									},1500);
								}else{
									remidnSubmitFormDialogErrorBarWidget.showMessage('Handle failed!');
								}
							},
							error : function(XMLHttpRequest, textStatus, errorThrown) {
								var msg = CUI.parseJSON(XMLHttpRequest.responseText);
