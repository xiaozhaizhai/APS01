...
<GridData DefaultNodeExpanded="false"   dgName="dg1445343942880" pageSize="${dg1445343942880Page.pageSize}" totalCount="${dg1445343942880Page.totalCount}" totalPages="${dg1445343942880Page.totalPages}" pageNo="${dg1445343942880Page.pageNo}" hasPre="${dg1445343942880Page.hasPre?string}" hasNext="${dg1445343942880Page.hasNext?string}" prePage="${dg1445343942880Page.prePage}" nextPage="${dg1445343942880Page.nextPage}">
	<thead>
				...
				<name
					...
					sort="false"  onPropertyChange="TimeRegionPar_dg1445343942880Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<startDate
						style="font-color:0303B3"
					sort="false"  onPropertyChange="TimeRegionPar_dg1445343942880Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    onChange="TimeRegionPar_dg1445343942880Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);TimeRegionPar_dg1445343942880Widget.evalCustomFunction(nRow,sFieldName,'aa()');"
						...
						fieldPermit="1" hideButtonsOnReadOnly="true" title="${getText('ec.view.readonlytitle')}"
									  ...
									  onBlur="document.body.click()" 
									   buttonImg="Calendar" onClick="initCalender(TimeRegionPar_dg1445343942880Widget,'TimeRegionPar_dg1445343942880Widget',nRow,sFieldName)" 
									   onBtnClick="showCalender(TimeRegionPar_dg1445343942880Widget,'TimeRegionPar_dg1445343942880Widget',nRow,sFieldName)" 
									  			 control="Text" 
	...
	<tbody>
		<#list dg1445343942880Page.result as row>
		<row  onClick="TimeRegionPar_dg1445343942880Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
