...
<#assign viewCode = 'X6Basic_1.0_timeRegion_TimeRegion' />
<#assign propertyCode = '' />
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
											...
											<#if item == 'unit.value' || item == '*'>
													<Cell><Data ss:Type="String">${getText('X6Basic.propertydisplayName.randon1441779032052')}</Data></Cell>
											</#if>
							...
							<Row ss:AutoFitHeight="0">
								<#list exportFieldList as item>
									<#if item?length gt 0>
												...
												<#if item == 'unit.value' || item == '*'>
													<Cell>
													<#if checkFieldPermission('unit','X6Basic_1.0_timeRegion_TimeRegion','X6Basic_1.0_timeRegion_TimeRegion_unit') != 0>
														<Data ss:Type="String">${getText('${(data.unit.value)!}')}</Data>
													</#if>
													</Cell>
												</#if>
								...
								<#if item??>
									<#if item.name?index_of(".")!=-1>
											...
											<Cell  ss:StyleID="${item.name}" >
												<#if checkFieldPermission('${(item.name)!}','X6Basic_1.0_timeRegion_TimeRegion','${(item.propertyCode)!}') != 0>
