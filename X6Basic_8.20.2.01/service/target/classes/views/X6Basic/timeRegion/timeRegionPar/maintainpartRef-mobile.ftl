...
<script type="text/javascript">
	CUI.ns("X6Basic.timeRegion.timeRegionPar","X6Basic.timeRegion.timeRegionPar.maintainpartRef");
</script>
<div class="container"  style="padding-top:10px;">
	...
	<div id="ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_queryForm_inputValue">
	...
	<form onsubmit="return false" id="ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_queryForm" exportUrl="/X6Basic/timeRegion/timeRegionPar/maintainpartRef-query.action">
		...
		<input type="hidden" id="dataTableSortColOrder" name="dataTableSortColOrder" />
						<@quickquery unique="LAST_QUERY_ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_queryForm" formId="ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_queryForm" idprefix="ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef" expandType="all" expand=true fieldcodes="X6Basic_1_0_timeRegion_TimeRegionPar_name_name:X6Basic.propertydisplayName.randon1459215074302||X6Basic_1_0_timeRegion_TimeRegionPar_year_year:X6Basic.propertydisplayName.randon1459215254357||X6Basic_1_0_timeRegion_TimeRegionPar_month_month:X6Basic.propertydisplayName.randon1459215052167" >
						...
						<div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 margin-bottom-5">						
				       	<@querybutton formId="ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_queryForm" type="search" onclick="X6Basic.timeRegion.timeRegionPar.maintainpartRef.commonQuery('query')" /> 
...
<script>
	$(function(){
		var buttons = $( 'button.btn-list', '#ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_queryForm' );
		if( buttons.length > 0 ){
			// 去掉按钮间的空格
...
<script type="text/javascript">
	 ...
	 */
	X6Basic.timeRegion.timeRegionPar.maintainpartRef.query = function(type,pageNo){
		...
		if(!checkListValid("ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_queryForm")) {
				return false;
		}
		var dataPost = "";
		...
		url += "&crossCompanyFlag=${Parameters.crossCompanyFlag!}";
	 	datatable_ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_query.setRequestDataUrl(url,dataPost);
		return false;
	};
	...
	foundation.common.${requestUri!}__callbackFunction = function(){
		...
		var oRows = datatable_ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_query.getSelectedRow();
		if(oRows.length == 0){
		...
		try{
			if(CUI("#X6Basic_timeRegion_timeRegionPar_maintainpartRef_callBackFuncName").val() != ""){
				eval(CUI("#X6Basic_timeRegion_timeRegionPar_maintainpartRef_callBackFuncName").val() + "(arrObj)");
			}
		}catch(e){
		}
	};
...
X6Basic_timeRegion_timeRegionPar_maintainpartRef_refresh = function(node){
	//var url = "/X6Basic/timeRegion/timeRegionPar/maintainpartRef-query.action";
	//if(node.layRec){
	//	datatable_ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_query.setRequestDataUrl(url,'&'+node.modelName+'.layRec=' + node.layRec,false);
	//}else{
	//	datatable_ec_X6Basic_timeRegion_timeRegionPar_maintainpartRef_query.setRequestDataUrl(url,'',false);
	//}
...
X6Basic.timeRegion.timeRegionPar.maintainpartRef.showLinkView = function(url,propertyId,row,formId,title,width,height,dialogType){
	...
	if(formId) {
		X6Basic.timeRegion.timeRegionPar.maintainpartRef.showDialog(url,formId,title,width,height);
	}else {
		openFullScreen(url);
	}
};
...
function checkListValid(formId){if($('#' + formId + ' input[name^="year_start"]').val()!=null && $('#' + formId + ' input[name^="year_start"]').val()!=''){if(!isInteger($('#' + formId + ' input[name^="year_start"]').val())){workbenchErrorBarWidget.showMessage("${getText('X6Basic.propertydisplayName.randon1459215254357')}${getText('ec.list.validate.integer')}");return false;}}if($('#' + formId + ' input[name^="year_end"]').val()!=null && $('#' + formId + ' input[name^="year_end"]').val()!=''){if(!isInteger($('#' + formId + ' input[name^="year_end"]').val())){workbenchErrorBarWidget.showMessage("${getText('X6Basic.propertydisplayName.randon1459215254357')}${getText('ec.list.validate.integer')}");return false;}}if($('#' + formId + ' input[name^="month_start"]').val()!=null && $('#' + formId + ' input[name^="month_start"]').val()!=''){if(!isInteger($('#' + formId + ' input[name^="month_start"]').val())){workbenchErrorBarWidget.showMessage("${getText('X6Basic.propertydisplayName.randon1459215052167')}${getText('ec.list.validate.integer')}");return false;}}if($('#' + formId + ' input[name^="month_end"]').val()!=null && $('#' + formId + ' input[name^="month_end"]').val()!=''){if(!isInteger($('#' + formId + ' input[name^="month_end"]').val())){workbenchErrorBarWidget.showMessage("${getText('X6Basic.propertydisplayName.randon1459215052167')}${getText('ec.list.validate.integer')}");return false;}}if($('#' + formId + ' input[name="month_start"]').val()!='' && $('#' + formId + ' input[name="month_end"]').val()!=''){if(parseInt($('#' + formId + ' input[name="month_start"]').val()) > parseInt($('#' + formId + ' input[name="month_end"]').val())){workbenchErrorBarWidget.showMessage("${getText('X6Basic.propertydisplayName.randon1459215052167')}${getText('ec.list.validate.end')}${getText('ec.list.validate.compare')}${getText('X6Basic.propertydisplayName.randon1459215052167')}${getText('ec.list.validate.start')}");return false;}}if($('#' + formId + ' input[name="year_start"]').val()!='' && $('#' + formId + ' input[name="year_end"]').val()!=''){if(parseInt($('#' + formId + ' input[name="year_start"]').val()) > parseInt($('#' + formId + ' input[name="year_end"]').val())){workbenchErrorBarWidget.showMessage("${getText('X6Basic.propertydisplayName.randon1459215254357')}${getText('ec.list.validate.end')}${getText('ec.list.validate.compare')}${getText('X6Basic.propertydisplayName.randon1459215254357')}${getText('ec.list.validate.start')}");return false;}}return true;};
	...
	/* CUSTOM CODE START(view-REFERENCE-X6Basic_1.0_timeRegion_maintainpartRef,js,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
/* CUSTOM CODE END */
	</script>
  </body>
