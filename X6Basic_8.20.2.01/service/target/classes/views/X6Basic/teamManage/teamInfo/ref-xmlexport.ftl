...
<#assign viewCode = 'X6Basic_1.0_teamManage_TeamInfo' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="principal.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="teamMemo">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'principal.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'teamMemo' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'code' || item == '*'>
													<Cell><Data ss:Type="String">${getText('X6Basic.propertydisplayName.radion1398303581302')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('X6Basic.propertydisplayName.radion1398303611599')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'dept.name' || item == '*'>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'createDate' || item == '*'>
													<Cell><Data ss:Type="String">${getText('X6Basic.propertydisplayName.radion1398303671784')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'principal.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('X6Basic.propertydisplayName.radion1398303738668')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'teamMemo' || item == '*'>
													<Cell><Data ss:Type="String">${getText('X6Basic.propertydisplayName.radion1398303797341')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												...
												<#if item == 'dept.name' || item == '*'>
													<Cell>
														...
														<#if data.dept.name??><Data ss:Type="String"><![CDATA[${(data.dept.name)!}]]></Data></#if>
												...
												<#if item == 'createDate' || item == '*'>
													<Cell ss:StyleID="s7">
														...
														<#if data.createDate??><Data ss:Type="DateTime">${(data.createDate)!?string("yyyy-MM-dd'T'HH:mm:ss.SSS")}</Data></#if>
												...
												<#if item == 'principal.name' || item == '*'>
													<Cell>
														...
														<#if data.principal.name??><Data ss:Type="String"><![CDATA[${(data.principal.name)!}]]></Data></#if>
												...
												<#if item == 'teamMemo' || item == '*'>
													<Cell>
														...
														<#if data.teamMemo??><Data ss:Type="String"><![CDATA[${(data.teamMemo)!}]]></Data></#if>
