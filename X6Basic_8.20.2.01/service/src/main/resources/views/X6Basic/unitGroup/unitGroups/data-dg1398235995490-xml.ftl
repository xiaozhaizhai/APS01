...
<GridData DefaultNodeExpanded="false"   dgName="dg1398235995490" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<code
					...
					sort="false"  onPropertyChange="BaseUnit_dg1398235995490Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<isMain
					sort="false"  onPropertyChange="BaseUnit_dg1398235995490Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    onChange="BaseUnit_dg1398235995490Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);BaseUnit_dg1398235995490Widget.evalCustomFunction(nRow,sFieldName,'isMainUnit(nRow,sFieldName)');"
	...
	<tbody>
		...
		<row  onClick="BaseUnit_dg1398235995490Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
			<#if cpObjHideKey?? && cpObjHideKey?has_content>
