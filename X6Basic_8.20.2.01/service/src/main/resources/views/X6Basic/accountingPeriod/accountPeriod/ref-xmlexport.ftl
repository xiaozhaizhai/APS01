...
<#assign viewCode = 'X6Basic_1.0_accountingPeriod_AccountPeriod' />
<#assign propertyCode = '' />
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'year.value' || item == '*'>
													<Cell><Data ss:Type="String">${getText('X6Basic.propertydisplayName.radion1400468158682')}</Data></Cell>
											</#if>
									...
									<#if item?length gt 0>
												<#if item == 'year.value' || item == '*'>
													<Cell>
														...
														<Data ss:Type="String">${getText('${(data.year.value)!}')}</Data>
