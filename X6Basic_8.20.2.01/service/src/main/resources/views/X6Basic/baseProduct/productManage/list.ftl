...
<title>${getText('X6Basic.viewtitle.radion1371469051979')}</title>
<@maincss/>
<@mainjs/>
<!-- CUSTOM CODE START(view-LIST-X6Basic_1.0_baseProduct_list,head,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) -->
<!-- 自定义代码 -->
...
X6Basic.baseProduct.productManage.list.showErrorMsg = function(msg) {
	// 优先使用当前独立页面的errorbar， 如果不存在尝试使用全局errorbar, 如果都不存在则使用浏览自带alert
	var errorbar = window.X6Basic_baseProduct_productManage_listErrorbarWidget || window.workbenchErrorBarWidget;
	if (errorbar){
...
X6Basic.baseProduct.productManage.list.createUploadBtn = function(params) {
	return {
		...
		handler:function(){
				list_operate_baseProduct_list.setUpload(params);
		}
	}
};
/**
...
X6Basic.baseProduct.productManage.list.createReferenceCopyBtn = function(params) {
	return {
		...
		align:"left",
		handler:function(){
			var funcName = eval("list_operate_baseProduct_list.X6Basic.baseProduct.productManage." + params.viewName + ".referenceCopy");
...
X6Basic.baseProduct.productManage.list.createPrintBtn = function(editViewName,url){
	return {
		...
		align:"left",
		handler:function(){
			var funcName = eval("list_operate_baseProduct_list.X6Basic.baseProduct.productManage." + editViewName + ".print");
			funcName(url);
		}
	}
};
...
X6Basic.baseProduct.productManage.list.createPrintSettingBtn = function(editViewName){
	return {
		...
		align:"left",
		handler:function(){
			var funcName = eval("list_operate_baseProduct_list.X6Basic.baseProduct.productManage." + editViewName + ".printSetting");
			funcName();
		}
	}
}
			...
			X6Basic.baseProduct.productManage.list.addlist = function() {
				var buttonCode='X6Basic_1.0_baseProduct_list_BUTTON_add';
					var treeParam = "";
					...
					<#if editView?? && editView.openType?? && editView.openType == 'dialog'>
						...
						otherParams.uploadBtn = X6Basic.baseProduct.productManage.list.createUploadBtn(otherParams);
						</#if>
						<#if editView.isReference?? && editView.isReference == true>
						otherParams.referenceCopyBtn = X6Basic.baseProduct.productManage.list.createReferenceCopyBtn({viewName:'${editView.name}', referenceUrl:'${editView.reference.url}'});
						</#if>			
						...
						var url='${editView.url}?entityCode=X6Basic_1.0_baseProduct' + "&${getPowerCode('${buttonPerfix!}list_add_add_X6Basic_1.0_baseProduct_list')}";
							<#if businessParam??>
					 			url += "&${businessParam}";	
					 		</#if>
				 		if(treeParam!=""){
				 		...
				 		url+='&buttonCode='+encodeURIComponent(buttonCode)+'&namespace=list_operate_baseProduct_list';
						X6Basic.baseProduct.productManage.list.showDialog(url,'X6Basic_baseProduct_productManage_${editView.name}_form','${getText(editView.title!)}',${(editView.width)!500},${(editView.height)!260},'${(editView.code)!}',buttonCode);
					<#else>
						var url='${editView.url}?entityCode=X6Basic_1.0_baseProduct' + "&${getPowerCode('${buttonPerfix!}list_add_add_X6Basic_1.0_baseProduct_list')}";
						if(treeParam!=""){
				 			url += treeParam;
...
openFullScreen(url);};
			...
			X6Basic.baseProduct.productManage.list.modifylist = function() {
				var buttonCode='X6Basic_1.0_baseProduct_list_BUTTON_modify';
				if(X6Basic.baseProduct.productManage.list.checkSelectedAny()){
					if(X6Basic.baseProduct.productManage.list.checkSelectedMore()) {
						var cid = ec_X6Basic_baseProduct_productManage_list_queryWidget.getSelectedRow()[0].cid;
						...
						<#if editView?? && editView.openType?? && editView.openType == 'dialog'>
							var otherParams = {};
							<#if editView.hasAttachment?? && editView.hasAttachment == true>
							otherParams.operateRecordId = X6Basic.baseProduct.productManage.list.getOperateRecordId();
							otherParams.uploadBtn = X6Basic.baseProduct.productManage.list.createUploadBtn(otherParams);
							</#if>
							<#if editView.isReference?? && editView.isReference == true>
							otherParams.referenceCopyBtn = X6Basic.baseProduct.productManage.list.createReferenceCopyBtn({viewName:'${editView.name}', referenceUrl:'${editView.reference.url}'});
							...
							otherParams.printBtn = X6Basic.baseProduct.productManage.list.createPrintBtn('${editView.name}','${editView.url}?entityCode=X6Basic_1.0_baseProduct&id=' + X6Basic.baseProduct.productManage.list.getOperateRecordId()+"&${getPowerCode('${buttonPerfix!}list_modify_modify_X6Basic_1.0_baseProduct_list')}");
							otherParams.printSettingBtn = X6Basic.baseProduct.productManage.list.createPrintSettingBtn('${editView.name}');
							</#if>
							...
							X6Basic.baseProduct.productManage.list.showDialog('${editView.url}?entityCode=X6Basic_1.0_baseProduct&buttonCode='+encodeURIComponent(buttonCode)+'&namespace=list_operate_baseProduct_list'+'&id=' + X6Basic.baseProduct.productManage.list.getOperateRecordId() + "&${getPowerCode('${buttonPerfix!}list_modify_modify_X6Basic_1.0_baseProduct_list')}",'X6Basic_baseProduct_productManage_${editView.name}_form','${getText(editView.title!)}',${(editView.width)!500},${(editView.height)!260},'${(editView.code)!}',buttonCode);
						<#else>
						   	var signatureStr='';
							var signatureInfo=signatureUtil.getSignatureInfo(false,buttonCode);
					 		if(signatureInfo[0] != '') {
							...
							openFullScreen('${editView.url}?entityCode=X6Basic_1.0_baseProduct'+signatureStr+'&id=' + X6Basic.baseProduct.productManage.list.getOperateRecordId() + "&${getPowerCode('${buttonPerfix!}list_modify_modify_X6Basic_1.0_baseProduct_list')}"<#if editView.width?? && editView.width?has_content>,${editView.width}</#if><#if editView.height?? && editView.height?has_content>,${editView.height}</#if>);
						</#if>
					}
				}
			};
			...
			X6Basic.baseProduct.productManage.list.dbmodifylist =function(event,oRow){
				...
				var checkPowerUrl="";
				if(cid != undefined){
		            checkPowerUrl="/foundation/userPermission/checkUserPower.action?menuOperateCodes=${buttonPerfix!}list_modify_modify_X6Basic_1.0_baseProduct_list&companyId="+cid;
		        }else{
		            checkPowerUrl="/foundation/userPermission/checkUserPower.action?menuOperateCodes=${buttonPerfix!}list_modify_modify_X6Basic_1.0_baseProduct_list";
		        }
   				CUI.ajax({
		   		   type: "POST",
		   		   async: false,
		   		   ...
		   		   success: function(res){
   			    	if(res['${buttonPerfix!}list_modify_modify_X6Basic_1.0_baseProduct_list'] == true) {
					var url = "${editView.url}?entityCode=X6Basic_1.0_baseProduct&buttonCode="+encodeURIComponent(buttonCode)+'&namespace=list_operate_baseProduct_list'+"&id=" + oRow.id + "&${getPowerCode('${buttonPerfix!}list_modify_modify_X6Basic_1.0_baseProduct_list')}";
				<#if editView?? && editView.openType?? && editView.openType == 'dialog'>
					var otherParams = {};
					<#if editView.hasAttachment?? && editView.hasAttachment == true>
					otherParams.operateRecordId = X6Basic.baseProduct.productManage.list.getOperateRecordId();
					...
					<#if editView.isPrint?? && editView.isPrint == true>
					otherParams.printBtn = X6Basic.baseProduct.productManage.list.createPrintBtn('${editView.name}',url);
					otherParams.printSettingBtn = X6Basic.baseProduct.productManage.list.createPrintSettingBtn('${editView.name}',url);
					</#if>
					otherParams.dialogType = '<#if (editView.dialogType)??>${editView.dialogType?substring(editView.dialogType?last_index_of('_') + 1)}<#else>OTHER</#if>';
					X6Basic.baseProduct.productManage.list.otherParams = otherParams;
...
openFullScreen(url);
			...
			*/
			delete_callback=function(params){
				var deleteUrl="/X6Basic/baseProduct/productManage/delete.action?ids=" + X6Basic.baseProduct.productManage.list.getOperateRecordId('del')+"&${getPowerCode('${buttonPerfix!}list_delete_del_X6Basic_1.0_baseProduct_list')}";
			...
			X6Basic.baseProduct.productManage.list.dellist = function(param) {
				var buttonCode='X6Basic_1.0_baseProduct_list_BUTTON_delete';
				if(X6Basic.baseProduct.productManage.list.checkSelectedValid(1)){
					var message = '';
					for(var i=0 ; i<datatable_ec_X6Basic_baseProduct_productManage_list_query.selectedRows.length ; i++){
                        var cid = datatable_ec_X6Basic_baseProduct_productManage_list_query.selectedRows[i].cid;
                        if(cid!='${Session.company.id}'){
                            var index = datatable_ec_X6Basic_baseProduct_productManage_list_query.selectedRows[i].rowHtmlObj.rowIndex+1;
					...
					var signatureInfo=signatureUtil.getSignatureInfo(false,buttonCode);
				 	if(signatureInfo[0] != '') {
						...
						CUI.post("/X6Basic/baseProduct/productManage/delete.action?ids=" + X6Basic.baseProduct.productManage.list.getOperateRecordId('del')+"&${getPowerCode('${buttonPerfix!}list_delete_del_X6Basic_1.0_baseProduct_list')}", X6Basic.baseProduct.productManage.callBackInfo, "json");
					}
				});
				}
			};
			...
			setTimeout(function(){
				try{X6Basic.baseProduct.productManage.list.Dlg.close();closeLoadPanel();}catch(e){}
							if(res.operateType == 'save') {
								...
								var _Current_PageNo = CUI('#PageLink_PageSelect','#' + ec_X6Basic_baseProduct_productManage_list_queryWidget.dataTableId).val();
							    if(null != queryFunc && "undefined" != queryFunc){
							    	if(typeof X6Basic_baseProduct_productManage_list_getLayout == "function"){
										...
										if(null != _Current_PageNo && undefined != _Current_PageNo && "" != _Current_PageNo) {
							    			eval(queryFunc.substr(0,queryFunc.lastIndexOf(')')) + "," + _Current_PageNo + "," +  (ec_X6Basic_baseProduct_productManage_list_queryWidget._sortKey && ec_X6Basic_baseProduct_productManage_list_queryWidget._sortMethod ? true : false) + ")");
								    	} else {
											eval(queryFunc);
								    	}
									...
									var needSort = ec_X6Basic_baseProduct_productManage_list_queryWidget._sortKey && ec_X6Basic_baseProduct_productManage_list_queryWidget._sortMethod ? true : false;
									if(typeof X6Basic_baseProduct_productManage_list_getLayout == "function"){
										var layout = X6Basic_baseProduct_productManage_list_getLayout();
										var node = layout.getNode();
										var modelCode = null;
								    	...
								    	X6Basic.baseProduct.productManage.list.node = node;	
										if(null != _Current_PageNo && undefined != _Current_PageNo && "" != _Current_PageNo) {
								    		X6Basic.baseProduct.productManage.list.query('query',_Current_PageNo,needSort);
								    	} else {
								    		X6Basic.baseProduct.productManage.list.query('query');
								    	}
										if(modelCode == 'X6Basic_1.0_baseProduct_ProductManage'){
	...
	/**
	 * 确认选中数据是否有效的
	 * @method X6Basic.baseProduct.productManage.list.checkSelectedvalid
	 * @param  type 0无效数据返回true 1有效数据返回true 
	 * @return Boolean
	 * @private
	 */
	X6Basic.baseProduct.productManage.list.checkSelectedValid = function(type) {
		if(X6Basic.baseProduct.productManage.list.checkSelectedAny()){
			for(var i=0;i<datatable_ec_X6Basic_baseProduct_productManage_list_query.selectedRows.length;i++){
				if(type==0){
					if(datatable_ec_X6Basic_baseProduct_productManage_list_query.selectedRows[i].valid){
						CUI.Dialog.alert("${getText('ec.common.checkvalidforrestore')}");
						return false;
					}
				}
				if(type==1){
					if(datatable_ec_X6Basic_baseProduct_productManage_list_query.selectedRows[i].valid === false){
	 ...
	 * @private
	 */
	X6Basic.baseProduct.productManage.list.showDialog = function(url,formId,title,width,height,viewCode,buttonCode) {
		var buttons = [];
									...
									<#if !operateBtn.ispermission?? || (operateBtn.ispermission)?string('true','false')=='false' 
										...
										buttons.push({
											...
											handler:function(){
												...
												list_operate_baseProduct_list.${funcname};
												</#if>
											}
										});
									</#if>
		...
		if(otherParams && otherParams.viewShowType != 'VIEW') {
			buttons.push({	
				...
				handler:function(){
					list_operate_baseProduct_list.CUI('#'+formId).submit();
				}
			});
		}
		if(otherParams && otherParams.viewShowType == 'VIEW') {
		...
		if ( X6Basic.baseProduct.productManage.list.Dlg ) {
			X6Basic.baseProduct.productManage.list.Dlg._config.url = url;
			X6Basic.baseProduct.productManage.list.Dlg._config.formId = formId;
			X6Basic.baseProduct.productManage.list.Dlg.setTitle( title )
			X6Basic.baseProduct.productManage.list.Dlg.setButtonbar( buttons );
		}else{
			X6Basic.baseProduct.productManage.list.Dlg =	new CUI.Dialog({
				...
				iframe: 'list_operate_baseProduct_list',
		...
		X6Basic.baseProduct.productManage.list.Dlg.resetSize( dialog_width, dialog_height )
	...
	X6Basic.baseProduct.productManage.list.approvalDialog = function(url,formId,title,width,height) {
		X6Basic.baseProduct.productManage.list.AppDlg =	new CUI.Dialog({
			...
			buttons:[{	
						...
						handler:function(){try{X6Basic.baseProduct.productManage.approval();}catch(e){}}
					},
					{	name:"${getText('common.button.cancel')}",
	...
	X6Basic.baseProduct.productManage.list.commonQuery = function(type) {
	  // 快速查询前回调
	  try {
	  	if (typeof X6Basic.baseProduct.productManage.list.beforeCommonQuery === 'function') {
	   		var ret = X6Basic.baseProduct.productManage.list.beforeCommonQuery( type );
	   		if (ret === false) return;
	  	}
	  } catch(e) {}
	  $(".nodeInfo").removeAttr("id");
	  ...
	  if(typeof X6Basic_baseProduct_productManage_list_cancelSelectedNode == 'function') {
	   X6Basic_baseProduct_productManage_list_cancelSelectedNode();
	   X6Basic.baseProduct.productManage.list.node = null;
	  }
	...
	X6Basic.baseProduct.productManage.list.query = function(type,pageNo,sortFlag){
		...
		if(sortFlag && datatable_ec_X6Basic_baseProduct_productManage_list_query._sortKey && datatable_ec_X6Basic_baseProduct_productManage_list_query._sortMethod){
			...
			CUI('#ec_X6Basic_baseProduct_productManage_list_queryForm #dataTableSortColName').val(datatable_ec_X6Basic_baseProduct_productManage_list_query._sortColumnName);
			...
			CUI('#ec_X6Basic_baseProduct_productManage_list_queryForm #dataTableSortColName').val('');
			CUI('#ec_X6Basic_baseProduct_productManage_list_queryForm #dataTableSortColOrder').val('');
		}
		var dataPost = "";
		var url = "";
			datatable_ec_X6Basic_baseProduct_productManage_list_query.setAttributeConfig('queryFunc', {
                writeOnce: true,
                value: "X6Basic.baseProduct.productManage.list.query('"+type+"')"
            }); 
		...
		var condobj={};
		condobj.viewCode="X6Basic_1.0_baseProduct_list";
		...
		CUI('input,select', CUI('#ec_X6Basic_baseProduct_productManage_list_queryForm')).each(function(index){
			if(CUI(this).attr('name') && CUI(this).attr('typeUse') != 'dataClassific' && !CUI(this).attr('name').startsWith('tree-')){
				...
				if(fastColValue != null && fastColValue != "undefined" && fastColValue != "") {
					if(ec_X6Basic_baseProduct_productManage_list_quickquery_info[fieldName]){
						var fieldObj={};
						fieldObj.type="0";
						fieldObj.columnName=ec_X6Basic_baseProduct_productManage_list_quickquery_info[fieldName].columnName;
						fieldObj.dbColumnType=ec_X6Basic_baseProduct_productManage_list_quickquery_info[fieldName].dbColumnType;
						if(fastCol.endsWith('_start')) {
							...
							if(ec_X6Basic_baseProduct_productManage_list_quickquery_info[fieldName].showField){
								var lowerCheck=CUI('#ec_X6Basic_baseProduct_productManage_list_queryForm *[name="'+ec_X6Basic_baseProduct_productManage_list_quickquery_info[fieldName].showField+'_bapLower"]');
							}else{
								var lowerCheck=CUI('#ec_X6Basic_baseProduct_productManage_list_queryForm *[name="'+fieldName+'_bapLower"]');
							}
							if(lowerCheck.length>0&&lowerCheck.prop('checked')){
								var layRec=ec_X6Basic_baseProduct_productManage_list_quickquery_info[fieldName].layRec;
								var tableName="";
								if(layRec.split(",").length>1){
									var level=layRec.split("-").length-1;
									tableName=layRec.split("-")[level-1].split(",")[0];
								}else{
									tableName=ec_X6Basic_baseProduct_productManage_list_quickquery_info["MainTableName"];
								}
	...
	X6Basic.baseProduct.productManage.list.downAttachment = function(row) {
		var url = "/foundation/workbench/download.action?id="+row.document.id + "&entityCode=X6Basic_1.0_baseProduct";
		openExportFrame(url);
	}
X6Basic.baseProduct.productManage.list.showLinkView = function(url,propertyId,row,formId,title,width,height,dialogType,viewCode){
	X6Basic.baseProduct.productManage.list.otherParams = {};
	...
	if(formId) {
		X6Basic.baseProduct.productManage.list.showDialog(url,formId,title,width,height,viewCode);
	}else {
		openFullScreen(url);
	}
};
...
})(jQuery);	/* CUSTOM CODE START(view-LIST-X6Basic_1.0_baseProduct_list,js,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
</script>
<@newAdvQueryJs viewCode='X6Basic_1.0_baseProduct_list' idprefix='ec_X6Basic_baseProduct_productManage_list' queryUrl='/X6Basic/baseProduct/productManage/list-query.action' queryFunc="X6Basic.baseProduct.productManage.list.query" ns="X6Basic.baseProduct.productManage.list" />
<@exportexcel action="/X6Basic/baseProduct/productManage/list-query.action"   getRequireDataAction="/X6Basic/baseProduct/productManage/list-getRequireData.action"    settingtext="设置" settingUrl="/foundation/workbench/getExportFilds.action?modelCode=X6Basic_1.0_baseProduct_ProductManage&&viewCode=X6Basic_1.0_baseProduct_list" prepareParams="" formId="" look="text" width=250 height=170 text=""  permissionCode="${permissionCode?default('X6Basic_1.0_baseProduct_list')}" prefix="ec_X6Basic_baseProduct_productManage_list_query" modelCode="X6Basic_1.0_baseProduct_ProductManage"  importFlag="false" viewCode="X6Basic_1.0_baseProduct_list" />
