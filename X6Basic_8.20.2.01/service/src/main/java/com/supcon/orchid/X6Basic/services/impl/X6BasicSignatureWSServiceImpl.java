...
import com.supcon.orchid.X6Basic.services.X6BasicSignatureWSService;
import com.supcon.orchid.X6Basic.services.X6BasicSignatureService;
/* CUSTOM CODE START(wsserviceimpl,import,X6Basic_1.0_ggSignetManage_Signature,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="X6BasicSignature",targetNamespace = "http://ws.supcon.com")
public class X6BasicSignatureWSServiceImpl implements X6BasicSignatureWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getSignatureByBusinessKey(
		...
		try{
			...
			X6BasicSignature signature = signatureService.loadSignatureByBussinessKey(businessKey);
			if(signature == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, signature);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateSignature(
		...
		try{
			if(signature == null){
				return set500Error(result, "签名不能为空。");
			}
			...
			if(!beforeUpdateSignature(signature)){
				return set404Error(result);
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO addSignature(
		...
		try{
			...
			try{
				checkSignatureData(signature);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			signatureService.saveSignature(signature, null);
			setSuccessResult(result, signature.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteSignatureByBusinessKey(
		...
		try{
			...
			try{
				signature = signatureService.loadSignatureByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddSignature(
		...
		try{
			...
			int count = 1;
			for(X6BasicSignature signature : signatureList){
				if(signature.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(signature.getId() == null || signature.getId().toString().trim().isEmpty()){
			...
			signatureService.batchImportBaseSignature(signatureList);
			for(X6BasicSignature signature : signatureList){
				idList.add(signature.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteSignatureByPk(
		...
		try{
			...
			for(String k : pk){
				...
				X6BasicSignature temp = null;
				try{
					temp = signatureService.getSignature(id);
				}catch(NullPointerException e){}
			...
			signatureService.deleteSignature(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteSignatureByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				X6BasicSignature temp = null;
				try{
					temp = signatureService.loadSignatureByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getSignaturePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(X6BasicSignature.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),X6BasicSignature.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,X6BasicSignature.class);
				} catch (Exception e) {
			...
			Page<X6BasicSignature> page = new Page<X6BasicSignature>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<X6BasicSignature> datas = signatureService.getByPage(page, detachedCriteria);
			List<X6BasicSignature> signatureList = datas.getResult();
			if (signatureList == null || signatureList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(signatureList.toArray());
			PaginationDTO<X6BasicSignature> pagination = new PaginationDTO<X6BasicSignature>(page);
	...
	/**
	 * 获取签名对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param signature
	 * @return
	 */
	private X6BasicSignature getSignature(X6BasicSignature signature) {
		if (signature.getId() == null && (signature.getId() == null || signature.getId().toString().trim() == ""))
			throw new RuntimeException("签名对象业务主键和id不能同时为空");
		if (signature.getId() != null) {
			X6BasicSignature result = signatureService.getSignature(signature .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return signatureService.loadSignatureByBussinessKey(signature.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个签名，则返回false，否则返回true
	 * @param signature
	 */
	private boolean beforeUpdateSignature(X6BasicSignature signature) {
		X6BasicSignature temp = getSignature(signature);
		if (temp == null) {
			return false;
		}
		if(signature.getId() == null){
			signature.setId(temp.getId());		
		}
		if(signature.getId() == null){
			signature.setId(temp.getId());
		}
		
		signature.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除签名对象下所有辅模型对象中的签名对象
	 * @param signature
	 */
	private void clearCircle(X6BasicSignature signature) {
	 ...
	 */
	private void checkSignatureData(X6BasicSignature signature){
	}
	/* CUSTOM CODE START(wsserviceimpl,functions,X6Basic_1.0_ggSignetManage_Signature,X6Basic_1.0) */
// 自定义代码
