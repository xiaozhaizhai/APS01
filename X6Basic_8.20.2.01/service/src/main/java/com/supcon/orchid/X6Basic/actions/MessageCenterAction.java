...
/* CUSTOM CODE START(action,import,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MessageCenterAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MessageCenter> {
	...
	//导出excel时,主模型内容
	List<MessageCenter> queryResults = new ArrayList<MessageCenter>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/messageManage/messageCenter/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/messageManage/messageCenter/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_messageManage_MessageCenter" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private Page<MessageCenter> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(id > 0){
				messageCenter = messageCenterService.getMessageCenter(id, viewCode);
			}
		}
	...
	public Page<MessageCenter> getPage() {
		if (null == page) {
			page = new Page<MessageCenter>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/messageManage/messageCenter/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = messageCenterService.getMessageCenterAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/messageManage/messageCenter/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String messageCenterReferenceCopy() throws Exception{
			MessageCenter oldMessageCenter = null;
			if(isEditNew()){
				...
				oldMessageCenter = new MessageCenter();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,messageCenter.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, messageCenter.getId());
				...
				oldMessageCenter = messageCenterService.getMessageCenter(id);
			}
			
			
			messageCenter = messageCenterService.getMessageCenter(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_messageManage_MessageCenter");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MessageCenter.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMessageCenter, null);
						Method setValue = MessageCenter.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(messageCenter, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			messageCenter.setCreateDepartmentId(oldMessageCenter.getCreateDepartmentId());
			messageCenter.setCreatePositionId(oldMessageCenter.getCreatePositionId());
			messageCenter.setCreateStaffId(oldMessageCenter.getCreateStaffId());
			messageCenter.setOwnerDepartmentId(oldMessageCenter.getOwnerDepartmentId());
			messageCenter.setOwnerPositionId(oldMessageCenter.getOwnerPositionId());
			messageCenter.setOwnerStaffId(oldMessageCenter.getOwnerStaffId());
			messageCenter.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=messageCenterService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return messageCenterReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			messageCenter = messageCenterService.getMessageCenter(this.messageCenter.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,messageCenter.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = messageCenterService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(messageCenterService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(messageCenterService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_messageManage_MessageCenter", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"messageCenter\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"messageCenter\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"messageCenter\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"messageCenter\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			messageCenterService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		messageCenterService.findDataGridPage(dg,dgclass,getDgPage(),messageCenter,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(messageCenter.getId() != null && messageCenter.getId() != -1){
				messageCenter = messageCenterService.getMessageCenter(messageCenter.getId());
			}
			messageCenterService.findDataGridPage(dg,dgclass,getDgPage(),messageCenter,searchCondition,params);
	...
	private void save() throws Exception {
		...
		messageCenterService.saveMessageCenter(messageCenter,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "submit");
		bap_validate_datagrids=messageCenterService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", messageCenter.getId());
		...
		fileuploads.put("linkId", messageCenter.getId());
		bapAfterCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/messageManage/messageCenter/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		messageCenterService.deleteMessageCenter(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/messageManage/messageCenter/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "restore");
		messageCenterService.restoreMessageCenter(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/messageManage/messageCenter/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/messageManage/messageCenter/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "showDealInfoData");
		if (null != tableInfoId) {
			int count=messageCenterService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = messageCenterService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = messageCenterService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MessageCenter> findExportDataInfos(Page<MessageCenter> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = messageCenterService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = messageCenterService.getAssProperty(p.getCode());	
									//String assModelCode = messageCenterService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			messageCenterService.findMessageCenters(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			messageCenterService.findMessageCenters(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MessageCenter> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MessageCenter result : results){
	...
	public boolean isEditNew(){
		//return null == this.messageCenter || null == this.messageCenter.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/messageManage/messageCenter/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/messageManage/messageCenter/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/messageManage/messageCenter/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/messageManage/messageCenter/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.MessageCenterService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_messageManage", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_messageManage");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/messageManage/messageCenter/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_messageManage_MessageCenter", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_messageManage_MessageCenter", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/messageManage/messageCenter/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/messageManage/messageCenter/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_messageManage_MessageCenter";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_messageManage_MessageCenter", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/messageManage/messageCenter/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
		_auditLog.setColumnStringA(null == messageCenter.getEmailTitle() ? "" : messageCenter.getEmailTitle().toString());
		_auditLog.setColumnStringC(messageCenter.getId().toString());
		_auditLog.setColumnStringD(MessageCenter.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_messageManage");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.MessageCenterAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, messageCenter,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, messageCenter,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,messageCenter);
		...
		printlog.setEntity("X6Basic_1.0_messageManage");
		...
		printlog.setBusinessId(messageCenter.getId());
		printlog.setBussinessKey(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
		printlog.setMainDisplay(null == messageCenter.getEmailTitle() ? "" : messageCenter.getEmailTitle().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/messageManage/messageCenter/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(messageCenter,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/messageManage/messageCenter/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(messageCenterService.getMessageCenter(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/messageManage/messageCenter/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/messageManage/messageCenter/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, messageCenter,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				messageCenter = messageCenterService.getMessageCenter(Long.parseLong(arrId[i]));
				if(null != messageCenter){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == messageCenter.getEmailTitle() ? "" : "," + messageCenter.getEmailTitle().toString()));
					} else {
						_auditLog.setColumnStringA(null == messageCenter.getEmailTitle() ? "" : messageCenter.getEmailTitle().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == messageCenter.getId() ? "" : "," + messageCenter.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == messageCenter.getId() ? "" : "," + messageCenter.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, messageCenter,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,messageCenter);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/messageManage/messageCenter/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MessageCenter> findExportPageInfos(Page<MessageCenter> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_messageManage" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = messageCenterService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MessageCenter queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = messageCenterService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = messageCenterService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_messageManage_MessageCenter");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_messageManage_MessageCenter")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_messageManage_MessageCenter");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_messageManage_MessageCenter");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = messageCenterService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
	@Action(value = "/X6Basic/messageManage/messageCenter/sendMessage", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String sendMessage(){
		messageCenterService.sendAllMessage();
