...
import com.supcon.orchid.X6Basic.services.X6BasicSyncLogImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicSyncLogDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_syncLogService")
@Transactional
public class X6BasicSyncLogServiceImpl  extends BaseServiceImpl<X6BasicSyncLog> implements X6BasicSyncLogService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicSyncLog> bapBaseService;
	...
	@Autowired
	private X6BasicSyncLogImportService syncLogImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicSyncLog.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  syncLogDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicSyncLog.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = syncLogDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicSyncLog.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicSyncLog.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  syncLogDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicSyncLog.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicSyncLog.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicSyncLog> list = (List<X6BasicSyncLog>) syncLogDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicSyncLog getSyncLog(long id){
		return getSyncLog(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSyncLog getSyncLog(long id, String viewCode){
		X6BasicSyncLog syncLog = syncLogDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getSyncLogAsJSON(long id, String include){
		X6BasicSyncLog syncLog = syncLogDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(syncLog, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.common.delete",operType="3")
	public void deleteSyncLog(X6BasicSyncLog syncLog){
		if(null != syncLog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncLog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == syncLog.getId() ? "" : syncLog.getId().toString());
			AuditUtil.setColumnStringA(null == syncLog.getEventTopic() ? "" : syncLog.getEventTopic().toString());
			AuditUtil.setColumnStringC(syncLog.getId().toString());
		}
		beforeDeleteSyncLog(syncLog);
		syncLogDao.delete(syncLog);
		afterDeleteSyncLog(syncLog);
		syncLogDao.flush();
		List<X6BasicSyncLog> params = new ArrayList<X6BasicSyncLog>();
		params.add(syncLog);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_syncLog");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.common.delete",operType="3")
	public void deleteSyncLog(List<Long> syncLogIds){
		deleteSyncLog(syncLogIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.common.delete",operType="3")
	public void deleteSyncLog(List<Long> syncLogIds, String eventTopic) {
		List<X6BasicSyncLog> syncLogs = new ArrayList<X6BasicSyncLog>();
		for(Long syncLogId : syncLogIds){
			X6BasicSyncLog syncLog = getSyncLog(syncLogId);
			syncLogs.add(syncLog);
			if(null != syncLog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncLog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == syncLog.getEventTopic() ? "" : "," + syncLog.getEventTopic().toString()));
				} else {
					AuditUtil.setColumnStringA(null == syncLog.getEventTopic() ? "" : syncLog.getEventTopic().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == syncLog.getId() ? "" : "," + syncLog.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == syncLog.getId() ? "" : syncLog.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == syncLog.getId() ? "" : "," + syncLog.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == syncLog.getId() ? "" : syncLog.getId().toString());
				}
			}
		}
		...
		if(syncLogs != null && syncLogs.size() > 0 ){
			for(X6BasicSyncLog syncLog : syncLogs){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),syncLog, syncLog.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(syncLogIds != null && syncLogIds.size() > 0) {
			String hql = "update " + X6BasicSyncLog.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = syncLogDao.createQuery(hql);
			query.setParameterList("ids", syncLogIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicSyncLog syncLogz : syncLogs){
				syncLogz.setValid(false);
				List<X6BasicSyncLog> params = new ArrayList<X6BasicSyncLog>();
				params.add(syncLogz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.common.delete",operType="3")
	public void deleteSyncLog(long syncLogId, int syncLogVersion){
		this.deleteSyncLog(syncLogId, syncLogVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.common.delete",operType="3")
	public void deleteSyncLog(long syncLogId, int syncLogVersion,SignatureLog signatureLog){
		X6BasicSyncLog syncLog = getSyncLog(syncLogId);
		if(syncLog != null && !syncLog.isValid()){
		...
		beforeDeleteSyncLog(syncLog);
		syncLogDao.delete(syncLogId, syncLogVersion);
		afterDeleteSyncLog(syncLog);
		syncLogDao.flush();
		List<X6BasicSyncLog> params = new ArrayList<X6BasicSyncLog>();
		params.add(syncLog);
		...
		if(signatureLog != null ) {
			...
			businessKey=syncLog.getId();
			signatureLog.setTableId(syncLog.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.common.delete",operType="3")
	public void deleteSyncLog(String syncLogIds){
		this.deleteSyncLog(syncLogIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.common.delete",operType="3")
	public void deleteSyncLog(String syncLogIds,SignatureLog signatureLog){
		deleteCollection(syncLogIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteSyncLog(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_syncLog#modelCode:X6Basic_1.0_syncLog_SyncLog#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="ec.engine.restore",operType="10")
	public void restoreSyncLog(String syncLogIds){
		restoreCollection(syncLogIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreSyncLog(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="ec.engine.restore",operType="10")
	public void restoreSyncLog(long syncLogId){
		
		findBusinessKeyUsed(syncLogId);	//判断业务主键是否重复
		...
		beforeRestoreSyncLog(syncLog);
		syncLog.setValid(true);
		syncLogDao.update(syncLog);
		afterRestoreSyncLog(syncLog);
		List<X6BasicSyncLog> params = new ArrayList<X6BasicSyncLog>();
		params.add(syncLog);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long syncLogId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_syncLog_SyncLog");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicSyncLog.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicSyncLog.TABLE_NAME +" where id = ? )";
			List<Object> list =  syncLogDao.createNativeQuery(sql,syncLogId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseSyncLog(List<X6BasicSyncLog>  syncLogs){
		for(X6BasicSyncLog syncLog:syncLogs)  {
			saveSyncLog(syncLog, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseSyncLog(List<X6BasicSyncLog>  syncLogs){
		for(X6BasicSyncLog syncLog:syncLogs)  {
			saveSyncLog(syncLog, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="ec.flow.save")
	public void saveSyncLog(X6BasicSyncLog syncLog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveSyncLog(syncLog, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="ec.flow.save")
	public void saveSyncLog(X6BasicSyncLog syncLog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveSyncLog(syncLog, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateSyncLogCodes(X6BasicSyncLog syncLog) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSyncLogCodes(syncLog, false);
	}
	
	public void generateSyncLogCodes(X6BasicSyncLog syncLog, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateSyncLogSummarys(X6BasicSyncLog syncLog) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSyncLogSummarys(syncLog, false);
	}

	public void generateSyncLogSummarys(X6BasicSyncLog syncLog, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog")
	public void saveSyncLog(X6BasicSyncLog syncLog,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveSyncLog(syncLog, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog")
	public void saveSyncLog(X6BasicSyncLog syncLog,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_syncLog";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_syncLog");
		String url = null;
		if(syncLog.getId() != null && syncLog.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncLog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_syncLog/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncLog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_syncLog/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, syncLog);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(syncLog);
		beforeSaveSyncLog(syncLog, viewIsView);

		if (viewIsView) {
			syncLogDao.saveWithRevertVersion(syncLog);
		} else {
			if(isNew)
				syncLogDao.save(syncLog);
			else
				syncLogDao.merge(syncLog);
		}
		if(null != syncLog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncLog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == syncLog.getId() ? "" : syncLog.getId().toString());
			AuditUtil.setColumnStringA(null == syncLog.getEventTopic() ? "" : syncLog.getEventTopic().toString());
		...
		dealDatagridsSave(syncLog,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateSyncLogCodes(syncLog, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateSyncLogSummarys(syncLog, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveSyncLog(syncLog, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, syncLog);
			}
		}
		List<X6BasicSyncLog> params = new ArrayList<X6BasicSyncLog>();
		syncLogDao.flush();
		syncLogDao.clear();
		syncLog = syncLogDao.load(syncLog.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicSyncLog syncLog){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog")
	public void mergeSyncLog(X6BasicSyncLog syncLog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(syncLog);
		beforeSaveSyncLog(syncLog);
		syncLogDao.merge(syncLog);
		if(null != syncLog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncLog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == syncLog.getId() ? "" : syncLog.getId().toString());
			AuditUtil.setColumnStringA(null == syncLog.getEventTopic() ? "" : syncLog.getEventTopic().toString());
		...
		dealDatagridsSave(syncLog,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveSyncLog(syncLog);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicSyncLog> findSyncLogs(Page<X6BasicSyncLog> page, Criterion... criterions) {
		return syncLogDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicSyncLogDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicSyncLogDealInfo> list=syncLogDao.createQuery("from " + X6BasicSyncLogDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(X6BasicSyncLogDealInfo dealInfo) {
		syncLogDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicSyncLogDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) syncLogDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicSyncLogDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = syncLogDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = syncLogDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicSyncLogDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = syncLogDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSyncLogs(Page<X6BasicSyncLog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findSyncLogs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSyncLogs(Page<X6BasicSyncLog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSyncLogs(Page<X6BasicSyncLog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicSyncLog> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"syncLog\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"syncLog\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"syncLog\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_syncLog_SyncLog" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_syncLog_SyncLog" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  syncLogDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  syncLogDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_syncLog").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"syncLog\"","X6Basic_1.0_syncLog",customCondition,"X6BASIC_SYNC_LOGS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"syncLog\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicSyncLogDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"syncLog\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"syncLog\".LAY_REC ASC, \"syncLog\".SORT ASC");
				} else {
					orderPart.append(" \"syncLog\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) syncLogDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=syncLogDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = syncLogDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicSyncLog> result = new ArrayList<X6BasicSyncLog>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicSyncLog.class,syncLogDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicSyncLog.class,syncLogDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicSyncLog syncLog : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(syncLog.getId(), "X6Basic_syncLog_syncLog");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(syncLog.getId(), "X6Basic_syncLog_syncLog");
							if(null != documents && !documents.isEmpty()) {
								syncLog.setDocument(documents.get(0));
								syncLog.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_syncLog")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_syncLog")+"X6Basic_1.0_syncLog".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_syncLog",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicSyncLog> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicSyncLog> result) {
		View view = null;
		...
		Map<Long, X6BasicSyncLog> rsMap = new HashMap<Long, X6BasicSyncLog>();
		for (X6BasicSyncLog syncLog : result) {
			ids.add(syncLog.getId());
			rsMap.put(syncLog.getId(), syncLog);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(syncLogDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = syncLogDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicSyncLog syncLog,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(syncLog.getOwnerStaffId());
		ti.setOwnerPositionId(syncLog.getOwnerPositionId());
		ti.setOwnerDepartmentId(syncLog.getOwnerDepartmentId());
		
		ti.setCreateStaff(syncLog.getCreateStaff());
		ti.setCreateStaffId(syncLog.getCreateStaffId());
		ti.setCreateTime(syncLog.getCreateTime());
		ti.setDeleteStaff(syncLog.getDeleteStaff());
		ti.setDeleteStaffId(syncLog.getDeleteStaffId());
		ti.setDeleteTime(syncLog.getDeleteTime());
		ti.setCreateDepartmentId(syncLog.getCreateDepartmentId());
		ti.setModifyStaff(syncLog.getModifyStaff());
		ti.setModifyStaffId(syncLog.getModifyStaffId());
		ti.setCreatePositionId(syncLog.getCreatePositionId());
		ti.setPositionLayRec(syncLog.getPositionLayRec());
		ti.setTableNo(syncLog.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_syncLog");
		ti.setTargetTableName("X6BASIC_SYNC_LOGS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_syncLog_SyncLog" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  syncLogDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"syncLog\"","X6Basic_1.0_syncLog","","X6BASIC_SYNC_LOGS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"syncLog\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"syncLog\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"syncLog\"","X6Basic_1.0_syncLog","","X6BASIC_SYNC_LOGS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("syncLog.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"syncLog\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("syncLog.mc\"").append(".SYNC_LOG) not in ("+notinValue+")");
			}
			
			SQLQuery query = syncLogDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("syncLog", "X6Basic_1.0_syncLog", "syncLog_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=syncLogDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicSyncLog.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicSyncLog.MODULE_CODE;}
	protected String getModelCode(){return X6BasicSyncLog.MODEL_CODE;}
	 ...
	 * @param syncLog 模型实例
	 */
	public void checkUniqueConstraint(X6BasicSyncLog syncLog){
	}
	
	
	@Override
	public List<X6BasicSyncLog> getSyncLogs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSyncLog.class);
		if(params!=null){
			...
			return syncLogDao.findByHql("from " + X6BasicSyncLog.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return syncLogDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<X6BasicSyncLog> findSyncLogsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSyncLog.class);
	...
	@Override
	public List<X6BasicSyncLog> findSyncLogsByHql(String hql, Object... objects){
		
		return   syncLogDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicSyncLog loadSyncLogByBussinessKey(X6BasicSyncLog syncLog){
		return loadSyncLogByBussinessKey(syncLog.getId());
	}
	
	@Override
	public Page<X6BasicSyncLog> getSyncLogs(Page<X6BasicSyncLog> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSyncLog.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = syncLogDao.findByPage(page, sql);
			}else{
				page = syncLogDao.findAllByPage(page);
			}
			return page;
		}
		page = syncLogDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSyncLog loadSyncLogByBussinessKey(Serializable bussinessKey){
		return syncLogDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicSyncLog> findByProperty(String propertyName, Object object){
		return syncLogDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSyncLog findEntityByProperty(String propertyName, Object object){
		return syncLogDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicSyncLog.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = syncLogDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_syncLog_SyncLog","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_syncLog", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicSyncLog syncLog,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,syncLogDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicSyncLog.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, syncLog);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",syncLog.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicSyncLog.TABLE_NAME +" where valid = 1";
			list = syncLogDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicSyncLog.TABLE_NAME;
			list = syncLogDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicSyncLog.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = syncLogDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicSyncLogDaoImpl syncLogDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchSyncLog(final List<X6BasicSyncLog> insertObjs, final List<X6BasicSyncLog> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchSyncLog(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchSyncLog(final List<X6BasicSyncLog> insertObjs, final List<X6BasicSyncLog> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1411469418376"),importCount));
		...
		Map<Long, X6BasicSyncLog> oldUpdateObjsMap = new HashMap<Long, X6BasicSyncLog>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicSyncLog bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicSyncLog> bizObjList = syncLogDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicSyncLog bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicSyncLog> nmeObjs = new ArrayList<X6BasicSyncLog>();
		...
		long id=autoGeneratorID.getNextId(X6BasicSyncLog.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicSyncLog syncLog:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicSyncLog.TABLE_NAME,1,dbId);
			if(syncLog.getId() != null ){
				m.put(syncLog.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) syncLogDao.createNativeQuery(sql, "X6Basic_1.0_syncLog_SyncLog"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		syncLogDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicSyncLog.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicSyncLog syncLog = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = syncLog.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicSyncLog.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicSyncLog.TABLE_NAME + " order by id desc";
//					Query query = syncLogDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicSyncLog syncLog = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = syncLog.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		syncLogDaoImpl.getSession().flush();
		syncLogDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicSyncLog bizObj = insertObjs.get(insertIndex);
			syncLogImportService.saveImportDataLog(bizObj, null, "id", "eventTopic");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicSyncLog bizObj = updateObjs.get(updateIndex);
			X6BasicSyncLog oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			syncLogImportService.saveImportDataLog(bizObj, oldBizObj, "id", "eventTopic");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = syncLogDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				syncLogDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = syncLogDao.createNativeQuery(sql, "X6Basic_1.0_syncLog_SyncLog").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = syncLogDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = syncLogDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = syncLogDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?syncLogDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  syncLogDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,syncLogDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="ec.flow.save")
	public void saveSyncLog(X6BasicSyncLog syncLog, DataGridService dataGridService){
		saveSyncLog(syncLog, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="ec.flow.save")
	public void saveSyncLog(X6BasicSyncLog syncLog, DataGridService dataGridService, String viewCode){
		saveSyncLog(syncLog, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog")
	public void saveSyncLog(X6BasicSyncLog syncLog, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveSyncLog(syncLog, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog")
	public void saveSyncLog(X6BasicSyncLog syncLog, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_syncLog";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_syncLog");
		String url = null;
		if(null != syncLog.getId() && syncLog.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncLog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog")
	public Map<Object, Object> syncLogDataGridImport(X6BasicSyncLog syncLog, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = syncLog.getVirtualId();
		
		syncLog = syncLogDao.load(syncLog.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, syncLog.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = syncLog.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(syncLog);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, syncLog.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog")
	public void mergeSyncLog(X6BasicSyncLog syncLog, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(syncLog);
		beforeSaveSyncLog(syncLog);
		syncLogDao.merge(syncLog);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="foundation.batch.save",operType="4")
	public void batchSaveSyncLog(X6BasicSyncLog syncLog, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "X6Basic_1.0_syncLog";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_syncLog");
	...
	@Override
	public Page<X6BasicSyncLog> getByPage(Page<X6BasicSyncLog> page,DetachedCriteria detachedCriteria){
		return syncLogDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_syncLog").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveSyncLog(X6BasicSyncLog syncLog, Object... objects){
		checkUniqueConstraint(syncLog);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveSyncLog(X6BasicSyncLog syncLog, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportSyncLog(List<X6BasicSyncLog> syncLogs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportSyncLog(List<X6BasicSyncLog> syncLogs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportSyncLog(List<X6BasicSyncLog> syncLogs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportSyncLog(List<X6BasicSyncLog> syncLogs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteSyncLog(X6BasicSyncLog syncLog){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteSyncLog(X6BasicSyncLog syncLog){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreSyncLog(X6BasicSyncLog syncLog){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreSyncLog(X6BasicSyncLog syncLog){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicSyncLog syncLog, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicSyncLog syncLog, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportSyncLog(List<X6BasicSyncLog> insertObjs, List<X6BasicSyncLog> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportSyncLog,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportSyncLog(List<X6BasicSyncLog> insertObjs, List<X6BasicSyncLog> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportSyncLog,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
