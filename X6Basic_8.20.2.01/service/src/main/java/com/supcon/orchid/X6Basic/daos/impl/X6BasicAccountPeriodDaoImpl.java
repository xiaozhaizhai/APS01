...
@Repository
public class X6BasicAccountPeriodDaoImpl extends ExtendedGenericDaoImpl<X6BasicAccountPeriod, Long> implements X6BasicAccountPeriodDao {
	@Override
	...
	@Override
	public X6BasicAccountPeriodDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicAccountPeriodDealInfo dealInfo = new X6BasicAccountPeriodDealInfo();
	...
	@Override
	public X6BasicAccountPeriodDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicAccountPeriod mainObj, Staff dealStaff) {
		X6BasicAccountPeriodDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicAccountPeriodDealInfo.class);
		...
		List<X6BasicAccountPeriodDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_accountingPeriod_AccountPeriod,X6Basic_1.0) */
// 自定义代码
