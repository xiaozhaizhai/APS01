...
@Repository
public class X6BasicSignatureDaoImpl extends ExtendedGenericDaoImpl<X6BasicSignature, Long> implements X6BasicSignatureDao {
	@Override
	...
	@Override
	public X6BasicSignatureDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicSignatureDealInfo dealInfo = new X6BasicSignatureDealInfo();
	...
	@Override
	public X6BasicSignatureDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicSignature mainObj, Staff dealStaff) {
		X6BasicSignatureDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicSignatureDealInfo.class);
		...
		List<X6BasicSignatureDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_ggSignetManage_Signature,X6Basic_1.0) */
// 自定义代码
