...
/* CUSTOM CODE START(action,import,X6Basic_1.0_signetManage_SignetSetting,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class SignetSettingAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<SignetSetting> {
	...
	private SignetSetting signetSetting;
	...
	//导出excel时,主模型内容
	List<SignetSetting> queryResults = new ArrayList<SignetSetting>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/signetManage/signetSetting/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/signetManage/signetSetting/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_signetManage_SignetSetting" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<SignetSetting> signetSettings;
	
	public List<SignetSetting> getSignetSettings() {
		return signetSettings;
	}
	...
	private Page<SignetSetting> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			signetSetting = new SignetSetting();
			if(signetSetting.getSingetType() != null) {
				signetSetting.setSingetType((SystemCode) systemCodeService.getSystemCode(signetSetting.getSingetType().getId()));
			}
		} else {
			if(id > 0){
				signetSetting = signetSettingService.getSignetSetting(id, viewCode);
			}
		}
	...
	public Page<SignetSetting> getPage() {
		if (null == page) {
			page = new Page<SignetSetting>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public SignetSetting getSignetSetting() {
		return signetSetting;
	}		

	public void setSignetSetting(SignetSetting signetSetting) {
		this.signetSetting = signetSetting;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/signetManage/signetSetting/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = signetSettingService.getSignetSettingAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/signetManage/signetSetting/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String signetSettingReferenceCopy() throws Exception{
			SignetSetting oldSignetSetting = null;
			if(isEditNew()){
				...
				oldSignetSetting = new SignetSetting();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,signetSetting.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, signetSetting.getId());
				...
				oldSignetSetting = signetSettingService.getSignetSetting(id);
			}
			
			
			signetSetting = signetSettingService.getSignetSetting(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_signetManage_SignetSetting");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = SignetSetting.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldSignetSetting, null);
						Method setValue = SignetSetting.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(signetSetting, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			signetSetting.setCreateDepartmentId(oldSignetSetting.getCreateDepartmentId());
			signetSetting.setCreatePositionId(oldSignetSetting.getCreatePositionId());
			signetSetting.setCreateStaffId(oldSignetSetting.getCreateStaffId());
			signetSetting.setOwnerDepartmentId(oldSignetSetting.getOwnerDepartmentId());
			signetSetting.setOwnerPositionId(oldSignetSetting.getOwnerPositionId());
			signetSetting.setOwnerStaffId(oldSignetSetting.getOwnerStaffId());
			signetSetting.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=signetSettingService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return signetSettingReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			signetSetting = signetSettingService.getSignetSetting(this.signetSetting.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,signetSetting.getId().toString());
			...
			List<Document> urlDocuments = documentService.getPictureByLinkIdAndType(signetSetting.getId(), "X6Basic_signetManage_signetSetting", "X6Basic_1.0_signetManage_SignetSetting_url");
			if(urlDocuments!=null && !urlDocuments.isEmpty()){
				String filPath = urlDocuments.get(0).getPath();
				signetSetting.setUrl(filPath);
				signetSetting.setUrlDocument(urlDocuments.get(0));
			}
		bapAfterCustomMethod(viewCode, viewName);
		if(null != printName && !printName.equals("")){
			signetSettingService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = signetSettingService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(signetSettingService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(signetSettingService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_signetManage_SignetSetting", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/signetManage/signetSetting/refer", results = { @Result(type = FREEMARKER, location = "refer.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"signetSetting\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"signetSetting\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"signetSetting\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"signetSetting\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			signetSettingService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/X6Basic/signetManage/signetSetting/refer-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.userinfo.name,result.singetType.id,result.singetType.value,result.userinfo.id,result.attrMap.*"})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		signetSettingService.findDataGridPage(dg,dgclass,getDgPage(),signetSetting,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(signetSetting.getId() != null && signetSetting.getId() != -1){
				signetSetting = signetSettingService.getSignetSetting(signetSetting.getId());
			}
			signetSettingService.findDataGridPage(dg,dgclass,getDgPage(),signetSetting,searchCondition,params);
	...
	private void save() throws Exception {
		...
		signetSettingService.saveSignetSetting(signetSetting,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/signetManage/signetSetting/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/signetManage/signetSetting/view/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "submit");
		bap_validate_datagrids=signetSettingService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", signetSetting.getId());
		...
		fileuploads.put("linkId", signetSetting.getId());
		bapAfterCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/signetManage/signetSetting/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		signetSettingService.deleteSignetSetting(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/signetManage/signetSetting/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "restore");
		signetSettingService.restoreSignetSetting(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/signetManage/signetSetting/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/signetManage/signetSetting/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "showDealInfoData");
		if (null != tableInfoId) {
			int count=signetSettingService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = signetSettingService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = signetSettingService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<SignetSetting> findExportDataInfos(Page<SignetSetting> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = signetSettingService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = signetSettingService.getAssProperty(p.getCode());	
									//String assModelCode = signetSettingService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			signetSettingService.findSignetSettings(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			signetSettingService.findSignetSettings(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<SignetSetting> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(SignetSetting result : results){
	...
	public boolean isEditNew(){
		//return null == this.signetSetting || null == this.signetSetting.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/signetManage/signetSetting/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/signetManage/signetSetting/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/signetManage/signetSetting/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/signetManage/signetSetting/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.SignetSettingService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_signetManage", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_signetManage");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/signetManage/signetSetting/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_signetManage_SignetSetting", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_signetManage_SignetSetting", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/signetManage/signetSetting/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/signetManage/signetSetting/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_signetManage_SignetSetting";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_signetManage_SignetSetting", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/signetManage/signetSetting/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		SignetSetting signetSetting = null;
		if(null != id) {
			signetSetting = signetSettingService.getSignetSetting(id);
		}
		...
		_auditLog.setColumnStringB(null == signetSetting.getId() ? "" : signetSetting.getId().toString());
		_auditLog.setColumnStringA(null == signetSetting.getName() ? "" : signetSetting.getName().toString());
		_auditLog.setColumnStringC(signetSetting.getId().toString());
		_auditLog.setColumnStringD(SignetSetting.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_signetManage");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.SignetSettingAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, signetSetting,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, signetSetting,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,signetSetting);
		...
		printlog.setEntity("X6Basic_1.0_signetManage");
		...
		printlog.setBusinessId(signetSetting.getId());
		printlog.setBussinessKey(null == signetSetting.getId() ? "" : signetSetting.getId().toString());
		printlog.setMainDisplay(null == signetSetting.getName() ? "" : signetSetting.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/signetManage/signetSetting/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(signetSetting,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/signetManage/signetSetting/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(signetSettingService.getSignetSetting(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/signetManage/signetSetting/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/signetManage/signetSetting/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, signetSetting,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				signetSetting = signetSettingService.getSignetSetting(Long.parseLong(arrId[i]));
				if(null != signetSetting){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == signetSetting.getName() ? "" : "," + signetSetting.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == signetSetting.getName() ? "" : signetSetting.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == signetSetting.getId() ? "" : "," + signetSetting.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == signetSetting.getId() ? "" : signetSetting.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == signetSetting.getId() ? "" : "," + signetSetting.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == signetSetting.getId() ? "" : signetSetting.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, signetSetting,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,signetSetting);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/signetManage/signetSetting/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<SignetSetting> findExportPageInfos(Page<SignetSetting> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_signetManage" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = signetSettingService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(SignetSetting queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = signetSettingService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = signetSettingService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_signetManage_SignetSetting");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_signetManage_SignetSetting")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_signetManage_SignetSetting");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_signetManage_SignetSetting");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = signetSettingService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_signetManage_SignetSetting,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_signetManage_SignetSetting,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_signetManage_SignetSetting,X6Basic_1.0) */
// 自定义代码
	@Action(value = "/X6Basic/signetManage/signetSetting/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
	"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.status,result.tableNo,result.name,result.singetType.value,result.userinfo.name,result.userinfo.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion"})  
	...
	public String listQuery() throws Exception{
		setFieldPermissionModelCode("X6Basic_1.0_signetManage_SignetSetting");
		params = new ArrayList<Param>();
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		params.add(new Param("\"signetSetting\".VALID", true));
		if (null != request.getParameter("advQueryCond") && request.getParameter("advQueryCond").trim().length() > 0) {
			params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
		}
		if (null != getDataTableSortColKey() && getDataTableSortColKey().length() > 0) {
			params.add(new Param("dataTable-sortColKey", getDataTableSortColKey()));
		}
		if (null != getDataTableSortColOrder() && getDataTableSortColOrder().length() > 0) {
			params.add(new Param("dataTable-sortColOrder", getDataTableSortColOrder()));
		}
				if(null != request.getParameter("name") && request.getParameter("name").trim().length() > 0) {
					params.add(new Param("\"signetSetting\".NAME",request.getParameter("name"),Param.LIKE_ALL));
					}
				if(null != request.getParameter("singetType") && request.getParameter("singetType").trim().length() > 0) {
					params.add(new Param("\"signetSetting\".SINGET_TYPE",request.getParameter("singetType"),Param.LIKE_UNSUPPORT));
					}
				if(null != request.getParameter("userinfo.id") && request.getParameter("userinfo.id").trim().length() > 0) {
					params.add(new Param("\"userinfo\".ID", request.getParameter("userinfo.id"), Param.LIKE_UNSUPPORT, "LONG"));
				}
				if(null != request.getParameter("userinfo.name") && request.getParameter("userinfo.name").trim().length() > 0) {
					params.add(new Param("\"userinfo\".NAME",request.getParameter("userinfo.name"),Param.LIKE_ALL));
					}
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"signetSetting\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		...
		if (null != request.getParameter("user.layRec") && request.getParameter("user.layRec").trim().length() > 0) {
			params.add(new Param("\"user\"."+Inflector.getInstance().columnize("layRec"), request.getParameter("user.layRec")));
		}
		if (null != request.getParameter("staff.layRec") && request.getParameter("staff.layRec").trim().length() > 0) {
			params.add(new Param("\"staff\"."+Inflector.getInstance().columnize("layRec"), request.getParameter("staff.layRec")));
		}
		if (null != request.getParameter("position.layRec") && request.getParameter("position.layRec").trim().length() > 0) {
			params.add(new Param("\"position\"."+Inflector.getInstance().columnize("layRec"), request.getParameter("position.layRec")));
		}
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"department\"."+Inflector.getInstance().columnize("layRec"), request.getParameter("department.layRec")));
		}
		currentSqlType = Sql.TYPE_LIST_QUERY;
		viewCode = "X6Basic_1.0_signetManage_list";
		generateCustomerCondition(viewCode);
		generateClassificCondition();
				//fieldFlag 暂定为zzsqd的内容
		//fieldFlag = ",*,dsyj,requestPerson.name,requestDept.name,name,createStaff.name,zzry.testSysCode,zzry.testEnum,";
	...
	// ######## X6Basic_1.0_signetManage_edit start
		/**
		 * X6Basic.viewtitle.radion1367133678500
		 *
		 * @return Dispatcher Result
		 * @throws Exception
		 */
		@Actions({
			@Action(value = "/X6Basic/signetManage/signetSetting/edit",  interceptorRefs={@InterceptorRef(value="orchidFullStack")}, results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
			@Action(value = "/X6Basic/signetManage/signetSetting/editPrint",  interceptorRefs={@InterceptorRef(value="orchidFullStack")}, results = { @Result(type = FREEMARKER, location = "print.ftl") })
		})
		...
		// ######## X6Basic_1.0_signetManage_edit end
		// ######## X6Basic_1.0_signetManage_list start
		/**
		 * X6Basic.viewtitle.radion1367133870220
		 *
		 * @return Dispatcher Result
		 * @throws Exception
		 */
		@Action(value = "/X6Basic/signetManage/signetSetting/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
		...
		@Action(value = "/X6Basic/signetManage/signetSetting/view",  results = { @Result(type = FREEMARKER, location = "view.ftl") })
