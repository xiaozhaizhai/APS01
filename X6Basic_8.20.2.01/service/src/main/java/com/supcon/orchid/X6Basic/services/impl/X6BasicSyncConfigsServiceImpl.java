...
import com.supcon.orchid.X6Basic.services.X6BasicSyncConfigsImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicSyncConfigsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_syncConfigsService")
@Transactional
public class X6BasicSyncConfigsServiceImpl  extends BaseServiceImpl<X6BasicSyncConfigs> implements X6BasicSyncConfigsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicSyncConfigs> bapBaseService;
	...
	@Autowired
	private X6BasicSyncConfigsImportService syncConfigsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicSyncConfigs.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  syncConfigsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicSyncConfigs.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = syncConfigsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicSyncConfigs.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicSyncConfigs.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  syncConfigsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicSyncConfigs.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicSyncConfigs.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicSyncConfigs> list = (List<X6BasicSyncConfigs>) syncConfigsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicSyncConfigs getSyncConfigs(long id){
		return getSyncConfigs(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSyncConfigs getSyncConfigs(long id, String viewCode){
		X6BasicSyncConfigs syncConfigs = syncConfigsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getSyncConfigsAsJSON(long id, String include){
		X6BasicSyncConfigs syncConfigs = syncConfigsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(syncConfigs, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.common.delete",operType="3")
	public void deleteSyncConfigs(X6BasicSyncConfigs syncConfigs){
		if(null != syncConfigs && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncConfigs.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
			AuditUtil.setColumnStringA(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
			AuditUtil.setColumnStringC(syncConfigs.getId().toString());
		}
		beforeDeleteSyncConfigs(syncConfigs);
		syncConfigsDao.delete(syncConfigs);
		afterDeleteSyncConfigs(syncConfigs);
		syncConfigsDao.flush();
		List<X6BasicSyncConfigs> params = new ArrayList<X6BasicSyncConfigs>();
		params.add(syncConfigs);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_syncConfig");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.common.delete",operType="3")
	public void deleteSyncConfigs(List<Long> syncConfigsIds){
		deleteSyncConfigs(syncConfigsIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.common.delete",operType="3")
	public void deleteSyncConfigs(List<Long> syncConfigsIds, String eventTopic) {
		List<X6BasicSyncConfigs> syncConfigss = new ArrayList<X6BasicSyncConfigs>();
		for(Long syncConfigsId : syncConfigsIds){
			X6BasicSyncConfigs syncConfigs = getSyncConfigs(syncConfigsId);
			syncConfigss.add(syncConfigs);
			if(null != syncConfigs && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncConfigs.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == syncConfigs.getId() ? "" : "," + syncConfigs.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == syncConfigs.getId() ? "" : "," + syncConfigs.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == syncConfigs.getId() ? "" : "," + syncConfigs.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(syncConfigss != null && syncConfigss.size() > 0 ){
			for(X6BasicSyncConfigs syncConfigs : syncConfigss){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),syncConfigs, syncConfigs.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(syncConfigsIds != null && syncConfigsIds.size() > 0) {
			String hql = "update " + X6BasicSyncConfigs.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = syncConfigsDao.createQuery(hql);
			query.setParameterList("ids", syncConfigsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicSyncConfigs syncConfigsz : syncConfigss){
				syncConfigsz.setValid(false);
				List<X6BasicSyncConfigs> params = new ArrayList<X6BasicSyncConfigs>();
				params.add(syncConfigsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_syncConfig");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.common.delete",operType="3")
	public void deleteSyncConfigs(long syncConfigsId, int syncConfigsVersion){
		this.deleteSyncConfigs(syncConfigsId, syncConfigsVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.common.delete",operType="3")
	public void deleteSyncConfigs(long syncConfigsId, int syncConfigsVersion,SignatureLog signatureLog){
		X6BasicSyncConfigs syncConfigs = getSyncConfigs(syncConfigsId);
		if(syncConfigs != null && !syncConfigs.isValid()){
		...
		beforeDeleteSyncConfigs(syncConfigs);
		syncConfigsDao.delete(syncConfigsId, syncConfigsVersion);
		afterDeleteSyncConfigs(syncConfigs);
		syncConfigsDao.flush();
		List<X6BasicSyncConfigs> params = new ArrayList<X6BasicSyncConfigs>();
		params.add(syncConfigs);
		...
		if(signatureLog != null ) {
			...
			businessKey=syncConfigs.getId();
			signatureLog.setTableId(syncConfigs.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.common.delete",operType="3")
	public void deleteSyncConfigs(String syncConfigsIds){
		this.deleteSyncConfigs(syncConfigsIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.common.delete",operType="3")
	public void deleteSyncConfigs(String syncConfigsIds,SignatureLog signatureLog){
		deleteCollection(syncConfigsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteSyncConfigs(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_syncConfig#modelCode:X6Basic_1.0_syncConfig_SyncConfigs#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="ec.engine.restore",operType="10")
	public void restoreSyncConfigs(String syncConfigsIds){
		restoreCollection(syncConfigsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreSyncConfigs(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="ec.engine.restore",operType="10")
	public void restoreSyncConfigs(long syncConfigsId){
		
		findBusinessKeyUsed(syncConfigsId);	//判断业务主键是否重复
		...
		beforeRestoreSyncConfigs(syncConfigs);
		syncConfigs.setValid(true);
		syncConfigsDao.update(syncConfigs);
		afterRestoreSyncConfigs(syncConfigs);
		List<X6BasicSyncConfigs> params = new ArrayList<X6BasicSyncConfigs>();
		params.add(syncConfigs);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long syncConfigsId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_syncConfig_SyncConfigs");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicSyncConfigs.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicSyncConfigs.TABLE_NAME +" where id = ? )";
			List<Object> list =  syncConfigsDao.createNativeQuery(sql,syncConfigsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseSyncConfigs(List<X6BasicSyncConfigs>  syncConfigss){
		for(X6BasicSyncConfigs syncConfigs:syncConfigss)  {
			saveSyncConfigs(syncConfigs, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseSyncConfigs(List<X6BasicSyncConfigs>  syncConfigss){
		for(X6BasicSyncConfigs syncConfigs:syncConfigss)  {
			saveSyncConfigs(syncConfigs, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="ec.flow.save")
	public void saveSyncConfigs(X6BasicSyncConfigs syncConfigs, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveSyncConfigs(syncConfigs, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="ec.flow.save")
	public void saveSyncConfigs(X6BasicSyncConfigs syncConfigs, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveSyncConfigs(syncConfigs, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateSyncConfigsCodes(X6BasicSyncConfigs syncConfigs) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSyncConfigsCodes(syncConfigs, false);
	}
	
	public void generateSyncConfigsCodes(X6BasicSyncConfigs syncConfigs, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateSyncConfigsSummarys(X6BasicSyncConfigs syncConfigs) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSyncConfigsSummarys(syncConfigs, false);
	}

	public void generateSyncConfigsSummarys(X6BasicSyncConfigs syncConfigs, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs")
	public void saveSyncConfigs(X6BasicSyncConfigs syncConfigs,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveSyncConfigs(syncConfigs, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs")
	public void saveSyncConfigs(X6BasicSyncConfigs syncConfigs,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_syncConfig";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_syncConfig");
		String url = null;
		if(syncConfigs.getId() != null && syncConfigs.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncConfigs.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_syncConfig/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncConfigs.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_syncConfig/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, syncConfigs);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(syncConfigs);
		beforeSaveSyncConfigs(syncConfigs, viewIsView);

		if (viewIsView) {
			syncConfigsDao.saveWithRevertVersion(syncConfigs);
		} else {
			if(isNew)
				syncConfigsDao.save(syncConfigs);
			else
				syncConfigsDao.merge(syncConfigs);
		}
		if(null != syncConfigs && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncConfigs.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
			AuditUtil.setColumnStringA(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
		...
		dealDatagridsSave(syncConfigs,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateSyncConfigsCodes(syncConfigs, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateSyncConfigsSummarys(syncConfigs, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveSyncConfigs(syncConfigs, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, syncConfigs);
			}
		}
		List<X6BasicSyncConfigs> params = new ArrayList<X6BasicSyncConfigs>();
		syncConfigsDao.flush();
		syncConfigsDao.clear();
		syncConfigs = syncConfigsDao.load(syncConfigs.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicSyncConfigs syncConfigs){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("syncConfigs", syncConfigs);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs")
	public void mergeSyncConfigs(X6BasicSyncConfigs syncConfigs, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(syncConfigs);
		beforeSaveSyncConfigs(syncConfigs);
		syncConfigsDao.merge(syncConfigs);
		if(null != syncConfigs && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncConfigs.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
			AuditUtil.setColumnStringA(null == syncConfigs.getId() ? "" : syncConfigs.getId().toString());
		...
		dealDatagridsSave(syncConfigs,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveSyncConfigs(syncConfigs);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicSyncConfigs> findSyncConfigss(Page<X6BasicSyncConfigs> page, Criterion... criterions) {
		return syncConfigsDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicSyncConfigsDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicSyncConfigsDealInfo> list=syncConfigsDao.createQuery("from " + X6BasicSyncConfigsDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(X6BasicSyncConfigsDealInfo dealInfo) {
		syncConfigsDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicSyncConfigsDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) syncConfigsDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicSyncConfigsDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = syncConfigsDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = syncConfigsDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicSyncConfigsDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = syncConfigsDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSyncConfigss(Page<X6BasicSyncConfigs> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findSyncConfigss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSyncConfigss(Page<X6BasicSyncConfigs> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSyncConfigss(Page<X6BasicSyncConfigs> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicSyncConfigs> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"syncConfigs\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"syncConfigs\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"syncConfigs\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_syncConfig_SyncConfigs" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_syncConfig_SyncConfigs" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  syncConfigsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  syncConfigsDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_syncConfig").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"syncConfigs\"","X6Basic_1.0_syncConfig",customCondition,"X6BASIC_SYNC_CONFIGS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"syncConfigs\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicSyncConfigsDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"syncConfigs\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"syncConfigs\".LAY_REC ASC, \"syncConfigs\".SORT ASC");
				} else {
					orderPart.append(" \"syncConfigs\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) syncConfigsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=syncConfigsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = syncConfigsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicSyncConfigs> result = new ArrayList<X6BasicSyncConfigs>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicSyncConfigs.class,syncConfigsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicSyncConfigs.class,syncConfigsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicSyncConfigs syncConfigs : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(syncConfigs.getId(), "X6Basic_syncConfig_syncConfigs");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(syncConfigs.getId(), "X6Basic_syncConfig_syncConfigs");
							if(null != documents && !documents.isEmpty()) {
								syncConfigs.setDocument(documents.get(0));
								syncConfigs.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_syncConfig")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_syncConfig")+"X6Basic_1.0_syncConfig".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_syncConfig",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicSyncConfigs> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicSyncConfigs> result) {
		View view = null;
		...
		Map<Long, X6BasicSyncConfigs> rsMap = new HashMap<Long, X6BasicSyncConfigs>();
		for (X6BasicSyncConfigs syncConfigs : result) {
			ids.add(syncConfigs.getId());
			rsMap.put(syncConfigs.getId(), syncConfigs);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(syncConfigsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = syncConfigsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicSyncConfigs syncConfigs,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(syncConfigs.getOwnerStaffId());
		ti.setOwnerPositionId(syncConfigs.getOwnerPositionId());
		ti.setOwnerDepartmentId(syncConfigs.getOwnerDepartmentId());
		
		ti.setCreateStaff(syncConfigs.getCreateStaff());
		ti.setCreateStaffId(syncConfigs.getCreateStaffId());
		ti.setCreateTime(syncConfigs.getCreateTime());
		ti.setDeleteStaff(syncConfigs.getDeleteStaff());
		ti.setDeleteStaffId(syncConfigs.getDeleteStaffId());
		ti.setDeleteTime(syncConfigs.getDeleteTime());
		ti.setCreateDepartmentId(syncConfigs.getCreateDepartmentId());
		ti.setModifyStaff(syncConfigs.getModifyStaff());
		ti.setModifyStaffId(syncConfigs.getModifyStaffId());
		ti.setCreatePositionId(syncConfigs.getCreatePositionId());
		ti.setPositionLayRec(syncConfigs.getPositionLayRec());
		ti.setTableNo(syncConfigs.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_syncConfig");
		ti.setTargetTableName("X6BASIC_SYNC_CONFIGS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_syncConfig_SyncConfigs" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  syncConfigsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"syncConfigs\"","X6Basic_1.0_syncConfig","","X6BASIC_SYNC_CONFIGS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"syncConfigs\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"syncConfigs\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"syncConfigs\"","X6Basic_1.0_syncConfig","","X6BASIC_SYNC_CONFIGS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("syncConfigs.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"syncConfigs\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"syncConfigs\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"syncConfigs\".CID = 1 OR \"syncConfigs\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("syncConfigs.mc\"").append(".SYNC_CONFIGS) not in ("+notinValue+")");
			}
			
			SQLQuery query = syncConfigsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("syncConfig", "X6Basic_1.0_syncConfig", "syncConfig_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=syncConfigsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicSyncConfigs.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicSyncConfigs.MODULE_CODE;}
	protected String getModelCode(){return X6BasicSyncConfigs.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param syncConfigs 模型实例
	 */
	public void checkUniqueConstraint(X6BasicSyncConfigs syncConfigs){
	}
	
	
	@Override
	public List<X6BasicSyncConfigs> getSyncConfigss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSyncConfigs.class);
		if(params!=null){
			...
			return syncConfigsDao.findByHql("from " + X6BasicSyncConfigs.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return syncConfigsDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicSyncConfigs> findSyncConfigssByHql(String hql, Object... objects){
		...
		return   syncConfigsDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicSyncConfigs loadSyncConfigsByBussinessKey(X6BasicSyncConfigs syncConfigs){
		return loadSyncConfigsByBussinessKey(syncConfigs.getId());
	}
	
	@Override
	public Page<X6BasicSyncConfigs> getSyncConfigss(Page<X6BasicSyncConfigs> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSyncConfigs.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = syncConfigsDao.findByPage(page, sql);
			}else{
				page = syncConfigsDao.findAllByPage(page);
			}
			return page;
		}
		page = syncConfigsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public X6BasicSyncConfigs loadSyncConfigsByBussinessKey(Serializable bussinessKey){
		return syncConfigsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicSyncConfigs> findByProperty(String propertyName, Object object){
		return syncConfigsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSyncConfigs findEntityByProperty(String propertyName, Object object){
		return syncConfigsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicSyncConfigs.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = syncConfigsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_syncConfig_SyncConfigs","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_syncConfig", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicSyncConfigs syncConfigs,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,syncConfigsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicSyncConfigs.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, syncConfigs);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",syncConfigs.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicSyncConfigs.TABLE_NAME +" where valid = 1";
			list = syncConfigsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicSyncConfigs.TABLE_NAME;
			list = syncConfigsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicSyncConfigs.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = syncConfigsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicSyncConfigsDaoImpl syncConfigsDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchSyncConfigs(final List<X6BasicSyncConfigs> insertObjs, final List<X6BasicSyncConfigs> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchSyncConfigs(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchSyncConfigs(final List<X6BasicSyncConfigs> insertObjs, final List<X6BasicSyncConfigs> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1409053271008"),importCount));
		...
		Map<Long, X6BasicSyncConfigs> oldUpdateObjsMap = new HashMap<Long, X6BasicSyncConfigs>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicSyncConfigs bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicSyncConfigs> bizObjList = syncConfigsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicSyncConfigs bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicSyncConfigs> nmeObjs = new ArrayList<X6BasicSyncConfigs>();
		...
		long id=autoGeneratorID.getNextId(X6BasicSyncConfigs.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicSyncConfigs syncConfigs:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicSyncConfigs.TABLE_NAME,1,dbId);
			if(syncConfigs.getId() != null ){
				m.put(syncConfigs.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) syncConfigsDao.createNativeQuery(sql, "X6Basic_1.0_syncConfig_SyncConfigs"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		syncConfigsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicSyncConfigs.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicSyncConfigs syncConfigs = updateObjs.get(updateIndex);
						Long id = syncConfigs.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = syncConfigs.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicSyncConfigs.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicSyncConfigs.TABLE_NAME + " order by id desc";
//					Query query = syncConfigsDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicSyncConfigs syncConfigs = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = syncConfigs.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		syncConfigsDaoImpl.getSession().flush();
		syncConfigsDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicSyncConfigs bizObj = insertObjs.get(insertIndex);
			syncConfigsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicSyncConfigs bizObj = updateObjs.get(updateIndex);
			X6BasicSyncConfigs oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			syncConfigsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = syncConfigsDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				syncConfigsDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = syncConfigsDao.createNativeQuery(sql, "X6Basic_1.0_syncConfig_SyncConfigs").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = syncConfigsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = syncConfigsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = syncConfigsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?syncConfigsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  syncConfigsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,syncConfigsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="ec.flow.save")
	public void saveSyncConfigs(X6BasicSyncConfigs syncConfigs, DataGridService dataGridService){
		saveSyncConfigs(syncConfigs, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="ec.flow.save")
	public void saveSyncConfigs(X6BasicSyncConfigs syncConfigs, DataGridService dataGridService, String viewCode){
		saveSyncConfigs(syncConfigs, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs")
	public void saveSyncConfigs(X6BasicSyncConfigs syncConfigs, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveSyncConfigs(syncConfigs, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs")
	public void saveSyncConfigs(X6BasicSyncConfigs syncConfigs, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_syncConfig";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_syncConfig");
		String url = null;
		if(null != syncConfigs.getId() && syncConfigs.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSyncConfigs.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs")
	public Map<Object, Object> syncConfigsDataGridImport(X6BasicSyncConfigs syncConfigs, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = syncConfigs.getVirtualId();
		
		syncConfigs = syncConfigsDao.load(syncConfigs.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, syncConfigs.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = syncConfigs.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(syncConfigs);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, syncConfigs.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs")
	public void mergeSyncConfigs(X6BasicSyncConfigs syncConfigs, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(syncConfigs);
		beforeSaveSyncConfigs(syncConfigs);
		syncConfigsDao.merge(syncConfigs);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncConfig",model="X6Basic_1.0_syncConfig_SyncConfigs",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicSyncConfigs> getByPage(Page<X6BasicSyncConfigs> page,DetachedCriteria detachedCriteria){
		return syncConfigsDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_syncConfig").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveSyncConfigs(X6BasicSyncConfigs syncConfigs, Object... objects){
		checkUniqueConstraint(syncConfigs);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveSyncConfigs(X6BasicSyncConfigs syncConfigs, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteSyncConfigs(X6BasicSyncConfigs syncConfigs){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteSyncConfigs(X6BasicSyncConfigs syncConfigs){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreSyncConfigs(X6BasicSyncConfigs syncConfigs){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreSyncConfigs(X6BasicSyncConfigs syncConfigs){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicSyncConfigs syncConfigs, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicSyncConfigs syncConfigs, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportSyncConfigs(List<X6BasicSyncConfigs> insertObjs, List<X6BasicSyncConfigs> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportSyncConfigs,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportSyncConfigs(List<X6BasicSyncConfigs> insertObjs, List<X6BasicSyncConfigs> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportSyncConfigs,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
