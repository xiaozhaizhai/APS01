...
/* CUSTOM CODE START(action,import,X6Basic_1.0_test_Test,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicTestAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicTest> {
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/test/test/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/test/test/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_test_Test" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicTest> tests;
	
	public List<X6BasicTest> getTests() {
		return tests;
	}
	...
	public void prepare() throws Exception {
		if (null == id) {
			test = new X6BasicTest();
		} else {
			if(id > 0){
				test = testService.getTest(id, viewCode);
			}
		}
	...
	public Page<X6BasicTest> getPage() {
		if (null == page) {
			page = new Page<X6BasicTest>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicTest getTest() {
		return test;
	}		

	public void setTest(X6BasicTest test) {
		this.test = test;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/test/test/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = testService.getTestAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/test/test/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String testReferenceCopy() throws Exception{
			X6BasicTest oldTest = null;
			if(isEditNew()){
				...
				oldTest = new X6BasicTest();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,test.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, test.getId());
				...
				oldTest = testService.getTest(id);
			}
			
			
			test = testService.getTest(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_test_Test");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicTest.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTest, null);
						Method setValue = X6BasicTest.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(test, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			test.setCreateDepartmentId(oldTest.getCreateDepartmentId());
			test.setCreatePositionId(oldTest.getCreatePositionId());
			test.setCreateStaffId(oldTest.getCreateStaffId());
			test.setOwnerDepartmentId(oldTest.getOwnerDepartmentId());
			test.setOwnerPositionId(oldTest.getOwnerPositionId());
			test.setOwnerStaffId(oldTest.getOwnerStaffId());
			bapAfterCustomMethod("X6Basic_1.0_test_Test", "referenceCopy");
			return SUCCESS;
		}
	// ######## X6Basic_1.0_test_list start
	/**
	 * X6Basic.viewtitle.randon1460695626823
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/test/test/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	public String list() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_test_list", "list");
		...
		bapAfterCustomMethod("X6Basic_1.0_test_list", "list");
		return SUCCESS;
	}
	
	// ######## X6Basic_1.0_test_list end
	...
	@Action(value = "/X6Basic/test/test/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.layRec,result.status,result.tableNo,result.name,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String listQuery() throws Exception{
		viewCode = "X6Basic_1.0_test_list";
		params = new ArrayList<Param>();
		params.add(new Param("\"test\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		bapBeforeCustomMethod(viewCode, "listQuery");
		setFieldPermissionModelCode("X6Basic_1.0_test_Test");
		...
		if (null != getDataTableSortColOrder() && getDataTableSortColOrder().length() > 0) {
			params.add(new Param("dataTable-sortColOrder", getDataTableSortColOrder()));
					...
					Param p = new Param("\"test\".NAME",request.getParameter("name"),Param.LIKE_ALL);
					params.add(p);
					}
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"test\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
	...
	@Action(value = "/X6Basic/test/test/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	...
	private void save(DataGridService dataGridService) throws Exception {
		...
		testService.saveTest(test, dataGridService, viewCode);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_test_Test", "submit");
		DataGridService dataGridService = null;
		
			save(dataGridService);
			responseMap.put("id", test.getId());
		...
		fileuploads.put("linkId", test.getId());
		bapAfterCustomMethod("X6Basic_1.0_test_Test", "submit");
		return SUCCESS;
	}
	@Action(value = "/X6Basic/test/test/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_test_Test", "delete");
		testService.deleteTest(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_test_Test", "delete");
		return SUCCESS;
	}
	
	
	@Actions({
			@Action(value = "/X6Basic/test/test/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/test/test/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_test_Test", "showDealInfoData");
		if (null != tableInfoId) {
			int count=testService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = testService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = testService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_test_Test", "showDealInfoData");
		return SUCCESS;
	}	

	@Override
	public Page<X6BasicTest> findExportDataInfos(Page<X6BasicTest> page) {
		...
		if(null != searchObjects && searchObjects.length > 0) {
			testService.findTests(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, searchObjects);
		} else {
			testService.findTests(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql);
		}
		return getPage();
	}
	...
	public boolean isEditNew(){
		//return null == this.test || null == this.test.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/test/test/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_test_Test", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_test_Test", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/test/test/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_test_Test", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/test/test/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/test/test/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicTestService"
		};
		bapBeforeCustomMethod("X6Basic_1.0_test_Test", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_test_Test", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/test/test/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				xlsResult = importService.imports(viewCode, "X6Basic_1.0_test_Test", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/test/test/downLoad", results = { @Result(type = "stream", params = {
  ...
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_test_Test", "downLoadXls");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/test/test/templateDownLoad", results = { @Result(type = "stream", params = {
			"contentType", "application/excel", "inputName", "inputStream",
			"contentDisposition", 
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String templateDownLoad() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_test_Test", "templateDownLoad");
		return SUCCESS;
	}
	...
	@Action(value = "/X6Basic/test/test/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		X6BasicTest test = null;
		if(null != id) {
			test = testService.getTest(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, test, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, test, null, null, true, this);
	...
	@Action(value = "/X6Basic/test/test/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/test/test/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				...
				if(test != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, test, null, null, true, this);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_test_Test,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_test_Test,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_test_Test,X6Basic_1.0) */
// 自定义代码
