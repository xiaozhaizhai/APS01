...
/* CUSTOM CODE START(action,import,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicTimeRegionParAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicTimeRegionPar> {
	...
	private X6BasicTimeRegionPar timeRegionPar;
	...
	//导出excel时,主模型内容
	List<X6BasicTimeRegionPar> queryResults = new ArrayList<X6BasicTimeRegionPar>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/timeRegion/timeRegionPar/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/timeRegion/timeRegionPar/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_timeRegion_TimeRegionPar" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicTimeRegionPar> timeRegionPars;
	
	public List<X6BasicTimeRegionPar> getTimeRegionPars() {
		return timeRegionPars;
	}
	...
	private Page<X6BasicTimeRegionPar> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			timeRegionPar = new X6BasicTimeRegionPar();
		} else {
			if(id > 0){
				timeRegionPar = timeRegionParService.getTimeRegionPar(id, viewCode);
			}
		}
	...
	public Page<X6BasicTimeRegionPar> getPage() {
		if (null == page) {
			page = new Page<X6BasicTimeRegionPar>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicTimeRegionPar getTimeRegionPar() {
		return timeRegionPar;
	}		

	public void setTimeRegionPar(X6BasicTimeRegionPar timeRegionPar) {
		this.timeRegionPar = timeRegionPar;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = timeRegionParService.getTimeRegionParAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.randon1459214032452
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String timeRegionParReferenceCopy() throws Exception{
			X6BasicTimeRegionPar oldTimeRegionPar = null;
			if(isEditNew()){
				...
				oldTimeRegionPar = new X6BasicTimeRegionPar();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,timeRegionPar.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, timeRegionPar.getId());
				...
				oldTimeRegionPar = timeRegionParService.getTimeRegionPar(id);
			}
			
			
			timeRegionPar = timeRegionParService.getTimeRegionPar(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_timeRegion_TimeRegionPar");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicTimeRegionPar.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTimeRegionPar, null);
						Method setValue = X6BasicTimeRegionPar.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(timeRegionPar, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			timeRegionPar.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/setAlertEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "setAlertEdit.ftl") }),
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/setAlertEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "setAlertEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=timeRegionParService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return timeRegionParReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			timeRegionPar = timeRegionParService.getTimeRegionPar(this.timeRegionPar.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,timeRegionPar.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/partList", results = { @Result(type = FREEMARKER, location = "partList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = timeRegionParService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(timeRegionParService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(timeRegionParService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_timeRegion_TimeRegionPar", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/lubricatepartRef", results = { @Result(type = FREEMARKER, location = "lubricatepartRef.ftl") })
		,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/maintainpartRef", results = { @Result(type = FREEMARKER, location = "maintainpartRef.ftl") })
		,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/meaPlanRef", results = { @Result(type = FREEMARKER, location = "meaPlanRef.ftl") })
		,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/monthPlanRef", results = { @Result(type = FREEMARKER, location = "monthPlanRef.ftl") })
		,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/partRef", results = { @Result(type = FREEMARKER, location = "partRef.ftl") })
		,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/timeRegRef", results = { @Result(type = FREEMARKER, location = "timeRegRef.ftl") })
		,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/weekPlanRef", results = { @Result(type = FREEMARKER, location = "weekPlanRef.ftl") })
		,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/yearPlanRef", results = { @Result(type = FREEMARKER, location = "yearPlanRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/partList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="partList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"timeRegionPar\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"timeRegionPar\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"timeRegionPar\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"timeRegionPar\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			timeRegionParService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/partList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/lubricatepartRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="lubricatepartRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/maintainpartRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="maintainpartRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/meaPlanRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="meaPlanRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/monthPlanRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="monthPlanRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/partRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="partRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/timeRegRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="timeRegRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/weekPlanRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="weekPlanRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/yearPlanRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.year,result.quarter,result.month,result.week,result.startDate,result.endDate,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="yearPlanRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		timeRegionParService.findDataGridPage(dg,dgclass,getDgPage(),timeRegionPar,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(timeRegionPar.getId() != null && timeRegionPar.getId() != -1){
				timeRegionPar = timeRegionParService.getTimeRegionPar(timeRegionPar.getId());
			}
			timeRegionParService.findDataGridPage(dg,dgclass,getDgPage(),timeRegionPar,searchCondition,params);
	...
	private void save() throws Exception {
		...
		timeRegionParService.saveTimeRegionPar(timeRegionPar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/timeRegion/timeRegionPar/setAlertEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "submit");
		bap_validate_datagrids=timeRegionParService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", timeRegionPar.getId());
		...
		fileuploads.put("linkId", timeRegionPar.getId());
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/timeRegion/timeRegionPar/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		timeRegionParService.deleteTimeRegionPar(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "restore");
		timeRegionParService.restoreTimeRegionPar(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicTimeRegionPar> findExportDataInfos(Page<X6BasicTimeRegionPar> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = timeRegionParService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = timeRegionParService.getAssProperty(p.getCode());	
									//String assModelCode = timeRegionParService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			timeRegionParService.findTimeRegionPars(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			timeRegionParService.findTimeRegionPars(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicTimeRegionPar> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicTimeRegionPar result : results){
	...
	public boolean isEditNew(){
		//return null == this.timeRegionPar || null == this.timeRegionPar.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/timeRegion/timeRegionPar/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/timeRegion/timeRegionPar/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicTimeRegionParService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicTimeRegionParService"
						, "com.supcon.orchid.X6Basic.services.X6BasicTimeRegionService"       
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_timeRegion_TimeRegionPar", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_timeRegion_TimeRegionPar", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_timeRegion_TimeRegionPar";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegionPar", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
		_auditLog.setColumnStringA(null == timeRegionPar.getName() ? "" : timeRegionPar.getName().toString());
		_auditLog.setColumnStringC(timeRegionPar.getId().toString());
		_auditLog.setColumnStringD(X6BasicTimeRegionPar.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicTimeRegionParAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, timeRegionPar,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, timeRegionPar,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,timeRegionPar);
		...
		printlog.setBusinessId(timeRegionPar.getId());
		printlog.setBussinessKey(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
		printlog.setMainDisplay(null == timeRegionPar.getName() ? "" : timeRegionPar.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(timeRegionPar,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(timeRegionParService.getTimeRegionPar(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, timeRegionPar,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				timeRegionPar = timeRegionParService.getTimeRegionPar(Long.parseLong(arrId[i]));
				if(null != timeRegionPar){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == timeRegionPar.getName() ? "" : "," + timeRegionPar.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == timeRegionPar.getName() ? "" : timeRegionPar.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == timeRegionPar.getId() ? "" : "," + timeRegionPar.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == timeRegionPar.getId() ? "" : "," + timeRegionPar.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, timeRegionPar,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,timeRegionPar);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_timeRegion_TimeRegionPar");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_timeRegion_TimeRegionPar")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_timeRegion_TimeRegionPar");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_timeRegion_TimeRegionPar");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = timeRegionParService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
private Long regionPartId;

	public Long getRegionPartId() {
		return regionPartId;
	}

	public void setRegionPartId(Long regionPartId) {
		this.regionPartId = regionPartId;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegionPar/deleteRegionPart", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String autoAppoint(){
		Boolean check=false;
		check=timeRegionParService.deleteRegionPart(regionPartId);   	
        if(check){
