...
import com.supcon.orchid.X6Basic.services.X6BasicRangePlanPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicRangePlanPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_rangePlanPartService")
@Transactional
public class X6BasicRangePlanPartServiceImpl  extends BaseServiceImpl<X6BasicRangePlanPart> implements X6BasicRangePlanPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicRangePlanPart> bapBaseService;
	...
	@Autowired
	private X6BasicRangePlanPartImportService rangePlanPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicRangePlanPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  rangePlanPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicRangePlanPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = rangePlanPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicRangePlanPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicRangePlanPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  rangePlanPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicRangePlanPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicRangePlanPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicRangePlanPart> list = (List<X6BasicRangePlanPart>) rangePlanPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicRangePlanPart getRangePlanPart(long id){
		return getRangePlanPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicRangePlanPart getRangePlanPart(long id, String viewCode){
		X6BasicRangePlanPart rangePlanPart = rangePlanPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getRangePlanPartAsJSON(long id, String include){
		X6BasicRangePlanPart rangePlanPart = rangePlanPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(rangePlanPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.common.delete",operType="3")
	public void deleteRangePlanPart(X6BasicRangePlanPart rangePlanPart){
		if(null != rangePlanPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlanPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
			AuditUtil.setColumnStringA(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
			AuditUtil.setColumnStringC(rangePlanPart.getId().toString());
		}
		beforeDeleteRangePlanPart(rangePlanPart);
		rangePlanPartDao.deletePhysical(rangePlanPart);
		afterDeleteRangePlanPart(rangePlanPart);
		rangePlanPartDao.flush();
		List<X6BasicRangePlanPart> params = new ArrayList<X6BasicRangePlanPart>();
		params.add(rangePlanPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_rangePlan");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.common.delete",operType="3")
	public void deleteRangePlanPart(List<Long> rangePlanPartIds){
		deleteRangePlanPart(rangePlanPartIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.common.delete",operType="3")
	public void deleteRangePlanPart(List<Long> rangePlanPartIds, String eventTopic) {
		planDailyService.deletePlanDailyByRangePlanPart(rangePlanPartIds);
		List<X6BasicRangePlanPart> rangePlanParts = new ArrayList<X6BasicRangePlanPart>();
		for(Long rangePlanPartId : rangePlanPartIds){
			X6BasicRangePlanPart rangePlanPart = getRangePlanPart(rangePlanPartId);
			rangePlanParts.add(rangePlanPart);
			if(null != rangePlanPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlanPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == rangePlanPart.getId() ? "" : "," + rangePlanPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == rangePlanPart.getId() ? "" : "," + rangePlanPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == rangePlanPart.getId() ? "" : "," + rangePlanPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(rangePlanParts != null && rangePlanParts.size() > 0 ){
			for(X6BasicRangePlanPart rangePlanPart : rangePlanParts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = X6BasicRangePlanPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(rangePlanPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),rangePlanPart, rangePlanPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(rangePlanPartIds != null && rangePlanPartIds.size() > 0) {
			String hql = "delete " + X6BasicRangePlanPart.JPA_NAME + " where id in(:ids)";
			Query query = rangePlanPartDao.createQuery(hql);
			query.setParameterList("ids", rangePlanPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicRangePlanPart rangePlanPartz : rangePlanParts){
				rangePlanPartz.setValid(false);
				List<X6BasicRangePlanPart> params = new ArrayList<X6BasicRangePlanPart>();
				params.add(rangePlanPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_rangePlan");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.common.delete",operType="3")
	public void deleteRangePlanPart(long rangePlanPartId, int rangePlanPartVersion){
		this.deleteRangePlanPart(rangePlanPartId, rangePlanPartVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.common.delete",operType="3")
	public void deleteRangePlanPart(long rangePlanPartId, int rangePlanPartVersion,SignatureLog signatureLog){
		X6BasicRangePlanPart rangePlanPart = getRangePlanPart(rangePlanPartId);
		...
		beforeDeleteRangePlanPart(rangePlanPart);
		rangePlanPartDao.delete(rangePlanPartId, rangePlanPartVersion);
		afterDeleteRangePlanPart(rangePlanPart);
		rangePlanPartDao.flush();
		List<X6BasicRangePlanPart> params = new ArrayList<X6BasicRangePlanPart>();
		params.add(rangePlanPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=rangePlanPart.getId();
			signatureLog.setTableId(rangePlanPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.common.delete",operType="3")
	public void deleteRangePlanPart(String rangePlanPartIds){
		this.deleteRangePlanPart(rangePlanPartIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.common.delete",operType="3")
	public void deleteRangePlanPart(String rangePlanPartIds,SignatureLog signatureLog){
		deleteCollection(rangePlanPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteRangePlanPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_rangePlan#modelCode:X6Basic_1.0_rangePlan_RangePlanPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="ec.engine.restore",operType="10")
	public void restoreRangePlanPart(String rangePlanPartIds){
		restoreCollection(rangePlanPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreRangePlanPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="ec.engine.restore",operType="10")
	public void restoreRangePlanPart(long rangePlanPartId){
		
		findBusinessKeyUsed(rangePlanPartId);	//判断业务主键是否重复
		...
		beforeRestoreRangePlanPart(rangePlanPart);
		rangePlanPart.setValid(true);
		rangePlanPartDao.update(rangePlanPart);
		afterRestoreRangePlanPart(rangePlanPart);
		List<X6BasicRangePlanPart> params = new ArrayList<X6BasicRangePlanPart>();
		params.add(rangePlanPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long rangePlanPartId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_rangePlan_RangePlanPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicRangePlanPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicRangePlanPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  rangePlanPartDao.createNativeQuery(sql,rangePlanPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseRangePlanPart(List<X6BasicRangePlanPart>  rangePlanParts){
		for(X6BasicRangePlanPart rangePlanPart:rangePlanParts)  {
			saveRangePlanPart(rangePlanPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseRangePlanPart(List<X6BasicRangePlanPart>  rangePlanParts){
		for(X6BasicRangePlanPart rangePlanPart:rangePlanParts)  {
			saveRangePlanPart(rangePlanPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="ec.flow.save")
	public void saveRangePlanPart(X6BasicRangePlanPart rangePlanPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveRangePlanPart(rangePlanPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="ec.flow.save")
	public void saveRangePlanPart(X6BasicRangePlanPart rangePlanPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveRangePlanPart(rangePlanPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateRangePlanPartCodes(X6BasicRangePlanPart rangePlanPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateRangePlanPartCodes(rangePlanPart, false);
	}
	
	public void generateRangePlanPartCodes(X6BasicRangePlanPart rangePlanPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateRangePlanPartSummarys(X6BasicRangePlanPart rangePlanPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateRangePlanPartSummarys(rangePlanPart, false);
	}

	public void generateRangePlanPartSummarys(X6BasicRangePlanPart rangePlanPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart")
	public void saveRangePlanPart(X6BasicRangePlanPart rangePlanPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveRangePlanPart(rangePlanPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart")
	public void saveRangePlanPart(X6BasicRangePlanPart rangePlanPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_rangePlan";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_rangePlan");
		String url = null;
		if(rangePlanPart.getId() != null && rangePlanPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlanPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_rangePlan/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlanPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_rangePlan/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, rangePlanPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(rangePlanPart);
		beforeSaveRangePlanPart(rangePlanPart, viewIsView);

		if (viewIsView) {
			rangePlanPartDao.saveWithRevertVersion(rangePlanPart);
		} else {
			if(isNew)
				rangePlanPartDao.save(rangePlanPart);
			else
				rangePlanPartDao.merge(rangePlanPart);
		}
		if(null != rangePlanPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlanPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
			AuditUtil.setColumnStringA(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
		...
		dealDatagridsSave(rangePlanPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateRangePlanPartCodes(rangePlanPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateRangePlanPartSummarys(rangePlanPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveRangePlanPart(rangePlanPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, rangePlanPart);
			}
		}
		List<X6BasicRangePlanPart> params = new ArrayList<X6BasicRangePlanPart>();
		rangePlanPartDao.flush();
		rangePlanPartDao.clear();
		rangePlanPart = rangePlanPartDao.load(rangePlanPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicRangePlanPart rangePlanPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("rangePlanPart", rangePlanPart);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart")
	public void mergeRangePlanPart(X6BasicRangePlanPart rangePlanPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(rangePlanPart);
		beforeSaveRangePlanPart(rangePlanPart);
		rangePlanPartDao.merge(rangePlanPart);
		if(null != rangePlanPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlanPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
			AuditUtil.setColumnStringA(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
		...
		dealDatagridsSave(rangePlanPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveRangePlanPart(rangePlanPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicRangePlanPart> findRangePlanParts(Page<X6BasicRangePlanPart> page, Criterion... criterions) {
		return rangePlanPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRangePlanParts(Page<X6BasicRangePlanPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findRangePlanParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRangePlanParts(Page<X6BasicRangePlanPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRangePlanParts(Page<X6BasicRangePlanPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicRangePlanPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"rangePlanPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							...
							s.append(" AND \"rangePlanPart\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"rangePlanPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"rangePlanPart\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"rangePlanPart\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			s.append(" ( \"rangePlanPart\".RPP_PLANID IN ( SELECT RP_ID FROM S2BASE_RANGEPLAN \"rangePlan\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "X6Basic_1.0_rangePlan" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  rangePlanPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  rangePlanPartDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_rangePlan").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rangePlan\"","X6Basic_1.0_rangePlan","",true);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"rangePlanPart\".LAY_REC ASC, \"rangePlanPart\".SORT ASC");
				} else {
					orderPart.append(" \"rangePlanPart\".RPP_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) rangePlanPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=rangePlanPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = rangePlanPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicRangePlanPart> result = new ArrayList<X6BasicRangePlanPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicRangePlanPart.class,rangePlanPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicRangePlanPart.class,rangePlanPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicRangePlanPart rangePlanPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(rangePlanPart.getId(), "X6Basic_rangePlan_rangePlanPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(rangePlanPart.getId(), "X6Basic_rangePlan_rangePlanPart");
							if(null != documents && !documents.isEmpty()) {
								rangePlanPart.setDocument(documents.get(0));
								rangePlanPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_rangePlan")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_rangePlan")+"X6Basic_1.0_rangePlan".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_rangePlan",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicRangePlanPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicRangePlanPart> result) {
		View view = null;
		...
		Map<Long, X6BasicRangePlanPart> rsMap = new HashMap<Long, X6BasicRangePlanPart>();
		for (X6BasicRangePlanPart rangePlanPart : result) {
			ids.add(rangePlanPart.getId());
			rsMap.put(rangePlanPart.getId(), rangePlanPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(rangePlanPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = rangePlanPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_rangePlan_RangePlanPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="X6Basic_1.0_rangePlan_RangePlanPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  rangePlanPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rangePlanPart\"","X6Basic_1.0_rangePlan","","S2BASE_RANGEPLANPART",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"rangePlanPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"rangePlanPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rangePlanPart\"","X6Basic_1.0_rangePlan","","S2BASE_RANGEPLANPART",true);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "X6Basic_1.0_rangePlan_RangePlanPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("rangePlanPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"rangePlanPart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"rangePlanPart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"rangePlanPart\".CID = 1 OR \"rangePlanPart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("rangePlanPart.mc\"").append(".RANGE_PLAN_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = rangePlanPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("rangePlan", "X6Basic_1.0_rangePlan", "rangePlan_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=rangePlanPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicRangePlanPart.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicRangePlanPart.MODULE_CODE;}
	protected String getModelCode(){return X6BasicRangePlanPart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param rangePlanPart 模型实例
	 */
	public void checkUniqueConstraint(X6BasicRangePlanPart rangePlanPart){
	}
	
	
	@Override
	public List<X6BasicRangePlanPart> getRangePlanParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicRangePlanPart.class);
		if(params!=null){
			...
			return rangePlanPartDao.findByHql("from " + X6BasicRangePlanPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return rangePlanPartDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicRangePlanPart> findRangePlanPartsByHql(String hql, Object... objects){
		...
		return   rangePlanPartDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicRangePlanPart loadRangePlanPartByBussinessKey(X6BasicRangePlanPart rangePlanPart){
		return loadRangePlanPartByBussinessKey(rangePlanPart.getId());
	}
	
	@Override
	public Page<X6BasicRangePlanPart> getRangePlanParts(Page<X6BasicRangePlanPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicRangePlanPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = rangePlanPartDao.findByPage(page, sql);
			}else{
				page = rangePlanPartDao.findAllByPage(page);
			}
			return page;
		}
		page = rangePlanPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public X6BasicRangePlanPart loadRangePlanPartByBussinessKey(Serializable bussinessKey){
		return rangePlanPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicRangePlanPart> findByProperty(String propertyName, Object object){
		return rangePlanPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicRangePlanPart findEntityByProperty(String propertyName, Object object){
		return rangePlanPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicRangePlanPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = rangePlanPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_rangePlan_RangePlanPart","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_rangePlan", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicRangePlanPart rangePlanPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,rangePlanPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicRangePlanPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, rangePlanPart);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",rangePlanPart.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicRangePlanPart.TABLE_NAME +" where valid = 1";
			list = rangePlanPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicRangePlanPart.TABLE_NAME;
			list = rangePlanPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicRangePlanPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = rangePlanPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicRangePlanPartDaoImpl rangePlanPartDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchRangePlanPart(final List<X6BasicRangePlanPart> insertObjs, final List<X6BasicRangePlanPart> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchRangePlanPart(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchRangePlanPart(final List<X6BasicRangePlanPart> insertObjs, final List<X6BasicRangePlanPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1398309186741"),importCount));
		...
		Map<Long, X6BasicRangePlanPart> oldUpdateObjsMap = new HashMap<Long, X6BasicRangePlanPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicRangePlanPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicRangePlanPart> bizObjList = rangePlanPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicRangePlanPart bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicRangePlanPart> nmeObjs = new ArrayList<X6BasicRangePlanPart>();
		...
		long id=autoGeneratorID.getNextId(X6BasicRangePlanPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicRangePlanPart rangePlanPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicRangePlanPart.TABLE_NAME,1,dbId);
			if(rangePlanPart.getId() != null ){
				m.put(rangePlanPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) rangePlanPartDao.createNativeQuery(sql, "X6Basic_1.0_rangePlan_RangePlanPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		rangePlanPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicRangePlanPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicRangePlanPart rangePlanPart = updateObjs.get(updateIndex);
						Long id = rangePlanPart.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = rangePlanPart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(rangePlanPart);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicRangePlanPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicRangePlanPart.TABLE_NAME + " order by id desc";
//					Query query = rangePlanPartDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicRangePlanPart rangePlanPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = rangePlanPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		rangePlanPartDaoImpl.getSession().flush();
		rangePlanPartDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicRangePlanPart bizObj = insertObjs.get(insertIndex);
			rangePlanPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicRangePlanPart bizObj = updateObjs.get(updateIndex);
			X6BasicRangePlanPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			rangePlanPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<X6BasicRangePlanPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<X6BasicRangePlanPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) rangePlanPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<X6BasicRangePlanPart> result = getResult(page, query, new PendingResultTransformer(X6BasicRangePlanPart.class,rangePlanPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(X6BasicRangePlanPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = rangePlanPartDao.createNativeQuery(sql, "X6Basic_1.0_rangePlan_RangePlanPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = rangePlanPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = rangePlanPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = rangePlanPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?rangePlanPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  rangePlanPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,rangePlanPartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="ec.flow.save")
	public void saveRangePlanPart(X6BasicRangePlanPart rangePlanPart, DataGridService dataGridService){
		saveRangePlanPart(rangePlanPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="ec.flow.save")
	public void saveRangePlanPart(X6BasicRangePlanPart rangePlanPart, DataGridService dataGridService, String viewCode){
		saveRangePlanPart(rangePlanPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart")
	public void saveRangePlanPart(X6BasicRangePlanPart rangePlanPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveRangePlanPart(rangePlanPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart")
	public void saveRangePlanPart(X6BasicRangePlanPart rangePlanPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_rangePlan";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_rangePlan");
		String url = null;
		if(null != rangePlanPart.getId() && rangePlanPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlanPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart")
	public Map<Object, Object> rangePlanPartDataGridImport(X6BasicRangePlanPart rangePlanPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = rangePlanPart.getVirtualId();
		
		rangePlanPart = rangePlanPartDao.load(rangePlanPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, rangePlanPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = rangePlanPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(rangePlanPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, rangePlanPart.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart")
	public void mergeRangePlanPart(X6BasicRangePlanPart rangePlanPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(rangePlanPart);
		beforeSaveRangePlanPart(rangePlanPart);
		rangePlanPartDao.merge(rangePlanPart);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlanPart",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicRangePlanPart> getByPage(Page<X6BasicRangePlanPart> page,DetachedCriteria detachedCriteria){
		return rangePlanPartDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_rangePlan").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveRangePlanPart(X6BasicRangePlanPart rangePlanPart, Object... objects){
		checkUniqueConstraint(rangePlanPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveRangePlanPart(X6BasicRangePlanPart rangePlanPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteRangePlanPart(X6BasicRangePlanPart rangePlanPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteRangePlanPart(X6BasicRangePlanPart rangePlanPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreRangePlanPart(X6BasicRangePlanPart rangePlanPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreRangePlanPart(X6BasicRangePlanPart rangePlanPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicRangePlanPart rangePlanPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicRangePlanPart rangePlanPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportRangePlanPart(List<X6BasicRangePlanPart> insertObjs, List<X6BasicRangePlanPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportRangePlanPart,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportRangePlanPart(List<X6BasicRangePlanPart> insertObjs, List<X6BasicRangePlanPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportRangePlanPart,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicRangePlanPart> findByHql(String hql,Object...objects){
		return rangePlanPartDao.findByHql(hql, objects);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicRangePlanPart findEntityByHql(String hql,Object...objects){
		return rangePlanPartDao.findEntityByHql(hql, objects);
	}
/* CUSTOM CODE END */
}
