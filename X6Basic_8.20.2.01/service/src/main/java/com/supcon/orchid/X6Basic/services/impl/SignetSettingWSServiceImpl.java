...
import com.supcon.orchid.X6Basic.services.SignetSettingWSService;
import com.supcon.orchid.X6Basic.services.SignetSettingService;
/* CUSTOM CODE START(wsserviceimpl,import,X6Basic_1.0_signetManage_SignetSetting,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="SignetSetting",targetNamespace = "http://ws.supcon.com")
public class SignetSettingWSServiceImpl implements SignetSettingWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getSignetSettingByBusinessKey(
		...
		try{
			...
			SignetSetting signetSetting = signetSettingService.loadSignetSettingByBussinessKey(businessKey);
			if(signetSetting == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, signetSetting);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateSignetSetting(
		...
		try{
			if(signetSetting == null){
				return set500Error(result, "印章管理不能为空。");
			}
			...
			if(!beforeUpdateSignetSetting(signetSetting)){
				return set404Error(result);
			}
			signetSettingService.saveSignetSetting(signetSetting, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addSignetSetting(
		...
		try{
			...
			try{
				checkSignetSettingData(signetSetting);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			signetSettingService.saveSignetSetting(signetSetting, null);
			setSuccessResult(result, signetSetting.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteSignetSettingByBusinessKey(
		...
		try{
			...
			try{
				signetSetting = signetSettingService.loadSignetSettingByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddSignetSetting(
		...
		try{
			...
			int count = 1;
			for(SignetSetting signetSetting : signetSettingList){
				if(signetSetting.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(signetSetting.getId() == null || signetSetting.getId().toString().trim().isEmpty()){
			...
			signetSettingService.batchImportBaseSignetSetting(signetSettingList);
			for(SignetSetting signetSetting : signetSettingList){
				idList.add(signetSetting.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteSignetSettingByPk(
		...
		try{
			...
			for(String k : pk){
				...
				SignetSetting temp = null;
				try{
					temp = signetSettingService.getSignetSetting(id);
				}catch(NullPointerException e){}
			...
			signetSettingService.deleteSignetSetting(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteSignetSettingByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				SignetSetting temp = null;
				try{
					temp = signetSettingService.loadSignetSettingByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getSignetSettingPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(SignetSetting.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),SignetSetting.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,SignetSetting.class);
				} catch (Exception e) {
			...
			Page<SignetSetting> page = new Page<SignetSetting>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<SignetSetting> datas = signetSettingService.getByPage(page, detachedCriteria);
			List<SignetSetting> signetSettingList = datas.getResult();
			if (signetSettingList == null || signetSettingList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(signetSettingList.toArray());
			PaginationDTO<SignetSetting> pagination = new PaginationDTO<SignetSetting>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			SignetSetting signetSetting = signetSettingService.getSignetSetting(id);
			if (signetSetting == null || !signetSetting.isValid()) {
				result.setErrorMessage("找不到印章管理");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取印章管理对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param signetSetting
	 * @return
	 */
	private SignetSetting getSignetSetting(SignetSetting signetSetting) {
		if (signetSetting.getId() == null && (signetSetting.getId() == null || signetSetting.getId().toString().trim() == ""))
			throw new RuntimeException("印章管理对象业务主键和id不能同时为空");
		if (signetSetting.getId() != null) {
			SignetSetting result = signetSettingService.getSignetSetting(signetSetting .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return signetSettingService.loadSignetSettingByBussinessKey(signetSetting.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个印章管理，则返回false，否则返回true
	 * @param signetSetting
	 */
	private boolean beforeUpdateSignetSetting(SignetSetting signetSetting) {
		SignetSetting temp = getSignetSetting(signetSetting);
		if (temp == null) {
			return false;
		}
		if(signetSetting.getId() == null){
			signetSetting.setId(temp.getId());		
		}
		if(signetSetting.getId() == null){
			signetSetting.setId(temp.getId());
		}
		
		signetSetting.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除印章管理对象下所有辅模型对象中的印章管理对象
	 * @param signetSetting
	 */
	private void clearCircle(SignetSetting signetSetting) {
	 ...
	 */
	private void checkSignetSettingData(SignetSetting signetSetting){
	}
	/* CUSTOM CODE START(wsserviceimpl,functions,X6Basic_1.0_signetManage_SignetSetting,X6Basic_1.0) */
// 自定义代码
