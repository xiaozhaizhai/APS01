...
@Repository
public class AttMachineDaoImpl extends ExtendedGenericDaoImpl<AttMachine, Long> implements AttMachineDao {
	@Override
	...
	@Override
	public AttMachineDealInfo copyAndSaveDealInfo(DealInfo di) {
		AttMachineDealInfo dealInfo = new AttMachineDealInfo();
	...
	@Override
	public AttMachineDealInfo copyAndSaveDealInfo(DealInfo di, AttMachine mainObj, Staff dealStaff) {
		AttMachineDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(AttMachineDealInfo.class);
		...
		List<AttMachineDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
