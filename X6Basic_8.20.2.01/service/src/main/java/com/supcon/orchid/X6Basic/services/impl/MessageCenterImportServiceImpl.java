...
@Service("messageCenterImportService")
public class MessageCenterImportServiceImpl implements MessageCenterImportService {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MessageCenter bizObj, MessageCenter oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("X6Basic.entityname.radion1370247414253") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getEmailTitle() ? "" : "," + oldBizObj.getEmailTitle().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getEmailTitle() ? "" : oldBizObj.getEmailTitle().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("X6Basic.entityname.radion1370247414253") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getEmailTitle() ? "" : "," + bizObj.getEmailTitle().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getEmailTitle() ? "" : bizObj.getEmailTitle().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(MessageCenter bizObj, String bussinesskeyFieldName,
