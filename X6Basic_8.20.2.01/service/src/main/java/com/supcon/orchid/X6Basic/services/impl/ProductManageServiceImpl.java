...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.ProductManageDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_productManageService")
@Transactional
public class ProductManageServiceImpl  extends BaseServiceImpl<ProductManage> implements ProductManageService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<ProductManage> bapBaseService;
	...
	@Autowired
	private ProductManageImportService productManageImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + ProductManage.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  productManageDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + ProductManage.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = productManageDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + ProductManage.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + ProductManage.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  productManageDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + ProductManage.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + ProductManage.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<ProductManage> list = (List<ProductManage>) productManageDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public ProductManage getProductManage(long id){
		return getProductManage(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public ProductManage getProductManage(long id, String viewCode){
		ProductManage productManage = productManageDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProductManageAsJSON(long id, String include){
		ProductManage productManage = productManageDao.load(id);
		try {
			productManage.setExtraColMap(XmlUtils.xml2Map(productManage.getExtraCol()));
		} catch (Exception e) {
		...
		return JSONPlainSerializer.serializeAsJSON(productManage, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.common.delete",operType="3")
	public void deleteProductManage(ProductManage productManage){
		if(null != productManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || ProductManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productManage.getCode() ? "" : productManage.getCode().toString());
			AuditUtil.setColumnStringA(null == productManage.getName() ? "" : productManage.getName().toString());
			AuditUtil.setColumnStringC(productManage.getId().toString());
		}
		beforeDeleteProductManage(productManage);
		productManageDao.delete(productManage);
		afterDeleteProductManage(productManage);
		productManageDao.flush();
		List<ProductManage> params = new ArrayList<ProductManage>();
		params.add(productManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_baseProduct");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.common.delete",operType="3")
	public void deleteProductManage(List<Long> productManageIds){
		deleteProductManage(productManageIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.common.delete",operType="3")
	public void deleteProductManage(List<Long> productManageIds, String eventTopic) {
		List<ProductManage> productManages = new ArrayList<ProductManage>();
		for(Long productManageId : productManageIds){
			ProductManage productManage = getProductManage(productManageId);
			productManages.add(productManage);
			if(null != productManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || ProductManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == productManage.getName() ? "" : "," + productManage.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == productManage.getName() ? "" : productManage.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == productManage.getCode() ? "" : "," + productManage.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == productManage.getCode() ? "" : productManage.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == productManage.getId() ? "" : "," + productManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == productManage.getId() ? "" : productManage.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(productManages != null && productManages.size() > 0 ){
			for(ProductManage productManage : productManages){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),productManage, productManage.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(productManageIds != null && productManageIds.size() > 0) {
			String hql = "update " + ProductManage.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = productManageDao.createQuery(hql);
			query.setParameterList("ids", productManageIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(ProductManage productManagez : productManages){
				productManagez.setValid(false);
				List<ProductManage> params = new ArrayList<ProductManage>();
				params.add(productManagez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_baseProduct");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.common.delete",operType="3")
	public void deleteProductManage(long productManageId, int productManageVersion){
		this.deleteProductManage(productManageId, productManageVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.common.delete",operType="3")
	public void deleteProductManage(long productManageId, int productManageVersion,SignatureLog signatureLog){
		ProductManage productManage = getProductManage(productManageId);
		if(productManage != null && !productManage.isValid()){
		...
		beforeDeleteProductManage(productManage);
		productManageDao.delete(productManageId, productManageVersion);
		afterDeleteProductManage(productManage);
		productManageDao.flush();
		List<ProductManage> params = new ArrayList<ProductManage>();
		params.add(productManage);
		...
		if(signatureLog != null ) {
			...
			businessKey=productManage.getCode();
			signatureLog.setTableId(productManage.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.common.delete",operType="3")
	public void deleteProductManage(String productManageIds){
		this.deleteProductManage(productManageIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.common.delete",operType="3")
	public void deleteProductManage(String productManageIds,SignatureLog signatureLog){
		deleteCollection(productManageIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProductManage(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_baseProduct#modelCode:X6Basic_1.0_baseProduct_ProductManage#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="ec.engine.restore",operType="10")
	public void restoreProductManage(String productManageIds){
		restoreCollection(productManageIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProductManage(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="ec.engine.restore",operType="10")
	public void restoreProductManage(long productManageId){
		
		findBusinessKeyUsed(productManageId);	//判断业务主键是否重复
		...
		beforeRestoreProductManage(productManage);
		productManage.setValid(true);
		productManageDao.update(productManage);
		afterRestoreProductManage(productManage);
		List<ProductManage> params = new ArrayList<ProductManage>();
		params.add(productManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long productManageId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_baseProduct_ProductManage");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + ProductManage.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ ProductManage.TABLE_NAME +" where id = ? )";
			List<Object> list =  productManageDao.createNativeQuery(sql,productManageId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProductManage(List<ProductManage>  productManages){
		for(ProductManage productManage:productManages)  {
			saveProductManage(productManage, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProductManage(List<ProductManage>  productManages){
		for(ProductManage productManage:productManages)  {
			saveProductManage(productManage, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="ec.flow.save")
	public void saveProductManage(ProductManage productManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProductManage(productManage, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="ec.flow.save")
	public void saveProductManage(ProductManage productManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProductManage(productManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProductManageCodes(ProductManage productManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductManageCodes(productManage, false);
	}
	
	public void generateProductManageCodes(ProductManage productManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProductManageSummarys(ProductManage productManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductManageSummarys(productManage, false);
	}

	public void generateProductManageSummarys(ProductManage productManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage")
	public void saveProductManage(ProductManage productManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProductManage(productManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage")
	public void saveProductManage(ProductManage productManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_baseProduct";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_baseProduct");
		String url = null;
		if(productManage.getId() != null && productManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || ProductManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_baseProduct/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || ProductManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_baseProduct/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, productManage);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(productManage);
		beforeSaveProductManage(productManage, viewIsView);

		if (viewIsView) {
			productManageDao.saveWithRevertVersion(productManage);
		} else {
			if(isNew)
				productManageDao.save(productManage);
			else
				productManageDao.merge(productManage);
		}
		if(null != productManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || ProductManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productManage.getCode() ? "" : productManage.getCode().toString());
			AuditUtil.setColumnStringA(null == productManage.getName() ? "" : productManage.getName().toString());
		...
		dealDatagridsSave(productManage,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProductManageCodes(productManage, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProductManageSummarys(productManage, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProductManage(productManage, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, productManage);
			}
		}
		List<ProductManage> params = new ArrayList<ProductManage>();
		productManageDao.flush();
		productManageDao.clear();
		productManage = productManageDao.load(productManage.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, ProductManage productManage){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("productManage", productManage);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage")
	public void mergeProductManage(ProductManage productManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(productManage);
		beforeSaveProductManage(productManage);
		productManageDao.merge(productManage);
		if(null != productManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || ProductManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productManage.getCode() ? "" : productManage.getCode().toString());
			AuditUtil.setColumnStringA(null == productManage.getName() ? "" : productManage.getName().toString());
		...
		dealDatagridsSave(productManage,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProductManage(productManage);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<ProductManage> findProductManages(Page<ProductManage> page, Criterion... criterions) {
		return productManageDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private ProductManageDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<ProductManageDealInfo> list=productManageDao.createQuery("from " + ProductManageDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(ProductManageDealInfo dealInfo) {
		productManageDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+ProductManageDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) productManageDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+ProductManageDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = productManageDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = productManageDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ ProductManageDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = productManageDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductManages(Page<ProductManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProductManages(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductManages(Page<ProductManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductManages(Page<ProductManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<ProductManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"productManage\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"productManage\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"productManage\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_baseProduct_ProductManage" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_baseProduct_ProductManage" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  productManageDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  productManageDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_baseProduct").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productManage\"","X6Basic_1.0_baseProduct",customCondition,"X6BASIC_PRODUCT_MENAGE",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"productManage\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+ProductManageDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"productManage\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"productManage\".LAY_REC ASC, \"productManage\".SORT ASC");
				} else {
					orderPart.append(" \"productManage\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) productManageDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=productManageDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = productManageDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<ProductManage> result = new ArrayList<ProductManage>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(ProductManage.class,productManageDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(ProductManage.class,productManageDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (ProductManage productManage : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(productManage.getId(), "X6Basic_baseProduct_productManage");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(productManage.getId(), "X6Basic_baseProduct_productManage");
							if(null != documents && !documents.isEmpty()) {
								productManage.setDocument(documents.get(0));
								productManage.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_baseProduct")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_baseProduct")+"X6Basic_1.0_baseProduct".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_baseProduct",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<ProductManage> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<ProductManage> result) {
		View view = null;
		...
		Map<Long, ProductManage> rsMap = new HashMap<Long, ProductManage>();
		for (ProductManage productManage : result) {
			ids.add(productManage.getId());
			rsMap.put(productManage.getId(), productManage);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(productManageDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = productManageDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(ProductManage productManage,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(productManage.getOwnerStaffId());
		ti.setOwnerPositionId(productManage.getOwnerPositionId());
		ti.setOwnerDepartmentId(productManage.getOwnerDepartmentId());
		
		ti.setCreateStaff(productManage.getCreateStaff());
		ti.setCreateStaffId(productManage.getCreateStaffId());
		ti.setCreateTime(productManage.getCreateTime());
		ti.setDeleteStaff(productManage.getDeleteStaff());
		ti.setDeleteStaffId(productManage.getDeleteStaffId());
		ti.setDeleteTime(productManage.getDeleteTime());
		ti.setCreateDepartmentId(productManage.getCreateDepartmentId());
		ti.setModifyStaff(productManage.getModifyStaff());
		ti.setModifyStaffId(productManage.getModifyStaffId());
		ti.setCreatePositionId(productManage.getCreatePositionId());
		ti.setPositionLayRec(productManage.getPositionLayRec());
		ti.setTableNo(productManage.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_baseProduct");
		ti.setTargetTableName("X6BASIC_PRODUCT_MENAGE");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_baseProduct_ProductManage" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  productManageDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productManage\"","X6Basic_1.0_baseProduct","","X6BASIC_PRODUCT_MENAGE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"productManage\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"productManage\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productManage\"","X6Basic_1.0_baseProduct","","X6BASIC_PRODUCT_MENAGE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("productManage.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"productManage\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"productManage\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"productManage\".CID = 1 OR \"productManage\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("productManage.mc\"").append(".PRODUCT_MANAGE) not in ("+notinValue+")");
			}
			
			SQLQuery query = productManageDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("baseProduct", "X6Basic_1.0_baseProduct", "baseProduct_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=productManageDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return ProductManage.ENTITY_CODE;}
	protected String getModuleCode(){return ProductManage.MODULE_CODE;}
	protected String getModelCode(){return ProductManage.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param productManage 模型实例
	 */
	public void checkUniqueConstraint(ProductManage productManage){
		if(productManage.getId()==null){
			List<ProductManage> productManages = productManageDao.findByCriteria(Restrictions.eq("code", productManage.getCode()), Restrictions.eq("valid", true));
			if(productManages!=null && !productManages.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1371468505565", getCurrentUser().getLanguage()), productManage.getCode());
			}
		} else {
			List<ProductManage> productManages = productManageDao.findByCriteria(Restrictions.ne("id", productManage.getId()), Restrictions.eq("code", productManage.getCode()), Restrictions.eq("valid", true));
			if(productManages!=null && !productManages.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1371468505565", getCurrentUser().getLanguage()), productManage.getCode());
			}
	...
	@Override
	public List<ProductManage> getProductManages(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(ProductManage.class);
		if(params!=null){
			...
			return productManageDao.findByHql("from " + ProductManage.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return productManageDao.findByCriteria(detachedCriteria);
	}
	...
	public List<ProductManage> findProductManagesByHql(String hql, Object... objects){
		...
		return   productManageDao.findByHql(hql, objects);
	}
	
	@Override
	public ProductManage loadProductManageByBussinessKey(ProductManage productManage){
		return loadProductManageByBussinessKey(productManage.getCode());
	}
	
	@Override
	public Page<ProductManage> getProductManages(Page<ProductManage> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(ProductManage.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = productManageDao.findByPage(page, sql);
			}else{
				page = productManageDao.findAllByPage(page);
			}
			return page;
		}
		page = productManageDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public ProductManage loadProductManageByBussinessKey(Serializable bussinessKey){
		return productManageDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<ProductManage> findByProperty(String propertyName, Object object){
		return productManageDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public ProductManage findEntityByProperty(String propertyName, Object object){
		return productManageDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + ProductManage.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = productManageDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_baseProduct_ProductManage","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_baseProduct", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(ProductManage productManage,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,productManageDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), ProductManage.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, productManage);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",productManage.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ ProductManage.TABLE_NAME +" where valid = 1";
			list = productManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ ProductManage.TABLE_NAME;
			list = productManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ ProductManage.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = productManageDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProductManage(final List<ProductManage> insertObjs, final List<ProductManage> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProductManage(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProductManage(final List<ProductManage> insertObjs, final List<ProductManage> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1371468466670"),importCount));
		...
		Map<Long, ProductManage> oldUpdateObjsMap = new HashMap<Long, ProductManage>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			ProductManage bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<ProductManage> bizObjList = productManageDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					ProductManage bizObj = bizObjList.get(mIndex);
		...
		final List<ProductManage> nmeObjs = new ArrayList<ProductManage>();
		...
		long id=autoGeneratorID.getNextId(ProductManage.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(ProductManage productManage:insertObjs)  {
			//id = autoGeneratorID.getNextId(ProductManage.TABLE_NAME,1,dbId);
			if(productManage.getCode() != null && !productManage.getCode().equals("") ){
				m.put(productManage.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) productManageDao.createNativeQuery(sql, "X6Basic_1.0_baseProduct_ProductManage"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		productManageDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + ProductManage.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						ProductManage productManage = updateObjs.get(updateIndex);
						Long id = productManage.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = productManage.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(productManage);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + ProductManage.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + ProductManage.TABLE_NAME + " order by id desc";
//					Query query = productManageDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						ProductManage productManage = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = productManage.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		productManageDaoImpl.getSession().flush();
		productManageDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			ProductManage bizObj = insertObjs.get(insertIndex);
			productManageImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			ProductManage bizObj = updateObjs.get(updateIndex);
			ProductManage oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			productManageImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = productManageDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				productManageDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = productManageDao.createNativeQuery(sql, "X6Basic_1.0_baseProduct_ProductManage").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = productManageDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = productManageDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = productManageDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?productManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  productManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,productManageDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="ec.flow.save")
	public void saveProductManage(ProductManage productManage, DataGridService dataGridService){
		saveProductManage(productManage, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="ec.flow.save")
	public void saveProductManage(ProductManage productManage, DataGridService dataGridService, String viewCode){
		saveProductManage(productManage, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage")
	public void saveProductManage(ProductManage productManage, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProductManage(productManage, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage")
	public void saveProductManage(ProductManage productManage, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_baseProduct";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_baseProduct");
		String url = null;
		if(null != productManage.getId() && productManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || ProductManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage")
	public Map<Object, Object> productManageDataGridImport(ProductManage productManage, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = productManage.getVirtualId();
		
		productManage = productManageDao.load(productManage.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, productManage.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = productManage.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(productManage);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, productManage.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage")
	public void mergeProductManage(ProductManage productManage, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(productManage);
		beforeSaveProductManage(productManage);
		productManageDao.merge(productManage);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_baseProduct",model="X6Basic_1.0_baseProduct_ProductManage",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<ProductManage> getByPage(Page<ProductManage> page,DetachedCriteria detachedCriteria){
		return productManageDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_baseProduct").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveProductManage(ProductManage productManage, Object... objects){
		checkUniqueConstraint(productManage);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProductManage(ProductManage productManage, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProductManage(ProductManage productManage){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProductManage(ProductManage productManage){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProductManage(ProductManage productManage){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProductManage(ProductManage productManage){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(ProductManage productManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(ProductManage productManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProductManage(List<ProductManage> insertObjs, List<ProductManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProductManage,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProductManage(List<ProductManage> insertObjs, List<ProductManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProductManage,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
