...
/* CUSTOM CODE START(action,import,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicMailServerAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicMailServer> {
	...
	private X6BasicMailServer mailServer;
	...
	//导出excel时,主模型内容
	List<X6BasicMailServer> queryResults = new ArrayList<X6BasicMailServer>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/mailServer/mailServer/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/mailServer/mailServer/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_mailServer_MailServer" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicMailServer> mailServers;
	
	public List<X6BasicMailServer> getMailServers() {
		return mailServers;
	}
	...
	private Page<X6BasicMailServer> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			mailServer = new X6BasicMailServer();
			if(mailServer.getDbType() != null) {
				mailServer.setDbType((SystemCode) systemCodeService.getSystemCode(mailServer.getDbType().getId()));
			}
		} else {
			if(id > 0){
				mailServer = mailServerService.getMailServer(id, viewCode);
			}
		}
	...
	public Page<X6BasicMailServer> getPage() {
		if (null == page) {
			page = new Page<X6BasicMailServer>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicMailServer getMailServer() {
		return mailServer;
	}		

	public void setMailServer(X6BasicMailServer mailServer) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/mailServer/mailServer/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = mailServerService.getMailServerAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/mailServer/mailServer/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1404797483241
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String mailServerReferenceCopy() throws Exception{
			X6BasicMailServer oldMailServer = null;
			if(isEditNew()){
				...
				oldMailServer = new X6BasicMailServer();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,mailServer.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, mailServer.getId());
				...
				oldMailServer = mailServerService.getMailServer(id);
			}
			
			
			mailServer = mailServerService.getMailServer(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_mailServer_MailServer");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicMailServer.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMailServer, null);
						Method setValue = X6BasicMailServer.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(mailServer, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			mailServer.setCreateDepartmentId(oldMailServer.getCreateDepartmentId());
			mailServer.setCreatePositionId(oldMailServer.getCreatePositionId());
			mailServer.setCreateStaffId(oldMailServer.getCreateStaffId());
			mailServer.setOwnerDepartmentId(oldMailServer.getOwnerDepartmentId());
			mailServer.setOwnerPositionId(oldMailServer.getOwnerPositionId());
			mailServer.setOwnerStaffId(oldMailServer.getOwnerStaffId());
			mailServer.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_mailServer_MailServer", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/mailServer/mailServer/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/mailServer/mailServer/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=mailServerService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return mailServerReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			mailServer = mailServerService.getMailServer(this.mailServer.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,mailServer.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/mailServer/mailServer/dbView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dbView.ftl") }),
		@Action(value = "/X6Basic/mailServer/mailServer/dbViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dbViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/mailServer/mailServer/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = mailServerService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(mailServerService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(mailServerService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_mailServer_MailServer", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/mailServer/mailServer/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/mailServer/mailServer/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.dbName,result.dbType.id,result.dbType.value,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"mailServer\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"mailServer\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"mailServer\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"mailServer\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/mailServer/mailServer/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/mailServer/mailServer/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.dbName,result.dbType.id,result.dbType.value,result.attrMap.*"})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/mailServer/mailServer/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_mailServer_MailServer");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		mailServerService.findDataGridPage(dg,dgclass,getDgPage(),mailServer,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(mailServer.getId() != null && mailServer.getId() != -1){
				mailServer = mailServerService.getMailServer(mailServer.getId());
			}
			mailServerService.findDataGridPage(dg,dgclass,getDgPage(),mailServer,searchCondition,params);
	...
	private void save() throws Exception {
		...
		mailServerService.saveMailServer(mailServer,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/mailServer/mailServer/dbView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/mailServer/mailServer/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "submit");
		bap_validate_datagrids=mailServerService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", mailServer.getId());
		...
		fileuploads.put("linkId", mailServer.getId());
		bapAfterCustomMethod("X6Basic_1.0_mailServer_MailServer", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/mailServer/mailServer/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		mailServerService.deleteMailServer(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_mailServer_MailServer", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/mailServer/mailServer/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "restore");
		mailServerService.restoreMailServer(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_mailServer_MailServer", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/mailServer/mailServer/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/mailServer/mailServer/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "showDealInfoData");
		if (null != tableInfoId) {
			int count=mailServerService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = mailServerService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = mailServerService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_mailServer_MailServer", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicMailServer> findExportDataInfos(Page<X6BasicMailServer> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = mailServerService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = mailServerService.getAssProperty(p.getCode());	
									//String assModelCode = mailServerService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			mailServerService.findMailServers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			mailServerService.findMailServers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicMailServer> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicMailServer result : results){
	...
	public boolean isEditNew(){
		//return null == this.mailServer || null == this.mailServer.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/mailServer/mailServer/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_mailServer_MailServer", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/mailServer/mailServer/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/mailServer/mailServer/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/mailServer/mailServer/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicMailServerService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_mailServer", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_mailServer");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_mailServer_MailServer", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/mailServer/mailServer/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_mailServer_MailServer", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_mailServer_MailServer", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/mailServer/mailServer/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/mailServer/mailServer/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_mailServer_MailServer";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_mailServer_MailServer", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/mailServer/mailServer/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == mailServer.getCode() ? "" : mailServer.getCode().toString());
		_auditLog.setColumnStringA(null == mailServer.getName() ? "" : mailServer.getName().toString());
		_auditLog.setColumnStringC(mailServer.getId().toString());
		_auditLog.setColumnStringD(X6BasicMailServer.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_mailServer");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicMailServerAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, mailServer,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, mailServer,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,mailServer);
		...
		printlog.setEntity("X6Basic_1.0_mailServer");
		...
		printlog.setBusinessId(mailServer.getId());
		printlog.setBussinessKey(null == mailServer.getCode() ? "" : mailServer.getCode().toString());
		printlog.setMainDisplay(null == mailServer.getName() ? "" : mailServer.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/mailServer/mailServer/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(mailServer,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/mailServer/mailServer/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(mailServerService.getMailServer(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/mailServer/mailServer/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/mailServer/mailServer/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, mailServer,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				mailServer = mailServerService.getMailServer(Long.parseLong(arrId[i]));
				if(null != mailServer){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == mailServer.getName() ? "" : "," + mailServer.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == mailServer.getName() ? "" : mailServer.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == mailServer.getCode() ? "" : "," + mailServer.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == mailServer.getCode() ? "" : mailServer.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == mailServer.getId() ? "" : "," + mailServer.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == mailServer.getId() ? "" : mailServer.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, mailServer,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,mailServer);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/mailServer/mailServer/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicMailServer> findExportPageInfos(Page<X6BasicMailServer> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_mailServer" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = mailServerService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicMailServer queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = mailServerService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = mailServerService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_mailServer_MailServer");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_mailServer_MailServer")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_mailServer_MailServer");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_mailServer_MailServer");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = mailServerService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
