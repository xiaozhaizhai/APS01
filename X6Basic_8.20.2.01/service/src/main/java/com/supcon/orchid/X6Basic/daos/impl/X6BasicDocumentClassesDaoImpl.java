...
@Repository
public class X6BasicDocumentClassesDaoImpl extends TreeDaoImpl<X6BasicDocumentClasses, Long> implements X6BasicDocumentClassesDao {
	@Override
	...
	@Override
	public X6BasicDocumentClassesDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicDocumentClassesDealInfo dealInfo = new X6BasicDocumentClassesDealInfo();
	...
	@Override
	public X6BasicDocumentClassesDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicDocumentClasses mainObj, Staff dealStaff) {
		X6BasicDocumentClassesDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicDocumentClassesDealInfo.class);
		...
		List<X6BasicDocumentClassesDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_documentClass_DocumentClasses,X6Basic_1.0) */
// 自定义代码
