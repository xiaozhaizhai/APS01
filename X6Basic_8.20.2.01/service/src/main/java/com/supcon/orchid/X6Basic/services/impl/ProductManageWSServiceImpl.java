...
import com.supcon.orchid.X6Basic.services.ProductManageWSService;
import com.supcon.orchid.X6Basic.services.ProductManageService;
/* CUSTOM CODE START(wsserviceimpl,import,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="ProductManage",targetNamespace = "http://ws.supcon.com")
public class ProductManageWSServiceImpl implements ProductManageWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getProductManageByBusinessKey(
		...
		try{
			...
			ProductManage productManage = productManageService.loadProductManageByBussinessKey(businessKey);
			if(productManage == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, productManage);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateProductManage(
		...
		try{
			if(productManage == null){
				return set500Error(result, "物品管理不能为空。");
			}
			...
			if(!beforeUpdateProductManage(productManage)){
				return set404Error(result);
			}
			productManageService.saveProductManage(productManage, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addProductManage(
		...
		try{
			...
			try{
				checkProductManageData(productManage);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			productManageService.saveProductManage(productManage, null);
			setSuccessResult(result, productManage.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteProductManageByBusinessKey(
		...
		try{
			...
			try{
				productManage = productManageService.loadProductManageByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddProductManage(
		...
		try{
			...
			int count = 1;
			for(ProductManage productManage : productManageList){
				if(productManage.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(productManage.getId() == null || productManage.getId().toString().trim().isEmpty()){
			...
			productManageService.batchImportBaseProductManage(productManageList);
			for(ProductManage productManage : productManageList){
				idList.add(productManage.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteProductManageByPk(
		...
		try{
			...
			for(String k : pk){
				...
				ProductManage temp = null;
				try{
					temp = productManageService.getProductManage(id);
				}catch(NullPointerException e){}
			...
			productManageService.deleteProductManage(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteProductManageByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				ProductManage temp = null;
				try{
					temp = productManageService.loadProductManageByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getProductManagePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(ProductManage.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),ProductManage.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,ProductManage.class);
				} catch (Exception e) {
			...
			Page<ProductManage> page = new Page<ProductManage>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<ProductManage> datas = productManageService.getByPage(page, detachedCriteria);
			List<ProductManage> productManageList = datas.getResult();
			if (productManageList == null || productManageList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(productManageList.toArray());
			PaginationDTO<ProductManage> pagination = new PaginationDTO<ProductManage>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			ProductManage productManage = productManageService.getProductManage(id);
			if (productManage == null || !productManage.isValid()) {
				result.setErrorMessage("找不到物品管理");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取物品管理对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param productManage
	 * @return
	 */
	private ProductManage getProductManage(ProductManage productManage) {
		if (productManage.getId() == null && (productManage.getId() == null || productManage.getId().toString().trim() == ""))
			throw new RuntimeException("物品管理对象业务主键和id不能同时为空");
		if (productManage.getId() != null) {
			ProductManage result = productManageService.getProductManage(productManage .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return productManageService.loadProductManageByBussinessKey(productManage.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个物品管理，则返回false，否则返回true
	 * @param productManage
	 */
	private boolean beforeUpdateProductManage(ProductManage productManage) {
		ProductManage temp = getProductManage(productManage);
		if (temp == null) {
			return false;
		}
		if(productManage.getId() == null){
			productManage.setId(temp.getId());		
		}
		if(productManage.getId() == null){
			productManage.setId(temp.getId());
		}
		
		productManage.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除物品管理对象下所有辅模型对象中的物品管理对象
	 * @param productManage
	 */
	private void clearCircle(ProductManage productManage) {
	 ...
	 */
	private void checkProductManageData(ProductManage productManage){
	}
	/* CUSTOM CODE START(wsserviceimpl,functions,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
