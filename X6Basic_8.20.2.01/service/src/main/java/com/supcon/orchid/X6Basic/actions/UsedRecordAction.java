...
/* CUSTOM CODE START(action,import,X6Basic_1.0_meetingroom_UsedRecord,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class UsedRecordAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<UsedRecord> {
	...
	//导出excel时,主模型内容
	List<UsedRecord> queryResults = new ArrayList<UsedRecord>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/meetingroom/usedRecord/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/meetingroom/usedRecord/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_meetingroom_UsedRecord" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	public void prepare() throws Exception {
		if (null == id) {
			usedRecord = new UsedRecord();
		} else {
			if(id > 0){
				usedRecord = usedRecordService.getUsedRecord(id, viewCode);
			}
		}
	...
	public Page<UsedRecord> getPage() {
		if (null == page) {
			page = new Page<UsedRecord>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public UsedRecord getUsedRecord() {
		return usedRecord;
	}		

	public void setUsedRecord(UsedRecord usedRecord) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/meetingroom/usedRecord/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = usedRecordService.getUsedRecordAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/meetingroom/usedRecord/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1367134288546
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String usedRecordReferenceCopy() throws Exception{
			UsedRecord oldUsedRecord = null;
			if(isEditNew()){
				...
				oldUsedRecord = new UsedRecord();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,usedRecord.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, usedRecord.getId());
				...
				oldUsedRecord = usedRecordService.getUsedRecord(id);
			}
			
			
			usedRecord = usedRecordService.getUsedRecord(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_meetingroom_UsedRecord");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = UsedRecord.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldUsedRecord, null);
						Method setValue = UsedRecord.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(usedRecord, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			usedRecord.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=usedRecordService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return usedRecordReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			usedRecord = usedRecordService.getUsedRecord(this.usedRecord.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,usedRecord.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = usedRecordService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(usedRecordService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(usedRecordService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_meetingroom_UsedRecord", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"usedRecord\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"usedRecord\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"usedRecord\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"usedRecord\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		usedRecordService.findDataGridPage(dg,dgclass,getDgPage(),usedRecord,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(usedRecord.getId() != null && usedRecord.getId() != -1){
				usedRecord = usedRecordService.getUsedRecord(usedRecord.getId());
			}
			usedRecordService.findDataGridPage(dg,dgclass,getDgPage(),usedRecord,searchCondition,params);
	...
	private void save() throws Exception {
		...
		usedRecordService.saveUsedRecord(usedRecord,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "submit");
		bap_validate_datagrids=usedRecordService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", usedRecord.getId());
		...
		fileuploads.put("linkId", usedRecord.getId());
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/meetingroom/usedRecord/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		usedRecordService.deleteUsedRecord(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/usedRecord/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "restore");
		usedRecordService.restoreUsedRecord(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<UsedRecord> findExportDataInfos(Page<UsedRecord> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = usedRecordService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = usedRecordService.getAssProperty(p.getCode());	
									//String assModelCode = usedRecordService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			usedRecordService.findUsedRecords(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			usedRecordService.findUsedRecords(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<UsedRecord> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(UsedRecord result : results){
	...
	public boolean isEditNew(){
		//return null == this.usedRecord || null == this.usedRecord.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/meetingroom/usedRecord/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/meetingroom/usedRecord/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/meetingroom/usedRecord/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/usedRecord/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.UsedRecordService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/usedRecord/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.UsedRecordService"
						, "com.supcon.orchid.foundation.services.StaffService"       
						, "com.supcon.orchid.X6Basic.services.MeetingRoomService"       
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_meetingroom_UsedRecord", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_meetingroom_UsedRecord", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/meetingroom/usedRecord/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/meetingroom/usedRecord/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_meetingroom_UsedRecord";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_UsedRecord", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/meetingroom/usedRecord/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == usedRecord.getId() ? "" : usedRecord.getId().toString());
		_auditLog.setColumnStringA(null == usedRecord.getId() ? "" : usedRecord.getId().toString());
		_auditLog.setColumnStringC(usedRecord.getId().toString());
		_auditLog.setColumnStringD(UsedRecord.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.UsedRecordAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, usedRecord,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, usedRecord,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,usedRecord);
		...
		printlog.setBusinessId(usedRecord.getId());
		printlog.setBussinessKey(null == usedRecord.getId() ? "" : usedRecord.getId().toString());
		printlog.setMainDisplay(null == usedRecord.getId() ? "" : usedRecord.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/meetingroom/usedRecord/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(usedRecord,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/usedRecord/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(usedRecordService.getUsedRecord(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/meetingroom/usedRecord/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/meetingroom/usedRecord/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, usedRecord,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				usedRecord = usedRecordService.getUsedRecord(Long.parseLong(arrId[i]));
				if(null != usedRecord){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == usedRecord.getId() ? "" : "," + usedRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == usedRecord.getId() ? "" : usedRecord.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == usedRecord.getId() ? "" : "," + usedRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == usedRecord.getId() ? "" : usedRecord.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == usedRecord.getId() ? "" : "," + usedRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == usedRecord.getId() ? "" : usedRecord.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, usedRecord,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,usedRecord);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/meetingroom/usedRecord/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_meetingroom_UsedRecord");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_meetingroom_UsedRecord")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_meetingroom_UsedRecord");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_meetingroom_UsedRecord");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = usedRecordService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_meetingroom_UsedRecord,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_meetingroom_UsedRecord,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_meetingroom_UsedRecord,X6Basic_1.0) */
// 自定义代码
