...
import com.supcon.orchid.X6Basic.services.X6BasicBaseUnitImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicBaseUnitDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_baseUnitService")
@Transactional
public class X6BasicBaseUnitServiceImpl  extends BaseServiceImpl<X6BasicBaseUnit> implements X6BasicBaseUnitService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicBaseUnit> bapBaseService;
	...
	@Autowired
	private X6BasicBaseUnitImportService baseUnitImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicBaseUnit.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  baseUnitDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicBaseUnit.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = baseUnitDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicBaseUnit.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicBaseUnit.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  baseUnitDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicBaseUnit.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicBaseUnit.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicBaseUnit> list = (List<X6BasicBaseUnit>) baseUnitDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicBaseUnit getBaseUnit(long id){
		return getBaseUnit(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicBaseUnit getBaseUnit(long id, String viewCode){
		X6BasicBaseUnit baseUnit = baseUnitDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBaseUnitAsJSON(long id, String include){
		X6BasicBaseUnit baseUnit = baseUnitDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(baseUnit, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.common.delete",operType="3")
	public void deleteBaseUnit(X6BasicBaseUnit baseUnit){
		if(null != baseUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicBaseUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
			AuditUtil.setColumnStringA(null == baseUnit.getName() ? "" : baseUnit.getName().toString());
			AuditUtil.setColumnStringC(baseUnit.getId().toString());
		}
		beforeDeleteBaseUnit(baseUnit);
		baseUnitDao.deletePhysical(baseUnit);
		afterDeleteBaseUnit(baseUnit);
		baseUnitDao.flush();
		List<X6BasicBaseUnit> params = new ArrayList<X6BasicBaseUnit>();
		params.add(baseUnit);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.common.delete",operType="3")
	public void deleteBaseUnit(List<Long> baseUnitIds){
		deleteBaseUnit(baseUnitIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.common.delete",operType="3")
	public void deleteBaseUnit(List<Long> baseUnitIds, String eventTopic) {
		List<X6BasicBaseUnit> baseUnits = new ArrayList<X6BasicBaseUnit>();
		for(Long baseUnitId : baseUnitIds){
			X6BasicBaseUnit baseUnit = getBaseUnit(baseUnitId);
			baseUnits.add(baseUnit);
			if(null != baseUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicBaseUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == baseUnit.getName() ? "" : "," + baseUnit.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == baseUnit.getName() ? "" : baseUnit.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == baseUnit.getId() ? "" : "," + baseUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == baseUnit.getId() ? "" : "," + baseUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
				}
			}
		}
		...
		if(baseUnits != null && baseUnits.size() > 0 ){
			for(X6BasicBaseUnit baseUnit : baseUnits){
				...
				java.lang.reflect.Field[] fields = X6BasicBaseUnit.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(baseUnit));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),baseUnit, baseUnit.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(baseUnitIds != null && baseUnitIds.size() > 0) {
			String hql = "delete " + X6BasicBaseUnit.JPA_NAME + " where id in(:ids)";
			Query query = baseUnitDao.createQuery(hql);
			query.setParameterList("ids", baseUnitIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicBaseUnit baseUnitz : baseUnits){
				baseUnitz.setValid(false);
				List<X6BasicBaseUnit> params = new ArrayList<X6BasicBaseUnit>();
				params.add(baseUnitz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_unitGroup");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.common.delete",operType="3")
	public void deleteBaseUnit(long baseUnitId, int baseUnitVersion){
		this.deleteBaseUnit(baseUnitId, baseUnitVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.common.delete",operType="3")
	public void deleteBaseUnit(long baseUnitId, int baseUnitVersion,SignatureLog signatureLog){
		X6BasicBaseUnit baseUnit = getBaseUnit(baseUnitId);
		...
		beforeDeleteBaseUnit(baseUnit);
		baseUnitDao.delete(baseUnitId, baseUnitVersion);
		afterDeleteBaseUnit(baseUnit);
		baseUnitDao.flush();
		List<X6BasicBaseUnit> params = new ArrayList<X6BasicBaseUnit>();
		params.add(baseUnit);
		...
		if(signatureLog != null ) {
			...
			businessKey=baseUnit.getId();
			signatureLog.setTableId(baseUnit.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.common.delete",operType="3")
	public void deleteBaseUnit(String baseUnitIds){
		this.deleteBaseUnit(baseUnitIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.common.delete",operType="3")
	public void deleteBaseUnit(String baseUnitIds,SignatureLog signatureLog){
		deleteCollection(baseUnitIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBaseUnit(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_unitGroup#modelCode:X6Basic_1.0_unitGroup_BaseUnit#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="ec.engine.restore",operType="10")
	public void restoreBaseUnit(String baseUnitIds){
		restoreCollection(baseUnitIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBaseUnit(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="ec.engine.restore",operType="10")
	public void restoreBaseUnit(long baseUnitId){
		
		findBusinessKeyUsed(baseUnitId);	//判断业务主键是否重复
		...
		beforeRestoreBaseUnit(baseUnit);
		baseUnit.setValid(true);
		baseUnitDao.update(baseUnit);
		afterRestoreBaseUnit(baseUnit);
		List<X6BasicBaseUnit> params = new ArrayList<X6BasicBaseUnit>();
		params.add(baseUnit);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long baseUnitId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_unitGroup_BaseUnit");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicBaseUnit.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicBaseUnit.TABLE_NAME +" where id = ? )";
			List<Object> list =  baseUnitDao.createNativeQuery(sql,baseUnitId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBaseUnit(List<X6BasicBaseUnit>  baseUnits){
		for(X6BasicBaseUnit baseUnit:baseUnits)  {
			saveBaseUnit(baseUnit, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBaseUnit(List<X6BasicBaseUnit>  baseUnits){
		for(X6BasicBaseUnit baseUnit:baseUnits)  {
			saveBaseUnit(baseUnit, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="ec.flow.save")
	public void saveBaseUnit(X6BasicBaseUnit baseUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBaseUnit(baseUnit, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="ec.flow.save")
	public void saveBaseUnit(X6BasicBaseUnit baseUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBaseUnit(baseUnit, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBaseUnitCodes(X6BasicBaseUnit baseUnit) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBaseUnitCodes(baseUnit, false);
	}
	
	public void generateBaseUnitCodes(X6BasicBaseUnit baseUnit, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBaseUnitSummarys(X6BasicBaseUnit baseUnit) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBaseUnitSummarys(baseUnit, false);
	}

	public void generateBaseUnitSummarys(X6BasicBaseUnit baseUnit, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit")
	public void saveBaseUnit(X6BasicBaseUnit baseUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBaseUnit(baseUnit, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit")
	public void saveBaseUnit(X6BasicBaseUnit baseUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "X6Basic_1.0_unitGroup");
		String url = null;
		if(baseUnit.getId() != null && baseUnit.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicBaseUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, baseUnit);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(baseUnit);
		beforeSaveBaseUnit(baseUnit, viewIsView);

		if (viewIsView) {
			baseUnitDao.saveWithRevertVersion(baseUnit);
		} else {
			if(isNew)
				baseUnitDao.save(baseUnit);
			else
				baseUnitDao.merge(baseUnit);
		}
		if(null != baseUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicBaseUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
			AuditUtil.setColumnStringA(null == baseUnit.getName() ? "" : baseUnit.getName().toString());
		...
		dealDatagridsSave(baseUnit,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBaseUnitCodes(baseUnit, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBaseUnitSummarys(baseUnit, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBaseUnit(baseUnit, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, baseUnit);
			}
		}
		List<X6BasicBaseUnit> params = new ArrayList<X6BasicBaseUnit>();
		baseUnitDao.flush();
		baseUnitDao.clear();
		baseUnit = baseUnitDao.load(baseUnit.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicBaseUnit baseUnit){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("baseUnit", baseUnit);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit")
	public void mergeBaseUnit(X6BasicBaseUnit baseUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(baseUnit);
		beforeSaveBaseUnit(baseUnit);
		baseUnitDao.merge(baseUnit);
		if(null != baseUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicBaseUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
			AuditUtil.setColumnStringA(null == baseUnit.getName() ? "" : baseUnit.getName().toString());
		...
		dealDatagridsSave(baseUnit,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBaseUnit(baseUnit);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicBaseUnit> findBaseUnits(Page<X6BasicBaseUnit> page, Criterion... criterions) {
		return baseUnitDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseUnits(Page<X6BasicBaseUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBaseUnits(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseUnits(Page<X6BasicBaseUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseUnits(Page<X6BasicBaseUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicBaseUnit> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"baseUnit\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							...
							s.append(" AND \"baseUnit\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"baseUnit\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"baseUnit\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"baseUnit\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			s.append(" ( \"baseUnit\".UNIT_GROUPID IN ( SELECT UNITGROUP_ID FROM S2BASE_UNITGROUP \"unitGroups\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "X6Basic_1.0_unitGroup" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  baseUnitDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  baseUnitDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_unitGroup").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"unitGroups\"","X6Basic_1.0_unitGroup","",true);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"baseUnit\".LAY_REC ASC, \"baseUnit\".SORT ASC");
				} else {
					orderPart.append(" \"baseUnit\".UNIT_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) baseUnitDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=baseUnitDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = baseUnitDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicBaseUnit> result = new ArrayList<X6BasicBaseUnit>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicBaseUnit.class,baseUnitDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicBaseUnit.class,baseUnitDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicBaseUnit baseUnit : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(baseUnit.getId(), "X6Basic_unitGroup_baseUnit");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(baseUnit.getId(), "X6Basic_unitGroup_baseUnit");
							if(null != documents && !documents.isEmpty()) {
								baseUnit.setDocument(documents.get(0));
								baseUnit.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicBaseUnit> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicBaseUnit> result) {
		View view = null;
		...
		Map<Long, X6BasicBaseUnit> rsMap = new HashMap<Long, X6BasicBaseUnit>();
		for (X6BasicBaseUnit baseUnit : result) {
			ids.add(baseUnit.getId());
			rsMap.put(baseUnit.getId(), baseUnit);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(baseUnitDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = baseUnitDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_unitGroup_BaseUnit" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="X6Basic_1.0_unitGroup_BaseUnit" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  baseUnitDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseUnit\"","X6Basic_1.0_unitGroup","","S2BASE_UNIT",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"baseUnit\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"baseUnit\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseUnit\"","X6Basic_1.0_unitGroup","","S2BASE_UNIT",true);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "X6Basic_1.0_unitGroup_BaseUnit" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("baseUnit.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"baseUnit\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("baseUnit.mc\"").append(".BASE_UNIT) not in ("+notinValue+")");
			}
			
			SQLQuery query = baseUnitDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=baseUnitDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicBaseUnit.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicBaseUnit.MODULE_CODE;}
	protected String getModelCode(){return X6BasicBaseUnit.MODEL_CODE;}
	 ...
	 * @param baseUnit 模型实例
	 */
	public void checkUniqueConstraint(X6BasicBaseUnit baseUnit){
	}
	
	
	@Override
	public List<X6BasicBaseUnit> getBaseUnits(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicBaseUnit.class);
		if(params!=null){
			...
			return baseUnitDao.findByHql("from " + X6BasicBaseUnit.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return baseUnitDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<X6BasicBaseUnit> findBaseUnitsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicBaseUnit.class);
	...
	@Override
	public List<X6BasicBaseUnit> findBaseUnitsByHql(String hql, Object... objects){
		
		return   baseUnitDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicBaseUnit loadBaseUnitByBussinessKey(X6BasicBaseUnit baseUnit){
		return loadBaseUnitByBussinessKey(baseUnit.getId());
	}
	
	@Override
	public Page<X6BasicBaseUnit> getBaseUnits(Page<X6BasicBaseUnit> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicBaseUnit.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = baseUnitDao.findByPage(page, sql);
			}else{
				page = baseUnitDao.findAllByPage(page);
			}
			return page;
		}
		page = baseUnitDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicBaseUnit> findByTreeDataGridUnitGroups(List<X6BasicBaseUnit> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(X6BasicBaseUnit baseUnit : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridUnitGroups(Page<X6BasicBaseUnit> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<X6BasicBaseUnit> baseUnits = dgPage.getResult();
			for(X6BasicBaseUnit baseUnit : baseUnits){
				
			}
			dgPage.setResult(baseUnits);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicBaseUnit loadBaseUnitByBussinessKey(Serializable bussinessKey){
		return baseUnitDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicBaseUnit> findByProperty(String propertyName, Object object){
		return baseUnitDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicBaseUnit findEntityByProperty(String propertyName, Object object){
		return baseUnitDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicBaseUnit.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = baseUnitDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_unitGroup_BaseUnit","runtime");
		try {
	...
	public void dealDatagridsSave(X6BasicBaseUnit baseUnit,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,baseUnitDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicBaseUnit.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, baseUnit);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",baseUnit.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicBaseUnit.TABLE_NAME +" where valid = 1";
			list = baseUnitDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicBaseUnit.TABLE_NAME;
			list = baseUnitDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicBaseUnit.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = baseUnitDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicBaseUnitDaoImpl baseUnitDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBaseUnit(final List<X6BasicBaseUnit> insertObjs, final List<X6BasicBaseUnit> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBaseUnit(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBaseUnit(final List<X6BasicBaseUnit> insertObjs, final List<X6BasicBaseUnit> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1398235403787"),importCount));
		...
		Map<Long, X6BasicBaseUnit> oldUpdateObjsMap = new HashMap<Long, X6BasicBaseUnit>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicBaseUnit bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicBaseUnit> bizObjList = baseUnitDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicBaseUnit bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicBaseUnit> nmeObjs = new ArrayList<X6BasicBaseUnit>();
		...
		long id=autoGeneratorID.getNextId(X6BasicBaseUnit.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicBaseUnit baseUnit:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicBaseUnit.TABLE_NAME,1,dbId);
			if(baseUnit.getId() != null ){
				m.put(baseUnit.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) baseUnitDao.createNativeQuery(sql, "X6Basic_1.0_unitGroup_BaseUnit"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		baseUnitDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicBaseUnit.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicBaseUnit baseUnit = updateObjs.get(updateIndex);
						Long id = baseUnit.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = baseUnit.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicBaseUnit.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicBaseUnit.TABLE_NAME + " order by id desc";
//					Query query = baseUnitDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicBaseUnit baseUnit = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = baseUnit.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		baseUnitDaoImpl.getSession().flush();
		baseUnitDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicBaseUnit bizObj = insertObjs.get(insertIndex);
			baseUnitImportService.saveImportDataLog(bizObj, null, "id", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicBaseUnit bizObj = updateObjs.get(updateIndex);
			X6BasicBaseUnit oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			baseUnitImportService.saveImportDataLog(bizObj, oldBizObj, "id", "name");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<X6BasicBaseUnit> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<X6BasicBaseUnit> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) baseUnitDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<X6BasicBaseUnit> result = getResult(page, query, new PendingResultTransformer(X6BasicBaseUnit.class,baseUnitDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(X6BasicBaseUnit model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = baseUnitDao.createNativeQuery(sql, "X6Basic_1.0_unitGroup_BaseUnit").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = baseUnitDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = baseUnitDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = baseUnitDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?baseUnitDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  baseUnitDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,baseUnitDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="ec.flow.save")
	public void saveBaseUnit(X6BasicBaseUnit baseUnit, DataGridService dataGridService){
		saveBaseUnit(baseUnit, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="ec.flow.save")
	public void saveBaseUnit(X6BasicBaseUnit baseUnit, DataGridService dataGridService, String viewCode){
		saveBaseUnit(baseUnit, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit")
	public void saveBaseUnit(X6BasicBaseUnit baseUnit, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBaseUnit(baseUnit, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit")
	public void saveBaseUnit(X6BasicBaseUnit baseUnit, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_unitGroup";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_unitGroup");
		String url = null;
		if(null != baseUnit.getId() && baseUnit.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicBaseUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit")
	public Map<Object, Object> baseUnitDataGridImport(X6BasicBaseUnit baseUnit, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = baseUnit.getVirtualId();
		
		baseUnit = baseUnitDao.load(baseUnit.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, baseUnit.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = baseUnit.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(baseUnit);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, baseUnit.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit")
	public void mergeBaseUnit(X6BasicBaseUnit baseUnit, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(baseUnit);
		beforeSaveBaseUnit(baseUnit);
		baseUnitDao.merge(baseUnit);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_BaseUnit",desc="foundation.batch.save",operType="4")
	public void batchSaveBaseUnit(X6BasicBaseUnit baseUnit, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "X6Basic_1.0_unitGroup";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_unitGroup");
	...
	@Override
	public Page<X6BasicBaseUnit> getByPage(Page<X6BasicBaseUnit> page,DetachedCriteria detachedCriteria){
		return baseUnitDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBaseUnit(X6BasicBaseUnit baseUnit, Object... objects){
		checkUniqueConstraint(baseUnit);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
		//新增单位时,检测单位编码是否重复 add by yy 2019-05-30
		if (null == baseUnit.getId() && checkUniqueBaseUnitCode(baseUnit)){
			throw new BAPException("单位代码已存在,不能重复!");
		}
/* CUSTOM CODE END */
	}
	
	private void afterSaveBaseUnit(X6BasicBaseUnit baseUnit, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBaseUnit(List<X6BasicBaseUnit> baseUnits, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBaseUnit(List<X6BasicBaseUnit> baseUnits){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBaseUnit(List<X6BasicBaseUnit> baseUnits, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBaseUnit(List<X6BasicBaseUnit> baseUnits){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBaseUnit(X6BasicBaseUnit baseUnit){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
		if(!bapBaseService.canDelete(baseUnit)){
			throw new BAPException("该单位已被其他业务引用，不能删除！");
        }
/* CUSTOM CODE END */
	}
	
	private void afterDeleteBaseUnit(X6BasicBaseUnit baseUnit){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBaseUnit(X6BasicBaseUnit baseUnit){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBaseUnit(X6BasicBaseUnit baseUnit){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicBaseUnit baseUnit, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicBaseUnit baseUnit, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBaseUnit(List<X6BasicBaseUnit> insertObjs, List<X6BasicBaseUnit> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBaseUnit,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBaseUnit(List<X6BasicBaseUnit> insertObjs, List<X6BasicBaseUnit> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBaseUnit,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
	/**
	 * 检测单位的编码是否重复
	 * @param basicBaseUnit
	 ...
	 */
	private boolean checkUniqueBaseUnitCode(X6BasicBaseUnit basicBaseUnit){
		boolean existFlag = false;

		String hql = " from X6BasicBaseUnit where code = ? and valid = true";

		List<X6BasicBaseUnit> baseUnitList = findBaseUnitsByHql(hql, basicBaseUnit.getCode());
