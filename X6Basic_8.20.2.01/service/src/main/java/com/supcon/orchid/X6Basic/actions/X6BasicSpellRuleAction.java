...
/* CUSTOM CODE START(action,import,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicSpellRuleAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicSpellRule> {
	...
	//导出excel时,主模型内容
	List<X6BasicSpellRule> queryResults = new ArrayList<X6BasicSpellRule>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/spellRule/spellRule/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/spellRule/spellRule/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_spellRule_SpellRule" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicSpellRule> spellRules;
	
	public List<X6BasicSpellRule> getSpellRules() {
		return spellRules;
	}
	...
	private Page<X6BasicSpellRule> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			spellRule = new X6BasicSpellRule();
		} else {
			if(id > 0){
				spellRule = spellRuleService.getSpellRule(id, viewCode);
			}
		}
	...
	public Page<X6BasicSpellRule> getPage() {
		if (null == page) {
			page = new Page<X6BasicSpellRule>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/spellRule/spellRule/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = spellRuleService.getSpellRuleAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/spellRule/spellRule/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1398306647942
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String spellRuleReferenceCopy() throws Exception{
			X6BasicSpellRule oldSpellRule = null;
			if(isEditNew()){
				...
				oldSpellRule = new X6BasicSpellRule();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,spellRule.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, spellRule.getId());
				...
				oldSpellRule = spellRuleService.getSpellRule(id);
			}
			
			
			spellRule = spellRuleService.getSpellRule(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_spellRule_SpellRule");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicSpellRule.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldSpellRule, null);
						Method setValue = X6BasicSpellRule.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(spellRule, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			spellRule.setCreateDepartmentId(oldSpellRule.getCreateDepartmentId());
			spellRule.setCreatePositionId(oldSpellRule.getCreatePositionId());
			spellRule.setCreateStaffId(oldSpellRule.getCreateStaffId());
			spellRule.setOwnerDepartmentId(oldSpellRule.getOwnerDepartmentId());
			spellRule.setOwnerPositionId(oldSpellRule.getOwnerPositionId());
			spellRule.setOwnerStaffId(oldSpellRule.getOwnerStaffId());
			spellRule.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRule", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/spellRule/spellRule/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/spellRule/spellRule/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=spellRuleService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return spellRuleReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			spellRule = spellRuleService.getSpellRule(this.spellRule.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,spellRule.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/spellRule/spellRule/ruleView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "ruleView.ftl") }),
		@Action(value = "/X6Basic/spellRule/spellRule/ruleViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "ruleViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/spellRule/spellRule/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = spellRuleService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(spellRuleService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(spellRuleService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_spellRule_SpellRule", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/spellRule/spellRule/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/spellRule/spellRule/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.times,result.srMemo,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"spellRule\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"spellRule\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"spellRule\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"spellRule\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/spellRule/spellRule/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/spellRule/spellRule/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.times,result.srMemo,result.attrMap.*"})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/spellRule/spellRule/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_spellRule_SpellRule");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		spellRuleService.findDataGridPage(dg,dgclass,getDgPage(),spellRule,"", null);
	...
	private Page<X6BasicSpellRulePart> dg1398316931895Page;
	public Page<X6BasicSpellRulePart> getDg1398316931895Page(){
		if(null == dg1398316931895Page){
			dg1398316931895Page = new Page<X6BasicSpellRulePart>(1,20);
		}
		return dg1398316931895Page;
	}
	
	private Class dg1398316931895Clazz = X6BasicSpellRulePart.class;
	
	public Class getDg1398316931895Clazz() {
		return dg1398316931895Clazz;
	}
	
	private List<X6BasicSpellRulePart> dg1398316931895List;
	public List<X6BasicSpellRulePart> getDg1398316931895List(){
		if(null == dg1398316931895List){
			dg1398316931895List = new ArrayList<X6BasicSpellRulePart>();
		}
		return dg1398316931895List;
	}
	public void setDg1398316931895ListJson(String json) {
		//dg1398316931895List = (List<X6BasicSpellRulePart>) spellRuleService.generateObjectFromJson(json, X6BasicSpellRulePart.class);
	}
	
	private List<Long> dg1398316931895DeletedIds;
	public List<Long> getDg1398316931895DeletedIds(){
		if(null == dg1398316931895DeletedIds){
			dg1398316931895DeletedIds = new ArrayList<Long>();
		}
		return dg1398316931895DeletedIds;
	}

	private Page<X6BasicSpellRulePart> dg1398317025305Page;
	public Page<X6BasicSpellRulePart> getDg1398317025305Page(){
		if(null == dg1398317025305Page){
			dg1398317025305Page = new Page<X6BasicSpellRulePart>(1,20);
		}
		return dg1398317025305Page;
	}
	
	private Class dg1398317025305Clazz = X6BasicSpellRulePart.class;
	
	public Class getDg1398317025305Clazz() {
		return dg1398317025305Clazz;
	}
	
	private List<X6BasicSpellRulePart> dg1398317025305List;
	public List<X6BasicSpellRulePart> getDg1398317025305List(){
		if(null == dg1398317025305List){
			dg1398317025305List = new ArrayList<X6BasicSpellRulePart>();
		}
		return dg1398317025305List;
	}
	public void setDg1398317025305ListJson(String json) {
		//dg1398317025305List = (List<X6BasicSpellRulePart>) spellRuleService.generateObjectFromJson(json, X6BasicSpellRulePart.class);
	}
	
	private List<Long> dg1398317025305DeletedIds;
	public List<Long> getDg1398317025305DeletedIds(){
		if(null == dg1398317025305DeletedIds){
			dg1398317025305DeletedIds = new ArrayList<Long>();
		}
		return dg1398317025305DeletedIds;
	}

	@Actions({
		
		@Action(value = "/X6Basic/spellRule/spellRule/data-dg1398316931895", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.teamOrder,result.startTime,result.endTime,result.srpMemo,result.id,result.version,result.spellRule.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1398316931895-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1398316931895.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/X6Basic/spellRule/spellRule/data-dg1398317025305", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.teamOrder,result.startTime,result.endTime,result.srpMemo,result.id,result.version,result.spellRule.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1398317025305-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1398317025305.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(spellRule.getId() != null && spellRule.getId() != -1){
				spellRule = spellRuleService.getSpellRule(spellRule.getId());
			}
			spellRuleService.findDataGridPage(dg,dgclass,getDgPage(),spellRule,searchCondition,params);
	...
	private void save() throws Exception {
		...
		spellRuleService.saveSpellRule(spellRule,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/spellRule/spellRule/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/spellRule/spellRule/ruleView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "submit");
		bap_validate_datagrids=spellRuleService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", spellRule.getId());
		...
		fileuploads.put("linkId", spellRule.getId());
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRule", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/spellRule/spellRule/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		spellRuleService.deleteSpellRule(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRule", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/spellRule/spellRule/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "restore");
		spellRuleService.restoreSpellRule(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRule", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/spellRule/spellRule/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/spellRule/spellRule/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "showDealInfoData");
		if (null != tableInfoId) {
			int count=spellRuleService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = spellRuleService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = spellRuleService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRule", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicSpellRule> findExportDataInfos(Page<X6BasicSpellRule> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = spellRuleService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = spellRuleService.getAssProperty(p.getCode());	
									//String assModelCode = spellRuleService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			spellRuleService.findSpellRules(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			spellRuleService.findSpellRules(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicSpellRule> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicSpellRule result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".SR_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.spellRule || null == this.spellRule.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/spellRule/spellRule/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRule", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/spellRule/spellRule/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/spellRule/spellRule/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/spellRule/spellRule/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicSpellRuleService"
						, "com.supcon.orchid.X6Basic.services.X6BasicSpellRulePartService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_spellRule", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_spellRule");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRule", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/spellRule/spellRule/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_spellRule_SpellRule", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_spellRule_SpellRule", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/spellRule/spellRule/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/spellRule/spellRule/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_spellRule_SpellRule";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRule", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/spellRule/spellRule/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == spellRule.getCode() ? "" : spellRule.getCode().toString());
		_auditLog.setColumnStringA(null == spellRule.getName() ? "" : spellRule.getName().toString());
		_auditLog.setColumnStringC(spellRule.getId().toString());
		_auditLog.setColumnStringD(X6BasicSpellRule.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_spellRule");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicSpellRuleAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, spellRule,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, spellRule,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,spellRule);
		...
		printlog.setEntity("X6Basic_1.0_spellRule");
		...
		printlog.setBusinessId(spellRule.getId());
		printlog.setBussinessKey(null == spellRule.getCode() ? "" : spellRule.getCode().toString());
		printlog.setMainDisplay(null == spellRule.getName() ? "" : spellRule.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/spellRule/spellRule/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(spellRule,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/spellRule/spellRule/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(spellRuleService.getSpellRule(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/spellRule/spellRule/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/spellRule/spellRule/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, spellRule,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				spellRule = spellRuleService.getSpellRule(Long.parseLong(arrId[i]));
				if(null != spellRule){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == spellRule.getName() ? "" : "," + spellRule.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == spellRule.getName() ? "" : spellRule.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == spellRule.getCode() ? "" : "," + spellRule.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == spellRule.getCode() ? "" : spellRule.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == spellRule.getId() ? "" : "," + spellRule.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == spellRule.getId() ? "" : spellRule.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, spellRule,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,spellRule);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/spellRule/spellRule/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicSpellRule> findExportPageInfos(Page<X6BasicSpellRule> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_spellRule" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = spellRuleService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicSpellRule queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = spellRuleService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = spellRuleService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_spellRule_SpellRule");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_spellRule_SpellRule")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_spellRule_SpellRule");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_spellRule_SpellRule");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = spellRuleService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
