...
import com.supcon.orchid.services.BAPSearchService;
...
import com.supcon.orchid.X6Basic.entities.MeetingRoomMneCode;
import com.supcon.orchid.X6Basic.services.MeetingRoomService;
import com.supcon.orchid.X6Basic.services.MeetingRoomImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.MeetingRoomDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
import java.util.Date;
import com.supcon.orchid.id.IdGenerator;
/* CUSTOM CODE END */

@Service("x6Basic_meetingRoomService")
@Transactional
public class MeetingRoomServiceImpl  extends BaseServiceImpl<MeetingRoom> implements MeetingRoomService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MeetingRoom> bapBaseService;
	...
	@Autowired
	private MeetingRoomImportService meetingRoomImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MeetingRoom.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  meetingRoomDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MeetingRoom.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = meetingRoomDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MeetingRoom.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MeetingRoom.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  meetingRoomDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MeetingRoom.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MeetingRoom.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MeetingRoom> list = (List<MeetingRoom>) meetingRoomDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MeetingRoom getMeetingRoom(long id){
		return getMeetingRoom(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MeetingRoom getMeetingRoom(long id, String viewCode){
		MeetingRoom meetingRoom = meetingRoomDao.load(id);

		// 一对多情况处理
		if(meetingRoom != null){
			this.setMeetingStaff(meetingRoom, viewCode);
		}
		return meetingRoom;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMeetingRoomAsJSON(long id, String include){
		MeetingRoom meetingRoom = meetingRoomDao.load(id);
		// 一对多情况处理
			this.setMeetingStaff(meetingRoom);

		if(meetingRoom == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(meetingRoom, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.common.delete",operType="3")
	public void deleteMeetingRoom(MeetingRoom meetingRoom){
		if(null != meetingRoom && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MeetingRoom.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
			AuditUtil.setColumnStringA(null == meetingRoom.getName() ? "" : meetingRoom.getName().toString());
			AuditUtil.setColumnStringC(meetingRoom.getId().toString());
		}
		beforeDeleteMeetingRoom(meetingRoom);
		meetingRoomDao.delete(meetingRoom);
		afterDeleteMeetingRoom(meetingRoom);
		meetingRoomDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/meetingRoom/del", meetingRoom,MeetingRoomMneCode.class);
		List<MeetingRoom> params = new ArrayList<MeetingRoom>();
		List<UsedRecord> usedRecordList = usedRecordDao.findByCriteria(Restrictions.eq("meeting", meetingRoom));
		meetingRoom.setUsedRecordList(usedRecordList);
		params.add(meetingRoom);
		...
		String delIds = "";
		// 一对多情况处理
		delIds = meetingRoom.getMeetingStaffmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			...
			if (!idList.isEmpty()) {
				List<UsedRecord> tmpList = usedRecordDao.findByCriteria(Restrictions.eq("meeting", meetingRoom), Restrictions.in("staff.id", idList), Restrictions.eq("valid", true));
				for (UsedRecord item : tmpList) {
					usedRecordDao.deletePhysical(item);
				}
			}
		}
			// TODO delete
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.common.delete",operType="3")
	public void deleteMeetingRoom(List<Long> meetingRoomIds){
		deleteMeetingRoom(meetingRoomIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.common.delete",operType="3")
	public void deleteMeetingRoom(List<Long> meetingRoomIds, String eventTopic) {
		List<MeetingRoom> meetingRooms = new ArrayList<MeetingRoom>();
		for(Long meetingRoomId : meetingRoomIds){
			MeetingRoom meetingRoom = getMeetingRoom(meetingRoomId);
			meetingRooms.add(meetingRoom);
			if(null != meetingRoom && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MeetingRoom.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == meetingRoom.getName() ? "" : "," + meetingRoom.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == meetingRoom.getName() ? "" : meetingRoom.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == meetingRoom.getId() ? "" : "," + meetingRoom.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == meetingRoom.getId() ? "" : "," + meetingRoom.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
				}
			}
		}
		...
		if(meetingRooms != null && meetingRooms.size() > 0 ){
			for(MeetingRoom meetingRoom : meetingRooms){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),meetingRoom, meetingRoom.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(meetingRoomIds != null && meetingRoomIds.size() > 0) {
			String hql = "update " + MeetingRoom.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = meetingRoomDao.createQuery(hql);
			query.setParameterList("ids", meetingRoomIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MeetingRoom meetingRoomz : meetingRooms){
				meetingRoomz.setValid(false);
				List<MeetingRoom> params = new ArrayList<MeetingRoom>();
				List<UsedRecord> usedRecordList = usedRecordDao.findByCriteria(Restrictions.eq("meeting", meetingRoomz));
				meetingRoomz.setUsedRecordList(usedRecordList);
				params.add(meetingRoomz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_meetingroom");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.common.delete",operType="3")
	public void deleteMeetingRoom(long meetingRoomId, int meetingRoomVersion){
		this.deleteMeetingRoom(meetingRoomId, meetingRoomVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.common.delete",operType="3")
	public void deleteMeetingRoom(long meetingRoomId, int meetingRoomVersion,SignatureLog signatureLog){
		MeetingRoom meetingRoom = getMeetingRoom(meetingRoomId);
		...
		beforeDeleteMeetingRoom(meetingRoom);
		meetingRoomDao.delete(meetingRoomId, meetingRoomVersion);
		afterDeleteMeetingRoom(meetingRoom);
		meetingRoomDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/meetingRoom/del", meetingRoom,MeetingRoomMneCode.class);
		List<MeetingRoom> params = new ArrayList<MeetingRoom>();
		...
		if(signatureLog != null ) {
			...
			businessKey=meetingRoom.getId();
			signatureLog.setTableId(meetingRoom.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.common.delete",operType="3")
	public void deleteMeetingRoom(String meetingRoomIds){
		this.deleteMeetingRoom(meetingRoomIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.common.delete",operType="3")
	public void deleteMeetingRoom(String meetingRoomIds,SignatureLog signatureLog){
		deleteCollection(meetingRoomIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMeetingRoom(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_meetingroom#modelCode:X6Basic_1.0_meetingroom_MeetingRoom#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="ec.engine.restore",operType="10")
	public void restoreMeetingRoom(String meetingRoomIds){
		restoreCollection(meetingRoomIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreMeetingRoom(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="ec.engine.restore",operType="10")
	public void restoreMeetingRoom(long meetingRoomId){
		
		findBusinessKeyUsed(meetingRoomId);	//判断业务主键是否重复
		...
		beforeRestoreMeetingRoom(meetingRoom);
		meetingRoom.setValid(true);
		meetingRoomDao.update(meetingRoom);
		afterRestoreMeetingRoom(meetingRoom);
		meetingRoomDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/meetingRoom/add", meetingRoom,MeetingRoomMneCode.class);
		List<MeetingRoom> params = new ArrayList<MeetingRoom>();
		List<UsedRecord> usedRecordList = usedRecordDao.findByCriteria(Restrictions.eq("meeting", meetingRoom));
		meetingRoom.setUsedRecordList(usedRecordList);
		params.add(meetingRoom);
	...
	public void findBusinessKeyUsed(long meetingRoomId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_meetingroom_MeetingRoom");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MeetingRoom.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MeetingRoom.TABLE_NAME +" where id = ? )";
			List<Object> list =  meetingRoomDao.createNativeQuery(sql,meetingRoomId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseMeetingRoom(List<MeetingRoom>  meetingRooms){
		for(MeetingRoom meetingRoom:meetingRooms)  {
			saveMeetingRoom(meetingRoom, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMeetingRoom(List<MeetingRoom>  meetingRooms){
		for(MeetingRoom meetingRoom:meetingRooms)  {
			saveMeetingRoom(meetingRoom, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="ec.flow.save")
	public void saveMeetingRoom(MeetingRoom meetingRoom, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMeetingRoom(meetingRoom, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="ec.flow.save")
	public void saveMeetingRoom(MeetingRoom meetingRoom, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMeetingRoom(meetingRoom, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateMeetingRoomCodes(MeetingRoom meetingRoom) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMeetingRoomCodes(meetingRoom, false);
	}
	
	public void generateMeetingRoomCodes(MeetingRoom meetingRoom, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateMeetingRoomSummarys(MeetingRoom meetingRoom) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMeetingRoomSummarys(meetingRoom, false);
	}

	public void generateMeetingRoomSummarys(MeetingRoom meetingRoom, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom")
	public void saveMeetingRoom(MeetingRoom meetingRoom,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveMeetingRoom(meetingRoom, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom")
	public void saveMeetingRoom(MeetingRoom meetingRoom,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "X6Basic_1.0_meetingroom");
		String url = null;
		if(meetingRoom.getId() != null && meetingRoom.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MeetingRoom.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, meetingRoom);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(meetingRoom);
		beforeSaveMeetingRoom(meetingRoom, viewIsView);

		if (viewIsView) {
			meetingRoomDao.saveWithRevertVersion(meetingRoom);
		} else {
			if(isNew)
				meetingRoomDao.save(meetingRoom);
			else
				meetingRoomDao.merge(meetingRoom);
		}
		if(null != meetingRoom && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MeetingRoom.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
			AuditUtil.setColumnStringA(null == meetingRoom.getName() ? "" : meetingRoom.getName().toString());
		...
		dealDatagridsSave(meetingRoom,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
			this.dealMeetingStaff(meetingRoom);
		
		// 根据配置规则生成编码
		try {
			generateMeetingRoomCodes(meetingRoom, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMeetingRoomSummarys(meetingRoom, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMeetingRoom(meetingRoom, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, meetingRoom);
			}
		}
		List<MeetingRoom> params = new ArrayList<MeetingRoom>();
		meetingRoomDao.flush();
		meetingRoomDao.clear();
		meetingRoom = meetingRoomDao.load(meetingRoom.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MeetingRoom meetingRoom){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("meetingRoom", meetingRoom);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom")
	public void mergeMeetingRoom(MeetingRoom meetingRoom, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(meetingRoom);
		beforeSaveMeetingRoom(meetingRoom);
		meetingRoomDao.merge(meetingRoom);
		if(null != meetingRoom && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MeetingRoom.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
			AuditUtil.setColumnStringA(null == meetingRoom.getName() ? "" : meetingRoom.getName().toString());
		...
		dealDatagridsSave(meetingRoom,null,dgLists,dgDeleteIDs,assFileUploads);

			// 一对多情况处理
			this.dealMeetingStaff(meetingRoom);
		afterSaveMeetingRoom(meetingRoom);
		meetingRoomDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/meetingRoom/add", meetingRoom,MeetingRoomMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MeetingRoom> findMeetingRooms(Page<MeetingRoom> page, Criterion... criterions) {
		return meetingRoomDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MeetingRoomDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MeetingRoomDealInfo> list=meetingRoomDao.createQuery("from " + MeetingRoomDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MeetingRoomDealInfo dealInfo) {
		meetingRoomDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MeetingRoomDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) meetingRoomDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MeetingRoomDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = meetingRoomDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = meetingRoomDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MeetingRoomDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = meetingRoomDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMeetingRooms(Page<MeetingRoom> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMeetingRooms(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMeetingRooms(Page<MeetingRoom> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMeetingRooms(Page<MeetingRoom> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MeetingRoom> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"meetingRoom\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"meetingRoom\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"meetingRoom\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_meetingroom_MeetingRoom" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_meetingroom_MeetingRoom" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  meetingRoomDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  meetingRoomDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_meetingroom").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"meetingRoom\"","X6Basic_1.0_meetingroom",customCondition,"X6BASIC_MEETING_ROOMS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"meetingRoom\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MeetingRoomDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"meetingRoom\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"meetingRoom\".LAY_REC ASC, \"meetingRoom\".SORT ASC");
				} else {
					orderPart.append(" \"meetingRoom\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) meetingRoomDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=meetingRoomDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = meetingRoomDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				...
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MeetingRoom.class,meetingRoomDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MeetingRoom.class,meetingRoomDao,maps));
				}
				...
				if(null != hasAttachment && hasAttachment) {
					for (MeetingRoom meetingRoom : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(meetingRoom.getId(), "X6Basic_meetingroom_meetingRoom");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(meetingRoom.getId(), "X6Basic_meetingroom_meetingRoom");
							if(null != documents && !documents.isEmpty()) {
								meetingRoom.setDocument(documents.get(0));
								meetingRoom.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MeetingRoom> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MeetingRoom> result) {
		View view = null;
		...
		Map<Long, MeetingRoom> rsMap = new HashMap<Long, MeetingRoom>();
		for (MeetingRoom meetingRoom : result) {
			ids.add(meetingRoom.getId());
			rsMap.put(meetingRoom.getId(), meetingRoom);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(meetingRoomDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = meetingRoomDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MeetingRoom meetingRoom,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(meetingRoom.getOwnerStaffId());
		ti.setOwnerPositionId(meetingRoom.getOwnerPositionId());
		ti.setOwnerDepartmentId(meetingRoom.getOwnerDepartmentId());
		
		ti.setCreateStaff(meetingRoom.getCreateStaff());
		ti.setCreateStaffId(meetingRoom.getCreateStaffId());
		ti.setCreateTime(meetingRoom.getCreateTime());
		ti.setDeleteStaff(meetingRoom.getDeleteStaff());
		ti.setDeleteStaffId(meetingRoom.getDeleteStaffId());
		ti.setDeleteTime(meetingRoom.getDeleteTime());
		ti.setCreateDepartmentId(meetingRoom.getCreateDepartmentId());
		ti.setModifyStaff(meetingRoom.getModifyStaff());
		ti.setModifyStaffId(meetingRoom.getModifyStaffId());
		ti.setCreatePositionId(meetingRoom.getCreatePositionId());
		ti.setPositionLayRec(meetingRoom.getPositionLayRec());
		ti.setTableNo(meetingRoom.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_meetingroom");
		ti.setTargetTableName("X6BASIC_MEETING_ROOMS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_meetingroom_MeetingRoom" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  meetingRoomDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"meetingRoom\"","X6Basic_1.0_meetingroom","","X6BASIC_MEETING_ROOMS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"meetingRoom\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"meetingRoom\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"meetingRoom\"","X6Basic_1.0_meetingroom","","X6BASIC_MEETING_ROOMS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("meetingRoom.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"meetingRoom\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"meetingRoom\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"meetingRoom\".CID = 1 OR \"meetingRoom\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("meetingRoom.mc\"").append(".MEETING_ROOM) not in ("+notinValue+")");
			}
			
			SQLQuery query = meetingRoomDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=meetingRoomDao.createNativeQuery(sql, menuCode);
	...
	// ================ 生成多选控件数据保存方法 start====================
	private void dealMeetingStaff(MeetingRoom meetingRoom) {
		String addIds = meetingRoom.getMeetingStaffAddIds();
		String delIds = meetingRoom.getMeetingStaffDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					UsedRecord tmp = new UsedRecord();
					tmp.setMeeting(meetingRoom);
					Staff indirectObj = new Staff();
					indirectObj.setId(Long.valueOf(item));
					tmp.setStaff(indirectObj);
					tmp.setCid(meetingRoom.getCid());
					usedRecordDao.save(tmp);
				}
			}
		}
	}

	private void setMeetingStaff(MeetingRoom meetingRoom) {
		setMeetingStaff(meetingRoom, null);
	}

	private void setMeetingStaff(MeetingRoom meetingRoom, String viewCode) {
		List<UsedRecord> tmpList = usedRecordDao.findByCriteria(Order.asc("id"), Restrictions.eq("meeting", meetingRoom), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (UsedRecord item : tmpList) {
			if(item.getStaff() != null) {
				...
				names += ReflectUtils.getPropertyValue(item.getStaff(), displayField);
			}
		}
		meetingRoom.setUsedRecordList(tmpList);
		meetingRoom.setMeetingStaffmultiselectIDs(ids);
		meetingRoom.setMeetingStaffmultiselectNames(names);
	}
	// ================ 生成多选控件数据保存方法 end ====================

	protected String getEntityCode(){return MeetingRoom.ENTITY_CODE;}
	protected String getModuleCode(){return MeetingRoom.MODULE_CODE;}
	protected String getModelCode(){return MeetingRoom.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param meetingRoom 模型实例
	 */
	public void checkUniqueConstraint(MeetingRoom meetingRoom){
	}
	 ...
	 */
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private void dealDocInfoForSearch(MeetingRoom meetingRoom) throws Exception {
		Long linkId = meetingRoom.getId();
		if(null != linkId) {
			String docType = MeetingRoom.DOC_TYPE;
			List<Document> docList = documentService.getAllByLinkIdAndType(linkId, docType);
			if(null != docList && !docList.isEmpty()) {
				List<String> docIdList = new ArrayList<String>();
				List<String> docNameList = new ArrayList<String>();
				List<String> docContentList = new ArrayList<String>();
				for(Document doc : docList) {
					Long docId = doc.getId();
					String docName = doc.getName();
					String docContent = bapSearchService.getTheDocumentInfo(doc.getPath());
					docIdList.add(docId.toString());
					docNameList.add(docName);
					docContentList.add(docContent);
				}
				meetingRoom.setBapSearchDocId(docIdList);
				meetingRoom.setBapSearchDocName(docNameList);
				meetingRoom.setBapSearchDocContent(docContentList);
			}
		}	
	}
	
	@Override
	public List<MeetingRoom> getMeetingRooms(String sql, List<Object> params) {
		...
		if(params!=null){
			...
			return meetingRoomDao.findByHql("from " + MeetingRoom.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return meetingRoomDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MeetingRoom> findMeetingRoomsByHql(String hql, Object... objects){
		...
		return   meetingRoomDao.findByHql(hql, objects);
	}
	
	@Override
	public MeetingRoom loadMeetingRoomByBussinessKey(MeetingRoom meetingRoom){
		return loadMeetingRoomByBussinessKey(meetingRoom.getId());
	}
	
	@Override
	public Page<MeetingRoom> getMeetingRooms(Page<MeetingRoom> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MeetingRoom.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = meetingRoomDao.findByPage(page, sql);
			}else{
				page = meetingRoomDao.findAllByPage(page);
			}
			return page;
		}
		page = meetingRoomDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public MeetingRoom loadMeetingRoomByBussinessKey(Serializable bussinessKey){
		return meetingRoomDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MeetingRoom> findByProperty(String propertyName, Object object){
		return meetingRoomDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MeetingRoom findEntityByProperty(String propertyName, Object object){
		return meetingRoomDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MeetingRoom.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = meetingRoomDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_meetingroom_MeetingRoom","runtime");
		try {
	...
	public void dealDatagridsSave(MeetingRoom meetingRoom,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,meetingRoomDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MeetingRoom.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, meetingRoom);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",meetingRoom.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MeetingRoom.TABLE_NAME +" where valid = 1";
			list = meetingRoomDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MeetingRoom.TABLE_NAME;
			list = meetingRoomDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MeetingRoom.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = meetingRoomDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MeetingRoomDaoImpl meetingRoomDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchMeetingRoom(final List<MeetingRoom> insertObjs, final List<MeetingRoom> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchMeetingRoom(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchMeetingRoom(final List<MeetingRoom> insertObjs, final List<MeetingRoom> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1366792543468"),importCount));
		...
		Map<Long, MeetingRoom> oldUpdateObjsMap = new HashMap<Long, MeetingRoom>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MeetingRoom bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MeetingRoom> bizObjList = meetingRoomDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MeetingRoom bizObj = bizObjList.get(mIndex);
		...
		final List<MeetingRoom> nmeObjs = new ArrayList<MeetingRoom>();
		...
		long id=autoGeneratorID.getNextId(MeetingRoom.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MeetingRoom meetingRoom:insertObjs)  {
			//id = autoGeneratorID.getNextId(MeetingRoom.TABLE_NAME,1,dbId);
			if(meetingRoom.getId() != null ){
				m.put(meetingRoom.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) meetingRoomDao.createNativeQuery(sql, "X6Basic_1.0_meetingroom_MeetingRoom"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		meetingRoomDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MeetingRoom.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MeetingRoom meetingRoom = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = meetingRoom.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MeetingRoom.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MeetingRoom.TABLE_NAME + " order by id desc";
//					Query query = meetingRoomDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MeetingRoom meetingRoom = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = meetingRoom.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MeetingRoomMneCode.class;
					...
					for(MeetingRoom meetingRoom:updateObjs)  {
						Long id = meetingRoom.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MeetingRoom meetingRoom:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(meetingRoom);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, meetingRoom.getId());
		...
		meetingRoomDaoImpl.getSession().flush();
		meetingRoomDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MeetingRoom bizObj = insertObjs.get(insertIndex);
			meetingRoomImportService.saveImportDataLog(bizObj, null, "id", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MeetingRoom bizObj = updateObjs.get(updateIndex);
			MeetingRoom oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			meetingRoomImportService.saveImportDataLog(bizObj, oldBizObj, "id", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = meetingRoomDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				meetingRoomDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = meetingRoomDao.createNativeQuery(sql, "X6Basic_1.0_meetingroom_MeetingRoom").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = meetingRoomDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = meetingRoomDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = meetingRoomDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?meetingRoomDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  meetingRoomDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,meetingRoomDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="ec.flow.save")
	public void saveMeetingRoom(MeetingRoom meetingRoom, DataGridService dataGridService){
		saveMeetingRoom(meetingRoom, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="ec.flow.save")
	public void saveMeetingRoom(MeetingRoom meetingRoom, DataGridService dataGridService, String viewCode){
		saveMeetingRoom(meetingRoom, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom")
	public void saveMeetingRoom(MeetingRoom meetingRoom, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveMeetingRoom(meetingRoom, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom")
	public void saveMeetingRoom(MeetingRoom meetingRoom, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_meetingroom";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_meetingroom");
		String url = null;
		if(null != meetingRoom.getId() && meetingRoom.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MeetingRoom.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom")
	public Map<Object, Object> meetingRoomDataGridImport(MeetingRoom meetingRoom, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = meetingRoom.getVirtualId();
		
		meetingRoom = meetingRoomDao.load(meetingRoom.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, meetingRoom.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = meetingRoom.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(meetingRoom);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, meetingRoom.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom")
	public void mergeMeetingRoom(MeetingRoom meetingRoom, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(meetingRoom);
		beforeSaveMeetingRoom(meetingRoom);
		meetingRoomDao.merge(meetingRoom);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_meetingroom",model="X6Basic_1.0_meetingroom_MeetingRoom",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MeetingRoom> getByPage(Page<MeetingRoom> page,DetachedCriteria detachedCriteria){
		return meetingRoomDao.findByPage(page, detachedCriteria);
	}
	@Override
	public List<UsedRecord> getUsedRecordList(MeetingRoom meetingRoom){
		return usedRecordDao.findByCriteria(Restrictions.eq("meeting", meetingRoom),Restrictions.eq("valid", true));
	}
	...
	private void beforeSaveMeetingRoom(MeetingRoom meetingRoom, Object... objects){
		checkUniqueConstraint(meetingRoom);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveMeetingRoom(MeetingRoom meetingRoom, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMeetingRoom(MeetingRoom meetingRoom){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteMeetingRoom(MeetingRoom meetingRoom){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreMeetingRoom(MeetingRoom meetingRoom){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreMeetingRoom(MeetingRoom meetingRoom){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MeetingRoom meetingRoom, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MeetingRoom meetingRoom, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportMeetingRoom(List<MeetingRoom> insertObjs, List<MeetingRoom> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMeetingRoom,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportMeetingRoom(List<MeetingRoom> insertObjs, List<MeetingRoom> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMeetingRoom,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
	...
	public boolean checkAdjustRoomUsed(Long applyId,Long adjustId,Long meetingRoomId,String entityCode,String adjEntityCode,Date startTime,Date endTime){
		String sql="select count(id) count from X6BASIC_USED_RECORDS where MEETING=? and ((START_TIME>? and START_TIME<?) or (END_TIME>? and END_TIME<?) or (START_TIME<? and END_TIME>?))";
		Long count=0L;
		String checkSql="select ID from X6BASIC_USED_RECORDS where (USE_TYPE=? and TYPE_ID=?) or (USE_TYPE=? and TYPE_ID=?)";
		List<Object> list=meetingRoomDao.createNativeQuery(checkSql,entityCode,applyId,adjEntityCode,adjustId).list();
		if(list!=null&&!list.isEmpty()){
			for(Object obj:list){
				sql+=" and ID!="+obj.toString();
			}
			
		}
		List<Object> checkList=meetingRoomDao.createNativeQuery(sql,meetingRoomId,startTime,endTime,startTime,endTime,startTime,endTime).list();
		if(checkList!=null&&!checkList.isEmpty()){
			count=((Number)checkList.get(0)).longValue();
		}
		if(count>0){
			return false;
		}
	...
	public boolean checkRoomUsed(Long typeId,Long meetingRoomId,String entityCode,Date startTime,Date endTime){
		...
		Long count=0L;
		if(typeId!=null){
			String checkSql="select ID from X6BASIC_USED_RECORDS where USE_TYPE=? and TYPE_ID=?";
			List<Object> list=meetingRoomDao.createNativeQuery(checkSql,entityCode,typeId).list();
			if(list!=null&&!list.isEmpty()){
				sql+=" and ID!="+list.get(0).toString();
			}
		}
		List<Object> list=meetingRoomDao.createNativeQuery(sql,meetingRoomId,startTime,endTime,startTime,endTime,startTime,endTime).list();
		if(list!=null&&!list.isEmpty()){
	...
	public void deleteRoomUseRecord(Long typeId ,String entityCode){
		String deleteSql="delete from X6BASIC_USED_RECORDS where USE_TYPE=? and TYPE_ID=?";
		meetingRoomDao.createNativeQuery(deleteSql,entityCode,typeId).executeUpdate();
	...
	public void updateRoomUseRecode(Long typeId,Long meetingRoomId ,String entityCode,Date startTime,Date endTime,String title){
		Long count=0L;
		String checkSql="select count(id) count from X6BASIC_USED_RECORDS where USE_TYPE=? and TYPE_ID=?";
		List<Object> list=meetingRoomDao.createNativeQuery(checkSql,entityCode, typeId).list();
		if(list!=null&&!list.isEmpty()){
			count=((Number)list.get(0)).longValue();
		}
		if(count>0){
			String updateSql="update X6BASIC_USED_RECORDS set START_TIME=?,END_TIME=?,MEETING=?,TITLE=? where USE_TYPE=? and TYPE_ID=?";
			meetingRoomDao.createNativeQuery(updateSql, startTime,endTime,meetingRoomId,title,entityCode, typeId).executeUpdate();
		}else{
			User user=(User)getCurrentUser();
			Staff staff=user.getStaff();
			
			Long id=idGenerator.getNextId("X6BASIC_USED_RECORDS");
			String sql="insert into X6BASIC_USED_RECORDS(id,VERSION,VALID,START_TIME,END_TIME,MEETING,USE_TYPE,TYPE_ID,STAFF,CID,TITLE) values(?,?,?,?,?,?,?,?,?,?,?)";
			meetingRoomDao.createNativeQuery(sql, id,1,1,startTime,endTime,meetingRoomId,entityCode,typeId,staff.getId(),getCurrentCompanyId(),title).executeUpdate();
			
		}
		
	}
	@Override
	public Page<MeetingRoom> findPageByHql(Page<MeetingRoom> page, String hql,
			Object...param) {
		return meetingRoomDao.findByPage(page, hql, param);
	}

	@Override
	public List<MeetingRoom> getUnuseRoomBetweenTime(Date startTime,
			Date endTime) {
		String hql="from MeetingRoom mr where mr.valid=true and mr.useFlag=true and mr.id not in (select ud.meeting.id from UsedRecord ud where (ud.startTime >? and ud.startTime<?) or (ud.endTime>? and ud.endTime<?) or (ud.startTime<? and ud.endTime>?))";
		return meetingRoomDao.findByHql(hql, startTime,endTime,startTime,endTime,startTime,endTime);
	}
	@Override
	public List<MeetingRoom> getAllMeetingRoom() {
		String hql=" from MeetingRoom mr where mr.valid=true and mr.useFlag=true order by mr.id ASC";
		return meetingRoomDao.findByHql(hql);
	}
/* CUSTOM CODE END */
}
