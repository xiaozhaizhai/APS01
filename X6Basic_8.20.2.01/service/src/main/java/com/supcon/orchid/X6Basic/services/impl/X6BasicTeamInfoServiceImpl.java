...
import com.supcon.orchid.X6Basic.services.X6BasicTeamInfoImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicTeamInfoDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_teamInfoService")
@Transactional
public class X6BasicTeamInfoServiceImpl  extends BaseServiceImpl<X6BasicTeamInfo> implements X6BasicTeamInfoService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicTeamInfo> bapBaseService;
	...
	@Autowired
	private X6BasicTeamInfoImportService teamInfoImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicTeamInfo.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  teamInfoDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicTeamInfo.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = teamInfoDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicTeamInfo.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicTeamInfo.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  teamInfoDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicTeamInfo.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicTeamInfo.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicTeamInfo> list = (List<X6BasicTeamInfo>) teamInfoDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicTeamInfo getTeamInfo(long id){
		return getTeamInfo(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicTeamInfo getTeamInfo(long id, String viewCode){
		X6BasicTeamInfo teamInfo = teamInfoDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getTeamInfoAsJSON(long id, String include){
		X6BasicTeamInfo teamInfo = teamInfoDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(teamInfo, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.common.delete",operType="3")
	public void deleteTeamInfo(X6BasicTeamInfo teamInfo){
		if(null != teamInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTeamInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == teamInfo.getCode() ? "" : teamInfo.getCode().toString());
			AuditUtil.setColumnStringA(null == teamInfo.getName() ? "" : teamInfo.getName().toString());
			AuditUtil.setColumnStringC(teamInfo.getId().toString());
		}
		beforeDeleteTeamInfo(teamInfo);
		teamInfoDao.delete(teamInfo);
		afterDeleteTeamInfo(teamInfo);
		teamInfoDao.flush();
		List<X6BasicTeamInfo> params = new ArrayList<X6BasicTeamInfo>();
		params.add(teamInfo);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_teamManage");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.common.delete",operType="3")
	public void deleteTeamInfo(List<Long> teamInfoIds){
		deleteTeamInfo(teamInfoIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.common.delete",operType="3")
	public void deleteTeamInfo(List<Long> teamInfoIds, String eventTopic) {
		List<X6BasicTeamInfo> teamInfos = new ArrayList<X6BasicTeamInfo>();
		for(Long teamInfoId : teamInfoIds){
			X6BasicTeamInfo teamInfo = getTeamInfo(teamInfoId);
			teamInfos.add(teamInfo);
			if(null != teamInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTeamInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == teamInfo.getName() ? "" : "," + teamInfo.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == teamInfo.getName() ? "" : teamInfo.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == teamInfo.getCode() ? "" : "," + teamInfo.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == teamInfo.getCode() ? "" : teamInfo.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == teamInfo.getId() ? "" : "," + teamInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == teamInfo.getId() ? "" : teamInfo.getId().toString());
				}
			}
		}
		...
		if(teamInfos != null && teamInfos.size() > 0 ){
			for(X6BasicTeamInfo teamInfo : teamInfos){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),teamInfo, teamInfo.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(teamInfoIds != null && teamInfoIds.size() > 0) {
			String hql = "update " + X6BasicTeamInfo.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = teamInfoDao.createQuery(hql);
			query.setParameterList("ids", teamInfoIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicTeamInfo teamInfoz : teamInfos){
				teamInfoz.setValid(false);
				List<X6BasicTeamInfo> params = new ArrayList<X6BasicTeamInfo>();
				params.add(teamInfoz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_teamManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.common.delete",operType="3")
	public void deleteTeamInfo(long teamInfoId, int teamInfoVersion){
		this.deleteTeamInfo(teamInfoId, teamInfoVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.common.delete",operType="3")
	public void deleteTeamInfo(long teamInfoId, int teamInfoVersion,SignatureLog signatureLog){
		X6BasicTeamInfo teamInfo = getTeamInfo(teamInfoId);
		...
		beforeDeleteTeamInfo(teamInfo);
		teamInfoDao.delete(teamInfoId, teamInfoVersion);
		afterDeleteTeamInfo(teamInfo);
		teamInfoDao.flush();
		List<X6BasicTeamInfo> params = new ArrayList<X6BasicTeamInfo>();
		params.add(teamInfo);
		...
		if(signatureLog != null ) {
			...
			businessKey=teamInfo.getCode();
			signatureLog.setTableId(teamInfo.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.common.delete",operType="3")
	public void deleteTeamInfo(String teamInfoIds){
		this.deleteTeamInfo(teamInfoIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.common.delete",operType="3")
	public void deleteTeamInfo(String teamInfoIds,SignatureLog signatureLog){
		deleteCollection(teamInfoIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteTeamInfo(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_teamManage#modelCode:X6Basic_1.0_teamManage_TeamInfo#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="ec.engine.restore",operType="10")
	public void restoreTeamInfo(String teamInfoIds){
		restoreCollection(teamInfoIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreTeamInfo(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="ec.engine.restore",operType="10")
	public void restoreTeamInfo(long teamInfoId){
		
		findBusinessKeyUsed(teamInfoId);	//判断业务主键是否重复
		...
		beforeRestoreTeamInfo(teamInfo);
		teamInfo.setValid(true);
		teamInfoDao.update(teamInfo);
		afterRestoreTeamInfo(teamInfo);
		List<X6BasicTeamInfo> params = new ArrayList<X6BasicTeamInfo>();
		params.add(teamInfo);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long teamInfoId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_teamManage_TeamInfo");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicTeamInfo.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicTeamInfo.TABLE_NAME +" where id = ? )";
			List<Object> list =  teamInfoDao.createNativeQuery(sql,teamInfoId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseTeamInfo(List<X6BasicTeamInfo>  teamInfos){
		for(X6BasicTeamInfo teamInfo:teamInfos)  {
			saveTeamInfo(teamInfo, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseTeamInfo(List<X6BasicTeamInfo>  teamInfos){
		for(X6BasicTeamInfo teamInfo:teamInfos)  {
			saveTeamInfo(teamInfo, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="ec.flow.save")
	public void saveTeamInfo(X6BasicTeamInfo teamInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveTeamInfo(teamInfo, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="ec.flow.save")
	public void saveTeamInfo(X6BasicTeamInfo teamInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveTeamInfo(teamInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateTeamInfoCodes(X6BasicTeamInfo teamInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTeamInfoCodes(teamInfo, false);
	}
	
	public void generateTeamInfoCodes(X6BasicTeamInfo teamInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateTeamInfoSummarys(X6BasicTeamInfo teamInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTeamInfoSummarys(teamInfo, false);
	}

	public void generateTeamInfoSummarys(X6BasicTeamInfo teamInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo")
	public void saveTeamInfo(X6BasicTeamInfo teamInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveTeamInfo(teamInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo")
	public void saveTeamInfo(X6BasicTeamInfo teamInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_teamManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_teamManage");
		String url = null;
		if(teamInfo.getId() != null && teamInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTeamInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_teamManage/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTeamInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_teamManage/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, teamInfo);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(teamInfo);
		beforeSaveTeamInfo(teamInfo, viewIsView);

		if (viewIsView) {
			teamInfoDao.saveWithRevertVersion(teamInfo);
		} else {
			if(isNew)
				teamInfoDao.save(teamInfo);
			else
				teamInfoDao.merge(teamInfo);
		}
		if(null != teamInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTeamInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == teamInfo.getCode() ? "" : teamInfo.getCode().toString());
			AuditUtil.setColumnStringA(null == teamInfo.getName() ? "" : teamInfo.getName().toString());
		...
		dealDatagridsSave(teamInfo,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateTeamInfoCodes(teamInfo, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateTeamInfoSummarys(teamInfo, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveTeamInfo(teamInfo, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, teamInfo);
			}
		}
		List<X6BasicTeamInfo> params = new ArrayList<X6BasicTeamInfo>();
		teamInfoDao.flush();
		teamInfoDao.clear();
		teamInfo = teamInfoDao.load(teamInfo.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicTeamInfo teamInfo){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("teamInfo", teamInfo);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo")
	public void mergeTeamInfo(X6BasicTeamInfo teamInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(teamInfo);
		beforeSaveTeamInfo(teamInfo);
		teamInfoDao.merge(teamInfo);
		if(null != teamInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTeamInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == teamInfo.getCode() ? "" : teamInfo.getCode().toString());
			AuditUtil.setColumnStringA(null == teamInfo.getName() ? "" : teamInfo.getName().toString());
		...
		dealDatagridsSave(teamInfo,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveTeamInfo(teamInfo);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicTeamInfo> findTeamInfos(Page<X6BasicTeamInfo> page, Criterion... criterions) {
		return teamInfoDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicTeamInfoDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicTeamInfoDealInfo> list=teamInfoDao.createQuery("from " + X6BasicTeamInfoDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(X6BasicTeamInfoDealInfo dealInfo) {
		teamInfoDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicTeamInfoDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) teamInfoDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicTeamInfoDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = teamInfoDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = teamInfoDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicTeamInfoDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = teamInfoDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTeamInfos(Page<X6BasicTeamInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findTeamInfos(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTeamInfos(Page<X6BasicTeamInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTeamInfos(Page<X6BasicTeamInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicTeamInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"teamInfo\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							...
							s.append(" AND \"teamInfo\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"teamInfo\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"teamInfo\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"teamInfo\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_teamManage_TeamInfo" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_teamManage_TeamInfo" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  teamInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  teamInfoDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_teamManage").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"teamInfo\"","X6Basic_1.0_teamManage",customCondition,"S2BASE_TEAMINFO",true);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"teamInfo\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicTeamInfoDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"teamInfo\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"teamInfo\".LAY_REC ASC, \"teamInfo\".SORT ASC");
				} else {
					orderPart.append(" \"teamInfo\".TEAM_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) teamInfoDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=teamInfoDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = teamInfoDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicTeamInfo> result = new ArrayList<X6BasicTeamInfo>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicTeamInfo.class,teamInfoDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicTeamInfo.class,teamInfoDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicTeamInfo teamInfo : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(teamInfo.getId(), "X6Basic_teamManage_teamInfo");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(teamInfo.getId(), "X6Basic_teamManage_teamInfo");
							if(null != documents && !documents.isEmpty()) {
								teamInfo.setDocument(documents.get(0));
								teamInfo.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_teamManage")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_teamManage")+"X6Basic_1.0_teamManage".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_teamManage",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicTeamInfo> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicTeamInfo> result) {
		View view = null;
		...
		Map<Long, X6BasicTeamInfo> rsMap = new HashMap<Long, X6BasicTeamInfo>();
		for (X6BasicTeamInfo teamInfo : result) {
			ids.add(teamInfo.getId());
			rsMap.put(teamInfo.getId(), teamInfo);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(teamInfoDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = teamInfoDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicTeamInfo teamInfo,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(teamInfo.getOwnerStaffId());
		ti.setOwnerPositionId(teamInfo.getOwnerPositionId());
		ti.setOwnerDepartmentId(teamInfo.getOwnerDepartmentId());
		
		ti.setCreateStaff(teamInfo.getCreateStaff());
		ti.setCreateStaffId(teamInfo.getCreateStaffId());
		ti.setCreateTime(teamInfo.getCreateTime());
		ti.setDeleteStaff(teamInfo.getDeleteStaff());
		ti.setDeleteStaffId(teamInfo.getDeleteStaffId());
		ti.setDeleteTime(teamInfo.getDeleteTime());
		ti.setCreateDepartmentId(teamInfo.getCreateDepartmentId());
		ti.setModifyStaff(teamInfo.getModifyStaff());
		ti.setModifyStaffId(teamInfo.getModifyStaffId());
		ti.setCreatePositionId(teamInfo.getCreatePositionId());
		ti.setPositionLayRec(teamInfo.getPositionLayRec());
		ti.setTableNo(teamInfo.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_teamManage");
		ti.setTargetTableName("S2BASE_TEAMINFO");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_teamManage_TeamInfo" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  teamInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"teamInfo\"","X6Basic_1.0_teamManage","","S2BASE_TEAMINFO",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"teamInfo\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"teamInfo\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"teamInfo\"","X6Basic_1.0_teamManage","","S2BASE_TEAMINFO",true);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("teamInfo.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"teamInfo\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("teamInfo.mc\"").append(".TEAM_INFO) not in ("+notinValue+")");
			}
			
			SQLQuery query = teamInfoDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("teamManage", "X6Basic_1.0_teamManage", "teamManage_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=teamInfoDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicTeamInfo.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicTeamInfo.MODULE_CODE;}
	protected String getModelCode(){return X6BasicTeamInfo.MODEL_CODE;}
	 ...
	 * @param teamInfo 模型实例
	 */
	public void checkUniqueConstraint(X6BasicTeamInfo teamInfo){
		if(null == teamInfo.getCode() || StringUtils.isEmpty(teamInfo.getCode().toString())){
			return;
		}
		if(teamInfo.getId()==null ){
			List<X6BasicTeamInfo> teamInfos = teamInfoDao.findByCriteria(Restrictions.eq("code", teamInfo.getCode()));
			if(teamInfos!=null && !teamInfos.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1398303581302", getCurrentUser().getLanguage()), teamInfo.getCode());
			}
		} else {
			List<X6BasicTeamInfo> teamInfos = teamInfoDao.findByCriteria(Restrictions.ne("id", teamInfo.getId()), Restrictions.eq("code", teamInfo.getCode()));
			if(teamInfos!=null && !teamInfos.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1398303581302", getCurrentUser().getLanguage()), teamInfo.getCode());
			}
	...
	@Override
	public List<X6BasicTeamInfo> getTeamInfos(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicTeamInfo.class);
		if(params!=null){
			...
			return teamInfoDao.findByHql("from " + X6BasicTeamInfo.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return teamInfoDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicTeamInfo> findTeamInfosByHql(String hql, Object... objects){
		...
		return   teamInfoDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicTeamInfo loadTeamInfoByBussinessKey(X6BasicTeamInfo teamInfo){
		return loadTeamInfoByBussinessKey(teamInfo.getCode());
	}
	
	@Override
	public Page<X6BasicTeamInfo> getTeamInfos(Page<X6BasicTeamInfo> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicTeamInfo.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = teamInfoDao.findByPage(page, sql);
			}else{
				page = teamInfoDao.findAllByPage(page);
			}
			return page;
		}
		page = teamInfoDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public X6BasicTeamInfo loadTeamInfoByBussinessKey(Serializable bussinessKey){
		return teamInfoDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicTeamInfo> findByProperty(String propertyName, Object object){
		return teamInfoDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicTeamInfo findEntityByProperty(String propertyName, Object object){
		return teamInfoDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicTeamInfo.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = teamInfoDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_teamManage_TeamInfo","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_teamManage", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicTeamInfo teamInfo,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,teamInfoDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicTeamInfo.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, teamInfo);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",teamInfo.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicTeamInfo.TABLE_NAME +" where valid = 1";
			list = teamInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicTeamInfo.TABLE_NAME;
			list = teamInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicTeamInfo.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = teamInfoDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicTeamInfoDaoImpl teamInfoDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchTeamInfo(final List<X6BasicTeamInfo> insertObjs, final List<X6BasicTeamInfo> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchTeamInfo(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchTeamInfo(final List<X6BasicTeamInfo> insertObjs, final List<X6BasicTeamInfo> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1398303528441"),importCount));
		...
		Map<Long, X6BasicTeamInfo> oldUpdateObjsMap = new HashMap<Long, X6BasicTeamInfo>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicTeamInfo bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicTeamInfo> bizObjList = teamInfoDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicTeamInfo bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicTeamInfo> nmeObjs = new ArrayList<X6BasicTeamInfo>();
		...
		long id=autoGeneratorID.getNextId(X6BasicTeamInfo.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicTeamInfo teamInfo:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicTeamInfo.TABLE_NAME,1,dbId);
			if(teamInfo.getCode() != null && !teamInfo.getCode().equals("") ){
				m.put(teamInfo.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) teamInfoDao.createNativeQuery(sql, "X6Basic_1.0_teamManage_TeamInfo"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		teamInfoDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicTeamInfo.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicTeamInfo teamInfo = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = teamInfo.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicTeamInfo.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicTeamInfo.TABLE_NAME + " order by id desc";
//					Query query = teamInfoDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicTeamInfo teamInfo = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = teamInfo.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		teamInfoDaoImpl.getSession().flush();
		teamInfoDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicTeamInfo bizObj = insertObjs.get(insertIndex);
			teamInfoImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicTeamInfo bizObj = updateObjs.get(updateIndex);
			X6BasicTeamInfo oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			teamInfoImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = teamInfoDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				teamInfoDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = teamInfoDao.createNativeQuery(sql, "X6Basic_1.0_teamManage_TeamInfo").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = teamInfoDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = teamInfoDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = teamInfoDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?teamInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  teamInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,teamInfoDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="ec.flow.save")
	public void saveTeamInfo(X6BasicTeamInfo teamInfo, DataGridService dataGridService){
		saveTeamInfo(teamInfo, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="ec.flow.save")
	public void saveTeamInfo(X6BasicTeamInfo teamInfo, DataGridService dataGridService, String viewCode){
		saveTeamInfo(teamInfo, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo")
	public void saveTeamInfo(X6BasicTeamInfo teamInfo, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveTeamInfo(teamInfo, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo")
	public void saveTeamInfo(X6BasicTeamInfo teamInfo, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_teamManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_teamManage");
		String url = null;
		if(null != teamInfo.getId() && teamInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTeamInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo")
	public Map<Object, Object> teamInfoDataGridImport(X6BasicTeamInfo teamInfo, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = teamInfo.getVirtualId();
		
		teamInfo = teamInfoDao.load(teamInfo.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, teamInfo.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = teamInfo.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(teamInfo);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, teamInfo.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo")
	public void mergeTeamInfo(X6BasicTeamInfo teamInfo, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(teamInfo);
		beforeSaveTeamInfo(teamInfo);
		teamInfoDao.merge(teamInfo);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_teamManage",model="X6Basic_1.0_teamManage_TeamInfo",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicTeamInfo> getByPage(Page<X6BasicTeamInfo> page,DetachedCriteria detachedCriteria){
		return teamInfoDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_teamManage").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveTeamInfo(X6BasicTeamInfo teamInfo, Object... objects){
		checkUniqueConstraint(teamInfo);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveTeamInfo(X6BasicTeamInfo teamInfo, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteTeamInfo(X6BasicTeamInfo teamInfo){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteTeamInfo(X6BasicTeamInfo teamInfo){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreTeamInfo(X6BasicTeamInfo teamInfo){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreTeamInfo(X6BasicTeamInfo teamInfo){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicTeamInfo teamInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicTeamInfo teamInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportTeamInfo(List<X6BasicTeamInfo> insertObjs, List<X6BasicTeamInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportTeamInfo,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportTeamInfo(List<X6BasicTeamInfo> insertObjs, List<X6BasicTeamInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportTeamInfo,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicTeamInfo> findByHql(String hql,Object...objects){
		return teamInfoDao.findByHql(hql, objects);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicTeamInfo findEntityByHql(String hql,Object...objects){
		return teamInfoDao.findEntityByHql(hql, objects);
	}
/* CUSTOM CODE END */
}
