...
import com.supcon.orchid.X6Basic.services.MessageCenterImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.MessageCenterDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
@Service("x6Basic_messageCenterService")
@Transactional
public class MessageCenterServiceImpl  extends BaseServiceImpl<MessageCenter> implements MessageCenterService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MessageCenter> bapBaseService;
	...
	@Autowired
	private MessageCenterImportService messageCenterImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MessageCenter.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  messageCenterDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MessageCenter.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = messageCenterDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MessageCenter.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MessageCenter.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  messageCenterDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MessageCenter.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MessageCenter.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MessageCenter> list = (List<MessageCenter>) messageCenterDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MessageCenter getMessageCenter(long id){
		return getMessageCenter(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MessageCenter getMessageCenter(long id, String viewCode){
		MessageCenter messageCenter = messageCenterDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMessageCenterAsJSON(long id, String include){
		MessageCenter messageCenter = messageCenterDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(messageCenter, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.common.delete",operType="3")
	public void deleteMessageCenter(MessageCenter messageCenter){
		if(null != messageCenter && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MessageCenter.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
			AuditUtil.setColumnStringA(null == messageCenter.getEmailTitle() ? "" : messageCenter.getEmailTitle().toString());
			AuditUtil.setColumnStringC(messageCenter.getId().toString());
		}
		beforeDeleteMessageCenter(messageCenter);
		messageCenterDao.delete(messageCenter);
		afterDeleteMessageCenter(messageCenter);
		messageCenterDao.flush();
		List<MessageCenter> params = new ArrayList<MessageCenter>();
		params.add(messageCenter);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_messageManage");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.common.delete",operType="3")
	public void deleteMessageCenter(List<Long> messageCenterIds){
		deleteMessageCenter(messageCenterIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.common.delete",operType="3")
	public void deleteMessageCenter(List<Long> messageCenterIds, String eventTopic) {
		List<MessageCenter> messageCenters = new ArrayList<MessageCenter>();
		for(Long messageCenterId : messageCenterIds){
			MessageCenter messageCenter = getMessageCenter(messageCenterId);
			messageCenters.add(messageCenter);
			if(null != messageCenter && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MessageCenter.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == messageCenter.getEmailTitle() ? "" : "," + messageCenter.getEmailTitle().toString()));
				} else {
					AuditUtil.setColumnStringA(null == messageCenter.getEmailTitle() ? "" : messageCenter.getEmailTitle().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == messageCenter.getId() ? "" : "," + messageCenter.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == messageCenter.getId() ? "" : "," + messageCenter.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
				}
			}
		}
		...
		if(messageCenters != null && messageCenters.size() > 0 ){
			for(MessageCenter messageCenter : messageCenters){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),messageCenter, messageCenter.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(messageCenterIds != null && messageCenterIds.size() > 0) {
			String hql = "update " + MessageCenter.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = messageCenterDao.createQuery(hql);
			query.setParameterList("ids", messageCenterIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MessageCenter messageCenterz : messageCenters){
				messageCenterz.setValid(false);
				List<MessageCenter> params = new ArrayList<MessageCenter>();
				params.add(messageCenterz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_messageManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.common.delete",operType="3")
	public void deleteMessageCenter(long messageCenterId, int messageCenterVersion){
		this.deleteMessageCenter(messageCenterId, messageCenterVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.common.delete",operType="3")
	public void deleteMessageCenter(long messageCenterId, int messageCenterVersion,SignatureLog signatureLog){
		MessageCenter messageCenter = getMessageCenter(messageCenterId);
		...
		beforeDeleteMessageCenter(messageCenter);
		messageCenterDao.delete(messageCenterId, messageCenterVersion);
		afterDeleteMessageCenter(messageCenter);
		messageCenterDao.flush();
		List<MessageCenter> params = new ArrayList<MessageCenter>();
		params.add(messageCenter);
		...
		if(signatureLog != null ) {
			...
			businessKey=messageCenter.getId();
			signatureLog.setTableId(messageCenter.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.common.delete",operType="3")
	public void deleteMessageCenter(String messageCenterIds){
		this.deleteMessageCenter(messageCenterIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.common.delete",operType="3")
	public void deleteMessageCenter(String messageCenterIds,SignatureLog signatureLog){
		deleteCollection(messageCenterIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMessageCenter(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_messageManage#modelCode:X6Basic_1.0_messageManage_MessageCenter#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="ec.engine.restore",operType="10")
	public void restoreMessageCenter(String messageCenterIds){
		restoreCollection(messageCenterIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreMessageCenter(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="ec.engine.restore",operType="10")
	public void restoreMessageCenter(long messageCenterId){
		
		findBusinessKeyUsed(messageCenterId);	//判断业务主键是否重复
		...
		beforeRestoreMessageCenter(messageCenter);
		messageCenter.setValid(true);
		messageCenterDao.update(messageCenter);
		afterRestoreMessageCenter(messageCenter);
		List<MessageCenter> params = new ArrayList<MessageCenter>();
		params.add(messageCenter);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long messageCenterId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_messageManage_MessageCenter");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MessageCenter.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MessageCenter.TABLE_NAME +" where id = ? )";
			List<Object> list =  messageCenterDao.createNativeQuery(sql,messageCenterId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseMessageCenter(List<MessageCenter>  messageCenters){
		for(MessageCenter messageCenter:messageCenters)  {
			saveMessageCenter(messageCenter, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMessageCenter(List<MessageCenter>  messageCenters){
		for(MessageCenter messageCenter:messageCenters)  {
			saveMessageCenter(messageCenter, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="ec.flow.save")
	public void saveMessageCenter(MessageCenter messageCenter, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMessageCenter(messageCenter, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="ec.flow.save")
	public void saveMessageCenter(MessageCenter messageCenter, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMessageCenter(messageCenter, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateMessageCenterCodes(MessageCenter messageCenter) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMessageCenterCodes(messageCenter, false);
	}
	
	public void generateMessageCenterCodes(MessageCenter messageCenter, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateMessageCenterSummarys(MessageCenter messageCenter) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMessageCenterSummarys(messageCenter, false);
	}

	public void generateMessageCenterSummarys(MessageCenter messageCenter, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter")
	public void saveMessageCenter(MessageCenter messageCenter,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveMessageCenter(messageCenter, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter")
	public void saveMessageCenter(MessageCenter messageCenter,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_messageManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_messageManage");
		String url = null;
		if(messageCenter.getId() != null && messageCenter.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MessageCenter.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_messageManage/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MessageCenter.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_messageManage/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, messageCenter);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(messageCenter);
		beforeSaveMessageCenter(messageCenter, viewIsView);

		if (viewIsView) {
			messageCenterDao.saveWithRevertVersion(messageCenter);
		} else {
			if(isNew)
				messageCenterDao.save(messageCenter);
			else
				messageCenterDao.merge(messageCenter);
		}
		if(null != messageCenter && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MessageCenter.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
			AuditUtil.setColumnStringA(null == messageCenter.getEmailTitle() ? "" : messageCenter.getEmailTitle().toString());
		...
		dealDatagridsSave(messageCenter,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateMessageCenterCodes(messageCenter, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMessageCenterSummarys(messageCenter, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMessageCenter(messageCenter, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, messageCenter);
			}
		}
		List<MessageCenter> params = new ArrayList<MessageCenter>();
		messageCenterDao.flush();
		messageCenterDao.clear();
		messageCenter = messageCenterDao.load(messageCenter.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MessageCenter messageCenter){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("messageCenter", messageCenter);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter")
	public void mergeMessageCenter(MessageCenter messageCenter, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(messageCenter);
		beforeSaveMessageCenter(messageCenter);
		messageCenterDao.merge(messageCenter);
		if(null != messageCenter && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MessageCenter.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == messageCenter.getId() ? "" : messageCenter.getId().toString());
			AuditUtil.setColumnStringA(null == messageCenter.getEmailTitle() ? "" : messageCenter.getEmailTitle().toString());
		...
		dealDatagridsSave(messageCenter,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveMessageCenter(messageCenter);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MessageCenter> findMessageCenters(Page<MessageCenter> page, Criterion... criterions) {
		return messageCenterDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MessageCenterDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MessageCenterDealInfo> list=messageCenterDao.createQuery("from " + MessageCenterDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MessageCenterDealInfo dealInfo) {
		messageCenterDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MessageCenterDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) messageCenterDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MessageCenterDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = messageCenterDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = messageCenterDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MessageCenterDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = messageCenterDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMessageCenters(Page<MessageCenter> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMessageCenters(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMessageCenters(Page<MessageCenter> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMessageCenters(Page<MessageCenter> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MessageCenter> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"messageCenter\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							...
							s.append(" AND \"messageCenter\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"messageCenter\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"messageCenter\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"messageCenter\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_messageManage_MessageCenter" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_messageManage_MessageCenter" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  messageCenterDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  messageCenterDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_messageManage").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"messageCenter\"","X6Basic_1.0_messageManage",customCondition,"X6BASIC_MESSAGE_CENTERS",true);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"messageCenter\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MessageCenterDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"messageCenter\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"messageCenter\".LAY_REC ASC, \"messageCenter\".SORT ASC");
				} else {
					orderPart.append(" \"messageCenter\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) messageCenterDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=messageCenterDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = messageCenterDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MessageCenter> result = new ArrayList<MessageCenter>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MessageCenter.class,messageCenterDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MessageCenter.class,messageCenterDao,maps));
				}
				...
				if(null != hasAttachment && hasAttachment) {
					for (MessageCenter messageCenter : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(messageCenter.getId(), "X6Basic_messageManage_messageCenter");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(messageCenter.getId(), "X6Basic_messageManage_messageCenter");
							if(null != documents && !documents.isEmpty()) {
								messageCenter.setDocument(documents.get(0));
								messageCenter.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_messageManage")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_messageManage")+"X6Basic_1.0_messageManage".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_messageManage",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MessageCenter> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MessageCenter> result) {
		View view = null;
		...
		Map<Long, MessageCenter> rsMap = new HashMap<Long, MessageCenter>();
		for (MessageCenter messageCenter : result) {
			ids.add(messageCenter.getId());
			rsMap.put(messageCenter.getId(), messageCenter);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(messageCenterDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = messageCenterDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MessageCenter messageCenter,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(messageCenter.getOwnerStaffId());
		ti.setOwnerPositionId(messageCenter.getOwnerPositionId());
		ti.setOwnerDepartmentId(messageCenter.getOwnerDepartmentId());
		
		ti.setCreateStaff(messageCenter.getCreateStaff());
		ti.setCreateStaffId(messageCenter.getCreateStaffId());
		ti.setCreateTime(messageCenter.getCreateTime());
		ti.setDeleteStaff(messageCenter.getDeleteStaff());
		ti.setDeleteStaffId(messageCenter.getDeleteStaffId());
		ti.setDeleteTime(messageCenter.getDeleteTime());
		ti.setCreateDepartmentId(messageCenter.getCreateDepartmentId());
		ti.setModifyStaff(messageCenter.getModifyStaff());
		ti.setModifyStaffId(messageCenter.getModifyStaffId());
		ti.setCreatePositionId(messageCenter.getCreatePositionId());
		ti.setPositionLayRec(messageCenter.getPositionLayRec());
		ti.setTableNo(messageCenter.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_messageManage");
		ti.setTargetTableName("X6BASIC_MESSAGE_CENTERS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_messageManage_MessageCenter" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  messageCenterDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"messageCenter\"","X6Basic_1.0_messageManage","","X6BASIC_MESSAGE_CENTERS",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"messageCenter\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"messageCenter\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"messageCenter\"","X6Basic_1.0_messageManage","","X6BASIC_MESSAGE_CENTERS",true);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("messageCenter.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"messageCenter\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"messageCenter\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"messageCenter\".CID = 1 OR \"messageCenter\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("messageCenter.mc\"").append(".MESSAGE_CENTER) not in ("+notinValue+")");
			}
			
			SQLQuery query = messageCenterDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("messageManage", "X6Basic_1.0_messageManage", "messageManage_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=messageCenterDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MessageCenter.ENTITY_CODE;}
	protected String getModuleCode(){return MessageCenter.MODULE_CODE;}
	protected String getModelCode(){return MessageCenter.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param messageCenter 模型实例
	 */
	public void checkUniqueConstraint(MessageCenter messageCenter){
	}
	
	
	@Override
	public List<MessageCenter> getMessageCenters(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MessageCenter.class);
		if(params!=null){
			...
			return messageCenterDao.findByHql("from " + MessageCenter.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return messageCenterDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MessageCenter> findMessageCentersByHql(String hql, Object... objects){
		...
		return   messageCenterDao.findByHql(hql, objects);
	}
	
	@Override
	public MessageCenter loadMessageCenterByBussinessKey(MessageCenter messageCenter){
		return loadMessageCenterByBussinessKey(messageCenter.getId());
	}
	
	@Override
	public Page<MessageCenter> getMessageCenters(Page<MessageCenter> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MessageCenter.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = messageCenterDao.findByPage(page, sql);
			}else{
				page = messageCenterDao.findAllByPage(page);
			}
			return page;
		}
		page = messageCenterDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public MessageCenter loadMessageCenterByBussinessKey(Serializable bussinessKey){
		return messageCenterDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MessageCenter> findByProperty(String propertyName, Object object){
		return messageCenterDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MessageCenter findEntityByProperty(String propertyName, Object object){
		return messageCenterDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MessageCenter.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = messageCenterDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_messageManage_MessageCenter","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_messageManage", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MessageCenter messageCenter,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,messageCenterDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MessageCenter.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, messageCenter);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",messageCenter.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MessageCenter.TABLE_NAME +" where valid = 1";
			list = messageCenterDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MessageCenter.TABLE_NAME;
			list = messageCenterDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MessageCenter.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = messageCenterDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MessageCenterDaoImpl messageCenterDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchMessageCenter(final List<MessageCenter> insertObjs, final List<MessageCenter> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchMessageCenter(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchMessageCenter(final List<MessageCenter> insertObjs, final List<MessageCenter> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1370247810414"),importCount));
		...
		Map<Long, MessageCenter> oldUpdateObjsMap = new HashMap<Long, MessageCenter>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MessageCenter bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MessageCenter> bizObjList = messageCenterDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MessageCenter bizObj = bizObjList.get(mIndex);
		...
		final List<MessageCenter> nmeObjs = new ArrayList<MessageCenter>();
		...
		long id=autoGeneratorID.getNextId(MessageCenter.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MessageCenter messageCenter:insertObjs)  {
			//id = autoGeneratorID.getNextId(MessageCenter.TABLE_NAME,1,dbId);
			if(messageCenter.getId() != null ){
				m.put(messageCenter.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) messageCenterDao.createNativeQuery(sql, "X6Basic_1.0_messageManage_MessageCenter"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		messageCenterDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MessageCenter.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MessageCenter messageCenter = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = messageCenter.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(messageCenter);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MessageCenter.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MessageCenter.TABLE_NAME + " order by id desc";
//					Query query = messageCenterDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MessageCenter messageCenter = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = messageCenter.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		messageCenterDaoImpl.getSession().flush();
		messageCenterDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MessageCenter bizObj = insertObjs.get(insertIndex);
			messageCenterImportService.saveImportDataLog(bizObj, null, "id", "emailTitle");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MessageCenter bizObj = updateObjs.get(updateIndex);
			MessageCenter oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			messageCenterImportService.saveImportDataLog(bizObj, oldBizObj, "id", "emailTitle");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = messageCenterDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				messageCenterDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = messageCenterDao.createNativeQuery(sql, "X6Basic_1.0_messageManage_MessageCenter").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = messageCenterDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = messageCenterDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = messageCenterDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?messageCenterDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  messageCenterDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,messageCenterDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="ec.flow.save")
	public void saveMessageCenter(MessageCenter messageCenter, DataGridService dataGridService){
		saveMessageCenter(messageCenter, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="ec.flow.save")
	public void saveMessageCenter(MessageCenter messageCenter, DataGridService dataGridService, String viewCode){
		saveMessageCenter(messageCenter, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter")
	public void saveMessageCenter(MessageCenter messageCenter, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveMessageCenter(messageCenter, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter")
	public void saveMessageCenter(MessageCenter messageCenter, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_messageManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_messageManage");
		String url = null;
		if(null != messageCenter.getId() && messageCenter.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MessageCenter.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter")
	public Map<Object, Object> messageCenterDataGridImport(MessageCenter messageCenter, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = messageCenter.getVirtualId();
		
		messageCenter = messageCenterDao.load(messageCenter.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, messageCenter.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = messageCenter.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(messageCenter);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, messageCenter.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter")
	public void mergeMessageCenter(MessageCenter messageCenter, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(messageCenter);
		beforeSaveMessageCenter(messageCenter);
		messageCenterDao.merge(messageCenter);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_messageManage",model="X6Basic_1.0_messageManage_MessageCenter",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MessageCenter> getByPage(Page<MessageCenter> page,DetachedCriteria detachedCriteria){
		return messageCenterDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_messageManage").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveMessageCenter(MessageCenter messageCenter, Object... objects){
		checkUniqueConstraint(messageCenter);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveMessageCenter(MessageCenter messageCenter, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMessageCenter(MessageCenter messageCenter){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteMessageCenter(MessageCenter messageCenter){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreMessageCenter(MessageCenter messageCenter){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreMessageCenter(MessageCenter messageCenter){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MessageCenter messageCenter, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MessageCenter messageCenter, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportMessageCenter(List<MessageCenter> insertObjs, List<MessageCenter> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMessageCenter,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportMessageCenter(List<MessageCenter> insertObjs, List<MessageCenter> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMessageCenter,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
	@Autowired
	...
	public void sendAllMessage() {
		List<MessageCenter> list = messageCenterDao.findByCriteria(Restrictions.le("warnTime", new Date()));
		List<Notification> notifications = new ArrayList<Notification>();
		if(list!=null && !list.isEmpty()){
			for(MessageCenter messageCenter : list){
				List<Notification> temp = this.getNotifications(messageCenter);
				if(!temp.isEmpty()){
		...
		if(!notifications.isEmpty()){
			...
			for(MessageCenter messageCenter : list){
				messageCenterDao.deletePhysical(messageCenter);
			}
		}
	}

	/**
	 * 获取{@link MessageCenter}的{@link Notification}
	 * @param messageCenter
	 * @return
	 */
	private List<Notification> getNotifications(MessageCenter messageCenter) {
		List<Notification> notifications = new ArrayList<Notification>();
		if (null != messageCenter) {
			String emailContent=messageCenter.getEmailContent();
			String emailTitle=messageCenter.getEmailTitle();
			String messageContent=messageCenter.getMessageContent();
			String pandionContent=messageCenter.getPandionContent();
			if (emailContent!= null &&emailContent.length() > 0) {
				Notification email = notificationService.createNotification(Notification.EMAIL_TYPE, new String[] { messageCenter
						.getAccepter().getEmail() }, (emailTitle!=null&&emailTitle.length()>0)?emailTitle.substring(1):"", emailContent.substring(1), "", null);
				notifications.add(email);
			}
			if (messageContent!= null && messageContent.length() > 0) {
				Notification sms = notificationService.createNotification(Notification.SMS_TYPE, new String[] { messageCenter.getAccepter()
						.getMobile() }, "", messageContent.substring(1), "", null);
				notifications.add(sms);
			}
			if (pandionContent != null && pandionContent.length() > 0) {
				Notification jabber = notificationService.createNotification(Notification.JABBER_TYPE, new String[] { messageCenter
						.getAccepter().getJabber() }, "", pandionContent.substring(1), "", null);
				notifications.add(jabber);
			}
		}
		return notifications;
	}
	private void sendNotification(List<Notification> notifications) {
		Notification[] notificationObj = new Notification[notifications.size()];
		notificationService.send(true, notifications.toArray(notificationObj));
	}
	/**
	 * 删除消息
	 * @param modelCode
	 ...
	 */
	@Override
	public void deleteMessagePhysical(String modelCode, Long projectId) {
		messageCenterDao.bulkExecute("delete MessageCenter where resModelCode=? and modelId=? ", modelCode, projectId);
	}
/* CUSTOM CODE END */
}
