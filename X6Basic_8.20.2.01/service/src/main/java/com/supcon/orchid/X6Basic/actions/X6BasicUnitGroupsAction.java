...
/* CUSTOM CODE START(action,import,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicUnitGroupsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicUnitGroups> {
	...
	private X6BasicUnitGroups unitGroups;
	...
	//导出excel时,主模型内容
	List<X6BasicUnitGroups> queryResults = new ArrayList<X6BasicUnitGroups>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/unitGroup/unitGroups/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/unitGroup/unitGroups/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_unitGroup_UnitGroups" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicUnitGroups> unitGroupss;
	
	public List<X6BasicUnitGroups> getUnitGroupss() {
		return unitGroupss;
	}
	...
	private Page<X6BasicUnitGroups> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			unitGroups = new X6BasicUnitGroups();
			if(unitGroups.getGroupBapType() != null) {
				unitGroups.setGroupBapType((SystemCode) systemCodeService.getSystemCode(unitGroups.getGroupBapType().getId()));
			}
		} else {
			if(id > 0){
				unitGroups = unitGroupsService.getUnitGroups(id, viewCode);
			}
		}
	...
	public Page<X6BasicUnitGroups> getPage() {
		if (null == page) {
			page = new Page<X6BasicUnitGroups>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicUnitGroups getUnitGroups() {
		return unitGroups;
	}		

	public void setUnitGroups(X6BasicUnitGroups unitGroups) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/unitGroup/unitGroups/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = unitGroupsService.getUnitGroupsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/unitGroup/unitGroups/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1398233591035
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String unitGroupsReferenceCopy() throws Exception{
			X6BasicUnitGroups oldUnitGroups = null;
			if(isEditNew()){
				...
				oldUnitGroups = new X6BasicUnitGroups();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,unitGroups.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, unitGroups.getId());
				...
				oldUnitGroups = unitGroupsService.getUnitGroups(id);
			}
			
			
			unitGroups = unitGroupsService.getUnitGroups(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_unitGroup_UnitGroups");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicUnitGroups.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldUnitGroups, null);
						Method setValue = X6BasicUnitGroups.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(unitGroups, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			unitGroups.setCreateDepartmentId(oldUnitGroups.getCreateDepartmentId());
			unitGroups.setCreatePositionId(oldUnitGroups.getCreatePositionId());
			unitGroups.setCreateStaffId(oldUnitGroups.getCreateStaffId());
			unitGroups.setOwnerDepartmentId(oldUnitGroups.getOwnerDepartmentId());
			unitGroups.setOwnerPositionId(oldUnitGroups.getOwnerPositionId());
			unitGroups.setOwnerStaffId(oldUnitGroups.getOwnerStaffId());
			unitGroups.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/unitGroups/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/unitGroup/unitGroups/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=unitGroupsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return unitGroupsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			unitGroups = unitGroupsService.getUnitGroups(this.unitGroups.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,unitGroups.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/unitGroups/unitView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "unitView.ftl") }),
		@Action(value = "/X6Basic/unitGroup/unitGroups/unitViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "unitViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/unitGroups/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = unitGroupsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(unitGroupsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(unitGroupsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_unitGroup_UnitGroups", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/unitGroups/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/unitGroups/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.groupMemo,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"unitGroups\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"unitGroups\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(listView.getUsedForTree()&&listView.getAssTreeModelCode()!=null){
			...
			if (null != request.getParameter("tree-"+listView.getAssTreeModelCode()+".companyId") && request.getParameter("tree-"+listView.getAssTreeModelCode()+".companyId").trim().length() > 0) {
				params.add(new Param("\"tree-"+listView.getAssTreeModelCode()+"\"."+Inflector.getInstance().columnize("companyId"),request.getParameter("tree-"+listView.getAssTreeModelCode()+".companyId"),Param.LIKE_UNSUPPORT,"LONG"));
			}
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"unitGroups\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"unitGroups\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/unitGroups/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/unitGroups/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.groupMemo,result.attrMap.*"})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/unitGroup/unitGroups/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_unitGroup_UnitGroups");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		unitGroupsService.findDataGridPage(dg,dgclass,getDgPage(),unitGroups,"", null);
	...
	private Page<X6BasicBaseUnit> dg1398235995490Page;
	public Page<X6BasicBaseUnit> getDg1398235995490Page(){
		if(null == dg1398235995490Page){
			dg1398235995490Page = new Page<X6BasicBaseUnit>(1,20);
		}
		return dg1398235995490Page;
	}
	
	private Class dg1398235995490Clazz = X6BasicBaseUnit.class;
	
	public Class getDg1398235995490Clazz() {
		return dg1398235995490Clazz;
	}
	
	private List<X6BasicBaseUnit> dg1398235995490List;
	public List<X6BasicBaseUnit> getDg1398235995490List(){
		if(null == dg1398235995490List){
			dg1398235995490List = new ArrayList<X6BasicBaseUnit>();
		}
		return dg1398235995490List;
	}
	public void setDg1398235995490ListJson(String json) {
		//dg1398235995490List = (List<X6BasicBaseUnit>) unitGroupsService.generateObjectFromJson(json, X6BasicBaseUnit.class);
	}
	
	private List<Long> dg1398235995490DeletedIds;
	public List<Long> getDg1398235995490DeletedIds(){
		if(null == dg1398235995490DeletedIds){
			dg1398235995490DeletedIds = new ArrayList<Long>();
		}
		return dg1398235995490DeletedIds;
	}

	private Page<X6BasicBaseUnit> dg1398236153862Page;
	public Page<X6BasicBaseUnit> getDg1398236153862Page(){
		if(null == dg1398236153862Page){
			dg1398236153862Page = new Page<X6BasicBaseUnit>(1,20);
		}
		return dg1398236153862Page;
	}
	
	private Class dg1398236153862Clazz = X6BasicBaseUnit.class;
	
	public Class getDg1398236153862Clazz() {
		return dg1398236153862Clazz;
	}
	
	private List<X6BasicBaseUnit> dg1398236153862List;
	public List<X6BasicBaseUnit> getDg1398236153862List(){
		if(null == dg1398236153862List){
			dg1398236153862List = new ArrayList<X6BasicBaseUnit>();
		}
		return dg1398236153862List;
	}
	public void setDg1398236153862ListJson(String json) {
		//dg1398236153862List = (List<X6BasicBaseUnit>) unitGroupsService.generateObjectFromJson(json, X6BasicBaseUnit.class);
	}
	
	private List<Long> dg1398236153862DeletedIds;
	public List<Long> getDg1398236153862DeletedIds(){
		if(null == dg1398236153862DeletedIds){
			dg1398236153862DeletedIds = new ArrayList<Long>();
		}
		return dg1398236153862DeletedIds;
	}

	@Actions({
		
		@Action(value = "/X6Basic/unitGroup/unitGroups/data-dg1398235995490", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.code,result.name,result.isMain,result.rate,result.comments,result.id,result.version,result.unitGroup.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1398235995490-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1398235995490.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/X6Basic/unitGroup/unitGroups/data-dg1398236153862", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.code,result.name,result.isMain,result.rate,result.comments,result.id,result.version,result.unitGroup.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1398236153862-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1398236153862.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(unitGroups.getId() != null && unitGroups.getId() != -1){
				unitGroups = unitGroupsService.getUnitGroups(unitGroups.getId());
			}
			unitGroupsService.findDataGridPage(dg,dgclass,getDgPage(),unitGroups,searchCondition,params);
	...
	private void save() throws Exception {
		...
		unitGroupsService.saveUnitGroups(unitGroups,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/unitGroup/unitGroups/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/unitGroup/unitGroups/unitView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "submit");
		bap_validate_datagrids=unitGroupsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", unitGroups.getId());
		...
		fileuploads.put("linkId", unitGroups.getId());
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/unitGroup/unitGroups/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		unitGroupsService.deleteUnitGroups(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/unitGroup/unitGroups/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "restore");
		unitGroupsService.restoreUnitGroups(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/unitGroup/unitGroups/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/unitGroup/unitGroups/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "showDealInfoData");
		if (null != tableInfoId) {
			int count=unitGroupsService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = unitGroupsService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = unitGroupsService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicUnitGroups> findExportDataInfos(Page<X6BasicUnitGroups> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = unitGroupsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = unitGroupsService.getAssProperty(p.getCode());	
									//String assModelCode = unitGroupsService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			unitGroupsService.findUnitGroupss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			unitGroupsService.findUnitGroupss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicUnitGroups> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicUnitGroups result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".UNITGROUP_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.unitGroups || null == this.unitGroups.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/unitGroup/unitGroups/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/unitGroup/unitGroups/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/unitGroup/unitGroups/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/unitGroup/unitGroups/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicUnitGroupsService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_unitGroup", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_unitGroup");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/unitGroup/unitGroups/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_unitGroup_UnitGroups", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_unitGroup_UnitGroups", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/unitGroup/unitGroups/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/unitGroup/unitGroups/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_unitGroup_UnitGroups";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_UnitGroups", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/unitGroup/unitGroups/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
		_auditLog.setColumnStringA(null == unitGroups.getName() ? "" : unitGroups.getName().toString());
		_auditLog.setColumnStringC(unitGroups.getId().toString());
		_auditLog.setColumnStringD(X6BasicUnitGroups.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_unitGroup");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicUnitGroupsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, unitGroups,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, unitGroups,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,unitGroups);
		...
		printlog.setEntity("X6Basic_1.0_unitGroup");
		...
		printlog.setBusinessId(unitGroups.getId());
		printlog.setBussinessKey(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
		printlog.setMainDisplay(null == unitGroups.getName() ? "" : unitGroups.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/unitGroup/unitGroups/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(unitGroups,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/unitGroup/unitGroups/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(unitGroupsService.getUnitGroups(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/unitGroup/unitGroups/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/unitGroup/unitGroups/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, unitGroups,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				unitGroups = unitGroupsService.getUnitGroups(Long.parseLong(arrId[i]));
				if(null != unitGroups){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == unitGroups.getName() ? "" : "," + unitGroups.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == unitGroups.getName() ? "" : unitGroups.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == unitGroups.getId() ? "" : "," + unitGroups.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == unitGroups.getId() ? "" : "," + unitGroups.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, unitGroups,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,unitGroups);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/unitGroup/unitGroups/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicUnitGroups> findExportPageInfos(Page<X6BasicUnitGroups> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_unitGroup" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = unitGroupsService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicUnitGroups queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = unitGroupsService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = unitGroupsService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_unitGroup_UnitGroups");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_unitGroup_UnitGroups")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_unitGroup_UnitGroups");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_unitGroup_UnitGroups");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = unitGroupsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
