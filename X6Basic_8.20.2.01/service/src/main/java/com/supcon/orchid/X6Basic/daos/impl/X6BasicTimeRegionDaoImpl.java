...
@Repository
public class X6BasicTimeRegionDaoImpl extends ExtendedGenericDaoImpl<X6BasicTimeRegion, Long> implements X6BasicTimeRegionDao {
	@Override
	...
	@Override
	public X6BasicTimeRegionDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicTimeRegionDealInfo dealInfo = new X6BasicTimeRegionDealInfo();
	...
	@Override
	public X6BasicTimeRegionDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicTimeRegion mainObj, Staff dealStaff) {
		X6BasicTimeRegionDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicTimeRegionDealInfo.class);
		...
		List<X6BasicTimeRegionDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
