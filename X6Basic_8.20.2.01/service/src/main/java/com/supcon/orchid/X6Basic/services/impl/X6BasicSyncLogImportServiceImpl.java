...
@Service("x6BasicSyncLogImportService")
public class X6BasicSyncLogImportServiceImpl implements X6BasicSyncLogImportService {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_syncLog",model="X6Basic_1.0_syncLog_SyncLog",desc="保存业务信息",operType="1")
	public void saveImportDataLog(X6BasicSyncLog bizObj, X6BasicSyncLog oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("X6Basic.entityname.radion1411469375523") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getEventTopic() ? "" : "," + oldBizObj.getEventTopic().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getEventTopic() ? "" : oldBizObj.getEventTopic().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("X6Basic.entityname.radion1411469375523") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getEventTopic() ? "" : "," + bizObj.getEventTopic().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getEventTopic() ? "" : bizObj.getEventTopic().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(X6BasicSyncLog bizObj, String bussinesskeyFieldName,
