...
/* CUSTOM CODE START(action,import,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicPlanDailyAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicPlanDaily> {
	...
	private X6BasicPlanDaily planDaily;
	...
	//导出excel时,主模型内容
	List<X6BasicPlanDaily> queryResults = new ArrayList<X6BasicPlanDaily>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/rangePlan/planDaily/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/rangePlan/planDaily/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_rangePlan_PlanDaily" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicPlanDaily> planDailys;
	
	public List<X6BasicPlanDaily> getPlanDailys() {
		return planDailys;
	}
	...
	private Page<X6BasicPlanDaily> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			planDaily = new X6BasicPlanDaily();
		} else {
			if(id > 0){
				planDaily = planDailyService.getPlanDaily(id, viewCode);
			}
		}
	...
	public Page<X6BasicPlanDaily> getPage() {
		if (null == page) {
			page = new Page<X6BasicPlanDaily>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicPlanDaily getPlanDaily() {
		return planDaily;
	}		

	public void setPlanDaily(X6BasicPlanDaily planDaily) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/rangePlan/planDaily/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = planDailyService.getPlanDailyAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/rangePlan/planDaily/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1398310135469
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String planDailyReferenceCopy() throws Exception{
			X6BasicPlanDaily oldPlanDaily = null;
			if(isEditNew()){
				...
				oldPlanDaily = new X6BasicPlanDaily();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,planDaily.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, planDaily.getId());
				...
				oldPlanDaily = planDailyService.getPlanDaily(id);
			}
			
			
			planDaily = planDailyService.getPlanDaily(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_PlanDaily");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicPlanDaily.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPlanDaily, null);
						Method setValue = X6BasicPlanDaily.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(planDaily, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			planDaily.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=planDailyService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return planDailyReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			planDaily = planDailyService.getPlanDaily(this.planDaily.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,planDaily.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = planDailyService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(planDailyService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(planDailyService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_rangePlan_PlanDaily", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"planDaily\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"planDaily\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"planDaily\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"planDaily\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		planDailyService.findDataGridPage(dg,dgclass,getDgPage(),planDaily,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(planDaily.getId() != null && planDaily.getId() != -1){
				planDaily = planDailyService.getPlanDaily(planDaily.getId());
			}
			planDailyService.findDataGridPage(dg,dgclass,getDgPage(),planDaily,searchCondition,params);
	...
	private void save() throws Exception {
		...
		planDailyService.savePlanDaily(planDaily,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "submit");
		bap_validate_datagrids=planDailyService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", planDaily.getId());
		...
		fileuploads.put("linkId", planDaily.getId());
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/rangePlan/planDaily/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		planDailyService.deletePlanDaily(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/planDaily/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "restore");
		planDailyService.restorePlanDaily(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicPlanDaily> findExportDataInfos(Page<X6BasicPlanDaily> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = planDailyService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = planDailyService.getAssProperty(p.getCode());	
									//String assModelCode = planDailyService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			planDailyService.findPlanDailys(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			planDailyService.findPlanDailys(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicPlanDaily> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicPlanDaily result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".RPD_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.planDaily || null == this.planDaily.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/rangePlan/planDaily/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/rangePlan/planDaily/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/rangePlan/planDaily/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/planDaily/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicPlanDailyService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/planDaily/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_rangePlan_PlanDaily", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_rangePlan_PlanDaily", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/rangePlan/planDaily/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/rangePlan/planDaily/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_rangePlan_PlanDaily";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_PlanDaily", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/rangePlan/planDaily/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == planDaily.getId() ? "" : planDaily.getId().toString());
		_auditLog.setColumnStringA(null == planDaily.getId() ? "" : planDaily.getId().toString());
		_auditLog.setColumnStringC(planDaily.getId().toString());
		_auditLog.setColumnStringD(X6BasicPlanDaily.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicPlanDailyAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, planDaily,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, planDaily,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,planDaily);
		...
		printlog.setBusinessId(planDaily.getId());
		printlog.setBussinessKey(null == planDaily.getId() ? "" : planDaily.getId().toString());
		printlog.setMainDisplay(null == planDaily.getId() ? "" : planDaily.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/rangePlan/planDaily/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(planDaily,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/planDaily/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(planDailyService.getPlanDaily(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/rangePlan/planDaily/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/rangePlan/planDaily/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, planDaily,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				planDaily = planDailyService.getPlanDaily(Long.parseLong(arrId[i]));
				if(null != planDaily){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == planDaily.getId() ? "" : "," + planDaily.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == planDaily.getId() ? "" : planDaily.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == planDaily.getId() ? "" : "," + planDaily.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == planDaily.getId() ? "" : planDaily.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == planDaily.getId() ? "" : "," + planDaily.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == planDaily.getId() ? "" : planDaily.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, planDaily,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,planDaily);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/rangePlan/planDaily/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_PlanDaily");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_rangePlan_PlanDaily")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_rangePlan_PlanDaily");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_PlanDaily");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = planDailyService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
