...
@Service("x6BasicSpellRulePartImportService")
public class X6BasicSpellRulePartImportServiceImpl implements X6BasicSpellRulePartImportService {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="保存业务信息",operType="1")
	public void saveImportDataLog(X6BasicSpellRulePart bizObj, X6BasicSpellRulePart oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("X6Basic.entityname.radion1398306606424") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getTeamOrder() ? "" : "," + oldBizObj.getTeamOrder().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getTeamOrder() ? "" : oldBizObj.getTeamOrder().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("X6Basic.entityname.radion1398306606424") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getTeamOrder() ? "" : "," + bizObj.getTeamOrder().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getTeamOrder() ? "" : bizObj.getTeamOrder().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(X6BasicSpellRulePart bizObj, String bussinesskeyFieldName,
