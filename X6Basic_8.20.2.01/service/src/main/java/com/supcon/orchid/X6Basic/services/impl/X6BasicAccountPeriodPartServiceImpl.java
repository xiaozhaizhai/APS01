...
import com.supcon.orchid.X6Basic.services.X6BasicAccountPeriodPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicAccountPeriodPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_accountPeriodPartService")
@Transactional
public class X6BasicAccountPeriodPartServiceImpl  extends BaseServiceImpl<X6BasicAccountPeriodPart> implements X6BasicAccountPeriodPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicAccountPeriodPart> bapBaseService;
	...
	@Autowired
	private X6BasicAccountPeriodPartImportService accountPeriodPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicAccountPeriodPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  accountPeriodPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicAccountPeriodPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = accountPeriodPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicAccountPeriodPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicAccountPeriodPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  accountPeriodPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicAccountPeriodPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicAccountPeriodPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicAccountPeriodPart> list = (List<X6BasicAccountPeriodPart>) accountPeriodPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicAccountPeriodPart getAccountPeriodPart(long id){
		return getAccountPeriodPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicAccountPeriodPart getAccountPeriodPart(long id, String viewCode){
		X6BasicAccountPeriodPart accountPeriodPart = accountPeriodPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getAccountPeriodPartAsJSON(long id, String include){
		X6BasicAccountPeriodPart accountPeriodPart = accountPeriodPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(accountPeriodPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.common.delete",operType="3")
	public void deleteAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart){
		if(null != accountPeriodPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAccountPeriodPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
			AuditUtil.setColumnStringA(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
			AuditUtil.setColumnStringC(accountPeriodPart.getId().toString());
		}
		beforeDeleteAccountPeriodPart(accountPeriodPart);
		accountPeriodPartDao.deletePhysical(accountPeriodPart);
		afterDeleteAccountPeriodPart(accountPeriodPart);
		accountPeriodPartDao.flush();
		List<X6BasicAccountPeriodPart> params = new ArrayList<X6BasicAccountPeriodPart>();
		params.add(accountPeriodPart);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.common.delete",operType="3")
	public void deleteAccountPeriodPart(List<Long> accountPeriodPartIds){
		deleteAccountPeriodPart(accountPeriodPartIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.common.delete",operType="3")
	public void deleteAccountPeriodPart(List<Long> accountPeriodPartIds, String eventTopic) {
		List<X6BasicAccountPeriodPart> accountPeriodParts = new ArrayList<X6BasicAccountPeriodPart>();
		for(Long accountPeriodPartId : accountPeriodPartIds){
			X6BasicAccountPeriodPart accountPeriodPart = getAccountPeriodPart(accountPeriodPartId);
			accountPeriodParts.add(accountPeriodPart);
			if(null != accountPeriodPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAccountPeriodPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == accountPeriodPart.getId() ? "" : "," + accountPeriodPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == accountPeriodPart.getId() ? "" : "," + accountPeriodPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == accountPeriodPart.getId() ? "" : "," + accountPeriodPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(accountPeriodParts != null && accountPeriodParts.size() > 0 ){
			for(X6BasicAccountPeriodPart accountPeriodPart : accountPeriodParts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = X6BasicAccountPeriodPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(accountPeriodPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),accountPeriodPart, accountPeriodPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(accountPeriodPartIds != null && accountPeriodPartIds.size() > 0) {
			String hql = "delete " + X6BasicAccountPeriodPart.JPA_NAME + " where id in(:ids)";
			Query query = accountPeriodPartDao.createQuery(hql);
			query.setParameterList("ids", accountPeriodPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicAccountPeriodPart accountPeriodPartz : accountPeriodParts){
				accountPeriodPartz.setValid(false);
				List<X6BasicAccountPeriodPart> params = new ArrayList<X6BasicAccountPeriodPart>();
				params.add(accountPeriodPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_accountingPeriod");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.common.delete",operType="3")
	public void deleteAccountPeriodPart(long accountPeriodPartId, int accountPeriodPartVersion){
		this.deleteAccountPeriodPart(accountPeriodPartId, accountPeriodPartVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.common.delete",operType="3")
	public void deleteAccountPeriodPart(long accountPeriodPartId, int accountPeriodPartVersion,SignatureLog signatureLog){
		X6BasicAccountPeriodPart accountPeriodPart = getAccountPeriodPart(accountPeriodPartId);
		...
		beforeDeleteAccountPeriodPart(accountPeriodPart);
		accountPeriodPartDao.delete(accountPeriodPartId, accountPeriodPartVersion);
		afterDeleteAccountPeriodPart(accountPeriodPart);
		accountPeriodPartDao.flush();
		List<X6BasicAccountPeriodPart> params = new ArrayList<X6BasicAccountPeriodPart>();
		params.add(accountPeriodPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=accountPeriodPart.getId();
			signatureLog.setTableId(accountPeriodPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.common.delete",operType="3")
	public void deleteAccountPeriodPart(String accountPeriodPartIds){
		this.deleteAccountPeriodPart(accountPeriodPartIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.common.delete",operType="3")
	public void deleteAccountPeriodPart(String accountPeriodPartIds,SignatureLog signatureLog){
		deleteCollection(accountPeriodPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteAccountPeriodPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_accountingPeriod#modelCode:X6Basic_1.0_accountingPeriod_AccountPeriodPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="ec.engine.restore",operType="10")
	public void restoreAccountPeriodPart(String accountPeriodPartIds){
		restoreCollection(accountPeriodPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreAccountPeriodPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="ec.engine.restore",operType="10")
	public void restoreAccountPeriodPart(long accountPeriodPartId){
		
		findBusinessKeyUsed(accountPeriodPartId);	//判断业务主键是否重复
		...
		beforeRestoreAccountPeriodPart(accountPeriodPart);
		accountPeriodPart.setValid(true);
		accountPeriodPartDao.update(accountPeriodPart);
		afterRestoreAccountPeriodPart(accountPeriodPart);
		List<X6BasicAccountPeriodPart> params = new ArrayList<X6BasicAccountPeriodPart>();
		params.add(accountPeriodPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long accountPeriodPartId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_accountingPeriod_AccountPeriodPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicAccountPeriodPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicAccountPeriodPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  accountPeriodPartDao.createNativeQuery(sql,accountPeriodPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseAccountPeriodPart(List<X6BasicAccountPeriodPart>  accountPeriodParts){
		for(X6BasicAccountPeriodPart accountPeriodPart:accountPeriodParts)  {
			saveAccountPeriodPart(accountPeriodPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseAccountPeriodPart(List<X6BasicAccountPeriodPart>  accountPeriodParts){
		for(X6BasicAccountPeriodPart accountPeriodPart:accountPeriodParts)  {
			saveAccountPeriodPart(accountPeriodPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="ec.flow.save")
	public void saveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveAccountPeriodPart(accountPeriodPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="ec.flow.save")
	public void saveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveAccountPeriodPart(accountPeriodPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateAccountPeriodPartCodes(X6BasicAccountPeriodPart accountPeriodPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAccountPeriodPartCodes(accountPeriodPart, false);
	}
	
	public void generateAccountPeriodPartCodes(X6BasicAccountPeriodPart accountPeriodPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateAccountPeriodPartSummarys(X6BasicAccountPeriodPart accountPeriodPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAccountPeriodPartSummarys(accountPeriodPart, false);
	}

	public void generateAccountPeriodPartSummarys(X6BasicAccountPeriodPart accountPeriodPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart")
	public void saveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveAccountPeriodPart(accountPeriodPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart")
	public void saveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "X6Basic_1.0_accountingPeriod");
		String url = null;
		if(accountPeriodPart.getId() != null && accountPeriodPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAccountPeriodPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, accountPeriodPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(accountPeriodPart);
		beforeSaveAccountPeriodPart(accountPeriodPart, viewIsView);

		if (viewIsView) {
			accountPeriodPartDao.saveWithRevertVersion(accountPeriodPart);
		} else {
			if(isNew)
				accountPeriodPartDao.save(accountPeriodPart);
			else
				accountPeriodPartDao.merge(accountPeriodPart);
		}
		if(null != accountPeriodPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAccountPeriodPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
			AuditUtil.setColumnStringA(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
		...
		dealDatagridsSave(accountPeriodPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateAccountPeriodPartCodes(accountPeriodPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateAccountPeriodPartSummarys(accountPeriodPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveAccountPeriodPart(accountPeriodPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, accountPeriodPart);
			}
		}
		List<X6BasicAccountPeriodPart> params = new ArrayList<X6BasicAccountPeriodPart>();
		accountPeriodPartDao.flush();
		accountPeriodPartDao.clear();
		accountPeriodPart = accountPeriodPartDao.load(accountPeriodPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicAccountPeriodPart accountPeriodPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("accountPeriodPart", accountPeriodPart);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart")
	public void mergeAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(accountPeriodPart);
		beforeSaveAccountPeriodPart(accountPeriodPart);
		accountPeriodPartDao.merge(accountPeriodPart);
		if(null != accountPeriodPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAccountPeriodPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
			AuditUtil.setColumnStringA(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
		...
		dealDatagridsSave(accountPeriodPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveAccountPeriodPart(accountPeriodPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicAccountPeriodPart> findAccountPeriodParts(Page<X6BasicAccountPeriodPart> page, Criterion... criterions) {
		return accountPeriodPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAccountPeriodParts(Page<X6BasicAccountPeriodPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findAccountPeriodParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAccountPeriodParts(Page<X6BasicAccountPeriodPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAccountPeriodParts(Page<X6BasicAccountPeriodPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicAccountPeriodPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"accountPeriodPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"accountPeriodPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"accountPeriodPart\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"accountPeriodPart\".ACCOUNT_PERIOD IN ( SELECT ID FROM X6BASIC_ACCOUNT_PERIODS \"accountPeriod\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "X6Basic_1.0_accountingPeriod" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  accountPeriodPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  accountPeriodPartDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_accountingPeriod").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"accountPeriod\"","X6Basic_1.0_accountingPeriod","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"accountPeriodPart\".LAY_REC ASC, \"accountPeriodPart\".SORT ASC");
				} else {
					orderPart.append(" \"accountPeriodPart\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) accountPeriodPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=accountPeriodPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = accountPeriodPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicAccountPeriodPart> result = new ArrayList<X6BasicAccountPeriodPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicAccountPeriodPart.class,accountPeriodPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicAccountPeriodPart.class,accountPeriodPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicAccountPeriodPart accountPeriodPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(accountPeriodPart.getId(), "X6Basic_accountingPeriod_accountPeriodPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(accountPeriodPart.getId(), "X6Basic_accountingPeriod_accountPeriodPart");
							if(null != documents && !documents.isEmpty()) {
								accountPeriodPart.setDocument(documents.get(0));
								accountPeriodPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicAccountPeriodPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicAccountPeriodPart> result) {
		View view = null;
		...
		Map<Long, X6BasicAccountPeriodPart> rsMap = new HashMap<Long, X6BasicAccountPeriodPart>();
		for (X6BasicAccountPeriodPart accountPeriodPart : result) {
			ids.add(accountPeriodPart.getId());
			rsMap.put(accountPeriodPart.getId(), accountPeriodPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(accountPeriodPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = accountPeriodPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_accountingPeriod_AccountPeriodPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="X6Basic_1.0_accountingPeriod_AccountPeriodPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  accountPeriodPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"accountPeriodPart\"","X6Basic_1.0_accountingPeriod","","X6BASIC_ACCOUNT_PERIOD_PARTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"accountPeriodPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"accountPeriodPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"accountPeriodPart\"","X6Basic_1.0_accountingPeriod","","X6BASIC_ACCOUNT_PERIOD_PARTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "X6Basic_1.0_accountingPeriod_AccountPeriodPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("accountPeriodPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"accountPeriodPart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"accountPeriodPart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"accountPeriodPart\".CID = 1 OR \"accountPeriodPart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("accountPeriodPart.mc\"").append(".ACCOUNT_PERIOD_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = accountPeriodPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=accountPeriodPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicAccountPeriodPart.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicAccountPeriodPart.MODULE_CODE;}
	protected String getModelCode(){return X6BasicAccountPeriodPart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param accountPeriodPart 模型实例
	 */
	public void checkUniqueConstraint(X6BasicAccountPeriodPart accountPeriodPart){
	}
	
	
	@Override
	public List<X6BasicAccountPeriodPart> getAccountPeriodParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicAccountPeriodPart.class);
		if(params!=null){
			...
			return accountPeriodPartDao.findByHql("from " + X6BasicAccountPeriodPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return accountPeriodPartDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicAccountPeriodPart> findAccountPeriodPartsByHql(String hql, Object... objects){
		...
		return   accountPeriodPartDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicAccountPeriodPart loadAccountPeriodPartByBussinessKey(X6BasicAccountPeriodPart accountPeriodPart){
		return loadAccountPeriodPartByBussinessKey(accountPeriodPart.getId());
	}
	
	@Override
	public Page<X6BasicAccountPeriodPart> getAccountPeriodParts(Page<X6BasicAccountPeriodPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicAccountPeriodPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = accountPeriodPartDao.findByPage(page, sql);
			}else{
				page = accountPeriodPartDao.findAllByPage(page);
			}
			return page;
		}
		page = accountPeriodPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicAccountPeriodPart> findByTreeDataGridAccountPeriod(List<X6BasicAccountPeriodPart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(X6BasicAccountPeriodPart accountPeriodPart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridAccountPeriod(Page<X6BasicAccountPeriodPart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<X6BasicAccountPeriodPart> accountPeriodParts = dgPage.getResult();
			for(X6BasicAccountPeriodPart accountPeriodPart : accountPeriodParts){
				
			}
			dgPage.setResult(accountPeriodParts);
		}
	...
	public X6BasicAccountPeriodPart loadAccountPeriodPartByBussinessKey(Serializable bussinessKey){
		return accountPeriodPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicAccountPeriodPart> findByProperty(String propertyName, Object object){
		return accountPeriodPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicAccountPeriodPart findEntityByProperty(String propertyName, Object object){
		return accountPeriodPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicAccountPeriodPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = accountPeriodPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_accountingPeriod_AccountPeriodPart","runtime");
		try {
	...
	public void dealDatagridsSave(X6BasicAccountPeriodPart accountPeriodPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,accountPeriodPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicAccountPeriodPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, accountPeriodPart);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",accountPeriodPart.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicAccountPeriodPart.TABLE_NAME +" where valid = 1";
			list = accountPeriodPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicAccountPeriodPart.TABLE_NAME;
			list = accountPeriodPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicAccountPeriodPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = accountPeriodPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicAccountPeriodPartDaoImpl accountPeriodPartDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchAccountPeriodPart(final List<X6BasicAccountPeriodPart> insertObjs, final List<X6BasicAccountPeriodPart> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchAccountPeriodPart(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchAccountPeriodPart(final List<X6BasicAccountPeriodPart> insertObjs, final List<X6BasicAccountPeriodPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1400468245576"),importCount));
		...
		Map<Long, X6BasicAccountPeriodPart> oldUpdateObjsMap = new HashMap<Long, X6BasicAccountPeriodPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicAccountPeriodPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicAccountPeriodPart> bizObjList = accountPeriodPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicAccountPeriodPart bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicAccountPeriodPart> nmeObjs = new ArrayList<X6BasicAccountPeriodPart>();
		...
		long id=autoGeneratorID.getNextId(X6BasicAccountPeriodPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicAccountPeriodPart accountPeriodPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicAccountPeriodPart.TABLE_NAME,1,dbId);
			if(accountPeriodPart.getId() != null ){
				m.put(accountPeriodPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) accountPeriodPartDao.createNativeQuery(sql, "X6Basic_1.0_accountingPeriod_AccountPeriodPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		accountPeriodPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicAccountPeriodPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicAccountPeriodPart accountPeriodPart = updateObjs.get(updateIndex);
						Long id = accountPeriodPart.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = accountPeriodPart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(accountPeriodPart);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicAccountPeriodPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicAccountPeriodPart.TABLE_NAME + " order by id desc";
//					Query query = accountPeriodPartDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicAccountPeriodPart accountPeriodPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = accountPeriodPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		accountPeriodPartDaoImpl.getSession().flush();
		accountPeriodPartDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicAccountPeriodPart bizObj = insertObjs.get(insertIndex);
			accountPeriodPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicAccountPeriodPart bizObj = updateObjs.get(updateIndex);
			X6BasicAccountPeriodPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			accountPeriodPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<X6BasicAccountPeriodPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<X6BasicAccountPeriodPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) accountPeriodPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<X6BasicAccountPeriodPart> result = getResult(page, query, new PendingResultTransformer(X6BasicAccountPeriodPart.class,accountPeriodPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(X6BasicAccountPeriodPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = accountPeriodPartDao.createNativeQuery(sql, "X6Basic_1.0_accountingPeriod_AccountPeriodPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = accountPeriodPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = accountPeriodPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = accountPeriodPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?accountPeriodPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  accountPeriodPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,accountPeriodPartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="ec.flow.save")
	public void saveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, DataGridService dataGridService){
		saveAccountPeriodPart(accountPeriodPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="ec.flow.save")
	public void saveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, DataGridService dataGridService, String viewCode){
		saveAccountPeriodPart(accountPeriodPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart")
	public void saveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveAccountPeriodPart(accountPeriodPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart")
	public void saveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_accountingPeriod";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_accountingPeriod");
		String url = null;
		if(null != accountPeriodPart.getId() && accountPeriodPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAccountPeriodPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart")
	public Map<Object, Object> accountPeriodPartDataGridImport(X6BasicAccountPeriodPart accountPeriodPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = accountPeriodPart.getVirtualId();
		
		accountPeriodPart = accountPeriodPartDao.load(accountPeriodPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, accountPeriodPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = accountPeriodPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(accountPeriodPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, accountPeriodPart.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart")
	public void mergeAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(accountPeriodPart);
		beforeSaveAccountPeriodPart(accountPeriodPart);
		accountPeriodPartDao.merge(accountPeriodPart);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_accountingPeriod",model="X6Basic_1.0_accountingPeriod_AccountPeriodPart",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicAccountPeriodPart> getByPage(Page<X6BasicAccountPeriodPart> page,DetachedCriteria detachedCriteria){
		return accountPeriodPartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, Object... objects){
		checkUniqueConstraint(accountPeriodPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
      if(accountPeriodPart.getId() != null){
			boolean canDelete = bapBaseService.canDelete(accountPeriodPart);
			if(!canDelete){
				String month = accountPeriodPart.getMonth().getValue();
				throw new BAPException(InternationalResource.get(month, getCurrentLanguage()) + "已被系统引用，无法删除，刷新后重试。");
			}
		}
/* CUSTOM CODE END */
	}
	
	private void afterDeleteAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicAccountPeriodPart accountPeriodPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicAccountPeriodPart accountPeriodPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportAccountPeriodPart(List<X6BasicAccountPeriodPart> insertObjs, List<X6BasicAccountPeriodPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportAccountPeriodPart,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportAccountPeriodPart(List<X6BasicAccountPeriodPart> insertObjs, List<X6BasicAccountPeriodPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportAccountPeriodPart,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicAccountPeriodPart> findByHql(String hql,Object...objects){
		return accountPeriodPartDao.findByHql(hql, objects);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicAccountPeriodPart findEntityByHql(String hql,Object...objects){
		return accountPeriodPartDao.findEntityByHql(hql, objects);
	}
	@Override
