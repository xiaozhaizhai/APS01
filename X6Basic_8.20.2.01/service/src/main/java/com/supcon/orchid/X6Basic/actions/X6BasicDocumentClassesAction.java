...
/* CUSTOM CODE START(action,import,X6Basic_1.0_documentClass_DocumentClasses,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicDocumentClassesAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicDocumentClasses> {
	...
	private X6BasicDocumentClasses documentClasses;
	...
	//导出excel时,主模型内容
	List<X6BasicDocumentClasses> queryResults = new ArrayList<X6BasicDocumentClasses>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/documentClass/documentClasses/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/documentClass/documentClasses/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_documentClass_DocumentClasses" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicDocumentClasses> documentClassess;
	
	public List<X6BasicDocumentClasses> getDocumentClassess() {
		return documentClassess;
	}
	...
	private Page<X6BasicDocumentClasses> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			documentClasses = new X6BasicDocumentClasses();
		} else {
			if(id > 0){
				documentClasses = documentClassesService.getDocumentClasses(id, viewCode);
			}
		}
	...
	public Page<X6BasicDocumentClasses> getPage() {
		if (null == page) {
			page = new Page<X6BasicDocumentClasses>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicDocumentClasses getDocumentClasses() {
		return documentClasses;
	}		

	public void setDocumentClasses(X6BasicDocumentClasses documentClasses) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/documentClass/documentClasses/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = documentClassesService.getDocumentClassesAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/documentClass/documentClasses/edit-extra-col")
	public String _editExtraCol() throws Exception{
		if(!isEditNew()){
			if(documentClasses.getExtraCol()!=null && documentClasses.getExtraCol().length() > 0) {
				Struts2Utils.renderXml(documentClasses.getExtraCol());
				return null;
			}
		}
	 ...
	 * @throws Exception
	 */
		public String documentClassesReferenceCopy() throws Exception{
			X6BasicDocumentClasses oldDocumentClasses = null;
			if(isEditNew()){
				...
				oldDocumentClasses = new X6BasicDocumentClasses();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,documentClasses.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, documentClasses.getId());
				...
				oldDocumentClasses = documentClassesService.getDocumentClasses(id);
			}
			
			
			documentClasses = documentClassesService.getDocumentClasses(refId);
			if (documentClasses.getExtraCol() != null && documentClasses.getExtraCol().length() > 0) {
				documentClasses.setExtraColMap(XmlUtils.xml2Map(documentClasses.getExtraCol()));
			}
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_documentClass_DocumentClasses");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicDocumentClasses.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldDocumentClasses, null);
						Method setValue = X6BasicDocumentClasses.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(documentClasses, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			documentClasses.setCreateDepartmentId(oldDocumentClasses.getCreateDepartmentId());
			documentClasses.setCreatePositionId(oldDocumentClasses.getCreatePositionId());
			documentClasses.setCreateStaffId(oldDocumentClasses.getCreateStaffId());
			documentClasses.setOwnerDepartmentId(oldDocumentClasses.getOwnerDepartmentId());
			documentClasses.setOwnerPositionId(oldDocumentClasses.getOwnerPositionId());
			documentClasses.setOwnerStaffId(oldDocumentClasses.getOwnerStaffId());
			documentClasses.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=documentClassesService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return documentClassesReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			documentClasses = documentClassesService.getDocumentClasses(this.documentClasses.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,documentClasses.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			documentClassesService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = documentClassesService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(documentClassesService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(documentClassesService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_documentClass_DocumentClasses", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"documentClasses\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"documentClasses\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"documentClasses\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"documentClasses\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			documentClassesService.excelExport();
			return "excel";
		} else {
	...
	public String viewTreeDrag() throws Exception{
		documentClassesService.drag(treecid, pid, cindex);
		return null;
	}
	...
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"documentClasses\".", "{alias}.");
					}
		...
		documentClassess = documentClassesService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			documentClassess = documentClassesService.getTreeInfo(treeInfo, null, null);
		}
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		documentClassesService.findDataGridPage(dg,dgclass,getDgPage(),documentClasses,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(documentClasses.getId() != null && documentClasses.getId() != -1){
				documentClasses = documentClassesService.getDocumentClasses(documentClasses.getId());
			}
			documentClassesService.findDataGridPage(dg,dgclass,getDgPage(),documentClasses,searchCondition,params);
	...
	private void save() throws Exception {
		...
		documentClassesService.saveDocumentClasses(documentClasses,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "submit");
		bap_validate_datagrids=documentClassesService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", documentClasses.getId());
		...
		fileuploads.put("linkId", documentClasses.getId());
		bapAfterCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/documentClass/documentClasses/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		documentClassesService.deleteDocumentClasses(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/documentClass/documentClasses/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "restore");
		documentClassesService.restoreDocumentClasses(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/documentClass/documentClasses/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/documentClass/documentClasses/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "showDealInfoData");
		if (null != tableInfoId) {
			int count=documentClassesService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = documentClassesService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = documentClassesService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicDocumentClasses> findExportDataInfos(Page<X6BasicDocumentClasses> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = documentClassesService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = documentClassesService.getAssProperty(p.getCode());	
									//String assModelCode = documentClassesService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			documentClassesService.findDocumentClassess(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			documentClassesService.findDocumentClassess(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicDocumentClasses> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicDocumentClasses result : results){
	...
	public boolean isEditNew(){
		//return null == this.documentClasses || null == this.documentClasses.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/documentClass/documentClasses/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/documentClass/documentClasses/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/documentClass/documentClasses/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/documentClass/documentClasses/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicDocumentClassesService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_documentClass", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_documentClass");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/documentClass/documentClasses/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_documentClass_DocumentClasses", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_documentClass_DocumentClasses", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/documentClass/documentClasses/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/documentClass/documentClasses/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_documentClass_DocumentClasses";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_documentClass_DocumentClasses", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/documentClass/documentClasses/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		X6BasicDocumentClasses documentClasses = null;
		if(null != id) {
			documentClasses = documentClassesService.getDocumentClasses(id);
		}
		...
		_auditLog.setColumnStringB(null == documentClasses.getCode() ? "" : documentClasses.getCode().toString());
		_auditLog.setColumnStringA(null == documentClasses.getName() ? "" : documentClasses.getName().toString());
		_auditLog.setColumnStringC(documentClasses.getId().toString());
		_auditLog.setColumnStringD(X6BasicDocumentClasses.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_documentClass");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicDocumentClassesAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, documentClasses,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, documentClasses,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,documentClasses);
		...
		printlog.setEntity("X6Basic_1.0_documentClass");
		...
		printlog.setBusinessId(documentClasses.getId());
		printlog.setBussinessKey(null == documentClasses.getCode() ? "" : documentClasses.getCode().toString());
		printlog.setMainDisplay(null == documentClasses.getName() ? "" : documentClasses.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/documentClass/documentClasses/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(documentClasses,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/documentClass/documentClasses/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(documentClassesService.getDocumentClasses(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/documentClass/documentClasses/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/documentClass/documentClasses/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, documentClasses,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				documentClasses = documentClassesService.getDocumentClasses(Long.parseLong(arrId[i]));
				if(null != documentClasses){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == documentClasses.getName() ? "" : "," + documentClasses.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == documentClasses.getName() ? "" : documentClasses.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == documentClasses.getCode() ? "" : "," + documentClasses.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == documentClasses.getCode() ? "" : documentClasses.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == documentClasses.getId() ? "" : "," + documentClasses.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == documentClasses.getId() ? "" : documentClasses.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, documentClasses,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,documentClasses);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//documentClassesService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/X6Basic/documentClass/documentClasses/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicDocumentClasses> findExportPageInfos(Page<X6BasicDocumentClasses> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_documentClass" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = documentClassesService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicDocumentClasses queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = documentClassesService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = documentClassesService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_documentClass_DocumentClasses");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_documentClass_DocumentClasses")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_documentClass_DocumentClasses");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_documentClass_DocumentClasses");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = documentClassesService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_documentClass_DocumentClasses,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_documentClass_DocumentClasses,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_documentClass_DocumentClasses,X6Basic_1.0) */
// 自定义代码
