...
import com.supcon.orchid.X6Basic.services.X6BasicPlanDailyImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicPlanDailyDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_planDailyService")
@Transactional
public class X6BasicPlanDailyServiceImpl  extends BaseServiceImpl<X6BasicPlanDaily> implements X6BasicPlanDailyService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicPlanDaily> bapBaseService;
	...
	@Autowired
	private X6BasicPlanDailyImportService planDailyImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicPlanDaily.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  planDailyDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicPlanDaily.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = planDailyDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicPlanDaily.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicPlanDaily.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  planDailyDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicPlanDaily.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicPlanDaily.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicPlanDaily> list = (List<X6BasicPlanDaily>) planDailyDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicPlanDaily getPlanDaily(long id){
		return getPlanDaily(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicPlanDaily getPlanDaily(long id, String viewCode){
		X6BasicPlanDaily planDaily = planDailyDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPlanDailyAsJSON(long id, String include){
		X6BasicPlanDaily planDaily = planDailyDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(planDaily, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.common.delete",operType="3")
	public void deletePlanDaily(X6BasicPlanDaily planDaily){
		if(null != planDaily && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicPlanDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == planDaily.getId() ? "" : planDaily.getId().toString());
			AuditUtil.setColumnStringA(null == planDaily.getId() ? "" : planDaily.getId().toString());
			AuditUtil.setColumnStringC(planDaily.getId().toString());
		}
		beforeDeletePlanDaily(planDaily);
		planDailyDao.deletePhysical(planDaily);
		afterDeletePlanDaily(planDaily);
		planDailyDao.flush();
		List<X6BasicPlanDaily> params = new ArrayList<X6BasicPlanDaily>();
		params.add(planDaily);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.common.delete",operType="3")
	public void deletePlanDaily(List<Long> planDailyIds){
		deletePlanDaily(planDailyIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.common.delete",operType="3")
	public void deletePlanDaily(List<Long> planDailyIds, String eventTopic) {
		List<X6BasicPlanDaily> planDailys = new ArrayList<X6BasicPlanDaily>();
		for(Long planDailyId : planDailyIds){
			X6BasicPlanDaily planDaily = getPlanDaily(planDailyId);
			planDailys.add(planDaily);
			if(null != planDaily && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicPlanDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == planDaily.getId() ? "" : "," + planDaily.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == planDaily.getId() ? "" : planDaily.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == planDaily.getId() ? "" : "," + planDaily.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == planDaily.getId() ? "" : planDaily.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == planDaily.getId() ? "" : "," + planDaily.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == planDaily.getId() ? "" : planDaily.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(planDailys != null && planDailys.size() > 0 ){
			for(X6BasicPlanDaily planDaily : planDailys){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = X6BasicPlanDaily.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(planDaily));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),planDaily, planDaily.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(planDailyIds != null && planDailyIds.size() > 0) {
			String hql = "delete " + X6BasicPlanDaily.JPA_NAME + " where id in(:ids)";
			Query query = planDailyDao.createQuery(hql);
			query.setParameterList("ids", planDailyIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicPlanDaily planDailyz : planDailys){
				planDailyz.setValid(false);
				List<X6BasicPlanDaily> params = new ArrayList<X6BasicPlanDaily>();
				params.add(planDailyz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_rangePlan");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.common.delete",operType="3")
	public void deletePlanDaily(long planDailyId, int planDailyVersion){
		this.deletePlanDaily(planDailyId, planDailyVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.common.delete",operType="3")
	public void deletePlanDaily(long planDailyId, int planDailyVersion,SignatureLog signatureLog){
		X6BasicPlanDaily planDaily = getPlanDaily(planDailyId);
		...
		beforeDeletePlanDaily(planDaily);
		planDailyDao.delete(planDailyId, planDailyVersion);
		afterDeletePlanDaily(planDaily);
		planDailyDao.flush();
		List<X6BasicPlanDaily> params = new ArrayList<X6BasicPlanDaily>();
		params.add(planDaily);
		...
		if(signatureLog != null ) {
			...
			businessKey=planDaily.getId();
			signatureLog.setTableId(planDaily.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.common.delete",operType="3")
	public void deletePlanDaily(String planDailyIds){
		this.deletePlanDaily(planDailyIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.common.delete",operType="3")
	public void deletePlanDaily(String planDailyIds,SignatureLog signatureLog){
		deleteCollection(planDailyIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePlanDaily(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_rangePlan#modelCode:X6Basic_1.0_rangePlan_PlanDaily#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="ec.engine.restore",operType="10")
	public void restorePlanDaily(String planDailyIds){
		restoreCollection(planDailyIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePlanDaily(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="ec.engine.restore",operType="10")
	public void restorePlanDaily(long planDailyId){
		
		findBusinessKeyUsed(planDailyId);	//判断业务主键是否重复
		...
		beforeRestorePlanDaily(planDaily);
		planDaily.setValid(true);
		planDailyDao.update(planDaily);
		afterRestorePlanDaily(planDaily);
		List<X6BasicPlanDaily> params = new ArrayList<X6BasicPlanDaily>();
		params.add(planDaily);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long planDailyId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_rangePlan_PlanDaily");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicPlanDaily.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicPlanDaily.TABLE_NAME +" where id = ? )";
			List<Object> list =  planDailyDao.createNativeQuery(sql,planDailyId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePlanDaily(List<X6BasicPlanDaily>  planDailys){
		for(X6BasicPlanDaily planDaily:planDailys)  {
			savePlanDaily(planDaily, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePlanDaily(List<X6BasicPlanDaily>  planDailys){
		for(X6BasicPlanDaily planDaily:planDailys)  {
			savePlanDaily(planDaily, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="ec.flow.save")
	public void savePlanDaily(X6BasicPlanDaily planDaily, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePlanDaily(planDaily, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="ec.flow.save")
	public void savePlanDaily(X6BasicPlanDaily planDaily, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePlanDaily(planDaily, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePlanDailyCodes(X6BasicPlanDaily planDaily) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePlanDailyCodes(planDaily, false);
	}
	
	public void generatePlanDailyCodes(X6BasicPlanDaily planDaily, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePlanDailySummarys(X6BasicPlanDaily planDaily) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePlanDailySummarys(planDaily, false);
	}

	public void generatePlanDailySummarys(X6BasicPlanDaily planDaily, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily")
	public void savePlanDaily(X6BasicPlanDaily planDaily,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePlanDaily(planDaily, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily")
	public void savePlanDaily(X6BasicPlanDaily planDaily,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "X6Basic_1.0_rangePlan");
		String url = null;
		if(planDaily.getId() != null && planDaily.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicPlanDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, planDaily);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(planDaily);
		beforeSavePlanDaily(planDaily, viewIsView);

		if (viewIsView) {
			planDailyDao.saveWithRevertVersion(planDaily);
		} else {
			if(isNew)
				planDailyDao.save(planDaily);
			else
				planDailyDao.merge(planDaily);
		}
		if(null != planDaily && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicPlanDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == planDaily.getId() ? "" : planDaily.getId().toString());
			AuditUtil.setColumnStringA(null == planDaily.getId() ? "" : planDaily.getId().toString());
		...
		dealDatagridsSave(planDaily,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePlanDailyCodes(planDaily, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePlanDailySummarys(planDaily, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePlanDaily(planDaily, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, planDaily);
			}
		}
		List<X6BasicPlanDaily> params = new ArrayList<X6BasicPlanDaily>();
		planDailyDao.flush();
		planDailyDao.clear();
		planDaily = planDailyDao.load(planDaily.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicPlanDaily planDaily){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("planDaily", planDaily);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily")
	public void mergePlanDaily(X6BasicPlanDaily planDaily, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(planDaily);
		beforeSavePlanDaily(planDaily);
		planDailyDao.merge(planDaily);
		if(null != planDaily && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicPlanDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == planDaily.getId() ? "" : planDaily.getId().toString());
			AuditUtil.setColumnStringA(null == planDaily.getId() ? "" : planDaily.getId().toString());
		...
		dealDatagridsSave(planDaily,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePlanDaily(planDaily);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicPlanDaily> findPlanDailys(Page<X6BasicPlanDaily> page, Criterion... criterions) {
		return planDailyDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPlanDailys(Page<X6BasicPlanDaily> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPlanDailys(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPlanDailys(Page<X6BasicPlanDaily> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPlanDailys(Page<X6BasicPlanDaily> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicPlanDaily> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"planDaily\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							...
							s.append(" AND \"planDaily\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"planDaily\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"planDaily\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"planDaily\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  planDailyDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  planDailyDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_rangePlan").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"planDaily\".LAY_REC ASC, \"planDaily\".SORT ASC");
				} else {
					orderPart.append(" \"planDaily\".RPD_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) planDailyDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=planDailyDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = planDailyDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicPlanDaily> result = new ArrayList<X6BasicPlanDaily>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicPlanDaily.class,planDailyDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicPlanDaily.class,planDailyDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicPlanDaily planDaily : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(planDaily.getId(), "X6Basic_rangePlan_planDaily");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(planDaily.getId(), "X6Basic_rangePlan_planDaily");
							if(null != documents && !documents.isEmpty()) {
								planDaily.setDocument(documents.get(0));
								planDaily.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicPlanDaily> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicPlanDaily> result) {
		View view = null;
		...
		Map<Long, X6BasicPlanDaily> rsMap = new HashMap<Long, X6BasicPlanDaily>();
		for (X6BasicPlanDaily planDaily : result) {
			ids.add(planDaily.getId());
			rsMap.put(planDaily.getId(), planDaily);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(planDailyDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = planDailyDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_rangePlan_PlanDaily" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="X6Basic_1.0_rangePlan_PlanDaily" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  planDailyDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"planDaily\"","X6Basic_1.0_rangePlan","","S2BASE_RANGEPLANDAILY",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"planDaily\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"planDaily\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"planDaily\"","X6Basic_1.0_rangePlan","","S2BASE_RANGEPLANDAILY",true);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "X6Basic_1.0_rangePlan_PlanDaily" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("planDaily.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"planDaily\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"planDaily\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"planDaily\".CID = 1 OR \"planDaily\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("planDaily.mc\"").append(".PLAN_DAILY) not in ("+notinValue+")");
			}
			
			SQLQuery query = planDailyDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=planDailyDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicPlanDaily.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicPlanDaily.MODULE_CODE;}
	protected String getModelCode(){return X6BasicPlanDaily.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param planDaily 模型实例
	 */
	public void checkUniqueConstraint(X6BasicPlanDaily planDaily){
	}
	
	
	@Override
	public List<X6BasicPlanDaily> getPlanDailys(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicPlanDaily.class);
		if(params!=null){
			...
			return planDailyDao.findByHql("from " + X6BasicPlanDaily.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return planDailyDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicPlanDaily> findPlanDailysByHql(String hql, Object... objects){
		...
		return   planDailyDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicPlanDaily loadPlanDailyByBussinessKey(X6BasicPlanDaily planDaily){
		return loadPlanDailyByBussinessKey(planDaily.getId());
	}
	
	@Override
	public Page<X6BasicPlanDaily> getPlanDailys(Page<X6BasicPlanDaily> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicPlanDaily.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = planDailyDao.findByPage(page, sql);
			}else{
				page = planDailyDao.findAllByPage(page);
			}
			return page;
		}
		page = planDailyDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deletePlanDailyByRangePlanPart(List<Long> rangePlanPartIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("rangePlanPartList", rangePlanPartIds);
		String hql = "select id from X6BasicPlanDaily where "
						+ "rangePlanPart.id in (:rangePlanPartList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> planDailyIds1 = planDailyDao.findByHql(hql, params);
		if(planDailyIds1 != null && !planDailyIds1.isEmpty()) {
		}
		hql = "delete X6BasicPlanDaily where "
						+ "rangePlanPart.id in (:rangePlanPartList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		planDailyDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicPlanDaily loadPlanDailyByBussinessKey(Serializable bussinessKey){
		return planDailyDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicPlanDaily> findByProperty(String propertyName, Object object){
		return planDailyDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicPlanDaily findEntityByProperty(String propertyName, Object object){
		return planDailyDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicPlanDaily.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = planDailyDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_rangePlan_PlanDaily","runtime");
		try {
	...
	public void dealDatagridsSave(X6BasicPlanDaily planDaily,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,planDailyDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicPlanDaily.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, planDaily);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",planDaily.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicPlanDaily.TABLE_NAME +" where valid = 1";
			list = planDailyDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicPlanDaily.TABLE_NAME;
			list = planDailyDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicPlanDaily.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = planDailyDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicPlanDailyDaoImpl planDailyDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPlanDaily(final List<X6BasicPlanDaily> insertObjs, final List<X6BasicPlanDaily> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPlanDaily(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPlanDaily(final List<X6BasicPlanDaily> insertObjs, final List<X6BasicPlanDaily> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1398310135469"),importCount));
		...
		Map<Long, X6BasicPlanDaily> oldUpdateObjsMap = new HashMap<Long, X6BasicPlanDaily>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicPlanDaily bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicPlanDaily> bizObjList = planDailyDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicPlanDaily bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicPlanDaily> nmeObjs = new ArrayList<X6BasicPlanDaily>();
		...
		long id=autoGeneratorID.getNextId(X6BasicPlanDaily.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicPlanDaily planDaily:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicPlanDaily.TABLE_NAME,1,dbId);
			if(planDaily.getId() != null ){
				m.put(planDaily.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) planDailyDao.createNativeQuery(sql, "X6Basic_1.0_rangePlan_PlanDaily"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		planDailyDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicPlanDaily.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicPlanDaily planDaily = updateObjs.get(updateIndex);
						Long id = planDaily.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = planDaily.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicPlanDaily.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicPlanDaily.TABLE_NAME + " order by id desc";
//					Query query = planDailyDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicPlanDaily planDaily = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = planDaily.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		planDailyDaoImpl.getSession().flush();
		planDailyDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicPlanDaily bizObj = insertObjs.get(insertIndex);
			planDailyImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicPlanDaily bizObj = updateObjs.get(updateIndex);
			X6BasicPlanDaily oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			planDailyImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<X6BasicPlanDaily> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<X6BasicPlanDaily> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) planDailyDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<X6BasicPlanDaily> result = getResult(page, query, new PendingResultTransformer(X6BasicPlanDaily.class,planDailyDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(X6BasicPlanDaily model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = planDailyDao.createNativeQuery(sql, "X6Basic_1.0_rangePlan_PlanDaily").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = planDailyDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = planDailyDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = planDailyDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?planDailyDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  planDailyDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,planDailyDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="ec.flow.save")
	public void savePlanDaily(X6BasicPlanDaily planDaily, DataGridService dataGridService){
		savePlanDaily(planDaily, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="ec.flow.save")
	public void savePlanDaily(X6BasicPlanDaily planDaily, DataGridService dataGridService, String viewCode){
		savePlanDaily(planDaily, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily")
	public void savePlanDaily(X6BasicPlanDaily planDaily, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePlanDaily(planDaily, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily")
	public void savePlanDaily(X6BasicPlanDaily planDaily, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_rangePlan";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_rangePlan");
		String url = null;
		if(null != planDaily.getId() && planDaily.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicPlanDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily")
	public Map<Object, Object> planDailyDataGridImport(X6BasicPlanDaily planDaily, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = planDaily.getVirtualId();
		
		planDaily = planDailyDao.load(planDaily.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, planDaily.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = planDaily.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(planDaily);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, planDaily.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily")
	public void mergePlanDaily(X6BasicPlanDaily planDaily, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(planDaily);
		beforeSavePlanDaily(planDaily);
		planDailyDao.merge(planDaily);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_PlanDaily",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicPlanDaily> getByPage(Page<X6BasicPlanDaily> page,DetachedCriteria detachedCriteria){
		return planDailyDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSavePlanDaily(X6BasicPlanDaily planDaily, Object... objects){
		checkUniqueConstraint(planDaily);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePlanDaily(X6BasicPlanDaily planDaily, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePlanDaily(X6BasicPlanDaily planDaily){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePlanDaily(X6BasicPlanDaily planDaily){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePlanDaily(X6BasicPlanDaily planDaily){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePlanDaily(X6BasicPlanDaily planDaily){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicPlanDaily planDaily, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicPlanDaily planDaily, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportPlanDaily(List<X6BasicPlanDaily> insertObjs, List<X6BasicPlanDaily> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPlanDaily,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportPlanDaily(List<X6BasicPlanDaily> insertObjs, List<X6BasicPlanDaily> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPlanDaily,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_rangePlan_PlanDaily,X6Basic_1.0) */
// 自定义代码
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicPlanDaily> findByHql(String hql,Object...objects){
		return planDailyDao.findByHql(hql, objects);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicPlanDaily findEntityByHql(String hql,Object...objects){
		return planDailyDao.findEntityByHql(hql, objects);
	}
/* CUSTOM CODE END */
}
