...
import com.supcon.orchid.X6Basic.services.AttMachineImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.AttMachineDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_attMachineService")
@Transactional
public class AttMachineServiceImpl  extends BaseServiceImpl<AttMachine> implements AttMachineService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<AttMachine> bapBaseService;
	...
	@Autowired
	private AttMachineImportService attMachineImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + AttMachine.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  attMachineDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + AttMachine.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = attMachineDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + AttMachine.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + AttMachine.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  attMachineDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + AttMachine.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + AttMachine.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<AttMachine> list = (List<AttMachine>) attMachineDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public AttMachine getAttMachine(long id){
		return getAttMachine(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public AttMachine getAttMachine(long id, String viewCode){
		AttMachine attMachine = attMachineDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getAttMachineAsJSON(long id, String include){
		AttMachine attMachine = attMachineDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(attMachine, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.common.delete",operType="3")
	public void deleteAttMachine(AttMachine attMachine){
		if(null != attMachine && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || AttMachine.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == attMachine.getCode() ? "" : attMachine.getCode().toString());
			AuditUtil.setColumnStringA(null == attMachine.getName() ? "" : attMachine.getName().toString());
			AuditUtil.setColumnStringC(attMachine.getId().toString());
		}
		beforeDeleteAttMachine(attMachine);
		attMachineDao.delete(attMachine);
		afterDeleteAttMachine(attMachine);
		attMachineDao.flush();
		List<AttMachine> params = new ArrayList<AttMachine>();
		params.add(attMachine);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_attMachine");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.common.delete",operType="3")
	public void deleteAttMachine(List<Long> attMachineIds){
		deleteAttMachine(attMachineIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.common.delete",operType="3")
	public void deleteAttMachine(List<Long> attMachineIds, String eventTopic) {
		List<AttMachine> attMachines = new ArrayList<AttMachine>();
		for(Long attMachineId : attMachineIds){
			AttMachine attMachine = getAttMachine(attMachineId);
			attMachines.add(attMachine);
			if(null != attMachine && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || AttMachine.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == attMachine.getName() ? "" : "," + attMachine.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == attMachine.getName() ? "" : attMachine.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == attMachine.getCode() ? "" : "," + attMachine.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == attMachine.getCode() ? "" : attMachine.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == attMachine.getId() ? "" : "," + attMachine.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == attMachine.getId() ? "" : attMachine.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(attMachines != null && attMachines.size() > 0 ){
			for(AttMachine attMachine : attMachines){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),attMachine, attMachine.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(attMachineIds != null && attMachineIds.size() > 0) {
			String hql = "update " + AttMachine.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = attMachineDao.createQuery(hql);
			query.setParameterList("ids", attMachineIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(AttMachine attMachinez : attMachines){
				attMachinez.setValid(false);
				List<AttMachine> params = new ArrayList<AttMachine>();
				params.add(attMachinez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_attMachine");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.common.delete",operType="3")
	public void deleteAttMachine(long attMachineId, int attMachineVersion){
		this.deleteAttMachine(attMachineId, attMachineVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.common.delete",operType="3")
	public void deleteAttMachine(long attMachineId, int attMachineVersion,SignatureLog signatureLog){
		AttMachine attMachine = getAttMachine(attMachineId);
		if(attMachine != null && !attMachine.isValid()){
		...
		beforeDeleteAttMachine(attMachine);
		attMachineDao.delete(attMachineId, attMachineVersion);
		afterDeleteAttMachine(attMachine);
		attMachineDao.flush();
		List<AttMachine> params = new ArrayList<AttMachine>();
		params.add(attMachine);
		...
		if(signatureLog != null ) {
			...
			businessKey=attMachine.getCode();
			signatureLog.setTableId(attMachine.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.common.delete",operType="3")
	public void deleteAttMachine(String attMachineIds){
		this.deleteAttMachine(attMachineIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.common.delete",operType="3")
	public void deleteAttMachine(String attMachineIds,SignatureLog signatureLog){
		deleteCollection(attMachineIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteAttMachine(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_attMachine#modelCode:X6Basic_1.0_attMachine_AttMachine#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="ec.engine.restore",operType="10")
	public void restoreAttMachine(String attMachineIds){
		restoreCollection(attMachineIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreAttMachine(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="ec.engine.restore",operType="10")
	public void restoreAttMachine(long attMachineId){
		
		findBusinessKeyUsed(attMachineId);	//判断业务主键是否重复
		...
		beforeRestoreAttMachine(attMachine);
		attMachine.setValid(true);
		attMachineDao.update(attMachine);
		afterRestoreAttMachine(attMachine);
		List<AttMachine> params = new ArrayList<AttMachine>();
		params.add(attMachine);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long attMachineId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_attMachine_AttMachine");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + AttMachine.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ AttMachine.TABLE_NAME +" where id = ? )";
			List<Object> list =  attMachineDao.createNativeQuery(sql,attMachineId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseAttMachine(List<AttMachine>  attMachines){
		for(AttMachine attMachine:attMachines)  {
			saveAttMachine(attMachine, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseAttMachine(List<AttMachine>  attMachines){
		for(AttMachine attMachine:attMachines)  {
			saveAttMachine(attMachine, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="ec.flow.save")
	public void saveAttMachine(AttMachine attMachine, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveAttMachine(attMachine, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="ec.flow.save")
	public void saveAttMachine(AttMachine attMachine, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveAttMachine(attMachine, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateAttMachineCodes(AttMachine attMachine) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAttMachineCodes(attMachine, false);
	}
	
	public void generateAttMachineCodes(AttMachine attMachine, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateAttMachineSummarys(AttMachine attMachine) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAttMachineSummarys(attMachine, false);
	}

	public void generateAttMachineSummarys(AttMachine attMachine, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine")
	public void saveAttMachine(AttMachine attMachine,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveAttMachine(attMachine, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine")
	public void saveAttMachine(AttMachine attMachine,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_attMachine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_attMachine");
		String url = null;
		if(attMachine.getId() != null && attMachine.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || AttMachine.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_attMachine/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || AttMachine.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_attMachine/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, attMachine);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(attMachine);
		beforeSaveAttMachine(attMachine, viewIsView);

		if (viewIsView) {
			attMachineDao.saveWithRevertVersion(attMachine);
		} else {
			if(isNew)
				attMachineDao.save(attMachine);
			else
				attMachineDao.merge(attMachine);
		}
		if(null != attMachine && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || AttMachine.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == attMachine.getCode() ? "" : attMachine.getCode().toString());
			AuditUtil.setColumnStringA(null == attMachine.getName() ? "" : attMachine.getName().toString());
		...
		dealDatagridsSave(attMachine,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateAttMachineCodes(attMachine, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateAttMachineSummarys(attMachine, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveAttMachine(attMachine, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, attMachine);
			}
		}
		List<AttMachine> params = new ArrayList<AttMachine>();
		attMachineDao.flush();
		attMachineDao.clear();
		attMachine = attMachineDao.load(attMachine.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, AttMachine attMachine){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("attMachine", attMachine);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine")
	public void mergeAttMachine(AttMachine attMachine, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(attMachine);
		beforeSaveAttMachine(attMachine);
		attMachineDao.merge(attMachine);
		if(null != attMachine && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || AttMachine.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == attMachine.getCode() ? "" : attMachine.getCode().toString());
			AuditUtil.setColumnStringA(null == attMachine.getName() ? "" : attMachine.getName().toString());
		...
		dealDatagridsSave(attMachine,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveAttMachine(attMachine);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<AttMachine> findAttMachines(Page<AttMachine> page, Criterion... criterions) {
		return attMachineDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private AttMachineDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<AttMachineDealInfo> list=attMachineDao.createQuery("from " + AttMachineDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(AttMachineDealInfo dealInfo) {
		attMachineDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+AttMachineDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) attMachineDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+AttMachineDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = attMachineDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = attMachineDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		List<Object[]> groupList = attMachineDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAttMachines(Page<AttMachine> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findAttMachines(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAttMachines(Page<AttMachine> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAttMachines(Page<AttMachine> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<AttMachine> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"attMachine\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"attMachine\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"attMachine\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_attMachine_AttMachine" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_attMachine_AttMachine" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  attMachineDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  attMachineDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_attMachine").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"attMachine\"","X6Basic_1.0_attMachine",customCondition,"X6BASIC_ATT_MACHINES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"attMachine\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+AttMachineDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"attMachine\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"attMachine\".LAY_REC ASC, \"attMachine\".SORT ASC");
				} else {
					orderPart.append(" \"attMachine\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) attMachineDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=attMachineDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = attMachineDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<AttMachine> result = new ArrayList<AttMachine>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(AttMachine.class,attMachineDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(AttMachine.class,attMachineDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (AttMachine attMachine : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(attMachine.getId(), "X6Basic_attMachine_attMachine");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(attMachine.getId(), "X6Basic_attMachine_attMachine");
							if(null != documents && !documents.isEmpty()) {
								attMachine.setDocument(documents.get(0));
								attMachine.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_attMachine")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_attMachine")+"X6Basic_1.0_attMachine".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_attMachine",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<AttMachine> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<AttMachine> result) {
		View view = null;
		...
		Map<Long, AttMachine> rsMap = new HashMap<Long, AttMachine>();
		for (AttMachine attMachine : result) {
			ids.add(attMachine.getId());
			rsMap.put(attMachine.getId(), attMachine);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(attMachineDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = attMachineDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(AttMachine attMachine,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(attMachine.getOwnerStaffId());
		ti.setOwnerPositionId(attMachine.getOwnerPositionId());
		ti.setOwnerDepartmentId(attMachine.getOwnerDepartmentId());
		
		ti.setCreateStaff(attMachine.getCreateStaff());
		ti.setCreateStaffId(attMachine.getCreateStaffId());
		ti.setCreateTime(attMachine.getCreateTime());
		ti.setDeleteStaff(attMachine.getDeleteStaff());
		ti.setDeleteStaffId(attMachine.getDeleteStaffId());
		ti.setDeleteTime(attMachine.getDeleteTime());
		ti.setCreateDepartmentId(attMachine.getCreateDepartmentId());
		ti.setModifyStaff(attMachine.getModifyStaff());
		ti.setModifyStaffId(attMachine.getModifyStaffId());
		ti.setCreatePositionId(attMachine.getCreatePositionId());
		ti.setPositionLayRec(attMachine.getPositionLayRec());
		ti.setTableNo(attMachine.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_attMachine");
		ti.setTargetTableName("X6BASIC_ATT_MACHINES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_attMachine_AttMachine" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  attMachineDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"attMachine\"","X6Basic_1.0_attMachine","","X6BASIC_ATT_MACHINES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"attMachine\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"attMachine\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"attMachine\"","X6Basic_1.0_attMachine","","X6BASIC_ATT_MACHINES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("attMachine.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"attMachine\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"attMachine\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"attMachine\".CID = 1 OR \"attMachine\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("attMachine.mc\"").append(".ATT_MACHINE) not in ("+notinValue+")");
			}
			
			SQLQuery query = attMachineDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("attMachine", "X6Basic_1.0_attMachine", "attMachine_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=attMachineDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return AttMachine.ENTITY_CODE;}
	protected String getModuleCode(){return AttMachine.MODULE_CODE;}
	protected String getModelCode(){return AttMachine.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param attMachine 模型实例
	 */
	public void checkUniqueConstraint(AttMachine attMachine){
		if(attMachine.getId()==null){
			List<AttMachine> attMachines = attMachineDao.findByCriteria(Restrictions.eq("code", attMachine.getCode()), Restrictions.eq("valid", true));
			if(attMachines!=null && !attMachines.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1367138903063", getCurrentUser().getLanguage()), attMachine.getCode());
			}
		} else {
			List<AttMachine> attMachines = attMachineDao.findByCriteria(Restrictions.ne("id", attMachine.getId()), Restrictions.eq("code", attMachine.getCode()), Restrictions.eq("valid", true));
			if(attMachines!=null && !attMachines.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1367138903063", getCurrentUser().getLanguage()), attMachine.getCode());
			}
	...
	@Override
	public List<AttMachine> getAttMachines(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(AttMachine.class);
		if(params!=null){
			...
			return attMachineDao.findByHql("from " + AttMachine.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return attMachineDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<AttMachine> findAttMachinesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(AttMachine.class);
	...
	@Override
	public List<AttMachine> findAttMachinesByHql(String hql, Object... objects){
		
		return   attMachineDao.findByHql(hql, objects);
	}
	
	@Override
	public AttMachine loadAttMachineByBussinessKey(AttMachine attMachine){
		return loadAttMachineByBussinessKey(attMachine.getCode());
	}
	
	@Override
	public Page<AttMachine> getAttMachines(Page<AttMachine> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(AttMachine.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = attMachineDao.findByPage(page, sql);
			}else{
				page = attMachineDao.findAllByPage(page);
			}
			return page;
		}
		page = attMachineDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public AttMachine loadAttMachineByBussinessKey(Serializable bussinessKey){
		return attMachineDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<AttMachine> findByProperty(String propertyName, Object object){
		return attMachineDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public AttMachine findEntityByProperty(String propertyName, Object object){
		return attMachineDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + AttMachine.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = attMachineDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_attMachine_AttMachine","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_attMachine", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(AttMachine attMachine,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,attMachineDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), AttMachine.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, attMachine);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",attMachine.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ AttMachine.TABLE_NAME +" where valid = 1";
			list = attMachineDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ AttMachine.TABLE_NAME;
			list = attMachineDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ AttMachine.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = attMachineDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private AttMachineDaoImpl attMachineDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchAttMachine(final List<AttMachine> insertObjs, final List<AttMachine> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchAttMachine(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchAttMachine(final List<AttMachine> insertObjs, final List<AttMachine> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1367137930784"),importCount));
		...
		Map<Long, AttMachine> oldUpdateObjsMap = new HashMap<Long, AttMachine>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			AttMachine bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<AttMachine> bizObjList = attMachineDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					AttMachine bizObj = bizObjList.get(mIndex);
		...
		final List<AttMachine> nmeObjs = new ArrayList<AttMachine>();
		...
		long id=autoGeneratorID.getNextId(AttMachine.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(AttMachine attMachine:insertObjs)  {
			//id = autoGeneratorID.getNextId(AttMachine.TABLE_NAME,1,dbId);
			if(attMachine.getCode() != null && !attMachine.getCode().equals("") ){
				m.put(attMachine.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) attMachineDao.createNativeQuery(sql, "X6Basic_1.0_attMachine_AttMachine"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		attMachineDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + AttMachine.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						AttMachine attMachine = updateObjs.get(updateIndex);
						Long id = attMachine.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = attMachine.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + AttMachine.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + AttMachine.TABLE_NAME + " order by id desc";
//					Query query = attMachineDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						AttMachine attMachine = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = attMachine.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		attMachineDaoImpl.getSession().flush();
		attMachineDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			AttMachine bizObj = insertObjs.get(insertIndex);
			attMachineImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			AttMachine bizObj = updateObjs.get(updateIndex);
			AttMachine oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			attMachineImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = attMachineDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				attMachineDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = attMachineDao.createNativeQuery(sql, "X6Basic_1.0_attMachine_AttMachine").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = attMachineDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = attMachineDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = attMachineDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?attMachineDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  attMachineDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,attMachineDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="ec.flow.save")
	public void saveAttMachine(AttMachine attMachine, DataGridService dataGridService){
		saveAttMachine(attMachine, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="ec.flow.save")
	public void saveAttMachine(AttMachine attMachine, DataGridService dataGridService, String viewCode){
		saveAttMachine(attMachine, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine")
	public void saveAttMachine(AttMachine attMachine, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveAttMachine(attMachine, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine")
	public void saveAttMachine(AttMachine attMachine, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_attMachine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_attMachine");
		String url = null;
		if(null != attMachine.getId() && attMachine.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || AttMachine.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine")
	public Map<Object, Object> attMachineDataGridImport(AttMachine attMachine, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = attMachine.getVirtualId();
		
		attMachine = attMachineDao.load(attMachine.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, attMachine.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = attMachine.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(attMachine);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, attMachine.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine")
	public void mergeAttMachine(AttMachine attMachine, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(attMachine);
		beforeSaveAttMachine(attMachine);
		attMachineDao.merge(attMachine);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_attMachine",model="X6Basic_1.0_attMachine_AttMachine",desc="foundation.batch.save",operType="4")
	public void batchSaveAttMachine(AttMachine attMachine, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "X6Basic_1.0_attMachine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_attMachine");
	...
	@Override
	public Page<AttMachine> getByPage(Page<AttMachine> page,DetachedCriteria detachedCriteria){
		return attMachineDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_attMachine").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveAttMachine(AttMachine attMachine, Object... objects){
		checkUniqueConstraint(attMachine);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveAttMachine(AttMachine attMachine, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
		checkCodeUnique(attMachine);
		if(!checkCodeUnique(attMachine)){
			throw new BAPException("attendence.attMachineCodealreadyexist","");
		}
/* CUSTOM CODE END */
	}
	
	@Override
	public void beforeExcelImportAttMachine(List<AttMachine> attMachines, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportAttMachine(List<AttMachine> attMachines){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportAttMachine(List<AttMachine> attMachines, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportAttMachine(List<AttMachine> attMachines){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteAttMachine(AttMachine attMachine){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteAttMachine(AttMachine attMachine){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreAttMachine(AttMachine attMachine){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreAttMachine(AttMachine attMachine){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(AttMachine attMachine, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(AttMachine attMachine, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportAttMachine(List<AttMachine> insertObjs, List<AttMachine> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportAttMachine,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportAttMachine(List<AttMachine> insertObjs, List<AttMachine> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportAttMachine,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
private boolean checkCodeUnique(AttMachine attMachine){
		
		List<AttMachine> listcounts=attMachineDao.createCriteria(
				Restrictions.ne("id", attMachine.getId()),
				Restrictions.eq("code", attMachine.getCode()),
				Restrictions.eq("valid", true)).list();
    ...
    //所有设置了考勤的考勤机
    @Override
    public List<AttMachine> findAttMachine() {
        return attMachineDao.createQuery("from AttMachine where ifatt=1 and valid=1").list();
        
    }
    
    //所有设置了消费的考勤机
    @Override
    public List<AttMachine> findConsumeMachine() {
        return attMachineDao.createQuery("from AttMachine where ifconsume=1 and valid=1").list();
    }
/* CUSTOM CODE END */
}
