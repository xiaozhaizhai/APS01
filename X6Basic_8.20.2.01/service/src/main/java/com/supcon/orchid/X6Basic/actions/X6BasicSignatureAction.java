...
/* CUSTOM CODE START(action,import,X6Basic_1.0_ggSignetManage_Signature,X6Basic_1.0) */
// 自定义代码
import java.util.Random;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicSignatureAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicSignature> {
	...
	private X6BasicSignature signature;
	...
	//导出excel时,主模型内容
	List<X6BasicSignature> queryResults = new ArrayList<X6BasicSignature>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/ggSignetManage/signature/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/ggSignetManage/signature/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_ggSignetManage_Signature" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicSignature> signatures;
	
	public List<X6BasicSignature> getSignatures() {
		return signatures;
	}
	...
	private Page<X6BasicSignature> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			signature = new X6BasicSignature();
		} else {
			if(id > 0){
				signature = signatureService.getSignature(id, viewCode);
			}
		}
	...
	public Page<X6BasicSignature> getPage() {
		if (null == page) {
			page = new Page<X6BasicSignature>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicSignature getSignature() {
		return signature;
	}		

	public void setSignature(X6BasicSignature signature) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/ggSignetManage/signature/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = signatureService.getSignatureAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/ggSignetManage/signature/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String signatureReferenceCopy() throws Exception{
			X6BasicSignature oldSignature = null;
			if(isEditNew()){
				...
				oldSignature = new X6BasicSignature();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,signature.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, signature.getId());
				...
				oldSignature = signatureService.getSignature(id);
			}
			
			
			signature = signatureService.getSignature(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_ggSignetManage_Signature");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicSignature.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldSignature, null);
						Method setValue = X6BasicSignature.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(signature, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			signature.setCreateDepartmentId(oldSignature.getCreateDepartmentId());
			signature.setCreatePositionId(oldSignature.getCreatePositionId());
			signature.setCreateStaffId(oldSignature.getCreateStaffId());
			signature.setOwnerDepartmentId(oldSignature.getOwnerDepartmentId());
			signature.setOwnerPositionId(oldSignature.getOwnerPositionId());
			signature.setOwnerStaffId(oldSignature.getOwnerStaffId());
			signature.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/ggSignetManage/signature/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/ggSignetManage/signature/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=signatureService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return signatureReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			signature = signatureService.getSignature(this.signature.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,signature.getId().toString());
			...
			List<Document> signaturePicDocuments = documentService.getPictureByLinkIdAndType(signature.getId(), "X6Basic_ggSignetManage_signature", "X6Basic_1.0_ggSignetManage_Signature_signaturePic");
			if(signaturePicDocuments!=null && !signaturePicDocuments.isEmpty()){
				String filPath = signaturePicDocuments.get(0).getPath();
				signature.setSignaturePic(filPath);
				signature.setSignaturePicDocument(signaturePicDocuments.get(0));
			}
		bapAfterCustomMethod(viewCode, viewName);
		if(null != printName && !printName.equals("")){
			signatureService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/ggSignetManage/signature/view",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "view.ftl") }),
		@Action(value = "/X6Basic/ggSignetManage/signature/viewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "viewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/ggSignetManage/signature/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = signatureService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(signatureService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(signatureService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_ggSignetManage_Signature", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/ggSignetManage/signature/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/ggSignetManage/signature/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.staff.name,result.description,result.signature,result.staff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.signatureAttachementInfo,result.signatureDocument.id,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"signature\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"signature\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"signature\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"signature\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/ggSignetManage/signature/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/ggSignetManage/signature/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.staff.name,result.description,result.staff.id,result.attrMap.*"})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/ggSignetManage/signature/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_ggSignetManage_Signature");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		signatureService.findDataGridPage(dg,dgclass,getDgPage(),signature,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(signature.getId() != null && signature.getId() != -1){
				signature = signatureService.getSignature(signature.getId());
			}
			signatureService.findDataGridPage(dg,dgclass,getDgPage(),signature,searchCondition,params);
	...
	private void save() throws Exception {
		...
		signatureService.saveSignature(signature,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/ggSignetManage/signature/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/ggSignetManage/signature/view/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "submit");
		bap_validate_datagrids=signatureService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", signature.getId());
		...
		fileuploads.put("linkId", signature.getId());
		bapAfterCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/ggSignetManage/signature/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		signatureService.deleteSignature(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/ggSignetManage/signature/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "restore");
		signatureService.restoreSignature(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/ggSignetManage/signature/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/ggSignetManage/signature/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "showDealInfoData");
		if (null != tableInfoId) {
			int count=signatureService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = signatureService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = signatureService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicSignature> findExportDataInfos(Page<X6BasicSignature> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = signatureService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = signatureService.getAssProperty(p.getCode());	
									//String assModelCode = signatureService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			signatureService.findSignatures(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			signatureService.findSignatures(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicSignature> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicSignature result : results){
	...
	public boolean isEditNew(){
		//return null == this.signature || null == this.signature.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/ggSignetManage/signature/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/ggSignetManage/signature/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/ggSignetManage/signature/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/ggSignetManage/signature/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicSignatureService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_ggSignetManage", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_ggSignetManage");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/ggSignetManage/signature/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_ggSignetManage_Signature", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_ggSignetManage_Signature", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/ggSignetManage/signature/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/ggSignetManage/signature/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_ggSignetManage_Signature";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_ggSignetManage_Signature", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/ggSignetManage/signature/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		X6BasicSignature signature = null;
		if(null != id) {
			signature = signatureService.getSignature(id);
		}
		...
		_auditLog.setColumnStringB(null == signature.getId() ? "" : signature.getId().toString());
		_auditLog.setColumnStringA(null == signature.getId() ? "" : signature.getId().toString());
		_auditLog.setColumnStringC(signature.getId().toString());
		_auditLog.setColumnStringD(X6BasicSignature.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_ggSignetManage");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicSignatureAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, signature,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, signature,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,signature);
		...
		printlog.setEntity("X6Basic_1.0_ggSignetManage");
		...
		printlog.setBusinessId(signature.getId());
		printlog.setBussinessKey(null == signature.getId() ? "" : signature.getId().toString());
		printlog.setMainDisplay(null == signature.getId() ? "" : signature.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/ggSignetManage/signature/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(signature,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/ggSignetManage/signature/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(signatureService.getSignature(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/ggSignetManage/signature/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/ggSignetManage/signature/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, signature,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				signature = signatureService.getSignature(Long.parseLong(arrId[i]));
				if(null != signature){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == signature.getId() ? "" : "," + signature.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == signature.getId() ? "" : signature.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == signature.getId() ? "" : "," + signature.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == signature.getId() ? "" : signature.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == signature.getId() ? "" : "," + signature.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == signature.getId() ? "" : signature.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, signature,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,signature);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/ggSignetManage/signature/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicSignature> findExportPageInfos(Page<X6BasicSignature> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_ggSignetManage" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = signatureService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicSignature queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = signatureService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = signatureService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_ggSignetManage_Signature");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_ggSignetManage_Signature")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_ggSignetManage_Signature");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_ggSignetManage_Signature");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = signatureService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_ggSignetManage_Signature,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_ggSignetManage_Signature,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_ggSignetManage_Signature,X6Basic_1.0) */
// 自定义代码
	...
	@Action(value = "/X6Basic/ggSignetManage/signature/getMySignet", results = { @Result(name = SUCCESS, type = "json", params = { "root",
	"responseMap" }) })
public String getMySignet() throws Exception {
		
		List<Object> params=new ArrayList<>();
		if(currentUserId!=null){
			params.add(currentUserId);
		}else{
			params.add(getCurrentStaff().getId());
		}
		
		List<X6BasicSignature> list=signatureService.getSignatures(" staff= ? and valid=1 " , params);
		if(list!=null && list.size()>0){
			Random random=new Random();
			Integer i=random.nextInt(list.size());
			X6BasicSignature signature=list.get(i);
			responseMap.put("linkId", signature.getId());
			responseMap.put("dealSuccessFlag", true);
		}else{
