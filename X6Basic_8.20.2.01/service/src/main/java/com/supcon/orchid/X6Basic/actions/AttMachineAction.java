...
/* CUSTOM CODE START(action,import,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class AttMachineAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<AttMachine> {
	...
	//导出excel时,主模型内容
	List<AttMachine> queryResults = new ArrayList<AttMachine>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/attMachine/attMachine/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/attMachine/attMachine/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_attMachine_AttMachine" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<AttMachine> attMachines;
	
	public List<AttMachine> getAttMachines() {
		return attMachines;
	}
	...
	public void prepare() throws Exception {
		if (null == id) {
			attMachine = new AttMachine();
			if(attMachine.getAttType() != null) {
				attMachine.setAttType((SystemCode) systemCodeService.getSystemCode(attMachine.getAttType().getId()));
			}
		} else {
			if(id > 0){
				attMachine = attMachineService.getAttMachine(id, viewCode);
			}
		}
	...
	public Page<AttMachine> getPage() {
		if (null == page) {
			page = new Page<AttMachine>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public AttMachine getAttMachine() {
		return attMachine;
	}		

	public void setAttMachine(AttMachine attMachine) {
		this.attMachine = attMachine;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/attMachine/attMachine/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = attMachineService.getAttMachineAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/attMachine/attMachine/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String attMachineReferenceCopy() throws Exception{
			AttMachine oldAttMachine = null;
			if(isEditNew()){
				...
				oldAttMachine = new AttMachine();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,attMachine.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, attMachine.getId());
				...
				oldAttMachine = attMachineService.getAttMachine(id);
			}
			
			
			attMachine = attMachineService.getAttMachine(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_attMachine_AttMachine");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = AttMachine.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldAttMachine, null);
						Method setValue = AttMachine.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(attMachine, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			attMachine.setCreateDepartmentId(oldAttMachine.getCreateDepartmentId());
			attMachine.setCreatePositionId(oldAttMachine.getCreatePositionId());
			attMachine.setCreateStaffId(oldAttMachine.getCreateStaffId());
			attMachine.setOwnerDepartmentId(oldAttMachine.getOwnerDepartmentId());
			attMachine.setOwnerPositionId(oldAttMachine.getOwnerPositionId());
			attMachine.setOwnerStaffId(oldAttMachine.getOwnerStaffId());
			attMachine.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_attMachine_AttMachine", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/attMachine/attMachine/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/attMachine/attMachine/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=attMachineService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return attMachineReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			attMachine = attMachineService.getAttMachine(this.attMachine.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,attMachine.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/attMachine/attMachine/view",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "view.ftl") }),
		@Action(value = "/X6Basic/attMachine/attMachine/viewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "viewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/attMachine/attMachine/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = attMachineService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(attMachineService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(attMachineService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_attMachine_AttMachine", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/attMachine/attMachine/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/attMachine/attMachine/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.attType.id,result.attType.value,result.ifatt,result.ifMeeting,result.ifconsume,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"attMachine\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"attMachine\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"attMachine\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"attMachine\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/attMachine/attMachine/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/attMachine/attMachine/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.attType.id,result.attType.value,result.ifatt,result.ifMeeting,result.ifconsume,result.attrMap.*"})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/attMachine/attMachine/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_attMachine_AttMachine");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		attMachineService.findDataGridPage(dg,dgclass,getDgPage(),attMachine,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(attMachine.getId() != null && attMachine.getId() != -1){
				attMachine = attMachineService.getAttMachine(attMachine.getId());
			}
			attMachineService.findDataGridPage(dg,dgclass,getDgPage(),attMachine,searchCondition,params);
	...
	private void save() throws Exception {
		...
		attMachineService.saveAttMachine(attMachine,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/attMachine/attMachine/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/attMachine/attMachine/view/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "submit");
		bap_validate_datagrids=attMachineService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", attMachine.getId());
		...
		fileuploads.put("linkId", attMachine.getId());
		bapAfterCustomMethod("X6Basic_1.0_attMachine_AttMachine", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/attMachine/attMachine/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		attMachineService.deleteAttMachine(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_attMachine_AttMachine", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/attMachine/attMachine/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "restore");
		attMachineService.restoreAttMachine(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_attMachine_AttMachine", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/attMachine/attMachine/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/attMachine/attMachine/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "showDealInfoData");
		if (null != tableInfoId) {
			int count=attMachineService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = attMachineService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = attMachineService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_attMachine_AttMachine", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<AttMachine> findExportDataInfos(Page<AttMachine> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = attMachineService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = attMachineService.getAssProperty(p.getCode());	
									//String assModelCode = attMachineService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			attMachineService.findAttMachines(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			attMachineService.findAttMachines(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<AttMachine> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(AttMachine result : results){
	...
	public boolean isEditNew(){
		//return null == this.attMachine || null == this.attMachine.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/attMachine/attMachine/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_attMachine_AttMachine", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/attMachine/attMachine/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/attMachine/attMachine/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/attMachine/attMachine/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.AttMachineService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_attMachine", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_attMachine");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_attMachine_AttMachine", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/attMachine/attMachine/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_attMachine_AttMachine", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_attMachine_AttMachine", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/attMachine/attMachine/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/attMachine/attMachine/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_attMachine_AttMachine";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_attMachine_AttMachine", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/attMachine/attMachine/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		AttMachine attMachine = null;
		if(null != id) {
			attMachine = attMachineService.getAttMachine(id);
		}
		...
		_auditLog.setColumnStringB(null == attMachine.getCode() ? "" : attMachine.getCode().toString());
		_auditLog.setColumnStringA(null == attMachine.getName() ? "" : attMachine.getName().toString());
		_auditLog.setColumnStringC(attMachine.getId().toString());
		_auditLog.setColumnStringD(AttMachine.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_attMachine");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.AttMachineAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, attMachine,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, attMachine,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,attMachine);
		...
		printlog.setEntity("X6Basic_1.0_attMachine");
		...
		printlog.setBusinessId(attMachine.getId());
		printlog.setBussinessKey(null == attMachine.getCode() ? "" : attMachine.getCode().toString());
		printlog.setMainDisplay(null == attMachine.getName() ? "" : attMachine.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/attMachine/attMachine/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(attMachine,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/attMachine/attMachine/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(attMachineService.getAttMachine(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/attMachine/attMachine/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/attMachine/attMachine/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, attMachine,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				attMachine = attMachineService.getAttMachine(Long.parseLong(arrId[i]));
				if(null != attMachine){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == attMachine.getName() ? "" : "," + attMachine.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == attMachine.getName() ? "" : attMachine.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == attMachine.getCode() ? "" : "," + attMachine.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == attMachine.getCode() ? "" : attMachine.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == attMachine.getId() ? "" : "," + attMachine.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == attMachine.getId() ? "" : attMachine.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, attMachine,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,attMachine);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/attMachine/attMachine/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<AttMachine> findExportPageInfos(Page<AttMachine> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_attMachine" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = attMachineService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(AttMachine queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = attMachineService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = attMachineService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_attMachine_AttMachine");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_attMachine_AttMachine")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_attMachine_AttMachine");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_attMachine_AttMachine");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = attMachineService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
