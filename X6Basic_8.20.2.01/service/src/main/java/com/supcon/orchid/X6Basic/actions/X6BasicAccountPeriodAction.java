...
/* CUSTOM CODE START(action,import,X6Basic_1.0_accountingPeriod_AccountPeriod,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicAccountPeriodAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicAccountPeriod> {
	...
	//导出excel时,主模型内容
	List<X6BasicAccountPeriod> queryResults = new ArrayList<X6BasicAccountPeriod>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/accountingPeriod/accountPeriod/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/accountingPeriod/accountPeriod/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_accountingPeriod_AccountPeriod" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicAccountPeriod> accountPeriods;
	
	public List<X6BasicAccountPeriod> getAccountPeriods() {
	...
	private Page<X6BasicAccountPeriod> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			accountPeriod = new X6BasicAccountPeriod();
			if(accountPeriod.getYear() != null) {
				accountPeriod.setYear((SystemCode) systemCodeService.getSystemCode(accountPeriod.getYear().getId()));
			}
		} else {
			if(id > 0){
				accountPeriod = accountPeriodService.getAccountPeriod(id, viewCode);
			}
		}
	...
	public Page<X6BasicAccountPeriod> getPage() {
		if (null == page) {
			page = new Page<X6BasicAccountPeriod>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = accountPeriodService.getAccountPeriodAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String accountPeriodReferenceCopy() throws Exception{
			X6BasicAccountPeriod oldAccountPeriod = null;
			if(isEditNew()){
				...
				oldAccountPeriod = new X6BasicAccountPeriod();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,accountPeriod.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, accountPeriod.getId());
				...
				oldAccountPeriod = accountPeriodService.getAccountPeriod(id);
			}
			
			
			accountPeriod = accountPeriodService.getAccountPeriod(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_accountingPeriod_AccountPeriod");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicAccountPeriod.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldAccountPeriod, null);
						Method setValue = X6BasicAccountPeriod.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(accountPeriod, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			accountPeriod.setCreateDepartmentId(oldAccountPeriod.getCreateDepartmentId());
			accountPeriod.setCreatePositionId(oldAccountPeriod.getCreatePositionId());
			accountPeriod.setCreateStaffId(oldAccountPeriod.getCreateStaffId());
			accountPeriod.setOwnerDepartmentId(oldAccountPeriod.getOwnerDepartmentId());
			accountPeriod.setOwnerPositionId(oldAccountPeriod.getOwnerPositionId());
			accountPeriod.setOwnerStaffId(oldAccountPeriod.getOwnerStaffId());
			accountPeriod.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=accountPeriodService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return accountPeriodReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			accountPeriod = accountPeriodService.getAccountPeriod(this.accountPeriod.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,accountPeriod.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/periodView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "periodView.ftl") }),
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/periodViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "periodViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = accountPeriodService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(accountPeriodService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(accountPeriodService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_accountingPeriod_AccountPeriod", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.year.id,result.year.value,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"accountPeriod\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"accountPeriod\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"accountPeriod\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"accountPeriod\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			accountPeriodService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.year.id,result.year.value,result.attrMap.*"})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_accountingPeriod_AccountPeriod");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		accountPeriodService.findDataGridPage(dg,dgclass,getDgPage(),accountPeriod,"", null);
	...
	private Page<X6BasicAccountPeriodPart> dg1400468411626Page;
	public Page<X6BasicAccountPeriodPart> getDg1400468411626Page(){
		if(null == dg1400468411626Page){
			dg1400468411626Page = new Page<X6BasicAccountPeriodPart>(1,20);
		}
		return dg1400468411626Page;
	}
	
	private Class dg1400468411626Clazz = X6BasicAccountPeriodPart.class;
	
	public Class getDg1400468411626Clazz() {
		return dg1400468411626Clazz;
	}
	
	private List<X6BasicAccountPeriodPart> dg1400468411626List;
	public List<X6BasicAccountPeriodPart> getDg1400468411626List(){
		if(null == dg1400468411626List){
			dg1400468411626List = new ArrayList<X6BasicAccountPeriodPart>();
		}
		return dg1400468411626List;
	}
	public void setDg1400468411626ListJson(String json) {
		//dg1400468411626List = (List<X6BasicAccountPeriodPart>) accountPeriodService.generateObjectFromJson(json, X6BasicAccountPeriodPart.class);
	}
	
	private List<Long> dg1400468411626DeletedIds;
	public List<Long> getDg1400468411626DeletedIds(){
		if(null == dg1400468411626DeletedIds){
			dg1400468411626DeletedIds = new ArrayList<Long>();
		}
		return dg1400468411626DeletedIds;
	}

	private Page<X6BasicAccountPeriodPart> dg1400468531194Page;
	public Page<X6BasicAccountPeriodPart> getDg1400468531194Page(){
		if(null == dg1400468531194Page){
			dg1400468531194Page = new Page<X6BasicAccountPeriodPart>(1,20);
		}
		return dg1400468531194Page;
	}
	
	private Class dg1400468531194Clazz = X6BasicAccountPeriodPart.class;
	
	public Class getDg1400468531194Clazz() {
		return dg1400468531194Clazz;
	}
	
	private List<X6BasicAccountPeriodPart> dg1400468531194List;
	public List<X6BasicAccountPeriodPart> getDg1400468531194List(){
		if(null == dg1400468531194List){
			dg1400468531194List = new ArrayList<X6BasicAccountPeriodPart>();
		}
		return dg1400468531194List;
	}
	public void setDg1400468531194ListJson(String json) {
		//dg1400468531194List = (List<X6BasicAccountPeriodPart>) accountPeriodService.generateObjectFromJson(json, X6BasicAccountPeriodPart.class);
	}
	
	private List<Long> dg1400468531194DeletedIds;
	public List<Long> getDg1400468531194DeletedIds(){
		if(null == dg1400468531194DeletedIds){
			dg1400468531194DeletedIds = new ArrayList<Long>();
		}
		return dg1400468531194DeletedIds;
	}

	@Actions({
		
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/data-dg1400468411626", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.month.id,result.month.value,result.startDate,result.endDate,result.isAccounted,result.id,result.version,result.accountPeriod.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1400468411626-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1400468411626.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/data-dg1400468531194", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.month.id,result.month.value,result.startDate,result.endDate,result.id,result.version,result.accountPeriod.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1400468531194-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1400468531194.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(accountPeriod.getId() != null && accountPeriod.getId() != -1){
				accountPeriod = accountPeriodService.getAccountPeriod(accountPeriod.getId());
			}
			accountPeriodService.findDataGridPage(dg,dgclass,getDgPage(),accountPeriod,searchCondition,params);
	...
	private void save() throws Exception {
		...
		accountPeriodService.saveAccountPeriod(accountPeriod,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/accountingPeriod/accountPeriod/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/accountingPeriod/accountPeriod/periodView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "submit");
		bap_validate_datagrids=accountPeriodService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", accountPeriod.getId());
		...
		fileuploads.put("linkId", accountPeriod.getId());
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		accountPeriodService.deleteAccountPeriod(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "restore");
		accountPeriodService.restoreAccountPeriod(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/accountingPeriod/accountPeriod/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/accountingPeriod/accountPeriod/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "showDealInfoData");
		if (null != tableInfoId) {
			int count=accountPeriodService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = accountPeriodService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = accountPeriodService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicAccountPeriod> findExportDataInfos(Page<X6BasicAccountPeriod> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = accountPeriodService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = accountPeriodService.getAssProperty(p.getCode());	
									//String assModelCode = accountPeriodService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			accountPeriodService.findAccountPeriods(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			accountPeriodService.findAccountPeriods(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicAccountPeriod> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicAccountPeriod result : results){
	...
	public boolean isEditNew(){
		//return null == this.accountPeriod || null == this.accountPeriod.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/accountingPeriod/accountPeriod/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/accountingPeriod/accountPeriod/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicAccountPeriodService"
						, "com.supcon.orchid.X6Basic.services.X6BasicAccountPeriodPartService"      
						, "com.supcon.orchid.X6Basic.services.X6BasicAccountPeriodPartService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_accountingPeriod", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_accountingPeriod");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_accountingPeriod_AccountPeriod", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_accountingPeriod_AccountPeriod", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_accountingPeriod_AccountPeriod";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriod", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == accountPeriod.getId() ? "" : accountPeriod.getId().toString());
		_auditLog.setColumnStringA(null == accountPeriod.getId() ? "" : accountPeriod.getId().toString());
		_auditLog.setColumnStringC(accountPeriod.getId().toString());
		_auditLog.setColumnStringD(X6BasicAccountPeriod.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_accountingPeriod");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicAccountPeriodAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, accountPeriod,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, accountPeriod,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,accountPeriod);
		...
		printlog.setEntity("X6Basic_1.0_accountingPeriod");
		...
		printlog.setBusinessId(accountPeriod.getId());
		printlog.setBussinessKey(null == accountPeriod.getId() ? "" : accountPeriod.getId().toString());
		printlog.setMainDisplay(null == accountPeriod.getId() ? "" : accountPeriod.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(accountPeriod,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(accountPeriodService.getAccountPeriod(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, accountPeriod,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				accountPeriod = accountPeriodService.getAccountPeriod(Long.parseLong(arrId[i]));
				if(null != accountPeriod){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == accountPeriod.getId() ? "" : "," + accountPeriod.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == accountPeriod.getId() ? "" : accountPeriod.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == accountPeriod.getId() ? "" : "," + accountPeriod.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == accountPeriod.getId() ? "" : accountPeriod.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == accountPeriod.getId() ? "" : "," + accountPeriod.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == accountPeriod.getId() ? "" : accountPeriod.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, accountPeriod,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,accountPeriod);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriod/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicAccountPeriod> findExportPageInfos(Page<X6BasicAccountPeriod> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_accountingPeriod" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = accountPeriodService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicAccountPeriod queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = accountPeriodService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = accountPeriodService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_accountingPeriod_AccountPeriod");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_accountingPeriod_AccountPeriod")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_accountingPeriod_AccountPeriod");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_accountingPeriod_AccountPeriod");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = accountPeriodService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_accountingPeriod_AccountPeriod,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_accountingPeriod_AccountPeriod,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_accountingPeriod_AccountPeriod,X6Basic_1.0) */
// 自定义代码
