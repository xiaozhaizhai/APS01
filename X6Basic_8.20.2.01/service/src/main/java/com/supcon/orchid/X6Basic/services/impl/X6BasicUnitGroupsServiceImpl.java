...
import com.supcon.orchid.X6Basic.services.X6BasicUnitGroupsImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicUnitGroupsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_unitGroupsService")
@Transactional
public class X6BasicUnitGroupsServiceImpl  extends BaseServiceImpl<X6BasicUnitGroups> implements X6BasicUnitGroupsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicUnitGroups> bapBaseService;
	...
	@Autowired
	private X6BasicUnitGroupsImportService unitGroupsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicUnitGroups.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  unitGroupsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicUnitGroups.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = unitGroupsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicUnitGroups.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicUnitGroups.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  unitGroupsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicUnitGroups.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicUnitGroups.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicUnitGroups> list = (List<X6BasicUnitGroups>) unitGroupsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicUnitGroups getUnitGroups(long id){
		return getUnitGroups(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicUnitGroups getUnitGroups(long id, String viewCode){
		X6BasicUnitGroups unitGroups = unitGroupsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getUnitGroupsAsJSON(long id, String include){
		X6BasicUnitGroups unitGroups = unitGroupsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(unitGroups, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.common.delete",operType="3")
	public void deleteUnitGroups(X6BasicUnitGroups unitGroups){
		if(null != unitGroups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicUnitGroups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
			AuditUtil.setColumnStringA(null == unitGroups.getName() ? "" : unitGroups.getName().toString());
			AuditUtil.setColumnStringC(unitGroups.getId().toString());
		}
		beforeDeleteUnitGroups(unitGroups);
		unitGroupsDao.delete(unitGroups);
		afterDeleteUnitGroups(unitGroups);
		unitGroupsDao.flush();
		List<X6BasicUnitGroups> params = new ArrayList<X6BasicUnitGroups>();
		List<X6BasicBaseUnit> baseUnitList = baseUnitDao.findByCriteria(Restrictions.eq("unitGroup", unitGroups));
		unitGroups.setBaseUnitList(baseUnitList);
		params.add(unitGroups);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_unitGroup");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.common.delete",operType="3")
	public void deleteUnitGroups(List<Long> unitGroupsIds){
		deleteUnitGroups(unitGroupsIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.common.delete",operType="3")
	public void deleteUnitGroups(List<Long> unitGroupsIds, String eventTopic) {
		List<X6BasicUnitGroups> unitGroupss = new ArrayList<X6BasicUnitGroups>();
		for(Long unitGroupsId : unitGroupsIds){
			X6BasicUnitGroups unitGroups = getUnitGroups(unitGroupsId);
			unitGroupss.add(unitGroups);
			if(null != unitGroups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicUnitGroups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == unitGroups.getName() ? "" : "," + unitGroups.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == unitGroups.getName() ? "" : unitGroups.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == unitGroups.getId() ? "" : "," + unitGroups.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == unitGroups.getId() ? "" : "," + unitGroups.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(unitGroupss != null && unitGroupss.size() > 0 ){
			for(X6BasicUnitGroups unitGroups : unitGroupss){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),unitGroups, unitGroups.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(unitGroupsIds != null && unitGroupsIds.size() > 0) {
			String hql = "update " + X6BasicUnitGroups.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = unitGroupsDao.createQuery(hql);
			query.setParameterList("ids", unitGroupsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicUnitGroups unitGroupsz : unitGroupss){
				unitGroupsz.setValid(false);
				List<X6BasicUnitGroups> params = new ArrayList<X6BasicUnitGroups>();
				List<X6BasicBaseUnit> baseUnitList = baseUnitDao.findByCriteria(Restrictions.eq("unitGroup", unitGroupsz));
				unitGroupsz.setBaseUnitList(baseUnitList);
				params.add(unitGroupsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_unitGroup");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.common.delete",operType="3")
	public void deleteUnitGroups(long unitGroupsId, int unitGroupsVersion){
		this.deleteUnitGroups(unitGroupsId, unitGroupsVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.common.delete",operType="3")
	public void deleteUnitGroups(long unitGroupsId, int unitGroupsVersion,SignatureLog signatureLog){
		X6BasicUnitGroups unitGroups = getUnitGroups(unitGroupsId);
		...
		beforeDeleteUnitGroups(unitGroups);
		unitGroupsDao.delete(unitGroupsId, unitGroupsVersion);
		afterDeleteUnitGroups(unitGroups);
		unitGroupsDao.flush();
		List<X6BasicUnitGroups> params = new ArrayList<X6BasicUnitGroups>();
		List<X6BasicBaseUnit> baseUnitList = baseUnitDao.findByCriteria(Restrictions.eq("unitGroup", unitGroups));
		...
		if(signatureLog != null ) {
			...
			businessKey=unitGroups.getId();
			signatureLog.setTableId(unitGroups.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.common.delete",operType="3")
	public void deleteUnitGroups(String unitGroupsIds){
		this.deleteUnitGroups(unitGroupsIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.common.delete",operType="3")
	public void deleteUnitGroups(String unitGroupsIds,SignatureLog signatureLog){
		deleteCollection(unitGroupsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteUnitGroups(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_unitGroup#modelCode:X6Basic_1.0_unitGroup_UnitGroups#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="ec.engine.restore",operType="10")
	public void restoreUnitGroups(String unitGroupsIds){
		restoreCollection(unitGroupsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreUnitGroups(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="ec.engine.restore",operType="10")
	public void restoreUnitGroups(long unitGroupsId){
		
		findBusinessKeyUsed(unitGroupsId);	//判断业务主键是否重复
		...
		beforeRestoreUnitGroups(unitGroups);
		unitGroups.setValid(true);
		unitGroupsDao.update(unitGroups);
		afterRestoreUnitGroups(unitGroups);
		List<X6BasicUnitGroups> params = new ArrayList<X6BasicUnitGroups>();
		List<X6BasicBaseUnit> baseUnitList = baseUnitDao.findByCriteria(Restrictions.eq("unitGroup", unitGroups));
		unitGroups.setBaseUnitList(baseUnitList);
		params.add(unitGroups);
	...
	public void findBusinessKeyUsed(long unitGroupsId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_unitGroup_UnitGroups");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicUnitGroups.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicUnitGroups.TABLE_NAME +" where id = ? )";
			List<Object> list =  unitGroupsDao.createNativeQuery(sql,unitGroupsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseUnitGroups(List<X6BasicUnitGroups>  unitGroupss){
		for(X6BasicUnitGroups unitGroups:unitGroupss)  {
			saveUnitGroups(unitGroups, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseUnitGroups(List<X6BasicUnitGroups>  unitGroupss){
		for(X6BasicUnitGroups unitGroups:unitGroupss)  {
			saveUnitGroups(unitGroups, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="ec.flow.save")
	public void saveUnitGroups(X6BasicUnitGroups unitGroups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveUnitGroups(unitGroups, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="ec.flow.save")
	public void saveUnitGroups(X6BasicUnitGroups unitGroups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveUnitGroups(unitGroups, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateUnitGroupsCodes(X6BasicUnitGroups unitGroups) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateUnitGroupsCodes(unitGroups, false);
	}
	
	public void generateUnitGroupsCodes(X6BasicUnitGroups unitGroups, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateUnitGroupsSummarys(X6BasicUnitGroups unitGroups) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateUnitGroupsSummarys(unitGroups, false);
	}

	public void generateUnitGroupsSummarys(X6BasicUnitGroups unitGroups, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups")
	public void saveUnitGroups(X6BasicUnitGroups unitGroups,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveUnitGroups(unitGroups, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups")
	public void saveUnitGroups(X6BasicUnitGroups unitGroups,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_unitGroup";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_unitGroup");
		String url = null;
		if(unitGroups.getId() != null && unitGroups.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicUnitGroups.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_unitGroup/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicUnitGroups.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_unitGroup/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, unitGroups);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(unitGroups);
		beforeSaveUnitGroups(unitGroups, viewIsView);

		if (viewIsView) {
			unitGroupsDao.saveWithRevertVersion(unitGroups);
		} else {
			if(isNew)
				unitGroupsDao.save(unitGroups);
			else
				unitGroupsDao.merge(unitGroups);
		}
		if(null != unitGroups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicUnitGroups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
			AuditUtil.setColumnStringA(null == unitGroups.getName() ? "" : unitGroups.getName().toString());
		...
		dealDatagridsSave(unitGroups,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateUnitGroupsCodes(unitGroups, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateUnitGroupsSummarys(unitGroups, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveUnitGroups(unitGroups, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, unitGroups);
			}
		}
		List<X6BasicUnitGroups> params = new ArrayList<X6BasicUnitGroups>();
		unitGroupsDao.flush();
		unitGroupsDao.clear();
		unitGroups = unitGroupsDao.load(unitGroups.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicUnitGroups unitGroups){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("unitGroups", unitGroups);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups")
	public void mergeUnitGroups(X6BasicUnitGroups unitGroups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(unitGroups);
		beforeSaveUnitGroups(unitGroups);
		unitGroupsDao.merge(unitGroups);
		if(null != unitGroups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicUnitGroups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == unitGroups.getId() ? "" : unitGroups.getId().toString());
			AuditUtil.setColumnStringA(null == unitGroups.getName() ? "" : unitGroups.getName().toString());
		...
		dealDatagridsSave(unitGroups,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveUnitGroups(unitGroups);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicUnitGroups> findUnitGroupss(Page<X6BasicUnitGroups> page, Criterion... criterions) {
		return unitGroupsDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicUnitGroupsDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicUnitGroupsDealInfo> list=unitGroupsDao.createQuery("from " + X6BasicUnitGroupsDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(X6BasicUnitGroupsDealInfo dealInfo) {
		unitGroupsDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicUnitGroupsDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) unitGroupsDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicUnitGroupsDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = unitGroupsDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = unitGroupsDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicUnitGroupsDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = unitGroupsDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findUnitGroupss(Page<X6BasicUnitGroups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findUnitGroupss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findUnitGroupss(Page<X6BasicUnitGroups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findUnitGroupss(Page<X6BasicUnitGroups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicUnitGroups> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"unitGroups\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(param.getName()!=null&&param.getName().startsWith("\"tree-")){
						...
						if(param.getName().indexOf(".COMPANY_ID") > 0) {
							companyId = Long.parseLong((String) param.getValue());
						}
						continue;
					}
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							s.append(" AND \"p\".CID in (1,").append(companyId+")");
						} else {
							s.append(" AND \"unitGroups\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"unitGroups\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"unitGroups\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"unitGroups\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_unitGroup_UnitGroups" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_unitGroup_UnitGroups" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  unitGroupsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  unitGroupsDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_unitGroup").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"unitGroups\"","X6Basic_1.0_unitGroup",customCondition,"S2BASE_UNITGROUP",true);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"unitGroups\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicUnitGroupsDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"unitGroups\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"unitGroups\".LAY_REC ASC, \"unitGroups\".SORT ASC");
				} else {
					orderPart.append(" \"unitGroups\".UNITGROUP_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) unitGroupsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=unitGroupsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = unitGroupsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicUnitGroups> result = new ArrayList<X6BasicUnitGroups>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicUnitGroups.class,unitGroupsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicUnitGroups.class,unitGroupsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicUnitGroups unitGroups : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(unitGroups.getId(), "X6Basic_unitGroup_unitGroups");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(unitGroups.getId(), "X6Basic_unitGroup_unitGroups");
							if(null != documents && !documents.isEmpty()) {
								unitGroups.setDocument(documents.get(0));
								unitGroups.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_unitGroup")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_unitGroup")+"X6Basic_1.0_unitGroup".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_unitGroup",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicUnitGroups> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicUnitGroups> result) {
		View view = null;
		...
		Map<Long, X6BasicUnitGroups> rsMap = new HashMap<Long, X6BasicUnitGroups>();
		for (X6BasicUnitGroups unitGroups : result) {
			ids.add(unitGroups.getId());
			rsMap.put(unitGroups.getId(), unitGroups);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(unitGroupsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = unitGroupsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicUnitGroups unitGroups,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(unitGroups.getOwnerStaffId());
		ti.setOwnerPositionId(unitGroups.getOwnerPositionId());
		ti.setOwnerDepartmentId(unitGroups.getOwnerDepartmentId());
		
		ti.setCreateStaff(unitGroups.getCreateStaff());
		ti.setCreateStaffId(unitGroups.getCreateStaffId());
		ti.setCreateTime(unitGroups.getCreateTime());
		ti.setDeleteStaff(unitGroups.getDeleteStaff());
		ti.setDeleteStaffId(unitGroups.getDeleteStaffId());
		ti.setDeleteTime(unitGroups.getDeleteTime());
		ti.setCreateDepartmentId(unitGroups.getCreateDepartmentId());
		ti.setModifyStaff(unitGroups.getModifyStaff());
		ti.setModifyStaffId(unitGroups.getModifyStaffId());
		ti.setCreatePositionId(unitGroups.getCreatePositionId());
		ti.setPositionLayRec(unitGroups.getPositionLayRec());
		ti.setTableNo(unitGroups.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_unitGroup");
		ti.setTargetTableName("S2BASE_UNITGROUP");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_unitGroup_UnitGroups" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  unitGroupsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"unitGroups\"","X6Basic_1.0_unitGroup","","S2BASE_UNITGROUP",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"unitGroups\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"unitGroups\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"unitGroups\"","X6Basic_1.0_unitGroup","","S2BASE_UNITGROUP",true);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("unitGroups.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"unitGroups\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"unitGroups\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"unitGroups\".CID = 1 OR \"unitGroups\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("unitGroups.mc\"").append(".UNIT_GROUPS) not in ("+notinValue+")");
			}
			
			SQLQuery query = unitGroupsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("unitGroup", "X6Basic_1.0_unitGroup", "unitGroup_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=unitGroupsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicUnitGroups.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicUnitGroups.MODULE_CODE;}
	protected String getModelCode(){return X6BasicUnitGroups.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param unitGroups 模型实例
	 */
	public void checkUniqueConstraint(X6BasicUnitGroups unitGroups){
	}
	
	
	@Override
	public List<X6BasicUnitGroups> getUnitGroupss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicUnitGroups.class);
		if(params!=null){
			...
			return unitGroupsDao.findByHql("from " + X6BasicUnitGroups.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return unitGroupsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<X6BasicUnitGroups> findUnitGroupssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicUnitGroups.class);
	...
	@Override
	public List<X6BasicUnitGroups> findUnitGroupssByHql(String hql, Object... objects){
		
		return   unitGroupsDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicUnitGroups loadUnitGroupsByBussinessKey(X6BasicUnitGroups unitGroups){
		return loadUnitGroupsByBussinessKey(unitGroups.getId());
	}
	
	@Override
	public Page<X6BasicUnitGroups> getUnitGroupss(Page<X6BasicUnitGroups> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicUnitGroups.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = unitGroupsDao.findByPage(page, sql);
			}else{
				page = unitGroupsDao.findAllByPage(page);
			}
			return page;
		}
		page = unitGroupsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicUnitGroups loadUnitGroupsByBussinessKey(Serializable bussinessKey){
		return unitGroupsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicUnitGroups> findByProperty(String propertyName, Object object){
		return unitGroupsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicUnitGroups findEntityByProperty(String propertyName, Object object){
		return unitGroupsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicUnitGroups.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = unitGroupsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_unitGroup_UnitGroups","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_unitGroup", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicUnitGroups unitGroups,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,unitGroupsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicUnitGroups.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, unitGroups);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",unitGroups.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicUnitGroups.TABLE_NAME +" where valid = 1";
			list = unitGroupsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicUnitGroups.TABLE_NAME;
			list = unitGroupsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicUnitGroups.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = unitGroupsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicUnitGroupsDaoImpl unitGroupsDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchUnitGroups(final List<X6BasicUnitGroups> insertObjs, final List<X6BasicUnitGroups> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchUnitGroups(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchUnitGroups(final List<X6BasicUnitGroups> insertObjs, final List<X6BasicUnitGroups> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1398233591035"),importCount));
		...
		Map<Long, X6BasicUnitGroups> oldUpdateObjsMap = new HashMap<Long, X6BasicUnitGroups>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicUnitGroups bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicUnitGroups> bizObjList = unitGroupsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicUnitGroups bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicUnitGroups> nmeObjs = new ArrayList<X6BasicUnitGroups>();
		...
		long id=autoGeneratorID.getNextId(X6BasicUnitGroups.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicUnitGroups unitGroups:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicUnitGroups.TABLE_NAME,1,dbId);
			if(unitGroups.getId() != null ){
				m.put(unitGroups.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) unitGroupsDao.createNativeQuery(sql, "X6Basic_1.0_unitGroup_UnitGroups"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		unitGroupsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicUnitGroups.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicUnitGroups unitGroups = updateObjs.get(updateIndex);
						Long id = unitGroups.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = unitGroups.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicUnitGroups.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicUnitGroups.TABLE_NAME + " order by id desc";
//					Query query = unitGroupsDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicUnitGroups unitGroups = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = unitGroups.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		unitGroupsDaoImpl.getSession().flush();
		unitGroupsDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicUnitGroups bizObj = insertObjs.get(insertIndex);
			unitGroupsImportService.saveImportDataLog(bizObj, null, "id", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicUnitGroups bizObj = updateObjs.get(updateIndex);
			X6BasicUnitGroups oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			unitGroupsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = unitGroupsDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				unitGroupsDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = unitGroupsDao.createNativeQuery(sql, "X6Basic_1.0_unitGroup_UnitGroups").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = unitGroupsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = unitGroupsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = unitGroupsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?unitGroupsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  unitGroupsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,unitGroupsDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1398235995490Page(Page<X6BasicBaseUnit> dg1398235995490Page,X6BasicUnitGroups unitGroups, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(X6BasicBaseUnit.class);
		criteria.add(Restrictions.eq("unitGroup", unitGroups));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"baseUnit\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		baseUnitDao.findByPage(dg1398235995490Page, criteria);
		baseUnitService.findByNormalDataGridUnitGroups(dg1398235995490Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1398236153862Page(Page<X6BasicBaseUnit> dg1398236153862Page,X6BasicUnitGroups unitGroups, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(X6BasicBaseUnit.class);
		criteria.add(Restrictions.eq("unitGroup", unitGroups));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		baseUnitDao.findByPage(dg1398236153862Page, criteria);
		baseUnitService.findByNormalDataGridUnitGroups(dg1398236153862Page);
	}
	
	 
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="ec.flow.save")
	public void saveUnitGroups(X6BasicUnitGroups unitGroups, DataGridService dataGridService){
		saveUnitGroups(unitGroups, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="ec.flow.save")
	public void saveUnitGroups(X6BasicUnitGroups unitGroups, DataGridService dataGridService, String viewCode){
		saveUnitGroups(unitGroups, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups")
	public void saveUnitGroups(X6BasicUnitGroups unitGroups, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveUnitGroups(unitGroups, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups")
	public void saveUnitGroups(X6BasicUnitGroups unitGroups, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_unitGroup";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_unitGroup");
		String url = null;
		if(null != unitGroups.getId() && unitGroups.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicUnitGroups.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups")
	public Map<Object, Object> unitGroupsDataGridImport(X6BasicUnitGroups unitGroups, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = unitGroups.getVirtualId();
		
		unitGroups = unitGroupsDao.load(unitGroups.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, unitGroups.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = unitGroups.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(unitGroups);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, unitGroups.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups")
	public void mergeUnitGroups(X6BasicUnitGroups unitGroups, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(unitGroups);
		beforeSaveUnitGroups(unitGroups);
		unitGroupsDao.merge(unitGroups);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_unitGroup",model="X6Basic_1.0_unitGroup_UnitGroups",desc="foundation.batch.save",operType="4")
	public void batchSaveUnitGroups(X6BasicUnitGroups unitGroups, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "X6Basic_1.0_unitGroup";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_unitGroup");
	...
	@Override
	public Page<X6BasicUnitGroups> getByPage(Page<X6BasicUnitGroups> page,DetachedCriteria detachedCriteria){
		return unitGroupsDao.findByPage(page, detachedCriteria);
	}
	@Override
	public List<X6BasicBaseUnit> getBaseUnitList(X6BasicUnitGroups unitGroups){
		return baseUnitDao.findByCriteria(Restrictions.eq("unitGroup", unitGroups),Restrictions.eq("valid", true));
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_unitGroup").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveUnitGroups(X6BasicUnitGroups unitGroups, Object... objects){
		checkUniqueConstraint(unitGroups);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
		//新建时,检测单位组代码是否重复 add by yy 2019-05-30
		if (null == unitGroups.getId() && checkUniqueUnitGroupsCode(unitGroups)) {
			throw new BAPException("单位组代码已存在,不能重复!");
		}
/* CUSTOM CODE END */
	}
	
	private void afterSaveUnitGroups(X6BasicUnitGroups unitGroups, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportUnitGroups(List<X6BasicUnitGroups> unitGroupss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportUnitGroups(List<X6BasicUnitGroups> unitGroupss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportUnitGroups(List<X6BasicUnitGroups> unitGroupss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportUnitGroups(List<X6BasicUnitGroups> unitGroupss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteUnitGroups(X6BasicUnitGroups unitGroups){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteUnitGroups(X6BasicUnitGroups unitGroups){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
		//单位组被删除时,把对应的表体数据也清除 add by yy 2019-05-30
		List<X6BasicBaseUnit> baseUnits = getBaseUnitIdsByGroupId(unitGroups);

		for (X6BasicBaseUnit baseUnit: baseUnits) {
			baseUnitService.deleteBaseUnit(baseUnit.getId(), baseUnit.getVersion());
		}
/* CUSTOM CODE END */
	}
	
	private void beforeRestoreUnitGroups(X6BasicUnitGroups unitGroups){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreUnitGroups(X6BasicUnitGroups unitGroups){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicUnitGroups unitGroups, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicUnitGroups unitGroups, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportUnitGroups(List<X6BasicUnitGroups> insertObjs, List<X6BasicUnitGroups> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportUnitGroups,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportUnitGroups(List<X6BasicUnitGroups> insertObjs, List<X6BasicUnitGroups> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportUnitGroups,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
	/**
	 * 通过单位组信息,查询出对应的单位信息,并返回
	 *
	 * @author yaoyao
	 * @param basicUnitGroups
	 * @return List<X6BasicBaseUnit>
	 *
	 * create by yy 2019-05-30
	 */
	private List<X6BasicBaseUnit> getBaseUnitIdsByGroupId(X6BasicUnitGroups basicUnitGroups){
		if (basicUnitGroups == null) {
			throw new BAPException("单位组不能为空!");
		}

		List<X6BasicBaseUnit> baseUnitList = baseUnitService.findBaseUnitsByHql(" from X6BasicBaseUnit where unitGroup.id = ? and valid = true", basicUnitGroups.getId());
	 ...
	 * create by yy 2019-05-30
	 */
	private boolean checkUniqueUnitGroupsCode(X6BasicUnitGroups basicUnitGroups){
		boolean existFlag = false;

		String hql = " from X6BasicUnitGroups where code = ? and valid = true";

		List<X6BasicUnitGroups> basicUnitGroupsList = findUnitGroupssByHql(hql, basicUnitGroups.getCode());

		if (null != basicUnitGroupsList && basicUnitGroupsList.size() > 0){
			existFlag = true;
		}
