...
@Repository
public class MessageCenterDaoImpl extends ExtendedGenericDaoImpl<MessageCenter, Long> implements MessageCenterDao {
	@Override
	...
	@Override
	public MessageCenterDealInfo copyAndSaveDealInfo(DealInfo di) {
		MessageCenterDealInfo dealInfo = new MessageCenterDealInfo();
	...
	@Override
	public MessageCenterDealInfo copyAndSaveDealInfo(DealInfo di, MessageCenter mainObj, Staff dealStaff) {
		MessageCenterDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MessageCenterDealInfo.class);
		...
		List<MessageCenterDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_messageManage_MessageCenter,X6Basic_1.0) */
// 自定义代码
