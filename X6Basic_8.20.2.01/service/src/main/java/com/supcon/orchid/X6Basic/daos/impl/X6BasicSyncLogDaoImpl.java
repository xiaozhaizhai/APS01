...
@Repository
public class X6BasicSyncLogDaoImpl extends ExtendedGenericDaoImpl<X6BasicSyncLog, Long> implements X6BasicSyncLogDao {
	@Override
	...
	@Override
	public X6BasicSyncLogDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicSyncLogDealInfo dealInfo = new X6BasicSyncLogDealInfo();
	...
	@Override
	public X6BasicSyncLogDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicSyncLog mainObj, Staff dealStaff) {
		X6BasicSyncLogDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicSyncLogDealInfo.class);
		...
		List<X6BasicSyncLogDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
