...
/* CUSTOM CODE START(action,import,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicSyncLogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicSyncLog> {
	...
	private X6BasicSyncLog syncLog;
	...
	//导出excel时,主模型内容
	List<X6BasicSyncLog> queryResults = new ArrayList<X6BasicSyncLog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/syncLog/syncLog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/syncLog/syncLog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_syncLog_SyncLog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicSyncLog> syncLogs;
	
	public List<X6BasicSyncLog> getSyncLogs() {
		return syncLogs;
	}
	...
	private Page<X6BasicSyncLog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			syncLog = new X6BasicSyncLog();
		} else {
			if(id > 0){
				syncLog = syncLogService.getSyncLog(id, viewCode);
			}
		}
	...
	public Page<X6BasicSyncLog> getPage() {
		if (null == page) {
			page = new Page<X6BasicSyncLog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicSyncLog getSyncLog() {
		return syncLog;
	}		

	public void setSyncLog(X6BasicSyncLog syncLog) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/syncLog/syncLog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = syncLogService.getSyncLogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/syncLog/syncLog/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String syncLogReferenceCopy() throws Exception{
			X6BasicSyncLog oldSyncLog = null;
			if(isEditNew()){
				...
				oldSyncLog = new X6BasicSyncLog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,syncLog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, syncLog.getId());
				...
				oldSyncLog = syncLogService.getSyncLog(id);
			}
			
			
			syncLog = syncLogService.getSyncLog(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_syncLog_SyncLog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicSyncLog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldSyncLog, null);
						Method setValue = X6BasicSyncLog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(syncLog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			syncLog.setCreateDepartmentId(oldSyncLog.getCreateDepartmentId());
			syncLog.setCreatePositionId(oldSyncLog.getCreatePositionId());
			syncLog.setCreateStaffId(oldSyncLog.getCreateStaffId());
			syncLog.setOwnerDepartmentId(oldSyncLog.getOwnerDepartmentId());
			syncLog.setOwnerPositionId(oldSyncLog.getOwnerPositionId());
			syncLog.setOwnerStaffId(oldSyncLog.getOwnerStaffId());
			syncLog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_syncLog_SyncLog", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=syncLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return syncLogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			syncLog = syncLogService.getSyncLog(this.syncLog.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,syncLog.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/syncLog/syncLog/viewPage",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "viewPage.ftl") }),
		@Action(value = "/X6Basic/syncLog/syncLog/viewPagePrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "viewPagePrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/syncLog/syncLog/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = syncLogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(syncLogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(syncLogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_syncLog_SyncLog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/syncLog/syncLog/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.syncStaff.name,result.userName,result.eventTopic,result.emailFlag,result.bbsFlag,result.pandionFlag,result.createTime,result.syncStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"syncLog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"syncLog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"syncLog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"syncLog\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/syncLog/syncLog/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/syncLog/syncLog/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_syncLog_SyncLog");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		syncLogService.findDataGridPage(dg,dgclass,getDgPage(),syncLog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(syncLog.getId() != null && syncLog.getId() != -1){
				syncLog = syncLogService.getSyncLog(syncLog.getId());
			}
			syncLogService.findDataGridPage(dg,dgclass,getDgPage(),syncLog,searchCondition,params);
	...
	private void save() throws Exception {
		...
		syncLogService.saveSyncLog(syncLog,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/syncLog/syncLog/viewPage/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "submit");
		bap_validate_datagrids=syncLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", syncLog.getId());
		...
		fileuploads.put("linkId", syncLog.getId());
		bapAfterCustomMethod("X6Basic_1.0_syncLog_SyncLog", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/syncLog/syncLog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		syncLogService.deleteSyncLog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_syncLog_SyncLog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/syncLog/syncLog/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "restore");
		syncLogService.restoreSyncLog(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_syncLog_SyncLog", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/syncLog/syncLog/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/syncLog/syncLog/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "showDealInfoData");
		if (null != tableInfoId) {
			int count=syncLogService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = syncLogService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = syncLogService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_syncLog_SyncLog", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicSyncLog> findExportDataInfos(Page<X6BasicSyncLog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = syncLogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = syncLogService.getAssProperty(p.getCode());	
									//String assModelCode = syncLogService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			syncLogService.findSyncLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			syncLogService.findSyncLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicSyncLog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicSyncLog result : results){
	...
	public boolean isEditNew(){
		//return null == this.syncLog || null == this.syncLog.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/syncLog/syncLog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_syncLog_SyncLog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/syncLog/syncLog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/syncLog/syncLog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/syncLog/syncLog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicSyncLogService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_syncLog", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_syncLog");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_syncLog_SyncLog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/syncLog/syncLog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_syncLog_SyncLog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_syncLog_SyncLog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/syncLog/syncLog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/syncLog/syncLog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_syncLog_SyncLog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_syncLog_SyncLog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/syncLog/syncLog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		X6BasicSyncLog syncLog = null;
		if(null != id) {
			syncLog = syncLogService.getSyncLog(id);
		}
		...
		_auditLog.setColumnStringB(null == syncLog.getId() ? "" : syncLog.getId().toString());
		_auditLog.setColumnStringA(null == syncLog.getEventTopic() ? "" : syncLog.getEventTopic().toString());
		_auditLog.setColumnStringC(syncLog.getId().toString());
		_auditLog.setColumnStringD(X6BasicSyncLog.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_syncLog");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicSyncLogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, syncLog,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, syncLog,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,syncLog);
		...
		printlog.setEntity("X6Basic_1.0_syncLog");
		...
		printlog.setBusinessId(syncLog.getId());
		printlog.setBussinessKey(null == syncLog.getId() ? "" : syncLog.getId().toString());
		printlog.setMainDisplay(null == syncLog.getEventTopic() ? "" : syncLog.getEventTopic().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/syncLog/syncLog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(syncLog,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/syncLog/syncLog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(syncLogService.getSyncLog(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/syncLog/syncLog/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/syncLog/syncLog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, syncLog,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				syncLog = syncLogService.getSyncLog(Long.parseLong(arrId[i]));
				if(null != syncLog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == syncLog.getEventTopic() ? "" : "," + syncLog.getEventTopic().toString()));
					} else {
						_auditLog.setColumnStringA(null == syncLog.getEventTopic() ? "" : syncLog.getEventTopic().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == syncLog.getId() ? "" : "," + syncLog.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == syncLog.getId() ? "" : syncLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == syncLog.getId() ? "" : "," + syncLog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == syncLog.getId() ? "" : syncLog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, syncLog,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,syncLog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/syncLog/syncLog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicSyncLog> findExportPageInfos(Page<X6BasicSyncLog> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_syncLog" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = syncLogService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicSyncLog queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = syncLogService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = syncLogService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_syncLog_SyncLog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_syncLog_SyncLog")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_syncLog_SyncLog");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_syncLog_SyncLog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = syncLogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
