...
/* CUSTOM CODE START(action,import,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicTeamInfoAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicTeamInfo> {
	...
	private X6BasicTeamInfo teamInfo;
	...
	//导出excel时,主模型内容
	List<X6BasicTeamInfo> queryResults = new ArrayList<X6BasicTeamInfo>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/teamManage/teamInfo/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/teamManage/teamInfo/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_teamManage_TeamInfo" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicTeamInfo> teamInfos;
	
	public List<X6BasicTeamInfo> getTeamInfos() {
		return teamInfos;
	}
	...
	private Page<X6BasicTeamInfo> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			teamInfo = new X6BasicTeamInfo();
		} else {
			if(id > 0){
				teamInfo = teamInfoService.getTeamInfo(id, viewCode);
			}
		}
	...
	public Page<X6BasicTeamInfo> getPage() {
		if (null == page) {
			page = new Page<X6BasicTeamInfo>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicTeamInfo getTeamInfo() {
		return teamInfo;
	}		

	public void setTeamInfo(X6BasicTeamInfo teamInfo) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/teamManage/teamInfo/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = teamInfoService.getTeamInfoAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/teamManage/teamInfo/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1398303528441
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String teamInfoReferenceCopy() throws Exception{
			X6BasicTeamInfo oldTeamInfo = null;
			if(isEditNew()){
				...
				oldTeamInfo = new X6BasicTeamInfo();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,teamInfo.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, teamInfo.getId());
				...
				oldTeamInfo = teamInfoService.getTeamInfo(id);
			}
			
			
			teamInfo = teamInfoService.getTeamInfo(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_teamManage_TeamInfo");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicTeamInfo.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTeamInfo, null);
						Method setValue = X6BasicTeamInfo.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(teamInfo, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			teamInfo.setCreateDepartmentId(oldTeamInfo.getCreateDepartmentId());
			teamInfo.setCreatePositionId(oldTeamInfo.getCreatePositionId());
			teamInfo.setCreateStaffId(oldTeamInfo.getCreateStaffId());
			teamInfo.setOwnerDepartmentId(oldTeamInfo.getOwnerDepartmentId());
			teamInfo.setOwnerPositionId(oldTeamInfo.getOwnerPositionId());
			teamInfo.setOwnerStaffId(oldTeamInfo.getOwnerStaffId());
			teamInfo.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/teamManage/teamInfo/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/teamManage/teamInfo/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=teamInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return teamInfoReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			teamInfo = teamInfoService.getTeamInfo(this.teamInfo.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,teamInfo.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/teamManage/teamInfo/teamView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "teamView.ftl") }),
		@Action(value = "/X6Basic/teamManage/teamInfo/teamViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "teamViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/teamManage/teamInfo/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = teamInfoService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(teamInfoService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(teamInfoService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_teamManage_TeamInfo", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/teamManage/teamInfo/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/teamManage/teamInfo/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.dept.name,result.createDate,result.principal.name,result.teamMemo,result.dept.id,result.principal.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"teamInfo\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"teamInfo\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"dept\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"teamInfo\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"teamInfo\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/teamManage/teamInfo/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/teamManage/teamInfo/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.dept.name,result.createDate,result.principal.name,result.teamMemo,result.dept.id,result.principal.id,result.attrMap.*"})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/teamManage/teamInfo/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_teamManage_TeamInfo");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		teamInfoService.findDataGridPage(dg,dgclass,getDgPage(),teamInfo,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(teamInfo.getId() != null && teamInfo.getId() != -1){
				teamInfo = teamInfoService.getTeamInfo(teamInfo.getId());
			}
			teamInfoService.findDataGridPage(dg,dgclass,getDgPage(),teamInfo,searchCondition,params);
	...
	private void save() throws Exception {
		...
		teamInfoService.saveTeamInfo(teamInfo,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/teamManage/teamInfo/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/teamManage/teamInfo/teamView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "submit");
		bap_validate_datagrids=teamInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", teamInfo.getId());
		...
		fileuploads.put("linkId", teamInfo.getId());
		bapAfterCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/teamManage/teamInfo/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		teamInfoService.deleteTeamInfo(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/teamManage/teamInfo/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "restore");
		teamInfoService.restoreTeamInfo(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/teamManage/teamInfo/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/teamManage/teamInfo/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "showDealInfoData");
		if (null != tableInfoId) {
			int count=teamInfoService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = teamInfoService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = teamInfoService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicTeamInfo> findExportDataInfos(Page<X6BasicTeamInfo> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = teamInfoService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = teamInfoService.getAssProperty(p.getCode());	
									//String assModelCode = teamInfoService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			teamInfoService.findTeamInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			teamInfoService.findTeamInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicTeamInfo> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicTeamInfo result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".TEAM_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.teamInfo || null == this.teamInfo.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/teamManage/teamInfo/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/teamManage/teamInfo/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/teamManage/teamInfo/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/teamManage/teamInfo/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicTeamInfoService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_teamManage", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_teamManage");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/teamManage/teamInfo/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_teamManage_TeamInfo", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_teamManage_TeamInfo", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/teamManage/teamInfo/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/teamManage/teamInfo/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_teamManage_TeamInfo";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_teamManage_TeamInfo", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/teamManage/teamInfo/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		X6BasicTeamInfo teamInfo = null;
		if(null != id) {
			teamInfo = teamInfoService.getTeamInfo(id);
		}
		...
		_auditLog.setColumnStringB(null == teamInfo.getCode() ? "" : teamInfo.getCode().toString());
		_auditLog.setColumnStringA(null == teamInfo.getName() ? "" : teamInfo.getName().toString());
		_auditLog.setColumnStringC(teamInfo.getId().toString());
		_auditLog.setColumnStringD(X6BasicTeamInfo.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_teamManage");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicTeamInfoAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, teamInfo,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, teamInfo,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,teamInfo);
		...
		printlog.setEntity("X6Basic_1.0_teamManage");
		...
		printlog.setBusinessId(teamInfo.getId());
		printlog.setBussinessKey(null == teamInfo.getCode() ? "" : teamInfo.getCode().toString());
		printlog.setMainDisplay(null == teamInfo.getName() ? "" : teamInfo.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/teamManage/teamInfo/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(teamInfo,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/teamManage/teamInfo/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(teamInfoService.getTeamInfo(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/teamManage/teamInfo/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/teamManage/teamInfo/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, teamInfo,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				teamInfo = teamInfoService.getTeamInfo(Long.parseLong(arrId[i]));
				if(null != teamInfo){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == teamInfo.getName() ? "" : "," + teamInfo.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == teamInfo.getName() ? "" : teamInfo.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == teamInfo.getCode() ? "" : "," + teamInfo.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == teamInfo.getCode() ? "" : teamInfo.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == teamInfo.getId() ? "" : "," + teamInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == teamInfo.getId() ? "" : teamInfo.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, teamInfo,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,teamInfo);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/teamManage/teamInfo/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicTeamInfo> findExportPageInfos(Page<X6BasicTeamInfo> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_teamManage" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = teamInfoService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicTeamInfo queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = teamInfoService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = teamInfoService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_teamManage_TeamInfo");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_teamManage_TeamInfo")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_teamManage_TeamInfo");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_teamManage_TeamInfo");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = teamInfoService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
