...
import com.supcon.orchid.X6Basic.services.X6BasicSpellRulePartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicSpellRulePartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_spellRulePartService")
@Transactional
public class X6BasicSpellRulePartServiceImpl  extends BaseServiceImpl<X6BasicSpellRulePart> implements X6BasicSpellRulePartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicSpellRulePart> bapBaseService;
	...
	@Autowired
	private X6BasicSpellRulePartImportService spellRulePartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicSpellRulePart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  spellRulePartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicSpellRulePart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = spellRulePartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicSpellRulePart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicSpellRulePart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  spellRulePartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicSpellRulePart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicSpellRulePart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicSpellRulePart> list = (List<X6BasicSpellRulePart>) spellRulePartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicSpellRulePart getSpellRulePart(long id){
		return getSpellRulePart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSpellRulePart getSpellRulePart(long id, String viewCode){
		X6BasicSpellRulePart spellRulePart = spellRulePartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getSpellRulePartAsJSON(long id, String include){
		X6BasicSpellRulePart spellRulePart = spellRulePartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(spellRulePart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.common.delete",operType="3")
	public void deleteSpellRulePart(X6BasicSpellRulePart spellRulePart){
		if(null != spellRulePart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRulePart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
			AuditUtil.setColumnStringA(null == spellRulePart.getTeamOrder() ? "" : spellRulePart.getTeamOrder().toString());
			AuditUtil.setColumnStringC(spellRulePart.getId().toString());
		}
		beforeDeleteSpellRulePart(spellRulePart);
		spellRulePartDao.deletePhysical(spellRulePart);
		afterDeleteSpellRulePart(spellRulePart);
		spellRulePartDao.flush();
		List<X6BasicSpellRulePart> params = new ArrayList<X6BasicSpellRulePart>();
		params.add(spellRulePart);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.common.delete",operType="3")
	public void deleteSpellRulePart(List<Long> spellRulePartIds){
		deleteSpellRulePart(spellRulePartIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.common.delete",operType="3")
	public void deleteSpellRulePart(List<Long> spellRulePartIds, String eventTopic) {
		List<X6BasicSpellRulePart> spellRuleParts = new ArrayList<X6BasicSpellRulePart>();
		for(Long spellRulePartId : spellRulePartIds){
			X6BasicSpellRulePart spellRulePart = getSpellRulePart(spellRulePartId);
			spellRuleParts.add(spellRulePart);
			if(null != spellRulePart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRulePart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == spellRulePart.getTeamOrder() ? "" : "," + spellRulePart.getTeamOrder().toString()));
				} else {
					AuditUtil.setColumnStringA(null == spellRulePart.getTeamOrder() ? "" : spellRulePart.getTeamOrder().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == spellRulePart.getId() ? "" : "," + spellRulePart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == spellRulePart.getId() ? "" : "," + spellRulePart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(spellRuleParts != null && spellRuleParts.size() > 0 ){
			for(X6BasicSpellRulePart spellRulePart : spellRuleParts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = X6BasicSpellRulePart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(spellRulePart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),spellRulePart, spellRulePart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(spellRulePartIds != null && spellRulePartIds.size() > 0) {
			String hql = "delete " + X6BasicSpellRulePart.JPA_NAME + " where id in(:ids)";
			Query query = spellRulePartDao.createQuery(hql);
			query.setParameterList("ids", spellRulePartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicSpellRulePart spellRulePartz : spellRuleParts){
				spellRulePartz.setValid(false);
				List<X6BasicSpellRulePart> params = new ArrayList<X6BasicSpellRulePart>();
				params.add(spellRulePartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_spellRule");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.common.delete",operType="3")
	public void deleteSpellRulePart(long spellRulePartId, int spellRulePartVersion){
		this.deleteSpellRulePart(spellRulePartId, spellRulePartVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.common.delete",operType="3")
	public void deleteSpellRulePart(long spellRulePartId, int spellRulePartVersion,SignatureLog signatureLog){
		X6BasicSpellRulePart spellRulePart = getSpellRulePart(spellRulePartId);
		...
		beforeDeleteSpellRulePart(spellRulePart);
		spellRulePartDao.delete(spellRulePartId, spellRulePartVersion);
		afterDeleteSpellRulePart(spellRulePart);
		spellRulePartDao.flush();
		List<X6BasicSpellRulePart> params = new ArrayList<X6BasicSpellRulePart>();
		params.add(spellRulePart);
		...
		if(signatureLog != null ) {
			...
			businessKey=spellRulePart.getId();
			signatureLog.setTableId(spellRulePart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.common.delete",operType="3")
	public void deleteSpellRulePart(String spellRulePartIds){
		this.deleteSpellRulePart(spellRulePartIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.common.delete",operType="3")
	public void deleteSpellRulePart(String spellRulePartIds,SignatureLog signatureLog){
		deleteCollection(spellRulePartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteSpellRulePart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_spellRule#modelCode:X6Basic_1.0_spellRule_SpellRulePart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="ec.engine.restore",operType="10")
	public void restoreSpellRulePart(String spellRulePartIds){
		restoreCollection(spellRulePartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreSpellRulePart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="ec.engine.restore",operType="10")
	public void restoreSpellRulePart(long spellRulePartId){
		
		findBusinessKeyUsed(spellRulePartId);	//判断业务主键是否重复
		...
		beforeRestoreSpellRulePart(spellRulePart);
		spellRulePart.setValid(true);
		spellRulePartDao.update(spellRulePart);
		afterRestoreSpellRulePart(spellRulePart);
		List<X6BasicSpellRulePart> params = new ArrayList<X6BasicSpellRulePart>();
		params.add(spellRulePart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long spellRulePartId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_spellRule_SpellRulePart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicSpellRulePart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicSpellRulePart.TABLE_NAME +" where id = ? )";
			List<Object> list =  spellRulePartDao.createNativeQuery(sql,spellRulePartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseSpellRulePart(List<X6BasicSpellRulePart>  spellRuleParts){
		for(X6BasicSpellRulePart spellRulePart:spellRuleParts)  {
			saveSpellRulePart(spellRulePart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseSpellRulePart(List<X6BasicSpellRulePart>  spellRuleParts){
		for(X6BasicSpellRulePart spellRulePart:spellRuleParts)  {
			saveSpellRulePart(spellRulePart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="ec.flow.save")
	public void saveSpellRulePart(X6BasicSpellRulePart spellRulePart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveSpellRulePart(spellRulePart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="ec.flow.save")
	public void saveSpellRulePart(X6BasicSpellRulePart spellRulePart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveSpellRulePart(spellRulePart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateSpellRulePartCodes(X6BasicSpellRulePart spellRulePart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSpellRulePartCodes(spellRulePart, false);
	}
	
	public void generateSpellRulePartCodes(X6BasicSpellRulePart spellRulePart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateSpellRulePartSummarys(X6BasicSpellRulePart spellRulePart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSpellRulePartSummarys(spellRulePart, false);
	}

	public void generateSpellRulePartSummarys(X6BasicSpellRulePart spellRulePart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart")
	public void saveSpellRulePart(X6BasicSpellRulePart spellRulePart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveSpellRulePart(spellRulePart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart")
	public void saveSpellRulePart(X6BasicSpellRulePart spellRulePart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "X6Basic_1.0_spellRule");
		String url = null;
		if(spellRulePart.getId() != null && spellRulePart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRulePart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, spellRulePart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(spellRulePart);
		beforeSaveSpellRulePart(spellRulePart, viewIsView);

		if (viewIsView) {
			spellRulePartDao.saveWithRevertVersion(spellRulePart);
		} else {
			if(isNew)
				spellRulePartDao.save(spellRulePart);
			else
				spellRulePartDao.merge(spellRulePart);
		}
		if(null != spellRulePart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRulePart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
			AuditUtil.setColumnStringA(null == spellRulePart.getTeamOrder() ? "" : spellRulePart.getTeamOrder().toString());
		...
		dealDatagridsSave(spellRulePart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateSpellRulePartCodes(spellRulePart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateSpellRulePartSummarys(spellRulePart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveSpellRulePart(spellRulePart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, spellRulePart);
			}
		}
		List<X6BasicSpellRulePart> params = new ArrayList<X6BasicSpellRulePart>();
		spellRulePartDao.flush();
		spellRulePartDao.clear();
		spellRulePart = spellRulePartDao.load(spellRulePart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicSpellRulePart spellRulePart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("spellRulePart", spellRulePart);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart")
	public void mergeSpellRulePart(X6BasicSpellRulePart spellRulePart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(spellRulePart);
		beforeSaveSpellRulePart(spellRulePart);
		spellRulePartDao.merge(spellRulePart);
		if(null != spellRulePart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRulePart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
			AuditUtil.setColumnStringA(null == spellRulePart.getTeamOrder() ? "" : spellRulePart.getTeamOrder().toString());
		...
		dealDatagridsSave(spellRulePart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveSpellRulePart(spellRulePart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicSpellRulePart> findSpellRuleParts(Page<X6BasicSpellRulePart> page, Criterion... criterions) {
		return spellRulePartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSpellRuleParts(Page<X6BasicSpellRulePart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findSpellRuleParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSpellRuleParts(Page<X6BasicSpellRulePart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSpellRuleParts(Page<X6BasicSpellRulePart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicSpellRulePart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"spellRulePart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							...
							s.append(" AND \"spellRulePart\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"spellRulePart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"spellRulePart\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"spellRulePart\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			s.append(" ( \"spellRulePart\".SRP_RULEID IN ( SELECT SR_ID FROM S2BASE_SPELLRULE \"spellRule\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "X6Basic_1.0_spellRule" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  spellRulePartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  spellRulePartDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_spellRule").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"spellRule\"","X6Basic_1.0_spellRule","",true);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"spellRulePart\".LAY_REC ASC, \"spellRulePart\".SORT ASC");
				} else {
					orderPart.append(" \"spellRulePart\".SRP_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) spellRulePartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=spellRulePartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = spellRulePartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicSpellRulePart> result = new ArrayList<X6BasicSpellRulePart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicSpellRulePart.class,spellRulePartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicSpellRulePart.class,spellRulePartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicSpellRulePart spellRulePart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(spellRulePart.getId(), "X6Basic_spellRule_spellRulePart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(spellRulePart.getId(), "X6Basic_spellRule_spellRulePart");
							if(null != documents && !documents.isEmpty()) {
								spellRulePart.setDocument(documents.get(0));
								spellRulePart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicSpellRulePart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicSpellRulePart> result) {
		View view = null;
		...
		Map<Long, X6BasicSpellRulePart> rsMap = new HashMap<Long, X6BasicSpellRulePart>();
		for (X6BasicSpellRulePart spellRulePart : result) {
			ids.add(spellRulePart.getId());
			rsMap.put(spellRulePart.getId(), spellRulePart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(spellRulePartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = spellRulePartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_spellRule_SpellRulePart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="X6Basic_1.0_spellRule_SpellRulePart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  spellRulePartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"spellRulePart\"","X6Basic_1.0_spellRule","","S2BASE_SPELLRULEPART",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"spellRulePart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"spellRulePart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"spellRulePart\"","X6Basic_1.0_spellRule","","S2BASE_SPELLRULEPART",true);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "X6Basic_1.0_spellRule_SpellRulePart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("spellRulePart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"spellRulePart\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"spellRulePart\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"spellRulePart\".CID = 1 OR \"spellRulePart\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("spellRulePart.mc\"").append(".SPELL_RULE_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = spellRulePartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=spellRulePartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicSpellRulePart.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicSpellRulePart.MODULE_CODE;}
	protected String getModelCode(){return X6BasicSpellRulePart.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param spellRulePart 模型实例
	 */
	public void checkUniqueConstraint(X6BasicSpellRulePart spellRulePart){
	}
	
	
	@Override
	public List<X6BasicSpellRulePart> getSpellRuleParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSpellRulePart.class);
		if(params!=null){
			...
			return spellRulePartDao.findByHql("from " + X6BasicSpellRulePart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return spellRulePartDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicSpellRulePart> findSpellRulePartsByHql(String hql, Object... objects){
		...
		return   spellRulePartDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicSpellRulePart loadSpellRulePartByBussinessKey(X6BasicSpellRulePart spellRulePart){
		return loadSpellRulePartByBussinessKey(spellRulePart.getId());
	}
	
	@Override
	public Page<X6BasicSpellRulePart> getSpellRuleParts(Page<X6BasicSpellRulePart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSpellRulePart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = spellRulePartDao.findByPage(page, sql);
			}else{
				page = spellRulePartDao.findAllByPage(page);
			}
			return page;
		}
		page = spellRulePartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicSpellRulePart> findByTreeDataGridSpellRule(List<X6BasicSpellRulePart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(X6BasicSpellRulePart spellRulePart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridSpellRule(Page<X6BasicSpellRulePart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<X6BasicSpellRulePart> spellRuleParts = dgPage.getResult();
			for(X6BasicSpellRulePart spellRulePart : spellRuleParts){
				
			}
			dgPage.setResult(spellRuleParts);
		}
	...
	public X6BasicSpellRulePart loadSpellRulePartByBussinessKey(Serializable bussinessKey){
		return spellRulePartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicSpellRulePart> findByProperty(String propertyName, Object object){
		return spellRulePartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSpellRulePart findEntityByProperty(String propertyName, Object object){
		return spellRulePartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicSpellRulePart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = spellRulePartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_spellRule_SpellRulePart","runtime");
		try {
	...
	public void dealDatagridsSave(X6BasicSpellRulePart spellRulePart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,spellRulePartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicSpellRulePart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, spellRulePart);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",spellRulePart.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicSpellRulePart.TABLE_NAME +" where valid = 1";
			list = spellRulePartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicSpellRulePart.TABLE_NAME;
			list = spellRulePartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicSpellRulePart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = spellRulePartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicSpellRulePartDaoImpl spellRulePartDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchSpellRulePart(final List<X6BasicSpellRulePart> insertObjs, final List<X6BasicSpellRulePart> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchSpellRulePart(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchSpellRulePart(final List<X6BasicSpellRulePart> insertObjs, final List<X6BasicSpellRulePart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1398306848646"),importCount));
		...
		Map<Long, X6BasicSpellRulePart> oldUpdateObjsMap = new HashMap<Long, X6BasicSpellRulePart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicSpellRulePart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicSpellRulePart> bizObjList = spellRulePartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicSpellRulePart bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicSpellRulePart> nmeObjs = new ArrayList<X6BasicSpellRulePart>();
		...
		long id=autoGeneratorID.getNextId(X6BasicSpellRulePart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicSpellRulePart spellRulePart:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicSpellRulePart.TABLE_NAME,1,dbId);
			if(spellRulePart.getId() != null ){
				m.put(spellRulePart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) spellRulePartDao.createNativeQuery(sql, "X6Basic_1.0_spellRule_SpellRulePart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		spellRulePartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicSpellRulePart.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicSpellRulePart spellRulePart = updateObjs.get(updateIndex);
						Long id = spellRulePart.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = spellRulePart.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(spellRulePart);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicSpellRulePart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicSpellRulePart.TABLE_NAME + " order by id desc";
//					Query query = spellRulePartDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicSpellRulePart spellRulePart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = spellRulePart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		spellRulePartDaoImpl.getSession().flush();
		spellRulePartDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicSpellRulePart bizObj = insertObjs.get(insertIndex);
			spellRulePartImportService.saveImportDataLog(bizObj, null, "id", "teamOrder");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicSpellRulePart bizObj = updateObjs.get(updateIndex);
			X6BasicSpellRulePart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			spellRulePartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "teamOrder");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<X6BasicSpellRulePart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<X6BasicSpellRulePart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) spellRulePartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<X6BasicSpellRulePart> result = getResult(page, query, new PendingResultTransformer(X6BasicSpellRulePart.class,spellRulePartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(X6BasicSpellRulePart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = spellRulePartDao.createNativeQuery(sql, "X6Basic_1.0_spellRule_SpellRulePart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = spellRulePartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = spellRulePartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = spellRulePartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?spellRulePartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  spellRulePartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,spellRulePartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="ec.flow.save")
	public void saveSpellRulePart(X6BasicSpellRulePart spellRulePart, DataGridService dataGridService){
		saveSpellRulePart(spellRulePart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="ec.flow.save")
	public void saveSpellRulePart(X6BasicSpellRulePart spellRulePart, DataGridService dataGridService, String viewCode){
		saveSpellRulePart(spellRulePart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart")
	public void saveSpellRulePart(X6BasicSpellRulePart spellRulePart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveSpellRulePart(spellRulePart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart")
	public void saveSpellRulePart(X6BasicSpellRulePart spellRulePart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_spellRule";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_spellRule");
		String url = null;
		if(null != spellRulePart.getId() && spellRulePart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRulePart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart")
	public Map<Object, Object> spellRulePartDataGridImport(X6BasicSpellRulePart spellRulePart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = spellRulePart.getVirtualId();
		
		spellRulePart = spellRulePartDao.load(spellRulePart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, spellRulePart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = spellRulePart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(spellRulePart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, spellRulePart.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart")
	public void mergeSpellRulePart(X6BasicSpellRulePart spellRulePart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(spellRulePart);
		beforeSaveSpellRulePart(spellRulePart);
		spellRulePartDao.merge(spellRulePart);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRulePart",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicSpellRulePart> getByPage(Page<X6BasicSpellRulePart> page,DetachedCriteria detachedCriteria){
		return spellRulePartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveSpellRulePart(X6BasicSpellRulePart spellRulePart, Object... objects){
		checkUniqueConstraint(spellRulePart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveSpellRulePart(X6BasicSpellRulePart spellRulePart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteSpellRulePart(X6BasicSpellRulePart spellRulePart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteSpellRulePart(X6BasicSpellRulePart spellRulePart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreSpellRulePart(X6BasicSpellRulePart spellRulePart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreSpellRulePart(X6BasicSpellRulePart spellRulePart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicSpellRulePart spellRulePart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicSpellRulePart spellRulePart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportSpellRulePart(List<X6BasicSpellRulePart> insertObjs, List<X6BasicSpellRulePart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportSpellRulePart,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportSpellRulePart(List<X6BasicSpellRulePart> insertObjs, List<X6BasicSpellRulePart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportSpellRulePart,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
