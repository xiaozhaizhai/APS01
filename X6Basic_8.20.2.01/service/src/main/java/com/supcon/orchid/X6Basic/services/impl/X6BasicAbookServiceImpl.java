...
import com.supcon.orchid.X6Basic.services.X6BasicAbookImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicAbookDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_abookService")
@Transactional
public class X6BasicAbookServiceImpl  extends BaseServiceImpl<X6BasicAbook> implements X6BasicAbookService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicAbook> bapBaseService;
	...
	@Autowired
	private X6BasicAbookImportService abookImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicAbook.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  abookDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicAbook.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = abookDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicAbook.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicAbook.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  abookDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicAbook.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicAbook.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicAbook> list = (List<X6BasicAbook>) abookDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicAbook getAbook(long id){
		return getAbook(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicAbook getAbook(long id, String viewCode){
		X6BasicAbook abook = abookDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getAbookAsJSON(long id, String include){
		X6BasicAbook abook = abookDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(abook, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.common.delete",operType="3")
	public void deleteAbook(X6BasicAbook abook){
		if(null != abook && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAbook.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == abook.getId() ? "" : abook.getId().toString());
			AuditUtil.setColumnStringA(null == abook.getName() ? "" : abook.getName().toString());
			AuditUtil.setColumnStringC(abook.getId().toString());
		}
		beforeDeleteAbook(abook);
		abookDao.delete(abook);
		afterDeleteAbook(abook);
		abookDao.flush();
		List<X6BasicAbook> params = new ArrayList<X6BasicAbook>();
		params.add(abook);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_addressBook");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.common.delete",operType="3")
	public void deleteAbook(List<Long> abookIds){
		deleteAbook(abookIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.common.delete",operType="3")
	public void deleteAbook(List<Long> abookIds, String eventTopic) {
		List<X6BasicAbook> abooks = new ArrayList<X6BasicAbook>();
		for(Long abookId : abookIds){
			X6BasicAbook abook = getAbook(abookId);
			abooks.add(abook);
			if(null != abook && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAbook.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == abook.getName() ? "" : "," + abook.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == abook.getName() ? "" : abook.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == abook.getId() ? "" : "," + abook.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == abook.getId() ? "" : abook.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == abook.getId() ? "" : "," + abook.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == abook.getId() ? "" : abook.getId().toString());
				}
			}
		}
		...
		if(abooks != null && abooks.size() > 0 ){
			for(X6BasicAbook abook : abooks){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),abook, abook.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(abookIds != null && abookIds.size() > 0) {
			String hql = "update " + X6BasicAbook.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = abookDao.createQuery(hql);
			query.setParameterList("ids", abookIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicAbook abookz : abooks){
				abookz.setValid(false);
				List<X6BasicAbook> params = new ArrayList<X6BasicAbook>();
				params.add(abookz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.common.delete",operType="3")
	public void deleteAbook(long abookId, int abookVersion){
		this.deleteAbook(abookId, abookVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.common.delete",operType="3")
	public void deleteAbook(long abookId, int abookVersion,SignatureLog signatureLog){
		X6BasicAbook abook = getAbook(abookId);
		...
		beforeDeleteAbook(abook);
		abookDao.delete(abookId, abookVersion);
		afterDeleteAbook(abook);
		abookDao.flush();
		List<X6BasicAbook> params = new ArrayList<X6BasicAbook>();
		params.add(abook);
		...
		if(signatureLog != null ) {
			...
			businessKey=abook.getId();
			signatureLog.setTableId(abook.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.common.delete",operType="3")
	public void deleteAbook(String abookIds){
		this.deleteAbook(abookIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.common.delete",operType="3")
	public void deleteAbook(String abookIds,SignatureLog signatureLog){
		deleteCollection(abookIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteAbook(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_addressBook#modelCode:X6Basic_1.0_addressBook_Abook#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="ec.engine.restore",operType="10")
	public void restoreAbook(String abookIds){
		restoreCollection(abookIds);
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="ec.engine.restore",operType="10")
	public void restoreAbook(long abookId){
		
		findBusinessKeyUsed(abookId);	//判断业务主键是否重复
		...
		beforeRestoreAbook(abook);
		abook.setValid(true);
		abookDao.update(abook);
		afterRestoreAbook(abook);
		List<X6BasicAbook> params = new ArrayList<X6BasicAbook>();
		params.add(abook);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long abookId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_addressBook_Abook");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicAbook.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicAbook.TABLE_NAME +" where id = ? )";
			List<Object> list =  abookDao.createNativeQuery(sql,abookId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseAbook(List<X6BasicAbook>  abooks){
		for(X6BasicAbook abook:abooks)  {
			saveAbook(abook, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseAbook(List<X6BasicAbook>  abooks){
		for(X6BasicAbook abook:abooks)  {
			saveAbook(abook, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="ec.flow.save")
	public void saveAbook(X6BasicAbook abook, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveAbook(abook, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="ec.flow.save")
	public void saveAbook(X6BasicAbook abook, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveAbook(abook, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateAbookCodes(X6BasicAbook abook) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAbookCodes(abook, false);
	}
	
	public void generateAbookCodes(X6BasicAbook abook, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateAbookSummarys(X6BasicAbook abook) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAbookSummarys(abook, false);
	}

	public void generateAbookSummarys(X6BasicAbook abook, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook")
	public void saveAbook(X6BasicAbook abook,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveAbook(abook, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook")
	public void saveAbook(X6BasicAbook abook,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_addressBook";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_addressBook");
		String url = null;
		if(abook.getId() != null && abook.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAbook.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_addressBook/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAbook.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_addressBook/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, abook);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(abook);
		beforeSaveAbook(abook, viewIsView);

		if (viewIsView) {
			abookDao.saveWithRevertVersion(abook);
		} else {
			if(isNew)
				abookDao.save(abook);
			else
				abookDao.merge(abook);
		}
		if(null != abook && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAbook.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == abook.getId() ? "" : abook.getId().toString());
			AuditUtil.setColumnStringA(null == abook.getName() ? "" : abook.getName().toString());
		...
		dealDatagridsSave(abook,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateAbookCodes(abook, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateAbookSummarys(abook, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveAbook(abook, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, abook);
			}
		}
		List<X6BasicAbook> params = new ArrayList<X6BasicAbook>();
		abookDao.flush();
		abookDao.clear();
		abook = abookDao.load(abook.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicAbook abook){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook")
	public void mergeAbook(X6BasicAbook abook, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(abook);
		beforeSaveAbook(abook);
		abookDao.merge(abook);
		if(null != abook && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAbook.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == abook.getId() ? "" : abook.getId().toString());
			AuditUtil.setColumnStringA(null == abook.getName() ? "" : abook.getName().toString());
		...
		dealDatagridsSave(abook,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveAbook(abook);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicAbook> findAbooks(Page<X6BasicAbook> page, Criterion... criterions) {
		return abookDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicAbookDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicAbookDealInfo> list=abookDao.createQuery("from " + X6BasicAbookDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(X6BasicAbookDealInfo dealInfo) {
		abookDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicAbookDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) abookDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicAbookDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = abookDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = abookDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicAbookDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = abookDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAbooks(Page<X6BasicAbook> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findAbooks(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAbooks(Page<X6BasicAbook> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAbooks(Page<X6BasicAbook> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicAbook> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"abook\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"abook\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"abook\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_addressBook_Abook" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_addressBook_Abook" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  abookDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  abookDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_addressBook").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"abook\"","X6Basic_1.0_addressBook",customCondition,"X6BASIC_ABOOKS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"abook\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicAbookDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"abook\".LAY_REC ASC, \"abook\".SORT ASC");
				} else {
					orderPart.append(" \"abook\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) abookDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=abookDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = abookDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicAbook> result = new ArrayList<X6BasicAbook>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicAbook.class,abookDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicAbook.class,abookDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicAbook abook : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(abook.getId(), "X6Basic_addressBook_abook");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(abook.getId(), "X6Basic_addressBook_abook");
							if(null != documents && !documents.isEmpty()) {
								abook.setDocument(documents.get(0));
								abook.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_addressBook")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_addressBook")+"X6Basic_1.0_addressBook".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_addressBook",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicAbook> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicAbook> result) {
		View view = null;
		...
		Map<Long, X6BasicAbook> rsMap = new HashMap<Long, X6BasicAbook>();
		for (X6BasicAbook abook : result) {
			ids.add(abook.getId());
			rsMap.put(abook.getId(), abook);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(abookDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = abookDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicAbook abook,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(abook.getOwnerStaffId());
		ti.setOwnerPositionId(abook.getOwnerPositionId());
		ti.setOwnerDepartmentId(abook.getOwnerDepartmentId());
		
		ti.setCreateStaff(abook.getCreateStaff());
		ti.setCreateStaffId(abook.getCreateStaffId());
		ti.setCreateTime(abook.getCreateTime());
		ti.setDeleteStaff(abook.getDeleteStaff());
		ti.setDeleteStaffId(abook.getDeleteStaffId());
		ti.setDeleteTime(abook.getDeleteTime());
		ti.setCreateDepartmentId(abook.getCreateDepartmentId());
		ti.setModifyStaff(abook.getModifyStaff());
		ti.setModifyStaffId(abook.getModifyStaffId());
		ti.setCreatePositionId(abook.getCreatePositionId());
		ti.setPositionLayRec(abook.getPositionLayRec());
		ti.setTableNo(abook.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_addressBook");
		ti.setTargetTableName("X6BASIC_ABOOKS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_addressBook_Abook" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  abookDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"abook\"","X6Basic_1.0_addressBook","","X6BASIC_ABOOKS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"abook\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"abook\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"abook\"","X6Basic_1.0_addressBook","","X6BASIC_ABOOKS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("abook.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"abook\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("abook.mc\"").append(".ABOOK) not in ("+notinValue+")");
			}
			
			SQLQuery query = abookDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("addressBook", "X6Basic_1.0_addressBook", "addressBook_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=abookDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicAbook.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicAbook.MODULE_CODE;}
	protected String getModelCode(){return X6BasicAbook.MODEL_CODE;}
	 ...
	 * @param abook 模型实例
	 */
	public void checkUniqueConstraint(X6BasicAbook abook){
	}
	
	
	@Override
	public List<X6BasicAbook> getAbooks(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicAbook.class);
		if(params!=null){
			...
			return abookDao.findByHql("from " + X6BasicAbook.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return abookDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<X6BasicAbook> findAbooksBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicAbook.class);
	...
	@Override
	public List<X6BasicAbook> findAbooksByHql(String hql, Object... objects){
		
		return   abookDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicAbook loadAbookByBussinessKey(X6BasicAbook abook){
		return loadAbookByBussinessKey(abook.getId());
	}
	
	@Override
	public Page<X6BasicAbook> getAbooks(Page<X6BasicAbook> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicAbook.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = abookDao.findByPage(page, sql);
			}else{
				page = abookDao.findAllByPage(page);
			}
			return page;
		}
		page = abookDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicAbook loadAbookByBussinessKey(Serializable bussinessKey){
		return abookDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicAbook> findByProperty(String propertyName, Object object){
		return abookDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicAbook findEntityByProperty(String propertyName, Object object){
		return abookDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicAbook.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = abookDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_addressBook_Abook","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_addressBook", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicAbook abook,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,abookDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicAbook.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, abook);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",abook.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicAbook.TABLE_NAME +" where valid = 1";
			list = abookDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicAbook.TABLE_NAME;
			list = abookDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicAbook.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = abookDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicAbookDaoImpl abookDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchAbook(final List<X6BasicAbook> insertObjs, final List<X6BasicAbook> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchAbook(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchAbook(final List<X6BasicAbook> insertObjs, final List<X6BasicAbook> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1406705125828"),importCount));
		...
		Map<Long, X6BasicAbook> oldUpdateObjsMap = new HashMap<Long, X6BasicAbook>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicAbook bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicAbook> bizObjList = abookDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicAbook bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicAbook> nmeObjs = new ArrayList<X6BasicAbook>();
		...
		long id=autoGeneratorID.getNextId(X6BasicAbook.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicAbook abook:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicAbook.TABLE_NAME,1,dbId);
			if(abook.getId() != null ){
				m.put(abook.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) abookDao.createNativeQuery(sql, "X6Basic_1.0_addressBook_Abook"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		abookDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicAbook.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicAbook abook = updateObjs.get(updateIndex);
						Long id = abook.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = abook.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicAbook.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicAbook.TABLE_NAME + " order by id desc";
//					Query query = abookDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicAbook abook = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = abook.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		abookDaoImpl.getSession().flush();
		abookDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicAbook bizObj = insertObjs.get(insertIndex);
			abookImportService.saveImportDataLog(bizObj, null, "id", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicAbook bizObj = updateObjs.get(updateIndex);
			X6BasicAbook oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			abookImportService.saveImportDataLog(bizObj, oldBizObj, "id", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = abookDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				abookDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = abookDao.createNativeQuery(sql, "X6Basic_1.0_addressBook_Abook").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = abookDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = abookDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = abookDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?abookDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  abookDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,abookDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="ec.flow.save")
	public void saveAbook(X6BasicAbook abook, DataGridService dataGridService){
		saveAbook(abook, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="ec.flow.save")
	public void saveAbook(X6BasicAbook abook, DataGridService dataGridService, String viewCode){
		saveAbook(abook, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook")
	public void saveAbook(X6BasicAbook abook, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveAbook(abook, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook")
	public void saveAbook(X6BasicAbook abook, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_addressBook";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_addressBook");
		String url = null;
		if(null != abook.getId() && abook.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicAbook.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook")
	public Map<Object, Object> abookDataGridImport(X6BasicAbook abook, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = abook.getVirtualId();
		
		abook = abookDao.load(abook.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, abook.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = abook.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(abook);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, abook.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook")
	public void mergeAbook(X6BasicAbook abook, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(abook);
		beforeSaveAbook(abook);
		abookDao.merge(abook);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_addressBook",model="X6Basic_1.0_addressBook_Abook",desc="foundation.batch.save",operType="4")
	public void batchSaveAbook(X6BasicAbook abook, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "X6Basic_1.0_addressBook";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_addressBook");
	...
	@Override
	public Page<X6BasicAbook> getByPage(Page<X6BasicAbook> page,DetachedCriteria detachedCriteria){
		return abookDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_addressBook").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveAbook(X6BasicAbook abook, Object... objects){
		checkUniqueConstraint(abook);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveAbook(X6BasicAbook abook, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportAbook(List<X6BasicAbook> abooks, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportAbook(List<X6BasicAbook> abooks){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportAbook(List<X6BasicAbook> abooks, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportAbook(List<X6BasicAbook> abooks){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteAbook(X6BasicAbook abook){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteAbook(X6BasicAbook abook){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreAbook(X6BasicAbook abook){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreAbook(X6BasicAbook abook){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicAbook abook, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicAbook abook, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportAbook(List<X6BasicAbook> insertObjs, List<X6BasicAbook> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportAbook,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportAbook(List<X6BasicAbook> insertObjs, List<X6BasicAbook> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportAbook,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
