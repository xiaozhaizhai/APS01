...
import com.supcon.orchid.X6Basic.services.X6BasicSyncLogWSService;
import com.supcon.orchid.X6Basic.services.X6BasicSyncLogService;
/* CUSTOM CODE START(wsserviceimpl,import,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="X6BasicSyncLog",targetNamespace = "http://ws.supcon.com")
public class X6BasicSyncLogWSServiceImpl implements X6BasicSyncLogWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getSyncLogByBusinessKey(
		...
		try{
			...
			X6BasicSyncLog syncLog = syncLogService.loadSyncLogByBussinessKey(businessKey);
			if(syncLog == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, syncLog);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateSyncLog(
		...
		try{
			if(syncLog == null){
				return set500Error(result, "同步日志表不能为空。");
			}
			...
			if(!beforeUpdateSyncLog(syncLog)){
				return set404Error(result);
			}
			syncLogService.saveSyncLog(syncLog, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addSyncLog(
		...
		try{
			...
			try{
				checkSyncLogData(syncLog);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			syncLogService.saveSyncLog(syncLog, null);
			setSuccessResult(result, syncLog.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteSyncLogByBusinessKey(
		...
		try{
			...
			try{
				syncLog = syncLogService.loadSyncLogByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddSyncLog(
		...
		try{
			...
			int count = 1;
			for(X6BasicSyncLog syncLog : syncLogList){
				if(syncLog.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(syncLog.getId() == null || syncLog.getId().toString().trim().isEmpty()){
			...
			syncLogService.batchImportBaseSyncLog(syncLogList);
			for(X6BasicSyncLog syncLog : syncLogList){
				idList.add(syncLog.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteSyncLogByPk(
		...
		try{
			...
			for(String k : pk){
				...
				X6BasicSyncLog temp = null;
				try{
					temp = syncLogService.getSyncLog(id);
				}catch(NullPointerException e){}
			...
			syncLogService.deleteSyncLog(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteSyncLogByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				X6BasicSyncLog temp = null;
				try{
					temp = syncLogService.loadSyncLogByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getSyncLogPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(X6BasicSyncLog.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),X6BasicSyncLog.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,X6BasicSyncLog.class);
				} catch (Exception e) {
			...
			Page<X6BasicSyncLog> page = new Page<X6BasicSyncLog>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<X6BasicSyncLog> datas = syncLogService.getByPage(page, detachedCriteria);
			List<X6BasicSyncLog> syncLogList = datas.getResult();
			if (syncLogList == null || syncLogList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(syncLogList.toArray());
			PaginationDTO<X6BasicSyncLog> pagination = new PaginationDTO<X6BasicSyncLog>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			X6BasicSyncLog syncLog = syncLogService.getSyncLog(id);
			if (syncLog == null || !syncLog.isValid()) {
				result.setErrorMessage("找不到同步日志表");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取同步日志表对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param syncLog
	 * @return
	 */
	private X6BasicSyncLog getSyncLog(X6BasicSyncLog syncLog) {
		if (syncLog.getId() == null && (syncLog.getId() == null || syncLog.getId().toString().trim() == ""))
			throw new RuntimeException("同步日志表对象业务主键和id不能同时为空");
		if (syncLog.getId() != null) {
			X6BasicSyncLog result = syncLogService.getSyncLog(syncLog .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return syncLogService.loadSyncLogByBussinessKey(syncLog.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个同步日志表，则返回false，否则返回true
	 * @param syncLog
	 */
	private boolean beforeUpdateSyncLog(X6BasicSyncLog syncLog) {
		X6BasicSyncLog temp = getSyncLog(syncLog);
		if (temp == null) {
			return false;
		}
		if(syncLog.getId() == null){
			syncLog.setId(temp.getId());		
		}
		if(syncLog.getId() == null){
			syncLog.setId(temp.getId());
		}
		
		syncLog.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除同步日志表对象下所有辅模型对象中的同步日志表对象
	 * @param syncLog
	 */
	private void clearCircle(X6BasicSyncLog syncLog) {
	 ...
	 */
	private void checkSyncLogData(X6BasicSyncLog syncLog){
	}
	/* CUSTOM CODE START(wsserviceimpl,functions,X6Basic_1.0_syncLog_SyncLog,X6Basic_1.0) */
// 自定义代码
