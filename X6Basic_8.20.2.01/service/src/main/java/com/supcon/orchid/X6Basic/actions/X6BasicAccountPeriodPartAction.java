...
/* CUSTOM CODE START(action,import,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicAccountPeriodPartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicAccountPeriodPart> {
	...
	private X6BasicAccountPeriodPart accountPeriodPart;
	...
	//导出excel时,主模型内容
	List<X6BasicAccountPeriodPart> queryResults = new ArrayList<X6BasicAccountPeriodPart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/accountingPeriod/accountPeriodPart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/accountingPeriod/accountPeriodPart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_accountingPeriod_AccountPeriodPart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicAccountPeriodPart> accountPeriodParts;
	
	public List<X6BasicAccountPeriodPart> getAccountPeriodParts() {
		return accountPeriodParts;
	}
	...
	private Page<X6BasicAccountPeriodPart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			accountPeriodPart = new X6BasicAccountPeriodPart();
			if(accountPeriodPart.getMonth() != null) {
				accountPeriodPart.setMonth((SystemCode) systemCodeService.getSystemCode(accountPeriodPart.getMonth().getId()));
			}
		} else {
			if(id > 0){
				accountPeriodPart = accountPeriodPartService.getAccountPeriodPart(id, viewCode);
			}
		}
	...
	public Page<X6BasicAccountPeriodPart> getPage() {
		if (null == page) {
			page = new Page<X6BasicAccountPeriodPart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicAccountPeriodPart getAccountPeriodPart() {
		return accountPeriodPart;
	}		

	public void setAccountPeriodPart(X6BasicAccountPeriodPart accountPeriodPart) {
		this.accountPeriodPart = accountPeriodPart;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = accountPeriodPartService.getAccountPeriodPartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1400468245576
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String accountPeriodPartReferenceCopy() throws Exception{
			X6BasicAccountPeriodPart oldAccountPeriodPart = null;
			if(isEditNew()){
				...
				oldAccountPeriodPart = new X6BasicAccountPeriodPart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,accountPeriodPart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, accountPeriodPart.getId());
				...
				oldAccountPeriodPart = accountPeriodPartService.getAccountPeriodPart(id);
			}
			
			
			accountPeriodPart = accountPeriodPartService.getAccountPeriodPart(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_accountingPeriod_AccountPeriodPart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicAccountPeriodPart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldAccountPeriodPart, null);
						Method setValue = X6BasicAccountPeriodPart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(accountPeriodPart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			accountPeriodPart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=accountPeriodPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return accountPeriodPartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			accountPeriodPart = accountPeriodPartService.getAccountPeriodPart(this.accountPeriodPart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,accountPeriodPart.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			accountPeriodPartService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = accountPeriodPartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(accountPeriodPartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(accountPeriodPartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_accountingPeriod_AccountPeriodPart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"accountPeriodPart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"accountPeriodPart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"accountPeriodPart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"accountPeriodPart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			accountPeriodPartService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		accountPeriodPartService.findDataGridPage(dg,dgclass,getDgPage(),accountPeriodPart,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(accountPeriodPart.getId() != null && accountPeriodPart.getId() != -1){
				accountPeriodPart = accountPeriodPartService.getAccountPeriodPart(accountPeriodPart.getId());
			}
			accountPeriodPartService.findDataGridPage(dg,dgclass,getDgPage(),accountPeriodPart,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == accountPeriodPart.getId()) {
		...
		accountPeriodPartService.saveAccountPeriodPart(accountPeriodPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "submit");
		bap_validate_datagrids=accountPeriodPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", accountPeriodPart.getId());
		...
		fileuploads.put("linkId", accountPeriodPart.getId());
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		accountPeriodPartService.deleteAccountPeriodPart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "restore");
		accountPeriodPartService.restoreAccountPeriodPart(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicAccountPeriodPart> findExportDataInfos(Page<X6BasicAccountPeriodPart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = accountPeriodPartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = accountPeriodPartService.getAssProperty(p.getCode());	
									//String assModelCode = accountPeriodPartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			accountPeriodPartService.findAccountPeriodParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			accountPeriodPartService.findAccountPeriodParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicAccountPeriodPart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicAccountPeriodPart result : results){
	...
	public boolean isEditNew(){
		//return null == this.accountPeriodPart || null == this.accountPeriodPart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/accountingPeriod/accountPeriodPart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/accountingPeriod/accountPeriodPart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicAccountPeriodPartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicAccountPeriodPartService"
						, "com.supcon.orchid.X6Basic.services.X6BasicAccountPeriodService"       
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_accountingPeriod_AccountPeriodPart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_accountingPeriod_AccountPeriodPart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_accountingPeriod_AccountPeriodPart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_accountingPeriod_AccountPeriodPart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
		_auditLog.setColumnStringA(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
		_auditLog.setColumnStringC(accountPeriodPart.getId().toString());
		_auditLog.setColumnStringD(X6BasicAccountPeriodPart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicAccountPeriodPartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, accountPeriodPart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, accountPeriodPart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,accountPeriodPart);
		...
		printlog.setBusinessId(accountPeriodPart.getId());
		printlog.setBussinessKey(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
		printlog.setMainDisplay(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(accountPeriodPart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(accountPeriodPartService.getAccountPeriodPart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, accountPeriodPart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				accountPeriodPart = accountPeriodPartService.getAccountPeriodPart(Long.parseLong(arrId[i]));
				if(null != accountPeriodPart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == accountPeriodPart.getId() ? "" : "," + accountPeriodPart.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == accountPeriodPart.getId() ? "" : "," + accountPeriodPart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == accountPeriodPart.getId() ? "" : "," + accountPeriodPart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == accountPeriodPart.getId() ? "" : accountPeriodPart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, accountPeriodPart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,accountPeriodPart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//accountPeriodPartService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/X6Basic/accountingPeriod/accountPeriodPart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_accountingPeriod_AccountPeriodPart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_accountingPeriod_AccountPeriodPart")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_accountingPeriod_AccountPeriodPart");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_accountingPeriod_AccountPeriodPart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = accountPeriodPartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_accountingPeriod_AccountPeriodPart,X6Basic_1.0) */
// 自定义代码
