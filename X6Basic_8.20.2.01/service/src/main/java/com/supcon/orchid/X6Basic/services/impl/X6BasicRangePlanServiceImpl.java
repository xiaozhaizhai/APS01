...
import com.supcon.orchid.X6Basic.services.X6BasicRangePlanImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicRangePlanDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_rangePlanService")
@Transactional
public class X6BasicRangePlanServiceImpl  extends BaseServiceImpl<X6BasicRangePlan> implements X6BasicRangePlanService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicRangePlan> bapBaseService;
	...
	@Autowired
	private X6BasicRangePlanImportService rangePlanImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicRangePlan.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  rangePlanDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicRangePlan.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = rangePlanDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicRangePlan.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicRangePlan.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  rangePlanDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicRangePlan.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicRangePlan.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicRangePlan> list = (List<X6BasicRangePlan>) rangePlanDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicRangePlan getRangePlan(long id){
		return getRangePlan(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicRangePlan getRangePlan(long id, String viewCode){
		X6BasicRangePlan rangePlan = rangePlanDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getRangePlanAsJSON(long id, String include){
		X6BasicRangePlan rangePlan = rangePlanDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(rangePlan, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.common.delete",operType="3")
	public void deleteRangePlan(X6BasicRangePlan rangePlan){
		if(null != rangePlan && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlan.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
			AuditUtil.setColumnStringA(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
			AuditUtil.setColumnStringC(rangePlan.getId().toString());
		}
		beforeDeleteRangePlan(rangePlan);
		rangePlanDao.delete(rangePlan);
		afterDeleteRangePlan(rangePlan);
		rangePlanDao.flush();
		List<X6BasicRangePlan> params = new ArrayList<X6BasicRangePlan>();
		params.add(rangePlan);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.common.delete",operType="3")
	public void deleteRangePlan(List<Long> rangePlanIds){
		deleteRangePlan(rangePlanIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.common.delete",operType="3")
	public void deleteRangePlan(List<Long> rangePlanIds, String eventTopic) {
		List<X6BasicRangePlan> rangePlans = new ArrayList<X6BasicRangePlan>();
		for(Long rangePlanId : rangePlanIds){
			X6BasicRangePlan rangePlan = getRangePlan(rangePlanId);
			rangePlans.add(rangePlan);
			if(null != rangePlan && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlan.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == rangePlan.getId() ? "" : "," + rangePlan.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == rangePlan.getId() ? "" : "," + rangePlan.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == rangePlan.getId() ? "" : "," + rangePlan.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(rangePlans != null && rangePlans.size() > 0 ){
			for(X6BasicRangePlan rangePlan : rangePlans){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),rangePlan, rangePlan.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(rangePlanIds != null && rangePlanIds.size() > 0) {
			String hql = "update " + X6BasicRangePlan.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = rangePlanDao.createQuery(hql);
			query.setParameterList("ids", rangePlanIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicRangePlan rangePlanz : rangePlans){
				rangePlanz.setValid(false);
				List<X6BasicRangePlan> params = new ArrayList<X6BasicRangePlan>();
				params.add(rangePlanz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_rangePlan");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.common.delete",operType="3")
	public void deleteRangePlan(long rangePlanId, int rangePlanVersion){
		this.deleteRangePlan(rangePlanId, rangePlanVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.common.delete",operType="3")
	public void deleteRangePlan(long rangePlanId, int rangePlanVersion,SignatureLog signatureLog){
		X6BasicRangePlan rangePlan = getRangePlan(rangePlanId);
		...
		beforeDeleteRangePlan(rangePlan);
		rangePlanDao.delete(rangePlanId, rangePlanVersion);
		afterDeleteRangePlan(rangePlan);
		rangePlanDao.flush();
		List<X6BasicRangePlan> params = new ArrayList<X6BasicRangePlan>();
		params.add(rangePlan);
		...
		if(signatureLog != null ) {
			...
			businessKey=rangePlan.getId();
			signatureLog.setTableId(rangePlan.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.common.delete",operType="3")
	public void deleteRangePlan(String rangePlanIds){
		this.deleteRangePlan(rangePlanIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.common.delete",operType="3")
	public void deleteRangePlan(String rangePlanIds,SignatureLog signatureLog){
		deleteCollection(rangePlanIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteRangePlan(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_rangePlan#modelCode:X6Basic_1.0_rangePlan_RangePlan#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="ec.engine.restore",operType="10")
	public void restoreRangePlan(String rangePlanIds){
		restoreCollection(rangePlanIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreRangePlan(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="ec.engine.restore",operType="10")
	public void restoreRangePlan(long rangePlanId){
		
		findBusinessKeyUsed(rangePlanId);	//判断业务主键是否重复
		...
		beforeRestoreRangePlan(rangePlan);
		rangePlan.setValid(true);
		rangePlanDao.update(rangePlan);
		afterRestoreRangePlan(rangePlan);
		List<X6BasicRangePlan> params = new ArrayList<X6BasicRangePlan>();
		params.add(rangePlan);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long rangePlanId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_rangePlan_RangePlan");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicRangePlan.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicRangePlan.TABLE_NAME +" where id = ? )";
			List<Object> list =  rangePlanDao.createNativeQuery(sql,rangePlanId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseRangePlan(List<X6BasicRangePlan>  rangePlans){
		for(X6BasicRangePlan rangePlan:rangePlans)  {
			saveRangePlan(rangePlan, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseRangePlan(List<X6BasicRangePlan>  rangePlans){
		for(X6BasicRangePlan rangePlan:rangePlans)  {
			saveRangePlan(rangePlan, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="ec.flow.save")
	public void saveRangePlan(X6BasicRangePlan rangePlan, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveRangePlan(rangePlan, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="ec.flow.save")
	public void saveRangePlan(X6BasicRangePlan rangePlan, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveRangePlan(rangePlan, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateRangePlanCodes(X6BasicRangePlan rangePlan) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateRangePlanCodes(rangePlan, false);
	}
	
	public void generateRangePlanCodes(X6BasicRangePlan rangePlan, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateRangePlanSummarys(X6BasicRangePlan rangePlan) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateRangePlanSummarys(rangePlan, false);
	}

	public void generateRangePlanSummarys(X6BasicRangePlan rangePlan, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan")
	public void saveRangePlan(X6BasicRangePlan rangePlan,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveRangePlan(rangePlan, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan")
	public void saveRangePlan(X6BasicRangePlan rangePlan,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "X6Basic_1.0_rangePlan");
		String url = null;
		if(rangePlan.getId() != null && rangePlan.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlan.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, rangePlan);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(rangePlan);
		beforeSaveRangePlan(rangePlan, viewIsView);

		if (viewIsView) {
			rangePlanDao.saveWithRevertVersion(rangePlan);
		} else {
			if(isNew)
				rangePlanDao.save(rangePlan);
			else
				rangePlanDao.merge(rangePlan);
		}
		if(null != rangePlan && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlan.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
			AuditUtil.setColumnStringA(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
		...
		dealDatagridsSave(rangePlan,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateRangePlanCodes(rangePlan, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateRangePlanSummarys(rangePlan, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveRangePlan(rangePlan, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, rangePlan);
			}
		}
		List<X6BasicRangePlan> params = new ArrayList<X6BasicRangePlan>();
		rangePlanDao.flush();
		rangePlanDao.clear();
		rangePlan = rangePlanDao.load(rangePlan.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicRangePlan rangePlan){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("rangePlan", rangePlan);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan")
	public void mergeRangePlan(X6BasicRangePlan rangePlan, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(rangePlan);
		beforeSaveRangePlan(rangePlan);
		rangePlanDao.merge(rangePlan);
		if(null != rangePlan && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlan.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
			AuditUtil.setColumnStringA(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
		...
		dealDatagridsSave(rangePlan,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveRangePlan(rangePlan);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicRangePlan> findRangePlans(Page<X6BasicRangePlan> page, Criterion... criterions) {
		return rangePlanDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicRangePlanDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicRangePlanDealInfo> list=rangePlanDao.createQuery("from " + X6BasicRangePlanDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	public void saveDealInfo(X6BasicRangePlanDealInfo dealInfo) {
		rangePlanDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicRangePlanDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) rangePlanDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicRangePlanDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = rangePlanDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = rangePlanDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicRangePlanDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = rangePlanDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRangePlans(Page<X6BasicRangePlan> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findRangePlans(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRangePlans(Page<X6BasicRangePlan> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRangePlans(Page<X6BasicRangePlan> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicRangePlan> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"rangePlan\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							...
							s.append(" AND \"rangePlan\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"rangePlan\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"rangePlan\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"rangePlan\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_rangePlan_RangePlan" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_rangePlan_RangePlan" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  rangePlanDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  rangePlanDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_rangePlan").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rangePlan\"","X6Basic_1.0_rangePlan",customCondition,"S2BASE_RANGEPLAN",true);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"rangePlan\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicRangePlanDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"rangePlan\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"rangePlan\".LAY_REC ASC, \"rangePlan\".SORT ASC");
				} else {
					orderPart.append(" \"rangePlan\".RP_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) rangePlanDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=rangePlanDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = rangePlanDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicRangePlan> result = new ArrayList<X6BasicRangePlan>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicRangePlan.class,rangePlanDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicRangePlan.class,rangePlanDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicRangePlan rangePlan : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(rangePlan.getId(), "X6Basic_rangePlan_rangePlan");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(rangePlan.getId(), "X6Basic_rangePlan_rangePlan");
							if(null != documents && !documents.isEmpty()) {
								rangePlan.setDocument(documents.get(0));
								rangePlan.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicRangePlan> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicRangePlan> result) {
		View view = null;
		...
		Map<Long, X6BasicRangePlan> rsMap = new HashMap<Long, X6BasicRangePlan>();
		for (X6BasicRangePlan rangePlan : result) {
			ids.add(rangePlan.getId());
			rsMap.put(rangePlan.getId(), rangePlan);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(rangePlanDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = rangePlanDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicRangePlan rangePlan,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(rangePlan.getOwnerStaffId());
		ti.setOwnerPositionId(rangePlan.getOwnerPositionId());
		ti.setOwnerDepartmentId(rangePlan.getOwnerDepartmentId());
		
		ti.setCreateStaff(rangePlan.getCreateStaff());
		ti.setCreateStaffId(rangePlan.getCreateStaffId());
		ti.setCreateTime(rangePlan.getCreateTime());
		ti.setDeleteStaff(rangePlan.getDeleteStaff());
		ti.setDeleteStaffId(rangePlan.getDeleteStaffId());
		ti.setDeleteTime(rangePlan.getDeleteTime());
		ti.setCreateDepartmentId(rangePlan.getCreateDepartmentId());
		ti.setModifyStaff(rangePlan.getModifyStaff());
		ti.setModifyStaffId(rangePlan.getModifyStaffId());
		ti.setCreatePositionId(rangePlan.getCreatePositionId());
		ti.setPositionLayRec(rangePlan.getPositionLayRec());
		ti.setTableNo(rangePlan.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_rangePlan");
		ti.setTargetTableName("S2BASE_RANGEPLAN");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_rangePlan_RangePlan" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  rangePlanDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rangePlan\"","X6Basic_1.0_rangePlan","","S2BASE_RANGEPLAN",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"rangePlan\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"rangePlan\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"rangePlan\"","X6Basic_1.0_rangePlan","","S2BASE_RANGEPLAN",true);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("rangePlan.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"rangePlan\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"rangePlan\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"rangePlan\".CID = 1 OR \"rangePlan\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("rangePlan.mc\"").append(".RANGE_PLAN) not in ("+notinValue+")");
			}
			
			SQLQuery query = rangePlanDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=rangePlanDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicRangePlan.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicRangePlan.MODULE_CODE;}
	protected String getModelCode(){return X6BasicRangePlan.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param rangePlan 模型实例
	 */
	public void checkUniqueConstraint(X6BasicRangePlan rangePlan){
	}
	
	
	@Override
	public List<X6BasicRangePlan> getRangePlans(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicRangePlan.class);
		if(params!=null){
			...
			return rangePlanDao.findByHql("from " + X6BasicRangePlan.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return rangePlanDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicRangePlan> findRangePlansByHql(String hql, Object... objects){
		...
		return   rangePlanDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicRangePlan loadRangePlanByBussinessKey(X6BasicRangePlan rangePlan){
		return loadRangePlanByBussinessKey(rangePlan.getId());
	}
	
	@Override
	public Page<X6BasicRangePlan> getRangePlans(Page<X6BasicRangePlan> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicRangePlan.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = rangePlanDao.findByPage(page, sql);
			}else{
				page = rangePlanDao.findAllByPage(page);
			}
			return page;
		}
		page = rangePlanDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public X6BasicRangePlan loadRangePlanByBussinessKey(Serializable bussinessKey){
		return rangePlanDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicRangePlan> findByProperty(String propertyName, Object object){
		return rangePlanDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicRangePlan findEntityByProperty(String propertyName, Object object){
		return rangePlanDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicRangePlan.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = rangePlanDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_rangePlan_RangePlan","runtime");
		try {
	...
	public void dealDatagridsSave(X6BasicRangePlan rangePlan,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,rangePlanDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicRangePlan.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, rangePlan);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",rangePlan.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicRangePlan.TABLE_NAME +" where valid = 1";
			list = rangePlanDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicRangePlan.TABLE_NAME;
			list = rangePlanDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicRangePlan.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = rangePlanDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicRangePlanDaoImpl rangePlanDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchRangePlan(final List<X6BasicRangePlan> insertObjs, final List<X6BasicRangePlan> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchRangePlan(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchRangePlan(final List<X6BasicRangePlan> insertObjs, final List<X6BasicRangePlan> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1398308731380"),importCount));
		...
		Map<Long, X6BasicRangePlan> oldUpdateObjsMap = new HashMap<Long, X6BasicRangePlan>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicRangePlan bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicRangePlan> bizObjList = rangePlanDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicRangePlan bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicRangePlan> nmeObjs = new ArrayList<X6BasicRangePlan>();
		...
		long id=autoGeneratorID.getNextId(X6BasicRangePlan.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicRangePlan rangePlan:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicRangePlan.TABLE_NAME,1,dbId);
			if(rangePlan.getId() != null ){
				m.put(rangePlan.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) rangePlanDao.createNativeQuery(sql, "X6Basic_1.0_rangePlan_RangePlan"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		rangePlanDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicRangePlan.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicRangePlan rangePlan = updateObjs.get(updateIndex);
						Long id = rangePlan.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = rangePlan.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicRangePlan.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicRangePlan.TABLE_NAME + " order by id desc";
//					Query query = rangePlanDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicRangePlan rangePlan = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = rangePlan.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		rangePlanDaoImpl.getSession().flush();
		rangePlanDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicRangePlan bizObj = insertObjs.get(insertIndex);
			rangePlanImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicRangePlan bizObj = updateObjs.get(updateIndex);
			X6BasicRangePlan oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			rangePlanImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = rangePlanDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				rangePlanDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = rangePlanDao.createNativeQuery(sql, "X6Basic_1.0_rangePlan_RangePlan").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = rangePlanDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = rangePlanDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = rangePlanDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?rangePlanDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  rangePlanDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,rangePlanDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="ec.flow.save")
	public void saveRangePlan(X6BasicRangePlan rangePlan, DataGridService dataGridService){
		saveRangePlan(rangePlan, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="ec.flow.save")
	public void saveRangePlan(X6BasicRangePlan rangePlan, DataGridService dataGridService, String viewCode){
		saveRangePlan(rangePlan, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan")
	public void saveRangePlan(X6BasicRangePlan rangePlan, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveRangePlan(rangePlan, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan")
	public void saveRangePlan(X6BasicRangePlan rangePlan, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_rangePlan";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_rangePlan");
		String url = null;
		if(null != rangePlan.getId() && rangePlan.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicRangePlan.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan")
	public Map<Object, Object> rangePlanDataGridImport(X6BasicRangePlan rangePlan, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = rangePlan.getVirtualId();
		
		rangePlan = rangePlanDao.load(rangePlan.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, rangePlan.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = rangePlan.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(rangePlan);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, rangePlan.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan")
	public void mergeRangePlan(X6BasicRangePlan rangePlan, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(rangePlan);
		beforeSaveRangePlan(rangePlan);
		rangePlanDao.merge(rangePlan);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_rangePlan",model="X6Basic_1.0_rangePlan_RangePlan",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicRangePlan> getByPage(Page<X6BasicRangePlan> page,DetachedCriteria detachedCriteria){
		return rangePlanDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveRangePlan(X6BasicRangePlan rangePlan, Object... objects){
		checkUniqueConstraint(rangePlan);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveRangePlan(X6BasicRangePlan rangePlan, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteRangePlan(X6BasicRangePlan rangePlan){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteRangePlan(X6BasicRangePlan rangePlan){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreRangePlan(X6BasicRangePlan rangePlan){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreRangePlan(X6BasicRangePlan rangePlan){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicRangePlan rangePlan, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicRangePlan rangePlan, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportRangePlan(List<X6BasicRangePlan> insertObjs, List<X6BasicRangePlan> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportRangePlan,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportRangePlan(List<X6BasicRangePlan> insertObjs, List<X6BasicRangePlan> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportRangePlan,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicRangePlan> findByHql(String hql,Object...objects){
		return rangePlanDao.findByHql(hql, objects);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicRangePlan findEntityByHql(String hql,Object...objects){
		return rangePlanDao.findEntityByHql(hql, objects);
	}
/* CUSTOM CODE END */
}
