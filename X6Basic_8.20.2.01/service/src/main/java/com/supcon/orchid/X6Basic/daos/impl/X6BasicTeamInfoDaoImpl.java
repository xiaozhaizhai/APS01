...
@Repository
public class X6BasicTeamInfoDaoImpl extends ExtendedGenericDaoImpl<X6BasicTeamInfo, Long> implements X6BasicTeamInfoDao {
	@Override
	...
	@Override
	public X6BasicTeamInfoDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicTeamInfoDealInfo dealInfo = new X6BasicTeamInfoDealInfo();
	...
	@Override
	public X6BasicTeamInfoDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicTeamInfo mainObj, Staff dealStaff) {
		X6BasicTeamInfoDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicTeamInfoDealInfo.class);
		...
		List<X6BasicTeamInfoDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_teamManage_TeamInfo,X6Basic_1.0) */
// 自定义代码
