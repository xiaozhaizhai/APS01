...
@Repository
public class ProductManageDaoImpl extends ExtendedGenericDaoImpl<ProductManage, Long> implements ProductManageDao {
	@Override
	...
	@Override
	public ProductManageDealInfo copyAndSaveDealInfo(DealInfo di) {
		ProductManageDealInfo dealInfo = new ProductManageDealInfo();
	...
	@Override
	public ProductManageDealInfo copyAndSaveDealInfo(DealInfo di, ProductManage mainObj, Staff dealStaff) {
		ProductManageDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(ProductManageDealInfo.class);
		...
		List<ProductManageDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
