...
@Repository
public class X6BasicUnitGroupsDaoImpl extends ExtendedGenericDaoImpl<X6BasicUnitGroups, Long> implements X6BasicUnitGroupsDao {
	@Override
	...
	@Override
	public X6BasicUnitGroupsDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicUnitGroupsDealInfo dealInfo = new X6BasicUnitGroupsDealInfo();
	...
	@Override
	public X6BasicUnitGroupsDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicUnitGroups mainObj, Staff dealStaff) {
		X6BasicUnitGroupsDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicUnitGroupsDealInfo.class);
		...
		List<X6BasicUnitGroupsDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_unitGroup_UnitGroups,X6Basic_1.0) */
// 自定义代码
