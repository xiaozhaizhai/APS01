...
/* CUSTOM CODE START(action,import,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicTimeRegionAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicTimeRegion> {
	...
	private X6BasicTimeRegion timeRegion;
	...
	//导出excel时,主模型内容
	List<X6BasicTimeRegion> queryResults = new ArrayList<X6BasicTimeRegion>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/timeRegion/timeRegion/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/timeRegion/timeRegion/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_timeRegion_TimeRegion" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicTimeRegion> timeRegions;
	
	public List<X6BasicTimeRegion> getTimeRegions() {
		return timeRegions;
	}
	...
	private Page<X6BasicTimeRegion> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			timeRegion = new X6BasicTimeRegion();
			if(timeRegion.getUnit() != null) {
				timeRegion.setUnit((SystemCode) systemCodeService.getSystemCode(timeRegion.getUnit().getId()));
			}
			if(timeRegion.getUseEntity() != null) {
				timeRegion.setUseEntity((SystemCode) systemCodeService.getSystemCode(timeRegion.getUseEntity().getId()));
			}
		} else {
			if(id > 0){
				timeRegion = timeRegionService.getTimeRegion(id, viewCode);
			}
		}
	...
	public Page<X6BasicTimeRegion> getPage() {
		if (null == page) {
			page = new Page<X6BasicTimeRegion>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicTimeRegion getTimeRegion() {
		return timeRegion;
	}		

	public void setTimeRegion(X6BasicTimeRegion timeRegion) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/timeRegion/timeRegion/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = timeRegionService.getTimeRegionAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/timeRegion/timeRegion/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.randon1459213973062
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String timeRegionReferenceCopy() throws Exception{
			X6BasicTimeRegion oldTimeRegion = null;
			if(isEditNew()){
				...
				oldTimeRegion = new X6BasicTimeRegion();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,timeRegion.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, timeRegion.getId());
				...
				oldTimeRegion = timeRegionService.getTimeRegion(id);
			}
			
			
			timeRegion = timeRegionService.getTimeRegion(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_timeRegion_TimeRegion");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicTimeRegion.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTimeRegion, null);
						Method setValue = X6BasicTimeRegion.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(timeRegion, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			timeRegion.setCreateDepartmentId(oldTimeRegion.getCreateDepartmentId());
			timeRegion.setCreatePositionId(oldTimeRegion.getCreatePositionId());
			timeRegion.setCreateStaffId(oldTimeRegion.getCreateStaffId());
			timeRegion.setOwnerDepartmentId(oldTimeRegion.getOwnerDepartmentId());
			timeRegion.setOwnerPositionId(oldTimeRegion.getOwnerPositionId());
			timeRegion.setOwnerStaffId(oldTimeRegion.getOwnerStaffId());
			timeRegion.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegion/timeRegionEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "timeRegionEdit.ftl") }),
		@Action(value = "/X6Basic/timeRegion/timeRegion/timeRegionEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "timeRegionEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=timeRegionService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return timeRegionReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			timeRegion = timeRegionService.getTimeRegion(this.timeRegion.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,timeRegion.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegion/timeReginList", results = { @Result(type = FREEMARKER, location = "timeReginList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = timeRegionService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(timeRegionService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(timeRegionService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_timeRegion_TimeRegion", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegion/timeReginList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.unit.id,result.unit.value,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="timeReginList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"timeRegion\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"timeRegion\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"timeRegion\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"timeRegion\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/timeRegion/timeRegion/timeReginList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/timeRegion/timeRegion/allTempleteDownload", results = { @Result(name="excel",type="excel",location="timeReginList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_timeRegion_TimeRegion");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		timeRegionService.findDataGridPage(dg,dgclass,getDgPage(),timeRegion,"", null);
	...
	private Page<X6BasicTimeRegionPar> dg1459214611112Page;
	public Page<X6BasicTimeRegionPar> getDg1459214611112Page(){
		if(null == dg1459214611112Page){
			dg1459214611112Page = new Page<X6BasicTimeRegionPar>(1,20);
		}
		return dg1459214611112Page;
	}
	
	private Class dg1459214611112Clazz = X6BasicTimeRegionPar.class;
	
	public Class getDg1459214611112Clazz() {
		return dg1459214611112Clazz;
	}
	
	private List<X6BasicTimeRegionPar> dg1459214611112List;
	public List<X6BasicTimeRegionPar> getDg1459214611112List(){
		if(null == dg1459214611112List){
			dg1459214611112List = new ArrayList<X6BasicTimeRegionPar>();
		}
		return dg1459214611112List;
	}
	public void setDg1459214611112ListJson(String json) {
		//dg1459214611112List = (List<X6BasicTimeRegionPar>) timeRegionService.generateObjectFromJson(json, X6BasicTimeRegionPar.class);
	}
	
	private List<Long> dg1459214611112DeletedIds;
	public List<Long> getDg1459214611112DeletedIds(){
		if(null == dg1459214611112DeletedIds){
			dg1459214611112DeletedIds = new ArrayList<Long>();
		}
		return dg1459214611112DeletedIds;
	}

	@Actions({
		
		@Action(value = "/X6Basic/timeRegion/timeRegion/data-dg1459214611112", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.startDate,result.endDate,result.year,result.quarter,result.month,result.week,result.id,result.version,result.timeRegionID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1459214611112-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1459214611112.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(timeRegion.getId() != null && timeRegion.getId() != -1){
				timeRegion = timeRegionService.getTimeRegion(timeRegion.getId());
			}
			timeRegionService.findDataGridPage(dg,dgclass,getDgPage(),timeRegion,searchCondition,params);
	...
	private void save() throws Exception {
		...
		timeRegionService.saveTimeRegion(timeRegion,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/timeRegion/timeRegion/timeRegionEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "submit");
		bap_validate_datagrids=timeRegionService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", timeRegion.getId());
		...
		fileuploads.put("linkId", timeRegion.getId());
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/timeRegion/timeRegion/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		timeRegionService.deleteTimeRegion(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegion/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "restore");
		timeRegionService.restoreTimeRegion(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/timeRegion/timeRegion/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/timeRegion/timeRegion/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "showDealInfoData");
		if (null != tableInfoId) {
			int count=timeRegionService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = timeRegionService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = timeRegionService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicTimeRegion> findExportDataInfos(Page<X6BasicTimeRegion> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = timeRegionService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = timeRegionService.getAssProperty(p.getCode());	
									//String assModelCode = timeRegionService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			timeRegionService.findTimeRegions(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			timeRegionService.findTimeRegions(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicTimeRegion> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicTimeRegion result : results){
	...
	public boolean isEditNew(){
		//return null == this.timeRegion || null == this.timeRegion.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/timeRegion/timeRegion/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/timeRegion/timeRegion/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/timeRegion/timeRegion/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegion/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicTimeRegionService"
						, "com.supcon.orchid.X6Basic.services.X6BasicTimeRegionParService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_timeRegion", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_timeRegion");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegion/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_timeRegion_TimeRegion", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_timeRegion_TimeRegion", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/timeRegion/timeRegion/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/timeRegion/timeRegion/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_timeRegion_TimeRegion";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_timeRegion_TimeRegion", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/timeRegion/timeRegion/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
		_auditLog.setColumnStringA(null == timeRegion.getName() ? "" : timeRegion.getName().toString());
		_auditLog.setColumnStringC(timeRegion.getId().toString());
		_auditLog.setColumnStringD(X6BasicTimeRegion.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_timeRegion");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicTimeRegionAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, timeRegion,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, timeRegion,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,timeRegion);
		...
		printlog.setEntity("X6Basic_1.0_timeRegion");
		...
		printlog.setBusinessId(timeRegion.getId());
		printlog.setBussinessKey(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
		printlog.setMainDisplay(null == timeRegion.getName() ? "" : timeRegion.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegion/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(timeRegion,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegion/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(timeRegionService.getTimeRegion(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/timeRegion/timeRegion/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/timeRegion/timeRegion/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, timeRegion,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				timeRegion = timeRegionService.getTimeRegion(Long.parseLong(arrId[i]));
				if(null != timeRegion){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == timeRegion.getName() ? "" : "," + timeRegion.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == timeRegion.getName() ? "" : timeRegion.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == timeRegion.getId() ? "" : "," + timeRegion.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == timeRegion.getId() ? "" : "," + timeRegion.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, timeRegion,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,timeRegion);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/timeRegion/timeRegion/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicTimeRegion> findExportPageInfos(Page<X6BasicTimeRegion> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_timeRegion" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = timeRegionService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicTimeRegion queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = timeRegionService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = timeRegionService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_timeRegion_TimeRegion");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_timeRegion_TimeRegion")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_timeRegion_TimeRegion");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_timeRegion_TimeRegion");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = timeRegionService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
	...
	String timeRePatrIds;

	public String getTimeRePatrIds() {
		return timeRePatrIds;
	}

	public void setTimeRePatrIds(String timeRePatrIds) {
		this.timeRePatrIds = timeRePatrIds;
	}
	
	@Action(value = "/X6Basic/timeRegion/timeRegion/delX6BasicAccountPeriodPart", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delX6BasicAccountPeriodPart() {
		timeRegionService.delX6BasicAccountPeriodPart(timeRePatrIds);
		responseMap.put("dealSuccessFlag", true);
		return SUCCESS;
	}
/* CUSTOM CODE END */
