...
/* CUSTOM CODE START(action,import,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicBaseUnitAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicBaseUnit> {
	...
	//导出excel时,主模型内容
	List<X6BasicBaseUnit> queryResults = new ArrayList<X6BasicBaseUnit>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/unitGroup/baseUnit/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/unitGroup/baseUnit/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_unitGroup_BaseUnit" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicBaseUnit> baseUnits;
	
	public List<X6BasicBaseUnit> getBaseUnits() {
		return baseUnits;
	}
	...
	private Page<X6BasicBaseUnit> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			baseUnit = new X6BasicBaseUnit();
		} else {
			if(id > 0){
				baseUnit = baseUnitService.getBaseUnit(id, viewCode);
			}
		}
	...
	public Page<X6BasicBaseUnit> getPage() {
		if (null == page) {
			page = new Page<X6BasicBaseUnit>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/unitGroup/baseUnit/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = baseUnitService.getBaseUnitAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/unitGroup/baseUnit/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1398235403787
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String baseUnitReferenceCopy() throws Exception{
			X6BasicBaseUnit oldBaseUnit = null;
			if(isEditNew()){
				...
				oldBaseUnit = new X6BasicBaseUnit();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseUnit.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, baseUnit.getId());
				...
				oldBaseUnit = baseUnitService.getBaseUnit(id);
			}
			
			
			baseUnit = baseUnitService.getBaseUnit(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_unitGroup_BaseUnit");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicBaseUnit.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBaseUnit, null);
						Method setValue = X6BasicBaseUnit.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(baseUnit, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			baseUnit.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=baseUnitService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return baseUnitReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			baseUnit = baseUnitService.getBaseUnit(this.baseUnit.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseUnit.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/baseUnit/unitList", results = { @Result(type = FREEMARKER, location = "unitList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = baseUnitService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(baseUnitService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(baseUnitService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_unitGroup_BaseUnit", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/baseUnit/unitRef", results = { @Result(type = FREEMARKER, location = "unitRef.ftl") })
		,
		@Action(value = "/X6Basic/unitGroup/baseUnit/unitRef2", results = { @Result(type = FREEMARKER, location = "unitRef2.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/baseUnit/unitList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.isMain,result.comments,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="unitList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"baseUnit\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"baseUnit\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"baseUnit\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"baseUnit\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/baseUnit/unitList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/unitGroup/baseUnit/unitRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.rate,result.isMain,result.comments,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="unitRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/X6Basic/unitGroup/baseUnit/unitRef2-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.rate,result.isMain,result.comments,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="unitRef2-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		baseUnitService.findDataGridPage(dg,dgclass,getDgPage(),baseUnit,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(baseUnit.getId() != null && baseUnit.getId() != -1){
				baseUnit = baseUnitService.getBaseUnit(baseUnit.getId());
			}
			baseUnitService.findDataGridPage(dg,dgclass,getDgPage(),baseUnit,searchCondition,params);
	...
	private void save() throws Exception {
		...
		baseUnitService.saveBaseUnit(baseUnit,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "submit");
		bap_validate_datagrids=baseUnitService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", baseUnit.getId());
		...
		fileuploads.put("linkId", baseUnit.getId());
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/unitGroup/baseUnit/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		baseUnitService.deleteBaseUnit(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/unitGroup/baseUnit/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "restore");
		baseUnitService.restoreBaseUnit(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicBaseUnit> findExportDataInfos(Page<X6BasicBaseUnit> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = baseUnitService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = baseUnitService.getAssProperty(p.getCode());	
									//String assModelCode = baseUnitService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			baseUnitService.findBaseUnits(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			baseUnitService.findBaseUnits(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicBaseUnit> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicBaseUnit result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".UNIT_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.baseUnit || null == this.baseUnit.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/unitGroup/baseUnit/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/unitGroup/baseUnit/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/unitGroup/baseUnit/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/unitGroup/baseUnit/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicBaseUnitService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/unitGroup/baseUnit/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_unitGroup_BaseUnit", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_unitGroup_BaseUnit", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/unitGroup/baseUnit/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/unitGroup/baseUnit/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_unitGroup_BaseUnit";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_unitGroup_BaseUnit", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/unitGroup/baseUnit/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
		_auditLog.setColumnStringA(null == baseUnit.getName() ? "" : baseUnit.getName().toString());
		_auditLog.setColumnStringC(baseUnit.getId().toString());
		_auditLog.setColumnStringD(X6BasicBaseUnit.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicBaseUnitAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, baseUnit,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, baseUnit,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,baseUnit);
		...
		printlog.setBusinessId(baseUnit.getId());
		printlog.setBussinessKey(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
		printlog.setMainDisplay(null == baseUnit.getName() ? "" : baseUnit.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/unitGroup/baseUnit/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(baseUnit,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/unitGroup/baseUnit/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(baseUnitService.getBaseUnit(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/unitGroup/baseUnit/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/unitGroup/baseUnit/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, baseUnit,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				baseUnit = baseUnitService.getBaseUnit(Long.parseLong(arrId[i]));
				if(null != baseUnit){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == baseUnit.getName() ? "" : "," + baseUnit.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == baseUnit.getName() ? "" : baseUnit.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == baseUnit.getId() ? "" : "," + baseUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == baseUnit.getId() ? "" : "," + baseUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == baseUnit.getId() ? "" : baseUnit.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, baseUnit,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,baseUnit);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/unitGroup/baseUnit/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_unitGroup_BaseUnit");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_unitGroup_BaseUnit")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_unitGroup_BaseUnit");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_unitGroup_BaseUnit");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = baseUnitService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_unitGroup_BaseUnit,X6Basic_1.0) */
// 自定义代码
