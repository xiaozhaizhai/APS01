...
@Repository
public class MeetingRoomDaoImpl extends ExtendedGenericDaoImpl<MeetingRoom, Long> implements MeetingRoomDao {
	@Override
	...
	@Override
	public MeetingRoomDealInfo copyAndSaveDealInfo(DealInfo di) {
		MeetingRoomDealInfo dealInfo = new MeetingRoomDealInfo();
	...
	@Override
	public MeetingRoomDealInfo copyAndSaveDealInfo(DealInfo di, MeetingRoom mainObj, Staff dealStaff) {
		MeetingRoomDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MeetingRoomDealInfo.class);
		...
		List<MeetingRoomDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
