...
import com.supcon.orchid.X6Basic.services.X6BasicTimeRegionImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicTimeRegionDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
import com.supcon.orchid.foundation.entities.Language;
import com.supcon.orchid.foundation.services.SystemCodeService;
/* CUSTOM CODE END */

@Service("x6Basic_timeRegionService")
@Transactional
public class X6BasicTimeRegionServiceImpl  extends BaseServiceImpl<X6BasicTimeRegion> implements X6BasicTimeRegionService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicTimeRegion> bapBaseService;
	...
	@Autowired
	private X6BasicTimeRegionImportService timeRegionImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicTimeRegion.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  timeRegionDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicTimeRegion.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = timeRegionDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicTimeRegion.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicTimeRegion.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  timeRegionDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicTimeRegion.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicTimeRegion.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicTimeRegion> list = (List<X6BasicTimeRegion>) timeRegionDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicTimeRegion getTimeRegion(long id){
		return getTimeRegion(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicTimeRegion getTimeRegion(long id, String viewCode){
		X6BasicTimeRegion timeRegion = timeRegionDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getTimeRegionAsJSON(long id, String include){
		X6BasicTimeRegion timeRegion = timeRegionDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(timeRegion, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegion(X6BasicTimeRegion timeRegion){
		if(null != timeRegion && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegion.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
			AuditUtil.setColumnStringA(null == timeRegion.getName() ? "" : timeRegion.getName().toString());
			AuditUtil.setColumnStringC(timeRegion.getId().toString());
		}
		beforeDeleteTimeRegion(timeRegion);
		timeRegionDao.delete(timeRegion);
		afterDeleteTimeRegion(timeRegion);
		timeRegionDao.flush();
		List<X6BasicTimeRegion> params = new ArrayList<X6BasicTimeRegion>();
		List<X6BasicTimeRegionPar> timeRegionParList = timeRegionParDao.findByCriteria(Restrictions.eq("timeRegionID", timeRegion));
		timeRegion.setTimeRegionParList(timeRegionParList);
		params.add(timeRegion);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_timeRegion");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegion(List<Long> timeRegionIds){
		deleteTimeRegion(timeRegionIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegion(List<Long> timeRegionIds, String eventTopic) {
		List<X6BasicTimeRegion> timeRegions = new ArrayList<X6BasicTimeRegion>();
		for(Long timeRegionId : timeRegionIds){
			X6BasicTimeRegion timeRegion = getTimeRegion(timeRegionId);
			timeRegions.add(timeRegion);
			if(null != timeRegion && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegion.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == timeRegion.getName() ? "" : "," + timeRegion.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == timeRegion.getName() ? "" : timeRegion.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == timeRegion.getId() ? "" : "," + timeRegion.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == timeRegion.getId() ? "" : "," + timeRegion.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(timeRegions != null && timeRegions.size() > 0 ){
			for(X6BasicTimeRegion timeRegion : timeRegions){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),timeRegion, timeRegion.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(timeRegionIds != null && timeRegionIds.size() > 0) {
			String hql = "update " + X6BasicTimeRegion.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = timeRegionDao.createQuery(hql);
			query.setParameterList("ids", timeRegionIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicTimeRegion timeRegionz : timeRegions){
				timeRegionz.setValid(false);
				List<X6BasicTimeRegion> params = new ArrayList<X6BasicTimeRegion>();
				List<X6BasicTimeRegionPar> timeRegionParList = timeRegionParDao.findByCriteria(Restrictions.eq("timeRegionID", timeRegionz));
				timeRegionz.setTimeRegionParList(timeRegionParList);
				params.add(timeRegionz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_timeRegion");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegion(long timeRegionId, int timeRegionVersion){
		this.deleteTimeRegion(timeRegionId, timeRegionVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegion(long timeRegionId, int timeRegionVersion,SignatureLog signatureLog){
		X6BasicTimeRegion timeRegion = getTimeRegion(timeRegionId);
		...
		beforeDeleteTimeRegion(timeRegion);
		timeRegionDao.delete(timeRegionId, timeRegionVersion);
		afterDeleteTimeRegion(timeRegion);
		timeRegionDao.flush();
		List<X6BasicTimeRegion> params = new ArrayList<X6BasicTimeRegion>();
		List<X6BasicTimeRegionPar> timeRegionParList = timeRegionParDao.findByCriteria(Restrictions.eq("timeRegionID", timeRegion));
		...
		if(signatureLog != null ) {
			...
			businessKey=timeRegion.getId();
			signatureLog.setTableId(timeRegion.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegion(String timeRegionIds){
		this.deleteTimeRegion(timeRegionIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegion(String timeRegionIds,SignatureLog signatureLog){
		deleteCollection(timeRegionIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteTimeRegion(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_timeRegion#modelCode:X6Basic_1.0_timeRegion_TimeRegion#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="ec.engine.restore",operType="10")
	public void restoreTimeRegion(String timeRegionIds){
		restoreCollection(timeRegionIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreTimeRegion(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="ec.engine.restore",operType="10")
	public void restoreTimeRegion(long timeRegionId){
		
		findBusinessKeyUsed(timeRegionId);	//判断业务主键是否重复
		...
		beforeRestoreTimeRegion(timeRegion);
		timeRegion.setValid(true);
		timeRegionDao.update(timeRegion);
		afterRestoreTimeRegion(timeRegion);
		List<X6BasicTimeRegion> params = new ArrayList<X6BasicTimeRegion>();
		List<X6BasicTimeRegionPar> timeRegionParList = timeRegionParDao.findByCriteria(Restrictions.eq("timeRegionID", timeRegion));
		timeRegion.setTimeRegionParList(timeRegionParList);
		params.add(timeRegion);
	...
	public void findBusinessKeyUsed(long timeRegionId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_timeRegion_TimeRegion");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicTimeRegion.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicTimeRegion.TABLE_NAME +" where id = ? )";
			List<Object> list =  timeRegionDao.createNativeQuery(sql,timeRegionId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseTimeRegion(List<X6BasicTimeRegion>  timeRegions){
		for(X6BasicTimeRegion timeRegion:timeRegions)  {
			saveTimeRegion(timeRegion, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseTimeRegion(List<X6BasicTimeRegion>  timeRegions){
		for(X6BasicTimeRegion timeRegion:timeRegions)  {
			saveTimeRegion(timeRegion, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="ec.flow.save")
	public void saveTimeRegion(X6BasicTimeRegion timeRegion, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveTimeRegion(timeRegion, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="ec.flow.save")
	public void saveTimeRegion(X6BasicTimeRegion timeRegion, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveTimeRegion(timeRegion, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateTimeRegionCodes(X6BasicTimeRegion timeRegion) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTimeRegionCodes(timeRegion, false);
	}
	
	public void generateTimeRegionCodes(X6BasicTimeRegion timeRegion, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateTimeRegionSummarys(X6BasicTimeRegion timeRegion) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTimeRegionSummarys(timeRegion, false);
	}

	public void generateTimeRegionSummarys(X6BasicTimeRegion timeRegion, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion")
	public void saveTimeRegion(X6BasicTimeRegion timeRegion,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveTimeRegion(timeRegion, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion")
	public void saveTimeRegion(X6BasicTimeRegion timeRegion,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_timeRegion";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_timeRegion");
		String url = null;
		if(timeRegion.getId() != null && timeRegion.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegion.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_timeRegion/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegion.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_timeRegion/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, timeRegion);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(timeRegion);
		beforeSaveTimeRegion(timeRegion, viewIsView);

		if (viewIsView) {
			timeRegionDao.saveWithRevertVersion(timeRegion);
		} else {
			if(isNew)
				timeRegionDao.save(timeRegion);
			else
				timeRegionDao.merge(timeRegion);
		}
		if(null != timeRegion && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegion.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
			AuditUtil.setColumnStringA(null == timeRegion.getName() ? "" : timeRegion.getName().toString());
		...
		dealDatagridsSave(timeRegion,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateTimeRegionCodes(timeRegion, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateTimeRegionSummarys(timeRegion, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveTimeRegion(timeRegion, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, timeRegion);
			}
		}
		List<X6BasicTimeRegion> params = new ArrayList<X6BasicTimeRegion>();
		timeRegionDao.flush();
		timeRegionDao.clear();
		timeRegion = timeRegionDao.load(timeRegion.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicTimeRegion timeRegion){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("timeRegion", timeRegion);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion")
	public void mergeTimeRegion(X6BasicTimeRegion timeRegion, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(timeRegion);
		beforeSaveTimeRegion(timeRegion);
		timeRegionDao.merge(timeRegion);
		if(null != timeRegion && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegion.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == timeRegion.getId() ? "" : timeRegion.getId().toString());
			AuditUtil.setColumnStringA(null == timeRegion.getName() ? "" : timeRegion.getName().toString());
		...
		dealDatagridsSave(timeRegion,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveTimeRegion(timeRegion);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicTimeRegion> findTimeRegions(Page<X6BasicTimeRegion> page, Criterion... criterions) {
		return timeRegionDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicTimeRegionDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicTimeRegionDealInfo> list=timeRegionDao.createQuery("from " + X6BasicTimeRegionDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(X6BasicTimeRegionDealInfo dealInfo) {
		timeRegionDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicTimeRegionDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) timeRegionDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicTimeRegionDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = timeRegionDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = timeRegionDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicTimeRegionDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = timeRegionDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTimeRegions(Page<X6BasicTimeRegion> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findTimeRegions(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTimeRegions(Page<X6BasicTimeRegion> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTimeRegions(Page<X6BasicTimeRegion> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicTimeRegion> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"timeRegion\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"timeRegion\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"timeRegion\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_timeRegion_TimeRegion" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_timeRegion_TimeRegion" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  timeRegionDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  timeRegionDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_timeRegion").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"timeRegion\"","X6Basic_1.0_timeRegion",customCondition,"X6BASIC_TIME_REGIONS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"timeRegion\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicTimeRegionDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"timeRegion\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"timeRegion\".LAY_REC ASC, \"timeRegion\".SORT ASC");
				} else {
					orderPart.append(" \"timeRegion\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) timeRegionDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=timeRegionDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = timeRegionDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicTimeRegion> result = new ArrayList<X6BasicTimeRegion>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicTimeRegion.class,timeRegionDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicTimeRegion.class,timeRegionDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicTimeRegion timeRegion : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(timeRegion.getId(), "X6Basic_timeRegion_timeRegion");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(timeRegion.getId(), "X6Basic_timeRegion_timeRegion");
							if(null != documents && !documents.isEmpty()) {
								timeRegion.setDocument(documents.get(0));
								timeRegion.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_timeRegion")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_timeRegion")+"X6Basic_1.0_timeRegion".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_timeRegion",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicTimeRegion> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicTimeRegion> result) {
		View view = null;
		...
		Map<Long, X6BasicTimeRegion> rsMap = new HashMap<Long, X6BasicTimeRegion>();
		for (X6BasicTimeRegion timeRegion : result) {
			ids.add(timeRegion.getId());
			rsMap.put(timeRegion.getId(), timeRegion);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(timeRegionDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = timeRegionDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicTimeRegion timeRegion,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(timeRegion.getOwnerStaffId());
		ti.setOwnerPositionId(timeRegion.getOwnerPositionId());
		ti.setOwnerDepartmentId(timeRegion.getOwnerDepartmentId());
		
		ti.setCreateStaff(timeRegion.getCreateStaff());
		ti.setCreateStaffId(timeRegion.getCreateStaffId());
		ti.setCreateTime(timeRegion.getCreateTime());
		ti.setDeleteStaff(timeRegion.getDeleteStaff());
		ti.setDeleteStaffId(timeRegion.getDeleteStaffId());
		ti.setDeleteTime(timeRegion.getDeleteTime());
		ti.setCreateDepartmentId(timeRegion.getCreateDepartmentId());
		ti.setModifyStaff(timeRegion.getModifyStaff());
		ti.setModifyStaffId(timeRegion.getModifyStaffId());
		ti.setCreatePositionId(timeRegion.getCreatePositionId());
		ti.setPositionLayRec(timeRegion.getPositionLayRec());
		ti.setTableNo(timeRegion.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_timeRegion");
		ti.setTargetTableName("X6BASIC_TIME_REGIONS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_timeRegion_TimeRegion" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  timeRegionDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"timeRegion\"","X6Basic_1.0_timeRegion","","X6BASIC_TIME_REGIONS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"timeRegion\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"timeRegion\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"timeRegion\"","X6Basic_1.0_timeRegion","","X6BASIC_TIME_REGIONS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("timeRegion.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"timeRegion\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"timeRegion\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"timeRegion\".CID = 1 OR \"timeRegion\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("timeRegion.mc\"").append(".TIME_REGION) not in ("+notinValue+")");
			}
			
			SQLQuery query = timeRegionDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("timeRegion", "X6Basic_1.0_timeRegion", "timeRegion_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=timeRegionDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicTimeRegion.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicTimeRegion.MODULE_CODE;}
	protected String getModelCode(){return X6BasicTimeRegion.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param timeRegion 模型实例
	 */
	public void checkUniqueConstraint(X6BasicTimeRegion timeRegion){
	}
	
	
	@Override
	public List<X6BasicTimeRegion> getTimeRegions(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicTimeRegion.class);
		if(params!=null){
			...
			return timeRegionDao.findByHql("from " + X6BasicTimeRegion.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return timeRegionDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicTimeRegion> findTimeRegionsByHql(String hql, Object... objects){
		...
		return   timeRegionDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicTimeRegion loadTimeRegionByBussinessKey(X6BasicTimeRegion timeRegion){
		return loadTimeRegionByBussinessKey(timeRegion.getId());
	}
	
	@Override
	public Page<X6BasicTimeRegion> getTimeRegions(Page<X6BasicTimeRegion> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicTimeRegion.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = timeRegionDao.findByPage(page, sql);
			}else{
				page = timeRegionDao.findAllByPage(page);
			}
			return page;
		}
		page = timeRegionDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public X6BasicTimeRegion loadTimeRegionByBussinessKey(Serializable bussinessKey){
		return timeRegionDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicTimeRegion> findByProperty(String propertyName, Object object){
		return timeRegionDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicTimeRegion findEntityByProperty(String propertyName, Object object){
		return timeRegionDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicTimeRegion.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = timeRegionDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_timeRegion_TimeRegion","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_timeRegion", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicTimeRegion timeRegion,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,timeRegionDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicTimeRegion.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, timeRegion);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",timeRegion.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicTimeRegion.TABLE_NAME +" where valid = 1";
			list = timeRegionDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicTimeRegion.TABLE_NAME;
			list = timeRegionDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicTimeRegion.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = timeRegionDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicTimeRegionDaoImpl timeRegionDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchTimeRegion(final List<X6BasicTimeRegion> insertObjs, final List<X6BasicTimeRegion> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchTimeRegion(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchTimeRegion(final List<X6BasicTimeRegion> insertObjs, final List<X6BasicTimeRegion> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.randon1459213973062"),importCount));
		...
		Map<Long, X6BasicTimeRegion> oldUpdateObjsMap = new HashMap<Long, X6BasicTimeRegion>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicTimeRegion bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicTimeRegion> bizObjList = timeRegionDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicTimeRegion bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicTimeRegion> nmeObjs = new ArrayList<X6BasicTimeRegion>();
		...
		long id=autoGeneratorID.getNextId(X6BasicTimeRegion.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicTimeRegion timeRegion:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicTimeRegion.TABLE_NAME,1,dbId);
			if(timeRegion.getId() != null ){
				m.put(timeRegion.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) timeRegionDao.createNativeQuery(sql, "X6Basic_1.0_timeRegion_TimeRegion"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		timeRegionDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicTimeRegion.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicTimeRegion timeRegion = updateObjs.get(updateIndex);
						Long id = timeRegion.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = timeRegion.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicTimeRegion.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicTimeRegion.TABLE_NAME + " order by id desc";
//					Query query = timeRegionDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicTimeRegion timeRegion = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = timeRegion.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		timeRegionDaoImpl.getSession().flush();
		timeRegionDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicTimeRegion bizObj = insertObjs.get(insertIndex);
			timeRegionImportService.saveImportDataLog(bizObj, null, "id", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicTimeRegion bizObj = updateObjs.get(updateIndex);
			X6BasicTimeRegion oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			timeRegionImportService.saveImportDataLog(bizObj, oldBizObj, "id", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = timeRegionDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				timeRegionDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = timeRegionDao.createNativeQuery(sql, "X6Basic_1.0_timeRegion_TimeRegion").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = timeRegionDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = timeRegionDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = timeRegionDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?timeRegionDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  timeRegionDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,timeRegionDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1459214611112Page(Page<X6BasicTimeRegionPar> dg1459214611112Page,X6BasicTimeRegion timeRegion, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(X6BasicTimeRegionPar.class);
		criteria.add(Restrictions.eq("timeRegionID", timeRegion));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"timeRegionPar\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		timeRegionParDao.findByPage(dg1459214611112Page, criteria);
		timeRegionParService.findByNormalDataGridTimeRegion(dg1459214611112Page);
	}
	
	 
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="ec.flow.save")
	public void saveTimeRegion(X6BasicTimeRegion timeRegion, DataGridService dataGridService){
		saveTimeRegion(timeRegion, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="ec.flow.save")
	public void saveTimeRegion(X6BasicTimeRegion timeRegion, DataGridService dataGridService, String viewCode){
		saveTimeRegion(timeRegion, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion")
	public void saveTimeRegion(X6BasicTimeRegion timeRegion, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveTimeRegion(timeRegion, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion")
	public void saveTimeRegion(X6BasicTimeRegion timeRegion, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_timeRegion";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_timeRegion");
		String url = null;
		if(null != timeRegion.getId() && timeRegion.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegion.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion")
	public Map<Object, Object> timeRegionDataGridImport(X6BasicTimeRegion timeRegion, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = timeRegion.getVirtualId();
		
		timeRegion = timeRegionDao.load(timeRegion.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, timeRegion.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = timeRegion.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(timeRegion);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, timeRegion.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion")
	public void mergeTimeRegion(X6BasicTimeRegion timeRegion, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(timeRegion);
		beforeSaveTimeRegion(timeRegion);
		timeRegionDao.merge(timeRegion);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegion",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicTimeRegion> getByPage(Page<X6BasicTimeRegion> page,DetachedCriteria detachedCriteria){
		return timeRegionDao.findByPage(page, detachedCriteria);
	}
	@Override
	public List<X6BasicTimeRegionPar> getTimeRegionParList(X6BasicTimeRegion timeRegion){
		return timeRegionParDao.findByCriteria(Restrictions.eq("timeRegionID", timeRegion),Restrictions.eq("valid", true));
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_timeRegion").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveTimeRegion(X6BasicTimeRegion timeRegion, Object... objects){
		checkUniqueConstraint(timeRegion);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
      
		//如果是新增执行下面的验证
		if(timeRegion.getId()==null){
			SystemCode userEntity=timeRegion.getUseEntity();
			if(userEntity==null){
				List<X6BasicTimeRegion> timeRegionList=this.getTimeRegions(" valid=1  and  useEntity is null", null);
				for(X6BasicTimeRegion timeRegionNew:timeRegionList){
					String unit="";
					if(timeRegionNew.getUnit()!=null){
						unit=timeRegionNew.getUnit().getId();
					}
					if(unit.equals(timeRegion.getUnit().getId())   ){
						if( "timeUnit/01".equals(unit)){
							throw new BAPException("单位为年且使用实体为空的记录已经存在，不允许重复制定");
						}
						if( "timeUnit/02".equals(unit)){
							throw new BAPException("单位为季度且使用实体为空的记录已经存在，不允许重复制定");
						}
						if( "timeUnit/03".equals(unit)){
							throw new BAPException("单位为月且使用实体为空的记录已经存在，不允许重复制定");
						}
						if( "timeUnit/04".equals(unit)){
							throw new BAPException("单位为周且使用实体为空的记录已经存在，不允许重复制定");
						}
					}
				}
			}
...
/* CUSTOM CODE END */
	...
	private void afterSaveTimeRegion(X6BasicTimeRegion timeRegion, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
		if("true".equals(isSynName)){//设置了一个是否同步的系统配置项
			SystemCode userEntity=timeRegion.getUseEntity();
			String unit=timeRegion.getUnit().getId();
			if(userEntity==null&&"timeUnit/03".equals(unit)){//如果符合要求执行更新
				List<X6BasicTimeRegionPar> timeRegionParList=timeRegionParService.getTimeRegionPars("  timeRegionID="+timeRegion.getId(), null);//获取到当前时间区间中的时间明细
				HashSet<String> timeYear =new HashSet<String>();
				//下面是对时间区间和会计期中年份的重复核算
				for(X6BasicTimeRegionPar timeRegionPar:timeRegionParList){
					String year=timeRegionPar.getYear().toString();
					String month=timeRegionPar.getMonth().toString();
					timeYear.add(year);
				}
				//以上是获取到未在会计期中使用的年份
				for(String year:timeYear){
					timeRegionParList=timeRegionParService.getTimeRegionPars("timeRegionID="+timeRegion.getId()+" and year="+year, null);//根据年重新获取到时间区间中的明细
					List<X6BasicAccountPeriod> accountPeriodList=accountPeriodService.getAccountPeriods(" valid=1  and  year='accYear/Y"+year+"'", null);
					if(accountPeriodList.size()>0&&accountPeriodList.get(0)!=null){
						//会计期已经有的该年份
						X6BasicAccountPeriod accountPeriodByYear=accountPeriodList.get(0);
						List<X6BasicAccountPeriodPart> accountPeriodPartList=accountPeriodPartService.getAccountPeriodParts(" valid=1  and   accountPeriod="+accountPeriodByYear.getId(), null);
						/*
						//对存在的会计期进行批量删除
						for(X6BasicAccountPeriodPart accountPeriodPartIds:accountPeriodPartList){
							accountPeriodPartService.deleteAccountPeriodPart(accountPeriodPartIds);
						}
						*/
						//对会计期进行添加或者修改
						for(X6BasicTimeRegionPar timeRegionPar:timeRegionParList){
							    Integer mon= timeRegionPar.getMonth();
							    List<X6BasicAccountPeriodPart> accountPeriodParts=accountPeriodPartService.
							    		getAccountPeriodParts(" valid=1  and   accountPeriod="+accountPeriodByYear.getId()+" and  month= 'accMonth/m"+timeRegionPar.getMonth()+"' ", null);
							    //当面年下已经存在本月的数据，则进行修改
							    if(accountPeriodParts.size()>0 && accountPeriodParts.get(0)!=null){
							    	X6BasicAccountPeriodPart part=accountPeriodParts.get(0);
							    	part.setStartDate(timeRegionPar.getStartDate());
							    	part.setEndDate(timeRegionPar.getEndDate());
							    	accountPeriodPartDao.merge(part);
							        
							    }else{
							     //当面年下不存在本月的数据，则进行新增
							    	X6BasicAccountPeriodPart accountPeriodPart=new X6BasicAccountPeriodPart();
									accountPeriodPart.setAccountPeriod(accountPeriodByYear);
									accountPeriodPart.setStartDate(timeRegionPar.getStartDate());
									accountPeriodPart.setEndDate(timeRegionPar.getEndDate());
									SystemCode systemCodeMonth=(SystemCode) systemCodeService.getSystemCodeByID("accMonth/m"+timeRegionPar.getMonth());
									accountPeriodPart.setMonth(systemCodeMonth);
									accountPeriodPartDao.save(accountPeriodPart);
							    }
						}
					}else{
						//会计期没有该年份
						X6BasicAccountPeriod accountPeriodByYear =new X6BasicAccountPeriod();
						SystemCode systemCodeYear=(SystemCode) systemCodeService.getSystemCodeByID("accYear/Y"+year);
						accountPeriodByYear.setYear(systemCodeYear);
						accountPeriodDao.save(accountPeriodByYear);//对会计期表体记性新增 
       ...
       //按照开始时间的先后次序进行排序
		...
		params.add(timeRegion.getId());
		//获取到当前时间区间中的时间明细
		List<X6BasicTimeRegionPar> timeRegionPartList=timeRegionParService.getTimeRegionPars("this_.TIME_REGIONID=? order by this_.START_DATE ", params);
		for(int i=0;i<timeRegionPartList.size();i++){
			X6BasicTimeRegionPar timeRegionPart=timeRegionPartList.get(i);
			//从新设置它的顺序
			timeRegionPart.setSort(i+1);
			timeRegionParDao.merge(timeRegionPart);
		}
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteTimeRegion(X6BasicTimeRegion timeRegion){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
      //名称[
	   	 String X6pageName= InternationalResource.get("X6Basic.timeRegion.radion137146912342", getCurrentLanguage());
	   	 //]中有时间区间已被其他业务引用，不能删除！
	   	 String X6pageDelete=InternationalResource.get("X6Basic.timeRegion.radion137142323444", getCurrentLanguage());
		List<Object> params=new ArrayList<Object>();
		params.add(timeRegion.getId());
		//获取到当前时间区间中的时间明细
		List<X6BasicTimeRegionPar> timeRegionPartList=timeRegionParService.getTimeRegionPars("this_.TIME_REGIONID=? order by this_.START_DATE ", params);
		for(X6BasicTimeRegionPar basicTimeRegionPart :timeRegionPartList){
			 if(!bapBasePartService.canDelete(basicTimeRegionPart)){
				 //如果时间区间明细中的某个时间区间被业务引用报错
				 throw new BAPException(X6pageName+timeRegion.getName()+X6pageDelete);
...
/* CUSTOM CODE END */
	...
	private void afterDeleteTimeRegion(X6BasicTimeRegion timeRegion){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreTimeRegion(X6BasicTimeRegion timeRegion){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreTimeRegion(X6BasicTimeRegion timeRegion){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicTimeRegion timeRegion, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicTimeRegion timeRegion, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportTimeRegion(List<X6BasicTimeRegion> insertObjs, List<X6BasicTimeRegion> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportTimeRegion,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportTimeRegion(List<X6BasicTimeRegion> insertObjs, List<X6BasicTimeRegion> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportTimeRegion,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_timeRegion_TimeRegion,X6Basic_1.0) */
// 自定义代码
	@Resource
  ...
  @Resource
	private IBAPBaseService<X6BasicTimeRegionPar> bapBasePartService;
  
  
  public void delX6BasicAccountPeriodPart(String timeRePatrIds){
		for(int i=0;i<timeRePatrIds.split(",").length;i++){
			if(timeRePatrIds.split(",")[i].length()>0){
				Long id=Long.parseLong(timeRePatrIds.split(",")[i]);
				//时间区间明细
				X6BasicTimeRegionPar part=timeRegionParService.getTimeRegionPar(id);
				//根据时间区间明细的年找会计期
				List<X6BasicAccountPeriod> accountPeriodList=accountPeriodService.getAccountPeriods(" valid=1  and   year='accYear/Y"+part.getYear()+"'", null);
				if(accountPeriodList.size()>0&&accountPeriodList.get(0)!=null){
					 //找到和时间区间对应的会计期
					 List<X6BasicAccountPeriodPart> accountPeriodParts=accountPeriodPartService.
					    		getAccountPeriodParts(" valid=1  and  accountPeriod="+accountPeriodList.get(0).getId()+" and  month= 'accMonth/m"+part.getMonth()+"' ", null);
					   if(accountPeriodParts.size()>0 && accountPeriodParts.get(0)!=null){
						   accountPeriodPartService.deleteAccountPeriodPart(accountPeriodParts.get(0));	
						   accountPeriodPartDao.flush();
					    }
					 //查找该年份下，明细表是否有数据，如果没有，则将表头数据删除
						 List<X6BasicAccountPeriodPart> accountPeriodParts2=accountPeriodPartService.
						    		getAccountPeriodParts("accountPeriod="+accountPeriodList.get(0).getId(), null);
						 if(accountPeriodParts2.size()<=0){
							 accountPeriodService.deleteAccountPeriod(accountPeriodList.get(0));  
						 }
