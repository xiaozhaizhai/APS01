...
/* CUSTOM CODE START(action,import,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
import java.util.Iterator;
...
public class MeetingRoomAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MeetingRoom> {
	...
	//导出excel时,主模型内容
	List<MeetingRoom> queryResults = new ArrayList<MeetingRoom>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/meetingroom/meetingRoom/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/meetingroom/meetingRoom/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_meetingroom_MeetingRoom" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(meetingRoom.getRoomSize() != null) {
				meetingRoom.setRoomSize((SystemCode) systemCodeService.getSystemCode(meetingRoom.getRoomSize().getId()));
			}
		} else {
			if(id > 0){
				meetingRoom = meetingRoomService.getMeetingRoom(id, viewCode);
			}
		}
	...
	public Page<MeetingRoom> getPage() {
		if (null == page) {
			page = new Page<MeetingRoom>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/meetingroom/meetingRoom/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = meetingRoomService.getMeetingRoomAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/meetingroom/meetingRoom/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String meetingRoomReferenceCopy() throws Exception{
			MeetingRoom oldMeetingRoom = null;
			if(isEditNew()){
				...
				oldMeetingRoom = new MeetingRoom();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,meetingRoom.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, meetingRoom.getId());
				...
				oldMeetingRoom = meetingRoomService.getMeetingRoom(id);
			}
			
			
			meetingRoom = meetingRoomService.getMeetingRoom(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_meetingroom_MeetingRoom");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MeetingRoom.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMeetingRoom, null);
						Method setValue = MeetingRoom.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(meetingRoom, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			meetingRoom.setCreateDepartmentId(oldMeetingRoom.getCreateDepartmentId());
			meetingRoom.setCreatePositionId(oldMeetingRoom.getCreatePositionId());
			meetingRoom.setCreateStaffId(oldMeetingRoom.getCreateStaffId());
			meetingRoom.setOwnerDepartmentId(oldMeetingRoom.getOwnerDepartmentId());
			meetingRoom.setOwnerPositionId(oldMeetingRoom.getOwnerPositionId());
			meetingRoom.setOwnerStaffId(oldMeetingRoom.getOwnerStaffId());
			meetingRoom.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/meetingroom/meetingRoom/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/meetingroom/meetingRoom/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=meetingRoomService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return meetingRoomReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			meetingRoom = meetingRoomService.getMeetingRoom(this.meetingRoom.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,meetingRoom.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/meetingroom/meetingRoom/view",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "view.ftl") }),
		@Action(value = "/X6Basic/meetingroom/meetingRoom/viewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "viewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/meetingroom/meetingRoom/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/X6Basic/meetingroom/meetingRoom/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = meetingRoomService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(meetingRoomService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(meetingRoomService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_meetingroom_MeetingRoom", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/meetingroom/meetingRoom/refer", results = { @Result(type = FREEMARKER, location = "refer.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/meetingroom/meetingRoom/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.roomSize.id,result.roomSize.value,result.projector,result.camera,result.matching,result.network,result.audio,result.useFlag,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"meetingRoom\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"meetingRoom\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"meetingRoom\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"meetingRoom\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			meetingRoomService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/X6Basic/meetingroom/meetingRoom/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/meetingroom/meetingRoom/refer-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.roomSize.id,result.roomSize.value,result.network,result.whiteBoard,result.audio,result.projector,result.camera,result.useFlag,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refer-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/meetingroom/meetingRoom/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_meetingroom_MeetingRoom");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		meetingRoomService.findDataGridPage(dg,dgclass,getDgPage(),meetingRoom,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(meetingRoom.getId() != null && meetingRoom.getId() != -1){
				meetingRoom = meetingRoomService.getMeetingRoom(meetingRoom.getId());
			}
			meetingRoomService.findDataGridPage(dg,dgclass,getDgPage(),meetingRoom,searchCondition,params);
	...
	private void save() throws Exception {
		...
		meetingRoomService.saveMeetingRoom(meetingRoom,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/meetingroom/meetingRoom/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/meetingroom/meetingRoom/view/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "submit");
		bap_validate_datagrids=meetingRoomService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", meetingRoom.getId());
		...
		fileuploads.put("linkId", meetingRoom.getId());
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/meetingroom/meetingRoom/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		meetingRoomService.deleteMeetingRoom(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/meetingRoom/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "restore");
		meetingRoomService.restoreMeetingRoom(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/meetingroom/meetingRoom/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/meetingroom/meetingRoom/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "showDealInfoData");
		if (null != tableInfoId) {
			int count=meetingRoomService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = meetingRoomService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = meetingRoomService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MeetingRoom> findExportDataInfos(Page<MeetingRoom> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = meetingRoomService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = meetingRoomService.getAssProperty(p.getCode());	
									//String assModelCode = meetingRoomService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			meetingRoomService.findMeetingRooms(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			meetingRoomService.findMeetingRooms(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MeetingRoom> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MeetingRoom result : results){
	...
	public boolean isEditNew(){
		//return null == this.meetingRoom || null == this.meetingRoom.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/meetingroom/meetingRoom/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/meetingroom/meetingRoom/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/meetingRoom/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.MeetingRoomService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_meetingroom", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_meetingroom");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/meetingRoom/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.MeetingRoomService"
						...
						, "com.supcon.orchid.X6Basic.services.AttMachineService"       
						, "com.supcon.orchid.X6Basic.services.UsedRecordService"
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_meetingroom_MeetingRoom", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_meetingroom_MeetingRoom", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_meetingroom_MeetingRoom";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_meetingroom_MeetingRoom", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
		_auditLog.setColumnStringA(null == meetingRoom.getName() ? "" : meetingRoom.getName().toString());
		_auditLog.setColumnStringC(meetingRoom.getId().toString());
		_auditLog.setColumnStringD(MeetingRoom.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_meetingroom");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.MeetingRoomAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, meetingRoom,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, meetingRoom,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,meetingRoom);
		...
		printlog.setEntity("X6Basic_1.0_meetingroom");
		...
		printlog.setBusinessId(meetingRoom.getId());
		printlog.setBussinessKey(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
		printlog.setMainDisplay(null == meetingRoom.getName() ? "" : meetingRoom.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/meetingroom/meetingRoom/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(meetingRoom,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/meetingRoom/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(meetingRoomService.getMeetingRoom(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, meetingRoom,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				meetingRoom = meetingRoomService.getMeetingRoom(Long.parseLong(arrId[i]));
				if(null != meetingRoom){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == meetingRoom.getName() ? "" : "," + meetingRoom.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == meetingRoom.getName() ? "" : meetingRoom.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == meetingRoom.getId() ? "" : "," + meetingRoom.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == meetingRoom.getId() ? "" : "," + meetingRoom.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == meetingRoom.getId() ? "" : meetingRoom.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, meetingRoom,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,meetingRoom);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MeetingRoom> findExportPageInfos(Page<MeetingRoom> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_meetingroom" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = meetingRoomService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MeetingRoom queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = meetingRoomService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = meetingRoomService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_meetingroom_MeetingRoom");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_meetingroom_MeetingRoom")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_meetingroom_MeetingRoom");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_meetingroom_MeetingRoom");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = meetingRoomService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_meetingroom_MeetingRoom,X6Basic_1.0) */
// 自定义代码
	/**
	 * 自定义参照
	 *
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/meetingroom/meetingRoom/reference", results = { @Result(type = FREEMARKER, location = "reference.ftl") })
	public String reference() throws Exception{
		if(condition!=null&&!"".equals(condition)){
			...
			if(arr.length>5){
				...
				TimeS=arr[2];
				TimeE=arr[3];
				startTime=DateUtils.parseDateTime(arr[4]);
				endTime=DateUtils.parseDateTime(arr[5]);
			}
	...
	private Page<MeetingRoom> freeMeetingRoomPage;

	public Page<MeetingRoom> getFreeMeetingRoomPage() {
		if (null == freeMeetingRoomPage) {
			freeMeetingRoomPage = new Page<MeetingRoom>(1, 60000);
		}
		return freeMeetingRoomPage;
	}
	private Page<MeetingRoom> allMeetingRoomPage;
	
	public Page<MeetingRoom> getAllMeetingRoomPage() {
		if (null == allMeetingRoomPage) {
			allMeetingRoomPage = new Page<MeetingRoom>(1, 60000);
		}
		return allMeetingRoomPage;
	}
	@Action(value = "/X6Basic/meetingroom/meetingRoom/reference-all", results = { @Result(type = JSON, params = { "root", "allMeetingRoomPage", "excludes", "*", "includes",
	"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.status,result.tableNo,result.name,result.roomSize.value,result.audio,result.matching,result.whiteBoard,result.projector,result.network,result.cinema"})
	,@Result(name="excel",type="excel",location="reference-xmlexport.ftl", params = { "root", "allMeetingRoomPage", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String referenceAllQuery() throws Exception{
		allMeetingRoomPage=meetingRoomService.getMeetingRooms(getAllMeetingRoomPage(),"from MeetingRoom where valid=true and useFlag=true",null,null);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/meetingroom/meetingRoom/reference-free", results = { @Result(type = JSON, params = { "root", "freeMeetingRoomPage", "excludes", "*", "includes",
	"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.status,result.tableNo,result.name,result.roomSize.value,result.audio,result.matching,result.whiteBoard,result.projector,result.network,result.cinema"})
	,@Result(name="excel",type="excel",location="reference-xmlexport.ftl", params = { "root", "freeMeetingRoomPage", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String referenceFreeQuery() throws Exception{
		StringBuilder hql=null;
		List<Object> param=new ArrayList<Object>();
		freeMeetingRoomPage=getFreeMeetingRoomPage();
		if(startTime!=null && endTime!=null){
			hql=new StringBuilder("from MeetingRoom mr where valid=true and useFlag=true and id not in (select ud.meeting.id from UsedRecord ud where (ud.startTime >=? and ud.startTime<=?) or (ud.endTime>=? and ud.endTime<=?) or (ud.startTime<=? and ud.endTime>=?))");
			param.add(startTime);
			param.add(endTime);
			param.add(startTime);
			param.add(endTime);
			param.add(startTime);
			param.add(endTime);
			freeMeetingRoomPage=meetingRoomService.findPageByHql(getFreeMeetingRoomPage(), hql.toString(), param.toArray());
		}
		return SUCCESS;
	}
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/getUnuseMeetingRoom", results = { @Result(type = JSON, params = {
			"root", "meetingRoomList" ,"excludes", "*", "includes","id,name"}) })
	public String showCalendarRemarkData() {
		meetingRoomList = meetingRoomService.getUnuseRoomBetweenTime(startTime,endTime);
		return SUCCESS;
	}
	...
	/** 
	 * 
	 * 通过会议室和时间获得会议室活动列表，用于自动选择会议室
	 * @param id
	 * @return Page<CalendarDetail>(pageNo, pageSize)
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/meetingroom/meetingRoom/getMeetingRecordByTime", results = { @Result(type = JSON_PLAIN, params = { "root", "json" }) })
	
	public String getMeetingRecordByTime() throws Exception{
		Map<String , List<UsedRecord>> mrMap=new HashMap<String, List<UsedRecord>>();
		String[] meetingRooms = meetingRoomIDs.split(",");
		for(int i = 0;i<meetingRooms.length;i++){
			String[] e=meetingRooms[i].split("_");
			if(e.length<2){
				MeetingRoom mr = meetingRoomService.getMeetingRoom(Long.valueOf(meetingRooms[i]));
				List<UsedRecord> cList = usedRecordService.getUsedRecordList(startTime, endTime, mr);
				String keyString = mr.getId()+"@@"+mr.getName();
				mrMap.put(keyString, cList);
			}else{
				mrMap.put(meetingRooms[i],new ArrayList<UsedRecord>());
			}
		}
		//自己转json
		if(mrMap==null||mrMap.size()<1){
			return null;
		}
		StringBuffer buffer=new StringBuffer();
		buffer.append("{");
		for (Iterator<String> iterator = mrMap.keySet().iterator(); iterator.hasNext();) {
			buffer.append("\"");
			String key = (String) iterator.next();
			buffer.append(key);
			buffer.append("\":[");
			if(!mrMap.get(key).isEmpty()){			
				List<UsedRecord> cl=mrMap.get(key);
				for(int i=0;i<cl.size();i++){
					UsedRecord cd = cl.get(i);
					buffer.append("{\"id\":"+cd.getTypeId()+",");
					if("meeting_1.0_meetingApply_MeetingApply".equals(cd.getUseType())){
						buffer.append("\"recordType\":\"Meeting\",");
					}else if("calendar_1.0_calendarDetail_CalendarDetail".equals(cd.getUseType())){
						buffer.append("\"recordType\":\"Detail\",");
					}
...
//		Struts2Utils.render(Struts2Utils.JSON_TYPE, test.toString());
		return SUCCESS;
	}
	/** 
	 * 
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/meetingroom/meetingRoom/getAllMeetingRecordByTime", results = { @Result(type = JSON_PLAIN, params = { "root", "json" }) })
	
	public String getAllMeetingRecordByTime() throws Exception{
		Map<String , List<UsedRecord>> mrMap=new HashMap<String, List<UsedRecord>>();
		List<MeetingRoom> mrList= meetingRoomService.getAllMeetingRoom();
...
//		Struts2Utils.render(Struts2Utils.JSON_TYPE, test.toString());
	...
	@Action(value = "/X6Basic/meetingroom/meetingRoom/usedRecordView", results = { @Result(type = FREEMARKER, location = "usedRecordView.ftl") })
	public String usedRecordView() throws Exception{
		return SUCCESS;
	}
/* CUSTOM CODE END */
}