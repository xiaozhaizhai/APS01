...
/* CUSTOM CODE START(action,import,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicRangePlanPartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicRangePlanPart> {
	...
	//导出excel时,主模型内容
	List<X6BasicRangePlanPart> queryResults = new ArrayList<X6BasicRangePlanPart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/rangePlan/rangePlanPart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/rangePlan/rangePlanPart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_rangePlan_RangePlanPart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicRangePlanPart> rangePlanParts;
	
	public List<X6BasicRangePlanPart> getRangePlanParts() {
		return rangePlanParts;
	}
	...
	private Page<X6BasicRangePlanPart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			rangePlanPart = new X6BasicRangePlanPart();
		} else {
			if(id > 0){
				rangePlanPart = rangePlanPartService.getRangePlanPart(id, viewCode);
			}
		}
	...
	public Page<X6BasicRangePlanPart> getPage() {
		if (null == page) {
			page = new Page<X6BasicRangePlanPart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = rangePlanPartService.getRangePlanPartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1398309186741
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String rangePlanPartReferenceCopy() throws Exception{
			X6BasicRangePlanPart oldRangePlanPart = null;
			if(isEditNew()){
				...
				oldRangePlanPart = new X6BasicRangePlanPart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,rangePlanPart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, rangePlanPart.getId());
				...
				oldRangePlanPart = rangePlanPartService.getRangePlanPart(id);
			}
			
			
			rangePlanPart = rangePlanPartService.getRangePlanPart(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_RangePlanPart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicRangePlanPart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldRangePlanPart, null);
						Method setValue = X6BasicRangePlanPart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(rangePlanPart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			rangePlanPart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=rangePlanPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return rangePlanPartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			rangePlanPart = rangePlanPartService.getRangePlanPart(this.rangePlanPart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,rangePlanPart.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = rangePlanPartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(rangePlanPartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(rangePlanPartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_rangePlan_RangePlanPart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"rangePlanPart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"rangePlanPart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"rangePlanPart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"rangePlanPart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			rangePlanPartService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		rangePlanPartService.findDataGridPage(dg,dgclass,getDgPage(),rangePlanPart,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(rangePlanPart.getId() != null && rangePlanPart.getId() != -1){
				rangePlanPart = rangePlanPartService.getRangePlanPart(rangePlanPart.getId());
			}
			rangePlanPartService.findDataGridPage(dg,dgclass,getDgPage(),rangePlanPart,searchCondition,params);
	...
	private void save() throws Exception {
		...
		rangePlanPartService.saveRangePlanPart(rangePlanPart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "submit");
		bap_validate_datagrids=rangePlanPartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", rangePlanPart.getId());
		...
		fileuploads.put("linkId", rangePlanPart.getId());
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/rangePlan/rangePlanPart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		rangePlanPartService.deleteRangePlanPart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "restore");
		rangePlanPartService.restoreRangePlanPart(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicRangePlanPart> findExportDataInfos(Page<X6BasicRangePlanPart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = rangePlanPartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = rangePlanPartService.getAssProperty(p.getCode());	
									//String assModelCode = rangePlanPartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			rangePlanPartService.findRangePlanParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			rangePlanPartService.findRangePlanParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicRangePlanPart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicRangePlanPart result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".RPP_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.rangePlanPart || null == this.rangePlanPart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/rangePlan/rangePlanPart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/rangePlan/rangePlanPart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicRangePlanPartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicRangePlanPartService"
						, "com.supcon.orchid.X6Basic.services.X6BasicRangePlanService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_rangePlan_RangePlanPart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_rangePlan_RangePlanPart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_rangePlan_RangePlanPart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlanPart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
		_auditLog.setColumnStringA(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
		_auditLog.setColumnStringC(rangePlanPart.getId().toString());
		_auditLog.setColumnStringD(X6BasicRangePlanPart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicRangePlanPartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, rangePlanPart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, rangePlanPart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,rangePlanPart);
		...
		printlog.setBusinessId(rangePlanPart.getId());
		printlog.setBussinessKey(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
		printlog.setMainDisplay(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(rangePlanPart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(rangePlanPartService.getRangePlanPart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, rangePlanPart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				rangePlanPart = rangePlanPartService.getRangePlanPart(Long.parseLong(arrId[i]));
				if(null != rangePlanPart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == rangePlanPart.getId() ? "" : "," + rangePlanPart.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == rangePlanPart.getId() ? "" : "," + rangePlanPart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == rangePlanPart.getId() ? "" : "," + rangePlanPart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == rangePlanPart.getId() ? "" : rangePlanPart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, rangePlanPart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,rangePlanPart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/rangePlan/rangePlanPart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_RangePlanPart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_rangePlan_RangePlanPart")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_rangePlan_RangePlanPart");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_RangePlanPart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = rangePlanPartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_rangePlan_RangePlanPart,X6Basic_1.0) */
// 自定义代码
