...
@Repository
public class X6BasicTestDaoImpl extends ExtendedGenericDaoImpl<X6BasicTest, Long> implements X6BasicTestDao {
	@Override
	...
	@Override
	public X6BasicTestDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicTestDealInfo dealInfo = new X6BasicTestDealInfo();
	...
	@Override
	public X6BasicTestDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicTest mainObj, Staff dealStaff) {
		X6BasicTestDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicTestDealInfo.class);
		...
		List<X6BasicTestDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	 /* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_test_Test,X6Basic_1.0) */
// 自定义代码
