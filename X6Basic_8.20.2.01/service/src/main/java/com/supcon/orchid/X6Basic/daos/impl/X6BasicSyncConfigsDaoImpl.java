...
@Repository
public class X6BasicSyncConfigsDaoImpl extends ExtendedGenericDaoImpl<X6BasicSyncConfigs, Long> implements X6BasicSyncConfigsDao {
	@Override
	...
	@Override
	public X6BasicSyncConfigsDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicSyncConfigsDealInfo dealInfo = new X6BasicSyncConfigsDealInfo();
	...
	@Override
	public X6BasicSyncConfigsDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicSyncConfigs mainObj, Staff dealStaff) {
		X6BasicSyncConfigsDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicSyncConfigsDealInfo.class);
		...
		List<X6BasicSyncConfigsDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_syncConfig_SyncConfigs,X6Basic_1.0) */
// 自定义代码
