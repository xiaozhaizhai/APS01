...
import com.supcon.orchid.X6Basic.services.X6BasicMailServerImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicMailServerDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_mailServerService")
@Transactional
public class X6BasicMailServerServiceImpl  extends BaseServiceImpl<X6BasicMailServer> implements X6BasicMailServerService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicMailServer> bapBaseService;
	...
	@Autowired
	private X6BasicMailServerImportService mailServerImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicMailServer.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  mailServerDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicMailServer.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = mailServerDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicMailServer.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicMailServer.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  mailServerDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicMailServer.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicMailServer.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicMailServer> list = (List<X6BasicMailServer>) mailServerDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicMailServer getMailServer(long id){
		return getMailServer(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicMailServer getMailServer(long id, String viewCode){
		X6BasicMailServer mailServer = mailServerDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMailServerAsJSON(long id, String include){
		X6BasicMailServer mailServer = mailServerDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(mailServer, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.common.delete",operType="3")
	public void deleteMailServer(X6BasicMailServer mailServer){
		if(null != mailServer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicMailServer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mailServer.getCode() ? "" : mailServer.getCode().toString());
			AuditUtil.setColumnStringA(null == mailServer.getName() ? "" : mailServer.getName().toString());
			AuditUtil.setColumnStringC(mailServer.getId().toString());
		}
		beforeDeleteMailServer(mailServer);
		mailServerDao.delete(mailServer);
		afterDeleteMailServer(mailServer);
		mailServerDao.flush();
		List<X6BasicMailServer> params = new ArrayList<X6BasicMailServer>();
		params.add(mailServer);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_mailServer");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.common.delete",operType="3")
	public void deleteMailServer(List<Long> mailServerIds){
		deleteMailServer(mailServerIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.common.delete",operType="3")
	public void deleteMailServer(List<Long> mailServerIds, String eventTopic) {
		List<X6BasicMailServer> mailServers = new ArrayList<X6BasicMailServer>();
		for(Long mailServerId : mailServerIds){
			X6BasicMailServer mailServer = getMailServer(mailServerId);
			mailServers.add(mailServer);
			if(null != mailServer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicMailServer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == mailServer.getName() ? "" : "," + mailServer.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == mailServer.getName() ? "" : mailServer.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == mailServer.getCode() ? "" : "," + mailServer.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == mailServer.getCode() ? "" : mailServer.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == mailServer.getId() ? "" : "," + mailServer.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == mailServer.getId() ? "" : mailServer.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(mailServers != null && mailServers.size() > 0 ){
			for(X6BasicMailServer mailServer : mailServers){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),mailServer, mailServer.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(mailServerIds != null && mailServerIds.size() > 0) {
			String hql = "update " + X6BasicMailServer.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = mailServerDao.createQuery(hql);
			query.setParameterList("ids", mailServerIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicMailServer mailServerz : mailServers){
				mailServerz.setValid(false);
				List<X6BasicMailServer> params = new ArrayList<X6BasicMailServer>();
				params.add(mailServerz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_mailServer");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.common.delete",operType="3")
	public void deleteMailServer(long mailServerId, int mailServerVersion){
		this.deleteMailServer(mailServerId, mailServerVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.common.delete",operType="3")
	public void deleteMailServer(long mailServerId, int mailServerVersion,SignatureLog signatureLog){
		X6BasicMailServer mailServer = getMailServer(mailServerId);
		...
		beforeDeleteMailServer(mailServer);
		mailServerDao.delete(mailServerId, mailServerVersion);
		afterDeleteMailServer(mailServer);
		mailServerDao.flush();
		List<X6BasicMailServer> params = new ArrayList<X6BasicMailServer>();
		params.add(mailServer);
		...
		if(signatureLog != null ) {
			...
			businessKey=mailServer.getCode();
			signatureLog.setTableId(mailServer.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.common.delete",operType="3")
	public void deleteMailServer(String mailServerIds){
		this.deleteMailServer(mailServerIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.common.delete",operType="3")
	public void deleteMailServer(String mailServerIds,SignatureLog signatureLog){
		deleteCollection(mailServerIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMailServer(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_mailServer#modelCode:X6Basic_1.0_mailServer_MailServer#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="ec.engine.restore",operType="10")
	public void restoreMailServer(String mailServerIds){
		restoreCollection(mailServerIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreMailServer(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="ec.engine.restore",operType="10")
	public void restoreMailServer(long mailServerId){
		
		findBusinessKeyUsed(mailServerId);	//判断业务主键是否重复
		...
		beforeRestoreMailServer(mailServer);
		mailServer.setValid(true);
		mailServerDao.update(mailServer);
		afterRestoreMailServer(mailServer);
		List<X6BasicMailServer> params = new ArrayList<X6BasicMailServer>();
		params.add(mailServer);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long mailServerId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_mailServer_MailServer");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicMailServer.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicMailServer.TABLE_NAME +" where id = ? )";
			List<Object> list =  mailServerDao.createNativeQuery(sql,mailServerId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseMailServer(List<X6BasicMailServer>  mailServers){
		for(X6BasicMailServer mailServer:mailServers)  {
			saveMailServer(mailServer, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMailServer(List<X6BasicMailServer>  mailServers){
		for(X6BasicMailServer mailServer:mailServers)  {
			saveMailServer(mailServer, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="ec.flow.save")
	public void saveMailServer(X6BasicMailServer mailServer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMailServer(mailServer, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="ec.flow.save")
	public void saveMailServer(X6BasicMailServer mailServer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMailServer(mailServer, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateMailServerCodes(X6BasicMailServer mailServer) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMailServerCodes(mailServer, false);
	}
	
	public void generateMailServerCodes(X6BasicMailServer mailServer, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateMailServerSummarys(X6BasicMailServer mailServer) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMailServerSummarys(mailServer, false);
	}

	public void generateMailServerSummarys(X6BasicMailServer mailServer, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer")
	public void saveMailServer(X6BasicMailServer mailServer,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveMailServer(mailServer, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer")
	public void saveMailServer(X6BasicMailServer mailServer,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_mailServer";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_mailServer");
		String url = null;
		if(mailServer.getId() != null && mailServer.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicMailServer.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_mailServer/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicMailServer.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_mailServer/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, mailServer);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(mailServer);
		beforeSaveMailServer(mailServer, viewIsView);

		if (viewIsView) {
			mailServerDao.saveWithRevertVersion(mailServer);
		} else {
			if(isNew)
				mailServerDao.save(mailServer);
			else
				mailServerDao.merge(mailServer);
		}
		if(null != mailServer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicMailServer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mailServer.getCode() ? "" : mailServer.getCode().toString());
			AuditUtil.setColumnStringA(null == mailServer.getName() ? "" : mailServer.getName().toString());
		...
		dealDatagridsSave(mailServer,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateMailServerCodes(mailServer, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMailServerSummarys(mailServer, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMailServer(mailServer, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, mailServer);
			}
		}
		List<X6BasicMailServer> params = new ArrayList<X6BasicMailServer>();
		mailServerDao.flush();
		mailServerDao.clear();
		mailServer = mailServerDao.load(mailServer.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicMailServer mailServer){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("mailServer", mailServer);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer")
	public void mergeMailServer(X6BasicMailServer mailServer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(mailServer);
		beforeSaveMailServer(mailServer);
		mailServerDao.merge(mailServer);
		if(null != mailServer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicMailServer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mailServer.getCode() ? "" : mailServer.getCode().toString());
			AuditUtil.setColumnStringA(null == mailServer.getName() ? "" : mailServer.getName().toString());
		...
		dealDatagridsSave(mailServer,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveMailServer(mailServer);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicMailServer> findMailServers(Page<X6BasicMailServer> page, Criterion... criterions) {
		return mailServerDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicMailServerDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicMailServerDealInfo> list=mailServerDao.createQuery("from " + X6BasicMailServerDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	public void saveDealInfo(X6BasicMailServerDealInfo dealInfo) {
		mailServerDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicMailServerDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) mailServerDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicMailServerDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = mailServerDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = mailServerDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicMailServerDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = mailServerDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMailServers(Page<X6BasicMailServer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMailServers(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMailServers(Page<X6BasicMailServer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMailServers(Page<X6BasicMailServer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicMailServer> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"mailServer\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"mailServer\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"mailServer\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_mailServer_MailServer" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_mailServer_MailServer" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  mailServerDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  mailServerDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_mailServer").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mailServer\"","X6Basic_1.0_mailServer",customCondition,"X6BASIC_MAIL_SERVERS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"mailServer\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicMailServerDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"mailServer\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"mailServer\".LAY_REC ASC, \"mailServer\".SORT ASC");
				} else {
					orderPart.append(" \"mailServer\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) mailServerDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=mailServerDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = mailServerDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicMailServer> result = new ArrayList<X6BasicMailServer>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicMailServer.class,mailServerDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicMailServer.class,mailServerDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicMailServer mailServer : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(mailServer.getId(), "X6Basic_mailServer_mailServer");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(mailServer.getId(), "X6Basic_mailServer_mailServer");
							if(null != documents && !documents.isEmpty()) {
								mailServer.setDocument(documents.get(0));
								mailServer.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_mailServer")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_mailServer")+"X6Basic_1.0_mailServer".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_mailServer",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicMailServer> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicMailServer> result) {
		View view = null;
		...
		Map<Long, X6BasicMailServer> rsMap = new HashMap<Long, X6BasicMailServer>();
		for (X6BasicMailServer mailServer : result) {
			ids.add(mailServer.getId());
			rsMap.put(mailServer.getId(), mailServer);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(mailServerDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = mailServerDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicMailServer mailServer,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(mailServer.getOwnerStaffId());
		ti.setOwnerPositionId(mailServer.getOwnerPositionId());
		ti.setOwnerDepartmentId(mailServer.getOwnerDepartmentId());
		
		ti.setCreateStaff(mailServer.getCreateStaff());
		ti.setCreateStaffId(mailServer.getCreateStaffId());
		ti.setCreateTime(mailServer.getCreateTime());
		ti.setDeleteStaff(mailServer.getDeleteStaff());
		ti.setDeleteStaffId(mailServer.getDeleteStaffId());
		ti.setDeleteTime(mailServer.getDeleteTime());
		ti.setCreateDepartmentId(mailServer.getCreateDepartmentId());
		ti.setModifyStaff(mailServer.getModifyStaff());
		ti.setModifyStaffId(mailServer.getModifyStaffId());
		ti.setCreatePositionId(mailServer.getCreatePositionId());
		ti.setPositionLayRec(mailServer.getPositionLayRec());
		ti.setTableNo(mailServer.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_mailServer");
		ti.setTargetTableName("X6BASIC_MAIL_SERVERS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_mailServer_MailServer" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  mailServerDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mailServer\"","X6Basic_1.0_mailServer","","X6BASIC_MAIL_SERVERS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"mailServer\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"mailServer\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mailServer\"","X6Basic_1.0_mailServer","","X6BASIC_MAIL_SERVERS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("mailServer.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"mailServer\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"mailServer\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"mailServer\".CID = 1 OR \"mailServer\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("mailServer.mc\"").append(".MAIL_SERVER) not in ("+notinValue+")");
			}
			
			SQLQuery query = mailServerDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("mailServer", "X6Basic_1.0_mailServer", "mailServer_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=mailServerDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicMailServer.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicMailServer.MODULE_CODE;}
	protected String getModelCode(){return X6BasicMailServer.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param mailServer 模型实例
	 */
	public void checkUniqueConstraint(X6BasicMailServer mailServer){
		if(mailServer.getId()==null){
			List<X6BasicMailServer> mailServers = mailServerDao.findByCriteria(Restrictions.eq("code", mailServer.getCode()), Restrictions.eq("valid", true));
			if(mailServers!=null && !mailServers.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1404797961791", getCurrentUser().getLanguage()), mailServer.getCode());
			}
		} else {
			List<X6BasicMailServer> mailServers = mailServerDao.findByCriteria(Restrictions.ne("id", mailServer.getId()), Restrictions.eq("code", mailServer.getCode()), Restrictions.eq("valid", true));
			if(mailServers!=null && !mailServers.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1404797961791", getCurrentUser().getLanguage()), mailServer.getCode());
			}
	...
	@Override
	public List<X6BasicMailServer> getMailServers(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicMailServer.class);
		if(params!=null){
			...
			return mailServerDao.findByHql("from " + X6BasicMailServer.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return mailServerDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicMailServer> findMailServersByHql(String hql, Object... objects){
		...
		return   mailServerDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicMailServer loadMailServerByBussinessKey(X6BasicMailServer mailServer){
		return loadMailServerByBussinessKey(mailServer.getCode());
	}
	
	@Override
	public Page<X6BasicMailServer> getMailServers(Page<X6BasicMailServer> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicMailServer.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = mailServerDao.findByPage(page, sql);
			}else{
				page = mailServerDao.findAllByPage(page);
			}
			return page;
		}
		page = mailServerDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public X6BasicMailServer loadMailServerByBussinessKey(Serializable bussinessKey){
		return mailServerDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicMailServer> findByProperty(String propertyName, Object object){
		return mailServerDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicMailServer findEntityByProperty(String propertyName, Object object){
		return mailServerDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicMailServer.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = mailServerDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_mailServer_MailServer","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_mailServer", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicMailServer mailServer,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,mailServerDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicMailServer.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, mailServer);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",mailServer.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicMailServer.TABLE_NAME +" where valid = 1";
			list = mailServerDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicMailServer.TABLE_NAME;
			list = mailServerDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicMailServer.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = mailServerDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicMailServerDaoImpl mailServerDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchMailServer(final List<X6BasicMailServer> insertObjs, final List<X6BasicMailServer> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchMailServer(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchMailServer(final List<X6BasicMailServer> insertObjs, final List<X6BasicMailServer> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1404797483241"),importCount));
		...
		Map<Long, X6BasicMailServer> oldUpdateObjsMap = new HashMap<Long, X6BasicMailServer>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicMailServer bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicMailServer> bizObjList = mailServerDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicMailServer bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicMailServer> nmeObjs = new ArrayList<X6BasicMailServer>();
		...
		long id=autoGeneratorID.getNextId(X6BasicMailServer.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicMailServer mailServer:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicMailServer.TABLE_NAME,1,dbId);
			if(mailServer.getCode() != null && !mailServer.getCode().equals("") ){
				m.put(mailServer.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) mailServerDao.createNativeQuery(sql, "X6Basic_1.0_mailServer_MailServer"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		mailServerDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicMailServer.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicMailServer mailServer = updateObjs.get(updateIndex);
						Long id = mailServer.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = mailServer.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicMailServer.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicMailServer.TABLE_NAME + " order by id desc";
//					Query query = mailServerDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicMailServer mailServer = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = mailServer.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		mailServerDaoImpl.getSession().flush();
		mailServerDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicMailServer bizObj = insertObjs.get(insertIndex);
			mailServerImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicMailServer bizObj = updateObjs.get(updateIndex);
			X6BasicMailServer oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			mailServerImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = mailServerDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				mailServerDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = mailServerDao.createNativeQuery(sql, "X6Basic_1.0_mailServer_MailServer").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = mailServerDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = mailServerDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = mailServerDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?mailServerDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  mailServerDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,mailServerDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="ec.flow.save")
	public void saveMailServer(X6BasicMailServer mailServer, DataGridService dataGridService){
		saveMailServer(mailServer, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="ec.flow.save")
	public void saveMailServer(X6BasicMailServer mailServer, DataGridService dataGridService, String viewCode){
		saveMailServer(mailServer, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer")
	public void saveMailServer(X6BasicMailServer mailServer, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveMailServer(mailServer, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer")
	public void saveMailServer(X6BasicMailServer mailServer, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_mailServer";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_mailServer");
		String url = null;
		if(null != mailServer.getId() && mailServer.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicMailServer.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer")
	public Map<Object, Object> mailServerDataGridImport(X6BasicMailServer mailServer, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = mailServer.getVirtualId();
		
		mailServer = mailServerDao.load(mailServer.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, mailServer.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = mailServer.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(mailServer);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, mailServer.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer")
	public void mergeMailServer(X6BasicMailServer mailServer, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(mailServer);
		beforeSaveMailServer(mailServer);
		mailServerDao.merge(mailServer);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicMailServer> getByPage(Page<X6BasicMailServer> page,DetachedCriteria detachedCriteria){
		return mailServerDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_mailServer").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveMailServer(X6BasicMailServer mailServer, Object... objects){
		checkUniqueConstraint(mailServer);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveMailServer(X6BasicMailServer mailServer, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMailServer(X6BasicMailServer mailServer){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteMailServer(X6BasicMailServer mailServer){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreMailServer(X6BasicMailServer mailServer){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreMailServer(X6BasicMailServer mailServer){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicMailServer mailServer, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicMailServer mailServer, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportMailServer(List<X6BasicMailServer> insertObjs, List<X6BasicMailServer> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMailServer,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportMailServer(List<X6BasicMailServer> insertObjs, List<X6BasicMailServer> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMailServer,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_mailServer_MailServer,X6Basic_1.0) */
// 自定义代码
