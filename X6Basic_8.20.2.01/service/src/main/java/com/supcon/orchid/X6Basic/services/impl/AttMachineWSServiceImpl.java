...
import com.supcon.orchid.X6Basic.services.AttMachineWSService;
import com.supcon.orchid.X6Basic.services.AttMachineService;
/* CUSTOM CODE START(wsserviceimpl,import,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="AttMachine",targetNamespace = "http://ws.supcon.com")
public class AttMachineWSServiceImpl implements AttMachineWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getAttMachineByBusinessKey(
		...
		try{
			...
			AttMachine attMachine = attMachineService.loadAttMachineByBussinessKey(businessKey);
			if(attMachine == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, attMachine);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateAttMachine(
		...
		try{
			if(attMachine == null){
				return set500Error(result, "考勤机管理不能为空。");
			}
			...
			if(!beforeUpdateAttMachine(attMachine)){
				return set404Error(result);
			}
			attMachineService.saveAttMachine(attMachine, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addAttMachine(
		...
		try{
			...
			try{
				checkAttMachineData(attMachine);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			attMachineService.saveAttMachine(attMachine, null);
			setSuccessResult(result, attMachine.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteAttMachineByBusinessKey(
		...
		try{
			...
			try{
				attMachine = attMachineService.loadAttMachineByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddAttMachine(
		...
		try{
			...
			int count = 1;
			for(AttMachine attMachine : attMachineList){
				if(attMachine.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(attMachine.getId() == null || attMachine.getId().toString().trim().isEmpty()){
			...
			attMachineService.batchImportBaseAttMachine(attMachineList);
			for(AttMachine attMachine : attMachineList){
				idList.add(attMachine.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteAttMachineByPk(
		...
		try{
			...
			for(String k : pk){
				...
				AttMachine temp = null;
				try{
					temp = attMachineService.getAttMachine(id);
				}catch(NullPointerException e){}
			...
			attMachineService.deleteAttMachine(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteAttMachineByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				AttMachine temp = null;
				try{
					temp = attMachineService.loadAttMachineByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getAttMachinePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(AttMachine.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),AttMachine.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,AttMachine.class);
				} catch (Exception e) {
			...
			Page<AttMachine> page = new Page<AttMachine>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<AttMachine> datas = attMachineService.getByPage(page, detachedCriteria);
			List<AttMachine> attMachineList = datas.getResult();
			if (attMachineList == null || attMachineList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(attMachineList.toArray());
			PaginationDTO<AttMachine> pagination = new PaginationDTO<AttMachine>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			AttMachine attMachine = attMachineService.getAttMachine(id);
			if (attMachine == null || !attMachine.isValid()) {
				result.setErrorMessage("找不到考勤机管理");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取考勤机管理对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param attMachine
	 * @return
	 */
	private AttMachine getAttMachine(AttMachine attMachine) {
		if (attMachine.getId() == null && (attMachine.getId() == null || attMachine.getId().toString().trim() == ""))
			throw new RuntimeException("考勤机管理对象业务主键和id不能同时为空");
		if (attMachine.getId() != null) {
			AttMachine result = attMachineService.getAttMachine(attMachine .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return attMachineService.loadAttMachineByBussinessKey(attMachine.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个考勤机管理，则返回false，否则返回true
	 * @param attMachine
	 */
	private boolean beforeUpdateAttMachine(AttMachine attMachine) {
		AttMachine temp = getAttMachine(attMachine);
		if (temp == null) {
			return false;
		}
		if(attMachine.getId() == null){
			attMachine.setId(temp.getId());		
		}
		if(attMachine.getId() == null){
			attMachine.setId(temp.getId());
		}
		
		attMachine.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除考勤机管理对象下所有辅模型对象中的考勤机管理对象
	 * @param attMachine
	 */
	private void clearCircle(AttMachine attMachine) {
	 ...
	 */
	private void checkAttMachineData(AttMachine attMachine){
	}
	/* CUSTOM CODE START(wsserviceimpl,functions,X6Basic_1.0_attMachine_AttMachine,X6Basic_1.0) */
// 自定义代码
