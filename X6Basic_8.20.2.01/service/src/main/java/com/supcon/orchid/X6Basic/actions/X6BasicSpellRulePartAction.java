...
/* CUSTOM CODE START(action,import,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicSpellRulePartAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicSpellRulePart> {
	...
	private X6BasicSpellRulePart spellRulePart;
	...
	//导出excel时,主模型内容
	List<X6BasicSpellRulePart> queryResults = new ArrayList<X6BasicSpellRulePart>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/spellRule/spellRulePart/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/spellRule/spellRulePart/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_spellRule_SpellRulePart" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicSpellRulePart> spellRuleParts;
	
	public List<X6BasicSpellRulePart> getSpellRuleParts() {
		return spellRuleParts;
	}
	...
	private Page<X6BasicSpellRulePart> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			spellRulePart = new X6BasicSpellRulePart();
		} else {
			if(id > 0){
				spellRulePart = spellRulePartService.getSpellRulePart(id, viewCode);
			}
		}
	...
	public Page<X6BasicSpellRulePart> getPage() {
		if (null == page) {
			page = new Page<X6BasicSpellRulePart>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicSpellRulePart getSpellRulePart() {
		return spellRulePart;
	}		

	public void setSpellRulePart(X6BasicSpellRulePart spellRulePart) {
		this.spellRulePart = spellRulePart;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/spellRule/spellRulePart/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = spellRulePartService.getSpellRulePartAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/spellRule/spellRulePart/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1398306848646
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String spellRulePartReferenceCopy() throws Exception{
			X6BasicSpellRulePart oldSpellRulePart = null;
			if(isEditNew()){
				...
				oldSpellRulePart = new X6BasicSpellRulePart();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,spellRulePart.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, spellRulePart.getId());
				...
				oldSpellRulePart = spellRulePartService.getSpellRulePart(id);
			}
			
			
			spellRulePart = spellRulePartService.getSpellRulePart(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_spellRule_SpellRulePart");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicSpellRulePart.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldSpellRulePart, null);
						Method setValue = X6BasicSpellRulePart.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(spellRulePart, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			spellRulePart.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=spellRulePartService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return spellRulePartReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			spellRulePart = spellRulePartService.getSpellRulePart(this.spellRulePart.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,spellRulePart.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = spellRulePartService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(spellRulePartService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(spellRulePartService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_spellRule_SpellRulePart", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"spellRulePart\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"spellRulePart\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"spellRulePart\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"spellRulePart\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			spellRulePartService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		spellRulePartService.findDataGridPage(dg,dgclass,getDgPage(),spellRulePart,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(spellRulePart.getId() != null && spellRulePart.getId() != -1){
				spellRulePart = spellRulePartService.getSpellRulePart(spellRulePart.getId());
			}
			spellRulePartService.findDataGridPage(dg,dgclass,getDgPage(),spellRulePart,searchCondition,params);
	...
	private void save() throws Exception {
		...
		spellRulePartService.saveSpellRulePart(spellRulePart,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "submit");
		bap_validate_datagrids=spellRulePartService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", spellRulePart.getId());
		...
		fileuploads.put("linkId", spellRulePart.getId());
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/spellRule/spellRulePart/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		spellRulePartService.deleteSpellRulePart(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/spellRule/spellRulePart/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "restore");
		spellRulePartService.restoreSpellRulePart(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicSpellRulePart> findExportDataInfos(Page<X6BasicSpellRulePart> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = spellRulePartService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = spellRulePartService.getAssProperty(p.getCode());	
									//String assModelCode = spellRulePartService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			spellRulePartService.findSpellRuleParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			spellRulePartService.findSpellRuleParts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicSpellRulePart> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicSpellRulePart result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".SRP_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.spellRulePart || null == this.spellRulePart.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/spellRule/spellRulePart/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/spellRule/spellRulePart/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/spellRule/spellRulePart/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/spellRule/spellRulePart/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicSpellRulePartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/spellRule/spellRulePart/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_spellRule_SpellRulePart", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_spellRule_SpellRulePart", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/spellRule/spellRulePart/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/spellRule/spellRulePart/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_spellRule_SpellRulePart";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_spellRule_SpellRulePart", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/spellRule/spellRulePart/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
		_auditLog.setColumnStringA(null == spellRulePart.getTeamOrder() ? "" : spellRulePart.getTeamOrder().toString());
		_auditLog.setColumnStringC(spellRulePart.getId().toString());
		_auditLog.setColumnStringD(X6BasicSpellRulePart.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicSpellRulePartAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, spellRulePart,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, spellRulePart,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,spellRulePart);
		...
		printlog.setBusinessId(spellRulePart.getId());
		printlog.setBussinessKey(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
		printlog.setMainDisplay(null == spellRulePart.getTeamOrder() ? "" : spellRulePart.getTeamOrder().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/spellRule/spellRulePart/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(spellRulePart,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/spellRule/spellRulePart/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(spellRulePartService.getSpellRulePart(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/spellRule/spellRulePart/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/spellRule/spellRulePart/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, spellRulePart,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				spellRulePart = spellRulePartService.getSpellRulePart(Long.parseLong(arrId[i]));
				if(null != spellRulePart){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == spellRulePart.getTeamOrder() ? "" : "," + spellRulePart.getTeamOrder().toString()));
					} else {
						_auditLog.setColumnStringA(null == spellRulePart.getTeamOrder() ? "" : spellRulePart.getTeamOrder().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == spellRulePart.getId() ? "" : "," + spellRulePart.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == spellRulePart.getId() ? "" : "," + spellRulePart.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == spellRulePart.getId() ? "" : spellRulePart.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, spellRulePart,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,spellRulePart);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/spellRule/spellRulePart/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_spellRule_SpellRulePart");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_spellRule_SpellRulePart")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_spellRule_SpellRulePart");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_spellRule_SpellRulePart");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = spellRulePartService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_spellRule_SpellRulePart,X6Basic_1.0) */
// 自定义代码
