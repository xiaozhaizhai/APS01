...
/* CUSTOM CODE START(action,import,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class ProductManageAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<ProductManage> {
	...
	//导出excel时,主模型内容
	List<ProductManage> queryResults = new ArrayList<ProductManage>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/baseProduct/productManage/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/baseProduct/productManage/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_baseProduct_ProductManage" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<ProductManage> productManages;
	
	public List<ProductManage> getProductManages() {
	...
	private Page<ProductManage> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(id > 0){
				productManage = productManageService.getProductManage(id, viewCode);
			}
		}
	...
	public Page<ProductManage> getPage() {
		if (null == page) {
			page = new Page<ProductManage>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/baseProduct/productManage/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = productManageService.getProductManageAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/baseProduct/productManage/edit-extra-col")
	public String _editExtraCol() throws Exception{
		if(!isEditNew()){
			if(productManage.getExtraCol()!=null && productManage.getExtraCol().length() > 0) {
				Struts2Utils.renderXml(productManage.getExtraCol());
				return null;
			}
		}
	 ...
	 * @throws Exception
	 */
		public String productManageReferenceCopy() throws Exception{
			ProductManage oldProductManage = null;
			if(isEditNew()){
				...
				oldProductManage = new ProductManage();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,productManage.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, productManage.getId());
				...
				oldProductManage = productManageService.getProductManage(id);
			}
			
			
			productManage = productManageService.getProductManage(refId);
			if (productManage.getExtraCol() != null && productManage.getExtraCol().length() > 0) {
				productManage.setExtraColMap(XmlUtils.xml2Map(productManage.getExtraCol()));
			}
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_baseProduct_ProductManage");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = ProductManage.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProductManage, null);
						Method setValue = ProductManage.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(productManage, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			productManage.setCreateDepartmentId(oldProductManage.getCreateDepartmentId());
			productManage.setCreatePositionId(oldProductManage.getCreatePositionId());
			productManage.setCreateStaffId(oldProductManage.getCreateStaffId());
			productManage.setOwnerDepartmentId(oldProductManage.getOwnerDepartmentId());
			productManage.setOwnerPositionId(oldProductManage.getOwnerPositionId());
			productManage.setOwnerStaffId(oldProductManage.getOwnerStaffId());
			productManage.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/baseProduct/productManage/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/baseProduct/productManage/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=productManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return productManageReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			productManage = productManageService.getProductManage(this.productManage.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,productManage.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/X6Basic/baseProduct/productManage/view",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "view.ftl") }),
		@Action(value = "/X6Basic/baseProduct/productManage/viewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "viewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/baseProduct/productManage/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = productManageService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(productManageService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(productManageService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_baseProduct_ProductManage", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/baseProduct/productManage/refer", results = { @Result(type = FREEMARKER, location = "refer.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/baseProduct/productManage/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.specific,result.unit,result.currQuantity,result.createStaff.name,result.createTime,result.createStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"productManage\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"productManage\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"productManage\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"productManage\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			productManageService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/X6Basic/baseProduct/productManage/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/X6Basic/baseProduct/productManage/refer-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.specific,result.unit,result.currQuantity,result.createStaff.name,result.createTime,result.createStaff.id,result.attrMap.*"})
	 ...
	 * @return
	 */
	@Action(value = "/X6Basic/baseProduct/productManage/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("X6Basic_1.0_baseProduct_ProductManage");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		productManageService.findDataGridPage(dg,dgclass,getDgPage(),productManage,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(productManage.getId() != null && productManage.getId() != -1){
				productManage = productManageService.getProductManage(productManage.getId());
			}
			productManageService.findDataGridPage(dg,dgclass,getDgPage(),productManage,searchCondition,params);
	...
	private void save() throws Exception {
		...
		productManageService.saveProductManage(productManage,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/baseProduct/productManage/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/X6Basic/baseProduct/productManage/view/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "submit");
		bap_validate_datagrids=productManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", productManage.getId());
		...
		fileuploads.put("linkId", productManage.getId());
		bapAfterCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/baseProduct/productManage/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		productManageService.deleteProductManage(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/baseProduct/productManage/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "restore");
		productManageService.restoreProductManage(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/baseProduct/productManage/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/baseProduct/productManage/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "showDealInfoData");
		if (null != tableInfoId) {
			int count=productManageService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = productManageService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = productManageService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<ProductManage> findExportDataInfos(Page<ProductManage> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = productManageService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = productManageService.getAssProperty(p.getCode());	
									//String assModelCode = productManageService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			productManageService.findProductManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			productManageService.findProductManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<ProductManage> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(ProductManage result : results){
	...
	public boolean isEditNew(){
		//return null == this.productManage || null == this.productManage.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/baseProduct/productManage/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/baseProduct/productManage/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/baseProduct/productManage/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/baseProduct/productManage/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.ProductManageService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_baseProduct", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_baseProduct");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/baseProduct/productManage/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_baseProduct_ProductManage", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_baseProduct_ProductManage", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/baseProduct/productManage/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/baseProduct/productManage/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_baseProduct_ProductManage";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_baseProduct_ProductManage", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/baseProduct/productManage/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		if(null != id) {
			productManage = productManageService.getProductManage(id);
		}
		...
		_auditLog.setColumnStringB(null == productManage.getCode() ? "" : productManage.getCode().toString());
		_auditLog.setColumnStringA(null == productManage.getName() ? "" : productManage.getName().toString());
		_auditLog.setColumnStringC(productManage.getId().toString());
		_auditLog.setColumnStringD(ProductManage.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_baseProduct");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.ProductManageAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, productManage,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, productManage,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,productManage);
		...
		printlog.setEntity("X6Basic_1.0_baseProduct");
		...
		printlog.setBusinessId(productManage.getId());
		printlog.setBussinessKey(null == productManage.getCode() ? "" : productManage.getCode().toString());
		printlog.setMainDisplay(null == productManage.getName() ? "" : productManage.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/baseProduct/productManage/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(productManage,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/baseProduct/productManage/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(productManageService.getProductManage(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/baseProduct/productManage/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/baseProduct/productManage/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, productManage,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				productManage = productManageService.getProductManage(Long.parseLong(arrId[i]));
				if(null != productManage){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == productManage.getName() ? "" : "," + productManage.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == productManage.getName() ? "" : productManage.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == productManage.getCode() ? "" : "," + productManage.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == productManage.getCode() ? "" : productManage.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == productManage.getId() ? "" : "," + productManage.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == productManage.getId() ? "" : productManage.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, productManage,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,productManage);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/baseProduct/productManage/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<ProductManage> findExportPageInfos(Page<ProductManage> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_baseProduct" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = productManageService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(ProductManage queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = productManageService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = productManageService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_baseProduct_ProductManage");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_baseProduct_ProductManage")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_baseProduct_ProductManage");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_baseProduct_ProductManage");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = productManageService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_baseProduct_ProductManage,X6Basic_1.0) */
// 自定义代码
