...
import com.supcon.orchid.X6Basic.services.X6BasicTimeRegionParImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicTimeRegionParDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_timeRegionParService")
@Transactional
public class X6BasicTimeRegionParServiceImpl  extends BaseServiceImpl<X6BasicTimeRegionPar> implements X6BasicTimeRegionParService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicTimeRegionPar> bapBaseService;
	...
	@Autowired
	private X6BasicTimeRegionParImportService timeRegionParImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicTimeRegionPar.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  timeRegionParDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicTimeRegionPar.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = timeRegionParDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicTimeRegionPar.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicTimeRegionPar.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  timeRegionParDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicTimeRegionPar.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicTimeRegionPar.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicTimeRegionPar> list = (List<X6BasicTimeRegionPar>) timeRegionParDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicTimeRegionPar getTimeRegionPar(long id){
		return getTimeRegionPar(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicTimeRegionPar getTimeRegionPar(long id, String viewCode){
		X6BasicTimeRegionPar timeRegionPar = timeRegionParDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getTimeRegionParAsJSON(long id, String include){
		X6BasicTimeRegionPar timeRegionPar = timeRegionParDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(timeRegionPar, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegionPar(X6BasicTimeRegionPar timeRegionPar){
		if(null != timeRegionPar && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegionPar.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
			AuditUtil.setColumnStringA(null == timeRegionPar.getName() ? "" : timeRegionPar.getName().toString());
			AuditUtil.setColumnStringC(timeRegionPar.getId().toString());
		}
		beforeDeleteTimeRegionPar(timeRegionPar);
		timeRegionParDao.deletePhysical(timeRegionPar);
		afterDeleteTimeRegionPar(timeRegionPar);
		timeRegionParDao.flush();
		List<X6BasicTimeRegionPar> params = new ArrayList<X6BasicTimeRegionPar>();
		params.add(timeRegionPar);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegionPar(List<Long> timeRegionParIds){
		deleteTimeRegionPar(timeRegionParIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegionPar(List<Long> timeRegionParIds, String eventTopic) {
		List<X6BasicTimeRegionPar> timeRegionPars = new ArrayList<X6BasicTimeRegionPar>();
		for(Long timeRegionParId : timeRegionParIds){
			X6BasicTimeRegionPar timeRegionPar = getTimeRegionPar(timeRegionParId);
			timeRegionPars.add(timeRegionPar);
			if(null != timeRegionPar && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegionPar.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == timeRegionPar.getName() ? "" : "," + timeRegionPar.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == timeRegionPar.getName() ? "" : timeRegionPar.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == timeRegionPar.getId() ? "" : "," + timeRegionPar.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == timeRegionPar.getId() ? "" : "," + timeRegionPar.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(timeRegionPars != null && timeRegionPars.size() > 0 ){
			for(X6BasicTimeRegionPar timeRegionPar : timeRegionPars){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = X6BasicTimeRegionPar.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(timeRegionPar));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),timeRegionPar, timeRegionPar.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(timeRegionParIds != null && timeRegionParIds.size() > 0) {
			String hql = "delete " + X6BasicTimeRegionPar.JPA_NAME + " where id in(:ids)";
			Query query = timeRegionParDao.createQuery(hql);
			query.setParameterList("ids", timeRegionParIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicTimeRegionPar timeRegionParz : timeRegionPars){
				timeRegionParz.setValid(false);
				List<X6BasicTimeRegionPar> params = new ArrayList<X6BasicTimeRegionPar>();
				params.add(timeRegionParz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_timeRegion");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegionPar(long timeRegionParId, int timeRegionParVersion){
		this.deleteTimeRegionPar(timeRegionParId, timeRegionParVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegionPar(long timeRegionParId, int timeRegionParVersion,SignatureLog signatureLog){
		X6BasicTimeRegionPar timeRegionPar = getTimeRegionPar(timeRegionParId);
		...
		beforeDeleteTimeRegionPar(timeRegionPar);
		timeRegionParDao.delete(timeRegionParId, timeRegionParVersion);
		afterDeleteTimeRegionPar(timeRegionPar);
		timeRegionParDao.flush();
		List<X6BasicTimeRegionPar> params = new ArrayList<X6BasicTimeRegionPar>();
		params.add(timeRegionPar);
		...
		if(signatureLog != null ) {
			...
			businessKey=timeRegionPar.getId();
			signatureLog.setTableId(timeRegionPar.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegionPar(String timeRegionParIds){
		this.deleteTimeRegionPar(timeRegionParIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.common.delete",operType="3")
	public void deleteTimeRegionPar(String timeRegionParIds,SignatureLog signatureLog){
		deleteCollection(timeRegionParIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteTimeRegionPar(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_timeRegion#modelCode:X6Basic_1.0_timeRegion_TimeRegionPar#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="ec.engine.restore",operType="10")
	public void restoreTimeRegionPar(String timeRegionParIds){
		restoreCollection(timeRegionParIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreTimeRegionPar(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="ec.engine.restore",operType="10")
	public void restoreTimeRegionPar(long timeRegionParId){
		
		findBusinessKeyUsed(timeRegionParId);	//判断业务主键是否重复
		...
		beforeRestoreTimeRegionPar(timeRegionPar);
		timeRegionPar.setValid(true);
		timeRegionParDao.update(timeRegionPar);
		afterRestoreTimeRegionPar(timeRegionPar);
		List<X6BasicTimeRegionPar> params = new ArrayList<X6BasicTimeRegionPar>();
		params.add(timeRegionPar);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long timeRegionParId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_timeRegion_TimeRegionPar");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicTimeRegionPar.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicTimeRegionPar.TABLE_NAME +" where id = ? )";
			List<Object> list =  timeRegionParDao.createNativeQuery(sql,timeRegionParId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseTimeRegionPar(List<X6BasicTimeRegionPar>  timeRegionPars){
		for(X6BasicTimeRegionPar timeRegionPar:timeRegionPars)  {
			saveTimeRegionPar(timeRegionPar, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseTimeRegionPar(List<X6BasicTimeRegionPar>  timeRegionPars){
		for(X6BasicTimeRegionPar timeRegionPar:timeRegionPars)  {
			saveTimeRegionPar(timeRegionPar, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="ec.flow.save")
	public void saveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveTimeRegionPar(timeRegionPar, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="ec.flow.save")
	public void saveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveTimeRegionPar(timeRegionPar, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateTimeRegionParCodes(X6BasicTimeRegionPar timeRegionPar) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTimeRegionParCodes(timeRegionPar, false);
	}
	
	public void generateTimeRegionParCodes(X6BasicTimeRegionPar timeRegionPar, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateTimeRegionParSummarys(X6BasicTimeRegionPar timeRegionPar) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTimeRegionParSummarys(timeRegionPar, false);
	}

	public void generateTimeRegionParSummarys(X6BasicTimeRegionPar timeRegionPar, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar")
	public void saveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveTimeRegionPar(timeRegionPar, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar")
	public void saveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "X6Basic_1.0_timeRegion");
		String url = null;
		if(timeRegionPar.getId() != null && timeRegionPar.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegionPar.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, timeRegionPar);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(timeRegionPar);
		beforeSaveTimeRegionPar(timeRegionPar, viewIsView);

		if (viewIsView) {
			timeRegionParDao.saveWithRevertVersion(timeRegionPar);
		} else {
			if(isNew)
				timeRegionParDao.save(timeRegionPar);
			else
				timeRegionParDao.merge(timeRegionPar);
		}
		if(null != timeRegionPar && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegionPar.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
			AuditUtil.setColumnStringA(null == timeRegionPar.getName() ? "" : timeRegionPar.getName().toString());
		...
		dealDatagridsSave(timeRegionPar,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateTimeRegionParCodes(timeRegionPar, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateTimeRegionParSummarys(timeRegionPar, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveTimeRegionPar(timeRegionPar, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, timeRegionPar);
			}
		}
		List<X6BasicTimeRegionPar> params = new ArrayList<X6BasicTimeRegionPar>();
		timeRegionParDao.flush();
		timeRegionParDao.clear();
		timeRegionPar = timeRegionParDao.load(timeRegionPar.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicTimeRegionPar timeRegionPar){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("timeRegionPar", timeRegionPar);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar")
	public void mergeTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(timeRegionPar);
		beforeSaveTimeRegionPar(timeRegionPar);
		timeRegionParDao.merge(timeRegionPar);
		if(null != timeRegionPar && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegionPar.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == timeRegionPar.getId() ? "" : timeRegionPar.getId().toString());
			AuditUtil.setColumnStringA(null == timeRegionPar.getName() ? "" : timeRegionPar.getName().toString());
		...
		dealDatagridsSave(timeRegionPar,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveTimeRegionPar(timeRegionPar);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicTimeRegionPar> findTimeRegionPars(Page<X6BasicTimeRegionPar> page, Criterion... criterions) {
		return timeRegionParDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTimeRegionPars(Page<X6BasicTimeRegionPar> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findTimeRegionPars(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTimeRegionPars(Page<X6BasicTimeRegionPar> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTimeRegionPars(Page<X6BasicTimeRegionPar> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicTimeRegionPar> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"timeRegionPar\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"timeRegionPar\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"timeRegionPar\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"timeRegionPar\".TIME_REGIONID IN ( SELECT ID FROM X6BASIC_TIME_REGIONS \"timeRegion\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "X6Basic_1.0_timeRegion" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  timeRegionParDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  timeRegionParDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_timeRegion").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"timeRegion\"","X6Basic_1.0_timeRegion","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"timeRegionPar\".LAY_REC ASC, \"timeRegionPar\".SORT ASC");
				} else {
					orderPart.append(" \"timeRegionPar\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) timeRegionParDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=timeRegionParDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = timeRegionParDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicTimeRegionPar> result = new ArrayList<X6BasicTimeRegionPar>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicTimeRegionPar.class,timeRegionParDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicTimeRegionPar.class,timeRegionParDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicTimeRegionPar timeRegionPar : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(timeRegionPar.getId(), "X6Basic_timeRegion_timeRegionPar");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(timeRegionPar.getId(), "X6Basic_timeRegion_timeRegionPar");
							if(null != documents && !documents.isEmpty()) {
								timeRegionPar.setDocument(documents.get(0));
								timeRegionPar.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicTimeRegionPar> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicTimeRegionPar> result) {
		View view = null;
		...
		Map<Long, X6BasicTimeRegionPar> rsMap = new HashMap<Long, X6BasicTimeRegionPar>();
		for (X6BasicTimeRegionPar timeRegionPar : result) {
			ids.add(timeRegionPar.getId());
			rsMap.put(timeRegionPar.getId(), timeRegionPar);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(timeRegionParDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = timeRegionParDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_timeRegion_TimeRegionPar" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="X6Basic_1.0_timeRegion_TimeRegionPar" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  timeRegionParDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"timeRegionPar\"","X6Basic_1.0_timeRegion","","X6BASIC_TIME_REGION_PARS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"timeRegionPar\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"timeRegionPar\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"timeRegionPar\"","X6Basic_1.0_timeRegion","","X6BASIC_TIME_REGION_PARS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "X6Basic_1.0_timeRegion_TimeRegionPar" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("timeRegionPar.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"timeRegionPar\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"timeRegionPar\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"timeRegionPar\".CID = 1 OR \"timeRegionPar\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("timeRegionPar.mc\"").append(".TIME_REGION_PAR) not in ("+notinValue+")");
			}
			
			SQLQuery query = timeRegionParDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=timeRegionParDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicTimeRegionPar.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicTimeRegionPar.MODULE_CODE;}
	protected String getModelCode(){return X6BasicTimeRegionPar.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param timeRegionPar 模型实例
	 */
	public void checkUniqueConstraint(X6BasicTimeRegionPar timeRegionPar){
	}
	
	
	@Override
	public List<X6BasicTimeRegionPar> getTimeRegionPars(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicTimeRegionPar.class);
		if(params!=null){
			...
			return timeRegionParDao.findByHql("from " + X6BasicTimeRegionPar.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return timeRegionParDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicTimeRegionPar> findTimeRegionParsByHql(String hql, Object... objects){
		...
		return   timeRegionParDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicTimeRegionPar loadTimeRegionParByBussinessKey(X6BasicTimeRegionPar timeRegionPar){
		return loadTimeRegionParByBussinessKey(timeRegionPar.getId());
	}
	
	@Override
	public Page<X6BasicTimeRegionPar> getTimeRegionPars(Page<X6BasicTimeRegionPar> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicTimeRegionPar.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = timeRegionParDao.findByPage(page, sql);
			}else{
				page = timeRegionParDao.findAllByPage(page);
			}
			return page;
		}
		page = timeRegionParDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicTimeRegionPar> findByTreeDataGridTimeRegion(List<X6BasicTimeRegionPar> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(X6BasicTimeRegionPar timeRegionPar : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridTimeRegion(Page<X6BasicTimeRegionPar> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<X6BasicTimeRegionPar> timeRegionPars = dgPage.getResult();
			for(X6BasicTimeRegionPar timeRegionPar : timeRegionPars){
				
			}
			dgPage.setResult(timeRegionPars);
		}
	...
	public X6BasicTimeRegionPar loadTimeRegionParByBussinessKey(Serializable bussinessKey){
		return timeRegionParDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicTimeRegionPar> findByProperty(String propertyName, Object object){
		return timeRegionParDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicTimeRegionPar findEntityByProperty(String propertyName, Object object){
		return timeRegionParDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicTimeRegionPar.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = timeRegionParDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_timeRegion_TimeRegionPar","runtime");
		try {
	...
	public void dealDatagridsSave(X6BasicTimeRegionPar timeRegionPar,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,timeRegionParDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicTimeRegionPar.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, timeRegionPar);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",timeRegionPar.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicTimeRegionPar.TABLE_NAME +" where valid = 1";
			list = timeRegionParDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicTimeRegionPar.TABLE_NAME;
			list = timeRegionParDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicTimeRegionPar.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = timeRegionParDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicTimeRegionParDaoImpl timeRegionParDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchTimeRegionPar(final List<X6BasicTimeRegionPar> insertObjs, final List<X6BasicTimeRegionPar> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchTimeRegionPar(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchTimeRegionPar(final List<X6BasicTimeRegionPar> insertObjs, final List<X6BasicTimeRegionPar> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.randon1459214032452"),importCount));
		...
		Map<Long, X6BasicTimeRegionPar> oldUpdateObjsMap = new HashMap<Long, X6BasicTimeRegionPar>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicTimeRegionPar bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicTimeRegionPar> bizObjList = timeRegionParDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicTimeRegionPar bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicTimeRegionPar> nmeObjs = new ArrayList<X6BasicTimeRegionPar>();
		...
		long id=autoGeneratorID.getNextId(X6BasicTimeRegionPar.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicTimeRegionPar timeRegionPar:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicTimeRegionPar.TABLE_NAME,1,dbId);
			if(timeRegionPar.getId() != null ){
				m.put(timeRegionPar.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) timeRegionParDao.createNativeQuery(sql, "X6Basic_1.0_timeRegion_TimeRegionPar"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		timeRegionParDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicTimeRegionPar.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicTimeRegionPar timeRegionPar = updateObjs.get(updateIndex);
						Long id = timeRegionPar.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = timeRegionPar.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(timeRegionPar);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicTimeRegionPar.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicTimeRegionPar.TABLE_NAME + " order by id desc";
//					Query query = timeRegionParDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicTimeRegionPar timeRegionPar = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = timeRegionPar.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		timeRegionParDaoImpl.getSession().flush();
		timeRegionParDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicTimeRegionPar bizObj = insertObjs.get(insertIndex);
			timeRegionParImportService.saveImportDataLog(bizObj, null, "id", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicTimeRegionPar bizObj = updateObjs.get(updateIndex);
			X6BasicTimeRegionPar oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			timeRegionParImportService.saveImportDataLog(bizObj, oldBizObj, "id", "name");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<X6BasicTimeRegionPar> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<X6BasicTimeRegionPar> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) timeRegionParDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<X6BasicTimeRegionPar> result = getResult(page, query, new PendingResultTransformer(X6BasicTimeRegionPar.class,timeRegionParDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(X6BasicTimeRegionPar model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = timeRegionParDao.createNativeQuery(sql, "X6Basic_1.0_timeRegion_TimeRegionPar").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = timeRegionParDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = timeRegionParDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = timeRegionParDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?timeRegionParDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  timeRegionParDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,timeRegionParDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="ec.flow.save")
	public void saveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, DataGridService dataGridService){
		saveTimeRegionPar(timeRegionPar, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="ec.flow.save")
	public void saveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, DataGridService dataGridService, String viewCode){
		saveTimeRegionPar(timeRegionPar, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar")
	public void saveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveTimeRegionPar(timeRegionPar, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar")
	public void saveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_timeRegion";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_timeRegion");
		String url = null;
		if(null != timeRegionPar.getId() && timeRegionPar.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicTimeRegionPar.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar")
	public Map<Object, Object> timeRegionParDataGridImport(X6BasicTimeRegionPar timeRegionPar, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = timeRegionPar.getVirtualId();
		
		timeRegionPar = timeRegionParDao.load(timeRegionPar.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, timeRegionPar.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = timeRegionPar.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(timeRegionPar);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, timeRegionPar.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar")
	public void mergeTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(timeRegionPar);
		beforeSaveTimeRegionPar(timeRegionPar);
		timeRegionParDao.merge(timeRegionPar);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_timeRegion",model="X6Basic_1.0_timeRegion_TimeRegionPar",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicTimeRegionPar> getByPage(Page<X6BasicTimeRegionPar> page,DetachedCriteria detachedCriteria){
		return timeRegionParDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, Object... objects){
		checkUniqueConstraint(timeRegionPar);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveTimeRegionPar(X6BasicTimeRegionPar timeRegionPar, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteTimeRegionPar(X6BasicTimeRegionPar timeRegionPar){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
      //时间区间[
	   	 String X6pageName= InternationalResource.get("X6Basic.timeRegion.radion137146923424", getCurrentLanguage());
	   	 //]已被其他业务引用，不能删除！
	   	 String X6pageDelete=InternationalResource.get("X6Basic.timeRegion.radion137142348224", getCurrentLanguage());
		 if(!bapBaseService.canDelete(timeRegionPar)){	
				throw new BAPException(X6pageName+timeRegionPar.getName()+X6pageDelete);
			}
/* CUSTOM CODE END */
	}
	
	private void afterDeleteTimeRegionPar(X6BasicTimeRegionPar timeRegionPar){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreTimeRegionPar(X6BasicTimeRegionPar timeRegionPar){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreTimeRegionPar(X6BasicTimeRegionPar timeRegionPar){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicTimeRegionPar timeRegionPar, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicTimeRegionPar timeRegionPar, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportTimeRegionPar(List<X6BasicTimeRegionPar> insertObjs, List<X6BasicTimeRegionPar> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportTimeRegionPar,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportTimeRegionPar(List<X6BasicTimeRegionPar> insertObjs, List<X6BasicTimeRegionPar> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportTimeRegionPar,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_timeRegion_TimeRegionPar,X6Basic_1.0) */
// 自定义代码
@Override
	public Boolean deleteRegionPart(Long regionPartId) {
		X6BasicTimeRegionPar timeregionPart=timeRegionParDao.load(regionPartId);
		if(timeregionPart!=null){
			if(!bapBaseService.canDelete(timeregionPart)){
				return true;
			}
		}
