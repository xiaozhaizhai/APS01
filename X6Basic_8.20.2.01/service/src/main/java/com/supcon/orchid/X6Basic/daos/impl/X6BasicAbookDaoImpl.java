...
@Repository
public class X6BasicAbookDaoImpl extends ExtendedGenericDaoImpl<X6BasicAbook, Long> implements X6BasicAbookDao {
	@Override
	...
	@Override
	public X6BasicAbookDealInfo copyAndSaveDealInfo(DealInfo di) {
		X6BasicAbookDealInfo dealInfo = new X6BasicAbookDealInfo();
	...
	@Override
	public X6BasicAbookDealInfo copyAndSaveDealInfo(DealInfo di, X6BasicAbook mainObj, Staff dealStaff) {
		X6BasicAbookDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(X6BasicAbookDealInfo.class);
		...
		List<X6BasicAbookDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
