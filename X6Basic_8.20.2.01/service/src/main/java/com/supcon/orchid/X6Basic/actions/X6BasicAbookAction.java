...
/* CUSTOM CODE START(action,import,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
import com.supcon.orchid.foundation.entities.Department;
import com.supcon.orchid.foundation.services.DepartmentService;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicAbookAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicAbook> {
	...
	//导出excel时,主模型内容
	List<X6BasicAbook> queryResults = new ArrayList<X6BasicAbook>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/addressBook/abook/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/addressBook/abook/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_addressBook_Abook" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicAbook> abooks;
	
	public List<X6BasicAbook> getAbooks() {
		return abooks;
	}
	...
	private Page<X6BasicAbook> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			abook = new X6BasicAbook();
		} else {
			if(id > 0){
				abook = abookService.getAbook(id, viewCode);
			}
		}
	...
	public Page<X6BasicAbook> getPage() {
		if (null == page) {
			page = new Page<X6BasicAbook>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public X6BasicAbook getAbook() {
		return abook;
	}		

	public void setAbook(X6BasicAbook abook) {
		this.abook = abook;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/addressBook/abook/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = abookService.getAbookAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/addressBook/abook/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String abookReferenceCopy() throws Exception{
			X6BasicAbook oldAbook = null;
			if(isEditNew()){
				...
				oldAbook = new X6BasicAbook();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,abook.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, abook.getId());
				...
				oldAbook = abookService.getAbook(id);
			}
			
			
			abook = abookService.getAbook(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_addressBook_Abook");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicAbook.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldAbook, null);
						Method setValue = X6BasicAbook.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(abook, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			abook.setCreateDepartmentId(oldAbook.getCreateDepartmentId());
			abook.setCreatePositionId(oldAbook.getCreatePositionId());
			abook.setCreateStaffId(oldAbook.getCreateStaffId());
			abook.setOwnerDepartmentId(oldAbook.getOwnerDepartmentId());
			abook.setOwnerPositionId(oldAbook.getOwnerPositionId());
			abook.setOwnerStaffId(oldAbook.getOwnerStaffId());
			abook.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_addressBook_Abook", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=abookService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return abookReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			abook = abookService.getAbook(this.abook.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,abook.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = abookService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(abookService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(abookService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_addressBook_Abook", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"abook\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"abook\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"abook\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"abook\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		abookService.findDataGridPage(dg,dgclass,getDgPage(),abook,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(abook.getId() != null && abook.getId() != -1){
				abook = abookService.getAbook(abook.getId());
			}
			abookService.findDataGridPage(dg,dgclass,getDgPage(),abook,searchCondition,params);
	...
	private void save() throws Exception {
		...
		abookService.saveAbook(abook,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "submit");
		bap_validate_datagrids=abookService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", abook.getId());
		...
		fileuploads.put("linkId", abook.getId());
		bapAfterCustomMethod("X6Basic_1.0_addressBook_Abook", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/addressBook/abook/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		abookService.deleteAbook(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_addressBook_Abook", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/addressBook/abook/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "restore");
		abookService.restoreAbook(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_addressBook_Abook", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/addressBook/abook/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/addressBook/abook/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "showDealInfoData");
		if (null != tableInfoId) {
			int count=abookService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = abookService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = abookService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_addressBook_Abook", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicAbook> findExportDataInfos(Page<X6BasicAbook> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = abookService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = abookService.getAssProperty(p.getCode());	
									//String assModelCode = abookService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			abookService.findAbooks(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			abookService.findAbooks(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicAbook> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicAbook result : results){
	...
	public boolean isEditNew(){
		//return null == this.abook || null == this.abook.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/addressBook/abook/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_addressBook_Abook", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/addressBook/abook/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/addressBook/abook/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/addressBook/abook/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicAbookService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_addressBook", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_addressBook");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_addressBook_Abook", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/addressBook/abook/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_addressBook_Abook", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_addressBook_Abook", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/addressBook/abook/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/addressBook/abook/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_addressBook_Abook";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_addressBook_Abook", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/addressBook/abook/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == abook.getId() ? "" : abook.getId().toString());
		_auditLog.setColumnStringA(null == abook.getName() ? "" : abook.getName().toString());
		_auditLog.setColumnStringC(abook.getId().toString());
		_auditLog.setColumnStringD(X6BasicAbook.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_addressBook");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicAbookAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, abook,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, abook,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,abook);
		...
		printlog.setEntity("X6Basic_1.0_addressBook");
		...
		printlog.setBusinessId(abook.getId());
		printlog.setBussinessKey(null == abook.getId() ? "" : abook.getId().toString());
		printlog.setMainDisplay(null == abook.getName() ? "" : abook.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/addressBook/abook/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(abook,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/addressBook/abook/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(abookService.getAbook(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/addressBook/abook/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/addressBook/abook/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, abook,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				abook = abookService.getAbook(Long.parseLong(arrId[i]));
				if(null != abook){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == abook.getName() ? "" : "," + abook.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == abook.getName() ? "" : abook.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == abook.getId() ? "" : "," + abook.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == abook.getId() ? "" : abook.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == abook.getId() ? "" : "," + abook.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == abook.getId() ? "" : abook.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, abook,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,abook);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/addressBook/abook/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicAbook> findExportPageInfos(Page<X6BasicAbook> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_addressBook" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = abookService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicAbook queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = abookService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = abookService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_addressBook_Abook");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_addressBook_Abook")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_addressBook_Abook");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_addressBook_Abook");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = abookService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_addressBook_Abook,X6Basic_1.0) */
// 自定义代码
  //  private static final long serialVersionUID = 1L;
	private Page<Map<String, Object>> records; // JDBC查询结果
	...
	private String workinfoFjhm;
	private String staffZWMC;
	private String staffFJH;
//	private String refernce;
	private boolean companyFlag=false;
//	private Long id;
	private List<Department> departmentList;
//	private String callBackFuncName;
	@Value("${bap.addressbook.need.dept.tree.flag}")
	private boolean addressbookNeedDeptTreeFlag = true;
	@Value("${include.mail.module.flag}")
	private boolean includeMailModuleFlag = false;

	public boolean getAddressbookNeedDeptTreeFlag() {
		return addressbookNeedDeptTreeFlag;
	}

	public boolean getIncludeMailModuleFlag() {
		return includeMailModuleFlag;
	}
...
//	}
	...
	public String getWorkinfoFjhm() {
		return workinfoFjhm;
	}
	public void setWorkinfoFjhm(String workinfoFjhm) {
		this.workinfoFjhm = workinfoFjhm;
	}
	public String getStaffZWMC() {
		return staffZWMC;
	}
	public void setStaffZWMC(String staffZWMC) {
		this.staffZWMC = staffZWMC;
	}
	public String getStaffFJH() {
		return staffFJH;
	}
	public void setStaffFJH(String staffFJH) {
		this.staffFJH = staffFJH;
	}
	//private static final Logger logger = LoggerFactory.getLogger(AddressBookAction.class);
	/**
	 * 通讯录查询
	 */
	@Action(value = "/X6Basic/addressBook/abook/getAddressBook", results = {
			@Result(name = "success", location = "../abook/addressBook.ftl", type = FREEMARKER),
			@Result(name = "mail", location = "../abook/addressBook.ftl", type =FREEMARKER) })
	public String getAddressBook() {
		if ("mail".equals(request.getParameter("_from_"))) {
			return "mail";
		}
		return SUCCESS;
	}
