...
import com.supcon.orchid.X6Basic.services.X6BasicSpellRuleImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.X6Basic.daos.impl.X6BasicSpellRuleDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("x6Basic_spellRuleService")
@Transactional
public class X6BasicSpellRuleServiceImpl  extends BaseServiceImpl<X6BasicSpellRule> implements X6BasicSpellRuleService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<X6BasicSpellRule> bapBaseService;
	...
	@Autowired
	private X6BasicSpellRuleImportService spellRuleImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + X6BasicSpellRule.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  spellRuleDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + X6BasicSpellRule.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = spellRuleDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicSpellRule.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + X6BasicSpellRule.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  spellRuleDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + X6BasicSpellRule.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + X6BasicSpellRule.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<X6BasicSpellRule> list = (List<X6BasicSpellRule>) spellRuleDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public X6BasicSpellRule getSpellRule(long id){
		return getSpellRule(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSpellRule getSpellRule(long id, String viewCode){
		X6BasicSpellRule spellRule = spellRuleDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getSpellRuleAsJSON(long id, String include){
		X6BasicSpellRule spellRule = spellRuleDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(spellRule, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.common.delete",operType="3")
	public void deleteSpellRule(X6BasicSpellRule spellRule){
		if(null != spellRule && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRule.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == spellRule.getCode() ? "" : spellRule.getCode().toString());
			AuditUtil.setColumnStringA(null == spellRule.getName() ? "" : spellRule.getName().toString());
			AuditUtil.setColumnStringC(spellRule.getId().toString());
		}
		beforeDeleteSpellRule(spellRule);
		spellRuleDao.delete(spellRule);
		afterDeleteSpellRule(spellRule);
		spellRuleDao.flush();
		List<X6BasicSpellRule> params = new ArrayList<X6BasicSpellRule>();
		List<X6BasicSpellRulePart> spellRulePartList = spellRulePartDao.findByCriteria(Restrictions.eq("spellRule", spellRule));
		spellRule.setSpellRulePartList(spellRulePartList);
		params.add(spellRule);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_spellRule");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.common.delete",operType="3")
	public void deleteSpellRule(List<Long> spellRuleIds){
		deleteSpellRule(spellRuleIds, null);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.common.delete",operType="3")
	public void deleteSpellRule(List<Long> spellRuleIds, String eventTopic) {
		List<X6BasicSpellRule> spellRules = new ArrayList<X6BasicSpellRule>();
		for(Long spellRuleId : spellRuleIds){
			X6BasicSpellRule spellRule = getSpellRule(spellRuleId);
			spellRules.add(spellRule);
			if(null != spellRule && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRule.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == spellRule.getName() ? "" : "," + spellRule.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == spellRule.getName() ? "" : spellRule.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == spellRule.getCode() ? "" : "," + spellRule.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == spellRule.getCode() ? "" : spellRule.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == spellRule.getId() ? "" : "," + spellRule.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == spellRule.getId() ? "" : spellRule.getId().toString());
				}
			}
		}
		...
		if(spellRules != null && spellRules.size() > 0 ){
			for(X6BasicSpellRule spellRule : spellRules){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),spellRule, spellRule.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(spellRuleIds != null && spellRuleIds.size() > 0) {
			String hql = "update " + X6BasicSpellRule.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = spellRuleDao.createQuery(hql);
			query.setParameterList("ids", spellRuleIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(X6BasicSpellRule spellRulez : spellRules){
				spellRulez.setValid(false);
				List<X6BasicSpellRule> params = new ArrayList<X6BasicSpellRule>();
				List<X6BasicSpellRulePart> spellRulePartList = spellRulePartDao.findByCriteria(Restrictions.eq("spellRule", spellRulez));
				spellRulez.setSpellRulePartList(spellRulePartList);
				params.add(spellRulez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "X6Basic_1.0_spellRule");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.common.delete",operType="3")
	public void deleteSpellRule(long spellRuleId, int spellRuleVersion){
		this.deleteSpellRule(spellRuleId, spellRuleVersion,null);
	}
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.common.delete",operType="3")
	public void deleteSpellRule(long spellRuleId, int spellRuleVersion,SignatureLog signatureLog){
		X6BasicSpellRule spellRule = getSpellRule(spellRuleId);
		...
		beforeDeleteSpellRule(spellRule);
		spellRuleDao.delete(spellRuleId, spellRuleVersion);
		afterDeleteSpellRule(spellRule);
		spellRuleDao.flush();
		List<X6BasicSpellRule> params = new ArrayList<X6BasicSpellRule>();
		List<X6BasicSpellRulePart> spellRulePartList = spellRulePartDao.findByCriteria(Restrictions.eq("spellRule", spellRule));
		...
		if(signatureLog != null ) {
			...
			businessKey=spellRule.getCode();
			signatureLog.setTableId(spellRule.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.common.delete",operType="3")
	public void deleteSpellRule(String spellRuleIds){
		this.deleteSpellRule(spellRuleIds,null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.common.delete",operType="3")
	public void deleteSpellRule(String spellRuleIds,SignatureLog signatureLog){
		deleteCollection(spellRuleIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteSpellRule(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:X6Basic_1.0#entityCode:X6Basic_1.0_spellRule#modelCode:X6Basic_1.0_spellRule_SpellRule#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="ec.engine.restore",operType="10")
	public void restoreSpellRule(String spellRuleIds){
		restoreCollection(spellRuleIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreSpellRule(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="ec.engine.restore",operType="10")
	public void restoreSpellRule(long spellRuleId){
		
		findBusinessKeyUsed(spellRuleId);	//判断业务主键是否重复
		...
		beforeRestoreSpellRule(spellRule);
		spellRule.setValid(true);
		spellRuleDao.update(spellRule);
		afterRestoreSpellRule(spellRule);
		List<X6BasicSpellRule> params = new ArrayList<X6BasicSpellRule>();
		List<X6BasicSpellRulePart> spellRulePartList = spellRulePartDao.findByCriteria(Restrictions.eq("spellRule", spellRule));
		spellRule.setSpellRulePartList(spellRulePartList);
		params.add(spellRule);
	...
	public void findBusinessKeyUsed(long spellRuleId){
		Property property = modelServiceFoundation.getBussinessProperty("X6Basic_1.0_spellRule_SpellRule");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + X6BasicSpellRule.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ X6BasicSpellRule.TABLE_NAME +" where id = ? )";
			List<Object> list =  spellRuleDao.createNativeQuery(sql,spellRuleId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseSpellRule(List<X6BasicSpellRule>  spellRules){
		for(X6BasicSpellRule spellRule:spellRules)  {
			saveSpellRule(spellRule, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseSpellRule(List<X6BasicSpellRule>  spellRules){
		for(X6BasicSpellRule spellRule:spellRules)  {
			saveSpellRule(spellRule, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="ec.flow.save")
	public void saveSpellRule(X6BasicSpellRule spellRule, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveSpellRule(spellRule, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="ec.flow.save")
	public void saveSpellRule(X6BasicSpellRule spellRule, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveSpellRule(spellRule, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateSpellRuleCodes(X6BasicSpellRule spellRule) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSpellRuleCodes(spellRule, false);
	}
	
	public void generateSpellRuleCodes(X6BasicSpellRule spellRule, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateSpellRuleSummarys(X6BasicSpellRule spellRule) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSpellRuleSummarys(spellRule, false);
	}

	public void generateSpellRuleSummarys(X6BasicSpellRule spellRule, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule")
	public void saveSpellRule(X6BasicSpellRule spellRule,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveSpellRule(spellRule, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule")
	public void saveSpellRule(X6BasicSpellRule spellRule,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_spellRule";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_spellRule");
		String url = null;
		if(spellRule.getId() != null && spellRule.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRule.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/x6Basic_10_spellRule/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRule.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/x6Basic_10_spellRule/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, spellRule);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(spellRule);
		beforeSaveSpellRule(spellRule, viewIsView);

		if (viewIsView) {
			spellRuleDao.saveWithRevertVersion(spellRule);
		} else {
			if(isNew)
				spellRuleDao.save(spellRule);
			else
				spellRuleDao.merge(spellRule);
		}
		if(null != spellRule && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRule.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == spellRule.getCode() ? "" : spellRule.getCode().toString());
			AuditUtil.setColumnStringA(null == spellRule.getName() ? "" : spellRule.getName().toString());
		...
		dealDatagridsSave(spellRule,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateSpellRuleCodes(spellRule, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateSpellRuleSummarys(spellRule, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveSpellRule(spellRule, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, spellRule);
			}
		}
		List<X6BasicSpellRule> params = new ArrayList<X6BasicSpellRule>();
		spellRuleDao.flush();
		spellRuleDao.clear();
		spellRule = spellRuleDao.load(spellRule.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, X6BasicSpellRule spellRule){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("spellRule", spellRule);
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule")
	public void mergeSpellRule(X6BasicSpellRule spellRule, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(spellRule);
		beforeSaveSpellRule(spellRule);
		spellRuleDao.merge(spellRule);
		if(null != spellRule && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRule.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == spellRule.getCode() ? "" : spellRule.getCode().toString());
			AuditUtil.setColumnStringA(null == spellRule.getName() ? "" : spellRule.getName().toString());
		...
		dealDatagridsSave(spellRule,null,dgLists,dgDeleteIDs,assFileUploads);

			// 一对多情况处理
		afterSaveSpellRule(spellRule);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<X6BasicSpellRule> findSpellRules(Page<X6BasicSpellRule> page, Criterion... criterions) {
		return spellRuleDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private X6BasicSpellRuleDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<X6BasicSpellRuleDealInfo> list=spellRuleDao.createQuery("from " + X6BasicSpellRuleDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	public void saveDealInfo(X6BasicSpellRuleDealInfo dealInfo) {
		spellRuleDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+X6BasicSpellRuleDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) spellRuleDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+X6BasicSpellRuleDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = spellRuleDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = spellRuleDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ X6BasicSpellRuleDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = spellRuleDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSpellRules(Page<X6BasicSpellRule> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findSpellRules(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSpellRules(Page<X6BasicSpellRule> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSpellRules(Page<X6BasicSpellRule> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<X6BasicSpellRule> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"spellRule\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					/*if(companyId != null) {
						if(type == Sql.TYPE_LIST_PENDING) {
							...
							s.append(" AND \"spellRule\".CID in (1,").append(companyId+")");
						}
					} else if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag){
							s.append(" AND \"spellRule\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}*/
	...
	//						s.append(" AND \"spellRule\".CID = ").append(getCurrentCompanyId());
	//					} else if(type == Sql.TYPE_LIST_REFERENCE){
	//						if(null != crossCompanyFlag && !crossCompanyFlag){
	//							s.append(" AND \"spellRule\".CID = ").append(getCurrentCompanyId());
	//						}
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "X6Basic_1.0_spellRule_SpellRule" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "X6Basic_1.0_spellRule_SpellRule" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  spellRuleDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  spellRuleDao.createNativeQuery(powerOperateSql, "X6Basic_1.0_spellRule").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"spellRule\"","X6Basic_1.0_spellRule",customCondition,"S2BASE_SPELLRULE",true);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"spellRule\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+X6BasicSpellRuleDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"spellRule\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"spellRule\".LAY_REC ASC, \"spellRule\".SORT ASC");
				} else {
					orderPart.append(" \"spellRule\".SR_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) spellRuleDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=spellRuleDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = spellRuleDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<X6BasicSpellRule> result = new ArrayList<X6BasicSpellRule>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(X6BasicSpellRule.class,spellRuleDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(X6BasicSpellRule.class,spellRuleDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (X6BasicSpellRule spellRule : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(spellRule.getId(), "X6Basic_spellRule_spellRule");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(spellRule.getId(), "X6Basic_spellRule_spellRule");
							if(null != documents && !documents.isEmpty()) {
								spellRule.setDocument(documents.get(0));
								spellRule.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("X6Basic_1.0_spellRule")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("X6Basic_1.0_spellRule")+"X6Basic_1.0_spellRule".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("X6Basic_1.0_spellRule",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<X6BasicSpellRule> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<X6BasicSpellRule> result) {
		View view = null;
		...
		Map<Long, X6BasicSpellRule> rsMap = new HashMap<Long, X6BasicSpellRule>();
		for (X6BasicSpellRule spellRule : result) {
			ids.add(spellRule.getId());
			rsMap.put(spellRule.getId(), spellRule);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(spellRuleDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = spellRuleDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(X6BasicSpellRule spellRule,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(spellRule.getOwnerStaffId());
		ti.setOwnerPositionId(spellRule.getOwnerPositionId());
		ti.setOwnerDepartmentId(spellRule.getOwnerDepartmentId());
		
		ti.setCreateStaff(spellRule.getCreateStaff());
		ti.setCreateStaffId(spellRule.getCreateStaffId());
		ti.setCreateTime(spellRule.getCreateTime());
		ti.setDeleteStaff(spellRule.getDeleteStaff());
		ti.setDeleteStaffId(spellRule.getDeleteStaffId());
		ti.setDeleteTime(spellRule.getDeleteTime());
		ti.setCreateDepartmentId(spellRule.getCreateDepartmentId());
		ti.setModifyStaff(spellRule.getModifyStaff());
		ti.setModifyStaffId(spellRule.getModifyStaffId());
		ti.setCreatePositionId(spellRule.getCreatePositionId());
		ti.setPositionLayRec(spellRule.getPositionLayRec());
		ti.setTableNo(spellRule.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("X6Basic_1.0_spellRule");
		ti.setTargetTableName("S2BASE_SPELLRULE");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="X6Basic_1.0_spellRule_SpellRule" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  spellRuleDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"spellRule\"","X6Basic_1.0_spellRule","","S2BASE_SPELLRULE",true);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"spellRule\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"spellRule\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"spellRule\"","X6Basic_1.0_spellRule","","S2BASE_SPELLRULE",true);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("spellRule.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"spellRule\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"spellRule\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"spellRule\".CID = 1 OR \"spellRule\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("spellRule.mc\"").append(".SPELL_RULE) not in ("+notinValue+")");
			}
			
			SQLQuery query = spellRuleDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("spellRule", "X6Basic_1.0_spellRule", "spellRule_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=spellRuleDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return X6BasicSpellRule.ENTITY_CODE;}
	protected String getModuleCode(){return X6BasicSpellRule.MODULE_CODE;}
	protected String getModelCode(){return X6BasicSpellRule.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param spellRule 模型实例
	 */
	public void checkUniqueConstraint(X6BasicSpellRule spellRule){
		if(null == spellRule.getCode() || StringUtils.isEmpty(spellRule.getCode().toString())){
			return;
		}
		if(spellRule.getId()==null ){
			List<X6BasicSpellRule> spellRules = spellRuleDao.findByCriteria(Restrictions.eq("code", spellRule.getCode()));
			if(spellRules!=null && !spellRules.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1398306715753", getCurrentUser().getLanguage()), spellRule.getCode());
			}
		} else {
			List<X6BasicSpellRule> spellRules = spellRuleDao.findByCriteria(Restrictions.ne("id", spellRule.getId()), Restrictions.eq("code", spellRule.getCode()));
			if(spellRules!=null && !spellRules.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"X6Basic.propertydisplayName.radion1398306715753", getCurrentUser().getLanguage()), spellRule.getCode());
			}
	...
	@Override
	public List<X6BasicSpellRule> getSpellRules(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSpellRule.class);
		if(params!=null){
			...
			return spellRuleDao.findByHql("from " + X6BasicSpellRule.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return spellRuleDao.findByCriteria(detachedCriteria);
	}
	...
	public List<X6BasicSpellRule> findSpellRulesByHql(String hql, Object... objects){
		...
		return   spellRuleDao.findByHql(hql, objects);
	}
	
	@Override
	public X6BasicSpellRule loadSpellRuleByBussinessKey(X6BasicSpellRule spellRule){
		return loadSpellRuleByBussinessKey(spellRule.getCode());
	}
	
	@Override
	public Page<X6BasicSpellRule> getSpellRules(Page<X6BasicSpellRule> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(X6BasicSpellRule.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = spellRuleDao.findByPage(page, sql);
			}else{
				page = spellRuleDao.findAllByPage(page);
			}
			return page;
		}
		page = spellRuleDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public X6BasicSpellRule loadSpellRuleByBussinessKey(Serializable bussinessKey){
		return spellRuleDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<X6BasicSpellRule> findByProperty(String propertyName, Object object){
		return spellRuleDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public X6BasicSpellRule findEntityByProperty(String propertyName, Object object){
		return spellRuleDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + X6BasicSpellRule.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = spellRuleDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("X6Basic_1.0_spellRule_SpellRule","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("X6Basic_1.0_spellRule", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(X6BasicSpellRule spellRule,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,spellRuleDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), X6BasicSpellRule.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, spellRule);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",spellRule.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicSpellRule.TABLE_NAME +" where valid = 1";
			list = spellRuleDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ X6BasicSpellRule.TABLE_NAME;
			list = spellRuleDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ X6BasicSpellRule.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = spellRuleDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private X6BasicSpellRuleDaoImpl spellRuleDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchSpellRule(final List<X6BasicSpellRule> insertObjs, final List<X6BasicSpellRule> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchSpellRule(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchSpellRule(final List<X6BasicSpellRule> insertObjs, final List<X6BasicSpellRule> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("X6Basic.modelname.radion1398306647942"),importCount));
		...
		Map<Long, X6BasicSpellRule> oldUpdateObjsMap = new HashMap<Long, X6BasicSpellRule>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			X6BasicSpellRule bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<X6BasicSpellRule> bizObjList = spellRuleDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					X6BasicSpellRule bizObj = bizObjList.get(mIndex);
		...
		final List<X6BasicSpellRule> nmeObjs = new ArrayList<X6BasicSpellRule>();
		...
		long id=autoGeneratorID.getNextId(X6BasicSpellRule.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(X6BasicSpellRule spellRule:insertObjs)  {
			//id = autoGeneratorID.getNextId(X6BasicSpellRule.TABLE_NAME,1,dbId);
			if(spellRule.getCode() != null && !spellRule.getCode().equals("") ){
				m.put(spellRule.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) spellRuleDao.createNativeQuery(sql, "X6Basic_1.0_spellRule_SpellRule"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		spellRuleDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + X6BasicSpellRule.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						X6BasicSpellRule spellRule = updateObjs.get(updateIndex);
						Long id = spellRule.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = spellRule.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + X6BasicSpellRule.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + X6BasicSpellRule.TABLE_NAME + " order by id desc";
//					Query query = spellRuleDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						X6BasicSpellRule spellRule = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = spellRule.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		spellRuleDaoImpl.getSession().flush();
		spellRuleDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			X6BasicSpellRule bizObj = insertObjs.get(insertIndex);
			spellRuleImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			X6BasicSpellRule bizObj = updateObjs.get(updateIndex);
			X6BasicSpellRule oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			spellRuleImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = spellRuleDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				spellRuleDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = spellRuleDao.createNativeQuery(sql, "X6Basic_1.0_spellRule_SpellRule").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = spellRuleDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = spellRuleDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = spellRuleDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?spellRuleDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  spellRuleDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,spellRuleDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1398316931895Page(Page<X6BasicSpellRulePart> dg1398316931895Page,X6BasicSpellRule spellRule, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(X6BasicSpellRulePart.class);
		criteria.add(Restrictions.eq("spellRule", spellRule));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"spellRulePart\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		spellRulePartDao.findByPage(dg1398316931895Page, criteria);
		spellRulePartService.findByNormalDataGridSpellRule(dg1398316931895Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1398317025305Page(Page<X6BasicSpellRulePart> dg1398317025305Page,X6BasicSpellRule spellRule, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(X6BasicSpellRulePart.class);
		criteria.add(Restrictions.eq("spellRule", spellRule));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		spellRulePartDao.findByPage(dg1398317025305Page, criteria);
		spellRulePartService.findByNormalDataGridSpellRule(dg1398317025305Page);
	}
	
	 
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="ec.flow.save")
	public void saveSpellRule(X6BasicSpellRule spellRule, DataGridService dataGridService){
		saveSpellRule(spellRule, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="ec.flow.save")
	public void saveSpellRule(X6BasicSpellRule spellRule, DataGridService dataGridService, String viewCode){
		saveSpellRule(spellRule, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule")
	public void saveSpellRule(X6BasicSpellRule spellRule, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveSpellRule(spellRule, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule")
	public void saveSpellRule(X6BasicSpellRule spellRule, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "X6Basic_1.0_spellRule";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "X6Basic_1.0_spellRule");
		String url = null;
		if(null != spellRule.getId() && spellRule.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || X6BasicSpellRule.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule")
	public Map<Object, Object> spellRuleDataGridImport(X6BasicSpellRule spellRule, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = spellRule.getVirtualId();
		
		spellRule = spellRuleDao.load(spellRule.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, spellRule.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = spellRule.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(spellRule);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, spellRule.getId());
		}
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule")
	public void mergeSpellRule(X6BasicSpellRule spellRule, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(spellRule);
		beforeSaveSpellRule(spellRule);
		spellRuleDao.merge(spellRule);
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_spellRule",model="X6Basic_1.0_spellRule_SpellRule",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<X6BasicSpellRule> getByPage(Page<X6BasicSpellRule> page,DetachedCriteria detachedCriteria){
		return spellRuleDao.findByPage(page, detachedCriteria);
	}
	@Override
	public List<X6BasicSpellRulePart> getSpellRulePartList(X6BasicSpellRule spellRule){
		return spellRulePartDao.findByCriteria(Restrictions.eq("spellRule", spellRule),Restrictions.eq("valid", true));
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("X6Basic_1.0_spellRule").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveSpellRule(X6BasicSpellRule spellRule, Object... objects){
		checkUniqueConstraint(spellRule);
	/* CUSTOM CODE START(serviceimpl,beforeSave,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveSpellRule(X6BasicSpellRule spellRule, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteSpellRule(X6BasicSpellRule spellRule){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteSpellRule(X6BasicSpellRule spellRule){
	/* CUSTOM CODE START(serviceimpl,afterDelete,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreSpellRule(X6BasicSpellRule spellRule){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreSpellRule(X6BasicSpellRule spellRule){
	/* CUSTOM CODE START(serviceimpl,afterRestore,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(X6BasicSpellRule spellRule, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(X6BasicSpellRule spellRule, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportSpellRule(List<X6BasicSpellRule> insertObjs, List<X6BasicSpellRule> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportSpellRule,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportSpellRule(List<X6BasicSpellRule> insertObjs, List<X6BasicSpellRule> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportSpellRule,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,X6Basic_1.0_spellRule_SpellRule,X6Basic_1.0) */
// 自定义代码
