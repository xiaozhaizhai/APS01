...
/* CUSTOM CODE START(action,import,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class X6BasicRangePlanAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<X6BasicRangePlan> {
	...
	//导出excel时,主模型内容
	List<X6BasicRangePlan> queryResults = new ArrayList<X6BasicRangePlan>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/rangePlan/rangePlan/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/X6Basic/rangePlan/rangePlan/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/X6Basic_1.0_rangePlan_RangePlan" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<X6BasicRangePlan> rangePlans;
	
	public List<X6BasicRangePlan> getRangePlans() {
		return rangePlans;
	}
	...
	private Page<X6BasicRangePlan> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			rangePlan = new X6BasicRangePlan();
		} else {
			if(id > 0){
				rangePlan = rangePlanService.getRangePlan(id, viewCode);
			}
		}
	...
	public Page<X6BasicRangePlan> getPage() {
		if (null == page) {
			page = new Page<X6BasicRangePlan>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/rangePlan/rangePlan/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = rangePlanService.getRangePlanAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/X6Basic/rangePlan/rangePlan/edit-extra-col")
	...
	/**
	 * X6Basic.modelname.radion1398308731380
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String rangePlanReferenceCopy() throws Exception{
			X6BasicRangePlan oldRangePlan = null;
			if(isEditNew()){
				...
				oldRangePlan = new X6BasicRangePlan();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,rangePlan.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, rangePlan.getId());
				...
				oldRangePlan = rangePlanService.getRangePlan(id);
			}
			
			
			rangePlan = rangePlanService.getRangePlan(refId);
			Model model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_RangePlan");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = X6BasicRangePlan.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldRangePlan, null);
						Method setValue = X6BasicRangePlan.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(rangePlan, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			rangePlan.setCreateDepartmentId(oldRangePlan.getCreateDepartmentId());
			rangePlan.setCreatePositionId(oldRangePlan.getCreatePositionId());
			rangePlan.setCreateStaffId(oldRangePlan.getCreateStaffId());
			rangePlan.setOwnerDepartmentId(oldRangePlan.getOwnerDepartmentId());
			rangePlan.setOwnerPositionId(oldRangePlan.getOwnerPositionId());
			rangePlan.setOwnerStaffId(oldRangePlan.getOwnerStaffId());
			rangePlan.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/X6Basic/rangePlan/rangePlan/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/X6Basic/rangePlan/rangePlan/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=rangePlanService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return rangePlanReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			rangePlan = rangePlanService.getRangePlan(this.rangePlan.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,rangePlan.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = rangePlanService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(rangePlanService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(rangePlanService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("X6Basic_1.0_rangePlan_RangePlan", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"rangePlan\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"rangePlan\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"rangePlan\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"rangePlan\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		rangePlanService.findDataGridPage(dg,dgclass,getDgPage(),rangePlan,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(rangePlan.getId() != null && rangePlan.getId() != -1){
				rangePlan = rangePlanService.getRangePlan(rangePlan.getId());
			}
			rangePlanService.findDataGridPage(dg,dgclass,getDgPage(),rangePlan,searchCondition,params);
	...
	private void save() throws Exception {
		...
		rangePlanService.saveRangePlan(rangePlan,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/X6Basic/rangePlan/rangePlan/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "submit");
		bap_validate_datagrids=rangePlanService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", rangePlan.getId());
		...
		fileuploads.put("linkId", rangePlan.getId());
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "submit");
		return SUCCESS;
	}

	@Action(value = "/X6Basic/rangePlan/rangePlan/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		rangePlanService.deleteRangePlan(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlan/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "restore");
		rangePlanService.restoreRangePlan(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/X6Basic/rangePlan/rangePlan/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/X6Basic/rangePlan/rangePlan/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "showDealInfoData");
		if (null != tableInfoId) {
			int count=rangePlanService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = rangePlanService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = rangePlanService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<X6BasicRangePlan> findExportDataInfos(Page<X6BasicRangePlan> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = rangePlanService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = rangePlanService.getAssProperty(p.getCode());	
									//String assModelCode = rangePlanService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			rangePlanService.findRangePlans(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			rangePlanService.findRangePlans(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<X6BasicRangePlan> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(X6BasicRangePlan result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".RP_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.rangePlan || null == this.rangePlan.getId();
		return null == this.id;
	}
	...
	@Action(value = "/X6Basic/rangePlan/rangePlan/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "attachmentInit");
		...
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/X6Basic/rangePlan/rangePlan/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/X6Basic/rangePlan/rangePlan/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlan/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicRangePlanService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("X6Basic_1.0_rangePlan", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("X6Basic_1.0_rangePlan");
		for(Model m : models){
		...
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "imports");
		...
		bapAfterCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlan/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.X6Basic.services.X6BasicRangePlanService"
						...
						, "com.supcon.orchid.X6Basic.services.X6BasicSpellRuleService"       
						, "com.supcon.orchid.X6Basic.services.X6BasicPlanDailyService"
						, "com.supcon.orchid.X6Basic.services.X6BasicRangePlanPartService"
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "X6Basic_1.0_rangePlan_RangePlan", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "X6Basic_1.0_rangePlan_RangePlan", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/X6Basic/rangePlan/rangePlan/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/X6Basic/rangePlan/rangePlan/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="X6Basic_1.0_rangePlan_RangePlan";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("X6Basic_1.0_rangePlan_RangePlan", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/X6Basic/rangePlan/rangePlan/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
		_auditLog.setColumnStringA(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
		_auditLog.setColumnStringC(rangePlan.getId().toString());
		_auditLog.setColumnStringD(X6BasicRangePlan.MODEL_CODE);
		...
		_auditLog.setEntity("X6Basic_1.0_rangePlan");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.X6Basic.actions.X6BasicRangePlanAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, rangePlan,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, rangePlan,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,rangePlan);
		...
		printlog.setEntity("X6Basic_1.0_rangePlan");
		...
		printlog.setBusinessId(rangePlan.getId());
		printlog.setBussinessKey(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
		printlog.setMainDisplay(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlan/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(rangePlan,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/X6Basic/rangePlan/rangePlan/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(rangePlanService.getRangePlan(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/X6Basic/rangePlan/rangePlan/loadBatchXmlRefer",
	...
	@Action(value = "/X6Basic/rangePlan/rangePlan/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, rangePlan,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				rangePlan = rangePlanService.getRangePlan(Long.parseLong(arrId[i]));
				if(null != rangePlan){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == rangePlan.getId() ? "" : "," + rangePlan.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == rangePlan.getId() ? "" : "," + rangePlan.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == rangePlan.getId() ? "" : "," + rangePlan.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == rangePlan.getId() ? "" : rangePlan.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, rangePlan,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,rangePlan);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/X6Basic/rangePlan/rangePlan/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<X6BasicRangePlan> findExportPageInfos(Page<X6BasicRangePlan> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "X6Basic_1.0_rangePlan" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = rangePlanService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(X6BasicRangePlan queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = rangePlanService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = rangePlanService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_RangePlan");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("X6Basic_1.0_rangePlan_RangePlan")!=null  ){
			model = (Model) excelExportModels.get("X6Basic_1.0_rangePlan_RangePlan");
		}else{
			model = modelServiceFoundation.getModel("X6Basic_1.0_rangePlan_RangePlan");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = rangePlanService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,X6Basic_1.0_rangePlan_RangePlan,X6Basic_1.0) */
// 自定义代码
