...
@Service("x6BasicMailServerImportService")
public class X6BasicMailServerImportServiceImpl implements X6BasicMailServerImportService {
	...
	@Override
	@AuditLog(entity="X6Basic_1.0_mailServer",model="X6Basic_1.0_mailServer_MailServer",desc="保存业务信息",operType="1")
	public void saveImportDataLog(X6BasicMailServer bizObj, X6BasicMailServer oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("X6Basic.entityname.radion1404797069424") + "信息");
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("X6Basic.entityname.radion1404797069424") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
	...
	private void setBussinessKeyAndMainDispalyField(X6BasicMailServer bizObj, String bussinesskeyFieldName,
