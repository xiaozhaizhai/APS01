...
import com.supcon.orchid.X6Basic.services.X6BasicDocumentClassesWSService;
import com.supcon.orchid.X6Basic.services.X6BasicDocumentClassesService;
/* CUSTOM CODE START(wsserviceimpl,import,X6Basic_1.0_documentClass_DocumentClasses,X6Basic_1.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="X6BasicDocumentClasses",targetNamespace = "http://ws.supcon.com")
public class X6BasicDocumentClassesWSServiceImpl implements X6BasicDocumentClassesWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getDocumentClassesByBusinessKey(
		...
		try{
			...
			X6BasicDocumentClasses documentClasses = documentClassesService.loadDocumentClassesByBussinessKey(businessKey);
			if(documentClasses == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, documentClasses);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateDocumentClasses(
		...
		try{
			if(documentClasses == null){
				return set500Error(result, "文档分类不能为空。");
			}
			...
			if(!beforeUpdateDocumentClasses(documentClasses)){
				return set404Error(result);
			}
			documentClassesService.saveDocumentClasses(documentClasses, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addDocumentClasses(
		...
		try{
			...
			if(documentClasses.getCode() == null){
				return set500Error(result, "业务主键不能为空。");
			}
			...
			try{
				checkDocumentClassesData(documentClasses);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			documentClassesService.saveDocumentClasses(documentClasses, null);
			setSuccessResult(result, documentClasses.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteDocumentClassesByBusinessKey(
		...
		try{
			...
			try{
				documentClasses = documentClassesService.loadDocumentClassesByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddDocumentClasses(
		...
		try{
			...
			int count = 1;
			for(X6BasicDocumentClasses documentClasses : documentClassesList){
				if(documentClasses.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(documentClasses.getCode() == null || documentClasses.getCode().toString().trim().isEmpty()){
			...
			documentClassesService.batchImportBaseDocumentClasses(documentClassesList);
			for(X6BasicDocumentClasses documentClasses : documentClassesList){
				idList.add(documentClasses.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteDocumentClassesByPk(
		...
		try{
			...
			for(String k : pk){
				...
				X6BasicDocumentClasses temp = null;
				try{
					temp = documentClassesService.getDocumentClasses(id);
				}catch(NullPointerException e){}
			...
			documentClassesService.deleteDocumentClasses(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteDocumentClassesByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				X6BasicDocumentClasses temp = null;
				try{
					temp = documentClassesService.loadDocumentClassesByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getDocumentClassesPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(X6BasicDocumentClasses.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),X6BasicDocumentClasses.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,X6BasicDocumentClasses.class);
				} catch (Exception e) {
			...
			Page<X6BasicDocumentClasses> page = new Page<X6BasicDocumentClasses>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<X6BasicDocumentClasses> datas = documentClassesService.getByPage(page, detachedCriteria);
			List<X6BasicDocumentClasses> documentClassesList = datas.getResult();
			if (documentClassesList == null || documentClassesList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(documentClassesList.toArray());
			PaginationDTO<X6BasicDocumentClasses> pagination = new PaginationDTO<X6BasicDocumentClasses>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			X6BasicDocumentClasses documentClasses = documentClassesService.getDocumentClasses(id);
			if (documentClasses == null || !documentClasses.isValid()) {
				result.setErrorMessage("找不到文档分类");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取文档分类对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param documentClasses
	 * @return
	 */
	private X6BasicDocumentClasses getDocumentClasses(X6BasicDocumentClasses documentClasses) {
		if (documentClasses.getId() == null && (documentClasses.getCode() == null || documentClasses.getCode().toString().trim() == ""))
			throw new RuntimeException("文档分类对象业务主键和id不能同时为空");
		if (documentClasses.getId() != null) {
			X6BasicDocumentClasses result = documentClassesService.getDocumentClasses(documentClasses .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return documentClassesService.loadDocumentClassesByBussinessKey(documentClasses.getCode());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个文档分类，则返回false，否则返回true
	 * @param documentClasses
	 */
	private boolean beforeUpdateDocumentClasses(X6BasicDocumentClasses documentClasses) {
		X6BasicDocumentClasses temp = getDocumentClasses(documentClasses);
		if (temp == null) {
			return false;
		}
		if(documentClasses.getId() == null){
			documentClasses.setId(temp.getId());		
		}
		if(documentClasses.getCode() == null){
			documentClasses.setCode(temp.getCode());
		}
		
		documentClasses.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除文档分类对象下所有辅模型对象中的文档分类对象
	 * @param documentClasses
	 */
	private void clearCircle(X6BasicDocumentClasses documentClasses) {
	 ...
	 */
	private void checkDocumentClassesData(X6BasicDocumentClasses documentClasses){
	}
	/* CUSTOM CODE START(wsserviceimpl,functions,X6Basic_1.0_documentClass_DocumentClasses,X6Basic_1.0) */
// 自定义代码
