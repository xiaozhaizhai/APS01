...
<script type="text/javascript">
		...
		CUI('input,select', CUI('#ec_X6Basic_meetingroom_meetingRoom_refer_queryForm')).each(function(index){
			if(CUI(this).attr('name') && CUI(this).attr('typeUse') != 'dataClassific'){
				...
				if(fastCol.indexOf(".") > -1 && !fastCol.endsWith(".id") && !fastCol.endsWith("_bapLower")) {
					var keyIdValue = CUI('#ec_X6Basic_meetingroom_meetingRoom_refer_queryForm *[name="'+fastCol.substr(0,fastCol.lastIndexOf('.'))+'.id"]').val();
					if(keyIdValue != null && keyIdValue != "undefined" && keyIdValue != "") {
						return;
					}
				}
				if(CUI(this).attr('type') == 'checkbox' && fastCol.endsWith("_bapLower")) {
					var compareKeyValue = CUI('#ec_X6Basic_meetingroom_meetingRoom_refer_queryForm *[name="'+fastCol.substr(0,fastCol.lastIndexOf('_bapLower'))+'"]').val();
					if(CUI(this).attr('columnType') == 'DATE' || CUI(this).attr('columnType') == 'DATETIME' || CUI(this).attr('columnType') == 'MONEY' || CUI(this).attr('columnType') == 'INTEGER' || CUI(this).attr('columnType') == 'DECIMAL' || CUI(this).attr('columnType') == 'LONG') {
						compareKeyValue = CUI('#ec_X6Basic_meetingroom_meetingRoom_refer_queryForm *[name="'+fastCol.substr(0,fastCol.lastIndexOf('_bapLower'))+'_start"]').val();
						if(compareKeyValue != null && compareKeyValue != "undefined" && compareKeyValue != "") {
							compareKeyValue = CUI('#ec_X6Basic_meetingroom_meetingRoom_refer_queryForm *[name="'+fastCol.substr(0,fastCol.lastIndexOf('_bapLower'))+'_end"]').val();
						}
					}
					if(compareKeyValue != null && compareKeyValue != "undefined" && compareKeyValue != "") {
						dataPost += "&" + fastCol + "=" + encodeURIComponent(CUI('#ec_X6Basic_meetingroom_meetingRoom_refer_queryForm *[name="'+fastCol+'"]').prop('checked'));
					}
				} else {
	...
	foundation.common.${requestUri!}__callbackFunction = function(){
		if(datatable_ec_X6Basic_meetingroom_meetingRoom_refer_query.selectedRows.length == 0){
			CUI.Dialog.alert("${getText('foundation.common.selectNullData')}");
			return false;
		}
		X6Basic.meetingroom.meetingRoom.refer.sendBackrefer(null,datatable_ec_X6Basic_meetingroom_meetingRoom_refer_query.selectedRows[0]);
	};
	/**
	 * 双击事件
	...
	X6Basic.meetingroom.meetingRoom.refer.sendBackrefer = function(event, oRow) {
		...
		var oRows = new Array();
		if(event == undefined){
			oRows = datatable_ec_X6Basic_meetingroom_meetingRoom_refer_query.selectedRows;
		}else{
			oRows.push(oRow);
		}
		if(oRows.length == 0){
...
X6Basic_meetingroom_meetingRoom_refer_getList = function() {
	 ...
	 * @private
	 */
	X6Basic.meetingroom.meetingRoom.refer.showDialog = function(url,formId,title,width,height) {
		var buttons = [];
	...
	/**
	 * 显示批量处理对话框
	 * @method X6Basic.meetingroom.meetingRoom.refer.approvalDialog
	 * @param {String} url
	 * @private
	 */
	X6Basic.meetingroom.meetingRoom.refer.approvalDialog = function(url,formId,title,width,height) {
		X6Basic.meetingroom.meetingRoom.refer.AppDlg =	new CUI.Dialog({
			title: title,
			url :url,
			formId: formId,
			modal:true,
		...
		X6Basic.meetingroom.meetingRoom.refer.AppDlg.show();
	};
...
<@exportexcel action="/X6Basic/meetingroom/meetingRoom/refer-query.action" settingtext="" settingUrl="" prepareParams="" formId="" look="text" width=250 height=170 text="" permissionCode="${permissionCode?default('X6Basic_1.0_meetingroom_refer')}" prefix="ec_X6Basic_meetingroom_meetingRoom_refer_query"/>
