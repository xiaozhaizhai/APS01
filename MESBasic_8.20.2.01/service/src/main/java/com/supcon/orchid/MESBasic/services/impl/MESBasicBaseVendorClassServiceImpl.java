...
import com.supcon.orchid.MESBasic.entities.MESBasicBaseVendorClassMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicBaseVendorClassService;
import com.supcon.orchid.MESBasic.services.MESBasicBaseVendorClassImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicBaseVendorClassDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_baseVendorClassService")
@Transactional
public class MESBasicBaseVendorClassServiceImpl  extends BaseServiceImpl<MESBasicBaseVendorClass> implements MESBasicBaseVendorClassService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicBaseVendorClass> bapBaseService;
	...
	@Autowired
	private MESBasicBaseVendorClassImportService baseVendorClassImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicBaseVendorClass.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  baseVendorClassDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicBaseVendorClass.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = baseVendorClassDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBaseVendorClass.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBaseVendorClass.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  baseVendorClassDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicBaseVendorClass.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicBaseVendorClass.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicBaseVendorClass> list = (List<MESBasicBaseVendorClass>) baseVendorClassDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicBaseVendorClass getBaseVendorClass(long id){
		return getBaseVendorClass(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseVendorClass getBaseVendorClass(long id, String viewCode){
		MESBasicBaseVendorClass baseVendorClass = baseVendorClassDao.load(id);

		// 一对多情况处理
		if(baseVendorClass != null){
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBaseVendorClassAsJSON(long id, String include){
		MESBasicBaseVendorClass baseVendorClass = baseVendorClassDao.load(id);
		try {
			baseVendorClass.setExtraColMap(XmlUtils.xml2Map(baseVendorClass.getExtraCol()));
		} catch (Exception e) {
		...
		return JSONPlainSerializer.serializeAsJSON(baseVendorClass, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendorClass(MESBasicBaseVendorClass baseVendorClass){
		if(null != baseVendorClass && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendorClass.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseVendorClass.getCode() ? "" : baseVendorClass.getCode().toString());
			AuditUtil.setColumnStringA(null == baseVendorClass.getName() ? "" : baseVendorClass.getName().toString());
			AuditUtil.setColumnStringC(baseVendorClass.getId().toString());
		}
		checkDeletePermit(baseVendorClass.getId());
		beforeDeleteBaseVendorClass(baseVendorClass);
		baseVendorClassDao.delete(baseVendorClass);
		afterDeleteBaseVendorClass(baseVendorClass);
		baseVendorClassDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseVendorClass/del", baseVendorClass,MESBasicBaseVendorClassMneCode.class);
		List<MESBasicBaseVendorClass> params = new ArrayList<MESBasicBaseVendorClass>();
		params.add(baseVendorClass);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseVenderClass");
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendorClass(List<Long> baseVendorClassIds){
		deleteBaseVendorClass(baseVendorClassIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendorClass(List<Long> baseVendorClassIds, String eventTopic) {
		List<MESBasicBaseVendorClass> baseVendorClasss = new ArrayList<MESBasicBaseVendorClass>();
		for(Long baseVendorClassId : baseVendorClassIds){
			MESBasicBaseVendorClass baseVendorClass = getBaseVendorClass(baseVendorClassId);
			baseVendorClasss.add(baseVendorClass);
			if(null != baseVendorClass && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendorClass.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == baseVendorClass.getName() ? "" : "," + baseVendorClass.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == baseVendorClass.getName() ? "" : baseVendorClass.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == baseVendorClass.getCode() ? "" : "," + baseVendorClass.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == baseVendorClass.getCode() ? "" : baseVendorClass.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == baseVendorClass.getId() ? "" : "," + baseVendorClass.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == baseVendorClass.getId() ? "" : baseVendorClass.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(baseVendorClasss != null && baseVendorClasss.size() > 0 ){
			for(MESBasicBaseVendorClass baseVendorClass : baseVendorClasss){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),baseVendorClass, baseVendorClass.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(baseVendorClassIds != null && baseVendorClassIds.size() > 0) {
			String hql = "update " + MESBasicBaseVendorClass.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = baseVendorClassDao.createQuery(hql);
			query.setParameterList("ids", baseVendorClassIds);
			query.executeUpdate();
			baseVendorClassDao.clearAllCacheByEntity();
		}
		...
		if(eventTopic==null){
			for(MESBasicBaseVendorClass baseVendorClassz : baseVendorClasss){
				baseVendorClassz.setValid(false);
				List<MESBasicBaseVendorClass> params = new ArrayList<MESBasicBaseVendorClass>();
				params.add(baseVendorClassz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_baseVenderClass");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendorClass(long baseVendorClassId, int baseVendorClassVersion){
		this.deleteBaseVendorClass(baseVendorClassId, baseVendorClassVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendorClass(long baseVendorClassId, int baseVendorClassVersion,SignatureLog signatureLog){
		MESBasicBaseVendorClass baseVendorClass = getBaseVendorClass(baseVendorClassId);
		if(baseVendorClass != null && !baseVendorClass.isValid()){
		...
		checkDeletePermit(baseVendorClassId);
		beforeDeleteBaseVendorClass(baseVendorClass);
		baseVendorClassDao.delete(baseVendorClassId, baseVendorClassVersion);
		afterDeleteBaseVendorClass(baseVendorClass);
		baseVendorClassDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseVendorClass/del", baseVendorClass,MESBasicBaseVendorClassMneCode.class);
		List<MESBasicBaseVendorClass> params = new ArrayList<MESBasicBaseVendorClass>();
		...
		if(signatureLog != null ) {
			...
			businessKey=baseVendorClass.getCode();
			signatureLog.setTableId(baseVendorClass.getId());
			if(businessKey != null) {
	...
	public void checkDeletePermit(Long id) throws BAPException{
		String baseVendorClassHql = "select count(baseVendorClass.id) as totalCoual from " + MESBasicBaseVendorClass.JPA_NAME + " as baseVendorClass where baseVendorClass.parentId=? and baseVendorClass.valid=true";
		List<Long> baseVendorClassCount = baseVendorClassDao.findByHql(baseVendorClassHql, id);
		if (baseVendorClassCount.get(0) > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendorClass(String baseVendorClassIds){
		this.deleteBaseVendorClass(baseVendorClassIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendorClass(String baseVendorClassIds,SignatureLog signatureLog){
		deleteCollection(baseVendorClassIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBaseVendorClass(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_baseVenderClass#modelCode:MESBasic_1_baseVenderClass_BaseVendorClass#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="ec.engine.restore",operType="10")
	public void restoreBaseVendorClass(String baseVendorClassIds){
		restoreCollection(baseVendorClassIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBaseVendorClass(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="ec.engine.restore",operType="10")
	public void restoreBaseVendorClass(long baseVendorClassId){
		
		findBusinessKeyUsed(baseVendorClassId);	//判断业务主键是否重复
		...
		checkDeletePermit(baseVendorClassId);
		beforeRestoreBaseVendorClass(baseVendorClass);
		baseVendorClass.setValid(true);
		baseVendorClassDao.update(baseVendorClass);
		afterRestoreBaseVendorClass(baseVendorClass);
		baseVendorClassDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseVendorClass/add", baseVendorClass,MESBasicBaseVendorClassMneCode.class);
		List<MESBasicBaseVendorClass> params = new ArrayList<MESBasicBaseVendorClass>();
		params.add(baseVendorClass);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long baseVendorClassId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_baseVenderClass_BaseVendorClass");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicBaseVendorClass.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicBaseVendorClass.TABLE_NAME +" where id = ? )";
			List<Object> list =  baseVendorClassDao.createNativeQuery(sql,baseVendorClassId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBaseVendorClass(List<MESBasicBaseVendorClass>  baseVendorClasss){
		for(MESBasicBaseVendorClass baseVendorClass:baseVendorClasss)  {
			saveBaseVendorClass(baseVendorClass, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBaseVendorClass(List<MESBasicBaseVendorClass>  baseVendorClasss){
		for(MESBasicBaseVendorClass baseVendorClass:baseVendorClasss)  {
			saveBaseVendorClass(baseVendorClass, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="ec.flow.save")
	public void saveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBaseVendorClass(baseVendorClass, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="ec.flow.save")
	public void saveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBaseVendorClass(baseVendorClass, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBaseVendorClassCodes(MESBasicBaseVendorClass baseVendorClass) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBaseVendorClassCodes(baseVendorClass, false);
	}
	
	public void generateBaseVendorClassCodes(MESBasicBaseVendorClass baseVendorClass, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBaseVendorClassSummarys(MESBasicBaseVendorClass baseVendorClass) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBaseVendorClassSummarys(baseVendorClass, false);
	}

	public void generateBaseVendorClassSummarys(MESBasicBaseVendorClass baseVendorClass, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass")
	public void saveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBaseVendorClass(baseVendorClass, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass")
	public void saveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_baseVenderClass";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseVenderClass");
		String url = null;
		if(baseVendorClass.getId() != null && baseVendorClass.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendorClass.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_baseVenderClass/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendorClass.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_baseVenderClass/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, baseVendorClass);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(baseVendorClass);
		beforeSaveBaseVendorClass(baseVendorClass, viewIsView);

		if (viewIsView) {
			baseVendorClassDao.saveWithRevertVersion(baseVendorClass);
		} else {
			baseVendorClassDao.save(baseVendorClass);
		}
		if(null != baseVendorClass && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendorClass.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseVendorClass.getCode() ? "" : baseVendorClass.getCode().toString());
			AuditUtil.setColumnStringA(null == baseVendorClass.getName() ? "" : baseVendorClass.getName().toString());
		...
		dealDatagridsSave(baseVendorClass,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBaseVendorClassCodes(baseVendorClass, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBaseVendorClassSummarys(baseVendorClass, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBaseVendorClass(baseVendorClass, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, baseVendorClass);
			}
		}
		List<MESBasicBaseVendorClass> params = new ArrayList<MESBasicBaseVendorClass>();
		baseVendorClassDao.flush();
		baseVendorClassDao.clear();
		baseVendorClass = baseVendorClassDao.load(baseVendorClass.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicBaseVendorClass baseVendorClass){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("baseVendorClass", baseVendorClass);
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass")
	public void mergeBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(baseVendorClass);
		beforeSaveBaseVendorClass(baseVendorClass);
		baseVendorClassDao.merge(baseVendorClass);
		if(null != baseVendorClass && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendorClass.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseVendorClass.getCode() ? "" : baseVendorClass.getCode().toString());
			AuditUtil.setColumnStringA(null == baseVendorClass.getName() ? "" : baseVendorClass.getName().toString());
			AuditUtil.setColumnStringC(baseVendorClass.getId().toString());
		}
		dealDatagridsSave(baseVendorClass,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBaseVendorClass(baseVendorClass);
		baseVendorClassDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseVendorClass/add", baseVendorClass,MESBasicBaseVendorClassMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicBaseVendorClass> findBaseVendorClasss(Page<MESBasicBaseVendorClass> page, Criterion... criterions) {
		return baseVendorClassDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicBaseVendorClassDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicBaseVendorClassDealInfo> list=baseVendorClassDao.createQuery("from " + MESBasicBaseVendorClassDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicBaseVendorClassDealInfo dealInfo) {
		baseVendorClassDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicBaseVendorClassDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) baseVendorClassDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicBaseVendorClassDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = baseVendorClassDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = baseVendorClassDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicBaseVendorClassDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = baseVendorClassDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	public List<String> getContainLower(String modelName, String fieldName, String exp, Object value, Boolean crossCompany) {
		...
		if(crossCompany) {
			layRecHql += " and cid = ?";
			return baseVendorClassDao.createQuery(layRecHql, value, getCurrentCompanyId()).list();
		}
		return baseVendorClassDao.createQuery(layRecHql, value).list();
	}
	
	private void dealListTreePageData(Page<MESBasicBaseVendorClass> baseVendorClasss, String viewCode){
		List<MESBasicBaseVendorClass> result = baseVendorClasss.getResult();
		if(null != result && !result.isEmpty()) {
			int rootPos = 0;
			MESBasicBaseVendorClass rootTree = result.get(0);
			...
			baseVendorClasss.setTreeToSurfaceMode(true);
			MESBasicBaseVendorClass baseVendorClassTree = baseVendorClassDao.buildTree("baseVendorClass" + viewCode, new ListPrepareData(baseVendorClasss), false);
			if(null != baseVendorClassTree.getChildren() && !baseVendorClassTree.getChildren().isEmpty()) {
				List<MESBasicBaseVendorClass> treeList = new ArrayList<MESBasicBaseVendorClass>();
				Tree.treeToList(baseVendorClassTree, treeList, false);
				if(treeList.size() == result.size()) {
					baseVendorClasss.setResult(treeList);
					baseVendorClasss.setTreeToSurfaceMode(false);
				} 
			} 
		}
	}
	 ...
	 * 
	 */
	private class ListPrepareData implements PrepareData<MESBasicBaseVendorClass,Long> {
		private Page<MESBasicBaseVendorClass> baseVendorClasss;

		public ListPrepareData(Page<MESBasicBaseVendorClass> baseVendorClasss) {
			this.baseVendorClasss = baseVendorClasss;
		}

		@Override
			public MESBasicBaseVendorClass root() {
				MESBasicBaseVendorClass baseVendorClass = new MESBasicBaseVendorClass();
				baseVendorClass.setId(-1l);
				return baseVendorClass;
			}
			
			@Override
			public List<MESBasicBaseVendorClass> prepare(TreeDao<MESBasicBaseVendorClass, Long> dao) {
				return baseVendorClasss.getResult();
			}
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseVendorClasss(Page<MESBasicBaseVendorClass> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBaseVendorClasss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseVendorClasss(Page<MESBasicBaseVendorClass> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseVendorClasss(Page<MESBasicBaseVendorClass> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicBaseVendorClass> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"baseVendorClass\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"baseVendorClass\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"baseVendorClass\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_baseVenderClass_BaseVendorClass" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_baseVenderClass_BaseVendorClass" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  baseVendorClassDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  baseVendorClassDao.createNativeQuery(powerOperateSql, "MESBasic_1_baseVenderClass").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseVendorClass\"","MESBasic_1_baseVenderClass",customCondition,"S2BASE_VENDORCLASS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"baseVendorClass\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicBaseVendorClassDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"baseVendorClass\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"baseVendorClass\".VC_LAYREC ASC, \"baseVendorClass\".SORT ASC");
				} else {
					orderPart.append(" \"baseVendorClass\".VC_LAYREC ASC, \"baseVendorClass\".VC_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) baseVendorClassDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=baseVendorClassDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = baseVendorClassDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicBaseVendorClass> result = new ArrayList<MESBasicBaseVendorClass>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicBaseVendorClass.class,baseVendorClassDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicBaseVendorClass.class,baseVendorClassDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicBaseVendorClass baseVendorClass : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(baseVendorClass.getId(), "MESBasic_baseVenderClass_baseVendorClass");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(baseVendorClass.getId(), "MESBasic_baseVenderClass_baseVendorClass");
							if(null != documents && !documents.isEmpty()) {
								baseVendorClass.setDocument(documents.get(0));
								baseVendorClass.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(page.isExportFlag()){
					...
					if(exportMap.get("businessKey")!=null){
						try {
							...
							MESBasicBaseVendorClass baseVendorClass = new MESBasicBaseVendorClass();
							getBusKeyMethod = baseVendorClass.getClass().getMethod(getBusKeyMethodStr);
					...
					if(exportMap.get("mainDisplay")!=null){
						try {
							...
							MESBasicBaseVendorClass baseVendorClass = new MESBasicBaseVendorClass();
							getMainDisMethod = baseVendorClass.getClass().getMethod(getMainDisMethodStr);
						} catch (NoSuchMethodException | SecurityException e) {
							log.error(e.getMessage(),e);
						}
					}
					...
					for (MESBasicBaseVendorClass baseVendorClass : result) {
						Long parentId = baseVendorClass.getParentId();
						if(parentId != null && parentId.longValue() != new Long(-1).longValue()){
							for (MESBasicBaseVendorClass baseVendorClass2 : result) {
								if(baseVendorClass2.getId() != null && baseVendorClass2.getId().longValue() == parentId.longValue()){
									try {
										if(getBusKeyMethod != null){
											Object busKey = getBusKeyMethod.invoke(baseVendorClass2);
											baseVendorClass.setParentNodeBusinessKey(busKey.toString());
										}
										if(getMainDisMethod != null){
											Object mainDis = getMainDisMethod.invoke(baseVendorClass2);
											baseVendorClass.setParentNodeMainDisplay(mainDis.toString());
										}	
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_baseVenderClass")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_baseVenderClass")+"MESBasic_1_baseVenderClass".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_baseVenderClass",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicBaseVendorClass> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicBaseVendorClass> result) {
		View view = null;
		...
		Map<Long, MESBasicBaseVendorClass> rsMap = new HashMap<Long, MESBasicBaseVendorClass>();
		for (MESBasicBaseVendorClass baseVendorClass : result) {
			ids.add(baseVendorClass.getId());
			rsMap.put(baseVendorClass.getId(), baseVendorClass);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(baseVendorClassDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = baseVendorClassDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicBaseVendorClass baseVendorClass,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(baseVendorClass.getOwnerStaffId());
		ti.setOwnerPositionId(baseVendorClass.getOwnerPositionId());
		ti.setOwnerDepartmentId(baseVendorClass.getOwnerDepartmentId());
		
		ti.setCreateStaff(baseVendorClass.getCreateStaff());
		ti.setCreateStaffId(baseVendorClass.getCreateStaffId());
		ti.setCreateTime(baseVendorClass.getCreateTime());
		ti.setDeleteStaff(baseVendorClass.getDeleteStaff());
		ti.setDeleteStaffId(baseVendorClass.getDeleteStaffId());
		ti.setDeleteTime(baseVendorClass.getDeleteTime());
		ti.setCreateDepartmentId(baseVendorClass.getCreateDepartmentId());
		ti.setModifyStaff(baseVendorClass.getModifyStaff());
		ti.setModifyStaffId(baseVendorClass.getModifyStaffId());
		ti.setCreatePositionId(baseVendorClass.getCreatePositionId());
		ti.setPositionLayRec(baseVendorClass.getPositionLayRec());
		ti.setTableNo(baseVendorClass.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_baseVenderClass");
		ti.setTargetTableName("S2BASE_VENDORCLASS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_baseVenderClass_BaseVendorClass" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  baseVendorClassDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseVendorClass\"","MESBasic_1_baseVenderClass","","S2BASE_VENDORCLASS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"baseVendorClass\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"baseVendorClass\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseVendorClass\"","MESBasic_1_baseVenderClass","","S2BASE_VENDORCLASS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("baseVendorClass.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"baseVendorClass\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"baseVendorClass\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"baseVendorClass\".CID = 1 OR \"baseVendorClass\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("baseVendorClass.mc\"").append(".BASE_VENDOR_CLASS) not in ("+notinValue+")");
			}
			
			SQLQuery query = baseVendorClassDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("baseVenderClass", "MESBasic_1_baseVenderClass", "baseVenderClass_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=baseVendorClassDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicBaseVendorClass.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicBaseVendorClass.MODULE_CODE;}
	protected String getModelCode(){return MESBasicBaseVendorClass.MODEL_CODE;}
	
	@Override
	public List<MESBasicBaseVendorClass> getTreeChildren(Long parentId, String cond, Object... params) {
		MESBasicBaseVendorClass parent = baseVendorClassDao.getTree("baseVendorClass_tree_" + parentId, parentId, new MESBasicBaseVendorClassPrepareData(cond, params), false);
	...
	@Override
	public List<MESBasicBaseVendorClass> getTreeInfo(Map<Long, String> treeInfo, String condition, List<Object> params) {
		...
		for(String layRec : mapKeys) {
			...
			MESBasicBaseVendorClass baseVendorClassTree = baseVendorClassDao.getTree("MESBasicBaseVendorClass" + rootLayRec + "-", rootId, new PrepareData<MESBasicBaseVendorClass, Long>() {
				@Override
				public MESBasicBaseVendorClass root() {
					MESBasicBaseVendorClass baseVendorClass = new MESBasicBaseVendorClass();
					baseVendorClass.setId(-1l);
				...
				@Override
				public List<MESBasicBaseVendorClass> prepare(TreeDao<MESBasicBaseVendorClass, Long> dao) {
					String hql = "from " + MESBasicBaseVendorClass.JPA_NAME + " where (" + searchCondition + ") and valid = true " + conditionSql + " order by layRec asc, sort asc";
					return dao.findByHql(hql, paramsList.toArray());
				}
			}, false);
			List<MESBasicBaseVendorClass> treeChildren = new ArrayList<MESBasicBaseVendorClass>();
			Tree.treeToList(baseVendorClassTree, treeChildren, true);
			treeList.addAll(treeChildren);
		}
		return treeList;
	}

	private class MESBasicBaseVendorClassPrepareData implements PrepareData<MESBasicBaseVendorClass, Long> {
		private String cond;
		private Object[] params;
		
		public MESBasicBaseVendorClassPrepareData(String cond, Object... params) {
			this.cond = cond;
			this.params = params;
		}
		...
		//@Override
		//public List<MESBasicBaseVendorClass> prepare(TreeDao<MESBasicBaseVendorClass, Long> dao) {
		//	String hql = "from " + MESBasicBaseVendorClass.JPA_NAME;
		//	if (null != cond && cond.trim().length() > 0) {
		//		hql += " where " + cond;
		//	}
		//	List<MESBasicBaseVendorClass> list = dao.findByHql(hql);
		//	return list;
		//}
		
		@Override
		public List<MESBasicBaseVendorClass> prepare(TreeDao<MESBasicBaseVendorClass, Long> dao) {
			List<MESBasicBaseVendorClass> list = null;
			List<MESBasicBaseVendorClass> result = new ArrayList<MESBasicBaseVendorClass>();
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseVendorClass.class);
			if(params!=null&&params.length>0){
				...
				list = dao.findByHql("from " + MESBasicBaseVendorClass.JPA_NAME + " " + ((cond==null || cond.isEmpty()) ? "" : "where " + cond + " order by layRec asc, sort asc")) ;
			}
			for(MESBasicBaseVendorClass m : list){
				result.add(m);
				if(m.getParentId()!=-1){
					while(m.getParentId()!=-1){
						MESBasicBaseVendorClass parent = dao.get(m.getParentId());
	...
	/**
	 * 检查模型字段的唯一性约束
	 * @param baseVendorClass 模型实例
	 */
	public void checkUniqueConstraint(MESBasicBaseVendorClass baseVendorClass){
		if(null == baseVendorClass.getCode() || StringUtils.isEmpty(baseVendorClass.getCode().toString())){
			return;
		}
		if(baseVendorClass.getId()==null ){
			List<MESBasicBaseVendorClass> baseVendorClasss = baseVendorClassDao.findByCriteria(Restrictions.eq("code", baseVendorClass.getCode()));
			if(baseVendorClasss!=null && !baseVendorClasss.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1449472741485", getCurrentUser().getLanguage()), baseVendorClass.getCode());
			}
		} else {
			List<MESBasicBaseVendorClass> baseVendorClasss = baseVendorClassDao.findByCriteria(Restrictions.ne("id", baseVendorClass.getId()), Restrictions.eq("code", baseVendorClass.getCode()));
			if(baseVendorClasss!=null && !baseVendorClasss.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1449472741485", getCurrentUser().getLanguage()), baseVendorClass.getCode());
			}
	...
	@Override
	public List<MESBasicBaseVendorClass> getBaseVendorClasss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseVendorClass.class);
		if(params!=null){
			List<Type> types = new ArrayList<Type>();
			for(Object obj : params){
			...
			return baseVendorClassDao.findByHql("from " + MESBasicBaseVendorClass.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return baseVendorClassDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicBaseVendorClass> findBaseVendorClasssByHql(String hql, Object... objects){
		...
		return   baseVendorClassDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicBaseVendorClass loadBaseVendorClassByBussinessKey(MESBasicBaseVendorClass baseVendorClass){
		return loadBaseVendorClassByBussinessKey(baseVendorClass.getCode());
	}
	
	@Override
	public Page<MESBasicBaseVendorClass> getBaseVendorClasss(Page<MESBasicBaseVendorClass> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseVendorClass.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = baseVendorClassDao.findByPage(page, sql);
			}else{
				page = baseVendorClassDao.findAllByPage(page);
			}
			return page;
		}
		page = baseVendorClassDao.findByPage(page, detachedCriteria);
		return page;
	}
	//==============DataGrid多选控件使用 start================
	
	@Override
	public void dealBaseVendorClassLeaf(List<Long> baseVendorClassIds) {
		if (baseVendorClassIds != null && baseVendorClassIds.size() > 0) {
			String hql = "update " + MESBasicBaseVendorClass.JPA_NAME + " as t1 set t1.leaf=1 where id in(:ids) and not exists (select t2.id from "
					+ MESBasicBaseVendorClass.JPA_NAME + " as t2 where t2.parentId = t1.id)";
			Query query = baseVendorClassDao.createQuery(hql);
			query.setParameterList("ids", baseVendorClassIds);
			query.executeUpdate();
		}
	...
	public void drag(long dragSource, long dragTarget, long sort){
		MESBasicBaseVendorClass source = getBaseVendorClass(dragSource);
		MESBasicBaseVendorClass oldParent = source.getParent();
		
		MESBasicBaseVendorClass target = null;
		if(dragTarget!=-1){
			target = getBaseVendorClass(dragTarget);
		} else {
			target = new MESBasicBaseVendorClass();
			target.setId(-1L);
		}
		baseVendorClassDao.move(source, target);
	}
	
	private void dragBaseVendorClass(MESBasicBaseVendorClass parent, List<MESBasicBaseVendorClass> children){
		for(MESBasicBaseVendorClass child : children){
			...
			saveBaseVendorClass(child, null,null,null);
			List<MESBasicBaseVendorClass> subChildren = getTreeChildren(child.getId(), "");
			if(subChildren!=null && subChildren.size()>0){
				dragBaseVendorClass(child, subChildren);
			}
		}
	}
	
	public List<MESBasicBaseVendorClass> convertTree(List<MESBasicBaseVendorClass> baseVendorClasss){
		List<MESBasicBaseVendorClass> trees = new ArrayList<MESBasicBaseVendorClass>();
		Map<Long, MESBasicBaseVendorClass> map = new LinkedHashMap<Long, MESBasicBaseVendorClass>(baseVendorClasss.size());
		Map<Long, Boolean> isParent = new HashMap<Long, Boolean>();
		for (MESBasicBaseVendorClass node : baseVendorClasss) {
		...
		for (MESBasicBaseVendorClass node : baseVendorClasss) {
			if (null == node.getParentId() ||Long.parseLong(node.getParentId().toString()) == -1) {
				...
				MESBasicBaseVendorClass parent = map.get(node.getParentId());
	...
	public MESBasicBaseVendorClass loadBaseVendorClassByBussinessKey(Serializable bussinessKey){
		return baseVendorClassDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicBaseVendorClass> findByProperty(String propertyName, Object object){
		return baseVendorClassDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseVendorClass findEntityByProperty(String propertyName, Object object){
		return baseVendorClassDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicBaseVendorClass.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = baseVendorClassDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_baseVenderClass_BaseVendorClass","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_baseVenderClass", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicBaseVendorClass baseVendorClass,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,baseVendorClassDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicBaseVendorClass.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, baseVendorClass);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",baseVendorClass.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBaseVendorClass.TABLE_NAME +" where valid = 1";
			list = baseVendorClassDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBaseVendorClass.TABLE_NAME;
			list = baseVendorClassDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicBaseVendorClass.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = baseVendorClassDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicBaseVendorClassDaoImpl baseVendorClassDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBaseVendorClass(final List<MESBasicBaseVendorClass> insertObjs, final List<MESBasicBaseVendorClass> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBaseVendorClass(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBaseVendorClass(final List<MESBasicBaseVendorClass> insertObjs, final List<MESBasicBaseVendorClass> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1449472699525"),importCount));
		...
		Map<Long, MESBasicBaseVendorClass> oldUpdateObjsMap = new HashMap<Long, MESBasicBaseVendorClass>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicBaseVendorClass bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicBaseVendorClass> bizObjList = baseVendorClassDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicBaseVendorClass bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicBaseVendorClass> nmeObjs = new ArrayList<MESBasicBaseVendorClass>();
		...
		long id=autoGeneratorID.getNextId(MESBasicBaseVendorClass.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicBaseVendorClass baseVendorClass:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicBaseVendorClass.TABLE_NAME,1,dbId);
			if(baseVendorClass.getCode() != null && !baseVendorClass.getCode().equals("") ){
				m.put(baseVendorClass.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		List<MESBasicBaseVendorClass> needChangeInsertObjList = new ArrayList<MESBasicBaseVendorClass>();
		if(insertNodeList != null && insertNodeList.size()>0 ){
			for(int i = 0; i < insertNodeList.size(); i++){
				...
				if(!needChange && !isRoot){
					MESBasicBaseVendorClass obj = insertObjs.get(i);
					...
					MESBasicBaseVendorClass parentObj = (MESBasicBaseVendorClass) insertNode.get(parentNode);
					*/
		...
		if(updateNodeList != null && updateNodeList.size()>0){
			for(int i = 0; i < updateNodeList.size(); i++){
				...
				MESBasicBaseVendorClass updateObj = updateObjs.get(i);
				...
				if(isRoot){
					...
					if(needChange!=null && !needChange){
						...
						MESBasicBaseVendorClass parentNode = (MESBasicBaseVendorClass) insertParentNode.get(parentNodeKey);
		...
		if(insertNodeList != null && insertNodeList.size()>0){
			for(int i = 0; i < insertNodeList.size(); i++){
				Map<String, Object> insertNode = insertNodeList.get(i);
				MESBasicBaseVendorClass insertObj = insertObjs.get(i);
		...
		if(updateObjs != null && updateObjs.size()>0 ){
			for(MESBasicBaseVendorClass obj : updateObjs){
				Object bus;
				try {
		...
		if(businessKey != null && mainDisplay != null && businessKey != mainDisplay  && paramList != null && paramList.size()>0){
			String hql = "select " + businessKey.getName() + "," + mainDisplay.getName() + ",id from " + MESBasicBaseVendorClass.JPA_NAME + " where " + businessKey.getName() + " in (:codes)";
			Query query =  baseVendorClassDao.createQuery(hql);
			query.setParameterList("codes", paramList);
			List<Object[]> list = query.list();
			if(list.size() == updateObjs.size()){
				for(int i = 0 ; i < updateObjs.size();i++ ){
					MESBasicBaseVendorClass obj = updateObjs.get(i);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) baseVendorClassDao.createNativeQuery(sql, "MESBasic_1_baseVenderClass_BaseVendorClass"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		baseVendorClassDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicBaseVendorClass.TABLE_NAME + " SET ";
					...
					updateSql += "parent_id = ?, VC_LAYNO = ?, VC_LAYREC = ?, FULL_PATH_NAME = ?";
					updateSql += " where " + dbId + " =?";
					
					updateps = conn.prepareStatement(updateSql);
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicBaseVendorClass baseVendorClass = updateObjs.get(updateIndex);
						Long id = baseVendorClass.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = baseVendorClass.getClass().getMethod(updateMethods.get(i));
								} catch (NoSuchMethodException | SecurityException e) {
									log.error(e.getMessage(),e);
								}
								Object obj = null;
								if(null != method){
									try {
										obj = method.invoke(baseVendorClass);
									} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
										log.error(e.getMessage(),e);
									}
								}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicBaseVendorClass.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
					...
					insertSql += dbId + ",CID,CREATE_STAFF_ID,CREATE_TIME,EFFECTIVE_STATE,PARENT_ID,VC_LAYNO,VC_LAYREC,FULL_PATH_NAME) VALUES (";
					for(int i = 0;i<columnMaps.size();i++){
						insertSql += "?,";
					}
					insertSql += "?,?,?,?,?,?,?,?,?)";
					
//					String idSql = "select id from " + MESBasicBaseVendorClass.TABLE_NAME + " order by id desc";
//					Query query = baseVendorClassDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicBaseVendorClass baseVendorClass = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = baseVendorClass.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicBaseVendorClassMneCode.class;
					...
					for(MESBasicBaseVendorClass baseVendorClass:updateObjs)  {
						Long id = baseVendorClass.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicBaseVendorClass baseVendorClass:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(baseVendorClass);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, baseVendorClass.getId());
		...
		baseVendorClassDaoImpl.getSession().flush();
		baseVendorClassDaoImpl.getSession().clear();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicBaseVendorClass bizObj = insertObjs.get(insertIndex);
			baseVendorClassImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicBaseVendorClass bizObj = updateObjs.get(updateIndex);
			MESBasicBaseVendorClass oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			baseVendorClassImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	private void changeFullPathName(List<Long> needChangeFullPathNameId, Map<Object, Object> needChangeFullPathName,final Property businessKey,Property mainDisplay){
		...
		sql.append(" from ");
		sql.append(MESBasicBaseVendorClass.TABLE_NAME);
		sql.append(" where ");
		for(Long id : needChangeFullPathNameId){
			sql.append(" VC_LAYREC like '%");
			sql.append(id.toString());
			sql.append("%' or");
		}
		String sqlStr = sql.toString();
		sqlStr = sqlStr.substring(0, sqlStr.length()-2);
		List<Object[]> lists =  baseVendorClassDao.createNativeQuery(sqlStr).list();
		...
		baseVendorClassDaoImpl.getSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
				...
				if(updateLists != null && updateLists.size()>0){
					String updateSql = "UPDATE " + MESBasicBaseVendorClass.TABLE_NAME + " SET FULL_PATH_NAME = ? WHERE " + businessKey.getColumnName() + " = ?";
					updateps = conn.prepareStatement(updateSql);
	...
	private Map<String, Object> buildInsertObjNode(List<Map<String, Object>> insertNodeList, List<MESBasicBaseVendorClass> insertObjs, 
			...
			for(int i =0; i<insertNodeList.size(); i++){
				...
				if(insertBuildedObj.get(busKey)!=null){
					...
					if(isRoot){
						...
						String parentNode = insertNode.get("parentNode").toString();
						MESBasicBaseVendorClass parentObj = (MESBasicBaseVendorClass) insertBuildedObj.get(parentNode);
	...
	public String getViewName(String modelCode){
		...
		String viewName = baseVendorClassDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				baseVendorClassDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = baseVendorClassDao.createNativeQuery(sql, "MESBasic_1_baseVenderClass_BaseVendorClass").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = baseVendorClassDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = baseVendorClassDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = baseVendorClassDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?baseVendorClassDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  baseVendorClassDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,baseVendorClassDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="ec.flow.save")
	public void saveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, DataGridService dataGridService){
		saveBaseVendorClass(baseVendorClass, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="ec.flow.save")
	public void saveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, DataGridService dataGridService, String viewCode){
		saveBaseVendorClass(baseVendorClass, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass")
	public void saveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBaseVendorClass(baseVendorClass, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass")
	public void saveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_baseVenderClass";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseVenderClass");
		String url = null;
		if(null != baseVendorClass.getId() && baseVendorClass.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendorClass.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass")
	public Map<Object, Object> baseVendorClassDataGridImport(MESBasicBaseVendorClass baseVendorClass, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = baseVendorClass.getVirtualId();
		
		baseVendorClass = baseVendorClassDao.load(baseVendorClass.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, baseVendorClass.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = baseVendorClass.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(baseVendorClass);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, baseVendorClass.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass")
	public void mergeBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(baseVendorClass);
		beforeSaveBaseVendorClass(baseVendorClass);
		baseVendorClassDao.merge(baseVendorClass);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVenderClass",model="MESBasic_1_baseVenderClass_BaseVendorClass",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicBaseVendorClass> getByPage(Page<MESBasicBaseVendorClass> page,DetachedCriteria detachedCriteria){
		return baseVendorClassDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_baseVenderClass").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, Object... objects){
		checkUniqueConstraint(baseVendorClass);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBaseVendorClass(MESBasicBaseVendorClass baseVendorClass, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
      //查找所加分类是否有供应商关联
		if(baseVendorClass.getParent() !=null){
			MESBasicBaseVendorClass parentbaseVendorClass = baseVendorClass.getParent();
			List<MESBasicBaseVendor> list=baseVendorDao.createQuery("from " + MESBasicBaseVendor.JPA_NAME + " where valid <> 0 and vendorVCCode.code=?", parentbaseVendorClass.getCode()).setMaxResults(1).list();
			if(list.size()>0){
				throw new BAPException("所选分类存在供应商关联，不允许增加下级分类！");
			}
		}
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBaseVendorClass(MESBasicBaseVendorClass baseVendorClass){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
      //查找分类是否有供应商关联
		List<MESBasicBaseVendor> list=baseVendorDao.createQuery("from " + MESBasicBaseVendor.JPA_NAME + " where valid <> 0 and vendorVCCode.code=?", baseVendorClass.getCode()).setMaxResults(1).list();
		if(list.size()>0){
			throw new BAPException("所选分类存在供应商关联，不允许删除分类！");
		}
/* CUSTOM CODE END */
	}
	
	private void afterDeleteBaseVendorClass(MESBasicBaseVendorClass baseVendorClass){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBaseVendorClass(MESBasicBaseVendorClass baseVendorClass){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBaseVendorClass(MESBasicBaseVendorClass baseVendorClass){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicBaseVendorClass baseVendorClass, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicBaseVendorClass baseVendorClass, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportBaseVendorClass(List<MESBasicBaseVendorClass> insertObjs, List<MESBasicBaseVendorClass> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBaseVendorClass,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportBaseVendorClass(List<MESBasicBaseVendorClass> insertObjs, List<MESBasicBaseVendorClass> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBaseVendorClass,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
	/*
    	根据传入目录Code判断该目录下是否含有供应商，是返回1，否返回0
  	*/
    public String hasVendor(long vendorClassId){
      	MESBasicBaseVendorClass vendorClass = getBaseVendorClass(vendorClassId);
      	
      
      	List<MESBasicBaseVendor> list=baseVendorDao.createQuery("from " + MESBasicBaseVendor.JPA_NAME + " where vendorVCCode.code=?", vendorClass.getCode()).setMaxResults(1).list();
		if(list.size()>0){
			return "1";
		}
		return "0";
