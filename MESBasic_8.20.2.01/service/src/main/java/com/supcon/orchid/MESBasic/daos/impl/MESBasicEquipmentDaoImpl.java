...
@Repository
public class MESBasicEquipmentDaoImpl extends ExtendedGenericDaoImpl<MESBasicEquipment, Long> implements MESBasicEquipmentDao {
	@Override
	...
	@Override
	public MESBasicEquipmentDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicEquipmentDealInfo dealInfo = new MESBasicEquipmentDealInfo();
	...
	@Override
	public MESBasicEquipmentDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicEquipment mainObj, Staff dealStaff) {
		MESBasicEquipmentDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicEquipmentDealInfo.class);
		...
		List<MESBasicEquipmentDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
