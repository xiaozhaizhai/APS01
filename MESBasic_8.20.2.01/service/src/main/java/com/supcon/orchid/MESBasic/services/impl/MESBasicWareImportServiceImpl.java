...
@Service("mESBasicWareImportService")
public class MESBasicWareImportServiceImpl implements MESBasicWareImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicWare bizObj, MESBasicWare oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1484030983271") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getWareName() ? "" : "," + oldBizObj.getWareName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getWareName() ? "" : oldBizObj.getWareName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getWareCode() ? "" : "," + oldBizObj.getWareCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getWareCode() ? "" : oldBizObj.getWareCode().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1484030983271") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getWareName() ? "" : "," + bizObj.getWareName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getWareName() ? "" : bizObj.getWareName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getWareCode() ? "" : "," + bizObj.getWareCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getWareCode() ? "" : bizObj.getWareCode().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicWare bizObj, String bussinesskeyFieldName,
