...
@Repository
public class MESBasicBatchInformationDaoImpl extends ExtendedGenericDaoImpl<MESBasicBatchInformation, Long> implements MESBasicBatchInformationDao {
	@Override
	...
	@Override
	public MESBasicBatchInformationDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicBatchInformationDealInfo dealInfo = new MESBasicBatchInformationDealInfo();
	...
	@Override
	public MESBasicBatchInformationDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicBatchInformation mainObj, Staff dealStaff) {
		MESBasicBatchInformationDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicBatchInformationDealInfo.class);
		...
		List<MESBasicBatchInformationDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
