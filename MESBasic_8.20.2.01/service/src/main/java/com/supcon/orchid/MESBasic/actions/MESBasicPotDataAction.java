...
/* CUSTOM CODE START(action,import,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicPotDataAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicPotData> {
	...
	private MESBasicPotData potData;
	...
	//导出excel时,主模型内容
	List<MESBasicPotData> queryResults = new ArrayList<MESBasicPotData>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/potDataHandler/potData/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/potDataHandler/potData/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_potDataHandler_PotData" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicPotData> potDatas;
	
	public List<MESBasicPotData> getPotDatas() {
		return potDatas;
	}
	...
	private Page<MESBasicPotData> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			potData = new MESBasicPotData();
			if(potData.getBusinessType() != null) {
				potData.setBusinessType((SystemCode) systemCodeService.getSystemCode(potData.getBusinessType().getId()));
			}
		} else {
			if(id > 0){
				potData = potDataService.getPotData(id, viewCode);
			}
		}
	...
	public Page<MESBasicPotData> getPage() {
		if (null == page) {
			page = new Page<MESBasicPotData>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicPotData getPotData() {
		return potData;
	}		

	public void setPotData(MESBasicPotData potData) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/potDataHandler/potData/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = potDataService.getPotDataAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/potDataHandler/potData/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String potDataReferenceCopy() throws Exception{
			MESBasicPotData oldPotData = null;
			if(isEditNew()){
				...
				oldPotData = new MESBasicPotData();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,potData.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, potData.getId());
				...
				oldPotData = potDataService.getPotData(id);
			}
			
			
			potData = potDataService.getPotData(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_potDataHandler_PotData");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicPotData.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPotData, null);
						Method setValue = MESBasicPotData.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(potData, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			potData.setCreateDepartmentId(oldPotData.getCreateDepartmentId());
			potData.setCreatePositionId(oldPotData.getCreatePositionId());
			potData.setCreateStaffId(oldPotData.getCreateStaffId());
			potData.setOwnerDepartmentId(oldPotData.getOwnerDepartmentId());
			potData.setOwnerPositionId(oldPotData.getOwnerPositionId());
			potData.setOwnerStaffId(oldPotData.getOwnerStaffId());
			potData.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_potDataHandler_PotData", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/potDataHandler/potData/dataEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataEdit.ftl") }),
		@Action(value = "/MESBasic/potDataHandler/potData/dataEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=potDataService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return potDataReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			potData = potDataService.getPotData(this.potData.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,potData.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/potDataHandler/potData/dataView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataView.ftl") }),
		@Action(value = "/MESBasic/potDataHandler/potData/dataViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/potDataHandler/potData/dataList", results = { @Result(type = FREEMARKER, location = "dataList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = potDataService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(potDataService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(potDataService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_potDataHandler_PotData", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/potDataHandler/potData/dataList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"potData\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"potData\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"potData\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"potData\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/potDataHandler/potData/dataList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/potDataHandler/potData/allTempleteDownload", results = { @Result(name="excel",type="excel",location="dataList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_potDataHandler_PotData");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		potDataService.findDataGridPage(dg,dgclass,getDgPage(),potData,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(potData.getId() != null && potData.getId() != -1){
				potData = potDataService.getPotData(potData.getId());
			}
			potDataService.findDataGridPage(dg,dgclass,getDgPage(),potData,searchCondition,params);
	...
	private void save() throws Exception {
		...
		potDataService.savePotData(potData,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/potDataHandler/potData/dataEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/potDataHandler/potData/dataView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "submit");
		bap_validate_datagrids=potDataService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", potData.getId());
		...
		fileuploads.put("linkId", potData.getId());
		bapAfterCustomMethod("MESBasic_1_potDataHandler_PotData", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/potDataHandler/potData/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		potDataService.deletePotData(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_potDataHandler_PotData", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/potDataHandler/potData/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "restore");
		potDataService.restorePotData(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_potDataHandler_PotData", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/potDataHandler/potData/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/potDataHandler/potData/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "showDealInfoData");
		if (null != tableInfoId) {
			int count=potDataService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = potDataService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = potDataService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_potDataHandler_PotData", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicPotData> findExportDataInfos(Page<MESBasicPotData> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = potDataService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = potDataService.getAssProperty(p.getCode());	
									//String assModelCode = potDataService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			potDataService.findPotDatas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			potDataService.findPotDatas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicPotData> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicPotData result : results){
	...
	public boolean isEditNew(){
		//return null == this.potData || null == this.potData.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/potDataHandler/potData/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_potDataHandler_PotData", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/potDataHandler/potData/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/potDataHandler/potData/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/potDataHandler/potData/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicPotDataService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_potDataHandler", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_potDataHandler");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_potDataHandler_PotData", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/potDataHandler/potData/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_potDataHandler_PotData", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_potDataHandler_PotData", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/potDataHandler/potData/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/potDataHandler/potData/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_potDataHandler_PotData";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_potDataHandler_PotData", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/potDataHandler/potData/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicPotData potData = null;
		if(null != id) {
			potData = potDataService.getPotData(id);
		}
		...
		_auditLog.setColumnStringB(null == potData.getTempID() ? "" : potData.getTempID().toString());
		_auditLog.setColumnStringA(null == potData.getId() ? "" : potData.getId().toString());
		_auditLog.setColumnStringC(potData.getId().toString());
		_auditLog.setColumnStringD(MESBasicPotData.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_potDataHandler");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicPotDataAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, potData,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, potData,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,potData);
		...
		printlog.setEntity("MESBasic_1_potDataHandler");
		...
		printlog.setBusinessId(potData.getId());
		printlog.setBussinessKey(null == potData.getTempID() ? "" : potData.getTempID().toString());
		printlog.setMainDisplay(null == potData.getId() ? "" : potData.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/potDataHandler/potData/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(potData,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/potDataHandler/potData/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(potDataService.getPotData(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/potDataHandler/potData/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/potDataHandler/potData/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, potData,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				potData = potDataService.getPotData(Long.parseLong(arrId[i]));
				if(null != potData){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == potData.getId() ? "" : "," + potData.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == potData.getId() ? "" : potData.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == potData.getTempID() ? "" : "," + potData.getTempID().toString()));
					} else {
						_auditLog.setColumnStringB(null == potData.getTempID() ? "" : potData.getTempID().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == potData.getId() ? "" : "," + potData.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == potData.getId() ? "" : potData.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, potData,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,potData);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/potDataHandler/potData/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicPotData> findExportPageInfos(Page<MESBasicPotData> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_potDataHandler" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = potDataService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicPotData queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = potDataService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = potDataService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_potDataHandler_PotData");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_potDataHandler_PotData")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_potDataHandler_PotData");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_potDataHandler_PotData");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = potDataService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
