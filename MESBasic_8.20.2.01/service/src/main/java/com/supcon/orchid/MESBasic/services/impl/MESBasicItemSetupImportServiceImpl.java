...
@Service("mESBasicItemSetupImportService")
public class MESBasicItemSetupImportServiceImpl implements MESBasicItemSetupImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemSetup",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicItemSetup bizObj, MESBasicItemSetup oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.radion1415258377559") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getItemName() ? "" : "," + oldBizObj.getItemName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getItemName() ? "" : oldBizObj.getItemName().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.radion1415258377559") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getItemName() ? "" : "," + bizObj.getItemName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getItemName() ? "" : bizObj.getItemName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicItemSetup bizObj, String bussinesskeyFieldName,
