...
@Repository
public class MESBasicBaseProdListDaoImpl extends TreeDaoImpl<MESBasicBaseProdList, Long> implements MESBasicBaseProdListDao {
	@Override
	...
	@Override
	public MESBasicBaseProdListDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicBaseProdListDealInfo dealInfo = new MESBasicBaseProdListDealInfo();
	...
	@Override
	public MESBasicBaseProdListDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicBaseProdList mainObj, Staff dealStaff) {
		MESBasicBaseProdListDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicBaseProdListDealInfo.class);
		...
		List<MESBasicBaseProdListDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_prodList_BaseProdList,MESBasic_1) */
// 自定义代码
