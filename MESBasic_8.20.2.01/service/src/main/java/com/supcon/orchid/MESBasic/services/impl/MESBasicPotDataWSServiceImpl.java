...
import com.supcon.orchid.MESBasic.services.MESBasicPotDataWSService;
import com.supcon.orchid.MESBasic.services.MESBasicPotDataService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicPotData",targetNamespace = "http://ws.supcon.com")
public class MESBasicPotDataWSServiceImpl implements MESBasicPotDataWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getPotDataByBusinessKey(
		...
		try{
			...
			MESBasicPotData potData = potDataService.loadPotDataByBussinessKey(businessKey);
			if(potData == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, potData);
		}catch(Throwable t){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getPotDataByPk(
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO updatePotData(
		@WebParam(name="potData")MESBasicPotData potData,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
			
		JWSResultDTO result = new JWSResultDTO();
		try{
			if(potData == null){
				return set500Error(result, "罐区数据接口不能为空。");
			}
			...
			if(!beforeUpdatePotData(potData)){
				return set404Error(result);
			}
			potDataService.savePotData(potData, null);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO addPotData(
		@WebParam(name="potData")MESBasicPotData potData,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				checkPotDataData(potData);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			potDataService.savePotData(potData, null);
			setSuccessResult(result, potData.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deletePotDataByBusinessKey(
		@WebParam(name="businessKey")String businessKey,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				potData = potDataService.loadPotDataByBussinessKey(businessKey);
	...
	@Override//
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deletePotDataByPk(
		@WebParam(name="pk")String pk,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddPotData(
		@WebParam(name="potDataList")List<MESBasicPotData> potDataList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			int count = 1;
			for(MESBasicPotData potData : potDataList){
				if(potData.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(potData.getId() == null || potData.getId().toString().trim().isEmpty()){
			...
			potDataService.batchImportBasePotData(potDataList);
			for(MESBasicPotData potData : potDataList){
				idList.add(potData.getId());
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdatePotData(
		@WebParam(name="potDataList")List<MESBasicPotData> potDataList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeletePotDataByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicPotData temp = null;
				try{
					temp = potDataService.getPotData(id);
				}catch(NullPointerException e){}
			...
			potDataService.deletePotData(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeletePotDataByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicPotData temp = null;
				try{
					temp = potDataService.loadPotDataByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getPotDataPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicPotData.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicPotData.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicPotData.class);
				} catch (Exception e) {
			...
			Page<MESBasicPotData> page = new Page<MESBasicPotData>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicPotData> datas = potDataService.getByPage(page, detachedCriteria);
			List<MESBasicPotData> potDataList = datas.getResult();
			if (potDataList == null || potDataList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(potDataList.toArray());
			PaginationDTO<MESBasicPotData> pagination = new PaginationDTO<MESBasicPotData>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicPotData potData = potDataService.getPotData(id);
			if (potData == null || !potData.isValid()) {
				result.setErrorMessage("找不到罐区数据接口");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取罐区数据接口对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param potData
	 * @return
	 */
	private MESBasicPotData getPotData(MESBasicPotData potData) {
		if (potData.getId() == null && (potData.getId() == null || potData.getId().toString().trim() == ""))
			throw new RuntimeException("罐区数据接口对象业务主键和id不能同时为空");
		if (potData.getId() != null) {
			MESBasicPotData result = potDataService.getPotData(potData .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return potDataService.loadPotDataByBussinessKey(potData.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个罐区数据接口，则返回false，否则返回true
	 * @param potData
	 */
	private boolean beforeUpdatePotData(MESBasicPotData potData) {
		MESBasicPotData temp = getPotData(potData);
		if (temp == null) {
			return false;
		}
		if(potData.getId() == null){
			potData.setId(temp.getId());		
		}
		if(potData.getId() == null){
			potData.setId(temp.getId());
		}
		
		potData.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除罐区数据接口对象下所有辅模型对象中的罐区数据接口对象
	 * @param potData
	 */
	private void clearCircle(MESBasicPotData potData) {
	 ...
	 */
	private void checkPotDataData(MESBasicPotData potData){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
