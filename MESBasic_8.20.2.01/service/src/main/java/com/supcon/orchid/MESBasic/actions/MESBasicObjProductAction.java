...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicObjProductAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicObjProduct> {
	...
	private MESBasicObjProduct objProduct;
	...
	//导出excel时,主模型内容
	List<MESBasicObjProduct> queryResults = new ArrayList<MESBasicObjProduct>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objProduct/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objProduct/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryModel_ObjProduct" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicObjProduct> objProducts;
	
	public List<MESBasicObjProduct> getObjProducts() {
		return objProducts;
	}
	...
	private Page<MESBasicObjProduct> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			objProduct = new MESBasicObjProduct();
		} else {
			if(id > 0){
				objProduct = objProductService.getObjProduct(id, viewCode);
			}
		}
	...
	public Page<MESBasicObjProduct> getPage() {
		if (null == page) {
			page = new Page<MESBasicObjProduct>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicObjProduct getObjProduct() {
		return objProduct;
	}		

	public void setObjProduct(MESBasicObjProduct objProduct) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objProduct/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = objProductService.getObjProductAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objProduct/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1460950716828
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String objProductReferenceCopy() throws Exception{
			MESBasicObjProduct oldObjProduct = null;
			if(isEditNew()){
				...
				oldObjProduct = new MESBasicObjProduct();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objProduct.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, objProduct.getId());
				...
				oldObjProduct = objProductService.getObjProduct(id);
			}
			
			
			objProduct = objProductService.getObjProduct(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjProduct");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicObjProduct.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldObjProduct, null);
						Method setValue = MESBasicObjProduct.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(objProduct, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			objProduct.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryModel_ObjProduct", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=objProductService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return objProductReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			objProduct = objProductService.getObjProduct(this.objProduct.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objProduct.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/objProduct/factoryRef2", results = { @Result(type = FREEMARKER, location = "factoryRef2.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = objProductService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(objProductService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(objProductService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryModel_ObjProduct", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/objProduct/factoryProdRefPart", results = { @Result(type = FREEMARKER, location = "factoryProdRefPart.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"objProduct\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"objProduct\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"objProduct\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"objProduct\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/objProduct/factoryProdRefPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.objFactory.name,result.objProduct.productName,result.objProduct.productCode,result.objFactory.id,result.objProduct.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="factoryProdRefPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		objProductService.findDataGridPage(dg,dgclass,getDgPage(),objProduct,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(objProduct.getId() != null && objProduct.getId() != -1){
				objProduct = objProductService.getObjProduct(objProduct.getId());
			}
			objProductService.findDataGridPage(dg,dgclass,getDgPage(),objProduct,searchCondition,params);
	...
	private void save() throws Exception {
		...
		objProductService.saveObjProduct(objProduct,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjProduct", "submit");
		bap_validate_datagrids=objProductService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", objProduct.getId());
		...
		fileuploads.put("linkId", objProduct.getId());
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjProduct", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryModel/objProduct/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjProduct", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		objProductService.deleteObjProduct(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjProduct", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objProduct/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjProduct", "restore");
		objProductService.restoreObjProduct(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjProduct", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicObjProduct> findExportDataInfos(Page<MESBasicObjProduct> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = objProductService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = objProductService.getAssProperty(p.getCode());	
									//String assModelCode = objProductService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			objProductService.findObjProducts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			objProductService.findObjProducts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicObjProduct> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicObjProduct result : results){
	...
	public boolean isEditNew(){
		//return null == this.objProduct || null == this.objProduct.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryModel/objProduct/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjProduct", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjProduct", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryModel/objProduct/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjProduct", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryModel/objProduct/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objProduct/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicObjProductService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjProduct", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjProduct", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objProduct/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryModel_ObjProduct", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryModel_ObjProduct", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryModel/objProduct/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjProduct", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryModel/objProduct/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryModel_ObjProduct";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjProduct", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryModel/objProduct/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == objProduct.getId() ? "" : objProduct.getId().toString());
		_auditLog.setColumnStringA(null == objProduct.getId() ? "" : objProduct.getId().toString());
		_auditLog.setColumnStringC(objProduct.getId().toString());
		_auditLog.setColumnStringD(MESBasicObjProduct.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicObjProductAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, objProduct,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, objProduct,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,objProduct);
		...
		printlog.setBusinessId(objProduct.getId());
		printlog.setBussinessKey(null == objProduct.getId() ? "" : objProduct.getId().toString());
		printlog.setMainDisplay(null == objProduct.getId() ? "" : objProduct.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryModel/objProduct/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(objProduct,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objProduct/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(objProductService.getObjProduct(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryModel/objProduct/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryModel/objProduct/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, objProduct,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objProduct = objProductService.getObjProduct(Long.parseLong(arrId[i]));
				if(null != objProduct){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == objProduct.getId() ? "" : "," + objProduct.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == objProduct.getId() ? "" : objProduct.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == objProduct.getId() ? "" : "," + objProduct.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == objProduct.getId() ? "" : objProduct.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == objProduct.getId() ? "" : "," + objProduct.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == objProduct.getId() ? "" : objProduct.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, objProduct,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,objProduct);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/factoryModel/objProduct/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjProduct");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryModel_ObjProduct")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryModel_ObjProduct");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjProduct");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = objProductService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
