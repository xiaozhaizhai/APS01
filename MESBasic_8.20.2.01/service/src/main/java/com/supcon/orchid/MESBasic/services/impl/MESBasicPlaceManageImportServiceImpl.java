...
@Service("mESBasicPlaceManageImportService")
public class MESBasicPlaceManageImportServiceImpl implements MESBasicPlaceManageImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicPlaceManage bizObj, MESBasicPlaceManage oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.radion1418607087374") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getPlaceName() ? "" : "," + oldBizObj.getPlaceName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getPlaceName() ? "" : oldBizObj.getPlaceName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getPlaceCode() ? "" : "," + oldBizObj.getPlaceCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getPlaceCode() ? "" : oldBizObj.getPlaceCode().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.radion1418607087374") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getPlaceName() ? "" : "," + bizObj.getPlaceName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getPlaceName() ? "" : bizObj.getPlaceName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getPlaceCode() ? "" : "," + bizObj.getPlaceCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getPlaceCode() ? "" : bizObj.getPlaceCode().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicPlaceManage bizObj, String bussinesskeyFieldName,
