...
import com.supcon.orchid.MESBasic.services.MESBasicCprodAttImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicCprodAttDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_cprodAttService")
@Transactional
public class MESBasicCprodAttServiceImpl  extends BaseServiceImpl<MESBasicCprodAtt> implements MESBasicCprodAttService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicCprodAtt> bapBaseService;
	...
	@Autowired
	private MESBasicCprodAttImportService cprodAttImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicCprodAtt.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  cprodAttDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicCprodAtt.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = cprodAttDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicCprodAtt.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicCprodAtt.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  cprodAttDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicCprodAtt.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicCprodAtt.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicCprodAtt> list = (List<MESBasicCprodAtt>) cprodAttDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicCprodAtt getCprodAtt(long id){
		return getCprodAtt(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicCprodAtt getCprodAtt(long id, String viewCode){
		MESBasicCprodAtt cprodAtt = cprodAttDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getCprodAttAsJSON(long id, String include){
		MESBasicCprodAtt cprodAtt = cprodAttDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(cprodAtt, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.common.delete",operType="3")
	public void deleteCprodAtt(MESBasicCprodAtt cprodAtt){
		if(null != cprodAtt && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCprodAtt.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
			AuditUtil.setColumnStringA(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
			AuditUtil.setColumnStringC(cprodAtt.getId().toString());
		}
		beforeDeleteCprodAtt(cprodAtt);
		cprodAttDao.deletePhysical(cprodAtt);
		afterDeleteCprodAtt(cprodAtt);
		cprodAttDao.flush();
		List<MESBasicCprodAtt> params = new ArrayList<MESBasicCprodAtt>();
		params.add(cprodAtt);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.common.delete",operType="3")
	public void deleteCprodAtt(List<Long> cprodAttIds){
		deleteCprodAtt(cprodAttIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.common.delete",operType="3")
	public void deleteCprodAtt(List<Long> cprodAttIds, String eventTopic) {
		List<MESBasicCprodAtt> cprodAtts = new ArrayList<MESBasicCprodAtt>();
		for(Long cprodAttId : cprodAttIds){
			MESBasicCprodAtt cprodAtt = getCprodAtt(cprodAttId);
			cprodAtts.add(cprodAtt);
			if(null != cprodAtt && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCprodAtt.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == cprodAtt.getId() ? "" : "," + cprodAtt.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == cprodAtt.getId() ? "" : "," + cprodAtt.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == cprodAtt.getId() ? "" : "," + cprodAtt.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(cprodAtts != null && cprodAtts.size() > 0 ){
			for(MESBasicCprodAtt cprodAtt : cprodAtts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicCprodAtt.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(cprodAtt));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),cprodAtt, cprodAtt.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(cprodAttIds != null && cprodAttIds.size() > 0) {
			String hql = "delete " + MESBasicCprodAtt.JPA_NAME + " where id in(:ids)";
			Query query = cprodAttDao.createQuery(hql);
			query.setParameterList("ids", cprodAttIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicCprodAtt cprodAttz : cprodAtts){
				cprodAttz.setValid(false);
				List<MESBasicCprodAtt> params = new ArrayList<MESBasicCprodAtt>();
				params.add(cprodAttz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_product");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.common.delete",operType="3")
	public void deleteCprodAtt(long cprodAttId, int cprodAttVersion){
		this.deleteCprodAtt(cprodAttId, cprodAttVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.common.delete",operType="3")
	public void deleteCprodAtt(long cprodAttId, int cprodAttVersion,SignatureLog signatureLog){
		MESBasicCprodAtt cprodAtt = getCprodAtt(cprodAttId);
		...
		beforeDeleteCprodAtt(cprodAtt);
		cprodAttDao.delete(cprodAttId, cprodAttVersion);
		afterDeleteCprodAtt(cprodAtt);
		cprodAttDao.flush();
		List<MESBasicCprodAtt> params = new ArrayList<MESBasicCprodAtt>();
		params.add(cprodAtt);
		...
		if(signatureLog != null ) {
			...
			businessKey=cprodAtt.getId();
			signatureLog.setTableId(cprodAtt.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.common.delete",operType="3")
	public void deleteCprodAtt(String cprodAttIds){
		this.deleteCprodAtt(cprodAttIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.common.delete",operType="3")
	public void deleteCprodAtt(String cprodAttIds,SignatureLog signatureLog){
		deleteCollection(cprodAttIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteCprodAtt(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_product#modelCode:MESBasic_1_product_CprodAtt#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="ec.engine.restore",operType="10")
	public void restoreCprodAtt(String cprodAttIds){
		restoreCollection(cprodAttIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreCprodAtt(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="ec.engine.restore",operType="10")
	public void restoreCprodAtt(long cprodAttId){
		
		findBusinessKeyUsed(cprodAttId);	//判断业务主键是否重复
		...
		beforeRestoreCprodAtt(cprodAtt);
		cprodAtt.setValid(true);
		cprodAttDao.update(cprodAtt);
		afterRestoreCprodAtt(cprodAtt);
		List<MESBasicCprodAtt> params = new ArrayList<MESBasicCprodAtt>();
		params.add(cprodAtt);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long cprodAttId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_product_CprodAtt");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicCprodAtt.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicCprodAtt.TABLE_NAME +" where id = ? )";
			List<Object> list =  cprodAttDao.createNativeQuery(sql,cprodAttId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseCprodAtt(List<MESBasicCprodAtt>  cprodAtts){
		for(MESBasicCprodAtt cprodAtt:cprodAtts)  {
			saveCprodAtt(cprodAtt, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseCprodAtt(List<MESBasicCprodAtt>  cprodAtts){
		for(MESBasicCprodAtt cprodAtt:cprodAtts)  {
			saveCprodAtt(cprodAtt, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="ec.flow.save")
	public void saveCprodAtt(MESBasicCprodAtt cprodAtt, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveCprodAtt(cprodAtt, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="ec.flow.save")
	public void saveCprodAtt(MESBasicCprodAtt cprodAtt, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveCprodAtt(cprodAtt, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateCprodAttCodes(MESBasicCprodAtt cprodAtt) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCprodAttCodes(cprodAtt, false);
	}
	
	public void generateCprodAttCodes(MESBasicCprodAtt cprodAtt, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateCprodAttSummarys(MESBasicCprodAtt cprodAtt) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCprodAttSummarys(cprodAtt, false);
	}

	public void generateCprodAttSummarys(MESBasicCprodAtt cprodAtt, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt")
	public void saveCprodAtt(MESBasicCprodAtt cprodAtt,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveCprodAtt(cprodAtt, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt")
	public void saveCprodAtt(MESBasicCprodAtt cprodAtt,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(cprodAtt.getId() != null && cprodAtt.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCprodAtt.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, cprodAtt);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(cprodAtt);
		beforeSaveCprodAtt(cprodAtt, viewIsView);

		if (viewIsView) {
			cprodAttDao.saveWithRevertVersion(cprodAtt);
		} else {
			if(isNew)
				cprodAttDao.save(cprodAtt);
			else
				cprodAttDao.merge(cprodAtt);
		}
		if(null != cprodAtt && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCprodAtt.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
			AuditUtil.setColumnStringA(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
		...
		dealDatagridsSave(cprodAtt,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateCprodAttCodes(cprodAtt, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateCprodAttSummarys(cprodAtt, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveCprodAtt(cprodAtt, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, cprodAtt);
			}
		}
		List<MESBasicCprodAtt> params = new ArrayList<MESBasicCprodAtt>();
		cprodAttDao.flush();
		cprodAttDao.clear();
		cprodAtt = cprodAttDao.load(cprodAtt.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicCprodAtt cprodAtt){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("cprodAtt", cprodAtt);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt")
	public void mergeCprodAtt(MESBasicCprodAtt cprodAtt, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(cprodAtt);
		beforeSaveCprodAtt(cprodAtt);
		cprodAttDao.merge(cprodAtt);
		if(null != cprodAtt && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCprodAtt.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
			AuditUtil.setColumnStringA(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
		...
		dealDatagridsSave(cprodAtt,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveCprodAtt(cprodAtt);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicCprodAtt> findCprodAtts(Page<MESBasicCprodAtt> page, Criterion... criterions) {
		return cprodAttDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCprodAtts(Page<MESBasicCprodAtt> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findCprodAtts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCprodAtts(Page<MESBasicCprodAtt> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCprodAtts(Page<MESBasicCprodAtt> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicCprodAtt> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"cprodAtt\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"cprodAtt\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"cprodAtt\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"cprodAtt\".PRODUCTID IN ( SELECT PRODUCT_ID FROM S2BASE_PRODUCT \"product\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  cprodAttDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  cprodAttDao.createNativeQuery(powerOperateSql, "MESBasic_1_product").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"cprodAtt\".LAY_REC ASC, \"cprodAtt\".SORT ASC");
				} else {
					orderPart.append(" \"cprodAtt\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) cprodAttDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=cprodAttDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = cprodAttDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicCprodAtt> result = new ArrayList<MESBasicCprodAtt>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicCprodAtt.class,cprodAttDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicCprodAtt.class,cprodAttDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicCprodAtt cprodAtt : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(cprodAtt.getId(), "MESBasic_product_cprodAtt");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(cprodAtt.getId(), "MESBasic_product_cprodAtt");
							if(null != documents && !documents.isEmpty()) {
								cprodAtt.setDocument(documents.get(0));
								cprodAtt.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicCprodAtt> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicCprodAtt> result) {
		View view = null;
		...
		Map<Long, MESBasicCprodAtt> rsMap = new HashMap<Long, MESBasicCprodAtt>();
		for (MESBasicCprodAtt cprodAtt : result) {
			ids.add(cprodAtt.getId());
			rsMap.put(cprodAtt.getId(), cprodAtt);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(cprodAttDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = cprodAttDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_product_CprodAtt" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_product_CprodAtt" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  cprodAttDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"cprodAtt\"","MESBasic_1_product","","MESBASIC_CPROD_ATTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"cprodAtt\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"cprodAtt\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"cprodAtt\"","MESBasic_1_product","","MESBASIC_CPROD_ATTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_product_CprodAtt" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("cprodAtt.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"cprodAtt\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("cprodAtt.mc\"").append(".CPROD_ATT) not in ("+notinValue+")");
			}
			
			SQLQuery query = cprodAttDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=cprodAttDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicCprodAtt.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicCprodAtt.MODULE_CODE;}
	protected String getModelCode(){return MESBasicCprodAtt.MODEL_CODE;}
	 ...
	 * @param cprodAtt 模型实例
	 */
	public void checkUniqueConstraint(MESBasicCprodAtt cprodAtt){
	}
	
	
	@Override
	public List<MESBasicCprodAtt> getCprodAtts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicCprodAtt.class);
		if(params!=null){
			...
			return cprodAttDao.findByHql("from " + MESBasicCprodAtt.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return cprodAttDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicCprodAtt> findCprodAttsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicCprodAtt.class);
	...
	@Override
	public List<MESBasicCprodAtt> findCprodAttsByHql(String hql, Object... objects){
		
		return   cprodAttDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicCprodAtt loadCprodAttByBussinessKey(MESBasicCprodAtt cprodAtt){
		return loadCprodAttByBussinessKey(cprodAtt.getId());
	}
	
	@Override
	public Page<MESBasicCprodAtt> getCprodAtts(Page<MESBasicCprodAtt> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicCprodAtt.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = cprodAttDao.findByPage(page, sql);
			}else{
				page = cprodAttDao.findAllByPage(page);
			}
			return page;
		}
		page = cprodAttDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicCprodAtt loadCprodAttByBussinessKey(Serializable bussinessKey){
		return cprodAttDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicCprodAtt> findByProperty(String propertyName, Object object){
		return cprodAttDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicCprodAtt findEntityByProperty(String propertyName, Object object){
		return cprodAttDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicCprodAtt.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = cprodAttDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_product_CprodAtt","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicCprodAtt cprodAtt,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,cprodAttDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicCprodAtt.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, cprodAtt);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",cprodAtt.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicCprodAtt.TABLE_NAME +" where valid = 1";
			list = cprodAttDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicCprodAtt.TABLE_NAME;
			list = cprodAttDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicCprodAtt.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = cprodAttDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicCprodAttDaoImpl cprodAttDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchCprodAtt(final List<MESBasicCprodAtt> insertObjs, final List<MESBasicCprodAtt> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchCprodAtt(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchCprodAtt(final List<MESBasicCprodAtt> insertObjs, final List<MESBasicCprodAtt> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1470295331697"),importCount));
		...
		Map<Long, MESBasicCprodAtt> oldUpdateObjsMap = new HashMap<Long, MESBasicCprodAtt>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicCprodAtt bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicCprodAtt> bizObjList = cprodAttDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicCprodAtt bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicCprodAtt> nmeObjs = new ArrayList<MESBasicCprodAtt>();
		...
		long id=autoGeneratorID.getNextId(MESBasicCprodAtt.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicCprodAtt cprodAtt:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicCprodAtt.TABLE_NAME,1,dbId);
			if(cprodAtt.getId() != null ){
				m.put(cprodAtt.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) cprodAttDao.createNativeQuery(sql, "MESBasic_1_product_CprodAtt"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		cprodAttDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicCprodAtt.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicCprodAtt cprodAtt = updateObjs.get(updateIndex);
						Long id = cprodAtt.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = cprodAtt.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicCprodAtt.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicCprodAtt.TABLE_NAME + " order by id desc";
//					Query query = cprodAttDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicCprodAtt cprodAtt = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = cprodAtt.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		cprodAttDaoImpl.getSession().flush();
		cprodAttDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicCprodAtt bizObj = insertObjs.get(insertIndex);
			cprodAttImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicCprodAtt bizObj = updateObjs.get(updateIndex);
			MESBasicCprodAtt oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			cprodAttImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicCprodAtt> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicCprodAtt> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) cprodAttDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicCprodAtt> result = getResult(page, query, new PendingResultTransformer(MESBasicCprodAtt.class,cprodAttDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicCprodAtt model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = cprodAttDao.createNativeQuery(sql, "MESBasic_1_product_CprodAtt").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = cprodAttDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = cprodAttDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = cprodAttDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?cprodAttDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  cprodAttDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,cprodAttDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="ec.flow.save")
	public void saveCprodAtt(MESBasicCprodAtt cprodAtt, DataGridService dataGridService){
		saveCprodAtt(cprodAtt, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="ec.flow.save")
	public void saveCprodAtt(MESBasicCprodAtt cprodAtt, DataGridService dataGridService, String viewCode){
		saveCprodAtt(cprodAtt, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt")
	public void saveCprodAtt(MESBasicCprodAtt cprodAtt, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveCprodAtt(cprodAtt, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt")
	public void saveCprodAtt(MESBasicCprodAtt cprodAtt, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(null != cprodAtt.getId() && cprodAtt.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCprodAtt.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt")
	public Map<Object, Object> cprodAttDataGridImport(MESBasicCprodAtt cprodAtt, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = cprodAtt.getVirtualId();
		
		cprodAtt = cprodAttDao.load(cprodAtt.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, cprodAtt.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = cprodAtt.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(cprodAtt);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, cprodAtt.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt")
	public void mergeCprodAtt(MESBasicCprodAtt cprodAtt, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(cprodAtt);
		beforeSaveCprodAtt(cprodAtt);
		cprodAttDao.merge(cprodAtt);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CprodAtt",desc="foundation.batch.save",operType="4")
	public void batchSaveCprodAtt(MESBasicCprodAtt cprodAtt, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
	...
	@Override
	public Page<MESBasicCprodAtt> getByPage(Page<MESBasicCprodAtt> page,DetachedCriteria detachedCriteria){
		return cprodAttDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveCprodAtt(MESBasicCprodAtt cprodAtt, Object... objects){
		checkUniqueConstraint(cprodAtt);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveCprodAtt(MESBasicCprodAtt cprodAtt, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportCprodAtt(List<MESBasicCprodAtt> cprodAtts, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportCprodAtt(List<MESBasicCprodAtt> cprodAtts){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportCprodAtt(List<MESBasicCprodAtt> cprodAtts, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportCprodAtt(List<MESBasicCprodAtt> cprodAtts){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteCprodAtt(MESBasicCprodAtt cprodAtt){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteCprodAtt(MESBasicCprodAtt cprodAtt){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreCprodAtt(MESBasicCprodAtt cprodAtt){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreCprodAtt(MESBasicCprodAtt cprodAtt){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicCprodAtt cprodAtt, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicCprodAtt cprodAtt, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportCprodAtt(List<MESBasicCprodAtt> insertObjs, List<MESBasicCprodAtt> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportCprodAtt,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportCprodAtt(List<MESBasicCprodAtt> insertObjs, List<MESBasicCprodAtt> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportCprodAtt,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
