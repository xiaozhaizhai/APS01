...
/* CUSTOM CODE START(action,import,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicWareAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicWare> {
	...
	//导出excel时,主模型内容
	List<MESBasicWare> queryResults = new ArrayList<MESBasicWare>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/wareMan/ware/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/wareMan/ware/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_wareMan_Ware" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicWare> wares;
	
	public List<MESBasicWare> getWares() {
		return wares;
	}
	...
	private Page<MESBasicWare> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			ware = new MESBasicWare();
			if(ware.getCargoStatue() != null) {
				ware.setCargoStatue((SystemCode) systemCodeService.getSystemCode(ware.getCargoStatue().getId()));
			}
			if(ware.getWareState() != null) {
				ware.setWareState((SystemCode) systemCodeService.getSystemCode(ware.getWareState().getId()));
			}
		} else {
			if(id > 0){
				ware = wareService.getWare(id, viewCode);
			}
		}
	...
	public Page<MESBasicWare> getPage() {
		if (null == page) {
			page = new Page<MESBasicWare>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/wareMan/ware/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = wareService.getWareAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/wareMan/ware/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String wareReferenceCopy() throws Exception{
			MESBasicWare oldWare = null;
			if(isEditNew()){
				...
				oldWare = new MESBasicWare();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,ware.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, ware.getId());
				...
				oldWare = wareService.getWare(id);
			}
			
			
			ware = wareService.getWare(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_wareMan_Ware");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicWare.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldWare, null);
						Method setValue = MESBasicWare.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(ware, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			ware.setCreateDepartmentId(oldWare.getCreateDepartmentId());
			ware.setCreatePositionId(oldWare.getCreatePositionId());
			ware.setCreateStaffId(oldWare.getCreateStaffId());
			ware.setOwnerDepartmentId(oldWare.getOwnerDepartmentId());
			ware.setOwnerPositionId(oldWare.getOwnerPositionId());
			ware.setOwnerStaffId(oldWare.getOwnerStaffId());
			ware.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_wareMan_Ware", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=wareService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return wareReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			ware = wareService.getWare(this.ware.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,ware.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/wareMan/ware/wareTreeOrderList", results = { @Result(type = FREEMARKER, location = "wareTreeOrderList.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/wareMan/ware/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = wareService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(wareService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(wareService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_wareMan_Ware", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/wareMan/ware/wareRef", results = { @Result(type = FREEMARKER, location = "wareRef.ftl") })
		,
		@Action(value = "/MESBasic/wareMan/ware/wareRefStore", results = { @Result(type = FREEMARKER, location = "wareRefStore.ftl") })
		,
		@Action(value = "/MESBasic/wareMan/ware/wareTreeOrderRef", results = { @Result(type = FREEMARKER, location = "wareTreeOrderRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"ware\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"ware\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"belongDept\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		if (null != request.getParameter("wareClass.layRec") && request.getParameter("wareClass.layRec").trim().length() > 0) {
			params.add(new Param("\"wareClass\".LAY_REC", request.getParameter("wareClass.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"ware\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"ware\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/wareMan/ware/wareRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.wareCode,result.wareName,result.wareAddress,result.storeKeeper.name,result.storeKeeper.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="wareRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/wareMan/ware/wareRefStore-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.wareCode,result.wareName,result.wareAddress,result.storeKeeper.name,result.isCargo,result.cargoStatue.id,result.cargoStatue.value,result.storeKeeper.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="wareRefStore-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/wareMan/ware/wareTreeOrderRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.wareCode,result.wareName,result.wareAddress,result.storeKeeper.name,result.wareClass.code,result.storeKeeper.id,result.wareClass.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="wareTreeOrderRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		wareService.findDataGridPage(dg,dgclass,getDgPage(),ware,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(ware.getId() != null && ware.getId() != -1){
				ware = wareService.getWare(ware.getId());
			}
			wareService.findDataGridPage(dg,dgclass,getDgPage(),ware,searchCondition,params);
	...
	private void save() throws Exception {
		...
		wareService.saveWare(ware,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "submit");
		bap_validate_datagrids=wareService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", ware.getId());
		...
		fileuploads.put("linkId", ware.getId());
		bapAfterCustomMethod("MESBasic_1_wareMan_Ware", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/wareMan/ware/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		wareService.deleteWare(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_wareMan_Ware", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/wareMan/ware/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "restore");
		wareService.restoreWare(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_wareMan_Ware", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/wareMan/ware/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/wareMan/ware/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "showDealInfoData");
		if (null != tableInfoId) {
			int count=wareService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = wareService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = wareService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_wareMan_Ware", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicWare> findExportDataInfos(Page<MESBasicWare> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = wareService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = wareService.getAssProperty(p.getCode());	
									//String assModelCode = wareService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			wareService.findWares(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			wareService.findWares(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicWare> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicWare result : results){
	...
	public boolean isEditNew(){
		//return null == this.ware || null == this.ware.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/wareMan/ware/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_wareMan_Ware", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/wareMan/ware/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/wareMan/ware/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/wareMan/ware/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicWareService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_wareMan", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_wareMan");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_wareMan_Ware", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/wareMan/ware/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicWareService"
						, "com.supcon.orchid.MESBasic.services.MESBasicWareClassService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_wareMan_Ware", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_wareMan_Ware", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/wareMan/ware/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/wareMan/ware/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_wareMan_Ware";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_wareMan_Ware", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/wareMan/ware/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == ware.getWareCode() ? "" : ware.getWareCode().toString());
		_auditLog.setColumnStringA(null == ware.getWareName() ? "" : ware.getWareName().toString());
		_auditLog.setColumnStringC(ware.getId().toString());
		_auditLog.setColumnStringD(MESBasicWare.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_wareMan");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicWareAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, ware,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, ware,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,ware);
		...
		printlog.setEntity("MESBasic_1_wareMan");
		...
		printlog.setBusinessId(ware.getId());
		printlog.setBussinessKey(null == ware.getWareCode() ? "" : ware.getWareCode().toString());
		printlog.setMainDisplay(null == ware.getWareName() ? "" : ware.getWareName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/wareMan/ware/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(ware,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/wareMan/ware/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(wareService.getWare(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/wareMan/ware/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/wareMan/ware/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, ware,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				ware = wareService.getWare(Long.parseLong(arrId[i]));
				if(null != ware){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == ware.getWareName() ? "" : "," + ware.getWareName().toString()));
					} else {
						_auditLog.setColumnStringA(null == ware.getWareName() ? "" : ware.getWareName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == ware.getWareCode() ? "" : "," + ware.getWareCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == ware.getWareCode() ? "" : ware.getWareCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == ware.getId() ? "" : "," + ware.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == ware.getId() ? "" : ware.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, ware,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,ware);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/wareMan/ware/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicWare> findExportPageInfos(Page<MESBasicWare> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_wareMan" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = wareService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicWare queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = wareService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = wareService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_wareMan_Ware");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_wareMan_Ware")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_wareMan_Ware");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_wareMan_Ware");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = wareService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
