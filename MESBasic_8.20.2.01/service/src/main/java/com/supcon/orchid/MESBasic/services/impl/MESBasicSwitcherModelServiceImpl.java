...
import com.supcon.orchid.MESBasic.services.MESBasicSwitcherModelImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicSwitcherModelDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_switcherModelService")
@Transactional
public class MESBasicSwitcherModelServiceImpl  extends BaseServiceImpl<MESBasicSwitcherModel> implements MESBasicSwitcherModelService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicSwitcherModel> bapBaseService;
	...
	@Autowired
	private MESBasicSwitcherModelImportService switcherModelImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicSwitcherModel.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  switcherModelDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicSwitcherModel.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = switcherModelDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicSwitcherModel.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicSwitcherModel.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  switcherModelDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicSwitcherModel.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicSwitcherModel.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicSwitcherModel> list = (List<MESBasicSwitcherModel>) switcherModelDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicSwitcherModel getSwitcherModel(long id){
		return getSwitcherModel(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicSwitcherModel getSwitcherModel(long id, String viewCode){
		MESBasicSwitcherModel switcherModel = switcherModelDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getSwitcherModelAsJSON(long id, String include){
		MESBasicSwitcherModel switcherModel = switcherModelDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(switcherModel, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.common.delete",operType="3")
	public void deleteSwitcherModel(MESBasicSwitcherModel switcherModel){
		if(null != switcherModel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicSwitcherModel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
			AuditUtil.setColumnStringA(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
			AuditUtil.setColumnStringC(switcherModel.getId().toString());
		}
		beforeDeleteSwitcherModel(switcherModel);
		switcherModelDao.deletePhysical(switcherModel);
		afterDeleteSwitcherModel(switcherModel);
		switcherModelDao.flush();
		List<MESBasicSwitcherModel> params = new ArrayList<MESBasicSwitcherModel>();
		params.add(switcherModel);
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.common.delete",operType="3")
	public void deleteSwitcherModel(List<Long> switcherModelIds){
		deleteSwitcherModel(switcherModelIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.common.delete",operType="3")
	public void deleteSwitcherModel(List<Long> switcherModelIds, String eventTopic) {
		List<MESBasicSwitcherModel> switcherModels = new ArrayList<MESBasicSwitcherModel>();
		for(Long switcherModelId : switcherModelIds){
			MESBasicSwitcherModel switcherModel = getSwitcherModel(switcherModelId);
			switcherModels.add(switcherModel);
			if(null != switcherModel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicSwitcherModel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == switcherModel.getId() ? "" : "," + switcherModel.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == switcherModel.getId() ? "" : "," + switcherModel.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == switcherModel.getId() ? "" : "," + switcherModel.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(switcherModels != null && switcherModels.size() > 0 ){
			for(MESBasicSwitcherModel switcherModel : switcherModels){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicSwitcherModel.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(switcherModel));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),switcherModel, switcherModel.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(switcherModelIds != null && switcherModelIds.size() > 0) {
			String hql = "delete " + MESBasicSwitcherModel.JPA_NAME + " where id in(:ids)";
			Query query = switcherModelDao.createQuery(hql);
			query.setParameterList("ids", switcherModelIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicSwitcherModel switcherModelz : switcherModels){
				switcherModelz.setValid(false);
				List<MESBasicSwitcherModel> params = new ArrayList<MESBasicSwitcherModel>();
				params.add(switcherModelz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_routine");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.common.delete",operType="3")
	public void deleteSwitcherModel(long switcherModelId, int switcherModelVersion){
		this.deleteSwitcherModel(switcherModelId, switcherModelVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.common.delete",operType="3")
	public void deleteSwitcherModel(long switcherModelId, int switcherModelVersion,SignatureLog signatureLog){
		MESBasicSwitcherModel switcherModel = getSwitcherModel(switcherModelId);
		...
		beforeDeleteSwitcherModel(switcherModel);
		switcherModelDao.delete(switcherModelId, switcherModelVersion);
		afterDeleteSwitcherModel(switcherModel);
		switcherModelDao.flush();
		List<MESBasicSwitcherModel> params = new ArrayList<MESBasicSwitcherModel>();
		params.add(switcherModel);
		...
		if(signatureLog != null ) {
			...
			businessKey=switcherModel.getId();
			signatureLog.setTableId(switcherModel.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.common.delete",operType="3")
	public void deleteSwitcherModel(String switcherModelIds){
		this.deleteSwitcherModel(switcherModelIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.common.delete",operType="3")
	public void deleteSwitcherModel(String switcherModelIds,SignatureLog signatureLog){
		deleteCollection(switcherModelIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteSwitcherModel(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_routine#modelCode:MESBasic_1_routine_SwitcherModel#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="ec.engine.restore",operType="10")
	public void restoreSwitcherModel(String switcherModelIds){
		restoreCollection(switcherModelIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreSwitcherModel(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="ec.engine.restore",operType="10")
	public void restoreSwitcherModel(long switcherModelId){
		
		findBusinessKeyUsed(switcherModelId);	//判断业务主键是否重复
		...
		beforeRestoreSwitcherModel(switcherModel);
		switcherModel.setValid(true);
		switcherModelDao.update(switcherModel);
		afterRestoreSwitcherModel(switcherModel);
		List<MESBasicSwitcherModel> params = new ArrayList<MESBasicSwitcherModel>();
		params.add(switcherModel);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long switcherModelId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_routine_SwitcherModel");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicSwitcherModel.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicSwitcherModel.TABLE_NAME +" where id = ? )";
			List<Object> list =  switcherModelDao.createNativeQuery(sql,switcherModelId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseSwitcherModel(List<MESBasicSwitcherModel>  switcherModels){
		for(MESBasicSwitcherModel switcherModel:switcherModels)  {
			saveSwitcherModel(switcherModel, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseSwitcherModel(List<MESBasicSwitcherModel>  switcherModels){
		for(MESBasicSwitcherModel switcherModel:switcherModels)  {
			saveSwitcherModel(switcherModel, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="ec.flow.save")
	public void saveSwitcherModel(MESBasicSwitcherModel switcherModel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveSwitcherModel(switcherModel, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="ec.flow.save")
	public void saveSwitcherModel(MESBasicSwitcherModel switcherModel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveSwitcherModel(switcherModel, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateSwitcherModelCodes(MESBasicSwitcherModel switcherModel) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSwitcherModelCodes(switcherModel, false);
	}
	
	public void generateSwitcherModelCodes(MESBasicSwitcherModel switcherModel, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateSwitcherModelSummarys(MESBasicSwitcherModel switcherModel) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSwitcherModelSummarys(switcherModel, false);
	}

	public void generateSwitcherModelSummarys(MESBasicSwitcherModel switcherModel, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel")
	public void saveSwitcherModel(MESBasicSwitcherModel switcherModel,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveSwitcherModel(switcherModel, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel")
	public void saveSwitcherModel(MESBasicSwitcherModel switcherModel,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_routine");
		String url = null;
		if(switcherModel.getId() != null && switcherModel.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicSwitcherModel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, switcherModel);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(switcherModel);
		beforeSaveSwitcherModel(switcherModel, viewIsView);

		if (viewIsView) {
			switcherModelDao.saveWithRevertVersion(switcherModel);
		} else {
			if(isNew)
				switcherModelDao.save(switcherModel);
			else
				switcherModelDao.merge(switcherModel);
		}
		if(null != switcherModel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicSwitcherModel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
			AuditUtil.setColumnStringA(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
		...
		dealDatagridsSave(switcherModel,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateSwitcherModelCodes(switcherModel, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateSwitcherModelSummarys(switcherModel, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveSwitcherModel(switcherModel, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, switcherModel);
			}
		}
		List<MESBasicSwitcherModel> params = new ArrayList<MESBasicSwitcherModel>();
		switcherModelDao.flush();
		switcherModelDao.clear();
		switcherModel = switcherModelDao.load(switcherModel.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicSwitcherModel switcherModel){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("switcherModel", switcherModel);
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel")
	public void mergeSwitcherModel(MESBasicSwitcherModel switcherModel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(switcherModel);
		beforeSaveSwitcherModel(switcherModel);
		switcherModelDao.merge(switcherModel);
		if(null != switcherModel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicSwitcherModel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
			AuditUtil.setColumnStringA(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
		...
		dealDatagridsSave(switcherModel,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveSwitcherModel(switcherModel);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicSwitcherModel> findSwitcherModels(Page<MESBasicSwitcherModel> page, Criterion... criterions) {
		return switcherModelDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSwitcherModels(Page<MESBasicSwitcherModel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findSwitcherModels(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSwitcherModels(Page<MESBasicSwitcherModel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSwitcherModels(Page<MESBasicSwitcherModel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicSwitcherModel> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"switcherModel\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"switcherModel\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"switcherModel\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"switcherModel\".CONFIG IN ( SELECT ID FROM MESBASIC_ROUTINES \"routine\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  switcherModelDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  switcherModelDao.createNativeQuery(powerOperateSql, "MESBasic_1_routine").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"switcherModel\".LAY_REC ASC, \"switcherModel\".SORT ASC");
				} else {
					orderPart.append(" \"switcherModel\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) switcherModelDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=switcherModelDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = switcherModelDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicSwitcherModel> result = new ArrayList<MESBasicSwitcherModel>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicSwitcherModel.class,switcherModelDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicSwitcherModel.class,switcherModelDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicSwitcherModel switcherModel : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(switcherModel.getId(), "MESBasic_routine_switcherModel");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(switcherModel.getId(), "MESBasic_routine_switcherModel");
							if(null != documents && !documents.isEmpty()) {
								switcherModel.setDocument(documents.get(0));
								switcherModel.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicSwitcherModel> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicSwitcherModel> result) {
		View view = null;
		...
		Map<Long, MESBasicSwitcherModel> rsMap = new HashMap<Long, MESBasicSwitcherModel>();
		for (MESBasicSwitcherModel switcherModel : result) {
			ids.add(switcherModel.getId());
			rsMap.put(switcherModel.getId(), switcherModel);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(switcherModelDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = switcherModelDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_routine_SwitcherModel" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_routine_SwitcherModel" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  switcherModelDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"switcherModel\"","MESBasic_1_routine","","MESBASIC_SWITCHER_MODELS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"switcherModel\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"switcherModel\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"switcherModel\"","MESBasic_1_routine","","MESBASIC_SWITCHER_MODELS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_routine_SwitcherModel" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("switcherModel.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"switcherModel\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"switcherModel\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"switcherModel\".CID = 1 OR \"switcherModel\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("switcherModel.mc\"").append(".SWITCHER_MODEL) not in ("+notinValue+")");
			}
			
			SQLQuery query = switcherModelDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=switcherModelDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicSwitcherModel.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicSwitcherModel.MODULE_CODE;}
	protected String getModelCode(){return MESBasicSwitcherModel.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param switcherModel 模型实例
	 */
	public void checkUniqueConstraint(MESBasicSwitcherModel switcherModel){
	}
	
	
	@Override
	public List<MESBasicSwitcherModel> getSwitcherModels(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicSwitcherModel.class);
		if(params!=null){
			...
			return switcherModelDao.findByHql("from " + MESBasicSwitcherModel.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return switcherModelDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicSwitcherModel> findSwitcherModelsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicSwitcherModel.class);
	...
	@Override
	public List<MESBasicSwitcherModel> findSwitcherModelsByHql(String hql, Object... objects){
		
		return   switcherModelDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicSwitcherModel loadSwitcherModelByBussinessKey(MESBasicSwitcherModel switcherModel){
		return loadSwitcherModelByBussinessKey(switcherModel.getId());
	}
	
	@Override
	public Page<MESBasicSwitcherModel> getSwitcherModels(Page<MESBasicSwitcherModel> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicSwitcherModel.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = switcherModelDao.findByPage(page, sql);
			}else{
				page = switcherModelDao.findAllByPage(page);
			}
			return page;
		}
		page = switcherModelDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicSwitcherModel> findByTreeDataGridRoutine(List<MESBasicSwitcherModel> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicSwitcherModel switcherModel : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridRoutine(Page<MESBasicSwitcherModel> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicSwitcherModel> switcherModels = dgPage.getResult();
			for(MESBasicSwitcherModel switcherModel : switcherModels){
				
			}
			dgPage.setResult(switcherModels);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicSwitcherModel loadSwitcherModelByBussinessKey(Serializable bussinessKey){
		return switcherModelDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicSwitcherModel> findByProperty(String propertyName, Object object){
		return switcherModelDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicSwitcherModel findEntityByProperty(String propertyName, Object object){
		return switcherModelDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicSwitcherModel.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = switcherModelDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_routine_SwitcherModel","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicSwitcherModel switcherModel,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,switcherModelDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicSwitcherModel.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, switcherModel);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",switcherModel.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicSwitcherModel.TABLE_NAME +" where valid = 1";
			list = switcherModelDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicSwitcherModel.TABLE_NAME;
			list = switcherModelDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicSwitcherModel.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = switcherModelDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicSwitcherModelDaoImpl switcherModelDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchSwitcherModel(final List<MESBasicSwitcherModel> insertObjs, final List<MESBasicSwitcherModel> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchSwitcherModel(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchSwitcherModel(final List<MESBasicSwitcherModel> insertObjs, final List<MESBasicSwitcherModel> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1416557727476"),importCount));
		...
		Map<Long, MESBasicSwitcherModel> oldUpdateObjsMap = new HashMap<Long, MESBasicSwitcherModel>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicSwitcherModel bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicSwitcherModel> bizObjList = switcherModelDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicSwitcherModel bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicSwitcherModel> nmeObjs = new ArrayList<MESBasicSwitcherModel>();
		...
		long id=autoGeneratorID.getNextId(MESBasicSwitcherModel.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicSwitcherModel switcherModel:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicSwitcherModel.TABLE_NAME,1,dbId);
			if(switcherModel.getId() != null ){
				m.put(switcherModel.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) switcherModelDao.createNativeQuery(sql, "MESBasic_1_routine_SwitcherModel"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		switcherModelDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicSwitcherModel.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicSwitcherModel switcherModel = updateObjs.get(updateIndex);
						Long id = switcherModel.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = switcherModel.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(switcherModel);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicSwitcherModel.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicSwitcherModel.TABLE_NAME + " order by id desc";
//					Query query = switcherModelDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicSwitcherModel switcherModel = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = switcherModel.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		switcherModelDaoImpl.getSession().flush();
		switcherModelDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicSwitcherModel bizObj = insertObjs.get(insertIndex);
			switcherModelImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicSwitcherModel bizObj = updateObjs.get(updateIndex);
			MESBasicSwitcherModel oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			switcherModelImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicSwitcherModel> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicSwitcherModel> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) switcherModelDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicSwitcherModel> result = getResult(page, query, new PendingResultTransformer(MESBasicSwitcherModel.class,switcherModelDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicSwitcherModel model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = switcherModelDao.createNativeQuery(sql, "MESBasic_1_routine_SwitcherModel").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = switcherModelDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = switcherModelDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = switcherModelDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?switcherModelDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  switcherModelDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,switcherModelDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="ec.flow.save")
	public void saveSwitcherModel(MESBasicSwitcherModel switcherModel, DataGridService dataGridService){
		saveSwitcherModel(switcherModel, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="ec.flow.save")
	public void saveSwitcherModel(MESBasicSwitcherModel switcherModel, DataGridService dataGridService, String viewCode){
		saveSwitcherModel(switcherModel, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel")
	public void saveSwitcherModel(MESBasicSwitcherModel switcherModel, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveSwitcherModel(switcherModel, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel")
	public void saveSwitcherModel(MESBasicSwitcherModel switcherModel, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
		String url = null;
		if(null != switcherModel.getId() && switcherModel.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicSwitcherModel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel")
	public Map<Object, Object> switcherModelDataGridImport(MESBasicSwitcherModel switcherModel, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = switcherModel.getVirtualId();
		
		switcherModel = switcherModelDao.load(switcherModel.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, switcherModel.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = switcherModel.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(switcherModel);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, switcherModel.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel")
	public void mergeSwitcherModel(MESBasicSwitcherModel switcherModel, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(switcherModel);
		beforeSaveSwitcherModel(switcherModel);
		switcherModelDao.merge(switcherModel);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_SwitcherModel",desc="foundation.batch.save",operType="4")
	public void batchSaveSwitcherModel(MESBasicSwitcherModel switcherModel, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
	...
	@Override
	public Page<MESBasicSwitcherModel> getByPage(Page<MESBasicSwitcherModel> page,DetachedCriteria detachedCriteria){
		return switcherModelDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveSwitcherModel(MESBasicSwitcherModel switcherModel, Object... objects){
		checkUniqueConstraint(switcherModel);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveSwitcherModel(MESBasicSwitcherModel switcherModel, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportSwitcherModel(List<MESBasicSwitcherModel> switcherModels, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportSwitcherModel(List<MESBasicSwitcherModel> switcherModels){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportSwitcherModel(List<MESBasicSwitcherModel> switcherModels, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportSwitcherModel(List<MESBasicSwitcherModel> switcherModels){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteSwitcherModel(MESBasicSwitcherModel switcherModel){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteSwitcherModel(MESBasicSwitcherModel switcherModel){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreSwitcherModel(MESBasicSwitcherModel switcherModel){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreSwitcherModel(MESBasicSwitcherModel switcherModel){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicSwitcherModel switcherModel, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicSwitcherModel switcherModel, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportSwitcherModel(List<MESBasicSwitcherModel> insertObjs, List<MESBasicSwitcherModel> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportSwitcherModel,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportSwitcherModel(List<MESBasicSwitcherModel> insertObjs, List<MESBasicSwitcherModel> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportSwitcherModel,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
