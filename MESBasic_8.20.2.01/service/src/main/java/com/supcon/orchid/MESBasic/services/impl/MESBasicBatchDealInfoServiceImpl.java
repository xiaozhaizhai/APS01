...
import com.supcon.orchid.MESBasic.services.MESBasicBatchDealInfoImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicBatchDealInfoDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_batchDealInfoService")
@Transactional
public class MESBasicBatchDealInfoServiceImpl  extends BaseServiceImpl<MESBasicBatchDealInfo> implements MESBasicBatchDealInfoService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicBatchDealInfo> bapBaseService;
	...
	@Autowired
	private MESBasicBatchDealInfoImportService batchDealInfoImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicBatchDealInfo.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchDealInfoDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicBatchDealInfo.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchDealInfoDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBatchDealInfo.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBatchDealInfo.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchDealInfoDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicBatchDealInfo.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicBatchDealInfo.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicBatchDealInfo> list = (List<MESBasicBatchDealInfo>) batchDealInfoDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchDealInfo getBatchDealInfo(long id){
		return getBatchDealInfo(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchDealInfo getBatchDealInfo(long id, String viewCode){
		MESBasicBatchDealInfo batchDealInfo = batchDealInfoDao.load(id);

		// 一对多情况处理
		if(batchDealInfo != null){
		}
		return batchDealInfo;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchDealInfoAsJSON(long id, String include){
		MESBasicBatchDealInfo batchDealInfo = batchDealInfoDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchDealInfo, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteBatchDealInfo(MESBasicBatchDealInfo batchDealInfo){
		if(null != batchDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
			AuditUtil.setColumnStringC(batchDealInfo.getId().toString());
		}
		beforeDeleteBatchDealInfo(batchDealInfo);
		batchDealInfoDao.deletePhysical(batchDealInfo);
		afterDeleteBatchDealInfo(batchDealInfo);
		batchDealInfoDao.flush();
		List<MESBasicBatchDealInfo> params = new ArrayList<MESBasicBatchDealInfo>();
		params.add(batchDealInfo);
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteBatchDealInfo(List<Long> batchDealInfoIds){
		deleteBatchDealInfo(batchDealInfoIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteBatchDealInfo(List<Long> batchDealInfoIds, String eventTopic) {
		List<MESBasicBatchDealInfo> batchDealInfos = new ArrayList<MESBasicBatchDealInfo>();
		for(Long batchDealInfoId : batchDealInfoIds){
			MESBasicBatchDealInfo batchDealInfo = getBatchDealInfo(batchDealInfoId);
			batchDealInfos.add(batchDealInfo);
			if(null != batchDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchDealInfo.getId() ? "" : "," + batchDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchDealInfo.getId() ? "" : "," + batchDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchDealInfo.getId() ? "" : "," + batchDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
				}
			}
		}
		
		for(MESBasicBatchDealInfo batchDealInfo : batchDealInfos){
			beforeDeleteBatchDealInfo(batchDealInfo);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchDealInfos != null && batchDealInfos.size() > 0 ){
			for(MESBasicBatchDealInfo batchDealInfo : batchDealInfos){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicBatchDealInfo.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchDealInfo));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchDealInfo, batchDealInfo.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchDealInfoIds != null && batchDealInfoIds.size() > 0) {
			String hql = "delete " + MESBasicBatchDealInfo.JPA_NAME + " where id in(:ids)";
			Query query = batchDealInfoDao.createQuery(hql);
			query.setParameterList("ids", batchDealInfoIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicBatchDealInfo batchDealInfoz : batchDealInfos){
				batchDealInfoz.setValid(false);
				List<MESBasicBatchDealInfo> params = new ArrayList<MESBasicBatchDealInfo>();
				params.add(batchDealInfoz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_batchInfo");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteBatchDealInfo(long batchDealInfoId, int batchDealInfoVersion){
		this.deleteBatchDealInfo(batchDealInfoId, batchDealInfoVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteBatchDealInfo(long batchDealInfoId, int batchDealInfoVersion,SignatureLog signatureLog){
		MESBasicBatchDealInfo batchDealInfo = getBatchDealInfo(batchDealInfoId);
		if(batchDealInfo != null && !batchDealInfo.isValid()){
		...
		beforeDeleteBatchDealInfo(batchDealInfo);
		batchDealInfoDao.delete(batchDealInfoId, batchDealInfoVersion);
		afterDeleteBatchDealInfo(batchDealInfo);
		batchDealInfoDao.flush();
		List<MESBasicBatchDealInfo> params = new ArrayList<MESBasicBatchDealInfo>();
		params.add(batchDealInfo);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchDealInfo.getId();
			signatureLog.setTableId(batchDealInfo.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteBatchDealInfo(String batchDealInfoIds){
		this.deleteBatchDealInfo(batchDealInfoIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteBatchDealInfo(String batchDealInfoIds,SignatureLog signatureLog){
		deleteCollection(batchDealInfoIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchDealInfo(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_batchInfo#modelCode:MESBasic_1_batchInfo_BatchDealInfo#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="ec.engine.restore",operType="10")
	public void restoreBatchDealInfo(String batchDealInfoIds){
		restoreCollection(batchDealInfoIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchDealInfo(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="ec.engine.restore",operType="10")
	public void restoreBatchDealInfo(long batchDealInfoId){
		
		findBusinessKeyUsed(batchDealInfoId);	//判断业务主键是否重复
		...
		beforeRestoreBatchDealInfo(batchDealInfo);
		batchDealInfo.setValid(true);
		batchDealInfoDao.update(batchDealInfo);
		afterRestoreBatchDealInfo(batchDealInfo);
		List<MESBasicBatchDealInfo> params = new ArrayList<MESBasicBatchDealInfo>();
		params.add(batchDealInfo);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchDealInfoId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_batchInfo_BatchDealInfo");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicBatchDealInfo.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicBatchDealInfo.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchDealInfoDao.createNativeQuery(sql,batchDealInfoId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchDealInfo(List<MESBasicBatchDealInfo>  batchDealInfos){
		for(MESBasicBatchDealInfo batchDealInfo:batchDealInfos)  {
			saveBatchDealInfo(batchDealInfo, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchDealInfo(List<MESBasicBatchDealInfo>  batchDealInfos){
		for(MESBasicBatchDealInfo batchDealInfo:batchDealInfos)  {
			saveBatchDealInfo(batchDealInfo, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="ec.flow.save")
	public void saveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchDealInfo(batchDealInfo, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="ec.flow.save")
	public void saveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchDealInfo(batchDealInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchDealInfoCodes(MESBasicBatchDealInfo batchDealInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchDealInfoCodes(batchDealInfo, false);
	}
	
	public void generateBatchDealInfoCodes(MESBasicBatchDealInfo batchDealInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchDealInfoSummarys(MESBasicBatchDealInfo batchDealInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchDealInfoSummarys(batchDealInfo, false);
	}

	public void generateBatchDealInfoSummarys(MESBasicBatchDealInfo batchDealInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo")
	public void saveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchDealInfo(batchDealInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo")
	public void saveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_batchInfo");
		String url = null;
		if(batchDealInfo.getId() != null && batchDealInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchDealInfo);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchDealInfo);
		beforeSaveBatchDealInfo(batchDealInfo, viewIsView);

		if (viewIsView) {
			batchDealInfoDao.saveWithRevertVersion(batchDealInfo);
		} else {
			if(isNew)
				batchDealInfoDao.save(batchDealInfo);
			else
				batchDealInfoDao.merge(batchDealInfo);
		}
		if(null != batchDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
		...
		dealDatagridsSave(batchDealInfo,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchDealInfoCodes(batchDealInfo, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchDealInfoSummarys(batchDealInfo, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchDealInfo(batchDealInfo, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchDealInfo);
			}
		}
		List<MESBasicBatchDealInfo> params = new ArrayList<MESBasicBatchDealInfo>();
		batchDealInfoDao.flush();
		batchDealInfoDao.clear();
		batchDealInfo = batchDealInfoDao.load(batchDealInfo.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicBatchDealInfo batchDealInfo){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchDealInfo", batchDealInfo);
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo")
	public void mergeBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchDealInfo);
		beforeSaveBatchDealInfo(batchDealInfo);
		batchDealInfoDao.merge(batchDealInfo);
		if(null != batchDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
		...
		dealDatagridsSave(batchDealInfo,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchDealInfo(batchDealInfo);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicBatchDealInfo> findBatchDealInfos(Page<MESBasicBatchDealInfo> page, Criterion... criterions) {
		return batchDealInfoDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchDealInfos(Page<MESBasicBatchDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchDealInfos(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchDealInfos(Page<MESBasicBatchDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchDealInfos(Page<MESBasicBatchDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicBatchDealInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchDealInfo\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchDealInfo\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchDealInfo\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"batchDealInfo\".BATCH_INFOID IN ( SELECT ID FROM BATCH_INFORMATION \"batchInformation\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  batchDealInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchDealInfoDao.createNativeQuery(powerOperateSql, "MESBasic_1_batchInfo").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchInformation\"","MESBasic_1_batchInfo","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchDealInfo\".LAY_REC ASC, \"batchDealInfo\".SORT ASC");
				} else {
					orderPart.append(" \"batchDealInfo\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchDealInfoDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchDealInfoDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchDealInfoDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicBatchDealInfo> result = new ArrayList<MESBasicBatchDealInfo>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicBatchDealInfo.class,batchDealInfoDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicBatchDealInfo.class,batchDealInfoDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicBatchDealInfo batchDealInfo : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchDealInfo.getId(), "MESBasic_batchInfo_batchDealInfo");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchDealInfo.getId(), "MESBasic_batchInfo_batchDealInfo");
							if(null != documents && !documents.isEmpty()) {
								batchDealInfo.setDocument(documents.get(0));
								batchDealInfo.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicBatchDealInfo> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicBatchDealInfo> result) {
		View view = null;
		...
		Map<Long, MESBasicBatchDealInfo> rsMap = new HashMap<Long, MESBasicBatchDealInfo>();
		for (MESBasicBatchDealInfo batchDealInfo : result) {
			ids.add(batchDealInfo.getId());
			rsMap.put(batchDealInfo.getId(), batchDealInfo);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchDealInfoDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchDealInfoDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_batchInfo_BatchDealInfo" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_batchInfo_BatchDealInfo" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchDealInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchDealInfo\"","MESBasic_1_batchInfo","","BATCH_DEALINFO",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchDealInfo\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchDealInfo\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchDealInfo\"","MESBasic_1_batchInfo","","BATCH_DEALINFO",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_batchInfo_BatchDealInfo" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchDealInfo.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchDealInfo\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchDealInfo\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchDealInfo\".CID = 1 OR \"batchDealInfo\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchDealInfo.mc\"").append(".BATCH_DEAL_INFO) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchDealInfoDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchDealInfoDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicBatchDealInfo.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicBatchDealInfo.MODULE_CODE;}
	protected String getModelCode(){return MESBasicBatchDealInfo.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchDealInfo 模型实例
	 */
	public void checkUniqueConstraint(MESBasicBatchDealInfo batchDealInfo){
	}
	
	
	@Override
	public List<MESBasicBatchDealInfo> getBatchDealInfos(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchDealInfo.class);
		if(params!=null){
			...
			return batchDealInfoDao.findByHql("from " + MESBasicBatchDealInfo.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchDealInfoDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicBatchDealInfo> findBatchDealInfosBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchDealInfo.class);
	...
	@Override
	public List<MESBasicBatchDealInfo> findBatchDealInfosByHql(String hql, Object... objects){
		
		return   batchDealInfoDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicBatchDealInfo loadBatchDealInfoByBussinessKey(MESBasicBatchDealInfo batchDealInfo){
		return loadBatchDealInfoByBussinessKey(batchDealInfo.getId());
	}
	
	@Override
	public Page<MESBasicBatchDealInfo> getBatchDealInfos(Page<MESBasicBatchDealInfo> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchDealInfo.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchDealInfoDao.findByPage(page, sql);
			}else{
				page = batchDealInfoDao.findAllByPage(page);
			}
			return page;
		}
		page = batchDealInfoDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchDealInfo loadBatchDealInfoByBussinessKey(Serializable bussinessKey){
		return batchDealInfoDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicBatchDealInfo> findByProperty(String propertyName, Object object){
		return batchDealInfoDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchDealInfo findEntityByProperty(String propertyName, Object object){
		return batchDealInfoDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicBatchDealInfo.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchDealInfoDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_batchInfo_BatchDealInfo","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicBatchDealInfo batchDealInfo,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchDealInfoDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicBatchDealInfo.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchDealInfo);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchDealInfo.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBatchDealInfo.TABLE_NAME +" where valid = 1";
			list = batchDealInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBatchDealInfo.TABLE_NAME;
			list = batchDealInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicBatchDealInfo.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchDealInfoDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicBatchDealInfoDaoImpl batchDealInfoDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchDealInfo(final List<MESBasicBatchDealInfo> insertObjs, final List<MESBasicBatchDealInfo> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchDealInfo(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchDealInfo(final List<MESBasicBatchDealInfo> insertObjs, final List<MESBasicBatchDealInfo> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1490774952864"),importCount));
		...
		Map<Long, MESBasicBatchDealInfo> oldUpdateObjsMap = new HashMap<Long, MESBasicBatchDealInfo>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicBatchDealInfo bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicBatchDealInfo> bizObjList = batchDealInfoDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicBatchDealInfo bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicBatchDealInfo> nmeObjs = new ArrayList<MESBasicBatchDealInfo>();
		...
		long id=autoGeneratorID.getNextId(MESBasicBatchDealInfo.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicBatchDealInfo batchDealInfo:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicBatchDealInfo.TABLE_NAME,1,dbId);
			if(batchDealInfo.getId() != null ){
				m.put(batchDealInfo.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			batchDealInfo.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchDealInfoDao.createNativeQuery(sql, "MESBasic_1_batchInfo_BatchDealInfo"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchDealInfoDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicBatchDealInfo.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicBatchDealInfo batchDealInfo = updateObjs.get(updateIndex);
						Long id = batchDealInfo.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchDealInfo.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchDealInfo);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicBatchDealInfo.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicBatchDealInfo.TABLE_NAME + " order by id desc";
//					Query query = batchDealInfoDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicBatchDealInfo batchDealInfo = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchDealInfo.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchDealInfoDaoImpl.getSession().flush();
		batchDealInfoDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicBatchDealInfo bizObj = insertObjs.get(insertIndex);
			batchDealInfoImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicBatchDealInfo bizObj = updateObjs.get(updateIndex);
			MESBasicBatchDealInfo oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchDealInfoImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicBatchDealInfo> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicBatchDealInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchDealInfoDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicBatchDealInfo> result = getResult(page, query, new PendingResultTransformer(MESBasicBatchDealInfo.class,batchDealInfoDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicBatchDealInfo model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchDealInfoDao.createNativeQuery(sql, "MESBasic_1_batchInfo_BatchDealInfo").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchDealInfoDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchDealInfoDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchDealInfoDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="ec.flow.save")
	public void saveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, DataGridService dataGridService){
		saveBatchDealInfo(batchDealInfo, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="ec.flow.save")
	public void saveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, DataGridService dataGridService, String viewCode){
		saveBatchDealInfo(batchDealInfo, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo")
	public void saveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchDealInfo(batchDealInfo, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo")
	public void saveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_batchInfo";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchInfo");
		String url = null;
		if(null != batchDealInfo.getId() && batchDealInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo")
	public Map<Object, Object> batchDealInfoDataGridImport(MESBasicBatchDealInfo batchDealInfo, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchDealInfo.getVirtualId();
		
		batchDealInfo = batchDealInfoDao.load(batchDealInfo.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchDealInfo.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchDealInfo.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchDealInfo);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchDealInfo.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo")
	public void mergeBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchDealInfo);
		beforeSaveBatchDealInfo(batchDealInfo);
		batchDealInfoDao.merge(batchDealInfo);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchDealInfo",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_batchInfo";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchInfo");
	...
	@Override
	public Page<MESBasicBatchDealInfo> getByPage(Page<MESBasicBatchDealInfo> page,DetachedCriteria detachedCriteria){
		return batchDealInfoDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, Object... objects){
		checkUniqueConstraint(batchDealInfo);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchDealInfo(MESBasicBatchDealInfo batchDealInfo, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchDealInfo(List<MESBasicBatchDealInfo> batchDealInfos, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchDealInfo(List<MESBasicBatchDealInfo> batchDealInfos){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchDealInfo(List<MESBasicBatchDealInfo> batchDealInfos, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchDealInfo(List<MESBasicBatchDealInfo> batchDealInfos){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchDealInfo(MESBasicBatchDealInfo batchDealInfo){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchDealInfo(MESBasicBatchDealInfo batchDealInfo){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchDealInfo(MESBasicBatchDealInfo batchDealInfo){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchDealInfo(MESBasicBatchDealInfo batchDealInfo){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicBatchDealInfo batchDealInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicBatchDealInfo batchDealInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchDealInfo(List<MESBasicBatchDealInfo> insertObjs, List<MESBasicBatchDealInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchDealInfo,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchDealInfo(List<MESBasicBatchDealInfo> insertObjs, List<MESBasicBatchDealInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchDealInfo,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
