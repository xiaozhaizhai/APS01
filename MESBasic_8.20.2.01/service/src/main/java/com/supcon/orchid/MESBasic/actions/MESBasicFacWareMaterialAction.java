...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicFacWareMaterialAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicFacWareMaterial> {
	...
	private MESBasicFacWareMaterial facWareMaterial;
	...
	//导出excel时,主模型内容
	List<MESBasicFacWareMaterial> queryResults = new ArrayList<MESBasicFacWareMaterial>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryWare/facWareMaterial/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryWare/facWareMaterial/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryWare_FacWareMaterial" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicFacWareMaterial> facWareMaterials;
	
	public List<MESBasicFacWareMaterial> getFacWareMaterials() {
		return facWareMaterials;
	}
	...
	private Page<MESBasicFacWareMaterial> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			facWareMaterial = new MESBasicFacWareMaterial();
		} else {
			if(id > 0){
				facWareMaterial = facWareMaterialService.getFacWareMaterial(id, viewCode);
			}
		}
	...
	public Page<MESBasicFacWareMaterial> getPage() {
		if (null == page) {
			page = new Page<MESBasicFacWareMaterial>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicFacWareMaterial getFacWareMaterial() {
		return facWareMaterial;
	}		

	public void setFacWareMaterial(MESBasicFacWareMaterial facWareMaterial) {
		this.facWareMaterial = facWareMaterial;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = facWareMaterialService.getFacWareMaterialAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1489732341394
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String facWareMaterialReferenceCopy() throws Exception{
			MESBasicFacWareMaterial oldFacWareMaterial = null;
			if(isEditNew()){
				...
				oldFacWareMaterial = new MESBasicFacWareMaterial();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,facWareMaterial.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, facWareMaterial.getId());
				...
				oldFacWareMaterial = facWareMaterialService.getFacWareMaterial(id);
			}
			
			
			facWareMaterial = facWareMaterialService.getFacWareMaterial(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryWare_FacWareMaterial");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicFacWareMaterial.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFacWareMaterial, null);
						Method setValue = MESBasicFacWareMaterial.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(facWareMaterial, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			facWareMaterial.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=facWareMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return facWareMaterialReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			facWareMaterial = facWareMaterialService.getFacWareMaterial(this.facWareMaterial.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,facWareMaterial.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			facWareMaterialService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = facWareMaterialService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(facWareMaterialService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(facWareMaterialService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryWare_FacWareMaterial", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"facWareMaterial\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"facWareMaterial\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"facWareMaterial\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"facWareMaterial\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			facWareMaterialService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		facWareMaterialService.findDataGridPage(dg,dgclass,getDgPage(),facWareMaterial,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(facWareMaterial.getId() != null && facWareMaterial.getId() != -1){
				facWareMaterial = facWareMaterialService.getFacWareMaterial(facWareMaterial.getId());
			}
			facWareMaterialService.findDataGridPage(dg,dgclass,getDgPage(),facWareMaterial,searchCondition,params);
	...
	private void save() throws Exception {
		...
		facWareMaterialService.saveFacWareMaterial(facWareMaterial,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "submit");
		bap_validate_datagrids=facWareMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", facWareMaterial.getId());
		...
		fileuploads.put("linkId", facWareMaterial.getId());
		bapAfterCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryWare/facWareMaterial/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		facWareMaterialService.deleteFacWareMaterial(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "restore");
		facWareMaterialService.restoreFacWareMaterial(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicFacWareMaterial> findExportDataInfos(Page<MESBasicFacWareMaterial> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = facWareMaterialService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = facWareMaterialService.getAssProperty(p.getCode());	
									//String assModelCode = facWareMaterialService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			facWareMaterialService.findFacWareMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			facWareMaterialService.findFacWareMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicFacWareMaterial> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicFacWareMaterial result : results){
	...
	public boolean isEditNew(){
		//return null == this.facWareMaterial || null == this.facWareMaterial.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryWare/facWareMaterial/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryWare/facWareMaterial/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicFacWareMaterialService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicFacWareMaterialService"
						, "com.supcon.orchid.MESBasic.services.MESBasicFactoryModelService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicDepartmentWareService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryWare_FacWareMaterial", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryWare_FacWareMaterial", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryWare_FacWareMaterial";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryWare_FacWareMaterial", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicFacWareMaterial facWareMaterial = null;
		if(null != id) {
			facWareMaterial = facWareMaterialService.getFacWareMaterial(id);
		}
		...
		_auditLog.setColumnStringB(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
		_auditLog.setColumnStringA(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
		_auditLog.setColumnStringC(facWareMaterial.getId().toString());
		_auditLog.setColumnStringD(MESBasicFacWareMaterial.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicFacWareMaterialAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, facWareMaterial,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, facWareMaterial,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,facWareMaterial);
		...
		printlog.setBusinessId(facWareMaterial.getId());
		printlog.setBussinessKey(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
		printlog.setMainDisplay(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(facWareMaterial,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(facWareMaterialService.getFacWareMaterial(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, facWareMaterial,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				facWareMaterial = facWareMaterialService.getFacWareMaterial(Long.parseLong(arrId[i]));
				if(null != facWareMaterial){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == facWareMaterial.getId() ? "" : "," + facWareMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == facWareMaterial.getId() ? "" : "," + facWareMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == facWareMaterial.getId() ? "" : "," + facWareMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, facWareMaterial,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,facWareMaterial);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//facWareMaterialService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/factoryWare/facWareMaterial/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryWare_FacWareMaterial");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryWare_FacWareMaterial")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryWare_FacWareMaterial");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryWare_FacWareMaterial");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = facWareMaterialService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
