...
@Service("mESBasicBaseCustomerClassImportService")
public class MESBasicBaseCustomerClassImportServiceImpl implements MESBasicBaseCustomerClassImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomerClass",model="MESBasic_1_baseCustomerClass_BaseCustomerClass",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicBaseCustomerClass bizObj, MESBasicBaseCustomerClass oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1449628164766") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getCcName() ? "" : "," + oldBizObj.getCcName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getCcName() ? "" : oldBizObj.getCcName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getCcCode() ? "" : "," + oldBizObj.getCcCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getCcCode() ? "" : oldBizObj.getCcCode().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1449628164766") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getCcName() ? "" : "," + bizObj.getCcName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getCcName() ? "" : bizObj.getCcName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getCcCode() ? "" : "," + bizObj.getCcCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getCcCode() ? "" : bizObj.getCcCode().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicBaseCustomerClass bizObj, String bussinesskeyFieldName,
