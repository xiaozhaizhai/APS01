...
@Repository
public class MESBasicEamTypeDaoImpl extends TreeDaoImpl<MESBasicEamType, Long> implements MESBasicEamTypeDao {
	@Override
	...
	@Override
	public MESBasicEamTypeDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicEamTypeDealInfo dealInfo = new MESBasicEamTypeDealInfo();
	...
	@Override
	public MESBasicEamTypeDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicEamType mainObj, Staff dealStaff) {
		MESBasicEamTypeDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicEamTypeDealInfo.class);
		...
		List<MESBasicEamTypeDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
