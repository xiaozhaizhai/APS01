...
import com.supcon.orchid.MESBasic.services.MESBasicProdAttributeImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicProdAttributeDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_prodAttributeService")
@Transactional
public class MESBasicProdAttributeServiceImpl  extends BaseServiceImpl<MESBasicProdAttribute> implements MESBasicProdAttributeService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicProdAttribute> bapBaseService;
	...
	@Autowired
	private MESBasicProdAttributeImportService prodAttributeImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicProdAttribute.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  prodAttributeDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicProdAttribute.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = prodAttributeDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdAttribute.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdAttribute.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  prodAttributeDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicProdAttribute.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicProdAttribute.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicProdAttribute> list = (List<MESBasicProdAttribute>) prodAttributeDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicProdAttribute getProdAttribute(long id){
		return getProdAttribute(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdAttribute getProdAttribute(long id, String viewCode){
		MESBasicProdAttribute prodAttribute = prodAttributeDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProdAttributeAsJSON(long id, String include){
		MESBasicProdAttribute prodAttribute = prodAttributeDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(prodAttribute, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.common.delete",operType="3")
	public void deleteProdAttribute(MESBasicProdAttribute prodAttribute){
		if(null != prodAttribute && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdAttribute.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodAttribute.getCode() ? "" : prodAttribute.getCode().toString());
			AuditUtil.setColumnStringA(null == prodAttribute.getName() ? "" : prodAttribute.getName().toString());
			AuditUtil.setColumnStringC(prodAttribute.getId().toString());
		}
		checkDeletePermit(prodAttribute.getId());
		beforeDeleteProdAttribute(prodAttribute);
		prodAttributeDao.delete(prodAttribute);
		afterDeleteProdAttribute(prodAttribute);
		prodAttributeDao.flush();
		List<MESBasicProdAttribute> params = new ArrayList<MESBasicProdAttribute>();
		params.add(prodAttribute);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_prodAttribute");
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.common.delete",operType="3")
	public void deleteProdAttribute(List<Long> prodAttributeIds){
		deleteProdAttribute(prodAttributeIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.common.delete",operType="3")
	public void deleteProdAttribute(List<Long> prodAttributeIds, String eventTopic) {
		List<MESBasicProdAttribute> prodAttributes = new ArrayList<MESBasicProdAttribute>();
		for(Long prodAttributeId : prodAttributeIds){
			MESBasicProdAttribute prodAttribute = getProdAttribute(prodAttributeId);
			prodAttributes.add(prodAttribute);
			if(null != prodAttribute && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdAttribute.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == prodAttribute.getName() ? "" : "," + prodAttribute.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == prodAttribute.getName() ? "" : prodAttribute.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == prodAttribute.getCode() ? "" : "," + prodAttribute.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == prodAttribute.getCode() ? "" : prodAttribute.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == prodAttribute.getId() ? "" : "," + prodAttribute.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == prodAttribute.getId() ? "" : prodAttribute.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(prodAttributes != null && prodAttributes.size() > 0 ){
			for(MESBasicProdAttribute prodAttribute : prodAttributes){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),prodAttribute, prodAttribute.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(prodAttributeIds != null && prodAttributeIds.size() > 0) {
			String hql = "update " + MESBasicProdAttribute.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = prodAttributeDao.createQuery(hql);
			query.setParameterList("ids", prodAttributeIds);
			query.executeUpdate();
			prodAttributeDao.clearAllCacheByEntity();
		}
		...
		if(eventTopic==null){
			for(MESBasicProdAttribute prodAttributez : prodAttributes){
				prodAttributez.setValid(false);
				List<MESBasicProdAttribute> params = new ArrayList<MESBasicProdAttribute>();
				params.add(prodAttributez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_prodAttribute");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.common.delete",operType="3")
	public void deleteProdAttribute(long prodAttributeId, int prodAttributeVersion){
		this.deleteProdAttribute(prodAttributeId, prodAttributeVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.common.delete",operType="3")
	public void deleteProdAttribute(long prodAttributeId, int prodAttributeVersion,SignatureLog signatureLog){
		MESBasicProdAttribute prodAttribute = getProdAttribute(prodAttributeId);
		if(prodAttribute != null && !prodAttribute.isValid()){
		...
		checkDeletePermit(prodAttributeId);
		beforeDeleteProdAttribute(prodAttribute);
		prodAttributeDao.delete(prodAttributeId, prodAttributeVersion);
		afterDeleteProdAttribute(prodAttribute);
		prodAttributeDao.flush();
		List<MESBasicProdAttribute> params = new ArrayList<MESBasicProdAttribute>();
		params.add(prodAttribute);
		...
		if(signatureLog != null ) {
			...
			businessKey=prodAttribute.getCode();
			signatureLog.setTableId(prodAttribute.getId());
			if(businessKey != null) {
	...
	public void checkDeletePermit(Long id) throws BAPException{
		String prodAttributeHql = "select count(prodAttribute.id) as totalCoual from " + MESBasicProdAttribute.JPA_NAME + " as prodAttribute where prodAttribute.parentId=? and prodAttribute.valid=true";
		List<Long> prodAttributeCount = prodAttributeDao.findByHql(prodAttributeHql, id);
		if (prodAttributeCount.get(0) > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.common.delete",operType="3")
	public void deleteProdAttribute(String prodAttributeIds){
		this.deleteProdAttribute(prodAttributeIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.common.delete",operType="3")
	public void deleteProdAttribute(String prodAttributeIds,SignatureLog signatureLog){
		deleteCollection(prodAttributeIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProdAttribute(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_prodAttribute#modelCode:MESBasic_1_prodAttribute_ProdAttribute#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="ec.engine.restore",operType="10")
	public void restoreProdAttribute(String prodAttributeIds){
		restoreCollection(prodAttributeIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProdAttribute(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="ec.engine.restore",operType="10")
	public void restoreProdAttribute(long prodAttributeId){
		
		findBusinessKeyUsed(prodAttributeId);	//判断业务主键是否重复
		...
		checkDeletePermit(prodAttributeId);
		beforeRestoreProdAttribute(prodAttribute);
		prodAttribute.setValid(true);
		prodAttributeDao.update(prodAttribute);
		afterRestoreProdAttribute(prodAttribute);
		List<MESBasicProdAttribute> params = new ArrayList<MESBasicProdAttribute>();
		params.add(prodAttribute);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long prodAttributeId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_prodAttribute_ProdAttribute");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicProdAttribute.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicProdAttribute.TABLE_NAME +" where id = ? )";
			List<Object> list =  prodAttributeDao.createNativeQuery(sql,prodAttributeId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProdAttribute(List<MESBasicProdAttribute>  prodAttributes){
		for(MESBasicProdAttribute prodAttribute:prodAttributes)  {
			saveProdAttribute(prodAttribute, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProdAttribute(List<MESBasicProdAttribute>  prodAttributes){
		for(MESBasicProdAttribute prodAttribute:prodAttributes)  {
			saveProdAttribute(prodAttribute, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="ec.flow.save")
	public void saveProdAttribute(MESBasicProdAttribute prodAttribute, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProdAttribute(prodAttribute, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="ec.flow.save")
	public void saveProdAttribute(MESBasicProdAttribute prodAttribute, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProdAttribute(prodAttribute, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProdAttributeCodes(MESBasicProdAttribute prodAttribute) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdAttributeCodes(prodAttribute, false);
	}
	
	public void generateProdAttributeCodes(MESBasicProdAttribute prodAttribute, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProdAttributeSummarys(MESBasicProdAttribute prodAttribute) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdAttributeSummarys(prodAttribute, false);
	}

	public void generateProdAttributeSummarys(MESBasicProdAttribute prodAttribute, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute")
	public void saveProdAttribute(MESBasicProdAttribute prodAttribute,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProdAttribute(prodAttribute, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute")
	public void saveProdAttribute(MESBasicProdAttribute prodAttribute,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_prodAttribute";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_prodAttribute");
		String url = null;
		if(prodAttribute.getId() != null && prodAttribute.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdAttribute.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_prodAttribute/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdAttribute.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_prodAttribute/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, prodAttribute);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(prodAttribute);
		beforeSaveProdAttribute(prodAttribute, viewIsView);

		if (viewIsView) {
			prodAttributeDao.saveWithRevertVersion(prodAttribute);
		} else {
			prodAttributeDao.save(prodAttribute);
		}
		if(null != prodAttribute && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdAttribute.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodAttribute.getCode() ? "" : prodAttribute.getCode().toString());
			AuditUtil.setColumnStringA(null == prodAttribute.getName() ? "" : prodAttribute.getName().toString());
		...
		dealDatagridsSave(prodAttribute,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProdAttributeCodes(prodAttribute, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProdAttributeSummarys(prodAttribute, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProdAttribute(prodAttribute, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, prodAttribute);
			}
		}
		List<MESBasicProdAttribute> params = new ArrayList<MESBasicProdAttribute>();
		prodAttributeDao.flush();
		prodAttributeDao.clear();
		prodAttribute = prodAttributeDao.load(prodAttribute.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicProdAttribute prodAttribute){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("prodAttribute", prodAttribute);
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute")
	public void mergeProdAttribute(MESBasicProdAttribute prodAttribute, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(prodAttribute);
		beforeSaveProdAttribute(prodAttribute);
		prodAttributeDao.merge(prodAttribute);
		if(null != prodAttribute && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdAttribute.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodAttribute.getCode() ? "" : prodAttribute.getCode().toString());
			AuditUtil.setColumnStringA(null == prodAttribute.getName() ? "" : prodAttribute.getName().toString());
			AuditUtil.setColumnStringC(prodAttribute.getId().toString());
		}
		dealDatagridsSave(prodAttribute,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProdAttribute(prodAttribute);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicProdAttribute> findProdAttributes(Page<MESBasicProdAttribute> page, Criterion... criterions) {
		return prodAttributeDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicProdAttributeDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicProdAttributeDealInfo> list=prodAttributeDao.createQuery("from " + MESBasicProdAttributeDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	public void saveDealInfo(MESBasicProdAttributeDealInfo dealInfo) {
		prodAttributeDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicProdAttributeDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) prodAttributeDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicProdAttributeDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = prodAttributeDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = prodAttributeDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicProdAttributeDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = prodAttributeDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	public List<String> getContainLower(String modelName, String fieldName, String exp, Object value, Boolean crossCompany) {
		...
		if(crossCompany) {
			layRecHql += " and cid = ?";
			return prodAttributeDao.createQuery(layRecHql, value, getCurrentCompanyId()).list();
		}
		return prodAttributeDao.createQuery(layRecHql, value).list();
	}
	
	private void dealListTreePageData(Page<MESBasicProdAttribute> prodAttributes, String viewCode){
		List<MESBasicProdAttribute> result = prodAttributes.getResult();
		if(null != result && !result.isEmpty()) {
			int rootPos = 0;
			MESBasicProdAttribute rootTree = result.get(0);
			...
			prodAttributes.setTreeToSurfaceMode(true);
			MESBasicProdAttribute prodAttributeTree = prodAttributeDao.buildTree("prodAttribute" + viewCode, new ListPrepareData(prodAttributes), false);
			if(null != prodAttributeTree.getChildren() && !prodAttributeTree.getChildren().isEmpty()) {
				List<MESBasicProdAttribute> treeList = new ArrayList<MESBasicProdAttribute>();
				Tree.treeToList(prodAttributeTree, treeList, false);
				if(treeList.size() == result.size()) {
					prodAttributes.setResult(treeList);
					prodAttributes.setTreeToSurfaceMode(false);
				} 
			} 
		}
	}
	 ...
	 * 
	 */
	private class ListPrepareData implements PrepareData<MESBasicProdAttribute,Long> {
		private Page<MESBasicProdAttribute> prodAttributes;

		public ListPrepareData(Page<MESBasicProdAttribute> prodAttributes) {
			this.prodAttributes = prodAttributes;
		}

		@Override
			public MESBasicProdAttribute root() {
				MESBasicProdAttribute prodAttribute = new MESBasicProdAttribute();
				prodAttribute.setId(-1l);
				return prodAttribute;
			}
			
			@Override
			public List<MESBasicProdAttribute> prepare(TreeDao<MESBasicProdAttribute, Long> dao) {
				return prodAttributes.getResult();
			}
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdAttributes(Page<MESBasicProdAttribute> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProdAttributes(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdAttributes(Page<MESBasicProdAttribute> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdAttributes(Page<MESBasicProdAttribute> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicProdAttribute> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"prodAttribute\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"prodAttribute\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"prodAttribute\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_prodAttribute_ProdAttribute" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_prodAttribute_ProdAttribute" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  prodAttributeDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  prodAttributeDao.createNativeQuery(powerOperateSql, "MESBasic_1_prodAttribute").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodAttribute\"","MESBasic_1_prodAttribute",customCondition,"S2BASE_PRODATTRIBUTE",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"prodAttribute\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicProdAttributeDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"prodAttribute\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"prodAttribute\".PRODATT_LAYREC ASC, \"prodAttribute\".SORT ASC");
				} else {
					orderPart.append(" \"prodAttribute\".PRODATT_LAYREC ASC, \"prodAttribute\".PRODATT_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) prodAttributeDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=prodAttributeDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = prodAttributeDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicProdAttribute> result = new ArrayList<MESBasicProdAttribute>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdAttribute.class,prodAttributeDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdAttribute.class,prodAttributeDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicProdAttribute prodAttribute : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(prodAttribute.getId(), "MESBasic_prodAttribute_prodAttribute");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(prodAttribute.getId(), "MESBasic_prodAttribute_prodAttribute");
							if(null != documents && !documents.isEmpty()) {
								prodAttribute.setDocument(documents.get(0));
								prodAttribute.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(page.isExportFlag()){
					...
					if(exportMap.get("businessKey")!=null){
						try {
							...
							MESBasicProdAttribute prodAttribute = new MESBasicProdAttribute();
							getBusKeyMethod = prodAttribute.getClass().getMethod(getBusKeyMethodStr);
					...
					if(exportMap.get("mainDisplay")!=null){
						try {
							...
							MESBasicProdAttribute prodAttribute = new MESBasicProdAttribute();
							getMainDisMethod = prodAttribute.getClass().getMethod(getMainDisMethodStr);
						} catch (NoSuchMethodException | SecurityException e) {
							log.error(e.getMessage(),e);
						}
					}
					...
					for (MESBasicProdAttribute prodAttribute : result) {
						Long parentId = prodAttribute.getParentId();
						if(parentId != null && parentId.longValue() != new Long(-1).longValue()){
							for (MESBasicProdAttribute prodAttribute2 : result) {
								if(prodAttribute2.getId() != null && prodAttribute2.getId().longValue() == parentId.longValue()){
									try {
										if(getBusKeyMethod != null){
											Object busKey = getBusKeyMethod.invoke(prodAttribute2);
											prodAttribute.setParentNodeBusinessKey(busKey.toString());
										}
										if(getMainDisMethod != null){
											Object mainDis = getMainDisMethod.invoke(prodAttribute2);
											prodAttribute.setParentNodeMainDisplay(mainDis.toString());
										}	
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_prodAttribute")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_prodAttribute")+"MESBasic_1_prodAttribute".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_prodAttribute",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicProdAttribute> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicProdAttribute> result) {
		View view = null;
		...
		Map<Long, MESBasicProdAttribute> rsMap = new HashMap<Long, MESBasicProdAttribute>();
		for (MESBasicProdAttribute prodAttribute : result) {
			ids.add(prodAttribute.getId());
			rsMap.put(prodAttribute.getId(), prodAttribute);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(prodAttributeDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = prodAttributeDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicProdAttribute prodAttribute,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(prodAttribute.getOwnerStaffId());
		ti.setOwnerPositionId(prodAttribute.getOwnerPositionId());
		ti.setOwnerDepartmentId(prodAttribute.getOwnerDepartmentId());
		
		ti.setCreateStaff(prodAttribute.getCreateStaff());
		ti.setCreateStaffId(prodAttribute.getCreateStaffId());
		ti.setCreateTime(prodAttribute.getCreateTime());
		ti.setDeleteStaff(prodAttribute.getDeleteStaff());
		ti.setDeleteStaffId(prodAttribute.getDeleteStaffId());
		ti.setDeleteTime(prodAttribute.getDeleteTime());
		ti.setCreateDepartmentId(prodAttribute.getCreateDepartmentId());
		ti.setModifyStaff(prodAttribute.getModifyStaff());
		ti.setModifyStaffId(prodAttribute.getModifyStaffId());
		ti.setCreatePositionId(prodAttribute.getCreatePositionId());
		ti.setPositionLayRec(prodAttribute.getPositionLayRec());
		ti.setTableNo(prodAttribute.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_prodAttribute");
		ti.setTargetTableName("S2BASE_PRODATTRIBUTE");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_prodAttribute_ProdAttribute" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  prodAttributeDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodAttribute\"","MESBasic_1_prodAttribute","","S2BASE_PRODATTRIBUTE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"prodAttribute\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"prodAttribute\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodAttribute\"","MESBasic_1_prodAttribute","","S2BASE_PRODATTRIBUTE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("prodAttribute.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"prodAttribute\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"prodAttribute\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"prodAttribute\".CID = 1 OR \"prodAttribute\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("prodAttribute.mc\"").append(".PROD_ATTRIBUTE) not in ("+notinValue+")");
			}
			
			SQLQuery query = prodAttributeDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("prodAttribute", "MESBasic_1_prodAttribute", "prodAttribute_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=prodAttributeDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicProdAttribute.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicProdAttribute.MODULE_CODE;}
	protected String getModelCode(){return MESBasicProdAttribute.MODEL_CODE;}
	
	@Override
	public List<MESBasicProdAttribute> getTreeChildren(Long parentId, String cond, Object... params) {
		MESBasicProdAttribute parent = prodAttributeDao.getTree("prodAttribute_tree_" + parentId, parentId, new MESBasicProdAttributePrepareData(cond, params), false);
	...
	@Override
	public List<MESBasicProdAttribute> getTreeInfo(Map<Long, String> treeInfo, String condition, List<Object> params) {
		...
		for(String layRec : mapKeys) {
			...
			MESBasicProdAttribute prodAttributeTree = prodAttributeDao.getTree("MESBasicProdAttribute" + rootLayRec + "-", rootId, new PrepareData<MESBasicProdAttribute, Long>() {
				@Override
				public MESBasicProdAttribute root() {
					MESBasicProdAttribute prodAttribute = new MESBasicProdAttribute();
					prodAttribute.setId(-1l);
				...
				@Override
				public List<MESBasicProdAttribute> prepare(TreeDao<MESBasicProdAttribute, Long> dao) {
					String hql = "from " + MESBasicProdAttribute.JPA_NAME + " where (" + searchCondition + ") and valid = true " + conditionSql + " order by layRec asc, sort asc";
					return dao.findByHql(hql, paramsList.toArray());
				}
			}, false);
			List<MESBasicProdAttribute> treeChildren = new ArrayList<MESBasicProdAttribute>();
			Tree.treeToList(prodAttributeTree, treeChildren, true);
			treeList.addAll(treeChildren);
		}
		return treeList;
	}

	private class MESBasicProdAttributePrepareData implements PrepareData<MESBasicProdAttribute, Long> {
		private String cond;
		private Object[] params;
		
		public MESBasicProdAttributePrepareData(String cond, Object... params) {
			this.cond = cond;
			this.params = params;
		}
		...
		//@Override
		//public List<MESBasicProdAttribute> prepare(TreeDao<MESBasicProdAttribute, Long> dao) {
		//	String hql = "from " + MESBasicProdAttribute.JPA_NAME;
		//	if (null != cond && cond.trim().length() > 0) {
		//		hql += " where " + cond;
		//	}
		//	List<MESBasicProdAttribute> list = dao.findByHql(hql);
		//	return list;
		//}
		
		@Override
		public List<MESBasicProdAttribute> prepare(TreeDao<MESBasicProdAttribute, Long> dao) {
			List<MESBasicProdAttribute> list = null;
			List<MESBasicProdAttribute> result = new ArrayList<MESBasicProdAttribute>();
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdAttribute.class);
			if(params!=null&&params.length>0){
				...
				list = dao.findByHql("from " + MESBasicProdAttribute.JPA_NAME + " " + ((cond==null || cond.isEmpty()) ? "" : "where " + cond + " order by layRec asc, sort asc")) ;
			}
			for(MESBasicProdAttribute m : list){
				result.add(m);
				if(m.getParentId()!=-1){
					while(m.getParentId()!=-1){
						MESBasicProdAttribute parent = dao.get(m.getParentId());
	...
	/**
	 * 检查模型字段的唯一性约束
	 * @param prodAttribute 模型实例
	 */
	public void checkUniqueConstraint(MESBasicProdAttribute prodAttribute){
		if(prodAttribute.getId()==null){
			List<MESBasicProdAttribute> prodAttributes = prodAttributeDao.findByCriteria(Restrictions.eq("code", prodAttribute.getCode()), Restrictions.eq("valid", true));
			if(prodAttributes!=null && !prodAttributes.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1476753458360", getCurrentUser().getLanguage()), prodAttribute.getCode());
			}
		} else {
			List<MESBasicProdAttribute> prodAttributes = prodAttributeDao.findByCriteria(Restrictions.ne("id", prodAttribute.getId()), Restrictions.eq("code", prodAttribute.getCode()), Restrictions.eq("valid", true));
			if(prodAttributes!=null && !prodAttributes.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1476753458360", getCurrentUser().getLanguage()), prodAttribute.getCode());
			}
	...
	@Override
	public List<MESBasicProdAttribute> getProdAttributes(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdAttribute.class);
		if(params!=null){
			List<Type> types = new ArrayList<Type>();
			for(Object obj : params){
			...
			return prodAttributeDao.findByHql("from " + MESBasicProdAttribute.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return prodAttributeDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicProdAttribute> findProdAttributesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdAttribute.class);
	...
	@Override
	public List<MESBasicProdAttribute> findProdAttributesByHql(String hql, Object... objects){
		
		return   prodAttributeDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicProdAttribute loadProdAttributeByBussinessKey(MESBasicProdAttribute prodAttribute){
		return loadProdAttributeByBussinessKey(prodAttribute.getCode());
	}
	
	@Override
	public Page<MESBasicProdAttribute> getProdAttributes(Page<MESBasicProdAttribute> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdAttribute.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = prodAttributeDao.findByPage(page, sql);
			}else{
				page = prodAttributeDao.findAllByPage(page);
			}
			return page;
		}
		page = prodAttributeDao.findByPage(page, detachedCriteria);
		return page;
	}
	//==============DataGrid多选控件使用 start================
	
	@Override
	public void dealProdAttributeLeaf(List<Long> prodAttributeIds) {
		if (prodAttributeIds != null && prodAttributeIds.size() > 0) {
			String hql = "update " + MESBasicProdAttribute.JPA_NAME + " as t1 set t1.leaf=1 where id in(:ids) and not exists (select t2.id from "
					+ MESBasicProdAttribute.JPA_NAME + " as t2 where t2.parentId = t1.id)";
			Query query = prodAttributeDao.createQuery(hql);
			query.setParameterList("ids", prodAttributeIds);
			query.executeUpdate();
		}
	...
	public void drag(long dragSource, long dragTarget, long sort){
		MESBasicProdAttribute source = getProdAttribute(dragSource);
		MESBasicProdAttribute oldParent = source.getParent();
		
		MESBasicProdAttribute target = null;
		if(dragTarget!=-1){
			target = getProdAttribute(dragTarget);
		} else {
			target = new MESBasicProdAttribute();
			target.setId(-1L);
		}
		prodAttributeDao.move(source, target);
	}
	
	private void dragProdAttribute(MESBasicProdAttribute parent, List<MESBasicProdAttribute> children){
		for(MESBasicProdAttribute child : children){
			...
			saveProdAttribute(child, null,null,null);
			List<MESBasicProdAttribute> subChildren = getTreeChildren(child.getId(), "");
			if(subChildren!=null && subChildren.size()>0){
				dragProdAttribute(child, subChildren);
			}
		}
	}
	
	public List<MESBasicProdAttribute> convertTree(List<MESBasicProdAttribute> prodAttributes){
		List<MESBasicProdAttribute> trees = new ArrayList<MESBasicProdAttribute>();
		Map<Long, MESBasicProdAttribute> map = new LinkedHashMap<Long, MESBasicProdAttribute>(prodAttributes.size());
		Map<Long, Boolean> isParent = new HashMap<Long, Boolean>();
		for (MESBasicProdAttribute node : prodAttributes) {
		...
		for (MESBasicProdAttribute node : prodAttributes) {
			if (null == node.getParentId() ||Long.parseLong(node.getParentId().toString()) == -1) {
				...
				MESBasicProdAttribute parent = map.get(node.getParentId());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdAttribute loadProdAttributeByBussinessKey(Serializable bussinessKey){
		return prodAttributeDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdAttribute> findByProperty(String propertyName, Object object){
		return prodAttributeDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdAttribute findEntityByProperty(String propertyName, Object object){
		return prodAttributeDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicProdAttribute.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = prodAttributeDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_prodAttribute_ProdAttribute","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_prodAttribute", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicProdAttribute prodAttribute,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,prodAttributeDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicProdAttribute.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, prodAttribute);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",prodAttribute.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdAttribute.TABLE_NAME +" where valid = 1";
			list = prodAttributeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdAttribute.TABLE_NAME;
			list = prodAttributeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicProdAttribute.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = prodAttributeDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicProdAttributeDaoImpl prodAttributeDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProdAttribute(final List<MESBasicProdAttribute> insertObjs, final List<MESBasicProdAttribute> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProdAttribute(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProdAttribute(final List<MESBasicProdAttribute> insertObjs, final List<MESBasicProdAttribute> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1471569095116"),importCount));
		...
		Map<Long, MESBasicProdAttribute> oldUpdateObjsMap = new HashMap<Long, MESBasicProdAttribute>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicProdAttribute bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicProdAttribute> bizObjList = prodAttributeDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicProdAttribute bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicProdAttribute> nmeObjs = new ArrayList<MESBasicProdAttribute>();
		...
		long id=autoGeneratorID.getNextId(MESBasicProdAttribute.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicProdAttribute prodAttribute:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicProdAttribute.TABLE_NAME,1,dbId);
			if(prodAttribute.getCode() != null && !prodAttribute.getCode().equals("") ){
				m.put(prodAttribute.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		List<MESBasicProdAttribute> needChangeInsertObjList = new ArrayList<MESBasicProdAttribute>();
		if(insertNodeList != null && insertNodeList.size()>0 ){
			for(int i = 0; i < insertNodeList.size(); i++){
				...
				if(!needChange && !isRoot){
					MESBasicProdAttribute obj = insertObjs.get(i);
					...
					MESBasicProdAttribute parentObj = (MESBasicProdAttribute) insertNode.get(parentNode);
					*/
		...
		if(updateNodeList != null && updateNodeList.size()>0){
			for(int i = 0; i < updateNodeList.size(); i++){
				...
				MESBasicProdAttribute updateObj = updateObjs.get(i);
				...
				if(isRoot){
					...
					if(needChange!=null && !needChange){
						...
						MESBasicProdAttribute parentNode = (MESBasicProdAttribute) insertParentNode.get(parentNodeKey);
		...
		if(insertNodeList != null && insertNodeList.size()>0){
			for(int i = 0; i < insertNodeList.size(); i++){
				Map<String, Object> insertNode = insertNodeList.get(i);
				MESBasicProdAttribute insertObj = insertObjs.get(i);
		...
		if(updateObjs != null && updateObjs.size()>0 ){
			for(MESBasicProdAttribute obj : updateObjs){
				Object bus;
				try {
		...
		if(businessKey != null && mainDisplay != null && businessKey != mainDisplay  && paramList != null && paramList.size()>0){
			String hql = "select " + businessKey.getName() + "," + mainDisplay.getName() + ",id from " + MESBasicProdAttribute.JPA_NAME + " where " + businessKey.getName() + " in (:codes)";
			Query query =  prodAttributeDao.createQuery(hql);
			query.setParameterList("codes", paramList);
			List<Object[]> list = query.list();
			if(list.size() == updateObjs.size()){
				for(int i = 0 ; i < updateObjs.size();i++ ){
					MESBasicProdAttribute obj = updateObjs.get(i);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) prodAttributeDao.createNativeQuery(sql, "MESBasic_1_prodAttribute_ProdAttribute"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		prodAttributeDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicProdAttribute.TABLE_NAME + " SET ";
					...
					updateSql += "parent_id = ?, PRODATT_LAYNO = ?, PRODATT_LAYREC = ?, FULL_PATH_NAME = ?";
					updateSql += " where " + dbId + " =?";
					
					updateps = conn.prepareStatement(updateSql);
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicProdAttribute prodAttribute = updateObjs.get(updateIndex);
						Long id = prodAttribute.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = prodAttribute.getClass().getMethod(updateMethods.get(i));
								} catch (NoSuchMethodException | SecurityException e) {
									log.error(e.getMessage(),e);
								}
								Object obj = null;
								if(null != method){
									try {
										obj = method.invoke(prodAttribute);
									} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
										log.error(e.getMessage(),e);
									}
								}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicProdAttribute.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
					...
					insertSql += dbId + ",CID,CREATE_STAFF_ID,CREATE_TIME,EFFECTIVE_STATE,PARENT_ID,PRODATT_LAYNO,PRODATT_LAYREC,FULL_PATH_NAME) VALUES (";
					for(int i = 0;i<columnMaps.size();i++){
						insertSql += "?,";
					}
					insertSql += "?,?,?,?,?,?,?,?,?)";
					
//					String idSql = "select id from " + MESBasicProdAttribute.TABLE_NAME + " order by id desc";
//					Query query = prodAttributeDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicProdAttribute prodAttribute = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = prodAttribute.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		prodAttributeDaoImpl.getSession().flush();
		prodAttributeDaoImpl.getSession().clear();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicProdAttribute bizObj = insertObjs.get(insertIndex);
			prodAttributeImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicProdAttribute bizObj = updateObjs.get(updateIndex);
			MESBasicProdAttribute oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			prodAttributeImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	private void changeFullPathName(List<Long> needChangeFullPathNameId, Map<Object, Object> needChangeFullPathName,final Property businessKey,Property mainDisplay){
		...
		sql.append(" from ");
		sql.append(MESBasicProdAttribute.TABLE_NAME);
		sql.append(" where ");
		for(Long id : needChangeFullPathNameId){
			sql.append(" PRODATT_LAYREC like '%");
			sql.append(id.toString());
			sql.append("%' or");
		}
		String sqlStr = sql.toString();
		sqlStr = sqlStr.substring(0, sqlStr.length()-2);
		List<Object[]> lists =  prodAttributeDao.createNativeQuery(sqlStr).list();
		...
		prodAttributeDaoImpl.getSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
				...
				if(updateLists != null && updateLists.size()>0){
					String updateSql = "UPDATE " + MESBasicProdAttribute.TABLE_NAME + " SET FULL_PATH_NAME = ? WHERE " + businessKey.getColumnName() + " = ?";
					updateps = conn.prepareStatement(updateSql);
	...
	private Map<String, Object> buildInsertObjNode(List<Map<String, Object>> insertNodeList, List<MESBasicProdAttribute> insertObjs, 
			...
			for(int i =0; i<insertNodeList.size(); i++){
				...
				if(insertBuildedObj.get(busKey)!=null){
					...
					if(isRoot){
						...
						String parentNode = insertNode.get("parentNode").toString();
						MESBasicProdAttribute parentObj = (MESBasicProdAttribute) insertBuildedObj.get(parentNode);
	...
	public String getViewName(String modelCode){
		...
		String viewName = prodAttributeDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				prodAttributeDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = prodAttributeDao.createNativeQuery(sql, "MESBasic_1_prodAttribute_ProdAttribute").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = prodAttributeDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = prodAttributeDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = prodAttributeDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?prodAttributeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  prodAttributeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,prodAttributeDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="ec.flow.save")
	public void saveProdAttribute(MESBasicProdAttribute prodAttribute, DataGridService dataGridService){
		saveProdAttribute(prodAttribute, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="ec.flow.save")
	public void saveProdAttribute(MESBasicProdAttribute prodAttribute, DataGridService dataGridService, String viewCode){
		saveProdAttribute(prodAttribute, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute")
	public void saveProdAttribute(MESBasicProdAttribute prodAttribute, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProdAttribute(prodAttribute, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute")
	public void saveProdAttribute(MESBasicProdAttribute prodAttribute, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_prodAttribute";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_prodAttribute");
		String url = null;
		if(null != prodAttribute.getId() && prodAttribute.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdAttribute.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute")
	public Map<Object, Object> prodAttributeDataGridImport(MESBasicProdAttribute prodAttribute, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = prodAttribute.getVirtualId();
		
		prodAttribute = prodAttributeDao.load(prodAttribute.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, prodAttribute.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = prodAttribute.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(prodAttribute);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, prodAttribute.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute")
	public void mergeProdAttribute(MESBasicProdAttribute prodAttribute, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(prodAttribute);
		beforeSaveProdAttribute(prodAttribute);
		prodAttributeDao.merge(prodAttribute);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodAttribute",model="MESBasic_1_prodAttribute_ProdAttribute",desc="foundation.batch.save",operType="4")
	public void batchSaveProdAttribute(MESBasicProdAttribute prodAttribute, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_prodAttribute";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_prodAttribute");
	...
	@Override
	public Page<MESBasicProdAttribute> getByPage(Page<MESBasicProdAttribute> page,DetachedCriteria detachedCriteria){
		return prodAttributeDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_prodAttribute").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveProdAttribute(MESBasicProdAttribute prodAttribute, Object... objects){
		checkUniqueConstraint(prodAttribute);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProdAttribute(MESBasicProdAttribute prodAttribute, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProdAttribute(List<MESBasicProdAttribute> prodAttributes, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProdAttribute(List<MESBasicProdAttribute> prodAttributes){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProdAttribute(List<MESBasicProdAttribute> prodAttributes, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProdAttribute(List<MESBasicProdAttribute> prodAttributes){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProdAttribute(MESBasicProdAttribute prodAttribute){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProdAttribute(MESBasicProdAttribute prodAttribute){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProdAttribute(MESBasicProdAttribute prodAttribute){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProdAttribute(MESBasicProdAttribute prodAttribute){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicProdAttribute prodAttribute, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicProdAttribute prodAttribute, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProdAttribute(List<MESBasicProdAttribute> insertObjs, List<MESBasicProdAttribute> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProdAttribute,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProdAttribute(List<MESBasicProdAttribute> insertObjs, List<MESBasicProdAttribute> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProdAttribute,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
