...
/* CUSTOM CODE START(action,import,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicAreaAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicArea> {
	...
	//导出excel时,主模型内容
	List<MESBasicArea> queryResults = new ArrayList<MESBasicArea>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/area/area/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/area/area/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_area_Area" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicArea> areas;
	
	public List<MESBasicArea> getAreas() {
		return areas;
	}
	...
	private Page<MESBasicArea> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			area = new MESBasicArea();
		} else {
			if(id > 0){
				area = areaService.getArea(id, viewCode);
			}
		}
	...
	public Page<MESBasicArea> getPage() {
		if (null == page) {
			page = new Page<MESBasicArea>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/area/area/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = areaService.getAreaAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/area/area/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String areaReferenceCopy() throws Exception{
			MESBasicArea oldArea = null;
			if(isEditNew()){
				...
				oldArea = new MESBasicArea();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,area.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, area.getId());
				...
				oldArea = areaService.getArea(id);
			}
			
			
			area = areaService.getArea(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_area_Area");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicArea.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldArea, null);
						Method setValue = MESBasicArea.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(area, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			area.setCreateDepartmentId(oldArea.getCreateDepartmentId());
			area.setCreatePositionId(oldArea.getCreatePositionId());
			area.setCreateStaffId(oldArea.getCreateStaffId());
			area.setOwnerDepartmentId(oldArea.getOwnerDepartmentId());
			area.setOwnerPositionId(oldArea.getOwnerPositionId());
			area.setOwnerStaffId(oldArea.getOwnerStaffId());
			area.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_area_Area", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/area/area/areaEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "areaEdit.ftl") }),
		@Action(value = "/MESBasic/area/area/areaEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "areaEditPrint.ftl")})
		,
		@Action(value = "/MESBasic/area/area/areaNewEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "areaNewEdit.ftl") }),
		@Action(value = "/MESBasic/area/area/areaNewEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "areaNewEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=areaService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return areaReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			area = areaService.getArea(this.area.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,area.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/area/area/areaList", results = { @Result(type = FREEMARKER, location = "areaList.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/area/area/arealayout", results = { @Result(type = FREEMARKER, location = "arealayout.ftl") })
		,
		@Action(value = "/MESBasic/area/area/areaRefLayout", results = { @Result(type = FREEMARKER, location = "areaRefLayout.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/area/area/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = areaService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(areaService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(areaService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_area_Area", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/area/area/areaListRef", results = { @Result(type = FREEMARKER, location = "areaListRef.ftl") })
		,
		@Action(value = "/MESBasic/area/area/areaTree", results = { @Result(type = FREEMARKER, location = "areaTree.ftl") })
		,
		@Action(value = "/MESBasic/area/area/areaTreeRef", results = { @Result(type = FREEMARKER, location = "areaTreeRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/area/area/areaList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.areaMemo,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="areaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"area\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"area\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"area\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"area\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/area/area/areaList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/area/area/areaListRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.areaMemo,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="areaListRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/MESBasic/area/area/areaTreeDrag")
	,
	@Action(value="/MESBasic/area/area/areaTreeRefDrag")
	})
	public String viewTreeDrag() throws Exception{
		areaService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/MESBasic/area/area/areaEditSort")
	,
	@Action(value="/MESBasic/area/area/arealayoutSort")
	,
	@Action(value="/MESBasic/area/area/areaListSort")
	,
	@Action(value="/MESBasic/area/area/areaListRefSort")
	,
	@Action(value="/MESBasic/area/area/areaMneSort")
	,
	@Action(value="/MESBasic/area/area/areaNewEditSort")
	,
	@Action(value="/MESBasic/area/area/areaRefLayoutSort")
	,
	@Action(value="/MESBasic/area/area/areaTreeSort")
	,
	@Action(value="/MESBasic/area/area/areaTreeRefSort")
	})
		...
		if(sortResult!=null && sortResult.length()>0){
			...
			for(String sort : sorts){
				if(sort!=null && sort.length()>0){
					...
					MESBasicArea area = areaService.getArea(id);
					area.setSort(s);
					areaService.saveArea(area, null,null,null);
				}
			}
		}
		return null;
	...
	@Action(value="/MESBasic/area/area/areaTreeData",
			results = { @Result(type = JSON, params = { "root", "areas", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/area/area/areaTreeRefData",
			results = { @Result(type = JSON, params = { "root", "areas", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"area\".", "{alias}.");
					}
		...
		areas = areaService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/MESBasic/area/area/areaTreeFullData",
			results = { @Result(type = JSON, params = { "root", "areas", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/area/area/areaTreeRefFullData",
			results = { @Result(type = JSON, params = { "root", "areas", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			areas = areaService.getTreeInfo(treeInfo, null, null);
		}
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/area/area/allTempleteDownload", results = { @Result(name="excel",type="excel",location="areaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_area_Area");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		areaService.findDataGridPage(dg,dgclass,getDgPage(),area,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(area.getId() != null && area.getId() != -1){
				area = areaService.getArea(area.getId());
			}
			areaService.findDataGridPage(dg,dgclass,getDgPage(),area,searchCondition,params);
	...
	private void save() throws Exception {
		...
		areaService.saveArea(area,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/area/area/areaEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/area/area/areaNewEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_area_Area", "submit");
		bap_validate_datagrids=areaService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", area.getId());
		...
		fileuploads.put("linkId", area.getId());
		bapAfterCustomMethod("MESBasic_1_area_Area", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/area/area/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_area_Area", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		areaService.deleteArea(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_area_Area", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/area/area/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_area_Area", "restore");
		areaService.restoreArea(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_area_Area", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/area/area/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/area/area/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_area_Area", "showDealInfoData");
		if (null != tableInfoId) {
			int count=areaService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = areaService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = areaService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_area_Area", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicArea> findExportDataInfos(Page<MESBasicArea> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = areaService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = areaService.getAssProperty(p.getCode());	
									//String assModelCode = areaService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			areaService.findAreas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			areaService.findAreas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicArea> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicArea result : results){
	...
	public boolean isEditNew(){
		//return null == this.area || null == this.area.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/area/area/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_area_Area", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_area_Area", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/area/area/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_area_Area", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/area/area/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/area/area/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicAreaService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_area", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_area");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_area_Area", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_area_Area", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/area/area/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_area_Area", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_area_Area", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/area/area/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_area_Area", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/area/area/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_area_Area";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_area_Area", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/area/area/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicArea area = null;
		if(null != id) {
		...
		_auditLog.setColumnStringB(null == area.getCode() ? "" : area.getCode().toString());
		_auditLog.setColumnStringA(null == area.getName() ? "" : area.getName().toString());
		_auditLog.setColumnStringC(area.getId().toString());
		_auditLog.setColumnStringD(MESBasicArea.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_area");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicAreaAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, area,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, area,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,area);
		...
		printlog.setEntity("MESBasic_1_area");
		...
		printlog.setBusinessId(area.getId());
		printlog.setBussinessKey(null == area.getCode() ? "" : area.getCode().toString());
		printlog.setMainDisplay(null == area.getName() ? "" : area.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/area/area/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(area,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/area/area/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(areaService.getArea(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/area/area/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/area/area/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, area,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				area = areaService.getArea(Long.parseLong(arrId[i]));
				if(null != area){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == area.getName() ? "" : "," + area.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == area.getName() ? "" : area.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == area.getCode() ? "" : "," + area.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == area.getCode() ? "" : area.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == area.getId() ? "" : "," + area.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == area.getId() ? "" : area.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, area,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,area);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/area/area/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicArea> findExportPageInfos(Page<MESBasicArea> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_area" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = areaService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicArea queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = areaService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = areaService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_area_Area");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_area_Area")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_area_Area");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_area_Area");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = areaService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
	...
	@Action(value = "/MESBasic/area/area/isLeafAndHasBaseInfo",results = {@Result(name = SUCCESS, type = JSON, params = {"root", "responseMap"})})
	public String isLeaf(){
		String isLeaf=areaService.isLeaf(areaID);
		String hasBaseInfo=areaService.hasBaseInfo(areaID);
