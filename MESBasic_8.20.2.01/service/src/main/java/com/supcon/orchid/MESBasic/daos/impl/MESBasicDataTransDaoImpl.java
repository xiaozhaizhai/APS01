...
@Repository
public class MESBasicDataTransDaoImpl extends ExtendedGenericDaoImpl<MESBasicDataTrans, Long> implements MESBasicDataTransDao {
	@Override
	...
	@Override
	public MESBasicDataTransDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicDataTransDealInfo dealInfo = new MESBasicDataTransDealInfo();
	...
	@Override
	public MESBasicDataTransDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicDataTrans mainObj, Staff dealStaff) {
		MESBasicDataTransDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicDataTransDealInfo.class);
		...
		List<MESBasicDataTransDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
