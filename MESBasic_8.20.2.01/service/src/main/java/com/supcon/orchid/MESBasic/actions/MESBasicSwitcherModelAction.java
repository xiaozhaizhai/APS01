...
/* CUSTOM CODE START(action,import,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicSwitcherModelAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicSwitcherModel> {
	...
	private MESBasicSwitcherModel switcherModel;
	...
	//导出excel时,主模型内容
	List<MESBasicSwitcherModel> queryResults = new ArrayList<MESBasicSwitcherModel>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/routine/switcherModel/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/routine/switcherModel/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_routine_SwitcherModel" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicSwitcherModel> switcherModels;
	
	public List<MESBasicSwitcherModel> getSwitcherModels() {
	...
	private Page<MESBasicSwitcherModel> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			switcherModel = new MESBasicSwitcherModel();
			if(switcherModel.getStype() != null) {
				switcherModel.setStype((SystemCode) systemCodeService.getSystemCode(switcherModel.getStype().getId()));
			}
		} else {
			if(id > 0){
				switcherModel = switcherModelService.getSwitcherModel(id, viewCode);
			}
		}
	...
	public Page<MESBasicSwitcherModel> getPage() {
		if (null == page) {
			page = new Page<MESBasicSwitcherModel>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicSwitcherModel getSwitcherModel() {
		return switcherModel;
	}		

	public void setSwitcherModel(MESBasicSwitcherModel switcherModel) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/routine/switcherModel/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = switcherModelService.getSwitcherModelAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/routine/switcherModel/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.radion1416557727476
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String switcherModelReferenceCopy() throws Exception{
			MESBasicSwitcherModel oldSwitcherModel = null;
			if(isEditNew()){
				...
				oldSwitcherModel = new MESBasicSwitcherModel();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,switcherModel.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, switcherModel.getId());
				...
				oldSwitcherModel = switcherModelService.getSwitcherModel(id);
			}
			
			
			switcherModel = switcherModelService.getSwitcherModel(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_routine_SwitcherModel");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicSwitcherModel.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldSwitcherModel, null);
						Method setValue = MESBasicSwitcherModel.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(switcherModel, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			switcherModel.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_routine_SwitcherModel", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=switcherModelService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return switcherModelReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			switcherModel = switcherModelService.getSwitcherModel(this.switcherModel.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,switcherModel.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = switcherModelService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(switcherModelService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(switcherModelService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_routine_SwitcherModel", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"switcherModel\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"switcherModel\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"switcherModel\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"switcherModel\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			switcherModelService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		switcherModelService.findDataGridPage(dg,dgclass,getDgPage(),switcherModel,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(switcherModel.getId() != null && switcherModel.getId() != -1){
				switcherModel = switcherModelService.getSwitcherModel(switcherModel.getId());
			}
			switcherModelService.findDataGridPage(dg,dgclass,getDgPage(),switcherModel,searchCondition,params);
	...
	private void save() throws Exception {
		...
		switcherModelService.saveSwitcherModel(switcherModel,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_routine_SwitcherModel", "submit");
		bap_validate_datagrids=switcherModelService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", switcherModel.getId());
		...
		fileuploads.put("linkId", switcherModel.getId());
		bapAfterCustomMethod("MESBasic_1_routine_SwitcherModel", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/routine/switcherModel/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_SwitcherModel", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		switcherModelService.deleteSwitcherModel(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_routine_SwitcherModel", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/switcherModel/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_SwitcherModel", "restore");
		switcherModelService.restoreSwitcherModel(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_routine_SwitcherModel", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicSwitcherModel> findExportDataInfos(Page<MESBasicSwitcherModel> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = switcherModelService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = switcherModelService.getAssProperty(p.getCode());	
									//String assModelCode = switcherModelService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			switcherModelService.findSwitcherModels(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			switcherModelService.findSwitcherModels(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicSwitcherModel> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicSwitcherModel result : results){
	...
	public boolean isEditNew(){
		//return null == this.switcherModel || null == this.switcherModel.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/routine/switcherModel/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_SwitcherModel", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_routine_SwitcherModel", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/routine/switcherModel/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_routine_SwitcherModel", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/routine/switcherModel/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/switcherModel/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicSwitcherModelService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_routine_SwitcherModel", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_routine_SwitcherModel", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/switcherModel/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_routine_SwitcherModel", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_routine_SwitcherModel", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/routine/switcherModel/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_SwitcherModel", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/routine/switcherModel/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_routine_SwitcherModel";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_routine_SwitcherModel", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/routine/switcherModel/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
		_auditLog.setColumnStringA(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
		_auditLog.setColumnStringC(switcherModel.getId().toString());
		_auditLog.setColumnStringD(MESBasicSwitcherModel.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicSwitcherModelAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, switcherModel,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, switcherModel,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,switcherModel);
		...
		printlog.setBusinessId(switcherModel.getId());
		printlog.setBussinessKey(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
		printlog.setMainDisplay(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/routine/switcherModel/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(switcherModel,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/switcherModel/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(switcherModelService.getSwitcherModel(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/routine/switcherModel/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/routine/switcherModel/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, switcherModel,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				switcherModel = switcherModelService.getSwitcherModel(Long.parseLong(arrId[i]));
				if(null != switcherModel){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == switcherModel.getId() ? "" : "," + switcherModel.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == switcherModel.getId() ? "" : "," + switcherModel.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == switcherModel.getId() ? "" : "," + switcherModel.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == switcherModel.getId() ? "" : switcherModel.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, switcherModel,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,switcherModel);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/routine/switcherModel/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_routine_SwitcherModel");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_routine_SwitcherModel")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_routine_SwitcherModel");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_routine_SwitcherModel");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = switcherModelService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_routine_SwitcherModel,MESBasic_1) */
// 自定义代码
