...
/* CUSTOM CODE START(action,import,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicBaseVendorAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBaseVendor> {
	...
	//导出excel时,主模型内容
	List<MESBasicBaseVendor> queryResults = new ArrayList<MESBasicBaseVendor>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseVendor/baseVendor/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseVendor/baseVendor/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_baseVendor_BaseVendor" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBaseVendor> baseVendors;
	
	public List<MESBasicBaseVendor> getBaseVendors() {
		return baseVendors;
	}
	...
	private Page<MESBasicBaseVendor> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			baseVendor = new MESBasicBaseVendor();
		} else {
			if(id > 0){
				baseVendor = baseVendorService.getBaseVendor(id, viewCode);
			}
		}
	...
	public Page<MESBasicBaseVendor> getPage() {
		if (null == page) {
			page = new Page<MESBasicBaseVendor>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseVendor/baseVendor/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = baseVendorService.getBaseVendorAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseVendor/baseVendor/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String baseVendorReferenceCopy() throws Exception{
			MESBasicBaseVendor oldBaseVendor = null;
			if(isEditNew()){
				...
				oldBaseVendor = new MESBasicBaseVendor();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseVendor.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, baseVendor.getId());
				...
				oldBaseVendor = baseVendorService.getBaseVendor(id);
			}
			
			
			baseVendor = baseVendorService.getBaseVendor(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_baseVendor_BaseVendor");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBaseVendor.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBaseVendor, null);
						Method setValue = MESBasicBaseVendor.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(baseVendor, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			baseVendor.setCreateDepartmentId(oldBaseVendor.getCreateDepartmentId());
			baseVendor.setCreatePositionId(oldBaseVendor.getCreatePositionId());
			baseVendor.setCreateStaffId(oldBaseVendor.getCreateStaffId());
			baseVendor.setOwnerDepartmentId(oldBaseVendor.getOwnerDepartmentId());
			baseVendor.setOwnerPositionId(oldBaseVendor.getOwnerPositionId());
			baseVendor.setOwnerStaffId(oldBaseVendor.getOwnerStaffId());
			baseVendor.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_baseVendor_BaseVendor", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVendor/baseVendor/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/MESBasic/baseVendor/baseVendor/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=baseVendorService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return baseVendorReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			baseVendor = baseVendorService.getBaseVendor(this.baseVendor.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseVendor.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "vendorView.ftl") }),
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "vendorViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorPart", results = { @Result(type = FREEMARKER, location = "vendorPart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVendor/baseVendor/refVendor", results = { @Result(type = FREEMARKER, location = "refVendor.ftl") })
		,
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorLayout", results = { @Result(type = FREEMARKER, location = "vendorLayout.ftl") })
		,
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorRef", results = { @Result(type = FREEMARKER, location = "vendorRef.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/baseVendor/baseVendor/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = baseVendorService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(baseVendorService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(baseVendorService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_baseVendor_BaseVendor", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVendor/baseVendor/refVendorPart", results = { @Result(type = FREEMARKER, location = "refVendorPart.ftl") })
		,
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorPartRef", results = { @Result(type = FREEMARKER, location = "vendorPartRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.vendorCode,result.vendorShortName,result.vendorName,result.vendorDegreeCode,result.vendorDegreeCodeForDisplay,result.vendorVCCode.name,result.vendorState,result.vendorStateForDisplay,result.vendorVCCode.id,result.vendorVCCode.code,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="vendorPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"baseVendor\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"baseVendor\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"vendorDepartID\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		if (null != request.getParameter("baseVendorClass.layRec") && request.getParameter("baseVendorClass.layRec").trim().length() > 0) {
			params.add(new Param("\"vendorVCCode\".VC_LAYREC", request.getParameter("baseVendorClass.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"baseVendor\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"baseVendor\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorPart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVendor/baseVendor/refVendorPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.vendorCode,result.vendorShortName,result.vendorName,result.vendorDegreeCode,result.vendorDegreeCodeForDisplay,result.vendorVCCode.name,result.vendorVCCode.id,result.vendorVCCode.code,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refVendorPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/baseVendor/baseVendor/vendorPartRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.vendorCode,result.vendorShortName,result.vendorName,result.vendorDegreeCode,result.vendorDegreeCodeForDisplay,result.vendorVCCode.name,result.vendorVCCode.id,result.vendorVCCode.code,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="vendorPartRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/baseVendor/baseVendor/allTempleteDownload", results = { @Result(name="excel",type="excel",location="vendorPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_baseVendor_BaseVendor");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		baseVendorService.findDataGridPage(dg,dgclass,getDgPage(),baseVendor,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(baseVendor.getId() != null && baseVendor.getId() != -1){
				baseVendor = baseVendorService.getBaseVendor(baseVendor.getId());
			}
			baseVendorService.findDataGridPage(dg,dgclass,getDgPage(),baseVendor,searchCondition,params);
	...
	private void save() throws Exception {
		...
		baseVendorService.saveBaseVendor(baseVendor,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/baseVendor/baseVendor/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/baseVendor/baseVendor/vendorView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "submit");
		bap_validate_datagrids=baseVendorService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", baseVendor.getId());
		...
		fileuploads.put("linkId", baseVendor.getId());
		bapAfterCustomMethod("MESBasic_1_baseVendor_BaseVendor", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/baseVendor/baseVendor/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		baseVendorService.deleteBaseVendor(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_baseVendor_BaseVendor", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVendor/baseVendor/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "restore");
		baseVendorService.restoreBaseVendor(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_baseVendor_BaseVendor", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/baseVendor/baseVendor/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/baseVendor/baseVendor/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "showDealInfoData");
		if (null != tableInfoId) {
			int count=baseVendorService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = baseVendorService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = baseVendorService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_baseVendor_BaseVendor", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBaseVendor> findExportDataInfos(Page<MESBasicBaseVendor> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = baseVendorService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = baseVendorService.getAssProperty(p.getCode());	
									//String assModelCode = baseVendorService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			baseVendorService.findBaseVendors(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			baseVendorService.findBaseVendors(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBaseVendor> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBaseVendor result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".VENDOR_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.baseVendor || null == this.baseVendor.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/baseVendor/baseVendor/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_baseVendor_BaseVendor", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/baseVendor/baseVendor/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/baseVendor/baseVendor/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVendor/baseVendor/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBaseVendorService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_baseVendor", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_baseVendor");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_baseVendor_BaseVendor", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVendor/baseVendor/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBaseVendorService"
						, "com.supcon.orchid.MESBasic.services.MESBasicBaseCustomerService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicBaseVendorClassService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_baseVendor_BaseVendor", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_baseVendor_BaseVendor", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/baseVendor/baseVendor/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/baseVendor/baseVendor/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_baseVendor_BaseVendor";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_baseVendor_BaseVendor", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/baseVendor/baseVendor/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == baseVendor.getVendorCode() ? "" : baseVendor.getVendorCode().toString());
		_auditLog.setColumnStringA(null == baseVendor.getVendorName() ? "" : baseVendor.getVendorName().toString());
		_auditLog.setColumnStringC(baseVendor.getId().toString());
		_auditLog.setColumnStringD(MESBasicBaseVendor.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_baseVendor");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBaseVendorAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, baseVendor,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, baseVendor,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,baseVendor);
		...
		printlog.setEntity("MESBasic_1_baseVendor");
		...
		printlog.setBusinessId(baseVendor.getId());
		printlog.setBussinessKey(null == baseVendor.getVendorCode() ? "" : baseVendor.getVendorCode().toString());
		printlog.setMainDisplay(null == baseVendor.getVendorName() ? "" : baseVendor.getVendorName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/baseVendor/baseVendor/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(baseVendor,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVendor/baseVendor/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(baseVendorService.getBaseVendor(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/baseVendor/baseVendor/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/baseVendor/baseVendor/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, baseVendor,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				baseVendor = baseVendorService.getBaseVendor(Long.parseLong(arrId[i]));
				if(null != baseVendor){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == baseVendor.getVendorName() ? "" : "," + baseVendor.getVendorName().toString()));
					} else {
						_auditLog.setColumnStringA(null == baseVendor.getVendorName() ? "" : baseVendor.getVendorName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == baseVendor.getVendorCode() ? "" : "," + baseVendor.getVendorCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == baseVendor.getVendorCode() ? "" : baseVendor.getVendorCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == baseVendor.getId() ? "" : "," + baseVendor.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == baseVendor.getId() ? "" : baseVendor.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, baseVendor,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,baseVendor);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/baseVendor/baseVendor/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicBaseVendor> findExportPageInfos(Page<MESBasicBaseVendor> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_baseVendor" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = baseVendorService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicBaseVendor queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = baseVendorService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = baseVendorService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_baseVendor_BaseVendor");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_baseVendor_BaseVendor")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_baseVendor_BaseVendor");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_baseVendor_BaseVendor");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = baseVendorService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
		/*
        * 在第一次创建供应商时，设置State字段初始值，默认启用
        */
		if("MESBasic_1_baseVendor_BaseVendor".equals(code) && "submit".equals(methodName)) {
			if(isEditNew()) {
				baseVendor.setVendorState("2");
			}
		}
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
		/*
        * 删除时同步更新S2 State字段
        */
		if("MESBasic_1_baseVendor_BaseVendor".equals(code) && "delete".equals(methodName)) {
			baseVendorService.doDeleteVendor(ids);
		}
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
	...
	@Action(value = "/MESBasic/baseVender/baseVendor/doEnableVendor", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String doEnableVendor() {
		baseVendorService.doEnableVendor(vendorId);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVender/baseVendor/doDisableVendor", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String doDisableVendor() {
		baseVendorService.doDisableVendor(vendorId);
		return SUCCESS;
	}
/* CUSTOM CODE END */
}