...
import com.supcon.orchid.MESBasic.services.MESBasicProdattrelImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicProdattrelDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_prodattrelService")
@Transactional
public class MESBasicProdattrelServiceImpl  extends BaseServiceImpl<MESBasicProdattrel> implements MESBasicProdattrelService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicProdattrel> bapBaseService;
	...
	@Autowired
	private MESBasicProdattrelImportService prodattrelImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicProdattrel.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  prodattrelDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicProdattrel.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = prodattrelDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdattrel.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdattrel.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  prodattrelDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicProdattrel.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicProdattrel.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicProdattrel> list = (List<MESBasicProdattrel>) prodattrelDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicProdattrel getProdattrel(long id){
		return getProdattrel(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdattrel getProdattrel(long id, String viewCode){
		MESBasicProdattrel prodattrel = prodattrelDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProdattrelAsJSON(long id, String include){
		MESBasicProdattrel prodattrel = prodattrelDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(prodattrel, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.common.delete",operType="3")
	public void deleteProdattrel(MESBasicProdattrel prodattrel){
		if(null != prodattrel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdattrel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
			AuditUtil.setColumnStringA(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
			AuditUtil.setColumnStringC(prodattrel.getId().toString());
		}
		beforeDeleteProdattrel(prodattrel);
		prodattrelDao.delete(prodattrel);
		afterDeleteProdattrel(prodattrel);
		prodattrelDao.flush();
		List<MESBasicProdattrel> params = new ArrayList<MESBasicProdattrel>();
		params.add(prodattrel);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_prodatriel");
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.common.delete",operType="3")
	public void deleteProdattrel(List<Long> prodattrelIds){
		deleteProdattrel(prodattrelIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.common.delete",operType="3")
	public void deleteProdattrel(List<Long> prodattrelIds, String eventTopic) {
		List<MESBasicProdattrel> prodattrels = new ArrayList<MESBasicProdattrel>();
		for(Long prodattrelId : prodattrelIds){
			MESBasicProdattrel prodattrel = getProdattrel(prodattrelId);
			prodattrels.add(prodattrel);
			if(null != prodattrel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdattrel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == prodattrel.getId() ? "" : "," + prodattrel.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == prodattrel.getId() ? "" : "," + prodattrel.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == prodattrel.getId() ? "" : "," + prodattrel.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(prodattrels != null && prodattrels.size() > 0 ){
			for(MESBasicProdattrel prodattrel : prodattrels){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),prodattrel, prodattrel.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(prodattrelIds != null && prodattrelIds.size() > 0) {
			String hql = "update " + MESBasicProdattrel.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = prodattrelDao.createQuery(hql);
			query.setParameterList("ids", prodattrelIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicProdattrel prodattrelz : prodattrels){
				prodattrelz.setValid(false);
				List<MESBasicProdattrel> params = new ArrayList<MESBasicProdattrel>();
				params.add(prodattrelz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_prodatriel");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.common.delete",operType="3")
	public void deleteProdattrel(long prodattrelId, int prodattrelVersion){
		this.deleteProdattrel(prodattrelId, prodattrelVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.common.delete",operType="3")
	public void deleteProdattrel(long prodattrelId, int prodattrelVersion,SignatureLog signatureLog){
		MESBasicProdattrel prodattrel = getProdattrel(prodattrelId);
		if(prodattrel != null && !prodattrel.isValid()){
		...
		beforeDeleteProdattrel(prodattrel);
		prodattrelDao.delete(prodattrelId, prodattrelVersion);
		afterDeleteProdattrel(prodattrel);
		prodattrelDao.flush();
		List<MESBasicProdattrel> params = new ArrayList<MESBasicProdattrel>();
		params.add(prodattrel);
		...
		if(signatureLog != null ) {
			...
			businessKey=prodattrel.getId();
			signatureLog.setTableId(prodattrel.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.common.delete",operType="3")
	public void deleteProdattrel(String prodattrelIds){
		this.deleteProdattrel(prodattrelIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.common.delete",operType="3")
	public void deleteProdattrel(String prodattrelIds,SignatureLog signatureLog){
		deleteCollection(prodattrelIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProdattrel(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_prodatriel#modelCode:MESBasic_1_prodatriel_Prodattrel#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="ec.engine.restore",operType="10")
	public void restoreProdattrel(String prodattrelIds){
		restoreCollection(prodattrelIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProdattrel(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="ec.engine.restore",operType="10")
	public void restoreProdattrel(long prodattrelId){
		
		findBusinessKeyUsed(prodattrelId);	//判断业务主键是否重复
		...
		beforeRestoreProdattrel(prodattrel);
		prodattrel.setValid(true);
		prodattrelDao.update(prodattrel);
		afterRestoreProdattrel(prodattrel);
		List<MESBasicProdattrel> params = new ArrayList<MESBasicProdattrel>();
		params.add(prodattrel);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long prodattrelId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_prodatriel_Prodattrel");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicProdattrel.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicProdattrel.TABLE_NAME +" where id = ? )";
			List<Object> list =  prodattrelDao.createNativeQuery(sql,prodattrelId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProdattrel(List<MESBasicProdattrel>  prodattrels){
		for(MESBasicProdattrel prodattrel:prodattrels)  {
			saveProdattrel(prodattrel, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProdattrel(List<MESBasicProdattrel>  prodattrels){
		for(MESBasicProdattrel prodattrel:prodattrels)  {
			saveProdattrel(prodattrel, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="ec.flow.save")
	public void saveProdattrel(MESBasicProdattrel prodattrel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProdattrel(prodattrel, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="ec.flow.save")
	public void saveProdattrel(MESBasicProdattrel prodattrel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProdattrel(prodattrel, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProdattrelCodes(MESBasicProdattrel prodattrel) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdattrelCodes(prodattrel, false);
	}
	
	public void generateProdattrelCodes(MESBasicProdattrel prodattrel, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProdattrelSummarys(MESBasicProdattrel prodattrel) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdattrelSummarys(prodattrel, false);
	}

	public void generateProdattrelSummarys(MESBasicProdattrel prodattrel, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel")
	public void saveProdattrel(MESBasicProdattrel prodattrel,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProdattrel(prodattrel, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel")
	public void saveProdattrel(MESBasicProdattrel prodattrel,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_prodatriel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_prodatriel");
		String url = null;
		if(prodattrel.getId() != null && prodattrel.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdattrel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_prodatriel/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdattrel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_prodatriel/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, prodattrel);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(prodattrel);
		beforeSaveProdattrel(prodattrel, viewIsView);

		if (viewIsView) {
			prodattrelDao.saveWithRevertVersion(prodattrel);
		} else {
			if(isNew)
				prodattrelDao.save(prodattrel);
			else
				prodattrelDao.merge(prodattrel);
		}
		if(null != prodattrel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdattrel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
			AuditUtil.setColumnStringA(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
		...
		dealDatagridsSave(prodattrel,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProdattrelCodes(prodattrel, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProdattrelSummarys(prodattrel, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProdattrel(prodattrel, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, prodattrel);
			}
		}
		List<MESBasicProdattrel> params = new ArrayList<MESBasicProdattrel>();
		prodattrelDao.flush();
		prodattrelDao.clear();
		prodattrel = prodattrelDao.load(prodattrel.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicProdattrel prodattrel){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("prodattrel", prodattrel);
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel")
	public void mergeProdattrel(MESBasicProdattrel prodattrel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(prodattrel);
		beforeSaveProdattrel(prodattrel);
		prodattrelDao.merge(prodattrel);
		if(null != prodattrel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdattrel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
			AuditUtil.setColumnStringA(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
		...
		dealDatagridsSave(prodattrel,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProdattrel(prodattrel);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicProdattrel> findProdattrels(Page<MESBasicProdattrel> page, Criterion... criterions) {
		return prodattrelDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicProdattrelDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicProdattrelDealInfo> list=prodattrelDao.createQuery("from " + MESBasicProdattrelDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicProdattrelDealInfo dealInfo) {
		prodattrelDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicProdattrelDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) prodattrelDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicProdattrelDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = prodattrelDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = prodattrelDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicProdattrelDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = prodattrelDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdattrels(Page<MESBasicProdattrel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProdattrels(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdattrels(Page<MESBasicProdattrel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdattrels(Page<MESBasicProdattrel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicProdattrel> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"prodattrel\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"prodattrel\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"prodattrel\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_prodatriel_Prodattrel" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_prodatriel_Prodattrel" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  prodattrelDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  prodattrelDao.createNativeQuery(powerOperateSql, "MESBasic_1_prodatriel").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodattrel\"","MESBasic_1_prodatriel",customCondition,"S2BASE_PRODATTREL",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"prodattrel\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicProdattrelDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"prodattrel\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"prodattrel\".LAY_REC ASC, \"prodattrel\".SORT ASC");
				} else {
					orderPart.append(" \"prodattrel\".PRODATTREL_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) prodattrelDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=prodattrelDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = prodattrelDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicProdattrel> result = new ArrayList<MESBasicProdattrel>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdattrel.class,prodattrelDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdattrel.class,prodattrelDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicProdattrel prodattrel : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(prodattrel.getId(), "MESBasic_prodatriel_prodattrel");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(prodattrel.getId(), "MESBasic_prodatriel_prodattrel");
							if(null != documents && !documents.isEmpty()) {
								prodattrel.setDocument(documents.get(0));
								prodattrel.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_prodatriel")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_prodatriel")+"MESBasic_1_prodatriel".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_prodatriel",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicProdattrel> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicProdattrel> result) {
		View view = null;
		...
		Map<Long, MESBasicProdattrel> rsMap = new HashMap<Long, MESBasicProdattrel>();
		for (MESBasicProdattrel prodattrel : result) {
			ids.add(prodattrel.getId());
			rsMap.put(prodattrel.getId(), prodattrel);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(prodattrelDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = prodattrelDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicProdattrel prodattrel,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(prodattrel.getOwnerStaffId());
		ti.setOwnerPositionId(prodattrel.getOwnerPositionId());
		ti.setOwnerDepartmentId(prodattrel.getOwnerDepartmentId());
		
		ti.setCreateStaff(prodattrel.getCreateStaff());
		ti.setCreateStaffId(prodattrel.getCreateStaffId());
		ti.setCreateTime(prodattrel.getCreateTime());
		ti.setDeleteStaff(prodattrel.getDeleteStaff());
		ti.setDeleteStaffId(prodattrel.getDeleteStaffId());
		ti.setDeleteTime(prodattrel.getDeleteTime());
		ti.setCreateDepartmentId(prodattrel.getCreateDepartmentId());
		ti.setModifyStaff(prodattrel.getModifyStaff());
		ti.setModifyStaffId(prodattrel.getModifyStaffId());
		ti.setCreatePositionId(prodattrel.getCreatePositionId());
		ti.setPositionLayRec(prodattrel.getPositionLayRec());
		ti.setTableNo(prodattrel.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_prodatriel");
		ti.setTargetTableName("S2BASE_PRODATTREL");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_prodatriel_Prodattrel" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  prodattrelDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodattrel\"","MESBasic_1_prodatriel","","S2BASE_PRODATTREL",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"prodattrel\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"prodattrel\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodattrel\"","MESBasic_1_prodatriel","","S2BASE_PRODATTREL",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("prodattrel.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"prodattrel\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"prodattrel\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"prodattrel\".CID = 1 OR \"prodattrel\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("prodattrel.mc\"").append(".PRODATTREL) not in ("+notinValue+")");
			}
			
			SQLQuery query = prodattrelDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("prodatriel", "MESBasic_1_prodatriel", "prodatriel_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=prodattrelDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicProdattrel.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicProdattrel.MODULE_CODE;}
	protected String getModelCode(){return MESBasicProdattrel.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param prodattrel 模型实例
	 */
	public void checkUniqueConstraint(MESBasicProdattrel prodattrel){
	}
	
	
	@Override
	public List<MESBasicProdattrel> getProdattrels(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdattrel.class);
		if(params!=null){
			...
			return prodattrelDao.findByHql("from " + MESBasicProdattrel.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return prodattrelDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicProdattrel> findProdattrelsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdattrel.class);
	...
	@Override
	public List<MESBasicProdattrel> findProdattrelsByHql(String hql, Object... objects){
		
		return   prodattrelDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicProdattrel loadProdattrelByBussinessKey(MESBasicProdattrel prodattrel){
		return loadProdattrelByBussinessKey(prodattrel.getId());
	}
	
	@Override
	public Page<MESBasicProdattrel> getProdattrels(Page<MESBasicProdattrel> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdattrel.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = prodattrelDao.findByPage(page, sql);
			}else{
				page = prodattrelDao.findAllByPage(page);
			}
			return page;
		}
		page = prodattrelDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdattrel loadProdattrelByBussinessKey(Serializable bussinessKey){
		return prodattrelDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdattrel> findByProperty(String propertyName, Object object){
		return prodattrelDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdattrel findEntityByProperty(String propertyName, Object object){
		return prodattrelDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicProdattrel.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = prodattrelDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_prodatriel_Prodattrel","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_prodatriel", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicProdattrel prodattrel,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,prodattrelDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicProdattrel.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, prodattrel);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",prodattrel.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdattrel.TABLE_NAME +" where valid = 1";
			list = prodattrelDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdattrel.TABLE_NAME;
			list = prodattrelDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicProdattrel.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = prodattrelDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicProdattrelDaoImpl prodattrelDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProdattrel(final List<MESBasicProdattrel> insertObjs, final List<MESBasicProdattrel> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProdattrel(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProdattrel(final List<MESBasicProdattrel> insertObjs, final List<MESBasicProdattrel> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1482909899954"),importCount));
		...
		Map<Long, MESBasicProdattrel> oldUpdateObjsMap = new HashMap<Long, MESBasicProdattrel>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicProdattrel bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicProdattrel> bizObjList = prodattrelDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicProdattrel bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicProdattrel> nmeObjs = new ArrayList<MESBasicProdattrel>();
		...
		long id=autoGeneratorID.getNextId(MESBasicProdattrel.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicProdattrel prodattrel:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicProdattrel.TABLE_NAME,1,dbId);
			if(prodattrel.getId() != null ){
				m.put(prodattrel.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) prodattrelDao.createNativeQuery(sql, "MESBasic_1_prodatriel_Prodattrel"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		prodattrelDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicProdattrel.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicProdattrel prodattrel = updateObjs.get(updateIndex);
						Long id = prodattrel.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = prodattrel.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicProdattrel.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicProdattrel.TABLE_NAME + " order by id desc";
//					Query query = prodattrelDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicProdattrel prodattrel = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = prodattrel.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		prodattrelDaoImpl.getSession().flush();
		prodattrelDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicProdattrel bizObj = insertObjs.get(insertIndex);
			prodattrelImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicProdattrel bizObj = updateObjs.get(updateIndex);
			MESBasicProdattrel oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			prodattrelImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = prodattrelDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				prodattrelDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = prodattrelDao.createNativeQuery(sql, "MESBasic_1_prodatriel_Prodattrel").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = prodattrelDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = prodattrelDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = prodattrelDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?prodattrelDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  prodattrelDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,prodattrelDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="ec.flow.save")
	public void saveProdattrel(MESBasicProdattrel prodattrel, DataGridService dataGridService){
		saveProdattrel(prodattrel, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="ec.flow.save")
	public void saveProdattrel(MESBasicProdattrel prodattrel, DataGridService dataGridService, String viewCode){
		saveProdattrel(prodattrel, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel")
	public void saveProdattrel(MESBasicProdattrel prodattrel, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProdattrel(prodattrel, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel")
	public void saveProdattrel(MESBasicProdattrel prodattrel, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_prodatriel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_prodatriel");
		String url = null;
		if(null != prodattrel.getId() && prodattrel.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdattrel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel")
	public Map<Object, Object> prodattrelDataGridImport(MESBasicProdattrel prodattrel, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = prodattrel.getVirtualId();
		
		prodattrel = prodattrelDao.load(prodattrel.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, prodattrel.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = prodattrel.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(prodattrel);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, prodattrel.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel")
	public void mergeProdattrel(MESBasicProdattrel prodattrel, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(prodattrel);
		beforeSaveProdattrel(prodattrel);
		prodattrelDao.merge(prodattrel);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodatriel",model="MESBasic_1_prodatriel_Prodattrel",desc="foundation.batch.save",operType="4")
	public void batchSaveProdattrel(MESBasicProdattrel prodattrel, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_prodatriel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_prodatriel");
	...
	@Override
	public Page<MESBasicProdattrel> getByPage(Page<MESBasicProdattrel> page,DetachedCriteria detachedCriteria){
		return prodattrelDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_prodatriel").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveProdattrel(MESBasicProdattrel prodattrel, Object... objects){
		checkUniqueConstraint(prodattrel);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProdattrel(MESBasicProdattrel prodattrel, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProdattrel(List<MESBasicProdattrel> prodattrels, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProdattrel(List<MESBasicProdattrel> prodattrels){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProdattrel(List<MESBasicProdattrel> prodattrels, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProdattrel(List<MESBasicProdattrel> prodattrels){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProdattrel(MESBasicProdattrel prodattrel){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProdattrel(MESBasicProdattrel prodattrel){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProdattrel(MESBasicProdattrel prodattrel){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProdattrel(MESBasicProdattrel prodattrel){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicProdattrel prodattrel, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicProdattrel prodattrel, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProdattrel(List<MESBasicProdattrel> insertObjs, List<MESBasicProdattrel> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProdattrel,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProdattrel(List<MESBasicProdattrel> insertObjs, List<MESBasicProdattrel> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProdattrel,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
