...
@Repository
public class MESBasicQACheckIndexDaoImpl extends ExtendedGenericDaoImpl<MESBasicQACheckIndex, Long> implements MESBasicQACheckIndexDao {
	@Override
	...
	@Override
	public MESBasicQACheckIndexDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicQACheckIndexDealInfo dealInfo = new MESBasicQACheckIndexDealInfo();
	...
	@Override
	public MESBasicQACheckIndexDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicQACheckIndex mainObj, Staff dealStaff) {
		MESBasicQACheckIndexDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicQACheckIndexDealInfo.class);
		...
		List<MESBasicQACheckIndexDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
