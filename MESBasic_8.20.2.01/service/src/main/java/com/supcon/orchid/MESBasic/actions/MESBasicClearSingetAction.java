...
/* CUSTOM CODE START(action,import,MESBasic_1_clearSinget_ClearSinget,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicClearSingetAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicClearSinget> {
	...
	private MESBasicClearSinget clearSinget;
	...
	//导出excel时,主模型内容
	List<MESBasicClearSinget> queryResults = new ArrayList<MESBasicClearSinget>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/clearSinget/clearSinget/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/clearSinget/clearSinget/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_clearSinget_ClearSinget" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicClearSinget> clearSingets;
	
	public List<MESBasicClearSinget> getClearSingets() {
		return clearSingets;
	}
	...
	private Page<MESBasicClearSinget> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(clearSinget.getClearSinget() != null) {
				clearSinget.setClearSinget((SystemCode) systemCodeService.getSystemCode(clearSinget.getClearSinget().getId()));
			}
		} else {
			if(id > 0){
				clearSinget = clearSingetService.getClearSinget(id, viewCode);
			}
		}
	...
	public Page<MESBasicClearSinget> getPage() {
		if (null == page) {
			page = new Page<MESBasicClearSinget>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicClearSinget getClearSinget() {
		return clearSinget;
	}		

	public void setClearSinget(MESBasicClearSinget clearSinget) {
		this.clearSinget = clearSinget;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/clearSinget/clearSinget/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = clearSingetService.getClearSingetAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/clearSinget/clearSinget/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1554194025939
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String clearSingetReferenceCopy() throws Exception{
			MESBasicClearSinget oldClearSinget = null;
			if(isEditNew()){
				...
				oldClearSinget = new MESBasicClearSinget();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,clearSinget.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, clearSinget.getId());
				...
				oldClearSinget = clearSingetService.getClearSinget(id);
			}
			
			
			clearSinget = clearSingetService.getClearSinget(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_clearSinget_ClearSinget");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicClearSinget.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldClearSinget, null);
						Method setValue = MESBasicClearSinget.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(clearSinget, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			clearSinget.setCreateDepartmentId(oldClearSinget.getCreateDepartmentId());
			clearSinget.setCreatePositionId(oldClearSinget.getCreatePositionId());
			clearSinget.setCreateStaffId(oldClearSinget.getCreateStaffId());
			clearSinget.setOwnerDepartmentId(oldClearSinget.getOwnerDepartmentId());
			clearSinget.setOwnerPositionId(oldClearSinget.getOwnerPositionId());
			clearSinget.setOwnerStaffId(oldClearSinget.getOwnerStaffId());
			clearSinget.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_clearSinget_ClearSinget", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=clearSingetService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return clearSingetReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			clearSinget = clearSingetService.getClearSinget(this.clearSinget.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,clearSinget.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = clearSingetService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(clearSingetService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(clearSingetService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_clearSinget_ClearSinget", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"clearSinget\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"clearSinget\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"bmDepartment\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"clearSinget\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"clearSinget\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			clearSingetService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		clearSingetService.findDataGridPage(dg,dgclass,getDgPage(),clearSinget,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(clearSinget.getId() != null && clearSinget.getId() != -1){
				clearSinget = clearSingetService.getClearSinget(clearSinget.getId());
			}
			clearSingetService.findDataGridPage(dg,dgclass,getDgPage(),clearSinget,searchCondition,params);
	...
	private void save() throws Exception {
		...
		clearSingetService.saveClearSinget(clearSinget,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "submit");
		bap_validate_datagrids=clearSingetService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", clearSinget.getId());
		...
		fileuploads.put("linkId", clearSinget.getId());
		bapAfterCustomMethod("MESBasic_1_clearSinget_ClearSinget", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/clearSinget/clearSinget/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		clearSingetService.deleteClearSinget(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_clearSinget_ClearSinget", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/clearSinget/clearSinget/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "restore");
		clearSingetService.restoreClearSinget(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_clearSinget_ClearSinget", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/clearSinget/clearSinget/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/clearSinget/clearSinget/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "showDealInfoData");
		if (null != tableInfoId) {
			int count=clearSingetService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = clearSingetService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = clearSingetService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_clearSinget_ClearSinget", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicClearSinget> findExportDataInfos(Page<MESBasicClearSinget> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = clearSingetService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = clearSingetService.getAssProperty(p.getCode());	
									//String assModelCode = clearSingetService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			clearSingetService.findClearSingets(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			clearSingetService.findClearSingets(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicClearSinget> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicClearSinget result : results){
	...
	public boolean isEditNew(){
		//return null == this.clearSinget || null == this.clearSinget.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/clearSinget/clearSinget/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_clearSinget_ClearSinget", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/clearSinget/clearSinget/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/clearSinget/clearSinget/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/clearSinget/clearSinget/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicClearSingetService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_clearSinget", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_clearSinget");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_clearSinget_ClearSinget", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/clearSinget/clearSinget/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_clearSinget_ClearSinget", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_clearSinget_ClearSinget", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/clearSinget/clearSinget/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/clearSinget/clearSinget/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_clearSinget_ClearSinget";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_clearSinget_ClearSinget", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/clearSinget/clearSinget/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicClearSinget clearSinget = null;
		if(null != id) {
			clearSinget = clearSingetService.getClearSinget(id);
		}
		...
		_auditLog.setColumnStringB(null == clearSinget.getId() ? "" : clearSinget.getId().toString());
		_auditLog.setColumnStringA(null == clearSinget.getId() ? "" : clearSinget.getId().toString());
		_auditLog.setColumnStringC(clearSinget.getId().toString());
		_auditLog.setColumnStringD(MESBasicClearSinget.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_clearSinget");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicClearSingetAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, clearSinget,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, clearSinget,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,clearSinget);
		...
		printlog.setEntity("MESBasic_1_clearSinget");
		...
		printlog.setBusinessId(clearSinget.getId());
		printlog.setBussinessKey(null == clearSinget.getId() ? "" : clearSinget.getId().toString());
		printlog.setMainDisplay(null == clearSinget.getId() ? "" : clearSinget.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/clearSinget/clearSinget/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(clearSinget,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/clearSinget/clearSinget/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(clearSingetService.getClearSinget(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/clearSinget/clearSinget/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/clearSinget/clearSinget/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, clearSinget,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				clearSinget = clearSingetService.getClearSinget(Long.parseLong(arrId[i]));
				if(null != clearSinget){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == clearSinget.getId() ? "" : "," + clearSinget.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == clearSinget.getId() ? "" : clearSinget.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == clearSinget.getId() ? "" : "," + clearSinget.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == clearSinget.getId() ? "" : clearSinget.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == clearSinget.getId() ? "" : "," + clearSinget.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == clearSinget.getId() ? "" : clearSinget.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, clearSinget,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,clearSinget);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/clearSinget/clearSinget/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicClearSinget> findExportPageInfos(Page<MESBasicClearSinget> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_clearSinget" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = clearSingetService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicClearSinget queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = clearSingetService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = clearSingetService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_clearSinget_ClearSinget");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_clearSinget_ClearSinget")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_clearSinget_ClearSinget");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_clearSinget_ClearSinget");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = clearSingetService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_clearSinget_ClearSinget,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_clearSinget_ClearSinget,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_clearSinget_ClearSinget,MESBasic_1) */
// 自定义代码
