...
import com.supcon.orchid.MESBasic.entities.MESBasicBaseCustomerMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicBaseCustomerService;
import com.supcon.orchid.MESBasic.services.MESBasicBaseCustomerImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicBaseCustomerDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_baseCustomerService")
@Transactional
public class MESBasicBaseCustomerServiceImpl  extends BaseServiceImpl<MESBasicBaseCustomer> implements MESBasicBaseCustomerService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicBaseCustomer> bapBaseService;
	...
	@Autowired
	private MESBasicBaseCustomerImportService baseCustomerImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicBaseCustomer.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  baseCustomerDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicBaseCustomer.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = baseCustomerDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBaseCustomer.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBaseCustomer.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  baseCustomerDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicBaseCustomer.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicBaseCustomer.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicBaseCustomer> list = (List<MESBasicBaseCustomer>) baseCustomerDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseCustomer getBaseCustomer(long id){
		return getBaseCustomer(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseCustomer getBaseCustomer(long id, String viewCode){
		MESBasicBaseCustomer baseCustomer = baseCustomerDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBaseCustomerAsJSON(long id, String include){
		MESBasicBaseCustomer baseCustomer = baseCustomerDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(baseCustomer, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.common.delete",operType="3")
	public void deleteBaseCustomer(MESBasicBaseCustomer baseCustomer){
		if(null != baseCustomer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseCustomer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseCustomer.getCustomerCode() ? "" : baseCustomer.getCustomerCode().toString());
			AuditUtil.setColumnStringA(null == baseCustomer.getCustomerName() ? "" : baseCustomer.getCustomerName().toString());
			AuditUtil.setColumnStringC(baseCustomer.getId().toString());
		}
		beforeDeleteBaseCustomer(baseCustomer);
		baseCustomerDao.delete(baseCustomer);
		afterDeleteBaseCustomer(baseCustomer);
		baseCustomerDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseCustomer/del", baseCustomer,MESBasicBaseCustomerMneCode.class);
		List<MESBasicBaseCustomer> params = new ArrayList<MESBasicBaseCustomer>();
		params.add(baseCustomer);
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.common.delete",operType="3")
	public void deleteBaseCustomer(List<Long> baseCustomerIds){
		deleteBaseCustomer(baseCustomerIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.common.delete",operType="3")
	public void deleteBaseCustomer(List<Long> baseCustomerIds, String eventTopic) {
		List<MESBasicBaseCustomer> baseCustomers = new ArrayList<MESBasicBaseCustomer>();
		for(Long baseCustomerId : baseCustomerIds){
			MESBasicBaseCustomer baseCustomer = getBaseCustomer(baseCustomerId);
			baseCustomers.add(baseCustomer);
			if(null != baseCustomer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseCustomer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == baseCustomer.getCustomerName() ? "" : "," + baseCustomer.getCustomerName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == baseCustomer.getCustomerName() ? "" : baseCustomer.getCustomerName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == baseCustomer.getCustomerCode() ? "" : "," + baseCustomer.getCustomerCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == baseCustomer.getCustomerCode() ? "" : baseCustomer.getCustomerCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == baseCustomer.getId() ? "" : "," + baseCustomer.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == baseCustomer.getId() ? "" : baseCustomer.getId().toString());
				}
			}
		}
		
		for(MESBasicBaseCustomer baseCustomer : baseCustomers){
			beforeDeleteBaseCustomer(baseCustomer);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(baseCustomers != null && baseCustomers.size() > 0 ){
			for(MESBasicBaseCustomer baseCustomer : baseCustomers){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),baseCustomer, baseCustomer.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(baseCustomerIds != null && baseCustomerIds.size() > 0) {
			String hql = "update " + MESBasicBaseCustomer.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = baseCustomerDao.createQuery(hql);
			query.setParameterList("ids", baseCustomerIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicBaseCustomer baseCustomerz : baseCustomers){
				baseCustomerz.setValid(false);
				List<MESBasicBaseCustomer> params = new ArrayList<MESBasicBaseCustomer>();
				params.add(baseCustomerz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_baseCustomer");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.common.delete",operType="3")
	public void deleteBaseCustomer(long baseCustomerId, int baseCustomerVersion){
		this.deleteBaseCustomer(baseCustomerId, baseCustomerVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.common.delete",operType="3")
	public void deleteBaseCustomer(long baseCustomerId, int baseCustomerVersion,SignatureLog signatureLog){
		MESBasicBaseCustomer baseCustomer = getBaseCustomer(baseCustomerId);
		if(baseCustomer != null && !baseCustomer.isValid()){
		...
		beforeDeleteBaseCustomer(baseCustomer);
		baseCustomerDao.delete(baseCustomerId, baseCustomerVersion);
		afterDeleteBaseCustomer(baseCustomer);
		baseCustomerDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseCustomer/del", baseCustomer,MESBasicBaseCustomerMneCode.class);
		List<MESBasicBaseCustomer> params = new ArrayList<MESBasicBaseCustomer>();
		...
		if(signatureLog != null ) {
			...
			businessKey=baseCustomer.getCustomerCode();
			signatureLog.setTableId(baseCustomer.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.common.delete",operType="3")
	public void deleteBaseCustomer(String baseCustomerIds){
		this.deleteBaseCustomer(baseCustomerIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.common.delete",operType="3")
	public void deleteBaseCustomer(String baseCustomerIds,SignatureLog signatureLog){
		deleteCollection(baseCustomerIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBaseCustomer(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_baseCustomer#modelCode:MESBasic_1_baseCustomer_BaseCustomer#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="ec.engine.restore",operType="10")
	public void restoreBaseCustomer(String baseCustomerIds){
		restoreCollection(baseCustomerIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBaseCustomer(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="ec.engine.restore",operType="10")
	public void restoreBaseCustomer(long baseCustomerId){
		
		findBusinessKeyUsed(baseCustomerId);	//判断业务主键是否重复
		...
		beforeRestoreBaseCustomer(baseCustomer);
		baseCustomer.setValid(true);
		baseCustomerDao.update(baseCustomer);
		afterRestoreBaseCustomer(baseCustomer);
		baseCustomerDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseCustomer/add", baseCustomer,MESBasicBaseCustomerMneCode.class);
		List<MESBasicBaseCustomer> params = new ArrayList<MESBasicBaseCustomer>();
		params.add(baseCustomer);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long baseCustomerId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_baseCustomer_BaseCustomer");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicBaseCustomer.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicBaseCustomer.TABLE_NAME +" where id = ? )";
			List<Object> list =  baseCustomerDao.createNativeQuery(sql,baseCustomerId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBaseCustomer(List<MESBasicBaseCustomer>  baseCustomers){
		for(MESBasicBaseCustomer baseCustomer:baseCustomers)  {
			saveBaseCustomer(baseCustomer, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBaseCustomer(List<MESBasicBaseCustomer>  baseCustomers){
		for(MESBasicBaseCustomer baseCustomer:baseCustomers)  {
			saveBaseCustomer(baseCustomer, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="ec.flow.save")
	public void saveBaseCustomer(MESBasicBaseCustomer baseCustomer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBaseCustomer(baseCustomer, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="ec.flow.save")
	public void saveBaseCustomer(MESBasicBaseCustomer baseCustomer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBaseCustomer(baseCustomer, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBaseCustomerCodes(MESBasicBaseCustomer baseCustomer) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBaseCustomerCodes(baseCustomer, false);
	}
	
	public void generateBaseCustomerCodes(MESBasicBaseCustomer baseCustomer, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBaseCustomerSummarys(MESBasicBaseCustomer baseCustomer) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBaseCustomerSummarys(baseCustomer, false);
	}

	public void generateBaseCustomerSummarys(MESBasicBaseCustomer baseCustomer, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer")
	public void saveBaseCustomer(MESBasicBaseCustomer baseCustomer,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBaseCustomer(baseCustomer, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer")
	public void saveBaseCustomer(MESBasicBaseCustomer baseCustomer,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_baseCustomer");
		String url = null;
		if(baseCustomer.getId() != null && baseCustomer.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseCustomer.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, baseCustomer);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(baseCustomer);
		beforeSaveBaseCustomer(baseCustomer, viewIsView);

		if (viewIsView) {
			baseCustomerDao.saveWithRevertVersion(baseCustomer);
		} else {
			if(isNew)
				baseCustomerDao.save(baseCustomer);
			else
				baseCustomerDao.merge(baseCustomer);
		}
		if(null != baseCustomer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseCustomer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseCustomer.getCustomerCode() ? "" : baseCustomer.getCustomerCode().toString());
			AuditUtil.setColumnStringA(null == baseCustomer.getCustomerName() ? "" : baseCustomer.getCustomerName().toString());
		...
		dealDatagridsSave(baseCustomer,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBaseCustomerCodes(baseCustomer, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBaseCustomerSummarys(baseCustomer, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBaseCustomer(baseCustomer, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, baseCustomer);
			}
		}
		List<MESBasicBaseCustomer> params = new ArrayList<MESBasicBaseCustomer>();
		baseCustomerDao.flush();
		baseCustomerDao.clear();
		baseCustomer = baseCustomerDao.load(baseCustomer.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicBaseCustomer baseCustomer){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("baseCustomer", baseCustomer);
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer")
	public void mergeBaseCustomer(MESBasicBaseCustomer baseCustomer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(baseCustomer);
		beforeSaveBaseCustomer(baseCustomer);
		baseCustomerDao.merge(baseCustomer);
		if(null != baseCustomer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseCustomer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseCustomer.getCustomerCode() ? "" : baseCustomer.getCustomerCode().toString());
			AuditUtil.setColumnStringA(null == baseCustomer.getCustomerName() ? "" : baseCustomer.getCustomerName().toString());
		...
		dealDatagridsSave(baseCustomer,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBaseCustomer(baseCustomer);
		baseCustomerDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseCustomer/add", baseCustomer,MESBasicBaseCustomerMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicBaseCustomer> findBaseCustomers(Page<MESBasicBaseCustomer> page, Criterion... criterions) {
		return baseCustomerDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicBaseCustomerDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicBaseCustomerDealInfo> list=baseCustomerDao.createQuery("from " + MESBasicBaseCustomerDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicBaseCustomerDealInfo dealInfo) {
		baseCustomerDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicBaseCustomerDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) baseCustomerDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicBaseCustomerDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = baseCustomerDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = baseCustomerDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicBaseCustomerDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = baseCustomerDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseCustomers(Page<MESBasicBaseCustomer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBaseCustomers(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseCustomers(Page<MESBasicBaseCustomer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseCustomers(Page<MESBasicBaseCustomer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicBaseCustomer> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"baseCustomer\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"baseCustomer\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"baseCustomer\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_baseCustomer_BaseCustomer" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_baseCustomer_BaseCustomer" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  baseCustomerDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  baseCustomerDao.createNativeQuery(powerOperateSql, "MESBasic_1_baseCustomer").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseCustomer\"","MESBasic_1_baseCustomer",customCondition,"S2BASE_CUSTOMER",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"baseCustomer\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicBaseCustomerDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"baseCustomer\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"baseCustomer\".LAY_REC ASC, \"baseCustomer\".SORT ASC");
				} else {
					orderPart.append(" \"baseCustomer\".CUSTOMER_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) baseCustomerDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=baseCustomerDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = baseCustomerDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicBaseCustomer> result = new ArrayList<MESBasicBaseCustomer>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicBaseCustomer.class,baseCustomerDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicBaseCustomer.class,baseCustomerDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicBaseCustomer baseCustomer : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(baseCustomer.getId(), "MESBasic_baseCustomer_baseCustomer");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(baseCustomer.getId(), "MESBasic_baseCustomer_baseCustomer");
							if(null != documents && !documents.isEmpty()) {
								baseCustomer.setDocument(documents.get(0));
								baseCustomer.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_baseCustomer")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_baseCustomer")+"MESBasic_1_baseCustomer".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_baseCustomer",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicBaseCustomer> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicBaseCustomer> result) {
		View view = null;
		...
		Map<Long, MESBasicBaseCustomer> rsMap = new HashMap<Long, MESBasicBaseCustomer>();
		for (MESBasicBaseCustomer baseCustomer : result) {
			ids.add(baseCustomer.getId());
			rsMap.put(baseCustomer.getId(), baseCustomer);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(baseCustomerDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = baseCustomerDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicBaseCustomer baseCustomer,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(baseCustomer.getOwnerStaffId());
		ti.setOwnerPositionId(baseCustomer.getOwnerPositionId());
		ti.setOwnerDepartmentId(baseCustomer.getOwnerDepartmentId());
		
		ti.setCreateStaff(baseCustomer.getCreateStaff());
		ti.setCreateStaffId(baseCustomer.getCreateStaffId());
		ti.setCreateTime(baseCustomer.getCreateTime());
		ti.setDeleteStaff(baseCustomer.getDeleteStaff());
		ti.setDeleteStaffId(baseCustomer.getDeleteStaffId());
		ti.setDeleteTime(baseCustomer.getDeleteTime());
		ti.setCreateDepartmentId(baseCustomer.getCreateDepartmentId());
		ti.setModifyStaff(baseCustomer.getModifyStaff());
		ti.setModifyStaffId(baseCustomer.getModifyStaffId());
		ti.setCreatePositionId(baseCustomer.getCreatePositionId());
		ti.setPositionLayRec(baseCustomer.getPositionLayRec());
		ti.setTableNo(baseCustomer.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_baseCustomer");
		ti.setTargetTableName("S2BASE_CUSTOMER");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_baseCustomer_BaseCustomer" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  baseCustomerDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseCustomer\"","MESBasic_1_baseCustomer","","S2BASE_CUSTOMER",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"baseCustomer\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"baseCustomer\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseCustomer\"","MESBasic_1_baseCustomer","","S2BASE_CUSTOMER",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("baseCustomer.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"baseCustomer\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"baseCustomer\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"baseCustomer\".CID = 1 OR \"baseCustomer\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("baseCustomer.mc\"").append(".BASE_CUSTOMER) not in ("+notinValue+")");
			}
			
			SQLQuery query = baseCustomerDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("baseCustomer", "MESBasic_1_baseCustomer", "baseCustomer_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=baseCustomerDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicBaseCustomer.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicBaseCustomer.MODULE_CODE;}
	protected String getModelCode(){return MESBasicBaseCustomer.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param baseCustomer 模型实例
	 */
	public void checkUniqueConstraint(MESBasicBaseCustomer baseCustomer){
		if(baseCustomer.getId()==null){
			List<MESBasicBaseCustomer> baseCustomers = baseCustomerDao.findByCriteria(Restrictions.eq("customerCode", baseCustomer.getCustomerCode()), Restrictions.eq("valid", true));
			if(baseCustomers!=null && !baseCustomers.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1449640688865", getCurrentUser().getLanguage()), baseCustomer.getCustomerCode());
			}
		} else {
			List<MESBasicBaseCustomer> baseCustomers = baseCustomerDao.findByCriteria(Restrictions.ne("id", baseCustomer.getId()), Restrictions.eq("customerCode", baseCustomer.getCustomerCode()), Restrictions.eq("valid", true));
			if(baseCustomers!=null && !baseCustomers.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1449640688865", getCurrentUser().getLanguage()), baseCustomer.getCustomerCode());
			}
	...
	@Override
	public List<MESBasicBaseCustomer> getBaseCustomers(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseCustomer.class);
		if(params!=null){
			...
			return baseCustomerDao.findByHql("from " + MESBasicBaseCustomer.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return baseCustomerDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicBaseCustomer> findBaseCustomersBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseCustomer.class);
	...
	@Override
	public List<MESBasicBaseCustomer> findBaseCustomersByHql(String hql, Object... objects){
		
		return   baseCustomerDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicBaseCustomer loadBaseCustomerByBussinessKey(MESBasicBaseCustomer baseCustomer){
		return loadBaseCustomerByBussinessKey(baseCustomer.getCustomerCode());
	}
	
	@Override
	public Page<MESBasicBaseCustomer> getBaseCustomers(Page<MESBasicBaseCustomer> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseCustomer.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = baseCustomerDao.findByPage(page, sql);
			}else{
				page = baseCustomerDao.findAllByPage(page);
			}
			return page;
		}
		page = baseCustomerDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseCustomer loadBaseCustomerByBussinessKey(Serializable bussinessKey){
		return baseCustomerDao.findEntityByCriteria(Restrictions.eq("customerCode", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicBaseCustomer> findByProperty(String propertyName, Object object){
		return baseCustomerDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseCustomer findEntityByProperty(String propertyName, Object object){
		return baseCustomerDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicBaseCustomer.JPA_NAME + " set valid=0 where customerCode in (:businessKeys)";
			Query query = baseCustomerDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_baseCustomer_BaseCustomer","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_baseCustomer", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicBaseCustomer baseCustomer,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,baseCustomerDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicBaseCustomer.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, baseCustomer);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",baseCustomer.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBaseCustomer.TABLE_NAME +" where valid = 1";
			list = baseCustomerDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBaseCustomer.TABLE_NAME;
			list = baseCustomerDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicBaseCustomer.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = baseCustomerDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicBaseCustomerDaoImpl baseCustomerDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBaseCustomer(final List<MESBasicBaseCustomer> insertObjs, final List<MESBasicBaseCustomer> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBaseCustomer(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBaseCustomer(final List<MESBasicBaseCustomer> insertObjs, final List<MESBasicBaseCustomer> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1449638984644"),importCount));
		...
		Map<Long, MESBasicBaseCustomer> oldUpdateObjsMap = new HashMap<Long, MESBasicBaseCustomer>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicBaseCustomer bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicBaseCustomer> bizObjList = baseCustomerDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicBaseCustomer bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicBaseCustomer> nmeObjs = new ArrayList<MESBasicBaseCustomer>();
		...
		long id=autoGeneratorID.getNextId(MESBasicBaseCustomer.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicBaseCustomer baseCustomer:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicBaseCustomer.TABLE_NAME,1,dbId);
			if(baseCustomer.getCustomerCode() != null && !baseCustomer.getCustomerCode().equals("") ){
				m.put(baseCustomer.getCustomerCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) baseCustomerDao.createNativeQuery(sql, "MESBasic_1_baseCustomer_BaseCustomer"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		baseCustomerDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicBaseCustomer.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicBaseCustomer baseCustomer = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = baseCustomer.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(baseCustomer);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicBaseCustomer.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
					...
					insertSql += dbId + ",CID,CUSTOMER_CREATEPERSON,CUSTOMER_CREATETIME,EFFECTIVE_STATE) VALUES (";
					for(int i = 0;i<columnMaps.size();i++){
						insertSql += "?,";
					}
					insertSql += "?,?,?,?,?)";
					
//					String idSql = "select id from " + MESBasicBaseCustomer.TABLE_NAME + " order by id desc";
//					Query query = baseCustomerDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicBaseCustomer baseCustomer = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = baseCustomer.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicBaseCustomerMneCode.class;
					...
					for(MESBasicBaseCustomer baseCustomer:updateObjs)  {
						Long id = baseCustomer.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicBaseCustomer baseCustomer:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(baseCustomer);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, baseCustomer.getId());
		...
		baseCustomerDaoImpl.getSession().flush();
		baseCustomerDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicBaseCustomer bizObj = insertObjs.get(insertIndex);
			baseCustomerImportService.saveImportDataLog(bizObj, null, "customerCode", "customerName");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicBaseCustomer bizObj = updateObjs.get(updateIndex);
			MESBasicBaseCustomer oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			baseCustomerImportService.saveImportDataLog(bizObj, oldBizObj, "customerCode", "customerName");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = baseCustomerDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				baseCustomerDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = baseCustomerDao.createNativeQuery(sql, "MESBasic_1_baseCustomer_BaseCustomer").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = baseCustomerDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = baseCustomerDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = baseCustomerDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?baseCustomerDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  baseCustomerDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,baseCustomerDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="ec.flow.save")
	public void saveBaseCustomer(MESBasicBaseCustomer baseCustomer, DataGridService dataGridService){
		saveBaseCustomer(baseCustomer, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="ec.flow.save")
	public void saveBaseCustomer(MESBasicBaseCustomer baseCustomer, DataGridService dataGridService, String viewCode){
		saveBaseCustomer(baseCustomer, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer")
	public void saveBaseCustomer(MESBasicBaseCustomer baseCustomer, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBaseCustomer(baseCustomer, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer")
	public void saveBaseCustomer(MESBasicBaseCustomer baseCustomer, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_baseCustomer";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseCustomer");
		String url = null;
		if(null != baseCustomer.getId() && baseCustomer.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseCustomer.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer")
	public Map<Object, Object> baseCustomerDataGridImport(MESBasicBaseCustomer baseCustomer, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = baseCustomer.getVirtualId();
		
		baseCustomer = baseCustomerDao.load(baseCustomer.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, baseCustomer.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = baseCustomer.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(baseCustomer);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, baseCustomer.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer")
	public void mergeBaseCustomer(MESBasicBaseCustomer baseCustomer, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(baseCustomer);
		beforeSaveBaseCustomer(baseCustomer);
		baseCustomerDao.merge(baseCustomer);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="foundation.batch.save",operType="4")
	public void batchSaveBaseCustomer(MESBasicBaseCustomer baseCustomer, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_baseCustomer";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseCustomer");
	...
	@Override
	public Page<MESBasicBaseCustomer> getByPage(Page<MESBasicBaseCustomer> page,DetachedCriteria detachedCriteria){
		return baseCustomerDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_baseCustomer").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBaseCustomer(MESBasicBaseCustomer baseCustomer, Object... objects){
		checkUniqueConstraint(baseCustomer);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBaseCustomer(MESBasicBaseCustomer baseCustomer, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBaseCustomer(List<MESBasicBaseCustomer> baseCustomers, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBaseCustomer(List<MESBasicBaseCustomer> baseCustomers){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBaseCustomer(List<MESBasicBaseCustomer> baseCustomers, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBaseCustomer(List<MESBasicBaseCustomer> baseCustomers){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBaseCustomer(MESBasicBaseCustomer baseCustomer){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
		//删除时修改启用状态；
		baseCustomer.setCustomerState("0");
/* CUSTOM CODE END */
	}
	
	private void afterDeleteBaseCustomer(MESBasicBaseCustomer baseCustomer){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBaseCustomer(MESBasicBaseCustomer baseCustomer){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBaseCustomer(MESBasicBaseCustomer baseCustomer){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicBaseCustomer baseCustomer, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicBaseCustomer baseCustomer, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBaseCustomer(List<MESBasicBaseCustomer> insertObjs, List<MESBasicBaseCustomer> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBaseCustomer,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBaseCustomer(List<MESBasicBaseCustomer> insertObjs, List<MESBasicBaseCustomer> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBaseCustomer,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
