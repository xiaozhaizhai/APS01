...
import com.supcon.orchid.MESBasic.services.MESBasicInterfaceLogWSService;
import com.supcon.orchid.MESBasic.services.MESBasicInterfaceLogService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_pimsInterface_InterfaceLog,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicInterfaceLog",targetNamespace = "http://ws.supcon.com")
public class MESBasicInterfaceLogWSServiceImpl implements MESBasicInterfaceLogWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getInterfaceLogByBusinessKey(
		...
		try{
			...
			MESBasicInterfaceLog interfaceLog = interfaceLogService.loadInterfaceLogByBussinessKey(businessKey);
			if(interfaceLog == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, interfaceLog);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateInterfaceLog(
		...
		try{
			if(interfaceLog == null){
				return set500Error(result, "调度日志不能为空。");
			}
			...
			if(!beforeUpdateInterfaceLog(interfaceLog)){
				return set404Error(result);
			}
			interfaceLogService.saveInterfaceLog(interfaceLog, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addInterfaceLog(
		...
		try{
			...
			try{
				checkInterfaceLogData(interfaceLog);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			interfaceLogService.saveInterfaceLog(interfaceLog, null);
			setSuccessResult(result, interfaceLog.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteInterfaceLogByBusinessKey(
		...
		try{
			...
			try{
				interfaceLog = interfaceLogService.loadInterfaceLogByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddInterfaceLog(
		...
		try{
			...
			int count = 1;
			for(MESBasicInterfaceLog interfaceLog : interfaceLogList){
				if(interfaceLog.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(interfaceLog.getId() == null || interfaceLog.getId().toString().trim().isEmpty()){
			...
			interfaceLogService.batchImportBaseInterfaceLog(interfaceLogList);
			for(MESBasicInterfaceLog interfaceLog : interfaceLogList){
				idList.add(interfaceLog.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteInterfaceLogByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicInterfaceLog temp = null;
				try{
					temp = interfaceLogService.getInterfaceLog(id);
				}catch(NullPointerException e){}
			...
			interfaceLogService.deleteInterfaceLog(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteInterfaceLogByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicInterfaceLog temp = null;
				try{
					temp = interfaceLogService.loadInterfaceLogByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getInterfaceLogPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicInterfaceLog.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicInterfaceLog.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicInterfaceLog.class);
				} catch (Exception e) {
			...
			Page<MESBasicInterfaceLog> page = new Page<MESBasicInterfaceLog>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicInterfaceLog> datas = interfaceLogService.getByPage(page, detachedCriteria);
			List<MESBasicInterfaceLog> interfaceLogList = datas.getResult();
			if (interfaceLogList == null || interfaceLogList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(interfaceLogList.toArray());
			PaginationDTO<MESBasicInterfaceLog> pagination = new PaginationDTO<MESBasicInterfaceLog>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicInterfaceLog interfaceLog = interfaceLogService.getInterfaceLog(id);
			if (interfaceLog == null || !interfaceLog.isValid()) {
				result.setErrorMessage("找不到调度日志");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取调度日志对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param interfaceLog
	 * @return
	 */
	private MESBasicInterfaceLog getInterfaceLog(MESBasicInterfaceLog interfaceLog) {
		if (interfaceLog.getId() == null && (interfaceLog.getId() == null || interfaceLog.getId().toString().trim() == ""))
			throw new RuntimeException("调度日志对象业务主键和id不能同时为空");
		if (interfaceLog.getId() != null) {
			MESBasicInterfaceLog result = interfaceLogService.getInterfaceLog(interfaceLog .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return interfaceLogService.loadInterfaceLogByBussinessKey(interfaceLog.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个调度日志，则返回false，否则返回true
	 * @param interfaceLog
	 */
	private boolean beforeUpdateInterfaceLog(MESBasicInterfaceLog interfaceLog) {
		MESBasicInterfaceLog temp = getInterfaceLog(interfaceLog);
		if (temp == null) {
			return false;
		}
		if(interfaceLog.getId() == null){
			interfaceLog.setId(temp.getId());		
		}
		if(interfaceLog.getId() == null){
			interfaceLog.setId(temp.getId());
		}
		
		interfaceLog.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除调度日志对象下所有辅模型对象中的调度日志对象
	 * @param interfaceLog
	 */
	private void clearCircle(MESBasicInterfaceLog interfaceLog) {
	 ...
	 */
	private void checkInterfaceLogData(MESBasicInterfaceLog interfaceLog){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_pimsInterface_InterfaceLog,MESBasic_1) */
// 自定义代码
