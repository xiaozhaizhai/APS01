...
import com.supcon.orchid.MESBasic.services.MESBasicObjPostionImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicObjPostionDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_objPostionService")
@Transactional
public class MESBasicObjPostionServiceImpl  extends BaseServiceImpl<MESBasicObjPostion> implements MESBasicObjPostionService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicObjPostion> bapBaseService;
	...
	@Autowired
	private MESBasicObjPostionImportService objPostionImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicObjPostion.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  objPostionDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicObjPostion.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = objPostionDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjPostion.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjPostion.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  objPostionDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicObjPostion.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicObjPostion.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicObjPostion> list = (List<MESBasicObjPostion>) objPostionDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicObjPostion getObjPostion(long id){
		return getObjPostion(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjPostion getObjPostion(long id, String viewCode){
		MESBasicObjPostion objPostion = objPostionDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getObjPostionAsJSON(long id, String include){
		MESBasicObjPostion objPostion = objPostionDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(objPostion, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.common.delete",operType="3")
	public void deleteObjPostion(MESBasicObjPostion objPostion){
		if(null != objPostion && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjPostion.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objPostion.getId() ? "" : objPostion.getId().toString());
			AuditUtil.setColumnStringA(null == objPostion.getId() ? "" : objPostion.getId().toString());
			AuditUtil.setColumnStringC(objPostion.getId().toString());
		}
		beforeDeleteObjPostion(objPostion);
		objPostionDao.deletePhysical(objPostion);
		afterDeleteObjPostion(objPostion);
		objPostionDao.flush();
		List<MESBasicObjPostion> params = new ArrayList<MESBasicObjPostion>();
		params.add(objPostion);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.common.delete",operType="3")
	public void deleteObjPostion(List<Long> objPostionIds){
		deleteObjPostion(objPostionIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.common.delete",operType="3")
	public void deleteObjPostion(List<Long> objPostionIds, String eventTopic) {
		List<MESBasicObjPostion> objPostions = new ArrayList<MESBasicObjPostion>();
		for(Long objPostionId : objPostionIds){
			MESBasicObjPostion objPostion = getObjPostion(objPostionId);
			objPostions.add(objPostion);
			if(null != objPostion && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjPostion.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == objPostion.getId() ? "" : "," + objPostion.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == objPostion.getId() ? "" : objPostion.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == objPostion.getId() ? "" : "," + objPostion.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == objPostion.getId() ? "" : objPostion.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == objPostion.getId() ? "" : "," + objPostion.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == objPostion.getId() ? "" : objPostion.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(objPostions != null && objPostions.size() > 0 ){
			for(MESBasicObjPostion objPostion : objPostions){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicObjPostion.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(objPostion));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),objPostion, objPostion.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(objPostionIds != null && objPostionIds.size() > 0) {
			String hql = "delete " + MESBasicObjPostion.JPA_NAME + " where id in(:ids)";
			Query query = objPostionDao.createQuery(hql);
			query.setParameterList("ids", objPostionIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicObjPostion objPostionz : objPostions){
				objPostionz.setValid(false);
				List<MESBasicObjPostion> params = new ArrayList<MESBasicObjPostion>();
				params.add(objPostionz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_factoryModel");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.common.delete",operType="3")
	public void deleteObjPostion(long objPostionId, int objPostionVersion){
		this.deleteObjPostion(objPostionId, objPostionVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.common.delete",operType="3")
	public void deleteObjPostion(long objPostionId, int objPostionVersion,SignatureLog signatureLog){
		MESBasicObjPostion objPostion = getObjPostion(objPostionId);
		...
		beforeDeleteObjPostion(objPostion);
		objPostionDao.delete(objPostionId, objPostionVersion);
		afterDeleteObjPostion(objPostion);
		objPostionDao.flush();
		List<MESBasicObjPostion> params = new ArrayList<MESBasicObjPostion>();
		params.add(objPostion);
		...
		if(signatureLog != null ) {
			...
			businessKey=objPostion.getId();
			signatureLog.setTableId(objPostion.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.common.delete",operType="3")
	public void deleteObjPostion(String objPostionIds){
		this.deleteObjPostion(objPostionIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.common.delete",operType="3")
	public void deleteObjPostion(String objPostionIds,SignatureLog signatureLog){
		deleteCollection(objPostionIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteObjPostion(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_factoryModel#modelCode:MESBasic_1_factoryModel_ObjPostion#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="ec.engine.restore",operType="10")
	public void restoreObjPostion(String objPostionIds){
		restoreCollection(objPostionIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreObjPostion(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="ec.engine.restore",operType="10")
	public void restoreObjPostion(long objPostionId){
		
		findBusinessKeyUsed(objPostionId);	//判断业务主键是否重复
		...
		beforeRestoreObjPostion(objPostion);
		objPostion.setValid(true);
		objPostionDao.update(objPostion);
		afterRestoreObjPostion(objPostion);
		List<MESBasicObjPostion> params = new ArrayList<MESBasicObjPostion>();
		params.add(objPostion);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long objPostionId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_factoryModel_ObjPostion");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicObjPostion.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicObjPostion.TABLE_NAME +" where id = ? )";
			List<Object> list =  objPostionDao.createNativeQuery(sql,objPostionId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseObjPostion(List<MESBasicObjPostion>  objPostions){
		for(MESBasicObjPostion objPostion:objPostions)  {
			saveObjPostion(objPostion, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseObjPostion(List<MESBasicObjPostion>  objPostions){
		for(MESBasicObjPostion objPostion:objPostions)  {
			saveObjPostion(objPostion, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="ec.flow.save")
	public void saveObjPostion(MESBasicObjPostion objPostion, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveObjPostion(objPostion, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="ec.flow.save")
	public void saveObjPostion(MESBasicObjPostion objPostion, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveObjPostion(objPostion, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateObjPostionCodes(MESBasicObjPostion objPostion) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjPostionCodes(objPostion, false);
	}
	
	public void generateObjPostionCodes(MESBasicObjPostion objPostion, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateObjPostionSummarys(MESBasicObjPostion objPostion) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjPostionSummarys(objPostion, false);
	}

	public void generateObjPostionSummarys(MESBasicObjPostion objPostion, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion")
	public void saveObjPostion(MESBasicObjPostion objPostion,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveObjPostion(objPostion, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion")
	public void saveObjPostion(MESBasicObjPostion objPostion,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(objPostion.getId() != null && objPostion.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjPostion.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, objPostion);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(objPostion);
		beforeSaveObjPostion(objPostion, viewIsView);

		if (viewIsView) {
			objPostionDao.saveWithRevertVersion(objPostion);
		} else {
			if(isNew)
				objPostionDao.save(objPostion);
			else
				objPostionDao.merge(objPostion);
		}
		if(null != objPostion && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjPostion.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objPostion.getId() ? "" : objPostion.getId().toString());
			AuditUtil.setColumnStringA(null == objPostion.getId() ? "" : objPostion.getId().toString());
		...
		dealDatagridsSave(objPostion,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateObjPostionCodes(objPostion, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateObjPostionSummarys(objPostion, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveObjPostion(objPostion, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, objPostion);
			}
		}
		List<MESBasicObjPostion> params = new ArrayList<MESBasicObjPostion>();
		objPostionDao.flush();
		objPostionDao.clear();
		objPostion = objPostionDao.load(objPostion.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicObjPostion objPostion){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("objPostion", objPostion);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion")
	public void mergeObjPostion(MESBasicObjPostion objPostion, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(objPostion);
		beforeSaveObjPostion(objPostion);
		objPostionDao.merge(objPostion);
		if(null != objPostion && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjPostion.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objPostion.getId() ? "" : objPostion.getId().toString());
			AuditUtil.setColumnStringA(null == objPostion.getId() ? "" : objPostion.getId().toString());
		...
		dealDatagridsSave(objPostion,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveObjPostion(objPostion);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicObjPostion> findObjPostions(Page<MESBasicObjPostion> page, Criterion... criterions) {
		return objPostionDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjPostions(Page<MESBasicObjPostion> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findObjPostions(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjPostions(Page<MESBasicObjPostion> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjPostions(Page<MESBasicObjPostion> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicObjPostion> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"objPostion\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"objPostion\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"objPostion\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"objPostion\".OBJ_FACTORY IN ( SELECT ID FROM factory_tbcompanystructure \"factoryModel\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  objPostionDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  objPostionDao.createNativeQuery(powerOperateSql, "MESBasic_1_factoryModel").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"objPostion\".LAY_REC ASC, \"objPostion\".SORT ASC");
				} else {
					orderPart.append(" \"objPostion\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) objPostionDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=objPostionDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = objPostionDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicObjPostion> result = new ArrayList<MESBasicObjPostion>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjPostion.class,objPostionDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjPostion.class,objPostionDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicObjPostion objPostion : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(objPostion.getId(), "MESBasic_factoryModel_objPostion");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(objPostion.getId(), "MESBasic_factoryModel_objPostion");
							if(null != documents && !documents.isEmpty()) {
								objPostion.setDocument(documents.get(0));
								objPostion.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicObjPostion> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicObjPostion> result) {
		View view = null;
		...
		Map<Long, MESBasicObjPostion> rsMap = new HashMap<Long, MESBasicObjPostion>();
		for (MESBasicObjPostion objPostion : result) {
			ids.add(objPostion.getId());
			rsMap.put(objPostion.getId(), objPostion);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(objPostionDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = objPostionDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_factoryModel_ObjPostion" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_factoryModel_ObjPostion" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  objPostionDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objPostion\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_POSTIONS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"objPostion\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"objPostion\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objPostion\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_POSTIONS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_factoryModel_ObjPostion" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("objPostion.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"objPostion\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"objPostion\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"objPostion\".CID = 1 OR \"objPostion\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("objPostion.mc\"").append(".OBJ_POSTION) not in ("+notinValue+")");
			}
			
			SQLQuery query = objPostionDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=objPostionDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicObjPostion.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicObjPostion.MODULE_CODE;}
	protected String getModelCode(){return MESBasicObjPostion.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param objPostion 模型实例
	 */
	public void checkUniqueConstraint(MESBasicObjPostion objPostion){
	}
	
	
	@Override
	public List<MESBasicObjPostion> getObjPostions(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjPostion.class);
		if(params!=null){
			...
			return objPostionDao.findByHql("from " + MESBasicObjPostion.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return objPostionDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicObjPostion> findObjPostionsByHql(String hql, Object... objects){
		...
		return   objPostionDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicObjPostion loadObjPostionByBussinessKey(MESBasicObjPostion objPostion){
		return loadObjPostionByBussinessKey(objPostion.getId());
	}
	
	@Override
	public Page<MESBasicObjPostion> getObjPostions(Page<MESBasicObjPostion> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjPostion.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = objPostionDao.findByPage(page, sql);
			}else{
				page = objPostionDao.findAllByPage(page);
			}
			return page;
		}
		page = objPostionDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjPostion> findByTreeDataGridFactoryModel(List<MESBasicObjPostion> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicObjPostion objPostion : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFactoryModel(Page<MESBasicObjPostion> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicObjPostion> objPostions = dgPage.getResult();
			for(MESBasicObjPostion objPostion : objPostions){
				
			}
			dgPage.setResult(objPostions);
		}
	...
	public MESBasicObjPostion loadObjPostionByBussinessKey(Serializable bussinessKey){
		return objPostionDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjPostion> findByProperty(String propertyName, Object object){
		return objPostionDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjPostion findEntityByProperty(String propertyName, Object object){
		return objPostionDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicObjPostion.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = objPostionDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_factoryModel_ObjPostion","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicObjPostion objPostion,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,objPostionDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicObjPostion.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, objPostion);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",objPostion.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjPostion.TABLE_NAME +" where valid = 1";
			list = objPostionDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjPostion.TABLE_NAME;
			list = objPostionDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicObjPostion.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = objPostionDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicObjPostionDaoImpl objPostionDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchObjPostion(final List<MESBasicObjPostion> insertObjs, final List<MESBasicObjPostion> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchObjPostion(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchObjPostion(final List<MESBasicObjPostion> insertObjs, final List<MESBasicObjPostion> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1460950588653"),importCount));
		...
		Map<Long, MESBasicObjPostion> oldUpdateObjsMap = new HashMap<Long, MESBasicObjPostion>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicObjPostion bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicObjPostion> bizObjList = objPostionDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicObjPostion bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicObjPostion> nmeObjs = new ArrayList<MESBasicObjPostion>();
		...
		long id=autoGeneratorID.getNextId(MESBasicObjPostion.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicObjPostion objPostion:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicObjPostion.TABLE_NAME,1,dbId);
			if(objPostion.getId() != null ){
				m.put(objPostion.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) objPostionDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjPostion"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		objPostionDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicObjPostion.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicObjPostion objPostion = updateObjs.get(updateIndex);
						Long id = objPostion.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = objPostion.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicObjPostion.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicObjPostion.TABLE_NAME + " order by id desc";
//					Query query = objPostionDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicObjPostion objPostion = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = objPostion.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		objPostionDaoImpl.getSession().flush();
		objPostionDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicObjPostion bizObj = insertObjs.get(insertIndex);
			objPostionImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicObjPostion bizObj = updateObjs.get(updateIndex);
			MESBasicObjPostion oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			objPostionImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicObjPostion> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicObjPostion> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) objPostionDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicObjPostion> result = getResult(page, query, new PendingResultTransformer(MESBasicObjPostion.class,objPostionDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicObjPostion model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = objPostionDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjPostion").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = objPostionDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = objPostionDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = objPostionDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?objPostionDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  objPostionDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,objPostionDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="ec.flow.save")
	public void saveObjPostion(MESBasicObjPostion objPostion, DataGridService dataGridService){
		saveObjPostion(objPostion, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="ec.flow.save")
	public void saveObjPostion(MESBasicObjPostion objPostion, DataGridService dataGridService, String viewCode){
		saveObjPostion(objPostion, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion")
	public void saveObjPostion(MESBasicObjPostion objPostion, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveObjPostion(objPostion, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion")
	public void saveObjPostion(MESBasicObjPostion objPostion, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(null != objPostion.getId() && objPostion.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjPostion.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion")
	public Map<Object, Object> objPostionDataGridImport(MESBasicObjPostion objPostion, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = objPostion.getVirtualId();
		
		objPostion = objPostionDao.load(objPostion.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, objPostion.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = objPostion.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(objPostion);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, objPostion.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion")
	public void mergeObjPostion(MESBasicObjPostion objPostion, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(objPostion);
		beforeSaveObjPostion(objPostion);
		objPostionDao.merge(objPostion);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjPostion",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicObjPostion> getByPage(Page<MESBasicObjPostion> page,DetachedCriteria detachedCriteria){
		return objPostionDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveObjPostion(MESBasicObjPostion objPostion, Object... objects){
		checkUniqueConstraint(objPostion);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveObjPostion(MESBasicObjPostion objPostion, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteObjPostion(MESBasicObjPostion objPostion){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteObjPostion(MESBasicObjPostion objPostion){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreObjPostion(MESBasicObjPostion objPostion){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreObjPostion(MESBasicObjPostion objPostion){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicObjPostion objPostion, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicObjPostion objPostion, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportObjPostion(List<MESBasicObjPostion> insertObjs, List<MESBasicObjPostion> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportObjPostion,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportObjPostion(List<MESBasicObjPostion> insertObjs, List<MESBasicObjPostion> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportObjPostion,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_factoryModel_ObjPostion,MESBasic_1) */
// 自定义代码
