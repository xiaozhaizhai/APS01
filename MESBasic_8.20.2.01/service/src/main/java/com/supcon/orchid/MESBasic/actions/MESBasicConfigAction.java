...
/* CUSTOM CODE START(action,import,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicConfigAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicConfig> {
	...
	//导出excel时,主模型内容
	List<MESBasicConfig> queryResults = new ArrayList<MESBasicConfig>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/routine/config/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/routine/config/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_routine_Config" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicConfig> configs;
	
	public List<MESBasicConfig> getConfigs() {
		return configs;
	}
	...
	private Page<MESBasicConfig> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			config = new MESBasicConfig();
			if(config.getMeasurement() != null) {
				config.setMeasurement((SystemCode) systemCodeService.getSystemCode(config.getMeasurement().getId()));
			}
			if(config.getPumpBelong() != null) {
				config.setPumpBelong((SystemCode) systemCodeService.getSystemCode(config.getPumpBelong().getId()));
			}
		} else {
			if(id > 0){
				config = configService.getConfig(id, viewCode);
			}
		}
	...
	public Page<MESBasicConfig> getPage() {
		if (null == page) {
			page = new Page<MESBasicConfig>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicConfig getConfig() {
		return config;
	}		

	public void setConfig(MESBasicConfig config) {
		this.config = config;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/routine/config/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = configService.getConfigAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/routine/config/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.radion1415337833221
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String configReferenceCopy() throws Exception{
			MESBasicConfig oldConfig = null;
			if(isEditNew()){
				...
				oldConfig = new MESBasicConfig();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,config.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, config.getId());
				...
				oldConfig = configService.getConfig(id);
			}
			
			
			config = configService.getConfig(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_routine_Config");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicConfig.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldConfig, null);
						Method setValue = MESBasicConfig.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(config, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			config.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_routine_Config", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=configService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return configReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			config = configService.getConfig(this.config.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,config.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = configService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(configService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(configService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_routine_Config", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/config/configRefForBatch", results = { @Result(type = FREEMARKER, location = "configRefForBatch.ftl") })
		,
		@Action(value = "/MESBasic/routine/config/routineConfigRef", results = { @Result(type = FREEMARKER, location = "routineConfigRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"config\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"config\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"config\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"config\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/config/configRefForBatch-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.routine.code,result.routine.name,result.name,result.formula,result.routine.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="configRefForBatch-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/routine/config/routineConfigRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.routine.code,result.routine.name,result.name,result.formula,result.pumpBelong.id,result.pumpBelong.value,result.forSource,result.forDestination,result.routine.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="routineConfigRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		configService.findDataGridPage(dg,dgclass,getDgPage(),config,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			if(config.getId() != null && config.getId() != -1){
				config = configService.getConfig(config.getId());
			}
			configService.findDataGridPage(dg,dgclass,getDgPage(),config,searchCondition,params);
	...
	private void save() throws Exception {
		...
		configService.saveConfig(config,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_routine_Config", "submit");
		bap_validate_datagrids=configService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		...
		fileuploads.put("linkId", config.getId());
		bapAfterCustomMethod("MESBasic_1_routine_Config", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/routine/config/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Config", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		configService.deleteConfig(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_routine_Config", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/config/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Config", "restore");
		configService.restoreConfig(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_routine_Config", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicConfig> findExportDataInfos(Page<MESBasicConfig> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = configService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = configService.getAssProperty(p.getCode());	
									//String assModelCode = configService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			configService.findConfigs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			configService.findConfigs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicConfig> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicConfig result : results){
	...
	public boolean isEditNew(){
		//return null == this.config || null == this.config.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/routine/config/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Config", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_routine_Config", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/routine/config/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_routine_Config", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/routine/config/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/config/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicConfigService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_routine_Config", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_routine_Config", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/config/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_routine_Config", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_routine_Config", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/routine/config/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Config", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/routine/config/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_routine_Config";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_routine_Config", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/routine/config/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == config.getId() ? "" : config.getId().toString());
		_auditLog.setColumnStringA(null == config.getName() ? "" : config.getName().toString());
		_auditLog.setColumnStringC(config.getId().toString());
		_auditLog.setColumnStringD(MESBasicConfig.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicConfigAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, config,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, config,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,config);
		...
		printlog.setBusinessId(config.getId());
		printlog.setBussinessKey(null == config.getId() ? "" : config.getId().toString());
		printlog.setMainDisplay(null == config.getName() ? "" : config.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/routine/config/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(config,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/config/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(configService.getConfig(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/routine/config/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/routine/config/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, config,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				config = configService.getConfig(Long.parseLong(arrId[i]));
				if(null != config){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == config.getName() ? "" : "," + config.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == config.getName() ? "" : config.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == config.getId() ? "" : "," + config.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == config.getId() ? "" : config.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == config.getId() ? "" : "," + config.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == config.getId() ? "" : config.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, config,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,config);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/routine/config/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_routine_Config");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_routine_Config")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_routine_Config");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_routine_Config");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = configService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
