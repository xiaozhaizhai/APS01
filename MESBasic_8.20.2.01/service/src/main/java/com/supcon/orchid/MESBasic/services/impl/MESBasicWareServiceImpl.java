...
import com.supcon.orchid.MESBasic.entities.MESBasicWareMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicWareService;
import com.supcon.orchid.MESBasic.services.MESBasicWareImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicWareDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_wareService")
@Transactional
public class MESBasicWareServiceImpl  extends BaseServiceImpl<MESBasicWare> implements MESBasicWareService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicWare> bapBaseService;
	...
	@Autowired
	private MESBasicWareImportService wareImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicWare.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  wareDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicWare.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = wareDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicWare.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicWare.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  wareDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicWare.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicWare.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicWare> list = (List<MESBasicWare>) wareDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicWare getWare(long id){
		return getWare(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicWare getWare(long id, String viewCode){
		MESBasicWare ware = wareDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getWareAsJSON(long id, String include){
		MESBasicWare ware = wareDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(ware, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.common.delete",operType="3")
	public void deleteWare(MESBasicWare ware){
		if(null != ware && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicWare.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == ware.getWareCode() ? "" : ware.getWareCode().toString());
			AuditUtil.setColumnStringA(null == ware.getWareName() ? "" : ware.getWareName().toString());
			AuditUtil.setColumnStringC(ware.getId().toString());
		}
		beforeDeleteWare(ware);
		wareDao.delete(ware);
		afterDeleteWare(ware);
		wareDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/ware/del", ware,MESBasicWareMneCode.class);
		List<MESBasicWare> params = new ArrayList<MESBasicWare>();
		params.add(ware);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_wareMan");
	...
	public void deleteWare(List<Long> wareIds){
		deleteWare(wareIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.common.delete",operType="3")
	public void deleteWare(List<Long> wareIds, String eventTopic) {
		List<MESBasicWare> wares = new ArrayList<MESBasicWare>();
		for(Long wareId : wareIds){
			MESBasicWare ware = getWare(wareId);
			wares.add(ware);
			if(null != ware && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicWare.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == ware.getWareName() ? "" : "," + ware.getWareName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == ware.getWareName() ? "" : ware.getWareName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == ware.getWareCode() ? "" : "," + ware.getWareCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == ware.getWareCode() ? "" : ware.getWareCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == ware.getId() ? "" : "," + ware.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == ware.getId() ? "" : ware.getId().toString());
				}
			}
		}
		...
		if(wares != null && wares.size() > 0 ){
			for(MESBasicWare ware : wares){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),ware, ware.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(wareIds != null && wareIds.size() > 0) {
			String hql = "update " + MESBasicWare.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = wareDao.createQuery(hql);
			query.setParameterList("ids", wareIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicWare warez : wares){
				warez.setValid(false);
				List<MESBasicWare> params = new ArrayList<MESBasicWare>();
				params.add(warez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.common.delete",operType="3")
	public void deleteWare(long wareId, int wareVersion){
		this.deleteWare(wareId, wareVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.common.delete",operType="3")
	public void deleteWare(long wareId, int wareVersion,SignatureLog signatureLog){
		MESBasicWare ware = getWare(wareId);
		if(ware != null && !ware.isValid()){
		...
		beforeDeleteWare(ware);
		wareDao.delete(wareId, wareVersion);
		afterDeleteWare(ware);
		wareDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/ware/del", ware,MESBasicWareMneCode.class);
		List<MESBasicWare> params = new ArrayList<MESBasicWare>();
		...
		if(signatureLog != null ) {
			...
			businessKey=ware.getWareCode();
			signatureLog.setTableId(ware.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.common.delete",operType="3")
	public void deleteWare(String wareIds){
		this.deleteWare(wareIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.common.delete",operType="3")
	public void deleteWare(String wareIds,SignatureLog signatureLog){
		deleteCollection(wareIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteWare(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_wareMan#modelCode:MESBasic_1_wareMan_Ware#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="ec.engine.restore",operType="10")
	public void restoreWare(String wareIds){
		restoreCollection(wareIds);
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="ec.engine.restore",operType="10")
	public void restoreWare(long wareId){
		
		findBusinessKeyUsed(wareId);	//判断业务主键是否重复
		...
		beforeRestoreWare(ware);
		ware.setValid(true);
		wareDao.update(ware);
		afterRestoreWare(ware);
		wareDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/ware/add", ware,MESBasicWareMneCode.class);
		List<MESBasicWare> params = new ArrayList<MESBasicWare>();
		params.add(ware);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long wareId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_wareMan_Ware");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicWare.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicWare.TABLE_NAME +" where id = ? )";
			List<Object> list =  wareDao.createNativeQuery(sql,wareId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseWare(List<MESBasicWare>  wares){
		for(MESBasicWare ware:wares)  {
			saveWare(ware, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseWare(List<MESBasicWare>  wares){
		for(MESBasicWare ware:wares)  {
			saveWare(ware, null);
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="ec.flow.save")
	public void saveWare(MESBasicWare ware, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveWare(ware, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="ec.flow.save")
	public void saveWare(MESBasicWare ware, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveWare(ware, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateWareCodes(MESBasicWare ware) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateWareCodes(ware, false);
	}
	
	public void generateWareCodes(MESBasicWare ware, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateWareSummarys(MESBasicWare ware) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateWareSummarys(ware, false);
	}

	public void generateWareSummarys(MESBasicWare ware, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware")
	public void saveWare(MESBasicWare ware,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveWare(ware, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware")
	public void saveWare(MESBasicWare ware,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_wareMan";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_wareMan");
		String url = null;
		if(ware.getId() != null && ware.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicWare.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_wareMan/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicWare.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_wareMan/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, ware);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(ware);
		beforeSaveWare(ware, viewIsView);

		if (viewIsView) {
			wareDao.saveWithRevertVersion(ware);
		} else {
			if(isNew)
				wareDao.save(ware);
			else
		...
		dealDatagridsSave(ware,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateWareCodes(ware, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateWareSummarys(ware, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveWare(ware, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, ware);
			}
		}
		List<MESBasicWare> params = new ArrayList<MESBasicWare>();
		wareDao.flush();
		wareDao.clear();
		ware = wareDao.load(ware.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicWare ware){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware")
	public void mergeWare(MESBasicWare ware, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(ware);
		beforeSaveWare(ware);
		wareDao.merge(ware);
		if(null != ware && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicWare.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
		...
		dealDatagridsSave(ware,null,dgLists,dgDeleteIDs,assFileUploads);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicWare> findWares(Page<MESBasicWare> page, Criterion... criterions) {
		return wareDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicWareDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicWareDealInfo> list=wareDao.createQuery("from " + MESBasicWareDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicWareDealInfo dealInfo) {
		wareDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicWareDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) wareDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicWareDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = wareDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = wareDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicWareDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = wareDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findWares(Page<MESBasicWare> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findWares(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findWares(Page<MESBasicWare> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findWares(Page<MESBasicWare> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicWare> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"ware\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"ware\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"ware\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_wareMan_Ware" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_wareMan_Ware" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  wareDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  wareDao.createNativeQuery(powerOperateSql, "MESBasic_1_wareMan").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"ware\"","MESBasic_1_wareMan",customCondition,"MATERIAL_WARES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"ware\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicWareDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"ware\".LAY_REC ASC, \"ware\".SORT ASC");
				} else {
					orderPart.append(" \"ware\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) wareDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=wareDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = wareDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicWare> result = new ArrayList<MESBasicWare>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicWare.class,wareDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicWare.class,wareDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicWare ware : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(ware.getId(), "MESBasic_wareMan_ware");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(ware.getId(), "MESBasic_wareMan_ware");
							if(null != documents && !documents.isEmpty()) {
								ware.setDocument(documents.get(0));
								ware.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_wareMan")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_wareMan")+"MESBasic_1_wareMan".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_wareMan",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicWare> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicWare> result) {
		View view = null;
		...
		Map<Long, MESBasicWare> rsMap = new HashMap<Long, MESBasicWare>();
		for (MESBasicWare ware : result) {
			ids.add(ware.getId());
			rsMap.put(ware.getId(), ware);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(wareDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = wareDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicWare ware,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(ware.getOwnerStaffId());
		ti.setOwnerPositionId(ware.getOwnerPositionId());
		ti.setOwnerDepartmentId(ware.getOwnerDepartmentId());
		
		ti.setCreateStaff(ware.getCreateStaff());
		ti.setCreateStaffId(ware.getCreateStaffId());
		ti.setCreateTime(ware.getCreateTime());
		ti.setDeleteStaff(ware.getDeleteStaff());
		ti.setDeleteStaffId(ware.getDeleteStaffId());
		ti.setDeleteTime(ware.getDeleteTime());
		ti.setCreateDepartmentId(ware.getCreateDepartmentId());
		ti.setModifyStaff(ware.getModifyStaff());
		ti.setModifyStaffId(ware.getModifyStaffId());
		ti.setCreatePositionId(ware.getCreatePositionId());
		ti.setPositionLayRec(ware.getPositionLayRec());
		ti.setTableNo(ware.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_wareMan");
		ti.setTargetTableName("MATERIAL_WARES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_wareMan_Ware" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  wareDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"ware\"","MESBasic_1_wareMan","","MATERIAL_WARES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"ware\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"ware\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"ware\"","MESBasic_1_wareMan","","MATERIAL_WARES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("ware.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"ware\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("ware.mc\"").append(".WARE) not in ("+notinValue+")");
			}
			
			SQLQuery query = wareDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("wareMan", "MESBasic_1_wareMan", "wareMan_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=wareDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicWare.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicWare.MODULE_CODE;}
	protected String getModelCode(){return MESBasicWare.MODEL_CODE;}
	 ...
	 * @param ware 模型实例
	 */
	public void checkUniqueConstraint(MESBasicWare ware){
		if(ware.getId()==null){
			List<MESBasicWare> wares = wareDao.findByCriteria(Restrictions.eq("wareCode", ware.getWareCode()), Restrictions.eq("valid", true));
			if(wares!=null && !wares.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1484031239866", getCurrentUser().getLanguage()), ware.getWareCode());
			}
		} else {
			List<MESBasicWare> wares = wareDao.findByCriteria(Restrictions.ne("id", ware.getId()), Restrictions.eq("wareCode", ware.getWareCode()), Restrictions.eq("valid", true));
			if(wares!=null && !wares.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1484031239866", getCurrentUser().getLanguage()), ware.getWareCode());
			}
	...
	@Override
	public List<MESBasicWare> getWares(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicWare.class);
		if(params!=null){
			...
			return wareDao.findByHql("from " + MESBasicWare.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return wareDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicWare> findWaresBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicWare.class);
	...
	@Override
	public List<MESBasicWare> findWaresByHql(String hql, Object... objects){
		
		return   wareDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicWare loadWareByBussinessKey(MESBasicWare ware){
		return loadWareByBussinessKey(ware.getWareCode());
	}
	
	@Override
	public Page<MESBasicWare> getWares(Page<MESBasicWare> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicWare.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = wareDao.findByPage(page, sql);
			}else{
				page = wareDao.findAllByPage(page);
			}
			return page;
		}
		page = wareDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicWare loadWareByBussinessKey(Serializable bussinessKey){
		return wareDao.findEntityByCriteria(Restrictions.eq("wareCode", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicWare> findByProperty(String propertyName, Object object){
		return wareDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicWare findEntityByProperty(String propertyName, Object object){
		return wareDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicWare.JPA_NAME + " set valid=0 where wareCode in (:businessKeys)";
			Query query = wareDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_wareMan_Ware","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_wareMan", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicWare ware,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,wareDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicWare.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, ware);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",ware.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicWare.TABLE_NAME +" where valid = 1";
			list = wareDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicWare.TABLE_NAME;
			list = wareDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicWare.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = wareDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicWareDaoImpl wareDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchWare(final List<MESBasicWare> insertObjs, final List<MESBasicWare> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchWare(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchWare(final List<MESBasicWare> insertObjs, final List<MESBasicWare> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1484031011977"),importCount));
		...
		Map<Long, MESBasicWare> oldUpdateObjsMap = new HashMap<Long, MESBasicWare>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicWare bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicWare> bizObjList = wareDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicWare bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicWare> nmeObjs = new ArrayList<MESBasicWare>();
		...
		long id=autoGeneratorID.getNextId(MESBasicWare.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicWare ware:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicWare.TABLE_NAME,1,dbId);
			if(ware.getWareCode() != null && !ware.getWareCode().equals("") ){
				m.put(ware.getWareCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) wareDao.createNativeQuery(sql, "MESBasic_1_wareMan_Ware"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		wareDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicWare.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicWare ware = updateObjs.get(updateIndex);
						Long id = ware.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = ware.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicWare.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicWare.TABLE_NAME + " order by id desc";
//					Query query = wareDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicWare ware = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = ware.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicWareMneCode.class;
					...
					for(MESBasicWare ware:updateObjs)  {
						Long id = ware.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicWare ware:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(ware);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, ware.getId());
		...
		wareDaoImpl.getSession().flush();
		wareDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicWare bizObj = insertObjs.get(insertIndex);
			wareImportService.saveImportDataLog(bizObj, null, "wareCode", "wareName");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicWare bizObj = updateObjs.get(updateIndex);
			MESBasicWare oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			wareImportService.saveImportDataLog(bizObj, oldBizObj, "wareCode", "wareName");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = wareDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				wareDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = wareDao.createNativeQuery(sql, "MESBasic_1_wareMan_Ware").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = wareDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = wareDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = wareDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?wareDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  wareDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,wareDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="ec.flow.save")
	public void saveWare(MESBasicWare ware, DataGridService dataGridService){
		saveWare(ware, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="ec.flow.save")
	public void saveWare(MESBasicWare ware, DataGridService dataGridService, String viewCode){
		saveWare(ware, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware")
	public void saveWare(MESBasicWare ware, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveWare(ware, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware")
	public void saveWare(MESBasicWare ware, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_wareMan";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_wareMan");
		String url = null;
		if(null != ware.getId() && ware.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicWare.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware")
	public Map<Object, Object> wareDataGridImport(MESBasicWare ware, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = ware.getVirtualId();
		
		ware = wareDao.load(ware.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, ware.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = ware.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(ware);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, ware.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware")
	public void mergeWare(MESBasicWare ware, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(ware);
		beforeSaveWare(ware);
		wareDao.merge(ware);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_wareMan",model="MESBasic_1_wareMan_Ware",desc="foundation.batch.save",operType="4")
	public void batchSaveWare(MESBasicWare ware, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_wareMan";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_wareMan");
	...
	@Override
	public Page<MESBasicWare> getByPage(Page<MESBasicWare> page,DetachedCriteria detachedCriteria){
		return wareDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_wareMan").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveWare(MESBasicWare ware, Object... objects){
		checkUniqueConstraint(ware);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveWare(MESBasicWare ware, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportWare(List<MESBasicWare> wares, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportWare(List<MESBasicWare> wares){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportWare(List<MESBasicWare> wares, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportWare(List<MESBasicWare> wares){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteWare(MESBasicWare ware){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteWare(MESBasicWare ware){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreWare(MESBasicWare ware){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreWare(MESBasicWare ware){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicWare ware, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicWare ware, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportWare(List<MESBasicWare> insertObjs, List<MESBasicWare> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportWare,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportWare(List<MESBasicWare> insertObjs, List<MESBasicWare> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportWare,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
   public Boolean getIsUser() {
		// 获取当前用户
		Long userId = ((User) getCurrentUser()).getId();
		Long currentStaffId = ((User) getCurrentUser()).getStaff().getId();
		// 判断当前用户是否已经分配了仓库权限
		String sql = "(select WARE_ID from MATERIAL_WARE_ROLE_INFOS where WARE_ROLE in ( select ROLE_ID from BASE_ROLEUSER where USER_ID = ?)) "
				+ " union ( select WARE_ID from MATERIAL_WARE_STAFF_INFOS  where WARE_STAFF = ? )";
		List<Object> wareList = wareDao.createNativeQuery(sql, userId,
				currentStaffId).list();
    ...
    protected String getCustomCondition(Page page, String viewCode, int type,String processKey, List<Param> params, List<Object> list) {
        ...
        // 仓库权限过滤
        if ("MESBasic_1_wareMan_wareTreeOrderList".equals(viewCode)
				||"MESBasic_1_wareMan_wareTreeOrderRef".equals(viewCode)
               ) {
        	if (getIsUser()) {
        		 sqlCustomConditon = " \"ware\".ID in  ((select WARE_ID from MATERIAL_WARE_ROLE_INFOS where WARE_ROLE in ( select ROLE_ID from BASE_ROLEUSER where USER_ID = "
                         + userId
                         + ")) "
                         + " union ( select WARE_ID from MATERIAL_WARE_STAFF_INFOS  where WARE_STAFF = "
                         + staffId + " ))";
