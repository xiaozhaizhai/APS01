...
import com.supcon.orchid.MESBasic.services.MESBasicWareWSService;
import com.supcon.orchid.MESBasic.services.MESBasicWareService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicWare",targetNamespace = "http://ws.supcon.com")
public class MESBasicWareWSServiceImpl implements MESBasicWareWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getWareByBusinessKey(
		...
		try{
			...
			MESBasicWare ware = wareService.loadWareByBussinessKey(businessKey);
			if(ware == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, ware);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateWare(
		...
		try{
			if(ware == null){
				return set500Error(result, "仓库表不能为空。");
			}
			...
			if(!beforeUpdateWare(ware)){
				return set404Error(result);
			}
			wareService.saveWare(ware, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addWare(
		...
		try{
			...
			wareService.saveWare(ware, null);
			setSuccessResult(result, ware.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteWareByBusinessKey(
		...
		try{
			...
			try{
				ware = wareService.loadWareByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddWare(
		...
		try{
			...
			int count = 1;
			for(MESBasicWare ware : wareList){
				if(ware.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(ware.getWareCode() == null || ware.getWareCode().toString().trim().isEmpty()){
			...
			wareService.batchImportBaseWare(wareList);
			for(MESBasicWare ware : wareList){
				idList.add(ware.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteWareByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicWare temp = null;
				try{
					temp = wareService.getWare(id);
				}catch(NullPointerException e){}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteWareByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicWare temp = null;
				try{
					temp = wareService.loadWareByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getWarePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicWare.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicWare.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicWare.class);
				} catch (Exception e) {
			...
			Page<MESBasicWare> page = new Page<MESBasicWare>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicWare> datas = wareService.getByPage(page, detachedCriteria);
			List<MESBasicWare> wareList = datas.getResult();
			if (wareList == null || wareList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(wareList.toArray());
			PaginationDTO<MESBasicWare> pagination = new PaginationDTO<MESBasicWare>(page);
	...
	/**
	 * 获取仓库表对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param ware
	 * @return
	 */
	private MESBasicWare getWare(MESBasicWare ware) {
		if (ware.getId() == null && (ware.getWareCode() == null || ware.getWareCode().toString().trim() == ""))
			throw new RuntimeException("仓库表对象业务主键和id不能同时为空");
		if (ware.getId() != null) {
			MESBasicWare result = wareService.getWare(ware .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return wareService.loadWareByBussinessKey(ware.getWareCode());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个仓库表，则返回false，否则返回true
	 * @param ware
	 */
	private boolean beforeUpdateWare(MESBasicWare ware) {
		if(ware.getWareName() == null){
			throw new RuntimeException("wareName不能为空。");
		}
		if(ware.getWareState() == null){
			throw new RuntimeException("wareState不能为空。");
		}
		MESBasicWare temp = getWare(ware);
		if (temp == null) {
			return false;
		}
		if(ware.getId() == null){
			ware.setId(temp.getId());		
		}
		if(ware.getWareCode() == null){
			ware.setWareCode(temp.getWareCode());
		}
		
		ware.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除仓库表对象下所有辅模型对象中的仓库表对象
	 * @param ware
	 */
	private void clearCircle(MESBasicWare ware) {
	 ...
	 */
	private void checkWareData(MESBasicWare ware){
		 if(ware.getWareName() == null){
		 	throw new RuntimeException("wareName不能为空。");
		 }
		 if(ware.getWareState() == null){
	...
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
