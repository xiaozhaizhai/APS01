...
import com.supcon.orchid.MESBasic.services.MESBasicProdattrelWSService;
import com.supcon.orchid.MESBasic.services.MESBasicProdattrelService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicProdattrel",targetNamespace = "http://ws.supcon.com")
public class MESBasicProdattrelWSServiceImpl implements MESBasicProdattrelWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getProdattrelByBusinessKey(
		...
		try{
			...
			MESBasicProdattrel prodattrel = prodattrelService.loadProdattrelByBussinessKey(businessKey);
			if(prodattrel == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, prodattrel);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateProdattrel(
		...
		try{
			if(prodattrel == null){
				return set500Error(result, "物品属性关联不能为空。");
			}
			...
			if(!beforeUpdateProdattrel(prodattrel)){
				return set404Error(result);
			}
			prodattrelService.saveProdattrel(prodattrel, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addProdattrel(
		...
		try{
			...
			try{
				checkProdattrelData(prodattrel);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			prodattrelService.saveProdattrel(prodattrel, null);
			setSuccessResult(result, prodattrel.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteProdattrelByBusinessKey(
		...
		try{
			...
			try{
				prodattrel = prodattrelService.loadProdattrelByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddProdattrel(
		...
		try{
			...
			int count = 1;
			for(MESBasicProdattrel prodattrel : prodattrelList){
				if(prodattrel.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(prodattrel.getId() == null || prodattrel.getId().toString().trim().isEmpty()){
			...
			prodattrelService.batchImportBaseProdattrel(prodattrelList);
			for(MESBasicProdattrel prodattrel : prodattrelList){
				idList.add(prodattrel.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteProdattrelByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicProdattrel temp = null;
				try{
					temp = prodattrelService.getProdattrel(id);
				}catch(NullPointerException e){}
			...
			prodattrelService.deleteProdattrel(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteProdattrelByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicProdattrel temp = null;
				try{
					temp = prodattrelService.loadProdattrelByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getProdattrelPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicProdattrel.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicProdattrel.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicProdattrel.class);
				} catch (Exception e) {
			...
			Page<MESBasicProdattrel> page = new Page<MESBasicProdattrel>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicProdattrel> datas = prodattrelService.getByPage(page, detachedCriteria);
			List<MESBasicProdattrel> prodattrelList = datas.getResult();
			if (prodattrelList == null || prodattrelList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(prodattrelList.toArray());
			PaginationDTO<MESBasicProdattrel> pagination = new PaginationDTO<MESBasicProdattrel>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicProdattrel prodattrel = prodattrelService.getProdattrel(id);
			if (prodattrel == null || !prodattrel.isValid()) {
				result.setErrorMessage("找不到物品属性关联");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取物品属性关联对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param prodattrel
	 * @return
	 */
	private MESBasicProdattrel getProdattrel(MESBasicProdattrel prodattrel) {
		if (prodattrel.getId() == null && (prodattrel.getId() == null || prodattrel.getId().toString().trim() == ""))
			throw new RuntimeException("物品属性关联对象业务主键和id不能同时为空");
		if (prodattrel.getId() != null) {
			MESBasicProdattrel result = prodattrelService.getProdattrel(prodattrel .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return prodattrelService.loadProdattrelByBussinessKey(prodattrel.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个物品属性关联，则返回false，否则返回true
	 * @param prodattrel
	 */
	private boolean beforeUpdateProdattrel(MESBasicProdattrel prodattrel) {
		MESBasicProdattrel temp = getProdattrel(prodattrel);
		if (temp == null) {
			return false;
		}
		if(prodattrel.getId() == null){
			prodattrel.setId(temp.getId());		
		}
		if(prodattrel.getId() == null){
			prodattrel.setId(temp.getId());
		}
		
		prodattrel.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除物品属性关联对象下所有辅模型对象中的物品属性关联对象
	 * @param prodattrel
	 */
	private void clearCircle(MESBasicProdattrel prodattrel) {
	 ...
	 */
	private void checkProdattrelData(MESBasicProdattrel prodattrel){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
