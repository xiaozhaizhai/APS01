...
/* CUSTOM CODE START(action,import,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicBatchDealInfoAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBatchDealInfo> {
	...
	private MESBasicBatchDealInfo batchDealInfo;
	...
	//导出excel时,主模型内容
	List<MESBasicBatchDealInfo> queryResults = new ArrayList<MESBasicBatchDealInfo>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/batchInfo/batchDealInfo/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/batchInfo/batchDealInfo/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_batchInfo_BatchDealInfo" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBatchDealInfo> batchDealInfos;
	
	public List<MESBasicBatchDealInfo> getBatchDealInfos() {
		return batchDealInfos;
	}
	...
	private Page<MESBasicBatchDealInfo> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchDealInfo = new MESBasicBatchDealInfo();
			if(batchDealInfo.getDealType() != null) {
				batchDealInfo.setDealType((SystemCode) systemCodeService.getSystemCode(batchDealInfo.getDealType().getId()));
			}
		} else {
			if(id > 0){
				batchDealInfo = batchDealInfoService.getBatchDealInfo(id, viewCode);
			}
		}
	...
	public Page<MESBasicBatchDealInfo> getPage() {
		if (null == page) {
			page = new Page<MESBasicBatchDealInfo>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicBatchDealInfo getBatchDealInfo() {
		return batchDealInfo;
	}		

	public void setBatchDealInfo(MESBasicBatchDealInfo batchDealInfo) {
		this.batchDealInfo = batchDealInfo;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchDealInfoService.getBatchDealInfoAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1490774952864
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchDealInfoReferenceCopy() throws Exception{
			MESBasicBatchDealInfo oldBatchDealInfo = null;
			if(isEditNew()){
				...
				oldBatchDealInfo = new MESBasicBatchDealInfo();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchDealInfo.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchDealInfo.getId());
				...
				oldBatchDealInfo = batchDealInfoService.getBatchDealInfo(id);
			}
			
			
			batchDealInfo = batchDealInfoService.getBatchDealInfo(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchDealInfo");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBatchDealInfo.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchDealInfo, null);
						Method setValue = MESBasicBatchDealInfo.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchDealInfo, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchDealInfo.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchDealInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchDealInfoReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchDealInfo = batchDealInfoService.getBatchDealInfo(this.batchDealInfo.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchDealInfo.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchDealInfo/batchDealList", results = { @Result(type = FREEMARKER, location = "batchDealList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchDealInfoService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchDealInfoService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchDealInfoService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_batchInfo_BatchDealInfo", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchDealInfo/batchDealList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchNum,result.sourceBatch,result.amount,result.dealType.id,result.dealType.value,result.dealStaff.name,result.dealDept.name,result.dealFromID,result.dealTableNo,result.workOrderID,result.processID,result.activeID,result.dealStaff.id,result.dealDept.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batchDealList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchDealInfo\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchDealInfo\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"dealDept\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchDealInfo\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchDealInfo\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchDealInfoService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchDealInfo/batchDealList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchDealInfoService.findDataGridPage(dg,dgclass,getDgPage(),batchDealInfo,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchDealInfo.getId() != null && batchDealInfo.getId() != -1){
				batchDealInfo = batchDealInfoService.getBatchDealInfo(batchDealInfo.getId());
			}
			batchDealInfoService.findDataGridPage(dg,dgclass,getDgPage(),batchDealInfo,searchCondition,params);
	...
	private void save() throws Exception {
		...
		batchDealInfoService.saveBatchDealInfo(batchDealInfo,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "submit");
		bap_validate_datagrids=batchDealInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchDealInfo.getId());
		...
		fileuploads.put("linkId", batchDealInfo.getId());
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/batchInfo/batchDealInfo/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchDealInfoService.deleteBatchDealInfo(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "restore");
		batchDealInfoService.restoreBatchDealInfo(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBatchDealInfo> findExportDataInfos(Page<MESBasicBatchDealInfo> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchDealInfoService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchDealInfoService.getAssProperty(p.getCode());	
									//String assModelCode = batchDealInfoService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchDealInfoService.findBatchDealInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchDealInfoService.findBatchDealInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBatchDealInfo> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBatchDealInfo result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchDealInfo || null == this.batchDealInfo.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/batchInfo/batchDealInfo/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/batchInfo/batchDealInfo/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBatchDealInfoService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBatchDealInfoService"
						, "com.supcon.orchid.MESBasic.services.MESBasicBatchInformationService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_batchInfo_BatchDealInfo", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_batchInfo_BatchDealInfo", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_batchInfo_BatchDealInfo";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchDealInfo", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicBatchDealInfo batchDealInfo = null;
		if(null != id) {
			batchDealInfo = batchDealInfoService.getBatchDealInfo(id);
		}
		...
		_auditLog.setColumnStringB(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
		_auditLog.setColumnStringA(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
		_auditLog.setColumnStringC(batchDealInfo.getId().toString());
		_auditLog.setColumnStringD(MESBasicBatchDealInfo.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBatchDealInfoAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchDealInfo,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchDealInfo,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchDealInfo);
		...
		printlog.setBusinessId(batchDealInfo.getId());
		printlog.setBussinessKey(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
		printlog.setMainDisplay(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchDealInfo,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchDealInfoService.getBatchDealInfo(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchDealInfo,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchDealInfo = batchDealInfoService.getBatchDealInfo(Long.parseLong(arrId[i]));
				if(null != batchDealInfo){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchDealInfo.getId() ? "" : "," + batchDealInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchDealInfo.getId() ? "" : "," + batchDealInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchDealInfo.getId() ? "" : "," + batchDealInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchDealInfo.getId() ? "" : batchDealInfo.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchDealInfo,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchDealInfo);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/batchInfo/batchDealInfo/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchDealInfo");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_batchInfo_BatchDealInfo")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_batchInfo_BatchDealInfo");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchDealInfo");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchDealInfoService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_batchInfo_BatchDealInfo,MESBasic_1) */
// 自定义代码
