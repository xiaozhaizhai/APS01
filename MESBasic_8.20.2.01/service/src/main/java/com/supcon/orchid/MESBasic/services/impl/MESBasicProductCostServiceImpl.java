...
import com.supcon.orchid.MESBasic.services.MESBasicProductCostImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicProductCostDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_productCostService")
@Transactional
public class MESBasicProductCostServiceImpl  extends BaseServiceImpl<MESBasicProductCost> implements MESBasicProductCostService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicProductCost> bapBaseService;
	...
	@Autowired
	private MESBasicProductCostImportService productCostImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicProductCost.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  productCostDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicProductCost.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = productCostDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProductCost.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProductCost.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  productCostDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicProductCost.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicProductCost.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicProductCost> list = (List<MESBasicProductCost>) productCostDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicProductCost getProductCost(long id){
		return getProductCost(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProductCost getProductCost(long id, String viewCode){
		MESBasicProductCost productCost = productCostDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProductCostAsJSON(long id, String include){
		MESBasicProductCost productCost = productCostDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(productCost, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.common.delete",operType="3")
	public void deleteProductCost(MESBasicProductCost productCost){
		if(null != productCost && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProductCost.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productCost.getId() ? "" : productCost.getId().toString());
			AuditUtil.setColumnStringA(null == productCost.getId() ? "" : productCost.getId().toString());
			AuditUtil.setColumnStringC(productCost.getId().toString());
		}
		beforeDeleteProductCost(productCost);
		productCostDao.deletePhysical(productCost);
		afterDeleteProductCost(productCost);
		productCostDao.flush();
		List<MESBasicProductCost> params = new ArrayList<MESBasicProductCost>();
		params.add(productCost);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.common.delete",operType="3")
	public void deleteProductCost(List<Long> productCostIds){
		deleteProductCost(productCostIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.common.delete",operType="3")
	public void deleteProductCost(List<Long> productCostIds, String eventTopic) {
		List<MESBasicProductCost> productCosts = new ArrayList<MESBasicProductCost>();
		for(Long productCostId : productCostIds){
			MESBasicProductCost productCost = getProductCost(productCostId);
			productCosts.add(productCost);
			if(null != productCost && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProductCost.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == productCost.getId() ? "" : "," + productCost.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == productCost.getId() ? "" : productCost.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == productCost.getId() ? "" : "," + productCost.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == productCost.getId() ? "" : productCost.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == productCost.getId() ? "" : "," + productCost.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == productCost.getId() ? "" : productCost.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(productCosts != null && productCosts.size() > 0 ){
			for(MESBasicProductCost productCost : productCosts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicProductCost.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(productCost));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),productCost, productCost.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(productCostIds != null && productCostIds.size() > 0) {
			String hql = "delete " + MESBasicProductCost.JPA_NAME + " where id in(:ids)";
			Query query = productCostDao.createQuery(hql);
			query.setParameterList("ids", productCostIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicProductCost productCostz : productCosts){
				productCostz.setValid(false);
				List<MESBasicProductCost> params = new ArrayList<MESBasicProductCost>();
				params.add(productCostz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_product");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.common.delete",operType="3")
	public void deleteProductCost(long productCostId, int productCostVersion){
		this.deleteProductCost(productCostId, productCostVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.common.delete",operType="3")
	public void deleteProductCost(long productCostId, int productCostVersion,SignatureLog signatureLog){
		MESBasicProductCost productCost = getProductCost(productCostId);
		...
		beforeDeleteProductCost(productCost);
		productCostDao.delete(productCostId, productCostVersion);
		afterDeleteProductCost(productCost);
		productCostDao.flush();
		List<MESBasicProductCost> params = new ArrayList<MESBasicProductCost>();
		params.add(productCost);
		...
		if(signatureLog != null ) {
			...
			businessKey=productCost.getId();
			signatureLog.setTableId(productCost.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.common.delete",operType="3")
	public void deleteProductCost(String productCostIds){
		this.deleteProductCost(productCostIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.common.delete",operType="3")
	public void deleteProductCost(String productCostIds,SignatureLog signatureLog){
		deleteCollection(productCostIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProductCost(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_product#modelCode:MESBasic_1_product_ProductCost#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="ec.engine.restore",operType="10")
	public void restoreProductCost(String productCostIds){
		restoreCollection(productCostIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProductCost(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="ec.engine.restore",operType="10")
	public void restoreProductCost(long productCostId){
		
		findBusinessKeyUsed(productCostId);	//判断业务主键是否重复
		...
		beforeRestoreProductCost(productCost);
		productCost.setValid(true);
		productCostDao.update(productCost);
		afterRestoreProductCost(productCost);
		List<MESBasicProductCost> params = new ArrayList<MESBasicProductCost>();
		params.add(productCost);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long productCostId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_product_ProductCost");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicProductCost.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicProductCost.TABLE_NAME +" where id = ? )";
			List<Object> list =  productCostDao.createNativeQuery(sql,productCostId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProductCost(List<MESBasicProductCost>  productCosts){
		for(MESBasicProductCost productCost:productCosts)  {
			saveProductCost(productCost, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProductCost(List<MESBasicProductCost>  productCosts){
		for(MESBasicProductCost productCost:productCosts)  {
			saveProductCost(productCost, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="ec.flow.save")
	public void saveProductCost(MESBasicProductCost productCost, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProductCost(productCost, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="ec.flow.save")
	public void saveProductCost(MESBasicProductCost productCost, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProductCost(productCost, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProductCostCodes(MESBasicProductCost productCost) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductCostCodes(productCost, false);
	}
	
	public void generateProductCostCodes(MESBasicProductCost productCost, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProductCostSummarys(MESBasicProductCost productCost) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductCostSummarys(productCost, false);
	}

	public void generateProductCostSummarys(MESBasicProductCost productCost, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost")
	public void saveProductCost(MESBasicProductCost productCost,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProductCost(productCost, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost")
	public void saveProductCost(MESBasicProductCost productCost,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(productCost.getId() != null && productCost.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProductCost.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, productCost);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(productCost);
		beforeSaveProductCost(productCost, viewIsView);

		if (viewIsView) {
			productCostDao.saveWithRevertVersion(productCost);
		} else {
			if(isNew)
				productCostDao.save(productCost);
			else
				productCostDao.merge(productCost);
		}
		if(null != productCost && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProductCost.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productCost.getId() ? "" : productCost.getId().toString());
			AuditUtil.setColumnStringA(null == productCost.getId() ? "" : productCost.getId().toString());
		...
		dealDatagridsSave(productCost,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProductCostCodes(productCost, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProductCostSummarys(productCost, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProductCost(productCost, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, productCost);
			}
		}
		List<MESBasicProductCost> params = new ArrayList<MESBasicProductCost>();
		productCostDao.flush();
		productCostDao.clear();
		productCost = productCostDao.load(productCost.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicProductCost productCost){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("productCost", productCost);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost")
	public void mergeProductCost(MESBasicProductCost productCost, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(productCost);
		beforeSaveProductCost(productCost);
		productCostDao.merge(productCost);
		if(null != productCost && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProductCost.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productCost.getId() ? "" : productCost.getId().toString());
			AuditUtil.setColumnStringA(null == productCost.getId() ? "" : productCost.getId().toString());
		...
		dealDatagridsSave(productCost,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProductCost(productCost);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicProductCost> findProductCosts(Page<MESBasicProductCost> page, Criterion... criterions) {
		return productCostDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductCosts(Page<MESBasicProductCost> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProductCosts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductCosts(Page<MESBasicProductCost> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductCosts(Page<MESBasicProductCost> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicProductCost> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"productCost\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"productCost\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"productCost\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  productCostDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  productCostDao.createNativeQuery(powerOperateSql, "MESBasic_1_product").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"productCost\".LAY_REC ASC, \"productCost\".SORT ASC");
				} else {
					orderPart.append(" \"productCost\".PC_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) productCostDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=productCostDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = productCostDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicProductCost> result = new ArrayList<MESBasicProductCost>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicProductCost.class,productCostDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicProductCost.class,productCostDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicProductCost productCost : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(productCost.getId(), "MESBasic_product_productCost");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(productCost.getId(), "MESBasic_product_productCost");
							if(null != documents && !documents.isEmpty()) {
								productCost.setDocument(documents.get(0));
								productCost.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicProductCost> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicProductCost> result) {
		View view = null;
		...
		Map<Long, MESBasicProductCost> rsMap = new HashMap<Long, MESBasicProductCost>();
		for (MESBasicProductCost productCost : result) {
			ids.add(productCost.getId());
			rsMap.put(productCost.getId(), productCost);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(productCostDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = productCostDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_product_ProductCost" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_product_ProductCost" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  productCostDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productCost\"","MESBasic_1_product","","S2BASE_PRODUCTCOST",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"productCost\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"productCost\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productCost\"","MESBasic_1_product","","S2BASE_PRODUCTCOST",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_product_ProductCost" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("productCost.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"productCost\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"productCost\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"productCost\".CID = 1 OR \"productCost\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("productCost.mc\"").append(".PRODUCT_COST) not in ("+notinValue+")");
			}
			
			SQLQuery query = productCostDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=productCostDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicProductCost.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicProductCost.MODULE_CODE;}
	protected String getModelCode(){return MESBasicProductCost.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param productCost 模型实例
	 */
	public void checkUniqueConstraint(MESBasicProductCost productCost){
	}
	
	
	@Override
	public List<MESBasicProductCost> getProductCosts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProductCost.class);
		if(params!=null){
			...
			return productCostDao.findByHql("from " + MESBasicProductCost.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return productCostDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicProductCost> findProductCostsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProductCost.class);
	...
	@Override
	public List<MESBasicProductCost> findProductCostsByHql(String hql, Object... objects){
		
		return   productCostDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicProductCost loadProductCostByBussinessKey(MESBasicProductCost productCost){
		return loadProductCostByBussinessKey(productCost.getId());
	}
	
	@Override
	public Page<MESBasicProductCost> getProductCosts(Page<MESBasicProductCost> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProductCost.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = productCostDao.findByPage(page, sql);
			}else{
				page = productCostDao.findAllByPage(page);
			}
			return page;
		}
		page = productCostDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProductCost loadProductCostByBussinessKey(Serializable bussinessKey){
		return productCostDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProductCost> findByProperty(String propertyName, Object object){
		return productCostDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProductCost findEntityByProperty(String propertyName, Object object){
		return productCostDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicProductCost.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = productCostDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_product_ProductCost","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicProductCost productCost,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,productCostDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicProductCost.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, productCost);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",productCost.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProductCost.TABLE_NAME +" where valid = 1";
			list = productCostDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProductCost.TABLE_NAME;
			list = productCostDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicProductCost.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = productCostDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicProductCostDaoImpl productCostDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProductCost(final List<MESBasicProductCost> insertObjs, final List<MESBasicProductCost> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProductCost(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProductCost(final List<MESBasicProductCost> insertObjs, final List<MESBasicProductCost> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1471523790726"),importCount));
		...
		Map<Long, MESBasicProductCost> oldUpdateObjsMap = new HashMap<Long, MESBasicProductCost>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicProductCost bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicProductCost> bizObjList = productCostDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicProductCost bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicProductCost> nmeObjs = new ArrayList<MESBasicProductCost>();
		...
		long id=autoGeneratorID.getNextId(MESBasicProductCost.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicProductCost productCost:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicProductCost.TABLE_NAME,1,dbId);
			if(productCost.getId() != null ){
				m.put(productCost.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) productCostDao.createNativeQuery(sql, "MESBasic_1_product_ProductCost"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		productCostDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicProductCost.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicProductCost productCost = updateObjs.get(updateIndex);
						Long id = productCost.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = productCost.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicProductCost.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicProductCost.TABLE_NAME + " order by id desc";
//					Query query = productCostDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicProductCost productCost = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = productCost.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		productCostDaoImpl.getSession().flush();
		productCostDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicProductCost bizObj = insertObjs.get(insertIndex);
			productCostImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicProductCost bizObj = updateObjs.get(updateIndex);
			MESBasicProductCost oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			productCostImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicProductCost> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicProductCost> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) productCostDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicProductCost> result = getResult(page, query, new PendingResultTransformer(MESBasicProductCost.class,productCostDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicProductCost model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = productCostDao.createNativeQuery(sql, "MESBasic_1_product_ProductCost").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = productCostDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = productCostDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = productCostDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?productCostDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  productCostDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,productCostDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="ec.flow.save")
	public void saveProductCost(MESBasicProductCost productCost, DataGridService dataGridService){
		saveProductCost(productCost, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="ec.flow.save")
	public void saveProductCost(MESBasicProductCost productCost, DataGridService dataGridService, String viewCode){
		saveProductCost(productCost, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost")
	public void saveProductCost(MESBasicProductCost productCost, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProductCost(productCost, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost")
	public void saveProductCost(MESBasicProductCost productCost, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(null != productCost.getId() && productCost.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProductCost.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost")
	public Map<Object, Object> productCostDataGridImport(MESBasicProductCost productCost, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = productCost.getVirtualId();
		
		productCost = productCostDao.load(productCost.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, productCost.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = productCost.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(productCost);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, productCost.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost")
	public void mergeProductCost(MESBasicProductCost productCost, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(productCost);
		beforeSaveProductCost(productCost);
		productCostDao.merge(productCost);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProductCost",desc="foundation.batch.save",operType="4")
	public void batchSaveProductCost(MESBasicProductCost productCost, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
	...
	@Override
	public Page<MESBasicProductCost> getByPage(Page<MESBasicProductCost> page,DetachedCriteria detachedCriteria){
		return productCostDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProductCost(MESBasicProductCost productCost, Object... objects){
		checkUniqueConstraint(productCost);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProductCost(MESBasicProductCost productCost, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProductCost(List<MESBasicProductCost> productCosts, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProductCost(List<MESBasicProductCost> productCosts){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProductCost(List<MESBasicProductCost> productCosts, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProductCost(List<MESBasicProductCost> productCosts){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProductCost(MESBasicProductCost productCost){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProductCost(MESBasicProductCost productCost){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProductCost(MESBasicProductCost productCost){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProductCost(MESBasicProductCost productCost){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicProductCost productCost, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicProductCost productCost, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProductCost(List<MESBasicProductCost> insertObjs, List<MESBasicProductCost> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProductCost,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProductCost(List<MESBasicProductCost> insertObjs, List<MESBasicProductCost> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProductCost,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
