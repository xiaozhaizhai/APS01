...
@Repository
public class MESBasicInterfaceLogDaoImpl extends ExtendedGenericDaoImpl<MESBasicInterfaceLog, Long> implements MESBasicInterfaceLogDao {
	@Override
	...
	@Override
	public MESBasicInterfaceLogDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicInterfaceLogDealInfo dealInfo = new MESBasicInterfaceLogDealInfo();
	...
	@Override
	public MESBasicInterfaceLogDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicInterfaceLog mainObj, Staff dealStaff) {
		MESBasicInterfaceLogDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicInterfaceLogDealInfo.class);
		...
		List<MESBasicInterfaceLogDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_pimsInterface_InterfaceLog,MESBasic_1) */
// 自定义代码
