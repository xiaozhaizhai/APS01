...
@Repository
public class MESBasicProductDaoImpl extends ExtendedGenericDaoImpl<MESBasicProduct, Long> implements MESBasicProductDao {
	@Override
	...
	@Override
	public MESBasicProductDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicProductDealInfo dealInfo = new MESBasicProductDealInfo();
	...
	@Override
	public MESBasicProductDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicProduct mainObj, Staff dealStaff) {
		MESBasicProductDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicProductDealInfo.class);
		...
		List<MESBasicProductDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_product_Product,MESBasic_1) */
// 自定义代码
