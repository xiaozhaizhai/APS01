...
import com.supcon.orchid.MESBasic.services.MESBasicBatchDataImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicBatchDataDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
import java.text.SimpleDateFormat;
/* CUSTOM CODE END */

@Service("mESBasic_batchDataService")
@Transactional
public class MESBasicBatchDataServiceImpl  extends BaseServiceImpl<MESBasicBatchData> implements MESBasicBatchDataService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicBatchData> bapBaseService;
	...
	@Autowired
	private MESBasicBatchDataImportService batchDataImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicBatchData.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchDataDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicBatchData.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchDataDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBatchData.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBatchData.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchDataDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicBatchData.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicBatchData.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicBatchData> list = (List<MESBasicBatchData>) batchDataDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicBatchData getBatchData(long id){
		return getBatchData(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchData getBatchData(long id, String viewCode){
		MESBasicBatchData batchData = batchDataDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchDataAsJSON(long id, String include){
		MESBasicBatchData batchData = batchDataDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchData, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.common.delete",operType="3")
	public void deleteBatchData(MESBasicBatchData batchData){
		if(null != batchData && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchData.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchData.getId() ? "" : batchData.getId().toString());
			AuditUtil.setColumnStringA(null == batchData.getTag() ? "" : batchData.getTag().toString());
			AuditUtil.setColumnStringC(batchData.getId().toString());
		}
		beforeDeleteBatchData(batchData);
		batchDataDao.delete(batchData);
		afterDeleteBatchData(batchData);
		batchDataDao.flush();
		List<MESBasicBatchData> params = new ArrayList<MESBasicBatchData>();
		params.add(batchData);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchDataHandler");
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.common.delete",operType="3")
	public void deleteBatchData(List<Long> batchDataIds){
		deleteBatchData(batchDataIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.common.delete",operType="3")
	public void deleteBatchData(List<Long> batchDataIds, String eventTopic) {
		List<MESBasicBatchData> batchDatas = new ArrayList<MESBasicBatchData>();
		for(Long batchDataId : batchDataIds){
			MESBasicBatchData batchData = getBatchData(batchDataId);
			batchDatas.add(batchData);
			if(null != batchData && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchData.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchData.getTag() ? "" : "," + batchData.getTag().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchData.getTag() ? "" : batchData.getTag().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchData.getId() ? "" : "," + batchData.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchData.getId() ? "" : batchData.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchData.getId() ? "" : "," + batchData.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchData.getId() ? "" : batchData.getId().toString());
				}
			}
		}
		...
		if(batchDatas != null && batchDatas.size() > 0 ){
			for(MESBasicBatchData batchData : batchDatas){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchData, batchData.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(batchDataIds != null && batchDataIds.size() > 0) {
			String hql = "update " + MESBasicBatchData.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = batchDataDao.createQuery(hql);
			query.setParameterList("ids", batchDataIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicBatchData batchDataz : batchDatas){
				batchDataz.setValid(false);
				List<MESBasicBatchData> params = new ArrayList<MESBasicBatchData>();
				params.add(batchDataz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_batchDataHandler");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.common.delete",operType="3")
	public void deleteBatchData(long batchDataId, int batchDataVersion){
		this.deleteBatchData(batchDataId, batchDataVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.common.delete",operType="3")
	public void deleteBatchData(long batchDataId, int batchDataVersion,SignatureLog signatureLog){
		MESBasicBatchData batchData = getBatchData(batchDataId);
		if(batchData != null && !batchData.isValid()){
		...
		beforeDeleteBatchData(batchData);
		batchDataDao.delete(batchDataId, batchDataVersion);
		afterDeleteBatchData(batchData);
		batchDataDao.flush();
		List<MESBasicBatchData> params = new ArrayList<MESBasicBatchData>();
		params.add(batchData);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchData.getId();
			signatureLog.setTableId(batchData.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.common.delete",operType="3")
	public void deleteBatchData(String batchDataIds){
		this.deleteBatchData(batchDataIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.common.delete",operType="3")
	public void deleteBatchData(String batchDataIds,SignatureLog signatureLog){
		deleteCollection(batchDataIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchData(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_batchDataHandler#modelCode:MESBasic_1_batchDataHandler_BatchData#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="ec.engine.restore",operType="10")
	public void restoreBatchData(String batchDataIds){
		restoreCollection(batchDataIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchData(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="ec.engine.restore",operType="10")
	public void restoreBatchData(long batchDataId){
		
		findBusinessKeyUsed(batchDataId);	//判断业务主键是否重复
		...
		beforeRestoreBatchData(batchData);
		batchData.setValid(true);
		batchDataDao.update(batchData);
		afterRestoreBatchData(batchData);
		List<MESBasicBatchData> params = new ArrayList<MESBasicBatchData>();
		params.add(batchData);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchDataId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_batchDataHandler_BatchData");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicBatchData.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicBatchData.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchDataDao.createNativeQuery(sql,batchDataId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchData(List<MESBasicBatchData>  batchDatas){
		for(MESBasicBatchData batchData:batchDatas)  {
			saveBatchData(batchData, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchData(List<MESBasicBatchData>  batchDatas){
		for(MESBasicBatchData batchData:batchDatas)  {
			saveBatchData(batchData, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="ec.flow.save")
	public void saveBatchData(MESBasicBatchData batchData, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchData(batchData, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="ec.flow.save")
	public void saveBatchData(MESBasicBatchData batchData, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchData(batchData, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchDataCodes(MESBasicBatchData batchData) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchDataCodes(batchData, false);
	}
	
	public void generateBatchDataCodes(MESBasicBatchData batchData, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchDataSummarys(MESBasicBatchData batchData) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchDataSummarys(batchData, false);
	}

	public void generateBatchDataSummarys(MESBasicBatchData batchData, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData")
	public void saveBatchData(MESBasicBatchData batchData,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchData(batchData, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData")
	public void saveBatchData(MESBasicBatchData batchData,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_batchDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchDataHandler");
		String url = null;
		if(batchData.getId() != null && batchData.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchData.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_batchDataHandler/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchData.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_batchDataHandler/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchData);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchData);
		beforeSaveBatchData(batchData, viewIsView);

		if (viewIsView) {
			batchDataDao.saveWithRevertVersion(batchData);
		} else {
			if(isNew)
				batchDataDao.save(batchData);
			else
				batchDataDao.merge(batchData);
		}
		if(null != batchData && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchData.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchData.getId() ? "" : batchData.getId().toString());
			AuditUtil.setColumnStringA(null == batchData.getTag() ? "" : batchData.getTag().toString());
		...
		dealDatagridsSave(batchData,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchDataCodes(batchData, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchDataSummarys(batchData, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchData(batchData, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchData);
			}
		}
		List<MESBasicBatchData> params = new ArrayList<MESBasicBatchData>();
		batchDataDao.flush();
		batchDataDao.clear();
		batchData = batchDataDao.load(batchData.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicBatchData batchData){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchData", batchData);
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData")
	public void mergeBatchData(MESBasicBatchData batchData, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchData);
		beforeSaveBatchData(batchData);
		batchDataDao.merge(batchData);
		if(null != batchData && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchData.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchData.getId() ? "" : batchData.getId().toString());
			AuditUtil.setColumnStringA(null == batchData.getTag() ? "" : batchData.getTag().toString());
		...
		dealDatagridsSave(batchData,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchData(batchData);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicBatchData> findBatchDatas(Page<MESBasicBatchData> page, Criterion... criterions) {
		return batchDataDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicBatchDataDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicBatchDataDealInfo> list=batchDataDao.createQuery("from " + MESBasicBatchDataDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicBatchDataDealInfo dealInfo) {
		batchDataDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicBatchDataDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) batchDataDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicBatchDataDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = batchDataDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = batchDataDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicBatchDataDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = batchDataDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchDatas(Page<MESBasicBatchData> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchDatas(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchDatas(Page<MESBasicBatchData> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchDatas(Page<MESBasicBatchData> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicBatchData> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchData\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchData\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchData\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_batchDataHandler_BatchData" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_batchDataHandler_BatchData" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  batchDataDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchDataDao.createNativeQuery(powerOperateSql, "MESBasic_1_batchDataHandler").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchData\"","MESBasic_1_batchDataHandler",customCondition,"MESBASIC_BATCH_DATAS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"batchData\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicBatchDataDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"batchData\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchData\".LAY_REC ASC, \"batchData\".SORT ASC");
				} else {
					orderPart.append(" \"batchData\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchDataDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchDataDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchDataDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicBatchData> result = new ArrayList<MESBasicBatchData>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicBatchData.class,batchDataDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicBatchData.class,batchDataDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicBatchData batchData : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchData.getId(), "MESBasic_batchDataHandler_batchData");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchData.getId(), "MESBasic_batchDataHandler_batchData");
							if(null != documents && !documents.isEmpty()) {
								batchData.setDocument(documents.get(0));
								batchData.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_batchDataHandler")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_batchDataHandler")+"MESBasic_1_batchDataHandler".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_batchDataHandler",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicBatchData> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicBatchData> result) {
		View view = null;
		...
		Map<Long, MESBasicBatchData> rsMap = new HashMap<Long, MESBasicBatchData>();
		for (MESBasicBatchData batchData : result) {
			ids.add(batchData.getId());
			rsMap.put(batchData.getId(), batchData);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchDataDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchDataDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicBatchData batchData,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(batchData.getOwnerStaffId());
		ti.setOwnerPositionId(batchData.getOwnerPositionId());
		ti.setOwnerDepartmentId(batchData.getOwnerDepartmentId());
		
		ti.setCreateStaff(batchData.getCreateStaff());
		ti.setCreateStaffId(batchData.getCreateStaffId());
		ti.setCreateTime(batchData.getCreateTime());
		ti.setDeleteStaff(batchData.getDeleteStaff());
		ti.setDeleteStaffId(batchData.getDeleteStaffId());
		ti.setDeleteTime(batchData.getDeleteTime());
		ti.setCreateDepartmentId(batchData.getCreateDepartmentId());
		ti.setModifyStaff(batchData.getModifyStaff());
		ti.setModifyStaffId(batchData.getModifyStaffId());
		ti.setCreatePositionId(batchData.getCreatePositionId());
		ti.setPositionLayRec(batchData.getPositionLayRec());
		ti.setTableNo(batchData.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_batchDataHandler");
		ti.setTargetTableName("MESBASIC_BATCH_DATAS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_batchDataHandler_BatchData" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchDataDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchData\"","MESBasic_1_batchDataHandler","","MESBASIC_BATCH_DATAS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchData\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchData\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchData\"","MESBasic_1_batchDataHandler","","MESBASIC_BATCH_DATAS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("batchData.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchData\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchData\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchData\".CID = 1 OR \"batchData\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchData.mc\"").append(".BATCH_DATA) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchDataDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("BD", "MESBasic_1_batchDataHandler", "BD_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchDataDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicBatchData.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicBatchData.MODULE_CODE;}
	protected String getModelCode(){return MESBasicBatchData.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchData 模型实例
	 */
	public void checkUniqueConstraint(MESBasicBatchData batchData){
	}
	
	
	@Override
	public List<MESBasicBatchData> getBatchDatas(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchData.class);
		if(params!=null){
			...
			return batchDataDao.findByHql("from " + MESBasicBatchData.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchDataDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicBatchData> findBatchDatasBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchData.class);
	...
	@Override
	public List<MESBasicBatchData> findBatchDatasByHql(String hql, Object... objects){
		
		return   batchDataDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicBatchData loadBatchDataByBussinessKey(MESBasicBatchData batchData){
		return loadBatchDataByBussinessKey(batchData.getId());
	}
	
	@Override
	public Page<MESBasicBatchData> getBatchDatas(Page<MESBasicBatchData> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchData.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchDataDao.findByPage(page, sql);
			}else{
				page = batchDataDao.findAllByPage(page);
			}
			return page;
		}
		page = batchDataDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchData loadBatchDataByBussinessKey(Serializable bussinessKey){
		return batchDataDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicBatchData> findByProperty(String propertyName, Object object){
		return batchDataDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchData findEntityByProperty(String propertyName, Object object){
		return batchDataDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicBatchData.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchDataDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_batchDataHandler_BatchData","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_batchDataHandler", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicBatchData batchData,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchDataDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicBatchData.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchData);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchData.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBatchData.TABLE_NAME +" where valid = 1";
			list = batchDataDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBatchData.TABLE_NAME;
			list = batchDataDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicBatchData.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchDataDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicBatchDataDaoImpl batchDataDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchData(final List<MESBasicBatchData> insertObjs, final List<MESBasicBatchData> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchData(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchData(final List<MESBasicBatchData> insertObjs, final List<MESBasicBatchData> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1446448588505"),importCount));
		...
		Map<Long, MESBasicBatchData> oldUpdateObjsMap = new HashMap<Long, MESBasicBatchData>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicBatchData bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicBatchData> bizObjList = batchDataDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicBatchData bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicBatchData> nmeObjs = new ArrayList<MESBasicBatchData>();
		...
		long id=autoGeneratorID.getNextId(MESBasicBatchData.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicBatchData batchData:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicBatchData.TABLE_NAME,1,dbId);
			if(batchData.getId() != null ){
				m.put(batchData.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchDataDao.createNativeQuery(sql, "MESBasic_1_batchDataHandler_BatchData"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchDataDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicBatchData.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicBatchData batchData = updateObjs.get(updateIndex);
						Long id = batchData.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchData.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicBatchData.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicBatchData.TABLE_NAME + " order by id desc";
//					Query query = batchDataDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicBatchData batchData = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchData.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchDataDaoImpl.getSession().flush();
		batchDataDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicBatchData bizObj = insertObjs.get(insertIndex);
			batchDataImportService.saveImportDataLog(bizObj, null, "id", "tag");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicBatchData bizObj = updateObjs.get(updateIndex);
			MESBasicBatchData oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchDataImportService.saveImportDataLog(bizObj, oldBizObj, "id", "tag");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = batchDataDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				batchDataDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = batchDataDao.createNativeQuery(sql, "MESBasic_1_batchDataHandler_BatchData").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchDataDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchDataDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchDataDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchDataDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchDataDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchDataDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="ec.flow.save")
	public void saveBatchData(MESBasicBatchData batchData, DataGridService dataGridService){
		saveBatchData(batchData, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="ec.flow.save")
	public void saveBatchData(MESBasicBatchData batchData, DataGridService dataGridService, String viewCode){
		saveBatchData(batchData, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData")
	public void saveBatchData(MESBasicBatchData batchData, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchData(batchData, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData")
	public void saveBatchData(MESBasicBatchData batchData, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_batchDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchDataHandler");
		String url = null;
		if(null != batchData.getId() && batchData.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchData.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData")
	public Map<Object, Object> batchDataDataGridImport(MESBasicBatchData batchData, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchData.getVirtualId();
		
		batchData = batchDataDao.load(batchData.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchData.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchData.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchData);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchData.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData")
	public void mergeBatchData(MESBasicBatchData batchData, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchData);
		beforeSaveBatchData(batchData);
		batchDataDao.merge(batchData);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchData(MESBasicBatchData batchData, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_batchDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchDataHandler");
	...
	@Override
	public Page<MESBasicBatchData> getByPage(Page<MESBasicBatchData> page,DetachedCriteria detachedCriteria){
		return batchDataDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_batchDataHandler").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBatchData(MESBasicBatchData batchData, Object... objects){
		checkUniqueConstraint(batchData);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
		String sql = "select insert_check from "+MESBasicBatchData.TABLE_NAME+" where id = "+batchData.getId();
		/*Byte check = (Byte)batchDataDao.createNativeQuery(sql).uniqueResult();
		if(check != null && check.equals(new Byte("1"))){
			throw new BAPException("本数据已经被其他人修改或删除，请刷页面后重试。");
		}*/
        if(batchDataDao.createNativeQuery(sql).uniqueResult() != null){
			if(batchDataDao.createNativeQuery(sql).uniqueResult().toString().equals("1")){
				throw new BAPException("本数据已经被其他人修改或删除，请刷页面后重试。");
			}
		}
/* CUSTOM CODE END */
	...
	private void afterSaveBatchData(MESBasicBatchData batchData, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
		batchDataDao.flush();
		batchDataDao.clear();
        //SimpleDateFormat sf = new SimpleDateFormat("YYYY-MM-dd HH:mm:ss");
		//String sqlId = "select min(id) from "+MESBasicBatchData.TABLE_NAME+" where id <> "+batchData.getId()+" and DATA_TIME >= '"+sf.format(batchData.getDataTime())+"'";
        BigDecimal naxId = (BigDecimal)itemSeqDao.createNativeQuery("select max_id from "+MESBasicItemSeq.TABLE_NAME+" where seq_Type = 'batchManag_1.0'").uniqueResult();
        Long maxId = new Long(1000);
        if(naxId != null){
        	maxId = naxId.longValue();
        }
        //修改成使用hql来查询数据 modify by yy
        String hql = " from MESBasicBatchData where dataTime >= ? and id != ? order by id asc";
		List<MESBasicBatchData>  batchDatas = batchDataDao.findByHql(hql, batchData.getDataTime(), batchData.getId());
		if(batchDatas.size() > 0){
			MESBasicBatchData minData = batchDatas.get(0);
			if(minData.getId() != null){
				Long id = minData.getId();
              
				if(maxId == null || (maxId.compareTo(id) < 0 && maxId.compareTo(batchData.getId()) < 0)){//没有执行过的位子才能插入
					if(id != null && id.compareTo(batchData.getId()) != 0){//修改没有修改时间
						if(batchData.getId().compareTo(id.longValue()) > 0){//情况1：往前插
							//ID+1
							batchDataDao.createNativeQuery("update "+MESBasicBatchData.TABLE_NAME+" set id=id+1 , version = version + 1 ,insert_check = 1 where id >= "+id.toString()).executeUpdate();
							//插入ID
							batchDataDao.createNativeQuery("update "+MESBasicBatchData.TABLE_NAME+" set id= "+id.toString()+" where id="+(batchData.getId()+1)).executeUpdate();
							if(maxId != null && maxId.compareTo(id) >= 0){//影响最大id
								itemSeqService.updateMaxIdSeq("batchManag_1.0", maxId+1);
							}
						}else{//情况2：往后插
							//ID+1
							batchDataDao.createNativeQuery("update "+MESBasicBatchData.TABLE_NAME+" set id=id+1 where id >= "+id.toString()).executeUpdate();
							//插入ID
							batchDataDao.createNativeQuery("update "+MESBasicBatchData.TABLE_NAME+" set id= "+id.toString()+",version = version + 1 where id="+batchData.getId()).executeUpdate();
						}
					}
				}
				batchDataDao.flush();
				batchDataDao.clear();
				batchData = batchDataDao.load(id.longValue());
			}
		}
/* CUSTOM CODE END */
	}
	...
	@Override
	public void beforeExcelImportBatchData(List<MESBasicBatchData> batchDatas, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchData(List<MESBasicBatchData> batchDatas){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchData(List<MESBasicBatchData> batchDatas, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchData(List<MESBasicBatchData> batchDatas){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchData(MESBasicBatchData batchData){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchData(MESBasicBatchData batchData){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchData(MESBasicBatchData batchData){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchData(MESBasicBatchData batchData){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicBatchData batchData, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicBatchData batchData, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchData(List<MESBasicBatchData> insertObjs, List<MESBasicBatchData> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchData,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchData(List<MESBasicBatchData> insertObjs, List<MESBasicBatchData> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchData,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
	@Override
	public Page<MESBasicBatchData> findDataTransPage(Page<MESBasicBatchData> page,String sql,Object... params){
		return batchDataDao.findByPage(page, sql,params);
	}
	
	@Override
	public void updateCheck(Long id){
		String sql = "update "+MESBasicBatchData.TABLE_NAME+" set insert_check = 0 where id = "+id;
		batchDataDao.createNativeQuery(sql).executeUpdate();
		batchDataDao.flush();
		batchDataDao.clear();
	}
/* CUSTOM CODE END */
