...
import com.supcon.orchid.MESBasic.entities.MESBasicBaseVendorMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicBaseVendorService;
import com.supcon.orchid.MESBasic.services.MESBasicBaseVendorImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicBaseVendorDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_baseVendorService")
@Transactional
public class MESBasicBaseVendorServiceImpl  extends BaseServiceImpl<MESBasicBaseVendor> implements MESBasicBaseVendorService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicBaseVendor> bapBaseService;
	...
	@Autowired
	private MESBasicBaseVendorImportService baseVendorImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicBaseVendor.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  baseVendorDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicBaseVendor.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = baseVendorDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBaseVendor.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBaseVendor.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  baseVendorDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicBaseVendor.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicBaseVendor.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicBaseVendor> list = (List<MESBasicBaseVendor>) baseVendorDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseVendor getBaseVendor(long id){
		return getBaseVendor(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseVendor getBaseVendor(long id, String viewCode){
		MESBasicBaseVendor baseVendor = baseVendorDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBaseVendorAsJSON(long id, String include){
		MESBasicBaseVendor baseVendor = baseVendorDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(baseVendor, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendor(MESBasicBaseVendor baseVendor){
		if(null != baseVendor && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendor.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseVendor.getVendorCode() ? "" : baseVendor.getVendorCode().toString());
			AuditUtil.setColumnStringA(null == baseVendor.getVendorName() ? "" : baseVendor.getVendorName().toString());
			AuditUtil.setColumnStringC(baseVendor.getId().toString());
		}
		beforeDeleteBaseVendor(baseVendor);
		baseVendorDao.delete(baseVendor);
		afterDeleteBaseVendor(baseVendor);
		baseVendorDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseVendor/del", baseVendor,MESBasicBaseVendorMneCode.class);
		List<MESBasicBaseVendor> params = new ArrayList<MESBasicBaseVendor>();
		params.add(baseVendor);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseVendor");
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendor(List<Long> baseVendorIds){
		deleteBaseVendor(baseVendorIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendor(List<Long> baseVendorIds, String eventTopic) {
		List<MESBasicBaseVendor> baseVendors = new ArrayList<MESBasicBaseVendor>();
		for(Long baseVendorId : baseVendorIds){
			MESBasicBaseVendor baseVendor = getBaseVendor(baseVendorId);
			baseVendors.add(baseVendor);
			if(null != baseVendor && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendor.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == baseVendor.getVendorName() ? "" : "," + baseVendor.getVendorName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == baseVendor.getVendorName() ? "" : baseVendor.getVendorName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == baseVendor.getVendorCode() ? "" : "," + baseVendor.getVendorCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == baseVendor.getVendorCode() ? "" : baseVendor.getVendorCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == baseVendor.getId() ? "" : "," + baseVendor.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == baseVendor.getId() ? "" : baseVendor.getId().toString());
				}
			}
		}
		
		for(MESBasicBaseVendor baseVendor : baseVendors){
			beforeDeleteBaseVendor(baseVendor);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(baseVendors != null && baseVendors.size() > 0 ){
			for(MESBasicBaseVendor baseVendor : baseVendors){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),baseVendor, baseVendor.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(baseVendorIds != null && baseVendorIds.size() > 0) {
			String hql = "update " + MESBasicBaseVendor.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = baseVendorDao.createQuery(hql);
			query.setParameterList("ids", baseVendorIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicBaseVendor baseVendorz : baseVendors){
				baseVendorz.setValid(false);
				List<MESBasicBaseVendor> params = new ArrayList<MESBasicBaseVendor>();
				params.add(baseVendorz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_baseVendor");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendor(long baseVendorId, int baseVendorVersion){
		this.deleteBaseVendor(baseVendorId, baseVendorVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendor(long baseVendorId, int baseVendorVersion,SignatureLog signatureLog){
		MESBasicBaseVendor baseVendor = getBaseVendor(baseVendorId);
		...
		beforeDeleteBaseVendor(baseVendor);
		baseVendorDao.delete(baseVendorId, baseVendorVersion);
		afterDeleteBaseVendor(baseVendor);
		baseVendorDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseVendor/del", baseVendor,MESBasicBaseVendorMneCode.class);
		List<MESBasicBaseVendor> params = new ArrayList<MESBasicBaseVendor>();
		...
		if(signatureLog != null ) {
			...
			businessKey=baseVendor.getVendorCode();
			signatureLog.setTableId(baseVendor.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendor(String baseVendorIds){
		this.deleteBaseVendor(baseVendorIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.common.delete",operType="3")
	public void deleteBaseVendor(String baseVendorIds,SignatureLog signatureLog){
		deleteCollection(baseVendorIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBaseVendor(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_baseVendor#modelCode:MESBasic_1_baseVendor_BaseVendor#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="ec.engine.restore",operType="10")
	public void restoreBaseVendor(String baseVendorIds){
		restoreCollection(baseVendorIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBaseVendor(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="ec.engine.restore",operType="10")
	public void restoreBaseVendor(long baseVendorId){
		
		findBusinessKeyUsed(baseVendorId);	//判断业务主键是否重复
		...
		beforeRestoreBaseVendor(baseVendor);
		baseVendor.setValid(true);
		baseVendorDao.update(baseVendor);
		afterRestoreBaseVendor(baseVendor);
		baseVendorDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseVendor/add", baseVendor,MESBasicBaseVendorMneCode.class);
		List<MESBasicBaseVendor> params = new ArrayList<MESBasicBaseVendor>();
		params.add(baseVendor);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long baseVendorId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_baseVendor_BaseVendor");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicBaseVendor.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicBaseVendor.TABLE_NAME +" where id = ? )";
			List<Object> list =  baseVendorDao.createNativeQuery(sql,baseVendorId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBaseVendor(List<MESBasicBaseVendor>  baseVendors){
		for(MESBasicBaseVendor baseVendor:baseVendors)  {
			saveBaseVendor(baseVendor, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBaseVendor(List<MESBasicBaseVendor>  baseVendors){
		for(MESBasicBaseVendor baseVendor:baseVendors)  {
			saveBaseVendor(baseVendor, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="ec.flow.save")
	public void saveBaseVendor(MESBasicBaseVendor baseVendor, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBaseVendor(baseVendor, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="ec.flow.save")
	public void saveBaseVendor(MESBasicBaseVendor baseVendor, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBaseVendor(baseVendor, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBaseVendorCodes(MESBasicBaseVendor baseVendor) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBaseVendorCodes(baseVendor, false);
	}
	
	public void generateBaseVendorCodes(MESBasicBaseVendor baseVendor, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBaseVendorSummarys(MESBasicBaseVendor baseVendor) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBaseVendorSummarys(baseVendor, false);
	}

	public void generateBaseVendorSummarys(MESBasicBaseVendor baseVendor, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor")
	public void saveBaseVendor(MESBasicBaseVendor baseVendor,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBaseVendor(baseVendor, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor")
	public void saveBaseVendor(MESBasicBaseVendor baseVendor,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_baseVendor";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseVendor");
		String url = null;
		if(baseVendor.getId() != null && baseVendor.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendor.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_baseVendor/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendor.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_baseVendor/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, baseVendor);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(baseVendor);
		beforeSaveBaseVendor(baseVendor, viewIsView);

		if (viewIsView) {
			baseVendorDao.saveWithRevertVersion(baseVendor);
		} else {
			if(isNew)
				baseVendorDao.save(baseVendor);
			else
				baseVendorDao.merge(baseVendor);
		}
		if(null != baseVendor && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendor.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseVendor.getVendorCode() ? "" : baseVendor.getVendorCode().toString());
			AuditUtil.setColumnStringA(null == baseVendor.getVendorName() ? "" : baseVendor.getVendorName().toString());
		...
		dealDatagridsSave(baseVendor,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBaseVendorCodes(baseVendor, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBaseVendorSummarys(baseVendor, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBaseVendor(baseVendor, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, baseVendor);
			}
		}
		List<MESBasicBaseVendor> params = new ArrayList<MESBasicBaseVendor>();
		baseVendorDao.flush();
		baseVendorDao.clear();
		baseVendor = baseVendorDao.load(baseVendor.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicBaseVendor baseVendor){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("baseVendor", baseVendor);
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor")
	public void mergeBaseVendor(MESBasicBaseVendor baseVendor, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(baseVendor);
		beforeSaveBaseVendor(baseVendor);
		baseVendorDao.merge(baseVendor);
		if(null != baseVendor && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendor.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == baseVendor.getVendorCode() ? "" : baseVendor.getVendorCode().toString());
			AuditUtil.setColumnStringA(null == baseVendor.getVendorName() ? "" : baseVendor.getVendorName().toString());
		...
		dealDatagridsSave(baseVendor,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBaseVendor(baseVendor);
		baseVendorDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/baseVendor/add", baseVendor,MESBasicBaseVendorMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicBaseVendor> findBaseVendors(Page<MESBasicBaseVendor> page, Criterion... criterions) {
		return baseVendorDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicBaseVendorDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicBaseVendorDealInfo> list=baseVendorDao.createQuery("from " + MESBasicBaseVendorDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicBaseVendorDealInfo dealInfo) {
		baseVendorDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicBaseVendorDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) baseVendorDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicBaseVendorDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = baseVendorDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = baseVendorDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicBaseVendorDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = baseVendorDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseVendors(Page<MESBasicBaseVendor> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBaseVendors(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseVendors(Page<MESBasicBaseVendor> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBaseVendors(Page<MESBasicBaseVendor> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicBaseVendor> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"baseVendor\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"baseVendor\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"baseVendor\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_baseVendor_BaseVendor" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_baseVendor_BaseVendor" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  baseVendorDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  baseVendorDao.createNativeQuery(powerOperateSql, "MESBasic_1_baseVendor").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseVendor\"","MESBasic_1_baseVendor",customCondition,"S2BASE_VENDOR",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"baseVendor\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicBaseVendorDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"baseVendor\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"baseVendor\".LAY_REC ASC, \"baseVendor\".SORT ASC");
				} else {
					orderPart.append(" \"baseVendor\".VENDOR_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) baseVendorDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=baseVendorDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = baseVendorDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicBaseVendor> result = new ArrayList<MESBasicBaseVendor>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicBaseVendor.class,baseVendorDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicBaseVendor.class,baseVendorDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicBaseVendor baseVendor : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(baseVendor.getId(), "MESBasic_baseVendor_baseVendor");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(baseVendor.getId(), "MESBasic_baseVendor_baseVendor");
							if(null != documents && !documents.isEmpty()) {
								baseVendor.setDocument(documents.get(0));
								baseVendor.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_baseVendor")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_baseVendor")+"MESBasic_1_baseVendor".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_baseVendor",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicBaseVendor> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicBaseVendor> result) {
		View view = null;
		...
		Map<Long, MESBasicBaseVendor> rsMap = new HashMap<Long, MESBasicBaseVendor>();
		for (MESBasicBaseVendor baseVendor : result) {
			ids.add(baseVendor.getId());
			rsMap.put(baseVendor.getId(), baseVendor);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(baseVendorDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = baseVendorDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicBaseVendor baseVendor,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(baseVendor.getOwnerStaffId());
		ti.setOwnerPositionId(baseVendor.getOwnerPositionId());
		ti.setOwnerDepartmentId(baseVendor.getOwnerDepartmentId());
		
		ti.setCreateStaff(baseVendor.getCreateStaff());
		ti.setCreateStaffId(baseVendor.getCreateStaffId());
		ti.setCreateTime(baseVendor.getCreateTime());
		ti.setDeleteStaff(baseVendor.getDeleteStaff());
		ti.setDeleteStaffId(baseVendor.getDeleteStaffId());
		ti.setDeleteTime(baseVendor.getDeleteTime());
		ti.setCreateDepartmentId(baseVendor.getCreateDepartmentId());
		ti.setModifyStaff(baseVendor.getModifyStaff());
		ti.setModifyStaffId(baseVendor.getModifyStaffId());
		ti.setCreatePositionId(baseVendor.getCreatePositionId());
		ti.setPositionLayRec(baseVendor.getPositionLayRec());
		ti.setTableNo(baseVendor.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_baseVendor");
		ti.setTargetTableName("S2BASE_VENDOR");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_baseVendor_BaseVendor" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  baseVendorDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseVendor\"","MESBasic_1_baseVendor","","S2BASE_VENDOR",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"baseVendor\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"baseVendor\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"baseVendor\"","MESBasic_1_baseVendor","","S2BASE_VENDOR",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("baseVendor.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"baseVendor\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"baseVendor\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"baseVendor\".CID = 1 OR \"baseVendor\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("baseVendor.mc\"").append(".BASE_VENDOR) not in ("+notinValue+")");
			}
			
			SQLQuery query = baseVendorDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("baseVendor", "MESBasic_1_baseVendor", "baseVendor_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=baseVendorDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicBaseVendor.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicBaseVendor.MODULE_CODE;}
	protected String getModelCode(){return MESBasicBaseVendor.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param baseVendor 模型实例
	 */
	public void checkUniqueConstraint(MESBasicBaseVendor baseVendor){
		if(baseVendor.getId()==null){
			List<MESBasicBaseVendor> baseVendors = baseVendorDao.findByCriteria(Restrictions.eq("vendorCode", baseVendor.getVendorCode()), Restrictions.eq("valid", true));
			if(baseVendors!=null && !baseVendors.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1449537304035", getCurrentUser().getLanguage()), baseVendor.getVendorCode());
			}
		} else {
			List<MESBasicBaseVendor> baseVendors = baseVendorDao.findByCriteria(Restrictions.ne("id", baseVendor.getId()), Restrictions.eq("vendorCode", baseVendor.getVendorCode()), Restrictions.eq("valid", true));
			if(baseVendors!=null && !baseVendors.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1449537304035", getCurrentUser().getLanguage()), baseVendor.getVendorCode());
			}
	...
	@Override
	public List<MESBasicBaseVendor> getBaseVendors(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseVendor.class);
		if(params!=null){
			...
			return baseVendorDao.findByHql("from " + MESBasicBaseVendor.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return baseVendorDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicBaseVendor> findBaseVendorsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseVendor.class);
	...
	@Override
	public List<MESBasicBaseVendor> findBaseVendorsByHql(String hql, Object... objects){
		
		return   baseVendorDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicBaseVendor loadBaseVendorByBussinessKey(MESBasicBaseVendor baseVendor){
		return loadBaseVendorByBussinessKey(baseVendor.getVendorCode());
	}
	
	@Override
	public Page<MESBasicBaseVendor> getBaseVendors(Page<MESBasicBaseVendor> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBaseVendor.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = baseVendorDao.findByPage(page, sql);
			}else{
				page = baseVendorDao.findAllByPage(page);
			}
			return page;
		}
		page = baseVendorDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseVendor loadBaseVendorByBussinessKey(Serializable bussinessKey){
		return baseVendorDao.findEntityByCriteria(Restrictions.eq("vendorCode", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicBaseVendor> findByProperty(String propertyName, Object object){
		return baseVendorDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBaseVendor findEntityByProperty(String propertyName, Object object){
		return baseVendorDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicBaseVendor.JPA_NAME + " set valid=0 where vendorCode in (:businessKeys)";
			Query query = baseVendorDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_baseVendor_BaseVendor","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_baseVendor", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicBaseVendor baseVendor,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,baseVendorDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicBaseVendor.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, baseVendor);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",baseVendor.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBaseVendor.TABLE_NAME +" where valid = 1";
			list = baseVendorDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBaseVendor.TABLE_NAME;
			list = baseVendorDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicBaseVendor.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = baseVendorDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicBaseVendorDaoImpl baseVendorDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBaseVendor(final List<MESBasicBaseVendor> insertObjs, final List<MESBasicBaseVendor> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBaseVendor(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBaseVendor(final List<MESBasicBaseVendor> insertObjs, final List<MESBasicBaseVendor> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1449536448032"),importCount));
		...
		Map<Long, MESBasicBaseVendor> oldUpdateObjsMap = new HashMap<Long, MESBasicBaseVendor>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicBaseVendor bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicBaseVendor> bizObjList = baseVendorDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicBaseVendor bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicBaseVendor> nmeObjs = new ArrayList<MESBasicBaseVendor>();
		...
		long id=autoGeneratorID.getNextId(MESBasicBaseVendor.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicBaseVendor baseVendor:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicBaseVendor.TABLE_NAME,1,dbId);
			if(baseVendor.getVendorCode() != null && !baseVendor.getVendorCode().equals("") ){
				m.put(baseVendor.getVendorCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) baseVendorDao.createNativeQuery(sql, "MESBasic_1_baseVendor_BaseVendor"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		baseVendorDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicBaseVendor.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicBaseVendor baseVendor = updateObjs.get(updateIndex);
						Long id = baseVendor.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = baseVendor.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicBaseVendor.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
					...
					insertSql += dbId + ",CID,VENDOR_CREATEPERSON,VENDOR_CREATETIME,EFFECTIVE_STATE) VALUES (";
					for(int i = 0;i<columnMaps.size();i++){
						insertSql += "?,";
					}
					insertSql += "?,?,?,?,?)";
					
//					String idSql = "select id from " + MESBasicBaseVendor.TABLE_NAME + " order by id desc";
//					Query query = baseVendorDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicBaseVendor baseVendor = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = baseVendor.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicBaseVendorMneCode.class;
					...
					for(MESBasicBaseVendor baseVendor:updateObjs)  {
						Long id = baseVendor.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicBaseVendor baseVendor:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(baseVendor);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, baseVendor.getId());
		...
		baseVendorDaoImpl.getSession().flush();
		baseVendorDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicBaseVendor bizObj = insertObjs.get(insertIndex);
			baseVendorImportService.saveImportDataLog(bizObj, null, "vendorCode", "vendorName");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicBaseVendor bizObj = updateObjs.get(updateIndex);
			MESBasicBaseVendor oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			baseVendorImportService.saveImportDataLog(bizObj, oldBizObj, "vendorCode", "vendorName");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = baseVendorDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				baseVendorDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = baseVendorDao.createNativeQuery(sql, "MESBasic_1_baseVendor_BaseVendor").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = baseVendorDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = baseVendorDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = baseVendorDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?baseVendorDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  baseVendorDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,baseVendorDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="ec.flow.save")
	public void saveBaseVendor(MESBasicBaseVendor baseVendor, DataGridService dataGridService){
		saveBaseVendor(baseVendor, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="ec.flow.save")
	public void saveBaseVendor(MESBasicBaseVendor baseVendor, DataGridService dataGridService, String viewCode){
		saveBaseVendor(baseVendor, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor")
	public void saveBaseVendor(MESBasicBaseVendor baseVendor, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBaseVendor(baseVendor, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor")
	public void saveBaseVendor(MESBasicBaseVendor baseVendor, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_baseVendor";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseVendor");
		String url = null;
		if(null != baseVendor.getId() && baseVendor.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBaseVendor.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor")
	public Map<Object, Object> baseVendorDataGridImport(MESBasicBaseVendor baseVendor, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = baseVendor.getVirtualId();
		
		baseVendor = baseVendorDao.load(baseVendor.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, baseVendor.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = baseVendor.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(baseVendor);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, baseVendor.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor")
	public void mergeBaseVendor(MESBasicBaseVendor baseVendor, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(baseVendor);
		beforeSaveBaseVendor(baseVendor);
		baseVendorDao.merge(baseVendor);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="foundation.batch.save",operType="4")
	public void batchSaveBaseVendor(MESBasicBaseVendor baseVendor, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_baseVendor";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_baseVendor");
	...
	@Override
	public Page<MESBasicBaseVendor> getByPage(Page<MESBasicBaseVendor> page,DetachedCriteria detachedCriteria){
		return baseVendorDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_baseVendor").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBaseVendor(MESBasicBaseVendor baseVendor, Object... objects){
		checkUniqueConstraint(baseVendor);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBaseVendor(MESBasicBaseVendor baseVendor, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBaseVendor(List<MESBasicBaseVendor> baseVendors, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBaseVendor(List<MESBasicBaseVendor> baseVendors){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBaseVendor(List<MESBasicBaseVendor> baseVendors, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBaseVendor(List<MESBasicBaseVendor> baseVendors){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBaseVendor(MESBasicBaseVendor baseVendor){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
		//删除时修改启用状态；
		baseVendor.setVendorState("0");
/* CUSTOM CODE END */
	}
	
	private void afterDeleteBaseVendor(MESBasicBaseVendor baseVendor){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBaseVendor(MESBasicBaseVendor baseVendor){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBaseVendor(MESBasicBaseVendor baseVendor){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicBaseVendor baseVendor, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicBaseVendor baseVendor, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBaseVendor(List<MESBasicBaseVendor> insertObjs, List<MESBasicBaseVendor> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBaseVendor,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBaseVendor(List<MESBasicBaseVendor> insertObjs, List<MESBasicBaseVendor> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBaseVendor,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
    /*
	根据传入供应商id：vendorId，启用该供应商
	*/
	public void doEnableVendor(long vendorId){
		List<Long> bussinessKeyList = new ArrayList<Long>();
		bussinessKeyList.add(Long.valueOf(vendorId));
		
		String hql = "update " + MESBasicBaseVendor.JPA_NAME + " set vendorState = '2' where id in(:ids)";
		Query query = baseVendorDao.createQuery(hql);
		query.setParameterList("ids", bussinessKeyList);
		query.executeUpdate();
	}
	
	/*
	根据传入供应商id：vendorId，停用该供应商
	*/
	public void doDisableVendor(long vendorId){
		List<Long> bussinessKeyList = new ArrayList<Long>();
		bussinessKeyList.add(Long.valueOf(vendorId));
		
		String hql = "update " + MESBasicBaseVendor.JPA_NAME + " set vendorState = '1' where id in(:ids)";
		Query query = baseVendorDao.createQuery(hql);
		query.setParameterList("ids", bussinessKeyList);
		query.executeUpdate();
	}
	...
	/*
	根据传入供应商id：ids字符串，删除该供应商
	*/
	public void doDeleteVendor(String ids){
		List<Long> bussinessKeyList = new ArrayList<Long>();
		String[] idst = ids.split(",");
		for(String idVersion : idst) {
			String id = idVersion.split("@")[0];
			String version = idVersion.split("@")[1];
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				bussinessKeyList.add(Long.valueOf(id));
			}
		}
		String hql = "update " + MESBasicBaseVendor.JPA_NAME + " set vendorState = '0' where id in(:ids)";
		Query query = baseVendorDao.createQuery(hql);
		query.setParameterList("ids", bussinessKeyList);
		query.executeUpdate();
	}
