...
/* CUSTOM CODE START(action,import,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicItemSeqAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicItemSeq> {
	...
	private MESBasicItemSeq itemSeq;
	...
	//导出excel时,主模型内容
	List<MESBasicItemSeq> queryResults = new ArrayList<MESBasicItemSeq>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/thirdDataHandler/itemSeq/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/thirdDataHandler/itemSeq/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_thirdDataHandler_ItemSeq" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicItemSeq> itemSeqs;
	
	public List<MESBasicItemSeq> getItemSeqs() {
		return itemSeqs;
	}
	...
	private Page<MESBasicItemSeq> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			itemSeq = new MESBasicItemSeq();
		} else {
			if(id > 0){
				itemSeq = itemSeqService.getItemSeq(id, viewCode);
			}
		}
	...
	public Page<MESBasicItemSeq> getPage() {
		if (null == page) {
			page = new Page<MESBasicItemSeq>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicItemSeq getItemSeq() {
		return itemSeq;
	}		

	public void setItemSeq(MESBasicItemSeq itemSeq) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = itemSeqService.getItemSeqAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1427346429041
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String itemSeqReferenceCopy() throws Exception{
			MESBasicItemSeq oldItemSeq = null;
			if(isEditNew()){
				...
				oldItemSeq = new MESBasicItemSeq();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,itemSeq.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, itemSeq.getId());
				...
				oldItemSeq = itemSeqService.getItemSeq(id);
			}
			
			
			itemSeq = itemSeqService.getItemSeq(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_thirdDataHandler_ItemSeq");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicItemSeq.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldItemSeq, null);
						Method setValue = MESBasicItemSeq.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(itemSeq, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			itemSeq.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=itemSeqService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return itemSeqReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			itemSeq = itemSeqService.getItemSeq(this.itemSeq.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,itemSeq.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = itemSeqService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(itemSeqService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(itemSeqService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_thirdDataHandler_ItemSeq", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"itemSeq\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"itemSeq\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"itemSeq\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"itemSeq\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		itemSeqService.findDataGridPage(dg,dgclass,getDgPage(),itemSeq,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(itemSeq.getId() != null && itemSeq.getId() != -1){
				itemSeq = itemSeqService.getItemSeq(itemSeq.getId());
			}
			itemSeqService.findDataGridPage(dg,dgclass,getDgPage(),itemSeq,searchCondition,params);
	...
	private void save() throws Exception {
		...
		itemSeqService.saveItemSeq(itemSeq,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "submit");
		bap_validate_datagrids=itemSeqService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", itemSeq.getId());
		...
		fileuploads.put("linkId", itemSeq.getId());
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		itemSeqService.deleteItemSeq(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "restore");
		itemSeqService.restoreItemSeq(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicItemSeq> findExportDataInfos(Page<MESBasicItemSeq> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = itemSeqService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = itemSeqService.getAssProperty(p.getCode());	
									//String assModelCode = itemSeqService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			itemSeqService.findItemSeqs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			itemSeqService.findItemSeqs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicItemSeq> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicItemSeq result : results){
	...
	public boolean isEditNew(){
		//return null == this.itemSeq || null == this.itemSeq.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/thirdDataHandler/itemSeq/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/thirdDataHandler/itemSeq/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicItemSeqService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_thirdDataHandler_ItemSeq", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_thirdDataHandler_ItemSeq", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_thirdDataHandler_ItemSeq";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_ItemSeq", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
		_auditLog.setColumnStringA(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
		_auditLog.setColumnStringC(itemSeq.getId().toString());
		_auditLog.setColumnStringD(MESBasicItemSeq.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicItemSeqAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, itemSeq,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, itemSeq,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,itemSeq);
		...
		printlog.setBusinessId(itemSeq.getId());
		printlog.setBussinessKey(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
		printlog.setMainDisplay(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(itemSeq,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(itemSeqService.getItemSeq(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, itemSeq,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				itemSeq = itemSeqService.getItemSeq(Long.parseLong(arrId[i]));
				if(null != itemSeq){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == itemSeq.getId() ? "" : "," + itemSeq.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == itemSeq.getId() ? "" : "," + itemSeq.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == itemSeq.getId() ? "" : "," + itemSeq.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, itemSeq,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,itemSeq);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/thirdDataHandler/itemSeq/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_thirdDataHandler_ItemSeq");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_thirdDataHandler_ItemSeq")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_thirdDataHandler_ItemSeq");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_thirdDataHandler_ItemSeq");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = itemSeqService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
