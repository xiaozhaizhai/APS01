...
/* CUSTOM CODE START(action,import,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicQACheckIndexAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicQACheckIndex> {
	...
	private MESBasicQACheckIndex qACheckIndex;
	...
	//导出excel时,主模型内容
	List<MESBasicQACheckIndex> queryResults = new ArrayList<MESBasicQACheckIndex>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/checkIndex/qACheckIndex/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/checkIndex/qACheckIndex/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_checkIndex_QACheckIndex" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicQACheckIndex> qACheckIndexs;
	
	public List<MESBasicQACheckIndex> getQACheckIndexs() {
		return qACheckIndexs;
	}
	...
	private Page<MESBasicQACheckIndex> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			qACheckIndex = new MESBasicQACheckIndex();
		} else {
			if(id > 0){
				qACheckIndex = qACheckIndexService.getQACheckIndex(id, viewCode);
			}
		}
	...
	public Page<MESBasicQACheckIndex> getPage() {
		if (null == page) {
			page = new Page<MESBasicQACheckIndex>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicQACheckIndex getQACheckIndex() {
		return qACheckIndex;
	}		

	public void setQACheckIndex(MESBasicQACheckIndex qACheckIndex) {
		this.qACheckIndex = qACheckIndex;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = qACheckIndexService.getQACheckIndexAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String qACheckIndexReferenceCopy() throws Exception{
			MESBasicQACheckIndex oldQACheckIndex = null;
			if(isEditNew()){
				...
				oldQACheckIndex = new MESBasicQACheckIndex();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,qACheckIndex.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, qACheckIndex.getId());
				...
				oldQACheckIndex = qACheckIndexService.getQACheckIndex(id);
			}
			
			
			qACheckIndex = qACheckIndexService.getQACheckIndex(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_checkIndex_QACheckIndex");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicQACheckIndex.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldQACheckIndex, null);
						Method setValue = MESBasicQACheckIndex.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(qACheckIndex, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			qACheckIndex.setCreateDepartmentId(oldQACheckIndex.getCreateDepartmentId());
			qACheckIndex.setCreatePositionId(oldQACheckIndex.getCreatePositionId());
			qACheckIndex.setCreateStaffId(oldQACheckIndex.getCreateStaffId());
			qACheckIndex.setOwnerDepartmentId(oldQACheckIndex.getOwnerDepartmentId());
			qACheckIndex.setOwnerPositionId(oldQACheckIndex.getOwnerPositionId());
			qACheckIndex.setOwnerStaffId(oldQACheckIndex.getOwnerStaffId());
			qACheckIndex.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=qACheckIndexService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return qACheckIndexReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			qACheckIndex = qACheckIndexService.getQACheckIndex(this.qACheckIndex.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,qACheckIndex.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/checkIndex/qACheckIndex/checkIndexEdit", results = { @Result(type = FREEMARKER, location = "checkIndexEdit.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = qACheckIndexService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(qACheckIndexService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(qACheckIndexService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_checkIndex_QACheckIndex", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/checkIndex/qACheckIndex/checkIndexRef", results = { @Result(type = FREEMARKER, location = "checkIndexRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/checkIndex/qACheckIndex/checkIndexEdit-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.checkIndexCode,result.checkIndexName,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="checkIndexEdit-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"qACheckIndex\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"qACheckIndex\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"qACheckIndex\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"qACheckIndex\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			qACheckIndexService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/checkIndex/qACheckIndex/checkIndexEdit-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/checkIndex/qACheckIndex/checkIndexRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.unitID.name,result.valueKind,result.showValues,result.memoField,result.unitID.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="checkIndexRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/allTempleteDownload", results = { @Result(name="excel",type="excel",location="checkIndexEdit-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_checkIndex_QACheckIndex");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		qACheckIndexService.findDataGridPage(dg,dgclass,getDgPage(),qACheckIndex,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(qACheckIndex.getId() != null && qACheckIndex.getId() != -1){
				qACheckIndex = qACheckIndexService.getQACheckIndex(qACheckIndex.getId());
			}
			qACheckIndexService.findDataGridPage(dg,dgclass,getDgPage(),qACheckIndex,searchCondition,params);
	...
	private void save() throws Exception {
		...
		qACheckIndexService.saveQACheckIndex(qACheckIndex,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "submit");
		bap_validate_datagrids=qACheckIndexService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", qACheckIndex.getId());
		...
		fileuploads.put("linkId", qACheckIndex.getId());
		bapAfterCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/checkIndex/qACheckIndex/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		qACheckIndexService.deleteQACheckIndex(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "restore");
		qACheckIndexService.restoreQACheckIndex(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/checkIndex/qACheckIndex/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/checkIndex/qACheckIndex/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "showDealInfoData");
		if (null != tableInfoId) {
			int count=qACheckIndexService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = qACheckIndexService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = qACheckIndexService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicQACheckIndex> findExportDataInfos(Page<MESBasicQACheckIndex> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = qACheckIndexService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = qACheckIndexService.getAssProperty(p.getCode());	
									//String assModelCode = qACheckIndexService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			qACheckIndexService.findQACheckIndexs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			qACheckIndexService.findQACheckIndexs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicQACheckIndex> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicQACheckIndex result : results){
	...
	public boolean isEditNew(){
		//return null == this.qACheckIndex || null == this.qACheckIndex.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/checkIndex/qACheckIndex/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/checkIndex/qACheckIndex/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicQACheckIndexService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_checkIndex", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_checkIndex");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_checkIndex_QACheckIndex", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_checkIndex_QACheckIndex", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_checkIndex_QACheckIndex";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_checkIndex_QACheckIndex", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicQACheckIndex qACheckIndex = null;
		if(null != id) {
			qACheckIndex = qACheckIndexService.getQACheckIndex(id);
		}
		...
		_auditLog.setColumnStringB(null == qACheckIndex.getCode() ? "" : qACheckIndex.getCode().toString());
		_auditLog.setColumnStringA(null == qACheckIndex.getName() ? "" : qACheckIndex.getName().toString());
		_auditLog.setColumnStringC(qACheckIndex.getId().toString());
		_auditLog.setColumnStringD(MESBasicQACheckIndex.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_checkIndex");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicQACheckIndexAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, qACheckIndex,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, qACheckIndex,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,qACheckIndex);
		...
		printlog.setEntity("MESBasic_1_checkIndex");
		...
		printlog.setBusinessId(qACheckIndex.getId());
		printlog.setBussinessKey(null == qACheckIndex.getCode() ? "" : qACheckIndex.getCode().toString());
		printlog.setMainDisplay(null == qACheckIndex.getName() ? "" : qACheckIndex.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(qACheckIndex,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(qACheckIndexService.getQACheckIndex(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, qACheckIndex,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				qACheckIndex = qACheckIndexService.getQACheckIndex(Long.parseLong(arrId[i]));
				if(null != qACheckIndex){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == qACheckIndex.getName() ? "" : "," + qACheckIndex.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == qACheckIndex.getName() ? "" : qACheckIndex.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == qACheckIndex.getCode() ? "" : "," + qACheckIndex.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == qACheckIndex.getCode() ? "" : qACheckIndex.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == qACheckIndex.getId() ? "" : "," + qACheckIndex.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == qACheckIndex.getId() ? "" : qACheckIndex.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, qACheckIndex,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,qACheckIndex);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/checkIndex/qACheckIndex/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicQACheckIndex> findExportPageInfos(Page<MESBasicQACheckIndex> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_checkIndex" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = qACheckIndexService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicQACheckIndex queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = qACheckIndexService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = qACheckIndexService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_checkIndex_QACheckIndex");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_checkIndex_QACheckIndex")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_checkIndex_QACheckIndex");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_checkIndex_QACheckIndex");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = qACheckIndexService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
