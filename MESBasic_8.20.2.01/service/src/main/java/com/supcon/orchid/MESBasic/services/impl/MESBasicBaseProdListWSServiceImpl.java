...
import com.supcon.orchid.MESBasic.services.MESBasicBaseProdListWSService;
import com.supcon.orchid.MESBasic.services.MESBasicBaseProdListService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_prodList_BaseProdList,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicBaseProdList",targetNamespace = "http://ws.supcon.com")
public class MESBasicBaseProdListWSServiceImpl implements MESBasicBaseProdListWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getBaseProdListByBusinessKey(
		...
		try{
			...
			MESBasicBaseProdList baseProdList = baseProdListService.loadBaseProdListByBussinessKey(businessKey);
			if(baseProdList == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, baseProdList);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateBaseProdList(
		...
		try{
			if(baseProdList == null){
				return set500Error(result, "物品目录不能为空。");
			}
			...
			if(!beforeUpdateBaseProdList(baseProdList)){
				return set404Error(result);
			}
			baseProdListService.saveBaseProdList(baseProdList, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addBaseProdList(
		...
		try{
			...
			if(baseProdList.getProdListCode() == null){
				return set500Error(result, "业务主键不能为空。");
			}
			...
			try{
				checkBaseProdListData(baseProdList);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			baseProdListService.saveBaseProdList(baseProdList, null);
			setSuccessResult(result, baseProdList.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteBaseProdListByBusinessKey(
		...
		try{
			...
			try{
				baseProdList = baseProdListService.loadBaseProdListByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddBaseProdList(
		...
		try{
			...
			int count = 1;
			for(MESBasicBaseProdList baseProdList : baseProdListList){
				if(baseProdList.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(baseProdList.getProdListCode() == null || baseProdList.getProdListCode().toString().trim().isEmpty()){
			...
			baseProdListService.batchImportBaseBaseProdList(baseProdListList);
			for(MESBasicBaseProdList baseProdList : baseProdListList){
				idList.add(baseProdList.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteBaseProdListByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicBaseProdList temp = null;
				try{
					temp = baseProdListService.getBaseProdList(id);
				}catch(NullPointerException e){}
			...
			baseProdListService.deleteBaseProdList(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteBaseProdListByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicBaseProdList temp = null;
				try{
					temp = baseProdListService.loadBaseProdListByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getBaseProdListPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicBaseProdList.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicBaseProdList.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicBaseProdList.class);
				} catch (Exception e) {
			...
			Page<MESBasicBaseProdList> page = new Page<MESBasicBaseProdList>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicBaseProdList> datas = baseProdListService.getByPage(page, detachedCriteria);
			List<MESBasicBaseProdList> baseProdListList = datas.getResult();
			if (baseProdListList == null || baseProdListList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(baseProdListList.toArray());
			PaginationDTO<MESBasicBaseProdList> pagination = new PaginationDTO<MESBasicBaseProdList>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicBaseProdList baseProdList = baseProdListService.getBaseProdList(id);
			if (baseProdList == null || !baseProdList.isValid()) {
				result.setErrorMessage("找不到物品目录");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取物品目录对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param baseProdList
	 * @return
	 */
	private MESBasicBaseProdList getBaseProdList(MESBasicBaseProdList baseProdList) {
		if (baseProdList.getId() == null && (baseProdList.getProdListCode() == null || baseProdList.getProdListCode().toString().trim() == ""))
			throw new RuntimeException("物品目录对象业务主键和id不能同时为空");
		if (baseProdList.getId() != null) {
			MESBasicBaseProdList result = baseProdListService.getBaseProdList(baseProdList .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return baseProdListService.loadBaseProdListByBussinessKey(baseProdList.getProdListCode());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个物品目录，则返回false，否则返回true
	 * @param baseProdList
	 */
	private boolean beforeUpdateBaseProdList(MESBasicBaseProdList baseProdList) {
		MESBasicBaseProdList temp = getBaseProdList(baseProdList);
		if (temp == null) {
			return false;
		}
		if(baseProdList.getId() == null){
			baseProdList.setId(temp.getId());		
		}
		if(baseProdList.getProdListCode() == null){
			baseProdList.setProdListCode(temp.getProdListCode());
		}
		
		baseProdList.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除物品目录对象下所有辅模型对象中的物品目录对象
	 * @param baseProdList
	 */
	private void clearCircle(MESBasicBaseProdList baseProdList) {
	 ...
	 */
	private void checkBaseProdListData(MESBasicBaseProdList baseProdList){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_prodList_BaseProdList,MESBasic_1) */
// 自定义代码
