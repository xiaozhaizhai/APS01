...
/* CUSTOM CODE START(action,import,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicFlowRecordAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicFlowRecord> {
	...
	private MESBasicFlowRecord flowRecord;
	...
	//导出excel时,主模型内容
	List<MESBasicFlowRecord> queryResults = new ArrayList<MESBasicFlowRecord>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/flowRecord/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/flowRecord/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_container_FlowRecord" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicFlowRecord> flowRecords;
	
	public List<MESBasicFlowRecord> getFlowRecords() {
		return flowRecords;
	}
	...
	private Page<MESBasicFlowRecord> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			flowRecord = new MESBasicFlowRecord();
		} else {
			if(id > 0){
				flowRecord = flowRecordService.getFlowRecord(id, viewCode);
			}
		}
	...
	public Page<MESBasicFlowRecord> getPage() {
		if (null == page) {
			page = new Page<MESBasicFlowRecord>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicFlowRecord getFlowRecord() {
		return flowRecord;
	}		

	public void setFlowRecord(MESBasicFlowRecord flowRecord) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/flowRecord/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = flowRecordService.getFlowRecordAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/flowRecord/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1555293134923
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String flowRecordReferenceCopy() throws Exception{
			MESBasicFlowRecord oldFlowRecord = null;
			if(isEditNew()){
				...
				oldFlowRecord = new MESBasicFlowRecord();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,flowRecord.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, flowRecord.getId());
				...
				oldFlowRecord = flowRecordService.getFlowRecord(id);
			}
			
			
			flowRecord = flowRecordService.getFlowRecord(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_container_FlowRecord");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicFlowRecord.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFlowRecord, null);
						Method setValue = MESBasicFlowRecord.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(flowRecord, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			flowRecord.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_container_FlowRecord", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=flowRecordService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return flowRecordReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			flowRecord = flowRecordService.getFlowRecord(this.flowRecord.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,flowRecord.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = flowRecordService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(flowRecordService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(flowRecordService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_container_FlowRecord", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"flowRecord\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"flowRecord\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"flowRecord\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"flowRecord\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		flowRecordService.findDataGridPage(dg,dgclass,getDgPage(),flowRecord,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(flowRecord.getId() != null && flowRecord.getId() != -1){
				flowRecord = flowRecordService.getFlowRecord(flowRecord.getId());
			}
			flowRecordService.findDataGridPage(dg,dgclass,getDgPage(),flowRecord,searchCondition,params);
	...
	private void save() throws Exception {
		...
		flowRecordService.saveFlowRecord(flowRecord,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_FlowRecord", "submit");
		bap_validate_datagrids=flowRecordService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", flowRecord.getId());
		...
		fileuploads.put("linkId", flowRecord.getId());
		bapAfterCustomMethod("MESBasic_1_container_FlowRecord", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/container/flowRecord/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_FlowRecord", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		flowRecordService.deleteFlowRecord(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_container_FlowRecord", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/flowRecord/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_FlowRecord", "restore");
		flowRecordService.restoreFlowRecord(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_container_FlowRecord", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicFlowRecord> findExportDataInfos(Page<MESBasicFlowRecord> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = flowRecordService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = flowRecordService.getAssProperty(p.getCode());	
									//String assModelCode = flowRecordService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			flowRecordService.findFlowRecords(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			flowRecordService.findFlowRecords(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicFlowRecord> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicFlowRecord result : results){
	...
	public boolean isEditNew(){
		//return null == this.flowRecord || null == this.flowRecord.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/container/flowRecord/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_FlowRecord", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_container_FlowRecord", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/container/flowRecord/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_FlowRecord", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/container/flowRecord/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/flowRecord/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicFlowRecordService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_container_FlowRecord", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_container_FlowRecord", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/flowRecord/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_container_FlowRecord", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_container_FlowRecord", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/container/flowRecord/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_FlowRecord", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/container/flowRecord/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_container_FlowRecord";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_container_FlowRecord", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/container/flowRecord/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicFlowRecord flowRecord = null;
		if(null != id) {
			flowRecord = flowRecordService.getFlowRecord(id);
		}
		...
		_auditLog.setColumnStringB(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
		_auditLog.setColumnStringA(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
		_auditLog.setColumnStringC(flowRecord.getId().toString());
		_auditLog.setColumnStringD(MESBasicFlowRecord.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicFlowRecordAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, flowRecord,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, flowRecord,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,flowRecord);
		...
		printlog.setBusinessId(flowRecord.getId());
		printlog.setBussinessKey(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
		printlog.setMainDisplay(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/container/flowRecord/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(flowRecord,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/flowRecord/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(flowRecordService.getFlowRecord(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/container/flowRecord/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/container/flowRecord/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, flowRecord,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				flowRecord = flowRecordService.getFlowRecord(Long.parseLong(arrId[i]));
				if(null != flowRecord){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == flowRecord.getId() ? "" : "," + flowRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == flowRecord.getId() ? "" : "," + flowRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == flowRecord.getId() ? "" : "," + flowRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, flowRecord,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,flowRecord);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/container/flowRecord/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_container_FlowRecord");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_container_FlowRecord")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_container_FlowRecord");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_container_FlowRecord");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = flowRecordService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
