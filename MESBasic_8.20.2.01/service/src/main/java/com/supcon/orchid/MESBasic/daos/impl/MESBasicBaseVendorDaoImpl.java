...
@Repository
public class MESBasicBaseVendorDaoImpl extends ExtendedGenericDaoImpl<MESBasicBaseVendor, Long> implements MESBasicBaseVendorDao {
	@Override
	...
	@Override
	public MESBasicBaseVendorDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicBaseVendorDealInfo dealInfo = new MESBasicBaseVendorDealInfo();
	...
	@Override
	public MESBasicBaseVendorDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicBaseVendor mainObj, Staff dealStaff) {
		MESBasicBaseVendorDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicBaseVendorDealInfo.class);
		...
		List<MESBasicBaseVendorDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_baseVendor_BaseVendor,MESBasic_1) */
// 自定义代码
