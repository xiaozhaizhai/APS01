...
@Repository
public class MESBasicStoreSetDaoImpl extends TreeDaoImpl<MESBasicStoreSet, Long> implements MESBasicStoreSetDao {
	@Override
	...
	@Override
	public MESBasicStoreSetDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicStoreSetDealInfo dealInfo = new MESBasicStoreSetDealInfo();
	...
	@Override
	public MESBasicStoreSetDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicStoreSet mainObj, Staff dealStaff) {
		MESBasicStoreSetDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicStoreSetDealInfo.class);
		...
		List<MESBasicStoreSetDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
