...
import com.supcon.orchid.MESBasic.services.MESBasicPotDataImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicPotDataDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
@Service("mESBasic_potDataService")
@Transactional
public class MESBasicPotDataServiceImpl  extends BaseServiceImpl<MESBasicPotData> implements MESBasicPotDataService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicPotData> bapBaseService;
	...
	@Autowired
	private MESBasicPotDataImportService potDataImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicPotData.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  potDataDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicPotData.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = potDataDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPotData.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPotData.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  potDataDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicPotData.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicPotData.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicPotData> list = (List<MESBasicPotData>) potDataDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicPotData getPotData(long id){
		return getPotData(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPotData getPotData(long id, String viewCode){
		MESBasicPotData potData = potDataDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPotDataAsJSON(long id, String include){
		MESBasicPotData potData = potDataDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(potData, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.common.delete",operType="3")
	public void deletePotData(MESBasicPotData potData){
		if(null != potData && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPotData.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == potData.getTempID() ? "" : potData.getTempID().toString());
			AuditUtil.setColumnStringA(null == potData.getId() ? "" : potData.getId().toString());
			AuditUtil.setColumnStringC(potData.getId().toString());
		}
		beforeDeletePotData(potData);
		potDataDao.delete(potData);
		afterDeletePotData(potData);
		potDataDao.flush();
		List<MESBasicPotData> params = new ArrayList<MESBasicPotData>();
		params.add(potData);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_potDataHandler");
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.common.delete",operType="3")
	public void deletePotData(List<Long> potDataIds){
		deletePotData(potDataIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.common.delete",operType="3")
	public void deletePotData(List<Long> potDataIds, String eventTopic) {
		List<MESBasicPotData> potDatas = new ArrayList<MESBasicPotData>();
		for(Long potDataId : potDataIds){
			MESBasicPotData potData = getPotData(potDataId);
			potDatas.add(potData);
			if(null != potData && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPotData.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == potData.getId() ? "" : "," + potData.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == potData.getId() ? "" : potData.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == potData.getTempID() ? "" : "," + potData.getTempID().toString()));
				} else {
					AuditUtil.setColumnStringB(null == potData.getTempID() ? "" : potData.getTempID().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == potData.getId() ? "" : "," + potData.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == potData.getId() ? "" : potData.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(potDatas != null && potDatas.size() > 0 ){
			for(MESBasicPotData potData : potDatas){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),potData, potData.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(potDataIds != null && potDataIds.size() > 0) {
			String hql = "update " + MESBasicPotData.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = potDataDao.createQuery(hql);
			query.setParameterList("ids", potDataIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicPotData potDataz : potDatas){
				potDataz.setValid(false);
				List<MESBasicPotData> params = new ArrayList<MESBasicPotData>();
				params.add(potDataz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.common.delete",operType="3")
	public void deletePotData(long potDataId, int potDataVersion){
		this.deletePotData(potDataId, potDataVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.common.delete",operType="3")
	public void deletePotData(long potDataId, int potDataVersion,SignatureLog signatureLog){
		MESBasicPotData potData = getPotData(potDataId);
		if(potData != null && !potData.isValid()){
		...
		beforeDeletePotData(potData);
		potDataDao.delete(potDataId, potDataVersion);
		afterDeletePotData(potData);
		potDataDao.flush();
		List<MESBasicPotData> params = new ArrayList<MESBasicPotData>();
		params.add(potData);
		...
		if(signatureLog != null ) {
			...
			businessKey=potData.getId();
			businessKey=potData.getTempID();
			signatureLog.setTableId(potData.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.common.delete",operType="3")
	public void deletePotData(String potDataIds){
		this.deletePotData(potDataIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.common.delete",operType="3")
	public void deletePotData(String potDataIds,SignatureLog signatureLog){
		deleteCollection(potDataIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePotData(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_potDataHandler#modelCode:MESBasic_1_potDataHandler_PotData#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="ec.engine.restore",operType="10")
	public void restorePotData(String potDataIds){
		restoreCollection(potDataIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePotData(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="ec.engine.restore",operType="10")
	public void restorePotData(long potDataId){
		
		findBusinessKeyUsed(potDataId);	//判断业务主键是否重复
		...
		beforeRestorePotData(potData);
		potData.setValid(true);
		potDataDao.update(potData);
		afterRestorePotData(potData);
		List<MESBasicPotData> params = new ArrayList<MESBasicPotData>();
		params.add(potData);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long potDataId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_potDataHandler_PotData");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicPotData.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicPotData.TABLE_NAME +" where id = ? )";
			List<Object> list =  potDataDao.createNativeQuery(sql,potDataId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePotData(List<MESBasicPotData>  potDatas){
		for(MESBasicPotData potData:potDatas)  {
			savePotData(potData, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePotData(List<MESBasicPotData>  potDatas){
		for(MESBasicPotData potData:potDatas)  {
			savePotData(potData, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="ec.flow.save")
	public void savePotData(MESBasicPotData potData, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePotData(potData, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="ec.flow.save")
	public void savePotData(MESBasicPotData potData, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePotData(potData, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePotDataCodes(MESBasicPotData potData) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePotDataCodes(potData, false);
	}
	
	public void generatePotDataCodes(MESBasicPotData potData, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePotDataSummarys(MESBasicPotData potData) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePotDataSummarys(potData, false);
	}

	public void generatePotDataSummarys(MESBasicPotData potData, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData")
	public void savePotData(MESBasicPotData potData,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePotData(potData, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData")
	public void savePotData(MESBasicPotData potData,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_potDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_potDataHandler");
		String url = null;
		if(potData.getId() != null && potData.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPotData.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_potDataHandler/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPotData.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_potDataHandler/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, potData);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(potData);
		beforeSavePotData(potData, viewIsView);

		if (viewIsView) {
			potDataDao.saveWithRevertVersion(potData);
		} else {
			if(isNew)
				potDataDao.save(potData);
			else
				potDataDao.merge(potData);
		}
		if(null != potData && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPotData.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == potData.getTempID() ? "" : potData.getTempID().toString());
			AuditUtil.setColumnStringA(null == potData.getId() ? "" : potData.getId().toString());
		...
		dealDatagridsSave(potData,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePotDataCodes(potData, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePotDataSummarys(potData, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePotData(potData, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, potData);
			}
		}
		List<MESBasicPotData> params = new ArrayList<MESBasicPotData>();
		potDataDao.flush();
		potDataDao.clear();
		potData = potDataDao.load(potData.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicPotData potData){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData")
	public void mergePotData(MESBasicPotData potData, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(potData);
		beforeSavePotData(potData);
		potDataDao.merge(potData);
		if(null != potData && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPotData.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == potData.getTempID() ? "" : potData.getTempID().toString());
			AuditUtil.setColumnStringA(null == potData.getId() ? "" : potData.getId().toString());
		...
		dealDatagridsSave(potData,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePotData(potData);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicPotData> findPotDatas(Page<MESBasicPotData> page, Criterion... criterions) {
		return potDataDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicPotDataDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicPotDataDealInfo> list=potDataDao.createQuery("from " + MESBasicPotDataDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicPotDataDealInfo dealInfo) {
		potDataDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicPotDataDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) potDataDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicPotDataDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = potDataDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = potDataDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicPotDataDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = potDataDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPotDatas(Page<MESBasicPotData> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPotDatas(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPotDatas(Page<MESBasicPotData> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPotDatas(Page<MESBasicPotData> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicPotData> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"potData\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"potData\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"potData\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_potDataHandler_PotData" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_potDataHandler_PotData" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  potDataDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  potDataDao.createNativeQuery(powerOperateSql, "MESBasic_1_potDataHandler").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"potData\"","MESBasic_1_potDataHandler",customCondition,"MESBASIC_POT_DATAS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"potData\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicPotDataDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"potData\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"potData\".LAY_REC ASC, \"potData\".SORT ASC");
				} else {
					orderPart.append(" \"potData\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) potDataDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=potDataDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = potDataDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicPotData> result = new ArrayList<MESBasicPotData>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicPotData.class,potDataDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicPotData.class,potDataDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicPotData potData : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(potData.getId(), "MESBasic_potDataHandler_potData");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(potData.getId(), "MESBasic_potDataHandler_potData");
							if(null != documents && !documents.isEmpty()) {
								potData.setDocument(documents.get(0));
								potData.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_potDataHandler")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_potDataHandler")+"MESBasic_1_potDataHandler".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_potDataHandler",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicPotData> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicPotData> result) {
		View view = null;
		...
		Map<Long, MESBasicPotData> rsMap = new HashMap<Long, MESBasicPotData>();
		for (MESBasicPotData potData : result) {
			ids.add(potData.getId());
			rsMap.put(potData.getId(), potData);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(potDataDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = potDataDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicPotData potData,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(potData.getOwnerStaffId());
		ti.setOwnerPositionId(potData.getOwnerPositionId());
		ti.setOwnerDepartmentId(potData.getOwnerDepartmentId());
		
		ti.setCreateStaff(potData.getCreateStaff());
		ti.setCreateStaffId(potData.getCreateStaffId());
		ti.setCreateTime(potData.getCreateTime());
		ti.setDeleteStaff(potData.getDeleteStaff());
		ti.setDeleteStaffId(potData.getDeleteStaffId());
		ti.setDeleteTime(potData.getDeleteTime());
		ti.setCreateDepartmentId(potData.getCreateDepartmentId());
		ti.setModifyStaff(potData.getModifyStaff());
		ti.setModifyStaffId(potData.getModifyStaffId());
		ti.setCreatePositionId(potData.getCreatePositionId());
		ti.setPositionLayRec(potData.getPositionLayRec());
		ti.setTableNo(potData.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_potDataHandler");
		ti.setTargetTableName("MESBASIC_POT_DATAS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_potDataHandler_PotData" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  potDataDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"potData\"","MESBasic_1_potDataHandler","","MESBASIC_POT_DATAS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"potData\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"potData\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"potData\"","MESBasic_1_potDataHandler","","MESBASIC_POT_DATAS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("potData.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"potData\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("potData.mc\"").append(".POT_DATA) not in ("+notinValue+")");
			}
			
			SQLQuery query = potDataDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("potDataHandler", "MESBasic_1_potDataHandler", "potDataHandler_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=potDataDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicPotData.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicPotData.MODULE_CODE;}
	protected String getModelCode(){return MESBasicPotData.MODEL_CODE;}
	 ...
	 * @param potData 模型实例
	 */
	public void checkUniqueConstraint(MESBasicPotData potData){
		if(null == potData.getTempID() || StringUtils.isEmpty(potData.getTempID().toString())){
			return;
		}
		if(potData.getId()==null ){
			List<MESBasicPotData> potDatas = potDataDao.findByCriteria(Restrictions.eq("tempID", potData.getTempID()));
			if(potDatas!=null && !potDatas.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1552359355955", getCurrentUser().getLanguage()), potData.getTempID());
			}
		} else {
			List<MESBasicPotData> potDatas = potDataDao.findByCriteria(Restrictions.ne("id", potData.getId()), Restrictions.eq("tempID", potData.getTempID()));
			if(potDatas!=null && !potDatas.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1552359355955", getCurrentUser().getLanguage()), potData.getTempID());
			}
	...
	@Override
	public List<MESBasicPotData> getPotDatas(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPotData.class);
		if(params!=null){
			...
			return potDataDao.findByHql("from " + MESBasicPotData.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return potDataDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicPotData> findPotDatasByHql(String hql, Object... objects){
		...
		return   potDataDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicPotData loadPotDataByBussinessKey(MESBasicPotData potData){
		return loadPotDataByBussinessKey(potData.getId());
	}
	
	@Override
	public Page<MESBasicPotData> getPotDatas(Page<MESBasicPotData> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPotData.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = potDataDao.findByPage(page, sql);
			}else{
				page = potDataDao.findAllByPage(page);
			}
			return page;
		}
		page = potDataDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public MESBasicPotData loadPotDataByBussinessKey(Serializable bussinessKey){
		return potDataDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicPotData> findByProperty(String propertyName, Object object){
		return potDataDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPotData findEntityByProperty(String propertyName, Object object){
		return potDataDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicPotData.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = potDataDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_potDataHandler_PotData","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_potDataHandler", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicPotData potData,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,potDataDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicPotData.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, potData);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",potData.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPotData.TABLE_NAME +" where valid = 1";
			list = potDataDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPotData.TABLE_NAME;
			list = potDataDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicPotData.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = potDataDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicPotDataDaoImpl potDataDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPotData(final List<MESBasicPotData> insertObjs, final List<MESBasicPotData> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPotData(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPotData(final List<MESBasicPotData> insertObjs, final List<MESBasicPotData> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1455958797173"),importCount));
		...
		Map<Long, MESBasicPotData> oldUpdateObjsMap = new HashMap<Long, MESBasicPotData>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicPotData bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicPotData> bizObjList = potDataDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicPotData bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicPotData> nmeObjs = new ArrayList<MESBasicPotData>();
		...
		long id=autoGeneratorID.getNextId(MESBasicPotData.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicPotData potData:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicPotData.TABLE_NAME,1,dbId);
			if(potData.getId() != null ){
				m.put(potData.getId(), id);
			}else{
				m.put(id, id);
			}
			if(potData.getTempID() != null && !potData.getTempID().equals("") ){
				m.put(potData.getTempID(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) potDataDao.createNativeQuery(sql, "MESBasic_1_potDataHandler_PotData"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		potDataDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicPotData.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicPotData potData = updateObjs.get(updateIndex);
						Long id = potData.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = potData.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicPotData.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicPotData.TABLE_NAME + " order by id desc";
//					Query query = potDataDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicPotData potData = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = potData.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		potDataDaoImpl.getSession().flush();
		potDataDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicPotData bizObj = insertObjs.get(insertIndex);
			potDataImportService.saveImportDataLog(bizObj, null, "tempID", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicPotData bizObj = updateObjs.get(updateIndex);
			MESBasicPotData oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			potDataImportService.saveImportDataLog(bizObj, oldBizObj, "tempID", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = potDataDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				potDataDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = potDataDao.createNativeQuery(sql, "MESBasic_1_potDataHandler_PotData").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = potDataDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = potDataDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = potDataDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?potDataDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  potDataDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,potDataDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="ec.flow.save")
	public void savePotData(MESBasicPotData potData, DataGridService dataGridService){
		savePotData(potData, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="ec.flow.save")
	public void savePotData(MESBasicPotData potData, DataGridService dataGridService, String viewCode){
		savePotData(potData, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData")
	public void savePotData(MESBasicPotData potData, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePotData(potData, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData")
	public void savePotData(MESBasicPotData potData, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_potDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_potDataHandler");
		String url = null;
		if(null != potData.getId() && potData.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPotData.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData")
	public Map<Object, Object> potDataDataGridImport(MESBasicPotData potData, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = potData.getVirtualId();
		
		potData = potDataDao.load(potData.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, potData.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = potData.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(potData);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, potData.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData")
	public void mergePotData(MESBasicPotData potData, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(potData);
		beforeSavePotData(potData);
		potDataDao.merge(potData);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicPotData> getByPage(Page<MESBasicPotData> page,DetachedCriteria detachedCriteria){
		return potDataDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_potDataHandler").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSavePotData(MESBasicPotData potData, Object... objects){
		checkUniqueConstraint(potData);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePotData(MESBasicPotData potData, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePotData(MESBasicPotData potData){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePotData(MESBasicPotData potData){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePotData(MESBasicPotData potData){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePotData(MESBasicPotData potData){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicPotData potData, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicPotData potData, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportPotData(List<MESBasicPotData> insertObjs, List<MESBasicPotData> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPotData,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportPotData(List<MESBasicPotData> insertObjs, List<MESBasicPotData> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPotData,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
public List<Object[]> getDataTransLimited(String sql, List<Object> params,
			int size) {
		SQLQuery query = potDataDao.createNativeQuery(sql, params.toArray());
	...
	public void update(MESBasicPotData potDatas) {
		potDataDao.update(potDatas);
	}

	@Override
	public void merge(MESBasicPotData potDatas) {
		potDataDao.merge(potDatas);
	...
	public void executeUpdate(String sql, Object... args) {
		SQLQuery query = potDataDao.createNativeQuery(sql, args);
		query.executeUpdate();
	}

	@Override
	public Query createQuery(String hql, Object... params) {
		return potDataDao.createQuery(hql, params);
	}

	@Override
	public Page<MESBasicPotData> findDataTransPage(Page<MESBasicPotData> page,
			String sql, Object... params) {
		return potDataDao.findByPage(page, sql,params);
	}

	@Override
	public Date getMinUnprocessedTime() {
		String sql = "select min(data_time) from MESBASIC_POT_DATAS where (processed is null or processed = 0) and valid = 1 and business_type='Business_Type/0'";
		Object minObj = potDataDao.createNativeQuery(sql).uniqueResult();
		if(minObj == null) {
			return null;
		}
		return (Date)minObj;
    ...
    @Override
    public String getMinUnprocessedTimeString(){
        // 改为获取记录的最早创建时间(dataTimeStr) - modified by ww 2019-04-19
        String sql = "select min(data_time_str) from MESBASIC_POT_DATAS where (processed is null or processed = 0) and valid = 1 and business_type='Business_Type/0'";
        Object minObj = potDataDao.createNativeQuery(sql).uniqueResult();
        if(minObj == null) {
            return null;
        }
        return minObj.toString();
    }

    @Override
    public BigDecimal getTagValueByUpdateTime(String tag, Date currentTime, BigDecimal interval, String factor) {
        if (currentTime == null){
            log.info("[getTagValueByUpdateTime] currentTime is null...");
            return null;
        }
        if (interval == null){
            log.info("[getTagValueByUpdateTime] interval is null...");
            return null;
        }

        // 根据tag和data_Time_Str字段模糊查询, 取最新的一条数据
        String sql = "SELECT TVALUE FROM MESBASIC_POT_DATAS WHERE BUSINESS_TYPE = 'Business_Type/0' AND TAG = ? ";
        SimpleDateFormat df;

        Calendar beforeTime = Calendar.getInstance();
        beforeTime.setTime(currentTime);
        // n分钟之前的时间
        beforeTime.add(Calendar.MINUTE, -interval.intValue());
        // 只精确到分钟, 目的是为了模糊查询
        df = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        String before = df.format(beforeTime.getTime());

        // 动态判断当前数据库类型(oracle or sqlServer)
        String dbName = DbUtils.getDbName();
        if (dbName == null) {
            return null;
        }
        // 解决data_Time_Str与data_time不一致时, 导致查询不到数据的问题
        if ("oracle".equals(dbName)){
            sql += " AND TO_CHAR(DATA_TIME,'yyyy-mm-dd hh24:mi') LIKE ? ORDER BY DATA_TIME DESC";
        } else if ("sqlserver".equals(dbName)){
            sql += " AND CONVERT(VARCHAR,DATA_TIME,120) LIKE ? ORDER BY DATA_TIME DESC";
        }

        // 只从MESBASIC_POT_DATAS表中查询10条记录
        List<Object> result = potDataDao.createNativeQuery(sql, tag, "%" + before + "%").setMaxResults(10).list();
        if (result.size() > 0){
            BigDecimal value = (BigDecimal) result.get(0);
            // 如果测点配置了修正公式, 则需要进一步计算 - added by ww 2019-05-31
            if (StringUtil.isNotEmptyOrNull(factor)) {
