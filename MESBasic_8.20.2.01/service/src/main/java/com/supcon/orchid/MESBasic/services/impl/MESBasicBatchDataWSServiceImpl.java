...
import com.supcon.orchid.MESBasic.services.MESBasicBatchDataWSService;
import com.supcon.orchid.MESBasic.services.MESBasicBatchDataService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicBatchData",targetNamespace = "http://ws.supcon.com")
public class MESBasicBatchDataWSServiceImpl implements MESBasicBatchDataWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getBatchDataByBusinessKey(
		...
		try{
			...
			MESBasicBatchData batchData = batchDataService.loadBatchDataByBussinessKey(businessKey);
			if(batchData == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, batchData);
		}catch(Throwable t){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getBatchDataByPk(
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO updateBatchData(
		@WebParam(name="batchData")MESBasicBatchData batchData,
		...
		try{
			...
			if(!beforeUpdateBatchData(batchData)){
				return set404Error(result);
			}
			batchDataService.saveBatchData(batchData, null);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO addBatchData(
		@WebParam(name="batchData")MESBasicBatchData batchData,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				checkBatchDataData(batchData);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			batchDataService.saveBatchData(batchData, null);
			setSuccessResult(result, batchData.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteBatchDataByBusinessKey(
		@WebParam(name="businessKey")String businessKey,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				batchData = batchDataService.loadBatchDataByBussinessKey(businessKey);
	...
	@Override//
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteBatchDataByPk(
		@WebParam(name="pk")String pk,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddBatchData(
		@WebParam(name="batchDataList")List<MESBasicBatchData> batchDataList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			int count = 1;
			for(MESBasicBatchData batchData : batchDataList){
				if(batchData.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(batchData.getId() == null || batchData.getId().toString().trim().isEmpty()){
			...
			batchDataService.batchImportBaseBatchData(batchDataList);
			for(MESBasicBatchData batchData : batchDataList){
				idList.add(batchData.getId());
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdateBatchData(
		@WebParam(name="batchDataList")List<MESBasicBatchData> batchDataList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteBatchDataByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicBatchData temp = null;
				try{
					temp = batchDataService.getBatchData(id);
				}catch(NullPointerException e){}
			...
			batchDataService.deleteBatchData(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteBatchDataByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicBatchData temp = null;
				try{
					temp = batchDataService.loadBatchDataByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getBatchDataPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicBatchData.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicBatchData.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicBatchData.class);
				} catch (Exception e) {
			...
			Page<MESBasicBatchData> page = new Page<MESBasicBatchData>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicBatchData> datas = batchDataService.getByPage(page, detachedCriteria);
			List<MESBasicBatchData> batchDataList = datas.getResult();
			if (batchDataList == null || batchDataList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(batchDataList.toArray());
			PaginationDTO<MESBasicBatchData> pagination = new PaginationDTO<MESBasicBatchData>(page);
	 ...
	 * @return
	 */
	private MESBasicBatchData getBatchData(MESBasicBatchData batchData) {
		if (batchData.getId() == null && (batchData.getId() == null || batchData.getId().toString().trim() == ""))
			throw new RuntimeException("数据接口对象业务主键和id不能同时为空");
		if (batchData.getId() != null) {
			MESBasicBatchData result = batchDataService.getBatchData(batchData .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return batchDataService.loadBatchDataByBussinessKey(batchData.getId());
		}
	}
	 ...
	 * @param batchData
	 */
	private boolean beforeUpdateBatchData(MESBasicBatchData batchData) {
		MESBasicBatchData temp = getBatchData(batchData);
		if (temp == null) {
			return false;
		}
		if(batchData.getId() == null){
			batchData.setId(temp.getId());		
		}
		if(batchData.getId() == null){
			batchData.setId(temp.getId());
		}
		
		batchData.setVersion(temp.getVersion());
		return true;
	}
	 ...
	 * @param batchData
	 */
	private void clearCircle(MESBasicBatchData batchData) {
	 ...
	 */
	private void checkBatchDataData(MESBasicBatchData batchData){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
