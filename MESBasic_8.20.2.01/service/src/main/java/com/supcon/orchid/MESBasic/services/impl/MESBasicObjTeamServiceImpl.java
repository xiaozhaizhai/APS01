...
import com.supcon.orchid.MESBasic.services.MESBasicObjTeamImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicObjTeamDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
import com.supcon.orchid.X6Basic.services.X6BasicTeamInfoService;
import com.supcon.orchid.X6Basic.entities.X6BasicTeamInfo;
/* CUSTOM CODE END */

@Service("mESBasic_objTeamService")
@Transactional
public class MESBasicObjTeamServiceImpl  extends BaseServiceImpl<MESBasicObjTeam> implements MESBasicObjTeamService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicObjTeam> bapBaseService;
	...
	@Autowired
	private MESBasicObjTeamImportService objTeamImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicObjTeam.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  objTeamDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicObjTeam.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = objTeamDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjTeam.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjTeam.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  objTeamDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicObjTeam.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicObjTeam.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicObjTeam> list = (List<MESBasicObjTeam>) objTeamDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjTeam getObjTeam(long id){
		return getObjTeam(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjTeam getObjTeam(long id, String viewCode){
		MESBasicObjTeam objTeam = objTeamDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getObjTeamAsJSON(long id, String include){
		MESBasicObjTeam objTeam = objTeamDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(objTeam, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.common.delete",operType="3")
	public void deleteObjTeam(MESBasicObjTeam objTeam){
		if(null != objTeam && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjTeam.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objTeam.getId() ? "" : objTeam.getId().toString());
			AuditUtil.setColumnStringA(null == objTeam.getId() ? "" : objTeam.getId().toString());
			AuditUtil.setColumnStringC(objTeam.getId().toString());
		}
		beforeDeleteObjTeam(objTeam);
		objTeamDao.deletePhysical(objTeam);
		afterDeleteObjTeam(objTeam);
		objTeamDao.flush();
		List<MESBasicObjTeam> params = new ArrayList<MESBasicObjTeam>();
		params.add(objTeam);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.common.delete",operType="3")
	public void deleteObjTeam(List<Long> objTeamIds){
		deleteObjTeam(objTeamIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.common.delete",operType="3")
	public void deleteObjTeam(List<Long> objTeamIds, String eventTopic) {
		List<MESBasicObjTeam> objTeams = new ArrayList<MESBasicObjTeam>();
		for(Long objTeamId : objTeamIds){
			MESBasicObjTeam objTeam = getObjTeam(objTeamId);
			objTeams.add(objTeam);
			if(null != objTeam && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjTeam.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == objTeam.getId() ? "" : "," + objTeam.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == objTeam.getId() ? "" : objTeam.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == objTeam.getId() ? "" : "," + objTeam.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == objTeam.getId() ? "" : objTeam.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == objTeam.getId() ? "" : "," + objTeam.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == objTeam.getId() ? "" : objTeam.getId().toString());
				}
			}
		}
		
		for(MESBasicObjTeam objTeam : objTeams){
			beforeDeleteObjTeam(objTeam);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(objTeams != null && objTeams.size() > 0 ){
			for(MESBasicObjTeam objTeam : objTeams){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicObjTeam.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(objTeam));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),objTeam, objTeam.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(objTeamIds != null && objTeamIds.size() > 0) {
			String hql = "delete " + MESBasicObjTeam.JPA_NAME + " where id in(:ids)";
			Query query = objTeamDao.createQuery(hql);
			query.setParameterList("ids", objTeamIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicObjTeam objTeamz : objTeams){
				objTeamz.setValid(false);
				List<MESBasicObjTeam> params = new ArrayList<MESBasicObjTeam>();
				params.add(objTeamz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.common.delete",operType="3")
	public void deleteObjTeam(long objTeamId, int objTeamVersion){
		this.deleteObjTeam(objTeamId, objTeamVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.common.delete",operType="3")
	public void deleteObjTeam(long objTeamId, int objTeamVersion,SignatureLog signatureLog){
		MESBasicObjTeam objTeam = getObjTeam(objTeamId);
		...
		beforeDeleteObjTeam(objTeam);
		objTeamDao.delete(objTeamId, objTeamVersion);
		afterDeleteObjTeam(objTeam);
		objTeamDao.flush();
		List<MESBasicObjTeam> params = new ArrayList<MESBasicObjTeam>();
		params.add(objTeam);
		...
		if(signatureLog != null ) {
			...
			businessKey=objTeam.getId();
			signatureLog.setTableId(objTeam.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.common.delete",operType="3")
	public void deleteObjTeam(String objTeamIds){
		this.deleteObjTeam(objTeamIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.common.delete",operType="3")
	public void deleteObjTeam(String objTeamIds,SignatureLog signatureLog){
		deleteCollection(objTeamIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteObjTeam(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_factoryModel#modelCode:MESBasic_1_factoryModel_ObjTeam#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="ec.engine.restore",operType="10")
	public void restoreObjTeam(String objTeamIds){
		restoreCollection(objTeamIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreObjTeam(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="ec.engine.restore",operType="10")
	public void restoreObjTeam(long objTeamId){
		
		findBusinessKeyUsed(objTeamId);	//判断业务主键是否重复
		...
		beforeRestoreObjTeam(objTeam);
		objTeam.setValid(true);
		objTeamDao.update(objTeam);
		afterRestoreObjTeam(objTeam);
		List<MESBasicObjTeam> params = new ArrayList<MESBasicObjTeam>();
		params.add(objTeam);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long objTeamId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_factoryModel_ObjTeam");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicObjTeam.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicObjTeam.TABLE_NAME +" where id = ? )";
			List<Object> list =  objTeamDao.createNativeQuery(sql,objTeamId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseObjTeam(List<MESBasicObjTeam>  objTeams){
		for(MESBasicObjTeam objTeam:objTeams)  {
			saveObjTeam(objTeam, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseObjTeam(List<MESBasicObjTeam>  objTeams){
		for(MESBasicObjTeam objTeam:objTeams)  {
			saveObjTeam(objTeam, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="ec.flow.save")
	public void saveObjTeam(MESBasicObjTeam objTeam, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveObjTeam(objTeam, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="ec.flow.save")
	public void saveObjTeam(MESBasicObjTeam objTeam, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveObjTeam(objTeam, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateObjTeamCodes(MESBasicObjTeam objTeam) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjTeamCodes(objTeam, false);
	}
	
	public void generateObjTeamCodes(MESBasicObjTeam objTeam, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateObjTeamSummarys(MESBasicObjTeam objTeam) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjTeamSummarys(objTeam, false);
	}

	public void generateObjTeamSummarys(MESBasicObjTeam objTeam, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam")
	public void saveObjTeam(MESBasicObjTeam objTeam,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveObjTeam(objTeam, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam")
	public void saveObjTeam(MESBasicObjTeam objTeam,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(objTeam.getId() != null && objTeam.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjTeam.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, objTeam);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(objTeam);
		beforeSaveObjTeam(objTeam, viewIsView);

		if (viewIsView) {
			objTeamDao.saveWithRevertVersion(objTeam);
		} else {
			if(isNew)
				objTeamDao.save(objTeam);
			else
				objTeamDao.merge(objTeam);
		}
		if(null != objTeam && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjTeam.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objTeam.getId() ? "" : objTeam.getId().toString());
			AuditUtil.setColumnStringA(null == objTeam.getId() ? "" : objTeam.getId().toString());
		...
		dealDatagridsSave(objTeam,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateObjTeamCodes(objTeam, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateObjTeamSummarys(objTeam, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveObjTeam(objTeam, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, objTeam);
			}
		}
		List<MESBasicObjTeam> params = new ArrayList<MESBasicObjTeam>();
		objTeamDao.flush();
		objTeamDao.clear();
		objTeam = objTeamDao.load(objTeam.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicObjTeam objTeam){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam")
	public void mergeObjTeam(MESBasicObjTeam objTeam, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(objTeam);
		beforeSaveObjTeam(objTeam);
		objTeamDao.merge(objTeam);
		if(null != objTeam && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjTeam.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objTeam.getId() ? "" : objTeam.getId().toString());
			AuditUtil.setColumnStringA(null == objTeam.getId() ? "" : objTeam.getId().toString());
		...
		dealDatagridsSave(objTeam,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveObjTeam(objTeam);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicObjTeam> findObjTeams(Page<MESBasicObjTeam> page, Criterion... criterions) {
		return objTeamDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjTeams(Page<MESBasicObjTeam> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findObjTeams(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjTeams(Page<MESBasicObjTeam> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjTeams(Page<MESBasicObjTeam> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicObjTeam> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"objTeam\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"objTeam\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"objTeam\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"objTeam\".OBJ_FACTORY IN ( SELECT ID FROM factory_tbcompanystructure \"factoryModel\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  objTeamDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  objTeamDao.createNativeQuery(powerOperateSql, "MESBasic_1_factoryModel").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"objTeam\".LAY_REC ASC, \"objTeam\".SORT ASC");
				} else {
					orderPart.append(" \"objTeam\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) objTeamDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=objTeamDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = objTeamDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicObjTeam> result = new ArrayList<MESBasicObjTeam>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjTeam.class,objTeamDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjTeam.class,objTeamDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicObjTeam objTeam : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(objTeam.getId(), "MESBasic_factoryModel_objTeam");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(objTeam.getId(), "MESBasic_factoryModel_objTeam");
							if(null != documents && !documents.isEmpty()) {
								objTeam.setDocument(documents.get(0));
								objTeam.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicObjTeam> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicObjTeam> result) {
		View view = null;
		...
		Map<Long, MESBasicObjTeam> rsMap = new HashMap<Long, MESBasicObjTeam>();
		for (MESBasicObjTeam objTeam : result) {
			ids.add(objTeam.getId());
			rsMap.put(objTeam.getId(), objTeam);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(objTeamDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = objTeamDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_factoryModel_ObjTeam" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_factoryModel_ObjTeam" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  objTeamDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objTeam\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_TEAMS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"objTeam\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"objTeam\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objTeam\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_TEAMS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_factoryModel_ObjTeam" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("objTeam.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"objTeam\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("objTeam.mc\"").append(".OBJ_TEAM) not in ("+notinValue+")");
			}
			
			SQLQuery query = objTeamDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=objTeamDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicObjTeam.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicObjTeam.MODULE_CODE;}
	protected String getModelCode(){return MESBasicObjTeam.MODEL_CODE;}
	 ...
	 * @param objTeam 模型实例
	 */
	public void checkUniqueConstraint(MESBasicObjTeam objTeam){
	}
	
	
	@Override
	public List<MESBasicObjTeam> getObjTeams(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjTeam.class);
		if(params!=null){
			...
			return objTeamDao.findByHql("from " + MESBasicObjTeam.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return objTeamDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicObjTeam> findObjTeamsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjTeam.class);
	...
	@Override
	public List<MESBasicObjTeam> findObjTeamsByHql(String hql, Object... objects){
		
		return   objTeamDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicObjTeam loadObjTeamByBussinessKey(MESBasicObjTeam objTeam){
		return loadObjTeamByBussinessKey(objTeam.getId());
	}
	
	@Override
	public Page<MESBasicObjTeam> getObjTeams(Page<MESBasicObjTeam> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjTeam.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = objTeamDao.findByPage(page, sql);
			}else{
				page = objTeamDao.findAllByPage(page);
			}
			return page;
		}
		page = objTeamDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjTeam> findByTreeDataGridFactoryModel(List<MESBasicObjTeam> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicObjTeam objTeam : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFactoryModel(Page<MESBasicObjTeam> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicObjTeam> objTeams = dgPage.getResult();
			for(MESBasicObjTeam objTeam : objTeams){
				
			}
			dgPage.setResult(objTeams);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjTeam loadObjTeamByBussinessKey(Serializable bussinessKey){
		return objTeamDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjTeam> findByProperty(String propertyName, Object object){
		return objTeamDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjTeam findEntityByProperty(String propertyName, Object object){
		return objTeamDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicObjTeam.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = objTeamDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_factoryModel_ObjTeam","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicObjTeam objTeam,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,objTeamDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicObjTeam.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, objTeam);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",objTeam.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjTeam.TABLE_NAME +" where valid = 1";
			list = objTeamDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjTeam.TABLE_NAME;
			list = objTeamDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicObjTeam.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = objTeamDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicObjTeamDaoImpl objTeamDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchObjTeam(final List<MESBasicObjTeam> insertObjs, final List<MESBasicObjTeam> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchObjTeam(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchObjTeam(final List<MESBasicObjTeam> insertObjs, final List<MESBasicObjTeam> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1490261243605"),importCount));
		...
		Map<Long, MESBasicObjTeam> oldUpdateObjsMap = new HashMap<Long, MESBasicObjTeam>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicObjTeam bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicObjTeam> bizObjList = objTeamDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicObjTeam bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicObjTeam> nmeObjs = new ArrayList<MESBasicObjTeam>();
		...
		long id=autoGeneratorID.getNextId(MESBasicObjTeam.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicObjTeam objTeam:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicObjTeam.TABLE_NAME,1,dbId);
			if(objTeam.getId() != null ){
				m.put(objTeam.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) objTeamDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjTeam"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		objTeamDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicObjTeam.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicObjTeam objTeam = updateObjs.get(updateIndex);
						Long id = objTeam.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = objTeam.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicObjTeam.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicObjTeam.TABLE_NAME + " order by id desc";
//					Query query = objTeamDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicObjTeam objTeam = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = objTeam.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		objTeamDaoImpl.getSession().flush();
		objTeamDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicObjTeam bizObj = insertObjs.get(insertIndex);
			objTeamImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicObjTeam bizObj = updateObjs.get(updateIndex);
			MESBasicObjTeam oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			objTeamImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicObjTeam> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicObjTeam> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) objTeamDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicObjTeam> result = getResult(page, query, new PendingResultTransformer(MESBasicObjTeam.class,objTeamDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicObjTeam model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = objTeamDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjTeam").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = objTeamDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = objTeamDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = objTeamDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?objTeamDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  objTeamDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,objTeamDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="ec.flow.save")
	public void saveObjTeam(MESBasicObjTeam objTeam, DataGridService dataGridService){
		saveObjTeam(objTeam, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="ec.flow.save")
	public void saveObjTeam(MESBasicObjTeam objTeam, DataGridService dataGridService, String viewCode){
		saveObjTeam(objTeam, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam")
	public void saveObjTeam(MESBasicObjTeam objTeam, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveObjTeam(objTeam, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam")
	public void saveObjTeam(MESBasicObjTeam objTeam, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(null != objTeam.getId() && objTeam.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjTeam.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam")
	public Map<Object, Object> objTeamDataGridImport(MESBasicObjTeam objTeam, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = objTeam.getVirtualId();
		
		objTeam = objTeamDao.load(objTeam.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, objTeam.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = objTeam.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(objTeam);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, objTeam.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam")
	public void mergeObjTeam(MESBasicObjTeam objTeam, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(objTeam);
		beforeSaveObjTeam(objTeam);
		objTeamDao.merge(objTeam);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjTeam",desc="foundation.batch.save",operType="4")
	public void batchSaveObjTeam(MESBasicObjTeam objTeam, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
	...
	@Override
	public Page<MESBasicObjTeam> getByPage(Page<MESBasicObjTeam> page,DetachedCriteria detachedCriteria){
		return objTeamDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveObjTeam(MESBasicObjTeam objTeam, Object... objects){
		checkUniqueConstraint(objTeam);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
 DetachedCriteria detachedCriteria=DetachedCriteria.forClass(MESBasicObjTeam.class);
			detachedCriteria.add(Restrictions.eq("valid",true));
			detachedCriteria.add(Restrictions.eq("objTeam",objTeam.getObjTeam()));
		    if(objTeam.getId()!=null){//修改
		    	detachedCriteria.add(Restrictions.ne("id",objTeam.getId()));
		    }
		    List<MESBasicObjTeam> basicObjTeams = objTeamDao.findByCriteria(detachedCriteria);
			
	      
				if (null !=basicObjTeams && !basicObjTeams.isEmpty()){
				String proNames="";
                String factoryName="";
				for (MESBasicObjTeam basicObjTeam : basicObjTeams){
					X6BasicTeamInfo tempTeam=teamInfoService.getTeamInfo(basicObjTeam.getObjTeam().getId());
                  if(basicObjTeam.getObjFactory().getId().equals(objTeam.getObjFactory().getId())){
						throw new BAPException("班组["+tempTeam.getName()+"]已存在于本模型中，不能重复添加！");
					}
					if(proNames.length()>0){
						proNames+="，"+tempTeam.getName();
                     	factoryName+=","+basicObjTeam.getObjFactory().getName();
					}else{
						proNames=tempTeam.getName();
                      	factoryName=basicObjTeam.getObjFactory().getName();
					}
				...
				if (proNames.length()>0){
					throw new BAPException("班组["+proNames+"]已分别存在于["+factoryName+"]模型中，不能重复添加！");
				}
			}
/* CUSTOM CODE END */
	}
	
	private void afterSaveObjTeam(MESBasicObjTeam objTeam, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportObjTeam(List<MESBasicObjTeam> objTeams, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportObjTeam(List<MESBasicObjTeam> objTeams){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportObjTeam(List<MESBasicObjTeam> objTeams, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportObjTeam(List<MESBasicObjTeam> objTeams){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteObjTeam(MESBasicObjTeam objTeam){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteObjTeam(MESBasicObjTeam objTeam){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreObjTeam(MESBasicObjTeam objTeam){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreObjTeam(MESBasicObjTeam objTeam){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicObjTeam objTeam, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicObjTeam objTeam, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportObjTeam(List<MESBasicObjTeam> insertObjs, List<MESBasicObjTeam> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportObjTeam,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportObjTeam(List<MESBasicObjTeam> insertObjs, List<MESBasicObjTeam> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportObjTeam,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
