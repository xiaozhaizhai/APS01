...
import com.supcon.orchid.MESBasic.services.MESBasicDataTransImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicDataTransDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
import java.text.SimpleDateFormat;
/* CUSTOM CODE END */

@Service("mESBasic_dataTransService")
@Transactional
public class MESBasicDataTransServiceImpl  extends BaseServiceImpl<MESBasicDataTrans> implements MESBasicDataTransService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicDataTrans> bapBaseService;
	...
	@Autowired
	private MESBasicDataTransImportService dataTransImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicDataTrans.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  dataTransDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicDataTrans.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = dataTransDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicDataTrans.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicDataTrans.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  dataTransDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicDataTrans.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicDataTrans.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicDataTrans> list = (List<MESBasicDataTrans>) dataTransDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicDataTrans getDataTrans(long id){
		return getDataTrans(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicDataTrans getDataTrans(long id, String viewCode){
		MESBasicDataTrans dataTrans = dataTransDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getDataTransAsJSON(long id, String include){
		MESBasicDataTrans dataTrans = dataTransDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(dataTrans, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.common.delete",operType="3")
	public void deleteDataTrans(MESBasicDataTrans dataTrans){
		if(null != dataTrans && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDataTrans.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
			AuditUtil.setColumnStringA(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
			AuditUtil.setColumnStringC(dataTrans.getId().toString());
		}
		beforeDeleteDataTrans(dataTrans);
		dataTransDao.delete(dataTrans);
		afterDeleteDataTrans(dataTrans);
		dataTransDao.flush();
		List<MESBasicDataTrans> params = new ArrayList<MESBasicDataTrans>();
		params.add(dataTrans);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_thirdDataHandler");
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.common.delete",operType="3")
	public void deleteDataTrans(List<Long> dataTransIds){
		deleteDataTrans(dataTransIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.common.delete",operType="3")
	public void deleteDataTrans(List<Long> dataTransIds, String eventTopic) {
		List<MESBasicDataTrans> dataTranss = new ArrayList<MESBasicDataTrans>();
		for(Long dataTransId : dataTransIds){
			MESBasicDataTrans dataTrans = getDataTrans(dataTransId);
			dataTranss.add(dataTrans);
			if(null != dataTrans && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDataTrans.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == dataTrans.getId() ? "" : "," + dataTrans.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == dataTrans.getId() ? "" : "," + dataTrans.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == dataTrans.getId() ? "" : "," + dataTrans.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(dataTranss != null && dataTranss.size() > 0 ){
			for(MESBasicDataTrans dataTrans : dataTranss){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),dataTrans, dataTrans.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(dataTransIds != null && dataTransIds.size() > 0) {
			String hql = "update " + MESBasicDataTrans.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = dataTransDao.createQuery(hql);
			query.setParameterList("ids", dataTransIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicDataTrans dataTransz : dataTranss){
				dataTransz.setValid(false);
				List<MESBasicDataTrans> params = new ArrayList<MESBasicDataTrans>();
				params.add(dataTransz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_thirdDataHandler");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.common.delete",operType="3")
	public void deleteDataTrans(long dataTransId, int dataTransVersion){
		this.deleteDataTrans(dataTransId, dataTransVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.common.delete",operType="3")
	public void deleteDataTrans(long dataTransId, int dataTransVersion,SignatureLog signatureLog){
		MESBasicDataTrans dataTrans = getDataTrans(dataTransId);
		...
		beforeDeleteDataTrans(dataTrans);
		dataTransDao.delete(dataTransId, dataTransVersion);
		afterDeleteDataTrans(dataTrans);
		dataTransDao.flush();
		List<MESBasicDataTrans> params = new ArrayList<MESBasicDataTrans>();
		params.add(dataTrans);
		...
		if(signatureLog != null ) {
			...
			businessKey=dataTrans.getId();
			signatureLog.setTableId(dataTrans.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.common.delete",operType="3")
	public void deleteDataTrans(String dataTransIds){
		this.deleteDataTrans(dataTransIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.common.delete",operType="3")
	public void deleteDataTrans(String dataTransIds,SignatureLog signatureLog){
		deleteCollection(dataTransIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteDataTrans(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_thirdDataHandler#modelCode:MESBasic_1_thirdDataHandler_DataTrans#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="ec.engine.restore",operType="10")
	public void restoreDataTrans(String dataTransIds){
		restoreCollection(dataTransIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreDataTrans(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="ec.engine.restore",operType="10")
	public void restoreDataTrans(long dataTransId){
		
		findBusinessKeyUsed(dataTransId);	//判断业务主键是否重复
		...
		beforeRestoreDataTrans(dataTrans);
		dataTrans.setValid(true);
		dataTransDao.update(dataTrans);
		afterRestoreDataTrans(dataTrans);
		List<MESBasicDataTrans> params = new ArrayList<MESBasicDataTrans>();
		params.add(dataTrans);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long dataTransId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_thirdDataHandler_DataTrans");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicDataTrans.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicDataTrans.TABLE_NAME +" where id = ? )";
			List<Object> list =  dataTransDao.createNativeQuery(sql,dataTransId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseDataTrans(List<MESBasicDataTrans>  dataTranss){
		for(MESBasicDataTrans dataTrans:dataTranss)  {
			saveDataTrans(dataTrans, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseDataTrans(List<MESBasicDataTrans>  dataTranss){
		for(MESBasicDataTrans dataTrans:dataTranss)  {
			saveDataTrans(dataTrans, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="ec.flow.save")
	public void saveDataTrans(MESBasicDataTrans dataTrans, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveDataTrans(dataTrans, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="ec.flow.save")
	public void saveDataTrans(MESBasicDataTrans dataTrans, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveDataTrans(dataTrans, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateDataTransCodes(MESBasicDataTrans dataTrans) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateDataTransCodes(dataTrans, false);
	}
	
	public void generateDataTransCodes(MESBasicDataTrans dataTrans, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateDataTransSummarys(MESBasicDataTrans dataTrans) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateDataTransSummarys(dataTrans, false);
	}

	public void generateDataTransSummarys(MESBasicDataTrans dataTrans, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans")
	public void saveDataTrans(MESBasicDataTrans dataTrans,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveDataTrans(dataTrans, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans")
	public void saveDataTrans(MESBasicDataTrans dataTrans,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_thirdDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_thirdDataHandler");
		String url = null;
		if(dataTrans.getId() != null && dataTrans.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDataTrans.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_thirdDataHandler/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDataTrans.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_thirdDataHandler/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, dataTrans);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(dataTrans);
		beforeSaveDataTrans(dataTrans, viewIsView);

		if (viewIsView) {
			dataTransDao.saveWithRevertVersion(dataTrans);
		} else {
			if(isNew)
				dataTransDao.save(dataTrans);
			else
				dataTransDao.merge(dataTrans);
		}
		if(null != dataTrans && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDataTrans.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
			AuditUtil.setColumnStringA(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
		...
		dealDatagridsSave(dataTrans,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateDataTransCodes(dataTrans, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateDataTransSummarys(dataTrans, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveDataTrans(dataTrans, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, dataTrans);
			}
		}
		List<MESBasicDataTrans> params = new ArrayList<MESBasicDataTrans>();
		dataTransDao.flush();
		dataTransDao.clear();
		dataTrans = dataTransDao.load(dataTrans.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicDataTrans dataTrans){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("dataTrans", dataTrans);
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans")
	public void mergeDataTrans(MESBasicDataTrans dataTrans, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(dataTrans);
		beforeSaveDataTrans(dataTrans);
		dataTransDao.merge(dataTrans);
		if(null != dataTrans && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDataTrans.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
			AuditUtil.setColumnStringA(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
		...
		dealDatagridsSave(dataTrans,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveDataTrans(dataTrans);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicDataTrans> findDataTranss(Page<MESBasicDataTrans> page, Criterion... criterions) {
		return dataTransDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicDataTransDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicDataTransDealInfo> list=dataTransDao.createQuery("from " + MESBasicDataTransDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicDataTransDealInfo dealInfo) {
		dataTransDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicDataTransDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) dataTransDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicDataTransDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = dataTransDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = dataTransDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicDataTransDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = dataTransDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDataTranss(Page<MESBasicDataTrans> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findDataTranss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDataTranss(Page<MESBasicDataTrans> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDataTranss(Page<MESBasicDataTrans> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicDataTrans> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"dataTrans\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"dataTrans\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"dataTrans\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_thirdDataHandler_DataTrans" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_thirdDataHandler_DataTrans" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  dataTransDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  dataTransDao.createNativeQuery(powerOperateSql, "MESBasic_1_thirdDataHandler").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"dataTrans\"","MESBasic_1_thirdDataHandler",customCondition,"MESBASIC_DATA_TRANS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"dataTrans\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicDataTransDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"dataTrans\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"dataTrans\".LAY_REC ASC, \"dataTrans\".SORT ASC");
				} else {
					orderPart.append(" \"dataTrans\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) dataTransDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=dataTransDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = dataTransDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicDataTrans> result = new ArrayList<MESBasicDataTrans>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicDataTrans.class,dataTransDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicDataTrans.class,dataTransDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicDataTrans dataTrans : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(dataTrans.getId(), "MESBasic_thirdDataHandler_dataTrans");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(dataTrans.getId(), "MESBasic_thirdDataHandler_dataTrans");
							if(null != documents && !documents.isEmpty()) {
								dataTrans.setDocument(documents.get(0));
								dataTrans.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_thirdDataHandler")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_thirdDataHandler")+"MESBasic_1_thirdDataHandler".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_thirdDataHandler",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicDataTrans> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicDataTrans> result) {
		View view = null;
		...
		Map<Long, MESBasicDataTrans> rsMap = new HashMap<Long, MESBasicDataTrans>();
		for (MESBasicDataTrans dataTrans : result) {
			ids.add(dataTrans.getId());
			rsMap.put(dataTrans.getId(), dataTrans);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(dataTransDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = dataTransDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicDataTrans dataTrans,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(dataTrans.getOwnerStaffId());
		ti.setOwnerPositionId(dataTrans.getOwnerPositionId());
		ti.setOwnerDepartmentId(dataTrans.getOwnerDepartmentId());
		
		ti.setCreateStaff(dataTrans.getCreateStaff());
		ti.setCreateStaffId(dataTrans.getCreateStaffId());
		ti.setCreateTime(dataTrans.getCreateTime());
		ti.setDeleteStaff(dataTrans.getDeleteStaff());
		ti.setDeleteStaffId(dataTrans.getDeleteStaffId());
		ti.setDeleteTime(dataTrans.getDeleteTime());
		ti.setCreateDepartmentId(dataTrans.getCreateDepartmentId());
		ti.setModifyStaff(dataTrans.getModifyStaff());
		ti.setModifyStaffId(dataTrans.getModifyStaffId());
		ti.setCreatePositionId(dataTrans.getCreatePositionId());
		ti.setPositionLayRec(dataTrans.getPositionLayRec());
		ti.setTableNo(dataTrans.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_thirdDataHandler");
		ti.setTargetTableName("MESBASIC_DATA_TRANS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_thirdDataHandler_DataTrans" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  dataTransDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"dataTrans\"","MESBasic_1_thirdDataHandler","","MESBASIC_DATA_TRANS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"dataTrans\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"dataTrans\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"dataTrans\"","MESBasic_1_thirdDataHandler","","MESBASIC_DATA_TRANS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("dataTrans.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"dataTrans\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"dataTrans\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"dataTrans\".CID = 1 OR \"dataTrans\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("dataTrans.mc\"").append(".DATA_TRANS) not in ("+notinValue+")");
			}
			
			SQLQuery query = dataTransDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("thirdDataHandler", "MESBasic_1_thirdDataHandler", "thirdDataHandler_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=dataTransDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicDataTrans.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicDataTrans.MODULE_CODE;}
	protected String getModelCode(){return MESBasicDataTrans.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param dataTrans 模型实例
	 */
	public void checkUniqueConstraint(MESBasicDataTrans dataTrans){
	}
	
	
	@Override
	public List<MESBasicDataTrans> getDataTranss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicDataTrans.class);
		if(params!=null){
			...
			return dataTransDao.findByHql("from " + MESBasicDataTrans.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return dataTransDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicDataTrans> findDataTranssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicDataTrans.class);
	...
	@Override
	public List<MESBasicDataTrans> findDataTranssByHql(String hql, Object... objects){
		
		return   dataTransDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicDataTrans loadDataTransByBussinessKey(MESBasicDataTrans dataTrans){
		return loadDataTransByBussinessKey(dataTrans.getId());
	}
	
	@Override
	public Page<MESBasicDataTrans> getDataTranss(Page<MESBasicDataTrans> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicDataTrans.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = dataTransDao.findByPage(page, sql);
			}else{
				page = dataTransDao.findAllByPage(page);
			}
			return page;
		}
		page = dataTransDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicDataTrans loadDataTransByBussinessKey(Serializable bussinessKey){
		return dataTransDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicDataTrans> findByProperty(String propertyName, Object object){
		return dataTransDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicDataTrans findEntityByProperty(String propertyName, Object object){
		return dataTransDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicDataTrans.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = dataTransDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_thirdDataHandler_DataTrans","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_thirdDataHandler", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicDataTrans dataTrans,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,dataTransDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicDataTrans.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, dataTrans);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",dataTrans.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicDataTrans.TABLE_NAME +" where valid = 1";
			list = dataTransDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicDataTrans.TABLE_NAME;
			list = dataTransDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicDataTrans.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = dataTransDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicDataTransDaoImpl dataTransDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchDataTrans(final List<MESBasicDataTrans> insertObjs, final List<MESBasicDataTrans> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchDataTrans(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchDataTrans(final List<MESBasicDataTrans> insertObjs, final List<MESBasicDataTrans> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1414977890489"),importCount));
		...
		Map<Long, MESBasicDataTrans> oldUpdateObjsMap = new HashMap<Long, MESBasicDataTrans>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicDataTrans bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicDataTrans> bizObjList = dataTransDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicDataTrans bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicDataTrans> nmeObjs = new ArrayList<MESBasicDataTrans>();
		...
		long id=autoGeneratorID.getNextId(MESBasicDataTrans.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicDataTrans dataTrans:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicDataTrans.TABLE_NAME,1,dbId);
			if(dataTrans.getId() != null ){
				m.put(dataTrans.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) dataTransDao.createNativeQuery(sql, "MESBasic_1_thirdDataHandler_DataTrans"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		dataTransDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicDataTrans.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicDataTrans dataTrans = updateObjs.get(updateIndex);
						Long id = dataTrans.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = dataTrans.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicDataTrans.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicDataTrans.TABLE_NAME + " order by id desc";
//					Query query = dataTransDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicDataTrans dataTrans = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = dataTrans.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		dataTransDaoImpl.getSession().flush();
		dataTransDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicDataTrans bizObj = insertObjs.get(insertIndex);
			dataTransImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicDataTrans bizObj = updateObjs.get(updateIndex);
			MESBasicDataTrans oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			dataTransImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = dataTransDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				dataTransDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = dataTransDao.createNativeQuery(sql, "MESBasic_1_thirdDataHandler_DataTrans").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = dataTransDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = dataTransDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = dataTransDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?dataTransDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  dataTransDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,dataTransDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="ec.flow.save")
	public void saveDataTrans(MESBasicDataTrans dataTrans, DataGridService dataGridService){
		saveDataTrans(dataTrans, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="ec.flow.save")
	public void saveDataTrans(MESBasicDataTrans dataTrans, DataGridService dataGridService, String viewCode){
		saveDataTrans(dataTrans, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans")
	public void saveDataTrans(MESBasicDataTrans dataTrans, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveDataTrans(dataTrans, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans")
	public void saveDataTrans(MESBasicDataTrans dataTrans, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_thirdDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_thirdDataHandler");
		String url = null;
		if(null != dataTrans.getId() && dataTrans.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDataTrans.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans")
	public Map<Object, Object> dataTransDataGridImport(MESBasicDataTrans dataTrans, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = dataTrans.getVirtualId();
		
		dataTrans = dataTransDao.load(dataTrans.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, dataTrans.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = dataTrans.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(dataTrans);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, dataTrans.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans")
	public void mergeDataTrans(MESBasicDataTrans dataTrans, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(dataTrans);
		beforeSaveDataTrans(dataTrans);
		dataTransDao.merge(dataTrans);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_DataTrans",desc="foundation.batch.save",operType="4")
	public void batchSaveDataTrans(MESBasicDataTrans dataTrans, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_thirdDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_thirdDataHandler");
	...
	@Override
	public Page<MESBasicDataTrans> getByPage(Page<MESBasicDataTrans> page,DetachedCriteria detachedCriteria){
		return dataTransDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_thirdDataHandler").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveDataTrans(MESBasicDataTrans dataTrans, Object... objects){
		checkUniqueConstraint(dataTrans);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveDataTrans(MESBasicDataTrans dataTrans, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
		//SimpleDateFormat sf = new SimpleDateFormat("YYYY-MM-dd");
		//String sqlId = "select min(id) from "+MESBasicDataTrans.TABLE_NAME+" where DATA_TIME >= '"+sf.format(dataTrans.getDataTime())+"'";
		//BigDecimal maxId = (BigDecimal)itemSeqDao.createNativeQuery("select max_id from "+MESBasicItemSeq.TABLE_NAME+" where seq_Type = 'batchManag_1.0'").uniqueResult();
		//BigDecimal id = (BigDecimal)dataTransDao.createNativeQuery(sqlId).uniqueResult();
		//if(maxId.compareTo(id) > 0){//maxId > id
		//	itemSeqService.updateMaxIdSeq("batchManag_1.0", maxId.add(BigDecimal.ONE).longValue());
		//}
		//ID + 1
		//dataTransDao.createNativeQuery("update "+MESBasicDataTrans.TABLE_NAME+" set id=id+1 where id >= "+id.toPlainString()).executeUpdate();
		//插入ID
		//dataTransDao.createNativeQuery("update "+MESBasicDataTrans.TABLE_NAME+" set id= "+id.toPlainString()+" where id - 1 ="+dataTrans.getId().toString()).executeUpdate();
/* CUSTOM CODE END */
	}
	
	@Override
	public void beforeExcelImportDataTrans(List<MESBasicDataTrans> dataTranss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportDataTrans(List<MESBasicDataTrans> dataTranss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportDataTrans(List<MESBasicDataTrans> dataTranss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportDataTrans(List<MESBasicDataTrans> dataTranss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteDataTrans(MESBasicDataTrans dataTrans){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteDataTrans(MESBasicDataTrans dataTrans){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreDataTrans(MESBasicDataTrans dataTrans){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreDataTrans(MESBasicDataTrans dataTrans){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicDataTrans dataTrans, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicDataTrans dataTrans, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportDataTrans(List<MESBasicDataTrans> insertObjs, List<MESBasicDataTrans> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportDataTrans,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportDataTrans(List<MESBasicDataTrans> insertObjs, List<MESBasicDataTrans> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportDataTrans,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
	...
	public List<Object[]> getDataTransLimited(String sql, List<Object> params, int size){
		SQLQuery query = dataTransDao.createNativeQuery(sql, params.toArray());
	...
	public Date getMinUnprocessedTime(){
		...
		String sql = "select min(data_time) from mesbasic_data_trans where (processed is null or processed = 0) and valid = 1 and business_type='Business_Type/0'";
		Object minObj = dataTransDao.createNativeQuery(sql).uniqueResult();
		if(minObj == null) {
			return null;
		}
		return (Date)minObj;
	}
  	public void update(MESBasicDataTrans dataTrans){
		dataTransDao.update(dataTrans);
	}
	
	public void merge(MESBasicDataTrans dataTrans){
		dataTransDao.merge(dataTrans);
	}
	...
	public void executeUpdate(String sql, Object... args){
		SQLQuery query = dataTransDao.createNativeQuery(sql, args);
		query.executeUpdate();
	}
	
	@Override
	public Query createQuery(String hql, Object... params) {
		return dataTransDao.createQuery(hql, params);
	}
	@Override
	public Page<MESBasicDataTrans> findDataTransPage(Page<MESBasicDataTrans> page,String sql,Object... params){
		return dataTransDao.findByPage(page, sql,params);
	}
/* CUSTOM CODE END */
}
