...
/* CUSTOM CODE START(action,import,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicBaseCustomerAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBaseCustomer> {
	...
	//导出excel时,主模型内容
	List<MESBasicBaseCustomer> queryResults = new ArrayList<MESBasicBaseCustomer>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseCustomer/baseCustomer/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseCustomer/baseCustomer/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_baseCustomer_BaseCustomer" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBaseCustomer> baseCustomers;
	
	public List<MESBasicBaseCustomer> getBaseCustomers() {
	...
	private Page<MESBasicBaseCustomer> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			baseCustomer = new MESBasicBaseCustomer();
		} else {
			if(id > 0){
				baseCustomer = baseCustomerService.getBaseCustomer(id, viewCode);
			}
		}
	...
	public Page<MESBasicBaseCustomer> getPage() {
		if (null == page) {
			page = new Page<MESBasicBaseCustomer>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = baseCustomerService.getBaseCustomerAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String baseCustomerReferenceCopy() throws Exception{
			MESBasicBaseCustomer oldBaseCustomer = null;
			if(isEditNew()){
				...
				oldBaseCustomer = new MESBasicBaseCustomer();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseCustomer.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, baseCustomer.getId());
				...
				oldBaseCustomer = baseCustomerService.getBaseCustomer(id);
			}
			
			
			baseCustomer = baseCustomerService.getBaseCustomer(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_baseCustomer_BaseCustomer");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBaseCustomer.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBaseCustomer, null);
						Method setValue = MESBasicBaseCustomer.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(baseCustomer, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			baseCustomer.setCreateDepartmentId(oldBaseCustomer.getCreateDepartmentId());
			baseCustomer.setCreatePositionId(oldBaseCustomer.getCreatePositionId());
			baseCustomer.setCreateStaffId(oldBaseCustomer.getCreateStaffId());
			baseCustomer.setOwnerDepartmentId(oldBaseCustomer.getOwnerDepartmentId());
			baseCustomer.setOwnerPositionId(oldBaseCustomer.getOwnerPositionId());
			baseCustomer.setOwnerStaffId(oldBaseCustomer.getOwnerStaffId());
			baseCustomer.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/editCustomer",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editCustomer.ftl") }),
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/editCustomerPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editCustomerPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=baseCustomerService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return baseCustomerReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			baseCustomer = baseCustomerService.getBaseCustomer(this.baseCustomer.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseCustomer.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/viewCustomer",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "viewCustomer.ftl") }),
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/viewCustomerPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "viewCustomerPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/custListPart", results = { @Result(type = FREEMARKER, location = "custListPart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/custFirst", results = { @Result(type = FREEMARKER, location = "custFirst.ftl") })
		,
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/custLayout", results = { @Result(type = FREEMARKER, location = "custLayout.ftl") })
		,
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/refCustLayout", results = { @Result(type = FREEMARKER, location = "refCustLayout.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = baseCustomerService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(baseCustomerService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(baseCustomerService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_baseCustomer_BaseCustomer", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/refCustListPart", results = { @Result(type = FREEMARKER, location = "refCustListPart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/custListPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.customerCode,result.customerShortName,result.customerName,result.customerCCCode.ccName,result.customerVocation,result.customerVocationForDisplay,result.customerDCCode,result.customerDCCodeForDisplay,result.customerDegreeCode,result.customerDegreeCodeForDisplay,result.customerProperty,result.customerPropertyForDisplay,result.ownerStaff.name,result.customerCCCode.id,result.customerCCCode.ccCode,result.ownerStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="custListPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"baseCustomer\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"baseCustomer\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("baseCustomerClass.layRec") && request.getParameter("baseCustomerClass.layRec").trim().length() > 0) {
			params.add(new Param("\"customerCCCode\".CC_LAYREC", request.getParameter("baseCustomerClass.layRec"), Param.EQUAL_LIKELEFT));	
		}
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"customerDepartID\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"baseCustomer\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"baseCustomer\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			baseCustomerService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/custListPart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/refCustListPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.customerCode,result.customerShortName,result.customerName,result.customerVocation,result.customerVocationForDisplay,result.customerDCCode,result.customerDCCodeForDisplay,result.customerDegreeCode,result.customerDegreeCodeForDisplay,result.customerProperty,result.customerPropertyForDisplay,result.ownerStaff.name,result.customerCCCode.ccCode,result.ownerStaff.id,result.customerCCCode.id,result.customerCCCode.ccCode,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refCustListPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/allTempleteDownload", results = { @Result(name="excel",type="excel",location="custListPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_baseCustomer_BaseCustomer");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		baseCustomerService.findDataGridPage(dg,dgclass,getDgPage(),baseCustomer,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(baseCustomer.getId() != null && baseCustomer.getId() != -1){
				baseCustomer = baseCustomerService.getBaseCustomer(baseCustomer.getId());
			}
			baseCustomerService.findDataGridPage(dg,dgclass,getDgPage(),baseCustomer,searchCondition,params);
	...
	private void save() throws Exception {
		...
		baseCustomerService.saveBaseCustomer(baseCustomer,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/baseCustomer/baseCustomer/editCustomer/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/baseCustomer/baseCustomer/viewCustomer/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "submit");
		bap_validate_datagrids=baseCustomerService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", baseCustomer.getId());
		...
		fileuploads.put("linkId", baseCustomer.getId());
		bapAfterCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/baseCustomer/baseCustomer/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		baseCustomerService.deleteBaseCustomer(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "restore");
		baseCustomerService.restoreBaseCustomer(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/baseCustomer/baseCustomer/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/baseCustomer/baseCustomer/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "showDealInfoData");
		if (null != tableInfoId) {
			int count=baseCustomerService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = baseCustomerService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = baseCustomerService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBaseCustomer> findExportDataInfos(Page<MESBasicBaseCustomer> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = baseCustomerService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = baseCustomerService.getAssProperty(p.getCode());	
									//String assModelCode = baseCustomerService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			baseCustomerService.findBaseCustomers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			baseCustomerService.findBaseCustomers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBaseCustomer> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBaseCustomer result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".CUSTOMER_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.baseCustomer || null == this.baseCustomer.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/baseCustomer/baseCustomer/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/baseCustomer/baseCustomer/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBaseCustomerService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_baseCustomer", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_baseCustomer");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBaseCustomerService"
						, "com.supcon.orchid.MESBasic.services.MESBasicBaseCustomerClassService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicBaseVendorService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_baseCustomer_BaseCustomer", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_baseCustomer_BaseCustomer", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_baseCustomer_BaseCustomer";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_baseCustomer_BaseCustomer", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicBaseCustomer baseCustomer = null;
		if(null != id) {
			baseCustomer = baseCustomerService.getBaseCustomer(id);
		}
		...
		_auditLog.setColumnStringB(null == baseCustomer.getCustomerCode() ? "" : baseCustomer.getCustomerCode().toString());
		_auditLog.setColumnStringA(null == baseCustomer.getCustomerName() ? "" : baseCustomer.getCustomerName().toString());
		_auditLog.setColumnStringC(baseCustomer.getId().toString());
		_auditLog.setColumnStringD(MESBasicBaseCustomer.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_baseCustomer");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBaseCustomerAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, baseCustomer,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, baseCustomer,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,baseCustomer);
		...
		printlog.setEntity("MESBasic_1_baseCustomer");
		...
		printlog.setBusinessId(baseCustomer.getId());
		printlog.setBussinessKey(null == baseCustomer.getCustomerCode() ? "" : baseCustomer.getCustomerCode().toString());
		printlog.setMainDisplay(null == baseCustomer.getCustomerName() ? "" : baseCustomer.getCustomerName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(baseCustomer,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(baseCustomerService.getBaseCustomer(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, baseCustomer,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				baseCustomer = baseCustomerService.getBaseCustomer(Long.parseLong(arrId[i]));
				if(null != baseCustomer){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == baseCustomer.getCustomerName() ? "" : "," + baseCustomer.getCustomerName().toString()));
					} else {
						_auditLog.setColumnStringA(null == baseCustomer.getCustomerName() ? "" : baseCustomer.getCustomerName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == baseCustomer.getCustomerCode() ? "" : "," + baseCustomer.getCustomerCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == baseCustomer.getCustomerCode() ? "" : baseCustomer.getCustomerCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == baseCustomer.getId() ? "" : "," + baseCustomer.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == baseCustomer.getId() ? "" : baseCustomer.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, baseCustomer,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,baseCustomer);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/baseCustomer/baseCustomer/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicBaseCustomer> findExportPageInfos(Page<MESBasicBaseCustomer> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_baseCustomer" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = baseCustomerService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicBaseCustomer queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = baseCustomerService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = baseCustomerService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_baseCustomer_BaseCustomer");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_baseCustomer_BaseCustomer")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_baseCustomer_BaseCustomer");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_baseCustomer_BaseCustomer");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = baseCustomerService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
