...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicObjTeamAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicObjTeam> {
	...
	private MESBasicObjTeam objTeam;
	...
	//导出excel时,主模型内容
	List<MESBasicObjTeam> queryResults = new ArrayList<MESBasicObjTeam>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objTeam/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objTeam/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryModel_ObjTeam" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicObjTeam> objTeams;
	
	public List<MESBasicObjTeam> getObjTeams() {
		return objTeams;
	}
	...
	private Page<MESBasicObjTeam> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			objTeam = new MESBasicObjTeam();
		} else {
			if(id > 0){
				objTeam = objTeamService.getObjTeam(id, viewCode);
			}
		}
	...
	public Page<MESBasicObjTeam> getPage() {
		if (null == page) {
			page = new Page<MESBasicObjTeam>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicObjTeam getObjTeam() {
		return objTeam;
	}		

	public void setObjTeam(MESBasicObjTeam objTeam) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objTeam/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = objTeamService.getObjTeamAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objTeam/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1490261243605
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String objTeamReferenceCopy() throws Exception{
			MESBasicObjTeam oldObjTeam = null;
			if(isEditNew()){
				...
				oldObjTeam = new MESBasicObjTeam();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objTeam.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, objTeam.getId());
				...
				oldObjTeam = objTeamService.getObjTeam(id);
			}
			
			
			objTeam = objTeamService.getObjTeam(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjTeam");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicObjTeam.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldObjTeam, null);
						Method setValue = MESBasicObjTeam.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(objTeam, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			objTeam.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryModel_ObjTeam", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=objTeamService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return objTeamReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			objTeam = objTeamService.getObjTeam(this.objTeam.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objTeam.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = objTeamService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(objTeamService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(objTeamService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryModel_ObjTeam", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"objTeam\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"objTeam\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"objTeam\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"objTeam\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		objTeamService.findDataGridPage(dg,dgclass,getDgPage(),objTeam,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(objTeam.getId() != null && objTeam.getId() != -1){
				objTeam = objTeamService.getObjTeam(objTeam.getId());
			}
			objTeamService.findDataGridPage(dg,dgclass,getDgPage(),objTeam,searchCondition,params);
	...
	private void save() throws Exception {
		...
		objTeamService.saveObjTeam(objTeam,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjTeam", "submit");
		bap_validate_datagrids=objTeamService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", objTeam.getId());
		...
		fileuploads.put("linkId", objTeam.getId());
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjTeam", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryModel/objTeam/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjTeam", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		objTeamService.deleteObjTeam(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjTeam", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objTeam/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjTeam", "restore");
		objTeamService.restoreObjTeam(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjTeam", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicObjTeam> findExportDataInfos(Page<MESBasicObjTeam> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = objTeamService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = objTeamService.getAssProperty(p.getCode());	
									//String assModelCode = objTeamService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			objTeamService.findObjTeams(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			objTeamService.findObjTeams(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicObjTeam> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicObjTeam result : results){
	...
	public boolean isEditNew(){
		//return null == this.objTeam || null == this.objTeam.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryModel/objTeam/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjTeam", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjTeam", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryModel/objTeam/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjTeam", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryModel/objTeam/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objTeam/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicObjTeamService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjTeam", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjTeam", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objTeam/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryModel_ObjTeam", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryModel_ObjTeam", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryModel/objTeam/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjTeam", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryModel/objTeam/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryModel_ObjTeam";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjTeam", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryModel/objTeam/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == objTeam.getId() ? "" : objTeam.getId().toString());
		_auditLog.setColumnStringA(null == objTeam.getId() ? "" : objTeam.getId().toString());
		_auditLog.setColumnStringC(objTeam.getId().toString());
		_auditLog.setColumnStringD(MESBasicObjTeam.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicObjTeamAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, objTeam,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, objTeam,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,objTeam);
		...
		printlog.setBusinessId(objTeam.getId());
		printlog.setBussinessKey(null == objTeam.getId() ? "" : objTeam.getId().toString());
		printlog.setMainDisplay(null == objTeam.getId() ? "" : objTeam.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryModel/objTeam/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(objTeam,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objTeam/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(objTeamService.getObjTeam(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryModel/objTeam/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryModel/objTeam/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, objTeam,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objTeam = objTeamService.getObjTeam(Long.parseLong(arrId[i]));
				if(null != objTeam){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == objTeam.getId() ? "" : "," + objTeam.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == objTeam.getId() ? "" : objTeam.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == objTeam.getId() ? "" : "," + objTeam.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == objTeam.getId() ? "" : objTeam.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == objTeam.getId() ? "" : "," + objTeam.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == objTeam.getId() ? "" : objTeam.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, objTeam,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,objTeam);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/factoryModel/objTeam/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjTeam");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryModel_ObjTeam")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryModel_ObjTeam");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjTeam");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = objTeamService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryModel_ObjTeam,MESBasic_1) */
// 自定义代码
