...
/* CUSTOM CODE START(action,import,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicCprodAttAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicCprodAtt> {
	...
	private MESBasicCprodAtt cprodAtt;
	...
	//导出excel时,主模型内容
	List<MESBasicCprodAtt> queryResults = new ArrayList<MESBasicCprodAtt>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/cprodAtt/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/cprodAtt/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_product_CprodAtt" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicCprodAtt> cprodAtts;
	
	public List<MESBasicCprodAtt> getCprodAtts() {
		return cprodAtts;
	}
	...
	private Page<MESBasicCprodAtt> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			cprodAtt = new MESBasicCprodAtt();
		} else {
			if(id > 0){
				cprodAtt = cprodAttService.getCprodAtt(id, viewCode);
			}
		}
	...
	public Page<MESBasicCprodAtt> getPage() {
		if (null == page) {
			page = new Page<MESBasicCprodAtt>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicCprodAtt getCprodAtt() {
		return cprodAtt;
	}		

	public void setCprodAtt(MESBasicCprodAtt cprodAtt) {
		this.cprodAtt = cprodAtt;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/cprodAtt/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = cprodAttService.getCprodAttAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/cprodAtt/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1470295331697
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String cprodAttReferenceCopy() throws Exception{
			MESBasicCprodAtt oldCprodAtt = null;
			if(isEditNew()){
				...
				oldCprodAtt = new MESBasicCprodAtt();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,cprodAtt.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, cprodAtt.getId());
				...
				oldCprodAtt = cprodAttService.getCprodAtt(id);
			}
			
			
			cprodAtt = cprodAttService.getCprodAtt(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_product_CprodAtt");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicCprodAtt.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCprodAtt, null);
						Method setValue = MESBasicCprodAtt.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(cprodAtt, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			cprodAtt.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_product_CprodAtt", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=cprodAttService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return cprodAttReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			cprodAtt = cprodAttService.getCprodAtt(this.cprodAtt.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,cprodAtt.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = cprodAttService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(cprodAttService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(cprodAttService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_product_CprodAtt", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"cprodAtt\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"cprodAtt\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("prodAttribute.layRec") && request.getParameter("prodAttribute.layRec").trim().length() > 0) {
			params.add(new Param("\"prodAttrId\".PRODATT_LAYREC", request.getParameter("prodAttribute.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"cprodAtt\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"cprodAtt\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		cprodAttService.findDataGridPage(dg,dgclass,getDgPage(),cprodAtt,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(cprodAtt.getId() != null && cprodAtt.getId() != -1){
				cprodAtt = cprodAttService.getCprodAtt(cprodAtt.getId());
			}
			cprodAttService.findDataGridPage(dg,dgclass,getDgPage(),cprodAtt,searchCondition,params);
	...
	private void save() throws Exception {
		...
		cprodAttService.saveCprodAtt(cprodAtt,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_CprodAtt", "submit");
		bap_validate_datagrids=cprodAttService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", cprodAtt.getId());
		...
		fileuploads.put("linkId", cprodAtt.getId());
		bapAfterCustomMethod("MESBasic_1_product_CprodAtt", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/product/cprodAtt/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_CprodAtt", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		cprodAttService.deleteCprodAtt(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_product_CprodAtt", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/cprodAtt/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_CprodAtt", "restore");
		cprodAttService.restoreCprodAtt(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_product_CprodAtt", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicCprodAtt> findExportDataInfos(Page<MESBasicCprodAtt> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = cprodAttService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = cprodAttService.getAssProperty(p.getCode());	
									//String assModelCode = cprodAttService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			cprodAttService.findCprodAtts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			cprodAttService.findCprodAtts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicCprodAtt> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicCprodAtt result : results){
	...
	public boolean isEditNew(){
		//return null == this.cprodAtt || null == this.cprodAtt.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/product/cprodAtt/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_CprodAtt", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_product_CprodAtt", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/product/cprodAtt/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_CprodAtt", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/product/cprodAtt/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/cprodAtt/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicCprodAttService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_product_CprodAtt", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_product_CprodAtt", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/cprodAtt/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_product_CprodAtt", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_product_CprodAtt", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/product/cprodAtt/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_CprodAtt", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/product/cprodAtt/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_product_CprodAtt";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_product_CprodAtt", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/product/cprodAtt/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
		_auditLog.setColumnStringA(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
		_auditLog.setColumnStringC(cprodAtt.getId().toString());
		_auditLog.setColumnStringD(MESBasicCprodAtt.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicCprodAttAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, cprodAtt,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, cprodAtt,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,cprodAtt);
		...
		printlog.setBusinessId(cprodAtt.getId());
		printlog.setBussinessKey(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
		printlog.setMainDisplay(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/product/cprodAtt/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(cprodAtt,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/cprodAtt/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(cprodAttService.getCprodAtt(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/product/cprodAtt/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/product/cprodAtt/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, cprodAtt,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				cprodAtt = cprodAttService.getCprodAtt(Long.parseLong(arrId[i]));
				if(null != cprodAtt){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == cprodAtt.getId() ? "" : "," + cprodAtt.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == cprodAtt.getId() ? "" : "," + cprodAtt.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == cprodAtt.getId() ? "" : "," + cprodAtt.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == cprodAtt.getId() ? "" : cprodAtt.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, cprodAtt,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,cprodAtt);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/product/cprodAtt/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_product_CprodAtt");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_product_CprodAtt")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_product_CprodAtt");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_product_CprodAtt");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = cprodAttService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_product_CprodAtt,MESBasic_1) */
// 自定义代码
