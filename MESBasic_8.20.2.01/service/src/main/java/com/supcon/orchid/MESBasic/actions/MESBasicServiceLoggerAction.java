...
/* CUSTOM CODE START(action,import,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicServiceLoggerAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicServiceLogger> {
	...
	private MESBasicServiceLogger serviceLogger;
	...
	//导出excel时,主模型内容
	List<MESBasicServiceLogger> queryResults = new ArrayList<MESBasicServiceLogger>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/backgroundServiceMonitor/serviceLogger/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/backgroundServiceMonitor/serviceLogger/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_backgroundServiceMonitor_ServiceLogger" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicServiceLogger> serviceLoggers;
	
	public List<MESBasicServiceLogger> getServiceLoggers() {
	...
	private Page<MESBasicServiceLogger> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			serviceLogger = new MESBasicServiceLogger();
		} else {
			if(id > 0){
				serviceLogger = serviceLoggerService.getServiceLogger(id, viewCode);
			}
		}
	...
	public Page<MESBasicServiceLogger> getPage() {
		if (null == page) {
			page = new Page<MESBasicServiceLogger>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicServiceLogger getServiceLogger() {
		return serviceLogger;
	}		

	public void setServiceLogger(MESBasicServiceLogger serviceLogger) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = serviceLoggerService.getServiceLoggerAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1429057309643
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String serviceLoggerReferenceCopy() throws Exception{
			MESBasicServiceLogger oldServiceLogger = null;
			if(isEditNew()){
				...
				oldServiceLogger = new MESBasicServiceLogger();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,serviceLogger.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, serviceLogger.getId());
				...
				oldServiceLogger = serviceLoggerService.getServiceLogger(id);
			}
			
			
			serviceLogger = serviceLoggerService.getServiceLogger(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_backgroundServiceMonitor_ServiceLogger");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicServiceLogger.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldServiceLogger, null);
						Method setValue = MESBasicServiceLogger.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(serviceLogger, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			serviceLogger.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=serviceLoggerService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return serviceLoggerReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			serviceLogger = serviceLoggerService.getServiceLogger(this.serviceLogger.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,serviceLogger.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/logHistory", results = { @Result(type = FREEMARKER, location = "logHistory.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = serviceLoggerService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(serviceLoggerService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(serviceLoggerService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_backgroundServiceMonitor_ServiceLogger", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/logHistory-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.logTime,result.logContent,result.mesService.serviceCode,result.mesService.serviceName,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="logHistory-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"serviceLogger\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"serviceLogger\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"serviceLogger\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"serviceLogger\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			serviceLoggerService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/logHistory-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		serviceLoggerService.findDataGridPage(dg,dgclass,getDgPage(),serviceLogger,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(serviceLogger.getId() != null && serviceLogger.getId() != -1){
				serviceLogger = serviceLoggerService.getServiceLogger(serviceLogger.getId());
			}
			serviceLoggerService.findDataGridPage(dg,dgclass,getDgPage(),serviceLogger,searchCondition,params);
	...
	private void save() throws Exception {
		...
		serviceLoggerService.saveServiceLogger(serviceLogger,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "submit");
		bap_validate_datagrids=serviceLoggerService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", serviceLogger.getId());
		...
		fileuploads.put("linkId", serviceLogger.getId());
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		serviceLoggerService.deleteServiceLogger(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "restore");
		serviceLoggerService.restoreServiceLogger(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicServiceLogger> findExportDataInfos(Page<MESBasicServiceLogger> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = serviceLoggerService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = serviceLoggerService.getAssProperty(p.getCode());	
									//String assModelCode = serviceLoggerService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			serviceLoggerService.findServiceLoggers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			serviceLoggerService.findServiceLoggers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicServiceLogger> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicServiceLogger result : results){
	...
	public boolean isEditNew(){
		//return null == this.serviceLogger || null == this.serviceLogger.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/backgroundServiceMonitor/serviceLogger/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/backgroundServiceMonitor/serviceLogger/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicServiceLoggerService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicServiceLoggerService"
						, "com.supcon.orchid.MESBasic.services.MESBasicMESServicesService"       
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_backgroundServiceMonitor_ServiceLogger", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_backgroundServiceMonitor_ServiceLogger", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_backgroundServiceMonitor_ServiceLogger";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_ServiceLogger", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
		_auditLog.setColumnStringA(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
		_auditLog.setColumnStringC(serviceLogger.getId().toString());
		_auditLog.setColumnStringD(MESBasicServiceLogger.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicServiceLoggerAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, serviceLogger,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, serviceLogger,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,serviceLogger);
		...
		printlog.setBusinessId(serviceLogger.getId());
		printlog.setBussinessKey(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
		printlog.setMainDisplay(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(serviceLogger,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(serviceLoggerService.getServiceLogger(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, serviceLogger,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				serviceLogger = serviceLoggerService.getServiceLogger(Long.parseLong(arrId[i]));
				if(null != serviceLogger){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == serviceLogger.getId() ? "" : "," + serviceLogger.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == serviceLogger.getId() ? "" : "," + serviceLogger.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == serviceLogger.getId() ? "" : "," + serviceLogger.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, serviceLogger,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,serviceLogger);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/serviceLogger/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_backgroundServiceMonitor_ServiceLogger");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_backgroundServiceMonitor_ServiceLogger")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_backgroundServiceMonitor_ServiceLogger");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_backgroundServiceMonitor_ServiceLogger");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = serviceLoggerService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
