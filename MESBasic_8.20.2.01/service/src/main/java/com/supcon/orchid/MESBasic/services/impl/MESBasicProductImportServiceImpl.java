...
@Service("mESBasicProductImportService")
public class MESBasicProductImportServiceImpl implements MESBasicProductImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_Product",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicProduct bizObj, MESBasicProduct oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.radion1415772266801") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getProductName() ? "" : "," + oldBizObj.getProductName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getProductName() ? "" : oldBizObj.getProductName().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.radion1415772266801") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getProductName() ? "" : "," + bizObj.getProductName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getProductName() ? "" : bizObj.getProductName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicProduct bizObj, String bussinesskeyFieldName,
