...
@Repository
public class MESBasicRoutineDaoImpl extends ExtendedGenericDaoImpl<MESBasicRoutine, Long> implements MESBasicRoutineDao {
	@Override
	...
	@Override
	public MESBasicRoutineDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicRoutineDealInfo dealInfo = new MESBasicRoutineDealInfo();
	...
	@Override
	public MESBasicRoutineDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicRoutine mainObj, Staff dealStaff) {
		MESBasicRoutineDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicRoutineDealInfo.class);
		...
		List<MESBasicRoutineDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
