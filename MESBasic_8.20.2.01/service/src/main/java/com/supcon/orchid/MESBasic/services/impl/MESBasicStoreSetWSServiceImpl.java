...
import com.supcon.orchid.MESBasic.services.MESBasicStoreSetWSService;
import com.supcon.orchid.MESBasic.services.MESBasicStoreSetService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicStoreSet",targetNamespace = "http://ws.supcon.com")
public class MESBasicStoreSetWSServiceImpl implements MESBasicStoreSetWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getStoreSetByBusinessKey(
		...
		try{
			...
			MESBasicStoreSet storeSet = storeSetService.loadStoreSetByBussinessKey(businessKey);
			if(storeSet == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, storeSet);
		}catch(Throwable t){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getStoreSetByPk(
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO updateStoreSet(
		@WebParam(name="storeSet")MESBasicStoreSet storeSet,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
			
		JWSResultDTO result = new JWSResultDTO();
		try{
			if(storeSet == null){
				return set500Error(result, "货位档案不能为空。");
			}
			...
			if(!beforeUpdateStoreSet(storeSet)){
				return set404Error(result);
			}
			storeSetService.saveStoreSet(storeSet, null);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO addStoreSet(
		@WebParam(name="storeSet")MESBasicStoreSet storeSet,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				checkStoreSetData(storeSet);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			storeSetService.saveStoreSet(storeSet, null);
			setSuccessResult(result, storeSet.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteStoreSetByBusinessKey(
		@WebParam(name="businessKey")String businessKey,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				storeSet = storeSetService.loadStoreSetByBussinessKey(businessKey);
	...
	@Override//
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteStoreSetByPk(
		@WebParam(name="pk")String pk,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddStoreSet(
		@WebParam(name="storeSetList")List<MESBasicStoreSet> storeSetList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			int count = 1;
			for(MESBasicStoreSet storeSet : storeSetList){
				if(storeSet.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(storeSet.getId() == null || storeSet.getId().toString().trim().isEmpty()){
			...
			storeSetService.batchImportBaseStoreSet(storeSetList);
			for(MESBasicStoreSet storeSet : storeSetList){
				idList.add(storeSet.getId());
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdateStoreSet(
		@WebParam(name="storeSetList")List<MESBasicStoreSet> storeSetList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteStoreSetByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicStoreSet temp = null;
				try{
					temp = storeSetService.getStoreSet(id);
				}catch(NullPointerException e){}
			...
			storeSetService.deleteStoreSet(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteStoreSetByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicStoreSet temp = null;
				try{
					temp = storeSetService.loadStoreSetByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getStoreSetPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicStoreSet.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicStoreSet.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicStoreSet.class);
				} catch (Exception e) {
			...
			Page<MESBasicStoreSet> page = new Page<MESBasicStoreSet>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicStoreSet> datas = storeSetService.getByPage(page, detachedCriteria);
			List<MESBasicStoreSet> storeSetList = datas.getResult();
			if (storeSetList == null || storeSetList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(storeSetList.toArray());
			PaginationDTO<MESBasicStoreSet> pagination = new PaginationDTO<MESBasicStoreSet>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicStoreSet storeSet = storeSetService.getStoreSet(id);
			if (storeSet == null || !storeSet.isValid()) {
				result.setErrorMessage("找不到货位档案");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取货位档案对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param storeSet
	 * @return
	 */
	private MESBasicStoreSet getStoreSet(MESBasicStoreSet storeSet) {
		if (storeSet.getId() == null && (storeSet.getId() == null || storeSet.getId().toString().trim() == ""))
			throw new RuntimeException("货位档案对象业务主键和id不能同时为空");
		if (storeSet.getId() != null) {
			MESBasicStoreSet result = storeSetService.getStoreSet(storeSet .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return storeSetService.loadStoreSetByBussinessKey(storeSet.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个货位档案，则返回false，否则返回true
	 * @param storeSet
	 */
	private boolean beforeUpdateStoreSet(MESBasicStoreSet storeSet) {
		MESBasicStoreSet temp = getStoreSet(storeSet);
		if (temp == null) {
			return false;
		}
		if(storeSet.getId() == null){
			storeSet.setId(temp.getId());		
		}
		if(storeSet.getId() == null){
			storeSet.setId(temp.getId());
		}
		
		storeSet.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除货位档案对象下所有辅模型对象中的货位档案对象
	 * @param storeSet
	 */
	private void clearCircle(MESBasicStoreSet storeSet) {
	 ...
	 */
	private void checkStoreSetData(MESBasicStoreSet storeSet){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
