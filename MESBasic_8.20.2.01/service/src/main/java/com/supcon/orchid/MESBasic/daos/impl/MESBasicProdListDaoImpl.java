...
@Repository
public class MESBasicProdListDaoImpl extends ExtendedGenericDaoImpl<MESBasicProdList, Long> implements MESBasicProdListDao {
	@Override
	...
	@Override
	public MESBasicProdListDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicProdListDealInfo dealInfo = new MESBasicProdListDealInfo();
	...
	@Override
	public MESBasicProdListDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicProdList mainObj, Staff dealStaff) {
		MESBasicProdListDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicProdListDealInfo.class);
		...
		List<MESBasicProdListDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	 /* CUSTOM CODE START(daoimpl,functions,MESBasic_1_prodList_ProdList,MESBasic_1) */
// 自定义代码
