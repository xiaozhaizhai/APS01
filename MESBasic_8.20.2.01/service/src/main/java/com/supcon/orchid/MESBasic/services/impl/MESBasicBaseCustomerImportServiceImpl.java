...
@Service("mESBasicBaseCustomerImportService")
public class MESBasicBaseCustomerImportServiceImpl implements MESBasicBaseCustomerImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseCustomer",model="MESBasic_1_baseCustomer_BaseCustomer",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicBaseCustomer bizObj, MESBasicBaseCustomer oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1449638926683") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getCustomerName() ? "" : "," + oldBizObj.getCustomerName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getCustomerName() ? "" : oldBizObj.getCustomerName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getCustomerCode() ? "" : "," + oldBizObj.getCustomerCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getCustomerCode() ? "" : oldBizObj.getCustomerCode().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1449638926683") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getCustomerName() ? "" : "," + bizObj.getCustomerName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getCustomerName() ? "" : bizObj.getCustomerName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getCustomerCode() ? "" : "," + bizObj.getCustomerCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getCustomerCode() ? "" : bizObj.getCustomerCode().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicBaseCustomer bizObj, String bussinesskeyFieldName,
