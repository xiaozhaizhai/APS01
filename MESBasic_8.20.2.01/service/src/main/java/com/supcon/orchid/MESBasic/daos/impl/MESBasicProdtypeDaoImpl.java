...
@Repository
public class MESBasicProdtypeDaoImpl extends ExtendedGenericDaoImpl<MESBasicProdtype, Long> implements MESBasicProdtypeDao {
	@Override
	...
	@Override
	public MESBasicProdtypeDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicProdtypeDealInfo dealInfo = new MESBasicProdtypeDealInfo();
	...
	@Override
	public MESBasicProdtypeDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicProdtype mainObj, Staff dealStaff) {
		MESBasicProdtypeDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicProdtypeDealInfo.class);
		...
		List<MESBasicProdtypeDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	 /* CUSTOM CODE START(daoimpl,functions,MESBasic_1_prodtype_Prodtype,MESBasic_1) */
// 自定义代码
