...
/* CUSTOM CODE START(action,import,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicCommDealInfoAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicCommDealInfo> {
	...
	private MESBasicCommDealInfo commDealInfo;
	...
	//导出excel时,主模型内容
	List<MESBasicCommDealInfo> queryResults = new ArrayList<MESBasicCommDealInfo>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/commDealInfo/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/commDealInfo/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_product_CommDealInfo" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicCommDealInfo> commDealInfos;
	
	public List<MESBasicCommDealInfo> getCommDealInfos() {
		return commDealInfos;
	}
	...
	private Page<MESBasicCommDealInfo> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			commDealInfo = new MESBasicCommDealInfo();
			if(commDealInfo.getBapType() != null) {
				commDealInfo.setBapType((SystemCode) systemCodeService.getSystemCode(commDealInfo.getBapType().getId()));
			}
		} else {
			if(id > 0){
				commDealInfo = commDealInfoService.getCommDealInfo(id, viewCode);
			}
		}
	...
	public Page<MESBasicCommDealInfo> getPage() {
		if (null == page) {
			page = new Page<MESBasicCommDealInfo>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicCommDealInfo getCommDealInfo() {
		return commDealInfo;
	}		

	public void setCommDealInfo(MESBasicCommDealInfo commDealInfo) {
		this.commDealInfo = commDealInfo;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/commDealInfo/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = commDealInfoService.getCommDealInfoAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/commDealInfo/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1471524614104
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String commDealInfoReferenceCopy() throws Exception{
			MESBasicCommDealInfo oldCommDealInfo = null;
			if(isEditNew()){
				...
				oldCommDealInfo = new MESBasicCommDealInfo();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,commDealInfo.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, commDealInfo.getId());
				...
				oldCommDealInfo = commDealInfoService.getCommDealInfo(id);
			}
			
			
			commDealInfo = commDealInfoService.getCommDealInfo(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_product_CommDealInfo");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicCommDealInfo.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCommDealInfo, null);
						Method setValue = MESBasicCommDealInfo.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(commDealInfo, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			commDealInfo.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_product_CommDealInfo", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=commDealInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return commDealInfoReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			commDealInfo = commDealInfoService.getCommDealInfo(this.commDealInfo.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,commDealInfo.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/commDealInfo/dealInfoList", results = { @Result(type = FREEMARKER, location = "dealInfoList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = commDealInfoService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(commDealInfoService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(commDealInfoService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_product_CommDealInfo", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/commDealInfo/dealInfoList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.dealerID.name,result.dealTime,result.dealContent,result.dealerID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="dealInfoList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"commDealInfo\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"commDealInfo\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"commDealInfo\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"commDealInfo\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			commDealInfoService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/commDealInfo/dealInfoList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		commDealInfoService.findDataGridPage(dg,dgclass,getDgPage(),commDealInfo,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(commDealInfo.getId() != null && commDealInfo.getId() != -1){
				commDealInfo = commDealInfoService.getCommDealInfo(commDealInfo.getId());
			}
			commDealInfoService.findDataGridPage(dg,dgclass,getDgPage(),commDealInfo,searchCondition,params);
	...
	private void save() throws Exception {
		...
		commDealInfoService.saveCommDealInfo(commDealInfo,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_CommDealInfo", "submit");
		bap_validate_datagrids=commDealInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", commDealInfo.getId());
		...
		fileuploads.put("linkId", commDealInfo.getId());
		bapAfterCustomMethod("MESBasic_1_product_CommDealInfo", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/product/commDealInfo/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_CommDealInfo", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		commDealInfoService.deleteCommDealInfo(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_product_CommDealInfo", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/commDealInfo/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_CommDealInfo", "restore");
		commDealInfoService.restoreCommDealInfo(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_product_CommDealInfo", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicCommDealInfo> findExportDataInfos(Page<MESBasicCommDealInfo> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = commDealInfoService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = commDealInfoService.getAssProperty(p.getCode());	
									//String assModelCode = commDealInfoService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			commDealInfoService.findCommDealInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			commDealInfoService.findCommDealInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicCommDealInfo> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicCommDealInfo result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".DEALINFO_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.commDealInfo || null == this.commDealInfo.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/product/commDealInfo/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_CommDealInfo", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_product_CommDealInfo", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/product/commDealInfo/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_CommDealInfo", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/product/commDealInfo/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/commDealInfo/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicCommDealInfoService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_product_CommDealInfo", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_product_CommDealInfo", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/commDealInfo/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_product_CommDealInfo", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_product_CommDealInfo", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/product/commDealInfo/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_CommDealInfo", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/product/commDealInfo/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_product_CommDealInfo";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_product_CommDealInfo", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/product/commDealInfo/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
		_auditLog.setColumnStringA(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
		_auditLog.setColumnStringC(commDealInfo.getId().toString());
		_auditLog.setColumnStringD(MESBasicCommDealInfo.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicCommDealInfoAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, commDealInfo,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, commDealInfo,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,commDealInfo);
		...
		printlog.setBusinessId(commDealInfo.getId());
		printlog.setBussinessKey(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
		printlog.setMainDisplay(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/product/commDealInfo/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(commDealInfo,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/commDealInfo/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(commDealInfoService.getCommDealInfo(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/product/commDealInfo/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/product/commDealInfo/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, commDealInfo,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				commDealInfo = commDealInfoService.getCommDealInfo(Long.parseLong(arrId[i]));
				if(null != commDealInfo){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == commDealInfo.getId() ? "" : "," + commDealInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == commDealInfo.getId() ? "" : "," + commDealInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == commDealInfo.getId() ? "" : "," + commDealInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, commDealInfo,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,commDealInfo);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/product/commDealInfo/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_product_CommDealInfo");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_product_CommDealInfo")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_product_CommDealInfo");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_product_CommDealInfo");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = commDealInfoService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
