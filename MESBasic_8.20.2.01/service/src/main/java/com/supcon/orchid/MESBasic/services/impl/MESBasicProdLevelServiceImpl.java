...
import com.supcon.orchid.MESBasic.services.MESBasicProdLevelImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicProdLevelDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_prodLevelService")
@Transactional
public class MESBasicProdLevelServiceImpl  extends BaseServiceImpl<MESBasicProdLevel> implements MESBasicProdLevelService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicProdLevel> bapBaseService;
	...
	@Autowired
	private MESBasicProdLevelImportService prodLevelImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicProdLevel.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  prodLevelDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicProdLevel.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = prodLevelDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdLevel.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdLevel.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  prodLevelDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicProdLevel.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicProdLevel.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicProdLevel> list = (List<MESBasicProdLevel>) prodLevelDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicProdLevel getProdLevel(long id){
		return getProdLevel(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdLevel getProdLevel(long id, String viewCode){
		MESBasicProdLevel prodLevel = prodLevelDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProdLevelAsJSON(long id, String include){
		MESBasicProdLevel prodLevel = prodLevelDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(prodLevel, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.common.delete",operType="3")
	public void deleteProdLevel(MESBasicProdLevel prodLevel){
		if(null != prodLevel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdLevel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
			AuditUtil.setColumnStringA(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
			AuditUtil.setColumnStringC(prodLevel.getId().toString());
		}
		beforeDeleteProdLevel(prodLevel);
		prodLevelDao.deletePhysical(prodLevel);
		afterDeleteProdLevel(prodLevel);
		prodLevelDao.flush();
		List<MESBasicProdLevel> params = new ArrayList<MESBasicProdLevel>();
		params.add(prodLevel);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.common.delete",operType="3")
	public void deleteProdLevel(List<Long> prodLevelIds){
		deleteProdLevel(prodLevelIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.common.delete",operType="3")
	public void deleteProdLevel(List<Long> prodLevelIds, String eventTopic) {
		List<MESBasicProdLevel> prodLevels = new ArrayList<MESBasicProdLevel>();
		for(Long prodLevelId : prodLevelIds){
			MESBasicProdLevel prodLevel = getProdLevel(prodLevelId);
			prodLevels.add(prodLevel);
			if(null != prodLevel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdLevel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == prodLevel.getId() ? "" : "," + prodLevel.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == prodLevel.getId() ? "" : "," + prodLevel.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == prodLevel.getId() ? "" : "," + prodLevel.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(prodLevels != null && prodLevels.size() > 0 ){
			for(MESBasicProdLevel prodLevel : prodLevels){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicProdLevel.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(prodLevel));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),prodLevel, prodLevel.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(prodLevelIds != null && prodLevelIds.size() > 0) {
			String hql = "delete " + MESBasicProdLevel.JPA_NAME + " where id in(:ids)";
			Query query = prodLevelDao.createQuery(hql);
			query.setParameterList("ids", prodLevelIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicProdLevel prodLevelz : prodLevels){
				prodLevelz.setValid(false);
				List<MESBasicProdLevel> params = new ArrayList<MESBasicProdLevel>();
				params.add(prodLevelz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_product");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.common.delete",operType="3")
	public void deleteProdLevel(long prodLevelId, int prodLevelVersion){
		this.deleteProdLevel(prodLevelId, prodLevelVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.common.delete",operType="3")
	public void deleteProdLevel(long prodLevelId, int prodLevelVersion,SignatureLog signatureLog){
		MESBasicProdLevel prodLevel = getProdLevel(prodLevelId);
		...
		beforeDeleteProdLevel(prodLevel);
		prodLevelDao.delete(prodLevelId, prodLevelVersion);
		afterDeleteProdLevel(prodLevel);
		prodLevelDao.flush();
		List<MESBasicProdLevel> params = new ArrayList<MESBasicProdLevel>();
		params.add(prodLevel);
		...
		if(signatureLog != null ) {
			...
			businessKey=prodLevel.getId();
			signatureLog.setTableId(prodLevel.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.common.delete",operType="3")
	public void deleteProdLevel(String prodLevelIds){
		this.deleteProdLevel(prodLevelIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.common.delete",operType="3")
	public void deleteProdLevel(String prodLevelIds,SignatureLog signatureLog){
		deleteCollection(prodLevelIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProdLevel(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_product#modelCode:MESBasic_1_product_ProdLevel#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="ec.engine.restore",operType="10")
	public void restoreProdLevel(String prodLevelIds){
		restoreCollection(prodLevelIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProdLevel(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="ec.engine.restore",operType="10")
	public void restoreProdLevel(long prodLevelId){
		
		findBusinessKeyUsed(prodLevelId);	//判断业务主键是否重复
		...
		beforeRestoreProdLevel(prodLevel);
		prodLevel.setValid(true);
		prodLevelDao.update(prodLevel);
		afterRestoreProdLevel(prodLevel);
		List<MESBasicProdLevel> params = new ArrayList<MESBasicProdLevel>();
		params.add(prodLevel);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long prodLevelId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_product_ProdLevel");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicProdLevel.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicProdLevel.TABLE_NAME +" where id = ? )";
			List<Object> list =  prodLevelDao.createNativeQuery(sql,prodLevelId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProdLevel(List<MESBasicProdLevel>  prodLevels){
		for(MESBasicProdLevel prodLevel:prodLevels)  {
			saveProdLevel(prodLevel, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProdLevel(List<MESBasicProdLevel>  prodLevels){
		for(MESBasicProdLevel prodLevel:prodLevels)  {
			saveProdLevel(prodLevel, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="ec.flow.save")
	public void saveProdLevel(MESBasicProdLevel prodLevel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProdLevel(prodLevel, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="ec.flow.save")
	public void saveProdLevel(MESBasicProdLevel prodLevel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProdLevel(prodLevel, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProdLevelCodes(MESBasicProdLevel prodLevel) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdLevelCodes(prodLevel, false);
	}
	
	public void generateProdLevelCodes(MESBasicProdLevel prodLevel, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProdLevelSummarys(MESBasicProdLevel prodLevel) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdLevelSummarys(prodLevel, false);
	}

	public void generateProdLevelSummarys(MESBasicProdLevel prodLevel, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel")
	public void saveProdLevel(MESBasicProdLevel prodLevel,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProdLevel(prodLevel, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel")
	public void saveProdLevel(MESBasicProdLevel prodLevel,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(prodLevel.getId() != null && prodLevel.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdLevel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, prodLevel);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(prodLevel);
		beforeSaveProdLevel(prodLevel, viewIsView);

		if (viewIsView) {
			prodLevelDao.saveWithRevertVersion(prodLevel);
		} else {
			if(isNew)
				prodLevelDao.save(prodLevel);
			else
				prodLevelDao.merge(prodLevel);
		}
		if(null != prodLevel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdLevel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
			AuditUtil.setColumnStringA(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
		...
		dealDatagridsSave(prodLevel,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProdLevelCodes(prodLevel, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProdLevelSummarys(prodLevel, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProdLevel(prodLevel, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, prodLevel);
			}
		}
		List<MESBasicProdLevel> params = new ArrayList<MESBasicProdLevel>();
		prodLevelDao.flush();
		prodLevelDao.clear();
		prodLevel = prodLevelDao.load(prodLevel.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicProdLevel prodLevel){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("prodLevel", prodLevel);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel")
	public void mergeProdLevel(MESBasicProdLevel prodLevel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(prodLevel);
		beforeSaveProdLevel(prodLevel);
		prodLevelDao.merge(prodLevel);
		if(null != prodLevel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdLevel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
			AuditUtil.setColumnStringA(null == prodLevel.getId() ? "" : prodLevel.getId().toString());
		...
		dealDatagridsSave(prodLevel,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProdLevel(prodLevel);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicProdLevel> findProdLevels(Page<MESBasicProdLevel> page, Criterion... criterions) {
		return prodLevelDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdLevels(Page<MESBasicProdLevel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProdLevels(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdLevels(Page<MESBasicProdLevel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdLevels(Page<MESBasicProdLevel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicProdLevel> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"prodLevel\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"prodLevel\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"prodLevel\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"prodLevel\".PRODLEVEL_PRODUCTID IN ( SELECT PRODUCT_ID FROM S2BASE_PRODUCT \"product\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  prodLevelDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  prodLevelDao.createNativeQuery(powerOperateSql, "MESBasic_1_product").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"prodLevel\".LAY_REC ASC, \"prodLevel\".SORT ASC");
				} else {
					orderPart.append(" \"prodLevel\".PRODLEVEL_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) prodLevelDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=prodLevelDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = prodLevelDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicProdLevel> result = new ArrayList<MESBasicProdLevel>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdLevel.class,prodLevelDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdLevel.class,prodLevelDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicProdLevel prodLevel : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(prodLevel.getId(), "MESBasic_product_prodLevel");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(prodLevel.getId(), "MESBasic_product_prodLevel");
							if(null != documents && !documents.isEmpty()) {
								prodLevel.setDocument(documents.get(0));
								prodLevel.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicProdLevel> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicProdLevel> result) {
		View view = null;
		...
		Map<Long, MESBasicProdLevel> rsMap = new HashMap<Long, MESBasicProdLevel>();
		for (MESBasicProdLevel prodLevel : result) {
			ids.add(prodLevel.getId());
			rsMap.put(prodLevel.getId(), prodLevel);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(prodLevelDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = prodLevelDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_product_ProdLevel" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_product_ProdLevel" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  prodLevelDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodLevel\"","MESBasic_1_product","","S2BASE_PRODLEVEL",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"prodLevel\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"prodLevel\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodLevel\"","MESBasic_1_product","","S2BASE_PRODLEVEL",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_product_ProdLevel" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("prodLevel.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"prodLevel\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"prodLevel\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"prodLevel\".CID = 1 OR \"prodLevel\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("prodLevel.mc\"").append(".PROD_LEVEL) not in ("+notinValue+")");
			}
			
			SQLQuery query = prodLevelDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=prodLevelDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicProdLevel.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicProdLevel.MODULE_CODE;}
	protected String getModelCode(){return MESBasicProdLevel.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param prodLevel 模型实例
	 */
	public void checkUniqueConstraint(MESBasicProdLevel prodLevel){
	}
	
	
	@Override
	public List<MESBasicProdLevel> getProdLevels(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdLevel.class);
		if(params!=null){
			...
			return prodLevelDao.findByHql("from " + MESBasicProdLevel.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return prodLevelDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicProdLevel> findProdLevelsByHql(String hql, Object... objects){
		...
		return   prodLevelDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicProdLevel loadProdLevelByBussinessKey(MESBasicProdLevel prodLevel){
		return loadProdLevelByBussinessKey(prodLevel.getId());
	}
	
	@Override
	public Page<MESBasicProdLevel> getProdLevels(Page<MESBasicProdLevel> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdLevel.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = prodLevelDao.findByPage(page, sql);
			}else{
				page = prodLevelDao.findAllByPage(page);
			}
			return page;
		}
		page = prodLevelDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdLevel> findByTreeDataGridProduct(List<MESBasicProdLevel> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicProdLevel prodLevel : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduct(Page<MESBasicProdLevel> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicProdLevel> prodLevels = dgPage.getResult();
			for(MESBasicProdLevel prodLevel : prodLevels){
				
			}
			dgPage.setResult(prodLevels);
		}
	...
	public MESBasicProdLevel loadProdLevelByBussinessKey(Serializable bussinessKey){
		return prodLevelDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdLevel> findByProperty(String propertyName, Object object){
		return prodLevelDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdLevel findEntityByProperty(String propertyName, Object object){
		return prodLevelDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicProdLevel.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = prodLevelDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_product_ProdLevel","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicProdLevel prodLevel,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,prodLevelDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicProdLevel.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, prodLevel);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",prodLevel.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdLevel.TABLE_NAME +" where valid = 1";
			list = prodLevelDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdLevel.TABLE_NAME;
			list = prodLevelDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicProdLevel.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = prodLevelDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicProdLevelDaoImpl prodLevelDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProdLevel(final List<MESBasicProdLevel> insertObjs, final List<MESBasicProdLevel> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProdLevel(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProdLevel(final List<MESBasicProdLevel> insertObjs, final List<MESBasicProdLevel> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1470193147949"),importCount));
		...
		Map<Long, MESBasicProdLevel> oldUpdateObjsMap = new HashMap<Long, MESBasicProdLevel>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicProdLevel bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicProdLevel> bizObjList = prodLevelDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicProdLevel bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicProdLevel> nmeObjs = new ArrayList<MESBasicProdLevel>();
		...
		long id=autoGeneratorID.getNextId(MESBasicProdLevel.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicProdLevel prodLevel:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicProdLevel.TABLE_NAME,1,dbId);
			if(prodLevel.getId() != null ){
				m.put(prodLevel.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) prodLevelDao.createNativeQuery(sql, "MESBasic_1_product_ProdLevel"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		prodLevelDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicProdLevel.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicProdLevel prodLevel = updateObjs.get(updateIndex);
						Long id = prodLevel.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = prodLevel.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicProdLevel.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicProdLevel.TABLE_NAME + " order by id desc";
//					Query query = prodLevelDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicProdLevel prodLevel = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = prodLevel.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		prodLevelDaoImpl.getSession().flush();
		prodLevelDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicProdLevel bizObj = insertObjs.get(insertIndex);
			prodLevelImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicProdLevel bizObj = updateObjs.get(updateIndex);
			MESBasicProdLevel oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			prodLevelImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicProdLevel> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicProdLevel> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) prodLevelDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicProdLevel> result = getResult(page, query, new PendingResultTransformer(MESBasicProdLevel.class,prodLevelDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicProdLevel model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = prodLevelDao.createNativeQuery(sql, "MESBasic_1_product_ProdLevel").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = prodLevelDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = prodLevelDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = prodLevelDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?prodLevelDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  prodLevelDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,prodLevelDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="ec.flow.save")
	public void saveProdLevel(MESBasicProdLevel prodLevel, DataGridService dataGridService){
		saveProdLevel(prodLevel, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="ec.flow.save")
	public void saveProdLevel(MESBasicProdLevel prodLevel, DataGridService dataGridService, String viewCode){
		saveProdLevel(prodLevel, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel")
	public void saveProdLevel(MESBasicProdLevel prodLevel, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProdLevel(prodLevel, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel")
	public void saveProdLevel(MESBasicProdLevel prodLevel, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(null != prodLevel.getId() && prodLevel.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdLevel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel")
	public Map<Object, Object> prodLevelDataGridImport(MESBasicProdLevel prodLevel, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = prodLevel.getVirtualId();
		
		prodLevel = prodLevelDao.load(prodLevel.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, prodLevel.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = prodLevel.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(prodLevel);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, prodLevel.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel")
	public void mergeProdLevel(MESBasicProdLevel prodLevel, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(prodLevel);
		beforeSaveProdLevel(prodLevel);
		prodLevelDao.merge(prodLevel);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdLevel",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicProdLevel> getByPage(Page<MESBasicProdLevel> page,DetachedCriteria detachedCriteria){
		return prodLevelDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProdLevel(MESBasicProdLevel prodLevel, Object... objects){
		checkUniqueConstraint(prodLevel);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProdLevel(MESBasicProdLevel prodLevel, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProdLevel(MESBasicProdLevel prodLevel){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProdLevel(MESBasicProdLevel prodLevel){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProdLevel(MESBasicProdLevel prodLevel){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProdLevel(MESBasicProdLevel prodLevel){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicProdLevel prodLevel, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicProdLevel prodLevel, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProdLevel(List<MESBasicProdLevel> insertObjs, List<MESBasicProdLevel> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProdLevel,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProdLevel(List<MESBasicProdLevel> insertObjs, List<MESBasicProdLevel> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProdLevel,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_product_ProdLevel,MESBasic_1) */
// 自定义代码
