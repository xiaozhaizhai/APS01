...
@Repository
public class MESBasicWareClassDaoImpl extends TreeDaoImpl<MESBasicWareClass, Long> implements MESBasicWareClassDao {
	@Override
	public void saveDealInfo(MESBasicWareClassDealInfo dealInfo){
	...
	@Override
	public MESBasicWareClassDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicWareClassDealInfo dealInfo = new MESBasicWareClassDealInfo();
	...
	@Override
	public MESBasicWareClassDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicWareClass mainObj, Staff dealStaff) {
		MESBasicWareClassDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicWareClassDealInfo.class);
		...
		List<MESBasicWareClassDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_wareClass_WareClass,MESBasic_1) */
// 自定义代码
