...
@Repository
public class MESBasicItemSetupDaoImpl extends ExtendedGenericDaoImpl<MESBasicItemSetup, Long> implements MESBasicItemSetupDao {
	@Override
	...
	@Override
	public MESBasicItemSetupDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicItemSetupDealInfo dealInfo = new MESBasicItemSetupDealInfo();
	...
	@Override
	public MESBasicItemSetupDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicItemSetup mainObj, Staff dealStaff) {
		MESBasicItemSetupDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicItemSetupDealInfo.class);
		...
		List<MESBasicItemSetupDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_itemManage_ItemSetup,MESBasic_1) */
// 自定义代码
