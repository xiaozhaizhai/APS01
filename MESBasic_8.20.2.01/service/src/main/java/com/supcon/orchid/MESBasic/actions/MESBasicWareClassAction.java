...
/* CUSTOM CODE START(action,import,MESBasic_1_wareClass_WareClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicWareClassAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicWareClass> {
	...
	//导出excel时,主模型内容
	List<MESBasicWareClass> queryResults = new ArrayList<MESBasicWareClass>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/wareClass/wareClass/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/wareClass/wareClass/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_wareClass_WareClass" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicWareClass> wareClasss;
	
	public List<MESBasicWareClass> getWareClasss() {
		return wareClasss;
	}
	...
	private Page<MESBasicWareClass> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			wareClass = new MESBasicWareClass();
		} else {
			if(id > 0){
				wareClass = wareClassService.getWareClass(id, viewCode);
			}
		}
	...
	public Page<MESBasicWareClass> getPage() {
		if (null == page) {
			page = new Page<MESBasicWareClass>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/wareClass/wareClass/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = wareClassService.getWareClassAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/wareClass/wareClass/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String wareClassReferenceCopy() throws Exception{
			MESBasicWareClass oldWareClass = null;
			if(isEditNew()){
				...
				oldWareClass = new MESBasicWareClass();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,wareClass.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, wareClass.getId());
				...
				oldWareClass = wareClassService.getWareClass(id);
			}
			
			
			wareClass = wareClassService.getWareClass(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_wareClass_WareClass");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicWareClass.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldWareClass, null);
						Method setValue = MESBasicWareClass.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(wareClass, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			wareClass.setCreateDepartmentId(oldWareClass.getCreateDepartmentId());
			wareClass.setCreatePositionId(oldWareClass.getCreatePositionId());
			wareClass.setCreateStaffId(oldWareClass.getCreateStaffId());
			wareClass.setOwnerDepartmentId(oldWareClass.getOwnerDepartmentId());
			wareClass.setOwnerPositionId(oldWareClass.getOwnerPositionId());
			wareClass.setOwnerStaffId(oldWareClass.getOwnerStaffId());
			wareClass.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_wareClass_WareClass", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=wareClassService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return wareClassReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			wareClass = wareClassService.getWareClass(this.wareClass.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,wareClass.getId().toString());
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/wareClass/wareClass/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = wareClassService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(wareClassService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(wareClassService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_wareClass_WareClass", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/wareClass/wareClass/classTreeRef", results = { @Result(type = FREEMARKER, location = "classTreeRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"wareClass\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"wareClass\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"wareClass\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"wareClass\".VALID = ?")>0){
	...
	@Action(value="/MESBasic/wareClass/wareClass/classTreeRefDrag")
	})
	public String viewTreeDrag() throws Exception{
		wareClassService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/MESBasic/wareClass/wareClass/classTreeRefSort")
	})
	...
	@Action(value="/MESBasic/wareClass/wareClass/classTreeRefData",
			results = { @Result(type = JSON, params = { "root", "wareClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"wareClass\".", "{alias}.");
					}
		...
		wareClasss = wareClassService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/MESBasic/wareClass/wareClass/classTreeRefFullData",
			results = { @Result(type = JSON, params = { "root", "wareClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			wareClasss = wareClassService.getTreeInfo(treeInfo, null, null);
		}
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		wareClassService.findDataGridPage(dg,dgclass,getDgPage(),wareClass,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(wareClass.getId() != null && wareClass.getId() != -1){
				wareClass = wareClassService.getWareClass(wareClass.getId());
			}
			wareClassService.findDataGridPage(dg,dgclass,getDgPage(),wareClass,searchCondition,params);
	...
	private void save() throws Exception {
		...
		wareClassService.saveWareClass(wareClass,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "submit");
		bap_validate_datagrids=wareClassService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", wareClass.getId());
		...
		fileuploads.put("linkId", wareClass.getId());
		bapAfterCustomMethod("MESBasic_1_wareClass_WareClass", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/wareClass/wareClass/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		wareClassService.deleteWareClass(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_wareClass_WareClass", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/wareClass/wareClass/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "restore");
		wareClassService.restoreWareClass(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_wareClass_WareClass", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/wareClass/wareClass/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/wareClass/wareClass/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "showDealInfoData");
		if (null != tableInfoId) {
			int count=wareClassService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = wareClassService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = wareClassService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_wareClass_WareClass", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicWareClass> findExportDataInfos(Page<MESBasicWareClass> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = wareClassService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = wareClassService.getAssProperty(p.getCode());	
									//String assModelCode = wareClassService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			wareClassService.findWareClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			wareClassService.findWareClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicWareClass> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicWareClass result : results){
	...
	public boolean isEditNew(){
		//return null == this.wareClass || null == this.wareClass.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/wareClass/wareClass/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_wareClass_WareClass", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/wareClass/wareClass/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/wareClass/wareClass/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/wareClass/wareClass/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicWareClassService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_wareClass", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_wareClass");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_wareClass_WareClass", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/wareClass/wareClass/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_wareClass_WareClass", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_wareClass_WareClass", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/wareClass/wareClass/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/wareClass/wareClass/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_wareClass_WareClass";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_wareClass_WareClass", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/wareClass/wareClass/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicWareClass wareClass = null;
		if(null != id) {
			wareClass = wareClassService.getWareClass(id);
		}
		...
		_auditLog.setColumnStringB(null == wareClass.getCode() ? "" : wareClass.getCode().toString());
		_auditLog.setColumnStringA(null == wareClass.getName() ? "" : wareClass.getName().toString());
		_auditLog.setColumnStringC(wareClass.getId().toString());
		_auditLog.setColumnStringD(MESBasicWareClass.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_wareClass");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicWareClassAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, wareClass,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, wareClass,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,wareClass);
		...
		printlog.setEntity("MESBasic_1_wareClass");
		...
		printlog.setBusinessId(wareClass.getId());
		printlog.setBussinessKey(null == wareClass.getCode() ? "" : wareClass.getCode().toString());
		printlog.setMainDisplay(null == wareClass.getName() ? "" : wareClass.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/wareClass/wareClass/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(wareClass,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/wareClass/wareClass/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(wareClassService.getWareClass(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/wareClass/wareClass/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/wareClass/wareClass/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, wareClass,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				wareClass = wareClassService.getWareClass(Long.parseLong(arrId[i]));
				if(null != wareClass){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == wareClass.getName() ? "" : "," + wareClass.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == wareClass.getName() ? "" : wareClass.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == wareClass.getCode() ? "" : "," + wareClass.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == wareClass.getCode() ? "" : wareClass.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == wareClass.getId() ? "" : "," + wareClass.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == wareClass.getId() ? "" : wareClass.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, wareClass,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,wareClass);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/wareClass/wareClass/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicWareClass> findExportPageInfos(Page<MESBasicWareClass> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_wareClass" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = wareClassService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicWareClass queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = wareClassService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = wareClassService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_wareClass_WareClass");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_wareClass_WareClass")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_wareClass_WareClass");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_wareClass_WareClass");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = wareClassService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_wareClass_WareClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_wareClass_WareClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_wareClass_WareClass,MESBasic_1) */
// 自定义代码
