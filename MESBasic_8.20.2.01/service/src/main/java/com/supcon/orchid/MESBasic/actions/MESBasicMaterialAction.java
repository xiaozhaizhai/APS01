...
/* CUSTOM CODE START(action,import,MESBasic_1_material_Material,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicMaterialAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicMaterial> {
	...
	private MESBasicMaterial material;
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			...
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_material_Material" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicMaterial> materials;
	
	public List<MESBasicMaterial> getMaterials() {
		return materials;
	}
	...
	private Page<MESBasicMaterial> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			material = new MESBasicMaterial();
		} else {
			if(id > 0){
				material = materialService.getMaterial(id, viewCode);
			}
		}
	...
	public Page<MESBasicMaterial> getPage() {
		if (null == page) {
			page = new Page<MESBasicMaterial>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicMaterial getMaterial() {
		return material;
	}		

	public void setMaterial(MESBasicMaterial material) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/material/material/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = materialService.getMaterialAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/material/material/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.radion1415321000682
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String materialReferenceCopy() throws Exception{
			MESBasicMaterial oldMaterial = null;
			if(isEditNew()){
				...
				oldMaterial = new MESBasicMaterial();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,material.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, material.getId());
				...
				oldMaterial = materialService.getMaterial(id);
			}
			
			
			material = materialService.getMaterial(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_material_Material");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicMaterial.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMaterial, null);
						Method setValue = MESBasicMaterial.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(material, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			material.setCreateDepartmentId(oldMaterial.getCreateDepartmentId());
			material.setCreatePositionId(oldMaterial.getCreatePositionId());
			material.setCreateStaffId(oldMaterial.getCreateStaffId());
			material.setOwnerDepartmentId(oldMaterial.getOwnerDepartmentId());
			material.setOwnerPositionId(oldMaterial.getOwnerPositionId());
			material.setOwnerStaffId(oldMaterial.getOwnerStaffId());
			bapAfterCustomMethod("MESBasic_1_material_Material", "referenceCopy");
			return SUCCESS;
		}
	// ######## MESBasic_1_material_materialEdit start
	/**
	 * MESBasic.viewtitle.radion1415321085000
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/material/material/materialEdit",  interceptorRefs={@InterceptorRef(value="orchidFullStack")}, results = { @Result(type = FREEMARKER, location = "materialEdit.ftl") })
	public String materialEdit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_material_materialEdit", "materialEdit");
		if (null != entityCode) {
		...
		bapAfterCustomMethod("MESBasic_1_material_materialEdit", "materialEdit");
		return SUCCESS;
	}
	
	// ######## MESBasic_1_material_materialEdit end
	// ######## MESBasic_1_material_materialList start
	/**
	 * MESBasic.viewtitle.radion1415321151720
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/material/material/materialList", results = { @Result(type = FREEMARKER, location = "materialList.ftl") })
	public String materialList() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_material_materialList", "materialList");
		...
		bapAfterCustomMethod("MESBasic_1_material_materialList", "materialList");
		return SUCCESS;
	}
	
	// ######## MESBasic_1_material_materialList end
	// ######## MESBasic_1_material_materialRef start
	/**
	 * MESBasic.viewtitle.radion1415322277102
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/material/material/materialRef", results = { @Result(type = FREEMARKER, location = "materialRef.ftl") })
	public String materialRef() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_material_materialRef", "materialRef");
		bapAfterCustomMethod("MESBasic_1_material_materialRef", "materialRef");
		return SUCCESS;
	}
	
	// ######## MESBasic_1_material_materialRef end
	// ######## MESBasic_1_material_materialView start
	/**
	 * MESBasic.viewtitle.radion1415321126792
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/material/material/materialView",  interceptorRefs={@InterceptorRef(value="orchidFullStack")}, results = { @Result(type = FREEMARKER, location = "materialView.ftl") })
	public String materialView() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_material_materialView", "materialView");
			if(null!=entityCode){
				businessCenterList = businessCenterService.getBusinessCenters(entityCode);
				for(BusinessCenter bc:businessCenterList){
					//String url=businessCenterService.getBusinessUrl(bc,material.getId().toString());
		...
		bapAfterCustomMethod("MESBasic_1_material_materialView", "materialView");
		return SUCCESS;
	}
	
	// ######## MESBasic_1_material_materialView end
	...
	@Action(value = "/MESBasic/material/material/materialList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.status,result.tableNo,result.code,result.name,result.remark,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion"})  
			
			,@Result(name="excel",type="excel",location="materialList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	public String materialListQuery() throws Exception{
		viewCode = "MESBasic_1_material_materialList";
		params = new ArrayList<Param>();
		params.add(new Param("\"material\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		bapBeforeCustomMethod(viewCode, "materialListQuery");
		setFieldPermissionModelCode("MESBasic_1_material_Material");
		...
		if (null != getDataTableSortColOrder() && getDataTableSortColOrder().length() > 0) {
			params.add(new Param("dataTable-sortColOrder", getDataTableSortColOrder()));
					...
					Param p = new Param("\"material\".CODE",request.getParameter("code"),Param.LIKE_ALL);
					params.add(p);
					}
				if(null != request.getParameter("name") && request.getParameter("name").trim().length() > 0) {
					Param p = new Param("\"material\".NAME",request.getParameter("name"),Param.LIKE_ALL);
					params.add(p);
					}
				if(null != request.getParameter("remark") && request.getParameter("remark").trim().length() > 0) {
					Param p = new Param("\"material\".REMARK",request.getParameter("remark"),Param.LIKE_ALL);
					params.add(p);
					}
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"material\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			bapAfterCustomMethod(viewCode, "materialListQuery");
			return "excel";
		} else {
		...
		@Action(value = "/MESBasic/material/material/materialRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.status,result.tableNo,result.code,result.name,result.remark"})
			,@Result(name="excel",type="excel",location="materialRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	public String materialRefQuery() throws Exception{
		viewCode = "MESBasic_1_material_materialRef";
		params = new ArrayList<Param>();
		...
		params.add(new Param("\"material\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		bapBeforeCustomMethod(viewCode, "materialRefQuery");
		setFieldPermissionModelCode("MESBasic_1_material_Material");
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		if (null != request.getParameter("advQueryCond") && request.getParameter("advQueryCond").trim().length() > 0) {
			params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
		...
		if(getPage().isExportFlag()) {
			bapAfterCustomMethod(viewCode, "materialRefQuery");
			return "excel";
		} else {
			findExportDataInfos(getPage());
			bapAfterCustomMethod(viewCode, "materialRefQuery");
	...
	private void save(DataGridService dataGridService) throws Exception {
		...
		materialService.saveMaterial(material, dataGridService, viewCode);
	...
	@Actions({ 
		@Action(value = "/MESBasic/material/material/materialEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/material/material/materialView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_material_Material", "submit");
		DataGridService dataGridService = null;
		
			save(dataGridService);
			responseMap.put("id", material.getId());

		Map<String, Object> fileuploads = (Map<String, Object>) request.getAttribute("__orchid_fileuploads__");
		fileuploads.put("linkId", material.getId());
		bapAfterCustomMethod("MESBasic_1_material_Material", "submit");
		return SUCCESS;
	}
	@Action(value = "/MESBasic/material/material/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_material_Material", "delete");
		materialService.deleteMaterial(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_material_Material", "delete");
		return SUCCESS;
	}
	
	
	@Actions({
			@Action(value = "/MESBasic/material/material/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/material/material/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_material_Material", "showDealInfoData");
		if (null != tableInfoId) {
			int count=materialService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = materialService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = materialService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_material_Material", "showDealInfoData");
		return SUCCESS;
	}	

	@Override
	public Page<MESBasicMaterial> findExportDataInfos(Page<MESBasicMaterial> page) {
		if(null != searchObjects && searchObjects.length > 0) {
			materialService.findMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, searchObjects);
		} else {
			materialService.findMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode);
		}
		return getPage();
	}

	public boolean isEditNew(){
		//return null == this.material || null == this.material.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/material/material/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_material_Material", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_material_Material", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/material/material/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_material_Material", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/material/material/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/material/material/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicMaterialService"
		};
		bapBeforeCustomMethod("MESBasic_1_material_Material", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_material_Material", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/material/material/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				xlsResult = importService.imports(viewCode, "MESBasic_1_material_Material", dataFile, time, name[0], isReplace, isIgnore, importServices);
	...
	@Action(value = "/MESBasic/material/material/downLoad", results = { @Result(type = "stream", params = {
  ...
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_material_Material", "downLoadXls");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/material/material/templateDownLoad", results = { @Result(type = "stream", params = {
			"contentType", "application/excel", "inputName", "inputStream",
			"contentDisposition", 
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String templateDownLoad() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_material_Material", "templateDownLoad");
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/material/material/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		MESBasicMaterial material = null;
		if(null != id) {
			material = materialService.getMaterial(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, material, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, material, null, null, true, this);
	...
	@Action(value = "/MESBasic/material/material/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/material/material/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				material = materialService.getMaterial(Integer.parseInt(id));
				if(material != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, material, null, null, true, this);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_material_Material,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void bapAfterCustomMethod(String code, String methodName) throws Exception {
	/* CUSTOM CODE START(action,methods,MESBasic_1_material_Material,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_material_Material,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
}