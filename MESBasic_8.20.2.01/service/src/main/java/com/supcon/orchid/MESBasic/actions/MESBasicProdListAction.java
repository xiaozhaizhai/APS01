...
/* CUSTOM CODE START(action,import,MESBasic_1_prodList_ProdList,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicProdListAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicProdList> {
	...
	private MESBasicProdList prodList;
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/prodList/prodList/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/prodList/prodList/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_prodList_ProdList" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicProdList> prodLists;
	
	public List<MESBasicProdList> getProdLists() {
		return prodLists;
	}
	...
	private Page<MESBasicProdList> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			prodList = new MESBasicProdList();
		} else {
			if(id > 0){
				prodList = prodListService.getProdList(id, viewCode);
			}
		}
	...
	public Page<MESBasicProdList> getPage() {
		if (null == page) {
			page = new Page<MESBasicProdList>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicProdList getProdList() {
		return prodList;
	}		

	public void setProdList(MESBasicProdList prodList) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/prodList/prodList/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prodListService.getProdListAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/prodList/prodList/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1437386364907
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String prodListReferenceCopy() throws Exception{
			MESBasicProdList oldProdList = null;
			if(isEditNew()){
				...
				oldProdList = new MESBasicProdList();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodList.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prodList.getId());
				...
				oldProdList = prodListService.getProdList(id);
			}
			
			
			prodList = prodListService.getProdList(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_prodList_ProdList");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicProdList.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProdList, null);
						Method setValue = MESBasicProdList.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prodList, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prodList.setCreateDepartmentId(oldProdList.getCreateDepartmentId());
			prodList.setCreatePositionId(oldProdList.getCreatePositionId());
			prodList.setCreateStaffId(oldProdList.getCreateStaffId());
			prodList.setOwnerDepartmentId(oldProdList.getOwnerDepartmentId());
			prodList.setOwnerPositionId(oldProdList.getOwnerPositionId());
			prodList.setOwnerStaffId(oldProdList.getOwnerStaffId());
			bapAfterCustomMethod("MESBasic_1_prodList_ProdList", "referenceCopy");
			return SUCCESS;
		}
	...
	private void save(DataGridService dataGridService) throws Exception {
		if(null == prodList.getId()) {
			responseMap.put("operate", "add");
		} else {
			responseMap.put("operate", "edit");
		}
		prodListService.saveProdList(prodList, dataGridService, viewCode);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_prodList_ProdList", "submit");
		DataGridService dataGridService = null;
		
			save(dataGridService);
			responseMap.put("id", prodList.getId());

		Map<String, Object> fileuploads = (Map<String, Object>) request.getAttribute("__orchid_fileuploads__");
		fileuploads.put("linkId", prodList.getId());
		bapAfterCustomMethod("MESBasic_1_prodList_ProdList", "submit");
		return SUCCESS;
	}
	@Action(value = "/MESBasic/prodList/prodList/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_ProdList", "delete");
		prodListService.deleteProdList(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_prodList_ProdList", "delete");
		return SUCCESS;
	}
	
	
	@Actions({
			@Action(value = "/MESBasic/prodList/prodList/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/prodList/prodList/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_ProdList", "showDealInfoData");
		if (null != tableInfoId) {
			int count=prodListService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = prodListService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = prodListService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_prodList_ProdList", "showDealInfoData");
		return SUCCESS;
	}	

	@Override
	public Page<MESBasicProdList> findExportDataInfos(Page<MESBasicProdList> page) {
		if(null != searchObjects && searchObjects.length > 0) {
			prodListService.findProdLists(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, searchObjects);
		} else {
			prodListService.findProdLists(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode);
		}
		return getPage();
	}

	public boolean isEditNew(){
		//return null == this.prodList || null == this.prodList.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/prodList/prodList/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_ProdList", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_prodList_ProdList", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/prodList/prodList/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_prodList_ProdList", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/prodList/prodList/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodList/prodList/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProdListService"
		};
		bapBeforeCustomMethod("MESBasic_1_prodList_ProdList", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_prodList_ProdList", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodList/prodList/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				xlsResult = importService.imports(viewCode, "MESBasic_1_prodList_ProdList", dataFile, time, name[0], isReplace, isIgnore, importServices);
	...
	@Action(value = "/MESBasic/prodList/prodList/downLoad", results = { @Result(type = "stream", params = {
  ...
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_ProdList", "downLoadXls");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodList/prodList/templateDownLoad", results = { @Result(type = "stream", params = {
			"contentType", "application/excel", "inputName", "inputStream",
			"contentDisposition", 
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String templateDownLoad() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_ProdList", "templateDownLoad");
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/prodList/prodList/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		MESBasicProdList prodList = null;
		if(null != id) {
			prodList = prodListService.getProdList(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, prodList, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, prodList, null, null, true, this);
	...
	@Action(value = "/MESBasic/prodList/prodList/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/prodList/prodList/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				prodList = prodListService.getProdList(Integer.parseInt(id));
				if(prodList != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, prodList, null, null, true, this);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_prodList_ProdList,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_prodList_ProdList,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_prodList_ProdList,MESBasic_1) */
// 自定义代码
