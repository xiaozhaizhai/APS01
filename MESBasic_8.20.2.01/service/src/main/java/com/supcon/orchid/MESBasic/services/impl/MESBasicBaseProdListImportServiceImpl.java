...
@Service("mESBasicBaseProdListImportService")
public class MESBasicBaseProdListImportServiceImpl implements MESBasicBaseProdListImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_prodList",model="MESBasic_1_prodList_BaseProdList",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicBaseProdList bizObj, MESBasicBaseProdList oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1450077412035") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getProdListName() ? "" : "," + oldBizObj.getProdListName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getProdListName() ? "" : oldBizObj.getProdListName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getProdListCode() ? "" : "," + oldBizObj.getProdListCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getProdListCode() ? "" : oldBizObj.getProdListCode().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1450077412035") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getProdListName() ? "" : "," + bizObj.getProdListName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getProdListName() ? "" : bizObj.getProdListName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getProdListCode() ? "" : "," + bizObj.getProdListCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getProdListCode() ? "" : bizObj.getProdListCode().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicBaseProdList bizObj, String bussinesskeyFieldName,
