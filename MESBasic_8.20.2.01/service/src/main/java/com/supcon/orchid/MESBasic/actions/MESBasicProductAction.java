...
/* CUSTOM CODE START(action,import,MESBasic_1_product_Product,MESBasic_1) */
// 自定义代码
import com.supcon.orchid.MESBasic.services.MESBasicFactoryModelService;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicProductAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicProduct> {
	...
	//导出excel时,主模型内容
	List<MESBasicProduct> queryResults = new ArrayList<MESBasicProduct>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/product/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/product/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_product_Product" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicProduct> products;
	
	public List<MESBasicProduct> getProducts() {
		return products;
	}
	...
	private Page<MESBasicProduct> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			product = new MESBasicProduct();
			if(product.getApproachUnit() != null) {
				product.setApproachUnit((SystemCode) systemCodeService.getSystemCode(product.getApproachUnit().getId()));
			}
			if(product.getAutoSupply() != null) {
				product.setAutoSupply((SystemCode) systemCodeService.getSystemCode(product.getAutoSupply().getId()));
			}
			if(product.getBapValueType() != null) {
				product.setBapValueType((SystemCode) systemCodeService.getSystemCode(product.getBapValueType().getId()));
			}
			if(product.getBatch() != null) {
				product.setBatch((SystemCode) systemCodeService.getSystemCode(product.getBatch().getId()));
			}
			if(product.getCheckMethodbap() != null) {
				product.setCheckMethodbap((SystemCode) systemCodeService.getSystemCode(product.getCheckMethodbap().getId()));
			}
			if(product.getInterfaceUrl() != null) {
				product.setInterfaceUrl((SystemCode) systemCodeService.getSystemCode(product.getInterfaceUrl().getId()));
			}
			if(product.getKeepReckon() != null) {
				product.setKeepReckon((SystemCode) systemCodeService.getSystemCode(product.getKeepReckon().getId()));
			}
			if(product.getKeepUnit() != null) {
				product.setKeepUnit((SystemCode) systemCodeService.getSystemCode(product.getKeepUnit().getId()));
			}
			if(product.getMfgStockMode() != null) {
				product.setMfgStockMode((SystemCode) systemCodeService.getSystemCode(product.getMfgStockMode().getId()));
			}
			if(product.getReqType() != null) {
				product.setReqType((SystemCode) systemCodeService.getSystemCode(product.getReqType().getId()));
			}
			if(product.getResetMark() != null) {
				product.setResetMark((SystemCode) systemCodeService.getSystemCode(product.getResetMark().getId()));
			}
			if(product.getStoreDeadlineUnit() != null) {
				product.setStoreDeadlineUnit((SystemCode) systemCodeService.getSystemCode(product.getStoreDeadlineUnit().getId()));
			}
		} else {
			if(id > 0){
				product = productService.getProduct(id, viewCode);
			}
		}
	...
	public Page<MESBasicProduct> getPage() {
		if (null == page) {
			page = new Page<MESBasicProduct>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/product/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = productService.getProductAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/product/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String productReferenceCopy() throws Exception{
			MESBasicProduct oldProduct = null;
			if(isEditNew()){
				...
				oldProduct = new MESBasicProduct();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,product.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, product.getId());
				...
				oldProduct = productService.getProduct(id);
			}
			
			
			product = productService.getProduct(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_product_Product");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicProduct.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProduct, null);
						Method setValue = MESBasicProduct.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(product, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			product.setCreateDepartmentId(oldProduct.getCreateDepartmentId());
			product.setCreatePositionId(oldProduct.getCreatePositionId());
			product.setCreateStaffId(oldProduct.getCreateStaffId());
			product.setOwnerDepartmentId(oldProduct.getOwnerDepartmentId());
			product.setOwnerPositionId(oldProduct.getOwnerPositionId());
			product.setOwnerStaffId(oldProduct.getOwnerStaffId());
			product.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_product_Product", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/product/prodEditforJS",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prodEditforJS.ftl") }),
		@Action(value = "/MESBasic/product/product/prodEditforJSPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prodEditforJSPrint.ftl")})
		,
		@Action(value = "/MESBasic/product/product/prodEditnew",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prodEditnew.ftl") }),
		@Action(value = "/MESBasic/product/product/prodEditnewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "prodEditnewPrint.ftl")})
		,
		@Action(value = "/MESBasic/product/product/productEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "productEdit.ftl") }),
		@Action(value = "/MESBasic/product/product/productEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "productEditPrint.ftl")})
		,
		@Action(value = "/MESBasic/product/product/productEditnew",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "productEditnew.ftl") }),
		@Action(value = "/MESBasic/product/product/productEditnewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "productEditnewPrint.ftl")})
		,
		@Action(value = "/MESBasic/product/product/resetDailyEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "resetDailyEdit.ftl") }),
		@Action(value = "/MESBasic/product/product/resetDailyEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "resetDailyEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=productService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return productReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			product = productService.getProduct(this.product.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,product.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/product/productView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "productView.ftl") }),
		@Action(value = "/MESBasic/product/product/productViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "productViewPrint.ftl")})
		,
		@Action(value = "/MESBasic/product/product/productViewnew",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "productViewnew.ftl") }),
		@Action(value = "/MESBasic/product/product/productViewnewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "productViewnewPrint.ftl")})
		,
		@Action(value = "/MESBasic/product/product/resetDailyView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "resetDailyView.ftl") }),
		@Action(value = "/MESBasic/product/product/resetDailyViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "resetDailyViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/product/productList", results = { @Result(type = FREEMARKER, location = "productList.ftl") })
		,
		@Action(value = "/MESBasic/product/product/productPart", results = { @Result(type = FREEMARKER, location = "productPart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/product/productLayout", results = { @Result(type = FREEMARKER, location = "productLayout.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct_batch", results = { @Result(type = FREEMARKER, location = "refProduct_batch.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct_BOM", results = { @Result(type = FREEMARKER, location = "refProduct_BOM.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductFiltLayout", results = { @Result(type = FREEMARKER, location = "refProductFiltLayout.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductLayout", results = { @Result(type = FREEMARKER, location = "refProductLayout.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductLayout_kc", results = { @Result(type = FREEMARKER, location = "refProductLayout_kc.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductLayout_sp", results = { @Result(type = FREEMARKER, location = "refProductLayout_sp.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductLayoutAC", results = { @Result(type = FREEMARKER, location = "refProductLayoutAC.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductPcTpLayout", results = { @Result(type = FREEMARKER, location = "refProductPcTpLayout.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/product/product/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = productService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(productService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(productService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_product_Product", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/product/customRef", results = { @Result(type = FREEMARKER, location = "customRef.ftl") })
		,
		@Action(value = "/MESBasic/product/product/productRef", results = { @Result(type = FREEMARKER, location = "productRef.ftl") })
		,
		@Action(value = "/MESBasic/product/product/productRefNew", results = { @Result(type = FREEMARKER, location = "productRefNew.ftl") })
		,
		@Action(value = "/MESBasic/product/product/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct", results = { @Result(type = FREEMARKER, location = "refProduct.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct_fiter", results = { @Result(type = FREEMARKER, location = "refProduct_fiter.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct_kc", results = { @Result(type = FREEMARKER, location = "refProduct_kc.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct_pactype", results = { @Result(type = FREEMARKER, location = "refProduct_pactype.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct_pd_batch", results = { @Result(type = FREEMARKER, location = "refProduct_pd_batch.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct_sp", results = { @Result(type = FREEMARKER, location = "refProduct_sp.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProduct2BOM", results = { @Result(type = FREEMARKER, location = "refProduct2BOM.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductAC", results = { @Result(type = FREEMARKER, location = "refProductAC.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductPart", results = { @Result(type = FREEMARKER, location = "refProductPart.ftl") })
		,
		@Action(value = "/MESBasic/product/product/refProductPart2", results = { @Result(type = FREEMARKER, location = "refProductPart2.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/product/productList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productCode,result.productName,result.productModel,result.productSpecif,result.productCostPrice,result.unitGroupID.name,result.unitGroupID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="productList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/productPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productCode,result.productName,result.productListID.prodListName,result.productSpecif,result.productModel,result.createTime,result.modifyTime,result.productListID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="productPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"product\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"product\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"factory\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		if (null != request.getParameter("baseProdList.layRec") && request.getParameter("baseProdList.layRec").trim().length() > 0) {
			params.add(new Param("\"productListID\".PRODLIST_LAYREC", request.getParameter("baseProdList.layRec"), Param.EQUAL_LIKELEFT));	
		}
		if (null != request.getParameter("storeSet.layRec") && request.getParameter("storeSet.layRec").trim().length() > 0) {
			params.add(new Param("\"storeSet\".LAY_REC", request.getParameter("storeSet.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"product\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"product\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/product/productList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/MESBasic/product/product/productPart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/product/customRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productCode,result.productName,result.productModel,result.productSpecif,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="customRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/productRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productSpecif,result.productModel,result.unitGroup.name,result.batch.id,result.batch.value,result.productBeSale,result.productBePropCheck,result.unitGroup.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="productRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/productRefNew-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productSpecif,result.productModel,result.unitGroup.name,result.batch.id,result.batch.value,result.productBeSale,result.productBePropCheck,result.unitGroup.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="productRefNew-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productCode,result.productName,result.productModel,result.productSpecif,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="ref-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProduct-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productListID.prodListName,result.productSpecif,result.productModel,result.productBaseUnit.name,result.unitGroup.name,result.productCostPrice,result.modifyTime,result.isNeedWeigh,result.goodState,result.goodStateForDisplay,result.batch.id,result.batch.value,result.limsUnit.name,result.productListID.id,result.productBaseUnit.id,result.unitGroup.id,result.limsUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProduct-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProduct_fiter-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productListID.prodListName,result.productSpecif,result.productModel,result.productBaseUnit.name,result.unitGroup.name,result.productCostPrice,result.modifyTime,result.isNeedWeigh,result.goodState,result.goodStateForDisplay,result.batch.id,result.batch.value,result.limsUnit.name,result.pieceNum,result.productListID.id,result.productBaseUnit.id,result.unitGroup.id,result.limsUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProduct_fiter-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProduct_kc-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productListID.prodListName,result.productSpecif,result.productModel,result.productBaseUnit.name,result.unitGroup.name,result.productCostPrice,result.modifyTime,result.isNeedWeigh,result.goodState,result.goodStateForDisplay,result.batch.id,result.batch.value,result.limsUnit.name,result.productBePropCheck,result.productListID.id,result.productBaseUnit.id,result.unitGroup.id,result.limsUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProduct_kc-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProduct_pactype-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productListID.prodListName,result.productSpecif,result.productModel,result.productBaseUnit.name,result.unitGroup.name,result.productCostPrice,result.productListID.modifyTime,result.isNeedWeigh,result.goodState,result.goodStateForDisplay,result.batch.id,result.batch.value,result.limsUnit.name,result.pieceNum,result.productListID.id,result.productBaseUnit.id,result.unitGroup.id,result.limsUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProduct_pactype-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProduct_pd_batch-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productListID.prodListName,result.productSpecif,result.productModel,result.productBaseUnit.name,result.productCostPrice,result.modifyTime,result.isNeedWeigh,result.isKeepDate,result.goodState,result.goodStateForDisplay,result.batch.id,result.batch.value,result.limsUnit.name,result.productListID.id,result.productBaseUnit.id,result.limsUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProduct_pd_batch-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProduct_sp-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productListID.prodListName,result.productSpecif,result.productModel,result.productBaseUnit.name,result.unitGroup.name,result.productCostPrice,result.modifyTime,result.isNeedWeigh,result.goodState,result.goodStateForDisplay,result.batch.id,result.batch.value,result.limsUnit.name,result.productListID.id,result.productBaseUnit.id,result.unitGroup.id,result.limsUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProduct_sp-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProduct2BOM-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productListID.prodListName,result.productSpecif,result.productModel,result.productBaseUnit.name,result.unitGroup.name,result.productCostPrice,result.modifyTime,result.isNeedWeigh,result.goodState,result.goodStateForDisplay,result.batch.id,result.batch.value,result.limsUnit.name,result.productListID.id,result.productBaseUnit.id,result.unitGroup.id,result.limsUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProduct2BOM-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProductAC-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.productListID.prodListName,result.productSpecif,result.productModel,result.productChartCode,result.productBaseUnit.name,result.productCostPrice,result.modifyTime,result.isNeedWeigh,result.goodState,result.goodStateForDisplay,result.batch.id,result.batch.value,result.limsUnit.name,result.productListID.id,result.productBaseUnit.id,result.limsUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProductAC-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProductPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.prodType,result.prodTypeForDisplay,result.productSpecif,result.productModel,result.modifyTime,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProductPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/product/product/refProductPart2-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.productName,result.productCode,result.prodType,result.prodTypeForDisplay,result.productSpecif,result.productModel,result.modifyTime,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProductPart2-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/product/product/allTempleteDownload", results = { @Result(name="excel",type="excel",location="productList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_product_Product");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		productService.findDataGridPage(dg,dgclass,getDgPage(),product,"", null);
	...
	private Page<MESBasicProdLevel> dg1572259458131Page;
	public Page<MESBasicProdLevel> getDg1572259458131Page(){
		if(null == dg1572259458131Page){
			dg1572259458131Page = new Page<MESBasicProdLevel>(1,20);
		}
		return dg1572259458131Page;
	}
	
	private Class dg1572259458131Clazz = MESBasicProdLevel.class;
	
	public Class getDg1572259458131Clazz() {
		return dg1572259458131Clazz;
	}
	
	private List<MESBasicProdLevel> dg1572259458131List;
	public List<MESBasicProdLevel> getDg1572259458131List(){
		if(null == dg1572259458131List){
			dg1572259458131List = new ArrayList<MESBasicProdLevel>();
		}
		return dg1572259458131List;
	}
	public void setDg1572259458131ListJson(String json) {
		//dg1572259458131List = (List<MESBasicProdLevel>) productService.generateObjectFromJson(json, MESBasicProdLevel.class);
	}
	
	private List<Long> dg1572259458131DeletedIds;
	public List<Long> getDg1572259458131DeletedIds(){
		if(null == dg1572259458131DeletedIds){
			dg1572259458131DeletedIds = new ArrayList<Long>();
		}
		return dg1572259458131DeletedIds;
	}

	private Page<MESBasicProdResetDaily> dg1572259458193Page;
	public Page<MESBasicProdResetDaily> getDg1572259458193Page(){
		if(null == dg1572259458193Page){
			dg1572259458193Page = new Page<MESBasicProdResetDaily>(1,20);
		}
		return dg1572259458193Page;
	}
	
	private Class dg1572259458193Clazz = MESBasicProdResetDaily.class;
	
	public Class getDg1572259458193Clazz() {
		return dg1572259458193Clazz;
	}
	
	private List<MESBasicProdResetDaily> dg1572259458193List;
	public List<MESBasicProdResetDaily> getDg1572259458193List(){
		if(null == dg1572259458193List){
			dg1572259458193List = new ArrayList<MESBasicProdResetDaily>();
		}
		return dg1572259458193List;
	}
	public void setDg1572259458193ListJson(String json) {
		//dg1572259458193List = (List<MESBasicProdResetDaily>) productService.generateObjectFromJson(json, MESBasicProdResetDaily.class);
	}
	
	private List<Long> dg1572259458193DeletedIds;
	public List<Long> getDg1572259458193DeletedIds(){
		if(null == dg1572259458193DeletedIds){
			dg1572259458193DeletedIds = new ArrayList<Long>();
		}
		return dg1572259458193DeletedIds;
	}

	private Page<MESBasicProdSecUnit> dg1572259458240Page;
	public Page<MESBasicProdSecUnit> getDg1572259458240Page(){
		if(null == dg1572259458240Page){
			dg1572259458240Page = new Page<MESBasicProdSecUnit>(1,20);
		}
		return dg1572259458240Page;
	}
	
	private Class dg1572259458240Clazz = MESBasicProdSecUnit.class;
	
	public Class getDg1572259458240Clazz() {
		return dg1572259458240Clazz;
	}
	
	private List<MESBasicProdSecUnit> dg1572259458240List;
	public List<MESBasicProdSecUnit> getDg1572259458240List(){
		if(null == dg1572259458240List){
			dg1572259458240List = new ArrayList<MESBasicProdSecUnit>();
		}
		return dg1572259458240List;
	}
	public void setDg1572259458240ListJson(String json) {
		//dg1572259458240List = (List<MESBasicProdSecUnit>) productService.generateObjectFromJson(json, MESBasicProdSecUnit.class);
	}
	
	private List<Long> dg1572259458240DeletedIds;
	public List<Long> getDg1572259458240DeletedIds(){
		if(null == dg1572259458240DeletedIds){
			dg1572259458240DeletedIds = new ArrayList<Long>();
		}
		return dg1572259458240DeletedIds;
	}

	private Page<MESBasicProdLevel> dg1524759114829Page;
	public Page<MESBasicProdLevel> getDg1524759114829Page(){
		if(null == dg1524759114829Page){
			dg1524759114829Page = new Page<MESBasicProdLevel>(1,20);
		}
		return dg1524759114829Page;
	}
	
	private Class dg1524759114829Clazz = MESBasicProdLevel.class;
	
	public Class getDg1524759114829Clazz() {
		return dg1524759114829Clazz;
	}
	
	private List<MESBasicProdLevel> dg1524759114829List;
	public List<MESBasicProdLevel> getDg1524759114829List(){
		if(null == dg1524759114829List){
			dg1524759114829List = new ArrayList<MESBasicProdLevel>();
		}
		return dg1524759114829List;
	}
	public void setDg1524759114829ListJson(String json) {
		//dg1524759114829List = (List<MESBasicProdLevel>) productService.generateObjectFromJson(json, MESBasicProdLevel.class);
	}
	
	private List<Long> dg1524759114829DeletedIds;
	public List<Long> getDg1524759114829DeletedIds(){
		if(null == dg1524759114829DeletedIds){
			dg1524759114829DeletedIds = new ArrayList<Long>();
		}
		return dg1524759114829DeletedIds;
	}

	private Page<MESBasicProdResetDaily> dg1524762478590Page;
	public Page<MESBasicProdResetDaily> getDg1524762478590Page(){
		if(null == dg1524762478590Page){
			dg1524762478590Page = new Page<MESBasicProdResetDaily>(1,20);
		}
		return dg1524762478590Page;
	}
	
	private Class dg1524762478590Clazz = MESBasicProdResetDaily.class;
	
	public Class getDg1524762478590Clazz() {
		return dg1524762478590Clazz;
	}
	
	private List<MESBasicProdResetDaily> dg1524762478590List;
	public List<MESBasicProdResetDaily> getDg1524762478590List(){
		if(null == dg1524762478590List){
			dg1524762478590List = new ArrayList<MESBasicProdResetDaily>();
		}
		return dg1524762478590List;
	}
	public void setDg1524762478590ListJson(String json) {
		//dg1524762478590List = (List<MESBasicProdResetDaily>) productService.generateObjectFromJson(json, MESBasicProdResetDaily.class);
	}
	
	private List<Long> dg1524762478590DeletedIds;
	public List<Long> getDg1524762478590DeletedIds(){
		if(null == dg1524762478590DeletedIds){
			dg1524762478590DeletedIds = new ArrayList<Long>();
		}
		return dg1524762478590DeletedIds;
	}

	private Page<MESBasicProdSecUnit> dg1530515613350Page;
	public Page<MESBasicProdSecUnit> getDg1530515613350Page(){
		if(null == dg1530515613350Page){
			dg1530515613350Page = new Page<MESBasicProdSecUnit>(1,20);
		}
		return dg1530515613350Page;
	}
	
	private Class dg1530515613350Clazz = MESBasicProdSecUnit.class;
	
	public Class getDg1530515613350Clazz() {
		return dg1530515613350Clazz;
	}
	
	private List<MESBasicProdSecUnit> dg1530515613350List;
	public List<MESBasicProdSecUnit> getDg1530515613350List(){
		if(null == dg1530515613350List){
			dg1530515613350List = new ArrayList<MESBasicProdSecUnit>();
		}
		return dg1530515613350List;
	}
	public void setDg1530515613350ListJson(String json) {
		//dg1530515613350List = (List<MESBasicProdSecUnit>) productService.generateObjectFromJson(json, MESBasicProdSecUnit.class);
	}
	
	private List<Long> dg1530515613350DeletedIds;
	public List<Long> getDg1530515613350DeletedIds(){
		if(null == dg1530515613350DeletedIds){
			dg1530515613350DeletedIds = new ArrayList<Long>();
		}
		return dg1530515613350DeletedIds;
	}

	private Page<MESBasicProdLevel> dg1471522536506Page;
	public Page<MESBasicProdLevel> getDg1471522536506Page(){
		if(null == dg1471522536506Page){
			dg1471522536506Page = new Page<MESBasicProdLevel>(1,20);
		}
		return dg1471522536506Page;
	}
	
	private Class dg1471522536506Clazz = MESBasicProdLevel.class;
	
	public Class getDg1471522536506Clazz() {
		return dg1471522536506Clazz;
	}
	
	private List<MESBasicProdLevel> dg1471522536506List;
	public List<MESBasicProdLevel> getDg1471522536506List(){
		if(null == dg1471522536506List){
			dg1471522536506List = new ArrayList<MESBasicProdLevel>();
		}
		return dg1471522536506List;
	}
	public void setDg1471522536506ListJson(String json) {
		//dg1471522536506List = (List<MESBasicProdLevel>) productService.generateObjectFromJson(json, MESBasicProdLevel.class);
	}
	
	private List<Long> dg1471522536506DeletedIds;
	public List<Long> getDg1471522536506DeletedIds(){
		if(null == dg1471522536506DeletedIds){
			dg1471522536506DeletedIds = new ArrayList<Long>();
		}
		return dg1471522536506DeletedIds;
	}

	private Page<MESBasicProdLevel> dg1476754911770Page;
	public Page<MESBasicProdLevel> getDg1476754911770Page(){
		if(null == dg1476754911770Page){
			dg1476754911770Page = new Page<MESBasicProdLevel>(1,20);
		}
		return dg1476754911770Page;
	}
	
	private Class dg1476754911770Clazz = MESBasicProdLevel.class;
	
	public Class getDg1476754911770Clazz() {
		return dg1476754911770Clazz;
	}
	
	private List<MESBasicProdLevel> dg1476754911770List;
	public List<MESBasicProdLevel> getDg1476754911770List(){
		if(null == dg1476754911770List){
			dg1476754911770List = new ArrayList<MESBasicProdLevel>();
		}
		return dg1476754911770List;
	}
	public void setDg1476754911770ListJson(String json) {
		//dg1476754911770List = (List<MESBasicProdLevel>) productService.generateObjectFromJson(json, MESBasicProdLevel.class);
	}
	
	private List<Long> dg1476754911770DeletedIds;
	public List<Long> getDg1476754911770DeletedIds(){
		if(null == dg1476754911770DeletedIds){
			dg1476754911770DeletedIds = new ArrayList<Long>();
		}
		return dg1476754911770DeletedIds;
	}

	private Page<MESBasicProdLevel> dg1476012237332Page;
	public Page<MESBasicProdLevel> getDg1476012237332Page(){
		if(null == dg1476012237332Page){
			dg1476012237332Page = new Page<MESBasicProdLevel>(1,20);
		}
		return dg1476012237332Page;
	}
	
	private Class dg1476012237332Clazz = MESBasicProdLevel.class;
	
	public Class getDg1476012237332Clazz() {
		return dg1476012237332Clazz;
	}
	
	private List<MESBasicProdLevel> dg1476012237332List;
	public List<MESBasicProdLevel> getDg1476012237332List(){
		if(null == dg1476012237332List){
			dg1476012237332List = new ArrayList<MESBasicProdLevel>();
		}
		return dg1476012237332List;
	}
	public void setDg1476012237332ListJson(String json) {
		//dg1476012237332List = (List<MESBasicProdLevel>) productService.generateObjectFromJson(json, MESBasicProdLevel.class);
	}
	
	private List<Long> dg1476012237332DeletedIds;
	public List<Long> getDg1476012237332DeletedIds(){
		if(null == dg1476012237332DeletedIds){
			dg1476012237332DeletedIds = new ArrayList<Long>();
		}
		return dg1476012237332DeletedIds;
	}

	private Page<MESBasicProdLevel> dg1476754989271Page;
	public Page<MESBasicProdLevel> getDg1476754989271Page(){
		if(null == dg1476754989271Page){
			dg1476754989271Page = new Page<MESBasicProdLevel>(1,20);
		}
		return dg1476754989271Page;
	}
	
	private Class dg1476754989271Clazz = MESBasicProdLevel.class;
	
	public Class getDg1476754989271Clazz() {
		return dg1476754989271Clazz;
	}
	
	private List<MESBasicProdLevel> dg1476754989271List;
	public List<MESBasicProdLevel> getDg1476754989271List(){
		if(null == dg1476754989271List){
			dg1476754989271List = new ArrayList<MESBasicProdLevel>();
		}
		return dg1476754989271List;
	}
	public void setDg1476754989271ListJson(String json) {
		//dg1476754989271List = (List<MESBasicProdLevel>) productService.generateObjectFromJson(json, MESBasicProdLevel.class);
	}
	
	private List<Long> dg1476754989271DeletedIds;
	public List<Long> getDg1476754989271DeletedIds(){
		if(null == dg1476754989271DeletedIds){
			dg1476754989271DeletedIds = new ArrayList<Long>();
		}
		return dg1476754989271DeletedIds;
	}

	private Page<MESBasicProdResetDaily> dg1473819996855Page;
	public Page<MESBasicProdResetDaily> getDg1473819996855Page(){
		if(null == dg1473819996855Page){
			dg1473819996855Page = new Page<MESBasicProdResetDaily>(1,20);
		}
		return dg1473819996855Page;
	}
	
	private Class dg1473819996855Clazz = MESBasicProdResetDaily.class;
	
	public Class getDg1473819996855Clazz() {
		return dg1473819996855Clazz;
	}
	
	private List<MESBasicProdResetDaily> dg1473819996855List;
	public List<MESBasicProdResetDaily> getDg1473819996855List(){
		if(null == dg1473819996855List){
			dg1473819996855List = new ArrayList<MESBasicProdResetDaily>();
		}
		return dg1473819996855List;
	}
	public void setDg1473819996855ListJson(String json) {
		//dg1473819996855List = (List<MESBasicProdResetDaily>) productService.generateObjectFromJson(json, MESBasicProdResetDaily.class);
	}
	
	private List<Long> dg1473819996855DeletedIds;
	public List<Long> getDg1473819996855DeletedIds(){
		if(null == dg1473819996855DeletedIds){
			dg1473819996855DeletedIds = new ArrayList<Long>();
		}
		return dg1473819996855DeletedIds;
	}

	private Page<MESBasicProdResetDaily> dg1476060528897Page;
	public Page<MESBasicProdResetDaily> getDg1476060528897Page(){
		if(null == dg1476060528897Page){
			dg1476060528897Page = new Page<MESBasicProdResetDaily>(1,20);
		}
		return dg1476060528897Page;
	}
	
	private Class dg1476060528897Clazz = MESBasicProdResetDaily.class;
	
	public Class getDg1476060528897Clazz() {
		return dg1476060528897Clazz;
	}
	
	private List<MESBasicProdResetDaily> dg1476060528897List;
	public List<MESBasicProdResetDaily> getDg1476060528897List(){
		if(null == dg1476060528897List){
			dg1476060528897List = new ArrayList<MESBasicProdResetDaily>();
		}
		return dg1476060528897List;
	}
	public void setDg1476060528897ListJson(String json) {
		//dg1476060528897List = (List<MESBasicProdResetDaily>) productService.generateObjectFromJson(json, MESBasicProdResetDaily.class);
	}
	
	private List<Long> dg1476060528897DeletedIds;
	public List<Long> getDg1476060528897DeletedIds(){
		if(null == dg1476060528897DeletedIds){
			dg1476060528897DeletedIds = new ArrayList<Long>();
		}
		return dg1476060528897DeletedIds;
	}

	@Actions({
		
		@Action(value = "/MESBasic/product/product/data-dg1572259458131", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.clevel,result.corder,result.rejectLvl,result.levelMemo,result.itemChange,result.productID.productCode,result.productID.productName,result.isUsed,result.id,result.version,result.productID.id,result.compareID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1572259458131-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1572259458131.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1572259458193", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.reinspect,result.reinspectUnit.id,result.reinspectUnit.value,result.expireDaysForNotify,result.expireDaysInterval,result.demo,result.id,result.version,result.productID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1572259458193-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1572259458193.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1572259458240", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.baseUnit.name,result.secUnit.name,result.changeRate,result.demo,result.id,result.version,result.baseUnit.id,result.secUnit.id,result.product.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1572259458240-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1572259458240.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1524759114829", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.clevel,result.corder,result.rejectLvl,result.levelMemo,result.itemChange,result.productID.productCode,result.productID.productName,result.isUsed,result.id,result.version,result.productID.id,result.compareID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1524759114829-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1524759114829.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1524762478590", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.reinspect,result.reinspectUnit.id,result.reinspectUnit.value,result.expireDaysForNotify,result.expireDaysInterval,result.demo,result.id,result.version,result.productID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1524762478590-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1524762478590.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1530515613350", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.baseUnit.name,result.secUnit.name,result.changeRate,result.demo,result.id,result.version,result.baseUnit.id,result.secUnit.id,result.product.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1530515613350-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1530515613350.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1471522536506", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.clevel,result.corder,result.rejectLvl,result.levelMemo,result.itemChange,result.compareID.productCode,result.compareID.productName,result.isUsed,result.MESBasic_1_product_productEditnewdg1471522536506_DATAGRID_CUSTOM_ae38de85_dc1d_4d00_ada7_14f51cf9230b,result.id,result.version,result.productID.id,result.compareID.id" , "dataGridCode", "MESBasic_1_product_productEditnewdg1471522536506" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1471522536506-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1471522536506.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1476754911770", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.clevel,result.corder,result.rejectLvl,result.levelMemo,result.itemChange,result.compareID.productCode,result.compareID.productName,result.isUsed,result.id,result.version,result.productID.id,result.compareID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1476754911770-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1476754911770.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1476012237332", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.clevel,result.corder,result.rejectLvl,result.levelMemo,result.itemChange,result.compareID.productCode,result.compareID.productName,result.id,result.version,result.productID.id,result.compareID.id" , "dataGridCode", "MESBasic_1_product_productViewnewdg1476012237332" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1476012237332-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1476012237332.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1476754989271", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.clevel,result.corder,result.rejectLvl,result.levelMemo,result.itemChange,result.compareID.productCode,result.compareID.productName,result.id,result.version,result.productID.id,result.compareID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1476754989271-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1476754989271.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1473819996855", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.resetCyc,result.notifyDays,result.id,result.version,result.productID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1473819996855-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1473819996855.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/product/product/data-dg1476060528897", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.resetCyc,result.notifyDays,result.id,result.version,result.productID.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1476060528897-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1476060528897.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			productService.findDataGridPage(dg,dgclass,getDgPage(),product,searchCondition,params);
	...
	private void save() throws Exception {
		...
		productService.saveProduct(product,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/product/product/prodEditforJS/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/product/product/prodEditnew/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/product/product/productEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/product/product/productEditnew/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/product/product/productView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/product/product/productViewnew/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/product/product/resetDailyEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/product/product/resetDailyView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_Product", "submit");
		bap_validate_datagrids=productService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		...
		fileuploads.put("linkId", product.getId());
		bapAfterCustomMethod("MESBasic_1_product_Product", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/product/product/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_Product", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		productService.deleteProduct(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_product_Product", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/product/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_Product", "restore");
		productService.restoreProduct(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_product_Product", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/product/product/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/product/product/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_Product", "showDealInfoData");
		if (null != tableInfoId) {
			int count=productService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = productService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = productService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_product_Product", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicProduct> findExportDataInfos(Page<MESBasicProduct> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = productService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = productService.getAssProperty(p.getCode());	
									//String assModelCode = productService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			productService.findProducts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			productService.findProducts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicProduct> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicProduct result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".PRODUCT_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.product || null == this.product.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/product/product/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_Product", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_product_Product", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/product/product/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_Product", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/product/product/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/product/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProductService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProdResetDailyService"      
						, "com.supcon.orchid.MESBasic.services.MESBasicProdLevelService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProdResetDailyService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProdSecUnitService"
						, "com.supcon.orchid.X6Basic.services.X6BasicBaseUnitService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCommDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCommDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCommDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCommDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCommDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCprodAttService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCprodAttService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCprodAttService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCprodAttService"
					, "com.supcon.orchid.MESBasic.services.MESBasicCprodAttService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProdDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProdDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProdDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProdDealInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProdDealInfoService"
					...
					, "com.supcon.orchid.MESBasic.services.MESBasicProdSecUnitService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProdSecUnitService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProductCostService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProductCostService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProductCostService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProductCostService"
					, "com.supcon.orchid.MESBasic.services.MESBasicProductCostService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_product", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_product");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_product_Product", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_product_Product", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/product/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProductService"
						, "com.supcon.orchid.MESBasic.services.MESBasicQACheckIndexService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicFactoryModelService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicBaseProdListService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicProductService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicPurchGroupService"       
						...
						, "com.supcon.orchid.X6Basic.services.X6BasicBaseUnitService"       
						, "com.supcon.orchid.X6Basic.services.X6BasicUnitGroupsService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicProdResetDailyService"      
						, "com.supcon.orchid.MESBasic.services.MESBasicProdLevelService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProdResetDailyService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProdSecUnitService"
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_product_Product", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_product_Product", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/product/product/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_Product", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/product/product/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_product_Product";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_product_Product", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/product/product/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == product.getId() ? "" : product.getId().toString());
		_auditLog.setColumnStringA(null == product.getProductName() ? "" : product.getProductName().toString());
		_auditLog.setColumnStringC(product.getId().toString());
		_auditLog.setColumnStringD(MESBasicProduct.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_product");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicProductAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, product,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, product,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,product);
		...
		printlog.setEntity("MESBasic_1_product");
		...
		printlog.setBusinessId(product.getId());
		printlog.setBussinessKey(null == product.getId() ? "" : product.getId().toString());
		printlog.setMainDisplay(null == product.getProductName() ? "" : product.getProductName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/product/product/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(product,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/product/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(productService.getProduct(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/product/product/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/product/product/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, product,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				product = productService.getProduct(Long.parseLong(arrId[i]));
				if(null != product){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == product.getProductName() ? "" : "," + product.getProductName().toString()));
					} else {
						_auditLog.setColumnStringA(null == product.getProductName() ? "" : product.getProductName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == product.getId() ? "" : "," + product.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == product.getId() ? "" : product.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == product.getId() ? "" : "," + product.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == product.getId() ? "" : product.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, product,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,product);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/product/product/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicProduct> findExportPageInfos(Page<MESBasicProduct> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_product" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = productService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		Object[] objs = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid." + "MESBasic" + ".services." + fullModelName + "Service"};
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicProduct queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = productService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = productService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_product_Product");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_product_Product")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_product_Product");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_product_Product");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = productService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_product_Product,MESBasic_1) */
// 自定义代码
			if(ids != null && ids.length() > 0){
				String[] idst = ids.split(",");
				for(String idVersion : idst){
					String id = idVersion.split("@")[0];
					if(factoryModelService.isProductLinked(Long.valueOf(id))){
						throw new BAPException(InternationalResource.get("MESBasic.product.productLinked"));
					}
				}
			}
		//bapBeforeCustomMethod("MESBasic_1_product_Product", "submit");
			  if("MESBasic_1_product_Product".equals(code)  &&  "submit".equals(methodName)){
				  if(product.getId()!=null && !"".equals(product.getId())){
						productService.checkIsWare(product.getId(),product.getBatch().getId());
					}  
			  }
      //配方BOM参照配料物品使用
		if("MESBasic_1_product_refProduct2BOM".equals(code) && "refProduct2BOMQuery".equals(methodName) ){
			if(productListStr != null && !"0".equals(productListStr)) {
				String customerSql = "\"product\".PRODUCT_ID in (" + productListStr + ")";
				params.add(new Param("customerSql", customerSql));
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_product_Product,MESBasic_1) */
// 自定义代码
   		Long productId = product.getId();
		if (productId != null) {
			if (null != dg1471522536506Page) {
				List<MESBasicProdLevel> prodLevelList = dg1471522536506Page.getResult();
				for (MESBasicProdLevel prodLevel : prodLevelList) {
					String clevel = prodLevel.getClevel();
					String checkFlag = "";
					if (clevel != null && clevel != "") {
						checkFlag = productService.checkLevelUsed(productId,clevel);
					}
					 if(checkFlag!=null&&checkFlag.length()>0){
						prodLevel.setIsUsed(true);
					} else {
						prodLevel.setIsUsed(false);
					}
				}
				dg1471522536506Page.setResult(prodLevelList);
			}
		}
      
      	// 参考复制时设置建档人、建档时间、最近维护人、最近维护时间
		if("MESBasic_1_product_Product".equals(code) && "referenceCopy".equals(methodName)){
			if(product.getId()==null){
				product.setCreator(((User)getCurrentUser()).getStaff());
				Date date = new Date();
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,functions,MESBasic_1_product_Product,MESBasic_1) */
// 自定义代码
	@Autowired
	private MESBasicFactoryModelService factoryModelService;
	

	private String unitGroupIds;

	public String getUnitGroupIds() {
		return unitGroupIds;
	}

	public void setUnitGroupIds(String unitGroupIds) {
...
/*查询主单位*/
	@Action(value = "/MESBasic/product/product/getUnitMain",results = { @Result(type = JSON, params = { "root", "responseMap" })  })
	public String getUnitMain() throws Exception {
	
		String str = productService.getUnitMain(unitGroupIds);
		String unitMain ="";
		String unitId ="";
		...
		if(str!=null&&str.length()!=0){
		 ...
		 responseMap.put("dealSuccessFlag", true);
		 responseMap.put("unitMain",unitMain);
		 responseMap.put("unitId",unitId);
		return SUCCESS;
    }
            
    /*查询选中的物品属性的名称*/
	private String baseLRC;
	
	public String getBaseLRC() {
		return baseLRC;
	}

	public void setBaseLRC(String baseLRC) {
		this.baseLRC = baseLRC;
	}

	@Action(value = "/MESBasic/product/prodAttr/getAttrName",results = { @Result(type = JSON, params = { "root", "responseMap" })  })
	public String getAttrNames() throws Exception {
	
		String attrName = productService.getAttrName(baseLRC);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("attrName", attrName);
		return SUCCESS;
    }  
    /*检查是否有价格历史*/
	...
	@Action(value = "/MESBasic/product/product/checkCostHistory",results = { @Result(type = JSON, params = { "root", "responseMap" })  })
	public String checkCostHistory() throws Exception {
	
		Boolean flag = productService.checkCostHistory(productCode);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("isCostHistory", flag);
		return SUCCESS;
    }
    /*复验周期submit*/
	@Action(value = "/MESBasic/product/product/resetDailyEdit/submit1", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	public String submitResetDaily() throws Exception{
		//bapBeforeCustomMethod("MESBasic_1_product_Product", "submit");
		bap_validate_datagrids = "dg1471522536506,dg1476754911770,dg1476012237332,dg1476754989271,dg1473819996855,dg1476060528897,";
		...
		dataGridService = new DataGridService() {
			...
			public void execute() {
				try {
					bapBeforeCustomMethod("MESBasic_1_product_Product", "saveDataGrid");
                  	dg1473819996855List = (List<MESBasicProdResetDaily>) productService.generateObjectFromJson(dgLists.get("dg1473819996855").toString(), MESBasicProdResetDaily.class);
				} catch (Exception e) {
					throw new BAPException(e);
				}/*
				if (dg1471522536506DeletedIds != null && !dg1471522536506DeletedIds.isEmpty()) {
					prodLevelService.deleteProdLevel(dg1471522536506DeletedIds, "noEvent");
				}
				if (dg1471522536506List != null && !dg1471522536506List.isEmpty()) {
					for(MESBasicProdLevel item : dg1471522536506List) {
						item.setProductID(product);
						prodLevelService.saveProdLevel(item, null, viewCode, "noEvent");
					}
				}
				if (dg1476754911770DeletedIds != null && !dg1476754911770DeletedIds.isEmpty()) {
					prodLevelService.deleteProdLevel(dg1476754911770DeletedIds, "noEvent");
				}
				if (dg1476754911770List != null && !dg1476754911770List.isEmpty()) {
					for(MESBasicProdLevel item : dg1476754911770List) {
						item.setProductID(product);
						prodLevelService.saveProdLevel(item, null, viewCode, "noEvent");
					}
				}
				if (dg1476012237332DeletedIds != null && !dg1476012237332DeletedIds.isEmpty()) {
					prodLevelService.deleteProdLevel(dg1476012237332DeletedIds, "noEvent");
				}
				if (dg1476012237332List != null && !dg1476012237332List.isEmpty()) {
					for(MESBasicProdLevel item : dg1476012237332List) {
						item.setProductID(product);
						prodLevelService.saveProdLevel(item, null, viewCode, "noEvent");
					}
				}
				if (dg1476754989271DeletedIds != null && !dg1476754989271DeletedIds.isEmpty()) {
					prodLevelService.deleteProdLevel(dg1476754989271DeletedIds, "noEvent");
				}
				if (dg1476754989271List != null && !dg1476754989271List.isEmpty()) {
					for(MESBasicProdLevel item : dg1476754989271List) {
						item.setProductID(product);
						prodLevelService.saveProdLevel(item, null, viewCode, "noEvent");
					}
				}*/
				if (dg1473819996855DeletedIds != null && !dg1473819996855DeletedIds.isEmpty()) {
					prodResetDailyService.deleteProdResetDaily(dg1473819996855DeletedIds, "noEvent");
				}
				if (dg1473819996855List != null && !dg1473819996855List.isEmpty()) {
					for(MESBasicProdResetDaily item : dg1473819996855List) {
						item.setProductID(product);
						prodResetDailyService.saveProdResetDaily(item, null, viewCode, "noEvent");
					}
				}/*
				if (dg1476060528897DeletedIds != null && !dg1476060528897DeletedIds.isEmpty()) {
					prodResetDailyService.deleteProdResetDaily(dg1476060528897DeletedIds, "noEvent");
				}
				if (dg1476060528897List != null && !dg1476060528897List.isEmpty()) {
					for(MESBasicProdResetDaily item : dg1476060528897List) {
						item.setProductID(product);
						prodResetDailyService.saveProdResetDaily(item, null, viewCode, "noEvent");
					}
				}*/
				try {
					bapAfterCustomMethod("MESBasic_1_product_Product", "saveDataGrid");
				} catch (Exception e) {
					throw new BAPException(e);
				}
			}
		...
		fileuploads.put("linkId", product.getId());*/
		//bapAfterCustomMethod("MESBasic_1_product_Product", "submit");
       	productService.resetDailySave(product.getId());
		return SUCCESS;
	}        
    /*查询质量等级是否被引用
	private Long productID;
	...
	@Action(value = "/MESBasic/product/product/checkLevelUsed",results = { @Result(type = JSON, params = { "root", "responseMap" })  })
	public String checkLevelUsed() throws Exception {
	
		String checkFlag = productService.checkLevelUsed(productID,level);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("checkFlag", checkFlag);
		return SUCCESS;
    }*/
/* CUSTOM CODE END */
}