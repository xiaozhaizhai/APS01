...
@Repository
public class MESBasicCustomerMaterialDaoImpl extends ExtendedGenericDaoImpl<MESBasicCustomerMaterial, Long> implements MESBasicCustomerMaterialDao {
	@Override
	...
	@Override
	public MESBasicCustomerMaterialDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicCustomerMaterialDealInfo dealInfo = new MESBasicCustomerMaterialDealInfo();
	...
	@Override
	public MESBasicCustomerMaterialDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicCustomerMaterial mainObj, Staff dealStaff) {
		MESBasicCustomerMaterialDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicCustomerMaterialDealInfo.class);
		...
		List<MESBasicCustomerMaterialDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_customerMaterial_CustomerMaterial,MESBasic_1) */
// 自定义代码
