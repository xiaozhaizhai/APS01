...
/* CUSTOM CODE START(action,import,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicStoreSetAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicStoreSet> {
	...
	//导出excel时,主模型内容
	List<MESBasicStoreSet> queryResults = new ArrayList<MESBasicStoreSet>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/storeSet/storeSet/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/storeSet/storeSet/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_storeSet_StoreSet" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicStoreSet> storeSets;
	
	public List<MESBasicStoreSet> getStoreSets() {
		return storeSets;
	}
	...
	private Page<MESBasicStoreSet> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			storeSet = new MESBasicStoreSet();
		} else {
			if(id > 0){
				storeSet = storeSetService.getStoreSet(id, viewCode);
			}
		}
	...
	public Page<MESBasicStoreSet> getPage() {
		if (null == page) {
			page = new Page<MESBasicStoreSet>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/storeSet/storeSet/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = storeSetService.getStoreSetAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/storeSet/storeSet/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1489731375532
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String storeSetReferenceCopy() throws Exception{
			MESBasicStoreSet oldStoreSet = null;
			if(isEditNew()){
				...
				oldStoreSet = new MESBasicStoreSet();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,storeSet.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, storeSet.getId());
				...
				oldStoreSet = storeSetService.getStoreSet(id);
			}
			
			
			storeSet = storeSetService.getStoreSet(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_storeSet_StoreSet");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicStoreSet.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldStoreSet, null);
						Method setValue = MESBasicStoreSet.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(storeSet, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			storeSet.setCreateDepartmentId(oldStoreSet.getCreateDepartmentId());
			storeSet.setCreatePositionId(oldStoreSet.getCreatePositionId());
			storeSet.setCreateStaffId(oldStoreSet.getCreateStaffId());
			storeSet.setOwnerDepartmentId(oldStoreSet.getOwnerDepartmentId());
			storeSet.setOwnerPositionId(oldStoreSet.getOwnerPositionId());
			storeSet.setOwnerStaffId(oldStoreSet.getOwnerStaffId());
			storeSet.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_storeSet_StoreSet", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=storeSetService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return storeSetReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			storeSet = storeSetService.getStoreSet(this.storeSet.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,storeSet.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = storeSetService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(storeSetService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(storeSetService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_storeSet_StoreSet", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/storeSet/storeSet/pubStoreRef", results = { @Result(type = FREEMARKER, location = "pubStoreRef.ftl") })
		,
		@Action(value = "/MESBasic/storeSet/storeSet/storeRef", results = { @Result(type = FREEMARKER, location = "storeRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"storeSet\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"storeSet\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"storeSet\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"storeSet\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/storeSet/storeSet/pubStoreRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.wareId.wareCode,result.wareId.wareName,result.placeSetCode,result.placeSetName,result.createStaff.name,result.inPriority,result.outPriority,result.wareId.id,result.createStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="pubStoreRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/storeSet/storeSet/storeRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.wareId.wareCode,result.wareId.wareName,result.placeSetCode,result.placeSetName,result.createStaff.name,result.inPriority,result.outPriority,result.wareId.id,result.createStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="storeRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String viewTreeDrag() throws Exception{
		storeSetService.drag(treecid, pid, cindex);
		return null;
	}
	...
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"storeSet\".", "{alias}.");
					}
		...
		storeSets = storeSetService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			storeSets = storeSetService.getTreeInfo(treeInfo, null, null);
		}
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		storeSetService.findDataGridPage(dg,dgclass,getDgPage(),storeSet,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(storeSet.getId() != null && storeSet.getId() != -1){
				storeSet = storeSetService.getStoreSet(storeSet.getId());
			}
			storeSetService.findDataGridPage(dg,dgclass,getDgPage(),storeSet,searchCondition,params);
	...
	private void save() throws Exception {
		...
		storeSetService.saveStoreSet(storeSet,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "submit");
		bap_validate_datagrids=storeSetService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", storeSet.getId());
		...
		fileuploads.put("linkId", storeSet.getId());
		bapAfterCustomMethod("MESBasic_1_storeSet_StoreSet", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/storeSet/storeSet/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		storeSetService.deleteStoreSet(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_storeSet_StoreSet", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/storeSet/storeSet/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "restore");
		storeSetService.restoreStoreSet(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_storeSet_StoreSet", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/storeSet/storeSet/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/storeSet/storeSet/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "showDealInfoData");
		if (null != tableInfoId) {
			int count=storeSetService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = storeSetService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = storeSetService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_storeSet_StoreSet", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicStoreSet> findExportDataInfos(Page<MESBasicStoreSet> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = storeSetService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = storeSetService.getAssProperty(p.getCode());	
									//String assModelCode = storeSetService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			storeSetService.findStoreSets(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			storeSetService.findStoreSets(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicStoreSet> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicStoreSet result : results){
	...
	public boolean isEditNew(){
		//return null == this.storeSet || null == this.storeSet.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/storeSet/storeSet/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_storeSet_StoreSet", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/storeSet/storeSet/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/storeSet/storeSet/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/storeSet/storeSet/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicStoreSetService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_storeSet", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_storeSet");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_storeSet_StoreSet", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/storeSet/storeSet/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_storeSet_StoreSet", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_storeSet_StoreSet", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/storeSet/storeSet/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/storeSet/storeSet/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_storeSet_StoreSet";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_storeSet_StoreSet", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/storeSet/storeSet/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicStoreSet storeSet = null;
		if(null != id) {
			storeSet = storeSetService.getStoreSet(id);
		}
		...
		_auditLog.setColumnStringB(null == storeSet.getPlaceSetCode() ? "" : storeSet.getPlaceSetCode().toString());
		_auditLog.setColumnStringA(null == storeSet.getPlaceSetName() ? "" : storeSet.getPlaceSetName().toString());
		_auditLog.setColumnStringC(storeSet.getId().toString());
		_auditLog.setColumnStringD(MESBasicStoreSet.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_storeSet");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicStoreSetAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, storeSet,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, storeSet,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,storeSet);
		...
		printlog.setEntity("MESBasic_1_storeSet");
		...
		printlog.setBusinessId(storeSet.getId());
		printlog.setBussinessKey(null == storeSet.getPlaceSetCode() ? "" : storeSet.getPlaceSetCode().toString());
		printlog.setMainDisplay(null == storeSet.getPlaceSetName() ? "" : storeSet.getPlaceSetName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/storeSet/storeSet/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(storeSet,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/storeSet/storeSet/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(storeSetService.getStoreSet(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/storeSet/storeSet/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/storeSet/storeSet/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, storeSet,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				storeSet = storeSetService.getStoreSet(Long.parseLong(arrId[i]));
				if(null != storeSet){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == storeSet.getPlaceSetName() ? "" : "," + storeSet.getPlaceSetName().toString()));
					} else {
						_auditLog.setColumnStringA(null == storeSet.getPlaceSetName() ? "" : storeSet.getPlaceSetName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == storeSet.getPlaceSetCode() ? "" : "," + storeSet.getPlaceSetCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == storeSet.getPlaceSetCode() ? "" : storeSet.getPlaceSetCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == storeSet.getId() ? "" : "," + storeSet.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == storeSet.getId() ? "" : storeSet.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, storeSet,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,storeSet);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/storeSet/storeSet/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicStoreSet> findExportPageInfos(Page<MESBasicStoreSet> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_storeSet" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = storeSetService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicStoreSet queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = storeSetService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = storeSetService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_storeSet_StoreSet");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_storeSet_StoreSet")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_storeSet_StoreSet");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_storeSet_StoreSet");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = storeSetService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
