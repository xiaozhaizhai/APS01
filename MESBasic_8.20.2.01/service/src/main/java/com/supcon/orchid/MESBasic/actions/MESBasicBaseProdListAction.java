...
/* CUSTOM CODE START(action,import,MESBasic_1_prodList_BaseProdList,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicBaseProdListAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBaseProdList> {
	...
	private MESBasicBaseProdList baseProdList;
	...
	//导出excel时,主模型内容
	List<MESBasicBaseProdList> queryResults = new ArrayList<MESBasicBaseProdList>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/prodList/baseProdList/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/prodList/baseProdList/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_prodList_BaseProdList" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBaseProdList> baseProdLists;
	
	public List<MESBasicBaseProdList> getBaseProdLists() {
		return baseProdLists;
	}
	...
	private Page<MESBasicBaseProdList> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			baseProdList = new MESBasicBaseProdList();
		} else {
			if(id > 0){
				baseProdList = baseProdListService.getBaseProdList(id, viewCode);
			}
		}
	...
	public Page<MESBasicBaseProdList> getPage() {
		if (null == page) {
			page = new Page<MESBasicBaseProdList>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicBaseProdList getBaseProdList() {
		return baseProdList;
	}		

	public void setBaseProdList(MESBasicBaseProdList baseProdList) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/prodList/baseProdList/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = baseProdListService.getBaseProdListAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/prodList/baseProdList/edit-extra-col")
	public String _editExtraCol() throws Exception{
		if(!isEditNew()){
			if(baseProdList.getExtraCol()!=null && baseProdList.getExtraCol().length() > 0) {
				Struts2Utils.renderXml(baseProdList.getExtraCol());
				return null;
			}
		}
	...
	/**
	 * MESBasic.modelname.randon1450078782864
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String baseProdListReferenceCopy() throws Exception{
			MESBasicBaseProdList oldBaseProdList = null;
			if(isEditNew()){
				...
				oldBaseProdList = new MESBasicBaseProdList();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseProdList.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, baseProdList.getId());
				...
				oldBaseProdList = baseProdListService.getBaseProdList(id);
			}
			
			
			baseProdList = baseProdListService.getBaseProdList(refId);
			if (baseProdList.getExtraCol() != null && baseProdList.getExtraCol().length() > 0) {
				baseProdList.setExtraColMap(XmlUtils.xml2Map(baseProdList.getExtraCol()));
			}
			Model model = modelServiceFoundation.getModel("MESBasic_1_prodList_BaseProdList");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBaseProdList.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBaseProdList, null);
						Method setValue = MESBasicBaseProdList.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(baseProdList, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			baseProdList.setCreateDepartmentId(oldBaseProdList.getCreateDepartmentId());
			baseProdList.setCreatePositionId(oldBaseProdList.getCreatePositionId());
			baseProdList.setCreateStaffId(oldBaseProdList.getCreateStaffId());
			baseProdList.setOwnerDepartmentId(oldBaseProdList.getOwnerDepartmentId());
			baseProdList.setOwnerPositionId(oldBaseProdList.getOwnerPositionId());
			baseProdList.setOwnerStaffId(oldBaseProdList.getOwnerStaffId());
			baseProdList.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_prodList_BaseProdList", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodList/baseProdList/editView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editView.ftl") }),
		@Action(value = "/MESBasic/prodList/baseProdList/editViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editViewPrint.ftl")})
		,
		@Action(value = "/MESBasic/prodList/baseProdList/mainView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainView.ftl") }),
		@Action(value = "/MESBasic/prodList/baseProdList/mainViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainViewPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=baseProdListService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return baseProdListReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			baseProdList = baseProdListService.getBaseProdList(this.baseProdList.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseProdList.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodList/baseProdList/editPart", results = { @Result(type = FREEMARKER, location = "editPart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodList/baseProdList/prodListLayout", results = { @Result(type = FREEMARKER, location = "prodListLayout.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = baseProdListService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(baseProdListService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(baseProdListService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_prodList_BaseProdList", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodList/baseProdList/refProdList", results = { @Result(type = FREEMARKER, location = "refProdList.ftl") })
		,
		@Action(value = "/MESBasic/prodList/baseProdList/refSparePartTree", results = { @Result(type = FREEMARKER, location = "refSparePartTree.ftl") })
		,
		@Action(value = "/MESBasic/prodList/baseProdList/refTree", results = { @Result(type = FREEMARKER, location = "refTree.ftl") })
		,
		@Action(value = "/MESBasic/prodList/baseProdList/tree", results = { @Result(type = FREEMARKER, location = "tree.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodList/baseProdList/editPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.prodListCode,result.prodListName,result.prodList,result.createTime,result.modifyTime,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"baseProdList\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"baseProdList\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"baseProdList\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"baseProdList\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			baseProdListService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodList/baseProdList/editPart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodList/baseProdList/refProdList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.prodListCode,result.prodListName,result.prodList,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProdList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/MESBasic/prodList/baseProdList/refSparePartTreeDrag")
	,
	@Action(value="/MESBasic/prodList/baseProdList/refTreeDrag")
	,
	@Action(value="/MESBasic/prodList/baseProdList/treeDrag")
	})
	public String viewTreeDrag() throws Exception{
		baseProdListService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/MESBasic/prodList/baseProdList/editPartSort")
	,
	@Action(value="/MESBasic/prodList/baseProdList/editViewSort")
	,
	@Action(value="/MESBasic/prodList/baseProdList/mainViewSort")
	,
	@Action(value="/MESBasic/prodList/baseProdList/prodListLayoutSort")
	,
	@Action(value="/MESBasic/prodList/baseProdList/refProdListSort")
	,
	@Action(value="/MESBasic/prodList/baseProdList/refSparePartTreeSort")
	,
	@Action(value="/MESBasic/prodList/baseProdList/refTreeSort")
	,
	@Action(value="/MESBasic/prodList/baseProdList/treeSort")
	})
	...
	@Action(value="/MESBasic/prodList/baseProdList/refSparePartTreeData",
			results = { @Result(type = JSON, params = { "root", "baseProdLists", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.prodListName"})}
		)
	,
	@Action(value="/MESBasic/prodList/baseProdList/refTreeData",
			results = { @Result(type = JSON, params = { "root", "baseProdLists", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.prodListName"})}
		)
	,
	@Action(value="/MESBasic/prodList/baseProdList/treeData",
			results = { @Result(type = JSON, params = { "root", "baseProdLists", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.prodListName"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"baseProdList\".", "{alias}.");
					}
		...
		baseProdLists = baseProdListService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/MESBasic/prodList/baseProdList/refSparePartTreeFullData",
			results = { @Result(type = JSON, params = { "root", "baseProdLists", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.prodListName"})}
		)
	,
	@Action(value="/MESBasic/prodList/baseProdList/refTreeFullData",
			results = { @Result(type = JSON, params = { "root", "baseProdLists", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.prodListName"})}
		)
	,
	@Action(value="/MESBasic/prodList/baseProdList/treeFullData",
			results = { @Result(type = JSON, params = { "root", "baseProdLists", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.prodListName"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			baseProdLists = baseProdListService.getTreeInfo(treeInfo, null, null);
		}
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/prodList/baseProdList/allTempleteDownload", results = { @Result(name="excel",type="excel",location="editPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_prodList_BaseProdList");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		baseProdListService.findDataGridPage(dg,dgclass,getDgPage(),baseProdList,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(baseProdList.getId() != null && baseProdList.getId() != -1){
				baseProdList = baseProdListService.getBaseProdList(baseProdList.getId());
			}
			baseProdListService.findDataGridPage(dg,dgclass,getDgPage(),baseProdList,searchCondition,params);
	...
	private void save() throws Exception {
		...
		baseProdListService.saveBaseProdList(baseProdList,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/prodList/baseProdList/editView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/prodList/baseProdList/mainView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "submit");
		bap_validate_datagrids=baseProdListService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", baseProdList.getId());
		...
		fileuploads.put("linkId", baseProdList.getId());
		bapAfterCustomMethod("MESBasic_1_prodList_BaseProdList", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/prodList/baseProdList/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		baseProdListService.deleteBaseProdList(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_prodList_BaseProdList", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodList/baseProdList/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "restore");
		baseProdListService.restoreBaseProdList(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_prodList_BaseProdList", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/prodList/baseProdList/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/prodList/baseProdList/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "showDealInfoData");
		if (null != tableInfoId) {
			int count=baseProdListService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = baseProdListService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = baseProdListService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_prodList_BaseProdList", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBaseProdList> findExportDataInfos(Page<MESBasicBaseProdList> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = baseProdListService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = baseProdListService.getAssProperty(p.getCode());	
									//String assModelCode = baseProdListService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
				...
				if(view.getAssModel().getDataType() == 2 && useForImportFlag){//树型结构且用于导出，加上父节点,同时在sql中加入layNo,layRec,parentId,sort,leaf
					...
					exportSqlPart1Add.append("\".PRODLIST_LAYNO AS \"layNo\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
					exportSqlPart1Add.append("\".PRODLIST_LAYREC AS \"layRec\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			baseProdListService.findBaseProdLists(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			baseProdListService.findBaseProdLists(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBaseProdList> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBaseProdList result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".PRODLIST_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.baseProdList || null == this.baseProdList.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/prodList/baseProdList/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_prodList_BaseProdList", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/prodList/baseProdList/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/prodList/baseProdList/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodList/baseProdList/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBaseProdListService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_prodList", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_prodList");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_prodList_BaseProdList", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodList/baseProdList/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_prodList_BaseProdList", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_prodList_BaseProdList", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/prodList/baseProdList/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/prodList/baseProdList/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_prodList_BaseProdList";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_prodList_BaseProdList", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/prodList/baseProdList/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicBaseProdList baseProdList = null;
		if(null != id) {
			baseProdList = baseProdListService.getBaseProdList(id);
		}
		...
		_auditLog.setColumnStringB(null == baseProdList.getProdListCode() ? "" : baseProdList.getProdListCode().toString());
		_auditLog.setColumnStringA(null == baseProdList.getProdListName() ? "" : baseProdList.getProdListName().toString());
		_auditLog.setColumnStringC(baseProdList.getId().toString());
		_auditLog.setColumnStringD(MESBasicBaseProdList.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_prodList");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBaseProdListAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, baseProdList,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, baseProdList,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,baseProdList);
		...
		printlog.setEntity("MESBasic_1_prodList");
		...
		printlog.setBusinessId(baseProdList.getId());
		printlog.setBussinessKey(null == baseProdList.getProdListCode() ? "" : baseProdList.getProdListCode().toString());
		printlog.setMainDisplay(null == baseProdList.getProdListName() ? "" : baseProdList.getProdListName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/prodList/baseProdList/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(baseProdList,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodList/baseProdList/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(baseProdListService.getBaseProdList(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/prodList/baseProdList/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/prodList/baseProdList/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, baseProdList,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				baseProdList = baseProdListService.getBaseProdList(Long.parseLong(arrId[i]));
				if(null != baseProdList){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == baseProdList.getProdListName() ? "" : "," + baseProdList.getProdListName().toString()));
					} else {
						_auditLog.setColumnStringA(null == baseProdList.getProdListName() ? "" : baseProdList.getProdListName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == baseProdList.getProdListCode() ? "" : "," + baseProdList.getProdListCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == baseProdList.getProdListCode() ? "" : baseProdList.getProdListCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == baseProdList.getId() ? "" : "," + baseProdList.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == baseProdList.getId() ? "" : baseProdList.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, baseProdList,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,baseProdList);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/prodList/baseProdList/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicBaseProdList> findExportPageInfos(Page<MESBasicBaseProdList> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_prodList" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = baseProdListService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicBaseProdList queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = baseProdListService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = baseProdListService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_prodList_BaseProdList");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_prodList_BaseProdList")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_prodList_BaseProdList");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_prodList_BaseProdList");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = baseProdListService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_prodList_BaseProdList,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_prodList_BaseProdList,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_prodList_BaseProdList,MESBasic_1) */
// 自定义代码
