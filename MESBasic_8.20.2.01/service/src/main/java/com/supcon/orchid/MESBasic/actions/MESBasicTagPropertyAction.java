...
/* CUSTOM CODE START(action,import,MESBasic_1_tagProperty_TagProperty,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicTagPropertyAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicTagProperty> {
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			...
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_tagProperty_TagProperty" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicTagProperty> tagPropertys;
	
	public List<MESBasicTagProperty> getTagPropertys() {
		return tagPropertys;
	}
	...
	private Page<MESBasicTagProperty> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			tagProperty = new MESBasicTagProperty();
			if(tagProperty.getTagType() != null) {
				tagProperty.setTagType((SystemCode) systemCodeService.getSystemCode(tagProperty.getTagType().getId()));
			}
			if(tagProperty.getTattribute() != null) {
				tagProperty.setTattribute((SystemCode) systemCodeService.getSystemCode(tagProperty.getTattribute().getId()));
			}
		} else {
			if(id > 0){
				tagProperty = tagPropertyService.getTagProperty(id, viewCode);
			}
		}
	...
	public Page<MESBasicTagProperty> getPage() {
		if (null == page) {
			page = new Page<MESBasicTagProperty>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tagProperty/tagProperty/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = tagPropertyService.getTagPropertyAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tagProperty/tagProperty/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String tagPropertyReferenceCopy() throws Exception{
			MESBasicTagProperty oldTagProperty = null;
			if(isEditNew()){
				...
				oldTagProperty = new MESBasicTagProperty();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,tagProperty.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, tagProperty.getId());
				...
				oldTagProperty = tagPropertyService.getTagProperty(id);
			}
			
			
			tagProperty = tagPropertyService.getTagProperty(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_tagProperty_TagProperty");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicTagProperty.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTagProperty, null);
						Method setValue = MESBasicTagProperty.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(tagProperty, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			tagProperty.setCreateDepartmentId(oldTagProperty.getCreateDepartmentId());
			tagProperty.setCreatePositionId(oldTagProperty.getCreatePositionId());
			tagProperty.setCreateStaffId(oldTagProperty.getCreateStaffId());
			tagProperty.setOwnerDepartmentId(oldTagProperty.getOwnerDepartmentId());
			tagProperty.setOwnerPositionId(oldTagProperty.getOwnerPositionId());
			tagProperty.setOwnerStaffId(oldTagProperty.getOwnerStaffId());
			bapAfterCustomMethod("MESBasic_1_tagProperty_TagProperty", "referenceCopy");
			return SUCCESS;
		}
	// ######## MESBasic_1_tagProperty_tagEdit start
	/**
	 * 位号的编辑视图
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tagProperty/tagProperty/tagEdit",  interceptorRefs={@InterceptorRef(value="orchidFullStack")}, results = { @Result(type = FREEMARKER, location = "tagEdit.ftl") })
	public String tagEdit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tagProperty_tagEdit", "tagEdit");
		if (null != entityCode) {
		...
		bapAfterCustomMethod("MESBasic_1_tagProperty_tagEdit", "tagEdit");
		return SUCCESS;
	}
	
	// ######## MESBasic_1_tagProperty_tagEdit end
	// ######## MESBasic_1_tagProperty_tagList start
	/**
	 * MESBasic.viewtitle.radion1414992768949
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tagProperty/tagProperty/tagList", results = { @Result(type = FREEMARKER, location = "tagList.ftl") })
	public String tagList() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tagProperty_tagList", "tagList");
		...
		bapAfterCustomMethod("MESBasic_1_tagProperty_tagList", "tagList");
		return SUCCESS;
	}
	
	// ######## MESBasic_1_tagProperty_tagList end
	// ######## MESBasic_1_tagProperty_tagView start
	/**
	 * 查看视图
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tagProperty/tagProperty/tagView",  interceptorRefs={@InterceptorRef(value="orchidFullStack")}, results = { @Result(type = FREEMARKER, location = "tagView.ftl") })
	public String tagView() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tagProperty_tagView", "tagView");
			if(null!=entityCode){
				businessCenterList = businessCenterService.getBusinessCenters(entityCode);
				for(BusinessCenter bc:businessCenterList){
					//String url=businessCenterService.getBusinessUrl(bc,tagProperty.getId().toString());
		...
		bapAfterCustomMethod("MESBasic_1_tagProperty_tagView", "tagView");
		return SUCCESS;
	}
	
	// ######## MESBasic_1_tagProperty_tagView end
	...
	@Action(value = "/MESBasic/tagProperty/tagProperty/tagList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.status,result.tableNo,result.createTime,result.code,result.name,result.tag,result.tagType.id,result.tagType.value,result.tattribute.id,result.tattribute.value,result.active,result.crtValue,result.updateTime,result.virtualTag,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion"})  
			
			,@Result(name="excel",type="excel",location="tagList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	public String tagListQuery() throws Exception{
		viewCode = "MESBasic_1_tagProperty_tagList";
		params = new ArrayList<Param>();
		params.add(new Param("\"tagProperty\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		bapBeforeCustomMethod(viewCode, "tagListQuery");
		setFieldPermissionModelCode("MESBasic_1_tagProperty_TagProperty");
		...
		if (null != getDataTableSortColOrder() && getDataTableSortColOrder().length() > 0) {
			params.add(new Param("dataTable-sortColOrder", getDataTableSortColOrder()));
					...
					Param p = new Param("\"tagProperty\".CODE",request.getParameter("code"),Param.LIKE_ALL);
					params.add(p);
					}
				if(null != request.getParameter("name") && request.getParameter("name").trim().length() > 0) {
					Param p = new Param("\"tagProperty\".NAME",request.getParameter("name"),Param.LIKE_ALL);
					params.add(p);
					}
				if(null != request.getParameter("tag") && request.getParameter("tag").trim().length() > 0) {
					Param p = new Param("\"tagProperty\".TAG",request.getParameter("tag"),Param.LIKE_ALL);
					params.add(p);
					}
				if(null != request.getParameter("tagType") && request.getParameter("tagType").trim().length() > 0) {
					Param p = new Param("\"tagProperty\".TAG_TYPE",request.getParameter("tagType"),Param.LIKE_UNSUPPORT);
					params.add(p);
					}
				if(null != request.getParameter("tattribute") && request.getParameter("tattribute").trim().length() > 0) {
					Param p = new Param("\"tagProperty\".TATTRIBUTE",request.getParameter("tattribute"),Param.LIKE_UNSUPPORT);
					params.add(p);
					}
				if(null != request.getParameter("virtualTag") && request.getParameter("virtualTag").trim().length() > 0) {
					Param p = new Param("\"tagProperty\".VIRTUAL_TAG",request.getParameter("virtualTag"),Param.LIKE_UNSUPPORT);
					params.add(p);
					}
				if(null != request.getParameter("active") && request.getParameter("active").trim().length() > 0) {
					Param p = new Param("\"tagProperty\".ACTIVE",request.getParameter("active"),Param.LIKE_UNSUPPORT);
					params.add(p);
					}
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"tagProperty\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			bapAfterCustomMethod(viewCode, "tagListQuery");
			return "excel";
		} else {
	...
	private void save(DataGridService dataGridService) throws Exception {
		if(null == tagProperty.getId()) {
			responseMap.put("operate", "add");
		} else {
			responseMap.put("operate", "edit");
		}
		tagPropertyService.saveTagProperty(tagProperty, dataGridService, viewCode);
	...
	@Actions({ 
		@Action(value = "/MESBasic/tagProperty/tagProperty/tagEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/tagProperty/tagProperty/tagView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tagProperty_TagProperty", "submit");
		DataGridService dataGridService = null;
		
			save(dataGridService);
			responseMap.put("id", tagProperty.getId());

		Map<String, Object> fileuploads = (Map<String, Object>) request.getAttribute("__orchid_fileuploads__");
		fileuploads.put("linkId", tagProperty.getId());
		bapAfterCustomMethod("MESBasic_1_tagProperty_TagProperty", "submit");
		return SUCCESS;
	}
	@Action(value = "/MESBasic/tagProperty/tagProperty/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tagProperty_TagProperty", "delete");
		tagPropertyService.deleteTagProperty(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_tagProperty_TagProperty", "delete");
		return SUCCESS;
	}
	
	
	@Actions({
			@Action(value = "/MESBasic/tagProperty/tagProperty/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/tagProperty/tagProperty/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tagProperty_TagProperty", "showDealInfoData");
		if (null != tableInfoId) {
			int count=tagPropertyService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = tagPropertyService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = tagPropertyService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_tagProperty_TagProperty", "showDealInfoData");
		return SUCCESS;
	}	

	@Override
	public Page<MESBasicTagProperty> findExportDataInfos(Page<MESBasicTagProperty> page) {
		if(null != searchObjects && searchObjects.length > 0) {
			tagPropertyService.findTagPropertys(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, searchObjects);
		} else {
			tagPropertyService.findTagPropertys(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode);
		}
		return getPage();
	}

	public boolean isEditNew(){
		//return null == this.tagProperty || null == this.tagProperty.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/tagProperty/tagProperty/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tagProperty_TagProperty", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_tagProperty_TagProperty", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/tagProperty/tagProperty/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tagProperty_TagProperty", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/tagProperty/tagProperty/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tagProperty/tagProperty/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicTagPropertyService"
		};
		bapBeforeCustomMethod("MESBasic_1_tagProperty_TagProperty", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_tagProperty_TagProperty", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tagProperty/tagProperty/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				xlsResult = importService.imports(viewCode, "MESBasic_1_tagProperty_TagProperty", dataFile, time, name[0], isReplace, isIgnore, importServices);
	...
	@Action(value = "/MESBasic/tagProperty/tagProperty/downLoad", results = { @Result(type = "stream", params = {
  ...
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tagProperty_TagProperty", "downLoadXls");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tagProperty/tagProperty/templateDownLoad", results = { @Result(type = "stream", params = {
			"contentType", "application/excel", "inputName", "inputStream",
			"contentDisposition", 
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String templateDownLoad() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tagProperty_TagProperty", "templateDownLoad");
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/tagProperty/tagProperty/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		MESBasicTagProperty tagProperty = null;
		if(null != id) {
			tagProperty = tagPropertyService.getTagProperty(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, tagProperty, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, tagProperty, null, null, true, this);
	...
	@Action(value = "/MESBasic/tagProperty/tagProperty/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/tagProperty/tagProperty/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				tagProperty = tagPropertyService.getTagProperty(Integer.parseInt(id));
				if(tagProperty != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, tagProperty, null, null, true, this);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_tagProperty_TagProperty,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void bapAfterCustomMethod(String code, String methodName) throws Exception {
	/* CUSTOM CODE START(action,methods,MESBasic_1_tagProperty_TagProperty,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_tagProperty_TagProperty,MESBasic_1) */
// 自定义代码
