...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicDepartmentWareAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicDepartmentWare> {
	...
	private MESBasicDepartmentWare departmentWare;
	...
	//导出excel时,主模型内容
	List<MESBasicDepartmentWare> queryResults = new ArrayList<MESBasicDepartmentWare>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryWare/departmentWare/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryWare/departmentWare/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryWare_DepartmentWare" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicDepartmentWare> departmentWares;
	
	public List<MESBasicDepartmentWare> getDepartmentWares() {
	...
	private Page<MESBasicDepartmentWare> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			departmentWare = new MESBasicDepartmentWare();
		} else {
			if(id > 0){
				departmentWare = departmentWareService.getDepartmentWare(id, viewCode);
			}
		}
	...
	public Page<MESBasicDepartmentWare> getPage() {
		if (null == page) {
			page = new Page<MESBasicDepartmentWare>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicDepartmentWare getDepartmentWare() {
		return departmentWare;
	}		

	public void setDepartmentWare(MESBasicDepartmentWare departmentWare) {
		this.departmentWare = departmentWare;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryWare/departmentWare/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = departmentWareService.getDepartmentWareAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryWare/departmentWare/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1489731905451
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String departmentWareReferenceCopy() throws Exception{
			MESBasicDepartmentWare oldDepartmentWare = null;
			if(isEditNew()){
				...
				oldDepartmentWare = new MESBasicDepartmentWare();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,departmentWare.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, departmentWare.getId());
				...
				oldDepartmentWare = departmentWareService.getDepartmentWare(id);
			}
			
			
			departmentWare = departmentWareService.getDepartmentWare(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryWare_DepartmentWare");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicDepartmentWare.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldDepartmentWare, null);
						Method setValue = MESBasicDepartmentWare.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(departmentWare, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			departmentWare.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryWare/departmentWare/departWareEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "departWareEdit.ftl") }),
		@Action(value = "/MESBasic/factoryWare/departmentWare/departWareEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "departWareEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=departmentWareService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return departmentWareReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			departmentWare = departmentWareService.getDepartmentWare(this.departmentWare.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,departmentWare.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryWare/departmentWare/departWarePart", results = { @Result(type = FREEMARKER, location = "departWarePart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryWare/departmentWare/factoryWareLayout", results = { @Result(type = FREEMARKER, location = "factoryWareLayout.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = departmentWareService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(departmentWareService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(departmentWareService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryWare_DepartmentWare", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryWare/departmentWare/departWarePart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.factoryId.code,result.factoryId.name,result.wareId.wareCode,result.wareId.wareName,result.storeId.placeSetCode,result.storeId.placeSetName,result.meno,result.factoryId.id,result.wareId.id,result.storeId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="departWarePart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"departmentWare\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"departmentWare\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"departmentWare\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"departmentWare\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			departmentWareService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryWare/departmentWare/departWarePart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		departmentWareService.findDataGridPage(dg,dgclass,getDgPage(),departmentWare,"", null);
	...
	private Page<MESBasicFacWareMaterial> dg1489738203672Page;
	public Page<MESBasicFacWareMaterial> getDg1489738203672Page(){
		if(null == dg1489738203672Page){
			dg1489738203672Page = new Page<MESBasicFacWareMaterial>(1,20);
		}
		return dg1489738203672Page;
	}
	
	private Class dg1489738203672Clazz = MESBasicFacWareMaterial.class;
	
	public Class getDg1489738203672Clazz() {
		return dg1489738203672Clazz;
	}
	
	private List<MESBasicFacWareMaterial> dg1489738203672List;
	public List<MESBasicFacWareMaterial> getDg1489738203672List(){
		if(null == dg1489738203672List){
			dg1489738203672List = new ArrayList<MESBasicFacWareMaterial>();
		}
		return dg1489738203672List;
	}
	public void setDg1489738203672ListJson(String json) {
		//dg1489738203672List = (List<MESBasicFacWareMaterial>) departmentWareService.generateObjectFromJson(json, MESBasicFacWareMaterial.class);
	}
	
	private List<Long> dg1489738203672DeletedIds;
	public List<Long> getDg1489738203672DeletedIds(){
		if(null == dg1489738203672DeletedIds){
			dg1489738203672DeletedIds = new ArrayList<Long>();
		}
		return dg1489738203672DeletedIds;
	}

	@Actions({
		
		@Action(value = "/MESBasic/factoryWare/departmentWare/data-dg1489738203672", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.productId.productModel,result.productId.productSpecif,result.meno,result.id,result.version,result.headID.id,result.factoryId.id,result.productId.id,result.storeId.id,result.wareId.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1489738203672-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1489738203672.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(departmentWare.getId() != null && departmentWare.getId() != -1){
				departmentWare = departmentWareService.getDepartmentWare(departmentWare.getId());
			}
			departmentWareService.findDataGridPage(dg,dgclass,getDgPage(),departmentWare,searchCondition,params);
	...
	private void save() throws Exception {
		...
		departmentWareService.saveDepartmentWare(departmentWare,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/factoryWare/departmentWare/departWareEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "submit");
		bap_validate_datagrids=departmentWareService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", departmentWare.getId());
		...
		fileuploads.put("linkId", departmentWare.getId());
		bapAfterCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryWare/departmentWare/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		departmentWareService.deleteDepartmentWare(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryWare/departmentWare/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "restore");
		departmentWareService.restoreDepartmentWare(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicDepartmentWare> findExportDataInfos(Page<MESBasicDepartmentWare> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = departmentWareService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = departmentWareService.getAssProperty(p.getCode());	
									//String assModelCode = departmentWareService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			departmentWareService.findDepartmentWares(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			departmentWareService.findDepartmentWares(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicDepartmentWare> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicDepartmentWare result : results){
	...
	public boolean isEditNew(){
		//return null == this.departmentWare || null == this.departmentWare.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryWare/departmentWare/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryWare/departmentWare/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryWare/departmentWare/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryWare/departmentWare/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicDepartmentWareService"
						, "com.supcon.orchid.MESBasic.services.MESBasicFacWareMaterialService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_factoryWare", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_factoryWare");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryWare/departmentWare/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryWare_DepartmentWare", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryWare_DepartmentWare", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryWare/departmentWare/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryWare/departmentWare/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryWare_DepartmentWare";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryWare_DepartmentWare", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryWare/departmentWare/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicDepartmentWare departmentWare = null;
		if(null != id) {
			departmentWare = departmentWareService.getDepartmentWare(id);
		}
		...
		_auditLog.setColumnStringB(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
		_auditLog.setColumnStringA(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
		_auditLog.setColumnStringC(departmentWare.getId().toString());
		_auditLog.setColumnStringD(MESBasicDepartmentWare.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_factoryWare");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicDepartmentWareAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, departmentWare,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, departmentWare,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,departmentWare);
		...
		printlog.setEntity("MESBasic_1_factoryWare");
		...
		printlog.setBusinessId(departmentWare.getId());
		printlog.setBussinessKey(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
		printlog.setMainDisplay(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryWare/departmentWare/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(departmentWare,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryWare/departmentWare/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(departmentWareService.getDepartmentWare(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryWare/departmentWare/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryWare/departmentWare/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, departmentWare,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				departmentWare = departmentWareService.getDepartmentWare(Long.parseLong(arrId[i]));
				if(null != departmentWare){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == departmentWare.getId() ? "" : "," + departmentWare.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == departmentWare.getId() ? "" : "," + departmentWare.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == departmentWare.getId() ? "" : "," + departmentWare.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, departmentWare,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,departmentWare);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/factoryWare/departmentWare/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryWare_DepartmentWare");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryWare_DepartmentWare")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryWare_DepartmentWare");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryWare_DepartmentWare");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = departmentWareService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
	...
	@Action(value = "/MESBasic/factoryWare/departmentWare/findOperStatue", results = { @Result(type = JSON, params = {
			"root", "responseMap" }) })
	public String findOperStatue() {
		String result=departmentWareService.findOperStatue(wareId);
