...
@Repository
public class MESBasicBaseCustomerClassDaoImpl extends TreeDaoImpl<MESBasicBaseCustomerClass, Long> implements MESBasicBaseCustomerClassDao {
	@Override
	...
	@Override
	public MESBasicBaseCustomerClassDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicBaseCustomerClassDealInfo dealInfo = new MESBasicBaseCustomerClassDealInfo();
	...
	@Override
	public MESBasicBaseCustomerClassDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicBaseCustomerClass mainObj, Staff dealStaff) {
		MESBasicBaseCustomerClassDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicBaseCustomerClassDealInfo.class);
		...
		List<MESBasicBaseCustomerClassDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_baseCustomerClass_BaseCustomerClass,MESBasic_1) */
// 自定义代码
