...
import com.supcon.orchid.MESBasic.services.MESBasicFacWareMaterialImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicFacWareMaterialDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_facWareMaterialService")
@Transactional
public class MESBasicFacWareMaterialServiceImpl  extends BaseServiceImpl<MESBasicFacWareMaterial> implements MESBasicFacWareMaterialService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicFacWareMaterial> bapBaseService;
	...
	@Autowired
	private MESBasicFacWareMaterialImportService facWareMaterialImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicFacWareMaterial.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  facWareMaterialDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicFacWareMaterial.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = facWareMaterialDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicFacWareMaterial.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicFacWareMaterial.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  facWareMaterialDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicFacWareMaterial.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicFacWareMaterial.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicFacWareMaterial> list = (List<MESBasicFacWareMaterial>) facWareMaterialDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicFacWareMaterial getFacWareMaterial(long id){
		return getFacWareMaterial(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFacWareMaterial getFacWareMaterial(long id, String viewCode){
		MESBasicFacWareMaterial facWareMaterial = facWareMaterialDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFacWareMaterialAsJSON(long id, String include){
		MESBasicFacWareMaterial facWareMaterial = facWareMaterialDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(facWareMaterial, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFacWareMaterial(MESBasicFacWareMaterial facWareMaterial){
		if(null != facWareMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFacWareMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
			AuditUtil.setColumnStringC(facWareMaterial.getId().toString());
		}
		beforeDeleteFacWareMaterial(facWareMaterial);
		facWareMaterialDao.deletePhysical(facWareMaterial);
		afterDeleteFacWareMaterial(facWareMaterial);
		facWareMaterialDao.flush();
		List<MESBasicFacWareMaterial> params = new ArrayList<MESBasicFacWareMaterial>();
		params.add(facWareMaterial);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFacWareMaterial(List<Long> facWareMaterialIds){
		deleteFacWareMaterial(facWareMaterialIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFacWareMaterial(List<Long> facWareMaterialIds, String eventTopic) {
		List<MESBasicFacWareMaterial> facWareMaterials = new ArrayList<MESBasicFacWareMaterial>();
		for(Long facWareMaterialId : facWareMaterialIds){
			MESBasicFacWareMaterial facWareMaterial = getFacWareMaterial(facWareMaterialId);
			facWareMaterials.add(facWareMaterial);
			if(null != facWareMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFacWareMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == facWareMaterial.getId() ? "" : "," + facWareMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == facWareMaterial.getId() ? "" : "," + facWareMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == facWareMaterial.getId() ? "" : "," + facWareMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(facWareMaterials != null && facWareMaterials.size() > 0 ){
			for(MESBasicFacWareMaterial facWareMaterial : facWareMaterials){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicFacWareMaterial.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(facWareMaterial));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),facWareMaterial, facWareMaterial.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(facWareMaterialIds != null && facWareMaterialIds.size() > 0) {
			String hql = "delete " + MESBasicFacWareMaterial.JPA_NAME + " where id in(:ids)";
			Query query = facWareMaterialDao.createQuery(hql);
			query.setParameterList("ids", facWareMaterialIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicFacWareMaterial facWareMaterialz : facWareMaterials){
				facWareMaterialz.setValid(false);
				List<MESBasicFacWareMaterial> params = new ArrayList<MESBasicFacWareMaterial>();
				params.add(facWareMaterialz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_factoryWare");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFacWareMaterial(long facWareMaterialId, int facWareMaterialVersion){
		this.deleteFacWareMaterial(facWareMaterialId, facWareMaterialVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFacWareMaterial(long facWareMaterialId, int facWareMaterialVersion,SignatureLog signatureLog){
		MESBasicFacWareMaterial facWareMaterial = getFacWareMaterial(facWareMaterialId);
		if(facWareMaterial != null && !facWareMaterial.isValid()){
		...
		beforeDeleteFacWareMaterial(facWareMaterial);
		facWareMaterialDao.delete(facWareMaterialId, facWareMaterialVersion);
		afterDeleteFacWareMaterial(facWareMaterial);
		facWareMaterialDao.flush();
		List<MESBasicFacWareMaterial> params = new ArrayList<MESBasicFacWareMaterial>();
		params.add(facWareMaterial);
		...
		if(signatureLog != null ) {
			...
			businessKey=facWareMaterial.getId();
			signatureLog.setTableId(facWareMaterial.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFacWareMaterial(String facWareMaterialIds){
		this.deleteFacWareMaterial(facWareMaterialIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFacWareMaterial(String facWareMaterialIds,SignatureLog signatureLog){
		deleteCollection(facWareMaterialIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFacWareMaterial(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_factoryWare#modelCode:MESBasic_1_factoryWare_FacWareMaterial#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="ec.engine.restore",operType="10")
	public void restoreFacWareMaterial(String facWareMaterialIds){
		restoreCollection(facWareMaterialIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFacWareMaterial(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="ec.engine.restore",operType="10")
	public void restoreFacWareMaterial(long facWareMaterialId){
		
		findBusinessKeyUsed(facWareMaterialId);	//判断业务主键是否重复
		...
		beforeRestoreFacWareMaterial(facWareMaterial);
		facWareMaterial.setValid(true);
		facWareMaterialDao.update(facWareMaterial);
		afterRestoreFacWareMaterial(facWareMaterial);
		List<MESBasicFacWareMaterial> params = new ArrayList<MESBasicFacWareMaterial>();
		params.add(facWareMaterial);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long facWareMaterialId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_factoryWare_FacWareMaterial");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicFacWareMaterial.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicFacWareMaterial.TABLE_NAME +" where id = ? )";
			List<Object> list =  facWareMaterialDao.createNativeQuery(sql,facWareMaterialId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFacWareMaterial(List<MESBasicFacWareMaterial>  facWareMaterials){
		for(MESBasicFacWareMaterial facWareMaterial:facWareMaterials)  {
			saveFacWareMaterial(facWareMaterial, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFacWareMaterial(List<MESBasicFacWareMaterial>  facWareMaterials){
		for(MESBasicFacWareMaterial facWareMaterial:facWareMaterials)  {
			saveFacWareMaterial(facWareMaterial, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="ec.flow.save")
	public void saveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFacWareMaterial(facWareMaterial, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="ec.flow.save")
	public void saveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFacWareMaterial(facWareMaterial, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFacWareMaterialCodes(MESBasicFacWareMaterial facWareMaterial) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFacWareMaterialCodes(facWareMaterial, false);
	}
	
	public void generateFacWareMaterialCodes(MESBasicFacWareMaterial facWareMaterial, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFacWareMaterialSummarys(MESBasicFacWareMaterial facWareMaterial) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFacWareMaterialSummarys(facWareMaterial, false);
	}

	public void generateFacWareMaterialSummarys(MESBasicFacWareMaterial facWareMaterial, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial")
	public void saveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFacWareMaterial(facWareMaterial, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial")
	public void saveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_factoryWare");
		String url = null;
		if(facWareMaterial.getId() != null && facWareMaterial.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFacWareMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, facWareMaterial);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(facWareMaterial);
		beforeSaveFacWareMaterial(facWareMaterial, viewIsView);

		if (viewIsView) {
			facWareMaterialDao.saveWithRevertVersion(facWareMaterial);
		} else {
			if(isNew)
				facWareMaterialDao.save(facWareMaterial);
			else
				facWareMaterialDao.merge(facWareMaterial);
		}
		if(null != facWareMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFacWareMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
		...
		dealDatagridsSave(facWareMaterial,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFacWareMaterialCodes(facWareMaterial, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFacWareMaterialSummarys(facWareMaterial, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFacWareMaterial(facWareMaterial, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, facWareMaterial);
			}
		}
		List<MESBasicFacWareMaterial> params = new ArrayList<MESBasicFacWareMaterial>();
		facWareMaterialDao.flush();
		facWareMaterialDao.clear();
		facWareMaterial = facWareMaterialDao.load(facWareMaterial.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicFacWareMaterial facWareMaterial){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("facWareMaterial", facWareMaterial);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial")
	public void mergeFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(facWareMaterial);
		beforeSaveFacWareMaterial(facWareMaterial);
		facWareMaterialDao.merge(facWareMaterial);
		if(null != facWareMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFacWareMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == facWareMaterial.getId() ? "" : facWareMaterial.getId().toString());
		...
		dealDatagridsSave(facWareMaterial,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFacWareMaterial(facWareMaterial);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicFacWareMaterial> findFacWareMaterials(Page<MESBasicFacWareMaterial> page, Criterion... criterions) {
		return facWareMaterialDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFacWareMaterials(Page<MESBasicFacWareMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFacWareMaterials(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFacWareMaterials(Page<MESBasicFacWareMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFacWareMaterials(Page<MESBasicFacWareMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicFacWareMaterial> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"facWareMaterial\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"facWareMaterial\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"facWareMaterial\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  facWareMaterialDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  facWareMaterialDao.createNativeQuery(powerOperateSql, "MESBasic_1_factoryWare").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"facWareMaterial\".LAY_REC ASC, \"facWareMaterial\".SORT ASC");
				} else {
					orderPart.append(" \"facWareMaterial\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) facWareMaterialDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=facWareMaterialDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = facWareMaterialDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicFacWareMaterial> result = new ArrayList<MESBasicFacWareMaterial>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicFacWareMaterial.class,facWareMaterialDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicFacWareMaterial.class,facWareMaterialDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicFacWareMaterial facWareMaterial : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(facWareMaterial.getId(), "MESBasic_factoryWare_facWareMaterial");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(facWareMaterial.getId(), "MESBasic_factoryWare_facWareMaterial");
							if(null != documents && !documents.isEmpty()) {
								facWareMaterial.setDocument(documents.get(0));
								facWareMaterial.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicFacWareMaterial> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicFacWareMaterial> result) {
		View view = null;
		...
		Map<Long, MESBasicFacWareMaterial> rsMap = new HashMap<Long, MESBasicFacWareMaterial>();
		for (MESBasicFacWareMaterial facWareMaterial : result) {
			ids.add(facWareMaterial.getId());
			rsMap.put(facWareMaterial.getId(), facWareMaterial);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(facWareMaterialDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = facWareMaterialDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_factoryWare_FacWareMaterial" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_factoryWare_FacWareMaterial" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  facWareMaterialDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"facWareMaterial\"","MESBasic_1_factoryWare","","MESBASIC_FAC_WARE_MATERIALS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"facWareMaterial\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"facWareMaterial\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"facWareMaterial\"","MESBasic_1_factoryWare","","MESBASIC_FAC_WARE_MATERIALS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_factoryWare_FacWareMaterial" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("facWareMaterial.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"facWareMaterial\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"facWareMaterial\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"facWareMaterial\".CID = 1 OR \"facWareMaterial\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("facWareMaterial.mc\"").append(".FAC_WARE_MATERIAL) not in ("+notinValue+")");
			}
			
			SQLQuery query = facWareMaterialDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=facWareMaterialDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicFacWareMaterial.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicFacWareMaterial.MODULE_CODE;}
	protected String getModelCode(){return MESBasicFacWareMaterial.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param facWareMaterial 模型实例
	 */
	public void checkUniqueConstraint(MESBasicFacWareMaterial facWareMaterial){
	}
	
	
	@Override
	public List<MESBasicFacWareMaterial> getFacWareMaterials(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicFacWareMaterial.class);
		if(params!=null){
			...
			return facWareMaterialDao.findByHql("from " + MESBasicFacWareMaterial.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return facWareMaterialDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicFacWareMaterial> findFacWareMaterialsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicFacWareMaterial.class);
	...
	@Override
	public List<MESBasicFacWareMaterial> findFacWareMaterialsByHql(String hql, Object... objects){
		
		return   facWareMaterialDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicFacWareMaterial loadFacWareMaterialByBussinessKey(MESBasicFacWareMaterial facWareMaterial){
		return loadFacWareMaterialByBussinessKey(facWareMaterial.getId());
	}
	
	@Override
	public Page<MESBasicFacWareMaterial> getFacWareMaterials(Page<MESBasicFacWareMaterial> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicFacWareMaterial.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = facWareMaterialDao.findByPage(page, sql);
			}else{
				page = facWareMaterialDao.findAllByPage(page);
			}
			return page;
		}
		page = facWareMaterialDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicFacWareMaterial> findByTreeDataGridDepartmentWare(List<MESBasicFacWareMaterial> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicFacWareMaterial facWareMaterial : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridDepartmentWare(Page<MESBasicFacWareMaterial> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicFacWareMaterial> facWareMaterials = dgPage.getResult();
			for(MESBasicFacWareMaterial facWareMaterial : facWareMaterials){
				
			}
			dgPage.setResult(facWareMaterials);
		}
	...
	@Override
	@Transactional
	public void deleteFacWareMaterialByDepartmentWare(List<Long> departmentWareIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("headIDList", departmentWareIds);
		String hql = "select id from MESBasicFacWareMaterial where "
						+ "headID.id in (:headIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> facWareMaterialIds1 = facWareMaterialDao.findByHql(hql, params);
		if(facWareMaterialIds1 != null && !facWareMaterialIds1.isEmpty()) {
		}
		hql = "delete MESBasicFacWareMaterial where "
						+ "headID.id in (:headIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		facWareMaterialDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFacWareMaterial loadFacWareMaterialByBussinessKey(Serializable bussinessKey){
		return facWareMaterialDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicFacWareMaterial> findByProperty(String propertyName, Object object){
		return facWareMaterialDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFacWareMaterial findEntityByProperty(String propertyName, Object object){
		return facWareMaterialDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicFacWareMaterial.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = facWareMaterialDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_factoryWare_FacWareMaterial","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicFacWareMaterial facWareMaterial,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,facWareMaterialDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicFacWareMaterial.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, facWareMaterial);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",facWareMaterial.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicFacWareMaterial.TABLE_NAME +" where valid = 1";
			list = facWareMaterialDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicFacWareMaterial.TABLE_NAME;
			list = facWareMaterialDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicFacWareMaterial.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = facWareMaterialDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicFacWareMaterialDaoImpl facWareMaterialDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchFacWareMaterial(final List<MESBasicFacWareMaterial> insertObjs, final List<MESBasicFacWareMaterial> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchFacWareMaterial(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFacWareMaterial(final List<MESBasicFacWareMaterial> insertObjs, final List<MESBasicFacWareMaterial> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1489732341394"),importCount));
		...
		Map<Long, MESBasicFacWareMaterial> oldUpdateObjsMap = new HashMap<Long, MESBasicFacWareMaterial>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicFacWareMaterial bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicFacWareMaterial> bizObjList = facWareMaterialDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicFacWareMaterial bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicFacWareMaterial> nmeObjs = new ArrayList<MESBasicFacWareMaterial>();
		...
		long id=autoGeneratorID.getNextId(MESBasicFacWareMaterial.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicFacWareMaterial facWareMaterial:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicFacWareMaterial.TABLE_NAME,1,dbId);
			if(facWareMaterial.getId() != null ){
				m.put(facWareMaterial.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) facWareMaterialDao.createNativeQuery(sql, "MESBasic_1_factoryWare_FacWareMaterial"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		facWareMaterialDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicFacWareMaterial.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicFacWareMaterial facWareMaterial = updateObjs.get(updateIndex);
						Long id = facWareMaterial.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = facWareMaterial.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(facWareMaterial);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicFacWareMaterial.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicFacWareMaterial.TABLE_NAME + " order by id desc";
//					Query query = facWareMaterialDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicFacWareMaterial facWareMaterial = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = facWareMaterial.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		facWareMaterialDaoImpl.getSession().flush();
		facWareMaterialDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicFacWareMaterial bizObj = insertObjs.get(insertIndex);
			facWareMaterialImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicFacWareMaterial bizObj = updateObjs.get(updateIndex);
			MESBasicFacWareMaterial oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			facWareMaterialImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicFacWareMaterial> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicFacWareMaterial> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) facWareMaterialDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicFacWareMaterial> result = getResult(page, query, new PendingResultTransformer(MESBasicFacWareMaterial.class,facWareMaterialDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicFacWareMaterial model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = facWareMaterialDao.createNativeQuery(sql, "MESBasic_1_factoryWare_FacWareMaterial").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = facWareMaterialDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = facWareMaterialDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = facWareMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?facWareMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  facWareMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,facWareMaterialDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="ec.flow.save")
	public void saveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, DataGridService dataGridService){
		saveFacWareMaterial(facWareMaterial, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="ec.flow.save")
	public void saveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, DataGridService dataGridService, String viewCode){
		saveFacWareMaterial(facWareMaterial, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial")
	public void saveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFacWareMaterial(facWareMaterial, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial")
	public void saveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryWare";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryWare");
		String url = null;
		if(null != facWareMaterial.getId() && facWareMaterial.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFacWareMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial")
	public Map<Object, Object> facWareMaterialDataGridImport(MESBasicFacWareMaterial facWareMaterial, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = facWareMaterial.getVirtualId();
		
		facWareMaterial = facWareMaterialDao.load(facWareMaterial.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, facWareMaterial.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = facWareMaterial.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(facWareMaterial);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, facWareMaterial.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial")
	public void mergeFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(facWareMaterial);
		beforeSaveFacWareMaterial(facWareMaterial);
		facWareMaterialDao.merge(facWareMaterial);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_FacWareMaterial",desc="foundation.batch.save",operType="4")
	public void batchSaveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_factoryWare";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryWare");
	...
	@Override
	public Page<MESBasicFacWareMaterial> getByPage(Page<MESBasicFacWareMaterial> page,DetachedCriteria detachedCriteria){
		return facWareMaterialDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, Object... objects){
		checkUniqueConstraint(facWareMaterial);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFacWareMaterial(MESBasicFacWareMaterial facWareMaterial, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportFacWareMaterial(List<MESBasicFacWareMaterial> facWareMaterials, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportFacWareMaterial(List<MESBasicFacWareMaterial> facWareMaterials){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportFacWareMaterial(List<MESBasicFacWareMaterial> facWareMaterials, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportFacWareMaterial(List<MESBasicFacWareMaterial> facWareMaterials){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFacWareMaterial(MESBasicFacWareMaterial facWareMaterial){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFacWareMaterial(MESBasicFacWareMaterial facWareMaterial){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFacWareMaterial(MESBasicFacWareMaterial facWareMaterial){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFacWareMaterial(MESBasicFacWareMaterial facWareMaterial){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicFacWareMaterial facWareMaterial, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicFacWareMaterial facWareMaterial, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportFacWareMaterial(List<MESBasicFacWareMaterial> insertObjs, List<MESBasicFacWareMaterial> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFacWareMaterial,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportFacWareMaterial(List<MESBasicFacWareMaterial> insertObjs, List<MESBasicFacWareMaterial> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFacWareMaterial,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_factoryWare_FacWareMaterial,MESBasic_1) */
// 自定义代码
