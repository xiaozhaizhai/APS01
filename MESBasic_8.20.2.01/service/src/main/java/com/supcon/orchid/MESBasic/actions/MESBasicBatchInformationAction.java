...
/* CUSTOM CODE START(action,import,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicBatchInformationAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBatchInformation> {
	...
	private MESBasicBatchInformation batchInformation;
	...
	//导出excel时,主模型内容
	List<MESBasicBatchInformation> queryResults = new ArrayList<MESBasicBatchInformation>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/batchInfo/batchInformation/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/batchInfo/batchInformation/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_batchInfo_BatchInformation" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBatchInformation> batchInformations;
	
	public List<MESBasicBatchInformation> getBatchInformations() {
	...
	private Page<MESBasicBatchInformation> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchInformation = new MESBasicBatchInformation();
			if(batchInformation.getCheckState() != null) {
				batchInformation.setCheckState((SystemCode) systemCodeService.getSystemCode(batchInformation.getCheckState().getId()));
			}
			if(batchInformation.getPassState() != null) {
				batchInformation.setPassState((SystemCode) systemCodeService.getSystemCode(batchInformation.getPassState().getId()));
			}
			if(batchInformation.getSourceType() != null) {
				batchInformation.setSourceType((SystemCode) systemCodeService.getSystemCode(batchInformation.getSourceType().getId()));
			}
		} else {
			if(id > 0){
				batchInformation = batchInformationService.getBatchInformation(id, viewCode);
			}
		}
	...
	public Page<MESBasicBatchInformation> getPage() {
		if (null == page) {
			page = new Page<MESBasicBatchInformation>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicBatchInformation getBatchInformation() {
		return batchInformation;
	}		

	public void setBatchInformation(MESBasicBatchInformation batchInformation) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/batchInfo/batchInformation/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchInformationService.getBatchInformationAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/batchInfo/batchInformation/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String batchInformationReferenceCopy() throws Exception{
			MESBasicBatchInformation oldBatchInformation = null;
			if(isEditNew()){
				...
				oldBatchInformation = new MESBasicBatchInformation();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchInformation.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchInformation.getId());
				...
				oldBatchInformation = batchInformationService.getBatchInformation(id);
			}
			
			
			batchInformation = batchInformationService.getBatchInformation(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchInformation");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBatchInformation.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchInformation, null);
						Method setValue = MESBasicBatchInformation.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchInformation, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchInformation.setCreateDepartmentId(oldBatchInformation.getCreateDepartmentId());
			batchInformation.setCreatePositionId(oldBatchInformation.getCreatePositionId());
			batchInformation.setCreateStaffId(oldBatchInformation.getCreateStaffId());
			batchInformation.setOwnerDepartmentId(oldBatchInformation.getOwnerDepartmentId());
			batchInformation.setOwnerPositionId(oldBatchInformation.getOwnerPositionId());
			batchInformation.setOwnerStaffId(oldBatchInformation.getOwnerStaffId());
			batchInformation.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_batchInfo_BatchInformation", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchInformationService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchInformationReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchInformation = batchInformationService.getBatchInformation(this.batchInformation.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchInformation.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchInformationService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchInformation/batchInfoList", results = { @Result(type = FREEMARKER, location = "batchInfoList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchInformationService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchInformationService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchInformationService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_batchInfo_BatchInformation", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchInformation/batchInfoList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchNum,result.product.productCode,result.product.productName,result.sourceType.id,result.sourceType.value,result.productionDate,result.validDate,result.recheckDate,result.checkDate,result.reNotify,result.manufacture,result.provider,result.supplierBatch,result.purchaseDate,result.arriveDate,result.checkState.id,result.checkState.value,result.passState.id,result.passState.value,result.isAvailable,result.product.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batchInfoList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchInformation\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchInformation\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchInformation\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchInformation\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchInformationService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchInformation/batchInfoList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/batchInfo/batchInformation/allTempleteDownload", results = { @Result(name="excel",type="excel",location="batchInfoList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchInformation");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchInformationService.findDataGridPage(dg,dgclass,getDgPage(),batchInformation,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchInformation.getId() != null && batchInformation.getId() != -1){
				batchInformation = batchInformationService.getBatchInformation(batchInformation.getId());
			}
			batchInformationService.findDataGridPage(dg,dgclass,getDgPage(),batchInformation,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchInformation.getId()) {
		...
		batchInformationService.saveBatchInformation(batchInformation,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "submit");
		bap_validate_datagrids=batchInformationService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchInformation.getId());
		...
		fileuploads.put("linkId", batchInformation.getId());
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchInformation", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/batchInfo/batchInformation/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchInformationService.deleteBatchInformation(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchInformation", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchInformation/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "restore");
		batchInformationService.restoreBatchInformation(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchInformation", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/batchInfo/batchInformation/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/batchInfo/batchInformation/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "showDealInfoData");
		if (null != tableInfoId) {
			int count=batchInformationService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = batchInformationService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = batchInformationService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchInformation", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBatchInformation> findExportDataInfos(Page<MESBasicBatchInformation> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchInformationService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchInformationService.getAssProperty(p.getCode());	
									//String assModelCode = batchInformationService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchInformationService.findBatchInformations(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchInformationService.findBatchInformations(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBatchInformation> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBatchInformation result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchInformation || null == this.batchInformation.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/batchInfo/batchInformation/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchInformation", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/batchInfo/batchInformation/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/batchInfo/batchInformation/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchInformation/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBatchInformationService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_batchInfo", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_batchInfo");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchInformation", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchInformation/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBatchInformationService"
						...
						, "com.supcon.orchid.MESBasic.services.MESBasicBatchDealInfoService"
						, "com.supcon.orchid.MESBasic.services.MESBasicBatchRelationshipService"
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_batchInfo_BatchInformation", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_batchInfo_BatchInformation", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/batchInfo/batchInformation/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/batchInfo/batchInformation/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_batchInfo_BatchInformation";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchInformation", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/batchInfo/batchInformation/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicBatchInformation batchInformation = null;
		if(null != id) {
			batchInformation = batchInformationService.getBatchInformation(id);
		}
		...
		_auditLog.setColumnStringB(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
		_auditLog.setColumnStringA(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
		_auditLog.setColumnStringC(batchInformation.getId().toString());
		_auditLog.setColumnStringD(MESBasicBatchInformation.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_batchInfo");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBatchInformationAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchInformation,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchInformation,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchInformation);
		...
		printlog.setEntity("MESBasic_1_batchInfo");
		...
		printlog.setBusinessId(batchInformation.getId());
		printlog.setBussinessKey(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
		printlog.setMainDisplay(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/batchInfo/batchInformation/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchInformation,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchInformation/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchInformationService.getBatchInformation(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/batchInfo/batchInformation/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/batchInfo/batchInformation/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchInformation,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchInformation = batchInformationService.getBatchInformation(Long.parseLong(arrId[i]));
				if(null != batchInformation){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchInformation.getId() ? "" : "," + batchInformation.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchInformation.getId() ? "" : "," + batchInformation.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchInformation.getId() ? "" : "," + batchInformation.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchInformation,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchInformation);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchInformationService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/batchInfo/batchInformation/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicBatchInformation> findExportPageInfos(Page<MESBasicBatchInformation> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_batchInfo" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = batchInformationService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicBatchInformation queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = batchInformationService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = batchInformationService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchInformation");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_batchInfo_BatchInformation")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_batchInfo_BatchInformation");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchInformation");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchInformationService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
