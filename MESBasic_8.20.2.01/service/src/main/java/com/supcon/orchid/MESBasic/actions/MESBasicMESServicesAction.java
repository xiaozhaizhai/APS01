...
/* CUSTOM CODE START(action,import,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicMESServicesAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicMESServices> {
	...
	private MESBasicMESServices mESServices;
	...
	//导出excel时,主模型内容
	List<MESBasicMESServices> queryResults = new ArrayList<MESBasicMESServices>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/backgroundServiceMonitor/mESServices/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/backgroundServiceMonitor/mESServices/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_backgroundServiceMonitor_MESServices" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicMESServices> mESServicess;
	
	public List<MESBasicMESServices> getMESServicess() {
		return mESServicess;
	}
	...
	private Page<MESBasicMESServices> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			mESServices = new MESBasicMESServices();
		} else {
			if(id > 0){
				mESServices = mESServicesService.getMESServices(id, viewCode);
			}
		}
	...
	public Page<MESBasicMESServices> getPage() {
		if (null == page) {
			page = new Page<MESBasicMESServices>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicMESServices getMESServices() {
		return mESServices;
	}		

	public void setMESServices(MESBasicMESServices mESServices) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = mESServicesService.getMESServicesAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String mESServicesReferenceCopy() throws Exception{
			MESBasicMESServices oldMESServices = null;
			if(isEditNew()){
				...
				oldMESServices = new MESBasicMESServices();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,mESServices.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, mESServices.getId());
				...
				oldMESServices = mESServicesService.getMESServices(id);
			}
			
			
			mESServices = mESServicesService.getMESServices(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_backgroundServiceMonitor_MESServices");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicMESServices.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMESServices, null);
						Method setValue = MESBasicMESServices.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(mESServices, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			mESServices.setCreateDepartmentId(oldMESServices.getCreateDepartmentId());
			mESServices.setCreatePositionId(oldMESServices.getCreatePositionId());
			mESServices.setCreateStaffId(oldMESServices.getCreateStaffId());
			mESServices.setOwnerDepartmentId(oldMESServices.getOwnerDepartmentId());
			mESServices.setOwnerPositionId(oldMESServices.getOwnerPositionId());
			mESServices.setOwnerStaffId(oldMESServices.getOwnerStaffId());
			mESServices.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "logEdit.ftl") }),
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "logEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=mESServicesService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return mESServicesReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			mESServices = mESServicesService.getMESServices(this.mESServices.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,mESServices.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "logView.ftl") }),
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "logViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logList", results = { @Result(type = FREEMARKER, location = "logList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = mESServicesService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(mESServicesService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(mESServicesService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_backgroundServiceMonitor_MESServices", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.serviceCode,result.serviceName,result.active,result.lastRunningTime,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="logList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"mESServices\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"mESServices\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"mESServices\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"mESServices\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			mESServicesService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/allTempleteDownload", results = { @Result(name="excel",type="excel",location="logList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_backgroundServiceMonitor_MESServices");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		mESServicesService.findDataGridPage(dg,dgclass,getDgPage(),mESServices,"", null);
	...
	private Page<MESBasicServiceLogger> dg1429057196635Page;
	public Page<MESBasicServiceLogger> getDg1429057196635Page(){
		if(null == dg1429057196635Page){
			dg1429057196635Page = new Page<MESBasicServiceLogger>(1,20);
		}
		return dg1429057196635Page;
	}
	
	private Class dg1429057196635Clazz = MESBasicServiceLogger.class;
	
	public Class getDg1429057196635Clazz() {
		return dg1429057196635Clazz;
	}
	
	private List<MESBasicServiceLogger> dg1429057196635List;
	public List<MESBasicServiceLogger> getDg1429057196635List(){
		if(null == dg1429057196635List){
			dg1429057196635List = new ArrayList<MESBasicServiceLogger>();
		}
		return dg1429057196635List;
	}
	public void setDg1429057196635ListJson(String json) {
		//dg1429057196635List = (List<MESBasicServiceLogger>) mESServicesService.generateObjectFromJson(json, MESBasicServiceLogger.class);
	}
	
	private List<Long> dg1429057196635DeletedIds;
	public List<Long> getDg1429057196635DeletedIds(){
		if(null == dg1429057196635DeletedIds){
			dg1429057196635DeletedIds = new ArrayList<Long>();
		}
		return dg1429057196635DeletedIds;
	}

	@Actions({
		
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/data-dg1429057196635", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.logTime,result.logContent,result.id,result.version,result.mesService.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1429057196635-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1429057196635.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(mESServices.getId() != null && mESServices.getId() != -1){
				mESServices = mESServicesService.getMESServices(mESServices.getId());
			}
			mESServicesService.findDataGridPage(dg,dgclass,getDgPage(),mESServices,searchCondition,params);
	...
	private void save() throws Exception {
		...
		mESServicesService.saveMESServices(mESServices,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/logView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "submit");
		bap_validate_datagrids=mESServicesService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", mESServices.getId());
		...
		fileuploads.put("linkId", mESServices.getId());
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		mESServicesService.deleteMESServices(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "restore");
		mESServicesService.restoreMESServices(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "showDealInfoData");
		if (null != tableInfoId) {
			int count=mESServicesService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = mESServicesService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = mESServicesService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicMESServices> findExportDataInfos(Page<MESBasicMESServices> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = mESServicesService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = mESServicesService.getAssProperty(p.getCode());	
									//String assModelCode = mESServicesService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			mESServicesService.findMESServicess(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			mESServicesService.findMESServicess(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicMESServices> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicMESServices result : results){
	...
	public boolean isEditNew(){
		//return null == this.mESServices || null == this.mESServices.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/backgroundServiceMonitor/mESServices/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/backgroundServiceMonitor/mESServices/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicMESServicesService"
						, "com.supcon.orchid.MESBasic.services.MESBasicServiceLoggerService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_backgroundServiceMonitor", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_backgroundServiceMonitor");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_backgroundServiceMonitor_MESServices", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_backgroundServiceMonitor_MESServices", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_backgroundServiceMonitor_MESServices";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_backgroundServiceMonitor_MESServices", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == mESServices.getServiceCode() ? "" : mESServices.getServiceCode().toString());
		_auditLog.setColumnStringA(null == mESServices.getServiceName() ? "" : mESServices.getServiceName().toString());
		_auditLog.setColumnStringC(mESServices.getId().toString());
		_auditLog.setColumnStringD(MESBasicMESServices.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_backgroundServiceMonitor");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicMESServicesAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, mESServices,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, mESServices,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,mESServices);
		...
		printlog.setEntity("MESBasic_1_backgroundServiceMonitor");
		...
		printlog.setBusinessId(mESServices.getId());
		printlog.setBussinessKey(null == mESServices.getServiceCode() ? "" : mESServices.getServiceCode().toString());
		printlog.setMainDisplay(null == mESServices.getServiceName() ? "" : mESServices.getServiceName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(mESServices,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(mESServicesService.getMESServices(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, mESServices,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				mESServices = mESServicesService.getMESServices(Long.parseLong(arrId[i]));
				if(null != mESServices){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == mESServices.getServiceName() ? "" : "," + mESServices.getServiceName().toString()));
					} else {
						_auditLog.setColumnStringA(null == mESServices.getServiceName() ? "" : mESServices.getServiceName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == mESServices.getServiceCode() ? "" : "," + mESServices.getServiceCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == mESServices.getServiceCode() ? "" : mESServices.getServiceCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == mESServices.getId() ? "" : "," + mESServices.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == mESServices.getId() ? "" : mESServices.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, mESServices,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,mESServices);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicMESServices> findExportPageInfos(Page<MESBasicMESServices> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_backgroundServiceMonitor" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = mESServicesService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicMESServices queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = mESServicesService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = mESServicesService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_backgroundServiceMonitor_MESServices");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_backgroundServiceMonitor_MESServices")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_backgroundServiceMonitor_MESServices");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_backgroundServiceMonitor_MESServices");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = mESServicesService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
	
	private String serviceCode;

	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/changeStatus",
			results = {@Result(name = SUCCESS, type = JSON, params = {"root", "batchPrintXmlInfo", "excludes", "*", "includes", "succeed,errMsg"})})
	public String changeStatus() throws Exception{
		
		batchPrintXmlInfo = new HashMap<String, Object>();
		
		String sql = "valid = 1 and SERVICE_CODE='" + serviceCode + "'";
		List<MESBasicMESServices> sList = mESServicesService.getMESServicess(sql, null);
		MESBasicMESServices mesService;
		if(sList.size() > 0){
			mesService = sList.get(0);
			mesService.setActive(mesService.getActive() != null ? !mesService.getActive():true);
			mESServicesService.mergeMESServices(mesService, null);
			
			batchPrintXmlInfo.put("succeed", true);
		}else{
			batchPrintXmlInfo.put("succeed", false);
			batchPrintXmlInfo.put("errMsg", "数据库发生异常，该服务已被意外删除。");
		}
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/backgroundServiceMonitor/mESServices/loadBackService",
			results = {@Result(name = SUCCESS, type = JSON)})
	public void loadBackService() throws Exception{
		String sql = "update mesbasic_messervices set cid = " + getCurrentCompanyId();
