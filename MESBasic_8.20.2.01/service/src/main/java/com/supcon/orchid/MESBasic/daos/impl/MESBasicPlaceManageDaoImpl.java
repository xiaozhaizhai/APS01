...
@Repository
public class MESBasicPlaceManageDaoImpl extends ExtendedGenericDaoImpl<MESBasicPlaceManage, Long> implements MESBasicPlaceManageDao {
	@Override
	...
	@Override
	public MESBasicPlaceManageDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicPlaceManageDealInfo dealInfo = new MESBasicPlaceManageDealInfo();
	...
	@Override
	public MESBasicPlaceManageDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicPlaceManage mainObj, Staff dealStaff) {
		MESBasicPlaceManageDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicPlaceManageDealInfo.class);
		...
		List<MESBasicPlaceManageDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
