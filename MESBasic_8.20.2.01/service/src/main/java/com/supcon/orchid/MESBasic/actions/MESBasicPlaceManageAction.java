...
/* CUSTOM CODE START(action,import,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicPlaceManageAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicPlaceManage> {
	...
	//导出excel时,主模型内容
	List<MESBasicPlaceManage> queryResults = new ArrayList<MESBasicPlaceManage>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/placeManage/placeManage/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/placeManage/placeManage/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_placeManage_PlaceManage" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicPlaceManage> placeManages;
	
	public List<MESBasicPlaceManage> getPlaceManages() {
		return placeManages;
	}
	...
	private Page<MESBasicPlaceManage> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			placeManage = new MESBasicPlaceManage();
		} else {
			if(id > 0){
				placeManage = placeManageService.getPlaceManage(id, viewCode);
			}
		}
	...
	public Page<MESBasicPlaceManage> getPage() {
		if (null == page) {
			page = new Page<MESBasicPlaceManage>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/placeManage/placeManage/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = placeManageService.getPlaceManageAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/placeManage/placeManage/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String placeManageReferenceCopy() throws Exception{
			MESBasicPlaceManage oldPlaceManage = null;
			if(isEditNew()){
				...
				oldPlaceManage = new MESBasicPlaceManage();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,placeManage.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, placeManage.getId());
				...
				oldPlaceManage = placeManageService.getPlaceManage(id);
			}
			
			
			placeManage = placeManageService.getPlaceManage(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_placeManage_PlaceManage");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicPlaceManage.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPlaceManage, null);
						Method setValue = MESBasicPlaceManage.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(placeManage, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			placeManage.setCreateDepartmentId(oldPlaceManage.getCreateDepartmentId());
			placeManage.setCreatePositionId(oldPlaceManage.getCreatePositionId());
			placeManage.setCreateStaffId(oldPlaceManage.getCreateStaffId());
			placeManage.setOwnerDepartmentId(oldPlaceManage.getOwnerDepartmentId());
			placeManage.setOwnerPositionId(oldPlaceManage.getOwnerPositionId());
			placeManage.setOwnerStaffId(oldPlaceManage.getOwnerStaffId());
			placeManage.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_placeManage_PlaceManage", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/placeManage/placeManage/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/MESBasic/placeManage/placeManage/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
		,
		@Action(value = "/MESBasic/placeManage/placeManage/placeView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "placeView.ftl") }),
		@Action(value = "/MESBasic/placeManage/placeManage/placeViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "placeViewPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=placeManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return placeManageReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			placeManage = placeManageService.getPlaceManage(this.placeManage.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,placeManage.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/placeManage/placeManage/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/placeManage/placeManage/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = placeManageService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(placeManageService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(placeManageService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_placeManage_PlaceManage", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/placeManage/placeManage/ref1", results = { @Result(type = FREEMARKER, location = "ref1.ftl") })
		,
		@Action(value = "/MESBasic/placeManage/placeManage/refrence", results = { @Result(type = FREEMARKER, location = "refrence.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/placeManage/placeManage/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.placeCode,result.placeName,result.placeManager.name,result.factoryNode.name,result.placeManager.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"placeManage\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"placeManage\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"factoryNode\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"placeManage\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"placeManage\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			placeManageService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/placeManage/placeManage/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/placeManage/placeManage/ref1-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.placeCode,result.placeName,result.placeManager.name,result.placeManager.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="ref1-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/placeManage/placeManage/refrence-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.placeCode,result.placeName,result.placeManager.name,result.factoryNode.name,result.placeManager.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refrence-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/placeManage/placeManage/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_placeManage_PlaceManage");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		placeManageService.findDataGridPage(dg,dgclass,getDgPage(),placeManage,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(placeManage.getId() != null && placeManage.getId() != -1){
				placeManage = placeManageService.getPlaceManage(placeManage.getId());
			}
			placeManageService.findDataGridPage(dg,dgclass,getDgPage(),placeManage,searchCondition,params);
	...
	private void save() throws Exception {
		...
		placeManageService.savePlaceManage(placeManage,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/placeManage/placeManage/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/placeManage/placeManage/placeView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "submit");
		bap_validate_datagrids=placeManageService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", placeManage.getId());
		...
		fileuploads.put("linkId", placeManage.getId());
		bapAfterCustomMethod("MESBasic_1_placeManage_PlaceManage", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/placeManage/placeManage/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		placeManageService.deletePlaceManage(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_placeManage_PlaceManage", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/placeManage/placeManage/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "restore");
		placeManageService.restorePlaceManage(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_placeManage_PlaceManage", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/placeManage/placeManage/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/placeManage/placeManage/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "showDealInfoData");
		if (null != tableInfoId) {
			int count=placeManageService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = placeManageService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = placeManageService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_placeManage_PlaceManage", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicPlaceManage> findExportDataInfos(Page<MESBasicPlaceManage> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = placeManageService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = placeManageService.getAssProperty(p.getCode());	
									//String assModelCode = placeManageService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			placeManageService.findPlaceManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			placeManageService.findPlaceManages(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicPlaceManage> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicPlaceManage result : results){
	...
	public boolean isEditNew(){
		//return null == this.placeManage || null == this.placeManage.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/placeManage/placeManage/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_placeManage_PlaceManage", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/placeManage/placeManage/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/placeManage/placeManage/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/placeManage/placeManage/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicPlaceManageService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_placeManage", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_placeManage");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_placeManage_PlaceManage", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/placeManage/placeManage/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_placeManage_PlaceManage", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_placeManage_PlaceManage", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/placeManage/placeManage/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/placeManage/placeManage/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_placeManage_PlaceManage";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_placeManage_PlaceManage", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/placeManage/placeManage/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == placeManage.getPlaceCode() ? "" : placeManage.getPlaceCode().toString());
		_auditLog.setColumnStringA(null == placeManage.getPlaceName() ? "" : placeManage.getPlaceName().toString());
		_auditLog.setColumnStringC(placeManage.getId().toString());
		_auditLog.setColumnStringD(MESBasicPlaceManage.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_placeManage");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicPlaceManageAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, placeManage,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, placeManage,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,placeManage);
		...
		printlog.setEntity("MESBasic_1_placeManage");
		...
		printlog.setBusinessId(placeManage.getId());
		printlog.setBussinessKey(null == placeManage.getPlaceCode() ? "" : placeManage.getPlaceCode().toString());
		printlog.setMainDisplay(null == placeManage.getPlaceName() ? "" : placeManage.getPlaceName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/placeManage/placeManage/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(placeManage,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/placeManage/placeManage/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(placeManageService.getPlaceManage(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/placeManage/placeManage/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/placeManage/placeManage/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, placeManage,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				placeManage = placeManageService.getPlaceManage(Long.parseLong(arrId[i]));
				if(null != placeManage){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == placeManage.getPlaceName() ? "" : "," + placeManage.getPlaceName().toString()));
					} else {
						_auditLog.setColumnStringA(null == placeManage.getPlaceName() ? "" : placeManage.getPlaceName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == placeManage.getPlaceCode() ? "" : "," + placeManage.getPlaceCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == placeManage.getPlaceCode() ? "" : placeManage.getPlaceCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == placeManage.getId() ? "" : "," + placeManage.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == placeManage.getId() ? "" : placeManage.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, placeManage,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,placeManage);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/placeManage/placeManage/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicPlaceManage> findExportPageInfos(Page<MESBasicPlaceManage> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_placeManage" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = placeManageService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicPlaceManage queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = placeManageService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = placeManageService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_placeManage_PlaceManage");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_placeManage_PlaceManage")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_placeManage_PlaceManage");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_placeManage_PlaceManage");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = placeManageService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
