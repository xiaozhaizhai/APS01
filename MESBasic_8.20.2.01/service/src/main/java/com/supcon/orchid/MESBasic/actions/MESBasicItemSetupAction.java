...
/* CUSTOM CODE START(action,import,MESBasic_1_itemManage_ItemSetup,MESBasic_1) */
// 自定义代码
...
public class MESBasicItemSetupAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicItemSetup> {
	...
	//导出excel时,主模型内容
	List<MESBasicItemSetup> queryResults = new ArrayList<MESBasicItemSetup>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/itemManage/itemSetup/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/itemManage/itemSetup/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_itemManage_ItemSetup" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicItemSetup> itemSetups;
	
	public List<MESBasicItemSetup> getItemSetups() {
		return itemSetups;
	}
	...
	private Page<MESBasicItemSetup> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			itemSetup = new MESBasicItemSetup();
			if(itemSetup.getCalcType() != null) {
				itemSetup.setCalcType((SystemCode) systemCodeService.getSystemCode(itemSetup.getCalcType().getId()));
			}
			if(itemSetup.getDataSouceType() != null) {
				itemSetup.setDataSouceType((SystemCode) systemCodeService.getSystemCode(itemSetup.getDataSouceType().getId()));
			}
			if(itemSetup.getDataType() != null) {
				itemSetup.setDataType((SystemCode) systemCodeService.getSystemCode(itemSetup.getDataType().getId()));
			}
			if(itemSetup.getEnergyType() != null) {
				itemSetup.setEnergyType((SystemCode) systemCodeService.getSystemCode(itemSetup.getEnergyType().getId()));
			}
			if(itemSetup.getFrequency() != null) {
				itemSetup.setFrequency((SystemCode) systemCodeService.getSystemCode(itemSetup.getFrequency().getId()));
			}
			if(itemSetup.getGatherLevel() != null) {
				itemSetup.setGatherLevel((SystemCode) systemCodeService.getSystemCode(itemSetup.getGatherLevel().getId()));
			}
			if(itemSetup.getItemClass() != null) {
				itemSetup.setItemClass((SystemCode) systemCodeService.getSystemCode(itemSetup.getItemClass().getId()));
			}
			if(itemSetup.getPurpose() != null) {
				itemSetup.setPurpose((SystemCode) systemCodeService.getSystemCode(itemSetup.getPurpose().getId()));
			}
			if(itemSetup.getTagType() != null) {
				itemSetup.setTagType((SystemCode) systemCodeService.getSystemCode(itemSetup.getTagType().getId()));
			}
			if(itemSetup.getWorkLine() != null) {
				itemSetup.setWorkLine((SystemCode) systemCodeService.getSystemCode(itemSetup.getWorkLine().getId()));
			}
		} else {
			if(id > 0){
				itemSetup = itemSetupService.getItemSetup(id, viewCode);
			}
		}
	...
	public Page<MESBasicItemSetup> getPage() {
		if (null == page) {
			page = new Page<MESBasicItemSetup>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/itemManage/itemSetup/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = itemSetupService.getItemSetupAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/itemManage/itemSetup/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String itemSetupReferenceCopy() throws Exception{
			MESBasicItemSetup oldItemSetup = null;
			if(isEditNew()){
				...
				oldItemSetup = new MESBasicItemSetup();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,itemSetup.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, itemSetup.getId());
				...
				oldItemSetup = itemSetupService.getItemSetup(id);
			}
			
			
			itemSetup = itemSetupService.getItemSetup(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_itemManage_ItemSetup");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicItemSetup.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldItemSetup, null);
						Method setValue = MESBasicItemSetup.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(itemSetup, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			itemSetup.setCreateDepartmentId(oldItemSetup.getCreateDepartmentId());
			itemSetup.setCreatePositionId(oldItemSetup.getCreatePositionId());
			itemSetup.setCreateStaffId(oldItemSetup.getCreateStaffId());
			itemSetup.setOwnerDepartmentId(oldItemSetup.getOwnerDepartmentId());
			itemSetup.setOwnerPositionId(oldItemSetup.getOwnerPositionId());
			itemSetup.setOwnerStaffId(oldItemSetup.getOwnerStaffId());
			itemSetup.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_itemManage_ItemSetup", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemManage/itemSetup/itemEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemEdit.ftl") }),
		@Action(value = "/MESBasic/itemManage/itemSetup/itemEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=itemSetupService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return itemSetupReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			itemSetup = itemSetupService.getItemSetup(this.itemSetup.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,itemSetup.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemManage/itemSetup/itemView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemView.ftl") }),
		@Action(value = "/MESBasic/itemManage/itemSetup/itemViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemManage/itemSetup/itemList", results = { @Result(type = FREEMARKER, location = "itemList.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/itemManage/itemSetup/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = itemSetupService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(itemSetupService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(itemSetupService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_itemManage_ItemSetup", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemManage/itemSetup/aggregateOnlyRef", results = { @Result(type = FREEMARKER, location = "aggregateOnlyRef.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/energyOnlyRef", results = { @Result(type = FREEMARKER, location = "energyOnlyRef.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemManualOnlyRef", results = { @Result(type = FREEMARKER, location = "itemManualOnlyRef.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemRef", results = { @Result(type = FREEMARKER, location = "itemRef.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemRefForContainer", results = { @Result(type = FREEMARKER, location = "itemRefForContainer.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemSelfList", results = { @Result(type = FREEMARKER, location = "itemSelfList.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemSelfRef", results = { @Result(type = FREEMARKER, location = "itemSelfRef.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/nonEnergyOnlyRef", results = { @Result(type = FREEMARKER, location = "nonEnergyOnlyRef.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/singleItemRef", results = { @Result(type = FREEMARKER, location = "singleItemRef.ftl") })
		,
		@Action(value = "/MESBasic/itemManage/itemSetup/switcherOnlyRef", results = { @Result(type = FREEMARKER, location = "switcherOnlyRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemManage/itemSetup/itemList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="itemList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"itemSetup\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"itemSetup\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"itemSetup\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"itemSetup\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemManage/itemSetup/itemList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemManage/itemSetup/aggregateOnlyRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="aggregateOnlyRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/energyOnlyRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.energyType.id,result.energyType.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemClass.id,result.itemClass.value,result.tagType.id,result.tagType.value,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="energyOnlyRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemManualOnlyRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="itemManualOnlyRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="itemRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemRefForContainer-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="itemRefForContainer-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemSelfList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="itemSelfList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/itemSelfRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="itemSelfRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/nonEnergyOnlyRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="nonEnergyOnlyRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/singleItemRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="singleItemRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/itemManage/itemSetup/switcherOnlyRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.itemNumber,result.itemName,result.tagType.id,result.tagType.value,result.itemClass.id,result.itemClass.value,result.virtualTag,result.itemPlace.placeName,result.crtValue,result.updatTime,result.itemPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="switcherOnlyRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/itemManage/itemSetup/allTempleteDownload", results = { @Result(name="excel",type="excel",location="itemList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_itemManage_ItemSetup");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		itemSetupService.findDataGridPage(dg,dgclass,getDgPage(),itemSetup,"", null);
	...
	private Page<MESBasicItemPart> dg1458633704446Page;
	public Page<MESBasicItemPart> getDg1458633704446Page(){
		if(null == dg1458633704446Page){
			dg1458633704446Page = new Page<MESBasicItemPart>(1,20);
		}
		return dg1458633704446Page;
	}
	
	private Class dg1458633704446Clazz = MESBasicItemPart.class;
	
	public Class getDg1458633704446Clazz() {
		return dg1458633704446Clazz;
	}
	
	private List<MESBasicItemPart> dg1458633704446List;
	public List<MESBasicItemPart> getDg1458633704446List(){
		if(null == dg1458633704446List){
			dg1458633704446List = new ArrayList<MESBasicItemPart>();
		}
		return dg1458633704446List;
	}
	public void setDg1458633704446ListJson(String json) {
		//dg1458633704446List = (List<MESBasicItemPart>) itemSetupService.generateObjectFromJson(json, MESBasicItemPart.class);
	}
	
	private List<Long> dg1458633704446DeletedIds;
	public List<Long> getDg1458633704446DeletedIds(){
		if(null == dg1458633704446DeletedIds){
			dg1458633704446DeletedIds = new ArrayList<Long>();
		}
		return dg1458633704446DeletedIds;
	}

	@Actions({
		
		@Action(value = "/MESBasic/itemManage/itemSetup/data-dg1458633704446", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.itemView.code,result.itemView.itemName,result.itemView.itemNumber,result.itemNumber,result.itemStatus.id,result.itemStatus.value,result.id,result.version,result.itemView.id,result.item.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1458633704446-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1458633704446.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(itemSetup.getId() != null && itemSetup.getId() != -1){
				itemSetup = itemSetupService.getItemSetup(itemSetup.getId());
			}
			itemSetupService.findDataGridPage(dg,dgclass,getDgPage(),itemSetup,searchCondition,params);
	...
	private void save() throws Exception {
		...
		itemSetupService.saveItemSetup(itemSetup,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/itemManage/itemSetup/itemEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/itemManage/itemSetup/itemView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "submit");
		bap_validate_datagrids=itemSetupService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", itemSetup.getId());
		...
		fileuploads.put("linkId", itemSetup.getId());
		bapAfterCustomMethod("MESBasic_1_itemManage_ItemSetup", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/itemManage/itemSetup/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		itemSetupService.deleteItemSetup(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_itemManage_ItemSetup", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/itemManage/itemSetup/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "restore");
		itemSetupService.restoreItemSetup(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_itemManage_ItemSetup", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/itemManage/itemSetup/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/itemManage/itemSetup/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "showDealInfoData");
		if (null != tableInfoId) {
			int count=itemSetupService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = itemSetupService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = itemSetupService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_itemManage_ItemSetup", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicItemSetup> findExportDataInfos(Page<MESBasicItemSetup> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = itemSetupService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = itemSetupService.getAssProperty(p.getCode());	
									//String assModelCode = itemSetupService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			itemSetupService.findItemSetups(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			itemSetupService.findItemSetups(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicItemSetup> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicItemSetup result : results){
	...
	public boolean isEditNew(){
		//return null == this.itemSetup || null == this.itemSetup.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/itemManage/itemSetup/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_itemManage_ItemSetup", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/itemManage/itemSetup/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/itemManage/itemSetup/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/itemManage/itemSetup/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicItemSetupService"
						, "com.supcon.orchid.MESBasic.services.MESBasicItemPartService"      
						, "com.supcon.orchid.MESBasic.services.MESBasicItemPartService"
					, "com.supcon.orchid.MESBasic.services.MESBasicItemPartService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_itemManage_ItemSetup", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/itemManage/itemSetup/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicItemSetupService"
						, "com.supcon.orchid.MESBasic.services.MESBasicPlaceManageService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_itemManage_ItemSetup", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_itemManage_ItemSetup", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/itemManage/itemSetup/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/itemManage/itemSetup/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_itemManage_ItemSetup";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_itemManage_ItemSetup", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/itemManage/itemSetup/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == itemSetup.getCode() ? "" : itemSetup.getCode().toString());
		_auditLog.setColumnStringA(null == itemSetup.getItemName() ? "" : itemSetup.getItemName().toString());
		_auditLog.setColumnStringC(itemSetup.getId().toString());
		_auditLog.setColumnStringD(MESBasicItemSetup.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicItemSetupAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, itemSetup,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, itemSetup,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,itemSetup);
		...
		printlog.setBusinessId(itemSetup.getId());
		printlog.setBussinessKey(null == itemSetup.getCode() ? "" : itemSetup.getCode().toString());
		printlog.setMainDisplay(null == itemSetup.getItemName() ? "" : itemSetup.getItemName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/itemManage/itemSetup/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(itemSetup,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/itemManage/itemSetup/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(itemSetupService.getItemSetup(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/itemManage/itemSetup/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/itemManage/itemSetup/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, itemSetup,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				itemSetup = itemSetupService.getItemSetup(Long.parseLong(arrId[i]));
				if(null != itemSetup){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == itemSetup.getItemName() ? "" : "," + itemSetup.getItemName().toString()));
					} else {
						_auditLog.setColumnStringA(null == itemSetup.getItemName() ? "" : itemSetup.getItemName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == itemSetup.getCode() ? "" : "," + itemSetup.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == itemSetup.getCode() ? "" : itemSetup.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == itemSetup.getId() ? "" : "," + itemSetup.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == itemSetup.getId() ? "" : itemSetup.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, itemSetup,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,itemSetup);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/itemManage/itemSetup/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicItemSetup> findExportPageInfos(Page<MESBasicItemSetup> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_itemManage" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = itemSetupService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicItemSetup queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = itemSetupService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = itemSetupService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_itemManage_ItemSetup");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_itemManage_ItemSetup")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_itemManage_ItemSetup");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_itemManage_ItemSetup");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = itemSetupService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_itemManage_ItemSetup,MESBasic_1) */
// 自定义代码
        if ("submit".equals(methodName) && "MESBasic_1_itemManage_ItemSetup".equals(code)) {
            ...
            if (itemSetup.getVirtualTag()) {
                ...
                if (null != formulaAuto && formulaAuto.length() > 0) {
                    ...
                    for (String s : codes) {
                        MESBasicItemSetup itemSetup = itemSetupDao.findEntityByCriteria(Restrictions.eq("code", s), Restrictions.eq("valid", true));
                        if (itemSetup == null) {
                            if (errorMsgAuto.length() == 0) {
                                errorMsgAuto.append("计算公式中的编码为[" + s + "]的位号不存在");
                            } else {
                                errorMsgAuto.append("</br>计算公式中的编码为[" + s + "]的位号不存在");
                            }
                            if (errorMsgAuto.length() > 0) {
                        ...
                        formulaAuto = formulaAuto.replaceAll("\\[" + s + "\\]", "1");
                    }

                }

                // 若勾选[手动设置相关测点], 不进行公式的模拟计算 modified by ww 2019-05-10
                if (!itemSetup.getManuallySet()) {
                    try {
                        ...
                        errorMsgAuto.append("计算公式中存在错误，请检查。");
                    }
                    if (errorMsgAuto.length() > 0) {
                        throw new BAPException(errorMsgAuto.toString());
                    }
            ...
            } else if (StringUtil.isNotEmptyOrNull(itemSetup.getFactor())) {
                String factor = itemSetup.getFactor();
                factor = factor.replaceAll("n", "1");

                try {
                    Calculator.scalculateJS(factor);
                } catch (Exception ex) {
                    if (errorMsgAuto.length() > 0) {
                        errorMsgAuto.append("</br>");
                    }
                    errorMsgAuto.append("修正公式中存在错误，请检查。");
                }
                if (errorMsgAuto.length() > 0) {
                    throw new BAPException(errorMsgAuto.toString());
                }
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_itemManage_ItemSetup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_itemManage_ItemSetup,MESBasic_1) */
// 自定义代码
	
	
	
	@Action(value = "/public/MESBasic/itemManage/itemSetup/itemSetupDataSync")
	public void itemDataSync(){
		itemSetupService.syncItemSetupData();
	}
	@Autowired
