...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicBatchInformationDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_batchInformationService")
@Transactional
public class MESBasicBatchInformationServiceImpl  extends BaseServiceImpl<MESBasicBatchInformation> implements MESBasicBatchInformationService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicBatchInformation> bapBaseService;
	...
	@Autowired
	private MESBasicBatchInformationImportService batchInformationImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicBatchInformation.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchInformationDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicBatchInformation.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchInformationDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBatchInformation.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBatchInformation.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchInformationDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicBatchInformation.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicBatchInformation.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicBatchInformation> list = (List<MESBasicBatchInformation>) batchInformationDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchInformation getBatchInformation(long id){
		return getBatchInformation(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchInformation getBatchInformation(long id, String viewCode){
		MESBasicBatchInformation batchInformation = batchInformationDao.load(id);

		// 一对多情况处理
		if(batchInformation != null){
			this.setBatchInfoIDDealDept(batchInformation, viewCode);
			this.setBatchInfoIDDealStaff(batchInformation, viewCode);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchInformationAsJSON(long id, String include){
		MESBasicBatchInformation batchInformation = batchInformationDao.load(id);
		// 一对多情况处理
			this.setBatchInfoIDDealDept(batchInformation);
			this.setBatchInfoIDDealStaff(batchInformation);

		if(batchInformation == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(batchInformation, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.common.delete",operType="3")
	public void deleteBatchInformation(MESBasicBatchInformation batchInformation){
		if(null != batchInformation && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchInformation.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
			AuditUtil.setColumnStringA(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
			AuditUtil.setColumnStringC(batchInformation.getId().toString());
		}
		beforeDeleteBatchInformation(batchInformation);
		batchInformationDao.delete(batchInformation);
		afterDeleteBatchInformation(batchInformation);
		batchInformationDao.flush();
		List<MESBasicBatchInformation> params = new ArrayList<MESBasicBatchInformation>();
		List<MESBasicBatchDealInfo> batchDealInfoList = batchDealInfoDao.findByCriteria(Restrictions.eq("batchInfoID", batchInformation));
		batchInformation.setBatchDealInfoList(batchDealInfoList);
		params.add(batchInformation);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchInfo");
		props.put("eventType", "delete");
		String delIds = "";
		// 一对多情况处理
		delIds = batchInformation.getBatchInfoIDDealDeptmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			...
			if (!idList.isEmpty()) {
				List<MESBasicBatchDealInfo> tmpList = batchDealInfoDao.findByCriteria(Restrictions.eq("batchInfoID", batchInformation), Restrictions.in("dealDept.id", idList), Restrictions.eq("valid", true));
				for (MESBasicBatchDealInfo item : tmpList) {
					batchDealInfoDao.deletePhysical(item);
				}
			}
		}
		delIds = batchInformation.getBatchInfoIDDealStaffmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<MESBasicBatchDealInfo> tmpList = batchDealInfoDao.findByCriteria(Restrictions.eq("batchInfoID", batchInformation), Restrictions.in("dealStaff.id", idList), Restrictions.eq("valid", true));
				for (MESBasicBatchDealInfo item : tmpList) {
					batchDealInfoDao.deletePhysical(item);
				}
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.common.delete",operType="3")
	public void deleteBatchInformation(List<Long> batchInformationIds){
		deleteBatchInformation(batchInformationIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.common.delete",operType="3")
	public void deleteBatchInformation(List<Long> batchInformationIds, String eventTopic) {
		List<MESBasicBatchInformation> batchInformations = new ArrayList<MESBasicBatchInformation>();
		for(Long batchInformationId : batchInformationIds){
			MESBasicBatchInformation batchInformation = getBatchInformation(batchInformationId);
			batchInformations.add(batchInformation);
			if(null != batchInformation && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchInformation.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchInformation.getId() ? "" : "," + batchInformation.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchInformation.getId() ? "" : "," + batchInformation.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchInformation.getId() ? "" : "," + batchInformation.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
				}
			}
		}
		
		for(MESBasicBatchInformation batchInformation : batchInformations){
			beforeDeleteBatchInformation(batchInformation);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchInformations != null && batchInformations.size() > 0 ){
			for(MESBasicBatchInformation batchInformation : batchInformations){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchInformation, batchInformation.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(batchInformationIds != null && batchInformationIds.size() > 0) {
			String hql = "update " + MESBasicBatchInformation.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = batchInformationDao.createQuery(hql);
			query.setParameterList("ids", batchInformationIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicBatchInformation batchInformationz : batchInformations){
				batchInformationz.setValid(false);
				List<MESBasicBatchInformation> params = new ArrayList<MESBasicBatchInformation>();
				List<MESBasicBatchDealInfo> batchDealInfoList = batchDealInfoDao.findByCriteria(Restrictions.eq("batchInfoID", batchInformationz));
				batchInformationz.setBatchDealInfoList(batchDealInfoList);
				params.add(batchInformationz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_batchInfo");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.common.delete",operType="3")
	public void deleteBatchInformation(long batchInformationId, int batchInformationVersion){
		this.deleteBatchInformation(batchInformationId, batchInformationVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.common.delete",operType="3")
	public void deleteBatchInformation(long batchInformationId, int batchInformationVersion,SignatureLog signatureLog){
		MESBasicBatchInformation batchInformation = getBatchInformation(batchInformationId);
		if(batchInformation != null && !batchInformation.isValid()){
		...
		beforeDeleteBatchInformation(batchInformation);
		batchInformationDao.delete(batchInformationId, batchInformationVersion);
		afterDeleteBatchInformation(batchInformation);
		batchInformationDao.flush();
		List<MESBasicBatchInformation> params = new ArrayList<MESBasicBatchInformation>();
		List<MESBasicBatchDealInfo> batchDealInfoList = batchDealInfoDao.findByCriteria(Restrictions.eq("batchInfoID", batchInformation));
		...
		if(signatureLog != null ) {
			...
			businessKey=batchInformation.getId();
			signatureLog.setTableId(batchInformation.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.common.delete",operType="3")
	public void deleteBatchInformation(String batchInformationIds){
		this.deleteBatchInformation(batchInformationIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.common.delete",operType="3")
	public void deleteBatchInformation(String batchInformationIds,SignatureLog signatureLog){
		deleteCollection(batchInformationIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchInformation(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_batchInfo#modelCode:MESBasic_1_batchInfo_BatchInformation#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="ec.engine.restore",operType="10")
	public void restoreBatchInformation(String batchInformationIds){
		restoreCollection(batchInformationIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchInformation(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="ec.engine.restore",operType="10")
	public void restoreBatchInformation(long batchInformationId){
		
		findBusinessKeyUsed(batchInformationId);	//判断业务主键是否重复
		...
		beforeRestoreBatchInformation(batchInformation);
		batchInformation.setValid(true);
		batchInformationDao.update(batchInformation);
		afterRestoreBatchInformation(batchInformation);
		List<MESBasicBatchInformation> params = new ArrayList<MESBasicBatchInformation>();
		List<MESBasicBatchDealInfo> batchDealInfoList = batchDealInfoDao.findByCriteria(Restrictions.eq("batchInfoID", batchInformation));
		batchInformation.setBatchDealInfoList(batchDealInfoList);
		params.add(batchInformation);
	...
	public void findBusinessKeyUsed(long batchInformationId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_batchInfo_BatchInformation");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicBatchInformation.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicBatchInformation.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchInformationDao.createNativeQuery(sql,batchInformationId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchInformation(List<MESBasicBatchInformation>  batchInformations){
		for(MESBasicBatchInformation batchInformation:batchInformations)  {
			saveBatchInformation(batchInformation, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchInformation(List<MESBasicBatchInformation>  batchInformations){
		for(MESBasicBatchInformation batchInformation:batchInformations)  {
			saveBatchInformation(batchInformation, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="ec.flow.save")
	public void saveBatchInformation(MESBasicBatchInformation batchInformation, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchInformation(batchInformation, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="ec.flow.save")
	public void saveBatchInformation(MESBasicBatchInformation batchInformation, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchInformation(batchInformation, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchInformationCodes(MESBasicBatchInformation batchInformation) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchInformationCodes(batchInformation, false);
	}
	
	public void generateBatchInformationCodes(MESBasicBatchInformation batchInformation, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchInformationSummarys(MESBasicBatchInformation batchInformation) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchInformationSummarys(batchInformation, false);
	}

	public void generateBatchInformationSummarys(MESBasicBatchInformation batchInformation, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation")
	public void saveBatchInformation(MESBasicBatchInformation batchInformation,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchInformation(batchInformation, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation")
	public void saveBatchInformation(MESBasicBatchInformation batchInformation,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_batchInfo";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchInfo");
		String url = null;
		if(batchInformation.getId() != null && batchInformation.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchInformation.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_batchInfo/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchInformation.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_batchInfo/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchInformation);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchInformation);
		beforeSaveBatchInformation(batchInformation, viewIsView);

		if (viewIsView) {
			batchInformationDao.saveWithRevertVersion(batchInformation);
		} else {
			if(isNew)
				batchInformationDao.save(batchInformation);
			else
				batchInformationDao.merge(batchInformation);
		}
		if(null != batchInformation && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchInformation.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
			AuditUtil.setColumnStringA(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
		...
		dealDatagridsSave(batchInformation,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
			this.dealBatchInfoIDDealDept(batchInformation);
			this.dealBatchInfoIDDealStaff(batchInformation);
		
		// 根据配置规则生成编码
		try {
			generateBatchInformationCodes(batchInformation, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchInformationSummarys(batchInformation, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchInformation(batchInformation, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchInformation);
			}
		}
		List<MESBasicBatchInformation> params = new ArrayList<MESBasicBatchInformation>();
		batchInformationDao.flush();
		batchInformationDao.clear();
		batchInformation = batchInformationDao.load(batchInformation.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicBatchInformation batchInformation){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchInformation", batchInformation);
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation")
	public void mergeBatchInformation(MESBasicBatchInformation batchInformation, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchInformation);
		beforeSaveBatchInformation(batchInformation);
		batchInformationDao.merge(batchInformation);
		if(null != batchInformation && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchInformation.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
			AuditUtil.setColumnStringA(null == batchInformation.getId() ? "" : batchInformation.getId().toString());
		...
		dealDatagridsSave(batchInformation,null,dgLists,dgDeleteIDs,assFileUploads);

			// 一对多情况处理
			this.dealBatchInfoIDDealDept(batchInformation);
			this.dealBatchInfoIDDealStaff(batchInformation);
		afterSaveBatchInformation(batchInformation);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicBatchInformation> findBatchInformations(Page<MESBasicBatchInformation> page, Criterion... criterions) {
		return batchInformationDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicBatchInformationDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicBatchInformationDealInfo> list=batchInformationDao.createQuery("from " + MESBasicBatchInformationDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicBatchInformationDealInfo dealInfo) {
		batchInformationDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicBatchInformationDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) batchInformationDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicBatchInformationDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = batchInformationDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = batchInformationDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicBatchInformationDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = batchInformationDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchInformations(Page<MESBasicBatchInformation> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchInformations(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchInformations(Page<MESBasicBatchInformation> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchInformations(Page<MESBasicBatchInformation> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicBatchInformation> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchInformation\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchInformation\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchInformation\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_batchInfo_BatchInformation" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_batchInfo_BatchInformation" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  batchInformationDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchInformationDao.createNativeQuery(powerOperateSql, "MESBasic_1_batchInfo").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchInformation\"","MESBasic_1_batchInfo",customCondition,"BATCH_INFORMATION",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"batchInformation\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicBatchInformationDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"batchInformation\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchInformation\".LAY_REC ASC, \"batchInformation\".SORT ASC");
				} else {
					orderPart.append(" \"batchInformation\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchInformationDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchInformationDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchInformationDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicBatchInformation> result = new ArrayList<MESBasicBatchInformation>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicBatchInformation.class,batchInformationDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicBatchInformation.class,batchInformationDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicBatchInformation batchInformation : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchInformation.getId(), "MESBasic_batchInfo_batchInformation");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchInformation.getId(), "MESBasic_batchInfo_batchInformation");
							if(null != documents && !documents.isEmpty()) {
								batchInformation.setDocument(documents.get(0));
								batchInformation.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_batchInfo")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_batchInfo")+"MESBasic_1_batchInfo".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_batchInfo",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicBatchInformation> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicBatchInformation> result) {
		View view = null;
		...
		Map<Long, MESBasicBatchInformation> rsMap = new HashMap<Long, MESBasicBatchInformation>();
		for (MESBasicBatchInformation batchInformation : result) {
			ids.add(batchInformation.getId());
			rsMap.put(batchInformation.getId(), batchInformation);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchInformationDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchInformationDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicBatchInformation batchInformation,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(batchInformation.getOwnerStaffId());
		ti.setOwnerPositionId(batchInformation.getOwnerPositionId());
		ti.setOwnerDepartmentId(batchInformation.getOwnerDepartmentId());
		
		ti.setCreateStaff(batchInformation.getCreateStaff());
		ti.setCreateStaffId(batchInformation.getCreateStaffId());
		ti.setCreateTime(batchInformation.getCreateTime());
		ti.setDeleteStaff(batchInformation.getDeleteStaff());
		ti.setDeleteStaffId(batchInformation.getDeleteStaffId());
		ti.setDeleteTime(batchInformation.getDeleteTime());
		ti.setCreateDepartmentId(batchInformation.getCreateDepartmentId());
		ti.setModifyStaff(batchInformation.getModifyStaff());
		ti.setModifyStaffId(batchInformation.getModifyStaffId());
		ti.setCreatePositionId(batchInformation.getCreatePositionId());
		ti.setPositionLayRec(batchInformation.getPositionLayRec());
		ti.setTableNo(batchInformation.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_batchInfo");
		ti.setTargetTableName("BATCH_INFORMATION");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_batchInfo_BatchInformation" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchInformationDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchInformation\"","MESBasic_1_batchInfo","","BATCH_INFORMATION",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchInformation\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchInformation\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchInformation\"","MESBasic_1_batchInfo","","BATCH_INFORMATION",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("batchInformation.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchInformation\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchInformation\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchInformation\".CID = 1 OR \"batchInformation\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchInformation.mc\"").append(".BATCH_INFORMATION) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchInformationDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("batchInfo", "MESBasic_1_batchInfo", "batchInfo_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchInformationDao.createNativeQuery(sql, menuCode);
	...
	// ================ 生成多选控件数据保存方法 start====================
	private void dealBatchInfoIDDealDept(MESBasicBatchInformation batchInformation) {
		String addIds = batchInformation.getBatchInfoIDDealDeptAddIds();
		String delIds = batchInformation.getBatchInfoIDDealDeptDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					MESBasicBatchDealInfo tmp = new MESBasicBatchDealInfo();
					tmp.setBatchInfoID(batchInformation);
					Department indirectObj = new Department();
					indirectObj.setId(Long.valueOf(item));
					tmp.setDealDept(indirectObj);
					tmp.setCid(batchInformation.getCid());
					batchDealInfoDao.save(tmp);
				}
			}
		}
	}

	private void setBatchInfoIDDealDept(MESBasicBatchInformation batchInformation) {
		setBatchInfoIDDealDept(batchInformation, null);
	}

	private void setBatchInfoIDDealDept(MESBasicBatchInformation batchInformation, String viewCode) {
		List<MESBasicBatchDealInfo> tmpList = batchDealInfoDao.findByCriteria(Order.asc("id"), Restrictions.eq("batchInfoID", batchInformation), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (MESBasicBatchDealInfo item : tmpList) {
			if(item.getDealDept() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getDealDept().getId();
				String displayField = "name";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getDealDept(), displayField);
			}
		}
		batchInformation.setBatchDealInfoList(tmpList);
		batchInformation.setBatchInfoIDDealDeptmultiselectIDs(ids);
		batchInformation.setBatchInfoIDDealDeptmultiselectNames(names);
	}
	private void dealBatchInfoIDDealStaff(MESBasicBatchInformation batchInformation) {
		String addIds = batchInformation.getBatchInfoIDDealStaffAddIds();
		String delIds = batchInformation.getBatchInfoIDDealStaffDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
	...
	private void setBatchInfoIDDealStaff(MESBasicBatchInformation batchInformation) {
		setBatchInfoIDDealStaff(batchInformation, null);
	}

	private void setBatchInfoIDDealStaff(MESBasicBatchInformation batchInformation, String viewCode) {
		List<MESBasicBatchDealInfo> tmpList = batchDealInfoDao.findByCriteria(Order.asc("id"), Restrictions.eq("batchInfoID", batchInformation), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (MESBasicBatchDealInfo item : tmpList) {
			if(item.getDealStaff() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getDealStaff().getId();
				String displayField = "name";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getDealStaff(), displayField);
			}
		}
		batchInformation.setBatchDealInfoList(tmpList);
		batchInformation.setBatchInfoIDDealStaffmultiselectIDs(ids);
		batchInformation.setBatchInfoIDDealStaffmultiselectNames(names);
	}
	// ================ 生成多选控件数据保存方法 end ====================

	protected String getEntityCode(){return MESBasicBatchInformation.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicBatchInformation.MODULE_CODE;}
	protected String getModelCode(){return MESBasicBatchInformation.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchInformation 模型实例
	 */
	public void checkUniqueConstraint(MESBasicBatchInformation batchInformation){
	}
	
	
	@Override
	public List<MESBasicBatchInformation> getBatchInformations(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchInformation.class);
		if(params!=null){
			...
			return batchInformationDao.findByHql("from " + MESBasicBatchInformation.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchInformationDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicBatchInformation> findBatchInformationsByHql(String hql, Object... objects){
		...
		return   batchInformationDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicBatchInformation loadBatchInformationByBussinessKey(MESBasicBatchInformation batchInformation){
		return loadBatchInformationByBussinessKey(batchInformation.getId());
	}
	
	@Override
	public Page<MESBasicBatchInformation> getBatchInformations(Page<MESBasicBatchInformation> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchInformation.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchInformationDao.findByPage(page, sql);
			}else{
				page = batchInformationDao.findAllByPage(page);
			}
			return page;
		}
		page = batchInformationDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public MESBasicBatchInformation loadBatchInformationByBussinessKey(Serializable bussinessKey){
		return batchInformationDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicBatchInformation> findByProperty(String propertyName, Object object){
		return batchInformationDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchInformation findEntityByProperty(String propertyName, Object object){
		return batchInformationDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicBatchInformation.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchInformationDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_batchInfo_BatchInformation","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_batchInfo", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicBatchInformation batchInformation,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchInformationDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicBatchInformation.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchInformation);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchInformation.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBatchInformation.TABLE_NAME +" where valid = 1";
			list = batchInformationDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBatchInformation.TABLE_NAME;
			list = batchInformationDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicBatchInformation.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchInformationDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicBatchInformationDaoImpl batchInformationDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchInformation(final List<MESBasicBatchInformation> insertObjs, final List<MESBasicBatchInformation> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchInformation(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchInformation(final List<MESBasicBatchInformation> insertObjs, final List<MESBasicBatchInformation> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1490773101098"),importCount));
		...
		Map<Long, MESBasicBatchInformation> oldUpdateObjsMap = new HashMap<Long, MESBasicBatchInformation>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicBatchInformation bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicBatchInformation> bizObjList = batchInformationDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicBatchInformation bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicBatchInformation> nmeObjs = new ArrayList<MESBasicBatchInformation>();
		...
		long id=autoGeneratorID.getNextId(MESBasicBatchInformation.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicBatchInformation batchInformation:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicBatchInformation.TABLE_NAME,1,dbId);
			if(batchInformation.getId() != null ){
				m.put(batchInformation.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			batchInformation.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchInformationDao.createNativeQuery(sql, "MESBasic_1_batchInfo_BatchInformation"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchInformationDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicBatchInformation.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicBatchInformation batchInformation = updateObjs.get(updateIndex);
						Long id = batchInformation.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchInformation.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchInformation);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicBatchInformation.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicBatchInformation.TABLE_NAME + " order by id desc";
//					Query query = batchInformationDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicBatchInformation batchInformation = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchInformation.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchInformationDaoImpl.getSession().flush();
		batchInformationDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicBatchInformation bizObj = insertObjs.get(insertIndex);
			batchInformationImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicBatchInformation bizObj = updateObjs.get(updateIndex);
			MESBasicBatchInformation oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchInformationImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = batchInformationDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				batchInformationDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = batchInformationDao.createNativeQuery(sql, "MESBasic_1_batchInfo_BatchInformation").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchInformationDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchInformationDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchInformationDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchInformationDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchInformationDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchInformationDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="ec.flow.save")
	public void saveBatchInformation(MESBasicBatchInformation batchInformation, DataGridService dataGridService){
		saveBatchInformation(batchInformation, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="ec.flow.save")
	public void saveBatchInformation(MESBasicBatchInformation batchInformation, DataGridService dataGridService, String viewCode){
		saveBatchInformation(batchInformation, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation")
	public void saveBatchInformation(MESBasicBatchInformation batchInformation, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchInformation(batchInformation, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation")
	public void saveBatchInformation(MESBasicBatchInformation batchInformation, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_batchInfo";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchInfo");
		String url = null;
		if(null != batchInformation.getId() && batchInformation.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchInformation.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation")
	public Map<Object, Object> batchInformationDataGridImport(MESBasicBatchInformation batchInformation, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchInformation.getVirtualId();
		
		batchInformation = batchInformationDao.load(batchInformation.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchInformation.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchInformation.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchInformation);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchInformation.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation")
	public void mergeBatchInformation(MESBasicBatchInformation batchInformation, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchInformation);
		beforeSaveBatchInformation(batchInformation);
		batchInformationDao.merge(batchInformation);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchInformation",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicBatchInformation> getByPage(Page<MESBasicBatchInformation> page,DetachedCriteria detachedCriteria){
		return batchInformationDao.findByPage(page, detachedCriteria);
	}
	@Override
	public List<MESBasicBatchDealInfo> getBatchDealInfoList(MESBasicBatchInformation batchInformation){
		return batchDealInfoDao.findByCriteria(Restrictions.eq("batchInfoID", batchInformation),Restrictions.eq("valid", true));
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_batchInfo").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBatchInformation(MESBasicBatchInformation batchInformation, Object... objects){
		checkUniqueConstraint(batchInformation);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchInformation(MESBasicBatchInformation batchInformation, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchInformation(MESBasicBatchInformation batchInformation){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchInformation(MESBasicBatchInformation batchInformation){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchInformation(MESBasicBatchInformation batchInformation){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchInformation(MESBasicBatchInformation batchInformation){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicBatchInformation batchInformation, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicBatchInformation batchInformation, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportBatchInformation(List<MESBasicBatchInformation> insertObjs, List<MESBasicBatchInformation> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchInformation,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportBatchInformation(List<MESBasicBatchInformation> insertObjs, List<MESBasicBatchInformation> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchInformation,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_batchInfo_BatchInformation,MESBasic_1) */
// 自定义代码
