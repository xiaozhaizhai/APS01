...
/* CUSTOM CODE START(action,import,MESBasic_1_baseCustomerClass_BaseCustomerClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicBaseCustomerClassAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBaseCustomerClass> {
	...
	//导出excel时,主模型内容
	List<MESBasicBaseCustomerClass> queryResults = new ArrayList<MESBasicBaseCustomerClass>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseCustomerClass/baseCustomerClass/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseCustomerClass/baseCustomerClass/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_baseCustomerClass_BaseCustomerClass" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBaseCustomerClass> baseCustomerClasss;
	
	public List<MESBasicBaseCustomerClass> getBaseCustomerClasss() {
		return baseCustomerClasss;
	}
	...
	private Page<MESBasicBaseCustomerClass> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			baseCustomerClass = new MESBasicBaseCustomerClass();
		} else {
			if(id > 0){
				baseCustomerClass = baseCustomerClassService.getBaseCustomerClass(id, viewCode);
			}
		}
	...
	public Page<MESBasicBaseCustomerClass> getPage() {
		if (null == page) {
			page = new Page<MESBasicBaseCustomerClass>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = baseCustomerClassService.getBaseCustomerClassAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/edit-extra-col")
	public String _editExtraCol() throws Exception{
		if(!isEditNew()){
			if(baseCustomerClass.getExtraCol()!=null && baseCustomerClass.getExtraCol().length() > 0) {
				Struts2Utils.renderXml(baseCustomerClass.getExtraCol());
				return null;
			}
		}
	 ...
	 * @throws Exception
	 */
		public String baseCustomerClassReferenceCopy() throws Exception{
			MESBasicBaseCustomerClass oldBaseCustomerClass = null;
			if(isEditNew()){
				...
				oldBaseCustomerClass = new MESBasicBaseCustomerClass();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseCustomerClass.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, baseCustomerClass.getId());
				...
				oldBaseCustomerClass = baseCustomerClassService.getBaseCustomerClass(id);
			}
			
			
			baseCustomerClass = baseCustomerClassService.getBaseCustomerClass(refId);
			if (baseCustomerClass.getExtraCol() != null && baseCustomerClass.getExtraCol().length() > 0) {
				baseCustomerClass.setExtraColMap(XmlUtils.xml2Map(baseCustomerClass.getExtraCol()));
			}
			Model model = modelServiceFoundation.getModel("MESBasic_1_baseCustomerClass_BaseCustomerClass");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBaseCustomerClass.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBaseCustomerClass, null);
						Method setValue = MESBasicBaseCustomerClass.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(baseCustomerClass, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			baseCustomerClass.setCreateDepartmentId(oldBaseCustomerClass.getCreateDepartmentId());
			baseCustomerClass.setCreatePositionId(oldBaseCustomerClass.getCreatePositionId());
			baseCustomerClass.setCreateStaffId(oldBaseCustomerClass.getCreateStaffId());
			baseCustomerClass.setOwnerDepartmentId(oldBaseCustomerClass.getOwnerDepartmentId());
			baseCustomerClass.setOwnerPositionId(oldBaseCustomerClass.getOwnerPositionId());
			baseCustomerClass.setOwnerStaffId(oldBaseCustomerClass.getOwnerStaffId());
			baseCustomerClass.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/editCust",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editCust.ftl") }),
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/editCustPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editCustPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=baseCustomerClassService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return baseCustomerClassReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			baseCustomerClass = baseCustomerClassService.getBaseCustomerClass(this.baseCustomerClass.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseCustomerClass.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			baseCustomerClassService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/mainView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainView.ftl") }),
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/mainViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/listCustPart", results = { @Result(type = FREEMARKER, location = "listCustPart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/custClassLayout", results = { @Result(type = FREEMARKER, location = "custClassLayout.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = baseCustomerClassService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(baseCustomerClassService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(baseCustomerClassService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_baseCustomerClass_BaseCustomerClass", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/refCustClaList", results = { @Result(type = FREEMARKER, location = "refCustClaList.ftl") })
		,
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/refCustClaTree", results = { @Result(type = FREEMARKER, location = "refCustClaTree.ftl") })
		,
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/treeCust", results = { @Result(type = FREEMARKER, location = "treeCust.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/listCustPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.ccCode,result.ccName,result.ccMemo,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="listCustPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"baseCustomerClass\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"baseCustomerClass\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"baseCustomerClass\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"baseCustomerClass\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			baseCustomerClassService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/listCustPart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/refCustClaList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.ccCode,result.ccName,result.ccMemo,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refCustClaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/refCustClaTreeDrag")
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/treeCustDrag")
	})
	public String viewTreeDrag() throws Exception{
		baseCustomerClassService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/custClassLayoutSort")
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/custMneSort")
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/editCustSort")
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/listCustPartSort")
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/mainViewSort")
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/refCustClaListSort")
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/refCustClaTreeSort")
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/treeCustSort")
	})
	...
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/refCustClaTreeData",
			results = { @Result(type = JSON, params = { "root", "baseCustomerClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.ccName"})}
		)
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/treeCustData",
			results = { @Result(type = JSON, params = { "root", "baseCustomerClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.ccName"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"baseCustomerClass\".", "{alias}.");
					}
		...
		baseCustomerClasss = baseCustomerClassService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/refCustClaTreeFullData",
			results = { @Result(type = JSON, params = { "root", "baseCustomerClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.ccName"})}
		)
	,
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/treeCustFullData",
			results = { @Result(type = JSON, params = { "root", "baseCustomerClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.ccName"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			baseCustomerClasss = baseCustomerClassService.getTreeInfo(treeInfo, null, null);
		}
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/allTempleteDownload", results = { @Result(name="excel",type="excel",location="listCustPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_baseCustomerClass_BaseCustomerClass");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		baseCustomerClassService.findDataGridPage(dg,dgclass,getDgPage(),baseCustomerClass,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(baseCustomerClass.getId() != null && baseCustomerClass.getId() != -1){
				baseCustomerClass = baseCustomerClassService.getBaseCustomerClass(baseCustomerClass.getId());
			}
			baseCustomerClassService.findDataGridPage(dg,dgclass,getDgPage(),baseCustomerClass,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == baseCustomerClass.getId()) {
		...
		baseCustomerClassService.saveBaseCustomerClass(baseCustomerClass,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/editCust/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/mainView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "submit");
		bap_validate_datagrids=baseCustomerClassService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", baseCustomerClass.getId());
		...
		fileuploads.put("linkId", baseCustomerClass.getId());
		bapAfterCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		baseCustomerClassService.deleteBaseCustomerClass(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "restore");
		baseCustomerClassService.restoreBaseCustomerClass(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "showDealInfoData");
		if (null != tableInfoId) {
			int count=baseCustomerClassService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = baseCustomerClassService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = baseCustomerClassService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBaseCustomerClass> findExportDataInfos(Page<MESBasicBaseCustomerClass> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = baseCustomerClassService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = baseCustomerClassService.getAssProperty(p.getCode());	
									//String assModelCode = baseCustomerClassService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
				...
				if(view.getAssModel().getDataType() == 2 && useForImportFlag){//树型结构且用于导出，加上父节点,同时在sql中加入layNo,layRec,parentId,sort,leaf
					...
					exportSqlPart1Add.append("\".CC_LAYNO AS \"layNo\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
					exportSqlPart1Add.append("\".CC_LAYREC AS \"layRec\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			baseCustomerClassService.findBaseCustomerClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			baseCustomerClassService.findBaseCustomerClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBaseCustomerClass> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBaseCustomerClass result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".CC_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.baseCustomerClass || null == this.baseCustomerClass.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/baseCustomerClass/baseCustomerClass/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBaseCustomerClassService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_baseCustomerClass", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_baseCustomerClass");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_baseCustomerClass_BaseCustomerClass", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_baseCustomerClass_BaseCustomerClass", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_baseCustomerClass_BaseCustomerClass";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_baseCustomerClass_BaseCustomerClass", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicBaseCustomerClass baseCustomerClass = null;
		if(null != id) {
			baseCustomerClass = baseCustomerClassService.getBaseCustomerClass(id);
		}
		...
		_auditLog.setColumnStringB(null == baseCustomerClass.getCcCode() ? "" : baseCustomerClass.getCcCode().toString());
		_auditLog.setColumnStringA(null == baseCustomerClass.getCcName() ? "" : baseCustomerClass.getCcName().toString());
		_auditLog.setColumnStringC(baseCustomerClass.getId().toString());
		_auditLog.setColumnStringD(MESBasicBaseCustomerClass.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_baseCustomerClass");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBaseCustomerClassAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, baseCustomerClass,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, baseCustomerClass,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,baseCustomerClass);
		...
		printlog.setEntity("MESBasic_1_baseCustomerClass");
		...
		printlog.setBusinessId(baseCustomerClass.getId());
		printlog.setBussinessKey(null == baseCustomerClass.getCcCode() ? "" : baseCustomerClass.getCcCode().toString());
		printlog.setMainDisplay(null == baseCustomerClass.getCcName() ? "" : baseCustomerClass.getCcName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(baseCustomerClass,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(baseCustomerClassService.getBaseCustomerClass(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, baseCustomerClass,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				baseCustomerClass = baseCustomerClassService.getBaseCustomerClass(Long.parseLong(arrId[i]));
				if(null != baseCustomerClass){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == baseCustomerClass.getCcName() ? "" : "," + baseCustomerClass.getCcName().toString()));
					} else {
						_auditLog.setColumnStringA(null == baseCustomerClass.getCcName() ? "" : baseCustomerClass.getCcName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == baseCustomerClass.getCcCode() ? "" : "," + baseCustomerClass.getCcCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == baseCustomerClass.getCcCode() ? "" : baseCustomerClass.getCcCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == baseCustomerClass.getId() ? "" : "," + baseCustomerClass.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == baseCustomerClass.getId() ? "" : baseCustomerClass.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, baseCustomerClass,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,baseCustomerClass);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//baseCustomerClassService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicBaseCustomerClass> findExportPageInfos(Page<MESBasicBaseCustomerClass> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_baseCustomerClass" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = baseCustomerClassService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicBaseCustomerClass queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = baseCustomerClassService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = baseCustomerClassService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_baseCustomerClass_BaseCustomerClass");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_baseCustomerClass_BaseCustomerClass")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_baseCustomerClass_BaseCustomerClass");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_baseCustomerClass_BaseCustomerClass");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = baseCustomerClassService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_baseCustomerClass_BaseCustomerClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_baseCustomerClass_BaseCustomerClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_baseCustomerClass_BaseCustomerClass,MESBasic_1) */
// 自定义代码
	...
	@Action(value = "/MESBasic/baseCustomerClass/baseCustomerClass/hasCustomer", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String hasCustomer() {
		String falg = baseCustomerClassService.hasCustomer(customerClassId);
		responseMap.put("hasCustomerFlag", falg);
		return SUCCESS;
	}
/* CUSTOM CODE END */
}