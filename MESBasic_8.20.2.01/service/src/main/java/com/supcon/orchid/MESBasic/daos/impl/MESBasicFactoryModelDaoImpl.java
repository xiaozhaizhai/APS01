...
@Repository
public class MESBasicFactoryModelDaoImpl extends TreeDaoImpl<MESBasicFactoryModel, Long> implements MESBasicFactoryModelDao {
	@Override
	...
	@Override
	public MESBasicFactoryModelDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicFactoryModelDealInfo dealInfo = new MESBasicFactoryModelDealInfo();
	...
	@Override
	public MESBasicFactoryModelDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicFactoryModel mainObj, Staff dealStaff) {
		MESBasicFactoryModelDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicFactoryModelDealInfo.class);
		...
		List<MESBasicFactoryModelDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_factoryModel_FactoryModel,MESBasic_1) */
// 自定义代码
