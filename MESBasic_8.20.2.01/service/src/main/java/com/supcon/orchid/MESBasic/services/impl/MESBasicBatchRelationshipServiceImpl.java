...
import com.supcon.orchid.MESBasic.services.MESBasicBatchRelationshipImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicBatchRelationshipDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_batchRelationshipService")
@Transactional
public class MESBasicBatchRelationshipServiceImpl  extends BaseServiceImpl<MESBasicBatchRelationship> implements MESBasicBatchRelationshipService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicBatchRelationship> bapBaseService;
	...
	@Autowired
	private MESBasicBatchRelationshipImportService batchRelationshipImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicBatchRelationship.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchRelationshipDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicBatchRelationship.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchRelationshipDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBatchRelationship.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicBatchRelationship.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchRelationshipDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicBatchRelationship.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicBatchRelationship.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicBatchRelationship> list = (List<MESBasicBatchRelationship>) batchRelationshipDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchRelationship getBatchRelationship(long id){
		return getBatchRelationship(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchRelationship getBatchRelationship(long id, String viewCode){
		MESBasicBatchRelationship batchRelationship = batchRelationshipDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchRelationshipAsJSON(long id, String include){
		MESBasicBatchRelationship batchRelationship = batchRelationshipDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchRelationship, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.common.delete",operType="3")
	public void deleteBatchRelationship(MESBasicBatchRelationship batchRelationship){
		if(null != batchRelationship && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchRelationship.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
			AuditUtil.setColumnStringA(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
			AuditUtil.setColumnStringC(batchRelationship.getId().toString());
		}
		beforeDeleteBatchRelationship(batchRelationship);
		batchRelationshipDao.deletePhysical(batchRelationship);
		afterDeleteBatchRelationship(batchRelationship);
		batchRelationshipDao.flush();
		List<MESBasicBatchRelationship> params = new ArrayList<MESBasicBatchRelationship>();
		params.add(batchRelationship);
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.common.delete",operType="3")
	public void deleteBatchRelationship(List<Long> batchRelationshipIds){
		deleteBatchRelationship(batchRelationshipIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.common.delete",operType="3")
	public void deleteBatchRelationship(List<Long> batchRelationshipIds, String eventTopic) {
		List<MESBasicBatchRelationship> batchRelationships = new ArrayList<MESBasicBatchRelationship>();
		for(Long batchRelationshipId : batchRelationshipIds){
			MESBasicBatchRelationship batchRelationship = getBatchRelationship(batchRelationshipId);
			batchRelationships.add(batchRelationship);
			if(null != batchRelationship && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchRelationship.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchRelationship.getId() ? "" : "," + batchRelationship.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchRelationship.getId() ? "" : "," + batchRelationship.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchRelationship.getId() ? "" : "," + batchRelationship.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
				}
			}
		}
		
		for(MESBasicBatchRelationship batchRelationship : batchRelationships){
			beforeDeleteBatchRelationship(batchRelationship);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchRelationships != null && batchRelationships.size() > 0 ){
			for(MESBasicBatchRelationship batchRelationship : batchRelationships){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicBatchRelationship.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchRelationship));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchRelationship, batchRelationship.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchRelationshipIds != null && batchRelationshipIds.size() > 0) {
			String hql = "delete " + MESBasicBatchRelationship.JPA_NAME + " where id in(:ids)";
			Query query = batchRelationshipDao.createQuery(hql);
			query.setParameterList("ids", batchRelationshipIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicBatchRelationship batchRelationshipz : batchRelationships){
				batchRelationshipz.setValid(false);
				List<MESBasicBatchRelationship> params = new ArrayList<MESBasicBatchRelationship>();
				params.add(batchRelationshipz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_batchInfo");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.common.delete",operType="3")
	public void deleteBatchRelationship(long batchRelationshipId, int batchRelationshipVersion){
		this.deleteBatchRelationship(batchRelationshipId, batchRelationshipVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.common.delete",operType="3")
	public void deleteBatchRelationship(long batchRelationshipId, int batchRelationshipVersion,SignatureLog signatureLog){
		MESBasicBatchRelationship batchRelationship = getBatchRelationship(batchRelationshipId);
		if(batchRelationship != null && !batchRelationship.isValid()){
		...
		beforeDeleteBatchRelationship(batchRelationship);
		batchRelationshipDao.delete(batchRelationshipId, batchRelationshipVersion);
		afterDeleteBatchRelationship(batchRelationship);
		batchRelationshipDao.flush();
		List<MESBasicBatchRelationship> params = new ArrayList<MESBasicBatchRelationship>();
		params.add(batchRelationship);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchRelationship.getId();
			signatureLog.setTableId(batchRelationship.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.common.delete",operType="3")
	public void deleteBatchRelationship(String batchRelationshipIds){
		this.deleteBatchRelationship(batchRelationshipIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.common.delete",operType="3")
	public void deleteBatchRelationship(String batchRelationshipIds,SignatureLog signatureLog){
		deleteCollection(batchRelationshipIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchRelationship(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_batchInfo#modelCode:MESBasic_1_batchInfo_BatchRelationship#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="ec.engine.restore",operType="10")
	public void restoreBatchRelationship(String batchRelationshipIds){
		restoreCollection(batchRelationshipIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchRelationship(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="ec.engine.restore",operType="10")
	public void restoreBatchRelationship(long batchRelationshipId){
		
		findBusinessKeyUsed(batchRelationshipId);	//判断业务主键是否重复
		...
		beforeRestoreBatchRelationship(batchRelationship);
		batchRelationship.setValid(true);
		batchRelationshipDao.update(batchRelationship);
		afterRestoreBatchRelationship(batchRelationship);
		List<MESBasicBatchRelationship> params = new ArrayList<MESBasicBatchRelationship>();
		params.add(batchRelationship);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchRelationshipId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_batchInfo_BatchRelationship");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicBatchRelationship.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicBatchRelationship.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchRelationshipDao.createNativeQuery(sql,batchRelationshipId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchRelationship(List<MESBasicBatchRelationship>  batchRelationships){
		for(MESBasicBatchRelationship batchRelationship:batchRelationships)  {
			saveBatchRelationship(batchRelationship, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchRelationship(List<MESBasicBatchRelationship>  batchRelationships){
		for(MESBasicBatchRelationship batchRelationship:batchRelationships)  {
			saveBatchRelationship(batchRelationship, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="ec.flow.save")
	public void saveBatchRelationship(MESBasicBatchRelationship batchRelationship, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchRelationship(batchRelationship, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="ec.flow.save")
	public void saveBatchRelationship(MESBasicBatchRelationship batchRelationship, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchRelationship(batchRelationship, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchRelationshipCodes(MESBasicBatchRelationship batchRelationship) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchRelationshipCodes(batchRelationship, false);
	}
	
	public void generateBatchRelationshipCodes(MESBasicBatchRelationship batchRelationship, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchRelationshipSummarys(MESBasicBatchRelationship batchRelationship) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchRelationshipSummarys(batchRelationship, false);
	}

	public void generateBatchRelationshipSummarys(MESBasicBatchRelationship batchRelationship, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship")
	public void saveBatchRelationship(MESBasicBatchRelationship batchRelationship,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchRelationship(batchRelationship, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship")
	public void saveBatchRelationship(MESBasicBatchRelationship batchRelationship,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_batchInfo");
		String url = null;
		if(batchRelationship.getId() != null && batchRelationship.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchRelationship.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchRelationship);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchRelationship);
		beforeSaveBatchRelationship(batchRelationship, viewIsView);

		if (viewIsView) {
			batchRelationshipDao.saveWithRevertVersion(batchRelationship);
		} else {
			if(isNew)
				batchRelationshipDao.save(batchRelationship);
			else
				batchRelationshipDao.merge(batchRelationship);
		}
		if(null != batchRelationship && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchRelationship.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
			AuditUtil.setColumnStringA(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
		...
		dealDatagridsSave(batchRelationship,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchRelationshipCodes(batchRelationship, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchRelationshipSummarys(batchRelationship, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchRelationship(batchRelationship, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchRelationship);
			}
		}
		List<MESBasicBatchRelationship> params = new ArrayList<MESBasicBatchRelationship>();
		batchRelationshipDao.flush();
		batchRelationshipDao.clear();
		batchRelationship = batchRelationshipDao.load(batchRelationship.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicBatchRelationship batchRelationship){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchRelationship", batchRelationship);
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship")
	public void mergeBatchRelationship(MESBasicBatchRelationship batchRelationship, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchRelationship);
		beforeSaveBatchRelationship(batchRelationship);
		batchRelationshipDao.merge(batchRelationship);
		if(null != batchRelationship && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchRelationship.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
			AuditUtil.setColumnStringA(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
		...
		dealDatagridsSave(batchRelationship,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchRelationship(batchRelationship);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicBatchRelationship> findBatchRelationships(Page<MESBasicBatchRelationship> page, Criterion... criterions) {
		return batchRelationshipDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchRelationships(Page<MESBasicBatchRelationship> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchRelationships(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchRelationships(Page<MESBasicBatchRelationship> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchRelationships(Page<MESBasicBatchRelationship> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicBatchRelationship> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchRelationship\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchRelationship\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchRelationship\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "MESBasic_1_batchInfo" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  batchRelationshipDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchRelationshipDao.createNativeQuery(powerOperateSql, "MESBasic_1_batchInfo").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchRelationship\".LAY_REC ASC, \"batchRelationship\".SORT ASC");
				} else {
					orderPart.append(" \"batchRelationship\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchRelationshipDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchRelationshipDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchRelationshipDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicBatchRelationship> result = new ArrayList<MESBasicBatchRelationship>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicBatchRelationship.class,batchRelationshipDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicBatchRelationship.class,batchRelationshipDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicBatchRelationship batchRelationship : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchRelationship.getId(), "MESBasic_batchInfo_batchRelationship");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchRelationship.getId(), "MESBasic_batchInfo_batchRelationship");
							if(null != documents && !documents.isEmpty()) {
								batchRelationship.setDocument(documents.get(0));
								batchRelationship.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicBatchRelationship> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicBatchRelationship> result) {
		View view = null;
		...
		Map<Long, MESBasicBatchRelationship> rsMap = new HashMap<Long, MESBasicBatchRelationship>();
		for (MESBasicBatchRelationship batchRelationship : result) {
			ids.add(batchRelationship.getId());
			rsMap.put(batchRelationship.getId(), batchRelationship);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchRelationshipDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchRelationshipDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_batchInfo_BatchRelationship" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_batchInfo_BatchRelationship" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchRelationshipDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchRelationship\"","MESBasic_1_batchInfo","","BATCH_RELATIONSHIP",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchRelationship\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchRelationship\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchRelationship\"","MESBasic_1_batchInfo","","BATCH_RELATIONSHIP",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_batchInfo_BatchRelationship" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchRelationship.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchRelationship\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchRelationship\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchRelationship\".CID = 1 OR \"batchRelationship\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchRelationship.mc\"").append(".BATCH_RELATIONSHIP) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchRelationshipDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchRelationshipDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicBatchRelationship.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicBatchRelationship.MODULE_CODE;}
	protected String getModelCode(){return MESBasicBatchRelationship.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchRelationship 模型实例
	 */
	public void checkUniqueConstraint(MESBasicBatchRelationship batchRelationship){
	}
	
	
	@Override
	public List<MESBasicBatchRelationship> getBatchRelationships(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchRelationship.class);
		if(params!=null){
			...
			return batchRelationshipDao.findByHql("from " + MESBasicBatchRelationship.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchRelationshipDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicBatchRelationship> findBatchRelationshipsByHql(String hql, Object... objects){
		...
		return   batchRelationshipDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicBatchRelationship loadBatchRelationshipByBussinessKey(MESBasicBatchRelationship batchRelationship){
		return loadBatchRelationshipByBussinessKey(batchRelationship.getId());
	}
	
	@Override
	public Page<MESBasicBatchRelationship> getBatchRelationships(Page<MESBasicBatchRelationship> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicBatchRelationship.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchRelationshipDao.findByPage(page, sql);
			}else{
				page = batchRelationshipDao.findAllByPage(page);
			}
			return page;
		}
		page = batchRelationshipDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public MESBasicBatchRelationship loadBatchRelationshipByBussinessKey(Serializable bussinessKey){
		return batchRelationshipDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicBatchRelationship> findByProperty(String propertyName, Object object){
		return batchRelationshipDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicBatchRelationship findEntityByProperty(String propertyName, Object object){
		return batchRelationshipDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicBatchRelationship.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchRelationshipDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_batchInfo_BatchRelationship","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicBatchRelationship batchRelationship,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchRelationshipDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicBatchRelationship.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchRelationship);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchRelationship.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBatchRelationship.TABLE_NAME +" where valid = 1";
			list = batchRelationshipDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicBatchRelationship.TABLE_NAME;
			list = batchRelationshipDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicBatchRelationship.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchRelationshipDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicBatchRelationshipDaoImpl batchRelationshipDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchRelationship(final List<MESBasicBatchRelationship> insertObjs, final List<MESBasicBatchRelationship> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchRelationship(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchRelationship(final List<MESBasicBatchRelationship> insertObjs, final List<MESBasicBatchRelationship> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1490775465362"),importCount));
		...
		Map<Long, MESBasicBatchRelationship> oldUpdateObjsMap = new HashMap<Long, MESBasicBatchRelationship>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicBatchRelationship bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicBatchRelationship> bizObjList = batchRelationshipDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicBatchRelationship bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicBatchRelationship> nmeObjs = new ArrayList<MESBasicBatchRelationship>();
		...
		long id=autoGeneratorID.getNextId(MESBasicBatchRelationship.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicBatchRelationship batchRelationship:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicBatchRelationship.TABLE_NAME,1,dbId);
			if(batchRelationship.getId() != null ){
				m.put(batchRelationship.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			batchRelationship.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchRelationshipDao.createNativeQuery(sql, "MESBasic_1_batchInfo_BatchRelationship"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchRelationshipDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicBatchRelationship.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicBatchRelationship batchRelationship = updateObjs.get(updateIndex);
						Long id = batchRelationship.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchRelationship.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchRelationship);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicBatchRelationship.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicBatchRelationship.TABLE_NAME + " order by id desc";
//					Query query = batchRelationshipDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicBatchRelationship batchRelationship = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchRelationship.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchRelationshipDaoImpl.getSession().flush();
		batchRelationshipDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicBatchRelationship bizObj = insertObjs.get(insertIndex);
			batchRelationshipImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicBatchRelationship bizObj = updateObjs.get(updateIndex);
			MESBasicBatchRelationship oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchRelationshipImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicBatchRelationship> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicBatchRelationship> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchRelationshipDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicBatchRelationship> result = getResult(page, query, new PendingResultTransformer(MESBasicBatchRelationship.class,batchRelationshipDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicBatchRelationship model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchRelationshipDao.createNativeQuery(sql, "MESBasic_1_batchInfo_BatchRelationship").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchRelationshipDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchRelationshipDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchRelationshipDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchRelationshipDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchRelationshipDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchRelationshipDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="ec.flow.save")
	public void saveBatchRelationship(MESBasicBatchRelationship batchRelationship, DataGridService dataGridService){
		saveBatchRelationship(batchRelationship, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="ec.flow.save")
	public void saveBatchRelationship(MESBasicBatchRelationship batchRelationship, DataGridService dataGridService, String viewCode){
		saveBatchRelationship(batchRelationship, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship")
	public void saveBatchRelationship(MESBasicBatchRelationship batchRelationship, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchRelationship(batchRelationship, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship")
	public void saveBatchRelationship(MESBasicBatchRelationship batchRelationship, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_batchInfo";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_batchInfo");
		String url = null;
		if(null != batchRelationship.getId() && batchRelationship.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicBatchRelationship.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship")
	public Map<Object, Object> batchRelationshipDataGridImport(MESBasicBatchRelationship batchRelationship, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchRelationship.getVirtualId();
		
		batchRelationship = batchRelationshipDao.load(batchRelationship.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchRelationship.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchRelationship.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchRelationship);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchRelationship.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship")
	public void mergeBatchRelationship(MESBasicBatchRelationship batchRelationship, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchRelationship);
		beforeSaveBatchRelationship(batchRelationship);
		batchRelationshipDao.merge(batchRelationship);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchInfo",model="MESBasic_1_batchInfo_BatchRelationship",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicBatchRelationship> getByPage(Page<MESBasicBatchRelationship> page,DetachedCriteria detachedCriteria){
		return batchRelationshipDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchRelationship(MESBasicBatchRelationship batchRelationship, Object... objects){
		checkUniqueConstraint(batchRelationship);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchRelationship(MESBasicBatchRelationship batchRelationship, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchRelationship(MESBasicBatchRelationship batchRelationship){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchRelationship(MESBasicBatchRelationship batchRelationship){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchRelationship(MESBasicBatchRelationship batchRelationship){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchRelationship(MESBasicBatchRelationship batchRelationship){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicBatchRelationship batchRelationship, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicBatchRelationship batchRelationship, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportBatchRelationship(List<MESBasicBatchRelationship> insertObjs, List<MESBasicBatchRelationship> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchRelationship,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportBatchRelationship(List<MESBasicBatchRelationship> insertObjs, List<MESBasicBatchRelationship> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchRelationship,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
