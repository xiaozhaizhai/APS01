...
@Repository
public class MESBasicBaseCustomerDaoImpl extends ExtendedGenericDaoImpl<MESBasicBaseCustomer, Long> implements MESBasicBaseCustomerDao {
	@Override
	...
	@Override
	public MESBasicBaseCustomerDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicBaseCustomerDealInfo dealInfo = new MESBasicBaseCustomerDealInfo();
	...
	@Override
	public MESBasicBaseCustomerDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicBaseCustomer mainObj, Staff dealStaff) {
		MESBasicBaseCustomerDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicBaseCustomerDealInfo.class);
		...
		List<MESBasicBaseCustomerDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_baseCustomer_BaseCustomer,MESBasic_1) */
// 自定义代码
