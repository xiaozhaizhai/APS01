...
import com.supcon.orchid.MESBasic.services.MESBasicObjProductImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicObjProductDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_objProductService")
@Transactional
public class MESBasicObjProductServiceImpl  extends BaseServiceImpl<MESBasicObjProduct> implements MESBasicObjProductService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicObjProduct> bapBaseService;
	...
	@Autowired
	private MESBasicObjProductImportService objProductImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicObjProduct.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  objProductDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicObjProduct.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = objProductDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjProduct.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjProduct.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  objProductDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicObjProduct.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicObjProduct.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicObjProduct> list = (List<MESBasicObjProduct>) objProductDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjProduct getObjProduct(long id){
		return getObjProduct(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjProduct getObjProduct(long id, String viewCode){
		MESBasicObjProduct objProduct = objProductDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getObjProductAsJSON(long id, String include){
		MESBasicObjProduct objProduct = objProductDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(objProduct, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.common.delete",operType="3")
	public void deleteObjProduct(MESBasicObjProduct objProduct){
		if(null != objProduct && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjProduct.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objProduct.getId() ? "" : objProduct.getId().toString());
			AuditUtil.setColumnStringA(null == objProduct.getId() ? "" : objProduct.getId().toString());
			AuditUtil.setColumnStringC(objProduct.getId().toString());
		}
		beforeDeleteObjProduct(objProduct);
		objProductDao.deletePhysical(objProduct);
		afterDeleteObjProduct(objProduct);
		objProductDao.flush();
		List<MESBasicObjProduct> params = new ArrayList<MESBasicObjProduct>();
		params.add(objProduct);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.common.delete",operType="3")
	public void deleteObjProduct(List<Long> objProductIds){
		deleteObjProduct(objProductIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.common.delete",operType="3")
	public void deleteObjProduct(List<Long> objProductIds, String eventTopic) {
		List<MESBasicObjProduct> objProducts = new ArrayList<MESBasicObjProduct>();
		for(Long objProductId : objProductIds){
			MESBasicObjProduct objProduct = getObjProduct(objProductId);
			objProducts.add(objProduct);
			if(null != objProduct && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjProduct.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == objProduct.getId() ? "" : "," + objProduct.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == objProduct.getId() ? "" : objProduct.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == objProduct.getId() ? "" : "," + objProduct.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == objProduct.getId() ? "" : objProduct.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == objProduct.getId() ? "" : "," + objProduct.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == objProduct.getId() ? "" : objProduct.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(objProducts != null && objProducts.size() > 0 ){
			for(MESBasicObjProduct objProduct : objProducts){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicObjProduct.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(objProduct));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),objProduct, objProduct.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(objProductIds != null && objProductIds.size() > 0) {
			String hql = "delete " + MESBasicObjProduct.JPA_NAME + " where id in(:ids)";
			Query query = objProductDao.createQuery(hql);
			query.setParameterList("ids", objProductIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicObjProduct objProductz : objProducts){
				objProductz.setValid(false);
				List<MESBasicObjProduct> params = new ArrayList<MESBasicObjProduct>();
				params.add(objProductz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_factoryModel");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.common.delete",operType="3")
	public void deleteObjProduct(long objProductId, int objProductVersion){
		this.deleteObjProduct(objProductId, objProductVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.common.delete",operType="3")
	public void deleteObjProduct(long objProductId, int objProductVersion,SignatureLog signatureLog){
		MESBasicObjProduct objProduct = getObjProduct(objProductId);
		...
		beforeDeleteObjProduct(objProduct);
		objProductDao.delete(objProductId, objProductVersion);
		afterDeleteObjProduct(objProduct);
		objProductDao.flush();
		List<MESBasicObjProduct> params = new ArrayList<MESBasicObjProduct>();
		params.add(objProduct);
		...
		if(signatureLog != null ) {
			...
			businessKey=objProduct.getId();
			signatureLog.setTableId(objProduct.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.common.delete",operType="3")
	public void deleteObjProduct(String objProductIds){
		this.deleteObjProduct(objProductIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.common.delete",operType="3")
	public void deleteObjProduct(String objProductIds,SignatureLog signatureLog){
		deleteCollection(objProductIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteObjProduct(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_factoryModel#modelCode:MESBasic_1_factoryModel_ObjProduct#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="ec.engine.restore",operType="10")
	public void restoreObjProduct(String objProductIds){
		restoreCollection(objProductIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreObjProduct(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="ec.engine.restore",operType="10")
	public void restoreObjProduct(long objProductId){
		
		findBusinessKeyUsed(objProductId);	//判断业务主键是否重复
		...
		beforeRestoreObjProduct(objProduct);
		objProduct.setValid(true);
		objProductDao.update(objProduct);
		afterRestoreObjProduct(objProduct);
		List<MESBasicObjProduct> params = new ArrayList<MESBasicObjProduct>();
		params.add(objProduct);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long objProductId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_factoryModel_ObjProduct");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicObjProduct.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicObjProduct.TABLE_NAME +" where id = ? )";
			List<Object> list =  objProductDao.createNativeQuery(sql,objProductId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseObjProduct(List<MESBasicObjProduct>  objProducts){
		for(MESBasicObjProduct objProduct:objProducts)  {
			saveObjProduct(objProduct, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseObjProduct(List<MESBasicObjProduct>  objProducts){
		for(MESBasicObjProduct objProduct:objProducts)  {
			saveObjProduct(objProduct, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="ec.flow.save")
	public void saveObjProduct(MESBasicObjProduct objProduct, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveObjProduct(objProduct, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="ec.flow.save")
	public void saveObjProduct(MESBasicObjProduct objProduct, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveObjProduct(objProduct, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateObjProductCodes(MESBasicObjProduct objProduct) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjProductCodes(objProduct, false);
	}
	
	public void generateObjProductCodes(MESBasicObjProduct objProduct, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateObjProductSummarys(MESBasicObjProduct objProduct) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjProductSummarys(objProduct, false);
	}

	public void generateObjProductSummarys(MESBasicObjProduct objProduct, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct")
	public void saveObjProduct(MESBasicObjProduct objProduct,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveObjProduct(objProduct, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct")
	public void saveObjProduct(MESBasicObjProduct objProduct,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(objProduct.getId() != null && objProduct.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjProduct.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, objProduct);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(objProduct);
		beforeSaveObjProduct(objProduct, viewIsView);

		if (viewIsView) {
			objProductDao.saveWithRevertVersion(objProduct);
		} else {
			if(isNew)
				objProductDao.save(objProduct);
			else
				objProductDao.merge(objProduct);
		}
		if(null != objProduct && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjProduct.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objProduct.getId() ? "" : objProduct.getId().toString());
			AuditUtil.setColumnStringA(null == objProduct.getId() ? "" : objProduct.getId().toString());
		...
		dealDatagridsSave(objProduct,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateObjProductCodes(objProduct, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateObjProductSummarys(objProduct, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveObjProduct(objProduct, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, objProduct);
			}
		}
		List<MESBasicObjProduct> params = new ArrayList<MESBasicObjProduct>();
		objProductDao.flush();
		objProductDao.clear();
		objProduct = objProductDao.load(objProduct.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicObjProduct objProduct){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("objProduct", objProduct);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct")
	public void mergeObjProduct(MESBasicObjProduct objProduct, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(objProduct);
		beforeSaveObjProduct(objProduct);
		objProductDao.merge(objProduct);
		if(null != objProduct && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjProduct.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objProduct.getId() ? "" : objProduct.getId().toString());
			AuditUtil.setColumnStringA(null == objProduct.getId() ? "" : objProduct.getId().toString());
		...
		dealDatagridsSave(objProduct,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveObjProduct(objProduct);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicObjProduct> findObjProducts(Page<MESBasicObjProduct> page, Criterion... criterions) {
		return objProductDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjProducts(Page<MESBasicObjProduct> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findObjProducts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjProducts(Page<MESBasicObjProduct> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjProducts(Page<MESBasicObjProduct> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicObjProduct> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"objProduct\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"objProduct\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"objProduct\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"objProduct\".OBJ_FACTORY IN ( SELECT ID FROM factory_tbcompanystructure \"factoryModel\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  objProductDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  objProductDao.createNativeQuery(powerOperateSql, "MESBasic_1_factoryModel").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"objProduct\".LAY_REC ASC, \"objProduct\".SORT ASC");
				} else {
					orderPart.append(" \"objProduct\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) objProductDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=objProductDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = objProductDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicObjProduct> result = new ArrayList<MESBasicObjProduct>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjProduct.class,objProductDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjProduct.class,objProductDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicObjProduct objProduct : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(objProduct.getId(), "MESBasic_factoryModel_objProduct");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(objProduct.getId(), "MESBasic_factoryModel_objProduct");
							if(null != documents && !documents.isEmpty()) {
								objProduct.setDocument(documents.get(0));
								objProduct.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicObjProduct> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicObjProduct> result) {
		View view = null;
		...
		Map<Long, MESBasicObjProduct> rsMap = new HashMap<Long, MESBasicObjProduct>();
		for (MESBasicObjProduct objProduct : result) {
			ids.add(objProduct.getId());
			rsMap.put(objProduct.getId(), objProduct);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(objProductDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = objProductDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_factoryModel_ObjProduct" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_factoryModel_ObjProduct" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  objProductDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objProduct\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_PRODUCTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"objProduct\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"objProduct\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objProduct\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_PRODUCTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_factoryModel_ObjProduct" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("objProduct.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"objProduct\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"objProduct\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"objProduct\".CID = 1 OR \"objProduct\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("objProduct.mc\"").append(".OBJ_PRODUCT) not in ("+notinValue+")");
			}
			
			SQLQuery query = objProductDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=objProductDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicObjProduct.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicObjProduct.MODULE_CODE;}
	protected String getModelCode(){return MESBasicObjProduct.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param objProduct 模型实例
	 */
	public void checkUniqueConstraint(MESBasicObjProduct objProduct){
	}
	
	
	@Override
	public List<MESBasicObjProduct> getObjProducts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjProduct.class);
		if(params!=null){
			...
			return objProductDao.findByHql("from " + MESBasicObjProduct.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return objProductDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicObjProduct> findObjProductsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjProduct.class);
	...
	@Override
	public List<MESBasicObjProduct> findObjProductsByHql(String hql, Object... objects){
		
		return   objProductDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicObjProduct loadObjProductByBussinessKey(MESBasicObjProduct objProduct){
		return loadObjProductByBussinessKey(objProduct.getId());
	}
	
	@Override
	public Page<MESBasicObjProduct> getObjProducts(Page<MESBasicObjProduct> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjProduct.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = objProductDao.findByPage(page, sql);
			}else{
				page = objProductDao.findAllByPage(page);
			}
			return page;
		}
		page = objProductDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjProduct> findByTreeDataGridFactoryModel(List<MESBasicObjProduct> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicObjProduct objProduct : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFactoryModel(Page<MESBasicObjProduct> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicObjProduct> objProducts = dgPage.getResult();
			for(MESBasicObjProduct objProduct : objProducts){
				
			}
			dgPage.setResult(objProducts);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjProduct loadObjProductByBussinessKey(Serializable bussinessKey){
		return objProductDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjProduct> findByProperty(String propertyName, Object object){
		return objProductDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjProduct findEntityByProperty(String propertyName, Object object){
		return objProductDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicObjProduct.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = objProductDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_factoryModel_ObjProduct","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicObjProduct objProduct,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,objProductDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicObjProduct.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, objProduct);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",objProduct.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjProduct.TABLE_NAME +" where valid = 1";
			list = objProductDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjProduct.TABLE_NAME;
			list = objProductDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicObjProduct.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = objProductDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicObjProductDaoImpl objProductDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchObjProduct(final List<MESBasicObjProduct> insertObjs, final List<MESBasicObjProduct> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchObjProduct(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchObjProduct(final List<MESBasicObjProduct> insertObjs, final List<MESBasicObjProduct> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1460950716828"),importCount));
		...
		Map<Long, MESBasicObjProduct> oldUpdateObjsMap = new HashMap<Long, MESBasicObjProduct>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicObjProduct bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicObjProduct> bizObjList = objProductDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicObjProduct bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicObjProduct> nmeObjs = new ArrayList<MESBasicObjProduct>();
		...
		long id=autoGeneratorID.getNextId(MESBasicObjProduct.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicObjProduct objProduct:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicObjProduct.TABLE_NAME,1,dbId);
			if(objProduct.getId() != null ){
				m.put(objProduct.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) objProductDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjProduct"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		objProductDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicObjProduct.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicObjProduct objProduct = updateObjs.get(updateIndex);
						Long id = objProduct.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = objProduct.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicObjProduct.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicObjProduct.TABLE_NAME + " order by id desc";
//					Query query = objProductDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicObjProduct objProduct = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = objProduct.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		objProductDaoImpl.getSession().flush();
		objProductDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicObjProduct bizObj = insertObjs.get(insertIndex);
			objProductImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicObjProduct bizObj = updateObjs.get(updateIndex);
			MESBasicObjProduct oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			objProductImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicObjProduct> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicObjProduct> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) objProductDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicObjProduct> result = getResult(page, query, new PendingResultTransformer(MESBasicObjProduct.class,objProductDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicObjProduct model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = objProductDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjProduct").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = objProductDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = objProductDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = objProductDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?objProductDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  objProductDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,objProductDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="ec.flow.save")
	public void saveObjProduct(MESBasicObjProduct objProduct, DataGridService dataGridService){
		saveObjProduct(objProduct, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="ec.flow.save")
	public void saveObjProduct(MESBasicObjProduct objProduct, DataGridService dataGridService, String viewCode){
		saveObjProduct(objProduct, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct")
	public void saveObjProduct(MESBasicObjProduct objProduct, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveObjProduct(objProduct, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct")
	public void saveObjProduct(MESBasicObjProduct objProduct, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(null != objProduct.getId() && objProduct.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjProduct.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct")
	public Map<Object, Object> objProductDataGridImport(MESBasicObjProduct objProduct, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = objProduct.getVirtualId();
		
		objProduct = objProductDao.load(objProduct.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, objProduct.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = objProduct.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(objProduct);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, objProduct.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct")
	public void mergeObjProduct(MESBasicObjProduct objProduct, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(objProduct);
		beforeSaveObjProduct(objProduct);
		objProductDao.merge(objProduct);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjProduct",desc="foundation.batch.save",operType="4")
	public void batchSaveObjProduct(MESBasicObjProduct objProduct, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
	...
	@Override
	public Page<MESBasicObjProduct> getByPage(Page<MESBasicObjProduct> page,DetachedCriteria detachedCriteria){
		return objProductDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveObjProduct(MESBasicObjProduct objProduct, Object... objects){
		checkUniqueConstraint(objProduct);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
      DetachedCriteria detachedCriteria=DetachedCriteria.forClass(MESBasicObjProduct.class);
		detachedCriteria.add(Restrictions.eq("valid",true));
		detachedCriteria.add(Restrictions.eq("objProduct",objProduct.getObjProduct()));
	    if(objProduct.getId()!=null){//修改
	    	detachedCriteria.add(Restrictions.ne("id",objProduct.getId()));
	    }
	    List<MESBasicObjProduct> basicObjProducts = objProductDao.findByCriteria(detachedCriteria);
		
      
		if (null !=basicObjProducts && !basicObjProducts.isEmpty()){
			String proNames="";
          	String factoryName="";
			for (MESBasicObjProduct basicObjProduct : basicObjProducts){
				MESBasicProduct tempProduct=productDao.load(basicObjProduct.getObjProduct().getId());
              if(basicObjProduct.getObjFactory().getId().equals(objProduct.getObjFactory().getId())){
					throw new BAPException("物品["+tempProduct.getProductName()+"]已存在于本模型中，不能重复添加！");
				}
				if(proNames.length()>0){
					proNames+="，"+tempProduct.getProductName();
                 	factoryName+=","+basicObjProduct.getObjFactory().getName();
				}else{
					proNames=tempProduct.getProductName();
                  	factoryName=basicObjProduct.getObjFactory().getName();
				}
			...
			if (proNames.length()>0)
				throw new BAPException("物品["+proNames+"]已分别存在于["+factoryName+"]模型中，不能重复添加！");
			}
/* CUSTOM CODE END */
	}
	
	private void afterSaveObjProduct(MESBasicObjProduct objProduct, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportObjProduct(List<MESBasicObjProduct> objProducts, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportObjProduct(List<MESBasicObjProduct> objProducts){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportObjProduct(List<MESBasicObjProduct> objProducts, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportObjProduct(List<MESBasicObjProduct> objProducts){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteObjProduct(MESBasicObjProduct objProduct){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteObjProduct(MESBasicObjProduct objProduct){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreObjProduct(MESBasicObjProduct objProduct){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreObjProduct(MESBasicObjProduct objProduct){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicObjProduct objProduct, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicObjProduct objProduct, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportObjProduct(List<MESBasicObjProduct> insertObjs, List<MESBasicObjProduct> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportObjProduct,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportObjProduct(List<MESBasicObjProduct> insertObjs, List<MESBasicObjProduct> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportObjProduct,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_factoryModel_ObjProduct,MESBasic_1) */
// 自定义代码
