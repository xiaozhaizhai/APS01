...
/* CUSTOM CODE START(action,import,MESBasic_1_container_Container,MESBasic_1) */
// 自定义代码
import java.util.Set;
import java.util.HashSet;
...
public class MESBasicContainerAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicContainer> {
	...
	//导出excel时,主模型内容
	List<MESBasicContainer> queryResults = new ArrayList<MESBasicContainer>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/container/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/container/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_container_Container" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicContainer> containers;
	
	public List<MESBasicContainer> getContainers() {
		return containers;
	}
	...
	private Page<MESBasicContainer> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			container = new MESBasicContainer();
			if(container.getContainerShape() != null) {
				container.setContainerShape((SystemCode) systemCodeService.getSystemCode(container.getContainerShape().getId()));
			}
			if(container.getContainerStatus() != null) {
				container.setContainerStatus((SystemCode) systemCodeService.getSystemCode(container.getContainerStatus().getId()));
			}
			if(container.getContainerType() != null) {
				container.setContainerType((SystemCode) systemCodeService.getSystemCode(container.getContainerType().getId()));
			}
		} else {
			if(id > 0){
				container = containerService.getContainer(id, viewCode);
			}
		}
	...
	public Page<MESBasicContainer> getPage() {
		if (null == page) {
			page = new Page<MESBasicContainer>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/container/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = containerService.getContainerAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/container/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String containerReferenceCopy() throws Exception{
			MESBasicContainer oldContainer = null;
			if(isEditNew()){
				...
				oldContainer = new MESBasicContainer();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,container.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, container.getId());
				...
				oldContainer = containerService.getContainer(id);
			}
			
			
			container = containerService.getContainer(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_container_Container");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicContainer.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldContainer, null);
						Method setValue = MESBasicContainer.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(container, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			container.setCreateDepartmentId(oldContainer.getCreateDepartmentId());
			container.setCreatePositionId(oldContainer.getCreatePositionId());
			container.setCreateStaffId(oldContainer.getCreateStaffId());
			container.setOwnerDepartmentId(oldContainer.getOwnerDepartmentId());
			container.setOwnerPositionId(oldContainer.getOwnerPositionId());
			container.setOwnerStaffId(oldContainer.getOwnerStaffId());
			container.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_container_Container", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/container/container/containerEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "containerEdit.ftl") }),
		@Action(value = "/MESBasic/container/container/containerEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "containerEditPrint.ftl")})
		,
		@Action(value = "/MESBasic/container/container/newContainerEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "newContainerEdit.ftl") }),
		@Action(value = "/MESBasic/container/container/newContainerEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "newContainerEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=containerService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return containerReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			container = containerService.getContainer(this.container.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,container.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/container/container/containerShadowView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "containerShadowView.ftl") }),
		@Action(value = "/MESBasic/container/container/containerShadowViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "containerShadowViewPrint.ftl")})
		,
		@Action(value = "/MESBasic/container/container/containerView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "containerView.ftl") }),
		@Action(value = "/MESBasic/container/container/containerViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "containerViewPrint.ftl")})
		,
		@Action(value = "/MESBasic/container/container/newContainerView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "newContainerView.ftl") }),
		@Action(value = "/MESBasic/container/container/newContainerViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "newContainerViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/container/container/containerList", results = { @Result(type = FREEMARKER, location = "containerList.ftl") })
		,
		@Action(value = "/MESBasic/container/container/newContainerList", results = { @Result(type = FREEMARKER, location = "newContainerList.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/container/container/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = containerService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(containerService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(containerService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_container_Container", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/container/container/containerListAVRef", results = { @Result(type = FREEMARKER, location = "containerListAVRef.ftl") })
		,
		@Action(value = "/MESBasic/container/container/containerMulRef", results = { @Result(type = FREEMARKER, location = "containerMulRef.ftl") })
		,
		@Action(value = "/MESBasic/container/container/containerRef", results = { @Result(type = FREEMARKER, location = "containerRef.ftl") })
		,
		@Action(value = "/MESBasic/container/container/containerSingleRef", results = { @Result(type = FREEMARKER, location = "containerSingleRef.ftl") })
		,
		@Action(value = "/MESBasic/container/container/newContaineMulRef", results = { @Result(type = FREEMARKER, location = "newContaineMulRef.ftl") })
		,
		@Action(value = "/MESBasic/container/container/newContainerRef", results = { @Result(type = FREEMARKER, location = "newContainerRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/container/container/containerList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.containerType.id,result.containerType.value,result.equip.name,result.containerStatus.id,result.containerStatus.value,result.department.name,result.model.name,result.product.productName,result.produceMgr,result.tankMgr,result.equip.id,result.department.id,result.model.id,result.product.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="containerList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/container/container/newContainerList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.containerShape.id,result.containerShape.value,result.containerStatus.id,result.containerStatus.value,result.product.productCode,result.product.productName,result.person.name,result.department.name,result.product.id,result.person.id,result.department.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="newContainerList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"container\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"container\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("tankModel.layRec") && request.getParameter("tankModel.layRec").trim().length() > 0) {
			params.add(new Param("\"model\".LAY_REC", request.getParameter("tankModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"container\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"container\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/container/container/containerList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/MESBasic/container/container/newContainerList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/container/container/containerListAVRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.equip.name,result.containerType.id,result.containerType.value,result.containerStatus.id,result.containerStatus.value,result.department.name,result.model.name,result.product.productName,result.produceMgr,result.tankMgr,result.equip.id,result.department.id,result.model.id,result.product.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="containerListAVRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/container/container/containerMulRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.containerShape.id,result.containerShape.value,result.containerStatus.id,result.containerStatus.value,result.product.productCode,result.product.productName,result.person.name,result.department.name,result.product.id,result.person.id,result.department.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="containerMulRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/container/container/containerRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.containerType.id,result.containerType.value,result.equip.name,result.containerStatus.id,result.containerStatus.value,result.department.name,result.model.name,result.product.productName,result.produceMgr,result.tankMgr,result.equip.id,result.department.id,result.model.id,result.product.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="containerRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/container/container/containerSingleRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.containerShape.id,result.containerShape.value,result.containerStatus.id,result.containerStatus.value,result.product.productCode,result.product.productName,result.person.name,result.department.name,result.product.id,result.person.id,result.department.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="containerSingleRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/container/container/newContaineMulRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.containerShape.id,result.containerShape.value,result.containerStatus.id,result.containerStatus.value,result.product.productCode,result.product.productName,result.person.name,result.department.name,result.product.id,result.person.id,result.department.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="newContaineMulRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/container/container/newContainerRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.containerShape.id,result.containerShape.value,result.containerStatus.id,result.containerStatus.value,result.product.productCode,result.product.productName,result.person.name,result.department.name,result.product.id,result.person.id,result.department.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="newContainerRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/container/container/allTempleteDownload", results = { @Result(name="excel",type="excel",location="containerList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_container_Container");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		containerService.findDataGridPage(dg,dgclass,getDgPage(),container,"", null);
	...
	private Page<MESBasicPropertyTags> dg1416532238441Page;
	public Page<MESBasicPropertyTags> getDg1416532238441Page(){
		if(null == dg1416532238441Page){
			dg1416532238441Page = new Page<MESBasicPropertyTags>(1,20);
		}
		return dg1416532238441Page;
	}
	
	private Class dg1416532238441Clazz = MESBasicPropertyTags.class;
	
	public Class getDg1416532238441Clazz() {
		return dg1416532238441Clazz;
	}
	
	private List<MESBasicPropertyTags> dg1416532238441List;
	public List<MESBasicPropertyTags> getDg1416532238441List(){
		if(null == dg1416532238441List){
			dg1416532238441List = new ArrayList<MESBasicPropertyTags>();
		}
		return dg1416532238441List;
	}
	public void setDg1416532238441ListJson(String json) {
		//dg1416532238441List = (List<MESBasicPropertyTags>) containerService.generateObjectFromJson(json, MESBasicPropertyTags.class);
	}
	
	private List<Long> dg1416532238441DeletedIds;
	public List<Long> getDg1416532238441DeletedIds(){
		if(null == dg1416532238441DeletedIds){
			dg1416532238441DeletedIds = new ArrayList<Long>();
		}
		return dg1416532238441DeletedIds;
	}

	private Page<MESBasicPropertyTags> dg1551701917640Page;
	public Page<MESBasicPropertyTags> getDg1551701917640Page(){
		if(null == dg1551701917640Page){
			dg1551701917640Page = new Page<MESBasicPropertyTags>(1,20);
		}
		return dg1551701917640Page;
	}
	
	private Class dg1551701917640Clazz = MESBasicPropertyTags.class;
	
	public Class getDg1551701917640Clazz() {
		return dg1551701917640Clazz;
	}
	
	private List<MESBasicPropertyTags> dg1551701917640List;
	public List<MESBasicPropertyTags> getDg1551701917640List(){
		if(null == dg1551701917640List){
			dg1551701917640List = new ArrayList<MESBasicPropertyTags>();
		}
		return dg1551701917640List;
	}
	public void setDg1551701917640ListJson(String json) {
		//dg1551701917640List = (List<MESBasicPropertyTags>) containerService.generateObjectFromJson(json, MESBasicPropertyTags.class);
	}
	
	private List<Long> dg1551701917640DeletedIds;
	public List<Long> getDg1551701917640DeletedIds(){
		if(null == dg1551701917640DeletedIds){
			dg1551701917640DeletedIds = new ArrayList<Long>();
		}
		return dg1551701917640DeletedIds;
	}

	private Page<MESBasicContainerExp> dg1553144257690Page;
	public Page<MESBasicContainerExp> getDg1553144257690Page(){
		if(null == dg1553144257690Page){
			dg1553144257690Page = new Page<MESBasicContainerExp>(1,20);
		}
		return dg1553144257690Page;
	}
	
	private Class dg1553144257690Clazz = MESBasicContainerExp.class;
	
	public Class getDg1553144257690Clazz() {
		return dg1553144257690Clazz;
	}
	
	private List<MESBasicContainerExp> dg1553144257690List;
	public List<MESBasicContainerExp> getDg1553144257690List(){
		if(null == dg1553144257690List){
			dg1553144257690List = new ArrayList<MESBasicContainerExp>();
		}
		return dg1553144257690List;
	}
	public void setDg1553144257690ListJson(String json) {
		//dg1553144257690List = (List<MESBasicContainerExp>) containerService.generateObjectFromJson(json, MESBasicContainerExp.class);
	}
	
	private List<Long> dg1553144257690DeletedIds;
	public List<Long> getDg1553144257690DeletedIds(){
		if(null == dg1553144257690DeletedIds){
			dg1553144257690DeletedIds = new ArrayList<Long>();
		}
		return dg1553144257690DeletedIds;
	}

	private Page<MESBasicFlowRecord> dg1555312971656Page;
	public Page<MESBasicFlowRecord> getDg1555312971656Page(){
		if(null == dg1555312971656Page){
			dg1555312971656Page = new Page<MESBasicFlowRecord>(1,20);
		}
		return dg1555312971656Page;
	}
	
	private Class dg1555312971656Clazz = MESBasicFlowRecord.class;
	
	public Class getDg1555312971656Clazz() {
		return dg1555312971656Clazz;
	}
	
	private List<MESBasicFlowRecord> dg1555312971656List;
	public List<MESBasicFlowRecord> getDg1555312971656List(){
		if(null == dg1555312971656List){
			dg1555312971656List = new ArrayList<MESBasicFlowRecord>();
		}
		return dg1555312971656List;
	}
	public void setDg1555312971656ListJson(String json) {
		//dg1555312971656List = (List<MESBasicFlowRecord>) containerService.generateObjectFromJson(json, MESBasicFlowRecord.class);
	}
	
	private List<Long> dg1555312971656DeletedIds;
	public List<Long> getDg1555312971656DeletedIds(){
		if(null == dg1555312971656DeletedIds){
			dg1555312971656DeletedIds = new ArrayList<Long>();
		}
		return dg1555312971656DeletedIds;
	}

	private Page<MESBasicMaintainLog> dg1555312985524Page;
	public Page<MESBasicMaintainLog> getDg1555312985524Page(){
		if(null == dg1555312985524Page){
			dg1555312985524Page = new Page<MESBasicMaintainLog>(1,20);
		}
		return dg1555312985524Page;
	}
	
	private Class dg1555312985524Clazz = MESBasicMaintainLog.class;
	
	public Class getDg1555312985524Clazz() {
		return dg1555312985524Clazz;
	}
	
	private List<MESBasicMaintainLog> dg1555312985524List;
	public List<MESBasicMaintainLog> getDg1555312985524List(){
		if(null == dg1555312985524List){
			dg1555312985524List = new ArrayList<MESBasicMaintainLog>();
		}
		return dg1555312985524List;
	}
	public void setDg1555312985524ListJson(String json) {
		//dg1555312985524List = (List<MESBasicMaintainLog>) containerService.generateObjectFromJson(json, MESBasicMaintainLog.class);
	}
	
	private List<Long> dg1555312985524DeletedIds;
	public List<Long> getDg1555312985524DeletedIds(){
		if(null == dg1555312985524DeletedIds){
			dg1555312985524DeletedIds = new ArrayList<Long>();
		}
		return dg1555312985524DeletedIds;
	}

	private Page<MESBasicPropertyTags> dg1551703065405Page;
	public Page<MESBasicPropertyTags> getDg1551703065405Page(){
		if(null == dg1551703065405Page){
			dg1551703065405Page = new Page<MESBasicPropertyTags>(1,20);
		}
		return dg1551703065405Page;
	}
	
	private Class dg1551703065405Clazz = MESBasicPropertyTags.class;
	
	public Class getDg1551703065405Clazz() {
		return dg1551703065405Clazz;
	}
	
	private List<MESBasicPropertyTags> dg1551703065405List;
	public List<MESBasicPropertyTags> getDg1551703065405List(){
		if(null == dg1551703065405List){
			dg1551703065405List = new ArrayList<MESBasicPropertyTags>();
		}
		return dg1551703065405List;
	}
	public void setDg1551703065405ListJson(String json) {
		//dg1551703065405List = (List<MESBasicPropertyTags>) containerService.generateObjectFromJson(json, MESBasicPropertyTags.class);
	}
	
	private List<Long> dg1551703065405DeletedIds;
	public List<Long> getDg1551703065405DeletedIds(){
		if(null == dg1551703065405DeletedIds){
			dg1551703065405DeletedIds = new ArrayList<Long>();
		}
		return dg1551703065405DeletedIds;
	}

	private Page<MESBasicContainerExp> dg1553145948499Page;
	public Page<MESBasicContainerExp> getDg1553145948499Page(){
		if(null == dg1553145948499Page){
			dg1553145948499Page = new Page<MESBasicContainerExp>(1,20);
		}
		return dg1553145948499Page;
	}
	
	private Class dg1553145948499Clazz = MESBasicContainerExp.class;
	
	public Class getDg1553145948499Clazz() {
		return dg1553145948499Clazz;
	}
	
	private List<MESBasicContainerExp> dg1553145948499List;
	public List<MESBasicContainerExp> getDg1553145948499List(){
		if(null == dg1553145948499List){
			dg1553145948499List = new ArrayList<MESBasicContainerExp>();
		}
		return dg1553145948499List;
	}
	public void setDg1553145948499ListJson(String json) {
		//dg1553145948499List = (List<MESBasicContainerExp>) containerService.generateObjectFromJson(json, MESBasicContainerExp.class);
	}
	
	private List<Long> dg1553145948499DeletedIds;
	public List<Long> getDg1553145948499DeletedIds(){
		if(null == dg1553145948499DeletedIds){
			dg1553145948499DeletedIds = new ArrayList<Long>();
		}
		return dg1553145948499DeletedIds;
	}

	@Actions({
		
		@Action(value = "/MESBasic/container/container/data-dg1416532238441", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.property.itemNumber,result.property.code,result.property.itemName,result.property.measureClass.id,result.property.measureClass.value,result.primary,result.id,result.version,result.container.id,result.property.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1416532238441-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1416532238441.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/container/container/data-dg1551701917640", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.property.code,result.property.itemName,result.property.tagType.id,result.property.tagType.value,result.property.itemClass.id,result.property.itemClass.value,result.property.itemNumber,result.primary,result.remark,result.MESBasic_1_container_newContainerEditdg1551701917640_DATAGRID_CUSTOM_10d3d896_7c24_4fd4_9dc1_9c9cbb6e099a,result.id,result.version,result.container.id,result.property.id" , "dataGridCode", "MESBasic_1_container_newContainerEditdg1551701917640" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1551701917640-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1551701917640.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/container/container/data-dg1553144257690", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.representCode,result.code,result.id,result.version,result.headId.id" , "dataGridCode", "MESBasic_1_container_newContainerEditdg1553144257690" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1553144257690-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1553144257690.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/container/container/data-dg1555312971656", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.containerId.name,result.businessType,result.action,result.liquidWeight,result.liquidVolume,result.gasWeight,result.gasVolume,result.startLevel,result.endLevel,result.levelDiff,result.productId.productName,result.startTime,result.endTime,result.source,result.sourceType,result.destination,result.desType,result.routineId.name,result.flowRecordId,result.id,result.version,result.productId.id,result.containerId.id,result.routineId.id" , "dataGridCode", "MESBasic_1_container_newContainerEditdg1555312971656" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1555312971656-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1555312971656.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/container/container/data-dg1555312985524", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.logType,result.jsonData,result.operation,result.maintainStaff,result.maintainDept,result.maintainTime,result.remark,result.id,result.version,result.tank.id" , "dataGridCode", "MESBasic_1_container_newContainerEditdg1555312985524" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1555312985524-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1555312985524.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/container/container/data-dg1551703065405", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.property.itemNumber,result.property.itemClass.id,result.property.itemClass.value,result.property.tagType.id,result.property.tagType.value,result.property.code,result.property.itemName,result.primary,result.remark,result.MESBasic_1_container_newContainerViewdg1551703065405_DATAGRID_CUSTOM_eb130ff0_691c_4fcf_8a84_eea47b441199,result.id,result.version,result.container.id,result.property.id" , "dataGridCode", "MESBasic_1_container_newContainerViewdg1551703065405" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1551703065405-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1551703065405.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/container/container/data-dg1553145948499", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.representCode,result.code,result.id,result.version,result.headId.id" , "dataGridCode", "MESBasic_1_container_newContainerViewdg1553145948499" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1553145948499-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1553145948499.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(container.getId() != null && container.getId() != -1){
				container = containerService.getContainer(container.getId());
			}
			containerService.findDataGridPage(dg,dgclass,getDgPage(),container,searchCondition,params);
	...
	private void save() throws Exception {
		...
		containerService.saveContainer(container,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/container/container/containerEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/container/container/containerShadowView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/container/container/containerView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/container/container/newContainerEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/container/container/newContainerView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_Container", "submit");
		bap_validate_datagrids=containerService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", container.getId());
		...
		fileuploads.put("linkId", container.getId());
		bapAfterCustomMethod("MESBasic_1_container_Container", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/container/container/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_Container", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		containerService.deleteContainer(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_container_Container", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/container/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_Container", "restore");
		containerService.restoreContainer(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_container_Container", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/container/container/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/container/container/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_Container", "showDealInfoData");
		if (null != tableInfoId) {
			int count=containerService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = containerService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = containerService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_container_Container", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicContainer> findExportDataInfos(Page<MESBasicContainer> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = containerService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = containerService.getAssProperty(p.getCode());	
									//String assModelCode = containerService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			containerService.findContainers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			containerService.findContainers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicContainer> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicContainer result : results){
	...
	public boolean isEditNew(){
		//return null == this.container || null == this.container.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/container/container/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_Container", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_container_Container", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/container/container/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_Container", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/container/container/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/container/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicContainerService"
						, "com.supcon.orchid.MESBasic.services.MESBasicPropertyTagsService"
						, "com.supcon.orchid.MESBasic.services.MESBasicItemSetupService"
						, "com.supcon.orchid.MESBasic.services.MESBasicContainerExpService"
						, "com.supcon.orchid.MESBasic.services.MESBasicFlowRecordService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProductService"
						, "com.supcon.orchid.MESBasic.services.MESBasicRoutineService"
						, "com.supcon.orchid.MESBasic.services.MESBasicMaintainLogService"
					, "com.supcon.orchid.MESBasic.services.MESBasicContainerExpService"
					, "com.supcon.orchid.MESBasic.services.MESBasicContainerExpService"
					, "com.supcon.orchid.MESBasic.services.MESBasicContainerExpService"
					, "com.supcon.orchid.MESBasic.services.MESBasicContainerExpService"
					...
					, "com.supcon.orchid.MESBasic.services.MESBasicFlowRecordService"
					, "com.supcon.orchid.MESBasic.services.MESBasicFlowRecordService"
					, "com.supcon.orchid.MESBasic.services.MESBasicFunctionInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicFunctionInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicFunctionInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicFunctionInfoService"
					, "com.supcon.orchid.MESBasic.services.MESBasicFunctionInfoService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_container", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_container");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_container_Container", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_container_Container", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/container/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicContainerService"
						, "com.supcon.orchid.MESBasic.services.MESBasicEquipmentService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicItemSetupService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicProductService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicTankModelService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_container_Container", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_container_Container", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/container/container/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_Container", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/container/container/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_container_Container";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_container_Container", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/container/container/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == container.getCode() ? "" : container.getCode().toString());
		_auditLog.setColumnStringA(null == container.getName() ? "" : container.getName().toString());
		_auditLog.setColumnStringC(container.getId().toString());
		_auditLog.setColumnStringD(MESBasicContainer.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_container");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicContainerAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, container,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, container,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,container);
		...
		printlog.setEntity("MESBasic_1_container");
		...
		printlog.setBusinessId(container.getId());
		printlog.setBussinessKey(null == container.getCode() ? "" : container.getCode().toString());
		printlog.setMainDisplay(null == container.getName() ? "" : container.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/container/container/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(container,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/container/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(containerService.getContainer(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/container/container/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/container/container/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, container,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				container = containerService.getContainer(Long.parseLong(arrId[i]));
				if(null != container){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == container.getName() ? "" : "," + container.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == container.getName() ? "" : container.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == container.getCode() ? "" : "," + container.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == container.getCode() ? "" : container.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == container.getId() ? "" : "," + container.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == container.getId() ? "" : container.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, container,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,container);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/container/container/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicContainer> findExportPageInfos(Page<MESBasicContainer> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_container" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = containerService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicContainer queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = containerService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = containerService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_container_Container");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_container_Container")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_container_Container");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_container_Container");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = containerService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_container_Container,MESBasic_1) */
// 自定义代码
        if("MESBasic_1_container_Container".equals(code) && "submit".equals(methodName)){

            StringBuffer sb = new StringBuffer();
            Set<String> tagproperty = new HashSet<String>();

            List<MESBasicPropertyTags> dgList = new ArrayList<>();
            String dg1416532238441 = getDgLists().get("dg1416532238441");
            // 新版的容器编辑视图中的位号PT
            String dg1551701917640 = getDgLists().get("dg1551701917640");
            // 字段对应列表PT
            String dg1553144257690 = getDgLists().get("dg1553144257690");
            if (dg1416532238441 != null){
                dgList = (List<MESBasicPropertyTags>) propertyTagsService.generateObjectFromJson(dg1416532238441, MESBasicPropertyTags.class);
            }
            if (dg1551701917640 != null){
                dgList = (List<MESBasicPropertyTags>) propertyTagsService.generateObjectFromJson(dg1551701917640, MESBasicPropertyTags.class);
            }
            if (dg1553144257690 != null){
                dg1553144257690List = (List<MESBasicContainerExp>) propertyTagsService.generateObjectFromJson(dg1553144257690, MESBasicContainerExp.class);
            }

            for(int i = 0; i < dgList.size(); i++){
                MESBasicPropertyTags tag = dgList.get(i);
                if(tag.getPrimary()){
                    MESBasicItemSetup item = itemSetupService.getItemSetup(tag.getProperty().getId());
                    if(item.getItemClass() != null){
                        String measureId = item.getItemClass().getId();
                        if(tagproperty.contains(measureId)){
                            if(sb.length() == 0){
                                sb.append("相同位号属性的生产位号，只能设置唯一的“主计量”\n以下属性的位号，存在多在主计量:");
                            }
                            sb.append("\n" + InternationalResource.get(item.getItemClass().getValue(), getUserLanguage()));
                        }
                        else{
                            tagproperty.add(measureId);
                        }
                    }else{
                        throw new BAPException("所选测点未设置位号属性：" + item.getCode() + "-" + item.getItemNumber());
                    }
                }
            }
            if(sb.length() > 0){
                String msg = sb.toString();
                throw new BAPException(msg);
            }

            // 模拟计算
            /*String f = container.getVolumneCalc();
            if(StringUtil.isNotEmptyOrNull(f)){
                for (MESBasicContainerExp exp : dg1553144257690List) {
                    String representCode = exp.getRepresentCode();
                    if (f.contains(representCode)){
                        // 将所有未知数替换成数字1
                        f = f.replaceAll(representCode, "1");
                    }
                }

                try{
                    Calculator.scalculateJS(f);
                }catch(Exception ex){
                    throw new BAPException("体积计算公式设置错误，请检查。");
                }
            }*/
        }
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_container_Container,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_container_Container,MESBasic_1) */
// 自定义代码
