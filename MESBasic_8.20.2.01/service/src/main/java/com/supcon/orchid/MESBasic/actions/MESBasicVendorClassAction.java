...
/* CUSTOM CODE START(action,import,MESBasic_1_baseVendor_VendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicVendorClassAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicVendorClass> {
	...
	private MESBasicVendorClass vendorClass;
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseVendor/vendorClass/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseVendor/vendorClass/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_baseVendor_VendorClass" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicVendorClass> vendorClasss;
	
	public List<MESBasicVendorClass> getVendorClasss() {
		return vendorClasss;
	}
	...
	private Page<MESBasicVendorClass> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			vendorClass = new MESBasicVendorClass();
		} else {
			if(id > 0){
				vendorClass = vendorClassService.getVendorClass(id, viewCode);
			}
		}
	...
	public Page<MESBasicVendorClass> getPage() {
		if (null == page) {
			page = new Page<MESBasicVendorClass>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicVendorClass getVendorClass() {
		return vendorClass;
	}		

	public void setVendorClass(MESBasicVendorClass vendorClass) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseVendor/vendorClass/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = vendorClassService.getVendorClassAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseVendor/vendorClass/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1436836898066
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String vendorClassReferenceCopy() throws Exception{
			MESBasicVendorClass oldVendorClass = null;
			if(isEditNew()){
				...
				oldVendorClass = new MESBasicVendorClass();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,vendorClass.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, vendorClass.getId());
				...
				oldVendorClass = vendorClassService.getVendorClass(id);
			}
			
			
			vendorClass = vendorClassService.getVendorClass(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_baseVendor_VendorClass");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicVendorClass.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldVendorClass, null);
						Method setValue = MESBasicVendorClass.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(vendorClass, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			bapAfterCustomMethod("MESBasic_1_baseVendor_VendorClass", "referenceCopy");
			return SUCCESS;
		}
	...
	private void save(DataGridService dataGridService) throws Exception {
		if(null == vendorClass.getId()) {
			responseMap.put("operate", "add");
		} else {
			responseMap.put("operate", "edit");
		}
		vendorClassService.saveVendorClass(vendorClass, dataGridService, viewCode);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseVendor_VendorClass", "submit");
		DataGridService dataGridService = null;
		
			save(dataGridService);
			responseMap.put("id", vendorClass.getId());

		Map<String, Object> fileuploads = (Map<String, Object>) request.getAttribute("__orchid_fileuploads__");
		fileuploads.put("linkId", vendorClass.getId());
		bapAfterCustomMethod("MESBasic_1_baseVendor_VendorClass", "submit");
		return SUCCESS;
	}
	@Action(value = "/MESBasic/baseVendor/vendorClass/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_VendorClass", "delete");
		vendorClassService.deleteVendorClass(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_baseVendor_VendorClass", "delete");
		return SUCCESS;
	}
	...
	@Override
	public Page<MESBasicVendorClass> findExportDataInfos(Page<MESBasicVendorClass> page) {
		if(null != searchObjects && searchObjects.length > 0) {
			vendorClassService.findVendorClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, searchObjects);
		} else {
			vendorClassService.findVendorClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode);
		}
		return getPage();
	}

	public boolean isEditNew(){
		//return null == this.vendorClass || null == this.vendorClass.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/baseVendor/vendorClass/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_VendorClass", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_baseVendor_VendorClass", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/baseVendor/vendorClass/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseVendor_VendorClass", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/baseVendor/vendorClass/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVendor/vendorClass/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicVendorClassService"
		};
		bapBeforeCustomMethod("MESBasic_1_baseVendor_VendorClass", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_baseVendor_VendorClass", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVendor/vendorClass/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				xlsResult = importService.imports(viewCode, "MESBasic_1_baseVendor_VendorClass", dataFile, time, name[0], isReplace, isIgnore, importServices);
	...
	@Action(value = "/MESBasic/baseVendor/vendorClass/downLoad", results = { @Result(type = "stream", params = {
  ...
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_VendorClass", "downLoadXls");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVendor/vendorClass/templateDownLoad", results = { @Result(type = "stream", params = {
			"contentType", "application/excel", "inputName", "inputStream",
			"contentDisposition", 
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String templateDownLoad() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVendor_VendorClass", "templateDownLoad");
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/baseVendor/vendorClass/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		MESBasicVendorClass vendorClass = null;
		if(null != id) {
			vendorClass = vendorClassService.getVendorClass(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, vendorClass, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, vendorClass, null, null, true, this);
	...
	@Action(value = "/MESBasic/baseVendor/vendorClass/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/baseVendor/vendorClass/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				vendorClass = vendorClassService.getVendorClass(Integer.parseInt(id));
				if(vendorClass != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, vendorClass, null, null, true, this);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_baseVendor_VendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_baseVendor_VendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_baseVendor_VendorClass,MESBasic_1) */
// 自定义代码
