...
@Service("mESBasicBatchDataImportService")
public class MESBasicBatchDataImportServiceImpl implements MESBasicBatchDataImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_batchDataHandler",model="MESBasic_1_batchDataHandler_BatchData",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicBatchData bizObj, MESBasicBatchData oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1446448528424") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getTag() ? "" : "," + oldBizObj.getTag().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getTag() ? "" : oldBizObj.getTag().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1446448528424") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getTag() ? "" : "," + bizObj.getTag().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getTag() ? "" : bizObj.getTag().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicBatchData bizObj, String bussinesskeyFieldName,
