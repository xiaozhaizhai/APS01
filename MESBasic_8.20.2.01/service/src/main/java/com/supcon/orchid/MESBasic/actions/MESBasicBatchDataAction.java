...
/* CUSTOM CODE START(action,import,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
...
public class MESBasicBatchDataAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBatchData> {
	...
	private MESBasicBatchData batchData;
	...
	//导出excel时,主模型内容
	List<MESBasicBatchData> queryResults = new ArrayList<MESBasicBatchData>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/batchDataHandler/batchData/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/batchDataHandler/batchData/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_batchDataHandler_BatchData" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBatchData> batchDatas;
	
	public List<MESBasicBatchData> getBatchDatas() {
		return batchDatas;
	}
	...
	private Page<MESBasicBatchData> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchData = new MESBasicBatchData();
			if(batchData.getBusinessType() != null) {
				batchData.setBusinessType((SystemCode) systemCodeService.getSystemCode(batchData.getBusinessType().getId()));
			}
			if(batchData.getSignalSource() != null) {
				batchData.setSignalSource((SystemCode) systemCodeService.getSystemCode(batchData.getSignalSource().getId()));
			}
		} else {
			if(id > 0){
				batchData = batchDataService.getBatchData(id, viewCode);
			}
		}
	...
	public Page<MESBasicBatchData> getPage() {
		if (null == page) {
			page = new Page<MESBasicBatchData>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicBatchData getBatchData() {
		return batchData;
	}		

	public void setBatchData(MESBasicBatchData batchData) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/batchDataHandler/batchData/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchDataService.getBatchDataAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/batchDataHandler/batchData/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1446448588505
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchDataReferenceCopy() throws Exception{
			MESBasicBatchData oldBatchData = null;
			if(isEditNew()){
				...
				oldBatchData = new MESBasicBatchData();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchData.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchData.getId());
				...
				oldBatchData = batchDataService.getBatchData(id);
			}
			
			
			batchData = batchDataService.getBatchData(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_batchDataHandler_BatchData");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBatchData.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchData, null);
						Method setValue = MESBasicBatchData.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchData, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchData.setCreateDepartmentId(oldBatchData.getCreateDepartmentId());
			batchData.setCreatePositionId(oldBatchData.getCreatePositionId());
			batchData.setCreateStaffId(oldBatchData.getCreateStaffId());
			batchData.setOwnerDepartmentId(oldBatchData.getOwnerDepartmentId());
			batchData.setOwnerPositionId(oldBatchData.getOwnerPositionId());
			batchData.setOwnerStaffId(oldBatchData.getOwnerStaffId());
			batchData.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_batchDataHandler_BatchData", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchDataHandler/batchData/dataEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataEdit.ftl") }),
		@Action(value = "/MESBasic/batchDataHandler/batchData/dataEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchDataService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchDataReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchData = batchDataService.getBatchData(this.batchData.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchData.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchDataHandler/batchData/dataView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataView.ftl") }),
		@Action(value = "/MESBasic/batchDataHandler/batchData/dataViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchDataHandler/batchData/dataList", results = { @Result(type = FREEMARKER, location = "dataList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchDataService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchDataService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchDataService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_batchDataHandler_BatchData", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchDataHandler/batchData/dataList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.processed,result.tag,result.tvalue,result.purchaseCode,result.content,result.businessType.id,result.businessType.value,result.dataTime,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="dataList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchData\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchData\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchData\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchData\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchDataHandler/batchData/dataList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/batchDataHandler/batchData/allTempleteDownload", results = { @Result(name="excel",type="excel",location="dataList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_batchDataHandler_BatchData");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchDataService.findDataGridPage(dg,dgclass,getDgPage(),batchData,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchData.getId() != null && batchData.getId() != -1){
				batchData = batchDataService.getBatchData(batchData.getId());
			}
			batchDataService.findDataGridPage(dg,dgclass,getDgPage(),batchData,searchCondition,params);
	...
	private void save() throws Exception {
		...
		batchDataService.saveBatchData(batchData,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/batchDataHandler/batchData/dataEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/batchDataHandler/batchData/dataView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "submit");
		bap_validate_datagrids=batchDataService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchData.getId());
		...
		fileuploads.put("linkId", batchData.getId());
		bapAfterCustomMethod("MESBasic_1_batchDataHandler_BatchData", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/batchDataHandler/batchData/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchDataService.deleteBatchData(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_batchDataHandler_BatchData", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchDataHandler/batchData/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "restore");
		batchDataService.restoreBatchData(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_batchDataHandler_BatchData", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/batchDataHandler/batchData/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/batchDataHandler/batchData/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "showDealInfoData");
		if (null != tableInfoId) {
			int count=batchDataService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = batchDataService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = batchDataService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_batchDataHandler_BatchData", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBatchData> findExportDataInfos(Page<MESBasicBatchData> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchDataService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchDataService.getAssProperty(p.getCode());	
									//String assModelCode = batchDataService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchDataService.findBatchDatas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchDataService.findBatchDatas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBatchData> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBatchData result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchData || null == this.batchData.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/batchDataHandler/batchData/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_batchDataHandler_BatchData", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/batchDataHandler/batchData/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/batchDataHandler/batchData/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchDataHandler/batchData/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBatchDataService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_batchDataHandler", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_batchDataHandler");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_batchDataHandler_BatchData", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchDataHandler/batchData/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_batchDataHandler_BatchData", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_batchDataHandler_BatchData", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/batchDataHandler/batchData/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/batchDataHandler/batchData/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_batchDataHandler_BatchData";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_batchDataHandler_BatchData", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/batchDataHandler/batchData/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicBatchData batchData = null;
		if(null != id) {
			batchData = batchDataService.getBatchData(id);
		}
		...
		_auditLog.setColumnStringB(null == batchData.getId() ? "" : batchData.getId().toString());
		_auditLog.setColumnStringA(null == batchData.getTag() ? "" : batchData.getTag().toString());
		_auditLog.setColumnStringC(batchData.getId().toString());
		_auditLog.setColumnStringD(MESBasicBatchData.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_batchDataHandler");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBatchDataAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchData,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchData,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchData);
		...
		printlog.setEntity("MESBasic_1_batchDataHandler");
		...
		printlog.setBusinessId(batchData.getId());
		printlog.setBussinessKey(null == batchData.getId() ? "" : batchData.getId().toString());
		printlog.setMainDisplay(null == batchData.getTag() ? "" : batchData.getTag().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/batchDataHandler/batchData/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchData,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchDataHandler/batchData/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchDataService.getBatchData(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/batchDataHandler/batchData/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/batchDataHandler/batchData/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchData,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchData = batchDataService.getBatchData(Long.parseLong(arrId[i]));
				if(null != batchData){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchData.getTag() ? "" : "," + batchData.getTag().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchData.getTag() ? "" : batchData.getTag().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchData.getId() ? "" : "," + batchData.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchData.getId() ? "" : batchData.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchData.getId() ? "" : "," + batchData.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchData.getId() ? "" : batchData.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchData,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchData);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/batchDataHandler/batchData/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicBatchData> findExportPageInfos(Page<MESBasicBatchData> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_batchDataHandler" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = batchDataService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicBatchData queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = batchDataService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = batchDataService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_batchDataHandler_BatchData");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_batchDataHandler_BatchData")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_batchDataHandler_BatchData");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_batchDataHandler_BatchData");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchDataService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
      if(code.equals("MESBasic_1_batchDataHandler_dataList")&&methodName.equals("dataListQuery")){
			Long maxId=0L;
			MESBasicItemSeq itemSeq=itemSeqService.getMaxIdSeq("batchManag_1.0");
			if(itemSeq!=null && itemSeq.getMaxId()!=null){
				maxId=itemSeq.getMaxId();
			}
			page=getPage();
			List<MESBasicBatchData> pageList=page.getResult();
			List<MESBasicBatchData> newPageList=new ArrayList<MESBasicBatchData>();
			for(MESBasicBatchData dataTrans :pageList){
				if(dataTrans.getId().compareTo(maxId)<=0){
					dataTrans.setProcessed(true);
				}
				newPageList.add(dataTrans);
			}
			page.setResult(newPageList);
			
		}else if(code.equals("MESBasic_1_batchDataHandler_dataEdit")&&methodName.equals("dataEdit")){
			if(!isEditNew()){//编辑
				Long maxId=0L;
				MESBasicItemSeq itemSeq=itemSeqService.getMaxIdSeq("batchManag_1.0");
				if(itemSeq!=null && itemSeq.getMaxId()!=null){
				...
				if(batchData.getId().compareTo(maxId)<=0){
					batchData.setProcessed(true);
				}
				batchDataService.updateCheck(batchData.getId());
			}
		}
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
@Autowired
