...
import com.supcon.orchid.MESBasic.services.MESBasicRoutineImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicRoutineDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
import net.sf.ehcache.Ehcache;
import com.supcon.orchid.cacheadmin.CacheAdmin;
import com.supcon.orchid.foundation.services.DataSourceService;
import com.supcon.orchid.MESBasic.lib.RoutineHelper;

import java.sql.SQLException;
/* CUSTOM CODE END */

@Service("mESBasic_routineService")
@Transactional
public class MESBasicRoutineServiceImpl  extends BaseServiceImpl<MESBasicRoutine> implements MESBasicRoutineService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicRoutine> bapBaseService;
	...
	@Autowired
	private MESBasicRoutineImportService routineImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicRoutine.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  routineDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicRoutine.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = routineDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicRoutine.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicRoutine.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  routineDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicRoutine.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicRoutine.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicRoutine> list = (List<MESBasicRoutine>) routineDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicRoutine getRoutine(long id){
		return getRoutine(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicRoutine getRoutine(long id, String viewCode){
		MESBasicRoutine routine = routineDao.load(id);

		// 一对多情况处理
		if(routine != null){
			this.setRoutinePump(routine, viewCode);
			this.setRoutineSwitcher(routine, viewCode);
			this.setRoutineMeter(routine, viewCode);
			this.setRoutineItemSetup(routine, viewCode);
			this.setConfigSwitcher(routine, viewCode);
		}
		return routine;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getRoutineAsJSON(long id, String include){
		MESBasicRoutine routine = routineDao.load(id);
		// 一对多情况处理
			this.setRoutinePump(routine);
			this.setRoutineSwitcher(routine);
			this.setRoutineMeter(routine);
			this.setRoutineItemSetup(routine);
			this.setConfigSwitcher(routine);

		if(routine == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(routine, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.common.delete",operType="3")
	public void deleteRoutine(MESBasicRoutine routine){
		if(null != routine && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicRoutine.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == routine.getCode() ? "" : routine.getCode().toString());
			AuditUtil.setColumnStringA(null == routine.getName() ? "" : routine.getName().toString());
			AuditUtil.setColumnStringC(routine.getId().toString());
		}
		beforeDeleteRoutine(routine);
		routineDao.delete(routine);
		afterDeleteRoutine(routine);
		routineDao.flush();
		List<MESBasicRoutine> params = new ArrayList<MESBasicRoutine>();
		List<MESBasicConfig> configList = configDao.findByCriteria(Restrictions.eq("routine", routine));
		routine.setConfigList(configList);
		List<MESBasicSwitcherModel> switcherModelList = switcherModelDao.findByCriteria(Restrictions.eq("config", routine));
		routine.setSwitcherModelList(switcherModelList);
		List<MESBasicInvolveItemsetups> involveItemsetupsList = involveItemsetupsDao.findByCriteria(Restrictions.eq("routine", routine));
		routine.setInvolveItemsetupsList(involveItemsetupsList);
		params.add(routine);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
		props.put("eventType", "delete");
		String delIds = "";
		// 一对多情况处理
		delIds = routine.getRoutinePumpmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			...
			if (!idList.isEmpty()) {
				List<MESBasicConfig> tmpList = configDao.findByCriteria(Restrictions.eq("routine", routine), Restrictions.in("pump.id", idList), Restrictions.eq("valid", true));
				for (MESBasicConfig item : tmpList) {
					configDao.deletePhysical(item);
				}
			}
		}
		delIds = routine.getRoutineSwitchermultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<MESBasicConfig> tmpList = configDao.findByCriteria(Restrictions.eq("routine", routine), Restrictions.in("switcher.id", idList), Restrictions.eq("valid", true));
				for (MESBasicConfig item : tmpList) {
					configDao.deletePhysical(item);
				}
			}
		}
		delIds = routine.getRoutineMetermultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<MESBasicConfig> tmpList = configDao.findByCriteria(Restrictions.eq("routine", routine), Restrictions.in("meter.id", idList), Restrictions.eq("valid", true));
				for (MESBasicConfig item : tmpList) {
					configDao.deletePhysical(item);
				}
			}
		}
		delIds = routine.getRoutineItemSetupmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<MESBasicInvolveItemsetups> tmpList = involveItemsetupsDao.findByCriteria(Restrictions.eq("routine", routine), Restrictions.in("itemSetup.id", idList), Restrictions.eq("valid", true));
				for (MESBasicInvolveItemsetups item : tmpList) {
					involveItemsetupsDao.deletePhysical(item);
				}
			}
		}
		delIds = routine.getConfigSwitchermultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
			...
			if (!idList.isEmpty()) {
				List<MESBasicSwitcherModel> tmpList = switcherModelDao.findByCriteria(Restrictions.eq("config", routine), Restrictions.in("switcher.id", idList), Restrictions.eq("valid", true));
				for (MESBasicSwitcherModel item : tmpList) {
					switcherModelDao.deletePhysical(item);
				}
			}
		}
			// TODO delete
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.common.delete",operType="3")
	public void deleteRoutine(List<Long> routineIds){
		deleteRoutine(routineIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.common.delete",operType="3")
	public void deleteRoutine(List<Long> routineIds, String eventTopic) {
		List<MESBasicRoutine> routines = new ArrayList<MESBasicRoutine>();
		for(Long routineId : routineIds){
			MESBasicRoutine routine = getRoutine(routineId);
			routines.add(routine);
			if(null != routine && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicRoutine.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == routine.getName() ? "" : "," + routine.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == routine.getName() ? "" : routine.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == routine.getCode() ? "" : "," + routine.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == routine.getCode() ? "" : routine.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == routine.getId() ? "" : "," + routine.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == routine.getId() ? "" : routine.getId().toString());
				}
			}
		}
		...
		if(routines != null && routines.size() > 0 ){
			for(MESBasicRoutine routine : routines){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),routine, routine.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(routineIds != null && routineIds.size() > 0) {
			String hql = "update " + MESBasicRoutine.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = routineDao.createQuery(hql);
			query.setParameterList("ids", routineIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicRoutine routinez : routines){
				routinez.setValid(false);
				List<MESBasicRoutine> params = new ArrayList<MESBasicRoutine>();
				List<MESBasicConfig> configList = configDao.findByCriteria(Restrictions.eq("routine", routinez));
				routinez.setConfigList(configList);
				List<MESBasicSwitcherModel> switcherModelList = switcherModelDao.findByCriteria(Restrictions.eq("config", routinez));
				routinez.setSwitcherModelList(switcherModelList);
				List<MESBasicInvolveItemsetups> involveItemsetupsList = involveItemsetupsDao.findByCriteria(Restrictions.eq("routine", routinez));
				routinez.setInvolveItemsetupsList(involveItemsetupsList);
				params.add(routinez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_routine");
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.common.delete",operType="3")
	public void deleteRoutine(long routineId, int routineVersion){
		this.deleteRoutine(routineId, routineVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.common.delete",operType="3")
	public void deleteRoutine(long routineId, int routineVersion,SignatureLog signatureLog){
		MESBasicRoutine routine = getRoutine(routineId);
		...
		beforeDeleteRoutine(routine);
		routineDao.delete(routineId, routineVersion);
		afterDeleteRoutine(routine);
		routineDao.flush();
		List<MESBasicRoutine> params = new ArrayList<MESBasicRoutine>();
		List<MESBasicConfig> configList = configDao.findByCriteria(Restrictions.eq("routine", routine));
		...
		if(signatureLog != null ) {
			...
			businessKey=routine.getCode();
			signatureLog.setTableId(routine.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.common.delete",operType="3")
	public void deleteRoutine(String routineIds){
		this.deleteRoutine(routineIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.common.delete",operType="3")
	public void deleteRoutine(String routineIds,SignatureLog signatureLog){
		deleteCollection(routineIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteRoutine(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_routine#modelCode:MESBasic_1_routine_Routine#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="ec.engine.restore",operType="10")
	public void restoreRoutine(String routineIds){
		restoreCollection(routineIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreRoutine(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="ec.engine.restore",operType="10")
	public void restoreRoutine(long routineId){
		
		findBusinessKeyUsed(routineId);	//判断业务主键是否重复
		...
		beforeRestoreRoutine(routine);
		routine.setValid(true);
		routineDao.update(routine);
		afterRestoreRoutine(routine);
		List<MESBasicRoutine> params = new ArrayList<MESBasicRoutine>();
		List<MESBasicConfig> configList = configDao.findByCriteria(Restrictions.eq("routine", routine));
		routine.setConfigList(configList);
		List<MESBasicSwitcherModel> switcherModelList = switcherModelDao.findByCriteria(Restrictions.eq("config", routine));
	...
	public void findBusinessKeyUsed(long routineId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_routine_Routine");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicRoutine.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicRoutine.TABLE_NAME +" where id = ? )";
			List<Object> list =  routineDao.createNativeQuery(sql,routineId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseRoutine(List<MESBasicRoutine>  routines){
		for(MESBasicRoutine routine:routines)  {
			saveRoutine(routine, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseRoutine(List<MESBasicRoutine>  routines){
		for(MESBasicRoutine routine:routines)  {
			saveRoutine(routine, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="ec.flow.save")
	public void saveRoutine(MESBasicRoutine routine, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveRoutine(routine, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="ec.flow.save")
	public void saveRoutine(MESBasicRoutine routine, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveRoutine(routine, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateRoutineCodes(MESBasicRoutine routine) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateRoutineCodes(routine, false);
	}
	
	public void generateRoutineCodes(MESBasicRoutine routine, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateRoutineSummarys(MESBasicRoutine routine) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateRoutineSummarys(routine, false);
	}

	public void generateRoutineSummarys(MESBasicRoutine routine, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine")
	public void saveRoutine(MESBasicRoutine routine,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveRoutine(routine, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine")
	public void saveRoutine(MESBasicRoutine routine,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
		String url = null;
		if(routine.getId() != null && routine.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicRoutine.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_routine/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicRoutine.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_routine/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, routine);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(routine);
		beforeSaveRoutine(routine, viewIsView);

		if (viewIsView) {
			routineDao.saveWithRevertVersion(routine);
		} else {
			if(isNew)
				routineDao.save(routine);
			else
				routineDao.merge(routine);
		}
		if(null != routine && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicRoutine.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == routine.getCode() ? "" : routine.getCode().toString());
			AuditUtil.setColumnStringA(null == routine.getName() ? "" : routine.getName().toString());
		...
		dealDatagridsSave(routine,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
			this.dealRoutinePump(routine);
			this.dealRoutineSwitcher(routine);
			this.dealRoutineMeter(routine);
			this.dealRoutineItemSetup(routine);
			this.dealConfigSwitcher(routine);
		
		// 根据配置规则生成编码
		try {
			generateRoutineCodes(routine, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateRoutineSummarys(routine, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveRoutine(routine, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, routine);
			}
		}
		List<MESBasicRoutine> params = new ArrayList<MESBasicRoutine>();
		routineDao.flush();
		routineDao.clear();
		routine = routineDao.load(routine.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicRoutine routine){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine")
	public void mergeRoutine(MESBasicRoutine routine, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(routine);
		beforeSaveRoutine(routine);
		routineDao.merge(routine);
		if(null != routine && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicRoutine.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == routine.getCode() ? "" : routine.getCode().toString());
			AuditUtil.setColumnStringA(null == routine.getName() ? "" : routine.getName().toString());
		...
		dealDatagridsSave(routine,null,dgLists,dgDeleteIDs,assFileUploads);
			...
			this.dealRoutineItemSetup(routine);
			this.dealConfigSwitcher(routine);
		afterSaveRoutine(routine);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicRoutine> findRoutines(Page<MESBasicRoutine> page, Criterion... criterions) {
		return routineDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicRoutineDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicRoutineDealInfo> list=routineDao.createQuery("from " + MESBasicRoutineDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicRoutineDealInfo dealInfo) {
		routineDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicRoutineDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) routineDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicRoutineDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = routineDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = routineDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicRoutineDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = routineDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRoutines(Page<MESBasicRoutine> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findRoutines(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRoutines(Page<MESBasicRoutine> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findRoutines(Page<MESBasicRoutine> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicRoutine> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"routine\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"routine\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"routine\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_routine_Routine" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_routine_Routine" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  routineDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  routineDao.createNativeQuery(powerOperateSql, "MESBasic_1_routine").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"routine\"","MESBasic_1_routine",customCondition,"MESBASIC_ROUTINES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"routine\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicRoutineDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"routine\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"routine\".LAY_REC ASC, \"routine\".SORT ASC");
				} else {
					orderPart.append(" \"routine\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) routineDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=routineDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = routineDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicRoutine> result = new ArrayList<MESBasicRoutine>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicRoutine.class,routineDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicRoutine.class,routineDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicRoutine routine : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(routine.getId(), "MESBasic_routine_routine");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(routine.getId(), "MESBasic_routine_routine");
							if(null != documents && !documents.isEmpty()) {
								routine.setDocument(documents.get(0));
								routine.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_routine")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_routine")+"MESBasic_1_routine".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_routine",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicRoutine> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicRoutine> result) {
		View view = null;
		...
		Map<Long, MESBasicRoutine> rsMap = new HashMap<Long, MESBasicRoutine>();
		for (MESBasicRoutine routine : result) {
			ids.add(routine.getId());
			rsMap.put(routine.getId(), routine);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(routineDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = routineDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicRoutine routine,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(routine.getOwnerStaffId());
		ti.setOwnerPositionId(routine.getOwnerPositionId());
		ti.setOwnerDepartmentId(routine.getOwnerDepartmentId());
		
		ti.setCreateStaff(routine.getCreateStaff());
		ti.setCreateStaffId(routine.getCreateStaffId());
		ti.setCreateTime(routine.getCreateTime());
		ti.setDeleteStaff(routine.getDeleteStaff());
		ti.setDeleteStaffId(routine.getDeleteStaffId());
		ti.setDeleteTime(routine.getDeleteTime());
		ti.setCreateDepartmentId(routine.getCreateDepartmentId());
		ti.setModifyStaff(routine.getModifyStaff());
		ti.setModifyStaffId(routine.getModifyStaffId());
		ti.setCreatePositionId(routine.getCreatePositionId());
		ti.setPositionLayRec(routine.getPositionLayRec());
		ti.setTableNo(routine.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_routine");
		ti.setTargetTableName("MESBASIC_ROUTINES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_routine_Routine" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  routineDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"routine\"","MESBasic_1_routine","","MESBASIC_ROUTINES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"routine\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"routine\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"routine\"","MESBasic_1_routine","","MESBASIC_ROUTINES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("routine.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"routine\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("routine.mc\"").append(".ROUTINE) not in ("+notinValue+")");
			}
			
			SQLQuery query = routineDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("routine", "MESBasic_1_routine", "routine_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=routineDao.createNativeQuery(sql, menuCode);
	...
	// ================ 生成多选控件数据保存方法 start====================
	private void dealRoutinePump(MESBasicRoutine routine) {
		String addIds = routine.getRoutinePumpAddIds();
		String delIds = routine.getRoutinePumpDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					MESBasicConfig tmp = new MESBasicConfig();
					tmp.setRoutine(routine);
					MESBasicItemSetup indirectObj = new MESBasicItemSetup();
					indirectObj.setId(Long.valueOf(item));
					tmp.setPump(indirectObj);
					tmp.setCid(routine.getCid());
					configDao.save(tmp);
				}
			}
		}
	...
	private void setRoutinePump(MESBasicRoutine routine) {
		setRoutinePump(routine, null);
	}

	private void setRoutinePump(MESBasicRoutine routine, String viewCode) {
		List<MESBasicConfig> tmpList = configDao.findByCriteria(Order.asc("id"), Restrictions.eq("routine", routine), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (MESBasicConfig item : tmpList) {
			if(item.getPump() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getPump().getId();
				String displayField = "itemName";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getPump(), displayField);
			}
		}
		routine.setConfigList(tmpList);
		routine.setRoutinePumpmultiselectIDs(ids);
		routine.setRoutinePumpmultiselectNames(names);
	}
	private void dealRoutineSwitcher(MESBasicRoutine routine) {
		String addIds = routine.getRoutineSwitcherAddIds();
		String delIds = routine.getRoutineSwitcherDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
	...
	private void setRoutineSwitcher(MESBasicRoutine routine) {
		setRoutineSwitcher(routine, null);
	}

	private void setRoutineSwitcher(MESBasicRoutine routine, String viewCode) {
		List<MESBasicConfig> tmpList = configDao.findByCriteria(Order.asc("id"), Restrictions.eq("routine", routine), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (MESBasicConfig item : tmpList) {
			if(item.getSwitcher() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getSwitcher().getId();
				String displayField = "itemName";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getSwitcher(), displayField);
			}
		}
		routine.setConfigList(tmpList);
		routine.setRoutineSwitchermultiselectIDs(ids);
		routine.setRoutineSwitchermultiselectNames(names);
	}
	private void dealRoutineMeter(MESBasicRoutine routine) {
		String addIds = routine.getRoutineMeterAddIds();
		String delIds = routine.getRoutineMeterDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
	...
	private void setRoutineMeter(MESBasicRoutine routine) {
		setRoutineMeter(routine, null);
	}

	private void setRoutineMeter(MESBasicRoutine routine, String viewCode) {
		List<MESBasicConfig> tmpList = configDao.findByCriteria(Order.asc("id"), Restrictions.eq("routine", routine), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (MESBasicConfig item : tmpList) {
			if(item.getMeter() != null) {
				...
				ids += item.getMeter().getId();
				String displayField = "itemName";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getMeter(), displayField);
			}
		}
		routine.setConfigList(tmpList);
		routine.setRoutineMetermultiselectIDs(ids);
		routine.setRoutineMetermultiselectNames(names);
	}
	private void dealRoutineItemSetup(MESBasicRoutine routine) {
		String addIds = routine.getRoutineItemSetupAddIds();
		String delIds = routine.getRoutineItemSetupDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					MESBasicInvolveItemsetups tmp = new MESBasicInvolveItemsetups();
					tmp.setRoutine(routine);
					MESBasicItemSetup indirectObj = new MESBasicItemSetup();
					indirectObj.setId(Long.valueOf(item));
					tmp.setItemSetup(indirectObj);
					tmp.setCid(routine.getCid());
					involveItemsetupsDao.save(tmp);
				}
			}
		}
	}

	private void setRoutineItemSetup(MESBasicRoutine routine) {
		setRoutineItemSetup(routine, null);
	}

	private void setRoutineItemSetup(MESBasicRoutine routine, String viewCode) {
		List<MESBasicInvolveItemsetups> tmpList = involveItemsetupsDao.findByCriteria(Order.asc("id"), Restrictions.eq("routine", routine), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (MESBasicInvolveItemsetups item : tmpList) {
			if(item.getItemSetup() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getItemSetup().getId();
				String displayField = "itemName";
				if(viewCode != null) {
					switch (viewCode) {
					case "MESBasic_1_routine_routineEdit":
						displayField = "code";
						break;
					case "MESBasic_1_routine_routineViewAll":
						displayField = "code";
						break;
					case "MESBasic_1_routine_routineView":
						displayField = "code";
						break;
					}
				}
				names += ReflectUtils.getPropertyValue(item.getItemSetup(), displayField);
			}
		}
		routine.setInvolveItemsetupsList(tmpList);
		routine.setRoutineItemSetupmultiselectIDs(ids);
		routine.setRoutineItemSetupmultiselectNames(names);
	}
	private void dealConfigSwitcher(MESBasicRoutine routine) {
		String addIds = routine.getConfigSwitcherAddIds();
		String delIds = routine.getConfigSwitcherDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					MESBasicSwitcherModel tmp = new MESBasicSwitcherModel();
					tmp.setConfig(routine);
					MESBasicItemSetup indirectObj = new MESBasicItemSetup();
					indirectObj.setId(Long.valueOf(item));
					tmp.setSwitcher(indirectObj);
					tmp.setCid(routine.getCid());
					switcherModelDao.save(tmp);
				}
			}
		}
	}

	private void setConfigSwitcher(MESBasicRoutine routine) {
		setConfigSwitcher(routine, null);
	}

	private void setConfigSwitcher(MESBasicRoutine routine, String viewCode) {
		List<MESBasicSwitcherModel> tmpList = switcherModelDao.findByCriteria(Order.asc("id"), Restrictions.eq("config", routine), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (MESBasicSwitcherModel item : tmpList) {
			if(item.getSwitcher() != null) {
		...
		routine.setSwitcherModelList(tmpList);
		routine.setConfigSwitchermultiselectIDs(ids);
		routine.setConfigSwitchermultiselectNames(names);
	}
	// ================ 生成多选控件数据保存方法 end ====================

	protected String getEntityCode(){return MESBasicRoutine.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicRoutine.MODULE_CODE;}
	protected String getModelCode(){return MESBasicRoutine.MODEL_CODE;}
	 ...
	 * @param routine 模型实例
	 */
	public void checkUniqueConstraint(MESBasicRoutine routine){
		if(routine.getId()==null){
			List<MESBasicRoutine> routines = routineDao.findByCriteria(Restrictions.eq("code", routine.getCode()), Restrictions.eq("valid", true));
			if(routines!=null && !routines.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.radion1415336575756", getCurrentUser().getLanguage()), routine.getCode());
			}
		} else {
			List<MESBasicRoutine> routines = routineDao.findByCriteria(Restrictions.ne("id", routine.getId()), Restrictions.eq("code", routine.getCode()), Restrictions.eq("valid", true));
			if(routines!=null && !routines.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.radion1415336575756", getCurrentUser().getLanguage()), routine.getCode());
			}
	...
	@Override
	public List<MESBasicRoutine> getRoutines(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicRoutine.class);
		if(params!=null){
			...
			return routineDao.findByHql("from " + MESBasicRoutine.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return routineDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicRoutine> findRoutinesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicRoutine.class);
	...
	@Override
	public List<MESBasicRoutine> findRoutinesByHql(String hql, Object... objects){
		
		return   routineDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicRoutine loadRoutineByBussinessKey(MESBasicRoutine routine){
		return loadRoutineByBussinessKey(routine.getCode());
	}
	
	@Override
	public Page<MESBasicRoutine> getRoutines(Page<MESBasicRoutine> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicRoutine.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = routineDao.findByPage(page, sql);
			}else{
				page = routineDao.findAllByPage(page);
			}
			return page;
		}
		page = routineDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicRoutine loadRoutineByBussinessKey(Serializable bussinessKey){
		return routineDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicRoutine> findByProperty(String propertyName, Object object){
		return routineDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicRoutine findEntityByProperty(String propertyName, Object object){
		return routineDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicRoutine.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = routineDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_routine_Routine","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_routine", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicRoutine routine,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,routineDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicRoutine.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, routine);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",routine.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicRoutine.TABLE_NAME +" where valid = 1";
			list = routineDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicRoutine.TABLE_NAME;
			list = routineDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicRoutine.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = routineDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicRoutineDaoImpl routineDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchRoutine(final List<MESBasicRoutine> insertObjs, final List<MESBasicRoutine> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchRoutine(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchRoutine(final List<MESBasicRoutine> insertObjs, final List<MESBasicRoutine> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1415336555124"),importCount));
		...
		Map<Long, MESBasicRoutine> oldUpdateObjsMap = new HashMap<Long, MESBasicRoutine>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicRoutine bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicRoutine> bizObjList = routineDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicRoutine bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicRoutine> nmeObjs = new ArrayList<MESBasicRoutine>();
		...
		long id=autoGeneratorID.getNextId(MESBasicRoutine.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicRoutine routine:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicRoutine.TABLE_NAME,1,dbId);
			if(routine.getCode() != null && !routine.getCode().equals("") ){
				m.put(routine.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) routineDao.createNativeQuery(sql, "MESBasic_1_routine_Routine"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		routineDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicRoutine.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicRoutine routine = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = routine.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicRoutine.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicRoutine.TABLE_NAME + " order by id desc";
//					Query query = routineDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicRoutine routine = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = routine.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		routineDaoImpl.getSession().flush();
		routineDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicRoutine bizObj = insertObjs.get(insertIndex);
			routineImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicRoutine bizObj = updateObjs.get(updateIndex);
			MESBasicRoutine oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			routineImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = routineDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				routineDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = routineDao.createNativeQuery(sql, "MESBasic_1_routine_Routine").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = routineDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = routineDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = routineDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?routineDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  routineDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,routineDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1415338444009Page(Page<MESBasicConfig> dg1415338444009Page,MESBasicRoutine routine, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(MESBasicConfig.class);
		criteria.add(Restrictions.eq("routine", routine));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"config\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		configDao.findByPage(dg1415338444009Page, criteria);
		configService.findByNormalDataGridRoutine(dg1415338444009Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1460684019181Page(Page<MESBasicSwitcherModel> dg1460684019181Page,MESBasicRoutine routine, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(MESBasicSwitcherModel.class);
		criteria.add(Restrictions.eq("config", routine));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"switcherModel\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		switcherModelDao.findByPage(dg1460684019181Page, criteria);
		switcherModelService.findByNormalDataGridRoutine(dg1460684019181Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1460684019272Page(Page<MESBasicConfig> dg1460684019272Page,MESBasicRoutine routine, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(MESBasicConfig.class);
		criteria.add(Restrictions.eq("routine", routine));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		configDao.findByPage(dg1460684019272Page, criteria);
		configService.findByNormalDataGridRoutine(dg1460684019272Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1460705667595Page(Page<MESBasicSwitcherModel> dg1460705667595Page,MESBasicRoutine routine, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(MESBasicSwitcherModel.class);
		criteria.add(Restrictions.eq("config", routine));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		switcherModelDao.findByPage(dg1460705667595Page, criteria);
		switcherModelService.findByNormalDataGridRoutine(dg1460705667595Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1460705667651Page(Page<MESBasicConfig> dg1460705667651Page,MESBasicRoutine routine, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(MESBasicConfig.class);
		criteria.add(Restrictions.eq("routine", routine));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		configDao.findByPage(dg1460705667651Page, criteria);
		configService.findByNormalDataGridRoutine(dg1460705667651Page);
	}
	
	 
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="ec.flow.save")
	public void saveRoutine(MESBasicRoutine routine, DataGridService dataGridService){
		saveRoutine(routine, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="ec.flow.save")
	public void saveRoutine(MESBasicRoutine routine, DataGridService dataGridService, String viewCode){
		saveRoutine(routine, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine")
	public void saveRoutine(MESBasicRoutine routine, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveRoutine(routine, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine")
	public void saveRoutine(MESBasicRoutine routine, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
		String url = null;
		if(null != routine.getId() && routine.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicRoutine.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine")
	public Map<Object, Object> routineDataGridImport(MESBasicRoutine routine, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = routine.getVirtualId();
		
		routine = routineDao.load(routine.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, routine.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = routine.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(routine);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, routine.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine")
	public void mergeRoutine(MESBasicRoutine routine, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(routine);
		beforeSaveRoutine(routine);
		routineDao.merge(routine);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Routine",desc="foundation.batch.save",operType="4")
	public void batchSaveRoutine(MESBasicRoutine routine, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
	...
	@Override
	public Page<MESBasicRoutine> getByPage(Page<MESBasicRoutine> page,DetachedCriteria detachedCriteria){
		return routineDao.findByPage(page, detachedCriteria);
	}
	@Override
	public List<MESBasicConfig> getConfigList(MESBasicRoutine routine){
		return configDao.findByCriteria(Restrictions.eq("routine", routine),Restrictions.eq("valid", true));
	}
	@Override
	public List<MESBasicInvolveItemsetups> getInvolveItemsetupsList(MESBasicRoutine routine){
		return involveItemsetupsDao.findByCriteria(Restrictions.eq("routine", routine),Restrictions.eq("valid", true));
	}
	@Override
	public List<MESBasicSwitcherModel> getSwitcherModelList(MESBasicRoutine routine){
		return switcherModelDao.findByCriteria(Restrictions.eq("config", routine),Restrictions.eq("valid", true));
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_routine").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveRoutine(MESBasicRoutine routine, Object... objects){
		checkUniqueConstraint(routine);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveRoutine(MESBasicRoutine routine, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
		routineDao.clear();
		routine = routineDao.load(routine.getId());
		if(routine.getRoutineStatus() == null){//新建的时候，默认的路径状态是空闲。
			routine.setRoutineStatus(new SystemCode("ROUTINE_STATUS/0"));
		}
		if(routine.getSrcContainer() != null){//源是储罐
			routine.setSrouce(routine.getSrcContainer().getCode());
		}else{
			routine.setSrouce("");
		}
		
		if(routine.getDesContainer() != null){
			routine.setDestination(routine.getDesContainer().getCode());//目的是储罐
		}else{
			routine.setDestination("");
		}
		//冗余目的设备
		MESBasicContainer container=routine.getDesContainer();
		if(container!=null){
			List<MESBasicEquipment> equips=equipmentDao.findByCriteria(Restrictions.eq("relateContainer", container),Restrictions.eq("valid",true));
			if(equips!=null && equips.size()>0){
				routine.setDestEqp(equips.get(0));
			}
		}
		//冗余目的设备
		MESBasicContainer container2=routine.getSrcContainer();
		if(container2!=null){
			List<MESBasicEquipment> equips2=equipmentDao.findByCriteria(Restrictions.eq("relateContainer", container2),Restrictions.eq("valid",true));
			if(equips2!=null && equips2.size()>0){
				routine.setSrouceEqp(equips2.get(0));
			}
		}
		routineDao.merge(routine);
		//MESBasicEquipment
		
		cache=cacheAdmin.getCache(ALL_ROUTINE);
		cache.removeAll();
/* CUSTOM CODE END */
	}
	
	@Override
	public void beforeExcelImportRoutine(List<MESBasicRoutine> routines, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportRoutine(List<MESBasicRoutine> routines){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportRoutine(List<MESBasicRoutine> routines, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportRoutine(List<MESBasicRoutine> routines){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteRoutine(MESBasicRoutine routine){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteRoutine(MESBasicRoutine routine){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
		cache=cacheAdmin.getCache(ALL_ROUTINE);
		cache.removeAll();
/* CUSTOM CODE END */
	...
	private void beforeRestoreRoutine(MESBasicRoutine routine){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreRoutine(MESBasicRoutine routine){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicRoutine routine, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicRoutine routine, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportRoutine(List<MESBasicRoutine> insertObjs, List<MESBasicRoutine> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportRoutine,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportRoutine(List<MESBasicRoutine> insertObjs, List<MESBasicRoutine> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportRoutine,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
	private static final String ALL_ROUTINE = "BATCHALLROUTINE";
	private static final String ROUTINE_NAME = "BATCHROUTINE";
	...
	public void updateRoutineStatus(){
		RoutineHelper rHelper = RoutineHelper.getRoutineHelper();
		rHelper.setRoutineConfig(configDao);
		rHelper.setRoutineDao(routineDao);
		rHelper.setSwitcherModelDao(switcherModelDao);
		rHelper.refreshRoutineStatus();
	}
	@Override
	@Transactional(readOnly=true,propagation=Propagation.SUPPORTS)
	public List<MESBasicRoutine> getRoutineByHql(String hql, Object... params){
		return routineDao.findByHql(hql, params);
	}
	
	@Override
	@Transactional(readOnly=true,propagation=Propagation.SUPPORTS)
	public MESBasicRoutine getRoutineBySignal(MESBasicItemSetup item){
		String sql = "select sw.id from "+MESBasicSwitcherModel.TABLE_NAME+" sw left join "+MESBasicRoutine.TABLE_NAME+" ro on sw.config = ro.id where sw.switcher = ? and sw.stype=? and sw.valid = 1 and ro.valid = 1";
		List<BigDecimal> list = switcherModelDao.createNativeQuery(sql, item.getId(),"MES_SWITHER/2").list();
		if(list==null || list.size()==0){
			return null;
		}else
		if( list.size()>1){
			throw new BAPException("一个信号对应2个路径，需要输出异常，测点编码为"+item.getCode()+"测点id="+item.getId());
			//一个物流路径对应了二个信号，需要输出异常
		}
		MESBasicSwitcherModel switcher=switcherModelDao.load(list.get(0).longValue());
		MESBasicRoutine routine=null;
		cache=cacheAdmin.getCache(ALL_ROUTINE);
		List<MESBasicSwitcherModel> slist = new ArrayList<MESBasicSwitcherModel>();
		slist.add(switcher);
		net.sf.ehcache.Element e = cache.get(ROUTINE_NAME+switcher.getConfig().getId());
		
		if (null != e) {
			try {
				routine=(MESBasicRoutine)e.getObjectValue();
				return routine;
			} catch (Exception e1) {
				cache.removeAll();
			}
		}else{
			routine=routineDao.load(switcher.getConfig().getId());
			List<MESBasicConfig> confinList=configDao.findByCriteria(Restrictions.eq("routine", routine),Restrictions.eq("valid",  true));
			List<MESBasicInvolveItemsetups> involveItemsetupsList=involveItemsetupsDao.findByCriteria(Restrictions.eq("routine", routine),Restrictions.eq("valid",  true));
			routine.setInvolveItemsetupsList(involveItemsetupsList);
			routine.setSwitcherModelList(slist);
			routine.setConfigList(confinList);
			cache.put(new net.sf.ehcache.Element(ROUTINE_NAME + routine.getId(), routine));
		}
	...
	@Override
	public boolean checkItem(String itemNumber){
		try {
			if(!dataSourceService.checkDBExist("BATCHMANAG_ROUTINE_SIGNALS")){
				return false;
			}
		...
		String sql = "select count(1) from BATCHMANAG_ROUTINE_SIGNALS where ITEM_NUMBER = ? and valid=1";
		return (int)routineDao.createNativeQuery(sql, itemNumber).uniqueResult() == 0;
	}
  
  	@Override
	public void saveSwitch(MESBasicSwitcherModel sm) {
		String sql = "select * from " + MESBasicSwitcherModel.TABLE_NAME + " where CONFIG = ? and SWITCHER = ? and valid = 1";
  		List<MESBasicSwitcherModel> sms = switcherModelDao.createNativeQuery(sql, sm.getConfig().getId(), sm.getConfig().getSwitcher().getId()).list();
		if(!(sms!=null && sms.size()>0)){//唯一性验证
			String updateSql = "update " + MESBasicSwitcherModel.TABLE_NAME + " set valid = 0 where CONFIG = ?";
			switcherModelDao.createNativeQuery(updateSql, sm.getConfig().getId()).executeUpdate();
			switcherModelDao.save(sm);
		}
	}

	@Override
	public List<Map<String, Object>> getTagsInfo(String code){
	    String hql = "from " + MESBasicPropertyTags.JPA_NAME + " where valid = true and container.code = ?";
        List<MESBasicPropertyTags> propertyTags = propertyTagsDao.findByHql(hql, code);
        if (propertyTags.isEmpty()){
            return null;
        }
        List<Map<String, Object>> propertyTagList = new ArrayList<>();
        for (MESBasicPropertyTags propertyTag : propertyTags) {
            MESBasicItemSetup itemSetup = propertyTag.getProperty();
            Map<String, Object> map = new HashMap<>();
            // 测点ID
            map.put("id", itemSetup.getId());
            // 测点编码
            map.put("code", itemSetup.getCode());
            // 测点名称
            map.put("itemName", itemSetup.getItemName());
            // 生产位号
            map.put("itemNumber", itemSetup.getItemNumber());
            // 位号属性
            map.put("itemClass", itemSetup.getItemClass() != null ? itemSetup.getItemClass().getId() : null);
            // 类型
            map.put("tagType", itemSetup.getTagType() != null ? itemSetup.getTagType().getId() : null);
            // 主计量
            map.put("primary", propertyTag.getPrimary());
            propertyTagList.add(map);
        }
        return propertyTagList;
    }

    @Override
    public Long getRoutineHeadId(String routineCode) {
	    // 根据路径编码查询
        String hql = "from " + MESBasicRoutine.JPA_NAME + " where code = ? and valid = true";
        MESBasicRoutine routine = routineDao.findEntityByHql(hql, routineCode);
        if (routine == null){
            return null;
        }
