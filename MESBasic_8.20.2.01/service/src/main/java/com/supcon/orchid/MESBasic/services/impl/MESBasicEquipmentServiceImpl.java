...
import com.supcon.orchid.MESBasic.entities.MESBasicEquipmentMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicEquipmentService;
import com.supcon.orchid.MESBasic.services.MESBasicEquipmentImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicEquipmentDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_equipmentService")
@Transactional
public class MESBasicEquipmentServiceImpl  extends BaseServiceImpl<MESBasicEquipment> implements MESBasicEquipmentService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicEquipment> bapBaseService;
	...
	@Autowired
	private MESBasicEquipmentImportService equipmentImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicEquipment.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  equipmentDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicEquipment.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = equipmentDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicEquipment.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicEquipment.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  equipmentDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicEquipment.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicEquipment.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicEquipment> list = (List<MESBasicEquipment>) equipmentDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicEquipment getEquipment(long id, String viewCode){
		MESBasicEquipment equipment = equipmentDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getEquipmentAsJSON(long id, String include){
		MESBasicEquipment equipment = equipmentDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(equipment, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.common.delete",operType="3")
	public void deleteEquipment(MESBasicEquipment equipment){
		if(null != equipment && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEquipment.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == equipment.getCode() ? "" : equipment.getCode().toString());
			AuditUtil.setColumnStringA(null == equipment.getName() ? "" : equipment.getName().toString());
			AuditUtil.setColumnStringC(equipment.getId().toString());
		}
		beforeDeleteEquipment(equipment);
		equipmentDao.delete(equipment);
		afterDeleteEquipment(equipment);
		equipmentDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/equipment/del", equipment,MESBasicEquipmentMneCode.class);
		List<MESBasicEquipment> params = new ArrayList<MESBasicEquipment>();
		params.add(equipment);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_equipManage");
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.common.delete",operType="3")
	public void deleteEquipment(List<Long> equipmentIds){
		deleteEquipment(equipmentIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.common.delete",operType="3")
	public void deleteEquipment(List<Long> equipmentIds, String eventTopic) {
		List<MESBasicEquipment> equipments = new ArrayList<MESBasicEquipment>();
		for(Long equipmentId : equipmentIds){
			MESBasicEquipment equipment = getEquipment(equipmentId);
			equipments.add(equipment);
			if(null != equipment && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEquipment.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == equipment.getName() ? "" : "," + equipment.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == equipment.getName() ? "" : equipment.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == equipment.getCode() ? "" : "," + equipment.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == equipment.getCode() ? "" : equipment.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == equipment.getId() ? "" : "," + equipment.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == equipment.getId() ? "" : equipment.getId().toString());
				}
			}
		}
		...
		if(equipments != null && equipments.size() > 0 ){
			for(MESBasicEquipment equipment : equipments){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),equipment, equipment.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(equipmentIds != null && equipmentIds.size() > 0) {
			String hql = "update " + MESBasicEquipment.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = equipmentDao.createQuery(hql);
			query.setParameterList("ids", equipmentIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicEquipment equipmentz : equipments){
				equipmentz.setValid(false);
				List<MESBasicEquipment> params = new ArrayList<MESBasicEquipment>();
				params.add(equipmentz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_equipManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.common.delete",operType="3")
	public void deleteEquipment(long equipmentId, int equipmentVersion){
		this.deleteEquipment(equipmentId, equipmentVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.common.delete",operType="3")
	public void deleteEquipment(long equipmentId, int equipmentVersion,SignatureLog signatureLog){
		MESBasicEquipment equipment = getEquipment(equipmentId);
		...
		beforeDeleteEquipment(equipment);
		equipmentDao.delete(equipmentId, equipmentVersion);
		afterDeleteEquipment(equipment);
		equipmentDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/equipment/del", equipment,MESBasicEquipmentMneCode.class);
		List<MESBasicEquipment> params = new ArrayList<MESBasicEquipment>();
		...
		if(signatureLog != null ) {
			...
			businessKey=equipment.getCode();
			signatureLog.setTableId(equipment.getId());
			if(businessKey != null) {
	...
	public void deleteEquipment(String equipmentIds){
		this.deleteEquipment(equipmentIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.common.delete",operType="3")
	public void deleteEquipment(String equipmentIds,SignatureLog signatureLog){
		deleteCollection(equipmentIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteEquipment(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_equipManage#modelCode:MESBasic_1_equipManage_Equipment#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="ec.engine.restore",operType="10")
	public void restoreEquipment(String equipmentIds){
		restoreCollection(equipmentIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreEquipment(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="ec.engine.restore",operType="10")
	public void restoreEquipment(long equipmentId){
		
		findBusinessKeyUsed(equipmentId);	//判断业务主键是否重复
		...
		beforeRestoreEquipment(equipment);
		equipment.setValid(true);
		equipmentDao.update(equipment);
		afterRestoreEquipment(equipment);
		equipmentDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/equipment/add", equipment,MESBasicEquipmentMneCode.class);
		List<MESBasicEquipment> params = new ArrayList<MESBasicEquipment>();
		params.add(equipment);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long equipmentId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_equipManage_Equipment");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicEquipment.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicEquipment.TABLE_NAME +" where id = ? )";
			List<Object> list =  equipmentDao.createNativeQuery(sql,equipmentId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseEquipment(List<MESBasicEquipment>  equipments){
		for(MESBasicEquipment equipment:equipments)  {
			saveEquipment(equipment, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseEquipment(List<MESBasicEquipment>  equipments){
		for(MESBasicEquipment equipment:equipments)  {
			saveEquipment(equipment, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="ec.flow.save")
	public void saveEquipment(MESBasicEquipment equipment, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveEquipment(equipment, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="ec.flow.save")
	public void saveEquipment(MESBasicEquipment equipment, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveEquipment(equipment, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateEquipmentCodes(MESBasicEquipment equipment) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateEquipmentCodes(equipment, false);
	}
	
	public void generateEquipmentCodes(MESBasicEquipment equipment, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateEquipmentSummarys(MESBasicEquipment equipment) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateEquipmentSummarys(equipment, false);
	}

	public void generateEquipmentSummarys(MESBasicEquipment equipment, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment")
	public void saveEquipment(MESBasicEquipment equipment,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveEquipment(equipment, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment")
	public void saveEquipment(MESBasicEquipment equipment,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_equipManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_equipManage");
		String url = null;
		if(equipment.getId() != null && equipment.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEquipment.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_equipManage/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEquipment.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_equipManage/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, equipment);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(equipment);
		beforeSaveEquipment(equipment, viewIsView);

		if (viewIsView) {
			equipmentDao.saveWithRevertVersion(equipment);
		} else {
			if(isNew)
		...
		dealDatagridsSave(equipment,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateEquipmentCodes(equipment, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateEquipmentSummarys(equipment, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveEquipment(equipment, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, equipment);
			}
		}
		List<MESBasicEquipment> params = new ArrayList<MESBasicEquipment>();
		equipmentDao.flush();
		equipmentDao.clear();
		equipment = equipmentDao.load(equipment.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicEquipment equipment){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("equipment", equipment);
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment")
	public void mergeEquipment(MESBasicEquipment equipment, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(equipment);
		beforeSaveEquipment(equipment);
		equipmentDao.merge(equipment);
		if(null != equipment && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEquipment.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == equipment.getCode() ? "" : equipment.getCode().toString());
			AuditUtil.setColumnStringA(null == equipment.getName() ? "" : equipment.getName().toString());
		...
		dealDatagridsSave(equipment,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveEquipment(equipment);
		equipmentDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/equipment/add", equipment,MESBasicEquipmentMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicEquipment> findEquipments(Page<MESBasicEquipment> page, Criterion... criterions) {
		return equipmentDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicEquipmentDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicEquipmentDealInfo> list=equipmentDao.createQuery("from " + MESBasicEquipmentDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicEquipmentDealInfo dealInfo) {
		equipmentDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicEquipmentDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) equipmentDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicEquipmentDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = equipmentDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = equipmentDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicEquipmentDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = equipmentDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findEquipments(Page<MESBasicEquipment> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findEquipments(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findEquipments(Page<MESBasicEquipment> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findEquipments(Page<MESBasicEquipment> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicEquipment> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"equipment\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"equipment\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"equipment\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_equipManage_Equipment" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_equipManage_Equipment" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  equipmentDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  equipmentDao.createNativeQuery(powerOperateSql, "MESBasic_1_equipManage").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"equipment\"","MESBasic_1_equipManage",customCondition,"EAM_BASEINFO",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"equipment\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicEquipmentDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"equipment\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"equipment\".LAY_REC ASC, \"equipment\".SORT ASC");
				} else {
					orderPart.append(" \"equipment\".EAM_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) equipmentDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=equipmentDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = equipmentDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicEquipment> result = new ArrayList<MESBasicEquipment>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicEquipment.class,equipmentDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicEquipment.class,equipmentDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicEquipment equipment : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(equipment.getId(), "MESBasic_equipManage_equipment");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(equipment.getId(), "MESBasic_equipManage_equipment");
							if(null != documents && !documents.isEmpty()) {
								equipment.setDocument(documents.get(0));
								equipment.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_equipManage")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_equipManage")+"MESBasic_1_equipManage".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_equipManage",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicEquipment> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicEquipment> result) {
		View view = null;
		...
		Map<Long, MESBasicEquipment> rsMap = new HashMap<Long, MESBasicEquipment>();
		for (MESBasicEquipment equipment : result) {
			ids.add(equipment.getId());
			rsMap.put(equipment.getId(), equipment);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(equipmentDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = equipmentDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicEquipment equipment,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(equipment.getOwnerStaffId());
		ti.setOwnerPositionId(equipment.getOwnerPositionId());
		ti.setOwnerDepartmentId(equipment.getOwnerDepartmentId());
		
		ti.setCreateStaff(equipment.getCreateStaff());
		ti.setCreateStaffId(equipment.getCreateStaffId());
		ti.setCreateTime(equipment.getCreateTime());
		ti.setDeleteStaff(equipment.getDeleteStaff());
		ti.setDeleteStaffId(equipment.getDeleteStaffId());
		ti.setDeleteTime(equipment.getDeleteTime());
		ti.setCreateDepartmentId(equipment.getCreateDepartmentId());
		ti.setModifyStaff(equipment.getModifyStaff());
		ti.setModifyStaffId(equipment.getModifyStaffId());
		ti.setCreatePositionId(equipment.getCreatePositionId());
		ti.setPositionLayRec(equipment.getPositionLayRec());
		ti.setTableNo(equipment.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_equipManage");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_equipManage_Equipment" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  equipmentDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"equipment\"","MESBasic_1_equipManage","","EAM_BASEINFO",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"equipment\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"equipment\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"equipment\"","MESBasic_1_equipManage","","EAM_BASEINFO",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("equipment.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"equipment\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"equipment\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"equipment\".CID = 1 OR \"equipment\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("equipment.mc\"").append(".EQUIPMENT) not in ("+notinValue+")");
			}
			
			SQLQuery query = equipmentDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("EQP", "MESBasic_1_equipManage", "EQP_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=equipmentDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicEquipment.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicEquipment.MODULE_CODE;}
	protected String getModelCode(){return MESBasicEquipment.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param equipment 模型实例
	 */
	public void checkUniqueConstraint(MESBasicEquipment equipment){
		if(null == equipment.getCode() || StringUtils.isEmpty(equipment.getCode().toString())){
			return;
		}
		if(equipment.getId()==null ){
			List<MESBasicEquipment> equipments = equipmentDao.findByCriteria(Restrictions.eq("code", equipment.getCode()));
			if(equipments!=null && !equipments.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1423967256308", getCurrentUser().getLanguage()), equipment.getCode());
			}
		} else {
			List<MESBasicEquipment> equipments = equipmentDao.findByCriteria(Restrictions.ne("id", equipment.getId()), Restrictions.eq("code", equipment.getCode()));
			if(equipments!=null && !equipments.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1423967256308", getCurrentUser().getLanguage()), equipment.getCode());
			}
	...
	@Override
	public List<MESBasicEquipment> getEquipments(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicEquipment.class);
		if(params!=null){
			...
			return equipmentDao.findByHql("from " + MESBasicEquipment.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return equipmentDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicEquipment> findEquipmentsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicEquipment.class);
	...
	@Override
	public List<MESBasicEquipment> findEquipmentsByHql(String hql, Object... objects){
		
		return   equipmentDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicEquipment loadEquipmentByBussinessKey(MESBasicEquipment equipment){
		return loadEquipmentByBussinessKey(equipment.getCode());
	}
	
	@Override
	public Page<MESBasicEquipment> getEquipments(Page<MESBasicEquipment> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicEquipment.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = equipmentDao.findByPage(page, sql);
			}else{
				page = equipmentDao.findAllByPage(page);
			}
			return page;
		}
		page = equipmentDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicEquipment loadEquipmentByBussinessKey(Serializable bussinessKey){
		return equipmentDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicEquipment> findByProperty(String propertyName, Object object){
		return equipmentDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicEquipment findEntityByProperty(String propertyName, Object object){
		return equipmentDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicEquipment.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = equipmentDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_equipManage_Equipment","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_equipManage", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicEquipment equipment,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,equipmentDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicEquipment.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, equipment);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",equipment.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicEquipment.TABLE_NAME +" where valid = 1";
			list = equipmentDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicEquipment.TABLE_NAME;
			list = equipmentDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicEquipment.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = equipmentDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicEquipmentDaoImpl equipmentDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchEquipment(final List<MESBasicEquipment> insertObjs, final List<MESBasicEquipment> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchEquipment(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchEquipment(final List<MESBasicEquipment> insertObjs, final List<MESBasicEquipment> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1423967194871"),importCount));
		...
		Map<Long, MESBasicEquipment> oldUpdateObjsMap = new HashMap<Long, MESBasicEquipment>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicEquipment bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicEquipment> bizObjList = equipmentDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicEquipment bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicEquipment> nmeObjs = new ArrayList<MESBasicEquipment>();
		...
		long id=autoGeneratorID.getNextId(MESBasicEquipment.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicEquipment equipment:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicEquipment.TABLE_NAME,1,dbId);
			if(equipment.getCode() != null && !equipment.getCode().equals("") ){
				m.put(equipment.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) equipmentDao.createNativeQuery(sql, "MESBasic_1_equipManage_Equipment"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		equipmentDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicEquipment.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicEquipment equipment = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = equipment.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicEquipment.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicEquipment.TABLE_NAME + " order by id desc";
//					Query query = equipmentDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicEquipment equipment = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = equipment.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicEquipmentMneCode.class;
					...
					for(MESBasicEquipment equipment:updateObjs)  {
						Long id = equipment.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicEquipment equipment:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(equipment);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, equipment.getId());
		...
		equipmentDaoImpl.getSession().flush();
		equipmentDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicEquipment bizObj = insertObjs.get(insertIndex);
			equipmentImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicEquipment bizObj = updateObjs.get(updateIndex);
			MESBasicEquipment oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			equipmentImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = equipmentDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				equipmentDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = equipmentDao.createNativeQuery(sql, "MESBasic_1_equipManage_Equipment").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = equipmentDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = equipmentDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = equipmentDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?equipmentDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  equipmentDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,equipmentDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="ec.flow.save")
	public void saveEquipment(MESBasicEquipment equipment, DataGridService dataGridService){
		saveEquipment(equipment, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="ec.flow.save")
	public void saveEquipment(MESBasicEquipment equipment, DataGridService dataGridService, String viewCode){
		saveEquipment(equipment, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment")
	public void saveEquipment(MESBasicEquipment equipment, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveEquipment(equipment, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment")
	public void saveEquipment(MESBasicEquipment equipment, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_equipManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_equipManage");
		String url = null;
		if(null != equipment.getId() && equipment.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEquipment.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment")
	public Map<Object, Object> equipmentDataGridImport(MESBasicEquipment equipment, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = equipment.getVirtualId();
		
		equipment = equipmentDao.load(equipment.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, equipment.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = equipment.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(equipment);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, equipment.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment")
	public void mergeEquipment(MESBasicEquipment equipment, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(equipment);
		beforeSaveEquipment(equipment);
		equipmentDao.merge(equipment);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_equipManage",model="MESBasic_1_equipManage_Equipment",desc="foundation.batch.save",operType="4")
	public void batchSaveEquipment(MESBasicEquipment equipment, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_equipManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_equipManage");
	...
	@Override
	public Page<MESBasicEquipment> getByPage(Page<MESBasicEquipment> page,DetachedCriteria detachedCriteria){
		return equipmentDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_equipManage").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveEquipment(MESBasicEquipment equipment, Object... objects){
		checkUniqueConstraint(equipment);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
        if(equipment.getIsRelateContainer() && equipment.getAutoCreateContainer() && equipment.getId() == null){//选择自动生成相应容器
			String code = equipment.getCode();
			MESBasicContainer container = containerService.getContainer(Restrictions.eq("code", code));
			if(container != null){
				throw new BAPException("已存在与当前设备相同编码的设备，无法自动创建， 请手工关联。" );
			}else{
				container = new MESBasicContainer();
				container.setCode(code);
				container.setName(equipment.getName());
				container.setContainerStatus(new SystemCode("CONTAINER_STATUS/0"));
				container.setContainerType(new SystemCode("CONTAINER_TYPE/0"));
				containerService.save(container);
				equipment.setRelateContainer(container);
			}
		}else if(equipment.getIsRelateContainer() && !equipment.getAutoCreateContainer() && equipment.getRelateContainer() == null && equipment.getId() == null){//选择了是容器，但是既没有选择自动创建容器，也没有手工关联到容器
			throw new BAPException("请选择自动创建容器，或者手工选择一个容器。");
		}else if(equipment.getIsRelateContainer() && equipment.getRelateContainer() == null){
			throw new BAPException("请选择一个关联的容器。");
		}
        //默认设备状态为在用，建档标记默认为已建档（已经有值不允许修改）
  		if("".equals(equipment.getEamState()) || equipment.getEamState() == null){
  			equipment.setEamState("01");//在用
  		}
  		if("".equals(equipment.getFileState()) || equipment.getFileState() == null){
  			equipment.setFileState("BEAM038/01");//已建档
            equipment.setFileDate(new Date());//建档日期
  		}
/* CUSTOM CODE END */
	}
	
	private void afterSaveEquipment(MESBasicEquipment equipment, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
	if (null != equipment.getWorkcenter()) {// 设备关联工厂模型
			//查询工厂模型不等于当前关联的模型并且设备为当前设备的辅模型数据 并删除
			List<MESBasicObjEquipment> objequs = objEquipmentDao.findByCriteria(Restrictions.eq("valid", true),Restrictions.eq("objEqu", equipment));
			boolean hasLinked = false;
			if(null != objequs){
				for(MESBasicObjEquipment objEquipment : objequs){
					MESBasicFactoryModel factoryModel = objEquipment.getObjFactory();
					if(null != factoryModel && factoryModel.getId().equals(equipment.getWorkcenter().getId())){//工厂模型已关联该设备
						hasLinked = true;
					} else {
			...
			if(!hasLinked){
				//将设备关联到工厂模型  新建辅模型数据
				MESBasicObjEquipment objequ = new MESBasicObjEquipment();
				objequ.setObjEqu(equipment);
				objequ.setEqus(equipment.getId().toString());
				objequ.setObjFactory(equipment.getWorkcenter());
				objEquipmentDao.save(objequ);
			}
		} else {
			//查询为当前设备的辅模型数据 并删除
			List<MESBasicObjEquipment> objequs = objEquipmentDao.findByCriteria(Restrictions.eq("valid", true),Restrictions.eq("objEqu", equipment));
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportEquipment(List<MESBasicEquipment> equipments, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportEquipment(List<MESBasicEquipment> equipments){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportEquipment(List<MESBasicEquipment> equipments, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportEquipment(List<MESBasicEquipment> equipments){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteEquipment(MESBasicEquipment equipment){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
			
		//已被工厂模型引有的设备不允许删除
		List<MESBasicObjEquipment> basicObjEquipments = objEquipmentDao.findByCriteria(Restrictions.eq("valid", true), Restrictions.eq("objEqu", equipment));
       if(null !=basicObjEquipments && !basicObjEquipments.isEmpty()){
    	   ...
    	   for (MESBasicObjEquipment basicObjEquipment : basicObjEquipments){
    		   if (null !=basicObjEquipment){
					List<MESBasicFactoryModel> basicFactoryModels = factoryModelDao.findByCriteria(Restrictions.eq("valid", true),
							Restrictions.eq("id", basicObjEquipment.getObjFactory().getId()));
                    for (MESBasicFactoryModel basicFactoryModel : basicFactoryModels){
                    	
                    	if (null !=basicFactoryModel){
                    		throw new BAPException("工厂模型已引用选中的设备，请先删除工厂模型中关联设备，再删除该设备！","");
                    	}
...
/* CUSTOM CODE END */
	...
	private void afterDeleteEquipment(MESBasicEquipment equipment){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
		if(null !=equipment.getWorkcenter()){
			//查询工厂模型不等于当前关联的模型并且设备为当前设备的辅模型数据 并删除
			List<MESBasicObjEquipment> objequs = objEquipmentDao.findByCriteria(Restrictions.eq("valid", true),Restrictions.eq("objEqu", equipment));
			if(null !=objequs){
				for (MESBasicObjEquipment objEquipment : objequs){
					MESBasicFactoryModel factoryModel = objEquipment.getObjFactory();
					if (null != factoryModel && factoryModel.getId().equals(equipment.getWorkcenter().getId())){
						objEquipmentDao.deletePhysical(objEquipment);
					}
				}
			}
...
/* CUSTOM CODE END */
	...
	private void beforeRestoreEquipment(MESBasicEquipment equipment){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreEquipment(MESBasicEquipment equipment){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicEquipment equipment, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicEquipment equipment, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportEquipment(List<MESBasicEquipment> insertObjs, List<MESBasicEquipment> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportEquipment,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
		for (MESBasicEquipment insertObj:insertObjs) {
			//默认设备状态为在用，建档标记默认为已建档（已经有值不允许修改）
			if(insertObj.getEamState() == null || "".equals(insertObj.getEamState())){
				insertObj.setEamState("01");//在用
			}
			if(insertObj.getFileState() == null || "".equals(insertObj.getFileState())){
				insertObj.setFileState("BEAM038/01");//已建档
				insertObj.setFileDate(new Date());//建档日期
			}
		}
		for (MESBasicEquipment updateObj:updateObjs) {
			//默认设备状态为在用，建档标记默认为已建档（已经有值不允许修改）
			if(updateObj.getEamState() == null || "".equals(updateObj.getEamState())){
				updateObj.setEamState("01");//在用
			}
			if(updateObj.getFileState() == null || "".equals(updateObj.getFileState())){
				updateObj.setFileState("BEAM038/01");//已建档
				updateObj.setFileDate(new Date());//建档日期
			}
		}
/* CUSTOM CODE END */
	}
	@Override
	public void afterImportEquipment(List<MESBasicEquipment> insertObjs, List<MESBasicEquipment> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportEquipment,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
