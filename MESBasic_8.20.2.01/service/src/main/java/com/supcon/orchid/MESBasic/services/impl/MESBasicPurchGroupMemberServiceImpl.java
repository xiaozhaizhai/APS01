...
import com.supcon.orchid.MESBasic.services.MESBasicPurchGroupMemberImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicPurchGroupMemberDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_purchGroupMemberService")
@Transactional
public class MESBasicPurchGroupMemberServiceImpl  extends BaseServiceImpl<MESBasicPurchGroupMember> implements MESBasicPurchGroupMemberService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicPurchGroupMember> bapBaseService;
	...
	@Autowired
	private MESBasicPurchGroupMemberImportService purchGroupMemberImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicPurchGroupMember.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  purchGroupMemberDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicPurchGroupMember.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = purchGroupMemberDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPurchGroupMember.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPurchGroupMember.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  purchGroupMemberDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicPurchGroupMember.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicPurchGroupMember.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicPurchGroupMember> list = (List<MESBasicPurchGroupMember>) purchGroupMemberDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPurchGroupMember getPurchGroupMember(long id){
		return getPurchGroupMember(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPurchGroupMember getPurchGroupMember(long id, String viewCode){
		MESBasicPurchGroupMember purchGroupMember = purchGroupMemberDao.load(id);

		// 一对多情况处理
		if(purchGroupMember != null){
		}
		return purchGroupMember;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPurchGroupMemberAsJSON(long id, String include){
		MESBasicPurchGroupMember purchGroupMember = purchGroupMemberDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(purchGroupMember, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.common.delete",operType="3")
	public void deletePurchGroupMember(MESBasicPurchGroupMember purchGroupMember){
		if(null != purchGroupMember && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroupMember.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
			AuditUtil.setColumnStringA(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
			AuditUtil.setColumnStringC(purchGroupMember.getId().toString());
		}
		beforeDeletePurchGroupMember(purchGroupMember);
		purchGroupMemberDao.deletePhysical(purchGroupMember);
		afterDeletePurchGroupMember(purchGroupMember);
		purchGroupMemberDao.flush();
		List<MESBasicPurchGroupMember> params = new ArrayList<MESBasicPurchGroupMember>();
		params.add(purchGroupMember);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_purch_Group");
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.common.delete",operType="3")
	public void deletePurchGroupMember(List<Long> purchGroupMemberIds){
		deletePurchGroupMember(purchGroupMemberIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.common.delete",operType="3")
	public void deletePurchGroupMember(List<Long> purchGroupMemberIds, String eventTopic) {
		List<MESBasicPurchGroupMember> purchGroupMembers = new ArrayList<MESBasicPurchGroupMember>();
		for(Long purchGroupMemberId : purchGroupMemberIds){
			MESBasicPurchGroupMember purchGroupMember = getPurchGroupMember(purchGroupMemberId);
			purchGroupMembers.add(purchGroupMember);
			if(null != purchGroupMember && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroupMember.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == purchGroupMember.getId() ? "" : "," + purchGroupMember.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == purchGroupMember.getId() ? "" : "," + purchGroupMember.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == purchGroupMember.getId() ? "" : "," + purchGroupMember.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
				}
			}
		}
		
		for(MESBasicPurchGroupMember purchGroupMember : purchGroupMembers){
			beforeDeletePurchGroupMember(purchGroupMember);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(purchGroupMembers != null && purchGroupMembers.size() > 0 ){
			for(MESBasicPurchGroupMember purchGroupMember : purchGroupMembers){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicPurchGroupMember.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(purchGroupMember));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),purchGroupMember, purchGroupMember.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(purchGroupMemberIds != null && purchGroupMemberIds.size() > 0) {
			String hql = "delete " + MESBasicPurchGroupMember.JPA_NAME + " where id in(:ids)";
			Query query = purchGroupMemberDao.createQuery(hql);
			query.setParameterList("ids", purchGroupMemberIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicPurchGroupMember purchGroupMemberz : purchGroupMembers){
				purchGroupMemberz.setValid(false);
				List<MESBasicPurchGroupMember> params = new ArrayList<MESBasicPurchGroupMember>();
				params.add(purchGroupMemberz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_purch_Group");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.common.delete",operType="3")
	public void deletePurchGroupMember(long purchGroupMemberId, int purchGroupMemberVersion){
		this.deletePurchGroupMember(purchGroupMemberId, purchGroupMemberVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.common.delete",operType="3")
	public void deletePurchGroupMember(long purchGroupMemberId, int purchGroupMemberVersion,SignatureLog signatureLog){
		MESBasicPurchGroupMember purchGroupMember = getPurchGroupMember(purchGroupMemberId);
		...
		beforeDeletePurchGroupMember(purchGroupMember);
		purchGroupMemberDao.delete(purchGroupMemberId, purchGroupMemberVersion);
		afterDeletePurchGroupMember(purchGroupMember);
		purchGroupMemberDao.flush();
		List<MESBasicPurchGroupMember> params = new ArrayList<MESBasicPurchGroupMember>();
		params.add(purchGroupMember);
		...
		if(signatureLog != null ) {
			...
			businessKey=purchGroupMember.getId();
			signatureLog.setTableId(purchGroupMember.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.common.delete",operType="3")
	public void deletePurchGroupMember(String purchGroupMemberIds){
		this.deletePurchGroupMember(purchGroupMemberIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.common.delete",operType="3")
	public void deletePurchGroupMember(String purchGroupMemberIds,SignatureLog signatureLog){
		deleteCollection(purchGroupMemberIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePurchGroupMember(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_purch_Group#modelCode:MESBasic_1_purch_Group_PurchGroupMember#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="ec.engine.restore",operType="10")
	public void restorePurchGroupMember(String purchGroupMemberIds){
		restoreCollection(purchGroupMemberIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePurchGroupMember(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="ec.engine.restore",operType="10")
	public void restorePurchGroupMember(long purchGroupMemberId){
		
		findBusinessKeyUsed(purchGroupMemberId);	//判断业务主键是否重复
		...
		beforeRestorePurchGroupMember(purchGroupMember);
		purchGroupMember.setValid(true);
		purchGroupMemberDao.update(purchGroupMember);
		afterRestorePurchGroupMember(purchGroupMember);
		List<MESBasicPurchGroupMember> params = new ArrayList<MESBasicPurchGroupMember>();
		params.add(purchGroupMember);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long purchGroupMemberId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_purch_Group_PurchGroupMember");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicPurchGroupMember.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicPurchGroupMember.TABLE_NAME +" where id = ? )";
			List<Object> list =  purchGroupMemberDao.createNativeQuery(sql,purchGroupMemberId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePurchGroupMember(List<MESBasicPurchGroupMember>  purchGroupMembers){
		for(MESBasicPurchGroupMember purchGroupMember:purchGroupMembers)  {
			savePurchGroupMember(purchGroupMember, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePurchGroupMember(List<MESBasicPurchGroupMember>  purchGroupMembers){
		for(MESBasicPurchGroupMember purchGroupMember:purchGroupMembers)  {
			savePurchGroupMember(purchGroupMember, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="ec.flow.save")
	public void savePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePurchGroupMember(purchGroupMember, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="ec.flow.save")
	public void savePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePurchGroupMember(purchGroupMember, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePurchGroupMemberCodes(MESBasicPurchGroupMember purchGroupMember) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePurchGroupMemberCodes(purchGroupMember, false);
	}
	
	public void generatePurchGroupMemberCodes(MESBasicPurchGroupMember purchGroupMember, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePurchGroupMemberSummarys(MESBasicPurchGroupMember purchGroupMember) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePurchGroupMemberSummarys(purchGroupMember, false);
	}

	public void generatePurchGroupMemberSummarys(MESBasicPurchGroupMember purchGroupMember, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember")
	public void savePurchGroupMember(MESBasicPurchGroupMember purchGroupMember,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePurchGroupMember(purchGroupMember, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember")
	public void savePurchGroupMember(MESBasicPurchGroupMember purchGroupMember,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_purch_Group";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_purch_Group");
		String url = null;
		if(purchGroupMember.getId() != null && purchGroupMember.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroupMember.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_purch_Group/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroupMember.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_purch_Group/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, purchGroupMember);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(purchGroupMember);
		beforeSavePurchGroupMember(purchGroupMember, viewIsView);

		if (viewIsView) {
			purchGroupMemberDao.saveWithRevertVersion(purchGroupMember);
		} else {
			if(isNew)
				purchGroupMemberDao.save(purchGroupMember);
			else
				purchGroupMemberDao.merge(purchGroupMember);
		}
		if(null != purchGroupMember && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroupMember.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
			AuditUtil.setColumnStringA(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
		...
		dealDatagridsSave(purchGroupMember,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePurchGroupMemberCodes(purchGroupMember, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePurchGroupMemberSummarys(purchGroupMember, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePurchGroupMember(purchGroupMember, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, purchGroupMember);
			}
		}
		List<MESBasicPurchGroupMember> params = new ArrayList<MESBasicPurchGroupMember>();
		purchGroupMemberDao.flush();
		purchGroupMemberDao.clear();
		purchGroupMember = purchGroupMemberDao.load(purchGroupMember.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicPurchGroupMember purchGroupMember){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("purchGroupMember", purchGroupMember);
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember")
	public void mergePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(purchGroupMember);
		beforeSavePurchGroupMember(purchGroupMember);
		purchGroupMemberDao.merge(purchGroupMember);
		if(null != purchGroupMember && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroupMember.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
			AuditUtil.setColumnStringA(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
		...
		dealDatagridsSave(purchGroupMember,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePurchGroupMember(purchGroupMember);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicPurchGroupMember> findPurchGroupMembers(Page<MESBasicPurchGroupMember> page, Criterion... criterions) {
		return purchGroupMemberDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPurchGroupMembers(Page<MESBasicPurchGroupMember> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPurchGroupMembers(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPurchGroupMembers(Page<MESBasicPurchGroupMember> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPurchGroupMembers(Page<MESBasicPurchGroupMember> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicPurchGroupMember> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"purchGroupMember\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"purchGroupMember\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"purchGroupMember\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"purchGroupMember\".MEMBER_GROUPID IN ( SELECT USERREPORT_ID FROM S2BASE_USERREPORT \"purchGroup\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "MESBasic_1_purch_Group" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  purchGroupMemberDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  purchGroupMemberDao.createNativeQuery(powerOperateSql, "MESBasic_1_purch_Group").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"purchGroup\"","MESBasic_1_purch_Group","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"purchGroupMember\".LAY_REC ASC, \"purchGroupMember\".SORT ASC");
				} else {
					orderPart.append(" \"purchGroupMember\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) purchGroupMemberDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=purchGroupMemberDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = purchGroupMemberDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicPurchGroupMember> result = new ArrayList<MESBasicPurchGroupMember>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicPurchGroupMember.class,purchGroupMemberDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicPurchGroupMember.class,purchGroupMemberDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicPurchGroupMember purchGroupMember : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(purchGroupMember.getId(), "MESBasic_purch_Group_purchGroupMember");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(purchGroupMember.getId(), "MESBasic_purch_Group_purchGroupMember");
							if(null != documents && !documents.isEmpty()) {
								purchGroupMember.setDocument(documents.get(0));
								purchGroupMember.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_purch_Group")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_purch_Group")+"MESBasic_1_purch_Group".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_purch_Group",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicPurchGroupMember> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicPurchGroupMember> result) {
		View view = null;
		...
		Map<Long, MESBasicPurchGroupMember> rsMap = new HashMap<Long, MESBasicPurchGroupMember>();
		for (MESBasicPurchGroupMember purchGroupMember : result) {
			ids.add(purchGroupMember.getId());
			rsMap.put(purchGroupMember.getId(), purchGroupMember);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(purchGroupMemberDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = purchGroupMemberDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_purch_Group_PurchGroupMember" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_purch_Group_PurchGroupMember" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  purchGroupMemberDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"purchGroupMember\"","MESBasic_1_purch_Group","","S2BASE_MEMBER",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"purchGroupMember\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"purchGroupMember\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"purchGroupMember\"","MESBasic_1_purch_Group","","S2BASE_MEMBER",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_purch_Group_PurchGroupMember" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("purchGroupMember.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"purchGroupMember\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"purchGroupMember\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"purchGroupMember\".CID = 1 OR \"purchGroupMember\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("purchGroupMember.mc\"").append(".PURCH_GROUP_MEMBER) not in ("+notinValue+")");
			}
			
			SQLQuery query = purchGroupMemberDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("purch_Group", "MESBasic_1_purch_Group", "purch_Group_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=purchGroupMemberDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicPurchGroupMember.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicPurchGroupMember.MODULE_CODE;}
	protected String getModelCode(){return MESBasicPurchGroupMember.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param purchGroupMember 模型实例
	 */
	public void checkUniqueConstraint(MESBasicPurchGroupMember purchGroupMember){
	}
	
	
	@Override
	public List<MESBasicPurchGroupMember> getPurchGroupMembers(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPurchGroupMember.class);
		if(params!=null){
			...
			return purchGroupMemberDao.findByHql("from " + MESBasicPurchGroupMember.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return purchGroupMemberDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicPurchGroupMember> findPurchGroupMembersBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPurchGroupMember.class);
	...
	@Override
	public List<MESBasicPurchGroupMember> findPurchGroupMembersByHql(String hql, Object... objects){
		
		return   purchGroupMemberDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicPurchGroupMember loadPurchGroupMemberByBussinessKey(MESBasicPurchGroupMember purchGroupMember){
		return loadPurchGroupMemberByBussinessKey(purchGroupMember.getId());
	}
	
	@Override
	public Page<MESBasicPurchGroupMember> getPurchGroupMembers(Page<MESBasicPurchGroupMember> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPurchGroupMember.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = purchGroupMemberDao.findByPage(page, sql);
			}else{
				page = purchGroupMemberDao.findAllByPage(page);
			}
			return page;
		}
		page = purchGroupMemberDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPurchGroupMember loadPurchGroupMemberByBussinessKey(Serializable bussinessKey){
		return purchGroupMemberDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicPurchGroupMember> findByProperty(String propertyName, Object object){
		return purchGroupMemberDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPurchGroupMember findEntityByProperty(String propertyName, Object object){
		return purchGroupMemberDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicPurchGroupMember.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = purchGroupMemberDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_purch_Group_PurchGroupMember","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_purch_Group", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicPurchGroupMember purchGroupMember,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,purchGroupMemberDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicPurchGroupMember.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, purchGroupMember);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",purchGroupMember.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPurchGroupMember.TABLE_NAME +" where valid = 1";
			list = purchGroupMemberDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPurchGroupMember.TABLE_NAME;
			list = purchGroupMemberDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicPurchGroupMember.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = purchGroupMemberDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicPurchGroupMemberDaoImpl purchGroupMemberDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPurchGroupMember(final List<MESBasicPurchGroupMember> insertObjs, final List<MESBasicPurchGroupMember> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPurchGroupMember(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPurchGroupMember(final List<MESBasicPurchGroupMember> insertObjs, final List<MESBasicPurchGroupMember> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1470291822089"),importCount));
		...
		Map<Long, MESBasicPurchGroupMember> oldUpdateObjsMap = new HashMap<Long, MESBasicPurchGroupMember>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicPurchGroupMember bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicPurchGroupMember> bizObjList = purchGroupMemberDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicPurchGroupMember bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicPurchGroupMember> nmeObjs = new ArrayList<MESBasicPurchGroupMember>();
		...
		long id=autoGeneratorID.getNextId(MESBasicPurchGroupMember.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicPurchGroupMember purchGroupMember:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicPurchGroupMember.TABLE_NAME,1,dbId);
			if(purchGroupMember.getId() != null ){
				m.put(purchGroupMember.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			purchGroupMember.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) purchGroupMemberDao.createNativeQuery(sql, "MESBasic_1_purch_Group_PurchGroupMember"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		purchGroupMemberDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicPurchGroupMember.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicPurchGroupMember purchGroupMember = updateObjs.get(updateIndex);
						Long id = purchGroupMember.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = purchGroupMember.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(purchGroupMember);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicPurchGroupMember.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicPurchGroupMember.TABLE_NAME + " order by id desc";
//					Query query = purchGroupMemberDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicPurchGroupMember purchGroupMember = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = purchGroupMember.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		purchGroupMemberDaoImpl.getSession().flush();
		purchGroupMemberDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicPurchGroupMember bizObj = insertObjs.get(insertIndex);
			purchGroupMemberImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicPurchGroupMember bizObj = updateObjs.get(updateIndex);
			MESBasicPurchGroupMember oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			purchGroupMemberImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicPurchGroupMember> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicPurchGroupMember> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) purchGroupMemberDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicPurchGroupMember> result = getResult(page, query, new PendingResultTransformer(MESBasicPurchGroupMember.class,purchGroupMemberDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicPurchGroupMember model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = purchGroupMemberDao.createNativeQuery(sql, "MESBasic_1_purch_Group_PurchGroupMember").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = purchGroupMemberDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = purchGroupMemberDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = purchGroupMemberDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?purchGroupMemberDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  purchGroupMemberDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,purchGroupMemberDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="ec.flow.save")
	public void savePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, DataGridService dataGridService){
		savePurchGroupMember(purchGroupMember, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="ec.flow.save")
	public void savePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, DataGridService dataGridService, String viewCode){
		savePurchGroupMember(purchGroupMember, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember")
	public void savePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePurchGroupMember(purchGroupMember, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember")
	public void savePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_purch_Group";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_purch_Group");
		String url = null;
		if(null != purchGroupMember.getId() && purchGroupMember.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroupMember.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember")
	public Map<Object, Object> purchGroupMemberDataGridImport(MESBasicPurchGroupMember purchGroupMember, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = purchGroupMember.getVirtualId();
		
		purchGroupMember = purchGroupMemberDao.load(purchGroupMember.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, purchGroupMember.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = purchGroupMember.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(purchGroupMember);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, purchGroupMember.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember")
	public void mergePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(purchGroupMember);
		beforeSavePurchGroupMember(purchGroupMember);
		purchGroupMemberDao.merge(purchGroupMember);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroupMember",desc="foundation.batch.save",operType="4")
	public void batchSavePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_purch_Group";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_purch_Group");
	...
	@Override
	public Page<MESBasicPurchGroupMember> getByPage(Page<MESBasicPurchGroupMember> page,DetachedCriteria detachedCriteria){
		return purchGroupMemberDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_purch_Group").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSavePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, Object... objects){
		checkUniqueConstraint(purchGroupMember);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePurchGroupMember(MESBasicPurchGroupMember purchGroupMember, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPurchGroupMember(List<MESBasicPurchGroupMember> purchGroupMembers, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPurchGroupMember(List<MESBasicPurchGroupMember> purchGroupMembers){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPurchGroupMember(List<MESBasicPurchGroupMember> purchGroupMembers, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPurchGroupMember(List<MESBasicPurchGroupMember> purchGroupMembers){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePurchGroupMember(MESBasicPurchGroupMember purchGroupMember){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePurchGroupMember(MESBasicPurchGroupMember purchGroupMember){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePurchGroupMember(MESBasicPurchGroupMember purchGroupMember){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePurchGroupMember(MESBasicPurchGroupMember purchGroupMember){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicPurchGroupMember purchGroupMember, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicPurchGroupMember purchGroupMember, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPurchGroupMember(List<MESBasicPurchGroupMember> insertObjs, List<MESBasicPurchGroupMember> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPurchGroupMember,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPurchGroupMember(List<MESBasicPurchGroupMember> insertObjs, List<MESBasicPurchGroupMember> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPurchGroupMember,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
