...
import com.supcon.orchid.MESBasic.services.MESBasicObjWareStoreImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicObjWareStoreDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_objWareStoreService")
@Transactional
public class MESBasicObjWareStoreServiceImpl  extends BaseServiceImpl<MESBasicObjWareStore> implements MESBasicObjWareStoreService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicObjWareStore> bapBaseService;
	...
	@Autowired
	private MESBasicObjWareStoreImportService objWareStoreImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicObjWareStore.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  objWareStoreDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicObjWareStore.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = objWareStoreDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjWareStore.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjWareStore.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  objWareStoreDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicObjWareStore.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicObjWareStore.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicObjWareStore> list = (List<MESBasicObjWareStore>) objWareStoreDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicObjWareStore getObjWareStore(long id){
		return getObjWareStore(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjWareStore getObjWareStore(long id, String viewCode){
		MESBasicObjWareStore objWareStore = objWareStoreDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getObjWareStoreAsJSON(long id, String include){
		MESBasicObjWareStore objWareStore = objWareStoreDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(objWareStore, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.common.delete",operType="3")
	public void deleteObjWareStore(MESBasicObjWareStore objWareStore){
		if(null != objWareStore && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjWareStore.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
			AuditUtil.setColumnStringA(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
			AuditUtil.setColumnStringC(objWareStore.getId().toString());
		}
		beforeDeleteObjWareStore(objWareStore);
		objWareStoreDao.deletePhysical(objWareStore);
		afterDeleteObjWareStore(objWareStore);
		objWareStoreDao.flush();
		List<MESBasicObjWareStore> params = new ArrayList<MESBasicObjWareStore>();
		params.add(objWareStore);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.common.delete",operType="3")
	public void deleteObjWareStore(List<Long> objWareStoreIds){
		deleteObjWareStore(objWareStoreIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.common.delete",operType="3")
	public void deleteObjWareStore(List<Long> objWareStoreIds, String eventTopic) {
		List<MESBasicObjWareStore> objWareStores = new ArrayList<MESBasicObjWareStore>();
		for(Long objWareStoreId : objWareStoreIds){
			MESBasicObjWareStore objWareStore = getObjWareStore(objWareStoreId);
			objWareStores.add(objWareStore);
			if(null != objWareStore && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjWareStore.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == objWareStore.getId() ? "" : "," + objWareStore.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == objWareStore.getId() ? "" : "," + objWareStore.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == objWareStore.getId() ? "" : "," + objWareStore.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(objWareStores != null && objWareStores.size() > 0 ){
			for(MESBasicObjWareStore objWareStore : objWareStores){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicObjWareStore.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(objWareStore));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),objWareStore, objWareStore.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(objWareStoreIds != null && objWareStoreIds.size() > 0) {
			String hql = "delete " + MESBasicObjWareStore.JPA_NAME + " where id in(:ids)";
			Query query = objWareStoreDao.createQuery(hql);
			query.setParameterList("ids", objWareStoreIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicObjWareStore objWareStorez : objWareStores){
				objWareStorez.setValid(false);
				List<MESBasicObjWareStore> params = new ArrayList<MESBasicObjWareStore>();
				params.add(objWareStorez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_factoryModel");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.common.delete",operType="3")
	public void deleteObjWareStore(long objWareStoreId, int objWareStoreVersion){
		this.deleteObjWareStore(objWareStoreId, objWareStoreVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.common.delete",operType="3")
	public void deleteObjWareStore(long objWareStoreId, int objWareStoreVersion,SignatureLog signatureLog){
		MESBasicObjWareStore objWareStore = getObjWareStore(objWareStoreId);
		...
		beforeDeleteObjWareStore(objWareStore);
		objWareStoreDao.delete(objWareStoreId, objWareStoreVersion);
		afterDeleteObjWareStore(objWareStore);
		objWareStoreDao.flush();
		List<MESBasicObjWareStore> params = new ArrayList<MESBasicObjWareStore>();
		params.add(objWareStore);
		...
		if(signatureLog != null ) {
			...
			businessKey=objWareStore.getId();
			signatureLog.setTableId(objWareStore.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.common.delete",operType="3")
	public void deleteObjWareStore(String objWareStoreIds){
		this.deleteObjWareStore(objWareStoreIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.common.delete",operType="3")
	public void deleteObjWareStore(String objWareStoreIds,SignatureLog signatureLog){
		deleteCollection(objWareStoreIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteObjWareStore(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_factoryModel#modelCode:MESBasic_1_factoryModel_ObjWareStore#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="ec.engine.restore",operType="10")
	public void restoreObjWareStore(String objWareStoreIds){
		restoreCollection(objWareStoreIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreObjWareStore(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="ec.engine.restore",operType="10")
	public void restoreObjWareStore(long objWareStoreId){
		
		findBusinessKeyUsed(objWareStoreId);	//判断业务主键是否重复
		...
		beforeRestoreObjWareStore(objWareStore);
		objWareStore.setValid(true);
		objWareStoreDao.update(objWareStore);
		afterRestoreObjWareStore(objWareStore);
		List<MESBasicObjWareStore> params = new ArrayList<MESBasicObjWareStore>();
		params.add(objWareStore);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long objWareStoreId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_factoryModel_ObjWareStore");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicObjWareStore.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicObjWareStore.TABLE_NAME +" where id = ? )";
			List<Object> list =  objWareStoreDao.createNativeQuery(sql,objWareStoreId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseObjWareStore(List<MESBasicObjWareStore>  objWareStores){
		for(MESBasicObjWareStore objWareStore:objWareStores)  {
			saveObjWareStore(objWareStore, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseObjWareStore(List<MESBasicObjWareStore>  objWareStores){
		for(MESBasicObjWareStore objWareStore:objWareStores)  {
			saveObjWareStore(objWareStore, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="ec.flow.save")
	public void saveObjWareStore(MESBasicObjWareStore objWareStore, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveObjWareStore(objWareStore, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="ec.flow.save")
	public void saveObjWareStore(MESBasicObjWareStore objWareStore, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveObjWareStore(objWareStore, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateObjWareStoreCodes(MESBasicObjWareStore objWareStore) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjWareStoreCodes(objWareStore, false);
	}
	
	public void generateObjWareStoreCodes(MESBasicObjWareStore objWareStore, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateObjWareStoreSummarys(MESBasicObjWareStore objWareStore) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjWareStoreSummarys(objWareStore, false);
	}

	public void generateObjWareStoreSummarys(MESBasicObjWareStore objWareStore, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore")
	public void saveObjWareStore(MESBasicObjWareStore objWareStore,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveObjWareStore(objWareStore, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore")
	public void saveObjWareStore(MESBasicObjWareStore objWareStore,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(objWareStore.getId() != null && objWareStore.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjWareStore.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, objWareStore);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(objWareStore);
		beforeSaveObjWareStore(objWareStore, viewIsView);

		if (viewIsView) {
			objWareStoreDao.saveWithRevertVersion(objWareStore);
		} else {
			if(isNew)
				objWareStoreDao.save(objWareStore);
			else
				objWareStoreDao.merge(objWareStore);
		}
		if(null != objWareStore && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjWareStore.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
			AuditUtil.setColumnStringA(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
		...
		dealDatagridsSave(objWareStore,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateObjWareStoreCodes(objWareStore, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateObjWareStoreSummarys(objWareStore, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveObjWareStore(objWareStore, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, objWareStore);
			}
		}
		List<MESBasicObjWareStore> params = new ArrayList<MESBasicObjWareStore>();
		objWareStoreDao.flush();
		objWareStoreDao.clear();
		objWareStore = objWareStoreDao.load(objWareStore.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicObjWareStore objWareStore){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("objWareStore", objWareStore);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore")
	public void mergeObjWareStore(MESBasicObjWareStore objWareStore, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(objWareStore);
		beforeSaveObjWareStore(objWareStore);
		objWareStoreDao.merge(objWareStore);
		if(null != objWareStore && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjWareStore.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
			AuditUtil.setColumnStringA(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
		...
		dealDatagridsSave(objWareStore,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveObjWareStore(objWareStore);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicObjWareStore> findObjWareStores(Page<MESBasicObjWareStore> page, Criterion... criterions) {
		return objWareStoreDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjWareStores(Page<MESBasicObjWareStore> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findObjWareStores(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjWareStores(Page<MESBasicObjWareStore> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjWareStores(Page<MESBasicObjWareStore> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicObjWareStore> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"objWareStore\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"objWareStore\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"objWareStore\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"objWareStore\".OBJ_FACTORY IN ( SELECT ID FROM factory_tbcompanystructure \"factoryModel\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  objWareStoreDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  objWareStoreDao.createNativeQuery(powerOperateSql, "MESBasic_1_factoryModel").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"objWareStore\".LAY_REC ASC, \"objWareStore\".SORT ASC");
				} else {
					orderPart.append(" \"objWareStore\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) objWareStoreDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=objWareStoreDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = objWareStoreDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicObjWareStore> result = new ArrayList<MESBasicObjWareStore>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjWareStore.class,objWareStoreDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjWareStore.class,objWareStoreDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicObjWareStore objWareStore : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(objWareStore.getId(), "MESBasic_factoryModel_objWareStore");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(objWareStore.getId(), "MESBasic_factoryModel_objWareStore");
							if(null != documents && !documents.isEmpty()) {
								objWareStore.setDocument(documents.get(0));
								objWareStore.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicObjWareStore> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicObjWareStore> result) {
		View view = null;
		...
		Map<Long, MESBasicObjWareStore> rsMap = new HashMap<Long, MESBasicObjWareStore>();
		for (MESBasicObjWareStore objWareStore : result) {
			ids.add(objWareStore.getId());
			rsMap.put(objWareStore.getId(), objWareStore);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(objWareStoreDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = objWareStoreDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_factoryModel_ObjWareStore" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_factoryModel_ObjWareStore" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  objWareStoreDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objWareStore\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_WARE_STORES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"objWareStore\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"objWareStore\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objWareStore\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_WARE_STORES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_factoryModel_ObjWareStore" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("objWareStore.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"objWareStore\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"objWareStore\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"objWareStore\".CID = 1 OR \"objWareStore\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("objWareStore.mc\"").append(".OBJ_WARE_STORE) not in ("+notinValue+")");
			}
			
			SQLQuery query = objWareStoreDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=objWareStoreDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicObjWareStore.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicObjWareStore.MODULE_CODE;}
	protected String getModelCode(){return MESBasicObjWareStore.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param objWareStore 模型实例
	 */
	public void checkUniqueConstraint(MESBasicObjWareStore objWareStore){
	}
	
	
	@Override
	public List<MESBasicObjWareStore> getObjWareStores(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjWareStore.class);
		if(params!=null){
			...
			return objWareStoreDao.findByHql("from " + MESBasicObjWareStore.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return objWareStoreDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicObjWareStore> findObjWareStoresByHql(String hql, Object... objects){
		...
		return   objWareStoreDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicObjWareStore loadObjWareStoreByBussinessKey(MESBasicObjWareStore objWareStore){
		return loadObjWareStoreByBussinessKey(objWareStore.getId());
	}
	
	@Override
	public Page<MESBasicObjWareStore> getObjWareStores(Page<MESBasicObjWareStore> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjWareStore.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = objWareStoreDao.findByPage(page, sql);
			}else{
				page = objWareStoreDao.findAllByPage(page);
			}
			return page;
		}
		page = objWareStoreDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjWareStore> findByTreeDataGridFactoryModel(List<MESBasicObjWareStore> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicObjWareStore objWareStore : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFactoryModel(Page<MESBasicObjWareStore> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicObjWareStore> objWareStores = dgPage.getResult();
			for(MESBasicObjWareStore objWareStore : objWareStores){
				
			}
			dgPage.setResult(objWareStores);
		}
	...
	public MESBasicObjWareStore loadObjWareStoreByBussinessKey(Serializable bussinessKey){
		return objWareStoreDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjWareStore> findByProperty(String propertyName, Object object){
		return objWareStoreDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjWareStore findEntityByProperty(String propertyName, Object object){
		return objWareStoreDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicObjWareStore.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = objWareStoreDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_factoryModel_ObjWareStore","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicObjWareStore objWareStore,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,objWareStoreDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicObjWareStore.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, objWareStore);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",objWareStore.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjWareStore.TABLE_NAME +" where valid = 1";
			list = objWareStoreDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjWareStore.TABLE_NAME;
			list = objWareStoreDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicObjWareStore.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = objWareStoreDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicObjWareStoreDaoImpl objWareStoreDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchObjWareStore(final List<MESBasicObjWareStore> insertObjs, final List<MESBasicObjWareStore> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchObjWareStore(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchObjWareStore(final List<MESBasicObjWareStore> insertObjs, final List<MESBasicObjWareStore> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1490058575739"),importCount));
		...
		Map<Long, MESBasicObjWareStore> oldUpdateObjsMap = new HashMap<Long, MESBasicObjWareStore>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicObjWareStore bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicObjWareStore> bizObjList = objWareStoreDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicObjWareStore bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicObjWareStore> nmeObjs = new ArrayList<MESBasicObjWareStore>();
		...
		long id=autoGeneratorID.getNextId(MESBasicObjWareStore.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicObjWareStore objWareStore:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicObjWareStore.TABLE_NAME,1,dbId);
			if(objWareStore.getId() != null ){
				m.put(objWareStore.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) objWareStoreDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjWareStore"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		objWareStoreDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicObjWareStore.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicObjWareStore objWareStore = updateObjs.get(updateIndex);
						Long id = objWareStore.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = objWareStore.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(objWareStore);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicObjWareStore.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicObjWareStore.TABLE_NAME + " order by id desc";
//					Query query = objWareStoreDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicObjWareStore objWareStore = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = objWareStore.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		objWareStoreDaoImpl.getSession().flush();
		objWareStoreDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicObjWareStore bizObj = insertObjs.get(insertIndex);
			objWareStoreImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicObjWareStore bizObj = updateObjs.get(updateIndex);
			MESBasicObjWareStore oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			objWareStoreImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicObjWareStore> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicObjWareStore> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) objWareStoreDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicObjWareStore> result = getResult(page, query, new PendingResultTransformer(MESBasicObjWareStore.class,objWareStoreDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicObjWareStore model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = objWareStoreDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjWareStore").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = objWareStoreDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = objWareStoreDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = objWareStoreDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?objWareStoreDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  objWareStoreDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,objWareStoreDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="ec.flow.save")
	public void saveObjWareStore(MESBasicObjWareStore objWareStore, DataGridService dataGridService){
		saveObjWareStore(objWareStore, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="ec.flow.save")
	public void saveObjWareStore(MESBasicObjWareStore objWareStore, DataGridService dataGridService, String viewCode){
		saveObjWareStore(objWareStore, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore")
	public void saveObjWareStore(MESBasicObjWareStore objWareStore, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveObjWareStore(objWareStore, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore")
	public void saveObjWareStore(MESBasicObjWareStore objWareStore, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(null != objWareStore.getId() && objWareStore.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjWareStore.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore")
	public Map<Object, Object> objWareStoreDataGridImport(MESBasicObjWareStore objWareStore, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = objWareStore.getVirtualId();
		
		objWareStore = objWareStoreDao.load(objWareStore.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, objWareStore.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = objWareStore.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(objWareStore);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, objWareStore.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore")
	public void mergeObjWareStore(MESBasicObjWareStore objWareStore, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(objWareStore);
		beforeSaveObjWareStore(objWareStore);
		objWareStoreDao.merge(objWareStore);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjWareStore",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicObjWareStore> getByPage(Page<MESBasicObjWareStore> page,DetachedCriteria detachedCriteria){
		return objWareStoreDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveObjWareStore(MESBasicObjWareStore objWareStore, Object... objects){
		checkUniqueConstraint(objWareStore);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveObjWareStore(MESBasicObjWareStore objWareStore, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteObjWareStore(MESBasicObjWareStore objWareStore){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteObjWareStore(MESBasicObjWareStore objWareStore){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreObjWareStore(MESBasicObjWareStore objWareStore){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreObjWareStore(MESBasicObjWareStore objWareStore){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicObjWareStore objWareStore, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicObjWareStore objWareStore, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportObjWareStore(List<MESBasicObjWareStore> insertObjs, List<MESBasicObjWareStore> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportObjWareStore,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportObjWareStore(List<MESBasicObjWareStore> insertObjs, List<MESBasicObjWareStore> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportObjWareStore,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
		@Autowired
	private MESBasicProductService ProductService;
@Override
	public String checkWareFactory(Long factoryCode, String productArr) {
      	...
      	String result=null;
		//通过工厂模型Id获取工程模型对象
		MESBasicFactoryModel model = factoryModelDao.load(factoryCode);
		//把物品编码分解为集合
		String[] productStr=productArr.split("-");
		//获取工厂模型的层次
		...
		if (null != layRec) {
			...
			for (int i = 0; i < layRecList.length - 1; i++) {
				for (int j = 0; j < productStr.length; j++) {
					...
					params.add(Long.valueOf(layRecList[i]));
					params.add(Long.valueOf(productStr[j]));
					//获取当前物品下上级节点的配料藏得集合
					List<MESBasicObjWareStore> objWareStoreList=this.getObjWareStores("this_.OBJ_FACTORY=? and this_.product=? and this_.valid=1 ",params);
					if (objWareStoreList.size() > 0) {
						MESBasicProduct product = ProductService.getProduct(Long.valueOf(productStr[j])); 
						codeSet.add(product.getProductCode());
					}
				}
			}
			//获取当前节点的下级节点的的工厂模型对象
			...
			//params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
			params.add(layRec+"_%");
			List<MESBasicFactoryModel> factoryModelList=factoryModelService.getFactoryModels("this_.lay_rec like ? and  this_.valid=1 ",params);
			
			for(int i=0;i<factoryModelList.size();i++){
				for (int j = 0; j < productStr.length; j++) {
					List<Object> newParams=new ArrayList<Object>();
					newParams.add(factoryModelList.get(i).getId());
					newParams.add(Long.valueOf(productStr[j]));
					//获取当前物品下上级节点的配料藏得集合
					List<MESBasicObjWareStore> newObjWareStore=this.getObjWareStores("this_.OBJ_FACTORY=? and this_.product=? and this_.valid=1 ",newParams);
					if (newObjWareStore.size() > 0) {
						MESBasicProduct product = ProductService.getProduct(Long.valueOf(productStr[j]));
						codeSet.add(product.getProductCode());
					}
				}
		...
		//如果当前节点的物料与上下级有相同的则返回物品编码，如果不同则返回""
