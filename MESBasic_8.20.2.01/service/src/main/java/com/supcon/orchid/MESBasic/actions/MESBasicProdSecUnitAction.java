...
/* CUSTOM CODE START(action,import,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
import org.json.JSONObject;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicProdSecUnitAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicProdSecUnit> {
	...
	private MESBasicProdSecUnit prodSecUnit;
	...
	//导出excel时,主模型内容
	List<MESBasicProdSecUnit> queryResults = new ArrayList<MESBasicProdSecUnit>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/prodSecUnit/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/prodSecUnit/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_product_ProdSecUnit" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicProdSecUnit> prodSecUnits;
	
	public List<MESBasicProdSecUnit> getProdSecUnits() {
		return prodSecUnits;
	}
	...
	private Page<MESBasicProdSecUnit> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			prodSecUnit = new MESBasicProdSecUnit();
		} else {
			if(id > 0){
				prodSecUnit = prodSecUnitService.getProdSecUnit(id, viewCode);
			}
		}
	...
	public Page<MESBasicProdSecUnit> getPage() {
		if (null == page) {
			page = new Page<MESBasicProdSecUnit>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicProdSecUnit getProdSecUnit() {
		return prodSecUnit;
	}		

	public void setProdSecUnit(MESBasicProdSecUnit prodSecUnit) {
		this.prodSecUnit = prodSecUnit;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/prodSecUnit/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prodSecUnitService.getProdSecUnitAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/prodSecUnit/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1530515212961
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String prodSecUnitReferenceCopy() throws Exception{
			MESBasicProdSecUnit oldProdSecUnit = null;
			if(isEditNew()){
				...
				oldProdSecUnit = new MESBasicProdSecUnit();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodSecUnit.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prodSecUnit.getId());
				...
				oldProdSecUnit = prodSecUnitService.getProdSecUnit(id);
			}
			
			
			prodSecUnit = prodSecUnitService.getProdSecUnit(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_product_ProdSecUnit");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicProdSecUnit.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProdSecUnit, null);
						Method setValue = MESBasicProdSecUnit.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prodSecUnit, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prodSecUnit.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_product_ProdSecUnit", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=prodSecUnitService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return prodSecUnitReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			prodSecUnit = prodSecUnitService.getProdSecUnit(this.prodSecUnit.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodSecUnit.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = prodSecUnitService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(prodSecUnitService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(prodSecUnitService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_product_ProdSecUnit", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/prodSecUnit/refProdSecUnit", results = { @Result(type = FREEMARKER, location = "refProdSecUnit.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"prodSecUnit\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"prodSecUnit\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"prodSecUnit\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"prodSecUnit\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			prodSecUnitService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/prodSecUnit/refProdSecUnit-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.product.productCode,result.product.productName,result.baseUnit.code,result.baseUnit.name,result.changeRate,result.secUnit.code,result.secUnit.name,result.product.id,result.baseUnit.id,result.secUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refProdSecUnit-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		prodSecUnitService.findDataGridPage(dg,dgclass,getDgPage(),prodSecUnit,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(prodSecUnit.getId() != null && prodSecUnit.getId() != -1){
				prodSecUnit = prodSecUnitService.getProdSecUnit(prodSecUnit.getId());
			}
			prodSecUnitService.findDataGridPage(dg,dgclass,getDgPage(),prodSecUnit,searchCondition,params);
	...
	private void save() throws Exception {
		...
		prodSecUnitService.saveProdSecUnit(prodSecUnit,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_ProdSecUnit", "submit");
		bap_validate_datagrids=prodSecUnitService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", prodSecUnit.getId());
		...
		fileuploads.put("linkId", prodSecUnit.getId());
		bapAfterCustomMethod("MESBasic_1_product_ProdSecUnit", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/product/prodSecUnit/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProdSecUnit", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prodSecUnitService.deleteProdSecUnit(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_product_ProdSecUnit", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/prodSecUnit/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProdSecUnit", "restore");
		prodSecUnitService.restoreProdSecUnit(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_product_ProdSecUnit", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicProdSecUnit> findExportDataInfos(Page<MESBasicProdSecUnit> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = prodSecUnitService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = prodSecUnitService.getAssProperty(p.getCode());	
									//String assModelCode = prodSecUnitService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			prodSecUnitService.findProdSecUnits(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			prodSecUnitService.findProdSecUnits(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicProdSecUnit> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicProdSecUnit result : results){
	...
	public boolean isEditNew(){
		//return null == this.prodSecUnit || null == this.prodSecUnit.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/product/prodSecUnit/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProdSecUnit", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_product_ProdSecUnit", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/product/prodSecUnit/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_ProdSecUnit", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/product/prodSecUnit/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/prodSecUnit/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProdSecUnitService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_product_ProdSecUnit", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_product_ProdSecUnit", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/prodSecUnit/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_product_ProdSecUnit", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_product_ProdSecUnit", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/product/prodSecUnit/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProdSecUnit", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/product/prodSecUnit/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_product_ProdSecUnit";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_product_ProdSecUnit", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/product/prodSecUnit/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicProdSecUnit prodSecUnit = null;
		if(null != id) {
			prodSecUnit = prodSecUnitService.getProdSecUnit(id);
		}
		...
		_auditLog.setColumnStringB(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
		_auditLog.setColumnStringA(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
		_auditLog.setColumnStringC(prodSecUnit.getId().toString());
		_auditLog.setColumnStringD(MESBasicProdSecUnit.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicProdSecUnitAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, prodSecUnit,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, prodSecUnit,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,prodSecUnit);
		...
		printlog.setBusinessId(prodSecUnit.getId());
		printlog.setBussinessKey(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
		printlog.setMainDisplay(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/product/prodSecUnit/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(prodSecUnit,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/prodSecUnit/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(prodSecUnitService.getProdSecUnit(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/product/prodSecUnit/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/product/prodSecUnit/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, prodSecUnit,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				prodSecUnit = prodSecUnitService.getProdSecUnit(Long.parseLong(arrId[i]));
				if(null != prodSecUnit){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == prodSecUnit.getId() ? "" : "," + prodSecUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == prodSecUnit.getId() ? "" : "," + prodSecUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == prodSecUnit.getId() ? "" : "," + prodSecUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, prodSecUnit,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,prodSecUnit);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/product/prodSecUnit/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_product_ProdSecUnit");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_product_ProdSecUnit")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_product_ProdSecUnit");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_product_ProdSecUnit");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = prodSecUnitService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
	...
	@Action(value = "/MESBasic/product/product/getGoodUnits",results = { @Result(type = JSON, params = { "root", "responseMap" })  })
	public String getGoodUnits() throws Exception {
		String[] code = goodCode.split(",");
		List<Map<String, Object>> lResult = new ArrayList<Map<String,Object>>();
		for(String s : code ){
			List<MESBasicProdSecUnit> list = prodSecUnitService.getProdUnit(s);
			Map<String, Object> map = new HashMap<String, Object>(); 
			if(list != null && list.size() > 0){
				map.put("changeRate", list.get(0).getChangeRate());
				map.put("secUnitId", list.get(0).getId());
				map.put("secUnitName", list.get(0).getSecUnit().getName());
				responseMap.put(s, new JSONObject(map).toString());
			}else{
				responseMap.put(s, new JSONObject(map).toString());
			}
		}
