...
import com.supcon.orchid.MESBasic.services.MESBasicServiceLoggerImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicServiceLoggerDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_serviceLoggerService")
@Transactional
public class MESBasicServiceLoggerServiceImpl  extends BaseServiceImpl<MESBasicServiceLogger> implements MESBasicServiceLoggerService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicServiceLogger> bapBaseService;
	...
	@Autowired
	private MESBasicServiceLoggerImportService serviceLoggerImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicServiceLogger.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  serviceLoggerDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicServiceLogger.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = serviceLoggerDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicServiceLogger.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicServiceLogger.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  serviceLoggerDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicServiceLogger.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicServiceLogger.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicServiceLogger> list = (List<MESBasicServiceLogger>) serviceLoggerDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicServiceLogger getServiceLogger(long id){
		return getServiceLogger(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicServiceLogger getServiceLogger(long id, String viewCode){
		MESBasicServiceLogger serviceLogger = serviceLoggerDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getServiceLoggerAsJSON(long id, String include){
		MESBasicServiceLogger serviceLogger = serviceLoggerDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(serviceLogger, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.common.delete",operType="3")
	public void deleteServiceLogger(MESBasicServiceLogger serviceLogger){
		if(null != serviceLogger && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicServiceLogger.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
			AuditUtil.setColumnStringA(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
			AuditUtil.setColumnStringC(serviceLogger.getId().toString());
		}
		beforeDeleteServiceLogger(serviceLogger);
		serviceLoggerDao.deletePhysical(serviceLogger);
		afterDeleteServiceLogger(serviceLogger);
		serviceLoggerDao.flush();
		List<MESBasicServiceLogger> params = new ArrayList<MESBasicServiceLogger>();
		params.add(serviceLogger);
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.common.delete",operType="3")
	public void deleteServiceLogger(List<Long> serviceLoggerIds){
		deleteServiceLogger(serviceLoggerIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.common.delete",operType="3")
	public void deleteServiceLogger(List<Long> serviceLoggerIds, String eventTopic) {
		List<MESBasicServiceLogger> serviceLoggers = new ArrayList<MESBasicServiceLogger>();
		for(Long serviceLoggerId : serviceLoggerIds){
			MESBasicServiceLogger serviceLogger = getServiceLogger(serviceLoggerId);
			serviceLoggers.add(serviceLogger);
			if(null != serviceLogger && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicServiceLogger.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == serviceLogger.getId() ? "" : "," + serviceLogger.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == serviceLogger.getId() ? "" : "," + serviceLogger.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == serviceLogger.getId() ? "" : "," + serviceLogger.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(serviceLoggers != null && serviceLoggers.size() > 0 ){
			for(MESBasicServiceLogger serviceLogger : serviceLoggers){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicServiceLogger.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(serviceLogger));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),serviceLogger, serviceLogger.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(serviceLoggerIds != null && serviceLoggerIds.size() > 0) {
			String hql = "delete " + MESBasicServiceLogger.JPA_NAME + " where id in(:ids)";
			Query query = serviceLoggerDao.createQuery(hql);
			query.setParameterList("ids", serviceLoggerIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicServiceLogger serviceLoggerz : serviceLoggers){
				serviceLoggerz.setValid(false);
				List<MESBasicServiceLogger> params = new ArrayList<MESBasicServiceLogger>();
				params.add(serviceLoggerz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.common.delete",operType="3")
	public void deleteServiceLogger(long serviceLoggerId, int serviceLoggerVersion){
		this.deleteServiceLogger(serviceLoggerId, serviceLoggerVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.common.delete",operType="3")
	public void deleteServiceLogger(long serviceLoggerId, int serviceLoggerVersion,SignatureLog signatureLog){
		MESBasicServiceLogger serviceLogger = getServiceLogger(serviceLoggerId);
		...
		beforeDeleteServiceLogger(serviceLogger);
		serviceLoggerDao.delete(serviceLoggerId, serviceLoggerVersion);
		afterDeleteServiceLogger(serviceLogger);
		serviceLoggerDao.flush();
		List<MESBasicServiceLogger> params = new ArrayList<MESBasicServiceLogger>();
		params.add(serviceLogger);
		...
		if(signatureLog != null ) {
			...
			businessKey=serviceLogger.getId();
			signatureLog.setTableId(serviceLogger.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.common.delete",operType="3")
	public void deleteServiceLogger(String serviceLoggerIds){
		this.deleteServiceLogger(serviceLoggerIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.common.delete",operType="3")
	public void deleteServiceLogger(String serviceLoggerIds,SignatureLog signatureLog){
		deleteCollection(serviceLoggerIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteServiceLogger(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_backgroundServiceMonitor#modelCode:MESBasic_1_backgroundServiceMonitor_ServiceLogger#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="ec.engine.restore",operType="10")
	public void restoreServiceLogger(String serviceLoggerIds){
		restoreCollection(serviceLoggerIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreServiceLogger(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="ec.engine.restore",operType="10")
	public void restoreServiceLogger(long serviceLoggerId){
		
		findBusinessKeyUsed(serviceLoggerId);	//判断业务主键是否重复
		...
		beforeRestoreServiceLogger(serviceLogger);
		serviceLogger.setValid(true);
		serviceLoggerDao.update(serviceLogger);
		afterRestoreServiceLogger(serviceLogger);
		List<MESBasicServiceLogger> params = new ArrayList<MESBasicServiceLogger>();
		params.add(serviceLogger);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long serviceLoggerId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_backgroundServiceMonitor_ServiceLogger");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicServiceLogger.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicServiceLogger.TABLE_NAME +" where id = ? )";
			List<Object> list =  serviceLoggerDao.createNativeQuery(sql,serviceLoggerId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseServiceLogger(List<MESBasicServiceLogger>  serviceLoggers){
		for(MESBasicServiceLogger serviceLogger:serviceLoggers)  {
			saveServiceLogger(serviceLogger, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseServiceLogger(List<MESBasicServiceLogger>  serviceLoggers){
		for(MESBasicServiceLogger serviceLogger:serviceLoggers)  {
			saveServiceLogger(serviceLogger, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="ec.flow.save")
	public void saveServiceLogger(MESBasicServiceLogger serviceLogger, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveServiceLogger(serviceLogger, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="ec.flow.save")
	public void saveServiceLogger(MESBasicServiceLogger serviceLogger, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveServiceLogger(serviceLogger, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateServiceLoggerCodes(MESBasicServiceLogger serviceLogger) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateServiceLoggerCodes(serviceLogger, false);
	}
	
	public void generateServiceLoggerCodes(MESBasicServiceLogger serviceLogger, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateServiceLoggerSummarys(MESBasicServiceLogger serviceLogger) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateServiceLoggerSummarys(serviceLogger, false);
	}

	public void generateServiceLoggerSummarys(MESBasicServiceLogger serviceLogger, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger")
	public void saveServiceLogger(MESBasicServiceLogger serviceLogger,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveServiceLogger(serviceLogger, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger")
	public void saveServiceLogger(MESBasicServiceLogger serviceLogger,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
		String url = null;
		if(serviceLogger.getId() != null && serviceLogger.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicServiceLogger.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, serviceLogger);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(serviceLogger);
		beforeSaveServiceLogger(serviceLogger, viewIsView);

		if (viewIsView) {
			serviceLoggerDao.saveWithRevertVersion(serviceLogger);
		} else {
			if(isNew)
				serviceLoggerDao.save(serviceLogger);
			else
				serviceLoggerDao.merge(serviceLogger);
		}
		if(null != serviceLogger && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicServiceLogger.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
			AuditUtil.setColumnStringA(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
		...
		dealDatagridsSave(serviceLogger,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateServiceLoggerCodes(serviceLogger, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateServiceLoggerSummarys(serviceLogger, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveServiceLogger(serviceLogger, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, serviceLogger);
			}
		}
		List<MESBasicServiceLogger> params = new ArrayList<MESBasicServiceLogger>();
		serviceLoggerDao.flush();
		serviceLoggerDao.clear();
		serviceLogger = serviceLoggerDao.load(serviceLogger.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicServiceLogger serviceLogger){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("serviceLogger", serviceLogger);
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger")
	public void mergeServiceLogger(MESBasicServiceLogger serviceLogger, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(serviceLogger);
		beforeSaveServiceLogger(serviceLogger);
		serviceLoggerDao.merge(serviceLogger);
		if(null != serviceLogger && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicServiceLogger.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
			AuditUtil.setColumnStringA(null == serviceLogger.getId() ? "" : serviceLogger.getId().toString());
		...
		dealDatagridsSave(serviceLogger,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveServiceLogger(serviceLogger);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicServiceLogger> findServiceLoggers(Page<MESBasicServiceLogger> page, Criterion... criterions) {
		return serviceLoggerDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findServiceLoggers(Page<MESBasicServiceLogger> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findServiceLoggers(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findServiceLoggers(Page<MESBasicServiceLogger> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findServiceLoggers(Page<MESBasicServiceLogger> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicServiceLogger> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"serviceLogger\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"serviceLogger\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"serviceLogger\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"serviceLogger\".MES_SERVICE IN ( SELECT ID FROM MESBASIC_MESSERVICES \"mESServices\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "MESBasic_1_backgroundServiceMonitor" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  serviceLoggerDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  serviceLoggerDao.createNativeQuery(powerOperateSql, "MESBasic_1_backgroundServiceMonitor").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mESServices\"","MESBasic_1_backgroundServiceMonitor","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"serviceLogger\".LAY_REC ASC, \"serviceLogger\".SORT ASC");
				} else {
					orderPart.append(" \"serviceLogger\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) serviceLoggerDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=serviceLoggerDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = serviceLoggerDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicServiceLogger> result = new ArrayList<MESBasicServiceLogger>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicServiceLogger.class,serviceLoggerDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicServiceLogger.class,serviceLoggerDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicServiceLogger serviceLogger : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(serviceLogger.getId(), "MESBasic_backgroundServiceMonitor_serviceLogger");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(serviceLogger.getId(), "MESBasic_backgroundServiceMonitor_serviceLogger");
							if(null != documents && !documents.isEmpty()) {
								serviceLogger.setDocument(documents.get(0));
								serviceLogger.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicServiceLogger> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicServiceLogger> result) {
		View view = null;
		...
		Map<Long, MESBasicServiceLogger> rsMap = new HashMap<Long, MESBasicServiceLogger>();
		for (MESBasicServiceLogger serviceLogger : result) {
			ids.add(serviceLogger.getId());
			rsMap.put(serviceLogger.getId(), serviceLogger);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(serviceLoggerDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = serviceLoggerDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_backgroundServiceMonitor_ServiceLogger" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_backgroundServiceMonitor_ServiceLogger" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  serviceLoggerDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"serviceLogger\"","MESBasic_1_backgroundServiceMonitor","","MESBASIC_SERVICE_LOGGERS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"serviceLogger\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"serviceLogger\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"serviceLogger\"","MESBasic_1_backgroundServiceMonitor","","MESBASIC_SERVICE_LOGGERS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_backgroundServiceMonitor_ServiceLogger" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("serviceLogger.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"serviceLogger\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"serviceLogger\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"serviceLogger\".CID = 1 OR \"serviceLogger\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("serviceLogger.mc\"").append(".SERVICE_LOGGER) not in ("+notinValue+")");
			}
			
			SQLQuery query = serviceLoggerDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=serviceLoggerDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicServiceLogger.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicServiceLogger.MODULE_CODE;}
	protected String getModelCode(){return MESBasicServiceLogger.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param serviceLogger 模型实例
	 */
	public void checkUniqueConstraint(MESBasicServiceLogger serviceLogger){
	}
	
	
	@Override
	public List<MESBasicServiceLogger> getServiceLoggers(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicServiceLogger.class);
		if(params!=null){
			...
			return serviceLoggerDao.findByHql("from " + MESBasicServiceLogger.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return serviceLoggerDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicServiceLogger> findServiceLoggersBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicServiceLogger.class);
	...
	@Override
	public List<MESBasicServiceLogger> findServiceLoggersByHql(String hql, Object... objects){
		
		return   serviceLoggerDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicServiceLogger loadServiceLoggerByBussinessKey(MESBasicServiceLogger serviceLogger){
		return loadServiceLoggerByBussinessKey(serviceLogger.getId());
	}
	
	@Override
	public Page<MESBasicServiceLogger> getServiceLoggers(Page<MESBasicServiceLogger> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicServiceLogger.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = serviceLoggerDao.findByPage(page, sql);
			}else{
				page = serviceLoggerDao.findAllByPage(page);
			}
			return page;
		}
		page = serviceLoggerDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicServiceLogger> findByTreeDataGridMESServices(List<MESBasicServiceLogger> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicServiceLogger serviceLogger : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridMESServices(Page<MESBasicServiceLogger> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicServiceLogger> serviceLoggers = dgPage.getResult();
			for(MESBasicServiceLogger serviceLogger : serviceLoggers){
				
			}
			dgPage.setResult(serviceLoggers);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicServiceLogger loadServiceLoggerByBussinessKey(Serializable bussinessKey){
		return serviceLoggerDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicServiceLogger> findByProperty(String propertyName, Object object){
		return serviceLoggerDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicServiceLogger findEntityByProperty(String propertyName, Object object){
		return serviceLoggerDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicServiceLogger.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = serviceLoggerDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_backgroundServiceMonitor_ServiceLogger","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicServiceLogger serviceLogger,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,serviceLoggerDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicServiceLogger.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, serviceLogger);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",serviceLogger.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicServiceLogger.TABLE_NAME +" where valid = 1";
			list = serviceLoggerDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicServiceLogger.TABLE_NAME;
			list = serviceLoggerDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicServiceLogger.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = serviceLoggerDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicServiceLoggerDaoImpl serviceLoggerDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchServiceLogger(final List<MESBasicServiceLogger> insertObjs, final List<MESBasicServiceLogger> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchServiceLogger(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchServiceLogger(final List<MESBasicServiceLogger> insertObjs, final List<MESBasicServiceLogger> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1429057309643"),importCount));
		...
		Map<Long, MESBasicServiceLogger> oldUpdateObjsMap = new HashMap<Long, MESBasicServiceLogger>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicServiceLogger bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicServiceLogger> bizObjList = serviceLoggerDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicServiceLogger bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicServiceLogger> nmeObjs = new ArrayList<MESBasicServiceLogger>();
		...
		long id=autoGeneratorID.getNextId(MESBasicServiceLogger.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicServiceLogger serviceLogger:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicServiceLogger.TABLE_NAME,1,dbId);
			if(serviceLogger.getId() != null ){
				m.put(serviceLogger.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) serviceLoggerDao.createNativeQuery(sql, "MESBasic_1_backgroundServiceMonitor_ServiceLogger"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		serviceLoggerDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicServiceLogger.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicServiceLogger serviceLogger = updateObjs.get(updateIndex);
						Long id = serviceLogger.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = serviceLogger.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(serviceLogger);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicServiceLogger.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicServiceLogger.TABLE_NAME + " order by id desc";
//					Query query = serviceLoggerDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicServiceLogger serviceLogger = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = serviceLogger.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		serviceLoggerDaoImpl.getSession().flush();
		serviceLoggerDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicServiceLogger bizObj = insertObjs.get(insertIndex);
			serviceLoggerImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicServiceLogger bizObj = updateObjs.get(updateIndex);
			MESBasicServiceLogger oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			serviceLoggerImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicServiceLogger> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicServiceLogger> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) serviceLoggerDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicServiceLogger> result = getResult(page, query, new PendingResultTransformer(MESBasicServiceLogger.class,serviceLoggerDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicServiceLogger model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = serviceLoggerDao.createNativeQuery(sql, "MESBasic_1_backgroundServiceMonitor_ServiceLogger").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = serviceLoggerDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = serviceLoggerDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = serviceLoggerDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?serviceLoggerDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  serviceLoggerDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,serviceLoggerDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="ec.flow.save")
	public void saveServiceLogger(MESBasicServiceLogger serviceLogger, DataGridService dataGridService){
		saveServiceLogger(serviceLogger, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="ec.flow.save")
	public void saveServiceLogger(MESBasicServiceLogger serviceLogger, DataGridService dataGridService, String viewCode){
		saveServiceLogger(serviceLogger, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger")
	public void saveServiceLogger(MESBasicServiceLogger serviceLogger, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveServiceLogger(serviceLogger, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger")
	public void saveServiceLogger(MESBasicServiceLogger serviceLogger, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_backgroundServiceMonitor";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
		String url = null;
		if(null != serviceLogger.getId() && serviceLogger.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicServiceLogger.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger")
	public Map<Object, Object> serviceLoggerDataGridImport(MESBasicServiceLogger serviceLogger, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = serviceLogger.getVirtualId();
		
		serviceLogger = serviceLoggerDao.load(serviceLogger.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, serviceLogger.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = serviceLogger.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(serviceLogger);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, serviceLogger.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger")
	public void mergeServiceLogger(MESBasicServiceLogger serviceLogger, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(serviceLogger);
		beforeSaveServiceLogger(serviceLogger);
		serviceLoggerDao.merge(serviceLogger);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_ServiceLogger",desc="foundation.batch.save",operType="4")
	public void batchSaveServiceLogger(MESBasicServiceLogger serviceLogger, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_backgroundServiceMonitor";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
	...
	@Override
	public Page<MESBasicServiceLogger> getByPage(Page<MESBasicServiceLogger> page,DetachedCriteria detachedCriteria){
		return serviceLoggerDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveServiceLogger(MESBasicServiceLogger serviceLogger, Object... objects){
		checkUniqueConstraint(serviceLogger);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveServiceLogger(MESBasicServiceLogger serviceLogger, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportServiceLogger(List<MESBasicServiceLogger> serviceLoggers, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportServiceLogger(List<MESBasicServiceLogger> serviceLoggers){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportServiceLogger(List<MESBasicServiceLogger> serviceLoggers, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportServiceLogger(List<MESBasicServiceLogger> serviceLoggers){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteServiceLogger(MESBasicServiceLogger serviceLogger){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteServiceLogger(MESBasicServiceLogger serviceLogger){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreServiceLogger(MESBasicServiceLogger serviceLogger){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreServiceLogger(MESBasicServiceLogger serviceLogger){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicServiceLogger serviceLogger, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicServiceLogger serviceLogger, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportServiceLogger(List<MESBasicServiceLogger> insertObjs, List<MESBasicServiceLogger> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportServiceLogger,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportServiceLogger(List<MESBasicServiceLogger> insertObjs, List<MESBasicServiceLogger> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportServiceLogger,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_backgroundServiceMonitor_ServiceLogger,MESBasic_1) */
// 自定义代码
	@Override
	public void deleteLoggerRecord(String sql, Object... args){
		serviceLoggerDao.createNativeQuery(sql, args).executeUpdate();
	}
/* CUSTOM CODE END */
}
