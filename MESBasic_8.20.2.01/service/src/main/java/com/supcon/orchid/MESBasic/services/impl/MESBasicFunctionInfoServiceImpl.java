...
import com.supcon.orchid.MESBasic.services.MESBasicFunctionInfoImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicFunctionInfoDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_functionInfoService")
@Transactional
public class MESBasicFunctionInfoServiceImpl  extends BaseServiceImpl<MESBasicFunctionInfo> implements MESBasicFunctionInfoService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicFunctionInfo> bapBaseService;
	...
	@Autowired
	private MESBasicFunctionInfoImportService functionInfoImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicFunctionInfo.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  functionInfoDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicFunctionInfo.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = functionInfoDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicFunctionInfo.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicFunctionInfo.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  functionInfoDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicFunctionInfo.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicFunctionInfo.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicFunctionInfo> list = (List<MESBasicFunctionInfo>) functionInfoDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFunctionInfo getFunctionInfo(long id){
		return getFunctionInfo(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFunctionInfo getFunctionInfo(long id, String viewCode){
		MESBasicFunctionInfo functionInfo = functionInfoDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFunctionInfoAsJSON(long id, String include){
		MESBasicFunctionInfo functionInfo = functionInfoDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(functionInfo, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.common.delete",operType="3")
	public void deleteFunctionInfo(MESBasicFunctionInfo functionInfo){
		if(null != functionInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFunctionInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
			AuditUtil.setColumnStringA(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
			AuditUtil.setColumnStringC(functionInfo.getId().toString());
		}
		beforeDeleteFunctionInfo(functionInfo);
		functionInfoDao.deletePhysical(functionInfo);
		afterDeleteFunctionInfo(functionInfo);
		functionInfoDao.flush();
		List<MESBasicFunctionInfo> params = new ArrayList<MESBasicFunctionInfo>();
		params.add(functionInfo);
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.common.delete",operType="3")
	public void deleteFunctionInfo(List<Long> functionInfoIds){
		deleteFunctionInfo(functionInfoIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.common.delete",operType="3")
	public void deleteFunctionInfo(List<Long> functionInfoIds, String eventTopic) {
		List<MESBasicFunctionInfo> functionInfos = new ArrayList<MESBasicFunctionInfo>();
		for(Long functionInfoId : functionInfoIds){
			MESBasicFunctionInfo functionInfo = getFunctionInfo(functionInfoId);
			functionInfos.add(functionInfo);
			if(null != functionInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFunctionInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == functionInfo.getId() ? "" : "," + functionInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == functionInfo.getId() ? "" : "," + functionInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == functionInfo.getId() ? "" : "," + functionInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
				}
			}
		}
		
		for(MESBasicFunctionInfo functionInfo : functionInfos){
			beforeDeleteFunctionInfo(functionInfo);
		}
		...
		if(functionInfos != null && functionInfos.size() > 0 ){
			for(MESBasicFunctionInfo functionInfo : functionInfos){
				...
				java.lang.reflect.Field[] fields = MESBasicFunctionInfo.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(functionInfo));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),functionInfo, functionInfo.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(functionInfoIds != null && functionInfoIds.size() > 0) {
			String hql = "delete " + MESBasicFunctionInfo.JPA_NAME + " where id in(:ids)";
			Query query = functionInfoDao.createQuery(hql);
			query.setParameterList("ids", functionInfoIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicFunctionInfo functionInfoz : functionInfos){
				functionInfoz.setValid(false);
				List<MESBasicFunctionInfo> params = new ArrayList<MESBasicFunctionInfo>();
				params.add(functionInfoz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_container");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.common.delete",operType="3")
	public void deleteFunctionInfo(long functionInfoId, int functionInfoVersion){
		this.deleteFunctionInfo(functionInfoId, functionInfoVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.common.delete",operType="3")
	public void deleteFunctionInfo(long functionInfoId, int functionInfoVersion,SignatureLog signatureLog){
		MESBasicFunctionInfo functionInfo = getFunctionInfo(functionInfoId);
		if(functionInfo != null && !functionInfo.isValid()){
		...
		beforeDeleteFunctionInfo(functionInfo);
		functionInfoDao.delete(functionInfoId, functionInfoVersion);
		afterDeleteFunctionInfo(functionInfo);
		functionInfoDao.flush();
		List<MESBasicFunctionInfo> params = new ArrayList<MESBasicFunctionInfo>();
		params.add(functionInfo);
		...
		if(signatureLog != null ) {
			...
			businessKey=functionInfo.getId();
			signatureLog.setTableId(functionInfo.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.common.delete",operType="3")
	public void deleteFunctionInfo(String functionInfoIds){
		this.deleteFunctionInfo(functionInfoIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.common.delete",operType="3")
	public void deleteFunctionInfo(String functionInfoIds,SignatureLog signatureLog){
		deleteCollection(functionInfoIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFunctionInfo(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_container#modelCode:MESBasic_1_container_FunctionInfo#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="ec.engine.restore",operType="10")
	public void restoreFunctionInfo(String functionInfoIds){
		restoreCollection(functionInfoIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFunctionInfo(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="ec.engine.restore",operType="10")
	public void restoreFunctionInfo(long functionInfoId){
		
		findBusinessKeyUsed(functionInfoId);	//判断业务主键是否重复
		...
		beforeRestoreFunctionInfo(functionInfo);
		functionInfo.setValid(true);
		functionInfoDao.update(functionInfo);
		afterRestoreFunctionInfo(functionInfo);
		List<MESBasicFunctionInfo> params = new ArrayList<MESBasicFunctionInfo>();
		params.add(functionInfo);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long functionInfoId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_container_FunctionInfo");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicFunctionInfo.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicFunctionInfo.TABLE_NAME +" where id = ? )";
			List<Object> list =  functionInfoDao.createNativeQuery(sql,functionInfoId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFunctionInfo(List<MESBasicFunctionInfo>  functionInfos){
		for(MESBasicFunctionInfo functionInfo:functionInfos)  {
			saveFunctionInfo(functionInfo, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFunctionInfo(List<MESBasicFunctionInfo>  functionInfos){
		for(MESBasicFunctionInfo functionInfo:functionInfos)  {
			saveFunctionInfo(functionInfo, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="ec.flow.save")
	public void saveFunctionInfo(MESBasicFunctionInfo functionInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFunctionInfo(functionInfo, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="ec.flow.save")
	public void saveFunctionInfo(MESBasicFunctionInfo functionInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFunctionInfo(functionInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFunctionInfoCodes(MESBasicFunctionInfo functionInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFunctionInfoCodes(functionInfo, false);
	}
	
	public void generateFunctionInfoCodes(MESBasicFunctionInfo functionInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFunctionInfoSummarys(MESBasicFunctionInfo functionInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFunctionInfoSummarys(functionInfo, false);
	}

	public void generateFunctionInfoSummarys(MESBasicFunctionInfo functionInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo")
	public void saveFunctionInfo(MESBasicFunctionInfo functionInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFunctionInfo(functionInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo")
	public void saveFunctionInfo(MESBasicFunctionInfo functionInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_container");
		String url = null;
		if(functionInfo.getId() != null && functionInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFunctionInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, functionInfo);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(functionInfo);
		beforeSaveFunctionInfo(functionInfo, viewIsView);

		if (viewIsView) {
			functionInfoDao.saveWithRevertVersion(functionInfo);
		} else {
			if(isNew)
				functionInfoDao.save(functionInfo);
			else
				functionInfoDao.merge(functionInfo);
		}
		if(null != functionInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFunctionInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
			AuditUtil.setColumnStringA(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
		...
		dealDatagridsSave(functionInfo,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFunctionInfoCodes(functionInfo, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFunctionInfoSummarys(functionInfo, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFunctionInfo(functionInfo, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, functionInfo);
			}
		}
		List<MESBasicFunctionInfo> params = new ArrayList<MESBasicFunctionInfo>();
		functionInfoDao.flush();
		functionInfoDao.clear();
		functionInfo = functionInfoDao.load(functionInfo.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicFunctionInfo functionInfo){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("functionInfo", functionInfo);
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo")
	public void mergeFunctionInfo(MESBasicFunctionInfo functionInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(functionInfo);
		beforeSaveFunctionInfo(functionInfo);
		functionInfoDao.merge(functionInfo);
		if(null != functionInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFunctionInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
			AuditUtil.setColumnStringA(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
		...
		dealDatagridsSave(functionInfo,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFunctionInfo(functionInfo);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicFunctionInfo> findFunctionInfos(Page<MESBasicFunctionInfo> page, Criterion... criterions) {
		return functionInfoDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFunctionInfos(Page<MESBasicFunctionInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFunctionInfos(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFunctionInfos(Page<MESBasicFunctionInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFunctionInfos(Page<MESBasicFunctionInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicFunctionInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"functionInfo\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"functionInfo\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"functionInfo\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  functionInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  functionInfoDao.createNativeQuery(powerOperateSql, "MESBasic_1_container").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"functionInfo\".LAY_REC ASC, \"functionInfo\".SORT ASC");
				} else {
					orderPart.append(" \"functionInfo\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) functionInfoDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=functionInfoDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = functionInfoDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicFunctionInfo> result = new ArrayList<MESBasicFunctionInfo>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicFunctionInfo.class,functionInfoDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicFunctionInfo.class,functionInfoDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicFunctionInfo functionInfo : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(functionInfo.getId(), "MESBasic_container_functionInfo");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(functionInfo.getId(), "MESBasic_container_functionInfo");
							if(null != documents && !documents.isEmpty()) {
								functionInfo.setDocument(documents.get(0));
								functionInfo.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicFunctionInfo> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicFunctionInfo> result) {
		View view = null;
		...
		Map<Long, MESBasicFunctionInfo> rsMap = new HashMap<Long, MESBasicFunctionInfo>();
		for (MESBasicFunctionInfo functionInfo : result) {
			ids.add(functionInfo.getId());
			rsMap.put(functionInfo.getId(), functionInfo);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(functionInfoDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = functionInfoDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_container_FunctionInfo" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_container_FunctionInfo" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  functionInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"functionInfo\"","MESBasic_1_container","","MESBASIC_FUNCTION_INFOS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"functionInfo\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"functionInfo\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"functionInfo\"","MESBasic_1_container","","MESBASIC_FUNCTION_INFOS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_container_FunctionInfo" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("functionInfo.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"functionInfo\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"functionInfo\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"functionInfo\".CID = 1 OR \"functionInfo\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("functionInfo.mc\"").append(".FUNCTION_INFO) not in ("+notinValue+")");
			}
			
			SQLQuery query = functionInfoDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=functionInfoDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicFunctionInfo.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicFunctionInfo.MODULE_CODE;}
	protected String getModelCode(){return MESBasicFunctionInfo.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param functionInfo 模型实例
	 */
	public void checkUniqueConstraint(MESBasicFunctionInfo functionInfo){
	}
	
	
	@Override
	public List<MESBasicFunctionInfo> getFunctionInfos(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicFunctionInfo.class);
		if(params!=null){
			...
			return functionInfoDao.findByHql("from " + MESBasicFunctionInfo.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return functionInfoDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicFunctionInfo> findFunctionInfosBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicFunctionInfo.class);
	...
	@Override
	public List<MESBasicFunctionInfo> findFunctionInfosByHql(String hql, Object... objects){
		
		return   functionInfoDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicFunctionInfo loadFunctionInfoByBussinessKey(MESBasicFunctionInfo functionInfo){
		return loadFunctionInfoByBussinessKey(functionInfo.getId());
	}
	
	@Override
	public Page<MESBasicFunctionInfo> getFunctionInfos(Page<MESBasicFunctionInfo> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicFunctionInfo.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = functionInfoDao.findByPage(page, sql);
			}else{
				page = functionInfoDao.findAllByPage(page);
			}
			return page;
		}
		page = functionInfoDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFunctionInfo loadFunctionInfoByBussinessKey(Serializable bussinessKey){
		return functionInfoDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicFunctionInfo> findByProperty(String propertyName, Object object){
		return functionInfoDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFunctionInfo findEntityByProperty(String propertyName, Object object){
		return functionInfoDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicFunctionInfo.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = functionInfoDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_container_FunctionInfo","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicFunctionInfo functionInfo,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,functionInfoDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicFunctionInfo.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, functionInfo);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",functionInfo.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicFunctionInfo.TABLE_NAME +" where valid = 1";
			list = functionInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicFunctionInfo.TABLE_NAME;
			list = functionInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicFunctionInfo.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = functionInfoDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicFunctionInfoDaoImpl functionInfoDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchFunctionInfo(final List<MESBasicFunctionInfo> insertObjs, final List<MESBasicFunctionInfo> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchFunctionInfo(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFunctionInfo(final List<MESBasicFunctionInfo> insertObjs, final List<MESBasicFunctionInfo> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1555292653876"),importCount));
		...
		Map<Long, MESBasicFunctionInfo> oldUpdateObjsMap = new HashMap<Long, MESBasicFunctionInfo>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicFunctionInfo bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicFunctionInfo> bizObjList = functionInfoDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicFunctionInfo bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicFunctionInfo> nmeObjs = new ArrayList<MESBasicFunctionInfo>();
		...
		long id=autoGeneratorID.getNextId(MESBasicFunctionInfo.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicFunctionInfo functionInfo:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicFunctionInfo.TABLE_NAME,1,dbId);
			if(functionInfo.getId() != null ){
				m.put(functionInfo.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) functionInfoDao.createNativeQuery(sql, "MESBasic_1_container_FunctionInfo"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		functionInfoDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicFunctionInfo.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicFunctionInfo functionInfo = updateObjs.get(updateIndex);
						Long id = functionInfo.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = functionInfo.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(functionInfo);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicFunctionInfo.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicFunctionInfo.TABLE_NAME + " order by id desc";
//					Query query = functionInfoDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicFunctionInfo functionInfo = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = functionInfo.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		functionInfoDaoImpl.getSession().flush();
		functionInfoDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicFunctionInfo bizObj = insertObjs.get(insertIndex);
			functionInfoImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicFunctionInfo bizObj = updateObjs.get(updateIndex);
			MESBasicFunctionInfo oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			functionInfoImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicFunctionInfo> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicFunctionInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) functionInfoDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicFunctionInfo> result = getResult(page, query, new PendingResultTransformer(MESBasicFunctionInfo.class,functionInfoDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicFunctionInfo model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = functionInfoDao.createNativeQuery(sql, "MESBasic_1_container_FunctionInfo").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = functionInfoDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = functionInfoDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = functionInfoDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?functionInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  functionInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,functionInfoDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="ec.flow.save")
	public void saveFunctionInfo(MESBasicFunctionInfo functionInfo, DataGridService dataGridService){
		saveFunctionInfo(functionInfo, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="ec.flow.save")
	public void saveFunctionInfo(MESBasicFunctionInfo functionInfo, DataGridService dataGridService, String viewCode){
		saveFunctionInfo(functionInfo, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo")
	public void saveFunctionInfo(MESBasicFunctionInfo functionInfo, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFunctionInfo(functionInfo, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo")
	public void saveFunctionInfo(MESBasicFunctionInfo functionInfo, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_container";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_container");
		String url = null;
		if(null != functionInfo.getId() && functionInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFunctionInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo")
	public Map<Object, Object> functionInfoDataGridImport(MESBasicFunctionInfo functionInfo, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = functionInfo.getVirtualId();
		
		functionInfo = functionInfoDao.load(functionInfo.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, functionInfo.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = functionInfo.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(functionInfo);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, functionInfo.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo")
	public void mergeFunctionInfo(MESBasicFunctionInfo functionInfo, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(functionInfo);
		beforeSaveFunctionInfo(functionInfo);
		functionInfoDao.merge(functionInfo);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FunctionInfo",desc="foundation.batch.save",operType="4")
	public void batchSaveFunctionInfo(MESBasicFunctionInfo functionInfo, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_container";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_container");
	...
	@Override
	public Page<MESBasicFunctionInfo> getByPage(Page<MESBasicFunctionInfo> page,DetachedCriteria detachedCriteria){
		return functionInfoDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFunctionInfo(MESBasicFunctionInfo functionInfo, Object... objects){
		checkUniqueConstraint(functionInfo);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFunctionInfo(MESBasicFunctionInfo functionInfo, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportFunctionInfo(List<MESBasicFunctionInfo> functionInfos, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportFunctionInfo(List<MESBasicFunctionInfo> functionInfos){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportFunctionInfo(List<MESBasicFunctionInfo> functionInfos, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportFunctionInfo(List<MESBasicFunctionInfo> functionInfos){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFunctionInfo(MESBasicFunctionInfo functionInfo){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFunctionInfo(MESBasicFunctionInfo functionInfo){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFunctionInfo(MESBasicFunctionInfo functionInfo){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFunctionInfo(MESBasicFunctionInfo functionInfo){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicFunctionInfo functionInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicFunctionInfo functionInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportFunctionInfo(List<MESBasicFunctionInfo> insertObjs, List<MESBasicFunctionInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFunctionInfo,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportFunctionInfo(List<MESBasicFunctionInfo> insertObjs, List<MESBasicFunctionInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFunctionInfo,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
