...
import com.supcon.orchid.MESBasic.services.MESBasicItemPartImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicItemPartDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_itemPartService")
@Transactional
public class MESBasicItemPartServiceImpl  extends BaseServiceImpl<MESBasicItemPart> implements MESBasicItemPartService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicItemPart> bapBaseService;
	...
	@Autowired
	private MESBasicItemPartImportService itemPartImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicItemPart.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  itemPartDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicItemPart.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = itemPartDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicItemPart.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicItemPart.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  itemPartDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicItemPart.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicItemPart.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicItemPart> list = (List<MESBasicItemPart>) itemPartDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicItemPart getItemPart(long id){
		return getItemPart(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicItemPart getItemPart(long id, String viewCode){
		MESBasicItemPart itemPart = itemPartDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getItemPartAsJSON(long id, String include){
		MESBasicItemPart itemPart = itemPartDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(itemPart, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.common.delete",operType="3")
	public void deleteItemPart(MESBasicItemPart itemPart){
		if(null != itemPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemPart.getId() ? "" : itemPart.getId().toString());
			AuditUtil.setColumnStringA(null == itemPart.getId() ? "" : itemPart.getId().toString());
			AuditUtil.setColumnStringC(itemPart.getId().toString());
		}
		beforeDeleteItemPart(itemPart);
		itemPartDao.deletePhysical(itemPart);
		afterDeleteItemPart(itemPart);
		itemPartDao.flush();
		List<MESBasicItemPart> params = new ArrayList<MESBasicItemPart>();
		params.add(itemPart);
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.common.delete",operType="3")
	public void deleteItemPart(List<Long> itemPartIds){
		deleteItemPart(itemPartIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.common.delete",operType="3")
	public void deleteItemPart(List<Long> itemPartIds, String eventTopic) {
		List<MESBasicItemPart> itemParts = new ArrayList<MESBasicItemPart>();
		for(Long itemPartId : itemPartIds){
			MESBasicItemPart itemPart = getItemPart(itemPartId);
			itemParts.add(itemPart);
			if(null != itemPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == itemPart.getId() ? "" : "," + itemPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == itemPart.getId() ? "" : itemPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == itemPart.getId() ? "" : "," + itemPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == itemPart.getId() ? "" : itemPart.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == itemPart.getId() ? "" : "," + itemPart.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == itemPart.getId() ? "" : itemPart.getId().toString());
				}
			}
		}
		...
		if(itemParts != null && itemParts.size() > 0 ){
			for(MESBasicItemPart itemPart : itemParts){
				...
				java.lang.reflect.Field[] fields = MESBasicItemPart.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(itemPart));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),itemPart, itemPart.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(itemPartIds != null && itemPartIds.size() > 0) {
			String hql = "delete " + MESBasicItemPart.JPA_NAME + " where id in(:ids)";
			Query query = itemPartDao.createQuery(hql);
			query.setParameterList("ids", itemPartIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicItemPart itemPartz : itemParts){
				itemPartz.setValid(false);
				List<MESBasicItemPart> params = new ArrayList<MESBasicItemPart>();
				params.add(itemPartz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_itemManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.common.delete",operType="3")
	public void deleteItemPart(long itemPartId, int itemPartVersion){
		this.deleteItemPart(itemPartId, itemPartVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.common.delete",operType="3")
	public void deleteItemPart(long itemPartId, int itemPartVersion,SignatureLog signatureLog){
		MESBasicItemPart itemPart = getItemPart(itemPartId);
		...
		beforeDeleteItemPart(itemPart);
		itemPartDao.delete(itemPartId, itemPartVersion);
		afterDeleteItemPart(itemPart);
		itemPartDao.flush();
		List<MESBasicItemPart> params = new ArrayList<MESBasicItemPart>();
		params.add(itemPart);
		...
		if(signatureLog != null ) {
			...
			businessKey=itemPart.getId();
			signatureLog.setTableId(itemPart.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.common.delete",operType="3")
	public void deleteItemPart(String itemPartIds){
		this.deleteItemPart(itemPartIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.common.delete",operType="3")
	public void deleteItemPart(String itemPartIds,SignatureLog signatureLog){
		deleteCollection(itemPartIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteItemPart(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_itemManage#modelCode:MESBasic_1_itemManage_ItemPart#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="ec.engine.restore",operType="10")
	public void restoreItemPart(String itemPartIds){
		restoreCollection(itemPartIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreItemPart(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="ec.engine.restore",operType="10")
	public void restoreItemPart(long itemPartId){
		
		findBusinessKeyUsed(itemPartId);	//判断业务主键是否重复
		...
		beforeRestoreItemPart(itemPart);
		itemPart.setValid(true);
		itemPartDao.update(itemPart);
		afterRestoreItemPart(itemPart);
		List<MESBasicItemPart> params = new ArrayList<MESBasicItemPart>();
		params.add(itemPart);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long itemPartId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_itemManage_ItemPart");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicItemPart.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicItemPart.TABLE_NAME +" where id = ? )";
			List<Object> list =  itemPartDao.createNativeQuery(sql,itemPartId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseItemPart(List<MESBasicItemPart>  itemParts){
		for(MESBasicItemPart itemPart:itemParts)  {
			saveItemPart(itemPart, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseItemPart(List<MESBasicItemPart>  itemParts){
		for(MESBasicItemPart itemPart:itemParts)  {
			saveItemPart(itemPart, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="ec.flow.save")
	public void saveItemPart(MESBasicItemPart itemPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveItemPart(itemPart, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="ec.flow.save")
	public void saveItemPart(MESBasicItemPart itemPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveItemPart(itemPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateItemPartCodes(MESBasicItemPart itemPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateItemPartCodes(itemPart, false);
	}
	
	public void generateItemPartCodes(MESBasicItemPart itemPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateItemPartSummarys(MESBasicItemPart itemPart) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateItemPartSummarys(itemPart, false);
	}

	public void generateItemPartSummarys(MESBasicItemPart itemPart, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart")
	public void saveItemPart(MESBasicItemPart itemPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveItemPart(itemPart, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart")
	public void saveItemPart(MESBasicItemPart itemPart,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_itemManage");
		String url = null;
		if(itemPart.getId() != null && itemPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, itemPart);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(itemPart);
		beforeSaveItemPart(itemPart, viewIsView);

		if (viewIsView) {
			itemPartDao.saveWithRevertVersion(itemPart);
		} else {
			if(isNew)
				itemPartDao.save(itemPart);
			else
				itemPartDao.merge(itemPart);
		}
		if(null != itemPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemPart.getId() ? "" : itemPart.getId().toString());
			AuditUtil.setColumnStringA(null == itemPart.getId() ? "" : itemPart.getId().toString());
		...
		dealDatagridsSave(itemPart,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateItemPartCodes(itemPart, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateItemPartSummarys(itemPart, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveItemPart(itemPart, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, itemPart);
			}
		}
		List<MESBasicItemPart> params = new ArrayList<MESBasicItemPart>();
		itemPartDao.flush();
		itemPartDao.clear();
		itemPart = itemPartDao.load(itemPart.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicItemPart itemPart){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("itemPart", itemPart);
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart")
	public void mergeItemPart(MESBasicItemPart itemPart, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(itemPart);
		beforeSaveItemPart(itemPart);
		itemPartDao.merge(itemPart);
		if(null != itemPart && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemPart.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemPart.getId() ? "" : itemPart.getId().toString());
			AuditUtil.setColumnStringA(null == itemPart.getId() ? "" : itemPart.getId().toString());
		...
		dealDatagridsSave(itemPart,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveItemPart(itemPart);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicItemPart> findItemParts(Page<MESBasicItemPart> page, Criterion... criterions) {
		return itemPartDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemParts(Page<MESBasicItemPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findItemParts(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemParts(Page<MESBasicItemPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemParts(Page<MESBasicItemPart> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicItemPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"itemPart\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"itemPart\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"itemPart\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"itemPart\".ITEM_VIEW IN ( SELECT ID FROM PELLET_ITEMSETUP \"itemSetup\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "MESBasic_1_itemManage" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  itemPartDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  itemPartDao.createNativeQuery(powerOperateSql, "MESBasic_1_itemManage").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"itemSetup\"","MESBasic_1_itemManage","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"itemPart\".LAY_REC ASC, \"itemPart\".SORT ASC");
				} else {
					orderPart.append(" \"itemPart\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) itemPartDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=itemPartDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = itemPartDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicItemPart> result = new ArrayList<MESBasicItemPart>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicItemPart.class,itemPartDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicItemPart.class,itemPartDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicItemPart itemPart : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(itemPart.getId(), "MESBasic_itemManage_itemPart");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(itemPart.getId(), "MESBasic_itemManage_itemPart");
							if(null != documents && !documents.isEmpty()) {
								itemPart.setDocument(documents.get(0));
								itemPart.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicItemPart> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicItemPart> result) {
		View view = null;
		...
		Map<Long, MESBasicItemPart> rsMap = new HashMap<Long, MESBasicItemPart>();
		for (MESBasicItemPart itemPart : result) {
			ids.add(itemPart.getId());
			rsMap.put(itemPart.getId(), itemPart);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(itemPartDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = itemPartDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_itemManage_ItemPart" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_itemManage_ItemPart" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  itemPartDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"itemPart\"","MESBasic_1_itemManage","","PELLET_ITEMPART",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"itemPart\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"itemPart\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"itemPart\"","MESBasic_1_itemManage","","PELLET_ITEMPART",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_itemManage_ItemPart" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("itemPart.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"itemPart\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("itemPart.mc\"").append(".ITEM_PART) not in ("+notinValue+")");
			}
			
			SQLQuery query = itemPartDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=itemPartDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicItemPart.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicItemPart.MODULE_CODE;}
	protected String getModelCode(){return MESBasicItemPart.MODEL_CODE;}
	 ...
	 * @param itemPart 模型实例
	 */
	public void checkUniqueConstraint(MESBasicItemPart itemPart){
	}
	
	
	@Override
	public List<MESBasicItemPart> getItemParts(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicItemPart.class);
		if(params!=null){
			...
			return itemPartDao.findByHql("from " + MESBasicItemPart.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return itemPartDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicItemPart> findItemPartsByHql(String hql, Object... objects){
		...
		return   itemPartDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicItemPart loadItemPartByBussinessKey(MESBasicItemPart itemPart){
		return loadItemPartByBussinessKey(itemPart.getId());
	}
	
	@Override
	public Page<MESBasicItemPart> getItemParts(Page<MESBasicItemPart> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicItemPart.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = itemPartDao.findByPage(page, sql);
			}else{
				page = itemPartDao.findAllByPage(page);
			}
			return page;
		}
		page = itemPartDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicItemPart> findByTreeDataGridItemSetup(List<MESBasicItemPart> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicItemPart itemPart : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridItemSetup(Page<MESBasicItemPart> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicItemPart> itemParts = dgPage.getResult();
			for(MESBasicItemPart itemPart : itemParts){
				
			}
			dgPage.setResult(itemParts);
		}
	...
	public MESBasicItemPart loadItemPartByBussinessKey(Serializable bussinessKey){
		return itemPartDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicItemPart> findByProperty(String propertyName, Object object){
		return itemPartDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicItemPart findEntityByProperty(String propertyName, Object object){
		return itemPartDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicItemPart.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = itemPartDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_itemManage_ItemPart","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicItemPart itemPart,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,itemPartDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicItemPart.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, itemPart);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",itemPart.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicItemPart.TABLE_NAME +" where valid = 1";
			list = itemPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicItemPart.TABLE_NAME;
			list = itemPartDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicItemPart.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = itemPartDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicItemPartDaoImpl itemPartDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchItemPart(final List<MESBasicItemPart> insertObjs, final List<MESBasicItemPart> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchItemPart(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchItemPart(final List<MESBasicItemPart> insertObjs, final List<MESBasicItemPart> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1415259448384"),importCount));
		...
		Map<Long, MESBasicItemPart> oldUpdateObjsMap = new HashMap<Long, MESBasicItemPart>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicItemPart bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicItemPart> bizObjList = itemPartDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicItemPart bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicItemPart> nmeObjs = new ArrayList<MESBasicItemPart>();
		...
		long id=autoGeneratorID.getNextId(MESBasicItemPart.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicItemPart itemPart:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicItemPart.TABLE_NAME,1,dbId);
			if(itemPart.getId() != null ){
				m.put(itemPart.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) itemPartDao.createNativeQuery(sql, "MESBasic_1_itemManage_ItemPart"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		itemPartDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicItemPart.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicItemPart itemPart = updateObjs.get(updateIndex);
						Long id = itemPart.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = itemPart.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicItemPart.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicItemPart.TABLE_NAME + " order by id desc";
//					Query query = itemPartDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicItemPart itemPart = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = itemPart.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		itemPartDaoImpl.getSession().flush();
		itemPartDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicItemPart bizObj = insertObjs.get(insertIndex);
			itemPartImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicItemPart bizObj = updateObjs.get(updateIndex);
			MESBasicItemPart oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			itemPartImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicItemPart> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicItemPart> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) itemPartDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicItemPart> result = getResult(page, query, new PendingResultTransformer(MESBasicItemPart.class,itemPartDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicItemPart model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = itemPartDao.createNativeQuery(sql, "MESBasic_1_itemManage_ItemPart").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = itemPartDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = itemPartDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = itemPartDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?itemPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  itemPartDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,itemPartDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="ec.flow.save")
	public void saveItemPart(MESBasicItemPart itemPart, DataGridService dataGridService){
		saveItemPart(itemPart, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="ec.flow.save")
	public void saveItemPart(MESBasicItemPart itemPart, DataGridService dataGridService, String viewCode){
		saveItemPart(itemPart, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart")
	public void saveItemPart(MESBasicItemPart itemPart, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveItemPart(itemPart, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart")
	public void saveItemPart(MESBasicItemPart itemPart, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_itemManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_itemManage");
		String url = null;
		if(null != itemPart.getId() && itemPart.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemPart.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart")
	public Map<Object, Object> itemPartDataGridImport(MESBasicItemPart itemPart, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = itemPart.getVirtualId();
		
		itemPart = itemPartDao.load(itemPart.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, itemPart.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = itemPart.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(itemPart);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, itemPart.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart")
	public void mergeItemPart(MESBasicItemPart itemPart, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(itemPart);
		beforeSaveItemPart(itemPart);
		itemPartDao.merge(itemPart);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemManage",model="MESBasic_1_itemManage_ItemPart",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicItemPart> getByPage(Page<MESBasicItemPart> page,DetachedCriteria detachedCriteria){
		return itemPartDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveItemPart(MESBasicItemPart itemPart, Object... objects){
		checkUniqueConstraint(itemPart);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
      itemPart.setIpID(itemPart.getId());
/* CUSTOM CODE END */
	}
	
	private void afterSaveItemPart(MESBasicItemPart itemPart, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteItemPart(MESBasicItemPart itemPart){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteItemPart(MESBasicItemPart itemPart){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreItemPart(MESBasicItemPart itemPart){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreItemPart(MESBasicItemPart itemPart){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicItemPart itemPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicItemPart itemPart, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportItemPart(List<MESBasicItemPart> insertObjs, List<MESBasicItemPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportItemPart,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportItemPart(List<MESBasicItemPart> insertObjs, List<MESBasicItemPart> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportItemPart,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_itemManage_ItemPart,MESBasic_1) */
// 自定义代码
