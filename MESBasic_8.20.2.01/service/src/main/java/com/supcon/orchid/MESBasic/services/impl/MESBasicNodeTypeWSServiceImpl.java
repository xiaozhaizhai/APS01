...
import com.supcon.orchid.MESBasic.services.MESBasicNodeTypeWSService;
import com.supcon.orchid.MESBasic.services.MESBasicNodeTypeService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_nodeType_NodeType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicNodeType",targetNamespace = "http://ws.supcon.com")
public class MESBasicNodeTypeWSServiceImpl implements MESBasicNodeTypeWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getNodeTypeByBusinessKey(
		...
		try{
			...
			MESBasicNodeType nodeType = nodeTypeService.loadNodeTypeByBussinessKey(businessKey);
			if(nodeType == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, nodeType);
		}catch(Throwable t){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getNodeTypeByPk(
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO updateNodeType(
		@WebParam(name="nodeType")MESBasicNodeType nodeType,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
			
		JWSResultDTO result = new JWSResultDTO();
		try{
			if(nodeType == null){
				return set500Error(result, "节点类型不能为空。");
			}
			...
			if(!beforeUpdateNodeType(nodeType)){
				return set404Error(result);
			}
			nodeTypeService.saveNodeType(nodeType, null);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO addNodeType(
		@WebParam(name="nodeType")MESBasicNodeType nodeType,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				checkNodeTypeData(nodeType);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			nodeTypeService.saveNodeType(nodeType, null);
			setSuccessResult(result, nodeType.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteNodeTypeByBusinessKey(
		@WebParam(name="businessKey")String businessKey,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				nodeType = nodeTypeService.loadNodeTypeByBussinessKey(businessKey);
	...
	@Override//
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteNodeTypeByPk(
		@WebParam(name="pk")String pk,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddNodeType(
		@WebParam(name="nodeTypeList")List<MESBasicNodeType> nodeTypeList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			int count = 1;
			for(MESBasicNodeType nodeType : nodeTypeList){
				if(nodeType.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(nodeType.getCode() == null || nodeType.getCode().toString().trim().isEmpty()){
			...
			nodeTypeService.batchImportBaseNodeType(nodeTypeList);
			for(MESBasicNodeType nodeType : nodeTypeList){
				idList.add(nodeType.getId());
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdateNodeType(
		@WebParam(name="nodeTypeList")List<MESBasicNodeType> nodeTypeList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteNodeTypeByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicNodeType temp = null;
				try{
					temp = nodeTypeService.getNodeType(id);
				}catch(NullPointerException e){}
			...
			nodeTypeService.deleteNodeType(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteNodeTypeByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicNodeType temp = null;
				try{
					temp = nodeTypeService.loadNodeTypeByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getNodeTypePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicNodeType.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicNodeType.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicNodeType.class);
				} catch (Exception e) {
			...
			Page<MESBasicNodeType> page = new Page<MESBasicNodeType>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicNodeType> datas = nodeTypeService.getByPage(page, detachedCriteria);
			List<MESBasicNodeType> nodeTypeList = datas.getResult();
			if (nodeTypeList == null || nodeTypeList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(nodeTypeList.toArray());
			PaginationDTO<MESBasicNodeType> pagination = new PaginationDTO<MESBasicNodeType>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicNodeType nodeType = nodeTypeService.getNodeType(id);
			if (nodeType == null || !nodeType.isValid()) {
				result.setErrorMessage("找不到节点类型");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取节点类型对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param nodeType
	 * @return
	 */
	private MESBasicNodeType getNodeType(MESBasicNodeType nodeType) {
		if (nodeType.getId() == null && (nodeType.getCode() == null || nodeType.getCode().toString().trim() == ""))
			throw new RuntimeException("节点类型对象业务主键和id不能同时为空");
		if (nodeType.getId() != null) {
			MESBasicNodeType result = nodeTypeService.getNodeType(nodeType .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return nodeTypeService.loadNodeTypeByBussinessKey(nodeType.getCode());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个节点类型，则返回false，否则返回true
	 * @param nodeType
	 */
	private boolean beforeUpdateNodeType(MESBasicNodeType nodeType) {
		if(nodeType.getName() == null){
			throw new RuntimeException("name不能为空。");
		}
		MESBasicNodeType temp = getNodeType(nodeType);
		if (temp == null) {
			return false;
		}
		if(nodeType.getId() == null){
			nodeType.setId(temp.getId());		
		}
		if(nodeType.getCode() == null){
			nodeType.setCode(temp.getCode());
		}
		
		nodeType.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除节点类型对象下所有辅模型对象中的节点类型对象
	 * @param nodeType
	 */
	private void clearCircle(MESBasicNodeType nodeType) {
	 ...
	 */
	private void checkNodeTypeData(MESBasicNodeType nodeType){
		 if(nodeType.getName() == null){
		 	throw new RuntimeException("name不能为空。");
		 }
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_nodeType_NodeType,MESBasic_1) */
// 自定义代码
