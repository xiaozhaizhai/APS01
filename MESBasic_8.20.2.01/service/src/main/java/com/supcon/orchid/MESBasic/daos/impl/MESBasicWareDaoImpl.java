...
@Repository
public class MESBasicWareDaoImpl extends ExtendedGenericDaoImpl<MESBasicWare, Long> implements MESBasicWareDao {
	@Override
	...
	@Override
	public MESBasicWareDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicWareDealInfo dealInfo = new MESBasicWareDealInfo();
	...
	@Override
	public MESBasicWareDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicWare mainObj, Staff dealStaff) {
		MESBasicWareDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicWareDealInfo.class);
		...
		List<MESBasicWareDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_wareMan_Ware,MESBasic_1) */
// 自定义代码
