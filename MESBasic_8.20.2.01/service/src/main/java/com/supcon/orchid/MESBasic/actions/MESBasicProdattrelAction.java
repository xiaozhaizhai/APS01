...
/* CUSTOM CODE START(action,import,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicProdattrelAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicProdattrel> {
	...
	private MESBasicProdattrel prodattrel;
	...
	//导出excel时,主模型内容
	List<MESBasicProdattrel> queryResults = new ArrayList<MESBasicProdattrel>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/prodatriel/prodattrel/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/prodatriel/prodattrel/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_prodatriel_Prodattrel" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicProdattrel> prodattrels;
	
	public List<MESBasicProdattrel> getProdattrels() {
		return prodattrels;
	}
	...
	private Page<MESBasicProdattrel> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			prodattrel = new MESBasicProdattrel();
		} else {
			if(id > 0){
				prodattrel = prodattrelService.getProdattrel(id, viewCode);
			}
		}
	...
	public Page<MESBasicProdattrel> getPage() {
		if (null == page) {
			page = new Page<MESBasicProdattrel>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicProdattrel getProdattrel() {
		return prodattrel;
	}		

	public void setProdattrel(MESBasicProdattrel prodattrel) {
		this.prodattrel = prodattrel;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/prodatriel/prodattrel/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prodattrelService.getProdattrelAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/prodatriel/prodattrel/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1482909899954
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String prodattrelReferenceCopy() throws Exception{
			MESBasicProdattrel oldProdattrel = null;
			if(isEditNew()){
				...
				oldProdattrel = new MESBasicProdattrel();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodattrel.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prodattrel.getId());
				...
				oldProdattrel = prodattrelService.getProdattrel(id);
			}
			
			
			prodattrel = prodattrelService.getProdattrel(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_prodatriel_Prodattrel");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicProdattrel.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProdattrel, null);
						Method setValue = MESBasicProdattrel.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prodattrel, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prodattrel.setCreateDepartmentId(oldProdattrel.getCreateDepartmentId());
			prodattrel.setCreatePositionId(oldProdattrel.getCreatePositionId());
			prodattrel.setCreateStaffId(oldProdattrel.getCreateStaffId());
			prodattrel.setOwnerDepartmentId(oldProdattrel.getOwnerDepartmentId());
			prodattrel.setOwnerPositionId(oldProdattrel.getOwnerPositionId());
			prodattrel.setOwnerStaffId(oldProdattrel.getOwnerStaffId());
			prodattrel.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_prodatriel_Prodattrel", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=prodattrelService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return prodattrelReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			prodattrel = prodattrelService.getProdattrel(this.prodattrel.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodattrel.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = prodattrelService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(prodattrelService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(prodattrelService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_prodatriel_Prodattrel", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"prodattrel\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"prodattrel\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("prodAttribute.layRec") && request.getParameter("prodAttribute.layRec").trim().length() > 0) {
			params.add(new Param("\"secattid\".PRODATT_LAYREC", request.getParameter("prodAttribute.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"prodattrel\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"prodattrel\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		prodattrelService.findDataGridPage(dg,dgclass,getDgPage(),prodattrel,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(prodattrel.getId() != null && prodattrel.getId() != -1){
				prodattrel = prodattrelService.getProdattrel(prodattrel.getId());
			}
			prodattrelService.findDataGridPage(dg,dgclass,getDgPage(),prodattrel,searchCondition,params);
	...
	private void save() throws Exception {
		...
		prodattrelService.saveProdattrel(prodattrel,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "submit");
		bap_validate_datagrids=prodattrelService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", prodattrel.getId());
		...
		fileuploads.put("linkId", prodattrel.getId());
		bapAfterCustomMethod("MESBasic_1_prodatriel_Prodattrel", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/prodatriel/prodattrel/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prodattrelService.deleteProdattrel(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_prodatriel_Prodattrel", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodatriel/prodattrel/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "restore");
		prodattrelService.restoreProdattrel(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_prodatriel_Prodattrel", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/prodatriel/prodattrel/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/prodatriel/prodattrel/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "showDealInfoData");
		if (null != tableInfoId) {
			int count=prodattrelService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = prodattrelService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = prodattrelService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_prodatriel_Prodattrel", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicProdattrel> findExportDataInfos(Page<MESBasicProdattrel> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = prodattrelService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = prodattrelService.getAssProperty(p.getCode());	
									//String assModelCode = prodattrelService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			prodattrelService.findProdattrels(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			prodattrelService.findProdattrels(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicProdattrel> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicProdattrel result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".PRODATTREL_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.prodattrel || null == this.prodattrel.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/prodatriel/prodattrel/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_prodatriel_Prodattrel", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/prodatriel/prodattrel/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/prodatriel/prodattrel/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodatriel/prodattrel/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProdattrelService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_prodatriel", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_prodatriel");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_prodatriel_Prodattrel", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodatriel/prodattrel/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProdattrelService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProdAttributeService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_prodatriel_Prodattrel", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_prodatriel_Prodattrel", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/prodatriel/prodattrel/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/prodatriel/prodattrel/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_prodatriel_Prodattrel";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_prodatriel_Prodattrel", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/prodatriel/prodattrel/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicProdattrel prodattrel = null;
		if(null != id) {
			prodattrel = prodattrelService.getProdattrel(id);
		}
		...
		_auditLog.setColumnStringB(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
		_auditLog.setColumnStringA(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
		_auditLog.setColumnStringC(prodattrel.getId().toString());
		_auditLog.setColumnStringD(MESBasicProdattrel.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_prodatriel");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicProdattrelAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, prodattrel,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, prodattrel,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,prodattrel);
		...
		printlog.setEntity("MESBasic_1_prodatriel");
		...
		printlog.setBusinessId(prodattrel.getId());
		printlog.setBussinessKey(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
		printlog.setMainDisplay(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/prodatriel/prodattrel/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(prodattrel,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodatriel/prodattrel/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(prodattrelService.getProdattrel(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/prodatriel/prodattrel/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/prodatriel/prodattrel/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, prodattrel,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				prodattrel = prodattrelService.getProdattrel(Long.parseLong(arrId[i]));
				if(null != prodattrel){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == prodattrel.getId() ? "" : "," + prodattrel.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == prodattrel.getId() ? "" : "," + prodattrel.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == prodattrel.getId() ? "" : "," + prodattrel.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == prodattrel.getId() ? "" : prodattrel.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, prodattrel,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,prodattrel);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/prodatriel/prodattrel/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicProdattrel> findExportPageInfos(Page<MESBasicProdattrel> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_prodatriel" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = prodattrelService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicProdattrel queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = prodattrelService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = prodattrelService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_prodatriel_Prodattrel");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_prodatriel_Prodattrel")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_prodatriel_Prodattrel");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_prodatriel_Prodattrel");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = prodattrelService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
