...
import com.supcon.orchid.MESBasic.services.MESBasicPositionDealImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicPositionDealDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
import com.supcon.orchid.foundation.entities.Position;
import com.supcon.orchid.foundation.services.PositionService;
/* CUSTOM CODE END */

@Service("mESBasic_positionDealService")
@Transactional
public class MESBasicPositionDealServiceImpl  extends BaseServiceImpl<MESBasicPositionDeal> implements MESBasicPositionDealService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicPositionDeal> bapBaseService;
	...
	@Autowired
	private MESBasicPositionDealImportService positionDealImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicPositionDeal.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  positionDealDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicPositionDeal.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = positionDealDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPositionDeal.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPositionDeal.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  positionDealDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicPositionDeal.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicPositionDeal.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicPositionDeal> list = (List<MESBasicPositionDeal>) positionDealDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicPositionDeal getPositionDeal(long id){
		return getPositionDeal(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPositionDeal getPositionDeal(long id, String viewCode){
		MESBasicPositionDeal positionDeal = positionDealDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPositionDealAsJSON(long id, String include){
		MESBasicPositionDeal positionDeal = positionDealDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(positionDeal, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.common.delete",operType="3")
	public void deletePositionDeal(MESBasicPositionDeal positionDeal){
		if(null != positionDeal && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPositionDeal.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
			AuditUtil.setColumnStringA(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
			AuditUtil.setColumnStringC(positionDeal.getId().toString());
		}
		beforeDeletePositionDeal(positionDeal);
		positionDealDao.deletePhysical(positionDeal);
		afterDeletePositionDeal(positionDeal);
		positionDealDao.flush();
		List<MESBasicPositionDeal> params = new ArrayList<MESBasicPositionDeal>();
		params.add(positionDeal);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.common.delete",operType="3")
	public void deletePositionDeal(List<Long> positionDealIds){
		deletePositionDeal(positionDealIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.common.delete",operType="3")
	public void deletePositionDeal(List<Long> positionDealIds, String eventTopic) {
		List<MESBasicPositionDeal> positionDeals = new ArrayList<MESBasicPositionDeal>();
		for(Long positionDealId : positionDealIds){
			MESBasicPositionDeal positionDeal = getPositionDeal(positionDealId);
			positionDeals.add(positionDeal);
			if(null != positionDeal && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPositionDeal.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == positionDeal.getId() ? "" : "," + positionDeal.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == positionDeal.getId() ? "" : "," + positionDeal.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == positionDeal.getId() ? "" : "," + positionDeal.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(positionDeals != null && positionDeals.size() > 0 ){
			for(MESBasicPositionDeal positionDeal : positionDeals){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicPositionDeal.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(positionDeal));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),positionDeal, positionDeal.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(positionDealIds != null && positionDealIds.size() > 0) {
			String hql = "delete " + MESBasicPositionDeal.JPA_NAME + " where id in(:ids)";
			Query query = positionDealDao.createQuery(hql);
			query.setParameterList("ids", positionDealIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicPositionDeal positionDealz : positionDeals){
				positionDealz.setValid(false);
				List<MESBasicPositionDeal> params = new ArrayList<MESBasicPositionDeal>();
				params.add(positionDealz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_factoryModel");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.common.delete",operType="3")
	public void deletePositionDeal(long positionDealId, int positionDealVersion){
		this.deletePositionDeal(positionDealId, positionDealVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.common.delete",operType="3")
	public void deletePositionDeal(long positionDealId, int positionDealVersion,SignatureLog signatureLog){
		MESBasicPositionDeal positionDeal = getPositionDeal(positionDealId);
		...
		beforeDeletePositionDeal(positionDeal);
		positionDealDao.delete(positionDealId, positionDealVersion);
		afterDeletePositionDeal(positionDeal);
		positionDealDao.flush();
		List<MESBasicPositionDeal> params = new ArrayList<MESBasicPositionDeal>();
		params.add(positionDeal);
		...
		if(signatureLog != null ) {
			...
			businessKey=positionDeal.getId();
			signatureLog.setTableId(positionDeal.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.common.delete",operType="3")
	public void deletePositionDeal(String positionDealIds){
		this.deletePositionDeal(positionDealIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.common.delete",operType="3")
	public void deletePositionDeal(String positionDealIds,SignatureLog signatureLog){
		deleteCollection(positionDealIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePositionDeal(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_factoryModel#modelCode:MESBasic_1_factoryModel_PositionDeal#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="ec.engine.restore",operType="10")
	public void restorePositionDeal(String positionDealIds){
		restoreCollection(positionDealIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePositionDeal(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="ec.engine.restore",operType="10")
	public void restorePositionDeal(long positionDealId){
		
		findBusinessKeyUsed(positionDealId);	//判断业务主键是否重复
		...
		beforeRestorePositionDeal(positionDeal);
		positionDeal.setValid(true);
		positionDealDao.update(positionDeal);
		afterRestorePositionDeal(positionDeal);
		List<MESBasicPositionDeal> params = new ArrayList<MESBasicPositionDeal>();
		params.add(positionDeal);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long positionDealId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_factoryModel_PositionDeal");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicPositionDeal.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicPositionDeal.TABLE_NAME +" where id = ? )";
			List<Object> list =  positionDealDao.createNativeQuery(sql,positionDealId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePositionDeal(List<MESBasicPositionDeal>  positionDeals){
		for(MESBasicPositionDeal positionDeal:positionDeals)  {
			savePositionDeal(positionDeal, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePositionDeal(List<MESBasicPositionDeal>  positionDeals){
		for(MESBasicPositionDeal positionDeal:positionDeals)  {
			savePositionDeal(positionDeal, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="ec.flow.save")
	public void savePositionDeal(MESBasicPositionDeal positionDeal, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePositionDeal(positionDeal, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="ec.flow.save")
	public void savePositionDeal(MESBasicPositionDeal positionDeal, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePositionDeal(positionDeal, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePositionDealCodes(MESBasicPositionDeal positionDeal) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePositionDealCodes(positionDeal, false);
	}
	
	public void generatePositionDealCodes(MESBasicPositionDeal positionDeal, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePositionDealSummarys(MESBasicPositionDeal positionDeal) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePositionDealSummarys(positionDeal, false);
	}

	public void generatePositionDealSummarys(MESBasicPositionDeal positionDeal, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal")
	public void savePositionDeal(MESBasicPositionDeal positionDeal,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePositionDeal(positionDeal, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal")
	public void savePositionDeal(MESBasicPositionDeal positionDeal,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(positionDeal.getId() != null && positionDeal.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPositionDeal.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, positionDeal);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(positionDeal);
		beforeSavePositionDeal(positionDeal, viewIsView);

		if (viewIsView) {
			positionDealDao.saveWithRevertVersion(positionDeal);
		} else {
			if(isNew)
				positionDealDao.save(positionDeal);
			else
				positionDealDao.merge(positionDeal);
		}
		if(null != positionDeal && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPositionDeal.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
			AuditUtil.setColumnStringA(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
		...
		dealDatagridsSave(positionDeal,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePositionDealCodes(positionDeal, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePositionDealSummarys(positionDeal, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePositionDeal(positionDeal, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, positionDeal);
			}
		}
		List<MESBasicPositionDeal> params = new ArrayList<MESBasicPositionDeal>();
		positionDealDao.flush();
		positionDealDao.clear();
		positionDeal = positionDealDao.load(positionDeal.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicPositionDeal positionDeal){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("positionDeal", positionDeal);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal")
	public void mergePositionDeal(MESBasicPositionDeal positionDeal, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(positionDeal);
		beforeSavePositionDeal(positionDeal);
		positionDealDao.merge(positionDeal);
		if(null != positionDeal && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPositionDeal.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
			AuditUtil.setColumnStringA(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
		...
		dealDatagridsSave(positionDeal,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePositionDeal(positionDeal);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicPositionDeal> findPositionDeals(Page<MESBasicPositionDeal> page, Criterion... criterions) {
		return positionDealDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPositionDeals(Page<MESBasicPositionDeal> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPositionDeals(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPositionDeals(Page<MESBasicPositionDeal> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPositionDeals(Page<MESBasicPositionDeal> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicPositionDeal> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"positionDeal\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"positionDeal\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"positionDeal\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  positionDealDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  positionDealDao.createNativeQuery(powerOperateSql, "MESBasic_1_factoryModel").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"positionDeal\".LAY_REC ASC, \"positionDeal\".SORT ASC");
				} else {
					orderPart.append(" \"positionDeal\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) positionDealDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=positionDealDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = positionDealDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicPositionDeal> result = new ArrayList<MESBasicPositionDeal>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicPositionDeal.class,positionDealDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicPositionDeal.class,positionDealDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicPositionDeal positionDeal : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(positionDeal.getId(), "MESBasic_factoryModel_positionDeal");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(positionDeal.getId(), "MESBasic_factoryModel_positionDeal");
							if(null != documents && !documents.isEmpty()) {
								positionDeal.setDocument(documents.get(0));
								positionDeal.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicPositionDeal> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicPositionDeal> result) {
		View view = null;
		...
		Map<Long, MESBasicPositionDeal> rsMap = new HashMap<Long, MESBasicPositionDeal>();
		for (MESBasicPositionDeal positionDeal : result) {
			ids.add(positionDeal.getId());
			rsMap.put(positionDeal.getId(), positionDeal);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(positionDealDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = positionDealDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_factoryModel_PositionDeal" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_factoryModel_PositionDeal" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  positionDealDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"positionDeal\"","MESBasic_1_factoryModel","","MESBASIC_POSITION_DEALS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"positionDeal\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"positionDeal\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"positionDeal\"","MESBasic_1_factoryModel","","MESBASIC_POSITION_DEALS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_factoryModel_PositionDeal" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("positionDeal.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"positionDeal\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"positionDeal\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"positionDeal\".CID = 1 OR \"positionDeal\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("positionDeal.mc\"").append(".POSITION_DEAL) not in ("+notinValue+")");
			}
			
			SQLQuery query = positionDealDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=positionDealDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicPositionDeal.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicPositionDeal.MODULE_CODE;}
	protected String getModelCode(){return MESBasicPositionDeal.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param positionDeal 模型实例
	 */
	public void checkUniqueConstraint(MESBasicPositionDeal positionDeal){
	}
	
	
	@Override
	public List<MESBasicPositionDeal> getPositionDeals(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPositionDeal.class);
		if(params!=null){
			...
			return positionDealDao.findByHql("from " + MESBasicPositionDeal.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return positionDealDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicPositionDeal> findPositionDealsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPositionDeal.class);
	...
	@Override
	public List<MESBasicPositionDeal> findPositionDealsByHql(String hql, Object... objects){
		
		return   positionDealDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicPositionDeal loadPositionDealByBussinessKey(MESBasicPositionDeal positionDeal){
		return loadPositionDealByBussinessKey(positionDeal.getId());
	}
	
	@Override
	public Page<MESBasicPositionDeal> getPositionDeals(Page<MESBasicPositionDeal> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPositionDeal.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = positionDealDao.findByPage(page, sql);
			}else{
				page = positionDealDao.findAllByPage(page);
			}
			return page;
		}
		page = positionDealDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPositionDeal loadPositionDealByBussinessKey(Serializable bussinessKey){
		return positionDealDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicPositionDeal> findByProperty(String propertyName, Object object){
		return positionDealDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPositionDeal findEntityByProperty(String propertyName, Object object){
		return positionDealDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicPositionDeal.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = positionDealDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_factoryModel_PositionDeal","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicPositionDeal positionDeal,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,positionDealDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicPositionDeal.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, positionDeal);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",positionDeal.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPositionDeal.TABLE_NAME +" where valid = 1";
			list = positionDealDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPositionDeal.TABLE_NAME;
			list = positionDealDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicPositionDeal.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = positionDealDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicPositionDealDaoImpl positionDealDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPositionDeal(final List<MESBasicPositionDeal> insertObjs, final List<MESBasicPositionDeal> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPositionDeal(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPositionDeal(final List<MESBasicPositionDeal> insertObjs, final List<MESBasicPositionDeal> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1460950790364"),importCount));
		...
		Map<Long, MESBasicPositionDeal> oldUpdateObjsMap = new HashMap<Long, MESBasicPositionDeal>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicPositionDeal bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicPositionDeal> bizObjList = positionDealDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicPositionDeal bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicPositionDeal> nmeObjs = new ArrayList<MESBasicPositionDeal>();
		...
		long id=autoGeneratorID.getNextId(MESBasicPositionDeal.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicPositionDeal positionDeal:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicPositionDeal.TABLE_NAME,1,dbId);
			if(positionDeal.getId() != null ){
				m.put(positionDeal.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) positionDealDao.createNativeQuery(sql, "MESBasic_1_factoryModel_PositionDeal"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		positionDealDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicPositionDeal.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicPositionDeal positionDeal = updateObjs.get(updateIndex);
						Long id = positionDeal.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = positionDeal.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(positionDeal);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicPositionDeal.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicPositionDeal.TABLE_NAME + " order by id desc";
//					Query query = positionDealDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicPositionDeal positionDeal = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = positionDeal.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		positionDealDaoImpl.getSession().flush();
		positionDealDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicPositionDeal bizObj = insertObjs.get(insertIndex);
			positionDealImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicPositionDeal bizObj = updateObjs.get(updateIndex);
			MESBasicPositionDeal oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			positionDealImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicPositionDeal> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicPositionDeal> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) positionDealDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicPositionDeal> result = getResult(page, query, new PendingResultTransformer(MESBasicPositionDeal.class,positionDealDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicPositionDeal model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = positionDealDao.createNativeQuery(sql, "MESBasic_1_factoryModel_PositionDeal").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = positionDealDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = positionDealDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = positionDealDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?positionDealDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  positionDealDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,positionDealDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="ec.flow.save")
	public void savePositionDeal(MESBasicPositionDeal positionDeal, DataGridService dataGridService){
		savePositionDeal(positionDeal, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="ec.flow.save")
	public void savePositionDeal(MESBasicPositionDeal positionDeal, DataGridService dataGridService, String viewCode){
		savePositionDeal(positionDeal, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal")
	public void savePositionDeal(MESBasicPositionDeal positionDeal, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePositionDeal(positionDeal, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal")
	public void savePositionDeal(MESBasicPositionDeal positionDeal, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(null != positionDeal.getId() && positionDeal.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPositionDeal.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal")
	public Map<Object, Object> positionDealDataGridImport(MESBasicPositionDeal positionDeal, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = positionDeal.getVirtualId();
		
		positionDeal = positionDealDao.load(positionDeal.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, positionDeal.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = positionDeal.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(positionDeal);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, positionDeal.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal")
	public void mergePositionDeal(MESBasicPositionDeal positionDeal, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(positionDeal);
		beforeSavePositionDeal(positionDeal);
		positionDealDao.merge(positionDeal);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_PositionDeal",desc="foundation.batch.save",operType="4")
	public void batchSavePositionDeal(MESBasicPositionDeal positionDeal, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
	...
	@Override
	public Page<MESBasicPositionDeal> getByPage(Page<MESBasicPositionDeal> page,DetachedCriteria detachedCriteria){
		return positionDealDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSavePositionDeal(MESBasicPositionDeal positionDeal, Object... objects){
		checkUniqueConstraint(positionDeal);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePositionDeal(MESBasicPositionDeal positionDeal, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPositionDeal(List<MESBasicPositionDeal> positionDeals, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPositionDeal(List<MESBasicPositionDeal> positionDeals){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPositionDeal(List<MESBasicPositionDeal> positionDeals, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPositionDeal(List<MESBasicPositionDeal> positionDeals){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePositionDeal(MESBasicPositionDeal positionDeal){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePositionDeal(MESBasicPositionDeal positionDeal){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePositionDeal(MESBasicPositionDeal positionDeal){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePositionDeal(MESBasicPositionDeal positionDeal){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicPositionDeal positionDeal, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicPositionDeal positionDeal, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPositionDeal(List<MESBasicPositionDeal> insertObjs, List<MESBasicPositionDeal> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPositionDeal,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPositionDeal(List<MESBasicPositionDeal> insertObjs, List<MESBasicPositionDeal> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPositionDeal,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
  @Override
	public void addPositionDeal(Long bapposition) {
		String sql = "select max(PIMS_POSTION) from " + MESBasicPositionDeal.TABLE_NAME;
		Number maxId = (Number) positionDealDao.createNativeQuery(sql).uniqueResult();
		if(null == maxId){
			maxId = 0;
		}
		MESBasicPositionDeal position = new MESBasicPositionDeal();
		position.setBapPosition(bapposition);
		position.setPimsPostion(maxId.longValue() + 1);
		positionDealDao.save(position);
	}

	@Override
	public void deletePosition(Long position) {
		String sql = "update " + MESBasicPositionDeal.JPA_NAME + " set valid=false where bapPosition=?" ;
		positionDealDao.bulkExecute(sql, position);
	}
/* CUSTOM CODE END */
}
