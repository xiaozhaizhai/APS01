...
@Repository
public class MESBasicPurchGroupDaoImpl extends ExtendedGenericDaoImpl<MESBasicPurchGroup, Long> implements MESBasicPurchGroupDao {
	@Override
	...
	@Override
	public MESBasicPurchGroupDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicPurchGroupDealInfo dealInfo = new MESBasicPurchGroupDealInfo();
	...
	@Override
	public MESBasicPurchGroupDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicPurchGroup mainObj, Staff dealStaff) {
		MESBasicPurchGroupDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicPurchGroupDealInfo.class);
		...
		List<MESBasicPurchGroupDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
