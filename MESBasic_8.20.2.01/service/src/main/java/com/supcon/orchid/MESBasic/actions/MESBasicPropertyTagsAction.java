...
/* CUSTOM CODE START(action,import,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicPropertyTagsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicPropertyTags> {
	...
	private MESBasicPropertyTags propertyTags;
	...
	//导出excel时,主模型内容
	List<MESBasicPropertyTags> queryResults = new ArrayList<MESBasicPropertyTags>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/propertyTags/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/propertyTags/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_container_PropertyTags" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicPropertyTags> propertyTagss;
	
	public List<MESBasicPropertyTags> getPropertyTagss() {
		return propertyTagss;
	}
	...
	private Page<MESBasicPropertyTags> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			propertyTags = new MESBasicPropertyTags();
		} else {
			if(id > 0){
				propertyTags = propertyTagsService.getPropertyTags(id, viewCode);
			}
		}
	...
	public Page<MESBasicPropertyTags> getPage() {
		if (null == page) {
			page = new Page<MESBasicPropertyTags>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicPropertyTags getPropertyTags() {
		return propertyTags;
	}		

	public void setPropertyTags(MESBasicPropertyTags propertyTags) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/propertyTags/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = propertyTagsService.getPropertyTagsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/propertyTags/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.radion1415869369969
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String propertyTagsReferenceCopy() throws Exception{
			MESBasicPropertyTags oldPropertyTags = null;
			if(isEditNew()){
				...
				oldPropertyTags = new MESBasicPropertyTags();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,propertyTags.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, propertyTags.getId());
				...
				oldPropertyTags = propertyTagsService.getPropertyTags(id);
			}
			
			
			propertyTags = propertyTagsService.getPropertyTags(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_container_PropertyTags");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicPropertyTags.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPropertyTags, null);
						Method setValue = MESBasicPropertyTags.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(propertyTags, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			propertyTags.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_container_PropertyTags", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=propertyTagsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return propertyTagsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			propertyTags = propertyTagsService.getPropertyTags(this.propertyTags.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,propertyTags.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = propertyTagsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(propertyTagsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(propertyTagsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_container_PropertyTags", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"propertyTags\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"propertyTags\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"propertyTags\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"propertyTags\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			propertyTagsService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		propertyTagsService.findDataGridPage(dg,dgclass,getDgPage(),propertyTags,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(propertyTags.getId() != null && propertyTags.getId() != -1){
				propertyTags = propertyTagsService.getPropertyTags(propertyTags.getId());
			}
			propertyTagsService.findDataGridPage(dg,dgclass,getDgPage(),propertyTags,searchCondition,params);
	...
	private void save() throws Exception {
		...
		propertyTagsService.savePropertyTags(propertyTags,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_PropertyTags", "submit");
		bap_validate_datagrids=propertyTagsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", propertyTags.getId());
		...
		fileuploads.put("linkId", propertyTags.getId());
		bapAfterCustomMethod("MESBasic_1_container_PropertyTags", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/container/propertyTags/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_PropertyTags", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		propertyTagsService.deletePropertyTags(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_container_PropertyTags", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/propertyTags/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_PropertyTags", "restore");
		propertyTagsService.restorePropertyTags(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_container_PropertyTags", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicPropertyTags> findExportDataInfos(Page<MESBasicPropertyTags> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = propertyTagsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = propertyTagsService.getAssProperty(p.getCode());	
									//String assModelCode = propertyTagsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			propertyTagsService.findPropertyTagss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			propertyTagsService.findPropertyTagss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicPropertyTags> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicPropertyTags result : results){
	...
	public boolean isEditNew(){
		//return null == this.propertyTags || null == this.propertyTags.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/container/propertyTags/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_PropertyTags", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_container_PropertyTags", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/container/propertyTags/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_PropertyTags", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/container/propertyTags/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/propertyTags/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicPropertyTagsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_container_PropertyTags", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_container_PropertyTags", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/propertyTags/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_container_PropertyTags", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_container_PropertyTags", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/container/propertyTags/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_PropertyTags", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/container/propertyTags/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_container_PropertyTags";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_container_PropertyTags", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/container/propertyTags/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
		_auditLog.setColumnStringA(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
		_auditLog.setColumnStringC(propertyTags.getId().toString());
		_auditLog.setColumnStringD(MESBasicPropertyTags.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicPropertyTagsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, propertyTags,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, propertyTags,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,propertyTags);
		...
		printlog.setBusinessId(propertyTags.getId());
		printlog.setBussinessKey(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
		printlog.setMainDisplay(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/container/propertyTags/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(propertyTags,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/propertyTags/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(propertyTagsService.getPropertyTags(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/container/propertyTags/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/container/propertyTags/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, propertyTags,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				propertyTags = propertyTagsService.getPropertyTags(Long.parseLong(arrId[i]));
				if(null != propertyTags){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == propertyTags.getId() ? "" : "," + propertyTags.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == propertyTags.getId() ? "" : "," + propertyTags.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == propertyTags.getId() ? "" : "," + propertyTags.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, propertyTags,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,propertyTags);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/container/propertyTags/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_container_PropertyTags");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_container_PropertyTags")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_container_PropertyTags");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_container_PropertyTags");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = propertyTagsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
