...
import com.supcon.orchid.MESBasic.services.MESBasicCommDealInfoImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicCommDealInfoDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_commDealInfoService")
@Transactional
public class MESBasicCommDealInfoServiceImpl  extends BaseServiceImpl<MESBasicCommDealInfo> implements MESBasicCommDealInfoService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicCommDealInfo> bapBaseService;
	...
	@Autowired
	private MESBasicCommDealInfoImportService commDealInfoImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicCommDealInfo.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  commDealInfoDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicCommDealInfo.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = commDealInfoDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicCommDealInfo.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicCommDealInfo.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  commDealInfoDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicCommDealInfo.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicCommDealInfo.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicCommDealInfo> list = (List<MESBasicCommDealInfo>) commDealInfoDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicCommDealInfo getCommDealInfo(long id){
		return getCommDealInfo(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicCommDealInfo getCommDealInfo(long id, String viewCode){
		MESBasicCommDealInfo commDealInfo = commDealInfoDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getCommDealInfoAsJSON(long id, String include){
		MESBasicCommDealInfo commDealInfo = commDealInfoDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(commDealInfo, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteCommDealInfo(MESBasicCommDealInfo commDealInfo){
		if(null != commDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCommDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
			AuditUtil.setColumnStringC(commDealInfo.getId().toString());
		}
		beforeDeleteCommDealInfo(commDealInfo);
		commDealInfoDao.deletePhysical(commDealInfo);
		afterDeleteCommDealInfo(commDealInfo);
		commDealInfoDao.flush();
		List<MESBasicCommDealInfo> params = new ArrayList<MESBasicCommDealInfo>();
		params.add(commDealInfo);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteCommDealInfo(List<Long> commDealInfoIds){
		deleteCommDealInfo(commDealInfoIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteCommDealInfo(List<Long> commDealInfoIds, String eventTopic) {
		List<MESBasicCommDealInfo> commDealInfos = new ArrayList<MESBasicCommDealInfo>();
		for(Long commDealInfoId : commDealInfoIds){
			MESBasicCommDealInfo commDealInfo = getCommDealInfo(commDealInfoId);
			commDealInfos.add(commDealInfo);
			if(null != commDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCommDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == commDealInfo.getId() ? "" : "," + commDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == commDealInfo.getId() ? "" : "," + commDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == commDealInfo.getId() ? "" : "," + commDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
				}
			}
		}
		
		for(MESBasicCommDealInfo commDealInfo : commDealInfos){
			beforeDeleteCommDealInfo(commDealInfo);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(commDealInfos != null && commDealInfos.size() > 0 ){
			for(MESBasicCommDealInfo commDealInfo : commDealInfos){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicCommDealInfo.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(commDealInfo));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),commDealInfo, commDealInfo.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(commDealInfoIds != null && commDealInfoIds.size() > 0) {
			String hql = "delete " + MESBasicCommDealInfo.JPA_NAME + " where id in(:ids)";
			Query query = commDealInfoDao.createQuery(hql);
			query.setParameterList("ids", commDealInfoIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicCommDealInfo commDealInfoz : commDealInfos){
				commDealInfoz.setValid(false);
				List<MESBasicCommDealInfo> params = new ArrayList<MESBasicCommDealInfo>();
				params.add(commDealInfoz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_product");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteCommDealInfo(long commDealInfoId, int commDealInfoVersion){
		this.deleteCommDealInfo(commDealInfoId, commDealInfoVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteCommDealInfo(long commDealInfoId, int commDealInfoVersion,SignatureLog signatureLog){
		MESBasicCommDealInfo commDealInfo = getCommDealInfo(commDealInfoId);
		...
		beforeDeleteCommDealInfo(commDealInfo);
		commDealInfoDao.delete(commDealInfoId, commDealInfoVersion);
		afterDeleteCommDealInfo(commDealInfo);
		commDealInfoDao.flush();
		List<MESBasicCommDealInfo> params = new ArrayList<MESBasicCommDealInfo>();
		params.add(commDealInfo);
		...
		if(signatureLog != null ) {
			...
			businessKey=commDealInfo.getId();
			signatureLog.setTableId(commDealInfo.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteCommDealInfo(String commDealInfoIds){
		this.deleteCommDealInfo(commDealInfoIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteCommDealInfo(String commDealInfoIds,SignatureLog signatureLog){
		deleteCollection(commDealInfoIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteCommDealInfo(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_product#modelCode:MESBasic_1_product_CommDealInfo#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="ec.engine.restore",operType="10")
	public void restoreCommDealInfo(String commDealInfoIds){
		restoreCollection(commDealInfoIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreCommDealInfo(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="ec.engine.restore",operType="10")
	public void restoreCommDealInfo(long commDealInfoId){
		
		findBusinessKeyUsed(commDealInfoId);	//判断业务主键是否重复
		...
		beforeRestoreCommDealInfo(commDealInfo);
		commDealInfo.setValid(true);
		commDealInfoDao.update(commDealInfo);
		afterRestoreCommDealInfo(commDealInfo);
		List<MESBasicCommDealInfo> params = new ArrayList<MESBasicCommDealInfo>();
		params.add(commDealInfo);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long commDealInfoId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_product_CommDealInfo");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicCommDealInfo.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicCommDealInfo.TABLE_NAME +" where id = ? )";
			List<Object> list =  commDealInfoDao.createNativeQuery(sql,commDealInfoId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseCommDealInfo(List<MESBasicCommDealInfo>  commDealInfos){
		for(MESBasicCommDealInfo commDealInfo:commDealInfos)  {
			saveCommDealInfo(commDealInfo, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseCommDealInfo(List<MESBasicCommDealInfo>  commDealInfos){
		for(MESBasicCommDealInfo commDealInfo:commDealInfos)  {
			saveCommDealInfo(commDealInfo, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="ec.flow.save")
	public void saveCommDealInfo(MESBasicCommDealInfo commDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveCommDealInfo(commDealInfo, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="ec.flow.save")
	public void saveCommDealInfo(MESBasicCommDealInfo commDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveCommDealInfo(commDealInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateCommDealInfoCodes(MESBasicCommDealInfo commDealInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCommDealInfoCodes(commDealInfo, false);
	}
	
	public void generateCommDealInfoCodes(MESBasicCommDealInfo commDealInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateCommDealInfoSummarys(MESBasicCommDealInfo commDealInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCommDealInfoSummarys(commDealInfo, false);
	}

	public void generateCommDealInfoSummarys(MESBasicCommDealInfo commDealInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo")
	public void saveCommDealInfo(MESBasicCommDealInfo commDealInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveCommDealInfo(commDealInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo")
	public void saveCommDealInfo(MESBasicCommDealInfo commDealInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(commDealInfo.getId() != null && commDealInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCommDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, commDealInfo);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(commDealInfo);
		beforeSaveCommDealInfo(commDealInfo, viewIsView);

		if (viewIsView) {
			commDealInfoDao.saveWithRevertVersion(commDealInfo);
		} else {
			if(isNew)
				commDealInfoDao.save(commDealInfo);
			else
				commDealInfoDao.merge(commDealInfo);
		}
		if(null != commDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCommDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
		...
		dealDatagridsSave(commDealInfo,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateCommDealInfoCodes(commDealInfo, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateCommDealInfoSummarys(commDealInfo, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveCommDealInfo(commDealInfo, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, commDealInfo);
			}
		}
		List<MESBasicCommDealInfo> params = new ArrayList<MESBasicCommDealInfo>();
		commDealInfoDao.flush();
		commDealInfoDao.clear();
		commDealInfo = commDealInfoDao.load(commDealInfo.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicCommDealInfo commDealInfo){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("commDealInfo", commDealInfo);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo")
	public void mergeCommDealInfo(MESBasicCommDealInfo commDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(commDealInfo);
		beforeSaveCommDealInfo(commDealInfo);
		commDealInfoDao.merge(commDealInfo);
		if(null != commDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCommDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == commDealInfo.getId() ? "" : commDealInfo.getId().toString());
		...
		dealDatagridsSave(commDealInfo,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveCommDealInfo(commDealInfo);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicCommDealInfo> findCommDealInfos(Page<MESBasicCommDealInfo> page, Criterion... criterions) {
		return commDealInfoDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCommDealInfos(Page<MESBasicCommDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findCommDealInfos(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCommDealInfos(Page<MESBasicCommDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCommDealInfos(Page<MESBasicCommDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicCommDealInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"commDealInfo\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"commDealInfo\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"commDealInfo\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"commDealInfo\".DEALINFO_ASSIGNID IN ( SELECT PRODUCT_ID FROM S2BASE_PRODUCT \"product\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  commDealInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  commDealInfoDao.createNativeQuery(powerOperateSql, "MESBasic_1_product").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"commDealInfo\".LAY_REC ASC, \"commDealInfo\".SORT ASC");
				} else {
					orderPart.append(" \"commDealInfo\".DEALINFO_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) commDealInfoDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=commDealInfoDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = commDealInfoDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicCommDealInfo> result = new ArrayList<MESBasicCommDealInfo>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicCommDealInfo.class,commDealInfoDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicCommDealInfo.class,commDealInfoDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicCommDealInfo commDealInfo : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(commDealInfo.getId(), "MESBasic_product_commDealInfo");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(commDealInfo.getId(), "MESBasic_product_commDealInfo");
							if(null != documents && !documents.isEmpty()) {
								commDealInfo.setDocument(documents.get(0));
								commDealInfo.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicCommDealInfo> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicCommDealInfo> result) {
		View view = null;
		...
		Map<Long, MESBasicCommDealInfo> rsMap = new HashMap<Long, MESBasicCommDealInfo>();
		for (MESBasicCommDealInfo commDealInfo : result) {
			ids.add(commDealInfo.getId());
			rsMap.put(commDealInfo.getId(), commDealInfo);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(commDealInfoDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = commDealInfoDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_product_CommDealInfo" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_product_CommDealInfo" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  commDealInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"commDealInfo\"","MESBasic_1_product","","COMM_DEALINFO",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"commDealInfo\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"commDealInfo\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"commDealInfo\"","MESBasic_1_product","","COMM_DEALINFO",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_product_CommDealInfo" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("commDealInfo.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"commDealInfo\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"commDealInfo\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"commDealInfo\".CID = 1 OR \"commDealInfo\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("commDealInfo.mc\"").append(".COMM_DEAL_INFO) not in ("+notinValue+")");
			}
			
			SQLQuery query = commDealInfoDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=commDealInfoDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicCommDealInfo.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicCommDealInfo.MODULE_CODE;}
	protected String getModelCode(){return MESBasicCommDealInfo.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param commDealInfo 模型实例
	 */
	public void checkUniqueConstraint(MESBasicCommDealInfo commDealInfo){
	}
	
	
	@Override
	public List<MESBasicCommDealInfo> getCommDealInfos(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicCommDealInfo.class);
		if(params!=null){
			...
			return commDealInfoDao.findByHql("from " + MESBasicCommDealInfo.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return commDealInfoDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicCommDealInfo> findCommDealInfosBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicCommDealInfo.class);
	...
	@Override
	public List<MESBasicCommDealInfo> findCommDealInfosByHql(String hql, Object... objects){
		
		return   commDealInfoDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicCommDealInfo loadCommDealInfoByBussinessKey(MESBasicCommDealInfo commDealInfo){
		return loadCommDealInfoByBussinessKey(commDealInfo.getId());
	}
	
	@Override
	public Page<MESBasicCommDealInfo> getCommDealInfos(Page<MESBasicCommDealInfo> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicCommDealInfo.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = commDealInfoDao.findByPage(page, sql);
			}else{
				page = commDealInfoDao.findAllByPage(page);
			}
			return page;
		}
		page = commDealInfoDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicCommDealInfo loadCommDealInfoByBussinessKey(Serializable bussinessKey){
		return commDealInfoDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicCommDealInfo> findByProperty(String propertyName, Object object){
		return commDealInfoDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicCommDealInfo findEntityByProperty(String propertyName, Object object){
		return commDealInfoDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicCommDealInfo.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = commDealInfoDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_product_CommDealInfo","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicCommDealInfo commDealInfo,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,commDealInfoDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicCommDealInfo.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, commDealInfo);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",commDealInfo.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicCommDealInfo.TABLE_NAME +" where valid = 1";
			list = commDealInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicCommDealInfo.TABLE_NAME;
			list = commDealInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicCommDealInfo.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = commDealInfoDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicCommDealInfoDaoImpl commDealInfoDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchCommDealInfo(final List<MESBasicCommDealInfo> insertObjs, final List<MESBasicCommDealInfo> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchCommDealInfo(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchCommDealInfo(final List<MESBasicCommDealInfo> insertObjs, final List<MESBasicCommDealInfo> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1471524614104"),importCount));
		...
		Map<Long, MESBasicCommDealInfo> oldUpdateObjsMap = new HashMap<Long, MESBasicCommDealInfo>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicCommDealInfo bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicCommDealInfo> bizObjList = commDealInfoDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicCommDealInfo bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicCommDealInfo> nmeObjs = new ArrayList<MESBasicCommDealInfo>();
		...
		long id=autoGeneratorID.getNextId(MESBasicCommDealInfo.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicCommDealInfo commDealInfo:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicCommDealInfo.TABLE_NAME,1,dbId);
			if(commDealInfo.getId() != null ){
				m.put(commDealInfo.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			commDealInfo.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) commDealInfoDao.createNativeQuery(sql, "MESBasic_1_product_CommDealInfo"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		commDealInfoDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicCommDealInfo.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicCommDealInfo commDealInfo = updateObjs.get(updateIndex);
						Long id = commDealInfo.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = commDealInfo.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(commDealInfo);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicCommDealInfo.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicCommDealInfo.TABLE_NAME + " order by id desc";
//					Query query = commDealInfoDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicCommDealInfo commDealInfo = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = commDealInfo.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		commDealInfoDaoImpl.getSession().flush();
		commDealInfoDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicCommDealInfo bizObj = insertObjs.get(insertIndex);
			commDealInfoImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicCommDealInfo bizObj = updateObjs.get(updateIndex);
			MESBasicCommDealInfo oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			commDealInfoImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicCommDealInfo> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicCommDealInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) commDealInfoDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicCommDealInfo> result = getResult(page, query, new PendingResultTransformer(MESBasicCommDealInfo.class,commDealInfoDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicCommDealInfo model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = commDealInfoDao.createNativeQuery(sql, "MESBasic_1_product_CommDealInfo").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = commDealInfoDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = commDealInfoDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = commDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?commDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  commDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,commDealInfoDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="ec.flow.save")
	public void saveCommDealInfo(MESBasicCommDealInfo commDealInfo, DataGridService dataGridService){
		saveCommDealInfo(commDealInfo, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="ec.flow.save")
	public void saveCommDealInfo(MESBasicCommDealInfo commDealInfo, DataGridService dataGridService, String viewCode){
		saveCommDealInfo(commDealInfo, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo")
	public void saveCommDealInfo(MESBasicCommDealInfo commDealInfo, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveCommDealInfo(commDealInfo, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo")
	public void saveCommDealInfo(MESBasicCommDealInfo commDealInfo, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(null != commDealInfo.getId() && commDealInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicCommDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo")
	public Map<Object, Object> commDealInfoDataGridImport(MESBasicCommDealInfo commDealInfo, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = commDealInfo.getVirtualId();
		
		commDealInfo = commDealInfoDao.load(commDealInfo.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, commDealInfo.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = commDealInfo.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(commDealInfo);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, commDealInfo.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo")
	public void mergeCommDealInfo(MESBasicCommDealInfo commDealInfo, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(commDealInfo);
		beforeSaveCommDealInfo(commDealInfo);
		commDealInfoDao.merge(commDealInfo);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_CommDealInfo",desc="foundation.batch.save",operType="4")
	public void batchSaveCommDealInfo(MESBasicCommDealInfo commDealInfo, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
	...
	@Override
	public Page<MESBasicCommDealInfo> getByPage(Page<MESBasicCommDealInfo> page,DetachedCriteria detachedCriteria){
		return commDealInfoDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveCommDealInfo(MESBasicCommDealInfo commDealInfo, Object... objects){
		checkUniqueConstraint(commDealInfo);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveCommDealInfo(MESBasicCommDealInfo commDealInfo, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportCommDealInfo(List<MESBasicCommDealInfo> commDealInfos, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportCommDealInfo(List<MESBasicCommDealInfo> commDealInfos){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportCommDealInfo(List<MESBasicCommDealInfo> commDealInfos, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportCommDealInfo(List<MESBasicCommDealInfo> commDealInfos){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteCommDealInfo(MESBasicCommDealInfo commDealInfo){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteCommDealInfo(MESBasicCommDealInfo commDealInfo){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreCommDealInfo(MESBasicCommDealInfo commDealInfo){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreCommDealInfo(MESBasicCommDealInfo commDealInfo){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicCommDealInfo commDealInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicCommDealInfo commDealInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportCommDealInfo(List<MESBasicCommDealInfo> insertObjs, List<MESBasicCommDealInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportCommDealInfo,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportCommDealInfo(List<MESBasicCommDealInfo> insertObjs, List<MESBasicCommDealInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportCommDealInfo,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_product_CommDealInfo,MESBasic_1) */
// 自定义代码
