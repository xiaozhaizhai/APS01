...
/* CUSTOM CODE START(action,import,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
...
public class MESBasicTankModelAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicTankModel> {
	...
	//导出excel时,主模型内容
	List<MESBasicTankModel> queryResults = new ArrayList<MESBasicTankModel>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/tankModel/tankModel/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/tankModel/tankModel/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_tankModel_TankModel" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicTankModel> tankModels;
	
	public List<MESBasicTankModel> getTankModels() {
		return tankModels;
	}
	...
	private Page<MESBasicTankModel> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			tankModel = new MESBasicTankModel();
			if(tankModel.getShape() != null) {
				tankModel.setShape((SystemCode) systemCodeService.getSystemCode(tankModel.getShape().getId()));
			}
		} else {
			if(id > 0){
				tankModel = tankModelService.getTankModel(id, viewCode);
			}
		}
	...
	public Page<MESBasicTankModel> getPage() {
		if (null == page) {
			page = new Page<MESBasicTankModel>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tankModel/tankModel/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = tankModelService.getTankModelAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tankModel/tankModel/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String tankModelReferenceCopy() throws Exception{
			MESBasicTankModel oldTankModel = null;
			if(isEditNew()){
				...
				oldTankModel = new MESBasicTankModel();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,tankModel.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, tankModel.getId());
				...
				oldTankModel = tankModelService.getTankModel(id);
			}
			
			
			tankModel = tankModelService.getTankModel(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_tankModel_TankModel");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicTankModel.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTankModel, null);
						Method setValue = MESBasicTankModel.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(tankModel, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			tankModel.setCreateDepartmentId(oldTankModel.getCreateDepartmentId());
			tankModel.setCreatePositionId(oldTankModel.getCreatePositionId());
			tankModel.setCreateStaffId(oldTankModel.getCreateStaffId());
			tankModel.setOwnerDepartmentId(oldTankModel.getOwnerDepartmentId());
			tankModel.setOwnerPositionId(oldTankModel.getOwnerPositionId());
			tankModel.setOwnerStaffId(oldTankModel.getOwnerStaffId());
			tankModel.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_tankModel_TankModel", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/tankModel/tankModel/modelEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "modelEdit.ftl") }),
		@Action(value = "/MESBasic/tankModel/tankModel/modelEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "modelEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=tankModelService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return tankModelReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			tankModel = tankModelService.getTankModel(this.tankModel.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,tankModel.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/tankModel/tankModel/modelView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "modelView.ftl") }),
		@Action(value = "/MESBasic/tankModel/tankModel/modelViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "modelViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/tankModel/tankModel/modelList", results = { @Result(type = FREEMARKER, location = "modelList.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/tankModel/tankModel/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = tankModelService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(tankModelService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(tankModelService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_tankModel_TankModel", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/tankModel/tankModel/modelRef", results = { @Result(type = FREEMARKER, location = "modelRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/tankModel/tankModel/modelList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.shape.id,result.shape.value,result.volume,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="modelList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"tankModel\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"tankModel\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"tankModel\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"tankModel\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/tankModel/tankModel/modelList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/tankModel/tankModel/modelRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.shape.id,result.shape.value,result.volume,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="modelRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String viewTreeDrag() throws Exception{
		tankModelService.drag(treecid, pid, cindex);
		return null;
	}
	...
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"tankModel\".", "{alias}.");
					}
		...
		tankModels = tankModelService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			tankModels = tankModelService.getTreeInfo(treeInfo, null, null);
		}
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/tankModel/tankModel/allTempleteDownload", results = { @Result(name="excel",type="excel",location="modelList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_tankModel_TankModel");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		tankModelService.findDataGridPage(dg,dgclass,getDgPage(),tankModel,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(tankModel.getId() != null && tankModel.getId() != -1){
				tankModel = tankModelService.getTankModel(tankModel.getId());
			}
			tankModelService.findDataGridPage(dg,dgclass,getDgPage(),tankModel,searchCondition,params);
	...
	private void save() throws Exception {
		...
		tankModelService.saveTankModel(tankModel,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/tankModel/tankModel/modelEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/tankModel/tankModel/modelView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "submit");
		bap_validate_datagrids=tankModelService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", tankModel.getId());
		...
		fileuploads.put("linkId", tankModel.getId());
		bapAfterCustomMethod("MESBasic_1_tankModel_TankModel", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/tankModel/tankModel/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		tankModelService.deleteTankModel(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_tankModel_TankModel", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tankModel/tankModel/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "restore");
		tankModelService.restoreTankModel(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_tankModel_TankModel", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/tankModel/tankModel/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/tankModel/tankModel/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "showDealInfoData");
		if (null != tableInfoId) {
			int count=tankModelService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = tankModelService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = tankModelService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_tankModel_TankModel", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicTankModel> findExportDataInfos(Page<MESBasicTankModel> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = tankModelService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = tankModelService.getAssProperty(p.getCode());	
									//String assModelCode = tankModelService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			tankModelService.findTankModels(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			tankModelService.findTankModels(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicTankModel> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicTankModel result : results){
	...
	public boolean isEditNew(){
		//return null == this.tankModel || null == this.tankModel.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/tankModel/tankModel/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_tankModel_TankModel", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/tankModel/tankModel/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/tankModel/tankModel/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tankModel/tankModel/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicTankModelService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_tankModel", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_tankModel");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_tankModel_TankModel", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tankModel/tankModel/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_tankModel_TankModel", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_tankModel_TankModel", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/tankModel/tankModel/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/tankModel/tankModel/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_tankModel_TankModel";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_tankModel_TankModel", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/tankModel/tankModel/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicTankModel tankModel = null;
		if(null != id) {
			tankModel = tankModelService.getTankModel(id);
		}
		...
		_auditLog.setColumnStringB(null == tankModel.getCode() ? "" : tankModel.getCode().toString());
		_auditLog.setColumnStringA(null == tankModel.getName() ? "" : tankModel.getName().toString());
		_auditLog.setColumnStringC(tankModel.getId().toString());
		_auditLog.setColumnStringD(MESBasicTankModel.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_tankModel");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicTankModelAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, tankModel,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, tankModel,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,tankModel);
		...
		printlog.setEntity("MESBasic_1_tankModel");
		...
		printlog.setBusinessId(tankModel.getId());
		printlog.setBussinessKey(null == tankModel.getCode() ? "" : tankModel.getCode().toString());
		printlog.setMainDisplay(null == tankModel.getName() ? "" : tankModel.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/tankModel/tankModel/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(tankModel,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tankModel/tankModel/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(tankModelService.getTankModel(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/tankModel/tankModel/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/tankModel/tankModel/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, tankModel,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				tankModel = tankModelService.getTankModel(Long.parseLong(arrId[i]));
				if(null != tankModel){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == tankModel.getName() ? "" : "," + tankModel.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == tankModel.getName() ? "" : tankModel.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == tankModel.getCode() ? "" : "," + tankModel.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == tankModel.getCode() ? "" : tankModel.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == tankModel.getId() ? "" : "," + tankModel.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == tankModel.getId() ? "" : tankModel.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, tankModel,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,tankModel);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/tankModel/tankModel/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicTankModel> findExportPageInfos(Page<MESBasicTankModel> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_tankModel" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = tankModelService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicTankModel queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = tankModelService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = tankModelService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_tankModel_TankModel");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_tankModel_TankModel")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_tankModel_TankModel");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_tankModel_TankModel");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = tankModelService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
		if("MESBasic_1_tankModel_TankModel".equals(code) && "submit".equals(methodName)){
			String f = tankModel.getVolume();
			if(StringUtil.isNotEmptyOrNull(f)){
				f = f.replaceAll("n", "1");
				try{
					Calculator.scalculateJS(f);
				}catch(Exception ex){
					throw new BAPException("储量计算公式设置错误，请检查。");
				}
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
