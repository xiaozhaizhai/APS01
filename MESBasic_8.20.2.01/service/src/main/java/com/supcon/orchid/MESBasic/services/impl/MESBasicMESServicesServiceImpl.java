...
import com.supcon.orchid.MESBasic.services.MESBasicMESServicesImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicMESServicesDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
...
@Service("mESBasic_mESServicesService")
@Transactional
public class MESBasicMESServicesServiceImpl  extends BaseServiceImpl<MESBasicMESServices> implements MESBasicMESServicesService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicMESServices> bapBaseService;
	...
	@Autowired
	private MESBasicMESServicesImportService mESServicesImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicMESServices.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  mESServicesDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicMESServices.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = mESServicesDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicMESServices.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicMESServices.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  mESServicesDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicMESServices.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicMESServices.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicMESServices> list = (List<MESBasicMESServices>) mESServicesDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicMESServices getMESServices(long id){
		return getMESServices(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicMESServices getMESServices(long id, String viewCode){
		MESBasicMESServices mESServices = mESServicesDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMESServicesAsJSON(long id, String include){
		MESBasicMESServices mESServices = mESServicesDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(mESServices, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.common.delete",operType="3")
	public void deleteMESServices(MESBasicMESServices mESServices){
		if(null != mESServices && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicMESServices.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mESServices.getServiceCode() ? "" : mESServices.getServiceCode().toString());
			AuditUtil.setColumnStringA(null == mESServices.getServiceName() ? "" : mESServices.getServiceName().toString());
			AuditUtil.setColumnStringC(mESServices.getId().toString());
		}
		beforeDeleteMESServices(mESServices);
		mESServicesDao.delete(mESServices);
		afterDeleteMESServices(mESServices);
		mESServicesDao.flush();
		List<MESBasicMESServices> params = new ArrayList<MESBasicMESServices>();
		List<MESBasicServiceLogger> serviceLoggerList = serviceLoggerDao.findByCriteria(Restrictions.eq("mesService", mESServices));
		mESServices.setServiceLoggerList(serviceLoggerList);
		params.add(mESServices);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.common.delete",operType="3")
	public void deleteMESServices(List<Long> mESServicesIds){
		deleteMESServices(mESServicesIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.common.delete",operType="3")
	public void deleteMESServices(List<Long> mESServicesIds, String eventTopic) {
		List<MESBasicMESServices> mESServicess = new ArrayList<MESBasicMESServices>();
		for(Long mESServicesId : mESServicesIds){
			MESBasicMESServices mESServices = getMESServices(mESServicesId);
			mESServicess.add(mESServices);
			if(null != mESServices && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicMESServices.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == mESServices.getServiceName() ? "" : "," + mESServices.getServiceName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == mESServices.getServiceName() ? "" : mESServices.getServiceName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == mESServices.getServiceCode() ? "" : "," + mESServices.getServiceCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == mESServices.getServiceCode() ? "" : mESServices.getServiceCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == mESServices.getId() ? "" : "," + mESServices.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == mESServices.getId() ? "" : mESServices.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(mESServicess != null && mESServicess.size() > 0 ){
			for(MESBasicMESServices mESServices : mESServicess){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),mESServices, mESServices.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(mESServicesIds != null && mESServicesIds.size() > 0) {
			String hql = "update " + MESBasicMESServices.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = mESServicesDao.createQuery(hql);
			query.setParameterList("ids", mESServicesIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicMESServices mESServicesz : mESServicess){
				mESServicesz.setValid(false);
				List<MESBasicMESServices> params = new ArrayList<MESBasicMESServices>();
				List<MESBasicServiceLogger> serviceLoggerList = serviceLoggerDao.findByCriteria(Restrictions.eq("mesService", mESServicesz));
				mESServicesz.setServiceLoggerList(serviceLoggerList);
				params.add(mESServicesz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.common.delete",operType="3")
	public void deleteMESServices(long mESServicesId, int mESServicesVersion){
		this.deleteMESServices(mESServicesId, mESServicesVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.common.delete",operType="3")
	public void deleteMESServices(long mESServicesId, int mESServicesVersion,SignatureLog signatureLog){
		MESBasicMESServices mESServices = getMESServices(mESServicesId);
		...
		beforeDeleteMESServices(mESServices);
		mESServicesDao.delete(mESServicesId, mESServicesVersion);
		afterDeleteMESServices(mESServices);
		mESServicesDao.flush();
		List<MESBasicMESServices> params = new ArrayList<MESBasicMESServices>();
		List<MESBasicServiceLogger> serviceLoggerList = serviceLoggerDao.findByCriteria(Restrictions.eq("mesService", mESServices));
		...
		if(signatureLog != null ) {
			...
			businessKey=mESServices.getServiceCode();
			signatureLog.setTableId(mESServices.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.common.delete",operType="3")
	public void deleteMESServices(String mESServicesIds){
		this.deleteMESServices(mESServicesIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.common.delete",operType="3")
	public void deleteMESServices(String mESServicesIds,SignatureLog signatureLog){
		deleteCollection(mESServicesIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMESServices(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_backgroundServiceMonitor#modelCode:MESBasic_1_backgroundServiceMonitor_MESServices#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="ec.engine.restore",operType="10")
	public void restoreMESServices(String mESServicesIds){
		restoreCollection(mESServicesIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreMESServices(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="ec.engine.restore",operType="10")
	public void restoreMESServices(long mESServicesId){
		
		findBusinessKeyUsed(mESServicesId);	//判断业务主键是否重复
		...
		beforeRestoreMESServices(mESServices);
		mESServices.setValid(true);
		mESServicesDao.update(mESServices);
		afterRestoreMESServices(mESServices);
		List<MESBasicMESServices> params = new ArrayList<MESBasicMESServices>();
		List<MESBasicServiceLogger> serviceLoggerList = serviceLoggerDao.findByCriteria(Restrictions.eq("mesService", mESServices));
		mESServices.setServiceLoggerList(serviceLoggerList);
		params.add(mESServices);
	...
	public void findBusinessKeyUsed(long mESServicesId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_backgroundServiceMonitor_MESServices");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicMESServices.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicMESServices.TABLE_NAME +" where id = ? )";
			List<Object> list =  mESServicesDao.createNativeQuery(sql,mESServicesId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseMESServices(List<MESBasicMESServices>  mESServicess){
		for(MESBasicMESServices mESServices:mESServicess)  {
			saveMESServices(mESServices, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMESServices(List<MESBasicMESServices>  mESServicess){
		for(MESBasicMESServices mESServices:mESServicess)  {
			saveMESServices(mESServices, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="ec.flow.save")
	public void saveMESServices(MESBasicMESServices mESServices, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMESServices(mESServices, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="ec.flow.save")
	public void saveMESServices(MESBasicMESServices mESServices, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMESServices(mESServices, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateMESServicesCodes(MESBasicMESServices mESServices) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMESServicesCodes(mESServices, false);
	}
	
	public void generateMESServicesCodes(MESBasicMESServices mESServices, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateMESServicesSummarys(MESBasicMESServices mESServices) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMESServicesSummarys(mESServices, false);
	}

	public void generateMESServicesSummarys(MESBasicMESServices mESServices, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices")
	public void saveMESServices(MESBasicMESServices mESServices,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveMESServices(mESServices, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices")
	public void saveMESServices(MESBasicMESServices mESServices,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_backgroundServiceMonitor";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
		String url = null;
		if(mESServices.getId() != null && mESServices.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicMESServices.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_backgroundServiceMonitor/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicMESServices.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_backgroundServiceMonitor/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, mESServices);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(mESServices);
		beforeSaveMESServices(mESServices, viewIsView);

		if (viewIsView) {
			mESServicesDao.saveWithRevertVersion(mESServices);
		} else {
			if(isNew)
				mESServicesDao.save(mESServices);
			else
				mESServicesDao.merge(mESServices);
		}
		if(null != mESServices && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicMESServices.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mESServices.getServiceCode() ? "" : mESServices.getServiceCode().toString());
			AuditUtil.setColumnStringA(null == mESServices.getServiceName() ? "" : mESServices.getServiceName().toString());
		...
		dealDatagridsSave(mESServices,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateMESServicesCodes(mESServices, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMESServicesSummarys(mESServices, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMESServices(mESServices, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, mESServices);
			}
		}
		List<MESBasicMESServices> params = new ArrayList<MESBasicMESServices>();
		mESServicesDao.flush();
		mESServicesDao.clear();
		mESServices = mESServicesDao.load(mESServices.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicMESServices mESServices){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("mESServices", mESServices);
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices")
	public void mergeMESServices(MESBasicMESServices mESServices, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(mESServices);
		beforeSaveMESServices(mESServices);
		mESServicesDao.merge(mESServices);
		if(null != mESServices && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicMESServices.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mESServices.getServiceCode() ? "" : mESServices.getServiceCode().toString());
			AuditUtil.setColumnStringA(null == mESServices.getServiceName() ? "" : mESServices.getServiceName().toString());
		...
		dealDatagridsSave(mESServices,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveMESServices(mESServices);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicMESServices> findMESServicess(Page<MESBasicMESServices> page, Criterion... criterions) {
		return mESServicesDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicMESServicesDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicMESServicesDealInfo> list=mESServicesDao.createQuery("from " + MESBasicMESServicesDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicMESServicesDealInfo dealInfo) {
		mESServicesDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicMESServicesDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) mESServicesDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicMESServicesDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = mESServicesDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = mESServicesDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicMESServicesDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = mESServicesDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMESServicess(Page<MESBasicMESServices> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMESServicess(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMESServicess(Page<MESBasicMESServices> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMESServicess(Page<MESBasicMESServices> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicMESServices> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"mESServices\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"mESServices\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"mESServices\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_backgroundServiceMonitor_MESServices" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_backgroundServiceMonitor_MESServices" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  mESServicesDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  mESServicesDao.createNativeQuery(powerOperateSql, "MESBasic_1_backgroundServiceMonitor").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mESServices\"","MESBasic_1_backgroundServiceMonitor",customCondition,"MESBASIC_MESSERVICES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"mESServices\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicMESServicesDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"mESServices\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"mESServices\".LAY_REC ASC, \"mESServices\".SORT ASC");
				} else {
					orderPart.append(" \"mESServices\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) mESServicesDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=mESServicesDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = mESServicesDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicMESServices> result = new ArrayList<MESBasicMESServices>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicMESServices.class,mESServicesDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicMESServices.class,mESServicesDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicMESServices mESServices : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(mESServices.getId(), "MESBasic_backgroundServiceMonitor_mESServices");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(mESServices.getId(), "MESBasic_backgroundServiceMonitor_mESServices");
							if(null != documents && !documents.isEmpty()) {
								mESServices.setDocument(documents.get(0));
								mESServices.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_backgroundServiceMonitor")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_backgroundServiceMonitor")+"MESBasic_1_backgroundServiceMonitor".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_backgroundServiceMonitor",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicMESServices> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicMESServices> result) {
		View view = null;
		...
		Map<Long, MESBasicMESServices> rsMap = new HashMap<Long, MESBasicMESServices>();
		for (MESBasicMESServices mESServices : result) {
			ids.add(mESServices.getId());
			rsMap.put(mESServices.getId(), mESServices);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(mESServicesDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = mESServicesDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicMESServices mESServices,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(mESServices.getOwnerStaffId());
		ti.setOwnerPositionId(mESServices.getOwnerPositionId());
		ti.setOwnerDepartmentId(mESServices.getOwnerDepartmentId());
		
		ti.setCreateStaff(mESServices.getCreateStaff());
		ti.setCreateStaffId(mESServices.getCreateStaffId());
		ti.setCreateTime(mESServices.getCreateTime());
		ti.setDeleteStaff(mESServices.getDeleteStaff());
		ti.setDeleteStaffId(mESServices.getDeleteStaffId());
		ti.setDeleteTime(mESServices.getDeleteTime());
		ti.setCreateDepartmentId(mESServices.getCreateDepartmentId());
		ti.setModifyStaff(mESServices.getModifyStaff());
		ti.setModifyStaffId(mESServices.getModifyStaffId());
		ti.setCreatePositionId(mESServices.getCreatePositionId());
		ti.setPositionLayRec(mESServices.getPositionLayRec());
		ti.setTableNo(mESServices.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_backgroundServiceMonitor");
		ti.setTargetTableName("MESBASIC_MESSERVICES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_backgroundServiceMonitor_MESServices" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  mESServicesDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mESServices\"","MESBasic_1_backgroundServiceMonitor","","MESBASIC_MESSERVICES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"mESServices\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"mESServices\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mESServices\"","MESBasic_1_backgroundServiceMonitor","","MESBASIC_MESSERVICES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("mESServices.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"mESServices\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"mESServices\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"mESServices\".CID = 1 OR \"mESServices\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("mESServices.mc\"").append(".MESSERVICES) not in ("+notinValue+")");
			}
			
			SQLQuery query = mESServicesDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("backgroundServiceMonitor", "MESBasic_1_backgroundServiceMonitor", "backgroundServiceMonitor_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=mESServicesDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicMESServices.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicMESServices.MODULE_CODE;}
	protected String getModelCode(){return MESBasicMESServices.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param mESServices 模型实例
	 */
	public void checkUniqueConstraint(MESBasicMESServices mESServices){
		if(mESServices.getId()==null){
			List<MESBasicMESServices> mESServicess = mESServicesDao.findByCriteria(Restrictions.eq("serviceCode", mESServices.getServiceCode()), Restrictions.eq("valid", true));
			if(mESServicess!=null && !mESServicess.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1429057324372", getCurrentUser().getLanguage()), mESServices.getServiceCode());
			}
		} else {
			List<MESBasicMESServices> mESServicess = mESServicesDao.findByCriteria(Restrictions.ne("id", mESServices.getId()), Restrictions.eq("serviceCode", mESServices.getServiceCode()), Restrictions.eq("valid", true));
			if(mESServicess!=null && !mESServicess.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1429057324372", getCurrentUser().getLanguage()), mESServices.getServiceCode());
			}
	...
	@Override
	public List<MESBasicMESServices> getMESServicess(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicMESServices.class);
		if(params!=null){
			...
			return mESServicesDao.findByHql("from " + MESBasicMESServices.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return mESServicesDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicMESServices> findMESServicessBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicMESServices.class);
	...
	@Override
	public List<MESBasicMESServices> findMESServicessByHql(String hql, Object... objects){
		
		return   mESServicesDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicMESServices loadMESServicesByBussinessKey(MESBasicMESServices mESServices){
		return loadMESServicesByBussinessKey(mESServices.getServiceCode());
	}
	
	@Override
	public Page<MESBasicMESServices> getMESServicess(Page<MESBasicMESServices> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicMESServices.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = mESServicesDao.findByPage(page, sql);
			}else{
				page = mESServicesDao.findAllByPage(page);
			}
			return page;
		}
		page = mESServicesDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicMESServices loadMESServicesByBussinessKey(Serializable bussinessKey){
		return mESServicesDao.findEntityByCriteria(Restrictions.eq("serviceCode", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicMESServices> findByProperty(String propertyName, Object object){
		return mESServicesDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicMESServices findEntityByProperty(String propertyName, Object object){
		return mESServicesDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicMESServices.JPA_NAME + " set valid=0 where serviceCode in (:businessKeys)";
			Query query = mESServicesDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_backgroundServiceMonitor_MESServices","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_backgroundServiceMonitor", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicMESServices mESServices,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,mESServicesDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicMESServices.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, mESServices);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",mESServices.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicMESServices.TABLE_NAME +" where valid = 1";
			list = mESServicesDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicMESServices.TABLE_NAME;
			list = mESServicesDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicMESServices.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = mESServicesDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicMESServicesDaoImpl mESServicesDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchMESServices(final List<MESBasicMESServices> insertObjs, final List<MESBasicMESServices> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchMESServices(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchMESServices(final List<MESBasicMESServices> insertObjs, final List<MESBasicMESServices> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1429057210032"),importCount));
		...
		Map<Long, MESBasicMESServices> oldUpdateObjsMap = new HashMap<Long, MESBasicMESServices>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicMESServices bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicMESServices> bizObjList = mESServicesDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicMESServices bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicMESServices> nmeObjs = new ArrayList<MESBasicMESServices>();
		...
		long id=autoGeneratorID.getNextId(MESBasicMESServices.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicMESServices mESServices:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicMESServices.TABLE_NAME,1,dbId);
			if(mESServices.getServiceCode() != null && !mESServices.getServiceCode().equals("") ){
				m.put(mESServices.getServiceCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) mESServicesDao.createNativeQuery(sql, "MESBasic_1_backgroundServiceMonitor_MESServices"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		mESServicesDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicMESServices.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicMESServices mESServices = updateObjs.get(updateIndex);
						Long id = mESServices.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = mESServices.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicMESServices.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicMESServices.TABLE_NAME + " order by id desc";
//					Query query = mESServicesDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicMESServices mESServices = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = mESServices.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		mESServicesDaoImpl.getSession().flush();
		mESServicesDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicMESServices bizObj = insertObjs.get(insertIndex);
			mESServicesImportService.saveImportDataLog(bizObj, null, "serviceCode", "serviceName");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicMESServices bizObj = updateObjs.get(updateIndex);
			MESBasicMESServices oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			mESServicesImportService.saveImportDataLog(bizObj, oldBizObj, "serviceCode", "serviceName");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = mESServicesDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				mESServicesDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = mESServicesDao.createNativeQuery(sql, "MESBasic_1_backgroundServiceMonitor_MESServices").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = mESServicesDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = mESServicesDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = mESServicesDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?mESServicesDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  mESServicesDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,mESServicesDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1429057196635Page(Page<MESBasicServiceLogger> dg1429057196635Page,MESBasicMESServices mESServices, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(MESBasicServiceLogger.class);
		criteria.add(Restrictions.eq("mesService", mESServices));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"serviceLogger\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		serviceLoggerDao.findByPage(dg1429057196635Page, criteria);
		serviceLoggerService.findByNormalDataGridMESServices(dg1429057196635Page);
	}
	
	 
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="ec.flow.save")
	public void saveMESServices(MESBasicMESServices mESServices, DataGridService dataGridService){
		saveMESServices(mESServices, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="ec.flow.save")
	public void saveMESServices(MESBasicMESServices mESServices, DataGridService dataGridService, String viewCode){
		saveMESServices(mESServices, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices")
	public void saveMESServices(MESBasicMESServices mESServices, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveMESServices(mESServices, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices")
	public void saveMESServices(MESBasicMESServices mESServices, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_backgroundServiceMonitor";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
		String url = null;
		if(null != mESServices.getId() && mESServices.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicMESServices.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices")
	public Map<Object, Object> mESServicesDataGridImport(MESBasicMESServices mESServices, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = mESServices.getVirtualId();
		
		mESServices = mESServicesDao.load(mESServices.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, mESServices.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = mESServices.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(mESServices);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, mESServices.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices")
	public void mergeMESServices(MESBasicMESServices mESServices, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(mESServices);
		beforeSaveMESServices(mESServices);
		mESServicesDao.merge(mESServices);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="foundation.batch.save",operType="4")
	public void batchSaveMESServices(MESBasicMESServices mESServices, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_backgroundServiceMonitor";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_backgroundServiceMonitor");
	...
	@Override
	public Page<MESBasicMESServices> getByPage(Page<MESBasicMESServices> page,DetachedCriteria detachedCriteria){
		return mESServicesDao.findByPage(page, detachedCriteria);
	}
	@Override
	public List<MESBasicServiceLogger> getServiceLoggerList(MESBasicMESServices mESServices){
		return serviceLoggerDao.findByCriteria(Restrictions.eq("mesService", mESServices),Restrictions.eq("valid", true));
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_backgroundServiceMonitor").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveMESServices(MESBasicMESServices mESServices, Object... objects){
		checkUniqueConstraint(mESServices);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveMESServices(MESBasicMESServices mESServices, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportMESServices(List<MESBasicMESServices> mESServicess, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportMESServices(List<MESBasicMESServices> mESServicess){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportMESServices(List<MESBasicMESServices> mESServicess, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportMESServices(List<MESBasicMESServices> mESServicess){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMESServices(MESBasicMESServices mESServices){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteMESServices(MESBasicMESServices mESServices){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreMESServices(MESBasicMESServices mESServices){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreMESServices(MESBasicMESServices mESServices){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicMESServices mESServices, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicMESServices mESServices, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportMESServices(List<MESBasicMESServices> insertObjs, List<MESBasicMESServices> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMESServices,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportMESServices(List<MESBasicMESServices> insertObjs, List<MESBasicMESServices> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMESServices,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
	
	@Override
	public MESBasicMESServices load(Long id){
		return mESServicesDao.load(id);
	}
	 ...
	 */
	@Override
	public void activeBatchDispatcher(){
		String sql = "from " + MESBasicMESServices.JPA_NAME + " where valid = true and serviceCode=?";
		List<MESBasicMESServices> sList = mESServicesDao.findByHql(sql,MESBasicMESServices.DISPATCH_SERVICE_CODE);
		if(sList.size() > 0){
			MESBasicMESServices mesService = sList.get(0);
			if(!mesService.getActive()){
				mesService.setActive(true);
				mESServicesDao.merge(mesService);
			}
		}
	}
	...
	public void stopBatchDispatcher(){
	...
	@Override
	public void mergeMESServices(MESBasicMESServices mESServices){
