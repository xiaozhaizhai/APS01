...
/* CUSTOM CODE START(action,import,MESBasic_1_customerMaterial_CustomerMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicCustomerMaterialAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicCustomerMaterial> {
	...
	private MESBasicCustomerMaterial customerMaterial;
	...
	//导出excel时,主模型内容
	List<MESBasicCustomerMaterial> queryResults = new ArrayList<MESBasicCustomerMaterial>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/customerMaterial/customerMaterial/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/customerMaterial/customerMaterial/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_customerMaterial_CustomerMaterial" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicCustomerMaterial> customerMaterials;
	
	public List<MESBasicCustomerMaterial> getCustomerMaterials() {
	...
	private Page<MESBasicCustomerMaterial> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			customerMaterial = new MESBasicCustomerMaterial();
		} else {
			if(id > 0){
				customerMaterial = customerMaterialService.getCustomerMaterial(id, viewCode);
			}
		}
	...
	public Page<MESBasicCustomerMaterial> getPage() {
		if (null == page) {
			page = new Page<MESBasicCustomerMaterial>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicCustomerMaterial getCustomerMaterial() {
		return customerMaterial;
	}		

	public void setCustomerMaterial(MESBasicCustomerMaterial customerMaterial) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = customerMaterialService.getCustomerMaterialAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1559177157767
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String customerMaterialReferenceCopy() throws Exception{
			MESBasicCustomerMaterial oldCustomerMaterial = null;
			if(isEditNew()){
				...
				oldCustomerMaterial = new MESBasicCustomerMaterial();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,customerMaterial.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, customerMaterial.getId());
				...
				oldCustomerMaterial = customerMaterialService.getCustomerMaterial(id);
			}
			
			
			customerMaterial = customerMaterialService.getCustomerMaterial(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_customerMaterial_CustomerMaterial");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicCustomerMaterial.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCustomerMaterial, null);
						Method setValue = MESBasicCustomerMaterial.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(customerMaterial, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			customerMaterial.setCreateDepartmentId(oldCustomerMaterial.getCreateDepartmentId());
			customerMaterial.setCreatePositionId(oldCustomerMaterial.getCreatePositionId());
			customerMaterial.setCreateStaffId(oldCustomerMaterial.getCreateStaffId());
			customerMaterial.setOwnerDepartmentId(oldCustomerMaterial.getOwnerDepartmentId());
			customerMaterial.setOwnerPositionId(oldCustomerMaterial.getOwnerPositionId());
			customerMaterial.setOwnerStaffId(oldCustomerMaterial.getOwnerStaffId());
			customerMaterial.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/customerMaterial/customerMaterial/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/MESBasic/customerMaterial/customerMaterial/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=customerMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return customerMaterialReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			customerMaterial = customerMaterialService.getCustomerMaterial(this.customerMaterial.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,customerMaterial.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			customerMaterialService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/customerMaterial/customerMaterial/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = customerMaterialService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(customerMaterialService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(customerMaterialService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_customerMaterial_CustomerMaterial", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/customerMaterial/customerMaterial/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.customer.customerCode,result.customer.customerName,result.material.productCode,result.material.productName,result.customerMaterialMemo,result.customer.id,result.material.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"customerMaterial\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"customerMaterial\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"customerMaterial\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"customerMaterial\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			customerMaterialService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/customerMaterial/customerMaterial/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_customerMaterial_CustomerMaterial");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		customerMaterialService.findDataGridPage(dg,dgclass,getDgPage(),customerMaterial,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(customerMaterial.getId() != null && customerMaterial.getId() != -1){
				customerMaterial = customerMaterialService.getCustomerMaterial(customerMaterial.getId());
			}
			customerMaterialService.findDataGridPage(dg,dgclass,getDgPage(),customerMaterial,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == customerMaterial.getId()) {
		...
		customerMaterialService.saveCustomerMaterial(customerMaterial,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/customerMaterial/customerMaterial/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "submit");
		bap_validate_datagrids=customerMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", customerMaterial.getId());
		...
		fileuploads.put("linkId", customerMaterial.getId());
		bapAfterCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/customerMaterial/customerMaterial/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		customerMaterialService.deleteCustomerMaterial(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "restore");
		customerMaterialService.restoreCustomerMaterial(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/customerMaterial/customerMaterial/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/customerMaterial/customerMaterial/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "showDealInfoData");
		if (null != tableInfoId) {
			int count=customerMaterialService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = customerMaterialService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = customerMaterialService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicCustomerMaterial> findExportDataInfos(Page<MESBasicCustomerMaterial> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = customerMaterialService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = customerMaterialService.getAssProperty(p.getCode());	
									//String assModelCode = customerMaterialService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			customerMaterialService.findCustomerMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			customerMaterialService.findCustomerMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicCustomerMaterial> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicCustomerMaterial result : results){
	...
	public boolean isEditNew(){
		//return null == this.customerMaterial || null == this.customerMaterial.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/customerMaterial/customerMaterial/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/customerMaterial/customerMaterial/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicCustomerMaterialService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_customerMaterial", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_customerMaterial");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_customerMaterial_CustomerMaterial", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_customerMaterial_CustomerMaterial", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_customerMaterial_CustomerMaterial";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_customerMaterial_CustomerMaterial", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicCustomerMaterial customerMaterial = null;
		if(null != id) {
			customerMaterial = customerMaterialService.getCustomerMaterial(id);
		}
		...
		_auditLog.setColumnStringB(null == customerMaterial.getId() ? "" : customerMaterial.getId().toString());
		_auditLog.setColumnStringA(null == customerMaterial.getId() ? "" : customerMaterial.getId().toString());
		_auditLog.setColumnStringC(customerMaterial.getId().toString());
		_auditLog.setColumnStringD(MESBasicCustomerMaterial.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_customerMaterial");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicCustomerMaterialAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, customerMaterial,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, customerMaterial,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,customerMaterial);
		...
		printlog.setEntity("MESBasic_1_customerMaterial");
		...
		printlog.setBusinessId(customerMaterial.getId());
		printlog.setBussinessKey(null == customerMaterial.getId() ? "" : customerMaterial.getId().toString());
		printlog.setMainDisplay(null == customerMaterial.getId() ? "" : customerMaterial.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(customerMaterial,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(customerMaterialService.getCustomerMaterial(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, customerMaterial,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				customerMaterial = customerMaterialService.getCustomerMaterial(Long.parseLong(arrId[i]));
				if(null != customerMaterial){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == customerMaterial.getId() ? "" : "," + customerMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == customerMaterial.getId() ? "" : customerMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == customerMaterial.getId() ? "" : "," + customerMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == customerMaterial.getId() ? "" : customerMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == customerMaterial.getId() ? "" : "," + customerMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == customerMaterial.getId() ? "" : customerMaterial.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, customerMaterial,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,customerMaterial);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//customerMaterialService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/customerMaterial/customerMaterial/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicCustomerMaterial> findExportPageInfos(Page<MESBasicCustomerMaterial> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_customerMaterial" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = customerMaterialService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicCustomerMaterial queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = customerMaterialService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = customerMaterialService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_customerMaterial_CustomerMaterial");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_customerMaterial_CustomerMaterial")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_customerMaterial_CustomerMaterial");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_customerMaterial_CustomerMaterial");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = customerMaterialService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_customerMaterial_CustomerMaterial,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_customerMaterial_CustomerMaterial,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_customerMaterial_CustomerMaterial,MESBasic_1) */
// 自定义代码
