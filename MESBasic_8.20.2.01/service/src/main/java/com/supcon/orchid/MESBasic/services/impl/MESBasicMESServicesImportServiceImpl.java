...
@Service("mESBasicMESServicesImportService")
public class MESBasicMESServicesImportServiceImpl implements MESBasicMESServicesImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_backgroundServiceMonitor",model="MESBasic_1_backgroundServiceMonitor_MESServices",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicMESServices bizObj, MESBasicMESServices oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1429057173329") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getServiceName() ? "" : "," + oldBizObj.getServiceName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getServiceName() ? "" : oldBizObj.getServiceName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getServiceCode() ? "" : "," + oldBizObj.getServiceCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getServiceCode() ? "" : oldBizObj.getServiceCode().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1429057173329") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getServiceName() ? "" : "," + bizObj.getServiceName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getServiceName() ? "" : bizObj.getServiceName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getServiceCode() ? "" : "," + bizObj.getServiceCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getServiceCode() ? "" : bizObj.getServiceCode().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicMESServices bizObj, String bussinesskeyFieldName,
