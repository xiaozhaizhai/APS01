...
/* CUSTOM CODE START(action,import,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicPurchGroupMemberAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicPurchGroupMember> {
	...
	private MESBasicPurchGroupMember purchGroupMember;
	...
	//导出excel时,主模型内容
	List<MESBasicPurchGroupMember> queryResults = new ArrayList<MESBasicPurchGroupMember>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/purch_Group/purchGroupMember/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/purch_Group/purchGroupMember/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_purch_Group_PurchGroupMember" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicPurchGroupMember> purchGroupMembers;
	
	public List<MESBasicPurchGroupMember> getPurchGroupMembers() {
		return purchGroupMembers;
	}
	...
	private Page<MESBasicPurchGroupMember> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			purchGroupMember = new MESBasicPurchGroupMember();
		} else {
			if(id > 0){
				purchGroupMember = purchGroupMemberService.getPurchGroupMember(id, viewCode);
			}
		}
	...
	public Page<MESBasicPurchGroupMember> getPage() {
		if (null == page) {
			page = new Page<MESBasicPurchGroupMember>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicPurchGroupMember getPurchGroupMember() {
		return purchGroupMember;
	}		

	public void setPurchGroupMember(MESBasicPurchGroupMember purchGroupMember) {
		this.purchGroupMember = purchGroupMember;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = purchGroupMemberService.getPurchGroupMemberAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1470291822089
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String purchGroupMemberReferenceCopy() throws Exception{
			MESBasicPurchGroupMember oldPurchGroupMember = null;
			if(isEditNew()){
				...
				oldPurchGroupMember = new MESBasicPurchGroupMember();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,purchGroupMember.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, purchGroupMember.getId());
				...
				oldPurchGroupMember = purchGroupMemberService.getPurchGroupMember(id);
			}
			
			
			purchGroupMember = purchGroupMemberService.getPurchGroupMember(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_purch_Group_PurchGroupMember");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicPurchGroupMember.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPurchGroupMember, null);
						Method setValue = MESBasicPurchGroupMember.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(purchGroupMember, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			purchGroupMember.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=purchGroupMemberService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return purchGroupMemberReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			purchGroupMember = purchGroupMemberService.getPurchGroupMember(this.purchGroupMember.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,purchGroupMember.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			purchGroupMemberService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = purchGroupMemberService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(purchGroupMemberService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(purchGroupMemberService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_purch_Group_PurchGroupMember", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"purchGroupMember\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"purchGroupMember\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"purchGroupMember\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"purchGroupMember\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			purchGroupMemberService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		purchGroupMemberService.findDataGridPage(dg,dgclass,getDgPage(),purchGroupMember,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(purchGroupMember.getId() != null && purchGroupMember.getId() != -1){
				purchGroupMember = purchGroupMemberService.getPurchGroupMember(purchGroupMember.getId());
			}
			purchGroupMemberService.findDataGridPage(dg,dgclass,getDgPage(),purchGroupMember,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == purchGroupMember.getId()) {
		...
		purchGroupMemberService.savePurchGroupMember(purchGroupMember,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "submit");
		bap_validate_datagrids=purchGroupMemberService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", purchGroupMember.getId());
		...
		fileuploads.put("linkId", purchGroupMember.getId());
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/purch_Group/purchGroupMember/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		purchGroupMemberService.deletePurchGroupMember(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "restore");
		purchGroupMemberService.restorePurchGroupMember(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicPurchGroupMember> findExportDataInfos(Page<MESBasicPurchGroupMember> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = purchGroupMemberService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = purchGroupMemberService.getAssProperty(p.getCode());	
									//String assModelCode = purchGroupMemberService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			purchGroupMemberService.findPurchGroupMembers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			purchGroupMemberService.findPurchGroupMembers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicPurchGroupMember> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicPurchGroupMember result : results){
	...
	public boolean isEditNew(){
		//return null == this.purchGroupMember || null == this.purchGroupMember.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/purch_Group/purchGroupMember/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/purch_Group/purchGroupMember/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicPurchGroupMemberService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_purch_Group_PurchGroupMember", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_purch_Group_PurchGroupMember", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_purch_Group_PurchGroupMember";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroupMember", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
		_auditLog.setColumnStringA(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
		_auditLog.setColumnStringC(purchGroupMember.getId().toString());
		_auditLog.setColumnStringD(MESBasicPurchGroupMember.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicPurchGroupMemberAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, purchGroupMember,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, purchGroupMember,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,purchGroupMember);
		...
		printlog.setBusinessId(purchGroupMember.getId());
		printlog.setBussinessKey(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
		printlog.setMainDisplay(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(purchGroupMember,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(purchGroupMemberService.getPurchGroupMember(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, purchGroupMember,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				purchGroupMember = purchGroupMemberService.getPurchGroupMember(Long.parseLong(arrId[i]));
				if(null != purchGroupMember){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == purchGroupMember.getId() ? "" : "," + purchGroupMember.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == purchGroupMember.getId() ? "" : "," + purchGroupMember.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == purchGroupMember.getId() ? "" : "," + purchGroupMember.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == purchGroupMember.getId() ? "" : purchGroupMember.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, purchGroupMember,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,purchGroupMember);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//purchGroupMemberService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/purch_Group/purchGroupMember/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_purch_Group_PurchGroupMember");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_purch_Group_PurchGroupMember")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_purch_Group_PurchGroupMember");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_purch_Group_PurchGroupMember");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = purchGroupMemberService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_purch_Group_PurchGroupMember,MESBasic_1) */
// 自定义代码
