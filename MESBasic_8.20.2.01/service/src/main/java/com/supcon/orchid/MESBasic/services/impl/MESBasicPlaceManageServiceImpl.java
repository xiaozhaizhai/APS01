...
import com.supcon.orchid.MESBasic.entities.MESBasicPlaceManageMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicPlaceManageService;
import com.supcon.orchid.MESBasic.services.MESBasicPlaceManageImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicPlaceManageDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_placeManageService")
@Transactional
public class MESBasicPlaceManageServiceImpl  extends BaseServiceImpl<MESBasicPlaceManage> implements MESBasicPlaceManageService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicPlaceManage> bapBaseService;
	...
	@Autowired
	private MESBasicPlaceManageImportService placeManageImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicPlaceManage.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  placeManageDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicPlaceManage.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = placeManageDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPlaceManage.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPlaceManage.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  placeManageDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicPlaceManage.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicPlaceManage.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicPlaceManage> list = (List<MESBasicPlaceManage>) placeManageDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicPlaceManage getPlaceManage(long id){
		return getPlaceManage(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPlaceManage getPlaceManage(long id, String viewCode){
		MESBasicPlaceManage placeManage = placeManageDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPlaceManageAsJSON(long id, String include){
		MESBasicPlaceManage placeManage = placeManageDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(placeManage, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.common.delete",operType="3")
	public void deletePlaceManage(MESBasicPlaceManage placeManage){
		if(null != placeManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPlaceManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == placeManage.getPlaceCode() ? "" : placeManage.getPlaceCode().toString());
			AuditUtil.setColumnStringA(null == placeManage.getPlaceName() ? "" : placeManage.getPlaceName().toString());
			AuditUtil.setColumnStringC(placeManage.getId().toString());
		}
		beforeDeletePlaceManage(placeManage);
		placeManageDao.delete(placeManage);
		afterDeletePlaceManage(placeManage);
		placeManageDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/placeManage/del", placeManage,MESBasicPlaceManageMneCode.class);
		List<MESBasicPlaceManage> params = new ArrayList<MESBasicPlaceManage>();
		params.add(placeManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_placeManage");
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.common.delete",operType="3")
	public void deletePlaceManage(List<Long> placeManageIds){
		deletePlaceManage(placeManageIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.common.delete",operType="3")
	public void deletePlaceManage(List<Long> placeManageIds, String eventTopic) {
		List<MESBasicPlaceManage> placeManages = new ArrayList<MESBasicPlaceManage>();
		for(Long placeManageId : placeManageIds){
			MESBasicPlaceManage placeManage = getPlaceManage(placeManageId);
			placeManages.add(placeManage);
			if(null != placeManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPlaceManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == placeManage.getPlaceName() ? "" : "," + placeManage.getPlaceName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == placeManage.getPlaceName() ? "" : placeManage.getPlaceName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == placeManage.getPlaceCode() ? "" : "," + placeManage.getPlaceCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == placeManage.getPlaceCode() ? "" : placeManage.getPlaceCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == placeManage.getId() ? "" : "," + placeManage.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == placeManage.getId() ? "" : placeManage.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(placeManages != null && placeManages.size() > 0 ){
			for(MESBasicPlaceManage placeManage : placeManages){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),placeManage, placeManage.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(placeManageIds != null && placeManageIds.size() > 0) {
			String hql = "update " + MESBasicPlaceManage.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = placeManageDao.createQuery(hql);
			query.setParameterList("ids", placeManageIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicPlaceManage placeManagez : placeManages){
				placeManagez.setValid(false);
				List<MESBasicPlaceManage> params = new ArrayList<MESBasicPlaceManage>();
				params.add(placeManagez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_placeManage");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.common.delete",operType="3")
	public void deletePlaceManage(long placeManageId, int placeManageVersion){
		this.deletePlaceManage(placeManageId, placeManageVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.common.delete",operType="3")
	public void deletePlaceManage(long placeManageId, int placeManageVersion,SignatureLog signatureLog){
		MESBasicPlaceManage placeManage = getPlaceManage(placeManageId);
		...
		beforeDeletePlaceManage(placeManage);
		placeManageDao.delete(placeManageId, placeManageVersion);
		afterDeletePlaceManage(placeManage);
		placeManageDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/placeManage/del", placeManage,MESBasicPlaceManageMneCode.class);
		List<MESBasicPlaceManage> params = new ArrayList<MESBasicPlaceManage>();
		...
		if(signatureLog != null ) {
			...
			businessKey=placeManage.getPlaceCode();
			signatureLog.setTableId(placeManage.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.common.delete",operType="3")
	public void deletePlaceManage(String placeManageIds){
		this.deletePlaceManage(placeManageIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.common.delete",operType="3")
	public void deletePlaceManage(String placeManageIds,SignatureLog signatureLog){
		deleteCollection(placeManageIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePlaceManage(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_placeManage#modelCode:MESBasic_1_placeManage_PlaceManage#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="ec.engine.restore",operType="10")
	public void restorePlaceManage(String placeManageIds){
		restoreCollection(placeManageIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePlaceManage(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="ec.engine.restore",operType="10")
	public void restorePlaceManage(long placeManageId){
		
		findBusinessKeyUsed(placeManageId);	//判断业务主键是否重复
		...
		beforeRestorePlaceManage(placeManage);
		placeManage.setValid(true);
		placeManageDao.update(placeManage);
		afterRestorePlaceManage(placeManage);
		placeManageDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/placeManage/add", placeManage,MESBasicPlaceManageMneCode.class);
		List<MESBasicPlaceManage> params = new ArrayList<MESBasicPlaceManage>();
		params.add(placeManage);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long placeManageId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_placeManage_PlaceManage");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicPlaceManage.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicPlaceManage.TABLE_NAME +" where id = ? )";
			List<Object> list =  placeManageDao.createNativeQuery(sql,placeManageId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePlaceManage(List<MESBasicPlaceManage>  placeManages){
		for(MESBasicPlaceManage placeManage:placeManages)  {
			savePlaceManage(placeManage, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePlaceManage(List<MESBasicPlaceManage>  placeManages){
		for(MESBasicPlaceManage placeManage:placeManages)  {
			savePlaceManage(placeManage, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="ec.flow.save")
	public void savePlaceManage(MESBasicPlaceManage placeManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePlaceManage(placeManage, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="ec.flow.save")
	public void savePlaceManage(MESBasicPlaceManage placeManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePlaceManage(placeManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePlaceManageCodes(MESBasicPlaceManage placeManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePlaceManageCodes(placeManage, false);
	}
	
	public void generatePlaceManageCodes(MESBasicPlaceManage placeManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePlaceManageSummarys(MESBasicPlaceManage placeManage) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePlaceManageSummarys(placeManage, false);
	}

	public void generatePlaceManageSummarys(MESBasicPlaceManage placeManage, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage")
	public void savePlaceManage(MESBasicPlaceManage placeManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePlaceManage(placeManage, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage")
	public void savePlaceManage(MESBasicPlaceManage placeManage,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_placeManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_placeManage");
		String url = null;
		if(placeManage.getId() != null && placeManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPlaceManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_placeManage/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPlaceManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_placeManage/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, placeManage);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(placeManage);
		beforeSavePlaceManage(placeManage, viewIsView);

		if (viewIsView) {
			placeManageDao.saveWithRevertVersion(placeManage);
		} else {
			if(isNew)
				placeManageDao.save(placeManage);
			else
				placeManageDao.merge(placeManage);
		}
		if(null != placeManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPlaceManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == placeManage.getPlaceCode() ? "" : placeManage.getPlaceCode().toString());
			AuditUtil.setColumnStringA(null == placeManage.getPlaceName() ? "" : placeManage.getPlaceName().toString());
		...
		dealDatagridsSave(placeManage,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePlaceManageCodes(placeManage, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePlaceManageSummarys(placeManage, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePlaceManage(placeManage, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, placeManage);
			}
		}
		List<MESBasicPlaceManage> params = new ArrayList<MESBasicPlaceManage>();
		placeManageDao.flush();
		placeManageDao.clear();
		placeManage = placeManageDao.load(placeManage.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicPlaceManage placeManage){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("placeManage", placeManage);
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage")
	public void mergePlaceManage(MESBasicPlaceManage placeManage, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(placeManage);
		beforeSavePlaceManage(placeManage);
		placeManageDao.merge(placeManage);
		if(null != placeManage && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPlaceManage.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == placeManage.getPlaceCode() ? "" : placeManage.getPlaceCode().toString());
			AuditUtil.setColumnStringA(null == placeManage.getPlaceName() ? "" : placeManage.getPlaceName().toString());
		...
		dealDatagridsSave(placeManage,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePlaceManage(placeManage);
		placeManageDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/placeManage/add", placeManage,MESBasicPlaceManageMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicPlaceManage> findPlaceManages(Page<MESBasicPlaceManage> page, Criterion... criterions) {
		return placeManageDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicPlaceManageDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicPlaceManageDealInfo> list=placeManageDao.createQuery("from " + MESBasicPlaceManageDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicPlaceManageDealInfo dealInfo) {
		placeManageDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicPlaceManageDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) placeManageDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicPlaceManageDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = placeManageDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = placeManageDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicPlaceManageDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = placeManageDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPlaceManages(Page<MESBasicPlaceManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPlaceManages(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPlaceManages(Page<MESBasicPlaceManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPlaceManages(Page<MESBasicPlaceManage> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicPlaceManage> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"placeManage\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"placeManage\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"placeManage\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_placeManage_PlaceManage" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_placeManage_PlaceManage" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  placeManageDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  placeManageDao.createNativeQuery(powerOperateSql, "MESBasic_1_placeManage").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"placeManage\"","MESBasic_1_placeManage",customCondition,"MESBASIC_PLACE_MENAGE",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"placeManage\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicPlaceManageDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"placeManage\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"placeManage\".LAY_REC ASC, \"placeManage\".SORT ASC");
				} else {
					orderPart.append(" \"placeManage\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) placeManageDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=placeManageDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = placeManageDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicPlaceManage> result = new ArrayList<MESBasicPlaceManage>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicPlaceManage.class,placeManageDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicPlaceManage.class,placeManageDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicPlaceManage placeManage : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(placeManage.getId(), "MESBasic_placeManage_placeManage");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(placeManage.getId(), "MESBasic_placeManage_placeManage");
							if(null != documents && !documents.isEmpty()) {
								placeManage.setDocument(documents.get(0));
								placeManage.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_placeManage")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_placeManage")+"MESBasic_1_placeManage".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_placeManage",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicPlaceManage> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicPlaceManage> result) {
		View view = null;
		...
		Map<Long, MESBasicPlaceManage> rsMap = new HashMap<Long, MESBasicPlaceManage>();
		for (MESBasicPlaceManage placeManage : result) {
			ids.add(placeManage.getId());
			rsMap.put(placeManage.getId(), placeManage);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(placeManageDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = placeManageDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicPlaceManage placeManage,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(placeManage.getOwnerStaffId());
		ti.setOwnerPositionId(placeManage.getOwnerPositionId());
		ti.setOwnerDepartmentId(placeManage.getOwnerDepartmentId());
		
		ti.setCreateStaff(placeManage.getCreateStaff());
		ti.setCreateStaffId(placeManage.getCreateStaffId());
		ti.setCreateTime(placeManage.getCreateTime());
		ti.setDeleteStaff(placeManage.getDeleteStaff());
		ti.setDeleteStaffId(placeManage.getDeleteStaffId());
		ti.setDeleteTime(placeManage.getDeleteTime());
		ti.setCreateDepartmentId(placeManage.getCreateDepartmentId());
		ti.setModifyStaff(placeManage.getModifyStaff());
		ti.setModifyStaffId(placeManage.getModifyStaffId());
		ti.setCreatePositionId(placeManage.getCreatePositionId());
		ti.setPositionLayRec(placeManage.getPositionLayRec());
		ti.setTableNo(placeManage.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_placeManage");
		ti.setTargetTableName("MESBASIC_PLACE_MENAGE");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_placeManage_PlaceManage" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  placeManageDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"placeManage\"","MESBasic_1_placeManage","","MESBASIC_PLACE_MENAGE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"placeManage\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"placeManage\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"placeManage\"","MESBasic_1_placeManage","","MESBASIC_PLACE_MENAGE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("placeManage.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"placeManage\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"placeManage\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"placeManage\".CID = 1 OR \"placeManage\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("placeManage.mc\"").append(".PLACE_MANAGE) not in ("+notinValue+")");
			}
			
			SQLQuery query = placeManageDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("placeManage", "MESBasic_1_placeManage", "placeManage_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=placeManageDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicPlaceManage.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicPlaceManage.MODULE_CODE;}
	protected String getModelCode(){return MESBasicPlaceManage.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param placeManage 模型实例
	 */
	public void checkUniqueConstraint(MESBasicPlaceManage placeManage){
		if(null == placeManage.getPlaceCode() || StringUtils.isEmpty(placeManage.getPlaceCode().toString())){
			return;
		}
		if(placeManage.getId()==null ){
			List<MESBasicPlaceManage> placeManages = placeManageDao.findByCriteria(Restrictions.eq("placeCode", placeManage.getPlaceCode()));
			if(placeManages!=null && !placeManages.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.radion1418607299231", getCurrentUser().getLanguage()), placeManage.getPlaceCode());
			}
		} else {
			List<MESBasicPlaceManage> placeManages = placeManageDao.findByCriteria(Restrictions.ne("id", placeManage.getId()), Restrictions.eq("placeCode", placeManage.getPlaceCode()));
			if(placeManages!=null && !placeManages.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.radion1418607299231", getCurrentUser().getLanguage()), placeManage.getPlaceCode());
			}
	...
	@Override
	public List<MESBasicPlaceManage> getPlaceManages(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPlaceManage.class);
		if(params!=null){
			...
			return placeManageDao.findByHql("from " + MESBasicPlaceManage.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return placeManageDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicPlaceManage> findPlaceManagesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPlaceManage.class);
	...
	@Override
	public List<MESBasicPlaceManage> findPlaceManagesByHql(String hql, Object... objects){
		
		return   placeManageDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicPlaceManage loadPlaceManageByBussinessKey(MESBasicPlaceManage placeManage){
		return loadPlaceManageByBussinessKey(placeManage.getPlaceCode());
	}
	
	@Override
	public Page<MESBasicPlaceManage> getPlaceManages(Page<MESBasicPlaceManage> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPlaceManage.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = placeManageDao.findByPage(page, sql);
			}else{
				page = placeManageDao.findAllByPage(page);
			}
			return page;
		}
		page = placeManageDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPlaceManage loadPlaceManageByBussinessKey(Serializable bussinessKey){
		return placeManageDao.findEntityByCriteria(Restrictions.eq("placeCode", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicPlaceManage> findByProperty(String propertyName, Object object){
		return placeManageDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPlaceManage findEntityByProperty(String propertyName, Object object){
		return placeManageDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicPlaceManage.JPA_NAME + " set valid=0 where placeCode in (:businessKeys)";
			Query query = placeManageDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_placeManage_PlaceManage","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_placeManage", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicPlaceManage placeManage,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,placeManageDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicPlaceManage.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, placeManage);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",placeManage.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPlaceManage.TABLE_NAME +" where valid = 1";
			list = placeManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPlaceManage.TABLE_NAME;
			list = placeManageDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicPlaceManage.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = placeManageDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicPlaceManageDaoImpl placeManageDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPlaceManage(final List<MESBasicPlaceManage> insertObjs, final List<MESBasicPlaceManage> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPlaceManage(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPlaceManage(final List<MESBasicPlaceManage> insertObjs, final List<MESBasicPlaceManage> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1418607150650"),importCount));
		...
		Map<Long, MESBasicPlaceManage> oldUpdateObjsMap = new HashMap<Long, MESBasicPlaceManage>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicPlaceManage bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicPlaceManage> bizObjList = placeManageDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicPlaceManage bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicPlaceManage> nmeObjs = new ArrayList<MESBasicPlaceManage>();
		...
		long id=autoGeneratorID.getNextId(MESBasicPlaceManage.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicPlaceManage placeManage:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicPlaceManage.TABLE_NAME,1,dbId);
			if(placeManage.getPlaceCode() != null && !placeManage.getPlaceCode().equals("") ){
				m.put(placeManage.getPlaceCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) placeManageDao.createNativeQuery(sql, "MESBasic_1_placeManage_PlaceManage"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		placeManageDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicPlaceManage.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicPlaceManage placeManage = updateObjs.get(updateIndex);
						Long id = placeManage.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = placeManage.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicPlaceManage.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicPlaceManage.TABLE_NAME + " order by id desc";
//					Query query = placeManageDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicPlaceManage placeManage = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = placeManage.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicPlaceManageMneCode.class;
					...
					for(MESBasicPlaceManage placeManage:updateObjs)  {
						Long id = placeManage.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicPlaceManage placeManage:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(placeManage);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, placeManage.getId());
		...
		placeManageDaoImpl.getSession().flush();
		placeManageDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicPlaceManage bizObj = insertObjs.get(insertIndex);
			placeManageImportService.saveImportDataLog(bizObj, null, "placeCode", "placeName");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicPlaceManage bizObj = updateObjs.get(updateIndex);
			MESBasicPlaceManage oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			placeManageImportService.saveImportDataLog(bizObj, oldBizObj, "placeCode", "placeName");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = placeManageDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				placeManageDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = placeManageDao.createNativeQuery(sql, "MESBasic_1_placeManage_PlaceManage").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = placeManageDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = placeManageDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = placeManageDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?placeManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  placeManageDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,placeManageDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="ec.flow.save")
	public void savePlaceManage(MESBasicPlaceManage placeManage, DataGridService dataGridService){
		savePlaceManage(placeManage, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="ec.flow.save")
	public void savePlaceManage(MESBasicPlaceManage placeManage, DataGridService dataGridService, String viewCode){
		savePlaceManage(placeManage, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage")
	public void savePlaceManage(MESBasicPlaceManage placeManage, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePlaceManage(placeManage, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage")
	public void savePlaceManage(MESBasicPlaceManage placeManage, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_placeManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_placeManage");
		String url = null;
		if(null != placeManage.getId() && placeManage.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPlaceManage.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage")
	public Map<Object, Object> placeManageDataGridImport(MESBasicPlaceManage placeManage, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = placeManage.getVirtualId();
		
		placeManage = placeManageDao.load(placeManage.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, placeManage.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = placeManage.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(placeManage);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, placeManage.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage")
	public void mergePlaceManage(MESBasicPlaceManage placeManage, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(placeManage);
		beforeSavePlaceManage(placeManage);
		placeManageDao.merge(placeManage);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_placeManage",model="MESBasic_1_placeManage_PlaceManage",desc="foundation.batch.save",operType="4")
	public void batchSavePlaceManage(MESBasicPlaceManage placeManage, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_placeManage";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_placeManage");
	...
	@Override
	public Page<MESBasicPlaceManage> getByPage(Page<MESBasicPlaceManage> page,DetachedCriteria detachedCriteria){
		return placeManageDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_placeManage").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSavePlaceManage(MESBasicPlaceManage placeManage, Object... objects){
		checkUniqueConstraint(placeManage);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePlaceManage(MESBasicPlaceManage placeManage, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPlaceManage(List<MESBasicPlaceManage> placeManages, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPlaceManage(List<MESBasicPlaceManage> placeManages){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPlaceManage(List<MESBasicPlaceManage> placeManages, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPlaceManage(List<MESBasicPlaceManage> placeManages){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePlaceManage(MESBasicPlaceManage placeManage){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePlaceManage(MESBasicPlaceManage placeManage){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
       if(!bapBaseService.canDelete(placeManage)){
			
			throw new BAPException("该位置已被其他业务引用，不能删除！");
		}
/* CUSTOM CODE END */
	}
	
	private void beforeRestorePlaceManage(MESBasicPlaceManage placeManage){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePlaceManage(MESBasicPlaceManage placeManage){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicPlaceManage placeManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicPlaceManage placeManage, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPlaceManage(List<MESBasicPlaceManage> insertObjs, List<MESBasicPlaceManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPlaceManage,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPlaceManage(List<MESBasicPlaceManage> insertObjs, List<MESBasicPlaceManage> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPlaceManage,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_placeManage_PlaceManage,MESBasic_1) */
// 自定义代码
