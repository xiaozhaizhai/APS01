...
@Repository
public class MESBasicNodeTypeDaoImpl extends ExtendedGenericDaoImpl<MESBasicNodeType, Long> implements MESBasicNodeTypeDao {
	@Override
	...
	@Override
	public MESBasicNodeTypeDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicNodeTypeDealInfo dealInfo = new MESBasicNodeTypeDealInfo();
	...
	@Override
	public MESBasicNodeTypeDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicNodeType mainObj, Staff dealStaff) {
		MESBasicNodeTypeDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicNodeTypeDealInfo.class);
		...
		List<MESBasicNodeTypeDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_nodeType_NodeType,MESBasic_1) */
// 自定义代码
