...
/* CUSTOM CODE START(action,import,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicBatchRelationshipAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBatchRelationship> {
	...
	private MESBasicBatchRelationship batchRelationship;
	...
	//导出excel时,主模型内容
	List<MESBasicBatchRelationship> queryResults = new ArrayList<MESBasicBatchRelationship>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/batchInfo/batchRelationship/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/batchInfo/batchRelationship/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_batchInfo_BatchRelationship" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBatchRelationship> batchRelationships;
	
	public List<MESBasicBatchRelationship> getBatchRelationships() {
	...
	private Page<MESBasicBatchRelationship> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			batchRelationship = new MESBasicBatchRelationship();
			if(batchRelationship.getBatchType() != null) {
				batchRelationship.setBatchType((SystemCode) systemCodeService.getSystemCode(batchRelationship.getBatchType().getId()));
			}
		} else {
			if(id > 0){
				batchRelationship = batchRelationshipService.getBatchRelationship(id, viewCode);
			}
		}
	...
	public Page<MESBasicBatchRelationship> getPage() {
		if (null == page) {
			page = new Page<MESBasicBatchRelationship>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicBatchRelationship getBatchRelationship() {
		return batchRelationship;
	}		

	public void setBatchRelationship(MESBasicBatchRelationship batchRelationship) {
		this.batchRelationship = batchRelationship;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/batchInfo/batchRelationship/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchRelationshipService.getBatchRelationshipAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/batchInfo/batchRelationship/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1490775465362
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchRelationshipReferenceCopy() throws Exception{
			MESBasicBatchRelationship oldBatchRelationship = null;
			if(isEditNew()){
				...
				oldBatchRelationship = new MESBasicBatchRelationship();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchRelationship.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchRelationship.getId());
				...
				oldBatchRelationship = batchRelationshipService.getBatchRelationship(id);
			}
			
			
			batchRelationship = batchRelationshipService.getBatchRelationship(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchRelationship");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBatchRelationship.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchRelationship, null);
						Method setValue = MESBasicBatchRelationship.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchRelationship, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchRelationship.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchRelationshipService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchRelationshipReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchRelationship = batchRelationshipService.getBatchRelationship(this.batchRelationship.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchRelationship.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchRelationshipService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchRelationship/batchRelationList", results = { @Result(type = FREEMARKER, location = "batchRelationList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchRelationshipService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchRelationshipService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchRelationshipService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_batchInfo_BatchRelationship", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchRelationship/batchRelationList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.mainBatch,result.mainProduct.productCode,result.mainProduct.productName,result.partBatch,result.partProduct.productCode,result.partProduct.productName,result.batchType.id,result.batchType.value,result.mainProduct.id,result.partProduct.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batchRelationList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchRelationship\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchRelationship\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchRelationship\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchRelationship\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchRelationshipService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/batchInfo/batchRelationship/batchRelationList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchRelationshipService.findDataGridPage(dg,dgclass,getDgPage(),batchRelationship,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchRelationship.getId() != null && batchRelationship.getId() != -1){
				batchRelationship = batchRelationshipService.getBatchRelationship(batchRelationship.getId());
			}
			batchRelationshipService.findDataGridPage(dg,dgclass,getDgPage(),batchRelationship,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchRelationship.getId()) {
		...
		batchRelationshipService.saveBatchRelationship(batchRelationship,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "submit");
		bap_validate_datagrids=batchRelationshipService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchRelationship.getId());
		...
		fileuploads.put("linkId", batchRelationship.getId());
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/batchInfo/batchRelationship/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchRelationshipService.deleteBatchRelationship(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchRelationship/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "restore");
		batchRelationshipService.restoreBatchRelationship(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBatchRelationship> findExportDataInfos(Page<MESBasicBatchRelationship> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchRelationshipService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchRelationshipService.getAssProperty(p.getCode());	
									//String assModelCode = batchRelationshipService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchRelationshipService.findBatchRelationships(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchRelationshipService.findBatchRelationships(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBatchRelationship> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBatchRelationship result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchRelationship || null == this.batchRelationship.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/batchInfo/batchRelationship/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/batchInfo/batchRelationship/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/batchInfo/batchRelationship/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchRelationship/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBatchRelationshipService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchRelationship/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_batchInfo_BatchRelationship", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_batchInfo_BatchRelationship", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/batchInfo/batchRelationship/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/batchInfo/batchRelationship/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_batchInfo_BatchRelationship";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_batchInfo_BatchRelationship", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/batchInfo/batchRelationship/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicBatchRelationship batchRelationship = null;
		if(null != id) {
			batchRelationship = batchRelationshipService.getBatchRelationship(id);
		}
		...
		_auditLog.setColumnStringB(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
		_auditLog.setColumnStringA(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
		_auditLog.setColumnStringC(batchRelationship.getId().toString());
		_auditLog.setColumnStringD(MESBasicBatchRelationship.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBatchRelationshipAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchRelationship,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchRelationship,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchRelationship);
		...
		printlog.setBusinessId(batchRelationship.getId());
		printlog.setBussinessKey(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
		printlog.setMainDisplay(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/batchInfo/batchRelationship/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchRelationship,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/batchInfo/batchRelationship/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchRelationshipService.getBatchRelationship(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/batchInfo/batchRelationship/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/batchInfo/batchRelationship/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchRelationship,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchRelationship = batchRelationshipService.getBatchRelationship(Long.parseLong(arrId[i]));
				if(null != batchRelationship){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchRelationship.getId() ? "" : "," + batchRelationship.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchRelationship.getId() ? "" : "," + batchRelationship.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchRelationship.getId() ? "" : "," + batchRelationship.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchRelationship.getId() ? "" : batchRelationship.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchRelationship,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchRelationship);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchRelationshipService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/batchInfo/batchRelationship/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchRelationship");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_batchInfo_BatchRelationship")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_batchInfo_BatchRelationship");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_batchInfo_BatchRelationship");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchRelationshipService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_batchInfo_BatchRelationship,MESBasic_1) */
// 自定义代码
