...
@Repository
public class MESBasicProducttypeDaoImpl extends ExtendedGenericDaoImpl<MESBasicProducttype, Long> implements MESBasicProducttypeDao {
	@Override
	...
	@Override
	public MESBasicProducttypeDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicProducttypeDealInfo dealInfo = new MESBasicProducttypeDealInfo();
	...
	@Override
	public MESBasicProducttypeDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicProducttype mainObj, Staff dealStaff) {
		MESBasicProducttypeDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicProducttypeDealInfo.class);
		...
		List<MESBasicProducttypeDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	 /* CUSTOM CODE START(daoimpl,functions,MESBasic_1_product_type_Producttype,MESBasic_1) */
// 自定义代码
