...
import com.supcon.orchid.MESBasic.entities.MESBasicAreaMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicAreaService;
import com.supcon.orchid.MESBasic.services.MESBasicAreaImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicAreaDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_areaService")
@Transactional
public class MESBasicAreaServiceImpl  extends BaseServiceImpl<MESBasicArea> implements MESBasicAreaService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicArea> bapBaseService;
	...
	@Autowired
	private MESBasicAreaImportService areaImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicArea.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  areaDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicArea.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = areaDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicArea.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicArea.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  areaDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicArea.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicArea.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicArea> list = (List<MESBasicArea>) areaDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicArea getArea(long id){
		return getArea(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicArea getArea(long id, String viewCode){
		MESBasicArea area = areaDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getAreaAsJSON(long id, String include){
		MESBasicArea area = areaDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(area, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.common.delete",operType="3")
	public void deleteArea(MESBasicArea area){
		if(null != area && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicArea.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == area.getCode() ? "" : area.getCode().toString());
			AuditUtil.setColumnStringA(null == area.getName() ? "" : area.getName().toString());
			AuditUtil.setColumnStringC(area.getId().toString());
		}
		checkDeletePermit(area.getId());
		beforeDeleteArea(area);
		areaDao.delete(area);
		afterDeleteArea(area);
		areaDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/area/del", area,MESBasicAreaMneCode.class);
		List<MESBasicArea> params = new ArrayList<MESBasicArea>();
		params.add(area);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_area");
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.common.delete",operType="3")
	public void deleteArea(List<Long> areaIds){
		deleteArea(areaIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.common.delete",operType="3")
	public void deleteArea(List<Long> areaIds, String eventTopic) {
		List<MESBasicArea> areas = new ArrayList<MESBasicArea>();
		for(Long areaId : areaIds){
			MESBasicArea area = getArea(areaId);
			areas.add(area);
			if(null != area && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicArea.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == area.getName() ? "" : "," + area.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == area.getName() ? "" : area.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == area.getCode() ? "" : "," + area.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == area.getCode() ? "" : area.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == area.getId() ? "" : "," + area.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == area.getId() ? "" : area.getId().toString());
				}
			}
		}
		
		for(MESBasicArea area : areas){
			beforeDeleteArea(area);
		}
		...
		if(areas != null && areas.size() > 0 ){
			for(MESBasicArea area : areas){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),area, area.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(areaIds != null && areaIds.size() > 0) {
			String hql = "update " + MESBasicArea.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = areaDao.createQuery(hql);
			query.setParameterList("ids", areaIds);
			query.executeUpdate();
			areaDao.clearAllCacheByEntity();
		}
		...
		if(eventTopic==null){
			for(MESBasicArea areaz : areas){
				areaz.setValid(false);
				List<MESBasicArea> params = new ArrayList<MESBasicArea>();
				params.add(areaz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.common.delete",operType="3")
	public void deleteArea(long areaId, int areaVersion){
		this.deleteArea(areaId, areaVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.common.delete",operType="3")
	public void deleteArea(long areaId, int areaVersion,SignatureLog signatureLog){
		MESBasicArea area = getArea(areaId);
		...
		checkDeletePermit(areaId);
		beforeDeleteArea(area);
		areaDao.delete(areaId, areaVersion);
		afterDeleteArea(area);
		areaDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/area/del", area,MESBasicAreaMneCode.class);
		List<MESBasicArea> params = new ArrayList<MESBasicArea>();
		...
		if(signatureLog != null ) {
			...
			businessKey=area.getCode();
			signatureLog.setTableId(area.getId());
			if(businessKey != null) {
	...
	public void checkDeletePermit(Long id) throws BAPException{
		String areaHql = "select count(area.id) as totalCoual from " + MESBasicArea.JPA_NAME + " as area where area.parentId=? and area.valid=true";
		List<Long> areaCount = areaDao.findByHql(areaHql, id);
		if (areaCount.get(0) > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.common.delete",operType="3")
	public void deleteArea(String areaIds){
		this.deleteArea(areaIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.common.delete",operType="3")
	public void deleteArea(String areaIds,SignatureLog signatureLog){
		deleteCollection(areaIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteArea(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_area#modelCode:MESBasic_1_area_Area#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="ec.engine.restore",operType="10")
	public void restoreArea(String areaIds){
		restoreCollection(areaIds);
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="ec.engine.restore",operType="10")
	public void restoreArea(long areaId){
		
		findBusinessKeyUsed(areaId);	//判断业务主键是否重复
		...
		checkDeletePermit(areaId);
		beforeRestoreArea(area);
		area.setValid(true);
		areaDao.update(area);
		afterRestoreArea(area);
		areaDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/area/add", area,MESBasicAreaMneCode.class);
		List<MESBasicArea> params = new ArrayList<MESBasicArea>();
		params.add(area);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long areaId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_area_Area");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicArea.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicArea.TABLE_NAME +" where id = ? )";
			List<Object> list =  areaDao.createNativeQuery(sql,areaId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseArea(List<MESBasicArea>  areas){
		for(MESBasicArea area:areas)  {
			saveArea(area, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseArea(List<MESBasicArea>  areas){
		for(MESBasicArea area:areas)  {
			saveArea(area, null);
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="ec.flow.save")
	public void saveArea(MESBasicArea area, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveArea(area, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="ec.flow.save")
	public void saveArea(MESBasicArea area, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveArea(area, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateAreaCodes(MESBasicArea area) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAreaCodes(area, false);
	}
	
	public void generateAreaCodes(MESBasicArea area, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateAreaSummarys(MESBasicArea area) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateAreaSummarys(area, false);
	}

	public void generateAreaSummarys(MESBasicArea area, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area")
	public void saveArea(MESBasicArea area,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveArea(area, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area")
	public void saveArea(MESBasicArea area,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_area";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_area");
		String url = null;
		if(area.getId() != null && area.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicArea.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_area/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicArea.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_area/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, area);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(area);
		beforeSaveArea(area, viewIsView);

		if (viewIsView) {
			areaDao.saveWithRevertVersion(area);
		} else {
			areaDao.save(area);
		}
		if(null != area && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicArea.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
		...
		dealDatagridsSave(area,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateAreaCodes(area, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateAreaSummarys(area, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveArea(area, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, area);
			}
		}
		List<MESBasicArea> params = new ArrayList<MESBasicArea>();
		areaDao.flush();
		areaDao.clear();
		area = areaDao.load(area.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicArea area){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area")
	public void mergeArea(MESBasicArea area, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(area);
		beforeSaveArea(area);
		areaDao.merge(area);
		if(null != area && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicArea.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
		...
		dealDatagridsSave(area,null,dgLists,dgDeleteIDs,assFileUploads);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicArea> findAreas(Page<MESBasicArea> page, Criterion... criterions) {
		return areaDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicAreaDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicAreaDealInfo> list=areaDao.createQuery("from " + MESBasicAreaDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicAreaDealInfo dealInfo) {
		areaDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicAreaDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) areaDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicAreaDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = areaDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = areaDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicAreaDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = areaDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	public List<String> getContainLower(String modelName, String fieldName, String exp, Object value, Boolean crossCompany) {
		...
		if(crossCompany) {
			layRecHql += " and cid = ?";
			return areaDao.createQuery(layRecHql, value, getCurrentCompanyId()).list();
		}
		return areaDao.createQuery(layRecHql, value).list();
	}
	
	private void dealListTreePageData(Page<MESBasicArea> areas, String viewCode){
		List<MESBasicArea> result = areas.getResult();
		if(null != result && !result.isEmpty()) {
			int rootPos = 0;
			MESBasicArea rootTree = result.get(0);
			...
			areas.setTreeToSurfaceMode(true);
			MESBasicArea areaTree = areaDao.buildTree("area" + viewCode, new ListPrepareData(areas), false);
			if(null != areaTree.getChildren() && !areaTree.getChildren().isEmpty()) {
				List<MESBasicArea> treeList = new ArrayList<MESBasicArea>();
				Tree.treeToList(areaTree, treeList, false);
				if(treeList.size() == result.size()) {
					areas.setResult(treeList);
					areas.setTreeToSurfaceMode(false);
				} 
			} 
		}
	}
	 ...
	 * 
	 */
	private class ListPrepareData implements PrepareData<MESBasicArea,Long> {
		private Page<MESBasicArea> areas;

		public ListPrepareData(Page<MESBasicArea> areas) {
			this.areas = areas;
		}

		@Override
			public MESBasicArea root() {
				MESBasicArea area = new MESBasicArea();
				area.setId(-1l);
				return area;
			}
			
			@Override
			public List<MESBasicArea> prepare(TreeDao<MESBasicArea, Long> dao) {
				return areas.getResult();
			}
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAreas(Page<MESBasicArea> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findAreas(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAreas(Page<MESBasicArea> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findAreas(Page<MESBasicArea> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicArea> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"area\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"area\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"area\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_area_Area" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_area_Area" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  areaDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  areaDao.createNativeQuery(powerOperateSql, "MESBasic_1_area").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"area\"","MESBasic_1_area",customCondition,"BEAM_AREAS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"area\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicAreaDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"area\".LAY_REC ASC, \"area\".SORT ASC");
				} else {
					orderPart.append(" \"area\".LAY_REC ASC, \"area\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) areaDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=areaDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = areaDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicArea> result = new ArrayList<MESBasicArea>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicArea.class,areaDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicArea.class,areaDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicArea area : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(area.getId(), "MESBasic_area_area");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(area.getId(), "MESBasic_area_area");
							if(null != documents && !documents.isEmpty()) {
								area.setDocument(documents.get(0));
								area.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(page.isExportFlag()){
					...
					if(exportMap.get("businessKey")!=null){
						try {
							...
							MESBasicArea area = new MESBasicArea();
							getBusKeyMethod = area.getClass().getMethod(getBusKeyMethodStr);
					...
					if(exportMap.get("mainDisplay")!=null){
						try {
							...
							MESBasicArea area = new MESBasicArea();
							getMainDisMethod = area.getClass().getMethod(getMainDisMethodStr);
						} catch (NoSuchMethodException | SecurityException e) {
							log.error(e.getMessage(),e);
						}
					}
					...
					for (MESBasicArea area : result) {
						...
						if(parentId != null && parentId.longValue() != new Long(-1).longValue()){
							for (MESBasicArea area2 : result) {
								if(area2.getId() != null && area2.getId().longValue() == parentId.longValue()){
									try {
										if(getBusKeyMethod != null){
											Object busKey = getBusKeyMethod.invoke(area2);
											area.setParentNodeBusinessKey(busKey.toString());
										}
										if(getMainDisMethod != null){
											Object mainDis = getMainDisMethod.invoke(area2);
											area.setParentNodeMainDisplay(mainDis.toString());
										}	
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_area")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_area")+"MESBasic_1_area".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_area",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicArea> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicArea> result) {
		View view = null;
		...
		Map<Long, MESBasicArea> rsMap = new HashMap<Long, MESBasicArea>();
		for (MESBasicArea area : result) {
			ids.add(area.getId());
			rsMap.put(area.getId(), area);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(areaDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = areaDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicArea area,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(area.getOwnerStaffId());
		ti.setOwnerPositionId(area.getOwnerPositionId());
		ti.setOwnerDepartmentId(area.getOwnerDepartmentId());
		
		ti.setCreateStaff(area.getCreateStaff());
		ti.setCreateStaffId(area.getCreateStaffId());
		ti.setCreateTime(area.getCreateTime());
		ti.setDeleteStaff(area.getDeleteStaff());
		ti.setDeleteStaffId(area.getDeleteStaffId());
		ti.setDeleteTime(area.getDeleteTime());
		ti.setCreateDepartmentId(area.getCreateDepartmentId());
		ti.setModifyStaff(area.getModifyStaff());
		ti.setModifyStaffId(area.getModifyStaffId());
		ti.setCreatePositionId(area.getCreatePositionId());
		ti.setPositionLayRec(area.getPositionLayRec());
		ti.setTableNo(area.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_area");
		ti.setTargetTableName("BEAM_AREAS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_area_Area" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  areaDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"area\"","MESBasic_1_area","","BEAM_AREAS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"area\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"area\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"area\"","MESBasic_1_area","","BEAM_AREAS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("area.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"area\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("area.mc\"").append(".AREA) not in ("+notinValue+")");
			}
			
			SQLQuery query = areaDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("area", "MESBasic_1_area", "area_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=areaDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicArea.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicArea.MODULE_CODE;}
	protected String getModelCode(){return MESBasicArea.MODEL_CODE;}
	
	@Override
	public List<MESBasicArea> getTreeChildren(Long parentId, String cond, Object... params) {
		MESBasicArea parent = areaDao.getTree("area_tree_" + parentId, parentId, new MESBasicAreaPrepareData(cond, params), false);
	...
	@Override
	public List<MESBasicArea> getTreeInfo(Map<Long, String> treeInfo, String condition, List<Object> params) {
		...
		for(String layRec : mapKeys) {
			...
			MESBasicArea areaTree = areaDao.getTree("MESBasicArea" + rootLayRec + "-", rootId, new PrepareData<MESBasicArea, Long>() {
				@Override
				public MESBasicArea root() {
					MESBasicArea area = new MESBasicArea();
					area.setId(-1l);
				...
				@Override
				public List<MESBasicArea> prepare(TreeDao<MESBasicArea, Long> dao) {
					String hql = "from " + MESBasicArea.JPA_NAME + " where (" + searchCondition + ") and valid = true " + conditionSql + " order by layRec asc, sort asc";
					return dao.findByHql(hql, paramsList.toArray());
				}
			}, false);
			List<MESBasicArea> treeChildren = new ArrayList<MESBasicArea>();
			Tree.treeToList(areaTree, treeChildren, true);
			treeList.addAll(treeChildren);
		}
		return treeList;
	}

	private class MESBasicAreaPrepareData implements PrepareData<MESBasicArea, Long> {
		private String cond;
		private Object[] params;
		
		public MESBasicAreaPrepareData(String cond, Object... params) {
			this.cond = cond;
			this.params = params;
		}
		...
		//@Override
		//public List<MESBasicArea> prepare(TreeDao<MESBasicArea, Long> dao) {
		//	String hql = "from " + MESBasicArea.JPA_NAME;
		//	if (null != cond && cond.trim().length() > 0) {
		//		hql += " where " + cond;
		//	}
		//	List<MESBasicArea> list = dao.findByHql(hql);
		//	return list;
		//}
		
		@Override
		public List<MESBasicArea> prepare(TreeDao<MESBasicArea, Long> dao) {
			List<MESBasicArea> list = null;
			List<MESBasicArea> result = new ArrayList<MESBasicArea>();
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicArea.class);
			if(params!=null&&params.length>0){
				...
				list = dao.findByHql("from " + MESBasicArea.JPA_NAME + " " + ((cond==null || cond.isEmpty()) ? "" : "where " + cond + " order by layRec asc, sort asc")) ;
			}
			for(MESBasicArea m : list){
				result.add(m);
				if(m.getParentId()!=-1){
					while(m.getParentId()!=-1){
						MESBasicArea parent = dao.get(m.getParentId());
	 ...
	 * @param area 模型实例
	 */
	public void checkUniqueConstraint(MESBasicArea area){
		if(null == area.getCode() || StringUtils.isEmpty(area.getCode().toString())){
			return;
		}
		if(area.getId()==null ){
			List<MESBasicArea> areas = areaDao.findByCriteria(Restrictions.eq("code", area.getCode()));
			if(areas!=null && !areas.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1524636490190", getCurrentUser().getLanguage()), area.getCode());
			}
		} else {
			List<MESBasicArea> areas = areaDao.findByCriteria(Restrictions.ne("id", area.getId()), Restrictions.eq("code", area.getCode()));
			if(areas!=null && !areas.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1524636490190", getCurrentUser().getLanguage()), area.getCode());
			}
	...
	@Override
	public List<MESBasicArea> getAreas(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicArea.class);
		if(params!=null){
			List<Type> types = new ArrayList<Type>();
			for(Object obj : params){
			...
			return areaDao.findByHql("from " + MESBasicArea.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return areaDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicArea> findAreasBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicArea.class);
	...
	@Override
	public List<MESBasicArea> findAreasByHql(String hql, Object... objects){
		
		return   areaDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicArea loadAreaByBussinessKey(MESBasicArea area){
		return loadAreaByBussinessKey(area.getCode());
	}
	
	@Override
	public Page<MESBasicArea> getAreas(Page<MESBasicArea> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicArea.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = areaDao.findByPage(page, sql);
			}else{
				page = areaDao.findAllByPage(page);
			}
			return page;
		}
		page = areaDao.findByPage(page, detachedCriteria);
		return page;
	}
	//==============DataGrid多选控件使用 start================
	
	@Override
	public void dealAreaLeaf(List<Long> areaIds) {
		if (areaIds != null && areaIds.size() > 0) {
			String hql = "update " + MESBasicArea.JPA_NAME + " as t1 set t1.leaf=1 where id in(:ids) and not exists (select t2.id from "
					+ MESBasicArea.JPA_NAME + " as t2 where t2.parentId = t1.id)";
			Query query = areaDao.createQuery(hql);
			query.setParameterList("ids", areaIds);
			query.executeUpdate();
		}
	...
	public void drag(long dragSource, long dragTarget, long sort){
		MESBasicArea source = getArea(dragSource);
		MESBasicArea oldParent = source.getParent();
		
		MESBasicArea target = null;
		if(dragTarget!=-1){
			target = getArea(dragTarget);
		} else {
			target = new MESBasicArea();
			target.setId(-1L);
		}
		areaDao.move(source, target);
	}
	
	private void dragArea(MESBasicArea parent, List<MESBasicArea> children){
		for(MESBasicArea child : children){
			...
			saveArea(child, null,null,null);
			List<MESBasicArea> subChildren = getTreeChildren(child.getId(), "");
			if(subChildren!=null && subChildren.size()>0){
				dragArea(child, subChildren);
			}
		}
	}
	
	public List<MESBasicArea> convertTree(List<MESBasicArea> areas){
		List<MESBasicArea> trees = new ArrayList<MESBasicArea>();
		Map<Long, MESBasicArea> map = new LinkedHashMap<Long, MESBasicArea>(areas.size());
		Map<Long, Boolean> isParent = new HashMap<Long, Boolean>();
		for (MESBasicArea node : areas) {
		...
		for (MESBasicArea node : areas) {
			if (null == node.getParentId() ||Long.parseLong(node.getParentId().toString()) == -1) {
				...
				MESBasicArea parent = map.get(node.getParentId());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicArea loadAreaByBussinessKey(Serializable bussinessKey){
		return areaDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicArea> findByProperty(String propertyName, Object object){
		return areaDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicArea findEntityByProperty(String propertyName, Object object){
		return areaDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicArea.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = areaDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_area_Area","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_area", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicArea area,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,areaDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicArea.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, area);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",area.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicArea.TABLE_NAME +" where valid = 1";
			list = areaDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicArea.TABLE_NAME;
			list = areaDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicArea.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = areaDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicAreaDaoImpl areaDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchArea(final List<MESBasicArea> insertObjs, final List<MESBasicArea> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchArea(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchArea(final List<MESBasicArea> insertObjs, final List<MESBasicArea> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1524635769096"),importCount));
		...
		Map<Long, MESBasicArea> oldUpdateObjsMap = new HashMap<Long, MESBasicArea>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicArea bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicArea> bizObjList = areaDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicArea bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicArea> nmeObjs = new ArrayList<MESBasicArea>();
		...
		long id=autoGeneratorID.getNextId(MESBasicArea.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicArea area:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicArea.TABLE_NAME,1,dbId);
			if(area.getCode() != null && !area.getCode().equals("") ){
				m.put(area.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		List<MESBasicArea> needChangeInsertObjList = new ArrayList<MESBasicArea>();
		if(insertNodeList != null && insertNodeList.size()>0 ){
			for(int i = 0; i < insertNodeList.size(); i++){
				...
				if(!needChange && !isRoot){
					MESBasicArea obj = insertObjs.get(i);
					...
					MESBasicArea parentObj = (MESBasicArea) insertNode.get(parentNode);
					*/
		...
		if(updateNodeList != null && updateNodeList.size()>0){
			for(int i = 0; i < updateNodeList.size(); i++){
				...
				MESBasicArea updateObj = updateObjs.get(i);
				...
				if(isRoot){
					...
					if(needChange!=null && !needChange){
						...
						MESBasicArea parentNode = (MESBasicArea) insertParentNode.get(parentNodeKey);
		...
		if(insertNodeList != null && insertNodeList.size()>0){
			for(int i = 0; i < insertNodeList.size(); i++){
				Map<String, Object> insertNode = insertNodeList.get(i);
				MESBasicArea insertObj = insertObjs.get(i);
		...
		if(updateObjs != null && updateObjs.size()>0 ){
			for(MESBasicArea obj : updateObjs){
				Object bus;
				try {
		...
		if(businessKey != null && mainDisplay != null && businessKey != mainDisplay  && paramList != null && paramList.size()>0){
			String hql = "select " + businessKey.getName() + "," + mainDisplay.getName() + ",id from " + MESBasicArea.JPA_NAME + " where " + businessKey.getName() + " in (:codes)";
			Query query =  areaDao.createQuery(hql);
			query.setParameterList("codes", paramList);
			List<Object[]> list = query.list();
			if(list.size() == updateObjs.size()){
				for(int i = 0 ; i < updateObjs.size();i++ ){
					MESBasicArea obj = updateObjs.get(i);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) areaDao.createNativeQuery(sql, "MESBasic_1_area_Area"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		areaDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicArea.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicArea area = updateObjs.get(updateIndex);
						Long id = area.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = area.getClass().getMethod(updateMethods.get(i));
								} catch (NoSuchMethodException | SecurityException e) {
									log.error(e.getMessage(),e);
								}
								Object obj = null;
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicArea.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicArea.TABLE_NAME + " order by id desc";
//					Query query = areaDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicArea area = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = area.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicAreaMneCode.class;
					...
					for(MESBasicArea area:updateObjs)  {
						Long id = area.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicArea area:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(area);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, area.getId());
		...
		areaDaoImpl.getSession().flush();
		areaDaoImpl.getSession().clear();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicArea bizObj = insertObjs.get(insertIndex);
			areaImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicArea bizObj = updateObjs.get(updateIndex);
			MESBasicArea oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			areaImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	private void changeFullPathName(List<Long> needChangeFullPathNameId, Map<Object, Object> needChangeFullPathName,final Property businessKey,Property mainDisplay){
		...
		sql.append(" from ");
		sql.append(MESBasicArea.TABLE_NAME);
		sql.append(" where ");
		...
		List<Object[]> lists =  areaDao.createNativeQuery(sqlStr).list();
		...
		areaDaoImpl.getSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
				...
				if(updateLists != null && updateLists.size()>0){
					String updateSql = "UPDATE " + MESBasicArea.TABLE_NAME + " SET FULL_PATH_NAME = ? WHERE " + businessKey.getColumnName() + " = ?";
					updateps = conn.prepareStatement(updateSql);
	...
	private Map<String, Object> buildInsertObjNode(List<Map<String, Object>> insertNodeList, List<MESBasicArea> insertObjs, 
			...
			for(int i =0; i<insertNodeList.size(); i++){
				...
				if(insertBuildedObj.get(busKey)!=null){
					...
					if(isRoot){
						...
						String parentNode = insertNode.get("parentNode").toString();
						MESBasicArea parentObj = (MESBasicArea) insertBuildedObj.get(parentNode);
	...
	public String getViewName(String modelCode){
		...
		String viewName = areaDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				areaDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = areaDao.createNativeQuery(sql, "MESBasic_1_area_Area").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = areaDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = areaDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = areaDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?areaDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  areaDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,areaDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="ec.flow.save")
	public void saveArea(MESBasicArea area, DataGridService dataGridService){
		saveArea(area, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="ec.flow.save")
	public void saveArea(MESBasicArea area, DataGridService dataGridService, String viewCode){
		saveArea(area, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area")
	public void saveArea(MESBasicArea area, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveArea(area, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area")
	public void saveArea(MESBasicArea area, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_area";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area")
	public Map<Object, Object> areaDataGridImport(MESBasicArea area, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = area.getVirtualId();
		
		area = areaDao.load(area.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, area.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = area.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(area);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, area.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area")
	public void mergeArea(MESBasicArea area, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(area);
		beforeSaveArea(area);
		areaDao.merge(area);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_area",model="MESBasic_1_area_Area",desc="foundation.batch.save",operType="4")
	public void batchSaveArea(MESBasicArea area, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_area";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_area");
	...
	@Override
	public Page<MESBasicArea> getByPage(Page<MESBasicArea> page,DetachedCriteria detachedCriteria){
		return areaDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_area").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveArea(MESBasicArea area, Object... objects){
		checkUniqueConstraint(area);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveArea(MESBasicArea area, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportArea(List<MESBasicArea> areas, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportArea(List<MESBasicArea> areas){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportArea(List<MESBasicArea> areas, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportArea(List<MESBasicArea> areas){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteArea(MESBasicArea area){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteArea(MESBasicArea area){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreArea(MESBasicArea area){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreArea(MESBasicArea area){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicArea area, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicArea area, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportArea(List<MESBasicArea> insertObjs, List<MESBasicArea> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportArea,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportArea(List<MESBasicArea> insertObjs, List<MESBasicArea> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportArea,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_area_Area,MESBasic_1) */
// 自定义代码
    @Override
	public String isLeaf(String areaID) {
		// TODO Auto-generated method stub
		String flag="";
		String sql="select * from BEAM_AREAS where Leaf='0' and Id="+areaID;
		List<Object[]> areaList = areaDao.createNativeQuery(sql).list(); 
		if(areaList.size()>0){
			flag="0";
		}
		return flag;
    ...
    @Override
	public String hasBaseInfo(String areaID) {
		// TODO Auto-generated method stub
        String flag="";
        String sql="select * from EAM_BASEINFO where VALID='1' and INSTALL_PLACE="+areaID;
        List<Object[]> baseInfoList = areaDao.createNativeQuery(sql).list(); 
