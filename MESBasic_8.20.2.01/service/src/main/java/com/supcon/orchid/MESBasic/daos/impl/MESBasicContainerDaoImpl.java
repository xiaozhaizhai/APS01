...
@Repository
public class MESBasicContainerDaoImpl extends ExtendedGenericDaoImpl<MESBasicContainer, Long> implements MESBasicContainerDao {
	@Override
	...
	@Override
	public MESBasicContainerDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicContainerDealInfo dealInfo = new MESBasicContainerDealInfo();
	...
	@Override
	public MESBasicContainerDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicContainer mainObj, Staff dealStaff) {
		MESBasicContainerDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicContainerDealInfo.class);
		...
		List<MESBasicContainerDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_container_Container,MESBasic_1) */
// 自定义代码
