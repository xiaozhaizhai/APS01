...
/* CUSTOM CODE START(action,import,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicPurchGroupAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicPurchGroup> {
	...
	private MESBasicPurchGroup purchGroup;
	...
	//导出excel时,主模型内容
	List<MESBasicPurchGroup> queryResults = new ArrayList<MESBasicPurchGroup>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/purch_Group/purchGroup/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/purch_Group/purchGroup/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_purch_Group_PurchGroup" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicPurchGroup> purchGroups;
	
	public List<MESBasicPurchGroup> getPurchGroups() {
		return purchGroups;
	}
	...
	private Page<MESBasicPurchGroup> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			purchGroup = new MESBasicPurchGroup();
		} else {
			if(id > 0){
				purchGroup = purchGroupService.getPurchGroup(id, viewCode);
			}
		}
	...
	public Page<MESBasicPurchGroup> getPage() {
		if (null == page) {
			page = new Page<MESBasicPurchGroup>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicPurchGroup getPurchGroup() {
		return purchGroup;
	}		

	public void setPurchGroup(MESBasicPurchGroup purchGroup) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/purch_Group/purchGroup/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = purchGroupService.getPurchGroupAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/purch_Group/purchGroup/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1470291767311
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String purchGroupReferenceCopy() throws Exception{
			MESBasicPurchGroup oldPurchGroup = null;
			if(isEditNew()){
				...
				oldPurchGroup = new MESBasicPurchGroup();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,purchGroup.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, purchGroup.getId());
				...
				oldPurchGroup = purchGroupService.getPurchGroup(id);
			}
			
			
			purchGroup = purchGroupService.getPurchGroup(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_purch_Group_PurchGroup");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicPurchGroup.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPurchGroup, null);
						Method setValue = MESBasicPurchGroup.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(purchGroup, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			purchGroup.setCreateDepartmentId(oldPurchGroup.getCreateDepartmentId());
			purchGroup.setCreatePositionId(oldPurchGroup.getCreatePositionId());
			purchGroup.setCreateStaffId(oldPurchGroup.getCreateStaffId());
			purchGroup.setOwnerDepartmentId(oldPurchGroup.getOwnerDepartmentId());
			purchGroup.setOwnerPositionId(oldPurchGroup.getOwnerPositionId());
			purchGroup.setOwnerStaffId(oldPurchGroup.getOwnerStaffId());
			purchGroup.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroup", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=purchGroupService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return purchGroupReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			purchGroup = purchGroupService.getPurchGroup(this.purchGroup.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,purchGroup.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/purch_Group/purchGroup/purchGroupList", results = { @Result(type = FREEMARKER, location = "purchGroupList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = purchGroupService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(purchGroupService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(purchGroupService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_purch_Group_PurchGroup", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/purch_Group/purchGroup/purchGroupRef", results = { @Result(type = FREEMARKER, location = "purchGroupRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/purch_Group/purchGroup/purchGroupList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.purchGroupName,result.purchGroupMemo,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="purchGroupList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"purchGroup\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"purchGroup\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"purchGroup\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"purchGroup\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/purch_Group/purchGroup/purchGroupList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/purch_Group/purchGroup/purchGroupRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.purchGroupName,result.purchGroupMemo,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="purchGroupRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/purch_Group/purchGroup/allTempleteDownload", results = { @Result(name="excel",type="excel",location="purchGroupList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_purch_Group_PurchGroup");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		purchGroupService.findDataGridPage(dg,dgclass,getDgPage(),purchGroup,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(purchGroup.getId() != null && purchGroup.getId() != -1){
				purchGroup = purchGroupService.getPurchGroup(purchGroup.getId());
			}
			purchGroupService.findDataGridPage(dg,dgclass,getDgPage(),purchGroup,searchCondition,params);
	...
	private void save() throws Exception {
		...
		purchGroupService.savePurchGroup(purchGroup,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "submit");
		bap_validate_datagrids=purchGroupService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", purchGroup.getId());
		...
		fileuploads.put("linkId", purchGroup.getId());
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroup", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/purch_Group/purchGroup/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		purchGroupService.deletePurchGroup(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroup", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroup/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "restore");
		purchGroupService.restorePurchGroup(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroup", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/purch_Group/purchGroup/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/purch_Group/purchGroup/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "showDealInfoData");
		if (null != tableInfoId) {
			int count=purchGroupService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = purchGroupService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = purchGroupService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroup", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicPurchGroup> findExportDataInfos(Page<MESBasicPurchGroup> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = purchGroupService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = purchGroupService.getAssProperty(p.getCode());	
									//String assModelCode = purchGroupService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			purchGroupService.findPurchGroups(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			purchGroupService.findPurchGroups(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicPurchGroup> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicPurchGroup result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".USERREPORT_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.purchGroup || null == this.purchGroup.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/purch_Group/purchGroup/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroup", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/purch_Group/purchGroup/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/purch_Group/purchGroup/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroup/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicPurchGroupService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_purch_Group", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_purch_Group");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_purch_Group_PurchGroup", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroup/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicPurchGroupService"
						...
						, "com.supcon.orchid.MESBasic.services.MESBasicPurchGroupMemberService"
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_purch_Group_PurchGroup", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_purch_Group_PurchGroup", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/purch_Group/purchGroup/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/purch_Group/purchGroup/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_purch_Group_PurchGroup";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_purch_Group_PurchGroup", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/purch_Group/purchGroup/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
		_auditLog.setColumnStringA(null == purchGroup.getPurchGroupName() ? "" : purchGroup.getPurchGroupName().toString());
		_auditLog.setColumnStringC(purchGroup.getId().toString());
		_auditLog.setColumnStringD(MESBasicPurchGroup.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_purch_Group");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicPurchGroupAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, purchGroup,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, purchGroup,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,purchGroup);
		...
		printlog.setEntity("MESBasic_1_purch_Group");
		...
		printlog.setBusinessId(purchGroup.getId());
		printlog.setBussinessKey(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
		printlog.setMainDisplay(null == purchGroup.getPurchGroupName() ? "" : purchGroup.getPurchGroupName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroup/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(purchGroup,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/purch_Group/purchGroup/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(purchGroupService.getPurchGroup(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/purch_Group/purchGroup/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/purch_Group/purchGroup/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, purchGroup,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				purchGroup = purchGroupService.getPurchGroup(Long.parseLong(arrId[i]));
				if(null != purchGroup){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == purchGroup.getPurchGroupName() ? "" : "," + purchGroup.getPurchGroupName().toString()));
					} else {
						_auditLog.setColumnStringA(null == purchGroup.getPurchGroupName() ? "" : purchGroup.getPurchGroupName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == purchGroup.getId() ? "" : "," + purchGroup.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == purchGroup.getId() ? "" : "," + purchGroup.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, purchGroup,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,purchGroup);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/purch_Group/purchGroup/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicPurchGroup> findExportPageInfos(Page<MESBasicPurchGroup> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_purch_Group" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = purchGroupService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicPurchGroup queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = purchGroupService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = purchGroupService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_purch_Group_PurchGroup");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_purch_Group_PurchGroup")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_purch_Group_PurchGroup");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_purch_Group_PurchGroup");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = purchGroupService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
