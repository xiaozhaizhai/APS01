...
@Repository
public class MESBasicItemRecordDaoImpl extends ExtendedGenericDaoImpl<MESBasicItemRecord, Long> implements MESBasicItemRecordDao {
	@Override
	...
	@Override
	public MESBasicItemRecordDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicItemRecordDealInfo dealInfo = new MESBasicItemRecordDealInfo();
	...
	@Override
	public MESBasicItemRecordDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicItemRecord mainObj, Staff dealStaff) {
		MESBasicItemRecordDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicItemRecordDealInfo.class);
		...
		List<MESBasicItemRecordDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
