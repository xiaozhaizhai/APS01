...
/* CUSTOM CODE START(action,import,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicInvolveItemsetupsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicInvolveItemsetups> {
	...
	private MESBasicInvolveItemsetups involveItemsetups;
	...
	//导出excel时,主模型内容
	List<MESBasicInvolveItemsetups> queryResults = new ArrayList<MESBasicInvolveItemsetups>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/routine/involveItemsetups/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/routine/involveItemsetups/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_routine_InvolveItemsetups" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicInvolveItemsetups> involveItemsetupss;
	
	public List<MESBasicInvolveItemsetups> getInvolveItemsetupss() {
		return involveItemsetupss;
	}
	...
	private Page<MESBasicInvolveItemsetups> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			involveItemsetups = new MESBasicInvolveItemsetups();
		} else {
			if(id > 0){
				involveItemsetups = involveItemsetupsService.getInvolveItemsetups(id, viewCode);
			}
		}
	...
	public Page<MESBasicInvolveItemsetups> getPage() {
		if (null == page) {
			page = new Page<MESBasicInvolveItemsetups>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicInvolveItemsetups getInvolveItemsetups() {
		return involveItemsetups;
	}		

	public void setInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups) {
		this.involveItemsetups = involveItemsetups;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/routine/involveItemsetups/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = involveItemsetupsService.getInvolveItemsetupsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/routine/involveItemsetups/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1426644977327
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String involveItemsetupsReferenceCopy() throws Exception{
			MESBasicInvolveItemsetups oldInvolveItemsetups = null;
			if(isEditNew()){
				...
				oldInvolveItemsetups = new MESBasicInvolveItemsetups();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,involveItemsetups.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, involveItemsetups.getId());
				...
				oldInvolveItemsetups = involveItemsetupsService.getInvolveItemsetups(id);
			}
			
			
			involveItemsetups = involveItemsetupsService.getInvolveItemsetups(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_routine_InvolveItemsetups");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicInvolveItemsetups.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldInvolveItemsetups, null);
						Method setValue = MESBasicInvolveItemsetups.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(involveItemsetups, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			involveItemsetups.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_routine_InvolveItemsetups", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=involveItemsetupsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return involveItemsetupsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			involveItemsetups = involveItemsetupsService.getInvolveItemsetups(this.involveItemsetups.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,involveItemsetups.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			involveItemsetupsService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = involveItemsetupsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(involveItemsetupsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(involveItemsetupsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_routine_InvolveItemsetups", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"involveItemsetups\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"involveItemsetups\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"involveItemsetups\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"involveItemsetups\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			involveItemsetupsService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		involveItemsetupsService.findDataGridPage(dg,dgclass,getDgPage(),involveItemsetups,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(involveItemsetups.getId() != null && involveItemsetups.getId() != -1){
				involveItemsetups = involveItemsetupsService.getInvolveItemsetups(involveItemsetups.getId());
			}
			involveItemsetupsService.findDataGridPage(dg,dgclass,getDgPage(),involveItemsetups,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == involveItemsetups.getId()) {
		...
		involveItemsetupsService.saveInvolveItemsetups(involveItemsetups,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_routine_InvolveItemsetups", "submit");
		bap_validate_datagrids=involveItemsetupsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", involveItemsetups.getId());
		...
		fileuploads.put("linkId", involveItemsetups.getId());
		bapAfterCustomMethod("MESBasic_1_routine_InvolveItemsetups", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/routine/involveItemsetups/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_InvolveItemsetups", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		involveItemsetupsService.deleteInvolveItemsetups(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_routine_InvolveItemsetups", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/involveItemsetups/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_InvolveItemsetups", "restore");
		involveItemsetupsService.restoreInvolveItemsetups(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_routine_InvolveItemsetups", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicInvolveItemsetups> findExportDataInfos(Page<MESBasicInvolveItemsetups> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = involveItemsetupsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = involveItemsetupsService.getAssProperty(p.getCode());	
									//String assModelCode = involveItemsetupsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			involveItemsetupsService.findInvolveItemsetupss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			involveItemsetupsService.findInvolveItemsetupss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicInvolveItemsetups> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicInvolveItemsetups result : results){
	...
	public boolean isEditNew(){
		//return null == this.involveItemsetups || null == this.involveItemsetups.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/routine/involveItemsetups/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_InvolveItemsetups", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_routine_InvolveItemsetups", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/routine/involveItemsetups/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_routine_InvolveItemsetups", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/routine/involveItemsetups/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/involveItemsetups/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicInvolveItemsetupsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_routine_InvolveItemsetups", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_routine_InvolveItemsetups", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/involveItemsetups/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_routine_InvolveItemsetups", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_routine_InvolveItemsetups", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/routine/involveItemsetups/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_InvolveItemsetups", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/routine/involveItemsetups/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_routine_InvolveItemsetups";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_routine_InvolveItemsetups", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/routine/involveItemsetups/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
		_auditLog.setColumnStringA(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
		_auditLog.setColumnStringC(involveItemsetups.getId().toString());
		_auditLog.setColumnStringD(MESBasicInvolveItemsetups.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicInvolveItemsetupsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, involveItemsetups,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, involveItemsetups,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,involveItemsetups);
		...
		printlog.setBusinessId(involveItemsetups.getId());
		printlog.setBussinessKey(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
		printlog.setMainDisplay(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/routine/involveItemsetups/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(involveItemsetups,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/involveItemsetups/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(involveItemsetupsService.getInvolveItemsetups(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/routine/involveItemsetups/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/routine/involveItemsetups/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, involveItemsetups,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				involveItemsetups = involveItemsetupsService.getInvolveItemsetups(Long.parseLong(arrId[i]));
				if(null != involveItemsetups){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == involveItemsetups.getId() ? "" : "," + involveItemsetups.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == involveItemsetups.getId() ? "" : "," + involveItemsetups.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == involveItemsetups.getId() ? "" : "," + involveItemsetups.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, involveItemsetups,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,involveItemsetups);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//involveItemsetupsService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/routine/involveItemsetups/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_routine_InvolveItemsetups");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_routine_InvolveItemsetups")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_routine_InvolveItemsetups");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_routine_InvolveItemsetups");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = involveItemsetupsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
