...
import com.supcon.orchid.MESBasic.services.MESBasicFlowRecordImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicFlowRecordDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_flowRecordService")
@Transactional
public class MESBasicFlowRecordServiceImpl  extends BaseServiceImpl<MESBasicFlowRecord> implements MESBasicFlowRecordService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicFlowRecord> bapBaseService;
	...
	@Autowired
	private MESBasicFlowRecordImportService flowRecordImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicFlowRecord.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  flowRecordDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicFlowRecord.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = flowRecordDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicFlowRecord.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicFlowRecord.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  flowRecordDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicFlowRecord.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicFlowRecord.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicFlowRecord> list = (List<MESBasicFlowRecord>) flowRecordDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFlowRecord getFlowRecord(long id){
		return getFlowRecord(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFlowRecord getFlowRecord(long id, String viewCode){
		MESBasicFlowRecord flowRecord = flowRecordDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFlowRecordAsJSON(long id, String include){
		MESBasicFlowRecord flowRecord = flowRecordDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(flowRecord, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.common.delete",operType="3")
	public void deleteFlowRecord(MESBasicFlowRecord flowRecord){
		if(null != flowRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFlowRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
			AuditUtil.setColumnStringA(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
			AuditUtil.setColumnStringC(flowRecord.getId().toString());
		}
		beforeDeleteFlowRecord(flowRecord);
		flowRecordDao.deletePhysical(flowRecord);
		afterDeleteFlowRecord(flowRecord);
		flowRecordDao.flush();
		List<MESBasicFlowRecord> params = new ArrayList<MESBasicFlowRecord>();
		params.add(flowRecord);
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.common.delete",operType="3")
	public void deleteFlowRecord(List<Long> flowRecordIds){
		deleteFlowRecord(flowRecordIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.common.delete",operType="3")
	public void deleteFlowRecord(List<Long> flowRecordIds, String eventTopic) {
		List<MESBasicFlowRecord> flowRecords = new ArrayList<MESBasicFlowRecord>();
		for(Long flowRecordId : flowRecordIds){
			MESBasicFlowRecord flowRecord = getFlowRecord(flowRecordId);
			flowRecords.add(flowRecord);
			if(null != flowRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFlowRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == flowRecord.getId() ? "" : "," + flowRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == flowRecord.getId() ? "" : "," + flowRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == flowRecord.getId() ? "" : "," + flowRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
				}
			}
		}
		
		for(MESBasicFlowRecord flowRecord : flowRecords){
			beforeDeleteFlowRecord(flowRecord);
		}
		...
		if(flowRecords != null && flowRecords.size() > 0 ){
			for(MESBasicFlowRecord flowRecord : flowRecords){
				...
				java.lang.reflect.Field[] fields = MESBasicFlowRecord.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(flowRecord));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),flowRecord, flowRecord.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(flowRecordIds != null && flowRecordIds.size() > 0) {
			String hql = "delete " + MESBasicFlowRecord.JPA_NAME + " where id in(:ids)";
			Query query = flowRecordDao.createQuery(hql);
			query.setParameterList("ids", flowRecordIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicFlowRecord flowRecordz : flowRecords){
				flowRecordz.setValid(false);
				List<MESBasicFlowRecord> params = new ArrayList<MESBasicFlowRecord>();
				params.add(flowRecordz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_container");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.common.delete",operType="3")
	public void deleteFlowRecord(long flowRecordId, int flowRecordVersion){
		this.deleteFlowRecord(flowRecordId, flowRecordVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.common.delete",operType="3")
	public void deleteFlowRecord(long flowRecordId, int flowRecordVersion,SignatureLog signatureLog){
		MESBasicFlowRecord flowRecord = getFlowRecord(flowRecordId);
		if(flowRecord != null && !flowRecord.isValid()){
		...
		beforeDeleteFlowRecord(flowRecord);
		flowRecordDao.delete(flowRecordId, flowRecordVersion);
		afterDeleteFlowRecord(flowRecord);
		flowRecordDao.flush();
		List<MESBasicFlowRecord> params = new ArrayList<MESBasicFlowRecord>();
		params.add(flowRecord);
		...
		if(signatureLog != null ) {
			...
			businessKey=flowRecord.getId();
			signatureLog.setTableId(flowRecord.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.common.delete",operType="3")
	public void deleteFlowRecord(String flowRecordIds){
		this.deleteFlowRecord(flowRecordIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.common.delete",operType="3")
	public void deleteFlowRecord(String flowRecordIds,SignatureLog signatureLog){
		deleteCollection(flowRecordIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFlowRecord(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_container#modelCode:MESBasic_1_container_FlowRecord#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="ec.engine.restore",operType="10")
	public void restoreFlowRecord(String flowRecordIds){
		restoreCollection(flowRecordIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFlowRecord(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="ec.engine.restore",operType="10")
	public void restoreFlowRecord(long flowRecordId){
		
		findBusinessKeyUsed(flowRecordId);	//判断业务主键是否重复
		...
		beforeRestoreFlowRecord(flowRecord);
		flowRecord.setValid(true);
		flowRecordDao.update(flowRecord);
		afterRestoreFlowRecord(flowRecord);
		List<MESBasicFlowRecord> params = new ArrayList<MESBasicFlowRecord>();
		params.add(flowRecord);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long flowRecordId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_container_FlowRecord");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicFlowRecord.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicFlowRecord.TABLE_NAME +" where id = ? )";
			List<Object> list =  flowRecordDao.createNativeQuery(sql,flowRecordId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFlowRecord(List<MESBasicFlowRecord>  flowRecords){
		for(MESBasicFlowRecord flowRecord:flowRecords)  {
			saveFlowRecord(flowRecord, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFlowRecord(List<MESBasicFlowRecord>  flowRecords){
		for(MESBasicFlowRecord flowRecord:flowRecords)  {
			saveFlowRecord(flowRecord, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="ec.flow.save")
	public void saveFlowRecord(MESBasicFlowRecord flowRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFlowRecord(flowRecord, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="ec.flow.save")
	public void saveFlowRecord(MESBasicFlowRecord flowRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFlowRecord(flowRecord, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFlowRecordCodes(MESBasicFlowRecord flowRecord) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFlowRecordCodes(flowRecord, false);
	}
	
	public void generateFlowRecordCodes(MESBasicFlowRecord flowRecord, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFlowRecordSummarys(MESBasicFlowRecord flowRecord) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFlowRecordSummarys(flowRecord, false);
	}

	public void generateFlowRecordSummarys(MESBasicFlowRecord flowRecord, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord")
	public void saveFlowRecord(MESBasicFlowRecord flowRecord,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFlowRecord(flowRecord, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord")
	public void saveFlowRecord(MESBasicFlowRecord flowRecord,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_container");
		String url = null;
		if(flowRecord.getId() != null && flowRecord.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFlowRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, flowRecord);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(flowRecord);
		beforeSaveFlowRecord(flowRecord, viewIsView);

		if (viewIsView) {
			flowRecordDao.saveWithRevertVersion(flowRecord);
		} else {
			if(isNew)
				flowRecordDao.save(flowRecord);
			else
				flowRecordDao.merge(flowRecord);
		}
		if(null != flowRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFlowRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
			AuditUtil.setColumnStringA(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
		...
		dealDatagridsSave(flowRecord,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFlowRecordCodes(flowRecord, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFlowRecordSummarys(flowRecord, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFlowRecord(flowRecord, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, flowRecord);
			}
		}
		List<MESBasicFlowRecord> params = new ArrayList<MESBasicFlowRecord>();
		flowRecordDao.flush();
		flowRecordDao.clear();
		flowRecord = flowRecordDao.load(flowRecord.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicFlowRecord flowRecord){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("flowRecord", flowRecord);
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord")
	public void mergeFlowRecord(MESBasicFlowRecord flowRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(flowRecord);
		beforeSaveFlowRecord(flowRecord);
		flowRecordDao.merge(flowRecord);
		if(null != flowRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFlowRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
			AuditUtil.setColumnStringA(null == flowRecord.getId() ? "" : flowRecord.getId().toString());
		...
		dealDatagridsSave(flowRecord,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFlowRecord(flowRecord);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicFlowRecord> findFlowRecords(Page<MESBasicFlowRecord> page, Criterion... criterions) {
		return flowRecordDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFlowRecords(Page<MESBasicFlowRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFlowRecords(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFlowRecords(Page<MESBasicFlowRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFlowRecords(Page<MESBasicFlowRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicFlowRecord> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"flowRecord\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"flowRecord\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"flowRecord\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"flowRecord\".CONTAINER_ID IN ( SELECT ID FROM MESBASIC_CONTAINERS \"container\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  flowRecordDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  flowRecordDao.createNativeQuery(powerOperateSql, "MESBasic_1_container").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"flowRecord\".LAY_REC ASC, \"flowRecord\".SORT ASC");
				} else {
					orderPart.append(" \"flowRecord\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) flowRecordDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=flowRecordDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = flowRecordDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicFlowRecord> result = new ArrayList<MESBasicFlowRecord>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicFlowRecord.class,flowRecordDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicFlowRecord.class,flowRecordDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicFlowRecord flowRecord : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(flowRecord.getId(), "MESBasic_container_flowRecord");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(flowRecord.getId(), "MESBasic_container_flowRecord");
							if(null != documents && !documents.isEmpty()) {
								flowRecord.setDocument(documents.get(0));
								flowRecord.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicFlowRecord> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicFlowRecord> result) {
		View view = null;
		...
		Map<Long, MESBasicFlowRecord> rsMap = new HashMap<Long, MESBasicFlowRecord>();
		for (MESBasicFlowRecord flowRecord : result) {
			ids.add(flowRecord.getId());
			rsMap.put(flowRecord.getId(), flowRecord);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(flowRecordDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = flowRecordDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_container_FlowRecord" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_container_FlowRecord" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  flowRecordDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"flowRecord\"","MESBasic_1_container","","MESBASIC_FLOW_RECORDS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"flowRecord\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"flowRecord\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"flowRecord\"","MESBasic_1_container","","MESBASIC_FLOW_RECORDS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_container_FlowRecord" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("flowRecord.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"flowRecord\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"flowRecord\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"flowRecord\".CID = 1 OR \"flowRecord\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("flowRecord.mc\"").append(".FLOW_RECORD) not in ("+notinValue+")");
			}
			
			SQLQuery query = flowRecordDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=flowRecordDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicFlowRecord.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicFlowRecord.MODULE_CODE;}
	protected String getModelCode(){return MESBasicFlowRecord.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param flowRecord 模型实例
	 */
	public void checkUniqueConstraint(MESBasicFlowRecord flowRecord){
	}
	
	
	@Override
	public List<MESBasicFlowRecord> getFlowRecords(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicFlowRecord.class);
		if(params!=null){
			...
			return flowRecordDao.findByHql("from " + MESBasicFlowRecord.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return flowRecordDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicFlowRecord> findFlowRecordsByHql(String hql, Object... objects){
		...
		return   flowRecordDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicFlowRecord loadFlowRecordByBussinessKey(MESBasicFlowRecord flowRecord){
		return loadFlowRecordByBussinessKey(flowRecord.getId());
	}
	
	@Override
	public Page<MESBasicFlowRecord> getFlowRecords(Page<MESBasicFlowRecord> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicFlowRecord.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = flowRecordDao.findByPage(page, sql);
			}else{
				page = flowRecordDao.findAllByPage(page);
			}
			return page;
		}
		page = flowRecordDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicFlowRecord> findByTreeDataGridContainer(List<MESBasicFlowRecord> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicFlowRecord flowRecord : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridContainer(Page<MESBasicFlowRecord> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicFlowRecord> flowRecords = dgPage.getResult();
			for(MESBasicFlowRecord flowRecord : flowRecords){
				
			}
			dgPage.setResult(flowRecords);
		}
	...
	public MESBasicFlowRecord loadFlowRecordByBussinessKey(Serializable bussinessKey){
		return flowRecordDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicFlowRecord> findByProperty(String propertyName, Object object){
		return flowRecordDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicFlowRecord findEntityByProperty(String propertyName, Object object){
		return flowRecordDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicFlowRecord.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = flowRecordDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_container_FlowRecord","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicFlowRecord flowRecord,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,flowRecordDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicFlowRecord.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, flowRecord);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",flowRecord.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicFlowRecord.TABLE_NAME +" where valid = 1";
			list = flowRecordDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicFlowRecord.TABLE_NAME;
			list = flowRecordDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicFlowRecord.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = flowRecordDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicFlowRecordDaoImpl flowRecordDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchFlowRecord(final List<MESBasicFlowRecord> insertObjs, final List<MESBasicFlowRecord> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchFlowRecord(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFlowRecord(final List<MESBasicFlowRecord> insertObjs, final List<MESBasicFlowRecord> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1555293134923"),importCount));
		...
		Map<Long, MESBasicFlowRecord> oldUpdateObjsMap = new HashMap<Long, MESBasicFlowRecord>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicFlowRecord bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicFlowRecord> bizObjList = flowRecordDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicFlowRecord bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicFlowRecord> nmeObjs = new ArrayList<MESBasicFlowRecord>();
		...
		long id=autoGeneratorID.getNextId(MESBasicFlowRecord.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicFlowRecord flowRecord:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicFlowRecord.TABLE_NAME,1,dbId);
			if(flowRecord.getId() != null ){
				m.put(flowRecord.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) flowRecordDao.createNativeQuery(sql, "MESBasic_1_container_FlowRecord"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		flowRecordDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicFlowRecord.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicFlowRecord flowRecord = updateObjs.get(updateIndex);
						Long id = flowRecord.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = flowRecord.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicFlowRecord.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicFlowRecord.TABLE_NAME + " order by id desc";
//					Query query = flowRecordDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicFlowRecord flowRecord = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = flowRecord.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		flowRecordDaoImpl.getSession().flush();
		flowRecordDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicFlowRecord bizObj = insertObjs.get(insertIndex);
			flowRecordImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicFlowRecord bizObj = updateObjs.get(updateIndex);
			MESBasicFlowRecord oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			flowRecordImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicFlowRecord> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicFlowRecord> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) flowRecordDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicFlowRecord> result = getResult(page, query, new PendingResultTransformer(MESBasicFlowRecord.class,flowRecordDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicFlowRecord model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = flowRecordDao.createNativeQuery(sql, "MESBasic_1_container_FlowRecord").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = flowRecordDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = flowRecordDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = flowRecordDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?flowRecordDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  flowRecordDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,flowRecordDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="ec.flow.save")
	public void saveFlowRecord(MESBasicFlowRecord flowRecord, DataGridService dataGridService){
		saveFlowRecord(flowRecord, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="ec.flow.save")
	public void saveFlowRecord(MESBasicFlowRecord flowRecord, DataGridService dataGridService, String viewCode){
		saveFlowRecord(flowRecord, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord")
	public void saveFlowRecord(MESBasicFlowRecord flowRecord, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFlowRecord(flowRecord, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord")
	public void saveFlowRecord(MESBasicFlowRecord flowRecord, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_container";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_container");
		String url = null;
		if(null != flowRecord.getId() && flowRecord.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicFlowRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord")
	public Map<Object, Object> flowRecordDataGridImport(MESBasicFlowRecord flowRecord, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = flowRecord.getVirtualId();
		
		flowRecord = flowRecordDao.load(flowRecord.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, flowRecord.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = flowRecord.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(flowRecord);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, flowRecord.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord")
	public void mergeFlowRecord(MESBasicFlowRecord flowRecord, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(flowRecord);
		beforeSaveFlowRecord(flowRecord);
		flowRecordDao.merge(flowRecord);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_FlowRecord",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicFlowRecord> getByPage(Page<MESBasicFlowRecord> page,DetachedCriteria detachedCriteria){
		return flowRecordDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFlowRecord(MESBasicFlowRecord flowRecord, Object... objects){
		checkUniqueConstraint(flowRecord);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFlowRecord(MESBasicFlowRecord flowRecord, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFlowRecord(MESBasicFlowRecord flowRecord){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFlowRecord(MESBasicFlowRecord flowRecord){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFlowRecord(MESBasicFlowRecord flowRecord){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFlowRecord(MESBasicFlowRecord flowRecord){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicFlowRecord flowRecord, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicFlowRecord flowRecord, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportFlowRecord(List<MESBasicFlowRecord> insertObjs, List<MESBasicFlowRecord> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFlowRecord,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportFlowRecord(List<MESBasicFlowRecord> insertObjs, List<MESBasicFlowRecord> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFlowRecord,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_container_FlowRecord,MESBasic_1) */
// 自定义代码
