...
/* CUSTOM CODE START(action,import,MESBasic_1_container_MaintainLog,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicMaintainLogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicMaintainLog> {
	...
	private MESBasicMaintainLog maintainLog;
	...
	//导出excel时,主模型内容
	List<MESBasicMaintainLog> queryResults = new ArrayList<MESBasicMaintainLog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/maintainLog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/maintainLog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_container_MaintainLog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicMaintainLog> maintainLogs;
	
	public List<MESBasicMaintainLog> getMaintainLogs() {
		return maintainLogs;
	}
	...
	private Page<MESBasicMaintainLog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			maintainLog = new MESBasicMaintainLog();
		} else {
			if(id > 0){
				maintainLog = maintainLogService.getMaintainLog(id, viewCode);
			}
		}
	...
	public Page<MESBasicMaintainLog> getPage() {
		if (null == page) {
			page = new Page<MESBasicMaintainLog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicMaintainLog getMaintainLog() {
		return maintainLog;
	}		

	public void setMaintainLog(MESBasicMaintainLog maintainLog) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/maintainLog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = maintainLogService.getMaintainLogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/maintainLog/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1555293405102
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String maintainLogReferenceCopy() throws Exception{
			MESBasicMaintainLog oldMaintainLog = null;
			if(isEditNew()){
				...
				oldMaintainLog = new MESBasicMaintainLog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,maintainLog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, maintainLog.getId());
				...
				oldMaintainLog = maintainLogService.getMaintainLog(id);
			}
			
			
			maintainLog = maintainLogService.getMaintainLog(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_container_MaintainLog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicMaintainLog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMaintainLog, null);
						Method setValue = MESBasicMaintainLog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(maintainLog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			maintainLog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_container_MaintainLog", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=maintainLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return maintainLogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			maintainLog = maintainLogService.getMaintainLog(this.maintainLog.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,maintainLog.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = maintainLogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(maintainLogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(maintainLogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_container_MaintainLog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"maintainLog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"maintainLog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"maintainLog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"maintainLog\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			maintainLogService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		maintainLogService.findDataGridPage(dg,dgclass,getDgPage(),maintainLog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(maintainLog.getId() != null && maintainLog.getId() != -1){
				maintainLog = maintainLogService.getMaintainLog(maintainLog.getId());
			}
			maintainLogService.findDataGridPage(dg,dgclass,getDgPage(),maintainLog,searchCondition,params);
	...
	private void save() throws Exception {
		...
		maintainLogService.saveMaintainLog(maintainLog,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_MaintainLog", "submit");
		bap_validate_datagrids=maintainLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", maintainLog.getId());
		...
		fileuploads.put("linkId", maintainLog.getId());
		bapAfterCustomMethod("MESBasic_1_container_MaintainLog", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/container/maintainLog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_MaintainLog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		maintainLogService.deleteMaintainLog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_container_MaintainLog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/maintainLog/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_MaintainLog", "restore");
		maintainLogService.restoreMaintainLog(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_container_MaintainLog", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicMaintainLog> findExportDataInfos(Page<MESBasicMaintainLog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = maintainLogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = maintainLogService.getAssProperty(p.getCode());	
									//String assModelCode = maintainLogService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			maintainLogService.findMaintainLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			maintainLogService.findMaintainLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicMaintainLog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicMaintainLog result : results){
	...
	public boolean isEditNew(){
		//return null == this.maintainLog || null == this.maintainLog.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/container/maintainLog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_MaintainLog", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_container_MaintainLog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/container/maintainLog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_MaintainLog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/container/maintainLog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/maintainLog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicMaintainLogService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_container_MaintainLog", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_container_MaintainLog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/maintainLog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_container_MaintainLog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_container_MaintainLog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/container/maintainLog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_MaintainLog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/container/maintainLog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_container_MaintainLog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_container_MaintainLog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/container/maintainLog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicMaintainLog maintainLog = null;
		if(null != id) {
			maintainLog = maintainLogService.getMaintainLog(id);
		}
		...
		_auditLog.setColumnStringB(null == maintainLog.getId() ? "" : maintainLog.getId().toString());
		_auditLog.setColumnStringA(null == maintainLog.getId() ? "" : maintainLog.getId().toString());
		_auditLog.setColumnStringC(maintainLog.getId().toString());
		_auditLog.setColumnStringD(MESBasicMaintainLog.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicMaintainLogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, maintainLog,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, maintainLog,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,maintainLog);
		...
		printlog.setBusinessId(maintainLog.getId());
		printlog.setBussinessKey(null == maintainLog.getId() ? "" : maintainLog.getId().toString());
		printlog.setMainDisplay(null == maintainLog.getId() ? "" : maintainLog.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/container/maintainLog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(maintainLog,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/maintainLog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(maintainLogService.getMaintainLog(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/container/maintainLog/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/container/maintainLog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, maintainLog,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				maintainLog = maintainLogService.getMaintainLog(Long.parseLong(arrId[i]));
				if(null != maintainLog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == maintainLog.getId() ? "" : "," + maintainLog.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == maintainLog.getId() ? "" : maintainLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == maintainLog.getId() ? "" : "," + maintainLog.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == maintainLog.getId() ? "" : maintainLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == maintainLog.getId() ? "" : "," + maintainLog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == maintainLog.getId() ? "" : maintainLog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, maintainLog,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,maintainLog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/container/maintainLog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_container_MaintainLog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_container_MaintainLog")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_container_MaintainLog");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_container_MaintainLog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = maintainLogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_container_MaintainLog,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_container_MaintainLog,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_container_MaintainLog,MESBasic_1) */
// 自定义代码
