...
import com.supcon.orchid.MESBasic.services.MESBasicStoreSetImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicStoreSetDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_storeSetService")
@Transactional
public class MESBasicStoreSetServiceImpl  extends BaseServiceImpl<MESBasicStoreSet> implements MESBasicStoreSetService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicStoreSet> bapBaseService;
	...
	@Autowired
	private MESBasicStoreSetImportService storeSetImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicStoreSet.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  storeSetDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicStoreSet.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = storeSetDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicStoreSet.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicStoreSet.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  storeSetDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicStoreSet.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicStoreSet.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicStoreSet> list = (List<MESBasicStoreSet>) storeSetDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicStoreSet getStoreSet(long id){
		return getStoreSet(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicStoreSet getStoreSet(long id, String viewCode){
		MESBasicStoreSet storeSet = storeSetDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getStoreSetAsJSON(long id, String include){
		MESBasicStoreSet storeSet = storeSetDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(storeSet, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.common.delete",operType="3")
	public void deleteStoreSet(MESBasicStoreSet storeSet){
		if(null != storeSet && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicStoreSet.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == storeSet.getPlaceSetCode() ? "" : storeSet.getPlaceSetCode().toString());
			AuditUtil.setColumnStringA(null == storeSet.getPlaceSetName() ? "" : storeSet.getPlaceSetName().toString());
			AuditUtil.setColumnStringC(storeSet.getId().toString());
		}
		checkDeletePermit(storeSet.getId());
		beforeDeleteStoreSet(storeSet);
		storeSetDao.delete(storeSet);
		afterDeleteStoreSet(storeSet);
		storeSetDao.flush();
		List<MESBasicStoreSet> params = new ArrayList<MESBasicStoreSet>();
		params.add(storeSet);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_storeSet");
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.common.delete",operType="3")
	public void deleteStoreSet(List<Long> storeSetIds){
		deleteStoreSet(storeSetIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.common.delete",operType="3")
	public void deleteStoreSet(List<Long> storeSetIds, String eventTopic) {
		List<MESBasicStoreSet> storeSets = new ArrayList<MESBasicStoreSet>();
		for(Long storeSetId : storeSetIds){
			MESBasicStoreSet storeSet = getStoreSet(storeSetId);
			storeSets.add(storeSet);
			if(null != storeSet && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicStoreSet.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == storeSet.getPlaceSetName() ? "" : "," + storeSet.getPlaceSetName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == storeSet.getPlaceSetName() ? "" : storeSet.getPlaceSetName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == storeSet.getPlaceSetCode() ? "" : "," + storeSet.getPlaceSetCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == storeSet.getPlaceSetCode() ? "" : storeSet.getPlaceSetCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == storeSet.getId() ? "" : "," + storeSet.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == storeSet.getId() ? "" : storeSet.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(storeSets != null && storeSets.size() > 0 ){
			for(MESBasicStoreSet storeSet : storeSets){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),storeSet, storeSet.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(storeSetIds != null && storeSetIds.size() > 0) {
			String hql = "update " + MESBasicStoreSet.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = storeSetDao.createQuery(hql);
			query.setParameterList("ids", storeSetIds);
			query.executeUpdate();
			storeSetDao.clearAllCacheByEntity();
		}
		...
		if(eventTopic==null){
			for(MESBasicStoreSet storeSetz : storeSets){
				storeSetz.setValid(false);
				List<MESBasicStoreSet> params = new ArrayList<MESBasicStoreSet>();
				params.add(storeSetz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_storeSet");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.common.delete",operType="3")
	public void deleteStoreSet(long storeSetId, int storeSetVersion){
		this.deleteStoreSet(storeSetId, storeSetVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.common.delete",operType="3")
	public void deleteStoreSet(long storeSetId, int storeSetVersion,SignatureLog signatureLog){
		MESBasicStoreSet storeSet = getStoreSet(storeSetId);
		if(storeSet != null && !storeSet.isValid()){
		...
		checkDeletePermit(storeSetId);
		beforeDeleteStoreSet(storeSet);
		storeSetDao.delete(storeSetId, storeSetVersion);
		afterDeleteStoreSet(storeSet);
		storeSetDao.flush();
		List<MESBasicStoreSet> params = new ArrayList<MESBasicStoreSet>();
		params.add(storeSet);
		...
		if(signatureLog != null ) {
			...
			businessKey=storeSet.getPlaceSetCode();
			signatureLog.setTableId(storeSet.getId());
			if(businessKey != null) {
	...
	public void checkDeletePermit(Long id) throws BAPException{
		String storeSetHql = "select count(storeSet.id) as totalCoual from " + MESBasicStoreSet.JPA_NAME + " as storeSet where storeSet.parentId=? and storeSet.valid=true";
		List<Long> storeSetCount = storeSetDao.findByHql(storeSetHql, id);
		if (storeSetCount.get(0) > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.common.delete",operType="3")
	public void deleteStoreSet(String storeSetIds){
		this.deleteStoreSet(storeSetIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.common.delete",operType="3")
	public void deleteStoreSet(String storeSetIds,SignatureLog signatureLog){
		deleteCollection(storeSetIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteStoreSet(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_storeSet#modelCode:MESBasic_1_storeSet_StoreSet#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="ec.engine.restore",operType="10")
	public void restoreStoreSet(String storeSetIds){
		restoreCollection(storeSetIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreStoreSet(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="ec.engine.restore",operType="10")
	public void restoreStoreSet(long storeSetId){
		
		findBusinessKeyUsed(storeSetId);	//判断业务主键是否重复
		...
		checkDeletePermit(storeSetId);
		beforeRestoreStoreSet(storeSet);
		storeSet.setValid(true);
		storeSetDao.update(storeSet);
		afterRestoreStoreSet(storeSet);
		List<MESBasicStoreSet> params = new ArrayList<MESBasicStoreSet>();
		params.add(storeSet);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long storeSetId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_storeSet_StoreSet");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicStoreSet.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicStoreSet.TABLE_NAME +" where id = ? )";
			List<Object> list =  storeSetDao.createNativeQuery(sql,storeSetId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseStoreSet(List<MESBasicStoreSet>  storeSets){
		for(MESBasicStoreSet storeSet:storeSets)  {
			saveStoreSet(storeSet, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseStoreSet(List<MESBasicStoreSet>  storeSets){
		for(MESBasicStoreSet storeSet:storeSets)  {
			saveStoreSet(storeSet, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="ec.flow.save")
	public void saveStoreSet(MESBasicStoreSet storeSet, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveStoreSet(storeSet, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="ec.flow.save")
	public void saveStoreSet(MESBasicStoreSet storeSet, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveStoreSet(storeSet, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateStoreSetCodes(MESBasicStoreSet storeSet) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateStoreSetCodes(storeSet, false);
	}
	
	public void generateStoreSetCodes(MESBasicStoreSet storeSet, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateStoreSetSummarys(MESBasicStoreSet storeSet) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateStoreSetSummarys(storeSet, false);
	}

	public void generateStoreSetSummarys(MESBasicStoreSet storeSet, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet")
	public void saveStoreSet(MESBasicStoreSet storeSet,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveStoreSet(storeSet, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet")
	public void saveStoreSet(MESBasicStoreSet storeSet,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_storeSet";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_storeSet");
		String url = null;
		if(storeSet.getId() != null && storeSet.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicStoreSet.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_storeSet/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicStoreSet.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_storeSet/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, storeSet);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(storeSet);
		beforeSaveStoreSet(storeSet, viewIsView);

		if (viewIsView) {
			storeSetDao.saveWithRevertVersion(storeSet);
		} else {
			storeSetDao.save(storeSet);
		}
		if(null != storeSet && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicStoreSet.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == storeSet.getPlaceSetCode() ? "" : storeSet.getPlaceSetCode().toString());
			AuditUtil.setColumnStringA(null == storeSet.getPlaceSetName() ? "" : storeSet.getPlaceSetName().toString());
		...
		dealDatagridsSave(storeSet,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateStoreSetCodes(storeSet, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateStoreSetSummarys(storeSet, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveStoreSet(storeSet, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, storeSet);
			}
		}
		List<MESBasicStoreSet> params = new ArrayList<MESBasicStoreSet>();
		storeSetDao.flush();
		storeSetDao.clear();
		storeSet = storeSetDao.load(storeSet.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicStoreSet storeSet){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("storeSet", storeSet);
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet")
	public void mergeStoreSet(MESBasicStoreSet storeSet, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(storeSet);
		beforeSaveStoreSet(storeSet);
		storeSetDao.merge(storeSet);
		if(null != storeSet && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicStoreSet.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == storeSet.getPlaceSetCode() ? "" : storeSet.getPlaceSetCode().toString());
			AuditUtil.setColumnStringA(null == storeSet.getPlaceSetName() ? "" : storeSet.getPlaceSetName().toString());
			AuditUtil.setColumnStringC(storeSet.getId().toString());
		}
		dealDatagridsSave(storeSet,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveStoreSet(storeSet);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicStoreSet> findStoreSets(Page<MESBasicStoreSet> page, Criterion... criterions) {
		return storeSetDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicStoreSetDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicStoreSetDealInfo> list=storeSetDao.createQuery("from " + MESBasicStoreSetDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicStoreSetDealInfo dealInfo) {
		storeSetDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicStoreSetDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) storeSetDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicStoreSetDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = storeSetDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = storeSetDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicStoreSetDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = storeSetDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	public List<String> getContainLower(String modelName, String fieldName, String exp, Object value, Boolean crossCompany) {
		...
		if(crossCompany) {
			layRecHql += " and cid = ?";
			return storeSetDao.createQuery(layRecHql, value, getCurrentCompanyId()).list();
		}
		return storeSetDao.createQuery(layRecHql, value).list();
	}
	
	private void dealListTreePageData(Page<MESBasicStoreSet> storeSets, String viewCode){
		List<MESBasicStoreSet> result = storeSets.getResult();
		if(null != result && !result.isEmpty()) {
			int rootPos = 0;
			MESBasicStoreSet rootTree = result.get(0);
			...
			storeSets.setTreeToSurfaceMode(true);
			MESBasicStoreSet storeSetTree = storeSetDao.buildTree("storeSet" + viewCode, new ListPrepareData(storeSets), false);
			if(null != storeSetTree.getChildren() && !storeSetTree.getChildren().isEmpty()) {
				List<MESBasicStoreSet> treeList = new ArrayList<MESBasicStoreSet>();
				Tree.treeToList(storeSetTree, treeList, false);
				if(treeList.size() == result.size()) {
					storeSets.setResult(treeList);
					storeSets.setTreeToSurfaceMode(false);
				} 
			} 
		}
	}
	 ...
	 * 
	 */
	private class ListPrepareData implements PrepareData<MESBasicStoreSet,Long> {
		private Page<MESBasicStoreSet> storeSets;

		public ListPrepareData(Page<MESBasicStoreSet> storeSets) {
			this.storeSets = storeSets;
		}

		@Override
			public MESBasicStoreSet root() {
				MESBasicStoreSet storeSet = new MESBasicStoreSet();
				storeSet.setId(-1l);
				return storeSet;
			}
			
			@Override
			public List<MESBasicStoreSet> prepare(TreeDao<MESBasicStoreSet, Long> dao) {
				return storeSets.getResult();
			}
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStoreSets(Page<MESBasicStoreSet> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findStoreSets(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStoreSets(Page<MESBasicStoreSet> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStoreSets(Page<MESBasicStoreSet> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicStoreSet> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"storeSet\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"storeSet\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"storeSet\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_storeSet_StoreSet" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_storeSet_StoreSet" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  storeSetDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  storeSetDao.createNativeQuery(powerOperateSql, "MESBasic_1_storeSet").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"storeSet\"","MESBasic_1_storeSet",customCondition,"MATERIAL_STORE_SETS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"storeSet\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicStoreSetDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"storeSet\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"storeSet\".LAY_REC ASC, \"storeSet\".SORT ASC");
				} else {
					orderPart.append(" \"storeSet\".LAY_REC ASC, \"storeSet\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) storeSetDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=storeSetDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = storeSetDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicStoreSet> result = new ArrayList<MESBasicStoreSet>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicStoreSet.class,storeSetDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicStoreSet.class,storeSetDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicStoreSet storeSet : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(storeSet.getId(), "MESBasic_storeSet_storeSet");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(storeSet.getId(), "MESBasic_storeSet_storeSet");
							if(null != documents && !documents.isEmpty()) {
								storeSet.setDocument(documents.get(0));
								storeSet.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(page.isExportFlag()){
					...
					if(exportMap.get("businessKey")!=null){
						try {
							...
							MESBasicStoreSet storeSet = new MESBasicStoreSet();
							getBusKeyMethod = storeSet.getClass().getMethod(getBusKeyMethodStr);
					...
					if(exportMap.get("mainDisplay")!=null){
						try {
							...
							MESBasicStoreSet storeSet = new MESBasicStoreSet();
							getMainDisMethod = storeSet.getClass().getMethod(getMainDisMethodStr);
						} catch (NoSuchMethodException | SecurityException e) {
							log.error(e.getMessage(),e);
						}
					}
					...
					for (MESBasicStoreSet storeSet : result) {
						Long parentId = storeSet.getParentId();
						if(parentId != null && parentId.longValue() != new Long(-1).longValue()){
							for (MESBasicStoreSet storeSet2 : result) {
								if(storeSet2.getId() != null && storeSet2.getId().longValue() == parentId.longValue()){
									try {
										if(getBusKeyMethod != null){
											Object busKey = getBusKeyMethod.invoke(storeSet2);
											storeSet.setParentNodeBusinessKey(busKey.toString());
										}
										if(getMainDisMethod != null){
											Object mainDis = getMainDisMethod.invoke(storeSet2);
											storeSet.setParentNodeMainDisplay(mainDis.toString());
										}	
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_storeSet")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_storeSet")+"MESBasic_1_storeSet".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_storeSet",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicStoreSet> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicStoreSet> result) {
		View view = null;
		...
		Map<Long, MESBasicStoreSet> rsMap = new HashMap<Long, MESBasicStoreSet>();
		for (MESBasicStoreSet storeSet : result) {
			ids.add(storeSet.getId());
			rsMap.put(storeSet.getId(), storeSet);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(storeSetDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = storeSetDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicStoreSet storeSet,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(storeSet.getOwnerStaffId());
		ti.setOwnerPositionId(storeSet.getOwnerPositionId());
		ti.setOwnerDepartmentId(storeSet.getOwnerDepartmentId());
		
		ti.setCreateStaff(storeSet.getCreateStaff());
		ti.setCreateStaffId(storeSet.getCreateStaffId());
		ti.setCreateTime(storeSet.getCreateTime());
		ti.setDeleteStaff(storeSet.getDeleteStaff());
		ti.setDeleteStaffId(storeSet.getDeleteStaffId());
		ti.setDeleteTime(storeSet.getDeleteTime());
		ti.setCreateDepartmentId(storeSet.getCreateDepartmentId());
		ti.setModifyStaff(storeSet.getModifyStaff());
		ti.setModifyStaffId(storeSet.getModifyStaffId());
		ti.setCreatePositionId(storeSet.getCreatePositionId());
		ti.setPositionLayRec(storeSet.getPositionLayRec());
		ti.setTableNo(storeSet.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_storeSet");
		ti.setTargetTableName("MATERIAL_STORE_SETS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_storeSet_StoreSet" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  storeSetDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"storeSet\"","MESBasic_1_storeSet","","MATERIAL_STORE_SETS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"storeSet\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"storeSet\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"storeSet\"","MESBasic_1_storeSet","","MATERIAL_STORE_SETS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("storeSet.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"storeSet\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("storeSet.mc\"").append(".STORE_SET) not in ("+notinValue+")");
			}
			
			SQLQuery query = storeSetDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("storeSet", "MESBasic_1_storeSet", "storeSet_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=storeSetDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicStoreSet.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicStoreSet.MODULE_CODE;}
	protected String getModelCode(){return MESBasicStoreSet.MODEL_CODE;}
	
	@Override
	public List<MESBasicStoreSet> getTreeChildren(Long parentId, String cond, Object... params) {
		MESBasicStoreSet parent = storeSetDao.getTree("storeSet_tree_" + parentId, parentId, new MESBasicStoreSetPrepareData(cond, params), false);
	...
	@Override
	public List<MESBasicStoreSet> getTreeInfo(Map<Long, String> treeInfo, String condition, List<Object> params) {
		...
		for(String layRec : mapKeys) {
			...
			MESBasicStoreSet storeSetTree = storeSetDao.getTree("MESBasicStoreSet" + rootLayRec + "-", rootId, new PrepareData<MESBasicStoreSet, Long>() {
				@Override
				public MESBasicStoreSet root() {
					MESBasicStoreSet storeSet = new MESBasicStoreSet();
					storeSet.setId(-1l);
				...
				@Override
				public List<MESBasicStoreSet> prepare(TreeDao<MESBasicStoreSet, Long> dao) {
					String hql = "from " + MESBasicStoreSet.JPA_NAME + " where (" + searchCondition + ") and valid = true " + conditionSql + " order by layRec asc, sort asc";
					return dao.findByHql(hql, paramsList.toArray());
				}
			}, false);
			List<MESBasicStoreSet> treeChildren = new ArrayList<MESBasicStoreSet>();
			Tree.treeToList(storeSetTree, treeChildren, true);
			treeList.addAll(treeChildren);
		}
		return treeList;
	}

	private class MESBasicStoreSetPrepareData implements PrepareData<MESBasicStoreSet, Long> {
		private String cond;
		private Object[] params;
		
		public MESBasicStoreSetPrepareData(String cond, Object... params) {
			this.cond = cond;
			this.params = params;
		}
		...
		//@Override
		//public List<MESBasicStoreSet> prepare(TreeDao<MESBasicStoreSet, Long> dao) {
		//	String hql = "from " + MESBasicStoreSet.JPA_NAME;
		//	if (null != cond && cond.trim().length() > 0) {
		//		hql += " where " + cond;
		//	}
		//	List<MESBasicStoreSet> list = dao.findByHql(hql);
		//	return list;
		//}
		
		@Override
		public List<MESBasicStoreSet> prepare(TreeDao<MESBasicStoreSet, Long> dao) {
			List<MESBasicStoreSet> list = null;
			List<MESBasicStoreSet> result = new ArrayList<MESBasicStoreSet>();
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicStoreSet.class);
			if(params!=null&&params.length>0){
				...
				list = dao.findByHql("from " + MESBasicStoreSet.JPA_NAME + " " + ((cond==null || cond.isEmpty()) ? "" : "where " + cond + " order by layRec asc, sort asc")) ;
			}
			for(MESBasicStoreSet m : list){
				result.add(m);
				if(m.getParentId()!=-1){
					while(m.getParentId()!=-1){
						MESBasicStoreSet parent = dao.get(m.getParentId());
	 ...
	 * @param storeSet 模型实例
	 */
	public void checkUniqueConstraint(MESBasicStoreSet storeSet){
		if(storeSet.getId()==null){
			List<MESBasicStoreSet> storeSets = storeSetDao.findByCriteria(Restrictions.eq("placeSetCode", storeSet.getPlaceSetCode()), Restrictions.eq("valid", true));
			if(storeSets!=null && !storeSets.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1489731651449", getCurrentUser().getLanguage()), storeSet.getPlaceSetCode());
			}
		} else {
			List<MESBasicStoreSet> storeSets = storeSetDao.findByCriteria(Restrictions.ne("id", storeSet.getId()), Restrictions.eq("placeSetCode", storeSet.getPlaceSetCode()), Restrictions.eq("valid", true));
			if(storeSets!=null && !storeSets.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1489731651449", getCurrentUser().getLanguage()), storeSet.getPlaceSetCode());
			}
	...
	@Override
	public List<MESBasicStoreSet> getStoreSets(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicStoreSet.class);
		if(params!=null){
			List<Type> types = new ArrayList<Type>();
			for(Object obj : params){
			...
			return storeSetDao.findByHql("from " + MESBasicStoreSet.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return storeSetDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicStoreSet> findStoreSetsByHql(String hql, Object... objects){
		...
		return   storeSetDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicStoreSet loadStoreSetByBussinessKey(MESBasicStoreSet storeSet){
		return loadStoreSetByBussinessKey(storeSet.getPlaceSetCode());
	}
	
	@Override
	public Page<MESBasicStoreSet> getStoreSets(Page<MESBasicStoreSet> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicStoreSet.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = storeSetDao.findByPage(page, sql);
			}else{
				page = storeSetDao.findAllByPage(page);
			}
			return page;
		}
		page = storeSetDao.findByPage(page, detachedCriteria);
		return page;
	}
	//==============DataGrid多选控件使用 start================
	
	@Override
	public void dealStoreSetLeaf(List<Long> storeSetIds) {
		if (storeSetIds != null && storeSetIds.size() > 0) {
			String hql = "update " + MESBasicStoreSet.JPA_NAME + " as t1 set t1.leaf=1 where id in(:ids) and not exists (select t2.id from "
					+ MESBasicStoreSet.JPA_NAME + " as t2 where t2.parentId = t1.id)";
			Query query = storeSetDao.createQuery(hql);
			query.setParameterList("ids", storeSetIds);
			query.executeUpdate();
		}
	...
	public void drag(long dragSource, long dragTarget, long sort){
		MESBasicStoreSet source = getStoreSet(dragSource);
		MESBasicStoreSet oldParent = source.getParent();
		
		MESBasicStoreSet target = null;
		if(dragTarget!=-1){
			target = getStoreSet(dragTarget);
		} else {
			target = new MESBasicStoreSet();
			target.setId(-1L);
		}
		storeSetDao.move(source, target);
	}
	
	private void dragStoreSet(MESBasicStoreSet parent, List<MESBasicStoreSet> children){
		for(MESBasicStoreSet child : children){
			...
			saveStoreSet(child, null,null,null);
			List<MESBasicStoreSet> subChildren = getTreeChildren(child.getId(), "");
			if(subChildren!=null && subChildren.size()>0){
				dragStoreSet(child, subChildren);
			}
		}
	}
	
	public List<MESBasicStoreSet> convertTree(List<MESBasicStoreSet> storeSets){
		List<MESBasicStoreSet> trees = new ArrayList<MESBasicStoreSet>();
		Map<Long, MESBasicStoreSet> map = new LinkedHashMap<Long, MESBasicStoreSet>(storeSets.size());
		Map<Long, Boolean> isParent = new HashMap<Long, Boolean>();
		for (MESBasicStoreSet node : storeSets) {
		...
		for (MESBasicStoreSet node : storeSets) {
			if (null == node.getParentId() ||Long.parseLong(node.getParentId().toString()) == -1) {
				...
				MESBasicStoreSet parent = map.get(node.getParentId());
	...
	public MESBasicStoreSet loadStoreSetByBussinessKey(Serializable bussinessKey){
		return storeSetDao.findEntityByCriteria(Restrictions.eq("placeSetCode", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicStoreSet> findByProperty(String propertyName, Object object){
		return storeSetDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicStoreSet findEntityByProperty(String propertyName, Object object){
		return storeSetDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicStoreSet.JPA_NAME + " set valid=0 where placeSetCode in (:businessKeys)";
			Query query = storeSetDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_storeSet_StoreSet","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_storeSet", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicStoreSet storeSet,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,storeSetDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicStoreSet.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, storeSet);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",storeSet.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicStoreSet.TABLE_NAME +" where valid = 1";
			list = storeSetDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicStoreSet.TABLE_NAME;
			list = storeSetDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicStoreSet.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = storeSetDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicStoreSetDaoImpl storeSetDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchStoreSet(final List<MESBasicStoreSet> insertObjs, final List<MESBasicStoreSet> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchStoreSet(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchStoreSet(final List<MESBasicStoreSet> insertObjs, final List<MESBasicStoreSet> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1489731375532"),importCount));
		...
		Map<Long, MESBasicStoreSet> oldUpdateObjsMap = new HashMap<Long, MESBasicStoreSet>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicStoreSet bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicStoreSet> bizObjList = storeSetDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicStoreSet bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicStoreSet> nmeObjs = new ArrayList<MESBasicStoreSet>();
		...
		long id=autoGeneratorID.getNextId(MESBasicStoreSet.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicStoreSet storeSet:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicStoreSet.TABLE_NAME,1,dbId);
			if(storeSet.getPlaceSetCode() != null && !storeSet.getPlaceSetCode().equals("") ){
				m.put(storeSet.getPlaceSetCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		List<MESBasicStoreSet> needChangeInsertObjList = new ArrayList<MESBasicStoreSet>();
		if(insertNodeList != null && insertNodeList.size()>0 ){
			for(int i = 0; i < insertNodeList.size(); i++){
				...
				if(!needChange && !isRoot){
					MESBasicStoreSet obj = insertObjs.get(i);
					...
					MESBasicStoreSet parentObj = (MESBasicStoreSet) insertNode.get(parentNode);
					*/
		...
		if(updateNodeList != null && updateNodeList.size()>0){
			for(int i = 0; i < updateNodeList.size(); i++){
				...
				MESBasicStoreSet updateObj = updateObjs.get(i);
				...
				if(isRoot){
					...
					if(needChange!=null && !needChange){
						...
						MESBasicStoreSet parentNode = (MESBasicStoreSet) insertParentNode.get(parentNodeKey);
		...
		if(insertNodeList != null && insertNodeList.size()>0){
			for(int i = 0; i < insertNodeList.size(); i++){
				Map<String, Object> insertNode = insertNodeList.get(i);
				MESBasicStoreSet insertObj = insertObjs.get(i);
		...
		if(updateObjs != null && updateObjs.size()>0 ){
			for(MESBasicStoreSet obj : updateObjs){
				Object bus;
				try {
		...
		if(businessKey != null && mainDisplay != null && businessKey != mainDisplay  && paramList != null && paramList.size()>0){
			String hql = "select " + businessKey.getName() + "," + mainDisplay.getName() + ",id from " + MESBasicStoreSet.JPA_NAME + " where " + businessKey.getName() + " in (:codes)";
			Query query =  storeSetDao.createQuery(hql);
			query.setParameterList("codes", paramList);
			List<Object[]> list = query.list();
			if(list.size() == updateObjs.size()){
				for(int i = 0 ; i < updateObjs.size();i++ ){
					MESBasicStoreSet obj = updateObjs.get(i);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) storeSetDao.createNativeQuery(sql, "MESBasic_1_storeSet_StoreSet"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		storeSetDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicStoreSet.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicStoreSet storeSet = updateObjs.get(updateIndex);
						Long id = storeSet.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = storeSet.getClass().getMethod(updateMethods.get(i));
								} catch (NoSuchMethodException | SecurityException e) {
									log.error(e.getMessage(),e);
								}
								Object obj = null;
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicStoreSet.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicStoreSet.TABLE_NAME + " order by id desc";
//					Query query = storeSetDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicStoreSet storeSet = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = storeSet.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		storeSetDaoImpl.getSession().flush();
		storeSetDaoImpl.getSession().clear();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicStoreSet bizObj = insertObjs.get(insertIndex);
			storeSetImportService.saveImportDataLog(bizObj, null, "placeSetCode", "placeSetName");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicStoreSet bizObj = updateObjs.get(updateIndex);
			MESBasicStoreSet oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			storeSetImportService.saveImportDataLog(bizObj, oldBizObj, "placeSetCode", "placeSetName");
		}
	...
	private void changeFullPathName(List<Long> needChangeFullPathNameId, Map<Object, Object> needChangeFullPathName,final Property businessKey,Property mainDisplay){
		...
		sql.append(" from ");
		sql.append(MESBasicStoreSet.TABLE_NAME);
		sql.append(" where ");
		...
		List<Object[]> lists =  storeSetDao.createNativeQuery(sqlStr).list();
		...
		storeSetDaoImpl.getSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
				...
				if(updateLists != null && updateLists.size()>0){
					String updateSql = "UPDATE " + MESBasicStoreSet.TABLE_NAME + " SET FULL_PATH_NAME = ? WHERE " + businessKey.getColumnName() + " = ?";
					updateps = conn.prepareStatement(updateSql);
	...
	private Map<String, Object> buildInsertObjNode(List<Map<String, Object>> insertNodeList, List<MESBasicStoreSet> insertObjs, 
			...
			for(int i =0; i<insertNodeList.size(); i++){
				...
				if(insertBuildedObj.get(busKey)!=null){
					...
					if(isRoot){
						...
						String parentNode = insertNode.get("parentNode").toString();
						MESBasicStoreSet parentObj = (MESBasicStoreSet) insertBuildedObj.get(parentNode);
	...
	public String getViewName(String modelCode){
		...
		String viewName = storeSetDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				storeSetDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = storeSetDao.createNativeQuery(sql, "MESBasic_1_storeSet_StoreSet").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = storeSetDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = storeSetDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = storeSetDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?storeSetDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  storeSetDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,storeSetDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="ec.flow.save")
	public void saveStoreSet(MESBasicStoreSet storeSet, DataGridService dataGridService){
		saveStoreSet(storeSet, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="ec.flow.save")
	public void saveStoreSet(MESBasicStoreSet storeSet, DataGridService dataGridService, String viewCode){
		saveStoreSet(storeSet, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet")
	public void saveStoreSet(MESBasicStoreSet storeSet, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveStoreSet(storeSet, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet")
	public void saveStoreSet(MESBasicStoreSet storeSet, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_storeSet";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_storeSet");
		String url = null;
		if(null != storeSet.getId() && storeSet.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicStoreSet.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet")
	public Map<Object, Object> storeSetDataGridImport(MESBasicStoreSet storeSet, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = storeSet.getVirtualId();
		
		storeSet = storeSetDao.load(storeSet.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, storeSet.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = storeSet.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(storeSet);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, storeSet.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet")
	public void mergeStoreSet(MESBasicStoreSet storeSet, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(storeSet);
		beforeSaveStoreSet(storeSet);
		storeSetDao.merge(storeSet);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicStoreSet> getByPage(Page<MESBasicStoreSet> page,DetachedCriteria detachedCriteria){
		return storeSetDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_storeSet").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveStoreSet(MESBasicStoreSet storeSet, Object... objects){
		checkUniqueConstraint(storeSet);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveStoreSet(MESBasicStoreSet storeSet, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteStoreSet(MESBasicStoreSet storeSet){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteStoreSet(MESBasicStoreSet storeSet){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreStoreSet(MESBasicStoreSet storeSet){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreStoreSet(MESBasicStoreSet storeSet){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicStoreSet storeSet, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicStoreSet storeSet, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportStoreSet(List<MESBasicStoreSet> insertObjs, List<MESBasicStoreSet> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportStoreSet,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportStoreSet(List<MESBasicStoreSet> insertObjs, List<MESBasicStoreSet> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportStoreSet,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_storeSet_StoreSet,MESBasic_1) */
// 自定义代码
    protected String getCustomCondition(Page page, String viewCode, int type,String processKey, List<Param> params, List<Object> list) {
        String sqlCustomConditon = "";
        Long userId = ((User) getCurrentUser()).getId();
        Long staffId = ((User) getCurrentUser()).getStaff().getId();
        // 仓库权限过滤
        if ("MESBasic_1_storeSet_pubStoreRef".equals(viewCode) || "MESBasic_1_storeSet_storeRef".equals(viewCode) || "MESBasic_1_storeSet_storeFactoryRef".equals(viewCode)) {
            if (wareService.getIsUser()) {
                sqlCustomConditon = " \"storeSet\".WARE_ID in  ((select WARE_ID from MATERIAL_WARE_ROLE_INFOS where WARE_ROLE in ( select ROLE_ID from BASE_ROLEUSER where USER_ID = "
                        + userId
                        + ")) "
                        + " union ( select WARE_ID from MATERIAL_WARE_STAFF_INFOS  where WARE_STAFF = "
                        + staffId + " ))";
            }
        }
        return sqlCustomConditon;
    }
/* CUSTOM CODE END */
}
