...
import com.supcon.orchid.MESBasic.services.MESBasicQACheckIndexImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicQACheckIndexDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_qACheckIndexService")
@Transactional
public class MESBasicQACheckIndexServiceImpl  extends BaseServiceImpl<MESBasicQACheckIndex> implements MESBasicQACheckIndexService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicQACheckIndex> bapBaseService;
	...
	@Autowired
	private MESBasicQACheckIndexImportService qACheckIndexImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicQACheckIndex.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  qACheckIndexDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicQACheckIndex.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = qACheckIndexDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicQACheckIndex.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicQACheckIndex.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  qACheckIndexDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicQACheckIndex.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicQACheckIndex.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicQACheckIndex> list = (List<MESBasicQACheckIndex>) qACheckIndexDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicQACheckIndex getQACheckIndex(long id){
		return getQACheckIndex(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicQACheckIndex getQACheckIndex(long id, String viewCode){
		MESBasicQACheckIndex qACheckIndex = qACheckIndexDao.load(id);

		// 一对多情况处理
		if(qACheckIndex != null){
		}
		return qACheckIndex;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getQACheckIndexAsJSON(long id, String include){
		MESBasicQACheckIndex qACheckIndex = qACheckIndexDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(qACheckIndex, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.common.delete",operType="3")
	public void deleteQACheckIndex(MESBasicQACheckIndex qACheckIndex){
		if(null != qACheckIndex && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicQACheckIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == qACheckIndex.getCode() ? "" : qACheckIndex.getCode().toString());
			AuditUtil.setColumnStringA(null == qACheckIndex.getName() ? "" : qACheckIndex.getName().toString());
			AuditUtil.setColumnStringC(qACheckIndex.getId().toString());
		}
		beforeDeleteQACheckIndex(qACheckIndex);
		qACheckIndexDao.delete(qACheckIndex);
		afterDeleteQACheckIndex(qACheckIndex);
		qACheckIndexDao.flush();
		List<MESBasicQACheckIndex> params = new ArrayList<MESBasicQACheckIndex>();
		params.add(qACheckIndex);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_checkIndex");
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.common.delete",operType="3")
	public void deleteQACheckIndex(List<Long> qACheckIndexIds){
		deleteQACheckIndex(qACheckIndexIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.common.delete",operType="3")
	public void deleteQACheckIndex(List<Long> qACheckIndexIds, String eventTopic) {
		List<MESBasicQACheckIndex> qACheckIndexs = new ArrayList<MESBasicQACheckIndex>();
		for(Long qACheckIndexId : qACheckIndexIds){
			MESBasicQACheckIndex qACheckIndex = getQACheckIndex(qACheckIndexId);
			qACheckIndexs.add(qACheckIndex);
			if(null != qACheckIndex && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicQACheckIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == qACheckIndex.getName() ? "" : "," + qACheckIndex.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == qACheckIndex.getName() ? "" : qACheckIndex.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == qACheckIndex.getCode() ? "" : "," + qACheckIndex.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == qACheckIndex.getCode() ? "" : qACheckIndex.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == qACheckIndex.getId() ? "" : "," + qACheckIndex.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == qACheckIndex.getId() ? "" : qACheckIndex.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(qACheckIndexs != null && qACheckIndexs.size() > 0 ){
			for(MESBasicQACheckIndex qACheckIndex : qACheckIndexs){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),qACheckIndex, qACheckIndex.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(qACheckIndexIds != null && qACheckIndexIds.size() > 0) {
			String hql = "update " + MESBasicQACheckIndex.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = qACheckIndexDao.createQuery(hql);
			query.setParameterList("ids", qACheckIndexIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicQACheckIndex qACheckIndexz : qACheckIndexs){
				qACheckIndexz.setValid(false);
				List<MESBasicQACheckIndex> params = new ArrayList<MESBasicQACheckIndex>();
				params.add(qACheckIndexz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_checkIndex");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.common.delete",operType="3")
	public void deleteQACheckIndex(long qACheckIndexId, int qACheckIndexVersion){
		this.deleteQACheckIndex(qACheckIndexId, qACheckIndexVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.common.delete",operType="3")
	public void deleteQACheckIndex(long qACheckIndexId, int qACheckIndexVersion,SignatureLog signatureLog){
		MESBasicQACheckIndex qACheckIndex = getQACheckIndex(qACheckIndexId);
		if(qACheckIndex != null && !qACheckIndex.isValid()){
		...
		beforeDeleteQACheckIndex(qACheckIndex);
		qACheckIndexDao.delete(qACheckIndexId, qACheckIndexVersion);
		afterDeleteQACheckIndex(qACheckIndex);
		qACheckIndexDao.flush();
		List<MESBasicQACheckIndex> params = new ArrayList<MESBasicQACheckIndex>();
		params.add(qACheckIndex);
		...
		if(signatureLog != null ) {
			...
			businessKey=qACheckIndex.getCode();
			signatureLog.setTableId(qACheckIndex.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.common.delete",operType="3")
	public void deleteQACheckIndex(String qACheckIndexIds){
		this.deleteQACheckIndex(qACheckIndexIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.common.delete",operType="3")
	public void deleteQACheckIndex(String qACheckIndexIds,SignatureLog signatureLog){
		deleteCollection(qACheckIndexIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteQACheckIndex(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_checkIndex#modelCode:MESBasic_1_checkIndex_QACheckIndex#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="ec.engine.restore",operType="10")
	public void restoreQACheckIndex(String qACheckIndexIds){
		restoreCollection(qACheckIndexIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreQACheckIndex(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="ec.engine.restore",operType="10")
	public void restoreQACheckIndex(long qACheckIndexId){
		
		findBusinessKeyUsed(qACheckIndexId);	//判断业务主键是否重复
		...
		beforeRestoreQACheckIndex(qACheckIndex);
		qACheckIndex.setValid(true);
		qACheckIndexDao.update(qACheckIndex);
		afterRestoreQACheckIndex(qACheckIndex);
		List<MESBasicQACheckIndex> params = new ArrayList<MESBasicQACheckIndex>();
		params.add(qACheckIndex);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long qACheckIndexId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_checkIndex_QACheckIndex");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicQACheckIndex.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicQACheckIndex.TABLE_NAME +" where id = ? )";
			List<Object> list =  qACheckIndexDao.createNativeQuery(sql,qACheckIndexId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseQACheckIndex(List<MESBasicQACheckIndex>  qACheckIndexs){
		for(MESBasicQACheckIndex qACheckIndex:qACheckIndexs)  {
			saveQACheckIndex(qACheckIndex, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseQACheckIndex(List<MESBasicQACheckIndex>  qACheckIndexs){
		for(MESBasicQACheckIndex qACheckIndex:qACheckIndexs)  {
			saveQACheckIndex(qACheckIndex, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="ec.flow.save")
	public void saveQACheckIndex(MESBasicQACheckIndex qACheckIndex, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveQACheckIndex(qACheckIndex, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="ec.flow.save")
	public void saveQACheckIndex(MESBasicQACheckIndex qACheckIndex, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveQACheckIndex(qACheckIndex, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateQACheckIndexCodes(MESBasicQACheckIndex qACheckIndex) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateQACheckIndexCodes(qACheckIndex, false);
	}
	
	public void generateQACheckIndexCodes(MESBasicQACheckIndex qACheckIndex, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
			qACheckIndexDao.flush();
			qACheckIndexDao.clear();
			MESBasicQACheckIndex modelinstance = getQACheckIndex(qACheckIndex.getId());
			...
			boolean isLegal;
		// code
		if (qACheckIndex.getCode() == null || qACheckIndex.getCode().isEmpty()) {
			String code_property_code = "MESBasic_1_checkIndex_QACheckIndex_code";
			...
			if(null != property){
				...
				if(isLegal){
					...
					qACheckIndex.setCode(codenewCodeValue);
				}
				
				if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicQACheckIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if (viewIsView) {
			qACheckIndexDao.saveWithRevertVersion(modelinstance);
		} else {
			qACheckIndexDao.save(modelinstance);
		}
	}
	
	public void generateQACheckIndexSummarys(MESBasicQACheckIndex qACheckIndex) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateQACheckIndexSummarys(qACheckIndex, false);
	}

	public void generateQACheckIndexSummarys(MESBasicQACheckIndex qACheckIndex, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex")
	public void saveQACheckIndex(MESBasicQACheckIndex qACheckIndex,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveQACheckIndex(qACheckIndex, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex")
	public void saveQACheckIndex(MESBasicQACheckIndex qACheckIndex,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_checkIndex";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_checkIndex");
		String url = null;
		if(qACheckIndex.getId() != null && qACheckIndex.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicQACheckIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_checkIndex/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicQACheckIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_checkIndex/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, qACheckIndex);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(qACheckIndex);
		beforeSaveQACheckIndex(qACheckIndex, viewIsView);

		if (viewIsView) {
			qACheckIndexDao.saveWithRevertVersion(qACheckIndex);
		} else {
			if(isNew)
				qACheckIndexDao.save(qACheckIndex);
			else
				qACheckIndexDao.merge(qACheckIndex);
		}
		if(null != qACheckIndex && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicQACheckIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == qACheckIndex.getCode() ? "" : qACheckIndex.getCode().toString());
			AuditUtil.setColumnStringA(null == qACheckIndex.getName() ? "" : qACheckIndex.getName().toString());
		...
		dealDatagridsSave(qACheckIndex,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateQACheckIndexCodes(qACheckIndex, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateQACheckIndexSummarys(qACheckIndex, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveQACheckIndex(qACheckIndex, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, qACheckIndex);
			}
		}
		List<MESBasicQACheckIndex> params = new ArrayList<MESBasicQACheckIndex>();
		qACheckIndexDao.flush();
		qACheckIndexDao.clear();
		qACheckIndex = qACheckIndexDao.load(qACheckIndex.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicQACheckIndex qACheckIndex){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("qACheckIndex", qACheckIndex);
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex")
	public void mergeQACheckIndex(MESBasicQACheckIndex qACheckIndex, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(qACheckIndex);
		beforeSaveQACheckIndex(qACheckIndex);
		qACheckIndexDao.merge(qACheckIndex);
		if(null != qACheckIndex && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicQACheckIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == qACheckIndex.getCode() ? "" : qACheckIndex.getCode().toString());
			AuditUtil.setColumnStringA(null == qACheckIndex.getName() ? "" : qACheckIndex.getName().toString());
		...
		dealDatagridsSave(qACheckIndex,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveQACheckIndex(qACheckIndex);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicQACheckIndex> findQACheckIndexs(Page<MESBasicQACheckIndex> page, Criterion... criterions) {
		return qACheckIndexDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicQACheckIndexDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicQACheckIndexDealInfo> list=qACheckIndexDao.createQuery("from " + MESBasicQACheckIndexDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicQACheckIndexDealInfo dealInfo) {
		qACheckIndexDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicQACheckIndexDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) qACheckIndexDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicQACheckIndexDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = qACheckIndexDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = qACheckIndexDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicQACheckIndexDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = qACheckIndexDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findQACheckIndexs(Page<MESBasicQACheckIndex> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findQACheckIndexs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findQACheckIndexs(Page<MESBasicQACheckIndex> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findQACheckIndexs(Page<MESBasicQACheckIndex> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicQACheckIndex> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"qACheckIndex\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"qACheckIndex\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"qACheckIndex\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_checkIndex_QACheckIndex" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_checkIndex_QACheckIndex" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  qACheckIndexDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  qACheckIndexDao.createNativeQuery(powerOperateSql, "MESBasic_1_checkIndex").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"qACheckIndex\"","MESBasic_1_checkIndex",customCondition,"LIMSBASIC_CHECK_INDICES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"qACheckIndex\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicQACheckIndexDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"qACheckIndex\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"qACheckIndex\".LAY_REC ASC, \"qACheckIndex\".SORT ASC");
				} else {
					orderPart.append(" \"qACheckIndex\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) qACheckIndexDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=qACheckIndexDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = qACheckIndexDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicQACheckIndex> result = new ArrayList<MESBasicQACheckIndex>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicQACheckIndex.class,qACheckIndexDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicQACheckIndex.class,qACheckIndexDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicQACheckIndex qACheckIndex : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(qACheckIndex.getId(), "MESBasic_checkIndex_qACheckIndex");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(qACheckIndex.getId(), "MESBasic_checkIndex_qACheckIndex");
							if(null != documents && !documents.isEmpty()) {
								qACheckIndex.setDocument(documents.get(0));
								qACheckIndex.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_checkIndex")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_checkIndex")+"MESBasic_1_checkIndex".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_checkIndex",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicQACheckIndex> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicQACheckIndex> result) {
		View view = null;
		...
		Map<Long, MESBasicQACheckIndex> rsMap = new HashMap<Long, MESBasicQACheckIndex>();
		for (MESBasicQACheckIndex qACheckIndex : result) {
			ids.add(qACheckIndex.getId());
			rsMap.put(qACheckIndex.getId(), qACheckIndex);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(qACheckIndexDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = qACheckIndexDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicQACheckIndex qACheckIndex,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(qACheckIndex.getOwnerStaffId());
		ti.setOwnerPositionId(qACheckIndex.getOwnerPositionId());
		ti.setOwnerDepartmentId(qACheckIndex.getOwnerDepartmentId());
		
		ti.setCreateStaff(qACheckIndex.getCreateStaff());
		ti.setCreateStaffId(qACheckIndex.getCreateStaffId());
		ti.setCreateTime(qACheckIndex.getCreateTime());
		ti.setDeleteStaff(qACheckIndex.getDeleteStaff());
		ti.setDeleteStaffId(qACheckIndex.getDeleteStaffId());
		ti.setDeleteTime(qACheckIndex.getDeleteTime());
		ti.setCreateDepartmentId(qACheckIndex.getCreateDepartmentId());
		ti.setModifyStaff(qACheckIndex.getModifyStaff());
		ti.setModifyStaffId(qACheckIndex.getModifyStaffId());
		ti.setCreatePositionId(qACheckIndex.getCreatePositionId());
		ti.setPositionLayRec(qACheckIndex.getPositionLayRec());
		ti.setTableNo(qACheckIndex.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_checkIndex");
		ti.setTargetTableName("LIMSBASIC_CHECK_INDICES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_checkIndex_QACheckIndex" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  qACheckIndexDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"qACheckIndex\"","MESBasic_1_checkIndex","","LIMSBASIC_CHECK_INDICES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"qACheckIndex\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"qACheckIndex\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"qACheckIndex\"","MESBasic_1_checkIndex","","LIMSBASIC_CHECK_INDICES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("qACheckIndex.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"qACheckIndex\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"qACheckIndex\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"qACheckIndex\".CID = 1 OR \"qACheckIndex\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("qACheckIndex.mc\"").append(".QACHECK_INDEX) not in ("+notinValue+")");
			}
			
			SQLQuery query = qACheckIndexDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("checkIndex", "MESBasic_1_checkIndex", "checkIndex_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
			//新的编码字段不需要初始化pattern
		this.codeCounter = this.counterManager.addCounter("MESBasic_1_checkIndex_QACheckIndex_code", "MESBasic_1_checkIndex", formatStr, CounterType.All);
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=qACheckIndexDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicQACheckIndex.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicQACheckIndex.MODULE_CODE;}
	protected String getModelCode(){return MESBasicQACheckIndex.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param qACheckIndex 模型实例
	 */
	public void checkUniqueConstraint(MESBasicQACheckIndex qACheckIndex){
		if(qACheckIndex.getId()==null){
			List<MESBasicQACheckIndex> qACheckIndexs = qACheckIndexDao.findByCriteria(Restrictions.eq("code", qACheckIndex.getCode()), Restrictions.eq("valid", true));
			if(qACheckIndexs!=null && !qACheckIndexs.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1491476504907", getCurrentUser().getLanguage()), qACheckIndex.getCode());
			}
		} else {
			List<MESBasicQACheckIndex> qACheckIndexs = qACheckIndexDao.findByCriteria(Restrictions.ne("id", qACheckIndex.getId()), Restrictions.eq("code", qACheckIndex.getCode()), Restrictions.eq("valid", true));
			if(qACheckIndexs!=null && !qACheckIndexs.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1491476504907", getCurrentUser().getLanguage()), qACheckIndex.getCode());
			}
	...
	@Override
	public List<MESBasicQACheckIndex> getQACheckIndexs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicQACheckIndex.class);
		if(params!=null){
			...
			return qACheckIndexDao.findByHql("from " + MESBasicQACheckIndex.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return qACheckIndexDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicQACheckIndex> findQACheckIndexsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicQACheckIndex.class);
	...
	@Override
	public List<MESBasicQACheckIndex> findQACheckIndexsByHql(String hql, Object... objects){
		
		return   qACheckIndexDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicQACheckIndex loadQACheckIndexByBussinessKey(MESBasicQACheckIndex qACheckIndex){
		return loadQACheckIndexByBussinessKey(qACheckIndex.getCode());
	}
	
	@Override
	public Page<MESBasicQACheckIndex> getQACheckIndexs(Page<MESBasicQACheckIndex> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicQACheckIndex.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = qACheckIndexDao.findByPage(page, sql);
			}else{
				page = qACheckIndexDao.findAllByPage(page);
			}
			return page;
		}
		page = qACheckIndexDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicQACheckIndex loadQACheckIndexByBussinessKey(Serializable bussinessKey){
		return qACheckIndexDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicQACheckIndex> findByProperty(String propertyName, Object object){
		return qACheckIndexDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicQACheckIndex findEntityByProperty(String propertyName, Object object){
		return qACheckIndexDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicQACheckIndex.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = qACheckIndexDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_checkIndex_QACheckIndex","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_checkIndex", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicQACheckIndex qACheckIndex,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,qACheckIndexDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicQACheckIndex.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, qACheckIndex);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",qACheckIndex.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicQACheckIndex.TABLE_NAME +" where valid = 1";
			list = qACheckIndexDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicQACheckIndex.TABLE_NAME;
			list = qACheckIndexDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicQACheckIndex.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = qACheckIndexDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicQACheckIndexDaoImpl qACheckIndexDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchQACheckIndex(final List<MESBasicQACheckIndex> insertObjs, final List<MESBasicQACheckIndex> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchQACheckIndex(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchQACheckIndex(final List<MESBasicQACheckIndex> insertObjs, final List<MESBasicQACheckIndex> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1491476207406"),importCount));
		...
		Map<Long, MESBasicQACheckIndex> oldUpdateObjsMap = new HashMap<Long, MESBasicQACheckIndex>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicQACheckIndex bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicQACheckIndex> bizObjList = qACheckIndexDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicQACheckIndex bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicQACheckIndex> nmeObjs = new ArrayList<MESBasicQACheckIndex>();
		...
		long id=autoGeneratorID.getNextId(MESBasicQACheckIndex.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicQACheckIndex qACheckIndex:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicQACheckIndex.TABLE_NAME,1,dbId);
			if(qACheckIndex.getCode() != null && !qACheckIndex.getCode().equals("") ){
				m.put(qACheckIndex.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) qACheckIndexDao.createNativeQuery(sql, "MESBasic_1_checkIndex_QACheckIndex"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		qACheckIndexDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicQACheckIndex.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicQACheckIndex qACheckIndex = updateObjs.get(updateIndex);
						Long id = qACheckIndex.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = qACheckIndex.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(qACheckIndex);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicQACheckIndex.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicQACheckIndex.TABLE_NAME + " order by id desc";
//					Query query = qACheckIndexDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicQACheckIndex qACheckIndex = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = qACheckIndex.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		qACheckIndexDaoImpl.getSession().flush();
		qACheckIndexDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicQACheckIndex bizObj = insertObjs.get(insertIndex);
			qACheckIndexImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicQACheckIndex bizObj = updateObjs.get(updateIndex);
			MESBasicQACheckIndex oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			qACheckIndexImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = qACheckIndexDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				qACheckIndexDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = qACheckIndexDao.createNativeQuery(sql, "MESBasic_1_checkIndex_QACheckIndex").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = qACheckIndexDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = qACheckIndexDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = qACheckIndexDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?qACheckIndexDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  qACheckIndexDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,qACheckIndexDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="ec.flow.save")
	public void saveQACheckIndex(MESBasicQACheckIndex qACheckIndex, DataGridService dataGridService){
		saveQACheckIndex(qACheckIndex, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="ec.flow.save")
	public void saveQACheckIndex(MESBasicQACheckIndex qACheckIndex, DataGridService dataGridService, String viewCode){
		saveQACheckIndex(qACheckIndex, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex")
	public void saveQACheckIndex(MESBasicQACheckIndex qACheckIndex, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveQACheckIndex(qACheckIndex, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex")
	public void saveQACheckIndex(MESBasicQACheckIndex qACheckIndex, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_checkIndex";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_checkIndex");
		String url = null;
		if(null != qACheckIndex.getId() && qACheckIndex.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicQACheckIndex.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex")
	public Map<Object, Object> qACheckIndexDataGridImport(MESBasicQACheckIndex qACheckIndex, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = qACheckIndex.getVirtualId();
		
		qACheckIndex = qACheckIndexDao.load(qACheckIndex.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, qACheckIndex.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = qACheckIndex.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(qACheckIndex);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, qACheckIndex.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex")
	public void mergeQACheckIndex(MESBasicQACheckIndex qACheckIndex, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(qACheckIndex);
		beforeSaveQACheckIndex(qACheckIndex);
		qACheckIndexDao.merge(qACheckIndex);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_checkIndex",model="MESBasic_1_checkIndex_QACheckIndex",desc="foundation.batch.save",operType="4")
	public void batchSaveQACheckIndex(MESBasicQACheckIndex qACheckIndex, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_checkIndex";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_checkIndex");
	...
	@Override
	public Page<MESBasicQACheckIndex> getByPage(Page<MESBasicQACheckIndex> page,DetachedCriteria detachedCriteria){
		return qACheckIndexDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_checkIndex").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveQACheckIndex(MESBasicQACheckIndex qACheckIndex, Object... objects){
		checkUniqueConstraint(qACheckIndex);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveQACheckIndex(MESBasicQACheckIndex qACheckIndex, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportQACheckIndex(List<MESBasicQACheckIndex> qACheckIndexs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportQACheckIndex(List<MESBasicQACheckIndex> qACheckIndexs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportQACheckIndex(List<MESBasicQACheckIndex> qACheckIndexs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportQACheckIndex(List<MESBasicQACheckIndex> qACheckIndexs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteQACheckIndex(MESBasicQACheckIndex qACheckIndex){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteQACheckIndex(MESBasicQACheckIndex qACheckIndex){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreQACheckIndex(MESBasicQACheckIndex qACheckIndex){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreQACheckIndex(MESBasicQACheckIndex qACheckIndex){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicQACheckIndex qACheckIndex, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicQACheckIndex qACheckIndex, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportQACheckIndex(List<MESBasicQACheckIndex> insertObjs, List<MESBasicQACheckIndex> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportQACheckIndex,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportQACheckIndex(List<MESBasicQACheckIndex> insertObjs, List<MESBasicQACheckIndex> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportQACheckIndex,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
