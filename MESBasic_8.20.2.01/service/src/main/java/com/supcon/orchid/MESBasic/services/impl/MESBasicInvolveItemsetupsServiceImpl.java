...
import com.supcon.orchid.MESBasic.services.MESBasicInvolveItemsetupsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicInvolveItemsetupsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_involveItemsetupsService")
@Transactional
public class MESBasicInvolveItemsetupsServiceImpl  extends BaseServiceImpl<MESBasicInvolveItemsetups> implements MESBasicInvolveItemsetupsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicInvolveItemsetups> bapBaseService;
	...
	@Autowired
	private MESBasicInvolveItemsetupsImportService involveItemsetupsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicInvolveItemsetups.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  involveItemsetupsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicInvolveItemsetups.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = involveItemsetupsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicInvolveItemsetups.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicInvolveItemsetups.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  involveItemsetupsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicInvolveItemsetups.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicInvolveItemsetups.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicInvolveItemsetups> list = (List<MESBasicInvolveItemsetups>) involveItemsetupsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicInvolveItemsetups getInvolveItemsetups(long id){
		return getInvolveItemsetups(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicInvolveItemsetups getInvolveItemsetups(long id, String viewCode){
		MESBasicInvolveItemsetups involveItemsetups = involveItemsetupsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getInvolveItemsetupsAsJSON(long id, String include){
		MESBasicInvolveItemsetups involveItemsetups = involveItemsetupsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(involveItemsetups, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.common.delete",operType="3")
	public void deleteInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups){
		if(null != involveItemsetups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicInvolveItemsetups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
			AuditUtil.setColumnStringA(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
			AuditUtil.setColumnStringC(involveItemsetups.getId().toString());
		}
		beforeDeleteInvolveItemsetups(involveItemsetups);
		involveItemsetupsDao.deletePhysical(involveItemsetups);
		afterDeleteInvolveItemsetups(involveItemsetups);
		involveItemsetupsDao.flush();
		List<MESBasicInvolveItemsetups> params = new ArrayList<MESBasicInvolveItemsetups>();
		params.add(involveItemsetups);
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.common.delete",operType="3")
	public void deleteInvolveItemsetups(List<Long> involveItemsetupsIds){
		deleteInvolveItemsetups(involveItemsetupsIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.common.delete",operType="3")
	public void deleteInvolveItemsetups(List<Long> involveItemsetupsIds, String eventTopic) {
		List<MESBasicInvolveItemsetups> involveItemsetupss = new ArrayList<MESBasicInvolveItemsetups>();
		for(Long involveItemsetupsId : involveItemsetupsIds){
			MESBasicInvolveItemsetups involveItemsetups = getInvolveItemsetups(involveItemsetupsId);
			involveItemsetupss.add(involveItemsetups);
			if(null != involveItemsetups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicInvolveItemsetups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == involveItemsetups.getId() ? "" : "," + involveItemsetups.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == involveItemsetups.getId() ? "" : "," + involveItemsetups.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == involveItemsetups.getId() ? "" : "," + involveItemsetups.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(involveItemsetupss != null && involveItemsetupss.size() > 0 ){
			for(MESBasicInvolveItemsetups involveItemsetups : involveItemsetupss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicInvolveItemsetups.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(involveItemsetups));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),involveItemsetups, involveItemsetups.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(involveItemsetupsIds != null && involveItemsetupsIds.size() > 0) {
			String hql = "delete " + MESBasicInvolveItemsetups.JPA_NAME + " where id in(:ids)";
			Query query = involveItemsetupsDao.createQuery(hql);
			query.setParameterList("ids", involveItemsetupsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicInvolveItemsetups involveItemsetupsz : involveItemsetupss){
				involveItemsetupsz.setValid(false);
				List<MESBasicInvolveItemsetups> params = new ArrayList<MESBasicInvolveItemsetups>();
				params.add(involveItemsetupsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_routine");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.common.delete",operType="3")
	public void deleteInvolveItemsetups(long involveItemsetupsId, int involveItemsetupsVersion){
		this.deleteInvolveItemsetups(involveItemsetupsId, involveItemsetupsVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.common.delete",operType="3")
	public void deleteInvolveItemsetups(long involveItemsetupsId, int involveItemsetupsVersion,SignatureLog signatureLog){
		MESBasicInvolveItemsetups involveItemsetups = getInvolveItemsetups(involveItemsetupsId);
		...
		beforeDeleteInvolveItemsetups(involveItemsetups);
		involveItemsetupsDao.delete(involveItemsetupsId, involveItemsetupsVersion);
		afterDeleteInvolveItemsetups(involveItemsetups);
		involveItemsetupsDao.flush();
		List<MESBasicInvolveItemsetups> params = new ArrayList<MESBasicInvolveItemsetups>();
		params.add(involveItemsetups);
		...
		if(signatureLog != null ) {
			...
			businessKey=involveItemsetups.getId();
			signatureLog.setTableId(involveItemsetups.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.common.delete",operType="3")
	public void deleteInvolveItemsetups(String involveItemsetupsIds){
		this.deleteInvolveItemsetups(involveItemsetupsIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.common.delete",operType="3")
	public void deleteInvolveItemsetups(String involveItemsetupsIds,SignatureLog signatureLog){
		deleteCollection(involveItemsetupsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteInvolveItemsetups(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_routine#modelCode:MESBasic_1_routine_InvolveItemsetups#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="ec.engine.restore",operType="10")
	public void restoreInvolveItemsetups(String involveItemsetupsIds){
		restoreCollection(involveItemsetupsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreInvolveItemsetups(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="ec.engine.restore",operType="10")
	public void restoreInvolveItemsetups(long involveItemsetupsId){
		
		findBusinessKeyUsed(involveItemsetupsId);	//判断业务主键是否重复
		...
		beforeRestoreInvolveItemsetups(involveItemsetups);
		involveItemsetups.setValid(true);
		involveItemsetupsDao.update(involveItemsetups);
		afterRestoreInvolveItemsetups(involveItemsetups);
		List<MESBasicInvolveItemsetups> params = new ArrayList<MESBasicInvolveItemsetups>();
		params.add(involveItemsetups);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long involveItemsetupsId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_routine_InvolveItemsetups");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicInvolveItemsetups.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicInvolveItemsetups.TABLE_NAME +" where id = ? )";
			List<Object> list =  involveItemsetupsDao.createNativeQuery(sql,involveItemsetupsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseInvolveItemsetups(List<MESBasicInvolveItemsetups>  involveItemsetupss){
		for(MESBasicInvolveItemsetups involveItemsetups:involveItemsetupss)  {
			saveInvolveItemsetups(involveItemsetups, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseInvolveItemsetups(List<MESBasicInvolveItemsetups>  involveItemsetupss){
		for(MESBasicInvolveItemsetups involveItemsetups:involveItemsetupss)  {
			saveInvolveItemsetups(involveItemsetups, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="ec.flow.save")
	public void saveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveInvolveItemsetups(involveItemsetups, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="ec.flow.save")
	public void saveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveInvolveItemsetups(involveItemsetups, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateInvolveItemsetupsCodes(MESBasicInvolveItemsetups involveItemsetups) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateInvolveItemsetupsCodes(involveItemsetups, false);
	}
	
	public void generateInvolveItemsetupsCodes(MESBasicInvolveItemsetups involveItemsetups, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateInvolveItemsetupsSummarys(MESBasicInvolveItemsetups involveItemsetups) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateInvolveItemsetupsSummarys(involveItemsetups, false);
	}

	public void generateInvolveItemsetupsSummarys(MESBasicInvolveItemsetups involveItemsetups, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups")
	public void saveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveInvolveItemsetups(involveItemsetups, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups")
	public void saveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_routine");
		String url = null;
		if(involveItemsetups.getId() != null && involveItemsetups.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicInvolveItemsetups.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, involveItemsetups);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(involveItemsetups);
		beforeSaveInvolveItemsetups(involveItemsetups, viewIsView);

		if (viewIsView) {
			involveItemsetupsDao.saveWithRevertVersion(involveItemsetups);
		} else {
			if(isNew)
				involveItemsetupsDao.save(involveItemsetups);
			else
				involveItemsetupsDao.merge(involveItemsetups);
		}
		if(null != involveItemsetups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicInvolveItemsetups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
			AuditUtil.setColumnStringA(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
		...
		dealDatagridsSave(involveItemsetups,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateInvolveItemsetupsCodes(involveItemsetups, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateInvolveItemsetupsSummarys(involveItemsetups, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveInvolveItemsetups(involveItemsetups, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, involveItemsetups);
			}
		}
		List<MESBasicInvolveItemsetups> params = new ArrayList<MESBasicInvolveItemsetups>();
		involveItemsetupsDao.flush();
		involveItemsetupsDao.clear();
		involveItemsetups = involveItemsetupsDao.load(involveItemsetups.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicInvolveItemsetups involveItemsetups){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("involveItemsetups", involveItemsetups);
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups")
	public void mergeInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(involveItemsetups);
		beforeSaveInvolveItemsetups(involveItemsetups);
		involveItemsetupsDao.merge(involveItemsetups);
		if(null != involveItemsetups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicInvolveItemsetups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
			AuditUtil.setColumnStringA(null == involveItemsetups.getId() ? "" : involveItemsetups.getId().toString());
		...
		dealDatagridsSave(involveItemsetups,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveInvolveItemsetups(involveItemsetups);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicInvolveItemsetups> findInvolveItemsetupss(Page<MESBasicInvolveItemsetups> page, Criterion... criterions) {
		return involveItemsetupsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInvolveItemsetupss(Page<MESBasicInvolveItemsetups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findInvolveItemsetupss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInvolveItemsetupss(Page<MESBasicInvolveItemsetups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findInvolveItemsetupss(Page<MESBasicInvolveItemsetups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicInvolveItemsetups> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"involveItemsetups\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"involveItemsetups\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"involveItemsetups\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"involveItemsetups\".ROUTINE IN ( SELECT ID FROM MESBASIC_ROUTINES \"routine\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "MESBasic_1_routine" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  involveItemsetupsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  involveItemsetupsDao.createNativeQuery(powerOperateSql, "MESBasic_1_routine").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"routine\"","MESBasic_1_routine","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"involveItemsetups\".LAY_REC ASC, \"involveItemsetups\".SORT ASC");
				} else {
					orderPart.append(" \"involveItemsetups\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) involveItemsetupsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=involveItemsetupsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = involveItemsetupsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicInvolveItemsetups> result = new ArrayList<MESBasicInvolveItemsetups>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicInvolveItemsetups.class,involveItemsetupsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicInvolveItemsetups.class,involveItemsetupsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicInvolveItemsetups involveItemsetups : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(involveItemsetups.getId(), "MESBasic_routine_involveItemsetups");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(involveItemsetups.getId(), "MESBasic_routine_involveItemsetups");
							if(null != documents && !documents.isEmpty()) {
								involveItemsetups.setDocument(documents.get(0));
								involveItemsetups.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicInvolveItemsetups> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicInvolveItemsetups> result) {
		View view = null;
		...
		Map<Long, MESBasicInvolveItemsetups> rsMap = new HashMap<Long, MESBasicInvolveItemsetups>();
		for (MESBasicInvolveItemsetups involveItemsetups : result) {
			ids.add(involveItemsetups.getId());
			rsMap.put(involveItemsetups.getId(), involveItemsetups);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(involveItemsetupsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = involveItemsetupsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_routine_InvolveItemsetups" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_routine_InvolveItemsetups" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  involveItemsetupsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"involveItemsetups\"","MESBasic_1_routine","","MESBASIC_INVOLVE_ITEMSETUPS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"involveItemsetups\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"involveItemsetups\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"involveItemsetups\"","MESBasic_1_routine","","MESBASIC_INVOLVE_ITEMSETUPS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_routine_InvolveItemsetups" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("involveItemsetups.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"involveItemsetups\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"involveItemsetups\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"involveItemsetups\".CID = 1 OR \"involveItemsetups\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("involveItemsetups.mc\"").append(".INVOLVE_ITEMSETUPS) not in ("+notinValue+")");
			}
			
			SQLQuery query = involveItemsetupsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=involveItemsetupsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicInvolveItemsetups.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicInvolveItemsetups.MODULE_CODE;}
	protected String getModelCode(){return MESBasicInvolveItemsetups.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param involveItemsetups 模型实例
	 */
	public void checkUniqueConstraint(MESBasicInvolveItemsetups involveItemsetups){
	}
	
	
	@Override
	public List<MESBasicInvolveItemsetups> getInvolveItemsetupss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicInvolveItemsetups.class);
		if(params!=null){
			...
			return involveItemsetupsDao.findByHql("from " + MESBasicInvolveItemsetups.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return involveItemsetupsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicInvolveItemsetups> findInvolveItemsetupssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicInvolveItemsetups.class);
	...
	@Override
	public List<MESBasicInvolveItemsetups> findInvolveItemsetupssByHql(String hql, Object... objects){
		
		return   involveItemsetupsDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicInvolveItemsetups loadInvolveItemsetupsByBussinessKey(MESBasicInvolveItemsetups involveItemsetups){
		return loadInvolveItemsetupsByBussinessKey(involveItemsetups.getId());
	}
	
	@Override
	public Page<MESBasicInvolveItemsetups> getInvolveItemsetupss(Page<MESBasicInvolveItemsetups> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicInvolveItemsetups.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = involveItemsetupsDao.findByPage(page, sql);
			}else{
				page = involveItemsetupsDao.findAllByPage(page);
			}
			return page;
		}
		page = involveItemsetupsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicInvolveItemsetups loadInvolveItemsetupsByBussinessKey(Serializable bussinessKey){
		return involveItemsetupsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicInvolveItemsetups> findByProperty(String propertyName, Object object){
		return involveItemsetupsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicInvolveItemsetups findEntityByProperty(String propertyName, Object object){
		return involveItemsetupsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicInvolveItemsetups.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = involveItemsetupsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_routine_InvolveItemsetups","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicInvolveItemsetups involveItemsetups,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,involveItemsetupsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicInvolveItemsetups.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, involveItemsetups);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",involveItemsetups.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicInvolveItemsetups.TABLE_NAME +" where valid = 1";
			list = involveItemsetupsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicInvolveItemsetups.TABLE_NAME;
			list = involveItemsetupsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicInvolveItemsetups.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = involveItemsetupsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicInvolveItemsetupsDaoImpl involveItemsetupsDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchInvolveItemsetups(final List<MESBasicInvolveItemsetups> insertObjs, final List<MESBasicInvolveItemsetups> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchInvolveItemsetups(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchInvolveItemsetups(final List<MESBasicInvolveItemsetups> insertObjs, final List<MESBasicInvolveItemsetups> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1426644977327"),importCount));
		...
		Map<Long, MESBasicInvolveItemsetups> oldUpdateObjsMap = new HashMap<Long, MESBasicInvolveItemsetups>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicInvolveItemsetups bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicInvolveItemsetups> bizObjList = involveItemsetupsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicInvolveItemsetups bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicInvolveItemsetups> nmeObjs = new ArrayList<MESBasicInvolveItemsetups>();
		...
		long id=autoGeneratorID.getNextId(MESBasicInvolveItemsetups.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicInvolveItemsetups involveItemsetups:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicInvolveItemsetups.TABLE_NAME,1,dbId);
			if(involveItemsetups.getId() != null ){
				m.put(involveItemsetups.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) involveItemsetupsDao.createNativeQuery(sql, "MESBasic_1_routine_InvolveItemsetups"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		involveItemsetupsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicInvolveItemsetups.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicInvolveItemsetups involveItemsetups = updateObjs.get(updateIndex);
						Long id = involveItemsetups.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = involveItemsetups.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(involveItemsetups);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicInvolveItemsetups.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicInvolveItemsetups.TABLE_NAME + " order by id desc";
//					Query query = involveItemsetupsDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicInvolveItemsetups involveItemsetups = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = involveItemsetups.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		involveItemsetupsDaoImpl.getSession().flush();
		involveItemsetupsDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicInvolveItemsetups bizObj = insertObjs.get(insertIndex);
			involveItemsetupsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicInvolveItemsetups bizObj = updateObjs.get(updateIndex);
			MESBasicInvolveItemsetups oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			involveItemsetupsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicInvolveItemsetups> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicInvolveItemsetups> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) involveItemsetupsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicInvolveItemsetups> result = getResult(page, query, new PendingResultTransformer(MESBasicInvolveItemsetups.class,involveItemsetupsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicInvolveItemsetups model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = involveItemsetupsDao.createNativeQuery(sql, "MESBasic_1_routine_InvolveItemsetups").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = involveItemsetupsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = involveItemsetupsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = involveItemsetupsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?involveItemsetupsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  involveItemsetupsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,involveItemsetupsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="ec.flow.save")
	public void saveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, DataGridService dataGridService){
		saveInvolveItemsetups(involveItemsetups, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="ec.flow.save")
	public void saveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, DataGridService dataGridService, String viewCode){
		saveInvolveItemsetups(involveItemsetups, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups")
	public void saveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveInvolveItemsetups(involveItemsetups, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups")
	public void saveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
		String url = null;
		if(null != involveItemsetups.getId() && involveItemsetups.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicInvolveItemsetups.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups")
	public Map<Object, Object> involveItemsetupsDataGridImport(MESBasicInvolveItemsetups involveItemsetups, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = involveItemsetups.getVirtualId();
		
		involveItemsetups = involveItemsetupsDao.load(involveItemsetups.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, involveItemsetups.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = involveItemsetups.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(involveItemsetups);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, involveItemsetups.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups")
	public void mergeInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(involveItemsetups);
		beforeSaveInvolveItemsetups(involveItemsetups);
		involveItemsetupsDao.merge(involveItemsetups);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_InvolveItemsetups",desc="foundation.batch.save",operType="4")
	public void batchSaveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
	...
	@Override
	public Page<MESBasicInvolveItemsetups> getByPage(Page<MESBasicInvolveItemsetups> page,DetachedCriteria detachedCriteria){
		return involveItemsetupsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, Object... objects){
		checkUniqueConstraint(involveItemsetups);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportInvolveItemsetups(List<MESBasicInvolveItemsetups> involveItemsetupss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportInvolveItemsetups(List<MESBasicInvolveItemsetups> involveItemsetupss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportInvolveItemsetups(List<MESBasicInvolveItemsetups> involveItemsetupss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportInvolveItemsetups(List<MESBasicInvolveItemsetups> involveItemsetupss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreInvolveItemsetups(MESBasicInvolveItemsetups involveItemsetups){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicInvolveItemsetups involveItemsetups, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicInvolveItemsetups involveItemsetups, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportInvolveItemsetups(List<MESBasicInvolveItemsetups> insertObjs, List<MESBasicInvolveItemsetups> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportInvolveItemsetups,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportInvolveItemsetups(List<MESBasicInvolveItemsetups> insertObjs, List<MESBasicInvolveItemsetups> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportInvolveItemsetups,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_routine_InvolveItemsetups,MESBasic_1) */
// 自定义代码
