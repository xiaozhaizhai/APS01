...
import com.supcon.orchid.MESBasic.services.MESBasicItemRecordImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicItemRecordDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_itemRecordService")
@Transactional
public class MESBasicItemRecordServiceImpl  extends BaseServiceImpl<MESBasicItemRecord> implements MESBasicItemRecordService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicItemRecord> bapBaseService;
	...
	@Autowired
	private MESBasicItemRecordImportService itemRecordImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicItemRecord.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  itemRecordDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicItemRecord.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = itemRecordDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicItemRecord.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicItemRecord.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  itemRecordDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicItemRecord.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicItemRecord.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicItemRecord> list = (List<MESBasicItemRecord>) itemRecordDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicItemRecord getItemRecord(long id){
		return getItemRecord(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicItemRecord getItemRecord(long id, String viewCode){
		MESBasicItemRecord itemRecord = itemRecordDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getItemRecordAsJSON(long id, String include){
		MESBasicItemRecord itemRecord = itemRecordDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(itemRecord, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.common.delete",operType="3")
	public void deleteItemRecord(MESBasicItemRecord itemRecord){
		if(null != itemRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
			AuditUtil.setColumnStringA(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
			AuditUtil.setColumnStringC(itemRecord.getId().toString());
		}
		beforeDeleteItemRecord(itemRecord);
		itemRecordDao.delete(itemRecord);
		afterDeleteItemRecord(itemRecord);
		itemRecordDao.flush();
		List<MESBasicItemRecord> params = new ArrayList<MESBasicItemRecord>();
		params.add(itemRecord);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_itemRecord");
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.common.delete",operType="3")
	public void deleteItemRecord(List<Long> itemRecordIds){
		deleteItemRecord(itemRecordIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.common.delete",operType="3")
	public void deleteItemRecord(List<Long> itemRecordIds, String eventTopic) {
		List<MESBasicItemRecord> itemRecords = new ArrayList<MESBasicItemRecord>();
		for(Long itemRecordId : itemRecordIds){
			MESBasicItemRecord itemRecord = getItemRecord(itemRecordId);
			itemRecords.add(itemRecord);
			if(null != itemRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == itemRecord.getId() ? "" : "," + itemRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == itemRecord.getId() ? "" : "," + itemRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == itemRecord.getId() ? "" : "," + itemRecord.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
				}
			}
		}
		...
		if(itemRecords != null && itemRecords.size() > 0 ){
			for(MESBasicItemRecord itemRecord : itemRecords){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),itemRecord, itemRecord.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(itemRecordIds != null && itemRecordIds.size() > 0) {
			String hql = "update " + MESBasicItemRecord.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = itemRecordDao.createQuery(hql);
			query.setParameterList("ids", itemRecordIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicItemRecord itemRecordz : itemRecords){
				itemRecordz.setValid(false);
				List<MESBasicItemRecord> params = new ArrayList<MESBasicItemRecord>();
				params.add(itemRecordz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_itemRecord");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.common.delete",operType="3")
	public void deleteItemRecord(long itemRecordId, int itemRecordVersion){
		this.deleteItemRecord(itemRecordId, itemRecordVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.common.delete",operType="3")
	public void deleteItemRecord(long itemRecordId, int itemRecordVersion,SignatureLog signatureLog){
		MESBasicItemRecord itemRecord = getItemRecord(itemRecordId);
		if(itemRecord != null && !itemRecord.isValid()){
		...
		beforeDeleteItemRecord(itemRecord);
		itemRecordDao.delete(itemRecordId, itemRecordVersion);
		afterDeleteItemRecord(itemRecord);
		itemRecordDao.flush();
		List<MESBasicItemRecord> params = new ArrayList<MESBasicItemRecord>();
		params.add(itemRecord);
		...
		if(signatureLog != null ) {
			...
			businessKey=itemRecord.getId();
			signatureLog.setTableId(itemRecord.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.common.delete",operType="3")
	public void deleteItemRecord(String itemRecordIds){
		this.deleteItemRecord(itemRecordIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.common.delete",operType="3")
	public void deleteItemRecord(String itemRecordIds,SignatureLog signatureLog){
		deleteCollection(itemRecordIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteItemRecord(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_itemRecord#modelCode:MESBasic_1_itemRecord_ItemRecord#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="ec.engine.restore",operType="10")
	public void restoreItemRecord(String itemRecordIds){
		restoreCollection(itemRecordIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreItemRecord(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="ec.engine.restore",operType="10")
	public void restoreItemRecord(long itemRecordId){
		
		findBusinessKeyUsed(itemRecordId);	//判断业务主键是否重复
		...
		beforeRestoreItemRecord(itemRecord);
		itemRecord.setValid(true);
		itemRecordDao.update(itemRecord);
		afterRestoreItemRecord(itemRecord);
		List<MESBasicItemRecord> params = new ArrayList<MESBasicItemRecord>();
		params.add(itemRecord);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long itemRecordId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_itemRecord_ItemRecord");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicItemRecord.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicItemRecord.TABLE_NAME +" where id = ? )";
			List<Object> list =  itemRecordDao.createNativeQuery(sql,itemRecordId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseItemRecord(List<MESBasicItemRecord>  itemRecords){
		for(MESBasicItemRecord itemRecord:itemRecords)  {
			saveItemRecord(itemRecord, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseItemRecord(List<MESBasicItemRecord>  itemRecords){
		for(MESBasicItemRecord itemRecord:itemRecords)  {
			saveItemRecord(itemRecord, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="ec.flow.save")
	public void saveItemRecord(MESBasicItemRecord itemRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveItemRecord(itemRecord, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="ec.flow.save")
	public void saveItemRecord(MESBasicItemRecord itemRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveItemRecord(itemRecord, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateItemRecordCodes(MESBasicItemRecord itemRecord) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateItemRecordCodes(itemRecord, false);
	}
	
	public void generateItemRecordCodes(MESBasicItemRecord itemRecord, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateItemRecordSummarys(MESBasicItemRecord itemRecord) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateItemRecordSummarys(itemRecord, false);
	}

	public void generateItemRecordSummarys(MESBasicItemRecord itemRecord, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord")
	public void saveItemRecord(MESBasicItemRecord itemRecord,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveItemRecord(itemRecord, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord")
	public void saveItemRecord(MESBasicItemRecord itemRecord,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_itemRecord";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_itemRecord");
		String url = null;
		if(itemRecord.getId() != null && itemRecord.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_itemRecord/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_itemRecord/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, itemRecord);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(itemRecord);
		beforeSaveItemRecord(itemRecord, viewIsView);

		if (viewIsView) {
			itemRecordDao.saveWithRevertVersion(itemRecord);
		} else {
			if(isNew)
				itemRecordDao.save(itemRecord);
			else
				itemRecordDao.merge(itemRecord);
		}
		if(null != itemRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
			AuditUtil.setColumnStringA(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
		...
		dealDatagridsSave(itemRecord,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateItemRecordCodes(itemRecord, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateItemRecordSummarys(itemRecord, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveItemRecord(itemRecord, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, itemRecord);
			}
		}
		List<MESBasicItemRecord> params = new ArrayList<MESBasicItemRecord>();
		itemRecordDao.flush();
		itemRecordDao.clear();
		itemRecord = itemRecordDao.load(itemRecord.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicItemRecord itemRecord){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("itemRecord", itemRecord);
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord")
	public void mergeItemRecord(MESBasicItemRecord itemRecord, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(itemRecord);
		beforeSaveItemRecord(itemRecord);
		itemRecordDao.merge(itemRecord);
		if(null != itemRecord && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
			AuditUtil.setColumnStringA(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
		...
		dealDatagridsSave(itemRecord,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveItemRecord(itemRecord);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicItemRecord> findItemRecords(Page<MESBasicItemRecord> page, Criterion... criterions) {
		return itemRecordDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicItemRecordDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicItemRecordDealInfo> list=itemRecordDao.createQuery("from " + MESBasicItemRecordDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicItemRecordDealInfo dealInfo) {
		itemRecordDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicItemRecordDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) itemRecordDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicItemRecordDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = itemRecordDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = itemRecordDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicItemRecordDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = itemRecordDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemRecords(Page<MESBasicItemRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findItemRecords(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemRecords(Page<MESBasicItemRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemRecords(Page<MESBasicItemRecord> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicItemRecord> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"itemRecord\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"itemRecord\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"itemRecord\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_itemRecord_ItemRecord" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_itemRecord_ItemRecord" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  itemRecordDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  itemRecordDao.createNativeQuery(powerOperateSql, "MESBasic_1_itemRecord").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"itemRecord\"","MESBasic_1_itemRecord",customCondition,"Pellet_ItemRecord",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"itemRecord\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicItemRecordDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"itemRecord\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"itemRecord\".LAY_REC ASC, \"itemRecord\".SORT ASC");
				} else {
					orderPart.append(" \"itemRecord\".IR_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) itemRecordDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=itemRecordDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = itemRecordDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicItemRecord> result = new ArrayList<MESBasicItemRecord>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicItemRecord.class,itemRecordDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicItemRecord.class,itemRecordDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicItemRecord itemRecord : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(itemRecord.getId(), "MESBasic_itemRecord_itemRecord");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(itemRecord.getId(), "MESBasic_itemRecord_itemRecord");
							if(null != documents && !documents.isEmpty()) {
								itemRecord.setDocument(documents.get(0));
								itemRecord.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_itemRecord")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_itemRecord")+"MESBasic_1_itemRecord".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_itemRecord",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicItemRecord> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicItemRecord> result) {
		View view = null;
		...
		Map<Long, MESBasicItemRecord> rsMap = new HashMap<Long, MESBasicItemRecord>();
		for (MESBasicItemRecord itemRecord : result) {
			ids.add(itemRecord.getId());
			rsMap.put(itemRecord.getId(), itemRecord);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(itemRecordDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = itemRecordDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicItemRecord itemRecord,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(itemRecord.getOwnerStaffId());
		ti.setOwnerPositionId(itemRecord.getOwnerPositionId());
		ti.setOwnerDepartmentId(itemRecord.getOwnerDepartmentId());
		
		ti.setCreateStaff(itemRecord.getCreateStaff());
		ti.setCreateStaffId(itemRecord.getCreateStaffId());
		ti.setCreateTime(itemRecord.getCreateTime());
		ti.setDeleteStaff(itemRecord.getDeleteStaff());
		ti.setDeleteStaffId(itemRecord.getDeleteStaffId());
		ti.setDeleteTime(itemRecord.getDeleteTime());
		ti.setCreateDepartmentId(itemRecord.getCreateDepartmentId());
		ti.setModifyStaff(itemRecord.getModifyStaff());
		ti.setModifyStaffId(itemRecord.getModifyStaffId());
		ti.setCreatePositionId(itemRecord.getCreatePositionId());
		ti.setPositionLayRec(itemRecord.getPositionLayRec());
		ti.setTableNo(itemRecord.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_itemRecord");
		ti.setTargetTableName("Pellet_ItemRecord");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_itemRecord_ItemRecord" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  itemRecordDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"itemRecord\"","MESBasic_1_itemRecord","","Pellet_ItemRecord",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"itemRecord\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"itemRecord\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"itemRecord\"","MESBasic_1_itemRecord","","Pellet_ItemRecord",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("itemRecord.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"itemRecord\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"itemRecord\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"itemRecord\".CID = 1 OR \"itemRecord\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("itemRecord.mc\"").append(".ITEM_RECORD) not in ("+notinValue+")");
			}
			
			SQLQuery query = itemRecordDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("Pellet_ItemRecord", "MESBasic_1_itemRecord", "Pellet_ItemRecord_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=itemRecordDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicItemRecord.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicItemRecord.MODULE_CODE;}
	protected String getModelCode(){return MESBasicItemRecord.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param itemRecord 模型实例
	 */
	public void checkUniqueConstraint(MESBasicItemRecord itemRecord){
	}
	
	
	@Override
	public List<MESBasicItemRecord> getItemRecords(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicItemRecord.class);
		if(params!=null){
			...
			return itemRecordDao.findByHql("from " + MESBasicItemRecord.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return itemRecordDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicItemRecord> findItemRecordsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicItemRecord.class);
	...
	@Override
	public List<MESBasicItemRecord> findItemRecordsByHql(String hql, Object... objects){
		
		return   itemRecordDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicItemRecord loadItemRecordByBussinessKey(MESBasicItemRecord itemRecord){
		return loadItemRecordByBussinessKey(itemRecord.getId());
	}
	
	@Override
	public Page<MESBasicItemRecord> getItemRecords(Page<MESBasicItemRecord> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicItemRecord.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = itemRecordDao.findByPage(page, sql);
			}else{
				page = itemRecordDao.findAllByPage(page);
			}
			return page;
		}
		page = itemRecordDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicItemRecord loadItemRecordByBussinessKey(Serializable bussinessKey){
		return itemRecordDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicItemRecord> findByProperty(String propertyName, Object object){
		return itemRecordDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicItemRecord findEntityByProperty(String propertyName, Object object){
		return itemRecordDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicItemRecord.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = itemRecordDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_itemRecord_ItemRecord","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_itemRecord", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicItemRecord itemRecord,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,itemRecordDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicItemRecord.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, itemRecord);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",itemRecord.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicItemRecord.TABLE_NAME +" where valid = 1";
			list = itemRecordDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicItemRecord.TABLE_NAME;
			list = itemRecordDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicItemRecord.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = itemRecordDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicItemRecordDaoImpl itemRecordDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchItemRecord(final List<MESBasicItemRecord> insertObjs, final List<MESBasicItemRecord> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchItemRecord(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchItemRecord(final List<MESBasicItemRecord> insertObjs, final List<MESBasicItemRecord> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1415340308405"),importCount));
		...
		Map<Long, MESBasicItemRecord> oldUpdateObjsMap = new HashMap<Long, MESBasicItemRecord>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicItemRecord bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicItemRecord> bizObjList = itemRecordDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicItemRecord bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicItemRecord> nmeObjs = new ArrayList<MESBasicItemRecord>();
		...
		long id=autoGeneratorID.getNextId(MESBasicItemRecord.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicItemRecord itemRecord:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicItemRecord.TABLE_NAME,1,dbId);
			if(itemRecord.getId() != null ){
				m.put(itemRecord.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) itemRecordDao.createNativeQuery(sql, "MESBasic_1_itemRecord_ItemRecord"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		itemRecordDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicItemRecord.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicItemRecord itemRecord = updateObjs.get(updateIndex);
						Long id = itemRecord.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = itemRecord.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicItemRecord.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicItemRecord.TABLE_NAME + " order by id desc";
//					Query query = itemRecordDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicItemRecord itemRecord = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = itemRecord.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		itemRecordDaoImpl.getSession().flush();
		itemRecordDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicItemRecord bizObj = insertObjs.get(insertIndex);
			itemRecordImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicItemRecord bizObj = updateObjs.get(updateIndex);
			MESBasicItemRecord oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			itemRecordImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = itemRecordDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				itemRecordDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = itemRecordDao.createNativeQuery(sql, "MESBasic_1_itemRecord_ItemRecord").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = itemRecordDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = itemRecordDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = itemRecordDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?itemRecordDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  itemRecordDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,itemRecordDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="ec.flow.save")
	public void saveItemRecord(MESBasicItemRecord itemRecord, DataGridService dataGridService){
		saveItemRecord(itemRecord, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="ec.flow.save")
	public void saveItemRecord(MESBasicItemRecord itemRecord, DataGridService dataGridService, String viewCode){
		saveItemRecord(itemRecord, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord")
	public void saveItemRecord(MESBasicItemRecord itemRecord, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveItemRecord(itemRecord, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord")
	public void saveItemRecord(MESBasicItemRecord itemRecord, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_itemRecord";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_itemRecord");
		String url = null;
		if(null != itemRecord.getId() && itemRecord.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemRecord.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord")
	public Map<Object, Object> itemRecordDataGridImport(MESBasicItemRecord itemRecord, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = itemRecord.getVirtualId();
		
		itemRecord = itemRecordDao.load(itemRecord.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, itemRecord.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = itemRecord.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(itemRecord);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, itemRecord.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord")
	public void mergeItemRecord(MESBasicItemRecord itemRecord, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(itemRecord);
		beforeSaveItemRecord(itemRecord);
		itemRecordDao.merge(itemRecord);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_itemRecord",model="MESBasic_1_itemRecord_ItemRecord",desc="foundation.batch.save",operType="4")
	public void batchSaveItemRecord(MESBasicItemRecord itemRecord, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_itemRecord";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_itemRecord");
	...
	@Override
	public Page<MESBasicItemRecord> getByPage(Page<MESBasicItemRecord> page,DetachedCriteria detachedCriteria){
		return itemRecordDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_itemRecord").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveItemRecord(MESBasicItemRecord itemRecord, Object... objects){
		checkUniqueConstraint(itemRecord);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveItemRecord(MESBasicItemRecord itemRecord, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportItemRecord(List<MESBasicItemRecord> itemRecords, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportItemRecord(List<MESBasicItemRecord> itemRecords){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportItemRecord(List<MESBasicItemRecord> itemRecords, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportItemRecord(List<MESBasicItemRecord> itemRecords){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteItemRecord(MESBasicItemRecord itemRecord){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteItemRecord(MESBasicItemRecord itemRecord){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreItemRecord(MESBasicItemRecord itemRecord){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreItemRecord(MESBasicItemRecord itemRecord){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicItemRecord itemRecord, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicItemRecord itemRecord, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportItemRecord(List<MESBasicItemRecord> insertObjs, List<MESBasicItemRecord> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportItemRecord,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportItemRecord(List<MESBasicItemRecord> insertObjs, List<MESBasicItemRecord> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportItemRecord,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
