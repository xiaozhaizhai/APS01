...
@Repository
public class MESBasicMESServicesDaoImpl extends ExtendedGenericDaoImpl<MESBasicMESServices, Long> implements MESBasicMESServicesDao {
	@Override
	...
	@Override
	public MESBasicMESServicesDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicMESServicesDealInfo dealInfo = new MESBasicMESServicesDealInfo();
	...
	@Override
	public MESBasicMESServicesDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicMESServices mainObj, Staff dealStaff) {
		MESBasicMESServicesDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicMESServicesDealInfo.class);
		...
		List<MESBasicMESServicesDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_backgroundServiceMonitor_MESServices,MESBasic_1) */
// 自定义代码
