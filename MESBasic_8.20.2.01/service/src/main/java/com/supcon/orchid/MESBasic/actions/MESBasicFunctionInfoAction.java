...
/* CUSTOM CODE START(action,import,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicFunctionInfoAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicFunctionInfo> {
	...
	private MESBasicFunctionInfo functionInfo;
	...
	//导出excel时,主模型内容
	List<MESBasicFunctionInfo> queryResults = new ArrayList<MESBasicFunctionInfo>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/functionInfo/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/functionInfo/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_container_FunctionInfo" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicFunctionInfo> functionInfos;
	
	public List<MESBasicFunctionInfo> getFunctionInfos() {
	...
	private Page<MESBasicFunctionInfo> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			functionInfo = new MESBasicFunctionInfo();
		} else {
			if(id > 0){
				functionInfo = functionInfoService.getFunctionInfo(id, viewCode);
			}
		}
	...
	public Page<MESBasicFunctionInfo> getPage() {
		if (null == page) {
			page = new Page<MESBasicFunctionInfo>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicFunctionInfo getFunctionInfo() {
		return functionInfo;
	}		

	public void setFunctionInfo(MESBasicFunctionInfo functionInfo) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/functionInfo/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = functionInfoService.getFunctionInfoAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/functionInfo/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1555292653876
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String functionInfoReferenceCopy() throws Exception{
			MESBasicFunctionInfo oldFunctionInfo = null;
			if(isEditNew()){
				...
				oldFunctionInfo = new MESBasicFunctionInfo();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,functionInfo.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, functionInfo.getId());
				...
				oldFunctionInfo = functionInfoService.getFunctionInfo(id);
			}
			
			
			functionInfo = functionInfoService.getFunctionInfo(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_container_FunctionInfo");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicFunctionInfo.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFunctionInfo, null);
						Method setValue = MESBasicFunctionInfo.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(functionInfo, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			functionInfo.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_container_FunctionInfo", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/container/functionInfo/expressionEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "expressionEdit.ftl") }),
		@Action(value = "/MESBasic/container/functionInfo/expressionEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "expressionEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=functionInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return functionInfoReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			functionInfo = functionInfoService.getFunctionInfo(this.functionInfo.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,functionInfo.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = functionInfoService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(functionInfoService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(functionInfoService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_container_FunctionInfo", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"functionInfo\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"functionInfo\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"functionInfo\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"functionInfo\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			functionInfoService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		functionInfoService.findDataGridPage(dg,dgclass,getDgPage(),functionInfo,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(functionInfo.getId() != null && functionInfo.getId() != -1){
				functionInfo = functionInfoService.getFunctionInfo(functionInfo.getId());
			}
			functionInfoService.findDataGridPage(dg,dgclass,getDgPage(),functionInfo,searchCondition,params);
	...
	private void save() throws Exception {
		...
		functionInfoService.saveFunctionInfo(functionInfo,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/container/functionInfo/expressionEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_FunctionInfo", "submit");
		bap_validate_datagrids=functionInfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", functionInfo.getId());
		...
		fileuploads.put("linkId", functionInfo.getId());
		bapAfterCustomMethod("MESBasic_1_container_FunctionInfo", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/container/functionInfo/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_FunctionInfo", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		functionInfoService.deleteFunctionInfo(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_container_FunctionInfo", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/functionInfo/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_FunctionInfo", "restore");
		functionInfoService.restoreFunctionInfo(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_container_FunctionInfo", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicFunctionInfo> findExportDataInfos(Page<MESBasicFunctionInfo> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = functionInfoService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = functionInfoService.getAssProperty(p.getCode());	
									//String assModelCode = functionInfoService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			functionInfoService.findFunctionInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			functionInfoService.findFunctionInfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicFunctionInfo> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicFunctionInfo result : results){
	...
	public boolean isEditNew(){
		//return null == this.functionInfo || null == this.functionInfo.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/container/functionInfo/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_FunctionInfo", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_container_FunctionInfo", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/container/functionInfo/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_FunctionInfo", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/container/functionInfo/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/functionInfo/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicFunctionInfoService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_container_FunctionInfo", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_container_FunctionInfo", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/functionInfo/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_container_FunctionInfo", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_container_FunctionInfo", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/container/functionInfo/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_FunctionInfo", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/container/functionInfo/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_container_FunctionInfo";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_container_FunctionInfo", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/container/functionInfo/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicFunctionInfo functionInfo = null;
		if(null != id) {
			functionInfo = functionInfoService.getFunctionInfo(id);
		}
		...
		_auditLog.setColumnStringB(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
		_auditLog.setColumnStringA(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
		_auditLog.setColumnStringC(functionInfo.getId().toString());
		_auditLog.setColumnStringD(MESBasicFunctionInfo.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicFunctionInfoAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, functionInfo,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, functionInfo,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,functionInfo);
		...
		printlog.setBusinessId(functionInfo.getId());
		printlog.setBussinessKey(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
		printlog.setMainDisplay(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/container/functionInfo/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(functionInfo,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/functionInfo/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(functionInfoService.getFunctionInfo(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/container/functionInfo/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/container/functionInfo/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, functionInfo,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				functionInfo = functionInfoService.getFunctionInfo(Long.parseLong(arrId[i]));
				if(null != functionInfo){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == functionInfo.getId() ? "" : "," + functionInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == functionInfo.getId() ? "" : "," + functionInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == functionInfo.getId() ? "" : "," + functionInfo.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == functionInfo.getId() ? "" : functionInfo.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, functionInfo,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,functionInfo);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/container/functionInfo/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_container_FunctionInfo");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_container_FunctionInfo")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_container_FunctionInfo");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_container_FunctionInfo");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = functionInfoService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_container_FunctionInfo,MESBasic_1) */
// 自定义代码
