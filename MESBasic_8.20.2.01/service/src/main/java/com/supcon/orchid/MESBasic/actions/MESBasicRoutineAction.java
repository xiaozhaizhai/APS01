...
/* CUSTOM CODE START(action,import,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
...
public class MESBasicRoutineAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicRoutine> {
	...
	//导出excel时,主模型内容
	List<MESBasicRoutine> queryResults = new ArrayList<MESBasicRoutine>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/routine/routine/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/routine/routine/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_routine_Routine" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicRoutine> routines;
	
	public List<MESBasicRoutine> getRoutines() {
		return routines;
	}
	...
	private Page<MESBasicRoutine> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			routine = new MESBasicRoutine();
			if(routine.getBusinessType() != null) {
				routine.setBusinessType((SystemCode) systemCodeService.getSystemCode(routine.getBusinessType().getId()));
			}
			if(routine.getDesType() != null) {
				routine.setDesType((SystemCode) systemCodeService.getSystemCode(routine.getDesType().getId()));
			}
			if(routine.getRootineUse() != null) {
				routine.setRootineUse((SystemCode) systemCodeService.getSystemCode(routine.getRootineUse().getId()));
			}
			if(routine.getRootType() != null) {
				routine.setRootType((SystemCode) systemCodeService.getSystemCode(routine.getRootType().getId()));
			}
			if(routine.getRoutineStatus() != null) {
				routine.setRoutineStatus((SystemCode) systemCodeService.getSystemCode(routine.getRoutineStatus().getId()));
			}
			if(routine.getRoutineStrategy() != null) {
				routine.setRoutineStrategy((SystemCode) systemCodeService.getSystemCode(routine.getRoutineStrategy().getId()));
			}
			if(routine.getSourceType() != null) {
				routine.setSourceType((SystemCode) systemCodeService.getSystemCode(routine.getSourceType().getId()));
			}
		} else {
			if(id > 0){
				routine = routineService.getRoutine(id, viewCode);
			}
		}
	...
	public Page<MESBasicRoutine> getPage() {
		if (null == page) {
			page = new Page<MESBasicRoutine>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/routine/routine/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = routineService.getRoutineAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/routine/routine/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String routineReferenceCopy() throws Exception{
			MESBasicRoutine oldRoutine = null;
			if(isEditNew()){
				...
				oldRoutine = new MESBasicRoutine();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,routine.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, routine.getId());
				...
				oldRoutine = routineService.getRoutine(id);
			}
			
			
			routine = routineService.getRoutine(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_routine_Routine");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicRoutine.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldRoutine, null);
						Method setValue = MESBasicRoutine.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(routine, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			routine.setCreateDepartmentId(oldRoutine.getCreateDepartmentId());
			routine.setCreatePositionId(oldRoutine.getCreatePositionId());
			routine.setCreateStaffId(oldRoutine.getCreateStaffId());
			routine.setOwnerDepartmentId(oldRoutine.getOwnerDepartmentId());
			routine.setOwnerPositionId(oldRoutine.getOwnerPositionId());
			routine.setOwnerStaffId(oldRoutine.getOwnerStaffId());
			routine.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_routine_Routine", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/routine/routineEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineEdit.ftl") }),
		@Action(value = "/MESBasic/routine/routine/routineEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineEditPrint.ftl")})
		,
		@Action(value = "/MESBasic/routine/routine/routineEditForTank",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineEditForTank.ftl") }),
		@Action(value = "/MESBasic/routine/routine/routineEditForTankPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineEditForTankPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=routineService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return routineReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			routine = routineService.getRoutine(this.routine.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,routine.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/routine/routineForTankView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineForTankView.ftl") }),
		@Action(value = "/MESBasic/routine/routine/routineForTankViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineForTankViewPrint.ftl")})
		,
		@Action(value = "/MESBasic/routine/routine/routineView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineView.ftl") }),
		@Action(value = "/MESBasic/routine/routine/routineViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineViewPrint.ftl")})
		,
		@Action(value = "/MESBasic/routine/routine/routineViewAll",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineViewAll.ftl") }),
		@Action(value = "/MESBasic/routine/routine/routineViewAllPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "routineViewAllPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/routine/routineList", results = { @Result(type = FREEMARKER, location = "routineList.ftl") })
		,
		@Action(value = "/MESBasic/routine/routine/routineRef3", results = { @Result(type = FREEMARKER, location = "routineRef3.ftl") })
		,
		@Action(value = "/MESBasic/routine/routine/routineTankList", results = { @Result(type = FREEMARKER, location = "routineTankList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = routineService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(routineService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(routineService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_routine_Routine", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/routine/routineRef", results = { @Result(type = FREEMARKER, location = "routineRef.ftl") })
		,
		@Action(value = "/MESBasic/routine/routine/routineRef2", results = { @Result(type = FREEMARKER, location = "routineRef2.ftl") })
		,
		@Action(value = "/MESBasic/routine/routine/routineRef4", results = { @Result(type = FREEMARKER, location = "routineRef4.ftl") })
		,
		@Action(value = "/MESBasic/routine/routine/routineTankRef", results = { @Result(type = FREEMARKER, location = "routineTankRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/routine/routineList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.srouceEqp.name,result.srouceEqpObj.equipmentNum,result.destEqp.name,result.destEquObj.equipmentNum,result.product.productName,result.density,result.routineStrategy.id,result.routineStrategy.value,result.srouceEqp.id,result.srouceEqpObj.id,result.destEqp.id,result.destEquObj.id,result.product.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="routineList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/routine/routine/routineRef3-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.srouceEqp.name,result.srouceEqpObj.equipmentNum,result.destEqp.name,result.destEquObj.equipmentNum,result.product.productName,result.density,result.srouceEqp.id,result.srouceEqpObj.id,result.destEqp.id,result.destEquObj.id,result.product.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="routineRef3-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/routine/routine/routineTankList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.routineStatus.id,result.routineStatus.value,result.rootineUse.id,result.rootineUse.value,result.srcContainer.name,result.sourceType.id,result.sourceType.value,result.desContainer.name,result.desType.id,result.desType.value,result.routineStrategy.id,result.routineStrategy.value,result.product.productName,result.density,result.businessType.id,result.businessType.value,result.srcContainer.id,result.desContainer.id,result.product.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="routineTankList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"routine\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"routine\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"routine\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"routine\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/routine/routineList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/MESBasic/routine/routine/routineRef3-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/MESBasic/routine/routine/routineTankList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/routine/routine/routineRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.srouceEqp.name,result.srouceEqpObj.equipmentNum,result.destEqp.name,result.destEquObj.equipmentNum,result.product.productName,result.density,result.routineStrategy.id,result.routineStrategy.value,result.srouceEqp.id,result.srouceEqpObj.id,result.destEqp.id,result.destEquObj.id,result.product.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="routineRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/routine/routine/routineRef2-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.destEqp.name,result.srouceEqp.name,result.product.productName,result.density,result.destEqp.id,result.srouceEqp.id,result.product.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="routineRef2-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/routine/routine/routineRef4-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.destEqp.name,result.destEquObj.equipmentNum,result.srouceEqp.name,result.srouceEqpObj.equipmentNum,result.product.productName,result.density,result.destEqp.id,result.destEquObj.id,result.srouceEqp.id,result.srouceEqpObj.id,result.product.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="routineRef4-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/routine/routine/routineTankRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.routineStatus.id,result.routineStatus.value,result.rootineUse.id,result.rootineUse.value,result.srcContainer.name,result.sourceType.id,result.sourceType.value,result.desContainer.name,result.desType.id,result.desType.value,result.routineStrategy.id,result.routineStrategy.value,result.product.productName,result.density,result.srouce,result.destination,result.businessType.id,result.businessType.value,result.srcContainer.id,result.desContainer.id,result.product.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="routineTankRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/routine/routine/allTempleteDownload", results = { @Result(name="excel",type="excel",location="routineList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_routine_Routine");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		routineService.findDataGridPage(dg,dgclass,getDgPage(),routine,"", null);
	...
	private Page<MESBasicConfig> dg1415338444009Page;
	public Page<MESBasicConfig> getDg1415338444009Page(){
		if(null == dg1415338444009Page){
			dg1415338444009Page = new Page<MESBasicConfig>(1,20);
		}
		return dg1415338444009Page;
	}
	
	private Class dg1415338444009Clazz = MESBasicConfig.class;
	
	public Class getDg1415338444009Clazz() {
		return dg1415338444009Clazz;
	}
	
	private List<MESBasicConfig> dg1415338444009List;
	public List<MESBasicConfig> getDg1415338444009List(){
		if(null == dg1415338444009List){
			dg1415338444009List = new ArrayList<MESBasicConfig>();
		}
		return dg1415338444009List;
	}
	public void setDg1415338444009ListJson(String json) {
		//dg1415338444009List = (List<MESBasicConfig>) routineService.generateObjectFromJson(json, MESBasicConfig.class);
	}
	
	private List<Long> dg1415338444009DeletedIds;
	public List<Long> getDg1415338444009DeletedIds(){
		if(null == dg1415338444009DeletedIds){
			dg1415338444009DeletedIds = new ArrayList<Long>();
		}
		return dg1415338444009DeletedIds;
	}

	private Page<MESBasicSwitcherModel> dg1460684019181Page;
	public Page<MESBasicSwitcherModel> getDg1460684019181Page(){
		if(null == dg1460684019181Page){
			dg1460684019181Page = new Page<MESBasicSwitcherModel>(1,20);
		}
		return dg1460684019181Page;
	}
	
	private Class dg1460684019181Clazz = MESBasicSwitcherModel.class;
	
	public Class getDg1460684019181Clazz() {
		return dg1460684019181Clazz;
	}
	
	private List<MESBasicSwitcherModel> dg1460684019181List;
	public List<MESBasicSwitcherModel> getDg1460684019181List(){
		if(null == dg1460684019181List){
			dg1460684019181List = new ArrayList<MESBasicSwitcherModel>();
		}
		return dg1460684019181List;
	}
	public void setDg1460684019181ListJson(String json) {
		//dg1460684019181List = (List<MESBasicSwitcherModel>) routineService.generateObjectFromJson(json, MESBasicSwitcherModel.class);
	}
	
	private List<Long> dg1460684019181DeletedIds;
	public List<Long> getDg1460684019181DeletedIds(){
		if(null == dg1460684019181DeletedIds){
			dg1460684019181DeletedIds = new ArrayList<Long>();
		}
		return dg1460684019181DeletedIds;
	}

	private Page<MESBasicConfig> dg1460684019272Page;
	public Page<MESBasicConfig> getDg1460684019272Page(){
		if(null == dg1460684019272Page){
			dg1460684019272Page = new Page<MESBasicConfig>(1,20);
		}
		return dg1460684019272Page;
	}
	
	private Class dg1460684019272Clazz = MESBasicConfig.class;
	
	public Class getDg1460684019272Clazz() {
		return dg1460684019272Clazz;
	}
	
	private List<MESBasicConfig> dg1460684019272List;
	public List<MESBasicConfig> getDg1460684019272List(){
		if(null == dg1460684019272List){
			dg1460684019272List = new ArrayList<MESBasicConfig>();
		}
		return dg1460684019272List;
	}
	public void setDg1460684019272ListJson(String json) {
		//dg1460684019272List = (List<MESBasicConfig>) routineService.generateObjectFromJson(json, MESBasicConfig.class);
	}
	
	private List<Long> dg1460684019272DeletedIds;
	public List<Long> getDg1460684019272DeletedIds(){
		if(null == dg1460684019272DeletedIds){
			dg1460684019272DeletedIds = new ArrayList<Long>();
		}
		return dg1460684019272DeletedIds;
	}

	private Page<MESBasicSwitcherModel> dg1460705667595Page;
	public Page<MESBasicSwitcherModel> getDg1460705667595Page(){
		if(null == dg1460705667595Page){
			dg1460705667595Page = new Page<MESBasicSwitcherModel>(1,20);
		}
		return dg1460705667595Page;
	}
	
	private Class dg1460705667595Clazz = MESBasicSwitcherModel.class;
	
	public Class getDg1460705667595Clazz() {
		return dg1460705667595Clazz;
	}
	
	private List<MESBasicSwitcherModel> dg1460705667595List;
	public List<MESBasicSwitcherModel> getDg1460705667595List(){
		if(null == dg1460705667595List){
			dg1460705667595List = new ArrayList<MESBasicSwitcherModel>();
		}
		return dg1460705667595List;
	}
	public void setDg1460705667595ListJson(String json) {
		//dg1460705667595List = (List<MESBasicSwitcherModel>) routineService.generateObjectFromJson(json, MESBasicSwitcherModel.class);
	}
	
	private List<Long> dg1460705667595DeletedIds;
	public List<Long> getDg1460705667595DeletedIds(){
		if(null == dg1460705667595DeletedIds){
			dg1460705667595DeletedIds = new ArrayList<Long>();
		}
		return dg1460705667595DeletedIds;
	}

	private Page<MESBasicConfig> dg1460705667651Page;
	public Page<MESBasicConfig> getDg1460705667651Page(){
		if(null == dg1460705667651Page){
			dg1460705667651Page = new Page<MESBasicConfig>(1,20);
		}
		return dg1460705667651Page;
	}
	
	private Class dg1460705667651Clazz = MESBasicConfig.class;
	
	public Class getDg1460705667651Clazz() {
		return dg1460705667651Clazz;
	}
	
	private List<MESBasicConfig> dg1460705667651List;
	public List<MESBasicConfig> getDg1460705667651List(){
		if(null == dg1460705667651List){
			dg1460705667651List = new ArrayList<MESBasicConfig>();
		}
		return dg1460705667651List;
	}
	public void setDg1460705667651ListJson(String json) {
		//dg1460705667651List = (List<MESBasicConfig>) routineService.generateObjectFromJson(json, MESBasicConfig.class);
	}
	
	private List<Long> dg1460705667651DeletedIds;
	public List<Long> getDg1460705667651DeletedIds(){
		if(null == dg1460705667651DeletedIds){
			dg1460705667651DeletedIds = new ArrayList<Long>();
		}
		return dg1460705667651DeletedIds;
	}

	@Actions({
		
		@Action(value = "/MESBasic/routine/routine/data-dg1415338444009", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.measurement.id,result.measurement.value,result.formula,result.testFormula,result.meter.itemName,result.meter.code,result.meter.itemNumber,result.maxAggValue,result.forSource,result.forDestination,result.isDefault,result.id,result.version,result.routine.id,result.switcher.id,result.pump.id,result.meter.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1415338444009-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1415338444009.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/routine/routine/data-dg1460684019181", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.stype.id,result.stype.value,result.switcher.code,result.switcher.itemName,result.switcher.tagType.id,result.switcher.tagType.value,result.switcher.itemNumber,result.switcher.itemClass.id,result.switcher.itemClass.value,result.forword,result.backword,result.id,result.version,result.switcher.id,result.config.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1460684019181-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1460684019181.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/routine/routine/data-dg1460684019272", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.measurement.id,result.measurement.value,result.formula,result.meter.code,result.meter.itemName,result.meter.tagType.id,result.meter.tagType.value,result.meter.itemNumber,result.meter.itemClass.id,result.meter.itemClass.value,result.maxAggValue,result.forSource,result.forDestination,result.isDefault,result.tankCode,result.id,result.version,result.routine.id,result.switcher.id,result.pump.id,result.meter.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1460684019272-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1460684019272.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/routine/routine/data-dg1460705667595", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.stype.id,result.stype.value,result.switcher.code,result.switcher.itemNumber,result.switcher.itemName,result.forword,result.backword,result.id,result.version,result.switcher.id,result.config.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1460705667595-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1460705667595.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/routine/routine/data-dg1460705667651", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.measurement.id,result.measurement.value,result.formula,result.meter.itemName,result.meter.code,result.meter.itemNumber,result.maxAggValue,result.forSource,result.forDestination,result.isDefault,result.id,result.version,result.routine.id,result.switcher.id,result.pump.id,result.meter.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1460705667651-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1460705667651.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(routine.getId() != null && routine.getId() != -1){
				routine = routineService.getRoutine(routine.getId());
			}
			routineService.findDataGridPage(dg,dgclass,getDgPage(),routine,searchCondition,params);
	...
	private void save() throws Exception {
		...
		routineService.saveRoutine(routine,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/routine/routine/routineEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/routine/routine/routineEditForTank/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/routine/routine/routineForTankView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/routine/routine/routineView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/routine/routine/routineViewAll/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "submit");
		bap_validate_datagrids=routineService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", routine.getId());
		...
		fileuploads.put("linkId", routine.getId());
		bapAfterCustomMethod("MESBasic_1_routine_Routine", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/routine/routine/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		routineService.deleteRoutine(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_routine_Routine", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/routine/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "restore");
		routineService.restoreRoutine(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_routine_Routine", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/routine/routine/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/routine/routine/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "showDealInfoData");
		if (null != tableInfoId) {
			int count=routineService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = routineService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = routineService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_routine_Routine", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicRoutine> findExportDataInfos(Page<MESBasicRoutine> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = routineService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = routineService.getAssProperty(p.getCode());	
									//String assModelCode = routineService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			routineService.findRoutines(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			routineService.findRoutines(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicRoutine> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicRoutine result : results){
	...
	public boolean isEditNew(){
		//return null == this.routine || null == this.routine.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/routine/routine/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_routine_Routine", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/routine/routine/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/routine/routine/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/routine/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicRoutineService"
						, "com.supcon.orchid.MESBasic.services.MESBasicConfigService"      
						, "com.supcon.orchid.MESBasic.services.MESBasicSwitcherModelService"      
						, "com.supcon.orchid.MESBasic.services.MESBasicConfigService"
						, "com.supcon.orchid.MESBasic.services.MESBasicItemSetupService"
						, "com.supcon.orchid.MESBasic.services.MESBasicSwitcherModelService"
					, "com.supcon.orchid.MESBasic.services.MESBasicConfigService"
					, "com.supcon.orchid.MESBasic.services.MESBasicConfigService"
					, "com.supcon.orchid.MESBasic.services.MESBasicInvolveItemsetupsService"
					, "com.supcon.orchid.MESBasic.services.MESBasicInvolveItemsetupsService"
					, "com.supcon.orchid.MESBasic.services.MESBasicInvolveItemsetupsService"
					, "com.supcon.orchid.MESBasic.services.MESBasicSwitcherModelService"
					, "com.supcon.orchid.MESBasic.services.MESBasicSwitcherModelService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_routine", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_routine");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_routine_Routine", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/routine/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicRoutineService"
						, "com.supcon.orchid.MESBasic.services.MESBasicContainerService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_routine_Routine", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_routine_Routine", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/routine/routine/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/routine/routine/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_routine_Routine";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_routine_Routine", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/routine/routine/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == routine.getCode() ? "" : routine.getCode().toString());
		_auditLog.setColumnStringA(null == routine.getName() ? "" : routine.getName().toString());
		_auditLog.setColumnStringC(routine.getId().toString());
		_auditLog.setColumnStringD(MESBasicRoutine.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_routine");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicRoutineAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, routine,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, routine,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,routine);
		...
		printlog.setEntity("MESBasic_1_routine");
		...
		printlog.setBusinessId(routine.getId());
		printlog.setBussinessKey(null == routine.getCode() ? "" : routine.getCode().toString());
		printlog.setMainDisplay(null == routine.getName() ? "" : routine.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/routine/routine/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(routine,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/routine/routine/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(routineService.getRoutine(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/routine/routine/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/routine/routine/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, routine,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				routine = routineService.getRoutine(Long.parseLong(arrId[i]));
				if(null != routine){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == routine.getName() ? "" : "," + routine.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == routine.getName() ? "" : routine.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == routine.getCode() ? "" : "," + routine.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == routine.getCode() ? "" : routine.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == routine.getId() ? "" : "," + routine.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == routine.getId() ? "" : routine.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, routine,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,routine);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/routine/routine/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicRoutine> findExportPageInfos(Page<MESBasicRoutine> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_routine" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = routineService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicRoutine queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = routineService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = routineService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_routine_Routine");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_routine_Routine")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_routine_Routine");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_routine_Routine");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = routineService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
		if("MESBasic_1_routine_Routine".equals(code) && "submit".equals(methodName)){
          
            // 获取pt的数据 - added by ww 2019-03-14
            if (getDgLists().containsKey("dg1415338444009")){
                String dg1415338444009Json = getDgLists().get("dg1415338444009");
                dg1415338444009List = (List<MESBasicConfig>) configService.generateObjectFromJson(dg1415338444009Json, MESBasicConfig.class);
            }
            if (getDgLists().containsKey("dg1460684019181")){
                String dg1460684019181Json = getDgLists().get("dg1460684019181");
                dg1460684019181List = (List<MESBasicSwitcherModel>) switcherModelService.generateObjectFromJson(dg1460684019181Json, MESBasicSwitcherModel.class);
            }
          
			StringBuilder errorMsgAuto = new StringBuilder();
			if (dg1415338444009List != null && dg1415338444009List.size() > 0) {
				for (int i = 0; i < dg1415338444009List.size(); i++) {
					if(dg1415338444009List.get(i).getMeasurement().getId().equals("measurement/001")){
						continue;
					}
					String formulaAuto = dg1415338444009List.get(i).getFormula();
					Set<String> codes = null;
					if (null != formulaAuto && formulaAuto.length() > 0) {
						codes = this.getCodes(formulaAuto);
						for (String s : codes) {
							MESBasicItemSetup itemSetup = itemSetupDao
									.findEntityByCriteria(
											Restrictions.eq("code", s),
											Restrictions.eq("valid", true));
							if (itemSetup == null) {
								if (errorMsgAuto.length() == 0) {
									errorMsgAuto.append("计算公式中的编码为[" + s
											+ "]的位号不存在");
								} else {
									errorMsgAuto.append("</br>计算公式中的编码为[" + s
											+ "]的位号不存在");
								}
								if (errorMsgAuto.length() > 0) {
									throw new BAPException(
											errorMsgAuto.toString());
								}
								break;
							}
							formulaAuto = formulaAuto.replaceAll("\\[" + s
									+ "\\]", "1");

						}
					}
					formulaAuto = formulaAuto.replaceAll("N", "1");
					try {
						Calculator.scalculateJS(formulaAuto);
					} catch (Exception ex) {
						if (errorMsgAuto.length() > 0) {
							errorMsgAuto.append("</br>");
						}
						errorMsgAuto.append("第"+(i+1)+"行计算公式中存在错误，请检查。");
					}
					if (errorMsgAuto.length() > 0) {
						throw new BAPException(errorMsgAuto.toString());
					}
			 ...
			 * 共工作路径的判断约束逻辑
			 */
			if(routine.getRoutineStrategy().getId().equals("ROUTINE_STRATEGY/0")){ //工作在单项模式
				List<MESBasicSwitcherModel> modelList = getDg1460684019181List();//泵和阀门的PT
				boolean findForwardPump = false;
				boolean findBackwardPump = false;
				boolean findSwither = false;
				for(MESBasicSwitcherModel model : modelList){
					if(model.getStype() == null){
						throw new BAPException("类型不能为空");
					}
					if(model.getSwitcher() == null){
						throw new BAPException("测点编码不能为空");
					}
					if(model.getStype().getId().equals("MES_SWITHER/1")){//泵都是单向的，不允许设置双向泵
						if(model.getBackword() && model.getForword()){
							throw new BAPException("泵只能工作在一个方向上，不能同时为正向和反向。");
						}else if(model.getBackword() && !model.getForword()){
							findBackwardPump = true;
						}else if(model.getForword() && !model.getBackword()){
							findForwardPump = true;
						}
					}else if(model.getStype().getId().equals("MES_SWITHER/0")){//开关没有方向，如果设置项是开关，则必然同时勾选正向和反向
						findSwither = true;
						model.setBackword(true);
						model.setForword(true);
					}
				}
				/**
				 * 路径上添加批次管理业务后，添加“指令”，路径不再需要一定有“阀门”
				 */
//				if(!findSwither){
//					throw new BAPException("请设置在单向模式下工作的路径的阀门。");
//				}
				
			}else if(routine.getRoutineStrategy().getId().equals("ROUTINE_STRATEGY/1")){//工作在双向模式
				...
				if(!findBackwardPump || !findForwardPump){
					throw new BAPException("工作在双向模式下的路径，请同时设置正向泵和反向泵；\n如果只工作在一个方向上，请选择路径工作模式为单项模式。");
				}
				if(!findSwither){
					throw new BAPException("请设置在双向模式下工作的路径的阀门。");
				}
			}
		}
		 ...
		 * 计量表设置的约束逻辑
		 */
		boolean findSourceDefault = false;
		boolean finddesDefault = false;
		List<MESBasicConfig> configList = getDg1415338444009List();//计量表定义的PT
		for(MESBasicConfig config : configList){
			if(config.getMeter() != null){
				//throw new BAPException("计量表不能为空！");
				if(StringUtil.isEmptyOrNull(config.getName())){
					config.setName(config.getMeter().getItemName());
				}
			}
			if(config.getForDestination() && config.getIsDefault()){
				if(!finddesDefault){
					finddesDefault = true;
				}else{
					throw new BAPException("只能有一只计量表作为“用于目的”的默认计量");
				}
			}
			if(config.getForSource() && config.getIsDefault()){
				if(!findSourceDefault){
					findSourceDefault = true;
				}else {
					throw new BAPException("只能有一只计量表作为“用于源”的默认计量");
				}
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码
       if("MESBasic_1_routine_Routine".equals(code) && "submit".equals(methodName)){
			if(routine.getSwitcher()!=null){
	   			MESBasicSwitcherModel sm = new MESBasicSwitcherModel();
	   			sm.setSwitcher(routine.getSwitcher());
	   			sm.setConfig(routine);
	           	sm.setStype(new SystemCode("MES_SWITHER/2"));
	   			routineService.saveSwitch(sm);
    	   }
		}
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_routine_Routine,MESBasic_1) */
// 自定义代码

	@Autowired
	private MESBasicItemSetupDao itemSetupDao;
	//计算公式检测用
	private Set<String> getCodes(String formula) {
	...
	@Action(value = "/public/MESBasic/routine/syncRoutineStatus")
	public void updateRoutineStatus(){
		routineService.updateRoutineStatus();
	}
	...
	@Autowired
	MESBasicContainerService containerService;
	
	@Autowired
	MESBasicEquipmentService equipmentService;
	
	@Autowired
	MESBasicObjEquipmentService objEquipmentService;

	@Action(value = "/public/MESBasic/routine/getOtherEquip", results = { @Result(name = SUCCESS, type = JSON, params = {
			"root", "responseMap", "excludes", "*", "includes", "*" }) })
	public String getOtherEquip() {
		MESBasicRoutine routine = routineService.getRoutine(routineId);
		MESBasicObjEquipment returnContainer = null;
		if (key != null && key.equals("from")) {// 源容器
			returnContainer = routine.getSrouceEqpObj();
		} else if(key != null && key.equals("to")) {// 目标容器
			returnContainer = routine.getDestEquObj();
		}
		if(returnContainer != null){
			returnContainer = objEquipmentService.getObjEquipment(returnContainer.getId());
			responseMap.put("equipmentId", returnContainer.getId());
			responseMap.put("equipmentName", returnContainer.getEquipmentNum());
		}
		return SUCCESS;
	}
	...
	@Action(value = "/public/MESBasic/routine/checkItem",  results = { @Result(name = SUCCESS, type = JSON, params = {
			"root", "responseMap", "excludes", "*", "includes", "*" }) })
	public String checkItem(){
		responseMap.put("result",routineService.checkItem(itemNumber));
		return SUCCESS;
	}

    /**
     * 路径编辑页面参照容器时获取当前容器中配置的所有测点信息
     *
     * @return
     * @throws Exception
     */
    @Action(value = "/public/MESBasic/routine/getTagsInfo", results = {@Result(name = SUCCESS, type = JSON, params = {
            "root", "responseMap", "excludes", "*", "includes", "tagsInfo.id,tagsInfo.code,tagsInfo.itemName,tagsInfo.itemNumber,tagsInfo.itemClass,tagsInfo.tagType,tagsInfo.primary"})})
    public String getTagsInfo() throws Exception {
        String containerCode = request.getParameter("containerCode");
        List<Map<String, Object>> tagsInfo = routineService.getTagsInfo(containerCode);
        responseMap.put("tagsInfo", tagsInfo);
        return SUCCESS;
    }
     ...
     * @throws Exception
     */
    @Action(value = "/public/MESBasic/routine/getRoutineHeadId", results = {@Result(name = SUCCESS, type = JSON, params = {
            "root", "responseMap"})})
    public String getRoutineHeadId() throws Exception {
        String routineCode = request.getParameter("routineCode");
        if (org.apache.commons.lang.StringUtils.isNotBlank(routineCode)){
            Long headId = routineService.getRoutineHeadId(routineCode);
            responseMap.put("headId", headId);
        }
        return SUCCESS;
    }
/* CUSTOM CODE END */
