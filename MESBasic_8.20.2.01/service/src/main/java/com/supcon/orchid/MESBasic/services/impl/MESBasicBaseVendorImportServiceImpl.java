...
@Service("mESBasicBaseVendorImportService")
public class MESBasicBaseVendorImportServiceImpl implements MESBasicBaseVendorImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_baseVendor",model="MESBasic_1_baseVendor_BaseVendor",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicBaseVendor bizObj, MESBasicBaseVendor oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1449536347672") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getVendorName() ? "" : "," + oldBizObj.getVendorName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getVendorName() ? "" : oldBizObj.getVendorName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getVendorCode() ? "" : "," + oldBizObj.getVendorCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getVendorCode() ? "" : oldBizObj.getVendorCode().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1449536347672") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getVendorName() ? "" : "," + bizObj.getVendorName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getVendorName() ? "" : bizObj.getVendorName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getVendorCode() ? "" : "," + bizObj.getVendorCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getVendorCode() ? "" : bizObj.getVendorCode().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicBaseVendor bizObj, String bussinesskeyFieldName,
