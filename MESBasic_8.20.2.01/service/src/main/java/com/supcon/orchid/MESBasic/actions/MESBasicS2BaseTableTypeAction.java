...
/* CUSTOM CODE START(action,import,MESBasic_1_tableType_S2BaseTableType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicS2BaseTableTypeAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicS2BaseTableType> {
	...
	private MESBasicS2BaseTableType s2BaseTableType;
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/tableType/s2BaseTableType/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/tableType/s2BaseTableType/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_tableType_S2BaseTableType" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicS2BaseTableType> s2BaseTableTypes;
	
	public List<MESBasicS2BaseTableType> getS2BaseTableTypes() {
		return s2BaseTableTypes;
	}
	...
	private Page<MESBasicS2BaseTableType> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			s2BaseTableType = new MESBasicS2BaseTableType();
		} else {
			if(id > 0){
				s2BaseTableType = s2BaseTableTypeService.getS2BaseTableType(id, viewCode);
			}
		}
	...
	public Page<MESBasicS2BaseTableType> getPage() {
		if (null == page) {
			page = new Page<MESBasicS2BaseTableType>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicS2BaseTableType getS2BaseTableType() {
		return s2BaseTableType;
	}		

	public void setS2BaseTableType(MESBasicS2BaseTableType s2BaseTableType) {
		this.s2BaseTableType = s2BaseTableType;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tableType/s2BaseTableType/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = s2BaseTableTypeService.getS2BaseTableTypeAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/tableType/s2BaseTableType/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String s2BaseTableTypeReferenceCopy() throws Exception{
			MESBasicS2BaseTableType oldS2BaseTableType = null;
			if(isEditNew()){
				...
				oldS2BaseTableType = new MESBasicS2BaseTableType();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,s2BaseTableType.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, s2BaseTableType.getId());
				...
				oldS2BaseTableType = s2BaseTableTypeService.getS2BaseTableType(id);
			}
			
			
			s2BaseTableType = s2BaseTableTypeService.getS2BaseTableType(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_tableType_S2BaseTableType");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicS2BaseTableType.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldS2BaseTableType, null);
						Method setValue = MESBasicS2BaseTableType.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(s2BaseTableType, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			s2BaseTableType.setCreateDepartmentId(oldS2BaseTableType.getCreateDepartmentId());
			s2BaseTableType.setCreatePositionId(oldS2BaseTableType.getCreatePositionId());
			s2BaseTableType.setCreateStaffId(oldS2BaseTableType.getCreateStaffId());
			s2BaseTableType.setOwnerDepartmentId(oldS2BaseTableType.getOwnerDepartmentId());
			s2BaseTableType.setOwnerPositionId(oldS2BaseTableType.getOwnerPositionId());
			s2BaseTableType.setOwnerStaffId(oldS2BaseTableType.getOwnerStaffId());
			bapAfterCustomMethod("MESBasic_1_tableType_S2BaseTableType", "referenceCopy");
			return SUCCESS;
		}
	...
	private void save(DataGridService dataGridService) throws Exception {
		if(null == s2BaseTableType.getId()) {
			responseMap.put("operate", "add");
		} else {
			responseMap.put("operate", "edit");
		}
		s2BaseTableTypeService.saveS2BaseTableType(s2BaseTableType, dataGridService, viewCode);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tableType_S2BaseTableType", "submit");
		DataGridService dataGridService = null;
		
			save(dataGridService);
			responseMap.put("id", s2BaseTableType.getId());
		...
		fileuploads.put("linkId", s2BaseTableType.getId());
		bapAfterCustomMethod("MESBasic_1_tableType_S2BaseTableType", "submit");
		return SUCCESS;
	}
	@Action(value = "/MESBasic/tableType/s2BaseTableType/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tableType_S2BaseTableType", "delete");
		s2BaseTableTypeService.deleteS2BaseTableType(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_tableType_S2BaseTableType", "delete");
		return SUCCESS;
	}
	
	
	@Actions({
			@Action(value = "/MESBasic/tableType/s2BaseTableType/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/tableType/s2BaseTableType/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tableType_S2BaseTableType", "showDealInfoData");
		if (null != tableInfoId) {
			int count=s2BaseTableTypeService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = s2BaseTableTypeService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = s2BaseTableTypeService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_tableType_S2BaseTableType", "showDealInfoData");
		return SUCCESS;
	}	

	@Override
	public Page<MESBasicS2BaseTableType> findExportDataInfos(Page<MESBasicS2BaseTableType> page) {
		if(null != searchObjects && searchObjects.length > 0) {
			s2BaseTableTypeService.findS2BaseTableTypes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, searchObjects);
		} else {
			s2BaseTableTypeService.findS2BaseTableTypes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode);
		}
		return getPage();
	}

	public boolean isEditNew(){
		//return null == this.s2BaseTableType || null == this.s2BaseTableType.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/tableType/s2BaseTableType/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tableType_S2BaseTableType", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_tableType_S2BaseTableType", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/tableType/s2BaseTableType/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_tableType_S2BaseTableType", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/tableType/s2BaseTableType/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tableType/s2BaseTableType/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicS2BaseTableTypeService"
		};
		bapBeforeCustomMethod("MESBasic_1_tableType_S2BaseTableType", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_tableType_S2BaseTableType", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tableType/s2BaseTableType/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				xlsResult = importService.imports(viewCode, "MESBasic_1_tableType_S2BaseTableType", dataFile, time, name[0], isReplace, isIgnore, importServices);
	...
	@Action(value = "/MESBasic/tableType/s2BaseTableType/downLoad", results = { @Result(type = "stream", params = {
  ...
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tableType_S2BaseTableType", "downLoadXls");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/tableType/s2BaseTableType/templateDownLoad", results = { @Result(type = "stream", params = {
			"contentType", "application/excel", "inputName", "inputStream",
			"contentDisposition", 
  "attachment;filename=\"${fileName}\"", "bufferSize", "4096" }) })
	public String templateDownLoad() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_tableType_S2BaseTableType", "templateDownLoad");
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/tableType/s2BaseTableType/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		MESBasicS2BaseTableType s2BaseTableType = null;
		if(null != id) {
			s2BaseTableType = s2BaseTableTypeService.getS2BaseTableType(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, s2BaseTableType, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, s2BaseTableType, null, null, true, this);
	...
	@Action(value = "/MESBasic/tableType/s2BaseTableType/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/tableType/s2BaseTableType/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				s2BaseTableType = s2BaseTableTypeService.getS2BaseTableType(Integer.parseInt(id));
				if(s2BaseTableType != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, s2BaseTableType, null, null, true, this);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_tableType_S2BaseTableType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_tableType_S2BaseTableType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_tableType_S2BaseTableType,MESBasic_1) */
// 自定义代码
