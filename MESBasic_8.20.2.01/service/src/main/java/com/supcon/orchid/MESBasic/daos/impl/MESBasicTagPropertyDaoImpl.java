...
@Repository
public class MESBasicTagPropertyDaoImpl extends ExtendedGenericDaoImpl<MESBasicTagProperty, Long> implements MESBasicTagPropertyDao {
	@Override
	...
	@Override
	public MESBasicTagPropertyDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicTagPropertyDealInfo dealInfo = new MESBasicTagPropertyDealInfo();
	...
	@Override
	public MESBasicTagPropertyDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicTagProperty mainObj, Staff dealStaff) {
		MESBasicTagPropertyDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicTagPropertyDealInfo.class);
		...
		List<MESBasicTagPropertyDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	 /* CUSTOM CODE START(daoimpl,functions,MESBasic_1_tagProperty_TagProperty,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
}