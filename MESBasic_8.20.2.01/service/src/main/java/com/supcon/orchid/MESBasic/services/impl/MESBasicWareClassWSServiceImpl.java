...
import com.supcon.orchid.MESBasic.services.MESBasicWareClassWSService;
import com.supcon.orchid.MESBasic.services.MESBasicWareClassService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_wareClass_WareClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicWareClass",targetNamespace = "http://ws.supcon.com")
public class MESBasicWareClassWSServiceImpl implements MESBasicWareClassWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getWareClassByBusinessKey(
		...
		try{
			...
			MESBasicWareClass wareClass = wareClassService.loadWareClassByBussinessKey(businessKey);
			if(wareClass == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, wareClass);
		}catch(Throwable t){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getWareClassByPk(
		...
		try{
			...
			if(wareClass == null || !wareClass.isValid()){
				return set404Error(result);
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO updateWareClass(
		@WebParam(name="wareClass")MESBasicWareClass wareClass,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
			
		JWSResultDTO result = new JWSResultDTO();
		try{
			if(wareClass == null){
				return set500Error(result, "仓库分类不能为空。");
			}
			...
			if(!beforeUpdateWareClass(wareClass)){
				return set404Error(result);
			}
			wareClassService.saveWareClass(wareClass, null);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO addWareClass(
		@WebParam(name="wareClass")MESBasicWareClass wareClass,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			if(wareClass.getId() != null){
				return set500Error(result, "新增方法不能设置id。");
			}
			if(wareClass.getCode() == null){
				return set500Error(result, "业务主键不能为空。");
			}
			...
			try{
				checkWareClassData(wareClass);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			wareClassService.saveWareClass(wareClass, null);
			setSuccessResult(result, wareClass.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteWareClassByBusinessKey(
		@WebParam(name="businessKey")String businessKey,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				wareClass = wareClassService.loadWareClassByBussinessKey(businessKey);
	...
	@Override//
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteWareClassByPk(
		@WebParam(name="pk")String pk,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddWareClass(
		@WebParam(name="wareClassList")List<MESBasicWareClass> wareClassList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			int count = 1;
			for(MESBasicWareClass wareClass : wareClassList){
				if(wareClass.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(wareClass.getCode() == null || wareClass.getCode().toString().trim().isEmpty()){
			...
			wareClassService.batchImportBaseWareClass(wareClassList);
			for(MESBasicWareClass wareClass : wareClassList){
				idList.add(wareClass.getId());
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdateWareClass(
		@WebParam(name="wareClassList")List<MESBasicWareClass> wareClassList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteWareClassByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicWareClass temp = null;
				try{
					temp = wareClassService.getWareClass(id);
				}catch(NullPointerException e){}
			...
			wareClassService.deleteWareClass(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteWareClassByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicWareClass temp = null;
				try{
					temp = wareClassService.loadWareClassByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getWareClassPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicWareClass.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicWareClass.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicWareClass.class);
				} catch (Exception e) {
			...
			Page<MESBasicWareClass> page = new Page<MESBasicWareClass>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicWareClass> datas = wareClassService.getByPage(page, detachedCriteria);
			List<MESBasicWareClass> wareClassList = datas.getResult();
			if (wareClassList == null || wareClassList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(wareClassList.toArray());
			PaginationDTO<MESBasicWareClass> pagination = new PaginationDTO<MESBasicWareClass>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicWareClass wareClass = wareClassService.getWareClass(id);
			if (wareClass == null || !wareClass.isValid()) {
				result.setErrorMessage("找不到仓库分类");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取仓库分类对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param wareClass
	 * @return
	 */
	private MESBasicWareClass getWareClass(MESBasicWareClass wareClass) {
		if (wareClass.getId() == null && (wareClass.getCode() == null || wareClass.getCode().toString().trim() == ""))
			throw new RuntimeException("仓库分类对象业务主键和id不能同时为空");
		if (wareClass.getId() != null) {
			MESBasicWareClass result = wareClassService.getWareClass(wareClass .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return wareClassService.loadWareClassByBussinessKey(wareClass.getCode());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个仓库分类，则返回false，否则返回true
	 * @param wareClass
	 */
	private boolean beforeUpdateWareClass(MESBasicWareClass wareClass) {
		if(wareClass.getName() == null){
			throw new RuntimeException("name不能为空。");
		}
		MESBasicWareClass temp = getWareClass(wareClass);
		if (temp == null) {
			return false;
		}
		if(wareClass.getId() == null){
			wareClass.setId(temp.getId());		
		}
		if(wareClass.getCode() == null){
			wareClass.setCode(temp.getCode());
		}
		
		wareClass.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除仓库分类对象下所有辅模型对象中的仓库分类对象
	 * @param wareClass
	 */
	private void clearCircle(MESBasicWareClass wareClass) {
	 ...
	 */
	private void checkWareClassData(MESBasicWareClass wareClass){
		 if(wareClass.getName() == null){
		 	throw new RuntimeException("name不能为空。");
		 }
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_wareClass_WareClass,MESBasic_1) */
// 自定义代码
