...
@Service("mESBasicPurchGroupImportService")
public class MESBasicPurchGroupImportServiceImpl implements MESBasicPurchGroupImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicPurchGroup bizObj, MESBasicPurchGroup oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1470291704097") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getPurchGroupName() ? "" : "," + oldBizObj.getPurchGroupName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getPurchGroupName() ? "" : oldBizObj.getPurchGroupName().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1470291704097") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getPurchGroupName() ? "" : "," + bizObj.getPurchGroupName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getPurchGroupName() ? "" : bizObj.getPurchGroupName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicPurchGroup bizObj, String bussinesskeyFieldName,
