...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicPositionDealAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicPositionDeal> {
	...
	private MESBasicPositionDeal positionDeal;
	...
	//导出excel时,主模型内容
	List<MESBasicPositionDeal> queryResults = new ArrayList<MESBasicPositionDeal>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/positionDeal/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/positionDeal/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryModel_PositionDeal" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicPositionDeal> positionDeals;
	
	public List<MESBasicPositionDeal> getPositionDeals() {
		return positionDeals;
	}
	...
	private Page<MESBasicPositionDeal> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			positionDeal = new MESBasicPositionDeal();
		} else {
			if(id > 0){
				positionDeal = positionDealService.getPositionDeal(id, viewCode);
			}
		}
	...
	public Page<MESBasicPositionDeal> getPage() {
		if (null == page) {
			page = new Page<MESBasicPositionDeal>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicPositionDeal getPositionDeal() {
		return positionDeal;
	}		

	public void setPositionDeal(MESBasicPositionDeal positionDeal) {
		this.positionDeal = positionDeal;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/positionDeal/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = positionDealService.getPositionDealAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/positionDeal/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1460950790364
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String positionDealReferenceCopy() throws Exception{
			MESBasicPositionDeal oldPositionDeal = null;
			if(isEditNew()){
				...
				oldPositionDeal = new MESBasicPositionDeal();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,positionDeal.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, positionDeal.getId());
				...
				oldPositionDeal = positionDealService.getPositionDeal(id);
			}
			
			
			positionDeal = positionDealService.getPositionDeal(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_PositionDeal");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicPositionDeal.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPositionDeal, null);
						Method setValue = MESBasicPositionDeal.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(positionDeal, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			positionDeal.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryModel_PositionDeal", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=positionDealService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return positionDealReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			positionDeal = positionDealService.getPositionDeal(this.positionDeal.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,positionDeal.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = positionDealService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(positionDealService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(positionDealService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryModel_PositionDeal", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"positionDeal\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"positionDeal\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"positionDeal\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"positionDeal\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			positionDealService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		positionDealService.findDataGridPage(dg,dgclass,getDgPage(),positionDeal,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(positionDeal.getId() != null && positionDeal.getId() != -1){
				positionDeal = positionDealService.getPositionDeal(positionDeal.getId());
			}
			positionDealService.findDataGridPage(dg,dgclass,getDgPage(),positionDeal,searchCondition,params);
	...
	private void save() throws Exception {
		...
		positionDealService.savePositionDeal(positionDeal,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_PositionDeal", "submit");
		bap_validate_datagrids=positionDealService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", positionDeal.getId());
		...
		fileuploads.put("linkId", positionDeal.getId());
		bapAfterCustomMethod("MESBasic_1_factoryModel_PositionDeal", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryModel/positionDeal/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_PositionDeal", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		positionDealService.deletePositionDeal(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryModel_PositionDeal", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/positionDeal/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_PositionDeal", "restore");
		positionDealService.restorePositionDeal(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryModel_PositionDeal", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicPositionDeal> findExportDataInfos(Page<MESBasicPositionDeal> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = positionDealService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = positionDealService.getAssProperty(p.getCode());	
									//String assModelCode = positionDealService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			positionDealService.findPositionDeals(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			positionDealService.findPositionDeals(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicPositionDeal> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicPositionDeal result : results){
	...
	public boolean isEditNew(){
		//return null == this.positionDeal || null == this.positionDeal.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryModel/positionDeal/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_PositionDeal", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_PositionDeal", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryModel/positionDeal/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_PositionDeal", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryModel/positionDeal/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/positionDeal/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicPositionDealService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_factoryModel_PositionDeal", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_PositionDeal", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/positionDeal/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryModel_PositionDeal", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryModel_PositionDeal", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryModel/positionDeal/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_PositionDeal", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryModel/positionDeal/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryModel_PositionDeal";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryModel_PositionDeal", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryModel/positionDeal/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
		_auditLog.setColumnStringA(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
		_auditLog.setColumnStringC(positionDeal.getId().toString());
		_auditLog.setColumnStringD(MESBasicPositionDeal.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicPositionDealAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, positionDeal,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, positionDeal,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,positionDeal);
		...
		printlog.setBusinessId(positionDeal.getId());
		printlog.setBussinessKey(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
		printlog.setMainDisplay(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryModel/positionDeal/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(positionDeal,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/positionDeal/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(positionDealService.getPositionDeal(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryModel/positionDeal/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryModel/positionDeal/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, positionDeal,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				positionDeal = positionDealService.getPositionDeal(Long.parseLong(arrId[i]));
				if(null != positionDeal){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == positionDeal.getId() ? "" : "," + positionDeal.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == positionDeal.getId() ? "" : "," + positionDeal.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == positionDeal.getId() ? "" : "," + positionDeal.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == positionDeal.getId() ? "" : positionDeal.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, positionDeal,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,positionDeal);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/factoryModel/positionDeal/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_PositionDeal");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryModel_PositionDeal")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryModel_PositionDeal");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_PositionDeal");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = positionDealService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryModel_PositionDeal,MESBasic_1) */
// 自定义代码
