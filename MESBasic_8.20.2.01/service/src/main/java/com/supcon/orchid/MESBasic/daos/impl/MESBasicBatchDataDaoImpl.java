...
@Repository
public class MESBasicBatchDataDaoImpl extends ExtendedGenericDaoImpl<MESBasicBatchData, Long> implements MESBasicBatchDataDao {
	@Override
	...
	@Override
	public MESBasicBatchDataDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicBatchDataDealInfo dealInfo = new MESBasicBatchDataDealInfo();
	...
	@Override
	public MESBasicBatchDataDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicBatchData mainObj, Staff dealStaff) {
		MESBasicBatchDataDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicBatchDataDealInfo.class);
		...
		List<MESBasicBatchDataDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_batchDataHandler_BatchData,MESBasic_1) */
// 自定义代码
