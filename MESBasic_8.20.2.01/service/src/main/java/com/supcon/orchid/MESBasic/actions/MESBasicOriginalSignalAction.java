...
/* CUSTOM CODE START(action,import,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
import com.supcon.orchid.MESBasic.services.MESBasicSignalDataTask;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicOriginalSignalAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicOriginalSignal> {
	...
	private MESBasicOriginalSignal originalSignal;
	...
	//导出excel时,主模型内容
	List<MESBasicOriginalSignal> queryResults = new ArrayList<MESBasicOriginalSignal>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/originalSignal/originalSignal/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/originalSignal/originalSignal/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_originalSignal_OriginalSignal" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicOriginalSignal> originalSignals;
	
	public List<MESBasicOriginalSignal> getOriginalSignals() {
		return originalSignals;
	}
	...
	private Page<MESBasicOriginalSignal> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			originalSignal = new MESBasicOriginalSignal();
			if(originalSignal.getSource() != null) {
				originalSignal.setSource((SystemCode) systemCodeService.getSystemCode(originalSignal.getSource().getId()));
			}
		} else {
			if(id > 0){
				originalSignal = originalSignalService.getOriginalSignal(id, viewCode);
			}
		}
	...
	public Page<MESBasicOriginalSignal> getPage() {
		if (null == page) {
			page = new Page<MESBasicOriginalSignal>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicOriginalSignal getOriginalSignal() {
		return originalSignal;
	}		

	public void setOriginalSignal(MESBasicOriginalSignal originalSignal) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/originalSignal/originalSignal/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = originalSignalService.getOriginalSignalAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/originalSignal/originalSignal/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String originalSignalReferenceCopy() throws Exception{
			MESBasicOriginalSignal oldOriginalSignal = null;
			if(isEditNew()){
				...
				oldOriginalSignal = new MESBasicOriginalSignal();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,originalSignal.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, originalSignal.getId());
				...
				oldOriginalSignal = originalSignalService.getOriginalSignal(id);
			}
			
			
			originalSignal = originalSignalService.getOriginalSignal(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_originalSignal_OriginalSignal");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicOriginalSignal.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldOriginalSignal, null);
						Method setValue = MESBasicOriginalSignal.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(originalSignal, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			originalSignal.setCreateDepartmentId(oldOriginalSignal.getCreateDepartmentId());
			originalSignal.setCreatePositionId(oldOriginalSignal.getCreatePositionId());
			originalSignal.setCreateStaffId(oldOriginalSignal.getCreateStaffId());
			originalSignal.setOwnerDepartmentId(oldOriginalSignal.getOwnerDepartmentId());
			originalSignal.setOwnerPositionId(oldOriginalSignal.getOwnerPositionId());
			originalSignal.setOwnerStaffId(oldOriginalSignal.getOwnerStaffId());
			originalSignal.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=originalSignalService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return originalSignalReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			originalSignal = originalSignalService.getOriginalSignal(this.originalSignal.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,originalSignal.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = originalSignalService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(originalSignalService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(originalSignalService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_originalSignal_OriginalSignal", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"originalSignal\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"originalSignal\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"originalSignal\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"originalSignal\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			originalSignalService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		originalSignalService.findDataGridPage(dg,dgclass,getDgPage(),originalSignal,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(originalSignal.getId() != null && originalSignal.getId() != -1){
				originalSignal = originalSignalService.getOriginalSignal(originalSignal.getId());
			}
			originalSignalService.findDataGridPage(dg,dgclass,getDgPage(),originalSignal,searchCondition,params);
	...
	private void save() throws Exception {
		...
		originalSignalService.saveOriginalSignal(originalSignal,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "submit");
		bap_validate_datagrids=originalSignalService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", originalSignal.getId());
		...
		fileuploads.put("linkId", originalSignal.getId());
		bapAfterCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/originalSignal/originalSignal/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		originalSignalService.deleteOriginalSignal(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/originalSignal/originalSignal/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "restore");
		originalSignalService.restoreOriginalSignal(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/originalSignal/originalSignal/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/originalSignal/originalSignal/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "showDealInfoData");
		if (null != tableInfoId) {
			int count=originalSignalService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = originalSignalService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = originalSignalService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicOriginalSignal> findExportDataInfos(Page<MESBasicOriginalSignal> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = originalSignalService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = originalSignalService.getAssProperty(p.getCode());	
									//String assModelCode = originalSignalService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			originalSignalService.findOriginalSignals(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			originalSignalService.findOriginalSignals(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicOriginalSignal> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicOriginalSignal result : results){
	...
	public boolean isEditNew(){
		//return null == this.originalSignal || null == this.originalSignal.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/originalSignal/originalSignal/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/originalSignal/originalSignal/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/originalSignal/originalSignal/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/originalSignal/originalSignal/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicOriginalSignalService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_originalSignal", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_originalSignal");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/originalSignal/originalSignal/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_originalSignal_OriginalSignal", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_originalSignal_OriginalSignal", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/originalSignal/originalSignal/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/originalSignal/originalSignal/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_originalSignal_OriginalSignal";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_originalSignal_OriginalSignal", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/originalSignal/originalSignal/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicOriginalSignal originalSignal = null;
		if(null != id) {
			originalSignal = originalSignalService.getOriginalSignal(id);
		}
		...
		_auditLog.setColumnStringB(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
		_auditLog.setColumnStringA(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
		_auditLog.setColumnStringC(originalSignal.getId().toString());
		_auditLog.setColumnStringD(MESBasicOriginalSignal.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_originalSignal");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicOriginalSignalAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, originalSignal,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, originalSignal,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,originalSignal);
		...
		printlog.setEntity("MESBasic_1_originalSignal");
		...
		printlog.setBusinessId(originalSignal.getId());
		printlog.setBussinessKey(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
		printlog.setMainDisplay(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/originalSignal/originalSignal/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(originalSignal,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/originalSignal/originalSignal/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(originalSignalService.getOriginalSignal(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/originalSignal/originalSignal/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/originalSignal/originalSignal/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, originalSignal,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				originalSignal = originalSignalService.getOriginalSignal(Long.parseLong(arrId[i]));
				if(null != originalSignal){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == originalSignal.getId() ? "" : "," + originalSignal.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == originalSignal.getId() ? "" : "," + originalSignal.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == originalSignal.getId() ? "" : "," + originalSignal.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, originalSignal,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,originalSignal);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/originalSignal/originalSignal/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicOriginalSignal> findExportPageInfos(Page<MESBasicOriginalSignal> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_originalSignal" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = originalSignalService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicOriginalSignal queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = originalSignalService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = originalSignalService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_originalSignal_OriginalSignal");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_originalSignal_OriginalSignal")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_originalSignal_OriginalSignal");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_originalSignal_OriginalSignal");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = originalSignalService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码 MESBasicOriginalSignalAction /MESBasic/originalSignal/originalSignal/delete
	@Autowired
	private MESBasicSignalDataTask signalDataTask;

	/**
	 * 调度任务: 从mes端发起请求获取pims发送来的位号做mark和quanlity字段处理到MESBASIC_ORIGINAL_SIGNALS表；
	 * 从mes端发起请求获取MESBASIC_ORIGINAL_SIGNALS位号插入到MESBASIC_BATCH_DATAS表
	 *
	 * @return
	 * @throws Exception
	 */
	@Action(value = "/public/MESBasic/originalSignal/originalSignal/signalDataTask", results = {@Result(type = JSON, params = {
			"root", "responseMap"})})
	public String originSinalDataTask(){
