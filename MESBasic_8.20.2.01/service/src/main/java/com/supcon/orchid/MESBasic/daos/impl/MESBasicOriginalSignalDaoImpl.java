...
@Repository
public class MESBasicOriginalSignalDaoImpl extends ExtendedGenericDaoImpl<MESBasicOriginalSignal, Long> implements MESBasicOriginalSignalDao {
	@Override
	...
	@Override
	public MESBasicOriginalSignalDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicOriginalSignalDealInfo dealInfo = new MESBasicOriginalSignalDealInfo();
	...
	@Override
	public MESBasicOriginalSignalDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicOriginalSignal mainObj, Staff dealStaff) {
		MESBasicOriginalSignalDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicOriginalSignalDealInfo.class);
		...
		List<MESBasicOriginalSignalDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
