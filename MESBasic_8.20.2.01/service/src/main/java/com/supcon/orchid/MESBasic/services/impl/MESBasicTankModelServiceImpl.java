...
import com.supcon.orchid.MESBasic.entities.MESBasicTankModelMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicTankModelService;
import com.supcon.orchid.MESBasic.services.MESBasicTankModelImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicTankModelDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_tankModelService")
@Transactional
public class MESBasicTankModelServiceImpl  extends BaseServiceImpl<MESBasicTankModel> implements MESBasicTankModelService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicTankModel> bapBaseService;
	...
	@Autowired
	private MESBasicTankModelImportService tankModelImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicTankModel.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  tankModelDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicTankModel.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = tankModelDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicTankModel.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicTankModel.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  tankModelDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicTankModel.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicTankModel.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicTankModel> list = (List<MESBasicTankModel>) tankModelDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicTankModel getTankModel(long id){
		return getTankModel(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicTankModel getTankModel(long id, String viewCode){
		MESBasicTankModel tankModel = tankModelDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getTankModelAsJSON(long id, String include){
		MESBasicTankModel tankModel = tankModelDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(tankModel, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.common.delete",operType="3")
	public void deleteTankModel(MESBasicTankModel tankModel){
		if(null != tankModel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicTankModel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == tankModel.getCode() ? "" : tankModel.getCode().toString());
			AuditUtil.setColumnStringA(null == tankModel.getName() ? "" : tankModel.getName().toString());
			AuditUtil.setColumnStringC(tankModel.getId().toString());
		}
		checkDeletePermit(tankModel.getId());
		beforeDeleteTankModel(tankModel);
		tankModelDao.delete(tankModel);
		afterDeleteTankModel(tankModel);
		tankModelDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/tankModel/del", tankModel,MESBasicTankModelMneCode.class);
		List<MESBasicTankModel> params = new ArrayList<MESBasicTankModel>();
		params.add(tankModel);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_tankModel");
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.common.delete",operType="3")
	public void deleteTankModel(List<Long> tankModelIds){
		deleteTankModel(tankModelIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.common.delete",operType="3")
	public void deleteTankModel(List<Long> tankModelIds, String eventTopic) {
		List<MESBasicTankModel> tankModels = new ArrayList<MESBasicTankModel>();
		for(Long tankModelId : tankModelIds){
			MESBasicTankModel tankModel = getTankModel(tankModelId);
			tankModels.add(tankModel);
			if(null != tankModel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicTankModel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == tankModel.getName() ? "" : "," + tankModel.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == tankModel.getName() ? "" : tankModel.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == tankModel.getCode() ? "" : "," + tankModel.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == tankModel.getCode() ? "" : tankModel.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == tankModel.getId() ? "" : "," + tankModel.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == tankModel.getId() ? "" : tankModel.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(tankModels != null && tankModels.size() > 0 ){
			for(MESBasicTankModel tankModel : tankModels){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),tankModel, tankModel.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(tankModelIds != null && tankModelIds.size() > 0) {
			String hql = "update " + MESBasicTankModel.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = tankModelDao.createQuery(hql);
			query.setParameterList("ids", tankModelIds);
			query.executeUpdate();
			tankModelDao.clearAllCacheByEntity();
		}
		...
		if(eventTopic==null){
			for(MESBasicTankModel tankModelz : tankModels){
				tankModelz.setValid(false);
				List<MESBasicTankModel> params = new ArrayList<MESBasicTankModel>();
				params.add(tankModelz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_tankModel");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.common.delete",operType="3")
	public void deleteTankModel(long tankModelId, int tankModelVersion){
		this.deleteTankModel(tankModelId, tankModelVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.common.delete",operType="3")
	public void deleteTankModel(long tankModelId, int tankModelVersion,SignatureLog signatureLog){
		MESBasicTankModel tankModel = getTankModel(tankModelId);
		if(tankModel != null && !tankModel.isValid()){
		...
		checkDeletePermit(tankModelId);
		beforeDeleteTankModel(tankModel);
		tankModelDao.delete(tankModelId, tankModelVersion);
		afterDeleteTankModel(tankModel);
		tankModelDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/tankModel/del", tankModel,MESBasicTankModelMneCode.class);
		List<MESBasicTankModel> params = new ArrayList<MESBasicTankModel>();
		...
		if(signatureLog != null ) {
			...
			businessKey=tankModel.getCode();
			signatureLog.setTableId(tankModel.getId());
			if(businessKey != null) {
	...
	public void checkDeletePermit(Long id) throws BAPException{
		String tankModelHql = "select count(tankModel.id) as totalCoual from " + MESBasicTankModel.JPA_NAME + " as tankModel where tankModel.parentId=? and tankModel.valid=true";
		List<Long> tankModelCount = tankModelDao.findByHql(tankModelHql, id);
		if (tankModelCount.get(0) > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.common.delete",operType="3")
	public void deleteTankModel(String tankModelIds){
		this.deleteTankModel(tankModelIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.common.delete",operType="3")
	public void deleteTankModel(String tankModelIds,SignatureLog signatureLog){
		deleteCollection(tankModelIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteTankModel(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_tankModel#modelCode:MESBasic_1_tankModel_TankModel#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="ec.engine.restore",operType="10")
	public void restoreTankModel(String tankModelIds){
		restoreCollection(tankModelIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreTankModel(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="ec.engine.restore",operType="10")
	public void restoreTankModel(long tankModelId){
		
		findBusinessKeyUsed(tankModelId);	//判断业务主键是否重复
		...
		checkDeletePermit(tankModelId);
		beforeRestoreTankModel(tankModel);
		tankModel.setValid(true);
		tankModelDao.update(tankModel);
		afterRestoreTankModel(tankModel);
		tankModelDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/tankModel/add", tankModel,MESBasicTankModelMneCode.class);
		List<MESBasicTankModel> params = new ArrayList<MESBasicTankModel>();
		params.add(tankModel);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long tankModelId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_tankModel_TankModel");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicTankModel.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicTankModel.TABLE_NAME +" where id = ? )";
			List<Object> list =  tankModelDao.createNativeQuery(sql,tankModelId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseTankModel(List<MESBasicTankModel>  tankModels){
		for(MESBasicTankModel tankModel:tankModels)  {
			saveTankModel(tankModel, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseTankModel(List<MESBasicTankModel>  tankModels){
		for(MESBasicTankModel tankModel:tankModels)  {
			saveTankModel(tankModel, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="ec.flow.save")
	public void saveTankModel(MESBasicTankModel tankModel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveTankModel(tankModel, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="ec.flow.save")
	public void saveTankModel(MESBasicTankModel tankModel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveTankModel(tankModel, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateTankModelCodes(MESBasicTankModel tankModel) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTankModelCodes(tankModel, false);
	}
	
	public void generateTankModelCodes(MESBasicTankModel tankModel, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateTankModelSummarys(MESBasicTankModel tankModel) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateTankModelSummarys(tankModel, false);
	}

	public void generateTankModelSummarys(MESBasicTankModel tankModel, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel")
	public void saveTankModel(MESBasicTankModel tankModel,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveTankModel(tankModel, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel")
	public void saveTankModel(MESBasicTankModel tankModel,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_tankModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_tankModel");
		String url = null;
		if(tankModel.getId() != null && tankModel.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicTankModel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_tankModel/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicTankModel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_tankModel/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, tankModel);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(tankModel);
		beforeSaveTankModel(tankModel, viewIsView);

		if (viewIsView) {
			tankModelDao.saveWithRevertVersion(tankModel);
		} else {
			tankModelDao.save(tankModel);
		}
		if(null != tankModel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicTankModel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == tankModel.getCode() ? "" : tankModel.getCode().toString());
			AuditUtil.setColumnStringA(null == tankModel.getName() ? "" : tankModel.getName().toString());
		...
		dealDatagridsSave(tankModel,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateTankModelCodes(tankModel, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateTankModelSummarys(tankModel, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveTankModel(tankModel, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, tankModel);
			}
		}
		List<MESBasicTankModel> params = new ArrayList<MESBasicTankModel>();
		tankModelDao.flush();
		tankModelDao.clear();
		tankModel = tankModelDao.load(tankModel.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicTankModel tankModel){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("tankModel", tankModel);
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel")
	public void mergeTankModel(MESBasicTankModel tankModel, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(tankModel);
		beforeSaveTankModel(tankModel);
		tankModelDao.merge(tankModel);
		if(null != tankModel && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicTankModel.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == tankModel.getCode() ? "" : tankModel.getCode().toString());
			AuditUtil.setColumnStringA(null == tankModel.getName() ? "" : tankModel.getName().toString());
			AuditUtil.setColumnStringC(tankModel.getId().toString());
		}
		dealDatagridsSave(tankModel,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveTankModel(tankModel);
		tankModelDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/tankModel/add", tankModel,MESBasicTankModelMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicTankModel> findTankModels(Page<MESBasicTankModel> page, Criterion... criterions) {
		return tankModelDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicTankModelDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicTankModelDealInfo> list=tankModelDao.createQuery("from " + MESBasicTankModelDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicTankModelDealInfo dealInfo) {
		tankModelDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicTankModelDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) tankModelDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicTankModelDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = tankModelDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = tankModelDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicTankModelDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = tankModelDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	public List<String> getContainLower(String modelName, String fieldName, String exp, Object value, Boolean crossCompany) {
		...
		if(crossCompany) {
			layRecHql += " and cid = ?";
			return tankModelDao.createQuery(layRecHql, value, getCurrentCompanyId()).list();
		}
		return tankModelDao.createQuery(layRecHql, value).list();
	}
	
	private void dealListTreePageData(Page<MESBasicTankModel> tankModels, String viewCode){
		List<MESBasicTankModel> result = tankModels.getResult();
		if(null != result && !result.isEmpty()) {
			int rootPos = 0;
			MESBasicTankModel rootTree = result.get(0);
			...
			tankModels.setTreeToSurfaceMode(true);
			MESBasicTankModel tankModelTree = tankModelDao.buildTree("tankModel" + viewCode, new ListPrepareData(tankModels), false);
			if(null != tankModelTree.getChildren() && !tankModelTree.getChildren().isEmpty()) {
				List<MESBasicTankModel> treeList = new ArrayList<MESBasicTankModel>();
				Tree.treeToList(tankModelTree, treeList, false);
				if(treeList.size() == result.size()) {
					tankModels.setResult(treeList);
					tankModels.setTreeToSurfaceMode(false);
				} 
			} 
		}
	}
	 ...
	 * 
	 */
	private class ListPrepareData implements PrepareData<MESBasicTankModel,Long> {
		private Page<MESBasicTankModel> tankModels;

		public ListPrepareData(Page<MESBasicTankModel> tankModels) {
			this.tankModels = tankModels;
		}

		@Override
			public MESBasicTankModel root() {
				MESBasicTankModel tankModel = new MESBasicTankModel();
				tankModel.setId(-1l);
				return tankModel;
			}
			
			@Override
			public List<MESBasicTankModel> prepare(TreeDao<MESBasicTankModel, Long> dao) {
				return tankModels.getResult();
			}
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTankModels(Page<MESBasicTankModel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findTankModels(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTankModels(Page<MESBasicTankModel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findTankModels(Page<MESBasicTankModel> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicTankModel> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"tankModel\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"tankModel\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"tankModel\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_tankModel_TankModel" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_tankModel_TankModel" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  tankModelDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  tankModelDao.createNativeQuery(powerOperateSql, "MESBasic_1_tankModel").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"tankModel\"","MESBasic_1_tankModel",customCondition,"MESBASIC_TANK_MODELS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"tankModel\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicTankModelDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"tankModel\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"tankModel\".LAY_REC ASC, \"tankModel\".SORT ASC");
				} else {
					orderPart.append(" \"tankModel\".LAY_REC ASC, \"tankModel\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) tankModelDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=tankModelDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = tankModelDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicTankModel> result = new ArrayList<MESBasicTankModel>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicTankModel.class,tankModelDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicTankModel.class,tankModelDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicTankModel tankModel : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(tankModel.getId(), "MESBasic_tankModel_tankModel");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(tankModel.getId(), "MESBasic_tankModel_tankModel");
							if(null != documents && !documents.isEmpty()) {
								tankModel.setDocument(documents.get(0));
								tankModel.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(page.isExportFlag()){
					...
					if(exportMap.get("businessKey")!=null){
						try {
							...
							MESBasicTankModel tankModel = new MESBasicTankModel();
							getBusKeyMethod = tankModel.getClass().getMethod(getBusKeyMethodStr);
					...
					if(exportMap.get("mainDisplay")!=null){
						try {
							...
							MESBasicTankModel tankModel = new MESBasicTankModel();
							getMainDisMethod = tankModel.getClass().getMethod(getMainDisMethodStr);
						} catch (NoSuchMethodException | SecurityException e) {
							log.error(e.getMessage(),e);
						}
					}
					...
					for (MESBasicTankModel tankModel : result) {
						Long parentId = tankModel.getParentId();
						if(parentId != null && parentId.longValue() != new Long(-1).longValue()){
							for (MESBasicTankModel tankModel2 : result) {
								if(tankModel2.getId() != null && tankModel2.getId().longValue() == parentId.longValue()){
									try {
										if(getBusKeyMethod != null){
											Object busKey = getBusKeyMethod.invoke(tankModel2);
											tankModel.setParentNodeBusinessKey(busKey.toString());
										}
										if(getMainDisMethod != null){
											Object mainDis = getMainDisMethod.invoke(tankModel2);
											tankModel.setParentNodeMainDisplay(mainDis.toString());
										}	
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_tankModel")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_tankModel")+"MESBasic_1_tankModel".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_tankModel",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicTankModel> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicTankModel> result) {
		View view = null;
		...
		Map<Long, MESBasicTankModel> rsMap = new HashMap<Long, MESBasicTankModel>();
		for (MESBasicTankModel tankModel : result) {
			ids.add(tankModel.getId());
			rsMap.put(tankModel.getId(), tankModel);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(tankModelDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = tankModelDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicTankModel tankModel,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(tankModel.getOwnerStaffId());
		ti.setOwnerPositionId(tankModel.getOwnerPositionId());
		ti.setOwnerDepartmentId(tankModel.getOwnerDepartmentId());
		
		ti.setCreateStaff(tankModel.getCreateStaff());
		ti.setCreateStaffId(tankModel.getCreateStaffId());
		ti.setCreateTime(tankModel.getCreateTime());
		ti.setDeleteStaff(tankModel.getDeleteStaff());
		ti.setDeleteStaffId(tankModel.getDeleteStaffId());
		ti.setDeleteTime(tankModel.getDeleteTime());
		ti.setCreateDepartmentId(tankModel.getCreateDepartmentId());
		ti.setModifyStaff(tankModel.getModifyStaff());
		ti.setModifyStaffId(tankModel.getModifyStaffId());
		ti.setCreatePositionId(tankModel.getCreatePositionId());
		ti.setPositionLayRec(tankModel.getPositionLayRec());
		ti.setTableNo(tankModel.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_tankModel");
		ti.setTargetTableName("MESBASIC_TANK_MODELS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_tankModel_TankModel" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  tankModelDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"tankModel\"","MESBasic_1_tankModel","","MESBASIC_TANK_MODELS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"tankModel\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"tankModel\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"tankModel\"","MESBasic_1_tankModel","","MESBASIC_TANK_MODELS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("tankModel.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"tankModel\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"tankModel\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"tankModel\".CID = 1 OR \"tankModel\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("tankModel.mc\"").append(".TANK_MODEL) not in ("+notinValue+")");
			}
			
			SQLQuery query = tankModelDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("tankModel", "MESBasic_1_tankModel", "tankModel_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=tankModelDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicTankModel.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicTankModel.MODULE_CODE;}
	protected String getModelCode(){return MESBasicTankModel.MODEL_CODE;}
	
	@Override
	public List<MESBasicTankModel> getTreeChildren(Long parentId, String cond, Object... params) {
		MESBasicTankModel parent = tankModelDao.getTree("tankModel_tree_" + parentId, parentId, new MESBasicTankModelPrepareData(cond, params), false);
	...
	@Override
	public List<MESBasicTankModel> getTreeInfo(Map<Long, String> treeInfo, String condition, List<Object> params) {
		...
		for(String layRec : mapKeys) {
			...
			MESBasicTankModel tankModelTree = tankModelDao.getTree("MESBasicTankModel" + rootLayRec + "-", rootId, new PrepareData<MESBasicTankModel, Long>() {
				@Override
				public MESBasicTankModel root() {
					MESBasicTankModel tankModel = new MESBasicTankModel();
					tankModel.setId(-1l);
				...
				@Override
				public List<MESBasicTankModel> prepare(TreeDao<MESBasicTankModel, Long> dao) {
					String hql = "from " + MESBasicTankModel.JPA_NAME + " where (" + searchCondition + ") and valid = true " + conditionSql + " order by layRec asc, sort asc";
					return dao.findByHql(hql, paramsList.toArray());
				}
			}, false);
			List<MESBasicTankModel> treeChildren = new ArrayList<MESBasicTankModel>();
			Tree.treeToList(tankModelTree, treeChildren, true);
			treeList.addAll(treeChildren);
		}
		return treeList;
	}

	private class MESBasicTankModelPrepareData implements PrepareData<MESBasicTankModel, Long> {
		private String cond;
		private Object[] params;
		
		public MESBasicTankModelPrepareData(String cond, Object... params) {
			this.cond = cond;
			this.params = params;
		}
		...
		//@Override
		//public List<MESBasicTankModel> prepare(TreeDao<MESBasicTankModel, Long> dao) {
		//	String hql = "from " + MESBasicTankModel.JPA_NAME;
		//	if (null != cond && cond.trim().length() > 0) {
		//		hql += " where " + cond;
		//	}
		//	List<MESBasicTankModel> list = dao.findByHql(hql);
		//	return list;
		//}
		
		@Override
		public List<MESBasicTankModel> prepare(TreeDao<MESBasicTankModel, Long> dao) {
			List<MESBasicTankModel> list = null;
			List<MESBasicTankModel> result = new ArrayList<MESBasicTankModel>();
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicTankModel.class);
			if(params!=null&&params.length>0){
				...
				list = dao.findByHql("from " + MESBasicTankModel.JPA_NAME + " " + ((cond==null || cond.isEmpty()) ? "" : "where " + cond + " order by layRec asc, sort asc")) ;
			}
			for(MESBasicTankModel m : list){
				result.add(m);
				if(m.getParentId()!=-1){
					while(m.getParentId()!=-1){
						MESBasicTankModel parent = dao.get(m.getParentId());
	...
	/**
	 * 检查模型字段的唯一性约束
	 * @param tankModel 模型实例
	 */
	public void checkUniqueConstraint(MESBasicTankModel tankModel){
		if(tankModel.getId()==null){
			List<MESBasicTankModel> tankModels = tankModelDao.findByCriteria(Restrictions.eq("code", tankModel.getCode()), Restrictions.eq("valid", true));
			if(tankModels!=null && !tankModels.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.radion1415150470967", getCurrentUser().getLanguage()), tankModel.getCode());
			}
		} else {
			List<MESBasicTankModel> tankModels = tankModelDao.findByCriteria(Restrictions.ne("id", tankModel.getId()), Restrictions.eq("code", tankModel.getCode()), Restrictions.eq("valid", true));
			if(tankModels!=null && !tankModels.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.radion1415150470967", getCurrentUser().getLanguage()), tankModel.getCode());
			}
	...
	@Override
	public List<MESBasicTankModel> getTankModels(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicTankModel.class);
		if(params!=null){
			List<Type> types = new ArrayList<Type>();
			for(Object obj : params){
			...
			return tankModelDao.findByHql("from " + MESBasicTankModel.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return tankModelDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicTankModel> findTankModelsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicTankModel.class);
	...
	@Override
	public List<MESBasicTankModel> findTankModelsByHql(String hql, Object... objects){
		
		return   tankModelDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicTankModel loadTankModelByBussinessKey(MESBasicTankModel tankModel){
		return loadTankModelByBussinessKey(tankModel.getCode());
	}
	
	@Override
	public Page<MESBasicTankModel> getTankModels(Page<MESBasicTankModel> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicTankModel.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = tankModelDao.findByPage(page, sql);
			}else{
				page = tankModelDao.findAllByPage(page);
			}
			return page;
		}
		page = tankModelDao.findByPage(page, detachedCriteria);
		return page;
	}
	//==============DataGrid多选控件使用 start================
	
	@Override
	public void dealTankModelLeaf(List<Long> tankModelIds) {
		if (tankModelIds != null && tankModelIds.size() > 0) {
			String hql = "update " + MESBasicTankModel.JPA_NAME + " as t1 set t1.leaf=1 where id in(:ids) and not exists (select t2.id from "
					+ MESBasicTankModel.JPA_NAME + " as t2 where t2.parentId = t1.id)";
			Query query = tankModelDao.createQuery(hql);
			query.setParameterList("ids", tankModelIds);
			query.executeUpdate();
		}
	...
	public void drag(long dragSource, long dragTarget, long sort){
		MESBasicTankModel source = getTankModel(dragSource);
		MESBasicTankModel oldParent = source.getParent();
		
		MESBasicTankModel target = null;
		if(dragTarget!=-1){
			target = getTankModel(dragTarget);
		} else {
			target = new MESBasicTankModel();
			target.setId(-1L);
		}
		tankModelDao.move(source, target);
	}
	
	private void dragTankModel(MESBasicTankModel parent, List<MESBasicTankModel> children){
		for(MESBasicTankModel child : children){
			...
			saveTankModel(child, null,null,null);
			List<MESBasicTankModel> subChildren = getTreeChildren(child.getId(), "");
			if(subChildren!=null && subChildren.size()>0){
				dragTankModel(child, subChildren);
			}
		}
	}
	
	public List<MESBasicTankModel> convertTree(List<MESBasicTankModel> tankModels){
		List<MESBasicTankModel> trees = new ArrayList<MESBasicTankModel>();
		Map<Long, MESBasicTankModel> map = new LinkedHashMap<Long, MESBasicTankModel>(tankModels.size());
		Map<Long, Boolean> isParent = new HashMap<Long, Boolean>();
		for (MESBasicTankModel node : tankModels) {
		...
		for (MESBasicTankModel node : tankModels) {
			if (null == node.getParentId() ||Long.parseLong(node.getParentId().toString()) == -1) {
				...
				MESBasicTankModel parent = map.get(node.getParentId());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicTankModel loadTankModelByBussinessKey(Serializable bussinessKey){
		return tankModelDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicTankModel> findByProperty(String propertyName, Object object){
		return tankModelDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicTankModel findEntityByProperty(String propertyName, Object object){
		return tankModelDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicTankModel.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = tankModelDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_tankModel_TankModel","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_tankModel", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicTankModel tankModel,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,tankModelDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicTankModel.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, tankModel);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",tankModel.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicTankModel.TABLE_NAME +" where valid = 1";
			list = tankModelDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicTankModel.TABLE_NAME;
			list = tankModelDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicTankModel.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = tankModelDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicTankModelDaoImpl tankModelDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchTankModel(final List<MESBasicTankModel> insertObjs, final List<MESBasicTankModel> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchTankModel(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchTankModel(final List<MESBasicTankModel> insertObjs, final List<MESBasicTankModel> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1415150418847"),importCount));
		...
		Map<Long, MESBasicTankModel> oldUpdateObjsMap = new HashMap<Long, MESBasicTankModel>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicTankModel bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicTankModel> bizObjList = tankModelDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicTankModel bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicTankModel> nmeObjs = new ArrayList<MESBasicTankModel>();
		...
		long id=autoGeneratorID.getNextId(MESBasicTankModel.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicTankModel tankModel:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicTankModel.TABLE_NAME,1,dbId);
			if(tankModel.getCode() != null && !tankModel.getCode().equals("") ){
				m.put(tankModel.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		List<MESBasicTankModel> needChangeInsertObjList = new ArrayList<MESBasicTankModel>();
		if(insertNodeList != null && insertNodeList.size()>0 ){
			for(int i = 0; i < insertNodeList.size(); i++){
				...
				if(!needChange && !isRoot){
					MESBasicTankModel obj = insertObjs.get(i);
					...
					MESBasicTankModel parentObj = (MESBasicTankModel) insertNode.get(parentNode);
					*/
		...
		if(updateNodeList != null && updateNodeList.size()>0){
			for(int i = 0; i < updateNodeList.size(); i++){
				...
				MESBasicTankModel updateObj = updateObjs.get(i);
				...
				if(isRoot){
					...
					if(needChange!=null && !needChange){
						...
						MESBasicTankModel parentNode = (MESBasicTankModel) insertParentNode.get(parentNodeKey);
		...
		if(insertNodeList != null && insertNodeList.size()>0){
			for(int i = 0; i < insertNodeList.size(); i++){
				Map<String, Object> insertNode = insertNodeList.get(i);
				MESBasicTankModel insertObj = insertObjs.get(i);
		...
		if(updateObjs != null && updateObjs.size()>0 ){
			for(MESBasicTankModel obj : updateObjs){
				Object bus;
				try {
		...
		if(businessKey != null && mainDisplay != null && businessKey != mainDisplay  && paramList != null && paramList.size()>0){
			String hql = "select " + businessKey.getName() + "," + mainDisplay.getName() + ",id from " + MESBasicTankModel.JPA_NAME + " where " + businessKey.getName() + " in (:codes)";
			Query query =  tankModelDao.createQuery(hql);
			query.setParameterList("codes", paramList);
			List<Object[]> list = query.list();
			if(list.size() == updateObjs.size()){
				for(int i = 0 ; i < updateObjs.size();i++ ){
					MESBasicTankModel obj = updateObjs.get(i);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) tankModelDao.createNativeQuery(sql, "MESBasic_1_tankModel_TankModel"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		tankModelDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicTankModel.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicTankModel tankModel = updateObjs.get(updateIndex);
						Long id = tankModel.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = tankModel.getClass().getMethod(updateMethods.get(i));
								} catch (NoSuchMethodException | SecurityException e) {
									log.error(e.getMessage(),e);
								}
								Object obj = null;
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicTankModel.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicTankModel.TABLE_NAME + " order by id desc";
//					Query query = tankModelDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicTankModel tankModel = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = tankModel.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicTankModelMneCode.class;
					...
					for(MESBasicTankModel tankModel:updateObjs)  {
						Long id = tankModel.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicTankModel tankModel:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(tankModel);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, tankModel.getId());
		...
		tankModelDaoImpl.getSession().flush();
		tankModelDaoImpl.getSession().clear();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicTankModel bizObj = insertObjs.get(insertIndex);
			tankModelImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicTankModel bizObj = updateObjs.get(updateIndex);
			MESBasicTankModel oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			tankModelImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	private void changeFullPathName(List<Long> needChangeFullPathNameId, Map<Object, Object> needChangeFullPathName,final Property businessKey,Property mainDisplay){
		...
		sql.append(" from ");
		sql.append(MESBasicTankModel.TABLE_NAME);
		sql.append(" where ");
		...
		List<Object[]> lists =  tankModelDao.createNativeQuery(sqlStr).list();
		...
		tankModelDaoImpl.getSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
				...
				if(updateLists != null && updateLists.size()>0){
					String updateSql = "UPDATE " + MESBasicTankModel.TABLE_NAME + " SET FULL_PATH_NAME = ? WHERE " + businessKey.getColumnName() + " = ?";
					updateps = conn.prepareStatement(updateSql);
	...
	private Map<String, Object> buildInsertObjNode(List<Map<String, Object>> insertNodeList, List<MESBasicTankModel> insertObjs, 
			...
			for(int i =0; i<insertNodeList.size(); i++){
				...
				if(insertBuildedObj.get(busKey)!=null){
					...
					if(isRoot){
						...
						String parentNode = insertNode.get("parentNode").toString();
						MESBasicTankModel parentObj = (MESBasicTankModel) insertBuildedObj.get(parentNode);
	...
	public String getViewName(String modelCode){
		...
		String viewName = tankModelDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				tankModelDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = tankModelDao.createNativeQuery(sql, "MESBasic_1_tankModel_TankModel").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = tankModelDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = tankModelDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = tankModelDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?tankModelDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  tankModelDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,tankModelDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="ec.flow.save")
	public void saveTankModel(MESBasicTankModel tankModel, DataGridService dataGridService){
		saveTankModel(tankModel, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="ec.flow.save")
	public void saveTankModel(MESBasicTankModel tankModel, DataGridService dataGridService, String viewCode){
		saveTankModel(tankModel, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel")
	public void saveTankModel(MESBasicTankModel tankModel, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveTankModel(tankModel, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel")
	public void saveTankModel(MESBasicTankModel tankModel, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_tankModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_tankModel");
		String url = null;
		if(null != tankModel.getId() && tankModel.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicTankModel.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel")
	public Map<Object, Object> tankModelDataGridImport(MESBasicTankModel tankModel, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = tankModel.getVirtualId();
		
		tankModel = tankModelDao.load(tankModel.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, tankModel.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = tankModel.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(tankModel);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, tankModel.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel")
	public void mergeTankModel(MESBasicTankModel tankModel, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(tankModel);
		beforeSaveTankModel(tankModel);
		tankModelDao.merge(tankModel);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_tankModel",model="MESBasic_1_tankModel_TankModel",desc="foundation.batch.save",operType="4")
	public void batchSaveTankModel(MESBasicTankModel tankModel, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_tankModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_tankModel");
	...
	@Override
	public Page<MESBasicTankModel> getByPage(Page<MESBasicTankModel> page,DetachedCriteria detachedCriteria){
		return tankModelDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_tankModel").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveTankModel(MESBasicTankModel tankModel, Object... objects){
		checkUniqueConstraint(tankModel);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveTankModel(MESBasicTankModel tankModel, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportTankModel(List<MESBasicTankModel> tankModels, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportTankModel(List<MESBasicTankModel> tankModels){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportTankModel(List<MESBasicTankModel> tankModels, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportTankModel(List<MESBasicTankModel> tankModels){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteTankModel(MESBasicTankModel tankModel){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteTankModel(MESBasicTankModel tankModel){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreTankModel(MESBasicTankModel tankModel){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreTankModel(MESBasicTankModel tankModel){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicTankModel tankModel, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicTankModel tankModel, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportTankModel(List<MESBasicTankModel> insertObjs, List<MESBasicTankModel> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportTankModel,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportTankModel(List<MESBasicTankModel> insertObjs, List<MESBasicTankModel> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportTankModel,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
