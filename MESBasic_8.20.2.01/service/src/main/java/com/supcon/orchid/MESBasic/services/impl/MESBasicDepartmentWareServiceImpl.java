...
import com.supcon.orchid.MESBasic.services.MESBasicDepartmentWareImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicDepartmentWareDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
import com.supcon.orchid.MESBasic.entities.MESBasicProduct;
import org.apache.cxf.common.util.CollectionUtils;
/* CUSTOM CODE END */

@Service("mESBasic_departmentWareService")
@Transactional
public class MESBasicDepartmentWareServiceImpl  extends BaseServiceImpl<MESBasicDepartmentWare> implements MESBasicDepartmentWareService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicDepartmentWare> bapBaseService;
	...
	@Autowired
	private MESBasicDepartmentWareImportService departmentWareImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicDepartmentWare.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  departmentWareDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicDepartmentWare.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = departmentWareDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicDepartmentWare.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicDepartmentWare.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  departmentWareDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicDepartmentWare.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicDepartmentWare.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicDepartmentWare> list = (List<MESBasicDepartmentWare>) departmentWareDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicDepartmentWare getDepartmentWare(long id){
		return getDepartmentWare(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicDepartmentWare getDepartmentWare(long id, String viewCode){
		MESBasicDepartmentWare departmentWare = departmentWareDao.load(id);

		// 一对多情况处理
		if(departmentWare != null){
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getDepartmentWareAsJSON(long id, String include){
		MESBasicDepartmentWare departmentWare = departmentWareDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(departmentWare, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.common.delete",operType="3")
	public void deleteDepartmentWare(MESBasicDepartmentWare departmentWare){
		if(null != departmentWare && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDepartmentWare.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
			AuditUtil.setColumnStringA(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
			AuditUtil.setColumnStringC(departmentWare.getId().toString());
		}
		beforeDeleteDepartmentWare(departmentWare);
		departmentWareDao.deletePhysical(departmentWare);
		afterDeleteDepartmentWare(departmentWare);
		departmentWareDao.flush();
		List<MESBasicDepartmentWare> params = new ArrayList<MESBasicDepartmentWare>();
		params.add(departmentWare);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryWare");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.common.delete",operType="3")
	public void deleteDepartmentWare(List<Long> departmentWareIds){
		deleteDepartmentWare(departmentWareIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.common.delete",operType="3")
	public void deleteDepartmentWare(List<Long> departmentWareIds, String eventTopic) {
		facWareMaterialService.deleteFacWareMaterialByDepartmentWare(departmentWareIds);
		List<MESBasicDepartmentWare> departmentWares = new ArrayList<MESBasicDepartmentWare>();
		for(Long departmentWareId : departmentWareIds){
			MESBasicDepartmentWare departmentWare = getDepartmentWare(departmentWareId);
			departmentWares.add(departmentWare);
			if(null != departmentWare && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDepartmentWare.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == departmentWare.getId() ? "" : "," + departmentWare.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == departmentWare.getId() ? "" : "," + departmentWare.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == departmentWare.getId() ? "" : "," + departmentWare.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(departmentWares != null && departmentWares.size() > 0 ){
			for(MESBasicDepartmentWare departmentWare : departmentWares){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicDepartmentWare.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(departmentWare));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),departmentWare, departmentWare.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(departmentWareIds != null && departmentWareIds.size() > 0) {
			String hql = "delete " + MESBasicDepartmentWare.JPA_NAME + " where id in(:ids)";
			Query query = departmentWareDao.createQuery(hql);
			query.setParameterList("ids", departmentWareIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicDepartmentWare departmentWarez : departmentWares){
				departmentWarez.setValid(false);
				List<MESBasicDepartmentWare> params = new ArrayList<MESBasicDepartmentWare>();
				params.add(departmentWarez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_factoryWare");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.common.delete",operType="3")
	public void deleteDepartmentWare(long departmentWareId, int departmentWareVersion){
		this.deleteDepartmentWare(departmentWareId, departmentWareVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.common.delete",operType="3")
	public void deleteDepartmentWare(long departmentWareId, int departmentWareVersion,SignatureLog signatureLog){
		MESBasicDepartmentWare departmentWare = getDepartmentWare(departmentWareId);
		if(departmentWare != null && !departmentWare.isValid()){
		...
		beforeDeleteDepartmentWare(departmentWare);
		departmentWareDao.delete(departmentWareId, departmentWareVersion);
		afterDeleteDepartmentWare(departmentWare);
		departmentWareDao.flush();
		List<MESBasicDepartmentWare> params = new ArrayList<MESBasicDepartmentWare>();
		params.add(departmentWare);
		...
		if(signatureLog != null ) {
			...
			businessKey=departmentWare.getId();
			signatureLog.setTableId(departmentWare.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.common.delete",operType="3")
	public void deleteDepartmentWare(String departmentWareIds){
		this.deleteDepartmentWare(departmentWareIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.common.delete",operType="3")
	public void deleteDepartmentWare(String departmentWareIds,SignatureLog signatureLog){
		deleteCollection(departmentWareIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteDepartmentWare(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_factoryWare#modelCode:MESBasic_1_factoryWare_DepartmentWare#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="ec.engine.restore",operType="10")
	public void restoreDepartmentWare(String departmentWareIds){
		restoreCollection(departmentWareIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreDepartmentWare(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="ec.engine.restore",operType="10")
	public void restoreDepartmentWare(long departmentWareId){
		
		findBusinessKeyUsed(departmentWareId);	//判断业务主键是否重复
		...
		beforeRestoreDepartmentWare(departmentWare);
		departmentWare.setValid(true);
		departmentWareDao.update(departmentWare);
		afterRestoreDepartmentWare(departmentWare);
		List<MESBasicDepartmentWare> params = new ArrayList<MESBasicDepartmentWare>();
		params.add(departmentWare);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long departmentWareId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_factoryWare_DepartmentWare");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicDepartmentWare.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicDepartmentWare.TABLE_NAME +" where id = ? )";
			List<Object> list =  departmentWareDao.createNativeQuery(sql,departmentWareId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseDepartmentWare(List<MESBasicDepartmentWare>  departmentWares){
		for(MESBasicDepartmentWare departmentWare:departmentWares)  {
			saveDepartmentWare(departmentWare, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseDepartmentWare(List<MESBasicDepartmentWare>  departmentWares){
		for(MESBasicDepartmentWare departmentWare:departmentWares)  {
			saveDepartmentWare(departmentWare, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="ec.flow.save")
	public void saveDepartmentWare(MESBasicDepartmentWare departmentWare, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveDepartmentWare(departmentWare, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="ec.flow.save")
	public void saveDepartmentWare(MESBasicDepartmentWare departmentWare, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveDepartmentWare(departmentWare, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateDepartmentWareCodes(MESBasicDepartmentWare departmentWare) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateDepartmentWareCodes(departmentWare, false);
	}
	
	public void generateDepartmentWareCodes(MESBasicDepartmentWare departmentWare, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateDepartmentWareSummarys(MESBasicDepartmentWare departmentWare) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateDepartmentWareSummarys(departmentWare, false);
	}

	public void generateDepartmentWareSummarys(MESBasicDepartmentWare departmentWare, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare")
	public void saveDepartmentWare(MESBasicDepartmentWare departmentWare,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveDepartmentWare(departmentWare, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare")
	public void saveDepartmentWare(MESBasicDepartmentWare departmentWare,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryWare";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryWare");
		String url = null;
		if(departmentWare.getId() != null && departmentWare.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDepartmentWare.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_factoryWare/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDepartmentWare.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_factoryWare/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, departmentWare);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(departmentWare);
		beforeSaveDepartmentWare(departmentWare, viewIsView);

		if (viewIsView) {
			departmentWareDao.saveWithRevertVersion(departmentWare);
		} else {
			if(isNew)
				departmentWareDao.save(departmentWare);
			else
				departmentWareDao.merge(departmentWare);
		}
		if(null != departmentWare && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDepartmentWare.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
			AuditUtil.setColumnStringA(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
		...
		dealDatagridsSave(departmentWare,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateDepartmentWareCodes(departmentWare, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateDepartmentWareSummarys(departmentWare, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveDepartmentWare(departmentWare, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, departmentWare);
			}
		}
		List<MESBasicDepartmentWare> params = new ArrayList<MESBasicDepartmentWare>();
		departmentWareDao.flush();
		departmentWareDao.clear();
		departmentWare = departmentWareDao.load(departmentWare.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicDepartmentWare departmentWare){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("departmentWare", departmentWare);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare")
	public void mergeDepartmentWare(MESBasicDepartmentWare departmentWare, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(departmentWare);
		beforeSaveDepartmentWare(departmentWare);
		departmentWareDao.merge(departmentWare);
		if(null != departmentWare && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDepartmentWare.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
			AuditUtil.setColumnStringA(null == departmentWare.getId() ? "" : departmentWare.getId().toString());
		...
		dealDatagridsSave(departmentWare,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveDepartmentWare(departmentWare);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicDepartmentWare> findDepartmentWares(Page<MESBasicDepartmentWare> page, Criterion... criterions) {
		return departmentWareDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDepartmentWares(Page<MESBasicDepartmentWare> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findDepartmentWares(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDepartmentWares(Page<MESBasicDepartmentWare> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDepartmentWares(Page<MESBasicDepartmentWare> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicDepartmentWare> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"departmentWare\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"departmentWare\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"departmentWare\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "MESBasic_1_factoryWare" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  departmentWareDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  departmentWareDao.createNativeQuery(powerOperateSql, "MESBasic_1_factoryWare").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"departmentWare\".LAY_REC ASC, \"departmentWare\".SORT ASC");
				} else {
					orderPart.append(" \"departmentWare\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) departmentWareDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=departmentWareDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = departmentWareDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicDepartmentWare> result = new ArrayList<MESBasicDepartmentWare>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicDepartmentWare.class,departmentWareDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicDepartmentWare.class,departmentWareDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicDepartmentWare departmentWare : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(departmentWare.getId(), "MESBasic_factoryWare_departmentWare");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(departmentWare.getId(), "MESBasic_factoryWare_departmentWare");
							if(null != documents && !documents.isEmpty()) {
								departmentWare.setDocument(documents.get(0));
								departmentWare.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_factoryWare")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_factoryWare")+"MESBasic_1_factoryWare".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_factoryWare",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicDepartmentWare> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicDepartmentWare> result) {
		View view = null;
		...
		Map<Long, MESBasicDepartmentWare> rsMap = new HashMap<Long, MESBasicDepartmentWare>();
		for (MESBasicDepartmentWare departmentWare : result) {
			ids.add(departmentWare.getId());
			rsMap.put(departmentWare.getId(), departmentWare);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(departmentWareDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = departmentWareDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_factoryWare_DepartmentWare" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_factoryWare_DepartmentWare" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  departmentWareDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"departmentWare\"","MESBasic_1_factoryWare","","MESBASIC_DEPARTMENT_WARES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"departmentWare\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"departmentWare\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"departmentWare\"","MESBasic_1_factoryWare","","MESBASIC_DEPARTMENT_WARES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_factoryWare_DepartmentWare" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("departmentWare.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"departmentWare\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"departmentWare\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"departmentWare\".CID = 1 OR \"departmentWare\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("departmentWare.mc\"").append(".DEPARTMENT_WARE) not in ("+notinValue+")");
			}
			
			SQLQuery query = departmentWareDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("factoryWare", "MESBasic_1_factoryWare", "factoryWare_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=departmentWareDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicDepartmentWare.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicDepartmentWare.MODULE_CODE;}
	protected String getModelCode(){return MESBasicDepartmentWare.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param departmentWare 模型实例
	 */
	public void checkUniqueConstraint(MESBasicDepartmentWare departmentWare){
	}
	
	
	@Override
	public List<MESBasicDepartmentWare> getDepartmentWares(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicDepartmentWare.class);
		if(params!=null){
			...
			return departmentWareDao.findByHql("from " + MESBasicDepartmentWare.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return departmentWareDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicDepartmentWare> findDepartmentWaresByHql(String hql, Object... objects){
		...
		return   departmentWareDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicDepartmentWare loadDepartmentWareByBussinessKey(MESBasicDepartmentWare departmentWare){
		return loadDepartmentWareByBussinessKey(departmentWare.getId());
	}
	
	@Override
	public Page<MESBasicDepartmentWare> getDepartmentWares(Page<MESBasicDepartmentWare> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicDepartmentWare.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = departmentWareDao.findByPage(page, sql);
			}else{
				page = departmentWareDao.findAllByPage(page);
			}
			return page;
		}
		page = departmentWareDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public MESBasicDepartmentWare loadDepartmentWareByBussinessKey(Serializable bussinessKey){
		return departmentWareDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicDepartmentWare> findByProperty(String propertyName, Object object){
		return departmentWareDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicDepartmentWare findEntityByProperty(String propertyName, Object object){
		return departmentWareDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicDepartmentWare.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = departmentWareDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_factoryWare_DepartmentWare","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_factoryWare", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicDepartmentWare departmentWare,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,departmentWareDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicDepartmentWare.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, departmentWare);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",departmentWare.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicDepartmentWare.TABLE_NAME +" where valid = 1";
			list = departmentWareDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicDepartmentWare.TABLE_NAME;
			list = departmentWareDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicDepartmentWare.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = departmentWareDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicDepartmentWareDaoImpl departmentWareDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchDepartmentWare(final List<MESBasicDepartmentWare> insertObjs, final List<MESBasicDepartmentWare> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchDepartmentWare(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchDepartmentWare(final List<MESBasicDepartmentWare> insertObjs, final List<MESBasicDepartmentWare> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1489731905451"),importCount));
		...
		Map<Long, MESBasicDepartmentWare> oldUpdateObjsMap = new HashMap<Long, MESBasicDepartmentWare>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicDepartmentWare bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicDepartmentWare> bizObjList = departmentWareDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicDepartmentWare bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicDepartmentWare> nmeObjs = new ArrayList<MESBasicDepartmentWare>();
		...
		long id=autoGeneratorID.getNextId(MESBasicDepartmentWare.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicDepartmentWare departmentWare:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicDepartmentWare.TABLE_NAME,1,dbId);
			if(departmentWare.getId() != null ){
				m.put(departmentWare.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) departmentWareDao.createNativeQuery(sql, "MESBasic_1_factoryWare_DepartmentWare"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		departmentWareDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicDepartmentWare.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicDepartmentWare departmentWare = updateObjs.get(updateIndex);
						Long id = departmentWare.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = departmentWare.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(departmentWare);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicDepartmentWare.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicDepartmentWare.TABLE_NAME + " order by id desc";
//					Query query = departmentWareDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicDepartmentWare departmentWare = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = departmentWare.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		departmentWareDaoImpl.getSession().flush();
		departmentWareDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicDepartmentWare bizObj = insertObjs.get(insertIndex);
			departmentWareImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicDepartmentWare bizObj = updateObjs.get(updateIndex);
			MESBasicDepartmentWare oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			departmentWareImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicDepartmentWare> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicDepartmentWare> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) departmentWareDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicDepartmentWare> result = getResult(page, query, new PendingResultTransformer(MESBasicDepartmentWare.class,departmentWareDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicDepartmentWare model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = departmentWareDao.createNativeQuery(sql, "MESBasic_1_factoryWare_DepartmentWare").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = departmentWareDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = departmentWareDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = departmentWareDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?departmentWareDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  departmentWareDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,departmentWareDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1489738203672Page(Page<MESBasicFacWareMaterial> dg1489738203672Page,MESBasicDepartmentWare departmentWare, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(MESBasicFacWareMaterial.class);
		criteria.add(Restrictions.eq("headID", departmentWare));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"facWareMaterial\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		facWareMaterialDao.findByPage(dg1489738203672Page, criteria);
		facWareMaterialService.findByNormalDataGridDepartmentWare(dg1489738203672Page);
	}
	
	 
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="ec.flow.save")
	public void saveDepartmentWare(MESBasicDepartmentWare departmentWare, DataGridService dataGridService){
		saveDepartmentWare(departmentWare, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="ec.flow.save")
	public void saveDepartmentWare(MESBasicDepartmentWare departmentWare, DataGridService dataGridService, String viewCode){
		saveDepartmentWare(departmentWare, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare")
	public void saveDepartmentWare(MESBasicDepartmentWare departmentWare, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveDepartmentWare(departmentWare, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare")
	public void saveDepartmentWare(MESBasicDepartmentWare departmentWare, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryWare";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryWare");
		String url = null;
		if(null != departmentWare.getId() && departmentWare.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicDepartmentWare.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare")
	public Map<Object, Object> departmentWareDataGridImport(MESBasicDepartmentWare departmentWare, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = departmentWare.getVirtualId();
		
		departmentWare = departmentWareDao.load(departmentWare.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, departmentWare.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = departmentWare.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(departmentWare);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, departmentWare.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare")
	public void mergeDepartmentWare(MESBasicDepartmentWare departmentWare, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(departmentWare);
		beforeSaveDepartmentWare(departmentWare);
		departmentWareDao.merge(departmentWare);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryWare",model="MESBasic_1_factoryWare_DepartmentWare",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicDepartmentWare> getByPage(Page<MESBasicDepartmentWare> page,DetachedCriteria detachedCriteria){
		return departmentWareDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_factoryWare").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveDepartmentWare(MESBasicDepartmentWare departmentWare, Object... objects){
		checkUniqueConstraint(departmentWare);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveDepartmentWare(MESBasicDepartmentWare departmentWare, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
      
      	departmentWare = departmentWareDao.load(departmentWare.getId());
		List<Object> params = new ArrayList<Object>();
		params.add(departmentWare.getId());
		//获取当前车间仓表体信息
		List<MESBasicFacWareMaterial> materialList = facWareMaterialService.getFacWareMaterials("this_.HEAD_ID=? and this_.valid=1 ", params);
		//获取当前节点的工厂模型
		MESBasicFactoryModel model = departmentWare.getFactoryId();
		//获取节点属性
		String layRec = model.getLayRec();
		//验证的结果集合
		String errorMassge="";
		//校验当前节点和父节点的物品不能重复
		if (null != layRec) {
			...
			for (int i = 0; i < layRecList.length - 1; i++) {
				//获取父节点的工厂模型
				MESBasicFactoryModel parentModel = factoryModelDao.load(Long.valueOf(layRecList[i]));
				String result="";
				for (MESBasicFacWareMaterial material : materialList) {
					//获取父节点下的物品信息
					List<Object> params1=new ArrayList<Object>();
					params1.add(Long.valueOf(layRecList[i]));
					params1.add(material.getProductId().getId());
					List<MESBasicFacWareMaterial> objWareStoreList=facWareMaterialService.getFacWareMaterials("this_.FACTORY_ID=? and this_.PRODUCT_ID=? and this_.valid=1 ",params1);
                 	for(int j=0;j<objWareStoreList.size();j++){
                 		List<Object> param=new ArrayList<Object>();
    					param.add(objWareStoreList.get(j).getHeadID().getId());
                 		List<MESBasicDepartmentWare> wareList = this.getDepartmentWares("this_.ID=? and this_.valid=1 ", param);
                 		if(wareList.size()==0){
                 			objWareStoreList.remove(j);
                 		}
					}
					if (objWareStoreList.size() > 0) {
						//所有重复物品的物品编码
						result+=material.getProductId().getProductCode()+"、";
					}
				}
				if(result!=""){
                  //errorMassge+=InternationalResource.get("factoryModel.error.message1", parentModel.getCode(),result.substring(0, result.length()-1));
					//errorMassge+="节点编码["+parentModel.getCode()+"]下的物料编码为["+result.substring(0, result.length()-1)+"]";
                  errorMassge+="物料编码为["+result.substring(0, result.length()-1)+"]的物料与节点编码为["+parentModel.getCode()+"]下的物料、";
				}
			}
		}
		//校验当前节点和子节点的物品不能重复
		...
		params3.add("%"+layRec+"-%");
		//获取子节点的工厂模型
		List<MESBasicFactoryModel> modelList=factoryModelService.getFactoryModels("this_.LAY_REC like ? and this_.valid=1 ",params3);
		for(MESBasicFactoryModel famodel:modelList){
			String result="";
			if(famodel!=null){
				for (MESBasicFacWareMaterial material : materialList) {
				
					List<Object> paramses=new ArrayList<Object>();
					paramses.add(famodel.getId());
					paramses.add(material.getProductId().getId());
					List<MESBasicFacWareMaterial> list=facWareMaterialService.getFacWareMaterials("this_.FACTORY_ID=? and this_.PRODUCT_ID=? and this_.valid=1 ",paramses);
					for(int j=0;j<list.size();j++){
                 		List<Object> param=new ArrayList<Object>();
    					param.add(list.get(j).getHeadID().getId());
                 		List<MESBasicDepartmentWare> wareList = this.getDepartmentWares("this_.ID=? and this_.valid=1 ", param);
                 		if(wareList.size()==0){
                 			list.remove(j);
                 		}
			...
			if(result!=""){
             	// errorMassge+=InternationalResource.get("factoryModel.error.message1", famodel.getCode(),result.substring(0, result.length()-1));
				//errorMassge+="节点编码["+famodel.getCode()+"]下的物料编码为["+result.substring(0, result.length()-1)+"]";
              errorMassge+="物料编码为["+result.substring(0, result.length()-1)+"]的物料与节点编码为["+famodel.getCode()+"]下的物料、";
			}
		}
		if(errorMassge!=""){
			throw new BAPException(errorMassge.subSequence(0, errorMassge.length()-1)+"重复！");
		}
		for (MESBasicFacWareMaterial material : materialList) {
			material.setFactoryId(departmentWare.getFactoryId());
			material.setWareId(departmentWare.getWareId());
			material.setStoreId(departmentWare.getStoreId());
			facWareMaterialDao.merge(material);
		}
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteDepartmentWare(MESBasicDepartmentWare departmentWare){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteDepartmentWare(MESBasicDepartmentWare departmentWare){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
		List<Long> departmentWareIds = new ArrayList<>();
		departmentWareIds.add(departmentWare.getId());
		facWareMaterialService.deleteFacWareMaterialByDepartmentWare(departmentWareIds);
/* CUSTOM CODE END */
	}
	
	private void beforeRestoreDepartmentWare(MESBasicDepartmentWare departmentWare){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreDepartmentWare(MESBasicDepartmentWare departmentWare){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicDepartmentWare departmentWare, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicDepartmentWare departmentWare, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportDepartmentWare(List<MESBasicDepartmentWare> insertObjs, List<MESBasicDepartmentWare> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportDepartmentWare,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportDepartmentWare(List<MESBasicDepartmentWare> insertObjs, List<MESBasicDepartmentWare> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportDepartmentWare,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_factoryWare_DepartmentWare,MESBasic_1) */
// 自定义代码

	@Override
	public String findOperStatue(String wareId) {
		MESBasicWare materialWare=wareDao.load(Long.parseLong(wareId));
		String  opetareStatue=materialWare.getCargoStatue().getId();
		return opetareStatue;
	}
/* CUSTOM CODE END */
}
