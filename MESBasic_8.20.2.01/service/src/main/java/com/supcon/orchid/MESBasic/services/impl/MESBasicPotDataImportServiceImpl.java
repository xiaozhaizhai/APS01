...
@Service("mESBasicPotDataImportService")
public class MESBasicPotDataImportServiceImpl implements MESBasicPotDataImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_potDataHandler",model="MESBasic_1_potDataHandler_PotData",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicPotData bizObj, MESBasicPotData oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1455958747117") + "信息");
			...
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getTempID() ? "" : "," + oldBizObj.getTempID().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getTempID() ? "" : oldBizObj.getTempID().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1455958747117") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
			...
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getTempID() ? "" : "," + bizObj.getTempID().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getTempID() ? "" : bizObj.getTempID().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicPotData bizObj, String bussinesskeyFieldName,
