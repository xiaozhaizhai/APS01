...
import com.supcon.orchid.MESBasic.services.MESBasicEquipmentWSService;
import com.supcon.orchid.MESBasic.services.MESBasicEquipmentService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicEquipment",targetNamespace = "http://ws.supcon.com")
public class MESBasicEquipmentWSServiceImpl implements MESBasicEquipmentWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getEquipmentByBusinessKey(
		...
		try{
			...
			MESBasicEquipment equipment = equipmentService.loadEquipmentByBussinessKey(businessKey);
			if(equipment == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, equipment);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateEquipment(
		...
		try{
			if(equipment == null){
				return set500Error(result, "设备不能为空。");
			}
			...
			if(!beforeUpdateEquipment(equipment)){
				return set404Error(result);
			}
			equipmentService.saveEquipment(equipment, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addEquipment(
		...
		try{
			...
			try{
				checkEquipmentData(equipment);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			equipmentService.saveEquipment(equipment, null);
			setSuccessResult(result, equipment.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteEquipmentByBusinessKey(
		...
		try{
			...
			try{
				equipment = equipmentService.loadEquipmentByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddEquipment(
		...
		try{
			...
			int count = 1;
			for(MESBasicEquipment equipment : equipmentList){
				if(equipment.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(equipment.getCode() == null || equipment.getCode().toString().trim().isEmpty()){
			...
			equipmentService.batchImportBaseEquipment(equipmentList);
			for(MESBasicEquipment equipment : equipmentList){
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteEquipmentByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicEquipment temp = null;
				try{
					temp = equipmentService.getEquipment(id);
				}catch(NullPointerException e){}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteEquipmentByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicEquipment temp = null;
				try{
					temp = equipmentService.loadEquipmentByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getEquipmentPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicEquipment.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicEquipment.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicEquipment.class);
				} catch (Exception e) {
			...
			Page<MESBasicEquipment> page = new Page<MESBasicEquipment>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicEquipment> datas = equipmentService.getByPage(page, detachedCriteria);
			List<MESBasicEquipment> equipmentList = datas.getResult();
			if (equipmentList == null || equipmentList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(equipmentList.toArray());
			PaginationDTO<MESBasicEquipment> pagination = new PaginationDTO<MESBasicEquipment>(page);
	...
	/**
	 * 获取设备对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param equipment
	 * @return
	 */
	private MESBasicEquipment getEquipment(MESBasicEquipment equipment) {
		if (equipment.getId() == null && (equipment.getCode() == null || equipment.getCode().toString().trim() == ""))
			throw new RuntimeException("设备对象业务主键和id不能同时为空");
		if (equipment.getId() != null) {
			MESBasicEquipment result = equipmentService.getEquipment(equipment .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return equipmentService.loadEquipmentByBussinessKey(equipment.getCode());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个设备，则返回false，否则返回true
	 * @param equipment
	 */
	private boolean beforeUpdateEquipment(MESBasicEquipment equipment) {
		if(equipment.getName() == null){
			throw new RuntimeException("name不能为空。");
		}
		MESBasicEquipment temp = getEquipment(equipment);
		if (temp == null) {
			return false;
		}
		if(equipment.getId() == null){
			equipment.setId(temp.getId());		
		}
		if(equipment.getCode() == null){
			equipment.setCode(temp.getCode());
		}
		
		equipment.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除设备对象下所有辅模型对象中的设备对象
	 * @param equipment
	 */
	private void clearCircle(MESBasicEquipment equipment) {
	 ...
	 */
	private void checkEquipmentData(MESBasicEquipment equipment){
		 if(equipment.getName() == null){
		 	throw new RuntimeException("name不能为空。");
		 }
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
