...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryModel_FactoryModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicFactoryModelAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicFactoryModel> {
	...
	//导出excel时,主模型内容
	List<MESBasicFactoryModel> queryResults = new ArrayList<MESBasicFactoryModel>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/factoryModel/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/factoryModel/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryModel_FactoryModel" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicFactoryModel> factoryModels;
	
	public List<MESBasicFactoryModel> getFactoryModels() {
	...
	private Page<MESBasicFactoryModel> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			factoryModel = new MESBasicFactoryModel();
			if(factoryModel.getArea() != null) {
				factoryModel.setArea((SystemCode) systemCodeService.getSystemCode(factoryModel.getArea().getId()));
			}
			if(factoryModel.getInterfaceUrl() != null) {
				factoryModel.setInterfaceUrl((SystemCode) systemCodeService.getSystemCode(factoryModel.getInterfaceUrl().getId()));
			}
			if(factoryModel.getRunState() != null) {
				factoryModel.setRunState((SystemCode) systemCodeService.getSystemCode(factoryModel.getRunState().getId()));
			}
		} else {
			if(id > 0){
				factoryModel = factoryModelService.getFactoryModel(id, viewCode);
			}
		}
	...
	public Page<MESBasicFactoryModel> getPage() {
		if (null == page) {
			page = new Page<MESBasicFactoryModel>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/factoryModel/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = factoryModelService.getFactoryModelAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/factoryModel/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String factoryModelReferenceCopy() throws Exception{
			MESBasicFactoryModel oldFactoryModel = null;
			if(isEditNew()){
				...
				oldFactoryModel = new MESBasicFactoryModel();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,factoryModel.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, factoryModel.getId());
				...
				oldFactoryModel = factoryModelService.getFactoryModel(id);
			}
			
			
			factoryModel = factoryModelService.getFactoryModel(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_FactoryModel");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicFactoryModel.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFactoryModel, null);
						Method setValue = MESBasicFactoryModel.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(factoryModel, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			factoryModel.setCreateDepartmentId(oldFactoryModel.getCreateDepartmentId());
			factoryModel.setCreatePositionId(oldFactoryModel.getCreatePositionId());
			factoryModel.setCreateStaffId(oldFactoryModel.getCreateStaffId());
			factoryModel.setOwnerDepartmentId(oldFactoryModel.getOwnerDepartmentId());
			factoryModel.setOwnerPositionId(oldFactoryModel.getOwnerPositionId());
			factoryModel.setOwnerStaffId(oldFactoryModel.getOwnerStaffId());
			factoryModel.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryModel_FactoryModel", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/burdenWareEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "burdenWareEdit.ftl") }),
		@Action(value = "/MESBasic/factoryModel/factoryModel/burdenWareEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "burdenWareEditPrint.ftl")})
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "factoryEdit.ftl") }),
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "factoryEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=factoryModelService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return factoryModelReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			factoryModel = factoryModelService.getFactoryModel(this.factoryModel.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,factoryModel.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "factoryView.ftl") }),
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "factoryViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryList", results = { @Result(type = FREEMARKER, location = "factoryList.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryListPart", results = { @Result(type = FREEMARKER, location = "factoryListPart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryChoiceRef", results = { @Result(type = FREEMARKER, location = "factoryChoiceRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryTreeList", results = { @Result(type = FREEMARKER, location = "factoryTreeList.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryWorkShopRef", results = { @Result(type = FREEMARKER, location = "factoryWorkShopRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factroyRef1", results = { @Result(type = FREEMARKER, location = "factroyRef1.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factroyRefMulty", results = { @Result(type = FREEMARKER, location = "factroyRefMulty.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/powerContChoiceRef", results = { @Result(type = FREEMARKER, location = "powerContChoiceRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/suitedLineChoiceRef", results = { @Result(type = FREEMARKER, location = "suitedLineChoiceRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/workCenterRef", results = { @Result(type = FREEMARKER, location = "workCenterRef.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/factoryModel/factoryModel/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = factoryModelService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(factoryModelService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(factoryModelService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryModel_FactoryModel", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryChoiceTreeRef", results = { @Result(type = FREEMARKER, location = "factoryChoiceTreeRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryMSelectPart", results = { @Result(type = FREEMARKER, location = "factoryMSelectPart.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryMSelectRef", results = { @Result(type = FREEMARKER, location = "factoryMSelectRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryProBatchRef", results = { @Result(type = FREEMARKER, location = "factoryProBatchRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryProductRef", results = { @Result(type = FREEMARKER, location = "factoryProductRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryRefPart", results = { @Result(type = FREEMARKER, location = "factoryRefPart.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryTreePart", results = { @Result(type = FREEMARKER, location = "factoryTreePart.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryTreeRef", results = { @Result(type = FREEMARKER, location = "factoryTreeRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryTreeVPart", results = { @Result(type = FREEMARKER, location = "factoryTreeVPart.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryWorkShop", results = { @Result(type = FREEMARKER, location = "factoryWorkShop.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/powderContainerRef", results = { @Result(type = FREEMARKER, location = "powderContainerRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/suitedLineSelectRef", results = { @Result(type = FREEMARKER, location = "suitedLineSelectRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/suitLineRef", results = { @Result(type = FREEMARKER, location = "suitLineRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/workCenterPartRef", results = { @Result(type = FREEMARKER, location = "workCenterPartRef.ftl") })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/workCenterTreeRef", results = { @Result(type = FREEMARKER, location = "workCenterTreeRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.description,result.nodeType.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="factoryList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryListPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.area.id,result.area.value,result.description,result.isEbr,result.department.name,result.assignBatchNo.itemName,result.endItem.itemName,result.nodeType.id,result.department.id,result.assignBatchNo.id,result.endItem.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="factoryListPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"factoryModel\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"factoryModel\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"department\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"factoryModel\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"factoryModel\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			factoryModelService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryListPart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryMSelectPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.description,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="factoryMSelectPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryMSelectRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.area.id,result.area.value,result.description,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="factoryMSelectRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryProBatchRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.nodeType.name,result.description,result.isEbr,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="factoryProBatchRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryProductRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.nodeType.name,result.description,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="factoryProductRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryRefPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.area.id,result.area.value,result.description,result.isEbr,result.department.name,result.assignBatchNo.itemName,result.endItem.itemName,result.nodeType.id,result.department.id,result.assignBatchNo.id,result.endItem.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="factoryRefPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/factoryWorkShop-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.description,result.nodeType.code,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="factoryWorkShop-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/powderContainerRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.area.id,result.area.value,result.description,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="powderContainerRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/suitedLineSelectRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.area.id,result.area.value,result.description,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="suitedLineSelectRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/suitLineRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.area.id,result.area.value,result.description,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="suitLineRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/factoryModel/factoryModel/workCenterPartRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.code,result.nodeType.name,result.area.id,result.area.value,result.description,result.nodeType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="workCenterPartRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryChoiceTreeRefDrag")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreePartDrag")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeRefDrag")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeVPartDrag")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/workCenterTreeRefDrag")
	})
	public String viewTreeDrag() throws Exception{
		factoryModelService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/MESBasic/factoryModel/factoryModel/burdenWareEditSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryChoiceRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryChoiceTreeRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryEditSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryListSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryListPartSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryMNESort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryMSelectPartSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryMSelectRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryProBatchRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryProductRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryRefPartSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeListSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreePartSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeVPartSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryViewSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryWorkShopSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryWorkShopRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factroyRef1Sort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factroyRefMultySort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/powderContainerRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/powerContChoiceRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/suitedLineChoiceRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/suitedLineSelectRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/suitLineRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/workCenterPartRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/workCenterRefSort")
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/workCenterTreeRefSort")
	})
	public String viewTreeSort() throws Exception{
		if(sortResult!=null && sortResult.length()>0){
			...
			for(String sort : sorts){
				if(sort!=null && sort.length()>0){
					...
					factoryModel.setSort(s);
					factoryModelService.saveFactoryModel(factoryModel, null,null,null);
				}
			}
		}
		return null;
	...
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryChoiceTreeRefData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreePartData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeRefData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeVPartData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/workCenterTreeRefData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeData() throws Exception{
		if(params == null){
			params = new ArrayList<Param>();
		}		
		String viewCode = request.getAttribute("ViewCode").toString();
		...
		if (ViewServiceFoundation.getView(viewCode) != null
				&& ViewServiceFoundation.getView(viewCode).getEntity() != null
				&& !ViewServiceFoundation.getView(viewCode).getEntity().getCrossCompanyFlag()
				&& !getCurrentCompanyId().equals(1L) && !isSingleMode) {
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"factoryModel\".", "{alias}.");
					}
		...
		bapBeforeCustomMethod(viewCode, viewName+"Data");
		factoryModels = factoryModelService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
		bapAfterCustomMethod(viewCode, viewName+"Data");
		return SUCCESS;
	}
	...
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryChoiceTreeRefFullData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreePartFullData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeRefFullData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/factoryTreeVPartFullData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/factoryModel/factoryModel/workCenterTreeRefFullData",
			results = { @Result(type = JSON, params = { "root", "factoryModels", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		String viewCode=request.getAttribute("ViewCode").toString();
		String viewName=request.getAttribute("ViewName").toString();
		Map<Long, String> treeInfo = new LinkedHashMap<Long, String>();
		bapBeforeCustomMethod(viewCode, viewName+"FullData");
		if(ids != null && ids.length() > 0) {
			...
			factoryModels = factoryModelService.getTreeInfo(treeInfo, null, null);
		}
		bapAfterCustomMethod(viewCode, viewName+"FullData");
		return SUCCESS;
	}
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/factoryModel/factoryModel/allTempleteDownload", results = { @Result(name="excel",type="excel",location="factoryList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_factoryModel_FactoryModel");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		factoryModelService.findDataGridPage(dg,dgclass,getDgPage(),factoryModel,"", null);
	...
	private Page<MESBasicObjWareStore> dg1490059032286Page;
	public Page<MESBasicObjWareStore> getDg1490059032286Page(){
		if(null == dg1490059032286Page){
			dg1490059032286Page = new Page<MESBasicObjWareStore>(1,20);
		}
		return dg1490059032286Page;
	}
	
	private Class dg1490059032286Clazz = MESBasicObjWareStore.class;
	
	public Class getDg1490059032286Clazz() {
		return dg1490059032286Clazz;
	}
	
	private List<MESBasicObjWareStore> dg1490059032286List;
	public List<MESBasicObjWareStore> getDg1490059032286List(){
		if(null == dg1490059032286List){
			dg1490059032286List = new ArrayList<MESBasicObjWareStore>();
		}
		return dg1490059032286List;
	}
	public void setDg1490059032286ListJson(String json) {
		//dg1490059032286List = (List<MESBasicObjWareStore>) factoryModelService.generateObjectFromJson(json, MESBasicObjWareStore.class);
	}
	
	private List<Long> dg1490059032286DeletedIds;
	public List<Long> getDg1490059032286DeletedIds(){
		if(null == dg1490059032286DeletedIds){
			dg1490059032286DeletedIds = new ArrayList<Long>();
		}
		return dg1490059032286DeletedIds;
	}

	private Page<MESBasicObjPostion> dg1460955592809Page;
	public Page<MESBasicObjPostion> getDg1460955592809Page(){
		if(null == dg1460955592809Page){
			dg1460955592809Page = new Page<MESBasicObjPostion>(1,20);
		}
		return dg1460955592809Page;
	}
	
	private Class dg1460955592809Clazz = MESBasicObjPostion.class;
	
	public Class getDg1460955592809Clazz() {
		return dg1460955592809Clazz;
	}
	
	private List<MESBasicObjPostion> dg1460955592809List;
	public List<MESBasicObjPostion> getDg1460955592809List(){
		if(null == dg1460955592809List){
			dg1460955592809List = new ArrayList<MESBasicObjPostion>();
		}
		return dg1460955592809List;
	}
	public void setDg1460955592809ListJson(String json) {
		//dg1460955592809List = (List<MESBasicObjPostion>) factoryModelService.generateObjectFromJson(json, MESBasicObjPostion.class);
	}
	
	private List<Long> dg1460955592809DeletedIds;
	public List<Long> getDg1460955592809DeletedIds(){
		if(null == dg1460955592809DeletedIds){
			dg1460955592809DeletedIds = new ArrayList<Long>();
		}
		return dg1460955592809DeletedIds;
	}

	private Page<MESBasicObjEquipment> dg1460958585675Page;
	public Page<MESBasicObjEquipment> getDg1460958585675Page(){
		if(null == dg1460958585675Page){
			dg1460958585675Page = new Page<MESBasicObjEquipment>(1,20);
		}
		return dg1460958585675Page;
	}
	
	private Class dg1460958585675Clazz = MESBasicObjEquipment.class;
	
	public Class getDg1460958585675Clazz() {
		return dg1460958585675Clazz;
	}
	
	private List<MESBasicObjEquipment> dg1460958585675List;
	public List<MESBasicObjEquipment> getDg1460958585675List(){
		if(null == dg1460958585675List){
			dg1460958585675List = new ArrayList<MESBasicObjEquipment>();
		}
		return dg1460958585675List;
	}
	public void setDg1460958585675ListJson(String json) {
		//dg1460958585675List = (List<MESBasicObjEquipment>) factoryModelService.generateObjectFromJson(json, MESBasicObjEquipment.class);
	}
	
	private List<Long> dg1460958585675DeletedIds;
	public List<Long> getDg1460958585675DeletedIds(){
		if(null == dg1460958585675DeletedIds){
			dg1460958585675DeletedIds = new ArrayList<Long>();
		}
		return dg1460958585675DeletedIds;
	}

	private Page<MESBasicObjProduct> dg1460958766002Page;
	public Page<MESBasicObjProduct> getDg1460958766002Page(){
		if(null == dg1460958766002Page){
			dg1460958766002Page = new Page<MESBasicObjProduct>(1,20);
		}
		return dg1460958766002Page;
	}
	
	private Class dg1460958766002Clazz = MESBasicObjProduct.class;
	
	public Class getDg1460958766002Clazz() {
		return dg1460958766002Clazz;
	}
	
	private List<MESBasicObjProduct> dg1460958766002List;
	public List<MESBasicObjProduct> getDg1460958766002List(){
		if(null == dg1460958766002List){
			dg1460958766002List = new ArrayList<MESBasicObjProduct>();
		}
		return dg1460958766002List;
	}
	public void setDg1460958766002ListJson(String json) {
		//dg1460958766002List = (List<MESBasicObjProduct>) factoryModelService.generateObjectFromJson(json, MESBasicObjProduct.class);
	}
	
	private List<Long> dg1460958766002DeletedIds;
	public List<Long> getDg1460958766002DeletedIds(){
		if(null == dg1460958766002DeletedIds){
			dg1460958766002DeletedIds = new ArrayList<Long>();
		}
		return dg1460958766002DeletedIds;
	}

	private Page<MESBasicObjTeam> dg1490261761014Page;
	public Page<MESBasicObjTeam> getDg1490261761014Page(){
		if(null == dg1490261761014Page){
			dg1490261761014Page = new Page<MESBasicObjTeam>(1,20);
		}
		return dg1490261761014Page;
	}
	
	private Class dg1490261761014Clazz = MESBasicObjTeam.class;
	
	public Class getDg1490261761014Clazz() {
		return dg1490261761014Clazz;
	}
	
	private List<MESBasicObjTeam> dg1490261761014List;
	public List<MESBasicObjTeam> getDg1490261761014List(){
		if(null == dg1490261761014List){
			dg1490261761014List = new ArrayList<MESBasicObjTeam>();
		}
		return dg1490261761014List;
	}
	public void setDg1490261761014ListJson(String json) {
		//dg1490261761014List = (List<MESBasicObjTeam>) factoryModelService.generateObjectFromJson(json, MESBasicObjTeam.class);
	}
	
	private List<Long> dg1490261761014DeletedIds;
	public List<Long> getDg1490261761014DeletedIds(){
		if(null == dg1490261761014DeletedIds){
			dg1490261761014DeletedIds = new ArrayList<Long>();
		}
		return dg1490261761014DeletedIds;
	}

	@Actions({
		
		@Action(value = "/MESBasic/factoryModel/factoryModel/data-dg1490059032286", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.ware.wareCode,result.ware.wareName,result.store.placeSetCode,result.store.placeSetName,result.product.productCode,result.product.productName,result.ware.cargoStatue.id,result.ware.cargoStatue.value,result.store.wareId.wareCode,result.id,result.version,result.store.id,result.product.id,result.ware.id,result.objFactory.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1490059032286-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1490059032286.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/data-dg1460955592809", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.objPos.name,result.objPos.code,result.objPos.department.name,result.objPos.department.fullPathName,result.id,result.version,result.objPos.id,result.objFactory.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1460955592809-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1460955592809.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/data-dg1460958585675", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.objDepartment.name,result.isFixed,result.equipmentNum,result.objEqu.name,result.equipmentType.id,result.equipmentType.value,result.isContainer,result.clearType.id,result.clearType.value,result.clearItem.itemName,result.id,result.version,result.clearItem.id,result.objDepartment.id,result.objEqu.id,result.objFactory.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1460958585675-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1460958585675.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/data-dg1460958766002", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.objProduct.productName,result.objProduct.productCode,result.objProduct.productSpecif,result.emsFlag,result.id,result.version,result.objProduct.id,result.objFactory.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1460958766002-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1460958766002.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/MESBasic/factoryModel/factoryModel/data-dg1490261761014", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.objTeam.name,result.objTeam.code,result.objTeam.dept.name,result.objTeam.principal.name,result.id,result.version,result.objTeam.id,result.objFactory.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1490261761014-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1490261761014.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(factoryModel.getId() != null && factoryModel.getId() != -1){
				factoryModel = factoryModelService.getFactoryModel(factoryModel.getId());
			}
			factoryModelService.findDataGridPage(dg,dgclass,getDgPage(),factoryModel,searchCondition,params);
	...
	private void save() throws Exception {
		...
		factoryModelService.saveFactoryModel(factoryModel,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/factoryModel/factoryModel/burdenWareEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/factoryModel/factoryModel/factoryEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/factoryModel/factoryModel/factoryView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "submit");
		bap_validate_datagrids=factoryModelService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", factoryModel.getId());
		...
		fileuploads.put("linkId", factoryModel.getId());
		bapAfterCustomMethod("MESBasic_1_factoryModel_FactoryModel", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryModel/factoryModel/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		factoryModelService.deleteFactoryModel(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryModel_FactoryModel", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/factoryModel/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "restore");
		factoryModelService.restoreFactoryModel(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryModel_FactoryModel", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/factoryModel/factoryModel/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/factoryModel/factoryModel/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "showDealInfoData");
		if (null != tableInfoId) {
			int count=factoryModelService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = factoryModelService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = factoryModelService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_FactoryModel", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicFactoryModel> findExportDataInfos(Page<MESBasicFactoryModel> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = factoryModelService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = factoryModelService.getAssProperty(p.getCode());	
									//String assModelCode = factoryModelService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
				...
				if(view.getAssModel().getDataType() == 2 && useForImportFlag){//树型结构且用于导出，加上父节点,同时在sql中加入layNo,layRec,parentId,sort,leaf
					String[] exportSqls = exportSql.split("FROM");
					String exportSqlPart1 = exportSqls[0];
					String exportSqlPart2 = exportSqls[1];
					String newExportSql = "";
					String sqlModelName = view.getAssModel().getModelName().substring(0, 1).toLowerCase() + view.getAssModel().getModelName().substring(1);
					...
					if(exportMap.get("businessKey") != null){
						Property busKey = (Property) exportMap.get("businessKey");
						Map<String, String> m = new HashMap<String, String>();
						m.put("showWidth", "140");
						m.put("nullable", "true");
						m.put("dispalyName", "父节点." + InternationalResource.get(busKey.getDisplayName(), language) );
						...
						Map<String, String> m1 = new HashMap<String, String>();
						m1.put("key", "parentNodeBusinessKey");
					...
					if(exportMap.get("mainDisplay")!=null && exportMap.get("mainDisplay") != exportMap.get("businessKey")){
						Property mainDis = (Property) exportMap.get("mainDisplay");
						Map<String, String> m2 = new HashMap<String, String>();
						m2.put("showWidth", "140");
						m2.put("nullable", "true");
						m2.put("dispalyName", "父节点." + InternationalResource.get(mainDis.getDisplayName(), language));
						...
						m3.put("key", "parentNodeMainDisplay");
		...
		if(null != searchObjects && searchObjects.length > 0) {
			factoryModelService.findFactoryModels(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			factoryModelService.findFactoryModels(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicFactoryModel> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicFactoryModel result : results){
	...
	public boolean isEditNew(){
		//return null == this.factoryModel || null == this.factoryModel.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryModel/factoryModel/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_FactoryModel", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryModel/factoryModel/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryModel/factoryModel/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/factoryModel/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicFactoryModelService"
						, "com.supcon.orchid.MESBasic.services.MESBasicObjEquipmentService"      
						, "com.supcon.orchid.MESBasic.services.MESBasicObjWareStoreService"
						...
						, "com.supcon.orchid.MESBasic.services.MESBasicObjPostionService"
						, "com.supcon.orchid.foundation.services.PositionService"
						, "com.supcon.orchid.foundation.services.DepartmentService"
						, "com.supcon.orchid.MESBasic.services.MESBasicObjEquipmentService"
						, "com.supcon.orchid.MESBasic.services.MESBasicEquipmentService"
						, "com.supcon.orchid.MESBasic.services.MESBasicItemSetupService"
						, "com.supcon.orchid.MESBasic.services.MESBasicObjProductService"
						, "com.supcon.orchid.MESBasic.services.MESBasicObjTeamService"
						, "com.supcon.orchid.X6Basic.services.X6BasicTeamInfoService"
						, "com.supcon.orchid.foundation.services.StaffService"
					, "com.supcon.orchid.MESBasic.services.MESBasicObjBatContainerService"
					, "com.supcon.orchid.MESBasic.services.MESBasicObjBatContainerService"
					, "com.supcon.orchid.MESBasic.services.MESBasicObjBatContainerService"
					, "com.supcon.orchid.MESBasic.services.MESBasicObjBatContainerService"
					, "com.supcon.orchid.MESBasic.services.MESBasicObjBatContainerService"
					...
					, "com.supcon.orchid.MESBasic.services.MESBasicObjWareStoreService"
					, "com.supcon.orchid.MESBasic.services.MESBasicObjWareStoreService"
					, "com.supcon.orchid.MESBasic.services.MESBasicPositionDealService"
					, "com.supcon.orchid.MESBasic.services.MESBasicPositionDealService"
					, "com.supcon.orchid.MESBasic.services.MESBasicPositionDealService"
					, "com.supcon.orchid.MESBasic.services.MESBasicPositionDealService"
					, "com.supcon.orchid.MESBasic.services.MESBasicPositionDealService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_factoryModel", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_factoryModel");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_FactoryModel", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/factoryModel/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicFactoryModelService"
						, "com.supcon.orchid.MESBasic.services.MESBasicItemSetupService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicNodeTypeService"       
						, "com.supcon.orchid.foundation.services.DepartmentService"       
						, "com.supcon.orchid.foundation.services.PositionService"       
						, "com.supcon.orchid.foundation.services.StaffService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicObjEquipmentService"      
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryModel_FactoryModel", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryModel_FactoryModel", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryModel/factoryModel/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryModel/factoryModel/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryModel_FactoryModel";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryModel_FactoryModel", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryModel/factoryModel/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == factoryModel.getCode() ? "" : factoryModel.getCode().toString());
		_auditLog.setColumnStringA(null == factoryModel.getName() ? "" : factoryModel.getName().toString());
		_auditLog.setColumnStringC(factoryModel.getId().toString());
		_auditLog.setColumnStringD(MESBasicFactoryModel.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_factoryModel");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicFactoryModelAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, factoryModel,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, factoryModel,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,factoryModel);
		...
		printlog.setEntity("MESBasic_1_factoryModel");
		...
		printlog.setBusinessId(factoryModel.getId());
		printlog.setBussinessKey(null == factoryModel.getCode() ? "" : factoryModel.getCode().toString());
		printlog.setMainDisplay(null == factoryModel.getName() ? "" : factoryModel.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryModel/factoryModel/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(factoryModel,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/factoryModel/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(factoryModelService.getFactoryModel(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryModel/factoryModel/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryModel/factoryModel/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, factoryModel,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				factoryModel = factoryModelService.getFactoryModel(Long.parseLong(arrId[i]));
				if(null != factoryModel){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == factoryModel.getName() ? "" : "," + factoryModel.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == factoryModel.getName() ? "" : factoryModel.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == factoryModel.getCode() ? "" : "," + factoryModel.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == factoryModel.getCode() ? "" : factoryModel.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == factoryModel.getId() ? "" : "," + factoryModel.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == factoryModel.getId() ? "" : factoryModel.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, factoryModel,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,factoryModel);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/factoryModel/factoryModel/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicFactoryModel> findExportPageInfos(Page<MESBasicFactoryModel> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_factoryModel" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = factoryModelService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicFactoryModel queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = factoryModelService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = factoryModelService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_FactoryModel");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryModel_FactoryModel")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryModel_FactoryModel");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_FactoryModel");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = factoryModelService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryModel_FactoryModel,MESBasic_1) */
// 自定义代码
        //配方-生产线关联参照
        if ("MESBasic_1_factoryModel_factoryProBatchRef".equals(code) && "factoryProBatchRefQuery".equals(methodName)) { //根据物品过滤数据
            if (formulaId != null && !"".equals(formulaId)) {
                String customerSql = "( \"factoryModel\".NODE_TYPE in(select ID from factory_tbnodetype where ( code = '004')) and  \"factoryModel\".ID in(select LINE_ID from RM_LINE_FORMULAS where ( FORMULA_ID in(select ID from RM_FORMULAS where ( ID = " + formulaId + " and valid = 1)) and valid = 1)))";
                params.add(new Param("customerSql", customerSql));
            } else {
                String customerSql = "( \"factoryModel\".NODE_TYPE in(select ID from factory_tbnodetype where ( code = '004' )))";
                params.add(new Param("customerSql", customerSql));
            }
        }
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryModel_FactoryModel,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryModel_FactoryModel,MESBasic_1) */
// 自定义代码
//节点类型
    ...
    @Action(value = "/MESBasic/factoryModel/factoryModel/checkNodeType", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String checkNodeType() {
        String result = factoryModelService.checkFatherNodeType(nodeType, parentId);
        responseMap.put("result", result);
        return SUCCESS;
    }

    @Action(value = "/MESBasic/factoryModel/factoryModel/checkEditNodeType", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String checkEditNodeType() {
        String result = factoryModelService.checkSonNodeType(nodeType, factoryId);
        responseMap.put("result", result);
        return SUCCESS;
    }

    @Action(value = "/MESBasic/factoryModel/factoryModel/emaChange", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String emaChange() {
        boolean hasEmsFlag = false;
        MESBasicFactoryModel facModel = factoryModelService.getFactoryModel(Long.parseLong(factoryId));
        List<MESBasicObjProduct> objProductList = objProductService.findByProperty("objFactory", facModel);
        for (MESBasicObjProduct mesBasicObjProduct : objProductList) {
            if (mesBasicObjProduct.getEmsFlag() == true) {
                hasEmsFlag = true;
                break;
            }
        }
        responseMap.put("hasEmsFlag", hasEmsFlag);
        return SUCCESS;
    }

    @Action(value = "/public/MESBasic/factoryModel/factoryModel/CheckDescA", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String CheckDescA() {
        String result = factoryModelService.getSystemCodeFullPathNameByDescA(equipmentTypeId);
        log.info("qqqqwwww" + result);
        responseMap.put("result", result);
        return SUCCESS;
    }

    @Action(value = "/public/MESBasic/factoryModel/factoryModel/GetParentNode", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String GetParentNode() {
        //当【生产区域】字段有值时带出部门对象的ID以及Name，并存放到form表单当中
        List<Object[]> result = factoryModelService.getParentNodeDepartment(parentId);
        //取父节点的【生产区域】字段，若有值则存放到form表单当中
        String workArea = factoryModelService.getParentNodeWorkArea(parentId);
        responseMap.put("workArea", workArea);
        for (Object[] objs : result) {
            String departmentId = String.valueOf(objs[0]);
            String departmentName = String.valueOf(objs[1]);
    ...
    @Action(value = "/public/MESBasic/factoryModel/factoryModel/GetBatchMode", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String GetBatchMode() {
        List<Object[]> BatchModeList = factoryModelService.getBatchMode();
        //判断是否上载批次模块
        int BatchModeCount = BatchModeList.size();
        List<Object> runProSetNum = null;
        if (BatchModeCount > 0 && factoryId != null) {
            runProSetNum = factoryModelService.getEnabled(factoryId);
        }
        responseMap.put("runProSetNum", (runProSetNum != null && runProSetNum.size() > 0) ? runProSetNum.size() : 0);
        responseMap.put("BatchModeCount", BatchModeCount);
        return SUCCESS;
    }

    public String objEquId;

    public String getObjEquId() {
        return objEquId;
    }

    public void setObjEquId(String objEquId) {
        this.objEquId = objEquId;
    }

    @Action(value = "/public/MESBasic/factoryModel/factoryModel/checkObjEquIsUsedByProRootSet", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String checkObjEquIsUsedByProRootSet() {
        List<Object> runProSetNum = factoryModelService.checkObjEquIsUsedByProRootSet(objEquId);
        responseMap.put("runProSetNum", (runProSetNum != null && runProSetNum.size() > 0) ? runProSetNum.size() : 0);
        return SUCCESS;
    }

    @Action(value = "/MESBasic/factoryModel/factoryModel/modifyActiveByProRootSetChecked", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String ModifyedCheckActiveByProRootSet() {
        //查询该工作单元是否被活动集合引用
            List<Object> avtivemodifyList = null;
            avtivemodifyList = factoryModelService.modifyActiveByProRootSetChecked(factoryId);
            responseMap.put("activeCount", (avtivemodifyList != null && avtivemodifyList.size() > 0) ? avtivemodifyList.size() : 0);
            if (avtivemodifyList.size()==0){
                avtivemodifyList.add(0);
            }
            responseMap.put("active", avtivemodifyList.get(0));
            return SUCCESS;

    }

    @Action(value = "/public/MESBasic/factoryModel/factoryModel/delEquipCheck", results = {@Result(type = JSON, params = {
            "root", "responseMap"})})
    public String delEquipCheck() {
        List<Object[]> BatchModeList = factoryModelService.getBatchMode();
        //判断是否上载批次模块
        int BatchModeCount = BatchModeList.size();
        List<Object> delInfo = null;
        if (BatchModeCount>0) {
            //查询删除工厂设备是否被工艺路线引用
            delInfo = factoryModelService.checkObjEquIsByProRootSet(objEquId);
        }
        responseMap.put("BatchModeCount",BatchModeCount);
        responseMap.put("runProSetNum", (delInfo != null && delInfo.size() > 0) ? delInfo.size() : 0);
        return SUCCESS;
    }
/* CUSTOM CODE END */
}