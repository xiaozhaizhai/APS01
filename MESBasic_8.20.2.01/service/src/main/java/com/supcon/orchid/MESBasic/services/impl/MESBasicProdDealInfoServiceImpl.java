...
import com.supcon.orchid.MESBasic.services.MESBasicProdDealInfoImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicProdDealInfoDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_prodDealInfoService")
@Transactional
public class MESBasicProdDealInfoServiceImpl  extends BaseServiceImpl<MESBasicProdDealInfo> implements MESBasicProdDealInfoService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicProdDealInfo> bapBaseService;
	...
	@Autowired
	private MESBasicProdDealInfoImportService prodDealInfoImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicProdDealInfo.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  prodDealInfoDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicProdDealInfo.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = prodDealInfoDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdDealInfo.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdDealInfo.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  prodDealInfoDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicProdDealInfo.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicProdDealInfo.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicProdDealInfo> list = (List<MESBasicProdDealInfo>) prodDealInfoDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicProdDealInfo getProdDealInfo(long id){
		return getProdDealInfo(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdDealInfo getProdDealInfo(long id, String viewCode){
		MESBasicProdDealInfo prodDealInfo = prodDealInfoDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProdDealInfoAsJSON(long id, String include){
		MESBasicProdDealInfo prodDealInfo = prodDealInfoDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(prodDealInfo, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteProdDealInfo(MESBasicProdDealInfo prodDealInfo){
		if(null != prodDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
			AuditUtil.setColumnStringC(prodDealInfo.getId().toString());
		}
		beforeDeleteProdDealInfo(prodDealInfo);
		prodDealInfoDao.deletePhysical(prodDealInfo);
		afterDeleteProdDealInfo(prodDealInfo);
		prodDealInfoDao.flush();
		List<MESBasicProdDealInfo> params = new ArrayList<MESBasicProdDealInfo>();
		params.add(prodDealInfo);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteProdDealInfo(List<Long> prodDealInfoIds){
		deleteProdDealInfo(prodDealInfoIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteProdDealInfo(List<Long> prodDealInfoIds, String eventTopic) {
		List<MESBasicProdDealInfo> prodDealInfos = new ArrayList<MESBasicProdDealInfo>();
		for(Long prodDealInfoId : prodDealInfoIds){
			MESBasicProdDealInfo prodDealInfo = getProdDealInfo(prodDealInfoId);
			prodDealInfos.add(prodDealInfo);
			if(null != prodDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == prodDealInfo.getId() ? "" : "," + prodDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == prodDealInfo.getId() ? "" : "," + prodDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == prodDealInfo.getId() ? "" : "," + prodDealInfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(prodDealInfos != null && prodDealInfos.size() > 0 ){
			for(MESBasicProdDealInfo prodDealInfo : prodDealInfos){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicProdDealInfo.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(prodDealInfo));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),prodDealInfo, prodDealInfo.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(prodDealInfoIds != null && prodDealInfoIds.size() > 0) {
			String hql = "delete " + MESBasicProdDealInfo.JPA_NAME + " where id in(:ids)";
			Query query = prodDealInfoDao.createQuery(hql);
			query.setParameterList("ids", prodDealInfoIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicProdDealInfo prodDealInfoz : prodDealInfos){
				prodDealInfoz.setValid(false);
				List<MESBasicProdDealInfo> params = new ArrayList<MESBasicProdDealInfo>();
				params.add(prodDealInfoz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_product");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteProdDealInfo(long prodDealInfoId, int prodDealInfoVersion){
		this.deleteProdDealInfo(prodDealInfoId, prodDealInfoVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteProdDealInfo(long prodDealInfoId, int prodDealInfoVersion,SignatureLog signatureLog){
		MESBasicProdDealInfo prodDealInfo = getProdDealInfo(prodDealInfoId);
		...
		beforeDeleteProdDealInfo(prodDealInfo);
		prodDealInfoDao.delete(prodDealInfoId, prodDealInfoVersion);
		afterDeleteProdDealInfo(prodDealInfo);
		prodDealInfoDao.flush();
		List<MESBasicProdDealInfo> params = new ArrayList<MESBasicProdDealInfo>();
		params.add(prodDealInfo);
		...
		if(signatureLog != null ) {
			...
			businessKey=prodDealInfo.getId();
			signatureLog.setTableId(prodDealInfo.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteProdDealInfo(String prodDealInfoIds){
		this.deleteProdDealInfo(prodDealInfoIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.common.delete",operType="3")
	public void deleteProdDealInfo(String prodDealInfoIds,SignatureLog signatureLog){
		deleteCollection(prodDealInfoIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProdDealInfo(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_product#modelCode:MESBasic_1_product_ProdDealInfo#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="ec.engine.restore",operType="10")
	public void restoreProdDealInfo(String prodDealInfoIds){
		restoreCollection(prodDealInfoIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProdDealInfo(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="ec.engine.restore",operType="10")
	public void restoreProdDealInfo(long prodDealInfoId){
		
		findBusinessKeyUsed(prodDealInfoId);	//判断业务主键是否重复
		...
		beforeRestoreProdDealInfo(prodDealInfo);
		prodDealInfo.setValid(true);
		prodDealInfoDao.update(prodDealInfo);
		afterRestoreProdDealInfo(prodDealInfo);
		List<MESBasicProdDealInfo> params = new ArrayList<MESBasicProdDealInfo>();
		params.add(prodDealInfo);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long prodDealInfoId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_product_ProdDealInfo");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicProdDealInfo.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicProdDealInfo.TABLE_NAME +" where id = ? )";
			List<Object> list =  prodDealInfoDao.createNativeQuery(sql,prodDealInfoId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProdDealInfo(List<MESBasicProdDealInfo>  prodDealInfos){
		for(MESBasicProdDealInfo prodDealInfo:prodDealInfos)  {
			saveProdDealInfo(prodDealInfo, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProdDealInfo(List<MESBasicProdDealInfo>  prodDealInfos){
		for(MESBasicProdDealInfo prodDealInfo:prodDealInfos)  {
			saveProdDealInfo(prodDealInfo, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="ec.flow.save")
	public void saveProdDealInfo(MESBasicProdDealInfo prodDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProdDealInfo(prodDealInfo, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="ec.flow.save")
	public void saveProdDealInfo(MESBasicProdDealInfo prodDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProdDealInfo(prodDealInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProdDealInfoCodes(MESBasicProdDealInfo prodDealInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdDealInfoCodes(prodDealInfo, false);
	}
	
	public void generateProdDealInfoCodes(MESBasicProdDealInfo prodDealInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProdDealInfoSummarys(MESBasicProdDealInfo prodDealInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdDealInfoSummarys(prodDealInfo, false);
	}

	public void generateProdDealInfoSummarys(MESBasicProdDealInfo prodDealInfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo")
	public void saveProdDealInfo(MESBasicProdDealInfo prodDealInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProdDealInfo(prodDealInfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo")
	public void saveProdDealInfo(MESBasicProdDealInfo prodDealInfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(prodDealInfo.getId() != null && prodDealInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, prodDealInfo);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(prodDealInfo);
		beforeSaveProdDealInfo(prodDealInfo, viewIsView);

		if (viewIsView) {
			prodDealInfoDao.saveWithRevertVersion(prodDealInfo);
		} else {
			if(isNew)
				prodDealInfoDao.save(prodDealInfo);
			else
				prodDealInfoDao.merge(prodDealInfo);
		}
		if(null != prodDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
		...
		dealDatagridsSave(prodDealInfo,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProdDealInfoCodes(prodDealInfo, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProdDealInfoSummarys(prodDealInfo, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProdDealInfo(prodDealInfo, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, prodDealInfo);
			}
		}
		List<MESBasicProdDealInfo> params = new ArrayList<MESBasicProdDealInfo>();
		prodDealInfoDao.flush();
		prodDealInfoDao.clear();
		prodDealInfo = prodDealInfoDao.load(prodDealInfo.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicProdDealInfo prodDealInfo){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("prodDealInfo", prodDealInfo);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo")
	public void mergeProdDealInfo(MESBasicProdDealInfo prodDealInfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(prodDealInfo);
		beforeSaveProdDealInfo(prodDealInfo);
		prodDealInfoDao.merge(prodDealInfo);
		if(null != prodDealInfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
			AuditUtil.setColumnStringA(null == prodDealInfo.getId() ? "" : prodDealInfo.getId().toString());
		...
		dealDatagridsSave(prodDealInfo,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProdDealInfo(prodDealInfo);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicProdDealInfo> findProdDealInfos(Page<MESBasicProdDealInfo> page, Criterion... criterions) {
		return prodDealInfoDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdDealInfos(Page<MESBasicProdDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProdDealInfos(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdDealInfos(Page<MESBasicProdDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdDealInfos(Page<MESBasicProdDealInfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicProdDealInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"prodDealInfo\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"prodDealInfo\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"prodDealInfo\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"prodDealInfo\".PDI_ASSIGNID IN ( SELECT PRODUCT_ID FROM S2BASE_PRODUCT \"product\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  prodDealInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  prodDealInfoDao.createNativeQuery(powerOperateSql, "MESBasic_1_product").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"prodDealInfo\".LAY_REC ASC, \"prodDealInfo\".SORT ASC");
				} else {
					orderPart.append(" \"prodDealInfo\".PDI_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) prodDealInfoDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=prodDealInfoDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = prodDealInfoDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicProdDealInfo> result = new ArrayList<MESBasicProdDealInfo>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdDealInfo.class,prodDealInfoDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdDealInfo.class,prodDealInfoDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicProdDealInfo prodDealInfo : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(prodDealInfo.getId(), "MESBasic_product_prodDealInfo");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(prodDealInfo.getId(), "MESBasic_product_prodDealInfo");
							if(null != documents && !documents.isEmpty()) {
								prodDealInfo.setDocument(documents.get(0));
								prodDealInfo.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicProdDealInfo> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicProdDealInfo> result) {
		View view = null;
		...
		Map<Long, MESBasicProdDealInfo> rsMap = new HashMap<Long, MESBasicProdDealInfo>();
		for (MESBasicProdDealInfo prodDealInfo : result) {
			ids.add(prodDealInfo.getId());
			rsMap.put(prodDealInfo.getId(), prodDealInfo);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(prodDealInfoDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = prodDealInfoDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_product_ProdDealInfo" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_product_ProdDealInfo" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  prodDealInfoDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodDealInfo\"","MESBasic_1_product","","S2BASE_PRODDEALINFO",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"prodDealInfo\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"prodDealInfo\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodDealInfo\"","MESBasic_1_product","","S2BASE_PRODDEALINFO",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_product_ProdDealInfo" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("prodDealInfo.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"prodDealInfo\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"prodDealInfo\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"prodDealInfo\".CID = 1 OR \"prodDealInfo\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("prodDealInfo.mc\"").append(".PROD_DEAL_INFO) not in ("+notinValue+")");
			}
			
			SQLQuery query = prodDealInfoDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=prodDealInfoDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicProdDealInfo.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicProdDealInfo.MODULE_CODE;}
	protected String getModelCode(){return MESBasicProdDealInfo.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param prodDealInfo 模型实例
	 */
	public void checkUniqueConstraint(MESBasicProdDealInfo prodDealInfo){
	}
	
	
	@Override
	public List<MESBasicProdDealInfo> getProdDealInfos(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdDealInfo.class);
		if(params!=null){
			...
			return prodDealInfoDao.findByHql("from " + MESBasicProdDealInfo.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return prodDealInfoDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicProdDealInfo> findProdDealInfosBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdDealInfo.class);
	...
	@Override
	public List<MESBasicProdDealInfo> findProdDealInfosByHql(String hql, Object... objects){
		
		return   prodDealInfoDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicProdDealInfo loadProdDealInfoByBussinessKey(MESBasicProdDealInfo prodDealInfo){
		return loadProdDealInfoByBussinessKey(prodDealInfo.getId());
	}
	
	@Override
	public Page<MESBasicProdDealInfo> getProdDealInfos(Page<MESBasicProdDealInfo> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdDealInfo.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = prodDealInfoDao.findByPage(page, sql);
			}else{
				page = prodDealInfoDao.findAllByPage(page);
			}
			return page;
		}
		page = prodDealInfoDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdDealInfo loadProdDealInfoByBussinessKey(Serializable bussinessKey){
		return prodDealInfoDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdDealInfo> findByProperty(String propertyName, Object object){
		return prodDealInfoDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdDealInfo findEntityByProperty(String propertyName, Object object){
		return prodDealInfoDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicProdDealInfo.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = prodDealInfoDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_product_ProdDealInfo","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicProdDealInfo prodDealInfo,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,prodDealInfoDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicProdDealInfo.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, prodDealInfo);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",prodDealInfo.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdDealInfo.TABLE_NAME +" where valid = 1";
			list = prodDealInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdDealInfo.TABLE_NAME;
			list = prodDealInfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicProdDealInfo.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = prodDealInfoDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicProdDealInfoDaoImpl prodDealInfoDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProdDealInfo(final List<MESBasicProdDealInfo> insertObjs, final List<MESBasicProdDealInfo> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProdDealInfo(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProdDealInfo(final List<MESBasicProdDealInfo> insertObjs, final List<MESBasicProdDealInfo> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1470380231866"),importCount));
		...
		Map<Long, MESBasicProdDealInfo> oldUpdateObjsMap = new HashMap<Long, MESBasicProdDealInfo>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicProdDealInfo bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicProdDealInfo> bizObjList = prodDealInfoDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicProdDealInfo bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicProdDealInfo> nmeObjs = new ArrayList<MESBasicProdDealInfo>();
		...
		long id=autoGeneratorID.getNextId(MESBasicProdDealInfo.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicProdDealInfo prodDealInfo:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicProdDealInfo.TABLE_NAME,1,dbId);
			if(prodDealInfo.getId() != null ){
				m.put(prodDealInfo.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) prodDealInfoDao.createNativeQuery(sql, "MESBasic_1_product_ProdDealInfo"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		prodDealInfoDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicProdDealInfo.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicProdDealInfo prodDealInfo = updateObjs.get(updateIndex);
						Long id = prodDealInfo.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = prodDealInfo.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(prodDealInfo);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicProdDealInfo.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicProdDealInfo.TABLE_NAME + " order by id desc";
//					Query query = prodDealInfoDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicProdDealInfo prodDealInfo = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = prodDealInfo.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		prodDealInfoDaoImpl.getSession().flush();
		prodDealInfoDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicProdDealInfo bizObj = insertObjs.get(insertIndex);
			prodDealInfoImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicProdDealInfo bizObj = updateObjs.get(updateIndex);
			MESBasicProdDealInfo oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			prodDealInfoImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicProdDealInfo> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicProdDealInfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) prodDealInfoDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicProdDealInfo> result = getResult(page, query, new PendingResultTransformer(MESBasicProdDealInfo.class,prodDealInfoDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicProdDealInfo model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = prodDealInfoDao.createNativeQuery(sql, "MESBasic_1_product_ProdDealInfo").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = prodDealInfoDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = prodDealInfoDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = prodDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?prodDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  prodDealInfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,prodDealInfoDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="ec.flow.save")
	public void saveProdDealInfo(MESBasicProdDealInfo prodDealInfo, DataGridService dataGridService){
		saveProdDealInfo(prodDealInfo, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="ec.flow.save")
	public void saveProdDealInfo(MESBasicProdDealInfo prodDealInfo, DataGridService dataGridService, String viewCode){
		saveProdDealInfo(prodDealInfo, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo")
	public void saveProdDealInfo(MESBasicProdDealInfo prodDealInfo, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProdDealInfo(prodDealInfo, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo")
	public void saveProdDealInfo(MESBasicProdDealInfo prodDealInfo, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(null != prodDealInfo.getId() && prodDealInfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdDealInfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo")
	public Map<Object, Object> prodDealInfoDataGridImport(MESBasicProdDealInfo prodDealInfo, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = prodDealInfo.getVirtualId();
		
		prodDealInfo = prodDealInfoDao.load(prodDealInfo.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, prodDealInfo.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = prodDealInfo.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(prodDealInfo);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, prodDealInfo.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo")
	public void mergeProdDealInfo(MESBasicProdDealInfo prodDealInfo, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(prodDealInfo);
		beforeSaveProdDealInfo(prodDealInfo);
		prodDealInfoDao.merge(prodDealInfo);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdDealInfo",desc="foundation.batch.save",operType="4")
	public void batchSaveProdDealInfo(MESBasicProdDealInfo prodDealInfo, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
	...
	@Override
	public Page<MESBasicProdDealInfo> getByPage(Page<MESBasicProdDealInfo> page,DetachedCriteria detachedCriteria){
		return prodDealInfoDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProdDealInfo(MESBasicProdDealInfo prodDealInfo, Object... objects){
		checkUniqueConstraint(prodDealInfo);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProdDealInfo(MESBasicProdDealInfo prodDealInfo, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProdDealInfo(List<MESBasicProdDealInfo> prodDealInfos, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProdDealInfo(List<MESBasicProdDealInfo> prodDealInfos){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProdDealInfo(List<MESBasicProdDealInfo> prodDealInfos, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProdDealInfo(List<MESBasicProdDealInfo> prodDealInfos){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProdDealInfo(MESBasicProdDealInfo prodDealInfo){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProdDealInfo(MESBasicProdDealInfo prodDealInfo){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProdDealInfo(MESBasicProdDealInfo prodDealInfo){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProdDealInfo(MESBasicProdDealInfo prodDealInfo){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicProdDealInfo prodDealInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicProdDealInfo prodDealInfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProdDealInfo(List<MESBasicProdDealInfo> insertObjs, List<MESBasicProdDealInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProdDealInfo,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProdDealInfo(List<MESBasicProdDealInfo> insertObjs, List<MESBasicProdDealInfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProdDealInfo,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_product_ProdDealInfo,MESBasic_1) */
// 自定义代码
