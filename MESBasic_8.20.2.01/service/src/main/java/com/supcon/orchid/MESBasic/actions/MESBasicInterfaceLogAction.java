...
/* CUSTOM CODE START(action,import,MESBasic_1_pimsInterface_InterfaceLog,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicInterfaceLogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicInterfaceLog> {
	...
	private MESBasicInterfaceLog interfaceLog;
	...
	//导出excel时,主模型内容
	List<MESBasicInterfaceLog> queryResults = new ArrayList<MESBasicInterfaceLog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/pimsInterface/interfaceLog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/pimsInterface/interfaceLog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_pimsInterface_InterfaceLog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicInterfaceLog> interfaceLogs;
	
	public List<MESBasicInterfaceLog> getInterfaceLogs() {
	...
	private Page<MESBasicInterfaceLog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			interfaceLog = new MESBasicInterfaceLog();
		} else {
			if(id > 0){
				interfaceLog = interfaceLogService.getInterfaceLog(id, viewCode);
			}
		}
	...
	public Page<MESBasicInterfaceLog> getPage() {
		if (null == page) {
			page = new Page<MESBasicInterfaceLog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicInterfaceLog getInterfaceLog() {
		return interfaceLog;
	}		

	public void setInterfaceLog(MESBasicInterfaceLog interfaceLog) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = interfaceLogService.getInterfaceLogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String interfaceLogReferenceCopy() throws Exception{
			MESBasicInterfaceLog oldInterfaceLog = null;
			if(isEditNew()){
				...
				oldInterfaceLog = new MESBasicInterfaceLog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,interfaceLog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, interfaceLog.getId());
				...
				oldInterfaceLog = interfaceLogService.getInterfaceLog(id);
			}
			
			
			interfaceLog = interfaceLogService.getInterfaceLog(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_pimsInterface_InterfaceLog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicInterfaceLog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldInterfaceLog, null);
						Method setValue = MESBasicInterfaceLog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(interfaceLog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			interfaceLog.setCreateDepartmentId(oldInterfaceLog.getCreateDepartmentId());
			interfaceLog.setCreatePositionId(oldInterfaceLog.getCreatePositionId());
			interfaceLog.setCreateStaffId(oldInterfaceLog.getCreateStaffId());
			interfaceLog.setOwnerDepartmentId(oldInterfaceLog.getOwnerDepartmentId());
			interfaceLog.setOwnerPositionId(oldInterfaceLog.getOwnerPositionId());
			interfaceLog.setOwnerStaffId(oldInterfaceLog.getOwnerStaffId());
			interfaceLog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=interfaceLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return interfaceLogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			interfaceLog = interfaceLogService.getInterfaceLog(this.interfaceLog.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,interfaceLog.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/pimsInterface/interfaceLog/mainView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainView.ftl") }),
		@Action(value = "/MESBasic/pimsInterface/interfaceLog/mainViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/pimsInterface/interfaceLog/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = interfaceLogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(interfaceLogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(interfaceLogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_pimsInterface_InterfaceLog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/pimsInterface/interfaceLog/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.demo,result.businessDate,result.createTime,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"interfaceLog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"interfaceLog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"interfaceLog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"interfaceLog\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			interfaceLogService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/pimsInterface/interfaceLog/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_pimsInterface_InterfaceLog");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		interfaceLogService.findDataGridPage(dg,dgclass,getDgPage(),interfaceLog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(interfaceLog.getId() != null && interfaceLog.getId() != -1){
				interfaceLog = interfaceLogService.getInterfaceLog(interfaceLog.getId());
			}
			interfaceLogService.findDataGridPage(dg,dgclass,getDgPage(),interfaceLog,searchCondition,params);
	...
	private void save() throws Exception {
		...
		interfaceLogService.saveInterfaceLog(interfaceLog,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/pimsInterface/interfaceLog/mainView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "submit");
		bap_validate_datagrids=interfaceLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", interfaceLog.getId());
		...
		fileuploads.put("linkId", interfaceLog.getId());
		bapAfterCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/pimsInterface/interfaceLog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		interfaceLogService.deleteInterfaceLog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "restore");
		interfaceLogService.restoreInterfaceLog(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/pimsInterface/interfaceLog/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/pimsInterface/interfaceLog/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "showDealInfoData");
		if (null != tableInfoId) {
			int count=interfaceLogService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = interfaceLogService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = interfaceLogService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicInterfaceLog> findExportDataInfos(Page<MESBasicInterfaceLog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = interfaceLogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = interfaceLogService.getAssProperty(p.getCode());	
									//String assModelCode = interfaceLogService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			interfaceLogService.findInterfaceLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			interfaceLogService.findInterfaceLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicInterfaceLog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicInterfaceLog result : results){
	...
	public boolean isEditNew(){
		//return null == this.interfaceLog || null == this.interfaceLog.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/pimsInterface/interfaceLog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/pimsInterface/interfaceLog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicInterfaceLogService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_pimsInterface", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_pimsInterface");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_pimsInterface_InterfaceLog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_pimsInterface_InterfaceLog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_pimsInterface_InterfaceLog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_pimsInterface_InterfaceLog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicInterfaceLog interfaceLog = null;
		if(null != id) {
			interfaceLog = interfaceLogService.getInterfaceLog(id);
		}
		...
		_auditLog.setColumnStringB(null == interfaceLog.getId() ? "" : interfaceLog.getId().toString());
		_auditLog.setColumnStringA(null == interfaceLog.getId() ? "" : interfaceLog.getId().toString());
		_auditLog.setColumnStringC(interfaceLog.getId().toString());
		_auditLog.setColumnStringD(MESBasicInterfaceLog.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_pimsInterface");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicInterfaceLogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, interfaceLog,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, interfaceLog,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,interfaceLog);
		...
		printlog.setEntity("MESBasic_1_pimsInterface");
		...
		printlog.setBusinessId(interfaceLog.getId());
		printlog.setBussinessKey(null == interfaceLog.getId() ? "" : interfaceLog.getId().toString());
		printlog.setMainDisplay(null == interfaceLog.getId() ? "" : interfaceLog.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(interfaceLog,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(interfaceLogService.getInterfaceLog(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, interfaceLog,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				interfaceLog = interfaceLogService.getInterfaceLog(Long.parseLong(arrId[i]));
				if(null != interfaceLog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == interfaceLog.getId() ? "" : "," + interfaceLog.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == interfaceLog.getId() ? "" : interfaceLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == interfaceLog.getId() ? "" : "," + interfaceLog.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == interfaceLog.getId() ? "" : interfaceLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == interfaceLog.getId() ? "" : "," + interfaceLog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == interfaceLog.getId() ? "" : interfaceLog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, interfaceLog,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,interfaceLog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicInterfaceLog> findExportPageInfos(Page<MESBasicInterfaceLog> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_pimsInterface" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = interfaceLogService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicInterfaceLog queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = interfaceLogService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = interfaceLogService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_pimsInterface_InterfaceLog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_pimsInterface_InterfaceLog")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_pimsInterface_InterfaceLog");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_pimsInterface_InterfaceLog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = interfaceLogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_pimsInterface_InterfaceLog,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_pimsInterface_InterfaceLog,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_pimsInterface_InterfaceLog,MESBasic_1) */
// 自定义代码
	private String tags;
	...
	@Action(value = "/MESBasic/pimsInterface/interfaceLog/fetchPIMSTagRealTimeValue",
			results = {@Result(name = SUCCESS, type = JSON, params = {"root", "batchPrintXmlInfo", "excludes", "*", "includes", "xmlRefer"})})
	public String fetchPIMSTagRealTimeValue() throws Exception{
		batchPrintXmlInfo = new HashMap<String, Object>();
		String xmlRefer = interfaceLogService.getTagRealValue(tags);
		batchPrintXmlInfo.put("xmlRefer", xmlRefer);
		return SUCCESS;
	}
/* CUSTOM CODE END */
