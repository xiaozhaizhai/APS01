...
@Repository
public class MESBasicTankModelDaoImpl extends TreeDaoImpl<MESBasicTankModel, Long> implements MESBasicTankModelDao {
	@Override
	...
	@Override
	public MESBasicTankModelDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicTankModelDealInfo dealInfo = new MESBasicTankModelDealInfo();
	...
	@Override
	public MESBasicTankModelDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicTankModel mainObj, Staff dealStaff) {
		MESBasicTankModelDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicTankModelDealInfo.class);
		...
		List<MESBasicTankModelDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_tankModel_TankModel,MESBasic_1) */
// 自定义代码
