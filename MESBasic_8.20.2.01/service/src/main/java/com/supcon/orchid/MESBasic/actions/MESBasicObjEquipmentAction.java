...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryModel_ObjEquipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicObjEquipmentAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicObjEquipment> {
	...
	private MESBasicObjEquipment objEquipment;
	...
	//导出excel时,主模型内容
	List<MESBasicObjEquipment> queryResults = new ArrayList<MESBasicObjEquipment>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objEquipment/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objEquipment/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryModel_ObjEquipment" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicObjEquipment> objEquipments;
	
	public List<MESBasicObjEquipment> getObjEquipments() {
	...
	private Page<MESBasicObjEquipment> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			objEquipment = new MESBasicObjEquipment();
			if(objEquipment.getClearType() != null) {
				objEquipment.setClearType((SystemCode) systemCodeService.getSystemCode(objEquipment.getClearType().getId()));
			}
			if(objEquipment.getEquipmentType() != null) {
				objEquipment.setEquipmentType((SystemCode) systemCodeService.getSystemCode(objEquipment.getEquipmentType().getId()));
			}
		} else {
			if(id > 0){
				objEquipment = objEquipmentService.getObjEquipment(id, viewCode);
			}
		}
	...
	public Page<MESBasicObjEquipment> getPage() {
		if (null == page) {
			page = new Page<MESBasicObjEquipment>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicObjEquipment getObjEquipment() {
		return objEquipment;
	}		

	public void setObjEquipment(MESBasicObjEquipment objEquipment) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objEquipment/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = objEquipmentService.getObjEquipmentAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objEquipment/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1460950269640
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String objEquipmentReferenceCopy() throws Exception{
			MESBasicObjEquipment oldObjEquipment = null;
			if(isEditNew()){
				...
				oldObjEquipment = new MESBasicObjEquipment();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objEquipment.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, objEquipment.getId());
				...
				oldObjEquipment = objEquipmentService.getObjEquipment(id);
			}
			
			
			objEquipment = objEquipmentService.getObjEquipment(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjEquipment");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicObjEquipment.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldObjEquipment, null);
						Method setValue = MESBasicObjEquipment.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(objEquipment, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			objEquipment.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=objEquipmentService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return objEquipmentReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			objEquipment = objEquipmentService.getObjEquipment(this.objEquipment.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objEquipment.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = objEquipmentService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(objEquipmentService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(objEquipmentService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryModel_ObjEquipment", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/objEquipment/objEquipmentRef", results = { @Result(type = FREEMARKER, location = "objEquipmentRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"objEquipment\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"objEquipment\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"objDepartment\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"objFactory\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"objEquipment\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"objEquipment\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			objEquipmentService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/factoryModel/objEquipment/objEquipmentRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.equipmentNum,result.isContainer,result.clearType.id,result.clearType.value,result.objFactory.name,result.objFactory.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="objEquipmentRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		objEquipmentService.findDataGridPage(dg,dgclass,getDgPage(),objEquipment,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(objEquipment.getId() != null && objEquipment.getId() != -1){
				objEquipment = objEquipmentService.getObjEquipment(objEquipment.getId());
			}
			objEquipmentService.findDataGridPage(dg,dgclass,getDgPage(),objEquipment,searchCondition,params);
	...
	private void save() throws Exception {
		...
		objEquipmentService.saveObjEquipment(objEquipment,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "submit");
		bap_validate_datagrids=objEquipmentService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", objEquipment.getId());
		...
		fileuploads.put("linkId", objEquipment.getId());
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryModel/objEquipment/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		objEquipmentService.deleteObjEquipment(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objEquipment/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "restore");
		objEquipmentService.restoreObjEquipment(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicObjEquipment> findExportDataInfos(Page<MESBasicObjEquipment> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = objEquipmentService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = objEquipmentService.getAssProperty(p.getCode());	
									//String assModelCode = objEquipmentService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			objEquipmentService.findObjEquipments(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			objEquipmentService.findObjEquipments(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicObjEquipment> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicObjEquipment result : results){
	...
	public boolean isEditNew(){
		//return null == this.objEquipment || null == this.objEquipment.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryModel/objEquipment/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryModel/objEquipment/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryModel/objEquipment/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objEquipment/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicObjEquipmentService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objEquipment/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryModel_ObjEquipment", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryModel_ObjEquipment", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryModel/objEquipment/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryModel/objEquipment/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryModel_ObjEquipment";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjEquipment", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryModel/objEquipment/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == objEquipment.getId() ? "" : objEquipment.getId().toString());
		_auditLog.setColumnStringA(null == objEquipment.getId() ? "" : objEquipment.getId().toString());
		_auditLog.setColumnStringC(objEquipment.getId().toString());
		_auditLog.setColumnStringD(MESBasicObjEquipment.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicObjEquipmentAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, objEquipment,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, objEquipment,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,objEquipment);
		...
		printlog.setBusinessId(objEquipment.getId());
		printlog.setBussinessKey(null == objEquipment.getId() ? "" : objEquipment.getId().toString());
		printlog.setMainDisplay(null == objEquipment.getId() ? "" : objEquipment.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryModel/objEquipment/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(objEquipment,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objEquipment/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(objEquipmentService.getObjEquipment(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryModel/objEquipment/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryModel/objEquipment/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, objEquipment,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objEquipment = objEquipmentService.getObjEquipment(Long.parseLong(arrId[i]));
				if(null != objEquipment){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == objEquipment.getId() ? "" : "," + objEquipment.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == objEquipment.getId() ? "" : objEquipment.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == objEquipment.getId() ? "" : "," + objEquipment.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == objEquipment.getId() ? "" : objEquipment.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == objEquipment.getId() ? "" : "," + objEquipment.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == objEquipment.getId() ? "" : objEquipment.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, objEquipment,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,objEquipment);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/factoryModel/objEquipment/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjEquipment");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryModel_ObjEquipment")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryModel_ObjEquipment");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjEquipment");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = objEquipmentService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryModel_ObjEquipment,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryModel_ObjEquipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryModel_ObjEquipment,MESBasic_1) */
// 自定义代码
