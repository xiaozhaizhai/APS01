...
import com.supcon.orchid.MESBasic.services.MESBasicPurchGroupImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicPurchGroupDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_purchGroupService")
@Transactional
public class MESBasicPurchGroupServiceImpl  extends BaseServiceImpl<MESBasicPurchGroup> implements MESBasicPurchGroupService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicPurchGroup> bapBaseService;
	...
	@Autowired
	private MESBasicPurchGroupImportService purchGroupImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicPurchGroup.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  purchGroupDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicPurchGroup.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = purchGroupDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPurchGroup.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPurchGroup.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  purchGroupDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicPurchGroup.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicPurchGroup.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicPurchGroup> list = (List<MESBasicPurchGroup>) purchGroupDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicPurchGroup getPurchGroup(long id){
		return getPurchGroup(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPurchGroup getPurchGroup(long id, String viewCode){
		MESBasicPurchGroup purchGroup = purchGroupDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPurchGroupAsJSON(long id, String include){
		MESBasicPurchGroup purchGroup = purchGroupDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(purchGroup, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.common.delete",operType="3")
	public void deletePurchGroup(MESBasicPurchGroup purchGroup){
		if(null != purchGroup && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroup.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
			AuditUtil.setColumnStringA(null == purchGroup.getPurchGroupName() ? "" : purchGroup.getPurchGroupName().toString());
			AuditUtil.setColumnStringC(purchGroup.getId().toString());
		}
		beforeDeletePurchGroup(purchGroup);
		purchGroupDao.delete(purchGroup);
		afterDeletePurchGroup(purchGroup);
		purchGroupDao.flush();
		List<MESBasicPurchGroup> params = new ArrayList<MESBasicPurchGroup>();
		params.add(purchGroup);
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.common.delete",operType="3")
	public void deletePurchGroup(List<Long> purchGroupIds){
		deletePurchGroup(purchGroupIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.common.delete",operType="3")
	public void deletePurchGroup(List<Long> purchGroupIds, String eventTopic) {
		List<MESBasicPurchGroup> purchGroups = new ArrayList<MESBasicPurchGroup>();
		for(Long purchGroupId : purchGroupIds){
			MESBasicPurchGroup purchGroup = getPurchGroup(purchGroupId);
			purchGroups.add(purchGroup);
			if(null != purchGroup && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroup.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == purchGroup.getPurchGroupName() ? "" : "," + purchGroup.getPurchGroupName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == purchGroup.getPurchGroupName() ? "" : purchGroup.getPurchGroupName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == purchGroup.getId() ? "" : "," + purchGroup.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == purchGroup.getId() ? "" : "," + purchGroup.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(purchGroups != null && purchGroups.size() > 0 ){
			for(MESBasicPurchGroup purchGroup : purchGroups){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),purchGroup, purchGroup.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(purchGroupIds != null && purchGroupIds.size() > 0) {
			String hql = "update " + MESBasicPurchGroup.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = purchGroupDao.createQuery(hql);
			query.setParameterList("ids", purchGroupIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicPurchGroup purchGroupz : purchGroups){
				purchGroupz.setValid(false);
				List<MESBasicPurchGroup> params = new ArrayList<MESBasicPurchGroup>();
				params.add(purchGroupz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_purch_Group");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.common.delete",operType="3")
	public void deletePurchGroup(long purchGroupId, int purchGroupVersion){
		this.deletePurchGroup(purchGroupId, purchGroupVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.common.delete",operType="3")
	public void deletePurchGroup(long purchGroupId, int purchGroupVersion,SignatureLog signatureLog){
		MESBasicPurchGroup purchGroup = getPurchGroup(purchGroupId);
		...
		beforeDeletePurchGroup(purchGroup);
		purchGroupDao.delete(purchGroupId, purchGroupVersion);
		afterDeletePurchGroup(purchGroup);
		purchGroupDao.flush();
		List<MESBasicPurchGroup> params = new ArrayList<MESBasicPurchGroup>();
		params.add(purchGroup);
		...
		if(signatureLog != null ) {
			...
			businessKey=purchGroup.getId();
			signatureLog.setTableId(purchGroup.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.common.delete",operType="3")
	public void deletePurchGroup(String purchGroupIds){
		this.deletePurchGroup(purchGroupIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.common.delete",operType="3")
	public void deletePurchGroup(String purchGroupIds,SignatureLog signatureLog){
		deleteCollection(purchGroupIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePurchGroup(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_purch_Group#modelCode:MESBasic_1_purch_Group_PurchGroup#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="ec.engine.restore",operType="10")
	public void restorePurchGroup(String purchGroupIds){
		restoreCollection(purchGroupIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePurchGroup(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="ec.engine.restore",operType="10")
	public void restorePurchGroup(long purchGroupId){
		
		findBusinessKeyUsed(purchGroupId);	//判断业务主键是否重复
		...
		beforeRestorePurchGroup(purchGroup);
		purchGroup.setValid(true);
		purchGroupDao.update(purchGroup);
		afterRestorePurchGroup(purchGroup);
		List<MESBasicPurchGroup> params = new ArrayList<MESBasicPurchGroup>();
		params.add(purchGroup);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long purchGroupId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_purch_Group_PurchGroup");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicPurchGroup.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicPurchGroup.TABLE_NAME +" where id = ? )";
			List<Object> list =  purchGroupDao.createNativeQuery(sql,purchGroupId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePurchGroup(List<MESBasicPurchGroup>  purchGroups){
		for(MESBasicPurchGroup purchGroup:purchGroups)  {
			savePurchGroup(purchGroup, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePurchGroup(List<MESBasicPurchGroup>  purchGroups){
		for(MESBasicPurchGroup purchGroup:purchGroups)  {
			savePurchGroup(purchGroup, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="ec.flow.save")
	public void savePurchGroup(MESBasicPurchGroup purchGroup, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePurchGroup(purchGroup, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="ec.flow.save")
	public void savePurchGroup(MESBasicPurchGroup purchGroup, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePurchGroup(purchGroup, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePurchGroupCodes(MESBasicPurchGroup purchGroup) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePurchGroupCodes(purchGroup, false);
	}
	
	public void generatePurchGroupCodes(MESBasicPurchGroup purchGroup, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePurchGroupSummarys(MESBasicPurchGroup purchGroup) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePurchGroupSummarys(purchGroup, false);
	}

	public void generatePurchGroupSummarys(MESBasicPurchGroup purchGroup, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup")
	public void savePurchGroup(MESBasicPurchGroup purchGroup,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePurchGroup(purchGroup, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup")
	public void savePurchGroup(MESBasicPurchGroup purchGroup,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_purch_Group");
		String url = null;
		if(purchGroup.getId() != null && purchGroup.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroup.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, purchGroup);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(purchGroup);
		beforeSavePurchGroup(purchGroup, viewIsView);

		if (viewIsView) {
			purchGroupDao.saveWithRevertVersion(purchGroup);
		} else {
			if(isNew)
				purchGroupDao.save(purchGroup);
			else
				purchGroupDao.merge(purchGroup);
		}
		if(null != purchGroup && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroup.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
			AuditUtil.setColumnStringA(null == purchGroup.getPurchGroupName() ? "" : purchGroup.getPurchGroupName().toString());
		...
		dealDatagridsSave(purchGroup,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePurchGroupCodes(purchGroup, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePurchGroupSummarys(purchGroup, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePurchGroup(purchGroup, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, purchGroup);
			}
		}
		List<MESBasicPurchGroup> params = new ArrayList<MESBasicPurchGroup>();
		purchGroupDao.flush();
		purchGroupDao.clear();
		purchGroup = purchGroupDao.load(purchGroup.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicPurchGroup purchGroup){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("purchGroup", purchGroup);
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup")
	public void mergePurchGroup(MESBasicPurchGroup purchGroup, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(purchGroup);
		beforeSavePurchGroup(purchGroup);
		purchGroupDao.merge(purchGroup);
		if(null != purchGroup && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroup.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == purchGroup.getId() ? "" : purchGroup.getId().toString());
			AuditUtil.setColumnStringA(null == purchGroup.getPurchGroupName() ? "" : purchGroup.getPurchGroupName().toString());
		...
		dealDatagridsSave(purchGroup,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePurchGroup(purchGroup);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicPurchGroup> findPurchGroups(Page<MESBasicPurchGroup> page, Criterion... criterions) {
		return purchGroupDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicPurchGroupDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicPurchGroupDealInfo> list=purchGroupDao.createQuery("from " + MESBasicPurchGroupDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicPurchGroupDealInfo dealInfo) {
		purchGroupDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicPurchGroupDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) purchGroupDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicPurchGroupDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = purchGroupDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = purchGroupDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicPurchGroupDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = purchGroupDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPurchGroups(Page<MESBasicPurchGroup> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPurchGroups(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPurchGroups(Page<MESBasicPurchGroup> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPurchGroups(Page<MESBasicPurchGroup> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicPurchGroup> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"purchGroup\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"purchGroup\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"purchGroup\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_purch_Group_PurchGroup" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_purch_Group_PurchGroup" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  purchGroupDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  purchGroupDao.createNativeQuery(powerOperateSql, "MESBasic_1_purch_Group").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"purchGroup\"","MESBasic_1_purch_Group",customCondition,"S2BASE_USERREPORT",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"purchGroup\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicPurchGroupDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"purchGroup\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"purchGroup\".LAY_REC ASC, \"purchGroup\".SORT ASC");
				} else {
					orderPart.append(" \"purchGroup\".USERREPORT_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) purchGroupDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=purchGroupDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = purchGroupDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicPurchGroup> result = new ArrayList<MESBasicPurchGroup>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicPurchGroup.class,purchGroupDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicPurchGroup.class,purchGroupDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicPurchGroup purchGroup : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(purchGroup.getId(), "MESBasic_purch_Group_purchGroup");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(purchGroup.getId(), "MESBasic_purch_Group_purchGroup");
							if(null != documents && !documents.isEmpty()) {
								purchGroup.setDocument(documents.get(0));
								purchGroup.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicPurchGroup> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicPurchGroup> result) {
		View view = null;
		...
		Map<Long, MESBasicPurchGroup> rsMap = new HashMap<Long, MESBasicPurchGroup>();
		for (MESBasicPurchGroup purchGroup : result) {
			ids.add(purchGroup.getId());
			rsMap.put(purchGroup.getId(), purchGroup);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(purchGroupDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = purchGroupDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicPurchGroup purchGroup,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(purchGroup.getOwnerStaffId());
		ti.setOwnerPositionId(purchGroup.getOwnerPositionId());
		ti.setOwnerDepartmentId(purchGroup.getOwnerDepartmentId());
		
		ti.setCreateStaff(purchGroup.getCreateStaff());
		ti.setCreateStaffId(purchGroup.getCreateStaffId());
		ti.setCreateTime(purchGroup.getCreateTime());
		ti.setDeleteStaff(purchGroup.getDeleteStaff());
		ti.setDeleteStaffId(purchGroup.getDeleteStaffId());
		ti.setDeleteTime(purchGroup.getDeleteTime());
		ti.setCreateDepartmentId(purchGroup.getCreateDepartmentId());
		ti.setModifyStaff(purchGroup.getModifyStaff());
		ti.setModifyStaffId(purchGroup.getModifyStaffId());
		ti.setCreatePositionId(purchGroup.getCreatePositionId());
		ti.setPositionLayRec(purchGroup.getPositionLayRec());
		ti.setTableNo(purchGroup.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_purch_Group");
		ti.setTargetTableName("S2BASE_USERREPORT");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_purch_Group_PurchGroup" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  purchGroupDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"purchGroup\"","MESBasic_1_purch_Group","","S2BASE_USERREPORT",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"purchGroup\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"purchGroup\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"purchGroup\"","MESBasic_1_purch_Group","","S2BASE_USERREPORT",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("purchGroup.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"purchGroup\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"purchGroup\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"purchGroup\".CID = 1 OR \"purchGroup\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("purchGroup.mc\"").append(".PURCH_GROUP) not in ("+notinValue+")");
			}
			
			SQLQuery query = purchGroupDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=purchGroupDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicPurchGroup.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicPurchGroup.MODULE_CODE;}
	protected String getModelCode(){return MESBasicPurchGroup.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param purchGroup 模型实例
	 */
	public void checkUniqueConstraint(MESBasicPurchGroup purchGroup){
	}
	
	
	@Override
	public List<MESBasicPurchGroup> getPurchGroups(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPurchGroup.class);
		if(params!=null){
			...
			return purchGroupDao.findByHql("from " + MESBasicPurchGroup.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return purchGroupDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicPurchGroup> findPurchGroupsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPurchGroup.class);
	...
	@Override
	public List<MESBasicPurchGroup> findPurchGroupsByHql(String hql, Object... objects){
		
		return   purchGroupDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicPurchGroup loadPurchGroupByBussinessKey(MESBasicPurchGroup purchGroup){
		return loadPurchGroupByBussinessKey(purchGroup.getId());
	}
	
	@Override
	public Page<MESBasicPurchGroup> getPurchGroups(Page<MESBasicPurchGroup> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPurchGroup.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = purchGroupDao.findByPage(page, sql);
			}else{
				page = purchGroupDao.findAllByPage(page);
			}
			return page;
		}
		page = purchGroupDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPurchGroup loadPurchGroupByBussinessKey(Serializable bussinessKey){
		return purchGroupDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicPurchGroup> findByProperty(String propertyName, Object object){
		return purchGroupDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPurchGroup findEntityByProperty(String propertyName, Object object){
		return purchGroupDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicPurchGroup.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = purchGroupDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_purch_Group_PurchGroup","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicPurchGroup purchGroup,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,purchGroupDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicPurchGroup.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, purchGroup);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",purchGroup.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPurchGroup.TABLE_NAME +" where valid = 1";
			list = purchGroupDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPurchGroup.TABLE_NAME;
			list = purchGroupDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicPurchGroup.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = purchGroupDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicPurchGroupDaoImpl purchGroupDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPurchGroup(final List<MESBasicPurchGroup> insertObjs, final List<MESBasicPurchGroup> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPurchGroup(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPurchGroup(final List<MESBasicPurchGroup> insertObjs, final List<MESBasicPurchGroup> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1470291767311"),importCount));
		...
		Map<Long, MESBasicPurchGroup> oldUpdateObjsMap = new HashMap<Long, MESBasicPurchGroup>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicPurchGroup bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicPurchGroup> bizObjList = purchGroupDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicPurchGroup bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicPurchGroup> nmeObjs = new ArrayList<MESBasicPurchGroup>();
		...
		long id=autoGeneratorID.getNextId(MESBasicPurchGroup.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicPurchGroup purchGroup:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicPurchGroup.TABLE_NAME,1,dbId);
			if(purchGroup.getId() != null ){
				m.put(purchGroup.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) purchGroupDao.createNativeQuery(sql, "MESBasic_1_purch_Group_PurchGroup"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		purchGroupDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicPurchGroup.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicPurchGroup purchGroup = updateObjs.get(updateIndex);
						Long id = purchGroup.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = purchGroup.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicPurchGroup.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicPurchGroup.TABLE_NAME + " order by id desc";
//					Query query = purchGroupDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicPurchGroup purchGroup = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = purchGroup.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		purchGroupDaoImpl.getSession().flush();
		purchGroupDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicPurchGroup bizObj = insertObjs.get(insertIndex);
			purchGroupImportService.saveImportDataLog(bizObj, null, "id", "purchGroupName");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicPurchGroup bizObj = updateObjs.get(updateIndex);
			MESBasicPurchGroup oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			purchGroupImportService.saveImportDataLog(bizObj, oldBizObj, "id", "purchGroupName");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = purchGroupDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				purchGroupDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = purchGroupDao.createNativeQuery(sql, "MESBasic_1_purch_Group_PurchGroup").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = purchGroupDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = purchGroupDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = purchGroupDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?purchGroupDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  purchGroupDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,purchGroupDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="ec.flow.save")
	public void savePurchGroup(MESBasicPurchGroup purchGroup, DataGridService dataGridService){
		savePurchGroup(purchGroup, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="ec.flow.save")
	public void savePurchGroup(MESBasicPurchGroup purchGroup, DataGridService dataGridService, String viewCode){
		savePurchGroup(purchGroup, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup")
	public void savePurchGroup(MESBasicPurchGroup purchGroup, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePurchGroup(purchGroup, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup")
	public void savePurchGroup(MESBasicPurchGroup purchGroup, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_purch_Group";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_purch_Group");
		String url = null;
		if(null != purchGroup.getId() && purchGroup.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPurchGroup.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup")
	public Map<Object, Object> purchGroupDataGridImport(MESBasicPurchGroup purchGroup, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = purchGroup.getVirtualId();
		
		purchGroup = purchGroupDao.load(purchGroup.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, purchGroup.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = purchGroup.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(purchGroup);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, purchGroup.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup")
	public void mergePurchGroup(MESBasicPurchGroup purchGroup, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(purchGroup);
		beforeSavePurchGroup(purchGroup);
		purchGroupDao.merge(purchGroup);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_purch_Group",model="MESBasic_1_purch_Group_PurchGroup",desc="foundation.batch.save",operType="4")
	public void batchSavePurchGroup(MESBasicPurchGroup purchGroup, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_purch_Group";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_purch_Group");
	...
	@Override
	public Page<MESBasicPurchGroup> getByPage(Page<MESBasicPurchGroup> page,DetachedCriteria detachedCriteria){
		return purchGroupDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSavePurchGroup(MESBasicPurchGroup purchGroup, Object... objects){
		checkUniqueConstraint(purchGroup);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePurchGroup(MESBasicPurchGroup purchGroup, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPurchGroup(List<MESBasicPurchGroup> purchGroups, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPurchGroup(List<MESBasicPurchGroup> purchGroups){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPurchGroup(List<MESBasicPurchGroup> purchGroups, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPurchGroup(List<MESBasicPurchGroup> purchGroups){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePurchGroup(MESBasicPurchGroup purchGroup){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePurchGroup(MESBasicPurchGroup purchGroup){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePurchGroup(MESBasicPurchGroup purchGroup){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePurchGroup(MESBasicPurchGroup purchGroup){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicPurchGroup purchGroup, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicPurchGroup purchGroup, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPurchGroup(List<MESBasicPurchGroup> insertObjs, List<MESBasicPurchGroup> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPurchGroup,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPurchGroup(List<MESBasicPurchGroup> insertObjs, List<MESBasicPurchGroup> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPurchGroup,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_purch_Group_PurchGroup,MESBasic_1) */
// 自定义代码
