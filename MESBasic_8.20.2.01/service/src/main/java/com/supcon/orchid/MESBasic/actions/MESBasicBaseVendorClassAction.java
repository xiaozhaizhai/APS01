...
/* CUSTOM CODE START(action,import,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicBaseVendorClassAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicBaseVendorClass> {
	...
	//导出excel时,主模型内容
	List<MESBasicBaseVendorClass> queryResults = new ArrayList<MESBasicBaseVendorClass>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseVenderClass/baseVendorClass/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/baseVenderClass/baseVendorClass/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_baseVenderClass_BaseVendorClass" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicBaseVendorClass> baseVendorClasss;
	
	public List<MESBasicBaseVendorClass> getBaseVendorClasss() {
		return baseVendorClasss;
	}
	...
	private Page<MESBasicBaseVendorClass> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			baseVendorClass = new MESBasicBaseVendorClass();
		} else {
			if(id > 0){
				baseVendorClass = baseVendorClassService.getBaseVendorClass(id, viewCode);
			}
		}
	...
	public Page<MESBasicBaseVendorClass> getPage() {
		if (null == page) {
			page = new Page<MESBasicBaseVendorClass>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = baseVendorClassService.getBaseVendorClassAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/edit-extra-col")
	public String _editExtraCol() throws Exception{
		if(!isEditNew()){
			if(baseVendorClass.getExtraCol()!=null && baseVendorClass.getExtraCol().length() > 0) {
				Struts2Utils.renderXml(baseVendorClass.getExtraCol());
				return null;
			}
		}
	 ...
	 * @throws Exception
	 */
		public String baseVendorClassReferenceCopy() throws Exception{
			MESBasicBaseVendorClass oldBaseVendorClass = null;
			if(isEditNew()){
				...
				oldBaseVendorClass = new MESBasicBaseVendorClass();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseVendorClass.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, baseVendorClass.getId());
				...
				oldBaseVendorClass = baseVendorClassService.getBaseVendorClass(id);
			}
			
			
			baseVendorClass = baseVendorClassService.getBaseVendorClass(refId);
			if (baseVendorClass.getExtraCol() != null && baseVendorClass.getExtraCol().length() > 0) {
				baseVendorClass.setExtraColMap(XmlUtils.xml2Map(baseVendorClass.getExtraCol()));
			}
			Model model = modelServiceFoundation.getModel("MESBasic_1_baseVenderClass_BaseVendorClass");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicBaseVendorClass.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBaseVendorClass, null);
						Method setValue = MESBasicBaseVendorClass.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(baseVendorClass, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			baseVendorClass.setCreateDepartmentId(oldBaseVendorClass.getCreateDepartmentId());
			baseVendorClass.setCreatePositionId(oldBaseVendorClass.getCreatePositionId());
			baseVendorClass.setCreateStaffId(oldBaseVendorClass.getCreateStaffId());
			baseVendorClass.setOwnerDepartmentId(oldBaseVendorClass.getOwnerDepartmentId());
			baseVendorClass.setOwnerPositionId(oldBaseVendorClass.getOwnerPositionId());
			baseVendorClass.setOwnerStaffId(oldBaseVendorClass.getOwnerStaffId());
			baseVendorClass.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/editView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editView.ftl") }),
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/editViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editViewPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=baseVendorClassService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return baseVendorClassReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			baseVendorClass = baseVendorClassService.getBaseVendorClass(this.baseVendorClass.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,baseVendorClass.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			baseVendorClassService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/mainView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainView.ftl") }),
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/mainViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/editPart", results = { @Result(type = FREEMARKER, location = "editPart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/venderClassLayout", results = { @Result(type = FREEMARKER, location = "venderClassLayout.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = baseVendorClassService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(baseVendorClassService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(baseVendorClassService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_baseVenderClass_BaseVendorClass", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/refVendor", results = { @Result(type = FREEMARKER, location = "refVendor.ftl") })
		,
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/refVendorTree", results = { @Result(type = FREEMARKER, location = "refVendorTree.ftl") })
		,
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/tree", results = { @Result(type = FREEMARKER, location = "tree.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/editPart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.vcMemo,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="editPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"baseVendorClass\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"baseVendorClass\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"baseVendorClass\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"baseVendorClass\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			baseVendorClassService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/editPart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/refVendor-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.vcMemo,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="refVendor-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/refVendorTreeDrag")
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/treeDrag")
	})
	public String viewTreeDrag() throws Exception{
		baseVendorClassService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/editPartSort")
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/editViewSort")
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/mainViewSort")
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/refVendorSort")
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/refVendorTreeSort")
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/treeSort")
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/venderClassLayoutSort")
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/vendorMneSort")
	})
	...
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/refVendorTreeData",
			results = { @Result(type = JSON, params = { "root", "baseVendorClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/treeData",
			results = { @Result(type = JSON, params = { "root", "baseVendorClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"baseVendorClass\".", "{alias}.");
					}
		...
		baseVendorClasss = baseVendorClassService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/refVendorTreeFullData",
			results = { @Result(type = JSON, params = { "root", "baseVendorClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/treeFullData",
			results = { @Result(type = JSON, params = { "root", "baseVendorClasss", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			baseVendorClasss = baseVendorClassService.getTreeInfo(treeInfo, null, null);
		}
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/allTempleteDownload", results = { @Result(name="excel",type="excel",location="editPart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_baseVenderClass_BaseVendorClass");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		baseVendorClassService.findDataGridPage(dg,dgclass,getDgPage(),baseVendorClass,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(baseVendorClass.getId() != null && baseVendorClass.getId() != -1){
				baseVendorClass = baseVendorClassService.getBaseVendorClass(baseVendorClass.getId());
			}
			baseVendorClassService.findDataGridPage(dg,dgclass,getDgPage(),baseVendorClass,searchCondition,params);
	...
	private void save() throws Exception {
		...
		baseVendorClassService.saveBaseVendorClass(baseVendorClass,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/editView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/mainView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "submit");
		bap_validate_datagrids=baseVendorClassService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", baseVendorClass.getId());
		...
		fileuploads.put("linkId", baseVendorClass.getId());
		bapAfterCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		baseVendorClassService.deleteBaseVendorClass(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "restore");
		baseVendorClassService.restoreBaseVendorClass(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "showDealInfoData");
		if (null != tableInfoId) {
			int count=baseVendorClassService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = baseVendorClassService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = baseVendorClassService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicBaseVendorClass> findExportDataInfos(Page<MESBasicBaseVendorClass> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = baseVendorClassService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = baseVendorClassService.getAssProperty(p.getCode());	
									//String assModelCode = baseVendorClassService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
				...
				if(view.getAssModel().getDataType() == 2 && useForImportFlag){//树型结构且用于导出，加上父节点,同时在sql中加入layNo,layRec,parentId,sort,leaf
					...
					exportSqlPart1Add.append("\".VC_LAYNO AS \"layNo\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
					exportSqlPart1Add.append("\".VC_LAYREC AS \"layRec\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			baseVendorClassService.findBaseVendorClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			baseVendorClassService.findBaseVendorClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicBaseVendorClass> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicBaseVendorClass result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".VC_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.baseVendorClass || null == this.baseVendorClass.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/baseVenderClass/baseVendorClass/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicBaseVendorClassService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_baseVenderClass", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_baseVenderClass");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_baseVenderClass_BaseVendorClass", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_baseVenderClass_BaseVendorClass", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_baseVenderClass_BaseVendorClass";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_baseVenderClass_BaseVendorClass", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicBaseVendorClass baseVendorClass = null;
		if(null != id) {
			baseVendorClass = baseVendorClassService.getBaseVendorClass(id);
		}
		...
		_auditLog.setColumnStringB(null == baseVendorClass.getCode() ? "" : baseVendorClass.getCode().toString());
		_auditLog.setColumnStringA(null == baseVendorClass.getName() ? "" : baseVendorClass.getName().toString());
		_auditLog.setColumnStringC(baseVendorClass.getId().toString());
		_auditLog.setColumnStringD(MESBasicBaseVendorClass.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_baseVenderClass");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicBaseVendorClassAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, baseVendorClass,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, baseVendorClass,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,baseVendorClass);
		...
		printlog.setEntity("MESBasic_1_baseVenderClass");
		...
		printlog.setBusinessId(baseVendorClass.getId());
		printlog.setBussinessKey(null == baseVendorClass.getCode() ? "" : baseVendorClass.getCode().toString());
		printlog.setMainDisplay(null == baseVendorClass.getName() ? "" : baseVendorClass.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(baseVendorClass,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(baseVendorClassService.getBaseVendorClass(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, baseVendorClass,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				baseVendorClass = baseVendorClassService.getBaseVendorClass(Long.parseLong(arrId[i]));
				if(null != baseVendorClass){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == baseVendorClass.getName() ? "" : "," + baseVendorClass.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == baseVendorClass.getName() ? "" : baseVendorClass.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == baseVendorClass.getCode() ? "" : "," + baseVendorClass.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == baseVendorClass.getCode() ? "" : baseVendorClass.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == baseVendorClass.getId() ? "" : "," + baseVendorClass.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == baseVendorClass.getId() ? "" : baseVendorClass.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, baseVendorClass,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,baseVendorClass);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//baseVendorClassService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicBaseVendorClass> findExportPageInfos(Page<MESBasicBaseVendorClass> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_baseVenderClass" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = baseVendorClassService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicBaseVendorClass queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = baseVendorClassService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = baseVendorClassService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_baseVenderClass_BaseVendorClass");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_baseVenderClass_BaseVendorClass")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_baseVenderClass_BaseVendorClass");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_baseVenderClass_BaseVendorClass");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = baseVendorClassService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_baseVenderClass_BaseVendorClass,MESBasic_1) */
// 自定义代码
	...
	@Action(value = "/MESBasic/baseVenderClass/baseVendorClass/hasVendor", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String hasVendor() {
		String falg = baseVendorClassService.hasVendor(vendorId);
		responseMap.put("hasVendorFlag", falg);
		return SUCCESS;
	}
/* CUSTOM CODE END */
}