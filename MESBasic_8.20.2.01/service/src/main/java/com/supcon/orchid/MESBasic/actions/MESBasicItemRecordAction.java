...
/* CUSTOM CODE START(action,import,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicItemRecordAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicItemRecord> {
	...
	private MESBasicItemRecord itemRecord;
	...
	//导出excel时,主模型内容
	List<MESBasicItemRecord> queryResults = new ArrayList<MESBasicItemRecord>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/itemRecord/itemRecord/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/itemRecord/itemRecord/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_itemRecord_ItemRecord" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicItemRecord> itemRecords;
	
	public List<MESBasicItemRecord> getItemRecords() {
		return itemRecords;
	}
	...
	private Page<MESBasicItemRecord> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			itemRecord = new MESBasicItemRecord();
		} else {
			if(id > 0){
				itemRecord = itemRecordService.getItemRecord(id, viewCode);
			}
		}
	...
	public Page<MESBasicItemRecord> getPage() {
		if (null == page) {
			page = new Page<MESBasicItemRecord>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicItemRecord getItemRecord() {
		return itemRecord;
	}		

	public void setItemRecord(MESBasicItemRecord itemRecord) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/itemRecord/itemRecord/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = itemRecordService.getItemRecordAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/itemRecord/itemRecord/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String itemRecordReferenceCopy() throws Exception{
			MESBasicItemRecord oldItemRecord = null;
			if(isEditNew()){
				...
				oldItemRecord = new MESBasicItemRecord();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,itemRecord.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, itemRecord.getId());
				...
				oldItemRecord = itemRecordService.getItemRecord(id);
			}
			
			
			itemRecord = itemRecordService.getItemRecord(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_itemRecord_ItemRecord");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicItemRecord.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldItemRecord, null);
						Method setValue = MESBasicItemRecord.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(itemRecord, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			itemRecord.setCreateDepartmentId(oldItemRecord.getCreateDepartmentId());
			itemRecord.setCreatePositionId(oldItemRecord.getCreatePositionId());
			itemRecord.setCreateStaffId(oldItemRecord.getCreateStaffId());
			itemRecord.setOwnerDepartmentId(oldItemRecord.getOwnerDepartmentId());
			itemRecord.setOwnerPositionId(oldItemRecord.getOwnerPositionId());
			itemRecord.setOwnerStaffId(oldItemRecord.getOwnerStaffId());
			itemRecord.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_itemRecord_ItemRecord", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=itemRecordService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return itemRecordReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			itemRecord = itemRecordService.getItemRecord(this.itemRecord.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,itemRecord.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemRecord/itemRecord/itemRecordView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemRecordView.ftl") }),
		@Action(value = "/MESBasic/itemRecord/itemRecord/itemRecordViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "itemRecordViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemRecord/itemRecord/itemRecordList", results = { @Result(type = FREEMARKER, location = "itemRecordList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = itemRecordService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(itemRecordService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(itemRecordService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_itemRecord_ItemRecord", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemRecord/itemRecord/itemRecordList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.itemNumber.code,result.recordTime,result.averageValue,result.maxValue,result.minValue,result.balanceValue,result.count,result.totalValue,result.realValue,result.planNo,result.reaultValue,result.itemNumber.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="itemRecordList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"itemRecord\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"itemRecord\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"itemRecord\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"itemRecord\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/itemRecord/itemRecord/itemRecordList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/itemRecord/itemRecord/allTempleteDownload", results = { @Result(name="excel",type="excel",location="itemRecordList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_itemRecord_ItemRecord");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		itemRecordService.findDataGridPage(dg,dgclass,getDgPage(),itemRecord,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(itemRecord.getId() != null && itemRecord.getId() != -1){
				itemRecord = itemRecordService.getItemRecord(itemRecord.getId());
			}
			itemRecordService.findDataGridPage(dg,dgclass,getDgPage(),itemRecord,searchCondition,params);
	...
	private void save() throws Exception {
		...
		itemRecordService.saveItemRecord(itemRecord,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/itemRecord/itemRecord/itemRecordView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "submit");
		bap_validate_datagrids=itemRecordService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", itemRecord.getId());
		...
		fileuploads.put("linkId", itemRecord.getId());
		bapAfterCustomMethod("MESBasic_1_itemRecord_ItemRecord", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/itemRecord/itemRecord/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		itemRecordService.deleteItemRecord(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_itemRecord_ItemRecord", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/itemRecord/itemRecord/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "restore");
		itemRecordService.restoreItemRecord(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_itemRecord_ItemRecord", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/itemRecord/itemRecord/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/itemRecord/itemRecord/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "showDealInfoData");
		if (null != tableInfoId) {
			int count=itemRecordService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = itemRecordService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = itemRecordService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_itemRecord_ItemRecord", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicItemRecord> findExportDataInfos(Page<MESBasicItemRecord> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = itemRecordService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = itemRecordService.getAssProperty(p.getCode());	
									//String assModelCode = itemRecordService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			itemRecordService.findItemRecords(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			itemRecordService.findItemRecords(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicItemRecord> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicItemRecord result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".IR_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.itemRecord || null == this.itemRecord.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/itemRecord/itemRecord/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_itemRecord_ItemRecord", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/itemRecord/itemRecord/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/itemRecord/itemRecord/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/itemRecord/itemRecord/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicItemRecordService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_itemRecord", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_itemRecord");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_itemRecord_ItemRecord", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/itemRecord/itemRecord/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicItemRecordService"
						...
						, "com.supcon.orchid.X6Basic.services.X6BasicPlanDailyService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_itemRecord_ItemRecord", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_itemRecord_ItemRecord", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/itemRecord/itemRecord/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/itemRecord/itemRecord/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_itemRecord_ItemRecord";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_itemRecord_ItemRecord", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/itemRecord/itemRecord/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicItemRecord itemRecord = null;
		if(null != id) {
			itemRecord = itemRecordService.getItemRecord(id);
		}
		...
		_auditLog.setColumnStringB(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
		_auditLog.setColumnStringA(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
		_auditLog.setColumnStringC(itemRecord.getId().toString());
		_auditLog.setColumnStringD(MESBasicItemRecord.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_itemRecord");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicItemRecordAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, itemRecord,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, itemRecord,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,itemRecord);
		...
		printlog.setEntity("MESBasic_1_itemRecord");
		...
		printlog.setBusinessId(itemRecord.getId());
		printlog.setBussinessKey(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
		printlog.setMainDisplay(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/itemRecord/itemRecord/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(itemRecord,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/itemRecord/itemRecord/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(itemRecordService.getItemRecord(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/itemRecord/itemRecord/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/itemRecord/itemRecord/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, itemRecord,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				itemRecord = itemRecordService.getItemRecord(Long.parseLong(arrId[i]));
				if(null != itemRecord){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == itemRecord.getId() ? "" : "," + itemRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == itemRecord.getId() ? "" : "," + itemRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == itemRecord.getId() ? "" : "," + itemRecord.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == itemRecord.getId() ? "" : itemRecord.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, itemRecord,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,itemRecord);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/itemRecord/itemRecord/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicItemRecord> findExportPageInfos(Page<MESBasicItemRecord> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_itemRecord" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = itemRecordService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicItemRecord queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = itemRecordService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = itemRecordService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_itemRecord_ItemRecord");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_itemRecord_ItemRecord")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_itemRecord_ItemRecord");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_itemRecord_ItemRecord");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = itemRecordService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
