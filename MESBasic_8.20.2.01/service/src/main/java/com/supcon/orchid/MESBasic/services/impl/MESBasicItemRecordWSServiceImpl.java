...
import com.supcon.orchid.MESBasic.services.MESBasicItemRecordWSService;
import com.supcon.orchid.MESBasic.services.MESBasicItemRecordService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicItemRecord",targetNamespace = "http://ws.supcon.com")
public class MESBasicItemRecordWSServiceImpl implements MESBasicItemRecordWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getItemRecordByBusinessKey(
		...
		try{
			...
			MESBasicItemRecord itemRecord = itemRecordService.loadItemRecordByBussinessKey(businessKey);
			if(itemRecord == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, itemRecord);
		}catch(Throwable t){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getItemRecordByPk(
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO updateItemRecord(
		@WebParam(name="itemRecord")MESBasicItemRecord itemRecord,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
			
		JWSResultDTO result = new JWSResultDTO();
		try{
			if(itemRecord == null){
				return set500Error(result, "位号数据统计不能为空。");
			}
			...
			if(!beforeUpdateItemRecord(itemRecord)){
				return set404Error(result);
			}
			itemRecordService.saveItemRecord(itemRecord, null);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO addItemRecord(
		@WebParam(name="itemRecord")MESBasicItemRecord itemRecord,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				checkItemRecordData(itemRecord);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			itemRecordService.saveItemRecord(itemRecord, null);
			setSuccessResult(result, itemRecord.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteItemRecordByBusinessKey(
		@WebParam(name="businessKey")String businessKey,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				itemRecord = itemRecordService.loadItemRecordByBussinessKey(businessKey);
	...
	@Override//
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteItemRecordByPk(
		@WebParam(name="pk")String pk,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddItemRecord(
		@WebParam(name="itemRecordList")List<MESBasicItemRecord> itemRecordList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			int count = 1;
			for(MESBasicItemRecord itemRecord : itemRecordList){
				if(itemRecord.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(itemRecord.getId() == null || itemRecord.getId().toString().trim().isEmpty()){
			...
			itemRecordService.batchImportBaseItemRecord(itemRecordList);
			for(MESBasicItemRecord itemRecord : itemRecordList){
				idList.add(itemRecord.getId());
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdateItemRecord(
		@WebParam(name="itemRecordList")List<MESBasicItemRecord> itemRecordList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteItemRecordByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicItemRecord temp = null;
				try{
					temp = itemRecordService.getItemRecord(id);
				}catch(NullPointerException e){}
			...
			itemRecordService.deleteItemRecord(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteItemRecordByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicItemRecord temp = null;
				try{
					temp = itemRecordService.loadItemRecordByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getItemRecordPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicItemRecord.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicItemRecord.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicItemRecord.class);
				} catch (Exception e) {
			...
			Page<MESBasicItemRecord> page = new Page<MESBasicItemRecord>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicItemRecord> datas = itemRecordService.getByPage(page, detachedCriteria);
			List<MESBasicItemRecord> itemRecordList = datas.getResult();
			if (itemRecordList == null || itemRecordList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(itemRecordList.toArray());
			PaginationDTO<MESBasicItemRecord> pagination = new PaginationDTO<MESBasicItemRecord>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicItemRecord itemRecord = itemRecordService.getItemRecord(id);
			if (itemRecord == null || !itemRecord.isValid()) {
				result.setErrorMessage("找不到位号数据统计");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取位号数据统计对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param itemRecord
	 * @return
	 */
	private MESBasicItemRecord getItemRecord(MESBasicItemRecord itemRecord) {
		if (itemRecord.getId() == null && (itemRecord.getId() == null || itemRecord.getId().toString().trim() == ""))
			throw new RuntimeException("位号数据统计对象业务主键和id不能同时为空");
		if (itemRecord.getId() != null) {
			MESBasicItemRecord result = itemRecordService.getItemRecord(itemRecord .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return itemRecordService.loadItemRecordByBussinessKey(itemRecord.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个位号数据统计，则返回false，否则返回true
	 * @param itemRecord
	 */
	private boolean beforeUpdateItemRecord(MESBasicItemRecord itemRecord) {
		MESBasicItemRecord temp = getItemRecord(itemRecord);
		if (temp == null) {
			return false;
		}
		if(itemRecord.getId() == null){
			itemRecord.setId(temp.getId());		
		}
		if(itemRecord.getId() == null){
			itemRecord.setId(temp.getId());
		}
		
		itemRecord.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除位号数据统计对象下所有辅模型对象中的位号数据统计对象
	 * @param itemRecord
	 */
	private void clearCircle(MESBasicItemRecord itemRecord) {
	 ...
	 */
	private void checkItemRecordData(MESBasicItemRecord itemRecord){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_itemRecord_ItemRecord,MESBasic_1) */
// 自定义代码
