...
import com.supcon.orchid.MESBasic.services.MESBasicObjBatContainerImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicObjBatContainerDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_objBatContainerService")
@Transactional
public class MESBasicObjBatContainerServiceImpl  extends BaseServiceImpl<MESBasicObjBatContainer> implements MESBasicObjBatContainerService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicObjBatContainer> bapBaseService;
	...
	@Autowired
	private MESBasicObjBatContainerImportService objBatContainerImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicObjBatContainer.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  objBatContainerDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicObjBatContainer.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = objBatContainerDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjBatContainer.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicObjBatContainer.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  objBatContainerDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicObjBatContainer.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicObjBatContainer.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicObjBatContainer> list = (List<MESBasicObjBatContainer>) objBatContainerDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjBatContainer getObjBatContainer(long id){
		return getObjBatContainer(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjBatContainer getObjBatContainer(long id, String viewCode){
		MESBasicObjBatContainer objBatContainer = objBatContainerDao.load(id);

		// 一对多情况处理
		if(objBatContainer != null){
		}
		return objBatContainer;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getObjBatContainerAsJSON(long id, String include){
		MESBasicObjBatContainer objBatContainer = objBatContainerDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(objBatContainer, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.common.delete",operType="3")
	public void deleteObjBatContainer(MESBasicObjBatContainer objBatContainer){
		if(null != objBatContainer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjBatContainer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
			AuditUtil.setColumnStringA(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
			AuditUtil.setColumnStringC(objBatContainer.getId().toString());
		}
		beforeDeleteObjBatContainer(objBatContainer);
		objBatContainerDao.deletePhysical(objBatContainer);
		afterDeleteObjBatContainer(objBatContainer);
		objBatContainerDao.flush();
		List<MESBasicObjBatContainer> params = new ArrayList<MESBasicObjBatContainer>();
		params.add(objBatContainer);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.common.delete",operType="3")
	public void deleteObjBatContainer(List<Long> objBatContainerIds){
		deleteObjBatContainer(objBatContainerIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.common.delete",operType="3")
	public void deleteObjBatContainer(List<Long> objBatContainerIds, String eventTopic) {
		List<MESBasicObjBatContainer> objBatContainers = new ArrayList<MESBasicObjBatContainer>();
		for(Long objBatContainerId : objBatContainerIds){
			MESBasicObjBatContainer objBatContainer = getObjBatContainer(objBatContainerId);
			objBatContainers.add(objBatContainer);
			if(null != objBatContainer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjBatContainer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == objBatContainer.getId() ? "" : "," + objBatContainer.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == objBatContainer.getId() ? "" : "," + objBatContainer.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == objBatContainer.getId() ? "" : "," + objBatContainer.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
				}
			}
		}
		
		for(MESBasicObjBatContainer objBatContainer : objBatContainers){
			beforeDeleteObjBatContainer(objBatContainer);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(objBatContainers != null && objBatContainers.size() > 0 ){
			for(MESBasicObjBatContainer objBatContainer : objBatContainers){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicObjBatContainer.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(objBatContainer));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),objBatContainer, objBatContainer.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(objBatContainerIds != null && objBatContainerIds.size() > 0) {
			String hql = "delete " + MESBasicObjBatContainer.JPA_NAME + " where id in(:ids)";
			Query query = objBatContainerDao.createQuery(hql);
			query.setParameterList("ids", objBatContainerIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicObjBatContainer objBatContainerz : objBatContainers){
				objBatContainerz.setValid(false);
				List<MESBasicObjBatContainer> params = new ArrayList<MESBasicObjBatContainer>();
				params.add(objBatContainerz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_factoryModel");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.common.delete",operType="3")
	public void deleteObjBatContainer(long objBatContainerId, int objBatContainerVersion){
		this.deleteObjBatContainer(objBatContainerId, objBatContainerVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.common.delete",operType="3")
	public void deleteObjBatContainer(long objBatContainerId, int objBatContainerVersion,SignatureLog signatureLog){
		MESBasicObjBatContainer objBatContainer = getObjBatContainer(objBatContainerId);
		if(objBatContainer != null && !objBatContainer.isValid()){
		...
		beforeDeleteObjBatContainer(objBatContainer);
		objBatContainerDao.delete(objBatContainerId, objBatContainerVersion);
		afterDeleteObjBatContainer(objBatContainer);
		objBatContainerDao.flush();
		List<MESBasicObjBatContainer> params = new ArrayList<MESBasicObjBatContainer>();
		params.add(objBatContainer);
		...
		if(signatureLog != null ) {
			...
			businessKey=objBatContainer.getId();
			signatureLog.setTableId(objBatContainer.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.common.delete",operType="3")
	public void deleteObjBatContainer(String objBatContainerIds){
		this.deleteObjBatContainer(objBatContainerIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.common.delete",operType="3")
	public void deleteObjBatContainer(String objBatContainerIds,SignatureLog signatureLog){
		deleteCollection(objBatContainerIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteObjBatContainer(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_factoryModel#modelCode:MESBasic_1_factoryModel_ObjBatContainer#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="ec.engine.restore",operType="10")
	public void restoreObjBatContainer(String objBatContainerIds){
		restoreCollection(objBatContainerIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreObjBatContainer(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="ec.engine.restore",operType="10")
	public void restoreObjBatContainer(long objBatContainerId){
		
		findBusinessKeyUsed(objBatContainerId);	//判断业务主键是否重复
		...
		beforeRestoreObjBatContainer(objBatContainer);
		objBatContainer.setValid(true);
		objBatContainerDao.update(objBatContainer);
		afterRestoreObjBatContainer(objBatContainer);
		List<MESBasicObjBatContainer> params = new ArrayList<MESBasicObjBatContainer>();
		params.add(objBatContainer);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long objBatContainerId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_factoryModel_ObjBatContainer");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicObjBatContainer.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicObjBatContainer.TABLE_NAME +" where id = ? )";
			List<Object> list =  objBatContainerDao.createNativeQuery(sql,objBatContainerId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseObjBatContainer(List<MESBasicObjBatContainer>  objBatContainers){
		for(MESBasicObjBatContainer objBatContainer:objBatContainers)  {
			saveObjBatContainer(objBatContainer, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseObjBatContainer(List<MESBasicObjBatContainer>  objBatContainers){
		for(MESBasicObjBatContainer objBatContainer:objBatContainers)  {
			saveObjBatContainer(objBatContainer, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="ec.flow.save")
	public void saveObjBatContainer(MESBasicObjBatContainer objBatContainer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveObjBatContainer(objBatContainer, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="ec.flow.save")
	public void saveObjBatContainer(MESBasicObjBatContainer objBatContainer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveObjBatContainer(objBatContainer, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateObjBatContainerCodes(MESBasicObjBatContainer objBatContainer) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjBatContainerCodes(objBatContainer, false);
	}
	
	public void generateObjBatContainerCodes(MESBasicObjBatContainer objBatContainer, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateObjBatContainerSummarys(MESBasicObjBatContainer objBatContainer) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateObjBatContainerSummarys(objBatContainer, false);
	}

	public void generateObjBatContainerSummarys(MESBasicObjBatContainer objBatContainer, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer")
	public void saveObjBatContainer(MESBasicObjBatContainer objBatContainer,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveObjBatContainer(objBatContainer, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer")
	public void saveObjBatContainer(MESBasicObjBatContainer objBatContainer,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(objBatContainer.getId() != null && objBatContainer.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjBatContainer.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, objBatContainer);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(objBatContainer);
		beforeSaveObjBatContainer(objBatContainer, viewIsView);

		if (viewIsView) {
			objBatContainerDao.saveWithRevertVersion(objBatContainer);
		} else {
			if(isNew)
				objBatContainerDao.save(objBatContainer);
			else
				objBatContainerDao.merge(objBatContainer);
		}
		if(null != objBatContainer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjBatContainer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
			AuditUtil.setColumnStringA(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
		...
		dealDatagridsSave(objBatContainer,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateObjBatContainerCodes(objBatContainer, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateObjBatContainerSummarys(objBatContainer, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveObjBatContainer(objBatContainer, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, objBatContainer);
			}
		}
		List<MESBasicObjBatContainer> params = new ArrayList<MESBasicObjBatContainer>();
		objBatContainerDao.flush();
		objBatContainerDao.clear();
		objBatContainer = objBatContainerDao.load(objBatContainer.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicObjBatContainer objBatContainer){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("objBatContainer", objBatContainer);
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer")
	public void mergeObjBatContainer(MESBasicObjBatContainer objBatContainer, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(objBatContainer);
		beforeSaveObjBatContainer(objBatContainer);
		objBatContainerDao.merge(objBatContainer);
		if(null != objBatContainer && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjBatContainer.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
			AuditUtil.setColumnStringA(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
		...
		dealDatagridsSave(objBatContainer,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveObjBatContainer(objBatContainer);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicObjBatContainer> findObjBatContainers(Page<MESBasicObjBatContainer> page, Criterion... criterions) {
		return objBatContainerDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjBatContainers(Page<MESBasicObjBatContainer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findObjBatContainers(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjBatContainers(Page<MESBasicObjBatContainer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findObjBatContainers(Page<MESBasicObjBatContainer> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicObjBatContainer> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"objBatContainer\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"objBatContainer\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"objBatContainer\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"objBatContainer\".OBJ_FACTORY IN ( SELECT ID FROM factory_tbcompanystructure \"factoryModel\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  objBatContainerDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  objBatContainerDao.createNativeQuery(powerOperateSql, "MESBasic_1_factoryModel").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"objBatContainer\".LAY_REC ASC, \"objBatContainer\".SORT ASC");
				} else {
					orderPart.append(" \"objBatContainer\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) objBatContainerDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=objBatContainerDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = objBatContainerDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicObjBatContainer> result = new ArrayList<MESBasicObjBatContainer>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjBatContainer.class,objBatContainerDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicObjBatContainer.class,objBatContainerDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicObjBatContainer objBatContainer : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(objBatContainer.getId(), "MESBasic_factoryModel_objBatContainer");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(objBatContainer.getId(), "MESBasic_factoryModel_objBatContainer");
							if(null != documents && !documents.isEmpty()) {
								objBatContainer.setDocument(documents.get(0));
								objBatContainer.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicObjBatContainer> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicObjBatContainer> result) {
		View view = null;
		...
		Map<Long, MESBasicObjBatContainer> rsMap = new HashMap<Long, MESBasicObjBatContainer>();
		for (MESBasicObjBatContainer objBatContainer : result) {
			ids.add(objBatContainer.getId());
			rsMap.put(objBatContainer.getId(), objBatContainer);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(objBatContainerDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = objBatContainerDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_factoryModel_ObjBatContainer" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_factoryModel_ObjBatContainer" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  objBatContainerDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objBatContainer\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_BAT_CONTAINERS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"objBatContainer\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"objBatContainer\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"objBatContainer\"","MESBasic_1_factoryModel","","MESBASIC_OBJ_BAT_CONTAINERS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_factoryModel_ObjBatContainer" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("objBatContainer.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"objBatContainer\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"objBatContainer\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"objBatContainer\".CID = 1 OR \"objBatContainer\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("objBatContainer.mc\"").append(".OBJ_BAT_CONTAINER) not in ("+notinValue+")");
			}
			
			SQLQuery query = objBatContainerDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=objBatContainerDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicObjBatContainer.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicObjBatContainer.MODULE_CODE;}
	protected String getModelCode(){return MESBasicObjBatContainer.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param objBatContainer 模型实例
	 */
	public void checkUniqueConstraint(MESBasicObjBatContainer objBatContainer){
	}
	
	
	@Override
	public List<MESBasicObjBatContainer> getObjBatContainers(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjBatContainer.class);
		if(params!=null){
			...
			return objBatContainerDao.findByHql("from " + MESBasicObjBatContainer.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return objBatContainerDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicObjBatContainer> findObjBatContainersByHql(String hql, Object... objects){
		...
		return   objBatContainerDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicObjBatContainer loadObjBatContainerByBussinessKey(MESBasicObjBatContainer objBatContainer){
		return loadObjBatContainerByBussinessKey(objBatContainer.getId());
	}
	
	@Override
	public Page<MESBasicObjBatContainer> getObjBatContainers(Page<MESBasicObjBatContainer> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicObjBatContainer.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = objBatContainerDao.findByPage(page, sql);
			}else{
				page = objBatContainerDao.findAllByPage(page);
			}
			return page;
		}
		page = objBatContainerDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public MESBasicObjBatContainer loadObjBatContainerByBussinessKey(Serializable bussinessKey){
		return objBatContainerDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicObjBatContainer> findByProperty(String propertyName, Object object){
		return objBatContainerDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicObjBatContainer findEntityByProperty(String propertyName, Object object){
		return objBatContainerDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicObjBatContainer.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = objBatContainerDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_factoryModel_ObjBatContainer","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicObjBatContainer objBatContainer,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,objBatContainerDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicObjBatContainer.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, objBatContainer);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",objBatContainer.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjBatContainer.TABLE_NAME +" where valid = 1";
			list = objBatContainerDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicObjBatContainer.TABLE_NAME;
			list = objBatContainerDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicObjBatContainer.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = objBatContainerDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicObjBatContainerDaoImpl objBatContainerDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchObjBatContainer(final List<MESBasicObjBatContainer> insertObjs, final List<MESBasicObjBatContainer> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchObjBatContainer(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchObjBatContainer(final List<MESBasicObjBatContainer> insertObjs, final List<MESBasicObjBatContainer> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1519650919000"),importCount));
		...
		Map<Long, MESBasicObjBatContainer> oldUpdateObjsMap = new HashMap<Long, MESBasicObjBatContainer>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicObjBatContainer bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicObjBatContainer> bizObjList = objBatContainerDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicObjBatContainer bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicObjBatContainer> nmeObjs = new ArrayList<MESBasicObjBatContainer>();
		...
		long id=autoGeneratorID.getNextId(MESBasicObjBatContainer.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicObjBatContainer objBatContainer:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicObjBatContainer.TABLE_NAME,1,dbId);
			if(objBatContainer.getId() != null ){
				m.put(objBatContainer.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			objBatContainer.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) objBatContainerDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjBatContainer"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		objBatContainerDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicObjBatContainer.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicObjBatContainer objBatContainer = updateObjs.get(updateIndex);
						Long id = objBatContainer.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = objBatContainer.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(objBatContainer);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicObjBatContainer.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicObjBatContainer.TABLE_NAME + " order by id desc";
//					Query query = objBatContainerDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicObjBatContainer objBatContainer = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = objBatContainer.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		objBatContainerDaoImpl.getSession().flush();
		objBatContainerDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicObjBatContainer bizObj = insertObjs.get(insertIndex);
			objBatContainerImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicObjBatContainer bizObj = updateObjs.get(updateIndex);
			MESBasicObjBatContainer oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			objBatContainerImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicObjBatContainer> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicObjBatContainer> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) objBatContainerDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicObjBatContainer> result = getResult(page, query, new PendingResultTransformer(MESBasicObjBatContainer.class,objBatContainerDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicObjBatContainer model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = objBatContainerDao.createNativeQuery(sql, "MESBasic_1_factoryModel_ObjBatContainer").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = objBatContainerDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = objBatContainerDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = objBatContainerDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?objBatContainerDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  objBatContainerDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,objBatContainerDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="ec.flow.save")
	public void saveObjBatContainer(MESBasicObjBatContainer objBatContainer, DataGridService dataGridService){
		saveObjBatContainer(objBatContainer, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="ec.flow.save")
	public void saveObjBatContainer(MESBasicObjBatContainer objBatContainer, DataGridService dataGridService, String viewCode){
		saveObjBatContainer(objBatContainer, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer")
	public void saveObjBatContainer(MESBasicObjBatContainer objBatContainer, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveObjBatContainer(objBatContainer, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer")
	public void saveObjBatContainer(MESBasicObjBatContainer objBatContainer, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_factoryModel";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_factoryModel");
		String url = null;
		if(null != objBatContainer.getId() && objBatContainer.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicObjBatContainer.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer")
	public Map<Object, Object> objBatContainerDataGridImport(MESBasicObjBatContainer objBatContainer, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = objBatContainer.getVirtualId();
		
		objBatContainer = objBatContainerDao.load(objBatContainer.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, objBatContainer.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = objBatContainer.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(objBatContainer);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, objBatContainer.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer")
	public void mergeObjBatContainer(MESBasicObjBatContainer objBatContainer, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(objBatContainer);
		beforeSaveObjBatContainer(objBatContainer);
		objBatContainerDao.merge(objBatContainer);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_factoryModel",model="MESBasic_1_factoryModel_ObjBatContainer",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicObjBatContainer> getByPage(Page<MESBasicObjBatContainer> page,DetachedCriteria detachedCriteria){
		return objBatContainerDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveObjBatContainer(MESBasicObjBatContainer objBatContainer, Object... objects){
		checkUniqueConstraint(objBatContainer);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveObjBatContainer(MESBasicObjBatContainer objBatContainer, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteObjBatContainer(MESBasicObjBatContainer objBatContainer){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteObjBatContainer(MESBasicObjBatContainer objBatContainer){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreObjBatContainer(MESBasicObjBatContainer objBatContainer){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreObjBatContainer(MESBasicObjBatContainer objBatContainer){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicObjBatContainer objBatContainer, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicObjBatContainer objBatContainer, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportObjBatContainer(List<MESBasicObjBatContainer> insertObjs, List<MESBasicObjBatContainer> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportObjBatContainer,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportObjBatContainer(List<MESBasicObjBatContainer> insertObjs, List<MESBasicObjBatContainer> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportObjBatContainer,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
