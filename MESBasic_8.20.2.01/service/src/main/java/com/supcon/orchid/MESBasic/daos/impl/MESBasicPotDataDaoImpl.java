...
@Repository
public class MESBasicPotDataDaoImpl extends ExtendedGenericDaoImpl<MESBasicPotData, Long> implements MESBasicPotDataDao {
	@Override
	...
	@Override
	public MESBasicPotDataDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicPotDataDealInfo dealInfo = new MESBasicPotDataDealInfo();
	...
	@Override
	public MESBasicPotDataDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicPotData mainObj, Staff dealStaff) {
		MESBasicPotDataDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicPotDataDealInfo.class);
		...
		List<MESBasicPotDataDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_potDataHandler_PotData,MESBasic_1) */
// 自定义代码
