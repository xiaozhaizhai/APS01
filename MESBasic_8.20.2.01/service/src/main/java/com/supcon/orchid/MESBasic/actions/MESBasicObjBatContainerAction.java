...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicObjBatContainerAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicObjBatContainer> {
	...
	private MESBasicObjBatContainer objBatContainer;
	...
	//导出excel时,主模型内容
	List<MESBasicObjBatContainer> queryResults = new ArrayList<MESBasicObjBatContainer>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objBatContainer/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objBatContainer/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryModel_ObjBatContainer" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicObjBatContainer> objBatContainers;
	
	public List<MESBasicObjBatContainer> getObjBatContainers() {
		return objBatContainers;
	}
	...
	private Page<MESBasicObjBatContainer> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			objBatContainer = new MESBasicObjBatContainer();
		} else {
			if(id > 0){
				objBatContainer = objBatContainerService.getObjBatContainer(id, viewCode);
			}
		}
	...
	public Page<MESBasicObjBatContainer> getPage() {
		if (null == page) {
			page = new Page<MESBasicObjBatContainer>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicObjBatContainer getObjBatContainer() {
		return objBatContainer;
	}		

	public void setObjBatContainer(MESBasicObjBatContainer objBatContainer) {
		this.objBatContainer = objBatContainer;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objBatContainer/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = objBatContainerService.getObjBatContainerAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objBatContainer/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1519650919000
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String objBatContainerReferenceCopy() throws Exception{
			MESBasicObjBatContainer oldObjBatContainer = null;
			if(isEditNew()){
				...
				oldObjBatContainer = new MESBasicObjBatContainer();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objBatContainer.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, objBatContainer.getId());
				...
				oldObjBatContainer = objBatContainerService.getObjBatContainer(id);
			}
			
			
			objBatContainer = objBatContainerService.getObjBatContainer(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjBatContainer");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicObjBatContainer.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldObjBatContainer, null);
						Method setValue = MESBasicObjBatContainer.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(objBatContainer, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			objBatContainer.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=objBatContainerService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return objBatContainerReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			objBatContainer = objBatContainerService.getObjBatContainer(this.objBatContainer.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objBatContainer.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			objBatContainerService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = objBatContainerService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(objBatContainerService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(objBatContainerService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryModel_ObjBatContainer", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"objBatContainer\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"objBatContainer\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"objBatContainer\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"objBatContainer\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			objBatContainerService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		objBatContainerService.findDataGridPage(dg,dgclass,getDgPage(),objBatContainer,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(objBatContainer.getId() != null && objBatContainer.getId() != -1){
				objBatContainer = objBatContainerService.getObjBatContainer(objBatContainer.getId());
			}
			objBatContainerService.findDataGridPage(dg,dgclass,getDgPage(),objBatContainer,searchCondition,params);
	...
	private void save() throws Exception {
		...
		objBatContainerService.saveObjBatContainer(objBatContainer,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "submit");
		bap_validate_datagrids=objBatContainerService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", objBatContainer.getId());
		...
		fileuploads.put("linkId", objBatContainer.getId());
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryModel/objBatContainer/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		objBatContainerService.deleteObjBatContainer(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objBatContainer/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "restore");
		objBatContainerService.restoreObjBatContainer(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicObjBatContainer> findExportDataInfos(Page<MESBasicObjBatContainer> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = objBatContainerService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = objBatContainerService.getAssProperty(p.getCode());	
									//String assModelCode = objBatContainerService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			objBatContainerService.findObjBatContainers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			objBatContainerService.findObjBatContainers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicObjBatContainer> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicObjBatContainer result : results){
	...
	public boolean isEditNew(){
		//return null == this.objBatContainer || null == this.objBatContainer.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryModel/objBatContainer/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryModel/objBatContainer/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryModel/objBatContainer/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objBatContainer/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicObjBatContainerService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objBatContainer/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryModel_ObjBatContainer", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryModel_ObjBatContainer", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryModel/objBatContainer/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryModel/objBatContainer/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryModel_ObjBatContainer";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjBatContainer", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryModel/objBatContainer/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicObjBatContainer objBatContainer = null;
		if(null != id) {
			objBatContainer = objBatContainerService.getObjBatContainer(id);
		}
		...
		_auditLog.setColumnStringB(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
		_auditLog.setColumnStringA(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
		_auditLog.setColumnStringC(objBatContainer.getId().toString());
		_auditLog.setColumnStringD(MESBasicObjBatContainer.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicObjBatContainerAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, objBatContainer,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, objBatContainer,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,objBatContainer);
		...
		printlog.setBusinessId(objBatContainer.getId());
		printlog.setBussinessKey(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
		printlog.setMainDisplay(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryModel/objBatContainer/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(objBatContainer,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objBatContainer/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(objBatContainerService.getObjBatContainer(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryModel/objBatContainer/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryModel/objBatContainer/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, objBatContainer,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objBatContainer = objBatContainerService.getObjBatContainer(Long.parseLong(arrId[i]));
				if(null != objBatContainer){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == objBatContainer.getId() ? "" : "," + objBatContainer.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == objBatContainer.getId() ? "" : "," + objBatContainer.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == objBatContainer.getId() ? "" : "," + objBatContainer.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == objBatContainer.getId() ? "" : objBatContainer.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, objBatContainer,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,objBatContainer);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//objBatContainerService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/MESBasic/factoryModel/objBatContainer/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjBatContainer");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryModel_ObjBatContainer")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryModel_ObjBatContainer");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjBatContainer");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = objBatContainerService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryModel_ObjBatContainer,MESBasic_1) */
// 自定义代码
