...
import com.supcon.orchid.MESBasic.services.MESBasicConfigImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicConfigDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

import com.supcon.mlp.lib.util.StringUtil;
/* CUSTOM CODE END */

@Service("mESBasic_configService")
@Transactional
public class MESBasicConfigServiceImpl  extends BaseServiceImpl<MESBasicConfig> implements MESBasicConfigService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicConfig> bapBaseService;
	...
	@Autowired
	private MESBasicConfigImportService configImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicConfig.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  configDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicConfig.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = configDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicConfig.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicConfig.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  configDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicConfig.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicConfig.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicConfig> list = (List<MESBasicConfig>) configDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicConfig getConfig(long id, String viewCode){
		MESBasicConfig config = configDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getConfigAsJSON(long id, String include){
		MESBasicConfig config = configDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(config, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.common.delete",operType="3")
	public void deleteConfig(MESBasicConfig config){
		if(null != config && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicConfig.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == config.getId() ? "" : config.getId().toString());
			AuditUtil.setColumnStringA(null == config.getName() ? "" : config.getName().toString());
			AuditUtil.setColumnStringC(config.getId().toString());
		}
		beforeDeleteConfig(config);
		configDao.deletePhysical(config);
		afterDeleteConfig(config);
		configDao.flush();
		List<MESBasicConfig> params = new ArrayList<MESBasicConfig>();
		params.add(config);
	...
	public void deleteConfig(List<Long> configIds){
		deleteConfig(configIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.common.delete",operType="3")
	public void deleteConfig(List<Long> configIds, String eventTopic) {
		List<MESBasicConfig> configs = new ArrayList<MESBasicConfig>();
		for(Long configId : configIds){
			MESBasicConfig config = getConfig(configId);
			configs.add(config);
			if(null != config && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicConfig.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == config.getName() ? "" : "," + config.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == config.getName() ? "" : config.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == config.getId() ? "" : "," + config.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == config.getId() ? "" : config.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == config.getId() ? "" : "," + config.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == config.getId() ? "" : config.getId().toString());
				}
			}
		}
		...
		if(configs != null && configs.size() > 0 ){
			for(MESBasicConfig config : configs){
				...
				java.lang.reflect.Field[] fields = MESBasicConfig.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(config));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),config, config.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(configIds != null && configIds.size() > 0) {
			String hql = "delete " + MESBasicConfig.JPA_NAME + " where id in(:ids)";
			Query query = configDao.createQuery(hql);
			query.setParameterList("ids", configIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicConfig configz : configs){
				configz.setValid(false);
				List<MESBasicConfig> params = new ArrayList<MESBasicConfig>();
				params.add(configz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.common.delete",operType="3")
	public void deleteConfig(long configId, int configVersion){
		this.deleteConfig(configId, configVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.common.delete",operType="3")
	public void deleteConfig(long configId, int configVersion,SignatureLog signatureLog){
		MESBasicConfig config = getConfig(configId);
		...
		beforeDeleteConfig(config);
		configDao.delete(configId, configVersion);
		afterDeleteConfig(config);
		configDao.flush();
		List<MESBasicConfig> params = new ArrayList<MESBasicConfig>();
		params.add(config);
		...
		if(signatureLog != null ) {
			...
			businessKey=config.getId();
			signatureLog.setTableId(config.getId());
			if(businessKey != null) {
	...
	public void deleteConfig(String configIds){
		this.deleteConfig(configIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.common.delete",operType="3")
	public void deleteConfig(String configIds,SignatureLog signatureLog){
		deleteCollection(configIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteConfig(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_routine#modelCode:MESBasic_1_routine_Config#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="ec.engine.restore",operType="10")
	public void restoreConfig(String configIds){
		restoreCollection(configIds);
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="ec.engine.restore",operType="10")
	public void restoreConfig(long configId){
		
		findBusinessKeyUsed(configId);	//判断业务主键是否重复
		...
		beforeRestoreConfig(config);
		config.setValid(true);
		configDao.update(config);
		afterRestoreConfig(config);
		List<MESBasicConfig> params = new ArrayList<MESBasicConfig>();
		params.add(config);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long configId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_routine_Config");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicConfig.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicConfig.TABLE_NAME +" where id = ? )";
			List<Object> list =  configDao.createNativeQuery(sql,configId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseConfig(List<MESBasicConfig>  configs){
		for(MESBasicConfig config:configs)  {
			saveConfig(config, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseConfig(List<MESBasicConfig>  configs){
		for(MESBasicConfig config:configs)  {
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="ec.flow.save")
	public void saveConfig(MESBasicConfig config, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveConfig(config, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="ec.flow.save")
	public void saveConfig(MESBasicConfig config, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveConfig(config, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateConfigCodes(MESBasicConfig config) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateConfigCodes(config, false);
	}
	
	public void generateConfigCodes(MESBasicConfig config, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateConfigSummarys(MESBasicConfig config) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateConfigSummarys(config, false);
	}

	public void generateConfigSummarys(MESBasicConfig config, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config")
	public void saveConfig(MESBasicConfig config,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveConfig(config, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config")
	public void saveConfig(MESBasicConfig config,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		if(config.getId() != null && config.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicConfig.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, config);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(config);
		beforeSaveConfig(config, viewIsView);

		if (viewIsView) {
			configDao.saveWithRevertVersion(config);
		} else {
			if(isNew)
		...
		dealDatagridsSave(config,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateConfigCodes(config, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateConfigSummarys(config, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveConfig(config, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, config);
			}
		}
		List<MESBasicConfig> params = new ArrayList<MESBasicConfig>();
		configDao.flush();
		configDao.clear();
		config = configDao.load(config.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicConfig config){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config")
	public void mergeConfig(MESBasicConfig config, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(config);
		beforeSaveConfig(config);
		configDao.merge(config);
		if(null != config && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicConfig.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == config.getId() ? "" : config.getId().toString());
			AuditUtil.setColumnStringA(null == config.getName() ? "" : config.getName().toString());
		...
		dealDatagridsSave(config,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveConfig(config);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicConfig> findConfigs(Page<MESBasicConfig> page, Criterion... criterions) {
		return configDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findConfigs(Page<MESBasicConfig> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findConfigs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findConfigs(Page<MESBasicConfig> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findConfigs(Page<MESBasicConfig> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicConfig> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"config\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"config\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"config\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"config\".ROUTINE IN ( SELECT ID FROM MESBASIC_ROUTINES \"routine\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  configDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  configDao.createNativeQuery(powerOperateSql, "MESBasic_1_routine").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"config\".LAY_REC ASC, \"config\".SORT ASC");
				} else {
					orderPart.append(" \"config\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) configDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=configDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = configDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicConfig> result = new ArrayList<MESBasicConfig>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicConfig.class,configDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicConfig.class,configDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicConfig config : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(config.getId(), "MESBasic_routine_config");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(config.getId(), "MESBasic_routine_config");
							if(null != documents && !documents.isEmpty()) {
								config.setDocument(documents.get(0));
								config.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicConfig> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicConfig> result) {
		View view = null;
		...
		Map<Long, MESBasicConfig> rsMap = new HashMap<Long, MESBasicConfig>();
		for (MESBasicConfig config : result) {
			ids.add(config.getId());
			rsMap.put(config.getId(), config);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(configDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = configDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_routine_Config" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_routine_Config" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  configDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"config\"","MESBasic_1_routine","","MESBASIC_CONFIGS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"config\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"config\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"config\"","MESBasic_1_routine","","MESBASIC_CONFIGS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_routine_Config" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("config.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"config\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("config.mc\"").append(".CONFIG) not in ("+notinValue+")");
			}
			
			SQLQuery query = configDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=configDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicConfig.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicConfig.MODULE_CODE;}
	protected String getModelCode(){return MESBasicConfig.MODEL_CODE;}
	 ...
	 * @param config 模型实例
	 */
	public void checkUniqueConstraint(MESBasicConfig config){
	}
	
	
	@Override
	public List<MESBasicConfig> getConfigs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicConfig.class);
		if(params!=null){
			...
			return configDao.findByHql("from " + MESBasicConfig.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return configDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicConfig> findConfigsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicConfig.class);
	...
	@Override
	public List<MESBasicConfig> findConfigsByHql(String hql, Object... objects){
		
		return   configDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicConfig loadConfigByBussinessKey(MESBasicConfig config){
		return loadConfigByBussinessKey(config.getId());
	}
	
	@Override
	public Page<MESBasicConfig> getConfigs(Page<MESBasicConfig> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicConfig.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = configDao.findByPage(page, sql);
			}else{
				page = configDao.findAllByPage(page);
			}
			return page;
		}
		page = configDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicConfig> findByTreeDataGridRoutine(List<MESBasicConfig> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicConfig config : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridRoutine(Page<MESBasicConfig> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicConfig> configs = dgPage.getResult();
			for(MESBasicConfig config : configs){
				
			}
			dgPage.setResult(configs);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicConfig loadConfigByBussinessKey(Serializable bussinessKey){
		return configDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicConfig> findByProperty(String propertyName, Object object){
		return configDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicConfig findEntityByProperty(String propertyName, Object object){
		return configDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicConfig.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = configDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_routine_Config","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicConfig config,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,configDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicConfig.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, config);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",config.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicConfig.TABLE_NAME +" where valid = 1";
			list = configDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicConfig.TABLE_NAME;
			list = configDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicConfig.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = configDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicConfigDaoImpl configDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchConfig(final List<MESBasicConfig> insertObjs, final List<MESBasicConfig> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchConfig(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchConfig(final List<MESBasicConfig> insertObjs, final List<MESBasicConfig> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1415337833221"),importCount));
		...
		Map<Long, MESBasicConfig> oldUpdateObjsMap = new HashMap<Long, MESBasicConfig>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicConfig bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicConfig> bizObjList = configDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicConfig bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicConfig> nmeObjs = new ArrayList<MESBasicConfig>();
		...
		long id=autoGeneratorID.getNextId(MESBasicConfig.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicConfig config:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicConfig.TABLE_NAME,1,dbId);
			if(config.getId() != null ){
				m.put(config.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) configDao.createNativeQuery(sql, "MESBasic_1_routine_Config"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		configDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicConfig.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicConfig config = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = config.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicConfig.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicConfig.TABLE_NAME + " order by id desc";
//					Query query = configDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicConfig config = insertObjs.get(insertIndex);
		...
		configDaoImpl.getSession().flush();
		configDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicConfig bizObj = insertObjs.get(insertIndex);
			configImportService.saveImportDataLog(bizObj, null, "id", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicConfig bizObj = updateObjs.get(updateIndex);
			MESBasicConfig oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			configImportService.saveImportDataLog(bizObj, oldBizObj, "id", "name");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicConfig> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicConfig> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) configDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicConfig> result = getResult(page, query, new PendingResultTransformer(MESBasicConfig.class,configDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicConfig model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = configDao.createNativeQuery(sql, "MESBasic_1_routine_Config").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = configDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = configDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = configDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?configDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  configDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,configDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="ec.flow.save")
	public void saveConfig(MESBasicConfig config, DataGridService dataGridService){
		saveConfig(config, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="ec.flow.save")
	public void saveConfig(MESBasicConfig config, DataGridService dataGridService, String viewCode){
		saveConfig(config, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config")
	public void saveConfig(MESBasicConfig config, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveConfig(config, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config")
	public void saveConfig(MESBasicConfig config, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config")
	public Map<Object, Object> configDataGridImport(MESBasicConfig config, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = config.getVirtualId();
		
		config = configDao.load(config.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, config.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = config.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(config);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, config.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config")
	public void mergeConfig(MESBasicConfig config, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(config);
		beforeSaveConfig(config);
		configDao.merge(config);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_routine",model="MESBasic_1_routine_Config",desc="foundation.batch.save",operType="4")
	public void batchSaveConfig(MESBasicConfig config, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_routine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_routine");
	...
	@Override
	public Page<MESBasicConfig> getByPage(Page<MESBasicConfig> page,DetachedCriteria detachedCriteria){
		return configDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveConfig(MESBasicConfig config, Object... objects){
		checkUniqueConstraint(config);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveConfig(MESBasicConfig config, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
		MESBasicRoutine routine = config.getRoutine();
		//先删除所有老的与当前路径相关的测点
		List<MESBasicInvolveItemsetups> oldInvoledList = involveItemsetupsDao.findByCriteria(Restrictions.eq("routine", routine));
		involveItemsetupsDao.deleteAll(oldInvoledList);
		
		Set<Long> involvedItemSet = new HashSet<Long>();
		//根据路径的测量配置，自动配置与当前路径相关的测点
		if(routine != null){
			List<MESBasicConfig> configList = configDao.findByCriteria(Restrictions.eq("routine", routine));
			StringBuilder sb = new StringBuilder();
			for(MESBasicConfig configItem : configList){
				String formula = configItem.getFormula();
				Set<String> codes = null;
				if (StringUtil.isNotEmptyOrNull(formula)) {
					codes = StringUtil.getCodes(formula);
					for (String s : codes) {
						MESBasicItemSetup itemSetupInCaculate = itemSetupDao.findEntityByCriteria(Restrictions.eq("code", s),Restrictions.eq("valid", true));
						if(itemSetupInCaculate == null){
							if(sb.length() > 0){
								sb.append("</ br>");
							}
							sb.append("计量仪表配置中，名称为\"" + configItem.getName() + "\"的配种中，编码为[" + s + "]的测点不存在。");
							continue;
						}
						if(!involvedItemSet.contains(itemSetupInCaculate.getId())){
							MESBasicInvolveItemsetups involvedItem = new MESBasicInvolveItemsetups();
							involvedItem.setRoutine(routine);
							involvedItem.setItemSetup(itemSetupInCaculate);
							involveItemsetupsDao.save(involvedItem);
							involvedItemSet.add(itemSetupInCaculate.getId());
						}
					}
				}
			}
			if(sb.length() > 0){
				throw new BAPException(sb.toString());
			}
		}
/* CUSTOM CODE END */
	}
	...
	@Override
	public void beforeExcelImportConfig(List<MESBasicConfig> configs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportConfig(List<MESBasicConfig> configs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportConfig(List<MESBasicConfig> configs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportConfig(List<MESBasicConfig> configs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteConfig(MESBasicConfig config){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteConfig(MESBasicConfig config){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreConfig(MESBasicConfig config){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreConfig(MESBasicConfig config){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicConfig config, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicConfig config, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportConfig(List<MESBasicConfig> insertObjs, List<MESBasicConfig> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportConfig,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportConfig(List<MESBasicConfig> insertObjs, List<MESBasicConfig> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportConfig,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_routine_Config,MESBasic_1) */
// 自定义代码
