...
/* CUSTOM CODE START(action,import,MESBasic_1_nodeType_NodeType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicNodeTypeAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicNodeType> {
	...
	//导出excel时,主模型内容
	List<MESBasicNodeType> queryResults = new ArrayList<MESBasicNodeType>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/nodeType/nodeType/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/nodeType/nodeType/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_nodeType_NodeType" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicNodeType> nodeTypes;
	
	public List<MESBasicNodeType> getNodeTypes() {
		return nodeTypes;
	}
	...
	private Page<MESBasicNodeType> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			nodeType = new MESBasicNodeType();
		} else {
			if(id > 0){
				nodeType = nodeTypeService.getNodeType(id, viewCode);
			}
		}
	...
	public Page<MESBasicNodeType> getPage() {
		if (null == page) {
			page = new Page<MESBasicNodeType>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/nodeType/nodeType/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = nodeTypeService.getNodeTypeAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/nodeType/nodeType/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String nodeTypeReferenceCopy() throws Exception{
			MESBasicNodeType oldNodeType = null;
			if(isEditNew()){
				...
				oldNodeType = new MESBasicNodeType();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,nodeType.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, nodeType.getId());
				...
				oldNodeType = nodeTypeService.getNodeType(id);
			}
			
			
			nodeType = nodeTypeService.getNodeType(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_nodeType_NodeType");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicNodeType.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldNodeType, null);
						Method setValue = MESBasicNodeType.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(nodeType, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			nodeType.setCreateDepartmentId(oldNodeType.getCreateDepartmentId());
			nodeType.setCreatePositionId(oldNodeType.getCreatePositionId());
			nodeType.setCreateStaffId(oldNodeType.getCreateStaffId());
			nodeType.setOwnerDepartmentId(oldNodeType.getOwnerDepartmentId());
			nodeType.setOwnerPositionId(oldNodeType.getOwnerPositionId());
			nodeType.setOwnerStaffId(oldNodeType.getOwnerStaffId());
			nodeType.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_nodeType_NodeType", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/nodeType/nodeType/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/MESBasic/nodeType/nodeType/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=nodeTypeService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return nodeTypeReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			nodeType = nodeTypeService.getNodeType(this.nodeType.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,nodeType.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/nodeType/nodeType/nodeview",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "nodeview.ftl") }),
		@Action(value = "/MESBasic/nodeType/nodeType/nodeviewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "nodeviewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/nodeType/nodeType/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/nodeType/nodeType/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = nodeTypeService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(nodeTypeService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(nodeTypeService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_nodeType_NodeType", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/nodeType/nodeType/noderef", results = { @Result(type = FREEMARKER, location = "noderef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/nodeType/nodeType/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.nodeComment,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"nodeType\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"nodeType\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"nodeType\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"nodeType\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/nodeType/nodeType/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/nodeType/nodeType/noderef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.nodeComment,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="noderef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/nodeType/nodeType/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_nodeType_NodeType");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		nodeTypeService.findDataGridPage(dg,dgclass,getDgPage(),nodeType,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(nodeType.getId() != null && nodeType.getId() != -1){
				nodeType = nodeTypeService.getNodeType(nodeType.getId());
			}
			nodeTypeService.findDataGridPage(dg,dgclass,getDgPage(),nodeType,searchCondition,params);
	...
	private void save() throws Exception {
		...
		nodeTypeService.saveNodeType(nodeType,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/nodeType/nodeType/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/nodeType/nodeType/nodeview/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "submit");
		bap_validate_datagrids=nodeTypeService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", nodeType.getId());
		...
		fileuploads.put("linkId", nodeType.getId());
		bapAfterCustomMethod("MESBasic_1_nodeType_NodeType", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/nodeType/nodeType/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		nodeTypeService.deleteNodeType(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_nodeType_NodeType", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/nodeType/nodeType/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "restore");
		nodeTypeService.restoreNodeType(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_nodeType_NodeType", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/nodeType/nodeType/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/nodeType/nodeType/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "showDealInfoData");
		if (null != tableInfoId) {
			int count=nodeTypeService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = nodeTypeService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = nodeTypeService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_nodeType_NodeType", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicNodeType> findExportDataInfos(Page<MESBasicNodeType> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = nodeTypeService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = nodeTypeService.getAssProperty(p.getCode());	
									//String assModelCode = nodeTypeService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			nodeTypeService.findNodeTypes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			nodeTypeService.findNodeTypes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicNodeType> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicNodeType result : results){
	...
	public boolean isEditNew(){
		//return null == this.nodeType || null == this.nodeType.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/nodeType/nodeType/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_nodeType_NodeType", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/nodeType/nodeType/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/nodeType/nodeType/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/nodeType/nodeType/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicNodeTypeService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_nodeType", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_nodeType");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_nodeType_NodeType", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/nodeType/nodeType/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_nodeType_NodeType", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_nodeType_NodeType", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/nodeType/nodeType/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/nodeType/nodeType/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_nodeType_NodeType";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_nodeType_NodeType", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/nodeType/nodeType/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		if(null != id) {
			nodeType = nodeTypeService.getNodeType(id);
		}
		...
		_auditLog.setColumnStringB(null == nodeType.getCode() ? "" : nodeType.getCode().toString());
		_auditLog.setColumnStringA(null == nodeType.getName() ? "" : nodeType.getName().toString());
		_auditLog.setColumnStringC(nodeType.getId().toString());
		_auditLog.setColumnStringD(MESBasicNodeType.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_nodeType");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicNodeTypeAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, nodeType,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, nodeType,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,nodeType);
		...
		printlog.setEntity("MESBasic_1_nodeType");
		...
		printlog.setBusinessId(nodeType.getId());
		printlog.setBussinessKey(null == nodeType.getCode() ? "" : nodeType.getCode().toString());
		printlog.setMainDisplay(null == nodeType.getName() ? "" : nodeType.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/nodeType/nodeType/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(nodeType,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/nodeType/nodeType/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(nodeTypeService.getNodeType(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/nodeType/nodeType/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/nodeType/nodeType/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, nodeType,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				nodeType = nodeTypeService.getNodeType(Long.parseLong(arrId[i]));
				if(null != nodeType){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == nodeType.getName() ? "" : "," + nodeType.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == nodeType.getName() ? "" : nodeType.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == nodeType.getCode() ? "" : "," + nodeType.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == nodeType.getCode() ? "" : nodeType.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == nodeType.getId() ? "" : "," + nodeType.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == nodeType.getId() ? "" : nodeType.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, nodeType,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,nodeType);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/nodeType/nodeType/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicNodeType> findExportPageInfos(Page<MESBasicNodeType> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_nodeType" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = nodeTypeService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicNodeType queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = nodeTypeService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = nodeTypeService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_nodeType_NodeType");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_nodeType_NodeType")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_nodeType_NodeType");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_nodeType_NodeType");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = nodeTypeService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_nodeType_NodeType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_nodeType_NodeType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_nodeType_NodeType,MESBasic_1) */
// 自定义代码
