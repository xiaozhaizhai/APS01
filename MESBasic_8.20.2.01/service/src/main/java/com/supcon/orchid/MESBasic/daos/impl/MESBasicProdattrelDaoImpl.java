...
@Repository
public class MESBasicProdattrelDaoImpl extends ExtendedGenericDaoImpl<MESBasicProdattrel, Long> implements MESBasicProdattrelDao {
	@Override
	...
	@Override
	public MESBasicProdattrelDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicProdattrelDealInfo dealInfo = new MESBasicProdattrelDealInfo();
	...
	@Override
	public MESBasicProdattrelDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicProdattrel mainObj, Staff dealStaff) {
		MESBasicProdattrelDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicProdattrelDealInfo.class);
		...
		List<MESBasicProdattrelDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,MESBasic_1_prodatriel_Prodattrel,MESBasic_1) */
// 自定义代码
