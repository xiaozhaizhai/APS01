...
@Service("mESBasicStoreSetImportService")
public class MESBasicStoreSetImportServiceImpl implements MESBasicStoreSetImportService {
	...
	@Override
	@AuditLog(entity="MESBasic_1_storeSet",model="MESBasic_1_storeSet_StoreSet",desc="保存业务信息",operType="1")
	public void saveImportDataLog(MESBasicStoreSet bizObj, MESBasicStoreSet oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("MESBasic.entityname.randon1489731295689") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getPlaceSetName() ? "" : "," + oldBizObj.getPlaceSetName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getPlaceSetName() ? "" : oldBizObj.getPlaceSetName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getPlaceSetCode() ? "" : "," + oldBizObj.getPlaceSetCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getPlaceSetCode() ? "" : oldBizObj.getPlaceSetCode().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("MESBasic.entityname.randon1489731295689") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getPlaceSetName() ? "" : "," + bizObj.getPlaceSetName().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getPlaceSetName() ? "" : bizObj.getPlaceSetName().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getPlaceSetCode() ? "" : "," + bizObj.getPlaceSetCode().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getPlaceSetCode() ? "" : bizObj.getPlaceSetCode().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(MESBasicStoreSet bizObj, String bussinesskeyFieldName,
