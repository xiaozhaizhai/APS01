...
import com.supcon.orchid.MESBasic.services.MESBasicEamTypeWSService;
import com.supcon.orchid.MESBasic.services.MESBasicEamTypeService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicEamType",targetNamespace = "http://ws.supcon.com")
public class MESBasicEamTypeWSServiceImpl implements MESBasicEamTypeWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getEamTypeByBusinessKey(
		...
		try{
			...
			MESBasicEamType eamType = eamTypeService.loadEamTypeByBussinessKey(businessKey);
			if(eamType == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, eamType);
		}catch(Throwable t){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getEamTypeByPk(
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO updateEamType(
		@WebParam(name="eamType")MESBasicEamType eamType,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
			
		JWSResultDTO result = new JWSResultDTO();
		try{
			if(eamType == null){
				return set500Error(result, "设备类型不能为空。");
			}
			...
			if(!beforeUpdateEamType(eamType)){
				return set404Error(result);
			}
			eamTypeService.saveEamType(eamType, null);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO addEamType(
		@WebParam(name="eamType")MESBasicEamType eamType,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				checkEamTypeData(eamType);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			eamTypeService.saveEamType(eamType, null);
			setSuccessResult(result, eamType.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteEamTypeByBusinessKey(
		@WebParam(name="businessKey")String businessKey,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				eamType = eamTypeService.loadEamTypeByBussinessKey(businessKey);
	...
	@Override//
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteEamTypeByPk(
		@WebParam(name="pk")String pk,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddEamType(
		@WebParam(name="eamTypeList")List<MESBasicEamType> eamTypeList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			int count = 1;
			for(MESBasicEamType eamType : eamTypeList){
				if(eamType.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(eamType.getCode() == null || eamType.getCode().toString().trim().isEmpty()){
			...
			eamTypeService.batchImportBaseEamType(eamTypeList);
			for(MESBasicEamType eamType : eamTypeList){
				idList.add(eamType.getId());
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdateEamType(
		@WebParam(name="eamTypeList")List<MESBasicEamType> eamTypeList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteEamTypeByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicEamType temp = null;
				try{
					temp = eamTypeService.getEamType(id);
				}catch(NullPointerException e){}
			...
			eamTypeService.deleteEamType(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteEamTypeByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicEamType temp = null;
				try{
					temp = eamTypeService.loadEamTypeByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getEamTypePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicEamType.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicEamType.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicEamType.class);
				} catch (Exception e) {
			...
			Page<MESBasicEamType> page = new Page<MESBasicEamType>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicEamType> datas = eamTypeService.getByPage(page, detachedCriteria);
			List<MESBasicEamType> eamTypeList = datas.getResult();
			if (eamTypeList == null || eamTypeList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(eamTypeList.toArray());
			PaginationDTO<MESBasicEamType> pagination = new PaginationDTO<MESBasicEamType>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicEamType eamType = eamTypeService.getEamType(id);
			if (eamType == null || !eamType.isValid()) {
				result.setErrorMessage("找不到设备类型");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取设备类型对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param eamType
	 * @return
	 */
	private MESBasicEamType getEamType(MESBasicEamType eamType) {
		if (eamType.getId() == null && (eamType.getCode() == null || eamType.getCode().toString().trim() == ""))
			throw new RuntimeException("设备类型对象业务主键和id不能同时为空");
		if (eamType.getId() != null) {
			MESBasicEamType result = eamTypeService.getEamType(eamType .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return eamTypeService.loadEamTypeByBussinessKey(eamType.getCode());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个设备类型，则返回false，否则返回true
	 * @param eamType
	 */
	private boolean beforeUpdateEamType(MESBasicEamType eamType) {
		if(eamType.getName() == null){
			throw new RuntimeException("name不能为空。");
		}
		MESBasicEamType temp = getEamType(eamType);
		if (temp == null) {
			return false;
		}
		if(eamType.getId() == null){
			eamType.setId(temp.getId());		
		}
		if(eamType.getCode() == null){
			eamType.setCode(temp.getCode());
		}
		
		eamType.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除设备类型对象下所有辅模型对象中的设备类型对象
	 * @param eamType
	 */
	private void clearCircle(MESBasicEamType eamType) {
	 ...
	 */
	private void checkEamTypeData(MESBasicEamType eamType){
		 if(eamType.getName() == null){
		 	throw new RuntimeException("name不能为空。");
		 }
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
