...
@Repository
public class MESBasicMaterialDaoImpl extends ExtendedGenericDaoImpl<MESBasicMaterial, Long> implements MESBasicMaterialDao {
	@Override
	...
	@Override
	public MESBasicMaterialDealInfo copyAndSaveDealInfo(DealInfo di) {
		MESBasicMaterialDealInfo dealInfo = new MESBasicMaterialDealInfo();
	...
	@Override
	public MESBasicMaterialDealInfo copyAndSaveDealInfo(DealInfo di, MESBasicMaterial mainObj, Staff dealStaff) {
		MESBasicMaterialDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(MESBasicMaterialDealInfo.class);
		...
		List<MESBasicMaterialDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	 /* CUSTOM CODE START(daoimpl,functions,MESBasic_1_material_Material,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
}