...
import com.supcon.orchid.MESBasic.services.MESBasicProdResetDailyImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicProdResetDailyDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_prodResetDailyService")
@Transactional
public class MESBasicProdResetDailyServiceImpl  extends BaseServiceImpl<MESBasicProdResetDaily> implements MESBasicProdResetDailyService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicProdResetDaily> bapBaseService;
	...
	@Autowired
	private MESBasicProdResetDailyImportService prodResetDailyImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicProdResetDaily.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  prodResetDailyDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicProdResetDaily.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = prodResetDailyDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdResetDaily.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdResetDaily.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  prodResetDailyDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicProdResetDaily.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicProdResetDaily.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicProdResetDaily> list = (List<MESBasicProdResetDaily>) prodResetDailyDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdResetDaily getProdResetDaily(long id){
		return getProdResetDaily(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdResetDaily getProdResetDaily(long id, String viewCode){
		MESBasicProdResetDaily prodResetDaily = prodResetDailyDao.load(id);

		// 一对多情况处理
		if(prodResetDaily != null){
		}
		return prodResetDaily;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProdResetDailyAsJSON(long id, String include){
		MESBasicProdResetDaily prodResetDaily = prodResetDailyDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(prodResetDaily, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.common.delete",operType="3")
	public void deleteProdResetDaily(MESBasicProdResetDaily prodResetDaily){
		if(null != prodResetDaily && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdResetDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
			AuditUtil.setColumnStringA(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
			AuditUtil.setColumnStringC(prodResetDaily.getId().toString());
		}
		beforeDeleteProdResetDaily(prodResetDaily);
		prodResetDailyDao.deletePhysical(prodResetDaily);
		afterDeleteProdResetDaily(prodResetDaily);
		prodResetDailyDao.flush();
		List<MESBasicProdResetDaily> params = new ArrayList<MESBasicProdResetDaily>();
		params.add(prodResetDaily);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.common.delete",operType="3")
	public void deleteProdResetDaily(List<Long> prodResetDailyIds){
		deleteProdResetDaily(prodResetDailyIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.common.delete",operType="3")
	public void deleteProdResetDaily(List<Long> prodResetDailyIds, String eventTopic) {
		List<MESBasicProdResetDaily> prodResetDailys = new ArrayList<MESBasicProdResetDaily>();
		for(Long prodResetDailyId : prodResetDailyIds){
			MESBasicProdResetDaily prodResetDaily = getProdResetDaily(prodResetDailyId);
			prodResetDailys.add(prodResetDaily);
			if(null != prodResetDaily && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdResetDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == prodResetDaily.getId() ? "" : "," + prodResetDaily.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == prodResetDaily.getId() ? "" : "," + prodResetDaily.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == prodResetDaily.getId() ? "" : "," + prodResetDaily.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
				}
			}
		}
		
		for(MESBasicProdResetDaily prodResetDaily : prodResetDailys){
			beforeDeleteProdResetDaily(prodResetDaily);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(prodResetDailys != null && prodResetDailys.size() > 0 ){
			for(MESBasicProdResetDaily prodResetDaily : prodResetDailys){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicProdResetDaily.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(prodResetDaily));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),prodResetDaily, prodResetDaily.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(prodResetDailyIds != null && prodResetDailyIds.size() > 0) {
			String hql = "delete " + MESBasicProdResetDaily.JPA_NAME + " where id in(:ids)";
			Query query = prodResetDailyDao.createQuery(hql);
			query.setParameterList("ids", prodResetDailyIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicProdResetDaily prodResetDailyz : prodResetDailys){
				prodResetDailyz.setValid(false);
				List<MESBasicProdResetDaily> params = new ArrayList<MESBasicProdResetDaily>();
				params.add(prodResetDailyz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_product");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.common.delete",operType="3")
	public void deleteProdResetDaily(long prodResetDailyId, int prodResetDailyVersion){
		this.deleteProdResetDaily(prodResetDailyId, prodResetDailyVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.common.delete",operType="3")
	public void deleteProdResetDaily(long prodResetDailyId, int prodResetDailyVersion,SignatureLog signatureLog){
		MESBasicProdResetDaily prodResetDaily = getProdResetDaily(prodResetDailyId);
		...
		beforeDeleteProdResetDaily(prodResetDaily);
		prodResetDailyDao.delete(prodResetDailyId, prodResetDailyVersion);
		afterDeleteProdResetDaily(prodResetDaily);
		prodResetDailyDao.flush();
		List<MESBasicProdResetDaily> params = new ArrayList<MESBasicProdResetDaily>();
		params.add(prodResetDaily);
		...
		if(signatureLog != null ) {
			...
			businessKey=prodResetDaily.getId();
			signatureLog.setTableId(prodResetDaily.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.common.delete",operType="3")
	public void deleteProdResetDaily(String prodResetDailyIds){
		this.deleteProdResetDaily(prodResetDailyIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.common.delete",operType="3")
	public void deleteProdResetDaily(String prodResetDailyIds,SignatureLog signatureLog){
		deleteCollection(prodResetDailyIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProdResetDaily(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_product#modelCode:MESBasic_1_product_ProdResetDaily#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="ec.engine.restore",operType="10")
	public void restoreProdResetDaily(String prodResetDailyIds){
		restoreCollection(prodResetDailyIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProdResetDaily(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="ec.engine.restore",operType="10")
	public void restoreProdResetDaily(long prodResetDailyId){
		
		findBusinessKeyUsed(prodResetDailyId);	//判断业务主键是否重复
		...
		beforeRestoreProdResetDaily(prodResetDaily);
		prodResetDaily.setValid(true);
		prodResetDailyDao.update(prodResetDaily);
		afterRestoreProdResetDaily(prodResetDaily);
		List<MESBasicProdResetDaily> params = new ArrayList<MESBasicProdResetDaily>();
		params.add(prodResetDaily);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long prodResetDailyId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_product_ProdResetDaily");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicProdResetDaily.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicProdResetDaily.TABLE_NAME +" where id = ? )";
			List<Object> list =  prodResetDailyDao.createNativeQuery(sql,prodResetDailyId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProdResetDaily(List<MESBasicProdResetDaily>  prodResetDailys){
		for(MESBasicProdResetDaily prodResetDaily:prodResetDailys)  {
			saveProdResetDaily(prodResetDaily, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProdResetDaily(List<MESBasicProdResetDaily>  prodResetDailys){
		for(MESBasicProdResetDaily prodResetDaily:prodResetDailys)  {
			saveProdResetDaily(prodResetDaily, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="ec.flow.save")
	public void saveProdResetDaily(MESBasicProdResetDaily prodResetDaily, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProdResetDaily(prodResetDaily, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="ec.flow.save")
	public void saveProdResetDaily(MESBasicProdResetDaily prodResetDaily, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProdResetDaily(prodResetDaily, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProdResetDailyCodes(MESBasicProdResetDaily prodResetDaily) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdResetDailyCodes(prodResetDaily, false);
	}
	
	public void generateProdResetDailyCodes(MESBasicProdResetDaily prodResetDaily, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProdResetDailySummarys(MESBasicProdResetDaily prodResetDaily) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdResetDailySummarys(prodResetDaily, false);
	}

	public void generateProdResetDailySummarys(MESBasicProdResetDaily prodResetDaily, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily")
	public void saveProdResetDaily(MESBasicProdResetDaily prodResetDaily,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProdResetDaily(prodResetDaily, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily")
	public void saveProdResetDaily(MESBasicProdResetDaily prodResetDaily,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(prodResetDaily.getId() != null && prodResetDaily.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdResetDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, prodResetDaily);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(prodResetDaily);
		beforeSaveProdResetDaily(prodResetDaily, viewIsView);

		if (viewIsView) {
			prodResetDailyDao.saveWithRevertVersion(prodResetDaily);
		} else {
			if(isNew)
				prodResetDailyDao.save(prodResetDaily);
			else
				prodResetDailyDao.merge(prodResetDaily);
		}
		if(null != prodResetDaily && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdResetDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
			AuditUtil.setColumnStringA(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
		...
		dealDatagridsSave(prodResetDaily,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProdResetDailyCodes(prodResetDaily, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProdResetDailySummarys(prodResetDaily, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProdResetDaily(prodResetDaily, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, prodResetDaily);
			}
		}
		List<MESBasicProdResetDaily> params = new ArrayList<MESBasicProdResetDaily>();
		prodResetDailyDao.flush();
		prodResetDailyDao.clear();
		prodResetDaily = prodResetDailyDao.load(prodResetDaily.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicProdResetDaily prodResetDaily){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("prodResetDaily", prodResetDaily);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily")
	public void mergeProdResetDaily(MESBasicProdResetDaily prodResetDaily, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(prodResetDaily);
		beforeSaveProdResetDaily(prodResetDaily);
		prodResetDailyDao.merge(prodResetDaily);
		if(null != prodResetDaily && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdResetDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
			AuditUtil.setColumnStringA(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
		...
		dealDatagridsSave(prodResetDaily,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProdResetDaily(prodResetDaily);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicProdResetDaily> findProdResetDailys(Page<MESBasicProdResetDaily> page, Criterion... criterions) {
		return prodResetDailyDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdResetDailys(Page<MESBasicProdResetDaily> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProdResetDailys(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdResetDailys(Page<MESBasicProdResetDaily> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdResetDailys(Page<MESBasicProdResetDaily> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicProdResetDaily> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"prodResetDaily\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"prodResetDaily\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"prodResetDaily\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"prodResetDaily\".PRODUCTID IN ( SELECT PRODUCT_ID FROM S2BASE_PRODUCT \"product\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "MESBasic_1_product" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  prodResetDailyDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  prodResetDailyDao.createNativeQuery(powerOperateSql, "MESBasic_1_product").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"product\"","MESBasic_1_product","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"prodResetDaily\".LAY_REC ASC, \"prodResetDaily\".SORT ASC");
				} else {
					orderPart.append(" \"prodResetDaily\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) prodResetDailyDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=prodResetDailyDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = prodResetDailyDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicProdResetDaily> result = new ArrayList<MESBasicProdResetDaily>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdResetDaily.class,prodResetDailyDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdResetDaily.class,prodResetDailyDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicProdResetDaily prodResetDaily : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(prodResetDaily.getId(), "MESBasic_product_prodResetDaily");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(prodResetDaily.getId(), "MESBasic_product_prodResetDaily");
							if(null != documents && !documents.isEmpty()) {
								prodResetDaily.setDocument(documents.get(0));
								prodResetDaily.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicProdResetDaily> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicProdResetDaily> result) {
		View view = null;
		...
		Map<Long, MESBasicProdResetDaily> rsMap = new HashMap<Long, MESBasicProdResetDaily>();
		for (MESBasicProdResetDaily prodResetDaily : result) {
			ids.add(prodResetDaily.getId());
			rsMap.put(prodResetDaily.getId(), prodResetDaily);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(prodResetDailyDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = prodResetDailyDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_product_ProdResetDaily" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_product_ProdResetDaily" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  prodResetDailyDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodResetDaily\"","MESBasic_1_product","","MESBASIC_PROD_RESET_DAILIES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"prodResetDaily\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"prodResetDaily\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodResetDaily\"","MESBasic_1_product","","MESBASIC_PROD_RESET_DAILIES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_product_ProdResetDaily" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("prodResetDaily.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"prodResetDaily\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"prodResetDaily\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"prodResetDaily\".CID = 1 OR \"prodResetDaily\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("prodResetDaily.mc\"").append(".PROD_RESET_DAILY) not in ("+notinValue+")");
			}
			
			SQLQuery query = prodResetDailyDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=prodResetDailyDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicProdResetDaily.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicProdResetDaily.MODULE_CODE;}
	protected String getModelCode(){return MESBasicProdResetDaily.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param prodResetDaily 模型实例
	 */
	public void checkUniqueConstraint(MESBasicProdResetDaily prodResetDaily){
	}
	
	
	@Override
	public List<MESBasicProdResetDaily> getProdResetDailys(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdResetDaily.class);
		if(params!=null){
			...
			return prodResetDailyDao.findByHql("from " + MESBasicProdResetDaily.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return prodResetDailyDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicProdResetDaily> findProdResetDailysByHql(String hql, Object... objects){
		...
		return   prodResetDailyDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicProdResetDaily loadProdResetDailyByBussinessKey(MESBasicProdResetDaily prodResetDaily){
		return loadProdResetDailyByBussinessKey(prodResetDaily.getId());
	}
	
	@Override
	public Page<MESBasicProdResetDaily> getProdResetDailys(Page<MESBasicProdResetDaily> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdResetDaily.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = prodResetDailyDao.findByPage(page, sql);
			}else{
				page = prodResetDailyDao.findAllByPage(page);
			}
			return page;
		}
		page = prodResetDailyDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdResetDaily> findByTreeDataGridProduct(List<MESBasicProdResetDaily> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicProdResetDaily prodResetDaily : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduct(Page<MESBasicProdResetDaily> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicProdResetDaily> prodResetDailys = dgPage.getResult();
			for(MESBasicProdResetDaily prodResetDaily : prodResetDailys){
				
			}
			dgPage.setResult(prodResetDailys);
		}
	...
	public MESBasicProdResetDaily loadProdResetDailyByBussinessKey(Serializable bussinessKey){
		return prodResetDailyDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdResetDaily> findByProperty(String propertyName, Object object){
		return prodResetDailyDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdResetDaily findEntityByProperty(String propertyName, Object object){
		return prodResetDailyDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicProdResetDaily.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = prodResetDailyDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_product_ProdResetDaily","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicProdResetDaily prodResetDaily,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,prodResetDailyDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicProdResetDaily.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, prodResetDaily);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",prodResetDaily.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdResetDaily.TABLE_NAME +" where valid = 1";
			list = prodResetDailyDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdResetDaily.TABLE_NAME;
			list = prodResetDailyDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicProdResetDaily.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = prodResetDailyDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicProdResetDailyDaoImpl prodResetDailyDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProdResetDaily(final List<MESBasicProdResetDaily> insertObjs, final List<MESBasicProdResetDaily> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProdResetDaily(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProdResetDaily(final List<MESBasicProdResetDaily> insertObjs, final List<MESBasicProdResetDaily> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1471690809334"),importCount));
		...
		Map<Long, MESBasicProdResetDaily> oldUpdateObjsMap = new HashMap<Long, MESBasicProdResetDaily>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicProdResetDaily bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicProdResetDaily> bizObjList = prodResetDailyDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicProdResetDaily bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicProdResetDaily> nmeObjs = new ArrayList<MESBasicProdResetDaily>();
		...
		long id=autoGeneratorID.getNextId(MESBasicProdResetDaily.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicProdResetDaily prodResetDaily:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicProdResetDaily.TABLE_NAME,1,dbId);
			if(prodResetDaily.getId() != null ){
				m.put(prodResetDaily.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			prodResetDaily.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) prodResetDailyDao.createNativeQuery(sql, "MESBasic_1_product_ProdResetDaily"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		prodResetDailyDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicProdResetDaily.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicProdResetDaily prodResetDaily = updateObjs.get(updateIndex);
						Long id = prodResetDaily.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = prodResetDaily.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(prodResetDaily);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicProdResetDaily.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicProdResetDaily.TABLE_NAME + " order by id desc";
//					Query query = prodResetDailyDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicProdResetDaily prodResetDaily = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = prodResetDaily.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		prodResetDailyDaoImpl.getSession().flush();
		prodResetDailyDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicProdResetDaily bizObj = insertObjs.get(insertIndex);
			prodResetDailyImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicProdResetDaily bizObj = updateObjs.get(updateIndex);
			MESBasicProdResetDaily oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			prodResetDailyImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicProdResetDaily> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicProdResetDaily> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) prodResetDailyDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicProdResetDaily> result = getResult(page, query, new PendingResultTransformer(MESBasicProdResetDaily.class,prodResetDailyDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicProdResetDaily model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = prodResetDailyDao.createNativeQuery(sql, "MESBasic_1_product_ProdResetDaily").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = prodResetDailyDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = prodResetDailyDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = prodResetDailyDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?prodResetDailyDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  prodResetDailyDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,prodResetDailyDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="ec.flow.save")
	public void saveProdResetDaily(MESBasicProdResetDaily prodResetDaily, DataGridService dataGridService){
		saveProdResetDaily(prodResetDaily, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="ec.flow.save")
	public void saveProdResetDaily(MESBasicProdResetDaily prodResetDaily, DataGridService dataGridService, String viewCode){
		saveProdResetDaily(prodResetDaily, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily")
	public void saveProdResetDaily(MESBasicProdResetDaily prodResetDaily, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProdResetDaily(prodResetDaily, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily")
	public void saveProdResetDaily(MESBasicProdResetDaily prodResetDaily, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(null != prodResetDaily.getId() && prodResetDaily.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdResetDaily.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily")
	public Map<Object, Object> prodResetDailyDataGridImport(MESBasicProdResetDaily prodResetDaily, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = prodResetDaily.getVirtualId();
		
		prodResetDaily = prodResetDailyDao.load(prodResetDaily.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, prodResetDaily.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = prodResetDaily.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(prodResetDaily);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, prodResetDaily.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily")
	public void mergeProdResetDaily(MESBasicProdResetDaily prodResetDaily, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(prodResetDaily);
		beforeSaveProdResetDaily(prodResetDaily);
		prodResetDailyDao.merge(prodResetDaily);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdResetDaily",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicProdResetDaily> getByPage(Page<MESBasicProdResetDaily> page,DetachedCriteria detachedCriteria){
		return prodResetDailyDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProdResetDaily(MESBasicProdResetDaily prodResetDaily, Object... objects){
		checkUniqueConstraint(prodResetDaily);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProdResetDaily(MESBasicProdResetDaily prodResetDaily, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProdResetDaily(MESBasicProdResetDaily prodResetDaily){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProdResetDaily(MESBasicProdResetDaily prodResetDaily){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProdResetDaily(MESBasicProdResetDaily prodResetDaily){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProdResetDaily(MESBasicProdResetDaily prodResetDaily){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicProdResetDaily prodResetDaily, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicProdResetDaily prodResetDaily, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProdResetDaily(List<MESBasicProdResetDaily> insertObjs, List<MESBasicProdResetDaily> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProdResetDaily,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProdResetDaily(List<MESBasicProdResetDaily> insertObjs, List<MESBasicProdResetDaily> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProdResetDaily,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
@Override
	public List<MESBasicProdResetDaily> findByProductId(Long goodId) {
		String hql = " from "+ MESBasicProdResetDaily.JPA_NAME + " where productID.id=?";
		return prodResetDailyDao.findByHql(hql,goodId);
	}
/* CUSTOM CODE END */
}
