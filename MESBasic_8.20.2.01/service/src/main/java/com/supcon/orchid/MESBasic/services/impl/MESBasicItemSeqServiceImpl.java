...
import com.supcon.orchid.MESBasic.services.MESBasicItemSeqImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicItemSeqDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
...
@Service("mESBasic_itemSeqService")
@Transactional
public class MESBasicItemSeqServiceImpl  extends BaseServiceImpl<MESBasicItemSeq> implements MESBasicItemSeqService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicItemSeq> bapBaseService;
	...
	@Autowired
	private MESBasicItemSeqImportService itemSeqImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicItemSeq.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  itemSeqDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicItemSeq.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = itemSeqDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicItemSeq.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicItemSeq.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  itemSeqDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicItemSeq.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicItemSeq.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicItemSeq> list = (List<MESBasicItemSeq>) itemSeqDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicItemSeq getItemSeq(long id){
		return getItemSeq(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicItemSeq getItemSeq(long id, String viewCode){
		MESBasicItemSeq itemSeq = itemSeqDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getItemSeqAsJSON(long id, String include){
		MESBasicItemSeq itemSeq = itemSeqDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(itemSeq, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.common.delete",operType="3")
	public void deleteItemSeq(MESBasicItemSeq itemSeq){
		if(null != itemSeq && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemSeq.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
			AuditUtil.setColumnStringA(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
			AuditUtil.setColumnStringC(itemSeq.getId().toString());
		}
		beforeDeleteItemSeq(itemSeq);
		itemSeqDao.deletePhysical(itemSeq);
		afterDeleteItemSeq(itemSeq);
		itemSeqDao.flush();
		List<MESBasicItemSeq> params = new ArrayList<MESBasicItemSeq>();
		params.add(itemSeq);
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.common.delete",operType="3")
	public void deleteItemSeq(List<Long> itemSeqIds){
		deleteItemSeq(itemSeqIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.common.delete",operType="3")
	public void deleteItemSeq(List<Long> itemSeqIds, String eventTopic) {
		List<MESBasicItemSeq> itemSeqs = new ArrayList<MESBasicItemSeq>();
		for(Long itemSeqId : itemSeqIds){
			MESBasicItemSeq itemSeq = getItemSeq(itemSeqId);
			itemSeqs.add(itemSeq);
			if(null != itemSeq && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemSeq.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == itemSeq.getId() ? "" : "," + itemSeq.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == itemSeq.getId() ? "" : "," + itemSeq.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == itemSeq.getId() ? "" : "," + itemSeq.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(itemSeqs != null && itemSeqs.size() > 0 ){
			for(MESBasicItemSeq itemSeq : itemSeqs){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicItemSeq.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(itemSeq));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),itemSeq, itemSeq.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(itemSeqIds != null && itemSeqIds.size() > 0) {
			String hql = "delete " + MESBasicItemSeq.JPA_NAME + " where id in(:ids)";
			Query query = itemSeqDao.createQuery(hql);
			query.setParameterList("ids", itemSeqIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicItemSeq itemSeqz : itemSeqs){
				itemSeqz.setValid(false);
				List<MESBasicItemSeq> params = new ArrayList<MESBasicItemSeq>();
				params.add(itemSeqz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.common.delete",operType="3")
	public void deleteItemSeq(long itemSeqId, int itemSeqVersion){
		this.deleteItemSeq(itemSeqId, itemSeqVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.common.delete",operType="3")
	public void deleteItemSeq(long itemSeqId, int itemSeqVersion,SignatureLog signatureLog){
		MESBasicItemSeq itemSeq = getItemSeq(itemSeqId);
		...
		beforeDeleteItemSeq(itemSeq);
		itemSeqDao.delete(itemSeqId, itemSeqVersion);
		afterDeleteItemSeq(itemSeq);
		itemSeqDao.flush();
		List<MESBasicItemSeq> params = new ArrayList<MESBasicItemSeq>();
		params.add(itemSeq);
		...
		if(signatureLog != null ) {
			...
			businessKey=itemSeq.getId();
			signatureLog.setTableId(itemSeq.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.common.delete",operType="3")
	public void deleteItemSeq(String itemSeqIds){
		this.deleteItemSeq(itemSeqIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.common.delete",operType="3")
	public void deleteItemSeq(String itemSeqIds,SignatureLog signatureLog){
		deleteCollection(itemSeqIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteItemSeq(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_thirdDataHandler#modelCode:MESBasic_1_thirdDataHandler_ItemSeq#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="ec.engine.restore",operType="10")
	public void restoreItemSeq(String itemSeqIds){
		restoreCollection(itemSeqIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreItemSeq(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="ec.engine.restore",operType="10")
	public void restoreItemSeq(long itemSeqId){
		
		findBusinessKeyUsed(itemSeqId);	//判断业务主键是否重复
		...
		beforeRestoreItemSeq(itemSeq);
		itemSeq.setValid(true);
		itemSeqDao.update(itemSeq);
		afterRestoreItemSeq(itemSeq);
		List<MESBasicItemSeq> params = new ArrayList<MESBasicItemSeq>();
		params.add(itemSeq);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long itemSeqId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_thirdDataHandler_ItemSeq");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicItemSeq.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicItemSeq.TABLE_NAME +" where id = ? )";
			List<Object> list =  itemSeqDao.createNativeQuery(sql,itemSeqId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseItemSeq(List<MESBasicItemSeq>  itemSeqs){
		for(MESBasicItemSeq itemSeq:itemSeqs)  {
			saveItemSeq(itemSeq, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseItemSeq(List<MESBasicItemSeq>  itemSeqs){
		for(MESBasicItemSeq itemSeq:itemSeqs)  {
			saveItemSeq(itemSeq, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="ec.flow.save")
	public void saveItemSeq(MESBasicItemSeq itemSeq, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveItemSeq(itemSeq, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="ec.flow.save")
	public void saveItemSeq(MESBasicItemSeq itemSeq, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveItemSeq(itemSeq, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateItemSeqCodes(MESBasicItemSeq itemSeq) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateItemSeqCodes(itemSeq, false);
	}
	
	public void generateItemSeqCodes(MESBasicItemSeq itemSeq, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateItemSeqSummarys(MESBasicItemSeq itemSeq) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateItemSeqSummarys(itemSeq, false);
	}

	public void generateItemSeqSummarys(MESBasicItemSeq itemSeq, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq")
	public void saveItemSeq(MESBasicItemSeq itemSeq,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveItemSeq(itemSeq, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq")
	public void saveItemSeq(MESBasicItemSeq itemSeq,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_thirdDataHandler");
		String url = null;
		if(itemSeq.getId() != null && itemSeq.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemSeq.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, itemSeq);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(itemSeq);
		beforeSaveItemSeq(itemSeq, viewIsView);

		if (viewIsView) {
			itemSeqDao.saveWithRevertVersion(itemSeq);
		} else {
			if(isNew)
				itemSeqDao.save(itemSeq);
			else
				itemSeqDao.merge(itemSeq);
		}
		if(null != itemSeq && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemSeq.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
			AuditUtil.setColumnStringA(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
		...
		dealDatagridsSave(itemSeq,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateItemSeqCodes(itemSeq, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateItemSeqSummarys(itemSeq, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveItemSeq(itemSeq, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, itemSeq);
			}
		}
		List<MESBasicItemSeq> params = new ArrayList<MESBasicItemSeq>();
		itemSeqDao.flush();
		itemSeqDao.clear();
		itemSeq = itemSeqDao.load(itemSeq.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicItemSeq itemSeq){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq")
	public void mergeItemSeq(MESBasicItemSeq itemSeq, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(itemSeq);
		beforeSaveItemSeq(itemSeq);
		itemSeqDao.merge(itemSeq);
		if(null != itemSeq && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemSeq.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
			AuditUtil.setColumnStringA(null == itemSeq.getId() ? "" : itemSeq.getId().toString());
		...
		dealDatagridsSave(itemSeq,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveItemSeq(itemSeq);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicItemSeq> findItemSeqs(Page<MESBasicItemSeq> page, Criterion... criterions) {
		return itemSeqDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemSeqs(Page<MESBasicItemSeq> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findItemSeqs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemSeqs(Page<MESBasicItemSeq> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findItemSeqs(Page<MESBasicItemSeq> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicItemSeq> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"itemSeq\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"itemSeq\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"itemSeq\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "MESBasic_1_thirdDataHandler" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  itemSeqDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  itemSeqDao.createNativeQuery(powerOperateSql, "MESBasic_1_thirdDataHandler").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"itemSeq\".LAY_REC ASC, \"itemSeq\".SORT ASC");
				} else {
					orderPart.append(" \"itemSeq\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) itemSeqDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=itemSeqDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = itemSeqDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicItemSeq> result = new ArrayList<MESBasicItemSeq>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicItemSeq.class,itemSeqDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicItemSeq.class,itemSeqDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicItemSeq itemSeq : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(itemSeq.getId(), "MESBasic_thirdDataHandler_itemSeq");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(itemSeq.getId(), "MESBasic_thirdDataHandler_itemSeq");
							if(null != documents && !documents.isEmpty()) {
								itemSeq.setDocument(documents.get(0));
								itemSeq.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicItemSeq> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicItemSeq> result) {
		View view = null;
		...
		Map<Long, MESBasicItemSeq> rsMap = new HashMap<Long, MESBasicItemSeq>();
		for (MESBasicItemSeq itemSeq : result) {
			ids.add(itemSeq.getId());
			rsMap.put(itemSeq.getId(), itemSeq);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(itemSeqDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = itemSeqDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_thirdDataHandler_ItemSeq" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_thirdDataHandler_ItemSeq" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  itemSeqDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"itemSeq\"","MESBasic_1_thirdDataHandler","","MESBASIC_ITEM_SEQS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"itemSeq\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"itemSeq\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"itemSeq\"","MESBasic_1_thirdDataHandler","","MESBASIC_ITEM_SEQS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_thirdDataHandler_ItemSeq" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("itemSeq.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"itemSeq\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("itemSeq.mc\"").append(".ITEM_SEQ) not in ("+notinValue+")");
			}
			
			SQLQuery query = itemSeqDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=itemSeqDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicItemSeq.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicItemSeq.MODULE_CODE;}
	protected String getModelCode(){return MESBasicItemSeq.MODEL_CODE;}
	 ...
	 * @param itemSeq 模型实例
	 */
	public void checkUniqueConstraint(MESBasicItemSeq itemSeq){
	}
	
	
	@Override
	public List<MESBasicItemSeq> getItemSeqs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicItemSeq.class);
		if(params!=null){
			...
			return itemSeqDao.findByHql("from " + MESBasicItemSeq.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return itemSeqDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicItemSeq> findItemSeqsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicItemSeq.class);
	...
	@Override
	public List<MESBasicItemSeq> findItemSeqsByHql(String hql, Object... objects){
		
		return   itemSeqDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicItemSeq loadItemSeqByBussinessKey(MESBasicItemSeq itemSeq){
		return loadItemSeqByBussinessKey(itemSeq.getId());
	}
	
	@Override
	public Page<MESBasicItemSeq> getItemSeqs(Page<MESBasicItemSeq> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicItemSeq.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = itemSeqDao.findByPage(page, sql);
			}else{
				page = itemSeqDao.findAllByPage(page);
			}
			return page;
		}
		page = itemSeqDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicItemSeq loadItemSeqByBussinessKey(Serializable bussinessKey){
		return itemSeqDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicItemSeq> findByProperty(String propertyName, Object object){
		return itemSeqDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicItemSeq findEntityByProperty(String propertyName, Object object){
		return itemSeqDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicItemSeq.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = itemSeqDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_thirdDataHandler_ItemSeq","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicItemSeq itemSeq,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,itemSeqDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicItemSeq.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, itemSeq);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",itemSeq.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicItemSeq.TABLE_NAME +" where valid = 1";
			list = itemSeqDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicItemSeq.TABLE_NAME;
			list = itemSeqDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicItemSeq.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = itemSeqDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicItemSeqDaoImpl itemSeqDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchItemSeq(final List<MESBasicItemSeq> insertObjs, final List<MESBasicItemSeq> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchItemSeq(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchItemSeq(final List<MESBasicItemSeq> insertObjs, final List<MESBasicItemSeq> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1427346429041"),importCount));
		...
		Map<Long, MESBasicItemSeq> oldUpdateObjsMap = new HashMap<Long, MESBasicItemSeq>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicItemSeq bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicItemSeq> bizObjList = itemSeqDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicItemSeq bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicItemSeq> nmeObjs = new ArrayList<MESBasicItemSeq>();
		...
		long id=autoGeneratorID.getNextId(MESBasicItemSeq.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicItemSeq itemSeq:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicItemSeq.TABLE_NAME,1,dbId);
			if(itemSeq.getId() != null ){
				m.put(itemSeq.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) itemSeqDao.createNativeQuery(sql, "MESBasic_1_thirdDataHandler_ItemSeq"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		itemSeqDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicItemSeq.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicItemSeq itemSeq = updateObjs.get(updateIndex);
						Long id = itemSeq.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = itemSeq.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicItemSeq.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicItemSeq.TABLE_NAME + " order by id desc";
//					Query query = itemSeqDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicItemSeq itemSeq = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = itemSeq.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		itemSeqDaoImpl.getSession().flush();
		itemSeqDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicItemSeq bizObj = insertObjs.get(insertIndex);
			itemSeqImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicItemSeq bizObj = updateObjs.get(updateIndex);
			MESBasicItemSeq oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			itemSeqImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicItemSeq> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicItemSeq> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) itemSeqDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicItemSeq> result = getResult(page, query, new PendingResultTransformer(MESBasicItemSeq.class,itemSeqDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicItemSeq model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = itemSeqDao.createNativeQuery(sql, "MESBasic_1_thirdDataHandler_ItemSeq").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = itemSeqDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = itemSeqDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = itemSeqDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?itemSeqDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  itemSeqDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,itemSeqDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="ec.flow.save")
	public void saveItemSeq(MESBasicItemSeq itemSeq, DataGridService dataGridService){
		saveItemSeq(itemSeq, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="ec.flow.save")
	public void saveItemSeq(MESBasicItemSeq itemSeq, DataGridService dataGridService, String viewCode){
		saveItemSeq(itemSeq, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq")
	public void saveItemSeq(MESBasicItemSeq itemSeq, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveItemSeq(itemSeq, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq")
	public void saveItemSeq(MESBasicItemSeq itemSeq, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_thirdDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_thirdDataHandler");
		String url = null;
		if(null != itemSeq.getId() && itemSeq.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicItemSeq.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq")
	public Map<Object, Object> itemSeqDataGridImport(MESBasicItemSeq itemSeq, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = itemSeq.getVirtualId();
		
		itemSeq = itemSeqDao.load(itemSeq.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, itemSeq.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = itemSeq.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(itemSeq);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, itemSeq.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq")
	public void mergeItemSeq(MESBasicItemSeq itemSeq, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(itemSeq);
		beforeSaveItemSeq(itemSeq);
		itemSeqDao.merge(itemSeq);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_thirdDataHandler",model="MESBasic_1_thirdDataHandler_ItemSeq",desc="foundation.batch.save",operType="4")
	public void batchSaveItemSeq(MESBasicItemSeq itemSeq, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_thirdDataHandler";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_thirdDataHandler");
	...
	@Override
	public Page<MESBasicItemSeq> getByPage(Page<MESBasicItemSeq> page,DetachedCriteria detachedCriteria){
		return itemSeqDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveItemSeq(MESBasicItemSeq itemSeq, Object... objects){
		checkUniqueConstraint(itemSeq);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveItemSeq(MESBasicItemSeq itemSeq, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportItemSeq(List<MESBasicItemSeq> itemSeqs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportItemSeq(List<MESBasicItemSeq> itemSeqs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportItemSeq(List<MESBasicItemSeq> itemSeqs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportItemSeq(List<MESBasicItemSeq> itemSeqs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteItemSeq(MESBasicItemSeq itemSeq){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteItemSeq(MESBasicItemSeq itemSeq){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreItemSeq(MESBasicItemSeq itemSeq){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreItemSeq(MESBasicItemSeq itemSeq){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicItemSeq itemSeq, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicItemSeq itemSeq, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportItemSeq(List<MESBasicItemSeq> insertObjs, List<MESBasicItemSeq> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportItemSeq,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportItemSeq(List<MESBasicItemSeq> insertObjs, List<MESBasicItemSeq> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportItemSeq,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_thirdDataHandler_ItemSeq,MESBasic_1) */
// 自定义代码
	@Override
	public MESBasicItemSeq getMaxIdSeq(String moduleCode){
		List<MESBasicItemSeq> list=itemSeqDao.findByCriteria(Restrictions.eq("seqType", moduleCode), Restrictions.eq("valid", true));
	...
	private PlatformTransactionManager txManager;
	 ...
	 public MESBasicItemSeq createItemSeq(final String moduleCode){
		 TransactionTemplate txTemplate = new TransactionTemplate(txManager);
		 txTemplate.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRES_NEW);
	  
		 MESBasicItemSeq seq= txTemplate.execute(new TransactionCallback<MESBasicItemSeq>() {
		  
		   @Override
		   public MESBasicItemSeq doInTransaction(TransactionStatus status) {
			   MESBasicItemSeq entity=new MESBasicItemSeq();
				entity.setValid(true);
				entity.setCid(getCurrentCompanyId());
				entity.setVersion(0);
				entity.setMaxId(0L);
				entity.setErrorFlag(false);
				entity.setSeqType(moduleCode);
				itemSeqDao.save(entity);
		    return entity;
		   }
		 });
	...
	@Override
	public void updateMaxIdSeq(final String moduleCode,final Long maxId){
		 //回填最大的maxid
		itemSeqDao.flush();
		itemSeqDao.clear();
		String updateMaxIdSql="update "+MESBasicItemSeq.TABLE_NAME+" set max_id=? where SEQ_TYPE=?";
		itemSeqDao.createNativeQuery(updateMaxIdSql, maxId,moduleCode).executeUpdate();
	}
	
	@Transactional(propagation=Propagation.REQUIRES_NEW)
	public void updateErrorSeq(String moduleCode,boolean errorFlag){
		String updateErrorSql="update "+MESBasicItemSeq.TABLE_NAME+" set ERROR_FLAG=? where SEQ_TYPE=?";
		int i=itemSeqDao.createNativeQuery(updateErrorSql, (errorFlag)?1:0,moduleCode).executeUpdate();
		if(i==0){
			MESBasicItemSeq entity=new MESBasicItemSeq();
			entity.setValid(true);
			entity.setCid(getCurrentCompanyId());
			...
			entity.setSeqType(MESBasicItemSeq.MODULE_CODE);
			itemSeqDao.save(entity);
		}
