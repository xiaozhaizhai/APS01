...
/* CUSTOM CODE START(action,import,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicProductCostAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicProductCost> {
	...
	private MESBasicProductCost productCost;
	...
	//导出excel时,主模型内容
	List<MESBasicProductCost> queryResults = new ArrayList<MESBasicProductCost>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/productCost/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/productCost/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_product_ProductCost" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicProductCost> productCosts;
	
	public List<MESBasicProductCost> getProductCosts() {
		return productCosts;
	}
	...
	private Page<MESBasicProductCost> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			productCost = new MESBasicProductCost();
			if(productCost.getBaptype() != null) {
				productCost.setBaptype((SystemCode) systemCodeService.getSystemCode(productCost.getBaptype().getId()));
			}
		} else {
			if(id > 0){
				productCost = productCostService.getProductCost(id, viewCode);
			}
		}
	...
	public Page<MESBasicProductCost> getPage() {
		if (null == page) {
			page = new Page<MESBasicProductCost>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicProductCost getProductCost() {
		return productCost;
	}		

	public void setProductCost(MESBasicProductCost productCost) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/productCost/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = productCostService.getProductCostAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/productCost/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1471523790726
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String productCostReferenceCopy() throws Exception{
			MESBasicProductCost oldProductCost = null;
			if(isEditNew()){
				...
				oldProductCost = new MESBasicProductCost();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,productCost.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, productCost.getId());
				...
				oldProductCost = productCostService.getProductCost(id);
			}
			
			
			productCost = productCostService.getProductCost(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_product_ProductCost");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicProductCost.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProductCost, null);
						Method setValue = MESBasicProductCost.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(productCost, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			productCost.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_product_ProductCost", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=productCostService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return productCostReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			productCost = productCostService.getProductCost(this.productCost.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,productCost.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/productCost/productCostList", results = { @Result(type = FREEMARKER, location = "productCostList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = productCostService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(productCostService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(productCostService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_product_ProductCost", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/productCost/productCostList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.baptype.id,result.baptype.value,result.cost,result.inputTime,result.inputStaffID.name,result.cmemo,result.typeS2,result.inputStaffID.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="productCostList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"productCost\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"productCost\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"productCost\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"productCost\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			productCostService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/product/productCost/productCostList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		productCostService.findDataGridPage(dg,dgclass,getDgPage(),productCost,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(productCost.getId() != null && productCost.getId() != -1){
				productCost = productCostService.getProductCost(productCost.getId());
			}
			productCostService.findDataGridPage(dg,dgclass,getDgPage(),productCost,searchCondition,params);
	...
	private void save() throws Exception {
		...
		productCostService.saveProductCost(productCost,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_ProductCost", "submit");
		bap_validate_datagrids=productCostService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", productCost.getId());
		...
		fileuploads.put("linkId", productCost.getId());
		bapAfterCustomMethod("MESBasic_1_product_ProductCost", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/product/productCost/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProductCost", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		productCostService.deleteProductCost(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_product_ProductCost", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/productCost/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProductCost", "restore");
		productCostService.restoreProductCost(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_product_ProductCost", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicProductCost> findExportDataInfos(Page<MESBasicProductCost> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = productCostService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = productCostService.getAssProperty(p.getCode());	
									//String assModelCode = productCostService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			productCostService.findProductCosts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			productCostService.findProductCosts(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicProductCost> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicProductCost result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".PC_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.productCost || null == this.productCost.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/product/productCost/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProductCost", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_product_ProductCost", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/product/productCost/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_ProductCost", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/product/productCost/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/productCost/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProductCostService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_product_ProductCost", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_product_ProductCost", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/productCost/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_product_ProductCost", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_product_ProductCost", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/product/productCost/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProductCost", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/product/productCost/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_product_ProductCost";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_product_ProductCost", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/product/productCost/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == productCost.getId() ? "" : productCost.getId().toString());
		_auditLog.setColumnStringA(null == productCost.getId() ? "" : productCost.getId().toString());
		_auditLog.setColumnStringC(productCost.getId().toString());
		_auditLog.setColumnStringD(MESBasicProductCost.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicProductCostAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, productCost,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, productCost,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,productCost);
		...
		printlog.setBusinessId(productCost.getId());
		printlog.setBussinessKey(null == productCost.getId() ? "" : productCost.getId().toString());
		printlog.setMainDisplay(null == productCost.getId() ? "" : productCost.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/product/productCost/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(productCost,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/productCost/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(productCostService.getProductCost(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/product/productCost/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/product/productCost/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, productCost,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				productCost = productCostService.getProductCost(Long.parseLong(arrId[i]));
				if(null != productCost){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == productCost.getId() ? "" : "," + productCost.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == productCost.getId() ? "" : productCost.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == productCost.getId() ? "" : "," + productCost.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == productCost.getId() ? "" : productCost.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == productCost.getId() ? "" : "," + productCost.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == productCost.getId() ? "" : productCost.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, productCost,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,productCost);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/product/productCost/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_product_ProductCost");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_product_ProductCost")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_product_ProductCost");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_product_ProductCost");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = productCostService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_product_ProductCost,MESBasic_1) */
// 自定义代码
