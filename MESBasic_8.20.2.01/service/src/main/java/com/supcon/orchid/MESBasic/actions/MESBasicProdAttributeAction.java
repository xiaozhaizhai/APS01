...
/* CUSTOM CODE START(action,import,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicProdAttributeAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicProdAttribute> {
	...
	private MESBasicProdAttribute prodAttribute;
	...
	//导出excel时,主模型内容
	List<MESBasicProdAttribute> queryResults = new ArrayList<MESBasicProdAttribute>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/prodAttribute/prodAttribute/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/prodAttribute/prodAttribute/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_prodAttribute_ProdAttribute" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicProdAttribute> prodAttributes;
	
	public List<MESBasicProdAttribute> getProdAttributes() {
	...
	private Page<MESBasicProdAttribute> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			prodAttribute = new MESBasicProdAttribute();
		} else {
			if(id > 0){
				prodAttribute = prodAttributeService.getProdAttribute(id, viewCode);
			}
		}
	...
	public Page<MESBasicProdAttribute> getPage() {
		if (null == page) {
			page = new Page<MESBasicProdAttribute>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicProdAttribute getProdAttribute() {
		return prodAttribute;
	}		

	public void setProdAttribute(MESBasicProdAttribute prodAttribute) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prodAttributeService.getProdAttributeAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1471569095116
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String prodAttributeReferenceCopy() throws Exception{
			MESBasicProdAttribute oldProdAttribute = null;
			if(isEditNew()){
				...
				oldProdAttribute = new MESBasicProdAttribute();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodAttribute.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prodAttribute.getId());
				...
				oldProdAttribute = prodAttributeService.getProdAttribute(id);
			}
			
			
			prodAttribute = prodAttributeService.getProdAttribute(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_prodAttribute_ProdAttribute");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicProdAttribute.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProdAttribute, null);
						Method setValue = MESBasicProdAttribute.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prodAttribute, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prodAttribute.setCreateDepartmentId(oldProdAttribute.getCreateDepartmentId());
			prodAttribute.setCreatePositionId(oldProdAttribute.getCreatePositionId());
			prodAttribute.setCreateStaffId(oldProdAttribute.getCreateStaffId());
			prodAttribute.setOwnerDepartmentId(oldProdAttribute.getOwnerDepartmentId());
			prodAttribute.setOwnerPositionId(oldProdAttribute.getOwnerPositionId());
			prodAttribute.setOwnerStaffId(oldProdAttribute.getOwnerStaffId());
			prodAttribute.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=prodAttributeService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return prodAttributeReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			prodAttribute = prodAttributeService.getProdAttribute(this.prodAttribute.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodAttribute.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodAttribute/prodAttribute/prodAttrLayout", results = { @Result(type = FREEMARKER, location = "prodAttrLayout.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = prodAttributeService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(prodAttributeService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(prodAttributeService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_prodAttribute_ProdAttribute", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodAttribute/prodAttribute/prodAtrrRef", results = { @Result(type = FREEMARKER, location = "prodAtrrRef.ftl") })
		,
		@Action(value = "/MESBasic/prodAttribute/prodAttribute/prodAtrrTree", results = { @Result(type = FREEMARKER, location = "prodAtrrTree.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"prodAttribute\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"prodAttribute\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"prodAttribute\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"prodAttribute\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			prodAttributeService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/MESBasic/prodAttribute/prodAttribute/prodAtrrRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="prodAtrrRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/MESBasic/prodAttribute/prodAttribute/prodAtrrTreeDrag")
	})
	public String viewTreeDrag() throws Exception{
		prodAttributeService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/MESBasic/prodAttribute/prodAttribute/prodAtrrRefSort")
	,
	@Action(value="/MESBasic/prodAttribute/prodAttribute/prodAtrrTreeSort")
	,
	@Action(value="/MESBasic/prodAttribute/prodAttribute/prodAttrLayoutSort")
	})
	...
	@Action(value="/MESBasic/prodAttribute/prodAttribute/prodAtrrTreeData",
			results = { @Result(type = JSON, params = { "root", "prodAttributes", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"prodAttribute\".", "{alias}.");
					}
		...
		prodAttributes = prodAttributeService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/MESBasic/prodAttribute/prodAttribute/prodAtrrTreeFullData",
			results = { @Result(type = JSON, params = { "root", "prodAttributes", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			prodAttributes = prodAttributeService.getTreeInfo(treeInfo, null, null);
		}
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		prodAttributeService.findDataGridPage(dg,dgclass,getDgPage(),prodAttribute,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(prodAttribute.getId() != null && prodAttribute.getId() != -1){
				prodAttribute = prodAttributeService.getProdAttribute(prodAttribute.getId());
			}
			prodAttributeService.findDataGridPage(dg,dgclass,getDgPage(),prodAttribute,searchCondition,params);
	...
	private void save() throws Exception {
		...
		prodAttributeService.saveProdAttribute(prodAttribute,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "submit");
		bap_validate_datagrids=prodAttributeService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", prodAttribute.getId());
		...
		fileuploads.put("linkId", prodAttribute.getId());
		bapAfterCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/prodAttribute/prodAttribute/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prodAttributeService.deleteProdAttribute(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "restore");
		prodAttributeService.restoreProdAttribute(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/prodAttribute/prodAttribute/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/prodAttribute/prodAttribute/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "showDealInfoData");
		if (null != tableInfoId) {
			int count=prodAttributeService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = prodAttributeService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = prodAttributeService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicProdAttribute> findExportDataInfos(Page<MESBasicProdAttribute> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = prodAttributeService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = prodAttributeService.getAssProperty(p.getCode());	
									//String assModelCode = prodAttributeService.getPropertyModelCode(assProperty);
				...
				if(view.getAssModel().getDataType() == 2 && useForImportFlag){//树型结构且用于导出，加上父节点,同时在sql中加入layNo,layRec,parentId,sort,leaf
					...
					exportSqlPart1Add.append("\".PRODATT_LAYNO AS \"layNo\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
					exportSqlPart1Add.append("\".PRODATT_LAYREC AS \"layRec\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			prodAttributeService.findProdAttributes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			prodAttributeService.findProdAttributes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicProdAttribute> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicProdAttribute result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".PRODATT_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.prodAttribute || null == this.prodAttribute.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/prodAttribute/prodAttribute/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/prodAttribute/prodAttribute/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProdAttributeService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_prodAttribute", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_prodAttribute");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_prodAttribute_ProdAttribute", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_prodAttribute_ProdAttribute", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_prodAttribute_ProdAttribute";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_prodAttribute_ProdAttribute", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicProdAttribute prodAttribute = null;
		if(null != id) {
			prodAttribute = prodAttributeService.getProdAttribute(id);
		}
		...
		_auditLog.setColumnStringB(null == prodAttribute.getCode() ? "" : prodAttribute.getCode().toString());
		_auditLog.setColumnStringA(null == prodAttribute.getName() ? "" : prodAttribute.getName().toString());
		_auditLog.setColumnStringC(prodAttribute.getId().toString());
		_auditLog.setColumnStringD(MESBasicProdAttribute.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_prodAttribute");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicProdAttributeAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, prodAttribute,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, prodAttribute,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,prodAttribute);
		...
		printlog.setEntity("MESBasic_1_prodAttribute");
		...
		printlog.setBusinessId(prodAttribute.getId());
		printlog.setBussinessKey(null == prodAttribute.getCode() ? "" : prodAttribute.getCode().toString());
		printlog.setMainDisplay(null == prodAttribute.getName() ? "" : prodAttribute.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(prodAttribute,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(prodAttributeService.getProdAttribute(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, prodAttribute,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				prodAttribute = prodAttributeService.getProdAttribute(Long.parseLong(arrId[i]));
				if(null != prodAttribute){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == prodAttribute.getName() ? "" : "," + prodAttribute.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == prodAttribute.getName() ? "" : prodAttribute.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == prodAttribute.getCode() ? "" : "," + prodAttribute.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == prodAttribute.getCode() ? "" : prodAttribute.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == prodAttribute.getId() ? "" : "," + prodAttribute.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == prodAttribute.getId() ? "" : prodAttribute.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, prodAttribute,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,prodAttribute);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/prodAttribute/prodAttribute/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicProdAttribute> findExportPageInfos(Page<MESBasicProdAttribute> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_prodAttribute" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = prodAttributeService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicProdAttribute queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = prodAttributeService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = prodAttributeService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_prodAttribute_ProdAttribute");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_prodAttribute_ProdAttribute")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_prodAttribute_ProdAttribute");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_prodAttribute_ProdAttribute");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = prodAttributeService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_prodAttribute_ProdAttribute,MESBasic_1) */
// 自定义代码
