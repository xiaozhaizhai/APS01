...
/* CUSTOM CODE START(action,import,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicProdResetDailyAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicProdResetDaily> {
	...
	private MESBasicProdResetDaily prodResetDaily;
	...
	//导出excel时,主模型内容
	List<MESBasicProdResetDaily> queryResults = new ArrayList<MESBasicProdResetDaily>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/prodResetDaily/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/product/prodResetDaily/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_product_ProdResetDaily" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicProdResetDaily> prodResetDailys;
	
	public List<MESBasicProdResetDaily> getProdResetDailys() {
		return prodResetDailys;
	}
	...
	private Page<MESBasicProdResetDaily> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			prodResetDaily = new MESBasicProdResetDaily();
			if(prodResetDaily.getReinspectUnit() != null) {
				prodResetDaily.setReinspectUnit((SystemCode) systemCodeService.getSystemCode(prodResetDaily.getReinspectUnit().getId()));
			}
		} else {
			if(id > 0){
				prodResetDaily = prodResetDailyService.getProdResetDaily(id, viewCode);
			}
		}
	...
	public Page<MESBasicProdResetDaily> getPage() {
		if (null == page) {
			page = new Page<MESBasicProdResetDaily>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicProdResetDaily getProdResetDaily() {
		return prodResetDaily;
	}		

	public void setProdResetDaily(MESBasicProdResetDaily prodResetDaily) {
		this.prodResetDaily = prodResetDaily;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/prodResetDaily/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = prodResetDailyService.getProdResetDailyAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/product/prodResetDaily/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1471690809334
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String prodResetDailyReferenceCopy() throws Exception{
			MESBasicProdResetDaily oldProdResetDaily = null;
			if(isEditNew()){
				...
				oldProdResetDaily = new MESBasicProdResetDaily();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodResetDaily.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, prodResetDaily.getId());
				...
				oldProdResetDaily = prodResetDailyService.getProdResetDaily(id);
			}
			
			
			prodResetDaily = prodResetDailyService.getProdResetDaily(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_product_ProdResetDaily");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicProdResetDaily.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProdResetDaily, null);
						Method setValue = MESBasicProdResetDaily.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(prodResetDaily, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			prodResetDaily.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_product_ProdResetDaily", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=prodResetDailyService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return prodResetDailyReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			prodResetDaily = prodResetDailyService.getProdResetDaily(this.prodResetDaily.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,prodResetDaily.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = prodResetDailyService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(prodResetDailyService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(prodResetDailyService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_product_ProdResetDaily", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"prodResetDaily\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"prodResetDaily\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"prodResetDaily\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"prodResetDaily\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			prodResetDailyService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		prodResetDailyService.findDataGridPage(dg,dgclass,getDgPage(),prodResetDaily,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(prodResetDaily.getId() != null && prodResetDaily.getId() != -1){
				prodResetDaily = prodResetDailyService.getProdResetDaily(prodResetDaily.getId());
			}
			prodResetDailyService.findDataGridPage(dg,dgclass,getDgPage(),prodResetDaily,searchCondition,params);
	...
	private void save() throws Exception {
		...
		prodResetDailyService.saveProdResetDaily(prodResetDaily,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_ProdResetDaily", "submit");
		bap_validate_datagrids=prodResetDailyService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", prodResetDaily.getId());
		...
		fileuploads.put("linkId", prodResetDaily.getId());
		bapAfterCustomMethod("MESBasic_1_product_ProdResetDaily", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/product/prodResetDaily/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProdResetDaily", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		prodResetDailyService.deleteProdResetDaily(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_product_ProdResetDaily", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/prodResetDaily/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProdResetDaily", "restore");
		prodResetDailyService.restoreProdResetDaily(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_product_ProdResetDaily", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicProdResetDaily> findExportDataInfos(Page<MESBasicProdResetDaily> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = prodResetDailyService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = prodResetDailyService.getAssProperty(p.getCode());	
									//String assModelCode = prodResetDailyService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			prodResetDailyService.findProdResetDailys(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			prodResetDailyService.findProdResetDailys(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicProdResetDaily> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicProdResetDaily result : results){
	...
	public boolean isEditNew(){
		//return null == this.prodResetDaily || null == this.prodResetDaily.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/product/prodResetDaily/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProdResetDaily", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_product_ProdResetDaily", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/product/prodResetDaily/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_product_ProdResetDaily", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/product/prodResetDaily/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/prodResetDaily/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicProdResetDailyService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_product_ProdResetDaily", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_product_ProdResetDaily", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/prodResetDaily/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_product_ProdResetDaily", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_product_ProdResetDaily", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/product/prodResetDaily/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_product_ProdResetDaily", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/product/prodResetDaily/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_product_ProdResetDaily";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_product_ProdResetDaily", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/product/prodResetDaily/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
		_auditLog.setColumnStringA(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
		_auditLog.setColumnStringC(prodResetDaily.getId().toString());
		_auditLog.setColumnStringD(MESBasicProdResetDaily.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicProdResetDailyAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, prodResetDaily,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, prodResetDaily,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,prodResetDaily);
		...
		printlog.setBusinessId(prodResetDaily.getId());
		printlog.setBussinessKey(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
		printlog.setMainDisplay(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/product/prodResetDaily/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(prodResetDaily,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/product/prodResetDaily/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(prodResetDailyService.getProdResetDaily(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/product/prodResetDaily/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/product/prodResetDaily/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, prodResetDaily,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				prodResetDaily = prodResetDailyService.getProdResetDaily(Long.parseLong(arrId[i]));
				if(null != prodResetDaily){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == prodResetDaily.getId() ? "" : "," + prodResetDaily.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == prodResetDaily.getId() ? "" : "," + prodResetDaily.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == prodResetDaily.getId() ? "" : "," + prodResetDaily.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == prodResetDaily.getId() ? "" : prodResetDaily.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, prodResetDaily,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,prodResetDaily);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/product/prodResetDaily/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_product_ProdResetDaily");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_product_ProdResetDaily")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_product_ProdResetDaily");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_product_ProdResetDaily");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = prodResetDailyService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_product_ProdResetDaily,MESBasic_1) */
// 自定义代码
