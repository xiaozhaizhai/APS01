...
import com.supcon.orchid.MESBasic.services.MESBasicPropertyTagsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicPropertyTagsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_propertyTagsService")
@Transactional
public class MESBasicPropertyTagsServiceImpl  extends BaseServiceImpl<MESBasicPropertyTags> implements MESBasicPropertyTagsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicPropertyTags> bapBaseService;
	...
	@Autowired
	private MESBasicPropertyTagsImportService propertyTagsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicPropertyTags.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  propertyTagsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicPropertyTags.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = propertyTagsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPropertyTags.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicPropertyTags.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  propertyTagsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicPropertyTags.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicPropertyTags.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicPropertyTags> list = (List<MESBasicPropertyTags>) propertyTagsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPropertyTags getPropertyTags(long id){
		return getPropertyTags(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPropertyTags getPropertyTags(long id, String viewCode){
		MESBasicPropertyTags propertyTags = propertyTagsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPropertyTagsAsJSON(long id, String include){
		MESBasicPropertyTags propertyTags = propertyTagsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(propertyTags, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.common.delete",operType="3")
	public void deletePropertyTags(MESBasicPropertyTags propertyTags){
		if(null != propertyTags && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPropertyTags.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
			AuditUtil.setColumnStringA(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
			AuditUtil.setColumnStringC(propertyTags.getId().toString());
		}
		beforeDeletePropertyTags(propertyTags);
		propertyTagsDao.deletePhysical(propertyTags);
		afterDeletePropertyTags(propertyTags);
		propertyTagsDao.flush();
		List<MESBasicPropertyTags> params = new ArrayList<MESBasicPropertyTags>();
		params.add(propertyTags);
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.common.delete",operType="3")
	public void deletePropertyTags(List<Long> propertyTagsIds){
		deletePropertyTags(propertyTagsIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.common.delete",operType="3")
	public void deletePropertyTags(List<Long> propertyTagsIds, String eventTopic) {
		List<MESBasicPropertyTags> propertyTagss = new ArrayList<MESBasicPropertyTags>();
		for(Long propertyTagsId : propertyTagsIds){
			MESBasicPropertyTags propertyTags = getPropertyTags(propertyTagsId);
			propertyTagss.add(propertyTags);
			if(null != propertyTags && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPropertyTags.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == propertyTags.getId() ? "" : "," + propertyTags.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == propertyTags.getId() ? "" : "," + propertyTags.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == propertyTags.getId() ? "" : "," + propertyTags.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
				}
			}
		}
		
		for(MESBasicPropertyTags propertyTags : propertyTagss){
			beforeDeletePropertyTags(propertyTags);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(propertyTagss != null && propertyTagss.size() > 0 ){
			for(MESBasicPropertyTags propertyTags : propertyTagss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicPropertyTags.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(propertyTags));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),propertyTags, propertyTags.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(propertyTagsIds != null && propertyTagsIds.size() > 0) {
			String hql = "delete " + MESBasicPropertyTags.JPA_NAME + " where id in(:ids)";
			Query query = propertyTagsDao.createQuery(hql);
			query.setParameterList("ids", propertyTagsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicPropertyTags propertyTagsz : propertyTagss){
				propertyTagsz.setValid(false);
				List<MESBasicPropertyTags> params = new ArrayList<MESBasicPropertyTags>();
				params.add(propertyTagsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_container");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.common.delete",operType="3")
	public void deletePropertyTags(long propertyTagsId, int propertyTagsVersion){
		this.deletePropertyTags(propertyTagsId, propertyTagsVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.common.delete",operType="3")
	public void deletePropertyTags(long propertyTagsId, int propertyTagsVersion,SignatureLog signatureLog){
		MESBasicPropertyTags propertyTags = getPropertyTags(propertyTagsId);
		...
		beforeDeletePropertyTags(propertyTags);
		propertyTagsDao.delete(propertyTagsId, propertyTagsVersion);
		afterDeletePropertyTags(propertyTags);
		propertyTagsDao.flush();
		List<MESBasicPropertyTags> params = new ArrayList<MESBasicPropertyTags>();
		params.add(propertyTags);
		...
		if(signatureLog != null ) {
			...
			businessKey=propertyTags.getId();
			signatureLog.setTableId(propertyTags.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.common.delete",operType="3")
	public void deletePropertyTags(String propertyTagsIds){
		this.deletePropertyTags(propertyTagsIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.common.delete",operType="3")
	public void deletePropertyTags(String propertyTagsIds,SignatureLog signatureLog){
		deleteCollection(propertyTagsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePropertyTags(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_container#modelCode:MESBasic_1_container_PropertyTags#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="ec.engine.restore",operType="10")
	public void restorePropertyTags(String propertyTagsIds){
		restoreCollection(propertyTagsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePropertyTags(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="ec.engine.restore",operType="10")
	public void restorePropertyTags(long propertyTagsId){
		
		findBusinessKeyUsed(propertyTagsId);	//判断业务主键是否重复
		...
		beforeRestorePropertyTags(propertyTags);
		propertyTags.setValid(true);
		propertyTagsDao.update(propertyTags);
		afterRestorePropertyTags(propertyTags);
		List<MESBasicPropertyTags> params = new ArrayList<MESBasicPropertyTags>();
		params.add(propertyTags);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long propertyTagsId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_container_PropertyTags");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicPropertyTags.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicPropertyTags.TABLE_NAME +" where id = ? )";
			List<Object> list =  propertyTagsDao.createNativeQuery(sql,propertyTagsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePropertyTags(List<MESBasicPropertyTags>  propertyTagss){
		for(MESBasicPropertyTags propertyTags:propertyTagss)  {
			savePropertyTags(propertyTags, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePropertyTags(List<MESBasicPropertyTags>  propertyTagss){
		for(MESBasicPropertyTags propertyTags:propertyTagss)  {
			savePropertyTags(propertyTags, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="ec.flow.save")
	public void savePropertyTags(MESBasicPropertyTags propertyTags, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePropertyTags(propertyTags, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="ec.flow.save")
	public void savePropertyTags(MESBasicPropertyTags propertyTags, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePropertyTags(propertyTags, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePropertyTagsCodes(MESBasicPropertyTags propertyTags) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePropertyTagsCodes(propertyTags, false);
	}
	
	public void generatePropertyTagsCodes(MESBasicPropertyTags propertyTags, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePropertyTagsSummarys(MESBasicPropertyTags propertyTags) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePropertyTagsSummarys(propertyTags, false);
	}

	public void generatePropertyTagsSummarys(MESBasicPropertyTags propertyTags, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags")
	public void savePropertyTags(MESBasicPropertyTags propertyTags,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePropertyTags(propertyTags, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags")
	public void savePropertyTags(MESBasicPropertyTags propertyTags,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_container");
		String url = null;
		if(propertyTags.getId() != null && propertyTags.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPropertyTags.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, propertyTags);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(propertyTags);
		beforeSavePropertyTags(propertyTags, viewIsView);

		if (viewIsView) {
			propertyTagsDao.saveWithRevertVersion(propertyTags);
		} else {
			if(isNew)
				propertyTagsDao.save(propertyTags);
			else
				propertyTagsDao.merge(propertyTags);
		}
		if(null != propertyTags && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPropertyTags.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
			AuditUtil.setColumnStringA(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
		...
		dealDatagridsSave(propertyTags,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePropertyTagsCodes(propertyTags, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePropertyTagsSummarys(propertyTags, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePropertyTags(propertyTags, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, propertyTags);
			}
		}
		List<MESBasicPropertyTags> params = new ArrayList<MESBasicPropertyTags>();
		propertyTagsDao.flush();
		propertyTagsDao.clear();
		propertyTags = propertyTagsDao.load(propertyTags.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicPropertyTags propertyTags){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("propertyTags", propertyTags);
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags")
	public void mergePropertyTags(MESBasicPropertyTags propertyTags, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(propertyTags);
		beforeSavePropertyTags(propertyTags);
		propertyTagsDao.merge(propertyTags);
		if(null != propertyTags && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPropertyTags.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
			AuditUtil.setColumnStringA(null == propertyTags.getId() ? "" : propertyTags.getId().toString());
		...
		dealDatagridsSave(propertyTags,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePropertyTags(propertyTags);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicPropertyTags> findPropertyTagss(Page<MESBasicPropertyTags> page, Criterion... criterions) {
		return propertyTagsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPropertyTagss(Page<MESBasicPropertyTags> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPropertyTagss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPropertyTagss(Page<MESBasicPropertyTags> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPropertyTagss(Page<MESBasicPropertyTags> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicPropertyTags> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"propertyTags\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"propertyTags\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"propertyTags\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"propertyTags\".CONTAINER IN ( SELECT ID FROM MESBASIC_CONTAINERS \"container\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  propertyTagsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  propertyTagsDao.createNativeQuery(powerOperateSql, "MESBasic_1_container").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"propertyTags\".LAY_REC ASC, \"propertyTags\".SORT ASC");
				} else {
					orderPart.append(" \"propertyTags\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) propertyTagsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=propertyTagsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = propertyTagsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicPropertyTags> result = new ArrayList<MESBasicPropertyTags>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicPropertyTags.class,propertyTagsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicPropertyTags.class,propertyTagsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicPropertyTags propertyTags : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(propertyTags.getId(), "MESBasic_container_propertyTags");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(propertyTags.getId(), "MESBasic_container_propertyTags");
							if(null != documents && !documents.isEmpty()) {
								propertyTags.setDocument(documents.get(0));
								propertyTags.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicPropertyTags> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicPropertyTags> result) {
		View view = null;
		...
		Map<Long, MESBasicPropertyTags> rsMap = new HashMap<Long, MESBasicPropertyTags>();
		for (MESBasicPropertyTags propertyTags : result) {
			ids.add(propertyTags.getId());
			rsMap.put(propertyTags.getId(), propertyTags);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(propertyTagsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = propertyTagsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_container_PropertyTags" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_container_PropertyTags" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  propertyTagsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"propertyTags\"","MESBasic_1_container","","MESBASIC_PROPERTY_TAGS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"propertyTags\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"propertyTags\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"propertyTags\"","MESBasic_1_container","","MESBASIC_PROPERTY_TAGS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_container_PropertyTags" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("propertyTags.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"propertyTags\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"propertyTags\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"propertyTags\".CID = 1 OR \"propertyTags\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("propertyTags.mc\"").append(".PROPERTY_TAGS) not in ("+notinValue+")");
			}
			
			SQLQuery query = propertyTagsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=propertyTagsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicPropertyTags.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicPropertyTags.MODULE_CODE;}
	protected String getModelCode(){return MESBasicPropertyTags.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param propertyTags 模型实例
	 */
	public void checkUniqueConstraint(MESBasicPropertyTags propertyTags){
	}
	
	
	@Override
	public List<MESBasicPropertyTags> getPropertyTagss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPropertyTags.class);
		if(params!=null){
			...
			return propertyTagsDao.findByHql("from " + MESBasicPropertyTags.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return propertyTagsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicPropertyTags> findPropertyTagssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPropertyTags.class);
	...
	@Override
	public List<MESBasicPropertyTags> findPropertyTagssByHql(String hql, Object... objects){
		
		return   propertyTagsDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicPropertyTags loadPropertyTagsByBussinessKey(MESBasicPropertyTags propertyTags){
		return loadPropertyTagsByBussinessKey(propertyTags.getId());
	}
	
	@Override
	public Page<MESBasicPropertyTags> getPropertyTagss(Page<MESBasicPropertyTags> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicPropertyTags.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = propertyTagsDao.findByPage(page, sql);
			}else{
				page = propertyTagsDao.findAllByPage(page);
			}
			return page;
		}
		page = propertyTagsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicPropertyTags> findByTreeDataGridContainer(List<MESBasicPropertyTags> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicPropertyTags propertyTags : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridContainer(Page<MESBasicPropertyTags> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicPropertyTags> propertyTagss = dgPage.getResult();
			for(MESBasicPropertyTags propertyTags : propertyTagss){
				
			}
			dgPage.setResult(propertyTagss);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPropertyTags loadPropertyTagsByBussinessKey(Serializable bussinessKey){
		return propertyTagsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicPropertyTags> findByProperty(String propertyName, Object object){
		return propertyTagsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicPropertyTags findEntityByProperty(String propertyName, Object object){
		return propertyTagsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicPropertyTags.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = propertyTagsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_container_PropertyTags","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicPropertyTags propertyTags,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,propertyTagsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicPropertyTags.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, propertyTags);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",propertyTags.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPropertyTags.TABLE_NAME +" where valid = 1";
			list = propertyTagsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicPropertyTags.TABLE_NAME;
			list = propertyTagsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicPropertyTags.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = propertyTagsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicPropertyTagsDaoImpl propertyTagsDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPropertyTags(final List<MESBasicPropertyTags> insertObjs, final List<MESBasicPropertyTags> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPropertyTags(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPropertyTags(final List<MESBasicPropertyTags> insertObjs, final List<MESBasicPropertyTags> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.radion1415869369969"),importCount));
		...
		Map<Long, MESBasicPropertyTags> oldUpdateObjsMap = new HashMap<Long, MESBasicPropertyTags>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicPropertyTags bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicPropertyTags> bizObjList = propertyTagsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicPropertyTags bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicPropertyTags> nmeObjs = new ArrayList<MESBasicPropertyTags>();
		...
		long id=autoGeneratorID.getNextId(MESBasicPropertyTags.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicPropertyTags propertyTags:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicPropertyTags.TABLE_NAME,1,dbId);
			if(propertyTags.getId() != null ){
				m.put(propertyTags.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			propertyTags.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) propertyTagsDao.createNativeQuery(sql, "MESBasic_1_container_PropertyTags"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		propertyTagsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicPropertyTags.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicPropertyTags propertyTags = updateObjs.get(updateIndex);
						Long id = propertyTags.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = propertyTags.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(propertyTags);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicPropertyTags.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicPropertyTags.TABLE_NAME + " order by id desc";
//					Query query = propertyTagsDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicPropertyTags propertyTags = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = propertyTags.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		propertyTagsDaoImpl.getSession().flush();
		propertyTagsDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicPropertyTags bizObj = insertObjs.get(insertIndex);
			propertyTagsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicPropertyTags bizObj = updateObjs.get(updateIndex);
			MESBasicPropertyTags oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			propertyTagsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicPropertyTags> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicPropertyTags> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) propertyTagsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicPropertyTags> result = getResult(page, query, new PendingResultTransformer(MESBasicPropertyTags.class,propertyTagsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicPropertyTags model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = propertyTagsDao.createNativeQuery(sql, "MESBasic_1_container_PropertyTags").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = propertyTagsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = propertyTagsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = propertyTagsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?propertyTagsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  propertyTagsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,propertyTagsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="ec.flow.save")
	public void savePropertyTags(MESBasicPropertyTags propertyTags, DataGridService dataGridService){
		savePropertyTags(propertyTags, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="ec.flow.save")
	public void savePropertyTags(MESBasicPropertyTags propertyTags, DataGridService dataGridService, String viewCode){
		savePropertyTags(propertyTags, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags")
	public void savePropertyTags(MESBasicPropertyTags propertyTags, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePropertyTags(propertyTags, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags")
	public void savePropertyTags(MESBasicPropertyTags propertyTags, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_container";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_container");
		String url = null;
		if(null != propertyTags.getId() && propertyTags.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicPropertyTags.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags")
	public Map<Object, Object> propertyTagsDataGridImport(MESBasicPropertyTags propertyTags, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = propertyTags.getVirtualId();
		
		propertyTags = propertyTagsDao.load(propertyTags.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, propertyTags.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = propertyTags.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(propertyTags);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, propertyTags.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags")
	public void mergePropertyTags(MESBasicPropertyTags propertyTags, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(propertyTags);
		beforeSavePropertyTags(propertyTags);
		propertyTagsDao.merge(propertyTags);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_PropertyTags",desc="foundation.batch.save",operType="4")
	public void batchSavePropertyTags(MESBasicPropertyTags propertyTags, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_container";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_container");
	...
	@Override
	public Page<MESBasicPropertyTags> getByPage(Page<MESBasicPropertyTags> page,DetachedCriteria detachedCriteria){
		return propertyTagsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSavePropertyTags(MESBasicPropertyTags propertyTags, Object... objects){
		checkUniqueConstraint(propertyTags);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePropertyTags(MESBasicPropertyTags propertyTags, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPropertyTags(List<MESBasicPropertyTags> propertyTagss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPropertyTags(List<MESBasicPropertyTags> propertyTagss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPropertyTags(List<MESBasicPropertyTags> propertyTagss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPropertyTags(List<MESBasicPropertyTags> propertyTagss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePropertyTags(MESBasicPropertyTags propertyTags){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePropertyTags(MESBasicPropertyTags propertyTags){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePropertyTags(MESBasicPropertyTags propertyTags){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePropertyTags(MESBasicPropertyTags propertyTags){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicPropertyTags propertyTags, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicPropertyTags propertyTags, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPropertyTags(List<MESBasicPropertyTags> insertObjs, List<MESBasicPropertyTags> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPropertyTags,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPropertyTags(List<MESBasicPropertyTags> insertObjs, List<MESBasicPropertyTags> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPropertyTags,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_container_PropertyTags,MESBasic_1) */
// 自定义代码
	public List<MESBasicPropertyTags> getTagProperties(String hql, Object... arg){
		return propertyTagsDao.findByHql(hql, arg);
	}
/* CUSTOM CODE END */
}
