...
/* CUSTOM CODE START(action,import,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicObjWareStoreAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicObjWareStore> {
	...
	private MESBasicObjWareStore objWareStore;
	...
	//导出excel时,主模型内容
	List<MESBasicObjWareStore> queryResults = new ArrayList<MESBasicObjWareStore>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objWareStore/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/factoryModel/objWareStore/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_factoryModel_ObjWareStore" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicObjWareStore> objWareStores;
	
	public List<MESBasicObjWareStore> getObjWareStores() {
		return objWareStores;
	}
	...
	private Page<MESBasicObjWareStore> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			objWareStore = new MESBasicObjWareStore();
		} else {
			if(id > 0){
				objWareStore = objWareStoreService.getObjWareStore(id, viewCode);
			}
		}
	...
	public Page<MESBasicObjWareStore> getPage() {
		if (null == page) {
			page = new Page<MESBasicObjWareStore>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicObjWareStore getObjWareStore() {
		return objWareStore;
	}		

	public void setObjWareStore(MESBasicObjWareStore objWareStore) {
		this.objWareStore = objWareStore;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objWareStore/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = objWareStoreService.getObjWareStoreAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/factoryModel/objWareStore/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1490058575739
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String objWareStoreReferenceCopy() throws Exception{
			MESBasicObjWareStore oldObjWareStore = null;
			if(isEditNew()){
				...
				oldObjWareStore = new MESBasicObjWareStore();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objWareStore.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, objWareStore.getId());
				...
				oldObjWareStore = objWareStoreService.getObjWareStore(id);
			}
			
			
			objWareStore = objWareStoreService.getObjWareStore(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjWareStore");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicObjWareStore.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldObjWareStore, null);
						Method setValue = MESBasicObjWareStore.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(objWareStore, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			objWareStore.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=objWareStoreService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return objWareStoreReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			objWareStore = objWareStoreService.getObjWareStore(this.objWareStore.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,objWareStore.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = objWareStoreService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(objWareStoreService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(objWareStoreService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_factoryModel_ObjWareStore", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"objWareStore\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"objWareStore\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"objWareStore\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"objWareStore\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			objWareStoreService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		objWareStoreService.findDataGridPage(dg,dgclass,getDgPage(),objWareStore,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(objWareStore.getId() != null && objWareStore.getId() != -1){
				objWareStore = objWareStoreService.getObjWareStore(objWareStore.getId());
			}
			objWareStoreService.findDataGridPage(dg,dgclass,getDgPage(),objWareStore,searchCondition,params);
	...
	private void save() throws Exception {
		...
		objWareStoreService.saveObjWareStore(objWareStore,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "submit");
		bap_validate_datagrids=objWareStoreService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", objWareStore.getId());
		...
		fileuploads.put("linkId", objWareStore.getId());
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/factoryModel/objWareStore/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		objWareStoreService.deleteObjWareStore(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objWareStore/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "restore");
		objWareStoreService.restoreObjWareStore(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicObjWareStore> findExportDataInfos(Page<MESBasicObjWareStore> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = objWareStoreService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = objWareStoreService.getAssProperty(p.getCode());	
									//String assModelCode = objWareStoreService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			objWareStoreService.findObjWareStores(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			objWareStoreService.findObjWareStores(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicObjWareStore> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicObjWareStore result : results){
	...
	public boolean isEditNew(){
		//return null == this.objWareStore || null == this.objWareStore.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/factoryModel/objWareStore/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/factoryModel/objWareStore/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/factoryModel/objWareStore/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objWareStore/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicObjWareStoreService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objWareStore/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_factoryModel_ObjWareStore", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_factoryModel_ObjWareStore", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/factoryModel/objWareStore/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/factoryModel/objWareStore/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_factoryModel_ObjWareStore";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_factoryModel_ObjWareStore", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/factoryModel/objWareStore/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
		_auditLog.setColumnStringA(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
		_auditLog.setColumnStringC(objWareStore.getId().toString());
		_auditLog.setColumnStringD(MESBasicObjWareStore.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicObjWareStoreAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, objWareStore,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, objWareStore,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,objWareStore);
		...
		printlog.setBusinessId(objWareStore.getId());
		printlog.setBussinessKey(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
		printlog.setMainDisplay(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/factoryModel/objWareStore/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(objWareStore,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/factoryModel/objWareStore/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(objWareStoreService.getObjWareStore(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/factoryModel/objWareStore/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/factoryModel/objWareStore/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, objWareStore,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objWareStore = objWareStoreService.getObjWareStore(Long.parseLong(arrId[i]));
				if(null != objWareStore){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == objWareStore.getId() ? "" : "," + objWareStore.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == objWareStore.getId() ? "" : "," + objWareStore.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == objWareStore.getId() ? "" : "," + objWareStore.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == objWareStore.getId() ? "" : objWareStore.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, objWareStore,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,objWareStore);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/factoryModel/objWareStore/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjWareStore");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_factoryModel_ObjWareStore")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_factoryModel_ObjWareStore");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_factoryModel_ObjWareStore");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = objWareStoreService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_factoryModel_ObjWareStore,MESBasic_1) */
// 自定义代码
 private Long factoryId;
		...
		@Action(value = "/MESBasic/factoryModel/factoryModel/checkWareFactory", results = { @Result(type = JSON, params = {
				"root", "responseMap" }) })
		public String checkWareFactory() {
			String result=objWareStoreService.checkWareFactory(factoryId, productArr);
