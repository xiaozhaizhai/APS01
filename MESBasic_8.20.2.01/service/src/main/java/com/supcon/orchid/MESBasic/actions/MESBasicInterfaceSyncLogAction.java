...
/* CUSTOM CODE START(action,import,MESBasic_1_interfaceSyncLog_InterfaceSyncLog,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicInterfaceSyncLogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicInterfaceSyncLog> {
	...
	private MESBasicInterfaceSyncLog interfaceSyncLog;
	...
	//导出excel时,主模型内容
	List<MESBasicInterfaceSyncLog> queryResults = new ArrayList<MESBasicInterfaceSyncLog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/interfaceSyncLog/interfaceSyncLog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/interfaceSyncLog/interfaceSyncLog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_interfaceSyncLog_InterfaceSyncLog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicInterfaceSyncLog> interfaceSyncLogs;
	
	public List<MESBasicInterfaceSyncLog> getInterfaceSyncLogs() {
	...
	private Page<MESBasicInterfaceSyncLog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			interfaceSyncLog = new MESBasicInterfaceSyncLog();
		} else {
			if(id > 0){
	...
	public Page<MESBasicInterfaceSyncLog> getPage() {
		if (null == page) {
			page = new Page<MESBasicInterfaceSyncLog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicInterfaceSyncLog getInterfaceSyncLog() {
		return interfaceSyncLog;
	}		

	public void setInterfaceSyncLog(MESBasicInterfaceSyncLog interfaceSyncLog) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1493192130394
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String interfaceSyncLogReferenceCopy() throws Exception{
			MESBasicInterfaceSyncLog oldInterfaceSyncLog = null;
			if(isEditNew()){
				...
				oldInterfaceSyncLog = new MESBasicInterfaceSyncLog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
			...
			Model model = modelServiceFoundation.getModel("MESBasic_1_interfaceSyncLog_InterfaceSyncLog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicInterfaceSyncLog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldInterfaceSyncLog, null);
						Method setValue = MESBasicInterfaceSyncLog.class.getMethod(setMethod, new Class[]{returnType});
			...
			bapAfterCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/logList", results = { @Result(type = FREEMARKER, location = "logList.ftl") })
	})
			...
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/logList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.logType,result.logTypeDes,result.logFault,result.logOperate,result.logSynFlag,result.logIsIgnore,result.logID,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	@Actions({
		...
		@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/logList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/allTempleteDownload", results = { @Result(name="excel",type="excel",location="logList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_interfaceSyncLog_InterfaceSyncLog");
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "submit");
		bap_validate_datagrids=interfaceSyncLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		...
		bapAfterCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		...
		bapAfterCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "restore");
		interfaceSyncLogService.restoreInterfaceSyncLog(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "showDealInfoData");
		if (null != tableInfoId) {
		...
		bapAfterCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicInterfaceSyncLog> findExportDataInfos(Page<MESBasicInterfaceSyncLog> page) {
		...
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicInterfaceSyncLog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicInterfaceSyncLog result : results){
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/interfaceSyncLog/interfaceSyncLog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/interfaceSyncLog/interfaceSyncLog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicInterfaceSyncLogService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_interfaceSyncLog", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_interfaceSyncLog");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_interfaceSyncLog_InterfaceSyncLog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_interfaceSyncLog_InterfaceSyncLog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_interfaceSyncLog_InterfaceSyncLog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_interfaceSyncLog_InterfaceSyncLog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicInterfaceSyncLog interfaceSyncLog = null;
		if(null != id) {
		...
		_auditLog.setColumnStringD(MESBasicInterfaceSyncLog.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_interfaceSyncLog");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicInterfaceSyncLogAction");
		...
		printlog.setEntity("MESBasic_1_interfaceSyncLog");
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/loadBatchXmlRefAndData",
	...
	@Action(value = "/MESBasic/interfaceSyncLog/interfaceSyncLog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicInterfaceSyncLog> findExportPageInfos(Page<MESBasicInterfaceSyncLog> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_interfaceSyncLog" + "_" + model.get("viewName");	
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicInterfaceSyncLog queryResult : queryResults){
...
//		}
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_interfaceSyncLog_InterfaceSyncLog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_interfaceSyncLog_InterfaceSyncLog")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_interfaceSyncLog_InterfaceSyncLog");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_interfaceSyncLog_InterfaceSyncLog");
		}
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_interfaceSyncLog_InterfaceSyncLog,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_interfaceSyncLog_InterfaceSyncLog,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_interfaceSyncLog_InterfaceSyncLog,MESBasic_1) */
// 自定义代码
