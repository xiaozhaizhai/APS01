...
import com.supcon.orchid.MESBasic.entities.MESBasicEamTypeMneCode;
import com.supcon.orchid.MESBasic.services.MESBasicEamTypeService;
import com.supcon.orchid.MESBasic.services.MESBasicEamTypeImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicEamTypeDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_eamTypeService")
@Transactional
public class MESBasicEamTypeServiceImpl  extends BaseServiceImpl<MESBasicEamType> implements MESBasicEamTypeService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicEamType> bapBaseService;
	...
	@Autowired
	private MESBasicEamTypeImportService eamTypeImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicEamType.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  eamTypeDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicEamType.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = eamTypeDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicEamType.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicEamType.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  eamTypeDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicEamType.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicEamType.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicEamType> list = (List<MESBasicEamType>) eamTypeDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public MESBasicEamType getEamType(long id){
		return getEamType(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicEamType getEamType(long id, String viewCode){
		MESBasicEamType eamType = eamTypeDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getEamTypeAsJSON(long id, String include){
		MESBasicEamType eamType = eamTypeDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(eamType, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.common.delete",operType="3")
	public void deleteEamType(MESBasicEamType eamType){
		if(null != eamType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEamType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == eamType.getCode() ? "" : eamType.getCode().toString());
			AuditUtil.setColumnStringA(null == eamType.getName() ? "" : eamType.getName().toString());
			AuditUtil.setColumnStringC(eamType.getId().toString());
		}
		checkDeletePermit(eamType.getId());
		beforeDeleteEamType(eamType);
		eamTypeDao.delete(eamType);
		afterDeleteEamType(eamType);
		eamTypeDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/eamType/del", eamType,MESBasicEamTypeMneCode.class);
		List<MESBasicEamType> params = new ArrayList<MESBasicEamType>();
		params.add(eamType);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_eamType");
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.common.delete",operType="3")
	public void deleteEamType(List<Long> eamTypeIds){
		deleteEamType(eamTypeIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.common.delete",operType="3")
	public void deleteEamType(List<Long> eamTypeIds, String eventTopic) {
		List<MESBasicEamType> eamTypes = new ArrayList<MESBasicEamType>();
		for(Long eamTypeId : eamTypeIds){
			MESBasicEamType eamType = getEamType(eamTypeId);
			eamTypes.add(eamType);
			if(null != eamType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEamType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == eamType.getName() ? "" : "," + eamType.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == eamType.getName() ? "" : eamType.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == eamType.getCode() ? "" : "," + eamType.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == eamType.getCode() ? "" : eamType.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == eamType.getId() ? "" : "," + eamType.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == eamType.getId() ? "" : eamType.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(eamTypes != null && eamTypes.size() > 0 ){
			for(MESBasicEamType eamType : eamTypes){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),eamType, eamType.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(eamTypeIds != null && eamTypeIds.size() > 0) {
			String hql = "update " + MESBasicEamType.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = eamTypeDao.createQuery(hql);
			query.setParameterList("ids", eamTypeIds);
			query.executeUpdate();
			eamTypeDao.clearAllCacheByEntity();
		}
		...
		if(eventTopic==null){
			for(MESBasicEamType eamTypez : eamTypes){
				eamTypez.setValid(false);
				List<MESBasicEamType> params = new ArrayList<MESBasicEamType>();
				params.add(eamTypez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.common.delete",operType="3")
	public void deleteEamType(long eamTypeId, int eamTypeVersion){
		this.deleteEamType(eamTypeId, eamTypeVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.common.delete",operType="3")
	public void deleteEamType(long eamTypeId, int eamTypeVersion,SignatureLog signatureLog){
		MESBasicEamType eamType = getEamType(eamTypeId);
		if(eamType != null && !eamType.isValid()){
		...
		checkDeletePermit(eamTypeId);
		beforeDeleteEamType(eamType);
		eamTypeDao.delete(eamTypeId, eamTypeVersion);
		afterDeleteEamType(eamType);
		eamTypeDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/eamType/del", eamType,MESBasicEamTypeMneCode.class);
		List<MESBasicEamType> params = new ArrayList<MESBasicEamType>();
		...
		if(signatureLog != null ) {
			...
			businessKey=eamType.getCode();
			signatureLog.setTableId(eamType.getId());
			if(businessKey != null) {
	...
	public void checkDeletePermit(Long id) throws BAPException{
		String eamTypeHql = "select count(eamType.id) as totalCoual from " + MESBasicEamType.JPA_NAME + " as eamType where eamType.parentId=? and eamType.valid=true";
		List<Long> eamTypeCount = eamTypeDao.findByHql(eamTypeHql, id);
		if (eamTypeCount.get(0) > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.common.delete",operType="3")
	public void deleteEamType(String eamTypeIds){
		this.deleteEamType(eamTypeIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.common.delete",operType="3")
	public void deleteEamType(String eamTypeIds,SignatureLog signatureLog){
		deleteCollection(eamTypeIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteEamType(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_eamType#modelCode:MESBasic_1_eamType_EamType#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="ec.engine.restore",operType="10")
	public void restoreEamType(String eamTypeIds){
		restoreCollection(eamTypeIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreEamType(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="ec.engine.restore",operType="10")
	public void restoreEamType(long eamTypeId){
		
		findBusinessKeyUsed(eamTypeId);	//判断业务主键是否重复
		...
		checkDeletePermit(eamTypeId);
		beforeRestoreEamType(eamType);
		eamType.setValid(true);
		eamTypeDao.update(eamType);
		afterRestoreEamType(eamType);
		eamTypeDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/eamType/add", eamType,MESBasicEamTypeMneCode.class);
		List<MESBasicEamType> params = new ArrayList<MESBasicEamType>();
		params.add(eamType);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long eamTypeId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_eamType_EamType");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicEamType.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicEamType.TABLE_NAME +" where id = ? )";
			List<Object> list =  eamTypeDao.createNativeQuery(sql,eamTypeId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseEamType(List<MESBasicEamType>  eamTypes){
		for(MESBasicEamType eamType:eamTypes)  {
			saveEamType(eamType, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseEamType(List<MESBasicEamType>  eamTypes){
		for(MESBasicEamType eamType:eamTypes)  {
			saveEamType(eamType, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="ec.flow.save")
	public void saveEamType(MESBasicEamType eamType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveEamType(eamType, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="ec.flow.save")
	public void saveEamType(MESBasicEamType eamType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveEamType(eamType, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateEamTypeCodes(MESBasicEamType eamType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateEamTypeCodes(eamType, false);
	}
	
	public void generateEamTypeCodes(MESBasicEamType eamType, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
			eamTypeDao.flush();
			eamTypeDao.clear();
			MESBasicEamType modelinstance = getEamType(eamType.getId());
			...
			boolean isLegal;
		// code
		if (eamType.getCode() == null || eamType.getCode().isEmpty()) {
			String code_property_code = "MESBasic_1_eamType_EamType_code";
			...
			if(null != property){
				...
				if(isLegal){
					...
					eamType.setCode(codenewCodeValue);
				}
				
				if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEamType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if (viewIsView) {
			eamTypeDao.saveWithRevertVersion(modelinstance);
		} else {
			eamTypeDao.save(modelinstance);
		}
	}
	
	public void generateEamTypeSummarys(MESBasicEamType eamType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateEamTypeSummarys(eamType, false);
	}

	public void generateEamTypeSummarys(MESBasicEamType eamType, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType")
	public void saveEamType(MESBasicEamType eamType,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveEamType(eamType, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType")
	public void saveEamType(MESBasicEamType eamType,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_eamType";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_eamType");
		String url = null;
		if(eamType.getId() != null && eamType.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEamType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_eamType/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEamType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_eamType/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, eamType);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(eamType);
		beforeSaveEamType(eamType, viewIsView);

		if (viewIsView) {
			eamTypeDao.saveWithRevertVersion(eamType);
		} else {
			eamTypeDao.save(eamType);
		}
		if(null != eamType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEamType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == eamType.getCode() ? "" : eamType.getCode().toString());
			AuditUtil.setColumnStringA(null == eamType.getName() ? "" : eamType.getName().toString());
		...
		dealDatagridsSave(eamType,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateEamTypeCodes(eamType, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateEamTypeSummarys(eamType, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveEamType(eamType, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, eamType);
			}
		}
		List<MESBasicEamType> params = new ArrayList<MESBasicEamType>();
		eamTypeDao.flush();
		eamTypeDao.clear();
		eamType = eamTypeDao.load(eamType.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicEamType eamType){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType")
	public void mergeEamType(MESBasicEamType eamType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(eamType);
		beforeSaveEamType(eamType);
		eamTypeDao.merge(eamType);
		if(null != eamType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEamType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == eamType.getCode() ? "" : eamType.getCode().toString());
			AuditUtil.setColumnStringA(null == eamType.getName() ? "" : eamType.getName().toString());
			AuditUtil.setColumnStringC(eamType.getId().toString());
		}
		dealDatagridsSave(eamType,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveEamType(eamType);
		eamTypeDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/eamType/add", eamType,MESBasicEamTypeMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicEamType> findEamTypes(Page<MESBasicEamType> page, Criterion... criterions) {
		return eamTypeDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicEamTypeDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicEamTypeDealInfo> list=eamTypeDao.createQuery("from " + MESBasicEamTypeDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicEamTypeDealInfo dealInfo) {
		eamTypeDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicEamTypeDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) eamTypeDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicEamTypeDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = eamTypeDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = eamTypeDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicEamTypeDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = eamTypeDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	public List<String> getContainLower(String modelName, String fieldName, String exp, Object value, Boolean crossCompany) {
		...
		if(crossCompany) {
			layRecHql += " and cid = ?";
			return eamTypeDao.createQuery(layRecHql, value, getCurrentCompanyId()).list();
		}
		return eamTypeDao.createQuery(layRecHql, value).list();
	}
	
	private void dealListTreePageData(Page<MESBasicEamType> eamTypes, String viewCode){
		List<MESBasicEamType> result = eamTypes.getResult();
		if(null != result && !result.isEmpty()) {
			int rootPos = 0;
			MESBasicEamType rootTree = result.get(0);
			...
			eamTypes.setTreeToSurfaceMode(true);
			MESBasicEamType eamTypeTree = eamTypeDao.buildTree("eamType" + viewCode, new ListPrepareData(eamTypes), false);
			if(null != eamTypeTree.getChildren() && !eamTypeTree.getChildren().isEmpty()) {
				List<MESBasicEamType> treeList = new ArrayList<MESBasicEamType>();
				Tree.treeToList(eamTypeTree, treeList, false);
				if(treeList.size() == result.size()) {
					eamTypes.setResult(treeList);
					eamTypes.setTreeToSurfaceMode(false);
				} 
			} 
		}
	}
	 ...
	 * 
	 */
	private class ListPrepareData implements PrepareData<MESBasicEamType,Long> {
		private Page<MESBasicEamType> eamTypes;

		public ListPrepareData(Page<MESBasicEamType> eamTypes) {
			this.eamTypes = eamTypes;
		}

		@Override
			public MESBasicEamType root() {
				MESBasicEamType eamType = new MESBasicEamType();
				eamType.setId(-1l);
				return eamType;
			}
			
			@Override
			public List<MESBasicEamType> prepare(TreeDao<MESBasicEamType, Long> dao) {
				return eamTypes.getResult();
			}
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findEamTypes(Page<MESBasicEamType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findEamTypes(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findEamTypes(Page<MESBasicEamType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findEamTypes(Page<MESBasicEamType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicEamType> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"eamType\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"eamType\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"eamType\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_eamType_EamType" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_eamType_EamType" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  eamTypeDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  eamTypeDao.createNativeQuery(powerOperateSql, "MESBasic_1_eamType").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"eamType\"","MESBasic_1_eamType",customCondition,"EAM_EAMTYPE",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"eamType\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicEamTypeDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"eamType\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"eamType\".EAMTYPE_LAYREC ASC, \"eamType\".SORT ASC");
				} else {
					orderPart.append(" \"eamType\".EAMTYPE_LAYREC ASC, \"eamType\".EAMTYPE_ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) eamTypeDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=eamTypeDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = eamTypeDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicEamType> result = new ArrayList<MESBasicEamType>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicEamType.class,eamTypeDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicEamType.class,eamTypeDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicEamType eamType : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(eamType.getId(), "MESBasic_eamType_eamType");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(eamType.getId(), "MESBasic_eamType_eamType");
							if(null != documents && !documents.isEmpty()) {
								eamType.setDocument(documents.get(0));
								eamType.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(page.isExportFlag()){
					...
					if(exportMap.get("businessKey")!=null){
						try {
							...
							MESBasicEamType eamType = new MESBasicEamType();
							getBusKeyMethod = eamType.getClass().getMethod(getBusKeyMethodStr);
					...
					if(exportMap.get("mainDisplay")!=null){
						try {
							...
							MESBasicEamType eamType = new MESBasicEamType();
							getMainDisMethod = eamType.getClass().getMethod(getMainDisMethodStr);
						} catch (NoSuchMethodException | SecurityException e) {
							log.error(e.getMessage(),e);
						}
					}
					...
					for (MESBasicEamType eamType : result) {
						Long parentId = eamType.getParentId();
						if(parentId != null && parentId.longValue() != new Long(-1).longValue()){
							for (MESBasicEamType eamType2 : result) {
								if(eamType2.getId() != null && eamType2.getId().longValue() == parentId.longValue()){
									try {
										if(getBusKeyMethod != null){
											Object busKey = getBusKeyMethod.invoke(eamType2);
											eamType.setParentNodeBusinessKey(busKey.toString());
										}
										if(getMainDisMethod != null){
											Object mainDis = getMainDisMethod.invoke(eamType2);
											eamType.setParentNodeMainDisplay(mainDis.toString());
										}	
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_eamType")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_eamType")+"MESBasic_1_eamType".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_eamType",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicEamType> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicEamType> result) {
		View view = null;
		...
		Map<Long, MESBasicEamType> rsMap = new HashMap<Long, MESBasicEamType>();
		for (MESBasicEamType eamType : result) {
			ids.add(eamType.getId());
			rsMap.put(eamType.getId(), eamType);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(eamTypeDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = eamTypeDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicEamType eamType,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(eamType.getOwnerStaffId());
		ti.setOwnerPositionId(eamType.getOwnerPositionId());
		ti.setOwnerDepartmentId(eamType.getOwnerDepartmentId());
		
		ti.setCreateStaff(eamType.getCreateStaff());
		ti.setCreateStaffId(eamType.getCreateStaffId());
		ti.setCreateTime(eamType.getCreateTime());
		ti.setDeleteStaff(eamType.getDeleteStaff());
		ti.setDeleteStaffId(eamType.getDeleteStaffId());
		ti.setDeleteTime(eamType.getDeleteTime());
		ti.setCreateDepartmentId(eamType.getCreateDepartmentId());
		ti.setModifyStaff(eamType.getModifyStaff());
		ti.setModifyStaffId(eamType.getModifyStaffId());
		ti.setCreatePositionId(eamType.getCreatePositionId());
		ti.setPositionLayRec(eamType.getPositionLayRec());
		ti.setTableNo(eamType.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_eamType");
		ti.setTargetTableName("EAM_EAMTYPE");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_eamType_EamType" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  eamTypeDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"eamType\"","MESBasic_1_eamType","","EAM_EAMTYPE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"eamType\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"eamType\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"eamType\"","MESBasic_1_eamType","","EAM_EAMTYPE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("eamType.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"eamType\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("eamType.mc\"").append(".EAM_TYPE) not in ("+notinValue+")");
			}
			
			SQLQuery query = eamTypeDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("eamType", "MESBasic_1_eamType", "eamType_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
			//新的编码字段不需要初始化pattern
		this.codeCounter = this.counterManager.addCounter("MESBasic_1_eamType_EamType_code", "MESBasic_1_eamType", formatStr, CounterType.All);
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=eamTypeDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicEamType.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicEamType.MODULE_CODE;}
	protected String getModelCode(){return MESBasicEamType.MODEL_CODE;}
	
	@Override
	public List<MESBasicEamType> getTreeChildren(Long parentId, String cond, Object... params) {
		MESBasicEamType parent = eamTypeDao.getTree("eamType_tree_" + parentId, parentId, new MESBasicEamTypePrepareData(cond, params), false);
	...
	@Override
	public List<MESBasicEamType> getTreeInfo(Map<Long, String> treeInfo, String condition, List<Object> params) {
		...
		for(String layRec : mapKeys) {
			...
			MESBasicEamType eamTypeTree = eamTypeDao.getTree("MESBasicEamType" + rootLayRec + "-", rootId, new PrepareData<MESBasicEamType, Long>() {
				@Override
				public MESBasicEamType root() {
					MESBasicEamType eamType = new MESBasicEamType();
					eamType.setId(-1l);
				...
				@Override
				public List<MESBasicEamType> prepare(TreeDao<MESBasicEamType, Long> dao) {
					String hql = "from " + MESBasicEamType.JPA_NAME + " where (" + searchCondition + ") and valid = true " + conditionSql + " order by layRec asc, sort asc";
					return dao.findByHql(hql, paramsList.toArray());
				}
			}, false);
			List<MESBasicEamType> treeChildren = new ArrayList<MESBasicEamType>();
			Tree.treeToList(eamTypeTree, treeChildren, true);
			treeList.addAll(treeChildren);
		}
		return treeList;
	}

	private class MESBasicEamTypePrepareData implements PrepareData<MESBasicEamType, Long> {
		private String cond;
		private Object[] params;
		
		public MESBasicEamTypePrepareData(String cond, Object... params) {
			this.cond = cond;
			this.params = params;
		}
		...
		//@Override
		//public List<MESBasicEamType> prepare(TreeDao<MESBasicEamType, Long> dao) {
		//	String hql = "from " + MESBasicEamType.JPA_NAME;
		//	if (null != cond && cond.trim().length() > 0) {
		//		hql += " where " + cond;
		//	}
		//	List<MESBasicEamType> list = dao.findByHql(hql);
		//	return list;
		//}
		
		@Override
		public List<MESBasicEamType> prepare(TreeDao<MESBasicEamType, Long> dao) {
			List<MESBasicEamType> list = null;
			List<MESBasicEamType> result = new ArrayList<MESBasicEamType>();
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicEamType.class);
			if(params!=null&&params.length>0){
				...
				list = dao.findByHql("from " + MESBasicEamType.JPA_NAME + " " + ((cond==null || cond.isEmpty()) ? "" : "where " + cond + " order by layRec asc, sort asc")) ;
			}
			for(MESBasicEamType m : list){
				result.add(m);
				if(m.getParentId()!=-1){
					while(m.getParentId()!=-1){
						MESBasicEamType parent = dao.get(m.getParentId());
	 ...
	 * @param eamType 模型实例
	 */
	public void checkUniqueConstraint(MESBasicEamType eamType){
		if(null == eamType.getCode() || StringUtils.isEmpty(eamType.getCode().toString())){
			return;
		}
		if(eamType.getId()==null ){
			List<MESBasicEamType> eamTypes = eamTypeDao.findByCriteria(Restrictions.eq("code", eamType.getCode()));
			if(eamTypes!=null && !eamTypes.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1516068419324", getCurrentUser().getLanguage()), eamType.getCode());
			}
		} else {
			List<MESBasicEamType> eamTypes = eamTypeDao.findByCriteria(Restrictions.ne("id", eamType.getId()), Restrictions.eq("code", eamType.getCode()));
			if(eamTypes!=null && !eamTypes.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"MESBasic.propertydisplayName.randon1516068419324", getCurrentUser().getLanguage()), eamType.getCode());
			}
	...
	@Override
	public List<MESBasicEamType> getEamTypes(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicEamType.class);
		if(params!=null){
			List<Type> types = new ArrayList<Type>();
			for(Object obj : params){
			...
			return eamTypeDao.findByHql("from " + MESBasicEamType.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return eamTypeDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicEamType> findEamTypesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicEamType.class);
	...
	@Override
	public List<MESBasicEamType> findEamTypesByHql(String hql, Object... objects){
		
		return   eamTypeDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicEamType loadEamTypeByBussinessKey(MESBasicEamType eamType){
		return loadEamTypeByBussinessKey(eamType.getCode());
	}
	
	@Override
	public Page<MESBasicEamType> getEamTypes(Page<MESBasicEamType> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicEamType.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = eamTypeDao.findByPage(page, sql);
			}else{
				page = eamTypeDao.findAllByPage(page);
			}
			return page;
		}
		page = eamTypeDao.findByPage(page, detachedCriteria);
		return page;
	}
	//==============DataGrid多选控件使用 start================
	
	@Override
	public void dealEamTypeLeaf(List<Long> eamTypeIds) {
		if (eamTypeIds != null && eamTypeIds.size() > 0) {
			String hql = "update " + MESBasicEamType.JPA_NAME + " as t1 set t1.leaf=1 where id in(:ids) and not exists (select t2.id from "
					+ MESBasicEamType.JPA_NAME + " as t2 where t2.parentId = t1.id)";
			Query query = eamTypeDao.createQuery(hql);
			query.setParameterList("ids", eamTypeIds);
			query.executeUpdate();
		}
	...
	public void drag(long dragSource, long dragTarget, long sort){
		MESBasicEamType source = getEamType(dragSource);
		MESBasicEamType oldParent = source.getParent();
		
		MESBasicEamType target = null;
		if(dragTarget!=-1){
			target = getEamType(dragTarget);
		} else {
			target = new MESBasicEamType();
			target.setId(-1L);
		}
		eamTypeDao.move(source, target);
	}
	
	private void dragEamType(MESBasicEamType parent, List<MESBasicEamType> children){
		for(MESBasicEamType child : children){
			...
			saveEamType(child, null,null,null);
			List<MESBasicEamType> subChildren = getTreeChildren(child.getId(), "");
			if(subChildren!=null && subChildren.size()>0){
				dragEamType(child, subChildren);
			}
		}
	}
	
	public List<MESBasicEamType> convertTree(List<MESBasicEamType> eamTypes){
		List<MESBasicEamType> trees = new ArrayList<MESBasicEamType>();
		Map<Long, MESBasicEamType> map = new LinkedHashMap<Long, MESBasicEamType>(eamTypes.size());
		Map<Long, Boolean> isParent = new HashMap<Long, Boolean>();
		for (MESBasicEamType node : eamTypes) {
		...
		for (MESBasicEamType node : eamTypes) {
			if (null == node.getParentId() ||Long.parseLong(node.getParentId().toString()) == -1) {
				...
				MESBasicEamType parent = map.get(node.getParentId());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicEamType loadEamTypeByBussinessKey(Serializable bussinessKey){
		return eamTypeDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicEamType> findByProperty(String propertyName, Object object){
		return eamTypeDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicEamType findEntityByProperty(String propertyName, Object object){
		return eamTypeDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicEamType.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = eamTypeDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_eamType_EamType","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_eamType", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicEamType eamType,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,eamTypeDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicEamType.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, eamType);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",eamType.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicEamType.TABLE_NAME +" where valid = 1";
			list = eamTypeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicEamType.TABLE_NAME;
			list = eamTypeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicEamType.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = eamTypeDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicEamTypeDaoImpl eamTypeDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchEamType(final List<MESBasicEamType> insertObjs, final List<MESBasicEamType> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchEamType(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchEamType(final List<MESBasicEamType> insertObjs, final List<MESBasicEamType> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1516068385649"),importCount));
		...
		Map<Long, MESBasicEamType> oldUpdateObjsMap = new HashMap<Long, MESBasicEamType>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicEamType bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicEamType> bizObjList = eamTypeDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicEamType bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicEamType> nmeObjs = new ArrayList<MESBasicEamType>();
		...
		long id=autoGeneratorID.getNextId(MESBasicEamType.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicEamType eamType:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicEamType.TABLE_NAME,1,dbId);
			if(eamType.getCode() != null && !eamType.getCode().equals("") ){
				m.put(eamType.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		List<MESBasicEamType> needChangeInsertObjList = new ArrayList<MESBasicEamType>();
		if(insertNodeList != null && insertNodeList.size()>0 ){
			for(int i = 0; i < insertNodeList.size(); i++){
				...
				if(!needChange && !isRoot){
					MESBasicEamType obj = insertObjs.get(i);
					...
					MESBasicEamType parentObj = (MESBasicEamType) insertNode.get(parentNode);
					*/
		...
		if(updateNodeList != null && updateNodeList.size()>0){
			for(int i = 0; i < updateNodeList.size(); i++){
				...
				MESBasicEamType updateObj = updateObjs.get(i);
				...
				if(isRoot){
					...
					if(needChange!=null && !needChange){
						...
						MESBasicEamType parentNode = (MESBasicEamType) insertParentNode.get(parentNodeKey);
		...
		if(insertNodeList != null && insertNodeList.size()>0){
			for(int i = 0; i < insertNodeList.size(); i++){
				Map<String, Object> insertNode = insertNodeList.get(i);
				MESBasicEamType insertObj = insertObjs.get(i);
		...
		if(updateObjs != null && updateObjs.size()>0 ){
			for(MESBasicEamType obj : updateObjs){
				Object bus;
				try {
		...
		if(businessKey != null && mainDisplay != null && businessKey != mainDisplay  && paramList != null && paramList.size()>0){
			String hql = "select " + businessKey.getName() + "," + mainDisplay.getName() + ",id from " + MESBasicEamType.JPA_NAME + " where " + businessKey.getName() + " in (:codes)";
			Query query =  eamTypeDao.createQuery(hql);
			query.setParameterList("codes", paramList);
			List<Object[]> list = query.list();
			if(list.size() == updateObjs.size()){
				for(int i = 0 ; i < updateObjs.size();i++ ){
					MESBasicEamType obj = updateObjs.get(i);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) eamTypeDao.createNativeQuery(sql, "MESBasic_1_eamType_EamType"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		eamTypeDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicEamType.TABLE_NAME + " SET ";
					...
					updateSql += "parent_id = ?, EAMTYPE_LAYNO = ?, EAMTYPE_LAYREC = ?, FULL_PATH_NAME = ?";
					updateSql += " where " + dbId + " =?";
					
					updateps = conn.prepareStatement(updateSql);
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicEamType eamType = updateObjs.get(updateIndex);
						Long id = eamType.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = eamType.getClass().getMethod(updateMethods.get(i));
								} catch (NoSuchMethodException | SecurityException e) {
									log.error(e.getMessage(),e);
								}
								Object obj = null;
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicEamType.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
					...
					insertSql += dbId + ",CID,CREATE_STAFF_ID,CREATE_TIME,EFFECTIVE_STATE,PARENT_ID,EAMTYPE_LAYNO,EAMTYPE_LAYREC,FULL_PATH_NAME) VALUES (";
					for(int i = 0;i<columnMaps.size();i++){
						insertSql += "?,";
					}
					insertSql += "?,?,?,?,?,?,?,?,?)";
					
//					String idSql = "select id from " + MESBasicEamType.TABLE_NAME + " order by id desc";
//					Query query = eamTypeDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicEamType eamType = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = eamType.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = MESBasicEamTypeMneCode.class;
					...
					for(MESBasicEamType eamType:updateObjs)  {
						Long id = eamType.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(MESBasicEamType eamType:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(eamType);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, eamType.getId());
		...
		eamTypeDaoImpl.getSession().flush();
		eamTypeDaoImpl.getSession().clear();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicEamType bizObj = insertObjs.get(insertIndex);
			eamTypeImportService.saveImportDataLog(bizObj, null, "code", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicEamType bizObj = updateObjs.get(updateIndex);
			MESBasicEamType oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			eamTypeImportService.saveImportDataLog(bizObj, oldBizObj, "code", "name");
		}
	...
	private void changeFullPathName(List<Long> needChangeFullPathNameId, Map<Object, Object> needChangeFullPathName,final Property businessKey,Property mainDisplay){
		...
		sql.append(" from ");
		sql.append(MESBasicEamType.TABLE_NAME);
		sql.append(" where ");
		for(Long id : needChangeFullPathNameId){
			sql.append(" EAMTYPE_LAYREC like '%");
			sql.append(id.toString());
			sql.append("%' or");
		}
		String sqlStr = sql.toString();
		sqlStr = sqlStr.substring(0, sqlStr.length()-2);
		List<Object[]> lists =  eamTypeDao.createNativeQuery(sqlStr).list();
		...
		eamTypeDaoImpl.getSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
				...
				if(updateLists != null && updateLists.size()>0){
					String updateSql = "UPDATE " + MESBasicEamType.TABLE_NAME + " SET FULL_PATH_NAME = ? WHERE " + businessKey.getColumnName() + " = ?";
					updateps = conn.prepareStatement(updateSql);
	...
	private Map<String, Object> buildInsertObjNode(List<Map<String, Object>> insertNodeList, List<MESBasicEamType> insertObjs, 
			...
			for(int i =0; i<insertNodeList.size(); i++){
				...
				if(insertBuildedObj.get(busKey)!=null){
					...
					if(isRoot){
						...
						String parentNode = insertNode.get("parentNode").toString();
						MESBasicEamType parentObj = (MESBasicEamType) insertBuildedObj.get(parentNode);
	...
	public String getViewName(String modelCode){
		...
		String viewName = eamTypeDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				eamTypeDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = eamTypeDao.createNativeQuery(sql, "MESBasic_1_eamType_EamType").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = eamTypeDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = eamTypeDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = eamTypeDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?eamTypeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  eamTypeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,eamTypeDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="ec.flow.save")
	public void saveEamType(MESBasicEamType eamType, DataGridService dataGridService){
		saveEamType(eamType, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="ec.flow.save")
	public void saveEamType(MESBasicEamType eamType, DataGridService dataGridService, String viewCode){
		saveEamType(eamType, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType")
	public void saveEamType(MESBasicEamType eamType, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveEamType(eamType, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType")
	public void saveEamType(MESBasicEamType eamType, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_eamType";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_eamType");
		String url = null;
		if(null != eamType.getId() && eamType.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicEamType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType")
	public Map<Object, Object> eamTypeDataGridImport(MESBasicEamType eamType, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = eamType.getVirtualId();
		
		eamType = eamTypeDao.load(eamType.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, eamType.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = eamType.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(eamType);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, eamType.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType")
	public void mergeEamType(MESBasicEamType eamType, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(eamType);
		beforeSaveEamType(eamType);
		eamTypeDao.merge(eamType);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_eamType",model="MESBasic_1_eamType_EamType",desc="foundation.batch.save",operType="4")
	public void batchSaveEamType(MESBasicEamType eamType, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_eamType";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_eamType");
	...
	@Override
	public Page<MESBasicEamType> getByPage(Page<MESBasicEamType> page,DetachedCriteria detachedCriteria){
		return eamTypeDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_eamType").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveEamType(MESBasicEamType eamType, Object... objects){
		checkUniqueConstraint(eamType);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveEamType(MESBasicEamType eamType, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportEamType(List<MESBasicEamType> eamTypes, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportEamType(List<MESBasicEamType> eamTypes){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportEamType(List<MESBasicEamType> eamTypes, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportEamType(List<MESBasicEamType> eamTypes){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteEamType(MESBasicEamType eamType){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteEamType(MESBasicEamType eamType){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreEamType(MESBasicEamType eamType){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreEamType(MESBasicEamType eamType){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicEamType eamType, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicEamType eamType, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportEamType(List<MESBasicEamType> insertObjs, List<MESBasicEamType> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportEamType,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportEamType(List<MESBasicEamType> insertObjs, List<MESBasicEamType> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportEamType,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
    @Override
	public String hasBaseInfo(String eamTypeId) {
		String flag="";
		if(!eamTypeId.equals("-1")){
		String eamTypeCode=getEamType(Integer.parseInt(eamTypeId)).getCode();
		String sql="select EAM_ID,VERSION from EAM_BaseInfo where EAM_TYpe='"+eamTypeCode+"'  and VALID='1'";
		List<Object[]> baseInfoList = equipmentDao.createNativeQuery(sql).list(); 
	...
	@Override
	public String isLeaf(String eamTypeId) {
		// TODO Auto-generated method stub

		String flag="";
		if(!eamTypeId.equals("-1")){
		  String sql="select EAMTYPE_ID,VERSION from EAM_EAMTYPE where LEAF='0' and EAMTYPE_ID="+eamTypeId;
		  List<Object[]> baseInfoList = equipmentDao.createNativeQuery(sql).list(); 
		  if(baseInfoList.size()>0){
			flag="0";
		  }
