...
/* CUSTOM CODE START(action,import,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicContainerExpAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicContainerExp> {
	...
	private MESBasicContainerExp containerExp;
	...
	//导出excel时,主模型内容
	List<MESBasicContainerExp> queryResults = new ArrayList<MESBasicContainerExp>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/containerExp/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/container/containerExp/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_container_ContainerExp" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicContainerExp> containerExps;
	
	public List<MESBasicContainerExp> getContainerExps() {
		return containerExps;
	}
	...
	private Page<MESBasicContainerExp> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			containerExp = new MESBasicContainerExp();
		} else {
			if(id > 0){
				containerExp = containerExpService.getContainerExp(id, viewCode);
			}
		}
	...
	public Page<MESBasicContainerExp> getPage() {
		if (null == page) {
			page = new Page<MESBasicContainerExp>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicContainerExp getContainerExp() {
		return containerExp;
	}		

	public void setContainerExp(MESBasicContainerExp containerExp) {
		this.containerExp = containerExp;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/containerExp/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = containerExpService.getContainerExpAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/container/containerExp/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.randon1553144862986
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String containerExpReferenceCopy() throws Exception{
			MESBasicContainerExp oldContainerExp = null;
			if(isEditNew()){
				...
				oldContainerExp = new MESBasicContainerExp();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,containerExp.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, containerExp.getId());
				...
				oldContainerExp = containerExpService.getContainerExp(id);
			}
			
			
			containerExp = containerExpService.getContainerExp(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_container_ContainerExp");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicContainerExp.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldContainerExp, null);
						Method setValue = MESBasicContainerExp.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(containerExp, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			containerExp.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_container_ContainerExp", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=containerExpService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return containerExpReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			containerExp = containerExpService.getContainerExp(this.containerExp.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,containerExp.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = containerExpService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(containerExpService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(containerExpService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_container_ContainerExp", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"containerExp\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"containerExp\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"containerExp\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"containerExp\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			containerExpService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		containerExpService.findDataGridPage(dg,dgclass,getDgPage(),containerExp,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(containerExp.getId() != null && containerExp.getId() != -1){
				containerExp = containerExpService.getContainerExp(containerExp.getId());
			}
			containerExpService.findDataGridPage(dg,dgclass,getDgPage(),containerExp,searchCondition,params);
	...
	private void save() throws Exception {
		...
		containerExpService.saveContainerExp(containerExp,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_ContainerExp", "submit");
		bap_validate_datagrids=containerExpService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", containerExp.getId());
		...
		fileuploads.put("linkId", containerExp.getId());
		bapAfterCustomMethod("MESBasic_1_container_ContainerExp", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/container/containerExp/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_ContainerExp", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		containerExpService.deleteContainerExp(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_container_ContainerExp", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/containerExp/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_ContainerExp", "restore");
		containerExpService.restoreContainerExp(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_container_ContainerExp", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicContainerExp> findExportDataInfos(Page<MESBasicContainerExp> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = containerExpService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = containerExpService.getAssProperty(p.getCode());	
									//String assModelCode = containerExpService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			containerExpService.findContainerExps(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			containerExpService.findContainerExps(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicContainerExp> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicContainerExp result : results){
	...
	public boolean isEditNew(){
		//return null == this.containerExp || null == this.containerExp.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/container/containerExp/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_ContainerExp", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_container_ContainerExp", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/container/containerExp/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_container_ContainerExp", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/container/containerExp/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/containerExp/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicContainerExpService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("MESBasic_1_container_ContainerExp", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_container_ContainerExp", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/containerExp/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_container_ContainerExp", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_container_ContainerExp", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/container/containerExp/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_container_ContainerExp", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/container/containerExp/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_container_ContainerExp";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_container_ContainerExp", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/container/containerExp/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicContainerExp containerExp = null;
		if(null != id) {
			containerExp = containerExpService.getContainerExp(id);
		}
		...
		_auditLog.setColumnStringB(null == containerExp.getId() ? "" : containerExp.getId().toString());
		_auditLog.setColumnStringA(null == containerExp.getId() ? "" : containerExp.getId().toString());
		_auditLog.setColumnStringC(containerExp.getId().toString());
		_auditLog.setColumnStringD(MESBasicContainerExp.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicContainerExpAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, containerExp,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, containerExp,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,containerExp);
		...
		printlog.setBusinessId(containerExp.getId());
		printlog.setBussinessKey(null == containerExp.getId() ? "" : containerExp.getId().toString());
		printlog.setMainDisplay(null == containerExp.getId() ? "" : containerExp.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/container/containerExp/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(containerExp,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/container/containerExp/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(containerExpService.getContainerExp(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/container/containerExp/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/container/containerExp/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, containerExp,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				containerExp = containerExpService.getContainerExp(Long.parseLong(arrId[i]));
				if(null != containerExp){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == containerExp.getId() ? "" : "," + containerExp.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == containerExp.getId() ? "" : containerExp.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == containerExp.getId() ? "" : "," + containerExp.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == containerExp.getId() ? "" : containerExp.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == containerExp.getId() ? "" : "," + containerExp.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == containerExp.getId() ? "" : containerExp.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, containerExp,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,containerExp);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/container/containerExp/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_container_ContainerExp");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_container_ContainerExp")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_container_ContainerExp");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_container_ContainerExp");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = containerExpService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
