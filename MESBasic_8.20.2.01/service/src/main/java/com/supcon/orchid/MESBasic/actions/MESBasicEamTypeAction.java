...
/* CUSTOM CODE START(action,import,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicEamTypeAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicEamType> {
	...
	//导出excel时,主模型内容
	List<MESBasicEamType> queryResults = new ArrayList<MESBasicEamType>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/eamType/eamType/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/eamType/eamType/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_eamType_EamType" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicEamType> eamTypes;
	
	public List<MESBasicEamType> getEamTypes() {
		return eamTypes;
	}
	...
	private Page<MESBasicEamType> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			eamType = new MESBasicEamType();
			if(eamType.getOtherspecial() != null) {
				eamType.setOtherspecial((SystemCode) systemCodeService.getSystemCode(eamType.getOtherspecial().getId()));
			}
		} else {
			if(id > 0){
				eamType = eamTypeService.getEamType(id, viewCode);
			}
		}
	...
	public Page<MESBasicEamType> getPage() {
		if (null == page) {
			page = new Page<MESBasicEamType>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/eamType/eamType/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = eamTypeService.getEamTypeAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/eamType/eamType/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String eamTypeReferenceCopy() throws Exception{
			MESBasicEamType oldEamType = null;
			if(isEditNew()){
				...
				oldEamType = new MESBasicEamType();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,eamType.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, eamType.getId());
				...
				oldEamType = eamTypeService.getEamType(id);
			}
			
			
			eamType = eamTypeService.getEamType(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_eamType_EamType");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicEamType.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldEamType, null);
						Method setValue = MESBasicEamType.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(eamType, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			eamType.setCreateDepartmentId(oldEamType.getCreateDepartmentId());
			eamType.setCreatePositionId(oldEamType.getCreatePositionId());
			eamType.setCreateStaffId(oldEamType.getCreateStaffId());
			eamType.setOwnerDepartmentId(oldEamType.getOwnerDepartmentId());
			eamType.setOwnerPositionId(oldEamType.getOwnerPositionId());
			eamType.setOwnerStaffId(oldEamType.getOwnerStaffId());
			eamType.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_eamType_EamType", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/eamType/eamType/typeEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "typeEdit.ftl") }),
		@Action(value = "/MESBasic/eamType/eamType/typeEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "typeEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=eamTypeService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return eamTypeReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			eamType = eamTypeService.getEamType(this.eamType.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,eamType.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/eamType/eamType/typePart", results = { @Result(type = FREEMARKER, location = "typePart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/eamType/eamType/typeLayout", results = { @Result(type = FREEMARKER, location = "typeLayout.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/eamType/eamType/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = eamTypeService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(eamTypeService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(eamTypeService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_eamType_EamType", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/eamType/eamType/typeAttributerRef", results = { @Result(type = FREEMARKER, location = "typeAttributerRef.ftl") })
		,
		@Action(value = "/MESBasic/eamType/eamType/typeRef", results = { @Result(type = FREEMARKER, location = "typeRef.ftl") })
		,
		@Action(value = "/MESBasic/eamType/eamType/typeTree", results = { @Result(type = FREEMARKER, location = "typeTree.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/eamType/eamType/typePart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.typeMemo,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"eamType\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"eamType\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"eamType\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"eamType\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/eamType/eamType/typePart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/eamType/eamType/typeAttributerRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.typeMemo,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="typeAttributerRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/eamType/eamType/typeRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.typeMemo,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="typeRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/MESBasic/eamType/eamType/typeTreeDrag")
	})
	public String viewTreeDrag() throws Exception{
		eamTypeService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/MESBasic/eamType/eamType/typeAttributerRefSort")
	,
	@Action(value="/MESBasic/eamType/eamType/typeEditSort")
	,
	@Action(value="/MESBasic/eamType/eamType/typeLayoutSort")
	,
	@Action(value="/MESBasic/eamType/eamType/typeMneSort")
	,
	@Action(value="/MESBasic/eamType/eamType/typePartSort")
	,
	@Action(value="/MESBasic/eamType/eamType/typeRefSort")
	,
	@Action(value="/MESBasic/eamType/eamType/typeTreeSort")
	})
	...
	@Action(value="/MESBasic/eamType/eamType/typeTreeData",
			results = { @Result(type = JSON, params = { "root", "eamTypes", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"eamType\".", "{alias}.");
					}
		...
		eamTypes = eamTypeService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/MESBasic/eamType/eamType/typeTreeFullData",
			results = { @Result(type = JSON, params = { "root", "eamTypes", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			eamTypes = eamTypeService.getTreeInfo(treeInfo, null, null);
		}
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/eamType/eamType/allTempleteDownload", results = { @Result(name="excel",type="excel",location="typePart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_eamType_EamType");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		eamTypeService.findDataGridPage(dg,dgclass,getDgPage(),eamType,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(eamType.getId() != null && eamType.getId() != -1){
				eamType = eamTypeService.getEamType(eamType.getId());
			}
			eamTypeService.findDataGridPage(dg,dgclass,getDgPage(),eamType,searchCondition,params);
	...
	private void save() throws Exception {
		...
		eamTypeService.saveEamType(eamType,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/eamType/eamType/typeEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "submit");
		bap_validate_datagrids=eamTypeService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", eamType.getId());
		...
		fileuploads.put("linkId", eamType.getId());
		bapAfterCustomMethod("MESBasic_1_eamType_EamType", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/eamType/eamType/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		eamTypeService.deleteEamType(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_eamType_EamType", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/eamType/eamType/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "restore");
		eamTypeService.restoreEamType(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_eamType_EamType", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/eamType/eamType/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/eamType/eamType/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "showDealInfoData");
		if (null != tableInfoId) {
			int count=eamTypeService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = eamTypeService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = eamTypeService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_eamType_EamType", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicEamType> findExportDataInfos(Page<MESBasicEamType> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = eamTypeService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = eamTypeService.getAssProperty(p.getCode());	
									//String assModelCode = eamTypeService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
				...
				if(view.getAssModel().getDataType() == 2 && useForImportFlag){//树型结构且用于导出，加上父节点,同时在sql中加入layNo,layRec,parentId,sort,leaf
					...
					exportSqlPart1Add.append("\".EAMTYPE_LAYNO AS \"layNo\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
					exportSqlPart1Add.append("\".EAMTYPE_LAYREC AS \"layRec\", ");
					exportSqlPart1Add.append("\"");
					exportSqlPart1Add.append(sqlModelName);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			eamTypeService.findEamTypes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			eamTypeService.findEamTypes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicEamType> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicEamType result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".EAMTYPE_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.eamType || null == this.eamType.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/eamType/eamType/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_eamType_EamType", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/eamType/eamType/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/eamType/eamType/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/eamType/eamType/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicEamTypeService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_eamType", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_eamType");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_eamType_EamType", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/eamType/eamType/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_eamType_EamType", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_eamType_EamType", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/eamType/eamType/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/eamType/eamType/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_eamType_EamType";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_eamType_EamType", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/eamType/eamType/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicEamType eamType = null;
		if(null != id) {
			eamType = eamTypeService.getEamType(id);
		}
		...
		_auditLog.setColumnStringB(null == eamType.getCode() ? "" : eamType.getCode().toString());
		_auditLog.setColumnStringA(null == eamType.getName() ? "" : eamType.getName().toString());
		_auditLog.setColumnStringC(eamType.getId().toString());
		_auditLog.setColumnStringD(MESBasicEamType.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_eamType");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicEamTypeAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, eamType,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, eamType,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,eamType);
		...
		printlog.setEntity("MESBasic_1_eamType");
		...
		printlog.setBusinessId(eamType.getId());
		printlog.setBussinessKey(null == eamType.getCode() ? "" : eamType.getCode().toString());
		printlog.setMainDisplay(null == eamType.getName() ? "" : eamType.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/eamType/eamType/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(eamType,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/eamType/eamType/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(eamTypeService.getEamType(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/eamType/eamType/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/eamType/eamType/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, eamType,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				eamType = eamTypeService.getEamType(Long.parseLong(arrId[i]));
				if(null != eamType){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == eamType.getName() ? "" : "," + eamType.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == eamType.getName() ? "" : eamType.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == eamType.getCode() ? "" : "," + eamType.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == eamType.getCode() ? "" : eamType.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == eamType.getId() ? "" : "," + eamType.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == eamType.getId() ? "" : eamType.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, eamType,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,eamType);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/eamType/eamType/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicEamType> findExportPageInfos(Page<MESBasicEamType> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_eamType" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = eamTypeService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicEamType queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = eamTypeService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = eamTypeService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_eamType_EamType");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_eamType_EamType")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_eamType_EamType");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_eamType_EamType");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = eamTypeService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_eamType_EamType,MESBasic_1) */
// 自定义代码
    private String eamTypeId;


	public String getEamTypeId() {
		return eamTypeId;
	}

	public void setEamTypeId(String eamTypeId) {
		this.eamTypeId = eamTypeId;
	}
	@Action(value = "/MESBasic/eamType/eamType/hasEamTypeAndIsLeaf",results = {@Result(name = SUCCESS, type = JSON, params = {"root", "responseMap"})})
	public String hasBaseInfo(){
        if(!"-1".equals(eamTypeId)){
           	String hasBaseInfo =eamTypeService.hasBaseInfo(eamTypeId);
            String isLeaf=eamTypeService.isLeaf(eamTypeId);
            responseMap.put("hasBaseInfo", hasBaseInfo);
            responseMap.put("isLeaf", isLeaf);                                                                
        }
        else{
            responseMap.put("hasBaseInfo", "");
            responseMap.put("isLeaf", "");      
        }
		return SUCCESS;
	}
/* CUSTOM CODE END */
