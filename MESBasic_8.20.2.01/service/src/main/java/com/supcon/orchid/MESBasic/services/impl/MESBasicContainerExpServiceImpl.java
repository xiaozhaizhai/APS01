...
import com.supcon.orchid.MESBasic.services.MESBasicContainerExpImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicContainerExpDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_containerExpService")
@Transactional
public class MESBasicContainerExpServiceImpl  extends BaseServiceImpl<MESBasicContainerExp> implements MESBasicContainerExpService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicContainerExp> bapBaseService;
	...
	@Autowired
	private MESBasicContainerExpImportService containerExpImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicContainerExp.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  containerExpDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicContainerExp.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = containerExpDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicContainerExp.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicContainerExp.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  containerExpDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicContainerExp.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicContainerExp.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicContainerExp> list = (List<MESBasicContainerExp>) containerExpDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicContainerExp getContainerExp(long id){
		return getContainerExp(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicContainerExp getContainerExp(long id, String viewCode){
		MESBasicContainerExp containerExp = containerExpDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getContainerExpAsJSON(long id, String include){
		MESBasicContainerExp containerExp = containerExpDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(containerExp, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.common.delete",operType="3")
	public void deleteContainerExp(MESBasicContainerExp containerExp){
		if(null != containerExp && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicContainerExp.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == containerExp.getId() ? "" : containerExp.getId().toString());
			AuditUtil.setColumnStringA(null == containerExp.getId() ? "" : containerExp.getId().toString());
			AuditUtil.setColumnStringC(containerExp.getId().toString());
		}
		beforeDeleteContainerExp(containerExp);
		containerExpDao.deletePhysical(containerExp);
		afterDeleteContainerExp(containerExp);
		containerExpDao.flush();
		List<MESBasicContainerExp> params = new ArrayList<MESBasicContainerExp>();
		params.add(containerExp);
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.common.delete",operType="3")
	public void deleteContainerExp(List<Long> containerExpIds){
		deleteContainerExp(containerExpIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.common.delete",operType="3")
	public void deleteContainerExp(List<Long> containerExpIds, String eventTopic) {
		List<MESBasicContainerExp> containerExps = new ArrayList<MESBasicContainerExp>();
		for(Long containerExpId : containerExpIds){
			MESBasicContainerExp containerExp = getContainerExp(containerExpId);
			containerExps.add(containerExp);
			if(null != containerExp && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicContainerExp.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == containerExp.getId() ? "" : "," + containerExp.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == containerExp.getId() ? "" : containerExp.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == containerExp.getId() ? "" : "," + containerExp.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == containerExp.getId() ? "" : containerExp.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == containerExp.getId() ? "" : "," + containerExp.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == containerExp.getId() ? "" : containerExp.getId().toString());
				}
			}
		}
		
		for(MESBasicContainerExp containerExp : containerExps){
			beforeDeleteContainerExp(containerExp);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(containerExps != null && containerExps.size() > 0 ){
			for(MESBasicContainerExp containerExp : containerExps){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicContainerExp.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(containerExp));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),containerExp, containerExp.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(containerExpIds != null && containerExpIds.size() > 0) {
			String hql = "delete " + MESBasicContainerExp.JPA_NAME + " where id in(:ids)";
			Query query = containerExpDao.createQuery(hql);
			query.setParameterList("ids", containerExpIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicContainerExp containerExpz : containerExps){
				containerExpz.setValid(false);
				List<MESBasicContainerExp> params = new ArrayList<MESBasicContainerExp>();
				params.add(containerExpz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_container");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.common.delete",operType="3")
	public void deleteContainerExp(long containerExpId, int containerExpVersion){
		this.deleteContainerExp(containerExpId, containerExpVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.common.delete",operType="3")
	public void deleteContainerExp(long containerExpId, int containerExpVersion,SignatureLog signatureLog){
		MESBasicContainerExp containerExp = getContainerExp(containerExpId);
		if(containerExp != null && !containerExp.isValid()){
		...
		beforeDeleteContainerExp(containerExp);
		containerExpDao.delete(containerExpId, containerExpVersion);
		afterDeleteContainerExp(containerExp);
		containerExpDao.flush();
		List<MESBasicContainerExp> params = new ArrayList<MESBasicContainerExp>();
		params.add(containerExp);
		...
		if(signatureLog != null ) {
			...
			businessKey=containerExp.getId();
			signatureLog.setTableId(containerExp.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.common.delete",operType="3")
	public void deleteContainerExp(String containerExpIds){
		this.deleteContainerExp(containerExpIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.common.delete",operType="3")
	public void deleteContainerExp(String containerExpIds,SignatureLog signatureLog){
		deleteCollection(containerExpIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteContainerExp(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_container#modelCode:MESBasic_1_container_ContainerExp#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="ec.engine.restore",operType="10")
	public void restoreContainerExp(String containerExpIds){
		restoreCollection(containerExpIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreContainerExp(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="ec.engine.restore",operType="10")
	public void restoreContainerExp(long containerExpId){
		
		findBusinessKeyUsed(containerExpId);	//判断业务主键是否重复
		...
		beforeRestoreContainerExp(containerExp);
		containerExp.setValid(true);
		containerExpDao.update(containerExp);
		afterRestoreContainerExp(containerExp);
		List<MESBasicContainerExp> params = new ArrayList<MESBasicContainerExp>();
		params.add(containerExp);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long containerExpId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_container_ContainerExp");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicContainerExp.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicContainerExp.TABLE_NAME +" where id = ? )";
			List<Object> list =  containerExpDao.createNativeQuery(sql,containerExpId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseContainerExp(List<MESBasicContainerExp>  containerExps){
		for(MESBasicContainerExp containerExp:containerExps)  {
			saveContainerExp(containerExp, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseContainerExp(List<MESBasicContainerExp>  containerExps){
		for(MESBasicContainerExp containerExp:containerExps)  {
			saveContainerExp(containerExp, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="ec.flow.save")
	public void saveContainerExp(MESBasicContainerExp containerExp, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveContainerExp(containerExp, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="ec.flow.save")
	public void saveContainerExp(MESBasicContainerExp containerExp, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveContainerExp(containerExp, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateContainerExpCodes(MESBasicContainerExp containerExp) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateContainerExpCodes(containerExp, false);
	}
	
	public void generateContainerExpCodes(MESBasicContainerExp containerExp, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateContainerExpSummarys(MESBasicContainerExp containerExp) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateContainerExpSummarys(containerExp, false);
	}

	public void generateContainerExpSummarys(MESBasicContainerExp containerExp, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp")
	public void saveContainerExp(MESBasicContainerExp containerExp,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveContainerExp(containerExp, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp")
	public void saveContainerExp(MESBasicContainerExp containerExp,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_container");
		String url = null;
		if(containerExp.getId() != null && containerExp.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicContainerExp.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, containerExp);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(containerExp);
		beforeSaveContainerExp(containerExp, viewIsView);

		if (viewIsView) {
			containerExpDao.saveWithRevertVersion(containerExp);
		} else {
			if(isNew)
				containerExpDao.save(containerExp);
			else
				containerExpDao.merge(containerExp);
		}
		if(null != containerExp && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicContainerExp.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == containerExp.getId() ? "" : containerExp.getId().toString());
			AuditUtil.setColumnStringA(null == containerExp.getId() ? "" : containerExp.getId().toString());
		...
		dealDatagridsSave(containerExp,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateContainerExpCodes(containerExp, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateContainerExpSummarys(containerExp, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveContainerExp(containerExp, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, containerExp);
			}
		}
		List<MESBasicContainerExp> params = new ArrayList<MESBasicContainerExp>();
		containerExpDao.flush();
		containerExpDao.clear();
		containerExp = containerExpDao.load(containerExp.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicContainerExp containerExp){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("containerExp", containerExp);
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp")
	public void mergeContainerExp(MESBasicContainerExp containerExp, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(containerExp);
		beforeSaveContainerExp(containerExp);
		containerExpDao.merge(containerExp);
		if(null != containerExp && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicContainerExp.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == containerExp.getId() ? "" : containerExp.getId().toString());
			AuditUtil.setColumnStringA(null == containerExp.getId() ? "" : containerExp.getId().toString());
		...
		dealDatagridsSave(containerExp,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveContainerExp(containerExp);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicContainerExp> findContainerExps(Page<MESBasicContainerExp> page, Criterion... criterions) {
		return containerExpDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findContainerExps(Page<MESBasicContainerExp> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findContainerExps(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findContainerExps(Page<MESBasicContainerExp> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findContainerExps(Page<MESBasicContainerExp> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicContainerExp> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"containerExp\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"containerExp\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"containerExp\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"containerExp\".HEAD_ID IN ( SELECT ID FROM MESBASIC_CONTAINERS \"container\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  containerExpDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  containerExpDao.createNativeQuery(powerOperateSql, "MESBasic_1_container").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"containerExp\".LAY_REC ASC, \"containerExp\".SORT ASC");
				} else {
					orderPart.append(" \"containerExp\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) containerExpDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=containerExpDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = containerExpDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicContainerExp> result = new ArrayList<MESBasicContainerExp>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicContainerExp.class,containerExpDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicContainerExp.class,containerExpDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicContainerExp containerExp : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(containerExp.getId(), "MESBasic_container_containerExp");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(containerExp.getId(), "MESBasic_container_containerExp");
							if(null != documents && !documents.isEmpty()) {
								containerExp.setDocument(documents.get(0));
								containerExp.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicContainerExp> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicContainerExp> result) {
		View view = null;
		...
		Map<Long, MESBasicContainerExp> rsMap = new HashMap<Long, MESBasicContainerExp>();
		for (MESBasicContainerExp containerExp : result) {
			ids.add(containerExp.getId());
			rsMap.put(containerExp.getId(), containerExp);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(containerExpDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = containerExpDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_container_ContainerExp" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_container_ContainerExp" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  containerExpDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"containerExp\"","MESBasic_1_container","","MESBASIC_CONTAINER_EXPS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"containerExp\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"containerExp\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"containerExp\"","MESBasic_1_container","","MESBASIC_CONTAINER_EXPS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_container_ContainerExp" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("containerExp.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"containerExp\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"containerExp\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"containerExp\".CID = 1 OR \"containerExp\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("containerExp.mc\"").append(".CONTAINER_EXP) not in ("+notinValue+")");
			}
			
			SQLQuery query = containerExpDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=containerExpDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicContainerExp.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicContainerExp.MODULE_CODE;}
	protected String getModelCode(){return MESBasicContainerExp.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param containerExp 模型实例
	 */
	public void checkUniqueConstraint(MESBasicContainerExp containerExp){
	}
	
	
	@Override
	public List<MESBasicContainerExp> getContainerExps(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicContainerExp.class);
		if(params!=null){
			...
			return containerExpDao.findByHql("from " + MESBasicContainerExp.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return containerExpDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicContainerExp> findContainerExpsByHql(String hql, Object... objects){
		...
		return   containerExpDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicContainerExp loadContainerExpByBussinessKey(MESBasicContainerExp containerExp){
		return loadContainerExpByBussinessKey(containerExp.getId());
	}
	
	@Override
	public Page<MESBasicContainerExp> getContainerExps(Page<MESBasicContainerExp> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicContainerExp.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = containerExpDao.findByPage(page, sql);
			}else{
				page = containerExpDao.findAllByPage(page);
			}
			return page;
		}
		page = containerExpDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicContainerExp> findByTreeDataGridContainer(List<MESBasicContainerExp> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicContainerExp containerExp : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridContainer(Page<MESBasicContainerExp> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicContainerExp> containerExps = dgPage.getResult();
			for(MESBasicContainerExp containerExp : containerExps){
				
			}
			dgPage.setResult(containerExps);
		}
	...
	public MESBasicContainerExp loadContainerExpByBussinessKey(Serializable bussinessKey){
		return containerExpDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicContainerExp> findByProperty(String propertyName, Object object){
		return containerExpDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicContainerExp findEntityByProperty(String propertyName, Object object){
		return containerExpDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicContainerExp.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = containerExpDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_container_ContainerExp","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicContainerExp containerExp,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,containerExpDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicContainerExp.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, containerExp);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",containerExp.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicContainerExp.TABLE_NAME +" where valid = 1";
			list = containerExpDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicContainerExp.TABLE_NAME;
			list = containerExpDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicContainerExp.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = containerExpDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicContainerExpDaoImpl containerExpDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchContainerExp(final List<MESBasicContainerExp> insertObjs, final List<MESBasicContainerExp> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchContainerExp(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchContainerExp(final List<MESBasicContainerExp> insertObjs, final List<MESBasicContainerExp> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1553144862986"),importCount));
		...
		Map<Long, MESBasicContainerExp> oldUpdateObjsMap = new HashMap<Long, MESBasicContainerExp>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicContainerExp bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicContainerExp> bizObjList = containerExpDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicContainerExp bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicContainerExp> nmeObjs = new ArrayList<MESBasicContainerExp>();
		...
		long id=autoGeneratorID.getNextId(MESBasicContainerExp.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicContainerExp containerExp:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicContainerExp.TABLE_NAME,1,dbId);
			if(containerExp.getId() != null ){
				m.put(containerExp.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			containerExp.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) containerExpDao.createNativeQuery(sql, "MESBasic_1_container_ContainerExp"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		containerExpDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicContainerExp.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicContainerExp containerExp = updateObjs.get(updateIndex);
						Long id = containerExp.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = containerExp.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(containerExp);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicContainerExp.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicContainerExp.TABLE_NAME + " order by id desc";
//					Query query = containerExpDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicContainerExp containerExp = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = containerExp.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		containerExpDaoImpl.getSession().flush();
		containerExpDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicContainerExp bizObj = insertObjs.get(insertIndex);
			containerExpImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicContainerExp bizObj = updateObjs.get(updateIndex);
			MESBasicContainerExp oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			containerExpImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicContainerExp> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicContainerExp> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) containerExpDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicContainerExp> result = getResult(page, query, new PendingResultTransformer(MESBasicContainerExp.class,containerExpDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicContainerExp model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = containerExpDao.createNativeQuery(sql, "MESBasic_1_container_ContainerExp").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = containerExpDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = containerExpDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = containerExpDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?containerExpDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  containerExpDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,containerExpDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="ec.flow.save")
	public void saveContainerExp(MESBasicContainerExp containerExp, DataGridService dataGridService){
		saveContainerExp(containerExp, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="ec.flow.save")
	public void saveContainerExp(MESBasicContainerExp containerExp, DataGridService dataGridService, String viewCode){
		saveContainerExp(containerExp, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp")
	public void saveContainerExp(MESBasicContainerExp containerExp, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveContainerExp(containerExp, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp")
	public void saveContainerExp(MESBasicContainerExp containerExp, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_container";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_container");
		String url = null;
		if(null != containerExp.getId() && containerExp.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicContainerExp.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp")
	public Map<Object, Object> containerExpDataGridImport(MESBasicContainerExp containerExp, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = containerExp.getVirtualId();
		
		containerExp = containerExpDao.load(containerExp.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, containerExp.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = containerExp.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(containerExp);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, containerExp.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp")
	public void mergeContainerExp(MESBasicContainerExp containerExp, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(containerExp);
		beforeSaveContainerExp(containerExp);
		containerExpDao.merge(containerExp);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_container",model="MESBasic_1_container_ContainerExp",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicContainerExp> getByPage(Page<MESBasicContainerExp> page,DetachedCriteria detachedCriteria){
		return containerExpDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveContainerExp(MESBasicContainerExp containerExp, Object... objects){
		checkUniqueConstraint(containerExp);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveContainerExp(MESBasicContainerExp containerExp, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteContainerExp(MESBasicContainerExp containerExp){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteContainerExp(MESBasicContainerExp containerExp){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreContainerExp(MESBasicContainerExp containerExp){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreContainerExp(MESBasicContainerExp containerExp){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicContainerExp containerExp, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicContainerExp containerExp, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportContainerExp(List<MESBasicContainerExp> insertObjs, List<MESBasicContainerExp> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportContainerExp,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportContainerExp(List<MESBasicContainerExp> insertObjs, List<MESBasicContainerExp> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportContainerExp,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_container_ContainerExp,MESBasic_1) */
// 自定义代码
