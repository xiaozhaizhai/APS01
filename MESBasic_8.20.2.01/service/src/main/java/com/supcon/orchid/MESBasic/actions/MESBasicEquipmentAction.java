...
/* CUSTOM CODE START(action,import,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class MESBasicEquipmentAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicEquipment> {
	...
	//导出excel时,主模型内容
	List<MESBasicEquipment> queryResults = new ArrayList<MESBasicEquipment>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/equipManage/equipment/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/equipManage/equipment/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_equipManage_Equipment" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicEquipment> equipments;
	
	public List<MESBasicEquipment> getEquipments() {
		return equipments;
	}
	...
	private Page<MESBasicEquipment> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			equipment = new MESBasicEquipment();
		} else {
			if(id > 0){
				equipment = equipmentService.getEquipment(id, viewCode);
			}
		}
	...
	public Page<MESBasicEquipment> getPage() {
		if (null == page) {
			page = new Page<MESBasicEquipment>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/equipManage/equipment/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = equipmentService.getEquipmentAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/equipManage/equipment/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String equipmentReferenceCopy() throws Exception{
			MESBasicEquipment oldEquipment = null;
			if(isEditNew()){
				...
				oldEquipment = new MESBasicEquipment();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,equipment.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, equipment.getId());
				...
				oldEquipment = equipmentService.getEquipment(id);
			}
			
			
			equipment = equipmentService.getEquipment(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_equipManage_Equipment");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicEquipment.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldEquipment, null);
						Method setValue = MESBasicEquipment.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(equipment, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			equipment.setCreateDepartmentId(oldEquipment.getCreateDepartmentId());
			equipment.setCreatePositionId(oldEquipment.getCreatePositionId());
			equipment.setCreateStaffId(oldEquipment.getCreateStaffId());
			equipment.setOwnerDepartmentId(oldEquipment.getOwnerDepartmentId());
			equipment.setOwnerPositionId(oldEquipment.getOwnerPositionId());
			equipment.setOwnerStaffId(oldEquipment.getOwnerStaffId());
			equipment.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_equipManage_Equipment", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/equipManage/equipment/edit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "edit.ftl") }),
		@Action(value = "/MESBasic/equipManage/equipment/editPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "editPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=equipmentService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return equipmentReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			equipment = equipmentService.getEquipment(this.equipment.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,equipment.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/equipManage/equipment/mainView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainView.ftl") }),
		@Action(value = "/MESBasic/equipManage/equipment/mainViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "mainViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/equipManage/equipment/list", results = { @Result(type = FREEMARKER, location = "list.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/MESBasic/equipManage/equipment/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = equipmentService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(equipmentService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(equipmentService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_equipManage_Equipment", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/equipManage/equipment/mobileRef", results = { @Result(type = FREEMARKER, location = "mobileRef.ftl") })
		,
		@Action(value = "/MESBasic/equipManage/equipment/ref", results = { @Result(type = FREEMARKER, location = "ref.ftl") })
		,
		@Action(value = "/MESBasic/equipManage/equipment/ref2", results = { @Result(type = FREEMARKER, location = "ref2.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/equipManage/equipment/list-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.eamType.name,result.installPlace.name,result.eamBuyTime,result.eamVendorName,result.eamProduceFirm,result.demo,result.eamType.id,result.eamType.code,result.installPlace.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"equipment\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"equipment\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("eamType.layRec") && request.getParameter("eamType.layRec").trim().length() > 0) {
			params.add(new Param("\"eamType\".EAMTYPE_LAYREC", request.getParameter("eamType.layRec"), Param.EQUAL_LIKELEFT));	
		}
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"eamUseDeptId\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		if (null != request.getParameter("area.layRec") && request.getParameter("area.layRec").trim().length() > 0) {
			params.add(new Param("\"installPlace\".LAY_REC", request.getParameter("area.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"workcenter\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"equipment\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"equipment\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/equipManage/equipment/list-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/equipManage/equipment/mobileRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.eamType.name,result.installPlace.name,result.eamBuyTime,result.eamProduceFirm,result.eamVendorName,result.demo,result.eamType.id,result.eamType.code,result.installPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="mobileRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/equipManage/equipment/ref-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.eamType.name,result.installPlace.name,result.eamBuyTime,result.eamProduceFirm,result.eamVendorName,result.demo,result.eamType.id,result.eamType.code,result.installPlace.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="ref-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/MESBasic/equipManage/equipment/ref2-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.eamBuyTime,result.eamProduceFirm,result.eamVendorName,result.demo,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="ref2-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/equipManage/equipment/allTempleteDownload", results = { @Result(name="excel",type="excel",location="list-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_equipManage_Equipment");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		equipmentService.findDataGridPage(dg,dgclass,getDgPage(),equipment,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(equipment.getId() != null && equipment.getId() != -1){
				equipment = equipmentService.getEquipment(equipment.getId());
			}
			equipmentService.findDataGridPage(dg,dgclass,getDgPage(),equipment,searchCondition,params);
	...
	private void save() throws Exception {
		...
		equipmentService.saveEquipment(equipment,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/equipManage/equipment/edit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/equipManage/equipment/mainView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "submit");
		bap_validate_datagrids=equipmentService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		...
		fileuploads.put("linkId", equipment.getId());
		bapAfterCustomMethod("MESBasic_1_equipManage_Equipment", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/equipManage/equipment/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		equipmentService.deleteEquipment(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_equipManage_Equipment", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/equipManage/equipment/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "restore");
		equipmentService.restoreEquipment(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_equipManage_Equipment", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/equipManage/equipment/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/equipManage/equipment/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "showDealInfoData");
		if (null != tableInfoId) {
			int count=equipmentService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = equipmentService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = equipmentService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_equipManage_Equipment", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicEquipment> findExportDataInfos(Page<MESBasicEquipment> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = equipmentService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = equipmentService.getAssProperty(p.getCode());	
									//String assModelCode = equipmentService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			equipmentService.findEquipments(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			equipmentService.findEquipments(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicEquipment> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicEquipment result : results){
	...
	public String  generteCustomerSql(View view,List<Map> mapDatas)   {
		...
		if(notHasId){
			sb.append(modelAlias + ".EAM_ID AS \"id\", ");
		}
	...
	public boolean isEditNew(){
		//return null == this.equipment || null == this.equipment.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/equipManage/equipment/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_equipManage_Equipment", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/equipManage/equipment/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/equipManage/equipment/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/equipManage/equipment/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicEquipmentService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_equipManage", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_equipManage");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_equipManage_Equipment", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/equipManage/equipment/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicEquipmentService"
						, "com.supcon.orchid.MESBasic.services.MESBasicAreaService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicContainerService"       
						, "com.supcon.orchid.MESBasic.services.MESBasicEamTypeService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_equipManage_Equipment", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_equipManage_Equipment", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/equipManage/equipment/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/equipManage/equipment/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_equipManage_Equipment";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_equipManage_Equipment", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/equipManage/equipment/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		MESBasicEquipment equipment = null;
		if(null != id) {
		...
		_auditLog.setColumnStringB(null == equipment.getCode() ? "" : equipment.getCode().toString());
		_auditLog.setColumnStringA(null == equipment.getName() ? "" : equipment.getName().toString());
		_auditLog.setColumnStringC(equipment.getId().toString());
		_auditLog.setColumnStringD(MESBasicEquipment.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_equipManage");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicEquipmentAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, equipment,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, equipment,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,equipment);
		...
		printlog.setEntity("MESBasic_1_equipManage");
		...
		printlog.setBusinessId(equipment.getId());
		printlog.setBussinessKey(null == equipment.getCode() ? "" : equipment.getCode().toString());
		printlog.setMainDisplay(null == equipment.getName() ? "" : equipment.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/equipManage/equipment/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(equipment,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/equipManage/equipment/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(equipmentService.getEquipment(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/equipManage/equipment/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/equipManage/equipment/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, equipment,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				equipment = equipmentService.getEquipment(Long.parseLong(arrId[i]));
				if(null != equipment){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == equipment.getName() ? "" : "," + equipment.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == equipment.getName() ? "" : equipment.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == equipment.getCode() ? "" : "," + equipment.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == equipment.getCode() ? "" : equipment.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == equipment.getId() ? "" : "," + equipment.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == equipment.getId() ? "" : equipment.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, equipment,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,equipment);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/equipManage/equipment/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicEquipment> findExportPageInfos(Page<MESBasicEquipment> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_equipManage" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = equipmentService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicEquipment queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = equipmentService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = equipmentService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_equipManage_Equipment");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_equipManage_Equipment")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_equipManage_Equipment");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_equipManage_Equipment");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = equipmentService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_equipManage_Equipment,MESBasic_1) */
// 自定义代码
