...
import com.supcon.orchid.MESBasic.services.MESBasicOriginalSignalImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicOriginalSignalDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_originalSignalService")
@Transactional
public class MESBasicOriginalSignalServiceImpl  extends BaseServiceImpl<MESBasicOriginalSignal> implements MESBasicOriginalSignalService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicOriginalSignal> bapBaseService;
	...
	@Autowired
	private MESBasicOriginalSignalImportService originalSignalImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicOriginalSignal.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  originalSignalDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicOriginalSignal.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = originalSignalDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicOriginalSignal.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicOriginalSignal.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  originalSignalDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicOriginalSignal.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicOriginalSignal.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicOriginalSignal> list = (List<MESBasicOriginalSignal>) originalSignalDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicOriginalSignal getOriginalSignal(long id){
		return getOriginalSignal(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicOriginalSignal getOriginalSignal(long id, String viewCode){
		MESBasicOriginalSignal originalSignal = originalSignalDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getOriginalSignalAsJSON(long id, String include){
		MESBasicOriginalSignal originalSignal = originalSignalDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(originalSignal, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.common.delete",operType="3")
	public void deleteOriginalSignal(MESBasicOriginalSignal originalSignal){
		if(null != originalSignal && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicOriginalSignal.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
			AuditUtil.setColumnStringA(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
			AuditUtil.setColumnStringC(originalSignal.getId().toString());
		}
		beforeDeleteOriginalSignal(originalSignal);
		originalSignalDao.delete(originalSignal);
		afterDeleteOriginalSignal(originalSignal);
		originalSignalDao.flush();
		List<MESBasicOriginalSignal> params = new ArrayList<MESBasicOriginalSignal>();
		params.add(originalSignal);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_originalSignal");
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.common.delete",operType="3")
	public void deleteOriginalSignal(List<Long> originalSignalIds){
		deleteOriginalSignal(originalSignalIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.common.delete",operType="3")
	public void deleteOriginalSignal(List<Long> originalSignalIds, String eventTopic) {
		List<MESBasicOriginalSignal> originalSignals = new ArrayList<MESBasicOriginalSignal>();
		for(Long originalSignalId : originalSignalIds){
			MESBasicOriginalSignal originalSignal = getOriginalSignal(originalSignalId);
			originalSignals.add(originalSignal);
			if(null != originalSignal && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicOriginalSignal.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == originalSignal.getId() ? "" : "," + originalSignal.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == originalSignal.getId() ? "" : "," + originalSignal.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == originalSignal.getId() ? "" : "," + originalSignal.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
				}
			}
		}
		
		for(MESBasicOriginalSignal originalSignal : originalSignals){
			beforeDeleteOriginalSignal(originalSignal);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(originalSignals != null && originalSignals.size() > 0 ){
			for(MESBasicOriginalSignal originalSignal : originalSignals){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),originalSignal, originalSignal.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(originalSignalIds != null && originalSignalIds.size() > 0) {
			String hql = "update " + MESBasicOriginalSignal.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = originalSignalDao.createQuery(hql);
			query.setParameterList("ids", originalSignalIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicOriginalSignal originalSignalz : originalSignals){
				originalSignalz.setValid(false);
				List<MESBasicOriginalSignal> params = new ArrayList<MESBasicOriginalSignal>();
				params.add(originalSignalz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_originalSignal");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.common.delete",operType="3")
	public void deleteOriginalSignal(long originalSignalId, int originalSignalVersion){
		this.deleteOriginalSignal(originalSignalId, originalSignalVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.common.delete",operType="3")
	public void deleteOriginalSignal(long originalSignalId, int originalSignalVersion,SignatureLog signatureLog){
		MESBasicOriginalSignal originalSignal = getOriginalSignal(originalSignalId);
		if(originalSignal != null && !originalSignal.isValid()){
		...
		beforeDeleteOriginalSignal(originalSignal);
		originalSignalDao.delete(originalSignalId, originalSignalVersion);
		afterDeleteOriginalSignal(originalSignal);
		originalSignalDao.flush();
		List<MESBasicOriginalSignal> params = new ArrayList<MESBasicOriginalSignal>();
		params.add(originalSignal);
		...
		if(signatureLog != null ) {
			...
			businessKey=originalSignal.getId();
			signatureLog.setTableId(originalSignal.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.common.delete",operType="3")
	public void deleteOriginalSignal(String originalSignalIds){
		this.deleteOriginalSignal(originalSignalIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.common.delete",operType="3")
	public void deleteOriginalSignal(String originalSignalIds,SignatureLog signatureLog){
		deleteCollection(originalSignalIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteOriginalSignal(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_originalSignal#modelCode:MESBasic_1_originalSignal_OriginalSignal#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="ec.engine.restore",operType="10")
	public void restoreOriginalSignal(String originalSignalIds){
		restoreCollection(originalSignalIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreOriginalSignal(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="ec.engine.restore",operType="10")
	public void restoreOriginalSignal(long originalSignalId){
		
		findBusinessKeyUsed(originalSignalId);	//判断业务主键是否重复
		...
		beforeRestoreOriginalSignal(originalSignal);
		originalSignal.setValid(true);
		originalSignalDao.update(originalSignal);
		afterRestoreOriginalSignal(originalSignal);
		List<MESBasicOriginalSignal> params = new ArrayList<MESBasicOriginalSignal>();
		params.add(originalSignal);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long originalSignalId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_originalSignal_OriginalSignal");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicOriginalSignal.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicOriginalSignal.TABLE_NAME +" where id = ? )";
			List<Object> list =  originalSignalDao.createNativeQuery(sql,originalSignalId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseOriginalSignal(List<MESBasicOriginalSignal>  originalSignals){
		for(MESBasicOriginalSignal originalSignal:originalSignals)  {
			saveOriginalSignal(originalSignal, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseOriginalSignal(List<MESBasicOriginalSignal>  originalSignals){
		for(MESBasicOriginalSignal originalSignal:originalSignals)  {
			saveOriginalSignal(originalSignal, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="ec.flow.save")
	public void saveOriginalSignal(MESBasicOriginalSignal originalSignal, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveOriginalSignal(originalSignal, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="ec.flow.save")
	public void saveOriginalSignal(MESBasicOriginalSignal originalSignal, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveOriginalSignal(originalSignal, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateOriginalSignalCodes(MESBasicOriginalSignal originalSignal) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateOriginalSignalCodes(originalSignal, false);
	}
	
	public void generateOriginalSignalCodes(MESBasicOriginalSignal originalSignal, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateOriginalSignalSummarys(MESBasicOriginalSignal originalSignal) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateOriginalSignalSummarys(originalSignal, false);
	}

	public void generateOriginalSignalSummarys(MESBasicOriginalSignal originalSignal, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal")
	public void saveOriginalSignal(MESBasicOriginalSignal originalSignal,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveOriginalSignal(originalSignal, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal")
	public void saveOriginalSignal(MESBasicOriginalSignal originalSignal,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_originalSignal";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_originalSignal");
		String url = null;
		if(originalSignal.getId() != null && originalSignal.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicOriginalSignal.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/mESBasic_1_originalSignal/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicOriginalSignal.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/mESBasic_1_originalSignal/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, originalSignal);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(originalSignal);
		beforeSaveOriginalSignal(originalSignal, viewIsView);

		if (viewIsView) {
			originalSignalDao.saveWithRevertVersion(originalSignal);
		} else {
			if(isNew)
				originalSignalDao.save(originalSignal);
			else
				originalSignalDao.merge(originalSignal);
		}
		if(null != originalSignal && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicOriginalSignal.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
			AuditUtil.setColumnStringA(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
		...
		dealDatagridsSave(originalSignal,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateOriginalSignalCodes(originalSignal, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateOriginalSignalSummarys(originalSignal, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveOriginalSignal(originalSignal, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, originalSignal);
			}
		}
		List<MESBasicOriginalSignal> params = new ArrayList<MESBasicOriginalSignal>();
		originalSignalDao.flush();
		originalSignalDao.clear();
		originalSignal = originalSignalDao.load(originalSignal.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicOriginalSignal originalSignal){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("originalSignal", originalSignal);
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal")
	public void mergeOriginalSignal(MESBasicOriginalSignal originalSignal, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(originalSignal);
		beforeSaveOriginalSignal(originalSignal);
		originalSignalDao.merge(originalSignal);
		if(null != originalSignal && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicOriginalSignal.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
			AuditUtil.setColumnStringA(null == originalSignal.getId() ? "" : originalSignal.getId().toString());
		...
		dealDatagridsSave(originalSignal,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveOriginalSignal(originalSignal);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicOriginalSignal> findOriginalSignals(Page<MESBasicOriginalSignal> page, Criterion... criterions) {
		return originalSignalDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private MESBasicOriginalSignalDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<MESBasicOriginalSignalDealInfo> list=originalSignalDao.createQuery("from " + MESBasicOriginalSignalDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(MESBasicOriginalSignalDealInfo dealInfo) {
		originalSignalDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+MESBasicOriginalSignalDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) originalSignalDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+MESBasicOriginalSignalDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = originalSignalDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = originalSignalDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ MESBasicOriginalSignalDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = originalSignalDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findOriginalSignals(Page<MESBasicOriginalSignal> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findOriginalSignals(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findOriginalSignals(Page<MESBasicOriginalSignal> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findOriginalSignals(Page<MESBasicOriginalSignal> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicOriginalSignal> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"originalSignal\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"originalSignal\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"originalSignal\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_originalSignal_OriginalSignal" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "MESBasic_1_originalSignal_OriginalSignal" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  originalSignalDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  originalSignalDao.createNativeQuery(powerOperateSql, "MESBasic_1_originalSignal").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"originalSignal\"","MESBasic_1_originalSignal",customCondition,"MESBASIC_ORIGINAL_SIGNALS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"originalSignal\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+MESBasicOriginalSignalDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"originalSignal\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"originalSignal\".LAY_REC ASC, \"originalSignal\".SORT ASC");
				} else {
					orderPart.append(" \"originalSignal\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) originalSignalDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=originalSignalDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = originalSignalDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicOriginalSignal> result = new ArrayList<MESBasicOriginalSignal>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicOriginalSignal.class,originalSignalDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicOriginalSignal.class,originalSignalDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicOriginalSignal originalSignal : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(originalSignal.getId(), "MESBasic_originalSignal_originalSignal");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(originalSignal.getId(), "MESBasic_originalSignal_originalSignal");
							if(null != documents && !documents.isEmpty()) {
								originalSignal.setDocument(documents.get(0));
								originalSignal.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("MESBasic_1_originalSignal")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("MESBasic_1_originalSignal")+"MESBasic_1_originalSignal".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("MESBasic_1_originalSignal",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicOriginalSignal> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicOriginalSignal> result) {
		View view = null;
		...
		Map<Long, MESBasicOriginalSignal> rsMap = new HashMap<Long, MESBasicOriginalSignal>();
		for (MESBasicOriginalSignal originalSignal : result) {
			ids.add(originalSignal.getId());
			rsMap.put(originalSignal.getId(), originalSignal);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(originalSignalDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = originalSignalDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(MESBasicOriginalSignal originalSignal,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(originalSignal.getOwnerStaffId());
		ti.setOwnerPositionId(originalSignal.getOwnerPositionId());
		ti.setOwnerDepartmentId(originalSignal.getOwnerDepartmentId());
		
		ti.setCreateStaff(originalSignal.getCreateStaff());
		ti.setCreateStaffId(originalSignal.getCreateStaffId());
		ti.setCreateTime(originalSignal.getCreateTime());
		ti.setDeleteStaff(originalSignal.getDeleteStaff());
		ti.setDeleteStaffId(originalSignal.getDeleteStaffId());
		ti.setDeleteTime(originalSignal.getDeleteTime());
		ti.setCreateDepartmentId(originalSignal.getCreateDepartmentId());
		ti.setModifyStaff(originalSignal.getModifyStaff());
		ti.setModifyStaffId(originalSignal.getModifyStaffId());
		ti.setCreatePositionId(originalSignal.getCreatePositionId());
		ti.setPositionLayRec(originalSignal.getPositionLayRec());
		ti.setTableNo(originalSignal.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("MESBasic_1_originalSignal");
		ti.setTargetTableName("MESBASIC_ORIGINAL_SIGNALS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="MESBasic_1_originalSignal_OriginalSignal" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  originalSignalDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"originalSignal\"","MESBasic_1_originalSignal","","MESBASIC_ORIGINAL_SIGNALS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"originalSignal\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"originalSignal\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"originalSignal\"","MESBasic_1_originalSignal","","MESBASIC_ORIGINAL_SIGNALS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("originalSignal.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"originalSignal\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"originalSignal\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"originalSignal\".CID = 1 OR \"originalSignal\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("originalSignal.mc\"").append(".ORIGINAL_SIGNAL) not in ("+notinValue+")");
			}
			
			SQLQuery query = originalSignalDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("originalSignal", "MESBasic_1_originalSignal", "originalSignal_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=originalSignalDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicOriginalSignal.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicOriginalSignal.MODULE_CODE;}
	protected String getModelCode(){return MESBasicOriginalSignal.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param originalSignal 模型实例
	 */
	public void checkUniqueConstraint(MESBasicOriginalSignal originalSignal){
	}
	
	
	@Override
	public List<MESBasicOriginalSignal> getOriginalSignals(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicOriginalSignal.class);
		if(params!=null){
			...
			return originalSignalDao.findByHql("from " + MESBasicOriginalSignal.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return originalSignalDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<MESBasicOriginalSignal> findOriginalSignalsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicOriginalSignal.class);
	...
	@Override
	public List<MESBasicOriginalSignal> findOriginalSignalsByHql(String hql, Object... objects){
		
		return   originalSignalDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicOriginalSignal loadOriginalSignalByBussinessKey(MESBasicOriginalSignal originalSignal){
		return loadOriginalSignalByBussinessKey(originalSignal.getId());
	}
	
	@Override
	public Page<MESBasicOriginalSignal> getOriginalSignals(Page<MESBasicOriginalSignal> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicOriginalSignal.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = originalSignalDao.findByPage(page, sql);
			}else{
				page = originalSignalDao.findAllByPage(page);
			}
			return page;
		}
		page = originalSignalDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicOriginalSignal loadOriginalSignalByBussinessKey(Serializable bussinessKey){
		return originalSignalDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicOriginalSignal> findByProperty(String propertyName, Object object){
		return originalSignalDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicOriginalSignal findEntityByProperty(String propertyName, Object object){
		return originalSignalDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicOriginalSignal.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = originalSignalDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_originalSignal_OriginalSignal","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("MESBasic_1_originalSignal", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(MESBasicOriginalSignal originalSignal,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,originalSignalDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicOriginalSignal.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, originalSignal);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",originalSignal.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicOriginalSignal.TABLE_NAME +" where valid = 1";
			list = originalSignalDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicOriginalSignal.TABLE_NAME;
			list = originalSignalDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicOriginalSignal.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = originalSignalDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicOriginalSignalDaoImpl originalSignalDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchOriginalSignal(final List<MESBasicOriginalSignal> insertObjs, final List<MESBasicOriginalSignal> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchOriginalSignal(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchOriginalSignal(final List<MESBasicOriginalSignal> insertObjs, final List<MESBasicOriginalSignal> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1556016262160"),importCount));
		...
		Map<Long, MESBasicOriginalSignal> oldUpdateObjsMap = new HashMap<Long, MESBasicOriginalSignal>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicOriginalSignal bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicOriginalSignal> bizObjList = originalSignalDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicOriginalSignal bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicOriginalSignal> nmeObjs = new ArrayList<MESBasicOriginalSignal>();
		...
		long id=autoGeneratorID.getNextId(MESBasicOriginalSignal.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicOriginalSignal originalSignal:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicOriginalSignal.TABLE_NAME,1,dbId);
			if(originalSignal.getId() != null ){
				m.put(originalSignal.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			originalSignal.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) originalSignalDao.createNativeQuery(sql, "MESBasic_1_originalSignal_OriginalSignal"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		originalSignalDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicOriginalSignal.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicOriginalSignal originalSignal = updateObjs.get(updateIndex);
						Long id = originalSignal.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = originalSignal.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(originalSignal);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicOriginalSignal.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicOriginalSignal.TABLE_NAME + " order by id desc";
//					Query query = originalSignalDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicOriginalSignal originalSignal = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = originalSignal.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		originalSignalDaoImpl.getSession().flush();
		originalSignalDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicOriginalSignal bizObj = insertObjs.get(insertIndex);
			originalSignalImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicOriginalSignal bizObj = updateObjs.get(updateIndex);
			MESBasicOriginalSignal oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			originalSignalImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = originalSignalDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				originalSignalDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = originalSignalDao.createNativeQuery(sql, "MESBasic_1_originalSignal_OriginalSignal").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = originalSignalDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = originalSignalDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = originalSignalDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?originalSignalDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  originalSignalDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,originalSignalDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="ec.flow.save")
	public void saveOriginalSignal(MESBasicOriginalSignal originalSignal, DataGridService dataGridService){
		saveOriginalSignal(originalSignal, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="ec.flow.save")
	public void saveOriginalSignal(MESBasicOriginalSignal originalSignal, DataGridService dataGridService, String viewCode){
		saveOriginalSignal(originalSignal, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal")
	public void saveOriginalSignal(MESBasicOriginalSignal originalSignal, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveOriginalSignal(originalSignal, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal")
	public void saveOriginalSignal(MESBasicOriginalSignal originalSignal, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_originalSignal";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_originalSignal");
		String url = null;
		if(null != originalSignal.getId() && originalSignal.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicOriginalSignal.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal")
	public Map<Object, Object> originalSignalDataGridImport(MESBasicOriginalSignal originalSignal, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = originalSignal.getVirtualId();
		
		originalSignal = originalSignalDao.load(originalSignal.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, originalSignal.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = originalSignal.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(originalSignal);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, originalSignal.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal")
	public void mergeOriginalSignal(MESBasicOriginalSignal originalSignal, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(originalSignal);
		beforeSaveOriginalSignal(originalSignal);
		originalSignalDao.merge(originalSignal);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_originalSignal",model="MESBasic_1_originalSignal_OriginalSignal",desc="foundation.batch.save",operType="4")
	public void batchSaveOriginalSignal(MESBasicOriginalSignal originalSignal, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "MESBasic_1_originalSignal";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_originalSignal");
	...
	@Override
	public Page<MESBasicOriginalSignal> getByPage(Page<MESBasicOriginalSignal> page,DetachedCriteria detachedCriteria){
		return originalSignalDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("MESBasic_1_originalSignal").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveOriginalSignal(MESBasicOriginalSignal originalSignal, Object... objects){
		checkUniqueConstraint(originalSignal);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveOriginalSignal(MESBasicOriginalSignal originalSignal, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportOriginalSignal(List<MESBasicOriginalSignal> originalSignals, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportOriginalSignal(List<MESBasicOriginalSignal> originalSignals){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportOriginalSignal(List<MESBasicOriginalSignal> originalSignals, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportOriginalSignal(List<MESBasicOriginalSignal> originalSignals){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteOriginalSignal(MESBasicOriginalSignal originalSignal){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteOriginalSignal(MESBasicOriginalSignal originalSignal){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreOriginalSignal(MESBasicOriginalSignal originalSignal){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreOriginalSignal(MESBasicOriginalSignal originalSignal){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicOriginalSignal originalSignal, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicOriginalSignal originalSignal, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportOriginalSignal(List<MESBasicOriginalSignal> insertObjs, List<MESBasicOriginalSignal> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportOriginalSignal,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportOriginalSignal(List<MESBasicOriginalSignal> insertObjs, List<MESBasicOriginalSignal> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportOriginalSignal,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_originalSignal_OriginalSignal,MESBasic_1) */
// 自定义代码
